...
function sourceState(name, result, detail = null) {
  if (result.status === "fulfilled" && !result.value.error) return { name, status: "verified", detail };
  const error = result.status === "rejected" ? result.reason?.message : result.value?.error?.message;
  return { name, status: "unverified", detail: error || detail || "Read unavailable" };
}
...
function unavailableReleaseHistory(source, error = null) {
  return { status: "unverified", source, checkedAt: new Date().toISOString(), deployments: [], error };
}
...
function normalizedEventRows(systemAlerts, securityIncidents) {
  ...
  return [...alerts, ...incidents].sort((left, right) => new Date(right.created_at || 0).getTime() - new Date(left.created_at || 0).getTime());
}
...
async function loadPlatformAdminConsole() {
  ...
  if (!access.success) {
    return {
      access,
      organizations: [], recentEvents: [], modules: [], staff: [], recentUsage: [], wallets: [], walletTransactions: [], providers: [], subscriptions: [], queueJobs: [], deadLetterJobs: [],
      operatorControl: { status: "review", counts: { critical: 0, high: 0, medium: 0, low: 0, total: 0, resolved: 0 }, coverage: { verified: 0, total: 0, sources: [] }, signals: [] },
      releaseState: releaseState(),
      releaseHistory: unavailableReleaseHistory("PLATFORM_ADMIN_ACCESS_REQUIRED"),
      health: { status: "degraded", runtime_state: "unverified", timestamp: new Date().toISOString(), duration_ms: 0, services: {} },
    };
  }
  ...
  const [
    ...
    supabaseAdmin.from("system_alerts").select("id,organization_id,alert_type,severity,title,message,status,created_at,resolved_at").is("resolved_at", null).order("created_at", { ascending: false }).limit(250),
    supabaseAdmin.from("enterprise_security_incidents").select("id,organization_id,incident_type,severity,incident_status,incident_summary,created_at,updated_at,resolved_at").is("resolved_at", null).order("created_at", { ascending: false }).limit(250),
    ...
    supabaseAdmin.from("wallet_transactions").select("*").order("created_at", { ascending: false }).limit(1000),
    supabaseAdmin.from("subscriptions").select("id,organization_id,company,email,currency,billing_cycle,subtotal,discount_total,final_monthly_total,final_yearly_total,selected_modules,status,created_at,updated_at").order("created_at", { ascending: false }).limit(500),
    supabaseAdmin.from("queue_jobs").select("id,organization_id,type,priority,status,error_message,retry_count,max_retries,worker_name,locked_at,started_at,completed_at,scheduled_for,dead_letter,created_at").order("created_at", { ascending: false }).limit(500),
    supabaseAdmin.from("dead_letter_jobs").select("id,original_job_id,type,failed_at").order("failed_at", { ascending: false }).limit(500),
    checkSystemHealth(),
    loadVercelDeploymentHistory(),
  ]);

  const health = healthResult.status === "fulfilled" ? healthResult.value : { status: "degraded", runtime_state: "unverified", timestamp: new Date().toISOString(), duration_ms: 0, services: {} };
  const releaseHistory = releaseHistoryResult.status === "fulfilled"
    ? releaseHistoryResult.value
    : unavailableReleaseHistory("VERCEL_DEPLOYMENT_HISTORY_PROMISE_REJECTED", releaseHistoryResult.reason?.message || "Deployment history unavailable");
  ...
  const usageFailureGroups = rowsFrom(usageFailureGroupsResult);
  const operatorCases = rowsFrom(operatorCasesResult);
  const wallets = rowsFrom(walletsResult);
  const exactFailureCount = usageFailureGroups.reduce((sum, row) => sum + Number(row.occurrence_count || 0), 0);
  ...
  const coverage = [
    ...
    sourceState("Failure fingerprints", usageFailureGroupsResult, `${usageFailureGroups.length} fingerprints · ${exactFailureCount} failed executions / 24h`),
    sourceState("Operator cases", operatorCasesResult, `${operatorCases.length} governed case records`),
    ...
    { name: "Runtime health", status: healthResult.status === "fulfilled" ? "verified" : "unverified", detail: healthResult.status === "fulfilled" ? `state ${health.status}` : healthResult.reason?.message || "Health probe unavailable" },
    { name: "Deployment history", status: releaseHistory.status === "verified" ? "verified" : "unverified", detail: releaseHistory.status === "verified" ? `${releaseHistory.deployments?.length || 0} production deployments` : releaseHistory.source },
  ];
  ...
  return {
    ...
    releaseHistory,
    operatorControl,
    providers: Object.values(PROVIDER_REGISTRY).map(provider => ({ id: provider.id, name: provider.name, category: provider.category, connectionModel: provider.connectionModel || "managed", capabilities: provider.capabilities || [], runtime: provider.runtime || null, runtimeAvailable: provider.runtimeAvailable === true, active: provider.active !== false })),
    health,
  };
}
    ...
    <div className="-mx-5 -my-5 min-h-[calc(100vh-61px)] bg-[#F7F6F3] px-5 py-10 text-[#2A2723] lg:-mx-7 lg:-my-6 lg:px-7">
      <div className="mx-auto max-w-3xl overflow-hidden rounded-[22px] border border-[#A37849]/14 bg-[#FFFDF9]">
        <div className="border-b border-black/[0.06] px-5 py-4">
          <div className="text-[8px] font-semibold uppercase tracking-[0.14em] text-[#8A633C]">Avantiqo Platform</div>
          <h1 className="mt-1 text-[20px] font-semibold tracking-[-0.03em] text-[#27231F]">Platform administrator access required</h1>
          <p className="mt-1 max-w-2xl text-[9px] leading-5 text-[#918B83]">This workspace is restricted to active PLATFORM_OWNER and SUPER_ADMIN staff accounts.</p>
        </div>
        <div className="px-5 py-4"><div className="rounded-xl border border-black/[0.07] bg-[#F6F4F0] px-4 py-3 text-[9px] text-[#746E66]">{status || 403} · {error || "Access denied"}</div></div>
      </div>
    </div>
  );
}
