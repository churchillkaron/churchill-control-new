        ...
        <section className="rounded-[32px] border border-black/[0.075] bg-white p-6">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-[#8A847C]">Only work assigned to you for the active organization and business day appears here. Location proof is required only when the assignment or protocol needs it, and completion protocols are enforced server-side.</p>
            </div>
                ...
                <article key={job.id} className={`rounded-[26px] border p-4 sm:rounded-[30px] sm:p-5 lg:p-6 ${job.id === nextJobId ? "border-[#D6A66A]/30 bg-[#D6A66A]/[0.045]" : "border-black/[0.075] bg-white"}`}>
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-3 text-[9px] font-black uppercase tracking-[0.14em] text-[#AAA49C]">{job.subjectLabel || "Work"}</div>
                      <h2 className="mt-1 text-xl font-black tracking-[-0.02em] sm:text-2xl">{job.serviceName}</h2>
                      <div className="mt-1 text-sm text-[#79736B]">{job.subjectName || job.customerName}</div>
