...
function currencyMinorUnitFactor(currency) {
  if (!currency) throw new Error("Wallet currency is not configured");
...
function Field({ label, hint, children }) {
  return (
    <label className="block">
      <span className="mb-2 block text-[11px] uppercase tracking-[0.2em] text-white/40">{label}</span>
      {children}
...
export default function MetaAdsPage() {
  ...
  async function loadReadiness() {
    ...
    try {
      ...
      const availableChannels = (data?.delivery_channels || [])
        .filter((item) => item.available)
        .map((item) => item.id);
      setReadiness(data);
      setForm((current) => ({
        ...current,
        country: current.country || data?.default_country || "",
        linkUrl: current.linkUrl || data?.default_destination_url || "",
        deliveryChannels: current.deliveryChannels.filter((channel) =>
          availableChannels.includes(channel)
        ).length
          ? current.deliveryChannels.filter((channel) => availableChannels.includes(channel))
  ...
  function validateStep(targetStep = step) {
    ...
    if (targetStep >= 2 && !form.deliveryChannels.length) throw new Error("Choose at least one available delivery channel");
    if (targetStep >= 3) {
      if (!walletCurrency) throw new Error("The organization wallet currency is not configured");
      if (!/^[A-Za-z]{2}$/.test(form.country.trim())) throw new Error("Enter a valid two-letter audience country code");
      ...
      if (Number(form.ageMin) < 18 || Number(form.ageMax) < Number(form.ageMin)) {
        throw new Error("Check the audience age range");
      }
    }
    if (targetStep >= 4) {
      ...
      if (form.destination === "WEBSITE" && !form.linkUrl.trim()) {
  ...
  async function submit() {
    ...
    try {
      ...
      const factor = currencyMinorUnitFactor(walletCurrency);
      const response = await fetch("/api/marketing/meta-ads", {
        ...
        body: JSON.stringify({
          ...
          adSet: {
            ...
            lifetime_budget: Math.round(budget * factor),
            targeting: {
        ...
        <header className="mb-8 flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-white/42">
              Create Facebook, Instagram and supported message campaigns through Avantiqo. One prepaid wallet, exact approved creative, and no separate provider billing setup.
            </p>
          </div>
