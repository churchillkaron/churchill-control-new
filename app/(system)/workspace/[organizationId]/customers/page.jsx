  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3] p-8 text-[#191919]">
      ...
      <input
        ...
        className="mb-6 w-full rounded-xl border border-black/[0.09] bg-white px-4 py-3"
      />
        ...
        {loading ? "Loading..." :
          customers.map(customer => (
            <div
              ...
              className={`cursor-pointer rounded-xl border border-black/[0.08] bg-white p-4 transition hover:border-[#D6A66A]/60 ${selectedCustomer?.id === customer.id ? "border-[#D6A66A] bg-[#FBF3E8]" : ""}`}
            >
              <div className="text-lg font-semibold">{customer.customer_name}</div>
              <div className="text-[#746E66]">{customer.customer_phone}</div>
      ...
      {selectedCustomer && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="p-4 rounded-xl border border-black/[0.08] bg-white flex flex-col items-start gap-2">
          ...
          <div className="mb-4 flex gap-4">
            {["profile","orders","reviews","support"].map(tab => (
              <button
                ...
                className={`px-4 py-2 rounded-xl ${activeTab===tab ? "bg-[#D6A66A] text-[#191919]" : "bg-white border border-black/[0.08] text-[#746E66]"}`}
              >
          ...
          <div>
            {activeTab==="profile" && (
              <div className="rounded-2xl border border-[#D6A66A]/35 bg-[#FBF8F3] p-6 space-y-3">
                <input
                  value={selectedCustomer.customer_name || ""}
                  onChange={e => setSelectedCustomer({...selectedCustomer, customer_name: e.target.value})}
                  className="w-full rounded-xl border border-black/[0.09] bg-white px-3 py-2"
                />
                <input
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-[#D6A66A] text-[#191919]"
                >
            ...
            {activeTab==="orders" && (
              <div className="rounded-xl border border-black/[0.08] bg-white p-4 text-[#746E66] max-h-[500px] overflow-y-auto">
                {orderHistory.map(order => (
                  <div key={order.id} className="mb-4 rounded-xl border border-black/[0.08] bg-[#FBF8F3] p-4">
            ...
            {activeTab==="reviews" && (
              <div className="rounded-xl border border-black/[0.08] bg-white p-4 text-[#746E66]">
