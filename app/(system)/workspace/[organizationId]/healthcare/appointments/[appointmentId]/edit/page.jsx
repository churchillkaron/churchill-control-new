...
export default function EditAppointmentPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl p-8">
      ...
      <form className="space-y-4 rounded-3xl border border-black/[0.08] bg-[#FBF8F3] p-8" onSubmit={save}>
        <input
          type="text"
          value={form.patient_id || ""}
          onChange={(e) => setForm({ ...form, patient_id: e.target.value })}
          className="w-full rounded-xl border border-black/[0.08] bg-transparent p-3"
        />
        ...
        <button type="submit" className="rounded-2xl bg-amber-600 px-6 py-3 text-[#191919]">Save Changes</button>
      </form>
    </main>
  );
}
