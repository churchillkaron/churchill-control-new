...
export default function HotelChannelManagerPage() {
  ...
  const loadProperty = useCallback(async () => {
    ...
    try {
      ...
      const [channelPayload, ratePayload] = await Promise.all([
        api(`/api/hotel/channels?${query}`),
        api(`/api/hotel/rates?${query}`),
      ]);
  ...
  const connected = providers.filter((provider) => provider.connection?.status === "ACTIVE").length;
  const setupPending = providers.filter((provider) => provider.connection && provider.connection.status !== "ACTIVE").length;
  const syncErrors = providers.filter((provider) => provider.connection?.last_error).length;
  ...
  async function createRatePlan() {
    ...
    try {
      const payload = await api("/api/hotel/rate-plans", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ organizationId, propertyId, name: planName, currencyCode: "THB" }),
      });
  ...
  async function saveRates() {
    ...
    if (!ratePlanId || !roomType || !dates.length) {
      setError("Choose a rate plan, room type and valid date range.");
      return;
    }
    setSaving(true); setError(""); setSuccess("");
    try {
      const payload = await api("/api/hotel/rates", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          organizationId, propertyId, ratePlanId, roomType,
          entries: dates.map((stayDate) => ({
            stayDate,
            rateAmount: Number(rateAmount || 0),
            inventory: inventory === "" ? null : Number(inventory),
            minStay: Number(minStay || 1),
            maxStay: maxStay === "" ? null : Number(maxStay),
  ...
  async function beginChannelSetup(providerId) {
    ...
    setSaving(true); setError(""); setSuccess("");
    try {
      const payload = await api("/api/hotel/channels", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ organizationId, propertyId, provider: providerId }),
      });
      setSuccess(`${payload.connection.display_name} setup opened. ${payload.onboarding}. Avantiqo will not claim the channel is live until certified connectivity is configured.`);
      await loadProperty();
    } catch (reason) { setError(reason.message); } finally { setSaving(false); }
  }
  ...
  return (
    <HotelWorkspaceShell
      ...
      subtitle="Set rates, inventory and stay restrictions once in Avantiqo, then distribute the governed state to connected booking channels. No separate extranets for ordinary daily rate changes."
      context={selectedProperty?.name || "Choose property"}
      actions={<HotelSecondaryAction href={`/workspace/${organizationId}/operations/hotel-setup`}>Hotel setup</HotelSecondaryAction>}
    >
      <HotelError>{error}</HotelError>
      <HotelSuccess>{success}</HotelSuccess>

      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <HotelMetric label="Live channels" value={connected} detail={`${setupPending} awaiting setup or certification`} />
        <HotelMetric label="Room types" value={roomTypes.length} detail={`${rooms.length} physical rooms in selected property`} />
        <HotelMetric label="Sync exceptions" value={syncErrors} detail="Provider errors requiring human attention" attention={syncErrors > 0} />
        <HotelMetric label="Stop-sell days" value={closedDates} detail={`${from} to ${to}`} attention={closedDates > 0} />
      </div>

      <HotelSection eyebrow="01 · Property" title="One commercial source of truth" detail="All channel pricing begins with the selected property. Hotels.com is controlled through Expedia Group connectivity where supported.">
        <div className="grid gap-3 p-4 md:grid-cols-[minmax(220px,360px)_1fr] md:p-5">
          <HotelField label="Property">
            <select className={hotelInputClass} value={propertyId} onChange={(event) => { setPropertyId(event.target.value); setRatePlanId(""); setRoomType(""); }}>
              <option value="">Choose property</option>
              {properties.map((property) => <option key={property.id} value={property.id}>{property.name}</option>)}
            </select>
          </HotelField>
          <div className="rounded-xl border border-black/[0.06] bg-[#FBFAF7] px-3 py-2.5 text-[8px] leading-4 text-[#817B73]">
            Avantiqo remains the master rate/inventory record. OTA portals are distribution endpoints. A provider only becomes <strong className="text-[#4A453F]">Active</strong> after official connectivity credentials, property mapping and provider onboarding are complete.
          </div>
        </div>
      </HotelSection>

      <HotelSection eyebrow="02 · Channels" title="Booking sites" detail="Connect each distribution network once. Daily pricing work stays inside Avantiqo.">
        {!propertyId ? <HotelEmptyState>Create or choose a hotel property first.</HotelEmptyState> : (
          <div className="divide-y divide-black/[0.05]">
            {providers.map((provider) => {
              const connection = provider.connection;
              const status = connection?.status || "NOT_CONNECTED";
              return (
                <div key={provider.id} className="grid gap-3 px-4 py-3.5 md:grid-cols-[minmax(180px,1fr)_minmax(220px,1.4fr)_140px_auto] md:items-center md:px-5">
                  <div>
                    <div className="text-[10px] font-semibold text-[#39342F]">{provider.name}</div>
                    <div className="mt-0.5 text-[7px] text-[#989188]">{provider.network}</div>
                  </div>
                  <div className="text-[8px] leading-4 text-[#817B73]">
                    {connection?.last_error ? <span className="text-red-700">{connection.last_error}</span> : connection?.last_success_at ? `Last successful sync ${new Date(connection.last_success_at).toLocaleString()}` : provider.onboarding}
                  </div>
                  <HotelStatusPill value={status} tone={status === "ACTIVE" ? "good" : status === "NOT_CONNECTED" ? "neutral" : "warning"} />
                  <HotelSecondaryAction disabled={saving || !propertyId} onClick={() => beginChannelSetup(provider.id)}>{connection ? "Review setup" : "Connect"}</HotelSecondaryAction>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(310px,0.5fr)]">
        <HotelSection eyebrow="03 · Rates & inventory" title="Change once, distribute everywhere" detail="Bulk-edit a date range. Rate, inventory and restrictions are stored canonically before any provider receives them.">
          <div className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4 md:p-5">
            ...
            <HotelField label="Rate plan">
              <select className={hotelInputClass} value={ratePlanId} onChange={(e) => setRatePlanId(e.target.value)}>
                <option value="">Choose plan</option>{ratePlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}
              </select>
            </HotelField>
            <HotelField label="Room type">
              <select className={hotelInputClass} value={roomType} onChange={(e) => { const next = e.target.value; setRoomType(next); const base = rooms.find((room) => room.room_type === next)?.base_rate; if (base !== undefined) setRateAmount(String(base)); }}>
                <option value="">Choose type</option>{roomTypes.map((type) => <option key={type} value={type}>{type}</option>)}
              </select>
            </HotelField>
          ...
          <div className="flex flex-wrap gap-2 border-t border-black/[0.05] px-4 py-3 md:px-5">
            {[["Stop sell", stopSell, setStopSell], ["Closed to arrival", closedToArrival, setClosedToArrival], ["Closed to departure", closedToDeparture, setClosedToDeparture]].map(([label, checked, setter]) => (
              <label key={label} className="flex items-center gap-2 rounded-lg border border-black/[0.07] bg-[#FBFAF7] px-2.5 py-2 text-[8px] font-medium text-[#69635D]">
        ...
        <HotelSection eyebrow="Rate plans" title="Commercial products" detail="Create the sellable plan once, then price it by room type and date.">
          <div className="space-y-3 p-4 md:p-5">
            ...
            <div className="space-y-1.5 pt-1">
              {ratePlans.map((plan) => <div key={plan.id} className="flex items-center justify-between rounded-lg border border-black/[0.06] px-2.5 py-2"><div><div className="text-[8px] font-semibold text-[#4B4640]">{plan.name}</div><div className="text-[7px] text-[#A09A92]">{plan.code} · {plan.currency_code}</div></div><HotelStatusPill value={plan.active ? "ACTIVE" : "INACTIVE"} tone={plan.active ? "good" : "neutral"} /></div>)}
              {!ratePlans.length && !loading ? <div className="text-[8px] text-[#989188]">No rate plans yet.</div> : null}
            </div>
          </div>
        </HotelSection>
      </div>
      ...
      <HotelSection eyebrow="04 · Published control" title="Current governed values" detail={`${calendar.length} date/room/rate records loaded for the selected window.`}>
        {!calendar.length ? <HotelEmptyState>No explicit rate-calendar overrides in this date range yet.</HotelEmptyState> : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[780px] border-collapse text-left text-[8px]">
              <thead className="bg-[#FAF9F6] text-[7px] uppercase tracking-[0.08em] text-[#918B83]"><tr><th className="px-4 py-2.5">Date</th><th className="px-3">Room type</th><th className="px-3">Rate</th><th className="px-3">Inventory</th><th className="px-3">Min stay</th><th className="px-3">Restrictions</th></tr></thead>
              <tbody className="divide-y divide-black/[0.05]">
                {calendar.slice(0, 200).map((row) => <tr key={row.id}><td className="px-4 py-2.5 font-medium text-[#4A453F]">{row.stay_date}</td><td className="px-3">{row.room_type}</td><td className="px-3 font-semibold">{money(row.rate_amount, selectedPlan?.currency_code || "THB")}</td><td className="px-3">{row.inventory ?? "Derived"}</td><td className="px-3">{row.min_stay}</td><td className="px-3"><div className="flex flex-wrap gap-1">{row.stop_sell ? <HotelStatusPill value="STOP SELL" tone="critical" /> : null}{row.closed_to_arrival ? <HotelStatusPill value="CTA" tone="warning" /> : null}{row.closed_to_departure ? <HotelStatusPill value="CTD" tone="warning" /> : null}{!row.stop_sell && !row.closed_to_arrival && !row.closed_to_departure ? <HotelStatusPill value="OPEN" tone="good" /> : null}</div></td></tr>)}
              </tbody>
            </table>
          </div>
        )}
