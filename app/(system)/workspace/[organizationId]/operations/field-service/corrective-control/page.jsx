...
function severityTone(value) {
  ...
  return "border-black/[0.08] bg-[#F7F6F3]/[0.025] text-[#777169]";
}
...
export default function CorrectiveServiceControlPage() {
  ...
  return <main className="min-h-[calc(100vh-61px)] bg-[#F7F6F3] px-4 py-5 text-[#201E1B] md:px-7 lg:px-9 lg:py-7"><div className="mx-auto max-w-[1580px]">
    ...
    <section className="mt-5 rounded-2xl border border-black/[0.07] bg-white p-2.5"><div className="flex flex-wrap gap-1">{[["open", "Open", state.counts.open || 0], ["overdue", "Overdue", state.counts.overdue || 0], ["resolved", "Resolved", state.counts.resolved || 0], ["all", "All", state.counts.total || 0]].map(([id, label, count]) => <button key={id} onClick={() => setTab(id)} className={`rounded-xl px-3.5 py-2 text-[9px] ${tab === id ? "bg-[#2E2A25] text-[#191919]" : "text-[#746D65]"}`}>{label}<span className="ml-1.5 opacity-60">{count}</span></button>)}</div></section>

    <section className="mt-4 overflow-hidden rounded-2xl border border-black/[0.07] bg-white"><div className="hidden gap-3 border-b border-black/[0.06] bg-[#FBFAF8] px-5 py-3 text-[8px] uppercase tracking-[0.08em] text-[#948D84] md:grid md:grid-cols-[minmax(0,1.5fr)_120px_160px_150px_190px]"><span>Customer / issue</span><span>Severity</span><span>Stage</span><span>Target</span><span>Next action</span></div><div className="divide-y divide-black/[0.05]">{rows.map((row) => { const action = actionFor(row); return <div key={row.work_request_id} className={`grid gap-3 px-4 py-4 text-[9px] md:grid-cols-[minmax(0,1.5fr)_120px_160px_150px_190px] md:px-5 ${row.overdue ? "bg-[#B7654C]/[0.025]" : ""}`}><div className="min-w-0"><div className="flex items-center gap-2"><span className="truncate text-[11px] font-medium">{row.customer_name || "Customer"}</span>{row.overdue ? <span className="rounded-full border border-[#B7654C]/20 bg-[#B7654C]/[0.06] px-2 py-0.5 text-[7px] uppercase text-[#914B38]">overdue</span> : null}</div><div className="mt-1 truncate text-[8px] text-[#817A72]">{row.customer_location_name || "Site not named"} · {row.service_name}</div><div className="mt-2 line-clamp-2 text-[9px] leading-4 text-[#5F5952]">{row.summary || "No issue summary recorded."}</div>{row.next_action ? <div className="mt-1 text-[8px] text-[#9A744B]">Next: {row.next_action.replaceAll("_", " ")}</div> : null}</div><div><div className="mb-1 text-[7px] uppercase tracking-[0.08em] text-[#A39C94] md:hidden">Severity</div><span className={`inline-flex rounded-full border px-2 py-1 text-[7px] uppercase tracking-[0.07em] ${severityTone(row.severity)}`}>{row.severity}</span></div><div><div className="mb-1 text-[7px] uppercase tracking-[0.08em] text-[#A39C94] md:hidden">Stage</div><span className={`inline-flex rounded-full border px-2 py-1 text-[7px] uppercase tracking-[0.07em] ${stageTone(row.stage)}`}>{stageLabel(row.stage)}</span>{row.corrective_assigned_to ? <div className="mt-2 flex items-center gap-1 text-[8px] text-[#817A72]"><UserRound size={8} /> Assigned</div> : null}</div><div><div className="mb-1 text-[7px] uppercase tracking-[0.08em] text-[#A39C94] md:hidden">Target</div><div className={`flex items-center gap-1 text-[8px] ${row.overdue ? "text-[#914B38]" : "text-[#817A72]"}`}><Clock3 size={9} />{formatDate(row.sla_due_at)}</div>{row.resolved_at ? <div className="mt-2 flex items-center gap-1 text-[8px] text-[#607057]"><CheckCircle2 size={9} />{formatDate(row.resolved_at)}</div> : null}</div><div><div className="mb-1 text-[7px] uppercase tracking-[0.08em] text-[#A39C94] md:hidden">Next action</div><div className="flex items-center md:justify-end">{action?.href ? <Link href={action.href} className="inline-flex items-center gap-1.5 rounded-lg bg-[#2C2925] px-3 py-2 text-[8px] font-medium text-[#191919]"><Wrench size={9} />{action.label}</Link> : action?.disabled ? <span className="inline-flex items-center gap-1.5 text-[8px] text-[#607057]"><CheckCircle2 size={10} />Resolved</span> : action ? <button disabled={Boolean(busy)} onClick={action.action} className="rounded-lg bg-[#2C2925] px-3 py-2 text-[8px] font-medium text-[#191919] disabled:opacity-35">{busy.startsWith(row.work_request_id) ? "Working…" : action.label}</button> : <span className="inline-flex items-center gap-1 text-[8px] text-[#98513D]"><AlertTriangle size={9} />Review state</span>}</div></div></div>; })}{!state.loading && rows.length === 0 ? <div className="p-8 text-center md:p-12"><CheckCircle2 className="mx-auto text-[#748267]" size={20} /><div className="mt-2 text-[11px] font-medium">No corrective work in this view</div><div className="mt-1 text-[9px] text-[#817A72]">When a technician records a site exception, it will enter this lifecycle automatically.</div></div> : null}</div></section>
