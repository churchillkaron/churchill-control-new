  ...
  const classes = tone === "good"
    ...
    : tone === "ready"
      ...
      : tone === "attention"
        ...
        : "border-black/[0.08] bg-[#F7F6F3]/[0.025] text-[#756F68]";
            ...
            {!detailLoading && report && prepared ? <div className="space-y-4">
              ...
              <section className="rounded-2xl border border-[#D6A66A]/20 bg-[#D6A66A]/[0.035] p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {report.billing?.eligible ? <button type="button" onClick={createInvoice} disabled={invoiceSaving} className="inline-flex items-center gap-1.5 rounded-lg bg-[#1D1B18] px-3.5 py-2.5 text-[9px] font-medium text-[#191919] transition hover:bg-[#F7F6F3] disabled:opacity-40"><ReceiptText size={10} />{invoiceSaving ? "Creating invoice…" : "Create Finance invoice"}</button> : null}
              ...
              <section className="rounded-2xl border border-black/[0.065] p-4">
                ...
                {customerConversations.length ? <div className="mt-3"><label className="block"><span className="text-[8px] font-medium uppercase tracking-[0.1em] text-[#918A82]">Customer conversation</span><select value={conversationId} onChange={(event) => setConversationId(event.target.value)} className="mt-1.5 w-full rounded-xl border border-black/[0.09] bg-white px-3 py-2.5 text-[9px] text-[#4A453F] outline-none focus:border-[#D6A66A]/60">{customerConversations.map((row) => <option key={row.id} value={row.id}>{conversationLabel(row)}</option>)}</select></label><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={createCommunicationsDraft} disabled={saving || !conversationId || Boolean(communicationDraft?.message_id)} className="inline-flex items-center gap-1.5 rounded-lg border border-[#D6A66A]/30 bg-[#D6A66A]/[0.08] px-3.5 py-2.5 text-[9px] font-medium text-[#76583A] disabled:opacity-40"><MessageSquareText size={10} />{saving ? "Creating draft…" : communicationDraft?.message_id ? "Draft created" : "Create Communications draft"}</button>{communicationDraft?.message_id ? <Link href={communicationsHref} className="inline-flex items-center gap-1.5 rounded-lg bg-[#1D1B18] px-3.5 py-2.5 text-[9px] font-medium text-[#191919]">Review & send in Communications <ArrowRight size={9} /></Link> : null}</div><div className="mt-2 text-[8px] leading-4 text-[#979188]">Creating the draft never sends the message. Commercial Communications owns the final review and explicit send.</div></div> : <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#C08A4A]/18 bg-[#C08A4A]/[0.05] px-3.5 py-3"><div><div className="text-[9px] font-medium text-[#755738]">No open external conversation is linked to this customer.</div><div className="mt-1 text-[8px] text-[#8E8173]">Open Customer Communications, connect the conversation, then return here to create the governed draft.</div></div><Link href={communicationsHref} className="inline-flex items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-3 py-2 text-[9px] font-medium text-[#665B4E]">Open Communications <ArrowRight size={9} /></Link></div>}
              </section>
            </div> : null}
          </section>
        </div>
