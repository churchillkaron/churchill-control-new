...
export default function OperationsFrontDeskPage() {
  ...
  return (
    <HotelWorkspaceShell
      ...
      active="front-desk"
      title="Front Desk"
      subtitle="Move arrivals, in-house stays and departures with the minimum safe number of steps. The queue is the work surface; status cards are secondary."
    ...
    >
      ...
      <section className="grid grid-cols-3 gap-3">
        <button type="button" onClick={() => setFilter("ARRIVALS")} className="text-left"><HotelMetric label="Arrivals" value={queues.ARRIVALS.length} detail="Ready for check-in work" attention={filter === "ARRIVALS" && queues.ARRIVALS.length > 0} /></button>
      ...
      <HotelSection
        ...
        detail="Open the guest action directly. No separate dashboard step is required."
      >
        {loading ? (
          <HotelEmptyState>Loading Front Desk…</HotelEmptyState>
        ) : filteredBookings.length ? (
          <div className="divide-y divide-black/[0.055]">
            <div className="hidden grid-cols-[minmax(190px,1.25fr)_150px_150px_110px_130px] gap-3 bg-[#FCFBF8] px-5 py-2 text-[7px] font-semibold uppercase tracking-[0.1em] text-[#969087] md:grid">
              <span>Guest / room</span><span>Arrival</span><span>Departure</span><span>Status</span><span>Next move</span>
            </div>
            {filteredBookings.map((booking) => {
              const bookingStatus = status(booking.status);
              const transitioning = transitioningId === booking.id;
              return (
                <div key={booking.id} className="grid gap-2 px-4 py-3 md:grid-cols-[minmax(190px,1.25fr)_150px_150px_110px_130px] md:items-center md:gap-3 md:px-5">
