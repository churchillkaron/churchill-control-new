...
export default function OperationsFrontDeskPage() {
    ...
    try {
      await hotelApi("/api/hotel/stays", {
        ...
        body: JSON.stringify({
          ...
          action: booking.room_id ? "MOVE_ROOM" : "ASSIGN_ROOM",
          roomId: resolver.roomId,
          reason: "Front Desk arrival readiness resolution",
        }),
      });
      setResolver({ bookingId: null, loading: false, busy: false, rooms: [], roomId: "", arrivalLink: "", error: "" });
      await loadBookings();
    } catch (reason) {
      setResolver((current) => ({ ...current, busy: false, error: reason.message || "Unable to assign room" }));
    }
  }
  ...
  async function createArrivalLink(booking) {
    ...
    try {
      const result = await hotelApi("/api/hotel/stays", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ organizationId, bookingId: booking.id, action: "CREATE_PRE_ARRIVAL" }),
      });
      ...
      setResolver({ bookingId: booking.id, loading: false, busy: false, rooms: [], roomId: "", arrivalLink: "", error: reason.message || "Unable to prepare guest arrival" });
    }
  }

  function blockedAction(booking) {
    const code = firstReadinessCode(booking);
    if (["ROOM_UNASSIGNED", "ROOM_NOT_FOUND", "ROOM_NOT_AVAILABLE"].includes(code)) {
      return <HotelSecondaryAction onClick={() => openRoomResolver(booking)}>Choose room</HotelSecondaryAction>;
    }
    if (code === "DEPOSIT_OUTSTANDING") {
      return <HotelSecondaryAction href={bookingWorkHref(organizationId, "hotel-payments", booking)}>Collect deposit</HotelSecondaryAction>;
    }
  ...
  function attentionAction(booking) {
    ...
    if (codes.has("REGISTRATION_INCOMPLETE")) {
      return <HotelSecondaryAction onClick={() => createArrivalLink(booking)}>Prepare arrival</HotelSecondaryAction>;
    }
  ...
  return (
    <HotelWorkspaceShell
      ...
      active="front-desk"
      title="Front Desk"
      subtitle="Work the exception, not the menu. Arrival blockers are visible before staff act and the safest resolution is available from the same desk queue; the server transition remains the final authority."
    ...
    >
      ...
      <HotelSection
        ...
        detail={filter === "ARRIVALS" ? "Resolve the exact blocker here: choose another governed-ready room, collect the required deposit, prepare registration, or open the exact stay record." : "Open the guest action directly. No separate dashboard step is required."}
      >
        {loading ? (
          <HotelEmptyState>Loading Front Desk…</HotelEmptyState>
        ) : filteredBookings.length ? (
          <div className="divide-y divide-black/[0.055]">
            <div className="hidden grid-cols-[minmax(190px,1.15fr)_130px_130px_105px_minmax(180px,0.95fr)_150px] gap-3 bg-[#FCFBF8] px-5 py-2 text-[7px] font-semibold uppercase tracking-[0.1em] text-[#969087] md:grid">
            ...
            {filteredBookings.map((booking) => {
              ...
              return (
                <div key={booking.id}>
                  <div className="grid gap-2 px-4 py-3 md:grid-cols-[minmax(190px,1.15fr)_130px_130px_105px_minmax(180px,0.95fr)_150px] md:items-center md:gap-3 md:px-5">
                    ...
                    <div className="min-w-0">
                      {bookingStatus === "RESERVED" ? <>
                        ...
                        {(readiness?.blockers?.length || 0) + (readiness?.attention?.length || 0) > 1 ? <div className="mt-0.5 text-[7px] font-semibold text-[#8A633C]">+{(readiness?.blockers?.length || 0) + (readiness?.attention?.length || 0) - 1} more item(s)</div> : null}
                      </> : <span className="text-[8px] text-[#918B83]">—</span>}
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {bookingStatus === "RESERVED" ? (
                        canCheckIn ? <>
                          <HotelPrimaryAction onClick={() => transitionBooking(booking, "CHECK_IN")} disabled={transitioning}><LogIn size={9} />{transitioning ? "Checking in" : "Check in"}</HotelPrimaryAction>
                          {readiness?.state === "NEEDS_ACTION" ? attentionAction(booking) : null}
                        </> : blockedAction(booking)
                  ...
                  {showResolver ? <div className="border-t border-black/[0.05] bg-[#FBFAF7] px-4 py-3 md:px-5">
                    {resolver.loading ? <div className="text-[8px] text-[#918B83]">Finding governed-ready rooms…</div> : resolver.arrivalLink ? <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div><div className="text-[8px] font-semibold text-[#403C37]">Secure arrival link ready</div><div className="mt-0.5 break-all text-[7px] text-[#918B83]">{resolver.arrivalLink}</div></div>
                      <div className="flex gap-2"><HotelSecondaryAction onClick={() => navigator.clipboard?.writeText(resolver.arrivalLink)}>Copy link</HotelSecondaryAction><HotelSecondaryAction href={bookingWorkHref(organizationId, "stay-control", booking)}>Open stay</HotelSecondaryAction></div>
                    </div> : resolver.rooms.length ? <div className="grid gap-2 md:grid-cols-[minmax(260px,420px)_auto_1fr] md:items-end">
                      <label><span className="text-[7px] font-semibold uppercase tracking-[0.1em] text-[#8D877F]">Governed-ready alternative</span><select className={`${hotelInputClass} mt-1.5`} value={resolver.roomId} onChange={(event) => setResolver((current) => ({ ...current, roomId: event.target.value }))}>{resolver.rooms.map((room) => <option key={room.id} value={room.id}>Room {room.room_number} · {room.room_type || "Room"}</option>)}</select></label>
                      <HotelPrimaryAction disabled={resolver.busy || !resolver.roomId} onClick={() => assignReadyRoom(booking)}>{resolver.busy ? "Assigning…" : booking.room_id ? "Move & resolve" : "Assign & resolve"}</HotelPrimaryAction>
                      <div className="text-[7px] leading-4 text-[#918B83]">The stay API re-checks property scope and `AVAILABLE` status before changing the assignment. A stale browser choice cannot force a room move.</div>
                    </div> : <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div><div className="text-[8px] font-semibold text-[#403C37]">No alternate room is ready</div><div className="mt-0.5 text-[7px] text-[#918B83]">Keep the arrival blocked and work the assigned room through Housekeeping, or change the reservation deliberately.</div></div>
                      <div className="flex gap-2"><HotelSecondaryAction href={hotelWorkspaceHref(organizationId, "housekeeping")}>Open Housekeeping</HotelSecondaryAction><HotelSecondaryAction href={bookingWorkHref(organizationId, "stay-control", booking)}>Open stay</HotelSecondaryAction></div>
                    </div>}
                    {resolver.error ? <div className="mt-2 text-[8px] text-red-800">{resolver.error}</div> : null}
                  </div> : null}
                </div>
              );
            })}
