...
export default function HotelOperationsControlPage() {
  ...
  const loadHotelRuntime = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const [bookings, rooms, housekeeping, maintenance, concierge] = await Promise.all([
  ...
  const state = useMemo(() => {
    ...
    const exceptions = [];
    if (dirtyRooms) exceptions.push({
      id: "dirty-rooms", title: "Release dirty rooms before demand reaches the desk",
      detail: `${dirtyRooms} room${dirtyRooms === 1 ? " is" : "s are"} not ready for sale or arrival.`,
      value: dirtyRooms, route: "housekeeping", status: "DIRTY", priority: 1,
    });
    if (blockedRooms) exceptions.push({
      id: "blocked-rooms", title: "Recover blocked room capacity",
      detail: `${blockedRooms} room${blockedRooms === 1 ? " is" : "s are"} unavailable because of an operating blocker.`,
      value: blockedRooms, route: "maintenance", status: "BLOCKED", priority: 1,
    });
    if (maintenance.length) exceptions.push({
      id: "maintenance", title: "Clear open property work",
      detail: `${maintenance.length} maintenance item${maintenance.length === 1 ? "" : "s"} can affect room readiness or guest experience.`,
      value: maintenance.length, route: "maintenance", status: "ATTENTION", priority: 2,
    });
    if (concierge.length) exceptions.push({
      id: "guest-requests", title: "Close open guest promises",
      detail: `${concierge.length} guest request${concierge.length === 1 ? " is" : "s are"} still open.`,
      value: concierge.length, route: "concierge", status: "OPEN", priority: 2,
    });
    if (arrivals.length) exceptions.push({
      id: "arrivals", title: "Prepare today's arrivals",
      detail: `${arrivals.length} arrival${arrivals.length === 1 ? "" : "s"} need a clean handoff into Front Desk.`,
      value: arrivals.length, route: "front-desk", status: "DUE", priority: 3,
    });
    if (departures.length) exceptions.push({
      id: "departures", title: "Protect today's departures",
      detail: `${departures.length} departure${departures.length === 1 ? "" : "s"} should close cleanly before room turnover.`,
      value: departures.length, route: "front-desk", status: "DUE", priority: 3,
    });
  ...
  if (organizationLoading || loading) {
    return <div className="min-h-[420px] bg-[#F7F6F3] p-8 text-[10px] text-[#77736C]">Preparing Hotel Control…</div>;
  }
  ...
  return (
    <HotelWorkspaceShell
      ...
      subtitle="Run the property by exception. Room readiness, arrivals, departures, maintenance and guest promises surface only when a person can move them."
      ...
      actions={<>
        ...
        <HotelSecondaryAction onClick={() => loadHotelRuntime({ silent: true })} disabled={refreshing}>
          <RefreshCw size={9} className={refreshing ? "animate-spin" : ""} /> Refresh
        </HotelSecondaryAction>
      </>}
    >
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.45fr)]">
        <HotelSection eyebrow="Priority work" title="Next human moves" detail="Operational exceptions ranked ahead of passive status. Healthy work stays quiet.">
          {state.exceptions.length ? (
            <div className="divide-y divide-black/[0.055]">
              {state.exceptions.slice(0, 10).map((item) => (
                <Link key={item.id} href={hotelWorkspaceHref(organizationId, item.route)} className="grid gap-2 px-4 py-3 transition hover:bg-[#FCFAF6] md:grid-cols-[minmax(260px,1.25fr)_minmax(240px,1fr)_80px_60px] md:items-center md:px-5">
                  <div className="min-w-0">
                    <div className="text-[9px] font-semibold text-[#403C37]">{item.title}</div>
                    <div className="mt-0.5 text-[7px] uppercase tracking-[0.08em] text-[#A09A92]">Hotel operation</div>
                  </div>
            ...
            <HotelEmptyState><span className="inline-flex items-center gap-2"><CheckCircle2 size={12} className="text-emerald-700" /> No surfaced hotel exception currently needs intervention.</span></HotelEmptyState>
          )}
        </HotelSection>
        ...
        <HotelSection eyebrow="Control state" title="Property context, not a dashboard" detail="Only numbers that help decide where staff should work next.">
          <div className="divide-y divide-black/[0.055]">
            {[
              ...
              ["Occupied", state.occupiedRooms, `${state.occupancy}% of room inventory`, "front-desk"],
