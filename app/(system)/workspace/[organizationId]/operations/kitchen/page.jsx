...
function toKitchenTicket(entry) {
  return {
    ...entry,
    station: entry.work_center?.name || entry.queue_name || "Kitchen",
    table_number: entry.context?.reference || null,
    order_number: entry.demand?.reference || entry.demand?.id || null,
    items: (entry.work_items || []).map((item) => ({
      ...(item.raw || {}),
      id: item.id,
      order_item_id: item.source_id,
      item_name: item.name,
      quantity: item.quantity,
      status: item.status,
      notes: item.notes,
      cooking_level: item.attributes?.cooking_level || null,
    })),
  };
}
...
export default function KitchenPage() {
  ...
  const loadKitchen = useCallback(async () => {
    ...
    try {
      ...
      setTickets((result.entries || []).map(toKitchenTicket));
    } catch (loadError) {
      setTickets([]);
  ...
  const visibleTickets = useMemo(() => {
    ...
    return tickets.filter(
      (ticket) =>
        !["COMPLETED", "SERVED", "CANCELLED", "VOID"].includes(
          statusOf(ticket.status)
        )
    );
  }, [filter, tickets]);
          ...
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm text-white/45">
                Live preparation work routed through operational queues and work centres.
              </p>
            </div>
            <button
              onClick={loadKitchen}
        ...
        <section className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {loading ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-4 grid grid-cols-2 gap-2">
                            ...
                            <button
                              disabled={busy || ["READY", "SERVED"].includes(status)}
            ...
            <div className="col-span-full rounded-3xl border border-dashed border-white/10 p-12 text-center text-white/35">
              No fulfillment work in this view.
            </div>
          )}
        </section>
      </div>
