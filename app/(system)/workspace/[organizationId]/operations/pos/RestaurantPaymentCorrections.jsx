...
export default function RestaurantPaymentCorrections({
  ...
  return (
    <section
      className="mt-3 rounded-[22px] border border-black/[0.07] bg-white p-3"
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-[#817B73]">
        ...
        <button
          ...
          onClick={begin}
          className="rounded-xl bg-[#D6A66A] px-3 py-2 text-[10px] font-semibold text-[#2B2118] disabled:opacity-30"
        >
          Correct payment
        </button>
      </div>
      ...
      {open ? (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-[#241b12]/20 p-4 backdrop-blur-sm">
          <div className="w-full max-w-[520px] rounded-[24px] border border-[#DCC8AE] bg-[#FCFAF6] p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#D6A66A]">Manager authority</div>
                <h2 className="mt-1 text-lg font-semibold text-[#2D2822]">Refund or reverse cash payment</h2>
                <p className="mt-1 text-xs text-[#817B73]">Full payment only. A reason is mandatory and the original payment is never deleted.</p>
              </div>
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="rounded-xl border border-black/[0.08] bg-white p-2 text-[#817B73]"
            ...
            <div className="mt-5 space-y-4">
              <label className="block">
                <span className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[#8A8178]">Payment</span>
                <select
                  value={selectedPaymentId || ""}
                  onChange={(event) => setSelectedPaymentId(event.target.value || null)}
                  className="mt-1.5 w-full rounded-xl border border-black/[0.08] bg-white px-3 py-2.5 text-xs text-[#2D2822]"
                >
              ...
              <div>
                <div className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[#8A8178]">Correction</div>
                <div className="mt-1.5 grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={action === "REFUND"
                      ...
                      : "flex items-center justify-center gap-2 rounded-xl border border-black/[0.08] bg-white px-3 py-2.5 text-xs text-[#625B53]"}
                  >
              ...
              {selectedPayment ? (
                <div className="rounded-xl border border-black/[0.07] bg-white p-3">
                  <div className="text-[9px] uppercase tracking-[0.14em] text-[#918B83]">Original cash payment</div>
                  <div className="mt-1 flex items-end justify-between gap-3">
                    <div className="text-xs text-[#746E66]">{paymentTitle(selectedPayment)}</div>
                    <div className="text-lg font-semibold text-[#2D2822]">{money(selectedPayment.amount, selectedPayment.currency || currencyCode)}</div>
                  </div>
                </div>
              ) : null}

              <label className="block">
                <span className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[#8A8178]">Required reason</span>
                <textarea
                  value={reason}
                  onChange={(event) => setReason(event.target.value)}
                  rows={3}
                  placeholder="Why is this payment being corrected?"
                  className="mt-1.5 w-full resize-none rounded-xl border border-black/[0.08] bg-white px-3 py-2.5 text-xs text-[#2D2822] outline-none"
                />
              </label>
              ...
              <button
                ...
                className="w-full rounded-xl bg-[#D6A66A] px-4 py-3 text-xs font-semibold text-[#2B2118] disabled:opacity-30"
              >
