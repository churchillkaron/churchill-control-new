...
export default function RetailCatalogWorkspace({
  ...
  const items = useMemo(() => runtime?.catalog?.items || [], [runtime?.catalog?.items]);
  const currency =
  ...
  return (
    <main className="min-h-screen bg-white px-4 py-5 text-[#191919] lg:px-6">
      <div className="mx-auto max-w-[1700px]">
        <header className="rounded-[26px] border border-black/[0.08] bg-[#FBF8F3] p-5">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-xs leading-5 text-[#746E66]">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-black/[0.08] px-3 py-2 text-xs text-[#5F5A54]"
            >
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-black/[0.08] bg-[#F7F6F3]/25 p-4">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#918B83]">Catalog</div>
              <div className="mt-2 text-2xl">{runtime?.catalog?.item_count || 0}</div>
              <div className="mt-1 text-[10px] text-[#918B83]">Canonical sellable items</div>
            </div>
            <div className="rounded-2xl border border-black/[0.08] bg-[#F7F6F3]/25 p-4">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#918B83]">Available</div>
              <div className="mt-2 text-2xl">{runtime?.catalog?.available_item_count || 0}</div>
              <div className="mt-1 text-[10px] text-[#918B83]">Available in selected entity</div>
            </div>
            <div className="rounded-2xl border border-[#D6A66A]/25 bg-[#D6A66A]/[0.06] p-4">
              ...
              <div className="mt-1 text-[10px] text-[#817A72]">No separate confirm screen required</div>
            </div>
          </div>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_360px]">
          <section>
            <div className="relative mb-4">
              <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#A19A92]" />
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Search item, SKU, barcode or category"
                className="w-full rounded-2xl border border-black/[0.08] bg-[#FBF8F3] py-3 pl-11 pr-4 text-sm outline-none placeholder:text-[#A9A39C] focus:border-[#D6A66A]/50"
              />
            </div>
            ...
            {visibleItems.length ? (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
                {visibleItems.map((item) => (
                  <article key={item.id} className="flex min-h-[190px] flex-col rounded-[22px] border border-black/[0.08] bg-[#FBF8F3] p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-[10px] text-[#918B83]">{[item.sku, item.barcode].filter(Boolean).join(" · ") || "No SKU or barcode"}</div>
                      </div>
                      <PackageSearch size={16} className="text-[#A9A39C]" />
                    </div>
              ...
              <div className="rounded-3xl border border-dashed border-black/[0.08] p-10 text-center text-sm text-[#918B83]">No catalog items match this search.</div>
            )}
          </section>

          <aside className="h-fit rounded-[24px] border border-black/[0.08] bg-[#FBF8F3] p-4 xl:sticky xl:top-24">
            <div className="flex items-center gap-3">
              <ShoppingBasket size={17} className="text-[#D6A66A]" />
              <div>
                <div className="text-sm font-semibold">Current basket</div>
                <div className="text-[10px] text-[#918B83]">{basketQuantity} item(s)</div>
              </div>
            </div>

            <div className="mt-4 space-y-2">
              {basketLines.length ? basketLines.map(({ item, quantity }) => (
                <div key={item.id} className="rounded-2xl border border-black/[0.08] bg-[#F7F6F3]/25 p-3">
                  <div className="flex justify-between gap-3">
                    <div className="min-w-0">
                      <div className="truncate text-xs font-medium">{item.name}</div>
                      <div className="mt-1 text-[10px] text-[#918B83]">{money(item.price, currency)} each</div>
                    </div>
                    <div className="text-xs">{money(item.price * quantity, currency)}</div>
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <button type="button" onClick={() => setQuantity(item.id, quantity - 1)} className="h-7 w-7 rounded-lg border border-black/[0.08]">−</button>
                    <div className="min-w-7 text-center text-xs">{quantity}</div>
                    <button type="button" onClick={() => setQuantity(item.id, quantity + 1)} className="h-7 w-7 rounded-lg border border-black/[0.08]">+</button>
                  </div>
                </div>
              )) : (
                <div className="rounded-2xl border border-dashed border-black/[0.08] p-5 text-center text-xs text-[#A19A92]">Add products to start a sale.</div>
              )}
            </div>

            <div className="mt-4 flex items-center justify-between border-t border-black/[0.08] pt-4">
              <span className="text-xs text-[#746E66]">Total</span>
            ...
            <p className="mt-3 text-[10px] leading-4 text-[#918B83]">
