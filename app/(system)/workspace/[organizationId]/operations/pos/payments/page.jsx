...
export default function PaymentsPage() {
  ...
  const loadPayableOrders = useCallback(async () => {
    ...
    const response = await fetch(
      `/api/restaurant/operations?scope=payments&organizationId=${encodeURIComponent(organizationId)}`,
  ...
  const loadPaymentState = useCallback(async (targetTable) => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const initial = requestedTable || orders.find((order) => order.table_number)?.table_number || "";
        if (cancelled) return;
  ...
  const selectedGross = useMemo(
    () => Number(selectedRows.reduce((sum, item) => sum + itemAmount(item, "remaining_amount", Number(item.price || 0) * Number(item.quantity || 1)), 0).toFixed(2)),
    [selectedRows]
  );
  ...
  async function changeTable(value) {
    ...
    try {
      ...
      router.replace(
        `/workspace/${organizationId}/operations/pos/payments?table=${encodeURIComponent(value)}`
      );
    } catch (loadError) {
      setPaymentState(null);
  ...
  async function pay(paymentAmount, partial, itemIds = []) {
    ...
    try {
      const response = await fetch(
        partial ? "/api/restaurant/payments/partial" : "/api/restaurant/payments/create",
        {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      if (result.fullyPaid || Number(result.remainingBalance || 0) <= 0) {
        ...
        router.push(
          `/workspace/${organizationId}/operations/pos/receipts${orderId ? `?order_id=${encodeURIComponent(orderId)}` : ""}`
        );
        return;
      }
  ...
  if (loading) {
    return <PageWrapper title="Payments" subtitle="Loading settlement"><div className="text-white/40">Loading...</div></PageWrapper>;
  }

  if (!paymentState) {
    return (
      <PageWrapper title="Payments" subtitle="Select an unpaid table">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {payableOrders.length ? payableOrders.map((order) => (
            ...
            >
              <div className="text-xl font-semibold">Table {order.table_number}</div>
              <div className="mt-2 text-sm text-white/45">{order.order_number || order.id}</div>
              <div className="mt-4 text-lg">{formatMoney(order.remaining_balance, currencyCode)}</div>
            </button>
          )) : <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-8 text-white/45">No unpaid restaurant orders.</div>}
        </div>
      </PageWrapper>
    );
  }
    ...
    <PageWrapper title="Payments" subtitle="Persisted billing and settlement">
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <section className="rounded-[30px] border border-white/10 bg-white/[0.03] p-6 xl:col-span-2">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-5xl font-light">{paymentState.session.table_number}</h2>
            </div>
            ...
            {items.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-2xl border px-4 py-4 text-left ${paid ? "border-emerald-400/15 bg-emerald-400/5 opacity-55" : selected ? "border-[#D6A66A]/50 bg-[#D6A66A]/10" : "border-white/10 bg-black/20"}`}
                >
                  <div>
                    <div className="font-medium">{item.item_name || item.name || "Item"}</div>
                    <div className="mt-1 text-xs text-white/40">{paid ? "Paid" : `${Number(item.quantity || 1)} item(s)`}</div>
                  </div>
        ...
        <aside className="rounded-[30px] border border-white/10 bg-white/[0.03] p-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {PAYMENT_OPTIONS.map((option) => {
              ...
              return (
                <button
                  key={option.value}
                  onClick={() => { paymentRequestKey.current = null; setPaymentMethod(option.value); }}
                  className={paymentMethod === option.value ? "rounded-2xl border border-[#D6A66A]/50 bg-[#D6A66A]/10 p-4 text-[#F3D7A2]" : "rounded-2xl border border-white/10 bg-black/20 p-4 text-white/55"}
                >
          ...
          <button disabled={actionLoading || selectedItems.length === 0 || selectedGross <= 0} onClick={() => pay(selectedGross, true, selectedItems)} className="mt-6 w-full rounded-2xl border border-[#D6A66A]/40 bg-[#D6A66A]/10 py-4 text-sm font-semibold text-[#F3D7A2] disabled:opacity-30">Pay Selected Items</button>
