...
function statusTone(status) {
  ...
  if (["REJECTED", "BLOCKED"].includes(value)) {
...
export default function ShiftPage({ posConfiguration }) {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      const [cashResponse, financeResponse] = await Promise.all([
        fetch(`/api/pos/cash-sessions?${search.toString()}`, {
          cache: "no-store",
          credentials: "include",
        }),
        fetch(`/api/finance/pos-cash-sessions/confirm?${financeSearch.toString()}`, {
          cache: "no-store",
          credentials: "include",
        }),
      ]);
      ...
      const [result, financeResult] = await Promise.all([
        readJson(cashResponse),
        readJson(financeResponse),
      ]);

      if (!cashResponse.ok || result.success === false) {
    ...
    try {
      const response = await fetch("/api/pos/cash-sessions", {
        ...
        body: JSON.stringify({
          ...
          notes: session ? sessionNotes(session) : null,
        }),
      });
  ...
  async function confirmAccounting(session) {
    ...
    try {
      ...
      const result = await readJson(response);
      if (!response.ok || result.success === false) {
        throw new Error(result.error || "Accounting confirmation failed");
      }
      ...
      <div className="mx-auto max-w-[1400px]">
        <header className="rounded-[34px] border border-white/10 bg-white/[0.035] p-7">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm text-white/45">
                Drawer reconciliation, manager review and Finance-confirmed final lock.
              </p>
            </div>
            <button
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-xs">
            <div className="rounded-full border border-white/10 px-3 py-1.5 text-white/45">
              Review authority: {actor?.can_review ? "Manager / Owner" : "View only"}
            </div>
            <div className="rounded-full border border-white/10 px-3 py-1.5 text-white/45">
              Finance confirmation: {financeCanConfirm ? "Authorized" : "Separate Finance authority"}
            </div>
          </div>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
            <article className="rounded-[30px] border border-white/10 bg-white/[0.03] p-7">
              ...
              <div className="mt-1 text-sm text-white/40">
                {actor?.role || actor?.staff_id || actor?.user_id || "Authenticated session"}
              </div>
              ...
              {!activeSession ? (
                <>
                  ...
                  />
                  <button
                    onClick={() => execute("OPEN", null)}
                    disabled={Boolean(actionLoading) || !entityId}
                    className="mt-5 w-full rounded-2xl bg-[#D6A66A] py-4 text-sm font-semibold text-black disabled:opacity-40"
                  >
                    {actionLoading === "OPEN:new" ? "Opening..." : "Open Cash Session"}
                  </button>
                </>
              ) : (
                <>
                  ...
                  <button
                    onClick={() => execute("CLOSE", activeSession)}
                  ...
                  >
                    {actionLoading?.startsWith("CLOSE:") ? "Reconciling..." : "Reconcile & Close Cash Session"}
                  </button>
                </>
              )}
            </article>
            ...
            <article className="rounded-[30px] border border-white/10 bg-white/[0.03] p-7">
              <div className="text-xs uppercase tracking-[0.2em] text-white/40">
                Reconciliation & governance history
              </div>
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 break-all text-[10px] text-white/30">{sessionId}</div>
                          </div>
                            ...
                            <div className="mt-4 border-t border-white/10 pt-4">
                              ...
                              {session.closed_by_name ? (
                                <div className="mt-3 text-[11px] text-white/30">
                                  Reconciled by {session.closed_by_name}
                                </div>
                              ) : null}
                            </div>

                            <div className="mt-4 grid grid-cols-2 gap-2">
                              <StatusBadge label="Manager review" status={approvalStatus} />
                              <StatusBadge label="Accounting" status={accountingStatus} />
                            </div>

                            {session.review_log ? (
                              <div className="mt-3 rounded-xl border border-white/10 bg-white/[0.025] p-3 text-xs text-white/45">
                                <div>
                                  Reviewed by {session.review_log.acted_by_name || session.review_log.role || "authorized manager"}
                                </div>
                                {session.review_log.notes ? (
                                  <div className="mt-1 text-white/60">{session.review_log.notes}</div>
                                ) : null}
                              </div>
                            ) : null}
                            ...
                            {approvalStatus === "PENDING" && actor?.can_review ? (
                              <div className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-300/[0.035] p-4">
                                <div className="flex items-center gap-2 text-xs font-semibold text-amber-100">
                                  <ShieldCheck size={15} /> Manager review
                                </div>
                                <textarea
                                  value={notes}
                                  onChange={(event) => updateSessionNotes(session, event.target.value)}
                                  placeholder="Review note or rejection reason"
                                  rows={2}
                                  className="mt-3 w-full resize-none rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm outline-none"
                                />
                                <div className="mt-3 grid grid-cols-2 gap-2">
                                  <button
                                    type="button"
                                    disabled={reviewing}
                                    onClick={() => execute("APPROVE", session)}
                                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-400 px-3 py-2.5 text-xs font-semibold text-black disabled:opacity-40"
                                  >
                                  ...
                                  <button
                                    ...
                                    onClick={() => execute("REJECT", session)}
                                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-400/30 bg-red-500/10 px-3 py-2.5 text-xs font-semibold text-red-100 disabled:opacity-40"
                                  >
                            ...
                            {approvalStatus === "APPROVED" && accountingStatus === "PENDING" ? (
                              <div className="mt-4 rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.035] p-4">
                                <div className="flex items-center gap-2 text-xs font-semibold text-cyan-100">
                                  <ShieldCheck size={15} /> Finance confirmation
                                </div>
                                ...
                                {financeCanConfirm ? (
                                  <>
                                    <textarea
                                      value={notes}
                                      onChange={(event) => updateSessionNotes(session, event.target.value)}
                                      placeholder="Accounting confirmation note"
                                      rows={2}
                                      className="mt-3 w-full resize-none rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm outline-none"
                                    />
                                    <button
                                      type="button"
                                      disabled={reviewing}
                                      onClick={() => confirmAccounting(session)}
                                      className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-3 py-2.5 text-xs font-semibold text-black disabled:opacity-40"
                                    >
                                      <LockKeyhole size={14} /> Confirm Accounting & Lock
                                    </button>
                                  </>
                                ) : (
                                  <div className="mt-3 text-xs leading-5 text-white/40">
                                    Awaiting a Finance-authorized user with finance.close.execute.
                                  </div>
                                )}
                              </div>
                            ) : null}

                            {approvalStatus === "REJECTED" || accountingStatus === "BLOCKED" ? (
                              <div className="mt-4 rounded-xl border border-red-400/20 bg-red-500/[0.06] p-3 text-xs text-red-100">
                                Reconciliation is blocked. A rejected drawer cannot proceed to accounting confirmation.
                              </div>
                            ) : null}

                            {session.period_closed && accountingStatus === "CONFIRMED" ? (
                              <div className="mt-4 rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] p-3 text-xs text-emerald-100">
                                <div className="flex items-center gap-2 font-semibold">
                                  <LockKeyhole size={14} /> Final accounting lock
                                </div>
                                <div className="mt-1 text-emerald-100/60">
                                  Finance posting evidence verified. This POS session is final.
                                  {session.accounting_confirmed_by_name
                                    ? ` Confirmed by ${session.accounting_confirmed_by_name}.`
                                    : ""}
                                </div>
                              </div>
                            ) : null}
