...
function statusPresentation(table) {
  ...
  if (["BILL REQUESTED", "PAYMENT_PENDING"].includes(status)) {
    return {
      ...
      pill: "border-[#D6A66A]/40 bg-[#D6A66A]/10 text-[#76583A]",
    };
  }
  if (["READY", "FOOD READY"].includes(status)) {
    return {
      ...
      pill: "border-amber-300/30 bg-amber-300/[0.08] text-amber-800",
    };
  }
  if (OPEN_STATUSES.has(status) || Number(table?.current_guests || 0) > 0) {
    return {
      ...
      pill: "border-emerald-300/20 bg-emerald-400/[0.07] text-emerald-800",
    };
  }
  return {
    label: "Available",
    card: "border-black/[0.08] bg-white",
    pill: "border-black/[0.08] bg-[#F7F6F3] text-[#746E66]",
  };
}
  ...
  return (
    <main className="min-h-screen bg-[#F7F6F3] px-4 py-5 text-[#191919] lg:px-6 lg:py-7">
      <div className="mx-auto max-w-[1760px]">
        <header className="rounded-[30px] border border-black/[0.07] bg-white p-5 lg:p-6">
          <div className="flex flex-wrap items-start justify-between gap-5">
            ...
            <button
              type="button"
              onClick={loadTables}
              className="inline-flex items-center gap-2 rounded-xl border border-black/[0.08] bg-white px-4 py-2.5 text-xs text-[#625B53]"
            >
          ...
          <div className="mt-5 grid grid-cols-2 gap-2 lg:grid-cols-4">
            {[
              ...
              <div key={label} className="rounded-2xl border border-black/[0.07] bg-[#FBF8F3] px-4 py-3">
                <div className="text-[9px] uppercase tracking-[0.18em] text-[#8A867F]">{label}</div>
                ...
                <button
                  ...
                  className={
                    activeZoneId === zone.id
                      ? "shrink-0 rounded-xl bg-[#D6A66A] px-4 py-2 text-xs font-semibold text-[#2B2118]"
                      : "shrink-0 rounded-xl border border-black/[0.08] bg-white px-4 py-2 text-xs text-[#625B53]"
                  }
                >
                  {zone.name}
                </button>
        ...
        <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
          <div className="rounded-[28px] border border-black/[0.07] bg-white p-4">
            {loading ? (
              <div className="flex min-h-[520px] items-center justify-center text-sm text-[#8A867F]">
                Loading live floor...
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="text-[9px] uppercase tracking-[0.2em] text-[#8A867F]">Table</div>
                          <div className="mt-1 text-3xl font-light">{labelOf(table)}</div>
                        </div>
                        <Users size={18} className="text-[#8A867F]" />
                      </div>
              ...
              <div className="flex min-h-[520px] items-center justify-center rounded-3xl border border-dashed border-black/[0.10] text-sm text-[#8A867F]">
                No restaurant tables are configured in this zone.
              </div>
            )}
          </div>

          <aside className="rounded-[28px] border border-black/[0.07] bg-white p-5 xl:sticky xl:top-4 xl:self-start">
            ...
            {selectedTable ? (
              <>
                <div className="mt-5 max-h-[300px] space-y-1.5 overflow-y-auto pr-1">
                  {selectedItems.length ? (
                    selectedItems.map((item) => (
                      <div key={item.id} className="flex items-center justify-between rounded-xl border border-black/[0.07] bg-[#FBF8F3] px-3 py-2.5 text-xs">
                        <div className="min-w-0">
                          <div className="truncate font-medium">{item.item_name || item.name || "Item"}</div>
                          <div className="mt-0.5 text-[10px] text-[#8A867F]">Seat {item.seat_position || item.seat_number || "—"}</div>
                        </div>
                        <div className="ml-3 text-[#746E66]">{Number(item.quantity || 1)} × {formatMoney(item.price, currencyCode)}</div>
                      </div>
                    ))
                  ) : (
                    <div className="rounded-2xl border border-dashed border-black/[0.10] px-4 py-8 text-center text-xs text-[#8A867F]">
                ...
                <button
                  ...
                  className="mt-5 w-full rounded-2xl bg-[#D6A66A] px-4 py-3.5 text-sm font-semibold text-[#2B2118]"
                >
                  Continue at stationary POS
                </button>
                <div className="mt-2 text-center text-[10px] leading-4 text-[#918B83]">
                  Order, split and settlement continue together on the cashier workstation. No separate payment workspace.
                </div>
              </>
            ) : (
              <div className="mt-5 rounded-2xl border border-dashed border-black/[0.10] p-5 text-xs leading-5 text-[#8A867F]">
