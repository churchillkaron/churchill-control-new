...
export default function MyDayPage() {
  ...
  const dateHeading = useMemo(() => {
    const value = runtime?.businessDate;
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && nextJob ? (
        <section className="overflow-hidden rounded-[34px] border border-cyan-400/25 bg-gradient-to-br from-cyan-500/15 via-white/[0.05] to-emerald-500/10 shadow-[0_20px_70px_rgba(34,211,238,0.08)]">
          <div className="h-[2px] bg-gradient-to-r from-cyan-400 to-emerald-400" />
          <div className="p-5">
              ...
              <div className="min-w-0">
                ...
                {nextJob.locationName ? (
                  <div className="mt-3 flex items-center gap-2 text-sm text-white/40">
                    ...
                    <span className="truncate">{nextJob.locationName}</span>
                  </div>
                ) : null}
              </div>
            </div>
        ...
        <section className="space-y-3">
          {jobs.map((job, index) => {
            ...
            return (
              <article
                key={job.id}
                className={`rounded-[30px] border p-4 backdrop-blur-3xl ${
                  isCompleted
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex min-w-0 items-start gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px] border border-white/10 bg-black/25 text-sm font-black text-white/70">
                      {index + 1}
                    </div>
                    <div className="min-w-0">
                  ...
                  <div className={`mt-3 flex items-start gap-2 rounded-2xl px-3 py-3 text-xs ${
                    ...
                    <div>
                      <div className="font-black">
                        GPS recorded {gps.action === "complete" ? "at completion" : "at start"}
                      </div>
                      {distanceLabel(gps.distanceFromDestinationMeters) ? (
                        <div className="mt-1 opacity-70">
                          {distanceLabel(gps.distanceFromDestinationMeters)}
                        </div>
                      ) : (
                        <div className="mt-1 opacity-70">Service location coordinates are not stored yet.</div>
                      )}
                    </div>
                  </div>
                ) : null}
                ...
                {!isCompleted ? (
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => runJobAction(job, isInProgress ? "complete" : "start")}
                      disabled={isBusy || !runtime?.shiftActive}
                      className={`flex h-12 items-center justify-center gap-2 rounded-[20px] text-xs font-black uppercase tracking-[0.11em] text-white disabled:opacity-35 ${
                        isInProgress
                    ...
                    >
                      ...
                      {isBusy
                        ? "GPS..."
                        : isInProgress
                          ? `Complete ${job.actionNoun}`
                          : `Start ${job.actionNoun}`}
                    </button>
                  </div>
                ) : null}
              </article>
