...
function temporalReports(plan = {}, fps = 30) {
  return list(plan.scenes).flatMap(
    (scene, sceneIndex) =>
...
function inspectPlan({
  ...
  const reports = temporalReports(
    storyboard.creativePlan,
    Number(input.fps || 30),
  );
  ...
  return {
    ...
    passed:
      storyboard.report.passed === true &&
      temporalFailures.length === 0,
    failure_count:
      storyboard.report.failures.length +
      temporalFailures.length,
    ...
    temporal: {
      ...
      total_frames: reports.reduce(
        (total, report) =>
          total + Number(report.total_frames || 0),
        0,
      ),
      timed_item_count: reports.reduce(
        (total, report) =>
          total + Number(report.timed_item_count || 0),
        0,
      ),
      reports,
    },
    failures: [
      ...storyboard.report.failures,
      ...temporalFailures,
    ],
  };
}
...
function groupRemainingFailures(
  audit,
  group,
) {
  const failures = list(
  ...
  return failures.filter((detail) => {
    if (detail.scope !== group.scope) {
      return false;
    }
...
function scopeOutputShape(group) {
  if (group.scope === "SHOT") {
    return {
      result: {
...
function sceneAndShot(plan, group) {
  const scene = group.scene_number
    ? list(plan.scenes)[group.scene_number - 1]
    : null;

  const shot =
    scene && group.shot_number
      ? list(scene.shots)[group.shot_number - 1]
      : null;
...
function applyGroupPatch({
  ...
  if (group.scope === "PLAN") {
    ...
    if (
      ...
      throw new Error(
        "CREATIVE_STORYBOARD_PLAN_PATCH_SCENES_FORBIDDEN",
      );
    }
  ...
  if (!Object.keys(scene).length) {
    throw new Error(
      "CREATIVE_STORYBOARD_REPAIR_SCENE_NOT_FOUND",
    );
  }

  if (group.scope === "SCENE") {
    const patch = object(
      result.scene_patch || result.patch,
    );
    ...
    if (
      ...
      throw new Error(
        "CREATIVE_STORYBOARD_SCENE_PATCH_SHOTS_FORBIDDEN",
      );
    }
  ...
  if (!Object.keys(shot).length) {
    throw new Error(
      "CREATIVE_STORYBOARD_REPAIR_SHOT_NOT_FOUND",
    );
  }

  const patch = object(
    result.shot_patch || result.patch,
  );
  ...
  output.scenes[sceneIndex].shots[shotIndex] = {
    ...
    reference_asset_ids:
      clone(shot.reference_asset_ids),
    assets: clone(shot.assets),
    master_still_contract:
      clone(shot.master_still_contract),
    temporal_contract:
      clone(shot.temporal_contract),
  };

  delete output.scenes[sceneIndex]
    .shots[shotIndex].scene_number;
...
function validateCandidate({
  ...
  const beforeReferences =
    referenceIds(currentPlan);
  const afterReferences =
    referenceIds(candidatePlan);

  if (
    JSON.stringify(beforeReferences.sort()) !==
    JSON.stringify(afterReferences.sort())
  ) {
    reasons.push(
...
async function focusedRepair({
  ...
  const execution = await reason({
    task: [
      ...
      "Do not rewrite the complete production bible.",
      "Do not change scene or shot structure, duration, canonical references, master-still contracts, temporal contracts, identities, factual claims, venue geometry or unrelated decisions.",
      "Use the canonical field names already present in the current production bible.",
      "Every value must be concrete, mission-specific and production-executable; no generic cinematic filler or schema placeholders.",
      "Preserve valid existing fields and explicitly list the failures addressed.",
    ].join(" "),
    input: {
      organization_id:
        job.organization_id,
      ...
      objective:
        object(job.input_snapshot).objective,
      brief:
        object(job.input_snapshot).brief,
  ...
  if (
    ...
    const error = new Error(
      "CREATIVE_STORYBOARD_FOCUSED_REPAIR_REASONING_FAILED",
    );
    error.code =
      "CREATIVE_STORYBOARD_FOCUSED_REPAIR_REASONING_FAILED";
    error.details = {
...
async function completeRepairStep({
  job,
  step,
  plan,
  audit,
    ...
    await supabaseAdmin
      ...
      .update({
        ...
        metrics: {
          ...
          kind: "REPAIR",
          convergence_version:
            "failure-addressed-storyboard-v1",
  ...
  const { data: nextStep, error: nextError } =
    await supabaseAdmin
      .from(STEPS)
      .select("*")
      .eq("job_id", job.id)
      .eq("step_key", NEXT_AUDIT_STEP)
      .single();
  ...
  try {
    ...
    if (
      ...
      return safeResponse({
        ...
        status: 409,
        error:
          "CREATIVE_STORYBOARD_REPAIR_STEP_NOT_AVAILABLE",
        details: {
    ...
    let plan = normalizeCreativeStoryboardPlan(
    ...
    if (!audit.temporal.passed) {
      return safeResponse({
        ...
        status: 422,
        error:
          "CREATIVE_STORYBOARD_CONVERGENCE_REQUIRES_VALID_TEMPORAL_BASELINE",
        audit,
        convergence,
      });
    }
    ...
    for (
      ...
      for (
        ...
        convergence.executions.push({
          ...
          attempt,
          provider:
            repair.execution.provider || null,
          model:
            repair.execution.model || null,
          confidence:
            Number(
              repair.execution.confidence || 0,
            ),
        });
        ...
        convergence.current_failure_count =
          audit.failure_count;
        convergence.current_storyboard_failure_count =
          audit.storyboard.failures.length;
      ...
      if (!accepted) {
        ...
        convergence.stop_reason =
          "GROUP_DID_NOT_PRODUCE_MEASURABLE_IMPROVEMENT";
        break;
      }
    }
    ...
    if (
      ...
      return safeResponse({
        ...
        status: 422,
        error:
          "CREATIVE_STORYBOARD_CONVERGENCE_DID_NOT_IMPROVE",
        audit,
        convergence,
      });
    }
    ...
    return safeResponse({
      ...
      error:
        error.message ||
        "CREATIVE_STORYBOARD_CONVERGENCE_FAILED",
      details: {
