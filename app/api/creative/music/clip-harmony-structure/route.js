...
import { analyzeMusicHarmonyStructure } from "@/lib/creative/music/runtime/CreativeMusicHarmonyStructureRuntime";
...
async function requireAccess(request, organizationId) {
  ...
  if (!access.success) {
    const error = new Error(access.error || "CREATIVE_MUSIC_HARMONY_STRUCTURE_ACCESS_FORBIDDEN");
...
async function projectInScope(organizationId, projectId) {
  ...
  if (!project || String(project.organization_id) !== String(organizationId)) {
    const error = new Error("CREATIVE_MUSIC_HARMONY_STRUCTURE_PROJECT_NOT_FOUND");
    error.status = 404;
    throw error;
  }
  return project;
...
export async function POST(request) {
  try {
    ...
    if (currentRevision !== expectedRevision) {
      const error = new Error(`CREATIVE_MUSIC_HARMONY_STRUCTURE_REVISION_CONFLICT:expected=${expectedRevision}:current=${currentRevision}`);
      error.status = 409;
      throw error;
    }
    const track = session.tracks?.find((entry) => entry.id === trackId);
    const clip = track?.clips?.find((entry) => entry.id === clipId);
    if (!track || !clip) throw new Error("CREATIVE_MUSIC_HARMONY_STRUCTURE_CLIP_NOT_FOUND");
    ...
    if (!asset || String(asset.organization_id) !== String(organizationId) || String(assetProjectId) !== String(projectId)) {
      throw new Error("CREATIVE_MUSIC_HARMONY_STRUCTURE_SOURCE_ASSET_NOT_FOUND");
    }

    const analysis = await analyzeMusicHarmonyStructure({
    ...
    nextClip.harmony_structure_analysis = {
      ...analysis,
    ...
    return NextResponse.json({
      success: true,
      contract: "AVANTIQO_MUSIC_CLIP_HARMONY_STRUCTURE_V1",
      analysis,
    ...
    return NextResponse.json({
      success: false,
      error: error?.message || "Music harmony structure analysis failed",
      provider_job_submitted: false,
      endpoint_mutation_performed: false,
