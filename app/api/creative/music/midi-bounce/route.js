...
import { fingerprintMusicMidiBounce } from "@/lib/creative/music/runtime/CreativeMusicMidiBounceFingerprintRuntime";
...
import { ensureMusicSamplerProject } from "@/lib/creative/music/runtime/CreativeMusicSamplerRuntime";
import { requireOrganizationAccess } from "@/lib/platform/security/requireOrganizationAccess";
import { getServiceSupabase } from "@/lib/shared/supabase/service";

const PERMISSIONS=Object.freeze(["creative.execute","creative.production.run","creative.*"]);const KEY="music_multitrack_project";const SAMPLER_KEY="music_sampler_project";const BUCKET="creative-assets";const MAX_BYTES=1024*1024*1024;
...
function samplerForTrack(project,midiTrack){const kind=text(midiTrack?.instrument?.kind).toLowerCase();if(!["drum_machine","sampler","drum_rack"].includes(kind))return null;return ensureMusicSamplerProject(project.metadata?.[SAMPLER_KEY]||{});}
...
async function register(body){
  ...
  const sourceMidiCoreFingerprint=fingerprintMusicMidiBounce({session:current,track:midiTrack,sampler:null});
  const sourceMidiFingerprint=fingerprintMusicMidiBounce({session:current,track:midiTrack,sampler:samplerForTrack(project,midiTrack)});
  if(text(body.source_midi_core_fingerprint)!==sourceMidiCoreFingerprint)throw new Error("CREATIVE_MUSIC_MIDI_BOUNCE_SOURCE_CORE_FINGERPRINT_MISMATCH");
  if(text(body.source_midi_fingerprint)!==sourceMidiFingerprint)throw new Error("CREATIVE_MUSIC_MIDI_BOUNCE_SOURCE_FINGERPRINT_MISMATCH");
  ...
  midiTrack.release_bounce={contract:"AVANTIQO_MUSIC_MIDI_RELEASE_BOUNCE_LINK_V2",asset_id:asset.id,audio_track_id:track.id,bounce_contract:bounceContract,source_midi_core_fingerprint:sourceMidiCoreFingerprint,source_midi_fingerprint:sourceMidiFingerprint,tempo_map_aware:tempoMapAware,created_at:new Date().toISOString(),source_midi_preserved:true};
