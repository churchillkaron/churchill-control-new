...
async function requireAccess(request, organizationId) {
  ...
  if (!access.success) { const error = new Error(access.error || "CREATIVE_MUSIC_MULTITRACK_ACCESS_FORBIDDEN"); error.status = access.status || 403; throw error; }
}
...
async function projectInScope(organizationId, projectId) {
  ...
  if (!project || String(project.organization_id) !== String(organizationId)) { const error = new Error("CREATIVE_MUSIC_MULTITRACK_PROJECT_NOT_FOUND"); error.status = 404; throw error; }
  return project;
}
...
async function publicSessionResult({ organizationId, projectId, project, session, persisted }) {
  ...
  return {
    ...
    unified_transport_contract: "AVANTIQO_MUSIC_UNIFIED_WORKSTATION_TRANSPORT_V2",
...
async function saveSession(organizationId, projectId, submitted) {
  ...
  const metadata = { ...(project.metadata || {}), [METADATA_KEY]: next, music_bpm: next.bpm, music_time_signature: next.time_signature, music_multitrack_updated_at: new Date().toISOString() };
...
export async function POST(request) {
  try {
    ...
    const result = action === "load" ? await loadSession(organizationId, projectId) : action === "save" ? await saveSession(organizationId, projectId, body.session) : null;
