...
function usableReferenceAsset(asset = {}) {
  ...
  if (/audio\//.test(text)) return false;
  if (/\.(mp3|wav|aac|m4a|flac)(?:\?|$)/.test(text)) return false;
  return Boolean(
    asset.file_url ||
    asset.image_url ||
    asset.thumbnail_url ||
...
function assetRank(asset = {}) {
  ...
  score += Math.min(25, Number(asset.performance_score || asset.score || 0));
...
export async function POST(req) {
  try {
    ...
    const assets = assetResolution.assets;
