...
export async function GET(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("accounts_receivable").select("*").eq("organization_id", access.organizationId).order("due_date", { ascending: true });
    if (error) throw error;
    return NextResponse.json({ success: true, receivables: data || [] });
  } catch (error) { const message = error.message || "Accounts receivable list failed"; return NextResponse.json({ success: false, error: message }, { status: statusFor(message) }); }
}
