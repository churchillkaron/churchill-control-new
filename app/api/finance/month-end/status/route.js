...
export async function GET(request) {
  try {
    ...
    if (!access.success) {
      return NextResponse.json({ success: false, error: access.error, periods: [] }, { status: access.status });
    }
    ...
    const message = error.message || "Month-end status load failed";
    return NextResponse.json(
      { success: false, error: message, periods: [] },
