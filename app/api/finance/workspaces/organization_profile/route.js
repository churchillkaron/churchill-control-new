...
function normalizePayload(body = {}) {
  ...
  payload.company_registration_number = cleanText(payload.company_registration_number);
...
function validateRequired(payload) {
  ...
  if (!Number.isInteger(payload.fiscal_year_start_month) || payload.fiscal_year_start_month < 1 || payload.fiscal_year_start_month > 12) {
...
async function validateProfile({ organizationId, payload }) {
  ...
  await validateCurrency(organizationId, payload.functional_currency, "Functional Currency");
  await validateCurrency(organizationId, payload.reporting_currency, "Reporting Currency");
}
...
function responseError(error, fallback) {
  ...
  const status = /permission denied/i.test(message)
    ? 403
    : /required|must be|not configured|valid|whole number|country code/i.test(message)
      ? 400
      : 500;
...
export async function GET(request) {
  try {
    ...
    if (!access.success) {
      return NextResponse.json({ success: false, error: access.error, rows: [] }, { status: access.status });
    }
    await requireFinanceWorkspacePermission({
      capabilityId: "organization_profile",
      operation: "read",
      access,
    });
    ...
    const row = profile
      ? {
          ...
          code: [profile.country_code, profile.functional_currency].filter(Boolean).join(" · "),
        }
      : null;
...
async function save(request) {
  ...
  return NextResponse.json({ success: true, record: normalizeFinanceOrganizationProfile(data) });
}
