...
export async function GET(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("healthcare_pharmacy_inventory")
      .select("*")
    ...
    console.error("HEALTHCARE_PHARMACY_GET_ERROR", error);
    return errorResponse(error?.message || "Pharmacy lookup failed");
  }
}
...
export async function POST(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("healthcare_pharmacy_inventory").insert([payload]).select();
    if (error) throw error;

    return NextResponse.json({ success: true, data: data || [] });
  } catch (error) {
    console.error("HEALTHCARE_PHARMACY_POST_ERROR", error);
    return errorResponse(error?.message || "Pharmacy item creation failed");
  }
}
