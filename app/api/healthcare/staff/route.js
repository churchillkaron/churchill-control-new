...
export async function GET(request) {
  try {
    ...
    console.error("HEALTHCARE_STAFF_GET_ERROR", error);
    return errorResponse(error?.message || "Healthcare staff lookup failed");
  }
}
...
export async function POST(request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("healthcare_staff").insert([payload]).select();
    if (error) throw error;

    return NextResponse.json({ success: true, data: data || [] });
  } catch (error) {
    console.error("HEALTHCARE_STAFF_POST_ERROR", error);
    return errorResponse(error?.message || "Healthcare staff creation failed");
  }
}
