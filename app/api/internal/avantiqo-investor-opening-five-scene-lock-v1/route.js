...
const TOKEN = "avq-investor-opening-five-scene-lock-20260822-v1";
...
const OPERATIONS = `${ORGANIZATION_ID}/unassigned/cbba2295-76c6-43ea-acf5-1511017cc63b-gemini-v24pbxy5sy1t.mp4`;
...
// origin-03 is 40 locked-script words over 16.875 seconds.
// "Finance knew one part of the business." = 7 words.
const FINANCE_SECONDS = 2.953;
// "Operations knew another." = 3 words.
const OPERATIONS_SECONDS = 1.266;
const NARRATION_SECONDS = FOUNDER_SECONDS + RESTAURANT_SECONDS + FINANCE_SECONDS + OPERATIONS_SECONDS;
const TOTAL_SECONDS = OPENING_SECONDS + NARRATION_SECONDS;
const NARRATION_DELAY_MS = Math.round(OPENING_SECONDS * 1000);
const OUTPUT_PATH = `${ORGANIZATION_ID}/${PROJECT_ID}/scene-previews-20260822/avantiqo-opening-five-scenes-locked-v1.mp4`;

const supabase = getServiceSupabase();

const LOCK = Object.freeze({
  contract: "AVANTIQO_INVESTOR_OPENING_FIVE_SCENE_LOCK_V1",
  locked: true,
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error("FIVE_SCENE_LOCK_EDITOR_TIMEOUT"));
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`FIVE_SCENE_LOCK_SOURCE_EMPTY:${storagePath}`);
...
async function normalizeVideo(ffmpeg, source, output, duration) {
  await run(ffmpeg, [
    "-y",
    "-i", source,
    "-t", String(duration),
...
async function concatVideo(ffmpeg, clips, output, directory) {
  const list = path.join(directory, "five-scenes.concat.txt");
  await fs.writeFile(
    list,
...
async function upload(localPath) {
  ...
  const { error } = await supabase.storage.from(BUCKET).upload(OUTPUT_PATH, bytes, {
    ...
    metadata: {
      ...
      scene_5: "LIVE_OPERATIONS_KITCHEN",
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-five-scene-lock-"));
  try {
    const source = {
      ...
      operations: path.join(directory, "operations-source.mp4"),
    ...
    await Promise.all([
      ...
      download(OPERATIONS, source.operations),
    ...
    const clips = [
      ...
      path.join(directory, "05-operations.mp4"),
    ];
    ...
    await normalizeVideo(ffmpeg, source.operations, clips[5], OPERATIONS_SECONDS);

    const picture = path.join(directory, "five-scenes-picture.mp4");
    const audio = path.join(directory, "five-scenes-audio.m4a");
    const final = path.join(directory, "avantiqo-opening-five-scenes-locked-v1.mp4");
