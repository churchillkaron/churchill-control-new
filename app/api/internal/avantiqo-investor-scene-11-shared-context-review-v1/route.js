...
const TOKEN = "avq-investor-scene-11-shared-context-review-v1-20260823";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_11_SHARED_CONTEXT_REVIEW_V1";
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260823/scene-11-shared-operating-context-review-v1.mp4`;
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("SCENE_11_REVIEW_TIMEOUT"));
      }
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`SCENE_11_SOURCE_EMPTY:${storagePath}`);
...
async function upload(localPath) {
  ...
  const { error } = await supabase.storage.from(BUCKET).upload(OUTPUT, bytes, {
    ...
    metadata: {
      ...
      scene: "11",
      role: "AUTHENTIC_PRODUCT_UI_IN_WORLD",
      narration: NARRATION,
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-scene11-context-"));
  try {
    ...
    const final = path.join(directory, "scene11-final.mp4");
...
export async function GET(request) {
  try {
    ...
    if (action === "status") return json({ success: true, contract: CONTRACT, scene: 11, output_ready: ready, output_path: OUTPUT });
