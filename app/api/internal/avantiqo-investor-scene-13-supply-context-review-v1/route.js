...
const TOKEN = "avq-investor-scene-13-supply-context-review-v1-20260823";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_13_SUPPLY_CONTEXT_REVIEW_V1";
...
const BACKGROUND = `${ORG}/unassigned/eef84bd3-c208-4ed8-bba0-6088a9b67ef9-gemini-thgn4qnk6hof.mp4`;
const AUTHENTIC_UI = `${ORG}/avantiqo-investor-film-20260820/ui/supply_chain.png`;
const AUTHENTIC_UI_ASSET_ID = "6c72782b-c95b-485c-a13d-964d9918c19f";
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260823/scene-13-supply-context-review-v1.mp4`;
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("SCENE_13_REVIEW_TIMEOUT"));
      }
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`SCENE_13_SOURCE_EMPTY:${storagePath}`);
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-scene13-supply-"));
  try {
    const background = path.join(directory, "procurement.mp4");
    const ui = path.join(directory, "supply-chain.png");
    ...
    const final = path.join(directory, "scene13-final.mp4");
    ...
    await run(ffmpeg, [
      ...
      "-t", String(DURATION),
      "-filter_complex",
      `[0:v]scale=1960:1103:force_original_aspect_ratio=increase,crop=${W}:${H}:x='(iw-ow)/2+3*sin(t*0.24)':y='(ih-oh)/2+2*sin(t*0.18)',fps=${FPS},eq=contrast=1.04:brightness=-0.012:saturation=0.91,format=yuv420p[bg];[1:v]format=rgba,fade=t=in:st=0.10:d=0.44:alpha=1,scale=w='if(lt(t,1.15),720+500*(t/1.15),1220)':h=-2:eval=frame[pane];[bg][pane]overlay=x='if(lt(t,1.15),1010-545*(t/1.15),465+4*sin(t*0.30))':y='if(lt(t,1.15),455-260*(t/1.15),195+3*sin(t*0.23))':shortest=1,format=yuv420p[v]`,
      "-map", "[v]",
      "-an",
      "-c:v", "libx264",
      "-preset", "fast",
...
export async function GET(request) {
  try {
    ...
    if (action === "status") return json({ success: true, contract: CONTRACT, scene: 13, output_ready: ready, output_path: OUTPUT });
