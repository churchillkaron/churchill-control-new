...
const TOKEN = "avq-investor-scene-14-finance-context-review-v1-20260823";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_14_FINANCE_CONTEXT_REVIEW_V1";
...
const AUTHENTIC_UI = `${ORG}/avantiqo-investor-film-20260820/ui/finance.png`;
const AUTHENTIC_UI_ASSET_ID = "ec166c5d-55ac-44f4-a7eb-5b6c03dab36f";
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260823/scene-14-finance-context-review-v1.mp4`;
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("SCENE_14_REVIEW_TIMEOUT"));
      }
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`SCENE_14_SOURCE_EMPTY:${storagePath}`);
...
async function makeOpticalPane(uiPath, outputPath) {
  ...
  const glass = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${paneW}" height="${paneH}">
    ...
    <path d="M28 17 H${paneW - 38}" stroke="#ffffff" stroke-opacity=".23" stroke-width="2" stroke-linecap="round"/>
  </svg>`);
...
async function makeContextTrace(outputPath) {
  const svg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    <defs>
      ...
      <linearGradient id="g" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#d6a66a" stop-opacity="0"/><stop offset=".42" stop-color="#d6a66a" stop-opacity=".34"/><stop offset="1" stop-color="#f8f4e9" stop-opacity=".72"/></linearGradient>
    </defs>
    <path d="M-80 770 C220 744 390 690 560 620" fill="none" stroke="url(#g)" stroke-width="1.5" stroke-linecap="round" filter="url(#soft)"/>
    <path d="M-80 770 C220 744 390 690 560 620" fill="none" stroke="#e6c98b" stroke-opacity=".44" stroke-width="1.0" stroke-linecap="round"/>
  </svg>`);
...
async function upload(localPath) {
  ...
  const { error } = await supabase.storage.from(BUCKET).upload(OUTPUT, bytes, {
    ...
    metadata: {
      ...
      scene: "14",
      role: "FINANCE_IN_SHARED_CONTEXT",
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-scene14-finance-"));
  try {
    const background = path.join(directory, "finance.mp4");
    const ui = path.join(directory, "finance-ui.png");
    ...
    const trace = path.join(directory, "context-trace.png");
    const picture = path.join(directory, "picture.mp4");
    const final = path.join(directory, "scene14-final.mp4");
    ...
    await Promise.all([makeOpticalPane(ui, pane), makeContextTrace(trace)]);
    ...
    await run(ffmpeg, [
      ...
      "-t", String(DURATION),
      "-filter_complex",
      `[0:v]scale=1960:1103:force_original_aspect_ratio=increase,crop=${W}:${H}:x='(iw-ow)/2+3*sin(t*0.22)':y='(ih-oh)/2+2*sin(t*0.16)',fps=${FPS},eq=contrast=1.04:brightness=-0.012:saturation=0.90,format=yuv420p[bg];[2:v]format=rgba,fade=t=in:st=0.02:d=0.40:alpha=1[trace];[bg][trace]overlay=0:0:shortest=1[ctx];[1:v]format=rgba,fade=t=in:st=0.12:d=0.44:alpha=1,scale=w='if(lt(t,1.10),700+500*(t/1.10),1200)':h=-2:eval=frame[pane];[ctx][pane]overlay=x='if(lt(t,1.10),1035-555*(t/1.10),480+4*sin(t*0.28))':y='if(lt(t,1.10),455-260*(t/1.10),195+3*sin(t*0.22))':shortest=1,format=yuv420p[v]`,
      "-map", "[v]",
      "-an",
      "-c:v", "libx264",
      "-preset", "fast",
...
export async function GET(request) {
  try {
    ...
    if (action === "status") return json({ success: true, contract: CONTRACT, scene: 14, output_ready: ready, output_path: OUTPUT });
