...
const TOKEN = "avq-investor-scene-16-organization-intelligence-review-v1-20260823";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_16_ORGANIZATION_INTELLIGENCE_REVIEW_V1";
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260823/scene-16-organization-intelligence-review-v1.mp4`;
...
async function render() {
  ...
  try {
    ...
    await run(ffmpeg, [
      "-y", "-stream_loop", "-1", "-ss", "2.0", "-i", background,
      "-loop", "1", "-framerate", String(FPS), "-i", pane,
      "-t", String(DURATION),
      "-filter_complex",
      `[0:v]scale=1960:1103:force_original_aspect_ratio=increase,crop=${W}:${H}:x='(iw-ow)/2+2*sin(t*0.19)':y='(ih-oh)/2+2*sin(t*0.14)',fps=${FPS},eq=contrast=1.035:brightness=-0.010:saturation=0.90,format=yuv420p[bg];[1:v]format=rgba,fade=t=in:st=0.05:d=0.35:alpha=1,scale=1170:-2[pane];[bg][pane]overlay=x='500+3*sin(t*0.24)':y='205+2*sin(t*0.19)':shortest=1,format=yuv420p[v]`,
