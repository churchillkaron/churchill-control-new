...
const TOKEN = "avq-investor-scene-16-software-change-review-v1-20260823";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_16_SOFTWARE_CHANGE_REVIEW_V1";
...
const H = 1080;
const FPS = 24;
const DURATION = 2.954;
const VOICE_START = 72.984;
const VOICE_END = 75.938;
const NARRATION = "And that changes what software can do.";

const BACKGROUND = `${ORG}/unassigned/b1f518b9-e1da-4153-a665-93d1768b42f3-gemini-5sts8qv981mh.mp4`;
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260823/scene-16-software-change-review-v1.mp4`;
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("SCENE_16_REVIEW_TIMEOUT"));
      }
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`SCENE_16_SOURCE_EMPTY:${storagePath}`);
...
async function makePane(uiPath, outputPath) {
  ...
  const frame = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${paneW}" height="${paneH}"><defs><linearGradient id="e" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#f5f3ed" stop-opacity=".48"/><stop offset=".5" stop-color="#d6a66a" stop-opacity=".25"/><stop offset="1" stop-color="#ffffff" stop-opacity=".12"/></linearGradient></defs><rect x="7" y="7" width="${paneW - 14}" height="${paneH - 14}" rx="30" fill="#ffffff" fill-opacity=".012" stroke="url(#e)" stroke-width="1.5"/></svg>`);
  await sharp({ create: { width: paneW, height: paneH, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } }).composite([{ input: masked, left: inset, top: inset }, { input: frame }]).png().toFile(outputPath);
}
...
async function upload(localPath) {
  ...
  const { error } = await supabase.storage.from(BUCKET).upload(OUTPUT, bytes, {
    ...
    metadata: {
      contract: CONTRACT,
      scene: "16",
      role: "ORGANIZATION_INTELLIGENCE_CONVERGENCE",
      narration: NARRATION,
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-scene16-"));
  try {
    const background = path.join(directory, "manager.mp4");
    const ui = path.join(directory, "org-intelligence.png");
    ...
    const final = path.join(directory, "scene16-final.mp4");
    await Promise.all([download(BACKGROUND, background), download(AUTHENTIC_UI, ui), download(VOICE, voice), download(SCORE, score)]);
    await makePane(ui, pane);
    await run(ffmpeg, ["-y", "-stream_loop", "-1", "-ss", "3.1", "-i", background, "-loop", "1", "-framerate", String(FPS), "-i", pane, "-t", String(DURATION), "-filter_complex", `[0:v]scale=1960:1103:force_original_aspect_ratio=increase,crop=${W}:${H}:x='(iw-ow)/2+2*sin(t*0.20)':y='(ih-oh)/2+1*sin(t*0.16)',fps=${FPS},eq=contrast=1.035:brightness=-0.012:saturation=0.90,format=yuv420p[bg];[1:v]format=rgba,fade=t=in:st=0.05:d=0.36:alpha=1,scale=w='if(lt(t,0.90),820+340*(t/0.90),1160)':h=-2:eval=frame[pane];[bg][pane]overlay=x='if(lt(t,0.90),930-430*(t/0.90),500+3*sin(t*0.26))':y='if(lt(t,0.90),430-225*(t/0.90),205+2*sin(t*0.20))':shortest=1,format=yuv420p[v]`, "-map", "[v]", "-an", "-c:v", "libx264", "-preset", "fast", "-crf", "16", "-pix_fmt", "yuv420p", "-r", String(FPS), "-movflags", "+faststart", picture]);
    await run(ffmpeg, ["-y", "-i", picture, "-i", voice, "-i", score, "-filter_complex", `[1:a]atrim=start=${VOICE_START}:end=${VOICE_END},asetpts=PTS-STARTPTS,volume=1.02[voice];[2:a]atrim=start=${VOICE_START}:end=${VOICE_END},asetpts=PTS-STARTPTS,volume=0.09[score];[voice][score]amix=inputs=2:duration=first:dropout_transition=0:normalize=0,alimiter=limit=0.95,atrim=duration=${DURATION}[a]`, "-map", "0:v:0", "-map", "[a]", "-t", String(DURATION), "-c:v", "copy", "-c:a", "aac", "-b:a", "320k", "-ar", "48000", "-ac", "2", "-movflags", "+faststart", final]);
    const stored = await upload(final);
    return { success: true, contract: CONTRACT, scene: 16, role: "ORGANIZATION_INTELLIGENCE_CONVERGENCE", duration_seconds: DURATION, narration: NARRATION, output_ready: true, output_path: OUTPUT, signed_url: await signedUrl(OUTPUT), rules: { dominant_full_screen_ui: false, synthetic_product_ui: false, fake_ui_present: false, card_wall_present: false, orb_present: false, image_generation_used: false, churchill_present: false }, ...stored };
  } finally {
...
export async function GET(request) {
  try {
    ...
    if (action === "status") return json({ success: true, contract: CONTRACT, scene: 16, output_ready: ready, output_path: OUTPUT });
