...
const TOKEN = "avq-investor-scene-19-customer-communications-review-v1-20260823";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_19_CUSTOMER_COMMUNICATIONS_REVIEW_V1";
...
const VOICE_START = 87.188;
const VOICE_END = 92.813;
...
const BACKGROUND = `${ORG}/unassigned/bd497620-f97e-48c1-9bf7-eb9ae95182e8-avantiqo-investor-manager-8b3e0ebb-860d-49a2-b477-49f93eccc620.mp4`;
const BACKGROUND_ASSET_ID = "4e207b74-03fa-49fa-8605-db781404181a";
const AUTHENTIC_UI = `${ORG}/avantiqo-investor-film-20260820/ui/customer_communications.png`;
const AUTHENTIC_UI_ASSET_ID = "6be3322f-1169-4d9e-86d9-ce8f67d26222";
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260823/scene-19-customer-communications-review-v1.mp4`;
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("SCENE_19_REVIEW_TIMEOUT"));
      }
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`SCENE_19_SOURCE_EMPTY:${storagePath}`);
...
async function makePane(uiPath, outputPath) {
  ...
  const glass = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${paneW}" height="${paneH}">
    <defs><linearGradient id="edge" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#f7f5ef" stop-opacity=".56"/><stop offset=".5" stop-color="#d6a66a" stop-opacity=".30"/><stop offset="1" stop-color="#ffffff" stop-opacity=".14"/></linearGradient></defs>
    <rect x="7" y="7" width="${paneW - 14}" height="${paneH - 14}" rx="30" fill="#ffffff" fill-opacity=".014" stroke="url(#edge)" stroke-width="1.6"/>
    <path d="M28 17 H${paneW - 38}" stroke="#ffffff" stroke-opacity=".22" stroke-width="2" stroke-linecap="round"/>
  </svg>`);
...
async function makeConnectionTrace(outputPath) {
  const svg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    <defs>
      ...
      <linearGradient id="g" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#f8f4e9" stop-opacity=".58"/><stop offset=".46" stop-color="#d6a66a" stop-opacity=".38"/><stop offset="1" stop-color="#d6a66a" stop-opacity="0"/></linearGradient>
    </defs>
    <path d="M1190 570 C1430 555 1630 590 1990 680" fill="none" stroke="url(#g)" stroke-width="1.5" stroke-linecap="round" filter="url(#soft)"/>
    <path d="M1190 570 C1430 555 1630 590 1990 680" fill="none" stroke="#e6c98b" stroke-opacity=".44" stroke-width="1" stroke-linecap="round"/>
  </svg>`);
...
async function upload(localPath) {
  ...
  const { error } = await supabase.storage.from(BUCKET).upload(OUTPUT, bytes, {
    ...
    metadata: {
      ...
      scene: "19",
      role: "CUSTOMER_COMMUNICATIONS_IN_SHARED_CONTEXT",
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-scene19-comms-"));
  try {
    ...
    const final = path.join(directory, "scene19-final.mp4");
    ...
    await Promise.all([makePane(ui, pane), makeConnectionTrace(trace)]);

    await run(ffmpeg, [
      "-y",
      "-stream_loop", "-1", "-ss", "1.5", "-i", background,
      "-loop", "1", "-framerate", String(FPS), "-i", pane,
      "-loop", "1", "-framerate", String(FPS), "-i", trace,
      "-t", String(DURATION),
      "-filter_complex",
      `[0:v]scale=1960:1103:force_original_aspect_ratio=increase,crop=${W}:${H}:x='(iw-ow)/2+3*sin(t*0.20)':y='(ih-oh)/2+2*sin(t*0.15)',fps=${FPS},eq=contrast=1.035:brightness=-0.010:saturation=0.91,format=yuv420p[bg];` +
      `[1:v]format=rgba,fade=t=in:st=0.06:d=0.38:alpha=1,fade=t=out:st=4.70:d=0.55:alpha=1,scale=w='if(lt(t,1.05),720+430*(t/1.05),1150)':h=-2:eval=frame[pane];` +
      `[bg][pane]overlay=x='if(lt(t,1.05),1010-505*(t/1.05),505+4*sin(t*0.27))':y='if(lt(t,1.05),445-235*(t/1.05),210+3*sin(t*0.20))'[withpane];` +
      `[2:v]format=rgba,fade=t=in:st=0.90:d=0.38:alpha=1,fade=t=out:st=4.55:d=0.45:alpha=1[trace];` +
      `[withpane][trace]overlay=0:0:shortest=1,format=yuv420p[v]`,
      "-map", "[v]", "-an",
      "-c:v", "libx264", "-preset", "fast", "-crf", "16", "-pix_fmt", "yuv420p", "-r", String(FPS), "-movflags", "+faststart", picture,
    ]);
    ...
    await run(ffmpeg, [
      ...
      "-c:v", "copy", "-c:a", "aac", "-b:a", "320k", "-ar", "48000", "-ac", "2", "-movflags", "+faststart", final,
    ]);
    ...
    return {
      ...
      rules: {
        approved_manager_background: true,
        authentic_customer_communications_ui: true,
        connected_conversation_trace: true,
        dominant_full_screen_ui: false,
        synthetic_product_ui: false,
        fake_ui_present: false,
...
export async function GET(request) {
  try {
    ...
    if (action === "status") return json({ success: true, contract: CONTRACT, scene: 19, output_ready: ready, output_path: OUTPUT });
