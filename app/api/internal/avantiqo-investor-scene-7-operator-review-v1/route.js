...
const TOKEN = "avq-investor-scene-7-operator-review-v1-20260822";
const CONTRACT = "AVANTIQO_INVESTOR_SCENE_7_OPERATOR_REVIEW_V1";
...
const SOURCE = `${ORG}/unassigned/8fce813d-68ac-4032-918e-0eee89871265-gemini-q1zghwo9x4g8.mp4`;
...
const OUTPUT = `${ORG}/${PROJECT}/scene-previews-20260822/scene-07-operator-integration-review-v1.mp4`;
...
function run(command, args, timeoutMs = 285000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("SCENE_7_REVIEW_TIMEOUT"));
      }
    }, timeoutMs);
...
async function download(storagePath, localPath) {
  ...
  if (!data) throw new Error(`SCENE_7_SOURCE_EMPTY:${storagePath}`);
...
async function render() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-scene7-review-"));
  try {
    const source = path.join(directory, "operator.mp4");
    ...
    const final = path.join(directory, "scene7-review.mp4");
    ...
    await run(ffmpeg, [
      ...
      "-an",
      "-vf",
      `scale=1960:1103:force_original_aspect_ratio=increase,crop=${W}:${H}:x='(iw-ow)/2+3*sin(t*0.28)':y='(ih-oh)/2+2*sin(t*0.19)',fps=${FPS},eq=contrast=1.035:brightness=-0.008:saturation=0.90,format=yuv420p`,
      "-c:v", "libx264",
      "-preset", "fast",
      "-crf", "16",
      "-pix_fmt", "yuv420p",
    ...
    const scoreFadeOutStart = Math.max(0, DURATION - 0.45);
    await run(ffmpeg, [
      "-y",
      "-i", voice,
      "-i", score,
      "-filter_complex",
      `[0:a]atrim=start=${NARRATION_START}:duration=${DURATION},asetpts=PTS-STARTPTS,volume=1.03[voice];[1:a]atrim=start=0:duration=${DURATION},asetpts=PTS-STARTPTS,afade=t=in:st=0:d=0.35,afade=t=out:st=${scoreFadeOutStart}:d=0.45,volume=0.105[score];[voice][score]amix=inputs=2:duration=first:dropout_transition=0:normalize=0,alimiter=limit=0.95,atrim=duration=${DURATION}[a]`,
      "-map", "[a]",
      "-c:a", "aac",
      "-b:a", "320k",
      "-ar", "48000",
...
export async function GET(request) {
  try {
    ...
    if (action === "status") return json({ success: true, contract: CONTRACT, scene: 7, output_ready: ready, output_path: OUTPUT });
