...
const TOKEN = "churchill-night-changes-v3-dart-r2c-20260822";
const PROVIDER = "gemini";
const MODEL = "gemini-omni-flash-preview";
const VERSION = "CHURCHILL_V3_DART_R2C_SAFE_SPORT_3S";
const TRUTH_ASSET_ID = "7bc9e891-e3d0-4b03-8b53-95ff255f31c6";
const TRUTH_ROLE = "AUTHENTIC_POOL_ELECTRIC_DARTS_GEOMETRY_TRUTH";

const COMMON = [
  "Premium photoreal hospitality cinema with restrained commercial lighting.",
  "The supplied Churchill games-room reference is immutable geometry truth.",
  "Preserve the exact orange and amber Churchill pool tables and the real electronic scoring cabinets with illuminated circular targets and scoring screens.",
  "This is a recreational soft-tip electronic target game inside a restaurant and bar.",
  "Keep all furniture, walls, proportions, cabinet geometry, screen placement and practical lighting unchanged.",
  "No people, no room redesign, no futuristic interface, no fantasy effect, no particles, no fake signage and no fake logo.",
  "Camera movement is minimal, smooth and deliberate.",
].join(" ");

const SHOTS = Object.freeze({
  dart_midflight_r2b: Object.freeze({
    seconds: 3,
    prompt: `${COMMON} Use a compressed long-lens commercial perspective in this exact Churchill room. One small soft-tip plastic game piece crosses the middle depth toward the authentic electronic scoring cabinet. Camera performs only a subtle controlled forward drift. The orange pool table remains foreground context and the electronic cabinet, illuminated circular target and scoring screens stay clearly recognisable. Keep motion physically plausible and elegant.`,
  }),
  dart_impact_r2b: Object.freeze({
    seconds: 3,
    prompt: `${COMMON} Compose a refined telephoto push toward the exact authentic electronic scoring cabinet. One small soft-tip plastic game piece reaches the illuminated circular scoring target and registers a score. Keep the cabinet and scoring screens clearly visible. At the scoring moment allow only a very brief practical exposure bloom from the existing target illumination, suitable for a match cut into the real Churchill stage spotlight.`,
  }),
});
...
async function project() {
  ...
  if (!mission?.id) throw new Error("CHURCHILL_V3_DART_R2C_MISSION_REQUIRED");
  ...
  if (!data) throw new Error("CHURCHILL_V3_DART_R2C_PROJECT_REQUIRED");
  if (data.metadata?.canonical_story_version !== CHURCHILL_NIGHT_CHANGES_STORY_VERSION) {
    throw new Error("CHURCHILL_V3_DART_R2C_STORY_VERSION_MISMATCH");
  }
  return data;
}
...
async function validateTruthAsset() {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) throw new Error("CHURCHILL_V3_DART_R2C_TRUTH_ASSET_MISSING");
  if (data.ai_generated === true) throw new Error("CHURCHILL_V3_DART_R2C_TRUTH_ASSET_MUST_BE_REAL");
  if (data.metadata?.truth_role !== TRUTH_ROLE) throw new Error("CHURCHILL_V3_DART_R2C_TRUTH_ROLE_INVALID");
}
...
async function credentialId() {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data?.id) throw new Error("CHURCHILL_V3_DART_R2C_GEMINI_CREDENTIAL_REQUIRED");
  return data.id;
}

function root(metadata = {}) {
  return metadata.churchill_v3_dart_r2c || {};
}

async function patch(p, key, value, status = "R2C_ACTIVE") {
  ...
  const next = {
    ...
    strategy: "TWO_SHORT_SAFE_SPORT_PLATES_WITH_AUTHENTIC_GEOMETRY",
  ...
  const { error } = await supabaseAdmin
    .from("creative_projects")
    .update({ metadata: { ...metadata, churchill_v3_dart_r2c: next }, updated_at: new Date().toISOString() })
    .eq("id", p.id)
    .eq("organization_id", ORGANIZATION_ID);
  if (error) throw error;
}
...
async function start(key) {
  ...
  if (!shot) throw new Error("CHURCHILL_V3_DART_R2C_SHOT_INVALID");
  const p = await project();
  await validateTruthAsset();
  const current = root(p.metadata).generations?.[key] || null;
  if (current?.version === VERSION && current?.status === "COMPLETED" && current?.output_reference) {
    return { success: true, reused: true, key, state: current };
  }
  if (current?.version === VERSION && current?.status === "PROCESSING" && current?.provider_job_id && current?.usage_id) {
    return { success: true, reused: true, key, state: current };
  }
  ...
  const result = await executeService({
    ...
    metadata: {
      module: "CREATIVE",
      operation: `CHURCHILL_V3_DART_R2C_${key.toUpperCase()}`,
...
async function poll(key) {
  if (!SHOTS[key]) throw new Error("CHURCHILL_V3_DART_R2C_SHOT_INVALID");
  const p = await project();
  const current = root(p.metadata).generations?.[key] || null;
  if (!current) throw new Error("CHURCHILL_V3_DART_R2C_NOT_STARTED");
  ...
  const result = await settlePendingService({
    ...
    metadata: {
      module: "CREATIVE",
      operation: `CHURCHILL_V3_DART_R2C_${key.toUpperCase()}_POLL`,
      command_identity: COMMAND_IDENTITY,
      version: VERSION,
      canonical_story_version: CHURCHILL_NIGHT_CHANGES_STORY_VERSION,
      creative_project_id: p.id,
  ...
  if (result?.failed) {
    const failed = {
      ...
      error: result.error || "R2C failed",
      completed_at: new Date().toISOString(),
    };
    await patch(p, key, failed, "R2C_FAILED");
  ...
  const outputReference =
    ...
    null;
  if (!outputReference) throw new Error("CHURCHILL_V3_DART_R2C_OUTPUT_REQUIRED");
  const completed = {
    ...current,
    status: "COMPLETED",
  ...
  await patch(p, key, completed, "R2C_REVIEW_REQUIRED");
...
export async function GET(request) {
  try {
    ...
    console.error("CHURCHILL_V3_DART_R2C_FAILED", { message: error?.message || String(error) });
