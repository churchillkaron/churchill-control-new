...
function run(command, args, timeoutMs = 180000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("CHURCHILL_V5_QC_MEDIA_TIMEOUT"));
      }
    }, timeoutMs);
    ...
    child.on("close", (code) => {
      ...
      if (code === 0) {
        ...
        reject(new Error(Buffer.concat(stderr).toString("utf8").slice(-12000) || `CHURCHILL_V5_QC_MEDIA_EXIT_${code}`));
      }
    });
  });
}
...
function sceneAt(seconds) {
  return TIMELINE.find((scene) => seconds >= scene.start && seconds < scene.end) || TIMELINE[TIMELINE.length - 1];
}
...
async function extractFrames(ffmpeg, master, directory) {
  ...
  await run(ffmpeg, [
    "-y",
    "-ss", String(SAMPLE_OFFSET),
    "-i", master,
    "-vf", `fps=1/${SAMPLE_STEP},scale=${FRAME_WIDTH}:${FRAME_HEIGHT}:force_original_aspect_ratio=increase,crop=${FRAME_WIDTH}:${FRAME_HEIGHT}`,
  ...
  for (let index = 0; index < SAMPLE_COUNT; index += 1) {
    ...
    const source = path.join(directory, `frame-${String(index + 1).padStart(2, "0")}.jpg`);
    const buffer = await sharp(source)
      .composite([{ input: labelSvg(`${scene.key.slice(6, 8)}  ${seconds.toFixed(1)}s`), top: 0, left: 0 }])
...
async function makeSheets(frames) {
  ...
  for (let sheetIndex = 0; sheetIndex < 3; sheetIndex += 1) {
    ...
    const composites = group.map((frame, index) => ({
      input: frame.buffer,
      left: (index % 4) * FRAME_WIDTH,
      top: Math.floor(index / 4) * FRAME_HEIGHT,
    }));
    const buffer = await sharp({
      create: { width: FRAME_WIDTH * 4, height: FRAME_HEIGHT * 3, channels: 3, background: "#080808" },
    })
    ...
    sheets.push({
      index: sheetIndex + 1,
      buffer,
      samples: group.map(({ seconds, scene_key, scene_label }) => ({ seconds, scene_key, scene_label })),
    });
  }
  return sheets;
}
...
async function uploadSheets(p, masterChecksum, sheets) {
  ...
  for (const sheet of sheets) {
    ...
    const { error } = await supabaseAdmin.storage.from(BUCKET).upload(target, sheet.buffer, {
      ...
      metadata: {
        ...
        qc_contract: CONTRACT,
        master_checksum: masterChecksum,
        non_generative_frame_extraction: "true",
        sheet_index: String(sheet.index),
      },
    });
    if (error) throw error;
    evidence.push({
      ...
      signed_url: await signed(target, 3600),
    });
  }
  return evidence;
}
...
function reviewPrompt(evidence) {
  const sampleMap = evidence.flatMap((sheet) => sheet.samples.map((sample) => ({
...
AUTHENTICITY IS A HARD CONTRACT:
...
HARD FAILS:
1. traditional/sisal/bristle/cork dartboard or wrong darts identity;
2. fake/malformed Churchill logo or readable invented branding;
3. warped/melting venue geometry or obvious synthetic room replacement;
4. malformed anatomy, duplicated people, broken faces/hands/limbs or synthetic identity replacement;
5. visibly rubbery/liquid object motion implied by sampled progression, physically impossible deformation, or obvious AI shimmer/texture collapse;
6. cheap neon/cyberpunk/hologram/UI language that is not part of the real venue;
7. frozen/repeated/blank/black material that makes a scene look broken or padded;
8. portrait-source treatment that looks amateur, overly blurred, badly scaled or compositionally disconnected;
9. transition discontinuity that visibly breaks the one-night cinematic flow;
10. logo epilogue that is not clean, premium and brand-faithful.

Do not invent defects not visible in the sampled frames. Still-frame evidence cannot fully prove motion smoothness or audio quality: identify those as human-motion/audio review items instead of hallucinating a failure.

Score EACH of the 15 scenes 0-100 for:
- cinematic_quality
- authenticity
- composition
- continuity
...
WORLD-CLASS PASS threshold per scene:
cinematic_quality >= 90; authenticity >= 95; composition >= 90; continuity >= 90; synthetic_artifact_freedom >= 94; brand_integrity >= 95; and zero hard_fail issues.
...
Return STRICT JSON only:
{
  ...
  "scenes":[
    {
      "scene_key":"scene_01_the_drop",
      "verdict":"PASS|FAIL|INSUFFICIENT_EVIDENCE",
      "scores":{"cinematic_quality":0,"authenticity":0,"composition":0,"continuity":0,"synthetic_artifact_freedom":0,"brand_integrity":0},
      "hard_fail_issues":[{"time_seconds":0,"issue":"specific visible defect","severity":"BLOCKER|MAJOR"}],
      "strengths":[],
      "issues":[],
      "repair_instructions":[]
    }
  ],
  "cross_scene_continuity_issues":[],
  "release_blockers":[],
  "human_motion_audio_review_items":[],
...
async function patchQc(p, evidence, review, execution) {
  ...
  const qc = {
    contract: CONTRACT,
    status: review?.verdict === "PASS" ? "MACHINE_PASS_HUMAN_REVIEW_REQUIRED" : "REPAIR_REQUIRED",
    review,
    evidence: evidence.map(({ signed_url, ...item }) => item),
    provider: execution?.provider || "openai",
    model: execution?.model || "gpt-4.1-mini",
    usage_id: execution?.usage?.id || null,
    machine_visual_review_complete: true,
  ...
  const { error } = await supabaseAdmin
    .from("creative_projects")
    .update({ metadata: { ...metadata, churchill_v5_qc: qc }, updated_at: new Date().toISOString() })
    .eq("id", p.id)
    .eq("organization_id", ORGANIZATION_ID);
  if (error) throw error;
  return qc;
...
async function executeReview() {
  ...
  if (master?.status !== "REVIEW_REQUIRED" || !master?.output_reference) {
    throw new Error("CHURCHILL_V5_QC_MASTER_REVIEW_OUTPUT_REQUIRED");
  }
  ...
  try {
    ...
    const sheets = await makeSheets(frames);
    const evidence = await uploadSheets(p, masterChecksum, sheets);
    ...
    const execution = await runAIService.execute({
      ...
      input: {
        ...
        model: "gpt-4.1-mini",
        assets: evidence.map((item) => item.signed_url),
        quantity: 1,
        prompt: reviewPrompt(evidence),
        temperature: 0,
        max_output_tokens: 8000,
      },
      metadata: {
        module: "CREATIVE",
        operation: "CHURCHILL_V5_WORLD_CLASS_MASTER_VISION_GATE",
        creative_project_id: p.id,
        master_checksum: masterChecksum,
        evidence_paths: evidence.map((item) => item.storage_path),
        non_generative_review_evidence: true,
      },
      category: "AI",
    });
    ...
    const qc = await patchQc(p, evidence, review, execution);
...
async function status() {
  ...
  return {
    ...
    qc: p.metadata?.churchill_v5_qc || null,
    policy: {
      evidence_is_non_generative_frame_extraction: true,
      machine_pass_does_not_equal_human_approval: true,
      approved_for_master: false,
      publication_authorized: false,
    },
  };
