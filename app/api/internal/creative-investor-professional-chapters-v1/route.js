...
async function render(request) {
  ...
  try { return NextResponse.json(await AvantiqoInvestorFilmProfessionalChaptersV1.render()); }
  catch (error) {
    console.error("INVESTOR_PROFESSIONAL_CHAPTERS_FAILURE", { message:error?.message || null, stack:error?.stack || null });
    return NextResponse.json({ success:false, contract:AvantiqoInvestorFilmProfessionalChaptersV1.CONTRACT, error:error?.message || "INVESTOR_PROFESSIONAL_CHAPTERS_FAILED" }, { status:500 });
  }
}
