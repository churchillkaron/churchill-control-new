...
const CONTRACT = "AVANTIQO_INVESTOR_FILM_MASTER_V7_SEMANTIC_STABLE_MICROCHUNK";
const FPS = 24;
const MASTER_SECONDS = 237.5;
const MICRO_ROOT = `${ORG}/${PROJECT}/spatial-master-v7-micro`;
const FINAL_PATH = `${ORG}/${PROJECT}/spatial-master-v7/avantiqo-investor-film-v7-semantic-stable-237.5s.mp4`;
...
const scene = ({ id, media, frames, domain = "", kicker = "AVANTIQO INTELLIGENCE", caps = [], signal = "", anchor = "left-top", mode = "flow", accent = "ice", semantic = "", sourceIn = 0 }) => Object.freeze({
  id, source_reference: storage(MEDIA[media]), source_kind: "video", source_in_seconds: sourceIn,
  duration_seconds: seconds(frames), domain, kicker, capabilities: caps, ai_signal: signal, anchor,
  overlay_mode: mode, accent, source_semantic_role: semantic,
});
...
const S = Object.freeze({
  ...
  world: scene({ id:"c1-world", media:"opening_world", frames:211, domain:"Real Businesses", kicker:"THE ORIGIN", signal:"Running real companies exposes the same disconnected operating problem.", caps:["Customers","People","Money"], anchor:"right-bottom", mode:"quiet", accent:"gold", semantic:"OPENING_WORLD_BUILDING" }),
  ...
  reveal: scene({ id:"c1-reveal", media:"avantiqo_reveal", frames:172, domain:"One Operating Context", kicker:"AVANTIQO", signal:"Organization, evidence and action begin from one shared business context.", caps:["Context","Evidence","Action"], anchor:"right-bottom", mode:"ai", accent:"gold", semantic:"FIRST_AVANTIQO_REVEAL" }),
  founderBuilt: scene({ id:"c1-founder-built", media:"founder_built", frames:61, mode:"none", semantic:"FOUNDER_BUILT_LIPSYNC" }),
  org: scene({ id:"c1-org", media:"manager", frames:182, domain:"Organization Intelligence", kicker:"AVANTIQO AI", signal:"Every action inherits the right organization, entity, period and authority.", caps:["Organization","Entity","Period","Permissions"], anchor:"left-bottom", mode:"ai", semantic:"MANAGER_COMMAND_VIEW" }),
  ...
  supply: scene({ id:"c1-supply", media:"procurement", frames:172, domain:"Supply Chain", kicker:"PROCUREMENT -> RECEIVING", signal:"Demand, purchasing and receiving stay connected to evidence and finance.", caps:["Request","Purchase Order","Goods Receipt"], anchor:"left-top", mode:"flow", accent:"gold", semantic:"PROCUREMENT_RECEIVING" }),
  finance: scene({ id:"c2-finance", media:"finance", frames:264, domain:"Finance", kicker:"FINANCIAL CORE", signal:"The ledger, cash, invoices and governance share the same operating context.", caps:["Ledger","Cash","Invoices","Controls"], anchor:"right-bottom", mode:"ai", accent:"gold", semantic:"FINANCE_LEDGER" }),
  ...
  opsAction: scene({ id:"c2-ops-action", media:"kitchen_alt", frames:222, domain:"Attention -> Action", kicker:"OPERATIONS", signal:"Signals become accountable work with ownership, status and evidence.", caps:["Attention","Assign","Execute","Complete"], anchor:"left-top", mode:"flow", semantic:"KITCHEN_EXECUTION_ALT" }),
  aiTransition: scene({ id:"c2-ai-transition", media:"avantiqo_reveal", frames:102, domain:"Understand -> Recommend -> Approve -> Execute", kicker:"AVANTIQO INTELLIGENCE", signal:"AI is useful when it can reason inside real business context and control.", anchor:"right-bottom", mode:"ai", accent:"gold", semantic:"FIRST_AVANTIQO_REVEAL" }),
  field: scene({ id:"c2-field", media:"field_a", frames:182, domain:"Field Service", kicker:"PEST CONTROL OPERATIONS", signal:"Dispatch, on-site work, evidence and completion form one traceable service flow.", caps:["Dispatch","Work","Evidence","Complete"], anchor:"right-bottom", mode:"flow", semantic:"FIELD_SERVICE_PEST_CONTROL_A" }),
  restaurant: scene({ id:"c2-restaurant", media:"restaurant_alt", frames:116, domain:"Restaurant", kicker:"CROSS-INDUSTRY", signal:"Orders, service and operating activity can run on the same architecture.", anchor:"left-bottom", mode:"quiet", accent:"gold", semantic:"RESTAURANT_WAITER_ORDER_ALT" }),
  ...
  fieldComplete: scene({ id:"c2-field-complete", media:"field_complete", frames:117, domain:"Connected Service", kicker:"FIELD EXECUTION", signal:"The same context follows the job from assignment through completion evidence.", caps:["Assigned","On Site","Verified"], anchor:"right-bottom", mode:"flow", semantic:"FIELD_SERVICE_JOB_COMPLETION" }),
  architecture: scene({ id:"c2-architecture", media:"avantiqo_reveal", frames:70, domain:"One Operating Architecture", kicker:"AVANTIQO", signal:"Different industries. One governed platform foundation.", anchor:"right-bottom", mode:"ai", accent:"gold", semantic:"FIRST_AVANTIQO_REVEAL" }),
  ...
  commercial: scene({ id:"c3-commercial", media:"restaurant", frames:105, domain:"Commercial", kicker:"SALE", signal:"A customer action enters the operating chain.", caps:["Customer","Order"], anchor:"right-top", mode:"quiet", semantic:"RESTAURANT_WAITER_ORDER" }),
  financeFlow: scene({ id:"c3-finance", media:"finance", frames:103, domain:"Finance", kicker:"SALE -> FINANCE", signal:"Revenue and financial evidence update in the same context.", caps:["Revenue","Evidence"], anchor:"left-bottom", mode:"quiet", accent:"gold", semantic:"FINANCE_LEDGER" }),
  supplyFlow: scene({ id:"c3-supply", media:"procurement", frames:103, domain:"Supply", kicker:"FINANCE -> SUPPLY", signal:"Demand and stock implications remain connected to purchasing and receiving.", caps:["Demand","Supply"], anchor:"right-top", mode:"quiet", semantic:"PROCUREMENT_RECEIVING" }),
  people: scene({ id:"c3-people", media:"people", frames:105, domain:"People", kicker:"SUPPLY -> PEOPLE", signal:"Responsibility and authority stay attached to the work.", caps:["Owner","Approval"], anchor:"right-bottom", mode:"quiet", semantic:"PEOPLE_ROSTER" }),
  ...
  governanceA: scene({ id:"c3-governance-a", media:"manager", frames:152, domain:"Governed AI", kicker:"CONTEXT + AUTHORITY", signal:"AI reasons inside organization context, permissions and workflow state.", caps:["Context","Permissions","Workflow"], anchor:"left-bottom", mode:"ai", accent:"gold", semantic:"MANAGER_COMMAND_VIEW" }),
  ...
  intelligence: scene({ id:"c3-intelligence", media:"avantiqo_reveal", frames:81, domain:"Avantiqo Intelligence", kicker:"UNDERSTAND THE BUSINESS", signal:"The goal is not another chatbot. It is intelligence grounded in how the business actually operates.", anchor:"right-bottom", mode:"ai", accent:"gold", semantic:"FIRST_AVANTIQO_REVEAL" }),
  ...
  strategyField: scene({ id:"c4-strategy-field", media:"field_c", frames:75, domain:"Enter Through Pain", kicker:"EXPANSION STRATEGY", signal:"Start with one painful workflow and prove value.", anchor:"right-bottom", mode:"quiet", accent:"gold", semantic:"FIELD_SERVICE_PEST_CONTROL_C" }),
...
if (TOTAL_FRAMES !== 5700) throw new Error(`INVESTOR_V7_MICRO_TIMELINE_INVALID:${TOTAL_FRAMES}`);
...
async function project(){const {data,error}=await supabaseAdmin.from("creative_projects").select("*").eq("id",PROJECT).eq("organization_id",ORG).maybeSingle();if(error)throw error;if(!data)throw new Error("INVESTOR_V7_PROJECT_NOT_FOUND");return data;}
async function signed(storagePath,expires=21600){const {data,error}=await supabaseAdmin.storage.from(BUCKET).createSignedUrl(storagePath,expires);if(error)throw error;if(!data?.signedUrl)throw new Error(`INVESTOR_V7_SIGNED_URL_MISSING:${storagePath}`);return data.signedUrl;}
...
async function renderChunk(index){
  const def=CHUNKS.find(c=>c.index===index); if(!def)throw new Error("INVESTOR_V7_CHUNK_INVALID");
  const p=await project();
  for(const item of def.scenes){const storagePath=item.source_reference.replace(`storage://${BUCKET}/`,"");if(!(await exists(storagePath)))throw new Error(`INVESTOR_V7_SOURCE_MISSING:${item.id}`);}
  ...
  const output=execution?.output; if(!output?.buffer?.length)throw new Error(`INVESTOR_V7_CHUNK_EMPTY:${index}`);
  const stored=await uploadVideo(output.buffer,chunkPath(index),{microchunk:true,chunk_index:index,duration_seconds:def.duration,total_frames:def.frames,semantic_sync:true,subject_safe_overlays:true,whole_scene_fade_to_black:false,founder_lipsync_targeted:true,opening_visible_lipsync_max_seconds:5.208333333,visual_language:output.visual_language||"STABLE_SUBJECT_SAFE_INTELLIGENCE_V2"});
...
async function renderFinal(){
  const ffmpeg=resolveCreativeFfmpegPath(),ffprobe=resolveCreativeFfprobePath(); if(!ffmpeg||!ffprobe)throw new Error("INVESTOR_V7_MEDIA_BINARY_NOT_READY");
  const p=await project(); for(const def of CHUNKS)if(!(await exists(chunkPath(def.index))))throw new Error(`INVESTOR_V7_CHUNK_NOT_READY:${def.index}`); if(!(await exists(NARRATION))||!(await exists(SCORE)))throw new Error("INVESTOR_V7_AUDIO_SOURCE_MISSING");
  ...
  const directory=await fs.mkdtemp(path.join(os.tmpdir(),"avantiqo-v7-micro-final-")); const list=path.join(directory,"chunks.txt"),visual=path.join(directory,"visual.mp4"),output=path.join(directory,"master.mp4");
  try{
    ...
    if(!video||!audio)throw new Error("INVESTOR_V7_AV_REQUIRED"); if(Number(video.width)!==1920||Number(video.height)!==1080)throw new Error("INVESTOR_V7_DIMENSIONS_INVALID"); if((video.r_frame_rate||video.avg_frame_rate)!=="24/1")throw new Error(`INVESTOR_V7_FPS_INVALID:${video.r_frame_rate||video.avg_frame_rate}`); if(Math.abs(duration-MASTER_SECONDS)>0.25)throw new Error(`INVESTOR_V7_DURATION_INVALID:${duration}`);
    const bytes=await fs.readFile(output),stored=await uploadVideo(bytes,FINAL_PATH,{final_master:true,microchunk_transport:true,cfr_24fps:true,semantic_sync:true,subject_safe_overlays:true,founder_visible_speaking:true,targeted_lipsync_repair:true,whole_scene_fade_to_black:false,narration_delay_seconds:8});
    ...
    const next={contract:CONTRACT,status:"RENDERED_REVIEW_REQUIRED",storage_path:FINAL_PATH,duration_seconds:duration,logo_seconds:8,narration_seconds:229.5,frame_rate:video.r_frame_rate||video.avg_frame_rate,semantic_visual_sync:true,subject_safe_overlays:true,whole_scene_fade_to_black:false,founder_visible_speaking:true,targeted_lipsync_repair:true,opening_visible_lipsync_max_seconds:seconds(125),microchunk_count:CHUNKS.length,checksum:stored.checksum,bytes:stored.bytes,technical_qc,updated_at:new Date().toISOString()};
    const {error}=await supabaseAdmin.from("creative_projects").update({metadata:{...metadata,spatial_investor_master_v7:next},updated_at:new Date().toISOString()}).eq("id",PROJECT).eq("organization_id",ORG); if(error)throw error;
    return{success:true,rendered:true,status:next.status,output_path:FINAL_PATH,signed_url:await signed(FINAL_PATH,86400),duration_seconds:duration,semantic_visual_sync:true,subject_safe_overlays:true,founder_visible_speaking:true,targeted_lipsync_repair:true,technical_qc,checksum:stored.checksum,bytes:stored.bytes};
  }finally{await fs.rm(directory,{recursive:true,force:true}).catch(()=>{});}
}

async function status(){
  const p=await project(),state=p.metadata?.spatial_investor_master_v7||{},chunks=[]; for(const def of CHUNKS)chunks.push({index:def.index,ready:await exists(chunkPath(def.index)),path:chunkPath(def.index),duration_seconds:def.duration,frames:def.frames,scenes:def.scenes.length}); const ready=await exists(FINAL_PATH);
