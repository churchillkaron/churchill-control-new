      ...
      try {
        heartbeatTimer = setInterval(() => {
          void (async () => {
            ...
            const freshRealProgress = Number.isFinite(latestAt) && (Date.now() - latestAt) < 4000;
            ...
            const concreteRepositoryLatest =
              /REPOSITORY_OPERATION_|VERIFICATION_|DETERMINISTIC_|COMMIT_/i.test(latestPhase) ||
              Boolean(latestEvent?.operation_id || latestFilePath || latestEvent?.command);
            if (concreteRepositoryLatest) return;
            if (passSettled) return;
            ...
            await publishCodeAILiveProgress({
                ...
                description: !genericLatest && latestDescription
                  ...
                  : latestFilePath
                    ...
                    : heartbeatCount === 1
                      ...
                      : heartbeatCount === 2
                        ? "No repository operation has arrived yet. I’m preparing a bounded recovery from the preserved mission state instead of leaving this pass idle."
              ...
              console.error("CODE_STUDIO_BACKGROUND_HEARTBEAT_PUBLISH_FAILED", {
                mission_id: missionId,
                error: text(error?.message || error, 700),
              });
            });
          })().catch((error) => {
            console.error("CODE_STUDIO_BACKGROUND_HEARTBEAT_FAILED", {
              mission_id: missionId,
              error: text(error?.message || error, 700),
            });
          });
