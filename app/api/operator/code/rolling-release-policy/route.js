import { createCodeAIRollingReleasePolicyCapability } from "@/lib/platform/capabilities/createCodeAIRollingReleasePolicyCapability";
...
export async function POST(request) {
  try {
    ...
    const context = { organizationId, actor: { id: access.user?.id || access.userId, email: access.user?.email || null }, permissions: access.permissions || [], callerRequest: request, metadata: { source: "CODE_STUDIO", explicit_rolling_policy_request: true } };
    const capability = createCodeAIRollingReleasePolicyCapability();
    capability.authorize({ context });
    const result = await capability.execute({ context, payload: { operation: body.operation } });
