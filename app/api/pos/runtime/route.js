...
export async function GET(request) {
  try {
    ...
    const organization = resolved.organization || { id: resolved.organizationId };
    const currencyCode =
      ...
      null;
    const applicationBinding = resolved.applicationBinding || null;
    const posInstallation = publicPOSInstallation(resolved.posInstallation);
    const templateBinding = resolved.templateBinding || null;
    const actionCapabilities = resolveActionCapabilities(resolved.access);
    ...
    return NextResponse.json({
      ...
      application: {
        ...
        status: resolved.application.status || null,
        context: resolved.application.context || null,
      ...
      terminal: {
        ...
        entity_id:
          applicationRuntime?.entity_id ||
          financialPolicy?.entityId ||
          requestedEntityId ||
          null,
        currency_code: currencyCode,
      },
