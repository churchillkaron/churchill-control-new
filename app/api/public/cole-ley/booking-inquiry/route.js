...
export async function POST(request) {
  try {
    ...
    if (!raw || raw.length > 12_000) {
      return NextResponse.json({ error: "INVALID_REQUEST_SIZE" }, { status: 400 });
    }
    ...
    const sameIpCount = (recent || []).filter(
      (row) => row?.attributes?.source_ip_hash === ipHash,
    ).length;
    if (sameIpCount >= MAX_PER_HOUR) {
      return NextResponse.json({ error: "RATE_LIMITED" }, { status: 429 });
    }

    const fingerprint = sha([
      inquiry.email,
      inquiry.eventDate.slice(0, 10),
    ...
    const duplicate = (duplicates || []).find(
      (row) => row?.attributes?.source_fingerprint === fingerprint,
    );
    if (duplicate) {
      return NextResponse.json({
        success: true,
        inquiry_id: duplicate.id,
        booking_code: duplicate.code,
        idempotent: true,
      });
    }
    ...
    const { data, error } = await supabase
      ...
      .select("id,code,status,scheduled_start,created_at")
      .single();
