...
function responseError(error, status = 500) {
  return Response.json(
    { success: false, error: error?.message || error || "Monitoring rounds could not be loaded." },
...
function projection(occurrence, workOrder) {
  ...
  try {
    ...
    const rows = candidates
      .map((occurrence) => {
        ...
        return workOrder ? projection(occurrence, workOrder) : null;
      })
      .filter(Boolean)
      .filter((row) => normalized(row.industry_key) === "pest_control")
      .sort(
        (a, b) =>
