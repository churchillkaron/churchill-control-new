...
function responseError(error, status = 500) {
  return Response.json(
    { success: false, error: error?.message || error || "Service report communication draft failed." },
...
export async function POST(request, { params }) {
  try {
    ...
    const result = await createServiceReportCommunicationDraft({
      ...
      actorPartyId: resolved.access?.staff?.party_id || resolved.access?.staff?.partyId || null,
