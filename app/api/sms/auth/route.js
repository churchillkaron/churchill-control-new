...
export async function GET(request){
  try{
    ...
    return NextResponse.redirect(new URL(`/workspace/${encodeURIComponent(access.organizationId)}/administration/sms-setup${query}`,url.origin));
  }catch(error){ return NextResponse.json({success:false,error:error?.message || "SMS setup could not start"},{status:500}); }
}
