...
export async function POST(request) {
  try {
    ...
    const { data: sourceRun, error: sourceError } = await supabaseAdmin.from("accounting_engagement_runs").select("*").eq("id", sourceRunId).eq("accounting_firm_id", access.organizationId).maybeSingle();
    ...
    const [templateResult, stepsResult, profileResult, engagementResult] = await Promise.all([
      ...
      supabaseAdmin.from("accounting_engagements").select("entity_id").eq("id", sourceRun.engagement_id).eq("accounting_firm_id", access.organizationId).maybeSingle(),
    ]);
    ...
    const entityId = sourceRun.entity_id || engagementResult.data?.entity_id || sourceRun.metadata?.entity_id || null;
    if (!entityId) return jsonError("Source work program is not bound to a legal entity", 409);
    const metadata = { template_version: template.version, rolled_from_run_id: sourceRun.id, entity_id: entityId };

    const { data: nextRun, error: runError } = await supabaseAdmin.from("accounting_engagement_runs").insert({
    ...
    try {
      ...
      const requestRows = (workItems || []).filter((item) => item.work_type === "CLIENT_REQUEST").map((item) => ({
        accounting_firm_id: access.organizationId,
        organization_id: sourceRun.organization_id,
        entity_id: entityId,
        run_id: nextRun.id,
      ...
      if (requestRows.length) { const { error: requestsError } = await supabaseAdmin.from("accounting_client_requests").insert(requestRows); if (requestsError) throw requestsError; }
      ...
      const { error: auditError } = await supabaseAdmin.from("organization_audit_logs").insert({
        ...
        after_data: nextRun,
        metadata: { source_run_id: sourceRun.id, template_id: sourceRun.template_id, legal_entity_id: entityId, evidence_carried_forward: false, budget_minutes: workRows.reduce((total, row) => total + Number(row.budget_minutes || 0), 0) },
        actor_email: access.user?.email || null,
      });
      if (auditError) throw auditError;

      return NextResponse.json({ success: true, run: nextRun, work_items: workItems || [], client_requests_created: requestRows.length, evidence_carried_forward: false, budget_minutes: workRows.reduce((total, row) => total + Number(row.budget_minutes || 0), 0) }, { status: 201 });
    } catch (error) {
