...
async function getTemplate(access, templateId) {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data || (data.organization_id && data.organization_id !== access.organizationId)) {
    throw new Error("Work program template is unavailable for this firm");
  }
  return data;
}
...
export async function GET(request) {
  try {
    ...
    let runsQuery = supabaseAdmin
      ...
      .select("id,organization_id,engagement_id,template_id,period_id,run_key,cadence,status,start_at,due_at,completed_at,rolled_from_run_id,created_at,updated_at")
      .eq("accounting_firm_id", access.organizationId)
    ...
    const [stepsResult, workItemsResult, requestsResult] = await Promise.all([
      templateIds.length
        ? supabaseAdmin
            ...
            .select("id,template_id,step_key,sequence_no,title,description,work_type,required_role,relative_due_days,due_anchor,dependency_step_keys,capability_id,evidence_required")
      ...
      runIds.length
        ? supabaseAdmin
            ...
            .select("id,run_id,step_key,sequence_no,title,description,work_type,required_role,assigned_to,status,start_at,due_at,completed_at,blocked_reason,dependency_step_keys,capability_id,finance_review_item_id,evidence,conclusion,metadata")
            .eq("accounting_firm_id", access.organizationId)
            .in("run_id", runIds)
            .order("sequence_no", { ascending: true })
        : Promise.resolve({ data: [], error: null }),
      runIds.length
        ? supabaseAdmin
            ...
            .select("id,run_id,work_item_id,title,instructions,status,due_at,sent_at,submitted_at,accepted_at,reminder_policy,client_response,metadata")
            .eq("accounting_firm_id", access.organizationId)
            .in("run_id", runIds)
            .order("due_at", { ascending: true, nullsFirst: false })
        : Promise.resolve({ data: [], error: null }),
    ...
    for (const clientRequest of requestsResult.data || []) {
    ...
    return NextResponse.json({
      ...
      runs: runs.map((run) => ({
        ...run,
        work_items: itemsByRun.get(run.id) || [],
        client_requests: requestsByRun.get(run.id) || [],
      })),
...
export async function POST(request) {
  try {
    ...
    if (!engagementId || !templateId || !runKey || !periodEnd) {
      return jsonError("engagementId, templateId, runKey and periodEnd are required");
    }
    ...
    const [engagement, template] = await Promise.all([
      getEngagement(access, engagementId),
      getTemplate(access, templateId),
    ]);

    const { data: profile, error: profileError } = await supabaseAdmin
      .from("accounting_client_profiles")
      .select("assigned_accountant_id,assigned_reviewer_id")
      .eq("accounting_firm_id", access.organizationId)
    ...
    const { data: run, error: runError } = await supabaseAdmin
      ...
      .single();
    if (runError) {
      if (runError.code === "23505") return jsonError("This engagement run already exists", 409);
      throw runError;
    }

    try {
      const workItems = (steps || []).map((step, index) => {
        ...
        return {
          ...
          metadata: { template_version: template.version, evidence_required: Boolean(step.evidence_required) },
        };
      });
      ...
      const clientRequestRows = (insertedItems || [])
        ...
        .map((item) => ({
          ...
          created_by: access.user?.id || null,
          metadata: { source: "work_program" },
        }));

      if (clientRequestRows.length) {
        const { error: requestsError } = await supabaseAdmin
          .from("accounting_client_requests")
          .insert(clientRequestRows);
      ...
      return NextResponse.json({
        success: true,
        run,
        work_items: insertedItems || [],
        client_requests_created: clientRequestRows.length,
      }, { status: 201 });
    } catch (error) {
