...
export default function AccountingClientPortalPage() {
  ...
  const counts = { requests: data.summary?.open_requests ?? data.requests?.length ?? 0, documents: data.documents?.length || 0, messages: data.summary?.messages ?? data.message_meta?.total ?? data.messages?.length ?? 0, tax: data.filings?.length || 0, billing: data.invoices?.length || 0, approvals: data.approvals?.filter((row) => ["PENDING","SENT","VIEWED"].includes(String(row.status || "").toUpperCase())).length || 0 };
