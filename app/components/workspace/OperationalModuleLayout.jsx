...
export default function OperationalModuleLayout({moduleName,tabs=[],kpis=[],sidePanel=null,children}){
    ...
    <div className="p-6 space-y-6 text-white font-sans">
      ...
      {kpis.length>0&&(
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {kpis.map(kpi=>(
            <div key={kpi.label} className="p-4 bg-white/5 backdrop-blur-lg rounded-2xl flex items-center space-x-3 shadow-lg">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {sidePanel&&(<div className="w-full lg:w-72 p-4 bg-white/5 backdrop-blur-lg rounded-2xl shadow-lg">{sidePanel}</div>)}
      </div>
      <div className="p-4 bg-white/5 backdrop-blur-lg rounded-2xl shadow-inner text-white/70">Operational Summary / Actions</div>
    </div>
  );
}
