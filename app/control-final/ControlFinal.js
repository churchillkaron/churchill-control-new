...
const DRAFT_KEY = 'churchill-control-v6-draft'
...
function getCurrentStock(row) {
  return Math.max(
    Number(row.openingStock || 0) + Number(row.producedQty || 0) - Number(row.soldQty || 0),
    0
  )
}

function getToProduce(row) {
  return Math.max(Number(row.par || 0) - getCurrentStock(row), 0)
}
...
function getDishRevenue(row) {
  return Number(row.soldQty || 0) * Number(row.price || 0)
}

function getDishCost(row) {
  return Number(row.soldQty || 0) * Number(row.cost || 0)
}
...
function normalizeRow(savedRow, baseRow) {
  return {
    ...
    cost: Number(baseRow.cost || 0),
    par: Number(savedRow?.par ?? baseRow.par ?? 0),
    openingStock: Number(savedRow?.openingStock ?? 0),
    soldQty: Number(savedRow?.soldQty ?? 0),
    producedQty: Number(savedRow?.producedQty ?? 0),
  }
}
...
export default function ControlFinal() {
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      try {
        ...
        let localDraft = null
        try {
          localDraft = safeParse(localStorage.getItem(DRAFT_KEY))
        } catch {
          localDraft = null
        }

        if (localDraft?.rows && Array.isArray(localDraft.rows)) {
          const mergedDraftRows = baseRows.map((baseRow) => {
            const savedRow = localDraft.rows.find((r) => r.name === baseRow.name)
            return normalizeRow(savedRow, baseRow)
          })

          if (active) {
            setRows(mergedDraftRows)
            setBusinessDate(localDraft.businessDate || new Date().toISOString().slice(0, 10))
            setCovers(Number(localDraft.covers || 0))
            setDrinkRevenue(Number(localDraft.drinkRevenue || 0))
            setAvgTicketTime(Number(localDraft.avgTicketTime || 0))
            setSecondRoundRate(Number(localDraft.secondRoundRate || 0))
            setComplaints(Number(localDraft.complaints || 0))
            setManagerNotes(localDraft.managerNotes || '')
          }
        }
        ...
        const latest = data[0]
        const parsed = safeParse(latest.dishes)
        ...
        const shouldUseHistory = !localDraft?.rows?.length

        if (shouldUseHistory) {
          const mergedHistoryRows = baseRows.map((baseRow) => {
            const savedRow = parsed.rows.find((r) => r.name === baseRow.name)
            return normalizeRow(savedRow, baseRow)
          })

          setRows(mergedHistoryRows)
          setBusinessDate(latest.date || new Date().toISOString().slice(0, 10))
          setCovers(Number(parsed?.meta?.covers || 0))
          setDrinkRevenue(Number(parsed?.meta?.drinkRevenue || 0))
          setAvgTicketTime(Number(parsed?.meta?.avgTicketTime || 0))
          setSecondRoundRate(Number(parsed?.meta?.secondRoundRate || 0))
          setComplaints(Number(parsed?.meta?.complaints || 0))
          setManagerNotes(parsed?.meta?.managerNotes || '')
        }
      } catch (err) {
        console.error('ControlFinal hydrate failed:', err)
      } finally {
  ...
  }, [hydrated, businessDate, covers, drinkRevenue, avgTicketTime, secondRoundRate, complaints, managerNotes, rows])
  ...
  const totalSold = useMemo(
    () => rows.reduce((sum, row) => sum + Number(row.soldQty || 0), 0),
    [rows]
  )

  const totalToProduce = useMemo(
    () => rows.reduce((sum, row) => sum + getToProduce(row), 0),
    [rows]
  )

  const lowStockCount = useMemo(
    () => rows.filter((row) => getToProduce(row) > 0).length,
    [rows]
  )
  ...
  const averageSold = useMemo(() => {
    const activeRows = rows.filter((row) => Number(row.soldQty || 0) > 0)
    if (!activeRows.length) return 1
    return activeRows.reduce((sum, row) => sum + Number(row.soldQty || 0), 0) / activeRows.length
  }, [rows])
  ...
  const ownerStatus = useMemo(() => {
    ...
    if (score >= 75) {
      return { score, status: 'GOOD', reasons, color: THEME.green }
    }
    if (score >= 45) {
      return { score, status: 'WARNING', reasons, color: THEME.yellow }
    }
    return { score, status: 'BAD', reasons, color: THEME.red }
  ...
  const aiInsights = useMemo(() => {
    ...
    if (foodCostPct > 0.35) {
      insights.push(
        `Food cost is elevated at ${formatPercent(foodCostPct)}. Focus on recipe control and price integrity before close.`
      )
    }

    if (avgTicketTime > 20) {
      insights.push(
        `Average ticket time is ${formatNumber(avgTicketTime)} minutes. Review kitchen flow and station coordination.`
      )
    }

    if (secondRoundRate > 0 && secondRoundRate < 0.45) {
      insights.push(
        `Second-round rate is ${formatPercent(secondRoundRate)}. Improve follow-up selling for drinks and desserts.`
      )
    }
      ...
      const payload = {
        ...
        dishes: JSON.stringify({
          ...
          rows: enrichedRows.map((row) => ({
            ...
            toProduce: Number(row.toProduce || 0),
            status: row.status,
        ...
        >
          <div>
            ...
            <div style={{ color: THEME.muted, fontSize: 12, marginTop: 4 }}>
              Daily Control Center | V6 Master
            </div>
          </div>
          ...
          >
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              ...
              <StatMiniCard
                ...
                valueColor={
                  foodCostPct > 0.35 ? THEME.red : foodCostPct > 0.3 ? THEME.yellow : THEME.green
                }
              />
            </div>
