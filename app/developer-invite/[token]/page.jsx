...
export default function DeveloperInvitePage(){
  ...
  useEffect(()=>{let active=true;(async()=>{try{const [{data:auth},response]=await Promise.all([supabase.auth.getSession(),fetch(`/api/developers/access/invitations/${encodeURIComponent(token)}`,{cache:"no-store"})]);const data=await response.json();if(!response.ok||!data?.success)throw new Error(data?.error||"Invitation unavailable");if(active){setInvite(data.invitation);setSession(auth?.session||null);}}catch(e){if(active)setError(e?.message||"Invitation unavailable");}finally{if(active)setLoading(false);}})();return()=>{active=false};},[token]);

  async function createAccount(){
    if(!invite?.email||password.length<8){setError("Use at least 8 characters for your password.");return;}
    setError("");setMessage("");
    const redirectTo=`${window.location.origin}/developer-invite/${encodeURIComponent(token)}`;
    const {data,error:signError}=await supabase.auth.signUp({email:invite.email,password,options:{emailRedirectTo:redirectTo,data:{avantiqo_developer_invite:true}}});
    if(signError){setError(signError.message);return;}
    if(data?.session){setSession(data.session);setMessage("Account ready. Accept the developer invitation to continue.");return;}
  ...
  async function useInvitedEmail(){
    ...
    window.location.href=`/login?portal=developer&next=${encodeURIComponent(`/developer-invite/${token}`)}`;
  }

  async function accept(){
    setError("");
    const response=await fetch(`/api/developers/access/invitations/${encodeURIComponent(token)}/accept`,{method:"POST"});
    const data=await response.json();
    if(!response.ok||!data?.success){setError(data?.error||"Unable to accept invitation");return;}
    window.location.href=data.redirect||"/developer-access";
  }

  return <main className="min-h-screen bg-[#F7F3EC] text-[#171614]">
    <PublicSiteHeader context="Developer invitation" audience="developers" tone="light" />
    <section className="mx-auto max-w-[980px] px-5 py-14 sm:px-7 lg:px-10 lg:py-20">
      <p className="text-[9px] font-semibold uppercase tracking-[.22em] text-[#9A744B]">DEVELOPER ACCESS</p>
      <h1 className="mt-4 text-[46px] font-medium leading-[.96] tracking-[-.055em] sm:text-[62px]">Join an organization&apos;s Developer Portal without becoming staff.</h1>
