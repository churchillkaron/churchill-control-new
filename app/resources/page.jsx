...
export default function ResourcesPage(){
  const kinds=["developer","solutions","insights","commerce","documents","intelligence"];
  return <main className="min-h-screen bg-[#F7F6F3] text-[#171614]">
    ...
    <section className="relative overflow-hidden border-b border-black/[0.06] bg-[#F4F0E8]">
      <div className="mx-auto grid max-w-[1540px] lg:min-h-[650px] lg:grid-cols-[44%_56%]">
        ...
        <div className="relative min-h-[480px] overflow-hidden border-t border-black/[0.06] lg:min-h-0 lg:border-l lg:border-t-0">
          <div className="absolute inset-0"><PublicArtStage kind="insights"/></div>
          <div className="absolute bottom-7 left-7 right-7 rounded-[22px] border border-white/[.12] bg-[#15120F]/80 p-5 text-white shadow-[0_28px_75px_rgba(0,0,0,.28)] backdrop-blur-xl">
            <div className="text-[7px] font-semibold uppercase tracking-[.20em] text-[#D6A66A]">KNOWLEDGE → DECISION → WORK</div>
            <div className="mt-3 text-[15px] leading-6 text-white/80">Understand the pattern, see the evidence, then move directly into the Avantiqo workflow that can act on it.</div>
          </div>
        </div>
      </div>
    </section>
    <section className="bg-[#FBFAF8]"><div className="mx-auto max-w-[1320px] px-5 py-16 sm:px-7 lg:px-10 lg:py-24">
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{GROUPS.map(([t,d,b],i)=><article key={t} className="group overflow-hidden rounded-[26px] border border-black/[0.075] bg-white shadow-[0_14px_40px_rgba(40,30,20,.03)] transition hover:-translate-y-0.5 hover:border-[#D6A66A]/35">
        <div className="relative h-[145px] overflow-hidden bg-[#171614]"><div className="absolute inset-0 scale-[1.34] transition duration-700 group-hover:scale-[1.39]"><PublicArtStage kind={kinds[i]}/></div><div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(8,7,6,.02),rgba(8,7,6,.58))]"/><div className="absolute left-4 top-4 text-[8px] font-bold text-[#F1C98E]">0{i+1}</div></div>
