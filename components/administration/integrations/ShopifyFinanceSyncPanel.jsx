...
export default function ShopifyFinanceSyncPanel({ organizationId }) {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-6 pb-12">
      <div className="rounded-3xl border border-black/[0.08] bg-white p-6 text-[#191919] shadow-2xl backdrop-blur-xl">
        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
          ...
          <div className="rounded-full border border-black/[0.08] bg-[#FBF8F3] px-4 py-2 text-xs font-semibold text-[#5F5A54]">
            {data?.finance?.mode === "POST_TO_FINANCE" ? "Finance posting enabled" : "Observe only"}
          </div>
        </div>

        {loading ? <p className="mt-6 text-sm text-[#746E66]">Loading Finance settings…</p> : null}
        ...
        {!loading && !data?.store ? (
          <p className="mt-6 text-sm text-[#746E66]">Connect Shopify before configuring settlement synchronization.</p>
        ) : null}
          ...
          <>
            <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-8">
              {metrics.map(([label, value]) => (
                <div key={label} className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-4">
                  <div className="text-2xl font-semibold text-[#191919]">{value}</div>
                  <div className="mt-1 text-xs text-[#746E66]">{label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              <label className="space-y-2">
                <span className="text-sm font-medium text-[#4F4A45]">Finance mode</span>
                <select
                  value={mode}
                  onChange={(event) => setMode(event.target.value)}
                  className="w-full rounded-2xl border border-black/[0.09] bg-white px-4 py-3 text-sm text-[#191919] outline-none"
                >
              ...
              <label className="space-y-2">
                <span className="text-sm font-medium text-[#4F4A45]">Settlement bank account</span>
                <select
                  value={bankAccountId}
                  onChange={(event) => setBankAccountId(event.target.value)}
                  disabled={!data?.store?.entity_id}
                  className="w-full rounded-2xl border border-black/[0.09] bg-white px-4 py-3 text-sm text-[#191919] outline-none disabled:opacity-40"
                >
              ...
              <div className="md:col-span-2 rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-4 text-sm text-[#746E66]">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <div className="mt-2 text-xs leading-5 text-[#817A72]">
                    ...
                    <div className="mt-1 text-xs leading-5 text-[#817A72]">
