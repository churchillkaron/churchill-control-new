...
function clamp(parameter, value) { if (parameter === "pan") return Math.max(-1, Math.min(1, finite(value, 0))); if (parameter === "spatial:azimuth_degrees") return Math.max(-180, Math.min(180, finite(value, 0))); if (parameter === "spatial:width_percent") return Math.max(0, Math.min(200, finite(value, 100))); if (parameter === "spatial:divergence_percent") return Math.max(0, Math.min(100, finite(value, 50))); return Math.max(-60, Math.min(12, finite(value, 0))); }
...
function baseValue(session, target, parameter) { if (target.type === "track") { const item = session.tracks.find((entry) => entry.id === target.id); if (parameter === "pan") return finite(item?.pan, 0); if (parameter === "spatial:azimuth_degrees") return finite(item?.spatial?.azimuth_degrees, 0); if (parameter === "spatial:width_percent") return finite(item?.spatial?.width_percent, 100); if (parameter === "spatial:divergence_percent") return finite(item?.spatial?.divergence_percent, 50); return finite(item?.gain_db, 0); } const bus = session.buses.find((entry) => entry.id === target.id); return parameter === "pan" ? finite(bus?.pan, 0) : finite(bus?.gain_db, 0); }
function parameterSpec(parameter) {
  if (parameter === "pan") return { label: "Pan", unit: "", min: -1, max: 1, step: .01, digits: 2 };
  if (parameter === "spatial:azimuth_degrees") return { label: "Spatial azimuth", unit: "°", min: -180, max: 180, step: 1, digits: 0 };
  if (parameter === "spatial:width_percent") return { label: "Spatial width", unit: "%", min: 0, max: 200, step: 1, digits: 0 };
  if (parameter === "spatial:divergence_percent") return { label: "Spatial divergence", unit: "%", min: 0, max: 100, step: 1, digits: 0 };
  return { label: "Fader", unit: " dB", min: -60, max: 12, step: .5, digits: 1 };
}
function laneThreshold(parameter) { return parameter.startsWith("spatial:") ? .25 : parameter === "pan" ? .005 : .05; }
...
function withPoint(session, target, parameter, time, value, overwriteWindow = false) {
  ...
  for (const point of points) {
    ...
    if (prior && Math.abs(point.time_seconds - prior.time_seconds) < 0.025 && Math.abs(point.value - prior.value) < laneThreshold(parameter)) continue;
    thinned.push(point);
  }
...
export default function MusicAutomationPanel({ session, track, playhead = 0, disabled = false, onChange }) {
  ...
  function LaneEditor({ target, parameter }) {
    const lane = findLane(target, parameter); const points = lane?.points || []; const writeValue = baseValue(session, target, parameter); const spec = parameterSpec(parameter); const label = spec.label; const unit = spec.unit;
    return <div className="rounded-xl border border-white/7 bg-black/15 p-3">
      ...
      <button type="button" disabled={disabled} onClick={() => commitLane(target, parameter, (draft) => { const time = Math.max(0, finite(playhead, 0)); const remaining = draft.points.filter((point) => Math.abs(point.time_seconds - time) > 0.001); remaining.push({ time_seconds: time, value: writeValue }); draft.points = remaining.sort((a, b) => a.time_seconds - b.time_seconds); })} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-[#d6a66a]/18 bg-[#d6a66a]/[0.04] px-2 py-2 text-[8px] text-[#efd29f]/65 disabled:opacity-25"><Plus className="h-3 w-3" /> Write {writeValue.toFixed(spec.digits)}{unit} at {finite(playhead, 0).toFixed(2)}s</button>
      <div className="mt-3 space-y-1.5">{points.map((point, index) => <div key={`${point.time_seconds}-${index}`} className="grid grid-cols-[1fr_1fr_24px] gap-1.5"><input disabled={disabled} type="number" min="0" step="0.01" value={point.time_seconds} onChange={(event) => commitLane(target, parameter, (draft) => { draft.points[index].time_seconds = Math.max(0, finite(event.target.value, 0)); draft.points.sort((a, b) => a.time_seconds - b.time_seconds); })} className="rounded-lg border border-white/7 bg-black/20 px-2 py-1.5 text-[8px] text-white/40 disabled:opacity-25" /><input disabled={disabled} type="number" min={spec.min} max={spec.max} step={spec.step} value={point.value} onChange={(event) => commitLane(target, parameter, (draft) => { draft.points[index].value = finite(event.target.value, 0); })} className="rounded-lg border border-white/7 bg-black/20 px-2 py-1.5 text-[8px] text-white/40 disabled:opacity-25" /><button type="button" disabled={disabled} onClick={() => commitLane(target, parameter, (draft) => { draft.points.splice(index, 1); })} className="inline-flex items-center justify-center rounded-lg border border-white/7 text-white/20 hover:text-red-100 disabled:opacity-25">×</button></div>)}{!points.length ? <div className="text-[8px] text-white/16">No automation points.</div> : null}</div>
  ...
  return <div className="rounded-2xl border border-white/8 bg-white/[0.018] p-4">
    ...
    {session.spatial_audio?.surround_enabled === true && ["POINT","WIDE"].includes(String(track.spatial?.mode || "BED").toUpperCase()) ? <div className="mt-4 space-y-2 rounded-xl border border-[#d6a66a]/12 bg-[#d6a66a]/[0.02] p-3"><div><div className="text-[8px] uppercase tracking-[0.14em] text-[#efd29f]/55">Picture-locked spatial automation</div><div className="mt-1 text-[7px] leading-4 text-white/20">Write movement at the current playhead. Offline {session.spatial_audio.layout_id} rendering interpolates these lanes across discrete speakers; stereo browser preview does not claim to reproduce the surround movement.</div></div><LaneEditor target={selectedTarget} parameter="spatial:azimuth_degrees" /><LaneEditor target={selectedTarget} parameter="spatial:elevation_degrees" /><LaneEditor target={selectedTarget} parameter="spatial:distance_meters" />{String(track.spatial?.mode || "").toUpperCase() === "WIDE" ? <LaneEditor target={selectedTarget} parameter="spatial:width_percent" /> : null}<LaneEditor target={selectedTarget} parameter="spatial:divergence_percent" /></div> : null}
