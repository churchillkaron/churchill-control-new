...
function LevelBar({ peak, clipping = false }) {
  return (
    <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
      <div className={`h-full transition-[width] duration-75 ${clipping ? "bg-red-300/80" : "bg-current text-emerald-100/55"}`} style={{ width: `${percentFromDb(peak)}%` }} />
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/[0.018] p-4">
      <div className="flex items-center justify-between gap-3">
        <div><div className="flex items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.18em] text-[#d6a66a]/65"><Activity className="h-3.5 w-3.5" /> Live engineering meters</div><div className="mt-1 text-[8px] text-white/20">Post-fader level, dynamics, stereo/phase and raw pre-cleanup source health</div></div>
