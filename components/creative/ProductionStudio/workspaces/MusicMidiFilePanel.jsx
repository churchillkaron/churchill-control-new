...
export default function MusicMidiFilePanel({ organizationId, projectId, session, disabled = false, onReload }) {
  ...
  async function importFile(file) {
    ...
    try {
      ...
      const response = await fetch("/api/creative/music/midi-file", {
        ...
        body:JSON.stringify({ action:"import_parsed", organization_id:organizationId, creative_project_id:projectId, expected_revision:session.revision || 0, parsed, replace_existing:false, apply_first_tempo:false, apply_first_time_signature:false }),
      });
      const body = await response.json();
      if (!response.ok || body.success === false) throw new Error(body.error || "MIDI import failed");
      setMessage(`${body.imported_track_count || 0} MIDI track(s) imported. Project tempo was preserved.`);
      await onReload?.();
  ...
  function exportFile() {
    ...
    try {
      setError(""); setMessage("");
      const bytes = encodeStandardMidiFile({ midi:session.midi, bpm:session.bpm, time_signature:session.time_signature });
      ...
      setMessage(`Exported ${session.midi.tracks?.length || 0} MIDI track(s) as Standard MIDI File.`);
  ...
  return <div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.018] p-4">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><div className="text-[9px] font-semibold uppercase tracking-[0.2em] text-[#d6a66a]/65">Standard MIDI File</div><div className="mt-1 text-[8px] leading-4 text-white/24">Move performances between Avantiqo and external DAWs without rendering them to audio. Import preserves raw notes/controllers; export writes format-1 .mid.</div></div>
