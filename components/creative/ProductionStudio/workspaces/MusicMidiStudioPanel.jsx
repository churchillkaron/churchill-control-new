...
export default function MusicMidiStudioPanel({ organizationId, projectId }) {
  ...
  async function load() {
    ...
    try {
      const response = await fetch("/api/creative/music/midi", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "load", organization_id: organizationId, creative_project_id: projectId }) });
      const body = await response.json();
      if (!response.ok || body.success === false) throw new Error(body.error || "MIDI project could not load");
      setSession(body.session || null);
    } catch (cause) { setError(cause?.message || "MIDI project could not load"); }
  ...
  return (
    <div className="mx-auto max-w-[1500px] p-6">
      <div className="mb-4">
        ...
        <div className="mt-1 text-lg font-medium text-white/78">Piano Roll, Harmony, Groove, Keyboard Performance, Drums, Sampler, Automation & Owned Instruments</div>
      ...
      {session ? <>
        ...
        <MusicMidiGroovePanel organizationId={organizationId} projectId={projectId} session={session} disabled={busy} onReload={load} />
