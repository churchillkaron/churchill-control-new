...
import { ArrowRight, Layers3, LayoutGrid, RotateCcw, Sparkles, WandSparkles } from "lucide-react";
...
export default function MusicProducerPanel({ organizationId, projectId, onOpen }) {
    ...
    <div className="overflow-hidden rounded-3xl border border-[#d6a66a]/20 bg-gradient-to-b from-[#d6a66a]/[0.05] to-black/30">
      <div className="border-b border-white/7 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><div className="flex items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.24em] text-[#d6a66a]/75"><Sparkles className="h-3.5 w-3.5"/> Music Producer</div><div className="mt-2 text-xl font-medium text-white/80">Shape the production from the real project</div><div className="mt-1 max-w-2xl text-[10px] leading-5 text-white/30">Avantiqo reads the actual arrangement, tracks, MIDI and sampler state, then proposes safe project changes. Every Producer action creates a reversible snapshot before it changes the project.</div></div>
      ...
      <div className="grid gap-4 p-6 lg:grid-cols-[1fr_340px]">
        <div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            <button type="button" onClick={()=>onOpen?.("arrange")} className="inline-flex items-center justify-between rounded-xl border border-white/8 bg-black/15 px-3 py-2.5 text-[8px] text-white/42"><span className="inline-flex items-center gap-1.5"><LayoutGrid className="h-3.5 w-3.5"/> Arrangement</span><ArrowRight className="h-3 w-3"/></button>
            <button type="button" onClick={()=>onOpen?.("midi")} className="inline-flex items-center justify-between rounded-xl border border-white/8 bg-black/15 px-3 py-2.5 text-[8px] text-white/42"><span className="inline-flex items-center gap-1.5"><WandSparkles className="h-3.5 w-3.5"/> MIDI</span><ArrowRight className="h-3 w-3"/></button>
            <button type="button" onClick={()=>onOpen?.("workstation")} className="inline-flex items-center justify-between rounded-xl border border-white/8 bg-black/15 px-3 py-2.5 text-[8px] text-white/42"><span className="inline-flex items-center gap-1.5"><Layers3 className="h-3.5 w-3.5"/> Workstation</span><ArrowRight className="h-3 w-3"/></button>
          </div>
        </div>
        ...
        <aside className="rounded-2xl border border-white/7 bg-black/20 p-4">
          ...
          <div className="mt-3 rounded-xl border border-white/6 p-2 text-[7px] leading-4 text-white/20">Producer changes project structure only. Audio rendering remains in the Workstation and Professional Release, and source assets stay preserved.</div>
        </aside>
      </div>
