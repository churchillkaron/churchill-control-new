    ...
    <div className="mt-3 rounded-xl border border-white/7 bg-black/15 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-[7px] leading-3 text-white/15">Key-aware note targets. Planning/review only; this never changes the audio.</div>
        </div>
        <button type="button" disabled={disabled || busy || !pitchCurrent} onClick={() => request("build")} className="rounded-lg border border-white/8 px-2 py-1.5 text-[8px] text-white/34 disabled:opacity-20">{busy ? status : plan ? "Rebuild plan" : "Build tuning plan"}</button>
      </div>

      {!pitchCurrent ? <div className="mt-2 text-[7px] text-amber-100/35">Analyze the current vocal pitch map first.</div> : null}

      <div className="mt-3 grid grid-cols-3 gap-2">
        <label className="block"><div className="mb-1 text-[7px] uppercase text-white/15">Strength</div><input disabled={disabled || busy} type="number" min="0" max="100" value={strength} onChange={(event) => setStrength(Math.max(0, Math.min(100, finite(event.target.value, 80))))} className="w-full rounded-lg border border-white/7 bg-black/25 px-2 py-1.5 text-[8px] text-white/42 disabled:opacity-20" /><div className="mt-1 text-[7px] text-white/12">%</div></label>
        <label className="block"><div className="mb-1 text-[7px] uppercase text-white/15">Keep ±</div><input disabled={disabled || busy} type="number" min="0" max="50" value={tolerance} onChange={(event) => setTolerance(Math.max(0, Math.min(50, finite(event.target.value, 10))))} className="w-full rounded-lg border border-white/7 bg-black/25 px-2 py-1.5 text-[8px] text-white/42 disabled:opacity-20" /><div className="mt-1 text-[7px] text-white/12">cents</div></label>
        <label className="block"><div className="mb-1 text-[7px] uppercase text-white/15">Max shift</div><input disabled={disabled || busy} type="number" min="0" max="600" value={maxCorrection} onChange={(event) => setMaxCorrection(Math.max(0, Math.min(600, finite(event.target.value, 200))))} className="w-full rounded-lg border border-white/7 bg-black/25 px-2 py-1.5 text-[8px] text-white/42 disabled:opacity-20" /><div className="mt-1 text-[7px] text-white/12">cents</div></label>
      </div>
        ...
        <div className="max-h-52 space-y-1 overflow-y-auto pr-1">
          {(plan.segments || []).slice(0, 60).map((segment) => (
            <div key={segment.id} className="grid grid-cols-[1fr_70px_52px] items-center gap-2 rounded-lg border border-white/6 px-2 py-1.5 text-[7px]">
              ...
              <input type="number" min="12" max="120" step="1" disabled={disabled || busy} defaultValue={segment.target_midi} onBlur={(event) => { const target = Math.round(finite(event.target.value, segment.target_midi)); if (target !== segment.target_midi) void approve(segment, target); }} className="rounded-md border border-white/7 bg-black/25 px-1.5 py-1 text-[7px] text-white/35 disabled:opacity-20" title="Target MIDI note" />
              <button type="button" disabled={disabled || busy || segment.approved} onClick={() => approve(segment)} className={`inline-flex items-center justify-center gap-1 rounded-md border px-1.5 py-1 ${segment.approved ? "border-emerald-300/10 text-emerald-100/45" : "border-white/7 text-white/30"} disabled:opacity-40`}><Check className="h-2.5 w-2.5" /> {segment.approved ? "OK" : "Approve"}</button>
            </div>
          ))}
        </div>
        <div className="flex items-start gap-2 rounded-lg border border-amber-300/10 bg-amber-300/[0.015] p-2 text-[7px] leading-3 text-amber-100/40"><LockKeyhole className="mt-0.5 h-3 w-3 shrink-0" /><div><div>Audio render locked: {plan.render_blocker || "DSP engine not certified"}.</div><div className="mt-1 text-amber-100/28">Approved targets are preserved now; formant-preserving dynamic pitch rendering will only unlock after its dedicated Music DSP engine and licensing/certification gate are complete.</div></div></div>
      </div> : null}
