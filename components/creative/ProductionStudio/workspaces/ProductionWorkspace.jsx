...
function nextShotAction({ shot, failed, reviews, running, candidates, tasks }) {
  if (!shot) {
    return {
      kind: "select",
      title: "Select a shot",
      detail: "Choose the exact shot you want to produce, compare or approve.",
    };
  }
  if (failed.length) {
    return {
      kind: "blocked",
      title: "Resolve the production block",
      detail: `${failed.length} step${failed.length === 1 ? "" : "s"} failed for this shot. Inspect the failure before another production pass.`,
    };
  }
  if (reviews.length) {
    return {
      kind: "review",
      title: `Inspect ${taskLabel(reviews[0])}`,
      detail: "Watch the real candidate in context and approve only after identity, performance, continuity and sync look right.",
    };
  }
  if (running.length) {
    return {
      ...
      detail: "Stay on this shot or review an earlier version while the new candidate finishes.",
    };
  }
  if (candidates.length > 1) {
    return {
      kind: "compare",
      title: "Choose the strongest version",
      detail: `${candidates.length} candidate versions exist for this shot. Compare them before committing the edit.`,
    };
  }
  if (candidates.length) {
    return {
      kind: "candidate",
      title: "Review the candidate in context",
      detail: "Check the full shot against story purpose and continuity before it moves into the final edit.",
    };
  }
  if (tasks.length) {
    return {
      kind: "waiting",
      title: "Continue the production chain",
      detail: "The shot has production steps but no viewable candidate yet.",
    };
  }
  return {
    kind: "produce",
    title: "Produce this shot",
    detail: "No production task or candidate exists yet. Run production from the project control above.",
  };
}

function CandidateMedia({ url, video, label }) {
  return (
    <div className="relative flex min-h-[390px] min-w-0 items-center justify-center overflow-hidden bg-[#22201D]">
      {label ? (
        <div className="absolute left-3 top-3 z-10 rounded-md border border-white/10 bg-black/55 px-2 py-1 text-[7px] font-semibold uppercase tracking-[0.09em] text-white/70 backdrop-blur-sm">
          {label}
        </div>
      ) : null}
      {url ? (
        video ? (
          <video src={url} controls preload="metadata" className="max-h-[600px] w-full object-contain" />
        ) : (
          <img src={url} alt={label || "Production candidate"} className="max-h-[600px] w-full object-contain" />
        )
      ) : (
...
export default function ProductionWorkspace({ runtime }) {
  ...
  const candidateRows = useMemo(() => {
    ...
    for (const task of [...selectedTasks].reverse()) {
      ...
      rows.push({
        task,
        url,
        score: evidence.total_score ?? evidence.overall_score ?? evidence.sync_score ?? null,
      });
    }
    return rows;
  }, [selectedTasks]);
  ...
  const comparisonCandidate =
    candidateRows.find((row) => row.task.id === compareCandidateId && row.task.id !== activeCandidate?.task.id) ||
    candidateRows.find((row) => row.task.id !== activeCandidate?.task.id) ||
    null;
  ...
  const comparisonIsVideo = isVideoValue(comparisonUrl);
  ...
  const selectedReviews = selectedTasks.filter((task) => task.status === "REVIEW");
  const selectedFailed = selectedTasks.filter((task) => ["FAILED", "SKIPPED"].includes(task.status));
  const selectedRunning = selectedTasks.filter((task) => task.status === "RUNNING");
  const selectedLipSyncReviews = selectedReviews.filter((task) => String(task.metadata?.contract || "").includes("LIPSYNC_VALIDATION"));
  const nextAction = nextShotAction({
    shot: selectedShot,
      ...
      <div className="grid min-h-[650px] xl:grid-cols-[300px_minmax(0,1fr)_300px]">
        ...
        <main className="min-w-0 bg-[#F6F3EE] p-4 lg:p-5">
          ...
          <div className={`mb-3 flex items-center justify-between gap-3 rounded-xl border px-3 py-2.5 ${nextAction.kind === "blocked" ? "border-red-700/15 bg-red-50" : nextAction.kind === "review" ? "border-amber-700/15 bg-amber-50" : "border-black/[0.07] bg-white"}`}>
            <div className="min-w-0">
              <div className="text-[7px] font-semibold uppercase tracking-[0.11em] text-[#918B83]">Recommended next action</div>
              <div className="mt-0.5 truncate text-[9px] font-semibold text-[#403C37]">{nextAction.title}</div>
              <div className="mt-0.5 text-[8px] leading-4 text-[#746E66]">{nextAction.detail}</div>
            </div>
            {nextAction.kind === "compare" && comparisonCandidate ? (
              <button type="button" onClick={() => setComparisonOpen(true)} className="shrink-0 rounded-lg border border-[#A37849]/15 bg-[#F5EEE5] px-2.5 py-2 text-[7px] font-semibold text-[#76583A]">Compare versions</button>
            ) : null}
            {nextAction.kind === "review" && selectedReviews[0] ? (
              <button type="button" onClick={() => approveTask(selectedReviews[0])} disabled={approvingId === selectedReviews[0].id} className="shrink-0 rounded-lg bg-[#25231F] px-2.5 py-2 text-[7px] font-semibold text-white disabled:opacity-40">
                {approvingId === selectedReviews[0].id ? "Approving…" : "Approve after inspection"}
              </button>
            ) : null}
          </div>
          ...
          <div className={`overflow-hidden rounded-xl border border-black/[0.08] bg-[#22201D] shadow-sm ${comparisonOpen && comparisonCandidate ? "grid md:grid-cols-2" : ""}`}>
            <CandidateMedia
              url={previewUrl}
              video={previewIsVideo}
              label={activeCandidate ? `Current · ${taskLabel(activeCandidate.task)}` : "Current"}
            />
            {comparisonOpen && comparisonCandidate ? (
              <div className="border-t border-white/10 md:border-l md:border-t-0">
                <CandidateMedia
                  url={comparisonUrl}
                  video={comparisonIsVideo}
                  label={`Compare · ${taskLabel(comparisonCandidate.task)}`}
                />
              </div>
            ) : null}
          </div>
          ...
          {candidateRows.length ? (
            <div className="mt-2 flex min-w-0 items-center gap-2 overflow-x-auto rounded-lg border border-black/[0.07] bg-white p-1.5">
              ...
              {candidateRows.map((row, index) => {
                const active = activeCandidate?.task.id === row.task.id;
                const version = candidateRows.length - index;
                return (
                  <button
                    key={row.task.id}
                    type="button"
                    onClick={() => setSelectedCandidateId(row.task.id)}
                    className={`shrink-0 rounded-md border px-2.5 py-1.5 text-left transition ${active ? "border-[#A37849]/20 bg-[#F5EEE5]" : "border-transparent hover:border-black/[0.07] hover:bg-[#FCFAF6]"}`}
                  >
                    <div className="text-[7px] font-semibold text-[#4A453F]">V{version} · {taskLabel(row.task)}</div>
                    <div className="mt-0.5 text-[6px] text-[#918B83]">{row.score !== null && row.score !== undefined ? `QC ${Number(row.score).toFixed(0)} · ` : ""}{row.task.status || "READY"}</div>
                  </button>
                );
              })}
              {candidateRows.length > 1 ? (
                <button
                  ...
                  className={`ml-auto shrink-0 rounded-md border px-2.5 py-1.5 text-[7px] font-semibold ${comparisonOpen ? "border-[#A37849]/20 bg-[#F5EEE5] text-[#76583A]" : "border-black/[0.07] text-[#716B63]"}`}
                >
                  {comparisonOpen ? "Single view" : "Compare"}
                </button>
              ) : null}
            </div>
          ) : null}
          ...
          {comparisonOpen && comparisonCandidate && candidateRows.length > 2 ? (
            <div className="mt-2 flex items-center gap-2 overflow-x-auto text-[7px] text-[#918B83]">
              <span className="shrink-0">Compare current against:</span>
              {candidateRows.filter((row) => row.task.id !== activeCandidate?.task.id).map((row, index) => (
                <button
                  key={row.task.id}
                  type="button"
                  onClick={() => setCompareCandidateId(row.task.id)}
                  className={`shrink-0 rounded-md px-2 py-1 ${comparisonCandidate?.task.id === row.task.id ? "bg-[#F5EEE5] font-semibold text-[#76583A]" : "hover:bg-white"}`}
                >
                  V{candidateRows.length - candidateRows.findIndex((item) => item.task.id === row.task.id)}
                </button>
              ))}
            </div>
          ) : null}
        ...
        <aside className="border-l border-black/[0.07] bg-white">
          <div className="border-b border-black/[0.06] px-4 py-3">
            ...
            <div className="mt-1 text-[10px] font-semibold text-[#403C37]">Selected-shot decisions</div>
            <div className="mt-1 text-[8px] leading-4 text-[#918B83]">Only holds and decisions for the shot in the viewer appear below.</div>
          </div>

          <div className="grid grid-cols-3 border-b border-black/[0.06] bg-[#FCFBF8]">
            {[["Review", selectedReviews.length], ["Lip-sync", selectedLipSyncReviews.length], ["Failed", selectedFailed.length]].map(([label, value]) => (
          ...
          <div className="max-h-[calc(100vh-410px)] overflow-y-auto p-3">
            {selectedFailed.map((task) => (
              <div key={task.id} className="mb-2 rounded-xl border border-red-700/15 bg-red-50 p-3">
                <div className="text-[9px] font-semibold text-red-950">{taskLabel(task)}</div>
                <div className="mt-1 text-[8px] leading-4 text-red-900/70">{task.error || "This production step failed and must be resolved before the shot can clear release."}</div>
              </div>
            ))}

            {selectedReviews.map((task) => {
              const evidence = qualityEvidence(task);
            ...
            {!selectedReviews.length && !selectedFailed.length ? (
              <div className="flex items-start gap-2 rounded-xl border border-emerald-700/10 bg-emerald-50/60 p-3 text-[8px] leading-4 text-emerald-800">
                ...
                <span>{reviews.length ? `No hold on this shot. ${reviews.length} review item${reviews.length === 1 ? " exists" : "s exist"} elsewhere in the project.` : "No human review hold on this shot right now."}</span>
              </div>
            ) : null}
            ...
            <div className="mt-4 border-t border-black/[0.06] pt-3">
              <div className="text-[7px] font-semibold uppercase tracking-[0.11em] text-[#918B83]">Project production state</div>
              <div className="mt-2 space-y-1.5 text-[8px] text-[#746E66]">
                <div className="flex justify-between"><span>Project reviews</span><span className="font-semibold text-[#4A453F]">{reviews.length}</span></div>
                <div className="flex justify-between"><span>Project failures</span><span className="font-semibold text-[#4A453F]">{failed.length}</span></div>
