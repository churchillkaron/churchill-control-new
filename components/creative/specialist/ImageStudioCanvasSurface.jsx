...
import { buildSafeZone } from "@/lib/creative/stills/runtime/CreativeImageStudioDesignRuntime.js";
...
export default function ImageStudioCanvasSurface({workspace,assets=[]}){
 ...
 const scale=Math.min(1,workspace.viewport.zoom); const w=board.width*scale,h=board.height*scale; const safe=buildSafeZone(board);
 ...
 return <div ref={shell} className="relative flex min-h-full items-center justify-center overflow-auto rounded-2xl border border-white/[0.07] bg-black/60 p-12" onPointerMove={move} onPointerUp={end} onPointerCancel={end}>
  <div className="relative shrink-0 overflow-hidden bg-white shadow-[0_30px_100px_rgba(0,0,0,.55)]" style={{width:w,height:h}} onPointerDown={(e)=>{workspace.selectLayers([]);regionStart(e);}} onPointerMove={regionMove} onPointerUp={end}>
   ...
   {workspace.comments.filter((comment)=>comment.artboard_id===board.id&&comment.status!=="RESOLVED").map((comment,index)=><div key={comment.id||index} title={comment.body||"Comment"} className="pointer-events-none absolute z-[70] flex h-5 w-5 items-center justify-center rounded-full bg-[#D6A66A] text-[8px] font-bold text-black" style={{left:n(comment.position?.x)*scale-10,top:n(comment.position?.y)*scale-10}}>{index+1}</div>)}
