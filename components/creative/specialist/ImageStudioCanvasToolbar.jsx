...
export default function ImageStudioCanvasToolbar({ workspace, persistence }) {
  ...
  return (
    <div className="mx-4 hidden items-center gap-1 rounded-lg border border-white/[0.07] bg-black/25 p-1 lg:flex">
      ...
      <button type="button" title="Undo" disabled={!workspace.historyPast?.length} onClick={workspace.undo} className="rounded-md p-1.5 text-white/30 hover:text-white/55 disabled:cursor-not-allowed disabled:opacity-20"><Undo2 className="h-3.5 w-3.5" /></button>
      <button type="button" title="Redo" disabled={!workspace.historyFuture?.length} onClick={workspace.redo} className="rounded-md p-1.5 text-white/30 hover:text-white/55 disabled:cursor-not-allowed disabled:opacity-20"><Redo2 className="h-3.5 w-3.5" /></button>
      <span className="mx-1 h-4 w-px bg-white/[0.08]" />
      ...
      <button type="button" title="Fit to view" onClick={() => workspace.requestFitToView?.()} className="rounded-md px-1.5 py-1 text-[9px] text-white/30 hover:text-white/55">Fit</button>
      <button type="button" title="Actual size" onClick={() => workspace.setViewport({ zoom: 1 })} className="rounded-md p-1.5 text-white/30 hover:text-white/55"><Maximize2 className="h-3.5 w-3.5" /></button>
