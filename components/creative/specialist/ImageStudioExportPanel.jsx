...
export default function ImageStudioExportPanel({workspace}) {
  ...
  return <section className="mt-5"><div className="text-[9px] font-semibold uppercase tracking-[.22em] text-[#948D84]">Output</div><div className="mt-2 flex gap-2"><select value={format} onChange={e=>setFormat(e.target.value)} className="rounded-md border border-[#DDD8D0] bg-[#FBFAF8] px-2 py-1.5 text-[9px] text-[#665F57]"><option>PNG</option><option>JPEG</option><option>PDF</option></select><button disabled={busy||!board} onClick={exportMaster} className="rounded-md border border-[#D6A66A]/20 bg-[#D6A66A]/[.06] px-3 py-1.5 text-[9px] text-[#D6A66A] disabled:opacity-30">{busy?"Rendering…":`Export ${format}`}</button></div>{error?<div className="mt-2 rounded-md border border-[#B36B52]/20 bg-[#B36B52]/[0.05] px-2 py-2 text-[8px] leading-4 text-[#8B4937]">Export blocked · {error}</div>:null}</section>;
}
