...
export default function ImageStudioFormatBar({ workspace }) {
  ...
  return <div className="flex flex-wrap items-center gap-1 border-b border-[#E2DED7] bg-white px-4 py-2">
    ...
    {IMAGE_STUDIO_FORMAT_PRESETS.slice(0,5).map((preset)=><button key={preset.id} onClick={()=>duplicate(preset)} className="rounded-md border border-[#E2DED7] px-2 py-1 text-[8px] text-[#8A837A] hover:border-[#D6A66A]/30 hover:text-[#D6A66A]" title={`Smart adapt as ${preset.label}`}><CopyPlus className="mr-1 inline h-3 w-3"/>{preset.label}</button>)}
