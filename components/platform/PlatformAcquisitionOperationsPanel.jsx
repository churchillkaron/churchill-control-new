...
function ageLabel(value) {
  ...
  if (!Number.isFinite(time) || !time) return "Age unavailable";
...
export default function PlatformAcquisitionOperationsPanel() {
  ...
  const ranked = useMemo(() => [...recent].sort((a, b) => {
    const oa = openByAcquisition.get(a.id); const ob = openByAcquisition.get(b.id); const now = Date.now();
    const ra = oa && new Date(oa.due_at).getTime() <= now ? 0 : oa ? 1 : TERMINAL.has(a.stage) ? 3 : 2;
    const rb = ob && new Date(ob.due_at).getTime() <= now ? 0 : ob ? 1 : TERMINAL.has(b.stage) ? 3 : 2;
    if (ra !== rb) return ra - rb;
    if (oa && ob) return new Date(oa.due_at) - new Date(ob.due_at);
    return new Date(a.stage_updated_at || a.updated_at || 0) - new Date(b.stage_updated_at || b.updated_at || 0);
  }), [recent, openByAcquisition]);
  ...
  const overdue = obligations.filter((row) => row.status === "OPEN" && new Date(row.due_at).getTime() <= now).length;
    ...
    <section className="bg-[#F4F3EF] px-4 pb-6 md:px-5" data-avantiqo-acquisition-operations="true">
      <div className="mx-auto max-w-[1680px] overflow-hidden rounded-[22px] border border-black/[0.07] bg-white">
        <div className="flex items-start justify-between gap-4 border-b border-black/[0.06] px-4 py-4">
          <div><div className="text-[8px] font-semibold uppercase tracking-[0.14em] text-[#8D877E]">Owner operating control</div><h2 className="mt-1.5 text-[17px] font-semibold tracking-[-0.025em] text-[#403C37]">Every live opportunity has one explicit next obligation.</h2><p className="mt-1 max-w-3xl text-[9px] leading-4 text-[#918B83]">Stage age is factual. Overdue is shown only when a persisted owner due time has passed. Completion requires evidence.</p></div>
