...
function ProductArt({ config }) {
  return <div className="absolute inset-0 overflow-hidden bg-[#171614] text-white">
    ...
    <div className="absolute bottom-[10%] left-[7%] right-[7%] rounded-[26px] border border-[#D6A66A]/24 bg-[#F2EBE0] p-5 text-[#171614] shadow-[0_28px_80px_rgba(0,0,0,.22)]"><div className="text-[7px] font-semibold uppercase tracking-[.18em] text-[#9A744B]">ONE AVANTIQO CORE UNDERNEATH</div><div className="mt-4 flex flex-wrap gap-2 text-[7px] text-[#6F675E]">{["Identity","Organization","Permissions","Business context","Evidence","Intelligence","Wallet","APIs"].map(x=><span key={x} className="rounded-full border border-[#D6A66A]/26 bg-[#FBFAF8] px-3 py-2">{x}</span>)}</div></div>
  </div>;
}
export default function ProductLandingPage({ config }) {
  return <main className="min-h-screen bg-[#F7F6F3] text-[#171614]">
    ...
    <section className="relative overflow-hidden border-b border-[#BDAF9E]/30 bg-[#F4F0E8]">
      ...
      <div className="relative mx-auto grid max-w-[1540px] lg:min-h-[720px] lg:grid-cols-[43%_57%]">
        <div className="flex items-center px-5 py-16 sm:px-7 lg:px-10 lg:py-20 xl:px-14"><div className="max-w-[620px]">
          <div className="inline-flex rounded-full border border-[#D6A66A]/28 bg-[#FBFAF8] px-3 py-1.5 text-[8px] font-semibold uppercase tracking-[.18em] text-[#8A633C]">{config.availability || "Available"} · Standalone product</div>
    ...
    <section id="product" className="border-b border-[#BDAF9E]/30 bg-[#FBFAF8]"><div className="mx-auto max-w-[1320px] px-5 py-18 sm:px-7 lg:px-10 lg:py-24">
      <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><div><p className="text-[9px] font-semibold uppercase tracking-[.22em] text-[#9A744B]">THE PRODUCT</p><h2 className="mt-3 text-[38px] font-medium leading-[1.02] tracking-[-.05em] sm:text-[52px]">Focused enough to buy. Deep enough to run the job.</h2></div><p className="max-w-2xl text-[13px] leading-7 text-[#706A62] lg:justify-self-end">{config.promise}</p></div>
    ...
    <section className="border-b border-[#BDAF9E]/30 bg-[#F3EFE7]"><div className="mx-auto max-w-[1320px] px-5 py-18 sm:px-7 lg:px-10 lg:py-24">
      <div className="mb-9"><p className="text-[9px] font-semibold uppercase tracking-[.22em] text-[#9A744B]">COMMERCIAL PACKAGING</p><h2 className="mt-3 text-[38px] font-medium leading-[1.02] tracking-[-.05em] sm:text-[52px]">A simple product first. Expansion when the customer is ready.</h2></div>
    ...
    <section className="bg-[#171614] text-white"><div className="mx-auto max-w-[1120px] px-5 py-20 text-center sm:px-7 lg:px-10 lg:py-28"><p className="text-[9px] font-semibold uppercase tracking-[.22em] text-[#D6A66A]">EXPAND WITHOUT MIGRATING</p><h2 className="mx-auto mt-4 max-w-4xl text-[42px] font-medium leading-[1.00] tracking-[-.055em] sm:text-[58px]">Start with {config.context}. Add the rest of Avantiqo when it earns its place.</h2><p className="mx-auto mt-5 max-w-2xl text-[13px] leading-7 text-white/45">The customer keeps the same organization, users, permissions, evidence and business context as they add Finance, Inventory, Commerce, Creative, Intelligence or the full Business OS.</p><div className="mt-8 flex flex-wrap justify-center gap-2.5"><a href="/start" className="inline-flex h-11 items-center rounded-full bg-[#D6A66A] px-5 text-[10px] font-semibold text-[#3F3327]">Start now →</a><a href="/business" className="inline-flex h-11 items-center rounded-full border border-white/20 px-5 text-[10px] font-semibold text-white/80">Explore Business OS</a></div></div></section>
  </main>;
}
