...
export default function PublicSiteHeader({ context, links = [], action = { label: "Login", href: "/login" } }) {
  return (
    <header className="sticky top-0 z-50 border-b border-black/[0.07] bg-[#F7F6F3]/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[64px] max-w-[1460px] items-center justify-between gap-4 px-5 sm:px-7 lg:px-10">
        <a href="/" className="flex min-w-0 items-center gap-3" aria-label="Avantiqo home">
          <span className="shrink-0 rounded-xl bg-[#171716] px-3 py-2 shadow-[0_4px_16px_rgba(0,0,0,.08)]"><Image src="/branding/avantiqo-wordmark.png" alt="Avantiqo" width={126} height={10} className="h-[10px] w-auto object-contain" priority /></span>
        ...
        <nav className="flex items-center gap-1" aria-label="Primary navigation">
          <div className="hidden items-center gap-1 lg:flex">{links.slice(0,3).map((link) => <a key={`${link.href}-${link.label}`} href={link.href} className="rounded-lg px-3 py-2 text-[10px] font-medium text-[#6C6963] transition hover:bg-white hover:text-[#292723]">{link.label}</a>)}</div>
