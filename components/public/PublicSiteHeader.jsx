      ...
      <div className="mx-auto flex h-[68px] max-w-[1540px] items-center justify-between gap-4 px-5 sm:px-7 lg:px-10 xl:px-14">
        ...
        <nav className="flex items-center gap-1" aria-label={`${context} navigation`}>
          ...
          <details className="group relative">
            ...
            <div className="absolute right-0 top-11 w-[min(720px,calc(100vw-2rem))] overflow-hidden rounded-[22px] border border-white/[0.09] bg-[#1A1815]/[0.99] p-3 shadow-[0_28px_90px_rgba(0,0,0,.30)] backdrop-blur-2xl">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {EXPLORE_GROUPS.map(([group, items]) => (
                  <div key={group} className="rounded-[14px] border border-white/[0.06] bg-white/[0.025] p-2">
                    <div className="px-2 pb-1.5 pt-1 text-[7px] font-semibold uppercase tracking-[0.17em] text-[#D6A66A]/80">{group}</div>
                    {items.map(([label, href]) => (
                      <a key={href} href={href} className="group/link flex items-center justify-between rounded-lg px-2 py-2 text-[9px] font-medium text-white/55 transition hover:bg-white/[0.05] hover:text-white">
