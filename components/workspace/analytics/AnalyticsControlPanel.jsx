  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 right-5 z-40 inline-flex h-11 items-center gap-2 rounded-xl border border-black/10 bg-[#1F1E1B] px-4 text-[12px] font-medium text-[#191919] shadow-[0_12px_36px_rgba(0,0,0,0.18)] transition hover:bg-[#F7F6F3]"
      >
          ...
          <div className="w-full max-w-[720px] overflow-hidden rounded-[24px] border border-black/10 bg-[#F7F6F3] shadow-[0_30px_80px_rgba(0,0,0,0.22)]">
            ...
            <div className="grid grid-cols-4 gap-1 border-b border-black/[0.07] bg-white px-4 pb-3">
              {[
                ...
                <button key={id} type="button" onClick={() => { setMode(id); setMessage(null); }} className={`flex items-center justify-center gap-1.5 rounded-lg px-2 py-2 text-[11px] font-medium ${mode === id ? "bg-[#1F1E1B] text-[#191919]" : "text-[#69635C] hover:bg-[#F7F5F1]"}`}>
                  <Icon size={13} /> {label}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-4 p-5">
              ...
              {mode === "capture" ? (
                <div className="rounded-2xl border border-black/[0.07] bg-white p-4">
                  ...
                  <button type="button" onClick={() => run("capture")} disabled={busy} className="mt-4 inline-flex h-10 items-center gap-2 rounded-xl bg-[#1F1E1B] px-4 text-[12px] font-medium text-[#191919] disabled:opacity-50">
              ...
              {mode === "target" ? (
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2"><button type="button" onClick={() => run("target")} disabled={busy} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#1F1E1B] px-4 text-[12px] font-medium text-[#191919] disabled:opacity-50">{busy ? <LoaderCircle size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} Save target</button></div>
                </div>
              ) : null}
              ...
              {mode === "alert" ? (
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className={conditionType === "OUTSIDE_RANGE" ? "self-end" : "sm:col-span-2"}><button type="button" onClick={() => run("alert")} disabled={busy || !clean(thresholdValue)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#1F1E1B] px-4 text-[12px] font-medium text-[#191919] disabled:opacity-50">{busy ? <LoaderCircle size={14} className="animate-spin" /> : <AlertTriangle size={14} />} Create alert rule</button></div>
                </div>
              ) : null}
              ...
              {mode === "forecast" ? (
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="sm:col-span-3"><p className="mb-3 text-[10px] leading-4 text-[#817B73]">Forecasts require captured actual snapshots. The engine refuses insufficient or mixed-currency evidence.</p><button type="button" onClick={() => run("forecast")} disabled={busy} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#1F1E1B] px-4 text-[12px] font-medium text-[#191919] disabled:opacity-50">{busy ? <LoaderCircle size={14} className="animate-spin" /> : <LineChart size={14} />} Run forecast</button></div>
                </div>
              ) : null}
