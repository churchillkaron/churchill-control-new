...
export default function CommunicationDraftReviewBanner({
  ...
  return (
    <section className="mx-auto mt-4 max-w-[1780px] px-4 text-[#191919] md:px-6">
      <div className="rounded-[24px] border border-amber-300/20 bg-amber-300/[0.055] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-xl font-semibold text-[#191919]/90">
              Review before customer delivery
            </h2>
            <p className="mt-2 text-xs leading-5 text-[#817A72]">
              This is the exact saved message for {participant}. Sending cannot replace its body or attachments.
            </p>
          </div>
          {message ? (
            <span className="rounded-full border border-black/[0.08] bg-[#F7F6F3]/20 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#5F5A54]">
              {message.status}
            </span>
          ) : null}
        </div>

        {loading ? <div className="mt-4 text-sm text-[#817A72]">Loading saved draft…</div> : null}
        ...
        {!loading && message ? (
          <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="rounded-2xl border border-black/[0.08] bg-[#F7F6F3]/20 p-4">
              {message.subject ? <div className="mb-3 text-sm font-semibold text-[#4F4A45]">{message.subject}</div> : null}
              {message.body ? <div className="whitespace-pre-wrap text-sm leading-6 text-[#5F5A54]">{message.body}</div> : null}
            </div>
            <div className="rounded-2xl border border-black/[0.08] bg-[#F7F6F3]/20 p-4">
              <div className="text-xs font-semibold text-[#5F5A54]">Attachments · {attachments.length}</div>
                ...
                {attachments.map((attachment, index) => {
                  ...
                  return url ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="block truncate rounded-xl border border-black/[0.08] bg-[#FBF8F3] px-3 py-2 text-xs text-[#5F5A54] hover:text-[#191919]"
                    >
                      {attachmentLabel(attachment)}
                    </a>
                  ) : null;
                })}
                {!attachments.length ? <div className="text-xs text-[#A19A92]">No attachments.</div> : null}
              </div>
            </div>
          </div>
        ) : null}
          ...
          <div className="mt-5 border-t border-black/[0.08] pt-4">
            {!confirming ? (
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-red-300/15 bg-red-300/[0.045] p-4">
                <div>
                  <div className="text-sm font-semibold text-[#2F2C28]">Confirm customer send</div>
                  <div className="mt-1 text-xs text-[#817A72]">Send this exact saved draft now? This action is irreversible.</div>
                </div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setConfirming(false)} disabled={sending} className="rounded-xl border border-black/[0.08] px-4 py-2 text-xs text-[#5F5A54]">Cancel</button>
