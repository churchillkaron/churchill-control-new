  ...
  return <div className="space-y-4">
    <section className="rounded-[22px] border border-black/[.07] bg-white p-6">
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-[1fr_1.2fr_160px_220px_auto] md:items-end">
        ...
        <button type="button" disabled={busy||!email.trim()} onClick={createInvite} className="h-10 rounded-xl border border-[#B98A52]/25 bg-[#D6A66A] px-4 text-[9px] font-semibold text-[#2C2117] hover:bg-[#C99A5E] disabled:opacity-40">Create invite</button>
      </div>
      ...
      {invite?<div className="mt-4 rounded-[16px] border border-[#B7793B]/18 bg-[#FBF6EF] p-4"><div className="text-[8px] font-semibold text-[#76502E]">Invitation ready · {invite.email}</div>{(()=>{const d=deliveryLabel(invite.delivery);return <div className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-[7px] font-semibold ${d.tone}`}>{d.label}</div>;})()}<div className="mt-2 break-all text-[8px] leading-4 text-[#7A6B5B]">{invite.url}</div><div className="mt-3 flex gap-2"><button type="button" onClick={copyLink} className="rounded-lg border border-[#B98A52]/25 bg-[#D6A66A] px-3 py-2 text-[8px] font-semibold text-[#2C2117] hover:bg-[#C99A5E]">Copy link</button><a href={invite.url} target="_blank" rel="noreferrer" className="rounded-lg border border-black/[.08] px-3 py-2 text-[8px] font-semibold text-[#655C52]">Preview</a></div></div>:null}
    </section>
