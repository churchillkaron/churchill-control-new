...
export default function FinanceDocumentTemplateBuilderEngine({
  ...
  async function save() {
    try {
      ...
      const response = await fetch("/api/finance/document-templates", {
        method: "POST",
      ...
      if (!response.ok || json?.success === false) {
        throw new Error(json?.error || "Unable to create document template");
      }

      onComplete?.();
      onClose?.();
    } catch (saveError) {
      setError(saveError.message || "Unable to create document template");
    } finally {
      setSaving(false);
    }
  }
        ...
        <section className="overflow-auto border-b border-white/10 p-6 lg:border-b-0 lg:border-r">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-light text-white">Create Document Template</h2>
              <p className="mt-2 max-w-2xl text-sm text-white/45">
                Build a branded, organisation-scoped document design. Versioning and lifecycle are managed automatically.
              </p>
            </div>
        ...
        <section className="overflow-auto bg-[#050505] p-6">
          ...
          <div className="mt-4 rounded-2xl bg-white p-8 text-black shadow-2xl">
            ...
            <div className="mt-7 grid grid-cols-2 gap-6 text-sm">
              <div>
                <div className="text-black/45">Bill To</div>
                <div className="mt-2 text-lg font-semibold">Customer Name</div>
                <div className="text-black/55">Customer address and tax details</div>
              </div>
              <div className="text-right text-black/60">
                <div>Document No: SAMPLE-0001</div>
                <div>Date: 28 Jul 2026</div>
                <div>Due: 27 Aug 2026</div>
              </div>
            </div>
            <table className="mt-8 w-full text-sm">
              <thead><tr className="border-b text-left"><th className="py-3">Description</th><th>Qty</th><th>Price</th><th className="text-right">Total</th></tr></thead>
              <tbody><tr className="border-b"><td className="py-4">Sample service</td><td>1</td><td>1,000.00</td><td className="text-right">1,000.00</td></tr></tbody>
            </table>
            <div className="mt-6 text-right">
              {values.show_tax_summary ? <div className="text-sm">Tax: resolved from transaction</div> : null}
              <div className="mt-2 text-2xl font-bold">Total: 1,000.00</div>
            </div>
