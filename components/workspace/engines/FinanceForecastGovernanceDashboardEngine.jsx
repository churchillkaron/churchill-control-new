...
export default function FinanceForecastGovernanceDashboardEngine({ action, organizationId, onClose }) {
  ...
  }, [view, draftRows, overrideRows, portfolioRows, deliveryRows, exceptionRows]);
      ...
      <div className="max-h-[92vh] w-full max-w-7xl overflow-y-auto rounded-[30px] border border-white/[0.08] bg-[#0b0b0b]/95 p-7 shadow-2xl shadow-black/80">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-4xl text-sm leading-6 text-white/45">Executive control for approved forecast coverage, policy-blocked drafts, exceptional approval evidence, exception accountability, escalation delivery, and canonical cross-entity accuracy.</p>
          </div>
        ...
        {data ? <>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.025] p-5"><div className="text-xs uppercase tracking-[0.16em] text-white/35">Exception Accountability</div><div className="mt-4 space-y-3 text-sm text-white/55"><div className="flex justify-between"><span>Overdue unresolved</span><span className={statusTone(summary.overdue_unresolved)}>{summary.overdue_unresolved || 0}</span></div><div className="flex justify-between"><span>Unassigned unresolved</span><span className={statusTone(summary.unassigned_unresolved)}>{summary.unassigned_unresolved || 0}</span></div><div className="flex justify-between"><span>Not yet governed</span><span className={statusTone(summary.not_yet_governed)}>{summary.not_yet_governed || 0}</span></div><div className="flex justify-between"><span>Delivered alerts</span><span className="text-white">{summary.delivered_notifications || 0}</span></div></div></div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">{[["exceptions","Exception Queue"],["approvals","Approval Readiness"],["overrides","Approval Overrides"],["portfolio","Entity Coverage"],["delivery","Delivery Evidence"]].map(([key,label]) => <button key={key} onClick={() => setView(key)} className={`rounded-xl border px-4 py-2 text-sm ${view === key ? "border-amber-300/35 bg-amber-300/10 text-amber-100" : "border-white/[0.08] bg-white/[0.025] text-white/45"}`}>{label}</button>)}</div>
