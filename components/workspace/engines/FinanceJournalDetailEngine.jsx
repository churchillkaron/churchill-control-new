...
export default function FinanceJournalDetailEngine({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#F7F6F3]/80 p-5 text-[#191919] backdrop-blur-xl">
      <div className="max-h-[92vh] w-full max-w-6xl overflow-auto rounded-[32px] border border-black/[0.08] bg-white p-7 shadow-2xl shadow-black/10">
        ...
        {loading ? (
          <div className="mt-8 text-[13px] text-[#746E66]">Loading journal...</div>
        ) : error ? (
            ...
            <div className="mt-7 grid gap-3 md:grid-cols-4">
              {[
                ...
                <div key={label} className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-4">
                  <div className="text-[10px] uppercase tracking-[0.2em] text-[#A19A92]">{label}</div>
                  <div className="mt-2 text-[13px] text-[#4F4A45]">{value}</div>
                </div>
              ))}
            </div>

            <div className="mt-6 overflow-hidden rounded-[24px] border border-black/[0.08]">
              <div className="grid grid-cols-[1fr_180px_180px] border-b border-black/[0.08] bg-[#FBF8F3] px-5 py-3 text-[11px] uppercase tracking-[0.18em] text-[#918B83]">
                <div>Account</div>
              ...
              {lines.length ? lines.map((line, index) => {
                ...
                return (
                  <div key={line.id || index} className="grid grid-cols-[1fr_180px_180px] border-b border-black/[0.06] px-5 py-4 text-[13px] last:border-b-0">
                    <div>
                      <div className="text-[#2F2C28]">
                        {account.code || "Account"}
                        {account.name ? ` · ${account.name}` : ""}
                      </div>
                      {line.description ? <div className="mt-1 text-[12px] text-[#918B83]">{line.description}</div> : null}
                    </div>
                    <div className="text-right text-[#5F5A54]">{money(line.debit)}</div>
                    <div className="text-right text-[#5F5A54]">{money(line.credit)}</div>
                  </div>
                );
              }) : (
                <div className="p-6 text-[13px] text-[#817A72]">No journal lines found.</div>
              )}
