...
export default function FinanceBankFeedConnectionPanel({ connection, organizationId, onRefresh }) {
  ...
  async function verifyProvider() {
    try {
      ...
      const response = await fetch("/api/finance/provider-activation", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ organizationId, action: "verify_bank_feed" }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok || body?.success === false) throw new Error(body?.error || "Bank provider verification failed");
      setResult({ verification: body.verification }); await onRefresh?.();
    } catch (e) { setError(e?.message || "Bank provider verification failed"); await onRefresh?.(); }
    finally { setBusy(""); }
  }
  ...
  return (
    <section className="mb-4 rounded-xl border border-[#A37849]/15 bg-[#FBF8F3] p-3.5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-[9px] sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-black/[0.05] bg-white px-2.5 py-2"><div className="text-[#999188]">Provider verification</div><div className="mt-0.5 font-semibold text-[#514B44]">{label(connection.provider_verification_status || (providerReady ? "Configured unverified" : "Not configured"))}</div><div className="mt-0.5 text-[8px] text-[#999188]">{connection.provider_last_verified_at ? `Verified ${dateTime(connection.provider_last_verified_at)}` : "No successful verification yet"}</div></div>
      </div>
      ...
      {!providerReady ? (
        ...
        <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={verifyProvider} disabled={Boolean(busy)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-3 text-[9px] font-semibold"><ShieldCheck size={11}/>{busy === "verify" ? "Testing…" : "Test connection"}</button><button type="button" onClick={connect} disabled={Boolean(busy)} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-[#1F1E1B] px-3 text-[9px] font-semibold text-white disabled:opacity-40"><ShieldCheck size={11} />{busy === "connect" ? "Opening bank…" : hasConsent ? "Reconnect bank" : "Connect bank"}</button></div>
      ) : (
        <div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" onClick={verifyProvider} disabled={Boolean(busy)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-3 text-[9px] font-semibold"><ShieldCheck size={11}/>{busy === "verify" ? "Testing…" : "Test connection"}</button><button type="button" onClick={syncNow} disabled={Boolean(busy)} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-[#1F1E1B] px-3 text-[9px] font-semibold text-white disabled:opacity-40"><RefreshCw size={11} className={busy === "sync" ? "animate-spin" : ""} />{busy === "sync" ? "Syncing…" : "Sync now"}</button><span className="text-[8px] text-[#918B83]">Only unseen provider transactions are imported.</span></div>
      )}
      ...
      {result?.verification?.verified ? <div className="mt-2 rounded-lg border border-emerald-700/15 bg-emerald-50 p-2 text-[8px] leading-4 text-emerald-800">Provider credential verified with a read-only Brankas request. No statement was imported.</div> : null}
    </section>
  );
}
