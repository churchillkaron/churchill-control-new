...
export default function FinanceClosePackageFreshnessRail({ organizationId, compact = false }) {
  ...
  if (state.loading && !state.data) {
    return (
      <section className="rounded-2xl border border-black/[0.07] bg-white px-4 py-3 text-[11px] text-[#817D76]">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="text-[11px] font-semibold uppercase tracking-[0.13em] text-[#8A867F]">Accounting package freshness</div>
            ...
            <div className="mt-1 max-w-5xl text-[11px] leading-4 text-[#756F67]">
              {state.error
            ...
            {isError && changedLabels.length ? (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {changedLabels.map((label, index) => (
                  <Link key={`${label}:${index}`} href={changedHref(organizationId, changedSections[index])} className="rounded-full border border-red-700/10 bg-white/75 px-2 py-1 text-[11px] font-semibold text-[#8F5848] transition hover:border-red-700/25">
                    {label} →
                  </Link>
                ))}
              </div>
        ...
        <div className="flex shrink-0 items-center gap-2 self-start">
          {!compact ? <span className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.07em] ${isError ? "border-red-700/15 bg-white/65 text-red-800" : trusted ? "border-emerald-700/15 bg-white/65 text-emerald-800" : "border-amber-700/15 bg-white/65 text-amber-800"}`}>{freshnessState || "UNPROVEN"}</span> : null}
          <button type="button" onClick={load} disabled={state.loading} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white/75 px-2.5 text-[11px] font-semibold text-[#716B63] disabled:opacity-50">
