...
export default function FinanceContinuousCloseRail({ organizationId }) {
  ...
  if (loading && !surfacedState) {
    return (
      <div className="mx-auto mb-4 flex min-h-[64px] max-w-[1720px] items-center justify-center rounded-[20px] border border-black/[0.07] bg-white text-[11px] text-[#817A72]">
      ...
      <div className="flex flex-col gap-3 border-b border-black/[0.06] px-4 py-3.5 lg:flex-row lg:items-center lg:justify-between md:px-5">
        <div className="min-w-0 max-w-4xl">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.05em] ${stateTone(surfacedState.state)}`}>
              {surfacedState.state.replace(/_/g, " ")}
            </span>
          </div>
          <p className="mt-0.5 text-[11px] leading-4 text-[#918B83]">{surfacedState.detail}</p>
        </div>
        <div className="flex items-center gap-2 self-start lg:self-auto">
          ...
          <Link href={financeHref(organizationId, "/finance/close")} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-[#25231F] px-3 text-[11px] font-semibold text-white">
        ...
        {controls.map((control) => (
          <Link key={control.id} href={financeHref(organizationId, control.href)} className="group bg-white px-3 py-2.5 transition hover:bg-[#FCFAF6]">
            <div className="flex items-center justify-between gap-2">
              <span className="truncate text-[11px] font-semibold text-[#4B4640]">{control.label}</span>
            ...
            <div className="mt-1 truncate text-[11px] text-[#99928A]">{control.detail}</div>
          </Link>
        ))}
      </div>
