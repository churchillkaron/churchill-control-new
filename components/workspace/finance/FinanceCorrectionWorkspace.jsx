...
export default function FinanceCorrectionWorkspace({ organizationId }) {
  ...
  const evidenceDocuments = useMemo(() => {
    const query = clean(evidenceQuery).toLowerCase();
    return [...state.documents]
      .filter((document) => {
        if (!query) return true;
        return [document.file_name, document.source, document.status, document.classification]
          .map((value) => clean(value).toLowerCase())
          .some((value) => value.includes(query));
      })
      .sort((a, b) => {
        const aSelected = form.documentIds.includes(a.id) ? 1 : 0;
        const bSelected = form.documentIds.includes(b.id) ? 1 : 0;
        if (aSelected !== bSelected) return bSelected - aSelected;
        return String(b.created_at || "").localeCompare(String(a.created_at || ""));
      });
  }, [state.documents, form.documentIds, evidenceQuery]);
  const evidenceBoundaries = selected?.metadata?.evidence?.boundary_validations || {};
  ...
  async function act(action, payload = {}) {
    ...
    } catch (error) { const message = error?.message || "Correction action failed"; await refreshAll().catch(() => null); setState((current) => ({ ...current, error: message })); return null; }
    finally { setBusy(""); }
  }
  ...
  return (
    <section aria-label="Accounting correction workflow" className="mx-auto mb-4 max-w-[1720px] overflow-hidden rounded-[22px] border border-[#A37849]/15 bg-[#FBF8F3] text-[#2A2723]">
      <div className="flex flex-col gap-3 border-b border-black/[0.06] px-4 py-3.5 lg:flex-row lg:items-center lg:justify-between md:px-5">
        <div><div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8A633C]"><ShieldCheck size={11} /> Correction control</div><h2 className="mt-1 text-[14px] font-semibold">Exception → evidence → approval → post → re-check</h2><p className="mt-0.5 text-[11px] leading-4 text-[#918B83]">The source exception is revalidated before submit, approval and posting. Selected evidence is scope-checked and approval-checked server-side.</p></div>
        <button onClick={refreshAll} disabled={state.loading || landing.refreshing} className="inline-flex h-8 items-center gap-1.5 self-start rounded-lg border border-black/[0.08] bg-white px-2.5 text-[11px] font-semibold text-[#76583A]"><RefreshCw size={10} className={state.loading || landing.refreshing ? "animate-spin" : ""} /> Refresh</button>
      </div>
      {state.error ? <div className="mx-4 mt-3 rounded-xl border border-red-700/15 bg-red-50 p-3 text-[11px] text-red-800 md:mx-5">{state.error}</div> : null}
      <div className="grid gap-4 p-4 xl:grid-cols-[minmax(300px,.65fr)_minmax(640px,1.35fr)] md:p-5">
        <div className="space-y-3">
          <div className="overflow-hidden rounded-2xl border border-black/[0.07] bg-white"><div className="border-b border-black/[0.06] bg-[#FAF9F7] px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8A867F]">Open structural exceptions · {exceptions.length}</div>{exceptions.slice(0,8).map((row) => { const hasOpenCase = activeCorrectionAccountIds.has(clean(row.account_id)); return <div key={row.account_id} className="border-b border-black/[0.05] p-3 last:border-0"><div className="flex items-start justify-between gap-2"><div><div className="text-[11px] font-semibold">{row.account_code ? `${row.account_code} · ` : ""}{row.account_name}</div><div className="mt-1 text-[11px] leading-3.5 text-[#8C857D]">{row.reason}</div></div><span className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase ${tone(row.state)}`}>{label(row.state)}</span></div><button onClick={() => create(row)} disabled={Boolean(busy) || hasOpenCase} className="mt-2 h-7 rounded-lg border border-[#A37849]/20 bg-[#FFF9F0] px-2.5 text-[11px] font-semibold text-[#76583A] disabled:opacity-40">{hasOpenCase ? "Correction already open" : "Open correction case"}</button></div>; })}</div>
          {state.corrections.length ? <div className="overflow-hidden rounded-2xl border border-black/[0.07] bg-white"><div className="border-b border-black/[0.06] px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8A867F]">Correction cases</div>{state.corrections.slice(0,12).map((row) => <button key={row.id} onClick={() => setSelectedId(row.id)} className={`w-full border-b border-black/[0.05] px-3 py-2.5 text-left last:border-0 ${selectedId === row.id ? "bg-[#FFF8EE]" : ""}`}><div className="flex items-center justify-between gap-2"><div className="truncate text-[11px] font-semibold">{row.metadata?.exception?.account_code || "Account"} · {row.metadata?.exception?.account_name || "Correction"}</div><span className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase ${tone(row.status)}`}>{row.status}</span></div><div className="mt-1 truncate text-[11px] text-[#958F87]">{row.metadata?.treatment?.summary}</div></button>)}</div> : null}
        </div>

        <div className="rounded-2xl border border-black/[0.07] bg-white p-4">
          {!selected ? <div className="flex min-h-[260px] items-center justify-center text-center text-[11px] text-[#8B857D]">Choose an exception to open a governed correction case.</div> : <>
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-black/[0.06] pb-3"><div><div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8A633C]">Correction case</div><div className="mt-1 text-[14px] font-semibold">{selected.metadata?.exception?.account_code} · {selected.metadata?.exception?.account_name}</div><div className="mt-1 text-[11px] text-[#8F8981]">Observed {money(selected.metadata?.exception?.closing_amount, selected.currency_code || currency)} · {selected.metadata?.exception?.reason}</div></div><span className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase ${tone(selected.status)}`}>{selected.status}</span></div>
            {selected.metadata?.recheck ? <div className={`mt-3 rounded-xl border p-3 text-[11px] ${selected.metadata.recheck.resolved ? "border-emerald-700/15 bg-emerald-50 text-emerald-800" : "border-amber-700/15 bg-amber-50 text-amber-800"}`}><div className="font-semibold">Exception re-check: {selected.metadata.recheck.resolved ? "original structural exception cleared" : label(selected.metadata.recheck.resulting_state)}</div><div className="mt-1">{selected.metadata.recheck.reason}</div></div> : null}
            <div className="mt-3 grid gap-3 lg:grid-cols-2"><label className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#8A867F]">Resolution mode<select disabled={!['DRAFT','REJECTED'].includes(selected.status)} value={form.resolutionMode} onChange={(e) => setForm((c) => ({ ...c, resolutionMode: e.target.value }))} className="mt-1 h-9 w-full rounded-lg border border-black/[0.08] bg-white px-2 text-[11px] normal-case"><option value="CONTROL">Control / configuration</option><option value="JOURNAL">Journal correction</option></select></label><div className="rounded-xl border border-black/[0.06] bg-[#FCFBF9] p-3"><div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase text-[#8A867F]"><FileCheck2 size={9}/> Evidence discipline</div><div className="mt-1 text-[11px] leading-4 text-[#777169]">Journal corrections require at least one selected source document. Enter only evidence-supported correcting lines; Avantiqo never invents the balancing amount or account. Control corrections may use a documented evidence basis when no governed file exists.</div></div></div>
            <div className="mt-3 grid gap-2"><textarea disabled={!['DRAFT','REJECTED'].includes(selected.status)} value={form.summary} onChange={(e) => setForm((c) => ({ ...c, summary: e.target.value }))} className="min-h-16 rounded-lg border border-black/[0.08] p-2 text-[11px]" placeholder="Accounting treatment"/><textarea disabled={!['DRAFT','REJECTED'].includes(selected.status)} value={form.rationale} onChange={(e) => setForm((c) => ({ ...c, rationale: e.target.value }))} className="min-h-14 rounded-lg border border-black/[0.08] p-2 text-[11px]" placeholder="Why this treatment is correct"/><textarea disabled={!['DRAFT','REJECTED'].includes(selected.status)} value={form.evidenceBasis} onChange={(e) => setForm((c) => ({ ...c, evidenceBasis: e.target.value }))} className="min-h-14 rounded-lg border border-black/[0.08] p-2 text-[11px]" placeholder="Evidence basis / source documents"/></div>
            <div className="mt-3 rounded-xl border border-black/[0.07] bg-[#FCFBF9] p-3">
              <div className="flex flex-wrap items-center justify-between gap-2"><div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#8A633C]">Governed source evidence</div><div className="text-[11px] text-[#8B857D]">{form.documentIds.length} selected · {state.documents.length} available</div></div>
              {state.documents.length ? <>
                <div className="relative mt-2"><Search size={10} className="absolute left-2 top-1/2 -translate-y-1/2 text-[#A19B93]"/><input value={evidenceQuery} onChange={(event) => setEvidenceQuery(event.target.value)} className="h-8 w-full rounded-lg border border-black/[0.08] bg-white pl-7 pr-2 text-[11px]" placeholder="Search all available evidence"/></div>
                <div className="mt-2 max-h-64 overflow-y-auto pr-1"><div className="grid gap-1.5 sm:grid-cols-2">{evidenceDocuments.map((document) => { const selectedDocument = form.documentIds.includes(document.id); const pendingApproval = document.approval_required === true && !document.approved_at; return <label key={document.id} className={`flex cursor-pointer items-start gap-2 rounded-lg border p-2 ${selectedDocument ? "border-[#A37849]/30 bg-[#FFF8EE]" : "border-black/[0.06] bg-white"}`}><input type="checkbox" checked={selectedDocument} onChange={() => toggleDocument(document.id)} disabled={!['DRAFT','REJECTED'].includes(selected.status) || pendingApproval} className="mt-0.5"/><span className="min-w-0"><span className="block truncate text-[11px] font-semibold">{document.file_name || "Source document"}</span><span className="mt-0.5 block text-[11px] uppercase tracking-[0.06em] text-[#989188]">{document.controlled ? `Controlled v${document.version_number || 1}` : "Legacy source"} · {pendingApproval ? "approval pending" : label(document.status || "available")}</span></span></label>; })}</div>{!evidenceDocuments.length ? <div className="py-4 text-center text-[11px] text-[#8B857D]">No evidence matches this search.</div> : null}</div>
              </> : <div className="mt-2 text-[11px] text-[#8B857D]">No source documents are currently available for this client/entity.</div>}
