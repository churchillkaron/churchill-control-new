...
export default function FinanceCorrectionWorkspace({ organizationId }) {
  ...
  const currency = businessContext.entity?.currency || businessContext.organization?.default_currency || null;
  const [state, setState] = useState({ loading: true, error: "", health: null, corrections: [], accounts: [], documents: [] });
  ...
  async function load() {
    ...
    try {
      ...
      healthUrl.searchParams.set("organizationId", organizationId); healthUrl.searchParams.set("entityId", entityId); healthUrl.searchParams.set("periodId", periodId);
      ...
      const [healthResponse, correctionsResponse] = await Promise.all([
        fetch(healthUrl.toString(), { cache: "no-store", credentials: "include" }),
        fetch(correctionsUrl.toString(), { cache: "no-store", credentials: "include" }),
      ]);
      const [healthBody, correctionBody] = await Promise.all([healthResponse.json().catch(() => ({})), correctionsResponse.json().catch(() => ({}))]);
      if (!healthResponse.ok || healthBody?.success === false) throw new Error(healthBody?.error || "Unable to load account exceptions");
      if (!correctionsResponse.ok || correctionBody?.success === false) throw new Error(correctionBody?.error || "Unable to load corrections");
      const corrections = correctionBody.corrections || [];
      setState({ loading: false, error: "", health: healthBody, corrections, accounts: correctionBody.accounts || [], documents: correctionBody.documents || [] });
  ...
  const exceptions = useMemo(() => (state.health?.health?.accounts || []).filter((row) => ["BLOCKED", "ACTION_REQUIRED"].includes(row.state)), [state.health]);
  ...
  useEffect(() => {
    ...
    setForm({ resolutionMode: meta.resolution_mode || "CONTROL", summary: meta.treatment?.summary || "", rationale: meta.treatment?.rationale || "", evidenceBasis: meta.treatment?.evidence_basis || "", postingDate: draft.posting_date || state.health?.context?.as_of || "", description: draft.description || "", lines: Array.isArray(draft.lines) ? draft.lines : [] });
  }, [selectedId]);
  ...
  async function act(action, payload = {}) {
    ...
    try {
      ...
      await load(); if (correction?.id) setSelectedId(correction.id); return body.result;
    } catch (error) { setState((current) => ({ ...current, error: error?.message || "Correction action failed" })); return null; }
    finally { setBusy(""); }
  }
  ...
  if (state.loading && !state.health) return <div className="mx-auto mb-4 flex min-h-[74px] max-w-[1720px] items-center justify-center rounded-[20px] border border-black/[0.07] bg-white text-[8px] text-[#817A72]"><LoaderCircle size={11} className="mr-2 animate-spin text-[#A37849]" /> Preparing correction control loop…</div>;
  ...
  return (
    <section aria-label="Accounting correction workflow" className="mx-auto mb-4 max-w-[1720px] overflow-hidden rounded-[22px] border border-[#A37849]/15 bg-[#FBF8F3] text-[#2A2723]">
      <div className="flex flex-col gap-3 border-b border-black/[0.06] px-4 py-3.5 lg:flex-row lg:items-center lg:justify-between md:px-5">
        <div><div className="flex items-center gap-1.5 text-[8px] font-semibold uppercase tracking-[0.14em] text-[#8A633C]"><ShieldCheck size={11} /> Correction control</div><h2 className="mt-1 text-[14px] font-semibold">Exception → evidence → approval → post → re-check</h2><p className="mt-0.5 text-[8px] leading-4 text-[#918B83]">Avantiqo proposes the treatment path, never the balancing amount. Journal corrections require substantiated lines, independent approval and atomic posting.</p></div>
        <button onClick={load} disabled={state.loading} className="inline-flex h-8 items-center gap-1.5 self-start rounded-lg border border-black/[0.08] bg-white px-2.5 text-[8px] font-semibold text-[#76583A]"><RefreshCw size={10} className={state.loading ? "animate-spin" : ""} /> Refresh</button>
      </div>
