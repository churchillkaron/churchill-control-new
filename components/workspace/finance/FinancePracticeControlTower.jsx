    ...
    <section className="rounded-[24px] border border-[#A37849]/20 bg-[#F9F5EF] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-[12px] leading-5 text-[#756F67]">Run the firm by exception: client workload, review stage, unresolved review points and the next deadline across every active engagement.</p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-6">
        ...
        <Metric label="Overdue" value={summary.overdue || 0} detail="Review items past due" attention />
        <Metric label="Review points" value={summary.open_review_points || 0} detail="Unresolved notes / queries" attention />
      </div>

      <div className="mt-4 overflow-x-auto rounded-2xl border border-black/[0.07] bg-white/85">
        <div className="min-w-[1080px]">
          <div className="grid grid-cols-[minmax(190px,1.5fr)_150px_150px_105px_95px_100px_125px_125px] gap-3 border-b border-black/[0.06] px-4 py-2.5 text-[9px] font-medium uppercase tracking-[0.12em] text-[#8A867F]">
            <span>Client</span><span>Preparer</span><span>Reviewer</span><span>Status</span><span>Ready</span><span>Overdue</span><span>Review points</span><span>Next deadline</span>
          </div>
          {clients.map((client) => (
            <div key={client.organization_id} className="grid grid-cols-[minmax(190px,1.5fr)_150px_150px_105px_95px_100px_125px_125px] items-center gap-3 border-b border-black/[0.05] px-4 py-3 text-[11px] last:border-0">
