...
export default function FinancePracticeOnboarding({ organizationId, onOpenBilling }) {
    ...
    try {
      ...
      setNotice(
        action === "request_signature"
          ? "Signature request created in the governed Documents runtime."
          : action === "set_entity"
            ? "Client legal entity assigned to the engagement."
            : "Engagement document linked."
      ); await load();
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-2">{(data?.engagements || []).map((engagement) => { const form = forms[engagement.id] || {}; const latestSignature = engagement.signatures?.[0]; return <section key={engagement.id} className="rounded-2xl border border-black/[0.07] bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-[10px] font-semibold text-[#403C37]">{engagement.client_name}</div><div className="mt-0.5 text-[8px] text-[#918B83]">{engagement.service_package || "Accounting engagement"}</div></div><span className={`rounded-full border px-2 py-1 text-[7px] font-semibold uppercase ${tone(engagement.readiness.state)}`}>{label(engagement.readiness.state)}</span></div><div className="mt-2 text-[8px] text-[#76583A]">Next: {engagement.readiness.next_action}</div>
      ...
      {!engagement.entity_id ? <div className="mt-3 flex flex-wrap items-end gap-2"><label className="min-w-[280px] flex-1 text-[7px] font-medium uppercase tracking-[0.1em] text-[#8C877F]">Client legal entity<select value={form.entityId || ""} onChange={(event) => patchForm(engagement.id, { entityId: event.target.value })} className="mt-1 h-8 w-full rounded-lg border border-black/[0.08] bg-white px-2 text-[8px] normal-case tracking-normal"><option value="">Select legal entity…</option>{(engagement.legal_entities || []).map((entity) => <option key={entity.id} value={entity.id}>{entity.display_name || entity.legal_name || entity.code}{entity.country ? ` · ${entity.country}` : ""}{entity.is_default_accounting_entity ? " · default" : ""}</option>)}</select></label><button type="button" disabled={saving || !form.entityId} onClick={() => act(engagement.id, "set_entity", { entityId: form.entityId })} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[#A37849]/20 bg-[#FBF7F1] px-3 text-[8px] font-semibold text-[#76583A] disabled:opacity-40">Set entity</button></div> : null}
      {!engagement.entity_id && !(engagement.legal_entities || []).length ? <div className="mt-2 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-700/15 bg-amber-50 px-3 py-2 text-[8px] text-amber-800"><div><b>This client has no active legal entity yet.</b><div className="mt-0.5">Create it in the client organization’s canonical Legal Entities workspace, then return here and refresh onboarding.</div></div><Link href={`/workspace/${engagement.organization_id}/finance/legal-entities`} className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg border border-amber-800/15 bg-white px-3 font-semibold text-amber-900"><Plus size={9} />Create client legal entity</Link></div> : null}
      {!engagement.billing_profile && engagement.entity_id && engagement.signatures?.some((row) => row.status === "SIGNED") ? <div className="mt-3 flex items-center justify-between gap-3 rounded-xl border border-black/[0.06] bg-[#FAF9F7] px-3 py-2.5"><div className="text-[8px] text-[#716B63]">Billing policy is the last commercial setup gate before this engagement becomes ready.</div><button type="button" onClick={() => onOpenBilling?.(engagement.id)} className="inline-flex h-8 shrink-0 items-center rounded-lg border border-[#A37849]/20 bg-white px-3 text-[8px] font-semibold text-[#76583A]">Configure billing</button></div> : null}
      {engagement.entity_id && !engagement.engagement_document && (data?.approved_documents || []).length ? <div className="mt-3 flex flex-wrap items-end gap-2"><label className="min-w-[280px] flex-1 text-[7px] font-medium uppercase tracking-[0.1em] text-[#8C877F]">Approved controlled document<select value={form.documentId || ""} onChange={(event) => patchForm(engagement.id, { documentId: event.target.value })} className="mt-1 h-8 w-full rounded-lg border border-black/[0.08] bg-white px-2 text-[8px] normal-case tracking-normal"><option value="">Select engagement letter…</option>{(data?.approved_documents || []).map((document) => <option key={document.id} value={document.id}>{document.document_name} · v{document.version_number}</option>)}</select></label><button type="button" disabled={saving || !form.documentId} onClick={() => act(engagement.id, "link_engagement_document", { documentId: form.documentId })} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[#A37849]/20 bg-[#FBF7F1] px-3 text-[8px] font-semibold text-[#76583A] disabled:opacity-40"><FileSignature size={10} />Link document</button></div> : null}
      {engagement.entity_id && !engagement.engagement_document && !(data?.approved_documents || []).length ? <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-700/15 bg-amber-50 px-3 py-2.5 text-[8px] text-amber-900"><div><b>No approved engagement document is available yet.</b><div className="mt-0.5 leading-4">Upload the engagement letter in the firm’s canonical Documents workspace, request and complete approval there, then return here and refresh onboarding. Approval remains bound to the controlled document version.</div></div><Link href={`/workspace/${organizationId}/documents`} className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg border border-amber-800/15 bg-white px-3 font-semibold text-amber-900"><FileSignature size={10} />Open Documents</Link></div> : null}
