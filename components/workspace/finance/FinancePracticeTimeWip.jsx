...
export default function FinancePracticeTimeWip({ organizationId, initialEngagementId = "" }) {
  ...
  async function createInvoice(engagementId) {
    ...
    try {
      ...
      setNotice(body?.recovered ? `Recovered existing Finance AR invoice and completed billing settlement: ${body.invoice_id}.` : `Customer invoice created through Finance AR: ${body.invoice_id}.`);
      await load();
  ...
  return <div className="space-y-4">
    ...
    <form onSubmit={saveBillingPolicy} className="rounded-2xl border border-black/[0.07] bg-white p-4">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="text-[8px] font-medium uppercase tracking-[0.1em] text-[#8C877F]"><span className="flex items-center justify-between gap-2"><span>Finance customer</span><Link href={`/workspace/${organizationId}/finance/customers?create=1`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1 text-[7px] font-semibold normal-case tracking-normal text-[#76583A]">Create customer <ExternalLink size={8}/></Link></span><select value={billingForm.customerPartyId} onChange={(event) => setBillingForm({ ...billingForm, customerPartyId: event.target.value })} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.08] bg-white px-2.5 text-[9px] normal-case tracking-normal"><option value="">Select customer party…</option>{(data?.billing_options?.customer_parties || []).map((party) => <option key={party.id} value={party.id}>{party.display_name || party.legal_name || party.email || party.id}</option>)}</select></label>
        ...
        <label className="text-[8px] font-medium uppercase tracking-[0.1em] text-[#8C877F]"><span className="flex items-center justify-between gap-2"><span>Revenue account</span><Link href={`/workspace/${organizationId}/finance/chart-of-accounts?create=1`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1 text-[7px] font-semibold normal-case tracking-normal text-[#76583A]">Create account <ExternalLink size={8}/></Link></span><select value={billingForm.revenueAccountId} onChange={(event) => setBillingForm({ ...billingForm, revenueAccountId: event.target.value })} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.08] bg-white px-2.5 text-[9px] normal-case tracking-normal"><option value="">Select revenue account…</option>{(data?.billing_options?.revenue_accounts || []).map((account) => <option key={account.id} value={account.id}>{account.account_code ? `${account.account_code} · ` : ""}{account.account_name || account.name || account.id}</option>)}</select></label>
        <label className="text-[8px] font-medium uppercase tracking-[0.1em] text-[#8C877F]"><span className="flex items-center justify-between gap-2"><span>Tax rule</span><Link href={`/workspace/${organizationId}/finance/tax-codes?create=1`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="inline-flex items-center gap-1 text-[7px] font-semibold normal-case tracking-normal text-[#76583A]">Create tax rule <ExternalLink size={8}/></Link></span><select disabled={!billingForm.billingEntityId} value={billingForm.taxRuleId} onChange={(event) => setBillingForm({ ...billingForm, taxRuleId: event.target.value, taxTreatmentConfirmed: false })} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.08] bg-white px-2.5 text-[9px] normal-case tracking-normal disabled:bg-[#F4F2EF]"><option value="">{!billingForm.billingEntityId ? "Choose billing entity first…" : applicableTaxRules.length ? "Select Finance tax rule…" : "No active sales tax rule for this jurisdiction"}</option>{applicableTaxRules.map((rule) => <option key={rule.id} value={rule.id}>{rule.tax_code} · {rule.tax_name} · {Math.round(number(rule.tax_rate) * 10000) / 100}% · {rule.tax_regime}</option>)}</select></label>
