...
function tone(value) {
  ...
  if (["BLOCKED", "CHANGES_REQUESTED", "SYSTEM_BLOCKED", "STALE"].includes(state)) return "border-red-700/15 bg-red-50 text-red-800";
  if (["READY_FOR_REVIEW", "READY", "IN_PROGRESS", "INSPECT", "UNPROVEN"].includes(state)) return "border-amber-700/15 bg-amber-50 text-amber-800";
  if (["VERIFIED", "REVIEWED", "CLEARED", "COMPLETE", "CURRENT"].includes(state)) return "border-emerald-700/15 bg-emerald-50 text-emerald-800";
...
function FreshnessBanner({ freshness, hasReviewerSignoff }) {
  if (!hasReviewerSignoff) {
    return (
      <div className="mt-3 rounded-xl border border-[#A37849]/12 bg-[#FFF9F0] px-3 py-2.5 text-[7px] leading-4 text-[#76583A]">
        <b>Freshness baseline not signed yet.</b> Reviewer sign-off will bind the approval to this exact governed accounting evidence. Later source changes will require re-review.
      </div>
    );
  }

  if (freshness?.trusted === true) {
    return (
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-emerald-700/10 bg-emerald-50/60 px-3 py-2.5 text-[7px] text-emerald-800">
        <span><b>Review current.</b> The reviewer sign-off still matches the live governed accounting evidence.</span>
        <span className="rounded-full border border-emerald-700/10 bg-white px-2 py-1 font-semibold uppercase">Current</span>
      </div>
    );
  }

  const changed = Array.isArray(freshness?.changed_labels) ? freshness.changed_labels : [];
  return (
    <div className="mt-3 rounded-xl border border-red-700/14 bg-red-50 p-3 text-red-800">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-[7px] font-semibold uppercase tracking-[0.08em]">Re-review required</div>
        <span className="rounded-full border border-red-700/12 bg-white px-2 py-1 text-[6px] font-semibold uppercase">{label(freshness?.state || "UNPROVEN")}</span>
      </div>
      <div className="mt-1 text-[7px] leading-4">{freshness?.reason || "The current accounting evidence no longer proves the stored reviewer sign-off."}</div>
      {changed.length ? <div className="mt-2 flex flex-wrap gap-1">{changed.map((item) => <span key={item} className="rounded-full border border-red-700/10 bg-white px-2 py-1 text-[6px] font-semibold">{item}</span>)}</div> : null}
      <div className="mt-2 text-[7px] leading-4">Refresh the evidence, inspect only what changed, then record a new reviewer decision. Partner clearance remains blocked until the review is current.</div>
    </div>
  );
}
...
export default function FinanceReviewerEvidencePanel({ organizationId, row }) {
  ...
  const freshness = evidence.review_freshness || null;
  const activeSignoffs = Array.isArray(review.active_signoffs) ? review.active_signoffs : [];
  const hasReviewerSignoff = activeSignoffs.some((signoff) => clean(signoff?.signoff_role).toUpperCase() === "REVIEWER");
  const freshnessBlocksDecision = hasReviewerSignoff && freshness?.trusted !== true;
  ...
  const decisionBlockers = [
    ...
    ...(freshnessBlocksDecision ? [freshness?.reason || "Reviewer evidence freshness is not proven"] : []),
  ];
    ...
    <div className="mt-4 rounded-xl border border-[#A37849]/15 bg-[#FFFDF9] p-3.5">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-black/[0.05] pb-3">
        <div>
          ...
          <div className="mt-1 text-[8px] leading-4 text-[#817A72]">Source evidence, deterministic accounting impact, review points, freshness and sign-offs are compressed into this same decision workpaper.</div>
        </div>
      ...
      <FreshnessBanner freshness={freshness} hasReviewerSignoff={hasReviewerSignoff} />
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-4">
        ...
        <Card title="Sign-offs" icon={BadgeCheck}><div className="text-[12px] font-semibold text-[#45403A]">{activeSignoffs.length}</div><div className="mt-0.5 text-[7px] text-[#989188]">Active governed sign-offs</div></Card>
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        ...
        <Card title="Reviewer trail" icon={BadgeCheck}><div className="space-y-1.5 text-[7px] text-[#5F5952]">{review.notes?.slice(0, 6).map((note) => <div key={note.id} className="rounded-lg border border-black/[0.05] bg-white px-2 py-1.5"><div className="font-semibold">{label(note.note_type)} · {label(note.status)}</div><div className="mt-0.5 text-[#817A72]">{note.body}</div></div>)}{!review.notes?.length ? <div className="text-[#8F8981]">No review points recorded.</div> : null}{activeSignoffs.map((signoff) => <div key={signoff.id} className="flex items-center gap-1.5 text-[#61715C]"><BadgeCheck size={8} /> {label(signoff.signoff_role)} signed {shortDate(signoff.signed_at)}</div>)}</div></Card>
      </div>

      {decisionBlockers.length ? <div className="mt-3 rounded-xl border border-red-700/12 bg-red-50 p-3"><div className="text-[7px] font-semibold uppercase tracking-[0.08em] text-red-800">Decision blockers visible in evidence</div><div className="mt-1.5 space-y-1 text-[7px] text-red-800">{decisionBlockers.map((blocker, index) => <div key={`${blocker}-${index}`}>{blocker}</div>)}</div></div> : <div className="mt-3 rounded-xl border border-emerald-700/10 bg-emerald-50/50 px-3 py-2 text-[7px] text-emerald-800">No evidence-layer blocker is surfaced here. The governed sign-off endpoint still performs the final server-side authorization, segregation-of-duties, freshness and portfolio checks.</div>}
    </div>
  );
}
