...
function Metric({ title, value, detail, warning = false, accent = false }) {
  return (
    <div className={`rounded-xl border px-3.5 py-3 ${accent ? "border-[#A37849]/18 bg-[#FFF9F0]" : "border-black/[0.07] bg-white"}`}>
      <div className="text-[11px] font-medium uppercase tracking-[0.13em] text-[#8C877F]">{title}</div>
      <div className={`mt-1.5 text-[20px] font-semibold tracking-[-0.03em] ${warning && Number(value) > 0 ? "text-[#9A533D]" : "text-[#2A2723]"}`}>{value}</div>
      <div className="mt-0.5 text-[11px] text-[#99938A]">{detail}</div>
    </div>
  );
}
...
function IntegrityChip({ children, good = false }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-semibold ${good ? "border-emerald-700/12 bg-emerald-50 text-emerald-800" : "border-[#A37849]/15 bg-white text-[#76583A]"}`}>
    ...
    return (
      <section className="rounded-[24px] border border-[#A37849]/15 bg-[#FBF8F3] p-4 md:p-5">
        <div className="mb-3 flex items-center justify-between gap-3 border-b border-black/[0.06] pb-3">
          <button type="button" onClick={() => setShowClientFile(false)} className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#76583A]"><ChevronLeft size={11} /> Back to review</button>
          <div className="text-[11px] text-[#99938A]">Full governed client file</div>
        </div>
  ...
  return (
    <section className="rounded-[24px] border border-[#A37849]/15 bg-[#FBF8F3] p-4 md:p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.16em] text-[#8A633C]"><ShieldCheck size={12} /> Finance Review Control Tower</div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            ...
            {tower.generated_at ? <span className="ml-1 text-[11px] text-[#A09A92]">Generated {new Date(tower.generated_at).toLocaleString()}</span> : null}
          </div>
          <div className="mt-2 text-[11px] text-[#99938A]">Signed in as <span className="font-semibold text-[#615B54]">{viewer.name || "Accounting team member"}</span>{viewer.role ? ` · ${label(viewer.role)}` : ""}</div>
        </div>
        <button type="button" onClick={() => load({ preserveSelection: true })} disabled={state.loading} className="inline-flex h-9 items-center gap-2 self-start rounded-xl border border-[#A37849]/20 bg-white px-3 text-[11px] font-semibold text-[#76583A] disabled:opacity-50 xl:self-auto"><RefreshCw size={11} className={state.loading ? "animate-spin" : ""} /> Refresh tower</button>
      </div>
      ...
      {capacity.length ? (
        <div className="mt-3 rounded-xl border border-black/[0.06] bg-white px-3 py-2.5">
          <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.11em] text-[#8A867F]"><UsersRound size={9} /> Review load</div>
          <div className="mt-2 flex gap-2 overflow-x-auto pb-0.5">
            {capacity.slice(0, 8).map((owner) => (
              <div key={owner.staff_account_id || "UNASSIGNED"} className="min-w-[150px] rounded-lg border border-black/[0.06] bg-[#FAF9F7] px-2.5 py-2">
                <div className="truncate text-[11px] font-semibold text-[#4A4640]">{owner.name}</div>
                <div className="mt-1 flex gap-2 text-[11px] text-[#918B83]"><span>{owner.items} items</span><span>{owner.hours} planned</span>{owner.overdue ? <span className="font-semibold text-[#9A533D]">{owner.overdue} overdue</span> : null}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex gap-1 overflow-x-auto">
          {FILTERS.map((item) => (
            <button key={item.id} type="button" onClick={() => setFilter(item.id)} className={`h-8 shrink-0 rounded-lg border px-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] ${filter === item.id ? "border-[#A37849]/25 bg-[#A37849]/[0.08] text-[#76583A]" : "border-black/[0.07] bg-white text-[#817D76]"}`}>{item.label}</button>
          ))}
        </div>
      ...
      {state.error ? <div className="mt-4 rounded-xl border border-red-700/15 bg-red-50 p-3 text-[11px] text-red-800">{state.error}</div> : null}
        ...
        visibleQueue.length ? (
          <div className="mt-4 grid min-h-[590px] gap-4 xl:grid-cols-[minmax(410px,0.76fr)_minmax(600px,1.24fr)]">
            <div className="overflow-hidden rounded-2xl border border-black/[0.07] bg-white">
              <div className="flex items-center justify-between gap-3 border-b border-black/[0.06] bg-[#FAF9F7] px-4 py-2.5"><span className="text-[11px] font-medium uppercase tracking-[0.11em] text-[#8A867F]">{visibleQueue.length} item{visibleQueue.length === 1 ? "" : "s"}</span><span className="text-[11px] text-[#A29C93]">Server rank · owner · stage · urgency</span></div>
                  ...
                  return (
                    <button key={row.id} type="button" onClick={() => { setSelectedId(row.id); setActionError(""); setChangeReason(""); }} className={`w-full border-b border-black/[0.05] px-4 py-3 text-left last:border-0 ${active ? "bg-[#FFF8EE]" : "hover:bg-[#FCFBF9]"}`}>
                      <div className="flex items-start gap-3">
                        <div className="pt-0.5 text-[11px] font-semibold tabular-nums text-[#B2ABA1]">{String(index + 1).padStart(2, "0")}</div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0"><div className="flex items-center gap-1.5"><div className="truncate text-[10px] font-semibold text-[#37342F]">{row.client_name}</div>{mine ? <span className="rounded border border-[#A37849]/15 bg-[#A37849]/[0.06] px-1 py-0.5 text-[11px] font-semibold uppercase text-[#76583A]">Mine</span> : null}</div><div className="mt-0.5 truncate text-[11px] font-medium text-[#5E5952]">{row.title}</div></div>
                            <span className={`shrink-0 rounded-full border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.06em] ${stageTone(row.stage)}`}>{copy.title}</span>
                          </div>
                          <div className="mt-2 text-[11px] font-semibold text-[#6E5237]">{row.next_action || copy.action}</div>
                          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#99938A]"><span className={row.overdue ? "font-semibold text-[#9A533D]" : ""}>Due {shortDate(row.due_at)}</span><span>{row.owner_name || "Unassigned"}</span><span>{hours(row.budget_minutes)} planned</span>{row.client_dependency?.open ? <span className="text-[#8A633C]">{row.client_dependency.open} client dependency</span> : null}</div>
                        </div>
                      </div>
                    </button>
                  );
            ...
            {selected ? (
              <div className="rounded-2xl border border-black/[0.07] bg-white">
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-black/[0.06] px-4 py-3.5">
                  <div className="min-w-0"><div className="text-[11px] font-medium uppercase tracking-[0.13em] text-[#8A633C]">Governed decision workpaper</div><div className="mt-1 text-[16px] font-semibold tracking-[-0.02em] text-[#312D28]">{selected.title}</div><div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#918B83]"><span>{selected.client_name}</span><span>{label(selected.required_role)}</span><span>Due {shortDate(selected.due_at)}</span>{selected.owner_id === viewerId ? <span className="font-semibold text-[#76583A]">Assigned to you</span> : selected.owner_name ? <span>{selected.owner_name}</span> : null}</div></div>
                ...
                <div className="p-4">
                  <div className={`rounded-xl border p-3 ${selected.stage === "BLOCKED" || selected.stage === "RETURNED" ? "border-red-700/12 bg-red-50" : selected.stage === "VERIFIED_HANDOFF" ? "border-emerald-700/10 bg-emerald-50/45" : "border-amber-700/10 bg-amber-50/45"}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-[11px] font-semibold uppercase tracking-[0.11em] text-[#8A867F]">Current server stage</div><div className="mt-1 text-[13px] font-semibold text-[#403A34]">{selectedStage.title}</div><div className="mt-1 max-w-2xl text-[11px] leading-4 text-[#776F66]">{selectedStage.detail}</div></div><span className={`rounded-full border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.06em] ${stageTone(selected.stage)}`}>{label(selected.stage)}</span></div>
                  </div>

                  <div className="mt-3 grid gap-2 md:grid-cols-5">
                    <div className="rounded-xl border border-black/[0.06] bg-[#FAF9F7] p-3"><div className="text-[11px] uppercase tracking-[0.1em] text-[#918B83]">Queue authority</div><div className="mt-1.5 text-[11px] font-semibold text-[#76583A]">Navigation only</div></div>
                    <div className="rounded-xl border border-black/[0.06] bg-[#FAF9F7] p-3"><div className="text-[11px] uppercase tracking-[0.1em] text-[#918B83]">Final authority</div><div className="mt-1.5 text-[11px] font-semibold text-[#556B51]">Live preflight</div></div>
                    <div className="rounded-xl border border-black/[0.06] bg-[#FAF9F7] p-3"><div className="text-[11px] uppercase tracking-[0.1em] text-[#918B83]">Prepared by</div><div className="mt-1.5 truncate text-[11px] font-semibold text-[#4A4640]">{selected.assigned_accountant || "Unassigned"}</div></div>
                    <div className="rounded-xl border border-black/[0.06] bg-[#FAF9F7] p-3"><div className="text-[11px] uppercase tracking-[0.1em] text-[#918B83]">Client dependency</div><div className={`mt-1.5 text-[11px] font-semibold ${selected.client_dependency?.open ? "text-[#9A7045]" : "text-[#556B51]"}`}>{selected.client_dependency?.open || 0} open</div></div>
                    <div className="rounded-xl border border-black/[0.06] bg-[#FAF9F7] p-3"><div className="text-[11px] uppercase tracking-[0.1em] text-[#918B83]">Budget</div><div className="mt-1.5 text-[11px] font-semibold text-[#4A4640]">{hours(selected.budget_minutes)}</div></div>
                  </div>

                  {selected.description ? <div className="mt-4 rounded-xl border border-black/[0.06] bg-[#FCFBF9] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#918B83]">Procedure performed</div><div className="mt-1.5 text-[11px] leading-4 text-[#625D56]">{selected.description}</div></div> : null}
                  {selected.conclusion ? <div className="mt-3 rounded-xl border border-black/[0.06] bg-[#FCFBF9] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#918B83]">Preparer conclusion</div><div className="mt-1.5 whitespace-pre-wrap text-[11px] leading-4 text-[#625D56]">{selected.conclusion}</div></div> : null}

                  <FinanceReviewerEvidencePanel organizationId={organizationId} row={selected} />

                  {selected.blocked_reason ? <div className="mt-3 rounded-xl border border-red-700/10 bg-red-50 p-3 text-[11px] text-red-800"><b>Work blocker:</b> {selected.blocked_reason}</div> : null}
                  {selected.client_dependency?.open ? <div className="mt-3 rounded-xl border border-amber-700/10 bg-amber-50/60 p-3 text-[11px] text-amber-900"><b>Client dependency:</b> {selected.client_dependency.open} open request{selected.client_dependency.open === 1 ? "" : "s"}{selected.client_dependency.overdue ? ` · ${selected.client_dependency.overdue} overdue` : ""}. Final authorization rechecks the current governed state.</div> : null}
                  {actionError ? <div className="mt-3 rounded-xl border border-red-700/15 bg-red-50 p-3 text-[11px] text-red-800">{actionError}</div> : null}

                  <div className="mt-4 rounded-xl border border-[#A37849]/15 bg-[#FFFDF9] p-3">
                    <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#8A633C]">Governed action</div><div className="mt-1 max-w-2xl text-[11px] leading-4 text-[#918B83]">The server-ranked stage is never enough to approve. Reviewer identity, segregation of duties, open review points, sign-offs, evidence approvals, exact ledger population and current accounting truth are rebuilt at the decision boundary.</div></div><button type="button" onClick={() => setShowClientFile(true)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-black/[0.07] bg-white px-2.5 text-[11px] font-semibold text-[#716B63]"><FolderOpen size={10} /> Full client file</button></div>

                    {clean(selected.status).toUpperCase() === "READY_FOR_REVIEW" ? (
                      <div className="mt-3 grid gap-2 md:grid-cols-[minmax(220px,1fr)_auto_auto]">
                        <input value={changeReason} onChange={(event) => setChangeReason(event.target.value)} placeholder="Reason if changes are required…" className="h-9 rounded-lg border border-black/[0.08] bg-white px-3 text-[11px] outline-none focus:border-[#A37849]/40" />
                        <button type="button" disabled={Boolean(busy) || !changeReason.trim()} onClick={() => lifecycle(selected, "request_changes", { reason: changeReason.trim() })} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-red-700/15 bg-red-50 px-3 text-[11px] font-semibold text-red-800 disabled:opacity-40"><AlertTriangle size={10} /> Request changes</button>
                        <button type="button" disabled={Boolean(busy) || selected.stage === "BLOCKED"} onClick={() => approve(selected)} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg bg-[#3F352A] px-4 text-[11px] font-semibold text-white disabled:opacity-40">{busy ? <LoaderCircle size={10} className="animate-spin" /> : <ShieldCheck size={10} />}{busy ? "Running live checks…" : "Run live preflight & approve"}</button>
                      </div>
                    ) : selected.stage === "RETURNED" ? (
                      <div className="mt-3 rounded-lg border border-black/[0.06] bg-[#FAF9F7] px-3 py-2.5 text-[11px] text-[#817D76]">Returned to preparation. It stays visible for control-tower awareness but offers no reviewer decision until resubmission.</div>
                    ) : selected.stage === "WAITING_ON_CLIENT" ? (
                      <div className="mt-3 rounded-lg border border-amber-700/10 bg-amber-50/50 px-3 py-2.5 text-[11px] text-amber-900">Client evidence is still outstanding. Open the client file to inspect the dependency rather than forcing a review decision.</div>
                    ) : selected.stage === "BLOCKED" ? (
                      <div className="mt-3 rounded-lg border border-red-700/10 bg-red-50 px-3 py-2.5 text-[11px] text-red-800">The accounting control path is blocked. Resolve the blocker before attempting completion.</div>
                    ) : (
                      <button type="button" disabled={Boolean(busy)} onClick={() => approve(selected)} className="mt-3 inline-flex h-9 items-center justify-center gap-1.5 rounded-lg bg-[#3F352A] px-4 text-[11px] font-semibold text-white disabled:opacity-40">{busy ? <LoaderCircle size={10} className="animate-spin" /> : clean(selected.status).toUpperCase() === "IN_PROGRESS" ? <CheckCircle2 size={10} /> : <ArrowRight size={10} />}{busy ? "Checking…" : selectedStage.action}</button>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="mt-4 rounded-2xl border border-black/[0.07] bg-white px-5 py-10 text-center"><CheckCircle2 size={20} className="mx-auto text-[#6F7E68]" /><div className="mt-2 text-[11px] font-semibold text-[#3D3934]">Nothing needs attention in this lane</div><div className="mx-auto mt-1 max-w-xl text-[11px] leading-5 text-[#8B867E]">Choose another control-tower filter. The server keeps returned, blocked, client-dependent and decision work separate so reviewers can move quickly without losing accounting truth.</div></div>
        )
      ) : null}

      <div className="mt-4 rounded-xl border border-black/[0.06] bg-[#FAF9F7] px-3 py-2.5 text-[11px] leading-4 text-[#817D76]"><div className="flex items-start gap-2"><UserRoundCheck size={10} className="mt-0.5 shrink-0 text-[#9A744B]" /><span>Avantiqo separates prioritization from authorization: the control tower ranks complete server-side populations, while reviewer and partner actions remain governed by live identity, segregation-of-duties and evidence checks.</span></div></div>
    </section>
  );
}
