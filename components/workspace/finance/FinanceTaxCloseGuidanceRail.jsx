...
export default function FinanceTaxCloseGuidanceRail({ organizationId, entityId, selectedVatReturnId }) {
  const [state, setState] = useState({ loading: false, error: "", preflight: null, selectedId: null });

  async function load() {
    if (!organizationId || !entityId || !selectedVatReturnId) {
      setState({ loading: false, error: "", preflight: null, selectedId: selectedVatReturnId || null });
      return;
    }
    try {
      ...
      if (!preflight?.return || preflight.return.id !== selectedVatReturnId) throw new Error("Tax close guidance could not resolve the selected VAT filing. Refresh Tax before continuing.");
      setState({ loading: false, error: "", preflight, selectedId: selectedVatReturnId });
    } catch (error) {
      setState({ loading: false, error: error?.message || "Tax close guidance could not be loaded", preflight: null, selectedId: selectedVatReturnId });
    }
  }

  useEffect(() => { load(); }, [organizationId, entityId, selectedVatReturnId]);

  const guidance = deriveFinanceTaxCloseGuidance(state.preflight);
    ...
    <div className="overflow-hidden rounded-xl border border-black/[0.07] bg-white text-[#2A2723]">
      <div className="flex flex-col gap-3 border-b border-black/[0.07] p-3.5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <div className="mt-1 text-[12px] font-semibold">What is stopping this filing, who should act, and what proves it is resolved.</div>
          <div className="mt-1 max-w-4xl text-[9px] leading-4 text-[#817B73]">Unlike a normal task list, these dependencies come from live VAT evidence. They disappear only when the accounting or authority condition is actually corrected; nobody can manually tick a blocker complete.</div>
        </div>
      ...
      {state.loading && !guidance ? <div className="p-4 text-[9px] text-[#817B73]">Reading current Tax evidence…</div> : null}
      ...
      {guidance ? <>
        ...
        <div className="border-t border-black/[0.07] bg-[#FAF9F7] px-3.5 py-2.5 text-[8px] leading-4 text-[#817B73]">{guidance.truth_rule} {guidance.communication_rule}</div>
      </> : null}
    </div>
  </section>;
}
