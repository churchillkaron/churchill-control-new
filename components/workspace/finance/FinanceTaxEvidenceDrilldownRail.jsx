...
import { ChevronLeft, ChevronRight, FileSearch2, RefreshCw, ShieldCheck } from "lucide-react";
...
function EvidenceRecord({ issue }) {
  ...
  return <div className="rounded-lg border border-black/[0.07] bg-white p-3">
    ...
    <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-black/[0.05] bg-[#FAF9F7] px-2.5 py-2 text-[8px] text-[#817B73]">
      <span className="font-mono">{issue.source_id || "context"}</span>
      <span>{target ? `Workspace target: ${target.workspace} · exact record · no context mutation` : "Governed context evidence"}</span>
    </div>
  </div>;
}
  ...
  return <section className="mx-auto mt-3 max-w-[1760px] px-4 sm:px-5 lg:px-6">
    <div className="overflow-hidden rounded-xl border border-black/[0.07] bg-white text-[#2A2723]">
      <div className="flex flex-col gap-3 border-b border-black/[0.07] p-3.5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[0.15em] text-[#9A7045]"><FileSearch2 size={11}/> Tax evidence inspector</span><span className="rounded-md border border-black/[0.07] bg-[#F7F6F3] px-2 py-1 text-[7px] font-semibold uppercase tracking-[0.07em] text-[#716B63]">Read only · full population</span></div>
          <div className="mt-1 text-[12px] font-semibold">Trace every blocker to the accounting evidence that caused it.</div>
          <div className="mt-1 max-w-4xl text-[9px] leading-4 text-[#817B73]">The inspector rebuilds live Tax truth, re-evaluates the selected blocker over the exact filing population, and shows compact source-document, tax-line, VAT-rule and posting proof. Viewing evidence never clears a blocker and never switches Business Context.</div>
        </div>
      ...
      {dependencies.length ? <div className="flex gap-1.5 overflow-x-auto border-b border-black/[0.07] bg-[#FAF9F7] p-2.5">{dependencies.map(item => <button key={item.code} type="button" onClick={() => setSelectedCode(item.code)} className={`shrink-0 rounded-lg border px-2.5 py-1.5 text-[8px] font-semibold ${item.code === selectedCode ? "border-[#A37849]/25 bg-[#FFF9F0] text-[#76583A]" : "border-black/[0.07] bg-white text-[#716B63]"}`}>{item.title} · {item.evidence_count || 0}</button>)}</div> : null}

      {selected ? <div className="grid gap-px border-b border-black/[0.07] bg-black/[0.05] lg:grid-cols-3">
        <div className="bg-[#FAF9F7] p-3"><div className="text-[8px] uppercase tracking-[0.08em] text-[#968F87]">Selected blocker</div><div className="mt-1 text-[10px] font-semibold">{selected.title}</div><div className="mt-0.5 text-[8px] text-[#918B83]">{selected.code}</div></div>
