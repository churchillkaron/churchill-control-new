...
export default function FinanceTaxWorkCenter({ organizationId, entityId, periodId, selectedVatReturnId, onSelectedVatReturnIdChange }) {
  ...
  const [form, setForm] = useState({ jurisdiction_code: "", period_start: "", period_end: "", filing_due_date: "", registration_reference: "", notes: "" });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setForm(current => ({ ...current, jurisdiction_code: current.jurisdiction_code || body.setup?.suggested_jurisdiction || body.setup?.vat_regimes?.[0] || "", registration_reference: current.registration_reference || body.setup?.registration_reference || "" }));
      } catch (loadError) {
  ...
  async function createReturn() {
    ...
    try {
      ...
      const body = await requestJson("/api/finance/tax/runtime", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ organizationId, entityId, periodId, jurisdictionCode: form.jurisdiction_code, periodStart: form.period_start, periodEnd: form.period_end, filingDueDate: form.filing_due_date || null, registrationReference: form.registration_reference || null, notes: form.notes }) });
    ...
    <main className="min-h-[calc(100vh-112px)] bg-[#F7F6F3] text-[#1B1A18]">
      <div className="mx-auto max-w-[1760px] px-4 py-4 sm:px-5 lg:px-6 lg:py-5">
        ...
        {!organizationId || !entityId ? <div className="mt-4 rounded-xl border border-amber-700/15 bg-amber-50 p-4 text-[12px] text-amber-900">Select a legal entity before working on tax filings.</div> : <>
          ...
          {newMode ? <section className="mt-3 overflow-hidden rounded-xl border border-black/[0.07] bg-white"><div className="border-b border-black/[0.07] px-4 py-3"><div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-[#9A7045]">Filing obligation</div><div className="mt-1 text-[13px] font-semibold">New VAT filing period</div><div className="mt-1 text-[10px] text-[#817B73]">Create the statutory work item first. Avantiqo will then evaluate the full accounting evidence population before calculation.</div></div><div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-5"><label className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[#817B73]">Jurisdiction<select value={form.jurisdiction_code} onChange={event => setForm(current => ({ ...current, jurisdiction_code: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2.5 text-[10px] font-normal normal-case tracking-normal"><option value="">Select</option>{workspace?.setup?.vat_regimes?.map(value => <option key={value}>{value}</option>)}</select></label><label className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[#817B73]">Period start<input type="date" value={form.period_start} onChange={event => setForm(current => ({ ...current, period_start: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2.5 text-[10px] font-normal normal-case tracking-normal" /></label><label className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[#817B73]">Period end<input type="date" value={form.period_end} onChange={event => setForm(current => ({ ...current, period_end: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2.5 text-[10px] font-normal normal-case tracking-normal" /></label><label className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[#817B73]">Filing due date<input type="date" value={form.filing_due_date} onChange={event => setForm(current => ({ ...current, filing_due_date: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2.5 text-[10px] font-normal normal-case tracking-normal" /></label><label className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[#817B73]">Registration reference<input value={form.registration_reference} onChange={event => setForm(current => ({ ...current, registration_reference: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2.5 text-[10px] font-normal normal-case tracking-normal" /></label></div><div className="px-4 pb-4"><textarea value={form.notes} onChange={event => setForm(current => ({ ...current, notes: event.target.value }))} placeholder="Optional filing notes…" className="min-h-[68px] w-full rounded-lg border border-black/[0.09] bg-[#FAF9F7] p-2.5 text-[10px] outline-none" /></div><div className="flex justify-end gap-2 border-t border-black/[0.07] p-3"><button onClick={() => setNewMode(false)} className="h-9 rounded-lg border border-black/[0.09] bg-white px-3 text-[10px] font-medium">Cancel</button><button onClick={createReturn} disabled={busy} className="h-9 rounded-lg bg-[#1F1E1B] px-3.5 text-[10px] font-semibold text-white disabled:opacity-40">{busy ? "Creating…" : "Create filing obligation"}</button></div></section> : null}
