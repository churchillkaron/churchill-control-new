...
export default function FinanceTaxPostingEvidenceReview({ issue, source, journal, navigation }) {
  ...
  return <div className="mt-3 overflow-hidden rounded-lg border border-red-700/15 bg-red-50/25">
    <div className="flex flex-col gap-2 border-b border-red-700/12 p-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-red-800">VAT posting proof</div>
        <div className="mt-1 text-[11px] font-semibold">{sideLabel} · exact source document, posting state and journal proof</div>
        <div className="mt-0.5 max-w-4xl text-[11px] leading-4 text-[#817B73]">This card presents the same posting evidence evaluated by the full live filing population. It does not post, approve, reverse or repair any accounting record.</div>
      </div>
      <span className="shrink-0 rounded-md border border-red-700/15 bg-red-50 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.07em] text-red-800">{postingFailureLabel(issue?.code)}</span>
    </div>
    ...
    <div className="grid gap-px bg-[#F7F6F3]/[0.05] sm:grid-cols-4">
      ...
      <div className="bg-white p-2.5"><div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#968F87]">Posting link</div><div className={`mt-1 break-all text-[11px] font-semibold ${journalIdentity ? "text-[#4E4943]" : "text-red-800"}`}>{journalIdentity || "Missing"}</div><div className="mt-0.5 text-[11px] text-[#918B83]">{linkProof}</div></div>
    </div>
    ...
    <div className="grid gap-2 p-3 lg:grid-cols-[1.15fr_0.85fr]">
      <div className="overflow-hidden rounded-lg border border-black/[0.06] bg-white">
        ...
        <div className="grid gap-px bg-[#F7F6F3]/[0.05] sm:grid-cols-4">
          ...
          <div className="bg-white p-2.5"><div className="text-[11px] uppercase text-[#968F87]">Reversed</div><div className={`mt-1 text-[11px] font-semibold ${reversed ? "text-red-800" : journal ? "text-emerald-800" : "text-red-800"}`}>{journal ? reversed ? "Yes" : "No" : "No journal"}</div></div>
        </div>
      </div>
      <div className="rounded-lg border border-black/[0.06] bg-white p-2.5">
        ...
        <div className="mt-1 text-[11px] font-semibold text-[#4E4943]">{postingRepairText(issue?.code)}</div>
        <div className="mt-1 text-[11px] leading-4 text-[#817B73]">Evidence cannot post, approve, unreverse or mark this blocker fixed. Live Tax preflight clears it only after governed source accounting truth satisfies the posting predicate.</div>
      </div>
    </div>
