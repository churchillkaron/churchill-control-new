...
export default function FinanceTaxReturnCloseSheet({ organizationId, entityId, selectedVatReturnId, onStageChange, onEvidenceFocus }) {
  ...
  if (!snapshot) {
    return <section className="mx-auto mt-3 max-w-[1760px] px-4 sm:px-5 lg:px-6"><div className="rounded-xl border border-black/[0.07] bg-white p-3 text-[11px] text-[#817B73]">{loading ? "Loading VAT close sheet…" : error || "Select a VAT filing to see the close sheet."}</div></section>;
  }
      ...
      <div className="overflow-hidden rounded-2xl border border-black/[0.08] bg-white shadow-[0_10px_35px_rgba(35,31,27,0.04)]">
        <div className="flex flex-col gap-3 border-b border-black/[0.07] px-4 py-3.5 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#9A7045]">VAT close sheet</div>
            <div className="mt-1 flex flex-wrap items-center gap-2"><h2 className="text-[16px] font-semibold tracking-[-0.02em] text-[#26231F]">{vatReturn.jurisdiction_code || "VAT"} · {date(vatReturn.period_start)} — {date(vatReturn.period_end)}</h2><span className={`rounded-md border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] ${tone(state)}`}>{state.replaceAll("_", " ") || "DRAFT"}</span></div>
            <div className="mt-1 text-[11px] text-[#817B73]">One filing, one current accounting truth, one next action.</div>
          </div>
          <div className="flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center">
            <button onClick={() => inspectEvidence()} disabled={busy || loading} className="h-9 rounded-lg border border-black/[0.09] bg-white px-3 text-[11px] font-semibold text-[#4B4640] disabled:cursor-not-allowed disabled:opacity-40">Inspect evidence</button>
            <button onClick={primaryAction} disabled={busy || loading} className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-[#1F1E1B] px-3.5 text-[11px] font-semibold text-[#191919] disabled:cursor-not-allowed disabled:opacity-40">{busy ? <RefreshCw size={12} className="animate-spin" /> : submitted ? <FileCheck2 size={12} /> : needsFix ? <AlertTriangle size={12} /> : <ArrowRight size={12} />}{busy ? "Working…" : primaryLabel}</button>
          </div>
        </div>

        <div className={`grid gap-px bg-[#F7F6F3]/[0.06] ${hasPreviousAdjustment ? "sm:grid-cols-3 xl:grid-cols-6" : "sm:grid-cols-2 xl:grid-cols-5"}`}>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Output VAT</div><div className="mt-1 text-[13px] font-semibold tabular-nums text-[#2E2A26]">{money(snapshot.current?.output_tax, currency)}</div><div className="mt-0.5 text-[11px] text-[#817B73]">{outputIncluded} included sales document{outputIncluded === 1 ? "" : "s"}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Input VAT</div><div className="mt-1 text-[13px] font-semibold tabular-nums text-[#2E2A26]">{money(snapshot.current?.input_tax, currency)}</div><div className="mt-0.5 text-[11px] text-[#817B73]">{inputIncluded} included purchase document{inputIncluded === 1 ? "" : "s"}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Net position</div><div className="mt-1 text-[13px] font-semibold tabular-nums text-[#2E2A26]">{payable > 0 ? `${money(payable, currency)} payable` : refund > 0 ? `${money(refund, currency)} refund` : money(0, currency)}</div><div className="mt-0.5 text-[11px] text-[#817B73]">Output VAT less input VAT</div></div>
          {hasPreviousAdjustment ? <div className="bg-[#FAF9F7] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Prior adjustment</div><div className="mt-1 text-[13px] font-semibold tabular-nums text-[#2E2A26]">{money(previousAdjustment, currency)}</div></div> : null}
          <div className="bg-[#FAF9F7] p-3"><div className="flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]"><CalendarClock size={10} /> Deadline</div><div className="mt-1 text-[13px] font-semibold text-[#2E2A26]">{date(vatReturn.filing_due_date)}</div><div className={`mt-0.5 text-[11px] ${dueDays !== null && dueDays < 0 ? "text-red-800" : "text-[#817B73]"}`}>{dueDays === null ? "Governed legal date unavailable" : dueDays < 0 ? `${Math.abs(dueDays)} day${Math.abs(dueDays) === 1 ? "" : "s"} overdue` : dueDays === 0 ? "Due today" : `${dueDays} day${dueDays === 1 ? "" : "s"} remaining`}{snapshot?.due?.legal_time_zone ? ` · ${snapshot.due.legal_time_zone}` : ""}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Readiness</div><div className={`mt-1 inline-flex items-center gap-1 text-[11px] font-semibold ${submitted || canSubmit ? "text-emerald-800" : needsFix ? "text-red-800" : "text-amber-900"}`}>{submitted || canSubmit ? <CheckCircle2 size={12} /> : needsFix ? <AlertTriangle size={12} /> : <RefreshCw size={12} />}{submitted ? "Filed" : canSubmit ? "Ready to file" : needsFix ? "Needs attention" : "Ready to calculate"}</div><div className="mt-0.5 text-[11px] text-[#817B73]">{blockers.length ? `${blockers.length} live blocker${blockers.length === 1 ? "" : "s"}` : snapshot.calculation_stale ? "Source evidence changed" : "Live preflight current"}</div></div>
        </div>

        <div className="border-t border-black/[0.07] px-4 py-3.5">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.13em] text-[#9A7045]">Review before filing</div>
              <div className="mt-1 text-[11px] text-[#817B73]">Reconcile the VAT result to the complete governed source population before recording the authority filing.</div>
            </div>
            <span className={`inline-flex w-fit items-center gap-1 rounded-md border px-2 py-1 text-[11px] font-semibold ${snapshot.calculation_stale || needsFix ? "border-red-700/15 bg-red-50 text-red-800" : submitted || hasCalculatedSnapshot ? "border-emerald-700/15 bg-emerald-50 text-emerald-800" : "border-amber-700/15 bg-amber-50 text-amber-900"}`}>{snapshot.calculation_stale || needsFix ? <AlertTriangle size={10} /> : submitted || hasCalculatedSnapshot ? <CheckCircle2 size={10} /> : <RefreshCw size={10} />}{calculationProofLabel}</span>
          </div>

          <div className="mt-3 grid overflow-hidden rounded-xl border border-black/[0.07] bg-[#F7F6F3]/[0.05] md:grid-cols-[1fr_auto_1fr_auto_1fr]">
            <div className="bg-white p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Sales VAT included</div><div className="mt-1 text-[12px] font-semibold tabular-nums text-[#312E2A]">{money(snapshot.current?.output_tax, currency)}</div><div className="mt-1 text-[11px] leading-4 text-[#817B73]">{outputIncluded} of {outputObserved} VAT-bearing sales document{outputObserved === 1 ? "" : "s"} included{creditNotes ? ` · ${creditNotes} credit note${creditNotes === 1 ? "" : "s"}` : ""}.</div></div>
            <div className="flex items-center justify-center bg-[#FAF9F7] px-3 py-2 text-[14px] font-semibold text-[#9B948C]">−</div>
            <div className="bg-white p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Purchase VAT included</div><div className="mt-1 text-[12px] font-semibold tabular-nums text-[#312E2A]">{money(snapshot.current?.input_tax, currency)}</div><div className="mt-1 text-[11px] leading-4 text-[#817B73]">{inputIncluded} of {inputObserved} VAT-bearing purchase document{inputObserved === 1 ? "" : "s"} included.</div></div>
            <div className="flex items-center justify-center bg-[#FAF9F7] px-3 py-2 text-[14px] font-semibold text-[#9B948C]">=</div>
            <div className="bg-[#FFF9F0] p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#8A633E]">Current VAT result</div><div className="mt-1 text-[12px] font-semibold tabular-nums text-[#312E2A]">{payable > 0 ? `${money(payable, currency)} payable` : refund > 0 ? `${money(refund, currency)} refund` : money(0, currency)}</div><div className="mt-1 text-[11px] leading-4 text-[#76583A]">Built from the live preflight population, not from a manually entered return total.</div></div>
          </div>

          <div className="mt-2 grid gap-2 lg:grid-cols-2">
            <div className="rounded-lg border border-black/[0.07] bg-[#FAF9F7] p-3">
              <div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Source coverage</div>
              <div className="mt-1 text-[11px] font-semibold text-[#3F3A35]">{exceptionTotal ? `${exceptionTotal} source exception${exceptionTotal === 1 ? "" : "s"} detected across the filing population.` : "Complete VAT source population has no recorded source exception."}</div>
              <div className="mt-1 text-[11px] leading-4 text-[#817B73]">Included means the document passed the governed coding, approval, posting and exchange-rate rules required for this VAT calculation. {warnings.length ? `${warnings.length} warning${warnings.length === 1 ? "" : "s"} still require human review.` : "No non-blocking review warning is open."}</div>
            </div>
            <div className="rounded-lg border border-black/[0.07] bg-[#FAF9F7] p-3">
              <div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-[#928C84]">Calculation evidence</div>
              {hasCalculatedSnapshot ? <><div className="mt-1 text-[11px] font-semibold text-[#3F3A35]">Last governed calculation · {dateTime(calculatedAt)}</div><div className="mt-1 text-[11px] leading-4 text-[#817B73]">{snapshot.calculation_stale ? "Evidence changed since the last calculation. Recalculate before filing." : "Stored calculation matches the current governed evidence population."}</div>{freshnessReasons.length ? <div className="mt-1 text-[11px] leading-4 text-red-800">Changed: {freshnessReasons.join(" · ")}</div> : null}</> : <><div className="mt-1 text-[11px] font-semibold text-[#3F3A35]">No governed calculation has been saved yet.</div><div className="mt-1 text-[11px] leading-4 text-[#817B73]">The figures above are the live evidence preview. Use Calculate from evidence to persist the filing calculation after all blocking checks pass.</div></>}
            </div>
          </div>
          ...
          {warnings.length ? <div className="mt-2 overflow-hidden rounded-xl border border-amber-800/15 bg-amber-50/40">
            <div className="flex flex-col gap-2 border-b border-amber-800/10 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between">
              <div><div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-amber-900">Review items · non-blocking</div><div className="mt-0.5 text-[11px] leading-4 text-[#817B73]">These items need accountant attention but do not become accounting truth by being acknowledged. Trace the source evidence before filing.</div></div>
              <button onClick={() => inspectEvidence(upper(warnings[0]?.code))} className="h-8 shrink-0 rounded-lg border border-amber-900/15 bg-white px-3 text-[11px] font-semibold text-amber-950">Inspect review evidence</button>
            </div>
            <div className="divide-y divide-amber-900/10">
              {warnings.map(item => {
                ...
                return <div key={code || item.label} className="grid gap-2 px-3 py-2.5 sm:grid-cols-[150px_1fr_auto] sm:items-start">
                  <div><div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#9A7045]">{warningControlLabel(code)}</div><div className="mt-0.5 text-[11px] text-[#928C84]">Warning · review only</div></div>
                  <div><div className="text-[11px] font-semibold text-[#3F3A35]">{item.label || code.replaceAll("_", " ")}</div><div className="mt-0.5 text-[11px] leading-4 text-[#817B73]">{item.detail || "Inspect the governed source evidence before filing."}</div></div>
                  <div className="flex items-center gap-2 sm:flex-col sm:items-end"><div className="text-left sm:text-right"><div className="text-[12px] font-semibold tabular-nums text-amber-950">{count}</div><div className="text-[11px] uppercase tracking-[0.08em] text-[#928C84]">item{count === 1 ? "" : "s"}</div></div><button onClick={() => inspectEvidence(code)} className="h-7 rounded-md border border-amber-900/15 bg-white px-2.5 text-[11px] font-semibold text-amber-950">Trace this control</button></div>
                </div>;
              })}
            </div>
          </div> : null}
        ...
        {needsFix ? <div className="flex items-start gap-2 border-t border-red-700/10 bg-red-50 px-4 py-3 text-[11px] leading-4 text-red-900"><AlertTriangle size={12} className="mt-0.5 shrink-0" /><div><div className="font-semibold">Do not calculate around a blocker.</div><div className="mt-0.5">Open Fix issues to correct the source accounting workflow. Ownership, client requests and AI advice cannot clear live accounting truth.</div></div></div> : null}

        {submissionOpen && canSubmit ? <div className="border-t border-black/[0.07] bg-[#FAF9F7] px-4 py-3"><div className="text-[11px] font-semibold uppercase tracking-[0.11em] text-[#817B73]">Real authority filing evidence</div><div className="mt-2 flex flex-col gap-2 sm:flex-row"><input value={submissionReference} onChange={event => setSubmissionReference(event.target.value)} placeholder="Submission / receipt reference" className="h-9 min-w-0 flex-1 rounded-lg border border-black/[0.09] bg-white px-3 text-[11px] outline-none" /><button onClick={() => setSubmissionOpen(false)} className="h-9 rounded-lg border border-black/[0.09] bg-white px-3 text-[11px] font-medium">Cancel</button><button onClick={recordSubmission} disabled={busy || !submissionReference.trim()} className="h-9 rounded-lg bg-[#1F1E1B] px-3.5 text-[11px] font-semibold text-[#191919] disabled:opacity-40">Confirm filed</button></div><div className="mt-1.5 text-[11px] leading-4 text-[#817B73]">Avantiqo records the authority reference you actually received. It does not claim a government connector submitted this return.</div></div> : null}

        {error ? <div className="border-t border-red-700/10 bg-red-50 px-4 py-2.5 text-[11px] text-red-800">{error}</div> : null}
      </div>
    </section>
  );
}
