...
export default function FinanceTaxSettlementRail({ organizationId, entityId }) {
  ...
  async function load() {
    ...
    try {
      ...
      const row = tax.preflight?.return || tax.returns?.[0] || null;
      if (!row || upper(row.status) !== "SUBMITTED") {
        setState({ loading: false, error: "", body: null, returnId: row?.id || null });
        return;
      }
      ...
      setAccountForm({
        recoverable_tax_account_id: config.recoverable_tax_account_id || "",
        payable_tax_account_id: config.payable_tax_account_id || "",
        settlement_account_id: config.settlement_account_id || "",
      });
      setSetupMode(!body.settlement?.configuration_ready);
      setCashForm(current => ({
        ...current,
        amount: body.settlement?.amount_remaining ? String(body.settlement.amount_remaining) : current.amount,
        bank_account_id: current.bank_account_id || body.bank_accounts?.find(item => item.is_default)?.id || body.bank_accounts?.[0]?.id || "",
      }));
    } catch (error) {
      setState(current => ({ ...current, loading: false, error: error?.message || "Tax settlement could not be loaded" }));
    }
  }
  ...
  async function saveSetup() {
    try {
      ...
      await requestJson("/api/finance/vat-returns/settlement", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ organizationId, entityId, ...accountForm }),
      });
      setSetupMode(false);
      await load();
    } catch (error) {
      setState(current => ({ ...current, error: error?.message || "Settlement setup could not be saved" }));
  ...
  async function action(action, extras = {}) {
    ...
    try {
      ...
      await requestJson("/api/finance/vat-returns/settlement", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ organizationId, entityId, vatReturnId: state.returnId, action, ...extras }),
      });
      await load();
  ...
  if (!organizationId || !entityId || (!state.loading && !state.body)) return null;
    ...
    <section className="mx-auto mt-3 max-w-[1760px] px-4 sm:px-5 lg:px-6">
      <div className="overflow-hidden rounded-xl border border-black/[0.07] bg-white text-[#2A2723]">
        <div className="flex flex-col gap-3 border-b border-black/[0.07] p-3.5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-[#9A7045]"><Landmark size={12} /> Tax settlement</span>
              {settlement ? <span className={`rounded-md border px-2 py-1 text-[8px] font-semibold uppercase tracking-[0.07em] ${stateTone(settlement.state)}`}>{String(settlement.state).replaceAll("_", " ")}</span> : null}
            </div>
            <div className="mt-1 text-[12px] font-semibold">Filing is not finished until the tax control balance is cleared.</div>
            <div className="mt-1 text-[9px] leading-4 text-[#817B73]">Paid is not cleared. Avantiqo keeps the authority liability, payment/refund journal and bank reconciliation evidence separate so accountants can see the exact remaining control.</div>
          </div>
        ...
        {settlement ? <>
          ...
          {setupMode ? <div className="p-3.5">
            <div className="text-[10px] font-semibold">Settlement setup required</div>
            <div className="mt-1 text-[9px] text-[#817B73]">Map the actual entity accounts once. Avantiqo does not guess VAT control accounts by account name.</div>
            <div className="mt-3 grid gap-2 lg:grid-cols-3">{[
              ["recoverable_tax_account_id", "Recoverable input VAT"],
              ["payable_tax_account_id", "Output VAT payable"],
              ["settlement_account_id", "Tax settlement control"],
            ].map(([key, label]) => <label key={key} className="text-[8px] font-semibold uppercase tracking-[0.08em] text-[#817B73]">{label}<select value={accountForm[key]} onChange={event => setAccountForm(current => ({ ...current, [key]: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2 text-[9px] font-normal normal-case tracking-normal"><option value="">Select account</option>{accountOptions.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>)}</div>
            <div className="mt-3 flex justify-end"><button onClick={saveSetup} disabled={busy} className="h-9 rounded-lg bg-[#1F1E1B] px-3.5 text-[9px] font-semibold text-white disabled:opacity-40">Save governed VAT accounts</button></div>
          </div> : null}

          {!setupMode && settlement.state === "LIABILITY_POSTING_REQUIRED" ? <div className="flex flex-col gap-2 p-3.5 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-[10px] font-semibold">Post filed VAT into settlement control</div><div className="mt-1 text-[9px] text-[#817B73]">Output VAT and recoverable input VAT are closed into the configured tax settlement account. Amendments post only their incremental delta.</div></div><div className="flex gap-2"><input type="date" value={postingDate} onChange={event => setPostingDate(event.target.value)} className="h-9 rounded-lg border border-black/[0.09] bg-white px-2 text-[9px]" /><button onClick={() => action("post_liability", { postingDate })} disabled={busy} className="h-9 rounded-lg bg-[#1F1E1B] px-3.5 text-[9px] font-semibold text-white disabled:opacity-40">Post tax liability</button></div></div> : null}
        ...
        </> : <div className="p-4 text-[9px] text-[#817B73]">Loading tax settlement evidence…</div>}
      </div>
    </section>
  );
}
