...
export default function FinanceTaxSettlementRail({ organizationId, entityId, selectedVatReturnId }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPostingDate(body.liability_posting_control?.default_posting_date || row.period_end || "");
      setPostingDateReason("");
  ...
  const accounts = useMemo(() => state.body?.accounts || [], [state.body?.accounts]);
  ...
  const postingControl = state.body?.liability_posting_control || null;
  const defaultPostingDate = postingControl?.default_posting_date || "";
  const alternatePostingDate = Boolean(postingDate && defaultPostingDate && postingDate !== defaultPostingDate);
  const defaultPeriodOpen = postingControl?.default_period_open === true;
  ...
  return <section className="mx-auto mt-3 max-w-[1760px] px-4 sm:px-5 lg:px-6">
    <div className="overflow-hidden rounded-xl border border-black/[0.07] bg-white text-[#2A2723]">
      <div className="flex flex-col gap-3 border-b border-black/[0.07] p-3.5 lg:flex-row lg:items-start lg:justify-between">
        <div><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-[#9A7045]"><Landmark size={12} /> Tax settlement</span>{settlement ? <span className={`rounded-md border px-2 py-1 text-[8px] font-semibold uppercase tracking-[0.07em] ${stateTone(settlement.state)}`}>{String(settlement.state).replaceAll("_", " ")}</span> : null}</div><div className="mt-1 text-[12px] font-semibold">Filing is not finished until the tax control balance is cleared.</div><div className="mt-1 text-[9px] leading-4 text-[#817B73]">Paid is not cleared. Avantiqo keeps the authority liability, payment/refund journal and bank reconciliation evidence separate so accountants can see the exact remaining control.</div></div>
      ...
      {settlement ? <>
        <div className="grid gap-px border-b border-black/[0.07] bg-[#F7F6F3]/[0.05] sm:grid-cols-2 lg:grid-cols-5">
          <div className="bg-[#FAF9F7] p-3"><div className="text-[8px] uppercase tracking-[0.08em] text-[#968F87]">Filed version</div><div className="mt-1 text-[11px] font-semibold">{target?.label || "Original"}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[8px] uppercase tracking-[0.08em] text-[#968F87]">Authority balance</div><div className="mt-1 text-[11px] font-semibold tabular-nums">{money(Math.abs(settlement.target_signed_balance), currency)} {settlement.target_signed_balance < 0 ? "refund" : "payable"}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[8px] uppercase tracking-[0.08em] text-[#968F87]">Cash settled</div><div className="mt-1 text-[11px] font-semibold tabular-nums">{money(Math.abs(settlement.cash_settled_signed), currency)}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[8px] uppercase tracking-[0.08em] text-[#968F87]">Remaining</div><div className="mt-1 text-[11px] font-semibold tabular-nums">{money(settlement.amount_remaining, currency)}</div></div>
          <div className="bg-[#FAF9F7] p-3"><div className="text-[8px] uppercase tracking-[0.08em] text-[#968F87]">Bank evidence</div><div className="mt-1 text-[11px] font-semibold">{settlement.bank_evidence_complete ? "Reconciled" : settlement.cash_events?.length ? "Match pending" : "Not applicable yet"}</div></div>
        </div>
        {settlement.needs_attention ? <div className="m-3 flex items-start gap-2 rounded-lg border border-red-700/15 bg-red-50 p-2.5 text-[9px] text-red-800"><AlertTriangle size={12} className="mt-0.5" /><div><b>Settlement evidence changed.</b> A linked liability/payment journal has been reversed. Avantiqo has stopped counting that journal toward the tax balance.</div></div> : null}
        {setupMode ? <div className="p-3.5"><div className="text-[10px] font-semibold">Settlement setup required</div><div className="mt-1 text-[9px] text-[#817B73]">Map the actual entity accounts once. Avantiqo does not guess VAT control accounts by account name.</div><div className="mt-3 grid gap-2 lg:grid-cols-3">{[["recoverable_tax_account_id", "Recoverable input VAT"],["payable_tax_account_id", "Output VAT payable"],["settlement_account_id", "Tax settlement control"]].map(([key,label]) => <label key={key} className="text-[8px] font-semibold uppercase tracking-[0.08em] text-[#817B73]">{label}<select value={accountForm[key]} onChange={event => setAccountForm(current => ({ ...current, [key]: event.target.value }))} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2 text-[9px] font-normal normal-case tracking-normal"><option value="">Select account</option>{accountOptions.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>)}</div><div className="mt-3 flex justify-end"><button onClick={saveSetup} disabled={busy} className="h-9 rounded-lg bg-[#1F1E1B] px-3.5 text-[9px] font-semibold text-[#191919] disabled:opacity-40">Save governed VAT accounts</button></div></div> : null}
        {!setupMode && settlement.state === "LIABILITY_POSTING_REQUIRED" ? <div className="border-t border-black/[0.07] p-3.5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-[10px] font-semibold">Post filed VAT into settlement control</div><div className="mt-1 max-w-3xl text-[9px] leading-4 text-[#817B73]">The liability journal defaults to the last day of the VAT period so the control transfer belongs to the reporting period. Amendments post only their incremental delta. The central Finance gateway still refuses a closed accounting period.</div></div><div className={`rounded-lg border px-2.5 py-2 text-[8px] font-semibold ${defaultPeriodOpen ? "border-emerald-700/15 bg-emerald-50 text-emerald-800" : "border-amber-700/15 bg-amber-50 text-amber-900"}`}>{defaultPeriodOpen ? "Period-end is open" : "Period-end is closed / unavailable"}</div></div>
          <div className="mt-3 grid gap-2 lg:grid-cols-[190px_minmax(260px,1fr)_auto] lg:items-end">
            <label className="text-[8px] font-semibold uppercase tracking-[0.08em] text-[#817B73]">Journal posting date<input type="date" value={postingDate} onChange={event => { setPostingDate(event.target.value); if (event.target.value === defaultPostingDate) setPostingDateReason(""); }} className="mt-1.5 h-9 w-full rounded-lg border border-black/[0.09] bg-white px-2 text-[9px] font-normal normal-case tracking-normal" /></label>
            <label className="text-[8px] font-semibold uppercase tracking-[0.08em] text-[#817B73]">{alternatePostingDate ? "Alternate-date reason" : "Period policy"}{alternatePostingDate ? <input value={postingDateReason} onChange={event => setPostingDateReason(event.target.value)} placeholder="Why the VAT liability is posted outside period-end" className="mt-1.5 h-9 w-full rounded-lg border border-amber-700/20 bg-amber-50/40 px-2.5 text-[9px] font-normal normal-case tracking-normal" /> : <div className="mt-1.5 flex h-9 items-center rounded-lg border border-black/[0.07] bg-[#FAF9F7] px-2.5 text-[9px] font-normal normal-case tracking-normal text-[#716B63]">Default: {date(defaultPostingDate)} · {postingControl?.default_period?.period_name || postingControl?.default_period?.status || "accounting period"}</div>}</label>
            <button onClick={() => action("post_liability", { postingDate, postingDateReason: alternatePostingDate ? postingDateReason : null })} disabled={busy || !postingDate || (alternatePostingDate && !clean(postingDateReason)) || (!defaultPeriodOpen && !alternatePostingDate)} className="h-9 rounded-lg bg-[#1F1E1B] px-3.5 text-[9px] font-semibold text-[#191919] disabled:cursor-not-allowed disabled:opacity-40">Post tax liability</button>
          </div>
      ...
      </> : state.loading ? <div className="p-4 text-[9px] text-[#817B73]">Loading tax settlement evidence…</div> : null}
    </div>
  </section>;
}
