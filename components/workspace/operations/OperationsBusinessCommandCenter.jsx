    ...
    <main className="min-h-[calc(100vh-61px)] bg-[#F7F6F3] px-4 py-6 text-[#191919] md:px-6 lg:px-8">
      <div className="mx-auto max-w-[1640px]">
        <header className="rounded-[26px] border border-black/[0.075] bg-white p-6 shadow-[0_12px_38px_rgba(31,27,20,0.055)] md:p-7">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div className="max-w-4xl">
              ...
              <p className="mt-2.5 max-w-3xl text-[13px] leading-6 text-[#6F6B64]">Start in the operating application configured for this organization. Daily work and exceptions come first; neutral specialist capabilities stay available when needed.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-black/[0.09] bg-white px-4 py-2.5 text-[12px] text-[#5E5A54] hover:border-[#D6A66A]/45 hover:text-[#8D6338]">
                <RefreshCw size={13} className={state.loading ? "animate-spin" : ""} /> Refresh
              </button>
              <button type="button" onClick={() => setShowManagement((current) => !current)} className="inline-flex items-center gap-2 rounded-xl border border-black/[0.09] bg-white px-4 py-2.5 text-[12px] text-[#5E5A54] hover:border-[#D6A66A]/45 hover:text-[#8D6338]">
                <Settings2 size={13} /> {showManagement ? "Hide tools" : "Manage Operations"}
              </button>
              {access.can?.administer ? (
                <Link href={`/workspace/${organizationId}/operations/access-control`} className="inline-flex items-center gap-2 rounded-xl border border-[#D6A66A]/30 bg-[#D6A66A]/[0.07] px-4 py-2.5 text-[12px] text-[#8D6338]">
        ...
        {solutions.length ? (
                ...
                <article key={solution.id} className="rounded-[24px] border border-black/[0.075] bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.025)]">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="max-w-2xl">
                      ...
                      <h2 className="mt-1.5 text-[22px] font-medium tracking-[-0.03em] text-[#1C1B19]">{solution.title}</h2>
                      <p className="mt-2 text-[12px] leading-5 text-[#7B7770]">{solution.description}</p>
                    </div>
                    {primary ? <Link href={primary.href} className="inline-flex items-center gap-2 rounded-xl bg-[#1D1B18] px-4 py-2.5 text-[12px] font-medium text-white">Open control <ArrowRight size={13} /></Link> : null}
                  </div>
                  <div className="mt-5 grid gap-x-5 border-t border-black/[0.07] pt-2 sm:grid-cols-2 lg:grid-cols-3">
                    {(solution.items || []).slice(1).map((item) => (
                      <Link key={item.id} href={item.href} className="group flex items-center justify-between gap-3 border-b border-black/[0.06] py-3">
                        <div className="min-w-0"><div className="text-[12px] font-medium text-[#3B3833] group-hover:text-[#8D6338]">{item.label}</div><div className="mt-0.5 truncate text-[10px] text-[#9A968E]">{item.description}</div></div>
          ...
          <div className="mt-5 rounded-2xl border border-[#D6A66A]/20 bg-white p-5 text-[12px] text-[#6F604F]">No industry solution is configured, so Avantiqo is showing the neutral Operations kernel.</div>
        )}
        ...
        <section className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          ...
          <Metric label="Due today" value={state.loading ? "…" : Number(metrics.due_today || 0)} detail="Work due today" />
          <Metric label="Overdue" value={state.loading ? "…" : Number(metrics.overdue || 0)} detail="Past due and still open" attention />
          <Metric label="Unassigned" value={state.loading ? "…" : Number(metrics.unassigned || 0)} detail="Needs accountable owner" attention />
          <Metric label="Priority" value={state.loading ? "…" : Number(metrics.high_priority || 0)} detail="High/critical active work" attention />
          <Metric label="Completed" value={state.loading ? "…" : Number(metrics.completed_today || 0)} detail="Completed today" />
        </section>
        ...
        <section className="mt-5 rounded-[24px] border border-black/[0.075] bg-white p-5">
          <div className="border-b border-black/[0.07] pb-4"><div className="text-[10px] font-medium uppercase tracking-[0.17em] text-[#8D8982]">Cross-industry attention</div><h2 className="mt-1.5 text-[19px] font-medium tracking-[-0.025em] text-[#1C1B19]">Work that needs intervention</h2></div>
          <div className="divide-y divide-black/[0.06]">
            {!state.loading && !attention.length ? <div className="flex items-center gap-3 py-7 text-[12px] text-[#77736C]"><CheckCircle2 size={16} className="text-[#718167]" />Nothing currently requires intervention.</div> : null}
            {attention.slice(0, 8).map((row) => {
              const item = itemsByCapability.get(row.capability_id);
              if (!item) return null;
              return <Link key={row.id} href={itemHref(item)} className="group flex items-center justify-between gap-4 py-3.5"><div className="min-w-0"><div className="truncate text-[12px] font-medium text-[#34312D] group-hover:text-[#8D6338]">{row.name || row.code || item.name}</div><div className="mt-1 text-[10px] text-[#9A968E]">{item.name} · {row.status || "Open"}{row.due_at ? ` · Due ${new Date(row.due_at).toLocaleString()}` : ""}</div></div><ArrowRight size={12} className="shrink-0 text-[#B7B3AB]" /></Link>;
            })}
          </div>
        </section>
        ...
        {showManagement ? (
          <section className="mt-5 rounded-[24px] border border-black/[0.075] bg-white p-5">
            <div className="flex items-start justify-between gap-4 border-b border-black/[0.07] pb-4"><div><div className="text-[10px] font-medium uppercase tracking-[0.17em] text-[#8D8982]">Neutral Operations kernel</div><h2 className="mt-1.5 text-[19px] font-medium tracking-[-0.025em] text-[#1C1B19]">Specialist tools</h2></div><div className="text-[10px] text-[#77736C]">Runtime {readiness.loading ? "checking" : readiness.status}</div></div>
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {authorisedGroups.map((group) => <article key={group.id} className="rounded-2xl border border-black/[0.07] bg-[#FBFAF8] p-3"><div className="px-2 py-1"><div className="text-[12px] font-medium text-[#35322E]">{group.name}</div><div className="mt-1 text-[10px] leading-4 text-[#96928A]">{group.description}</div></div><div className="mt-2 divide-y divide-black/[0.055]">{group.items.map((item) => <Link key={item.capabilityId} href={itemHref(item)} className="group flex items-center justify-between gap-3 px-2 py-2.5"><span className="truncate text-[11px] text-[#5C5851] group-hover:text-[#8D6338]">{item.name}</span><ArrowRight size={10} className="shrink-0 text-[#B7B3AB]" /></Link>)}</div></article>)}
            </div>
          </section>
        ) : null}
      </div>
