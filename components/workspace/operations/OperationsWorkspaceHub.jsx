...
const PRIMARY_ACTIONS = Object.freeze([
  { capabilityId: "queue-entries", label: "Work Queue", description: "Review waiting, prioritised and unassigned operational work.", icon: ListTodo },
  { capabilityId: "work-items", label: "Active Work", description: "Open, assign, progress and complete operational work.", icon: Activity },
  { capabilityId: "operational-runs", label: "Active Runs", description: "Coordinate repeatable rounds, batches and operating cycles.", icon: TimerReset },
  { capabilityId: "handoffs", label: "Handoffs", description: "Transfer ownership and execution context without losing control.", icon: GitBranch },
  { capabilityId: "incidents", label: "Incidents", description: "Triage and resolve operational disruption and risk.", icon: AlertTriangle },
  { capabilityId: "dispatch-boards", label: "Dispatch Board", description: "See planned, dispatched, assigned and unowned work together.", icon: LayoutDashboard },
]);
...
const COMMAND_SECTIONS = Object.freeze([
  {
    ...
    icon: Activity,
    capabilityIds: ["work-items", "operational-runs", "assignments", "handoffs", "dispatch", "queue-entries"],
  },
  {
    id: "planning",
    title: "Planning",
    ...
    icon: CalendarDays,
    capabilityIds: ["operational-plans", "work-schedules", "capacity-planning", "recurring-work", "schedule-conflicts", "resource-planning"],
  },
  {
    id: "control",
    title: "Control",
    ...
    icon: ClipboardCheck,
    capabilityIds: ["operational-approvals", "checklists", "completion-evidence", "exceptions-holds", "escalations", "corrective-actions"],
  },
  {
    id: "intelligence",
    title: "Intelligence",
    ...
    icon: BarChart3,
    capabilityIds: ["operational-events", "operational-timeline", "work-history", "audit-trail", "demand-signals", "dispatch-boards"],
  },
]);

const SOLUTION_ICONS = Object.freeze({
  command: LayoutDashboard,
  pos: MonitorSmartphone,
  service: Users,
  tables: Store,
  production: UtensilsCrossed,
  handoff: GitBranch,
  orders: ShoppingCart,
  payments: CreditCard,
...
function SolutionWorkspace({ solution }) {
  return (
    <section className="rounded-[30px] border border-[#D6A66A]/25 bg-[radial-gradient(circle_at_top_left,rgba(214,166,106,0.12),transparent_38%),rgba(255,255,255,0.025)] p-5 shadow-2xl shadow-black/20 md:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {solution.items.map((item) => {
          const Icon = SOLUTION_ICONS[item.icon] || Hammer;
...
export default function OperationsWorkspaceHub() {
  ...
  const operationalSolutions = useMemo(() => resolveOrganizationOperationalSolutions({
    organization: businessContext.organization,
    organizationId,
  }), [businessContext.organization, organizationId]);
  ...
  const commandSections = useMemo(() => COMMAND_SECTIONS
    .map((section) => ({
      ...section,
      items: section.capabilityIds.map((capabilityId) => itemsByCapabilityId.get(capabilityId)).filter(Boolean),
    }))
  ...
  const visibleCapabilities = filteredGroups.reduce((sum, group) => sum + group.items.length, 0);
  const genuinelyHasNoAccess = !access.loading && !access.error && totalAuthorisedCapabilities === 0;
    ...
    <main className="min-h-screen px-6 pb-12 pt-20 text-white md:pt-24">
      <div className="mx-auto max-w-[1540px]">
        <WorkspaceHeader
          ...
          description="Open the installed operating solution for this organization, then manage cross-industry execution, planning, control and intelligence."
          actions={(
                    ...
                    return (
                      <article key={section.id} className="rounded-[28px] border border-white/10 bg-white/[0.03] p-5">
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          {section.items.map((item) => <CommandLink key={item.capabilityId} organizationId={organizationId} item={item} />)}
                        </div>
                      </article>
                    );
                  })}
