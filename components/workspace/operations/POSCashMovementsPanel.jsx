  ...
  return (
    <article className="rounded-[30px] border border-black/[0.08] bg-white p-7 xl:col-span-2">
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-[#918B83]">
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Active drawer only</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Manager / Owner</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Finance journal required</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Does not change revenue</span>
      </div>
      ...
      {!actor?.can_move_cash ? (
        <div className="mt-5 rounded-2xl border border-black/[0.08] p-4 text-sm text-[#918B83]">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-5">
          ...
          <label className="mt-5 block text-[10px] uppercase tracking-[0.18em] text-[#918B83]">
            Amount
          </label>
          <input
            type="number"
            ...
            className="mt-2 w-full rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-lg outline-none"
          />

          <label className="mt-4 block text-[10px] uppercase tracking-[0.18em] text-[#918B83]">
            Finance counter account
          </label>
          <select
            value={counterAccountId}
            onChange={(event) => setCounterAccountId(event.target.value)}
            className="mt-2 w-full rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm outline-none"
          >
          ...
          <textarea
            ...
            rows={3}
            className="mt-2 w-full resize-none rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm outline-none"
          />
              ...
              movements.map((movement) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 text-xs text-[#746E66]">
                      {account
                        ? `${account.account_code} · ${account.account_name}`
      ...
      <div className="mt-5 rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-4 text-xs leading-5 text-[#918B83]">
