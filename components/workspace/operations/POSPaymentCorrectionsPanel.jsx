...
export default function POSPaymentCorrectionsPanel({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/pos/payment-corrections?${search.toString()}`, {
        cache: "no-store",
        credentials: "include",
      });
  ...
  return (
    <article className="rounded-[30px] border border-white/10 bg-white/[0.03] p-7 xl:col-span-2">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="text-xs uppercase tracking-[0.2em] text-[#D6A66A]">Payment corrections</div>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <div>
          <div className="text-xs uppercase tracking-[0.18em] text-white/35">Eligible settled cash payments</div>
          <div className="mt-3 space-y-3">
            {loading ? (
              <div className="rounded-2xl border border-white/10 p-5 text-sm text-white/35">Loading payments...</div>
                ...
                return (
                  <div key={payment.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="font-semibold">{payment.document_number || payment.payment_reference || `Payment ${String(payment.id).slice(0, 8)}`}</div>
                        <div className="mt-1 text-[11px] text-white/30">{payment.paid_at ? new Date(payment.paid_at).toLocaleString() : "Settled payment"}</div>
                      </div>
                      <div className="text-lg font-semibold text-[#E2C48A]">{formatMoney(payment.amount, payment.currency || currencyCode)}</div>
                    </div>
                    <textarea
                      value={reason}
                      onChange={(event) => setReasonByPayment((current) => ({ ...current, [payment.id]: event.target.value }))}
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      <button type="button" disabled={busy || !activeSessionId || !actor?.can_correct || !reason.trim()} onClick={() => execute(payment, "REFUND")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-300 px-3 py-2.5 text-xs font-semibold text-black disabled:opacity-35">
                        <RotateCcw size={14} /> Refund cash
                      </button>
                      <button type="button" disabled={busy || !activeSessionId || !actor?.can_correct || !reason.trim()} onClick={() => execute(payment, "REVERSAL")} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-400/30 bg-red-500/10 px-3 py-2.5 text-xs font-semibold text-red-100 disabled:opacity-35">
              ...
              <div className="rounded-2xl border border-white/10 p-5 text-sm text-white/35">No uncorrected settled cash payments found.</div>
            )}
          </div>
        </div>

        <div>
          <div className="text-xs uppercase tracking-[0.18em] text-white/35">Correction evidence</div>
          <div className="mt-3 max-h-[520px] space-y-3 overflow-y-auto pr-1">
            {corrections.length ? corrections.map((correction) => (
              <div key={correction.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs font-semibold text-white/70">{String(correction.correction_type || "").toUpperCase()}</span>
                  <span className="font-semibold">{formatMoney(correction.amount, correction.currency_code || currencyCode)}</span>
                </div>
                <div className="mt-2 text-xs leading-5 text-white/45">{correction.reason}</div>
                <div className="mt-2 text-[10px] text-white/25">{correction.created_at ? new Date(correction.created_at).toLocaleString() : correction.id}</div>
              </div>
            )) : (
              <div className="rounded-2xl border border-white/10 p-5 text-sm text-white/35">No refunds or reversals recorded.</div>
            )}
          </div>
        </div>
      </div>
