  ...
  return (
    <article className="rounded-[30px] border border-black/[0.08] bg-white p-7 xl:col-span-2">
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-[#918B83]">
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Manager approval</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">No self-approval</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Receipt evidence required</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Finance settlement</span>
        <span className="rounded-full border border-black/[0.08] px-3 py-1.5">Immutable evidence</span>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <div className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-5">
          ...
          <div className="mt-3 grid gap-3">
            <select value={cashLocationId} onChange={(event) => setCashLocationId(event.target.value)} className="rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm">
            ...
            <select value={advanceAccountId} onChange={(event) => setAdvanceAccountId(event.target.value)} className="rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm">
            ...
            <select value={replenishSourceLocationId} onChange={(event) => setReplenishSourceLocationId(event.target.value)} className="rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm">
              <option value="">Replenishment source</option>
              {replenishSources.filter((row) => row.id !== cashLocationId).map((row) => (
                <option key={row.id} value={row.id}>{row.name}</option>
              ))}
            </select>
            <input type="number" min="0" step="0.01" value={targetBalance} onChange={(event) => setTargetBalance(event.target.value)} placeholder="Target balance (optional)" className="rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm" />
        ...
        <div className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-5">
          ...
          <select value={requestFundId} onChange={(event) => setRequestFundId(event.target.value)} className="mt-3 w-full rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm">
          ...
          <textarea rows={2} value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="Business purpose" className="mt-3 w-full resize-none rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm" />
          <input type="number" min="0" step="0.01" value={requestedAmount} onChange={(event) => setRequestedAmount(event.target.value)} placeholder="Requested amount" className="mt-3 w-full rounded-xl border border-black/[0.09] bg-white px-4 py-3 text-sm" />
      ...
      <div className="mt-6 space-y-4">
        <div className="text-xs uppercase tracking-[0.18em] text-[#918B83]">Lifecycle evidence</div>
        {loading ? (
          <div className="rounded-2xl border border-black/[0.08] p-5 text-sm text-[#918B83]">Loading petty cash...</div>
        ) : requests.length ? (
          requests.map((request) => {
            ...
            return (
              <div key={request.id} className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="font-semibold">{request.purpose}</div>
                    <div className="mt-1 text-xs text-[#918B83]">{request.requested_at ? new Date(request.requested_at).toLocaleString() : request.id}</div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="mt-1 text-[10px] uppercase text-[#918B83]">{status}</div>
                  </div>
                </div>

                {status === "PENDING" && actor?.can_manage ? (
                  <div className="mt-4 grid gap-2 sm:grid-cols-[0.4fr_1fr_auto_auto]">
                    <input type="number" min="0" step="0.01" value={decisionDraft.amount ?? request.requested_amount} onChange={(event) => setDecisionByRequest((current) => ({ ...current, [request.id]: { ...current[request.id], amount: event.target.value } }))} className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                    <input value={decisionDraft.notes || ""} onChange={(event) => setDecisionByRequest((current) => ({ ...current, [request.id]: { ...current[request.id], notes: event.target.value } }))} placeholder="Approval note / rejection reason" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                ...
                {disbursement ? (
                  <div className="mt-4 rounded-xl border border-black/[0.08] p-4">
                    <div className="flex flex-wrap justify-between gap-3 text-xs">
                      <span className="text-[#817A72]">Disbursed {disbursement.disbursement_date} · {disbursement.status}</span>
                    ...
                    {String(disbursement.status || "").toUpperCase() !== "SETTLED" ? (
                      <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                        <select value={receiptDraft.expenseAccountId || ""} onChange={(event) => setReceiptByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], expenseAccountId: event.target.value } }))} className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs">
                        ...
                        <input type="number" min="0" step="0.01" value={receiptDraft.amount || ""} onChange={(event) => setReceiptByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], amount: event.target.value } }))} placeholder="Receipt amount" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                        <input value={receiptDraft.receiptReference || ""} onChange={(event) => setReceiptByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], receiptReference: event.target.value } }))} placeholder="Receipt reference" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                        <input value={receiptDraft.evidenceUrl || ""} onChange={(event) => setReceiptByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], evidenceUrl: event.target.value } }))} placeholder="Evidence URL" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                        <input value={receiptDraft.supplier || ""} onChange={(event) => setReceiptByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], supplier: event.target.value } }))} placeholder="Supplier (optional)" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                        <input type="date" value={receiptDraft.receiptDate || today()} onChange={(event) => setReceiptByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], receiptDate: event.target.value } }))} className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                        <button type="button" disabled={actionLoading === `receipt:${disbursement.id}`} onClick={() => addReceipt(disbursement)} className="rounded-xl border border-black/[0.10] px-3 py-2 text-xs disabled:opacity-35">Add receipt evidence</button>
                      </div>
                    ) : null}
                    ...
                    {String(disbursement.status || "").toUpperCase() === "EVIDENCE_SUBMITTED" && actor?.can_settle ? (
                      <div className="mt-4 grid gap-2 sm:grid-cols-[0.5fr_1fr_auto]">
                        <input type="date" value={settlementDraft.date || today()} onChange={(event) => setSettlementByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], date: event.target.value } }))} className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                        <input value={settlementDraft.reference || ""} onChange={(event) => setSettlementByDisbursement((current) => ({ ...current, [disbursement.id]: { ...current[disbursement.id], reference: event.target.value } }))} placeholder="Settlement reference" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
          ...
          <div className="rounded-2xl border border-black/[0.08] p-5 text-sm text-[#918B83]">No petty cash requests recorded.</div>
        )}
      </div>
      ...
      {funds.length && actor?.can_manage ? (
        <div className="mt-6 grid gap-4 xl:grid-cols-2">
          {funds.map((fund) => {
            ...
            return (
              <div key={fund.id} className="rounded-2xl border border-black/[0.08] bg-[#FBF8F3] p-5">
                <div className="flex justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.18em] text-[#918B83]">Replenish fund</div>
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-[0.5fr_1fr_auto]">
                  <input type="number" min="0" step="0.01" value={draft.amount || ""} onChange={(event) => setReplenishByFund((current) => ({ ...current, [fund.id]: { ...current[fund.id], amount: event.target.value } }))} placeholder="Amount" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
                  <input value={draft.reason || ""} onChange={(event) => setReplenishByFund((current) => ({ ...current, [fund.id]: { ...current[fund.id], reason: event.target.value } }))} placeholder="Replenishment reason" className="rounded-xl border border-black/[0.09] bg-white px-3 py-2 text-xs" />
