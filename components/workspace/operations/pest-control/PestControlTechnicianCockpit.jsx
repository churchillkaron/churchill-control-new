...
function Step({ number, label, ready, active, detail }) {
  return <div className={`rounded-xl border px-3 py-2.5 ${active ? "border-[#D6A66A]/35 bg-[#D6A66A]/[0.06]" : ready ? "border-[#748267]/16 bg-[#748267]/[0.04]" : "border-black/[0.06] bg-white"}`}><div className="flex items-center gap-2"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[8px] font-medium ${ready ? "bg-[#748267] text-[#191919]" : active ? "bg-[#D6A66A] text-[#2C2925]" : "bg-[#EEEAE4] text-[#8D877F]"}`}>{ready ? "✓" : number}</span><div className="min-w-0"><div className={`truncate text-[9px] font-medium ${active ? "text-[#76583A]" : "text-[#5F5952]"}`}>{label}</div><div className="mt-0.5 truncate text-[7px] text-[#9A938A]">{detail}</div></div></div></div>;
}
...
export default function PestControlTechnicianCockpit({ organizationId }) {
  ...
  }, [selected, selectedId, loadMonitoring, loadException]);
  useEffect(() => {
