  ...
  return (
    <div className={`rounded-xl border px-3 py-2.5 ${active ? "border-[#D6A66A]/35 bg-[#D6A66A]/[0.06]" : done ? "border-[#748267]/16 bg-[#748267]/[0.04]" : "border-black/[0.06] bg-white"}`}>
      <div className="flex items-center gap-2">
        <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[8px] font-medium ${done ? "bg-[#748267] text-white" : active ? "bg-[#D6A66A] text-[#2C2925]" : "bg-[#EEEAE4] text-[#8D877F]"}`}>{done ? "✓" : number}</span>
        <div className="min-w-0"><div className={`truncate text-[9px] font-medium ${active ? "text-[#76583A]" : "text-[#5F5952]"}`}>{label}</div>{detail ? <div className="mt-0.5 truncate text-[7px] text-[#9A938A]">{detail}</div> : null}</div>
      </div>
    </div>
  );
}
...
export default function PestControlTechnicianCockpit({ organizationId }) {
  ...
  const evidenceHref = selected?.occurrence_id
    ? `/workspace/${encodeURIComponent(organizationId)}/operations/completion-evidence/${encodeURIComponent(selected.occurrence_id)}`
    : `/workspace/${encodeURIComponent(organizationId)}/operations/completion-evidence`;
  const monitoringHref = selected?.occurrence_id
    ? `${fieldServiceHref}/monitoring-round/${encodeURIComponent(selected.occurrence_id)}`
    : `${fieldServiceHref}/monitoring-rounds`;
      ...
      <div className="mx-auto max-w-[1680px]">
        <header className="flex flex-wrap items-end justify-between gap-4 border-b border-black/[0.07] pb-5">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-[11px] leading-5 text-[#777169]">Arrive once, work the exact visit, capture proof, record exceptions, and close only when every governed requirement is satisfied.</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => { load(); if (selected) loadMonitoring(selected); }} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-black/[0.08] bg-white text-[#806143]" aria-label="Refresh technician queue"><RefreshCw size={11} className={state.loading || monitoring.loading ? "animate-spin" : ""} /></button>
          </div>
        </header>
          ...
          <aside className="xl:sticky xl:top-5">
            <div className="rounded-2xl border border-black/[0.075] bg-white p-3 shadow-[0_1px_2px_rgba(0,0,0,0.025)]">
              ...
              <div className="mb-3 grid grid-cols-3 gap-1 rounded-xl bg-[#F4F2EE] p-1">
                {[["today", "Today"], ["active", "Active"], ["all", "All"]].map(([value, label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-lg px-2 py-2 text-[8px] font-medium ${filter === value ? "bg-white text-[#5C4935] shadow-sm" : "text-[#8D877F]"}`}>{label}</button>)}
              </div>
            ...
            <section className="overflow-hidden rounded-2xl border border-black/[0.075] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.025)]">
              <div className="border-b border-black/[0.06] px-5 py-5 md:px-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="min-w-0"><div className="text-[9px] font-medium uppercase tracking-[0.13em] text-[#9A744B]">{formatDate(selected.scheduled_start || selected.occurrence_at)} · {formatTime(selected.scheduled_start || selected.occurrence_at)}–{formatTime(selected.scheduled_end || selected.due_at)}</div><h2 className="mt-1.5 truncate text-[22px] font-medium tracking-[-0.035em] text-[#27231F]">{selected.customer_name || "Customer"}</h2><div className="mt-1 text-[11px] text-[#777169]">{selected.service_name || selected.name || "Service visit"}</div></div>
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-5">
                  <Step number="1" label="Arrival" state={stepStates.arrival} detail={visitStarted ? "Checked in" : "Confirm on site"} />
                  <Step number="2" label="Service work" state={stepStates.work} detail={`${completedRequired}/${requiredFields.length} required`} />
                  {monitoringApplicable ? <Step number="3" label="Monitoring" state={stepStates.monitoring} detail={monitoringReady ? "Coverage ready" : `${monitoring.round?.pending_required_points || 0} pending`} /> : <Step number="3" label="Monitoring" state="done" detail="Not required" />}
                  <Step number="4" label="Evidence" state={stepStates.proof} detail={proofReady ? "Proof ready" : "Proof required"} />
                  <Step number="5" label="Complete" state={stepStates.finish} detail={isTerminal(selected) ? "Reconciled" : completionReady ? "Ready to close" : "Waiting"} />
                </div>
              </div>

              <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_310px]">
                <div className="p-5 md:p-6">
                  <div className={`rounded-2xl border p-4 ${visitStarted || isTerminal(selected) ? "border-[#748267]/16 bg-[#748267]/[0.035]" : "border-[#D6A66A]/24 bg-[#D6A66A]/[0.045]"}`}>
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div><div className="text-[9px] font-medium uppercase tracking-[0.13em] text-[#8A867F]">1 · Arrival</div><h3 className="mt-1 text-[15px] font-medium text-[#322E29]">{visitStarted || isTerminal(selected) ? "Arrival confirmed" : "Confirm you are on site"}</h3><p className="mt-1 text-[9px] leading-4 text-[#817A72]">{visitStarted || isTerminal(selected) ? "This exact occurrence is open for governed field execution." : "Starting the visit locks execution to this customer, site, work order and scheduled occurrence."}</p></div>
                      {!visitStarted && !isTerminal(selected) ? <button type="button" disabled={busy === "start" || !selected.assigned_to || !protocol} onClick={() => execute("start")} className="inline-flex items-center gap-1.5 rounded-xl bg-[#2C2925] px-4 py-3 text-[10px] font-medium text-white disabled:cursor-not-allowed disabled:opacity-35"><Play size={10} />{busy === "start" ? "Checking in…" : "Arrived · start visit"}</button> : <span className="inline-flex items-center gap-1.5 rounded-full border border-[#748267]/18 bg-white px-3 py-1.5 text-[8px] font-medium text-[#607057]"><CheckCircle2 size={9} /> On site</span>}
                    </div>
                    {!selected.assigned_to ? <div className="mt-3 text-[8px] text-[#98513D]">A technician must be assigned before arrival can be confirmed.</div> : !protocol ? <div className="mt-3 text-[8px] text-[#98513D]">A snapshotted treatment protocol is required before arrival can start execution.</div> : null}
                  </div>

                  <div className="mt-5">
                    <div className="text-[9px] font-medium uppercase tracking-[0.13em] text-[#8A867F]">Site context</div>
                    <h3 className="mt-1 text-[15px] font-medium text-[#322E29]">Instructions before work</h3>
                  ...
                  <div className="mt-6 border-t border-black/[0.06] pt-5">
                    ...
                    {!visitStarted && !isTerminal(selected) ? <div className="mt-3 rounded-xl border border-black/[0.06] bg-[#F7F6F3] p-3 text-[9px] text-[#8D877F]">Confirm arrival before recording treatment work.</div> : null}
                  ...
                  <div className="mt-6 border-t border-black/[0.06] pt-5">
                    <div className="text-[9px] font-medium uppercase tracking-[0.13em] text-[#8A867F]">4 · Evidence</div><h3 className="mt-1 text-[15px] font-medium text-[#322E29]">Prove the service</h3>
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">{[...evidence, ...externalFields.map((field) => field.label || field.key)].filter(Boolean).map((label) => <div key={label} className="flex items-center gap-2 rounded-xl border border-black/[0.06] bg-[#FBFAF8] px-3 py-3 text-[9px] text-[#625D56]"><FileCheck2 size={10} className="text-[#9A744B]" />{label}</div>)}{!proofRequired ? <div className="flex items-center gap-2 rounded-xl border border-[#748267]/15 bg-[#748267]/[0.04] px-3 py-3 text-[9px] text-[#65705D]"><CheckCircle2 size={10} />No external proof required by this protocol.</div> : null}</div>
                    {proofRequired ? <div className="mt-3 rounded-xl border border-[#C08A4A]/18 bg-[#C08A4A]/[0.04] p-3.5"><div className="text-[9px] font-medium text-[#76583A]">Completion Evidence required</div><div className="mt-1 text-[8px] leading-3 text-[#8D806F]">Capture governed photos, signatures or location against this exact occurrence. The evidence record remains separate from technician-entered treatment data.</div><div className="mt-2.5 flex flex-col gap-2 sm:flex-row"><input disabled={!visitStarted || isTerminal(selected)} value={evidenceId} onChange={(event) => setEvidenceId(event.target.value)} placeholder="Completion Evidence record ID" className="min-w-0 flex-1 rounded-lg border border-black/[0.09] bg-white px-3 py-2 text-[10px] outline-none focus:border-[#D6A66A]/60 disabled:bg-[#F4F2EE]" /><Link href={evidenceHref} className={`inline-flex items-center justify-center rounded-lg border border-black/[0.08] bg-white px-3 py-2 text-[9px] font-medium ${visitStarted ? "text-[#665B4E]" : "pointer-events-none text-[#B2ACA4]"}`}>{evidenceId ? "Review evidence" : "Capture evidence"}</Link></div></div> : null}
                  </div>

                  <div className="mt-6 border-t border-black/[0.06] pt-5">
                    <div className="text-[9px] font-medium uppercase tracking-[0.13em] text-[#8A867F]">5 · Outcome</div><h3 className="mt-1 text-[15px] font-medium text-[#322E29]">Close the loop</h3>
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      <label className="rounded-xl border border-black/[0.06] bg-[#FBFAF8] p-3.5"><span className="text-[9px] font-medium text-[#4A4540]">Service outcome</span><select disabled={!visitStarted || isTerminal(selected)} value={outcome} onChange={(event) => setOutcome(event.target.value)} className="mt-2 w-full rounded-lg border border-black/[0.09] bg-white px-3 py-2.5 text-[11px] disabled:bg-[#F4F2EE]"><option value="completed">Completed as planned</option><option value="follow_up">Follow-up required</option><option value="issue_found">Issue found</option></select></label>
                      <label className={`rounded-xl border p-3.5 ${outcome !== "completed" && !text(followUpNotes) ? "border-[#C08A4A]/20 bg-[#C08A4A]/[0.035]" : "border-black/[0.06] bg-[#FBFAF8]"}`}><span className="text-[9px] font-medium text-[#4A4540]">Technician decision note {outcome !== "completed" ? <span className="text-[#9A744B]">· required</span> : null}</span><textarea disabled={!visitStarted || isTerminal(selected)} value={followUpNotes} onChange={(event) => setFollowUpNotes(event.target.value)} className="mt-2 min-h-20 w-full resize-y rounded-lg border border-black/[0.09] bg-white px-3 py-2 text-[11px] disabled:bg-[#F4F2EE]" placeholder={outcome === "completed" ? "Optional note for service history." : "Describe the issue, risk, or exact next action needed."} /></label>
                    </div>
                  </div>
                </div>
                ...
                <aside className="border-t border-black/[0.06] bg-[#FBFAF8] p-5 lg:border-l lg:border-t-0">
                  <div className="lg:sticky lg:top-5">
                    <div className="flex items-center gap-1.5 text-[9px] font-medium uppercase tracking-[0.13em] text-[#8A867F]"><ClipboardCheck size={10} /> Live completion gate</div>
                    <div className="mt-3 space-y-2">
                      <div className="flex items-center justify-between rounded-xl bg-white px-3 py-3 text-[9px]"><span className="text-[#6D675F]">Arrival / started</span><span className={visitStarted || isTerminal(selected) ? "text-[#607057]" : "text-[#98513D]"}>{visitStarted || isTerminal(selected) ? "Ready" : "Not started"}</span></div>
                      ...
                      <div className="flex items-center justify-between rounded-xl bg-white px-3 py-3 text-[9px]"><span className="text-[#6D675F]">Required treatment fields</span><span className={protocolReady ? "text-[#607057]" : "text-[#98513D]"}>{completedRequired}/{requiredFields.length}</span></div>
                      {monitoringApplicable ? <div className="flex items-center justify-between rounded-xl bg-white px-3 py-3 text-[9px]"><span className="text-[#6D675F]">Monitoring coverage</span><span className={monitoringReady ? "text-[#607057]" : "text-[#98513D]"}>{monitoring.loading ? "Checking…" : monitoring.error ? "Unavailable" : monitoring.round?.completion_ready ? `${monitoring.round.checked_required_points}/${monitoring.round.required_points} ready` : `${monitoring.round?.pending_required_points || 0} pending`}</span></div> : null}
                      ...
                      <div className="flex items-center justify-between rounded-xl bg-white px-3 py-3 text-[9px]"><span className="text-[#6D675F]">Outcome decision</span><span className={outcomeReady ? "text-[#607057]" : "text-[#98513D]"}>{outcomeReady ? (outcome === "completed" ? "Complete" : outcome === "follow_up" ? "Follow-up" : "Issue") : "Note required"}</span></div>
                    </div>
                    <div className="mt-4 rounded-xl border border-black/[0.06] bg-white p-3.5 text-[8px] leading-4 text-[#837C73]">Completion is tied to this exact scheduled occurrence. Avantiqo validates the live protocol, monitoring coverage, proof and outcome before reconciling the work order and creating any required follow-up.</div>
                    {notice ? <div className={`mt-3 rounded-xl border px-3.5 py-3 text-[9px] leading-4 ${notice.toLowerCase().includes("could") || notice.toLowerCase().includes("required") || notice.toLowerCase().includes("cannot") || notice.toLowerCase().includes("incomplete") || notice.toLowerCase().includes("failed") ? "border-[#B36B52]/20 bg-[#B36B52]/[0.05] text-[#8B4937]" : "border-[#748267]/18 bg-[#748267]/[0.05] text-[#607057]"}`}>{notice}</div> : null}
                    ...
                    {!completionReady && !isTerminal(selected) ? <div className="mt-2 text-center text-[8px] leading-3 text-[#9C958D]">The close action unlocks only when every live service requirement above is ready.</div> : null}
                  </div>
                </aside>
              </div>
            </section>
