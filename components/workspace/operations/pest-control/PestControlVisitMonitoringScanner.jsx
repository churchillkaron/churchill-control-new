...
export default function PestControlVisitMonitoringScanner({ organizationId, occurrenceId, initialLookup = "" }) {
  ...
  const activityObserved = normalized(form.activityLevel) !== "none";
  ...
  const resolvePoint = useCallback(async (value) => {
    ...
    try {
      const response = await fetch(`/api/service-management/monitoring-points/resolve?organizationId=${encodeURIComponent(organizationId)}&lookup=${encodeURIComponent(candidate)}`, { cache: "no-store", credentials: "include" });
      ...
      const roundResponse = await fetch(`/api/service-management/monitoring-round?organizationId=${encodeURIComponent(organizationId)}&occurrenceId=${encodeURIComponent(occurrenceId)}`, { cache: "no-store", credentials: "include" });
  ...
  async function record(event) {
    ...
    if (!point?.id) return;
    if (activityObserved && !text(form.pestName)) {
      setError("Identify the pest before saving observed activity.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/service-management/monitoring-round/check", {
        ...
        body: JSON.stringify({ organizationId, occurrenceId, pointId: point.id, ...form, pestName: activityObserved ? form.pestName : "", count: activityObserved ? form.count : "0", clientMutationId: randomId() }),
      });
      const body = await response.json().catch(() => ({}));
