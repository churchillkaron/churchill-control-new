...
function duplicateActionGuard(control, decision) {
  ...
  const currentRevision = nonNegativeInteger(control?.evidence_revision);
  const duplicate = [...normalizedGuardedActionHistory(control?.guarded_actions)]
    .reverse()
    .find((entry) => {
      if (entry.evidence_revision !== currentRevision) return false;
...
- Equivalent completed read, search, or run actions are rejected until a different execution or governed research step changes the evidence revision. A read whose requested line range is already covered by an earlier read of the same file is equivalent evidence and is rejected. Do not loop on duplicate observations.
