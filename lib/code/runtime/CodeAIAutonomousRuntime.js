...
async function planNext({ context, objective, state, iteration }) {
  ...
  const decision = parsePlannerDecision(plannerOutput(result));
...
export async function executeAutonomousCodeMission({
  ...
  for (let iteration = 1; iteration <= maximum; iteration += 1) {
    ...
    if (decision.action === "block") {
      state = {
        ...state,
        status: "blocked",
        blockers: [decision.description || decision.reason || "CODE_AI_AUTONOMOUS_BLOCKED"],
        updated_at: new Date().toISOString(),
      };
      return {
        success: false,
  ...
  state = {
    ...state,
    status: "blocked",
    blockers: ["CODE_AI_AUTONOMOUS_ITERATION_BUDGET_EXHAUSTED"],
    updated_at: new Date().toISOString(),
  };
  return {
    success: false,
    contract: CONTRACT,
    status: "blocked",
    reason: "CODE_AI_AUTONOMOUS_ITERATION_BUDGET_EXHAUSTED",
    state,
    iterations: maximum,
  };
}
