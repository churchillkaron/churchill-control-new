...
function normalizedActionInput(action, input) {
  ...
  if (action === "read") {
    return {
      ...
      start_line: positiveLineNumber(source.start_line),
      end_line: positiveLineNumber(source.end_line),
    };
  }
  if (action === "search") {
    return {
...
function duplicateActionGuard(control, decision) {
  ...
  const duplicate = [...normalizedGuardedActionHistory(control?.guarded_actions)]
    ...
    .find((entry) =>
      ...
      entry.evidence_revision === currentRevision,
    );
...
- Equivalent completed read, search, or run actions are rejected until a different execution or governed research step changes the evidence revision. Do not loop on duplicate observations.
