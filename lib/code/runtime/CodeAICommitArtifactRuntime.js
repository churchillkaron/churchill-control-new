...
export function isCodeAICommitArtifactEligible(state = {}) {
  ...
  return (
    text(source.status, 100) === "completed" &&
    completion.complete === true &&
    completion.verified === true &&
    completion.final_diff_observed === true &&
    blockers.length === 0 &&
    !text(source.current_operation_id, 200) &&
    Array.isArray(source.source_changes) &&
    source.source_changes.length > 0 &&
    (!Array.isArray(source.range_source_changes) || source.range_source_changes.length === 0) &&
...
export async function persistCodeAICommitArtifact({
  ...
  if (!isCodeAICommitArtifactEligible(state)) {
    throw new Error("CODE_AI_COMMIT_ARTIFACT_VERIFIED_COMPLETION_REQUIRED");
  }
