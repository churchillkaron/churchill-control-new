...
export function sealCodeAICompetitiveBenchmarkStoredEvidence(
  evidence,
  { storage_repository_commit = null, persisted_at = null } = {},
) {
  const source = object(evidence);
...
export function verifyCodeAICompetitiveBenchmarkStoredEvidenceIntegrity(metadata) {
  const source = object(metadata);
  const supplied = text(source.history_integrity_sha256, 80).toLowerCase();
  if (!SHA256.test(supplied)) {
    return { valid: false, legacy_unsealed: !supplied, expected_sha256: null, supplied_sha256: supplied || null };
  }
  const core = { ...source };
  delete core.history_integrity_sha256;
  delete core.storage_repository_commit;
  delete core.persisted_at;
  delete core.ordinary_memory_recall;
  const expected = fingerprint(core);
