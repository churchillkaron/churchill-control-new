...
const MEMORY_HEAD_SCOPE = "platform_code_competitive_benchmark_history_head";
const MEMORY_HEAD_KEY = "code_ai_competitive_benchmark_history_head:v1";
const MEMORY_HEAD_SOURCE = "code_ai_competitive_benchmark_history_head_runtime";
...
const HISTORY_HEAD_CONTRACT = "AVANTIQO_CODE_COMPETITIVE_HISTORY_HEAD_V1";
const HISTORY_HEAD_ATTESTATION_CONTRACT = "AVANTIQO_CODE_COMPETITIVE_HISTORY_HEAD_ATTESTATION_V1";
const HISTORY_HEAD_DOMAIN = "AVANTIQO_CODE_COMPETITIVE_HISTORY_HEAD_V1\n";
...
function historyHeadAttestationPayload(value, keyId) {
  const source = object(value);
  const core = { ...source };
  delete core.head_attestation;
  delete core.head_integrity_sha256;
  return `${HISTORY_HEAD_DOMAIN}key_id:${text(keyId, 80)}\n${JSON.stringify(canonical(core))}`;
}

function historyHeadAttestationDigest(value, secret, keyId) {
  return crypto.createHmac("sha256", secret)
    .update(historyHeadAttestationPayload(value, keyId), "utf8")
    .digest("hex");
}

export function sealCodeAICompetitiveBenchmarkHistoryHead(
  sealedEvidence,
  { persisted_at = null, env = process.env } = {},
) {
  const evidence = object(sealedEvidence);
  const memoryKey = text(evidence.history_memory_key, 240);
  const historyDigest = text(evidence.history_attestation?.digest, 80).toLowerCase();
  const historyIntegrity = text(evidence.history_integrity_sha256, 80).toLowerCase();
  const evidencePersistedAt = text(evidence.persisted_at, 120);
  const sourceCommit = text(evidence.storage_repository_commit, 80).toLowerCase();
  if (!memoryKey || !SHA256.test(historyDigest) || !SHA256.test(historyIntegrity) || !GIT_SHA.test(sourceCommit)) {
    throw new Error("CODE_AI_COMPETITIVE_HISTORY_HEAD_EVIDENCE_INVALID");
  }
  if (!Number.isFinite(Date.parse(evidencePersistedAt))) {
    throw new Error("CODE_AI_COMPETITIVE_HISTORY_HEAD_EVIDENCE_TIME_REQUIRED");
  }
  const headPersistedAt = text(persisted_at, 120) || evidencePersistedAt;
  if (!Number.isFinite(Date.parse(headPersistedAt))) {
    throw new Error("CODE_AI_COMPETITIVE_HISTORY_HEAD_TIME_REQUIRED");
  }
  const core = {
    contract: HISTORY_HEAD_CONTRACT,
    latest_memory_key: memoryKey,
    latest_history_attestation_digest: historyDigest,
    latest_history_integrity_sha256: historyIntegrity,
    latest_persisted_at: evidencePersistedAt,
    source_repository_commit: sourceCommit,
    head_persisted_at: headPersistedAt,
    ordinary_memory_recall: false,
  };
  const keyring = historyAttestationKeyring(env);
  const keyId = keyring.active_key_id;
  ...
  return {
    ...core,
    head_integrity_sha256: fingerprint(core),
    head_attestation: {
      contract: HISTORY_HEAD_ATTESTATION_CONTRACT,
      algorithm: HISTORY_ATTESTATION_ALGORITHM,
      key_id: keyId,
      digest: historyHeadAttestationDigest(core, secret, keyId),
      server_only: true,
    },
  };
}

export function verifyCodeAICompetitiveBenchmarkHistoryHead(
  headMetadata,
  { row_updated_at = null, evidence_metadata = null, evidence_memory_key = null, env = process.env } = {},
) {
  const head = object(headMetadata);
  const attestation = object(head.head_attestation);
  const keyId = text(attestation.key_id, 80);
  const keyring = historyAttestationKeyring(env, { required: false });
  const secret = !keyring.invalid && HISTORY_KEY_ID.test(keyId) ? keyring.keys[keyId] || null : null;
  const suppliedDigest = text(attestation.digest, 80).toLowerCase();
  const contractValid =
    text(head.contract, 180) === HISTORY_HEAD_CONTRACT &&
    text(attestation.contract, 180) === HISTORY_HEAD_ATTESTATION_CONTRACT &&
    text(attestation.algorithm, 80) === HISTORY_ATTESTATION_ALGORITHM &&
    attestation.server_only === true &&
    SHA256.test(suppliedDigest) &&
    Boolean(secret);
  let hmacValid = false;
  if (contractValid) {
    const expected = historyHeadAttestationDigest(head, secret, keyId);
    const suppliedBuffer = Buffer.from(suppliedDigest, "hex");
    const expectedBuffer = Buffer.from(expected, "hex");
    hmacValid = suppliedBuffer.length === expectedBuffer.length && crypto.timingSafeEqual(suppliedBuffer, expectedBuffer);
  }
  const fingerprintCore = { ...head };
  delete fingerprintCore.head_attestation;
  delete fingerprintCore.head_integrity_sha256;
  const suppliedFingerprint = text(head.head_integrity_sha256, 80).toLowerCase();
  const fingerprintValid = SHA256.test(suppliedFingerprint) && suppliedFingerprint === fingerprint(fingerprintCore);
  const headPersistedAt = text(head.head_persisted_at, 120);
  const rowUpdatedAt = text(row_updated_at, 120);
  const rowTimestampValid =
    Number.isFinite(Date.parse(headPersistedAt)) &&
    Number.isFinite(Date.parse(rowUpdatedAt)) &&
    Date.parse(headPersistedAt) === Date.parse(rowUpdatedAt);
  const evidence = object(evidence_metadata);
  const evidenceBound =
    text(head.latest_memory_key, 240) === text(evidence_memory_key, 240) &&
    text(head.latest_memory_key, 240) === text(evidence.history_memory_key, 240) &&
    text(head.latest_history_attestation_digest, 80).toLowerCase() === text(evidence.history_attestation?.digest, 80).toLowerCase() &&
    text(head.latest_history_integrity_sha256, 80).toLowerCase() === text(evidence.history_integrity_sha256, 80).toLowerCase() &&
    text(head.latest_persisted_at, 120) === text(evidence.persisted_at, 120) &&
    text(head.source_repository_commit, 80).toLowerCase() === text(evidence.storage_repository_commit, 80).toLowerCase();
  const valid = contractValid && hmacValid && fingerprintValid && rowTimestampValid && evidenceBound;
  const reason = !contractValid
    ? (!secret ? "HISTORY_HEAD_KEY_UNKNOWN" : "HISTORY_HEAD_ATTESTATION_INVALID")
    : !hmacValid || !fingerprintValid
      ? "HISTORY_HEAD_INTEGRITY_INVALID"
      : !rowTimestampValid
        ? "HISTORY_HEAD_ROW_TIMESTAMP_MISMATCH"
        : !evidenceBound
          ? "HISTORY_HEAD_LATEST_ROW_MISMATCH"
          : null;
  return {
    valid,
    key_id: keyId || null,
...
export async function persistCodeAICompetitiveBenchmarkEvidence({
  ...
  const sealedHead = sealCodeAICompetitiveBenchmarkHistoryHead(sealedMetadata, { persisted_at: now, env });
  const persistedHead = await supabaseAdmin
    .from(MEMORY_TABLE)
    .upsert({
      organization_id: organizationId,
      party_id: null,
      ...
      conversation_id: null,
      source_turn_id: null,
      memory_scope: MEMORY_HEAD_SCOPE,
      memory_key: MEMORY_HEAD_KEY,
      memory_type: "fact",
      subject: "Avantiqo Code Competitive Benchmark History Head",
      content: "Tamper-evident pointer to the latest durable competitive Code benchmark evidence.",
      importance: 0.05,
      confidence: 1,
      source: MEMORY_HEAD_SOURCE,
      active: true,
      metadata: sealedHead,
      updated_at: now,
    }, { onConflict: "organization_id,memory_scope,memory_key" })
    .select("id,updated_at")
    .maybeSingle();
  if (persistedHead.error) throw persistedHead.error;
  if (!persistedHead.data?.id) throw new Error("CODE_AI_COMPETITIVE_BENCHMARK_HISTORY_HEAD_PERSIST_FAILED");
  await trimCompetitiveBenchmarkEvidenceHistory(organizationId);
  return {
    persisted: true,
    row_id: persisted.data.id,
    head_row_id: persistedHead.data.id,
...
export async function loadLatestCodeAICompetitiveBenchmarkEvidence() {
  ...
  const recordIntegrity = verifyCodeAICompetitiveBenchmarkStoredEvidenceRecord(metadata, {
    row_updated_at: loaded.data.updated_at,
    row_memory_key: loaded.data.memory_key,
  });
  const loadedHead = await supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id,memory_key,metadata,updated_at")
    .eq("organization_id", organizationId)
    .eq("memory_scope", MEMORY_HEAD_SCOPE)
    .eq("memory_key", MEMORY_HEAD_KEY)
    .eq("active", true)
    .maybeSingle();
  if (loadedHead.error) throw loadedHead.error;
  const headIntegrity = loadedHead.data?.id
    ? verifyCodeAICompetitiveBenchmarkHistoryHead(object(loadedHead.data.metadata), {
        row_updated_at: loadedHead.data.updated_at,
        evidence_metadata: metadata,
        evidence_memory_key: loaded.data.memory_key,
      })
    : { valid: false, reason: "HISTORY_HEAD_REQUIRED" };
  const integrity = {
    ...recordIntegrity,
    valid: recordIntegrity.valid === true && headIntegrity.valid === true,
    history_head_valid: headIntegrity.valid === true,
    history_head_reason: headIntegrity.reason || null,
    reason: recordIntegrity.valid !== true
      ? recordIntegrity.reason
      : headIntegrity.valid !== true
        ? headIntegrity.reason || "HISTORY_HEAD_INVALID"
        : null,
  };
  ...
  return {
    ...
    history_head_row_id: loadedHead.data?.id || null,
    authorization_effect: "NONE",
  };
}
...
export const CodeAICompetitiveBenchmarkEvidenceRuntime = Object.freeze({
  ...
  sealHistoryHead: sealCodeAICompetitiveBenchmarkHistoryHead,
  verifyHistoryHead: verifyCodeAICompetitiveBenchmarkHistoryHead,
  ...
  history_deletion_rollback_resistance_enforced: true,
  provider_routing_authority: false,
  production_deploy_authority: false,
});
