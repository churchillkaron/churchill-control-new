...
const HISTORY_ATTESTATION_CONTRACT = "AVANTIQO_CODE_COMPETITIVE_HISTORY_ATTESTATION_V1";
...
const HISTORY_ATTESTATION_DOMAIN = "AVANTIQO_CODE_COMPETITIVE_HISTORY_EVIDENCE_V1\n";
...
function historyAttestationSecret(env = process.env, { required = true } = {}) {
  const secret = String(env?.[HISTORY_ATTESTATION_SECRET_ENV] ?? "");
  if (Buffer.byteLength(secret, "utf8") >= MIN_HISTORY_ATTESTATION_SECRET_BYTES) return secret;
  if (required) throw new Error("CODE_AI_COMPETITIVE_HISTORY_ATTESTATION_SECRET_REQUIRED");
  return null;
}
function historyAttestationPayload(value) {
  const source = object(value);
  const core = { ...source };
  delete core.history_attestation;
  delete core.history_integrity_sha256;
  return HISTORY_ATTESTATION_DOMAIN + JSON.stringify(canonical(core));
}
function historyAttestationDigest(value, secret) {
  return crypto.createHmac("sha256", secret).update(historyAttestationPayload(value), "utf8").digest("hex");
}
...
export function sealCodeAICompetitiveBenchmarkStoredEvidence(
  evidence,
  { storage_repository_commit = null, persisted_at = null, env = process.env } = {},
) {
  const source = object(evidence);
  ...
  const secret = historyAttestationSecret(env);
  return {
    ...core,
    ...
    history_attestation: {
      ...
      digest: historyAttestationDigest(core, secret),
      server_only: true,
    },
  };
}
...
export function verifyCodeAICompetitiveBenchmarkStoredEvidenceIntegrity(
  ...
  const secret = historyAttestationSecret(env, { required: false });
  if (
    text(attestation.contract, 180) !== HISTORY_ATTESTATION_CONTRACT ||
    ...
    return {
      ...
      reason: !secret ? "ATTESTATION_SECRET_UNAVAILABLE" : "ATTESTATION_REQUIRED",
      supplied_digest: suppliedDigest || null,
    };
  }
  const expectedDigest = historyAttestationDigest(source, secret);
...
export async function loadLatestCodeAICompetitiveBenchmarkEvidence() {
  ...
  const integrity = verifyCodeAICompetitiveBenchmarkStoredEvidenceIntegrity(metadata);
  const safeMetadata = integrity.valid
    ? metadata
    : {
        ...metadata,
