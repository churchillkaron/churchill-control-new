import { certifyCodeAIFrontierLatency } from "./CodeAIFrontierLatencyCertificationRuntime.js";
...
const CASE_ID = /^[a-z0-9][a-z0-9_-]{0,159}$/;
const CATEGORY_ID = /^[a-z0-9][a-z0-9_-]{0,79}$/;
...
export function validateCodeAICompetitiveOwnedReportShape(
  ...
  const generatedAtRaw = String(source.generated_at ?? "");
  const generatedAt = Date.parse(generatedAtRaw);
  if (!RUN_ID.test(text(source.benchmark_run_id, 80))) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_BENCHMARK_RUN_ID_REQUIRED");
  }
  ...
  if (!Number.isFinite(generatedAt)) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_MEASURED_AT_REQUIRED");
  }
  const canonicalGeneratedAt = new Date(generatedAt).toISOString();
  if (generatedAtRaw !== canonicalGeneratedAt) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_GENERATED_AT_CANONICAL_REQUIRED");
  }
  if (generatedAt > Date.now() + MAX_FUTURE_CLOCK_SKEW_MS) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_FUTURE_TIMESTAMP_FORBIDDEN");
  }
  ...
  for (const observation of observations) {
    const rawCaseId = String(observation?.case_id ?? "");
    const rawCategory = String(observation?.category ?? "");
    if (rawCaseId !== text(rawCaseId, 160) || !CASE_ID.test(rawCaseId)) {
      throw new Error("CODE_AI_COMPETITIVE_OWNED_CASE_ID_INVALID");
    }
    if (rawCategory !== text(rawCategory, 80) || !CATEGORY_ID.test(rawCategory)) {
      throw new Error("CODE_AI_COMPETITIVE_OWNED_CATEGORY_INVALID");
    }
    ...
    if (!Array.isArray(observation?.failures)) {
      throw new Error("CODE_AI_COMPETITIVE_OWNED_CASE_FAILURES_REQUIRED");
    }
    const failureCodes = observation.failures.map((value) => text(value, 160)).filter(Boolean);
    if (
      failureCodes.length !== observation.failures.length ||
      failureCodes.length > 20 ||
      new Set(failureCodes).size !== failureCodes.length ||
      failureCodes.some((value) => !/^[A-Za-z0-9_:-]+$/.test(value))
    ) {
      throw new Error("CODE_AI_COMPETITIVE_OWNED_CASE_FAILURES_INVALID");
    }
    if ((observation.passed === true && failureCodes.length !== 0) || (observation.passed === false && failureCodes.length === 0)) {
      throw new Error("CODE_AI_COMPETITIVE_OWNED_CASE_PASS_FAILURE_MISMATCH");
    }
    ...
    if (observation?.passed === true) {
      ...
      if (
        ...
        throw new Error("CODE_AI_COMPETITIVE_OWNED_QUALITY_EVIDENCE_REQUIRED");
      }
    }
    if (observation?.passed === false) {
      if (
        ...
        throw new Error("CODE_AI_COMPETITIVE_OWNED_FAILED_CASE_QUALITY_EVIDENCE_FORBIDDEN");
      }
    }
  ...
  const latencyCertification = object(summary.latency_certification);
  const latencyLimits = object(latencyCertification.limits);
  const warmP50Limit = Number(latencyLimits.warm_p50_ms);
  const warmP95Limit = Number(latencyLimits.warm_p95_ms);
  const coldStartLimit = Number(latencyLimits.cold_start_ms);
  const minimumWarmSamples = Number(latencyLimits.minimum_warm_samples);
  if (
    !Number.isFinite(warmP50Limit) || warmP50Limit <= 0 || warmP50Limit > 2500 ||
    !Number.isFinite(warmP95Limit) || warmP95Limit <= 0 || warmP95Limit > 4000 ||
    !Number.isFinite(coldStartLimit) || coldStartLimit <= 0 || coldStartLimit > 10000 ||
    !Number.isInteger(minimumWarmSamples) || minimumWarmSamples < 10
  ) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_LATENCY_LIMITS_INVALID");
  }
  if (observations.some((item) =>
    typeof item?.code_cpu_fallback !== "boolean" ||
    typeof item?.code_runtime_model_already_gpu_resident !== "boolean"
  )) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_LATENCY_OBSERVATION_PROVENANCE_REQUIRED");
  }
  const recomputedLatencyCertification = certifyCodeAIFrontierLatency(observations, {
    warm_p50_limit_ms: warmP50Limit,
    warm_p95_limit_ms: warmP95Limit,
    cold_start_limit_ms: coldStartLimit,
    minimum_warm_samples: minimumWarmSamples,
  });
  if (JSON.stringify(canonical(latencyCertification)) !== JSON.stringify(canonical(recomputedLatencyCertification))) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_LATENCY_CERTIFICATION_MISMATCH");
  }
  ...
  if (
    ...
    summary.correctness_passed !== correctnessPassed ||
    typeof latencyCertification.passed !== "boolean" ||
    summary.passed !== (correctnessPassed && latencyCertification.passed === true) ||
    summary.complete_suite !== true
  ) {
    throw new Error("CODE_AI_COMPETITIVE_OWNED_SUMMARY_MISMATCH");
  }
