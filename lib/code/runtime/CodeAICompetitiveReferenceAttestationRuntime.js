...
export function validateCodeAICompetitiveReferenceReportShape(
  ...
  if (generatedAtRaw !== canonicalGeneratedAt) {
    throw new Error("CODE_AI_COMPETITIVE_REFERENCE_GENERATED_AT_CANONICAL_REQUIRED");
  }
  if (generatedAt > Date.now() + MAX_FUTURE_CLOCK_SKEW_MS) {
    throw new Error("CODE_AI_COMPETITIVE_REFERENCE_FUTURE_TIMESTAMP_FORBIDDEN");
  }
  ...
  for (const observation of observations) {
    ...
    if (rawCaseId !== text(rawCaseId, 160) || !CASE_ID.test(rawCaseId)) {
      throw new Error("CODE_AI_COMPETITIVE_REFERENCE_CASE_ID_INVALID");
    }
    if (rawCategory !== text(rawCategory, 80) || !CATEGORY_ID.test(rawCategory)) {
      throw new Error("CODE_AI_COMPETITIVE_REFERENCE_CATEGORY_INVALID");
    }
