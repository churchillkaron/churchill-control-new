...
function qualityScore(parsed, requiredEvidence = []) {
  const evidence = object(parsed?.evidence);
  const fields = ["diagnosis", "solution", "verification"];
  const fieldScores = fields.map((field) => {
    const value = text(parsed?.[field], 6000);
    if (value.length < 20) return 0;
    if (value.length >= 220) return 1;
    if (value.length >= 120) return 0.85;
    if (value.length >= 60) return 0.7;
    return 0.55;
  });
  const evidenceScores = requiredEvidence.map((key) => {
    const value = text(evidence?.[key], 3000);
    if (value.length < 12) return 0;
    if (value.length >= 120) return 1;
    if (value.length >= 60) return 0.85;
    if (value.length >= 30) return 0.7;
    return 0.55;
  });
  const all = [...fieldScores, ...evidenceScores];
  if (!all.length) return 0;
  return Number((all.reduce((sum, value) => sum + value, 0) / all.length).toFixed(4));
}
...
export function gradeCodeAICompetitiveReferenceCase(benchmarkCase, rawText) {
  ...
  return {
    ...
    failures,
    quality_score: failures.length === 0 ? qualityScore(parsed, required) : 0,
