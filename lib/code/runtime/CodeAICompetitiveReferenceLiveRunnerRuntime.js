...
function evaluationAnchorSets(benchmarkCase) {
  return list(benchmarkCase?.evaluation_anchors)
    .map((value) => normalizedAnchorTokens(value))
    .filter((tokens) => tokens.length > 0);
}

function scenarioSpecificityScore(benchmarkCase, parsed, requiredEvidence = []) {
  const anchorSets = evaluationAnchorSets(benchmarkCase);
  if (!anchorSets.length) return 1;
  const evidence = object(parsed?.evidence);
  const responseTokens = new Set(normalizedAnchorTokens([
    text(parsed?.diagnosis, 6000),
    text(parsed?.solution, 6000),
    text(parsed?.verification, 6000),
    ...requiredEvidence.map((key) => text(evidence?.[key], 3000)),
  ].join(" ")));
  const matched = anchorSets.filter((tokens) => tokens.every((token) => responseTokens.has(token))).length;
  return Number((matched / anchorSets.length).toFixed(4));
}

function evidenceGroundingScore(benchmarkCase, parsed, requiredEvidence = []) {
  const evidence = object(parsed?.evidence);
  const caseAnchors = normalizedAnchorTokens([
    text(benchmarkCase?.title, 1200),
    text(benchmarkCase?.category, 240),
    ...list(benchmarkCase?.evaluation_anchors),
  ].join(" "));
...
function responseTemplateParts(benchmarkCase, parsed, requiredEvidence = []) {
  const evidence = object(parsed?.evidence);
  const anchorTokens = new Set(normalizedAnchorTokens([
    text(benchmarkCase?.title, 1200),
    text(benchmarkCase?.category, 240),
    ...requiredEvidence,
    ...list(benchmarkCase?.evaluation_anchors),
  ...
  return [
    ...
    text(parsed?.verification, 6000),
    ...requiredEvidence.map((key) => text(evidence?.[key], 3000)),
  ].map((value) => normalizedAnchorTokens(value).filter((token) => !anchorTokens.has(token)));
}

function responseTemplateFingerprint(benchmarkCase, parsed, requiredEvidence = []) {
  const normalized = responseTemplateParts(benchmarkCase, parsed, requiredEvidence)
...
function responseTemplateSimhash64(benchmarkCase, parsed, requiredEvidence = []) {
  const parts = responseTemplateParts(benchmarkCase, parsed, requiredEvidence);
  const features = [];
  for (const tokens of parts) {
    for (let index = 0; index < tokens.length; index += 1) {
      features.push(`u:${tokens[index]}`);
      if (index + 1 < tokens.length) features.push(`b:${tokens[index]}:${tokens[index + 1]}`);
    }
  }
  if (!features.length) return null;
  const weights = Array.from({ length: 64 }, () => 0);
  for (const feature of features) {
    const bits = BigInt(`0x${createHash("sha256").update(feature, "utf8").digest("hex").slice(0, 16)}`);
    for (let bit = 0; bit < 64; bit += 1) {
      weights[bit] += ((bits >> BigInt(bit)) & 1n) === 1n ? 1 : -1;
    }
  }
  let result = 0n;
  for (let bit = 0; bit < 64; bit += 1) {
...
function evidenceDistinctnessScore(parsed, requiredEvidence = []) {
  if (requiredEvidence.length < 2) return 1;
  const evidence = object(parsed?.evidence);
  const tokenSets = requiredEvidence.map((key) => new Set(normalizedAnchorTokens(evidence?.[key])));
  ...
  for (let left = 0; left < tokenSets.length; left += 1) {
    for (let right = left + 1; right < tokenSets.length; right += 1) {
      ...
      maximumSimilarity = Math.max(maximumSimilarity, shared / union.size);
    }
  }
  return Number((1 - maximumSimilarity).toFixed(4));
}
...
function qualityScore(benchmarkCase, parsed, requiredEvidence = []) {
  ...
  const distinctness = evidenceDistinctnessScore(parsed, requiredEvidence);
  const scenarioSpecificity = scenarioSpecificityScore(benchmarkCase, parsed, requiredEvidence);
  const grounding =
    0.3 * evidenceGrounding +
    0.3 * narrativeGrounding +
    0.2 * distinctness +
    0.2 * scenarioSpecificity;
  return Number((depth * (0.5 + 0.5 * grounding)).toFixed(4));
}
...
export function gradeCodeAICompetitiveReferenceCase(benchmarkCase, rawText) {
  ...
  return {
    ...
    evidence_distinctness_score: failures.length === 0 ? evidenceDistinctnessScore(parsed, required) : 0,
    response_template_fingerprint_sha256: failures.length === 0 ? responseTemplateFingerprint(item, parsed, required) : null,
    response_template_simhash64: failures.length === 0 ? responseTemplateSimhash64(item, parsed, required) : null,
...
export async function runCodeAICompetitiveReferenceLiveBenchmark({
  ...
  for (const benchmarkCase of cases) {
    ...
    observations.push({
      ...
      response_template_fingerprint_sha256: text(grade.response_template_fingerprint_sha256, 80) || null,
      response_template_simhash64: text(grade.response_template_simhash64, 32) || null,
  ...
  const report = {
    ...
    economics: {
      estimated_supplier_cost_usd: Number(totalCost.toFixed(8)),
