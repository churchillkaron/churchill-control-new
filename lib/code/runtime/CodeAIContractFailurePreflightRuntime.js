...
const CHECKS = Object.freeze({
  ...
  TS_COMPILER_EXPORTED_TYPE_REMOVED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_CONTRACTS",
    description: "Preserve compiler-resolved cross-file TypeScript public contracts before refactors.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_EXPORTED_TYPE_PROPERTY_REMOVED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_PROPERTIES",
    description: "Preserve compiler-resolved inherited or aliased TypeScript properties across files.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_EXPORTED_TYPE_PROPERTY_OPTIONALITY_CHANGED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_OPTIONALITY",
    description: "Preserve compiler-resolved public TypeScript property optionality across files.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_PARAMETER_PROPERTY_REMOVED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_INPUTS",
    description: "Preserve compiler-resolved cross-file input object properties.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_PARAMETER_OPTIONAL_PROPERTY_TIGHTENED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_INPUT_OPTIONALITY",
    description: "Do not tighten compiler-resolved optional cross-file input properties.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_RETURN_PROPERTY_REMOVED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_RETURNS",
    description: "Preserve compiler-resolved cross-file return object properties.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_RETURN_REQUIRED_PROPERTY_WEAKENED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_RETURN_GUARANTEES",
    description: "Preserve compiler-resolved required return guarantees across files.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_EXPORTED_TYPE_GENERIC_ARITY_CHANGED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_GENERIC_ARITY",
    description: "Preserve compiler-resolved exported generic arity across dependent files.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  TS_COMPILER_FUNCTION_GENERIC_ARITY_CHANGED: {
    id: "PRESERVE_COMPILER_RESOLVED_TYPESCRIPT_FUNCTION_GENERICS",
    description: "Preserve compiler-resolved exported function generic arity across dependent call sites.",
    evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 1,
  },
  ...
  TS_EXPORTED_TYPE_REMOVED: { id: "PRESERVE_TYPESCRIPT_EXPORTED_TYPES", description: "Preserve exported TypeScript object contracts before public type edits.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
  TS_EXPORTED_TYPE_PROPERTY_REMOVED: { id: "PRESERVE_TYPESCRIPT_EXPORTED_PROPERTIES", description: "Preserve exported TypeScript object properties before type edits.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
  TS_EXPORTED_TYPE_PROPERTY_OPTIONALITY_CHANGED: { id: "PRESERVE_TYPESCRIPT_PROPERTY_OPTIONALITY", description: "Preserve required-versus-optional semantics on exported TypeScript properties.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
  TS_PARAMETER_PROPERTY_REMOVED: { id: "PRESERVE_TYPESCRIPT_PARAMETER_PROPERTIES", description: "Preserve accepted TypeScript object-parameter properties before function edits.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
  TS_PARAMETER_OPTIONAL_PROPERTY_TIGHTENED: { id: "PRESERVE_TYPESCRIPT_OPTIONAL_INPUTS", description: "Do not tighten existing optional TypeScript input properties without caller migration proof.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
  TS_RETURN_PROPERTY_REMOVED: { id: "PRESERVE_TYPESCRIPT_RETURN_PROPERTIES", description: "Preserve typed return properties promised by exported functions.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
  TS_RETURN_REQUIRED_PROPERTY_WEAKENED: { id: "PRESERVE_TYPESCRIPT_REQUIRED_RETURNS", description: "Preserve required guarantees in typed return object contracts.", evidence: (graph) => Number(graph.ast_parse_success_count || 0) > 0 },
