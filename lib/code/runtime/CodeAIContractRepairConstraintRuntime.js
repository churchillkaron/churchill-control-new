...
function constraintForViolation(violation = {}) {
  ...
  const property = text(source.property, 240) || null;
  ...
  else if (kind === "TS_EXPORTED_TYPE_REMOVED") requirement = `Preserve exported TypeScript type ${symbol}.`;
  else if (kind === "TS_EXPORTED_TYPE_PROPERTY_REMOVED") requirement = `Preserve exported TypeScript property ${property} on ${symbol}.`;
  else if (kind === "TS_EXPORTED_TYPE_PROPERTY_OPTIONALITY_CHANGED") requirement = `Preserve optional/required compatibility for ${symbol}.${property}.`;
  else if (kind === "TS_PARAMETER_PROPERTY_REMOVED") requirement = `Preserve accepted parameter property ${property} for ${symbol}.`;
  else if (kind === "TS_PARAMETER_OPTIONAL_PROPERTY_TIGHTENED") requirement = `Keep parameter property ${symbol}.${property} optional for existing callers.`;
  else if (kind === "TS_RETURN_PROPERTY_REMOVED") requirement = `Preserve typed return property ${property} from ${symbol}.`;
  else if (kind === "TS_RETURN_REQUIRED_PROPERTY_WEAKENED") requirement = `Keep typed return property ${symbol}.${property} required.`;
  else if (kind === "SOURCE_SELF_IMPORT_INTRODUCED") requirement = "Do not import the file from itself. Preserve and repair the existing implementation in place.";
  else if (kind === "OBSERVED_PUBLIC_SURFACE_ERASED") requirement = `Preserve the observed exported API surface: ${list(source.removed_symbols).map((item) => text(item, 240)).filter(Boolean).join(", ") || "existing exports"}.`;
  else if (kind === "REPAIR_EXPORTED_SYMBOL_REMOVED") requirement = `Preserve exported symbol ${symbol} during this repair unless repository evidence proves a coordinated migration.`;
