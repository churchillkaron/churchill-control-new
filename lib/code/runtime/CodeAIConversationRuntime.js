...
function fastCodeIntent(message, recentConversation = []) {
  ...
  if (hasAny(current, repoActions) && (hasAny(current, repoSubjects) || /^\s*(fix|implement|change|update|debug|test|inspect|continue|build)\b/i.test(message))) {
    return { intent: "repository_work", confidence: 0.93, reason: "Requests executable repository work" };
  }
  return null;
}
