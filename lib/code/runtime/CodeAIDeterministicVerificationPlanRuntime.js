...
} from "./CodeAIDependencyAwareVerifierSelectionRuntime.js";
import {
  codeAIVerifierKey,
...
function verificationSignature(operation = {}) {
  return codeAIVerifierKey(operation?.input) || "";
}
...
function candidate({ id, description, command, args, env = null, family, obligation = null }) {
  return {
    id,
    action: "verify",
    description,
...
function isolatedNextBuildCandidate(paths, state, risk) {
  if (!["high", "critical"].includes(risk)) return [];
  const guidance = text(state?.repository_guidance?.verification_commands_text, 8000).toLowerCase();
  const nextEvidence = /(?:next\s+build|npm\s+run\s+build)/.test(guidance);
  const nextSurfaceChanged = paths.some((filePath) =>
    /^(?:app|components|pages|src\/app|src\/pages|lib)\//.test(filePath) || /^next\.config\./.test(filePath));
  if (!nextEvidence || !nextSurfaceChanged) return [];
  return [candidate({
    id: "deterministic_independent_next_build_1",
    description: "Controller-owned isolated Next.js production build. Uses a dedicated dist directory so verification cannot race the live dev server.",
    command: "npm",
    args: ["run", "build"],
  ...
  return list(repairClosure?.unresolved_failed_verifiers)
    ...
    .map((item, index) => {
      ...
      return candidate({
        ...
        description:
          "Controller-owned replay of an unresolved failed verifier using the exact original command signature and bounded verifier environment. This debt must pass before completion.",
        command,
        args,
...
function dependencyAwareCandidates(selection) {
  return list(selection?.candidates)
    .filter((item) => item?.executable === true && item?.verifier?.command)
    .slice(0, 6)
    .map((item, index) => candidate({
      id: `deterministic_dependency_aware_test_${index + 1}`,
      description: `Controller-owned dependency-aware verifier for ${text(item.path, 1200)}. ${text(item.reason, 1200)}`,
      command: text(item.verifier.command, 300),
      args: list(item.verifier.args).map((arg) => text(arg, 1200)),
      family: text(item.family, 120) || "tests",
      obligation: "DEPENDENCY_AWARE_BEHAVIORAL_GATE",
    }));
}
...
export function planCodeAIDeterministicVerificationGates({
  ...
  const dependencyAwareSelection = selectCodeAIDependencyAwareVerifiers({ state });
  ...
  const proposed = [
    ...dependencyAwareCandidates(dependencyAwareSelection),
    ...isolatedNextBuildCandidate(extantPaths, state, risk),
  ...
  return {
    ...
    observed_impacted_test_paths: unique([
      ...list(dependencyAwareSelection.selected_test_paths),
      ...list(behavioralVerification.observed_impacted_test_paths),
    ]).slice(0, 30),
    dependency_aware_verifier_selection: dependencyAwareSelection,
    ...
    planned_dependency_aware_gate_count:
      operations.filter((operation) => operation.deterministic_obligation === "DEPENDENCY_AWARE_BEHAVIORAL_GATE").length,
