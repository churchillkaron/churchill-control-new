...
const MAX_STRATEGIC_SEARCHES = 6;
...
export function resolveCodeAIEmployeeFastStartSeedPaths({
  ...
  const contractDiscovery = object(objective_context).contract_aware_discovery;
  return [...new Set([
    ...list(contractDiscovery?.priority_paths).map(safeRepositoryFilePath).filter(Boolean),
...
export function resolveCodeAIEmployeeFastStartStrategicSearchTerms({
  ...
  const contractDiscovery = object(objective_context).contract_aware_discovery;
  return [...new Set([
    ...list(contractDiscovery?.strategic_search_terms).map((item) => text(item, 160)).filter(Boolean),
...
function appendFastStartEvidence(state, evidence) {
  ...
  return {
    ...
    employee_fast_start: {
      ...
      contract_aware_discovery_active: object(evidence.contract_aware_discovery).active === true,
      contract_aware_discovery: object(evidence.contract_aware_discovery),
...
function workerWarmingResult({ state, worker, elapsedMs, seedPaths, strategicSearchTerms, warmIdleMs }) {
  const warmingState = appendFastStartEvidence({
    ...
    contract_aware_discovery: object(objective_context).contract_aware_discovery,
...
export async function executeCodeAIEmployeeFastStartMission({
  ...
  const boundObjectiveContext = {
    ...object(resume_state?.objective_context),
  ...
  const preparationPromise = (async () => {
    ...
    const prepared = await executeCodeAIMission({
      ...
      resume_state: null,
      mission_id: objective_context?.mission_id || null,
      timeout_ms,
      workspace_target: objective_context?.workspace_target || null,
      organization_id: context?.organizationId || objective_context?.organization_id || null,
      device_id: objective_context?.device_id || null,
      device_session_id: objective_context?.device_session_id || null,
    });
