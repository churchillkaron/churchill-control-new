...
} from "./CodeAIRuntimeEvidenceCoverageRuntime.js";
import {
  collectCodeAIRuntimeEvidence,
  CODE_AI_RUNTIME_EVIDENCE_COLLECTOR_CONTRACT,
} from "./CodeAIRuntimeEvidenceCollectorRuntime.js";
import {
...
} from "./CodeAIStrategicReviewGateRuntime.js";
...
function explicitReadOnlyMission(state = {}) {
  const source = object(state);
  return isCodeAIExplicitReadOnlyIntent(
    source.employee_mission?.owner_intent,
    source.objective_context?.owner_objective,
    source.objective,
  );
}

function readOnlyEvidenceVerified(state = {}) {
  const source = object(state);
  return list(source.evidence).some((entry) =>
    text(entry?.kind, 120) === "operation" &&
    text(entry?.status, 80) === "completed" &&
    /^(search|read|inspect|diff)$/.test(text(entry?.action, 80))
  );
}

export function resolveCodeAIEmployeeReasoningBudget({
  reasoning_call_budget = null,
  local_device_mission = false,
  objective = "",
  owner_intent = "",
  objective_context = null,
  resume_state = null,
} = {}) {
  const resolved = resolveCodeAIReasoningCallBudget(reasoning_call_budget, {
    max_budget: local_device_mission ? MAX_CODE_AI_LOCAL_REASONING_CALL_BUDGET : undefined,
  });
  const readOnly = isCodeAIExplicitReadOnlyIntent(
    objective,
    owner_intent,
    objective_context?.owner_objective,
    resume_state?.employee_mission?.owner_intent,
    resume_state?.objective_context?.owner_objective,
    resume_state?.objective,
  );
  return readOnly ? Math.min(2, resolved) : resolved;
}
...
export function assessCodeAIEmployeeCompletion(state = {}) {
  ...
  const readOnlyMission = explicitReadOnlyMission(source);
  const readOnlyVerified = readOnlyMission && !changed && readOnlyEvidenceVerified(source);
  const verified = successfulVerification(source) || readOnlyVerified;
  ...
  const runtimeEvidenceCoverage = assessCodeAIRuntimeEvidenceCoverage({
    state: source,
    quality: worldClass,
    behavioral_verification: behavioralVerification,
  });
  ...
  const strategicReview = assessCodeAIStrategicReviewGate({
    ...source,
    worldclass_quality: worldClass,
  });

  const blockers = [];
  if (!changed && !readOnlyMission) blockers.push("CODE_AI_EMPLOYEE_IMPLEMENTATION_REQUIRED");
  if (readOnlyMission && !readOnlyVerified) blockers.push("CODE_AI_EMPLOYEE_READ_ONLY_EVIDENCE_REQUIRED");
  ...
  if (changed && runtimeEvidenceCoverage.required && runtimeEvidenceCoverage.verified !== true) {
    blockers.push(
      `CODE_AI_EMPLOYEE_RUNTIME_EVIDENCE_REQUIRED:${list(runtimeEvidenceCoverage.missing_evidence).join(",") || "UNSPECIFIED"}`,
    );
  }
  ...
  if (!readOnlyMission && strategicReview.required && strategicReview.verified !== true) {
    blockers.push(...list(strategicReview.blockers));
  }
  ...
  return {
    ...
    changed,
    read_only_mission: readOnlyMission,
    read_only_evidence_verified: readOnlyVerified,
    verified,
    ...
    runtime_evidence_coverage: runtimeEvidenceCoverage,
...
function normalizeEmployeeState(state, ownerIntent, passNumber, completion = null) {
  ...
  const runtimeEvidenceCoverage =
    completion?.runtime_evidence_coverage || source.runtime_evidence_coverage || null;
  ...
  const strategicReview =
    completion?.strategic_review || source.strategic_review || null;
  return {
    ...source,
    ...
    employee_mission: {
      ...
      runtime_evidence_required_for_high_risk_ui_api: true,
      failed_verifier_closure_required: true,
      product_completion_criteria_required: true,
      strategic_review_required_when_risk_or_ambiguity_demands: true,
    ...
    evidence: [
      ...list(state?.evidence),
      {
        ...
        runtime_evidence_required:
          assessed.runtime_evidence_coverage?.required === true,
        runtime_evidence_verified:
          assessed.runtime_evidence_coverage?.verified === true,
        runtime_evidence_missing:
          list(assessed.runtime_evidence_coverage?.missing_evidence),
        ...
        strategic_review_required: assessed.strategic_review?.required === true,
        strategic_review_verified: assessed.strategic_review?.verified === true,
...
async function collectDeterministicRuntimeEvidence(state, completion) {
  const source = object(state);
  const assessed = object(completion);
  const coverage = object(assessed.runtime_evidence_coverage);
  if (!coverage.required || coverage.verified === true) {
    return { state: source, completion: assessed, collection: null };
  }
  try {
    const collection = await collectCodeAIRuntimeEvidence({
      state: source,
      quality: assessed.worldclass_quality,
      behavioral_verification: assessed.behavioral_verification,
    });
    if (!list(collection.evidence).length) {
      return { state: source, completion: assessed, collection };
    }
    const nextState = {
      ...source,
      runtime_evidence: [
        ...list(source.runtime_evidence),
        ...list(collection.evidence),
      ].slice(-60),
      evidence: [
        ...list(source.evidence),
        {
          at: new Date().toISOString(),
          kind: "runtime_evidence_collection",
          contract: CODE_AI_RUNTIME_EVIDENCE_COLLECTOR_CONTRACT,
          collected: true,
          passed_probe_count: Number(collection.passed_probe_count || 0),
          failed_probe_count: Number(collection.failed_probe_count || 0),
          test_paths: list(collection.evidence).map((item) => text(item?.test_path, 1000)).filter(Boolean),
          provider_execution_submitted: false,
    ...
    return {
      state: nextState,
      completion: assessCodeAIEmployeeCompletion(nextState),
      collection,
    };
  } catch (error) {
    return {
      state: {
        ...
        evidence: [
          ...list(source.evidence),
          {
            ...
            contract: CODE_AI_RUNTIME_EVIDENCE_COLLECTOR_CONTRACT,
            collected: false,
            reason: sanitizeCodeAIErrorReason(error, {
...
function employeeResult(result, state, passNumber) {
  ...
  return {
    ...
    runtime_evidence_coverage: completion.runtime_evidence_coverage,
    repair_closure: completion.repair_closure,
    product_completion_criteria: completion.product_completion_criteria,
    strategic_review: completion.strategic_review,
...
function employeePassObjective(goal, completion, objectiveContext) {
  ...
  if (completion && completion.complete !== true) {
    ...
    const runtimeEvidenceCoverage = object(completion.runtime_evidence_coverage);
    if (runtimeEvidenceCoverage.required && runtimeEvidenceCoverage.verified !== true) {
      const missing = list(runtimeEvidenceCoverage.missing_evidence)
        .map((item) => text(item, 300))
        .filter(Boolean);
      parts.push(
        "Runtime behavior evidence is still required for this high/critical UI or API change. Unit tests alone are not enough.",
        runtimeEvidenceCoverage.changed_ui_paths?.length
          ? "For UI changes, produce browser/interactive-preview/E2E evidence after the final edit, including console/hydration failures when relevant."
          : null,
        runtimeEvidenceCoverage.changed_api_paths?.length
          ? "For API changes, run the observed impacted integration test or produce request/response/replay/trace evidence against the changed route after the final edit."
          : null,
        ...(missing.length ? [`Missing runtime evidence: ${missing.join(", ")}.`] : []),
      );
    }
    ...
    if (repairClosure.required && repairClosure.verified !== true) {
      ...
      parts.push(
        ...
        ...unresolved.map((item) =>
          `- ${codeAIVerifierDisplay(item)}`
        ),
      );
    }

    const strategicReview = object(completion.strategic_review);
    if (strategicReview.required && strategicReview.verified !== true) {
      parts.push(
        "Independent strategic review is mandatory for this mission before completion. Require both the architecture/performance and adversarial-risk reviewers to complete successfully; a partial or unavailable council is not sufficient evidence.",
      );
    }
...
export async function executeCodeAIEmployeeMission({
  ...
  const localDeviceMission =
    text(objective_context?.workspace_target || resume_state?.objective_context?.workspace_target, 80).toUpperCase() === "DEVICE";
  const budget = resolveCodeAIEmployeeReasoningBudget({
    reasoning_call_budget,
    local_device_mission: localDeviceMission,
    objective: goal,
  ...
  const interactiveYieldAfterPass = interactivePreview?.authorized === true;
  ...
  for (let pass = 1; pass <= maximumPasses; pass += 1) {
    ...
    if (text(lastResult?.status, 100) === "planner_pending") {
      // A pending local planner execution is a continuation boundary owned by the
      // background mission runner. Yield immediately with the exact pending
      // descriptor instead of multiplying the planner poll window across employee
      // passes inside one request.
      return employeeResult({
        ...object(lastResult),
        continuation_yield: true,
      }, state, pass);
    }

    let completion = assessCodeAIEmployeeCompletion(state);
    const runtimeCollection = await collectDeterministicRuntimeEvidence(state, completion);
    state = runtimeCollection.state;
    completion = runtimeCollection.completion;
    ...
    if (interactiveYieldAfterPass) {
      return employeeResult({
        ...object(lastResult),
        success: false,
        status: "running",
        reason: null,
        interactive_yield: true,
      }, state, pass);
    }
  }
