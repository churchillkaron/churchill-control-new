...
} from "./CodeAIZeroIdlePolicyRuntime.js";
import {
  projectCodeAICustomerArtifact,
} from "./CodeAICustomerArtifactRuntime.js";
...
function appendZeroIdleEvidence(state, {
  ...
  const evidence = {
    ...
    contract_aware_discovery: object(contractAwareDiscovery),
    ...
    repository_head_reconciliation_contract:
      CODE_AI_REPOSITORY_HEAD_RECONCILIATION_CONTRACT,
    repository_head_reconciliation_status: headReconciliation?.status || null,
  ...
  return {
    ...
    employee_fast_start: {
      ...
      seed_path_count: seedPaths.length,
      contract_aware_discovery_active: object(contractAwareDiscovery).active === true,
      contract_aware_discovery: object(contractAwareDiscovery),
      deterministic_start_elapsed_ms: elapsedMs,
      repository_head_reconciliation_contract:
        CODE_AI_REPOSITORY_HEAD_RECONCILIATION_CONTRACT,
...
export async function executeCodeAIEmployeeZeroIdleFastStartMission({
  ...
  let seededState = appendZeroIdleEvidence(seededStateRaw, {
    ...
    ownerIntent,
    headReconciliation,
    contractAwareDiscovery: object(objective_context).contract_aware_discovery,
  });
  ...
  const customerArtifact = projectCodeAICustomerArtifact(result, {
    repository_url,
  });

  return {
    ...object(result),
    customer_artifact: customerArtifact,
