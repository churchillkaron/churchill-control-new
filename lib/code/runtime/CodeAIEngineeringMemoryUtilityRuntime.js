...
const MUTATION_ACTIONS = new Set(["apply_files", "replace_range", "delete_files", "rename_files"]);
...
async function trimEngineeringMemoryUtilityObservations({ orgId, actor }) {
...
export async function recordCodeAIEngineeringMemoryUtility({
  ...
  const writtenCount = observations.filter((item) => item.written).length;
  const trimmedCount = writtenCount > 0
    ? await trimEngineeringMemoryUtilityObservations({ orgId, actor })
    : 0;
