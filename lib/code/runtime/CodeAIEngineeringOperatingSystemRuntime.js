...
function precisionAdversarialProof(state = {}, value = {}, expectedContract = "") {
  const source = object(value);
  if (
    text(source.contract, 180) !== expectedContract ||
    source.passed !== true ||
    source.verified !== true
  ) {
    return false;
  }
  const ids = list(source.evidence_operation_ids)
    .map((id) => text(id, 240))
    .filter(Boolean);
  ...
  return ids.every((id) => operationSucceeded(operations.get(id)));
}
function performanceMeasurementProof(value = {}) {
  const source = object(value);
  const before = Number(source.before);
  const after = Number(source.after);
  const minimum = Number(source.minimum_improvement_percent);
  const measured = Number(source.measured_improvement_percent);
  const evidenceOperationIds = list(source.evidence_operation_ids).map((id) => text(id, 240));
  return source.contract === "AVANTIQO_CODE_PERFORMANCE_EVIDENCE_V1" &&
    source.passed === true &&
    Boolean(text(source.benchmark_key, 240)) &&
    Boolean(text(source.metric, 240)) &&
    ["lower_is_better", "higher_is_better"].includes(text(source.direction, 80).toLowerCase()) &&
    Number.isFinite(before) &&
    Number.isFinite(after) &&
    Number.isFinite(minimum) &&
    Number.isFinite(measured) &&
    measured >= minimum &&
    Boolean(text(source.before_operation_id, 240)) &&
    Boolean(text(source.after_operation_id, 240)) &&
    source.before_operation_id !== source.after_operation_id &&
    evidenceOperationIds.includes(text(source.before_operation_id, 240)) &&
    evidenceOperationIds.includes(text(source.after_operation_id, 240));
}
function independentFinalReviewProof(state = {}) {
  const gate = object(state?.final_independent_review_gate);
  return gate.contract === "AVANTIQO_CODE_AI_FINAL_INDEPENDENT_REVIEW_V1" &&
    gate.verified === true &&
    gate.fingerprint_matches === true &&
    !text(gate.blocker, 240) &&
    Number(gate.required_approvals || 0) >= 2 &&
    Number(gate.observed_approvals || 0) >= Number(gate.required_approvals || 0) &&
    Number(gate.blocking_finding_count || 0) === 0;
}
function architectureGraphProof(state = {}) {
  const changed = changedPaths(state);
  if (!changed.length) return true;
  const graph = deriveCodeAICausalGraph(state);
  if (graph?.contract !== "AVANTIQO_CODE_AI_CAUSAL_GRAPH_V1" || Number(graph?.node_count || 0) < 1) {
    return false;
  }
  const nodes = new Map(
    list(graph.nodes)
      .map((node) => [text(node?.id, 1200), object(node)])
      .filter(([id]) => Boolean(id))
  );
  return changed.every((filePath) => {
    const node = nodes.get(text(filePath, 1200));
    return node?.kind === "changed" && node?.source_content_observed === true;
  });
}
function durableRuntimeProof(state = {}) {
  const source = object(state);
  const baseCommit = text(source.base_commit, 160);
  return source.contract === "AVANTIQO_CODE_AI_MISSION_V1" &&
    Boolean(text(source.mission_id, 240)) &&
    Boolean(text(source.repository_url, 1000)) &&
    Boolean(text(source.ref, 160)) &&
    /^[a-f0-9]{40,64}$/i.test(baseCommit) &&
    Array.isArray(source.completed_operation_ids) &&
    Boolean(text(source.created_at, 120)) &&
    Boolean(text(source.updated_at, 120));
}
function hypothesisDebuggingProof(state = {}) {
  const debugging = object(state?.hypothesis_debugging);
  const hypotheses = list(debugging.hypotheses);
  if (debugging.contract !== "AVANTIQO_CODE_HYPOTHESIS_DEBUGGING_V1" || hypotheses.length < 3) {
    return false;
  }
  ...
  for (const item of hypotheses) {
    const hypothesis = object(item);
    const status = text(hypothesis.status, 80).toUpperCase();
    if (!["SUPPORTED", "ELIMINATED"].includes(status)) return false;
    if (status === "SUPPORTED") supported += 1;
    const ids = list(hypothesis.evidence_operation_ids)
      .map((id) => text(id, 240))
      .filter(Boolean);
    if (!ids.length || !ids.every((id) => observedOperationIds.has(id))) return false;
  }
  return supported >= 1;
}
function multiRepositoryProof(value = {}) {
  const coordination = object(value);
  const repositories = list(coordination.repositories);
  const order = list(coordination.dependency_order).map((id) => text(id, 500));
  if (
    coordination.contract !== "AVANTIQO_CODE_AI_MULTI_REPOSITORY_MISSION_V1" ||
    coordination.all_verified !== true ||
    coordination.independent_heads_required !== true ||
    coordination.independent_verification_required !== true ||
    coordination.commit_authority !== false ||
    coordination.merge_authority !== false ||
    coordination.deploy_authority !== false ||
    repositories.length < 2 ||
    order.length !== repositories.length
  ) {
    return false;
  }
  const ids = repositories.map((repo) => text(repo?.id, 500)).filter(Boolean);
  if (new Set(ids).size !== repositories.length || new Set(order).size !== repositories.length) return false;
  if (!ids.every((id) => order.includes(id))) return false;
  return repositories.every((repo) =>
    Boolean(text(repo?.repository_url, 1000)) &&
    /^[a-f0-9]{40,64}$/i.test(text(repo?.base_commit, 160)) &&
    repo?.verified === true
  );
}
function productProgramManagerProof(state = {}) {
  const portfolio = object(state?.product_portfolio);
  const roadmap = list(portfolio.roadmap);
  const policy = object(portfolio.executor_policy);
  if (
    portfolio.contract !== "AVANTIQO_PRODUCT_ENGINEERING_PORTFOLIO_V1" ||
    !Boolean(text(portfolio.portfolio_id, 240)) ||
    !Boolean(text(portfolio.business_goal, 4000)) ||
    !Boolean(text(portfolio.repository_url, 1000)) ||
    text(portfolio.ref, 80) !== "main" ||
    !/^[a-f0-9]{40,64}$/i.test(text(portfolio.current_main_head, 160)) ||
    roadmap.length < 1 ||
    roadmap.length > 4
  ) {
    return false;
  ...
  if (
    policy.main_only !== true ||
    policy.maximum_active_engineering_cycles !== 1 ||
    policy.parallel_code_execution_allowed !== false ||
    policy.branch_or_worktree_fanout_allowed !== false ||
    policy.automatic_commit_allowed !== false ||
    policy.automatic_deploy_allowed !== false ||
    policy.automatic_migration_execution_allowed !== false
  ) {
    return false;
  }
  return roadmap.every((node) =>
    Boolean(text(node?.node_id, 240)) &&
    Boolean(text(node?.objective, 4000)) &&
    Array.isArray(node?.dependencies) &&
    Array.isArray(node?.evidence_paths) &&
    node?.execution_serialized_by_main_only === true
  );
}
function hiddenBenchmarkProof(state = {}) {
  const hidden = object(state?.hidden_benchmark_certification);
  const hiddenVerified = hidden.contract === "AVANTIQO_CODE_AI_HIDDEN_BENCHMARK_V2" &&
    hidden.held_out === true &&
    hidden.verified === true &&
    Number(hidden.case_count || 0) > 0 &&
    Number(hidden.pass_rate || 0) >= 0.9 &&
    hidden.independent_verification_required === true &&
    hidden.candidate_self_report_authority === false;
  if (hiddenVerified) return true;

  const competitive = object(state?.competitive_benchmark);
  return competitive.contract === "AVANTIQO_CODE_COMPETITIVE_BENCHMARK_EVIDENCE_V1" &&
    competitive.attested === true &&
    competitive.competitive_certified === true &&
    competitive.evidence_current === true &&
    competitive.superiority_claim_allowed === true;
}
...
function successfulPostMutationOperationIds(state = {}, applyIndex = -1) {
  if (applyIndex < 0) return new Set();
  return new Set(
    evidence(state)
      .filter((entry, index) =>
        index > applyIndex &&
...
function postMutationRecordProof(state = {}, record = {}, applyIndex = -1) {
  if (applyIndex < 0) return true;
  const postIds = successfulPostMutationOperationIds(state, applyIndex);
  if (!postIds.size) return false;
  return list(record?.evidence_operation_ids).some((id) =>
    postIds.has(text(id, 240))
  );
}
function browserVerificationProof(state = {}, applyIndex = -1) {
  if (applyIndex < 0) {
    return hasVerificationFamily(state, "browser") ||
  ...
  const postBrowserOperationIds = new Set(
    evidence(state)
      .filter((entry, index) =>
        index > applyIndex &&
        entry?.kind === "operation" &&
  ...
  if (
    verification(state).some((item) =>
      item?.passed === true &&
      lower(item?.family) === "browser" &&
      postBrowserOperationIds.has(text(item?.operation_id, 240))
    )
  ) return true;
  return postBrowserOperationIds.size > 0;
}
function postMutationObservabilityProof(state = {}, applyIndex = -1) {
  ...
  return list(state?.observability_evidence?.records).some((record) =>
    postMutationRecordProof(state, record, applyIndex)
  );
}
...
function readinessSignals(state, classification, routing = {}) {
  ...
  const memoryObserved = Boolean(
    state?.verified_engineering_memory?.contract === "AVANTIQO_CODE_AI_VERIFIED_ENGINEERING_MEMORY_V1" &&
    state?.verified_engineering_memory?.evaluated === true &&
    state?.verified_engineering_memory?.attestation_required === true &&
    state?.verified_engineering_memory?.verified_completion_required === true &&
    state?.verified_engineering_memory?.current_head_revalidation_required === true &&
    state?.verified_engineering_memory?.patch_replay_allowed === false &&
    state?.verified_engineering_memory?.automatic_knowledge_promotion === false &&
    state?.verified_engineering_memory?.authorization_effect === "NONE"
  );
  const selfImprovementObserved = Boolean(
    state?.formed_engineering_skills?.contract === "AVANTIQO_CODE_AI_ENGINEERING_SKILL_V1" &&
    state?.formed_engineering_skills?.evaluated === true &&
    state?.formed_engineering_skills?.lifecycle_evaluated === true &&
    state?.formed_engineering_skills?.current_head_revalidation_required === true &&
    state?.formed_engineering_skills?.patch_replay_allowed === false &&
    state?.formed_engineering_skills?.persisted_as_trusted_rule === false &&
    state?.formed_engineering_skills?.automatic_knowledge_promotion === false &&
    state?.formed_engineering_skills?.authorization_effect === "NONE"
  );
  ...
  const specialistRequested = Number(state?.parallel_specialist_review?.reviewer_count_requested || 0);
  const specialistSucceeded = Number(state?.parallel_specialist_review?.reviewer_count_succeeded || 0);
  const specialistReview = Boolean(
    state?.parallel_specialist_review?.completed === true &&
    specialistRequested >= 2 &&
    specialistSucceeded === specialistRequested &&
    state?.parallel_specialist_review?.architecture_performance_review_present === true &&
    state?.parallel_specialist_review?.adversarial_risk_review_present === true
  );
  const independentReview = independentFinalReviewProof(state);
  return {
    architecture_brain: architectureGraphProof(state),
    multi_agent_team: !classification.high_risk && !classification.broad_program
      ? true
    ...
    hypothesis_debugging: !classification.defect || hypothesisDebuggingProof(state),
    durable_runtime: durableRuntimeProof(state),
    browser_verification: !classification.ui || browserVerificationProof(state, applyIndex),
    database_department: !classification.database || Boolean(
      state?.database_review?.passed === true &&
      (applyIndex >= 0
        ? postMutationRecordProof(state, state?.database_review, applyIndex)
        : hasSuccessfulVerification(state))
    ),
    scm_multi_repo: !classification.multi_repo || multiRepositoryProof(state?.multi_repository_coordination),
    adversarial_testing: !(classification.high_risk || classification.defect) || Boolean(
      ...
      precisionAdversarialProof(
        state,
        state?.precision_evidence?.mutation_testing,
        "AVANTIQO_CODE_MUTATION_TEST_V1",
      ) ||
      precisionAdversarialProof(
        state,
        state?.precision_evidence?.property_fuzz,
        "AVANTIQO_CODE_FUZZ_TEST_V1",
      )
    ),
    runtime_observability: !classification.runtime || postMutationObservabilityProof(state, applyIndex),
    performance_proof: !classification.performance || performanceMeasurementProof(state?.performance_evidence),
    security_engineering: !classification.security || Boolean(
      state?.security_review?.passed === true &&
      (specialistReview || independentReview) &&
      (applyIndex >= 0
        ? postMutationRecordProof(state, state?.security_review, applyIndex)
        : hasSuccessfulVerification(state))
    ),
    engineering_memory: memoryObserved,
    self_improvement: selfImprovementObserved,
    dynamic_routing: routingObserved,
    program_manager: !classification.broad_program || productProgramManagerProof(state),
    hidden_benchmark: !classification.broad_program || hiddenBenchmarkProof(state),
  };
}
