...
function readinessSignals(state, classification) {
  const applyIndex = operationIndex(state, (entry) => entry?.action === "apply_files" || entry?.action === "patch");
  const preMutationVerificationIndex = operationIndex(state, (entry) => ["verify", "run", "browser_verify"].includes(entry?.action));
  const postMutationVerification = evidence(state).some((entry, index) =>
    index > applyIndex && entry?.kind === "operation" && ["verify", "run", "browser_verify"].includes(entry?.action) && Number(entry?.result?.exit_code ?? 0) === 0
  );
  const graph = deriveCodeAICausalGraph(state);
  const memoryObserved = Boolean(
    state?.verified_engineering_memory?.contract ||
    state?.engineering_memory_utility?.contract ||
  ...
  return {
    ...
    reproduction_first: !classification.defect || Boolean(
      state?.reproduction?.before_failure_observed === true &&
      state?.reproduction?.after_pass_observed === true
    ) || (preMutationVerificationIndex >= 0 && (applyIndex < 0 || preMutationVerificationIndex < applyIndex) && postMutationVerification),
    hypothesis_debugging: !classification.defect || list(state?.hypothesis_debugging?.hypotheses).length >= 3 || evidence(state).some((entry) => entry?.kind === "causal_hypothesis_record"),
    durable_runtime: Boolean(text(state?.mission_id, 240) || state?.attestation),
    browser_verification: !classification.ui || hasVerificationFamily(state, "browser") || evidence(state).some((entry) => entry?.action === "browser_verify" && entry?.status === "completed"),
    database_department: !classification.database || Boolean(state?.database_review?.passed === true && hasSuccessfulVerification(state)),
    ...
    adversarial_testing: !(classification.high_risk || classification.defect) || Boolean(state?.behavioral_verification?.verified === true || state?.employee_completion?.behavioral_verification?.verified === true || postMutationVerification),
    runtime_observability: !classification.runtime || Boolean(list(state?.observability_evidence?.records).length > 0 || runtimeEvidenceCount(state) > 0),
    ...
    security_engineering: !classification.security || Boolean(state?.security_review?.passed === true && (specialistReview || independentReview)),
    engineering_memory: memoryObserved || true,
    ...
    hidden_benchmark: !classification.broad_program || Boolean(state?.hidden_benchmark_certification?.verified === true || state?.competitive_benchmark?.attested === true || state?.benchmark_scorecard?.held_out === true),
  };
}
...
export function finalizeCodeAIEngineeringOperatingSystem({ prepared_control = null, result = null } = {}) {
  ...
  const signals = readinessSignals(state, classification);
