...
export function prepareCodeAIEngineeringPrecisionOS({objective,state={},objective_context={}}={}){
 ...
 const readOnlyFastPath=isCodeAIExplicitReadOnlyIntent(objective,objective_context?.owner_objective,state?.objective);
 const directive=(readOnlyFastPath?[
  "AVANTIQO ENGINEERING PRECISION OS V1 — READ-ONLY FAST PATH",
  "Use deterministic inspect/search/read/diff evidence only; source mutation remains forbidden.",
  "Prefer exact owner-named paths and bounded verification. Do not activate mutation, fuzz, migration, staging or release machinery unless the owner requested it.",
  "Missing evidence remains unknown. Keep raw chain-of-thought private and persist only facts, decisions and evidence.",
 ]:[
