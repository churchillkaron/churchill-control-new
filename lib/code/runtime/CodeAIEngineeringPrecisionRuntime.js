...
export function deriveCodeAIWatchdog({events=[],pendingSince=null,nowMs=Date.now()}={}){
  const recent=list(events).slice(-12);const fingerprints=recent.map(e=>sha(JSON.stringify([e?.action,e?.description,e?.input])));const repeated=fingerprints.length>=4&&new Set(fingerprints.slice(-4)).size===1;const pendingAge=pendingSince?Math.max(0,nowMs-Date.parse(pendingSince)):0;const stale=pendingAge>15*60*1000;return{contract:"AVANTIQO_CODE_WATCHDOG_V1",repeated_no_progress:repeated,pending_age_ms:pendingAge,stale_pending:stale,recovery_required:repeated||stale,recovery_action:stale?"REACQUIRE_OR_RESUME":repeated?"REPLAN_WITH_NEW_EVIDENCE":"NONE"};
}
