...
function cacheKey({ orgId, actor, mission, repositoryUrl, ref }) {
  return [
    orgId,
    actor,
    mission,
    normalizedRepository(repositoryUrl),
    text(ref, 160).toLowerCase(),
  ].join("|");
}

function cachedReceipt(key) {
  const entry = hotCache.get(key);
  if (!entry) return null;
  if (Date.now() - entry.cached_at > HOT_CACHE_TTL_MS) {
    hotCache.delete(key);
    return null;
  }
...
function cacheReceipt(key, receipt) {
  if (hotCache.size >= HOT_CACHE_MAX_ENTRIES && !hotCache.has(key)) {
    const oldest = [...hotCache.entries()].sort(
      (a, b) => Number(a[1]?.cached_at || 0) - Number(b[1]?.cached_at || 0),
    )[0];
...
async function readVisibleReceipt({
  orgId,
  actor,
  mission,
  repositoryUrl,
  ...
  const expectedRepository = normalizedRepository(repositoryUrl);
  ...
  for (const row of result.data || []) {
    ...
    if (expectedRepository) {
      const repository = normalizedRepository(metadata.repository_url);
...
export async function loadCodeAIEngineeringSkillVisibleReceipt({
  ...
  const key = cacheKey({ orgId, actor, mission, repositoryUrl, ref });
  const cached = cachedReceipt(key);
  if (cached) return cached;

  const existing = inFlightLoads.get(key);
  if (existing) return existing;

  const loadPromise = readVisibleReceipt({
    orgId,
    actor,
    mission,
    repositoryUrl,
    ...
    .then((receipt) => cacheReceipt(key, receipt))
    .finally(() => {
...
export const CodeAIEngineeringSkillVisibleReceiptRuntime = Object.freeze({
  ...
  hot_cache_ttl_ms: HOT_CACHE_TTL_MS,
  hot_cache_max_entries: HOT_CACHE_MAX_ENTRIES,
  in_flight_load_deduplication: true,
  cache_scope: "organization_actor_mission_repository_ref",
  contains_raw_reasoning: false,
  contains_raw_source: false,
  contains_raw_patch: false,
  automatic_knowledge_promotion: false,
