...
const MAX_SOURCE_CHANGE_CHARS = 12000;
...
function requiredApprovalsForRisk(risk) {
  if (["critical", "high"].includes(normalizedRisk(risk))) return REVIEW_ROLES.length;
  return 0;
}
...
export function assessCodeAIFinalIndependentReviewGate(state = {}, quality = null) {
  ...
  const approvedReviewerRoles = unique(
    list(review.reviews)
      .filter((entry) =>
        entry?.success === true &&
        entry?.approved === true &&
        text(entry?.verdict, 80) === "approve"
      )
      .map((entry) => text(entry?.role, 120))
  );
  const observedApprovals = approvedReviewerRoles.length;
  const requiredReviewerRoles = REVIEW_ROLES.map((role) => role.id);
  const requiredReviewerCoverage = requiredReviewerRoles.every((role) =>
    approvedReviewerRoles.includes(role)
  );
  const actualBlockingFindings = list(review.reviews).reduce(
    (sum, entry) => sum + Math.max(0, Number(entry?.blocking_finding_count || 0)),
    0,
  );
  const repairVerdicts = list(review.reviews).filter((entry) =>
    text(entry?.verdict, 80) === "repair_required"
  ).length;
  const blockingFindings = Math.max(
    actualBlockingFindings,
    Math.max(0, Number(review.blocking_finding_count || 0)),
  );
  ...
  else if (blockingFindings > 0 || repairVerdicts > 0) blocker = "CODE_AI_FINAL_INDEPENDENT_REVIEW_REPAIR_REQUIRED";
...
function reviewerPrompt({ role, objective, state, quality }) {
  return [
    ...
    `OWNER INTENT: ${text(objective, 4500)}`,
...
export async function runCodeAIFinalIndependentReview({
  ...
  const settled = await Promise.allSettled(reviewRoles.map((role) => runReasoning({
  ...
  const reviews = settled.map((entry, index) => entry.status === "fulfilled"
    ? normalizeReview(reviewRoles[index], entry.value)
    : failedReview(reviewRoles[index], entry.reason));
  ...
  return {
    ...
    local_review_default: true,
    modal_review_fallback_allowed: false,
