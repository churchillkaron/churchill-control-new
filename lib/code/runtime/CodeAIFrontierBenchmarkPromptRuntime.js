...
export function renderCodeAIFrontierBenchmarkPrompt({ prompt_contract, benchmark_case } = {}) {
  ...
  const observedEvidence = list(item.observed_evidence).map((value) => text(value, 2000)).filter(Boolean);
  const constraints = list(item.constraints).map((value) => text(value, 2000)).filter(Boolean);
  const replacements = {
    "{{title}}": text(item.title, 1200),
    "{{category}}": text(item.category, 240),
    "{{scenario_json}}": JSON.stringify(text(item.scenario, 6000)),
    "{{observed_evidence_json}}": JSON.stringify(observedEvidence),
