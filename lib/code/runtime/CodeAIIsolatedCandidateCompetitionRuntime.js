...
export async function runCodeAIIsolatedCandidateCompetition({
  ...
  const strategyCompetition = filterCodeAIStrategyCompetitionByObservedContracts({
    state: resumeState,
    competition: object(strategy_competition),
  });
  if (!eligibleForTrial({ policy, strategyCompetition, resumeState })) {
    return {
      ...
      executed: false,
      candidate_count: 0,
      contract_rejected_candidate_count: Number(strategyCompetition.contract_rejected_count || 0),
      contract_rejected_candidates: list(strategyCompetition.contract_rejected_candidates).slice(0, 12),
      candidates: [],
      winner_index: null,
      winner_result: null,
  ...
  const requestedBudget = Number(input?.reasoning_call_budget);
  const totalBudget = Number.isInteger(requestedBudget) && requestedBudget > 0
    ? Math.min(8, requestedBudget)
    : 4;
  const remainingBudget = Math.max(0, totalBudget - baseUsed);
  if (remainingBudget < ranked.length) {
    return {
      contract: CODE_AI_ISOLATED_CANDIDATE_EXECUTION_CONTRACT,
      status: "NOT_RUN_INSUFFICIENT_REASONING_BUDGET",
      executed: false,
      candidate_count: 0,
      contract_rejected_candidate_count: Number(strategyCompetition.contract_rejected_count || 0),
      contract_rejected_candidates: list(strategyCompetition.contract_rejected_candidates).slice(0, 12),
      ...
      winner_index: null,
      winner_result: null,
      fallback_to_single_writer_required: true,
      reasoning_call_budget: totalBudget,
      reasoning_calls_already_used: baseUsed,
      reasoning_calls_required_for_competition: ranked.length,
      reasoning_calls_consumed: 0,
      source_mutation_authority: false,
      main_branch_parallel_mutation_forbidden: true,
      commit_authority: false,
      deploy_authority: false,
  ...
  const candidateInputs = ranked.map((candidate, index) => ({
    ...
    context: {
      ...
      metadata: {
        ...object(input?.context?.metadata),
        codeAIIsolatedCandidateTrial: true,
        codeAIIsolatedCandidateIndex: index + 1,
        codeAIIsolatedCandidateParentMissionId:
          text(resumeState?.mission_id, 240) || null,
      },
    },
  ...
  const winnerResult = winner && settled[winner.index].status === "fulfilled"
    ...
    : null;
  const trialReasoningCallsConsumed = summaries.reduce(
    (total, summary) => total + Math.max(0, Number(summary.reasoning_calls || 0) - baseUsed),
    0,
  );
  ...
  return {
    ...
    reasoning_call_budget: totalBudget,
    reasoning_calls_already_used: baseUsed,
    reasoning_calls_consumed: trialReasoningCallsConsumed,
    parent_reasoning_calls_after_competition: baseUsed + trialReasoningCallsConsumed,
    trial_reasoning_calls_per_candidate_maximum: 1,
    shared_live_progress_suppressed_for_candidates: true,
    parent_mission_progress_authoritative: true,
    source_mutation_authority: false,
    main_branch_parallel_mutation_forbidden: true,
