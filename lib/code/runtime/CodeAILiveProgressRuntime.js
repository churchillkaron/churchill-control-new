...
const DURABLE_PROGRESS_CHECKPOINT_MS = 15 * 1000;
const WORKER_LEASE_TOUCH_INTERVAL_MS = 60 * 1000;
const TERMINAL_PROGRESS_STATES = new Set(["blocked", "cancelled", "canceled", "completed", "failed", "stopped"]);
const DURABLE_PROGRESS_PHASES = new Set(["MISSION_ACCEPTED", "MISSION_TERMINAL", "OWNER_STOPPED"]);
const EPHEMERAL_CACHE_GLOBAL_KEY = "__avantiqo_code_live_progress_cache_v1__";

function ephemeralCache() {
  if (!(globalThis[EPHEMERAL_CACHE_GLOBAL_KEY] instanceof Map)) {
    globalThis[EPHEMERAL_CACHE_GLOBAL_KEY] = new Map();
  }
  return globalThis[EPHEMERAL_CACHE_GLOBAL_KEY];
}
...
function withDeadline(promise, timeoutMs, fallbackValue) {
  const duration = Math.max(50, Number(timeoutMs) || 500);
  return Promise.race([
    Promise.resolve(promise),
    new Promise((resolve) => setTimeout(() => resolve(fallbackValue), duration)),
  ]);
}
...
function memoryKey(actor, deviceSessionId = null) {
  const session = text(deviceSessionId, 160);
  const scope = session ? `${actor}::device-session::${session}` : actor;
  const digest = crypto.createHash("sha256").update(scope, "utf8").digest("hex").slice(0, 32);
  return `code_ai_live_progress:v1:${digest}`;
}

function ephemeralProgressKey(context = {}, deviceSessionId = null) {
  const orgId = organizationId(context);
  const actor = actorId(context);
  if (!orgId || !actor) return null;
  return `${orgId}::${actor}::${text(deviceSessionId, 160) || "organization"}`;
}

function cachedProgress(context = {}, deviceSessionId = null) {
  const key = ephemeralProgressKey(context, deviceSessionId);
  if (!key) return null;
  return ephemeralCache().get(key) || null;
}

function writeCachedProgress(context = {}, deviceSessionId = null, liveProgress = null) {
  const key = ephemeralProgressKey(context, deviceSessionId);
  if (!key || !liveProgress) return;
  const current = object(ephemeralCache().get(key));
  ephemeralCache().set(key, {
    ...current,
...
function markCachedRuntimeTimestamp(context = {}, deviceSessionId = null, field, value = null) {
  const key = ephemeralProgressKey(context, deviceSessionId);
  if (!key || !field) return;
  const current = object(ephemeralCache().get(key));
  ephemeralCache().set(key, {
...
function shouldPersistProgress({ cached, sameMission, compact, projection }) {
  if (!sameMission) return true;
  if (DURABLE_PROGRESS_PHASES.has(text(compact?.phase, 80).toUpperCase())) return true;
  if (TERMINAL_PROGRESS_STATES.has(text(compact?.status || projection?.state_status, 80).toLowerCase())) return true;
  return elapsedSince(
    cached?.last_durable_checkpoint_attempt_at || cached?.last_durable_write_at,
  ) >= DURABLE_PROGRESS_CHECKPOINT_MS;
}
function progressDeviceSessionId(context = {}, state = {}, event = {}) {
  return text(
    context.device_session_id ||
    context.deviceSessionId ||
    state.device_session_id ||
    state.objective_context?.device_session_id ||
...
function compactEvent(event = {}) {
  ...
  return {
    ...
    start_line: Number.isFinite(Number(source.start_line || source.line)) ? Math.max(1, Number(source.start_line || source.line)) : null,
    end_line: Number.isFinite(Number(source.end_line)) ? Math.max(1, Number(source.end_line)) : null,
...
function compactEngineeringPlan(value = {}) {
  const source = object(value);
  ...
  return {
    ...
    revision_reasons: list(source.revision_reasons)
    ...
    phases: list(source.phases).slice(0, 8).map((phase) => ({
      phase: text(phase?.phase, 120) || null,
      status: text(phase?.status, 80) || null,
      task_count: Number(phase?.task_count || 0),
      completed_count: Number(phase?.completed_count || 0),
      in_progress_count: Number(phase?.in_progress_count || 0),
    })),
    tasks: list(source.tasks).slice(0, 14).map((task) => ({
    ...
    business_acceptance: {
      explicit_criteria_bound:
        source?.business_acceptance?.explicit_criteria_bound === true,
...
function projectionFromState(state = {}, event = null) {
  ...
  return {
    ...
    device_id: text(source.device_id, 160) || text(objectiveContext.device_id, 160) || text(event?.device_id, 160) || null,
    device_session_id: text(source.device_session_id, 160) || text(objectiveContext.device_session_id, 160) || text(event?.device_session_id, 160) || null,
    ...
    files_changed: list(source.files_changed)
      ...
      .filter(Boolean)
      .slice(0, 100),
    engineering_plan: compactEngineeringPlan(source.engineering_plan),
...
async function loadRow(context, deviceSessionId = null) {
  const orgId = organizationId(context);
  const actor = actorId(context);
  if (!orgId || !actor) return null;
  ...
  const result = await supabaseAdmin
    ...
    .eq("memory_key", memoryKey(actor, deviceSessionId))
    .eq("active", true)
    .maybeSingle();
...
export async function publishCodeAILiveProgress({
  ...
  if (!orgId || !actor) {
    return {
      ...
      worker_lease_refreshed: false,
    };
  }

  const deviceSessionId = progressDeviceSessionId(context, state, event);
  const cached = cachedProgress(context, deviceSessionId);
  const previousProgress = object(cached?.live_progress);
  ...
  const sameMission = Boolean(
    ...
    previousProgress.mission_id &&
    projection.mission_id === previousProgress.mission_id
  );
  const events = [
    ...(sameMission ? list(previousProgress.events) : []),
    compact,
  ].slice(-MAX_EVENTS);
  const liveProgress = {
    ...projection,
  ...
  const persistRequired = shouldPersistProgress({
    cached,
    sameMission,
    compact,
    projection,
  });
  writeCachedProgress(context, deviceSessionId, liveProgress);

  let workerLeaseRefreshed = false;
  if (elapsedSince(cached?.last_worker_lease_attempt_at) >= WORKER_LEASE_TOUCH_INTERVAL_MS) {
    markCachedRuntimeTimestamp(context, deviceSessionId, "last_worker_lease_attempt_at", now);
    workerLeaseRefreshed = await withDeadline(
      refreshActiveWorkerLease(),
      120,
      false,
    );
    if (workerLeaseRefreshed) {
      markCachedRuntimeTimestamp(context, deviceSessionId, "last_worker_lease_touch_at", now);
    }
  }
  ...
  if (!persistRequired) {
    return {
      persisted: false,
      reason: "CODE_AI_LIVE_PROGRESS_LOCAL_HOT_PATH",
      updated_at: now,
  ...
  markCachedRuntimeTimestamp(context, deviceSessionId, "last_durable_checkpoint_attempt_at", now);
  try {
    const supabaseAdmin = await adminClient();
    const result = await withDeadline(
      ...
      .upsert({
        ...
        memory_key: memoryKey(actor, deviceSessionId),
        memory_type: "fact",
      ...
      .maybeSingle(),
      220,
      { data: null, error: new Error("CODE_AI_LIVE_PROGRESS_DURABLE_WRITE_TIMEOUT") },
    );

    if (result?.error) throw result.error;
    if (result.data?.id) {
      markCachedRuntimeTimestamp(context, deviceSessionId, "last_durable_write_at", result.data?.updated_at || now);
    }
    return {
    ...
    return {
      persisted: false,
      reason: "CODE_AI_LIVE_PROGRESS_DURABLE_WRITE_UNAVAILABLE",
      durable_error: text(error?.message || error, 500) || null,
      updated_at: now,
      worker_lease_refreshed: workerLeaseRefreshed,
      ephemeral: true,
      live_progress: liveProgress,
...
export async function loadCodeAILiveProgress({ context = {}, device_session_id = null } = {}) {
  const orgId = organizationId(context);
  const actor = actorId(context);
  if (!orgId || !actor) {
    return { found: false, live_progress: null };
  }
  const requestedDeviceSessionId = text(
    device_session_id || context.device_session_id || context.deviceSessionId,
    160,
  ) || null;
  const cached = cachedProgress(context, requestedDeviceSessionId);
  if (cached?.live_progress) {
    return {
      found: true,
      row_id: null,
      updated_at: cached.updated_at || null,
      live_progress: object(cached.live_progress),
      ephemeral: true,
      local_authoritative: true,
      durable_lookup_performed: false,
    };
  }

  let row = null;
  try {
    const durableLoad = await withDeadline(
      loadRow(context, requestedDeviceSessionId)
        .then((value) => ({ value, error: null }))
        .catch((error) => ({ value: null, error })),
      450,
      { value: null, error: new Error("CODE_AI_LIVE_PROGRESS_DURABLE_LOAD_TIMEOUT") },
    );
    if (durableLoad?.error) throw durableLoad.error;
    row = durableLoad?.value || null;
  } catch (error) {
    if (cached?.live_progress) {
      return {
        found: true,
  ...
  const durableProgress = object(metadata.live_progress);
  const durableAt = Date.parse(text(row.updated_at));
  const cachedAt = Date.parse(text(cached?.updated_at));
  if (cached?.live_progress && Number.isFinite(cachedAt) && (!Number.isFinite(durableAt) || cachedAt > durableAt)) {
    return {
      found: true,
      row_id: row.id,
      updated_at: cached.updated_at || row.updated_at || null,
      live_progress: object(cached.live_progress),
      ephemeral: true,
    };
  }
