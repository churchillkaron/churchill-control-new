...
function verifierKey(entry = {}) {
  return codeAIVerifierKey(entry);
}
...
function safeStrategyDecision(state = {}) {
  const competition = object(state.solution_strategy_competition);
  const isolated = object(state.isolated_candidate_competition);
  return {
    contract: text(competition.contract, 180) || null,
    candidate_count: Number(competition.candidate_count || 0),
    selected_direction: text(competition?.selected?.direction, 1600) || null,
    rejected_direction: text(competition?.strongest_rejected?.direction, 1600) || null,
    selection_margin: Number(competition.selection_margin || 0),
    isolated_candidate_status: text(isolated.status, 120) || null,
    isolated_candidate_winner_index: Number(isolated.winner_index || 0) || null,
...
function missionPerformanceProjection(state = {}, row = {}) {
  const completion = object(state.employee_completion);
  const passes = Number(state?.employee_mission?.employee_passes_used || 0);
  const reasoningCalls = Number(state?.work_package_control?.reasoning_calls_used || 0);
  const failureCount = list(state.failures).length;
  const sourceRepairCount = list(state.repairs).length;
  const finalReviewRepairCount = list(state.evidence).filter((entry) =>
    text(entry?.kind, 120) === "final_independent_review_controller" &&
    text(entry?.status, 120) === "repair_required"
  ).length;
  const employeeContinuationCount = list(state.evidence).filter((entry) =>
    text(entry?.kind, 120) === "employee_controller" &&
    text(entry?.status, 120) === "continue_until_verified_complete"
  ).length;
  const repairCount = sourceRepairCount + finalReviewRepairCount + employeeContinuationCount;
  const ownerInterventionCount = list(state.evidence)
    .filter((entry) => text(entry?.kind, 120) === "owner_intervention").length;
  const started = Date.parse(state.created_at || row.created_at || "");
  const finished = Date.parse(state.updated_at || row.updated_at || "");
  const wallMs = Number.isFinite(started) && Number.isFinite(finished) && finished >= started
    ? finished - started
    : null;
  const fullyVerified = verifiedComplete(state);
  const isolatedCandidateCompetitionUsed =
    state?.isolated_candidate_competition?.executed === true;
  const directFirstPass = Boolean(
    fullyVerified &&
    failureCount === 0 &&
    repairCount === 0 &&
    passes <= 1 &&
    reasoningCalls <= 1 &&
    !isolatedCandidateCompetitionUsed
  );
  return {
    reasoning_calls_used: Number.isFinite(reasoningCalls) ? reasoningCalls : 0,
    employee_passes_used: Number.isFinite(passes) ? passes : 0,
    failure_count: failureCount,
    repair_count: repairCount,
    source_repair_count: sourceRepairCount,
    final_review_repair_count: finalReviewRepairCount,
    employee_continuation_count: employeeContinuationCount,
    owner_intervention_count: ownerInterventionCount,
    first_pass_success: directFirstPass,
    direct_first_pass_success: directFirstPass,
    candidate_assisted_verified_completion:
      fullyVerified && isolatedCandidateCompetitionUsed,
    isolated_candidate_competition_used: isolatedCandidateCompetitionUsed,
    runtime_evidence_required:
      completion?.runtime_evidence_coverage?.required === true,
    runtime_evidence_verified:
      completion?.runtime_evidence_coverage?.verified === true,
    wall_ms: wallMs,
    wall_clock_source:
      state.created_at && state.updated_at ? "MISSION_STATE" : "HISTORY_ROW_FALLBACK",
  };
}


function missionEngineeringHotspots(state = {}) {
  const world = object(state.world_class_intelligence);
  const projection = object(world.engineering_hotspots);
  const items = list(projection.items).map((item) => ({
    priority: text(item?.priority, 20) || null,
    area: text(item?.area, 120) || null,
    score: Number(item?.score || 0),
    evidence: text(item?.evidence, 1200) || null,
    recommendation: text(item?.recommendation, 1200) || null,
    affected_paths: list(item?.affected_paths).map((path) => text(path, 1000)).filter(Boolean).slice(0, 12),
  })).filter((item) => item.area);
  return {
    contract: text(projection.contract, 180) || "AVANTIQO_CODE_AI_ENGINEERING_HOTSPOT_PROJECTION_V1",
    hotspot_count: items.length,
    highest_priority: text(projection.highest_priority, 20) || null,
    weighted_severity: Number(projection.weighted_severity || 0),
    items,
...
function summaryProjection(row, scope, search = null) {
  ...
  const performance = missionPerformanceProjection(state, row);
  const engineeringHotspots = missionEngineeringHotspots(state);
  ...
  return {
    ...
    project_identity: object(state.project_identity),
...
export async function listCodeAIMissionHistory({
  ...
  const performanceSessions = sessions.slice(0, 50);
  const performance = aggregateCodeAIEngineeringPerformance(performanceSessions);
  const performanceTrend = compareCodeAIEngineeringPerformanceWindows(performanceSessions);
  const improvementBacklog = deriveCodeAIEngineeringImprovementBacklog(
    performance,
    performanceTrend,
  );
  const engineeringHotspotBacklog = aggregateCodeAIEngineeringHotspotBacklog(performanceSessions);
  return {
    contract: CODE_AI_MISSION_HISTORY_CONTRACT,
    sessions: visible,
    performance,
    ...
    engineering_hotspot_backlog: engineeringHotspotBacklog,
...
export async function loadLatestCodeAISemanticIndexSnapshot({
  context = {},
  repositoryUrl,
  ref = "main",
} = {}) {
  const loaded = await artifactRows({ context, limit: MAX_HISTORY_ROWS });
  const targetRepository = normalizedRepository(repositoryUrl);
  const targetRef = text(ref, 160).toLowerCase() || "main";
  for (const row of loaded.rows) {
    const integrity = integrityProjection(row, loaded.scope);
    ...
    if (targetRepository && normalizedRepository(state.repository_url) !== targetRepository) continue;
    if (targetRef && text(state.ref, 160).toLowerCase() !== targetRef) continue;
    const index = object(state.semantic_repository_index);
    if (text(index.contract, 180) !== "AVANTIQO_CODE_INCREMENTAL_SEMANTIC_INDEX_V1") continue;
    return { found: true, contract: "AVANTIQO_CODE_SEMANTIC_INDEX_HISTORY_V1", mission_id: text(state.mission_id,240)||null, repository_head: text(state.base_commit,160)||null, index, integrity_verified: true, current_head_revalidation_required: true, authority_effect: "NONE" };
  }
  return { found: false, contract: "AVANTIQO_CODE_SEMANTIC_INDEX_HISTORY_V1", authority_effect: "NONE" };
}

export async function loadLatestCodeAIArchitectureBrainSnapshot({
  context = {},
  repositoryUrl,
  ref = "main",
} = {}) {
  ...
  for (const row of loaded.rows) {
    ...
    if (targetRepository && normalizedRepository(state.repository_url) !== targetRepository) continue;
    if (targetRef && text(state.ref, 160).toLowerCase() !== targetRef) continue;
    const architecture = object(state?.engineering_operating_system?.architecture_brain);
    if (!Object.keys(architecture).length || !object(architecture.graph).contract) continue;
    return {
      found: true,
      contract: "AVANTIQO_CODE_ARCHITECTURE_BRAIN_HISTORY_V1",
      mission_id: text(state.mission_id, 240) || null,
      repository_url: text(state.repository_url, 1000) || null,
      ref: text(state.ref, 160) || null,
      repository_head: text(architecture.repository_head || state.base_commit, 160) || null,
      graph_hash: text(architecture.graph_hash, 160) || null,
  ...
  return { found: false, contract: "AVANTIQO_CODE_ARCHITECTURE_BRAIN_HISTORY_V1", authority_effect: "NONE" };
}
...
export async function loadCodeAIMissionHistoryDetail({
  ...
  return {
    ...
    session: {
      ...
      strategy_decision: safeStrategyDecision(state),
      interventions,
      ...
      failures: list(state.failures).slice(-20).map((item) => ({
        ...
        contract_violations: list(item?.result?.violations).slice(0, 12).map((violation) => ({
          kind: text(violation?.kind, 160) || null,
          path: text(violation?.path, 1000) || null,
          symbol: text(violation?.symbol, 240) || null,
          field: text(violation?.field, 240) || null,
          key: text(violation?.key, 240) || null,
          table: text(violation?.table, 500) || null,
          rpc: text(violation?.rpc, 500) || null,
          column: text(violation?.column, 240) || null,
          property: text(violation?.property, 240) || null,
          parameter_index: Number.isInteger(Number(violation?.parameter_index)) ? Number(violation.parameter_index) : null,
          was_optional: violation?.was_optional === true,
          proposed_optional: violation?.proposed_optional === true,
        })).filter((violation) => violation.kind),
      })),
...
export const CodeAIMissionHistoryRuntime = Object.freeze({
  ...
  latestArchitectureBrain: loadLatestCodeAIArchitectureBrainSnapshot,
  latestSemanticIndex: loadLatestCodeAISemanticIndexSnapshot,
