...
function missionPerformanceProjection(state = {}, row = {}) {
  ...
  const repairCount = list(state.repairs).length;
  ...
  const finished = Date.parse(row.updated_at || "");
  ...
  return {
    ...
    first_pass_success:
      fullyVerified && failureCount === 0 && repairCount === 0 && passes <= 1,
    isolated_candidate_competition_used:
