...
const WORKSPACE_TRANSPORT_RETRY_LIMIT = 1;
const WORKSPACE_TRANSPORT_RETRY_CODES = new Set([
  "ECONNRESET",
  "ETIMEDOUT",
  "EPIPE",
  "UND_ERR_SOCKET",
...
function isRetryableWorkspaceTransportError(error) {
  const message = text(error?.message || error).toLowerCase();
  const code = text(error?.code || error?.cause?.code).toUpperCase();
  return Boolean(
    message === "terminated" ||
    message.includes("socket hang up") ||
    message.includes("fetch failed") ||
    message.includes("connection reset") ||
    WORKSPACE_TRANSPORT_RETRY_CODES.has(code)
  );
}

function workspaceTransportRetryState(resumeState, error, retryCount) {
  ...
  return {
    ...
    evidence: [
      ...
      {
        at: now(),
        kind: "workspace_transport_retry",
        retry_count: retryCount,
        retry_limit: WORKSPACE_TRANSPORT_RETRY_LIMIT,
        error: text(error?.message || error),
        planner_decision_reused: true,
        planner_iteration_reused: true,
        provider_inference_reused: true,
        production_side_effect_replayed: false,
      },
...
function statusPath(value) {
  ...
  if (!raw) return null;
  const candidate = raw.length >= 4 && raw[2] === " "
    ? raw.slice(3)
    : raw.replace(/^[A-Z?!]{1,2}\s+/, "");
  const renamed = candidate.includes(" -> ") ? candidate.split(" -> ").pop() : candidate;
  return text(renamed) || null;
}

function changedPaths(diff = {}) {
  return [...new Set(list(diff.status).map(statusPath).filter(Boolean))];
}

async function refreshSourceChanges(workspace, state, diff = null) {
  const snapshot = diff || await workspace.diff();
  const actualChanged = new Set(changedPaths(snapshot));
  ...
  for (const change of declared) {
    ...
    refreshed.push({ path: change.path, content: file.content });
  }
  ...
  return {
    ...
    declared_changed_paths: refreshed.map((item) => item.path),
    unexpected_changed_paths: [...actualChanged].filter(
      (path) => !refreshed.some((item) => item.path === path),
    ),
  };
}
...
async function executeCodeAIMissionOnce({
  objective,
  repository_url,
  ref = "main",
  operations,
  ...
  try {
    ...
    for (const operation of plan) {
      ...
      try {
        ...
        if (isRetryableWorkspaceTransportError(error)) throw error;
...
export async function executeCodeAIMission(input = {}) {
  let retryState = input?.resume_state || null;
  let retryCount = 0;

  while (true) {
    try {
      return await executeCodeAIMissionOnce({
        ...input,
        resume_state: retryState,
      });
    } catch (error) {
      if (
        !retryState ||
        !isRetryableWorkspaceTransportError(error) ||
        retryCount >= WORKSPACE_TRANSPORT_RETRY_LIMIT
      ) {
        throw error;
      }
      retryCount += 1;
      retryState = workspaceTransportRetryState(retryState, error, retryCount);
    }
  }
}
...
export const CodeAIMissionRuntime = Object.freeze({
  ...
  workspace_transport_retry_limit: WORKSPACE_TRANSPORT_RETRY_LIMIT,
