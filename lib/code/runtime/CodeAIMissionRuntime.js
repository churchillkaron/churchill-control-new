...
function repositoryOperationProgress(operation = {}, status = "running") {
  const action = text(operation.action, 80).toLowerCase();
  const input = object(operation.input);
  const filePath = text(input.file_path || input.path || input.files?.[0]?.path, 1000);
  ...
  } else if (["apply_files", "delete_files", "rename_files"].includes(action)) {
    description = completed
...
function missingRepositoryReadPath(operation, error) {
  ...
  return /enoent|no such file or directory|file not found|path not found|does not exist|code_ai_missing_read_path_replan_required/.test(message);
}
