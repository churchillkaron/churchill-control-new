...
async function executeOperation(workspace, operation, state) {
  switch (operation.action) {
    ...
    case "apply_files": {
      const result = await workspace.applyFiles(list(operation.input.files));
    ...
    case "run": {
      assertMissionCommand(operation.input);
      const result = await workspace.run(operation.input);
