...
function assertObservedHypothesisEvidenceOperationIds(state, evidenceOperationIds = []) {
  const ids = [...new Set(list(evidenceOperationIds).map((item) => text(item, 240)).filter(Boolean))].slice(0, 24);
  if (!ids.length) throw new Error("CODE_AI_HYPOTHESIS_EVIDENCE_REQUIRED");
  ...
  if (missing.length) {
    throw new Error(`CODE_AI_HYPOTHESIS_EVIDENCE_NOT_OBSERVED:${missing.join(",")}`);
  }
  return ids;
}
    ...
    case "record_hypotheses": {
      const hypotheses = list(operation.input?.hypotheses).slice(0, 8).map((entry, index) => {
        ...
        if (["ELIMINATED", "SUPPORTED"].includes(status)) {
          assertObservedHypothesisEvidenceOperationIds(
            state,
      ...
      if (hypotheses.length < 3) throw new Error("CODE_AI_MINIMUM_THREE_HYPOTHESES_REQUIRED");
...
export async function executeCodeAIMission({
  ...
  preserve_workspace_session = false,
  ...
  if (local_session_id && !state.local_session_id) state.local_session_id = text(local_session_id);
  state.objective_context = {
    ...
    ...(local_session_id ? { local_session_id: text(local_session_id) } : {}),
  };
  ...
  const resolvedWorkspaceTarget = text(workspace_target || state.objective_context?.workspace_target).toUpperCase();
  const effectiveLocalSessionId =
    text(local_session_id || state.local_session_id || state.objective_context?.local_session_id) || null;
  const workspace = await CodeWorkspaceRuntime.open({
    ...
    ...(resolvedWorkspaceTarget === "LOCAL_COMPUTER" && effectiveLocalSessionId
      ? { session_id: effectiveLocalSessionId }
      : {}),
    ...(timeout_ms ? { timeout_ms } : {}),
  });
  if (
    resolvedWorkspaceTarget === "LOCAL_COMPUTER" &&
    workspace.session_id
  ) {
    state.local_session_id = text(workspace.session_id);
    state.objective_context.local_session_id = state.local_session_id;
  }
  ...
  try {
    ...
    addEvidence(state, {
      ...
      local_session_id: state.local_session_id || workspace.session_id || null,
      local_session_reused: workspace.resume?.reused_session === true,
    });
    appendRepositoryGuidanceEvidence(state);
    ...
    const preserveLocalWorkspace =
      resolvedWorkspaceTarget === "LOCAL_COMPUTER" &&
      preserve_workspace_session === true;
    if (!device_session_id && !preserveLocalWorkspace) await workspace.stop();
  }
}
