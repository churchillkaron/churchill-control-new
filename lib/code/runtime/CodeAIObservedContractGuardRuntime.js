...
function repairOrDebugObjective(state = {}) {
  const objective = [
    text(state?.objective_context?.owner_objective, 5000),
    text(state?.employee_mission?.owner_intent, 5000),
    text(state?.objective, 5000),
  ].filter(Boolean).join("\n");
  return /\b(?:fix|repair|debug|correct|restore)\b/i.test(objective);
}
const NON_DATA_MEMBER_PROPERTIES = new Set([
  "map", "reduce", "filter", "forEach", "find", "findIndex", "some", "every",
  "includes", "slice", "splice", "join", "concat", "push", "pop", "shift",
  "unshift", "sort", "reverse", "trim", "toLowerCase", "toUpperCase",
  "replace", "replaceAll", "match", "test", "exec", "split", "startsWith",
  "endsWith", "charAt", "length", "isArray", "isFinite", "isNaN",
]);
const NON_DATA_MEMBER_RECEIVERS = new Set([
  "Array", "Object", "Number", "String", "Boolean", "Math", "JSON", "Promise",
  "Date", "RegExp", "console", "process", "Buffer", "crypto",
]);
function maskQuotedLiterals(content) {
...
function observedDataMemberProperties(content) {
  const found = [];
  const source = maskQuotedLiterals(
    String(content ?? "")
      .replace(/\/\*[\s\S]*?\*\//g, " ")
      .replace(/(^|\s)\/\/[^\n\r]*/g, "$1 "),
  );
  const pattern = /\b([A-Za-z_$][A-Za-z0-9_$]*)\s*(?:\?\.|\.)\s*([A-Za-z_$][A-Za-z0-9_$]*)\b/g;
  for (const match of source.matchAll(pattern)) {
    const receiver = match[1];
    const property = match[2];
    if (NON_DATA_MEMBER_RECEIVERS.has(receiver) || NON_DATA_MEMBER_PROPERTIES.has(property)) continue;
    found.push({ receiver, property });
  }
  return [...new Map(found.map((item) => [`${item.receiver}.${item.property}`, item])).values()];
}
function objectiveMemberProperties(state = {}) {
  const objective = text(state?.objective || state?.objective_context?.owner_objective, 5000);
  return new Set(observedDataMemberProperties(objective).map((item) => item.property));
}
...
function observedObjectLiteralPropertyNames(content) {
  const source = String(content ?? "")
    .replace(/\/\*[\s\S]*?\*\//g, " ")
    .replace(/(^|\s)\/\/[^\n\r]*/g, "$1 ");
  const found = [];
...
function authoritativeVerifierEvidencePaths(state = {}) {
  ...
  const verifierReferences = new Set([
    text(context.authoritative_verification_command, 1200),
    ...list(context.authoritative_verification_args).map((item) => text(item, 1200)),
  ].filter(Boolean));
  return declared.filter((path) => verifierReferences.has(path));
}

function authoritativeVerifierPropertyNames(state = {}) {
  const found = [];
  for (const verifierPath of authoritativeVerifierEvidencePaths(state)) {
    const content = observedReadContent(state, verifierPath);
    if (!content) continue;
    found.push(
      ...observedDataMemberProperties(content).map((item) => item.property),
      ...observedObjectLiteralPropertyNames(content),
    );
  }
  return new Set(unique(found));
}

function writeMap(writes) {
  return new Map(list(writes).map((item) => [text(item?.path, 1200), String(item?.content ?? "")]).filter(([path]) => path));
}
function normalizedRelativeImportTarget(filePath, specifier) {
  const raw = text(specifier, 1200).replaceAll("\\", "/");
  if (!raw.startsWith(".")) return null;
  const parts = text(filePath, 1200).replaceAll("\\", "/").split("/");
  parts.pop();
...
function selfImportObserved(filePath, analysis) {
  const normalizedPath = text(filePath, 1200).replaceAll("\\", "/");
  const extensionMatch = normalizedPath.match(/(\.[cm]?[jt]sx?)$/i);
  const extension = extensionMatch?.[1] || "";
  return list(analysis?.imports).some((entry) => {
    const target = normalizedRelativeImportTarget(normalizedPath, entry?.source);
    return Boolean(target && (target === normalizedPath || (extension && `${target}${extension}` === normalizedPath)));
  });
}
...
export function assessCodeAIObservedContractCompatibility({ state = {}, writes = [] } = {}) {
  ...
  const proposedWriteContent = normalizedWrites.map((item) => item.content).join("\n");
  const explicitObjectiveMembers = observedDataMemberProperties(
    state?.objective || state?.objective_context?.owner_objective || "",
  );
  const explicitVerifierProperties = authoritativeVerifierPropertyNames(state);
  ...
  for (const write of normalizedWrites) {
    ...
    const beforeContent = observedReadContent(state, write.path) || "";
    const beforeAnalysis = analyzeCodeAISourceDependencies(write.path, beforeContent);
    ...
    if (selfImportObserved(write.path, afterAnalysis)) {
      obligations.push({ path: write.path, kind: "SOURCE_MUST_NOT_IMPORT_ITSELF" });
      violations.push({ path: write.path, kind: "SOURCE_SELF_IMPORT_INTRODUCED" });
    }
    if (
      beforeContent.length > 0 &&
      beforeExports.size > 0 &&
      afterExports.size === 0 &&
      write.content.length < Math.max(120, Math.floor(beforeContent.length * 0.6))
    ) {
      obligations.push({ path: write.path, kind: "OBSERVED_PUBLIC_SURFACE_PRESERVATION", symbols: [...beforeExports].slice(0, 20) });
      violations.push({
        path: write.path,
        kind: "OBSERVED_PUBLIC_SURFACE_ERASED",
        removed_symbols: [...beforeExports].slice(0, 20),
        before_chars: beforeContent.length,
        proposed_chars: write.content.length,
      });
    }
    ...
    if (repairOrDebugObjective(state)) {
      for (const symbol of beforeExports) {
        if (afterExports.has(symbol) || callerSymbols.includes(symbol)) continue;
        obligations.push({ path: write.path, kind: "REPAIR_EXPORTED_SYMBOL_PRESERVATION", symbol });
        violations.push({ path: write.path, kind: "REPAIR_EXPORTED_SYMBOL_REMOVED", symbol });
      }
      for (const member of observedDataMemberProperties(beforeContent)) {
        const escapedProperty = member.property.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const propertyPattern = new RegExp(`(?:\\?\\.|\\.)\\s*${escapedProperty}\\b`);
        if (propertyPattern.test(proposedWriteContent)) continue;
        const explicitMigration = explicitObjectiveMembers.find((candidate) =>
          candidate.receiver === member.receiver &&
          candidate.property !== member.property
        );
        if (explicitMigration) {
          const escapedReplacement = explicitMigration.property.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
          const migrationPattern = new RegExp(`(?:\\?\\.|\\.)\\s*${escapedReplacement}\\b`);
          if (migrationPattern.test(proposedWriteContent)) {
            const key = `COHERENT_DATA_MEMBER_MIGRATION:${write.path}:${member.receiver}:${member.property}:${explicitMigration.property}`;
            if (!migrationKeys.has(key)) {
              migrationKeys.add(key);
              coherentMigrations.push({
                kind: "COHERENT_DATA_MEMBER_MIGRATION",
                path: write.path,
                receiver: member.receiver,
                from_property: member.property,
                to_property: explicitMigration.property,
              });
            }
            continue;
          }
        }
        const verifierReplacementCandidates = [...explicitVerifierProperties]
          .filter((property) => property !== member.property)
          .filter((property) => {
            const safeProperty = property.replace(/\$/g, "\\$");
            return new RegExp("(?:\\?\\.|\\.)\\s*" + safeProperty + "\\b").test(proposedWriteContent);
          });
        if (verifierReplacementCandidates.length === 1) {
          const replacementProperty = verifierReplacementCandidates[0];
          const key = [
            "AUTHORITATIVE_VERIFIER_DATA_MEMBER_MIGRATION",
            write.path,
            member.receiver,
            member.property,
            replacementProperty,
          ...
          if (!migrationKeys.has(key)) {
            migrationKeys.add(key);
            coherentMigrations.push({
              kind: "AUTHORITATIVE_VERIFIER_DATA_MEMBER_MIGRATION",
              path: write.path,
              receiver: member.receiver,
              from_property: member.property,
              to_property: replacementProperty,
              verifier_evidence_paths: authoritativeVerifierEvidencePaths(state),
            });
          }
          continue;
        }
        obligations.push({
          path: write.path,
          kind: "REPAIR_OBSERVED_DATA_MEMBER_PROPERTY",
          receiver: member.receiver,
          property: member.property,
        });
        violations.push({
          path: write.path,
          kind: "REPAIR_OBSERVED_DATA_MEMBER_PROPERTY_REMOVED",
          receiver: member.receiver,
          property: member.property,
        });
      }
