...
const INTERVENTION_ACTIONS = new Set(["STEER", "REQUEST_CHANGES", "STOP"]);
...
export async function submitCodeAIOwnerControl({
  ...
  if (!orgId) throw new Error("CODE_AI_OWNER_INTERVENTION_ORGANIZATION_REQUIRED");
  if (!actor) throw new Error("CODE_AI_OWNER_INTERVENTION_ACTOR_REQUIRED");
  if (!["APPROVE_PATCH", "STOP"].includes(controlAction) && !boundedInstruction) {
...
export async function consumePendingCodeAIOwnerStopAtSafeBoundary({
  context = {},
  missionId,
} = {}) {
  const rows = await scopedRows({ context, missionId, active: true });
  const pending = [...rows].reverse().find((row) => {
    const metadata = object(row.metadata);
    return (
      text(metadata.status, 80) === "PENDING" &&
      text(metadata.action, 80) === "STOP" &&
      metadata.consume_at_safe_boundary === true
    );
  });
  if (!pending?.id) return { applied: false, intervention: null };
  ...
  const result = await compareAndSetActiveRow(pending, {
    ...
    metadata: {
      ...
      status: "APPLIED",
      claim_id: claimId,
      claimed_at: boundaryAt,
      applied_at: boundaryAt,
      applied_claim_id: claimId,
      claim_expires_at: null,
      owner_stop: true,
      applied_at_safe_boundary: true,
      stop_requires_fresh_reasoning: false,
  ...
  return { applied: true, intervention: projection(result) };
}

export async function applyClaimedCodeAIOwnerStop({
  context = {},
  missionId,
  interventionId,
  claimId,
  ...
  if (!intervention || !claim) {
    throw new Error("CODE_AI_OWNER_STOP_CLAIM_IDENTITY_REQUIRED");
  }
...
export const CodeAIOwnerInterventionRuntime = Object.freeze({
  ...
  consume_stop_at_safe_boundary: consumePendingCodeAIOwnerStopAtSafeBoundary,
  apply_stop: applyClaimedCodeAIOwnerStop,
