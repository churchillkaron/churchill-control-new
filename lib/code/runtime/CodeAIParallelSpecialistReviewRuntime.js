...
function reusableReview(existing, fingerprint) {
  ...
  return (
    ...
    text(source.status, 80) === "COMPLETED" &&
    text(source.fingerprint, 12000) === fingerprint &&
    Number(source.reviewer_count_succeeded || 0) === ROLE_DEFINITIONS.length &&
    source.architecture_performance_review_present === true &&
    source.adversarial_risk_review_present === true &&
    list(source.reviews).length === ROLE_DEFINITIONS.length
  );
}
...
function reviewerPrompt({ role, objective, state, repositoryImpact, externalResearch }) {
  ...
  return [
    ...
    `BOUNDED CURRENT REPOSITORY EVIDENCE: ${jsonBounded(compactState, MAX_EVIDENCE_CHARS)}`,
...
export async function runCodeAIParallelSpecialistReview({
  ...
  const promises = roles.map((role) => runReasoning({
  ...
  const reviews = settled.map((entry, index) => entry.status === "fulfilled"
    ? normalizeReview(roles[index], entry.value)
    : failedReview(roles[index], entry.reason));
