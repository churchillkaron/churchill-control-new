...
const SAFE_EVIDENCE_FIRST_ACTIONS = new Set([
  "record_reproduction",
  "record_hypotheses",
...
const SAFE_DISCARDED_TRAILING_ACTIONS = new Set([
  "read",
  "search",
  "diff",
  "history",
...
function safeEvidenceFirstObjectWithReadOnlyTrailingJunk(raw) {
  ...
  if (end < 0 || depth !== 0 || inString) return null;
  ...
  const action = actionOf(parsed);
  if (!SAFE_EVIDENCE_FIRST_ACTIONS.has(action)) return null;

  const trailing = raw.slice(end + 1).trim();
  if (!trailing.startsWith(",")) return null;
  const trailingActions = [...trailing.matchAll(/"action"\s*:\s*"([^"]+)"/g)]
    .map((match) => text(match[1], 80).toLowerCase())
    .filter(Boolean);
  if (!trailingActions.length) return null;
  if (trailingActions.some((candidate) => !SAFE_DISCARDED_TRAILING_ACTIONS.has(candidate))) {
    return null;
  }
  ...
  return {
    parsed,
    normalization: {
      mode: "evidence_first_with_read_only_trailing_junk",
      object_count: 1 + trailingActions.length,
      discarded_count: trailingActions.length,
      discarded_actions: trailingActions,
      action,
    },
  };
}
...
export function parseCodeAIPlannerOutput(value, { maxChars = 24000 } = {}) {
  ...
  const evidenceFirstRecovery = safeEvidenceFirstObjectWithReadOnlyTrailingJunk(raw);
  if (evidenceFirstRecovery) return evidenceFirstRecovery;
