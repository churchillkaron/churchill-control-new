...
function plannerProgressDescription(kind, executionInput = {}, pending = {}, pollCount = 0) {
  ...
  if (kind === "queued") return `The local Code engine is working on the next repository step.${detail.iterationText}${detail.targetText}${detail.objectiveText}`;
  if (kind === "checking") return `The same local Code job is still running; I’m checking its result without restarting it.${detail.iterationText}${detail.targetText}${detail.objectiveText}${pollCount > 1 ? ` Result check ${pollCount}.` : ""}`;
  ...
  return `Code is working on the current repository step.${detail.targetText}${detail.objectiveText}`;
}
...
async function fastLocalPlannerJobStatus(pending) {
  ...
  if (!providerJobId || !isIntelligenceLocalQueueJob(providerJobId)) return null;
...
export async function executeCodeAIPlannerRequest({
  ...
  while (Date.now() < deadline) {
    ...
    const fastLocalStatus = await fastLocalPlannerJobStatus(pending);
    if (fastLocalStatus) {
