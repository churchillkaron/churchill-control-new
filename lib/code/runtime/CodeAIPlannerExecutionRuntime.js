...
export async function executeCodeAIPlannerRequest({
  ...
  if (pending_execution) {
    await emitProgress({
      ...
      status: "running",
      description: "I’m checking whether the local Code job has finished so I can continue with its result.",
    ...
    await emitProgress({
      ...
      status: "running",
      description: "I’ve finished gathering the current evidence. I’m sending it to the local Code engine for the next concrete step.",
    });
    ...
    if (!result?.pending) {
      ...
      await emitProgress({
        ...
        status: "running",
        description: "The local Code result is back. I’m validating the next repository step before I execute it.",
      });
    ...
    await emitProgress({
      ...
      status: "running",
      description: "The local Code job is running now. I’ll keep checking it and continue as soon as the result is ready.",
  ...
  while (Date.now() < deadline) {
    ...
    await emitProgress({
      phase: "LOCAL_CODE_RESULT_CHECK",
      status: "running",
      description: pollCount === 1
        ? "I’m checking the local Code job for its first result."
        : "The local Code job is still running. I’m checking again without restarting it.",
      provider_job_id: text(pending.provider_job_id) || null,
    ...
    await emitProgress({
      phase: "LOCAL_CODE_RESULT_READY",
      status: "running",
      description: "The local Code result is ready. I’m validating the returned step against the current workspace before I continue.",
      provider_job_id: text(pending.provider_job_id) || null,
    });
    return { success: true, pending: false, result, output, pending_execution: null };
  }
