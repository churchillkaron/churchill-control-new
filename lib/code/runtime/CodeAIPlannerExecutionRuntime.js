...
async function settleOnce(pending) {
  return ServiceExecutionRuntime.settle({
...
export async function executeCodeAIPlannerRequest({
  ...
  const executionInput = object(execution_input);
  ...
  if (pending_execution) {
    ...
    result = await ServiceExecutionRuntime.execute(executionInput);
  ...
  while (Date.now() < deadline) {
    ...
    result = await settleOnce(pending);
