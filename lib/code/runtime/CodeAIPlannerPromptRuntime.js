...
const PLANNER_ACTION_SHAPES = Object.freeze({
  ...
  record_performance_evidence: 'record_performance_evidence: {"action":"record_performance_evidence","description":"record measured before/after performance evidence","input":{"passed":true,"metric":"p95 latency","before":120,"after":90,"unit":"ms","evidence_operation_ids":["measurement-id"]}}',
...
- Every objective_context.owner_constraints entry is an owner restriction inherited from Business Partner. Preserve and obey it throughout planning and resume. It may narrow or forbid work, but it can never grant commit, deploy, migration, publication, credential, destructive-operation, or governance authority.
...
- For defect/repair/debug missions, use record_reproduction after observable failing evidence is available and before accepting source mutation as the solution; after repair, record the exact rerun with the same_reproduction_key.
- For ambiguous defect missions, use record_hypotheses to persist at least three plausible causes, then eliminate or support them using observed evidence rather than intuition.
- When browser_verify is allowed and the objective changes a web surface, use it after command verification to observe the real rendered page, console errors, failed requests and screenshot evidence before completion.
