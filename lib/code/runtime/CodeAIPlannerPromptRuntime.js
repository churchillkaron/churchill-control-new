...
const PLANNER_ACTION_SHAPES = Object.freeze({
  ...
  replace_range: 'replace_range: {"action":"replace_range","description":"replace only an observed bounded range","input":{"file_path":"path","start_line":1,"end_line":3,"expected":"exact observed original lines","replacement":"replacement lines"}}',
...
function plannerForwardProgressDirective(state, allowedActions = []) {
  ...
  const currentSourceRevision = integer(control.source_revision) || 0;
  const recentEvidence = list(source.evidence);
  const reproducedFailureAtCurrentRevision = recentEvidence.some((entry) =>
    entry?.kind === "operation" &&
    entry?.action === "record_reproduction" &&
    entry?.status === "completed" &&
    entry?.result?.status === "FAILED" &&
    (integer(entry?.result?.source_revision) || 0) === currentSourceRevision
  );
  const hypothesesEstablished = recentEvidence.some((entry) =>
    entry?.kind === "operation" &&
    entry?.action === "record_hypotheses" &&
    entry?.status === "completed" &&
    list(entry?.result?.hypotheses).length >= 3
  );
  const implementationRequired =
    source?.objective_context?.implementation_required === true;
  const sourceChangeCount = integer(source?.source_change_count) || 0;
  const mutationFocusRequired =
    implementationRequired &&
    sourceChangeCount === 0 &&
    reproducedFailureAtCurrentRevision &&
    hypothesesEstablished &&
    (allowed.has("apply_files") || allowed.has("replace_range"));
  const lines = [];
  ...
  if (mutationFocusRequired) {
    lines.push(
      "MUTATION FOCUS: the defect has an observed failing reproduction and hypotheses are established for the current source revision, but no mission source change exists yet. Use the observed source evidence to make the smallest allowed repair now with apply_files or replace_range unless one genuinely new dependency is required; do not recycle equivalent reads, diff, verification, reproduction, or completion.",
    );
  }
...
- For ambiguous defects, record at least three plausible hypotheses. A hypothesis may be ELIMINATED or SUPPORTED only when it includes one or more observed evidence_operation_ids; otherwise keep it PLAUSIBLE.
- Before declaring a defect repaired, update the hypothesis record so the investigated set is resolved: at least one hypothesis must be SUPPORTED, no required hypothesis may remain PLAUSIBLE, and every SUPPORTED or ELIMINATED conclusion must cite observed evidence_operation_ids.
