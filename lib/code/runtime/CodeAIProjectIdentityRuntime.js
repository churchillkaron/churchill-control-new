...
function observedPaths(state = {}) {
  ...
  return [...new Set([
    ...
    ...list(source.evidence).map((entry) => entry?.file_path || entry?.result?.file_path || entry?.result?.path),
    ...list(source.source_read_evidence).map((entry) => entry?.file_path || entry?.result?.file_path || entry?.result?.path),
