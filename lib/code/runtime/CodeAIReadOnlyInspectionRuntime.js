...
import { publishAvantiqoLiveExecution } from "../../platform/runtime/AvantiqoLiveExecutionRuntime.js";
import { extractCodeAIExplicitRepositoryPaths } from "./CodeAIReadOnlyIntentRuntime.js";
...
export function explicitCodeAIReadOnlyPaths(message) {
  return extractCodeAIExplicitRepositoryPaths(message, 12);
}
...
export async function runCodeAIReadOnlyInspection({ context = {}, message, repository_url = DEFAULT_REPOSITORY, ref = "main", workspace_target = null, timeout_ms = 120000 } = {}) {
  ...
  try {
    ...
    const explicitPaths = explicitCodeAIReadOnlyPaths(message);
    await publish(context, {
      phase: "CODE_INSPECTION_SEARCH",
      description: explicitPaths.length
        ? `Using ${explicitPaths.length} exact repository path${explicitPaths.length === 1 ? "" : "s"} named by the owner.`
        : `Searching current main for ${label} pages, components, APIs and tests.`,
    });
    const groups = explicitPaths.length ? [] : [
    ...
    const groupedPaths = [...explicitPaths];
    ...
    const selected = explicitPaths.length
      ? paths.slice(0, MAX_FILES)
      : [...paths].sort((a, b) => filePriority(b) - filePriority(a) || a.localeCompare(b)).slice(0, MAX_FILES);
    const evidenceFiles = [];
    const findings = [];
