...
export function assessCodeAIRepairClosure(state = {}) {
  ...
  const failedBeforeFinalEdit = [];
  const successfulAfterFinalEdit = [];
  ...
  for (const test of list(state?.tests)) {
    ...
    if (exitCode !== 0 && lastEdit >= 0 && position.index < lastEdit) {
      failedBeforeFinalEdit.push(record);
    }
    if (exitCode === 0 && lastEdit >= 0 && position.index > lastEdit) {
      successfulAfterFinalEdit.push(record);
    }
  }

  const successfulSignatures = new Set(
    successfulAfterFinalEdit.map((item) => item.signature),
  );
  const unresolved = failedBeforeFinalEdit
    .filter((item) => !successfulSignatures.has(item.signature));
  const required = failedBeforeFinalEdit.length > 0;
  ...
  return {
    ...
    failed_verifier_count_before_final_edit: failedBeforeFinalEdit.length,
    closed_failed_verifier_count:
      failedBeforeFinalEdit.length - unresolved.length,
    ...
    closure_rule: "SAME_VERIFICATION_COMMAND_SIGNATURE_MUST_PASS_AFTER_FINAL_EDIT",
