...
function protectedBaselineDigest(item, verification) {
  return sha256(JSON.stringify({
    ...
    case_definition_sha256: text(item?.case_definition_sha256, 80).toLowerCase(),
...
export function assessCodeAIRepositoryTaskBenchmark(report = {}) {
  ...
  const cases = observations.map((item) => {
    ...
    const gates = {
      ...
      verification_case_definition_bound:
        sha(item?.case_definition_sha256) &&
        sha(verification?.case_definition_sha256) &&
        text(verification?.case_definition_sha256, 80).toLowerCase() === text(item?.case_definition_sha256, 80).toLowerCase(),
  ...
  return {
    ...
    repository_verification_must_bind_exact_case_definition: true,
    ...
    protected_baseline_must_bind_exact_case_definition: true,
