...
function finalMissionEditEpoch(state = {}) {
  let latest = null;
  for (const entry of list(state?.evidence)) {
    ...
    if (!codeAIEditAction(entry?.action)) continue;
    const observed = epoch(entry?.at);
...
function runtimeEvidenceFresh(entry, finalEditEpoch) {
  const source = object(entry);
  if (source.final_patch_replayed_in_isolated_workspace === true) return true;
  if (finalEditEpoch === null) return false;
  const observed = epoch(source.at || source.observed_at || source.created_at);
  return observed !== null && observed > finalEditEpoch;
}
...
export function assessCodeAIRuntimeEvidenceCoverage({
  ...
  const finalEditEpoch = finalMissionEditEpoch(state);
  const freshItems = items.filter((entry) => runtimeEvidenceFresh(entry, finalEditEpoch));
  const staleItems = items.filter((entry) => !runtimeEvidenceFresh(entry, finalEditEpoch));
  const browsers = browserEvidence(freshItems);
  const apiRuntime = apiRuntimeEvidence(freshItems);
  ...
  return {
    ...
    fresh_runtime_evidence_count: freshItems.length,
    stale_runtime_evidence_count: staleItems.length,
    final_edit_epoch_observed: finalEditEpoch,
    runtime_evidence_must_postdate_final_edit: required,
