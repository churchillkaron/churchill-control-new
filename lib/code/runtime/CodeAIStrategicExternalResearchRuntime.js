...
export async function runCodeAIStrategicExternalResearch({
  ...
  if (
    prior.contract === CODE_AI_STRATEGIC_EXTERNAL_RESEARCH_CONTRACT &&
    prior.required === true &&
    text(prior.query, 12000) === query &&
    ["RESEARCH_COMPLETED", "CANONICAL_PRODUCT_KNOWLEDGE_REUSED", "HYBRID_VERIFIED_KNOWLEDGE_REUSED"].includes(text(prior.status, 160))
  ) {
    return {
      ...prior,
