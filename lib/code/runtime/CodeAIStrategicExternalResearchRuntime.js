...
const LOCAL_REPOSITORY_VERIFICATION_SIGNAL =
  /\b(?:inspect|read|verify|validate|lint|typecheck|type-check|test|node\s+--check|check\s+(?:the\s+)?(?:file|route|module|workspace|repository|repo)|make\s+no\s+(?:source\s+)?changes?|no\s+(?:source\s+)?changes?|do\s+not\s+(?:change|modify|edit|commit|deploy))\b/i;
...
function researchDecisionSource(objective) {
  const source = text(objective, 9000);
  const latestInstruction = source.match(
    /Latest instruction:\s*([\s\S]*?)(?:\n\nConversation and approved visual context:|\n\nInspect the live repository|$)/i,
  );
  const ownerSource = text(latestInstruction?.[1] || source, 5000);
  const controllerBoundary = ownerSource.search(
    /\n(?:BOUND PRODUCT COMPLETION CRITERIA\.|EMPLOYEE COMPLETION GAPS FROM THE GOVERNED CONTROLLER:)/,
  );
  return controllerBoundary >= 0
    ? text(ownerSource.slice(0, controllerBoundary), 5000)
    : ownerSource;
}

export function resolveCodeAIStrategicExternalResearchNeed(objective) {
  const source = researchDecisionSource(objective);
  ...
  const localRepositoryVerification = LOCAL_REPOSITORY_VERIFICATION_SIGNAL.test(source);
  const localVerificationOnly = Boolean(
    localRepositoryVerification &&
    !explicit &&
    !optimization &&
    !comparative
  );
  ...
  return {
    ...
    local_repository_verification_signal: localRepositoryVerification,
    local_repository_verification_only: localVerificationOnly,
