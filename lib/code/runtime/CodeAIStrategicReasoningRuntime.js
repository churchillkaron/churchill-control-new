...
import {
  ...
  resolveCodeAIIsolatedCandidateCompetitionPolicy,
  resolveCodeAIAdaptiveReasoningBudget,
} from "./CodeAIWorldClassIntelligenceRuntime.js";
import {
  runCodeAIIsolatedCandidateCompetition,
  CODE_AI_ISOLATED_CANDIDATE_EXECUTION_CONTRACT,
} from "./CodeAIIsolatedCandidateCompetitionRuntime.js";
import { filterCodeAIStrategyCompetitionByObservedContracts } from "./CodeAIStrategyContractAdmissibilityRuntime.js";
...
function implementationPresent(state = {}) {
  ...
  const missionOwnedMutation = list(source.evidence).some((entry) =>
  ...
  if (!missionOwnedMutation) return false;
  return (
...
export function buildCodeAIStrategicObjective({
  ...
  solution_strategy_competition = null,
  ...
  const strategyCompetitionContext =
    formatCodeAISolutionStrategyCompetitionForObjective(solution_strategy_competition);

  return [goal, protocol, planContext, impactContext, researchContext, specialistContext, strategyCompetitionContext]
    .filter(Boolean)
    .join("\n\n");
}
...
function strategicMetadata({
  ...
  return {
    ...
    solution_strategy_competition_contract:
      text(solutionStrategyCompetition?.contract, 160) || null,
    solution_strategy_candidate_count:
      Number(solutionStrategyCompetition?.candidate_count || 0),
    solution_strategy_selected:
      text(solutionStrategyCompetition?.selected?.direction, 1800) || null,
    solution_strategy_rejected:
      text(solutionStrategyCompetition?.strongest_rejected?.direction, 1800) || null,
    solution_strategy_selection_margin:
      Number(solutionStrategyCompetition?.selection_margin || 0),
    isolated_candidate_execution_contract:
      CODE_AI_ISOLATED_CANDIDATE_EXECUTION_CONTRACT,
    isolated_candidate_execution_status:
      text(result?.state?.isolated_candidate_competition?.status, 120) || null,
    isolated_candidate_execution_winner_index:
      Number(result?.state?.isolated_candidate_competition?.winner_index || 0) || null,
    strategic_adaptive_reasoning_budget:
      Number(solutionStrategyCompetition?.strategic_adaptive_reasoning_budget || 0) || null,
    solution_strategy_additional_reasoning_calls_consumed: 0,
...
function researchBlockedResult({
  ...
  const strategic = strategicMetadata({
    ...
    solutionStrategyCompetition: null,
...
export async function executeCodeAIStrategicBatchedMission(input = {}) {
  ...
  let initialPlanningState = stateForPlanning(input, originalObjective);
  if (!text(initialPlanningState.base_commit, 160)) {
    const objectiveContext = object(initialPlanningState.objective_context || input.objective_context);
    const inspected = await executeCodeAIMission({
      objective: originalObjective,
      repository_url: initialPlanningState.repository_url,
      ref: initialPlanningState.ref || "main",
      operations: [{
        id: "strategic_initial_inspect",
        action: "inspect",
        description: "Establish repository head and repository guidance before strategic reasoning.",
        input: {},
      }],
      resume_state: null,
      mission_id: text(objectiveContext.mission_id, 240) || text(initialPlanningState.mission_id, 240) || null,
      timeout_ms: input.timeout_ms || null,
      workspace_target: objectiveContext.workspace_target || null,
      organization_id: objectiveContext.organization_id || input?.context?.organizationId || input?.context?.organization_id || null,
      device_id: objectiveContext.device_id || null,
      device_session_id: objectiveContext.device_session_id || null,
    });
    if (!inspected?.state?.base_commit) {
      throw new Error(inspected?.reason || "CODE_AI_STRATEGIC_INITIAL_INSPECTION_FAILED");
    }
    initialPlanningState = stateForPlanning({
      ...input,
      resume_state: {
        ...object(inspected.state),
        objective_context: objectiveContext,
        employee_mission: object(initialPlanningState.employee_mission),
      },
    }, originalObjective);
  }
  ...
  const researchObjective = text(
    input?.objective_context?.owner_objective ||
    plannedResumeState?.objective_context?.owner_objective ||
    originalObjective,
    9000,
  );
  ...
  const implementationAlreadyPresent = implementationPresent(plannedResumeState);
  const hasConcreteRepositoryEvidence =
    Number(repositoryImpact?.observed_path_count || 0) > 0 ||
    list(repositoryImpact?.observed_paths).length > 0 ||
    list(plannedResumeState?.files_changed).length > 0 ||
    implementationAlreadyPresent;
  const preImplementationSpecialistReviewRequired =
    ["high", "critical"].includes(text(repositoryImpact?.risk, 80).toLowerCase()) ||
    repositoryImpact?.requires_contract_attention === true;
  const specialistReviewRequired =
    hasConcreteRepositoryEvidence &&
    (implementationAlreadyPresent || preImplementationSpecialistReviewRequired);
  const specialistReview = specialistReviewRequired
    ? await resolveParallelSpecialistCouncil({
        context: input.context,
        objective: originalObjective,
        resumeState: plannedResumeState,
        repositoryImpact,
    ...
    : {
        contract: CODE_AI_PARALLEL_SPECIALIST_REVIEW_CONTRACT,
        status: "DEFERRED_UNTIL_IMPLEMENTATION_OR_HIGH_RISK",
        completed: false,
        required: false,
        failure_reason: null,
        concurrent_dispatch: false,
        ...
        authorization_effect: "NONE",
        execution_effect: "ADVISORY_CONTEXT_ONLY",
        deferred_until_implementation_or_high_risk: true,
        deferred_until_concrete_repository_evidence: hasConcreteRepositoryEvidence !== true,
        raw_reasoning_persisted: false,
      };

  const rawSolutionStrategyCompetition = competeCodeAISolutionStrategies({
    specialist_review: specialistReview,
    repository_impact: repositoryImpact,
    runtime_evidence: [
      ...list(plannedResumeState?.runtime_evidence),
      ...list(plannedResumeState?.objective_context?.runtime_evidence),
      ...list(plannedResumeState?.objective_context?.browser_evidence),
      ...list(plannedResumeState?.objective_context?.trace_evidence),
      ...list(plannedResumeState?.objective_context?.metric_evidence),
    ],
  });
  const solutionStrategyCompetition = filterCodeAIStrategyCompetitionByObservedContracts({
    state: plannedResumeState,
    competition: rawSolutionStrategyCompetition,
  });
  ...
  const strategicObjective = buildCodeAIStrategicObjective({
    ...
    repository_impact: repositoryImpact,
    specialist_review: specialistReview,
    solution_strategy_competition: solutionStrategyCompetition,
    engineering_plan: initialPlan,
  });
  const strategicAdaptiveReasoningBudget = resolveCodeAIAdaptiveReasoningBudget({
    objective: originalObjective,
    state: plannedResumeState,
    repository_impact: repositoryImpact,
    strategy_competition: solutionStrategyCompetition,
  });
  const adaptiveBudgetMayEscalate =
    plannedResumeState?.objective_context?.adaptive_reasoning_budget_applied === true;
  const effectiveReasoningBudget = adaptiveBudgetMayEscalate
    ? Math.max(
        Number(input.reasoning_call_budget || 0),
        Number(strategicAdaptiveReasoningBudget.recommended_reasoning_calls || 0),
      )
    : input.reasoning_call_budget;
  const enrichedStrategyCompetition = {
    ...object(solutionStrategyCompetition),
    strategic_adaptive_reasoning_budget:
      Number(strategicAdaptiveReasoningBudget.recommended_reasoning_calls || 0),
    effective_reasoning_call_budget: Number(effectiveReasoningBudget || 0),
    caller_reasoning_budget_preserved: !adaptiveBudgetMayEscalate,
  };
  const isolatedCandidatePolicy = resolveCodeAIIsolatedCandidateCompetitionPolicy({
    risk: repositoryImpact?.risk,
    strategyCompetition: enrichedStrategyCompetition,
  });
  const isolatedCandidateCompetition = await runCodeAIIsolatedCandidateCompetition({
    policy: isolatedCandidatePolicy,
    strategy_competition: enrichedStrategyCompetition,
    input: {
      ...input,
      objective: strategicObjective,
      reasoning_call_budget: effectiveReasoningBudget,
    },
    resume_state: {
      ...plannedResumeState,
      parallel_specialist_review: specialistReview,
      solution_strategy_competition: enrichedStrategyCompetition,
    },
  });
  const selectedCandidateResult = object(isolatedCandidateCompetition?.winner_result);
  const trialReasoningCallsConsumed = Number(
    isolatedCandidateCompetition?.reasoning_calls_consumed || 0,
  );
  const reasoningCallsBeforeCompetition = Number(
    plannedResumeState?.work_package_control?.reasoning_calls_used || 0,
  );
  const parentReasoningCallsAfterCompetition =
    reasoningCallsBeforeCompetition + trialReasoningCallsConsumed;
  const competitionReceipt = {
    ...object(isolatedCandidateCompetition),
    winner_result: undefined,
  };
  const parentWorkPackageControl = {
    ...object(plannedResumeState?.work_package_control),
    reasoning_call_budget: Number(effectiveReasoningBudget || 0),
    reasoning_calls_used: parentReasoningCallsAfterCompetition,
  };
  const result = Object.keys(selectedCandidateResult).length
    ? {
        ...selectedCandidateResult,
        state: {
          ...object(selectedCandidateResult.state),
          mission_id: plannedResumeState?.mission_id || selectedCandidateResult?.state?.mission_id || null,
          work_package_control: {
            ...object(selectedCandidateResult?.state?.work_package_control),
            ...parentWorkPackageControl,
          },
          isolated_candidate_competition: competitionReceipt,
        },
      }
    : await executeBatchedAutonomousCodeMissionWithDeterministicConvergence({
        ...input,
        objective: strategicObjective,
        reasoning_call_budget: effectiveReasoningBudget,
        resume_state: {
          ...plannedResumeState,
          work_package_control: parentWorkPackageControl,
          isolated_candidate_competition: competitionReceipt,
        },
      });
  ...
  const strategic = strategicMetadata({
    ...
    repositoryImpact,
    specialistReview,
    solutionStrategyCompetition: enrichedStrategyCompetition,
    ...
    state: result?.state
      ? {
          ...
          isolated_candidate_competition:
            result?.state?.isolated_candidate_competition || null,
        }
      : result?.state,
