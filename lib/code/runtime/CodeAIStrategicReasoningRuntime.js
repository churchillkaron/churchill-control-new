...
export function buildCodeAIStrategicObjective({
  ...
  return [goal, protocol, researchContext].filter(Boolean).join("\n\n");
}

function strategicMetadata({ result, resumeState, externalResearch }) {
...
function researchBlockedResult({ originalObjective, resumeState, research }) {
  const state = {
    ...object(resumeState),
