...
const TYPE_TIGHTENING = /\b(?:make|change|convert|tighten)\b.{0,50}\b(?:required|mandatory|non[- ]?optional)\b/i;
...
export function deriveCodeAIObservedStrategyContracts(state = {}) {
  ...
  for (const [path, content] of docs.entries()) {
    ...
    for (const typeContract of list(analysis?.type_contracts)) {
      if (typeContract?.exported !== true) continue;
      for (const property of list(typeContract?.properties)) {
        push({
          kind: property?.optional === true ? "TS_EXPORTED_OPTIONAL_PROPERTY" : "TS_EXPORTED_REQUIRED_PROPERTY",
          token: text(property?.name, 240), path, symbol: text(typeContract?.name, 240), optional: property?.optional === true,
        });
      }
    }
    for (const fn of list(analysis?.function_contracts)) {
      ...
      for (const shape of list(fn?.parameter_object_shapes)) {
        for (const property of list(shape?.properties)) {
          push({ kind: property?.optional === true ? "TS_PARAMETER_OPTIONAL_PROPERTY" : "TS_PARAMETER_REQUIRED_PROPERTY", token: text(property?.name, 240), path, symbol: text(fn?.name, 240), optional: property?.optional === true });
        }
      }
      for (const property of list(fn?.typed_return_shape?.properties)) {
        push({ kind: property?.optional === true ? "TS_RETURN_OPTIONAL_PROPERTY" : "TS_RETURN_REQUIRED_PROPERTY", token: text(property?.name, 240), path, symbol: text(fn?.name, 240), optional: property?.optional === true });
      }
      if (/(^|\/)app\/.*\/route\.(?:js|jsx|ts|tsx)$/i.test(path)) {
        for (const field of list(fn?.response_fields)) push({ kind: "NEXT_ROUTE_RESPONSE_FIELD", token: text(field, 240), path, symbol: text(fn?.name, 240) });
      }
    }
    for (const schema of list(analysis?.schema_references)) {
      const schemaKind = text(schema?.kind, 80);
      if (schemaKind === "table") {
        ...
        for (const key of list(schema?.filter_keys)) push({ kind: "SUPABASE_FILTER_KEY", token: text(key, 240), path, table: text(schema?.name, 500) });
        for (const field of list(schema?.mutation_fields)) push({ kind: "SUPABASE_MUTATION_FIELD", token: text(field, 240), path, table: text(schema?.name, 500) });
      }
      if (schemaKind === "rpc") {
        for (const key of list(schema?.argument_keys)) push({ kind: "SUPABASE_RPC_ARGUMENT_KEY", token: text(key, 240), path, rpc: text(schema?.name, 500) });
      }
    }
  }
  return {
...
function explicitContradictions(direction, obligations) {
  ...
  if (!source || (!DESTRUCTIVE.test(source) && !TYPE_TIGHTENING.test(source))) return [];
  const conflicts = [];
  for (const obligation of obligations) {
    ...
    for (const match of source.matchAll(re)) {
      ...
      const destructive = DESTRUCTIVE.test(evidence);
      const tightening = obligation?.optional === true && TYPE_TIGHTENING.test(evidence);
      if (destructive || tightening) { conflicts.push({ ...obligation, evidence: text(evidence, 240) }); break; }
    }
  }
