...
function analyzeAst(filePath, content) {
  ...
  try {
    ...
    return {
      ...
      imports: [], exports: [], calls: [], schema_references: [], function_contracts: [], result_field_reads: [],
  ...
  const staticReturnFields = (functionNode) => {
    ...
    return unique(fields).slice(0, 80);
  };
  const parameterContract = (params = []) => {
    ...
    for (const param of params) {
      if (!param) continue;
      if (param.type === "RestElement") { hasRest = true; continue; }
      maxArity += 1;
      if (param.type !== "AssignmentPattern") minArity += 1;
      const candidate = param.type === "AssignmentPattern" ? param.left : param;
  ...
  const walked = walkAst(ast, (node) => {
    ...
    if (node.type === "ExportNamedDeclaration") {
      ...
      if (decl?.type === "FunctionDeclaration" && decl.id?.name && functionContracts.length < MAX_FUNCTION_CONTRACTS) {
        ...
        const entry = { name: decl.id.name, exported: true, ...shape, return_fields: staticReturnFields(decl) };
    ...
    if (node.type === "FunctionDeclaration" && node.id?.name && !functionContractByName.has(node.id.name) && functionContracts.length < MAX_FUNCTION_CONTRACTS) {
      ...
      const entry = { name: node.id.name, exported: false, ...shape, return_fields: staticReturnFields(node) };
      functionContracts.push(entry);
    ...
    if (callee?.type === "Identifier") {
      ...
      if (member === "select" && schemaReferences.length < MAX_SCHEMA_REFS) {
        const table = findFromTable(callee.object);
        const selected = literalString(node.arguments?.[0]);
        const columns = selected && selected !== "*"
          ? unique(selected.split(",").map((item) => item.trim().split(/[(!:]/)[0].trim()).filter(Boolean)).slice(0, 80)
          : [];
        if (table && columns.length) {
          const existing = schemaReferences.find((item) => item.kind === "table" && item.name === table && item.relation === "references_database_table");
          if (existing) existing.selected_columns = unique([...(existing.selected_columns || []), ...columns]);
          else schemaReferences.push({ kind: "table", name: table, relation: "references_database_table", selected_columns: columns });
        }
      }
...
export function analyzeCodeAISourceDependencies(filePath, content) {
  ...
  if (/\.sql$/i.test(path)) {
    return {
      ...
      syntax_aware: false,
      parsed: true,
      imports: [], exports: [], calls: [], function_contracts: [], result_field_reads: [],
