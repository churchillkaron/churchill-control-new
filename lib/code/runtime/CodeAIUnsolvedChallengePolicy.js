...
function latestPlannerBlock(state = {}) {
  return [...list(state?.evidence)]
    .reverse()
    .find((entry) =>
      text(entry?.kind, 120) === "autonomous_planner" &&
      text(entry?.decision?.action, 80).toLowerCase() === "block",
    ) || null;
}

function terminalConstraintFromText(value) {
  const source = text(value, 5000);
  const fundamental = source.match(
    /FUNDAMENTAL_CONSTRAINT\s*:\s*(mathematical_constraint|physical_constraint)\s*:/i,
  );
  if (fundamental) {
    return {
      class: "fundamental",
      constraint_type: fundamental[1].toLowerCase(),
    };
  }
  const external = source.match(
    /EXTERNAL_CONSTRAINT\s*:\s*(policy_or_law_constraint|external_dependency_constraint|environment_constraint|resource_constraint)\s*:/i,
  );
  if (external) {
    return {
      class: "external",
      constraint_type: external[1].toLowerCase(),
    };
  }
  return null;
}
...
export function assessCodeAIBlockDecision({ state = {}, decision = {} } = {}) {
  ...
  const blockers = [];
  const observed = new Set(observedOperationIds(state));
...
export function assessCodeAIBlockedMission({ state = {}, result = {} } = {}) {
  const plannerBlock = latestPlannerBlock(state);
  const statement = [
    text(plannerBlock?.decision?.description, 2400),
    text(plannerBlock?.decision?.reason, 2400),
    text(result?.reason, 2400),
  ].filter(Boolean).join(" ");
  const terminal = terminalConstraintFromText(statement);
  const evidenceIds = observedOperationIds(state);
  const blockers = [];

  if (!terminal) blockers.push("CODE_AI_BLOCK_TERMINAL_CONSTRAINT_NOT_PROVEN");
  if (!evidenceIds.length) blockers.push("CODE_AI_BLOCK_OBSERVED_EVIDENCE_REQUIRED");
  if (terminal?.class === "fundamental" && evidenceIds.length < 2) {
    blockers.push("CODE_AI_BLOCK_FUNDAMENTAL_INDEPENDENT_EVIDENCE_REQUIRED");
  }
  if (/\bimpossible\b/i.test(statement) && terminal?.class !== "fundamental") {
    blockers.push("CODE_AI_BLOCK_UNSUPPORTED_IMPOSSIBLE_CLAIM");
  }

  const accepted = blockers.length === 0 && Boolean(terminal);
  return {
    contract: CONTRACT,
    accepted,
    disposition: accepted
      ? terminal.class === "fundamental"
        ? "fundamental_constraint_proven"
        : "blocked_by_external_constraint"
      : "unsolved_continue_exploration",
    constraint_class: terminal?.class || null,
    constraint_type: terminal?.constraint_type || null,
    evidence_operation_ids: evidenceIds.slice(-20),
    blockers,
    required_exploration_action: accepted ? null : "challenge_assumptions_research_experiment_or_alternative",
    terminal_format_required:
      "FUNDAMENTAL_CONSTRAINT:<mathematical_constraint|physical_constraint>:... or EXTERNAL_CONSTRAINT:<policy_or_law_constraint|external_dependency_constraint|environment_constraint|resource_constraint>:...",
    principle:
      "Not previously done, unknown, difficult, expensive, architectural failure, implementation failure, and failed attempts are UNSOLVED states, not proof of impossibility.",
    authorization_effect: "NONE",
  };
}
...
export const CodeAIUnsolvedChallengePolicy = Object.freeze({
  ...
  assessBlockedMission: assessCodeAIBlockedMission,
