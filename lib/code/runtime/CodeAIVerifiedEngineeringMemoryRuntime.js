...
import {
  ...
  CODE_AI_CONTRACT_FAILURE_LEARNING_CONTRACT,
} from "./CodeAIContractFailureLearningRuntime.js";
import {
...
function safeMatch(session = {}, detail = null, utility = null) {
  ...
  return {
    ...
    failures: list(source.failures).slice(-20).map((entry) => ({
      message: text(entry?.message, 800) || null,
      contract_violations: list(entry?.contract_violations).slice(0, 12).map((violation) => ({
    ...
    negative_engineering_lessons: [
      ...list(source.failures)
        .map((entry) => text(entry?.message, 800))
        .filter(Boolean)
        .map((message) => `Previously observed failure: ${message}`),
      ...(text(source?.strategy_decision?.rejected_direction, 1600)
        ? [`Previously rejected strategy: ${text(source.strategy_decision.rejected_direction, 1600)}`]
        : []),
    ].slice(0, 8),
    prior_strategy_decision: {
      selected_direction: text(source?.strategy_decision?.selected_direction, 1600) || null,
      rejected_direction: text(source?.strategy_decision?.rejected_direction, 1600) || null,
      selection_margin: Number(source?.strategy_decision?.selection_margin || 0),
    },
...
function formatVerifier(entry = {}) {
  return codeAIVerifierDisplay(entry);
}
...
export function formatCodeAIVerifiedEngineeringMemoryForObjective(memory = {}) {
  ...
  for (const match of matches) {
    ...
    if (match.negative_engineering_lessons?.length) {
      lines.push(
        `  prior negative evidence to avoid or re-check: ${match.negative_engineering_lessons.join(" | ")}`,
      );
    }
...
export async function retrieveCodeAIVerifiedEngineeringMemory({
  ...
  const contractFailureLearning = deriveRecurringCodeAIContractFailureLessons(matches);
  return {
    contract: CODE_AI_VERIFIED_ENGINEERING_MEMORY_CONTRACT,
    evaluated: true,
    matches,
    recurring_contract_failure_learning: contractFailureLearning,
    recurring_contract_failure_lessons: list(contractFailureLearning.negative_engineering_lessons),
    contract_failure_learning_contract: CODE_AI_CONTRACT_FAILURE_LEARNING_CONTRACT,
