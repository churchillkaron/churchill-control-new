...
export function objectiveRequiresImplementation(value) {
...
function repairMalformedApplyFilesFileBoundary(value) {
  const raw = text(value, MAX_PLANNER_OUTPUT_CHARS);
  const missingOperationBoundary = raw.replace(
    /"\}\]\}\],"verify":/g,
    '"}]}}],"verify":',
  );
  if (missingOperationBoundary !== raw) {
    for (const candidate of [
      missingOperationBoundary,
      missingOperationBoundary.endsWith("}}")
        ? missingOperationBoundary.slice(0, -1)
        : null,
    ].filter(Boolean)) {
      try {
        const parsed = JSON.parse(candidate);
        ...
        // Continue with the legacy deterministic boundary repairs below.
      }
    }
  }
...
export function parseCodeAIWorkPackage(value, {
  ...
  let verificationIndexes = operations
    ...
    .filter((index) => index >= 0);
  let authoritativeVerificationIndexes = [];
  if (authoritative_verification?.command) {
    const authoritativeCommand = text(authoritative_verification.command, 300);
    const authoritativeArgs = list(authoritative_verification.args)
      .slice(0, 24)
      .map((item) => text(item, 500));
    authoritativeVerificationIndexes = verificationIndexes.filter((index) => {
      const input = object(operations[index]?.input);
      const commandMatches = text(input.command, 300) === authoritativeCommand;
      const args = list(input.args).slice(0, 24).map((item) => text(item, 500));
      return commandMatches &&
        args.length === authoritativeArgs.length &&
        args.every((item, argIndex) => item === authoritativeArgs[argIndex]);
    });
    for (const index of authoritativeVerificationIndexes) {
      controllerNormalizations.push({
        kind: "ADOPT_MATCHING_VERIFY_AS_CONTROLLER_AUTHORITATIVE",
        package_index: index + 1,
        authorization_effect: "NONE",
      });
    }
  ...
  if (mutationIndexes.length) {
    ...
    const acceptedVerificationIndexes = authoritative_verification?.command
      ? authoritativeVerificationIndexes
      : verificationIndexes;
    if (!acceptedVerificationIndexes.some((index) => index > lastMutation)) {
      const promotableRunIndex = authoritative_verification?.command
        ? -1
      ...
      if (promotableRunIndex >= 0) {
        ...
        authoritativeVerificationIndexes = [...authoritativeVerificationIndexes, controllerVerifyIndex];
        controllerNormalizations.push({
    ...
    const finalAcceptedVerificationIndexes = authoritative_verification?.command
      ? authoritativeVerificationIndexes
      : verificationIndexes;
    if (!finalAcceptedVerificationIndexes.some((index) => index > lastMutation)) {
      throw new Error("CODE_AI_WORK_PACKAGE_MUTATION_REQUIRES_LATER_VERIFICATION");
    }
...
function completedReadEvidencePath(entry) {
  const result = object(entry?.result);
  if (result.expected_new_target === true && result.allowed_edit_target === true) {
    return text(result.requested_path, 1000);
  }
...
function completedReadPaths(state) {
  return new Set(
    allCompletedReadEvidence(state)
      .map((entry) => completedReadEvidencePath(entry))
      .filter(Boolean),
  );
}
...
function compactDeclaredEvidenceReads(state) {
  ...
  for (const entry of allCompletedReadEvidence(state)) {
    const filePath = completedReadEvidencePath(entry);
  ...
  return new Set(
    list(workPackage?.controller_normalizations)
      .filter((entry) => [
        ...
        "ADOPT_MATCHING_VERIFY_AS_CONTROLLER_AUTHORITATIVE",
