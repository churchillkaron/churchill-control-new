...
export function resolveCodeAIWorkPackageActionPolicy({
  ...
  const repairState = Boolean(compact.latest_failed_verification) || compact.current_source_changes.length > 0;
...
function packagePrompt({ objective, objectiveContext, state, callNumber, budget }) {
  ...
  const repairGuidance = compact.latest_failed_verification
    ? [
        ...
        "Do not spend another reasoning call on read/search; those actions are disabled during repair.",
      ].join(" ")
    : null;
  const phaseGuidance = actionPolicy.discovery_locked
    ? [
        "DISCOVERY IS COMPLETE AND LOCKED FOR THIS CALL.",
        "The computer has already loaded every declared evidence path required for this bounded mission.",
        "Do not return search, read, inspection, commentary, or another request for context.",
        "Return implementation now. The first model-supplied operation must be apply_files containing all coherent source edits together.",
        "The deterministic controller will append the authoritative verification command if you omit it, and will append the final diff review if you omit it.",
      ].join(" ")
  ...
  const outputExample = actionPolicy.discovery_locked
    ? `Return exactly one JSON object shaped like: {"contract":"${CODE_AI_WORK_PACKAGE_CONTRACT}","phase":"implementation","summary":"coherent implementation","operations":[{"action":"apply_files","description":"apply the complete coherent repair","input":{"files":[{"path":"relative/path","content":"complete final file content"}]}},{"action":"verify","description":"run the authoritative verification","input":{"command":"node","args":["path/to/test.mjs"]}},{"action":"diff","description":"review the final diff","input":{}}]}`
  ...
  const instruction = [
    ...
    "If the mission states an authoritative verification command and your mutation package omits verification, the deterministic controller may append that exact verification without another reasoning call.",
    ...
    `Maximum model-supplied operations in one package: ${MAX_PACKAGE_OPERATIONS}. Controller-owned finalization may add deterministic verification and diff operations.`,
    outputExample,
...
function plannerExecutionInput({ context, objective, objectiveContext, state, callNumber, budget }) {
  ...
  return {
    ...
    metadata: {
      ...
      code_ai_controller_authoritative_verify: true,
