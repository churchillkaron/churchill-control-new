...
import {
  CODE_AI_ENGINEERING_HOTSPOT_PREFLIGHT_CONTRACT,
  projectCodeAIEngineeringHotspotPreflight,
  unavailableCodeAIEngineeringHotspotPreflight,
  formatCodeAIEngineeringHotspotPreflightForObjective,
} from "./CodeAIEngineeringHotspotPreflightRuntime.js";
import {
  deriveCodeAIContractFailurePreflight,
  formatCodeAIContractFailurePreflightForObjective,
  CODE_AI_CONTRACT_FAILURE_PREFLIGHT_CONTRACT,
} from "./CodeAIContractFailurePreflightRuntime.js";
...
function boundedOptionalPreflight(operation, label, timeoutMs = 5000) {
  let timer = null;
  return Promise.race([
    Promise.resolve().then(operation),
    new Promise((_, reject) => {
      timer = setTimeout(() => reject(new Error(`CODE_AI_OPTIONAL_PREFLIGHT_TIMEOUT:${label}`)), timeoutMs);
    }),
  ]).finally(() => {
...
async function resolveVerifiedEngineeringMemory(input, state) {
  ...
  if (object(input?.objective_context?.verified_engineering_memory).contract) {
    return object(input.objective_context.verified_engineering_memory);
  }
  try {
    return await boundedOptionalPreflight(
      () => retrieveCodeAIVerifiedEngineeringMemory({
...
async function resolveEngineeringHotspotPreflight(input, state) {
  if (object(state).engineering_hotspot_preflight?.contract) {
    return object(state).engineering_hotspot_preflight;
  }
  try {
    const { listCodeAIMissionHistory } = await import("./CodeAIMissionHistoryRuntime.js");
    const history = await boundedOptionalPreflight(
      ...
      "engineering_hotspot_history",
    );
    return projectCodeAIEngineeringHotspotPreflight(history?.engineering_hotspot_backlog);
  } catch (error) {
    return unavailableCodeAIEngineeringHotspotPreflight(error?.message || error);
  }
}

function stateWithEngineeringHotspotPreflight(state, preflight) {
  const source = object(state);
  const projection = object(preflight);
  return {
    ...source,
    engineering_hotspot_preflight: projection,
    evidence: [
      ...list(source.evidence),
      {
        at: new Date().toISOString(),
        kind: "engineering_hotspot_preflight",
        contract: CODE_AI_ENGINEERING_HOTSPOT_PREFLIGHT_CONTRACT,
        status: projection.active === true ? "recurring_hotspots_bound" : "no_recurring_hotspots_bound",
        hotspot_count: Number(projection.hotspot_count || 0),
...
function objectiveWithEngineeringHotspotPreflight(objective, preflight) {
  const context = formatCodeAIEngineeringHotspotPreflightForObjective(preflight);
  return [text(objective, 10000), context].filter(Boolean).join("\n\n");
}
...
async function resolveEngineeringSkills(input, state) {
  ...
  try {
    const derived = await boundedOptionalPreflight(
      () => deriveCodeAIEngineeringSkills({
        context: object(input.context),
        objective: input.objective,
        repositoryUrl: input.repository_url,
...
function stateWithEngineeringMemory(state, memory) {
  ...
  return {
    ...
    verified_engineering_memory: {
      ...
      matches,
      recurring_contract_failure_learning: object(memory?.recurring_contract_failure_learning),
      recurring_contract_failure_lessons: list(memory?.recurring_contract_failure_lessons).slice(0, 8),
      utility_contract:
...
function stateWithContractFailurePreflight(state, preflight) {
  const source = object(state);
  const value = object(preflight);
  return {
    ...source,
    contract_failure_preflight: value,
    evidence: [
      ...list(source.evidence),
      {
        at: new Date().toISOString(),
        kind: "contract_failure_preflight",
        contract: CODE_AI_CONTRACT_FAILURE_PREFLIGHT_CONTRACT,
        status: value.active === true ? "recurring_contract_checks_bound" : "no_recurring_contract_checks",
        check_count: Number(value.check_count || 0),
        evidence_ready_count: Number(value.evidence_ready_count || 0),
        evidence_incomplete_count: Number(value.evidence_incomplete_count || 0),
...
function objectiveWithContractFailurePreflight(objective, preflight) {
  const formatted = formatCodeAIContractFailurePreflightForObjective(preflight);
  return [text(objective, 10000), formatted].filter(Boolean).join("\n\n");
}
...
function stateWithAppliedOwnerStop(state, intervention) {
  const source = object(state);
  const now = new Date().toISOString();
  return {
    ...source,
    ...
    owner_intervention: {
      ...object(source.owner_intervention),
      id: intervention?.id || source?.owner_intervention?.id || null,
      contract: text(intervention?.contract, 180) || CODE_AI_OWNER_INTERVENTION_CONTRACT,
      lifecycle_contract: text(intervention?.lifecycle_contract, 180) || CODE_AI_OWNER_INTERVENTION_LIFECYCLE_CONTRACT,
      action: "STOP",
      instruction: null,
      status: "APPLIED",
      fresh_reasoning_required: false,
    ...
    evidence: [
      ...list(source.evidence),
      {
        ...
        lifecycle_contract: text(intervention?.lifecycle_contract, 180) || CODE_AI_OWNER_INTERVENTION_LIFECYCLE_CONTRACT,
        action: "STOP",
        status: "owner_stop_applied_at_safe_boundary",
        authorization_effect: "REDUCE_EXECUTION_ONLY",
        commit_authority: false,
        production_deploy_authority: false,
        source_mutation_performed: false,
...
export async function executeBatchedAutonomousCodeMission(input = {}) {
  ...
  const hotspotsAlreadyBound = Boolean(resumeState.engineering_hotspot_preflight?.contract);
  const [memory, skills, hotspotPreflight] = await Promise.all([
    resolveVerifiedEngineeringMemory(input, resumeState),
    resolveEngineeringSkills(input, resumeState),
    resolveEngineeringHotspotPreflight(input, resumeState),
  ]);
  let workingState = memoryAlreadyBound
    ? resumeState
    : stateWithEngineeringMemory(resumeState, memory);
  const contractFailurePreflightAlreadyBound = Boolean(workingState.contract_failure_preflight?.contract);
  const contractFailurePreflight = contractFailurePreflightAlreadyBound
    ? object(workingState.contract_failure_preflight)
    : deriveCodeAIContractFailurePreflight({ state: workingState });
  if (!contractFailurePreflightAlreadyBound) {
    workingState = stateWithContractFailurePreflight(workingState, contractFailurePreflight);
  }
  if (!skillsAlreadyBound) {
    workingState = stateWithEngineeringSkills(workingState, skills);
  }
  if (!hotspotsAlreadyBound) {
    workingState = stateWithEngineeringHotspotPreflight(workingState, hotspotPreflight);
  }
  let workingObjective = memoryAlreadyBound
    ? text(input.objective, 12000)
    : objectiveWithEngineeringMemory(input.objective, memory);
  if (!contractFailurePreflightAlreadyBound) {
    workingObjective = objectiveWithContractFailurePreflight(workingObjective, contractFailurePreflight);
  }
  if (!skillsAlreadyBound) {
    workingObjective = objectiveWithEngineeringSkills(workingObjective, skills);
  }
  if (!hotspotsAlreadyBound) {
    workingObjective = objectiveWithEngineeringHotspotPreflight(workingObjective, hotspotPreflight);
  }
  ...
  if (text(intervention?.action, 80) === "STOP" && intervention?.claim_id) {
    const appliedStop = await applyClaimedCodeAIOwnerStop({
      context: object(input.context),
      missionId,
      interventionId: intervention.id,
      claimId: intervention.claim_id,
    });
    if (appliedStop?.applied !== true || !appliedStop?.intervention) {
      return ownerInterventionReconciliationBlocked(
        workingState,
        "CODE_AI_OWNER_STOP_APPLY_RECONCILIATION_REQUIRED",
      );
    }
    const stoppedState = stateWithAppliedOwnerStop(workingState, appliedStop.intervention);
    await publishCodeAILiveProgress({
      ...
      event: {
        ...
        status: "stopped",
        mission_id: missionId,
        description: "Code stopped at the next governed safe boundary on owner request.",
        reason: "CODE_AI_OWNER_STOP_REQUESTED",
        files_changed: list(stoppedState.files_changed),
      },
    }).catch((error) => {
      console.error(JSON.stringify({
        event: "AVANTIQO_CODE_OWNER_STOP_PROGRESS_PUBLISH_FAILED",
        mission_id: missionId,
        reason: text(error?.message || error, 500),
        mission_already_stopped: true,
        authorization_effect: "NONE",
      }));
    });
    return {
  ...
  const result = {
    ...
    engineering_hotspot_preflight:
      strategicResult?.state?.engineering_hotspot_preflight || workingState.engineering_hotspot_preflight || null,
    contract_failure_preflight:
      strategicResult?.state?.contract_failure_preflight || workingState.contract_failure_preflight || null,
  };
...
export const CodeAIWorkPackageRuntime = Object.freeze({
  ...
  engineering_hotspot_preflight_contract: CODE_AI_ENGINEERING_HOTSPOT_PREFLIGHT_CONTRACT,
  recurring_hotspots_inform_first_pass_strategy: true,
  engineering_hotspot_current_head_revalidation_required: true,
  engineering_hotspot_historical_patch_replay_allowed: false,
  engineering_hotspot_mutation_authority: false,
  contract_failure_preflight_contract: CODE_AI_CONTRACT_FAILURE_PREFLIGHT_CONTRACT,
  recurring_contract_failures_inform_first_reasoning_pass: true,
  contract_failure_preflight_model_call_performed: false,
  contract_failure_preflight_mutation_authority: false,
