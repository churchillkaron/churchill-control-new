...
function plannerInput({ context, objective, objectiveContext, state, callNumber, budget, developerAttachments = [] }) {
  ...
  return {
    ...
    input: {
      ...
      temperature: plannerOutputRepair.compact_json_only === true ? 0 : 0.1,
      max_output_tokens: plannerOutputRepair.compact_json_only === true ? 1400 : 4096,
      structured_specification: {
        ...
        planner_output_repair_required: plannerOutputRepair.compact_json_only === true,
...
export async function executeBatchedAutonomousCodeMissionLive({
  ...
  try {
    ...
    const reason = text(error?.message || error, 2000);
    if (reason === "CODE_AI_WORK_PACKAGE_JSON_INVALID" && control.reasoning_calls_used < control.reasoning_call_budget) {
      ...
      await safeProgress(context, state, {
        phase: "PLANNER_OUTPUT_REPAIR_REQUIRED",
        status: "running",
        mission_id: state.mission_id,
        reasoning_call: callNumber,
        description: "Planner output was structurally invalid; requesting a compact valid JSON repair package on the next bounded reasoning call.",
        reason,
        files_changed: list(state.files_changed),
      });
      return {
