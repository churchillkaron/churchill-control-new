...
function promptTransport({ objective, objectiveContext, state, callNumber, budget }) {
  ...
  const phaseGuidance = actionPolicy.implementation_required
    ? "DISCOVERY IS LOCKED. Declared evidence is already loaded or the latest verifier failed. Do not ask for more context. Do not search or read. Implement or repair now; the first operation must be apply_files with all coherent edits together."
  ...
  const repairGuidance = repair
    ...
    : null;
  const outputExample = actionPolicy.implementation_required
  ...
  return {
    ...
    input: {
      ...
      structured_specification: {
        ...
        implementation_required: actionPolicy.implementation_required,
        verification_failed: actionPolicy.verification_failed,
        repair_state: actionPolicy.repair_state,
    ...
    metadata: {
      ...
      code_ai_implementation_required: actionPolicy.implementation_required,
