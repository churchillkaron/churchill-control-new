...
function plannerInput({
  ...
  const focusedRepairTargetPath = undefinedSymbolRepair?.target_path
    ...
    || (plannerOutputRepair.compact_json_only === true && !multiFileStructuredRepair
  ...
  const plannerOutputRepairInstruction = plannerOutputRepair.known_missing_path_repeat === true
    ...
    ? [multiFileStructuredRepair
        ? `PLANNER OUTPUT REPAIR: The previous planner response was structurally invalid or truncated JSON while attempting the first coherent multi-file implementation. Return exactly one valid JSON object matching AVANTIQO_CODE_AI_WORK_PACKAGE_V1 with no markdown fence or commentary. Preserve the original coherent implementation across exactly these remaining controller-declared files: ${JSON.stringify(remainingAllowedEditPaths)}. Use one apply_files operation containing complete final contents for all required files; controller-owned authoritative verification and diff may follow. Do not add unrelated files or discovery operations.`
...
export async function executeBatchedAutonomousCodeMissionLive({
  ...
  const objectiveContext = normalizedObjectiveContext({
    ...
    owner_objective: text(suppliedObjectiveContext.owner_objective, 12000) || goal,
  });
