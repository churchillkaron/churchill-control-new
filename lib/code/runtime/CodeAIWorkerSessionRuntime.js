...
export const CODE_AI_WORKER_SESSION_CONTRACT =
  "AVANTIQO_CODE_AI_WORKER_SESSION_V1";

const MEMORY_TABLE = "intelligence_memories";
const MEMORY_SCOPE = "code_ai_worker_session";
const MEMORY_KEY = "code_ai_worker_session:v1:shared";
...
function memoryRecord(session, existing = null) {
  ...
  return {
    ...
    content:
      "Server-owned Code worker session control state. This state is excluded from ordinary Intelligence recall and contains no Pod bearer token.",
    importance: 0.05,
    confidence: 1,
...
async function assertNoForeignCodePod(sessionId = null) {
  ...
  if (foreign.length) {
    throw new Error("CODE_AI_WORKER_SESSION_SHARED_VOLUME_BUSY");
  }
}
...
async function health(session) {
  ...
  try {
    ...
    return { ready: false, reason: text(error?.name || error?.message, 120) || "HEALTH_PENDING" };
  }
}
...
async function expireSession(row, session, reason) {
  await deletePod(session?.pod_id);
  ...
  await updateSession(row, expiredSession).catch(() => null);
...
export async function reapExpiredCodeAIWorkerSession() {
  ...
  if (!row || !session || !activeState(session) || !expired(session)) {
    return { reaped: false, session_state: session?.state || null };
  }
  await expireSession(row, session, "IDLE_EXPIRED");
...
export async function ensureCodeAIWorkerSession({ idle_ms = DEFAULT_IDLE_MS } = {}) {
  ...
  if (activeState(session) && expired(session)) {
    await expireSession(row, session, "IDLE_EXPIRED");
    claim = await claimSession(idleMs);
    row = claim.row;
...
export async function resolveCodeAIWorkerSessionTransport() {
  const row = await loadControlRow();
  const session = sessionFromRow(row);
  if (!session || session.state !== "READY" || expired(session)) {
    return null;
  }
