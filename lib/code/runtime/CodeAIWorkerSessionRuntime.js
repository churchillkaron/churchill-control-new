...
const IMAGE =
  "ghcr.io/churchillkaron/avantiqo-code-pod@sha256:daba714fde0b149cb82d779a3a114fd10e701de03722a36b1e2041c4adc19b3e";
...
const READY_TRANSPORT_HEALTH_TIMEOUT_MS = 5000;
const READY_TRANSPORT_HEALTH_ATTEMPTS = 3;
const READY_TRANSPORT_HEALTH_RETRY_DELAY_MS = 500;
const POD_CONTROL_TIMEOUT_MS = 30_000;
const POD_LIFECYCLE_REFRESH_MS = 5000;
const REQUIRED_HTTP_PORT = "8000/http";
...
const podLifecycleCache = new Map();
...
function podDesiredStatus(pod) {
  return text(pod?.desiredStatus ?? pod?.desired_status).toUpperCase();
}

function podRuntimeStatus(pod) {
  return text(pod?.status ?? pod?.runtimeStatus).toUpperCase();
}

function activePod(pod) {
  const desired = podDesiredStatus(pod);
  const status = podRuntimeStatus(pod);
  return !TERMINAL_POD.has(desired || status);
}

function podLifecycleSnapshot(pod) {
  const desiredStatus = podDesiredStatus(pod);
  const runtimeStatus = podRuntimeStatus(pod);
  const terminal = TERMINAL_POD.has(desiredStatus) || TERMINAL_POD.has(runtimeStatus);
  const machinePresent = Boolean(pod?.machine || text(pod?.machineId));
  const publicIpPresent = Boolean(text(pod?.publicIp));
  const ports = list(pod?.ports).map((entry) => text(entry)).filter(Boolean);
  const httpPortExposed = ports.includes(REQUIRED_HTTP_PORT);
  const lastStartedAt = text(pod?.lastStartedAt) || null;
  const phase = terminal
    ? "POD_TERMINAL"
    : !machinePresent || !lastStartedAt
      ? "POD_PROVISIONING"
      : !publicIpPresent || !httpPortExposed
        ? "POD_NETWORKING"
        : "POD_HTTP_STARTUP";
  return {
    pod_record_present: true,
    pod_terminal: terminal,
    pod_lifecycle_phase: phase,
    pod_desired_status: desiredStatus || null,
    pod_runtime_status: runtimeStatus || null,
    pod_machine_present: machinePresent,
    pod_public_ip_present: publicIpPresent,
    pod_http_port_exposed: httpPortExposed,
    pod_last_started_at: lastStartedAt,
    pod_last_status_change: text(pod?.lastStatusChange, 500) || null,
  };
}

async function inspectPodLifecycle(session, { force = false } = {}) {
  const podId = text(session?.pod_id);
  if (!podId) {
    return {
      pod_record_present: false,
      pod_terminal: false,
      pod_lifecycle_phase: "POD_ID_PENDING",
      pod_desired_status: null,
      pod_runtime_status: null,
      pod_machine_present: false,
      pod_public_ip_present: false,
      pod_http_port_exposed: false,
      pod_last_started_at: null,
      pod_last_status_change: null,
    };
  }
  const cached = podLifecycleCache.get(podId);
  if (
    !force &&
    cached &&
    Date.now() - cached.checked_at < POD_LIFECYCLE_REFRESH_MS
  ) {
    return cached.value;
  }
  try {
    ...
    const value = pod
      ? podLifecycleSnapshot(pod)
      : {
          pod_record_present: false,
          pod_terminal: false,
          pod_lifecycle_phase: "POD_RECORD_PENDING",
          pod_desired_status: null,
          pod_runtime_status: null,
          pod_machine_present: false,
          pod_public_ip_present: false,
    ...
    podLifecycleCache.set(podId, { checked_at: Date.now(), value });
    return value;
  } catch (error) {
    return {
      pod_record_present: null,
      pod_terminal: false,
      pod_lifecycle_phase: "POD_LIFECYCLE_UNKNOWN",
      pod_desired_status: null,
      pod_runtime_status: null,
      pod_machine_present: null,
      pod_public_ip_present: null,
      pod_http_port_exposed: null,
      pod_last_started_at: null,
      pod_last_status_change: null,
      pod_lifecycle_reason: text(error?.message || error, 300) || "RUNPOD_MANAGEMENT_STATUS_UNAVAILABLE",
    };
  }
}
...
async function deletePodVerified(podId) {
  ...
  podLifecycleCache.delete(text(podId));
...
async function health(session, { timeout_ms = HEALTH_TIMEOUT_MS } = {}) {
  const baseUrl = text(session?.pod_base_url);
  if (!baseUrl) {
    const lifecycle = await inspectPodLifecycle(session);
    return {
      ready: false,
  ...
  try {
    ...
    if (!response.ok) {
      const lifecycle = await inspectPodLifecycle(session);
      const terminalStatus = lifecycle.pod_desired_status || lifecycle.pod_runtime_status || "UNKNOWN";
      return {
        ready: false,
        transport_ready: false,
        engine_loaded: false,
        engine_loading: false,
        pod_proxy_http_status: response.status,
        ...lifecycle,
        reason: lifecycle.pod_terminal === true
          ? `POD_TERMINAL_${terminalStatus}`
          : `HTTP_${response.status}_${lifecycle.pod_lifecycle_phase || "POD_HTTP_STARTUP"}`,
      };
    }
    ...
    return {
      ...
      engine_loading: body?.engine_loading === true,
    ...
    return {
      ...
      reason: lifecycle.pod_terminal === true
        ? `POD_TERMINAL_${terminalStatus}`
        : text(error?.name || error?.message, 120) || "HEALTH_PENDING",
    };
  }
}
...
async function confirmReadyTransportHealth(session) {
  let latest = null;
  for (let attempt = 1; attempt <= READY_TRANSPORT_HEALTH_ATTEMPTS; attempt += 1) {
    latest = await health(session, { timeout_ms: READY_TRANSPORT_HEALTH_TIMEOUT_MS });
    if (latest.engine_loaded === true) return latest;
    if (attempt >= READY_TRANSPORT_HEALTH_ATTEMPTS) break;
    console.error(JSON.stringify({
      event: "AVANTIQO_CODE_READY_TRANSPORT_HEALTH_RETRY",
      contract: CODE_AI_WORKER_SESSION_CONTRACT,
      attempt,
      max_attempts: READY_TRANSPORT_HEALTH_ATTEMPTS,
      reason: text(latest?.reason, 160) || "NOT_READY",
      session_state: "READY",
    ...
    await sleep(READY_TRANSPORT_HEALTH_RETRY_DELAY_MS * attempt);
  }
  return latest || {
    ready: false,
    transport_ready: false,
    engine_loaded: false,
    engine_loading: false,
    reason: "READY_TRANSPORT_HEALTH_UNAVAILABLE",
  };
}
...
function warmingResult(session, readiness, reason) {
  return {
    ...
    engine_loading: readiness?.engine_loading === true,
    pod_record_present: readiness?.pod_record_present ?? null,
    pod_terminal: readiness?.pod_terminal === true,
    pod_lifecycle_phase: text(readiness?.pod_lifecycle_phase, 120) || null,
    pod_desired_status: text(readiness?.pod_desired_status, 120) || null,
    pod_runtime_status: text(readiness?.pod_runtime_status, 120) || null,
    pod_machine_present: readiness?.pod_machine_present ?? null,
    pod_public_ip_present: readiness?.pod_public_ip_present ?? null,
    pod_http_port_exposed: readiness?.pod_http_port_exposed ?? null,
    pod_last_started_at: text(readiness?.pod_last_started_at, 160) || null,
    pod_last_status_change: text(readiness?.pod_last_status_change, 500) || null,
    pod_proxy_http_status: Number.isFinite(Number(readiness?.pod_proxy_http_status))
      ? Number(readiness.pod_proxy_http_status)
      : null,
...
export async function ensureCodeAIWorkerSession({ idle_ms = DEFAULT_IDLE_MS } = {}) {
  ...
  if (!readiness.transport_ready) {
    if (readiness.pod_terminal === true) {
      const terminalStatus = readiness.pod_desired_status || readiness.pod_runtime_status || "UNKNOWN";
      await expireSession(row, session, `POD_TERMINAL_${terminalStatus}`);
      throw new Error(`CODE_AI_WORKER_SESSION_POD_TERMINAL:${terminalStatus}`);
    }
  ...
  return {
    ...
    engine_loaded: true,
    engine_loading: false,
    pod_record_present: finalReadiness.pod_record_present ?? null,
    pod_terminal: false,
    pod_lifecycle_phase: "POD_HTTP_READY",
    pod_desired_status: text(finalReadiness.pod_desired_status, 120) || null,
    pod_runtime_status: text(finalReadiness.pod_runtime_status, 120) || null,
    pod_machine_present: finalReadiness.pod_machine_present ?? null,
    pod_public_ip_present: finalReadiness.pod_public_ip_present ?? null,
    pod_http_port_exposed: finalReadiness.pod_http_port_exposed ?? null,
    pod_last_started_at: text(finalReadiness.pod_last_started_at, 160) || null,
    pod_last_status_change: text(finalReadiness.pod_last_status_change, 500) || null,
    pod_proxy_http_status: Number.isFinite(Number(finalReadiness.pod_proxy_http_status))
      ? Number(finalReadiness.pod_proxy_http_status)
      : 200,
    engine_warmup_job_present: Boolean(session.engine_warmup_job_id),
    engine_warmup_status: text(session.engine_warmup_status, 80) || null,
    reasoning_call_consumed_by_warmup: false,
...
export async function resolveCodeAIWorkerSessionTransport() {
  ...
  const readiness = await confirmReadyTransportHealth(session);
