...
const CONTRACT = "AVANTIQO_CODE_AI_WORLDCLASS_COMMIT_GUARD_V5";
...
export function assertCodeAIWorldClassCommitReady(state = {}) {
  ...
  if (testProvenance.required === true && testProvenance.verified !== true) {
    throw new Error(
      `CODE_AI_COMMIT_INDEPENDENT_TEST_PROVENANCE_REQUIRED:risk=${testProvenance.risk}:changed_tests=${testProvenance.changed_test_path_count}:unchanged_impacted_tests=${testProvenance.unchanged_matched_impacted_test_count}`,
    );
  }

  const repairClosure = assessCodeAIRepairClosure(state);
  if (repairClosure.required === true && repairClosure.verified !== true) {
    throw new Error(
      `CODE_AI_COMMIT_FAILED_VERIFIER_CLOSURE_REQUIRED:unresolved=${repairClosure.unresolved_failed_verifier_count}`,
    );
  }
  ...
  if (strategicReview.required === true && strategicReview.verified !== true) {
    throw new Error(
      `CODE_AI_COMMIT_STRATEGIC_REVIEW_REQUIRED:${strategicReview.review_status || "NOT_VERIFIED"}`,
    );
  }
  ...
  return {
    ...
    verification_families: list(quality.fresh_verification_families).map((item) => text(item)).filter(Boolean),
    ...
    independent_test_provenance_required: testProvenance.required === true,
    independent_test_provenance_verified: testProvenance.verified === true,
    independent_test_provenance_trust_basis: testProvenance.trust_basis,
    independent_test_provenance_changed_test_count:
      Number(testProvenance.changed_test_path_count || 0),
    independent_test_provenance_unchanged_impacted_test_count:
      Number(testProvenance.unchanged_matched_impacted_test_count || 0),
    repair_closure_required: repairClosure.required === true,
    repair_closure_verified: repairClosure.verified === true,
    repair_closure_failed_verifier_count:
      Number(repairClosure.failed_verifier_count_before_final_edit || 0),
    repair_closure_unresolved_failed_verifier_count:
      Number(repairClosure.unresolved_failed_verifier_count || 0),
    strategic_review_required: strategicReview.required === true,
    strategic_review_verified: strategicReview.verified === true,
    strategic_review_architecture_performance_present:
      strategicReview.architecture_performance_review_present === true,
    strategic_review_adversarial_risk_present:
      strategicReview.adversarial_risk_review_present === true,
