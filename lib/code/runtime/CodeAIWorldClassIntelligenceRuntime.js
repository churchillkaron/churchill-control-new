...
function pathEvidence(state) {
  ...
  const paths = [
    ...
    ...list(source.declared_evidence_reads).map((entry) => entry?.result?.file_path || entry?.result?.path),
    ...list(source.source_read_evidence).map((entry) => entry?.result?.file_path || entry?.result?.path),
    ...list(source.evidence).map((entry) => entry?.result?.file_path || entry?.result?.path),
  ];
...
function canonicalSchemaName(kind, name) {
  const normalizedKind = text(kind, 80).toLowerCase();
  const normalizedName = text(name, 500);
  if (!normalizedName) return "";
  if (normalizedName.includes(".")) return normalizedName;
  if (["table", "view", "function", "rpc"].includes(normalizedKind)) {
    return `public.${normalizedName}`;
  }
  return normalizedName;
}

function schemaNodeId(kind, name) {
  const normalizedKind = text(kind, 80);
  const normalizedName = canonicalSchemaName(normalizedKind, name);
  return normalizedKind && normalizedName ? `schema:${normalizedKind}:${normalizedName}` : "";
}
...
export function deriveCodeAICausalGraph(state = {}) {
  ...
  const analyses = new Map([...documents.entries()].map(([filePath, content]) => [
    filePath,
    analyzeCodeAISourceDependencies(filePath, content),
  ]));
  const schemaNodeIds = unique([...analyses.values()].flatMap((analysis) =>
    list(analysis?.schema_references).map((entry) => schemaNodeId(entry?.kind, entry?.name))
  )).slice(0, 80);
  const observedPaths = new Set(unique([...paths, ...documents.keys()]));
  const nodes = [
    ...[...observedPaths].slice(0, 100).map((path) => ({
      id: path,
      kind: changed.has(path) ? "changed" : "evidence",
      source_content_observed: documents.has(path),
    })),
    ...schemaNodeIds.map((id) => ({ id, kind: "schema", source_content_observed: false })),
  ].slice(0, 160);
  const boundedFilePaths = new Set(nodes.filter((node) => node.kind !== "schema").map((node) => node.id));
  const boundedNodeIds = new Set(nodes.map((node) => node.id));
  const edges = [];
  const seenEdges = new Set();
  const exportsByPath = new Map([...analyses.entries()].map(([filePath, analysis]) => [
    filePath,
    new Set(analysis?.parsed === true && list(analysis?.exports).length
      ? list(analysis.exports)
      : exportedSymbols(documents.get(filePath))),
  ]));
  let staticImportEdgeCount = 0;
  let symbolCallEdgeCount = 0;
  let schemaDependencyEdgeCount = 0;
  let astImportEdgeCount = 0;
  let astSymbolCallEdgeCount = 0;

  for (const [fromPath, content] of documents.entries()) {
    if (!boundedFilePaths.has(fromPath)) continue;
    const analysis = object(analyses.get(fromPath));
    const parsedImports = analysis.parsed === true
      ? list(analysis.imports).map((entry) => text(entry?.source, 1200)).filter((specifier) => /^\.{1,2}\//.test(specifier))
      : [];
    const importSpecifiers = parsedImports.length ? unique(parsedImports) : relativeImportSpecifiers(content);
    for (const specifier of importSpecifiers) {
      const resolved = resolveObservedImport(fromPath, specifier, boundedFilePaths);
      ...
      edges.push({
        ...
        evidence: specifier,
        syntax_evidence: parsedImports.length ? "AST" : "FALLBACK",
      });
      staticImportEdgeCount += 1;
      if (parsedImports.length) astImportEdgeCount += 1;
      if (edges.length >= 260) break;
    }
    if (edges.length >= 260) break;

    const astCalls = analysis.parsed === true ? list(analysis.calls) : [];
    if (astCalls.length) {
      for (const call of astCalls) {
        const specifier = text(call?.source, 1200);
        if (!/^\.{1,2}\//.test(specifier)) continue;
        const resolved = resolveObservedImport(fromPath, specifier, boundedFilePaths);
        if (!resolved) continue;
        const targetSymbol = text(call?.imported, 240);
        const local = text(call?.local, 240);
        const key = `call:${fromPath}->${resolved}:${targetSymbol}:${local}`;
        if (seenEdges.has(key)) continue;
        seenEdges.add(key);
        ...
        edges.push({
          ...
          import_kind: text(call?.import_kind, 80),
          import_specifier: specifier,
          target_export_observed: targetSymbol === "default" ? observedExports.has("default") : observedExports.has(targetSymbol),
          observed_argument_count: Number.isInteger(call?.argument_count) ? call.argument_count : null,
          evidence: local ? `${local}(` : targetSymbol,
          syntax_evidence: "AST",
        });
        symbolCallEdgeCount += 1;
        astSymbolCallEdgeCount += 1;
        if (symbolCallEdgeCount >= 100 || edges.length >= 260) break;
      }
    } else {
      for (const binding of importBindings(content)) {
        const resolved = resolveObservedImport(fromPath, binding.specifier, boundedFilePaths);
        if (!resolved) continue;
    ...
    if (analysis.parsed === true) {
      for (const fieldRead of list(analysis.result_field_reads)) {
        const specifier = text(fieldRead?.source, 1200);
        if (!/^\.{1,2}\//.test(specifier)) continue;
        const resolved = resolveObservedImport(fromPath, specifier, boundedFilePaths);
        if (!resolved) continue;
        const targetSymbol = text(fieldRead?.imported, 240);
        const field = text(fieldRead?.field, 240);
        if (!field) continue;
        const key = `result-field:${fromPath}->${resolved}:${targetSymbol}:${field}`;
        if (seenEdges.has(key)) continue;
        seenEdges.add(key);
        edges.push({
          from: fromPath,
          to: resolved,
          relation: "reads_imported_result_field",
          target_symbol: targetSymbol,
          result_field: field,
          import_specifier: specifier,
          syntax_evidence: "AST",
    ...
    for (const schemaRef of list(analysis.schema_references)) {
      const target = schemaNodeId(schemaRef?.kind, schemaRef?.name);
      if (!boundedNodeIds.has(target)) continue;
      const relation = text(schemaRef?.relation, 120) || "references_schema_object";
      const key = `schema:${fromPath}->${target}:${relation}`;
      if (seenEdges.has(key)) continue;
      seenEdges.add(key);
      edges.push({
        from: fromPath,
        to: target,
        relation,
        schema_kind: text(schemaRef?.kind, 80),
        schema_name: canonicalSchemaName(schemaRef?.kind, schemaRef?.name),
        selected_columns: list(schemaRef?.selected_columns).slice(0, 80),
        syntax_evidence: analysis.parser === "BABEL_AST" ? "AST" : "STRUCTURED_SQL",
      });
      schemaDependencyEdgeCount += 1;
      if (schemaDependencyEdgeCount >= 100 || edges.length >= 260) break;
    }
  }
  ...
  const changedConsumers = [...changed]
    .filter((filePath) => boundedFilePaths.has(filePath))
    .map((filePath) => ({
      path: filePath,
      ...
      observed_symbol_calls: edges
        ...
        .map((edge) => ({
          ...
          observed_argument_count: Number.isInteger(edge.observed_argument_count) ? edge.observed_argument_count : null,
          syntax_evidence: edge.syntax_evidence,
        })),
      observed_exports: [...(exportsByPath.get(filePath) || [])].slice(0, 40),
    }));
  const changedSchemaImpacts = [...changed]
    .filter((filePath) => boundedFilePaths.has(filePath))
    .flatMap((filePath) => {
      const changedSchemaEdges = edges.filter((edge) =>
        edge.from === filePath &&
        String(edge.to || "").startsWith("schema:") &&
        /^(?:defines_|alters_|references_database_|calls_database_)/.test(String(edge.relation || ""))
      );
      return changedSchemaEdges.map((edge) => ({
        changed_path: filePath,
        schema_id: edge.to,
        schema_kind: edge.schema_kind || null,
        schema_name: edge.schema_name || null,
        change_relation: edge.relation,
        observed_consumers: edges
          .filter((candidate) => candidate.to === edge.to && candidate.from !== filePath)
          .slice(0, 30)
          .map((candidate) => ({
            path: candidate.from,
            relation: candidate.relation,
            syntax_evidence: candidate.syntax_evidence || null,
          })),
      }));
    })
    .filter((entry, index, all) =>
      all.findIndex((candidate) =>
        candidate.changed_path === entry.changed_path && candidate.schema_id === entry.schema_id
      ) === index
    )
    .slice(0, 40);
  const unresolvedRelativeImportCount = [...documents.entries()].reduce((count, [fromPath, content]) => {
    const analysis = object(analyses.get(fromPath));
    const parsed = analysis.parsed === true
      ? list(analysis.imports).map((entry) => text(entry?.source, 1200)).filter((specifier) => /^\.{1,2}\//.test(specifier))
      : relativeImportSpecifiers(content);
    return count + unique(parsed).filter((specifier) => !resolveObservedImport(fromPath, specifier, boundedFilePaths)).length;
  }, 0);
  const astParseSuccessCount = [...analyses.values()].filter((analysis) => analysis?.parser === "BABEL_AST" && analysis?.parsed === true).length;
  const astParseFailureCount = [...analyses.values()].filter((analysis) => analysis?.parser === "BABEL_AST" && analysis?.parsed !== true).length;
  const sqlDocumentsObserved = [...analyses.values()].filter((analysis) => analysis?.parser === "STRUCTURED_SQL_RELATIONSHIP_SCANNER").length;
  return {
    ...
    changed_schema_impacts: changedSchemaImpacts,
    static_import_edges_observed: staticImportEdgeCount,
    imported_symbol_call_edges_observed: symbolCallEdgeCount,
    schema_dependency_edges_observed: schemaDependencyEdgeCount,
    ast_static_import_edges_observed: astImportEdgeCount,
    ast_imported_symbol_call_edges_observed: astSymbolCallEdgeCount,
    ast_parse_success_count: astParseSuccessCount,
    ast_parse_failure_count: astParseFailureCount,
    sql_documents_observed: sqlDocumentsObserved,
    unresolved_relative_import_count: unresolvedRelativeImportCount,
    source_documents_observed: documents.size,
    syntax_aware_dependency_parser: astParseSuccessCount > 0,
    syntax_aware_call_analysis: astSymbolCallEdgeCount > 0,
    structured_schema_dependency_analysis: schemaDependencyEdgeCount > 0 || sqlDocumentsObserved > 0,
    authoritative_dependency_parser: false,
    authoritative_call_graph: false,
    authoritative_schema_graph: false,
    bounded_static_dependency_analysis: true,
    bounded_symbol_call_analysis: true,
    bounded_schema_dependency_analysis: true,
    incomplete_evidence_must_not_be_treated_as_no_dependency: true,
    purpose: "BOUNDARY_BLAST_RADIUS_AND_SCHEMA_REASONING",
...
function negativeMemory(state = {}, objectiveContext = {}) {
  ...
  const recurringContractLessons = list(
    source?.verified_engineering_memory?.recurring_contract_failure_lessons,
  );
  return unique([
    ...list(source.negative_engineering_memory),
    ...list(context.negative_engineering_memory),
    ...verifiedMemoryLessons,
    ...recurringContractLessons,
...
function causalSchemaRelationships(causalGraph = {}) {
  return list(causalGraph?.changed_schema_impacts)
    .filter((entry) => list(entry?.observed_consumers).length > 0)
    .slice(0, 8);
}
...
function formatWorldClassDirective(control) {
  ...
  const schemaRelationships = causalSchemaRelationships(control?.causal_graph);
  return [
    ...
    `CAUSAL GRAPH: ${Number(control?.causal_graph?.node_count || 0)} nodes / ${Number(control?.causal_graph?.static_import_edges_observed || 0)} observed static import edges / ${Number(control?.causal_graph?.imported_symbol_call_edges_observed || 0)} observed imported-symbol call edges / ${Number(control?.causal_graph?.schema_dependency_edges_observed || 0)} schema edges; AST parsed=${Number(control?.causal_graph?.ast_parse_success_count || 0)}, AST failures=${Number(control?.causal_graph?.ast_parse_failure_count || 0)}, unresolved relative imports=${Number(control?.causal_graph?.unresolved_relative_import_count || 0)}. Treat missing edges as unknown when source evidence is incomplete.`,
    ...
    schemaRelationships.length
      ? `OBSERVED SCHEMA COMPATIBILITY OBLIGATIONS: ${schemaRelationships.map((item) => `${item.changed_path} -> ${item.schema_kind}:${item.schema_name} -> ${list(item.observed_consumers).map((consumer) => `${consumer.path}(${consumer.relation})`).join(",")}`).join(" | ")}. Revalidate these observed schema consumers before accepting a migration or persistence change.`
      : null,
...
export function prepareCodeAIWorldClassMission(options = {}) {
  ...
  const directive = readOnlyFastPath
    ? [
        "AVANTIQO WORLD-CLASS ENGINEERING CONTROL V1 — READ-ONLY FAST PATH",
        "Use current repository evidence only. Source mutation, candidate implementation competition, commit and deployment are forbidden.",
        "Prefer exact owner-named paths and finish when the requested facts are directly observed. Do not expand into speculative architecture work.",
        "Completion still requires truthful evidence; missing facts remain unknown and governance gates remain authoritative.",
      ].join("\n")
    : formatWorldClassDirective(control);
