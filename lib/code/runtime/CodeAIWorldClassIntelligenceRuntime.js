...
export function deriveCodeAICausalGraph(state = {}) {
  ...
  const schemaNodeIds = unique([...analyses.values()].flatMap((analysis) =>
    list(analysis?.schema_references).map((entry) => `schema:${text(entry?.kind, 80)}:${text(entry?.name, 500)}`)
  )).slice(0, 80);
  ...
  for (const [fromPath, content] of documents.entries()) {
    ...
    for (const schemaRef of list(analysis.schema_references)) {
      const target = `schema:${text(schemaRef?.kind, 80)}:${text(schemaRef?.name, 500)}`;
      ...
      edges.push({
        ...
        schema_name: text(schemaRef?.name, 500),
