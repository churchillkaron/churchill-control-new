...
export function assessCodeAIWorldClassQuality(state = {}) {
  ...
  const freshVerification = lastEditPosition >= 0
    ? freshVerificationEvidence(state, positions, lastEditPosition)
    : [];
  ...
  const sourceChangePaths = unique(list(state?.source_changes).map((change) => change?.path));
  ...
  if (lastEditPosition < 0) blockers.push("CODE_AI_WORLDCLASS_EDIT_EVIDENCE_REQUIRED");
