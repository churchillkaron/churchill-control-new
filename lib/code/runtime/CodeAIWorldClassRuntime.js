...
function qualityEvidence(assessment) {
  return {
    at: now(),
...
function attachQuality(state, assessment, { enforceCompletion = false } = {}) {
  const source = object(state);
  const evidence = [...list(source.evidence), qualityEvidence(assessment)]
...
export async function executeWorldClassCodeMission(input = {}) {
  const result = await executeAutonomousCodeMission(input);
