...
export async function recoverVerifiedCodeMissionCommit({
  ...
  const [currentRef, history] = await Promise.all([
    github("/git/ref/heads/main"),
    github(`/commits?sha=main&per_page=${RECOVERY_HISTORY_LIMIT}`),
  ]);
  ...
  throw new Error("CODE_AI_GITHUB_RECOVERY_VERIFIED_COMMIT_NOT_FOUND");
}
