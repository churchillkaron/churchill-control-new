...
import { normalizeCodeAISourceChanges } from "./CodeAISourceChangePolicy.js";
...
function normalizedChanges(state) {
  const changes = normalizeCodeAISourceChanges(state?.source_changes, {
    maxTotalWriteBytes: MAX_TOTAL_BYTES,
  });
...
function samePathSet(actualPaths, expectedChanges) {
  const actual = [...new Set(actualPaths.map((item) => text(item)).filter(Boolean))].sort();
...
function comparisonChangedPaths(comparison) {
  return [...new Set(
    list(comparison?.files).flatMap((file) => [
      text(file?.filename),
...
async function verifyCommitTreeChanges({
  github,
  candidateSha,
  expectedParent = null,
  changes,
  ...
  if (expectedParent && (parents.length !== 1 || text(parents[0]?.sha) !== expectedParent)) {
    return null;
  }

  const tree = await github(`/git/trees/${encodeURIComponent(treeSha)}?recursive=1`);
  if (tree?.truncated === true) return null;
  const entries = new Map(
    list(tree?.tree)
      .map((entry) => [text(entry?.path), entry])
  ...
  for (const change of changes) {
    ...
    if (!entry || text(entry?.type) !== "blob" || !text(entry?.sha)) return null;
    const blob = await github(`/git/blobs/${encodeURIComponent(text(entry.sha))}`);
    if (text(blob?.encoding).toLowerCase() !== "base64" || !text(blob?.content)) return null;
...
async function candidateMatchesAttestedChanges({
  ...
  if (!samePathSet(comparisonChangedPaths(comparison), changes)) return null;
  return verifyCommitTreeChanges({
    github,
    candidateSha,
...
export async function recoverVerifiedCodeMissionCommit({
  ...
  for (const candidate of candidates) {
    ...
    return {
      ...
      deleted_file_count: changes.filter((change) => change.operation === "delete").length,
      force: false,
      verified: true,
...
export async function promoteVerifiedCodeMissionReviewCommit({
  mission_state,
  env = process.env,
  fetch_impl = globalThis.fetch,
} = {}) {
  ...
  assertMissionReady(state, repository);
  const { changes, totalBytes } = normalizedChanges(state);
  const review = object(state.review_delivery);
  const reviewSha = text(review.commit_sha);
  const reviewBranch = text(review.review_branch);
  if (review.verified !== true || !/^[0-9a-f]{40}$/i.test(reviewSha) || !reviewBranch) {
    throw new Error("CODE_AI_GITHUB_VERIFIED_REVIEW_COMMIT_REQUIRED");
  }
  if (text(review.previous_commit || state.base_commit) !== text(state.base_commit)) {
    throw new Error("CODE_AI_GITHUB_REVIEW_BASE_MISMATCH");
  }
  ...
  const branchPath = reviewBranch.split("/").map(encodeURIComponent).join("/");
  const branchRef = await github(`/git/ref/heads/${branchPath}`);
  if (text(branchRef?.object?.sha) !== reviewSha) {
    throw new Error("CODE_AI_GITHUB_REVIEW_BRANCH_COMMIT_MOVED");
  }
  const matched = await verifyCommitTreeChanges({
    github,
    candidateSha: reviewSha,
    expectedParent: actualBase,
    changes,
  });
  if (!matched?.tree_sha) throw new Error("CODE_AI_GITHUB_REVIEW_COMMIT_VERIFICATION_FAILED");

  await github("/git/refs/heads/main", {
    method: "PATCH",
    body: { sha: reviewSha, force: false },
  });
  const verifiedRef = await github("/git/ref/heads/main");
  if (text(verifiedRef?.object?.sha) !== reviewSha) {
    throw new Error("CODE_AI_GITHUB_REVIEW_PROMOTION_VERIFICATION_FAILED");
  }
  ...
  return {
    ...
    force: false,
    verified: true,
    promoted_verified_review_commit: true,
...
export async function commitVerifiedCodeMission({
  ...
  const createdTree = await github("/git/trees", {
    ...
    body: {
      ...
      tree: changes.map((change) => change.operation === "delete"
        ? {
            path: change.path,
  ...
  const [verifiedRef, verifiedCommit, verifiedChanges] = await Promise.all([
  ...
  if (
    ...
    parentSha !== actualBase ||
    verifiedTreeSha !== treeSha ||
    verifiedChanges?.tree_sha !== treeSha
  ) {
...
function normalizedReviewBranchName(value, state) {
  const explicit = text(value).toLowerCase().replace(/[^a-z0-9._/-]+/g, "-").replace(/^-+|-+$/g, "");
  const mission = text(state?.mission_id).toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "");
  const suffix = text(state?.base_commit).slice(0, 8).toLowerCase();
  const candidate = explicit || `avantiqo/code/${mission || "mission"}-${suffix || "review"}`;
  const branch = candidate.startsWith("avantiqo/code/") ? candidate : `avantiqo/code/${candidate}`;
  if (branch.length > 120 || branch.includes("..") || branch.endsWith("/") || branch.startsWith("-")) {
    throw new Error("CODE_AI_GITHUB_REVIEW_BRANCH_INVALID");
  }
  return branch;
}

export async function createVerifiedCodeMissionPullRequest({
  mission_state,
  title,
  body = "",
  branch_name = null,
  ...
  assertMissionReady(state, repository);
  const { changes, totalBytes } = normalizedChanges(state);
  const prTitle = text(title);
  if (!prTitle) throw new Error("CODE_AI_GITHUB_REVIEW_TITLE_REQUIRED");
  if (prTitle.length > 200) throw new Error("CODE_AI_GITHUB_REVIEW_TITLE_TOO_LONG");
  const branch = normalizedReviewBranchName(branch_name, state);
  ...
  const createdCommit = await github("/git/commits", {
    method: "POST",
    body: { message: prTitle, tree: treeSha, parents: [actualBase] },
  });
  const commitSha = text(createdCommit?.sha);
  if (!commitSha) throw new Error("CODE_AI_GITHUB_CREATED_COMMIT_MISSING");
  ...
  const pull = await github("/pulls", {
    ...
    body: {
      ...
      head: branch,
      base: "main",
      body: text(body).slice(0, 12000) || "Verified Avantiqo Code mission. Merge remains a separate human/governed action.",
      maintainer_can_modify: true,
      draft: true,
    },
  });
  const pullNumber = Number(pull?.number);
  const pullUrl = text(pull?.html_url);
  if (!Number.isInteger(pullNumber) || pullNumber <= 0 || !pullUrl) {
    throw new Error("CODE_AI_GITHUB_REVIEW_PULL_REQUEST_INVALID");
  }
  const branchRef = await github(`/git/ref/heads/${branch.split("/").map(encodeURIComponent).join("/")}`);
  if (text(branchRef?.object?.sha) !== commitSha) throw new Error("CODE_AI_GITHUB_REVIEW_BRANCH_VERIFICATION_FAILED");
  if (text(pull?.head?.ref) && text(pull.head.ref) !== branch) throw new Error("CODE_AI_GITHUB_REVIEW_HEAD_MISMATCH");
  if (text(pull?.base?.ref) && text(pull.base.ref) !== "main") throw new Error("CODE_AI_GITHUB_REVIEW_BASE_MISMATCH");

  return {
    success: true,
    contract: "AVANTIQO_CODE_GITHUB_REVIEW_V1",
    repository: repository.full_name,
    base_branch: "main",
...
export const CodeGitHubCommitRuntime = Object.freeze({
  ...
  promoteReviewCommit: promoteVerifiedCodeMissionReviewCommit,
  recover: recoverVerifiedCodeMissionCommit,
  createReviewPullRequest: createVerifiedCodeMissionPullRequest,
});
