...
function authoritativeVerification(state) {
  return resolveCodeAIOwnerVerificationCommand({
    objective: text(state?.objective, 12000),
    objective_context: object(state?.objective_context),
  });
}

function authoritativeVerificationOperationIds(state) {
  const tests = verificationTestByOperationId(state);
  const resolved = authoritativeVerification(state);
  const expectedSignature = resolved
...
function deterministicCompletionEvidence(state, criteria) {
  ...
  const rules = [];
  const verificationBinding = authoritativeVerification(state);
  ...
  return {
    ...
    authoritative_verification_source:
      verificationBinding?.source || null,
  ...
  return [...wanted].map((operationId) => {
    ...
    return {
      ...
      args: list(test.args)
        ...
        .filter(Boolean)
        .slice(0, 20),
      env: normalizeCodeAIVerifierEnvironment(test.env),
