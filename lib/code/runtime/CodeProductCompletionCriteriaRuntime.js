...
function structuredAuthoritativeVerification(state) {
  const context = object(state?.objective_context);
  ...
  return {
    ...
    args: list(context.authoritative_verification_args)
      .slice(0, 24)
      .map((arg) => text(arg, 1000))
      .filter(Boolean),
  };
}

function legacyAuthoritativeVerificationSignature(state) {
  const objective = text(state?.objective, 12000);
  const marker = "authoritative verification command is";
  const markerIndex = objective.toLowerCase().indexOf(marker);
  if (markerIndex < 0) return null;
  let candidate = objective.slice(markerIndex + marker.length).trim();
  const stopPhrases = [
...
function authoritativeVerificationOperationIds(state) {
  ...
  const structured = structuredAuthoritativeVerification(state);
  const expectedSignature = structured
    ? [structured.command, ...structured.args].join(" ").trim()
    : legacyAuthoritativeVerificationSignature(state);
  if (!expectedSignature) return [];
  return successfulVerificationOperationIds(state).filter((operationId) =>
    commandSignature(tests.get(operationId)) === expectedSignature
  );
}
...
function legacyExplicitAllowedChangePaths(state) {
  const objective = text(state?.objective, 12000);
...
function explicitAllowedChangePaths(state) {
  const structured = unique(object(state?.objective_context).allowed_edit_paths);
  return structured.length ? structured : legacyExplicitAllowedChangePaths(state);
}
...
function deterministicCompletionEvidence(state, criteria) {
  ...
  return {
    ...
    authoritative_verification_source:
      structuredAuthoritativeVerification(state)
        ? "STRUCTURED_OBJECTIVE_CONTEXT"
        : "LEGACY_OBJECTIVE_TEXT",
    explicit_allowed_change_paths: allowedPaths,
    allowed_change_paths_source:
      unique(object(state?.objective_context).allowed_edit_paths).length
        ? "STRUCTURED_OBJECTIVE_CONTEXT"
        : "LEGACY_OBJECTIVE_TEXT",
...
export function projectCodeProductCompletionCriteria(state) {
  ...
  return {
    ...
    authoritative_verification_source:
      deterministic.authoritative_verification_source,
    explicit_allowed_change_paths:
      deterministic.explicit_allowed_change_paths,
    allowed_change_paths_source:
      deterministic.allowed_change_paths_source,
