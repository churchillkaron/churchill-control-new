...
const DISCOVERY_PATHSPEC_BATCH_SIZE = 60;
...
async function discoverPaths(workspace) {
  ...
  for (
    ...
    batches.push(DISCOVERY_PATHSPECS.slice(offset, offset + DISCOVERY_PATHSPEC_BATCH_SIZE));
  }
  const outputs = await mapConcurrent(batches, 3, async (pathspecBatch, index) => {
    ...
    if (result.exit_code !== 0) {
      const error = new Error(
        `CODE_AI_REPOSITORY_INTELLIGENCE_DISCOVERY_FAILED:${result.exit_code}:batch=${index + 1}`,
      );
      error.details = {
        ...result,
        discovery_batch: index + 1,
  ...
  return {
    ...
    discovery_batches: batches.length,
  };
}
...
async function collectInstructions(workspace, paths, maximumFiles = MAX_INSTRUCTION_FILES) {
  ...
  return mapConcurrent(selected, 6, async (filePath) => {
...
async function collectPackageManifests(workspace, paths, maximumFiles = MAX_PACKAGE_MANIFESTS) {
  const manifests = paths
    .filter(isPackageManifest)
    .sort((left, right) => left.split("/").length - right.split("/").length || left.localeCompare(right))
    .slice(0, maximumFiles);
  return mapConcurrent(manifests, 6, async (filePath) => {
    const read = await readBounded(workspace, filePath, 24000);
    return parsePackageManifest(read.content, filePath);
  });
}
...
export async function inspectCodeRepositoryIntelligence(workspace, { on_progress = null } = {}) {
  ...
  const interactive = interactivePreview?.authorized === true;

  await emitProgress({
    phase: "REPOSITORY_BASELINE_CHECK",
    status: "running",
    action: "inspect",
    description: "I’m checking the current branch, commit, and workspace state first.",
  });
  const [baseline, discovered] = await Promise.all([
    workspace.inspect(),
    (async () => {
      await emitProgress({
        phase: "REPOSITORY_PATH_DISCOVERY",
        status: "running",
        action: "search",
        description: "I’m mapping the tracked project files so I can narrow the relevant code safely.",
      });
      return discoverPaths(workspace);
    })(),
  ]);

  await emitProgress({
    phase: "REPOSITORY_GUIDANCE_READ",
    status: "running",
    action: "read",
    description: "I found the project structure. I’m reading the repository guidance and local engineering rules now.",
  });
  const [instructions, packages] = await Promise.all([
    collectInstructions(workspace, discovered.paths, interactive ? 6 : MAX_INSTRUCTION_FILES),
    (async () => {
      await emitProgress({
        phase: "PACKAGE_MANIFEST_READ",
        status: "running",
        action: "read",
        description: "I’m loading the relevant package manifests and test commands so I know how this project is verified.",
      });
      return collectPackageManifests(workspace, discovered.paths, interactive ? 6 : MAX_PACKAGE_MANIFESTS);
    })(),
  ]);

  await emitProgress({
    phase: "REPOSITORY_CONTEXT_READY",
    status: "running",
    action: "inspect",
    description: "The repository context is ready. I’m moving on to the next concrete code decision.",
  });
