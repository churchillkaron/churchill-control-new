...
const localWorkspaceSessions = new Map();
...
function localSessionRecord(sessionId) {
  const id = text(sessionId, 160);
  if (!id) return null;
  const record = localWorkspaceSessions.get(id);
  if (!record) return null;
  if (Date.now() > Number(record.expires_at || 0)) {
    localWorkspaceSessions.delete(id);
    clearTimeout(record.timer);
    record.workspace?.stop?.().catch(() => null);
    return null;
  }
  return record;
}

function refreshLocalSession(record) {
  if (!record) return null;
  record.expires_at = Date.now() + LOCAL_SESSION_TTL_MS;
  clearTimeout(record.timer);
  record.timer = setTimeout(() => {
    const current = localWorkspaceSessions.get(record.session_id);
    if (current !== record) return;
    localWorkspaceSessions.delete(record.session_id);
    record.workspace?.stop?.().catch(() => null);
...
async function acquireRepositoryWorktreeLock(sourceRoot) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (
        (owner?.pid && !processStillAlive(owner.pid)) ||
        (info && Date.now() - info.mtimeMs > WORKTREE_LOCK_STALE_MS)
      ) {
        await rm(lockPath, { force: true }).catch(() => null);
        continue;
      }
  ...
  try {
    ...
    const workspace = {
      ...
      stop: async () => {
        if (stopped) return;
        stopped = true;
        const record = localWorkspaceSessions.get(sessionId);
        if (record) {
          clearTimeout(record.timer);
          localWorkspaceSessions.delete(sessionId);
        }
    ...
    localWorkspaceSessions.set(sessionId, record);
    refreshLocalSession(record);
    return workspace;
  } catch (error) {
    await cleanupLocalWorktree(sourceRoot, workspaceRoot);
    throw error;
...
export async function attachLocalCodeWorkspace({
  session_id,
  repository_url,
  ref = "main",
  base_commit = null,
  ...
  const record = localSessionRecord(session_id);
  if (!record) throw new Error("CODE_AI_LOCAL_SESSION_NOT_FOUND");
  const repositoryUrl = assertRepositoryUrl(repository_url);
  const gitRef = assertGitRef(ref);
  if (
    normalizedRepositoryIdentity(record.repository_url) !==
    normalizedRepositoryIdentity(repositoryUrl)
  ) {
    throw new Error("CODE_AI_LOCAL_SESSION_REPOSITORY_MISMATCH");
  }
  if (text(record.ref, 160) !== text(gitRef, 160)) {
    throw new Error("CODE_AI_LOCAL_SESSION_REF_MISMATCH");
  }
  const expectedBaseCommit = text(base_commit, 160);
  if (
    expectedBaseCommit &&
    text(record.base_commit, 160) !== expectedBaseCommit
  ) {
    throw new Error("CODE_AI_LOCAL_SESSION_BASE_COMMIT_MISMATCH");
  }
  if (resume_patch !== null && resume_patch !== undefined) {
    const observed = await record.workspace.diff();
    if (String(observed.patch || "") !== String(resume_patch || "")) {
      throw new Error("CODE_AI_LOCAL_SESSION_PATCH_MISMATCH");
    }
  }
  refreshLocalSession(record);
  return {
...
export async function closeLocalCodeWorkspaceSession({ session_id } = {}) {
  const record = localSessionRecord(session_id);
  if (!record) return { closed: false, missing: true };
  await record.workspace.stop();
...
export const CodeWorkspaceLocalRuntime = Object.freeze({
  ...
  attach: attachLocalCodeWorkspace,
  closeSession: closeLocalCodeWorkspaceSession,
