...
async function projectInventoryLevel(event, storeAsset, data) {
  ...
  const observedAvailable = data.available == null ? null : number(data.available);
  const asset = await saveAsset({
    event,
    storeAsset,
  ...
  const observation = await supabaseAdmin.rpc("record_system_event_atomic", {
    p_organization_id: event.organization_id,
    p_type: "SHOPIFY_INVENTORY_LEVEL_OBSERVED",
    p_payload: {
      organization_id: event.organization_id,
      provider_event_id: event.id,
      connection_id: text(eventEnvelope(event).connection_id),
      inventory_level_asset_id: asset?.id || null,
      shopify_inventory_item_id: inventoryItemId,
      shopify_location_id: locationId,
      observed_available: observedAvailable,
      observed_at: data.updated_at || event.created_at || new Date().toISOString(),
    },
    p_idempotency_key: `shopify-inventory-observed:${event.id}`,
  });
  ...
  return {
    ...
    asset_type: "shopify_inventory_level",
    inventory_observation_event_id: observation.data?.event?.id || null,
  };
}
