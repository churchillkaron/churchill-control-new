...
const FINANCE_RECOVERY_EVENT = "SHOPIFY_ORDER_LIFECYCLE_RECONCILIATION_REQUESTED";
...
function syncState(connection) {
  return object(object(connection?.metadata).shopify_reconciliation);
}

function stateFor(connection, capability) {
  return object(syncState(connection)[capabilityKey(capability)]);
}
...
function shouldStartNewCycle(connection) {
  const state = syncState(connection);
  const lastFull = Date.parse(state.last_full_sync_at || "");
  if (!Number.isFinite(lastFull)) {
    return !text(state.started_at);
  }
  if (!allComplete(state)) return false;
  return Date.now() - lastFull >= FULL_SYNC_INTERVAL_MS;
}

async function saveSync(connection, patch) {
  const metadata = object(connection.metadata);
  const next = { ...syncState(connection), ...patch };
  const result = await supabaseAdmin
    .from("organization_channel_connections")
    .update({
      metadata: { ...metadata, shopify_reconciliation: next },
...
function normalizeOrder(node) {
  ...
  return {
    ...
    currency:
      node.currencyCode ||
      object(object(node.currentTotalPriceSet).shopMoney).currencyCode ||
      null,
...
async function ingest({ connection, topic, data, identity }) {
  return ProviderEventRuntime.ingest({
    organization_id: connection.organization_id,
    connection_id: connection.id,
    provider_id: "shopify",
    event_type: `shopify.${topic.replace(/\//g, ".")}`,
    external_event_id: `reconcile:${topic}:${identity}:${stableHash(data)}`,
    customer_reference:
...
async function recordFinanceLifecycleRecovery({ connection, providerEvent, data }) {
  if (!data?.id) return null;
  const identity = `${connection.id}:${data.id}:${data.updated_at || stableHash(data)}`;
  const result = await supabaseAdmin.rpc("record_system_event_atomic", {
    p_organization_id: connection.organization_id,
    p_type: FINANCE_RECOVERY_EVENT,
    p_payload: {
      organization_id: connection.organization_id,
      provider: "shopify",
      ...
      connection_id: connection.id,
      shop: text(connection?.metadata?.shop).toLowerCase(),
      topic: "orders/reconciliation",
      source: "SHOPIFY_RECONCILIATION",
      shopify_order_id: data.id,
      observed_at: data.updated_at || new Date().toISOString(),
    },
    p_idempotency_key: `shopify-finance-lifecycle-reconcile:${identity}`,
  });
  if (result.error) throw result.error;
  return result.data?.event?.id || null;
}

async function ingestPage({ connection, capability, providerData }) {
  let ingested = 0;
  let duplicates = 0;
  const count = (event) => {
  ...
  if (capability === "commerce.shopify.products.read") {
    for (const node of array(providerData?.products?.nodes)) {
      ...
      if (data.id) count(await ingest({ connection, topic: "products/update", data, identity: data.id }));
    }
  }
  ...
  if (capability === "commerce.shopify.orders.read") {
    for (const node of array(providerData?.orders?.nodes)) {
      ...
      if (data.id) {
        const providerEvent = await ingest({
          connection,
        ...
        count(providerEvent);
        await recordFinanceLifecycleRecovery({ connection, providerEvent, data });
      }
    }
  }
  ...
  if (capability === "commerce.shopify.inventory.read") {
    for (const node of array(providerData?.inventoryItems?.nodes)) {
      ...
      if (!data.id) continue;
      count(await ingest({ connection, topic: "inventory_items/update", data, identity: data.id }));
  ...
  if (capability === "commerce.shopify.locations.read") {
    for (const node of array(providerData?.locations?.nodes)) {
      ...
      if (data.id) count(await ingest({ connection, topic: "locations/update", data, identity: data.id }));
    }
  }
...
function collectionFor(providerData, capability) {
...
async function reconcileCapability({ connection, capability, currency }) {
  const key = capabilityKey(capability);
  const current = stateFor(connection, capability);
  ...
  const providerData = object(object(execution?.output).output);
  const collection = collectionFor(providerData, capability);
  const pageInfo = object(collection.pageInfo);
  const page = await ingestPage({ connection, capability, providerData });
  const reconciliation = syncState(connection);
  const nextCapability = {
    ...
    last_error: null,
    ingested: Number(current.ingested || 0) + page.ingested,
    duplicates: Number(current.duplicates || 0) + page.duplicates,
    usage_id: execution?.usage?.id || null,
  };

  const nextConnection = await saveSync(connection, {
    ...reconciliation,
    [key]: nextCapability,
  });
...
export async function reconcileShopifyConnections({ connectionLimit = 5 } = {}) {
  ...
  for (let connection of connections) {
    ...
    if (shouldStartNewCycle(connection)) {
      connection = await saveSync(connection, {
    ...
    try {
      for (const capability of CAPABILITIES) {
        const result = await reconcileCapability({ connection, capability, currency });
        capabilityResults.push({ ...result, connection: undefined });
        connection = result.connection || connection;
      }

      const state = syncState(connection);
      const complete = allComplete(state);
      if (complete) {
        connection = await saveSync(connection, {
          ...state,
          status: "READY",
