...
export async function deliverCommunicationMessage({
  ...
  const messageSource = String(persistedMessage?.metadata?.source || message?.metadata?.source || "")
    .trim()
    .toUpperCase();
  const sourceContext = persistedMessage?.metadata?.source_context || message?.metadata?.source_context || null;
  const systemAutomationSource = ["AVANTIQO_SECRETARY", "AVANTIQO_CUSTOMER_PORTAL"].includes(messageSource);
  const executionPartyId = systemAutomationSource ? null : partyId || null;
  if (transientBody != null && !(sensitiveBody === true && messageSource === "AVANTIQO_CUSTOMER_PORTAL")) {
    throw new Error("COMMUNICATION_TRANSIENT_BODY_NOT_AUTHORIZED");
  }
  const deliveryBody = transientBody == null ? persistedMessage.body : String(transientBody);
  const lineageMetadata = {
    ...(messageSource ? { source: messageSource } : {}),
    ...(sourceContext && typeof sourceContext === "object" ? { source_context: sourceContext } : {}),
  };
  ...
  try {
    ...
    const result = await executeService({
      ...
      metadata: {
        ...
        execution_actor: systemAutomationSource ? messageSource : "REQUEST_PARTY",
        attachment_count: media.length,
        sensitive_body_transient: sensitiveBody === true,
      },
    });
    ...
    const providerErrorCode = String(error?.code || "").trim() || null;
    const providerErrorMessage = String(error?.message || "").trim() || null;
    return setMessageDeliveryState({
      organizationId,
      messageId: message.id,
      status: "FAILED",
      ...
      metadata: {
        ...
        provider_error_message: sensitiveBody ? null : providerErrorMessage?.slice(0, 500) || null,
      },
    });
  }
}
