...
function syncState(connection) {
  return object(object(connection?.metadata).email_sync);
}

function pushRequested(connection) {
  return syncState(connection).push_requested === true;
}

function due(connection, intervalMinutes) {
  const sync = syncState(connection);
  if (sync.push_requested === true) return true;
...
export async function syncEmailConnection({ connection }) {
  ...
  const metadata = object(connection.metadata);
  const syncStateValue = object(metadata.email_sync);
  ...
  try {
    const result = await executeService({
      ...
      input: {
        currency,
        cursor: object(syncStateValue.cursor),
        quantity: 1,
      },
      metadata: {
    ...
    await persistConnectionSync({
      ...
      patch: {
        ...
        last_push_consumed_at: syncStateValue.push_requested === true
          ? completedAt
          : syncStateValue.last_push_consumed_at || null,
      },
    });
...
export async function syncDueEmailConnections({
  ...
  const { data, error } = await supabaseAdmin
    ...
    .limit(Math.max(connectionLimit * 10, 100));
  if (error) throw error;

  const ordered = [...(data || [])].sort((a, b) => {
    const pushDifference = Number(pushRequested(b)) - Number(pushRequested(a));
    if (pushDifference) return pushDifference;
