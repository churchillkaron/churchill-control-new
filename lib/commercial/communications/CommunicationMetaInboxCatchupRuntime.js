...
function readinessCandidates(connections, now = Date.now()) {
  return connections
...
export async function refreshDueMetaMessagingReadiness({
  connectionLimit = MAX_READINESS_PREFLIGHT,
} = {}) {
  const limit = boundedInteger(
    connectionLimit,
    MAX_READINESS_PREFLIGHT,
    1,
    MAX_READINESS_PREFLIGHT,
  );
  const connections = await listActiveMetaConnections();
  const candidates = readinessCandidates(connections);
  const selected = candidates.slice(0, limit);
  const results = [];

  for (const connection of selected) {
    try {
      const readiness = await persistMessagingReadiness(connection);
      results.push({
        ...
        webhookSubscriptionHealthy:
          readiness?.webhook_subscription?.healthy === true,
        error: readiness.success === true ? null : readiness.error || null,
      });
    } catch (error) {
      results.push({
        organizationId: connection.organization_id,
  ...
  return {
    ...
    eligibleConnections: candidates.length,
...
export async function syncDueMetaCommunicationHistory({
  ...
  const now = Date.now();
  const readinessPreflight = readinessCandidates(connections, now)
