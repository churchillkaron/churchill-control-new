...
function conversationAccountIds(conversation) {
  const ids = new Set();
  for (const participant of rows(conversation?.participants)) {
    const id = text(participant?.id);
    if (id) ids.add(id);
  }
  for (const message of Array.isArray(conversation?.messages) ? conversation.messages : []) {
    const senderId = text(message?.from?.id);
    if (senderId) ids.add(senderId);
    for (const recipient of messageRecipients(message)) {
      const recipientId = text(recipient?.id);
...
function platformForConversation(conversation, { pageId, instagramBusinessId }) {
  const ids = conversationAccountIds(conversation);
  if (instagramBusinessId && ids.has(instagramBusinessId)) {
    return {
...
export async function syncMetaCommunicationHistory({ organizationId }) {
  ...
  const totals = new Map([
    ["facebook_messenger", { provider: "facebook_messenger", platform: "messenger", conversationCount: 0, messageCount: 0 }],
    ["instagram_messaging", { provider: "instagram_messaging", platform: "instagram", conversationCount: 0, messageCount: 0 }],
  ]);
  let unclassifiedConversationCount = 0;

  for (const remoteConversation of remote.conversations || []) {
    const classification = platformForConversation(remoteConversation, {
      pageId,
      instagramBusinessId,
    });
    if (!classification) {
      unclassifiedConversationCount += 1;
      continue;
    }
    ...
    const total = totals.get(classification.provider);
    total.conversationCount += outcome.conversationCreated;
    total.messageCount += outcome.messageCount;
  }

  const results = [...totals.values()].map((row) => ({
    ...row,
    skipped: row.provider === "instagram_messaging" && !instagramBusinessId,
  }));
