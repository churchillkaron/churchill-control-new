...
async function conversationByThread({
  organizationId,
  provider,
  connectionId,
  externalThreadId,
...
async function syncConversation({
  ...
  const participantProfile = object(participant?.profile);
  const participantPicture = object(participant?.picture);
  const participantPictureData = object(participantPicture?.data);
  const participantImageUrl = text(
    participant?.profile_pic ||
    participant?.profile_picture_url ||
    participant?.picture_url ||
    participant?.avatar_url ||
    participantProfile.picture_url ||
    participantProfile.profile_pic ||
    participantPictureData.url ||
    participantPicture.url,
  );
  ...
  if (!conversation) {
    conversation = await createConversation({
      ...
      metadata: {
        ...
        meta_platform: platform,
        ...(participantImageUrl ? { participant_profile_image_url: participantImageUrl } : {}),
      },
    });
