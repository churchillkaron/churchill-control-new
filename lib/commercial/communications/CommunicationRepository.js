...
function messageEventTime(row) {
  return new Date(row?.sent_at || row?.received_at || row?.created_at || 0).getTime();
}
...
export async function listMessages({ organizationId, conversationId, limit = 100 }) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .limit(cappedLimit);
  if (error) throw error;
  return [...(data || [])].sort((left, right) => messageEventTime(left) - messageEventTime(right));
}
...
export async function listLatestMessages({ organizationId, conversationIds = [] }) {
  ...
  return rows
    .sort((left, right) => messageEventTime(right) - messageEventTime(left))
