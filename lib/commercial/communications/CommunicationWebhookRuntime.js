import { supabaseAdmin } from "@/lib/shared/supabase/admin";
import { emitBusinessPartnerBusinessEvent } from "@/lib/operator/runtime/BusinessPartnerBusinessEventAdapterRuntime";
import {
  createAttachments,
  createConversation,
  createMessage,
  updateConversation,
  updateMessage,
} from "./CommunicationRepository";

function text(value) {
  return String(value ?? "").trim();
}

function object(value) {
  return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}

function lower(value) {
  return text(value).toLowerCase();
}

function upper(value) {
  return text(value).toUpperCase();
}

async function persistContactProfileImage({ organizationId, partyId, imageUrl, provider }) {
  const normalizedPartyId = text(partyId);
  const normalizedImageUrl = text(imageUrl);
  if (!normalizedPartyId || !normalizedImageUrl) return;

  const existing = await supabaseAdmin
    .from("secretary_contact_profiles")
    .select("id,metadata")
    .eq("organization_id", organizationId)
    .eq("party_id", normalizedPartyId)
    .maybeSingle();
  if (existing.error) throw existing.error;

  const now = new Date().toISOString();
  const metadata = {
    ...object(existing.data?.metadata),
    profile_image_url: normalizedImageUrl,
    profile_image_provider: text(provider) || null,
    profile_image_updated_at: now,
  };

  if (existing.data?.id) {
    const updated = await supabaseAdmin
      .from("secretary_contact_profiles")
      .update({ metadata, updated_at: now })
      .eq("organization_id", organizationId)
      .eq("party_id", normalizedPartyId);
    if (updated.error) throw updated.error;
    return;
  }

  const inserted = await supabaseAdmin.from("secretary_contact_profiles").insert({
    organization_id: organizationId,
    party_id: normalizedPartyId,
    metadata,
  });
  if (inserted.error && inserted.error.code !== "23505") throw inserted.error;
}

function providerTimestamp(value) {
  if (!value) return null;
  const numeric = Number(value);
  if (Number.isFinite(numeric)) {
    const milliseconds = numeric < 100000000000 ? numeric * 1000 : numeric;
    const date = new Date(milliseconds);
    return Number.isNaN(date.getTime()) ? null : date.toISOString();
  }
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? null : date.toISOString();
}

async function oneConversation({
  organizationId,
  provider,
  connectionId,
  externalThreadId,
  participantId,
}) {
  const normalizedThreadId = text(externalThreadId);
  const normalizedParticipantId = text(participantId);

  const base = () => supabaseAdmin
    .from("communication_conversations")
    .select("*")
    .eq("organization_id", organizationId)
    .eq("provider", provider)
    .eq("connection_id", connectionId);

  if (normalizedThreadId) {
    const exact = await base()
      .eq("external_thread_id", normalizedThreadId)
      .order("updated_at", { ascending: false })
      .limit(1);
    if (exact.error) throw exact.error;
    const exactRow = exact.data?.[0] || null;

    const participantMatches = normalizedParticipantId
      ? await base()
          .eq("external_participant_id", normalizedParticipantId)
          .order("updated_at", { ascending: false })
          .limit(10)
      : { data: [], error: null };
    if (participantMatches.error) throw participantMatches.error;

    const canonical = (participantMatches.data || []).find((row) => {
      const rowThread = text(row.external_thread_id);
      return rowThread && rowThread !== normalizedParticipantId;
    });
    return canonical || exactRow || participantMatches.data?.[0] || null;
  }

  const participantMatch = await base()
    .eq("external_participant_id", normalizedParticipantId)
    .order("updated_at", { ascending: false })
    .limit(1);
  if (participantMatch.error) throw participantMatch.error;
  return participantMatch.data?.[0] || null;
}

async function messageByExternalId({ organizationId, provider, externalMessageId }) {
  if (!text(externalMessageId)) return null;
  const { data, error } = await supabaseAdmin
    .from("communication_messages")
    .select("*")
    .eq("organization_id", organizationId)
    .eq("provider", provider)
    .eq("external_message_id", text(externalMessageId))
    .limit(1);
  if (error) throw error;
  return data?.[0] || null;
}

function attachmentRows({ organizationId, messageId, attachments = [] }) {
  return (Array.isArray(attachments) ? attachments : [])
    .map((entry) => {
      const attachment = object(entry);
      const externalUrl = text(attachment.external_url || attachment.url) || null;
      const storagePath = text(attachment.storage_path) || null;
      if (!externalUrl && !storagePath) return null;
      const size = Number(attachment.size_bytes ?? attachment.size);
      return {
        organization_id: organizationId,
        message_id: messageId,
        storage_path: storagePath,
        external_url: externalUrl,
        file_name: text(attachment.file_name || attachment.name) || null,
        mime_type: text(attachment.mime_type || attachment.type) || null,
        size_bytes: Number.isFinite(size) && size >= 0 ? Math.trunc(size) : null,
        metadata: object(attachment.metadata),
      };
    })
    .filter(Boolean)
    .slice(0, 20);
}

export async function resolveCommunicationConnectionByAsset({
  provider,
  assetType,
  externalId,
}) {
  const normalizedProvider = lower(provider);
  const normalizedExternalId = text(externalId);
  if (!normalizedProvider || !text(assetType) || !normalizedExternalId) return null;

  const { data: assets, error: assetError } = await supabaseAdmin
    .from("organization_channel_assets")
    .select("id,organization_id,connection_id,channel_provider,asset_type,external_id,name,metadata")
    .eq("channel_provider", normalizedProvider)
    .eq("asset_type", text(assetType))
    .eq("external_id", normalizedExternalId)
    .limit(2);
  if (assetError) throw assetError;
  if (!assets?.length) return null;
  if (assets.length > 1) throw new Error("COMMUNICATION_CHANNEL_ASSET_AMBIGUOUS");

  return resolveCommunicationConnectionById({
    provider: normalizedProvider,
    connectionId: assets[0].connection_id,
    organizationId: assets[0].organization_id,
    asset: assets[0],
  });
}

export async function resolveCommunicationConnectionById({
  provider,
  connectionId,
  organizationId = null,
  asset = null,
}) {
  const normalizedProvider = lower(provider);
  const normalizedConnectionId = text(connectionId);
  if (!normalizedProvider || !normalizedConnectionId) return null;

  let query = supabaseAdmin
    .from("organization_channel_connections")
    .select("id,organization_id,provider,channel_type,credentials_reference,status,metadata")
    .eq("id", normalizedConnectionId)
    .eq("provider", normalizedProvider)
    .limit(1);
  if (text(organizationId)) query = query.eq("organization_id", text(organizationId));

  const { data, error } = await query;
  if (error) throw error;
  const connection = data?.[0] || null;
  if (!connection || upper(connection.status) !== "ACTIVE") return null;
  return { ...connection, asset: asset || null };
}

export async function ingestInboundCommunication({
  connection,
  providerOverride = null,
  channelTypeOverride = null,
  conversationMetadata = {},
  externalMessageId,
  externalThreadId = null,
  participantId,
  participantName = null,
  participantAddress = null,
  recipientAddress = null,
  messageType = "TEXT",
  body = null,
  receivedAt = null,
  metadata = {},
  attachments = [],
}) {
  const organizationId = text(connection?.organization_id);
  const connectionId = text(connection?.id);
  const provider = lower(providerOverride || connection?.provider);
  const channelType = text(channelTypeOverride || connection?.channel_type || provider);
  const participant = text(participantId);
  const providerMessageId = text(externalMessageId);
  if (!organizationId || !connectionId || !provider || !participant || !providerMessageId) {
    throw new Error("COMMUNICATION_INBOUND_IDENTITY_REQUIRED");
  }

  const duplicate = await messageByExternalId({
    organizationId,
    provider,
    externalMessageId: providerMessageId,
  });
  if (duplicate) return { duplicate: true, message: duplicate, conversation: null };

  let conversation = await oneConversation({
    organizationId,
    provider,
    connectionId,
    externalThreadId,
    participantId: participant,
  });

  const now = providerTimestamp(receivedAt) || new Date().toISOString();
  if (!conversation) {
    conversation = await createConversation({
      organization_id: organizationId,
      connection_id: connectionId,
      provider,
      channel_type: channelType,
      external_thread_id: text(externalThreadId) || null,
      external_participant_id: participant,
      external_participant_name: text(participantName) || null,
      external_participant_address: text(participantAddress) || participant,
      subject: null,
      status: "OPEN",
      unread_count: 0,
      last_message_at: now,
      last_inbound_at: now,
      metadata: {
        source: "PROVIDER_WEBHOOK",
        connection_name: object(connection.metadata).display_name || object(connection.metadata).verified_name || object(connection.metadata).page_name || null,
        connection_provider: lower(connection.provider),
        connection_asset_id: connection?.asset?.id || null,
        connection_asset_type: connection?.asset?.asset_type || null,
        connection_asset_external_id: connection?.asset?.external_id || null,
        ...object(conversationMetadata),
      },
    });
  }

  let message;
  try {
    message = await createMessage({
      organization_id: organizationId,
      conversation_id: conversation.id,
      connection_id: connectionId,
      provider,
      channel_type: channelType,
      direction: "INBOUND",
      message_type: upper(messageType) || "TEXT",
      external_message_id: providerMessageId,
      sender_address: text(participantAddress) || participant,
      recipient_address: text(recipientAddress) || null,
      body: body == null ? null : String(body),
      status: "RECEIVED",
      received_at: now,
      metadata: object(metadata),
    });
    const savedAttachments = await createAttachments(
      attachmentRows({ organizationId, messageId: message.id, attachments }),
    );
    message = { ...message, attachments: savedAttachments };
  } catch (error) {
    if (error?.code === "23505") {
      const existing = await messageByExternalId({
        organizationId,
        provider,
        externalMessageId: providerMessageId,
      });
      if (existing) return { duplicate: true, message: existing, conversation };
    }
    throw error;
  }

  conversation = await updateConversation({
    organizationId,
    conversationId: conversation.id,
    patch: {
      external_participant_name: text(participantName) || conversation.external_participant_name || null,
      external_participant_address: text(participantAddress) || conversation.external_participant_address || participant,
      unread_count: Number(conversation.unread_count || 0) + 1,
      last_message_at: now,
      last_inbound_at: now,
      status: "OPEN",
      metadata: {
        ...object(conversation.metadata),
        ...object(conversationMetadata),
      },
    },
  });

  const providerImageUrl = text(object(conversationMetadata).participant_profile_image_url);
  if (conversation.customer_party_id && providerImageUrl) {
    await persistContactProfileImage({
      organizationId,
      partyId: conversation.customer_party_id,
      imageUrl: providerImageUrl,
      provider,
    });
  }

  await emitBusinessPartnerBusinessEvent({
    organization_id: organizationId,
    event_source: `communication:${provider}`,
    event_type: "MESSAGE_RECEIVED",
    correlations: [
      `conversation:${conversation.id}`,
      text(externalThreadId) ? `thread:${text(externalThreadId)}` : null,
      `participant:${participant}`,
      text(participantAddress) ? `participant:${lower(participantAddress)}` : null,
    ],
    identity: `${provider}:${providerMessageId}`,
    evidence: {
      communication_message_id: message.id,
      external_message_id: providerMessageId,
      conversation_id: conversation.id,
      external_thread_id: text(externalThreadId) || null,
      participant_id: participant,
      participant_address: text(participantAddress) || null,
      channel_type: channelType,
      received_at: now,
    },
  }).catch((error) => console.error("BUSINESS_PARTNER_COMMUNICATION_EVENT_ADAPTER_FAILED", error));

  return { duplicate: false, message, conversation };
}

const DELIVERY_RANK = {
  QUEUED: 0,
  SENDING: 1,
  SENT: 2,
  DELIVERED: 3,
  READ: 4,
};

export async function applyCommunicationDeliveryStatus({
  connection,
  providerOverride = null,
  externalMessageId,
  status,
  providerTimestamp: statusTimestamp = null,
  errorCode = null,
  errorMessage = null,
  metadata = {},
}) {
  const organizationId = text(connection?.organization_id);
  const provider = lower(providerOverride || connection?.provider);
  const providerMessageId = text(externalMessageId);
  const normalizedStatus = upper(status);
  if (!organizationId || !provider || !providerMessageId) return { matched: false, message: null };
  if (!["SENT", "DELIVERED", "READ", "FAILED"].includes(normalizedStatus)) {
    return { matched: false, message: null };
  }

  const existing = await messageByExternalId({
    organizationId,
    provider,
    externalMessageId: providerMessageId,
  });
  if (!existing) return { matched: false, message: null };

  const currentStatus = upper(existing.status);
  if (currentStatus === "FAILED" && normalizedStatus !== "FAILED") {
    return { matched: true, ignored: true, message: existing };
  }
  if (
    normalizedStatus !== "FAILED" &&
    DELIVERY_RANK[currentStatus] != null &&
    DELIVERY_RANK[normalizedStatus] != null &&
    DELIVERY_RANK[normalizedStatus] < DELIVERY_RANK[currentStatus]
  ) {
    return { matched: true, ignored: true, message: existing };
  }

  const providerTime = providerTimestamp(statusTimestamp);
  const nextMetadata = {
    ...object(existing.metadata),
    ...object(metadata),
    provider_delivery_status: normalizedStatus,
    provider_delivery_timestamp: providerTime,
  };

  const message = await updateMessage({
    organizationId,
    messageId: existing.id,
    patch: {
      status: normalizedStatus,
      error_code: normalizedStatus === "FAILED" ? text(errorCode) || null : null,
      error_message: normalizedStatus === "FAILED" ? text(errorMessage) || null : null,
      sent_at:
        existing.sent_at ||
        (["SENT", "DELIVERED", "READ"].includes(normalizedStatus)
          ? providerTime || new Date().toISOString()
          : null),
      metadata: nextMetadata,
    },
  });

  return { matched: true, ignored: false, message };
}
