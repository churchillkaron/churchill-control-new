...
async function markThreadRead({ organizationId, staffId, threadId }) {
  const pageSize = 500;
  let offset = 0;
  ...
  while (true) {
    ...
    await markMessagesRead({ staffId, messages: page });
...
export async function getInternalConversationTimeline({
  ...
  if (markRead) {
    await markThreadRead({ organizationId, staffId, threadId });
  }
