...
function attachmentReference(attachment = {}) {
  return text(attachment.external_url || attachment.url || attachment.storage_path);
}

function validAttachmentReference(reference) {
  if (!reference) return false;
  if (/^https?:\/\//i.test(reference) || reference.startsWith("storage://")) return true;
  return /^\/api\/finance\/customer-invoices\/[^/]+\/pdf(?:\?|$)/.test(reference);
}
...
export const manifest = defineCapability({
  ...
  operatorVerification: { capability_key: "commercial.communication.read", payload_from_result: { conversation_id: ["conversation_id"], message_id: ["message_id"] }, derivation: "declared_result_bound_draft_message_verifier" },
  inputSchema: {
    type: "object",
    properties: {
      conversation_id: {
      ...
      attachments: {
        ...
        items: {
          type: "object",
          properties: {
            url: { type: "string", description: "Exact prior Business Partner presentation-artifact URL." },
            external_url: { type: "string", description: "Exact external media URL when already normalized." },
            storage_path: { type: "string", description: "Canonical storage:// reference when applicable." },
            file_name: { type: "string", description: "Optional attachment filename." },
            name: { type: "string", description: "Optional artifact label used as filename fallback." },
        ...
        description: "Optional exact artifacts. Internal URLs are limited to canonical Finance customer-invoice/receipt PDFs; HTTPS and storage:// references are accepted for other media.",
      },
      source_context: {
        type: "object",
...
export function validate({ payload = {} }) {
  ...
  if (!text(payload.body) && !attachments.length) {
    const error = new Error("MESSAGE_BODY_OR_ATTACHMENT_REQUIRED");
    error.status = 400;
    throw error;
  }
  for (const attachment of attachments) {
    if (!validAttachmentReference(attachmentReference(attachment))) {
      const error = new Error("COMMUNICATION_ATTACHMENT_REFERENCE_INVALID");
      error.status = 400;
      throw error;
    }
  }
