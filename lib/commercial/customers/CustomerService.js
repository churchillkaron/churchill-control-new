...
export async function findCustomerIdentityMatches({
  ...
  const customerName = text(name);
  if (!customerName) return [];
  const safeLimit = Math.max(1, Math.min(Number(limit) || 12, 50));

  const displayResult = await supabaseAdmin
    .from("parties")
    .select("id, display_name, legal_name, party_type, status")
    .eq("organization_id", organizationId)
    .ilike("display_name", customerName)
    .limit(safeLimit);
  if (displayResult.error) throw displayResult.error;

  let parties = displayResult.data || [];
  if (!parties.length) {
    const legalResult = await supabaseAdmin
      .from("parties")
      .select("id, display_name, legal_name, party_type, status")
      .eq("organization_id", organizationId)
      .ilike("legal_name", customerName)
      .limit(safeLimit);
    if (legalResult.error) throw legalResult.error;
    parties = legalResult.data || [];
  }

  const partyIds = [...new Set(parties.map((row) => row.id).filter(Boolean))];
  if (!partyIds.length) return [];
  ...
  const customerIds = new Set((relationshipResult.data || []).map((row) => String(row.party_id)));
  return parties
    .filter((party) => customerIds.has(String(party.id)))
    .map((party) => ({
      id: party.id,
      party_id: party.id,
      customer_name: party.display_name,
