...
export async function listQuotations({
  ...
  return quotations.map((quotation) => ({
    ...
    document_date: quotation.quotation_date || null,
...
export async function createQuotation({
  ...
  const quotationDate = text(body.quotationDate || body.quotation_date || body.date) || todayDate();
  ...
  if (result.error) {
    ...
    throw attachActionIdentityEvidence(result.error, [`idempotency_key:${idempotencyKey}`]);
  }

  const quotationId = uuidOrNull(result.data?.quotation_id);
  if (quotationId) {
    const dated = await supabaseAdmin
      .from("commercial_quotations")
      .update({ quotation_date: quotationDate })
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
      .eq("id", quotationId);
    if (dated.error) throw attachActionIdentityEvidence(dated.error, [`quotation_id:${quotationId}`, `idempotency_key:${idempotencyKey}`]);
  }
  return {
    ...(result.data || {}),
    success: true,
...
export async function transitionQuotation({
  ...
  const quotation = await loadQuotation({ organizationId, entityId, quotationId });
  const portalProvisioning = action === "SEND" && quotation?.party_id
