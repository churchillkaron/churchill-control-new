...
async function recentPublishedReplies(organizationId, limit = 24) {
...
function fallbackAnalysis(review, policy) {
  ...
  if (rating >= 4) {
    return {
      response: hasText
        ...
        : `Thank you, ${firstName}, for the ${rating}-star rating. We truly appreciate your visit to ${brand} and hope to see you again soon.`,
      language_code: "en",
      sentiment: "POSITIVE",
  ...
  if (rating >= 3) {
    return {
      response: `Thank you, ${firstName}, for taking the time to share your ${rating}-star feedback with ${brand}. We appreciate it and will use it to keep improving the guest experience.`,
      language_code: "en",
      sentiment: "MIXED",
      sentiment_score: 0,
  ...
  return {
    response: `Thank you, ${firstName}, for telling us about your experience at ${brand}. We’re sorry it did not meet expectations, and we would appreciate the chance to understand the concerns you raised and follow up properly.`,
    language_code: "en",
    sentiment: "NEGATIVE",
    sentiment_score: -0.9,
...
- If there is no written comment, personalize with the reviewer's first name and rating without inventing an experience.
...
export async function processReview(review) {
  ...
  if (hasWrittenReview && !analysis.ai_generated) {
    ...
    return {
      ...
      status: "FAILED",
      published: false,
      error: analysis.ai_error || "Personalized review response generation failed",
    };
  }
  ...
  // Low-rating reviews still open an internal recovery case even when the
  // organization has chosen to auto-publish a public acknowledgement.
...
export async function syncGoogleReviews({ organizationId, maxReviews = 200 }) {
  ...
  if (!assets.length) {
    return {
      ...
      retryAt: null,
      error:
        "Map at least one Google Business Profile location to a legal entity before review synchronization.",
    };
  }
