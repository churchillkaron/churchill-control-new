...
export async function confirmSalesOrder({
  ...
  const orderResult = await supabaseAdmin.from("sales_orders")
    .select("id,party_id")
  ...
  if (orderResult.error) {
    portalProvisioning = {
      provisioned: false,
      access: null,
      error: orderResult.error.message || "Sales-order customer Party could not be resolved for portal access",
    };
  } else if (orderResult.data?.party_id) {
    portalProvisioning = await provisionCustomerPortalAccess({
      organizationId,
      partyId: orderResult.data.party_id,
