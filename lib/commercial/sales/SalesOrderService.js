...
export async function createSalesOrderDraft({
  ...
  if (rpcResult.error) {
    ...
    throw attachActionIdentityEvidence(rpcResult.error, [`idempotency_key:${idempotencyKey}`]);
  }
...
export async function listSalesOrders({ organizationId, entityId, limit = 200, salesOrderId = null, idempotencyKey = null }) {
  if (!entityId) {
  ...
  if (salesOrderId) orderQuery = orderQuery.eq("id", salesOrderId);
  if (idempotencyKey) orderQuery = orderQuery.eq("idempotency_key", idempotencyKey);
  const orderResult = await orderQuery.order("created_at", { ascending: false }).limit(salesOrderId || idempotencyKey ? 1 : Math.max(1, Math.min(Number(limit) || 200, 500)));
