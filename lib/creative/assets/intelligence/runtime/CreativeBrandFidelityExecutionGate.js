...
function brandMarkCompositing(task = {}) {
  const value = object(task.input?.shot_bible?.finishing?.brand_mark_compositing);
  if (value.contract !== BRAND_MARK_COMPOSITING_CONTRACT || value.required !== true) {
    return null;
  }
  return value;
}
...
function assetChecksum(asset = {}) {
  return text(
    ...
    asset.analysis?.technical_metadata?.checksum_sha256 ||
...
async function enforceBrandMarkCompositing(task = {}) {
  const compositing = brandMarkCompositing(task);
  if (!compositing) return task;

  const sourceAssetId = text(compositing.source_asset_id);
  const expectedChecksum = text(compositing.source_checksum_sha256);
  if (!sourceAssetId || !expectedChecksum) {
    throw new Error(
      `CREATIVE_BRAND_MARK_SOURCE_EVIDENCE_REQUIRED:${task.id || "unknown"}`,
    );
  }
  ...
  if (!CreativeShotAssetScopeRuntime.verify(scope)) {
    throw new Error(
      `CREATIVE_BRAND_MARK_ASSET_SCOPE_REQUIRED:${task.id || "unknown"}`,
    );
  }
  if (!list(scope.creative_asset_ids).map(text).includes(sourceAssetId)) {
    throw new Error(
      `CREATIVE_BRAND_MARK_SOURCE_OUTSIDE_ASSET_SCOPE:${task.id || "unknown"}:${sourceAssetId}`,
    );
  }

  const asset = await CreativeAssetsRuntime.get(sourceAssetId);
  if (!asset || text(asset.organization_id) !== text(task.organization_id)) {
    throw new Error(
      `CREATIVE_BRAND_MARK_SOURCE_ASSET_NOT_FOUND:${task.id || "unknown"}:${sourceAssetId}`,
    );
  }
  const provenance = CreativeBrandFidelityRuntime.classify(asset);
  if (!provenance.trusted_for_brand_fidelity_primary) {
    throw new Error(
      `CREATIVE_BRAND_MARK_SOURCE_ASSET_UNTRUSTED:${task.id || "unknown"}:${sourceAssetId}`,
    );
  }

  const actualChecksum = assetChecksum(asset);
  if (!actualChecksum) {
    throw new Error(
      `CREATIVE_BRAND_MARK_SOURCE_CHECKSUM_UNAVAILABLE:${task.id || "unknown"}:${sourceAssetId}`,
    );
  }
  if (actualChecksum !== expectedChecksum) {
    throw new Error(
      `CREATIVE_BRAND_MARK_SOURCE_CHECKSUM_MISMATCH:${task.id || "unknown"}:${sourceAssetId}`,
    );
  }
  ...
  return ProductionTaskRuntime.update(task.id, {
    metadata: {
      ...
      brand_mark_compositing_execution_gate: {
        contract: "CREATIVE_BRAND_MARK_COMPOSITING_EXECUTION_GATE_V1",
        passed: true,
        source_asset_id: sourceAssetId,
        source_checksum_sha256: actualChecksum,
        source_classification: provenance.classification,
...
async function enforce(task = {}) {
  ...
  let verifiedTask = await enforceBrandMarkCompositing(task);
  const contracts = CreativeBrandFidelityRuntime.contracts;
  const fidelity = brandRule(verifiedTask, contracts.brand_fidelity);
  if (!fidelity || fidelity.required !== true) return verifiedTask;

  const truth = brandRule(verifiedTask, contracts.brand_truth);
  if (
    !truth ||
    text(truth.brand_truth_hash) !== text(fidelity.brand_truth_hash)
  ) {
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_TRUTH_REQUIRED:${verifiedTask.id || "unknown"}`,
    );
  }

  const lineage = storyLineage(verifiedTask);
  for (const key of [
    "research_report_id",
    "research_identity",
    ...
    if (!text(lineage[key])) {
      throw new Error(
        `CREATIVE_BRAND_FIDELITY_STORY_LINEAGE_REQUIRED:${verifiedTask.id || "unknown"}:${key}`,
      );
    }
  }
  if (
    ...
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_RESEARCH_LINEAGE_MISMATCH:${verifiedTask.id || "unknown"}`,
    );
  }

  const primary = primarySource(verifiedTask);
  const trusted = new Set(
    list(fidelity.trusted_primary_source_asset_ids).map(text).filter(Boolean),
  );
  if (!primary || !trusted.has(primary)) {
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_TRUSTED_PRIMARY_SOURCE_REQUIRED:${verifiedTask.id || "unknown"}`,
    );
  }

  const asset = await CreativeAssetsRuntime.get(primary);
  if (!asset || text(asset.organization_id) !== text(verifiedTask.organization_id)) {
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_PRIMARY_ASSET_NOT_FOUND:${verifiedTask.id || "unknown"}:${primary}`,
    );
  }
  const provenance = CreativeBrandFidelityRuntime.classify(asset);
  if (!provenance.trusted_for_brand_fidelity_primary) {
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_PRIMARY_ASSET_UNTRUSTED:${verifiedTask.id || "unknown"}:${primary}`,
    );
  }

  const scope = object(verifiedTask.input?.requirements?.asset_scope);
  if (!CreativeShotAssetScopeRuntime.verify(scope)) {
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_ASSET_SCOPE_REQUIRED:${verifiedTask.id || "unknown"}`,
    );
  }
  if (
    ...
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_ASSET_SCOPE_MISMATCH:${verifiedTask.id || "unknown"}:${primary}`,
    );
  }

  const perceptualReviewNodeId = text(
    verifiedTask.input?.requirements?.perceptual_review_node_id ||
    verifiedTask.input?.provider_parameters?.perceptual_review_node_id ||
    verifiedTask.input?.generation?.provider_parameters?.perceptual_review_node_id ||
    verifiedTask.metadata?.perceptual_review_node_id,
  );
  if (
    ...
    throw new Error(
      `CREATIVE_BRAND_FIDELITY_POST_GENERATION_REVIEW_REQUIRED:${verifiedTask.id || "unknown"}`,
    );
  }

  verifiedTask = await ProductionTaskRuntime.update(verifiedTask.id, {
    metadata: {
      ...object(verifiedTask.metadata),
