...
function shotRoleHints(shot = {}, scene = {}) {
  ...
  return {
    ...
    product: list(shot.products || scene.products).length > 0 ||
      productVocabulary(shot, scene).some((token) => corpus.has(token)),
  };
}

// Product detection used to test a fixed hospitality word list
// ("dish", "food", "drink", "menu"), which meant a clinic, workshop or
// manufacturer had no product vocabulary at all while a restaurant had a
// privileged one. The vocabulary is now taken from the shot and scene themselves:
// whatever the declared products, categories and subjects are actually called.
// Only category-neutral structural words remain as a floor.
const NEUTRAL_PRODUCT_TOKENS = Object.freeze(["product", "item", "offering"]);

function productVocabulary(shot = {}, scene = {}) {
  const declared = [
    ...list(shot.products),
    ...list(scene.products),
    ...list(shot.subjects),
  ...
  const tokens = new Set(NEUTRAL_PRODUCT_TOKENS);

  for (const entry of declared) {
    const source =
      typeof entry === "string"
        ? entry
        : [entry?.name, entry?.label, entry?.title, entry?.category, entry?.type]
            .filter(Boolean)
            .join(" ");

    for (const word of normalizeWords(source)) tokens.add(word);
  }
