...
const APPROVAL_LITERAL = "APPROVE SOURCE SEMANTIC REPAIR MAX 8.736 THB";
const REPAIR_CONTRACT = "CREATIVE_SOURCE_SEMANTIC_REPAIR_EXECUTION_V1";
...
function validatePlan(plan = {}, approval = {}) {
  ...
  const approvedMaximum = finite(approval.maximum_thb);

  if (plan.contract !== "CREATIVE_SOURCE_SEMANTIC_REPAIR_PLAN_V1") {
  ...
  if (text(approval.literal) !== APPROVAL_LITERAL) {
    blockers.push("EXPLICIT_APPROVAL_LITERAL_INVALID");
  }
  if (approvedMaximum !== 8.736) {
    blockers.push("EXPLICIT_APPROVAL_MAXIMUM_INVALID");
  }
  if (money(plan.pricing?.approval_ceiling) !== approvedMaximum) {
    blockers.push("PLAN_APPROVAL_CEILING_MISMATCH");
  }
  if (text(plan.pricing?.currency).toUpperCase() !== "THB") {
    blockers.push("REPAIR_PLAN_CURRENCY_INVALID");
  }
  if (list(plan.work_items).length !== 19) {
    blockers.push("REPAIR_PLAN_WORK_ITEM_COUNT_INVALID");
  }
  if (plan.authorization?.production_authorized !== false) {
    blockers.push("REPAIR_PLAN_PRODUCTION_MUST_REMAIN_UNAUTHORIZED");
  }
  if (blockers.length) {
    throw new Error(`SOURCE_SEMANTIC_REPAIR_APPROVAL_BLOCKED:${blockers.join(",")}`);
  }

  return {
    contract: "CREATIVE_SOURCE_SEMANTIC_REPAIR_APPROVAL_V1",
    literal: approval.literal,
    maximum_thb: approvedMaximum,
...
Do not infer objects, people, actions, entrances, doors, furniture, food,
drinks, logos, venue features, identity, profession, demographics, location,
business activity, events, motion between frames, rights, consent, or intended
use unless they are visibly supported by this image. Do not use the filename as
visual evidence. Report uncertainty explicitly.
...
Rules:
- VERIFIED means the image was actually inspected, not that every object is known.
- Include only visible evidence. Empty arrays are correct when evidence is absent.
- A visible opening is not automatically a door. A wall sign is not automatically
  an entrance. A person near a table is not automatically staff or a guest.
- Sparse video frames do not prove motion or actions between frames.
...
async function selectedPreflight(plan, organizationId) {
  ...
  if (text(pricing.currency).toUpperCase() !== text(plan.pricing.currency).toUpperCase()) {
...
function itemRecord({ item, analysis, execution = null, recovered = false }) {
  return {
    contract: "CREATIVE_SOURCE_SEMANTIC_REPAIR_ITEM_V1",
    item_id: item.id,
    kind: item.kind,
    sample_index: item.sample_index,
    sample_fraction: item.sample_fraction,
    ...
    pricing_id:
      ...
      item.pricing_id,
    customer_price: money(execution?.pricing?.customer_price || 0),
...
function aggregateVideoAnalysis(asset, items, planHash) {
  ...
  if (ordered.length !== 3) {
    throw new Error(`SOURCE_VIDEO_SAMPLE_COUNT_INVALID:${asset.id}:${ordered.length}`);
  }
  ...
  return {
    ...
    verification_reason: allVerified
      ? "Three temporally distributed source-video frames were inspected"
      : "One or more sampled source-video frames lacked verified evidence",
    media_kind: "video",
    ...
    description: descriptions
      ...
      .join(" | "),
    summary: "Sparse temporal source evidence from frames sampled at 15%, 50%, and 85% of the video.",
    ...
    incompatible_uses: unique([
      ...
      "Do not infer unseen actions or scene geometry between sampled frames",
    ]),
    ...
    motion_characteristics: {
      ...
      sample_fractions: ordered.map((item) => item.sample_fraction),
    },
...
async function persistAssetAnalysis({ asset, finalAnalysis, plan, approval }) {
  ...
  const metadata = {
    ...
    semantic_repair_approval_maximum_thb: approval.maximum_thb,
...
async function persistItemProgress({ asset, items, plan, approval }) {
  const analysis = {
    ...
    semantic_repair: {
      contract: REPAIR_CONTRACT,
      plan_hash: plan.plan_hash,
      media_kind: list(Object.values(items)).some((item) =>
        item.kind === "VIDEO_FRAME_SEMANTIC_ANALYSIS",
      ) ? "video" : "image",
      items,
      completed: false,
      approval_maximum_thb: approval.maximum_thb,
      updated_at: new Date().toISOString(),
    },
  };
  const { data, error } = await supabaseAdmin
...
async function executeItem({ item, asset, image, plan, approval, attempt }) {
  const execution = await ServiceExecutionRuntime.execute({
    ...
    metadata: {
      module: "CREATIVE",
      operation: "SOURCE_SEMANTIC_REPAIR_V1",
      ...
      approved_maximum_thb: approval.maximum_thb,
...
export async function executeCreativeSourceSemanticRepair({
  ...
  const maximumCalls = Number(plan.counts.total_analysis_count || 19) + 1;
  const remainingCeiling = money(approval.maximum_thb - charged);
  if (charged > approval.maximum_thb) {
  ...
  try {
    for (const item of list(plan.work_items)) {
      ...
      if (item.kind === "IMAGE_SEMANTIC_ANALYSIS") {
        ...
        const framePath = path.join(
          tempRoot,
          `${asset.id}-${item.sample_index + 1}.jpg`,
        );
        extractVideoFrame({
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        ...
        if (projectedSpend > approval.maximum_thb) {
          throw new Error(
            `SOURCE_SEMANTIC_REPAIR_APPROVAL_CEILING_WOULD_BE_EXCEEDED:${projectedSpend}:${approval.maximum_thb}`,
          );
        }
        ...
        if (attempt === 1 && paidCallCount >= maximumCalls) break;
      }
    ...
    for (const assetId of ids) {
      let asset = await refreshAsset(assetId, plan.organization_id);
      ...
      const finalAnalysis = assetWorkItems[0]?.kind === "IMAGE_SEMANTIC_ANALYSIS"
        ...
        : aggregateVideoAnalysis(asset, items, plan.plan_hash);
    ...
    if (finalCharged > approval.maximum_thb) {
      throw new Error(
        `SOURCE_SEMANTIC_REPAIR_FINAL_CHARGE_EXCEEDS_APPROVAL:${finalCharged}:${approval.maximum_thb}`,
      );
    }
    ...
    const reportCore = {
      ...
      cost: {
        approval_ceiling: approval.maximum_thb,
        charged: finalCharged,
        remaining_authorized_amount: money(approval.maximum_thb - finalCharged),
