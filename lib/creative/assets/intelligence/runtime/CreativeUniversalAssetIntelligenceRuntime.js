...
function textAngleTags(asset = {}) {
  ...
  if (/\b(singing|performing|dancing|movement|walking|running)\b/.test(source)) tags.push("PERFORMANCE_BODY");
  return tags;
}
...
function personAsset(asset = {}) {
  ...
  return Boolean(
    ...
    object(asset.analysis?.identity).id ||
...
function assetRoles(asset = {}) {
  ...
  if (productObservations(asset).length || /\b(product|bottle|package|dish|food|drink|vehicle|equipment|furniture)\b/.test(source)) {
    roles.push("PRODUCT_IDENTITY_REFERENCE");
  }
  if (/\b(logo|wordmark|brand mark|brandmark|icon)\b/.test(source)) roles.push("BRAND_MARK_REFERENCE");
  if (/\b(location|venue|building|interior|exterior|restaurant|bar|hotel|office|factory|store|shop|room|landscape)\b/.test(source)) {
    roles.push("LOCATION_REFERENCE");
  }
  if (/\b(style|moodboard|palette|typography|material|texture|inspiration)\b/.test(source)) roles.push("STYLE_REFERENCE");
  ...
  return [...grouped.entries()].map(([key, group], index) => {
    ...
    return {
      ...
      body_reference_ids: references.filter((reference) =>
        ["FULL_BODY", "THREE_QUARTER_BODY", "HALF_BODY", "PERFORMANCE_BODY"]
          .includes(reference.body_coverage) ||
        reference.angles.includes("PERFORMANCE_BODY"),
      ).map((reference) => reference.asset_id),
...
export const CreativeUniversalAssetIntelligenceRuntime = {
  analyze({ project = {}, brief = {}, assets = [] } = {}) {
    ...
    return {
      contract: "UNIVERSAL_ASSET_INTELLIGENCE_V2",
