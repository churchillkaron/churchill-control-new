...
function assetNodeKey(key = "") {
  const value = text(key).toLowerCase();
  // Approval records are governance evidence stored in the asset graph, not media
  // that a provider may consume. Treating them as shot assets causes a sealed,
  // approved production to fail least-privilege isolation before dispatch.
  if (/(?:^|_)approval_record_asset_node_ids?$/.test(value)) return false;
  return /(?:^|_)asset_node_ids?$/.test(value);
}
...
export const CreativeShotAssetScopeRuntime = {
  build({ node, graph_nodes = [], edges = [], project_asset_ids = [] } = {}) {
    ...
    const reviewNode = reviewOrValidationNode(node);
    if (sourceAssetIds.length && !primarySourceAssetId && !reviewNode) {
    ...
    const scope = {
      ...
      source_binding_contract: primarySourceAssetId
        ...
        : reviewNode
          ? "EXPLICIT_REVIEW_REFERENCE_SCOPE_V1"
          : null,
      ...
      access_policy: {
        ...
        provider_input_mode: primarySourceAssetId
          ? "EXPLICIT_PRIMARY_SOURCE_ONLY"
          : reviewNode
            ? "EXPLICIT_REVIEW_REFERENCES_AND_DEPENDENCIES_ONLY"
            : "NO_SOURCE_ASSET",
