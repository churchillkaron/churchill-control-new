...
export const AssetReuseEngine = {
  async resolveNode(node, organization_id, reusableCandidates = null) {
    ...
    const reusable = Array.isArray(reusableCandidates)
      ? reusableCandidates
  ...
  async optimizeGraph({
    ...
    for (const node of nodes) {
      if (!node.generation?.required) continue;
      const policy = reusePolicy(node);
      if (!policy.enabled || reusableByType.has(policy.asset_node_type)) continue;
      reusableByType.set(
        policy.asset_node_type,
        await CreativeAssetGraphRuntime.findReusable({
          organization_id,
    ...
    graph.nodes = await Promise.all(
      nodes.map((node) => {
        if (!node.generation?.required) return node;
        const policy = reusePolicy(node);
        const reusable = policy.enabled
          ? reusableByType.get(policy.asset_node_type) || []
          : null;
        return this.resolveNode(node, organization_id, reusable);
      }),
    );
    return graph;
  },
