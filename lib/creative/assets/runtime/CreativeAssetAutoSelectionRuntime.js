...
const DELIVERY_TERMS = [
  "campaign design", "campaign package", "content package", "facebook feed",
  "instagram feed", "social feed", "social post", "facebook post",
  "instagram post", "poster", "flyer", "banner", "thumbnail", "storyboard",
...
function mediaKind(asset = {}, node = {}) {
  ...
  if (mime.startsWith("video/") || declared.includes("video") || ["mp4", "mov", "m4v", "webm", "mkv", "avi"].includes(ext)) {
    return "VIDEO";
  }
  if (mime.startsWith("audio/") || declared.includes("audio") || ["mp3", "wav", "m4a", "aac", "flac", "ogg"].includes(ext)) {
    return "AUDIO";
  }
  if (mime.startsWith("image/") || declared.includes("image") || declared.includes("logo") || ["jpg", "jpeg", "png", "webp", "gif", "avif", "heic", "heif"].includes(ext)) {
    return "IMAGE";
  }
  return "FILE";
}
...
function roles(asset = {}, node = {}) {
  const source = semanticText(asset, node);
...
function score(candidate, { intent, organization, preferredRole = null } = {}) {
  const source = semanticText(candidate.asset, candidate.node);
