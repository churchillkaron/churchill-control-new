...
export function reconcileVerifiedBrandAssetPlan({ plan = {}, project = {}, assets = [] } = {}) {
  ...
  if (manifest.some((entry) => text(entry?.asset_id || entry?.id || entry) === bindingId)) return plan;
  const assignments = list(plan.deliverables).map((deliverable) => text(deliverable?.id)).filter(Boolean);
