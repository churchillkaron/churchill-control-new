...
export function reconcileVerifiedBrandAssetPlan({ plan = {}, project = {}, assets = [] } = {}) {
  ...
  if (!bindingId) return plan;
  const boundAsset = list(assets).find((asset) => text(asset?.id || asset?.asset_id) === bindingId);
  if (!boundAsset || !authoritativeBrandAsset(boundAsset)) return plan;

  const reconciledPlan = reconcileLogoSourceReferences(structuredClone(plan), bindingId);
  const manifest = list(reconciledPlan.asset_manifest);
  if (manifest.some((entry) => text(entry?.asset_id || entry?.id || entry) === bindingId)) return reconciledPlan;
  const assignments = list(reconciledPlan.deliverables).map((deliverable) => text(deliverable?.id)).filter(Boolean);
  if (!assignments.length) return reconciledPlan;
