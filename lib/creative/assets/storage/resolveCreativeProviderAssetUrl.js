...
import { CreativeAssetsRuntime } from "../runtime/CreativeAssetsRuntime.js";

const STORAGE_PREFIX = "storage://";
const DEFAULT_PROVIDER_INPUT_URL_TTL_SECONDS = 900;
const MAX_PROVIDER_INPUT_URL_TTL_SECONDS = 3600;
...
function imageDataUri(value) {
  ...
  if (!/^data:image\/(?:jpe?g|png|webp);base64,/i.test(source)) return null;
  const separator = source.indexOf(",");
  const payload = source.slice(separator + 1);
  if (!payload || !/^[A-Za-z0-9+/]+={0,2}$/.test(payload)) {
    throw new Error("CREATIVE_PROVIDER_INPUT_IMAGE_DATA_URI_INVALID");
  }
  return source;
}
...
function assetId(value) {
  ...
  if (!value || typeof value !== "object") return "";
  const source = text(value.asset_id || value.assetId || value.id);
...
function providerInputTtl() {
  ...
  const expiresIn = configured || DEFAULT_PROVIDER_INPUT_URL_TTL_SECONDS;
  if (expiresIn > MAX_PROVIDER_INPUT_URL_TTL_SECONDS) {
    throw new Error("CREATIVE_PROVIDER_INPUT_URL_TTL_SECONDS_TOO_LARGE");
  }
  return expiresIn;
}
...
async function resolveDirectReference({ organization_id, direct }) {
  const inlineImage = imageDataUri(direct);
