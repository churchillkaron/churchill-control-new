...
from "../storage/uploadCreativeAsset.js";
import { analyzeCreativeAsset }
from "../intelligence/analyzeCreativeAsset.js";
import { saveCreativeAsset }
from "../repositories/saveCreativeAsset.js";
import { analyzeCreativeSubject }
from "../intelligence/analyzeCreativeSubject.js";
import {
  creativeAssetNodeIntelligence,
  creativeAssetSemanticEvidence,
} from "../intelligence/CreativeAssetSemanticEvidenceRuntime.js";
import { calculateAssetScore }
from "../../../ai/scoring/calculateAssetScore.js";
import { getOrCreateBusinessProfile }
from "../../../ai/profiles/getOrCreateBusinessProfile.js";
import {
  CreativeMediaInspectionRuntime,
} from "../../media/runtime/CreativeMediaInspectionRuntime.js";
import {
  CreativeAssetGraphRuntime,
} from "../graph/runtime/CreativeAssetGraphRuntime.js";
import {
  CREATIVE_ASSET_NODE_STATUS,
  CREATIVE_ASSET_NODE_TYPES,
} from "../graph/documents/CreativeAssetNode.js";
import {
  CreativeFullSongContractRuntime,
} from "../../audio/runtime/CreativeFullSongContractRuntime.js";
import {
  createCreativeFontAssetFlow,
} from "../fonts/createCreativeFontAssetFlow.js";
...
function graphNodeType(mediaKind, assetType) {
  ...
  if (/\b(song|music|soundtrack|master track)\b/i.test(type)) {
    return CREATIVE_ASSET_NODE_TYPES.MUSIC;
  }
...
export async function createCreativeAssetFlow({
  ...
  if (text(assetType).toLowerCase() === "font") {
    return createCreativeFontAssetFlow({
      organizationId,
      pageId,
      creativeMissionId,
      creativeProjectId,
  ...
  const businessProfile = await getOrCreateBusinessProfile({ organizationId });
  ...
  const semantic = creativeAssetSemanticEvidence({ analysis });
  ...
  const enrichedAnalysis = {
    ...analysis,
    semantic_evidence_contract: semantic.contract,
    semantic_evidence_count: semantic.evidence_count,
    semantic_verified: semantic.verified,
  ...
  const asset = await saveCreativeAsset({
    ...
    thumbnailUrl: null,
    aiSuggestedType: semantic.verified
      ? text(analysis.scene_type) || null
      : null,
    score,
    analysis: { ...enrichedAnalysis, score },
    metadata: {
      ...
      size_bytes: upload.size_bytes,
      checksum_sha256: upload.checksum_sha256,
      analysis_status: semantic.verified ? "VERIFIED" : semantic.status,
      semantic_analysis_status: semantic.status,
      semantic_evidence_count: semantic.evidence_count,
  ...
  const assetNode = await CreativeAssetGraphRuntime.create({
    ...
    technical,
    intelligence: creativeAssetNodeIntelligence(enrichedAnalysis, score),
    reuse: {
      reusable: false,
    ...
    review: {
      ai_reviewed: semantic.verified,
      human_reviewed: false,
      approved: false,
      approved_by: null,
      notes: semantic.verified
        ? "Semantic evidence verified; reuse remains blocked pending explicit human rights and consent approval."
        : "Semantic evidence missing or unverified; production and reuse are blocked.",
    },
    metadata: {
      page_id: pageId,
  ...
  const fullSongContract = await CreativeFullSongContractRuntime.apply({
    organization_id: organizationId,
    creative_project_id: creativeProjectId,
    asset,
  ...
  return {
    success: true,
    asset,
    asset_node: fullSongContract.asset_node || assetNode,
    full_song_contract: fullSongContract,
    upload: {
      bucket: upload.bucket,
      path: upload.path,
