...
function deliveryArgs(delivery, sourcePath, outputPath, master) {
  const rate = delivery.sample_rate || master.sample_rate;
  const args = ["-y", "-i", sourcePath, "-ar", String(rate), "-ac", String(delivery.channels || master.channels)];
  switch (delivery.format) {
    case "wav": {
      const codec = delivery.codec || "pcm_s24le";
      if (codec === "pcm_s24le") args.push("-af", `aresample=${rate}:osf=s32:output_sample_bits=24:dither_method=triangular`, "-c:a", codec);
      else args.push("-c:a", codec);
      args.push("-rf64","auto");
      if (delivery.metadata?.bwf?.enabled === true) { const bwf=delivery.metadata.bwf; args.push("-write_bext","1","-metadata",`description=${text(bwf.description||"Avantiqo Professional Audio Engine")}`,"-metadata",`originator=${text(bwf.originator||"Avantiqo")}`,"-metadata",`originator_reference=${text(bwf.originator_reference||"")}`,"-metadata",`time_reference=${Math.max(0,Math.round(Number(bwf.time_reference_samples)||0))}`); }
      break;
    }
...
export const CreativeAudioFinishingRuntime = {
  async finish(task) {
    ...
    try {
      ...
      const masterPath = path.join(root, "master-float.wav");
      ...
      await run(ffmpeg, [
        ...
        "-ar", String(contract.master.sample_rate), "-ac", String(contract.master.channels), "-c:a", "pcm_f32le", masterPath,
      ]);
      ...
      const report = {
        contract: "AVANTIQO_AUDIO_MASTER_REPORT_V2",
        master_id: identity,
        ...
        internal_master_format: "float32",
        final_integer_pcm_conversion_only: true,
        deliveries: files.map((file, index) => ({ name: file.name, url: file.url, checksum: file.checksum, probe: file.probe, dither_applied: deliveries[index]?.format === "wav" && (deliveries[index]?.codec || "pcm_s24le") === "pcm_s24le", dither_method: deliveries[index]?.format === "wav" && (deliveries[index]?.codec || "pcm_s24le") === "pcm_s24le" ? "TPDF_TRIANGULAR" : null })),
