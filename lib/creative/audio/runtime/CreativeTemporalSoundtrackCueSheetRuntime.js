...
function shotCue({ scene, shot, sceneIndex, shotIndex, startSeconds }) {
  ...
  return {
    ...
    sync_events: list(audio.sync_events).map((entry) => ({ at_seconds: finite(entry?.at_seconds), event: text(entry?.event), priority: text(entry?.priority).toUpperCase() || "SUPPORT" })).filter((entry) => entry.at_seconds !== null && entry.event),
    ...
    dramaturgy: {
      scene_type: text(scene.audio_dramaturgy?.scene_type) || null,
      picture_is_timing_authority: scene.audio_dramaturgy?.picture_is_timing_authority === true,
      music_is_not_constant_bed: scene.audio_dramaturgy?.music_is_not_constant_bed === true,
      foreground_physical_sound_may_override_score:
        scene.audio_dramaturgy?.foreground_physical_sound_may_override_score === true,
      silence_or_density_drop_required:
        scene.audio_dramaturgy?.silence_or_density_drop_required === true,
      proximity_sound_required: scene.audio_dramaturgy?.proximity_sound_required === true,
    },
  };
}
...
function cueSheet(input = {}) {
  ...
  const sheet = {
    ...
    mix_governance: {
      ...
      picture_locked_sfx_required: true,
      music_may_not_operate_as_constant_background_bed: true,
      foreground_physical_sound_may_override_music: true,
      scene_audio_dramaturgy_is_binding: true,
      picture_cut_and_sync_events_are_timing_authority: true,
      music_intensity_must_follow_energy_curve: true,
      sfx_density_must_follow_energy_curve: true,
      silence_is_a_designed_dynamic_event: true,
      spatial_soundfield_required_for_premium_temporal: true,
      foreground_sync_events_center_or_picture_motivated: true,
      environment_beds_may_use_stereo_width_and_depth: true,
      transition_motion_may_pan_only_when_picture_motivated: true,
      surround_derivatives_must_preserve_same_spatial_cue_graph: true,
    },
