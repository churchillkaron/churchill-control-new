...
function soundtrackPrompt(plan = {}, duration = 0) {
  ...
  return [
    `Create an original ${duration}-second instrumental editorial soundtrack for the approved commercial film.`,
    ...
    "The music must support dialogue, environmental ambience, action-synchronised effects and editorial transitions without masking them.",
...
function soundtrackNode(input = {}, duration = 0) {
  ...
  return createProductionNode({
    ...
    description: "Generate the exact-duration original score used by the final approved film mix.",
    duration_seconds: duration,
    priority: 20,
    intent: {
      purpose: "Carry the full emotional and editorial arc while leaving space for authentic environmental sound.",
  ...
  ProductionGraphRuntime.plan = async function planWithTemporalSoundtrack(input = {}) {
    ...
    return ProductionGraphRuntime.update(graph.id, {
      ...
      metadata: {
        ...
        temporal_soundtrack_industry_neutral: true,
      },
    });
  };
}
