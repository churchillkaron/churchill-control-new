...
function explicitNotRequired(value) {
  if (value === false) return true;
  if (typeof value === "string") return /^(NOT_REQUIRED|N\/A|NONE)$/i.test(text(value));
  const row = object(value);
  return row.required === false &&
    /^(NOT_REQUIRED|N\/A|NONE)$/i.test(text(row.status || row.disposition || row.decision));
}
function meaningfulObject(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  return Object.values(value).some((entry) => {
    if (typeof entry === "string") return meaningful(entry);
    if (typeof entry === "number") return Number.isFinite(entry);
    if (typeof entry === "boolean") return entry === true;
    if (Array.isArray(entry)) return entry.some(Boolean);
    if (entry && typeof entry === "object") return meaningfulObject(entry);
    return false;
  });
}
function disciplineDeclared(value) {
  if (explicitNotRequired(value)) return true;
  if (Array.isArray(value)) return value.length > 0;
  return meaningfulObject(value);
}
function shotBibleFor(task = {}, shot = {}) {
  return object(
    shot.metadata?.shot_bible_source ||
    task.input?.requirements?.shot_bible_source ||
    task.input?.shot_bible ||
    task.metadata?.shot_bible,
  );
}
function soundDesignComplete(value) {
  if (explicitNotRequired(value)) return true;
  const row = object(value);
  return [row.mix_intent, row.source_sound, row.silence, row.sound_events, row.sync_events]
    .some((entry) => Array.isArray(entry) ? entry.length > 0 : meaningful(entry));
}
...
export const CreativePreProductionIntelligenceCertificationRuntime = Object.freeze({
  ...
  async inspect({ organization_id, creative_project_id, production_graph_id = null } = {}) {
    ...
    const [project, allTasks, allShots, briefs, researchReports] = await Promise.all([
      ...
      ResearchRepository.listCertificationMetadata({
        organization_id,
        creative_project_id,
      }),
    ]);
    ...
    const planned = tasks.map((task) => {
      ...
      const shotBible = shotBibleFor(task, shot);
      return { task, shot, decision, craft, shotBible, grounding: groundingState.grounding, groundingRequired: groundingState.required, duration, order: shotOrder(task, shot) };
    ...
    const soundMissing = planned.filter(({ decision, craft, shotBible }) =>
      !meaningful(decision.audio_direction?.mix_intent) ||
      !meaningful(craft.sound_sync_point) ||
      !soundDesignComplete(shotBible.sound_design),
    );
    const coverageMissing = planned.filter(({ shot, shotBible }) =>
      !disciplineDeclared(shot.coverage || shot.metadata?.coverage || shotBible.coverage) ||
      !disciplineDeclared(shot.metadata?.scene_coverage_plan || shotBible.scene_coverage_plan) ||
      !disciplineDeclared(shot.metadata?.cinematic_coverage || shotBible.cinematic_coverage),
    );
    const worldBibleMissing = planned.filter(({ shotBible }) =>
      !disciplineDeclared(shotBible.world) ||
      !meaningful(shotBible.time_of_day) ||
      !meaningful(shotBible.weather) ||
      !meaningful(shotBible.materials_surfaces),
    );
    const subjectTruthMissing = planned.filter(({ shotBible }) =>
      !disciplineDeclared(shotBible.subject_truth),
    );
    const vfxPlanningMissing = planned.filter(({ shot, shotBible }) =>
      !disciplineDeclared(shot.vfx || shotBible.vfx),
    );
    const simulationPlanningMissing = planned.filter(({ shot, shotBible }) =>
      !disciplineDeclared(shot.simulation || shotBible.simulation),
    );
    const compositingPlanningMissing = planned.filter(({ shotBible }) =>
      !disciplineDeclared(shotBible.compositing),
    );
    ...
    const currentFullMasterHashes = [...new Set(
      planned.map(({ task, shot }) => text(shot.metadata?.full_master_hash || task.metadata?.full_master_hash)).filter(Boolean),
    )];
    const currentFullMasterHash = currentFullMasterHashes.length === 1
      ? currentFullMasterHashes[0]
      : null;
    const scopedTimelineShots = allShots.filter((shot) =>
      !shot.archived_at &&
      Boolean(text(shot.metadata?.investor_first_minute_scope || shot.metadata?.first_minute_scope)) &&
      (!currentFullMasterHash || text(shot.metadata?.full_master_hash) === currentFullMasterHash),
    );
    const generatedDuration = planned.reduce((sum, entry) => sum + entry.duration, 0);
    const scopedTimelineDuration = scopedTimelineShots.reduce(
      (sum, shot) => sum + (finite(shot.duration_seconds) || 0),
      0,
    );
    const totalDuration = firstMinute && scopedTimelineShots.length
      ? scopedTimelineDuration
      : generatedDuration;
    ...
    const checks = [
      ...
      check("coverage_plan_complete", coverageMissing.length === 0, coverageMissing.map((entry) => entry.task.id), "PREPRODUCTION_COVERAGE_PLAN_INCOMPLETE"),
      check("world_bible_complete", worldBibleMissing.length === 0, worldBibleMissing.map((entry) => entry.task.id), "PREPRODUCTION_WORLD_BIBLE_INCOMPLETE"),
      check("subject_truth_complete", subjectTruthMissing.length === 0, subjectTruthMissing.map((entry) => entry.task.id), "PREPRODUCTION_SUBJECT_TRUTH_INCOMPLETE"),
      check("vfx_plan_declared", vfxPlanningMissing.length === 0, vfxPlanningMissing.map((entry) => entry.task.id), "PREPRODUCTION_VFX_PLAN_UNDECLARED"),
      check("simulation_plan_declared", simulationPlanningMissing.length === 0, simulationPlanningMissing.map((entry) => entry.task.id), "PREPRODUCTION_SIMULATION_PLAN_UNDECLARED"),
      check("compositing_plan_declared", compositingPlanningMissing.length === 0, compositingPlanningMissing.map((entry) => entry.task.id), "PREPRODUCTION_COMPOSITING_PLAN_UNDECLARED"),
    ...
    return {
      ...
      evidence: {
        ...
        generated_media_duration_seconds: Number(generatedDuration.toFixed(3)),
        scoped_timeline_shot_count: scopedTimelineShots.length,
        scoped_timeline_duration_seconds: Number(scopedTimelineDuration.toFixed(3)),
        scoped_full_master_hash: currentFullMasterHash,
