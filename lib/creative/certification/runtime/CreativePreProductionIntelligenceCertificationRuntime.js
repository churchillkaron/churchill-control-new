...
export const CreativePreProductionIntelligenceCertificationRuntime = Object.freeze({
  ...
  async inspect({ organization_id, creative_project_id } = {}) {
    ...
    const [project, allTasks, allShots] = await Promise.all([
    ...
    const tasks = allTasks.filter(active).filter(temporalShotTask);
    if (!tasks.length) {
      return {
        contract: CREATIVE_PREPRODUCTION_INTELLIGENCE_CERTIFICATION_CONTRACT,
        required: false,
    ...
    const purposes = planned.map(({ decision }) => normalized(decision.dramatic_objective)).filter(Boolean);
