...
import {
  CreativeColorManagementAuthorityRuntime,
} from "@/lib/creative/color/runtime/CreativeColorManagementAuthorityRuntime";
...
function namedInputProfile({ project, exportProfile, inputColor } = {}) {
  const projectColor = object(project.metadata?.color_management);
  const profileColor = object(exportProfile.color_management || exportProfile.colorManagement);
  const explicit = text(
    profileColor.input_profile ||
    profileColor.inputProfile ||
    projectColor.input_profile ||
    projectColor.inputProfile,
    100,
  ).toUpperCase();
  if (explicit) return explicit;
  const transfer = text(inputColor?.transfer, 100).toLowerCase();
  const primaries = text(inputColor?.primaries, 100).toLowerCase();
  if (["bt709", "iec61966-2-1", "srgb"].includes(transfer)) {
    return transfer === "srgb" || transfer === "iec61966-2-1" ? "SRGB" : "REC709";
  }
  if (transfer.includes("linear") && primaries.includes("bt709")) return "LINEAR_SRGB";
  throw new Error(`COLOR_NAMED_INPUT_PROFILE_REQUIRED:${transfer || "UNKNOWN"}:${primaries || "UNKNOWN"}`);
}
...
function inheritedMasterMetadata(baseRender = {}) {
  ...
  return {
    ...
    audio_studio_qc_sealed:
      metadata.audio_studio_qc_sealed === true,
    audio_studio_qc_seal_hash:
      metadata.audio_studio_qc_seal_hash || null,
    audio_studio_finish_task_id:
      metadata.audio_studio_finish_task_id || null,
    audio_studio_master_id:
      metadata.audio_studio_master_id || null,
    audio_studio_master_checksum:
      metadata.audio_studio_master_checksum || null,
...
export const CreativeColorFinishingRuntime = Object.freeze({
  ...
  async finish({
    ...
    try {
      ...
      const authority = CreativeColorManagementAuthorityRuntime.resolve({
        input_profile: namedInputProfile({
          project,
          exportProfile: export_profile,
          inputColor,
        }),
        output_target: target.id,
        ocio_config_id:
          export_profile.color_management?.ocio_config_id ||
          export_profile.colorManagement?.ocioConfigId ||
          project.metadata?.color_management?.ocio_config_id ||
          null,
        creative_look_id:
          export_profile.color_management?.creative_look_id ||
          project.metadata?.color_management?.creative_look_id ||
          null,
      });
      if (authority.status !== "READY") {
        throw new Error(`COLOR_MANAGEMENT_AUTHORITY_BLOCKED:${authority.blockers.join(",")}`);
      }
      ...
      const identity = planIdentity({
        ...
        colorManagementAuthorityHash: authority.color_pipeline_hash,
      });
      ...
      const preliminaryNode = createCreativeAssetNode({
        ...
        metadata: {
          ...
          color_management_authority_contract: authority.contract,
          color_management_pipeline_hash: authority.color_pipeline_hash,
          color_management_input_profile: authority.input?.id || null,
          color_management_working_space: authority.working_space?.id || null,
          color_management_output_target: authority.output?.id || null,
          color_management_single_final_authority: true,
