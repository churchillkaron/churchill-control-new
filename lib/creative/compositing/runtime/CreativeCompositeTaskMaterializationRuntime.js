...
function byTask(nodes=[],taskId,shotId=null){
  const symbolic=text(taskId).toUpperCase();
  if(symbolic==="AUTO_MULTIPASS:ATMOSPHERE"){
    return nodes.find(n=>text(n.metadata?.shot_id)===text(shotId)&&text(n.metadata?.pass_id)==="atmosphere"&&n.metadata?.blocked!==true)||null;
  }
  if(symbolic==="AUTO_MULTIPASS:THREAT_HERO_LAYER"){
    return nodes.find(n=>text(n.metadata?.shot_id)===text(shotId)&&text(n.metadata?.pass_id)==="threat-hero-layer"&&n.metadata?.blocked!==true)||null;
  }
  return nodes.find(n=>text(n.production_task_id||n.metadata?.production_task_id)===text(taskId)&&n.metadata?.blocked!==true)||null;
}
function assetForLayer(layer={},nodes=[],shotId){if(layer.layer_role==="BASE_PLATE"||layer.auto_resolved_selected_candidate===true)return layer.asset_node_id?nodes.find(n=>n.id===layer.asset_node_id)||null:layer.production_task_id?byTask(nodes,layer.production_task_id,shotId):selectedBase(nodes,shotId);if(layer.asset_node_id)return nodes.find(n=>n.id===layer.asset_node_id)||null;if(layer.production_task_id)return byTask(nodes,layer.production_task_id,shotId);return null;}
