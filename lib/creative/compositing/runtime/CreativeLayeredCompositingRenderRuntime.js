...
function verifyGovernedSource(layer, node, foundationAuthority = null) {
  ...
  const foundationBoundImageAsset = [
    ...
    "COMPOSITING_SOURCE",
    "STORYBOARD_FRAME",
  ].includes(imageAssetClass);
  if (foundationBoundImageAsset) {
    if (node.metadata?.release_approved !== true) {
      blockers.push("COMPOSITING_IMAGE_STUDIO_RELEASE_APPROVAL_REQUIRED");
    }
    if (node.metadata?.image_asset_perceptual_qc_sealed !== true) {
      blockers.push("COMPOSITING_IMAGE_STUDIO_PERCEPTUAL_QC_REQUIRED");
    }
    if (node.metadata?.image_asset_pack_qc_sealed !== true) {
      blockers.push("COMPOSITING_IMAGE_STUDIO_PACK_QC_REQUIRED");
    }
    if (foundationAuthority?.passed !== true) {
      blockers.push("COMPOSITING_IMAGE_STUDIO_FOUNDATION_AUTHORITY_REQUIRED");
    } else if (
      text(node.metadata?.image_foundation_authority_digest) !==
      text(foundationAuthority.foundation_authority_digest)
    ) {
      blockers.push("COMPOSITING_IMAGE_STUDIO_FOUNDATION_STALE");
    }
  }
  ...
  if (["VFX_ELEMENT", "SET_EXTENSION", "SCREEN_INSERT", "ATMOSPHERE", "LIGHTING_PASS", "REFLECTION_SHADOW", "BEAUTY_CLEANUP"].includes(layer.layer_role)) {
    const vfxApplicable =
...
export const CreativeLayeredCompositingRenderRuntime = Object.freeze({
  ...
  async render({
    ...
    for (const layer of contract.layers) {
      ...
      for (const blocker of verifyGovernedSource(layer, node, foundationAuthority)) {
