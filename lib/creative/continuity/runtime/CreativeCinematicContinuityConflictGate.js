...
function cinematicContractFailures(task = {}) {
  const requirements = object(task.input?.requirements);
  const bible = object(requirements.continuity_bible);
  const grammar = object(requirements.cinematic_grammar);
  const beauty = object(requirements.beauty_system);
  const subjectLock = object(requirements.persistent_subject_lock);
  const failures = [];
  if (!Object.keys(bible).length) failures.push("CONTINUITY_BIBLE_REQUIRED");
  if (!Object.keys(grammar).length) failures.push("CINEMATIC_GRAMMAR_REQUIRED");
  if (!Object.keys(beauty).length) failures.push("CINEMATIC_BEAUTY_SYSTEM_REQUIRED");
  if (text(subjectLock.contract) !== "CREATIVE_PERSISTENT_SUBJECT_LOCK_V1") {
    failures.push("PERSISTENT_SUBJECT_LOCK_REQUIRED");
  }
  for (const field of [
    "world_identity",
    "hero_subject_identity",
    "location_identity",
    "lighting_state",
    "spatial_map",
  ]) {
    if (!text(bible[field])) failures.push(`CONTINUITY_BIBLE_${field.toUpperCase()}_REQUIRED`);
  }
  for (const field of [
    "visual_hierarchy", "negative_space", "depth_strategy", "light_strategy",
    "colour_strategy", "material_strategy", "lens_strategy",
  ]) {
    if (!text(beauty[field])) failures.push(`CINEMATIC_BEAUTY_${field.toUpperCase()}_REQUIRED`);
  }
  if (!text(requirements.beauty_intent)) failures.push("SHOT_BEAUTY_INTENT_REQUIRED");
  if (!text(requirements.focal_path)) failures.push("SHOT_FOCAL_PATH_REQUIRED");
  if (!text(requirements.depth_layers)) failures.push("SHOT_DEPTH_LAYERS_REQUIRED");
  ...
  for (const field of [
    ...
    if (!text(grammar[field])) failures.push(`CINEMATIC_GRAMMAR_${field.toUpperCase()}_REQUIRED`);
  }
  if (list(grammar.reveal_ladder).length < 3) failures.push("CINEMATIC_REVEAL_LADDER_REQUIRED");
  if (!text(requirements.reveal_stage)) failures.push("SHOT_REVEAL_STAGE_REQUIRED");
  if (!text(requirements.mystery_function)) failures.push("SHOT_MYSTERY_FUNCTION_REQUIRED");
  if (!text(requirements.inherited_world_state)) failures.push("SHOT_INHERITED_WORLD_STATE_REQUIRED");
  if (text(subjectLock.contract) === "CREATIVE_PERSISTENT_SUBJECT_LOCK_V1") {
    ...
    if (text(subjectLock.lock_hash) !== hash(expectedCanonical)) {
      failures.push("PERSISTENT_SUBJECT_LOCK_HASH_MISMATCH");
    }
    if (text(subjectLock.scene_id) && text(task.scene_id) && text(subjectLock.scene_id) !== text(task.scene_id)) {
      failures.push("PERSISTENT_SUBJECT_LOCK_SCENE_MISMATCH");
    }
  }
...
async function bindOrReject(task = {}) {
  ...
  const contractFailures = cinematicContractFailures(task);
  if (contractFailures.length) {
    return ProductionTaskRuntime.fail(
      task.id,
      new Error(`CREATIVE_CINEMATIC_CONTRACT_INCOMPLETE:${contractFailures.join(",")}`),
      {
        cinematic_contract_failures: contractFailures,
        provider_submission_blocked: true,
        gpu_spend_blocked: true,
        cinematic_contract_checked_before_provider_submission: true,
      },
    );
  }
