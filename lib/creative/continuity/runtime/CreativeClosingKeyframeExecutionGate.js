...
function reviewPassed(task = {}) {
  ...
  return evidence.passed === true &&
    ...
    (
      ...
      (
        evidence.camera_authority_preserved === true &&
        evidence.continuity_state_preserved === true &&
        evidence.material_truth_preserved === true &&
        evidence.performance_state_preserved === true
      )
    ) &&
...
async function prepareClosingKeyframe(task = {}) {
  ...
  let imageStudioPackage = null;
  let compiledReferences = null;
  if (task.metadata?.image_studio_closing_authority_required === true) {
    const assetNodes = await CreativeAssetGraphRuntime.list({
      organization_id: task.organization_id,
      creative_project_id: task.creative_project_id,
    });
    imageStudioPackage = CreativeImageProductionPackageRuntime.build({
      task,
      asset_nodes: assetNodes,
      previsualization_authority: task.input?.requirements?.previsualization_blueprint || null,
    });
    if (imageStudioPackage.passed !== true) {
      throw new Error(
        `CLOSING_KEYFRAME_IMAGE_STUDIO_PACKAGE_BLOCKED:${imageStudioPackage.failures.join(",")}`,
      );
    }
    const sourceAssets = [
      imageStudioPackage.hero?.url
        ? { url: imageStudioPackage.hero.url, role: "IMAGE_STUDIO_SELECTED_HERO_FRAME", asset_node_id: imageStudioPackage.hero.asset_node_id }
        : null,
      imageStudioPackage.continuity?.url
        ? { url: imageStudioPackage.continuity.url, role: "IMAGE_STUDIO_CONTINUITY_REFERENCE", asset_node_id: imageStudioPackage.continuity.asset_node_id }
        : null,
      imageStudioPackage.performance_reference?.url
        ? { url: imageStudioPackage.performance_reference.url, role: "IMAGE_STUDIO_PERFORMANCE_REFERENCE", asset_node_id: imageStudioPackage.performance_reference.asset_node_id }
        : null,
      imageStudioPackage.contact_detail_reference?.url
        ? { url: imageStudioPackage.contact_detail_reference.url, role: "IMAGE_STUDIO_CONTACT_DETAIL_REFERENCE", asset_node_id: imageStudioPackage.contact_detail_reference.asset_node_id }
        : null,
      ...list(imageStudioPackage.character_multiview_assets).map((item) => ({
        url:item.url,
        role:`IMAGE_STUDIO_CHARACTER_${text(item.view_id||"VIEW")}`,
        asset_node_id:item.asset_node_id,
      })),
      ...list(imageStudioPackage.threat_multiview_assets).map((item) => ({
        url:item.url,
        role:`IMAGE_STUDIO_THREAT_${text(item.view_id||"VIEW")}`,
        asset_node_id:item.asset_node_id,
      })),
      ...list(imageStudioPackage.material_truth_assets).map((item) => ({
        url: item.url,
        role: `IMAGE_STUDIO_MATERIAL_TRUTH_${text(item.material_truth_key || "REFERENCE")}`,
        asset_node_id: item.asset_node_id,
      })),
    ].filter(Boolean);
    const endpointReferences = references.map((value) => ({
      url: typeof value === "string" ? value : value?.url,
      role: "CLOSING_KEYFRAME_REFERENCE",
    })).filter((value) => value.url);
    compiledReferences = CreativeImageProviderReferenceCompilerRuntime.compile({
      task,
      source_assets: [
        ...sourceAssets,
    ...
    if (compiledReferences.provider_transport_safe !== true) {
      throw new Error(
        "CLOSING_KEYFRAME_REFERENCE_TRANSPORT_UNSAFE:" +
        [
          ...list(compiledReferences.missing_mandatory_roles),
          ...list(compiledReferences.missing_mandatory_groups),
        ].join(","),
      );
    }
  }
  const preparedSourceAssets = compiledReferences?.source_assets || [
    ...list(task.input?.source_assets),
  ...
  return ProductionTaskRuntime.update(task.id, {
    input: {
      ...object(task.input),
      reference_images: preparedSourceAssets.map((item) => item.url).filter(Boolean),
      source_assets: preparedSourceAssets,
      image_production_package: imageStudioPackage,
      image_provider_reference_manifest: compiledReferences,
      generation: {
        ...
        provider_parameters: {
          ...
          image_camera_authority:
            imageStudioPackage?.camera_authority || null,
          closing_camera_state:
            imageStudioPackage?.camera_authority?.end || null,
          image_production_package_digest:
            imageStudioPackage?.production_package_digest || null,
          input_fidelity: "high",
        },
      },
      provider_parameters: {
    ...
    metadata: {
      ...
      closing_keyframe_reference_count: preparedSourceAssets.length,
      closing_keyframe_reference_transport_safe:
        compiledReferences?.provider_transport_safe !== false,
      closing_keyframe_reference_omitted_count:
        Number(compiledReferences?.omitted_references?.length || 0),
      image_studio_closing_package_bound: Boolean(imageStudioPackage?.passed),
      image_studio_closing_package_digest:
        imageStudioPackage?.production_package_digest || null,
      image_studio_closing_reference_manifest_contract:
        compiledReferences?.contract || null,
    },
  });
}
...
async function prepareClosingKeyframeReview(task = {}) {
  ...
  const reviewReferences = CreativeImageProviderReferenceCompilerRuntime.compile({
    task:keyframe,
    source_assets:[
      ...list(keyframe.input?.source_assets),
      ...references.map((value) => ({
        url: typeof value === "string" ? value : value?.url,
        role: "CLOSING_KEYFRAME_REFERENCE",
      })).filter((value) => value.url),
  ...
  if (reviewReferences.provider_transport_safe !== true) {
    throw new Error(
      "CLOSING_KEYFRAME_REVIEW_REFERENCE_TRANSPORT_UNSAFE:" +
      [
        ...list(reviewReferences.missing_mandatory_roles),
        ...list(reviewReferences.missing_mandatory_groups),
      ].join(","),
    );
  }
  ...
  return ProductionTaskRuntime.update(task.id, {
    input: {
      ...
      assets: [
        ...
        ...reviewReferences.source_assets,
      ],
      image_provider_reference_manifest:reviewReferences,
      generation: {
        ...object(task.input?.generation),
        instructions:
          "Evaluate the immutable closing-frame quality contract exactly. Compare the generated endpoint against all supplied Image Studio authority references. Require camera authority, continuity state, material truth and performance state to remain preserved. Return strict JSON evidence.",
        provider_parameters: {
          ...object(task.input?.generation?.provider_parameters),
    ...
    metadata: {
      ...
      image_studio_closing_authority_required:
        keyframe.metadata?.image_studio_closing_authority_required === true,
      image_studio_closing_package_digest:
        keyframe.metadata?.image_studio_closing_package_digest || null,
      closing_keyframe_review_reference_count:
        reviewReferences.compiled_asset_count,
      closing_keyframe_review_reference_transport_safe:
        reviewReferences.provider_transport_safe,
      closing_keyframe_review_reference_omitted_count:
        reviewReferences.omitted_references.length,
    },
  });
}
