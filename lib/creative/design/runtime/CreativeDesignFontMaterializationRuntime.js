...
const CONTRACT = "CREATIVE_DESIGN_FONT_MATERIALIZATION_V2";
...
export async function materializeCreativeDesignFonts({
  ...
  try {
    for (const fontAssetId of fontIds) {
      ...
      const mimeType = text(material.mime_type || node.technical?.mime_type) || "font/ttf";
      if (!ALLOWED_FONT_MIME.has(mimeType)) {
        throw new Error(`CREATIVE_DESIGN_FONT_MIME_UNSUPPORTED:${node.id}:${mimeType}`);
      }
