...
async function defaultPlatformMatch(request) {
  const nodes = await CreativePlatformFontAssetRuntime.list();
  return choose(nodes, request);
}
...
export async function resolveCreativeDesignFont({
  ...
  const platformMatch = await defaultPlatformMatch(request);
