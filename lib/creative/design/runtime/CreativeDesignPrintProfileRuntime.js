...
const CONTRACT = "CREATIVE_DESIGN_PRINT_PROFILE_V4";
...
function requestedBleedEdges(profile) {
  return ["top", "right", "bottom", "left"]
    .filter((edge) => Number(profile.bleed?.[edge] || 0) > 0);
}

function pageBackgroundBleedDeclaration(page) {
  const print = object(page.print);
  return {
    capable: print.background_bleed_capable === true,
    extend: print.background_extends_to_bleed === true,
  };
}

function nodeBleedDeclaration(node) {
  const print = object(node.print);
  return {
    capable: print.bleed_capable === true,
    extend: print.extend_to_bleed === true,
  };
}
...
function inspectBleedSources(page, profile) {
  const requestedEdges = requestedBleedEdges(profile);
  const sources = [];
  const declarationIssues = [];
  const covered = new Set();
  const backgroundDeclaration = pageBackgroundBleedDeclaration(page);

  if (backgroundDeclaration.capable !== backgroundDeclaration.extend) {
    declarationIssues.push({
      severity: "ERROR",
      code: "PRINT_BLEED_SOURCE_DECLARATION_INCOMPLETE",
      page_id: page.id,
      source_type: "PAGE_BACKGROUND",
    });
  }

  if (
    page.background != null &&
    backgroundDeclaration.capable &&
    backgroundDeclaration.extend
  ) {
    for (const edge of requestedEdges) covered.add(edge);
    sources.push({
      source_type: "PAGE_BACKGROUND",
      source_id: page.id,
      supported: true,
      covered_edges: [...requestedEdges],
      explicitly_bleed_capable: true,
      explicitly_extends_to_bleed: true,
    });
  }

  for (const node of page.nodes) {
    if (node.visible === false) continue;
    const declaration = nodeBleedDeclaration(node);
    if (!declaration.capable && !declaration.extend) continue;
    if (declaration.capable !== declaration.extend) {
      declarationIssues.push({
        severity: "ERROR",
        code: "PRINT_BLEED_SOURCE_DECLARATION_INCOMPLETE",
        page_id: page.id,
    ...
    const supported = ["IMAGE", "SHAPE"].includes(node.type);
    const coveredEdges = supported
      ? requestedEdges.filter((edge) => frameTouchesEdge(node.frame, page, edge))
      : [];
    for (const edge of coveredEdges) covered.add(edge);
    sources.push({
      source_type: node.type,
      source_id: node.id,
      supported,
      covered_edges: coveredEdges,
      explicitly_bleed_capable: true,
      explicitly_extends_to_bleed: true,
    });
  }
  ...
  return {
    ...
    missing_edges: requestedEdges.filter((edge) => !covered.has(edge)),
    sources,
    declaration_issues: declarationIssues,
    explicit_bleed_source_required: requestedEdges.length > 0,
  };
}
...
function pushMissingAssetIssue(issues, required, detail) {
  issues.push({
    severity: required ? "ERROR" : "WARNING",
    code: required
      ? "PRINT_ASSET_RENDER_EVIDENCE_REQUIRED"
      : "PRINT_ASSET_RENDER_EVIDENCE_MISSING",
    ...detail,
  });
}
...
export function validateCreativeDesignPrintProfile(rawDocument = {}, options = {}) {
  ...
  const requireAssetEvidence = options.require_asset_evidence === true;
  const issues = [];
  const rasterEvidence = [];
  const bleedSourceEvidence = [];
  ...
  if (mode === "PRINT") {
    for (const profile of profiles) {
      ...
      const bleedEvidence = inspectBleedSources(page, profile);
      bleedSourceEvidence.push(bleedEvidence);
      issues.push(...bleedEvidence.declaration_issues);
      for (const source of bleedEvidence.sources.filter((entry) => !entry.supported)) {
        issues.push({
          severity: "ERROR",
          code: "PRINT_BLEED_SOURCE_TYPE_UNSUPPORTED",
          page_id: page.id,
          node_id: source.source_id,
          source_type: source.source_type,
        });
      }
      for (const edge of bleedEvidence.missing_edges) {
        issues.push({
          severity: "ERROR",
          code: "PRINT_BLEED_SOURCE_REQUIRED",
          page_id: page.id,
          edge,
        });
      }
      ...
      for (const node of page.nodes) {
        ...
        if (!binding) {
          pushMissingAssetIssue(issues, requireAssetEvidence, {
            page_id: page.id,
            node_id: node.id,
            asset_id: assetId,
          });
        ...
        if (!dpi) {
          issues.push({
            severity: requireAssetEvidence ? "ERROR" : "WARNING",
            code: requireAssetEvidence
              ? "PRINT_RASTER_DIMENSIONS_REQUIRED"
              : "PRINT_RASTER_DIMENSIONS_MISSING",
            page_id: page.id,
            node_id: node.id,
            asset_id: assetId,
          });
  ...
  if (cmykRequested && !iccProfile) {
    issues.push({ severity: "ERROR", code: "CMYK_ICC_PROFILE_REQUIRED" });
  }
  if (cmykRequested) {
    issues.push({
      severity: "ERROR",
  ...
  return {
    ...
    bleed_source_evidence: bleedSourceEvidence,
    issues,
    error_count: errorCount,
    warning_count: warningCount,
    release_blocked: errorCount > 0,
    release_asset_evidence_required: requireAssetEvidence,
    trim_box_evidence: true,
    bleed_box_evidence: true,
    explicit_bleed_source_required: true,
