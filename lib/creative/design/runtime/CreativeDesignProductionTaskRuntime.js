...
const CONTRACT = "CREATIVE_DESIGN_PRODUCTION_TASK_RUNTIME_V6";
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.design.production-task-runtime.v1",
);
const supabaseAdmin = getServiceSupabase();
const MEASURED_TYPOGRAPHY_RENDER_CAPABILITIES = new Set([
  "creative.design.render.svg",
  "creative.design.render.png",
  "creative.design.render.pdf",
]);
...
async function renderPdfTask(task, document) {
  return withRenderMaterials(task, document, async ({
    ...
    const rendered = await renderCreativeDesignDocumentToPdf(document, {
      ...
      density: task.input?.density || task.input?.output_spec?.density || 300,
      pdf_text_density:
        task.input?.pdf_text_density ||
        task.input?.output_spec?.pdf_text_density ||
        600,
    });
    ...
    return {
      ...
      font_evidence: fontEvidence,
      asset_evidence: assetEvidence,
      typography_evidence: rendered.typography_evidence,
      typography_measurement_count: rendered.typography_measurement_count,
      measured_typography: rendered.measured_typography === true,
      measured_typography_pending_pdf_vector_path: false,
      native_vector_text_line_count: rendered.native_vector_text_line_count,
      shaped_raster_text_line_count: rendered.shaped_raster_text_line_count,
      selectable_text_layer_complete: rendered.selectable_text_layer_complete,
      complex_script_shaping_engine: rendered.complex_script_shaping_engine,
      complex_script_render_mode: rendered.complex_script_render_mode,
      text_density: rendered.text_density,
      deterministic: true,
      provider_called: false,
    };
  });
  ...
  ProductionTaskRuntime.dispatch = async function dispatchWithCreativeDesign(id) {
    ...
    await ProductionTaskRuntime.update(id, {
      ...
      metadata: {
        ...
        measured_typography_required: MEASURED_TYPOGRAPHY_RENDER_CAPABILITIES.has(
          capability(task),
        ),
      },
    });
...
export const CreativeDesignProductionTaskRuntime = Object.freeze({
  ...
  wallet_charge_required: false,
  exact_asset_materialization_required: true,
  measured_typography_svg_png_required: true,
  measured_typography_pdf_required: true,
  measured_typography_pdf_vector_path_pending: false,
  pdf_complex_script_shaping_engine: "PANGO_HARFBUZZ_FREETYPE",
