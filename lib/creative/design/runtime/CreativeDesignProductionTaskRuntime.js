...
const CONTRACT = "CREATIVE_DESIGN_PRODUCTION_TASK_RUNTIME_V5";
...
async function renderPdfTask(task, document) {
  return withRenderMaterials(task, document, async ({
    ...
    return {
      ...
      typography_evidence: typographyEvidence,
      typography_measurement_count: typographyMeasurementCount,
      measured_typography: false,
      measured_typography_pending_pdf_vector_path: true,
      deterministic: true,
      provider_called: false,
    };
  });
  ...
  ProductionTaskRuntime.dispatch = async function dispatchWithCreativeDesign(id) {
    ...
    await ProductionTaskRuntime.update(id, {
      ...
      metadata: {
        ...
        measured_typography_required: capability(task) !== "creative.design.render.pdf",
      },
    });
...
export const CreativeDesignProductionTaskRuntime = Object.freeze({
  ...
  measured_typography_pdf_vector_path_pending: true,
