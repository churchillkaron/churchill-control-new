...
const CONTRACT = "CREATIVE_DESIGN_PRODUCTION_TASK_RUNTIME_V2";
...
async function withFonts(task, document, work) {
  const materialized = await materializeCreativeDesignFonts({
    document,
    media_policy: object(task.input?.media_policy || task.metadata?.media_policy),
  });
  try {
    return await work(materialized.bindings, materialized.evidence);
  } finally {
...
async function renderSvgTask(task, document) {
  return withFonts(task, document, async (fontBindings, fontEvidence) => {
    const rendered = renderCreativeDesignDocumentToSvg(document, {
      font_bindings: fontBindings,
    });
    if (!rendered.success) {
      throw new Error("CREATIVE_DESIGN_SVG_RENDER_REPAIR_REQUIRED");
    }
...
async function renderPngTask(task, document) {
  return withFonts(task, document, async (fontBindings, fontEvidence) => {
    ...
    if (rendered.text_overflow_nodes?.length) {
      throw new Error("CREATIVE_DESIGN_PNG_TEXT_OVERFLOW");
    }
...
async function executeLocal(task) {
  ...
  if (id === "creative.design.compose") {
    ...
    return {
      ...composed,
      design_document: composed.document,
    };
  }
...
function install() {
  ...
  ProductionTaskRuntime.dispatch = async function dispatchWithCreativeDesign(id) {
    ...
    if (!task || !isCreativeDesignProductionTask(task)) {
