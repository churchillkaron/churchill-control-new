...
const CONTRACT = "CREATIVE_DESIGN_QUALITY_V3";
...
export function inspectCreativeDesignDocument(rawDocument = {}, options = {}) {
  ...
  const collisionReport = inspectCreativeDesignCollisions(
    document,
    object(options.collision_options),
  );
  const issues = [];
  ...
  for (const page of document.pages) {
    for (const node of page.nodes) {
      ...
      if (["IMAGE", "VECTOR"].includes(node.type) && text(node.asset_id)) {
        const assetEvidence = evidenceForNode(svgRender, node.id, node.type);
        if (!assetEvidence.length || assetEvidence.some((entry) => entry.exact_asset_binding_present !== true)) {
          issues.push({
            severity: "ERROR",
            code: "EXACT_VISUAL_ASSET_BINDING_MISSING",
            page_id: page.id,
            node_id: node.id,
          });
        }
  ...
  for (const nodeId of svgRender.text_overflow_nodes || []) {
    issues.push({ severity: "ERROR", code: "TEXT_OVERFLOW", node_id: nodeId });
  }
  for (const nodeId of svgRender.table_overflow_nodes || []) {
    issues.push({ severity: "ERROR", code: "TABLE_OVERFLOW", node_id: nodeId });
  }
  ...
  return {
    ...
    exact_visual_assets_required: true,
    collision_free_protected_content_required: true,
