...
const CONTRACT = "CREATIVE_DESIGN_REPAIR_V1";
const REPAIR_PLAN_CONTRACT = "CREATIVE_DESIGN_REPAIR_PLAN_V1";
...
function mergeTypography(node, patch = {}) {
  return {
    ...node,
    typography: {
...
function assertOperationAllowed(document, page, node, operation) {
  if (!page) {
    throw new Error(`CREATIVE_DESIGN_REPAIR_PAGE_NOT_FOUND:${operation.page_id || "unknown"}`);
  }
  if (!node) {
    throw new Error(`CREATIVE_DESIGN_REPAIR_NODE_NOT_FOUND:${operation.node_id || "unknown"}`);
  }
  if (node.locked === true) {
  ...
  const changesTruth = [
    ...
    "SET_ASSET_REFERENCE",
    "SET_TABLE_ROWS",
  ].includes(text(operation.operation).toUpperCase());
  if (changesTruth && nodeIsDataBound(document, node)) {
...
function applyOperation(document, rawOperation) {
  ...
  const page = pageFor(document, operation.page_id, operation.node_id);
  ...
  if (kind === "SET_FRAME") {
    ...
    repaired = mergeTypography(node, operation.typography);
    ...
    repaired = {
      ...
      rows: Array.isArray(operation.rows)
        ? operation.rows.map((row, rowIndex) => ({
            ...object(node.rows?.[rowIndex]),
            ...object(row),
          }))
        : node.rows,
      typography: operation.typography
        ? {
            ...object(node.typography),
            ...object(operation.typography),
          }
        : node.typography,
    };
  } else if (kind === "SET_CONTENT") {
    repaired = { ...node, content: String(operation.content ?? "") };
  } else if (kind === "SET_CODE_VALUE") {
    repaired = { ...node, value: String(operation.value ?? "") };
  } else if (kind === "SET_ASSET_REFERENCE") {
    repaired = { ...node, asset_reference: text(operation.asset_reference) };
  } else if (kind === "SET_TABLE_ROWS") {
    repaired = { ...node, rows: clone(operation.rows || []) };
  } else {
...
export function planCreativeDesignMechanicalRepairs(rawDocument = {}, report = null) {
  ...
  for (const issue of list(quality.issues)) {
    ...
    const identity = `${operation.operation}:${operation.node_id}:${operation.source_issue_code}`;
