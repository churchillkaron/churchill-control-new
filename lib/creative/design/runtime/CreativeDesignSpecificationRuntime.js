...
const CONTRACT = "CREATIVE_PARTNER_DESIGN_SPECIFICATION_V3";
...
function platformFontSummaries() {
  return AVANTIQO_FONT_LIBRARY.map((entry) => ({
    asset_id: entry.id,
...
function compactMasterPlan(master = {}) {
  ...
  return {
    ...
    art_direction: object(plan.art_direction),
...
function authorityFor(plan) {
  ...
  const productionDesign = roleDecision(plan, "production_designer");
  const brand = roleDecision(plan, "brand_director");
  const copy = roleDecision(plan, "copy_director");
  const artDirection = object(plan.art_direction);
  if (text(art.status).toUpperCase() !== "ACTIVE") {
    throw new Error("CREATIVE_DESIGN_ART_DIRECTOR_DECISION_REQUIRED");
  }
  if (workflowKind === "STILL" && text(productionDesign.status).toUpperCase() !== "ACTIVE") {
    throw new Error("CREATIVE_DESIGN_PRODUCTION_DESIGNER_DECISION_REQUIRED");
  }
  if (text(brand.status).toUpperCase() !== "ACTIVE") {
    throw new Error("CREATIVE_DESIGN_BRAND_DIRECTOR_DECISION_REQUIRED");
  }
  if (workflowKind === "STILL" && !text(artDirection.governing_visual_idea)) {
    throw new Error("CREATIVE_DESIGN_ART_DIRECTION_DOSSIER_REQUIRED");
  }
  return {
    creative_master_plan_hash: digest(plan, "creative-master-plan"),
    art_direction_id: workflowKind === "STILL"
      ? digest(artDirection, "art-direction")
      : digest(art, "art-direction"),
    art_director_decision_id: digest(art, "art-director-decision"),
    production_design_id: workflowKind === "STILL"
      ? digest(productionDesign, "production-design")
      : null,
...
function normalizeSpecification({
  ...
  const fontIds = new Set([
    ...
    ...AVANTIQO_FONT_LIBRARY.map((entry) => entry.id),
  ]);
        ...
        pages: [
          {
            ...
            safe_area: "object",
            print: {
              background_bleed_capable: "boolean; true only when the page background is intentionally printable through bleed",
              background_extends_to_bleed: "boolean; must match background_bleed_capable when positive bleed is supplied",
            },
            nodes: [
              {
                id: "string",
                ...
                typography: {
                  font_asset_id: "exact supplied organization FONT asset id or platform-font:<slug>",
                ...
                print: {
                  bleed_capable: "boolean; only IMAGE or SHAPE may be true",
                  extend_to_bleed: "boolean; true only when the element intentionally touches trim and must extend through requested bleed",
                },
                locked: "boolean",
              },
            ],
...
async function createReasonedSpecification({
  ...
  const organizationFonts = assets.filter((asset) => asset.type === CREATIVE_ASSET_NODE_TYPES.FONT);
  const platformFonts = platformFontSummaries();
  const fonts = [...organizationFonts, ...platformFonts];

  const decision = await reason({
    task:
      "Act as the Avantiqo Art/Brand/Copy design-direction worker behind the Creative Partner. Convert the approved Creative Master Plan into one exact editable design specification for the Avantiqo Layout & Typesetting Engine. Make the composition decision: page/artboard geometry, hierarchy, grid, typography, exact source-asset placement, data-bound regions and export intent. For print work, treat bleed as an explicit design decision: when positive bleed is requested, explicitly mark only a real page background or an IMAGE/SHAPE that touches the relevant trim edge and is intended to continue through bleed; never mark TEXT, TABLE, QR, BARCODE or external VECTOR assets as bleed-capable. Do not claim bleed intent when the artwork does not actually support it. Do not use a fixed poster/menu/brochure template. Do not choose AI providers. Do not write image-generator prompts. Never invent a logo, font, price, product, date, legal fact or other business truth. For typography use only an exact supplied organization FONT asset id or an exact platform-font id from the supplied Avantiqo font library. If brand direction requires an exact organization brand font, do not substitute a platform font. Use only exact supplied image/vector asset ids and governed data source ids. Generated imagery, when the master plan requires it, is represented only as a later governed asset dependency and must never contain final typography or exact logos inside pixels.",
    input: {
      organization_id,
      workflow_kind: workflowKind,
      ...
      organization_font_assets: organizationFonts,
      avantiqo_font_library: platformFonts,
      ...
      architecture: {
        ...
        print_bleed_model: "EXPLICIT_PAGE_BACKGROUND_OR_EDGE_TOUCHING_IMAGE_OR_SHAPE_ONLY",
        final_text_generated_inside_image_pixels: false,
        exact_logo_redrawing_by_generation: false,
        repair_model: "BOUNDED_NODE_LEVEL",
        brand_locked_font_requires_organization_font: true,
        general_typography_may_use_avantiqo_font_library: true,
      },
    },
    constraints: {
      preserve_master_plan: true,
      preserve_locked_brand_assets: true,
      font_ids_only_from_available_fonts: true,
      brand_locked_fonts_only_from_organization_font_assets: true,
      exact_business_data_only_from_governed_sources_or_verified_master_plan: true,
      positive_bleed_requires_explicit_supported_source_per_edge: true,
      bleed_source_must_be_page_background_image_or_shape: true,
      no_templates: true,
