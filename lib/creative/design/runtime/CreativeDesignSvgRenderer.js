...
function focalAxis(value, low, middle, high) {
...
function imagePreserveAspectRatio(node = {}) {
  if (node.fit === "contain") return "xMidYMid meet";
  const focal = object(node.focal_point || node.metadata?.focal_point);
  const x = focalAxis(focal.x, "xMin", "xMid", "xMax");
  const y = focalAxis(focal.y, "YMin", "YMid", "YMax");
  return `${x}${y} slice`;
}
...
function renderImage(node, assetBindings, evidence) {
  ...
  evidence.push({
    ...
    focal_point: object(node.focal_point || node.metadata?.focal_point),
    preserve_aspect_ratio: imagePreserveAspectRatio(node),
  });

  return `<image data-node-id="${escapeXml(node.id)}" href="${escapeXml(href)}" x="${frame.x}" y="${frame.y}" width="${frame.width}" height="${frame.height}" preserveAspectRatio="${imagePreserveAspectRatio(node)}" opacity="${number(node.opacity, 1)}" />`;
}
