...
function renderText(node, fontBindings, evidence) {
  ...
  const layout = wrapCreativeDesignText(node.content, frame, typography);
  evidence.push({
    node_id: node.id,
    type: "TEXT",
    ...
    estimated_text_measurement: layout.estimated,
  ...
  for (const cell of layout.cells) {
    ...
    const x = align === "center"
      ...
      : align === "right"
        ...
        : innerFrame.x;
    const fontSize = number(typography.font_size, 16);
    const layoutText = wrapCreativeDesignText(cell.content, innerFrame, typography);
...
export function renderCreativeDesignDocumentToSvg(rawDocument = {}, options = {}) {
  ...
  return {
    ...
    contract: "CREATIVE_DESIGN_SVG_RENDER_V5",
