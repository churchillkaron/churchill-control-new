...
function renderNode(node, pageId, tableLayouts, fontBindings, evidence) {
...
function renderPage(page, documentHash, fontBindings, tableLayouts) {
  const background = page.background
    ? `<rect width="100%" height="100%" fill="${escapeXml(page.background)}" />`
    : "";
  const evidence = [];
  const nodes = page.nodes.map((node) =>
    renderNode(node, page.id, tableLayouts, fontBindings, evidence)).join("\n");
...
export function renderCreativeDesignDocumentToSvg(rawDocument = {}, options = {}) {
  ...
  return {
    success: textOverflowNodes.length === 0 && tableOverflowNodes.length === 0,
    contract: "CREATIVE_DESIGN_SVG_RENDER_V3",
