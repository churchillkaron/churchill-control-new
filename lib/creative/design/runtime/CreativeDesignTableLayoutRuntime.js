...
const CONTRACT = "CREATIVE_DESIGN_TABLE_LAYOUT_V3";
...
function rowHeight(node, row, rowIndex, columns, frameWidth, textLayouts) {
  ...
  for (let columnIndex = 0; columnIndex < columns.length; columnIndex += 1) {
    ...
    const cellId = `${node.id}:r${rowIndex}:c${columnIndex}`;
    const layout = measuredLayoutFor(textLayouts, cellId) || wrapCreativeDesignText(
      cell.content,
...
function layoutTableNode(node, textLayouts) {
  ...
  const cells = [];
  const rowFrames = [];
  let measuredTypographyUsed = false;
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    ...
    for (let columnIndex = 0; columnIndex < columns.length; columnIndex += 1) {
      ...
      if (measuredLayoutFor(textLayouts, id)?.actual_font_measurement === true) {
        measuredTypographyUsed = true;
      }
      cells.push({
        id,
        row_index: rowIndex,
  ...
  return {
    ...
    measured_typography_used: measuredTypographyUsed,
  };
}

export function layoutCreativeDesignTables(rawDocument = {}, options = {}) {
  ...
  for (const page of document.pages) {
    for (const node of page.nodes) {
      ...
      tables.push({
        page_id: page.id,
        ...layoutTableNode(node, textLayouts),
      });
    }
  }
  ...
  return {
    ...
    content_aware_row_sizing: true,
    unicode_segmented: true,
    measured_typography_used: tables.some((table) => table.measured_typography_used),
  };
}
