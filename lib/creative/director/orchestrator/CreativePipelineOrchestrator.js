...
function preserveApprovedTemporalGovernance(resolvedMaster = {}, approvedMaster = null) {
  ...
  const selectedConceptId =
    ...
    null;
  const councilHash = council.council_hash || approvedPlan.production?.concept_council_hash || null;
  const conceptHash = council.concept_hash || approvedPlan.production?.selected_concept_hash || null;
  return {
    ...resolvedMaster,
    independent_concept_council:
      Object.keys(council).length ? council : resolvedMaster.independent_concept_council || null,
    plan: {
      ...resolvedPlan,
      ...
      concept_council: Object.keys(council).length ? council : resolvedPlan.concept_council,
      production: {
...
async function materializeDirection({
  ...
  const incomingMasterIsLaterAuthority = Boolean(
    ...
    (
      tribunalApprovedMaster.preproduction_creative_repair?.after?.passed === true ||
      tribunalApprovedMaster.production_room_bootstrap?.production_room_pipeline?.contract ||
      tribunalApprovedMaster.plan?.production_room_bootstrap?.production_room_pipeline?.contract
    )
  );
  let resolvedMaster =
    (incomingMasterIsLaterAuthority ? tribunalApprovedMaster : null) ||
