...
async function materializeDirection({
  ...
  if (!master || !Object.keys(object(master)).length) {
    throw new Error("CREATIVE_GOVERNED_MASTER_REQUIRED_FOR_MATERIALIZATION");
  }
  ...
  const research = resolvedMaster.research || (await ResearchRuntime.list({
    organization_id,
    creative_project_id,
  })).find((item) =>
...
export async function buildCreativePipeline(input = {}) {
  ...
  if (!input.master || !Object.keys(object(input.master)).length) {
    throw new Error("CREATIVE_GOVERNED_MASTER_REQUIRED_FOR_MATERIALIZATION");
  }
