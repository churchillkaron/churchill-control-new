...
function temporalDirectionCheckpoint(project = {}, master = null) {
  ...
  if (!state.master?.plan?.scenes?.length) return null;
...
async function materializeDirection({
  ...
  const research = researched.research || resolvedMaster.research || (await ResearchRuntime.list({
    organization_id,
    creative_project_id,
  })).find((item) =>
