...
function temporalDirectionCheckpoint(project = {}, master = null) {
...
async function materializeDirection({
  ...
  const researched = await ResearchRuntime.resolveCreativeDirectionResearch({
    organization_id,
    mission,
    project,
    brief,
  ...
  const durableTemporalMaster = temporalDirectionCheckpoint(currentProject || project, tribunalApprovedMaster);
  const completeDurableTemporalMaster =
    durableTemporalMaster && list(durableTemporalMaster.plan?.scenes).length
      ? durableTemporalMaster
      : null;
  const approvedTemporalMasterNeedsMaterialization =
    tribunalApprovedMaster && !list(tribunalApprovedMaster.plan?.scenes).length;
  let resolvedMaster =
    completeDurableTemporalMaster ||
    (approvedTemporalMasterNeedsMaterialization
  ...
  resolvedMaster = {
    ...resolvedMaster,
    plan: ensureUniversalTemporalDossier(resolvedMaster.plan, {
      synthesis: resolvedMaster.universal_creative_synthesis || {},
      identities: list(resolvedMaster.plan?.identity_profiles),
      audio: null,
      isMusicVideo:
        resolvedMaster.music_video === true ||
        resolvedMaster.plan?.production?.source_audio_required === true,
    }),
  };
  const roomStages = list(production_room_bootstrap?.production_room_pipeline?.stages);
  const sealedUpstreamStageIds = ["RESEARCH_ROOM", "CREATIVE_FLOOR", "CONCEPT_COMPETITION", "TRIBUNAL"];
  const sealedUpstreamAuthority =
    resumed_from_approved_council === true &&
    sealedUpstreamStageIds.every((stageId) =>
      roomStages.some((stage) =>
        text(stage?.id).toUpperCase() === stageId &&
        text(stage?.status).toUpperCase() === "SEALED",
      ),
    ) &&
    roomStages.some((stage) =>
      text(stage?.id).toUpperCase() === "TECHNICAL_SCOUT" &&
      ["READY", "SEALED"].includes(text(stage?.status).toUpperCase()),
    ) &&
    resolvedMaster.plan?.creative_tribunal?.passed === true &&
    resolvedMaster.plan?.creative_tribunal?.verdict?.passed === true;
  if (sealedUpstreamAuthority) {
    resolvedMaster = {
      ...resolvedMaster,
      plan: {
        ...resolvedMaster.plan,
        sealed_preproduction_authority: {
          contract: "CREATIVE_SEALED_PREPRODUCTION_AUTHORITY_V1",
          passed: true,
          resumed_from_approved_council: true,
          sealed_upstream_stage_ids: sealedUpstreamStageIds,
          technical_scout_ready: true,
          tribunal_hash: resolvedMaster.plan?.creative_tribunal?.tribunal_hash || null,
          tribunal_weighted_score: Number(
            resolvedMaster.plan?.creative_tribunal?.verdict?.weighted_score || 0,
          ),
        },
      },
    };
  }
  const sealedWorldClassGate = object(resolvedMaster.plan?.world_class_concept_intelligence);
  if (process.env.CREATIVE_DEBUG_RESUME === "true") {
    console.log("[creative-resume-debug]", JSON.stringify({
      master_keys: Object.keys(object(resolvedMaster)),
      plan_keys: Object.keys(object(resolvedMaster.plan)),
      top_gate: resolvedMaster.plan?.world_class_concept_intelligence || null,
      independent_council: resolvedMaster.independent_concept_council || null,
      plan_independent_council: resolvedMaster.plan?.independent_concept_council || null,
      concept_council: resolvedMaster.plan?.concept_council || null,
      tribunal: resolvedMaster.plan?.creative_tribunal || null,
    }));
  }
  const sealedCouncil = object(
    resolvedMaster.independent_concept_council ||
    resolvedMaster.plan?.independent_concept_council ||
    resolvedMaster.plan?.concept_council,
  );
  const sealedCouncilGate = object(sealedCouncil.world_class_gate);
  const sealedCouncilGateStillCurrent =
    sealedCouncilGate.contract === WORLD_CLASS_CONCEPT_POLICY.contract &&
    sealedCouncilGate.passed === true &&
    sealedCouncilGate.b_grade_concept_forbidden === true &&
    Boolean(text(sealedCouncilGate.selected_concept_id)) &&
    Number(sealedCouncilGate.weighted_score) >= WORLD_CLASS_CONCEPT_POLICY.minimum_weighted_score &&
    Number(sealedCouncilGate.selector_confidence) >= WORLD_CLASS_CONCEPT_POLICY.minimum_selector_confidence;
  const sealedTopLevelGateStillCurrent =
    sealedWorldClassGate.contract === WORLD_CLASS_CONCEPT_POLICY.contract &&
    sealedWorldClassGate.passed === true &&
    sealedWorldClassGate.b_grade_concept_forbidden === true &&
    sealedWorldClassGate.temporal_council_enforced === true &&
    object(sealedWorldClassGate.council).passed === true;
  const sealedTribunalApprovedWorldClassDirection =
    Boolean(tribunalApprovedMaster) &&
    (sealedTopLevelGateStillCurrent || sealedCouncilGateStillCurrent);
  if (sealedTribunalApprovedWorldClassDirection && !sealedTopLevelGateStillCurrent) {
    const resumedWorldClassGate = Object.freeze({
      contract: WORLD_CLASS_CONCEPT_POLICY.contract,
      passed: true,
      b_grade_concept_forbidden: true,
      temporal_council_enforced: true,
      council: sealedCouncilGate,
      policy: WORLD_CLASS_CONCEPT_POLICY,
      resumed_from_sealed_council: true,
    });
    resolvedMaster = {
      ...resolvedMaster,
      plan: {
        ...resolvedMaster.plan,
        world_class_concept_intelligence: resumedWorldClassGate,
        concept_council: {
          ...object(resolvedMaster.plan?.concept_council),
          world_class_gate: sealedCouncilGate,
        },
        validation_summary: {
          ...object(resolvedMaster.plan?.validation_summary),
          world_class_concept_intelligence: resumedWorldClassGate,
        },
      },
      world_class_concept_intelligence: resumedWorldClassGate,
  ...
  if (!sealedTribunalApprovedWorldClassDirection && !sealedUpstreamAuthority) {
...
export async function buildCreativePipeline(input = {}) {
  ...
  let graph = await ProductionGraphRuntime.plan({
    organization_id,
    creative_mission_id,
    creative_project_id,
