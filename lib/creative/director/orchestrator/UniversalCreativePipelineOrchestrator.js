...
function stripTransportDetails(value, depth = 0) {
  if (depth > 20) return null;
  if (Array.isArray(value)) {
    return value.map((item) => stripTransportDetails(item, depth + 1));
  }
  if (!value || typeof value !== "object") return value;

  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => !FORBIDDEN_PERSISTENCE_KEYS.has(key))
      .map(([key, child]) => [key, stripTransportDetails(child, depth + 1)]),
  );
}
...
async function materializeAgencyDirection({
  ...
  const kind = workflow.workflow_kind;

  if (workflow.executor !== "UNIVERSAL") {
    throw new Error(`CREATIVE_UNIVERSAL_WORKFLOW_REQUIRED:${kind}`);
  }
  ...
  if (!strategy) {
    strategy = await CreativeStrategyRuntime.create({
      ...
      visual_direction: {
        creative_system:
          plan.concept?.creative_system || plan.concept?.visual_system || {},
...
async function materializeProductionTasks({
  ...
  const existingByNode = new Map(
    existing
      .filter((task) =>
        ...
        String(task.production_graph_id) === String(production_graph_id),
      )
  ...
  for (const rawStep of list(executionPlan.steps)) {
    const step = stripTransportDetails(rawStep);
    const prior = existingByNode.get(step.node_id);
    if (prior) {
      if (String(prior.status || "").toUpperCase() === "WAITING") {
        ...
        const cleanInput = stripTransportDetails(step.input || {});
        const cleanMetadata = stripTransportDetails(step.metadata || {});
        const refreshed = await ProductionTaskRuntime.update(prior.id, {
          ...
          input: {
            ...
            requirements,
            generation: {
              ...stripTransportDetails(generation),
              service: step.service_code,
              capability: step.capability,
            },
            output_spec: generation.output_spec || requirements.output_spec || cleanMetadata.output_spec || {},
            promptless_persistence: true,
          },
          cost: { estimated: estimatedCost, currency, approved: estimatedCost <= 0 ? true : false },
          timing: { ...object(prior.timing), estimated_seconds: Number(step.estimated_seconds || 0) },
          metadata: {
        ...
        taskByNode.set(step.node_id, refreshed);
      } else {
    ...
    const task = await ProductionTaskRuntime.create({
      ...
      status: "WAITING",
      title: cleanMetadata.node_title || cleanInput.title || "",
      description: cleanInput.description || cleanMetadata.intent?.purpose || "",
      service_id: step.service_code,
      service_code: step.service_code,
      capability: step.capability,
      ...
      metadata: {
        ...
        release_candidate: cleanMetadata.release_candidate === true,
        provider_prompt_persisted: false,
        provider_prompts_persisted: false,
        provider_parameters_persisted: false,
      },
