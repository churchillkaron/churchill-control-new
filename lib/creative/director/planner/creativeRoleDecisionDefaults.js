...
export function applyDerivedRoleDecisions(plan = {}, roles = []) {
  ...
  for (const role of roles) {
    ...
    if (governanceFactory) {
      const governed = governanceFactory(plan);
      const existingGovernance = object(decisions[role.id]);
      if (
        text(existingGovernance.status).toUpperCase() !== "ACTIVE" ||
        existingGovernance.derived_from_system_governance !== true
      ) {
        decisions[role.id] = governed;
        derived += 1;
      }
      continue;
    }
    ...
    if (eligible) {
      ...
      const backfilled = stillRoleBackfill(role.id, plan);
      if (backfilled) {
