  ...
  return Object.fromEntries(
    CREATIVE_AGENCY_ROLES.map((role) => [
      role.id,
      {
        ...
        status:
          "Required string. Exactly ACTIVE or NOT_REQUIRED.",
        decision:
          "Required non-empty string. State the concrete decision owned by this role when ACTIVE, or the concrete reason the discipline is unnecessary when NOT_REQUIRED.",
        evidence:
          "Required JSON array. ACTIVE roles require one or more exact evidence references grounded in the supplied mission, brief, project, assets, research or approved history. NOT_REQUIRED roles may use an empty array. Never return a scalar string.",
        confidence:
          "Required number from 0 to 100 reflecting confidence in this role decision.",
        risks:
          "Required JSON array of concrete unresolved risks owned by this role. Empty is valid only when there are no material role-specific risks.",
        repair_instructions:
          "Required JSON array of bounded repairs owned by this role. Empty is valid only when no role-specific repair is required.",
      },
    ]),
  );
}
