...
import {
  advertisingFillerDisclosure,
} from "@/lib/creative/director/registry/CreativeAdvertisingFillerRegistry";
...
const CREATIVE_EXCELLENCE_GATE = Object.freeze({
  ...
  core_mechanism_dramaturgy:
    "When the proposition has a differentiated internal mechanism -- an engine, intelligence, algorithm, material system, network, process, architecture or other causal core -- test whether that mechanism should become a protagonist rather than a feature list. If it is story-critical, dramatize how inputs enter it, how its parts or layers coordinate, what transformation occurs inside it, and what consequences emerge from it. Translate architecture, flow, energy, memory, reasoning, force, timing or feedback into authored image, sound, motion and scale. Do not reduce a powerful mechanism to a dashboard demo, explanatory UI tour, generic glowing network or decorative technical graphic. The audience should feel the character and capability of the mechanism before or while they understand it, the way great product films make machinery itself emotionally legible.",
  scale_synthesis:
    "When the mission is global, systemic, technological, intelligent, future-facing or otherwise larger than one local incident, earn that scale through a designed progression between human consequence, physical reality, the core mechanism and the larger system it influences. Micro and macro scale may coexist: material detail, circuitry, neural or mathematical structures, cities, industries, Earth or cosmic perspective can all be valid when they reveal the same causal idea. Global or universe imagery is never permission for decorative space shots; every scale change must add meaning, tension, beauty or proof and must eventually reconnect to human stakes.",
  benchmark_relative_authorship:
    "When explicit quality-floor references are supplied, judge the direction against their level of authorship, tension, sensory craft, originality, mechanism storytelling and emotional control -- not against contract completeness. A familiar category formula, competent software explainer or generic problem-solution advertisement cannot self-certify as world-class merely because it is coherent and executable. If the work would feel ordinary beside the declared benchmark floor, creative_review.passed must be false and the direction must be rebuilt. Benchmarks are never templates to copy; match their creative ambition and finish while inventing a different idea.",
  anti_cliche_test:
    `Identify the most likely category, AI-generation and advertising clichés for this exact mission and deliberately design against them. Rejected patterns must be mission-specific, not generic statements such as avoid generic content. These specific constructions are treated as advertising filler wherever they appear and must not be used: ${advertisingFillerDisclosure()}. Describing an audience journey or a customer journey is ordinary strategy language and is not what this forbids. Do not merely avoid these words: each one stands in for craft that has not been decided yet, so replace it with the observable execution it gestures at, exactly as craft_translation requires. Three uses across the direction is rejected outright by the decision gate, not merely scored down.`,
  medium_necessity:
  ...
  structural_invention:
    "Decide the form, do not inherit it. The shape of the work -- how many scenes, how long each holds, what order they run in, how many frames or panels or screens, where the work breaks its own pattern -- is a creative decision the story earns, never a default derived from duration or format. A film may be one unbroken take, three long movements, a rapid montage, a non-linear order, or a repeating motif that returns changed. A still may be a single frame, an asymmetric series, a sequence read in an unexpected order, or one image that withholds its subject. State plainly what formal risk this work takes and what it would cost if it failed. If the structure you return is the one anybody would default to for this brief, medium and length, it is the wrong answer: reject it and find the shape this specific story needs. Novelty for its own sake is not the goal either -- the unusual choice must be the one that makes the idea land harder, and you must say why.",
  surface_invention:
    "Structure alone is not imagination. Inside whatever shape you chose, the work must be built on a device -- a repeatable creative mechanism that does something to the idea that competent coverage cannot. Camera movement is not a device: a push-in, a pan, a track and an orbit are how a camera behaves, and describing one in detail is craft, not invention. A device is closer to a rule the work obeys or breaks: a frame that refuses to show the subject until it has earned it, an edit that cuts on sound rather than picture, type that contradicts what is on screen, an object that persists across every cut, a colour that only appears when someone lies, a scale that is wrong on purpose, one continuous sound holding unrelated images together, a transformation that could not happen in life. The same applies to stills, where the device may live in the crop, the sequence order, the material, the typography, or what is withheld from the frame. Typography, effects and sound are creative instruments here and not a finishing layer: if this work uses text, decide what the text does beyond naming things, and if it uses none, decide that too. The call to action is part of the work and takes a form: a card at the end is one option among many and never the default. Name the devices this brief would reach for by reflex and reject them explicitly. Then apply the device where it earns its place and leave the rest plain, because a device on every shot is decoration and reads as noise.",
  craft_translation:
    "Translate every abstract adjective into observable execution decisions. Premium, cinematic, authentic, natural, luxury, bold, elegant, energetic, emotional, seamless, effortless, immersive, elevated, curated, timeless, iconic and similar words are insufficient unless expressed through composition, timing, performance, material, typography, sound, interaction, lighting, edit, spatial behavior or other medium-relevant craft. Seamless is the common failure in audio and edit direction: replace it with what the audience would actually perceive, such as no audible edit point, matched room tone across the cut, or a crossfade landing under the downbeat.",
  source_asset_intent:
    "Every selected asset must have a deliberate production role or explicit exclusion. When source fidelity matters, state what exact identity, product, logo, venue, text, performance, geometry or material truth must remain unchanged and what transformations are allowed.\n\nTranscribe, do not paraphrase. Where an asset's evidence contains legible text -- signage, a wordmark, a menu, a price, a street name -- reproduce that wording verbatim in the source truths, in the order and grouping it appears, and require it to be composited from the source or set as type rather than reproduced inside generated pixels. A plan that says the signage must match while never quoting the signage has not pinned anything, and the work comes back with plausible words that are not the real ones.\n\nThe same applies to what the source establishes about light and material. Where the evidence fixes a lighting state, a colour temperature, a surface or a texture, say what that state is and what would violate it, so a grade cannot introduce a cast the location never had and finishing cannot smooth away a texture that was the point.",
  production_proof:
    "Every planned execution step must have a necessary creative purpose, an evidence-derived output specification and a real enabled service/capability pair. Do not add steps merely to make the plan look sophisticated.\n\nA source truth cannot be produced by asking a generator to imitate it. Where the work must reproduce a logo, wordmark, neon sign, signage, exact text, a product's own geometry or any other mark whose exact appearance is the point, the plan needs a step that composites or places that mark from the source asset, and the generation step must be told to leave room for it rather than to replicate it. Naming the mark in continuity anchors and then handing it to an image generator is not fidelity: it produces a plausible logo, in approximately the right colour, with edges that are nearly right, which is precisely how a brand mark reads as counterfeit. State which marks are composited from source, which step does it, and what the generated layer must therefore omit.\n\nWhen the organization has no capability that can composite -- and that is a real situation, since an organization may be able to generate and upscale an image and not to place a layer into one -- do not plan the step anyway and do not quietly generate the mark as though the problem did not exist. Say which marks cannot be reproduced faithfully with the capabilities available, name the capability that is missing, and record it as a repair required before release. A plan that states what it cannot guarantee is worth more than one that guarantees what it cannot do, and the gap is then something the organization can fix rather than something it discovers in the finished work.",
  finishing_proof:
...
const COMMON_CONTRACT = Object.freeze({
  ...
  concept: Object.freeze({
    ...
    signature_device:
      "The one specific device this work is built on -- the thing a viewer would describe to someone else afterwards. A device is a repeatable creative mechanism, not a subject or a mood: what the camera, the cut, the type, the sound, the effect, the material or the interaction actually does. Name it concretely, state what it does to the idea that plain coverage could not, and state where it appears and where it deliberately does not. A work with no device is coverage.",
    refused_devices:
      "The devices this brief would default to, named and rejected with the reason each is the expected answer. Name the actual mechanisms, not categories: the slow push-in on the product, the title card over black, the logo end frame, the crossfade between pretty shots, the drone establishing wide, the text sliding in from the left, the grid of centred product photographs. A rejection list that names no mechanism is not a rejection.",
    call_to_action:
      "Earned action causally connected to the idea and audience state, or an explicit explanation that no CTA is appropriate. Decide its form as well as its content: an action performed on screen, a spoken line, a typographic event, an object, a sound, a held silence, something the audience does, or a deliberate withholding. A text card at the end is one option among many and never the default -- if that is the choice, say why the idea needs it.",
    target_audience:
  ...
  creative_review: Object.freeze({
    ...
    rejected_patterns:
      "Four or more predictable, generic, derivative, AI-looking or context-inappropriate approaches deliberately rejected before selection. Three is the floor below which the plan is invalid, not the standard. Rejections must be specific to this mission and explain the failure mode, and each must be substantial enough to stand on its own rather than a short label.",
    weakest_link:
      "The single weakest remaining aspect of the selected direction, stated precisely rather than defended.",
    craft_risks:
      "Four or more concrete medium-specific craft failures that could make the final work look generic, synthetic, confusing, derivative, physically false, emotionally flat or unfinished. Each must be substantial enough to stand on its own rather than a short label.",
    finishing_requirements:
      "Four or more concrete final-craft requirements needed to make the work release-ready rather than merely generated or technically complete. Each must be inspectable and substantial enough to stand on its own rather than a short label.",
...
const TEMPORAL_SCHEMA = Object.freeze({
  story: Object.freeze({
    ...
    human_desire:
      "Concrete human want, need, responsibility or hope that makes the proposition emotionally legible. It must come from mission/research evidence rather than invented melodrama.",
    emotional_contradiction:
      "Two feelings, needs, beliefs or pressures that cannot both remain comfortably true; this creates inner tension rather than merely external plot activity.",
    vulnerability_or_risk:
      "What can be lost, exposed, disappointed, left unresolved or placed under pressure at a human level. Business and technology stories may use trust, control, responsibility, pride, uncertainty or consequence when evidence supports them.",
    empathy_path:
      "How observable action, behaviour, consequence, sound or withheld information moves the audience from watching the subject to feeling with them.",
    escalation:
    ...
    call_to_action:
      "Action integrated into the resolution, or explicit no-CTA reasoning, including the form it takes rather than only its wording.",
    emotional_arc: "Precise emotional progression with at least four distinct audience state changes, each transition caused by an observable story event rather than a list of mood adjectives.",
    emotional_payoff:
      "The earned final feeling and the causal reason it can land only after the preceding desire, contradiction, vulnerability, proof and turn have accumulated.",
  ...
  scenes: Object.freeze({
    ...
    emotional_job:
      "Specific felt-state change this scene must cause in the audience and the observable event, behavior, consequence, sound or withholding that causes it.",
    human_stake:
      "Concrete desire, vulnerability, responsibility, relationship or consequence made tangible in this scene; never a generic label such as humanity or connection.",
  ...
  shot: Object.freeze({
    ...
    first_pass_intent:
      "Mandatory pre-generation self-critique record containing story_delta, visible_event, edit_reason, continuity_anchor, sound_picture_event and predicted_failure. The director must solve these before generation; downstream review is a backstop, not the authoring mechanism.",
    device:
      "How this shot participates in the work's signature device, or an explicit statement that this shot is deliberately plain and what the plainness is doing for the moment that follows it. Plain shots are legitimate and necessary -- a device applied to every shot is decoration -- but plainness must be a decision, not an omission. Do not restate the camera movement here; state what the shot is doing that coverage would not.",
    ...
    graphics:
      "Typography and graphic behavior as a creative instrument, not a caption layer: what text or mark appears, how it enters, moves, breaks, obscures, reveals or interacts with the image, and what it contributes that the image alone could not. Type may lead the frame, disagree with the picture, arrive early, or be absent entirely as a decision. Exact text and logos still remain outside generated pixels when fidelity matters, which is a rendering constraint on where the pixels come from and never a limit on what the type is allowed to do.",
    vfx:
      "Effects, cleanup and compositing. Invisible credibility is the correct default for continuity and repair work, and an effect may equally be the idea itself -- a transformation, an impossible continuity, a material behaving wrongly, a scale break, a persistence across a cut. Decide which of the two this shot is doing and say what the effect is for.",
...
const STILL_ART_DIRECTION_SCHEMA = Object.freeze({
  art_direction: Object.freeze({
    governing_visual_idea: "One concrete visual proposition that governs the whole still system and cannot be replaced by a generic category aesthetic.",
    focal_hierarchy: "Exact primary, secondary and tertiary attention order, including what must dominate and what must stay quiet.",
    composition_strategy: "Observable spatial composition rules: scale, placement, asymmetry/symmetry, tension, rhythm and relationship between hero imagery and exact typography/logo layers.",
    negative_space_strategy: "Where negative space is protected, why it exists and which responsive crops must preserve it.",
    depth_strategy: "Foreground, midground, background, atmosphere, occlusion and scale logic or another explicit depth system.",
    material_language: "Specific materials, surface qualities, reflections, texture, wear/precision and what would make them look cheap or synthetic.",
    lighting_palette: "Motivated light direction, contrast, palette, highlight behavior, shadow detail and forbidden color/lighting drift.",
    typography_strategy: "Role of type in the composition, hierarchy, placement behavior and what must remain deterministic outside generated pixels.",
    brand_asset_strategy: "Exact source-locked logos/marks/products/text, their placement/compositing treatment and what generation must leave blank or omit.",
    crop_responsive_rules: "How the visual survives required aspect ratios without reducing the work to center-crop automation.",
    production_world_rules: "Buildable environment/object/world rules owned by production design: geometry, scale, materials, props, surfaces and continuity.",
    generation_requirements: "Structured visual requirements the image worker must execute; the worker may not reinterpret the governing art direction.",
    finishing_requirements: "Retouching, compositing, edges, typography, logo, color and export evidence required before release.",
    forbidden_devices: "JSON array of concrete visual shortcuts/cliches explicitly prohibited for this direction.",
    review_criteria: "JSON array of inspectable intended-vs-rendered criteria the Art Director must use to accept or reject the result.",
  }),
});
...
const WORKFLOW_CONTRACTS = Object.freeze({
  ...
  STILL: Object.freeze({
    ...
    required_sections: Object.freeze(["art_direction", "deliverables", "production"]),
    structured_schema: STILL_ART_DIRECTION_SCHEMA,
    craft_contract: Object.freeze({
      art_authority:
        "STILL work requires active executive creative, brand, art-direction, production-design and asset-intelligence authority. The Art Director owns the governing visual idea and the Production Designer owns the buildable visual world; image generation is a worker and may not substitute for either decision.",
      composition:
      ...
      image_direction:
        "Source treatment, photography/illustration/generation/compositing decisions, exact-asset fidelity, material behavior and depth cues. Generated imagery must execute an approved Art Direction Dossier rather than invent the design at provider time.",
      typography:
...
const GLOBAL_RULES = Object.freeze([
  ...
  "For STILL work, executive_creative_director, brand_director, art_director, production_designer and asset_intelligence_director are non-waivable creative authorities. No image-generation step may replace their decisions, and no still may reach release through a generator-plus-layout shortcut.",
  ...
  "When autonomous research supplies creative_grounding, consume it as evidence rather than decoration: preserve every SELECTED real-world reference role and continuity constraint relevant to the deliverable, account for unresolved grounding explicitly, and never replace a grounded subject with a generic category approximation.",
  "When creative_grounding.spatial_path.required is true, its ordered nodes and directed edges are canonical spatial evidence. Any planned aerial/drone camera movement must use the existing aerial_cinematography contract and preserve that directionality from origin through waypoints toward destination. Reject visually attractive camera paths that move against the grounded route, reverse waypoint order, teleport between nodes, or substitute a different landmark/location.",
...
// The seven workflow contracts are 10,345 of the 25,899 characters in this request -- forty per
// cent of it -- and six of them describe a medium the job is not in. An audio package was carrying
// the full craft contracts for temporal, still, document, interactive, software and campaign work,
// all of them visually oriented, and audio has been the weakest case in every run.
//
// When the operative workflow is already known the others are reduced to their identity: kind,
// aliases and executor. That is everything needed to confirm the choice was the right one and to
// recognise an alias, without carrying craft guidance for a medium being not used. The full
// contract is still sent for every workflow when the job's medium is genuinely undetermined,
// because then the model does need them all to choose between.
export function buildCreativeMasterPlanDecisionContract(operativeWorkflowKind = null) {
  // This module has no text() helper of its own; normalising inline rather than reaching for one
  // that does not exist here.
  const operative = String(operativeWorkflowKind ?? "").trim().toUpperCase();
  ...
  const workflows = CreativeWorkflowRegistry.list().map((workflow) => {
    ...
    if (operative && workflow.workflow_kind !== operative) return identity;
    return { ...identity, contract: getCreativeWorkflowContract(workflow.workflow_kind) };
  });
