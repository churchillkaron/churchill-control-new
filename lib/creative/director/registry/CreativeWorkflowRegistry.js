...
function findRegistered(value, source = "UNSPECIFIED") {
  const normalized = normalize(value);
  if (!normalized) return null;
  const workflow = byAlias.get(normalized) || null;
  return workflow
...
export const CreativeWorkflowRegistry = Object.freeze({
  ...
  resolveAlias(value) {
    return findRegistered(value, "ALIAS");
  },
  ...
  require(workflowKind) {
    // Aliases were declared on every workflow and never consulted here, so a plan
    // naming IMAGE, VIDEO, FILM, POSTER or MENU was rejected as an unregistered
    // workflow even though the registry lists those exact terms. Canonical kinds
    // are tried first so a canonical name always wins.
    const normalized = normalize(workflowKind);
    const workflow = byKind.get(normalized) || byAlias.get(normalized) || null;
    if (!workflow) {
      throw new Error(
