...
function stateFor(projectId) {
  ...
  if (!ROUND_STATE.has(projectId)) {
    ROUND_STATE.set(projectId, {
      round: 1,
      attempts: [],
      current: {
        concepts: {},
  ...
  return ROUND_STATE.get(projectId);
}
...
function captureResult(input, result) {
  const projectId = projectKey(input);
  if (!projectId) return;
...
function scoreFailures(state) {
  ...
  for (const [criticId, report] of Object.entries(state.current.critics)) {
    const minimum = finite(WORLD_CLASS_CONCEPT_POLICY.critic_minimums[criticId]);
        ...
        ? {
            ...
            metadata: {
              ...
              autonomous_concept_regeneration_contract:
                WORLD_CLASS_CONCEPT_POLICY.regeneration.contract,
              autonomous_concept_round: state?.round || 1,
            },
          }
        : input,
    );
...
function requestedMaxRounds(input = {}) {
  const configured = finite(
    input.project?.metadata?.creative_intelligence_policy?.regeneration?.max_rounds ??
    input.brief?.metadata?.creative_intelligence_policy?.regeneration?.max_rounds ??
    input.project?.metadata?.concept_regeneration_max_rounds ??
    input.brief?.metadata?.concept_regeneration_max_rounds,
  );
  const fallback = WORLD_CLASS_CONCEPT_POLICY.regeneration.default_max_rounds;
  return clamp(
    configured === null ? fallback : Math.floor(configured),
    1,
    WORLD_CLASS_CONCEPT_POLICY.regeneration.hard_max_rounds,
  );
}
...
function enrichInput(input, state) {
  if (!state?.diagnostic) return input;
  return {
    ...input,
    brief: {
      ...object(input.brief),
...
function finalizeResult(result, state, maxRounds) {
  const history = state.attempts.slice(
    -WORLD_CLASS_CONCEPT_POLICY.regeneration.persisted_failure_history_limit,
  );
  const evidence = {
    contract: WORLD_CLASS_CONCEPT_POLICY.regeneration.contract,
    passed: true,
    ...
    provider_execution: WORLD_CLASS_CONCEPT_POLICY.regeneration.provider_execution,
  };
  return {
    ...result,
    plan: {
...
function install() {
  ...
  CreativeUniversalTemporalDirectionRuntime.create = async function createWithAutonomousConceptRegeneration(input = {}) {
    ...
    ROUND_STATE.delete(projectId);
    const state = stateFor(projectId);
    const maxRounds = requestedMaxRounds(input);
        ...
        try {
          ...
          if (
            ...
            separation.similarity >
              WORLD_CLASS_CONCEPT_POLICY.regeneration.maximum_cross_round_similarity
          ) {
          ...
          return finalizeResult(result, state, maxRounds);
        } catch (error) {
