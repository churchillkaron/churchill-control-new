...
} from "./CreativeConceptCouncilRuntime";
import {
...
const RETRYABLE_FAILURES = [
  ...
  "CREATIVE_EXECUTIVE_CONCEPT_SELECTION_REJECTED_LINEAGE",
  "CREATIVE_EXECUTIVE_CONCEPT_SELECTION_FLAGSHIP_REJECTED",
  "INDEPENDENT_CONCEPT_PREMIUM_FLAGSHIP_",
  "INDEPENDENT_CONCEPTS_SEMANTICALLY_TOO_SIMILAR",
  "INDEPENDENT_CONCEPT_TITLES_NOT_DISTINCT",
];

function councilRegenerationOperation(value) {
  ...
  return (
    ...
    operation.startsWith("CREATIVE_SELECTED_CONCEPT_")
  );
}
...
function fallbackCriticFailures(state, error) {
  const card = object(error?.selected_scorecard);
  const scores = object(card.critic_scores);
  return Object.entries(state.policy.critic_minimums).flatMap(([critic, required]) => {
    const score = finite(scores[critic]);
    if (score !== null && score >= required) return [];
    return [{
      concept_id: text(card.concept_id),
      critic,
      score,
      required,
      failures: [],
      cliche_or_risk_hits: [],
      mandatory_repairs: [],
      rejection_reason: "World-class critic floor not met.",
    }];
  });
}

function failureDiagnostic(state, error) {
  const errorCouncil = object(error?.council);
  const capturedConcepts = Object.values(state.current.concepts);
  const concepts = (capturedConcepts.length ? capturedConcepts : list(errorCouncil.concepts))
    .map(conceptSummary);
  const capturedFailures = scoreFailures(state);
  const criticFailures = capturedFailures.length
    ? capturedFailures
    : fallbackCriticFailures(state, error);
...
function councilFromResult(result = {}) {
  return object(
    result.council ||
...
function selectedConcept(result = {}) {
  return object(councilFromResult(result).selection?.selected_concept);
}

function assertWorldClassCouncilResult(result = {}, policy = WORLD_CLASS_CONCEPT_POLICY) {
  const council = councilFromResult(result);
  const selection = object(council.selection);
  const selectedId = text(selection.selected_concept_id || selection.selected_concept?.id);
  const card = list(council.scorecards).find((row) => text(row.concept_id) === selectedId);
  if (!selectedId || !card) {
    const error = new Error("CREATIVE_WORLD_CLASS_CONCEPT_SELECTION_REQUIRED");
    error.council = council;
    throw error;
  }
  const scores = object(card.critic_scores);
  const failed = Object.entries(policy.critic_minimums)
    .filter(([critic, minimum]) => {
      const score = finite(scores[critic]);
  ...
  if (
    card.all_critics_passed !== true ||
    failed.length ||
    weighted === null ||
    weighted < policy.minimum_weighted_score
  ) {
    const labels = [
      ...failed,
      weighted === null || weighted < policy.minimum_weighted_score ? "weighted_score" : null,
      card.all_critics_passed !== true && !failed.length ? "critic_veto" : null,
    ].filter(Boolean);
    const error = new Error(`CREATIVE_WORLD_CLASS_CONCEPT_SCORE_FAILED:${labels.join(",")}`);
    error.council = council;
    error.selected_scorecard = card;
    throw error;
  }
  const confidence = finite(selection.confidence);
  if (confidence === null || confidence < policy.minimum_selector_confidence) {
    const error = new Error("CREATIVE_WORLD_CLASS_CONCEPT_CONFIDENCE_FAILED");
    error.council = council;
    error.selected_scorecard = card;
    throw error;
  }
...
function directionApprovalOperationsForRound(input = {}, state) {
  const operations = list(input.directionApprovalOperations);
  if (state.round <= 1) return operations;
  return operations.filter((entry) => !councilRegenerationOperation(entry?.operation));
}
...
function enrichInput(input, state) {
  return {
    ...
    directionApprovalOperations: directionApprovalOperationsForRound(input, state),
    project: {
...
async function executeWithRegeneration({ input = {}, execute }) {
  const existingState = ROUND_CONTEXT.getStore();
  if (existingState) return execute(enrichInput(input, existingState));

  const projectId = text(input.project?.id || input.creative_project_id || input.project_id);
  if (!projectId) return execute(input);
  ...
  return ROUND_CONTEXT.run(state, async () => {
    while (state.round <= maxRounds) {
      ...
      try {
        const result = await execute(enrichInput(input, state));
        assertWorldClassCouncilResult(result, policy);
        ...
        if (
          ...
          error.council = councilFromResult(result);
          throw error;
        }
        ...
        const errorConcepts = list(error?.council?.concepts);
        const failedConcepts = Object.values(state.current.concepts);
        state.previous_concepts.push(...(failedConcepts.length ? failedConcepts : errorConcepts));
        state.attempts.push({
          round: state.round,
          diagnostic,
          concept_count: failedConcepts.length || errorConcepts.length,
        });
...
function install() {
  ...
  if (!CreativeConceptCouncilRuntime[INSTALL_FLAG]) {
    const run = CreativeConceptCouncilRuntime.run.bind(CreativeConceptCouncilRuntime);
    Object.defineProperty(CreativeConceptCouncilRuntime, INSTALL_FLAG, {
      value: true,
      enumerable: false,
      configurable: false,
    });
    CreativeConceptCouncilRuntime.run = async function runWithAutonomousConceptRegeneration(input = {}, directed = {}) {
      return executeWithRegeneration({
        input: {
          ...input,
          directionApprovalOperations: list(input.project?.metadata?.paid_direction_approval?.operations),
        },
        execute: (roundInput) => run({
          ...roundInput,
          project: {
            ...object(roundInput.project),
            metadata: {
              ...object(roundInput.project?.metadata),
              paid_direction_approval: {
                ...object(roundInput.project?.metadata?.paid_direction_approval),
                operations: list(roundInput.directionApprovalOperations),
              },
            },
          },
        }, directed),
  ...
  CreativeUniversalTemporalDirectionRuntime.create = async function createWithAutonomousConceptRegeneration(input = {}) {
    // An explicitly supplied approved Master is downstream concept authority.
    // Production resume may still need to build missing temporal scenes, but it
    // must not reopen concept competition after Council/Tribunal lineage has
    // already selected the concept. Fresh direction without an approved Master
    // continues through the strict autonomous-regeneration policy below.
    const approvedPlan = object(input.approved_master?.plan);
    if (text(approvedPlan.concept?.id)) {
      return create(input);
    }
    return executeWithRegeneration({ input, execute: create });
  };
