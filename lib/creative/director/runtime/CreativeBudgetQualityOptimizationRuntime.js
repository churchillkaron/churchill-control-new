...
function attemptsFor(tier, shot = {}) {
  const strategy = object(shot.generation_strategy);
  const candidatePolicy = object(strategy.candidate_policy);
  const boundedMaximum = finite(candidatePolicy.maximum_paid_candidates);
  ...
  return {
    ...
    maximum_total_candidates: boundedMaximum !== null ? Math.max(1, Math.min(3, boundedMaximum)) : defaultMaximum,
    maximum_repair_rounds: tier === "HERO" || tier === "CRITICAL" ? 2 : 1,
    shotgun_generation_forbidden: true,
    alternate_requires_failed_review: true,
    surgical_repair_before_regeneration: true,
  };
}
...
function buildPolicy(shot, index, total) {
  ...
  const policy = {
    ...
    generation_budget: attemptsFor(tier, shot),
...
export const CreativeBudgetQualityOptimizationRuntime = Object.freeze({
  ...
  build(input = {}) {
    ...
    const summary = {
      ...
      optimization_policy: "START_ONE_CANDIDATE_PER_SHOT_THEN_ALLOCATE_REPAIR_OR_ALTERNATE_BUDGET_ONLY_AFTER_REVIEW_FAILURE_WITHOUT_LOWERING_QUALITY_FLOORS",
      quality_floor_may_not_be_lowered: true,
      provider_selection_owner: "SERVICE_RUNTIME",
      provider_calls_executed: 0,
    };
