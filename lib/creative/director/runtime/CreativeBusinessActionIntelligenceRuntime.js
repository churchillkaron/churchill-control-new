...
function businessActionValidationContext({ plan = {}, project = {}, brief = {} } = {}) {
  const duration = temporalDuration(plan, project, brief);
  const scenes = list(plan.scenes);
  const sceneCount = scenes.length || finite(project.metadata?.scene_count) || null;
  const plannedShotCount = scenes.reduce(
    (total, scene) => total + list(scene.shots).length,
    0,
  );
  const shotCount = plannedShotCount || finite(project.metadata?.shot_count) || null;
  const singleContinuousShot =
    project.metadata?.single_continuous_shot === true ||
    plan.temporal_contract?.single_continuous_shot === true ||
    (sceneCount === 1 && shotCount === 1);

  return {
    duration_seconds: duration,
    scene_count: sceneCount,
    shot_count: shotCount,
    single_continuous_shot: singleContinuousShot,
  };
}

function isShortFormSingleShot(context = {}) {
  const duration = finite(context.duration_seconds);
  const sceneCount = finite(context.scene_count);
  const shotCount = finite(context.shot_count);
  return Boolean(
    duration !== null &&
    duration <= 5 &&
    (
      context.single_continuous_shot === true ||
      (sceneCount === 1 && shotCount === 1)
    )
  );
}
...
function prompt({ input = {}, directed = {} } = {}) {
  ...
  const validationContext = businessActionValidationContext({
    plan,
    project,
    brief,
  });
  const duration = validationContext.duration_seconds;
  const minimumWowHypotheses = isShortFormSingleShot(validationContext) ? 1 : 2;
...
- WOW HYPOTHESIS DEPTH: produce at least ${minimumWowHypotheses} complete, distinct wow hypothesis${minimumWowHypotheses === 1 ? "" : "es"}. For a five-second single continuous shot, one concentrated truthful payoff is valid; do not invent a second spectacle merely to satisfy count.
...
export function validateCreativeBusinessActionIntelligence(
  value = {},
  validationContext = {},
) {
  ...
  const minimumWowHypotheses = isShortFormSingleShot(validationContext) ? 1 : 2;
  const wowHypotheses = list(source.wow_hypotheses);
  if (wowHypotheses.length < minimumWowHypotheses) {
    failures.push("BUSINESS_ACTION_WOW_HYPOTHESES_REQUIRED");
  }
  wowHypotheses.forEach((item, index) => {
    requireText(item.setup, `BUSINESS_ACTION_WOW_SETUP_REQUIRED:${index + 1}`, 12);
    requireText(item.visible_payoff, `BUSINESS_ACTION_WOW_VISIBLE_PAYOFF_REQUIRED:${index + 1}`, 20);
    requireText(item.sonic_payoff, `BUSINESS_ACTION_WOW_SONIC_PAYOFF_REQUIRED:${index + 1}`, 16);
    requireText(item.truth_anchor, `BUSINESS_ACTION_WOW_TRUTH_ANCHOR_REQUIRED:${index + 1}`, 16);
    requireText(item.audience_effect, `BUSINESS_ACTION_WOW_AUDIENCE_EFFECT_REQUIRED:${index + 1}`, 12);
  });
...
export async function createCreativeBusinessActionIntelligence({
  ...
  return {
    intelligence: validateCreativeBusinessActionIntelligence(
      output,
      validationContext,
    ),
    result,
