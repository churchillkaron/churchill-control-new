...
import {
  bootstrapCinematicImpactFromPlan,
} from "./CreativeCinematicImpactBootstrapRuntime";
...
function parseReasoningJson(value) {
...
function normalizedReasoningOutput(result = {}) {
  ...
  while (queue.length) {
    ...
    if (depth > 12 || current == null) continue;
    ...
    const candidate = typeof current === "string"
      ...
      : current && typeof current === "object"
        ...
        : null;
    const parsed = candidate ? parseReasoningJson(candidate) : null;
    if (parsed) return parsed.result && typeof parsed.result === "object" ? parsed.result : parsed;
    if (current && typeof current === "object" && !Array.isArray(current)) {
      if (text(current.contract).startsWith("CREATIVE_")) return current.result || current;
      for (const key of ["output", "result", "data", "response", "raw", "provider_result"]) {
        if (current[key] != null) queue.push({ value: current[key], depth: depth + 1 });
      }
    }
  }
  return null;
...
async function executeReasoning({ input, operation, prompt, maxOutputTokens, semanticIdentity = null, executionLane = null }) {
  ...
  const result = await ServiceExecutionRuntime.execute({
    ...
    metadata: {
      ...
      creative_direction_semantic_hash: semanticIdentity ? digest(semanticIdentity) : null,
    },
  });
...
function minimumUnderstandingWowHypotheses(input = {}, directed = {}) {
...
function explicitlyForbidsVisibleActors(plan = {}) {
  const scenes = list(plan.scenes);
  if (!scenes.length) return false;

  const actorFree = scenes.every(
    (scene) =>
      list(scene.actors).length === 0 &&
      list(scene.shots).every((shot) => list(shot.actors).length === 0),
  );
  if (!actorFree) return false;

  const constraints = scenes
    .flatMap((scene) =>
  ...
  return constraints.some(
    (rule) =>
      rule.includes("no visible human actor") ||
      rule.includes("no visible human") ||
      rule.includes("no visible actor") ||
      rule.includes("without visible human"),
  );
}

function visiblePopulationRequired(plan = {}, understanding = {}) {
  if (explicitlyForbidsVisibleActors(plan)) return false;
  return object(understanding.human_ecosystem).required === true;
}

function allShotsHaveControlledCamera(plan = {}) {
  const shots = list(plan.scenes).flatMap((scene) => list(scene.shots));
  return Boolean(
    shots.length &&
    shots.every(
      (shot) =>
        text(shot.camera?.stabilization).length >= 8 &&
        text(shot.camera?.focus_transition).length >= 8,
    )
  );
}

function hasStructuredPacing(plan = {}) {
  const contract = object(plan.cinematic_impact_contract);
  const scenes = list(plan.scenes);
  const shots = scenes.flatMap((scene) => list(scene.shots));
  return Boolean(
    text(contract.retention_strategy).length >= 30 &&
    scenes.every(
      (scene) =>
        text(scene.metadata?.cinematic_impact?.action_escalation).length >= 20,
    ) &&
    shots.every(
      (shot) =>
        text(shot.camera?.movement_speed).length >= 8 &&
        text(shot.frame_plan?.progression).length >= 16,
    )
  );
}

function hasLightingVariation(plan = {}) {
  const contract = object(plan.cinematic_impact_contract);
  const scenes = list(plan.scenes);
  const shots = scenes.flatMap((scene) => list(scene.shots));
  const contractLanguage = normalized([
    contract.retention_strategy,
    contract.action_grammar,
    contract.environment_transformation_thesis,
  ].join(" "));
  return Boolean(
    (contractLanguage.includes("light") || contractLanguage.includes("reflection")) &&
    scenes.every(
      (scene) =>
        text(scene.metadata?.cinematic_impact?.action_escalation).length >= 20,
    ) &&
    shots.every((shot) => Object.keys(object(shot.lighting)).length > 0)
  );
}

// The brand-subject test previously keyed on hospitality words ("menu",
// "signature glass"), so a repair request about a clinic's equipment or a
// factory's output never registered as brand-related while a restaurant's did.
// These tokens name the concept rather than any industry's version of it.
const BRAND_SUBJECT_TOKENS = Object.freeze([
  "brand",
  "product",
  "logo",
  "signage",
...
function mentionsBrandSubject(value) {
  const haystack = String(value ?? "").toLowerCase();
  return BRAND_SUBJECT_TOKENS.some((token) => haystack.includes(token));
}

function hasBrandAnchoring(plan = {}) {
  return list(plan.scenes).some((scene) =>
    Boolean(
      text(scene.location?.name) &&
      (
        list(scene.products).length > 0 ||
        list(scene.shots).some(
          (shot) =>
            text(shot.continuity?.product) ||
            text(shot.production_design?.props),
        )
      )
    ),
  );
...
function critiqueRepairDisposition(plan = {}, repair = {}) {
  const repairText = normalized([
    repair.problem,
    repair.required_change,
    repair.reason,
  ].join(" "));

  if (
    explicitlyForbidsVisibleActors(plan) &&
    (
      repairText.includes("visible human") ||
      repairText.includes("human presence") ||
      repairText.includes("visible people") ||
      repairText.includes("visible actor") ||
      repairText.includes("silhouette") ||
      repairText.includes("hand gesture")
    )
  ) {
    return "CONFLICTS_WITH_APPROVED_NO_VISIBLE_ACTORS_CONSTRAINT";
  }

  if (
    (
      repairText.includes("stabil") ||
      repairText.includes("jitter") ||
      repairText.includes("focus")
    ) &&
    allShotsHaveControlledCamera(plan)
  ) {
    return "ALREADY_CONTROLLED_BY_CAMERA_PLAN";
  }

  if (
    (
      repairText.includes("pacing") ||
      repairText.includes("timing") ||
      repairText.includes("five second") ||
      repairText.includes("5 second") ||
      repairText.includes("duration")
    ) &&
    hasStructuredPacing(plan)
  ) {
    return "ALREADY_CONTROLLED_BY_TEMPORAL_PLAN";
  }

  if (
    (
      repairText.includes("lighting") ||
      repairText.includes("reflection")
    ) &&
    hasLightingVariation(plan)
  ) {
    return "ALREADY_CONTROLLED_BY_LIGHTING_AND_ACTION_PLAN";
  }

  if (
    mentionsBrandSubject(repairText) &&
    hasBrandAnchoring(plan)
  ) {
    return "ALREADY_CONTROLLED_BY_BRAND_ANCHORS";
  }
...
function understandingPrompt(input = {}, directed = {}) {
  ...
  const minimumWowHypotheses = minimumUnderstandingWowHypotheses(
    input,
    directed,
  );
...
Return strict JSON only:
{
  ...
  "human_ecosystem": {
    "required": true,
    "reason": "evidence-based reason visible story-world people are or are not essential on screen",
    "role_categories": [{
      "role": "evidence-derived on-screen story-world role category",
      "story_function": "why this visible role matters inside the story world",
      "visible_behaviours": ["specific believable on-screen action"],
      "identity_mode": "VERIFIED_IDENTITY|GENERATED_SUPPORTING_CAST"
    }],
    "social_dynamics": ["on-screen interaction pattern that makes the story world feel alive"]
  },
...
- Produce at least ${minimumWowHypotheses} distinct, evidence-grounded wow hypotheses. For a five-second single continuous shot, two strong alternatives are sufficient exploration; do not invent a third spectacle solely to satisfy count.
- human_ecosystem is strictly about visible people inside the story world. Camera operators, lighting technicians, sound designers, editors, set crews and other behind-the-scenes production staff are production support, not on-screen population, and must never make human_ecosystem.required true by themselves.
- Respect explicit actor and identity constraints from the approved plan. If visible actors are forbidden, human_ecosystem.required must be false and role_categories must be empty; use environment, sound, objects, reflections and other lawful story devices instead.
- If the real experience is social or operational and visible people are permitted, identify the on-screen human ecosystem needed to make it believable. Do not default to an empty physical world when the approved truth requires people.
...
function validateUnderstanding(value = {}, minimumWowHypotheses = 3, minimumHighValueActions = 3) {
  const failures = [];
  ...
  if (list(action.high_value_actions).length < minimumHighValueActions) {
  ...
  if (list(source.wow_hypotheses).length < minimumWowHypotheses) {
...
async function createUnderstanding(input = {}, directed = {}) {
  ...
  return {
    understanding: validateUnderstanding(
      execution.output,
      minimumWowHypotheses,
    ),
...
function evaluateImpactPlan(plan = {}) {
  ...
  const framings = shots
    ...
    .filter(Boolean);
  const cameraPlatforms = shots
    .map((shot) => normalized(shot.camera?.platform))
    .filter(Boolean);
  const lensIntents = shots
    .map((shot) => normalized(shot.camera?.lens_intent))
    .filter(Boolean);
  const shortFormHero = duration <= 6;
  const distinctCameraMinimum = shortFormHero
    ? Math.min(shots.length, 2)
    : Math.min(shots.length, Math.max(3, Math.ceil(shots.length * 0.6)));
  ...
  const distinctMovementMinimum = shortFormHero
    ? Math.min(shots.length, 1)
    : Math.min(shots.length, Math.max(3, Math.ceil(shots.length * 0.35)));
  ...
  const distinctFramingMinimum = shortFormHero
    ? Math.min(shots.length, 1)
    : Math.min(shots.length, Math.max(3, Math.ceil(shots.length * 0.3)));
  ...
  if (!shortFormHero && shots.length >= 6 && new Set(lensIntents).size < 3) {
    failures.push("LENS_INTENT_DIVERSITY_TOO_LOW");
  }
  const distinctPlatformMinimum = shortFormHero
    ? 1
    : shots.length >= 16 ? 4 : shots.length >= 8 ? 3 : shots.length >= 4 ? 2 : 1;
  if (new Set(cameraPlatforms).size < Math.min(shots.length, distinctPlatformMinimum)) {
    failures.push(
      `CAMERA_PLATFORM_DIVERSITY_TOO_LOW:${new Set(cameraPlatforms).size}:${Math.min(shots.length, distinctPlatformMinimum)}`,
    );
  }
  const platformCounts = cameraPlatforms.reduce((map, value) => {
    map.set(value, Number(map.get(value) || 0) + 1);
    return map;
  }, new Map());
  const maximumRepeatedPlatform = Math.max(...platformCounts.values(), 0);
  if (!shortFormHero && shots.length >= 8 && maximumRepeatedPlatform > Math.ceil(shots.length * 0.55)) {
    failures.push(
      `DOMINANT_CAMERA_PLATFORM_REPEATED_TOO_OFTEN:${maximumRepeatedPlatform}:${shots.length}`,
    );
  }
  const pushZoomCount = movementPaths.filter((value) => /\b(push|push in|push-in|zoom|dolly in)\b/.test(value)).length;
  if (!shortFormHero && shots.length >= 6 && pushZoomCount > Math.ceil(shots.length * 0.35)) {
    failures.push(`DEFAULT_PUSH_ZOOM_LANGUAGE_OVERUSED:${pushZoomCount}:${shots.length}`);
  }
  for (let index = 1; index < cameraSignatures.length; index += 1) {
    if (!shortFormHero && cameraSignatures[index] === cameraSignatures[index - 1]) {
  ...
  const effectivePopulationRequired = visiblePopulationRequired(
    plan,
    understanding,
  );
  if (
    !explicitlyForbidsVisibleActors(plan) &&
    population.required !== effectivePopulationRequired
  ) {
    failures.push("POPULATION_STRATEGY_UNDERSTANDING_MISMATCH");
  }
  if (effectivePopulationRequired) {
  ...
  return {
    ...
    population_required: effectivePopulationRequired,
    population_role_count: new Set(roleValues(plan)).size,
    visible_actor_constraint_enforced: explicitlyForbidsVisibleActors(plan),
  };
}
  ...
  return `
{
  "cinematic_impact_contract": {
    ...
    "population_strategy": {
      ...
      "role_categories": ["evidence-derived visible on-screen role category"],
      "reason": "why visible story-world people are or are not required according to the supplied understanding and approved constraints"
    },
    "wow_moments": [{
...
- On-screen population is governed by the approved plan and identity constraints. Behind-the-scenes production crew never count as visible population. Never add visible people when the approved negative constraints forbid them.
- Build scenes with foreground, midground and background life whenever visible story-world people are required and permitted. When visible actors are forbidden, create life through lawful environmental action, objects, light, sound, reflections and spatial change instead.
- Derive visible human roles dynamically from research and business truth. Supporting cast is role-based, never invented named identities.
...
- Make behaviour specific and consequential. When people are permitted, they must affect objects, space, each other or the story state rather than exist as decorative extras.
...
${JSON.stringify(critiqueUnderstandingPacket(understanding))}
...
function critiqueEvidencePacket(plan = {}) {
  const scenes = list(plan.scenes);
  return {
    workflow_kind: plan.workflow_kind,
    concept: {
      hook: text(plan.concept?.hook),
      creative_thesis: text(plan.concept?.creative_thesis),
      emotional_promise: text(plan.concept?.emotional_promise),
      call_to_action: text(plan.concept?.call_to_action),
      target_audience: text(plan.concept?.target_audience?.description),
      refused_devices: list(plan.concept?.refused_devices),
    },
    story: {
      audience_tension: text(plan.story?.audience_tension),
      observable_proof: text(plan.story?.observable_proof),
      turn: text(plan.story?.turn),
      escalation: text(plan.story?.escalation),
      resolution: text(plan.story?.resolution),
      emotional_arc: text(plan.story?.emotional_arc),
    },
    deliverable: (() => {
      const deliverable = object(list(plan.deliverables)[0]);
      return {
        duration_seconds: finite(deliverable.duration_seconds, null),
        aspect_ratio: text(deliverable.output_spec?.aspect_ratio || deliverable.aspect_ratio),
        resolution: text(deliverable.output_spec?.resolution || deliverable.resolution),
        frame_rate: finite(deliverable.output_spec?.frame_rate || deliverable.frame_rate, null),
        audio_required: deliverable.output_spec?.audio_required === true || deliverable.audio_required === true,
      };
    })(),
    cinematic_impact_contract: object(plan.cinematic_impact_contract),
    asset_evidence: list(plan.asset_manifest).map((entry) => ({
      asset_id: text(entry?.asset_id || entry?.id),
      role: text(entry?.role || entry?.type),
      reason: text(entry?.reason).slice(0, 240),
    })),
    scenes: scenes.map((scene) => ({
      id: text(scene.id),
      objective: text(scene.objective),
      emotion: text(scene.emotion),
      story_state_before: text(scene.story_state_before),
      state_change: text(scene.state_change),
      story_state_after: text(scene.story_state_after),
      location: {
        name: text(scene.location?.name),
        atmosphere: text(scene.location?.atmosphere),
        scale: text(scene.location?.scale),
        reference_asset_id: text(scene.location?.reference_asset_id),
      },
      reference_asset_ids: list(scene.reference_asset_ids),
      tension: {
        pressure_before: finite(scene.tension?.pressure_before, null),
        pressure_after: finite(scene.tension?.pressure_after, null),
        payoff: text(scene.tension?.payoff),
      },
      cinematic_impact: {
        attention_hook: text(scene.metadata?.cinematic_impact?.attention_hook),
        action_escalation: text(scene.metadata?.cinematic_impact?.action_escalation),
        reveal_or_payoff: text(scene.metadata?.cinematic_impact?.reveal_or_payoff),
      },
      shots: list(scene.shots).map((shot) => ({
        id: text(shot.id),
        duration_seconds: finite(shot.duration_seconds, 0),
        subject: text(shot.subject),
        purpose: text(shot.purpose),
        action: text(shot.action),
        camera: {
          platform: text(shot.camera?.platform),
          framing: text(shot.camera?.framing),
          lens_intent: text(shot.camera?.lens_intent),
          movement_path: text(shot.camera?.movement_path),
          movement_motivation: text(shot.camera?.movement_motivation),
        },
        production_design: {
          environment: text(shot.production_design?.environment),
          texture_detail: text(shot.production_design?.texture_detail),
        },
        audio: {
          source_sound: text(shot.audio?.source_sound),
          mix_intent: text(shot.audio?.mix_intent),
        },
        negative_constraints: list(shot.negative_constraints).slice(0, 4),
        cinematic_impact: {
          attention_mechanism: text(shot.metadata?.cinematic_impact?.attention_mechanism),
          visual_question: text(shot.metadata?.cinematic_impact?.visual_question),
          payoff_or_reveal: text(shot.metadata?.cinematic_impact?.payoff_or_reveal),
        },
      })),
    })),
  };
...
function critiqueUnderstandingPacket(understanding = {}) {
  return {
    audience_model: {
      primary_audience: text(understanding.audience_model?.primary_audience),
      current_state: text(understanding.audience_model?.current_state),
      desired_state: text(understanding.audience_model?.desired_state),
      attention_triggers: list(understanding.audience_model?.attention_triggers).slice(0, 4),
      attention_risks: list(understanding.audience_model?.attention_risks).slice(0, 4),
      credibility_signals: list(understanding.audience_model?.credibility_signals).slice(0, 4),
      emotional_sequence: list(understanding.audience_model?.emotional_sequence).slice(0, 5),
    },
    human_ecosystem: {
      required: understanding.human_ecosystem?.required === true,
      reason: text(understanding.human_ecosystem?.reason),
      role_categories: list(understanding.human_ecosystem?.role_categories),
    },
    environment_intelligence: {
      recognition_anchors: list(understanding.environment_intelligence?.recognition_anchors),
      scale_opportunities: list(understanding.environment_intelligence?.scale_opportunities),
      sensory_contrasts: list(understanding.environment_intelligence?.sensory_contrasts),
    },
    action_intelligence: {
      high_value_actions: list(understanding.action_intelligence?.high_value_actions).slice(0, 6),
      escalation_logic: text(understanding.action_intelligence?.escalation_logic),
    },
    camera_intelligence: {
      principles: list(understanding.camera_intelligence?.principles),
      repetition_risks: list(understanding.camera_intelligence?.repetition_risks),
    },
    wow_hypotheses: list(understanding.wow_hypotheses).slice(0, 4),
    creative_threats: list(understanding.creative_threats).slice(0, 5),
  };
}

function critiqueOutputTokenBudget(plan = {}) {
  const duration = list(plan.scenes).flatMap((scene) => list(scene.shots))
    .reduce((sum, shot) => sum + Number(shot.duration_seconds || 0), 0);
...
function critiquePrompt({ input, plan, understanding, deterministic, round }) {
  const evidence = critiqueEvidencePacket(plan);
  return `
...
Return strict JSON only:
{
  ...
  "scores": {
    ...
    "art_direction": 0,
    "visual_design": 0,
    "graphic_motion_design": 0,
    "material_authority": 0,
    "technology_imagination": 0,
    "cinematic_invention": 0,
    "camera_language": 0,
...
- Art direction is not a palette adjective. Score whether architecture, object design, spatial hierarchy, typography/graphics, materials, light and atmosphere form one authored world with mission-specific logic.
- Visual design must create designed frames, not merely photograph ordinary scenes attractively. Reward composition systems, negative-space logic, scale relationships, foreground/midground/background design and images with clear graphic authorship.
- Graphic/motion design is scored for meaning, hierarchy and integration with the moving image; generic HUD overlays, floating UI and decorative data are weak unless the mission specifically earns them.
- Material authority means surfaces, reflections, translucency, density, texture, wear, machining, fabric, glass, metal, skin, atmosphere or other relevant matter behave specifically enough that the world feels physically authored rather than generically rendered.
- Technology imagination scores whether advanced systems are visualized with an original physical/design logic instead of neon lines, particles, generic holograms or stock sci-fi language.
- Cinematic invention scores whether the film contains images, transformations, optical ideas, spatial experiences, editorial constructions or sound-picture events that could not be summarized as ordinary coverage plus polish.
- Camera language is secondary to image invention. A technically sophisticated push, orbit, crane or aerial move over a generic image is still weak filmmaking.
- Treat explicit negative constraints, identity rules, duration and approved story facts as hard law. Never mark compliance with a hard constraint as a weakness or required repair.
- A required repair must identify a real unsatisfied, legal, actionable change inside the approved creative scope. Do not restate a risk as required when the structured plan already explicitly controls that risk; retain such residual execution risk as a non-blocking weakness instead.
- Behind-the-scenes production crew are not on-screen population. Do not request visible actors, silhouettes, hands or people when the approved plan forbids visible human actors.
...
function normalizeCritique(value = {}, round = 0, plan = {}) {
  const source = object(value);
  ...
  const rawRequiredRepairs = list(source.required_repairs);
  const advisoryRepairs = [];
  const requiredRepairs = [];
  rawRequiredRepairs.forEach((repair) => {
    const disposition = critiqueRepairDisposition(plan, repair);
    if (disposition) {
      advisoryRepairs.push({
        ...repair,
        release_disposition: "ADVISORY",
        release_disposition_reason: disposition,
      });
    } else {
      requiredRepairs.push(repair);
    }
  });
  ...
  return {
    ...
    advisory_repairs: advisoryRepairs,
    raw_required_repair_count: rawRequiredRepairs.length,
    blocking_required_repair_count: requiredRepairs.length,
    advisory_repair_count: advisoryRepairs.length,
...
async function critiquePlan({ input, plan, understanding, deterministic, round }) {
  const maxOutputTokens = critiqueOutputTokenBudget(plan);
  const execution = await executeReasoning({
    input,
    operation: CRITIQUE_OPERATION,
    prompt: critiquePrompt({ input, plan, understanding, deterministic, round }),
    maxOutputTokens,
    ...(maxOutputTokens <= 1800 ? { executionLane: "fast" } : {}),
    semanticIdentity: {
      operation: CRITIQUE_OPERATION,
      round,
      evidence: critiqueEvidencePacket(plan),
      understanding: critiqueUnderstandingPacket(understanding),
      deterministic_failures: deterministic.failures,
    },
  });
  return {
    critique: normalizeCritique(execution.output, round, plan),
    result: execution.result,
  };
}
...
REPAIR RULES
- Resolve every deterministic finding and every blocking semantic repair unless they conflict with truth, rights, identity, duration, explicit negative constraints or the approved story.
- Never add visible actors, silhouettes, hands or people when the approved plan forbids visible human actors. Behind-the-scenes production staff do not justify on-screen population.
...
- Do not repeat a repair already explicitly satisfied by structured camera, pacing, lighting, brand, identity or continuity controls.
...
- If the world feels empty and visible people are permitted, derive the correct human roles and behaviours from the supplied understanding; do not add decorative anonymous bodies with no story function.
...
async function runDirectorLoop(input = {}, directed = {}) {
  ...
  let deterministicBootstrapUsed = false;
  let initialDesignExecuted = false;
  const minimumWowHypotheses = minimumUnderstandingWowHypotheses(input, directed);

  try {
    const bootstrap = bootstrapCinematicImpactFromPlan({ plan: directed.plan });
    const bootstrapShotCount = list(directed.plan?.scenes)
      .flatMap((scene) => list(scene.shots)).length;
    understanding = validateUnderstanding(
      bootstrap.understanding,
      minimumWowHypotheses,
      Math.max(1, Math.min(3, bootstrapShotCount || 1)),
    );
    const candidatePlan = {
      ...bootstrap.plan,
      cinematic_understanding: understanding,
      cinematic_impact_contract: {
        ...object(bootstrap.plan?.cinematic_impact_contract),
        understanding_hash: understanding.understanding_hash,
      },
    };
    const deterministic = evaluateImpactPlan(candidatePlan);
    if (deterministic.passed) {
      plan = candidatePlan;
      deterministicBootstrapUsed = true;
    } else {
      understanding = null;
    }
  } catch {
  ...
  if (!deterministicBootstrapUsed) {
    const understandingRun = await createUnderstanding(input, directed);
    understanding = understandingRun.understanding;
    const initial = await createInitialDesign(input, directed, understanding);
    plan = initial.plan;
    executionResults = [understandingRun.result, initial.result];
    initialDesignExecuted = true;
  }
  ...
  const reviewSummary = {
    ...
    understanding_hash: understanding.understanding_hash,
    initial_design_executed: initialDesignExecuted,
    deterministic_bootstrap_used: deterministicBootstrapUsed,
    ...
    final_blocking_required_repair_count:
      finalCritique.blocking_required_repair_count || 0,
    final_advisory_repair_count: finalCritique.advisory_repair_count || 0,
    ...
    plan: {
      ...
      validation_summary: {
        ...
        cinematic_impact_semantic_review: {
          ...
          blocking_required_repair_count:
            finalCritique.blocking_required_repair_count || 0,
          advisory_repair_count: finalCritique.advisory_repair_count || 0,
