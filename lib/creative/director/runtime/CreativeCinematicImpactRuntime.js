...
function evaluateImpactPlan(plan = {}) {
  ...
  const distinctPlatformMinimum = shots.length >= 16 ? 4 : shots.length >= 8 ? 3 : shots.length >= 4 ? 2 : 1;
  ...
  if (shots.length >= 8 && maximumRepeatedPlatform > Math.ceil(shots.length * 0.55)) {
    failures.push(
  ...
  if (shots.length >= 6 && pushZoomCount > Math.ceil(shots.length * 0.35)) {
