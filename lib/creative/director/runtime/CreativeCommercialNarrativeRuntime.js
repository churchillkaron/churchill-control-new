...
function validCausalLink(value, sentinel) {
  const current = text(value);
  if (current.toUpperCase() === sentinel) return true;
...
function validateNarrative(plan = {}, value = {}) {
  ...
  sceneArcs.forEach((arc, sceneIndex) => {
    ...
    if (!validCausalLink(arc.causal_link_from_previous, "OPENING")) {
      failures.push(`COMMERCIAL_NARRATIVE_CAUSAL_FROM_REQUIRED:${sceneId}`);
    }
    if (!validCausalLink(arc.causal_link_to_next, "CLOSURE")) {
      failures.push(`COMMERCIAL_NARRATIVE_CAUSAL_TO_REQUIRED:${sceneId}`);
    }
