...
function validateNarrative(plan = {}, value = {}) {
  ...
  const firstRole = text(sceneArcs[0]?.primary_role).toUpperCase();
  if (!["HOOK", "QUESTION", "SETUP", "REVEAL"].includes(firstRole)) {
    failures.push("COMMERCIAL_NARRATIVE_OPENING_ROLE_WEAK");
  }
  const lastRole = text(sceneArcs.at(-1)?.primary_role).toUpperCase();
  if (!["PAYOFF", "RESULT", "CTA", "EMOTION", "TRANSFORMATION"].includes(lastRole)) {
