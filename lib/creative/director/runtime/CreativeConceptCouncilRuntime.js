...
async function recoverSettledConceptDirector(context = {}, director = {}) {
  const operation = `CREATIVE_CONCEPT_DIRECTOR_${director.id.toUpperCase()}_V1`;
  ...
  const output = normalizedReasoningOutput(usage.output_text || metadata.provider_result || metadata.result || usage);
...
function mergeRevisedPlan(originalPlan, revision, council) {
  ...
  return normalizeDecisionGateCompatibility(normalizeSelectedProductionPolicy(mergedPlan), selected);
}
