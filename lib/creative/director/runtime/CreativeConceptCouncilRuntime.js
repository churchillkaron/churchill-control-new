...
async function recoverSettledConceptDirector(context = {}, director = {}, evidence = {}) {
  const inputHash = hash({
    stage: "INDEPENDENT_DIRECTOR",
...
function positiveMissionContract(input = {}) {
  ...
  return {
    ...
    autonomous_story_required:
      projectMetadata.autonomous_story_required === true ||
      missionMetadata.autonomous_story_required === true ||
      briefMissionMetadata.autonomous_story_required === true,
    master_story_required:
    ...
    full_master_film_target_minutes:
      projectMetadata.full_master_film_target_minutes ||
      missionMetadata.full_master_film_target_minutes ||
      briefMissionMetadata.full_master_film_target_minutes || null,
    generate_only_chapter_1:
      projectMetadata.generate_only_chapter_1 === true ||
      missionMetadata.generate_only_chapter_1 === true ||
      briefMissionMetadata.generate_only_chapter_1 === true,
  };
}

function withSemanticMissionContract(plan = {}, input = {}) {
  return {
    ...object(plan),
    semantic_mission_contract: positiveMissionContract(input),
  };
}
...
async function generateIndependentConcepts(context, evidence) {
  const calls = DIRECTOR_MANDATES.map(async (director) => {
    ...
    const recovered = await recoverSettledConceptDirector(context, director, evidence);
...
async function runIndependentCritic(context, evidence, concepts, critic) {
  ...
  const criticEvidence = compactCouncilEvidence(evidence);
  const criticConcepts = concepts.map(conceptCriticSnapshot);
  const inputHash = hash({
    stage: "INDEPENDENT_CRITIC",
    critic_id: critic.id,
    evidence: criticEvidence,
    concepts: criticConcepts,
  });
  const recovered = await recoverSettledCouncilOperation(context, operation, inputHash);
  const result = recovered || await reason({
    ...context,
    operation,
    prompt: criticPrompt(critic, criticEvidence, criticConcepts),
...
async function runIndependentCritics(context, evidence, concepts) {
  const settled = await Promise.all(
    CRITIC_MANDATES.map((critic) => runIndependentCritic(context, evidence, concepts, critic)),
  );
  return {
...
RULES
- Select only a concept with all_critics_passed=true and weighted_score >= ${policy.minimum_weighted_score}. All critic scores must meet the world-class critic floors: ${JSON.stringify(policy.critic_minimums)}.
...
async function selectConcept(context, evidence, concepts, reports, cards, policy = WORLD_CLASS_CONCEPT_POLICY) {
  ...
  const { output, result } = settledSelection || await reason({
    ...
    prompt: selectorPrompt(selectorEvidence, selectorConcepts, reports, cards, policy),
  ...
  const currentPolicySelectionValid = Boolean(
    selectedConcept &&
    selectedCard?.all_critics_passed &&
    selectedCard.weighted_score >= policy.minimum_weighted_score
  );
  // Historical paid selections may be replayed only as intermediate evidence. They
  // cannot authorize production: the repaired plan must still clear the current
  // world-class gate after Council repair and Tribunal evidence reconciliation.
  if (!selectedConcept || !selectedCard || (!settledSelection && !currentPolicySelectionValid)) {
    throw new Error("CREATIVE_EXECUTIVE_CONCEPT_SELECTION_INVALID");
  }
  const expectedSelectedConceptHash = hash(selectedConcept);
  if (text(output.selected_concept_hash) !== expectedSelectedConceptHash) {
    throw new Error(`CREATIVE_EXECUTIVE_CONCEPT_HASH_MISMATCH:${selectedId || "missing"}`);
  }
  return {
    selection: {
      ...output,
...
function planRevisionPrompt(plan, evidence, council) {
  ...
  const structureLock = list(plan.scenes).map((scene) => ({
    ...
    duration_seconds: finite(
      scene?.duration_seconds ?? scene?.output_spec?.duration_seconds,
    ),
  }));
...
- Apply every mandatory Council repair and make beginning, middle, payoff, emotional arc, message, brand role and exact duration intent explicit.${masterStoryRequirement}
...
async function resumeApprovedCouncilPlan(input = {}) {
  ...
  const postRepairCheckpoint =
    approvedMaster.resume_checkpoint_contract === "CREATIVE_POST_REPAIR_MASTER_CHECKPOINT_V1";
  const lineageRecoveryCheckpoint =
    approvedMaster.resume_checkpoint_contract === "CREATIVE_STORY_LINEAGE_RECOVERY_V1";
  const planForValidation = (postRepairCheckpoint || lineageRecoveryCheckpoint)
    ? withSemanticMissionContract(approvedPlan, input)
    : withSemanticMissionContract(
        selectedConceptDominance(approvedPlan, approvedCouncil),
        input,
      );
  const repairedMaster = postRepairCheckpoint
    ? await CreativeMasterPlanRuntime.validateExistingPlan({
        organization_id: organizationId,
        mission,
        project,
        brief: object(input.brief),
    ...
    : await CreativeMasterPlanRuntime.repairExistingPlan({
        organization_id: organizationId,
        mission,
        project,
        brief: object(input.brief),
...
async function runCouncil(input, directed) {
  ...
  const revisionEvidence = compactCouncilEvidence(evidence);
  const revisionInputHash = hash({
    stage: "SELECTED_CONCEPT_PLAN_REVISION",
    selected_concept_hash: conceptHash,
    council_hash: councilHash,
    evidence: revisionEvidence,
  });
  const revision = await recoverSettledCouncilOperation(context, revisionOperation, revisionInputHash) || await reason({
    ...context,
  ...
  const normalizedPlan = normalizeTemporalMechanicalContract(
    withSemanticMissionContract(mergedPlan, input),
    {
  ...
  const repairedMaster = await CreativeMasterPlanRuntime.repairExistingPlan({
    ...
    assets: list(input.assets),
    plan: normalizedPlan,
    repair_results: await recoverSettledPostRevisionRepairs(context, revisionOperation),
  });
