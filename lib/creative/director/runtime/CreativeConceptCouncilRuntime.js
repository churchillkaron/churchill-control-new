...
- Rewrite creative purpose, action, performance, environment, camera, lighting, design and provider prompt so they unmistakably execute the selected concept.
...
- Provider prompts must be concrete temporal instructions, not adjectives or summaries.
...
function detailedPrompt({ concept, selection, scene, shot, originalPrompt }) {
  ...
  return [
    `APPROVED CONCEPT: ${text(concept.title)} — ${text(concept.central_proposition)}.`,
    `SHOT PURPOSE: ${text(shot.purpose)}.`,
    `SUBJECT AND ACTION: ${text(shot.subject)}; ${text(shot.action)}.`,
    `PERFORMANCE: ${text(shot.performance)}.`,
    `OPENING: ${text(shot.frame_plan?.opening_frame)}.`,
    `PROGRESSION: ${text(shot.frame_plan?.progression)}.`,
    `CLOSING: ${text(shot.frame_plan?.closing_frame)}.`,
    `ENVIRONMENT: ${JSON.stringify(shot.production_design || scene.location || {})}.`,
    `CAMERA: ${JSON.stringify(shot.camera || {})}.`,
    `LIGHTING: ${JSON.stringify(shot.lighting || {})}.`,
    `MUSIC EVIDENCE: section ${text(music.musical_role || music.section_id)}, ${finite(music.audio_start_seconds) ?? "?"}s to ${finite(music.audio_end_seconds) ?? "?"}s, energy ${finite(music.energy_start) ?? "?"} to ${finite(music.energy_end) ?? "?"}, rhythmic density ${finite(music.rhythmic_density) ?? "?"}, required environment change ${text(music.environment_change)}.`,
    identity.profile_id
      ? `IDENTITY: preserve exact profile ${identity.profile_id}, atlas ${text(identity.identity_atlas_hash || identity.identity_atlas_asset_node_id)}, face and body proportions; exclude uploaded reference backgrounds.`
      : null,
    performance.lip_sync_required === true
      ? `PERFORMANCE AUDIO: generate the motion plate for real audio-conditioned lip-sync using source ${text(performance.primary_audio_asset_id)} from ${finite(performance.audio_start_seconds)}s to ${finite(performance.audio_end_seconds)}s; mouth must remain visible.`
      : null,
    `MANDATORY COUNCIL REPAIRS: ${list(selection.mandatory_repairs_before_planning).join(" | ") || "none"}.`,
    text(shot.generation?.provider_prompt),
    originalPrompt ? `PRESERVED TECHNICAL DIRECTION: ${originalPrompt}` : null,
    "NEGATIVE: no concept drift, no generic montage, no repeated motif, no literal lyric illustration, no copied reference background, no identity drift, no lookalike, no filler, no impossible movement, no synthetic text or watermark.",
  ].filter(Boolean).join("\n\n");
}
...
function mergeRevisedPlan(originalPlan, revision, council) {
  ...
  const scenes = originalScenes.map((originalScene, sceneIndex) => {
    ...
    const shots = originalShots.map((originalShot, shotIndex) => {
      ...
      const revisedGeneration = object(revisedShot.generation);
      const originalGeneration = object(originalShot.generation);
      const merged = {
        ...originalShot,
        ...revisedShot,
        id: originalShot.id,
      ...
      merged.generation = {
        ...
        provider_prompt: detailedPrompt({
          concept: selected,
          selection: council.selection,
          scene: { ...originalScene, ...revisedScene },
          shot: merged,
          originalPrompt: text(originalGeneration.provider_prompt),
        }),
      };
      merged.metadata = {
