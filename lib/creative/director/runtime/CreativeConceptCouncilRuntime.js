...
const CRITIC_MANDATES = Object.freeze([
  ...
  },
  {
    ...
    role: "HUMAN_PLACE_PATIENCE_AND_FILM_LANGUAGE_CRITIC",
    weight: 0,
    minimum: 94,
    mandate: "Act as veto authority for lived human truth, causal place truth, cinematic patience, tangible physical evidence, sound-led tension and earned payoff. Reject posed category demonstrations, interchangeable locations, tourism wallpaper, generic anonymous workers, synthetic crowd behaviour, consequence-free montage, explanatory technology B-roll, constant-bed scoring, arbitrary visual anomalies, rapid location hopping and luxury-ad poetry with no causal realization. A concept passes only when people behave as people rather than labels, place materially changes action or meaning, physical details make the idea tangible, sound and silence change with picture events, important actions have time to register, consequences propagate causally, and the final payoff makes the audience understand something they could not understand at the opening without resorting to explanation.",
  },
  {
    id: "story_creativity",
...
async function reason({
  ...
  const result = await ServiceExecutionRuntime.execute({
    ...
    metadata: {
      ...
      council_input_hash: inputHash || null,
    },
  });
...
async function recoverSettledCouncilOperation(context = {}, operation = "", expectedInputHash = null) {
  ...
  if (expectedInputHash && text(metadata.council_input_hash) !== text(expectedInputHash)) return null;
...
async function recoverSettledPostRevisionRepairs(context = {}, revisionOperation = "") {
  const operations = list(context.directionApprovalOperations);
  const revisionIndex = operations
    .map((entry, index) => [entry, index])
    .filter(([entry]) => text(entry?.operation).toUpperCase() === text(revisionOperation).toUpperCase())
    .map(([, index]) => index)
    .at(-1);
  if (!Number.isInteger(revisionIndex)) return [];
  const entries = operations.slice(revisionIndex + 1).filter((entry) =>
    text(entry?.operation).toUpperCase() === "MASTER_PLAN_CONTRACT_REPAIR_V1" &&
    text(entry?.usage_id) && entry?.completed_at
  );
  const latestEntries = entries.slice(-2);
  const usages = await Promise.all(latestEntries.map((entry) => UsageRuntime.get(entry.usage_id)));
  return usages.filter((usage) => usage && text(usage.status).toUpperCase() === "SUCCESS").map((usage) => {
    const metadata = object(usage.metadata);
    const providerResult = object(metadata.provider_result || metadata.result);
...
function compactResearchForCouncil(research = null) {
  const source = blindResearchEvidence(research);
  ...
  return {
    ...
    market: source.market || {},
    commercial_intelligence: source.commercial_intelligence || {},
    strategic_synthesis: source.strategic_synthesis || {},
    claims: list(source.claims).slice(0, 12),
    sources: list(source.sources).slice(0, 12).map((entry) => ({
      id: entry?.id || null,
...
function compactCouncilEvidence(evidence = {}) {
  const source = object(evidence);
  const project = object(source.project);
  return {
    mission_contract: object(source.mission_contract),
    project: {
      id: project.id || null,
    ...
    research: compactResearchForCouncil(source.research),
    measured_audio: source.measured_audio || null,
    music_world: source.music_world || null,
    identity_profiles: list(source.identity_profiles),
    product_profiles: list(source.product_profiles),
    brand_mark_profiles: list(source.brand_mark_profiles),
    location_profiles: list(source.location_profiles),
    asset_manifest: list(source.asset_manifest),
    deliverables: list(source.deliverables),
    production_constraints: object(source.production_constraints),
  };
}

function conceptCriticSnapshot(concept = {}) {
  const source = object(concept);
  return {
    concept_hash: hash(source),
    id: text(source.id),
    director_role: source.director_role || null,
    title: source.title || null,
    central_proposition: source.central_proposition || null,
    original_world: source.original_world || null,
    governing_world_rule: source.governing_world_rule || null,
    dramatic_question: source.dramatic_question || null,
    causal_story: source.causal_story || null,
    beginning: source.beginning || null,
    escalation: source.escalation || null,
    turn: source.turn || null,
    resolution: source.resolution || null,
    environment_progression: source.environment_progression || null,
    performance_integration: source.performance_integration || null,
    music_fit: source.music_fit || null,
    brand_fit: source.brand_fit || null,
    audience_feeling: source.audience_feeling || null,
    signature_images: list(source.signature_images).slice(0, 6),
    scene_arc: list(source.scene_arc).slice(0, 8),
    motif_system: list(source.motif_system).slice(0, 6),
    camera_language: source.camera_language || null,
    lighting_language: source.lighting_language || null,
    editing_language: source.editing_language || null,
    production_approach: source.production_approach || null,
    strategic_basis: object(source.strategic_basis),
    known_risks: list(source.known_risks).slice(0, 8),
    anti_cliche_rules: list(source.anti_cliche_rules).slice(0, 8),
  };
}
...
Return strict JSON only:
{
  "concept": {
    ...
    "world_class_execution_design": {
      "human_consequence": "",
      "physical_evidence": [{"detail":"", "material_behavior":"", "story_consequence":""}],
      "sound_tension": {"sonic_motif":"", "silence_strategy":"", "picture_locked_punctuation":"", "escalation":""},
      "causal_connection": {"opening_question":"", "propagation_rule":"", "proof_chain":[""], "connection_reveal":""},
      "pacing": {"breathing_space":"", "minimum_hero_hold_seconds":4, "location_change_rule":"", "anti_montage_rule":""},
      "payoff_realization": ""
    },
    "campaign_extensions": [""],
    "known_risks": [""],
...
- Treat EVIDENCE.research.strategic_synthesis as the strategic authority when present. Use its factual statements and source references, but resolve any conflict in favor of EVIDENCE.mission_contract.
- Treat EVIDENCE.research.creative_grounding.benchmark_lab as pre-direction craft intelligence when present. Use its transferable craft principles to raise narrative, cinematography, editing, sound, humanity, place and production ambition. Do not imitate or reconstruct any benchmark film, shot, script, music, branded composition, performer, product staging or distinctive expression. The benchmark changes craft decisions, not the identity of the work.
- Before returning the concept, test it against the benchmark craft DNA: it must create genuine scale contrast, human consequence, place causality, iconic-frame potential, editorial variation, sonic evolution and an earned non-generic reveal. If the concept is merely a corporate explainer made cinematic, reject it internally and invent a stronger concept.
- world_class_execution_design is mandatory. Ground the concept in at least one tangible physical detail whose material behaviour changes story meaning; define a sound-and-silence tension system tied to picture events; define a causal proof chain that makes separated events feel connected rather than coincidental; reserve deliberate breathing space instead of rapid location hopping; and design a payoff_realization that changes the audience's understanding rather than merely showing a logo or poetic object.
- Do not solve technology stories with generic dashboards, glowing grids, green indicators, synchronized transport, holograms or explanatory UI unless evidence makes that physical consequence uniquely necessary. Prefer observable real-world cause and effect.
...
async function generateIndependentConcepts(context, evidence) {
  ...
  for (let conceptIndex = 0; conceptIndex < concepts.length; conceptIndex += 1) {
    ...
    if (flagshipCollision) throw new Error(`INDEPENDENT_CONCEPT_${flagshipCollision.type}:${concept.id}`);

    if (longForm) {
      const hero = object(concept.hero_agency);
      for (const field of ["hero_identity", "desire_or_purpose", "consequential_choice", "constraint", "cost_or_risk"]) {
      ...
      if (requiresMystery) {
        const mystery = object(concept.opening_mystery);
        for (const field of ["unanswered_question", "first_anomaly", "withheld_truth", "first_reveal_boundary", "sound_and_silence_logic"]) {
          if (text(mystery[field]).length < 20) throw new Error(`INDEPENDENT_CONCEPT_OPENING_MYSTERY_REQUIRED:${concept.id}:${field}`);
        }
        if (text(mystery.reveal_phase).toUpperCase() !== "ACT_2_OR_LATER") throw new Error(`INDEPENDENT_CONCEPT_REVEAL_TOO_EARLY:${concept.id}`);
      ...
      if (requiresGlobal && !storyOnly) {
        ...
        if (worlds.length < 4 || places.size < 4 || industries.size < 4) throw new Error(`INDEPENDENT_CONCEPT_GLOBAL_BUSINESS_WORLDS_REQUIRED:${concept.id}:4`);
    ...
    if (list(evidence.capability_evidence).length && !storyOnly) {
      ...
      if (usedDomains.size < minimumDomains) throw new Error(`INDEPENDENT_CONCEPT_CAPABILITY_DOMAIN_BREADTH_REQUIRED:${concept.id}:${minimumDomains}`);
      const capabilityArc = list(concept.capability_arc);
      if (capabilityArc.length < minimumCapabilities) throw new Error(`INDEPENDENT_CONCEPT_CAPABILITY_ARC_REQUIRED:${concept.id}:${minimumCapabilities}`);
      ...
      if (arcCapabilityIds.size < minimumCapabilities) throw new Error(`INDEPENDENT_CONCEPT_CAPABILITY_ARC_BREADTH_REQUIRED:${concept.id}:${minimumCapabilities}`);
    ...
    const physical = list(execution.physical_evidence);
    const sound = object(execution.sound_tension);
    const causal = object(execution.causal_connection);
    const pacing = object(execution.pacing);
    const executionRequired = [
      execution.human_consequence,
      physical[0]?.detail,
      physical[0]?.material_behavior,
      physical[0]?.story_consequence,
      sound.sonic_motif,
      sound.silence_strategy,
      sound.picture_locked_punctuation,
      causal.opening_question,
      causal.propagation_rule,
      causal.connection_reveal,
      pacing.breathing_space,
      pacing.location_change_rule,
      pacing.anti_montage_rule,
      execution.payoff_realization,
    ];
    if (executionRequired.some((value) => text(value).length < 18) || list(causal.proof_chain).length < 2) {
      throw new Error(`INDEPENDENT_CONCEPT_WORLD_CLASS_EXECUTION_REQUIRED:${concept.id}`);
    }
    const minimumHeroHold = finite(pacing.minimum_hero_hold_seconds);
    if (minimumHeroHold === null || minimumHeroHold < 4) {
      throw new Error(`INDEPENDENT_CONCEPT_PATIENCE_HOLD_REQUIRED:${concept.id}`);
    }
  ...
  for (let distinctnessAttempt = 0; distinctnessAttempt <= 2; distinctnessAttempt += 1) {
    ...
    if (distinctnessAttempt === 2) {
      if (conflict.reason === "GOVERNING_DEVICE_COLLISION") {
        throw new Error(`INDEPENDENT_CONCEPTS_GOVERNING_DEVICE_COLLISION:${concepts[conflict.right].id}`);
      }
...
Return strict JSON only:
{
  ...
  "evaluations": [{
    ...
    "concept_hash": "copy the exact concept_hash supplied for this concept",
    "score": 0,
    "passed": false,
    "strengths": [""],
    "failures": [""],
...
- For the human_place_patience critic, explicitly test whether human behavior is lived rather than posed, whether place causally matters rather than serving as interchangeable scenery, whether tangible material/physical detail makes the story believable, whether sound and silence are authored around picture events, whether separated consequences form a legible causal proof chain, whether important action/consequence beats have enough temporal patience to register, and whether the payoff changes audience understanding. Treat generic workers, tourism imagery, explanatory tech B-roll, constant-bed scoring, arbitrary anomalies, rapid location hopping, consequence-free montage, restless cutting and decorative luxury poetry as failures.
...
function normalizeIndependentCriticReport({ critic, output = {}, concepts = [] } = {}) {
  ...
  for (const concept of concepts) {
    const conceptId = text(concept.id);
    const evaluation = evaluations.find((item) => item.concept_id === conceptId);
    if (!evaluation) {
      throw new Error(`CREATIVE_CONCEPT_CRITIC_CONCEPT_MISSING:${critic.id}:${conceptId}`);
    }
    const expectedConceptHash = hash(concept);
    if (text(evaluation.concept_hash) !== expectedConceptHash) {
      throw new Error(`CREATIVE_CONCEPT_CRITIC_CONCEPT_HASH_MISMATCH:${critic.id}:${conceptId}`);
    }
  }
  return {
    critic_id: critic.id,
...
function applyIndependentCriticReport({ council = {}, concepts = [], report = {} } = {}) {
  ...
  return Object.freeze({
    council: next,
    selected_concept_passed: selectedCard.all_critics_passed === true && selectedCard.weighted_score >= WORLD_CLASS_CONCEPT_POLICY.minimum_weighted_score,
    selected_scorecard: selectedCard,
    added_critic_id: criticId,
  });
}

function scorecard(concepts, reports, policy = WORLD_CLASS_CONCEPT_POLICY) {
  return concepts.map((concept) => {
    ...
    for (const report of reports) {
      ...
      const criticMinimum = Number(policy.critic_minimums?.[report.critic_id]);
      if (
        !evaluation.passed ||
        (Number.isFinite(criticMinimum) && Number(evaluation.score) < criticMinimum)
      ) passed = false;
    ...
    if (weightedScore < policy.minimum_weighted_score) passed = false;
    return {
...
function selectorPrompt(evidence, concepts, reports, cards, policy = WORLD_CLASS_CONCEPT_POLICY) {
  return `
...
{
  "selected_concept_id": "",
  "selected_concept_hash": "copy the exact concept_hash supplied for the selected concept",
  "selection_reason": "",
  "decisive_strengths": [""],
...
- Film-language veto is equally binding: do not select a concept that lacks tangible physical evidence, picture-locked sound/silence logic, patient breathing room, a causal proof chain, or an earned realization at payoff. A visually beautiful technology montage is not sufficient.
...
async function selectConcept(context, evidence, concepts, reports, cards, policy = WORLD_CLASS_CONCEPT_POLICY) {
  const qualifying = cards.filter((card) => card.all_critics_passed && card.weighted_score >= policy.minimum_weighted_score);
  const selectionOperation = "CREATIVE_EXECUTIVE_CONCEPT_SELECTION_V1";
  const selectorEvidence = compactCouncilEvidence(evidence);
  const selectorConcepts = concepts.map(conceptCriticSnapshot);
  const inputHash = hash({
    stage: "EXECUTIVE_SELECTION",
    evidence: selectorEvidence,
    concepts: selectorConcepts,
    reports,
    cards,
  });
  const settledSelection = await recoverSettledCouncilOperation(context, selectionOperation, inputHash);
  if (!settledSelection) {
    const qualifying = cards.filter((card) => card.all_critics_passed && card.weighted_score >= policy.minimum_weighted_score);
  ...
  const selectedUnderRepair = !(selectedCard.all_critics_passed && selectedCard.weighted_score >= policy.minimum_weighted_score);
...
function planRevisionPrompt(plan, evidence, council) {
  ...
  const structureLock = list(plan.scenes).map((scene) => ({
    id: text(scene?.id),
    duration_seconds: finite(scene?.duration_seconds ?? scene?.output_spec?.duration_seconds),
  }));
  const hasDirectedShots = list(plan.scenes).some((scene) => list(scene.shots).length > 0);
  const missionContract = positiveMissionContract({
    mission: evidence?.mission,
    project: evidence?.project,
    brief: evidence?.brief,
  });
  const masterStoryRequirement = missionContract.master_story_required === true
    ? `\n- The mission requires the COMPLETE master story before production. story_architecture.master_story must contain substantive beginning, middle, payoff and an ordered chapters array for the full ${missionContract.full_master_film_target_minutes || "requested"}-minute work. ${missionContract.generate_only_chapter_1 === true ? "The locked scenes below remain only the current generation chapter/excerpt; do not expand them to the full film." : ""}`
    : "";
  if (!hasDirectedShots) {
    return `
...
- Preserve every original scene id, scene count, duration and chronological order exactly. Return each locked scene id exactly once.
- Fit the winning concept into the locked scene slots. If its concept arc contains more beats than there are slots, combine beats creatively inside existing scenes; never add, remove, split, merge or reorder scenes.
...
- If the winning concept says real, practical, physical, or no CGI while the approved plan uses synthetic generation, treat those words as visual-truth and physical-plausibility targets, not as a requirement for live-action capture. Never replace the approved AI video generation path with a physical shoot unless the existing plan already requires one.
...
- Carry the selected concept's world_class_execution_design into the master plan and make each scene serve its human consequence, physical evidence, sonic tension, causal proof chain, pacing discipline and earned payoff. Do not weaken these into mood adjectives.
...
EXACT STRUCTURE LOCK — RETURN THIS COUNT AND THESE IDS, ONCE EACH, IN THIS ORDER
${JSON.stringify(structureLock)}
`;
  }
  return `
...
NON-NEGOTIABLE RULES
- Preserve every original scene id, shot id, scene count, shot count, duration and chronological order exactly. Return each locked scene id exactly once.
- Fit the winning concept into the locked scene and shot slots; combine extra concept beats inside those slots instead of adding structure.
...
- If the winning concept says real, practical, physical, or no CGI while the approved plan uses synthetic generation, treat those words as visual-truth and physical-plausibility targets, never as permission to replace the approved generation path with a physical shoot.
...
- Preserve world_class_execution_design and translate it into shot-level physical evidence, picture-locked sound/silence events, causal transitions, deliberate hold time and an earned payoff; do not replace it with generic cinematic styling.
- Every shot must cause a new story, performance, environment or musical state. No filler and no repeated visual beat.
- Apply every mandatory repair from the council.${masterStoryRequirement}
...
function preserveAuthorizedStoryLineage(basePlan = {}, candidatePlan = {}) {
  const base = object(basePlan);
  const candidate = object(candidatePlan);
  const baseConcept = object(base.concept);
  const candidateConcept = object(candidate.concept);

  return {
    ...candidate,
    concept: {
      ...structuredClone(baseConcept),
    ...
    creative_system: structuredClone(base.creative_system),
    signature_images: structuredClone(base.signature_images),
    concept_candidates: structuredClone(base.concept_candidates),
    concept_council: structuredClone(base.concept_council),
    selected_concept_id: base.selected_concept_id,
    story_lineage_lock: {
      contract: "CREATIVE_STORY_LINEAGE_LOCK_V1",
      selected_concept_id: text(base.selected_concept_id || baseConcept.id),
      concept_title: text(baseConcept.title),
      story_hash: hash({
        concept: baseConcept,
        story: base.story,
        creative_system: base.creative_system,
        signature_images: base.signature_images,
        selected_concept_id: base.selected_concept_id,
      }),
...
async function runCouncil(input, directed) {
  ...
  const runRevision = () => reason({
    ...
    prompt: planRevisionPrompt(directed.plan, revisionEvidence, council),
    maxOutputTokens: 16000,
    inputHash: revisionInputHash,
  });
  if (!revision) {
...
export const CreativeConceptCouncilRuntime = {
  ...
  rejectedLineageCollision(concept = {}, { project = null, rejectedCreativeLineage = null } = {}) {
    return rejectedLineageCollision(concept, {
      rejectedCreativeLineage:
        rejectedCreativeLineage ||
        list(project?.metadata?.rejected_direction_history),
    });
  },
};
