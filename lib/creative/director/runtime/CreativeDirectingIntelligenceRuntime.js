...
export const CreativeDirectingIntelligenceRuntime = Object.freeze({
  ...
  verifyShotDecision(value = {}) {
    ...
    const agencyCraft = object(decision.agency_craft);
    if (agencyCraft.contract !== "AVANTIQO_REFERENCE_CALIBRATED_AGENCY_CRAFT_V1") {
      throw new Error("DIRECTING_INTELLIGENCE_AGENCY_CRAFT_DISPATCH_REQUIRED");
    }
    for (const field of [
      "story_delta",
      "sensory_delta",
      ...
      if (!text(agencyCraft[field])) {
        throw new Error(`DIRECTING_INTELLIGENCE_AGENCY_CRAFT_DISPATCH_FIELD_REQUIRED:${field}`);
      }
    }
    if (agencyCraft.beautiful_but_disconnected_shot_forbidden !== true ||
        agencyCraft.generated_feel_forbidden !== true) {
      throw new Error("DIRECTING_INTELLIGENCE_AGENCY_CRAFT_DISPATCH_POLICY_INVALID");
    }
