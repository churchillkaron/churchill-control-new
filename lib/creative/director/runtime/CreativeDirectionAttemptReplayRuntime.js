...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.direction-attempt-replay.v2",
);
...
function serviceResultFromUsage(usage = {}, sourceApprovalId = "") {
  ...
  return {
    ...
    settlement: "CHARGED",
    attempt_replay: true,
    attempt_replay_source_approval_id: sourceApprovalId || null,
...
async function loadState(input = {}) {
  ...
  const sourceApprovalId = text(
    process.env.CREATIVE_DIRECTION_REPLAY_SOURCE_APPROVAL_ID,
  );
  const activeApprovalId = text(
    process.env.CREATIVE_DIRECTION_REPLAY_ACTIVE_APPROVAL_ID,
  );
  ...
  if (sourceApprovalId === activeApprovalId) {
    throw new Error(
      "CREATIVE_DIRECTION_ATTEMPT_REPLAY_APPROVAL_ISOLATION_REQUIRED",
    );
  }
  if (
    startSequence === null ||
  ...
  if (
    sourceApproval.contract !== "CREATIVE_DIRECTION_BUDGET_APPROVAL_V2" ||
    text(sourceApproval.id) !== sourceApprovalId ||
    text(sourceApproval.command_identity) !== expectedCommandIdentity ||
    text(sourceApproval.status).toUpperCase() !== "COMPLETED" ||
    Number(sourceApproval.call_count) !== Number(sourceApproval.maximum_calls)
  ) {
    throw new Error(
      "CREATIVE_DIRECTION_ATTEMPT_REPLAY_SOURCE_APPROVAL_INVALID",
    );
  }
  ...
  const activeApprovedAt = validDate(activeApproval.approved_at);
  const activeExpiresAt = validDate(activeApproval.expires_at);
  const now = Date.now();
  if (
    activeApproval.contract !== "CREATIVE_DIRECTION_BUDGET_APPROVAL_V2" ||
    text(activeApproval.id) !== activeApprovalId ||
    text(activeApproval.command_identity) !== expectedCommandIdentity ||
    activeApproval.approved !== true ||
    !["APPROVED", "IN_PROGRESS"].includes(
      text(activeApproval.status).toUpperCase(),
    ) ||
    activeApprovedAt === null ||
    activeExpiresAt === null ||
    activeApprovedAt > now ||
    activeExpiresAt <= now ||
    Number(activeApproval.maximum_calls) !== 3 ||
    Number(activeApproval.call_count || 0) !== 0 ||
    list(activeApproval.operations).length !== 0
  ) {
    throw new Error(
      "CREATIVE_DIRECTION_ATTEMPT_REPLAY_ACTIVE_APPROVAL_INVALID",
    );
  }

  const operations = list(sourceApproval.operations)
    .map((entry) => ({
      ...object(entry),
  ...
  for (const entry of operations) {
    ...
    if (
      text(usage.metadata?.direction_approval_id) !== sourceApprovalId
    ) {
      throw new Error(
        ...
        if (!expected) {
          ...
          return executeWithoutReplay({
            ...
            metadata: {
              ...
              creative_direction_attempt_replay_active_approval_id:
                state.activeApprovalId,
              creative_direction_attempt_replay_source_approval_id:
                state.sourceApprovalId,
            },
          });
        }
