...
async function updateApproval(project, approval, patch) {
  ...
  const currentApproval = object(
    current?.metadata?.paid_direction_approval || approval,
  );
...
function estimatedDirectionUsage(input = {}) {
  ...
  // Conservative prompt estimate for reservation only. Settlement still uses provider-reported actual usage.
  const inputTokens = Math.max(1, Math.ceil(prompt.length / 2.5));
...
async function currentPricing(approval, usage = { quantity: 1 }) {
  ...
  if (
    ...
    Number(pricing.customer_price) > Number(approval.maximum_per_call_customer_price)
  ) {
  ...
  async function executeGovernedDirection(input = {}) {
    ...
    const governedInput = {
      ...
      metadata: {
        ...
        direction_budget_maximum_customer_price: approval.maximum_customer_price,
        direction_budget_remaining_before_call: state.remaining,
        direction_budget_token_cap_applied: bounded.capped,
        direction_budget_estimated_input_tokens: estimatedUsage.input_tokens,
        direction_budget_max_output_tokens: estimatedUsage.output_tokens,
        direction_approval_currency: approval.currency,
        direction_approval_capability: approval.capability,
        direction_approval_model: approval.model,
        direction_execution_serialized: true,
        ...(benchmark
          ? {
              execution_scope: BENCHMARK_SCOPE,
              benchmark_only: true,
