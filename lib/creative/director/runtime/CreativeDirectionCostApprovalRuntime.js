...
function approvalTimeWindowValid({ approval = {}, status = "", approvedAt, expiresAt, now }) {
  ...
  // expires_at is a start-by deadline, not a finish-by deadline. Once an approved
  // governed pipeline has actually started, authority remains bounded by the
  // existing call ceiling, THB ceiling, operation allowlist, provider/model and
  // media/publication flags. An arbitrary wall-clock timeout must not strand a
  // long-running Studio mission that still has unused approved calls and budget.
  return status === "IN_PROGRESS" && Number(approval.call_count || 0) > 0;
}
...
async function recoverSettledNextDirectionUsage({ project, approval, operation }) {
...
export async function renewCreativeDirectionApprovalWindow({
  ...
  if (Number(approval.call_count || 0) >= Number(approval.maximum_calls || 0)) {
