...
} from "@/lib/platform/service-runtime/providers/avantiqo-intelligence/AvantiqoIntelligenceHierarchicalLocalRuntime.js";
...
export function installCreativeDirectionCostApprovalGate() {
  ...
  async function executeGovernedDirection(input = {}) {
    ...
    const internalOwnedMusicVideo = Boolean(
      project.metadata?.music_video === true &&
      (
        project.metadata?.full_song === true ||
        text(project.metadata?.duration_mode).toUpperCase() === "FULL_SOURCE_AUDIO" ||
        text(project.metadata?.temporal_contract?.mode).toUpperCase() === "FULL_SOURCE_AUDIO"
      )
    );
    if (internalOwnedMusicVideo) {
      const requestRevision = text(input.metadata?.creative_direction_request_hash).slice(0, 16) || "unversioned";
      const localInput = {
        capability: text(input.service_id) || "ai.reasoning.execute",
        execution_lane: "deep",
        local_compute_required: true,
        infrastructure_policy: "local_only",
        ...
        context: {
          ...
          usage_id: `creative-temporal:${projectId}:${operation}:direction-gate:${requestRevision}`,
        },
        prompt: input.input?.prompt || "",
        messages: input.input?.messages || [],
        system_prompt: input.input?.system_prompt || input.input?.systemPrompt || "",
        max_output_tokens: input.input?.max_output_tokens || input.input?.maxOutputTokens || 4096,
        response_format: input.input?.response_format || input.input?.responseFormat || { type: "json_object" },
        metadata: {
          ...(input.metadata || {}),
          internal_creative_local_authority: true,
          external_provider_authorized: false,
          customer_price_ceiling: 0,
          supplier_billing_required: false,
          provider_supplier_account_verification_required: false,
        },
      };
      return shouldUseHierarchicalLocalIntelligence(localInput)
        ? executeHierarchicalLocalIntelligence(localInput)
        : executeIntelligenceLocalQueueAndWait(localInput, {
            timeout_ms: 600000,
            poll_ms: 500,
          });
    }
