...
function stableProjectScope(project = {}, input = {}, policy = {}) {
  ...
  return {
    ...
    creative_direction_lineage_id: metadata.creative_direction_lineage_id || null,
  };
}
...
function requestIdentity(input = {}, project = {}, policy = {}) {
  ...
  const semanticHash = text(input.metadata?.creative_direction_semantic_hash);
  const baseRequestHash = semanticHash || hash({
    service_id: text(input.service_id),
    operation,
    input: input.input || {},
  });
  const lineageId = text(project.metadata?.creative_direction_lineage_id);
  // Crash/network retries inside one creative lineage must reuse settled paid work,
  // but an explicit human rejection starts a new lineage inside the same mission
  // and approval. Salt both request and scope identity only when a lineage exists,
  // preserving exact-resume compatibility for every pre-lineage project.
  const requestHash = lineageId
    ? hash({ request_hash: baseRequestHash, creative_direction_lineage_id: lineageId })
    : baseRequestHash;
  return {
    operation,
    slot,
...
function serviceResultFromUsage(usage = {}, operation = "") {
  const providerResult = normalizeRecoveredProviderResult(
    object(usage.metadata?.result || usage.metadata?.provider_result),
    operation,
  );
  return {
    success: true,
...
function recoverableDirectionPayload(usage = {}) {
  const payload = object(usage.metadata?.result || usage.metadata?.provider_result);
  if (!Object.keys(payload).length) return false;
  const candidate = payload?.output?.text ?? payload?.text ?? payload?.output?.content ?? payload?.content ?? null;
  if (candidate && typeof candidate === "object" && !Array.isArray(candidate)) return true;
  if (typeof candidate !== "string") return false;
  const trimmed = candidate.trim().replace(/^```(?:json)?\s*/i, "").replace(/```$/, "").trim();
  const slices = [trimmed];
  const balanced = firstBalancedJsonObject(trimmed);
  if (balanced) slices.push(balanced);
  return slices.some((value) => {
    try {
...
function usageValidForScope({
  ...
  if (!recoverableDirectionPayload(usage)) return false;
  return true;
}
...
async function recoverExactResult(input = {}, project, policy, identity) {
  ...
  const explicitCandidate = list(policy.recovery_entries).find((entry) =>
    text(entry.operation).toUpperCase() === operation &&
    text(entry.slot || "default") === identity.slot,
  );
  const explicit = explicitCandidate &&
    text(explicitCandidate.request_hash) === text(identity.request_hash)
    ? explicitCandidate
    : null;
  if (explicitCandidate && !explicit) {
    console.log(`CREATIVE_DIRECTION_EXACT_RESUME_STALE_ENTRY=${operation}:${identity.slot}`);
  }
  ...
  if (explicit) {
    ...
    const identityValid = text(usage.id) === text(explicit.usage_id) &&
      text(usage.status).toUpperCase() === "SUCCESS" &&
      text(usage.category).toUpperCase() === "CREATIVE_DIRECTION" &&
      text(usage.organization_id) === text(input.organization_id) &&
      text(usage.metadata?.creative_project_id) === text(project.id) &&
      text(usage.metadata?.creative_mission_id) === text(project.creative_mission_id || input.metadata?.creative_mission_id) &&
      text(usage.metadata?.operation).toUpperCase() === operation &&
      text(usage.metadata?.direction_approval_id) === text(explicit.source_approval_id);
    if (!identityValid) {
      throw new Error(
        `CREATIVE_DIRECTION_EXACT_RESUME_RECOVERY_ENTRY_INVALID:${operation}:${identity.slot}`,
      );
    }
    if (!recoverableDirectionPayload(usage)) {
      console.log(`CREATIVE_DIRECTION_EXACT_RESUME_UNRECOVERABLE=${operation}:${identity.slot}:${usage.id}`);
      return null;
    }
    if (
    ...
    async function executeWithExactResume(input = {}) {
      ...
      return executeWithoutExactResume({
        ...
        metadata: {
          ...
          creative_direction_resume_request_hash: identity.request_hash,
