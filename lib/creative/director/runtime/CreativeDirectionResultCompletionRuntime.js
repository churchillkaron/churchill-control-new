...
function resultRecoveryDisabled(input = {}) {
  return Boolean(
    text(
      process.env
    ...
    input.metadata
      ...
      === true ||
    input.metadata
      ?.disable_direction_result_recovery
      === true
  );
}
...
function requestIdentity(input = {}, lineageId = "") {
  ...
  const request = {
    ...
    creative_direction_lineage_id: text(lineageId) || null,
  };
  return crypto
    .createHash("sha256")
...
function completeShotDirection(payload, context) {
  ...
  return {
    ...source,
    shots: list(list(completed.scenes)[0]?.shots).length
      ? list(completed.scenes)[0].shots
      : list(source.shots),
    direction_completion: {
      ...object(completed.completion),
...
async function recoverExistingDirectionUsage(input = {}, requestHash) {
  ...
  const candidates = rows
    .filter((row) => Object.keys(object(row.metadata?.provider_result || row.metadata?.result)).length > 0)
    .sort((left, right) =>
      Date.parse(right.updated_at || right.created_at || 0) -
      Date.parse(left.updated_at || left.created_at || 0),
    );

  // A Studio challenger selection is durable creative authority. Its settled operation
  // lineage is stronger recovery evidence than a reconstructed request hash because
  // harmless envelope changes (research metadata, wrapper versions) can change the hash
  // without changing the selected Studio work. Replay only the exact usage ids sealed in
  // the checkpoint, and only when organization/project/mission/operation all still match.
  const project = await CreativeProjectRuntime.get(projectId);
  const challenger = object(project?.metadata?.creative_challenger_selection_checkpoint);
  const settledUsageId = text(object(challenger.settled_operation_usage_ids)[operation]);
  if (
    challenger.contract === "CREATIVE_CHALLENGER_SELECTION_CHECKPOINT_V1" &&
    challenger.organization_id === organizationId &&
    challenger.creative_project_id === projectId &&
    text(challenger.creative_mission_id) === text(input.metadata?.creative_mission_id) &&
    settledUsageId
  ) {
    const settled = candidates.find((row) => text(row.id) === settledUsageId);
    if (!settled) {
      throw new Error(`CREATIVE_CHALLENGER_SETTLED_USAGE_INVALID:${operation}:${settledUsageId}`);
    }
    return {
      result: serviceResultFromUsage(settled),
      usage_id: settled.id,
      request_hash: requestHash,
      challenger_lineage_replay: true,
    };
  }
...
export function installCreativeDirectionResultCompletion() {
  ...
  ServiceExecutionRuntime.execute = async function executeWithCompletion(input = {}) {
    ...
    const lineageId = text(
      input.metadata?.creative_direction_lineage_id ||
      project?.metadata?.creative_direction_lineage_id,
    );
    const requestHash = requestIdentity(input, lineageId);
    const recoveryDisabled =
      resultRecoveryDisabled(input);

    const recovered = recoveryDisabled
      ? null
      : await recoverExistingDirectionUsage(
          input,
          requestHash,
        );
    ...
    const governedInput = {
      ...
      metadata: {
        ...
        creative_direction_request_hash:
          requestHash,
        creative_direction_lineage_id:
          lineageId || null,
        creative_direction_result_recovery_disabled:
          recoveryDisabled,
      },
