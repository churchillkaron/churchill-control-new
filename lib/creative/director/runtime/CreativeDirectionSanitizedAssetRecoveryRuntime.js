...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.direction-sanitized-asset-recovery.v1",
);
...
async function recover(input = {}) {
  ...
  let candidates = rows
    ...
    .filter((row) =>
      text(row.metadata?.direction_approval_id) ===
      text(compatibility.original.id),
    )
    .filter((row) => Object.keys(object(row.metadata?.result)).length > 0)
    .sort((left, right) =>
      timestamp(left.created_at || left.updated_at) -
      timestamp(right.created_at || right.updated_at),
    );
  ...
  return serviceResultFromUsage(selected, {
    contract: "CREATIVE_DIRECTION_SANITIZED_ASSET_RECOVERY_V1",
    mode: "ORIGINAL_APPROVAL_USAGE_SEQUENCE",
