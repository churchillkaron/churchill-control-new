...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.direction-sanitized-asset-recovery.v4",
);

const NON_RECOVERABLE_OPERATIONS = new Set([
...
function operationUsageEvidence(approval = {}, operation = "") {
  const entries = list(approval.operations).filter((entry) =>
    text(entry?.operation).toUpperCase() === operation,
  );
  return {
    valid_usage_ids: entries
      .map((entry) => uuid(entry?.usage_id))
      .filter(Boolean),
    invalid_usage_references: entries
      .map((entry) => text(entry?.usage_id))
      .filter((value) => value && !uuid(value)),
  };
}

function validRecoveredUsage({
  selected,
  organizationId,
  projectId,
  operation,
  ...
  return Boolean(
    ...
    text(selected.metadata?.direction_approval_id) === approvalId &&
    Object.keys(object(selected.metadata?.result)).length
  );
}

async function scopedCandidates({
  organizationId,
  projectId,
  operation,
  approvalId,
...
async function recover(input = {}) {
  ...
  if (NON_RECOVERABLE_OPERATIONS.has(operation)) return null;
  ...
  const evidence = operationUsageEvidence(
  ...
  let selected = null;
  let recoveryMode = "ORIGINAL_APPROVAL_USAGE_ID_SEQUENCE";

  const usageId = repeatable
    ? evidence.valid_usage_ids[cursor] || null
    : evidence.valid_usage_ids[evidence.valid_usage_ids.length - 1] || null;

  if (usageId) {
    const candidate = await UsageRuntime.get(usageId);
    if (validRecoveredUsage({
      selected: candidate,
      organizationId,
      projectId,
      operation,
      approvalId: text(compatibility.original.id),
    })) {
      selected = candidate;
    }
  }
  ...
  if (!selected) {
    recoveryMode = "SCOPED_PROJECT_OPERATION_APPROVAL_LOOKUP";
    const candidates = await scopedCandidates({
      organizationId,
      projectId,
      operation,
      approvalId: text(compatibility.original.id),
  ...
  if (!validRecoveredUsage({
    ...
    throw new Error(
      `CREATIVE_DIRECTION_SANITIZED_RECOVERY_USAGE_INVALID:` +
      `${text(selected?.id) || "MISSING"}`,
    );
  }
  ...
  return serviceResultFromUsage(selected, {
    contract: "CREATIVE_DIRECTION_SANITIZED_ASSET_RECOVERY_V4",
    mode: recoveryMode,
    ...
    invalid_approval_usage_references_ignored:
      evidence.invalid_usage_references,
    selected_concept_plan_revision_recovery_prohibited: true,
    reason:
      "GENERIC_NON_FILE_CONTAINER_REMOVED_WITHOUT_CHANGING_CONCRETE_CREATIVE_EVIDENCE",
    organization_wide_usage_scan_used: false,
    non_uuid_usage_lookup_blocked: true,
