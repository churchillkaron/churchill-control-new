...
async function runTemporalDirector({
  ...
  for (const entry of entries) {
    ...
    let correctionExecution =
      null;

    const initialSectionOnly =
      temporalSectionFailuresOnly(
        candidate.failures,
      );

    if (
      candidate.failures.length &&
      !motivationFailuresOnly(
        candidate.failures,
      ) &&
      !initialSectionOnly
    ) {
    ...
    let sectionExecutions = [];

    if (candidate.failures.length) {
      const completion =
        await completeMissingTemporalSections({
          definition,
          job,
          input,
          plan,
      ...
      sectionCalls +=
        sectionExecutions.length;
    }

    let motivationExecution =
      null;
      ...
      throw jobError(
        "CREATIVE_TEMPORAL_DIRECTOR_SHOT_CONTRACT_REJECTED",
        {
          ...
          correction_attempted:
            correctionExecution !==
            null,
          focused_section_calls:
            sectionExecutions.length,
          motivation_attempted:
            motivationExecution !==
            null,
          response_shape:
