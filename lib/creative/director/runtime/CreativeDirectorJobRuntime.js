...
function inspectTemporalCandidate({
  ...
  const canonicalTemporalContract =
    object(inspection.contract);

  const canonicalMasterStill =
    object(
      canonicalTemporalContract.master_still,
    );

  const canonicalCandidateShot = {
    ...candidateShot,
    master_still_contract:
      canonicalMasterStill,
    temporal_contract:
      canonicalTemporalContract,
  };
...
const TEMPORAL_MOTIVATION_DEPARTMENTS = [
  "camera",
  "performance",
  "objects_products",
  "lighting",
...
function temporalMotivationAddress({
  department,
  trackNumber,
  atMs,
}) {
...
function temporalMotivationRequirements(
  contract = {},
) {
  const requirements = {};
  ...
  for (
    const department
    of TEMPORAL_MOTIVATION_DEPARTMENTS
  ) {
    const section =
      object(contract[department]);

    for (
      const [trackIndex, trackValue]
      of list(section.tracks).entries()
    ) {
      const track =
        object(trackValue);
      ...
      for (
        const [keyframeIndex, keyframeValue]
        of list(track.keyframes).entries()
      ) {
        const keyframe =
        ...
        const atMs =
          Number(keyframe.at_ms);

        const address =
          temporalMotivationAddress({
            department,
            trackNumber,
            atMs,
          });
        ...
        requirements[address] = {
          ...
          property:
            track.property || null,
          initial_state:
            track.initial_state ?? null,
          final_state:
            track.final_state ?? null,
          keyframe_state:
            keyframe.state ?? null,
          interpolation:
            keyframe.interpolation || null,
          physical_rules:
            list(track.physical_rules),
          immutable_rules:
            list(track.immutable_rules),
          acceptance_criteria:
            list(
              track.acceptance_criteria,
            ),
        };
      }
    }
...
function temporalMotivationOutputShape(
  requirements = {},
) {
  return {
    result: {
      motivation_by_address:
        Object.fromEntries(
...
function temporalMotivationResult(
  execution,
  requirements,
  entry,
) {
  if (
    ...
    throw jobError(
      "CREATIVE_TEMPORAL_MOTIVATION_REASONING_FAILED",
      {
        scene_number:
          entry.sceneNumberValue,
        shot_number:
  ...
  const source =
    candidates.find((candidate) =>
      isPlainObject(
        candidate.motivation_by_address,
      ),
    ) ||
    candidates.find((candidate) =>
  ...
  const unexpected =
    Object.keys(motivations).filter(
      (address) =>
        !Object.prototype
          .hasOwnProperty.call(
            requirements,
  ...
  if (
    ...
    throw jobError(
      "CREATIVE_TEMPORAL_MOTIVATION_RESULT_INVALID",
      {
        scene_number:
          entry.sceneNumberValue,
        shot_number:
...
async function reasonTemporalMotivations({
  definition,
  job,
  input,
  plan,
  ...
  const execution = await reason({
    task: [
      `Complete only the missing keyframe motivations for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`,
      "Return exactly one non-empty motivation for every supplied motivation address.",
      "Each motivation must explain why that exact state is required at that exact time.",
      "Ground every motivation in the shot purpose, narrative beat, entering state, leaving handoff, physical plausibility, audience perception, product or identity truth, and measurable acceptance criteria where relevant.",
      "A start-state motivation must explain why the shot must begin in that state.",
      "An end-state motivation must explain why the shot must arrive at or preserve that state for the next shot.",
      "Do not use generic phrases such as cinematic impact, visual interest, professional look, dynamic feel, or enhance engagement.",
      "Do not alter timing, state, interpolation, owner, subject, property, camera, lighting, performance, objects, sound, editorial, continuity, references, duration, or structure.",
      "Do not omit an address and do not return additional addresses.",
    ].join(" "),
    input: {
      organization_id:
        job.organization_id,
      ...
      canonical_master_still:
        candidate.masterStill,
      canonical_temporal_contract:
        candidate.temporalContract,
      missing_motivation_addresses:
        requirements,
    },
    constraints: {
      exactly_one_shot: true,
      exact_address_set_required: true,
      all_addresses_required: true,
      non_empty_motivations_required: true,
      causal_reasoning_required: true,
      preserve_all_states: true,
      preserve_all_timing: true,
      preserve_all_references: true,
      preserve_all_continuity: true,
      no_generic_filler: true,
      no_contract_rewrite: true,
    },
    outputShape:
      temporalMotivationOutputShape(
        requirements,
      ),
    temperature: 0.3,
    maxOutputTokens: Math.max(
    ...
    metadata: {
      ...
      temporal_shot_number:
        entry.shotNumberValue,
      temporal_motivation_completion:
        true,
      temporal_motivation_count:
        Object.keys(
          requirements,
        ).length,
    },
...
function applyTemporalMotivations(
  contract = {},
  motivations = {},
) {
  const output =
  ...
  for (
    ...
    const section =
      object(output[department]);

    output[department] = {
      ...section,
      tracks:
        list(section.tracks).map(
...
function motivationFailuresOnly(
  failures = [],
) {
  return (
    failures.length > 0 &&
    failures.every(
      (failure) =>
        String(failure).includes(
          "keyframe motivation missing",
        ),
    )
  );
}

function temporalCheckpointKey(value = {}) {
  return temporalPatchKey(
    Number(value.scene_number),
    Number(value.shot_number),
  );
}

function replaceTemporalRecord(
  records = [],
  record = {},
) {
  const key =
    temporalCheckpointKey(record);

  return [
    ...list(records).filter(
      (candidate) =>
        temporalCheckpointKey(
          candidate,
        ) !== key,
    ),
    clone(record),
...
async function updateTemporalActivity({
  job,
  entry,
  phase,
  completedShots,
  ...
  const fresh =
    await getJobRow(job.id);

  const pipeline =
    object(fresh.pipeline_result);
...
async function persistTemporalCheckpoint({
  job,
  patch,
  report,
  executionSummary,
...
async function completeTemporalCheckpointState({
  job,
  totalShots,
}) {
  const fresh =
...
function temporalMissingDepartments(
  failures = [],
) {
  return TEMPORAL_MOTIVATION_DEPARTMENTS
    .filter((department) =>
      failures.some((failure) =>
        String(failure).includes(
          `${department} temporal direction missing`,
        ),
      ),
    );
}

function temporalGovernanceFailures(
  failures = [],
) {
  const patterns = [
...
function temporalSectionFailuresOnly(
  failures = [],
) {
...
function temporalDepartmentCompletionShape({
  department,
  durationMs,
}) {
  return {
...
function temporalDepartmentResult({
  execution,
  department,
  entry,
}) {
  if (
    ...
    throw jobError(
      "CREATIVE_TEMPORAL_DEPARTMENT_REASONING_FAILED",
      {
        department,
        scene_number:
          entry.sceneNumberValue,
  ...
  if (!isPlainObject(contract)) {
    throw jobError(
      "CREATIVE_TEMPORAL_DEPARTMENT_CONTRACT_REQUIRED",
      {
        department,
        scene_number:
          entry.sceneNumberValue,
...
async function reasonTemporalDepartmentCompletion({
  definition,
  job,
  input,
  plan,
  ...
  return reason({
    task: [
      `Create only the ${department} temporal department contract for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`,
      "The existing master still, duration, story, identities, references, other departments and continuity are locked.",
      "Return a complete department_contract with specific tracks or events.",
      "Every track requires owner, subject, property, initial state, final state, chronological keyframes, motivations, physical rules and measurable acceptance criteria.",
      "The first keyframe must be at 0ms and the final keyframe must equal the exact shot duration.",
      "Stable properties require explicit locked state across the full duration.",
      "Do not return another department and do not rewrite the complete temporal contract.",
      "Do not use generic cinematic filler.",
    ].join(" "),
    input: {
      organization_id:
        job.organization_id,
      ...
      locked_master_still:
        candidate.masterStill,
      existing_temporal_contract:
        candidate.temporalContract,
      existing_department_contract:
        candidate.temporalContract?.[
          department
        ] ||
        {},
    ...
    constraints: {
      ...
      preserve_other_departments: true,
    ...
    outputShape:
      temporalDepartmentCompletionShape({
        department,
        durationMs,
      }),
    temperature: 0.3,
    ...
    metadata: {
      ...
      temporal_shot_number:
        entry.shotNumberValue,
      temporal_department_completion:
        department,
    },
  });
}

function temporalGovernanceCompletionShape() {
  return {
    result: {
      continuity: {
        entering_state: {},
...
function temporalGovernanceResult({
  execution,
  entry,
}) {
  if (
    ...
    throw jobError(
      "CREATIVE_TEMPORAL_GOVERNANCE_REASONING_FAILED",
      {
        scene_number:
          entry.sceneNumberValue,
        shot_number:
  ...
  return {
    continuity:
      object(contract.continuity),
    immutable_locks:
      list(
        contract.immutable_locks,
      ),
    directed_evolution:
      list(
        contract.directed_evolution,
      ),
    quality_requirements:
      object(
...
async function reasonTemporalGovernanceCompletion({
  definition,
  job,
  input,
  plan,
  ...
  return reason({
    task: [
      `Complete only temporal continuity and global governance for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`,
      "Return entering state, leaving state, continuity locks, handoff requirements, global immutable locks, directed evolution and measurable temporal quality requirements.",
      "Ground the entering state in the prior shot and the leaving state in the required next-shot handoff.",
      "Preserve all existing department tracks, timing, states, master-still details, identities and canonical references.",
      "Do not rewrite department tracks or the complete production bible.",
    ...
    input: {
      ...
      locked_master_still:
        candidate.masterStill,
      locked_department_contracts:
        candidate.temporalContract,
      missing_governance_failures:
        failures,
    },
    constraints: {
      exactly_one_shot: true,
      ...
      causal_handoff_required: true,
      measurable_quality_required: true,
      no_generic_filler: true,
    },
    outputShape:
      temporalGovernanceCompletionShape(),
    temperature: 0.3,
    ...
    metadata: {
      ...
      temporal_shot_number:
        entry.shotNumberValue,
      temporal_governance_completion:
        true,
    },
  });
}

async function completeMissingTemporalSections({
  definition,
  job,
  input,
  plan,
  ...
  const missingDepartments =
    temporalMissingDepartments(
      candidate.failures,
    );

  for (
    const department
    of missingDepartments
  ) {
    await updateTemporalActivity({
      job,
      entry,
      phase:
    ...
    const execution =
      await reasonTemporalDepartmentCompletion({
        definition,
        job,
        input,
        plan,
  ...
  if (governanceFailures.length) {
    await updateTemporalActivity({
      ...
      entry,
      phase:
        "CONTINUITY_AND_GOVERNANCE",
      completedShots,
      totalShots,
    });

    const execution =
      await reasonTemporalGovernanceCompletion({
        definition,
        job,
        input,
        plan,
...
function temporalMasterStillOutputShape({
  sceneNumberValue,
  shotNumberValue,
  scene,
  shot,
...
async function reasonTemporalMasterStill({
  definition,
  job,
  input,
  assets,
  ...
  return reason({
    task: [
      correction
        ? `Correct only the master-still contract for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`
        : `Create only the exact master-still contract for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`,
      "This is frame 0 at time 0ms and the physical origin of every permitted motion in the shot.",
      "Define exact camera, subject, object, location, lighting, environment, focus and exposure state.",
      "Define safe motion space, immutable locks, permitted motion, prohibited changes and measurable approval requirements.",
      "Preserve the shot purpose, opening frame, identities, products, venue geometry and assigned canonical references.",
      "Use exactly the canonical reference IDs already assigned to this shot.",
      "Do not create temporal tracks, edit another shot or use generic cinematic filler.",
      correction
        ? "Address every supplied master-still failure while preserving valid existing fields."
        : "Return a complete and production-executable master-still contract.",
    ].join(" "),
    input: {
      organization_id:
        job.organization_id,
      ...
      existing_master_still:
        existingMasterStill,
      correction_failures:
        failures,
    },
    constraints: {
      exactly_one_master_still:
        true,
      frame_number:
        0,
      frame_time_ms:
        0,
      exact_reference_set_required:
        true,
      preserve_identity:
        true,
      preserve_product_truth:
        true,
      preserve_location_geometry:
        true,
      measurable_approval_required:
        true,
      no_temporal_tracks:
        true,
      no_generic_filler:
    ...
    outputShape:
      temporalMasterStillOutputShape({
        sceneNumberValue:
          entry.sceneNumberValue,
        shotNumberValue:
          entry.shotNumberValue,
    ...
    metadata: {
      ...
      temporal_stage:
        correction
          ? "MASTER_STILL_CORRECTION"
          : "MASTER_STILL",
    },
  });
}

function temporalMasterStillResult({
  execution,
  entry,
}) {
  if (
    ...
    throw jobError(
      "CREATIVE_TEMPORAL_MASTER_STILL_REASONING_FAILED",
      {
        scene_number:
          entry.sceneNumberValue,
        shot_number:
  ...
  if (!isPlainObject(contract)) {
    throw jobError(
      "CREATIVE_TEMPORAL_MASTER_STILL_REQUIRED",
      {
        scene_number:
          entry.sceneNumberValue,
        shot_number:
  ...
  const expected =
    new Set(
      list(
        entry.shot.reference_asset_ids,
      ).map(String),
    );
...
function temporalMasterStillFailures(
  failures = [],
) {
  return failures.filter(
    (failure) =>
...
function temporalDepartmentBlockingFailures(
  failures = [],
  department,
) {
  return failures.filter(
...
function temporalExecutionSummary({
  execution,
  phase,
  department = null,
}) {
...
function temporalCandidateFromContracts({
  entry,
  fps,
  masterStill = {},
  temporalContract = {},
  ...
  return inspectTemporalCandidate({
    entry,
    execution: {
      provider:
        execution?.provider ||
...
async function persistTemporalPartial({
  job,
  entry,
  masterStill,
  temporalContract,
  ...
  const record = {
    ...
    master_still_contract:
      clone(masterStill),
    temporal_contract:
      clone(temporalContract),
    completed_departments:
      [...completedDepartments],
    governance_completed:
      governanceCompleted === true,
    motivation_completed:
      motivationCompleted === true,
    executions:
      list(executions).slice(-30),
    updated_at:
      now(),
  };
  ...
  return updateJob(job.id, {
    ...
    pipeline_result: {
      ...
      temporal_direction: {
        ...
        active_phase:
          phase,
        partial_departments_completed:
          [...completedDepartments],
        partial_governance_completed:
          governanceCompleted === true,
        partial_motivation_completed:
          motivationCompleted === true,
        completed_shots:
          completedShots,
        total_shots:
...
async function clearTemporalPartial({
  job,
  entry,
}) {
  const fresh =
...
async function reasonGranularTemporalDepartment({
  definition,
  job,
  input,
  plan,
  ...
  return reason({
    task: [
      correction
        ? `Correct only the ${department} temporal department for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`
        : `Create only the ${department} temporal department for scene ${entry.sceneNumberValue} shot ${entry.shotNumberValue}.`,
      "The master still, duration, story purpose, identities, canonical references and all other completed departments are locked.",
      "Return a complete department_contract with specific tracks or events.",
      "Every track requires an owner, subject, property, initial state, final state, chronological keyframes, interpolation, causal motivations, physical rules and measurable acceptance criteria.",
      "The first keyframe must be at 0ms and the final keyframe must equal the exact shot duration.",
      "Stable properties require an explicit locked state for the full duration rather than omission.",
      "Do not return another department or rewrite the complete temporal contract.",
      "Do not use generic cinematic filler.",
      correction
        ? "Address every supplied department failure while preserving valid existing decisions."
        : "Make every visible or audible change explainable and testable.",
    ].join(" "),
    input: {
      organization_id:
        job.organization_id,
    ...
    constraints: {
      ...
      exact_duration_required:
        true,
      preserve_master_still:
        true,
      preserve_other_departments:
        true,
      preserve_references:
        true,
      physically_plausible:
        true,
      measurable_acceptance_required:
        true,
      causal_motivation_required:
        true,
      no_generic_filler:
        true,
    },
    ...
    metadata: {
      ...
      temporal_stage:
        correction
          ? `DEPARTMENT_${department}_CORRECTION`
          : `DEPARTMENT_${department}`,
    },
  });
}
...
async function runTemporalDirector({
  ...
  const checkpointState =
    temporalCheckpointState(job);

  const checkpointByKey =
    new Map(
      list(
        checkpointState.patches,
      ).map((patch) => [
        temporalCheckpointKey(
          patch,
        ),
  ...
  const checkpointExecutions =
    new Map(
      list(
        checkpointState.executions,
      ).map((execution) => [
        temporalCheckpointKey(
          execution,
        ),
  ...
  let partialHits = 0;
  let masterStillCalls = 0;
  let departmentCalls = 0;
  let departmentCorrectionCalls = 0;
  let governanceCalls = 0;
  let motivationCalls = 0;
  ...
  for (const entry of entries) {
    ...
    if (checkpoint) {
      const checkpointCandidate =
        temporalCandidateFromContracts({
          entry,
          fps,
          masterStill:
            checkpoint.master_still_contract,
          temporalContract:
    ...
    let completedDepartments =
      new Set(
        list(
          partial.completed_departments,
        ).map(String),
      );

    let governanceCompleted =
      partial.governance_completed ===
      true;

    let motivationCompleted =
      partial.motivation_completed ===
      true;
    ...
    let masterFailures =
      temporalMasterStillFailures(
        candidate.failures,
      );

    if (masterFailures.length) {
      await updateTemporalActivity({
        job,
        entry,
        phase:
      ...
      let execution =
        await reasonTemporalMasterStill({
          definition,
          job,
          input,
          assets,
      ...
      if (masterFailures.length) {
        throw jobError(
          "CREATIVE_TEMPORAL_MASTER_STILL_REJECTED",
          {
            scene_number:
              entry.sceneNumberValue,
            shot_number:
    ...
    for (
      ...
      let blockingFailures =
        temporalDepartmentBlockingFailures(
          candidate.failures,
          department,
        );
      ...
      let execution =
        await reasonGranularTemporalDepartment({
          definition,
          job,
          input,
          plan,
      ...
      if (blockingFailures.length) {
        await persistTemporalPartial({
          ...
          totalShots:
            entries.length,
          phase:
            `DEPARTMENT_${department}_REJECTED`,
        });
      ...
      await persistTemporalPartial({
        ...
        totalShots:
          entries.length,
        phase:
          `DEPARTMENT_${department}_COMPLETE`,
      });
    }
    ...
    const motivationRequirements =
      temporalMotivationRequirements(
        candidate.temporalContract,
      );
    ...
    if (
      ...
      const completion =
        await reasonTemporalMotivations({
          definition,
          job,
          input,
          plan,
    ...
    await persistTemporalCheckpoint({
      job,
      patch,
      report,
      executionSummary,
  ...
  await completeTemporalCheckpointState({
    job,
    totalShots:
      entries.length,
  });
...
export const CreativeDirectorJobRuntime = {
  ...
  async replanStructure({
    ...
    if (
      ...
      throw jobError(
        "CREATIVE_DIRECTOR_JOB_NOT_IN_ORGANIZATION",
      );
    }
    ...
    const replanHistory = list(
      pipeline.structural_replans,
    );

    if (replanHistory.length >= 2) {
      throw jobError(
        "CREATIVE_DIRECTOR_STRUCTURAL_REPLAN_LIMIT_REACHED",
        {
          job_id,
          structural_replan_count:
    ...
    const replanned =
      await CreativeShotDirectorRuntime.replanStructure({
        ...
        platform:
          input.platform || "multi-channel",
        budgetMode:
          input.budget_mode || "quality-first",
        productionSpecification:
          input.production_specification,
        currentPlan: object(job.current_plan),
        structuralFailure:
          reason || job.error || {},
      });
    ...
    const steps = await getStepRows(job_id);
    const restartStep = steps.find(
      (step) =>
        step.step_key ===
        "executive_narrative_editorial",
    );

    if (!restartStep) {
      throw jobError(
        "CREATIVE_DIRECTOR_STRUCTURAL_REPLAN_RESTART_STEP_MISSING",
      );
    }

    const resetKeys = STEP_DEFINITIONS
      .filter(
        (definition) =>
          definition.key !== "initial_director",
      )
    ...
    const { error: resetError } =
      await supabaseAdmin
        ...
        .eq("job_id", job_id)
        .in("step_key", resetKeys);

    if (resetError) throw resetError;

    const replanRecord = {
      replan_number:
        replanHistory.length + 1,
      reason: reason || job.error || null,
      production_specification_key:
        input.production_specification
          ?.specification_key || null,
      report: replanned.report,
      provider:
        replanned.execution?.provider || null,
      model:
        replanned.execution?.model || null,
      created_at: now(),
    };
    ...
    await updateJob(job_id, {
      ...
      status: "WAITING",
      current_step_key:
        restartStep.step_key,
      current_step_index:
        restartStep.step_index,
      completed_steps: 1,
      progress_percent:
        Math.round(
          1 / STEP_DEFINITIONS.length * 10000,
        ) / 100,
      storyboard_audit: {},
      error: null,
      lease_token: null,
