...
function normalizedShotPatchEntries(
  ...
  return patchArray(
    ...
    const number = Number(source.shot_number);
    ...
    if (!Number.isInteger(number) || number <= 0) {
      throw jobError(
        "CREATIVE_DIRECTOR_JOB_SHOT_PATCH_NUMBER_INVALID",
        {
          ...context,
          patch_index: index,
          received: source.shot_number ?? null,
        },
      );
    }
...
function normalizedScenePatchEntries(
  ...
  return patchArray(
    ...
    const number = Number(source.scene_number);
    ...
    if (!Number.isInteger(number) || number <= 0) {
      throw jobError(
        "CREATIVE_DIRECTOR_JOB_SCENE_PATCH_NUMBER_INVALID",
        {
          ...context,
          patch_index: index,
          received: source.scene_number ?? null,
        },
      );
    }
