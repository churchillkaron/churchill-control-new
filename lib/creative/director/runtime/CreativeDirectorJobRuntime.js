...
function temporalResultSource(
  ...
  const arrayEntry = list(
    result.shot_temporal_patches,
  )[0];

  const source = object(
    result.shot_temporal_patch ||
    result.temporal_patch ||
    arrayEntry ||
    result,
  );

  const receivedScene =
    source.scene_number === undefined ||
    source.scene_number === null
      ? sceneNumberValue
      : Number(source.scene_number);

  const receivedShot =
    source.shot_number === undefined ||
    source.shot_number === null
      ? shotNumberValue
