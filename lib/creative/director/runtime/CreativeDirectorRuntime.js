...
async function buildRoutedPipeline(input = {}) {
  ...
  const routedInput = {
    ...
    production_room_bootstrap: resolution.production_room_bootstrap || null,
    resumed_from_approved_council: resolution.resumed_from_approved_council === true,
    workflow_kind: resolution.workflow.workflow_kind,
  };

  let pipeline;
  if (resolution.workflow.executor === "TEMPORAL") {
    pipeline = await buildCreativePipeline(routedInput);
  } else if (resolution.workflow.executor === "UNIVERSAL") {
    pipeline = await buildUniversalCreativePipeline(routedInput);
  } else {
    throw new Error(
  ...
  await CreativeWorkflowResolutionRuntime.finalizeResolvedHandoff({
...
export const CreativeDirectorRuntime = {
  async build(input = {}) {
    const requestedWorkflowKind = String(input.workflow_kind || "").trim().toUpperCase();
    const temporalResume = Boolean(
      requestedWorkflowKind === "TEMPORAL" &&
      (input.creative_project_id || input.project_id) &&
      (input.creative_mission_id || input.mission_id)
    );

    // Existing temporal productions must resolve their durable mission/project/brief first.
    // Running the generic Creative Partner preflight with ID-only resume input loses the
    // film context and can incorrectly enter BENCHMARK_REVIEW_PREVIEW before the temporal
    // director ever sees the approved production authority.
    if (temporalResume) {
      return buildRoutedPipeline(input);
    }
