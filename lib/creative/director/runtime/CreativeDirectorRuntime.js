...
import {
  CreativeDirectorCouncilRuntime,
} from "./CreativeDirectorCouncilRuntime";

import {
  convergeCreativeStoryBeats,
} from "@/lib/creative/storyboard/runtime/CreativeStoryBeatConvergence";

import {
  convergeCreativeShotIdentities,
} from "@/lib/creative/storyboard/runtime/CreativeShotIdentityConvergence";

import {
  convergeCreativeStoryboardExecutionContracts,
} from "@/lib/creative/storyboard/runtime/CreativeStoryboardExecutionContractConvergence";
...
} from "@/lib/creative/storyboard/runtime/CreativeIntegerDurationCapacityConvergence";

import {
  convergeCreativePerformanceDirections,
} from "@/lib/creative/storyboard/runtime/CreativePerformanceDirectionConvergence";
...
async function persistSelectedConcept({
  ...
  const payload = {
    ...
    camera_style:
      ...
      creativePlan.metadata?.director_council?.camera_language ||
      "",
    music_style:
    ...
    revision_reason: existing[0]
      ? "Director council regenerated selected concept"
      : "Director council persisted selected concept",
    metadata: {
      ...(existing[0]?.metadata || {}),
      source: "creative_director_council_runtime",
      director_version:
        creativePlan.production_version ||
        "dynamic-director-council-v1",
      ...
      story_beat_convergence:
        creativePlan.metadata?.story_beat_convergence || null,
      director_council:
        creativePlan.metadata?.director_council || null,
    },
  };
...
function convergeForInspection({
  ...
  const durationReady = convergeCreativeIntegerDurations({
    creativePlan: executionReady,
    targetDuration,
  });

  return convergeCreativePerformanceDirections({
    creativePlan: durationReady,
  });
}
...
export const CreativeDirectorRuntime = {
  async build(input = {}) {
    ...
    const seedPlan = await CreativeShotDirectorRuntime.direct({
      organization_id: scope.organization_id,
    ...
    const storyBeatPlan = convergeCreativeStoryBeats({
      creativePlan: seedPlan,
      brief: input.brief || {},
      targetDuration: duration,
    });
    const identityPlan = convergeCreativeShotIdentities(
      storyBeatPlan,
    );

    const council = await CreativeDirectorCouncilRuntime.enrich(
      councilInput({
        input,
        scope,
        objective,
        duration,
        initialPlan: identityPlan,
      }),
    );

    let reviewedPlan = convergeForInspection({
      creativePlan: council.creativePlan,
      targetDuration: duration,
    });
    let inspection = inspectCreativeStoryboardPlan({
      creativePlan: reviewedPlan,
      targetDuration: duration,
      brief: input.brief || {},
      assets: input.assets || [],
    });
    ...
    if (!inspection.report.passed) {
      const repair = await CreativeDirectorCouncilRuntime.repair({
        ...councilInput({
          input,
          scope,
          objective,
          duration,
          initialPlan: reviewedPlan,
        }),
        creativePlan: reviewedPlan,
        audit: inspection.report,
      });
      repairReport = repair.report;
      reviewedPlan = convergeForInspection({
        creativePlan: repair.creativePlan,
        targetDuration: duration,
      });
      inspection = inspectCreativeStoryboardPlan({
        creativePlan: reviewedPlan,
        targetDuration: duration,
        brief: input.brief || {},
        assets: input.assets || [],
    ...
    const storyboardContract = enforceCreativeStoryboardPlan({
      creativePlan: inspection.creativePlan,
      targetDuration: duration,
      brief: input.brief || {},
      assets: input.assets || [],
    });
    const creativePlan = storyboardContract.creativePlan;
    ...
    return {
      ...
      storyboard_contract: {
        ...
        execution_contract_convergence:
          creativePlan.metadata?.storyboard_execution_contract_convergence ||
          null,
        performance_direction_convergence:
          creativePlan.metadata?.performance_direction_convergence ||
          null,
        targeted_repair_attempted: Boolean(repairReport),
      },
      plan_only: true,
