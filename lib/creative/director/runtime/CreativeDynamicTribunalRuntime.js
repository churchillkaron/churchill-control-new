...
function aggregate(reviews = [], floor) {
  ...
  if (weightedScore < floor) passed = false;
...
function repairPayload({ context, plan, tribunal }) {
  return {
    ...
    output: "Return the repaired Creative Master Plan JSON. Preserve every valid field not explicitly changed by a repair.",
    rules: [
...
export const CreativeDynamicTribunalRuntime = Object.freeze({
  ...
  async review({
    ...
    if (!tribunal.verdict.passed) {
      ...
      plan = mergeCreativeRepairedPlan(plan, repair.output);
    ...
    if (!tribunal.verdict.passed) {
      const blocker = tribunal.verdict.blockers?.[0] || null;
      const blockerText = blocker
        ? `:blocker=${blocker.reviewer_id || "unknown"}:${blocker.score ?? "invalid"}/${blocker.required_floor}`
        : "";
      const error = new Error(
        `CREATIVE_DYNAMIC_TRIBUNAL_REJECTED:${tribunal.verdict.weighted_score}:${tribunal.verdict.required_floor}${blockerText}`,
      );
