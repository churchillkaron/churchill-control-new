...
function activeReviewerAuthority(reviewer = {}, plan = {}) {
  ...
  const mandate = discipline === "BRAND_TRUTH" && !exactBrandTruth
    ? "Verify brand continuity against current evidence and exact supplied brand assets. Treat qualitative brand guidance as qualitative. Do not require an exact hex color, font family, spacing percentage or other exact brand specification unless authoritative CREATIVE_BRAND_TRUTH_V1 evidence in the current plan supports that exact value."
    : normalized.mandate;
  return {
    id: normalized.id,
...
function assertReviewAuthority({ reviewer = {}, review = {}, plan = {} } = {}) {
  ...
  const quotedEvidencePattern = /[\"'`](.{24,240}?)[\"'`]/g;
  for (const evidence of list(review.evidence_used).map(text).filter(Boolean)) {
    for (const match of evidence.matchAll(quotedEvidencePattern)) {
      const fragment = text(match[1]);
      if (fragment && /[A-Za-z]/.test(fragment) && !scopedEvidenceText.includes(fragment.toLowerCase()) && !planText.includes(fragment.toLowerCase())) {
  ...
  if (discipline !== "BRAND_TRUTH") {
    const demandsExactBrandTruth = /provide exact (?:brand )?(?:color )?hex|provide exact hex|exact brand color hex|exact font family|exact brand spacing/.test(body);
...
function repairPayload({ context, plan, tribunal, rejected_repairs = [] }) {
  ...
  const blockingTribunal = {
    contract: tribunal?.contract || CONTRACT,
    panel: tribunal?.panel || null,
    verdict: {
      ...object(tribunal?.verdict),
      blockers: list(tribunal?.verdict?.blockers),
      mandatory_repairs: blocking.flatMap((entry) => list(entry.mandatory_repairs)),
      failures: blocking.flatMap((entry) => list(entry.failures)),
    },
    reviews: list(tribunal?.reviews).filter((row) =>
      blockingIds.has(text(row?.review?.reviewer_id || row?.reviewer?.id)),
    ),
  };
...
async function runReviews({
  ...
  const settledResults = await Promise.allSettled(
    reviewers.map(async (reviewer) => {
      ...
      if (settled && (exactScopedEvidenceMatch || verifiedLegacyPass)) {
