...
async function runReviews({
  ...
  const settledByReviewer = new Map(
    list(settled_reviews).map((row) => {
      ...
      return [text(review.reviewer_id), { review, usage: row.usage || null, billing: row.billing || null }];
    }).filter(([id]) => Boolean(id)),
  );
  const results = await Promise.all(
    reviewers.map(async (reviewer) => {
      const settled = settledByReviewer.get(text(reviewer.id));
      if (settled?.review?.passed === true && finite(settled.review.score) >= floor) {
        return { reviewer, review: settled.review, usage: settled.usage, billing: settled.billing, reused: true };
      }
...
async function inspect({
  ...
  return {
    contract: CONTRACT,
    panel,
    reviews: reviews.map(({ reviewer, review, reused }) => ({ reviewer, review, reused: reused === true })),
    verdict,
    usage: {
      panel: panelUsage,
...
export const CreativeDynamicTribunalRuntime = Object.freeze({
  ...
  async review({
    ...
    if (list(settled_reviews).length && text(settled_review_plan_hash) !== canonicalPlanHash) {
