...
async function runReviews({
  ...
  const settledResults = await Promise.allSettled(
    reviewers.map(async (reviewer) => {
      ...
      const legacySchemaOnlyEquivalent = Boolean(
        settled &&
        legacy_settled_review_source_plan &&
        hash(schemaComparablePlan(legacy_settled_review_source_plan)) === hash(schemaComparablePlan(plan))
      );
      if (
        settled?.review &&
        ...
        (
          ...
          legacySchemaOnlyEquivalent ||
          passedReviewSafeToReuseAfterRepair({ reviewer, review: settled.review, plan })
        )
      ) {
      ...
      let reviewRun = await reason({
        ...
        payload,
        expects: "reviewer_id",
        max_output_tokens: targetedRecheck ? 180 : 3000,
      });
