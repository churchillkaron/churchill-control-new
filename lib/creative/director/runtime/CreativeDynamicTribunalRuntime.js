...
function assertReviewAuthority({ reviewer = {}, review = {}, plan = {} } = {}) {
  ...
  for (const evidence of list(review.evidence_used).map(text).filter(Boolean)) {
    for (const match of evidence.matchAll(quotedEvidencePattern)) {
      ...
      if (fragment && /[A-Za-z]/.test(fragment) && !scopedEvidenceText.includes(fragment.toLowerCase())) {
        throw new Error(`CREATIVE_TRIBUNAL_REVIEW_AUTHORITY_VIOLATION:${text(reviewer.id)}:UNSUPPORTED_QUOTED_EVIDENCE`);
      }
    }
  }

  if (discipline === "PHYSICAL_REALISM") {
  ...
  if (discipline === "BRAND_TRUTH" && !hasExactBrandTruthAuthority(plan)) {
    const exactBrandFailure = /#[0-9a-f]{6}|helvetica|exact font|exact hex|primary color violation/.test(body);
    if (exactBrandFailure) {
...
export const CreativeDynamicTribunalRuntime = Object.freeze({
  ...
  replaySettledRepair({ plan = {}, output = {}, assets = [], available_capabilities = [] } = {}) {
    try {
