...
function fatalConceptReplacementRequired(tribunal = {}) {
  return blockingReviewerBrief(tribunal).some((entry) => {
    ...
    return /(?:concept (?:itself|fundamentally|is the)|core visual concept|complete conceptual replacement|unrecoverable without .*replacement|replace the .*concept)/.test(combined);
  });
}
