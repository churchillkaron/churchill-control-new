  ...
  return {
    ...
    contract: {
      ...
      rules: [
        ...
        "Reviewer ids are data, not schema placeholders: derive each id from that reviewer's actual discipline, use lowercase kebab-case, keep ids unique, and never return reviewer-id, stable-reviewer-id, unique-reviewer-id, example-reviewer-id or any wording copied from this contract.",
...
function normalizeReviewerPolicy(reviewer = {}) {
  ...
  return {
    ...reviewer,
    mandate: "Validate only externally checkable factual claims that the plan explicitly presents as real-world truth. Fictional cinematic world-rules, brand metaphors, creative positioning, imagined causal devices, illustrative narrative events, internal UI, and organization-owned creative assets are not external factual claims merely because they appear in the plan. Do not require external proof for them. If the plan contains no explicit externally checkable factual claim, the correct factual verdict is a pass with no factual repair required.",
    ...
    factual_claim_boundary: "EXPLICIT_REAL_WORLD_CLAIMS_ONLY_V2",
  };
}
...
function reviewerDiscipline(reviewer = {}) {
  ...
  if (/rights|safety|licen[cs]|provenance|clearance|protected material|identity-bearing material/.test(mandate)) return "RIGHTS_SAFETY";
  if (/visual[-_ ]?continuity|cinematic visual language|visual language|cinematography|camera language|camera movement|shot continuity|visual consistency/.test(mandate)) return "VISUAL_CONTINUITY";
...
function selectedPlanAssetIds(canonical = {}) {
  ...
  const visit = (value, key = "", depth = 0) => {
    ...
    if (typeof value !== "object") {
      if (typeof value === "string" && /asset(?:_id|_ids)?|source_asset|primary_source/i.test(key)) {
        ids.add(value);
      }
      return;
    }
    for (const [childKey, child] of Object.entries(value)) {
      if (/asset(?:_id|_ids)?|source_asset|primary_source/i.test(childKey)) {
        if (Array.isArray(child)) child.map(text).filter(Boolean).forEach((id) => ids.add(id));
        else if (typeof child === "string" && text(child)) ids.add(text(child));
      }
      visit(child, childKey, depth + 1);
    }
  };
  visit(withoutManifest);
  return ids;
}

function rightsSafetyAssetEvidence(canonical = {}) {
  const selectedIds = selectedPlanAssetIds(canonical);
  const manifest = list(canonical.asset_manifest);
  ...
  switch (reviewerDiscipline(reviewer)) {
    ...
    case "RIGHTS_SAFETY":
      return {
        workflow_kind: canonical.workflow_kind,
        concept: {
          selected_concept_id: canonical.concept?.selected_concept_id || canonical.concept?.id || null,
          brand_rules: canonical.concept?.brand_rules || null,
          refused_devices: canonical.concept?.refused_devices || [],
        },
        story: {
          hook: canonical.story?.hook || null,
          resolution: canonical.story?.resolution || null,
          causal_story: canonical.story?.causal_story || null,
        },
        selected_assets: rightsSafetyAssetEvidence(canonical),
        deliverables: list(canonical.deliverables).map((deliverable) => ({
          id: deliverable.id,
        ...
        scenes: list(canonical.scenes).map((scene) => ({
          ...
          shots: list(scene.shots).map((shot) => ({
            id: shot.id,
            primary_source_asset_id: shot.primary_source_asset_id,
    ...
    case "VISUAL_CONTINUITY":
      return {
        ...
        concept: {
          selected_concept_id: canonical.concept?.selected_concept_id || canonical.concept?.id || null,
          visual_system: canonical.concept?.visual_system || null,
          environment_progression: canonical.concept?.environment_progression || null,
          irreversible_turns: canonical.concept?.irreversible_turns || [],
        },
        story: {
          causal_story: canonical.story?.causal_story || null,
          hook: canonical.story?.hook || null,
...
function activeReviewerAuthority(reviewer = {}, plan = {}) {
  ...
  const mandate = discipline === "BRAND_TRUTH" && !exactBrandTruth
    ...
    : discipline === "PRODUCTION_FEASIBILITY"
      ? "Judge whether the authored temporal plan is executable with registered capabilities and continuity constraints. A five-second single-shot statement applies to the authored beat or shot it describes; do not reinterpret it as requiring the entire 60-second film to occur in one five-second take."
    : discipline === "SOUND_VISUAL_SYNC"
      ? "Judge only the sound and music actually planned in the canonical creative plan. Organization catalog audio, available assets, prior scores, narration files, or generated music are not planned soundtrack usage unless the current plan explicitly references or assigns them. Do not fail a sound plan merely because excluded or unreferenced audio assets exist in project context."
      : discipline === "RIGHTS_SAFETY"
        ? "Judge rights, safety, provenance and clearance only for assets, identities, protected materials or claims actually selected, referenced or assigned by the canonical plan. Organization catalog availability is not planned use. Assets explicitly excluded or absent from selected_assets create no clearance requirement for this film."
      : discipline === "VISUAL_CONTINUITY"
        ? "Judge visual continuity at the temporal unit actually authored by the plan: shot continuity within each shot/beat and motivated continuity across scene transitions. Do not reinterpret 'single uninterrupted shot per beat' as a requirement that the entire 60-second film be one continuous take unless the canonical plan explicitly states that."
        : normalized.mandate;
  return {
    id: normalized.id,
...
function reviewPayload({ reviewer, context, plan, floor }) {
  ...
  if (reviewerDiscipline(reviewer) === "PRODUCTION_FEASIBILITY") {
    return {
      ...
      rules: [
        ...
        "For temporal work, interpret shot-duration constraints at the shot/beat they describe. Do not require the full film duration to fit inside one five-second shot unless the canonical plan explicitly says so.",
      ],
      context: { project: { production_type: context?.project?.production_type || null } },
    };
  }
  if (reviewerDiscipline(reviewer) === "RIGHTS_SAFETY") {
    return {
      task: "Independently judge rights, safety, provenance and clearance for only the material actually selected or referenced by the current canonical film plan.",
      reviewer: activeReviewer,
      reviewer_scoped_evidence: reviewerPlanEvidence(reviewer, plan),
      required_release_floor: floor,
      output: {
        ...
        evidence_used: ["specific selected asset or rights evidence"],
      },
      rules: [
        `passed may be true only when score is at least ${floor}.`,
        "Judge only assets and protected material actually present in selected_assets, scene references, shot references, or deliverable production requirements.",
        "Do not infer use from organization catalog availability, asset manifest membership, prior projects, narration libraries, soundtrack tests or unused brand materials.",
        "An asset explicitly excluded by the canonical plan, or absent from selected_assets and all scene/shot references, creates no clearance or approval obligation for this film.",
        "Do not require a soundtrack asset when the canonical sound design intentionally uses silence, environmental sound, generated score direction, or another authored audio plan without selecting a catalog track.",
        "Rights review may verify the final brand/logo asset actually selected for reveal, but may not pull unrelated brand or audio files into scope.",
        "Judge safety and provenance separately from creative taste; do not invent creative repairs outside this mandate.",
      ],
      context: {
  ...
  if (reviewerDiscipline(reviewer) === "VISUAL_CONTINUITY") {
    return {
      task: "Independently judge whether shot-level visual continuity, camera grammar, motivated transitions and environmental progression preserve the approved film.",
      reviewer: activeReviewer,
      reviewer_scoped_evidence: reviewerPlanEvidence(reviewer, plan),
      required_release_floor: floor,
      output: {
        ...
        fatal_rejection_reason: null,
        weakest_link: "single weakest point",
        evidence_used: ["specific visual evidence"],
      },
      rules: [
        `passed may be true only when score is at least ${floor}.`,
        "Judge continuity within each authored shot/beat and the motivated transition between beats.",
        "A statement that a beat is a single uninterrupted shot does not mean the entire film must be one continuous take.",
        "Do not require the hero to remain visible through every scene unless the canonical scene architecture explicitly requires that continuity.",
        "Do not invent a requirement that geographically distinct approved beats must physically transform into each other inside one frame.",
        "Preserve the approved environment progression and causal visual anchors without forcing impossible geography.",
      ],
      context: {
        mission: { objective: context?.mission?.objective || null },
        project: { title: context?.project?.title || null, production_type: context?.project?.production_type || null },
  ...
  if (reviewerDiscipline(reviewer) === "SOUND_VISUAL_SYNC") {
    return {
      task: "Independently judge whether the current film's planned sound design and music are coherent, causally motivated, synchronized to picture and faithful to the approved concept.",
      reviewer: activeReviewer,
      reviewer_scoped_evidence: reviewerPlanEvidence(reviewer, plan),
      required_release_floor: floor,
      ...
      output: {
        ...
        fatal_rejection_reason: null,
        weakest_link: "single weakest point",
        evidence_used: ["specific planned sound evidence"],
      },
      rules: [
        `passed may be true only when score is at least ${floor}.`,
        "Judge only audio/music explicitly planned in reviewer_scoped_evidence.",
        "The existence of organization catalog audio, prior soundtrack assets, narration, stems or generated music does not mean those assets are used.",
        "Fail an asset-based sound issue only when the canonical plan explicitly assigns, references or selects that asset for this film.",
        "If the plan explicitly excludes pre-composed music and instead specifies silence plus environmental/human-caused sound emergence, judge that authored sound plan on its own merits.",
        "Do not inspect or infer soundtrack usage from broader project asset context.",
      ],
      context: {
        mission: { objective: context?.mission?.objective || null },
        project: { title: context?.project?.title || null, production_type: context?.project?.production_type || null },
...
function assertReviewAuthority({ reviewer = {}, review = {}, plan = {} } = {}) {
  ...
  if (discipline === "RIGHTS_SAFETY") {
    const allegesUnselectedCatalogUse =
      /(narration|soundtrack|music|score|asset)[^.]{0,160}(listed|available|manifest|catalog)[^.]{0,160}(must|requires|failure|repair|clearance|approval)/.test(body);
    const selectedEvidenceMentionsCatalogAsset =
      /(narration|soundtrack|music|score)/.test(scopedEvidenceText) &&
      /(selected_assets|reference_assets|primary_source_asset_id|production_steps)/.test(scopedEvidenceText);
    if (allegesUnselectedCatalogUse && !selectedEvidenceMentionsCatalogAsset) {
      throw new Error(`CREATIVE_TRIBUNAL_REVIEW_AUTHORITY_VIOLATION:${text(reviewer.id)}:UNSELECTED_CATALOG_ASSET_INVENTED`);
    }
  }

  if (discipline === "VISUAL_CONTINUITY") {
    const falselyRequiresWholeFilmSingleTake =
      /(entire|full|whole)[^.]{0,80}(60[- ]second|film)[^.]{0,80}(single|continuous|uninterrupted) (?:take|shot|frame)/.test(body) ||
      /(desert|valley)[^.]{0,120}(becoming|transform|morph)[^.]{0,120}(leaf|water|city|satellite)/.test(body);
    const planExplicitlyRequiresWholeFilmSingleTake =
      /(entire|full|whole)[^.]{0,80}(60[- ]second|film)[^.]{0,80}(single|continuous|uninterrupted) (?:take|shot|frame)/.test(planText);
    if (falselyRequiresWholeFilmSingleTake && !planExplicitlyRequiresWholeFilmSingleTake) {
      throw new Error(`CREATIVE_TRIBUNAL_REVIEW_AUTHORITY_VIOLATION:${text(reviewer.id)}:WHOLE_FILM_SINGLE_TAKE_INVENTED`);
    }
  }
...
function deterministicAntiClicheConceptBlocker(plan = {}, floor = 92, panel = null) {
  ...
  const genericGridIntegration = gridCore && shardCore && integrationCore;
  const glowingThreadCore = /(?:single|one)[^]{0,120}(?:continuous|unbroken|flowing|stream|ribbon)[^]{0,120}(?:line|thread|path|stream|ribbon)[^]{0,120}(?:blue|glow|light)/.test(coreEvidence);
  const abstractDarkCore = /(?:abstract|minimalist)[^]{0,120}(?:dark background|negative space)/.test(coreEvidence);
  const workflowUnityCore = /(?:unified workflow|cohesive path|frictionless|harmony|integration)/.test(coreEvidence);
  const genericGlowingConnectivity = glowingThreadCore && workflowUnityCore;
  const abstractLightAnchorCore = /(?:single|stable|central|sole)[^]{0,100}(?:anchor point|light anchor|beacon|orb|point of light)[^]{0,120}(?:blue|glow|light|radiat)/.test(coreEvidence);
  const anchorUnityCore = /(?:unified workflow|unified system|governance|frictionless|harmony|calm|organized|flow)/.test(coreEvidence);
  const genericAbstractLightAnchor = abstractLightAnchorCore && anchorUnityCore;
  const abstractLightFormCore = /(?:single|sole|stable|central)[^]{0,120}(?:light form|light core|harmonic core|luminous core|glowing core)[^]{0,120}(?:blue|glow|light|radiat|unified|workflow|governance)/.test(coreEvidence);
  const genericAbstractLightForm = abstractLightFormCore && (workflowUnityCore || anchorUnityCore);
  const abstractWaveCore = /(?:single|seamless|harmonic|resonance)[^]{0,120}(?:wave|waves|current|flow)[^]{0,160}(?:blue|light|unified|workflow|governance|frictionless|harmony)/.test(coreEvidence);
  const genericAbstractWaveFlow = abstractWaveCore && (workflowUnityCore || anchorUnityCore);
  if (!genericGridIntegration && !genericGlowingConnectivity && !genericAbstractLightAnchor && !genericAbstractLightForm && !genericAbstractWaveFlow) return null;
  ...
  return {
    ...
    rules: [
      ...
      requiresConceptReplacement
        ? "A blocking reviewer has rejected the selected concept itself, not merely its execution. Perform a true concept replacement: change concept.title, concept.signature_device, concept.creative_thesis, concept.hook, concept.narrative, concept.visual_system, story hook/resolution/observable_proof/anti_cliche_strategy, and every production-step title, purpose, narrative_structure and selected_signature_device that still encodes the rejected concept. Preserve selected_concept_id only as the governance slot identifier. Do not keep the old concept name or signature device, do not merely rename/reskin the same metaphor, and do not copy a reviewer-suggested example metaphor verbatim as the new creative idea. Invent a distinct, mission-specific visual idea from the supplied business truth and constraints. Any anti_cliche_rules, refused_devices, director repairs, motif limits or production constraints that were introduced only to support the now-rejected concept are stale and must not constrain the replacement; preserve only constraints independently grounded in the original mission, verified rights/assets, brand evidence, medium, safety, or registered production capabilities. A replacement must contain an observable, concrete business relationship or consequence; abstract light, grids, nodes, ribbons, streams, waves, currents, cores, or generic connectivity/unification symbols are not sufficient proof by themselves."
        : null,
      blocking.length
...
async function runReviews({
  ...
  const settledResults = await Promise.allSettled(
    reviewers.map(async (reviewer) => {
      ...
      for (let schemaAttempt = 1; schemaAttempt <= 2; schemaAttempt += 1) {
        const basePayload = reviewPayload({ reviewer, context, plan, floor });
        let reasoned = null;
        try {
          ...
          const message = text(error?.message || error);
          if (schemaAttempt === 1 && message.endsWith("_JSON_REQUIRED")) {
            continue;
          }
          throw error;
        }
        ...
        if (score === null || score < 0 || score > 100 || !passedIsBoolean || reviewUsesTemplatePlaceholder(output)) {
          ...
          throw new Error(
            `CREATIVE_TRIBUNAL_REVIEW_SCHEMA_INVALID:${reviewer.id}:score=${text(output?.score) || "missing"}:passed=${typeof output?.passed}:template_placeholder=${reviewUsesTemplatePlaceholder(output)}`,
          );
        }
        ...
        try {
          ...
          if (
            schemaAttempt === 1 &&
            message.startsWith("CREATIVE_TRIBUNAL_REVIEW_AUTHORITY_VIOLATION:")
          ) {
            continue;
          }
          throw error;
      ...
      if (!acceptedOutput || !acceptedResult) {
        throw new Error(`CREATIVE_TRIBUNAL_REVIEW_NOT_ACCEPTED:${reviewer.id}`);
      }
      return {
        reviewer,
  ...
  if (rejected) {
    ...
    const error = rejected.reason instanceof Error
      ? rejected.reason
      : new Error(structuredReason || "CREATIVE_TRIBUNAL_REVIEW_FAILED");
