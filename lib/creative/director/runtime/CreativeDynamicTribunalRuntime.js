...
function normalizedOutput(result = {}) {
  ...
  return parseJson(output.text || output.content || output);
}
...
function repairPayload({ context, plan, tribunal }) {
  return {
    ...
    output: "Return the complete repaired Creative Master Plan JSON only.",
    rules: [
...
async function inspect({
  ...
  const reviewers = validatePanel(panelRun.output.reviewers);
  ...
  async review({
    ...
    if (!tribunal.verdict.passed) {
      const error = new Error(
        `CREATIVE_DYNAMIC_TRIBUNAL_REJECTED:${tribunal.verdict.weighted_score}:${tribunal.verdict.required_floor}`,
      );
