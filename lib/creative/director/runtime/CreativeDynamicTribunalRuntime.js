...
function reviewerDiscipline(reviewer = {}) {
  const mandate = `${text(reviewer.role)} ${text(reviewer.mandate)}`.toLowerCase();
  if (/asset|continuity|source|manifest/.test(mandate)) return "ASSET_CONTINUITY";
  if (/brand|identity|logo|wordmark|signage|truth/.test(mandate)) return "BRAND_TRUTH";
...
function brandProductionEvidence(plan = {}) {
  const signal = /brand|logo|wordmark|sign|text|chalkboard|typograph|identity/i;
