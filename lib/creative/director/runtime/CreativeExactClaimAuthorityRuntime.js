...
function replacePlacementClaims(value, key, unsupportedPlacements) {
  ...
  if (/(?:^|_)(?:placement|position)(?:_|$)/i.test(key)) return replaceUnsupported(next);
  next = next.replace(/[^.!?]*(?:logo|wordmark)[^.!?]*[.!?]?/gi, (segment) => replaceUnsupported(segment));
  return next;
}

function normalizeLegacyAuthorityPlaceholders(value) {
  if (Array.isArray(value)) return value.map(normalizeLegacyAuthorityPlaceholders);
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, nested]) => [key, normalizeLegacyAuthorityPlaceholders(nested)]));
  }
  if (typeof value !== "string") return value;
  return value
    .replace(/evidence-bound brand color/gi, "creative palette color (not claimed as an official brand color)")
    .replace(/\b(?:[A-Za-z0-9&.'-]+\s+){1,5}(blue|red|green|orange|yellow|purple|violet|pink|gold|silver|black|white|gray|grey|teal|cyan|magenta|navy)\s+(?:\(creative palette color|creative palette color)\s+\(not claimed as an official brand color\)\)?/gi, (_match, color) => `creative palette ${String(color).toLowerCase()} (not claimed as an official brand color)`)
    .replace(/verified production font/gi, "deterministic production typeface (not claimed as an official brand font)")
    .replace(/verified business proof/gi, "source-verified business proof required before release");
}
...
export function groundCreativeExactClaims({ plan = {}, mission = {} } = {}) {
  const normalizedPlan = normalizeLegacyAuthorityPlaceholders(structuredClone(plan));
  const truth = exactBrandTruth(normalizedPlan);
  const authority = authorityText(mission, truth);
  const tokens = collect(normalizedPlan);
  const unsupported = {
  ...
  if (![...Object.values(unsupported)].some((set) => set.size)) return normalizedPlan;
  return transform(normalizedPlan, "plan", unsupported);
}
