...
function normalizeLegacyAuthorityPlaceholders(value) {
  ...
  return value
    ...
    .replace(/\b(?:[A-Za-z0-9&.'-]+\s+){1,4}(blue|red|green|orange|yellow|purple|violet|pink|gold|silver|black|white|gray|grey|teal|cyan|magenta|navy)\s+\(creative palette color \(not claimed as an official brand color\)\)/gi, (_match, color) => `creative palette ${String(color).toLowerCase()} (not claimed as an official brand color)`)
