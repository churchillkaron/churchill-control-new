...
function transform(value, key, unsupported) {
  ...
  for (const token of unsupported.fonts) next = replaceToken(next, token, "verified production font");
  for (const token of unsupported.metrics) next = replaceToken(next, token, "verified business proof");
  if (/(?:logo|wordmark|placement|position)/i.test(`${key} ${value}`)) {
    for (const token of unsupported.placements) next = replaceToken(next, token, "design-defined safe area");
  }
  return next;
}
