...
function collectExactTokens(value, path = [], out = { hexes: new Set(), fonts: new Set(), metrics: new Set() }) {
  const key = path.at(-1) || "";
...
export function groundCreativeExactClaims({ plan = {}, mission = {} } = {}) {
  ...
  if (!unsupported.hexes.size && !unsupported.fonts.size && !unsupported.metrics.size) return plan;
