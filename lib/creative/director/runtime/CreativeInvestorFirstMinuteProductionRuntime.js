...
const STORY_LINEAGE_CONTRACT = "CREATIVE_STORY_LINEAGE_CONTRACT_V1";
const INVESTOR_FIRST_MINUTE_RESEARCH_IDENTITY =
  "avantiqo-investor-film-first-minute-alignment-proof-v1";
...
function canonicalStoryLineage(value = {}) {
  const lineage = object(value.story_lineage || value.metadata?.story_lineage || value);
  return {
    ...lineage,
    contract: STORY_LINEAGE_CONTRACT,
    research_identity:
      text(lineage.research_identity) || INVESTOR_FIRST_MINUTE_RESEARCH_IDENTITY,
...
function assertAuthoritativeMaster(plan = {}) {
  ...
  const lineage = canonicalStoryLineage(plan);
...
export function scopeInvestorFirstMinuteMaster(master = {}) {
  const sourcePlan = planFrom(master);
  const fullPlan = {
    ...sourcePlan,
    story_lineage: canonicalStoryLineage(sourcePlan),
  };
    ...
    for (const [shotIndex, shot] of list(scene.shots).entries()) {
      ...
      scopedShots.push({
        ...
        metadata: {
          ...
          story_lineage: canonicalStoryLineage(fullPlan),
...
async function materializeScopedRows({ organization_id, creative_project_id, storyboard, scoped }) {
  ...
  const scenes = [];
  const shots = [];
  const lineage = canonicalStoryLineage(scoped.plan);
...
export async function prepareInvestorFirstMinuteProduction(input = {}) {
  ...
  let graph = list(existingGraphs).find((candidate) =>
    text(candidate.storyboard_id) === text(storyboard.id) &&
    text(candidate.metadata?.investor_first_minute_scope) === CREATIVE_INVESTOR_FIRST_MINUTE_PRODUCTION_CONTRACT &&
    text(candidate.metadata?.full_master_hash) === scoped.full_master_hash &&
    text(candidate.metadata?.master_plan_hash) === text(lineage.master_plan_hash),
  ) || null;
  const productionGraphResumed = Boolean(graph?.id);
  if (!graph) {
    ...
    graph = await ProductionGraphRuntime.update(graph.id, {
      metadata: {
  ...
  const optimizedGraph = productionGraphResumed
    ? graph
    : await AssetReuseEngine.optimizeGraph({ organization_id, creative_project_id, graph });
  ...
  return {
    ...
    production_graph_resumed: productionGraphResumed,
