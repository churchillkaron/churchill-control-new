...
function operativeWorkflowInstructions(request) {
  ...
  return [
    `This job's workflow_kind is ${operative.workflow_kind}, declared as ${operative.declared_as}. Return exactly that workflow_kind and design under operative_workflow.contract; the other workflow contracts are context only.`,
    required.length
      ? `The ${operative.workflow_kind} contract makes these sections mandatory and complete: ${required.join(", ")}. A plan missing any of them is invalid, so populate every field the contract defines for each.`
      : null,
  ].filter(Boolean);
}
