...
function normalizeLegacyCreativeReviewContract(plan = {}) {
  ...
  const existing = list(review.finishing_requirements).map(text).filter(Boolean);
  if (existing.length >= 2) return plan;
  const derived = [...list(review.repair_instructions), ...list(review.craft_risks)].map(text).filter((entry) => entry.length >= 20).slice(0, 2);
  if (derived.length < 2) return plan;
  return { ...plan, creative_review: { ...review, finishing_requirements: derived, finishing_requirements_derived_from_legacy_review: true } };
}
