...
// The manifest may key an entry by asset_id, id, or be a bare id string, matching how
// the validator reads it -- so the comparison uses the same rule the gate applies rather
// than a stricter guess.
function manifestAssetId(entry) {
  if (typeof entry === "string") return text(entry);
  return text(entry?.asset_id || entry?.id);
}

function unaccountedSelectedAssetIds(plan = {}, assets = []) {
  // singletonObjectList returns its input unchanged when it is neither an array nor an
  // object, so an absent manifest comes back as undefined. That is exactly the case this
  // helper exists to describe, so the result is normalised through list() before use.
  const manifested = new Set(
    list(singletonObjectList(plan.asset_manifest))
      .map(manifestAssetId)
      .filter(Boolean),
  );
  return list(assets)
