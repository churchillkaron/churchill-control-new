...
function allowDegradedDirection(project = {}) {
  return (
    project.metadata?.allow_degraded_creative_direction === true ||
...
function assetIdentity(asset = {}, index = 0) {
  return {
    asset_id: asset.id || asset.asset_id || `input-asset-${index + 1}`,
...
function fallbackPlan({ brief = {}, project = {}, assets = [] }) {
  const objective = text(
    brief.creative_objective ||
      brief.business_goal ||
      project.objective,
    "Create an original production that serves the stated business goal.",
  );
  ...
  return {
    ...
    concept: {
      ...
      call_to_action: text(brief.requested_action, "Review before any release."),
    ...
    deliverables: [
      {
        ...
        output_spec: {
          duration_seconds: Number(brief.duration_seconds || project.target_duration || 30),
    ...
    scenes: [],
    quality: {
      minimum_scene_score: 100,
      regenerate_below_score: 100,
...
industry-neutral agency that must create original, culturally specific, commercially
useful work without generic AI aesthetics. Providers are production workers; Avantiqo
owns every creative and operational decision.
...
Return strict JSON only. No markdown. The complete schema is:
{
  ...
  "concept": {
    ...
    "narrative": "complete creative narrative",
  ...
  "scenes": [{
    ...
    "objective": "unique story purpose",
    ...
    "shots": [{
      ...
      "frame_plan": {
        ...
        "progression": "second-by-second or beat-by-beat visible progression",
      ...
      "reference_assets": [{"asset_id":"", "role":""}],
      ...
      "generation": {
        ...
        "provider_prompt": "complete provider-ready prompt containing all visual and temporal direction",
...
- Every active agency role must return a structured decision, evidence, confidence, risks and repair instructions.
...
- Specify camera, lighting, design, continuity, sound, graphics, VFX, transitions and negative constraints.
...
export const CreativeMasterPlanRuntime = {
  async create({
    ...
    try {
      ...
      if (!allowDegradedDirection(project)) {
      ...
      return {
        ...
        model: "creative-master-plan-degraded-v2",
        usage: null,
        billing: null,
        fallback: true,
        degraded: true,
