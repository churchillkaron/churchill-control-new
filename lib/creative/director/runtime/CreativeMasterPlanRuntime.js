...
function normalizedPlan(result) {
  ...
  for (const section of ["concept", "creative_review", "deliverables", "story", "production", "scenes"]) {
  ...
  return {
    ...promotedSections,
    workflow_kind:
      promotedSections.workflow_kind || operativeWorkflow.workflow_kind || null,
    asset_manifest: singletonObjectList(promotedSections.asset_manifest),
    deliverables: singletonObjectList(promotedSections.deliverables),
    role_decisions: normalizeRoleDecisions(promotedSections.role_decisions),
    concept: {
      ...promotedConcept,
      creative_system:
        promotedConcept.creative_system || promotedConcept.visual_system || null,
      visual_system:
        promotedConcept.visual_system || promotedConcept.creative_system || null,
    },
  };
}
...
function masterPlanAssetEvidence(asset = {}) {
  ...
  const { semantic_repair: _semanticRepair, frame_samples: _frameSamples, raw: _raw, ...analysisEvidence } = analysis;
  return {
    id: asset.id,
