...
function isAuthorizedRecoveredStory(project = {}, plan = {}) {
  ...
  if (recovery.contract !== "CREATIVE_STORY_LINEAGE_RECOVERY_V1" || recovery.user_authorized !== true) return false;
  ...
  return (
    ...
    text(recoveredConcept.causal_story || recoveredStory.causal_story) === text(currentConcept.causal_story || currentStory.causal_story) &&
    JSON.stringify(list(recoveredConcept.irreversible_turns)) === JSON.stringify(list(currentConcept.irreversible_turns))
  );
}

function preserveAuthorizedRecoveredStory(project = {}, candidatePlan = {}) {
  const recovery = object(project?.metadata?.creative_story_lineage_recovery);
  if (recovery.contract !== "CREATIVE_STORY_LINEAGE_RECOVERY_V1" || recovery.user_authorized !== true) return candidatePlan;
  const recoveredPlan = object(recovery.master?.plan);
  ...
  return {
    ...
    concept: {
      ...
      ...(candidateConcept.target_audience !== undefined ? { target_audience: structuredClone(candidateConcept.target_audience) } : {}),
    },
...
async function repairInvalidPlan({
  ...
  const executeRepair = (repairPlan, repairValidationError) => ServiceExecutionRuntime.execute({
    organization_id,
    service_id: "ai.reasoning.execute",
    provider_id: null,
    category: "CREATIVE_DIRECTION",
