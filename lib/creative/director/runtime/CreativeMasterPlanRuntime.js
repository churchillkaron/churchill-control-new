...
function normalizedPlan(result) {
  ...
  return {
    ...
    role_decisions: normalizeRoleDecisions(plan.role_decisions),
    concept: {
      ...concept,
      creative_system:
...
function repairRequest({
  ...
  return {
    ...
    rules: [
      ...
      "Return role_decisions as one JSON object keyed by exact registered role_id, never an array.",
      ...
      "Every ACTIVE role listed in execution_checklist.active_roles_missing_evidence must receive real evidence from immutable context or be changed to NOT_REQUIRED with a concrete reason when the discipline is genuinely unnecessary.",
