...
const MAXIMUM_CONTRACT_REPAIR_ATTEMPTS = 4;
...
function recoveredStoryFingerprint(plan = {}) {
  ...
  return JSON.stringify({
    ...
    payoff: concept.payoff || null,
    causal_story: concept.causal_story || object(plan.story).causal_story || null,
    governing_world_rule: concept.governing_world_rule || null,
    irreversible_turns: list(concept.irreversible_turns),
...
function isAuthorizedRecoveredStory(project = {}, plan = {}) {
  ...
  const recoveredPlan = object(recovery.master?.plan);
  if (!Object.keys(recoveredPlan).length) return false;

  const recoveredConcept = object(recoveredPlan.concept);
  const currentConcept = object(plan.concept);
  const recoveredStory = object(recoveredPlan.story);
  const currentStory = object(plan.story);
  return (
    text(recoveredConcept.title) === text(currentConcept.title) &&
    text(recoveredConcept.central_proposition) === text(currentConcept.central_proposition) &&
    text(recoveredConcept.governing_world_rule) === text(currentConcept.governing_world_rule) &&
    text(recoveredConcept.causal_story || recoveredStory.causal_story) ===
      text(currentConcept.causal_story || currentStory.causal_story) &&
    JSON.stringify(list(recoveredConcept.irreversible_turns)) ===
      JSON.stringify(list(currentConcept.irreversible_turns))
  );
}
...
function preserveAuthorizedRecoveredStory(project = {}, candidatePlan = {}) {
  ...
  const recoveredPlan = object(recovery.master?.plan);
  if (!Object.keys(recoveredPlan).length) return candidatePlan;

  const recoveredConcept = object(recoveredPlan.concept);
  const candidateConcept = object(candidatePlan.concept);
  return {
    ...candidatePlan,
    concept: {
    ...
    story_lineage_lock: {
      ...
      fingerprint: recoveredStoryFingerprint(recoveredPlan),
...
function structuredRepairPlan(parsed = {}) {
  ...
  const separateMasterStory = object(parsed?.master_story);
  const storyArchitecture = object(plan.story_architecture);

  return {
    ...plan,
    ...(Object.keys(separateMasterStory).length
      ? {
          story_architecture: {
            ...storyArchitecture,
            master_story: separateMasterStory,
          },
        }
      : {}),
...
function stripProviderTransportDetails(value) {
  ...
  return Object.fromEntries(
    Object.entries(value)
      ...
      .map(([key, nested]) => [key, stripProviderTransportDetails(nested)]),
  );
}
...
function creativeProjectPromptSnapshot(project = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      protected_opening_authority: object(metadata.protected_opening_authority),
      ...
      defer_product_explanation_until_later_chapters:
        metadata.defer_product_explanation_until_later_chapters === true,
    },
  };
}
...
function semanticMissionContractSnapshot({ mission = {}, project = {}, brief = {} } = {}) {
  ...
  return {
    ...
    global_scale_required: required("global_scale_required"),
    universe_scale_required: required("universe_scale_required"),
    technology_ai_future_required: required("technology_ai_future_required"),
    core_mechanism_dramatization_required: required("core_mechanism_dramatization_required"),
    core_mechanism: first("core_mechanism"),
...
async function repairInvalidPlan({
  ...
  const repairRequestHash = createHash("sha256")
  ...
  const merged = preserveAuthorizedRecoveredStory(
    project,
    mergeCreativeRepairedPlan(plan, safeRepair),
  );
  return {
  ...
  async repairExistingPlan({
    ...
    for (let attempt = 0; attempt <= MAXIMUM_CONTRACT_REPAIR_ATTEMPTS; attempt += 1) {
      try {
        ...
        const useRecoveredDecisionReview =
          isAuthorizedRecoveredStory(project, plan) &&
          recoveredDecisionReviewOnlyFailure(validationError);
        const repair = useRecoveredDecisionReview
          ? await repairRecoveredDecisionReview({
              organization_id,
              mission,
              project,
              brief,
