...
import {
  CreativeConceptCouncilRuntime,
} from "@/lib/creative/director/runtime/CreativeConceptCouncilRuntime";
import {
...
const MAXIMUM_DIRECTOR_CHARTER_REPAIR_ATTEMPTS = 1;
...
function directorCharterPrompt({ mission, project, brief, evidence, assetAuthority, duration }) {
  return JSON.stringify({
    ...
    assignment: {
      ...
      protected_opening_seconds:
        brief?.metadata?.chapter_one_target_seconds ?? mission?.metadata?.chapter_one_target_seconds ?? null,
      product_explanation_deferred_until_after_protected_opening:
        project?.metadata?.defer_product_explanation_until_later_chapters === true,
      aspect_ratio: "16:9",
    ...
    required_output: {
      ...
      protected_opening_strategy: {
        global_scale_is_primary: true,
        geographies_or_world_regions: ["at least three materially distinct places or world regions"],
        industries_or_infrastructures: ["at least three materially distinct sectors, systems or infrastructures"],
        human_presence: "how people make the scale emotionally legible without turning the opening into one company's operational story",
        mystery_tension_arc: "how the first minute builds the feeling that a vast intelligence is already moving through the real world",
        product_explanation_present: false,
        single_business_event_dominates: false,
        independent_world_signals_before_connection: true,
        ordinary_business_event_begins_after_protected_opening: true,
        opening_story_form: "multiple independent real-world signals across places and systems that gradually feel connected, without sharing one timestamp, object, source event or operational cause",
        threshold_into_next_chapter: "the earned reveal/question that makes Chapter Two necessary without explaining Avantiqo inside the protected opening",
      },
      signature_device: {
        mechanism: "string",
    ...
    creative_authority_order: [
      "EXPLICIT_USER_AND_MISSION_CONSTRAINTS",
      "MISSION_OBJECTIVE_AND_CHAPTER_REQUIREMENTS",
      "BRIEF_CREATIVE_CONSTRAINTS",
      "CANONICAL_PRODUCT_EVIDENCE",
      "DIRECTOR_CHARTER_CREATIVE_CHOICES",
      "MARKET_RESEARCH_STRATEGY_CONTEXT",
    ],
    immutable_mission_constraints: {
      mission_objective: text(mission?.objective),
      project_objective: text(project?.objective),
      chapter_one_target_seconds: brief?.metadata?.chapter_one_target_seconds ?? mission?.metadata?.chapter_one_target_seconds ?? null,
      defer_product_explanation_until_later_chapters: project?.metadata?.defer_product_explanation_until_later_chapters === true,
      creative_constraints: list(brief?.metadata?.creative_constraints),
      old_investor_script_authority: false,
    },
    mandatory_rules: [
      "Explicit user, mission and brief constraints are immutable creative authority. They outrank every charter-derived story choice, product-proof preference, benchmark inference and market-research suggestion.",
      "If the mission defines an opening chapter, deferred reveal, geography, mystery, scale, pacing, or what must NOT yet be explained, preserve those requirements exactly through the charter and later production stages.",
      "Product proof may determine later movements, but it may not invade or replace an explicitly deferred opening chapter. Do not move product explanation earlier merely because a causal business example is easier to prove.",
      ...
      `Use at least ${MINIMUM_PRODUCT_PROOF_AREAS} materially different product-proof areas. Returning fewer is a hard contract failure.`,
      "When product explanation is deferred, all product-proof areas belong after the protected opening window. Design the full later-film proof architecture now, but do not spend any of it inside the protected opening.",
      "The protected opening is for global scale, mystery, tension, infrastructure, technology, people and consequence without explaining Avantiqo or presenting a business-case proof sequence.",
      "When a protected opening is requested as global/worldwide, it must materially traverse at least three distinct geographies or world regions and at least three distinct industries, infrastructures or human systems. A single port, warehouse, company, delivery, invoice, customer, factory or other ordinary-business event may NOT dominate this window.",
      "The protected opening must create the sensation that one vast intelligence is awakening or moving through the real world before the viewer understands what it is. The mystery must come from cinematic cause, rhythm, scale and connection, not holograms, floating interfaces, generic AI orbs or decorative data overlays.",
      "The ordinary business event used to prove Avantiqo's causal mechanism belongs after the protected opening when product explanation is deferred. Never let product-proof convenience rewrite the opening into that business event.",
      "Inside the protected opening, use multiple independent world signals before any causal business story begins. They may rhyme through timing, motion, sound, geometry, human behavior or infrastructure, but they must not share one timestamp, label, document, shipment, incident, object or source event, and must not be revealed as consequences of one delivery, supplier delay, invoice, customer, company or operational incident.",
      "The protected opening threshold may reveal that the world is connected or that an intelligence is present, but it must not reveal that the preceding places share one timestamp, one object, one shipment or one ordinary business event. The audience may feel a pattern before knowing its source; the concrete causal proof begins only in Chapter Two or later.",
...
function validateDirectorCharter({ charter, project, evidence }) {
  ...
  if (project?.metadata?.defer_product_explanation_until_later_chapters === true) {
    const opening = object(charter.protected_opening_strategy);
    if (
      opening.global_scale_is_primary !== true ||
      opening.product_explanation_present !== false ||
      opening.single_business_event_dominates !== false ||
      opening.independent_world_signals_before_connection !== true ||
      opening.ordinary_business_event_begins_after_protected_opening !== true ||
      list(opening.geographies_or_world_regions).length < 3 ||
      list(opening.industries_or_infrastructures).length < 3 ||
      !text(opening.opening_story_form) ||
      !text(opening.mystery_tension_arc) ||
      !text(opening.threshold_into_next_chapter)
    ) {
      throw new Error("CREATIVE_INVESTOR_FILM_PROTECTED_OPENING_CHARTER_INSUFFICIENT");
    }
    const protectedOpeningText = [
      opening.opening_story_form,
      opening.mystery_tension_arc,
      opening.threshold_into_next_chapter,
      charter.narrative_architecture?.form,
      charter.narrative_architecture?.opening_question,
      charter.audience_tension,
    ].map(text).join(" ").toLowerCase();
    const singleEventOpeningPatterns = [
      "one event", "single event", "same event", "delivery delay", "supplier delay",
      "one delivery", "one supplier", "chain reaction", "same timestamp", "shared timestamp",
      "same label", "same shipping label", "same label detail", "same evidence",
      "same shipment", "same container", "one event through", "single causal chain",
      "identical timestamp", "identical timestamps", "same moment in time",
    ];
    if (singleEventOpeningPatterns.some((pattern) => protectedOpeningText.includes(pattern))) {
      throw new Error("CREATIVE_INVESTOR_FILM_PROTECTED_OPENING_SINGLE_EVENT_LEAK");
    }
  }
  if (
...
function mergeDirectorCharterRepair(base, patch) {
  ...
  for (const [key, value] of Object.entries(patch)) {
    if (Array.isArray(value)) {
      ...
      merged[key] = mergeDirectorCharterRepair(source[key], value);
    } else {
      merged[key] = value;
    }
  }
...
function directorCharterRepairPrompt({ charter, failure, mission, project, brief }) {
  return JSON.stringify({
    task: "Repair the Director Charter without changing the mission. Return the complete corrected Director Charter JSON only.",
    failure,
    immutable_rules: [
      "Preserve explicit user, mission and brief authority.",
      "The protected first minute is a worldwide intelligence-awakening experience, not a disguised ordinary business-event proof sequence.",
      "Protected-opening signals must be independent: no shared timestamp, label, document, shipment, container, customer, company, incident or source event.",
      "Do not retroactively reveal the protected-opening locations as consequences of one ordinary business event.",
      "Concrete causal business proof starts only after the protected opening.",
      "Preserve valid canonical evidence IDs and later-film product-proof strategy unless the validation failure requires a correction.",
      ...([
        "CREATIVE_INVESTOR_FILM_PROTECTED_OPENING_SINGLE_EVENT_LEAK",
        "CREATIVE_INVESTOR_FILM_PROTECTED_OPENING_CHARTER_INSUFFICIENT",
      ].includes(failure) ? [
        "Rewrite every conflicting opening-level narrative field together: audience_tension, narrative_architecture.form, narrative_architecture.opening_question, protected_opening_strategy.opening_story_form, protected_opening_strategy.mystery_tension_arc and protected_opening_strategy.threshold_into_next_chapter.",
        "None of those fields may describe the protected opening as one event, one shipment, one supplier, one delivery delay, one causal chain, one incident, or three places reacting to the same cause.",
        "The protected opening must remain multiple independent world signals whose relationship is only that a larger intelligence seems to perceive them; any concrete ordinary business causal chain begins after the protected opening.",
        "Do not merely swap nouns or locations. Repair the story logic itself so the opening is genuinely global, independent and mysterious.",
        "protected_opening_strategy must remain a complete object containing global_scale_is_primary, geographies_or_world_regions, industries_or_infrastructures, human_presence, mystery_tension_arc, product_explanation_present, single_business_event_dominates, independent_world_signals_before_connection, ordinary_business_event_begins_after_protected_opening, opening_story_form and threshold_into_next_chapter.",
        "threshold_into_next_chapter belongs inside protected_opening_strategy and must never be emitted as a top-level Charter field.",
        "Do not use identical timestamps, the same moment in time, or any shared physical evidence as the device connecting the independent opening signals. Rhyming sound, motion, geometry, human behavior or scale is allowed; shared causal evidence is not."
      ] : []),
    ],
    mission_objective: text(mission?.objective),
    project_objective: text(project?.objective),
    brief_constraints: list(brief?.metadata?.creative_constraints),
...
async function createDirectorCharter({
  ...
  let execution = await runIntelligenceReasoningLoop({
    organization_id,
    execution_lane: "deep",
    system: directorCharterSystemPrompt(),
    messages: [{ role: "user", content: directorCharterPrompt({ mission, project, brief, evidence, assetAuthority, duration }) }],
  ...
  let charter = parseJson(execution.text);

  for (let attempt = 0; attempt <= MAXIMUM_DIRECTOR_CHARTER_REPAIR_ATTEMPTS; attempt += 1) {
    try {
      validateDirectorCharter({ charter, project, evidence });
      return { charter, execution };
    } catch (error) {
      if (attempt === MAXIMUM_DIRECTOR_CHARTER_REPAIR_ATTEMPTS) throw error;
      const failure = text(error?.message) || "CREATIVE_INVESTOR_FILM_DIRECTOR_CHARTER_INVALID";
      execution = await runIntelligenceReasoningLoop({
        organization_id,
        execution_lane: "deep",
        system: directorCharterSystemPrompt(),
        messages: [{ role: "user", content: directorCharterRepairPrompt({ charter, failure, mission, project, brief }) }],
        response_format: { type: "json_object" },
        temperature: 0.2,
        max_output_tokens: 12000,
        max_turns: 2,
        ...
        metadata: {
          module: "CREATIVE",
          operation: "CREATIVE_INVESTOR_FILM_DIRECTOR_CHARTER_REPAIR",
          creative_mission_id: mission?.id || mission?.creative_mission_id || null,
          creative_project_id: project?.id || null,
          director_charter_repair_attempt: attempt + 1,
          director_charter_validation_failure: failure,
        },
      });
      assertOwnedIntelligence(execution, "DIRECTOR_CHARTER_REPAIR");
      charter = mergeDirectorCharterRepair(charter, parseJson(execution.text));
    }
  }
  throw new Error("CREATIVE_INVESTOR_FILM_DIRECTOR_CHARTER_INVALID");
}
  ...
  return {
    project: {
      ...
      metadata: {
        ...
        protected_opening_authority: object(charter.protected_opening_strategy),
...
export async function createOwnedInvestorFilmMission(input = {}) {
  ...
  const storyMaster = await CreativeMasterPlanRuntime.create({
    organization_id,
    mission,
  ...
  const councilMaster = {
    ...
    usage: {
      ...object(storyMaster.usage),
      concept_council: council.usage,
    },
    billing: {
      ...object(storyMaster.billing),
      concept_council: council.billing,
    },
  };

  const tribunalMaster = await CreativeDynamicTribunalRuntime.review({
    organization_id,
    creative_mission_id: mission.id || mission.creative_mission_id || null,
    ...
    brief: enriched.brief,
    assets,
    available_capabilities: storyMaster.available_production_capabilities || [],
    master: councilMaster,
  });

  const temporal = CreativeWorldClassConceptIntelligenceRuntime.enforce(
    await CreativeUniversalTemporalDirectionRuntime.create({
      ...
      brief: enriched.brief,
      assets,
      approved_master: tribunalMaster,
    }),
  );
  ...
  return {
    ...
    independent_concept_council: council.council,
    tribunal_approved_master: tribunalMaster,
