...
import "@/lib/creative/director/runtime/CreativeDirectionExactResumeRuntime";
...
import "@/lib/creative/director/runtime/CreativeCinematicImpactRuntime";
...
function approvedResearch(project = {}) {
  ...
  if (
    ...
    !["COMPLETED", "COMPLETED_FROM_EXISTING_USAGE"].includes(text(approval.status).toUpperCase()) ||
...
function approvedDirection(project = {}) {
  if (InternalCreativeResearchRuntime.applies(project)) {
    return {
      contract: "CREATIVE_INTERNAL_DIRECTION_AUTHORITY_V1",
      id: `internal-direction:${project.id}`,
      approved: true,
      status: "APPROVED",
      scope: "OWNED_LOCAL_INTERNAL_CREATIVE",
      command_identity: text(project.metadata?.command_identity) || null,
      provider: "avantiqo-intelligence",
      infrastructure_provider: "AVANTIQO_LOCAL_NODE_V1",
      maximum_customer_price: 0,
      supplier_cost_estimate: 0,
      currency: "THB",
...
function researchForBrief(report = {}) {
  return {
    ...
    company_truth: report.metadata?.company_truth || report.company_truth || {},
    brand_intelligence: report.metadata?.brand_intelligence || report.brand_intelligence || {},
    audience: report.audience || {},
    competitor_analysis: report.metadata?.competitor_analysis || {},
    market: report.metadata?.market || {},
    commercial_intelligence: report.metadata?.commercial_intelligence || report.commercial_intelligence || {},
    messaging: report.messaging || {},
    creative_grounding: report.metadata?.creative_grounding || {},
    strategic_synthesis: report.metadata?.strategic_synthesis || {},
    creative_opportunities: report.metadata?.creative_opportunities || report.creative_opportunities || {},
    recommendations: report.recommendations || {},
    trends: report.trends || {},
    keywords: report.keywords || [],
    claims: report.metadata?.claims || report.claims || [],
    sources: report.metadata?.sources || report.sources || [],
...
export async function prepareProjectDirection({
  ...
  const internalCreative = InternalCreativeResearchRuntime.applies(project);
  const researchApproval = internalCreative ? {
    contract: "CREATIVE_INTERNAL_RESEARCH_AUTHORITY_V1",
    approved: true,
    status: "COMPLETED",
    scope: "OWNED_LOCAL_INTERNAL_CREATIVE",
    local_only: true,
    external_provider_authorized: false,
  } : approvedResearch(project);
  const directionApproval = approvedDirection(project);
  const internalReports = internalCreative
    ? await ResearchRepository.list({
        organization_id,
        creative_project_id,
      })
    : [];
  const internalReport = internalReports.find((item) =>
    item?.metadata?.validation?.passed === true &&
    text(item?.metadata?.validation?.policy?.mode).toUpperCase() === "INTERNAL_CREATIVE"
  ) || null;
  ...
  if (internalCreative) researchApproval.research_report_id = report.id;
