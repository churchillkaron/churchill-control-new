...
function explicitSingleShotIntent(prompt) {
  const source = text(prompt).toLowerCase().replace(/[_-]+/g, " ").replace(/\s+/g, " ");
  if (!source) return false;
  const direct = [
    /\bsingle\s+continuous\s+shot\b/,
    /\bsingle\s+shot\b/,
    /\bone\s+take\b/,
    /\bsingle\s+take\b/,
    /\b(?:create|make|produce|deliver|render)\s+one\b[^.!?\n]{0,120}\bshot\b/,
  ];
  return direct.some((pattern) => pattern.test(source));
}
...
function explicitPlanningContract(input = {}, prompt = "") {
  ...
  const singleContinuous =
    jsonBooleanFromPrompt(prompt, [
      "single_continuous_shot",
      "singleContinuousShot",
    ]) === true ||
    explicitSingleShotIntent(prompt);
