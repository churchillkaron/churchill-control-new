...
function oneShotScene(plan = {}, entries = [], authority = null, critique = {}) {
  ...
  const openingAdjustment = repairs.openingScalePercent
    ? ` Apply the approved critique adjustment: opening subject frame presence is ${repairs.openingScalePercent}% larger than the rejected opening.`
    : "";
  ...
  const shot = {
    ...
    frame_plan: {
      ...
      opening_frame: `${repairs.openingScalePercent
        ...
        : (text(firstShot.frame_plan?.opening_frame) || text(firstShot.subject))}${openingAdjustment}`.trim(),
      progression,
