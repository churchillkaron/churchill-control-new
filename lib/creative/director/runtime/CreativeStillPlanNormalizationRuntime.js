...
export function normalizeCreativeStillPlan(plan = {}) {
  ...
  if (refusesPhysicalObjects(concept)) {
    const directors = object(next.directors);
    const strategyDirector = object(directors.strategy_director);
    next.directors = {
      ...directors,
      strategy_director: {
        ...strategyDirector,
        risks: replaceForbiddenPhysicalMetaphor(strategyDirector.risks, signature),
        repair_instructions: replaceForbiddenPhysicalMetaphor(strategyDirector.repair_instructions, signature),
      },
    };
  }
