...
function balancedJsonObjectCandidates(sourceValue) {
...
function parseJson(value) {
  ...
  candidates.push(...balancedJsonObjectCandidates(source));
  ...
  for (const candidate of candidates) {
    ...
    seen.add(normalized);
    const parsed = parsedJsonObject(normalized);
...
function normalizedOutput(result) {
  ...
  while (queue.length) {
    ...
    if (current && typeof current === "object" && !Array.isArray(current)) {
      ...
      const settledProviderResult = current.usage?.metadata?.provider_result;
      if (settledProviderResult !== undefined && settledProviderResult !== null) {
        queue.push({ value: settledProviderResult, depth: branchDepth + 1 });
      }
      const metadataProviderResult = current.metadata?.provider_result;
      if (metadataProviderResult !== undefined && metadataProviderResult !== null) {
        queue.push({ value: metadataProviderResult, depth: branchDepth + 1 });
      }
      const billingProviderResult = current.billing?.usage?.metadata?.provider_result;
      if (billingProviderResult !== undefined && billingProviderResult !== null) {
        queue.push({ value: billingProviderResult, depth: branchDepth + 1 });
      }
    }
  }
...
function normalizeApprovedStoryContract(plan = {}) {
  ...
  const hook =
    text(story.hook) ||
    text(concept.hook) ||
    text(concept.narrative) ||
    "A specific change disturbs the established world and gives the audience a concrete question to follow.";
  const causal =
    text(story.causal_story) ||
    text(concept.narrative) ||
    text(concept.creative_thesis) ||
    `${hook} Each subsequent event must arise from the previous one and visibly change the state of the world.`;
  const governingIdea =
    text(concept.creative_thesis) ||
    text(concept.central_proposition) ||
    text(concept.message) ||
    causal;
  const payoff =
    text(story.payoff) ||
    text(story.emotional_payoff) ||
    text(concept.message) ||
    `The final consequence resolves the question established by the hook and makes the governing idea observable rather than merely stated.`;
  return {
    ...plan,
    story: {
      ...story,
      hook,
      audience_tension:
        text(story.audience_tension) ||
        `The audience keeps asking what the initial change means and how far its consequences will spread before the governing idea becomes undeniable.`,
      human_desire:
        text(story.human_desire) ||
        `The central human presence wants to understand or act meaningfully inside the changing world rather than merely witness disconnected spectacle.`,
      emotional_contradiction:
        text(story.emotional_contradiction) ||
        `The world appears familiar enough to trust, yet each consequence makes that familiarity less certain and forces a more deliberate human response.`,
      vulnerability_or_risk:
        text(story.vulnerability_or_risk) ||
        `If the central choice has no visible consequence, the story becomes decorative; if the consequence arrives without an earned choice, the governing idea becomes arbitrary.`,
      empathy_path:
        text(story.empathy_path) ||
        `Begin with observable human attention, deepen identification through specific choices and consequences, and let the audience discover the meaning through what changes on screen.`,
      escalation:
        text(story.escalation) ||
        `Each event increases consequence, specificity or scale while preserving the same causal world established by the hook.`,
      observable_proof:
        text(story.observable_proof) ||
        `The governing idea is proven by a visible before-and-after state caused by the central human choice and its downstream consequences.`,
      turn:
        text(story.turn) ||
        `The decisive turn occurs when a human choice changes the causal direction of the established world instead of merely observing it.`,
      resolution:
        text(story.resolution) ||
        `The consequences settle into a new observable state that resolves the audience question and reveals the meaning of the governing idea.`,
      call_to_action:
        text(story.call_to_action) ||
        `Leave the audience with the consequence of ${governingIdea} rather than ending on explanatory product-demo language.`,
      emotional_arc: (() => {
        ...
        return states.length >= 4
          ? current
          : "observation -> curiosity -> mounting consequence -> decisive human choice -> earned recognition";
      })(),
      emotional_payoff: text(story.emotional_payoff) || payoff,
      anti_cliche_strategy:
        text(story.anti_cliche_strategy) ||
        "Reject generic montage, decorative technology spectacle, unexplained transformation and exposition that is not earned by visible causal action.",
      causal_story: causal,
      payoff,
    },
  };
...
function normalizeShotExecutionContract(shot = {}, scene = {}, shotIndex = 0) {
  ...
  return normalizeShotCompatibility({
    ...
    purpose,
    first_pass_intent: {
      story_delta: executableText(shot.first_pass_intent?.story_delta, `This beat adds a new causal fact to the scene: ${purpose}`, 20),
      visible_event: executableText(shot.first_pass_intent?.visible_event, `The frame begins in the prior state and visibly changes because ${action}`, 20),
      edit_reason: executableText(shot.first_pass_intent?.edit_reason, `This angle exists because it creates a motivated editorial handoff into the next causal beat rather than repeating coverage.`, 20),
      continuity_anchor: executableText(shot.first_pass_intent?.continuity_anchor, `Preserve subject identity, established geography, screen direction, wardrobe/product state and material world continuity across this cut.`, 15),
      sound_picture_event: executableText(shot.first_pass_intent?.sound_picture_event, `Anchor the visible state change to a physically motivated source sound, breath, contact, mechanism or deliberate silence.`, 15),
      predicted_failure: executableText(shot.first_pass_intent?.predicted_failure, `Prevent generic AI coverage, posing, floating camera, continuity drift, synthetic material behavior and any shot that leaves the story state unchanged.`, 20),
    },
    device,
    frame_plan: {
      ...framePlan,
...
function ensureTemporalCadenceCoverage(plan = {}) {
  ...
  const validatorCadence = deriveTemporalCadenceContract({ ...plan, scenes }, scenes);
  const legacyTarget = duration >= 45 ? Math.max(10, Math.ceil(duration / 6)) : Math.max(1, Math.ceil(duration / 7.5));
  const target = Math.max(legacyTarget, Number(validatorCadence.minimum_shot_count) || 1);
  let count = scenes.reduce((sum, scene) => sum + scene.shots.length, 0);
  if (validatorCadence.premium_temporal === true && count < target) {
    return {
      ...plan,
      scenes,
      temporal_cadence_repair: {
        contract: "CREATIVE_PREMIUM_AUTHORED_COVERAGE_REPAIR_REQUIRED_V1",
        passed: false,
        current_shot_count: count,
        required_shot_count: target,
        mechanical_posthoc_split_forbidden: true,
        owner: "TEMPORAL_SCENE_SHOT_DIRECTION",
      },
    };
  }
  while (count < target) {
...
function shotCountRange(duration, { scene = {}, plan = {} } = {}) {
  const language = [
    ...
    ...list(plan.scenes).flatMap((entry) => [
      entry?.title,
      entry?.objective,
      entry?.emotion,
      entry?.transition_logic,
      entry?.tension?.audience_question,
    ]),
  ].map(text).join(" ").toLowerCase();
  ...
  // Premium temporal work needs authored editorial coverage, not one generated clip per narrative movement.
  // This is a craft-density floor rather than a mandate for frantic cutting: intentional long takes remain valid.
  ...
  let maxAverageSeconds = premiumTemporal ? 2.75 : 4;
  if (patient) maxAverageSeconds = premiumTemporal ? 3.5 : 5.5;
  if (tensionDriven) maxAverageSeconds = Math.min(maxAverageSeconds, premiumTemporal ? 2.5 : 3.75);
  if (kinetic) maxAverageSeconds = Math.min(maxAverageSeconds, premiumTemporal ? 1.75 : 2.5);
  if (deliberateLongTake) maxAverageSeconds = Math.max(maxAverageSeconds, duration);
  const minimum = deliberateLongTake ? 1 : Math.max(3, Math.ceil(duration / maxAverageSeconds));
  const referenceAverage = premiumTemporal
    ? (kinetic ? 1.5 : tensionDriven ? 2 : patient ? 3 : 2.25)
    : (kinetic ? 2 : tensionDriven ? 2.75 : patient ? 4.5 : 3);
...
function temporalLocalOutputTokenBudget(operation, requested) {
  const requestedTokens = Number(requested) || 1024;
  const cap = text(operation).toUpperCase() === "TEMPORAL_MASTER_PLAN_CONTRACT_REPAIR_V1"
    ? 8192
    : 6000;
  return Math.max(1, Math.min(requestedTokens, cap));
}
...
function temporalReasoningUsageId({
  ...
  const requestFingerprint = createHash("sha256")
    .update(JSON.stringify({
      ...
      max_output_tokens: temporalLocalOutputTokenBudget(operation, maxOutputTokens),
...
async function executeReasoning({
  ...
  if (localOnly) {
    const localInput = {
      ...
      hierarchical_child_timeout_ms: 600000,
      context: {
        organization_id,
...
function temporalContractRepairPrompt({ plan, validationError, assets, attempt }) {
  const allFailures = Array.isArray(validationError?.validation?.failures)
    ...
    : [];
  const uniqueFailuresByPath = (entries = []) => {
    const seen = new Set();
    return entries.filter((entry) => {
      const signature = text(entry.path) || `${text(entry.code)}:${text(entry.message)}`;
      if (seen.has(signature)) return false;
      seen.add(signature);
      return true;
    });
  };
  const roleFailures = uniqueFailuresByPath(
    allFailures.filter((entry) => /^role_decisions\./.test(text(entry.path))),
  );
  const otherFailures = uniqueFailuresByPath(
    allFailures.filter((entry) => !/^role_decisions\./.test(text(entry.path))),
  );
  // Role records are verbose. If every missing role is emitted in one repair, the local
  // deep final can terminate mid-object even though each individual decision is valid.
  // Repair them in bounded chunks and leave room for shot/contract failures so each
  // pass remains a complete parseable JSON patch.
  const roleBatch = roleFailures.slice(0, Math.min(6, MAXIMUM_CONTRACT_REPAIR_FAILURES_PER_PASS));
  const remainingSlots = Math.max(0, MAXIMUM_CONTRACT_REPAIR_FAILURES_PER_PASS - roleBatch.length);
  const failures = [...roleBatch, ...otherFailures.slice(0, remainingSlots)];
...
- Never mark an applicable film discipline NOT_REQUIRED when the authored temporal plan gives it material production responsibility; applicable film departments with concrete scene, shot, world, sound or finishing work MUST be ACTIVE.
...
SHOT COUNT: choose it from this scene's action. Permitted ${range.minimum} to ${range.maximum}, reference ${range.preferred}. The reference is not a ceiling. Premium/high-budget work must add purposeful inserts, reactions, macro details, VFX plates, editorial bridges and alternate coverage when the story earns them. Do not stretch a scene into a few long generated clips merely to reduce shot count. If the film needs flagship-scale density, achieve it across the full scene architecture while keeping each scene's authored shot package specific and executable.
...
export const CreativeTemporalMasterPlanRuntime = {
  async create({
    ...
    let approvedMasterPlan = object(approved_master?.plan || approved_master);
    ...
    if (tribunalSeeded && list(approvedMasterPlan.scenes).length) {
      let approvedPlan = applyDerivedRoleDecisions({
        ...approvedMasterPlan,
        quality_profile: project.quality_profile || approvedMasterPlan.quality_profile || null,
      }, CREATIVE_AGENCY_ROLES);
      ...
      try {
        const validation = assertCreativeMasterPlan({ plan: approvedPlan, assets: normalizedAssets });
        return {
          plan: {
            ...approvedPlan,
            taste_memory: tasteMemory,
          ...
          validation,
          contract_repair: {
            executed: false, attempts: 0,
            maximum_attempts: MAXIMUM_CONTRACT_REPAIR_ATTEMPTS, rejected_repairs: [],
          },
          provider: null, model: null,
          usage: { calls: 0, items: [] }, billing: { calls: 0, items: [] },
          fallback: false, degraded: false,
          chunked_temporal_direction: false, reused_approved_master: true,
        };
      } catch (reuseError) {
        approvedMasterPlan = {
          ...approvedPlan,
          scenes: [],
          validation: null,
          stale_approved_master_reauthoring: {
            contract: "CREATIVE_STALE_APPROVED_MASTER_REAUTHORING_V1",
            required: true,
            reason: text(reuseError?.message) || "CURRENT_MASTER_PLAN_VALIDATION_FAILED",
            approved_concept_and_story_preserved: true,
            stale_scene_and_shot_architecture_reuse_forbidden: true,
          },
        };
      }
    }
    ...
    if (tribunalSeeded) {
      ...
      basePlan = {
        ...
        workflow_kind: "TEMPORAL",
        quality_profile: project.quality_profile || object(baseExecution.output).quality_profile || null,
    ...
    const scenePlanningPlan = { ...basePlan, scenes };
    const filmShotRange = shotCountRange(duration, { scene: {}, plan: scenePlanningPlan });
    const baseSceneShotRanges = scenes.map((scene) =>
      shotCountRange(scene.duration_seconds, { scene, plan: scenePlanningPlan })
    );
    const baseMinimumTotal = baseSceneShotRanges.reduce((sum, range) => sum + Number(range.minimum || 0), 0);
    const authoredCoverageDeficit = Math.max(0, Number(filmShotRange.minimum || 0) - baseMinimumTotal);
    const totalSceneDuration = Math.max(0.001, scenes.reduce((sum, scene) => sum + (Number(scene.duration_seconds) || 0), 0));
    const sceneCoverageBoosts = scenes.map((scene) =>
      authoredCoverageDeficit > 0
        ? Math.floor(authoredCoverageDeficit * ((Number(scene.duration_seconds) || 0) / totalSceneDuration))
        : 0
    );
    let assignedBoost = sceneCoverageBoosts.reduce((sum, value) => sum + value, 0);
    for (let index = 0; assignedBoost < authoredCoverageDeficit; index = (index + 1) % Math.max(1, scenes.length)) {
      sceneCoverageBoosts[index] += 1;
      assignedBoost += 1;
    }
    async function planScene(scene, sceneIndex) {
      const baseShotRange = baseSceneShotRanges[sceneIndex] || shotCountRange(scene.duration_seconds, { scene, plan: scenePlanningPlan });
      const minimum = Number(baseShotRange.minimum || 1) + Number(sceneCoverageBoosts[sceneIndex] || 0);
      const shotRange = {
        ...baseShotRange,
        minimum,
        reference: Math.max(minimum, Number(baseShotRange.reference || minimum)),
        preferred: Math.max(minimum, Number(baseShotRange.preferred || minimum)),
        maximum: Math.max(minimum + 2, Number(baseShotRange.maximum || minimum + 2)),
        film_authored_coverage_floor: Number(filmShotRange.minimum || 0),
        scene_authored_coverage_boost: Number(sceneCoverageBoosts[sceneIndex] || 0),
      };
      ...
      for (let attempt = 1; attempt <= MAXIMUM_SCENE_SHOT_ATTEMPTS; attempt += 1) {
        try {
          ...
          if (shots.length < shotRange.minimum) {
            sceneFailure = `CREATIVE_TEMPORAL_SCENE_AUTHORED_COVERAGE_REQUIRED:${scene.id}:${shots.length}/${shotRange.minimum}`;
            outstandingFailures = [{
              code: "TEMPORAL_SCENE_AUTHORED_COVERAGE_REQUIRED",
              path: `scenes.${sceneIndex}.shots`,
              message: `Scene requires at least ${shotRange.minimum} authored shots for its ${scene.duration_seconds}s premium cadence; provider returned ${shots.length}. Mechanical post-hoc splitting is forbidden.`,
            }];
            continue;
          }
      ...
      if (!shots.length || shots.length < shotRange.minimum) {
        throw new Error(
          `CREATIVE_TEMPORAL_SCENE_AUTHORED_COVERAGE_REQUIRED:${scene.id}:${shots.length}/${shotRange.minimum}:${MAXIMUM_SCENE_SHOT_ATTEMPTS}_ATTEMPTS:${sceneFailure}`,
        );
      }
    ...
    for (let attempt = 0; attempt <= MAXIMUM_CONTRACT_REPAIR_ATTEMPTS; attempt += 1) {
      try {
        ...
        const candidate = applyDerivedRoleDecisions(
          scrubUnknownTemporalAssetReferences(
            normalizeTemporalQualityContract(
              normalizeTemporalMechanicalContract(
                mergedRepair,
