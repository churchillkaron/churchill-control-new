...
function deterministicSceneArchitectureFallback({ basePlan = {}, duration = 60 } = {}) {
  ...
  const beats = (arrowBeats.length >= 4 ? arrowBeats : fallbackBeats).slice(0, 7);
  if (beats.length < 3) {
...
Return strict JSON only as {"scenes":[...]} with exactly one entry for each scene id:
...
async function createShotInventionMap({ organization_id, missionId, projectId, basePlan, scenes, project, brief }) {
  const execution = await executeReasoning({
    ...
    missionId,
    projectId,
    prompt: shotInventionPrompt({ basePlan, scenes, project, brief }),
    maxOutputTokens: 12000,
  });
  const entries = list(execution.output.scenes);
  ...
  return { map: entries, result: execution.result };
}
...
- Never mark an applicable film discipline NOT_REQUIRED merely to satisfy validation or reduce work.
...
Every shot must contain:
{
  ...
  "hero_asset_truth": {
    "mode": "SOURCE_LOCKED_EXACT|REFERENCE_GROUNDED_CLASS|ORIGINAL_SYNTHETIC|NOT_APPLICABLE",
    "reference_asset_ids": ["exact approved asset ids when source-locked"],
    "exact_geometry_claimed": false,
    "limitations": ["specific limits on geometry/configuration certainty when reference-grounded"]
  },
...
function normalizeShotCompatibility(shot = {}, scene = {}) {
  validateSignatureFrameDesign(shot);
  validateSourceReinterpretation(shot);
  const framePlan = object(shot.frame_plan);
  const audio = object(shot.audio);
...
export const CreativeTemporalMasterPlanRuntime = {
  async create({
    ...
    executions.push(shotInvention.result);
      ...
      for (let attempt = 1; attempt <= MAXIMUM_SCENE_SHOT_ATTEMPTS; attempt += 1) {
        try {
          ...
          const candidate = allocateDurations(shots, scene.duration_seconds, 0.5)
            .map((shot) => normalizeShotCompatibility(shot, scene));
