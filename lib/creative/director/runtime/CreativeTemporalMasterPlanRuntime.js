...
export const CreativeTemporalMasterPlanRuntime = {
  async create({
    ...
    // A film is one shot call per scene, so a 205 second master is around fifteen calls after the
    // base plan and architecture. A single one of them failing used to lose every call before it:
    // one unparseable response or one empty shots array threw, and fourteen completed scenes were
    // discarded along with the film. That is the most expensive failure in the studio.
    //
    // Each scene is retried once on its own before the film is abandoned. A transient bad response
    // costs one call rather than all of them, and a scene that fails twice still fails closed.
    for (const [sceneIndex, scene] of scenes.entries()) {
      ...
      for (let attempt = 1; attempt <= MAXIMUM_SCENE_SHOT_ATTEMPTS; attempt += 1) {
        try {
          ...
          // Retried rather than rethrown: an unparseable or truncated response for one scene says
          // nothing about the fourteen that already succeeded.
