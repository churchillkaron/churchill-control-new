...
async function executeReasoning({
  ...
  const replay = await recoverChallengerSettledDirection({
    organization_id, projectId, missionId, operation, prompt,
  });
  let result = replay || await ServiceExecutionRuntime.execute({
    organization_id,
  ...
  if (result?.pending === true) {
    result = await awaitServiceExecutionCompletion({
      execution: result,
      organization_id,
