...
function temporalReasoningUsageId({
  ...
  const requestFingerprint = createHash("sha256")
    .update(JSON.stringify({
      idempotency_contract: "CREATIVE_TEMPORAL_REASONING_REQUEST_V2",
      ...
      hierarchical_contract: "AVANTIQO_HIERARCHICAL_LOCAL_REASONING_V1",
    }))
    .digest("hex")
    .slice(0, 24);
...
async function createShotInventionMap({ organization_id, missionId, projectId, basePlan, scenes, project, brief }) {
  ...
  const entries = list(execution.output.scenes).map((entry) => ({
    ...entry,
