...
function deterministicSceneArchitectureFallback({ basePlan = {}, duration = 60 } = {}) {
  ...
  const splitApprovedProgression = (value) => text(value)
    .split(/\s*(?:→|->|=>|⟶|⇢)\s*/u)
    .map((beat) => text(beat).replace(/[.]+$/u, ""))
    .filter(Boolean);
  const progressionBeats = splitApprovedProgression(
    concept.environment_progression || story.environment_progression,
  );
  const causalArrowBeats = splitApprovedProgression(causal);
  const irreversibleBeats = list(concept.irreversible_turns)
    .map((beat) => text(beat))
    .filter(Boolean);
  const proofBeats = list(
    concept.world_class_execution_design?.causal_connection?.proof_chain ||
    story.world_class_execution_design?.causal_connection?.proof_chain,
  )
    .map((beat) => text(beat))
    .filter(Boolean);
  const fallbackBeats = [
  ...
  const approvedBeatSource =
    progressionBeats.length >= 3 ? progressionBeats :
    causalArrowBeats.length >= 3 ? causalArrowBeats :
    irreversibleBeats.length >= 3 ? irreversibleBeats :
    proofBeats.length >= 3 ? proofBeats :
    fallbackBeats;
  const beats = [...approvedBeatSource];
  const earnedReveal = text(story.payoff) || text(story.resolution) || text(concept.payoff) || text(concept.message);
  if (
    earnedReveal &&
    beats.length < 7 &&
    !beats.some((beat) => /avantiqo|reveal/i.test(beat)) &&
    /avantiqo|reveal/i.test(earnedReveal)
  ) {
    beats.push(earnedReveal);
  }
  const boundedBeats = beats.slice(0, 7);
  if (boundedBeats.length < 3) {
    throw new Error("CREATIVE_TEMPORAL_APPROVED_STORY_BEATS_REQUIRED");
  }

  const sceneSeeds = boundedBeats.map((beat, index) => ({
    id: `scene-${String(index + 1).padStart(2, "0")}`,
    title: beat,
    duration_seconds: total / boundedBeats.length,
  }));
  ...
  return boundedBeats.map((beat, index) => {
    const previousBeat = index > 0 ? boundedBeats[index - 1] : text(story.hook) || "The approved world begins in its opening state.";
    const nextBeat = index < boundedBeats.length - 1 ? boundedBeats[index + 1] : payoff || "The earned Avantiqo reveal resolves the chain.";
    const isFinal = index === boundedBeats.length - 1;
...
function normalizeShotExecutionContract(shot = {}, scene = {}, shotIndex = 0) {
  const title = executableText(shot.title, `Camera beat ${shotIndex + 1}: ${text(scene.title) || text(scene.state_change) || "approved story beat"}`, 8);
  const approvedBeat = text(scene.state_change) || text(scene.title) || text(scene.objective) || "the approved causal change";
  const subject = executableText(shot.subject, `The exact approved scene subject and physical evidence of this beat: ${approvedBeat}.`, 8);
  const action = executableText(shot.action, `Make the approved causal beat visibly occur without substituting a new protagonist, device, location or mechanism: ${approvedBeat}.`);
  const performance = executableText(shot.performance, `Performance is limited to whatever human presence the approved scene explicitly requires; otherwise the environment itself carries the causal change: ${approvedBeat}.`);
  const purpose = executableText(shot.purpose, `Advance the approved causal story by making this camera beat prove the scene change: ${approvedBeat}.`);
  const device = executableText(shot.device, `Keep this beat physically faithful to the approved causal mechanism "${approvedBeat}"; no substitute device, interface or explanatory technology is introduced.`);
  ...
  return normalizeShotCompatibility({
    ...
    camera: {
      ...camera,
      framing: executableText(camera.framing, "medium close framing that preserves face, hands and decision context", 5),
    ...
    lighting: {
      ...lighting,
      source: executableText(lighting.source, "Motivated natural or practical light appropriate to the approved scene and its physical environment.", 5),
      direction: executableText(lighting.direction, "Direction follows the approved environment and reveals the causal subject without artificial glamour.", 5),
      contrast: executableText(lighting.contrast, "Restrained cinematic contrast that preserves physically plausible detail.", 5),
      colour: executableText(lighting.colour, "Naturalistic colour temperature appropriate to the approved environment and story progression.", 5),
      exposure_intent: executableText(lighting.exposure_intent, "Protect the approved causal subject and environmental detail while preserving the intended stillness and emergence.", 5),
    },
    production_design: {
      ...productionDesign,
      environment: executableText(productionDesign.environment, `The exact approved scene environment for "${text(scene.title) || approvedBeat}", preserving its natural/physical story state without corporate substitution.`, 5),
      wardrobe: executableText(productionDesign.wardrobe, list(scene.actors).length ? "Wardrobe follows only the human identity explicitly authorized by this approved scene." : "No wardrobe is introduced unless this approved scene explicitly contains a human subject.", 5),
      props: executableText(productionDesign.props, `Only physical objects explicitly required by the approved beat "${approvedBeat}" are present.`, 5),
      materials: executableText(productionDesign.materials, "Natural and physically plausible materials appropriate to the approved scene; no synthetic corporate showroom substitutions.", 5),
      texture_detail: executableText(productionDesign.texture_detail, "Tactile real-world micro-detail appropriate to the approved environment prevents synthetic appearance.", 5),
    },
    continuity: {
      ...continuity,
      identity: executableText(continuity.identity, list(scene.actors).length ? "Preserve only the human identity explicitly authorized by this approved scene." : "No human identity continuity is introduced unless the approved scene requires it.", 5),
      product: executableText(continuity.product, "Do not introduce a product or interface before the approved story explicitly earns the Avantiqo reveal.", 5),
      location: executableText(continuity.location, `Preserve the approved environment and physical geography of "${text(scene.title) || approvedBeat}" within this beat.`, 5),
      wardrobe: executableText(continuity.wardrobe, list(scene.actors).length ? "Preserve the approved human wardrobe state across adjacent coverage." : "No wardrobe continuity is required because no human subject is introduced by this beat.", 5),
      screen_direction: executableText(continuity.screen_direction, "Preserve established spatial direction and motivated eyelines within the approved beat.", 5),
      spatial_geography: executableText(continuity.spatial_geography, "Preserve the physical positions of the approved subject, terrain, objects and causal action across this beat.", 5),
    },
    audio: {
      ...audio,
      source_sound: executableText(audio.source_sound, "Physically motivated environmental sound from the approved beat; silence remains authoritative until the story explicitly earns a sound.", 5),
      mix_intent: executableText(audio.mix_intent, "Prioritize the approved causal sound event and natural environmental depth over decorative cinematic noise.", 10),
...
function shotCountRange(duration, { scene = {}, plan = {} } = {}) {
  ...
  return {
    ...
    reference,
    preferred: reference,
    maximum: Math.min(12, Math.max(minimum + 2, Math.round(duration / 1.75))),
    cadence_basis: {
      duration_seconds: duration,
...
async function executeReasoning({
  ...
  if (localOnly) {
    const localInput = {
      ...
      context: {
        organization_id,
        usage_id: `creative-temporal:${projectId}:${operation}`,
      },
      prompt,
...
GOVERNING PLAN
${JSON.stringify({ mission_authority: basePlan.mission_authority, concept: basePlan.concept, story: basePlan.story, deliverables: basePlan.deliverables })}
...
function shotPlanPrompt({
  ...
  const spatialPathRequired =
    object(brief?.creative_grounding?.spatial_path).required === true ||
    object(brief?.research?.creative_grounding?.spatial_path).required === true;
  return `
...
${spatialPathRequired ? "- RESEARCH creative_grounding.spatial_path.required is true for this project. Preserve its ordered_nodes and directed_edges in physical camera geography. Aerial/drone shots must populate aerial_cinematography using the existing Studio aerial contract and move consistently toward the evidence-backed next node/destination; a view that travels against the grounded route is invalid." : "- No evidence-backed spatial route is required for this project. Do not invent ordered_nodes, directed_edges, route validation, or a grounded-route failure. For aerial shots, direct only physically plausible camera movement required by the approved scene."}
- Do not create aerial_cinematography for non-aerial shots.
...
export const CreativeTemporalMasterPlanRuntime = {
  async create({
    ...
    if (!temporalVideoPair) {
      throw new Error("CREATIVE_TEMPORAL_VIDEO_CAPABILITY_REQUIRED");
    }
    ...
    function deterministicApprovedMasterBeat(scene, sceneIndex) {
      ...
      const videoPair = temporalVideoPair;
      ...
      return normalizeShotExecutionContract({
        ...
        subject: `The exact physical subject of the approved scene: ${text(scene.title) || text(scene.state_change) || "approved story beat"}.`,
        action: `The approved causal change occurs visibly and without substitution: ${text(scene.state_change) || text(scene.story_state_after) || text(scene.objective) || "the approved beat changes the world"}.`,
        performance: list(scene.actors).length
          ? `Human performance is limited to the actor explicitly authorized by this scene and moves from ${text(scene.story_state_before) || "the prior state"} to ${text(scene.story_state_after) || "the approved changed state"} through restrained physical behavior.`
          : `No human performance is invented; the approved environment itself carries the causal change from ${text(scene.story_state_before) || "the prior state"} to ${text(scene.story_state_after) || "the changed state"}.`,
        duration_seconds: durationSeconds,
        medium: "generated-video",
        frame_plan: {
          opening_frame: `Open on the approved scene subject and environment clearly anchored in its starting state: ${text(scene.story_state_before) || text(scene.title) || "the approved state before change"}.`,
          ...
          closing_frame: `End on the approved physical consequence of this beat: ${text(scene.story_state_after) || text(scene.transition_logic) || "the next approved scene inherits a changed state"}.`,
        },
        camera: {
    ...
    async function planScene(scene, sceneIndex) {
      const shotRange = tribunalSeeded
        ? {
            minimum: 1,
            reference: 1,
            preferred: 1,
            ...
            cadence_basis: {
              ...
              approved_resume_master_beat: true,
            },
          }
        : shotCountRange(scene.duration_seconds, { scene, plan: basePlan });
      let shots = [];
      ...
      for (let attempt = 1; attempt <= MAXIMUM_SCENE_SHOT_ATTEMPTS; attempt += 1) {
        try {
          ...
          const candidate = allocateDurations(shots, scene.duration_seconds, 0.5)
            .map((shot, shotIndex) => {
              const normalized = normalizeShotExecutionContract(shot, scene, shotIndex);
              const shotDuration = Math.max(0.5, Number(normalized.duration_seconds) || 0.5);
              return {
                ...normalized,
                medium: "generated-video",
                generation: {
                  ...object(normalized.generation),
                  required: true,
                  service: temporalVideoPair.service,
                  capability: temporalVideoPair.capability,
                  output_spec: {
                    ...outputSpec,
                    ...object(normalized.generation?.output_spec),
                    duration_seconds: shotDuration,
                    aspect_ratio: text(normalized.generation?.output_spec?.aspect_ratio) || text(outputSpec.aspect_ratio) || "16:9",
                    resolution: text(normalized.generation?.output_spec?.resolution) || text(outputSpec.resolution) || "1920x1080",
                    frame_rate: Number(normalized.generation?.output_spec?.frame_rate) || Number(outputSpec.frame_rate) || 24,
                  },
                },
              };
            });
