...
- generation.required must be boolean true for generated-video shots; generation.output_spec must carry the master aspect_ratio, resolution and a positive numeric frame_rate.
...
- Do not weaken researched subject/location grounding into 'generic' geography, generic machinery or generic offshore language. Preserve the named researched subject, Norwegian/North Sea setting, helicopter/platform identity and reference authority already present in the plan.

AGENCY ROLE CONTRACT
- Every role in APPLICABLE TEMPORAL ROLES below MUST be ACTIVE. Return status, a concrete mission-specific decision, evidence as a JSON array, confidence 0-100, risks as a JSON array, and repair_instructions as a JSON array.
- Only roles listed in INAPPLICABLE ROLES may be NOT_REQUIRED, with a concrete workflow reason.
- Never mark an applicable film discipline NOT_REQUIRED merely to satisfy validation.
  ...
  async create({
    ...
    for (let attempt = 0; attempt <= MAXIMUM_CONTRACT_REPAIR_ATTEMPTS; attempt += 1) {
      try {
        ...
        const candidate = mergeCreativeRepairedPlan(plan, normalizedRepair);
        try {
