import { requiredEliteFilmDepartments } from "./CreativeEliteFilmDepartmentActivationRuntime";
...
function sceneNarrativeMismatch(scene = {}, value = "") {
  const current = `${text(scene.location?.country)} ${text(scene.location?.city)} ${text(scene.location?.specific)} ${text(scene.title)}`.toLowerCase();
  const source = text(value).toLowerCase();
  if (!source) return false;
  const groups = [
    ["thailand", "thai", "phuket", "patong"],
    ["norway", "norwegian", "fjord", "aurlandsfjord", "sognefjord"],
    ["sweden", "swedish", "stockholm", "archipelago"],
  ];
  const own = groups.find((group) => group.some((term) => current.includes(term)));
  if (!own) return false;
  if (groups.some((group) => group !== own && group.some((term) => source.includes(term)))) return true;
  const officeOnly = /accountant|office|glass partition|business context|avantiqo ui|desk|workspace/.test(source);
  const currentIsOffice = /accountant|office|desk|workspace/.test(current);
  if (officeOnly && !currentIsOffice) return true;
  return false;
}
function normalizeSceneNarrative(scene = {}) {
  const place = text(scene.location?.specific) || text(scene.location?.city) || text(scene.location?.country) || text(scene.title) || "the current scene geography";
  const objective = text(scene.objective) || text(scene.story_function) || `Advance the authored story through ${place}.`;
  return {
    ...scene,
    story_state_before: sceneNarrativeMismatch(scene, scene.story_state_before)
      ? `The prior scene hands off its established visual motif into ${place} without changing the motif identity.`
      : scene.story_state_before,
    state_change: sceneNarrativeMismatch(scene, scene.state_change)
      ? `${objective} The established motif changes only through the authored environmental behavior of ${place}.`
      : scene.state_change,
    story_state_after: sceneNarrativeMismatch(scene, scene.story_state_after)
      ? `The motif is now visibly established within ${place} and is ready to hand off to the next scene.`
      : scene.story_state_after,
    transition_logic: sceneNarrativeMismatch(scene, scene.transition_logic)
      ? `Transition into ${place} by preserving the motif's geometry, direction and energy rather than borrowing geography or story state from another scene.`
...
function normalizeSceneEvidence(scene = {}, assets = [], plan = {}) {
  ...
  const location = officeGeoId && /office/.test(sceneText)
    ...
    : scene.location;
  return normalizeSceneNarrative({ ...scene, location, reference_asset_ids: sceneRefs, shots });
}
...
function normalizedPrevisualizationContracts(shot = {}, scene = {}, shotIndex = 0) {
  ...
  const motionSource = `${text(shot.action)} ${text(shot.frame_plan?.progression)} ${text(shot.performance_direction)} ${text(shot.performance)}`;
  const moving = /(move|moves|moving|drive|drives|fly|flies|approach|depart|takeoff|take off|land|landing|walk|run|travel|advance|retreat|cross|descend|rise|accelerat|decelerat|click|press|scan|swipe|type|lift|turn|rotate|open|close)/i.test(motionSource);
...
function normalizeShot(shot, scene, shotIndex, masterSpec, fallbackGeneration, planContext = {}) {
  ...
  const authoredFocusTarget = text(camera.focus_target || camera.focus);
  const focusTarget = meaningfulDirection(authoredFocusTarget) || (text(shot.subject)
    ? `Focus remains on ${text(shot.subject)} so the audience reads the authored action without visual ambiguity.`
    : `Focus remains on the primary authored subject so the shot hierarchy stays stable.`);
  ...
  if (text(generation.service) === "ai.video.edit" && !sourceBinding.primarySourceAssetId) {
    generation.service = "ai.video.generate";
  ...
  return {
    ...
    frame_plan: {
      ...framePlan,
      opening_frame: (() => {
        const opening = text(framePlan.opening_frame);
        return opening.length >= 30 ? opening : `Open on ${opening || text(shot.subject) || "the authored subject"}, framed clearly enough to establish subject, spatial relationship and visual state before the action progresses.`;
      })(),
    ...
    camera: {
      ...
      movement_path: meaningfulDirection(camera.movement_path) || meaningfulDirection(camera.movement) || "Locked off on the authored support; the frame does not travel because subject action and sound carry the beat.",
      movement_speed: (() => {
        const authoredSpeed = meaningfulDirection(camera.movement_speed);
        if (authoredSpeed.length >= 5) return authoredSpeed;
        if (authoredSpeed) return `Camera moves at a ${authoredSpeed.toLowerCase()} controlled pace, preserving the authored subject timing and avoiding acceleration drift.`;
        const authoredMovement = meaningfulDirection(camera.movement);
        return authoredMovement
          ? `Movement speed follows the authored camera move at a controlled pace: ${authoredMovement}.`
          : "Zero travel speed: the camera remains deliberately locked for the whole shot so visible subject change carries the beat.";
      })(),
      camera_distance: (() => {
        const distance = text(camera.camera_distance);
        if (distance.length >= 5) return distance;
        if (distance) return `${distance} from the authored subject, preserving the intended scale and perspective.`;
        return "Camera distance preserves the authored framing and subject scale without changing spatial intent.";
      })(),
    ...
    production_design: {
      ...productionDesign,
      wardrobe: meaningfulDirection(productionDesign.wardrobe) || (noActors
        ? explicitAbsence("wardrobe", "no human talent appears in this shot, so the environment and physical action carry the visual information.")
        : "Wardrobe remains continuous with the established scene talent and supports the authored place, role and performance without introducing new branding."),
    ...
    continuity: {
      ...
      product: meaningfulDirection(continuity.product) || (productNames ? `Product continuity remains ${productNames}.` : explicitAbsence("separate product", "the shot is governed by the established scene subject.")),
...
function activeRoleDecision(roleId, plan = {}) {
  ...
  const rules = {
    ...
    production_designer: `Own the authored physical world, materials, surfaces, props and spatial continuity required to make ${thesis} visually specific rather than generic.`,
    previsualization_supervisor: `Translate the approved shot architecture for ${thesis} into executable staging, camera geometry, timing and transition evidence before paid generation.`,
    post_production_supervisor: `Own the finishing dependency graph for ${thesis}, ensuring editorial, VFX, color, sound and delivery evidence converge before release.`,
    color_di_supervisor: `Own exposure, contrast, palette continuity and final DI intent so every approved shot belongs to one authored visual world for ${thesis}.`,
    sound_design_supervisor: `Own source sound, designed effects, spatial perspective and silence as story-bearing elements synchronized to the authored picture for ${thesis}.`,
    mix_finishing_engineer: `Own final loudness, dynamics, stem integrity, sync and delivery mix evidence so the finished temporal master preserves ${thesis}.`,
    cg_asset_supervisor: `Own generated and CG asset truth, topology/material continuity and approved source bindings needed by ${thesis}.`,
    compositing_supervisor: `Own integration, edge, light-wrap, grain, depth and plate continuity so synthetic elements remain physically credible in ${thesis}.`,
    tracking_roto_supervisor: `Own tracking, matchmove and roto evidence required for physically coherent integration in ${thesis}.`,
    simulation_physics_supervisor: `Own physically plausible motion, forces, particles and mechanical behavior where the authored shots for ${thesis} require simulation.`,
    action_motion_supervisor: `Own action geography, subject motion and camera choreography so movement in ${thesis} remains legible, motivated and safe.`,
    second_unit_director: `Own specialist action and insert coverage that extends ${thesis} without weakening continuity or duplicating primary-unit coverage.`,
    location_production_supervisor: `Own named-place truth, observable geography and location continuity so ${thesis} never relies on generic or false geography.`,
    human_place_truth_director: `Own human behavior, cultural texture and place-specific observational truth required by ${thesis}.`,
    technical_subject_supervisor: `Own subject-specific technical truth and reference fidelity required for credible depiction in ${thesis}.`,
    product_capability_director: `Own truthful product capability demonstration and visible cause-effect evidence required by ${thesis}.`,
  };
  const decision = rules[roleId] || `Activate ${roleId} as a required elite-film authority for this temporal plan. Own the discipline-specific evidence, handoff quality and repair decisions needed to execute ${thesis} without lowering the approved quality floor.`;
  return { status: "ACTIVE", decision, evidence: [evidence], confidence: 95, risks: [], repair_instructions: [] };
}
function normalizeTemporalNarrativeCompleteness(plan = {}) {
  const concept = object(plan.concept);
  const story = { ...object(plan.story) };
  const humanTruth = object(concept.human_truth);
  const strategicBasis = object(concept.strategic_basis);
  const deliverables = list(plan.deliverables);
  const conceptMessage = text(concept.message || concept.central_proposition || story.observable_proof);
  const humanTension = text(humanTruth.emotional_contradiction || strategicBasis.central_tension || concept.task_truth?.human_tension);
  const humanDesire = text(story.human_desire || concept.hero_agency?.desire_or_purpose || humanTruth.why_it_matters || conceptMessage);
  const vulnerability = text(story.vulnerability_or_risk || concept.hero_agency?.cost_or_risk || list(concept.known_risks)[0]);
  const empathyPath = text(story.empathy_path || concept.performance_integration || concept.audience_feeling);
  const emotionalPayoff = text(story.emotional_payoff || concept.payoff || concept.audience_feeling || conceptMessage);
  const callToAction = text(story.call_to_action) || `Invite the audience to recognize ${conceptMessage || "the value created when decisions are made with full context"} without direct solicitation or a product-demo ending.`;
  const emotionalArc = text(story.emotional_arc);
  const normalizedArc = emotionalArc.split(/\s*(?:→|->|>)\s*/).filter(Boolean).length >= 4
    ? emotionalArc
    : "Mystery → Curiosity → Wonder → Awe, with each transition caused by a visible human decision and its consequence.";
  return {
    ...plan,
    deliverables: deliverables.length ? deliverables.map((d, index) => index === 0 ? {
      ...d,
      id: text(d.id) || "evolution-1min-master",
      purpose: text(d.purpose) || `Deliver the approved one-minute Evolution investor-film pilot as a complete cinematic proof of ${conceptMessage || "the approved story and visual thesis"}.`,
    } : d) : [{
      id: "evolution-1min-master",
      type: "VIDEO",
      purpose: `Deliver the approved one-minute Evolution investor-film pilot as a complete cinematic proof of ${conceptMessage || "the approved story and visual thesis"}.`,
      output_spec: {},
    }],
    story: {
      ...story,
      human_desire: humanDesire || "Humans want to make better decisions with full context while retaining agency over consequential actions.",
      emotional_contradiction: text(story.emotional_contradiction) || humanTension || "Visible human choices feel local while their consequences are shaped by an intelligence spanning a much larger connected world.",
      vulnerability_or_risk: vulnerability || "If the intelligence becomes too visible it feels intrusive; if it stays too subtle the human value can be missed.",
      empathy_path: empathyPath || "Stay with ordinary human work and let the audience feel the stakes through recognizable decisions, hesitation, relief and consequence before the larger system is revealed.",
      call_to_action: callToAction,
      emotional_payoff: emotionalPayoff || "The audience reaches awe through the realization that human agency and global intelligence can reinforce one another without erasing the person making the decision.",
      emotional_arc: normalizedArc,
    },
  };
}
...
function ensureAssetManifest(plan = {}, assets = []) {
  ...
  for (const entry of existing) {
    if (text(entry.disposition).toUpperCase() !== "EXCLUDE" && !list(entry.assignments).length) {
      entry.assignments = deliverableIds;
    }
  }
...
function completeGovernanceRoles(plan = {}) {
  ...
  const mandatory = new Set([
    "sound_director","motion_design_director","vfx_director","quality_director",
    "rights_safety_director","release_director","performance_director",
    ...requiredEliteFilmDepartments(plan),
  ]);
  for (const id of mandatory) {
    if (text(current[id]?.status).toUpperCase() !== "ACTIVE") next[id] = activeRoleDecision(id, plan);
  }
  return { ...plan, role_decisions: next };
}

export function normalizeTemporalMechanicalContract(plan = {}, { duration_seconds = null, assets = [] } = {}) {
  plan = normalizeTemporalNarrativeCompleteness(plan);
