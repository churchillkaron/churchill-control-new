...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.temporal-semantic-plan-gate.v2",
);
...
function repairShot(shot = {}, patch = {}, scene = {}) {
  ...
  const providerPrompt = [
    `STORY FUNCTION: ${text(patch.purpose || repaired.purpose)}.`,
    `VISIBLE SUBJECT: ${text(patch.subject || repaired.subject)}.`,
    `VISIBLE ACTION: ${text(patch.action || repaired.action)}.`,
    `PERFORMANCE: ${text(patch.performance || repaired.performance)}.`,
    `SCENE OBJECTIVE: ${text(scene.objective)}.`,
    text(generation.provider_prompt),
    "Do not repeat any earlier arrival, entrance, threshold or discovery beat unless this shot is explicitly the opening threshold beat. Preserve exact supplied identities, venue evidence, products and brand assets. No synthetic text, logo, watermark or poster inside generated pixels.",
  ].filter(Boolean).join("\n\n");
...
function repairHospitalityArc(plan = {}, validation = {}) {
  ...
  if (scenes.length !== 5 || scenes.some((scene) => !list(scene.shots).length)) {
    return null;
  }
  ...
  const phaseDefinitions = [
    {
      ...
      story_state_after:
        "The guest and audience are fully inside; all later scenes must advance beyond arrival.",
      transition_logic:
      ...
      shotPatches: [
        {
          ...
          action:
            "The guest notices the real entrance, commits to the visit and reaches the doorway without pausing for a second discovery beat.",
          performance:
    ...
    },
    {
      ...
      story_state_before:
        "The guest is inside but has not yet experienced the venue's hospitality or products.",
      state_change:
        "Attentive service and authentic food and drinks create the first tangible reward for entering.",
      story_state_after:
      ...
      shotPatches: [
        {
          ...
          purpose:
            "Show the guest being welcomed and ordering through natural staff interaction rather than another exterior or entrance beat.",
      ...
      shotPatches: [
        {
          ...
          action:
            "A natural gesture offers participation; the newcomer accepts and joins without another arrival or service beat.",
          performance:
    ...
    },
    {
      ...
      transition_logic:
        "After the peak, the film can resolve calmly with belonging, a final human exchange and an earned invitation.",
      shotPatches: [
        {
      ...
      shotPatches: [
        {
          ...
          purpose:
            "Let the emotional peak settle into a warm proof of new connection rather than another activity beat.",
        ...
        },
        {
          ...
          action:
            "The guest looks across the room, exchanges one final acknowledgement and remains inside the shared world rather than repeating an exit or entrance.",
          performance:
  ...
  const repairedScenes = scenes.map((scene, sceneIndex) => {
    const definition = phaseDefinitions[sceneIndex];
    ...
    return {
      ...
      metadata: {
        ...
        temporal_semantic_repair: {
          contract: "CREATIVE_TEMPORAL_SEMANTIC_REPAIR_V1",
  ...
  return {
    ...
    production: {
      ...
      temporal_semantic_repair_contract:
        "CREATIVE_TEMPORAL_SEMANTIC_REPAIR_V1",
