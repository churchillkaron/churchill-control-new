...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.temporal-semantic-plan-gate.v5",
);
const REPAIR_CONTRACT = "CREATIVE_TEMPORAL_SEMANTIC_REPAIR_V4";
...
function corpus(plan = {}, input = {}) {
  return safeJson({
    ...
    brief: input.brief,
    selected_assets: list(input.assets).map((asset) => ({
...
function boundedHospitalityArc(plan = {}, input = {}) {
  const source = corpus(plan, input);
  return Boolean(
  ...
  return [
    ...
    },
    {
      ...
      shotPatches: [
        ...
        {
          title: "Food and Drinks at the Table",
          purpose:
          ...
          action:
            "Staff place the verified dishes and drinks at the table; the guest tastes and responds naturally while the camera reveals texture, freshness and social warmth.",
          performance:
    ...
    return {
      ...
      metadata: {
        ...
        temporal_semantic_repair: {
          ...
          eligibility_source:
            "PLAN_PLUS_ORIGINAL_PROJECT_BRIEF_MISSION_AND_SELECTED_ASSETS",
          ...
          inanimate_arrival_language_prohibited: true,
        },
      },
    };
  });
  ...
  CreativeMasterPlanRuntime.create =
    async function createWithTemporalSemanticGate(input = {}) {
      ...
      if (
        ...
        boundedHospitalityArc(plan, input)
      ) {
