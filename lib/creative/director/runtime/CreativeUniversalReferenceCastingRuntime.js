...
function fidelityPrompt(contracts = []) {
  ...
  return [
    ...
    "REFERENCE MEDIA DEFINES IDENTITY AND WORLD TRUTH, NOT FINAL PRODUCTION VALUE. For premium temporal work, preserve the evidence-backed subject while re-photographing/reconstructing it cinematically through authored lighting, atmosphere, lensing, camera position, material response, depth, motion, invisible compositing and physically grounded VFX/CGI. Do not simply animate the uploaded still, add generic parallax, drift, stock particles or a digital zoom and call it cinematic transformation.",
    "A new viewpoint may be invented only when the available source/world evidence supports the geometry and continuity needed to keep the venue/product/subject truthful. Otherwise retain a defensible viewpoint and elevate the image photometrically and cinematically rather than hallucinating architecture.",
  ].join("\n");
}
