...
import {
  CreativeSingleShotRecoveryRuntime,
} from "./CreativeSingleShotRecoveryRuntime";
...
function lyricEvidence(project = {}, brief = {}) {
  const projectEvidence = object(project.metadata?.music_video_handoff?.music_intelligence?.lyric_evidence);
  const briefEvidence = object(brief.metadata?.lyric_evidence || brief.lyric_evidence);
  const evidence = Object.keys(briefEvidence).length ? briefEvidence : projectEvidence;
  const transcript = text(evidence.transcript || evidence.lyrics);
  if (!transcript) return null;
  return {
    ...evidence,
    transcript,
    phrase_timings: list(evidence.phrase_timings),
  };
}
...
function normalizedReasoningOutput(result = {}) {
  ...
  if (value && typeof value === "object") {
    const nested = value.result ?? value.text ?? value.content ?? null;
    if (nested && typeof nested === "object") return nested.result || nested;
    if (typeof nested === "string") {
      const first = nested.indexOf("{");
      const last = nested.lastIndexOf("}");
      if (first >= 0 && last > first) {
        try {
          const parsed = JSON.parse(nested.slice(first, last + 1));
...
async function createCreativeSynthesis({ organization_id, mission, project, brief, assets, identities, audio }) {
  const lyrics = lyricEvidence(project, brief);
  const musicIntelligence = object(project.metadata?.music_video_handoff?.music_intelligence);
  const compactProject = {
    id: project.id,
    name: project.name || project.title || null,
    target_duration: project.target_duration || project.target_duration_seconds || 210,
    music_video: true,
    full_song: project.metadata?.full_song === true,
    duration_mode: project.metadata?.duration_mode || "FULL_SOURCE_AUDIO",
    music: {
      bpm: musicIntelligence.bpm || project.metadata?.music_session?.bpm || null,
      time_signature: musicIntelligence.time_signature || project.metadata?.music_session?.time_signature || null,
      keyscale: musicIntelligence.keyscale || project.metadata?.music_session?.keyscale || null,
      instrumental: musicIntelligence.instrumental ?? project.metadata?.music_session?.instrumental ?? null,
    },
  };
  const compactAudio = audio ? {
    asset_id: assetId(audio),
    name: audio.name || audio.title || audio.file_name || null,
    duration_seconds: audio.technical?.duration_seconds || audio.analysis?.duration_seconds || audio.metadata?.duration_seconds || null,
  } : null;
  const compactAssets = list(assets).map((asset) => ({
    asset_id: assetId(asset),
    kind: assetKind(asset),
    name: asset.name || asset.title || asset.file_name || null,
    identity_name: asset.analysis?.identity?.name || asset.analysis?.identity?.label || null,
    tags: list(asset.tags).slice(0, 8),
  }));
  const compactLyrics = lyrics ? {
    contract: lyrics.contract || "AVANTIQO_MUSIC_LYRIC_EVIDENCE_V1",
    language: lyrics.language || null,
    transcript: lyrics.transcript,
    phrase_timings: list(lyrics.phrase_timings).map((phrase) => [
      phrase.phrase_index ?? null,
      phrase.start_seconds ?? null,
      phrase.end_seconds ?? null,
    ]),
  } : null;
  const compactBrief = {
    creative_objective: brief.creative_objective || brief.objective || null,
    duration_seconds: brief.duration_seconds || compactProject.target_duration,
    music_video: true,
  };
...
Analyse the complete supplied project as a physical, musical and environmental experience.
For a vocal song, the supplied lyric evidence is the PRIMARY NARRATIVE AUTHORITY: the story must express the actual lyrical meaning, emotional progression and turning point. Tempo, groove, energy, social scale, camera, performance and environment should interpret and amplify that meaning, not replace it with an unrelated concept.
...
- For vocal songs, every concept must remain faithful to the supplied lyric narrative and emotional turn. Concepts may be metaphorical, cinematic or non-literal, but they may not invent a different story.
- Rhythm, bass, percussion, builds, movement, social energy, environment and audience feeling must support the lyric-led narrative rather than compete with it.
- Reject generic heartbreak symbols, repeated literal lyric illustrations, empty beauty shots, slow visuals over energetic music, generic people walking alone, disconnected montage, and any recycled ripple/wave/world-transformation motif that is not explicitly grounded in the lyrics.
...
LYRIC EVIDENCE
${JSON.stringify(compactLyrics)}

IDENTITY PROFILES
${JSON.stringify(list(identities).map((identity) => ({
  id: identity.id || identity.identity_id || null,
  name: identity.name || identity.label || null,
  asset_ids: list(identity.asset_ids || identity.assets).slice(0, 12),
  rules: list(identity.rules).slice(0, 8),
})))}

PRIMARY AUDIO
${JSON.stringify(compactAudio)}
...
function identityForShot(shot, identities) {
  ...
  if (explicitProfileId) {
    const explicit = list(identities).find((profile) => text(profile.id) === explicitProfileId);
    if (explicit) return explicit;
  }

  const labels = list(shot.actors)
    .flatMap((actor) => [actor?.name, actor?.label, actor?.identity_key])
    .map((value) => text(value).toLowerCase())
    .filter(Boolean);
  if (!labels.length) return null;

  return list(identities).find((profile) => {
    const key = text(profile.identity_key).toLowerCase();
    if (!key) return false;
...
function enrichPlan({ plan, synthesis, identities, audio, isMusicVideo }) {
  ...
  const scenes = list(plan.scenes).map((scene) => {
    const shots = list(scene.shots).map((shot) => {
      ...
      if (profile) identityRequiredShots += 1;
  ...
  if (identityRequiredShots > 0 && identityBoundShots !== identityRequiredShots) {
    throw new Error(`UNIVERSAL_IDENTITY_REFERENCES_REQUIRED:required=${identityRequiredShots};bound=${identityBoundShots}`);
  }
  if (isMusicVideo) {
  ...
  return {
    ...
    validation_summary: {
      ...
      identity_required_shot_count: identityRequiredShots,
...
export function ensureUniversalTemporalDossier(plan = {}, { synthesis = {}, identities = [], audio = null, isMusicVideo = false } = {}) {
  ...
  const candidates = list(plan.concept_candidates).length ? list(plan.concept_candidates) : list(council.candidates);
  const selectedId = text(plan.selected_concept_id) || text(selection.selected_concept_id) || text(selected.id);
  const audioId = audio ? assetId(audio) : text(plan.production?.primary_audio_asset_id) || null;
  const musicWorld = Object.keys(object(plan.music_world)).length ? object(plan.music_world) : object(synthesis.music_world);
  const concept = object(plan.concept);
  const story = object(plan.story);
  const targetAudience = typeof concept.target_audience === "string"
    ? text(concept.target_audience)
    : text(concept.target_audience?.description);
  const causalStory = text(story.causal_story || concept.narrative || concept.causal_story);
  const payoff = text(story.payoff || concept.message || concept.emotional_promise);
  const observableSystem = text(
    concept.creative_system ||
    concept.signature_device ||
    concept.visual_system?.world ||
    concept.message,
  );
  const antiClicheInputs = [
    ...list(concept.refused_devices).map(text),
    ...list(plan.anti_cliche_rules).map(text),
    ...list(selected.anti_cliche_rules).map(text),
    ...list(synthesis.anti_cliche_rules).map(text),
  ].filter(Boolean);
  const enrichedStory = {
    ...story,
    audience_tension: text(story.audience_tension) ||
      `${targetAudience || "The intended audience"} begins from skepticism and pressure; the opening question must make the human cost of automated control immediately felt rather than explained.`,
    escalation: text(story.escalation) || causalStory ||
      "Each beat must increase the human consequence and visible system response until the approved turning point becomes unavoidable.",
    observable_proof: text(story.observable_proof) || observableSystem ||
      "The creative system must become visible through physical environmental response, human behavior and causal state change rather than explanatory interface shots.",
    turn: text(story.turn) ||
      "The turn occurs when the protagonist stops auto-approving the system and the environment begins reflecting human presence instead of prediction, converting internal tension into observable change.",
    resolution: text(story.resolution) || payoff ||
      "The final state resolves the human tension first, then earns the Avantiqo reveal as the consequence of that change rather than as an appended end card.",
    anti_cliche_strategy: text(story.anti_cliche_strategy) ||
      `Reject generic AI-ad shorthand, product-demo montage and decorative technology spectacle${antiClicheInputs.length ? `; specifically avoid: ${antiClicheInputs.join("; ")}` : ""}. Preserve one causal human story and an earned reveal.`,
  };
  return {
    ...plan,
    story: enrichedStory,
    music_world: musicWorld,
    identity_profiles: list(plan.identity_profiles).length ? list(plan.identity_profiles) : identities,
    concept_candidates: candidates,
    selected_concept_id: selectedId || null,
    concept_selection_reason: text(plan.concept_selection_reason) || text(selection.selection_reason) || text(synthesis.selection_reason),
    anti_cliche_rules: list(plan.anti_cliche_rules).length ? list(plan.anti_cliche_rules) : (list(selected.anti_cliche_rules).length ? list(selected.anti_cliche_rules) : list(synthesis.anti_cliche_rules)),
    motif_limits: list(plan.motif_limits).length ? list(plan.motif_limits) : list(synthesis.motif_limits),
    performance_policy: Object.keys(object(plan.performance_policy)).length ? object(plan.performance_policy) : object(synthesis.performance_policy),
    production: {
      ...object(plan.production),
      reuse_assets: false,
      reuse_policy: "NO_REUSE_UNLESS_EXPLICITLY_APPROVED",
      ...
      original_music_required: !isMusicVideo,
      sound_design_required: true,
      identity_verification_required: identities.length > 0 || plan.production?.identity_verification_required === true,
      dry_run_dossier_required_before_paid_generation: true,
    },
    deliverables: list(plan.deliverables).map((deliverable) => ({
      ...deliverable,
      output_spec: {
        ...object(deliverable.output_spec),
        audio: isMusicVideo ? "preserve the supplied primary soundtrack exactly" : (deliverable.output_spec?.audio || "create original exact-duration instrumental music and authentic sound design during production, with no copyrighted imitation"),
        audio_required: true,
        source_audio_required: isMusicVideo,
        original_music_required: !isMusicVideo,
        sound_design_required: true,
        primary_audio_asset_id: isMusicVideo ? audioId : null,
      },
    })),
  };
}
...
export const CreativeUniversalTemporalDirectionRuntime = {
  async create(input = {}) {
    ...
    const lyrics = lyricEvidence(project, brief);
    const instrumental = project.metadata?.music_video_handoff?.music_intelligence?.instrumental === true;
    if (isMusicVideo && !instrumental && !lyrics?.transcript) {
    ...
    const enrichedBrief = {
      ...
      metadata: {
        ...
        director_mandate: {
          lyrics_are_primary_narrative_authority_for_vocal_music_video: true,
          lyrics_are_one_signal_only: false,
    ...
    if (temporal.reused_approved_master === true) {
      const reusedPlan = ensureUniversalTemporalDossier(temporal.plan, {
      ...
      return {
        ...temporal,
        plan: reusedPlan,
        single_shot_recovery: null,
        cinematic_coverage: temporal.plan?.cinematic_coverage || null,
        coverage_validation: null,
        universal_creative_synthesis: null,
        universal_identity_profiles: temporal.plan?.identity_profiles || identities,
        primary_audio_asset_id: temporal.plan?.production?.primary_audio_asset_id || (audio ? assetId(audio) : null),
        music_video: isMusicVideo,
      };
    }
    ...
    const oneShotRecovery =
      enrichedPlan.temporal_contract?.single_continuous_shot === true
        ? CreativeSingleShotRecoveryRuntime.recover({ plan: enrichedPlan })
        : { recovered: false, plan: enrichedPlan };
    const preCoveragePlan = oneShotRecovery.plan;
    const coverage = await CreativeCinematicCoverageAuthoringRuntime.create({
      organization_id: organizationId,
      mission: object(input.mission),
      project,
      brief: enrichedBrief,
      plan: preCoveragePlan,
    });
    ...
    return {
      ...temporal,
      plan,
      single_shot_recovery: oneShotRecovery.recovered
        ? {
            contract: oneShotRecovery.contract,
            spatial_authority: oneShotRecovery.spatial_authority,
          }
        : null,
