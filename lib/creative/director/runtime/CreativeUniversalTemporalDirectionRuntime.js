...
function enrichPlan({ plan, synthesis, identities, audio, isMusicVideo }) {
  ...
  return {
    ...
    production: {
      ...
      audio_required: isMusicVideo,
      identity_verification_required: identities.length > 0,
    ...
    deliverables: list(plan.deliverables).map((deliverable) => ({
      ...
      output_spec: {
        ...
        audio: isMusicVideo ? "preserve the supplied primary soundtrack exactly" : deliverable.output_spec?.audio,
        audio_required: isMusicVideo,
