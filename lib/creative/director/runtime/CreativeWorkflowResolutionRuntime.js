...
function storedTemporalDirectionCheckpoint(project = {}, context = {}) {
  ...
  return restoreTribunalLineage(master, master);
}
...
export const CreativeWorkflowResolutionRuntime = Object.freeze({
  ...
  async resolve(input = {}) {
    ...
    const initialMaster =
      (!forceDirectionRestart
        ...
        : null) ||
      await CreativeMasterPlanRuntime.create({
        organization_id: context.organization_id,
        mission: context.mission,
        project: directionProject,
        brief: directionBrief,
  ...
  async resumeApprovedCouncil(input = {}) {
    ...
    const tribunalResumePlan = object(tribunalResume.repaired_plan);
    const tribunalMaster = Object.keys(tribunalResumePlan).length
      ? { ...councilMaster, plan: tribunalResumePlan }
      : councilMaster;
    if (tribunalMaster !== councilMaster) {
