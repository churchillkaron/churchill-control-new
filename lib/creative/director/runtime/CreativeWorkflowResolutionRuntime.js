...
async function recoverSettledTribunalResume({ context, project, master, available_capabilities = [] }) {
  ...
  let replayPlan = normalizeRecoveredTribunalPlan(master.plan, context.mission);
  ...
  for (let index = 0; index < settledEntries.length; index += 1) {
    ...
    if (storedEvidenceHash && ![currentEvidenceHash, legacyEvidenceHash].includes(storedEvidenceHash)) continue;
    // Migrate an exact legacy broad-scope receipt to the current discipline-scoped hash.
    // This is a one-time compatibility bridge; it never reuses a review whose underlying
    // current-plan evidence changed.
...
export const CreativeWorkflowResolutionRuntime = Object.freeze({
  ...
  async resolve(input = {}) {
    const context = await resolveContext(input);
    // Explicit human creative rejection outranks every persisted direction checkpoint.
    // Check this before temporal/council/post-repair recovery so a rejected idea can
    // never re-enter the next creative lineage merely because it has durable state.
    // Research evidence, mission authority, spend approval and usage ledger remain intact.
    const forceDirectionRestart = input.force_direction_restart === true;
    ...
    // Crash recovery should resume paid settled direction work, but explicit human
    // rejection is different: the rejected creative idea must not become the seed
    // of the next direction merely because its provider call already settled. A
    // fresh-direction restart preserves the same mission, research evidence, spend
    // approval and ledger while deliberately bypassing creative-plan recovery.
    let initialMaster = forceDirectionRestart
      ? null
      : await recoverSettledInitialMaster({
          context,
          project: directionProject,
          brief: directionBrief,
          assets: context.assets,
    ...
    const tribunalMaster = {
      ...tribunalMasterRaw,
      plan: CreativeStillPlanNormalizationRuntime.normalize(
        CreativeExactClaimAuthorityRuntime.ground({
  ...
  async resumeApprovedCouncil(input = {}) {
    ...
    const approvedMaster = {
      ...
      plan: CreativeVerifiedBrandAssetBindingRuntime.reconcilePlan({
        plan: approvedMasterSource.plan,
        project: context.project,
        assets: context.assets,
      }),
    };
