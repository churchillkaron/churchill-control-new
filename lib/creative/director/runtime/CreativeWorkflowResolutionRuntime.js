...
function storedRecoveredTemporalAuthoritySeed(project = {}, context = {}) {
  const recovery = object(project.metadata?.[STORY_LINEAGE_RECOVERY_METADATA_KEY]);
  if (recovery.contract !== "CREATIVE_STORY_LINEAGE_RECOVERY_V1") return null;
  if (recovery.user_authorized !== true) return null;
  if (recovery.organization_id && recovery.organization_id !== context.organization_id) return null;
  if (recovery.creative_mission_id && recovery.creative_mission_id !== context.creative_mission_id) return null;
  if (recovery.creative_project_id && recovery.creative_project_id !== context.creative_project_id) return null;

  const recoveredMaster = object(recovery.master);
  const recoveredPlan = Object.keys(object(recoveredMaster.plan)).length
    ? object(recoveredMaster.plan)
    : recoveredMaster;
  if (text(recoveredPlan.workflow_kind) !== "TEMPORAL") return null;
  const recoveredConceptId = text(
    recoveredPlan.selected_concept_id ||
    recoveredPlan.concept?.id ||
    recoveredMaster.independent_concept_council?.selection?.selected_concept_id ||
    recoveredPlan.concept_council?.selection?.selected_concept_id
  );
  if (!recoveredConceptId) return null;

  const temporalState = object(project.metadata?.[TEMPORAL_DIRECTION_CHECKPOINT_METADATA_KEY]);
  const temporalMaster = object(temporalState.master);
  const temporalPlan = object(temporalMaster.plan);
  const temporalConceptId = text(temporalPlan.selected_concept_id || temporalPlan.concept?.id);
  const tribunal = object(temporalPlan.creative_tribunal);
  if (temporalConceptId !== recoveredConceptId) return null;
  if (tribunal.passed !== true || tribunal.verdict?.passed !== true) return null;

  const council = object(
    recoveredMaster.independent_concept_council ||
    recoveredPlan.independent_concept_council ||
    recoveredPlan.concept_council
  );
  if (!Object.keys(council).length) return null;

  return {
    ...recoveredMaster,
    creative_tribunal: tribunal,
    independent_concept_council: council,
    lineage_recovery_authority: recovery,
    recovered_temporal_authority_seed: true,
    plan: {
      ...recoveredPlan,
      workflow_kind: "TEMPORAL",
      quality_profile: project.quality_profile || recoveredPlan.quality_profile || null,
      scenes: [],
      validation: null,
      creative_tribunal: tribunal,
      concept_council: council,
      independent_concept_council: council,
      selected_concept_id: recoveredConceptId,
...
export const CreativeWorkflowResolutionRuntime = Object.freeze({
  ...
  async resolve(input = {}) {
    ...
    // A user-authorized recovered story with a matching Tribunal verdict is later
    // creative authority than stale paid model output. Reuse only concept/story/council/
    // Tribunal authority and deliberately discard every old scene/shot so temporal
    // direction must author current-floor coverage from scratch.
    const recoveredTemporalAuthoritySeed = forceDirectionRestart
      ? null
      : storedRecoveredTemporalAuthoritySeed(directionProject, context);
    let initialMaster = recoveredTemporalAuthoritySeed || await recoverSettledInitialMaster({
      context,
      project: directionProject,
      brief: directionBrief,
      assets: context.assets,
    ...
    const rejectedLineage = recoveredTemporalAuthoritySeed
      ? null
      : rejectedDirectionLineageMatch(directionProject, initialMaster.plan);
    if (rejectedLineage) {
    ...
    let master;
    if (recoveredTemporalAuthoritySeed) {
      master = recoveredTemporalAuthoritySeed;
    } else {
      ...
      master = await reviewWithDurableResume(context, {
        organization_id: context.organization_id,
        creative_mission_id: context.creative_mission_id,
        creative_project_id: context.creative_project_id,
        mission: context.mission,
