...
async function recoverSettledTribunalResume({ context, project, master, available_capabilities = [] }) {
  ...
  const passingByReviewer = new Map();
  ...
  for (let index = 0; index < settledEntries.length; index += 1) {
    ...
    if (operation === "CREATIVE_DYNAMIC_TRIBUNAL_REPAIR_V1") {
      ...
      for (const [reviewerId, row] of [...passingByReviewer.entries()]) {
        const currentReviewer = reviewerForPlan(reviewerId, replayPlan);
        if (!currentReviewer) {
          passingByReviewer.delete(reviewerId);
          continue;
        }
        ...
        if (currentHash !== row.review_evidence_hash) passingByReviewer.delete(reviewerId);
        else passingByReviewer.set(reviewerId, { ...row, reviewer: currentReviewer });
      }
      continue;
    }
    ...
    const passed = normalizedReview.passed === true && Number(normalizedReview.score) >= requiredFloor && !String(normalizedReview.fatal_rejection_reason || "").trim();
    if (!passed) {
      passingByReviewer.delete(reviewerId);
      continue;
    }
    passingByReviewer.set(reviewerId, {
      reviewer,
  ...
  const settledReviews = [...passingByReviewer.entries()].flatMap(([reviewerId, row]) => {
    const reviewer = reviewerForPlan(reviewerId, replayPlan);
    if (!reviewer) return [];
    const currentHash = CreativeDynamicTribunalRuntime.reviewEvidenceHash({
      reviewer,
