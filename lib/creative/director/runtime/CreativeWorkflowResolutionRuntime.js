...
async function reviewWithDurableResume(context = {}, reviewInput = {}) {
  ...
  if (approvedCheckpoint) {
    const currentPlan = reviewInput.master?.plan || {};
    const approvedPlanHash = CreativeDynamicTribunalRuntime.reviewPlanHash(approvedCheckpoint.plan);
    const currentPlanHash = CreativeDynamicTribunalRuntime.reviewPlanHash(currentPlan);
    if (approvedPlanHash === currentPlanHash) {
      return restoreTribunalLineage(approvedCheckpoint, reviewInput.master);
    }
...
export const CreativeWorkflowResolutionRuntime = Object.freeze({
  ...
  async resolve(input = {}) {
    ...
    let temporalCheckpoint = forceDirectionRestart
      ? null
      : storedTemporalDirectionCheckpoint(context.project, context);
    if (
      temporalCheckpoint &&
      masterStoryRequired(context) &&
      !completeMasterStory(temporalCheckpoint.plan)
    ) {
      temporalCheckpoint = null;
    }
