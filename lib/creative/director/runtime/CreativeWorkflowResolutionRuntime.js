...
async function bootstrapResolvedProductionRooms({ context, project, brief, researched = {}, master }) {
  ...
  if (
    ...
    resolvedMaster = await CreativeUniversalTemporalDirectionRuntime.create({
      organization_id: context.organization_id,
      mission: context.mission,
      project,
      brief,
      assets: researched?.assets || context.assets || [],
      approved_master: resolvedMaster,
    });
  }
...
export const CreativeWorkflowResolutionRuntime = Object.freeze({
  ...
  async resolve(input = {}) {
    ...
    const lineageRecovery = forceDirectionRestart
      ? null
      : storedStoryLineageRecoveryCheckpoint(context.project, context);
    if (lineageRecovery) {
      return CreativeWorkflowResolutionRuntime.resumeApprovedCouncil({
        ...input,
        approved_master: lineageRecovery,
