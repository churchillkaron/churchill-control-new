...
export const CreativeWorkflowResolutionRuntime = Object.freeze({
  async resolve(input = {}) {
    ...
    const directionProject = learned.project || constrainedProject;
    ...
    const master = await CreativeDynamicTribunalRuntime.review({
      ...
      available_capabilities:
        initialMaster.available_production_capabilities || [],
    ...
    if (declared && workflow.workflow_kind !== declared.workflow_kind) {
      throw new Error(
        `CREATIVE_WORKFLOW_CONSTRAINT_MISMATCH:` +
        `${declared.workflow_kind}:${workflow.workflow_kind}`,
      );
    }
