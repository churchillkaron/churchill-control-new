...
function policyInput(input = {}) {
  ...
  const existingProjectPolicy = object(
    project.metadata?.creative_intelligence_policy,
  );
  const existingBriefPolicy = object(
    brief.metadata?.creative_intelligence_policy,
  );
  const policy = {
    ...
    critic_minimums: {
      ...
      ...object(existingProjectPolicy.critic_minimums),
      ...object(existingBriefPolicy.critic_minimums),
    },
  };
...
function universalConceptGate(plan = {}) {
...
function councilGate(result = {}) {
  ...
  const failedCritics = Object.entries(
    WORLD_CLASS_CONCEPT_POLICY.critic_minimums,
  )
    .filter(([id, minimum]) => {
  ...
  if (
    ...
    weightedScore === null ||
    weightedScore < WORLD_CLASS_CONCEPT_POLICY.minimum_weighted_score
  ) {
    throw new Error(
  ...
  if (
    confidence === null ||
    confidence < WORLD_CLASS_CONCEPT_POLICY.minimum_selector_confidence
  ) {
  ...
  const excessiveSimilarity = pairwise.find((item) =>
    ...
    finite(item.similarity) > WORLD_CLASS_CONCEPT_POLICY.maximum_pairwise_similarity,
  );
  ...
  if (list(selected.signature_images).length < WORLD_CLASS_CONCEPT_POLICY.minimum_signature_images) {
    selectedFailures.push("signature_images");
  }
  if (list(selected.campaign_extensions).length < WORLD_CLASS_CONCEPT_POLICY.minimum_campaign_extensions) {
    selectedFailures.push("campaign_extensions");
  }
  if (list(selected.anti_cliche_rules).length < WORLD_CLASS_CONCEPT_POLICY.minimum_anti_cliche_rules) {
  ...
  return {
    ...
    critic_minimums: WORLD_CLASS_CONCEPT_POLICY.critic_minimums,
    maximum_pairwise_similarity:
      WORLD_CLASS_CONCEPT_POLICY.maximum_pairwise_similarity,
    b_grade_concept_forbidden: true,
  };
}
...
function enforceResult(result = {}) {
  ...
  const baseline = universalConceptGate(result.plan);
  const council = councilGate(result);
  const gate = {
    ...baseline,
...
function installTemporalGate() {
  ...
  CreativeUniversalTemporalDirectionRuntime.create = async function createWithWorldClassConcept(input = {}) {
    return enforceResult(await create(policyInput(input)));
  };
}
