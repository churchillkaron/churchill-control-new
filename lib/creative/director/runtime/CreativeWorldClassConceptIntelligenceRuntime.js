...
function councilGate(result = {}, policy = WORLD_CLASS_CONCEPT_POLICY) {
  ...
  // Challenger selection is a different governance contract from the independent
  // concept council. It carries a selected concept and selector lineage but does not
  // own critic scorecards. The universal concept gate and downstream Tribunal remain
  // authoritative for challenger-selected temporal work; applying the legacy council
  // scorecard contract here creates a false CREATIVE_WORLD_CLASS_CONCEPT_SELECTION_REQUIRED.
  if (text(council.contract) === "CREATIVE_STUDIO_CHALLENGER_SELECTION_V1") return null;
