...
function campaignExtensionEvidence(result = {}, selected = {}, policy = WORLD_CLASS_CONCEPT_POLICY) {
  ...
  const channels = unique(
    list(result.plan?.deliverables).flatMap((deliverable) => list(deliverable?.channels)),
  );
  const derived = [];
  if (
  ...
  return { authored, derived, effective: unique([...authored, ...derived]) };
}
...
function councilGate(result = {}, policy = WORLD_CLASS_CONCEPT_POLICY) {
  ...
  if (campaignExtensions.effective.length < policy.minimum_campaign_extensions) {
