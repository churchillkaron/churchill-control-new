...
const NON_EMPTY_REQUIRED_ARRAY_PATHS = new Set([
...
// clears explicitly.
function mergeArray(baseValue, repairedArray, { preserveUnmentioned = false, preserveRequiredNonEmpty = false, path = [] } = {}) {
  if (!repairedArray.length) {
    if (preserveRequiredNonEmpty && Array.isArray(baseValue) && baseValue.length) return baseValue;
  ...
  if (preserveUnmentioned) {
    ...
    if (repairEntries.every(({ key }) => Boolean(key))) {
      ...
      for (const { entry, key } of repairEntries) {
        ...
        if (isObject(matched)) {
          patches.set(key, mergeCreativeRepairedPlan(matched, entry, path));
        } else {
          additions.push(entry);
        }
      }

      const coversEntireBase =
        additions.length === 0 &&
        repairEntries.length === base.length &&
        repairEntries.every(({ key }) => byIdentity.has(key));
      if (coversEntireBase) {
        return repairEntries.map(({ key }) => patches.get(key) || byIdentity.get(key));
      }
  ...
  return repairedArray.map((entry, index) => {
    ...
    return isObject(matched) ? mergeCreativeRepairedPlan(matched, entry, path) : entry;
  });
}

export function mergeCreativeRepairedPlan(basePlan = {}, repairedPlan = {}, path = []) {
  ...
  for (const [key, repairedValue] of Object.entries(repairedPlan)) {
    ...
    if (Array.isArray(repairedValue)) {
      ...
      merged[key] = mergeArray(baseValue, repairedValue, {
        ...
        preserveRequiredNonEmpty: NON_EMPTY_REQUIRED_ARRAY_PATHS.has(nextPath.join(".")),
        path: nextPath,
      });
      continue;
    }
    ...
    if (isObject(repairedValue) && isObject(baseValue)) {
      merged[key] = mergeCreativeRepairedPlan(baseValue, repairedValue, [...path, key]);
      continue;
    }
