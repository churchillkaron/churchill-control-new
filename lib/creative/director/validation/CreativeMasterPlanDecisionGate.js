...
function validateConceptDepth(plan, failures, { require_temporal_direction = true } = {}) {
  ...
  if (require_temporal_direction && text(plan.workflow_kind).toUpperCase() === "TEMPORAL") {
...
// Filler was scored but never rejected, so the repair loop was never told about it. A plan saying
// "seamlessly integrated mix" passed here untouched and then lost 21 points and failed outright at
// scoring. Telling the director not to use these words was tried twice and did not work; rejecting the
// plan hands the problem to the repair paths, which do work when the gate names what is wrong.
//
// The threshold matches the benchmark's own hard failure: three uses is where it stops being a slip.
function validateAdvertisingFiller(plan, failures) {
  const hits = advertisingFillerHits(directionProse(plan));
  const total = hits.reduce((sum, entry) => sum + entry.count, 0);
  if (total < 3) return;

  push(
    failures,
    "ADVERTISING_FILLER_REJECTED",
    "concept",
    "Direction must not lean on advertising filler. Replace each phrase with the observable craft it stands in for.",
    hits.map((entry) => `${entry.id}x${entry.count}`).join(", "),
  );
}

// The prose a reader would actually see, which is where filler lives. Structured fields such as service
// ids and output specs are excluded so a capability name can never read as a cliché.
function directionProse(plan = {}) {
  const concept = object(plan.concept);
  const story = object(plan.story);
  const review = object(plan.creative_review);
  return [
    concept.title, concept.creative_thesis, concept.hook, concept.message, concept.narrative,
    concept.creative_system, concept.emotional_promise, concept.call_to_action,
    typeof concept.target_audience === "string" ? concept.target_audience : "",
    story.hook, story.audience_tension, story.escalation, story.observable_proof,
    story.turn, story.resolution, story.call_to_action, story.emotional_arc,
    story.anti_cliche_strategy,
    review.selected_direction_reason, review.weakest_link,
    ...list(review.rejected_patterns).map(text),
    ...list(review.craft_risks).map(text),
    ...list(review.finishing_requirements).map(text),
    ...list(plan.deliverables).map((entry) => text(entry?.purpose)),
    ...list(plan.scenes).flatMap((scene) => [text(scene?.objective), text(scene?.state_change)]),
  ]
    .map(text)
    .filter(Boolean)
    .join("\n");
...
function validateSemanticMissionContract(plan, failures) {
  ...
  const masterStory = object(architecture.master_story);
  const chapters = list(masterStory.chapters);
  if (!Object.keys(masterStory).length || chapters.length === 0) {
    push(
      failures,
      "CREATIVE_MASTER_STORY_REQUIRED",
      "story_architecture.master_story",
      "The semantic mission requires a complete master story before production planning can continue",
      contract.master_story_duration_minutes || true,
    );
    return;
  }
  ...
  requireDepth(
    failures,
    masterStory.beginning,
    "story_architecture.master_story.beginning",
    50,
    "CREATIVE_MASTER_STORY_BEGINNING_TOO_SHALLOW",
    "The complete master story requires a substantive beginning that establishes the larger narrative, not only the currently generated excerpt",
  );
  requireDepth(
    failures,
    masterStory.middle,
    "story_architecture.master_story.middle",
    50,
    "CREATIVE_MASTER_STORY_MIDDLE_TOO_SHALLOW",
    "The complete master story requires a substantive middle that develops the larger narrative beyond the current excerpt",
  );
  requireDepth(
    failures,
    masterStory.payoff,
    "story_architecture.master_story.payoff",
    50,
    "CREATIVE_MASTER_STORY_PAYOFF_TOO_SHALLOW",
    "The complete master story requires an earned final payoff for the larger work",
  );

  const concept = object(object(plan.common_plan_contract).concept || plan.concept);
  const story = object(object(plan.common_plan_contract).story || plan.story);
  const scenes = list(object(plan.common_plan_contract).scenes || plan.scenes);
  const roles = object(object(plan.common_plan_contract).role_decisions || plan.role_decisions);
  const activeTechnicalAuthorship = [
    "graphic_design_director",
    "motion_design_director",
    "vfx_director",
    "technical_architect",
    "art_director",
  ].some((role) => text(object(roles[role]).status).toUpperCase() === "ACTIVE");
  const sceneTechnicalLanguage = scenes.some((scene) => {
    const current = object(scene);
    const sceneText = depthText([
      current.visual_style,
      current.graphics,
      current.vfx,
      current.production_design,
      current.shots,
    ]).toLowerCase();
    return /(graph|node|signal|data structure|neural|memory|reasoning|decision|circuit|architecture|particle|mathematical|algorithm|intelligence|processing)/i.test(sceneText);
  });
  const directionText = depthText([concept, story, scenes]).toLowerCase();
  const mechanismHiddenByDesign = /(intelligence|mechanism|system|brain|ai).{0,120}(never shown|not shown|remain(?:s)? invisible|never seen)|unseen hand|no (?:vfx|graphics|motion graphics|synthetic visuals)/i.test(directionText);

  if (
    contract.core_mechanism_dramatization_required === true &&
    (mechanismHiddenByDesign || (!activeTechnicalAuthorship && !sceneTechnicalLanguage))
  ) {
    push(
      failures,
      "SEMANTIC_CORE_MECHANISM_NOT_DRAMATIZED",
      "common_plan_contract.concept.creative_system",
      "The mission requires the causal core to become perceptible story material; an invisible-by-design mechanism or effects-only proof is insufficient",
      concept.creative_system || concept.signature_device || null,
    );
  }

  if (
    contract.technology_ai_future_required === true &&
    !activeTechnicalAuthorship &&
    !sceneTechnicalLanguage
  ) {
    push(
      failures,
      "SEMANTIC_TECHNOLOGY_FUTURE_LANGUAGE_MISSING",
      "role_decisions",
      "The mission explicitly requires technology, intelligence and future-facing authored language; all technical visual authorship cannot be marked not required",
      Object.fromEntries([
        "graphic_design_director",
        "motion_design_director",
        "vfx_director",
        "technical_architect",
      ].map((role) => [role, object(roles[role]).status || null])),
    );
  }

  if (contract.universe_scale_required === true) {
    const requestedChapter = chapters.find(
      (chapter) => Number(chapter?.chapter_number) === Number(contract.chapter_number || 1),
    ) || chapters[0] || {};
    const excerptScaleText = depthText([requestedChapter, scenes]).toLowerCase();
    if (!/(universe|cosmic|space|orbit|planet|earth|moon|stars|galaxy|planetary)/i.test(excerptScaleText)) {
      push(
        failures,
        "SEMANTIC_UNIVERSE_SCALE_MISSING",
        "story_architecture.master_story.chapters",
        "Universe-scale visual language is required in the requested excerpt, not deferred to a later chapter",
        requestedChapter,
      );
    }
  }
...
export function validateCreativeMasterPlanDecision({
  ...
  require_temporal_direction = true,
} = {}) {
  const failures = [];
  const normalized = object(plan);

  validateConceptDepth(normalized, failures, { require_temporal_direction });
  if (require_temporal_direction) validateAdvertisingFiller(normalized, failures);
  const thresholds = validateCreativeReview(normalized, failures);
  validateSemanticMissionContract(normalized, failures);
  ...
  return {
    ...
    temporal_direction_required: require_temporal_direction === true,
    failures,
  };
}
