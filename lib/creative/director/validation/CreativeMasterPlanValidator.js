...
export function assertCreativeMasterPlan(input = {}) {
  ...
  if (!validation.passed) {
    ...
    // Codes alone could not be acted on: the same three codes appeared run after
    // run while the offending values differed, so each diagnosis needed another
    // paid reasoning call. The field and rejected value are now in the message,
    // and error.validation still carries the full detail.
    const detail = validation.failures
      ...
      .map((item) => {
        ...
        return `${item.code}@${path}${evidence}`;
      })
      .join("; ");
