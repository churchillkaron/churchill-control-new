...
function validateShot(shot, sceneIndex, shotIndex, failures) {
  ...
  requireText(failures, generation.provider_prompt, `${base}.generation.provider_prompt`, { minimum: 120 });
  requireText(failures, generation.negative_prompt, `${base}.generation.negative_prompt`, { minimum: 40 });
  if (!Object.keys(object(generation.output_spec)).length) {
    push(
      failures,
