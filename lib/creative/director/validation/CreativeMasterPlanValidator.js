...
import {
  eliteFilmDepartmentActivationFailures,
} from "@/lib/creative/director/runtime/CreativeEliteFilmDepartmentActivationRuntime.js";
...
const MANDATORY_ACTIVE_GOVERNANCE_ROLES = new Set([
  "quality_director",
  "rights_safety_director",
  "release_director",
]);

const MANDATORY_ACTIVE_ROLES_BY_WORKFLOW = Object.freeze({
  STILL: new Set([
...
function roleMustBeActive(roleId, workflowKind) {
  return MANDATORY_ACTIVE_GOVERNANCE_ROLES.has(roleId) ||
    MANDATORY_ACTIVE_ROLES_BY_WORKFLOW[workflowKind]?.has(roleId) === true;
}

function validateRoleDecisions(plan, workflowKind, failures) {
  const decisions = object(plan.role_decisions);
  for (const role of CREATIVE_MASTER_PLAN_ROLES) {
    ...
    if (status === "NOT_REQUIRED") {
      if (roleMustBeActive(role.id, workflowKind)) {
        push(
          failures,
          "AGENCY_ROLE_ACTIVE_REQUIRED",
          `role_decisions.${role.id}.status`,
          `Role ${role.id} is mandatory for ${workflowKind} work and cannot be waived`,
          status,
        );
      }
      requireText(
    ...
    const normalizedDecision = text(decision.decision).toLowerCase().replace(/\s+/g, " ");
    const normalizedMandate = text(role.mandate).toLowerCase().replace(/\s+/g, " ");
    if (normalizedDecision && normalizedDecision === normalizedMandate) {
      push(
        failures,
        "AGENCY_ROLE_DECISION_MANDATE_ECHO",
        `role_decisions.${role.id}.decision`,
        `Active role ${role.id} must make a mission-specific decision rather than repeat its registry mandate`,
        decision.decision,
      );
    }
    const roleEvidence = list(decision.evidence).map(text).filter(Boolean);
    ...
    } else if (roleEvidence.every((entry) => /(?:role|discipline).*(?:required|mandated).*(?:contract|project)|(?:contract|project).*(?:requires|required).*(?:role|discipline)/i.test(entry))) {
      push(
        failures,
        "AGENCY_ROLE_EVIDENCE_GENERIC",
        `role_decisions.${role.id}.evidence`,
        `Active role ${role.id} requires concrete mission, research, scene, shot or asset evidence rather than contract-existence evidence`,
        roleEvidence,
      );
    }
...
function validateAssetManifest(plan, assets, failures) {
  ...
  // A manifest entry for an asset that does not exist.
  //
  // This loop walks the supplied ids and asks whether each has an entry, so an entry for an id nobody
  // supplied was never looked at. One film returned thirteen entries of which nine were invented, and
  // only the genuinely missing ones were reported -- the invented nine passed in silence, on both
  // executors. Selected assets going unaccounted and the manifest naming assets that do not exist are
  // two different faults and both are worth saying out loud.
  const selectedIdSet = new Set(selectedIds);
  for (const [id] of entries) {
    if (selectedIdSet.has(id)) continue;
    push(
      failures,
      "MANIFEST_ASSET_UNKNOWN",
      `asset_manifest.${id}`,
      `${id} is not one of the supplied assets and cannot have a manifest entry. Use only the exact supplied ids -- a checksum, a hashed storage path or a file name is not an asset id.`,
      id,
    );
  }
  ...
  for (const id of selectedIds) {
    ...
    if (
      ...
      push(
        ...
        `Asset ${id} is dispositioned ${disposition} but names no deliverable it serves. Every asset that is not EXCLUDE must list the id of each deliverable it is used in, including a REFERENCE asset, which names the deliverable it informs rather than the one it appears in.`,
      );
    }
  }
...
function validateAssignedAssetProductionCoverage(plan, failures) {
  const referenced = new Set();
  for (const deliverable of list(plan.deliverables)) {
    for (const step of list(deliverable.production_steps)) {
      const ids = [
        step?.output_spec?.source_asset_id,
        step?.requirements?.source_asset_id,
  ...
  for (const scene of list(plan.scenes)) {
    for (const shot of list(scene?.shots)) {
      const ids = [
        ...
        ...list(shot?.reference_assets).map((entry) => entry?.asset_id || entry?.id),
      ].map(text).filter(Boolean);
      for (const id of ids) referenced.add(id);
    }
  }
  ...
  for (const entry of list(plan.asset_manifest)) {
    ...
    if (!id || text(entry?.disposition).toUpperCase() !== "ASSIGNED") continue;
    if (referenced.has(id)) continue;
    push(
      failures,
      "ASSIGNED_ASSET_PRODUCTION_STEP_REQUIRED",
      `asset_manifest.${id}`,
      `Assigned asset ${id} is not consumed by any production step. Every ASSIGNED asset must be bound to at least one governed production step via source_asset_id.`,
      id,
    );
  }
}
...
function validateStory(plan, failures) {
  ...
  const emotionalArc = text(story.emotional_arc);
  const emotionalStates = emotionalArc
    .split(/(?:→|->|=>|;|\bthen\b|\bto\b)/i)
    .map(text)
    .filter((entry) => entry.length >= 3);
  if (emotionalStates.length < 4) {
    push(
      failures,
      "STORY_EMOTIONAL_ARC_TOO_FLAT",
      "story.emotional_arc",
      "Premium temporal stories require at least four distinct felt states with causal progression; a short mood label or generic curiosity-to-confidence arc is emotionally underwritten.",
      { emotional_state_count: emotionalStates.length, emotional_arc: emotionalArc },
    );
  }

  const emotionalCore = [
    story.human_desire,
    story.emotional_contradiction,
    story.vulnerability_or_risk,
    story.empathy_path,
    story.emotional_payoff,
  ].map(text).join(" ").toLowerCase();
  const genericEmotion = /^(?:emotion|emotional|humanity|connection|inspiration|awe|confidence|trust|hope|curiosity|excitement|wonder|belonging)[ .,!-]*$/i;
  for (const [field, value] of Object.entries({
    human_desire: story.human_desire,
    emotional_contradiction: story.emotional_contradiction,
    vulnerability_or_risk: story.vulnerability_or_risk,
    empathy_path: story.empathy_path,
    emotional_payoff: story.emotional_payoff,
  })) {
    if (genericEmotion.test(text(value))) {
      push(
        failures,
        "STORY_GENERIC_EMOTION_REJECTED",
        `story.${field}`,
        `${field} must describe a specific human cause, conflict, consequence or earned feeling rather than a generic emotion label.`,
        value,
      );
    }
  }
  if (!/(because|when|after|until|but|yet|while|so that|therefore|as |through|by )/.test(emotionalCore)) {
    push(
      failures,
      "STORY_EMOTION_CAUSALITY_REQUIRED",
      "story",
      "The emotional core names states but not enough causality. Connect desire, contradiction, vulnerability, empathy and payoff to observable events or consequences in the story.",
    );
  }
}

function validateShotSourceBinding(shot, base, failures, allowedAssetIds = null) {
  ...
  //
  // This check verified that a shot's ids agreed with each other and never that they were real, so an
  // invented id passed as long as it was self-consistent. A film put a 64-character sha256 in
  // primary_source_asset_id and as an AUDIO_REFERENCE -- a content hash lifted from the asset payload,
  // where real ids sit beside hash-shaped technical fields -- and the only complaint was that it did
  // not match the PRIMARY_SOURCE entry. The reported error was a mismatch; the actual error was that
  // the asset did not exist, which is the one worth saying.
  if (allowedAssetIds) {
    const seen = new Set();
    for (const [index, reference] of references.entries()) {
      ...
      if (!allowedAssetIds.has(id)) {
        push(
          failures,
          "SHOT_ASSET_REFERENCE_UNKNOWN",
          `${base}.reference_assets.${index}.asset_id`,
          `${id} is not one of the supplied assets. Use only the exact ids in SUPPLIED ASSET IDS -- a hash, filename or any other value from an asset's payload is not its id.`,
          id,
        );
      }
    }
    if (primarySourceId && !allowedAssetIds.has(primarySourceId)) {
      push(
        failures,
        "SHOT_ASSET_REFERENCE_UNKNOWN",
        `${base}.primary_source_asset_id`,
        `${primarySourceId} is not one of the supplied assets. Use only the exact ids in SUPPLIED ASSET IDS -- a hash, filename or any other value from an asset's payload is not its id.`,
        primarySourceId,
      );
    }
  }
...
// Camera fields that describe a property of another camera field, and so have nothing to say when that
// other field reports there is nothing to describe.
//
// movement_speed is the speed of the movement in movement_path. A locked-off shot answered
// movement_path with "locked off on tripod, no movement", stabilization with "locked tripod" and
// movement_motivation with "stillness supports emotional gravity" -- it stated the absence as a
// decision exactly as asked -- and was then required to write prose about the speed of a movement that
// does not exist. "none" is the only truthful answer available, and no wording of the guidance can fix
// a field with nothing in it. This was the last surviving instance of that failure after the guidance
// had cleared every other one.
//
// The dependency is narrow on purpose: the parent must carry real direction, so a shot cannot answer
// both with a blank. It does leave one gap I am choosing to accept -- a shot describing a real dolly
// move could report no speed for it and pass -- because the parent has then described the move, and
// trading a rare unstated speed for a systematic false rejection of every still frame is the better
// side of that bargain.
const DEPENDENT_CAMERA_FIELDS = Object.freeze({ movement_speed: "movement_path" });
...
function validateShot(shot, sceneIndex, shotIndex, failures, quality = {}, allowedAssetIds = null) {
  ...
  const firstPassIntent = object(shot.first_pass_intent);
  for (const [field, minimum] of [
    ["story_delta", 20],
    ["visible_event", 20],
    ["edit_reason", 20],
    ["continuity_anchor", 15],
    ["sound_picture_event", 15],
    ["predicted_failure", 20],
  ]) {
    requireText(failures, firstPassIntent[field], `${base}.first_pass_intent.${field}`, { minimum });
  }
  ...
  for (const [section, fields] of Object.entries({
    ...
    for (const field of fields) {
      ...
      // A property of a field that already reported nothing to describe.
      const dependsOn = section === "camera" ? DEPENDENT_CAMERA_FIELDS[field] : null;
      if (
        dependsOn &&
        statedAbsence(source[field]) &&
        text(source[dependsOn]).length >= 5 &&
        !statedAbsence(source[dependsOn])
      ) continue;
  ...
  const energyLevel = finite(shot.energy_level);
  if (energyLevel === null || energyLevel < 0 || energyLevel > 100) {
    push(failures, "SHOT_ENERGY_LEVEL_REQUIRED", `${base}.energy_level`, "Temporal Studio shots require energy_level from 0 to 100 so picture, score and SFX share one intensity curve", shot.energy_level);
  }
  const tempoRole = text(shot.tempo_role).toUpperCase();
  if (!["HOLD", "BUILD", "ACCELERATE", "PEAK", "RELEASE", "SILENCE"].includes(tempoRole)) {
    push(failures, "SHOT_TEMPO_ROLE_REQUIRED", `${base}.tempo_role`, "tempo_role must be HOLD, BUILD, ACCELERATE, PEAK, RELEASE or SILENCE", shot.tempo_role);
  }
  const syncEvents = list(audio.sync_events);
  if (syncEvents.some((entry) => finite(entry?.at_seconds) === null || !text(entry?.event))) {
    push(failures, "SHOT_AUDIO_SYNC_EVENT_INVALID", `${base}.audio.sync_events`, "Each sync event requires an exact at_seconds value and a physically motivated audible event");
  }
  ...
  validateShotSourceBinding(shot, base, failures, allowedAssetIds);
...
export function deriveTemporalCadenceContract(plan = {}, scenesInput = null) {
  const scenes = Array.isArray(scenesInput) ? scenesInput : list(plan.scenes);
  const shots = scenes.flatMap((scene) => list(scene.shots));
  const declaredDuration = finite(plan.temporal_contract?.duration_seconds) ??
    finite(list(plan.deliverables)[0]?.output_spec?.duration_seconds) ??
    shots.reduce((sum, shot) => sum + Math.max(0, finite(shot.duration_seconds) || 0), 0);
  const language = [
    ...
    ...scenes.flatMap((scene) => [scene.objective, scene.emotion, scene.transition_logic]),
  ].map(text).join(" ").toLowerCase();
  const deliberateLongTake = Boolean(
    plan.temporal_contract?.long_take_strategy?.intentional === true ||
    plan.story?.long_take_strategy?.intentional === true ||
  ...
  const premiumTemporal = Boolean(
    /PREMIUM|WORLD_CLASS|FLAGSHIP|ELITE/.test(text(plan.quality_profile).toUpperCase()) ||
    plan.production?.temporal_quality?.premium === true ||
    plan.semantic_benchmark_floor?.world_class === true ||
    plan.world_class_concept_intelligence?.passed === true
  );
  let technicalMaximumAverageShotSeconds = 4;
  if (patient) technicalMaximumAverageShotSeconds = 5.5;
  if (tensionDriven) technicalMaximumAverageShotSeconds = Math.min(technicalMaximumAverageShotSeconds, 3.75);
  if (kinetic) technicalMaximumAverageShotSeconds = Math.min(technicalMaximumAverageShotSeconds, 2.5);
  if (deliberateLongTake) technicalMaximumAverageShotSeconds = Math.max(technicalMaximumAverageShotSeconds, 15);
  let authoredCoverageAverageSeconds = premiumTemporal ? 2.75 : technicalMaximumAverageShotSeconds;
  if (premiumTemporal && patient) authoredCoverageAverageSeconds = 3.5;
  if (premiumTemporal && tensionDriven) authoredCoverageAverageSeconds = Math.min(authoredCoverageAverageSeconds, 2.5);
  if (premiumTemporal && kinetic) authoredCoverageAverageSeconds = Math.min(authoredCoverageAverageSeconds, 1.75);
  if (deliberateLongTake) authoredCoverageAverageSeconds = technicalMaximumAverageShotSeconds;
  const technicalMinimumShotCount = declaredDuration > 0
    ? Math.max(1, Math.ceil(declaredDuration / technicalMaximumAverageShotSeconds))
    : 1;
  const authoredCoverageFloorCount = declaredDuration > 0
    ? Math.max(1, Math.ceil(declaredDuration / authoredCoverageAverageSeconds))
    : 1;
  const minimumShotCount = Math.max(technicalMinimumShotCount, authoredCoverageFloorCount);
  const maximumAverageShotSeconds = authoredCoverageAverageSeconds;
  return {
    contract: "CREATIVE_TEMPORAL_CADENCE_CONTRACT_V1",
    duration_seconds: declaredDuration || 0,
    scene_count: scenes.length,
    shot_count: shots.length,
    minimum_shot_count: minimumShotCount,
    technical_minimum_shot_count: technicalMinimumShotCount,
    authored_coverage_floor_count: authoredCoverageFloorCount,
    authored_coverage_average_seconds: authoredCoverageAverageSeconds,
    premium_temporal: premiumTemporal,
    maximum_average_shot_seconds: maximumAverageShotSeconds,
    actual_average_shot_seconds: shots.length && declaredDuration > 0
      ? Number((declaredDuration / shots.length).toFixed(3))
      : null,
    pacing_signals: { patient, tension_driven: tensionDriven, kinetic },
    deliberate_long_take: deliberateLongTake,
    narrative_movements_are_not_shots: true,
  };
}

function validateTemporalCadence(plan, scenes, failures) {
  const cadence = deriveTemporalCadenceContract(plan, scenes);
  if (cadence.duration_seconds < 20 || cadence.deliberate_long_take) return cadence;
  if (cadence.shot_count < cadence.minimum_shot_count) {
    push(
      failures,
      "TEMPORAL_SHOT_CADENCE_TOO_SPARSE",
      "scenes",
      `This ${cadence.duration_seconds.toFixed(3)} second film has ${cadence.shot_count} shots, but its authored pacing/tension requires at least ${cadence.minimum_shot_count}. Narrative movements and scenes are not camera shots; expand each movement into purposeful visual coverage with withholding, contrast, escalation and payoff rather than stretching a few generated clips.`,
      cadence,
    );
  }
  if (
    cadence.duration_seconds >= 45 &&
    cadence.scene_count > 0 &&
    cadence.scene_count <= 4 &&
    cadence.shot_count <= cadence.scene_count
  ) {
    push(
      failures,
      "TEMPORAL_NARRATIVE_MOVEMENT_SHOT_CONFLATION",
      "scenes",
      "The plan maps each high-level scene/movement to a single camera shot. Premium temporal direction must separate narrative movement, scene and shot layers so tension can build through editorial progression.",
      cadence,
    );
  }
  return cadence;
...
function validateDynamicTemporalRhythm(plan, scenes, failures) {
  if (plan.temporal_contract?.dynamic_rhythm_required !== true) return;
  const shots = scenes.flatMap((scene) => list(scene.shots));
  if (shots.length < 6) return;
  const durations = shots.map((shot) => finite(shot.duration_seconds)).filter((value) => value !== null && value > 0);
  const energies = shots.map((shot) => finite(shot.energy_level)).filter((value) => value !== null);
  const roles = shots.map((shot) => text(shot.tempo_role).toUpperCase()).filter(Boolean);
  const masterDuration = finite(plan.temporal_contract?.duration_seconds) || durations.reduce((sum, value) => sum + value, 0);
  if (masterDuration < 20 || durations.length !== shots.length) return;
  ...
  for (let index = 1; index < durations.length; index += 1) {
    if (Math.abs(durations[index] - durations[index - 1]) <= 0.12) run += 1;
    else run = 1;
    maxRun = Math.max(maxRun, run);
  }
  if (bands.size < 3 || cv < 0.16 || maxRun > 3) {
    push(failures, "TEMPORAL_RHYTHM_TOO_UNIFORM", "scenes", "Premium temporal work requires visibly varied shot duration and cannot run as a metronomic chain of equal-length shots", { duration_bands: bands.size, coefficient_of_variation: Number(cv.toFixed(3)), maximum_uniform_run: maxRun });
  }
  if (energies.length === shots.length) {
    const spread = Math.max(...energies) - Math.min(...energies);
    if (spread < 25) push(failures, "TEMPORAL_ENERGY_CURVE_TOO_FLAT", "scenes", "Premium temporal work requires a real felt energy curve with meaningful dynamic contrast", { energy_spread: spread });
  }
  if (!roles.includes("ACCELERATE") || !roles.some((role) => role === "RELEASE" || role === "SILENCE")) {
    push(failures, "TEMPORAL_TEMPO_CONTRAST_REQUIRED", "scenes", "Films longer than 20 seconds require at least one acceleration and one release or silence state");
  }
}

function validateTemporalPlan(plan, failures, allowedAssetIds = null) {
  ...
  // The declared length is a contract, not a form choice.
  //
  // temporal_contract announced scene_duration_sum_must_equal_master and exact_duration_required, and
  // nothing checked either one -- the block was metadata. A full-song film came back 66.26 seconds
  // against a 205.16 second song, two scenes and five shots where the director had produced eight and
  // thirty-nine, and passed with zero structural failures. The tribunal's repair had replaced the
  // scenes array with an abbreviated one (its own rules say any array returned replaces the array in
  // full) and the loss was invisible because no rule compared the result to the length it owed.
  //
  // The master duration is read from the deliverable output spec as well as the temporal contract,
  // because the same truncation that drops scenes can drop the contract block that would have caught
  // it, and the output spec is required independently.
  const declaredDuration = finite(plan.temporal_contract?.duration_seconds) ??
    finite(list(plan.deliverables)[0]?.output_spec?.duration_seconds);
  if (declaredDuration && declaredDuration > 0) {
    const sceneSum = scenes.reduce(
      (total, scene) => total + (finite(scene.duration_seconds) || 0),
      0,
    );
    // A tenth of a second of rounding across a film is fair; a missing scene is not.
    if (Math.abs(sceneSum - declaredDuration) > 0.1) {
      push(
        failures,
        "TEMPORAL_SCENE_DURATION_SUM_MISMATCH",
        "scenes",
        `Scene durations sum to ${sceneSum.toFixed(3)} seconds but this work is ${declaredDuration.toFixed(3)} seconds long. Every second of the master must be covered by a scene. If the structure changed, rebalance the scenes to cover the full duration rather than shortening the work.`,
        { scene_duration_sum: Number(sceneSum.toFixed(3)), declared_duration: declaredDuration },
      );
    }
  }

  validateTemporalCadence(plan, scenes, failures);
  validateDynamicTemporalRhythm(plan, scenes, failures);
  for (const code of eliteFilmDepartmentActivationFailures(plan)) {
    const roleId = code.split(":")[1] || "unknown";
    push(
      failures,
      code,
      `role_decisions.${roleId}`,
      `Premium temporal production requires the ${roleId.replaceAll("_", " ")} department to be ACTIVE and authoritative before production planning can proceed.`,
  ...
  scenes.forEach((scene, sceneIndex) => {
    ...
    shots.forEach((shot, shotIndex) => {
      ...
      shotIds.add(shotId);
      validateShot(shot, sceneIndex, shotIndex, failures, object(plan.quality), allowedAssetIds);
    });
  });
}

function validateStillArtDirection(plan, workflow, failures) {
  if (workflow.workflow_kind !== "STILL") return;
  const art = object(plan.art_direction);
  const textFields = [
    "governing_visual_idea",
    "focal_hierarchy",
    ...
    "crop_responsive_rules",
    "production_world_rules",
    "generation_requirements",
    "finishing_requirements",
  ];
  for (const field of textFields) {
    requireText(failures, art[field], `art_direction.${field}`, { minimum: 24 });
  }
  for (const field of ["forbidden_devices", "review_criteria"]) {
    const values = list(art[field]).map(text).filter(Boolean);
    if (values.length < 3) {
      push(
        failures,
        "STILL_ART_DIRECTION_LIST_REQUIRED",
        `art_direction.${field}`,
        `${field} requires at least three concrete inspectable entries`,
        art[field],
      );
    }
  }
...
// fail there.
function approvedSceneLineageFailures(shots = [], scene = {}, sceneIndex = 0) {
  const failures = [];
  const authority = object(scene);
  if (!Object.keys(authority).length) return failures;

  const authorityCorpus = JSON.stringify({
    title: authority.title,
    objective: authority.objective,
    human_stake: authority.human_stake,
    story_state_before: authority.story_state_before,
    state_change: authority.state_change,
    story_state_after: authority.story_state_after,
    transition_logic: authority.transition_logic,
    location: authority.location,
    actors: authority.actors,
    brand_rules: authority.brand_rules,
  }).toLowerCase();
  const shotCorpus = JSON.stringify(list(shots)).toLowerCase();
  const stop = new Set([
    "scene", "story", "state", "approved", "causal", "human", "audience", "world",
    "film", "shot", "change", "transition", "reveal", "environment", "exactly",
    "without", "through", "before", "after", "final", "visible", "physical",
  ]);
  const anchors = [
    ...text(authority.title).toLowerCase().matchAll(/[a-z][a-z0-9-]{3,}/g),
    ...text(authority.state_change).toLowerCase().matchAll(/[a-z][a-z0-9-]{3,}/g),
    ...text(authority.story_state_after).toLowerCase().matchAll(/[a-z][a-z0-9-]{3,}/g),
  ]
    .map((match) => match[0])
    .filter((token) => !stop.has(token));
  const uniqueAnchors = [...new Set(anchors)];
  if (uniqueAnchors.length && !uniqueAnchors.some((token) => shotCorpus.includes(token))) {
    push(
      failures,
      "APPROVED_SCENE_LINEAGE_DRIFT",
      `scenes[${sceneIndex}].shots`,
      "Shot direction abandoned the approved scene authority. Preserve the scene's exact causal subject/location/mechanism; do not substitute a new story world.",
      { approved_scene: authority.title || authority.state_change, required_anchors: uniqueAnchors.slice(0, 12) },
    );
  }

  for (const substitute of ["manager", "office", "dashboard", "sensor", "hologram", "corporate", "operations environment"]) {
    if (shotCorpus.includes(substitute) && !authorityCorpus.includes(substitute)) {
      push(
        failures,
        "APPROVED_SCENE_SUBSTITUTE_WORLD_FORBIDDEN",
        `scenes[${sceneIndex}].shots`,
        `Shot direction invented substitute-world element "${substitute}" that is absent from the approved scene.`,
        substitute,
      );
    }
  }
...
export function creativeTemporalSceneShotFailures({
  ...
  const failures = [];
  // The supplied ids travel in so a scene inventing an asset is caught on its own retry rather than
  // surviving to final assembly, where the reason arrives as a mismatch instead of an invention.
  const allowedAssetIds = assets
    ? new Set(list(assets).map(assetId).filter(Boolean))
    : null;
  list(shots).forEach((shot, shotIndex) => {
    validateShot(shot, sceneIndex, shotIndex, failures, object(quality), allowedAssetIds);
  });
  failures.push(...approvedSceneLineageFailures(shots, approved_scene, sceneIndex));
  return failures;
}

export function validateCreativeMasterPlan({ plan, assets = [], require_temporal_direction = true } = {}) {
  const failures = [];
  ...
  for (const field of [
    ...
    requireText(failures, concept[field], `concept.${field}`, { minimum: field === "title" ? 8 : 15 });
  }

  scanForbiddenKeys(normalized, "", failures);
  const manifest = validateAssetManifest(normalized, assets, failures);
  validateAssignedAssetProductionCoverage(normalized, failures);

  if (workflow) {
    validateRoleDecisions(normalized, workflow.workflow_kind, failures);
    validateStillArtDirection(normalized, workflow, failures);
    validateDeliverables(normalized, workflow, failures);
    validateCampaignSystem(normalized, workflow, failures);
    if (workflow.executor === "TEMPORAL" && require_temporal_direction) {
      validateTemporalPlan(normalized, failures, new Set(manifest.selected_asset_ids));
    }
  }
