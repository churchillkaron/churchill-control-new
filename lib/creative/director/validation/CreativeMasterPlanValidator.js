    ...
    if (status === "NOT_REQUIRED") {
      if (roleMustBeActive(role.id, workflowKind)) {
        push(
          ...
          `Governance role ${role.id} is mandatory and cannot be waived for ${workflowKind} work`,
          status,
        );
      }
      requireText(
...
function validateShot(shot, sceneIndex, shotIndex, failures, quality = {}, allowedAssetIds = null) {
  ...
  for (const code of creativeHeroAssetTruthFailures(shot)) {
    push(
      failures,
      code,
      `${base}.hero_asset_truth`,
      `Hero asset truth failed governed authority check: ${code}`,
      shot.hero_asset_truth || null,
    );
  }
...
export function deriveTemporalCadenceContract(plan = {}, scenesInput = null) {
  ...
  let maximumAverageShotSeconds = 7.5;
  if (patient) maximumAverageShotSeconds = 8.5;
  if (tensionDriven) maximumAverageShotSeconds = Math.min(maximumAverageShotSeconds, 6);
  if (kinetic) maximumAverageShotSeconds = Math.min(maximumAverageShotSeconds, 4.5);
  if (deliberateLongTake) maximumAverageShotSeconds = Math.max(maximumAverageShotSeconds, 15);
  const minimumShotCount = declaredDuration > 0
    ? Math.max(1, Math.ceil(declaredDuration / maximumAverageShotSeconds))
    : 1;
  return {
...
function validateDynamicTemporalRhythm(plan, scenes, failures) {
  ...
  const durationScaleRatio = shortestDuration > 0 ? longestDuration / shortestDuration : 0;
  ...
  if (durationScaleRatio < 1.8) {
    push(failures, "TEMPORAL_DURATION_SCALE_CONTRAST_REQUIRED", "scenes", "Premium temporal work requires meaningful contrast between shorter and longer shots, not only small timing variation", { duration_scale_ratio: Number(durationScaleRatio.toFixed(3)) });
  }
