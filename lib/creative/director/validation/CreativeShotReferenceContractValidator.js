...
function distinctiveLocationEvidence(asset = {}) {
  const analysis = object(asset.analysis);
  const intelligence = object(analysis.intelligence);
  const metadata = object(asset.metadata);
  const evidence = object(
    analysis.location_reference_evidence ||
    intelligence.location_reference_evidence ||
    metadata.location_reference_evidence,
  );
  const anchors = unique([
    list(analysis.location_anchors),
    list(analysis.locationAnchors),
    list(analysis.distinctive_geographic_anchors),
    list(intelligence.location_anchors),
    list(intelligence.distinctive_geographic_anchors),
    list(metadata.distinctive_geographic_anchors),
    list(evidence.observable_anchors),
  ]);
  return Boolean(
    evidence.distinctive_geography_certified === true ||
    evidence.named_location_visually_verified === true ||
    anchors.length > 0
  );
}

function locationEvidence(asset = {}) {
  return distinctiveLocationEvidence(asset);
}

function locationEvidence(asset = {}) {
  return visualLocationEvidence(asset);
}
...
function roleEvidence(asset = {}) {
  return {
    ...
    visually_identifiable_location: visualLocationEvidence(asset),
...
function semanticFitness(asset = {}) {
  const analysis = object(asset.analysis);
  const intelligence = object(analysis.intelligence);
  return object(analysis.semantic_fitness || intelligence.semantic_fitness);
}

function normalizedClaim(value = {}) {
  const claim = object(value);
  return {
    kind: text(claim.kind).toUpperCase(),
    value: text(claim.value),
    audience_must_recognize: claim.audience_must_recognize === true,
  };
}

function claimMatches(expected, observed) {
  const left = text(expected).toLowerCase();
  const right = text(observed).toLowerCase();
  return Boolean(left && right && (left === right || left.includes(right) || right.includes(left)));
}

function validateSemanticClaim({ row, asset, path, failures }) {
  const claim = normalizedClaim(row.raw?.semantic_claim);
  if (!claim.kind && !claim.value) return;
  if (!claim.kind || !claim.value) {
    push(failures, "SHOT_REFERENCE_SEMANTIC_CLAIM_INVALID", `${path}.semantic_claim`, "Semantic claim requires both kind and value", claim);
    return;
  }
  const fitness = semanticFitness(asset);
  if (claim.kind === "LOCATION") {
    const visible = object(fitness.visually_identifiable_location);
    if (visible.identifiable !== true || Number(visible.confidence || 0) < 85 || !claimMatches(claim.value, visible.name)) {
      push(failures, "SHOT_REFERENCE_LOCATION_CLAIM_NOT_VISUALLY_PROVEN", `${path}.semantic_claim`, "Location provenance or generic scenery cannot prove an audience-facing location claim", { claim, visible });
    }
  }
  if (claim.kind === "PROFESSION") {
    const profession = object(fitness.visibly_supported_profession);
    if (profession.identifiable !== true || Number(profession.confidence || 0) < 85 || !claimMatches(claim.value, profession.profession)) {
      push(failures, "SHOT_REFERENCE_PROFESSION_CLAIM_NOT_VISUALLY_PROVEN", `${path}.semantic_claim`, "A company source or generic office cannot prove an audience-facing profession claim", { claim, profession });
    }
  }
}
...
function validateShot({ shot, sceneIndex, shotIndex, assetById, failures }) {
  ...
  for (const [referenceIndex, row] of rows.entries()) {
    ...
    validateSemanticClaim({ row, asset, path, failures });
  }
