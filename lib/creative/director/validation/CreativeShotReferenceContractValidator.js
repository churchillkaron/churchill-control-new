...
function visualLocationEvidence(asset = {}) {
  const analysis = object(asset.analysis);
  const intelligence = object(analysis.intelligence);
  const fitness = object(analysis.semantic_fitness || intelligence.semantic_fitness);
  const visible = object(fitness.visually_identifiable_location);
  const anchors = object(analysis.location_anchors || intelligence.location_anchors);
  return Boolean(
    visible.identifiable === true &&
    text(visible.name) &&
    Number(visible.confidence || 0) >= 85 &&
    (list(visible.anchors).length || list(anchors.landmarks).length)
  );
}
