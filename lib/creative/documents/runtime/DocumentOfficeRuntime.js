import { createStoredZip, xmlEscape } from "./OpenXmlPackageRuntime";

function paragraph(value, style = null) {
  const styleXml = style ? `<w:pPr><w:pStyle w:val="${style}"/></w:pPr>` : "";
  return `<w:p>${styleXml}<w:r><w:t xml:space="preserve">${xmlEscape(value)}</w:t></w:r></w:p>`;
}

export function renderDocumentDocx(document = {}) {
  const body = [
    paragraph(document.title, "Title"),
    document.subtitle ? paragraph(document.subtitle, "Subtitle") : "",
    ...(document.sections || []).flatMap((section) => [
      paragraph(section.heading, "Heading1"),
      ...(section.paragraphs || []).map((value) => paragraph(value)),
      ...(section.bullets || []).map((value) =>
        `<w:p><w:pPr><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr></w:pPr><w:r><w:t>${xmlEscape(value)}</w:t></w:r></w:p>`,
      ),
    ]),
...
function pptParagraph(value, size = 1800, bold = false) {
  return `<a:p><a:r><a:rPr lang="en-GB" sz="${size}"${bold ? " b=\"1\"" : ""}/><a:t>${xmlEscape(value)}</a:t></a:r><a:endParaRPr lang="en-GB" sz="${size}"/></a:p>`;
}

function slideXml(title, body) {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="0" cy="0"/><a:chOff x="0" y="0"/><a:chExt cx="0" cy="0"/></a:xfrm></p:grpSpPr><p:sp><p:nvSpPr><p:cNvPr id="2" name="Title"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr><p:spPr><a:xfrm><a:off x="685800" y="457200"/><a:ext cx="7772400" cy="914400"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/><a:ln><a:noFill/></a:ln></p:spPr><p:txBody><a:bodyPr/><a:lstStyle/>${pptParagraph(title, 3000, true)}</p:txBody></p:sp><p:sp><p:nvSpPr><p:cNvPr id="3" name="Content"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr><p:spPr><a:xfrm><a:off x="685800" y="1600200"/><a:ext cx="7772400" cy="4572000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/><a:ln><a:noFill/></a:ln></p:spPr><p:txBody><a:bodyPr wrap="square"/><a:lstStyle/>${body.map((value) => pptParagraph(value)).join("")}</p:txBody></p:sp></p:spTree></p:cSld><p:clrMapOvr><a:masterClrMapping/></p:clrMapOvr></p:sld>`;
}

export function renderDocumentPptx(document = {}) {
  const slides = [
    { title: document.title, body: [document.subtitle].filter(Boolean) },
    ...(document.sections || []).map((section) => ({
      title: section.heading,
      body: [
        ...(section.paragraphs || []),
        ...(section.bullets || []).map((value) => `• ${value}`),
      ],
    })),
  ];
  ...
  const entries = [
    ...
    },
    {
      ...
      data: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Avantiqo"><a:themeElements><a:clrScheme name="Avantiqo"><a:dk1><a:srgbClr val="111111"/></a:dk1><a:lt1><a:srgbClr val="FFFFFF"/></a:lt1><a:dk2><a:srgbClr val="222222"/></a:dk2><a:lt2><a:srgbClr val="F2F2F2"/></a:lt2><a:accent1><a:srgbClr val="D6A66A"/></a:accent1><a:accent2><a:srgbClr val="666666"/></a:accent2><a:accent3><a:srgbClr val="888888"/></a:accent3><a:accent4><a:srgbClr val="AAAAAA"/></a:accent4><a:accent5><a:srgbClr val="CCCCCC"/></a:accent5><a:accent6><a:srgbClr val="E0E0E0"/></a:accent6><a:hlink><a:srgbClr val="0563C1"/></a:hlink><a:folHlink><a:srgbClr val="954F72"/></a:folHlink></a:clrScheme><a:fontScheme name="Avantiqo"><a:majorFont><a:latin typeface="Aptos Display"/></a:majorFont><a:minorFont><a:latin typeface="Aptos"/></a:minorFont></a:fontScheme><a:fmtScheme name="Avantiqo"><a:fillStyleLst/><a:lnStyleLst/><a:effectStyleLst/><a:bgFillStyleLst/></a:fmtScheme></a:themeElements></a:theme>`,
    },
  ];
  slides.forEach((slide, index) => {
    entries.push({ name: `ppt/slides/slide${index + 1}.xml`, data: slideXml(slide.title, slide.body) });
    entries.push({
