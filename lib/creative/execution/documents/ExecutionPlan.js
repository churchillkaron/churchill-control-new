...
export function createExecutionPlan(data = {}) {
  ...
  return {
    ...
    status:
      data.status ?? EXECUTION_PLAN_STATUS.DRAFT,
    execution_mode:
      data.execution_mode ?? "production",
    estimated_cost:
...
export function createExecutionStep(data = {}) {
  return {
    ...
    service:
      data.service ?? data.service_code ?? null,
    capability: data.capability,
    service_code:
      data.service_code ?? data.service ?? null,
    priority: Number(data.priority ?? 100),
    depends_on: data.depends_on ?? [],
    status:
      data.status ?? EXECUTION_STEP_STATUS.WAITING,
    estimated_cost:
      Number(data.estimated_cost ?? 0),
    estimated_seconds:
