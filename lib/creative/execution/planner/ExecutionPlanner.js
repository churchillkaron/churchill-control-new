  ...
  plan.steps = nodes
    ...
    .map((node) => {
      const dependencies = edges
        ...
        .map((edge) => edge.from);
      const assets = list(node.assets);
      const generationInput = node.generation.input || {};
      const specification = generationInput.specification ||
        node.requirements?.specification ||
        {};
      const authorizedReferenceAssetIds = uniqueReferenceIds(
        generationInput.authorized_reference_asset_ids,
        generationInput.reference_assets,
        generationInput.assets,
        assets,
        specification.shot?.assets,
        specification.shot?.reference_asset_ids,
        specification.shot?.reference_pack?.asset_ids,
        specification.shot?.reference_pack?.reference_asset_ids,
      );
      ...
      return createExecutionStep({
        ...
        input: {
          ...
          assets,
          reference_assets:
            list(generationInput.reference_assets).length
              ? generationInput.reference_assets
              : assets,
