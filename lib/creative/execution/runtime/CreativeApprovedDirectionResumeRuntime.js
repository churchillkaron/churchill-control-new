...
const CONTRACT = "CREATIVE_APPROVED_DIRECTION_EXACT_RESUME_V1";
...
async function ensureSourceNode({
  ...
  const result = await AssetGraphRepository.createOrFindByMetadataIdentity({
    ...
    node: createCreativeAssetNode({
      ...
      name: `Approved direction source — ${role}`,
      description: "Exact-resume source material from the approved Studio direction.",
      url,
      ...
      review: {
        ...
        human_reviewed: true,
        approved: true,
        notes: "Bound from the approved locked Studio project contract.",
      },
      metadata: {
...
async function ensureTimeline({
  ...
  const result = await AssetGraphRepository.createOrFindByMetadataIdentity({
    ...
    node: createCreativeAssetNode({
      ...
      description:
        "Deterministic exact-resume EDL. Creative direction is locked and not re-generated.",
      lineage: {
        source: "approved_direction_exact_resume",
      ...
      review: {
        ...
        human_reviewed: true,
        approved: true,
        notes: "Uses the already-approved Studio story/timing contract without re-direction.",
      },
      metadata: {
