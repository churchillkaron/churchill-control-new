...
function legacyDossierAuthorization(task = {}) {
  const metadata = object(task.metadata);
  const ceiling = finite(metadata.approved_cost_ceiling);
  const taskMaximum = finite(task.cost?.estimated);
  const currency = text(metadata.approved_cost_currency || task.cost?.currency).toUpperCase();
  if (
    metadata.production_dossier_mode !== "LEGACY_PRODUCTION_DOSSIER" ||
    metadata.production_dossier_gate_passed !== true ||
    task.cost?.approved !== true ||
    !text(metadata.production_dossier_approval_record_asset_node_id) ||
    !text(metadata.approved_dossier_hash) ||
    !text(metadata.approved_plan_hash) ||
    !text(metadata.approved_graph_hash) ||
    !text(metadata.approved_execution_hash) ||
    ceiling === null || ceiling <= 0 ||
    taskMaximum === null || taskMaximum <= 0 || taskMaximum > ceiling + 0.000001 ||
    !currency
  ) return null;
  return {
    contract: "CREATIVE_LEGACY_DOSSIER_EXECUTION_APPROVAL_V1",
    production_authorized: true,
    publication_authorized: false,
    maximum_customer_price: ceiling,
    currency,
    approval_record_asset_node_id: metadata.production_dossier_approval_record_asset_node_id,
    dossier_hash: metadata.approved_dossier_hash,
  };
}

function guardFromTask(task = {}) {
  const explicit = task.metadata?.approved_cost_guard ||
    task.input?.approved_cost_guard || task.cost?.guard;
  if (explicit) return normalizedGuard(explicit);
  const dossier = legacyDossierAuthorization(task);
  if (!dossier) return normalizedGuard(explicit);
  return normalizedGuard({
    maximum_customer_price: task.cost?.estimated,
    currency: dossier.currency,
    reference: `dossier:${dossier.approval_record_asset_node_id}:task:${task.id}`,
    estimated_quantity: task.timing?.estimated_seconds || null,
  });
}

function taskProvider(task = {}, execution = {}) {
  const payload = object(execution.input);
  const policy = object(task.input?.provider_policy || task.metadata?.provider_policy);
  const preferred = Array.isArray(policy.preferred_providers) ? policy.preferred_providers.filter(Boolean) : [];
  return text(
    execution.provider_id ||
    task.provider_id ||
    payload.generation?.provider ||
    payload.provider ||
    task.metadata?.pricing_quote?.provider ||
...
function taskModel(task = {}, execution = {}) {
  ...
  return text(
    ...
    payload.model ||
    task.metadata?.model ||
    task.metadata?.pricing_quote?.model,
  ) || null;
}
...
function taskQuantity(task = {}, execution = {}) {
  ...
  return finite(
    ...
    task.metadata?.pricing_quote?.quantity ??
...
function executionApproval(task = {}, execution = {}) {
  return sealedApproval(task) ||
    singleMediaApproval(task, execution) ||
    legacyDossierAuthorization(task);
}

function benchmarkReviewPricingAllowed(task = {}, provider = "") {
  const policy = object(task.input?.provider_policy || task.metadata?.provider_policy);
  const quote = object(task.metadata?.pricing_quote);
  const singleMediaProvider = text(task.metadata?.media_generation_authorization?.provider);
  const dossier = legacyDossierAuthorization(task);
  const dossierQuoteMatches = Boolean(
    dossier &&
    text(provider) === text(quote.provider) &&
    text(quote.currency).toUpperCase() === text(task.cost?.currency).toUpperCase() &&
    sameMoney(quote.customer_price, task.cost?.estimated)
  );
  return Boolean(
    text(policy.execution_scope).toUpperCase() === "BENCHMARK_REVIEW_PREVIEW" &&
    policy.benchmark_only === true &&
    policy.owned_only_required === true &&
    policy.external_fallback_allowed === false &&
    (text(provider) === singleMediaProvider || dossierQuoteMatches)
  );
}

function benchmarkPricingRecordAllowed(pricing = {}) {
...
async function resolveGuardPricing({ task, provider, capability, model, guard, quantity }) {
  try {
    return await PricingRuntime.resolve({
      provider,
      capability,
      model,
    ...
    if (!benchmarkReviewPricingAllowed(task, provider)) throw error;
    const record = await getProviderPricing({
      provider,
      capability,
      model,
      currency: guard.currency,
    ...
    if (!record || !benchmarkPricingRecordAllowed(record)) throw error;
    return PricingRuntime.resolveRecord({
      pricing: { ...record, benchmark_review_preview_authorized: true },
      provider,
      capability,
      model,
      currency: guard.currency,
...
async function assertPriceWithinGuard({ task, execution, guard }) {
  ...
  const pricing = await resolveGuardPricing({
    task,
    provider,
    capability,
    model,
