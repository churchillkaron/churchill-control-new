...
} from "@/lib/creative/research/runtime/AutonomousResearchDirectorRuntime";
import {
...
const RESEARCH_IDENTITY_CONTRACT = "CREATIVE_RESEARCH_ORGANIZATION_IDENTITY_V3";
...
function researchIdentity(project = {}) {
  ...
  const digest = crypto
    ...
    .digest("hex")
    .slice(0, 20);
  return `creative-project-research-v3:${project.id}:${digest}`;
}
...
async function executeProjectResearch(job, controls, payload) {
  ...
  await controls.heartbeat({
    ...
    message: "Running organization-grounded autonomous research",
  ...
  const report = await AutonomousResearchDirectorRuntime.run({
    organization_id: job.organization_id,
    mission: mission || {},
    project,
    brief,
...
function completedMessage(jobType) {
  return jobType === JOB_TYPES.PROJECT_RESEARCH
    ? "Organization-grounded autonomous research completed"
    : "Dense semantic verification completed";
}
...
export const CreativeExecutionJobRuntime = {
  ...
  async enqueueProjectResearch({
    ...
    const { job, created } = await CreativeExecutionJobRepository.enqueue({
      ...
      progress: {
        stage: "QUEUED",
        message: "Waiting for organization-grounded research worker",
        research_grounding_version:
          project.metadata?.research_grounding_version || null,
        research_transport_version: RESEARCH_TRANSPORT_VERSION,
        research_identity_contract: RESEARCH_IDENTITY_CONTRACT,
