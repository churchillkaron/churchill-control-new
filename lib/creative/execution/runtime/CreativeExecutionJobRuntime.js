  ...
  switch (job.job_type) {
    case JOB_TYPES.SHORTLIST_VERIFY:
      await controls.heartbeat({
        ...
        message: "Inspecting selected source moments",
      });
      return CreativeBoundedShortlistVerificationRuntime.verifyProject({
...
export const CreativeExecutionJobRuntime = {
  ...
  async processOne({
    ...
    try {
      ...
      const delaySeconds = permanentFailure(error)
        ? 0
      ...
      const updated = await CreativeExecutionJobRepository.retry({
        ...
        progress: {
          stage: permanentFailure(error) ? "FAILED" : "RETRY_SCHEDULED",
      ...
      return {
        ...
        permanent_failure: permanentFailure(error),
        job: updated,
        error: failure,
        production_started: false,
      };
