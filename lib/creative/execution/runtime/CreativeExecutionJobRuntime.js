...
} from "@/lib/creative/research/runtime/AutonomousResearchDirectorV4Runtime";
import {
  InternalCreativeResearchRuntime,
} from "@/lib/creative/research/runtime/InternalCreativeResearchRuntime";
import { createResearchReport } from "@/lib/creative/research/documents/ResearchReport";
import * as ResearchRepository from "@/lib/creative/research/repositories/ResearchRepository";
import {
  CreativeProjectDirectionRuntime,
...
function permanentFailure(error) {
  ...
  return [
    ...
    "JOB_PLAN_IDENTITY_MISMATCH",
    "JOB_CONTEXT_MISMATCH",
    "ORGANIZATION_ID REQUIRED",
    "CREATIVE_PROJECT_ID REQUIRED",
    "CREATIVE_PAID_RESEARCH_APPROVAL_REQUIRED",
    "CREATIVE_RESEARCH_APPROVED_COST_EXCEEDED",
    "CREATIVE_RESEARCH_RETRY_REQUIRED",
    "CREATIVE_PAID_DIRECTION_APPROVAL_REQUIRED",
    "CREATIVE_DIRECTION_APPROVAL",
    "CREATIVE_DIRECTION_FAILED_CLOSED",
    "CREATIVE_DIRECTION_GRAPH_MUTATION_DETECTED",
    "CREATIVE_DIRECTION_TASK_MUTATION_DETECTED",
    "INPUT_MISMATCH",
...
function researchBrief(project = {}, payload = {}) {
  ...
  return {
    ...
    duration_seconds:
      finite(supplied.duration_seconds, 0) ||
      finite(project.target_duration, 0) ||
      finite(metadata.temporal_contract?.duration_seconds, 0) ||
      null,
    metadata: {
      ...
      research_grounding_version:
        text(metadata.research_grounding_version) || null,
      research_transport_version: RESEARCH_TRANSPORT_VERSION,
      research_identity_contract: RESEARCH_IDENTITY_CONTRACT,
    },
  };
}

function researchIdentity(project = {}) {
  const metadata = object(project.metadata);
  const digest = crypto
    .createHash("sha256")
    .update(JSON.stringify({
      creative_request: text(metadata.creative_request),
      command_identity: text(metadata.command_identity),
      research_grounding_version:
        text(metadata.research_grounding_version),
      research_transport_version: RESEARCH_TRANSPORT_VERSION,
      research_identity_contract: RESEARCH_IDENTITY_CONTRACT,
    ...
    .digest("hex")
    .slice(0, 20);
  return `creative-project-research-v5:${project.id}:${digest}`;
}

async function executeProjectResearch(job, controls, payload) {
  const project = await CreativeProjectRuntime.get(job.creative_project_id);
  if (!project) throw new Error("PROJECT_NOT_FOUND");
  if (text(project.organization_id) !== text(job.organization_id)) {
    throw new Error("JOB_CONTEXT_MISMATCH:PROJECT_ORGANIZATION");
  }

  const missionId =
    text(payload.creative_mission_id) ||
    text(project.creative_mission_id) ||
    null;
  const mission = missionId
    ? await CreativeMissionRuntime.get(missionId)
    : null;
  if (mission && text(mission.organization_id) !== text(job.organization_id)) {
  ...
  const brief = researchBrief(project, payload);

  await controls.heartbeat({
    stage: "RESEARCHING_PROJECT",
    message: "Running evidence-first structured autonomous research",
    creative_mission_id: missionId,
    asset_count: assets.length,
    research_grounding_version:
      project.metadata?.research_grounding_version || null,
    research_transport_version: RESEARCH_TRANSPORT_VERSION,
    research_identity_contract: RESEARCH_IDENTITY_CONTRACT,
  });

  const report = InternalCreativeResearchRuntime.applies(project)
    ? await ResearchRepository.create(createResearchReport(
    ...
    : await AutonomousResearchDirectorV4Runtime.run({
        organization_id: job.organization_id,
        mission: mission || {},
        project,
        brief,
  ...
  return {
    research_report_id: report?.id || null,
    ...
    creative_grounding: report?.metadata?.creative_grounding || {},
...
async function executeJob(job, controls) {
  ...
  if (job.job_type === JOB_TYPES.PROJECT_RESEARCH) {
    return executeProjectResearch(job, controls, payload);
  }

  if (job.job_type === JOB_TYPES.PROJECT_DIRECTION) {
    return CreativeProjectDirectionRuntime.execute({
      job,
      controls,
      payload,
    });
...
function completedMessage(jobType) {
  if (jobType === JOB_TYPES.PROJECT_RESEARCH) {
    return "Evidence-first structured autonomous research completed";
  }
  if (jobType === JOB_TYPES.PROJECT_DIRECTION) {
    return "Exact-duration governed Creative direction completed";
  }
  return "Dense semantic verification completed";
}
...
export const CreativeExecutionJobRuntime = {
  ...
  async preflightShortlistVerification({
    ...
    return CreativeDenseSemanticExecutionPlanRuntime.preflight({
      organization_id,
      creative_project_id,
      policy,
      country,
  ...
  async enqueueProjectResearch({
    ...
    const idempotencyKey = researchIdentity(project);
    const { job, created } = await CreativeExecutionJobRepository.enqueue({
      organization_id,
      creative_project_id,
      job_type: JOB_TYPES.PROJECT_RESEARCH,
      idempotency_key: idempotencyKey,
      priority: 20,
      maximum_attempts: 1,
      payload: {
      ...
      progress: {
        stage: "QUEUED",
        message: "Waiting for evidence-first structured research worker",
        research_grounding_version:
          project.metadata?.research_grounding_version || null,
        research_transport_version: RESEARCH_TRANSPORT_VERSION,
        research_identity_contract: RESEARCH_IDENTITY_CONTRACT,
  ...
  async enqueueProjectDirection({
    ...
    const prepared = await CreativeProjectDirectionRuntime.prepare({
      organization_id,
      creative_project_id,
      creative_mission_id,
      payload: { brief: object(brief) },
    });
    const idempotencyKey = [
      "creative-project-direction-v1",
      creative_project_id,
      prepared.direction_hash.slice(0, 20),
    ].join(":");

    const { job, created } = await CreativeExecutionJobRepository.enqueue({
      organization_id,
      creative_project_id,
      job_type: JOB_TYPES.PROJECT_DIRECTION,
      idempotency_key: idempotencyKey,
      priority: 30,
      maximum_attempts: 1,
      payload: {
        creative_mission_id:
          creative_mission_id || prepared.mission_id || null,
        brief: object(brief),
        ...
        direction_contract: CreativeProjectDirectionRuntime.contract,
        production_authorized: false,
      ...
      progress: {
        stage: "QUEUED",
        message: "Waiting for exact-duration governed Creative direction worker",
        direction_hash: prepared.direction_hash,
        direction_approval_id: prepared.direction_approval.id,
        research_report_id: prepared.research_report.id,
        direction_contract: CreativeProjectDirectionRuntime.contract,
  ...
  async processOne({
    ...
    try {
      ...
      const completed = await CreativeExecutionJobRepository.complete({
        ...
        progress: {
          stage: "COMPLETED",
          message: completedMessage(job.job_type),
      ...
      const isPermanent =
        ...
        job.job_type === JOB_TYPES.PROJECT_DIRECTION ||
