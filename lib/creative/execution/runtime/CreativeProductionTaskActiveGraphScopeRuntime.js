...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.production-task-active-graph-scope.v2",
);
const CONTRACT = "CREATIVE_PRODUCTION_TASK_ACTIVE_GRAPH_SCOPE_V2";
...
function newest(left = {}, right = {}) {
  return Date.parse(right.created_at || right.updated_at || 0) -
...
async function activeGraphId({
  ...
  const active = list(data).filter(activeGraph).sort(newest);
      ...
      return list(tasks)
        ...
        .map((task) => ({
          ...
          metadata: {
            ...
            active_graph_scope_explicit:
              Boolean(explicitGraphId),
          },
        }));
    };
}
