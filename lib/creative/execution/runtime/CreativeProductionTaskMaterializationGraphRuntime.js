...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.production-task-materialization-graph.v3-capability-only",
);
const GRAPH_CONTRACT = "CREATIVE_PRODUCTION_TASK_MATERIALIZATION_GRAPH_V3_CAPABILITY_ONLY";
...
function capabilityOnlyNode(sourceNode = {}) {
  const node = CreativeProductionTaskMaterializationRuntime.stripPrompts(sourceNode);
  ...
  const {
    ...
    ...generationWithoutProvider
  } = generation;
  const {
    provider_id: ignoredMetadataProviderId,
    provider: ignoredMetadataProvider,
    preferred_provider: ignoredMetadataPreferredProvider,
    preferred_providers: ignoredMetadataPreferredProviders,
    ...metadataWithoutProvider
  } = metadata;

  return {
    ...node,
    provider_id: null,
    generation: generationWithoutProvider,
    metadata: {
      ...metadataWithoutProvider,
...
function install() {
  ...
  ProductionGraphRuntime.plan = async function planWithTaskMaterializationContracts(input = {}) {
    ...
    const nodes = list(graph.nodes).map((sourceNode) => {
      const node = capabilityOnlyNode(sourceNode);
      ...
      if (node.requirements?.task_materialization_contract?.provider_id) {
        throw new Error(
          `PRODUCTION_TASK_PROVIDER_PIN_FORBIDDEN:${node.id}`,
        );
      }
      return node;
    });
    ...
    const invalid = generationNodes.filter((node) => {
      const contract = object(node.requirements?.task_materialization_contract);
      if (!CreativeProductionTaskMaterializationRuntime.verify(contract)) {
        return true;
      }
      if (contract.provider_id) return true;
      if (
    ...
    return ProductionGraphRuntime.update(graph.id, {
      ...
      metadata: {
        ...
        task_materialization_provider_ids_preserved: false,
        task_materialization_provider_pins_forbidden: true,
        task_materialization_capability_only: true,
        task_materialization_service_runtime_owned_first: true,
