...
function taskTypeFor(node = {}) {
  ...
  ) return PRODUCTION_TASK_TYPES.FIRST_LAST_FRAME_TO_VIDEO;
  if (
  ...
  if (
    ...
    capability.includes("video-to-video") ||
    type === "VIDEO_TO_VIDEO"
  ) return PRODUCTION_TASK_TYPES.VIDEO_TO_VIDEO;
  if (capability === "ai.video.edit" || type === "EDIT_VIDEO") {
    return PRODUCTION_TASK_TYPES.EDIT_VIDEO;
  }
  if (capability === "ai.video.inpaint" || type === "INPAINT_VIDEO") {
    return PRODUCTION_TASK_TYPES.INPAINT_VIDEO;
  }
  if (capability.includes("image.upscale")) return PRODUCTION_TASK_TYPES.UPSCALE;
  if (capability === "ai.image.edit" || type === "EDIT_IMAGE") {
    return PRODUCTION_TASK_TYPES.EDIT_IMAGE;
  }
  if (capability === "ai.image.inpaint" || type === "INPAINT_IMAGE") {
    return PRODUCTION_TASK_TYPES.INPAINT_IMAGE;
  }
  if (capability === "ai.image.outpaint" || type === "OUTPAINT_IMAGE") {
    return PRODUCTION_TASK_TYPES.OUTPAINT_IMAGE;
  }
