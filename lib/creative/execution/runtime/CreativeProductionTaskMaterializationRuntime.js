...
function promptlessPersistenceRequired(input = {}, requirements = {}) {
  return input.promptless_persistence === true ||
    requirements.promptless_persistence === true ||
    input.repair_context?.promptless_persistence === true ||
    input.repair_specification?.promptless_source_of_truth === true;
}
...
function normalizeTaskInput(data = {}, contract = {}) {
  ...
  if (promptlessPersistenceRequired(input, requirements)) {
    return normalized;
  }
...
function preserveVerifiedPairRepairPayload(data = {}, contract = {}) {
  return data.metadata?.pair_aware_repair === true &&
    data.metadata?.generated_media_perceptual_pair_repair === true &&
    [
      "GENERATED_MEDIA_PERCEPTUAL_REPLACEMENT_SOURCE_V1",
      "GENERATED_MEDIA_PERCEPTUAL_REPLACEMENT_REVIEW_V1",
    ].includes(text(data.metadata?.repair_payload_contract)) &&
    promptlessPersistenceRequired(
      object(data.input),
      object(data.input?.requirements),
    ) &&
    text(data.metadata?.execution_node_id) === text(contract.node_id);
}
...
function install() {
  ...
  ProductionTaskRuntime.create = async function createWithNodeContract(data = {}) {
    ...
    assertNodeMatch(data, contract);
    if (preserveVerifiedPairRepairPayload(data, contract)) {
      return createWithoutMaterialization(data);
    }
