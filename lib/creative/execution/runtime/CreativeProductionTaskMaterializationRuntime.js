...
function stripForbidden(value, depth = 0) {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => stripForbidden(item, depth + 1));
  }
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(
    Object.entries(value)
      ...
      .map(([key, child]) => [key, stripForbidden(child, depth + 1)]),
  );
}
...
function contractFor(node = {}) {
  ...
  const contract = {
    ...
    reference_assets: stripForbidden(list(node.requirements?.reference_assets)),
    reference_asset_ids: list(node.requirements?.reference_asset_ids),
    output_spec: stripForbidden(
...
function normalizeTaskInput(data = {}, contract = {}) {
  const input = stripForbidden(object(data.input));
  const requirements = stripForbidden(object(input.requirements));
  return {
    ...input,
    node_id: contract.node_id,
    node_type: contract.node_type,
    requirements,
    source_assets: stripForbidden(list(input.source_assets)),
    reference_assets: list(input.reference_assets).length
      ? stripForbidden(list(input.reference_assets))
      : stripForbidden(list(contract.reference_assets)),
    reference_asset_ids: list(input.reference_asset_ids).length
      ? list(input.reference_asset_ids)
      : list(contract.reference_asset_ids),
    generation: stripForbidden(object(input.generation)),
    provider_parameters: stripForbidden({
