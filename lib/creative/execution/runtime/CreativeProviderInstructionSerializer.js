...
function visualInstruction(input = {}, context = {}, service = "") {
  ...
  if (imageOnly) {
    const frameRequirements = object(context.requirements);
    const frameIntent = object(context.intent);
    const camera = object(frameRequirements.camera);
    const productionDesign = object(frameRequirements.production_design);
    const artDirectionAuthority = object(frameRequirements.art_direction_authority);
    const artDirectionDossier = object(artDirectionAuthority.dossier);
    const stillContract = {
      governing_visual_idea: artDirectionDossier.governing_visual_idea || null,
      subject: frameIntent.subject || frameRequirements.subject || null,
      visible_state_or_action: frameIntent.action || frameRequirements.action || null,
      framing: camera.framing || null,
      viewpoint_and_lens: camera.lens_intent || camera.lens || null,
      lighting: frameRequirements.lighting || null,
      location: frameRequirements.location || null,
      materials: productionDesign.materials || null,
      performance: frameRequirements.performance || null,
    };
    return [
      "Photoreal image. Depict only the approved visible scene described below.",
      "Use natural photographic geometry, physically credible materials, realistic depth, believable light, and the approved framing and viewpoint.",
      targetedRepair ? section("Targeted QA repair", repairSpecification) : null,
      section("Approved still frame", stillContract),
    ].filter(Boolean).join("\n\n");
  }
  ...
  return [
    ...
    section("Structured shot contract", structuredVisualContract),
  ].filter(Boolean).join("\n\n");
}

function imageNegativeInstruction(input = {}, context = {}) {
  const requirements = object(context.requirements);
  const productionDesign = object(requirements.production_design);
  const explicit = [
    ...list(requirements.negative_constraints),
    ...list(requirements.must_avoid),
  ...
  return [...explicit, ...apparatus, ...artifacts].join(", ");
}

export function serializeCreativeProviderNegativeInstruction(input = {}) {
  const context = creativeContext(input);
  const service = capability(input);
  if (!service.includes("image")) return "";
  return imageNegativeInstruction(input, context);
}
