...
function validateTaskCostGuard(task = {}) {
  ...
  const guardCurrency = text(guard.currency).toUpperCase();
  const taskCurrency = text(task.cost?.currency).toUpperCase();
  const approvalCurrency = text(sealedApproval(task)?.currency).toUpperCase();
  if (!guardCurrency) {
    throw new Error("SEALED_PRODUCTION_TASK_COST_GUARD_CURRENCY_REQUIRED");
  }
  if (taskCurrency && taskCurrency !== guardCurrency) {
    throw new Error("SEALED_PRODUCTION_TASK_COST_GUARD_CURRENCY_MISMATCH");
  }
  if (approvalCurrency && approvalCurrency !== guardCurrency) {
    throw new Error("SEALED_PRODUCTION_TASK_APPROVAL_CURRENCY_MISMATCH");
  }
