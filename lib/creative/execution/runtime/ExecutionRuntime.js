...
export const ExecutionRuntime = {
  ...
  async create(input = {}) {
    ...
    const existing = basePlan.production_graph_id
      ? await Repository.getLatestByGraph({
          organization_id: basePlan.organization_id,
          creative_project_id: basePlan.creative_project_id,
          production_graph_id: basePlan.production_graph_id,
        })
      : null;
    const plan = existing || await quoteExecutionPlan(basePlan);

    const execution = existing || await Repository.create(plan);
