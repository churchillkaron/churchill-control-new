...
function workflowFor(project = {}, tasks = []) {
  ...
  if (!workflow) {
    throw new Error("CREATIVE_UNIVERSAL_FINALISATION_WORKFLOW_REQUIRED");
  }
  if (workflow.finaliser !== "UNIVERSAL") {
    throw new Error(
      `CREATIVE_UNIVERSAL_FINALISATION_WORKFLOW_REQUIRED:${workflow.workflow_kind}`,
    );
  }
  return workflow;
}
...
export const CreativeUniversalFinalisationRuntime = {
  async run({ organization_id, creative_project_id } = {}) {
    ...
    const workflow = workflowFor(project, tasks).workflow_kind;
