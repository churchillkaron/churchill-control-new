...
const GENERIC_ROLE_PATTERN = /\b(staff|staff member|employee|team member|service team|bartender|bar staff|server|waiter|waitress|chef|cook|host|hostess|cashier|manager|supervisor|worker|crew|operator|attendant|assistant|technician|specialist|customer|client|guest|visitor|patron|diner|audience|crowd|participant|passerby|family|couple|friends?|companion|adult|woman|man|girl|boy|people|person|human|extra|extras|player|players|newcomer|arrival|regular|regulars|local|locals|party|group|principal|service lead|team lead)\b/i;
const NARRATIVE_ROLE_PATTERN = /\b(the|a|an)?\s*(hesitant|curious|first[- ]?time|returning|new|arriving|solo|young|older|friendly|welcoming|energetic|social|laughing|celebrating|nearby|lead|supporting|featured|background|foreground)?\s*(newcomer|arrival|guest|customer|client|visitor|patron|diner|regular|local|friend|companion|couple|family|group|party|crowd|participant|player|staff|employee|bartender|server|waiter|waitress|chef|cook|host|hostess|manager|supervisor|team|crew|performer|musician|singer|artist|principal)\b/i;
const HUMAN_PATTERN = /\b(person|people|human|participant|artist|performer|musician|singer|actor|actress|model|dancer|staff|employee|bartender|server|waiter|waitress|chef|host|hostess|customer|client|guest|visitor|patron|diner|audience|crowd|family|couple|friends?|companion|adult|woman|man|girl|boy|face|portrait|player|players|newcomer|arrival|regular|regulars|local|locals|party|group|principal)\b/i;
const AUTO_PROFILE_PATTERN = /^(identity-profile-\d+|shot-explicit-identity|primary-performer|auto(?:matic)?[-_: ]identity|runtime[-_: ]identity)/i;
const VERIFIED_IDENTITY_SOURCE_PATTERN = /\b(manual|verified|approved|asset[-_ ]manifest|identity[-_ ]registry|person[-_ ]record|human[-_ ]approval|user[-_ ]selected|explicit[-_ ]identity)\b/i;
...
function properPersonalName(value) {
  const source = text(value);
  if (!source || GENERIC_ROLE_PATTERN.test(source) || NARRATIVE_ROLE_PATTERN.test(source)) {
    return false;
  }
  const words = source.split(/\s+/).filter(Boolean);
  if (words.length < 2 || words.length > 5) return false;
  if (/^(the|a|an)\b/i.test(source)) return false;
...
function likelyNamedActor(actor) {
  ...
  if (actorIdentityId(actor)) return true;
  if (name) return properPersonalName(name);
  if (Object.keys(record).length) return false;
  return properPersonalName(actor);
}

function narrativeRoleActor(actor) {
  const record = object(actor);
  if (actorIdentityId(actor)) return false;
  const name = text(record.name);
  const role = text(record.role || record.label || record.description);
  if (role && !properPersonalName(name)) return true;
  const label = actorLabel(actor);
  if (!label) return false;
  return GENERIC_ROLE_PATTERN.test(label) ||
    NARRATIVE_ROLE_PATTERN.test(label) ||
    /^(the|a|an)\b/i.test(label);
}

function genericActor(actor) {
  if (narrativeRoleActor(actor)) return true;
  const record = object(actor);
  if (actorIdentityId(actor) || properPersonalName(record.name || actor)) return false;
  return Boolean(actorLabel(actor));
}
...
function genericRoleShot(shot = {}) {
  ...
  return GENERIC_ROLE_PATTERN.test(corpus) || NARRATIVE_ROLE_PATTERN.test(corpus);
}
...
function runtimeInferredIdentity(shot = {}) {
  const profileId = identityProfileId(shot);
  const performance = object(
    shot.performance_contract || shot.metadata?.performance_contract,
  );
  ...
  const source = text(
    requirements.source ||
    requirements.identity_source ||
    metadata.identity_assignment_source ||
    metadata.identity_source,
  );
  return Boolean(
    AUTO_PROFILE_PATTERN.test(profileId) ||
    performance.version === "CREATIVE_PERFORMANCE_CONTRACT_V1" ||
    metadata.performance_contract?.version ===
      "CREATIVE_PERFORMANCE_CONTRACT_V1" ||
    metadata.identity_profile ||
    /runtime|automatic|inferred|performance/i.test(source)
  );
}

function authoritativeIdentitySource(shot = {}) {
  const requirements = object(shot.identity_requirements);
  const metadata = object(shot.metadata);
  const source = text(
    requirements.source ||
  ...
  return Boolean(
    requirements.authoritative === true ||
    requirements.identity_authoritative === true ||
    requirements.user_confirmed === true ||
    metadata.identity_authoritative === true ||
    metadata.identity_human_approved === true ||
    VERIFIED_IDENTITY_SOURCE_PATTERN.test(source)
  );
}
...
function explicitRealIdentityEvidence(shot = {}) {
  ...
  const lock = object(shot.generation?.identity_lock);
  const actorIdentity = list(shot.actors).some((actor) => Boolean(
    actorIdentityId(actor),
  ));
  if (actorIdentity || authoritativeIdentitySource(shot)) return true;

  const namedActor = namedActorShot(shot);
  const references = identityReferenceAssetIds(shot);
  const profileId = identityProfileId(shot);
  if (namedActor && (references.length || profileId)) return true;

  if (
    genericRoleShot(shot) &&
    runtimeInferredIdentity(shot) &&
    !authoritativeIdentitySource(shot)
  ) {
    return false;
  }

  if (references.length || profileId) return true;
  if (
    ...
    return namedActor || authoritativeIdentitySource(shot);
  }
  return false;
}
...
function classifyShot(shot = {}) {
  ...
  if (
    genericRoleShot(shot) &&
    !authoritativeIdentitySource(shot) &&
    !list(shot.actors).some((actor) => Boolean(actorIdentityId(actor)))
  ) {
    return {
      mode: "SYNTHETIC_CAST",
      human: true,
      ...
      synthetic_cast: true,
      unresolved: false,
      reason: runtimeInferredIdentity(shot)
        ? "GENERIC_ROLE_WITH_STALE_RUNTIME_IDENTITY"
        : "GENERIC_NARRATIVE_ROLE",
    };
  }
  if (explicitRealIdentityEvidence(shot)) {
    return {
      ...
      synthetic_cast: false,
      unresolved: false,
      reason: "AUTHORITATIVE_REAL_IDENTITY_EVIDENCE",
    };
  }
...
function neutralizeText(value) {
  return text(value)
    ...
    .replace(/\b(artist|performer|singer|actor|actress|model|dancer|musician)\b/gi, "featured talent")
    .replace(/\b(staff|employee|bartender|server|waiter|waitress|chef|host|hostess)\b/gi, "service team")
    .replace(/\b(customer|client|guest|visitor|patron|diner|newcomer|regular)\b/gi, "participant")
...
function crowdShot(shot = {}) {
  return list(shot.actors).length > 2 ||
    /\b(crowd|audience|group|people|customers|clients|guests|visitors|patrons|diners|family|friends|team|staff|crew|extras|participants|players|regulars|locals)\b/i.test(
      shotCorpus(shot),
    );
}
  ...
  return {
    ...
    metadata: {
      ...
      synthetic_cast_record_normalization_contract:
        "CREATIVE_CAST_CLASSIFICATION_V2",
      synthetic_cast_classification_reason: classification.reason,
      stale_runtime_identity_removed: runtimeInferredIdentity(shot),
...
function normalizePlan(plan = {}) {
  return {
    ...
    production: {
      ...
      cast_classification_contract: "CREATIVE_CAST_CLASSIFICATION_V2",
    },
  };
}
