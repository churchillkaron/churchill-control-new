...
async function prepareVideoFromKeyframe(task = {}) {
  ...
  const existingSourceAssets = list(task.input?.source_assets);
  const sourceAssets = [
    ...existingSourceAssets.filter((asset) =>
      text(asset?.role) !== "APPROVED_IDENTITY_STORY_KEYFRAME",
    ),
    { url, role: "APPROVED_IDENTITY_STORY_KEYFRAME" },
  ];
  const continuationFirstFrameBound = Boolean(
    text(task.input?.first_frame) ||
    text(task.input?.provider_parameters?.first_frame) ||
    existingSourceAssets.some((asset) =>
  ...
  return ProductionTaskRuntime.update(task.id, {
    ...
    metadata: {
      ...
      temporal_first_frame_preserved_through_identity_gate:
        continuationFirstFrameBound,
      identity_and_temporal_conditioning_coexist:
        continuationFirstFrameBound,
    },
  });
}
