...
function taskIdentity(parentId,type,parentVersion){return `image-derivative:${parentId}:${parentVersion}:${type.toLowerCase()}`;}
...
export async function ensureImageAssetDerivatives({organization_id,creative_project_id}={}){
  ...
  for(const parent of list(nodes).filter(eligible)){
    ...
    for(const spec of derivativePlan(parent)){
      const identity=taskIdentity(parent.id,spec.type,parentVersion);
