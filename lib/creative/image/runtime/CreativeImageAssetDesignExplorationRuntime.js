...
- The selected candidate must score at least 94 for premium visual authorship, 96 for continuity fidelity and 94 for production usability.
    ...
    const task=await ProductionTaskRuntime.create({
      ...
      input:{
        ...
        requirements:{
          ...
          minimum_premium_visual_authorship_score:94,
          minimum_continuity_fidelity_score:96,
          minimum_production_usability_score:94,
...
export async function reconcileImageAssetExplorationSelections({organization_id,creative_project_id}={}){
  ...
  for(const task of list(tasks).filter(t=>t.metadata?.image_asset_design_exploration_selection===true)){
    ...
    const validSelection=
      ...
      Number(result?.premium_visual_authorship_score)>=94 &&
      Number(result?.continuity_fidelity_score)>=96 &&
      Number(result?.production_usability_score)>=94 &&
