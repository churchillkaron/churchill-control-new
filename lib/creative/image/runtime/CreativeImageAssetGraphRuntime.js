...
function actorIdentityKeys(shot={}){
  return [...new Set(
    list(shot.actors)
...
function identityKey(shot={}){
  ...
  const actorKeys=actorIdentityKeys(shot);
  return actorKeys.length===1?actorKeys[0]:null;
}
function characterIdentityAmbiguous(shot={}){
  return !identityKey(shot)&&actorIdentityKeys(shot).length>1;
}
...
function sceneAuthorityEvidence(scene={},shots=[]){
  ...
  const scoped=list(shots)
    ...
    .map(s=>({
      ...
      actor_identity_keys:actorIdentityKeys(s),
      subject_identity_ambiguous:characterIdentityAmbiguous(s),
...
function assetPrompt(assetClass,shot={},scene={}){
  ...
  const imageLearning=object(scene.image_studio_learning);
  ...
  const common = [
    ...
    list(imageLearning.recurring_failure_patterns).length
      ? "ADVISORY IMAGE STUDIO REJECT MEMORY: "+JSON.stringify(imageLearning.recurring_failure_patterns)
      : "",
    list(imageLearning.accepted_rejected_design_evidence).length
      ? "ADVISORY IMAGE STUDIO DESIGN EVIDENCE: "+JSON.stringify(
          list(imageLearning.accepted_rejected_design_evidence).map(item=>({
            asset_class:item.asset_class,
            accepted_variation_axis:item.accepted_variation_axis,
            rejected_variation_axis:item.rejected_variation_axis,
            rejection_reasons:item.rejection_reasons,
          }))
        )
      : "",
    "LEARNING GOVERNANCE: prior failures must not be repeated without new evidence; prior accepted work may inform quality judgment but its style/composition must never be copied.",
    sharedSceneAsset
...
function requiredAssets(shot={},scene={}){
  ...
  if(humanExpected(shot)){
    if(!characterIdentityAmbiguous(shot)) out.push("CHARACTER_SHEET");
    out.push("PERFORMANCE_REFERENCE");
  }
  ...
  apply({graph,shots=[],scenes=[]}={}){
    ...
    const sceneMap=new Map(list(scenes).map(s=>[
      text(s.id),
      {
        ...
        image_studio_learning:object(graph.metadata?.taste_memory?.image_studio_learning),
      },
    ]));
