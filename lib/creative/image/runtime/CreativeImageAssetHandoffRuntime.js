...
export function selectImageAsset({
  ...
  foundation_authority_digest=null,
  asset_classes=[],
  ...
  const candidates=list(asset_nodes).filter(node=>{
    ...
    if(threat_identity_key&&text(node.metadata?.threat_identity_key)!==text(threat_identity_key)) return false;
    if(
      foundation_authority_digest &&
      text(node.metadata?.image_foundation_authority_digest)!==
        text(foundation_authority_digest)
    ) return false;
