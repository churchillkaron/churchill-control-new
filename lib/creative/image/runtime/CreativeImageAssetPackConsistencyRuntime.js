...
function prompt(group,nodes){
  ...
  const identityMap=nodes.map(node=>({
    ...
    threat_identity_key:node.metadata?.threat_identity_key||null,
  }));
  ...
  const distinctThreatKeys=[...new Set(identityMap.map(row=>text(row.threat_identity_key)).filter(Boolean))];
...
Return strict JSON only:
{
  ...
  "threat_identity_separation_valid": true,
  "threat_geometry_by_key": {},
...
Distinct governed threat keys: ${JSON.stringify(distinctThreatKeys)}
...
- Threat identity keys are equally authoritative. Different non-empty threat_identity_key values represent different governed pursuers/hero objects and MUST remain visually distinct.
- Set threat_identity_separation_valid=false if two threat identities collapse into one generic design, swap geometry, share impossible markings, or leak one governed threat design into another.
- threat_geometry_by_key must score every visible governed threat identity independently from 0..100.
...
function passed(result={}){
  ...
  if(result.threat_identity_separation_valid!==true)return false;
  ...
  for(const value of Object.values(object(result.threat_geometry_by_key))){
    const score=finite(value);if(score===null||score<96)return false;
  }
...
function reviewBatches(assets=[],limit=10){
  const ordered=[...list(assets)].sort((a,b)=>a.id.localeCompare(b.id));
  if(ordered.length<=limit)return [ordered];
  const globalAnchors=ordered.filter(node=>
    assetClass(node)==="ENVIRONMENT_LOOKFRAME"
  ).slice(0,1);
  const characterByIdentity=new Map(
    ordered
      .filter(node=>assetClass(node)==="CHARACTER_SHEET"&&identityKey(node))
      .map(node=>[identityKey(node),node])
  );
  const threatByIdentity=new Map(
    ordered
      .filter(node=>assetClass(node)==="THREAT_DESIGN"&&threatIdentityKey(node))
      .map(node=>[threatIdentityKey(node),node])
  );
  const remaining=ordered.filter(node=>!globalAnchors.some(anchor=>anchor.id===node.id));
  const batches=[];
  let cursor=0;
  while(cursor<remaining.length){
    const batch=[...globalAnchors];
    let progressed=false;
    while(cursor<remaining.length){
      const candidate=remaining[cursor];
      const identity=identityKey(candidate);
      const governedThreatKey=threatIdentityKey(candidate);
      const character=identity?characterByIdentity.get(identity):null;
      const threat=governedThreatKey?threatByIdentity.get(governedThreatKey):null;
      const additions=[
        ...(character&&!batch.some(item=>item.id===character.id)?[character]:[]),
        ...(threat&&!batch.some(item=>item.id===threat.id)?[threat]:[]),
        ...(batch.some(item=>item.id===candidate.id)?[]:[candidate]),
      ];
      if(batch.length+additions.length>limit){
        if(!progressed) {
          for(const item of additions){
            if(batch.length<limit&&!batch.some(existing=>existing.id===item.id))batch.push(item);
          }
          cursor+=1;
          progressed=true;
        }
      ...
      for(const item of additions){
        if(!batch.some(existing=>existing.id===item.id))batch.push(item);
      }
      cursor+=1;
      progressed=true;
    }
  ...
  const covered=new Set(batches.flat().map(node=>node.id));
  for(const missing of ordered.filter(node=>!covered.has(node.id))){
    const anchors=globalAnchors.slice(0,Math.max(0,limit-1));
    batches.push([...anchors,missing].slice(0,limit));
  }
  return batches;
}
...
export async function ensureImageAssetPackReviews({organization_id,creative_project_id}={}){
  ...
  for(const [group,assets] of groups){
    ...
    const batches=reviewBatches(assets,10);
    const allAssetIds=assets.map(a=>a.id).sort();
    for(let batchIndex=0;batchIndex<batches.length;batchIndex+=1){
      const batchAssets=batches[batchIndex];
      const batchHash=hash({
        pack_hash:packHash,
        batch_index:batchIndex,
        asset_node_ids:batchAssets.map(a=>a.id).sort(),
      });
      const prior=tasks.find(t=>text(t.metadata?.image_asset_pack_batch_hash)===batchHash);
      ...
      try{
        ...
        blocked.push({
          continuity_group_id:group,
          reason:"IMAGE_ASSET_PACK_SIGNING_FAILED",
          batch_index:batchIndex,
      ...
      const task=await ProductionTaskRuntime.create({
        ...
        scene_id:authority(batchAssets[0]).scene_id||batchAssets[0].metadata?.scene_id||null,
        shot_id:null,
        type:"EXECUTE_CAPABILITY",
        status:"WAITING",
        title:`Review Image Asset Pack · ${group} · ${batchIndex+1}/${batches.length}`,
        description:"Cross-review a bounded, identity-aware Image Studio continuity batch. Every full pack member is covered across overlapping batches before release.",
        ...
        input:{
          ...
          prompt:prompt(group,batchAssets),
          provider_prompt:prompt(group,batchAssets),
          requirements:{
            image_asset_pack_review:true,
            continuity_group_id:group,
            image_asset_node_ids:batchAssets.map(a=>a.id),
            image_asset_pack_all_asset_node_ids:allAssetIds,
            pack_hash:packHash,
            batch_hash:batchHash,
            batch_index:batchIndex,
            batch_count:batches.length,
            maximum_provider_media_references:10,
        ...
        metadata:{
          ...
          image_asset_pack_batch_hash:batchHash,
          image_asset_pack_batch_index:batchIndex,
          image_asset_pack_batch_count:batches.length,
          continuity_group_id:group,
          image_asset_node_ids:batchAssets.map(a=>a.id),
          image_asset_pack_all_asset_node_ids:allAssetIds,
          quality_gate:true,
...
export async function reconcileImageAssetPackReviews({organization_id,creative_project_id}={}){
  ...
  const reviews=list(tasks).filter(t=>t.metadata?.image_asset_pack_review===true);
  const multiGroups=new Map();
  for(const task of reviews.filter(t=>Number(t.metadata?.image_asset_pack_batch_count||1)>1)){
    const key=text(task.metadata?.image_asset_pack_hash);
    const bucket=multiGroups.get(key)||[];
    bucket.push(task);
    multiGroups.set(key,bucket);
  }
  const handled=new Set();
  for(const [packHash,groupTasks] of multiGroups){
    for(const task of groupTasks)handled.add(task.id);
    const expected=Math.max(...groupTasks.map(task=>Number(task.metadata?.image_asset_pack_batch_count||1)));
    if(groupTasks.length<expected||groupTasks.some(task=>["WAITING","RUNNING"].includes(text(task.status)))){
      pending.push(...groupTasks);
      continue;
    }
    if(groupTasks.every(task=>task.metadata?.image_asset_pack_qc_sealed===true)){
      sealed.push(...groupTasks);
      continue;
    }
    const results=groupTasks.map(task=>({
      task,
      result:text(task.status)==="COMPLETED"?parseJson(task.output):null,
    }));
    const invalid=results.filter(item=>!item.result||!passed(item.result));
    const allAssetIds=[...new Set(groupTasks.flatMap(task=>list(task.metadata?.image_asset_pack_all_asset_node_ids)))];
    if(invalid.length){
      const failures=[...new Set(invalid.flatMap(item=>list(item.result?.failures)).filter(Boolean))];
      const repairs=[...new Set(invalid.flatMap(item=>list(item.result?.repair_instructions)).filter(Boolean))];
      for(const id of allAssetIds){
        ...
        await AssetGraphRepository.update(id,{
          metadata:{
            ...
            image_asset_pack_hash:packHash,
            image_asset_pack_review_task_ids:groupTasks.map(task=>task.id),
            image_asset_pack_failures:failures,
            image_asset_pack_repair_instructions:repairs,
          },
        });
      }
      for(const {task,result} of results){
        if(text(task.status)==="COMPLETED"){
          failed.push(await ProductionTaskRuntime.update(task.id,{
            status:"FAILED",
            error:"IMAGE_ASSET_PACK_BATCH_CONSISTENCY_FAILED",
            review:{...object(task.review),approved:false,required:false},
            output:{...object(task.output),image_asset_pack_consistency:result||null},
          }));
    ...
    const sealHash=hash({
      contract:CREATIVE_IMAGE_ASSET_PACK_QC_SEAL_CONTRACT,
      pack_hash:packHash,
      continuity_group_id:groupTasks[0]?.metadata?.continuity_group_id||null,
      batch_hashes:groupTasks.map(task=>task.metadata?.image_asset_pack_batch_hash).sort(),
      batch_scores:results.map(item=>item.result),
    });
    for(const id of allAssetIds){
      const node=await AssetGraphRepository.getById(id);if(!node)continue;
      await AssetGraphRepository.update(id,{
        metadata:{
          ...
          image_asset_pack_hash:packHash,
          image_asset_pack_review_task_ids:groupTasks.map(task=>task.id),
          image_asset_pack_batch_count:expected,
        },
      });
    }
    for(const {task,result} of results){
  ...
  for(const task of reviews.filter(task=>!handled.has(task.id))){
