...
function seal(node={}){return node.metadata?.image_asset_pack_qc_seal_hash||node.metadata?.image_asset_exploration_selection_seal_hash||null;}
function fingerprint(node){return node?{id:node.id,checksum:node.technical?.checksum||null,seal:seal(node)}:null;}
...
export function evaluateImageFoundationAuthority({
  ...
  if(threatExpected(task)){
    threat=CreativeImageAssetHandoffRuntime.select({
      asset_nodes,
      scene_id:sceneId,
      shot_id:null,
      continuity_group_id:group,
