...
function stableSourceVersion(node={}){
  return {
    id:node.id||null,
    checksum:node.technical?.checksum||null,
    selection_seal:node.metadata?.image_asset_exploration_selection_seal_hash||null,
    localized_repair_review_task_id:node.metadata?.localized_repair_review_task_id||null,
    image_asset_review_task_id:node.metadata?.image_asset_review_task_id||null,
    perceptual_qc_sealed:node.metadata?.image_asset_perceptual_qc_sealed===true,
  };
}
function materialSourceDigest(nodes=[]){
  return hash(
    list(nodes)
      .map(stableSourceVersion)
  ...
  for(const [group,groupTasks] of groups){
    ...
    for(const key of materialKeys(groupTasks)){
      for(const scope of materialScopeVariants(key,sourceNodes)){
        ...
        const sourceDigest=materialSourceDigest(scopedNodes);
        const sourceVersions=scopedNodes.map(stableSourceVersion);
        const identity=`material-truth:${group}:${key.toLowerCase()}:${scope.kind.toLowerCase()}:${scopeToken}:${sourceDigest}`;
        ...
        const task=await ProductionTaskRuntime.create({
          ...
          input:{
            ...
            requirements:{
              ...
              material_truth_reference:{
                ...
                source_asset_versions:sourceVersions,
                source_asset_version_digest:sourceDigest,
          ...
          metadata:{
            ...
            material_truth_source_asset_versions:sourceVersions,
            material_truth_source_asset_version_digest:sourceDigest,
...
function currentMaterialSourceDigest(node={},assetNodes=[]){
  const ids=list(node.metadata?.material_truth_source_asset_node_ids);
  if(!ids.length)return null;
  const current=ids.map(id=>list(assetNodes).find(candidate=>text(candidate.id)===text(id))).filter(Boolean);
  if(current.length!==ids.length)return null;
  return materialSourceDigest(current);
}
export function isMaterialTruthSourceCurrent(node={},assetNodes=[]){
  const expected=text(node.metadata?.material_truth_source_asset_version_digest);
  if(!expected)return false;
  return text(currentMaterialSourceDigest(node,assetNodes))===expected;
}
function approvedMaterialNode(node={},assetNodes=[]){
  return materialCandidateNode(node) &&
    node.status==="APPROVED" &&
    node.review?.approved===true &&
    node.metadata?.material_truth_pack_qc_sealed===true &&
    node.metadata?.release_approved===true &&
    isMaterialTruthSourceCurrent(node,assetNodes);
}
...
export async function reconcileMaterialTruthReferences({organization_id,creative_project_id}={}){
  ...
  for(const task of list(tasks).filter(t=>t.metadata?.material_truth_reference===true&&text(t.status)==="COMPLETED")){
    const sourceIds=list(
      task.metadata?.material_truth_source_asset_node_ids ||
      task.input?.requirements?.material_truth_reference?.source_asset_node_ids
    );
    const expectedSourceDigest=text(
      task.metadata?.material_truth_source_asset_version_digest ||
      task.input?.requirements?.material_truth_reference?.source_asset_version_digest
    );
    const currentSources=sourceIds
      .map(id=>nodes.find(candidate=>text(candidate.id)===text(id)))
      .filter(Boolean);
    const sourceCurrent=
      Boolean(expectedSourceDigest) &&
      currentSources.length===sourceIds.length &&
      materialSourceDigest(currentSources)===expectedSourceDigest;
    if(!sourceCurrent){
      blocked.push({
        task_id:task.id,
        reason:"MATERIAL_TRUTH_SOURCE_STALE_BEFORE_RECONCILIATION",
        source_asset_node_ids:sourceIds,
      });
      await ProductionTaskRuntime.update(task.id,{
        status:"FAILED",
        error:"IMAGE_MATERIAL_TRUTH_SOURCE_STALE",
        metadata:{
          ...object(task.metadata),
          material_truth_source_stale:true,
          material_truth_source_stale_at:"RECONCILIATION",
        },
      });
      continue;
    }
    ...
    await AssetGraphRepository.update(node.id,{
      ...
      metadata:{
        ...
        material_truth_source_asset_versions:list(
          task.metadata?.material_truth_source_asset_versions ||
          task.input?.requirements?.material_truth_reference?.source_asset_versions
        ),
        material_truth_source_asset_version_digest:
          task.metadata?.material_truth_source_asset_version_digest ||
          task.input?.requirements?.material_truth_reference?.source_asset_version_digest ||
          null,
...
export async function ensureMaterialTruthQc({organization_id,creative_project_id}={}){
  ...
  for(const node of list(nodes).filter(materialCandidateNode)){
    const current=isMaterialTruthSourceCurrent(node,nodes);
    if(!current){
      if(
        node.status==="APPROVED" ||
        node.metadata?.material_truth_pack_qc_sealed===true ||
        ...
        await AssetGraphRepository.update(node.id,{
          ...
          review:{
            ...object(node.review),
            approved:false,
            notes:"Material truth source authority changed; prior material reference revoked and must be regenerated.",
          },
          metadata:{
            ...object(node.metadata),
            material_truth_source_stale:true,
  ...
  for(const [groupKey,assets] of groups){
    ...
    const packHash=hash({
      ...
      assets:assets.map(a=>({
        id:a.id,
        checksum:a.technical?.checksum||null,
        key:a.metadata?.material_truth_key||null,
        source_asset_version_digest:a.metadata?.material_truth_source_asset_version_digest||null,
...
export async function reconcileMaterialTruthQc({organization_id,creative_project_id}={}){
  ...
  for(const task of list(tasks).filter(t=>t.metadata?.material_truth_qc===true&&text(t.status)==="COMPLETED"&&t.metadata?.material_truth_qc_reconciled!==true)){
    ...
    const materialIds=list(task.metadata?.material_asset_node_ids);
    const reviewedNodes=materialIds
      .map(id=>nodes.find(node=>text(node.id)===text(id)))
      .filter(Boolean);
    const sourcesCurrent=
      reviewedNodes.length===materialIds.length &&
      reviewedNodes.every(node=>isMaterialTruthSourceCurrent(node,nodes));
    const pass=sourcesCurrent &&
      result?.passed===true &&
    ...
    const sealHash=pass?hash({
      contract:CREATIVE_IMAGE_MATERIAL_TRUTH_QC_CONTRACT,
      pack_hash:task.metadata?.material_truth_pack_hash,
      source_asset_version_digests:reviewedNodes
        .map(node=>node.metadata?.material_truth_source_asset_version_digest||null)
        .filter(Boolean)
        .sort(),
      result,
    }):null;
    for(const id of list(task.metadata?.material_asset_node_ids)){
      ...
      await AssetGraphRepository.update(id,{
        ...
        metadata:{
          ...
          material_truth_source_stale:!sourcesCurrent,
    ...
    await ProductionTaskRuntime.update(task.id,{
      ...
      ...(pass?{}:{
        status:"FAILED",
        error:sourcesCurrent
          ?"IMAGE_MATERIAL_TRUTH_QC_FAILED"
          :"IMAGE_MATERIAL_TRUTH_SOURCE_STALE_DURING_QC",
      }),
    });
...
export function selectMaterialTruthPack({
  ...
  const assets=list(asset_nodes).filter(node=>{
    if(!approvedMaterialNode(node,asset_nodes)||node.metadata?.material_truth_pack_qc_sealed!==true) return false;
  ...
  return {
    ...
    assets:assets.map(node=>({
      ...
      source_asset_version_digest:node.metadata?.material_truth_source_asset_version_digest||null,
      source_current:true,
    })),
