...
export async function ensureMaterialTruthReferences({organization_id,creative_project_id}={}){
  ...
  for(const [group,groupTasks] of groups){
    ...
    for(const key of materialKeys(groupTasks)){
      const identity=`material-truth:${group}:${key.toLowerCase()}`;
      ...
      const prompt=generationPrompt({key,group,tasks:groupTasks});
      const task=await ProductionTaskRuntime.create({
        ...
        title:`Material Truth · ${key.replaceAll("_"," ")}`,
        ...
        input:{
          ...
          requirements:{
            ...
            material_truth_reference:{
              ...
              source_asset_node_ids:sourceNodes.map(node=>node.id),
          ...
          provider_parameters:{
            ...
            reference_images:sourceNodes.map(node=>node.url),
            source_asset_node_ids:sourceNodes.map(node=>node.id),
            derive_from_selected_visual_authority:true,
          },
        },
        ...
        metadata:{
          ...
          material_truth_source_asset_node_ids:sourceNodes.map(node=>node.id),
...
export async function ensureMaterialTruthQc({organization_id,creative_project_id}={}){
  ...
  for(const [group,assets] of groups){
    const packHash=hash(assets.map(a=>({id:a.id,checksum:a.technical?.checksum||null,key:a.metadata?.material_truth_key||null})).sort((a,b)=>a.id.localeCompare(b.id)));
    ...
    const task=await ProductionTaskRuntime.create({
      ...
      type:"QUALITY_REVIEW",status:"WAITING",title:`Review Material Truth Pack · ${group}`,
      ...
      input:{
        ...
        requirements:{material_truth_qc:true,continuity_group_id:group,material_asset_node_ids:assets.map(a=>a.id),material_truth_pack_hash:packHash,fail_closed:true},
      ...
      metadata:{
        ...
        continuity_group_id:group,material_asset_node_ids:assets.map(a=>a.id),
...
export function selectMaterialTruthPack({asset_nodes=[],continuity_group_id}={}){
  const assets=list(asset_nodes).filter(node=>
    approvedMaterialNode(node) &&
    node.metadata?.material_truth_pack_qc_sealed===true &&
    text(node.metadata?.continuity_group_id||node.metadata?.image_asset_authority?.continuity_group_id)===text(continuity_group_id)
  );
  ...
  return {
    ...
    qc_seal_hash:assets[0]?.metadata?.material_truth_pack_qc_seal_hash||null,
