...
export function buildImageProductionPackage({task={},asset_nodes=[],previsualization_authority=null}={}){
  ...
  const failures=[];
  if(human(r)&&!governedIdentityKey&&governedActorKeys.length>1){
    failures.push("PRODUCTION_PACKAGE_SUBJECT_IDENTITY_REQUIRED");
  }
  ...
  if(human(r)){
    ...
    const base=find(asset_nodes,{scene_id:sceneId,shot_id:null,continuity_group_id:group,identity_key:identityKey||null,asset_classes:["CHARACTER_SHEET"],downstream_role:"VIDEO"});
    ...
    performanceReference=find(asset_nodes,{
      scene_id:sceneId,
      shot_id:shotId,
      continuity_group_id:group,
      foundation_authority_digest:currentFoundationDigest,
      asset_classes:["PERFORMANCE_REFERENCE"],
      downstream_role:"VIDEO",
    });
    ...
    if(contact(r)){
      contactReference=find(asset_nodes,{
        scene_id:sceneId,
        shot_id:shotId,
        continuity_group_id:group,
        foundation_authority_digest:currentFoundationDigest,
        asset_classes:["CONTACT_DETAIL_REFERENCE"],
        downstream_role:"VIDEO",
      });
  ...
  let threatPack=null;
  let governedThreatKey=foundationAuthority.threat_identity_key||requestedThreatKey||null;
  if(threat(r)){
    ...
    const base=threatSelection.selected;
    if(base&&!governedThreatKey) governedThreatKey=text(base.metadata?.threat_identity_key)||null;
    threatPack=base?CreativeImageAssetMultiViewHandoffRuntime.select({asset_nodes,parent_asset_node_id:base.id,continuity_group_id:group}):null;
    if(threatPack?.complete!==true) failures.push("PRODUCTION_PACKAGE_THREAT_MULTIVIEW_REQUIRED");
  }
  const materials=materialHeavy(r)?CreativeImageMaterialTruthPackRuntime.select({asset_nodes,continuity_group_id:group,identity_key:governedIdentityKey,threat_identity_key:governedThreatKey}):null;
