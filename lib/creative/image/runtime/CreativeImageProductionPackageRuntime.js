...
export function buildImageProductionPackage({task={},asset_nodes=[],previsualization_authority=null}={}){
  ...
  let threatPack=null;
  if(threat(r)){
    const base=find(asset_nodes,{scene_id:sceneId,shot_id:null,continuity_group_id:group,asset_classes:["THREAT_DESIGN"],downstream_role:"VIDEO"});
  ...
  const materials=materialHeavy(r)?CreativeImageMaterialTruthPackRuntime.select({asset_nodes,continuity_group_id:group}):null;
