...
async function intelligencePlan({
  ...
  try {
      ...
      outputShape: {
        result: {
          ...
          strategic_interpretation: {
            human_truth: "string", central_tension: "string", category_convention_response: "string",
            brand_asset_strategy: "string", attention_strategy: "string", creative_mandate: "string", unknowns: ["string"]
          },
          evidence_decision_trace: [{ decision: "string", evidence_source_ids: ["string"], consequence: "string" }],
        },
      },
      temperature: 0.45,
    });
    ...
    const researchSources = new Set(list(research?.sources).map((entry) => text(entry?.id)).filter(Boolean));
    const strategicResearch = object(research?.strategic_synthesis);
    const strategicInterpretation = object(result.strategic_interpretation);
    const evidenceDecisionTrace = list(result.evidence_decision_trace);
    if (Object.keys(strategicResearch).length) {
      for (const field of ["human_truth", "central_tension", "category_convention_response", "brand_asset_strategy", "attention_strategy", "creative_mandate"]) {
        if (!text(strategicInterpretation[field])) {
          throw new Error(`CREATIVE_INTELLIGENCE_STRATEGIC_INTERPRETATION_REQUIRED:${field}`);
        }
      }
      if (!evidenceDecisionTrace.length) {
        throw new Error("CREATIVE_INTELLIGENCE_EVIDENCE_DECISION_TRACE_REQUIRED");
      }
      const invalidEvidenceIds = evidenceDecisionTrace
        .flatMap((entry) => list(entry?.evidence_source_ids))
        .map(text)
        .filter(Boolean)
        .filter((id) => !researchSources.has(id));
      if (invalidEvidenceIds.length) {
        throw new Error(`CREATIVE_INTELLIGENCE_EVIDENCE_SOURCE_INVALID:${[...new Set(invalidEvidenceIds)].join(",")}`);
      }
      if (evidenceDecisionTrace.some((entry) => !text(entry?.decision) || !text(entry?.consequence) || !list(entry?.evidence_source_ids).length)) {
        throw new Error("CREATIVE_INTELLIGENCE_EVIDENCE_DECISION_TRACE_INCOMPLETE");
      }
    }
    return {
      ...
      evidence_decision_trace: evidenceDecisionTrace.slice(0, 20),
    ...
    if (Object.keys(object(research)).length) {
      const failure = new Error(`CREATIVE_INTELLIGENCE_RESEARCH_BOUND_REASONING_FAILED:${text(error?.message || error)}`);
      failure.cause = error;
      throw failure;
    }
    return {
...
export const CreativeIntelligenceRuntime = {
  ...
  async createCreativePlan({
    ...
    const researchPacket = object(research);
    if (Object.keys(researchPacket).length && researchPacket.validation?.passed !== true) {
      throw new Error("CREATIVE_INTELLIGENCE_VALIDATED_RESEARCH_REQUIRED");
    }
    ...
    return {
      ...
      strategic_interpretation: plan.strategic_interpretation || {},
      evidence_decision_trace: plan.evidence_decision_trace || [],
      research_bound: Object.keys(researchPacket).length > 0,
      requested_outputs: explicitOutputs,
