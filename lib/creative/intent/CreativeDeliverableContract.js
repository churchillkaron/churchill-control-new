...
export function resolveCreativeDeliverableMedium(deliverable = {}) {
  ...
  if (/hero.*film|film.*hero|film[_\s-]*master|master[_\s-]*(film|video)|social.*cutdown|cutdown.*social|reels?|shorts?|trailer|episode/.test(identity)) {
    return "FILM";
  }
  if (/scene.*still|still.*scene|keyframes?|master.*still|still.*pack|key.*art|image.*system|approval.*frame/.test(identity)) {
    return "IMAGE";
  }
  if (/sound.*design|sounddesign|audio.*pack|stem.*pack|music.*pack|voice.*pack/.test(identity)) {
    return "AUDIO";
  }
  if (/typograph|brand.*endcard|endcard.*system|motion.*graphic|animated.*identity/.test(identity)) {
    return "MULTIMEDIA";
  }
  ...
  if (/image_to_video|video\.generate|\bvideo\b|\bfilm\b|\b24[- ]?30fps\b|\b30fps\b|1080x1920/.test(technical)) {
    return "FILM";
  }
  if (/\.wav|\.mp3|48khz|music\.generate|sfx\.generate/.test(technical) &&
      !/image_to_video|video\.generate|\bvideo\b|\bfilm\b/.test(technical)) {
    return "AUDIO";
  }
  if (/png|jpe?g|keyframe|image\.generate|image\.analyze/.test(technical) &&
      !/image_to_video|video\.generate|\bvideo\b|\bfilm\b/.test(technical)) {
    return "IMAGE";
  }
...
function canonicalTitle(deliverable, medium) {
  ...
  if (supplied && !GENERIC_TITLE_PATTERN.test(supplied)) {
    if (medium === "FILM" && /music|sound package/i.test(supplied)) {
      return /cutdown|reel|short/i.test(identityText(deliverable))
        ? "Social Campaign Cutdown Series"
        : "Churchill Cinematic Hero Film";
    }
    return supplied;
  }
  ...
  const identity = identityText(deliverable);
  if (medium === "FILM") {
    return /cutdown|reel|short/i.test(identity)
      ? "Social Campaign Cutdown Series"
      : "Churchill Cinematic Hero Film";
  }
  if (medium === "IMAGE") return "Scene Master Stills & Approval Frames";
  if (medium === "AUDIO") return "Cinematic Sound Design Master Pack";
  if (medium === "MULTIMEDIA") return "Motion Typography & Brand Endcard System";
...
function description(deliverable, title, medium) {
  ...
  if (supplied && !wrongMedium && !generic) return supplied;
