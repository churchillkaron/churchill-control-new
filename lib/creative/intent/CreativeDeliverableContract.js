...
function idText(deliverable = {}) {
  return String(deliverable.id || "").trim().toLowerCase();
}
...
function mediumFromId(deliverable = {}) {
  const id = idText(deliverable);
  if (!id) return null;

  if (
    /^(?:film|video)(?:[-_]|$)/.test(id) ||
    /(?:^|[-_])(?:cutdown|reel|short|trailer|episode)(?:[-_]|$)/.test(id)
  ) {
    return "FILM";
  }
  if (
    /^(?:still|stills|image|keyframe|keyframes|keyart)(?:[-_]|$)/.test(id) ||
    /(?:^|[-_])(?:still|stills|keyframe|keyframes|keyart)(?:[-_]|$)/.test(id)
  ) {
    return "IMAGE";
  }
  if (
    /^(?:audio|music|sound|stem|voice)(?:[-_]|$)/.test(id) ||
    /(?:^|[-_])(?:audio|sounddesign|stem)(?:[-_]|$)/.test(id)
  ) {
    return "AUDIO";
  }
  if (
    /^(?:multi|multimedia|motion)(?:[-_]|$)/.test(id) ||
    /(?:^|[-_])(?:typography|endcard|motiongraphics)(?:[-_]|$)/.test(id)
  ) {
    return "MULTIMEDIA";
  }
  if (/^(?:website|web)(?:[-_]|$)/.test(id)) return "WEBSITE";
  if (/^menu(?:[-_]|$)/.test(id)) return "MENU";
  if (/^(?:document|copy|script)(?:[-_]|$)/.test(id)) return "DOCUMENT";
  if (/^(?:presentation|deck)(?:[-_]|$)/.test(id)) return "PRESENTATION";
  return null;
}
...
function canonicalTitle(deliverable, medium) {
  ...
  if (medium === "FILM") {
    if (/social|cutdown|reel|short|dj/.test(identity)) {
