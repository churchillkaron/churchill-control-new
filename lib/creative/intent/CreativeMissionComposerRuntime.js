...
const UNIVERSAL_WORKSPACES = new Set(WORKSPACE_ORDER);
...
const RIGHTS_ASSERTION_PATTERN = /(owns? all|fully owns?|rights?[- ]?cleared|reference[- ]?cleared|full[- ]?rights|royalty[- ]?free|licensed(?: for)?|all rights secured|commercial rights secured|permission secured|cleared for campaign use)/i;

const RIGHTS_DEPENDENCY_PATTERN =
  /\b(approved|licensed|cleared|owned|permission|rights?)\b.*\b(asset|music|voice|logo|likeness|image|video|track)\b|\b(asset|music|voice|logo|likeness|image|video|track)\b.*\b(approved|licensed|cleared|owned|permission|rights?)\b/i;

// CREATIVE_MISSION_DIRECTOR_CONTEXT_BUDGET_V8
// CREATIVE_MISSION_DIRECTOR_LIST_HELPER_V8_2
function list(value) {
...
function structuredArray(value, label = "Decision") {
  ...
  return value
    .map((item, index) => {
      ...
      return title
        ? {
            id: `${label.toLowerCase().replace(/\W+/g, "_")}_${index + 1}`,
            title,
          }
        : null;
    })
...
function deliverableFingerprint(item = {}) {
  return [
    item.id,
    item.title,
    item.name,
    item.medium,
    item.form,
    ...stringArray(item.formats),
    ...stringArray(item.channels),
    ...stringArray(item.capabilities),
    ...stringArray(item.execution_capabilities),
    item.metadata?.purpose,
    item.metadata?.for,
    item.specifications?.purpose,
    item.specifications?.files,
    item.specifications?.tracks,
  ]
    .flat(Infinity)
    .filter(Boolean)
    .join(" ")
...
function resolveDeliverableMedium(item = {}, request = "") {
  ...
  const fingerprint =
    deliverableFingerprint(item);

  if (
    /social\s*cutdown|reels?|shorts?|short\s*video|episode|film|video|movie|cinema|trailer|image_to_video|video\.generate/.test(
      fingerprint,
    )
  ) {
    return "FILM";
  ...
  if (
    /typograph|brand\s*kit|brandkit|logo\s*system|endcard|lottie|motion\s*graphic/.test(
      fingerprint,
    )
  ) {
    return "MULTIMEDIA";
  ...
  if (
    /scene\s*still|stills?\s*pack|master\s*still|approval\s*frame|key\s*art|image\s*suite|photo|poster|\.png|\.jpe?g/.test(
      fingerprint,
    )
  ) {
    return "IMAGE";
  ...
  if (
    /sound\s*design|sounddesign|audio|music|voice|\.wav|\.mp3|sfx|foley/.test(
      fingerprint,
    )
  ) {
    return "AUDIO";
  ...
  if (
    /website|webpage|landing|web\s*experience/.test(
      fingerprint,
    )
  ) {
    return "WEBSITE";
  ...
  if (/menu/.test(fingerprint)) {
    return "MENU";
  }

  if (/document|copy|script/.test(fingerprint)) {
    return "DOCUMENT";
  }

  if (/presentation|deck/.test(fingerprint)) {
    return "PRESENTATION";
  }

  return inferredMedium(request);
}

function sanitizeEvidenceClaims(value) {
  if (typeof value === "string") {
    return value
      .replace(
        /assumed\s+(?:to\s+be\s+)?cleared(?:\s+by\s+evidence\s+as\s+uploaded)?/gi,
        "requires explicit rights-evidence verification",
      )
      .replace(
        /\breference[- ]cleared\b/gi,
        "reference-grounded; rights evidence required",
      )
      .replace(
        /\bfull[- ]rights\b/gi,
        "synthetic source; rights evidence required",
      )
      .replace(
        /\bbrand[- ]owned\b/gi,
        "brand-aligned; rights evidence required",
      )
      .replace(
        /\bapproved\s+score\b/gi,
        "score pending rights-evidence verification",
      )
      .replace(
        /approved visual,\s*music,?\s*and voice assets/gi,
        "referenced visual, music, and voice evidence with rights verification pending",
      )
      .replace(
        /\bcleared\s+for\s+(commercial|campaign|public)\s+use\b/gi,
        "pending explicit rights-evidence verification",
      );
  }

  if (Array.isArray(value)) {
    return value.map(
      sanitizeEvidenceClaims,
    );
  }
  ...
  if (
    ...
    for (
      ...
      if (
        key === "approved_assets_used"
      ) {
        output.asset_evidence_status =
          item === true
            ? "UNVERIFIED"
            : sanitizeEvidenceClaims(item);
        continue;
      }

      if (
        /^(?:full_rights|rights_cleared|reference_cleared)$/i.test(
          key,
        )
      ) {
        output[`${key}_status`] =
...
function dependencyText(value = null) {
  if (
    typeof value === "string" ||
    typeof value === "number"
  ) {
    return compactText(
      sanitizeEvidenceClaims(value),
      240,
    );
  }
  ...
  if (
    ...
    return compactText(
      sanitizeEvidenceClaims(candidate),
      240,
    );
  }
...
function normalizeDependencies(value) {
  ...
  const filtered = supplied.filter(
    (dependency) =>
      !RIGHTS_DEPENDENCY_PATTERN.test(
        dependency,
      ),
  );

  if (
    filtered.length !== supplied.length
  ) {
    filtered.push(
      "Rights evidence must be verified before any referenced asset is publicly released.",
    );
  }
...
function defaultSuccessCriteria(item = {}, medium = null) {
  const supplied = stringArray(item.success_criteria);
  ...
  if (resolved === "FILM") {
    return [
      ...
      "Picture, voice, music, foley, sound effects, typography, and channel formatting pass full-output quality review before release.",
    ];
  }

  if (resolved === "IMAGE") {
    return [
      "Composition, subject identity, product truth, typography, resolution, and channel crop pass visual quality review.",
      "Every released image is traceable to approved business truth and reference evidence.",
    ];
  }

  if (resolved === "AUDIO") {
    return [
      "Music, voice, ambience, and sound effects pass technical, creative, and rights-evidence review before release.",
      "The final mix supports the intended narrative without clipping, masking, abrupt endings, or unusable channel levels.",
    ];
  }

  if (resolved === "MULTIMEDIA") {
    return [
      "Motion identity, typography, timing, transparency, format, and brand consistency pass full playback review.",
      "The system integrates cleanly with every intended master and channel version.",
    ];
  }

  return [
    "The complete deliverable passes evidence-based quality review before release.",
    "The released output remains traceable to its mission, business truth, source evidence, and production lineage.",
  ];
}

function productionRoleFingerprint(
  item = {},
) {
  return [
    deliverableFingerprint(item),
    ...
    item.metadata?.social_platform,
    item.metadata?.derivative_policy,
  ]
    .flat(Infinity)
    .filter(Boolean)
    .join(" ")
...
function deliverableProductionRole(
  item = {},
  medium = null,
) {
  const resolved =
    medium ||
    resolveDeliverableMedium(item);
  ...
  const explicit = String(
    item.metadata?.production_role || "",
  ).toUpperCase();

  if (
    ["MASTER", "CUTDOWN"].includes(
      explicit,
    )
  ) {
    return explicit;
  ...
  const fingerprint =
    productionRoleFingerprint(item);

  if (
    item.metadata?.master_version === true ||
    item.metadata?.hero === true ||
    /\bhero\b|film[_\s-]*master|master[_\s-]*(film|video)|(?:^|[_\s-])master_film(?:$|[_\s-])/.test(
      fingerprint,
    ) &&
    !/derived|adapted|cutdown|reel|short|feed|story/.test(
      fingerprint,
    )
  ) {
    return "MASTER";
  ...
  if (
    ...
    /social[_\s-]*cutdown|reels?|shortform|short[_\s-]*video|stories|shorts|facebook[_\s-]*feed|instagram[_\s-]*feed/.test(
      fingerprint,
    ) ||
    /derived?\s+from\s+(?:the\s+)?master|adapted\s+from\s+(?:the\s+)?master|source.*master\s+(?:film|timeline|edit)|master[_\s-]*(?:film|timeline|edit|project)/.test(
      fingerprint,
    )
  ) {
    return "CUTDOWN";
...
function titleConflictsWithMedium(
  value = "",
  medium = "",
) {
  ...
  if (medium === "AUDIO") {
    return /film|video|movie|hero|reel|cutdown|trailer|stills?|keyframe|poster|website|menu/.test(
      supplied,
    );
  }

  if (medium === "IMAGE") {
    return /audio|music|voice|sound\s*pack|film|video|movie|reel|cutdown/.test(
      supplied,
    );
  }

  if (medium === "FILM") {
    return /audio\s*pack|music\s*pack|voice\s*pack|sound\s*pack|stills?\s*pack|keyframe\s*pack/.test(
      supplied,
    );
  }
...
function deliverableTitle(
  ...
  const productionRole =
    deliverableProductionRole(
      item,
      resolvedMedium,
    );

  const supplied = compactText(
  ...
  if (productionRole === "MASTER") {
    if (
      supplied &&
      !titleConflictsWithMedium(
        supplied,
        "FILM",
      ) &&
      !/cutdown|reel|short|stories/i.test(
        supplied,
      )
    ) {
      return supplied;
    ...
    return "Cinematic Hero Film";
  }

  if (productionRole === "CUTDOWN") {
    if (
      supplied &&
      /cutdown|reel|short|stories|feed/i.test(
        supplied,
      )
    ) {
      return supplied;
  ...
  if (resolvedMedium === "AUDIO") {
    if (
      /voice|voiceover|narrat/.test(
        fingerprint,
      )
    ) {
      return "Campaign Voiceover Package";
    }

    if (
      /music|score|sound|audio|sfx|foley/.test(
        fingerprint,
      )
    ) {
      return "Campaign Music & Sound Package";
    }
    ...
    return "Campaign Audio Package";
  }

  if (
    resolvedMedium === "IMAGE" &&
    /scene[_\s-]*stills?|stills?[_\s-]*pack|approval[_\s-]*frame|keyframe|key[_\s-]*art/.test(
      fingerprint,
    )
  ) {
  ...
  if (
    ...
    /typograph|branding|brand[_\s-]*pack|endcard|logo|motion/.test(
      fingerprint,
    )
  ) {
  ...
  if (
    supplied &&
    !/^(?:creative\s+)?deliverable\s*\d*$/i.test(
      supplied,
    ) &&
    !titleConflictsWithMedium(
      supplied,
  ...
  switch (resolvedMedium) {
    case "FILM":
      return "Cinematic Hero Film";
    case "IMAGE":
      return "Campaign Key Art & Scene Stills";
    case "AUDIO":
      return "Campaign Music & Sound Package";
    case "WEBSITE":
      return "Campaign Web Experience";
    case "MENU":
      return "Campaign Menu System";
    case "MULTIMEDIA":
      return "Campaign Motion Graphics System";
    default:
      return `Creative Output ${index + 1}`;
  }
}

function deliverableFormats(item = {}, medium = null) {
  ...
  const specifications = item.specifications && typeof item.specifications === "object"
    ? item.specifications
    : {};
  if (Array.isArray(specifications.sizes)) return stringArray(specifications.sizes);
  if (Array.isArray(specifications.files)) return stringArray(specifications.files);

  const resolved = medium || resolveDeliverableMedium(item);
  const channels = stringArray(item.channels).join(" ").toLowerCase();

  if (resolved === "FILM" && /instagram|facebook|tiktok|social/.test(channels)) {
    return ["9:16 vertical", "4:5 feed", "1:1 square"];
  }
  if (resolved === "FILM") return ["16:9 master"];
  if (resolved === "IMAGE") return ["16:9", "4:5", "9:16"];
  if (resolved === "AUDIO") return ["WAV", "MP3"];
  if (resolved === "MULTIMEDIA") return ["MOV", "PNG sequence", "Lottie JSON"];
  return [];
}

function requiredExecutionCapabilities(item = {}, medium = null) {
  const resolved = medium || resolveDeliverableMedium(item);
  ...
  if (resolved === "MULTIMEDIA" && /typograph|brand|endcard|logo|motion/.test(fingerprint)) {
    return ["ai.text.generate", "ai.image.generate", "ai.image.analyze"];
  }
  if (["WEBSITE", "MENU", "DOCUMENT", "PRESENTATION"].includes(resolved)) {
    return ["ai.text.generate", "ai.image.generate", "ai.image.analyze"];
  }
...
function executionCapabilities(
  ...
  const supplied =
    stringArray(
      item.execution_capabilities,
    ).filter(
      (capability) =>
        capability.includes("."),
    );

  let compatible = supplied;

  if (resolved === "AUDIO") {
    compatible = supplied.filter(
      (capability) =>
        /^ai\.(?:audio|music|sfx|voice|text|reasoning)\./.test(
          capability,
        ),
    );
  }
...
function deliverableDescription(item = {}, request = "", index = 0, medium = null) {
  const supplied = String(item.description || item.objective || "").trim();
  if (supplied && !sameMeaning(supplied, request)) return supplied;

  const resolved = medium || resolveDeliverableMedium(item, request);
  const title = deliverableTitle(item, index, resolved);
  const channels = stringArray(item.channels);
  const formats = deliverableFormats(item, resolved);
  const channelText = channels.length ? ` for ${channels.join(", ")}` : "";
  const formatText = formats.length ? ` in ${formats.join(", ")}` : "";

  return `Produce ${title} as a ${resolved}${channelText}${formatText}, grounded in approved business truth and reference assets, with its own purpose, craft, quality evidence, and release contract.`;
}
...
function normalizeDeliverables(value, request) {
  ...
  const normalized = source.map((item, index) => {
    const medium =
      resolveDeliverableMedium(item, request);
    const productionRole =
      deliverableProductionRole(item, medium);

    return {
      id: String(
        item?.id || `deliverable_${index + 1}`,
      ),
      title:
        deliverableTitle(item, index, medium),
      description:
      ...
      medium,
      formats:
        deliverableFormats(item, medium),
      channels:
        stringArray(item?.channels),
      capabilities:
        stringArray(item?.capabilities),
      execution_capabilities:
        executionCapabilities(item, medium),
      dependencies:
        normalizeDependencies(item?.dependencies),
      success_criteria:
        defaultSuccessCriteria(item, medium),
      specifications:
        sanitizeEvidenceClaims(
          item?.specifications &&
  ...
  const films = normalized.filter(
    (deliverable) =>
      deliverable.medium === "FILM",
  );
  ...
  if (films.length) {
    ...
    for (const deliverable of films) {
      if (deliverable === master) {
        deliverable.metadata
          .production_role = "MASTER";
        deliverable.metadata
          .derivative_policy = null;
        continue;
      }
...
function workflowItem(item) {
  ...
  return {
    ...
    title: compactText(
      object.title || object.name || (typeof item === "string" ? item : ""),
      80,
    ),
...
function normalizeWorkflow(value) {
  ...
  const defaults = new Map(
    DEFAULT_WORKFLOW.map((item) => [item.workspace_id, item]),
  );
...
function normalizeAssumptions(value) {
  return stringArray(value)
    .filter(
      (assumption) =>
        !RIGHTS_ASSERTION_PATTERN.test(
          assumption,
        ),
    )
    .map(sanitizeEvidenceClaims);
}

function ensureRightsDecisionGate(value) {
  const gates = structuredArray(
    value,
    "Decision Gate",
  ).map((gate, index) => {
    ...
    if (
      /assumed\s+(?:to\s+be\s+)?cleared|approved\s+score|brand[- ]owned|rights?[- ]?cleared\s+by\s+evidence\s+as\s+uploaded/i.test(
        evidenceText,
      )
    ) {
      return {
        id:
          gate?.id ||
          `rights_evidence_gate_${index + 1}`,
        title:
          "Verify asset rights evidence before release",
        description:
          "Uploading an asset is not proof of ownership, permission, music clearance, likeness consent, or commercial usage rights. Verify explicit evidence or replace the asset before release.",
      };
    }
    ...
    return {
      ...
      id:
        gate?.id ||
        `decision_gate_${index + 1}`,
      title:
        title ||
        `Decision Gate ${index + 1}`,
      description:
        description || null,
    };
  });

  const hasRightsGate = gates.some(
    (gate) =>
      /right|license|likeness|music|ownership|permission/i.test(
  ...
  if (!hasRightsGate) {
    gates.push({
      id: "rights_evidence_gate",
      title:
        "Verify rights evidence before publishing",
      description:
        "Confirm ownership or permission for every logo, likeness, music track, voice, uploaded asset, and third-party reference before public release.",
    });
  }
...
function fallbackBlueprint({ request, reason = null, details = null }) {
  return {
    ...
    assumptions: [],
    blocking_questions: [],
    decision_gates: ensureRightsDecisionGate([]),
    optional_real_world_extensions: [],
...
function normalizeBlueprint(value, request) {
  ...
  return {
    ...
    decision_gates: ensureRightsDecisionGate(source.decision_gates),
    optional_real_world_extensions: structuredArray(
      source.optional_real_world_extensions,
...
function directorJson(execution = {}) {
  const candidates = [
...
- Design multiple scenes and shots with real durations, action beats, camera movement, performance, lighting, continuity, typography, music, foley, sound effects, transitions, climax, and channel-specific versions.
...
- Never claim an asset, music track, likeness, logo, location, voice, or effect is owned, licensed, royalty-free, or rights-cleared unless the supplied evidence explicitly proves it. Put unresolved rights or safety matters in decision_gates.
...
Use the correct medium for each independent output:
- hero films, reels, cutdowns: FILM
- keyframes, still packs, posters, key art: IMAGE
- music, sound-design, voice, SFX packs: AUDIO
- motion typography, animated identity, endcard systems: MULTIMEDIA
Never label every output FILM simply because it supports a film campaign.
...
Every FILM deliverable must include ai.image.generate, ai.image.analyze, ai.video.image_to_video or ai.video.generate, ai.music.generate, and ai.sfx.generate.
Every IMAGE deliverable must include ai.image.generate and ai.image.analyze.

Never use generic titles such as Deliverable 1 or Creative Deliverable 1. Never repeat the full user request as every deliverable description. Never return OPEN when the request clearly names a medium.
...
${JSON.stringify(compactMissionDirectorContext(context || {}))}
    ...
    if (!parsed) {
      return fallbackBlueprint({
        ...
        reason: "AI_DIRECTOR_INVALID_JSON",
      });
    }
