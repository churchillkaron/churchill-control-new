...
function deliverableTitle(item = {}, index = 0, medium = null) {
  ...
  switch (medium || resolveDeliverableMedium(item)) {
    case "FILM":
...
function normalizeDeliverables(value, request) {
  ...
  return source.map((item, index) => {
    ...
    return {
      ...
      description: deliverableDescription(item, request, index, medium),
      medium,
      formats: deliverableFormats(item, medium),
      channels: stringArray(item?.channels),
      capabilities: stringArray(item?.capabilities),
      execution_capabilities: executionCapabilities(item, medium),
...
${JSON.stringify(context || {})}
    ...
    const parsed = parseJson(directorText(execution));
    if (!parsed) {
