...
function normalizeDeliverables(value, request) {
  ...
  return source.map((item, index) => ({
    ...
    title: compactText(item?.title, 100) || `Deliverable ${index + 1}`,
...
function normalizeWorkflow(value) {
  if (!Array.isArray(value) || !value.length) return DEFAULT_WORKFLOW;
  const workflow = value.map((item, index) => {
    const workspaceId = String(item?.workspace_id || item?.workspace || item?.id || "production").trim().toLowerCase();
    if (!UNIVERSAL_WORKSPACES.has(workspaceId)) return null;
    return {
      id: String(item?.id || `${workspaceId}_${index + 1}`),
      workspace_id: workspaceId,
      title: compactText(item?.title || item?.name, 80) || workspaceId.replace(/_/g, " "),
  ...
  if (!workflow.some((item) => item.workspace_id === "mission")) workflow.unshift(DEFAULT_WORKFLOW[0]);
...
function normalizeBlueprint(value, request) {
  ...
  return {
    ...
    audience: source.audience && typeof source.audience === "object" ? source.audience : {},
...
Every deliverable must name a concrete medium, formats, channels, capabilities, dependencies, success criteria, specifications and metadata. Never return OPEN when the request clearly names film, video, image, menu, website, banner or another concrete medium.
