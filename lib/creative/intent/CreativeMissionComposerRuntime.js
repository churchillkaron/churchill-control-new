...
function sanitizeEvidenceClaims(value) {
  ...
  if (Array.isArray(value)) {
    return value.map(sanitizeEvidenceClaims);
  }
...
function deliverableProductionRole(item = {}, medium = null) {
  const resolved = medium || resolveDeliverableMedium(item);
  ...
  if (["MASTER", "CUTDOWN"].includes(explicit)) {
    return explicit;
  }
  ...
  if (
    ...
    /\bhero\b|film[_\s-]*master|master[_\s-]*(film|video)/.test(
      fingerprint,
    )
  ) {
    return "MASTER";
  ...
  if (
    ...
    /social[_\s-]*cutdown|reels?|shortform|short[_\s-]*video|stories|shorts/.test(
      fingerprint,
    )
  ) {
    return "CUTDOWN";
...
function deliverableTitle(item = {}, index = 0, medium = null) {
  const resolvedMedium =
    medium || resolveDeliverableMedium(item);
  const fingerprint = deliverableFingerprint(item);
  const productionRole =
    deliverableProductionRole(item, resolvedMedium);
  const supplied =
    compactText(item.title || item.name, 100);

  if (productionRole === "MASTER") {
    if (
      supplied &&
      !/cutdown|reel|short|stories/i.test(supplied)
    ) {
      return supplied;
    }
  ...
  if (/scene[_\s-]*stills?|stills?[_\s-]*pack|approval[_\s-]*frame/.test(fingerprint)) {
    return "Scene Master Stills & Approval Frames";
  }
  if (/social[_\s-]*cutdown|reels?|shortform|short[_\s-]*video/.test(fingerprint)) {
    return "Social Campaign Cutdown Series";
  }
  if (/sound[_\s-]*design|sounddesign|audio[_\s-]*pack/.test(fingerprint)) {
    return "Cinematic Sound Design Master Pack";
  }
  if (/typograph|brand[_\s-]*kit|endcard|logo[_\s-]*system/.test(fingerprint)) {
    return "Motion Typography & Brand Endcard System";
  }
...
function executionCapabilities(item = {}, medium = null) {
  const supplied = stringArray(item.execution_capabilities)
    .filter((capability) => capability.includes("."));
  const required = requiredExecutionCapabilities(item, medium);
  return [...new Set([...supplied, ...required])];
}
  ...
  if (films.length) {
    ...
    for (const deliverable of films) {
      if (deliverable === master) {
        deliverable.metadata.production_role =
          "MASTER";
        deliverable.metadata.derivative_policy =
          null;
        continue;
      }
...
function normalizeAssumptions(value) {
  return stringArray(value).filter(
    (assumption) => !RIGHTS_ASSERTION_PATTERN.test(assumption),
  );
}
...
function normalizeBlueprint(value, request) {
  ...
  return {
    ...
    business_goal: String(
      source.business_goal || source.goal || request,
    ).trim(),
    objective: String(
      source.objective || source.desired_outcome || request,
    ).trim(),
    creative_thesis: String(
      source.creative_thesis || source.thesis || fallback.creative_thesis,
    ).trim(),
