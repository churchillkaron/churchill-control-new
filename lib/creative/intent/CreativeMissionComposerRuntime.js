...
function resolveDeliverableMedium(item = {}, request = "") {
  ...
  if (/sound\s*design|sounddesign|audio|music|voice|\.wav|\.mp3|sfx|foley/.test(fingerprint)) {
    return "AUDIO";
  }
  if (/typograph|brand\s*kit|brandkit|logo\s*system|endcard|lottie|motion\s*graphic/.test(fingerprint)) {
    return "MULTIMEDIA";
  }
  if (/scene\s*still|stills?\s*pack|master\s*still|approval\s*frame|key\s*art|image\s*suite|photo|poster|\.png|\.jpe?g/.test(fingerprint)) {
    return "IMAGE";
  }
  if (/social\s*cutdown|reels?|short\s*video|episode|film|video|movie|cinema|trailer|image_to_video/.test(fingerprint)) {
    return "FILM";
  }
  if (/website|webpage|landing|web\s*experience/.test(fingerprint)) return "WEBSITE";
  if (/menu/.test(fingerprint)) return "MENU";
  if (/document|copy|script/.test(fingerprint)) return "DOCUMENT";
  if (/presentation|deck/.test(fingerprint)) return "PRESENTATION";

  return inferredMedium(request);
}

function normalizeDependencies(value) {
  const supplied = stringArray(value);
  const filtered = supplied.filter(
    (dependency) => !RIGHTS_DEPENDENCY_PATTERN.test(dependency),
  );

  if (filtered.length !== supplied.length) {
    filtered.push(
...
function deliverableProductionRole(item = {}, medium = null) {
  ...
  if (["MASTER", "CUTDOWN", "INDEPENDENT"].includes(explicit)) {
    return explicit;
  }
...
function ensureRightsDecisionGate(value) {
  const gates = structuredArray(value, "Decision Gate");
  const hasRightsGate = gates.some((gate) =>
    /right|license|likeness|music|ownership/i.test(
  ...
  if (!hasRightsGate) {
    gates.push({
      ...
      title: "Verify rights evidence before publishing",
      description:
