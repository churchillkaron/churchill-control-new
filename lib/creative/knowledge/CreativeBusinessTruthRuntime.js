...
function reusableAssetTruth(row = {}) {
  return cleanObject({
    ...
    approved_for_reuse: row.status === "APPROVED" || row.metadata?.approved_for_reuse === true,
    lineage: cleanObject({
...
async function queryReusableAssets(organization_id) {
  const { data, error } = await supabaseAdmin.from("creative_asset_nodes").select("*").eq("organization_id", organization_id).eq("status", "APPROVED").order("created_at", { ascending: false }).limit(MAX_REUSABLE_ASSETS);
  if (error) throw error;
  return data || [];
}
...
export const CreativeBusinessTruthRuntime = {
  async hydrate({ organization_id, entity_id = null, period_id = null, creative_mission_id = null, creative_project_id = null, captured_by = null, persist = true } = {}) {
    ...
    const [organizationResult, currencyResult, locationsResult, assetsResult, reusableResult] = await Promise.allSettled([
    ...
    const sourceManifest = [
      ...
      { source: "creative_asset_nodes", status: reusableResult.status === "fulfilled" ? "LIVE" : "UNAVAILABLE", record_count: reusableAssets.length },
    ];
    ...
    const payload = {
      ...
      organization,
      locations,
      assets: { uploaded_references: uploadedAssets, approved_reusable: reusableAssets },
