...
function reusableAssetTruth(row = {}) {
  return cleanObject({
    ...
    approved_for_reuse: row.reuse?.approved_for_reuse === true,
    lineage: cleanObject({
...
async function queryReusableAssets(organization_id) {
  const { data, error } = await supabaseAdmin
...
function failure(source, result) {
  ...
  return {
    source,
    status: "UNAVAILABLE",
    error: compact(result.reason?.message || result.reason, 500),
  };
}
...
async function persistSnapshot({
  ...
  const row = {
    ...
    metadata: {
      scope_enforced: true,
      field_policy: "ALLOWLISTED_BOUNDED_FACTS",
      prompt_safe: true,
    },
  };
  ...
  const { data, error } = await supabaseAdmin
    ...
    .upsert(row, {
      onConflict:
        "organization_id,payload_hash,creative_mission_id,creative_project_id",
      ignoreDuplicates: false,
    })
    .select("id,payload_hash,captured_at,schema_version")
    .single();
...
export const CreativeBusinessTruthRuntime = {
  async hydrate({
    ...
    const [organizationResult, currencyResult, locationsResult, assetsResult, reusableResult] =
    ...
    const organization = organizationTruth(
      organizationResult.value,
      resultValue(currencyResult, null),
    );
    const locations = resultValue(locationsResult, []).map(locationTruth);
    const uploadedAssets = resultValue(assetsResult, []).map(uploadedAssetTruth);
    ...
    const sourceManifest = [
      ...
      {
        source: "organization_currency",
        status: currencyResult.status === "fulfilled" ? "LIVE" : "UNAVAILABLE",
        record_count: currencyResult.status === "fulfilled" ? 1 : 0,
      },
      {
        source: "business_locations",
        status: locationsResult.status === "fulfilled" ? "LIVE" : "UNAVAILABLE",
        record_count: locations.length,
      },
      {
        source: "creative_assets",
        status: assetsResult.status === "fulfilled" ? "LIVE" : "UNAVAILABLE",
        record_count: uploadedAssets.length,
      },
      {
        source: "creative_asset_nodes",
        status: reusableResult.status === "fulfilled" ? "LIVE" : "UNAVAILABLE",
        record_count: reusableAssets.length,
      },
    ];
    const payload = {
      schema_version: "creative-business-truth-v1",
