...
export function buildTemporalTasteMemory({ learning = {} } = {}) {
  ...
  const imageStudioRecurringFailures = Object.entries(
    productionEvidence.image_studio_failure_frequency || {},
  )
    .map(([reason, count]) => ({ reason: text(reason, 220), count: Number(count) || 0 }))
    .filter((entry) => entry.reason && entry.count >= 2)
    .sort((left, right) => right.count - left.count)
    .slice(0, 24);
  const imageStudioExplorationPairs = list(
    productionEvidence.image_studio_exploration_pairs,
  )
    .slice(0, 24)
    .map((pair) => ({
      exploration_group_id: text(pair.exploration_group_id, 120),
      asset_class: text(pair.asset_class, 80),
      accepted_asset_node_id: text(pair.accepted_asset_node_id, 120),
      rejected_asset_node_id: text(pair.rejected_asset_node_id, 120),
      accepted_variation_axis: text(pair.accepted_variation_axis, 100),
      rejected_variation_axis: text(pair.rejected_variation_axis, 100),
      accepted_selection_score: Number.isFinite(Number(pair.accepted_selection_score))
        ? Number(pair.accepted_selection_score)
        : null,
      rejection_reasons: list(pair.rejection_reasons)
        .map((reason) => text(reason, 180))
        .filter(Boolean),
      evidence_only: true,
      copy_forbidden: true,
    }));
  ...
  return Object.freeze({
    ...
    image_studio_learning: {
      recurring_failure_patterns: imageStudioRecurringFailures,
      accepted_rejected_design_evidence: imageStudioExplorationPairs,
      advisory_only: true,
      style_copy_forbidden: true,
      repeat_known_failure_without_new_evidence_forbidden: true,
    },
