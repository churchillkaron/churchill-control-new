...
import { CreativeMeasuredMaterialLibraryRuntime } from "@/lib/creative/materials/runtime/CreativeMeasuredMaterialLibraryRuntime";
...
function normalize(raw={},index=0){const r=object(raw);const cls=text(r.material_class||r.class,80).toUpperCase();const measured=object(r.measured_profile);const resolvedMeasured=measured.profile_id&&CreativeMeasuredMaterialLibraryRuntime.presets[text(measured.profile_id,80).toUpperCase()]?CreativeMeasuredMaterialLibraryRuntime.resolve({profile_id:measured.profile_id,manufacturer:measured.manufacturer,scan_asset_node_id:measured.scan_asset_node_id,colorimetric_reference:measured.colorimetric_reference}):null;const pbr={...(resolvedMeasured?.defaults||{}),...object(r.pbr)};const micro={...(resolvedMeasured?.microstructure||{}),...object(r.microstructure)};return{
  ...
  material_interchange:object(r.material_interchange),
  material_graph:object(r.material_graph),
  measured_profile:{
    source:text(measured.source||resolvedMeasured?.source,200)||null,
    manufacturer:text(measured.manufacturer||resolvedMeasured?.manufacturer,200)||null,
    profile_id:text(measured.profile_id||resolvedMeasured?.profile_id,300)||null,
    brdf_model:text(measured.brdf_model||resolvedMeasured?.brdf_model,100)||null,
    measured_ior:measured.measured_ior==null?(resolvedMeasured?.measured_ior??null):finite(measured.measured_ior,1.5,1,3),
    colorimetric_reference:text(measured.colorimetric_reference||resolvedMeasured?.colorimetric_reference,300)||null,
    scan_asset_node_id:text(measured.scan_asset_node_id||resolvedMeasured?.scan_asset_node_id,500)||null,
    profile_hash:resolvedMeasured?.profile_hash||null,
  },
...
function blockers(material){const b=[];if(!material.material_id)b.push("MATERIAL_ID_REQUIRED");if(!material.continuity_key)b.push("MATERIAL_CONTINUITY_KEY_REQUIRED");if(material.material_graph?.contract&&material.material_graph.contract!=="AVANTIQO_MATERIALX_GRAPH_TRANSLATION_V1")b.push(`MATERIAL_GRAPH_CONTRACT_UNSUPPORTED:${material.material_id}`);if(material.material_interchange?.source_format==="MATERIALX"&&!/^[a-f0-9]{64}$/i.test(text(material.material_interchange?.source_checksum,100)))b.push(`MATERIALX_SOURCE_CHECKSUM_REQUIRED:${material.material_id}`);if(material.material_interchange?.axf_conversion&&material.material_interchange.axf_conversion.status!=="READY")b.push(`AXF_CONVERSION_AUTHORITY_REQUIRED:${material.material_id}`);if(material.material_class==="AUTOMOTIVE_PAINT"&&material.coat_weight<=0)b.push(`AUTOMOTIVE_CLEARCOAT_REQUIRED:${material.material_id}`);if(material.material_class==="CARBON_FIBER"&&material.microstructure.carbon_weave_scale<=0)b.push(`CARBON_WEAVE_SCALE_REQUIRED:${material.material_id}`);if(material.material_class==="GLASS_OPTICAL"&&(material.transmission<.5||material.ior<=1))b.push(`OPTICAL_GLASS_TRANSMISSION_IOR_REQUIRED:${material.material_id}`);if(material.measured_profile.scan_asset_node_id&&!material.measured_profile.source)b.push(`MEASURED_PROFILE_SOURCE_REQUIRED:${material.material_id}`);for(const map of material.texture_maps)if(!map.asset_node_id||!map.channel)b.push(`TEXTURE_MAP_BINDING_REQUIRED:${material.material_id}`);return b;}
