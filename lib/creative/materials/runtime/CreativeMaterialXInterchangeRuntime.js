...
function standardSurfaceBody(xml) {
  ...
  if (/<(?:nodegraph|image|tiledimage|procedural|geompropvalue)\b/i.test(source)) {
    throw new Error("MATERIALX_COMPLEX_GRAPH_REQUIRES_BAKED_OR_GRAPH_TRANSLATION");
  }
...
export function importMaterialXStandardSurface({
  ...
  const inputs = parseInputs(parsed.body);
