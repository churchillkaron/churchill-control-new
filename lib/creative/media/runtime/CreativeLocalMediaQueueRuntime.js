...
export async function executeLocalMediaJob({ organization_id, usage_id, operation, source, options = {}, extension = "mp4" } = {}) {
  ...
  if (inserted.error || !inserted.data?.id) throw inserted.error || new Error("AVANTIQO_LOCAL_MEDIA_QUEUE_INSERT_FAILED");
  const row = await waitForJob(inserted.data.id);
  ...
  return { success: true, provider_job_id: `${JOB_PREFIX}${inserted.data.id}`, infrastructure_provider: INFRASTRUCTURE, node_id: row.node_id, model: text(row.model) || "ffmpeg-9.0.1", metrics: object(row.metrics), output: { ...out, storage_reference: storageReference, ...(assetUrl ? { asset_url: assetUrl } : {}) } };
}
