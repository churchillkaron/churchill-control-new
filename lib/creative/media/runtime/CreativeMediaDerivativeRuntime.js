...
import {
  creativeStorageUri,
} from "../../assets/storage/CreativePrivateStorageRuntime.js";
import {
  materializeMedia,
...
import { CreativeLocalMediaQueueRuntime } from "./CreativeLocalMediaQueueRuntime.js";
    ...
    try {
      for (const profile of profiles) {
        ...
        if (useImageEngine) {
          ...
          const suppliedArgs = profile.ffmpeg_args || profile.ffmpegArgs || profile.args;
          ...
          if (!Array.isArray(suppliedArgs) || suppliedArgs.length === 0) {
            try {
              ...
              console.error("AVANTIQO_MEDIA_DERIVATIVE_LOCAL_FALLBACK", { error: String(error?.message || error).slice(0, 500) });
            }
          }
          if (!renderedLocally) await createFfmpegDerivative(materialized.file_path, outputPath, profile, policy);
        }
