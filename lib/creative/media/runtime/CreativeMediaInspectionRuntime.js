...
function inferKind(mimeType, streams = []) {
  ...
  if (root && root !== "application") return root;
...
async function fetchRemoteToFile({ url, filePath, policy = {} }) {
  ...
  const timeoutMs = configuredNumber(
    ...
    "timeout_ms",
    "timeoutMs",
    "CREATIVE_MEDIA_INSPECTION_TIMEOUT_MS",
  );
  const maximumRedirects = Number(
    policy.max_redirects ??
    policy.maxRedirects ??
    process.env.CREATIVE_MEDIA_MAX_REDIRECTS ??
    0,
  );
  ...
  try {
    while (true) {
      ...
      if (!effectiveMaximum) {
        throw new Error("MEDIA_MAX_BYTES_REQUIRED_FOR_UNBOUNDED_RESPONSE");
      }
      if (contentLength && contentLength > effectiveMaximum) {
        throw new Error("MEDIA_EXCEEDS_INSPECTION_LIMIT");
      }
      if (!response.body) {
        throw new Error("MEDIA_RESPONSE_BODY_REQUIRED");
      }
...
export async function materializeMedia({
  ...
  if (!file && !url) {
    throw new Error("MEDIA_FILE_OR_URL_REQUIRED");
  }
...
async function runJsonCommand(command, args, timeoutMs = null) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (timer) clearTimeout(timer);
      if (code !== 0) {
        reject(new Error(Buffer.concat(stderr).toString("utf8") || `MEDIA_PROBE_EXIT_${code}`));
        return;
      }
      ...
      try {
        ...
        reject(new Error("MEDIA_PROBE_INVALID_JSON"));
      }
    });
  });
}
...
async function inspectImage(filePath, mimeType) {
  ...
  return {
    media_kind: "image",
    mime_type: normalizeMime(mimeType) || (metadata.format ? `image/${metadata.format}` : null),
