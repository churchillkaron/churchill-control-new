...
export const CreativeMediaRuntime = {
  async extractContactSheet({ video_url, frame_count = 9 } = {}) {
    ...
    return withWorkspace("avantiqo-video-qa-", async (directory) => {
      ...
      await runFfmpeg([
        "-i", input,
        "-vf", `thumbnail,scale=480:-2,tile=${columns}x${rows}:padding=6:margin=6`,
        "-frames:v", "1",
        "-q:v", "2",
        output,
      ]);
  ...
  async composeMaster({ edit_decision_list = [], width = 1920, height = 1080, fps = 30 } = {}) {
    ...
    return withWorkspace("avantiqo-final-render-", async (directory) => {
      const normalized = [];
      ...
      const finalPath = path.join(directory, "master.mp4");
      await runFfmpeg([
        "-f", "concat",
        "-safe", "0",
        "-i", concatFile,
