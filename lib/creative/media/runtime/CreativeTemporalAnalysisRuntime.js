...
import {
  materializeMedia,
} from "./CreativeMediaInspectionRuntime.js";
import {
  createCreativeAssetNode,
  CREATIVE_ASSET_NODE_STATUS,
  CREATIVE_ASSET_NODE_TYPES,
} from "../../assets/graph/documents/CreativeAssetNode.js";
import * as AssetGraphRepository
from "../../assets/graph/repositories/CreativeAssetGraphRepository.js";
...
function inheritedEvidence(parent, range) {
  const metadata = object(parent.metadata);
  const timing = object(metadata.timing);

  return {
    production_graph_id: metadata.production_graph_id || null,
    production_task_id:
      parent.production_task_id ||
      metadata.production_task_id ||
      null,
    scene_id: metadata.scene_id || null,
    shot_id: metadata.shot_id || null,
    timeline_role: metadata.timeline_role || null,
    include_in_master: metadata.include_in_master !== false,
    requirements: metadata.requirements || null,
    restrictions: object(metadata.restrictions),
    rights: object(metadata.rights),
    consent: object(metadata.consent),
    identity: object(metadata.identity),
    provider_job_id: metadata.provider_job_id || null,
    usage_id: metadata.usage_id || null,
    billing_invoice_id: metadata.billing_invoice_id || null,
    timing: {
      ...timing,
      source_in_seconds: range.start_seconds,
      source_out_seconds: range.end_seconds,
  ...
  async analyze({
    ...
    try {
      ...
      for (const range of ranges) {
        const evidence = inheritedEvidence(parent, range);
        const node = createCreativeAssetNode({
          organization_id,
          creative_project_id: parent.creative_project_id,
          creative_asset_id: parent.creative_asset_id,
          production_task_id: evidence.production_task_id,
          ...
          lineage: {
            ...
            provider_id: parent.lineage?.provider_id || null,
            capability: "media.scene.detect",
            generation_version: options.version || 2,
          },
          technical: {
            ...(parent.technical || {}),
            duration_seconds: range.duration_seconds,
          ...
          intelligence: {
            ...(parent.intelligence || {}),
            tags: tags(
              parent.intelligence?.tags,
              options.tags,
              ["temporal-clip"],
            ),
            safety_status:
              parent.intelligence?.safety_status ||
              "UNKNOWN",
          },
          cost: {
            currency: parent.cost?.currency || null,
            estimated: 0,
            actual: 0,
            saved_by_reuse: 0,
          },
