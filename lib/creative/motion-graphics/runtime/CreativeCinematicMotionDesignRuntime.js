...
function normalizeEvent(raw={},index=0){
  ...
  return{
    ...
    world_space:r.world_space!==false,camera_relationship:text(r.camera_relationship,800),material_behavior:text(r.material_behavior,1200),
...
function blockersFor(event,{flagship=true}={}){
  ...
  if(event.integration_mode==="SOURCE_WORLD_INTEGRATED"&&(!event.camera_solution_asset_node_id||!event.depth_asset_node_id))b.push(`CINEMATIC_MOTION_RECONSTRUCTION_BINDING_REQUIRED:${event.event_id}`);
