...
function bindIntegratedEvents(events=[],assets=[],shotId){
  ...
  return list(events).map(event=>{
    ...
    return{...event,camera_solution_asset_node_id:event.camera_solution_asset_node_id||camera?.id||null,depth_asset_node_id:event.depth_asset_node_id||depth?.id||null,material_map_asset_node_id:event.material_map_asset_node_id||materials?.id||null};
  });
}
