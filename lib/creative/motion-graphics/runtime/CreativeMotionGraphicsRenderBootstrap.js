...
import {
  CreativeMotionGraphicsQualityRuntime,
} from "./CreativeMotionGraphicsQualityRuntime";
...
function install() {
  ...
  CreativeEdlRenderRuntime.render = async function renderWithMotionGraphics(input = {}) {
    ...
    try {
      ...
      const sealed = motion.render?.metadata?.motion_graphics_qc_sealed === true
        ? {
            ...
            seal_hash: motion.render.metadata.motion_graphics_qc_seal_hash,
          }
        : await CreativeMotionGraphicsQualityRuntime.seal({
            organization_id: input.organization_id,
            render: motion.render,
          });
      if (sealed.evaluation?.passed !== true) {
        throw new Error(
          `MOTION_GRAPHICS_QC_BLOCKED:${sealed.evaluation?.blockers?.join(",") || "UNKNOWN"}`,
        );
      }
      return {
        ...baseResult,
        pre_motion_graphics_render: baseResult.render,
        render: sealed.render,
        technical_qc: motion.technical_qc || baseResult.technical_qc,
        motion_graphics: {
          contract: CreativeMotionGraphicsRenderRuntime.contract,
          applicable: true,
          reused: motion.reused === true,
          render_asset_node_id: sealed.render?.id || null,
          ...
          qc_contract: CreativeMotionGraphicsQualityRuntime.contract,
          qc_seal_contract: CreativeMotionGraphicsQualityRuntime.seal_contract,
          qc_passed: true,
          qc_seal_hash: sealed.seal_hash,
          qc_report_id: sealed.report?.id || null,
        },
      };
    } finally {
      depth -= 1;
