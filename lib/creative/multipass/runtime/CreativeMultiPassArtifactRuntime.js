...
export async function sealVfxArtifact({asset_node_id,vfx_qc_seal_hash,vfx_contract_hash=null}={}){
  ...
  if(!["vfx-integration","lighting-interaction","reflection-shadow","atmosphere","threat-hero-layer"].includes(text(node.metadata?.pass_id))) throw new Error("VFX_ARTIFACT_PASS_MISMATCH");
  return AssetGraphRepository.update(asset_node_id,{
    status:"APPROVED",
...
export async function sealCinematicMotionArtifact({asset_node_id,motion_qc_seal_hash,motion_contract_hash=null}={}){
  if(!text(asset_node_id)) throw new Error("CINEMATIC_MOTION_ARTIFACT_NODE_REQUIRED");
  if(!/^[a-f0-9]{64}$/i.test(text(motion_qc_seal_hash))) throw new Error("CINEMATIC_MOTION_QC_SEAL_HASH_REQUIRED");
  const node=await AssetGraphRepository.getById(asset_node_id);
  if(!node) throw new Error("CINEMATIC_MOTION_ARTIFACT_NODE_NOT_FOUND");
  if(text(node.metadata?.pass_id)!=="cinematic-motion") throw new Error("CINEMATIC_MOTION_ARTIFACT_PASS_MISMATCH");
  return AssetGraphRepository.update(asset_node_id,{
    status:"APPROVED",
    review:{...object(node.review),ai_reviewed:true,approved:true,notes:`Cinematic motion QC sealed: ${motion_qc_seal_hash}.`},
    metadata:{...object(node.metadata),cinematic_motion_qc_sealed:true,cinematic_motion_qc_seal_contract:"AVANTIQO_CINEMATIC_MOTION_DESIGN_QC_SEAL_V1",cinematic_motion_qc_seal_hash:motion_qc_seal_hash,cinematic_motion_contract_hash:motion_contract_hash,release_approved:true},
  });
}
export const CreativeMultiPassArtifactRuntime=Object.freeze({contract:CREATIVE_MULTIPASS_ARTIFACT_CONTRACT,persist:persistMultiPassArtifact,sealSimulation:sealSimulationArtifact,sealVfx:sealVfxArtifact,sealCinematicMotion:sealCinematicMotionArtifact});
