...
function requiresPremiumPasses(shot = {}) {
  ...
  return sr.required === true || Boolean(shot.scene_reconstruction_contract) || list(shot.vfx).length > 0 || Boolean(shot.simulation_contract) || Boolean(shot.compositing_contract);
}
...
function build(shot = {}) {
  ...
  const hasGraphics = Boolean(list(shot.graphics).length || Object.keys(object(shot.typography)).length);
  ...
  if (hasVfx) passes.push(pass('vfx-integration', 'VFX_INTEGRATION', hasSimulation ? ['base-plate','simulation'] : ['base-plate']));
  passes.push(pass('atmosphere', 'ATMOSPHERE', ['base-plate'], false));
  passes.push(pass('lighting-interaction', 'LIGHTING_INTERACTION', ['base-plate'], hasSource || hasVfx));
  passes.push(pass('reflection-shadow', 'REFLECTION_SHADOW', ['lighting-interaction'], hasSource || hasVfx));
  ...
  passes.push(pass('color-di', 'COLOR_DI', ['optical-finish']));
  passes.push(pass('shot-qc', 'PERCEPTUAL_AND_TECHNICAL_QC', ['color-di']));
