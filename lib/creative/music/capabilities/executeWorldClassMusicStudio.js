...
import { executeMusicIntentionalEdit } from "@/lib/creative/music/runtime/CreativeMusicIntentionalEditExecutionRuntime";
import { interpretMusicProjectDecision } from "@/lib/creative/music/runtime/CreativeMusicDecisionInterpreterRuntime";
import { deriveMusicSectionMap } from "@/lib/creative/music/runtime/CreativeMusicSectionMapRuntime";
import { restoreMusicVersionAsNew } from "@/lib/creative/music/runtime/CreativeMusicVersionLineageRuntime";
import {
...
function bindDecisionRangesToVersion(patch = {}, state = {}) {
  const masterAssetId = text(state.current_master_asset_id) || null;
  const versionId = text(state.current_version_id || masterAssetId) || null;
  const bind = (rows) => Array.isArray(rows) ? rows.map((row) => ({
    ...row, master_asset_id: masterAssetId, version_id: versionId,
    provenance_fingerprint: "BUSINESS_PARTNER_CURRENT_MASTER",
  })) : rows;
  return {
    ...patch,
    ...(patch.approved_sections ? { approved_sections: bind(patch.approved_sections) } : {}),
    ...(patch.protected_ranges ? { protected_ranges: bind(patch.protected_ranges) } : {}),
  };
}
...
export const manifest = defineCapability({
  ...
  operatorAliases: [
    ...
    "restore this music version", "go back to this music version", "restore the earlier master",
  ],  operatorExamples: [
  ...
  inputSchema: {
    ...
    properties: {
      ...
      authorized_stage: { type: "string" },
      professional_continue: { type: "boolean" },
      mix_asset_id: { type: "string" },
      corrected_vocal_asset_id: { type: "string" },
      human_listening_review_approved: { type: "boolean" },
      approved_by: { type: "string" },
      repair_plan_hash: { type: "string" },
      restore_version_asset_id: { type: "string" },
      expected_change_set_fingerprint: { type: "string" },
      ...
      mastering_destinations: { type: "array", items: { type: "string" } },
...
export function validate({ payload = {} }) {
  if (payload.professional_continue === true || text(payload.authorized_stage)) {
    if (!text(payload.creative_project_id) || !text(payload.source_asset_id) || !text(payload.authorized_stage)) {
      const error = new Error("CREATIVE_MUSIC_PROFESSIONAL_CONTINUATION_REFERENCE_REQUIRED"); error.status = 400; throw error;
    }
    return true;
  }
  if (text(payload.restore_version_asset_id)) {
    if (!text(payload.creative_project_id)) { const error = new Error("CREATIVE_MUSIC_VERSION_RESTORE_PROJECT_REQUIRED"); error.status = 400; throw error; }
    return true;
  }
  const intentionalEdit = text(payload.expected_change_set_fingerprint);
  if (intentionalEdit) {
    if (!text(payload.creative_project_id) || !text(payload.intended_delta || payload.objective)) {
      const error = new Error("CREATIVE_MUSIC_INTENTIONAL_EDIT_REFERENCE_REQUIRED");
      error.status = 400;
      throw error;
    }
  ...
  const repair = text(payload.repair_plan_hash);
  if (repair) {
...
export async function execute({ context, payload = {} }) {
  if (payload.professional_continue === true || text(payload.authorized_stage)) {
    const resolvedContinuation = await resolveOperatorCreativeProject({ organizationId: context.organizationId, creativeProjectId: payload.creative_project_id, requestRef: null });
    if (!text(resolvedContinuation?.project?.id)) throw new Error("CREATIVE_MUSIC_PROFESSIONAL_CONTINUATION_PROJECT_NOT_FOUND");
    const result = await executeWorldClassMusicStudio({ ...payload, professional_continue: true, organization_id: context.organizationId, creative_project_id: resolvedContinuation.project.id, creative_mission_id: text(payload.creative_mission_id || resolvedContinuation.mission?.id || resolvedContinuation.project.creative_mission_id) || null });
    return { ...result, creative_project_id: resolvedContinuation.project.id, publish_authorized: false };
  }
  if (text(payload.restore_version_asset_id)) {
    const result = await restoreMusicVersionAsNew({
      organization_id: context.organizationId,
      creative_project_id: payload.creative_project_id,
      historical_asset_id: payload.restore_version_asset_id,
      summary: payload.decision_summary || payload.objective || null,
    });
    return { ...result, creative_project_id: payload.creative_project_id, publish_authorized: false };
  }
  if (text(payload.expected_change_set_fingerprint)) {
    const result = await executeMusicIntentionalEdit({
      organization_id: context.organizationId,
      creative_project_id: payload.creative_project_id,
      creative_mission_id: text(payload.creative_mission_id) || null,
      ...
      expected_change_set_fingerprint: payload.expected_change_set_fingerprint,
  ...
  const sectionMap = deriveMusicSectionMap(
    result?.production_binding?.preproduction_brief ||
    result?.session?.music_production_binding?.preproduction_brief ||
    result?.master_asset?.metadata?.music_production_binding?.preproduction_brief ||
    {},
  );
  ...
  const conversationStatePatch = bindDecisionRangesToVersion({ ...(payload.conversation_state_patch || {}) }, loadedConversation.state);
  const interpretedDecisionPatch = bindDecisionRangesToVersion(interpretedDecision.patch || {}, loadedConversation.state);
  delete conversationStatePatch.current_master_asset_id;
  delete conversationStatePatch.current_version_id;
  delete conversationStatePatch.version_lineage;
  delete conversationStatePatch.listening_evidence;
  ...
  try {
    conversationState = await updateMusicConversationState({
      ...
      creative_project_id: resolved.project.id,
      patch: {
        ...interpretedDecisionPatch,
        ...conversationStatePatch,
        ...(!loadedConversation.state.creative_intent && text(payload.objective) ? { creative_intent: text(payload.objective) } : {}),
        ...(sectionMap.sections.length ? { approved_sections: sectionMap.sections.map((section) => ({
          ...section,
          master_asset_id: result?.master_asset?.id || result?.asset?.id || null,
          version_id: result?.master_asset?.id || result?.asset?.id || null,
          provenance_fingerprint: sectionMap.contract,
        })) } : {}),
        ...(result?.master_asset?.id ? { current_master_asset_id: result.master_asset.id, current_version_id: result.master_asset.id } : {}),
