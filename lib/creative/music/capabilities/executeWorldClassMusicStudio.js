...
export async function execute({ context, payload = {} }) {
  ...
  const versionLineage = result?.master_asset?.id
    ? [{ id: result.master_asset.id, master_asset_id: result.master_asset.id, summary: text(payload.decision_summary) || null }]
    : (result?.asset?.id ? [{ id: result.asset.id, master_asset_id: result.asset.id, summary: text(payload.decision_summary) || null }] : []);
  ...
  try {
    conversationState = await updateMusicConversationState({
      ...
      decision_summary: text(payload.decision_summary),
    });
  } catch (error) {
