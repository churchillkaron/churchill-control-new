...
export async function execute({ context, payload = {} }) {
  if (!text(payload.creative_project_id)) return buildWorldClassMusicStudioPlan(payload);
  const loaded = await loadMusicConversationState({
    organization_id: context.organizationId,
    creative_project_id: payload.creative_project_id,
  });
  return buildWorldClassMusicStudioPlan({
    ...payload,
    music_conversation_context: musicConversationExecutionContext(loaded.state),
  });
}
