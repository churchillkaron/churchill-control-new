...
import { buildMusicDestinationMasteringPlan } from "@/lib/creative/music/runtime/CreativeMusicDestinationMasteringRuntime";
...
export async function execute({ context, payload = {} }) {
  ...
  const masteringPlan = buildMusicDestinationMasteringPlan(payload);
  if (!text(payload.creative_project_id)) return {
    ...buildWorldClassMusicStudioPlan(payload),
    conversation_decision_preview: conversationDecisionPreview,
    music_mastering_plan: masteringPlan,
  };
