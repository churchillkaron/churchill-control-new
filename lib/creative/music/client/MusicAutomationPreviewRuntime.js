...
function transformedValue(parameter, value) {
  if (parameter === "gain_db" || String(parameter || "").startsWith("send_db:")) return Math.max(0.000001, dbToGain(value));
...
function scheduleLane({ param, lane, startSeconds, stopAtSeconds, contextStartTime }) {
  ...
  for (const point of points) {
    ...
    } else if (lane.parameter === "gain_db" || String(lane.parameter || "").startsWith("send_db:")) {
...
export function scheduleMusicMixerAutomation({
  ...
  for (const lane of session?.automation_lanes || []) {
    ...
    if (!param) {
      if (String(lane.parameter || "").startsWith("spatial:")) { offlineSpatialLaneCount += 1; continue; }
  ...
  return {
    ...
    offline_spatial_lane_count: offlineSpatialLaneCount,
    spatial_automation_browser_preview_claimed: false,
  };
}
