...
export function evaluateMusicLatencyCalibrationReuse(calibration = {}, current = {}, nowMs = Date.now()) {
  ...
  const sameInput = Boolean(calibration.input_device_id) && calibration.input_device_id === current.input_device_id;
  ...
  if (!sameInput) reasons.push("INPUT_DEVICE_CHANGED");
  ...
  return { reuse_allowed: reuseAllowed, fresh, same_input: sameInput, same_sample_rate: sameRate, same_path: samePath, output_path_verified: sinkKnown && sameSink, reasons };
}
...
export async function runMusicLatencyCalibration({ deviceId = null, outputDeviceId = null, sampleRate = null, pathType = "HARDWARE_LOOPBACK" } = {}) {
  ...
  try {
    ...
    return {
      contract: "AVANTIQO_MUSIC_LATENCY_CALIBRATION_V2",
