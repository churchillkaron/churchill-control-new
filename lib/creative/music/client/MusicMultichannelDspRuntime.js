...
export function applyMusicLinkedMultichannelSidechain(targetChannels=[],keyChannels=[],sampleRate=48000,settings={}){
  if(settings?.enabled===false)return{channels:targetChannels.map(ch=>new Float32Array(ch)),gain_reduction_db_min:0,linked_detector:true,sidechain_applied:false};
  const frames=Math.max(0,...targetChannels.map(ch=>ch.length),...keyChannels.map(ch=>ch.length)),out=targetChannels.map(()=>new Float32Array(frames));
  const threshold=clamp(settings.threshold_dbfs,-60,0,-30),ratio=clamp(settings.ratio,1,20,3),knee=clamp(settings.knee_db,0,24,6),maxReduction=Math.max(0,clamp(settings.max_reduction_db,0,18,6)),attack=Math.max(.0001,finite(settings.attack_ms,12)/1000),release=Math.max(.001,finite(settings.release_ms,220)/1000),attackCoeff=Math.exp(-1/(attack*sampleRate)),releaseCoeff=Math.exp(-1/(release*sampleRate));let env=0,minGr=0;
  for(let i=0;i<frames;i++){let detector=0;for(const ch of keyChannels)detector=Math.max(detector,Math.abs(finite(ch?.[i],0)));env=detector>env?attackCoeff*env+(1-attackCoeff)*detector:releaseCoeff*env+(1-releaseCoeff)*detector;const level=env>1e-12?20*Math.log10(env):-Infinity;let gr=compressionGainDb(level,threshold,ratio,knee);gr=Math.max(-maxReduction,gr);minGr=Math.min(minGr,gr);const gain=dbToGain(gr);for(let c=0;c<out.length;c++)out[c][i]=finite(targetChannels[c]?.[i],0)*gain;}
  return{channels:out,gain_reduction_db_min:Number(minGr.toFixed(3)),linked_detector:true,sidechain_applied:true};
}
...
export const CreativeMusicMultichannelDspRuntime=Object.freeze({applyEq:applyMusicMultichannelEq,applyLinkedCompression:applyMusicLinkedMultichannelCompression,applyLinkedSidechain:applyMusicLinkedMultichannelSidechain,applyBusProcessing:applyMusicMultichannelBusProcessing,sum:sumMusicMultichannelBuffers});
