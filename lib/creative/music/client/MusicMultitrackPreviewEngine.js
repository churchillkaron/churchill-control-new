...
function createReverbBus(context, bus, destination) {
  ...
  return { input, type: "reverb", nodes: [preDelay, damping, convolver, wet] };
}
...
function createDelayBus(context, bus, destination) {
  ...
  wet.connect(destination);
  return { input, type: "delay", nodes: [lowCut, delay, highCut, feedback, wet] };
}
...
export async function startMusicMultitrackPreview({
  ...
  return {
    contract: "AVANTIQO_MUSIC_MULTITRACK_BROWSER_PREVIEW_V4",
