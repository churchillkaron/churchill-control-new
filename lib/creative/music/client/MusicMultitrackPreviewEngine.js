...
function connectTrackStrip(context, track, destination, auxInputs, trackMeter) {
...
function startLiveMeterPublisher({ masterAnalyser, trackMeters }) {
  ...
  const timer = setInterval(() => {
    ...
    publishMeter({
      contract: "AVANTIQO_MUSIC_LIVE_ENGINEERING_METER_V1",
      active: true,
  ...
  return () => {
    ...
    publishMeter({
      ...
      active: false,
      timestamp_ms: Date.now(),
      master: { peak_dbfs: -Infinity, rms_dbfs: -Infinity, clipping: false, headroom_db: Infinity },
      tracks: [],
    });
  };
}
...
export async function startMusicMultitrackPreview({ session, assetUrls, startSeconds = 0, stopAtSeconds = null, onEnded } = {}) {
  ...
  for (const track of session.tracks || []) {
    ...
    const { clipBus } = connectTrackStrip(context, previewTrack, master, auxInputs, meter);
  ...
  const stopMetering = startLiveMeterPublisher({ masterAnalyser: analyser, trackMeters });
  ...
  return {
    contract: "AVANTIQO_MUSIC_MULTITRACK_BROWSER_PREVIEW_V6",
