...
function scheduleClip(context, buffer, clip, trackBus, transportStartSeconds, contextStartTime, sources) {
  ...
  const scheduledDuration = Math.min(
...
export async function startMusicMultitrackPreview({
  ...
  for (const track of session.tracks || []) {
    ...
    for (const clip of track.clips || []) {
      ...
      scheduleClip(context, buffer, clip, clipBus, startSeconds, contextStartTime, sources);
      maxEnd = Math.max(maxEnd, finite(clip.start_seconds, 0) + finite(clip.duration_seconds, 0));
    }
  }

  const duration = Math.max(0, maxEnd - startSeconds);
  let ended = false;
  const finish = () => {
    if (ended) return;
    ended = true;
  ...
  const timer = duration > 0 ? setTimeout(finish, (duration + 0.1) * 1000) : null;

  return {
    contract: "AVANTIQO_MUSIC_MULTITRACK_BROWSER_PREVIEW_V1",
