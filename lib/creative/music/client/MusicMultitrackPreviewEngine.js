...
function connectTrackStrip(context, track, destination, auxInputs, trackMeter, stereoMeteringAvailable, sourceDiagnosticsAvailable) {
  ...
  if (stereoMeteringAvailable) {
    const stereoMeter = createStereoMeterNode(context, (stereo) => { trackMeter.stereo = stereo; });
    pan.connect(stereoMeter);
    stereoMeter.connect(postFaderAnalyser);
  } else pan.connect(postFaderAnalyser);
...
async function preloadTrackBuffers(context, session, assetUrls) {
  ...
  for (const track of session.tracks || []) {
    ...
    for (const clip of resolveMusicTrackPreviewClips(track)) {
      ...
      if (!url) continue;
      clips.push({ clip, buffer: await loadBuffer(context, url) });
    }
...
function startLiveMeterPublisher({ masterAnalyser, masterStereo, trackMeters, groupOutputs, stereoMeteringAvailable, sourceDiagnosticsAvailable }) {
  ...
  const timer = setInterval(() => {
    ...
    for (const meter of trackMeters.values()) {
      ...
      tracks.push({
        ...
        output_bus_id: meter.output_bus_id,
        ...levels,
    ...
    for (const [groupId, nodes] of groupOutputs || []) {
      ...
      groups.push({
        group_id: groupId,
        group_name: nodes.bus.name || "Group",
        output_bus_id: nodes.bus.output_bus_id || "bus-master",
        gain_db: finite(nodes.bus.gain_db, 0),
        pan: finite(nodes.bus.pan, 0),
        mute: nodes.bus.mute === true,
        ...levels,
        headroom_db: Number.isFinite(levels.peak_dbfs) ? Math.max(0, -levels.peak_dbfs) : Infinity,
      });
    }
    publishMeter({
      contract: "AVANTIQO_MUSIC_LIVE_ENGINEERING_METER_V6",
      active: true,
...
export async function startMusicMultitrackPreview({ session, assetUrls, startSeconds = 0, stopAtSeconds = null, onEnded } = {}) {
  ...
  if (dynamicsRequired) await context.audioWorklet.addModule("/audio/avantiqo-music-dynamics-worklet.js");
  ...
  if (context.audioWorklet && typeof globalThis.AudioWorkletNode === "function") {
    try { await context.audioWorklet.addModule(STEREO_METER_MODULE); stereoMeteringAvailable = true; } catch { stereoMeteringAvailable = false; }
    try { await context.audioWorklet.addModule(SOURCE_DIAGNOSTICS_MODULE); sourceDiagnosticsAvailable = true; } catch { sourceDiagnosticsAvailable = false; }
  }
  ...
  master.gain.value = masterBus.mute === true ? 0 : dbToGain(masterBus.gain_db || 0);
  ...
  } else master.connect(analyser);
  analyser.connect(context.destination);

  const groupGraph = createMusicGroupBusPreviewGraph(context, session, master);
  const auxInputs = createAuxBuses(context, session, master);
  const automationTargets = new Map();
  if (masterBus.mute !== true) automationTargets.set("master:bus-master:gain_db", master.gain);
  ...
  for (const prepared of preparedTracks) {
    ...
    const meter = {
      ...
      output_bus_id: track.output_bus_id || "bus-master",
      gate_reduction_db: 0,
      gate_open: true,
    ...
    if (!effectiveMute) automationTargets.set(`track:${track.id}:gain_db`, nodes.fader.gain);
    automationTargets.set(`track:${track.id}:pan`, nodes.pan.pan);

    for (const { clip, buffer } of prepared.clips) {
      scheduleClip(context, buffer, clip, nodes.clipBus, startSeconds, contextStartTime, sources, stopAtSeconds);
      maxEnd = Math.max(maxEnd, finite(clip.start_seconds, 0) + finite(clip.duration_seconds, 0));
    }
  }

  const automationPreview = scheduleMusicMixerAutomation({
    session,
  ...
  const stopMetering = startLiveMeterPublisher({
    ...
    masterStereo,
    trackMeters,
    groupOutputs: groupGraph.outputs,
  ...
  return {
    contract: "AVANTIQO_MUSIC_MULTITRACK_BROWSER_PREVIEW_V13",
