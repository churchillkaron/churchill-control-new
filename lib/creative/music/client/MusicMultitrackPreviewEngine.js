...
function connectTrackStrip(context, track, destination, auxInputs) {
  ...
  if (gate) {
    const gateNode = createWorkletInsert(context, "avantiqo-music-gate", gate);
  ...
  if (deesser) {
    const deesserNode = createWorkletInsert(context, "avantiqo-music-deesser", deesser);
...
export async function startMusicMultitrackPreview({ session, assetUrls, startSeconds = 0, stopAtSeconds = null, onEnded } = {}) {
  ...
  for (const track of session.tracks || []) {
    ...
    const { clipBus } = connectTrackStrip(context, previewTrack, master, auxInputs);
  ...
  return {
    contract: "AVANTIQO_MUSIC_MULTITRACK_BROWSER_PREVIEW_V5",
