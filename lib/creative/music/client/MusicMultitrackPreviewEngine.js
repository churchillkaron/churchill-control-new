...
function connectTrackStrip(context, track, destination, auxInputs, trackMeter, stereoMeteringAvailable) {
...
function startLiveMeterPublisher({ masterAnalyser, masterStereo, trackMeters, stereoMeteringAvailable }) {
  ...
  const timer = setInterval(() => {
    ...
    publishMeter({
      contract: "AVANTIQO_MUSIC_LIVE_ENGINEERING_METER_V2",
      active: true,
...
export async function startMusicMultitrackPreview({ session, assetUrls, startSeconds = 0, stopAtSeconds = null, onEnded } = {}) {
  ...
  for (const track of session.tracks || []) {
    ...
    const { clipBus } = connectTrackStrip(context, previewTrack, master, auxInputs, meter, stereoMeteringAvailable);
  ...
  const stopMetering = startLiveMeterPublisher({ masterAnalyser: analyser, masterStereo, trackMeters, stereoMeteringAvailable });
  ...
  return {
    contract: "AVANTIQO_MUSIC_MULTITRACK_BROWSER_PREVIEW_V8",
