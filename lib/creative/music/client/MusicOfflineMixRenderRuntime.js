...
function needsDynamicsWorklet(session) {
  return (session?.tracks || []).some((track) => enabledInsert(track, "gate") || enabledInsert(track, "deesser")) || (session?.audio_post_mix?.dialogue_priority !== false && session?.audio_post_mix?.dialogue_ducking?.enabled !== false && Boolean(session?.audio_post_mix));
}
...
function createParallelCompressorBus(context, bus, destination) {
  const p=bus?.parameters||{},input=context.createGain(),highPass=context.createBiquadFilter(),compressor=context.createDynamicsCompressor(),makeup=context.createGain(),wet=context.createGain();
  highPass.type="highpass";highPass.frequency.value=Math.max(20,Math.min(400,finite(p.high_pass_hz,90)));highPass.Q.value=.707;
  compressor.threshold.value=Math.max(-60,Math.min(0,finite(p.threshold_db,-30)));compressor.ratio.value=Math.max(1,Math.min(20,finite(p.ratio,8)));compressor.attack.value=Math.max(.0001,Math.min(.2,finite(p.attack_ms,3)/1000));compressor.release.value=Math.max(.01,Math.min(2,finite(p.release_ms,120)/1000));compressor.knee.value=Math.max(0,Math.min(40,finite(p.knee_db,6)));
  makeup.gain.value=dbToGain(finite(p.makeup_db,8));wet.gain.value=bus?.mute===true?0:dbToGain(finite(p.wet_db,-10)+finite(bus?.gain_db,0));
  input.connect(highPass);highPass.connect(compressor);compressor.connect(makeup);makeup.connect(wet);wet.connect(destination);return input;
}
...
function createAuxBuses(context, session, masterInput) {
  ...
  for (const bus of (session?.buses || []).filter((entry) => entry.type === "aux")) {
    ...
    if (type === "compressor") result.set(bus.id, createParallelCompressorBus(context, bus, masterInput));
  }
  return result;
}
...
function connectTrackStrip(context, track, destination, auxInputs) {
  ...
  if (track.evidence_render_bypass_processing === true) {
    const fader = context.createGain(); fader.gain.value = 1;
    const pan = context.createStereoPanner(); pan.pan.value = 0;
    chain.connect(fader); fader.connect(pan); pan.connect(destination);
    return { clipBus, fader, pan, sendGains: new Map() };
  }
  ...
  return { clipBus, fader, pan, sendGains };
}

function scheduleClip(context, buffer, clip, destination, timelineStartSeconds = 0, timelineEndSeconds = Infinity) {
  ...
  const originalSourceOffset = Math.max(0, finite(clip.source_offset_seconds, 0));
  const originalDuration = Math.min(Math.max(0, finite(clip.duration_seconds, 0)), Math.max(0, buffer.duration - originalSourceOffset));
  const clipStart = Math.max(0, finite(clip.start_seconds, 0)), clipEnd = clipStart + originalDuration;
  const overlapStart = Math.max(clipStart, finite(timelineStartSeconds, 0)), overlapEnd = Math.min(clipEnd, Number.isFinite(timelineEndSeconds) ? timelineEndSeconds : clipEnd);
  const duration = Math.max(0, overlapEnd - overlapStart); if (duration <= 0) return;
  const sourceOffset = originalSourceOffset + (overlapStart - clipStart);
  const source = context.createBufferSource(); source.buffer = buffer;
  const gain = context.createGain(); const baseGain = dbToGain(clip.gain_db);
  source.connect(gain); gain.connect(destination);
  const when = Math.max(0, overlapStart - finite(timelineStartSeconds, 0));
  const fadeIn = Math.min(originalDuration / 2, Math.max(0, finite(clip.fade_in_seconds, 0)));
  const fadeOut = Math.min(originalDuration / 2, Math.max(0, finite(clip.fade_out_seconds, 0)));
  const gainAt = (absoluteTime) => { let factor = 1; if (fadeIn > 0 && absoluteTime < clipStart + fadeIn) factor = Math.min(factor, Math.max(0, (absoluteTime - clipStart) / fadeIn)); if (fadeOut > 0 && absoluteTime > clipEnd - fadeOut) factor = Math.min(factor, Math.max(0, (clipEnd - absoluteTime) / fadeOut)); return baseGain * factor; };
  gain.gain.setValueAtTime(gainAt(overlapStart), when);
  const fadeInEnd = clipStart + fadeIn; if (fadeIn > 0 && fadeInEnd > overlapStart && fadeInEnd < overlapEnd) gain.gain.linearRampToValueAtTime(baseGain, when + (fadeInEnd - overlapStart));
  const fadeOutStart = clipEnd - fadeOut; if (fadeOut > 0 && fadeOutStart > overlapStart && fadeOutStart < overlapEnd) gain.gain.setValueAtTime(baseGain, when + (fadeOutStart - overlapStart));
  if (fadeOut > 0 && overlapEnd > fadeOutStart) gain.gain.linearRampToValueAtTime(gainAt(overlapEnd), when + duration);
...
export async function renderMusicMultitrackOffline({ session, assetUrls, expectedDurationSeconds = null, renderWindow = null } = {}) {
  ...
  const windowStart = Math.max(0, finite(renderWindow?.start_seconds, 0)), windowEnd = Number.isFinite(renderWindow?.end_seconds) ? Math.max(windowStart, renderWindow.end_seconds) : null;
  const programDuration = windowEnd != null ? Math.max(0, windowEnd - windowStart) : (Number.isFinite(expectedDurationSeconds) ? Math.max(clipEnd, expectedDurationSeconds) : clipEnd);
  ...
  for (const item of prepared) {
    ...
    for (const [busId, param] of nodes.sendGains || []) automationTargets.set(`track:${item.track.id}:send_db:${busId}`, param);
    for (const entry of item.clips) scheduleClip(context, entry.buffer, entry.clip, nodes.clipBus, windowStart, windowEnd ?? Infinity);
  }

  const automation = scheduleMusicMixerAutomation({ session, targets: automationTargets, startSeconds: windowStart, stopAtSeconds: windowEnd ?? (windowStart + programDuration), contextStartTime: 0 });
  const audioBuffer = await context.startRendering();
  const levels = analyseMusicAudioBuffer(audioBuffer);
  const wav = encodeMusicAudioBufferWav24(audioBuffer, { bwf: session.picture_lock?.picture_lock_digest ? { enabled: true, start_timecode: session.picture_lock.start_timecode || "00:00:00:00", frame_rate: session.picture_lock.frame_rate || 24, picture_lock_digest: session.picture_lock.picture_lock_digest, description: `${session.title || "Avantiqo"} professional audio render`, originator: "Avantiqo Professional Audio Engine" } : null });
  return {
    ...
    internal_processing_bits: 32,
    delivery_conversion: { format: "pcm_s24le", bit_depth: 24, dither_applied: false, dither_required_before_final_master_delivery: true },
    program_duration_seconds: programDuration,
    render_window: { start_seconds: windowStart, end_seconds: windowEnd ?? (windowStart + programDuration), absolute_timeline: true },
    ...
    audio_post_dialogue_ducking_applied: groupGraph.dialogue_sidechain_applied === true,
    audio_post_dialogue_ducking_target_count: groupGraph.dialogue_sidechain_target_count || 0,
