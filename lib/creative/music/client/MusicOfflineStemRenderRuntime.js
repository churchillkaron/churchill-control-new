...
export async function renderProfessionalAudioStemOffline({ session, assetUrls, stemId, language = null, expectedDurationSeconds = null, renderWindow = null } = {}) {
  const stemSession = buildProfessionalAudioStemSession(session, stemId, { language });
  const rendered = await renderMusicMultitrackOffline({ session: stemSession, assetUrls, expectedDurationSeconds: Number.isFinite(expectedDurationSeconds) ? expectedDurationSeconds : durationOfSession(session), renderWindow });
...
export async function renderMusicTrackStemOffline({ session, assetUrls, trackId, expectedDurationSeconds = null, renderWindow = null } = {}) {
...
export async function renderMusicTrackPreFaderSendOffline({ session, assetUrls, trackId, expectedDurationSeconds = null, renderWindow = null } = {}) {
  const sendSession = buildMusicTrackPreFaderSendSession(session, trackId);
  const rendered = await renderMusicMultitrackOffline({ session: sendSession, assetUrls, expectedDurationSeconds: Number.isFinite(expectedDurationSeconds) ? expectedDurationSeconds : durationOfSession(session), renderWindow });
...
export async function renderMusicTrackEvidenceOffline({ session, assetUrls, trackId, expectedDurationSeconds = null, renderWindow = null } = {}) {
...
export async function renderMusicGroupStemOffline({ session, assetUrls, groupId, expectedDurationSeconds = null, renderWindow = null } = {}) {
...
export async function renderMusicVariantMixOffline({ session, assetUrls, variant, expectedDurationSeconds = null, renderWindow = null } = {}) {
