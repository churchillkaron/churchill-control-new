...
export function buildProfessionalAudioStemSession(sessionInput, stemId) {
  const session = clone(sessionInput);
  const selected = tracksForProfessionalAudioStem(session.tracks || [], stemId);
...
export async function renderProfessionalAudioStemOffline({ session, assetUrls, stemId, expectedDurationSeconds = null } = {}) {
  const stemSession = buildProfessionalAudioStemSession(session, stemId);
  ...
  return { ...rendered, contract: "AVANTIQO_PROFESSIONAL_AUDIO_STEM_RENDER_V1", render_kind: "POST_STEM", post_stem_id: text(stemId).toUpperCase(), master_processing_applied: true, aux_returns_applied: true, destructive_processing: false };
}
