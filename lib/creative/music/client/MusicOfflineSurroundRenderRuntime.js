...
export async function renderMusicSurroundPremasterOffline({ session, assetUrls, expectedDurationSeconds = null, renderWindow = null } = {}) {
  ...
  for (const track of audibleTracks) {
    ...
    const spatialized = spatialLanes.length
      ? mixMusicTrackIntoSpatialChannelsAutomated({ output_channels: emptySpatialChannels(spatial.channel_count), layout: spatial.layout_id, left, right, spatial: track.spatial || {}, sample_rate: sampleRate, lfe_low_pass_hz: spatial.lfe.low_pass_hz, automation_lanes: spatialLanes, timeline_offset_seconds: Math.max(0, finite(renderWindow?.start_seconds,0)) })
