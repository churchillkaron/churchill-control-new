...
import { applyMusicMultichannelBusProcessing, sumMusicMultichannelBuffers } from "./MusicMultichannelDspRuntime.js";
...
export async function renderMusicSurroundPremasterOffline({ session, assetUrls, expectedDurationSeconds = null } = {}) {
  ...
  for (const track of audibleTracks) {
    ...
    const spatialized = spatialLanes.length
      ? mixMusicTrackIntoSpatialChannelsAutomated({ output_channels: emptySpatialChannels(spatial.channel_count), layout: spatial.layout_id, left, right, spatial: track.spatial || {}, sample_rate: sampleRate, lfe_low_pass_hz: spatial.lfe.low_pass_hz, automation_lanes: spatialLanes })
      : mixMusicTrackIntoSpatialChannels({ output_channels: emptySpatialChannels(spatial.channel_count), layout: spatial.layout_id, left, right, spatial: track.spatial || {}, sample_rate: sampleRate, lfe_low_pass_hz: spatial.lfe.low_pass_hz });
    for(const send of track.sends||[]){
      ...
      const gained=applyMusicSpatialSendGainAutomation(sendLeft,sendRight,sampleRate,send,lane);auxInputs.set(send.bus_id,sumMusicStereoSendPair(auxInputs.get(send.bus_id),gained));
    ...
    trackRenders.push({ track_id: track.id, source_contract: rendered.contract, spatial: clone(track.spatial || {}), spatial_automation_lane_count: spatialLanes.length, output_bus_id: destination, render_duration_seconds: rendered.render_duration_seconds });
  }
  ...
  for(const group of groupProcessingOrder(session)){
    ...
    if(destination==="bus-master")outputChannels=sumMusicMultichannelBuffers(outputChannels,processed.channels);
    else if(groupInputs.has(destination))groupInputs.set(destination,sumMusicMultichannelBuffers(groupInputs.get(destination),processed.channels));
    else throw new Error(`CREATIVE_MUSIC_SURROUND_GROUP_OUTPUT_NOT_FOUND:${group.id}:${destination}`);
    groupRenders.push({group_id:group.id,output_bus_id:destination,gain_reduction_db_min:processed.gain_reduction_db_min,linked_compression:true});
  }
  ...
  for(const aux of auxBuses){
    ...
    auxRenders.push({bus_id:aux.id,effect_type:aux.effect_type,spatial:clone(aux.spatial||{}),tail_seconds:wet.tail_seconds,contract:wet.contract});
  }
