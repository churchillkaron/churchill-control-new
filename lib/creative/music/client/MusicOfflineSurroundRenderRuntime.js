...
import { analyseMusicDiscreteChannels, applyMusicSpatialSendGainAutomation, downmixMusicSpatialChannelsToStereo, mixMusicTrackIntoSpatialChannels, sumMusicStereoSendPair } from "./MusicSpatialMixMathRuntime.js";
...
export async function renderMusicSurroundPremasterOffline({ session, assetUrls, expectedDurationSeconds = null } = {}) {
  ...
  for (const track of audibleTracks) {
    ...
    const spatialized = mixMusicTrackIntoSpatialChannels({ output_channels: emptySpatialChannels(spatial.channel_count), layout: spatial.layout_id, left, right, spatial: track.spatial || {}, sample_rate: sampleRate, lfe_low_pass_hz: spatial.lfe.low_pass_hz });
    ...
    trackRenders.push({ track_id: track.id, source_contract: rendered.contract, spatial: clone(track.spatial || {}), output_bus_id: destination, render_duration_seconds: rendered.render_duration_seconds });
  }
