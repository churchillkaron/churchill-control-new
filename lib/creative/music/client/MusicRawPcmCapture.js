import { analyzeMusicCaptureQc } from "./MusicCaptureQcRuntime.js";
...
export async function startMusicRawPcmCapture({
  ...
  let passFrameBase = 0;
  let passContextFrameStart = null;
  let passContextFrameEnd = null;
  let chunkGapCount = 0;
  ...
  recorder.port.onmessage = (event) => {
    if (event.data?.type === "pcm" && Array.isArray(event.data.channels)) {
      ...
      const contextFrameStart = Number(event.data.context_frame_start);
      const contextFrameEnd = Number(event.data.context_frame_end);
      if (Number.isFinite(contextFrameStart) && !Number.isFinite(passContextFrameStart)) passContextFrameStart = contextFrameStart;
      if (Number.isFinite(contextFrameEnd)) passContextFrameEnd = contextFrameEnd;
  ...
  function mapContextFrameToPerformanceMs(frame) {
    if (!Number.isFinite(frame) || !Number.isFinite(context.sampleRate) || context.sampleRate <= 0) return null;
    const contextTime = frame / context.sampleRate;
    const stamp = typeof context.getOutputTimestamp === "function" ? context.getOutputTimestamp() : null;
    if (stamp && Number.isFinite(stamp.contextTime) && Number.isFinite(stamp.performanceTime)) return stamp.performanceTime + (contextTime - stamp.contextTime) * 1000;
    return null;
  }
  ...
  function materializePass({ allowEmpty = false } = {}) {
    ...
    const contextFrameDuration = Number.isFinite(passContextFrameStart) && Number.isFinite(passContextFrameEnd) ? Math.max(0,passContextFrameEnd-passContextFrameStart) : null;
    const contextDurationSeconds = Number.isFinite(contextFrameDuration) ? contextFrameDuration/context.sampleRate : null;
    const clockDriftMs = Number.isFinite(contextDurationSeconds) ? (duration-contextDurationSeconds)*1000 : null;
    const timing = {
      contract: "AVANTIQO_MUSIC_CAPTURE_TIMING_V1",
      context_frame_start: passContextFrameStart,
      context_frame_end: passContextFrameEnd,
      context_duration_seconds: contextDurationSeconds,
      pcm_duration_seconds: duration,
      clock_drift_ms: Number.isFinite(clockDriftMs) ? Number(clockDriftMs.toFixed(3)) : null,
      capture_start_performance_ms: mapContextFrameToPerformanceMs(passContextFrameStart),
      capture_end_performance_ms: mapContextFrameToPerformanceMs(passContextFrameEnd),
      output_timestamp_mapping_available: typeof context.getOutputTimestamp === "function",
      automatic_timeline_shift_allowed: false,
    };
    ...
    return {
      ...
      capture_clock_drift_ms: timing.clock_drift_ms,
    };
  }
  ...
  function resetPass() {
    ...
    passContextFrameStart = null;
    passContextFrameEnd = null;
    chunkGapCount = 0;
  ...
  return {
    contract: "AVANTIQO_MUSIC_RAW_PCM_CAPTURE_V5",
    sample_rate: context.sampleRate,
    browser_processing_disabled: truth.browser_processing_disabled,
    browser_processing_verification: truth.browser_processing_verification,
    requested_browser_processing_disabled: true,
