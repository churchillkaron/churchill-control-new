...
export function mixMusicTrackIntoSpatialChannels({ output_channels, layout: layoutInput, left, right = null, spatial = {}, sample_rate = 48000, lfe_low_pass_hz = 120 } = {}) {
  ...
  if (mode === "POINT") {
    ...
    addMonoWithGains(mono, Object.fromEntries(Object.entries(deriveMusicPointSpeakerGains(layout.id, spatial.azimuth_degrees, spatial.divergence_percent)).map(([speaker,gain])=>[speaker,gain*distanceGain])));
  } else if (mode === "WIDE") {
...
export function mixMusicTrackIntoSpatialChannelsAutomated({output_channels,layout:layoutInput,left,right=null,spatial={},sample_rate=48000,lfe_low_pass_hz=120,automation_lanes=[],timeline_offset_seconds=0}={}){
  ...
  const gainsAt=(time,side=0)=>{const az=value("spatial:azimuth_degrees",time,finite(spatial.azimuth_degrees,0)),div=value("spatial:divergence_percent",time,finite(spatial.divergence_percent,50)),distance=value("spatial:distance_meters",time,finite(spatial.distance_meters,1)),distanceGain=deriveMusicDistanceGain({...spatial,distance_meters:distance});const base=mode==="POINT"?deriveMusicPointSpeakerGains(layout.id,az,div):deriveMusicPointSpeakerGains(layout.id,az+(side<0?-value("spatial:width_percent",time,finite(spatial.width_percent,100))*.3:value("spatial:width_percent",time,finite(spatial.width_percent,100))*.3),div);return Object.fromEntries(Object.entries(base).map(([speaker,gain])=>[speaker,gain*distanceGain]));};
