...
export function applyMusicSpatialSendGainAutomation(left,right,sampleRate,send,lane=null){const frames=Math.max(left?.length||0,right?.length||0),l=new Float32Array(frames),r=new Float32Array(frames),base=finite(send?.level_db,-18);for(let i=0;i<frames;i++){const automated=lane?.enabled!==false?laneValueAtTime(lane,i/sampleRate):null;const gain=dbToGain(Number.isFinite(automated)?automated:base);l[i]=finite(left?.[i],0)*gain;r[i]=finite(right?.[i],0)*gain;}return{left:l,right:r};}
...
export function mixMusicTrackIntoSpatialChannelsAutomated({output_channels,layout:layoutInput,left,right=null,spatial={},sample_rate=48000,lfe_low_pass_hz=120,automation_lanes=[]}={}){
