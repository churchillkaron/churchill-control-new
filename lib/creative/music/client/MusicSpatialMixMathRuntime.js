...
export function deriveMusicDistanceGain({distance_meters=1,distance_model="inverse",ref_distance_meters=1,max_distance_meters=100,rolloff_factor=1}={}) {
  const distance = Math.max(0.1, finite(distance_meters, 1));
  const ref = Math.max(0.1, finite(ref_distance_meters, 1));
  const max = Math.max(ref, finite(max_distance_meters, 100));
  const rolloff = Math.max(0, finite(rolloff_factor, 1));
  const clamped = Math.min(max, distance);
  const model = String(distance_model || "inverse").toLowerCase();
  let gain = 1;
  if (model === "linear") gain = 1 - rolloff * ((clamped - ref) / Math.max(1e-9, max - ref));
  else if (model === "exponential") gain = (clamped / ref) ** (-rolloff);
  else gain = ref / (ref + rolloff * Math.max(0, clamped - ref));
  return Number(Math.max(0, Math.min(1, gain)).toFixed(8));
}

const SPEAKER_POSITIONS = Object.freeze({
  stereo: Object.freeze({ L: {az:-30,el:0}, R:{az:30,el:0} }),
  "5.1": Object.freeze({ L:{az:-30,el:0}, R:{az:30,el:0}, C:{az:0,el:0}, Ls:{az:-110,el:0}, Rs:{az:110,el:0} }),
  "7.1": Object.freeze({ L:{az:-30,el:0}, R:{az:30,el:0}, C:{az:0,el:0}, Ls:{az:-90,el:0}, Rs:{az:90,el:0}, Lrs:{az:-150,el:0}, Rrs:{az:150,el:0} }),
  "7.1.4": Object.freeze({ L:{az:-30,el:0}, R:{az:30,el:0}, C:{az:0,el:0}, Ls:{az:-90,el:0}, Rs:{az:90,el:0}, Lrs:{az:-150,el:0}, Rrs:{az:150,el:0}, Tfl:{az:-30,el:45}, Tfr:{az:30,el:45}, Tbl:{az:-150,el:45}, Tbr:{az:150,el:45} }),
});

function sphericalAngularDistance(azA,elA,azB,elB){const a1=wrapDegrees(azA)*Math.PI/180,e1=clamp(elA,-90,90,0)*Math.PI/180,a2=wrapDegrees(azB)*Math.PI/180,e2=clamp(elB,-90,90,0)*Math.PI/180,cos=Math.sin(e1)*Math.sin(e2)+Math.cos(e1)*Math.cos(e2)*Math.cos(a1-a2);return Math.acos(Math.max(-1,Math.min(1,cos)))*180/Math.PI;}
export function deriveMusicPointSpeakerGains(layoutInput, azimuthDegrees = 0, divergencePercent = 0, elevationDegrees = 0) {
  const layout = normalizeMusicSpatialLayout(layoutInput);
  const azimuth = wrapDegrees(azimuthDegrees), elevation = clamp(elevationDegrees,-90,90,0);
  const speakers = layout.speaker_order.filter((speaker) => speaker !== "LFE" && SPEAKER_POSITIONS[layout.id]?.[speaker]);
  if (!speakers.length) return {};
  const ranked = speakers.map((speaker) => ({ speaker, distance: sphericalAngularDistance(azimuth,elevation,SPEAKER_POSITIONS[layout.id][speaker].az,SPEAKER_POSITIONS[layout.id][speaker].el) })).sort((a, b) => a.distance - b.distance);
...
export function mixMusicTrackIntoSpatialChannels({ output_channels, layout: layoutInput, left, right = null, spatial = {}, sample_rate = 48000, lfe_low_pass_hz = 120 } = {}) {
  ...
  if (mode === "POINT") {
    const distanceGain = deriveMusicDistanceGain(spatial);
    addMonoWithGains(mono, Object.fromEntries(Object.entries(deriveMusicPointSpeakerGains(layout.id, spatial.azimuth_degrees, spatial.divergence_percent, spatial.elevation_degrees)).map(([speaker,gain])=>[speaker,gain*distanceGain])));
  } else if (mode === "WIDE") {
    const widthDegrees = clamp(spatial.width_percent, 0, 200, 100) * 0.6;
    const leftGains = deriveMusicPointSpeakerGains(layout.id, finite(spatial.azimuth_degrees, 0) - widthDegrees / 2, spatial.divergence_percent, spatial.elevation_degrees);
    const rightGains = deriveMusicPointSpeakerGains(layout.id, finite(spatial.azimuth_degrees, 0) + widthDegrees / 2, spatial.divergence_percent, spatial.elevation_degrees);
    const distanceGain = deriveMusicDistanceGain(spatial);
    addMonoWithGains(l, Object.fromEntries(Object.entries(leftGains).map(([speaker, gain]) => [speaker, gain * distanceGain / Math.SQRT2])));
    addMonoWithGains(r, Object.fromEntries(Object.entries(rightGains).map(([speaker, gain]) => [speaker, gain * distanceGain / Math.SQRT2])));
  } else {
    ...
    const centerGain = dbToGain(finite(spatial.center_send_db,-120));
    ...
    const surroundGain = dbToGain(finite(spatial.surround_send_db,-120));
  ...
  const lfeGain = dbToGain(finite(spatial.lfe_send_db,-120));
...
export function mixMusicTrackIntoSpatialChannelsAutomated({output_channels,layout:layoutInput,left,right=null,spatial={},sample_rate=48000,lfe_low_pass_hz=120,automation_lanes=[],timeline_offset_seconds=0}={}){
  ...
  const gainsAt=(time,side=0)=>{const az=value("spatial:azimuth_degrees",time,finite(spatial.azimuth_degrees,0)),el=value("spatial:elevation_degrees",time,finite(spatial.elevation_degrees,0)),div=value("spatial:divergence_percent",time,finite(spatial.divergence_percent,50)),distance=value("spatial:distance_meters",time,finite(spatial.distance_meters,1)),distanceGain=deriveMusicDistanceGain({...spatial,distance_meters:distance});const base=mode==="POINT"?deriveMusicPointSpeakerGains(layout.id,az,div,el):deriveMusicPointSpeakerGains(layout.id,az+(side<0?-value("spatial:width_percent",time,finite(spatial.width_percent,100))*.3:value("spatial:width_percent",time,finite(spatial.width_percent,100))*.3),div,el);return Object.fromEntries(Object.entries(base).map(([speaker,gain])=>[speaker,gain*distanceGain]));};
  ...
  const lfeGain=dbToGain(finite(spatial.lfe_send_db,-120));if(lfeGain&&index.has("LFE")){const filtered=lowPassCascade(mono,sample_rate,lfe_low_pass_hz),out=channels[index.get("LFE")];for(let i=0;i<frames;i++)out[i]+=filtered[i]*lfeGain;}return channels;
}
