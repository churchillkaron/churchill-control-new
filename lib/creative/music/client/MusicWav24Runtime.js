...
export function encodeMusicChannelsWav24(channelsInput = [], sampleRate = 48000, options = {}) {
  ...
  const channelCount = Math.max(1, Math.min(16, channels.length));
  ...
  const channelMask = channelCount === 6 ? 0x060f : channelCount === 8 ? 0x063f : channelCount === 12 ? 0x2d63f : channelCount === 2 ? 0x0003 : channelCount === 1 ? 0x0004 : 0;
...
export function encodeMusicAudioBufferWav24(audioBuffer, options = {}) {
  ...
  const channels = [];
  for (let index = 0; index < Math.min(16, audioBuffer.numberOfChannels); index += 1) channels.push(audioBuffer.getChannelData(index));
