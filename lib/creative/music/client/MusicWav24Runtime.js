...
export function encodeMusicChannelsWav24(channelsInput = [], sampleRate = 48000, options = {}) {
  ...
  const extensible = channelCount > 2;
  const headerBytes = extensible ? 68 : 44;
  ...
  if (extensible) {
    ...
    [0x80,0x00,0x00,0xaa,0x00,0x38,0x9b,0x71].forEach((byte,index)=>view.setUint8(52+index,byte));
    writeText(60, "data");
  ...
  return {
    contract: "AVANTIQO_MUSIC_WAV24_ENCODER_V3",
    blob: new Blob([buffer], { type: "audio/wav" }),
    array_buffer: buffer,
    sample_rate: rate,
    channels: channelCount,
    channel_layout: layout || (channelCount === 6 ? "5.1" : channelCount === 8 ? "7.1" : channelCount === 2 ? "stereo" : "mono"),
...
export function encodeMusicAudioBufferWav24(audioBuffer, options = {}) {
  ...
  const channels = [];
  for (let index = 0; index < Math.min(8, audioBuffer.numberOfChannels); index += 1) {
