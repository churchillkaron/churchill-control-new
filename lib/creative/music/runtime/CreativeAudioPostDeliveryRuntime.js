...
export function sealAudioPostDeliveryPackage(packageState={},input={}){
  ...
  const evidence={contract:"AVANTIQO_AUDIO_POST_DELIVERY_SEAL_V1",engine_contract:packageState.engine_contract,project_revision:packageState.project_revision,picture_lock_digest:packageState.picture_lock_digest,language:packageState.language,channel_layout:packageState.channel_layout,channels:packageState.channels,full_mix_asset_id:packageState.full_mix?.asset_id||null,stem_assets:(packageState.stems||[]).map(row=>({delivery_code:row.delivery_code,asset_id:row.asset_id,channels:row.channels,channel_layout:row.channel_layout,technical_qc_passed:row.technical_qc_passed})).sort((a,b)=>a.delivery_code.localeCompare(b.delivery_code)),me_asset_id:packageState.me?.asset_id||null,final_qc:{contract:packageState.final_qc?.contract||null,passed:packageState.final_qc?.passed===true,master_asset_id:packageState.final_qc?.master_asset_id||null,fold_down_passed:packageState.final_qc?.fold_down_qc?.passed===true,me_coherence_passed:packageState.final_qc?.me_coherence?.passed===true,language_version:packageState.final_qc?.language_version||null},listening_review:{contract:packageState.listening_review?.contract||null,status:packageState.listening_review?.status||null,approved:packageState.listening_review?.approved===true,current:packageState.listening_review?.current===true,binding_hash:packageState.listening_review?.binding_hash||null,reviewed_at:packageState.listening_review?.reviewed_at||null,reviewer_user_id:packageState.listening_review?.reviewer_user_id||null},release_ready:true};
