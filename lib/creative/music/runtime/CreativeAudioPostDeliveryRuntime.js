...
export function buildAudioPostDeliveryPackage({session={},assets=[],language="en",require_full_mix=true}={}){
  ...
  return{contract:CONTRACT,engine_contract:"AVANTIQO_PROFESSIONAL_AUDIO_ENGINE_V1",project_revision:revision,picture_lock_digest:session.picture_lock?.picture_lock_digest||null,language:lang,re_recording_mix:reRecordingMix,channel_layout:session.spatial_audio?.layout_id||"stereo",channels:expectedChannels,editorial,full_mix:{required:require_full_mix,asset_id:master?.id||null,kind:master?assetKind(master):null,release_candidate:master?.metadata?.release_candidate===true,technical_qc_passed:master?master.metadata?.technical_validation_passed!==false&&master.metadata?.surround_validation_passed!==false:false},stems:stemRows,me:{required:manifest.me.required_for_audio_post,asset_id:me?.asset_id||null,technical_qc_passed:me?.technical_qc_passed===true},warnings:[...new Set(warnings)],failures:uniqueFailures,release_ready:uniqueFailures.length===0,picture_lock_required:true,current_revision_only:true,editorial_approval_required:true,role_guessing_forbidden:true,original_sources_preserved:true};
}
...
export function sealAudioPostDeliveryPackage(packageState={},input={}){
  ...
  const evidence={contract:"AVANTIQO_AUDIO_POST_DELIVERY_SEAL_V1",engine_contract:packageState.engine_contract,project_revision:packageState.project_revision,picture_lock_digest:packageState.picture_lock_digest,language:packageState.language,channel_layout:packageState.channel_layout,channels:packageState.channels,full_mix_asset_id:packageState.full_mix?.asset_id||null,stem_assets:(packageState.stems||[]).map(row=>({delivery_code:row.delivery_code,asset_id:row.asset_id,channels:row.channels,channel_layout:row.channel_layout,technical_qc_passed:row.technical_qc_passed})).sort((a,b)=>a.delivery_code.localeCompare(b.delivery_code)),me_asset_id:packageState.me?.asset_id||null,release_ready:true};
