...
export function evaluateAudioPostFinalQc({session={},assets=[],language="en",duration_tolerance_seconds=.05}={}){
  ...
  const stems=currentAssets.filter(asset=>kind(asset)==="PROFESSIONAL_AUDIO_STEM"&&(!["DX","VO","ADR"].includes(code(asset))||text(asset.metadata?.delivery_language).toLowerCase()===lang));
  ...
  const master=currentAssets.find(asset=>["SURROUND_MASTER","MASTER"].includes(kind(asset))&&asset.metadata?.release_candidate===true&&(!hasLanguageRoles||text(asset.metadata?.delivery_language).toLowerCase()===lang))||null;
  if(!master)failures.push("AUDIO_POST_FINAL_QC_MASTER_REQUIRED");
  const deliveryProfileId=text(master?.metadata?.delivery_profile_id||"picture_post")||"picture_post";
  const profileDrift=stems.filter(asset=>text(asset.metadata?.delivery_profile_id||deliveryProfileId)!==deliveryProfileId);if(profileDrift.length)failures.push("AUDIO_POST_DELIVERY_PROFILE_DRIFT");
  const referenceDuration=master?durationOf(master):Math.max(0,...stems.map(durationOf).filter(Number.isFinite));
  const bwfRequired=Boolean(session.picture_lock?.picture_lock_digest);
  const bwfRows=[];
  if(bwfRequired){if(master&&master.metadata?.bwf_verified!==true)failures.push("AUDIO_POST_MASTER_BWF_REQUIRED");for(const asset of stems){const passed=asset.metadata?.bwf_verified===true&&asset.metadata?.bext_present===true&&Number.isFinite(finite(asset.metadata?.time_reference_samples,null));if(!passed)failures.push(`AUDIO_POST_STEM_BWF_REQUIRED:${code(asset)}`);bwfRows.push({delivery_code:code(asset),asset_id:asset.id,passed,start_timecode:asset.metadata?.start_timecode||null,time_reference_samples:asset.metadata?.time_reference_samples??null});}}
  const durationRows=[];
  ...
  return{contract:CONTRACT,delivery_profile_id:deliveryProfileId,passed:uniqueFailures.length===0,release_qc_ready:uniqueFailures.length===0,project_revision:revision,language:lang,expected_layout:expectedLayout,expected_channels:expectedChannels,master_asset_id:master?.id||null,reference_duration_seconds:referenceDuration,duration_tolerance_seconds:duration_tolerance_seconds,duration_alignment:durationRows,format_alignment:formatRows,bwf_delivery:{required:bwfRequired,master_passed:!bwfRequired||master?.metadata?.bwf_verified===true,stems:bwfRows},fold_down_qc:foldDown,language_version:{clip_count:languageClips.length,mismatch_count:languageMismatches.length,unapproved_count:unapproved.length},me_coherence:{required:meRequired,asset_id:me?.id||null,source_coherence_checked:Boolean(me),passed:!uniqueFailures.includes("AUDIO_POST_ME_SOURCE_COHERENCE_FAILED")},dialogue_intelligibility:dialogueIntelligibility,failures:uniqueFailures,warnings:[...new Set(warnings)],actual_rendered_asset_metadata_is_authority:true,provider_job_submitted:false,artistic_acceptance_performed:false};
}
