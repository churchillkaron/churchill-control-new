...
import { createMusicVocalCorrectionRequestTask } from "./CreativeMusicVocalCorrectionTaskRuntime";

const CONTRACT = "AVANTIQO_MUSIC_AUTO_STUDIO_LOCAL_EXECUTION_V3";
...
async function createFinishTask({ organizationId, projectId, missionId, sourceTask, plan }) {
  ...
  return ProductionTaskRuntime.create({
    ...
    metadata: {
      ...
      storage_policy: { bucket: MUSIC_BUCKET },
      mastered_music_source_version: Number(sourceTask?.metadata?.music_source_version || 1),
    },
  });
}
...
export async function executeMusicAutoStudioLocal(input = {}) {
  ...
  const vocalCorrectionRequest = await createMusicVocalCorrectionRequestTask({
    organizationId,
    projectId,
    missionId,
    plan,
  ...
  // Keep the currently certified/local path intact. V2 correction is durable
  // queued work and may only execute through the governed owned Modal Music capability.
  // Once that work completes and passes human review, a later continuation will
  // master the corrected V2 source instead of silently bypassing certification.
  ...
  const planBlockers = plan.readiness.elite_studio_blockers.filter((blocker) => blocker.stage !== "vocal_engineering");
  const vocalBlocker = vocalCorrectionRequest.created
    ? {
        stage: "vocal_engineering",
        code: "CERTIFIED_VOCAL_CORRECTION_PENDING",
        message: "The restored V1 vocal has a durable V2 pitch/timing correction request. It must execute through the certified owned Modal vocal-correction capability and pass human listening review before the corrected source can replace V1 in the master.",
        task_id: vocalCorrectionRequest.task.id,
        governed_execution_required: true,
        execution_contract: "AVANTIQO_OWNED_AUDIO_GOVERNED_V2",
        execution_lane: "ai.audio.vocal-correct",
      }
    : vocalCorrectionRequest.eligibility.blocker;
  const remainingBlockers = [...planBlockers, ...(vocalBlocker ? [vocalBlocker] : [])];
  ...
  return {
    ...
    vocal_correction_task_id: vocalCorrectionRequest.task?.id || null,
    ...
    vocal_engineering: {
      ...
      correction_request: {
        created: vocalCorrectionRequest.created,
        task_id: vocalCorrectionRequest.task?.id || null,
        eligibility: vocalCorrectionRequest.eligibility,
        source_music_version: 1,
        target_music_version: vocalCorrectionRequest.created ? 2 : null,
        governed_execution_required: vocalCorrectionRequest.created,
        execution_contract: vocalCorrectionRequest.created ? "AVANTIQO_OWNED_AUDIO_GOVERNED_V2" : null,
        execution_lane: vocalCorrectionRequest.created ? "ai.audio.vocal-correct" : null,
        provider_job_submitted: false,
      },
    },
    output: publicOutput(completed.output),
    execution: {
      runtime: "AVANTIQO_MUSIC_LOCAL_RESTORATION_PLUS_AUDIO_FINISHING_WITH_DURABLE_VOCAL_CORRECTION_REQUEST",
      local_execution: true,
      mastered_music_source_version: 1,
      external_cloud_job_submitted: false,
      provider_job_submitted: false,
      endpoint_mutation_performed: false,
      direct_workers_max_write: false,
      governed_gpu_execution_used_for_this_execution: false,
      future_vocal_correction_governed_execution_required: vocalCorrectionRequest.created,
    },
  };
}
