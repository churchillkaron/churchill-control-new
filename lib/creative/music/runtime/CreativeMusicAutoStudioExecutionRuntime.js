...
export async function executeMusicAutoStudioLocal(input = {}) {
  ...
  // queued work and may only execute from the Safe Lease controlled Music lane.
  ...
  const vocalBlocker = vocalCorrectionRequest.created
    ? {
        ...
        message: "The restored V1 vocal has a durable V2 pitch/timing correction request. It must execute through Safe Lease V2 and pass human listening review before the corrected source can replace V1 in the master.",
  ...
  return {
    ...
    vocal_engineering: {
      ...
      correction_request: {
        ...
        safe_lease_required: vocalCorrectionRequest.created,
        safe_lease_contract: vocalCorrectionRequest.created ? "AVANTIQO_RUNPOD_SAFE_LEASE_V2" : null,
        safe_lease_lane: vocalCorrectionRequest.created ? "music-vocal-correction" : null,
    ...
    execution: {
      ...
      future_vocal_correction_safe_lease_required: vocalCorrectionRequest.created,
    },
  };
}
