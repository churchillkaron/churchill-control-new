...
function stage({
  ...
  return {
    ...
    automatic,
    required,
    governed_execution_required: governedExecution,
    execution_contract: governedExecution ? OWNED_EXECUTION_CONTRACT : null,
    notes,
  };
}
...
export function buildMusicAutoStudioPlan(input = {}) {
    ...
    ...(stemIntelligence
      ? [stage({
          ...
          notes: [
            ...
            "Execution may only route through the certified owned Modal audio capability; direct provider bypass is forbidden.",
          ],
        })]
      : []),
    ...(vocalEngineering
  ...
  return {
    ...
    execution_policy: {
      local_audio_finishing_requires_gpu: false,
      gpu_stage_requires_governed_execution: stages.some((entry) => entry.governed_execution_required),
      owned_execution_contract: OWNED_EXECUTION_CONTRACT,
      modal_only_gpu_execution: true,
