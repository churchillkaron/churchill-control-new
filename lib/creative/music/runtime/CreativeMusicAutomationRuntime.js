const CONTRACT = "AVANTIQO_MUSIC_MIXER_AUTOMATION_V2";
const TARGET_TYPES = Object.freeze(["track", "group", "master"]);
const PARAMETERS = Object.freeze(["gain_db", "pan", "send_db:bus-reverb", "send_db:bus-delay", "send_db:bus-vocal-parallel", "spatial:azimuth_degrees", "spatial:elevation_degrees", "spatial:distance_meters", "spatial:width_percent", "spatial:divergence_percent", "spatial:occlusion_percent", "spatial:reflection_amount_percent"]);
...
function clampValue(parameter, value) {
  if (parameter === "pan") return Math.max(-1, Math.min(1, finite(value, 0)));
  if (parameter === "spatial:azimuth_degrees") return Math.max(-180, Math.min(180, finite(value, 0)));
  if (parameter === "spatial:elevation_degrees") return Math.max(-90, Math.min(90, finite(value, 0)));
  if (parameter === "spatial:distance_meters") return Math.max(0.1, Math.min(500, finite(value, 2)));
  if (parameter === "spatial:width_percent") return Math.max(0, Math.min(200, finite(value, 100)));
  if (parameter === "spatial:divergence_percent") return Math.max(0, Math.min(100, finite(value, 50)));
  if (parameter === "spatial:occlusion_percent") return Math.max(0, Math.min(100, finite(value, 0)));
  if (parameter === "spatial:reflection_amount_percent") return Math.max(0, Math.min(100, finite(value, 25)));
  return Math.max(-60, Math.min(12, finite(value, 0)));
}
...
export function createMusicAutomationLane(input = {}) {
  ...
  if (parameter.startsWith("spatial:") && targetType !== "track") throw new Error("CREATIVE_MUSIC_SPATIAL_AUTOMATION_TRACK_ONLY");
