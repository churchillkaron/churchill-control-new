...
export function musicCompRenderBasis(comp = {}) {
  const regions = (Array.isArray(comp.regions) ? comp.regions : []).map((region) => ({
    id: text(region.id) || null,
    take_id: text(region.take_id),
    source_asset_id: text(region.source_asset_id),
    start_seconds: finite(region.start_seconds, 0),
    end_seconds: finite(region.end_seconds, 0),
    source_offset_seconds: Math.max(0, finite(region.source_offset_seconds, 0)),
    gain_db: finite(region.gain_db, 0),
    fade_in_seconds: Math.max(0, finite(region.fade_in_seconds, 0)),
    fade_out_seconds: Math.max(0, finite(region.fade_out_seconds, 0)),
  }));
  return {
    contract: "AVANTIQO_MUSIC_COMP_RENDER_BASIS_V1",
    comp_id: text(comp.id),
    track_id: text(comp.track_id),
    start_seconds: finite(comp.start_seconds, regions[0]?.start_seconds || 0),
    end_seconds: finite(comp.end_seconds, regions.at(-1)?.end_seconds || 0),
    duration_seconds: finite(comp.duration_seconds, Math.max(0, finite(comp.end_seconds, 0) - finite(comp.start_seconds, 0))),
    crossfade_default_seconds: Math.max(0, finite(comp.crossfade_default_seconds, 0)),
    regions,
  };
}
...
export const CreativeMusicCompingRuntime = {
  ...
  renderBasis: musicCompRenderBasis,
};
