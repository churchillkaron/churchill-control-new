...
function normalizeRange(value = {}) {
  ...
  return {
    ...
    version_id: text(value.version_id, 160) || null,
    provenance_fingerprint: text(value.provenance_fingerprint, 160) || null,
  };
}
...
export function emptyMusicConversationState() {
  return {
    ...
    version_lineage: [],
    current_master_asset_id: null,
    current_version_id: null,
    listening_evidence: null,
    professional_production: null,
    professional_next_stage: null,
    professional_production_state: null,
    recent_decisions: [],
    revision: 0,
    updated_at: null,
  };
...
export function mergeMusicConversationState(current = {}, patch = {}) {
  ...
  const next = {
    ...
    current_master_asset_id: text(patch.current_master_asset_id ?? base.current_master_asset_id, 160) || null,
    current_version_id: text(patch.current_version_id ?? base.current_version_id, 160) || null,
    listening_evidence: patch.listening_evidence ?? base.listening_evidence ?? null,
    professional_production: patch.professional_production ?? base.professional_production ?? null,
    professional_next_stage: patch.professional_next_stage ?? base.professional_next_stage ?? null,
    professional_production_state: patch.professional_production_state ?? base.professional_production_state ?? null,
...
export function musicConversationExecutionContext(state = {}) {
  ...
  return {
    ...
    current_master_asset_id: source.current_master_asset_id,
    current_version_id: source.current_version_id,
    listening_evidence: source.listening_evidence || null,
    professional_production: source.professional_production || null,
    professional_next_stage: source.professional_next_stage || null,
    professional_production_state: source.professional_production_state || null,
