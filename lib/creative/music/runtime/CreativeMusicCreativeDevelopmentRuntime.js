...
export function validateMusicPreproductionBrief(brief = {}, options = {}) {
  ...
  const tempo = object(brief.tempo_map);
  const tempoCandidates = [tempo.bpm, tempo.primary_bpm, tempo.tempo_bpm, ...Object.keys(tempo)];
  if (!tempoCandidates.some((value) => Number.isFinite(Number(value)))) failures.push("MUSIC_PREPRODUCTION_BPM_REQUIRED");
