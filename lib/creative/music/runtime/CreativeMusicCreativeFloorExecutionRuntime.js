...
async function critic(input, criticSpec, conceptRow, researchReport) {
  const result = await runReason(`MUSIC_STUDIO_CRITIC_${criticSpec.id.toUpperCase()}`, input, {
    critic: criticSpec,
    concept: conceptRow,
    research: researchReport,
    independent_blind_review: true,
  }, { score: 0, passed: false, evidence: ["string"], failures: ["string"], repair: ["string"] });
...
export async function executeMusicCreativeFloor(input = {}) {
  ...
  for (const conceptRow of concepts) {
    for (const criticSpec of development.concept_competition.critics) {
      criticReviews.push(await critic(input, criticSpec, conceptRow, researchReport));
    }
  }
