...
async function songwriterLock(input, finalConcept, researchReport) {
  ...
  const constraints = {
    ...
    research: researchReport,
    source_lyrics: text(input.lyrics || input.lyric_source || input.source_lyrics),
    source_lyrics_are_emotional_material_not_a_rigid_final_draft: true,
    preserve_core_meaning_and_personal_voice: true,
    improve_grammar_meter_singability_repetition_hook_and_section_structure: true,
    tasteful_new_lines_allowed_when_they_support_the_same_story: true,
  ...
  const first = await runReason("MUSIC_STUDIO_SONGWRITER_LOCK", input, constraints, outputShape);
  const validation = validateSongwriterContract(first);
  if (validation.passed) return first;
  const repaired = await runReason("MUSIC_STUDIO_SONGWRITER_REPAIR", {
...
async function preproduction(input, finalConcept, researchReport, songwriterContract = null) {
  ...
  const outputShape = {
    ...
    vocal_contract: { required: false, lyrics: "string", vocal_language: "string", vocal_delivery: "string", preserve_core_meaning: true },
  };
  const constraints = {
    approved_concept: finalConcept,
    research: researchReport,
    ...
    supplied_lyrics_are_source_material_not_rigid_copy: vocalRequired,
