...
async function preproduction(input, finalConcept, researchReport) {
  ...
  const first = await runReason("MUSIC_STUDIO_PREPRODUCTION_LOCK", input, constraints, outputShape);
  ...
  const repaired = await runReason("MUSIC_STUDIO_PREPRODUCTION_REPAIR", {
    ...input,
