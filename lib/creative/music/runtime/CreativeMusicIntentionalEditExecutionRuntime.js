...
export async function executeMusicIntentionalEdit({
  ...
  if (!organization_id || !creative_project_id || !expected_change_set_fingerprint) {
  ...
  const resolvedMasterAssetId = text(master_asset_id || conversationContext.current_master_asset_id);
  if (!resolvedMasterAssetId) {
    const error = new Error("CREATIVE_MUSIC_CURRENT_MASTER_REQUIRED");
    error.status = 409;
    error.code = "CREATIVE_MUSIC_CURRENT_MASTER_REQUIRED";
    throw error;
  }
  const sourceAsset = await exactAsset({ organization_id, creative_project_id, master_asset_id: resolvedMasterAssetId });
  ...
  const result = await executeService({
    ...
    metadata: {
      ...
      creative_project_id,
      creative_mission_id,
      source_master_asset_id: resolvedMasterAssetId,
      music_change_set_fingerprint: changeSet.change_set_fingerprint,
  ...
  const currentMasterSettlement = await settleCurrentMusicMaster({
    organization_id,
    creative_project_id,
    master_asset_id: finishing.master_asset.id,
    parent_master_asset_id: resolvedMasterAssetId,
    summary: changeSet.intended_delta,
  });
  return {
    contract: CONTRACT,
    ...
    current_master_settlement: currentMasterSettlement,
    finishing,
    dailies,
    tribunal,
