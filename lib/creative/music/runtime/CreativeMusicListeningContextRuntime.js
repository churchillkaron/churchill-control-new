...
function semanticBrief(semantic = {}, sections = []) {
  ...
  if (rhythm.rhythm_analysis_ready === true) {
    ...
    const meter = object(rhythm.meter_intelligence);
    if (meter.meter_analysis_ready === true && text(meter.time_signature_candidate, 32)) {
      brief.push(`Measured meter candidate: ${text(meter.time_signature_candidate, 32)} (downbeat confidence ${rounded(meter.downbeat_confidence, 3)}).`);
    }
  }
  ...
  const melody = object(source.melody_intelligence);
  const form = object(source.form_intelligence);
  if (melody.melody_analysis_ready === true) {
    const motifs = list(melody.repeated_motif_candidates).slice(0, 3);
    if (motifs.length) {
      for (const motif of motifs) {
        const first = finite(motif.first_start_seconds); const repeat = finite(motif.repeat_start_seconds);
        if (first === null || repeat === null) continue;
        const section = sectionAt(first, sections);
        brief.push(`Measured repeated melodic-motif candidate around ${rounded(first,2)}s${section ? ` in ${section}` : ""} and ${rounded(repeat,2)}s (similarity ${rounded(motif.similarity,3)}).`);
      }
    } else {
      const phrase = list(melody.phrase_candidates)[0];
      if (phrase) brief.push(`Measured dominant melodic contour candidate at ${rounded(phrase.start_seconds,2)}-${rounded(phrase.end_seconds,2)}s (${String(phrase.contour_direction||"mixed").toLowerCase()} contour).`);
    }
  }
  if (form.form_analysis_ready === true) {
    const sequence = list(form.neutral_regions).map((row) => text(row.neutral_label, 16)).filter(Boolean).join(" → ");
    if (sequence) brief.push(`Measured neutral form: ${sequence}. Semantic names such as verse/chorus are not inferred from audio.`);
    for (const item of list(form.return_candidates).slice(0, 2)) {
      const at = finite(item.at_seconds);
      if (at === null) continue;
      const section = sectionAt(at, sections);
      brief.push(`Measured structural return candidate ${text(item.neutral_label, 16)} at ${rounded(at, 2)}s${section ? ` in approved ${section}` : ""}${finite(item.recurrence_similarity) !== null ? ` (similarity ${rounded(item.recurrence_similarity, 3)})` : ""}.`);
    }
    for (const item of list(form.adjacent_contrasts).filter((row) => row.contrast_state === "HIGH").slice(0, 2)) {
      brief.push(`Measured high structural contrast ${text(item.from_label, 16)} → ${text(item.to_label, 16)} at ${rounded(item.at_seconds, 2)}s.`);
    }
  }
...
export function buildMusicListeningContext({ evidence = {}, current = {}, sections = [] } = {}) {
  ...
  return Object.freeze({
    ...
    downbeat_analysis_available: semantic.rhythm_intelligence?.downbeat_analysis_ready === true,
    melody_analysis_available: semantic.melody_intelligence?.melody_analysis_ready === true,
    form_analysis_available: semantic.form_intelligence?.form_analysis_ready === true,
