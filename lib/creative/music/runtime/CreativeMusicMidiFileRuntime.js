const CONTRACT = "AVANTIQO_MUSIC_STANDARD_MIDI_FILE_V1";
...
export function parseStandardMidiFile(input) {
  ...
  for (let trackIndex = 0; trackIndex < trackCount; trackIndex += 1) {
    ...
    tracks.push({ name, midi_channel: primaryChannel, start_beat: 0, duration_beats: endBeat, notes, control_events: controls });
    offset = end;
  }
...
export function encodeStandardMidiFile({ midi, bpm = 120, time_signature = "4/4" } = {}) {
  const ppq = Math.round(clamp(midi?.ppq, 96, 3840, 960));
  const tracks = Array.isArray(midi?.tracks) ? midi.tracks : [];
  const [numeratorRaw, denominatorRaw] = text(time_signature || "4/4").split("/");
  ...
  const conductor = [
    ...
    0x00, 0xff, 0x58, 0x04, numerator, Math.round(Math.log2(denominator)), 24, 8,
    0x00, 0xff, 0x2f, 0x00,
  ];
