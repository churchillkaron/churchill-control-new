import { MUSIC_OWNED_INSTRUMENT_CONTRACT, normalizeMusicOwnedInstrument } from "./CreativeMusicOwnedInstrumentRuntime.js";
...
export function createMusicMidiTrack(input = {}) {
  return {
    ...
    instrument: {
      ...
      design: input.instrument?.design ? normalizeMusicOwnedInstrument(input.instrument.design, input.instrument?.preset_id || "studio_keys") : null,
...
export function validateMusicMidiProject(project = {}) {
  ...
  for (const track of project.tracks) {
    ...
    if (track.instrument?.design) {
      const normalizedInstrument = normalizeMusicOwnedInstrument(track.instrument.design, track.instrument?.preset_id || "studio_keys");
      if (track.instrument.design.contract !== MUSIC_OWNED_INSTRUMENT_CONTRACT || track.instrument.design.fingerprint !== normalizedInstrument.fingerprint) {
        throw new Error("CREATIVE_MUSIC_MIDI_INSTRUMENT_DESIGN_INVALID");
      }
    }
