...
export function analyzeMusicMixEngineer(session={}, evidenceBundle={}, arrangement={}, baseline=null){
  ...
  if(finite(evidenceBundle?.ambiguous_source_track_count,0)>0) issues.push({code:"TRACK_RENDER_REQUIRED_FOR_EVIDENCE",message:`${Math.round(finite(evidenceBundle.ambiguous_source_track_count,0))} edited track${finite(evidenceBundle.ambiguous_source_track_count,0)===1?"":"s"} contain multiple source assets. Source-level evidence is withheld until a current track render is available.`});
