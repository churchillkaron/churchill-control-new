...
function trackDecision(track={}, evidence={}){
  ...
  if(r==="VOCAL"){
    ...
    const vocalLowCrest=measured&&crest!==null&&crest<8, vocalHighCrest=measured&&crest!==null&&crest>14; const vocalRatio=vocalLowCrest?2:vocalHighCrest?3.2:3, vocalAttack=vocalLowCrest?28:vocalHighCrest?16:20, vocalMakeup=vocalLowCrest?.5:1; strip.compressor={...strip.compressor,enabled:true,threshold_db:-20,ratio:vocalRatio,attack_ms:vocalAttack,release_ms:110,knee_db:6,makeup_db:vocalMakeup}; if(vocalLowCrest) decisions.push("preserve measured vocal transient life");
    ...
    const bassLowCrest=measured&&crest!==null&&crest<8, bassHighCrest=measured&&crest!==null&&crest>12; strip.compressor={...strip.compressor,enabled:true,threshold_db:-18,ratio:bassLowCrest?2.5:3.5,attack_ms:bassLowCrest?34:bassHighCrest?28:30,release_ms:140,knee_db:5,makeup_db:0}; if(bassLowCrest) decisions.push("protect measured bass articulation"); pan=0;
    decisions.push("mono-compatible low-end anchor","controlled sustain","preserve attack");
  } else if(r==="DRUMS"){
    strip.high_pass_hz=Math.max(28,finite(strip.high_pass_hz,20)); const drumLowCrest=measured&&crest!==null&&crest<9, drumHighCrest=measured&&crest!==null&&crest>14; strip.compressor={...strip.compressor,enabled:true,threshold_db:-16,ratio:drumLowCrest?1.8:drumHighCrest?2.4:2.2,attack_ms:drumLowCrest?42:drumHighCrest?30:34,release_ms:drumLowCrest?120:90,knee_db:4,makeup_db:drumLowCrest?0:0.25}; if(drumLowCrest) decisions.push("restore measured drum transient space"); saturation={drive_db:2,mix:0.08,output_db:0};
