...
const AUX_TYPES = Object.freeze(["reverb", "delay", "compressor"]);
...
export function createMusicAuxBus(input = {}) {
  ...
  const base = {
    ...
    mute: input.mute === true,
    solo: input.solo === true,
    spatial: normalizeMusicSpatialTrack(input.spatial || { mode: "BED", center_send_db: -120, lfe_send_db: -120, surround_send_db: -120 }),
    destructive_processing_allowed: false,
  };
  if (type === "reverb") {
    return {
  ...
  return {
    ...base,
    parameters: {
      threshold_db: clamp(input.parameters?.threshold_db, -60, 0, -30),
      ratio: clamp(input.parameters?.ratio, 1, 20, 8),
      attack_ms: clamp(input.parameters?.attack_ms, 0.1, 200, 3),
      release_ms: clamp(input.parameters?.release_ms, 10, 2000, 120),
      knee_db: clamp(input.parameters?.knee_db, 0, 40, 6),
      makeup_db: clamp(input.parameters?.makeup_db, -12, 24, 8),
      high_pass_hz: clamp(input.parameters?.high_pass_hz, 20, 400, 90),
      wet_db: clamp(input.parameters?.wet_db, -60, 12, -10),
    },
  };
}
...
export function ensureMusicEngineeringBuses(session = {}) {
  ...
  if (!next.buses.some((bus) => bus.id === "bus-reverb")) next.buses.push(createMusicAuxBus({ id: "bus-reverb", type: "reverb" }));
  if (!next.buses.some((bus) => bus.id === "bus-delay")) next.buses.push(createMusicAuxBus({ id: "bus-delay", type: "delay" }));
  for (const bus of next.buses) {
    if (bus.type === "aux") { bus.output_bus_id = "bus-master"; bus.spatial = normalizeMusicSpatialTrack(bus.spatial || { mode: "BED", center_send_db: -120, lfe_send_db: -120, surround_send_db: -120 }); }
  }
  return next;
}
...
export function upsertMusicGroupBus(session = {}, input = {}) {
  ...
  if (existing >= 0) {
    ...
    if (next.buses.filter((bus) => bus.type === "group").length >= GROUP_LIMIT) throw new Error(`CREATIVE_MUSIC_GROUP_BUS_LIMIT:${GROUP_LIMIT}`);
...
export function removeMusicGroupBus(session = {}, busId) {
  ...
  next.automation_lanes = (next.automation_lanes || []).filter((lane) => !(lane.target_type === "group" && lane.target_id === id));
...
export function validateMusicMixerRouting(session = {}) {
  ...
  for (const bus of buses) {
    if (bus.type === "aux" && !AUX_TYPES.includes(text(bus.effect_type))) throw new Error(`CREATIVE_MUSIC_MIXER_AUX_INVALID:${bus.id}`);
    if (bus.type === "aux" && text(bus.output_bus_id || "bus-master") !== "bus-master") throw new Error(`CREATIVE_MUSIC_AUX_OUTPUT_UNSUPPORTED:${bus.id}`);
    ...
    if (bus.type === "group") {
      ...
      if (busMap.get(output)?.type === "aux") throw new Error(`CREATIVE_MUSIC_GROUP_DIRECT_AUX_OUTPUT_FORBIDDEN:${bus.id}:${output}`);
    }
  ...
  for (const track of session.tracks || []) {
    ...
    for (const send of track.sends || []) {
      ...
      if (busMap.get(text(send.bus_id))?.type !== "aux") throw new Error(`CREATIVE_MUSIC_SEND_TARGET_NOT_AUX:${send.bus_id}`);
  ...
  return {
    ...
    aux_outputs_master_only: true,
    group_automation_cleanup_on_delete: true,
    cycle_safe: true,
