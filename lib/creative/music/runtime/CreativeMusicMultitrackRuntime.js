import { createMusicSpatialAudioConfig, normalizeMusicSpatialTrack, validateMusicSpatialAudio } from "./CreativeMusicSpatialAudioRuntime.js";
import { normalizeProfessionalAudioRole } from "./CreativeProfessionalAudioEngineRuntime.js";
import { createMusicCinematicSoundObject } from "./CreativeMusicCinematicSoundObjectRuntime.js";
...
export function createMusicMultitrackProject(input = {}) {
  ...
  return {
    ...
    automation_lanes: [],
    sound_objects: Array.isArray(input.sound_objects) ? input.sound_objects.map((object) => createMusicCinematicSoundObject(object)) : [],
    versions: [],
    spatial_audio: createMusicSpatialAudioConfig(input.spatial_audio || { layout_id: input.channel_layout || "stereo" }),
    engineering: {
    ...
    capabilities: {
      ...
      multichannel_session_model: true,
      surround_5_1_session_model: true,
      surround_7_1_session_model: true,
      browser_surround_monitoring_certified: false,
      dolby_branded_delivery_certified: false,
    },
  };
}
...
export function createMusicTrack(input = {}) {
  ...
  return {
    ...
    audio_role: normalizeProfessionalAudioRole(input.audio_role, type === "vocal" ? "SONG_VOCAL" : type === "foley" ? "FOLEY" : type === "ambience" ? "AMBIENCE" : ["guitar","bass","keys","drums","instrument","backing"].includes(type) ? "INSTRUMENT" : "PROGRAM"),
    vocal_role: type === "vocal" ? (text(input.vocal_role).toUpperCase() || null) : null,
    foley_cue_id: type === "foley" ? (text(input.foley_cue_id) || null) : null,
    picture_lock_digest: ["foley","ambience"].includes(type) ? (text(input.picture_lock_digest) || null) : null,
    ...
    spatial: normalizeMusicSpatialTrack(input.spatial || {}),
    clips: [],
    takes: [],
    comp: null,
    inserts: [],
...
export function createMusicClip(input = {}) {
  ...
  return {
    ...
    foley_cue_id: text(input.foley_cue_id) || null,
...
export function createMusicTake(input = {}) {
  ...
  return {
    ...
    source_offset_seconds: Math.max(0, finite(input.source_offset_seconds, 0)),
    source_duration_seconds: Math.max(0, finite(input.source_duration_seconds, input.duration_seconds)),
...
export function validateMusicMultitrackProject(project = {}) {
  ...
  validateMusicSpatialAudio(project.spatial_audio || { layout_id: "stereo" });
  if (!Array.isArray(project.tracks) || project.tracks.length > 128) {
    throw new Error("CREATIVE_MUSIC_MULTITRACK_TRACK_LIMIT_INVALID");
  }
  const soundObjects = Array.isArray(project.sound_objects) ? project.sound_objects : [];
  if (soundObjects.length > 512) throw new Error("CREATIVE_MUSIC_SOUND_OBJECT_LIMIT_INVALID");
  if (soundObjects.length && !project.picture_lock?.picture_lock_digest) throw new Error("CREATIVE_MUSIC_SOUND_OBJECT_PICTURE_LOCK_REQUIRED");
  const soundObjectIds = new Set();
  for (const object of soundObjects) {
    if (!object?.id || soundObjectIds.has(object.id)) throw new Error("CREATIVE_MUSIC_SOUND_OBJECT_ID_INVALID");
    soundObjectIds.add(object.id);
    if (object.contract !== "AVANTIQO_MUSIC_CINEMATIC_SOUND_OBJECT_V1") throw new Error("CREATIVE_MUSIC_SOUND_OBJECT_CONTRACT_INVALID");
    if (project.picture_lock?.picture_lock_digest && object.picture_lock_digest && object.picture_lock_digest !== project.picture_lock.picture_lock_digest) throw new Error("CREATIVE_MUSIC_SOUND_OBJECT_PICTURE_LOCK_STALE");
    if ((object.layers || []).some((layer) => layer.destructive_processing_allowed === true)) throw new Error("CREATIVE_MUSIC_SOUND_OBJECT_DESTRUCTIVE_LAYER_FORBIDDEN");
  }
  ...
  for (const track of project.tracks || []) {
    ...
    normalizeMusicSpatialTrack(track.spatial || {});
  ...
  return {
    ...
    multichannel_session_model_supported: true,
    spatial_layout_id: project.spatial_audio?.layout_id || "stereo",
    cinematic_sound_object_count: soundObjectIds.size,
    cinematic_sound_objects_persisted: true,
  };
}
