...
function rhythmPulseAnalysis(samples, sampleRate, tempo = {}) {
  ...
  const beatAccents = [];
  const maxBeats = Math.min(96, Math.floor((onset.envelope.length - bestPhase) / period));
  for (let beat = 0; beat < maxBeats; beat += 1) {
    const center = bestPhase + beat * period;
    let local = 0;
    for (let offset = -beatRadius; offset <= beatRadius; offset += 1) {
      const index = center + offset;
      if (index >= 0 && index < onset.envelope.length) local += onset.envelope[index];
    }
    beatAccents.push(local);
  }
  const accentMax = Math.max(...beatAccents, 1e-9);
  ...
  return {
    ...
    beat_accent_profile: beatAccents.map((value, index) => ({ beat_index: index, accent: Number((value / accentMax).toFixed(4)) })),
...
function dominantPitchFrame(samples, start, frameSize, sampleRate) {
  let mean = 0; for (let i=0;i<frameSize;i++) mean += samples[start+i] || 0; mean /= frameSize;
  let energy = 0; for (let i=0;i<frameSize;i++){ const v=(samples[start+i]||0)-mean; energy += v*v; }
  const rms = Math.sqrt(energy / Math.max(1, frameSize));
  if (rms < 0.008) return null;
  const minLag=Math.max(2,Math.floor(sampleRate/1000)); const maxLag=Math.min(frameSize-4,Math.ceil(sampleRate/80));
  let bestLag=0,best=-1;
  for(let lag=minLag;lag<=maxLag;lag++){ let dot=0,a=0,b=0; const usable=frameSize-lag; for(let i=0;i<usable;i++){ const x=(samples[start+i]||0)-mean,y=(samples[start+i+lag]||0)-mean; dot+=x*y;a+=x*x;b+=y*y;} const score=dot/Math.max(1e-12,Math.sqrt(a*b)); if(score>best){best=score;bestLag=lag;} }
  if(bestLag<=0 || best<0.70) return null;
  const hz=sampleRate/bestLag; const midi=69+12*Math.log2(hz/440);
  if(!Number.isFinite(midi) || midi<28 || midi>96) return null;
  return { midi:Number(midi.toFixed(2)), confidence:Number(best.toFixed(3)) };
}

function melodyMeasurement(samples, sampleRate) {
  const maxSamples=Math.min(samples.length, Math.round(sampleRate*120)); const frameSize=Math.round(sampleRate*0.08); const hop=Math.round(sampleRate*0.30);
  const points=[]; let attempted=0;
  for(let start=0; start+frameSize<=maxSamples; start+=hop){ attempted+=1; const hit=dominantPitchFrame(samples,start,frameSize,sampleRate); if(hit) points.push({time_seconds:Number((start/sampleRate).toFixed(3)),...hit}); }
  const notes=[];
  for(const point of points){ const midi=Math.round(point.midi); const prev=notes.at(-1); if(prev && prev.midi===midi && point.time_seconds-prev.end_seconds<=0.65){ prev.end_seconds=Number((point.time_seconds+0.30).toFixed(3)); prev.confidence_sum+=point.confidence; prev.frames+=1; } else notes.push({midi,start_seconds:point.time_seconds,end_seconds:Number((point.time_seconds+0.30).toFixed(3)),confidence_sum:point.confidence,frames:1}); }
  const compact=notes.filter(n=>n.frames>=2).map(n=>({midi:n.midi,start_seconds:n.start_seconds,end_seconds:n.end_seconds,confidence:Number((n.confidence_sum/n.frames).toFixed(3))}));
  const phrases=[]; let current=[]; for(const note of compact){ if(current.length && note.start_seconds-current.at(-1).end_seconds>0.9){ if(current.length>=2) phrases.push(current); current=[]; } current.push(note); } if(current.length>=2) phrases.push(current);
  const voiced=points.length; const mean=voiced?points.reduce((sum,p)=>sum+p.confidence,0)/voiced:0;
  return { contract:'AVANTIQO_MUSIC_MELODY_MEASUREMENT_V1', voiced_coverage_ratio:Number((voiced/Math.max(1,attempted)).toFixed(3)), mean_pitch_confidence:Number(mean.toFixed(3)), phrases:phrases.slice(0,24).map((notes)=>({start_seconds:notes[0].start_seconds,end_seconds:notes.at(-1).end_seconds,confidence:Number((notes.reduce((sum,n)=>sum+n.confidence,0)/notes.length).toFixed(3)),notes})), measured_from_audio:true, raw_pitch_frames_persisted:false, polyphonic_source_assumed_monophonic:false };
}
...
export async function analyzeMusicMusicalContent({
  ...
  try {
    ...
    const melody_measurement = melodyMeasurement(samples, ANALYSIS_RATE);
