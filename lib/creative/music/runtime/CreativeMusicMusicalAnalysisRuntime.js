...
function buildOnsetEnvelope(samples, sampleRate) {
  ...
  return envelope.map((value) => Math.max(0, value - mean * 0.55));
}
...
function chromaVector(samples, sampleRate) {
  ...
  let windows = 0;
  const midiNotes = [];
  for (let midi = 36; midi <= 83; midi += 1) midiNotes.push({ midi, frequency: 440 * 2 ** ((midi - 69) / 12), pitchClass: midi % 12 });
...
export async function analyzeMusicMusicalContent({
  ...
  try {
    ...
    if (Number.isFinite(Number(duration_seconds)) && Number(duration_seconds) > 0) trimArgs.push("-t", String(Math.min(900, Number(duration_seconds))));
    await run(ffmpeg, [
