...
export const MUSIC_PERCEPTUAL_TRANSLATION_CONTRACT = "AVANTIQO_MUSIC_PERCEPTUAL_TRANSLATION_V2";
...
export async function analyzeMusicPerceptualTranslation({
  ...
  try {
    ...
    const sub = await bandRms(ffmpeg, material.file_path, 30, 80);
    const punch = await bandRms(ffmpeg, material.file_path, 80, 160);
    const presence = await bandRms(ffmpeg, material.file_path, 2500, 5000);
    const air = await bandRms(ffmpeg, material.file_path, 8000, 11500);
    const lowVsBody = Number((db(low) - db(body)).toFixed(2));
    const harshVsBody = Number((db(harsh) - db(body)).toFixed(2));
    const subVsPunch = Number((db(sub) - db(punch)).toFixed(2));
    const presenceVsBody = Number((db(presence) - db(body)).toFixed(2));
    const airVsBody = Number((db(air) - db(body)).toFixed(2));
    ...
    const premiumRecordEvidence = {
      dynamic_life: { passed: stereo.crest_factor_db >= 6 && stereo.crest_factor_db <= 20, crest_factor_db: stereo.crest_factor_db },
      low_end_control: { passed: subVsPunch >= -10 && subVsPunch <= 6, sub_vs_punch_db: subVsPunch },
      upper_mid_smoothness: { passed: presenceVsBody <= 4.5, presence_vs_body_db: presenceVsBody },
      air_balance: { passed: airVsBody <= 2.5, air_vs_body_db: airVsBody },
      stereo_confidence: { passed: checks.stereo_stability && checks.mono_compatibility, correlation: stereo.stereo_correlation, mono_fold_down_loss_db: stereo.mono_fold_down_loss_db },
    };
    const premiumTechnicalPass = Object.values(premiumRecordEvidence).every((row) => row.passed === true);
    ...
    return Object.freeze({
      ...
      metrics: {
        ...
        sub_vs_punch_db: subVsPunch,
        presence_vs_body_db: presenceVsBody,
        air_vs_body_db: airVsBody,
      },
      thresholds: limits,
      premium_record_evidence: premiumRecordEvidence,
      premium_technical_pass: premiumTechnicalPass,
      ...
      premium_quality_claim_authorized: false,
