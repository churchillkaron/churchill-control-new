...
import { prepareProfessionalVocalProduction, certifyProfessionalVocalProduction } from "./CreativeMusicProfessionalVocalProductionRuntime.js";
...
import { reviewProfessionalPremasterListening, masterProfessionalPremaster, verifyProfessionalTranslation, runProfessionalDailies, runProfessionalFinalTribunal } from "./CreativeMusicProfessionalFinalizationRuntime.js";
...
function evidenceFromAsset(asset={}){
  ...
  return {
    ...
    premaster_listening_passed:m.professional_premaster_listening_passed===true,
...
export async function continueMusicProfessionalProduction(input={}){
  ...
  if(!sourceAsset || text(sourceAsset.organization_id)!==organizationId || assetProjectId(sourceAsset)!==projectId) throw new Error("CREATIVE_MUSIC_PRO_CONTINUATION_SOURCE_SCOPE_MISMATCH");
  input={
    ...input,
  ...
  if(next.stage_id==="STEM_SEPARATION"){
    ...
    if(text(input.corrected_vocal_asset_id)){
      execution=await certifyProfessionalVocalProduction({ ...input, source_asset_id:sourceAssetId });
      ...
      const state={ manifest:next.manifest, next_stage:next.stage_id || null, next_action:next.next_action || null, source_asset_id:sourceAssetId, vocal_certification:execution, updated_at:new Date().toISOString() };
      await updateMusicConversationState({ organization_id:organizationId, creative_project_id:projectId, patch:{ professional_production_state:state }, decision_summary:`Professional vocal production approved after human listening review; next stage ${next.stage_id || "COMPLETE"}.` });
  ...
  } else if(next.stage_id==="PREMASTER_LISTENING"){
    execution=await reviewProfessionalPremasterListening({ ...input, source_asset_id:sourceAssetId });
    sourceAsset=await CreativeAssetsRuntime.get(sourceAssetId);
    next=nextMusicProfessionalProductionAction({ plan:activePlan, input, evidence:evidenceFromAsset(sourceAsset) });
    const state={ manifest:next.manifest, next_stage:next.stage_id || null, next_action:next.next_action || null, source_asset_id:sourceAssetId, premaster_listening_status:execution.status, updated_at:new Date().toISOString() };
    await updateMusicConversationState({ organization_id:organizationId, creative_project_id:projectId, patch:{ professional_production_state:state }, decision_summary:`Independent pre-master listening ${execution.passed ? "passed" : "requires mix repair"}; next stage ${next.stage_id || "PREMASTER_LISTENING"}.` });
