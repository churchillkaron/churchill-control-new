...
async function scopedAsset(id, organizationId, projectId, code){
  ...
  if(!asset || text(asset.organization_id)!==organizationId || text(asset.creative_project_id)!==projectId) throw new Error(code);
  return asset;
}
...
export const CreativeMusicProfessionalFinalizationRuntime=Object.freeze({ contract:CONTRACT, master:masterProfessionalPremaster, translation:verifyProfessionalTranslation, dailies:runProfessionalDailies, tribunal:runProfessionalFinalTribunal });
