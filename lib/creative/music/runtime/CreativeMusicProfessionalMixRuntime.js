...
import { buildMusicPremasterBusDecision } from "./CreativeMusicPremasterBusEngineerRuntime.js";
...
export async function acceptProfessionalMixRender(input={}){
  ...
  await CreativeAssetsRuntime.update(sourceAssetId,{ metadata:{ ...(sourceAsset.metadata || {}), professional_mix_passed:true, professional_mix_asset_id:mixAssetId, professional_mix_evidence:evidence, professional_premaster_qc_passed:false, professional_premaster_listening_passed:false, professional_premaster_listening:null } });
  return evidence;
}
...
export async function certifyProfessionalPremaster(input={}){
  ...
  const currentMaster=(project.metadata?.[MULTITRACK_KEY]?.buses||[]).find(b=>b.id==="bus-master");
  const busDecision=buildMusicPremasterBusDecision(signalReview,currentMaster?.processing||{});
  const qc={ contract:"AVANTIQO_MUSIC_PROFESSIONAL_PREMASTER_QC_V3", passed, checks, failures, mix_asset_id:mixAssetId, validated_at:new Date().toISOString(), signal_review:signalReview, repair_targets:signalReview.repair_targets, bus_decision:busDecision, mastering_allowed:passed, measured_from_saved_premaster:true };
...
export async function applyProfessionalPremasterRepair(input={}){
  ...
  const master=(next.buses||[]).find(b=>b.id==="bus-master");
  const busDecision=qc?.bus_decision;
  if(master && busDecision?.status==="TECHNICAL_BUS_REPAIR_READY"){master.processing=structuredClone(busDecision.processing||master.processing||{});for(const change of busDecision.changes||[])applied.push({code:change.code,target:"MASTER_BUS",...change});}
  for(const repair of repairs){
    if(repair.code==="RESTORE_PREMASTER_HEADROOM"){if(master){master.gain_db=Math.max(-18,finite(master.gain_db,0)-1.5);applied.push({code:repair.code,change_db:-1.5});}}
  ...
  await CreativeAssetsRuntime.update(sourceAssetId,{metadata:{...(sourceAsset.metadata||{}),professional_mix_passed:false,professional_premaster_qc_passed:false,professional_premaster_listening_passed:false,professional_premaster_listening:null,professional_mix_asset_id:null,professional_premaster_qc:{...qc,invalidated_by_repair:true,invalidated_at:new Date().toISOString()},professional_premaster_repair:{contract:"AVANTIQO_MUSIC_PREMASTER_REPAIR_V2",source_revision:previousRevision,new_revision:next.revision,applied,bus_decision:busDecision||null,requires_new_mix_render:true,artistic_listening_review_required:true}}});
...
export async function applyProfessionalPremasterListeningRepair(input={}){
  const organizationId=text(input.organization_id), projectId=text(input.creative_project_id), sourceAssetId=text(input.source_asset_id);
  if(!organizationId || !projectId || !sourceAssetId) throw new Error("CREATIVE_MUSIC_PRO_LISTENING_REPAIR_CONTEXT_REQUIRED");
  const [project,sourceAsset]=await Promise.all([CreativeProjectRepository.getById(projectId),CreativeAssetsRuntime.get(sourceAssetId)]);
  if(!project || text(project.organization_id)!==organizationId || !sourceAsset || text(sourceAsset.organization_id)!==organizationId || assetProjectId(sourceAsset)!==projectId) throw new Error("CREATIVE_MUSIC_PRO_LISTENING_REPAIR_SCOPE_MISMATCH");
  const listening=sourceAsset.metadata?.professional_premaster_listening, plan=listening?.repair_plan;
  if(listening?.passed===true || plan?.status!=="SAFE_REPAIR_PLAN_READY" || !Array.isArray(plan.actions) || !plan.actions.length) throw new Error("CREATIVE_MUSIC_PRO_LISTENING_REPAIR_NOT_AVAILABLE");
  if(text(plan.mix_asset_id)!==text(sourceAsset.metadata?.professional_mix_asset_id)) throw new Error("CREATIVE_MUSIC_PRO_LISTENING_REPAIR_STALE");
  const current=ensureMusicEngineeringBuses(structuredClone(project.metadata?.[MULTITRACK_KEY]||{})); validateMusicMultitrackProject(current); validateMusicMixerRouting(current);
  const next=structuredClone(current), applied=[];
  const role=(track={})=>{const hay=`${text(track.type)} ${text(track.name)}`.toLowerCase();if(/bass|sub/.test(hay))return"BASS";if(/kick|drum|snare|perc/.test(hay))return"DRUMS";if(/vocal|lead|vox|singer/.test(hay))return"VOCAL";if(/guitar|keys|piano|synth|pad|backing|stem|music|instrument/.test(hay))return"SUPPORT";return"OTHER";};
  const adjustSend=(track,busId,delta)=>{track.sends=Array.isArray(track.sends)?track.sends:[];const send=track.sends.find(row=>row.bus_id===busId);if(!send)return false;send.level_db=Math.max(-60,Math.min(6,finite(send.level_db,-18)+delta));return true;};
  for(const action of plan.actions){
    if(action.code==="LISTENING_REDUCE_HARSHNESS"){for(const track of next.tracks||[]){if(!["VOCAL","SUPPORT"].includes(role(track)))continue;track.channel_strip=track.channel_strip||{};track.channel_strip.presence_db=Math.max(-12,finite(track.channel_strip.presence_db,0)+action.change_db);}applied.push({code:action.code,target:action.target,change_db:action.change_db});}
    else if(action.code==="LISTENING_TIGHTEN_LOW_END"){for(const track of next.tracks||[]){if(role(track)!=="BASS")continue;track.channel_strip=track.channel_strip||{};track.channel_strip.low_shelf_db=Math.max(-12,Math.min(12,finite(track.channel_strip.low_shelf_db,0)+action.low_shelf_db));track.gain_db=Math.max(-60,Math.min(12,finite(track.gain_db,0)+action.gain_db));}applied.push({code:action.code,target:action.target});}
    else if(action.code==="LISTENING_RESTORE_DYNAMICS"){for(const track of next.tracks||[]){const c=track.channel_strip?.compressor;if(c?.enabled!==true)continue;c.ratio=Math.max(1,Number((finite(c.ratio,2.5)*action.ratio_scale).toFixed(2)));c.threshold_db=Math.min(0,finite(c.threshold_db,-18)+action.threshold_delta_db);}applied.push({code:action.code,target:action.target});}
    else if(["LISTENING_VOCAL_FORWARD","LISTENING_VOCAL_BACK"].includes(action.code)){for(const track of next.tracks||[]){if(role(track)!=="VOCAL")continue;track.gain_db=Math.max(-60,Math.min(12,finite(track.gain_db,0)+action.gain_db));adjustSend(track,"bus-reverb",action.reverb_delta_db);}applied.push({code:action.code,target:action.target,gain_db:action.gain_db,reverb_delta_db:action.reverb_delta_db});}
    else if(["LISTENING_MORE_DEPTH","LISTENING_LESS_DEPTH"].includes(action.code)){let changed=0;for(const track of next.tracks||[]){if(!["VOCAL","SUPPORT"].includes(role(track)))continue;if(adjustSend(track,"bus-reverb",action.reverb_delta_db))changed++;}if(changed)applied.push({code:action.code,target:action.target,reverb_delta_db:action.reverb_delta_db,track_count:changed});}
  }
  if(!applied.length) throw new Error("CREATIVE_MUSIC_PRO_LISTENING_REPAIR_NO_SAFE_CHANGES");
  const previousRevision=Math.max(0,Math.round(finite(current.revision,0))); next.revision=previousRevision+1;
  const snapshot={contract:"AVANTIQO_MUSIC_PREMASTER_LISTENING_REPAIR_SNAPSHOT_V1",created_at:new Date().toISOString(),session:current,listening,plan,immutable:true,reversible:true};
  const history=[...(Array.isArray(project.metadata?.music_premaster_listening_repair_history)?project.metadata.music_premaster_listening_repair_history:[]),snapshot].slice(-12);
  await CreativeProjectRepository.update(projectId,{metadata:{...(project.metadata||{}),[MULTITRACK_KEY]:next,music_premaster_listening_repair_history:history,music_multitrack_updated_at:new Date().toISOString()}});
  await CreativeAssetsRuntime.update(sourceAssetId,{metadata:{...(sourceAsset.metadata||{}),professional_mix_passed:false,professional_mix_asset_id:null,professional_premaster_qc_passed:false,professional_premaster_qc:null,professional_premaster_listening_passed:false,professional_premaster_listening:{...listening,invalidated_by_repair:true,invalidated_at:new Date().toISOString()},professional_premaster_listening_repair:{contract:"AVANTIQO_MUSIC_PREMASTER_LISTENING_REPAIR_V1",source_revision:previousRevision,new_revision:next.revision,applied,requires_new_mix_render:true,requires_fresh_technical_qc:true,requires_fresh_listening:true,artistic_listening_review_required:true}}});
  return {contract:CONTRACT,status:"LISTENING_REPAIR_APPLIED",source_revision:previousRevision,new_revision:next.revision,applied,requires_new_mix_render:true,old_premaster_stale:true,reversible:true};
}

export const CreativeMusicProfessionalMixRuntime=Object.freeze({ contract:CONTRACT, prepare:prepareProfessionalMix, acceptMixRender:acceptProfessionalMixRender, certifyPremaster:certifyProfessionalPremaster, repairPremaster:applyProfessionalPremasterRepair, repairPremasterListening:applyProfessionalPremasterListeningRepair });
