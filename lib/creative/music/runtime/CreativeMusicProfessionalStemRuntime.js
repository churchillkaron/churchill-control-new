...
function pendingStemState(metadata={}){ const state=object(metadata.professional_stem_pending); return text(state.usage_id) ? state : null; }

async function settlePendingStem({ organizationId, sourceAsset, pending }) {
  const usage=await UsageRuntime.get(text(pending.usage_id));
  if(!usage || text(usage.organization_id)!==organizationId || text(usage.capability)!=="ai.audio.stems") throw new Error("CREATIVE_MUSIC_PRO_STEMS_PENDING_USAGE_INVALID");
  const providerJobId=text(usage.provider_request_id || usage.metadata?.provider_request_id || pending.provider_job_id);
  if(!providerJobId) throw new Error("CREATIVE_MUSIC_PRO_STEMS_PENDING_PROVIDER_JOB_REQUIRED");
  const result=await settlePendingService({ organization_id:organizationId, provider:text(usage.provider), provider_job_id:providerJobId, usage_id:usage.id, pricing:{},
    quantity:finite(usage.quantity,null), unit:text(usage.unit)||null, credential_id:null, started_at:text(usage.execution_started_at || usage.created_at)||null,
    metadata:{ module:"CREATIVE", operation:"AVANTIQO_MUSIC_PROFESSIONAL_STEMS_SETTLE", creative_project_id:assetProjectId(sourceAsset), professional_release:true } });
  return { result, usage, providerJobId };
}
...
function stemTrackType(key){ return key === "vocals" ? "vocal" : key === "other" ? "instrument" : key; }
...
export async function executeProfessionalStemSeparation(input={}) {
  ...
  if(sourceAssetId){
    ...
    if(!sourceAsset || text(sourceAsset.organization_id)!==organizationId || assetProjectId(sourceAsset)!==projectId) throw new Error("CREATIVE_MUSIC_PRO_STEMS_SOURCE_ASSET_SCOPE_MISMATCH");
    if(sourceAsset.metadata?.professional_stems_ready===true && Array.isArray(sourceAsset.metadata?.professional_stems) && sourceAsset.metadata.professional_stems.length>=2){
      return { contract:CONTRACT, success:true, pending:false, failed:false, usage_id:text(sourceAsset.metadata?.professional_stem_usage_id)||null, provider_status:"completed", assets:[], multitrack:null,
        idempotent_existing_completion:true, professional_evidence:{ stems_ready:true, stems:sourceAsset.metadata.professional_stems, source_asset_id:sourceAssetId } };
    }
  }
  let result=null, usage=null, providerJobId=null;
  const pending=sourceAsset ? pendingStemState(sourceAsset.metadata) : null;
  if(pending){
    const settled=await settlePendingStem({ organizationId, sourceAsset, pending });
    result=settled.result; usage=settled.usage; providerJobId=settled.providerJobId;
  } else {
    ...
    result=await executeService({ organization_id:organizationId, bill_to_organization_id:organizationId, entity_id:text(input.entity_id)||null,
    ...
    usage=result?.usage?.id ? await UsageRuntime.get(result.usage.id) : result?.usage || null;
    providerJobId=text(result?.provider_job_id || usage?.provider_request_id || usage?.metadata?.provider_request_id) || null;
  }
  if(result?.pending===true){
    const pendingState={ contract:"AVANTIQO_MUSIC_PROFESSIONAL_STEM_PENDING_V1", usage_id:usage?.id || result?.usage?.id || null, provider_job_id:providerJobId,
      provider_status:result?.provider_status || "pending", started_at:pending?.started_at || new Date().toISOString(), last_checked_at:new Date().toISOString() };
    if(sourceAssetId) await CreativeAssetsRuntime.update(sourceAssetId,{ metadata:{ ...(sourceAsset.metadata || {}), professional_stem_pending:pendingState, professional_stems_ready:false } });
    return { contract:CONTRACT, success:true, pending:true, failed:false, usage_id:pendingState.usage_id, provider_status:pendingState.provider_status, assets:[], multitrack:null, resumed_existing_pending:Boolean(pending) };
  }
  if(result?.failed===true){
    if(sourceAssetId) await CreativeAssetsRuntime.update(sourceAssetId,{ metadata:{ ...(sourceAsset.metadata || {}), professional_stem_pending:null, professional_stem_last_failure:{ usage_id:usage?.id || pending?.usage_id || null, provider_status:result?.provider_status || "failed", failed_at:new Date().toISOString() } } });
    return { contract:CONTRACT, success:false, pending:false, failed:true, usage_id:usage?.id || pending?.usage_id || null, provider_status:result?.provider_status || "failed", assets:[], multitrack:null, resumed_existing_pending:Boolean(pending) };
  }
  usage=usage || (result?.usage?.id ? await UsageRuntime.get(result.usage.id) : result?.usage || null);
  ...
  if(sourceAssetId){
    const latest=await CreativeAssetsRuntime.get(sourceAssetId);
    await CreativeAssetsRuntime.update(sourceAssetId,{ metadata:{ ...(latest?.metadata || sourceAsset?.metadata || {}), professional_stem_pending:null, professional_stems_ready:stems.length>=2, professional_stems:stems.map(a=>a.id), professional_stem_usage_id:usage?.id || pending?.usage_id || null, professional_stems_completed_at:new Date().toISOString() } });
  }
  return { contract:CONTRACT, success:true, pending:false, failed:false, usage_id:usage?.id || pending?.usage_id || null, provider_status:result?.provider_status || "completed", assets:stems.map(publicAsset), multitrack,
    resumed_existing_pending:Boolean(pending), professional_evidence:{ stems_ready:stems.length>=2, stems:stems.map(a=>a.id), source_asset_id:sourceAssetId || null } };
}
