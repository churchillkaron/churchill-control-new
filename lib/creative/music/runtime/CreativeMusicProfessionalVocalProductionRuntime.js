...
export async function prepareProfessionalVocalProduction(input={}){
  ...
  if(!project || text(project.organization_id)!==organizationId || !sourceAsset || text(sourceAsset.organization_id)!==organizationId || text(sourceAsset.creative_project_id)!==projectId) throw new Error("CREATIVE_MUSIC_PRO_VOCAL_SCOPE_MISMATCH");
...
export async function certifyProfessionalVocalProduction(input={}){
  ...
  if(!sourceAsset || !corrected || text(sourceAsset.organization_id)!==organizationId || text(corrected.organization_id)!==organizationId || text(sourceAsset.creative_project_id)!==projectId || text(corrected.creative_project_id)!==projectId) throw new Error("CREATIVE_MUSIC_PRO_VOCAL_CERT_SCOPE_MISMATCH");
