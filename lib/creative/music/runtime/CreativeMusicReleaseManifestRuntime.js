...
import { evaluateMusicMasterSet, musicSourceGenerationId, musicMasterSetId } from "./CreativeMusicMasterSetLineageRuntime.js";
...
export function buildMusicReleaseManifest({
  ...
  const masterSetLineage = finishing.master_set_lineage || evaluateMusicMasterSet({ sourceAsset: source_asset, masterVariants: finishing.master_variants });
  if (release_ready === true && masterSetLineage.release_ready !== true) throw new Error("CREATIVE_MUSIC_RELEASE_MASTER_SET_STALE_OR_MIXED");
  const core = {
    ...
    source: {
      ...
      music_generation_id: musicSourceGenerationId(source_asset),
      music_master_set_id: musicMasterSetId(source_asset),
    },
    mastering: {
    ...
    master_set: {
      contract: text(masterSetLineage.contract) || null,
      generation_id: text(masterSetLineage.expected_generation_id) || null,
      master_set_id: text(masterSetLineage.expected_master_set_id) || null,
      lineage_current: masterSetLineage.lineage_current === true,
      mixed_generation: masterSetLineage.mixed_generation === true,
      destination_and_translation_qc_current: masterSetLineage.destination_and_translation_qc_current === true,
      blockers: list(masterSetLineage.blockers).map((value) => text(value)).filter(Boolean),
    },
    masters,
    quality: {
      dailies_passed: dailies?.report?.passed === true,
      tribunal_release_ready: tribunal?.release_ready === true,
      release_ready: release_ready === true && masterSetLineage.release_ready === true,
    },
    governance: {
      ...
      same_master_set_generation_required: true,
    },
  };
