...
import { fingerprintMusicMidiBounce } from "./CreativeMusicMidiBounceFingerprintRuntime";
import { ensureMusicEngineeringBuses, validateMusicMixerRouting } from "./CreativeMusicMixerRoutingRuntime";
import { normalizeMusicChannelStrip, validateMusicMultitrackProject } from "./CreativeMusicMultitrackRuntime";
import { createMusicSpatialAudioConfig, deriveMusicStereoDownmixMatrix } from "./CreativeMusicSpatialAudioRuntime.js";
import { normalizeMusicSourceCleanup, validateMusicSourceCleanup } from "./CreativeMusicSourceCleanupRuntime";
import { resolveProfessionalAudioDeliveryProfile, validateProfessionalAudioDeliveryProfile } from "./CreativeProfessionalAudioDeliveryProfileRuntime.js";
import { planProfessionalAudioLongFormDelivery } from "./CreativeProfessionalAudioLongFormRuntime.js";

const CONTRACT = "AVANTIQO_MUSIC_RELEASE_RENDER_PLAN_V6";
const RENDERER = "AVANTIQO_MUSIC_OFFLINE_AUDIO_RENDERER_V1";
const SPATIAL_RENDERER = "AVANTIQO_MUSIC_OFFLINE_SPATIAL_RENDERER_V1";
...
const MASTERING_PROFILES = Object.freeze({
  ...
  archival: { target_lufs: -18, true_peak_dbtp: -3, loudness_range_lu: 14, sample_rate: 96000 },
});
...
function midiReleaseBlockers(session = {}) {
  ...
  for (const midiTrack of audibleMidiTracks(session)) {
    ...
    const currentCoreFingerprint = fingerprintMusicMidiBounce({ session, track: midiTrack, sampler: null });
    const structurallyValid = link?.contract === "AVANTIQO_MUSIC_MIDI_RELEASE_BOUNCE_LINK_V2"
      && link?.stale !== true
    ...
    if (!structurallyValid) {
      blockers.push(blocker(
        link?.stale === true ? "MIDI_RELEASE_BOUNCE_STALE" : "MIDI_TRACKS_REQUIRE_AUDIO_BOUNCE_FOR_RELEASE",
        link?.stale === true
          ? "This MIDI/instrument bounce was invalidated by a source or sampler change. Re-bounce it before release rendering."
          : "Bounce every audible MIDI/instrument track to a linked 24-bit audio track before release rendering.",
        { midi_track_id: midiTrack.id, midi_track_name: midiTrack.name || "MIDI Track", stale_reason: link?.stale_reason || null },
      ));
      continue;
    }
    if (text(link.source_midi_core_fingerprint) !== currentCoreFingerprint || text(audioTrack.source_midi_core_fingerprint) !== currentCoreFingerprint) {
      blockers.push(blocker(
        "MIDI_RELEASE_BOUNCE_STALE",
        "This MIDI/instrument performance changed after its audio bounce. Re-bounce it before release rendering.",
        { midi_track_id: midiTrack.id, midi_track_name: midiTrack.name || "MIDI Track", stale_reason: "MIDI_OR_TEMPO_STATE_CHANGED" },
      ));
    }
  }
  return blockers;
...
function trackPlan(track = {}) {
  ...
  return {
    ...
    channel_strip: normalizeMusicChannelStrip(track.channel_strip || {}), spatial: clone(track.spatial || {}), inserts: clone(track.inserts || []), sends: clone(track.sends || []),
    ...
    source_midi_core_fingerprint: text(track.source_midi_core_fingerprint) || null,
    source_midi_fingerprint: text(track.source_midi_fingerprint) || null,
...
export function buildMusicReleaseRenderPlan(sessionInput = {}, input = {}) {
  ...
  const spatial = createMusicSpatialAudioConfig(session.spatial_audio || { layout_id: "stereo" });
  ...
  const deliveryProfile = resolveProfessionalAudioDeliveryProfile(input.delivery_profile || input.delivery_profile_id || "music_release");
  const deliveryProfileValidation = validateProfessionalAudioDeliveryProfile({ session, profile: deliveryProfile, mastering: input.mastering || input });
  const blockers = releaseBlockers(session, tracks);
  for (const code of deliveryProfileValidation.failures) blockers.push(blocker(code, `Delivery profile ${deliveryProfile.label} is not satisfied.`, { delivery_profile_id: deliveryProfile.id }));
  const songEnd = Math.max(0, ...tracks.flatMap((track) => track.clips.map((clip) => clip.start_seconds + clip.duration_seconds)));
  const longForm = planProfessionalAudioLongFormDelivery({ duration_seconds: songEnd, sample_rate: Math.round(finite(session.sample_rate,48000)), channels: spatial.channel_count, bit_depth: 24 });
  if(longForm.server_render_required) blockers.push(blocker("LONG_FORM_SERVER_RENDER_REQUIRED","This PCM render is too large for the single-buffer browser release path. Use the governed server/segmented long-form renderer.",{estimated_pcm_bytes:longForm.estimated_pcm_bytes,rf64_required:longForm.rf64_required}));
  ...
  return {
    contract: CONTRACT, delivery_profile: deliveryProfile, delivery_profile_validation: deliveryProfileValidation, renderer: spatial.surround_enabled ? SPATIAL_RENDERER : RENDERER, project_id: session.id,
    project_revision: Math.max(0, Math.round(finite(session.revision, 0))), title: text(session.title || "Music Project"),
    sample_rate: Math.round(finite(session.sample_rate, 48000)), bit_depth: 24, channels: spatial.channel_count, channel_layout: spatial.layout_id, speaker_order: [...spatial.speaker_order], ffmpeg_channel_layout: spatial.ffmpeg_channel_layout, duration_seconds: songEnd,
    long_form_delivery: longForm,
    precision: { internal_processing_format: "float32", internal_processing_bits: 32, delivery_bit_depth: 24, delivery_dither_required: true, source_render_sample_rate: Math.round(finite(session.sample_rate, 48000)), native_high_resolution: Math.round(finite(session.sample_rate, 48000)) >= 88200 },
    bpm: finite(session.bpm, 96), time_signature: session.time_signature || "4/4", source_asset_ids: sourceAssetIds,
    spatial_audio: {
      ...spatial,
      stereo_downmix_qc_matrix: deriveMusicStereoDownmixMatrix(spatial.layout_id),
      exact_multichannel_renderer_required: spatial.surround_enabled,
      stereo_renderer_for_surround_forbidden: spatial.surround_enabled,
    },
    midi_release_policy: {
      audible_midi_track_count: audibleMidiTracks(session).length,
      explicit_bounce_required: true,
      source_fingerprint_required: true,
      stale_bounce_blocked: true,
      sampler_mutation_invalidates_bounce: true,
    ...
    exports: {
      ...
      release_wav: { enabled: true, format: "wav", codec: "pcm_s24le", bit_depth: 24, sample_rate: Math.round(finite(session.sample_rate, 48000)), channels: spatial.channel_count, channel_layout: spatial.layout_id, dither_required: true },
      surround_premaster_wav: { enabled: spatial.surround_enabled, format: "wav", codec: "pcm_s24le", bit_depth: 24, sample_rate: Math.round(finite(session.sample_rate, 48000)), channels: spatial.channel_count, channel_layout: spatial.layout_id, speaker_order: [...spatial.speaker_order], exact_spatial_render_required: true },
      stereo_downmix_qc: { enabled: spatial.surround_enabled, matrix: deriveMusicStereoDownmixMatrix(spatial.layout_id), delivery_master: false },
      archival_wav: { enabled: text(input.profile || input.mastering_profile).toLowerCase() === "archival", format: "wav", codec: "pcm_s24le", bit_depth: 24, sample_rate: 96000, native_high_resolution: Math.round(finite(session.sample_rate, 48000)) >= 88200, upsampled_if_enabled: Math.round(finite(session.sample_rate, 48000)) < 88200 },
    ...
    readiness: {
      ...
      midi_source_fingerprint_required: audibleMidiTracks(session).length > 0,
      ...
      release_limiter_via_canonical_audio_finish: !spatial.surround_enabled,
      multichannel_renderer_required: spatial.surround_enabled,
      stereo_offline_renderer_allowed: !spatial.surround_enabled,
      surround_downmix_qc_required: spatial.surround_enabled,
      per_channel_true_peak_qc_required: spatial.surround_enabled,
      lfe_bandlimit_qc_required: spatial.lfe.enabled,
    },
    safety: {
      ...
      stale_midi_bounce_blocks_release: true,
      silent_midi_omission_forbidden: true,
      release_finish_stage_separate: true, surround_never_silently_folded_to_stereo: true, long_form_browser_buffer_guard: true, rf64_auto_on_server_finish: true, dolby_branding_requires_licensed_toolchain: true, provider_job_submitted: false, endpoint_mutation_performed: false,
    },
  };
}

export const CreativeMusicReleaseRenderPlanRuntime = {
  contract: CONTRACT, delivery_profile: deliveryProfile, delivery_profile_validation: deliveryProfileValidation, renderer: RENDERER, masteringProfiles: MASTERING_PROFILES,
