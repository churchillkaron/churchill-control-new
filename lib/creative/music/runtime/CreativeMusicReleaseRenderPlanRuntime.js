...
const CONTRACT = "AVANTIQO_MUSIC_RELEASE_RENDER_PLAN_V1";
const RENDERER = "AVANTIQO_MUSIC_SERVER_RENDERER_V1";
...
function selectedTakePlaybackClips(track = {}) {
  ...
  return [{
    id: `release-take-${selected.id}`,
    source_asset_id: selected.source_asset_id,
    start_seconds: Math.max(0, finite(selected.start_seconds, 0)),
    duration_seconds: Math.max(0, finite(selected.duration_seconds, 0)),
...
export function resolveMusicReleaseTrackClips(track = {}) {
  return (compPlaybackClips(track) || selectedTakePlaybackClips(track))
    ...
    .map((clip) => ({
      ...
      start_seconds: Math.max(0, finite(clip.start_seconds, 0)),
      ...
      warp_mode: text(clip.warp_mode || "off").toLowerCase(),
      comp_region: clip.comp_region === true,
      selected_take: clip.selected_take === true,
      destructive_edit: false,
    }));
}
...
function releaseBlockers(session, tracks) {
  ...
  for (const track of tracks) {
    for (const clip of track.clips) {
      if (clip.loop_enabled) blockers.push(blocker(
        "LOOP_CLIP_RELEASE_RENDER_PENDING",
        "Loop-expanded clips are not yet server-render certified.",
        { track_id: track.id, clip_id: clip.id },
      ));
      if (clip.warp_mode && clip.warp_mode !== "off") blockers.push(blocker(
        "WARP_RELEASE_RENDER_PENDING",
        "Warp/time-stretch must be server-render certified before release.",
        { track_id: track.id, clip_id: clip.id, warp_mode: clip.warp_mode },
      ));
    }
    for (const send of track.sends || []) {
      if (send.enabled !== false) blockers.push(blocker(
        "AUX_SEND_RELEASE_RENDER_PENDING",
        "Audible aux sends must be reproduced by the server renderer before release.",
        { track_id: track.id, send_id: send.id, bus_id: send.bus_id },
      ));
    }
  }
  for (const lane of session.automation_lanes || []) {
    if (lane.enabled !== false && (lane.points || []).length) blockers.push(blocker(
      "AUTOMATION_RELEASE_RENDER_PENDING",
      "Mixer automation must be reproduced by the server renderer before release.",
      { lane_id: lane.id, target_type: lane.target_type, target_id: lane.target_id, parameter: lane.parameter },
    ));
  }
  return blockers;
}
...
function trackPlan(track = {}) {
  validateMusicComp(track);
  validateMusicInserts(track);
  validateMusicSourceCleanup(track);
  return {
    id: track.id,
    ...
    gain_db: finite(track.gain_db, 0),
    pan: finite(track.pan, 0),
    output_bus_id: text(track.output_bus_id || "bus-master"),
    source_cleanup: normalizeMusicSourceCleanup(track.source_cleanup || {}),
    channel_strip: normalizeMusicChannelStrip(track.channel_strip || {}),
    inserts: clone(track.inserts || []),
    sends: clone(track.sends || []),
    clips: resolveMusicReleaseTrackClips(track),
    take_count: (track.takes || []).length,
    comp_active: Boolean(track.comp?.regions?.length),
...
export function buildMusicReleaseRenderPlan(sessionInput = {}, input = {}) {
  ...
  validateMusicGroupProcessing(session);
  validateMusicMasterProcessing(session);
  validateMusicAutomation(session);
  ...
  const groups = (session.buses || []).filter((bus) => bus.type === "group").map((bus) => ({
    id: bus.id,
    name: bus.name || "Group",
    output_bus_id: text(bus.output_bus_id || "bus-master"),
    gain_db: finite(bus.gain_db, 0),
    pan: finite(bus.pan, 0),
    mute: bus.mute === true,
    processing: normalizeMusicGroupProcessing(bus.processing || {}),
    destructive_processing_allowed: false,
  }));
  ...
  const vocalTrackIds = tracks.filter((track) => ["vocal"].includes(track.type) && track.mute !== true).map((track) => track.id);
  ...
  return {
    ...
    project_revision: Math.max(0, Math.round(finite(session.revision, 0))),
    title: text(session.title || "Music Project"),
    sample_rate: Math.round(finite(session.sample_rate, 48000)),
    bit_depth: 24,
    channels: 2,
    duration_seconds: songEnd,
    bpm: finite(session.bpm, 96),
    time_signature: session.time_signature || "4/4",
    source_asset_ids: sourceAssetIds,
    tracks,
    groups,
    group_render_order: groupRenderOrder(groups).map((group) => group.id),
    master: {
      id: "bus-master",
      gain_db: finite(masterBus.gain_db, 0),
      mute: masterBus.mute === true,
      processing: masterProcessing,
      mastering: masteringProfile(input.mastering || input, masterProcessing),
    },
    exports: {
      mix_pre_master: { enabled: true, format: "wav", codec: "pcm_f32le", stage: "post-master-processing-pre-release-limiter" },
      ...
      group_stems: { enabled: input.group_stems !== false, group_ids: groups.filter((group) => group.mute !== true).map((group) => group.id), stage: "post-group-processing" },
      instrumental: { enabled: input.instrumental === true, track_ids: nonVocalTrackIds },
      acapella: { enabled: input.acapella === true, track_ids: vocalTrackIds },
    },
    readiness: {
      release_render_ready: blockers.length === 0 && sourceAssetIds.length > 0 && songEnd > 0,
      blocker_count: blockers.length,
      blockers,
      source_assets_required: sourceAssetIds.length,
      static_pan_supported: true,
      clip_reverse_supported: true,
      track_cleanup_supported: true,
      track_eq_compression_supported: true,
      engineering_inserts_supported: true,
      nested_groups_supported: true,
      group_eq_compression_supported: true,
      master_eq_compression_supported: true,
      aux_send_render_supported: false,
      mixer_automation_render_supported: false,
      loop_clip_render_supported: false,
      warp_render_supported: false,
      release_limiter_via_canonical_audio_finish: true,
    },
    safety: {
      original_assets_immutable: true,
      destructive_processing: false,
      render_creates_new_assets_only: true,
      browser_preview_is_not_release_master: true,
      release_blocked_on_unsupported_mix_state: true,
