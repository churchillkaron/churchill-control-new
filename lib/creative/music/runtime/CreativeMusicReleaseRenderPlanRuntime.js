...
import { ensureMusicSamplerProject } from "./CreativeMusicSamplerRuntime";
...
function samplerForMidiTrack(input = {}, midiTrack = {}) {
  const kind = text(midiTrack.instrument?.kind).toLowerCase();
  if (!["drum_machine", "sampler", "drum_rack"].includes(kind)) return null;
  return ensureMusicSamplerProject(input.sampler || {});
}

function midiReleaseBlockers(session = {}, input = {}) {
  const blockers = [];
  ...
  for (const midiTrack of audibleMidiTracks(session)) {
    ...
    const currentFingerprint = fingerprintMusicMidiBounce({ session, track: midiTrack, sampler: samplerForMidiTrack(input, midiTrack) });
    ...
    if (text(link.source_midi_fingerprint) !== currentFingerprint || text(audioTrack.source_midi_fingerprint) !== currentFingerprint) {
      blockers.push(blocker(
...
function releaseBlockers(session, tracks, input = {}) {
  const blockers = [...midiReleaseBlockers(session, input)];
...
export function buildMusicReleaseRenderPlan(sessionInput = {}, input = {}) {
  ...
  const blockers = releaseBlockers(session, tracks, input);
