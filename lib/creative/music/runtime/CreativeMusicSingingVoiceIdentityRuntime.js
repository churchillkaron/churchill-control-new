...
export const MUSIC_SINGING_VOICE_FOUNDATION_MODEL = "Plachta/Seed-VC";
export const MUSIC_SINGING_VOICE_MODEL_PROFILE = "SEED_VC_V1_SVC_44K_FT_EMA_V2_ZERO_SHOT_RESEARCH_V1";
...
export async function buildSingingVoiceIdentityExecutionInput({ organization_id, entity_id = null, voice_library_profile_id, source_audio, source_is_isolated_vocal = false, source_duration_seconds = null, reference_is_isolated_vocal = false, reference_duration_seconds = null, reference_snr_db = null, reference_accompaniment_ratio = null, reference_reverb_ratio = null, reference_clipping_ratio = null } = {}) {
  if (!text(source_audio)) throw new Error("MUSIC_SINGING_VOICE_SOURCE_AUDIO_REQUIRED");
  const plan = await buildSingingVoiceIdentityPlan({ organization_id, entity_id, voice_library_profile_id, source_audio, source_is_isolated_vocal, source_duration_seconds, preserve_source_pitch: true, consent_verified: true, model_license_verified: false, runtime_certified: false, reference_is_isolated_vocal, reference_duration_seconds, reference_snr_db, reference_accompaniment_ratio, reference_reverb_ratio, reference_clipping_ratio });
  const reference = await resolveVoiceReferenceForExecution({ organizationId: organization_id, entityId: entity_id, profileId: voice_library_profile_id, purpose: "SINGING" });
  return { capability: MUSIC_SINGING_VOICE_CONVERSION_CAPABILITY, voice_library_service_id: reference?.organization_service_id || null, source_audio, voice_reference: reference?.voice_reference ? { ...reference.voice_reference, audio_base64: undefined } : null, requirements: { consent_confirmed: true, use_scope: "SINGING", source_isolated_vocal: source_is_isolated_vocal === true, reference_isolated_vocal: reference_is_isolated_vocal === true }, provider_parameters: { local_only: true, research_candidate: true, voice_library_profile_id, model_license: "GPL-3.0", checkpoint_sha256: "42aef93ffe65857c840d270252fa040f7ba04514945ec460f3ac1ac2a96de684", gpl_compliance_review_required: true }, metadata: { singing_voice_plan_contract: plan.contract, research_plan_blockers: plan.blockers, production_certified: false, production_routing_allowed: false, human_identity_review_required: true } };
}
...
export const CreativeMusicSingingVoiceIdentityRuntime = Object.freeze({
  ...
  executionInput: buildSingingVoiceIdentityExecutionInput,
});
