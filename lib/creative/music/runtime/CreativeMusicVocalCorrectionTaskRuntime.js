...
export const MUSIC_VOCAL_CORRECTION_EXECUTION_CONTRACT =
  "AVANTIQO_OWNED_AUDIO_GOVERNED_V2";
export const MUSIC_VOCAL_CORRECTION_EXECUTION_LANE =
...
export function musicVocalCorrectionEligibility({ plan, restoration } = {}) {
  ...
  return {
    ...
    blocker: {
      ...
      execution_contract: MUSIC_VOCAL_CORRECTION_EXECUTION_CONTRACT,
      execution_lane: MUSIC_VOCAL_CORRECTION_EXECUTION_LANE,
    },
  };
}
...
export async function createMusicVocalCorrectionRequestTask({
  ...
  const task = await ProductionTaskRuntime.create({
    ...
    description: "Durable request for certified isolated-vocal pitch and whole-phrase timing correction. Execution is permitted only through the governed owned vocal-correction capability lane. Local Node 01 is preferred when healthy and authorized; external owned compute remains a fallback only when policy allows it.",
    ...
    metadata: {
      ...
      execution_contract: MUSIC_VOCAL_CORRECTION_EXECUTION_CONTRACT,
      execution_lane: MUSIC_VOCAL_CORRECTION_EXECUTION_LANE,
      owned_governed_execution_only: true,
      local_node_preferred: true,
      external_owned_fallback_policy_controlled: true,
