...
import { buildMusicVocalPerformanceEvidence } from "./CreativeMusicVocalPerformanceAnalysisRuntime.js";

const CONTRACT = "AVANTIQO_MUSIC_VOCAL_ENGINEERING_LOCAL_V1";

export const MUSIC_VOCAL_ENGINEERING_EVIDENCE_CONTRACT = "AVANTIQO_MUSIC_VOCAL_ENGINEERING_EVIDENCE_V1";
const BUCKET = "creative-assets";
const MAX_SOURCE_BYTES = 1_073_741_824;
const ANALYSIS_WINDOW_SECONDS = 120;
const PERFORMANCE_RATE = 16000;
const OWNED_EXECUTION_CONTRACT = "AVANTIQO_OWNED_AUDIO_GOVERNED_V2";
...
function engineeringEvidenceSummary({ volume = {}, spectrum = {} } = {}) {
  ...
  return {
    source_mean_db: finite(volume.mean_db, null),
    source_peak_dbfs: finite(volume.peak_dbfs, null),
    body_mean_db: finite(spectrum.body?.mean_db, null),
    presence_mean_db: finite(spectrum.presence?.mean_db, null),
    sibilance_mean_db: finite(spectrum.sibilance?.mean_db, null),
    rumble_mean_db: finite(spectrum.rumble?.mean_db, null),
    body_vs_presence_db: bodyVsPresence,
    presence_vs_body_db: presenceVsBody,
    sibilance_vs_presence_db: sibilanceVsPresence,
    rumble_vs_body_db: rumbleVsBody,
    body_dominant: bodyVsPresence !== null ? bodyVsPresence > 4 : null,
    presence_deficit: presenceVsBody !== null ? presenceVsBody < -7 : null,
    sibilance_elevated: sibilanceVsPresence !== null ? sibilanceVsPresence > -2.5 : null,
    sub_rumble_elevated: rumbleVsBody !== null ? rumbleVsBody > -7 : null,
  };
}
...
function buildAdaptiveChain({ sourceRole, filters, volume, spectrum }) {
  ...
  return {
    ...
    pitch_correction: {
      ...
      governed_execution_required: mode === "SOLO_VOCAL",
      execution_contract: mode === "SOLO_VOCAL" ? OWNED_EXECUTION_CONTRACT : null,
      execution_lane: mode === "SOLO_VOCAL" ? "ai.audio.vocal-correct" : null,
    },
    timing_correction: {
      automatic_decision_required: mode === "SOLO_VOCAL",
...
export async function analyzeMusicVocalEngineeringEvidence(input = {}) {
  ...
  if (!sourceReference) throw new Error("CREATIVE_MUSIC_VOCAL_ENGINEERING_EVIDENCE_SOURCE_REQUIRED");
  const ffmpeg = text(input.ffmpeg_path || process.env.CREATIVE_FFMPEG_PATH || "ffmpeg");
  const materialized = await materializeMedia({
    url: sourceReference,
    file_name: input.file_name || "vocal-source",
    mime_type: input.mime_type || null,
    organization_id: organizationId,
    policy: { max_bytes: MAX_SOURCE_BYTES, probe_timeout_ms: 120000 },
  });
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-music-vocal-evidence-"));
  try {
    const offset = Math.max(0, finite(input.source_offset_seconds, 0));
    const requestedDuration = finite(input.duration_seconds, null);
    const analysisPath = path.join(root, "evidence-source.wav");
    const args = ["-y"];
    if (offset > 0) args.push("-ss", String(offset));
    args.push("-i", materialized.file_path);
    if (requestedDuration && requestedDuration > 0) args.push("-t", String(Math.min(requestedDuration, 900)));
    args.push("-vn", "-ac", "1", "-ar", "48000", "-c:a", "pcm_s24le", analysisPath);
    await run(ffmpeg, args, { timeoutMs: 300000 });
    const filters = await availableFilters(ffmpeg);
    const volume = await analyseVolume(ffmpeg, analysisPath);
    const spectrum = await spectralSnapshot(ffmpeg, analysisPath, filters);
    const performancePath = path.join(root, "performance.f32le");
    await run(ffmpeg, ["-y", "-i", analysisPath, "-vn", "-ac", "1", "-ar", String(PERFORMANCE_RATE), "-f", "f32le", "-acodec", "pcm_f32le", performancePath], { timeoutMs: 300000 });
    const performance = buildMusicVocalPerformanceEvidence({ samples: pcmFloat32(await fs.readFile(performancePath)), sample_rate: PERFORMANCE_RATE });
    return {
      contract: MUSIC_VOCAL_ENGINEERING_EVIDENCE_CONTRACT,
      source_checksum: materialized.checksum || null,
      measured: engineeringEvidenceSummary({ volume, spectrum }),
...
export const CreativeMusicVocalEngineeringRuntime = Object.freeze({
  ...
  analyzeEvidence: analyzeMusicVocalEngineeringEvidence,
});
