...
const ROLE_MAP = Object.freeze({
  lead: "LEAD", lead_vocal: "LEAD", double: "DOUBLE", doubles: "DOUBLE", double_vocal: "DOUBLE",
...
function pitchSummary(analysis = {}) {
  ...
  return {
    ...
    sustained_performance: list(analysis.sustained_performance).slice(0, MAX_REGIONS).map((row) => ({
      start_seconds: rounded(row.start_seconds, 3),
      end_seconds: rounded(row.end_seconds, 3),
      note: text(row.note, 24) || null,
      pitch_stability_std_cents: rounded(row.pitch_stability_std_cents, 1),
      pitch_span_cents: rounded(row.pitch_span_cents, 1),
      vibrato_candidate: row.vibrato_candidate === true,
      vibrato_candidate_rate_hz: rounded(row.vibrato_candidate_rate_hz, 2),
      vibrato_candidate_extent_cents: rounded(row.vibrato_candidate_extent_cents, 1),
    })),
    sustained_note_count: Number(analysis.sustained_note_count || 0),
    vibrato_candidate_count: Number(analysis.vibrato_candidate_count || 0),
    vibrato_is_signal_candidate_not_artistic_intent: analysis.vibrato_is_signal_candidate_not_artistic_intent === true,
    measured: true,
  };
}
...
function engineeringSummary(evidence = {}) {
  ...
  return {
    ...
    dynamics: evidence.performance ? {
      active_dynamic_range_db: rounded(evidence.performance.active_dynamic_range_db, 2),
      median_crest_db: rounded(evidence.performance.median_crest_db, 2),
      windows: list(evidence.performance.dynamics_windows).slice(0, MAX_REGIONS),
    } : null,
    sibilant_event_candidates: list(evidence.performance?.sibilant_event_candidates).slice(0, MAX_REGIONS),
    air_noise_event_candidates: list(evidence.performance?.air_noise_event_candidates).slice(0, MAX_REGIONS),
    breath_events_confirmed: evidence.performance?.breath_events_confirmed === true,
    signal_proxy_only: evidence.performance?.signal_proxy_only === true,
    measured: true,
  };
}
...
function projectBrief(clips = []) {
  ...
  for (const clip of clips.slice(0, 6)) {
    ...
    if (clip.engineering?.dynamics?.active_dynamic_range_db !== null && clip.engineering?.dynamics?.active_dynamic_range_db !== undefined) brief.push(`Vocal clip ${clip.clip_id}: measured active dynamic range ${clip.engineering.dynamics.active_dynamic_range_db} dB with median crest ${clip.engineering.dynamics.median_crest_db} dB.`);
    if (clip.pitch?.vibrato_candidate_count > 0) brief.push(`Vocal clip ${clip.clip_id}: ${clip.pitch.vibrato_candidate_count} sustained-note vibrato signal candidate(s); artistic intent is not inferred.`);
    if (clip.engineering?.air_noise_event_candidates?.length) brief.push(`Vocal clip ${clip.clip_id}: ${clip.engineering.air_noise_event_candidates.length} air/noise signal candidate region(s); breath identity is unconfirmed.`);
  }
...
export function buildMusicVocalIntelligence({ multitrack_session = {} } = {}) {
  ...
  return Object.freeze({
    ...
    air_noise_candidates_available: clips.some((clip) => (clip.engineering?.air_noise_event_candidates?.length || 0) > 0),
    sibilant_event_candidates_available: clips.some((clip) => (clip.engineering?.sibilant_event_candidates?.length || 0) > 0),
    dynamics_envelope_available: clips.some((clip) => (clip.engineering?.dynamics?.windows?.length || 0) > 0),
    sustained_pitch_performance_available: clips.some((clip) => (clip.pitch?.sustained_performance?.length || 0) > 0),
