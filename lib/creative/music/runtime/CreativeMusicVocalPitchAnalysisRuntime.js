...
function sustainedPerformance(rawFrames, segments) {
  const sustained = [];
  for (const segment of segments.filter((entry) => entry.duration_seconds >= 0.5).slice(0, 24)) {
    const frames = rawFrames.filter((frame) => frame.voiced && frame.confidence >= 0.5 && frame.time_seconds >= segment.start_seconds && frame.time_seconds <= segment.end_seconds);
    const midi = frames.map((frame) => frame.midi_float).filter(Number.isFinite);
    if (midi.length < 8) continue;
    const center = median(midi);
    const cents = midi.map((value) => (value - center) * 100);
    const variance = cents.reduce((sum, value) => sum + value * value, 0) / cents.length;
    const p10 = percentile(cents, 0.1);
    const p90 = percentile(cents, 0.9);
    let crossings = 0;
    let previous = 0;
    for (const value of cents) {
      const sign = value > 3 ? 1 : value < -3 ? -1 : 0;
      if (sign && previous && sign !== previous) crossings += 1;
      if (sign) previous = sign;
    }
    const duration = Math.max(0.001, segment.duration_seconds);
    const rate = crossings / (2 * duration);
    const extent = Number.isFinite(p10) && Number.isFinite(p90) ? (p90 - p10) / 2 : null;
    const vibratoCandidate = duration >= 0.8 && crossings >= 4 && rate >= 3 && rate <= 9 && extent >= 6 && extent <= 120;
    sustained.push({
      start_seconds: Math.round(segment.start_seconds * 1000) / 1000,
      end_seconds: Math.round(segment.end_seconds * 1000) / 1000,
      note: segment.note,
      pitch_stability_std_cents: Math.round(Math.sqrt(variance) * 10) / 10,
      pitch_span_cents: Number.isFinite(p10) && Number.isFinite(p90) ? Math.round((p90 - p10) * 10) / 10 : null,
      vibrato_candidate: vibratoCandidate,
      vibrato_candidate_rate_hz: vibratoCandidate ? Math.round(rate * 100) / 100 : null,
      vibrato_candidate_extent_cents: vibratoCandidate ? Math.round(extent * 10) / 10 : null,
      measured_from_pitch_frames: true,
...
export async function analyzeMusicVocalPitch({
  ...
  try {
    ...
    const sustained = sustainedPerformance(rawFrames, segments);
    ...
    return {
      ...
      sustained_note_count: sustained.length,
      vibrato_candidate_count: sustained.filter((entry) => entry.vibrato_candidate).length,
