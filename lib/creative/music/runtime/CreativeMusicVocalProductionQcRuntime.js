...
export function evaluateMusicVocalProductionQc(session={}){
  const routing=inspectMusicVocalProduction(session),intelligence=buildMusicVocalIntelligence({multitrack_session:session}),blockers=[],reviewFlags=[],clips=activeVocalClips(session),evidenceByClip=new Map((intelligence.clips||[]).map(row=>[row.clip_id,row]));
  ...
  const unique=[...new Set(blockers)];return{contract:CONTRACT,applicable:true,technical_ready_for_listening_review:unique.length===0,release_certified:false,human_listening_review_required:true,blockers:unique,review_flags:reviewFlags,routing,intelligence_summary:{clip_count:intelligence.clip_count,sibilance_available:intelligence.sibilance_available,tonal_balance_available:intelligence.tonal_balance_available,dynamics_envelope_available:intelligence.dynamics_envelope_available},source_audio_rewritten:false,artistic_quality_inferred:false,automatic_release_authorized:false};
}
