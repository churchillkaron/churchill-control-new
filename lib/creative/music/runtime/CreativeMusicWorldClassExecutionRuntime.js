...
async function executeGeneratedMusic(input, plan) {
  ...
  const session = { ...generation.session, music_production_binding: bound.binding, music_direction_contract: bound.input.music_direction_contract, music_world_class_plan: plan };
  ...
  return {
    ...
    quality: finalTribunal ? {
      status: finalTribunal.release_ready ? "PASS" : finalTribunal.status,
      release_ready: finalTribunal.release_ready === true,
