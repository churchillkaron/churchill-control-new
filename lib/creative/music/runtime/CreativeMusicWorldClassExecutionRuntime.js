...
import { continueMusicProfessionalProduction } from "./CreativeMusicProfessionalContinuationRuntime.js";
import { buildMusicProfessionalProductionManifest, nextMusicProfessionalProductionAction } from "./CreativeMusicProfessionalProductionRuntime.js";
...
async function executeGeneratedMusic(input, plan) {
  ...
  if (professionalReleaseRequested && sourceAsset) {
    const professionalSourceMetadata = {
      ...(sourceAsset.metadata || {}),
      professional_release_requested: true,
      professional_release_started_at: sourceAsset.metadata?.professional_release_started_at || new Date().toISOString(),
      professional_release_standard: "PROFESSIONAL_RELEASE",
    };
    await CreativeAssetsRuntime.update(sourceAsset.id, { metadata: professionalSourceMetadata });
    sourceAsset.metadata = professionalSourceMetadata;
    const professionalProduction = buildMusicProfessionalProductionManifest({
      plan, input, evidence: { source_generated: true, source_asset_id: sourceAsset.id },
    });
    const professionalNext = nextMusicProfessionalProductionAction({
      plan, input, evidence: { source_generated: true, source_asset_id: sourceAsset.id },
    });
    return {
      success: true, pending: false, failed: false, provider_status: result?.provider_status || "completed",
      settlement: result?.settlement || null, usage_id: usage?.id || null, plan, creative_floor: creativeFloor,
      production_binding: bound.binding, session, asset: publicAsset(sourceAsset), master_asset: null, master_assets: [],
      professional_production: professionalProduction, professional_next_stage: professionalNext,
      professional_release_requested: true, finishing: null, dailies: null, tribunal: null, repair: null,
      output: result?.output || null, quality: worldClassMusicQualityPending(plan, {}),
      release_ready: false, publish_authorized: false,
    };
  }
  ...
  const professionalNext = nextMusicProfessionalProductionAction({ plan, input, evidence: {
    source_generated: Boolean(finalSourceAsset?.id),
    source_asset_id: finalSourceAsset?.id || null,
    stems_ready: finalSourceAsset?.metadata?.professional_stems_ready === true,
    stems: finalSourceAsset?.metadata?.professional_stems || [],
  ...
  return {
    ...
    professional_next_stage: professionalNext,
...
export async function executeWorldClassMusicStudio(input = {}) {
  ...
  if (input.professional_continue === true || text(input.authorized_stage)) {
    return continueMusicProfessionalProduction({ ...input, organization_id: organizationId, creative_project_id: projectId });
  }
