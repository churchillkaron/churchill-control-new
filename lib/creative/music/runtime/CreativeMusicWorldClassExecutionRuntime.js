...
async function executeGeneratedMusic(input, plan) {
  ...
  const releaseReady = finalTribunal?.release_ready === true && finishing?.destination_qc_passed === true && finishing?.perceptual_translation_passed === true;
  const releaseManifest = sourceAsset && finishing ? buildMusicReleaseManifest({
    organization_id: organizationId,
    creative_project_id: text(input.creative_project_id),
    creative_mission_id: text(input.creative_mission_id) || null,
  ...
  if (releaseManifest) await CreativeMusicFinishingRuntime.persistReleaseManifest({ sourceAsset, masterVariants: finishing.master_variants, manifest: releaseManifest });
  ...
  return {
    ...
    perceptual_translation: finishing?.perceptual_translation || null,
    ...
    quality: finalTribunal ? {
      status: releaseReady
        ? "PASS"
        : (finishing?.perceptual_translation_passed === false ? "PERCEPTUAL_TRANSLATION_REVIEW_REQUIRED" : (finishing?.destination_qc_passed === false ? "DESTINATION_MASTER_QC_REQUIRED" : finalTribunal.status)),
      release_ready: releaseReady,
