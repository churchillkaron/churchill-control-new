...
async function executeGeneratedMusic(input, plan) {
  ...
  const session = { ...generation.session, music_production_binding: bound.binding, music_direction_contract: bound.input.music_direction_contract };
  ...
  return {
    ...
    repair: tribunal?.repair || null,
    output: result?.output || null,
    quality: tribunal ? {
      status: tribunal.release_ready ? "PASS" : tribunal.status,
      release_ready: tribunal.release_ready === true,
      intended_vs_rendered: tribunal.review || null,
