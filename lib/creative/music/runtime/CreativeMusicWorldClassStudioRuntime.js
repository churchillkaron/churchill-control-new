...
const CAPABILITIES = Object.freeze([
  ...
  ["professional_vocal_production", "Route declared lead, double, harmony, backing, ad-lib and choir vocals through dedicated production buses, sends, rides and QC without rewriting sources", "creative.music.vocal-production", "IMPLEMENTED"],
  ...
  ["vocal_role_separation", "Separate lead, backing, harmony and choir roles without substituting ordinary four-stem separation", "ai.audio.vocal-role-separate", "RESEARCH_RUNTIME_READY_CERTIFICATION_GATED"],
  ...
  ["singing_voice_identity", "Use an authorized Voice Library identity for zero-shot singing voice conversion", "ai.audio.singing-voice-convert", "RESEARCH_RUNTIME_READY_CERTIFICATION_GATED"],
  ...
  ["pitch_tuning", "Analyze and tune vocal or monophonic pitch", "creative.music.vocal_tune.render", "CERTIFICATION_GATED"],
  ["timing_correction", "Analyze and correct vocal/instrument timing", "creative.music.vocal_timing", "CERTIFICATION_GATED"],
  ...
  ["sfx", "Create and edit sound effects, transitions and sonic-brand assets", "ai.sfx.generate", "CERTIFICATION_GATED"],
  ["foley", "Create, record, import and edit synchronized Foley and ambience against locked picture", "creative.audio.foley", "IMPLEMENTED"],
  ["picture_aware_foley_cues", "Propose synchronized Foley cues from explicit tracked picture events and surface evidence without auto-selecting sound", "creative.audio.foley-picture-cues", "IMPLEMENTED"],
  ...
  ["cinematic_sound_synthesis", "Design layered cinematic effects with transient, body, subharmonic, granular, resonant and morph engines bound to Sound Objects", "creative.audio.sound-synthesis", "IMPLEMENTED"],
  ...
  ["printmaster_versions", "Create separately reviewed theatrical, near-field/home and streaming printmaster variants from the same approved premixes", "creative.audio.printmasters", "IMPLEMENTED"],
  ...
  ["quality_review", "Independent musical, perceptual and technical review", "creative.music.quality", "IMPLEMENTED"],
]);
...
const INTENT_HINTS = Object.freeze({
  ...
  professional_vocal_production: ["vocal production", "lead vocal", "vocal doubles", "vocal harmonies", "backing vocals"],
...
export function buildWorldClassMusicStudioPlan(input = {}) {
  ...
  if (["record_vocals", "professional_vocal_production", "pitch_tuning", "timing_correction", "singing_voice_identity"].some((id) => selectedIds.has(id))) {
  ...
  return {
    ...
    active_audio_rooms: CreativeProfessionalAudioEngineRuntime.listRooms().filter((room) => {
      if (room.id === "MUSIC_PRODUCTION") return selected.some((item) => ["compose_music","create_song","record_vocals","record_instruments","professional_vocal_production","mix"].includes(item.id));
