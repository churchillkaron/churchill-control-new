...
const CAPABILITIES = Object.freeze([
  ...
  ["foley", "Create and edit synchronized foley and ambience", "creative.audio.foley", "PLANNED"],
...
const INTENT_HINTS = Object.freeze({
  ...
  audio_for_video: ["music for video", "score video", "sync to video", "soundtrack"],
...
function capabilityRecord(row) {
  return { id: row[0], description: row[1], capability: row[2], status: row[3] };
}
...
export function buildWorldClassMusicStudioPlan(input = {}) {
  ...
  if (["record_vocals", "pitch_tuning", "timing_correction"].some((id) => selectedIds.has(id))) {
  ...
  if (["stem_separation", "remove_vocals", "isolate_vocals", "backing_track"].some((id) => selectedIds.has(id))) {
    workerIds.add("stem_specialist");
  }
  if (["midi", "melody", "harmony_chords", "drums_groove", "sampler"].some((id) => selectedIds.has(id))) {
    workerIds.add("midi_sampler");
  }
  if (["sfx", "foley", "audio_for_video"].some((id) => selectedIds.has(id))) {
    ["sound_designer", "sfx_foley"].forEach((id) => workerIds.add(id));
  }
  if (["mix", "master", "backing_track", "compose_music", "create_song", "remix", "audio_for_video"].some((id) => selectedIds.has(id))) {
    workerIds.add("mix_engineer");
  }
  if (["master", "backing_track", "compose_music", "create_song", "remix", "audio_for_video"].some((id) => selectedIds.has(id))) {
  ...
  const creativeDevelopment = buildMusicCreativeDevelopment({ ...input, objective });
  return {
    contract: CONTRACT,
