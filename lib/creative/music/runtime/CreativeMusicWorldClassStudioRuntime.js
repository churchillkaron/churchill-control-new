...
const PROFESSIONAL_AUDIO_ROOMS = Object.freeze([
  { id: "MUSIC_PRODUCTION", name: "Music Production", purpose: "Songwriting, composition, recording, vocal/instrument production, editing and music mixing." },
  { id: "SOUND_DESIGN", name: "Sound Design", purpose: "Layered cinematic sound objects, SFX, Foley, ambience, sonic branding and designed texture." },
  { id: "AUDIO_POST", name: "Audio Post", purpose: "Picture lock, dialogue/VO/ADR, Foley, effects, ambience, music edit, spatial automation and re-recording mix." },
  { id: "MASTERING_DELIVERY", name: "Mastering & Delivery", purpose: "Stereo, 5.1 and 7.1 mastering, M&E/stems, loudness/true-peak conformance, QC and release packaging." },
]);
...
export function buildWorldClassMusicStudioPlan(input = {}) {
  ...
  if (["audio_for_video", "cinematic_sound_design", "audio_post", "surround_mix", "professional_post_stems"].some((id) => selectedIds.has(id))) {
  ...
  return {
    ...
    professional_audio_engine: { contract: CreativeProfessionalAudioEngineRuntime.contract, shared_engine: true, rooms: PROFESSIONAL_AUDIO_ROOMS.map((room) => ({ ...room })), dolby_branding_requires_licensed_toolchain: true },
    active_audio_rooms: PROFESSIONAL_AUDIO_ROOMS.filter((room) => {
