...
async function hasCanonicalExistingProduction(ref) {
  const tasks = await ProductionTaskRuntime.list({
    organization_id: ref.organization_id,
    creative_project_id: ref.creative_project_id,
  });
  return tasks.some((task) => Boolean(task?.production_graph_id));
}
...
export const CreativePartnerMissionRuntime = Object.freeze({
  ...
  async advance(input = {}) {
    ...
    const canonicalExistingProduction =
      ACTIVE_PRODUCTION_STAGES.has(state?.stage)
        ? await hasCanonicalExistingProduction(ref)
        : false;
    const result =
      ACTIVE_PRODUCTION_STAGES.has(state?.stage) && canonicalExistingProduction
