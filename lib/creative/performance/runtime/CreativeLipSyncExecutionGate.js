...
import {
  CreativeLipSyncAudioSegmentRuntime,
} from "@/lib/creative/performance/runtime/CreativeLipSyncAudioSegmentRuntime";

const FLAG = Symbol.for("avantiqo.creative.lipsync-gate.v3");
...
async function prepareExactAudio(task, range) {
  const existing = task.input?.lip_sync_audio_segment;
  if (
    existing?.storage_reference &&
    Number(existing.source_audio_start_seconds) === range.start &&
    Number(existing.source_audio_end_seconds) === range.end
  ) {
    return existing;
  }

  return CreativeLipSyncAudioSegmentRuntime.prepare({
...
async function bindLipSync(task = {}) {
  ...
  const exactAudio = await prepareExactAudio(task, range);
  const input = parameters(task);
  const mode = text(
  ...
  return ProductionTaskRuntime.update(task.id, {
    input: {
      ...
      video,
      video_source: video,
      audio: exactAudio.storage_reference,
      audio_source: exactAudio.storage_reference,
      lip_sync_audio_segment: exactAudio,
      audio_start_seconds: 0,
      audio_end_seconds: exactAudio.duration_seconds,
      source_audio_start_seconds: range.start,
      source_audio_end_seconds: range.end,
    ...
    metadata: {
      ...
      source_audio_start_seconds: range.start,
      source_audio_end_seconds: range.end,
      lip_sync_audio_segment_path: exactAudio.path,
      lip_sync_audio_segment_duration_seconds: exactAudio.duration_seconds,
      vocal_performance_mode: mode,
      audio_conditioned: true,
      exact_audio_window_bound: true,
      exact_audio_segment_materialized: true,
    },
  });
}
...
async function bindValidation(task = {}) {
  ...
  const exactAudio = synced.input?.lip_sync_audio_segment ||
    await prepareExactAudio(task, range);
  const input = parameters(task);
  const mode = text(
    input.vocal_performance_mode ||
    task.input?.requirements?.vocal_performance_mode ||
