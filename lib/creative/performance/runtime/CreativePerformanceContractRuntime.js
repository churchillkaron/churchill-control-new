...
function visibleSpeechState(shot = {}, planShot = {}, performerVisible = null) {
  const explicitSpeaking = explicitBoolean(shot, planShot, "speaking_visible");
  if (explicitSpeaking !== null) return explicitSpeaking;

  const explicitDialogue = explicitBoolean(shot, planShot, "dialogue_visible");
  if (explicitDialogue !== null) return explicitDialogue;

  const spokenDialogue = text(
    planShot.dialogue ||
    shot.dialogue ||
    planShot.performance_contract?.dialogue ||
    shot.performance_contract?.dialogue,
  );

  return performerVisible === true && Boolean(spokenDialogue);
}

function qualityProfile(creativePlan = {}) {
  return text(
    creativePlan.quality?.profile ||
    creativePlan.production?.quality_profile ||
    creativePlan.production?.qualityProfile ||
    creativePlan.metadata?.quality_profile,
...
export function buildCreativePerformanceContracts({
  ...
  const resolvedQualityProfile = qualityProfile(creative_plan);
  const premiumFilm = resolvedQualityProfile === "WORLD_CLASS_LUXURY_FILM";
  ...
  for (const shot of sortedShots) {
    ...
    const speakingVisible = visibleSpeechState(shot, planShot, performerVisible);
    const vocalPerformanceVisible = Boolean(
      singingVisible === true || speakingVisible === true,
    );
    ...
    if (vocalPerformanceVisible && !primaryAudio?.asset_id) {
      shotIssues.push("PRIMARY_AUDIO_ASSET_UNRESOLVED");
    }
    if (vocalPerformanceVisible && mouthVisible === null) {
    ...
    const contract = {
      version: "CREATIVE_PERFORMANCE_CONTRACT_V3",
      evidence_contract: "STRUCTURED_PERFORMANCE_EVIDENCE_V2",
      quality_profile: resolvedQualityProfile,
      performer_visible: performerVisible,
      singing_visible: singingVisible,
      speaking_visible: speakingVisible,
      dialogue_visible: speakingVisible,
      vocal_performance_visible: vocalPerformanceVisible,
      vocal_performance_mode:
        singingVisible === true
          ? "SINGING"
      ...
      minimum_sync_score: premiumFilm ? 96 : 88,
      minimum_identity_score: premiumFilm ? 96 : 90,
      minimum_performance_score: premiumFilm ? 92 : 82,
      ...
      off_camera_narration_does_not_trigger_lipsync: true,
    ...
    enrichedById.set(text(shot.id), {
      ...
      metadata: {
        ...
        performance_contract: contract,
        identity_profile: identityProfile,
        creative_quality_profile: resolvedQualityProfile,
      },
    });
  ...
  const performanceContext = {
    version: "CREATIVE_PERFORMANCE_CONTEXT_V3",
    evidence_contract: "STRUCTURED_PERFORMANCE_EVIDENCE_V2",
    quality_profile: resolvedQualityProfile,
    ...
    lip_sync_shot_ids: enrichedShots
      ...
      .map((shot) => shot.id),
    speaking_lip_sync_shot_ids: enrichedShots
      .filter((shot) =>
        shot.performance_contract?.lip_sync_required &&
        shot.performance_contract?.speaking_visible === true,
      )
      .map((shot) => shot.id),
    singing_lip_sync_shot_ids: enrichedShots
      .filter((shot) =>
        shot.performance_contract?.lip_sync_required &&
        shot.performance_contract?.singing_visible === true,
      )
      .map((shot) => shot.id),
...
export const CreativePerformanceContractRuntime = Object.freeze({
  contract: "CREATIVE_PERFORMANCE_CONTRACT_V3",
