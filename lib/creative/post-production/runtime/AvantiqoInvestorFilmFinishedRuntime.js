...
const OUTPUT_PATH = `${OUTPUT_DIR}/avantiqo-investor-film-v5-identity-safe-review.mp4`;
...
async function mediaDuration(ffmpeg, source) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", () => {
      ...
      if (!duration) reject(new Error("MEDIA_DURATION_UNAVAILABLE"));
...
async function renderLogoClip(ffmpeg, logoRaw, output, duration) {
  await run(ffmpeg, [
    "-y", ...THREAD_ARGS,
    "-f", "rawvideo", "-pixel_format", "rgba", "-video_size", "1280x720", "-framerate", "24", "-i", logoRaw,
    "-vf", `tpad=stop_mode=clone:stop_duration=${duration},zoompan=z='min(zoom+0.00025,1.035)':d=1:s=1280x720:fps=24,fade=t=in:st=.2:d=.75,fade=t=out:st=${Math.max(0, duration - .65)}:d=.6,format=yuv420p`,
    "-an", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "19", "-r", "24", "-t", String(duration), output,
  ]);
}

async function renderFounderPlate(ffmpeg, founderUrl, output, duration, variant) {
  const inc = [0.000055, 0.00004, 0.000065, 0.000045, 0.00006][variant] || 0.00005;
  await run(ffmpeg, [
    "-y", ...THREAD_ARGS,
    "-loop", "1", "-framerate", "24", "-i", founderUrl,
    "-vf", `scale=1360:765:force_original_aspect_ratio=increase,crop=1280:720,zoompan=z='min(zoom+${inc},1.04)':d=1:s=1280x720:fps=24,eq=contrast=1.02:saturation=.97,format=yuv420p`,
    "-an", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "19", "-r", "24", "-t", String(duration), output,
  ]);
}

async function renderScreenRise(ffmpeg, baseUrl, screenRaw, mapRaw, output) {
  const filter = [
  ...
  await run(ffmpeg, [
    ...
    "-filter_complex", filter, "-map", "[v]", "-an", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "19", "-r", "24", "-t", "9", output,
  ]);
}

async function normalizeSegment(ffmpeg, source, output, duration) {
  await run(ffmpeg, [
    "-y", ...THREAD_ARGS,
    "-i", source,
    "-t", duration.toFixed(3),
    "-vf", "scale=1280:720:force_original_aspect_ratio=increase,crop=1280:720,fps=24,setsar=1,eq=contrast=1.02:saturation=.97:brightness=-.003,format=yuv420p",
    "-an", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "20", "-r", "24", "-video_track_timescale", "90000", output,
  ]);
}

async function concatenateSegments(ffmpeg, files, output, directory) {
  const listPath = path.join(directory, "segments.txt");
  await fs.writeFile(listPath, files.map((file) => `file '${file.replaceAll("'", "'\\''")}'`).join("\n"));
  await run(ffmpeg, ["-y", ...THREAD_ARGS, "-f", "concat", "-safe", "0", "-i", listPath, "-c", "copy", "-movflags", "+faststart", output]);
}
...
function fitEditToNarration(duration) {
  ...
  if (duration - used > 0.2) {
    result.push({ key: close[0], duration: Math.min(close[1], duration - used) });
  }
  return result;
}
...
export const AvantiqoInvestorFilmFinishedRuntime = {
  ...
  async status() {
    return {
      ...
      render_strategy: "ONE_SEGMENT_AT_A_TIME_FFMPEG_V2",
  ...
  async render() {
    ...
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-investor-v5-"));
    try {
      const narrationUrl = await signedUrl(NARRATION_PATH, 3600);
      const scoreUrl = await signedUrl(SCORE_PATH, 3600);
      const founderUrl = await signedUrl(FOUNDER_REFERENCE_PATH, 3600);
      const narrationDuration = await mediaDuration(ffmpeg, narrationUrl);
      const edit = fitEditToNarration(narrationDuration);

      const sourceUrls = new Map();
      for (const key of [...new Set(edit.map((item) => item.key).filter((key) => SOURCES[key]))]) {
        sourceUrls.set(key, await signedUrl(SOURCES[key], 3600));
      }
      ...
      for (let index = 0; index < 5; index += 1) {
        ...
        await renderFounderPlate(ffmpeg, founderUrl, target, 9.4, index);
        local.set(key, target);
      }

      if (edit.some((item) => item.key === "b03")) {
        const target = path.join(directory, "screen-rise.mp4");
        await renderScreenRise(ffmpeg, sourceUrls.get("b03"), screenRaw, mapRaw, target);
        local.set("b03", target);
      }

      const segments = [
        { source: intro, duration: 4.5 },
        ...edit.map((item) => ({
          source: local.get(item.key) || sourceUrls.get(item.key),
          duration: item.duration,
        })),
        { source: outro, duration: 5 },
      ];

      const normalized = [];
      for (let index = 0; index < segments.length; index += 1) {
        const target = path.join(directory, `segment-${String(index).padStart(3, "0")}.mp4`);
        await normalizeSegment(ffmpeg, segments[index].source, target, segments[index].duration);
        normalized.push(target);
      }

      const videoOnly = path.join(directory, "video-only.mp4");
      await concatenateSegments(ffmpeg, normalized, videoOnly, directory);
      ...
      await run(ffmpeg, [
        "-y", ...THREAD_ARGS,
        "-i", narrationUrl, "-stream_loop", "-1", "-i", scoreUrl,
        "-filter_complex", audioFilter, "-map", "[aout]",
        "-c:a", "aac", "-b:a", "160k", "-ar", "48000", "-ac", "2", "-t", totalDuration.toFixed(3), audioOnly,
      ]);
      ...
      await run(ffmpeg, [
        ...
        "-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-c:a", "copy", "-movflags", "+faststart", "-shortest", output,
      ]);
      ...
      return {
        ...
        lip_sync_applied: false,
        review_master_quality: "H264_CRF20_SEQUENTIAL_AAC160K",
