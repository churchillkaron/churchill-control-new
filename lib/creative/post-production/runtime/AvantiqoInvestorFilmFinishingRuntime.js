...
function xfadeChain(labels, durations, transition = 0.42) {
  ...
  for (let index = 1; index < labels.length; index += 1) {
    ...
    filters.push(`[${current}][${next}]xfade=transition=${transitions[(index - 1) % transitions.length]}:duration=${transition}:offset=${offset.toFixed(3)}[${output}]`);
...
export const AvantiqoInvestorFilmFinishingRuntime = {
  ...
  async render({ mode = "review", useScore = true } = {}) {
    ...
    try {
      ...
      const filler = ["b07", "b08", "b09", "b10", "b14", "b15", "b18", "b19", "b20"];
      let fillerIndex = 0;
      ...
      const args = [
        ...
        "-t", String(introDuration + outroDuration + 2),
        "-i", logoPath,
      ];
      ...
      args.push(
        ...
        "-map", "[video]", "-map", "[audio]",
        "-c:v", "libx264", "-preset", review ? "veryfast" : "slow",
        review ? "-crf" : "-b:v", review ? "20" : "285k",
        ...(review ? [] : ["-maxrate", "340k", "-bufsize", "680k"]),
        "-pix_fmt", "yuv420p", "-c:a", "aac",
        "-b:a", review ? "160k" : "64k", "-ar", "48000", "-ac", "2",
        "-movflags", "+faststart", "-t", targetVisualDuration.toFixed(3), output,
      );
