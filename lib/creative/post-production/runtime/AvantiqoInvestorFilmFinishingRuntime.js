    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720">
      <defs>
        <radialGradient id="glow" cx="50%" cy="49%" r="42%">
          <stop offset="0" stop-color="#b69552" stop-opacity="0.12"/>
          <stop offset="0.46" stop-color="#66502a" stop-opacity="0.045"/>
          <stop offset="1" stop-color="#000000" stop-opacity="0"/>
        </radialGradient>
      </defs>
      <rect width="1280" height="720" fill="url(#glow)"/>
      <line x1="150" y1="360" x2="1130" y2="360" stroke="#d3b46d" stroke-opacity="0.12" stroke-width="1"/>
      <circle cx="228" cy="360" r="2.2" fill="#d3b46d" fill-opacity="0.30"/>
      <circle cx="1052" cy="360" r="2.2" fill="#d3b46d" fill-opacity="0.30"/>
    </svg>
  `);
...
async function renderLogoTs(ffmpeg, rawPath, outputPath, duration, outro = false) {
  ...
  const filter = [
    ...
    "eq=contrast=1.04:saturation=0.94:brightness=-0.008",
...
async function cutSourceTs(ffmpeg, sourcePath, outputPath, duration) {
  await run(ffmpeg, [
    "-y",
    "-i", sourcePath,
    "-t", String(duration),
...
function expandedEditForNarration(narrationDuration) {
  const edit = BASE_EDIT.map(([key, duration]) => [key, duration]);
  const requiredContent = narrationDuration + 1.0;
  let current = edit.reduce((sum, [, duration]) => sum + duration, 0);
  let index = 0;
  while (current < requiredContent) {
    const item = FILLER[index % FILLER.length];
    edit.splice(Math.max(1, edit.length - 1), 0, [...item]);
    current += item[1];
    index += 1;
  }
  return edit;
...
export const AvantiqoInvestorFilmFinishingRuntime = {
  ...
  async status() {
    ...
    return {
      ...
      canonical_cedar_narration_ready: await storageExists(NARRATION_PATH),
      ...
      render_strategy: "FAST_TS_STREAM_COPY_MASTER_V4",
      source_count: Object.keys(SOURCES).length,
      base_edit_segment_count: BASE_EDIT.length,
  ...
  async render({ mode = "review", useScore = true } = {}) {
    ...
    if (!(await storageExists(NARRATION_PATH))) {
      throw new Error("CANONICAL_CEDAR_NARRATION_REQUIRED");
    }
    if (!(await storageExists(BUSINESS_LOOP_V2_PATH))) {
      throw new Error("BUSINESS_LOOP_V2_REQUIRED");
    }

    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-investor-master-v4-"));
    ...
    try {
      ...
      const edit = expandedEditForNarration(narrationDuration);
      ...
      await mapLimit(uniqueKeys, 4, async (key) => {
      ...
      const introTs = path.join(directory, "segment-000-intro.ts");
      await renderLogoTs(ffmpeg, rawLogo, introTs, INTRO_SECONDS, false);
      segments.push(introTs);

      for (let index = 0; index < edit.length; index += 1) {
        const [sourceKey, duration] = edit[index];
        const segmentPath = path.join(
          directory,
          `segment-${String(index + 1).padStart(3, "0")}-${sourceKey}.ts`,
        );
        await cutSourceTs(
          ffmpeg,
      ...
      const outroTs = path.join(
        directory,
        `segment-${String(edit.length + 1).padStart(3, "0")}-outro.ts`,
      );
      await renderLogoTs(ffmpeg, rawLogo, outroTs, OUTRO_SECONDS, true);
      segments.push(outroTs);
      ...
      const visualMaster = path.join(directory, "visual-master.ts");
      await run(ffmpeg, [
        "-y",
        "-fflags", "+genpts",
        "-f", "concat",
      ...
      const targetDuration = Math.min(
        visualDuration,
        Math.max(narrationEndsAt + 2.5, narrationEndsAt + OUTRO_SECONDS),
      );
      ...
      if (score) {
        audioFilters.push(
          `[2:a]volume=0.095,afade=t=in:st=0:d=2.4,afade=t=out:st=${Math.max(0, targetDuration - 4).toFixed(3)}:d=3.5,atrim=duration=${targetDuration.toFixed(3)},aresample=48000[music]`,
        );
        audioFilters.push(
      ...
      const finalDuration = await mediaDuration(ffmpeg, output);
      const storagePath = `${OUTPUT_DIR}/avantiqo-investor-film-${mode}-v4-cinematic.mp4`;
      ...
      return {
        ...
        mode,
        render_strategy: "FAST_TS_STREAM_COPY_MASTER_V4",
        canonical_cedar_narration: true,
        ...
        next_pass: "LOCK_TIMECODES_THEN_LIPSYNC_FOUNDER_WINDOWS",
