...
async function renderReveal(ffmpeg, logoRaw, output) {
  ...
  const filter = [
    ...
    `[l0]scale=w='iw*(${scale})':h='ih*(${scale})':eval=frame,rotate='${tilt}':c=none:ow=rotw(iw):oh=roth(ih),boxblur=18:2,colorchannelmixer=rr=.72:gg=.49:bb=.17:aa=.28[glow]`,
    `[l1]scale=w='iw*(${scale})':h='ih*(${scale})':eval=frame,rotate='${tilt}':c=none:ow=rotw(iw):oh=roth(ih),colorchannelmixer=rr=.42:gg=.30:bb=.14:aa=.82[depth]`,
    `[l2]scale=w='iw*(${scale})':h='ih*(${scale})':eval=frame,rotate='${tilt}':c=none:ow=rotw(iw):oh=roth(ih),fade=t=in:st=.18:d=.82:alpha=1[main]`,
    ...
    "[2:v]format=rgba,rotate='0.20':c=none:ow=rotw(iw):oh=roth(ih),boxblur=20:2,fade=t=in:st=1.55:d=.30:alpha=1,fade=t=out:st=2.70:d=.45:alpha=1[beam]",
    "[s3][beam]overlay=x='-420+(W+760)*min(max((t-1.55)/1.35,0),1)':y='(H-h)/2':eval=frame[s4]",
    `[s4]vignette=PI/7,fade=t=in:st=0:d=.25,fade=t=out:st=${DURATION - .55}:d=.5,format=yuv420p[v]`,
  ].join(";");
