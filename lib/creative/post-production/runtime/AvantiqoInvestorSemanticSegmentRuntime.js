...
async function renderStill(ffmpeg, source, output, duration, variant = 0) {
  ...
  await run(ffmpeg, [
    ...
    "-vf", [
      ...
      "vignette=PI/12",
      "fade=t=in:st=0:d=.28",
      `fade=t=out:st=${Math.max(0, duration - .28)}:d=.28`,
      "format=yuv420p",
    ].join(","),
    "-c:v", "libx264",
    "-preset", "veryfast",
...
async function renderLogoHold(ffmpeg, logoPath, directory, output, duration) {
  const frame = path.join(directory, "approved-logo-final-frame.png");
  await run(ffmpeg, [
    "-y",
    "-sseof", "-0.08",
    "-i", logoPath,
...
async function renderUiSequence({ ffmpeg, directory, manifest, beat, slots, output }) {
  ...
  for (const [index, slot] of requested.entries()) {
    ...
    const image = path.join(directory, `${beat.id}-${index}-${slot}.png`);
    const clip = path.join(directory, `${beat.id}-${index}-${slot}.mp4`);
    await download(item.normalized_path, image);
...
async function renderFounderBeat({ ffmpeg, directory, beat, output }) {
  ...
  if (beat.id === "close-02") {
    // close-01 and close-02 share one continuous 6.75 second founder lip-sync file.
    // Use the second sentence from the continuous file for close-02.
