...
const MISSION_CONTRACT = "AVANTIQO_INVESTOR_STUDIO_MISSION_V2";
...
async function ensureMissionAndProject(job) {
...
async function prepareJob(job, organization) {
  const metadata = missionMetadata(job);
  const { mission, project, resumed } = await ensureMissionAndProject(job);
...
export async function prepareInvestorStudioScenes({ scenes } = {}) {
  ...
  return {
    success: true,
    contract: "AVANTIQO_INVESTOR_STUDIO_PREPARATION_V2",
    organization_id: ORGANIZATION_ID,
    fresh_generation_required: true,
...
export async function executeInvestorStudioScenes({
  ...
  return {
    ...
    contract: "AVANTIQO_INVESTOR_STUDIO_EXECUTION_V2",
    organization_id: ORGANIZATION_ID,
    provider_selection_exposed: false,
    fresh_generation_required: true,
    results,
