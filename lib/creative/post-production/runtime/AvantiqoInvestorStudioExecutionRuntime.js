...
const MAXIMUM_DYNAMIC_TRIBUNAL_CALLS = 22;
...
async function ensureInvestorDirectionApproval({ job, project }) {
  ...
  const maximumCalls = 12 + MAXIMUM_DYNAMIC_TRIBUNAL_CALLS + maximumTemporalSceneCalls(job.duration_seconds);
...
function missionMetadata(job, organization = {}) {
  ...
  return {
    ...
    automatic_image_generation_allowed: false,
    video_first_generation_required: true,
...
function validateFreshPlan(job, routed) {
  ...
  const imageGenerationTasks = tasks.filter((task) => taskCapability(task) === "ai.image.generate");
  if (imageGenerationTasks.length) {
    throw new Error(`INVESTOR_STUDIO_AUTOMATIC_IMAGE_GENERATION_FORBIDDEN:SCENE_${job.scene}:${imageGenerationTasks.map((task) => task.id).join(",")}`);
  }
