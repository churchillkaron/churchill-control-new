...
function sceneJob(scene, index) {
  ...
  const humanGeneration = investorHumanGenerationPolicy(scene.scene);

  return Object.freeze({
    contract: "AVANTIQO_INVESTOR_STUDIO_SCENE_JOB_V2_HUMAN_CONTROLLED",
    ...
    human_generation: humanGeneration,
    ...
    review_policy: humanGeneration.human_mode_required
      ? "MULTI_KEYFRAME_GENERATE_THEN_EXACT_MASTER_QC_THEN_VISUAL_APPROVAL_THEN_RELEASE"
      : "GENERATE_THEN_RENDER_WITH_LOCKED_NARRATION_THEN_USER_APPROVAL",
  });
}
...
export async function resolveInvestorStudioGenerationJobs({
  ...
  return Object.freeze({
    contract: "AVANTIQO_INVESTOR_STUDIO_GENERATION_RESOLUTION_V2_HUMAN_CONTROLLED",
    organization_id: organizationId,
    capability_source: "ORGANIZATION_SERVICE_RUNTIME_PLUS_AVANTIQO_LOCAL_CREATIVE_CORE",
    provider_selection_exposed: false,
    human_generation_policy_bound: true,
...
export function assertInvestorStudioGenerationReady(resolution) {
  ...
  for (const job of list(resolution.jobs)) {
    if (!job.human_generation?.contract) {
      throw new Error(`INVESTOR_STUDIO_HUMAN_POLICY_NOT_BOUND:SCENE_${job.scene}`);
    }
  }
  return resolution;
}
