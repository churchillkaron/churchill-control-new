...
function resolvedBoundary({
  ...
  const causal = fromRequirement.editorial_causality && typeof fromRequirement.editorial_causality === "object"
    ? fromRequirement.editorial_causality
    : {};
  const blockers = [];
  const causalMotivation = text(causal.cut_motivation, 120).toUpperCase();
  const causalTrigger = text(causal.cut_trigger, 800);
  const causalHandoff = text(causal.information_handoff, 1200);
  const causalChange = text(causal.what_changes_after_cut, 1200);
  if (!causalMotivation) blockers.push("EDITORIAL_CAUSAL_MOTIVATION_REQUIRED");
  if (causalTrigger.length < 10) blockers.push("EDITORIAL_CAUSAL_TRIGGER_REQUIRED");
  if (causalHandoff.length < 14) blockers.push("EDITORIAL_CAUSAL_HANDOFF_REQUIRED");
  if (causalChange.length < 12) blockers.push("EDITORIAL_CAUSAL_STATE_CHANGE_REQUIRED");
  ...
  const base = {
    ...
    editorial_causality: {
      cut_motivation: causalMotivation || null,
      cut_trigger: causalTrigger || null,
      information_handoff: causalHandoff || null,
      sound_bridge: text(causal.sound_bridge, 1000) || null,
      visual_match_or_contrast: text(causal.visual_match_or_contrast, 1000) || null,
      what_is_withheld: text(causal.what_is_withheld, 1000) || null,
      what_changes_after_cut: causalChange || null,
    },
    ...
    policy: {
      ...
      every_boundary_requires_causal_motivation: true,
      angle_change_alone_is_not_cut_motivation: true,
      aggressive_cutting_requires_information_handoff: true,
    },
  };
  return {
    ...base,
