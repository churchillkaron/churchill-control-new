...
function profile(input={}){return{
  contract:"AVANTIQO_OPTICAL_PROFILE_V1",
  lens_k1:clamp(input.lens_k1,-.18,.18,-.018),lens_k2:clamp(input.lens_k2,-.12,.12,.006),
  chromatic_shift_pixels:clamp(input.chromatic_shift_pixels,0,6,.55),vignette_angle:clamp(input.vignette_angle,.15,1.4,.52),
  bloom_sigma:clamp(input.bloom_sigma,0,12,2.2),bloom_strength:clamp(input.bloom_strength,0,.35,.065),
  grain_strength:clamp(input.grain_strength,0,24,3.2),preserve_audio:true,color_grade_forbidden:true,deterministic_replay_required:true,
};}
