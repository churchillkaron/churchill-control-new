...
import "@/lib/creative/post-production/runtime/CreativeEditorialAssemblyRenderBootstrap";
import "@/lib/creative/audio/runtime/CreativeMasterSoundtrackRenderGate";
import "@/lib/creative/post-production/runtime/CreativeProfessionalFinishingBootstrap";
import "@/lib/creative/color/runtime/CreativeColorFinishingBootstrap";
import {
...
export function creativeAssetEligibleForMaster(node = {}) {
  const productionRoomContract =
    node.metadata?.requirements?.production_room_pipeline?.contract ||
    node.metadata?.production_room_pipeline?.contract ||
    null;
  const governedByProductionRooms =
    productionRoomContract === "CREATIVE_PRODUCTION_ROOM_PIPELINE_V1";
  const dailiesApproved =
    node.metadata?.dailies_contract === "CREATIVE_DAILIES_ROOM_V1" &&
    node.metadata?.dailies_approved === true &&
    node.metadata?.dailies_report?.passed === true;

  return (
    node.status !== "REJECTED" &&
    node.metadata?.blocked !== true &&
    node.metadata?.include_in_master !== false &&
    (!governedByProductionRooms || dailiesApproved)
  );
}
...
function isSourceVideo(node = {}) {
  return (
    ...
    creativeAssetEligibleForMaster(node) &&
...
function automaticTracks(nodes = [], timeline = {}) {
  ...
  const audio = nodes
    ...
    .filter((node) => node.url && creativeAssetEligibleForMaster(node))
    .map((node) => ({
...
export const CreativePostProductionRuntime = {
  async run({
    organization_id,
    creative_project_id,
    approved_timeline_asset_node_id = null,
  } = {}) {
    ...
    if (!approved_timeline_asset_node_id) {
    ...
    let timeline = null;
    let requirements = [];
    let prepared = { status: "LOCKED_TIMELINE", videos: [], clips: [], moments: [] };

    if (approved_timeline_asset_node_id) {
      timeline = allNodes.find((node) =>
        node.id === approved_timeline_asset_node_id &&
        node.type === CREATIVE_ASSET_NODE_TYPES.TIMELINE
      ) || null;
      if (!timeline) throw new Error("APPROVED_TIMELINE_NOT_FOUND");
      if (timeline.creative_project_id !== creative_project_id) {
        throw new Error("APPROVED_TIMELINE_PROJECT_MISMATCH");
      }
      if (timeline.metadata?.human_picture_lock_required !== true) {
        throw new Error("DIRECTED_PICTURE_LOCK_TIMELINE_REQUIRED");
      }
      requirements = list(timeline.metadata?.requirements);
    } else {
    ...
    const tracks = automaticTracks(allNodes, timeline);
    ...
    const sourceNodes = timeline.metadata?.edit_decision_list
      .map((edit) => allNodes.find((node) =>
        node.id === edit.source_asset_node_id ||
        node.id === edit.source_clip_node_id,
      ))
      .filter((node) => node && creativeAssetEligibleForMaster(node));
    ...
    const perceptualPolicy = policy.quality.perceptual_policy ||
      ...
      {};
    const collectFrameDynamics = policy.quality.collect_frame_dynamics !== false;
    const collectAudioDynamics = policy.quality.collect_audio_dynamics !== false;
    if (
      policy.quality.require_perceptual_qc === true ||
      Object.keys(perceptualPolicy).length ||
      collectFrameDynamics ||
      collectAudioDynamics
    ) {
      perceptual = await CreativePerceptualQualityRuntime.analyze({
        ...
        policy: {
          ...policy.render,
          ...perceptualPolicy,
          collect_frame_dynamics: collectFrameDynamics,
          collect_audio_dynamics: collectAudioDynamics,
        },
      });
    }
    ...
    return {
      ...
      timeline,
      approved_timeline_consumed: Boolean(approved_timeline_asset_node_id),
      semantic_recomposition_performed: !approved_timeline_asset_node_id,
      tracks,
      export_profile: exportProfile,
