...
const CONTRACT = "CREATIVE_PROFESSIONAL_FINISHING_V2";
...
function gradeFilters(shot = {}) {
  // Final color authority belongs exclusively to CreativeColorFinishingRuntime.
  // Shot lighting/color metadata remains creative intent and must not apply a second grade here.
  void shot;
  return [];
}
...
function textEntries(shot = {}, duration = 0) {
  ...
  return entries.map((entry) => {
    const role = text(entry.role || entry.type || entry.kind || "TITLE").toUpperCase();
    const copy = text(
      entry.text || entry.title || entry.copy || entry.content || entry.label,
    );
    const defaultPosition = role === "LEGAL" ? "BOTTOM" : "BOTTOM";
    return {
      role,
      text: copy,
      text_sha256: sha256(copy),
      position: text(entry.position || entry.placement || defaultPosition).toUpperCase(),
      start_seconds: clamp(
        number(entry.start_seconds ?? entry.startSeconds) || 0.12,
        0,
        Math.max(0, duration),
      ),
      duration_seconds:
        positive(entry.duration_seconds ?? entry.durationSeconds) ||
        Math.min(role === "LEGAL" ? 5 : 3, Math.max(0.5, duration - 0.12)),
      ...
      font_size: positive(entry.font_size ?? entry.fontSize),
      font_color: text(entry.font_color || entry.fontColor || entry.color) || "white",
      border_width: Math.max(0, number(entry.border_width ?? entry.borderWidth) ?? 2),
      border_color: text(entry.border_color || entry.borderColor) || "black@0.55",
    };
...
function directorControls(shot = {}, duration = 0) {
  return {
    ...
    titles: textEntries(shot, duration),
...
function buildSegments(timeline = {}, shots = []) {
  ...
  const segments = edits.map((edit, index) => {
    ...
    if (!inputDuration) {
      throw new Error(`PROFESSIONAL_FINISHING_EDIT_DURATION_REQUIRED:${index + 1}`);
    }
  ...
  const completeDirectorCoverage = segments.length > 0 &&
    segments.every((segment) =>
      segment.shot && positive(segment.controls.target_duration_seconds));
  const directorDuration = completeDirectorCoverage
    ? segments.reduce(
        (sum, segment) => sum + segment.controls.target_duration_seconds,
        0,
      )
    : null;
  ...
  if (retimingSafe) {
    for (const segment of segments) {
      const speed =
        segment.input_duration_seconds / segment.controls.target_duration_seconds;
...
function fadeFilters(transitionIn, transitionOut, duration) {
  ...
  if (transitionOut.type !== "CUT") {
    ...
    if (d > 0.01) {
      filters.push(
        `fade=t=out:st=${Math.max(0, duration - d).toFixed(3)}:d=${d.toFixed(3)}`,
      );
    }
  }
  return filters;
...
function audioFadeFilters(transitionIn, transitionOut, duration) {
  ...
  if (transitionOut.type !== "CUT") {
    ...
    if (d > 0.01) {
      filters.push(
        `afade=t=out:st=${Math.max(0, duration - d).toFixed(3)}:d=${d.toFixed(3)}`,
      );
    }
  }
  return filters;
...
function exactFontRequirement(entry = {}) {
  return Boolean(
    entry.font_asset_id ||
    entry.font_family ||
    entry.font_file_reference,
  );
}

async function materializeFontBindings({
  organization_id,
  plan,
  nodes,
  policy,
  ...
  const requested = [
    ...new Set(
      plan.segments
        .flatMap((segment) => segment.controls.titles)
        .filter(exactFontRequirement)
        .map((entry) => entry.font_asset_id)
        .filter(Boolean),
    ),
  ];
  ...
  for (const assetId of requested) {
    const asset = nodes.find((node) => node.id === assetId);
    if (!asset) throw new Error(`FONT_ASSET_NOT_FOUND:${assetId}`);
    if (asset.type !== CREATIVE_ASSET_NODE_TYPES.FONT) {
      throw new Error(`FONT_ASSET_TYPE_INVALID:${assetId}`);
    }
    if (
      asset.status === CREATIVE_ASSET_NODE_STATUS.REJECTED ||
      asset.status === CREATIVE_ASSET_NODE_STATUS.ARCHIVED ||
      !asset.url
    ) {
      throw new Error(`FONT_ASSET_NOT_RENDERABLE:${assetId}`);
    }
    const material = await materializeMedia({
      ...
      file_name:
        ...
        asset.name ||
        `font-${asset.id}.ttf`,
      mime_type: asset.technical?.mime_type || null,
      organization_id,
      policy,
    });
    materials.push(material);
    bindings.set(assetId, {
      asset_node_id: asset.id,
      file_path: material.file_path,
      checksum: material.checksum || asset.technical?.checksum || null,
      mime_type: material.mime_type || asset.technical?.mime_type || null,
      family:
        asset.metadata?.font_family ||
  ...
  for (const entry of plan.segments.flatMap((segment) => segment.controls.titles)) {
    if (!exactFontRequirement(entry)) continue;
    if (!entry.font_asset_id) {
      throw new Error("EXACT_FONT_ASSET_ID_REQUIRED");
    }
    if (!bindings.has(entry.font_asset_id)) {
      throw new Error(`FONT_BINDING_REQUIRED:${entry.font_asset_id}`);
    }
  }
...
function drawtextFilter(title, segmentDuration, fontBindings) {
  const start = clamp(title.start_seconds, 0, Math.max(0, segmentDuration));
  const end = clamp(
    start + title.duration_seconds,
    start,
    Math.max(start, segmentDuration),
  );
  const y = title.position.includes("TOP")
    ...
    : title.position.includes("CENTER") || title.position.includes("MIDDLE")
      ...
      : title.role === "LEGAL"
        ? "h*0.90"
        : "h*0.82";
  const binding = title.font_asset_id
    ? fontBindings.get(title.font_asset_id)
    : null;
  const fontSize = title.font_size
    ? String(title.font_size)
    : title.role === "LEGAL"
      ? "h/36"
      : "h/18";
  const options = [
    `drawtext=text='${escapeFilterValue(title.text)}'`,
  ];
  if (binding?.file_path) {
    options.push(`fontfile='${escapeFilterValue(binding.file_path)}'`);
  }
  options.push(
    "x=(w-text_w)/2",
    `y=${y}`,
    `fontsize=${fontSize}`,
    `fontcolor=${escapeFilterValue(title.font_color)}`,
    `borderw=${title.border_width}`,
    `bordercolor=${escapeFilterValue(title.border_color)}`,
    `enable='between(t,${start.toFixed(3)},${end.toFixed(3)})'`,
  );
  return options.join(":");
}

function videoSegmentFilter(segment, outputIndex, fontBindings) {
  const controls = segment.controls;
  const filters = [
    ...
    ...controls.titles.map((title) =>
      drawtextFilter(title, segment.output_duration_seconds, fontBindings)),
    "setsar=1",
  ];
...
function textRenderEvidence(plan, fontBindings) {
  return plan.segments.flatMap((segment) =>
    segment.controls.titles.map((entry) => {
      const binding = entry.font_asset_id
        ? fontBindings.get(entry.font_asset_id)
        : null;
      return {
        ...
        exact_font_required: exactFontRequirement(entry),
        font_asset_id: entry.font_asset_id,
        font_family_declared: entry.font_family,
        font_asset_checksum: binding?.checksum || null,
        font_asset_mime_type: binding?.mime_type || null,
        font_binding_proven: exactFontRequirement(entry)
          ? Boolean(binding?.asset_node_id && binding?.checksum)
          : true,
      };
    }),
  );
...
function reportFor(plan, applied, reason = null, evidence = []) {
  const exact = evidence.filter((entry) => entry.exact_font_required);
  const legal = evidence.filter((entry) => entry.role === "LEGAL");
  const cta = evidence.filter((entry) => entry.role === "CTA");
  return {
    contract: CONTRACT,
    applied,
    reason,
    ...
    deterministic_text_contract: TEXT_EVIDENCE_CONTRACT,
    deterministic_text_count: evidence.length,
    exact_font_requirement_count: exact.length,
    exact_font_binding_count: exact.filter((entry) => entry.font_binding_proven).length,
    legal_copy_count: legal.length,
    cta_copy_count: cta.length,
  };
}

export const CreativeProfessionalFinishingRuntime = Object.freeze({
  contract: CONTRACT,
  text_evidence_contract: TEXT_EVIDENCE_CONTRACT,
  ...
  async finish({
    ...
    if (existing) {
      return {
        ...
        report:
          existing.metadata?.professional_finishing ||
          reportFor(plan, true, null, list(existing.metadata?.text_render_evidence)),
        reused: true,
      };
    }
    ...
    const fonts = await materializeFontBindings({
      organization_id,
      plan,
      nodes,
      policy,
    ...
    try {
      ...
      plan.segments.forEach((segment, index) => {
        filters.push(videoSegmentFilter(segment, index, fonts.bindings));
      ...
      const outputPath = path.join(
        directory,
        `professional-finish.${extension(export_profile)}`,
      );
      const args = [
        "-y",
        "-i",
      ...
      if (subtitleMode === "mux") args.push("-map", "0:s?");

      const videoCodec = export_profile.video_codec || export_profile.videoCodec;
      if (!videoCodec) throw new Error("EXPORT_VIDEO_CODEC_REQUIRED");
      args.push("-c:v", String(videoCodec));
      ...
      if (subtitleMode === "mux") {
        args.push(
          "-c:s",
          String(export_profile.subtitle_codec || export_profile.subtitleCodec || "copy"),
        );
      }
      ...
      const evidence = textRenderEvidence(plan, fonts.bindings);
      const report = reportFor(plan, true, null, evidence);
      const node = createCreativeAssetNode({
        ...
        description:
          "Director-controlled professional finishing master with deterministic typography, legal-copy and CTA evidence.",
        url: uploaded.url,
        ...
        metadata: {
          ...
          deterministic_text_render_contract: TEXT_EVIDENCE_CONTRACT,
          text_render_evidence: evidence,
          deterministic_text_render_complete:
            evidence.every((entry) => entry.font_binding_proven),
          exact_font_binding_proven:
            evidence
              .filter((entry) => entry.exact_font_required)
              .every((entry) => entry.font_binding_proven),
          legal_copy_rendered_count:
            evidence.filter((entry) => entry.role === "LEGAL").length,
          cta_copy_rendered_count:
            evidence.filter((entry) => entry.role === "CTA").length,
          ...
          segment_controls: plan.segments.map((segment) => ({
            ...
            text_sha256: segment.controls.titles.map((entry) => entry.text_sha256),
            exact_font_asset_ids: segment.controls.titles
              .map((entry) => entry.font_asset_id)
              .filter(Boolean),
