...
import { CreativePremiumLayerPassBridgeRuntime } from "@/lib/creative/multipass/runtime/CreativePremiumLayerPassBridgeRuntime";
...
export async function ensureShotEditRelease({organization_id,creative_project_id}={}){
  ...
  if(graph&&optical.length){
    ...
    let reconciled=CreativePremiumLayerPassBridgeRuntime.reconcile({graph,asset_nodes:nodes});
    reconciled=CreativeOpticalFinishPassBridgeRuntime.reconcile({graph:reconciled,asset_nodes:nodes});
