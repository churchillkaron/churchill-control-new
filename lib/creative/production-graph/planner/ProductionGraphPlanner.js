...
import { buildCinematicExecutionContract } from "../../video/runtime/CreativeCinematicExecutionContractRuntime.js";
...
function compactProductionRoomPipelineForGraph(pipeline = {}) {
...
function sceneIntent(scene = {}) {
  return {
    ...
    cinematic_intelligence: object(scene.cinematic_intelligence),
    audio_dramaturgy: object(scene.audio_dramaturgy),
...
const DEPARTMENT_STAGE_OUTPUTS = Object.freeze({
  previsualization_supervisor: ["approved camera blocking", "timing and geography proof", "shot feasibility notes"],
  production_designer: ["approved environment/material/prop state", "continuity-safe world bible"],
  director_of_photography: ["approved lens/camera/light execution", "exposure and movement intent"],
  cg_asset_supervisor: ["approved production assets", "identity/material/scale locks"],
  simulation_physics_supervisor: ["approved physical simulation", "motion/interaction plausibility evidence"],
  tracking_roto_supervisor: ["approved track/matchmove/roto state", "integration masks and spatial solve"],
  compositing_supervisor: ["approved integrated composite", "edge/light/contact/reflection continuity"],
  editor: ["approved editorial use", "cut motivation and neighboring-shot compatibility"],
  color_di_supervisor: ["approved DI/color state", "shot-to-shot color and density continuity"],
  sound_design_supervisor: ["approved source/SFX/Foley design", "picture-locked sync events"],
  mix_finishing_engineer: ["approved final mix/master", "loudness/dynamics/spatial delivery evidence"],
  post_production_supervisor: ["approved conform/online/finishing state", "all upstream handoffs reconciled"],
  quality_director: ["independent technical/perceptual/continuity QC", "release-blocking defects resolved"],
});

function departmentStageOutputs(roleId) {
  return list(DEPARTMENT_STAGE_OUTPUTS[roleId]).length
    ? list(DEPARTMENT_STAGE_OUTPUTS[roleId])
    : ["role-specific approved deliverable", "downstream handoff evidence"];
}
...
function shotRequirements(scene = {}, shot = {}, creativePlan = {}) {
  ...
  const cinematicExecution = buildCinematicExecutionContract({
    scene, shot, requirements: {
      scene_context: sceneIntent(scene), subject, purpose: shot.purpose || "", mood,
      action: shot.action || "", location, camera: shot.camera || {}, lighting: shot.lighting || {},
      production_design: shot.production_design || {}, frame_plan: framePlan,
      cinematic_dna: shot.cinematic_dna || {}, continuity_bible: scene.continuity_bible || {},
      cinematic_grammar: scene.cinematic_grammar || {}, beauty_system: scene.beauty_system || {},
      persistent_subject_lock: shot.persistent_subject_lock || scene.persistent_subject_lock || {},
      beauty_intent: shot.beauty_intent, focal_path: shot.focal_path, depth_layers: shot.depth_layers,
      reveal_stage: shot.reveal_stage, mystery_function: shot.mystery_function, inherited_world_state: shot.inherited_world_state,
    },
  });
  ...
  return {
    ...
    scene_cinematic_intelligence: object(scene.cinematic_intelligence),
    scene_audio_dramaturgy: object(scene.audio_dramaturgy),
    coverage: object(shot.coverage),
    coverage_role: shot.coverage_role || shot.coverage?.coverage_role || null,
    subject,
    ...
    environmental_continuity_state: object(shot.environmental_continuity_state),
    pursuit_spatial_choreography: object(shot.pursuit_spatial_choreography),
    pursuit_performance_choreography: object(shot.pursuit_performance_choreography),
    editorial_causality: object(shot.editorial_causality),
    continuity_bible: cinematicExecution.continuity_bible,
    cinematic_grammar: cinematicExecution.cinematic_grammar,
    beauty_system: cinematicExecution.beauty_system,
    beauty_intent: cinematicExecution.beauty_intent,
    focal_path: cinematicExecution.focal_path,
    depth_layers: cinematicExecution.depth_layers,
    persistent_subject_lock: cinematicExecution.persistent_subject_lock,
    reveal_stage: cinematicExecution.reveal_stage,
    mystery_function: cinematicExecution.mystery_function,
    inherited_world_state: cinematicExecution.inherited_world_state,
    ...
    cinematic_motion_design:
      Object.keys(object(shot.cinematic_motion_design)).length
        ? shot.cinematic_motion_design
        : {
            flagship: true,
            events: list(object(shot.graphics).cinematic_motion_events),
          },
    vfx: shot.vfx || [],
    simulation: shot.simulation || null,
    simulation_contract: object(shot.simulation_contract),
    generation_strategy: object(shot.generation_strategy),
    feasibility_forecast: object(shot.feasibility_forecast),
    signature_frame_design: object(shot.signature_frame_design),
    source_reinterpretation: object(shot.source_reinterpretation),
    scene_reconstruction_contract: object(shot.scene_reconstruction_contract),
    multipass_contract: object(shot.multipass_contract),
...
export function buildProductionGraph({
  ...
  const sealedProductionRoomIds = new Set(
    list(productionRoomPipeline.stages)
      .filter((stage) => stage?.status === "SEALED")
      .map((stage) => stage.id),
  );
  const activeProductionRoomStageInputs = Object.fromEntries(
    Object.entries(productionRoomStageInputs)
      .filter(([stageId]) => !sealedProductionRoomIds.has(stageId)),
  );
  const activeProductionRoomReports = Object.fromEntries(
    Object.entries(productionRoomInitialReports)
      .filter(([stageId]) => !sealedProductionRoomIds.has(stageId)),
  );
  const graphProductionRoomPipeline =
    compactProductionRoomPipelineForGraph(productionRoomPipeline);
  ...
  const graph = createProductionGraph({
    ...
    metadata: {
      ...
      taste_memory: object(creative_plan.taste_memory),
      finishing_intelligence: object(creative_plan.finishing_intelligence),
      temporal_consistency_intelligence: object(creative_plan.temporal_consistency_intelligence),
      ...
      production_room_pipeline: graphProductionRoomPipeline,
      production_room_stage_inputs: activeProductionRoomStageInputs,
      production_room_workstream_reports: activeProductionRoomReports,
      ...
      department_work_unit_count: Number(departmentBreakdown?.department_work_unit_count || 0),
      average_department_work_units_per_shot: Number(departmentBreakdown?.average_department_work_units_per_shot || 0),
      department_work_units: list(departmentBreakdown?.department_work_units),
    ...
    for (const shot of sceneShots) {
      ...
      const requirements = {
        ...
        production_room_pipeline: {
          contract: productionRoomPipeline.contract || null,
        },
        production_room_entry_gate: productionRoomGate,
        finishing_intelligence: object(creative_plan.finishing_intelligence),
        temporal_consistency_intelligence: object(creative_plan.temporal_consistency_intelligence),
      ...
      let previousDepartmentStageId = null;
      for (const stage of list(departmentHandoff?.stages)) {
        const roleId = text(stage.role_id);
        if (!roleId) continue;
        const stageNodeId = `department-stage:${shot.id}:${String(stage.index).padStart(2, "0")}:${roleId}`;
        graph.nodes.push(createProductionNode({
          id: stageNodeId,
          type: "DEPARTMENT_STAGE",
          title: `${shot.title || shot.id} · ${roleId}`,
          description: `Binding ${roleId} handoff for ${shot.id}; downstream work cannot treat this discipline as implicit review.`,
          requirements: {
            shot_id: shot.id,
            role_id: roleId,
            stage_index: Number(stage.index || 0),
            input_digest: stage.input_digest || null,
            stage_digest: stage.stage_digest || null,
            required_outputs: departmentStageOutputs(roleId),
            approval_required_before_downstream_handoff: true,
            independent_evidence_required: true,
            inherited_previsualization_digest: departmentHandoff.root_previsualization_digest || null,
          },
          metadata: {
            scene_id: scene.id,
            creative_mission_id,
            shot_id: shot.id,
            role_id: roleId,
            department_handoff_contract: departmentHandoff.contract || null,
            provider_execution_allowed: false,
        ...
        graph.edges.push(createProductionEdge({
          ...
          to: stageNodeId,
          type: "CONTAINS",
          metadata: { role_id: roleId, stage_index: Number(stage.index || 0) },
        }));
        if (previousDepartmentStageId) {
          graph.edges.push(createProductionEdge({
            from: previousDepartmentStageId,
            to: stageNodeId,
            type: "FOLLOWS",
            metadata: { approval_required_before_handoff: true },
          }));
          graph.edges.push(createProductionEdge({
            from: stageNodeId,
            to: previousDepartmentStageId,
            type: "DEPENDS_ON",
            metadata: { upstream_department_evidence_required: true },
          }));
        }
        previousDepartmentStageId = stageNodeId;
      }
  ...
  const departmentStageNodes = graph.nodes.filter((node) => node.type === "DEPARTMENT_STAGE");
  graph.metadata.department_stage_node_count = departmentStageNodes.length;
  graph.metadata.average_department_stages_per_shot = list(shots).length
    ? Number((departmentStageNodes.length / list(shots).length).toFixed(3))
    : 0;
  graph.metadata.specialist_stage_handoffs_enforced = departmentStageNodes.length > 0;
  graph.metadata.high_budget_production_graph_principle =
    "SHOT_IS_A_PRODUCTION_GRAPH_NOT_AN_INDEPENDENT_GENERATION";
  return graph;
}
