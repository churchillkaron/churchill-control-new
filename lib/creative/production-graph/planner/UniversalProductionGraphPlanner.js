...
function stillArtAuthority(plan = {}) {
  if (text(plan.workflow_kind).toUpperCase() !== "STILL") return null;
  const roles = object(plan.role_decisions);
  const art = object(plan.art_direction);
  for (const roleId of ["executive_creative_director", "brand_director", "art_director", "production_designer", "asset_intelligence_director"]) {
    if (text(roles[roleId]?.status).toUpperCase() !== "ACTIVE") {
      throw new Error(`STILL_ART_AUTHORITY_ROLE_REQUIRED:${roleId}`);
    }
  }
  if (!text(art.governing_visual_idea)) throw new Error("STILL_ART_DIRECTION_DOSSIER_REQUIRED");
  return Object.freeze({
    contract: "CREATIVE_STILL_ART_DIRECTION_AUTHORITY_V1",
    creative_master_plan_hash: digest(plan, "creative-master-plan"),
    art_direction_id: digest(art, "art-direction"),
    art_director_decision_id: digest(roles.art_director, "art-director-decision"),
    production_design_id: digest(roles.production_designer, "production-design"),
    brand_direction_id: digest(roles.brand_director, "brand-direction"),
    dossier: art,
    image_worker_authority: "EXECUTE_ONLY",
...
function addStepNode({
  ...
  const artAuthority = stillArtAuthority(creativePlan);
  const mergedOutputSpec = {
    ...object(deliverable?.output_spec),
    ...step.output_spec,
    ...(artAuthority ? { art_direction_authority: artAuthority } : {}),
  };
  ...
  const stillPrevisualization = workflowKind === "STILL" &&
    text(step.capability).startsWith("ai.image.")
    ? buildCreativeStillPrevisualizationBlueprint({
    ...
    : null;
  const stillProductionAuthority = stillPrevisualization
    ? buildCreativeStillProductionAuthority({
        previsualization: stillPrevisualization,
        requirements: baseRequirements,
  ...
  graph.nodes.push(createProductionNode({
    ...
    requirements: {
      ...
      creative_contract: CONTRACT,
      concept: object(creativePlan.concept),
      art_direction: object(creativePlan.art_direction),
      art_direction_authority: artAuthority,
      role_decisions: object(creativePlan.role_decisions),
      quality_policy: object(creativePlan.quality),
      channels: list(deliverable?.channels),
      languages: list(deliverable?.languages),
      output_spec: mergedOutputSpec,
      reference_assets: referenceAssets,
      ...(stillPrevisualization ? { still_previsualization_blueprint: stillPrevisualization } : {}),
      ...(stillProductionAuthority ? { still_production_authority: stillProductionAuthority } : {}),
    },
    assets: directAssets,
    generation: {
      required: true,
