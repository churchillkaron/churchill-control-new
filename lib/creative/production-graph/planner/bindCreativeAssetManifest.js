...
export function bindCreativeAssetManifest({
  ...
  const deliverables = list(creative_plan.deliverables);
  const deliverableIds = new Set(
    deliverables.map((item) => text(item.id)).filter(Boolean),
  );
  const productionStepIds = new Set(
    deliverables
      .flatMap((item) => list(item.production_steps))
  ...
  const semanticShotTargets = new Map();
  const terminalPlanScene = planScenes.at(-1) || null;
  const terminalPlanShot = list(terminalPlanScene?.shots).at(-1) || null;
  const terminalPersistedShotId = terminalPlanShot?.id
    ? shotTargets.get(text(terminalPlanShot.id)) || null
    : null;
  const finalRevealEvidence = [
    terminalPlanShot?.transition_out,
    terminalPlanShot?.purpose,
    terminalPlanShot?.action,
    terminalPlanScene?.transition_out,
    terminalPlanScene?.purpose,
    creative_plan?.concept?.payoff,
    creative_plan?.concept?.narrative,
  ]
    .map(text)
    .join(" ")
    .toLowerCase();
  if (terminalPersistedShotId && finalRevealEvidence.includes("reveal")) {
    semanticShotTargets.set("final_reveal", terminalPersistedShotId);
  }

  const allTargetIds = new Set([
    ...deliverableIds,
    ...productionStepIds,
    ...semanticShotTargets.keys(),
    ...sceneTargets.keys(),
    ...shotTargets.keys(),
  ]);
  ...
  function targetShotIds(assignment) {
    ...
    if (semanticShotTargets.has(assignment)) return [semanticShotTargets.get(assignment)];
  ...
  for (const entry of list(creative_plan.asset_manifest)) {
    ...
    const productionStepOnly = assignments.every((assignment) =>
      productionStepIds.has(assignment),
    );
    if (!boundShotIds.length && !productionStepOnly) {
  ...
  const boundShots = shots.map((shot) => {
    ...
    const needsPrimary = generatedShot(generation) && direct.length > 0;
  ...
  return {
    ...
    creative_plan: {
      ...
      asset_binding: {
        ...
        production_step_assignment_exposed_as_shot_source: false,
        guarded_semantic_shot_aliases: [...semanticShotTargets.keys()],
