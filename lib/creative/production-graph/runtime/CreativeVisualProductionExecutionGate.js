...
import {
  CreativeImageProductionProofManifestRuntime,
} from "@/lib/creative/image/runtime/CreativeImageProductionProofManifestRuntime";
...
function actorIdentityKeys(task = {}) {
  ...
  return [...new Set(
    list(requirements.actors)
      .map((actor) => actor && typeof actor === "object"
...
async function bindApprovedImageStudioAuthority(task = {}) {
  ...
  const hero = CreativeImageAssetHandoffRuntime.select({
    ...
    foundation_authority_digest:
      imageProductionPackage.foundation_authority_digest || null,
  ...
  const governedActorKeys = actorIdentityKeys(task);
  const governedIdentityKey = explicitIdentityKey ||
    (governedActorKeys.length === 1 ? governedActorKeys[0] : null);
  if (
    humanSubjectExpected(task) &&
    !governedIdentityKey &&
    governedActorKeys.length > 1
  ) {
    throw new Error("IMAGE_STUDIO_SUBJECT_IDENTITY_REQUIRED");
  }
  const governedThreatKey = threatSubjectExpected(task)
    ? threatIdentityKey(task)
    : null;
  ...
  const productionProof = await CreativeImageProductionProofManifestRuntime.ensure({
    task,
    asset_nodes: nodes,
    previsualization_authority: previsualizationAuthority,
    production_package: imageProductionPackage,
    shot_ready: imageShotReady,
    provider_references: providerReferences,
    camera_authority: imageCameraAuthority,
  });
  ...
  return ProductionTaskRuntime.update(task.id, {
    input: {
      ...
      image_production_proof: productionProof,
      generation: {
    ...
    metadata: {
      ...
      image_production_proof_contract: productionProof.contract,
      image_production_proof_hash: productionProof.proof_hash,
      image_production_proof_asset_node_id:
        productionProof.proof_asset_node_id,
      image_production_proof_bound: true,
    },
  });
}
