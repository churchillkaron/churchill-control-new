...
import {
  CreativeVisualConditioningAuthorityRuntime,
} from "@/lib/creative/video/runtime/CreativeVisualConditioningAuthorityRuntime";
...
function identityBoundHumanShot(shot = {}) {
  ...
  const references = list(
    ...
    shot.metadata?.identity_reference_asset_ids,
  );
  return Boolean(
    profileId ||
    references.length ||
    contract.identity_lock_required === true ||
    contract.identity_verification_required === true ||
...
function validateUniversalCreativeContract({ creative_plan = {}, shots = [] } = {}) {
  ...
  const challenger = temporal && object(plan.concept_council).contract === "CREATIVE_STUDIO_CHALLENGER_SELECTION_V1";
  ...
  const identityBoundHumanShots = list(shots).filter(identityBoundHumanShot);
  const missingIdentity = identityBoundHumanShots.filter((shot) => {
    const contract = object(shot.performance_contract || shot.metadata?.performance_contract);
    const requirements = object(shot.identity_requirements);
    return !list(
      contract.identity_reference_asset_ids ||
  ...
  const humanVideoShots = identityBoundHumanShots.filter(videoShot);
...
async function buildPlannedGraph(input = {}) {
  ...
  const conditionedGraph = CreativeVisualConditioningAuthorityRuntime.apply({
