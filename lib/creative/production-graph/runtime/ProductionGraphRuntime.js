...
import {
  CreativeImageAssetGraphRuntime,
} from "@/lib/creative/image/runtime/CreativeImageAssetGraphRuntime";
...
import {
  CreativeSceneReconstructionRuntime,
} from "@/lib/creative/reconstruction/runtime/CreativeSceneReconstructionRuntime";
import {
  CreativeMultiPassShotRuntime,
} from "@/lib/creative/multipass/runtime/CreativeMultiPassShotRuntime";
...
function compactProductionRoomPipelineForPersistence(pipeline = {}) {
  if (!pipeline?.contract) return pipeline;
  return {
    ...pipeline,
    stages: list(pipeline.stages).map((stage) => {
      if (stage?.status !== "SEALED") return stage;
      const report = object(stage.report);
      const evidence = object(stage.evidence);
      return {
        ...stage,
        report: undefined,
        evidence: undefined,
        sealed_evidence_summary: {
          report_contract: report.contract || evidence.report?.contract || evidence.room_report?.contract || null,
          passed:
            report.passed === true ||
            evidence.report?.passed === true ||
            evidence.room_report?.passed === true ||
            null,
          room: report.room || evidence.report?.room || evidence.room_report?.room || null,
          authority: report.authority || null,
          full_evidence_authority: "creative_preproduction_state",
        },
      };
    }),
  };
...
function hydrateAuthoritativePlanShots(shots = [], creativePlan = {}) {
  const authoredById = new Map();
  for (const scene of list(creativePlan.scenes)) {
    for (const authored of list(scene.shots)) {
      if (text(authored?.id)) authoredById.set(text(authored.id), authored);
    }
  }
  return list(shots).map((row) => {
    const authoredId = text(
      row?.metadata?.master_plan_shot_id ||
      row?.master_plan_shot_id ||
      row?.plan_shot_id,
    );
    const authored = authoredById.get(authoredId);
    ...
    return {
      ...
      virtual_camera_state: {
        ...object(row.virtual_camera_state),
        ...object(authored.virtual_camera_state),
      },
      subject_motion_choreography: {
        ...object(row.subject_motion_choreography),
        ...object(authored.subject_motion_choreography),
      },
      metadata: { ...object(authored.metadata), ...object(row.metadata) },
    };
  });
}
...
function namedHumanShot(shot = {}) {
  if (!humanShot(shot)) return false;
  const genericRole = /^(?:child|boy|girl|woman|man|person|people|human|observer|worker|staff|employee|manager|owner|founder|artist|performer|singer|actor|actress|model|dancer|customer|guest|driver|technician|pilot)$/i;
  return list(shot.actors).some((actor) => {
    ...
    const value = text(actor);
    return value.length > 1 && !genericRole.test(value);
  });
}
...
function validateRecoveredConceptCouncil(plan = {}) {
  const council = object(plan.concept_council);
  if (council.contract !== "CREATIVE_STORY_LINEAGE_RECOVERY_COUNCIL_V1") return false;
  const provenance = object(council.provenance);
  const selection = object(council.selection);
  const selectedConcept = object(selection.selected_concept);
  const selectedConceptId = text(
    plan.selected_concept_id ||
    selection.selected_concept_id ||
    selectedConcept.id,
  );
  if (
    provenance.recovered_lineage_authority !== true ||
    Number(provenance.source_round || 0) < 1 ||
    !list(provenance.source_usage_ids).length ||
    !text(provenance.completion_repair_usage_id)
  ) {
    throw new Error("RECOVERED_CREATIVE_COUNCIL_PROVENANCE_REQUIRED");
  }
  if (!selectedConceptId || text(selectedConcept.id) !== selectedConceptId) {
    throw new Error("RECOVERED_CREATIVE_SELECTED_CONCEPT_MISMATCH");
  }
  if (plan.creative_tribunal?.passed !== true || plan.creative_tribunal?.verdict?.passed !== true) {
    throw new Error("RECOVERED_CREATIVE_TRIBUNAL_APPROVAL_REQUIRED");
  }
  if (!text(council.council_hash) || !text(council.concept_hash)) {
    throw new Error("RECOVERED_CREATIVE_COUNCIL_HASHES_REQUIRED");
  }
  if (
    text(plan.production?.concept_council_hash) !== text(council.council_hash) ||
    text(plan.production?.selected_concept_hash) !== text(council.concept_hash)
  ) {
    throw new Error("RECOVERED_CREATIVE_COUNCIL_HASH_MISMATCH");
  }
  return true;
}

function validateChallengerSelection(plan = {}) {
  const council = object(plan.concept_council);
  const selection = object(council.selection);
  if (council.contract !== "CREATIVE_STUDIO_CHALLENGER_SELECTION_V1") return false;
  if (!text(selection.selected_concept_id) || selection.selected_concept_id !== plan.selected_concept_id) {
    throw new Error("CHALLENGER_SELECTED_CONCEPT_ID_MISMATCH");
  }
  if (!text(selection.selector_job_id) || !text(selection.selected_base_usage_id)) {
    throw new Error("CHALLENGER_SELECTION_LINEAGE_REQUIRED");
  }
  if (!Object.keys(object(selection.selected_concept)).length) {
    throw new Error("CHALLENGER_SELECTED_CONCEPT_REQUIRED");
  }
  if (!text(council.council_hash) || !text(council.concept_hash)) {
    throw new Error("CHALLENGER_SELECTION_HASHES_REQUIRED");
  }
  if (plan.creative_tribunal?.passed !== true) {
    throw new Error("CHALLENGER_TRIBUNAL_APPROVAL_REQUIRED");
  }
  if (text(plan.production?.concept_council_hash) !== text(council.council_hash) ||
      text(plan.production?.selected_concept_hash) !== text(council.concept_hash)) {
    throw new Error("CHALLENGER_SELECTION_HASH_MISMATCH");
  }
  return true;
}
...
function validateUniversalCreativeContract({ creative_plan = {}, shots = [] } = {}) {
  ...
  const temporal = workflowKind({ creative_plan: plan }) === "TEMPORAL";
  const councilContract = text(object(plan.concept_council).contract);
  const challenger = temporal && councilContract === "CREATIVE_STUDIO_CHALLENGER_SELECTION_V1";
  const recovered = temporal && councilContract === "CREATIVE_STORY_LINEAGE_RECOVERY_COUNCIL_V1";
  const legacyUniversal = list(plan.concept_candidates).length >= 3 &&
    Boolean(text(plan.selected_concept_id)) &&
    Boolean(object(plan.music_world));
  if (temporal && challenger) {
    validateChallengerSelection(plan);
  } else if (temporal && recovered) {
    validateRecoveredConceptCouncil(plan);
  } else if (temporal) {
    if (!legacyUniversal) throw new Error(`UNIVERSAL_CREATIVE_DOSSIER_REQUIRED_BEFORE_PRODUCTION:${text(plan.concept_council?.contract) || "no-contract"}:${text(plan.selected_concept_id) || "no-selected"}:${plan.creative_tribunal?.passed === true ? "tribunal-pass" : "tribunal-missing"}:${text(plan.production?.concept_council_hash) ? "council-hash" : "no-council-hash"}`);
...
function approvalMetadata(plan = {}) {
  const productionRoomBootstrap = object(plan.production_room_bootstrap);
  const productionRoomPipeline = compactProductionRoomPipelineForPersistence(
    plan.production_room_pipeline ||
    productionRoomBootstrap.production_room_pipeline ||
    null,
  );
  const sealedStageIds = new Set(
    list(productionRoomPipeline?.stages)
      .filter((stage) => stage?.status === "SEALED")
      .map((stage) => stage.id),
  );
  const stageInputs = object(
    productionRoomBootstrap.stage_inputs ||
    productionRoomBootstrap.production_room_stage_inputs,
  );
  const reportsByStage = object(productionRoomBootstrap.reports_by_stage);
  const activeStageInputs = Object.fromEntries(
    Object.entries(stageInputs).filter(([stageId]) => !sealedStageIds.has(stageId)),
  );
  const activeReportsByStage = Object.fromEntries(
    Object.entries(reportsByStage).filter(([stageId]) => !sealedStageIds.has(stageId)),
  );
  const {
  ...
  return {
    approval_plan_snapshot: approvalPlanSnapshot,
    approval_plan_hash: hash(plan),
    production_room_pipeline: productionRoomPipeline,
    production_room_stage_inputs: activeStageInputs,
    production_room_workstream_reports: activeReportsByStage,
    production_specialist_wave_audit:
      list(productionRoomBootstrap.specialist_wave_audit),
...
async function buildPlannedGraph(input = {}) {
  ...
  if (kind && kind !== "TEMPORAL") {
    let universalGraph = buildUniversalProductionGraph(input);
    if (kind === "STILL") {
      universalGraph = CreativeGeneratedMediaPerceptualGraphRuntime.apply({
        graph: universalGraph,
        shots: [],
      });
      universalGraph.metadata = {
        ...object(universalGraph.metadata),
        still_art_direction_review_required: true,
        still_art_direction_review_contract: "GENERATED_MEDIA_PERCEPTUAL_REVIEW_V1",
      };
    }
    universalGraph.metadata = {
      ...object(universalGraph.metadata),
  ...
  const reconstructed = CreativeSceneReconstructionRuntime.author({
    shots: input.shots,
    creative_plan: input.creative_plan,
  });
  const multipassAuthored = CreativeMultiPassShotRuntime.author({
    shots: reconstructed.shots,
    creative_plan: reconstructed.creative_plan,
  });
  const plannedInput = {
    ...input,
    shots: multipassAuthored.shots,
    creative_plan: multipassAuthored.creative_plan,
  };

  const authoredRuntimeShots = hydrateAuthoritativePlanShots(
    plannedInput.shots,
    plannedInput.creative_plan,
  );
  const bound = bindCreativeAssetManifest({
    scenes: plannedInput.scenes,
    shots: authoredRuntimeShots,
    creative_plan: plannedInput.creative_plan,
  });
  ...
  const baseGraphRaw = buildProductionGraph({
    ...plannedInput,
    ...performanceBound,
  });
  const baseGraph = CreativeMultiPassShotRuntime.applyToGraph(
    baseGraphRaw,
    performanceBound.shots,
  );
  baseGraph.metadata = {
    ...object(baseGraph.metadata),
    ...object(reconstructed.metadata),
    ...object(multipassAuthored.metadata),
    scene_reconstruction_contract: CreativeSceneReconstructionRuntime.contract,
    multipass_contract: CreativeMultiPassShotRuntime.contract,
  };
  ...
  const imageAssetGraph = CreativeImageAssetGraphRuntime.apply({
    graph: visualGraph,
    shots: performanceBound.shots,
    scenes: plannedInput.scenes,
  });
  ...
  finalGraph.metadata = {
    ...
    ...approvalMetadata(plannedInput.creative_plan),
    planning_mode: "PURE_IN_MEMORY_BEFORE_MATERIALIZATION",
  };
...
export const ProductionGraphRuntime = {
  ...
  async listSummaries(input = {}) {
    return Repository.listSummariesByProject(input);
  },
  ...
  async executeProductionSpecialistWave(id, {
    ...
    for (let attempt = 1; attempt <= 4; attempt += 1) {
      const latestMetadata = object(latest.metadata);
      const latestRoomPlan = latestMetadata.production_room_pipeline;
      if (!latestRoomPlan?.contract) throw new Error("PRODUCTION_ROOM_PIPELINE_REQUIRED");
      merged = mergeSpecialistWaveState({
        production_room_pipeline: latestRoomPlan,
        reports_by_stage: object(latestMetadata.production_room_workstream_reports),
        prior_audit: latestMetadata.production_specialist_wave_audit,
        wave_result: wave,
      });
      try {
        graph = await Repository.updateIfUnchanged(id, {
          metadata: {
            ...latestMetadata,
        ...
        if (
          String(error?.message || error) !== "PRODUCTION_GRAPH_CONCURRENT_UPDATE_CONFLICT" ||
          attempt === 4
        ) throw error;
        latest = await Repository.getById(id);
        if (!latest) throw new Error("PRODUCTION_GRAPH_NOT_FOUND");
      }
    }
