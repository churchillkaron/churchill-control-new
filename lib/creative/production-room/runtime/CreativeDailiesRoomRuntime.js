...
export function evaluateDailiesTake({ take = {}, reviews = [], floor = CREATIVE_DAILIES_RELEASE_FLOOR } = {}) {
  ...
  for (const row of rows) {
    ...
    if (text(row.family).toUpperCase() === "INTENT_FIDELITY") {
      const comparison = row.comparison && typeof row.comparison === "object" ? row.comparison : {};
      const required = ["frame_design", "dp_intent", "physical_world", "visual_journey", "benchmark_craft"];
      for (const dimension of required) {
        const dimensionScore = number(comparison[dimension]?.score ?? comparison[dimension]);
        const dimensionPassed = comparison[dimension]?.passed ?? (dimensionScore !== null && dimensionScore >= floor);
        if (dimensionScore === null) failures.push(`DAILIES_INTENT_COMPARISON_REQUIRED:${dimension}`);
        else if (dimensionScore < floor || dimensionPassed !== true) failures.push(`DAILIES_INTENT_COMPARISON_REJECTED:${dimension}`);
      }
      if (row.rendered_against_authored_contracts !== true) failures.push("DAILIES_RENDERED_VS_AUTHORED_PROOF_REQUIRED");
    }
  }
  const weakest = rows.length ? Math.min(...rows.map((row) => number(row.score) ?? 0)) : 0;
  const weakestRows = rows.filter((row) => (number(row.score) ?? 0) === weakest);
  const rejectedRows = rows.filter((row) => row.passed !== true || (number(row.score) ?? 0) < floor);
  const learningSignal = {
    contract: "CREATIVE_DAILIES_LEARNING_SIGNAL_V1",
    rejected_families: rejectedRows.map((row) => text(row.family).toUpperCase()).filter(Boolean),
    weakest_families: weakestRows.map((row) => text(row.family).toUpperCase()).filter(Boolean),
    weakest_score: weakest,
    failure_codes: [...new Set(rejectedRows.flatMap((row) => list(row.failure_codes).map(text)).filter(Boolean))],
    repair_focus: [...new Set(rejectedRows.flatMap((row) => list(row.repair_focus).map(text)).filter(Boolean))],
    evidence_only: true,
    advisory_only: true,
  };
...
export async function recordDailiesAssetDecision({ organization_id, asset_node_id, reviews = [], floor = CREATIVE_DAILIES_RELEASE_FLOOR } = {}) {
  ...
  const AssetGraphRepository = await import("../../assets/graph/repositories/CreativeAssetGraphRepository.js");
  ...
  return AssetGraphRepository.update(asset_node_id, {
    metadata: {
      ...
      dailies_learning_signal: report.learning_signal,
