...
export function evaluateConceptCompetition({ council = {} } = {}) {
  const recoveryProvenance = council.provenance || {};
  const recoveryCouncil =
    ...
    text(recoveryProvenance.completion_repair_usage_id) &&
    list(recoveryProvenance.source_usage_ids).length > 0 &&
    Number(recoveryProvenance.source_round || 0) > 0 &&
    list(council.concepts).length >= 1 &&
    text(council.selection?.selected_concept?.id);
  if (recoveryCouncil) {
    return Object.freeze({
      contract: CREATIVE_FRONT_PRODUCTION_ROOMS_CONTRACT,
      ...
      completion_repair_usage_id: recoveryProvenance.completion_repair_usage_id,
    });
  }
