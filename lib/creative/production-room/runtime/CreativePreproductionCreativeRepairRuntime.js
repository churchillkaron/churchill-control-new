...
export async function repairCreativeFloorPlan({
  ...
  let result;
  try {
    result = await executeApprovedPreproductionReasoning({
      organization_id,
      creative_project_id,
    ...
    if (text(error?.message).includes("CREATIVE_PREPRODUCTION_SPECIALIST_BUDGET_APPROVAL_REQUIRED")) {
      error.creative_floor_failures = effectiveFailures;
      error.benchmark_after = recovered?.after || before;
      error.message =
        `${text(error.message)}:CREATIVE_FLOOR_FAILURES=${effectiveFailures.join(",") || "UNKNOWN"}`;
    }
    throw error;
  }
