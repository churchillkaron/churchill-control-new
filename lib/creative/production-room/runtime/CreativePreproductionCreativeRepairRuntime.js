...
function normalizeFailureKeyedRepairPatch(value = {}) {
  const source = object(value.repair_patch || value.creative_floor || value);
  ...
  if (
    ...
    return {
      ...
      signature_images: list(source.signature_images),
  ...
  const generic = object(source.GENERIC_TECH_REVEAL);
  const sound = object(source.SOUND_WORLD_TOO_FLAT);
...
function normalizedBenchmarkLab(value = {}) {
  ...
  const studies = referenceAssets.slice(0, 3).map((entry, index) => ({
    title:
      text(object(lab[`study_${index + 1}`]).title) ||
    ...
    analysis: {
      ...analysis,
      ...object(object(lab[`study_${index + 1}`]).analysis),
    },
  }));
  return {
    ...lab,
    ...
    studies,
    craft_dna: Object.keys(object(lab.craft_dna)).length
