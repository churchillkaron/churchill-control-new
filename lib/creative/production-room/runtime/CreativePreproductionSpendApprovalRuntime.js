...
export async function executeApprovedPreproductionReasoning({
  ...
  return enqueue(creative_project_id, async () => {
    ...
    const usage = estimatedUsage(execution_input.input || {});
    ...
    if (Number(pricing.customer_price) > maximumForCall) {
      throw new Error(`CREATIVE_PREPRODUCTION_SPECIALIST_ESTIMATE_EXCEEDS_APPROVED_CALL:${pricing.customer_price}:${maximumForCall}`);
    }
