...
import { getProviderPricingById } from "@/lib/platform/service-runtime/pricing/repositories/ProviderPricingRepository";
...
function benchmarkLocalApproval(approval = {}) {
  return Boolean(
    ...
    approval.studio_preproduction_review === true
  );
}

async function approvedPricing(approval, usage) {
  if (!benchmarkLocalApproval(approval)) {
...
function preproductionRequestFingerprint({
  operation,
  execution_input = {},
  outputTokens = null,
  provider = null,
  ...
  return createHash("sha256")
    .update(JSON.stringify({
      ...
      service_id: text(execution_input.service_id || "ai.reasoning.execute"),
      provider: text(provider),
      model: text(model),
      capability: text(capability),
      execution_lane: text(
        execution_input.input?.execution_lane ||
        execution_input.input?.executionLane,
      ).toLowerCase(),
      prompt: text(execution_input.input?.prompt),
      response_format: object(execution_input.input?.response_format),
      max_output_tokens: Number(outputTokens || execution_input.input?.max_output_tokens || 0),
      metadata_operation: text(execution_input.metadata?.operation).toUpperCase(),
    }))
    .digest("hex")
    .slice(0, 24);
}
...
async function boundedUsage({ approval, requestedUsage, maximumForCall }) {
  ...
  const priceFor = (usage) => approvedPricing(approval, usage);
...
async function settlePreproductionResult(result = {}, executionRuntime, governedInput = {}) {
  if (result?.pending !== true) return result;
  if (!executionRuntime?.settle) throw new Error("CREATIVE_PREPRODUCTION_SPECIALIST_SETTLEMENT_RUNTIME_REQUIRED");
  const provider = text(result.provider);
  const providerJobId = text(result.provider_job_id || result.provider_request_id);
  const usageId = text(result.usage?.id);
  if (!provider || !providerJobId || !usageId) {
    throw new Error("CREATIVE_PREPRODUCTION_SPECIALIST_PENDING_IDENTITY_REQUIRED");
  }
  let current = result;
  for (let attempt = 0; attempt < 120; attempt += 1) {
    await new Promise((resolve) => setTimeout(resolve, 2000));
    current = await executionRuntime.settle({
      ...
      provider_status_input: { model: result.model || null },
    });
    if (current?.failed === true) {
      throw new Error(`CREATIVE_PREPRODUCTION_SPECIALIST_PROVIDER_FAILED:${text(current.error) || text(current.provider_status) || "UNKNOWN"}`);
    }
    if (current?.pending !== true) {
      return {
        ...
        model:
          ...
          current.settled_pricing?.model ||
          current.usage?.provider_model ||
          result.model ||
          result.pricing?.model ||
          result.reservation_pricing?.model ||
          null,
        pricing: current.pricing || current.settled_pricing || result.pricing || result.reservation_pricing || null,
        reservation_pricing: current.reservation_pricing || result.reservation_pricing || null,
      };
    }
  }
  throw new Error(`CREATIVE_PREPRODUCTION_SPECIALIST_PROVIDER_TIMEOUT:${provider}:${providerJobId}`);
}
...
export async function executeApprovedPreproductionReasoning({
  ...
  return enqueue(creative_project_id, async () => {
    ...
    let result;
    try {
      const requestFingerprint = preproductionRequestFingerprint({
        operation,
        execution_input,
      ...
      const governedInput = {
        ...
        input: {
          ...
          execution_idempotency_key:
            `creative-preproduction:${creative_project_id}:${text(operation).toUpperCase()}:${requestFingerprint}`,
        ...
        cost_guard: {
          ...
          reference: `${approval.id}:${Number(approval.call_count || 0) + 1}:${text(operation).toUpperCase()}:${requestFingerprint}`,
        },
        metadata: {
      ...
      result = await execution_runtime.execute(governedInput);
      result = await settlePreproductionResult(result, execution_runtime, governedInput);
    } catch (error) {
      await persist(project, approval, {
        approved: true,
        status: "APPROVED",
