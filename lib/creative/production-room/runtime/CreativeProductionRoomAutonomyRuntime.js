...
export function decideProductionRoomAutonomy({
  ...
  if (
    room.stage_id === "DAILIES" &&
    (
      !list(stageInput.department_reviews).length ||
      !list(stageInput.approved_takes).length
    )
  ) {
    return Object.freeze({
      contract: CREATIVE_PRODUCTION_ROOM_AUTONOMY_CONTRACT,
      action: "AWAIT_MEDIA_GENERATION",
      stage_id: room.stage_id,
      blockers: [
        ...(!list(stageInput.department_reviews).length ? ["DAILIES_DEPARTMENT_REVIEWS_REQUIRED"] : []),
        ...(!list(stageInput.approved_takes).length ? ["DAILIES_APPROVED_TAKES_REQUIRED"] : []),
      ],
      office,
      zero_media_generation: true,
    });
