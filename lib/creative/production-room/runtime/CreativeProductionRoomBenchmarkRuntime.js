...
export function evaluateProductionRoomBenchmark() {
  ...
  if (CREATIVE_PRODUCTION_ROOM_STAGES.length !== 17) failures.push("PRODUCTION_ROOM_SEVENTEEN_STAGES_REQUIRED");
  ...
  const orchestratedReportRooms = new Set(["TRIBUNAL", "MASTERING"]);
  for (const stage of CREATIVE_PRODUCTION_ROOM_STAGES) {
    if (!orchestratedReportRooms.has(stage.id) && !specialistsForStage(stage.id).length) {
      failures.push(`PRODUCTION_ROOM_STAGE_UNSTAFFED:${stage.id}`);
    }
  }
