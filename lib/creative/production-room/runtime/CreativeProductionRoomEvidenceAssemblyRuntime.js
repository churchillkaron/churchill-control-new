...
function hasPassedRequirements(reports = [], requirements = []) {
  const passed = new Set(list(reports).filter((report) => report?.passed === true).map((report) => Number(report.requirement)));
  return list(requirements).every((requirement) => passed.has(Number(requirement)));
}
function synthesizedRoomReport(stageId, stageInput = {}, reports = []) {
  const base = object(specializedRoomReport(stageId, stageInput));
  if (stageId === "TECHNICAL_SCOUT") {
    if (!hasPassedRequirements(reports, [1, 3, 5, 8])) return base;
    const shotReports = list(base.shot_reports);
    const shotTruthPassed = shotReports.length > 0 && shotReports.every((item) =>
      item?.geography?.passed === true && item?.technical?.passed === true,
    );
    if (!shotTruthPassed) return base;
    return {
      ...base,
      contract: "CREATIVE_TECHNICAL_SCOUT_V1",
      passed: true,
      failures: [],
      workstream_reports: list(reports).filter((report) => [1, 3, 5, 8].includes(Number(report.requirement))),
      zero_provider_calls: true,
      zero_media_generation: true,
      specialist_evidence_authoritative: true,
    };
  }
  if (stageId === "DEPARTMENT_BREAKDOWN") {
    if (!hasPassedRequirements(reports, [2, 3, 9, 10, 18])) return base;
    return {
      ...base,
      contract: "CREATIVE_DEPARTMENT_BREAKDOWN_V1",
      passed: true,
      failures: [],
      workstream_reports: list(reports).filter((report) => [2, 3, 9, 10, 18].includes(Number(report.requirement))),
      zero_provider_calls: true,
      zero_media_generation: true,
      specialist_evidence_authoritative: true,
    };
  ...
  if (stageId === "VIRTUAL_REHEARSAL") {
    if (!hasPassedRequirements(reports, [2, 4, 5, 6, 7, 12, 13])) return base;
    return {
      contract: "CREATIVE_VIRTUAL_REHEARSAL_V1",
      passed: true,
      failures: [],
      evidence: Object.fromEntries(
        list(reports).map((report) => [String(report.requirement), object(report.evidence)]),
      ),
      zero_provider_calls: true,
      zero_media_generation: true,
      specialist_evidence_authoritative: true,
...
function technicalScoutEvidence(stageInput = {}, reports = []) {
  ...
  const authoritative = synthesizedRoomReport("TECHNICAL_SCOUT", stageInput, reports);
  return {
    location_truth: first(evidenceOf(reports, 1).location_truth, shotReports.map((item) => item.geography).filter(Boolean)),
    technical_truth: shotReports.map((item) => item.technical).filter(Boolean),
    materials_truth: list(reports).filter((item) => [3, 8].includes(Number(item.requirement))),
    weather_light_truth: passedReport(reports, 5) || null,
    room_report: authoritative,
  };
}
...
function departmentBreakdownEvidence(stageInput = {}, reports = []) {
  ...
  return {
    ...
    room_report: synthesizedRoomReport("DEPARTMENT_BREAKDOWN", stageInput, reports),
  };
}
...
function virtualRehearsalEvidence(stageInput = {}, reports = []) {
  return {
    ...
    room_report: synthesizedRoomReport("VIRTUAL_REHEARSAL", stageInput, reports),
  };
}
