...
function assertTrustedUpstreamHandoff(stageId, report = {}) {
  ...
  if (["CREATIVE_FLOOR", "CONCEPT_COMPETITION"].includes(stageId)) {
    if (report.contract !== CREATIVE_FRONT_PRODUCTION_ROOMS_CONTRACT || report.room !== stageId) {
      throw new Error(`CREATIVE_${stageId}_REPORT_REQUIRED`);
    }
    if (report.passed !== true) {
      const failures = list(report.failures).map((failure) => text(failure)).filter(Boolean);
      throw new Error(`CREATIVE_${stageId}_REPORT_FAILED:${failures.length ? failures.join(",") : "UNKNOWN"}`);
    }
    return;
  }
