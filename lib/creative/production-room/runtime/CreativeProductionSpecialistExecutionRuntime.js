...
function normalizeWorkstreamEvidence(work_order = {}, evidence = {}) {
  const normalized = { ...object(evidence) };
  if (Number(work_order.requirement) === 6 && (!Array.isArray(normalized.edit_points) || normalized.edit_points.length === 0)) {
    const timing = Array.isArray(normalized.timing_map) ? normalized.timing_map : [];
    const derived = timing.slice(0, -1).map((item, index) => ({
      at_seconds: Number(item?.end_seconds),
      purpose: `Contingency editorial recovery point ${index + 1} derived from approved choreography timing boundary`,
      preserve_primary_continuous_take: true,
    })).filter((item) => Number.isFinite(item.at_seconds) && item.at_seconds > 0);
    if (derived.length) normalized.edit_points = derived;
  }
  return normalized;
}
...
async function recoverPendingWorkOrder({ organization_id, creative_project_id, work_order }) {
  ...
  const parsedEvidence = parseOutput(settled) || parseUsageEvidence(usage);
  if (!parsedEvidence) return null;
  const evidence = normalizeWorkstreamEvidence(work_order, parsedEvidence);
...
async function recoverSettledWorkOrder({ organization_id, creative_project_id, work_order }) {
  ...
  if (!match) return null;
  const parsedEvidence = parseUsageEvidence(match);
  if (!parsedEvidence) return null;
  const evidence = normalizeWorkstreamEvidence(work_order, parsedEvidence);
  const completion = completeProductionWorkOrder({ work_order, evidence });
  if (completion.passed !== true) return null;
...
function executionPrompt({ work_order, production_context }) {
  return JSON.stringify({
    contract: CREATIVE_PRODUCTION_SPECIALIST_EXECUTION_CONTRACT,
    instruction:
      "Act only as the assigned film-production specialist workstream. Return one JSON object containing every required output key with substantive evidence. Placeholder values such as evidence_missing, unknown, not provided, none, n/a or equivalent are invalid. Use the supplied approved research, approved story, approved concept and approved scenes as authority. Do not invent external sources, factual claims, real-world findings, identities, rights, locations, capabilities or research evidence. For creative production-design workstreams, you ARE explicitly authorized to author concrete design decisions that realize the approved story and scenes: sets, dressing, props, wardrobe, surfaces, lighting, materials, camera, action, effects, editorial, sound and continuity rules may be designed as creative decisions when they do not contradict approved evidence. Clearly treat those as production decisions rather than researched facts. For research/scouting, source_manifest must contain at least two distinct approved evidence references when available. Do not create media, call media providers, alter governance or approve release. If factual evidence is genuinely insufficient, preserve the uncertainty without replacing required creative-design outputs with placeholders.",
    work_order: {
      stage_id: work_order.stage_id,
      requirement: work_order.requirement,
      workstream_id: work_order.workstream_id,
