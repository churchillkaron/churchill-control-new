...
function deferredProductionDaylightFinding() {
  return Object.freeze({
    status: "DEFERRED_UNTIL_PRODUCTION_LOCATION",
    basis: "No production location and date are yet bound at Research Room stage. Weather, daylight, sun path and exposure become factual production evidence at Technical Scout once the shot location and timing are selected.",
    authority: "NO_WEATHER_FACTS_INVENTED",
  });
}
...
export function buildResearchRoomReport({ research = {}, universal_asset_intelligence = {}, brief = {} } = {}) {
  ...
  const evidence = {
    ...
    weather_daylight_findings: nonempty(
      ...
      deferredProductionDaylightFinding(),
    ),
