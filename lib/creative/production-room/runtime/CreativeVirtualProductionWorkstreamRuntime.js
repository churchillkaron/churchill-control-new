...
function placeholderEvidence(value) {
  ...
  return /^(evidence missing|missing|unknown|n\/?a|not provided|not available|insufficient evidence|none)$/.test(normalized);
}
function hasEvidence(value) {
  if (Array.isArray(value)) return value.some((entry) => hasEvidence(entry));
  if (value && typeof value === "object") {
    const entries = Object.values(value);
    return entries.length > 0 && entries.some((entry) => hasEvidence(entry));
  }
  return text(value).length > 0 && !placeholderEvidence(value);
}
