...
const HASH_ALGORITHMS = {
  LEGACY_JSON: "SHA256_JSON_STRINGIFY_V1",
  STABLE_JSON: "SHA256_STABLE_JSON_V1",
};
...
function validateCandidate({
  ...
  const legacyCalculatedHash = jsonHash(story);
  const canonicalStoryHash = stableHash(story);
  const normalizationHash = text(
    normalization.approval_candidate_hash,
  );
  const declaredHash = text(
    approval_candidate_hash || normalizationHash,
  );
  ...
  if (!declaredHash || !normalizationHash) {
    throw runtimeError(
  ...
  if (declaredHash !== normalizationHash) {
    throw runtimeError(
      "CREATIVE_APPROVAL_CANDIDATE_DECLARED_HASH_MISMATCH",
      {
  ...
  const matchesLegacy = declaredHash === legacyCalculatedHash;
  const matchesCanonical = declaredHash === canonicalStoryHash;
  ...
  if (!matchesLegacy && !matchesCanonical) {
    throw runtimeError(
      "CREATIVE_APPROVAL_CANDIDATE_HASH_MISMATCH",
      {
        declared_hash: declaredHash,
        normalization_hash: normalizationHash,
        legacy_calculated_hash: legacyCalculatedHash,
  ...
  return {
    candidate,
    story,
    approval_candidate_hash: declaredHash,
    approval_hash_algorithm: matchesCanonical
      ? HASH_ALGORITHMS.STABLE_JSON
      : HASH_ALGORITHMS.LEGACY_JSON,
    canonical_story_hash: canonicalStoryHash,
  };
}
  ...
  async issue({
    ...
    const authorizationPayload = {
      ...
      approval_hash_algorithm:
        validated.approval_hash_algorithm,
      canonical_story_hash:
        validated.canonical_story_hash,
      proof_shot_key: key,
      scene_number: selected.scene_number,
      shot_number: selected.shot_number,
      shot_hash: stableHash(selected.shot),
      reference_asset_ids: references,
      authorization_scope: scope,
      issued_at: issuedAt,
    };
    const authorizationHash = stableHash(authorizationPayload);
