...
const POLICY_VERSION =
  "CREATIVE_EVIDENCE_REQUIREMENT_POLICY_V3_EXPLICIT_BINDING";
...
export function buildCreativeEvidenceRoleManifest({
  ...
  const authorizedAssets = list(assets).filter((asset) =>
    assetAliases(asset).some((assetId) => authorizedIds.has(assetId)),
  );
  ...
  const manifest = buildV1Manifest({
    scene: sanitized.scene,
    shot: sanitized.shot,
  ...
  const generated = new Set(sanitized.generated_roles);
  const bindings = list(manifest.bindings).filter((binding) =>
    !generated.has(normalizeRole(binding.role)),
  );
  const blockers = list(manifest.blockers).filter((blocker) =>
    ![...generated].some((role) =>
      String(blocker).includes(`ROLE_${role}_`),
    ),
  );
  const requiredRoles = list(manifest.required_roles).filter((role) =>
    !generated.has(normalizeRole(role)),
  );

  return {
    ...manifest,
    version: "CREATIVE_EVIDENCE_ROLE_CONTRACT_V3_EXPLICIT_BINDING",
    policy_version: POLICY_VERSION,
