...
function resolveCasting(specification = {}, task = {}) {
  ...
  const sources = [
    {
      name: "TASK_PREPARED_CASTING",
      value: preparedCasting,
    },
    {
      name: "SHOT_CASTING",
      value: shotCasting,
  ...
  const actorValues = firstCastingActors(
    preparedCasting,
    shotCasting,
    sceneCasting,
    { actors: shot.actors },
  ...
  const actors = actorValues.map(normalizeActor);
  ...
  return {
    ...
    prepared_contract_present:
      meaningfulCastingSource(preparedCasting),
    actors,
...
function resolveCompositionPlan(specification = {}, task = {}) {
  ...
  const protectedRegions = list(
    plan.protected_regions || plan.protectedRegions,
  );
...
export function buildCreativeGenerationEvidenceContract({
  ...
  const contract = {
    version: "creative-generation-evidence-v2",
    service_id,
