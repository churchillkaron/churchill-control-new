...
function plannedCost(tasks = []) {
  return tasks.reduce((sum, candidate) => {
    const quote = object(candidate.metadata?.pricing_quote);
    if (quote.production_routing_allowed === false) return sum;
    return sum + Math.max(0, finite(candidate.cost?.estimated) || 0);
  }, 0);
}
...
function supersessionEvidence(task = {}, taskMap = new Map()) {
  ...
  if (replacement) {
    ...
    const taskIsPriorRepair = [
      REPLACEMENT_SOURCE_CONTRACT,
      REPLACEMENT_REVIEW_CONTRACT,
    ].includes(text(task.metadata?.repair_payload_contract));
    const chainedRepair =
      taskIsPriorRepair &&
      originalAttempt !== null &&
      originalAttempt > 0 &&
      replacementAttempt === originalAttempt + 1;
...
async function approvedSealedDossier(task, graph) {
  ...
  const approvedCurrency = currency(graphApproval.currency);
  const taskApprovalCurrency = currency(taskApproval.currency);
  const costPlan = object(graph.cost_plan);
  const costPlanCurrency = currency(costPlan.currency);
  const taskCurrency = currency(task.cost?.currency);
  if (!approvedCurrency) {
    throw new Error("SEALED_PRODUCTION_APPROVAL_CURRENCY_REQUIRED");
  }
  for (const [label, candidate] of [
    ["TASK_APPROVAL", taskApprovalCurrency],
    ["GRAPH_COST_PLAN", costPlanCurrency],
    ["TASK_COST", taskCurrency],
  ]) {
    if (candidate && candidate !== approvedCurrency) {
      throw new Error(`SEALED_PRODUCTION_${label}_CURRENCY_MISMATCH`);
    }
  }
  ...
  const guardCurrency = currency(guard.currency);
  if (
    taskMaximum === null ||
    taskMaximum <= 0 ||
    taskMaximum > ceiling + 0.000001 ||
    !guardCurrency ||
    guardCurrency !== approvedCurrency ||
...
function install() {
  ...
  ProductionTaskRuntime.dispatch = async function dispatchWithProductionDossierGate(id) {
    ...
    const updated = await ProductionTaskRuntime.updateAuthorizationEvidence(task.id, {
      cost: {
        ...(task.cost || {}),
        approved: true,
      },
      metadata: {
        ...
        approved_cost_currency: evidence.currency || task.cost?.currency || null,
