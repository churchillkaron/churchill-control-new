...
function stripProviderTransport(value) {
  if (Array.isArray(value)) {
    return value.map(stripProviderTransport);
  }
  if (!value || typeof value !== "object") return value;

  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => !PROVIDER_TRANSPORT_KEYS.has(key))
      .filter(([, nested]) => nested !== undefined)
      .map(([key, nested]) => [key, stripProviderTransport(nested)]),
  );
}
...
function planSnapshot(graph = {}) {
  ...
  return stripProviderTransport(snapshot);
}
...
function graphSnapshot(graph = {}) {
  return {
    ...
    cost_plan: stripProviderTransport(graph.cost_plan),
    production_plan: stripProviderTransport(graph.production_plan),
    nodes: list(graph.nodes).map((node) => ({
      ...
      intent: stripProviderTransport(node.intent),
      requirements: stripProviderTransport(node.requirements),
      assets: node.assets,
      generation: stripProviderTransport(node.generation),
      metadata: stripProviderTransport(node.metadata),
    })),
    edges: list(graph.edges).map((edge) => ({
      ...
      to: edge.to,
      type: edge.type,
      metadata: stripProviderTransport(edge.metadata),
    })),
    metadata: {
...
function executionSnapshot(executionPlan = {}) {
  return {
    ...
    steps: list(executionPlan.steps).map((step) => ({
      ...
      input: stripProviderTransport(step.input),
      metadata: stripProviderTransport(step.metadata),
    })),
  };
}
...
function generationSummary(executionPlan = {}) {
  ...
  for (const step of list(executionPlan.steps)) {
    ...
    const provider = text(
      step.metadata?.pricing_quote?.provider ||
      step.input?.generation?.provider ||
      step.metadata?.generation?.provider ||
      step.input?.provider_id ||
      "AUTO",
    );
    const quote = object(step.metadata?.pricing_quote);
    const productionRoutable = quote.production_routing_allowed !== false;
    const cost = productionRoutable
      ? Math.max(0, finite(step.estimated_cost) || 0)
      : 0;
    estimatedCost += cost;
    byCapability[capability] = (byCapability[capability] || 0) + 1;
    byProvider[provider] = (byProvider[provider] || 0) + 1;
    if (productionRoutable && (cost > 0 || capability.startsWith("ai."))) paidOrExternalTaskCount += 1;
  }
...
function generationDirectionPresent(node = {}) {
  if (node.generation?.required !== true) return true;

  const capability = text(node.generation?.capability);
  if (!capability.startsWith("ai.")) return true;

  return Boolean(
    Object.keys(object(node.generation?.output_spec)).length ||
    Object.keys(object(node.intent)).length ||
    Object.keys(object(node.requirements)).length ||
...
function sceneDossier(graph = {}) {
  ...
  return nodes
    ...
    .map((scene) => ({
      ...
      intent: stripProviderTransport(scene.intent),
      requirements: stripProviderTransport(scene.requirements),
      shots: list(children.get(scene.id)).map((id) => nodeMap.get(id)).filter(Boolean)
        .map((shot) => ({
          ...
          generation: {
            ...
            direction_source: shot.generation?.direction_source || null,
            output_spec: stripProviderTransport(shot.generation?.output_spec || {}),
          },
        })),
    }));
}
...
function approvalChecklist({ plan, graph, executionPlan, summary }) {
  const failures = [];
  const temporal = text(plan.workflow_kind || graph.metadata?.workflow_kind).toUpperCase() === "TEMPORAL";
  if (temporal && !text(plan.selected_concept_id)) failures.push("SELECTED_CONCEPT_REQUIRED");
  if (temporal && !text(plan.concept_council?.council_hash || plan.production?.concept_council_hash)) {
    failures.push("CONCEPT_COUNCIL_HASH_REQUIRED");
  }
  if (!temporal && !text(plan.concept?.title)) failures.push("CONCEPT_TITLE_REQUIRED");
  ...
  if (summary.paid_or_external_task_count > 0 && !text(
    plan.production?.currency || graph.cost_plan?.currency || executionPlan.currency,
  )) {
  ...
  for (const node of list(graph.nodes).filter(
    (candidate) => candidate.generation?.required === true,
  )) {
    if (!generationDirectionPresent(node)) {
      failures.push(`STRUCTURED_GENERATION_DIRECTION_REQUIRED:${node.id}`);
    }
  }
  ...
  return {
    ...
    checks: {
      ...
      structured_generation_direction_present: list(graph.nodes)
        .filter((node) => node.generation?.required === true)
        .every(generationDirectionPresent),
      provider_transport_persisted: false,
    },
  };
}
...
function dossierDocument({ graph, executionPlan }) {
  ...
  const content = {
    ...
    story_architecture: stripProviderTransport(plan.story_architecture || {}),
    measured_audio: {
      evidence_hash:
    ...
    deliverables: stripProviderTransport(plan.deliverables || []),
  ...
  async materialize({
    ...
    if (!node) {
      ...
      node = await AssetGraphRepository.create(createCreativeAssetNode({
        ...
        description: "Immutable zero-generation production dossier containing the approved concept, evidence, scenes, shots, structured direction, references, reuse decisions, generation count and cost ceiling request.",
        lineage: {
