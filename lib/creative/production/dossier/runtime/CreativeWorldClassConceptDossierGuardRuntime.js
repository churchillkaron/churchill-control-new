...
function assertWorldClassConcept(productionGraph = {}) {
  ...
  const sealedAuthority = object(plan.sealed_preproduction_authority);
  const sealedAuthorityAccepted =
    sealedAuthority.contract === "CREATIVE_SEALED_PREPRODUCTION_AUTHORITY_V1" &&
    sealedAuthority.passed === true &&
    sealedAuthority.resumed_from_approved_council === true &&
    sealedAuthority.technical_scout_ready === true &&
    Array.isArray(sealedAuthority.sealed_upstream_stage_ids) &&
    ["RESEARCH_ROOM", "CREATIVE_FLOOR", "CONCEPT_COMPETITION", "TRIBUNAL"].every((stageId) =>
      sealedAuthority.sealed_upstream_stage_ids.includes(stageId),
    ) &&
    Number(sealedAuthority.tribunal_weighted_score) >= 90;

  if (sealedAuthorityAccepted) {
