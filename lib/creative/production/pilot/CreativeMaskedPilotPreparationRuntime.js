...
function safePlan(input = {}) {
  ...
  if (!ALLOWED_MODES.has(mode)) {
    throw new Error("CREATIVE_COMPOSITION_MODE_INVALID");
  }
  ...
  const isMasked = mode !== "FULL_SCENE_REFERENCE_SYNTHESIS";

  if (isMasked && !placementRegions.length) {
    throw new Error("CREATIVE_MASK_PLACEMENT_REGION_REQUIRED");
  }
  if (isMasked && !protectedRegions.length) {
    throw new Error("CREATIVE_PROTECTED_BRAND_REGION_REQUIRED");
  }

  const castingMode = castingActors.some(
  ...
  return {
    ...
    composition_plan: {
      ...
      brand_mode: isMasked
        ? "SOURCE_PIXELS_ONLY"
        : "REFERENCE_GROUNDED_WITH_POST_OVERLAY",
      ...
      exact_pixels_outside_mask_required: isMasked,
      full_scene_regeneration_required: !isMasked,
      generated_text_allowed: false,
      logo_redraw_allowed: false,
      exact_brand_overlay_required: !isMasked,
    },
  };
}
...
export const CreativeMaskedPilotPreparationRuntime = {
  async prepare({
    ...
    if (
      ...
      throw new Error(
        "CREATIVE_COMPOSITION_PLAN_CANNOT_REWRITE_SPENT_OR_COMPLETED_TASK",
      );
    }
    ...
    const fullScene =
      prepared.composition_plan.mode === "FULL_SCENE_REFERENCE_SYNTHESIS";
    const nextInput = {
      ...(task.input || {}),
      mode: fullScene
        ...
        shot: {
          ...
          reference_pack: {
            ...
            preserve: [
              ...new Set([
                ...
                "Recognizable real venue geometry, proportions, materials and spatial relationships",
                "Exact referenced staff identities, wardrobe characteristics and human proportions",
                "Exact approved logo, wordmark, signage spelling, geometry, color and placement through source preservation or post overlay",
              ]),
            ],
            may_change: fullScene
              ? [
                  "Camera viewpoint and lens while preserving spatial truth",
                  "Lighting, atmosphere and cinematic finishing",
                  "Human pose, expression and blocking required by the director",
                  "Background detail only when supported by references and physically plausible",
                ]
              : [
            ...
            never_change: [
              ...new Set([
                ...
                "Core venue architecture and recognizable entrance geometry",
                "Logo, wordmark, signage text, spelling, geometry, color or placement",
                "Declared staff count, wardrobe role and greeting action",
              ]),
            ],
          },
          negative_constraints: [
            ...new Set([
              ...
              "No rectangular pasted figures or collage-like compositing",
              "No cropped bodies, floating feet, missing heads or disconnected shadows",
              "No generated logo, wordmark, signage, label, lettering or text",
              "No generic replacement venue",
              "No undeclared people",
              "No artificial AI-film look, waxy skin, synthetic lighting or game-render appearance",
            ]),
          ],
        },
      },
    ...
    const updated = await ProductionTaskRuntime.update(
      task.id,
      {
        ...
        metadata: {
          ...(task.metadata || {}),
          attempt: 0,
          provider_status: "COMPOSITION_PLAN_READY",
          preflight_blocked: false,
          preflight_code: null,
          masked_composition_prepared: !fullScene,
          full_scene_synthesis_prepared: fullScene,
    ...
    return {
      ...
      next_gate: "MASTER_STILL_READY_FOR_EXPLICIT_GENERATION",
    };
  },
};
