...
function isPreflightBlocked(task = {}) {
  return Boolean(
    task.metadata?.preflight_blocked === true ||
    task.metadata?.provider_status === "PREFLIGHT_BLOCKED" ||
    task.error === PREFLIGHT_BLOCKED ||
    task.metadata?.structured_failure?.message === PREFLIGHT_BLOCKED,
  );
}

function preflightCode(task = {}) {
  return (
...
async function normalizePreflightBlockedTask(task = {}) {
  if (!task?.id || !isPreflightBlocked(task)) {
    return task;
  }
  ...
  return ProductionTaskRuntime.update(
    task.id,
    {
      ...
      metadata: {
        ...
        attempt: Math.max(
          0,
          Number(task.metadata?.attempt || 0) - 1,
        ),
        provider_status: "PREFLIGHT_BLOCKED",
        preflight_blocked: true,
        preflight_code: preflightCode(task),
        preflight_blocked_at:
          task.metadata?.preflight_blocked_at ||
...
async function materializeStep({
  ...
  if (existing?.status === "FAILED") {
    return isPreflightBlocked(existing)
      ? normalizePreflightBlockedTask(existing)
      : existing;
  }
...
async function resetPreflightBlockedTaskForRetry(
  task = {},
  {
    authorized = false,
    organization_id,
  ...
  const spendStarted =
    ...
    Boolean(task.output?.provider_submission) ||
  ...
  if (spendStarted) {
    const error = new Error(
      "PREFLIGHT_RETRY_FORBIDDEN_AFTER_SPEND_STARTED",
    );
  ...
  return ProductionTaskRuntime.update(
    task.id,
    {
      ...
      metadata: {
        ...(task.metadata || {}),
        attempt: 0,
        provider_status: "PREFLIGHT_RETRY_READY",
        preflight_blocked: false,
        preflight_code: null,
        preflight_retry_authorized: true,
        preflight_retry_authorized_at:
          new Date().toISOString(),
        provider_dispatched: false,
        wallet_reserved: false,
        usage_created: false,
...
function summarizeTask(task = {}) {
  return {
    ...
    preflight_blocked: isPreflightBlocked(task),
    preflight_code: preflightCode(task),
    cost: taskCost(task),
  };
}
...
export const CreativeMasterStillPilotRuntime = {
  async run({
    ...
    masterTask =
      await resetPreflightBlockedTaskForRetry(
        masterTask,
        {
          authorized:
            retry_preflight_blocked === true,
          organization_id,
          creative_project_id,
        },
      );
    ...
    masterTask = await normalizePreflightBlockedTask(masterTask);
    ...
    const blocked = isPreflightBlocked(masterTask);
    const passed =
      !blocked &&
    ...
    return {
      ...
      preflight: {
        blocked,
        code: preflightCode(masterTask),
        retry_authorized:
          retry_preflight_blocked === true,
        provider_dispatched:
          masterTask.metadata?.provider_dispatched === true,
        usage_created:
          masterTask.metadata?.usage_created === true,
        wallet_reserved:
          masterTask.metadata?.wallet_reserved === true,
        wallet_charged:
          masterTask.metadata?.wallet_charged === true,
      },
      video_tasks_materialized: 0,
      video_tasks_dispatched: 0,
      production_dispatched: !blocked,
      next_gate: blocked
