...
function dependencyFailed(taskMap, dependencyId) {
  const status = taskMap.get(dependencyId)?.status;
...
async function dispatchCreativeTask(task) {
  ...
  if (localWebsiteOperation(task)) return dispatchWebsiteTask(task);
  if (localSoftwareOperation(task)) return dispatchSoftwareTask(task);
  if (localAudioOperation(task)) return dispatchAudioTask(task);
...
export const ProductionQueueRuntime = {
  async build({ organization_id, creative_project_id }) {
    ...
    for (const task of tasks) {
      ...
      } else if ((task.status === "WAITING" || task.status === "PLANNED") && canRun) {
