...
async function dispatchCreativeTask(task) {
  ...
  if (localWebsiteOperation(task)) {
    return dispatchWebsiteTask(task);
  }

  if (localSoftwareOperation(task)) {
    return dispatchSoftwareTask(task);
  }
...
export const ProductionQueueRuntime = {
  ...
  async dispatchAll(input, {
    ...
    while (passes < maxPasses && dispatched.length < maxTasks) {
      ...
      const stillValidations = await StillValidationTaskRuntime.ensure(input);
      const websiteValidations = await WebsiteValidationTaskRuntime.ensure(input);
      const softwareValidations = await SoftwareValidationTaskRuntime.ensure(input);
      if (
        stillValidations.length ||
        websiteValidations.length ||
        softwareValidations.length
      ) progressed = true;
  ...
  async dispatchNext(input) {
    ...
    if (isWebsiteQualityTask(next)) {
      next = await bindWebsiteScreenshotForReview(next);
    }
    if (isSoftwareQualityTask(next)) {
      next = await bindSoftwareEvidenceForReview(next);
    }
