...
function infrastructureErrorCode(error) {
  return String(
    error?.code ||
    error?.cause?.code ||
    error?.details?.code ||
...
function retryableInfrastructureError(error) {
  const code = infrastructureErrorCode(error);
  const message = infrastructureErrorMessage(error).toLowerCase();
  return [
    "57014",
    "ETIMEDOUT",
    "ECONNRESET",
...
function pollingIncident(task, error) {
  return {
    task_id: task.id,
    ...
    error: infrastructureErrorMessage(error),
    retryable: true,
...
function scopedInput(input = {}) {
  ...
  return {
    ...
    production_graph_id: input.production_graph_id || null,
  };
}

export const ProductionQueueRuntime = {
  async build(input = {}) {
    const scope = scopedInput(input);
    const tasks = await ProductionTaskRuntime.list(scope);
    if (
      scope.production_graph_id &&
      tasks.some((task) => task.production_graph_id !== scope.production_graph_id)
    ) {
      throw new Error("CREATIVE_PRODUCTION_QUEUE_GRAPH_SCOPE_VIOLATION");
    }
  ...
  async pollRunning(input, { maxTasks = 100 } = {}) {
    ...
    for (const task of pending) {
      try {
        ...
        if (!retryableInfrastructureError(error)) throw error;
        const incident = pollingIncident(task, error);
        incidents.push(incident);
        console.warn(
          `PROVIDER_POLL_TRANSIENT_INFRASTRUCTURE_ERROR=` +
          `${incident.task_id}|${incident.provider || "NONE"}|` +
          `${incident.provider_job_id || "NONE"}|${incident.code}|${incident.error}`,
        );
      }
    }
  ...
  async dispatchAll(input, {
    ...
    while (passes < maxPasses && dispatched.length < maxTasks) {
      ...
      const repair = await CreativeAutonomousRepairDirectorRuntime.ensure(scope);
      ...
      const validations = await Promise.all([
        StillValidationTaskRuntime.ensure(scope),
        WebsiteValidationTaskRuntime.ensure(scope),
        SoftwareValidationTaskRuntime.ensure(scope),
        AudioValidationTaskRuntime.ensure(scope),
        CampaignValidationTaskRuntime.ensure(scope),
      ]);
      if (validations.some((items) => items.length)) progressed = true;

      if (pollRunning) {
        const pollResult = await this.pollRunning(scope, {
        ...
        if (pollResult.incident_total) {
          pollIncidents.push(...pollResult.incidents);
          progressed = true;
        }
      }

      const next = await this.dispatchNext(scope);
      if (next) {
    ...
    const queue = await this.build(scope);
  ...
  async dispatchNext(input) {
    ...
    if (isCampaignCoherenceTask(next) && !next.metadata?.campaign_package_task_id) {
      ...
      const current = await this.build(scope);
