...
async function dispatchCreativeTask(task) {
  ...
  if (capability === "creative.document.render") {
    try {
      const output = await CreativeDocumentProductionRuntime.render(task);
  ...
  if (capability === "creative.document.validate") {
    try {
      const output = await CreativeDocumentProductionRuntime.validate(task);
      return ProductionTaskRuntime.complete(task.id, {
        provider: "avantiqo-local-document-worker",
        settlement: "LOCAL_EXECUTION",
        output,
