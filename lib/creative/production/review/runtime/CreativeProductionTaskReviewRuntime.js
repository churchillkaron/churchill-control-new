...
function v2VocalReviewReady(task = {}) {
  const automatedPassed =
    task.metadata?.automated_vocal_performance_validation_passed === true &&
    CreativeProductionTaskReviewSettlementGate.lipSyncReviewPassed(task);
  const humanOnlyReady =
    task.metadata?.human_only_validation_required === true &&
    task.metadata?.trusted_automated_validation_unavailable === true &&
    CreativeProductionTaskReviewSettlementGate.trustedAutomationUnavailable(task);
  return automatedPassed || humanOnlyReady;
}
...
const REVIEW_CONTRACTS = Object.freeze({
  IDENTITY_KEYFRAME_REVIEW_V1: {
    ...
    validate: (task) =>
      task.metadata?.automated_identity_keyframe_validation_passed === true &&
      CreativeProductionTaskReviewSettlementGate.identityReviewPassed(task),
  },
...
function approvalIdentity(task = {}, approver = {}) {
  return crypto.createHash("sha256").update(JSON.stringify({
    ...
    automated_validation_passed:
      task.metadata?.automated_identity_keyframe_validation_passed ||
      task.metadata?.automated_lipsync_validation_passed ||
      task.metadata?.automated_vocal_performance_validation_passed ||
      null,
    human_only_validation_required:
      task.metadata?.human_only_validation_required === true,
...
export const CreativeProductionTaskReviewRuntime = {
  async approve({
    ...
    if (!policy.validate(task)) {
      throw new Error(`PRODUCTION_REVIEW_VALIDATION_NOT_READY:${contract}`);
    }
    ...
    return {
      ...
      reused: false,
      approval: {
        contract: "AUTHENTICATED_PRODUCTION_TASK_REVIEW_APPROVAL_V2",
