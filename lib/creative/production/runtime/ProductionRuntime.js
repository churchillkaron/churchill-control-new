...
function productionState({ queue, coverageBefore, readiness, polled = 0, mode = "PRODUCTION" }) {
  const coverage = inspectCreativeCoverageQueue(queue);
  const assetsCreated = queue.completed.filter((task) =>
    Boolean(task.output?.asset_node_id || task.output?.asset_id),
  ).length;
  return {
    success: queue.failed.length === 0 && queue.blocked.length === 0,
    complete: false,
    status: queue.running.length ? "PROVIDER_JOBS_RUNNING" : "PRODUCTION_IN_PROGRESS",
    mode,
    dispatched: 0,
    polled,
    passes: 0,
...
export const ProductionRuntime = {
  ...
  async inspectCoverage({ organization_id, creative_project_id, production_graph_id = null }) {
    ...
    return {
      ...
      video_readiness: await CreativeVideoProductionReadinessRuntime.inspect({ queue }),
    };
  },

  async pollProduction({ organization_id, creative_project_id, production_graph_id = null }) {
    if (!organization_id) throw new Error("organization_id required");
    if (!creative_project_id) throw new Error("creative_project_id required");
    ...
    const poll = await ProductionQueueRuntime.pollRunning(input);
    const after = poll.total > 0
      ? await ProductionQueueRuntime.build(input)
      : before;
    const videoReadiness = await CreativeVideoProductionReadinessRuntime.inspect({
      queue: after,
    });
  ...
  async runProduction({ organization_id, creative_project_id, production_graph_id = null }) {
    if (!organization_id) throw new Error("organization_id required");
    if (!creative_project_id) throw new Error("creative_project_id required");
    ...
    let before = await ProductionQueueRuntime.build(input);
    const coverageBefore = inspectCreativeCoverageQueue(before);
    const preproductionCertification = await CreativePreProductionIntelligenceCertificationRuntime.inspect(input);

    if (preproductionCertification.required && !preproductionCertification.passed) {
      const error = new Error("CREATIVE_PREPRODUCTION_INTELLIGENCE_NOT_CERTIFIED");
      error.status = 409;
      error.preproduction_certification = preproductionCertification;
      throw error;
    }
    ...
    let videoReadiness = await CreativeVideoProductionReadinessRuntime.inspect({
      queue: before,
    });

    if (videoReadiness.running_task_count > 0) {
      const poll = await ProductionQueueRuntime.pollRunning(input);
      before = await ProductionQueueRuntime.build(input);
      videoReadiness = await CreativeVideoProductionReadinessRuntime.inspect({
        queue: before,
      });
    ...
    if (videoReadiness.required && !videoReadiness.ready) {
      const error = new Error(
        videoReadiness.status === "BUSY"
          ? "CREATIVE_VIDEO_RUNTIME_BUSY"
          : "CREATIVE_VIDEO_RUNTIME_NOT_READY",
      );
      error.status = 409;
      error.readiness = videoReadiness;
      throw error;
    }
    ...
    return {
      ...
      preproduction_certification: preproductionCertification,
      assets_created: assetsCreated,
      finalisation,
