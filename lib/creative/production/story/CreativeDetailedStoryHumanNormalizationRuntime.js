...
const RUNTIME_VERSION =
  "CREATIVE_DETAILED_STORY_HUMAN_NORMALIZATION_V2";
...
function applyDirectives(story, directives) {
  ...
  for (const directive of directives) {
    ...
    const matchedForbidden = directive.forbidden_phrases.filter(
      (phrase) => replacementText.includes(phrase.toLowerCase()),
    );
  ...
  async run({
    ...
    return {
      ...
      human_normalization: {
        ...
        forbidden_phrase_scope: "replacement_fields_only",
