...
const TEMPORAL_PRODUCTION_TYPES = new Set([
  PRODUCTION_TYPES.VIDEO,
  PRODUCTION_TYPES.AUDIO,
]);
...
function normalizeProductionType(value) {
  const normalized = String(value || PRODUCTION_TYPES.VIDEO)
    .trim()
    .toUpperCase();

  return Object.values(PRODUCTION_TYPES).includes(normalized)
    ? normalized
    : PRODUCTION_TYPES.VIDEO;
}
...
function normalizeTargetDuration({
  ...
  if (!TEMPORAL_PRODUCTION_TYPES.has(productionType)) {
    return null;
  }
