...
function candidatePolicy(task = {}) {
  return task.input?.requirements?.generation_strategy?.candidate_policy ||
    task.input?.generation_strategy?.candidate_policy ||
    task.metadata?.generation_strategy?.candidate_policy ||
    {};
}
function paidCandidateCount(nodes = [], shotId = null) {
  if (!shotId) return 0;
...
export const CreativeAutonomousRecoveryOrchestratorRuntime = Object.freeze({
  ...
  async analyze({ organization_id, creative_project_id } = {}) {
    ...
    for (const task of tasks.filter(active)) {
      ...
      const policy = candidatePolicy(task);
      const maximumPaidCandidates = finite(policy.maximum_paid_candidates);
      const existingPaidCandidates = paidCandidateCount(nodes, shotId);
      if (maximumPaidCandidates !== null && existingPaidCandidates >= maximumPaidCandidates && !winner) {
        decisions.push({
          task_id: task.id,
          shot_id: shotId || null,
          action: "BLOCK",
          reason: "GENERATION_CANDIDATE_BUDGET_EXHAUSTED",
          existing_paid_candidates: existingPaidCandidates,
          maximum_paid_candidates: maximumPaidCandidates,
      ...
      if (instructions.length) {
        decisions.push({
          ...
          new_paid_candidate_authorized: false,
        });
        continue;
      }
      decisions.push({
        ...
        quality_floor_preserved: true,
        candidate_policy: policy,
        alternate_generation_requires_failed_review: policy.alternate_generation_requires_failed_review === true,
        new_paid_candidate_authorized: true,
      });
    }
    return {
      contract: CONTRACT,
