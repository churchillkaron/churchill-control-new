...
function visualRepairInstruction(value) {
  const row = text(value).toLowerCase();
  return /^(subject|action|opening frame|opening state|progression|closing frame|closing state|camera|lighting):/.test(row);
}

function effectiveRepairInstructions(source = {}, instructions = []) {
  const current = list(instructions).map(text).filter(Boolean);
  if (current.some(visualRepairInstruction)) return current;
  const inherited = list(source.input?.repair_specification?.required_repairs)
    .map(text)
    .filter(Boolean);
  if (!inherited.some(visualRepairInstruction)) return current;
  return [...new Set([...inherited, ...current])];
}
...
function completedQualityFailure(task = {}) {
  return ["COMPLETED", "FAILED"].includes(task.status) &&
    isQualityTask(task) &&
    !qualityPassed(task.output) &&
    (
      list(task.output?.perceptual_validation?.evidence?.repair_instructions).length > 0 ||
      list(task.output?.raw?.output?.result?.repair_instructions).length > 0 ||
      list(task.output?.raw?.result?.repair_instructions).length > 0 ||
      list(task.output?.output?.raw?.output?.result?.repair_instructions).length > 0 ||
      list(task.output?.output?.raw?.result?.repair_instructions).length > 0 ||
      list(task.output?.provider_poll?.output?.result?.repair_instructions).length > 0
    );
}
...
function repairedMetadata(source, { attempt, identity, qualityTask = null, failures, instructions }) {
  ...
  return {
    ...
    promptless_repair: true,
    structured_repair_source_of_truth: true,
  };
}
...
async function createReplacement({ source, qualityTask = null, failures, instructions, policy }) {
  ...
  const effectiveInstructions = effectiveRepairInstructions(source, instructions);
  ...
  const replacement = await ProductionTaskRuntime.create({
    ...
    input: {
      ...
      repair_specification: {
        contract: "CREATIVE_STRUCTURED_AUTONOMOUS_REPAIR_V1",
        promptless_source_of_truth: true,
        attempt,
        failures,
        required_repairs: effectiveInstructions,
        preserve: {
    ...
    cost: {
      ...(source.cost || {}),
      actual: 0,
      approved: source.cost?.approved === true || Number(source.cost?.estimated || 0) <= 0,
    },
...
async function createRepairReview({ quality, replacement, failures, instructions, policy }) {
  ...
  const review = await ProductionTaskRuntime.create({
    ...
    cost: {
      ...(quality.cost || {}),
      actual: 0,
      approved: quality.cost?.approved === true || Number(quality.cost?.estimated || 0) <= 0,
    },
  ...
  async ensure({ organization_id, creative_project_id } = {}) {
    ...
    for (const task of tasks) {
      ...
      const instructions = quality
        ...
        : [
            `Correct this failed production stage: ${text(source.error) || "execution failed"}.`,
    ...
    return { created, blocked, policy, enabled: true };
  },
};
