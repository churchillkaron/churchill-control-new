...
function paidRepairAllowed(source, project) {
  const estimated = Math.max(0, finite(source.cost?.estimated, 0));
  if (estimated <= 0) return { allowed: true, estimated, allowance: repairAllowance(project) };
  const allowance = repairAllowance(project);
  return { allowed: allowance >= estimated, estimated, allowance };
}
...
export const CreativeAutonomousRepairDirectorRuntime = {
  async ensure({ organization_id, creative_project_id, production_graph_id = null } = {}) {
    ...
    for (const task of tasks) {
      ...
      const spending = paidRepairAllowed(source, project);
      if (!spending.allowed) {
        blocked.push({
          task_id: task.id,
          source_task_id: source.id,
