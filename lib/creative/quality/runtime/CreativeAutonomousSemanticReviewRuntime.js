import { CreativeCinematicReferenceGrammarRuntime } from "./CreativeCinematicReferenceGrammarRuntime";
...
function reviewPrompt({ policy, render, context }) {
  ...
  return [
    ...
    `Cinematic reference grammar: ${JSON.stringify(CreativeCinematicReferenceGrammarRuntime.snapshot())}`,
    "Judge against those abstract craft principles only; never reward imitation of a reference brand or copied shot sequence.",
    ...
    "Reject metronomic pacing: repeated equal-length shots, repeated camera behaviour, repeated visual scale or constant intensity that makes the film feel like an AI montage rather than authored cinema.",
    "Reject decorative face holds. A close human face must carry a visible emotional or causal state change; otherwise the edit should prefer action, hands, movement, objects, geography or interaction.",
    "For premium temporal work, music, SFX, ambience and silence must react to exact picture events and energy changes. Constant-bed scoring or generic unsynchronised effects are release-blocking when structured audio evidence says picture-locked direction was required.",
    "Canonical brand lockups must remain visually exclusive and exact. Reject extra generated globes, symbols, duplicate marks, pseudo-logo geometry or decorative structures inside or immediately behind a canonical logo unless explicitly required by the approved brand system.",
