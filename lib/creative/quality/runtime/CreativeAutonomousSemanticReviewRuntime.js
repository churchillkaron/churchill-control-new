...
function reviewPrompt({ policy, render, context }) {
  ...
  return [
    ...
    "IMPORTANT AUDIO EVIDENCE RULE: the supplied contact sheet has no audible signal. For music_and_sound_design and mix_hierarchy_and_silence, do not claim that you heard or acoustically inspected the film. Use only the supplied structured final_audio evidence: authenticated human approval of the master soundtrack plus measured pre/post-finishing PCM integrity. If that evidence is missing or contradictory, FAIL the relevant audio check. Do not infer subjective acoustic qualities from images.",
