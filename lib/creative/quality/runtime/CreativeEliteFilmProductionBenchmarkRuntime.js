...
function executionChecks(plan = {}) {
  ...
  const checks = {
    ...
    signature_frame_design: shots.length > 0 && shots.every((shot) => {
      const design = object(shot.signature_frame_design);
      return design.required === true &&
        text(design.hero_frame).length >= 36 &&
        text(design.graphic_silhouette).length >= 28 &&
        text(design.foreground_midground_background).length >= 28 &&
        text(design.material_light_event).length >= 28 &&
        text(design.controlled_palette).length >= 24 &&
        text(design.natural_irregularity).length >= 28 &&
        text(design.atmosphere_physics).length >= 28 &&
        text(design.optical_character).length >= 28 &&
        text(design.performance_microtruth).length >= 28 &&
        text(design.fear_or_desire_focus).length >= 24 &&
        text(design.anti_game_camera_rule).length >= 28 &&
        text(design.board_comparison_test).length >= 28;
    }),
