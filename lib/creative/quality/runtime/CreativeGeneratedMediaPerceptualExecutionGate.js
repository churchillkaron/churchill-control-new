...
const CORE_SCORE_FIELDS = Object.freeze(new Set([
  "overall_score", "story_score", "environment_score", "camera_score",
  "anatomy_score", "identity_score", "product_fidelity_score", "music_energy_score",
  "performance_score", "continuity_score", "physics_score", "artifact_score",
]));
...
const SCORE_FIELDS = Object.freeze({
  ...
  artifact_score: ["artifact", "artifacts"],
...
function outputUrl(output = {}) {
  ...
  for (const value of candidates) {
    const reference = value.asset_url ||
      value.assetUrl ||
...
function normalizeReviewEvidence(value = {}) {
  ...
  for (const [canonicalKey, aliasKeys] of Object.entries(SCORE_FIELDS)) {
    ...
    if (candidates.length === 0) {
      if (CORE_SCORE_FIELDS.has(canonicalKey)) missingFields.push(canonicalKey);
      continue;
    }
...

{
  ...
  "scores": {
    ...
    "artifact_score": 0,
    "composition_score": 0,
    "aesthetic_distinction_score": 0,
    "tension_curiosity_score": 0,
    "production_value_score": 0,
    "depth_atmosphere_score": 0,
    "environmental_aliveness_score": 0
  },
...
FAIL-CLOSED RULES
- Treat the expected cinematic_minimum contract as a HARD admission gate.
- Calibrate craft against Lamborghini Revuelto “From Now On” principles only; imitation is forbidden.
- Beauty without physical truth is failure.
...
- Apply the CONSUMER-AI DIFFERENTIATION TEST. When source media is supplied, ask whether a competent user could plausibly obtain substantially the same result from an ordinary one-click image-to-video generator using that source plus a short prompt. If yes, FAIL the shot. Premium Avantiqo work must add authored cinematography, physically integrated VFX/CGI or production design, meaningful spatial/temporal transformation, material realism, sound-picture events, compositing or another clearly observable layer of professional film craft beyond simple source animation.
- Source-conditioned media must preserve source truth while visibly increasing production value. Reject generic parallax, slow zoom, camera drift, face animation, stock fog/particles, arbitrary neon/glow, simple depth-map motion or cosmetic relighting when those are the primary transformation.
- When signature_frame_design is present, find the strongest actual frame and compare it directly against hero_frame, graphic_silhouette, depth layering, material_light_event, controlled_palette, natural_irregularity, atmosphere_physics, optical_character, performance_microtruth, emotional focal detail, anti-game-camera rule and board_comparison_test. If no actual frame could sit beside the approved visual-world signature board without obvious quality or authorship collapse, FAIL the shot.
- Explicitly inspect repeating geometry, evenly spaced trees/objects, cloned foliage, procedural roots, plastic bark, uniform rain, detached volumetric beams, perfect grime distribution, over-smoothed skin and symmetric environment layout. These are generative/CG tells and are release-blocking unless the approved design intentionally requires them.
- Light must interact with matter. Rain, mist, wet bark, fabric, skin, glass, metal and ground must show distance-dependent scattering, occlusion, reflection, absorption and contact appropriate to the shot; decorative light cones pasted into empty air fail the premium material-light gate.
- When the shot contract includes source_reinterpretation, verify its production_value_transformation, new_viewpoint_logic, vfx_cgi_integration and consumer_ai_failure_test against the actual rendered media. A beautiful result that ignores that authored reinterpretation is a FAIL.
- Reject centered third-person chase framing, body-height follow cameras and videogame-avatar blocking unless the exact approved anti_game_camera_rule explains an authored visual reason and the rendered frame visibly earns it.
- When pursuit_spatial_choreography is present, verify the rendered predator-target-camera triangle, line of action, target/threat headings, proximity, occlusion, search/attack vector, obstacle clearance and entry/exit handoff. Reject threat teleportation, impossible proximity jumps, accidental axis reversals, decorative symmetric pursuers, search beams that ignore the authored threat position, and cuts that make the audience lose causal geography without an explicitly motivated axis break and recovery cue.
- When pursuit_performance_choreography is present, verify the visible stimulus -> involuntary reaction -> deliberate decision -> physical action -> changed state chain. Reject generic running loops, stock fear faces, frozen panic expressions, repeated identical glances, unmotivated head turns, breath that ignores exertion, footwork that ignores terrain, obstacle contact with no body consequence, fatigue that resets, mud/wetness/injury that disappears, or reaction inserts that add no new threat information or emotional state.
- Performance microtruth must be visible in the framing that claims it. A face shot must reveal specific changed eye/breath/jaw/head behavior; a feet/hands shot must reveal pressure, terrain contact, protection, injury, hesitation or survival mechanics. Do not award performance quality for written intent that is not visible in the actual render.
- Reject technically correct but visually cheap, flat, generic or emotionally inert work. Major-brand work must have deliberate visual hierarchy, aesthetic distinction, premium production value, dimensional depth/atmosphere, and environmental life where the story expects a living world.
- Tension/curiosity must be created through reveal, withholding, contrast, blocking, sound-picture implication or unresolved visual pressure when the shot purpose calls for it; generic prettiness is not tension.
...
if (!ProductionTaskRuntime[FLAG]) {
  ...
  ProductionTaskRuntime.poll = async function pollWithGeneratedMediaPerceptualGate(id) {
    ...
    const contract = text(before.metadata?.contract);
