...
function validation(task = {}) {
  ...
  const booleans = [
    ...
    evidence.requested_camera_correct !== false,
    evidence.story_contribution_present !== false,
    evidence.anatomy_valid !== false,
    evidence.physics_valid !== false,
    evidence.continuity_valid !== false,
    ...
    expected.person_expected ? evidence.person_count_correct !== false : true,
    expected.identity_expected ? evidence.identity_preserved === true : true,
    expected.product_expected ? evidence.product_preserved === true : true,
    expected.music_expected ? evidence.music_energy_translated !== false : true,
  ];
  return {
    passed: booleans.every(Boolean) && Object.values(checks).every(Boolean),
    checks,
    evidence,
  };
}

function qualityOutput(task, evaluated) {
  return {
    ...object(task.output),
    passed: evaluated.passed,
    overall_score: finite(evaluated.evidence?.overall_score),
    failures: list(evaluated.evidence?.failures),
    repair_instructions: list(evaluated.evidence?.repair_instructions),
    perceptual_validation: evaluated,
  };
}

async function holdOrRelease(task = {}) {
  ...
  if (!evaluated.passed) {
    if (source) {
      await ProductionTaskRuntime.update(source.id, {
        ...
        metadata: {
          ...
          perceptual_quality_state: "QUALITY_REJECTED",
    ...
    return ProductionTaskRuntime.update(task.id, {
      ...
      output: qualityOutput(task, evaluated),
    });
  }
  ...
  if (source) {
    await ProductionTaskRuntime.update(source.id, {
      ...
      metadata: {
        ...
        perceptual_quality_state: "QUALITY_APPROVED",
        perceptual_review_task_id: task.id,
        approved_for_downstream_after_perceptual_review: true,
        rejected_before_editing: false,
        release_hold: false,
        perceptual_validation_evaluated_at: evaluatedAt,
      },
      output: {
