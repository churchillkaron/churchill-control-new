  ...
  apply({ graph, shots = [] } = {}) {
    ...
    for (const sourceNode of [...nodes].filter(needsReview)) {
      ...
      const reviewNode = createProductionNode({
        ...
        assets: expectation.reference_asset_ids,
        generation: {
          required: true,
        ...
        metadata: {
          ...
          workflow_kind: sourceNode.metadata?.workflow_kind || graph.metadata?.workflow_kind || null,
          story_lineage: sourceNode.metadata?.story_lineage || graph.metadata?.story_lineage || null,
          source_generation_node_id: sourceNode.id,
          ...
          reference_asset_ids: expectation.reference_asset_ids,
        },
      });
