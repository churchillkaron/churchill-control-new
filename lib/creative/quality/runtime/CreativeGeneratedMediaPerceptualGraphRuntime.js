...
function mediaKind(node = {}) {
  ...
  if (
    ...
    capability.includes("image.inpaint") ||
    capability.includes("image.outpaint") ||
  ...
  if (
    ...
    capability.includes("video.first_last_frame_to_video") ||
    capability.includes("video.video_to_video") ||
    capability.includes("video.edit") ||
    capability.includes("video.inpaint") ||
    capability.includes("video.lip_sync") ||
    capability.includes("image_to_video") ||
    type === "SHOT" ||
    type === "VIDEO" ||
    type === "FIRST_LAST_FRAME_TO_VIDEO" ||
    type === "VIDEO_TO_VIDEO" ||
    type === "EDIT_VIDEO" ||
    type === "INPAINT_VIDEO" ||
...
function thresholds({ kind, identity, person, product, music } = {}) {
  // This is an admission floor, not an average target. A shot that is technically
  // usable but visibly below elite global campaign craft must never reach editing.
  const worldClassFloor = 94;
  return {
    minimum_overall_score: worldClassFloor,
    minimum_story_score: worldClassFloor,
    minimum_environment_score: worldClassFloor,
    minimum_camera_score: worldClassFloor,
    minimum_anatomy_score: person ? worldClassFloor : 0,
    minimum_identity_score: identity ? worldClassFloor : 0,
    minimum_product_fidelity_score: product ? worldClassFloor : 0,
    minimum_music_energy_score: music ? worldClassFloor : 0,
    minimum_performance_score: person ? worldClassFloor : 0,
    minimum_continuity_score: kind === "VIDEO" ? worldClassFloor : 0,
    minimum_physics_score: kind === "VIDEO" ? worldClassFloor : 0,
    minimum_artifact_score: worldClassFloor,
  };
}

function cinematicMinimumContract(node = {}, shot = {}, kind = null) {
  const coverage = object(
    shot.coverage ||
    shot.agency_craft ||
    node.requirements?.coverage ||
    node.requirements?.agency_craft,
  );
  return {
    contract: "AVANTIQO_ELITE_VISUAL_MINIMUM_V1",
    minimum_standard: "ELITE_GLOBAL_LUXURY_LAUNCH_FILM",
    calibration_reference: "LAMBORGHINI_REVUELTO_FROM_NOW_ON",
    principles_only: true,
    imitation_forbidden: true,
    world_class_floor: 94,
    media_kind: kind,
    mandatory_qualities: [
      "authored_composition_and_focal_hierarchy",
      "foreground_midground_background_depth",
      "controlled_contrast_and_sculpted_light",
      "tactile_material_surface_and_atmospheric_truth",
      "specific_environment_not_generic_ai_location",
      "credible_scale_weight_reflection_contact_and_motion",
      "intentional_visual_tension_or_reveal",
      "story_or_sensory_delta_not_decorative_filler",
      "premium_realism_without_stock_or_generated_feel",
    ],
    video_only_qualities: kind === "VIDEO" ? [
      "motivated_camera_path_or_intentional_stillness",
      "shot_arrives_from_and_causes_adjacent_story_state",
      "opening_progression_closing_each_have_visual_progress",
      "no_unearned_black_frames_or_dead_air",
      "no_overlong_static_hold_without_dramatic_reason",
      "performance_environment_and_camera_remain_alive",
      "sound_picture_sync_point_is_visually_supportable",
    ] : [],
    image_only_qualities: kind === "IMAGE" ? [
      "frame_stands_as_premium_key_art_without_motion_to_rescue_it",
      "subject_separation_and_depth_are_immediately_legible",
      "microdetail_survives_close_inspection",
      "pose_expression_and_environment_feel_observed_not_generated",
      "central_object_reads_as_intentionally_designed_and_brand_ownable_not_a_literal_stock_symbol",
      "industrial_design_materials_joinery_and_internal_architecture_feel_authored_not_decoratively_generated",
      "conceptual_metaphor_is_translated_into_product_design_rather_than_literal_anatomy_or_iconography",
    ] : [],
    hard_reject_signatures: [
      "flat_cheap_or_uniform_lighting",
      "generic_stock_or_ai_composition",
      "empty_lifeless_environment_when_people_or_activity_are_story_relevant",
      "random_city_or_geography_when_location_identity_matters",
      "beauty_without_story_connection",
      "camera_motion_used_as_a_substitute_for_an_idea",
      "plastic_materials_or_synthetic_skin",
      "fog_bloom_neon_or_particles_used_to_hide_weak_art_direction",
      "repeated_visual_idea_without_escalation",
      "visible_generation_artifacts_or_temporal_instability",
      "literal_anatomical_or_stock_symbol_rendering_when_the_approved_direction_requires_an_ownable_designed_object",
      "chip_or_processor_pasted_onto_a_brain_or_other_metaphor_instead_of_integrated_industrial_design",
      "medical_science_illustration_language_in_premium_brand_or_product_key_art",
      "random_circuitry_wires_or_network_clutter_used_as_a_substitute_for_designed_structure",
    ],
    authored_intent: {
      story_delta: text(coverage.story_delta) || null,
      sensory_delta: text(coverage.sensory_delta) || null,
      material_detail: text(coverage.material_detail) || null,
      causal_link_in: text(coverage.causal_link_in) || null,
      causal_link_out: text(coverage.causal_link_out) || null,
      sound_sync_point: text(coverage.sound_sync_point) || null,
      generative_risk: text(coverage.generative_risk) || null,
      reject_if: text(coverage.reject_if) || null,
    },
  };
}
...
function expectedContract(node = {}, shot = {}) {
  ...
  const firstLast = capability === "ai.video.first_last_frame_to_video";
  ...
  return {
    ...
    media_kind: kind,
    source_capability: capability || null,
    first_last_frame_conditioning_expected: firstLast,
    ...
    production_design:
      shot.production_design || node.requirements?.production_design || {},
    art_direction: object(node.requirements?.art_direction),
    art_direction_authority: object(node.requirements?.art_direction_authority),
    ...
    cinematic_minimum: cinematicMinimumContract(node, shot, kind),
    cinematic_dna: shot.cinematic_dna || node.requirements?.cinematic_dna || null,
  ...
  apply({ graph, shots = [] } = {}) {
    ...
    for (const sourceNode of [...nodes].filter(needsReview)) {
      ...
      const reviewNode = createProductionNode({
        ...
        description: "Reject generated media before editing when elite visual authorship, identity, anatomy, camera, environment, story, material realism, depth, tension, performance, continuity, physics or synthetic-artifact evidence fails.",
        ...
        requirements: {
          source_generation_node_id: sourceNode.id,
          story_lineage:
            sourceNode.story_lineage ||
            sourceNode.metadata?.story_lineage ||
            sourceNode.requirements?.story_lineage ||
            null,
          ...
          compare_first_frame_when_required: expectation.first_last_frame_conditioning_expected,
          compare_last_frame_when_required: expectation.first_last_frame_conditioning_expected,
          cinematic_minimum_contract: expectation.cinematic_minimum,
          reject_below_elite_visual_minimum: true,
          reject_before_editing: true,
        },
        // The review node must inherit the source node's assigned production assets so
        // strict shot isolation can verify any primary-source binding. Generated output
        // itself still arrives through the dependency edge, not through this asset list.
        assets: [...new Set([
          ...list(sourceNode.assets),
          expectation.primary_source_asset_id,
        ].filter(Boolean))],
        generation: {
          required: true,
        ...
        metadata: {
          ...
          workflow_kind: sourceNode.metadata?.workflow_kind || null,
          story_lineage:
            sourceNode.story_lineage ||
            sourceNode.metadata?.story_lineage ||
            sourceNode.requirements?.story_lineage ||
          ...
          first_last_frame_conditioning_expected:
            expectation.first_last_frame_conditioning_expected,
          ...
          thresholds: expectation.thresholds,
          cinematic_minimum_contract: expectation.cinematic_minimum.contract,
          cinematic_minimum_standard: expectation.cinematic_minimum.minimum_standard,
          reject_below_elite_visual_minimum: true,
          reject_before_editing: true,
      ...
      inserted.push({
        ...
        media_kind: expectation.media_kind,
        source_capability: expectation.source_capability,
        first_last_frame_conditioning_expected:
          expectation.first_last_frame_conditioning_expected,
        scene_id: expectation.scene_id,
        shot_id: expectation.shot_id,
