...
function structuredRepairSpecification({ source, review, failures, instructions }) {
  ...
  const validation = validationEvidence(review);
  const priorAttempt = Math.max(
    Number(source.metadata?.repair_attempt || 0),
    Number(review.metadata?.repair_attempt || 0),
    0,
  );
  const hardRejects = list(validation.evidence?.matched_hard_reject_signatures);
  const fullDirectionReplacement = priorAttempt >= 3 && hardRejects.length > 0;
  return {
    contract: "CREATIVE_GENERATED_MEDIA_PERCEPTUAL_STRUCTURED_REPAIR_V1",
    promptless_source_of_truth: true,
    repair_mode: fullDirectionReplacement ? "FULL_DIRECTION_REPLACEMENT" : "TARGETED_REPAIR",
    preserve_approved_direction: !fullDirectionReplacement,
    preserve_shot_bible: !fullDirectionReplacement,
    preserve_identity_truth: true,
    preserve_product_truth: true,
    preserve_timing: true,
    preserve_continuity: true,
    change_only_failed_requirements: !fullDirectionReplacement,
...
async function createPair({ source, review, project }) {
  ...
  await ProductionTaskRuntime.update(source.id, {
    metadata: {
      ...
      superseded_by_repair_identity: identity,
      superseded_by_repair_attempt: attempt,
