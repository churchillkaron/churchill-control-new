...
const CONTRACT = "CREATIVE_MASTER_FILM_DIRECTOR_REVIEW_V1";
...
function titleEntries(shot = {}) {
  const graphics = object(shot.graphics);
  return list(graphics.titles || shot.typography?.titles).map((entry) =>
    typeof entry === "string" ? { text: entry } : object(entry),
  ).filter((entry) => text(entry.text || entry.title || entry.copy || entry.content || entry.label));
}
...
function transitionRequiresFinishing(value) {
  ...
  return Boolean(source && !/^(cut|hard cut|match cut|smash cut|straight cut)$/.test(source));
}

function finishingRequired(shots = []) {
  return shots.some((shot) =>
    titleEntries(shot).length > 0 ||
...
function evaluateFinishing({ project = {}, post_production = {}, shots = [] } = {}) {
  ...
  const technicalQc = object(post_production.technical_qc || renderMetadata.technical_qc);
  const tracks = object(post_production.tracks);
  const overlays = list(tracks.overlays);
  const overlayIds = new Set(overlays.map((item) => text(item.asset_node_id)).filter(Boolean));
  const segmentControls = list(renderMetadata.segment_controls);
  const segmentMap = new Map(segmentControls.map((item) => [text(item.shot_id), item]));
  const titleRequirements = shots.map((shot) => ({
    shot_id: text(shot.id),
    titles: titleEntries(shot),
  })).filter((item) => item.shot_id && item.titles.length);
  const titleMismatches = titleRequirements.filter((item) =>
    Number(segmentMap.get(item.shot_id)?.title_count || 0) !== item.titles.length,
  );
  const exactFontRequirements = titleRequirements.flatMap((item) =>
    item.titles.filter(explicitFontRequired).map((entry) => ({
  ...
  const missingLogoOverlays = logoRequirements.filter((item) => !overlayIds.has(item.asset_id));
  ...
  const targetChannels = [...new Set(list(project.target_channels).map(text).filter(Boolean))];
  const checks = [
    {
    ...
    },
    {
      ...
      passed: !finishingNeeded || Boolean(renderMetadata.professional_finishing_contract),
      evidence: {
    ...
    },
    {
      ...
      passed: !finishingNeeded || renderMetadata.professional_finishing?.complete_director_coverage === true,
      evidence: renderMetadata.professional_finishing || null,
    },
    {
      id: "deterministic_title_count_exact",
      passed: titleMismatches.length === 0,
      evidence: {
        required_shot_count: titleRequirements.length,
        mismatches: titleMismatches.map((item) => item.shot_id),
      },
    },
    {
      id: "exact_font_rendering_proven",
      passed: exactFontRequirements.length === 0,
      evidence: exactFontRequirements.length
        ? {
            reason: "Current deterministic finishing runtime does not persist proof of explicit font-file/typeface binding.",
            requirements: exactFontRequirements,
          }
        : { explicit_font_requirement_count: 0 },
    },
    {
    ...
    },
    {
      ...
      evidence: {
        ...
        final_master_audio_verified: renderMetadata.final_master_audio_verified === true,
    ...
    },
    {
      ...
      passed: targetChannels.length <= 1 || Boolean(post_production.export_profile_source),
      evidence: {
        target_channels: targetChannels,
        master_export_profile_source: post_production.export_profile_source || null,
  ...
  return {
    contract: "CREATIVE_MASTER_FILM_DETERMINISTIC_FINISHING_V1",
...
async function persistReport({
  ...
  const identity = digest({
    ...
    semantic_quality_identity: semantic_report.metadata?.semantic_quality_identity || null,
    finishing,
  });
  ...
  const repairInstructions = semantic.checks.flatMap((check) => check.repair_instructions);
  const node = createCreativeAssetNode({
    ...
    intelligence: {
      ...
      tags: ["master-film", "director-review", "weakest-link", "deterministic-finishing"],
    },
...
export const CreativeMasterFilmDirectorReviewRuntime = Object.freeze({
  ...
  hardenSemanticPolicy(policy = {}) {
    ...
    return {
      ...current,
      version: `${text(current.version) || "AVANTIQO_SEMANTIC_QUALITY"}_MASTER_FILM_WORLD_CLASS_V1`,
