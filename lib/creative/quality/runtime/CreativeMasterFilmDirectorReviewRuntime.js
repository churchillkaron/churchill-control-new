...
import {
  evaluateCinematicFrameDynamics,
} from "./CreativeCinematicFrameDynamicsRuntime";
import {
  evaluateCinematicAudioDynamics,
} from "./CreativeCinematicAudioDynamicsRuntime";
import {
...
function evaluateMasterIntentFidelity(value = {}) {
  const source = object(value);
  const required = ["story", "visual_journey", "frame_design", "dp_intent", "physical_world", "dailies", "editorial"];
  const failures = [];
  const checks = required.map((id) => {
    const proof = object(source[id]);
    const score = finite(proof.score);
    const passed = proof.passed === true && score !== null && score >= WORLD_CLASS_QUALITY_FLOORS.minimum_release_score && list(proof.evidence).length > 0;
    if (!passed) failures.push(`MASTER_INTENT_FIDELITY_REQUIRED:${id.toUpperCase()}`);
    return { id, passed, score, evidence: list(proof.evidence) };
  });
  if (source.temporal_continuity_sealed !== true) failures.push("MASTER_TEMPORAL_CONTINUITY_SEAL_REQUIRED");
  if (source.bounded_repairs_resolved !== true) failures.push("MASTER_BOUNDED_REPAIRS_RESOLUTION_REQUIRED");
  if (source.vfx_inheritance_verified !== true) failures.push("MASTER_VFX_INHERITANCE_REQUIRED");
  return {
    passed: failures.length === 0,
    checks,
    failures,
    temporal_continuity_sealed: source.temporal_continuity_sealed === true,
    bounded_repairs_resolved: source.bounded_repairs_resolved === true,
    vfx_inheritance_verified: source.vfx_inheritance_verified === true,
  };
}
...
function evaluateFinishing({ project = {}, post_production = {}, shots = [] } = {}) {
  ...
  const checks = [
    ...
    {
      id: "canonical_brand_lockup_exclusive",
      passed: logoRequirements.length === 0 || Boolean(
        renderMetadata.canonical_brand_lockup_exclusive_verified === true &&
        renderMetadata.generated_brand_substitution_detected !== true
      ),
      evidence: {
        required: logoRequirements.length > 0,
        canonical_brand_lockup_exclusive_verified:
          renderMetadata.canonical_brand_lockup_exclusive_verified === true,
        generated_brand_substitution_detected:
          renderMetadata.generated_brand_substitution_detected === true,
        rule: "No generated globe, duplicate symbol, pseudo-logo or unapproved decorative structure may contaminate the canonical final lockup.",
      },
    },
    {
      id: "cinematic_dynamic_rhythm_verified",
      passed: !hasMasterSoundtrack || Boolean(
        renderMetadata.cinematic_dynamic_rhythm_verified === true &&
        renderMetadata.picture_locked_audio_verified === true &&
        renderMetadata.spatial_soundfield_verified === true,
      ),
      evidence: {
        required: hasMasterSoundtrack,
        dynamic_rhythm: renderMetadata.cinematic_dynamic_rhythm_verified === true,
        picture_locked_audio: renderMetadata.picture_locked_audio_verified === true,
        spatial_soundfield: renderMetadata.spatial_soundfield_verified === true,
        note: "Premium temporal masters must prove pacing, score, SFX, silence and spatial sound react to the actual edit rather than remain flat or generic.",
      },
    },
    {
...
function evaluateFrameDynamics({ project = {}, post_production = {} } = {}) {
  const perceptual = object(post_production.perceptual_quality);
  const metrics = object(perceptual.metadata?.events?.frame_dynamics);
  const duration = finite(
    metrics.duration_seconds ??
    post_production.render?.technical?.duration_seconds ??
    project.target_duration ??
    project.metadata?.target_duration,
  );
  const intentionalRestraint =
    project.metadata?.cinematic_grammar?.intentional_restraint === true ||
    post_production.intentional_visual_restraint === true;
  return evaluateCinematicFrameDynamics({
    metrics,
...
function evaluateAudioDynamics({ project = {}, post_production = {} } = {}) {
  const perceptual = object(post_production.perceptual_quality);
  const metrics = object(perceptual.metadata?.events?.audio_dynamics);
  const duration = finite(
    metrics.duration_seconds ??
    post_production.render?.technical?.duration_seconds ??
    project.target_duration ??
  ...
  const intentionalRestraint =
    project.metadata?.cinematic_grammar?.intentional_audio_restraint === true ||
    post_production.intentional_audio_restraint === true;
  return evaluateCinematicAudioDynamics({
    metrics,
    duration_seconds: duration,
    intentional_restraint: intentionalRestraint,
  });
...
async function persistReport({
  ...
  const passed = semantic.passed && finishing.passed && frame_dynamics?.passed === true && audio_dynamics?.passed === true && intent_fidelity?.passed === true;
  ...
  const node = createCreativeAssetNode({
    ...
    metadata: {
      ...
      failed_checks: [
        ...
        ...list(frame_dynamics?.failures),
        ...list(audio_dynamics?.failures),
        ...list(intent_fidelity?.failures),
      ],
      repair_instructions: repairInstructions,
      deterministic_finishing: finishing,
      cinematic_frame_dynamics: frame_dynamics,
      cinematic_audio_dynamics: audio_dynamics,
      master_intent_fidelity: intent_fidelity,
      reference_frame_dynamics_required: true,
      reference_audio_dynamics_required: true,
...
export const CreativeMasterFilmDirectorReviewRuntime = Object.freeze({
  ...
  hardenSemanticPolicy(policy = {}) {
    ...
    return {
      ...
      require_cinematic_dynamic_rhythm_review: true,
      require_picture_locked_audio_review: true,
      require_spatial_soundfield_review: true,
      master_film_director_review_contract: CONTRACT,
      weakest_link_enforced: true,
      b_grade_release_forbidden: true,
      require_master_intent_fidelity: true,
      require_temporal_continuity_seal: true,
      require_bounded_repairs_resolved: true,
      require_vfx_inheritance_verified: true,
    };
  },
  ...
  async evaluate({
    ...
    const frameDynamics = evaluateFrameDynamics({ project, post_production });
    const audioDynamics = evaluateAudioDynamics({ project, post_production });
    const intentFidelity = evaluateMasterIntentFidelity(post_production.intent_fidelity);
    const report = await persistReport({
      ...
      semantic,
      finishing,
      frame_dynamics: frameDynamics,
      audio_dynamics: audioDynamics,
      intent_fidelity: intentFidelity,
    });

    return {
      contract: CONTRACT,
      passed: semantic.passed && finishing.passed && frameDynamics.passed && audioDynamics.passed && intentFidelity.passed,
      semantic,
      deterministic_finishing: finishing,
      cinematic_frame_dynamics: frameDynamics,
      cinematic_audio_dynamics: audioDynamics,
      master_intent_fidelity: intentFidelity,
      report,
    };
  },
});
