...
import {
  CreativeDailiesRoomRuntime,
} from "@/lib/creative/production-room/runtime/CreativeDailiesRoomRuntime";
...
const SIMULATION_QC_CONTRACT = "AVANTIQO_SIMULATION_QC_V1";
const SIMULATION_QC_SEAL_CONTRACT = "AVANTIQO_SIMULATION_QC_SEAL_V1";
...
const SCORE_THRESHOLD_MAP = Object.freeze({
  ...
  composition_score: "minimum_composition_score",
  aesthetic_distinction_score: "minimum_aesthetic_distinction_score",
  tension_curiosity_score: "minimum_tension_curiosity_score",
  production_value_score: "minimum_production_value_score",
  depth_atmosphere_score: "minimum_depth_atmosphere_score",
  environmental_aliveness_score: "minimum_environmental_aliveness_score",
});

const CINEMATIC_MERIT_FLOOR = 94;
const CINEMATIC_HARD_DIMENSIONS = Object.freeze([
  "story_score",
  "camera_score",
  "performance_score",
  "environment_score",
...
const CINEMATIC_WEIGHTS = Object.freeze({
  ...
  aesthetic_distinction_score: 0.15,
  production_value_score: 0.14,
  depth_atmosphere_score: 0.10,
  tension_curiosity_score: 0.10,
  environmental_aliveness_score: 0.08,
  story_score: 0.08,
  camera_score: 0.06,
...
function simulationApplicable(source = {}) {
  const input = object(source.input);
  const requirements = object(input.requirements);
  const contract = object(
    input.simulation_contract ||
    requirements.simulation_contract ||
    source.metadata?.simulation_contract_data,
  );
  return text(contract.contract) === SIMULATION_CONTRACT ||
    text(source.metadata?.simulation_contract) === SIMULATION_CONTRACT;
}

function simulationQcPassed(review = {}, source = {}) {
  if (!simulationApplicable(source)) return true;
  const sourceHash = text(source.metadata?.simulation_qc_seal_hash);
  const reviewHash = text(review.metadata?.simulation_qc_seal_hash);
  return source.metadata?.simulation_qc_contract === SIMULATION_QC_CONTRACT &&
    review.metadata?.simulation_qc_contract === SIMULATION_QC_CONTRACT &&
    source.metadata?.simulation_qc_seal_contract === SIMULATION_QC_SEAL_CONTRACT &&
    review.metadata?.simulation_qc_seal_contract === SIMULATION_QC_SEAL_CONTRACT &&
    source.metadata?.simulation_qc_sealed === true &&
    review.metadata?.simulation_qc_sealed === true &&
    Boolean(sourceHash) &&
    sourceHash === reviewHash;
}
...
function dailiesReviews({ scores = {}, continuityPassed = false, failedChecks = [] } = {}) {
  const evidence = object(scores.evidence);
  const floor = 94;
  const minScore = (...values) => {
    const rows = values.map(finite).filter((value) => value !== null);
    return rows.length ? Math.min(...rows) : 0;
  };
  const rows = [
    ["DIRECTING", minScore(evidence.story_score, evidence.performance_score), ["story_score", "performance_score"]],
    ["CINEMATOGRAPHY", minScore(evidence.camera_score), ["camera_score"]],
    ["CONTINUITY", minScore(evidence.continuity_score), ["continuity_score", "continuity_qc_seal"]],
    ["TECHNICAL_TRUTH", minScore(evidence.physics_score, evidence.artifact_score), ["physics_score", "artifact_score"]],
    ["PERCEPTUAL_QUALITY", minScore(evidence.overall_score), ["overall_score"]],
  ];
  return rows.map(([family, score, fields]) => ({
    family,
    reviewer_id: `autonomous-dailies:${family.toLowerCase()}`,
    score,
    passed: score >= floor && (family !== "CONTINUITY" || continuityPassed === true),
    evidence: fields.map((field) => `existing-review-evidence:${field}`),
    failure_codes: list(failedChecks).filter((code) => {
      const value = text(code).toLowerCase();
      if (family === "DIRECTING") return /story|performance|tension|edit|coverage|generic/.test(value);
      if (family === "CINEMATOGRAPHY") return /camera|light|depth|composition|cinematic|beauty|frame/.test(value);
      if (family === "CONTINUITY") return /continuity|identity|geometry|wardrobe|screen.direction/.test(value);
      if (family === "TECHNICAL_TRUTH") return /physics|artifact|simulation|vfx|material|anatomy/.test(value);
      return true;
    }),
    repair_focus: fields.map((field) => `raise-${field}-above-${floor}`),
  }));
}

function cinematicMeritPassed(merit = {}) {
  if (finite(merit.score) === null || merit.score < CINEMATIC_MERIT_FLOOR) return false;
  return CINEMATIC_HARD_DIMENSIONS.every((field) => {
    const score = finite(merit.dimensions?.[field]);
    return score !== null && score >= CINEMATIC_MERIT_FLOOR;
  });
}
...
async function bridge(review = {}) {
  ...
  const sourceRequirements = object(source.input?.requirements);
  const craftProfile = {
    production_stage: source.metadata?.production_step_id || source.metadata?.pass_id || source.capability || null,
    coverage_role: sourceRequirements.coverage_role || sourceRequirements.coverage?.coverage_role || null,
    camera: object(sourceRequirements.camera),
    performance_direction: sourceRequirements.performance_direction || sourceRequirements.performance || null,
    signature_frame_design: object(sourceRequirements.signature_frame_design),
    lighting: object(sourceRequirements.lighting),
    production_design: object(sourceRequirements.production_design),
    scene_cinematic_intelligence: object(sourceRequirements.scene_cinematic_intelligence),
    scene_audio_dramaturgy: object(sourceRequirements.scene_audio_dramaturgy),
  };
  ...
  const simulationPassed = simulationQcPassed(review, source);
  const merit = cinematicMerit(scores.evidence);
  const cinematicBeautyPassed = cinematicMeritPassed(merit);
  const baseHardGatePassed = continuityPassed && humanQualityPassed && cinematicBeautyPassed;
  const passed =
  ...
  const failedChecks = [
    ...
    ...(simulationPassed ? [] : ["simulation_qc_seal"]),
    ...(cinematicBeautyPassed ? [] : ["cinematic_beauty"]),
  ];

  const updatedAsset = await AssetGraphRepository.update(asset.id, {
    intelligence: {
    ...
    metadata: {
      ...
      shot_candidate_craft_profile: craftProfile,
      shot_candidate_dimension_scores: {
        story: finite(scores.evidence?.story_score),
        camera: finite(scores.evidence?.camera_score),
        performance: finite(scores.evidence?.performance_score),
        continuity: finite(scores.evidence?.continuity_score),
        physics: finite(scores.evidence?.physics_score),
        artifacts: finite(scores.evidence?.artifact_score),
        environment: finite(scores.evidence?.environment_score),
        music_energy: finite(scores.evidence?.music_energy_score),
        product_fidelity: finite(scores.evidence?.product_fidelity_score),
      },
      ...
      shot_candidate_simulation_applicable: simulationApplicable(source),
      shot_candidate_simulation_qc_passed: simulationPassed,
      shot_candidate_simulation_qc_contract: SIMULATION_QC_CONTRACT,
      shot_candidate_simulation_qc_seal_contract: SIMULATION_QC_SEAL_CONTRACT,
      shot_candidate_simulation_qc_seal_hash:
        simulationPassed && simulationApplicable(source)
          ? text(review.metadata?.simulation_qc_seal_hash)
          : null,
      shot_candidate_cinematic_merit_score: cinematicBeautyPassed ? merit.score : null,
      shot_candidate_cinematic_beauty_passed: cinematicBeautyPassed,
      shot_candidate_cinematic_merit_floor: CINEMATIC_MERIT_FLOOR,
  ...
  if (!cinemaSource(source)) return updatedAsset;
  return CreativeDailiesRoomRuntime.recordAssetDecision({
    organization_id: review.organization_id,
    asset_node_id: updatedAsset.id,
    reviews: dailiesReviews({ scores, continuityPassed, failedChecks }),
  });
}
