...
function evaluate(events, policy) {
  ...
  add("black_duration", maxBlack !== null, events.black_duration_seconds <= maxBlack, {
    actual_seconds: events.black_duration_seconds,
    maximum_seconds: maxBlack,
    segments: events.black_segments,
  });
  add("freeze_duration", maxFreeze !== null, events.freeze_duration_seconds <= maxFreeze, {
    actual_seconds: events.freeze_duration_seconds,
    maximum_seconds: maxFreeze,
    segments: events.freeze_segments,
  });
  add("silence_duration", maxSilence !== null, events.silence_duration_seconds <= maxSilence, {
  ...
  add(
    ...
    events.integrated_lufs !== null && Math.abs(events.integrated_lufs - targetLoudness) <= loudnessTolerance,
    {
...
export const CreativePerceptualQualityRuntime = {
  async analyze({
    ...
    if (blackPictureThreshold !== null && blackPixelThreshold !== null) {
      filters.push(`blackdetect=d=${blackPictureThreshold}:pix_th=${blackPixelThreshold}`);
    }
    if (freezeNoise !== null) {
    ...
    try {
      const text = await run(
        ffmpegPath,
        ["-hide_banner", "-i", materialized.file_path, "-filter_complex", filters.join(","), "-f", "null", "-"],
        timeoutMs,
      );
