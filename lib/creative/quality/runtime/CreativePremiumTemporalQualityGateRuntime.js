...
const TEMPORAL_PROJECT_TYPES = new Set([
  "VIDEO",
  "FILM",
  "ANIMATION",
  "TEMPORAL",
...
function enabled(project = {}) {
  ...
  const contract = text(metadata.studio_project_contract).toUpperCase();
  const productionType = text(
    project.production_type || metadata.production_type,
  ).toUpperCase();
  return (
    TEMPORAL_PROJECT_TYPES.has(productionType) ||
    metadata.premium_temporal_quality === true ||
