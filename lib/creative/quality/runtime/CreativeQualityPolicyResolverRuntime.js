...
} from "./CreativeSemanticQualityRuntime.js";
...
function generatedCreativePolicy(context) {
  ...
  return {
    ...
    minimum_scene_score: context.temporal ? 94 : context.still ? 92 : 90,
    regenerate_below_score: context.temporal ? 94 : context.still ? 90 : 88,
...
function generatedSemanticPolicy(context) {
  return {
    ...
    minimum_confidence: context.temporal ? 85 : 75,
    minimum_score: context.temporal ? 94 : context.still ? 90 : 88,
...
function explicitCreativePolicy(mission = {}, project = {}) {
  ...
  const projectSource = upper(project.metadata?.creative_quality_policy_source);
  if (Object.keys(projectPolicy).length && projectSource === "EXPLICIT") return projectPolicy;
...
function explicitSemanticPolicy(mission = {}, project = {}) {
  ...
  const projectSource = upper(project.metadata?.semantic_quality_policy_source);
  if (Object.keys(projectPolicy).length && projectSource === "EXPLICIT") return projectPolicy;
  ...
  resolve({ mission = {}, project = {} } = {}) {
    ...
    return {
      ...
      semantic_policy_source: Object.keys(explicitSemantic).length
        ? "EXPLICIT"
        : "CANONICAL_CONTEXT_PROFILE",
      resolver_version: "CREATIVE_QUALITY_POLICY_RESOLVER_V2",
    };
  },
};
