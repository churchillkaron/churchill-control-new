...
export const CreativeRenderRepairExecutionRuntime = {
  async execute({
    ...
    if (!repair_plan_asset_node_id) {
      throw new Error("repair_plan_asset_node_id required");
    }
    ...
    const result = await CreativeEdlRenderRuntime.render({
      ...
      policy: {
        ...policy,
        render_execution_identity: executionIdentity,
      },
      force: true,
    });
    ...
    const execution = createCreativeAssetNode({
      ...
      metadata: {
        ...
        result_render_identity:
          result.render?.metadata?.render_identity || null,
    ...
    try {
      ...
      const current = await AssetGraphRepository.listByProject({
        organization_id,
        creative_project_id: plan.creative_project_id,
      });
      const winner = current.find((node) =>
        node.type === CREATIVE_ASSET_NODE_TYPES.REPAIR_PLAN &&
        node.metadata?.repair_execution_identity === executionIdentity,
      );
      if (!winner) throw error;
      return { execution: winner, render: result.render, reused: true };
    }
  },
};
