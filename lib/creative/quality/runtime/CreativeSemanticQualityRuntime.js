...
export const CREATIVE_SEMANTIC_QUALITY_CHECKS = Object.freeze([
  ...
  "tension_and_reveal_architecture",
  "emotional_arc",
...
const TEMPORAL_EVIDENCE_CHECKS = new Set([
  "identity_continuity",
  "product_continuity",
  "anatomy_and_object_integrity",
  "physics_and_contact",
...
function validatePolicy(policy = {}) {
  ...
  const checkMinimumScores = object(policy.check_minimum_scores);
  ...
  if (unknownChecks.length) {
    failures.push({ code: "SEMANTIC_REQUIRED_CHECKS_INVALID", check_ids: unknownChecks });
  }
  if (minimumConfidence === null || minimumConfidence < 0 || minimumConfidence > 100) {
    failures.push({ code: "SEMANTIC_MINIMUM_CONFIDENCE_POLICY_INVALID", actual: policy.minimum_confidence });
  }
  if (minimumScore === null || minimumScore < 0 || minimumScore > 100) {
    failures.push({ code: "SEMANTIC_MINIMUM_SCORE_POLICY_INVALID", actual: policy.minimum_score });
  }
  for (const [id, value] of Object.entries(checkMinimumScores)) {
    const score = finite(value);
    if (!CHECK_IDS.has(id) || score === null || score < 0 || score > 100) {
      failures.push({ code: "SEMANTIC_CHECK_MINIMUM_SCORE_POLICY_INVALID", check_id: id, actual: value });
    }
  }
  ...
  return {
    ...
    check_minimum_scores: checkMinimumScores,
    require_timestamped_failures: policy.require_timestamped_failures === true,
    forensic_temporal_review_required: policy.forensic_temporal_review_required === true,
    require_audio_review: requireAudioReview,
    version,
  };
}

function minimumForCheck(policyValidation, checkId) {
  return finite(policyValidation.check_minimum_scores?.[checkId]) ?? policyValidation.minimum_score;
}
...
function validateReview(review = {}, policy = {}) {
  ...
  const checks = policyValidation.required_checks.map((id) => normalizeCheck(id, checksInput[id]));

  for (const check of checks) {
    if (!check.status || !["PASS", "FAIL", "NOT_APPLICABLE"].includes(check.status)) {
      failures.push({ code: "SEMANTIC_CHECK_STATUS_REQUIRED", check_id: check.id });
      continue;
    }
    if (check.status === "NOT_APPLICABLE") {
      if (!check.evidence.length) failures.push({ code: "NOT_APPLICABLE_EVIDENCE_REQUIRED", check_id: check.id });
      continue;
    }
    if (!check.evidence.length) failures.push({ code: "SEMANTIC_CHECK_EVIDENCE_REQUIRED", check_id: check.id });
    if (check.confidence === null || check.confidence < 0 || check.confidence > 100) {
      failures.push({ code: "SEMANTIC_CHECK_CONFIDENCE_INVALID", check_id: check.id, actual: check.confidence });
    } else if (policyValidation.minimum_confidence !== null && check.confidence < policyValidation.minimum_confidence) {
      failures.push({ code: "SEMANTIC_CHECK_CONFIDENCE_LOW", check_id: check.id, actual: check.confidence, minimum: policyValidation.minimum_confidence });
    }
    if (check.score === null || check.score < 0 || check.score > 100) {
      failures.push({ code: "SEMANTIC_CHECK_SCORE_INVALID", check_id: check.id, actual: check.score });
    }
    if (check.status === "PASS" && check.passed !== true) {
      failures.push({ code: "SEMANTIC_CHECK_PASS_FLAG_REQUIRED", check_id: check.id });
    }
    const checkMinimum = minimumForCheck(policyValidation, check.id);
    if (check.status === "PASS" && check.score !== null && checkMinimum !== null && check.score < checkMinimum) {
      failures.push({ code: "SEMANTIC_CHECK_SCORE_LOW", check_id: check.id, actual: check.score, minimum: checkMinimum });
    }
    if (check.status === "FAIL" && check.passed === true) {
      failures.push({ code: "SEMANTIC_CHECK_FAIL_FLAG_INVALID", check_id: check.id });
    }
    if (check.status === "FAIL" && !check.repair_instructions.length) {
      failures.push({ code: "SEMANTIC_REPAIR_INSTRUCTIONS_REQUIRED", check_id: check.id });
    }
    if (
      check.status === "FAIL" &&
      policyValidation.require_timestamped_failures &&
      TEMPORAL_EVIDENCE_CHECKS.has(check.id) &&
      !check.timestamps.length
    ) {
      failures.push({ code: "SEMANTIC_TEMPORAL_FAILURE_TIMESTAMPS_REQUIRED", check_id: check.id });
    }
  }
  ...
  if (!sampledFrames.length && !sampledClips.length) failures.push({ code: "VISUAL_SAMPLE_EVIDENCE_REQUIRED" });
  if (policyValidation.forensic_temporal_review_required === true) {
    const forensic = sampledClips.some((item) => object(item).forensic_temporal_review === true);
    if (!forensic) failures.push({ code: "FORENSIC_TEMPORAL_SAMPLE_EVIDENCE_REQUIRED" });
  }
  if (policyValidation.require_audio_review === true && !sampledAudio.length) failures.push({ code: "AUDIO_SAMPLE_EVIDENCE_REQUIRED" });
  ...
  const overallScore = scored.length
    ...
    : null;
  const weakest = scored.reduce((selected, check) => {
    if (!selected) return check;
    return Number(check.score) < Number(selected.score) ? check : selected;
  }, null);
  const weakestMinimum = weakest ? minimumForCheck(policyValidation, weakest.id) : null;
  if (weakest && weakestMinimum !== null && weakest.score < weakestMinimum) {
    failures.push({
      code: "SEMANTIC_WEAKEST_LINK_BELOW_FLOOR",
      check_id: weakest.id,
      actual: weakest.score,
      minimum: weakestMinimum,
    });
  }
  ...
  return {
    ...
    policy: {
      ...
      check_minimum_scores: policyValidation.check_minimum_scores,
      forensic_temporal_review_required: policyValidation.forensic_temporal_review_required,
      require_timestamped_failures: policyValidation.require_timestamped_failures,
    ...
    repair_plan: failedChecks.flatMap((check) => check.repair_instructions.map((instruction) => ({
      check_id: check.id,
      instruction,
...
export const CreativeSemanticQualityRuntime = {
  ...
  async record({ organization_id, render_asset_node_id, review = {}, policy = {}, force = false } = {}) {
    ...
    if (!render || render.organization_id !== organization_id || render.type !== CREATIVE_ASSET_NODE_TYPES.FINAL_RENDER) {
      throw new Error("Final render asset not found");
    }

    const identity = reviewIdentity(render, review, policy);
    const projectNodes = await AssetGraphRepository.listByProject({ organization_id, creative_project_id: render.creative_project_id });
    const existing = !force ? projectNodes.find((node) =>
    ...
    const node = createCreativeAssetNode({
      ...
      status: evaluation.passed ? CREATIVE_ASSET_NODE_STATUS.REVIEW : CREATIVE_ASSET_NODE_STATUS.REJECTED,
      ...
      intelligence: {
        ...
        tags: ["semantic-quality", "perceptual-review", "agency-grade", "weakest-link"],
      },
    ...
    return { report: await AssetGraphRepository.create(node), reused: false };
  },
};
