...
import { CreativeTrainingCandidateRuntime } from "@/lib/creative/learning/runtime/CreativeTrainingCandidateRuntime";

const CONTRACT = "CREATIVE_SHOT_CANDIDATE_SELECTION_V2";
...
function candidateScore(node = {}) {
  return {
    cinematic: finite(node.metadata?.shot_candidate_cinematic_merit_score, 0),
...
function eligible(node = {}) {
  ...
  return node.type === CREATIVE_ASSET_NODE_TYPES.VIDEO &&
    ...
    node.metadata?.shot_candidate_hard_human_quality_passed !== false &&
    node.metadata?.shot_candidate_cinematic_beauty_passed === true &&
    score.weakest >= WORLD_CLASS_FLOOR &&
    score.overall >= WORLD_CLASS_FLOOR &&
    score.cinematic >= WORLD_CLASS_FLOOR;
}

function compare(left, right) {
  const a = candidateScore(left);
  const b = candidateScore(right);
  if (a.cinematic !== b.cinematic) return b.cinematic - a.cinematic;
...
export const CreativeShotCandidateSelectionRuntime = {
  ...
  async select({ organization_id, creative_project_id, shot_id } = {}) {
    ...
    if (!qualified.length) {
      return {
        ...
        candidates: candidates.map((node) => ({
          ...
          hard_human_quality_passed:
            node.metadata?.shot_candidate_hard_human_quality_passed !== false,
          cinematic_beauty_passed:
            node.metadata?.shot_candidate_cinematic_beauty_passed === true,
          ...candidateScore(node),
    ...
    for (const candidate of candidates) {
      ...
      await AssetGraphRepository.update(candidate.id, {
        metadata: {
          ...
          shot_candidate_selection_policy:
            "HARD_QUALITY_GATE_THEN_CINEMATIC_MERIT",
          shot_candidate_selected: selected,
          selected_for_master: selected,
          include_in_master: selected,
          selection_cinematic_merit_score: candidateScore(candidate).cinematic,
          selection_weakest_score: candidateScore(candidate).weakest,
          selection_overall_score: candidateScore(candidate).overall,
          selected_at: selected ? selectedAt : null,
          rejected_by_candidate_competition: !selected,
        },
      });
    }
      ...
      if (task && text(task.organization_id) === text(organization_id)) {
        await ProductionTaskRuntime.update(task.id, {
          metadata: {
            ...
            selected_candidate_cinematic_merit_score: candidateScore(winner).cinematic,
    ...
    await CreativeTrainingCandidateRuntime.capture({
      organization_id,
      creative_project_id,
    });
    ...
    return {
      ...
      selection_policy: "HARD_QUALITY_GATE_THEN_CINEMATIC_MERIT",
      winner: {
