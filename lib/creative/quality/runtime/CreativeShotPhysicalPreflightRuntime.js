...
function feasibilityFailures(shot = {}) {
  const forecast = object(shot.feasibility_forecast || shot.generation?.feasibility_forecast);
  const failures = [];
  if (text(forecast.readiness) === "REDESIGN_REQUIRED") failures.push("SHOT_FEASIBILITY_REDESIGN_REQUIRED");
  if (text(forecast.readiness) === "DECOMPOSE_BEFORE_GENERATION" &&
      object(shot.generation_strategy).pass_policy?.multipass_required !== true) {
    failures.push("SHOT_FEASIBILITY_MULTIPASS_REQUIRED");
  }
  return failures;
}
