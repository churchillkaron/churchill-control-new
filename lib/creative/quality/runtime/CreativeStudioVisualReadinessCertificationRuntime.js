...
export function unlockStudioVisualGenerationCertification(
  certification = {},
  {
    source,
    scoped_project_id,
  ...
  const valid =
    ...
    Number(certification.required_score) === 100 &&
    certification.zero_paid_media_generation === true &&
    certification.known_failure_suite_passed === true &&
    Boolean(text(certification.benchmark_suite_digest));
  if (!valid) {
    throw new Error("STUDIO_VISUAL_CERTIFICATION_REQUIRED_FOR_UNLOCK");
  }
  if (text(source) !== "USER_EXPLICIT_APPROVAL") {
    throw new Error("STUDIO_VISUAL_GENERATION_EXPLICIT_HUMAN_APPROVAL_REQUIRED");
  }
  if (!text(scoped_project_id)) {
    throw new Error("STUDIO_VISUAL_GENERATION_SCOPED_PROJECT_REQUIRED");
  }
  return Object.freeze({
    ...certification,
    generation_unlocked: true,
    scoped_project_id: text(scoped_project_id),
    unlock_source: "USER_EXPLICIT_APPROVAL",
...
export const CreativeStudioVisualReadinessCertificationRuntime = Object.freeze({
  ...
  unlock: unlockStudioVisualGenerationCertification,
