...
import { CreativeMasterFilmDirectorReviewRuntime } from "./CreativeMasterFilmDirectorReviewRuntime";
import { CreativeQualityPolicyResolverRuntime } from "./CreativeQualityPolicyResolverRuntime";
import { CreativeReleaseReadinessRuntime } from "@/lib/creative/release/runtime/CreativeReleaseReadinessRuntime";
import { ProviderResolver } from "@/lib/platform/service-runtime/providers/ProviderResolver";
import {
...
function configuredSemanticPolicy(project = {}) {
  ...
  return object(
    ...
    project.metadata?.semantic_quality_policy,
  );
}

async function semanticPolicy({ organization_id, project } = {}) {
  const configured = configuredSemanticPolicy(project);
  const resolved = CreativeQualityPolicyResolverRuntime.resolve({ project });
  let policy = CreativeMasterFilmDirectorReviewRuntime.hardenSemanticPolicy({
    ...object(resolved.semantic_quality_policy),
    ...configured,
  });
  const capability = text(policy.capability || "ai.image.analyze");
  if (!text(policy.service_id)) policy.service_id = capability;

  if (!text(policy.model)) {
    const selected = await ProviderResolver.resolveProvider({
      organization_id,
      capability,
      preferredProvider: text(policy.provider_id) || null,
      policy: object(policy.provider_policy),
    });
    policy = {
      ...policy,
      provider_id: text(policy.provider_id) || selected.provider,
      model: selected.model,
      pricing_id: selected.pricing_id || null,
      provider_selection_contract: "SERVICE_DOMAIN_EXECUTION_SELECTION_V1",
      provider_selection_owned_by_service_domain: true,
    };
  }

  if (!text(policy.version) || !text(policy.service_id) || !text(policy.model)) {
    throw new Error("SEMANTIC_REVIEW_POLICY_REQUIRED");
  }
  return policy;
}
...
export const CreativeTemporalSemanticRepairRuntime = {
  async evaluate({ organization_id, creative_project_id, post_production = {} } = {}) {
    ...
    let policy;
    try {
      policy = await semanticPolicy({ organization_id, project });
    } catch (error) {
      return {
        ...post_production,
        status: "REVIEW_REQUIRED",
        semantic_quality: null,
        semantic_quality_blocker: error.message || "SEMANTIC_REVIEW_POLICY_REQUIRED",
      };
    }
    ...
    if (reportPassed(report)) {
      const masterFilm = await CreativeMasterFilmDirectorReviewRuntime.evaluate({
        organization_id,
        creative_project_id,
        post_production,
        semantic_report: report,
      });
      if (!masterFilm.passed) {
        return {
          ...post_production,
          status: "REVIEW_REQUIRED",
          semantic_quality: report,
          master_film_quality: masterFilm.report,
          master_film_quality_gate: masterFilm,
