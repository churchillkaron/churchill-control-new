...
function resolveTargetTaskIds({ report, timeline, nodes, tasks }) {
  const entries = repairEntries(report);
  ...
  return [...ids].filter((id) => tasks.some((task) => task.id === id));
}
...
function instructionsForTask(report, task) {
  ...
  return instructions.length ? [...new Set(instructions)] : [
...
async function createReplacement({ task, report, project }) {
  ...
  const failures = failedChecks(report).map((check) => check.id);
  ...
  const replacement = await ProductionTaskRuntime.create({
    ...
    type: task.type,
    status: "WAITING",
    title: `Repair ${task.title || "film shot"}`,
    description: `Targeted semantic film repair attempt ${attempt}.`,
    service_id: task.service_id,
    service_code: task.service_code,
    capability: task.capability,
    provider_id: task.provider_id || null,
    priority: Math.max(0, Number(task.priority || 100) - 0.25),
    depends_on: list(task.depends_on),
    input: {
      ...(task.input || {}),
      prompt: [
        task.input?.prompt || task.input?.provider_prompt || "",
        "REPAIR ONLY THIS REJECTED FILM STAGE. Do not redesign or regenerate unaffected scenes or shots.",
  ...
  async evaluate({ organization_id, creative_project_id, post_production = {} } = {}) {
    ...
    for (const id of targetTaskIds) {
      ...
      try {
        ...
        created.push({ source_task_id: task.id, replacement_task_id: replacement.id });
      } catch (error) {
