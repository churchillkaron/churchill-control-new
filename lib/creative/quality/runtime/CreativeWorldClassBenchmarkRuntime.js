...
export const CREATIVE_WORLD_CLASS_BENCHMARK_THRESHOLDS = Object.freeze({
  minimum_cases: 5,
  minimum_case_score: 94,
  minimum_overall_score: 94,
  maximum_pairwise_direction_similarity: 0.55,
});
...
// Filler is defined by the director registry, not here. The benchmark measures the studio's standard;
// it does not get to hold a private copy of it. Keeping one here is exactly how the score came to charge
// for patterns the disclosure had already narrowed.
export const CREATIVE_GENERIC_LANGUAGE_PATTERNS = CREATIVE_ADVERTISING_FILLER;

export function genericLanguageHits(plan = {}) {
  return advertisingFillerHits(compactDirection(plan));
}
