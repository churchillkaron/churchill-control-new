...
// Which anchors were hit and which were missed, so a specificity score can be explained
// instead of appearing as a bare number. This is the heaviest weighted dimension at 0.18,
// and a score of 75 gave no indication of whether the direction was vague or whether one
// declared anchor had no supporting evidence to ground it -- the second being a problem
// with the case, not the direction, since the contract forbids inventing anchors merely to
// sound specific.
export function anchorCoverage(plan = {}, benchmark = {}) {
  ...
  const missed = anchors.filter((anchor) => !direction.includes(anchor.toLowerCase()));
  return { anchors, matched, missed };
}
...
// The house list of advertising clichés the benchmark penalises, named so it can be both scored
// and disclosed. Keeping it private meant the contract asked the director to "identify the most
// likely advertising clichés and design against them" while the benchmark judged against a
// specific list it never shared -- the same gap that had rejected_patterns supplying three when
// full marks needed four.
export const CREATIVE_GENERIC_LANGUAGE_PATTERNS = Object.freeze([
  { id: "elevate_your", pattern: /\belevate your\b/g },
  { id: "unforgettable_experience", pattern: /\bunforgettable experience\b/g },
  // The cliché is the tagline form, "Where luxury meets convenience", which sits at the head of
  // a clause. Unanchored and greedy, this matched any sentence containing "where" before "meets"
  // -- "a shot list where camera movement meets the beat", "the room where the band meets the
  // crowd" -- penalising exact craft description as advertising language. Anchoring to clause
  // position keeps the tagline and spares the craft.
  { id: "where_x_meets_y", pattern: /(?:^|[.!?;]\s+|["'“])where \w+(?: \w+){0,3} meets \w+/g },
  { id: "more_than_just", pattern: /\bmore than just\b/g },
  { id: "discover_the_difference", pattern: /\bdiscover the difference\b/g },
  { id: "unlock_potential", pattern: /\bunlock .* potential\b/g },
  // redefine(?:d|s|ing)? could never match "redefining": the stem drops its e, so the most common
  // advertising form of the word was slipping through untouched. This makes the check stricter
  // rather than looser.
  { id: "redefine", pattern: /\bredefin(?:e|ed|es|ing)\b/g },
  // "journey" alone caught ordinary strategy and craft language -- audience journey, customer
  // journey, narrative journey -- which are the correct terms for what they describe. The cliché
  // is the possessive and the promise: your journey, the journey begins.
  { id: "journey", pattern: /\b(?:your|our|their) journey\b|\bjourney (?:begins|starts|awaits|continues)\b/g },
  { id: "premium_experience", pattern: /\bpremium experience\b/g },
  { id: "seamless", pattern: /\bseamless(?:ly)?\b/g },
  { id: "innovative_solutions", pattern: /\binnovative solutions?\b/g },
  { id: "cutting_edge", pattern: /\bcutting[- ]edge\b/g },
  { id: "game_changer", pattern: /\bgame[- ]changer\b/g },
  { id: "transform_your", pattern: /\btransform your\b/g },
]);

// Which clichés actually fired, so a flat deduction can be explained. Two of these patterns are
// broad enough to catch honest prose -- "journey" matches ordinary strategy language like customer
// journey, and "where .* meets .*" is unanchored and greedy -- so knowing which one fired decides
// whether the direction needs improving or the pattern needs narrowing.
export function genericLanguageHits(plan = {}) {
  const direction = compactDirection(plan).toLowerCase();
  return CREATIVE_GENERIC_LANGUAGE_PATTERNS.flatMap((entry) => {
    const matches = direction.match(entry.pattern) || [];
    return matches.length
      ? [{ id: entry.id, count: matches.length, example: text(matches[0]).slice(0, 90) }]
      : [];
  });
}

// The penalty is computed from the same list the diagnostic reports and the contract discloses.
// It previously kept its own inline copy of the patterns, so narrowing "journey" and anchoring
// "where X meets Y" changed the diagnostic and the disclosure while the score went on using the
// broad originals. churchill-audio-package showed the two disagreeing in the open: the penalty
// reported 35, its maximum, while the hits reported a single cliché appearing three times, worth 21.
// One list, one source of truth, so that cannot recur.
function genericLanguagePenalty(plan = {}) {
  const hits = genericLanguageHits(plan).reduce((sum, entry) => sum + entry.count, 0);
...
export function scoreCreativeWorldClassBenchmarkCase(entry = {}) {
  ...
  return {
    ...
    metrics: {
      ...
      // Which capabilities the plan actually reaches for. A case whose deliverable is
      // generated audio should be planning against the enabled audio services, and without
      // this there was no way to tell whether it did.
      production_capability_pairs: unique(
        steps
          .map((step) => {
            const service = text(step?.service || step?.generation?.service);
            const capability = text(step?.capability || step?.generation?.capability);
            return service && capability ? `${service}:${capability}` : "";
          })
          .filter(Boolean),
      ),
      generic_language_hits: genericLanguageHits(plan),
      anchors_matched: anchorCoverage(plan, benchmark).matched,
      anchors_missed: anchorCoverage(plan, benchmark).missed,
