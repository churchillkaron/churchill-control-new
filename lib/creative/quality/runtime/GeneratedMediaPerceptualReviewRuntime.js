    ...
    return {
      ...
      input: {
        ...
        provider_parameters: {
          ...
          source_generation_task_id: sourceTask.id,
...
async function execute(task) {
  ...
  try {
    ...
    const review = object(result?.output?.output || result?.output || result);
    const actual = finite(
    ...
    return ProductionTaskRepository.update(task.id, {
      ...
      output: {
        provider: result?.provider || task.provider_id || null,
      ...
      metadata: {
        ...
        perceptual_review_frame_count: prepared.input.reference_images.length,
      },
      timing: {
        ...(task.timing || {}),
