...
const BOUNDED_FAST_CREATIVE_TASKS = new Set([
  "MUSIC_STUDIO_CRITIC_PANEL",
  "MUSIC_STUDIO_PREPRODUCTION_REPAIR",
]);
const BOUNDED_FAST_CREATIVE_TASK_OUTPUT_CAPS = Object.freeze({
  MUSIC_STUDIO_CRITIC_PANEL: 1800,
  MUSIC_STUDIO_PREPRODUCTION_REPAIR: 2400,
});

function supervisorOperationForTask(task) {
  const value = text(task);
  return BOUNDED_FAST_CREATIVE_TASKS.has(value)
    ? value
    : "CREATIVE_INTELLIGENCE_SUPERVISION";
}

function supervisorOutputBudgetForTask(task) {
  return Number(BOUNDED_FAST_CREATIVE_TASK_OUTPUT_CAPS[text(task)] || 2800);
}
...
async function settleGovernedFallbackExecution(execution, { organizationId, task }) {
  ...
  if (current?.pending === true) {
    await ServiceExecutionRuntime.cancelPending({
      ...
      usage_id: usageId,
      pricing: object(execution.pricing),
      metadata: { module: "CREATIVE", creative_task: task, intelligence_execution_lane: "deep" },
      execution_lane: "deep",
...
export async function reason({
  ...
  try {
    ...
    const ownedResult = assertOutputShape(owned.parsed?.result || owned.parsed, outputShape, "owned_supervisor");
    return {
      task,
  ...
  try {
    ...
    if (parsed) {
      const fallbackResult = assertOutputShape(parsed.result || parsed, outputShape, "governed_fallback");
      return {
        task,
  ...
  assertOutputShape(local.result, outputShape, "deterministic_local_fallback");
  return local;
}
