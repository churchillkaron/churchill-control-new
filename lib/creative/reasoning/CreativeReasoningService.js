...
export async function reason({
  ...
  if (text(task, 160).startsWith("MUSIC_STUDIO_")) {
    ...
    try {
      ...
      const result = assertOutputShape(parsed?.result || parsed, outputShape, "music_node01_local_structured");
      ...
      if (repairable) {
        try {
          ...
          const repairedResult = assertOutputShape(
            repairedParsed?.result || repairedParsed,
            outputShape,
