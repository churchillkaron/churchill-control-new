...
function positiveInteger(value, fallback, maximum, label) {
  ...
  if (number <= 0) {
    throw new Error(`CREATIVE_REASONING_BUDGET_POSITIVE_LIMIT_REQUIRED:${label}`);
  }
  const normalized = Math.floor(number);
  if (normalized > maximum) {
    throw new Error(
      `CREATIVE_REASONING_BUDGET_TECHNICAL_LIMIT_EXCEEDED:${label}:${normalized}:${maximum}`,
    );
  }
  return normalized;
}
...
function requestedOutputTokens(input = {}) {
  ...
  if (number === null || number <= 0) {
    throw new Error("CREATIVE_REASONING_REQUESTED_OUTPUT_TOKENS_INVALID");
  }
...
function preflight(budget, input = {}) {
  ...
  if (tokens > TECHNICAL_LIMITS.maximum_single_call_output_tokens) {
    throw new Error(
      `CREATIVE_REASONING_TECHNICAL_SINGLE_CALL_LIMIT_EXCEEDED:${operation}:${tokens}:${TECHNICAL_LIMITS.maximum_single_call_output_tokens}`,
    );
  }
...
function recordFailure(budget, input = {}, error) {
  budget.operations.push({
    operation: text(input.metadata?.operation) || "CREATIVE_REASONING",
    sequence: budget.call_count + 1,
    failed: true,
    ...
    const executeOne = async () => {
      ...
      try {
        ...
        if (!budget.failure) {
          ...
          recordFailure(budget, input, error);
        }
        throw error;
      }
    };
