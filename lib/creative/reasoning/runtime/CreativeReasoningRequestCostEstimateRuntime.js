...
function install() {
  ...
  ServiceExecutionRuntime.execute = async function executeWithReasoningEstimate(input = {}) {
    ...
    const upstreamInputTokens = finite(
      incomingGuard.estimated_input_tokens ?? incomingGuard.estimatedInputTokens,
    );
    const upstreamOutputTokens = finite(
      incomingGuard.estimated_output_tokens ?? incomingGuard.estimatedOutputTokens,
    );
    const upstreamQuantity = finite(
      incomingGuard.estimated_quantity ?? incomingGuard.estimatedQuantity,
    );
    const guardedInputTokens = Math.max(
      usageEstimate.estimated_input_tokens,
      upstreamInputTokens || 0,
    );
    const guardedOutputTokens = Math.max(
      usageEstimate.estimated_output_tokens,
      upstreamOutputTokens || 0,
    );
    const guardedQuantity = Math.max(
      usageEstimate.estimated_quantity,
      upstreamQuantity || 0,
    );
    const preservedUpstreamHeadroom =
      guardedInputTokens > usageEstimate.estimated_input_tokens ||
      guardedOutputTokens > usageEstimate.estimated_output_tokens ||
      guardedQuantity > usageEstimate.estimated_quantity;

    return executeWithoutEstimate({
      ...input,
      cost_guard: {
        ...incomingGuard,
        estimated_input_tokens: guardedInputTokens,
        estimated_output_tokens: guardedOutputTokens,
        estimated_quantity: guardedQuantity,
      },
      metadata: {
        ...
        creative_reasoning_estimated_quantity:
          usageEstimate.estimated_quantity,
        creative_reasoning_preserved_upstream_reservation_headroom:
          preservedUpstreamHeadroom,
        creative_reasoning_guarded_input_tokens: guardedInputTokens,
        creative_reasoning_guarded_output_tokens: guardedOutputTokens,
        creative_reasoning_guarded_quantity: guardedQuantity,
      },
    });
  };
}
