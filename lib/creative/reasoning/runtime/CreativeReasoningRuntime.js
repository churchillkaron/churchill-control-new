...
function buildInitialProduction({
  ...
  return {
    production_version: "creative-runtime-v3-avantiqo-intelligence-supervised",
    ...
    constraints: {
      ...
      intelligence_supervised: true,
      whole_production_quality: true,
    },
    knowledge: [],
    decision_trace: [],
    hypotheses: [],
    production_graph: {
      ...
      quality_standard: {
        ...
        require_story_coherence: true,
        require_audio_intent: true,
        require_channel_fit: true,
      },
    },
  };
}
...
async function addDecision(production, task, input, outputShape) {
  ...
  production.decision_trace.push({
    ...
    intelligence_supervision: result.intelligence_supervision || null,
    raw_reasoning_persisted: false,
...
function buildDraftScenes({ production, storyResult, sceneResult }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    scenes.push({
      ...
      sound_role: source.sound_role || null,
      narration_role: source.narration_role || null,
...
export async function CreativeReasoningThink({
  ...
  const strategy = await addDecision(
    production,
    "Find the strongest business and creative strategy for this objective. Define audience, desired response, emotional journey and the few assumptions that still require evidence.",
    {
  ...
  production.desired_response = strategy?.desiredResponse || null;
  ...
  production.assumptions = normalizeArray(strategy?.assumptions);
  production.risks = normalizeArray(strategy?.risks);

  const story = await addDecision(
    production,
    "Create multiple original story candidates and select the strongest one. Reject generic advertising structures when they do not fit the brand, evidence or objective.",
    {
      organization_id: organizationId,
      objective: production.objective,
      desired_response: production.desired_response,
  ...
  production.production_graph.selected_story = {
    ...
    rejected_directions: normalizeArray(story?.weakDirectionsRejected),
  };

  const soundDirection = await addDecision(
    production,
    "Design the sound and voice direction for the selected story before scene production. Decide when music, sound effects, silence, narration, spoken dialogue or transcription are actually needed. Use only supplied canonical capabilities and never generate audio just because the capability exists.",
    {
      organization_id: organizationId,
      objective: production.objective,
      audience: production.audience,
    ...
    },
    {
      result: {
        ...
        sfx_needed: "boolean",
        sfx_role: "string|null",
        narration_needed: "boolean",
        narration_role: "string|null",
        transcription_needed: "boolean",
        silence_strategy: "string|null",
        required_capabilities: ["exact supplied canonical capability string"],
      },
    },
  );

  production.production_graph.sound_direction = {
    music_needed: soundDirection?.music_needed === true,
    music_role: soundDirection?.music_role || null,
    sfx_needed: soundDirection?.sfx_needed === true,
    sfx_role: soundDirection?.sfx_role || null,
    narration_needed: soundDirection?.narration_needed === true,
    narration_role: soundDirection?.narration_role || null,
    transcription_needed: soundDirection?.transcription_needed === true,
    silence_strategy: soundDirection?.silence_strategy || null,
    required_capabilities: normalizeArray(soundDirection?.required_capabilities)
      .map((value) => canonicalCapability(value, null))
      .filter(Boolean),
  };

  const scenes = await addDecision(
    production,
    "Plan the scenes needed to express the selected story with minimum unnecessary generation. For every scene choose exactly one supplied canonical capability_needed and define the scene's sound/narration role when relevant. Never name or choose an AI vendor/provider.",
    {
      organization_id: organizationId,
      objective: production.objective,
      desired_response: production.desired_response,
      ...
      emotional_journey: production.production_graph.emotional_journey,
      selected_story: production.production_graph.selected_story,
      sound_direction: production.production_graph.sound_direction,
      duration_seconds: production.duration_seconds,
      budget_mode: production.budget_mode,
      asset_count: production.assets.length,
  ...
  const renderStrategy = await addDecision(
    production,
    "Decide a capability-level production strategy that maximizes final quality while avoiding unnecessary generation. Include how specialist Image, Cinema, Audio, Voice and Code capabilities should collaborate when required. Do not choose or discuss AI vendors/providers.",
    {
      organization_id: organizationId,
      objective: production.objective,
    ...
    },
    {
      result: {
        ...
        specialistSequence: ["exact supplied canonical capability string"],
  ...
  production.production_graph.render_strategy = {
    ...
    specialist_sequence: normalizeArray(renderStrategy?.specialistSequence)
      .map((value) => canonicalCapability(value, null))
      .filter(Boolean),
    execution_policy: "CAPABILITY_ONLY_SERVICE_RUNTIME_OWNED_FIRST",
    provider_selection_exposed: false,
    quality_risks: normalizeArray(renderStrategy?.qualityRisks),
