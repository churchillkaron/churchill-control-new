...
function capabilitiesFor(contract = {}) {
  const mediaKind = text(contract.source_media_kind || contract.media_kind || 'IMAGE').toUpperCase();
  const cameraCapability = mediaKind === 'VIDEO'
    ? CREATIVE_TOOL_CAPABILITIES.CAMERA_TRACK
    : CREATIVE_TOOL_CAPABILITIES.SINGLE_VIEW_CAMERA_PROXY;
  const segmentationCapability = mediaKind === 'VIDEO'
    ? CREATIVE_TOOL_CAPABILITIES.SEGMENTATION
    : CREATIVE_TOOL_CAPABILITIES.IMAGE_SEGMENTATION;
  return Object.freeze([
    ['camera_solution', cameraCapability],
...
function readiness(contract = {}) {
  ...
  const capabilityDefinitions = capabilitiesFor(contract);
  const capabilities = Object.fromEntries(
    capabilityDefinitions.map(([key, capability]) => [key, resolveCapability(capability)]),
  );
  const requiredKeys = [];
  ...
  return {
    ...
    source_media_kind: text(contract.source_media_kind || contract.media_kind || 'IMAGE').toUpperCase(),
