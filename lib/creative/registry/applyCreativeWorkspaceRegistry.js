...
function preserveCommercialDesignOwnership({ commercial, groups, index, group }) {
  if (!commercial || index < 0 || !group) return;

  const items = list(group.items).map((item) => {
    if (
      item?.id !== "design_studio" &&
      item?.route !== "/commercial/design"
    ) {
    ...
    return {
      ...
      description:
        "Creative Studio hub for autonomous production plus Image, Video, Voice, Music, Code, Marketing and Web specialist studios.",
      order: 30,
      status: "active",
      hidden: false,
    };
  ...
  groups[index] = {
    ...group,
    description:
      "Campaigns, assets, publishing, growth execution and the complete Avantiqo Design studio hub.",
    items,
  };

  commercial.groups = groups;
}
...
export function applyCreativeWorkspaceRegistry(registry) {
  ...
  preserveCommercialDesignOwnership(commercialMarketing);
