...
function identity(subject, scope, approver, approvedCostCeiling = null) {
...
function currentApproval(node, subject, scope) {
  ...
  if (subject.type !== CREATIVE_ASSET_NODE_TYPES.PRODUCTION_DOSSIER) return true;
...
export const CreativeApprovalRuntime = {
  ...
  async approve({
    ...
    const approvalIdentity = identity(subject, scope, approver, ceiling);
    const nodes = await AssetGraphRepository.listByProject({
      organization_id,
      creative_project_id: subject.creative_project_id,
    });
