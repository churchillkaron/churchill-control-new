...
import {
  CreativeProductionGraphApprovalMutationRuntime,
} from "@/lib/creative/production-graph/runtime/CreativeProductionGraphApprovalMutationRuntime";
import {
...
async function approveProductionDossierSubject({ subject, approval, ceiling }) {
  ...
  const approvedCurrency = text(
    subject.metadata?.currency || graph.cost_plan?.currency,
  ).toUpperCase();
  await CreativeProductionGraphApprovalMutationRuntime.approve(graph.id, {
    status: "APPROVED",
    cost_plan: {
      ...(graph.cost_plan || {}),
      currency: approvedCurrency || graph.cost_plan?.currency || null,
    ...
    metadata: {
      ...
      approved_execution_hash: subject.metadata?.execution_hash || null,
      approved_cost_ceiling: ceiling,
      approved_cost_currency: approvedCurrency || null,
      production_currency: approvedCurrency || graph.metadata?.production_currency || null,
