...
function exactAudioState(master = {}) {
  return {
    ...
    audio_studio_qc_sealed:
      master.metadata?.audio_studio_qc_sealed === true,
    audio_studio_qc_seal_hash:
      master.metadata?.audio_studio_qc_seal_hash || null,
    audio_studio_finish_task_id:
      master.metadata?.audio_studio_finish_task_id || null,
    audio_studio_master_id:
      master.metadata?.audio_studio_master_id || null,
    audio_studio_master_checksum:
      master.metadata?.audio_studio_master_checksum || null,
  };
}
...
function qcFor(master, snapshot, picture, audio) {
  ...
  const checks = [
    ...
    {
      id: "audio_studio_qc_seal_preserved_when_applicable",
      passed:
        !soundtrackRequired ||
        (
          audio.audio_studio_qc_sealed === true &&
          /^[a-f0-9]{64}$/i.test(text(audio.audio_studio_qc_seal_hash))
        ),
      evidence: {
        audio_studio_qc_sealed: audio.audio_studio_qc_sealed,
        audio_studio_qc_seal_hash: audio.audio_studio_qc_seal_hash,
        audio_studio_finish_task_id: audio.audio_studio_finish_task_id,
      },
    },
  ];
