...
import {
  buildPublicationContentBinding,
} from "@/lib/creative/release/runtime/CreativePublicationContentBindingRuntime";
...
function executionBriefPublicationText(brief = {}) {
  const content = brief?.content;
  if (!content || typeof content !== "object" || Array.isArray(content)) return null;
  const value = content.message ?? content.caption ?? content.text;
  return value === undefined || value === null ? null : String(value);
}

function commandIdentity(readiness, releasePackage, derivative, target, contentBinding, executionBriefIdentity = null) {
  return crypto.createHash("sha256").update(JSON.stringify({
    ...
    publication_content_binding_identity: contentBinding.identity,
    execution_brief_identity: executionBriefIdentity,
  })).digest("hex");
}
...
export const CreativePublishCommandRuntime = {
  async create({
    ...
    const requestedPublicationText = executionBriefPublicationText(execution_brief);
    const publicationTarget = requestedPublicationText === null
      ? target
      : {
          ...target,
          metadata: {
            ...(target.metadata || {}),
            message: requestedPublicationText,
          },
        };
    ...
    const targetChannel = normalized(publicationTarget.channel || publicationTarget.id || publicationTarget.key);
    ...
    const contentBinding = buildPublicationContentBinding({
      target: publicationTarget,
      derivative,
      derivative_render: derivativeRender,
    });
    const safeExecutionBrief = execution_brief ? safeObject(execution_brief) : null;
    const executionBriefIdentity = safeExecutionBrief
      ? crypto.createHash("sha256").update(JSON.stringify(safeExecutionBrief)).digest("hex")
      : null;
    ...
    const command = createCreativeAssetNode({
      ...
      description: "Connector-neutral approved publish command bound to the current certified release-package derivative and approved publication-content identity.",
      lineage: {
      ...
      metadata: {
        ...
        publication_content_binding: contentBinding,
        publication_content_binding_identity: contentBinding.identity,
        execution_brief: safeExecutionBrief,
        execution_brief_identity: executionBriefIdentity,
        approved_publication_text_digest: contentBinding.approved_text_digest,
        approved_publication_text_length: contentBinding.approved_text_length,
        ...
        publish_target_id,
        publish_target: safeTarget(publicationTarget),
        execution_brief: safeExecutionBrief,
        execution_brief_identity: executionBriefIdentity,
