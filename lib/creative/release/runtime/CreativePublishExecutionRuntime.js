...
async function settleExisting({
  ...
  const completed = hasDeliveryEvidence(evidence);
  const updated = await AssetGraphRepository.update(execution.id, {
    ...
    metadata: {
      ...metadata,
      ...evidence,
      execution_status: completed ? "COMPLETED" : "EVIDENCE_REQUIRED",
      settlement: result.settlement || "CHARGED",
      usage_id: result.usage?.id || metadata.usage_id || null,
      billing_invoice_id:
        result.billing?.invoice?.id ||
        metadata.billing_invoice_id ||
        null,
      last_polled_at: new Date().toISOString(),
      completed_at: new Date().toISOString(),
      executed_by_user_id: executed_by.user_id,
  ...
  await updateCommand(command, {
    ...
    external_publication_url: evidence.external_publication_url,
  });
...
export const CreativePublishExecutionRuntime = {
  async execute({
    ...
    const existing = newest(nodes.filter((node) =>
    ...
    try {
      ...
      const created = await AssetGraphRepository.create(failed);
      await updateCommand(command, {
        execution_status: "FAILED",
        publish_execution_asset_node_id: created.id,
        publication_error: error.message,
      });
      return { execution: created, reused: false, settled: true };
    }
    ...
    const execution = createCreativeAssetNode({
      ...
      description: pending
        ...
        : completed
          ? "Publish connector returned external delivery evidence."
          : "Publish connector completed without a verifiable external publication identifier or URL.",
      lineage: {
        source: "publish_connector_execution",
    ...
    const created = await AssetGraphRepository.create(execution);
    await updateCommand(command, {
      execution_status: executionStatus,
      publish_execution_asset_node_id: created.id,
      external_publication_id: evidence.external_publication_id,
