...
function normalizedResult(payload = {}) {
  ...
  if (evidence.external_publication_id || SUCCESS.has(status)) {
...
function identities(execution = {}) {
  ...
  const providerJobId = text(execution.metadata?.provider_job_id) || null;
  ...
  if (
    !providerJobId &&
    execution.metadata?.execution_status !== "RECONCILIATION_REQUIRED"
  ) {
    throw new Error("PUBLISH_PROVIDER_JOB_REQUIRED");
  }
...
async function settleFailure({ execution, claimed, normalized, usage, pricing }) {
  if (usage.status === "SUCCESS") {
    throw new Error("PUBLISH_USAGE_ALREADY_SUCCESSFUL");
  }
  ...
  if (usage.status !== "FAILED") {
    await UsageRuntime.fail({
      ...
      error: new Error(
        boundedError(
          normalized.error?.message || normalized.error || normalized.status,
  ...
  return AssetGraphRepository.settlePublishReconciliation({
    ...
    evidence: {
      ...
      error: boundedError(normalized.error?.message || normalized.error),
      settlement: "RELEASED",
      usage_id: usage.id,
    },
  });
...
async function settleSuccess({ execution, claimed, normalized, usage, pricing }) {
  if (!normalized.evidence.external_publication_id) {
    throw new Error("EXTERNAL_PUBLICATION_ID_REQUIRED");
  }
  if (usage.status === "FAILED") {
    throw new Error("PUBLISH_USAGE_ALREADY_FAILED");
  }
...
async function apply({ execution, payload }) {
  ...
  if (normalized.state === "PENDING") {
    if (!providerJobId) {
      throw new Error("PUBLISH_PROVIDER_JOB_REQUIRED_FOR_PROGRESS");
    }
...
export const CreativePublishReconciliationRuntime = {
  ...
  async poll({ organization_id, execution_id }) {
    ...
    if (!providerJobId) {
      throw new Error("PUBLISH_PROVIDER_JOB_REQUIRED_FOR_POLLING");
    }
