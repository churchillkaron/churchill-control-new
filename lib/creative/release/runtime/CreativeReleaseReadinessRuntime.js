...
function identity(project, timeline, render, evidence) {
  return crypto.createHash("sha256").update(JSON.stringify({
    ...
    evidence: evidence.map((node) => ({
      ...
      passed:
        node.metadata?.passed ??
        node.metadata?.technical_qc?.passed ??
        null,
    })),
  })).digest("hex");
}
...
export const CreativeReleaseReadinessRuntime = {
  async evaluate({
    ...
    const requireHumanRenderApproval = releasePolicy.require_human_render_approval !== false;
    ...
    const technicalQcPassed = render?.metadata?.technical_qc?.passed === true;
    const perceptualReport = render
      ? newest(nodes, (node) =>
          node.type === CREATIVE_ASSET_NODE_TYPES.QUALITY_REPORT &&
          node.parent_asset_node_id === render.id &&
          node.metadata?.report_kind === "PERCEPTUAL_MEDIA_QUALITY",
        )
      : null;
    ...
    const checks = [
      check(
        ...
        Boolean(timeline),
        timeline?.id || null,
        "No release timeline exists.",
      ),
      check(
        "timeline_requirements_complete",
        true,
        list(timeline.metadata?.missing_requirements).length === 0,
        timeline.metadata?.missing_requirements || [],
        "Timeline has unresolved production requirements.",
      ),
      check(
        "final_render_present",
        requireRender,
        Boolean(render),
        render?.id || null,
        "No final render exists for the selected timeline.",
      ),
      check(
        "final_render_not_rejected",
        requireRender,
        Boolean(render) && render.status !== CREATIVE_ASSET_NODE_STATUS.REJECTED,
        render?.status || null,
        "Final render is rejected.",
      ),
      check(
        "technical_qc_passed",
        requireTechnicalQc,
        technicalQcPassed,
        render?.metadata?.technical_qc || null,
        "Technical quality control has not passed.",
      ),
      check(
        "perceptual_qc_passed",
        requirePerceptualQc,
        perceptualReport?.metadata?.passed === true,
        perceptualReport?.id || null,
        "Perceptual quality control has not passed.",
      ),
      check(
        "release_gate_passed",
        requireReleaseGate,
        releaseGateReport?.metadata?.passed === true,
        releaseGateReport?.id || null,
        "Rights, consent, licence or identity release gate has not passed.",
      ),
      check(
        "release_gate_human_approved",
        requireHumanReleaseGateApproval,
        releaseGateReport?.review?.approved === true &&
          Boolean(releaseGateReport?.review?.approved_by),
        releaseGateReport?.review || null,
        "Release-gate evidence lacks authenticated human approval.",
      ),
      check(
        "final_render_human_approved",
        requireHumanRenderApproval,
        render?.review?.approved === true && Boolean(render?.review?.approved_by),
        render?.review || null,
        "Final render lacks authenticated human approval.",
      ),
      check(
        "no_open_repair_plan",
        true,
        !openRepairPlan,
        openRepairPlan?.id || null,
        "An unresolved repair plan remains attached to the final render.",
      ),
      check(
        "publish_targets_configured",
        releasePolicy.require_publish_targets === true,
        list(project.metadata?.publish_targets).length > 0,
    ...
    const evidence = [timeline, render, perceptualReport, releaseGateReport, openRepairPlan]
      .filter(Boolean);
    ...
    const report = createCreativeAssetNode({
      ...
      description: "Immutable release-readiness audit across timeline, evidence, render and quality gates.",
      lineage: {
      ...
      review: {
        ...
        approved: false,
        notes: passed
          ? "All configured release checks passed; publishing still requires authenticated human release approval."
