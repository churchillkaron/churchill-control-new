...
const CONTRACT = "CREATIVE_TEMPORAL_CHANNEL_DELIVERY_V3";
...
async function markDerivative({
  ...
  if (!render?.id) return render;
  return AssetGraphRepository.update(render.id, {
    metadata: {
      ...
      source_final_mastering_report_id: finalMastering.report?.id || null,
      source_final_mastering_seal_hash:
        finalMastering.seal?.seal_hash ||
        finalMastering.report?.metadata?.final_mastering_seal_hash ||
        null,
...
async function persistReport({
  ...
  return AssetGraphRepository.create(createCreativeAssetNode({
    ...
    description:
      "Deterministic delivery proof for every configured target channel, bound to the immutable final-master seal, export profile, exact derivative checksum, technical conformance and delivery-audio evidence.",
    lineage: {
    ...
    review: {
      ...
      approved: false,
      notes: passed
        ? "Every target-channel derivative is bound to the certified final master and passed deterministic delivery checks."
        : "One or more delivery or final-master checks failed.",
    },
    metadata: {
      contract: CONTRACT,
      ...
      derivatives_bound_to_exact_master_checksum: true,
