...
const CONTRACT = "CREATIVE_TEMPORAL_CHANNEL_DELIVERY_V1";
...
function deliveryPassed(result = {}) {
...
async function persistReport({
  ...
  return AssetGraphRepository.create(createCreativeAssetNode({
    ...
    description:
      "Deterministic delivery proof for every configured target channel, including export-profile resolution, technical QC, professional finishing and final-master audio integrity.",
    lineage: {
    ...
    intelligence: {
      ...
      tags: ["channel-delivery", "deterministic", "technical-qc", "final-audio"],
    },
    ...
    review: {
      ...
      approved: false,
      notes: passed
        ? "Every configured target-channel derivative passed deterministic delivery checks."
        : "One or more configured target-channel derivatives failed deterministic delivery checks.",
    },
    metadata: {
      contract: CONTRACT,
  ...
  async deliver({
    ...
    for (const channel of channels) {
      try {
        ...
        const passed = deliveryPassed(rendered);
        deliveries.push({
          channel,
          passed,
          ...
          render_asset_node_id: rendered.render?.id || null,
          render_url: rendered.render?.url || null,
          render_checksum: rendered.render?.technical?.checksum || null,
          technical_qc_passed: rendered.technical_qc?.passed === true,
          professional_finishing_contract:
            rendered.render?.metadata?.professional_finishing_contract ||
            rendered.professional_finishing?.contract || null,
          final_master_audio_verified: Boolean(
            rendered.render?.metadata?.final_master_audio_verified === true ||
            rendered.professional_final_audio_integrity?.passed === true,
          ),
          error: passed ? null : "CHANNEL_DELIVERY_INTEGRITY_FAILED",
        });
      } catch (error) {
        deliveries.push({
          channel,
    ...
    const passed = deliveries.every((delivery) => delivery.passed);
