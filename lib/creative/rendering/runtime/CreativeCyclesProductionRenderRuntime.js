...
export async function renderCyclesProduction({project,scene}={}){
  ...
  const rigged=CreativeMechanicalRigRuntime.compile({objects:baseCfg.objects,rigs:list(scene?.rigs)});
  ...
  try{
    ...
    const renderCfg={...cfg,objects:modelBinding.objects,material_library:textureBinding.library};
