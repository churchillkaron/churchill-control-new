...
export async function buildResearchPlan(
  ...
  return [
    ...
    ...(benchmarkTargets.length ? [{
      id: "benchmark_reference_films",
      objective: "Research every declared benchmark as evidence for transferable craft mechanisms, never as a template to copy.",
      required: true,
      subjects: benchmarkTargets,
      evidence: [
        "credible or first-party source evidence identifying each declared benchmark film or campaign",
        "observable narrative, editing, cinematography, visual beauty, humanity, place, sound and production-craft mechanisms",
        "transferable craft principles plus explicit anti-copy boundaries",
      ],
    }] : []),
    {
    ...
    {
      id: "mission_grounding",
      objective: "Infer the exact real-world subjects and states this specific creative mission must preserve, then gather sufficient evidence for downstream generation and review without using a category template.",
      required: true,
      evidence: [
        "mission-derived entities, locations, people, products, events, documents, offers, data or other factual subjects when relevant",
        "authentic visual/reference sources for every materially distinct subject, waypoint or state that Studio must preserve",
        "explicit uncertainty and rejection evidence when a candidate source is ambiguous, generic, stale, wrong-entity or otherwise unsafe to ground production",
      ],
    },
    {
