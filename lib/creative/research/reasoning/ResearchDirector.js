...
export async function buildResearchPlan(
  ...
  // A creative project title is not a company identity.
  //
  // The one-clip proof named its project "8 second clip for Churchill when staff welcome a
  // customer in the entrance". Falling back to project.name therefore asked web research to
  // resolve that creative request as if it were a business name. Several paid attempts then
  // resolved Avantiqo/Aventiqo instead of Churchill and never reached strategy or production.
  //
  // The authoritative organization record wins. Explicit company/brand fields remain valid for
  // an organization producing work for a named brand, but the generic project title can never
  // impersonate the company again.
  const namedCompany = text(
    organizationIdentity.name ||
    organizationIdentity.legal_name ||
