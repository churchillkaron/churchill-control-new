...
export const AutonomousResearchDirectorRuntime = {
  async run({
    ...
    const contextIdentity = researchContextIdentity({
      contract: "CREATIVE_RESEARCH_CONTEXT_V2",
    ...
    try {
      result = await ServiceExecutionRuntime.execute({
        ...
        input: {
          ...
          // Web evidence remains mandatory, but the provider is allowed to formulate
          // focused search queries. Acceptance below requires a real web_search_call plus
          // the source/claim evidence contract, so skipping search still fails closed.
          quantity: 1,
