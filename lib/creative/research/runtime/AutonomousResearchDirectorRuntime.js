    ...
    try {
      result = await ServiceExecutionRuntime.execute({
        ...
        input: {
          ...
          // No response_format here, deliberately. Research is validated by parsing its output as JSON and
          // my first instinct was to request json_object, which the provider refuses outright when tools are
          // in play: "Web Search cannot be used with JSON mode". The prompt already demands strict JSON with
          // no markdown, so the fix belongs in reading the response rather than in constraining it -- see
          // unwrapResearchOutput, which now handles the array of output items that tool calls produce.
          quantity: 1,
