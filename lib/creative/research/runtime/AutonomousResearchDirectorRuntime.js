...
export const AutonomousResearchDirectorRuntime = {
  async run({
    ...
    try {
      ...
      const chargedPrice = Number(error?.customer_price ?? error?.charged_customer_price ?? 0);
      const unchargedFailure = !Number.isFinite(chargedPrice) || chargedPrice <= 0;
      await updateResearchApproval(scopedProject, approval, {
        approved: unchargedFailure ? true : false,
        status: "EXECUTION_FAILED",
        consumed_at: unchargedFailure ? null : new Date().toISOString(),
        charged_customer_price: Number.isFinite(chargedPrice) ? chargedPrice : 0,
