  ...
  async run({ organization_id, mission = {}, project = {}, brief = {}, assets = [], force = false } = {}) {
    ...
    try {
      if (benchmarkBaseReport && benchmarkTargets.length) {
        ...
        const localBenchmark = localResearchSynthesisEnabled();
        const estimatedInput = localBenchmark
          ? Math.max(1, Math.ceil(prompt.length / 3.2))
          : Math.max(3000, Math.ceil(prompt.length / 3));
        const estimatedOutput = localBenchmark ? 2200 : 3600;
        const benchmarkResult = await awaitResearchExecution(await ServiceExecutionRuntime.execute({
          organization_id,
          service_id: "ai.reasoning.execute",
          provider_id: localBenchmark ? "avantiqo-intelligence" : approval.provider,
          category: "CREATIVE_RESEARCH",
          input: {
          ...
          provider_policy: {
            allowed_providers: [localBenchmark ? "avantiqo-intelligence" : approval.provider],
            preferred_providers: [localBenchmark ? "avantiqo-intelligence" : approval.provider],
            preferred_models: localBenchmark ? [AVANTIQO_INTELLIGENCE_LOCAL_MODEL] : approval.model ? [approval.model] : [],
            ...(localBenchmark
              ? {
                  allowed_models: [AVANTIQO_INTELLIGENCE_LOCAL_MODEL],
                  execution_scope: "BENCHMARK_REVIEW_PREVIEW",
