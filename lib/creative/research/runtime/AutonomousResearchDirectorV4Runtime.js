  ...
  async run({ organization_id, mission = {}, project = {}, brief = {}, assets = [], force = false } = {}) {
    ...
    try {
      ...
      for (const priorUsage of priorStructuredUsages) {
        const recovered = structuredResultFromUsage(priorUsage);
        try {
          let recoveredValidation = normalizeAndValidateResearch({
            ...
            policy,
            context_identity: contextIdentity,
            researched_at: dossier.researched_at || priorUsage.updated_at || priorUsage.created_at || currentDate,
          });
          recoveredValidation = validateEvidenceBinding(recoveredValidation, dossier);
          if (policy.mode !== "CREATIVE_GROUNDING") {
          ...
          // A stored provider success is only reusable when it passes the current contract.
        }
      }
