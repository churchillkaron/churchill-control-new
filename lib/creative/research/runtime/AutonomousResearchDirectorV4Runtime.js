...
- For premium temporal/video work, creative_grounding.benchmark_lab is mandatory when PUBLIC WEB EVIDENCE contains relevant reference films or campaign-film sources. Study at least three distinct references when available. Deconstruct each reference across narrative, editing, cinematography, visual_beauty, humanity, place, sound and production_craft. Extract transferable craft principles only: never copy shots, branded compositions, scripts, music, logos, character identity, or distinctive protected expression. craft_dna must contain at least eight transferable_principles plus at least three anti_copy_rules, sound_principles, editorial_principles and cinematography_principles. A benchmark is not a mood label: explain the observable craft mechanism and what decision it should change downstream.
    ...
    try {
      ...
      for (const priorUsage of priorStructuredUsages) {
        ...
        try {
          ...
          recoveredValidation = validateEvidenceBinding(recoveredValidation, recoveredDossier);
          if (policy.mode !== "CREATIVE_GROUNDING") {
          ...
          if (policy.require_benchmark_lab) {
            try {
              ...
              relaxedValidation = validateEvidenceBinding(relaxedValidation, recoveredDossier);
              if (policy.mode !== "CREATIVE_GROUNDING") {
      ...
      validated = validateEvidenceBinding(validated, finalDossier);
      if (policy.mode !== "CREATIVE_GROUNDING") {
