...
import { AVANTIQO_INTELLIGENCE_LOCAL_MODEL } from "@/lib/platform/service-runtime/providers/avantiqo-intelligence/AvantiqoIntelligenceLocalPolicy.js";
...
function compactLocalResearchPrompt({ context, plan, policy, currentDate, dossier }) {
  ...
  const compactContext = {
    organization: {
      canonical_name: organization.canonical_name || organization.name || null,
      official_website: organization.official_website || null,
    ...
    project: {
      ...
      objective: project.objective || null,
      workflow_kind: project.metadata?.workflow_kind || null,
    },
    brief: {
      objective: brief.objective || brief.summary || null,
      constraints: list(brief.constraints).slice(0, 8),
    },
  };
  const compactSources = list(dossier.sources).slice(0, 6).map((source, index) => ({
    ...
    source_type: text(source.source_type || "public_web_source", 80),
    official: source.official === true,
    primary: source.primary === true,
    retrieved_at: source.retrieved_at || dossier.researched_at || currentDate,
    excerpt: text(source.excerpt, 420),
  }));
  const compactPlan = list(plan).slice(0, 8).map((item) => ({
    id: text(item?.id, 80),
    objective: text(item?.objective || item?.description, 240),
    subjects: list(item?.subjects).slice(0, 4).map((value) => text(value, 120)),
  }));
  return `Return exactly one compact JSON object for Avantiqo Creative Studio research. Use ONLY the supplied context and evidence; never invent facts or URLs. Preserve source ids and exact URLs. Keep every field concise.

CONTEXT
${JSON.stringify(compactContext)}

EVIDENCE
${JSON.stringify({ researched_at: dossier.researched_at || currentDate, sources: compactSources })}

PLAN
${JSON.stringify(compactPlan)}

REQUIRED TOP-LEVEL KEYS
summary, company_resolution, company_truth, brand_intelligence, audience, competitor_analysis, market, commercial_intelligence, messaging, strategic_synthesis, creative_grounding, creative_opportunities, recommendations, trends, keywords, sources, claims, confidence.

RULES
- company_resolution is an object with status, canonical_name, official_website, location, industry, matched_internal_identity, reasoning.
- audience is an object. competitor_analysis is an object with competitors.
- strategic_synthesis must include strategic_problem, strategic_opportunity, creative_mandate, human_truths, category_conventions, breakable_conventions, competitor_patterns, distinctive_brand_assets, cultural_context, attention_opportunities, contradictions, must_not_do.
- strategic_problem, strategic_opportunity and creative_mandate are objects with statement, creative_consequence, misuse_risk, confidence, source_ids.
- creative_grounding must include mode, truth_sensitivity, reasoning, entities, evidence_targets, reference_candidates, continuity_constraints. Include spatial_path only when evidence supports a real route.
- sources must reuse EVIDENCE ids/URLs exactly. claims must cite source_ids and distinguish verified/partial/unresolved.
- Include at least ${Math.min(4, Number(policy.minimum_external_sources || 0))} external sources and ${Math.min(6, Number(policy.minimum_verified_claims || 0))} verified claims when supported.
- Do not create a story, storyboard, campaign concept, provider prompt or shot plan.
- Preserve uncertainty instead of fabricating evidence.
...
function bindLocalResearchEvidence(result = {}, dossier = {}, context = {}) {
  ...
  if (officialWebsite && !sources.some((source) => canonicalUrl(source.url) === officialWebsite)) {
    sources.unshift({
      ...
      title: text(organization.canonical_name || organization.name || "Official website"),
      url: officialWebsite,
      publisher: text(organization.canonical_name || organization.name),
  ...
  const entities = list(grounding.entities).map((entry, index) => {
    ...
    return { ...bound, name: text(bound.name || bound.subject || bound.statement || bound.id), role: text(bound.role || "MISSION_GROUNDING_SUBJECT") };
  });
  const evidenceTargets = list(grounding.evidence_targets || grounding.evidenceTargets).map((entry, index) => {
    ...
    return { ...bound, subject: text(bound.subject || bound.statement || bound.id), status: text(bound.status || "SUPPORTED").toUpperCase(), required: bound.required === true };
  });
  const confidenceValue = payload.confidence;
  const confidenceText = text(object(confidenceValue).overall ?? confidenceValue).toUpperCase();
  const confidence = Number.isFinite(Number(object(confidenceValue).overall ?? confidenceValue))
    ? Number(object(confidenceValue).overall ?? confidenceValue)
    : confidenceText === "HIGH" ? 85 : confidenceText === "MEDIUM" ? 70 : confidenceText === "LOW" ? 40 : 75;
  ...
  const claims = list(payload.claims).map((claim, index) => {
    ...
    return {
      ...
      notes: requestedVerified && !sourceIds.length
  ...
  const verifiedClaimCount = claims.filter((claim) =>
    claim?.verified === true &&
    list(claim?.source_ids).length > 0
  ).length;
  if (verifiedClaimCount < 2) {
    for (const claim of authoritativeClaims) {
      ...
      if (claims.filter((entry) => entry?.verified === true && list(entry?.source_ids).length > 0).length >= 2) break;
    }
  }
  ...
  const missionConstraint = text(
    ...
    object(context.brief).objective ||
  ...
  return {
    ...
    audience: { ...audience, evidence_source_ids: audienceIds.length ? audienceIds : primaryId ? [primaryId] : ["authoritative_context"] },
    market: { ...market, evidence_source_ids: marketIds.length ? marketIds : primaryId ? [primaryId] : ["authoritative_context"] },
    strategic_synthesis: {
      ...strategic,
...
export function sealedResearchRoomStructuredResult({ project = {}, identity = {}, policy = {} } = {}) {
  const state = object(project.metadata?.creative_preproduction_state);
  if (text(state.organization_id) !== text(project.organization_id)) return null;
  if (text(state.creative_project_id) !== text(project.id)) return null;
  const stages = list(state.production_room_pipeline?.stages);
  const room = stages.find((stage) => text(stage?.id).toUpperCase() === "RESEARCH_ROOM");
  if (text(room?.status).toUpperCase() !== "SEALED" || room?.report?.passed !== true) return null;
  const evidence = object(room.report?.evidence || room.report?.workstream_report?.evidence);
  const manifest = list(evidence.source_manifest);
  const officialSource = manifest.find((entry) => /official|authoritative/i.test(text(entry?.source_id))) || manifest[0];
  const officialId = text(officialSource?.source_id) || "authoritative-official-website";
  const officialUrl = canonicalUrl(officialSource?.url) || canonicalUrl(identity.official_website) || canonicalUrl(project.metadata?.official_website);
  if (!officialUrl) return null;
  const canonicalName = text(identity.canonical_name || project.metadata?.organization_name || project.metadata?.organization_legal_name);
  if (!canonicalName) return null;
  const location = text(identity.address || identity.country || evidence.location_findings?.location || project.metadata?.organization_country || "TH");
  const industry = text(project.metadata?.organization_industry || evidence.location_findings?.industry || "Business Operating System / Enterprise Software");
  const sourceIds = [officialId];
  const supported = (id, statement, consequence, risk) => ({
    id, statement, creative_consequence: consequence, misuse_risk: risk, confidence: 95, source_ids: sourceIds,
  });
  const recoveredSources = manifest
    .map((entry, index) => ({
      id: text(entry?.source_id) || `source-${index + 1}`,
      title: text(entry?.claim) || `Research source ${index + 1}`,
      url: canonicalUrl(entry?.url),
      publisher: /official|authoritative/i.test(text(entry?.source_id)) ? canonicalName : text(entry?.claim) || canonicalName,
      source_type: /official|authoritative/i.test(text(entry?.source_id)) ? "official_website" : "public_web_source",
      retrieved_at: state.persisted_at || new Date().toISOString(),
      published_at: null,
      official: /official|authoritative/i.test(text(entry?.source_id)),
      primary: /official|authoritative/i.test(text(entry?.source_id)),
      internal: false,
      excerpt: text(entry?.claim),
    }))
    .filter((entry) => entry.url);
  const payload = {
    summary: `${canonicalName} is the verified organization subject for this creative mission. Reuse the sealed Research Room evidence and preserve factual identity while developing the film.`,
    company_resolution: {
      status: "RESOLVED", canonical_name: canonicalName, official_website: officialUrl, location, industry,
      matched_internal_identity: canonicalName, reasoning: "Recovered from the sealed Research Room and authoritative organization context.",
    },
    company_truth: { canonical_name: canonicalName, official_website: officialUrl, industry },
    brand_intelligence: { source_ids: sourceIds },
    audience: {
      evidence_source_ids: sourceIds,
      context: text(evidence.cultural_findings?.context),
      needs: list(evidence.cultural_findings?.needs),
      language: text(evidence.cultural_findings?.language),
    },
    competitor_analysis: { competitors: [] },
    market: { evidence_source_ids: sourceIds },
    commercial_intelligence: {},
    messaging: {},
    strategic_synthesis: {
      strategic_problem: supported("strategic-problem", "The creative work must remain recognizably tied to the verified Avantiqo organization instead of collapsing into generic enterprise-software imagery.", "Preserve Avantiqo identity and real organizational cues in every downstream creative decision.", "Generic imagery can satisfy category language while losing the verified subject."),
      strategic_opportunity: supported("strategic-opportunity", "Use the verified Avantiqo identity and the mission's human-centered premise to create a distinctive cinematic experience without inventing unsupported external claims.", "Build emotion and specificity from verified identity plus the approved creative brief, not from fabricated market facts.", "Unsupported claims or borrowed category tropes would weaken trust and originality."),
      creative_mandate: supported("creative-mandate", "Create a world-class 60-second Avantiqo film whose story, craft and reveal are grounded in the verified organization and the approved mission constraints.", "Keep the Avantiqo reveal earned by the story and preserve factual brand identity.", "A disconnected reveal or generic AI montage would break mission continuity."),
      human_truths: [supported("human-truth-1", "The approved mission is explicitly human-centered: emotion, wonder, tension and curiosity must be carried by observable human experience rather than abstract software spectacle.", "Prioritize human consequence, performance and material reality.", "Abstract technology shorthand can erase the human center of the film.")],
      category_conventions: [supported("category-convention-1", "Enterprise-software films commonly risk becoming generic product demos or anonymous category montage, both explicitly rejected by this mission.", "Use narrative causality and cinematic craft rather than UI-tour conventions.", "Falling back to generic software advertising would violate the approved brief.")],
      breakable_conventions: [], competitor_patterns: [], distinctive_brand_assets: [], cultural_context: [], attention_opportunities: [], contradictions: [],
      must_not_do: ["Do not invent unsupported external facts or claims.", "Do not turn the film into a generic product demo or random AI montage.", "Do not detach the Avantiqo reveal from the approved human-centered story."],
    },
    creative_grounding: {
      mode: "NONE", truth_sensitivity: "LOW",
      reasoning: "Organization identity is already verified and sealed; downstream creative development must preserve it while no route-specific real-world grounding is required.",
      entities: [], evidence_targets: [], reference_candidates: [],
      continuity_constraints: ["Preserve the verified Avantiqo organization identity and official website evidence across downstream creative decisions."],
    },
    creative_opportunities: [], recommendations: [], trends: [], keywords: ["Avantiqo", "business operating system", "cinematic brand film"],
    sources: recoveredSources.length ? recoveredSources : [{
      id: officialId, title: text(officialSource?.claim) || canonicalName, url: officialUrl, publisher: canonicalName,
      source_type: "official_website", retrieved_at: state.persisted_at || new Date().toISOString(), published_at: null,
      official: true, primary: true, internal: false, excerpt: text(officialSource?.claim) || canonicalName,
    }],
    claims: [
      { id: "claim-organization-identity", claim: `The governed organization identity is ${canonicalName}.`, category: "organization_identity", source_ids: sourceIds, confidence: 100, verification_status: "VERIFIED", verified: true, public_usable: false, sensitive: false, expires_at: null, notes: "Recovered from sealed Research Room evidence." },
      { id: "claim-official-website", claim: `The governed official website is ${officialUrl}.`, category: "official_website", source_ids: sourceIds, confidence: 100, verification_status: "VERIFIED", verified: true, public_usable: false, sensitive: false, expires_at: null, notes: "Recovered from sealed Research Room evidence." },
    ],
    confidence: { overall: 95 },
  };
  return {
    success: true, pending: false, failed: false, provider: "avantiqo-intelligence", model: "SEALED_RESEARCH_ROOM_RECOVERY",
    usage: null, output: { text: JSON.stringify(payload), status: "completed", finish_reason: "sealed_research_room_recovery" },
    billing: null, recovered_from_sealed_research_room: true,
    evidence_dossier: {
      usage_id: null,
      researched_at: state.persisted_at || new Date().toISOString(),
      output_text: JSON.stringify(evidence),
      sources: recoveredSources.length ? recoveredSources : payload.sources,
      raw: null,
    },
  };
}
...
function discoveryQueries({ context, plan }) {
  ...
  const identity = [organization.canonical_name, locality]
    ...
    .filter(Boolean)
    .join(" ");
  const industry = text(organization.industry || project.industry || mission.industry, 300);
  const benchmarkTargets = benchmarkTargetsFromPlan(plan);
  const planObjective = (id) => {
    const item = list(plan).find(
      (candidate) => text(candidate?.id).toLowerCase() === String(id || "").trim().toLowerCase(),
    );
    return text(item?.objective || item?.description || item?.query, 500);
  };
  ...
  async repairCompletedSynthesis({ organization_id, mission = {}, project = {}, brief = {}, assets = [] } = {}) {
    ...
    try {
      ...
      repairResult = await awaitResearchExecution(await ServiceExecutionRuntime.execute({
        ...
        provider_policy: {
          allowed_providers: [localResearchSynthesisEnabled() ? "avantiqo-intelligence" : approval.provider],
          preferred_providers: [localResearchSynthesisEnabled() ? "avantiqo-intelligence" : approval.provider],
          preferred_models: localResearchSynthesisEnabled()
            ? [AVANTIQO_INTELLIGENCE_LOCAL_MODEL]
            : approval.model
              ? [approval.model]
              : [],
          ...(localResearchSynthesisEnabled()
            ? {
    ...
    try {
      if (benchmarkBaseReport && benchmarkTargets.length) {
        ...
        const benchmarkResult = await awaitResearchExecution(await ServiceExecutionRuntime.execute({
          organization_id,
          service_id: "ai.reasoning.execute",
          provider_id: localResearchSynthesisEnabled() ? "avantiqo-intelligence" : approval.provider,
          category: "CREATIVE_RESEARCH",
          input: {
      ...
      if (!structuredResult) {
        const sealedRecovery = sealedResearchRoomStructuredResult({ project: scopedProject, identity: organizationIdentity, policy });
        if (sealedRecovery) {
          structuredResult = sealedRecovery;
          dossier = sealedRecovery.evidence_dossier || dossier;
          evidenceReused = true;
        }
      }
      ...
      const synthesisPrompt = localSynthesis
        ? compactLocalResearchPrompt({ context, plan, policy, currentDate, dossier })
        : policy.mode === "CREATIVE_GROUNDING"
      ...
      // Local Node01 synthesis is deliberately compact enough to fit the owned 6k context.
      // External/deep execution keeps the full evidence contract, but can only be reached
      // when the provider boundary receives explicit Modal approval.
      const estimatedSynthesisInputTokens = localSynthesis
        ? Math.max(1, Math.ceil(synthesisPrompt.length / 3.2))
        : Math.max(12000, Math.ceil(synthesisPrompt.length / 3));
      ...
      if (!structuredResult) structuredResult = await awaitResearchExecution(await ServiceExecutionRuntime.execute({
        ...
        provider_policy: {
          ...
          preferred_models: localSynthesis
            ? [AVANTIQO_INTELLIGENCE_LOCAL_MODEL]
            : approval.model
              ? [approval.model]
              : [],
      ...
      validated = validateEvidenceBinding(validated, finalDossier, context);
      if (policy.mode !== "CREATIVE_GROUNDING") {
        validated = validateResolvedOrganization(validated, organizationIdentity);
      }
