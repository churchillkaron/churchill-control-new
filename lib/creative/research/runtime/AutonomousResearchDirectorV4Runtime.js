...
function discoveryQueries({ context, plan }) {
  ...
  return [...new Set([
    ...
    ...fragments.map((fragment) => [identity, fragment].filter(Boolean).join(" ")),
    ...
    try {
      ...
      structuredResult = await awaitResearchExecution(await ServiceExecutionRuntime.execute({
        ...
        cost_guard: {
          ...
          estimated_input_tokens: Number(approval.estimated_input_tokens || 12000),
          estimated_output_tokens: Number(approval.estimated_output_tokens || 8000),
      ...
      const unchargedFailure = spend <= 0;
      await updateResearchApproval(scopedProject, approval, {
        approved: unchargedFailure ? true : false,
        status: message.includes("VALIDATION") || message.includes("JSON_REQUIRED") || message.includes("SOURCE_BINDING")
          ? "VALIDATION_FAILED"
          : "EXECUTION_FAILED",
        consumed_at: unchargedFailure ? null : new Date().toISOString(),
        evidence_usage_id: dossier?.usage_id || null,
        structured_usage_id: structuredResult?.usage?.id || null,
        charged_customer_price: spend,
        validation_error: message,
        retry_required: true,
        approval_reusable_after_uncharged_failure: unchargedFailure,
        evidence_reused: evidenceReused,
      }).catch(() => null);
