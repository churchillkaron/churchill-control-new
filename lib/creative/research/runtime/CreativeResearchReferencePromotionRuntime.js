...
function plausibleVisualUrl(value) {
  ...
  if (/\/(?:ansatte|employees?|staff|people)\//.test(lower)) return false;
  try {
    ...
    if ((width > 0 && width < 640) || (height > 0 && height < 480)) return false;
  } catch {}
  return true;
}
  ...
  async promote({ organization_id, creative_mission_id = null, creative_project_id, research, existing_assets = [] } = {}) {
    ...
    for (const reference of refs) {
      ...
      try {
        ...
        const mapping = { asset_id: asset.id, asset_node_id: node?.id || null, media_url: mediaUrl };
        mappings.set(text(reference.id), mapping);
        promoted.push({ reference_id: reference.id || null, ...mapping });
      } catch (error) {
    ...
    for (const item of promoted) {
      const parentId = text(refs.find((ref) => text(ref.id) === text(item.reference_id))?.parent_reference_id);
      if (!parentId) continue;
      const children = childrenByParent.get(parentId) || [];
      children.push({ reference_id: item.reference_id, asset_id: item.asset_id, asset_node_id: item.asset_node_id, media_url: item.media_url });
    ...
    const referenceCandidates = list(grounding.reference_candidates).map((reference) => {
      ...
      const visualChildren = childrenByParent.get(text(reference.id)) || [];
      if (mapped) return { ...reference, creative_asset_id: mapped.asset_id, creative_asset_node_id: mapped.asset_node_id, promotion_status: "PROMOTED_REFERENCE_ONLY" };
      if (visualChildren.length) return {
        ...reference,
        visual_reference_assets: visualChildren,
        extracted_visual_asset_ids: visualChildren.map((item) => item.asset_id),
        promotion_status: "PROMOTED_PAGE_VISUALS_REFERENCE_ONLY",
      };
      return reference;
    });
