...
export function resolveResearchPolicy(project = {}, brief = {}) {
  ...
  return {
    ...
    require_benchmark_lab: requireBenchmarkLab,
    benchmark_floor: benchmarkFloor,
    minimum_benchmark_studies: requireBenchmarkLab
      ? Math.max(1, Math.min(3, benchmarkFloor.length || 3))
      : 0,
  };
}
...
export function normalizeAndValidateResearch({
  ...
  if (policy.require_benchmark_lab && benchmarkStudy?.passed !== true) {
    blockers.push("CREATIVE_BENCHMARK_LAB_REQUIRED");
    for (const failure of benchmarkStudy?.failures || []) {
      blockers.push(`CREATIVE_BENCHMARK_LAB:${failure}`);
    }
  }
