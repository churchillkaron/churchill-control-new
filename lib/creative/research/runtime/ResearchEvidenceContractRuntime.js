...
import { evaluateBenchmarkStudy } from "@/lib/creative/director/runtime/CreativeBenchmarkLabRuntime";

function list(value) {
  return Array.isArray(value) ? value.filter(Boolean) : [];
}

function listOrOne(value) {
...
function normalizeSource(value, index, retrievedAt) {
  ...
  return {
    ...
    _alias_ids: [...new Set(list(item._alias_ids || item.alias_ids || item.aliasIds).map(text).filter(Boolean))],
...
function mergeSources(declared, citations, retrievedAt) {
  ...
  for (const source of normalized) {
    ...
    byUrl.set(source.url, prior ? {
      ...
      id: prior.id,
      _alias_ids: [...new Set([
        ...list(prior._alias_ids),
        prior.id,
        ...list(source._alias_ids),
        source.id,
      ].map(text).filter(Boolean))].filter((id) => id !== prior.id),
...
function publicSource(source = {}) {
  const { _alias_ids, ...publicValue } = source;
  return publicValue;
}
...
function directMediaUrl(value) {
  ...
  try {
    ...
    return /\.(?:avif|gif|jpe?g|png|webp|svg|mp4|mov|webm|m4v|mp3|wav|m4a|aac|flac)$/.test(pathname);
  } catch {
    return false;
  }
}
...
function normalizeCreativeGrounding(value = {}, sourceIds = new Set()) {
  ...
  const normalizeEvidenceItem = (entry, index, prefix) => {
    ...
    const structured = Boolean(entry && typeof entry === "object" && !Array.isArray(entry));
    return {
      ...object(entry),
  ...
  const referenceCandidates = list(item.reference_candidates || item.referenceCandidates).map((entry, index) => {
    ...
    return {
      ...
      media_url: mediaUrl,
      source_type: text(entry?.source_type || entry?.sourceType).toLowerCase(),
      rights_status: text(entry?.rights_status || entry?.rightsStatus || "UNRESOLVED").toUpperCase(),
...
function normalizeStrategicSynthesis(value = {}, sourceIds = new Set()) {
  ...
  return {
    ...
    human_truths: listOrOne(item.human_truths || item.humanTruths).map((entry, index) => normalizeEvidenceItem(entry, index, "human-truth")),
    category_conventions: listOrOne(item.category_conventions || item.categoryConventions).map((entry, index) => normalizeEvidenceItem(entry, index, "category-convention")),
    breakable_conventions: listOrOne(item.breakable_conventions || item.breakableConventions).map((entry, index) => normalizeEvidenceItem(entry, index, "breakable-convention")),
    competitor_patterns: listOrOne(item.competitor_patterns || item.competitorPatterns).map((entry, index) => normalizeEvidenceItem(entry, index, "competitor-pattern")),
    distinctive_brand_assets: listOrOne(item.distinctive_brand_assets || item.distinctiveBrandAssets).map((entry, index) => normalizeEvidenceItem(entry, index, "brand-asset")),
    cultural_context: listOrOne(item.cultural_context || item.culturalContext).map((entry, index) => normalizeEvidenceItem(entry, index, "cultural-context")),
    attention_opportunities: listOrOne(item.attention_opportunities || item.attentionOpportunities).map((entry, index) => normalizeEvidenceItem(entry, index, "attention-opportunity")),
    ...
    must_not_do: listOrOne(item.must_not_do || item.mustNotDo).map((entry) => typeof entry === "string" ? text(entry) : text(entry?.statement || entry?.decision || entry?.risk)).filter(Boolean),
  };
}
...
export function resolveResearchPolicy(project = {}, brief = {}) {
  ...
  const benchmarkFloor = list(
    project.metadata?.benchmark_floor ||
    brief.benchmark_floor ||
    brief.metadata?.benchmark_floor,
  ).map(text).filter(Boolean);
  const requireBenchmarkLab =
    configured.require_benchmark_lab === true ||
    benchmarkFloor.length > 0;
  ...
  return {
    ...
    require_reference_candidates: configured.require_reference_candidates ?? groundingMode,
    require_strategic_evidence: configured.require_strategic_evidence ?? true,
    require_grounding_evidence: configured.require_grounding_evidence ?? true,
  };
}
...
export function normalizeAndValidateResearch({
  ...
  const sourceIds = new Set([
    ...
    ...sources.flatMap((source) => [source.id, ...list(source._alias_ids)]),
  ]);
  ...
  const rawResolutionStatus = text(resolution.status).toUpperCase();
  const resolutionStatus = ["VERIFIED", "MATCHED", "CONFIRMED"].includes(rawResolutionStatus)
    ? "RESOLVED"
    : rawResolutionStatus;
  const competitorAnalysis = object(payload.competitor_analysis || payload.competitorAnalysis);
  const competitors = list(competitorAnalysis.competitors);
  const competitorEvidence = competitors.map((competitor) => validateEvidenceReferences(competitor, sourceIds));
  const competitorInvalidSourceIds = competitorEvidence.flatMap((entry) => entry.invalid_source_ids);
  const competitorsWithoutEvidence = competitors.filter((competitor, index) =>
    text(competitor?.name || competitor?.title || competitor?.company || competitor?.product) &&
    !(competitorEvidence[index]?.valid_source_ids || []).length
  );
  ...
  const confidence = finite(object(payload.confidence).overall ?? payload.confidence, 0);
  ...
  const benchmarkStudy = policy.require_benchmark_lab
    ? evaluateBenchmarkStudy({
        benchmark_lab: object(creativeGrounding.benchmark_lab),
        minimum_studies: policy.minimum_benchmark_studies || 3,
      })
    : null;
  ...
  const invalidGroundingMode = !["NONE", "REAL_WORLD"].includes(creativeGrounding.mode);
  const malformedGroundingEvidence = [
    ...creativeGrounding.entities,
    ...creativeGrounding.evidence_targets,
    ...list(creativeGrounding.spatial_path?.ordered_nodes),
    ...list(creativeGrounding.spatial_path?.directed_edges),
  ].filter((item) => item?.structured !== true);
  const unsupportedGroundingEvidence = [
    ...creativeGrounding.entities,
    ...creativeGrounding.evidence_targets,
  ].filter((item) => creativeGrounding.mode === "REAL_WORLD" && !(item?.source_ids || []).length);
  const webpageMasqueradingAsMedia = selectedReferences.filter((item) => {
    if (!item.media_url || !item.source_url) return false;
    return canonicalComparableUrl(item.media_url) === canonicalComparableUrl(item.source_url) && !directMediaUrl(item.media_url);
  });
  const unverifiedPublicRights = selectedReferences.filter((item) =>
    ["public_web_source", "web", "web_search_source", "web_citation"].includes(text(item.source_type).toLowerCase()) &&
    !["", "UNKNOWN", "UNRESOLVED"].includes(text(item.rights_status).toUpperCase())
  );
  ...
  if (policy.require_strategic_evidence && strategicInvalidSourceIds.length) blockers.push("STRATEGIC_SYNTHESIS_INVALID_SOURCES");
  if (policy.require_strategic_evidence && strategicUnsupported.length) blockers.push("STRATEGIC_SYNTHESIS_EVIDENCE_REQUIRED");
  ...
  if (policy.require_competitor_analysis && !competitors.length) {
    blockers.push("COMPETITOR_ANALYSIS_REQUIRED");
  }
  if (competitorInvalidSourceIds.length) blockers.push("COMPETITOR_ANALYSIS_INVALID_SOURCES");
  if (policy.require_competitor_analysis && competitorsWithoutEvidence.length) {
    blockers.push("COMPETITOR_ANALYSIS_EVIDENCE_REQUIRED");
  }
  ...
  if (invalidGroundingMode) blockers.push("CREATIVE_GROUNDING_MODE_INVALID");
  if (malformedGroundingEvidence.length) blockers.push("CREATIVE_GROUNDING_STRUCTURED_EVIDENCE_REQUIRED");
  if (unsupportedGroundingEvidence.length) blockers.push("CREATIVE_GROUNDING_EVIDENCE_REQUIRED");
  if (webpageMasqueradingAsMedia.length) blockers.push("CREATIVE_GROUNDING_DIRECT_MEDIA_URL_REQUIRED");
  if (unverifiedPublicRights.length) blockers.push("CREATIVE_GROUNDING_PUBLIC_RIGHTS_UNVERIFIED");
  ...
  if (groundingActive && policy.require_reference_candidates && !creativeGrounding.reference_candidates.length) blockers.push("CREATIVE_GROUNDING_REFERENCES_REQUIRED");
  if (policy.require_grounding_evidence && requiredTargetsWithoutEvidence.length) blockers.push("CREATIVE_GROUNDING_REQUIRED_TARGET_UNSUPPORTED");
  if (policy.require_grounding_evidence && selectedReferencesWithoutEvidence.length) blockers.push("CREATIVE_GROUNDING_SELECTED_REFERENCE_UNSUPPORTED");
  ...
  const validation = {
    ...
    benchmark_lab_required: policy.require_benchmark_lab === true,
    benchmark_lab_passed: benchmarkStudy?.passed ?? null,
    benchmark_lab_study_count: benchmarkStudy?.study_count ?? 0,
    benchmark_lab_failures: benchmarkStudy?.failures || [],
...
export function researchReportIsReusable(report = {}, { context_identity, policy } = {}) {
  ...
  if (policy?.require_benchmark_lab === true) {
    const grounding = object(report.creative_grounding || report.metadata?.creative_grounding);
    const benchmark = evaluateBenchmarkStudy({
      benchmark_lab: object(grounding.benchmark_lab),
      minimum_studies: policy.minimum_benchmark_studies || 3,
    });
    if (benchmark.passed !== true) return false;
  }
