...
function validCompletedResearchReport({
  report,
  project = {},
  brief = {},
  policy,
  allow_missing_benchmark_lab = false,
} = {}) {
  ...
  if (policy.require_benchmark_lab === true && allow_missing_benchmark_lab !== true) {
    const grounding = object(report.metadata?.creative_grounding || report.creative_grounding);
...
function sealedResearchRoomLineageReport({ project = {}, existing = [] } = {}) {
  const state = object(project.metadata?.creative_preproduction_state);
  if (
    plainText(state.organization_id) !== plainText(project.organization_id) ||
    plainText(state.creative_project_id) !== plainText(project.id)
  ) return null;
  const room = list(state.production_room_pipeline?.stages)
    .find((stage) => plainText(stage?.id).toUpperCase() === "RESEARCH_ROOM");
  const researchIdentity = plainText(room?.report?.research_identity);
  if (
    plainText(room?.status).toUpperCase() !== "SEALED" ||
    room?.report?.passed !== true ||
    !researchIdentity
  ) return null;
  return list(existing).find((report) =>
    report?.metadata?.validation?.passed === true &&
    plainText(report?.metadata?.research_identity) === researchIdentity &&
    (!plainText(report.organization_id) || plainText(report.organization_id) === plainText(project.organization_id)) &&
    (!plainText(report.creative_project_id) || plainText(report.creative_project_id) === plainText(project.id))
  ) || null;
}

function reusableCompletedProjectResearchReport({
  project = {},
  brief = {},
  existing = [],
  allow_missing_benchmark_lab = false,
  ...
  return list(existing).find((report) =>
    validCompletedResearchReport({
      report,
      project,
      brief,
      policy,
      allow_missing_benchmark_lab,
    }),
  ) || null;
}

function completedPaidResearchReport({
  project = {},
  brief = {},
  existing = [],
  allow_missing_benchmark_lab = false,
...
async function resolveResearch({
  ...
  assets,
  forceResearch,
  reuseCompletedResearchOnDirectionRestart = false,
}) {
  ...
  if (!forceResearch) {
    const sealedLineage = sealedResearchRoomLineageReport({
      project: { ...project, organization_id: organizationId },
      existing,
    });
    if (sealedLineage) return sealedLineage;

    if (reuseCompletedResearchOnDirectionRestart === true) {
      const reusable = reusableCompletedProjectResearchReport({
        project: { ...project, organization_id: organizationId },
        brief,
        existing,
        allow_missing_benchmark_lab: true,
      });
    ...
    const completed = completedPaidResearchReport({
      ...
      existing,
      allow_missing_benchmark_lab:
        reuseCompletedResearchOnDirectionRestart === true,
    });
  ...
  const researchApproval = object(project.metadata?.paid_research_approval);
  const repairableFailure =
    researchApproval.retry_required === true &&
    researchApproval.approved === true &&
    ["VALIDATION_FAILED", "EXECUTION_FAILED", "REPAIR_FAILED"].includes(
      text(researchApproval.status).toUpperCase(),
    );
  if (repairableFailure && forceResearch !== true) {
    return AutonomousResearchDirectorV4Runtime.repairCompletedSynthesis({
      organization_id: organizationId,
      mission,
      project,
      brief,
...
export async function resolveCreativeDirectionResearch(input = {}) {
  ...
  let research = await resolveResearch({
    ...
    reuseCompletedResearchOnDirectionRestart:
      input.reuse_completed_research_on_direction_restart === true,
  });
