...
import {
  CreativeDirectedEditTimelineRuntime,
} from "@/lib/creative/post-production/runtime/CreativeDirectedEditTimelineRuntime";
...
function requirementFromTask(task = {}) {
  ...
  return {
    ...
    editorial_causality: requirements.editorial_causality || intent.editorial_causality || null,
    transition_in: requirements.transition_in || intent.transition_in || null,
    transition_out: requirements.transition_out || intent.transition_out || null,
  };
}
...
function finalEditSourceVideo(node = {}) {
  return eligibleSourceVideo(node) &&
    node.metadata?.final_edit_source === true &&
    node.metadata?.include_in_master === true &&
...
async function ensureTimeline({ organization_id, creative_project_id }) {
  ...
  const shots = await ShotRuntime.list({ organization_id, creative_project_id });
  if (shots.length) {
    const unreleased = shots.filter((shot) =>
      shot.metadata?.shot_release_ready_for_edit !== true ||
      !text(shot.metadata?.final_shot_asset_node_id)
    );
    if (unreleased.length) {
      return {
        status: "AWAITING_SHOT_RELEASE",
        timeline: null,
        unreleased_shot_ids: unreleased.map((shot) => shot.id),
      };
    }
  } else {
  ...
  if (shots.length) {
    const directed = await CreativeDirectedEditTimelineRuntime.build({
      organization_id,
      creative_project_id,
      shots,
      asset_nodes: initialNodes,
    });
    return {
      status: "READY",
      timeline: directed.timeline,
      reused: directed.reused === true,
  ...
  const finalSources = initialNodes.filter(finalEditSourceVideo);
  const videos = shots.length ? finalSources : initialNodes.filter(eligibleSourceVideo);
  if (shots.length && finalSources.length !== shots.length) {
    return {
      status: "AWAITING_FINAL_EDIT_SOURCES",
      timeline: null,
      released_shot_count: shots.length,
      final_edit_source_count: finalSources.length,
    };
  }
  if (!videos.length) {
