...
export const CREATIVE_ATOMIC_SHOT_SET_REVISION_CONTRACT =
  "AVANTIQO_ATOMIC_SHOT_SET_REVISION_V2";
...
function prompt({ instruction, scope, shots, preservedShots }) {
  return `
...
Return strict JSON only:
{
  ...
  "changes": [
    {
      "shot_id": "exact supplied editable shot id",
      "patch": {
        "camera": {},
        "coverage": {},
        "continuity": {},
...
EDITABLE VERIFIED SHOT SET
${JSON.stringify(shots.map(shotSnapshot))}

PRESERVED IMMUTABLE SHOTS
${JSON.stringify(preservedShots.map(shotSnapshot))}

RULES
- Return every EDITABLE supplied shot exactly once and no other shot.
- PRESERVED IMMUTABLE SHOTS are context only: never return them in changes and never suggest changing them.
...
function validateProposal({ output, scope, shots, preservedShots }) {
  ...
  const preservedIds = new Set(preservedShots.map((shot) => text(shot.id, 180)));
  const byId = new Map(shots.map((shot) => [text(shot.id, 180), shot]));
  const seen = new Set();
  return changes.map((change) => {
    const shotId = text(change?.shot_id, 180);
    if (preservedIds.has(shotId)) {
      throw new Error(`CREATIVE_ATOMIC_MULTI_REVISION_PRESERVED_SHOT_PROPOSED:${shotId}`);
    }
...
function preservedGuards(shots = []) {
...
export const CreativeAtomicShotSetRevisionRuntime = Object.freeze({
  ...
  async revise({
    ...
    const preservedShots = list(preserved_shots);
    ...
    const editableIds = new Set(verifiedShots.map((shot) => text(shot.id, 180)));
    const overlap = preservedShots
      .map((shot) => text(shot.id, 180))
      .filter((id) => editableIds.has(id));
    if (overlap.length) {
      throw new Error(`CREATIVE_ATOMIC_MULTI_REVISION_PRESERVED_OVERLAP:${overlap.join(",")}`);
    }

    const maxOutputTokens = Math.min(16000, 2500 + verifiedShots.length * 700);
    const result = await ServiceExecutionRuntime.execute({
      ...
      input: {
        ...
        ...(verifiedShots.length <= 2 ? { execution_lane: "fast" } : {}),
    ...
    const { data, error } = await supabaseAdmin.rpc(
      "creative_apply_guarded_shot_set_revision_atomic",
      {
        ...
        p_changes: changes,
        p_preserved_guards: preservedGuards(preservedShots),
      },
    );
    ...
    return {
      ...
      atomic_commit: true,
      all_or_nothing: true,
      preserved_shots_guarded: data?.preserved_shots_guarded === true,
