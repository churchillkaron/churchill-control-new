import { loadWithOperatorExecutionBoundary } from "@/lib/operator/runtime/OperatorExecutionBoundaryRuntime.mjs";
import { MissionEngine } from "./engines/MissionEngine";
import { DirectorEngine } from "./engines/DirectorEngine";
import { ResearchEngine } from "./engines/ResearchEngine";
import { PlanningEngine } from "./engines/PlanningEngine";
import { ActorEngine } from "./engines/ActorEngine";
import { AssetEngine } from "./engines/AssetEngine";
import { VoiceEngine } from "./engines/VoiceEngine";
import { MusicEngine } from "./engines/MusicEngine";
import { SceneEngine } from "./engines/SceneEngine";
import { ShotEngine } from "./engines/ShotEngine";
import { TimelineEngine } from "./engines/TimelineEngine";
import { RenderEngine } from "./engines/RenderEngine";
import { QAEngine } from "./engines/QAEngine";
import { PublishingEngine } from "./engines/PublishingEngine";
import { LearningEngine } from "./engines/LearningEngine";

export const CreativeRuntime = {

  domain: "creative",

  name: "Creative Studio",

  version: "1.0.0",

  capabilities: {
    studio: {
      prepareProject: () => loadWithOperatorExecutionBoundary(() =>
        import(
          "@/lib/creative/studio/capabilities/prepareStudioProject"
        )),
      inspectProject: () =>
        import(
          "@/lib/creative/studio/capabilities/inspectStudioProject"
        ),
      inspectDirection: () =>
        import(
          "@/lib/creative/studio/capabilities/inspectStudioDirection"
        ),
      reviseShot: () => loadWithOperatorExecutionBoundary(() =>
        import(
          "@/lib/creative/studio/capabilities/reviseStudioShot"
        )),
      planShotSetRevision: () =>
        import(
          "@/lib/creative/studio/capabilities/planStudioShotSetRevision"
        ),
      reviseShotSet: () => loadWithOperatorExecutionBoundary(() =>
        import(
          "@/lib/creative/studio/capabilities/reviseStudioShotSet"
        )),
      restoreShotSetRevision: () => loadWithOperatorExecutionBoundary(() =>
        import(
          "@/lib/creative/studio/capabilities/restoreStudioShotSetRevision"
        )),
    },
    production: {
      inspect: () =>
        import(
          "@/lib/creative/production/capabilities/inspectCreativeProduction"
        ),
      run: () => loadWithOperatorExecutionBoundary(() =>
        import(
          "@/lib/creative/production/capabilities/runCreativeProduction"
        )),
    },
    image: {
      inspectWorldClassStudio: () =>
        import(
          "@/lib/creative/stills/capabilities/inspectWorldClassImageStudio"
        ),
      planWorldClassProduction: () =>
        import(
          "@/lib/creative/stills/capabilities/planWorldClassImageStudio"
        ),
    },
    music: {
      inspectWorldClassStudio: () =>
        import(
          "@/lib/creative/music/capabilities/inspectWorldClassMusicStudio"
        ),
      planWorldClassProduction: () =>
        import(
          "@/lib/creative/music/capabilities/planWorldClassMusicStudio"
        ),
      developWorldClassProduction: () =>
        import(
          "@/lib/creative/music/capabilities/developWorldClassMusicStudio"
        ),
      reviewWorldClassDailies: () =>
        import(
          "@/lib/creative/music/capabilities/reviewWorldClassMusicDailies"
        ),
      planWorldClassSurgicalRepair: () =>
        import(
          "@/lib/creative/music/capabilities/planWorldClassSurgicalRepair"
        ),
      executeWorldClassProduction: () => loadWithOperatorExecutionBoundary(() =>
        import(
          "@/lib/creative/music/capabilities/executeWorldClassMusicStudio"
        )),
    },
  },

  engines: {
    mission: MissionEngine,
    director: DirectorEngine,
    research: ResearchEngine,
    planning: PlanningEngine,
    actor: ActorEngine,
    asset: AssetEngine,
    voice: VoiceEngine,
    music: MusicEngine,
    scene: SceneEngine,
    shot: ShotEngine,
    timeline: TimelineEngine,
    render: RenderEngine,
    qa: QAEngine,
    publishing: PublishingEngine,
    learning: LearningEngine,
  },

  get(name) {
    return this.engines[name];
  },

  all() {
    return this.engines;
  }

};