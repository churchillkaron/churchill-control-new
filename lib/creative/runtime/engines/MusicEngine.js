import { buildMusicVocalRoleSeparationRequest, MUSIC_VOCAL_ROLE_SEPARATOR_CAPABILITY as OWNED_VOCAL_ROLE_SEPARATOR_CAPABILITY } from "../../music/runtime/CreativeMusicVocalRoleSeparationRuntime.js";
...
function configuredCertifiedAudioCapabilities() {
  return new Set(String(process.env.AVANTIQO_AUDIO_CERTIFIED_CAPABILITIES || "ai.music.generate")
    .split(",").map((value) => value.trim()).filter(Boolean));
}
function resolvedCertification(contract) {
  if (!contract || contract.implementation !== "IMPLEMENTED") return contract?.certification || "NOT_IMPLEMENTED";
  if (!configuredCertifiedAudioCapabilities().has(contract.capability)) return contract.certification;
  if (contract.capability === "ai.audio.stems") {
    const runtimeReady = ["1", "true", "yes", "on"].includes(String(process.env.AVANTIQO_AUDIO_ENGINE_ENABLED || "").toLowerCase()) && ["1", "true", "yes", "on"].includes(String(process.env.AVANTIQO_MUSIC_SEPARATOR_ENGINE_ENABLED || "").toLowerCase()) && ["1", "true", "yes", "on"].includes(String(process.env.AVANTIQO_MUSIC_SEPARATOR_ENGINE_CERTIFIED || "").toLowerCase()) && Boolean(String(process.env.MODAL_TOKEN_ID || process.env.AVANTIQO_MODAL_TOKEN_ID || "").trim()) && Boolean(String(process.env.MODAL_TOKEN_SECRET || process.env.AVANTIQO_MODAL_TOKEN_SECRET || "").trim());
    return runtimeReady ? "CERTIFIED" : contract.certification;
  }
  return "CERTIFIED";
}
...
const VOCAL_REMOVAL_MODES = Object.freeze({ ALL_VOCALS: "ALL_VOCALS", LEAD_ONLY_KEEP_BACKING: "LEAD_ONLY_KEEP_BACKING" });
const VOCAL_ROLE_SEPARATOR_CAPABILITY = OWNED_VOCAL_ROLE_SEPARATOR_CAPABILITY;
...
export function normalizeBackingTrackRequest(input = {}) {
  ...
  const vocalRemovalMode = text(input.vocal_removal_mode || input.vocalRemovalMode || VOCAL_REMOVAL_MODES.ALL_VOCALS).toUpperCase();
  if (!Object.values(VOCAL_REMOVAL_MODES).includes(vocalRemovalMode)) {
    const error = new Error(`CREATIVE_MUSIC_VOCAL_REMOVAL_MODE_INVALID:${vocalRemovalMode || "MISSING"}`);
    error.code = "CREATIVE_MUSIC_VOCAL_REMOVAL_MODE_INVALID";
    throw error;
  }
  ...
  return {
    ...
    separator: {
      ...
      vocal_role_separation_required: vocalRemovalMode === VOCAL_REMOVAL_MODES.LEAD_ONLY_KEEP_BACKING,
      vocal_role_capability: vocalRemovalMode === VOCAL_REMOVAL_MODES.LEAD_ONLY_KEEP_BACKING ? VOCAL_ROLE_SEPARATOR_CAPABILITY : null,
      vocal_role_request: vocalRemovalMode === VOCAL_REMOVAL_MODES.LEAD_ONLY_KEEP_BACKING
        ? buildMusicVocalRoleSeparationRequest({ mode: vocalRemovalMode })
        : null,
    },
    processing: {
      remove_vocals: true,
      vocal_removal_mode: vocalRemovalMode,
      preserve_backing_vocals: vocalRemovalMode === VOCAL_REMOVAL_MODES.LEAD_ONLY_KEEP_BACKING,
...
function buildStemSeparationPlan(key, contract, input, sourceAudio) {
  ...
  const vocalRoleMode = text(input.vocal_role_mode || input.vocalRoleMode).toUpperCase();
  const vocalRoleRequested = !backingTrack && vocalRoleMode === "ROLE_STEMS";
  const vocalRoleRequest = vocalRoleRequested
    ? buildMusicVocalRoleSeparationRequest({ mode: "ROLE_STEMS" })
    : backing.separator?.vocal_role_request || null;
  ...
  return {
    ...
    certification: resolvedCertification(contract),
    executable: contract.implementation === "IMPLEMENTED"
      && resolvedCertification(contract) === "CERTIFIED"
      && !vocalRoleRequest,
    session: backing,
    separation: {
      model_lane: STEM_SEPARATOR_LANE,
      ...
      vocal_role_request: vocalRoleRequest,
    },
    ...
    output_spec: {
      ...
      sample_rate: 44100,
      channels: 2,
      stems: vocalRoleRequest ? [...vocalRoleRequest.required_outputs] : [...STEM_NAMES],
      backing_track: backingTrack,
      vocal_role_separation: Boolean(vocalRoleRequest),
...
export function buildMusicTransformationPlan(operation, input = {}) {
  ...
  const plan = {
    ...
    implementation: contract.implementation,
    certification: resolvedCertification(contract),
    executable: contract.implementation === "IMPLEMENTED" && resolvedCertification(contract) === "CERTIFIED",
    session,
    generation: {
      caption: session.direction,
      lyrics: session.lyrics,
...
export function musicCapabilityState() {
  return {
    compose: "CERTIFIED",
    remix: resolvedCertification(ADVANCED_CAPABILITIES.remix),
    edit: resolvedCertification(ADVANCED_CAPABILITIES.edit),
    extend: resolvedCertification(ADVANCED_CAPABILITIES.extend),
    stems: resolvedCertification(ADVANCED_CAPABILITIES.stems),
    backing_track: resolvedCertification(ADVANCED_CAPABILITIES.backing_track),
...
export const MUSIC_VOCAL_REMOVAL_MODES = VOCAL_REMOVAL_MODES;
export const MUSIC_VOCAL_ROLE_SEPARATOR_CAPABILITY = VOCAL_ROLE_SEPARATOR_CAPABILITY;