...
function buildStemSeparationPlan(key, contract, input, sourceAudio) {
  ...
  return {
    ...
    executable: contract.implementation === "IMPLEMENTED" && resolvedCertification(contract) === "CERTIFIED" && !(backing.processing?.vocal_removal_mode === VOCAL_REMOVAL_MODES.LEAD_ONLY_KEEP_BACKING),
    session: backing,
    separation: {
      model_lane: STEM_SEPARATOR_LANE,
      quality_profile: STEM_SEPARATOR_PROFILE,
