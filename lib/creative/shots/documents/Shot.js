...
function brandMarkCompositing(data = {}) {
  const graphics = object(data.graphics);
  const metadata = object(data.metadata);
  return promptless(object(
    data.brand_mark_compositing ||
    graphics.brand_mark_compositing ||
    metadata.brand_mark_compositing,
  ));
}
...
function shotBibleSource(data = {}, generation = {}) {
  return promptless({
    ...
    brand_mark_compositing: brandMarkCompositing(data),
  });
}
...
export function createShot(data = {}) {
  ...
  const compositing = brandMarkCompositing(data);
  const graphics = {
    ...object(data.graphics),
    ...(Object.keys(compositing).length
  ...
  return {
    ...
    metadata: {
      ...(data.metadata || {}),
      subject,
      shot_bible_source: shotBibleSource(data, generation),
