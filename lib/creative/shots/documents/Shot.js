...
export function createShot(data = {}) {
  ...
  const generation = object(data.generation || data.metadata?.generation);
  ...
  return {
    ...
    subject: text(data.subject) || text(data.metadata?.subject) || text(data.purpose),
    action: data.action || "",
    performance:
