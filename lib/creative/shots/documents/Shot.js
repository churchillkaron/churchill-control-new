...
function structured(value) {
  if (Array.isArray(value)) return promptless(value);
  return promptless(object(value));
}
...
function shotBibleSource(data = {}, generation = {}) {
  return promptless({
    ...
    subject_truth: structured(data.subject_truth || data.metadata?.subject_truth),
    research_grounding: structured(data.research_grounding || data.metadata?.research_grounding),
    reference_evidence: array(data.reference_evidence || data.metadata?.reference_evidence),
    cinematic_beauty_intent: structured(data.cinematic_beauty_intent || data.metadata?.cinematic_beauty_intent),
    rights_requirements: object(data.rights_requirements),
    sound_design: object(data.sound_design),
    vfx: structured(data.vfx),
    simulation: structured(data.simulation),
    compositing: structured(data.compositing),
    world: structured(data.world),
    world_id: data.world_id || null,
    spatial_geography: data.spatial_geography || null,
    architecture_geometry: data.architecture_geometry || null,
    materials_surfaces: data.materials_surfaces || null,
    signage_readable_text: data.signage_readable_text || null,
    time_of_day: data.time_of_day || null,
    weather: data.weather || null,
    atmosphere: data.atmosphere || null,
    background_population: data.background_population || null,
    scene_scale: data.scene_scale || null,
    world_reference_assets: array(data.world_reference_assets),
    world_override: structured(data.world_override),
...
export function createShot(data = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      virtual_camera_state: structured(data.virtual_camera_state || data.metadata?.virtual_camera_state),
      coverage_contract:
      ...
      simulation_intent_preserved: true,
      compositing_intent_preserved: true,
      world_consistency_intent_preserved: true,
