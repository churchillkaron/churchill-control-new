...
import { CreativeLiquidSimulationRenderRuntime } from "./CreativeLiquidSimulationRenderRuntime.js";
import { CreativeClothSimulationRenderRuntime } from "./CreativeClothSimulationRenderRuntime.js";
import { CreativeSoftBodySimulationRenderRuntime } from "./CreativeSoftBodySimulationRenderRuntime.js";
...
export async function executeSimulationPass({organization_id,creative_project_id,creative_mission_id=null,project,shot_id,simulation_contract,output_spec={}}={}){
  ...
  for(const simulation of list(contract.simulations)){
    ...
    } else if(simulation.simulation_class==="LIQUID_FLUID"){
      rendered=await CreativeLiquidSimulationRenderRuntime.render({project,simulation,output_spec});
    } else if(simulation.simulation_class==="DEFORMABLE_CLOTH"){
      rendered=await CreativeClothSimulationRenderRuntime.render({project,simulation,output_spec});
    } else if(simulation.simulation_class==="DEFORMABLE_SOFT_BODY"){
      rendered=await CreativeSoftBodySimulationRenderRuntime.render({project,simulation,output_spec});
    } else {
