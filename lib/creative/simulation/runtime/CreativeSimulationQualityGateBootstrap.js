...
async function seal(review, source, evaluation) {
  if (source?.id) {
    const simulationAssetNodeId = source.output?.asset_node_id || null;
    if (simulationAssetNodeId) {
      await CreativeMultiPassArtifactRuntime.sealSimulation({
        asset_node_id: simulationAssetNodeId,
        simulation_qc_seal_hash: evaluation.seal_hash,
        simulation_contract_hash: evaluation.simulation_contract_hash,
      });
    }
    await ProductionTaskRuntime.update(source.id, {
      metadata: {
