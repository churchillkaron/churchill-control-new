...
function authoredExecutionParameters(request = {}, simulationClass, index = 0) {
  const explicit = object(request.execution_parameters || request.execution || request.numeric_parameters);
  const seed = Math.round(clamp(explicit.seed ?? request.seed, 1, 2147483646, 41000 + index * 997));
  if (simulationClass === SIMULATION_CLASSES.PARTICLE_GRANULAR) {
    return {
      contract: "AVANTIQO_PARTICLE_SIMULATION_NUMERIC_PROFILE_V1",
      backend_family: "DETERMINISTIC_BALLISTIC_PARTICLE_PLATE",
      seed,
      particle_count: Math.round(clamp(explicit.particle_count ?? request.particle_count, 8, 2000, 180)),
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.2, 20, 3.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 120, 24)),
      emitter_position: Array.isArray(explicit.emitter_position) ? explicit.emitter_position.slice(0, 3).map((v) => finite(v, 0)) : [0, 0, 0],
      emitter_radius: clamp(explicit.emitter_radius, 0.001, 5, 0.025),
      speed_min: clamp(explicit.speed_min, 0.01, 100, 0.8),
      speed_max: clamp(explicit.speed_max, 0.01, 100, 3.2),
      cone_degrees: clamp(explicit.cone_degrees, 1, 180, 75),
      gravity_mps2: clamp(explicit.gravity_mps2, -30, 30, -9.81),
      linear_drag: clamp(explicit.linear_drag, 0, 10, 0.35),
      particle_radius_min: clamp(explicit.particle_radius_min, 0.0001, 1, 0.002),
      particle_radius_max: clamp(explicit.particle_radius_max, 0.0001, 1, 0.008),
      restitution: clamp(explicit.restitution, 0, 1, 0.18),
      floor_z: finite(explicit.floor_z, -0.08),
      opacity_start: clamp(explicit.opacity_start, 0, 1, 0.9),
      opacity_end: clamp(explicit.opacity_end, 0, 1, 0.05),
      deterministic_replay_required: true,
    };
  }
  if (simulationClass === SIMULATION_CLASSES.RIGID_BODY) {
    return {
      contract: "AVANTIQO_RIGID_BODY_NUMERIC_PROFILE_V1",
      backend_family: "BLENDER_RIGID_BODY",
      seed,
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.2, 20, 3.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 120, 24)),
      gravity_mps2: clamp(explicit.gravity_mps2, -30, 30, -9.81),
      mass_kg: clamp(explicit.mass_kg ?? request.mass_kg, 0.001, 10000, 0.17),
      friction: clamp(explicit.friction, 0, 2, 0.25),
      restitution: clamp(explicit.restitution, 0, 1, 0.55),
      linear_damping: clamp(explicit.linear_damping, 0, 1, 0.04),
      angular_damping: clamp(explicit.angular_damping, 0, 1, 0.08),
      solver_substeps: Math.round(clamp(explicit.solver_substeps, 1, 100, 12)),
      solver_iterations: Math.round(clamp(explicit.solver_iterations, 1, 100, 20)),
      object_shape: text(explicit.object_shape || request.object_shape || "SPHERE", 40).toUpperCase(),
      object_dimensions: Array.isArray(explicit.object_dimensions) ? explicit.object_dimensions.slice(0, 3).map((v) => clamp(v, 0.001, 20, 0.12)) : [0.12, 0.12, 0.12],
      initial_position: Array.isArray(explicit.initial_position) ? explicit.initial_position.slice(0, 3).map((v) => finite(v, 0)) : [0, 0, 0.25],
      initial_velocity: Array.isArray(explicit.initial_velocity) ? explicit.initial_velocity.slice(0, 3).map((v) => clamp(v, -100, 100, 0)) : [1.8, 0, 0],
      initial_angular_velocity: Array.isArray(explicit.initial_angular_velocity) ? explicit.initial_angular_velocity.slice(0, 3).map((v) => clamp(v, -100, 100, 0)) : [0, 0, 2.5],
      floor_z: finite(explicit.floor_z, 0),
      collision_margin: clamp(explicit.collision_margin, 0, 0.5, 0.002),
      deterministic_replay_required: true,
    };
  }
  if (simulationClass === SIMULATION_CLASSES.DEFORMABLE_SOFT_BODY) {
    return {
      contract: "AVANTIQO_SOFT_BODY_SIMULATION_NUMERIC_PROFILE_V1",
      backend_family: "BLENDER_SOFT_BODY",
      seed,
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.4, 20, 4.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 60, 24)),
      simulation_quality: Math.round(clamp(explicit.simulation_quality, 4, 30, 12)),
      mass_kg: clamp(explicit.mass_kg, 0.001, 100, 0.8),
      pull_stiffness: clamp(explicit.pull_stiffness, 0, 1, 0.8),
      push_stiffness: clamp(explicit.push_stiffness, 0, 1, 0.8),
      bending_stiffness: clamp(explicit.bending_stiffness, 0, 10, 3.0),
      damping: clamp(explicit.damping, 0, 50, 5),
      plasticity: clamp(explicit.plasticity, 0, 100, 0),
      goal_strength: clamp(explicit.goal_strength, 0, 1, 0.0),
      use_self_collision: explicit.use_self_collision === true,
      collision_ball_size: clamp(explicit.collision_ball_size, 0.01, 5, 0.49),
      object_scale: Array.isArray(explicit.object_scale) ? explicit.object_scale.slice(0,3).map(v=>clamp(v,0.05,10,0.45)) : [0.45,0.45,0.45],
      object_position: Array.isArray(explicit.object_position) ? explicit.object_position.slice(0,3).map(v=>finite(v,0)) : [0,0,1.0],
      impactor_position_start: Array.isArray(explicit.impactor_position_start) ? explicit.impactor_position_start.slice(0,3).map(v=>finite(v,0)) : [-1.5,0,1.0],
      impactor_position_end: Array.isArray(explicit.impactor_position_end) ? explicit.impactor_position_end.slice(0,3).map(v=>finite(v,0)) : [-0.15,0,1.0],
      impact_frame: Math.round(clamp(explicit.impact_frame, 2, 600, 12)),
      impactor_radius: clamp(explicit.impactor_radius, 0.02, 5, 0.22),
      floor_z: finite(explicit.floor_z, 0),
      deterministic_replay_required: true,
    };
  }
  if (simulationClass === SIMULATION_CLASSES.DEFORMABLE_CLOTH) {
    return {
      contract: "AVANTIQO_CLOTH_SIMULATION_NUMERIC_PROFILE_V1",
      backend_family: "BLENDER_CLOTH",
      seed,
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.4, 20, 4.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 60, 24)),
      simulation_quality: Math.round(clamp(explicit.simulation_quality, 4, 30, 12)),
      mass_kg: clamp(explicit.mass_kg, 0.001, 20, 0.35),
      tension_stiffness: clamp(explicit.tension_stiffness, 0.1, 100, 15),
      compression_stiffness: clamp(explicit.compression_stiffness, 0.1, 100, 15),
      shear_stiffness: clamp(explicit.shear_stiffness, 0.1, 100, 8),
      bending_stiffness: clamp(explicit.bending_stiffness, 0.01, 100, 0.8),
      tension_damping: clamp(explicit.tension_damping, 0, 50, 5),
      compression_damping: clamp(explicit.compression_damping, 0, 50, 5),
      shear_damping: clamp(explicit.shear_damping, 0, 50, 5),
      bending_damping: clamp(explicit.bending_damping, 0, 50, 1),
      air_damping: clamp(explicit.air_damping, 0, 20, 1.5),
      self_collision: explicit.self_collision !== false,
      self_collision_distance: clamp(explicit.self_collision_distance, 0.001, 0.2, 0.008),
      collision_quality: Math.round(clamp(explicit.collision_quality, 1, 20, 6)),
      cloth_width: clamp(explicit.cloth_width, 0.1, 20, 2.2),
      cloth_height: clamp(explicit.cloth_height, 0.1, 20, 1.5),
      subdivisions_x: Math.round(clamp(explicit.subdivisions_x, 8, 200, 60)),
      subdivisions_y: Math.round(clamp(explicit.subdivisions_y, 8, 200, 40)),
      pinned_edge: text(explicit.pinned_edge || request.pinned_edge || "TOP", 20).toUpperCase(),
      wind_strength: clamp(explicit.wind_strength, 0, 10000, 650),
      wind_noise: clamp(explicit.wind_noise, 0, 10, 1.1),
      wind_direction: Array.isArray(explicit.wind_direction) ? explicit.wind_direction.slice(0,3).map(v=>finite(v,0)) : [1,0,0.15],
      cloth_position: Array.isArray(explicit.cloth_position) ? explicit.cloth_position.slice(0,3).map(v=>finite(v,0)) : [0,0,1.5],
      floor_z: finite(explicit.floor_z, 0),
      deterministic_replay_required: true,
    };
  }
  if (simulationClass === SIMULATION_CLASSES.LIQUID_FLUID) {
    return {
      contract: "AVANTIQO_LIQUID_SIMULATION_NUMERIC_PROFILE_V1",
      backend_family: "BLENDER_MANTA_LIQUID",
      seed,
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.4, 20, 4.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 60, 24)),
      domain_resolution: Math.round(clamp(explicit.domain_resolution, 32, 256, 96)),
      time_scale: clamp(explicit.time_scale, 0.1, 4, 1.0),
      viscosity_base: clamp(explicit.viscosity_base, 0, 10, 1.0),
      viscosity_exponent: Math.round(clamp(explicit.viscosity_exponent, -10, 10, -6)),
      surface_tension: clamp(explicit.surface_tension, 0, 10, 0.0),
      particle_radius: clamp(explicit.particle_radius, 0.5, 3, 1.0),
      mesh_scale: Math.round(clamp(explicit.mesh_scale, 1, 4, 2)),
      mesh_particle_radius: clamp(explicit.mesh_particle_radius, 0.5, 3, 1.4),
      smoothing_positive: Math.round(clamp(explicit.smoothing_positive, 0, 20, 1)),
      smoothing_negative: Math.round(clamp(explicit.smoothing_negative, 0, 20, 1)),
      emission_start_frame: Math.round(clamp(explicit.emission_start_frame, 1, 600, 1)),
      emission_end_frame: Math.round(clamp(explicit.emission_end_frame, 1, 600, 42)),
      emitter_position: Array.isArray(explicit.emitter_position) ? explicit.emitter_position.slice(0,3).map(v=>finite(v,0)) : [0,0,1.2],
      emitter_scale: Array.isArray(explicit.emitter_scale) ? explicit.emitter_scale.slice(0,3).map(v=>clamp(v,0.01,10,0.12)) : [0.12,0.12,0.12],
      initial_velocity: Array.isArray(explicit.initial_velocity) ? explicit.initial_velocity.slice(0,3).map(v=>clamp(v,-20,20,0)) : [0,0,-1.2],
      domain_scale: Array.isArray(explicit.domain_scale) ? explicit.domain_scale.slice(0,3).map(v=>clamp(v,0.1,20,1)) : [1.0,1.0,1.4],
      container_type: text(explicit.container_type || request.container_type || "BASIN", 40).toUpperCase(),
      container_position: Array.isArray(explicit.container_position) ? explicit.container_position.slice(0,3).map(v=>finite(v,0)) : [0,0,0],
      container_radius: clamp(explicit.container_radius, 0.05, 10, 0.45),
      container_height: clamp(explicit.container_height, 0.05, 20, 0.9),
      collision_margin: clamp(explicit.collision_margin, 0, 0.25, 0.002),
      deterministic_replay_required: true,
    };
  }
  if (simulationClass === SIMULATION_CLASSES.PYRO_SMOKE_FIRE) {
    return {
      contract: "AVANTIQO_PYRO_SIMULATION_NUMERIC_PROFILE_V1",
      backend_family: "BLENDER_MANTA_GAS",
      seed,
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.4, 20, 4.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 60, 24)),
      domain_resolution: Math.round(clamp(explicit.domain_resolution, 32, 256, 96)),
      time_scale: clamp(explicit.time_scale, 0.1, 4, 1.0),
      vorticity: clamp(explicit.vorticity, 0, 4, 1.1),
      buoyancy_density: clamp(explicit.buoyancy_density, -5, 5, 1.0),
      buoyancy_heat: clamp(explicit.buoyancy_heat, -5, 5, 1.5),
      density: clamp(explicit.density, 0, 5, 1.0),
      fuel: clamp(explicit.fuel, 0, 5, 1.25),
      temperature: clamp(explicit.temperature, -5, 5, 1.2),
      flame_smoke: text(explicit.flame_smoke || request.flame_smoke || "FIRE_AND_SMOKE", 40).toUpperCase(),
      emission_start_frame: Math.round(clamp(explicit.emission_start_frame, 1, 600, 1)),
      emission_end_frame: Math.round(clamp(explicit.emission_end_frame, 1, 600, 36)),
      dissolve_speed: Math.round(clamp(explicit.dissolve_speed, 1, 250, 55)),
      noise_upres_factor: Math.round(clamp(explicit.noise_upres_factor, 1, 4, 2)),
      adaptive_domain: explicit.adaptive_domain !== false,
      emitter_position: Array.isArray(explicit.emitter_position) ? explicit.emitter_position.slice(0,3).map(v=>finite(v,0)) : [0,0,0],
      emitter_scale: Array.isArray(explicit.emitter_scale) ? explicit.emitter_scale.slice(0,3).map(v=>clamp(v,0.01,10,0.18)) : [0.18,0.18,0.08],
      domain_scale: Array.isArray(explicit.domain_scale) ? explicit.domain_scale.slice(0,3).map(v=>clamp(v,0.1,20,1.0)) : [1.0,1.0,1.8],
      deterministic_replay_required: true,
    };
  }
  if (simulationClass === SIMULATION_CLASSES.DESTRUCTION_FRACTURE) {
    return {
      contract: "AVANTIQO_FRACTURE_SIMULATION_NUMERIC_PROFILE_V1",
      backend_family: "BLENDER_RIGID_BODY_FRACTURE",
      seed,
      duration_seconds: clamp(explicit.duration_seconds ?? request.duration_seconds, 0.2, 20, 3.0),
      frame_rate: Math.round(clamp(explicit.frame_rate ?? request.frame_rate, 12, 120, 24)),
      gravity_mps2: clamp(explicit.gravity_mps2, -30, 30, -9.81),
      source_mass_kg: clamp(explicit.source_mass_kg ?? request.mass_kg, 0.001, 10000, 0.6),
      fracture_piece_count: Math.round(clamp(explicit.fracture_piece_count, 4, 128, 24)),
      fracture_radius: clamp(explicit.fracture_radius, 0.02, 10, 0.32),
      impulse_strength: clamp(explicit.impulse_strength, 0.01, 100, 3.6),
      friction: clamp(explicit.friction, 0, 2, 0.32),
      restitution: clamp(explicit.restitution, 0, 1, 0.12),
      linear_damping: clamp(explicit.linear_damping, 0, 1, 0.08),
      angular_damping: clamp(explicit.angular_damping, 0, 1, 0.1),
      solver_substeps: Math.round(clamp(explicit.solver_substeps, 1, 100, 18)),
      solver_iterations: Math.round(clamp(explicit.solver_iterations, 1, 100, 24)),
      trigger_frame: Math.round(clamp(explicit.trigger_frame, 1, 600, 8)),
      source_position: Array.isArray(explicit.source_position) ? explicit.source_position.slice(0, 3).map((v) => finite(v, 0)) : [0, 0, 0.45],
      floor_z: finite(explicit.floor_z, 0),
      deterministic_replay_required: true,
    };
  }
  return {
    contract: "AVANTIQO_SIMULATION_NUMERIC_PROFILE_PENDING_V1",
    backend_family: "UNSUPPORTED_UNTIL_DEDICATED_SOLVER_AVAILABLE",
    seed,
    deterministic_replay_required: true,
...
function normalizeSimulation(request = {}, src = {}, index = 0) {
  ...
  return {
    ...
    execution_parameters: authoredExecutionParameters(request, simulationClass, index),
...
function evaluateSimulation(simulation = {}) {
  ...
  if (!text(simulation.execution_parameters?.contract, 300)) blockers.push(issue("SIMULATION_NUMERIC_PROFILE_REQUIRED", "execution_parameters", "Simulation requires an authored numeric execution profile before rendering."));
  if (simulation.execution_parameters?.execution_blocked === true) {
    blockers.push(issue("SIMULATION_BACKEND_NOT_IMPLEMENTED", "execution_parameters.backend_family", `No certified execution backend is implemented for ${simulation.simulation_class}.`));
  }
