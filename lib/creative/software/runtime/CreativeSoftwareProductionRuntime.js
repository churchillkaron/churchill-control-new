...
function validateSandboxReport(report, identity, requirements, outputEntries) {
  const failures = [];
  ...
  if (!report.runner?.policy_hash) failures.push("SOFTWARE_SANDBOX_POLICY_HASH_REQUIRED");
  ...
  async build(task) {
    ...
    try {
      ...
      if (report.runner?.policy_hash !== policyHash) {
        report.runner = { ...(report.runner || {}), policy_hash: report.runner?.policy_hash || null };
      }
      const outputEntries = await collectFiles(outputDirectory);
      const audit = validateSandboxReport(report, identity, contract.requirements, outputEntries);
      if (!audit.passed) {
