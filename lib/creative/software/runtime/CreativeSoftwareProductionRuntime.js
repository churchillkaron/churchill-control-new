...
function validateSandboxReport(report, identity) {
  const failures = [];
  ...
  if (report.isolation?.network_disabled !== true && report.network_access !== true) {
  ...
  if (report.build?.passed !== true) failures.push("SOFTWARE_BUILD_FAILED");
  if (report.tests?.passed !== true) failures.push("SOFTWARE_TESTS_FAILED");
  if (report.security?.passed !== true) failures.push("SOFTWARE_SECURITY_AUDIT_FAILED");
  if (!Array.isArray(report.artifacts) || !report.artifacts.length) {
    failures.push("SOFTWARE_BUILD_ARTIFACT_REQUIRED");
  }
...
export const CreativeSoftwareProductionRuntime = {
  async build(task) {
    ...
    const identity = crypto.createHash("sha256").update(JSON.stringify({
      ...
      commands: commandManifest(contract.commands),
    ...
    try {
      ...
      const audit = validateSandboxReport(report, identity);
      if (!audit.passed) {
  ...
  async validate(task) {
    ...
    if (build.sandbox_report?.build?.passed !== true) failures.push("SOFTWARE_BUILD_PASS_REQUIRED");
    if (build.sandbox_report?.tests?.passed !== true) failures.push("SOFTWARE_TEST_PASS_REQUIRED");
    if (build.sandbox_report?.security?.passed !== true) failures.push("SOFTWARE_SECURITY_PASS_REQUIRED");
