...
function leaseState(row = {}) {
  return row.state?.execution_lock_lease || {};
}

function lockToken(row = {}) {
  return leaseState(row).token || null;
}

function lockExpired(row = {}, leaseMs) {
  if (!row.execution_lock) return true;
  const lockedAt = Date.parse(row.locked_at || "");
  if (!Number.isFinite(lockedAt)) return true;
  return Date.now() - lockedAt > leaseMs;
}

function nextState(row = {}, token, values = {}) {
  return {
    ...(row.state || {}),
...
function ownsLock(row, token) {
  return Boolean(
    row?.execution_lock === true &&
    token &&
    lockToken(row) === token,
  );
}
...
function startHeartbeat(values, token, leaseMs) {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      const row = await Repository.get(missionId).catch(() => null);
      if (!ownsLock(row, token)) stopHeartbeat(missionId);
    }
  }, intervalMs);
  timer.unref?.();
...
export async function acquireExecutionLock(input) {
  ...
  const row = await existingState(patch);
  ...
  const existingToken = lockToken(row);
  if (row.execution_lock && existingToken === token) {
    const renewed = await renewExecutionLock({
      ...values,
      execution_lock_token: token,
      execution_lock_lease_ms: leaseMs,
  ...
  if (row.execution_lock && !lockExpired(row, leaseMs)) {
    return false;
  }

  const acquired = await Repository.compareAndSwapExecutionLock({
    creative_mission_id: patch.creative_mission_id,
    expected_execution_lock: row.execution_lock === true,
    expected_locked_at: row.locked_at || null,
    values: {
      ...mergeState(row, patch),
      execution_lock: true,
      locked_at: now,
      state: nextState(row, token, {
        acquired_at: now,
        heartbeat_at: now,
        lease_ms: leaseMs,
        reclaimed_stale_lock: row.execution_lock === true,
...
export async function renewExecutionLock(input) {
  ...
  for (let attempt = 1; attempt <= LOCK_CAS_RETRY_LIMIT; attempt += 1) {
    const row = await existingState(patch);
    if (!ownsLock(row, token)) return false;
...
export async function releaseExecutionLock(input) {
  ...
  for (let attempt = 1; attempt <= LOCK_CAS_RETRY_LIMIT; attempt += 1) {
    const row = await existingState(patch);
    if (!row) return null;
    if (!row.execution_lock) return row;
