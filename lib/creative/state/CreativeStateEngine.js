...
const EXECUTION_LOCK_LEASE_MS = 30 * 60 * 1000;
const NON_RECLAIMABLE_LOCK_STAGES = new Set([
  PIPELINE_STAGES.EXECUTING,
  PIPELINE_STAGES.PRODUCING,
  PIPELINE_STAGES.RENDERING,
  PIPELINE_STAGES.REVIEWING,
...
function reclaimableStaleLock(state = {}, now = Date.now()) {
  if (!state?.execution_lock) return false;
  if (NON_RECLAIMABLE_LOCK_STAGES.has(normalizeStage(state.stage))) return false;
  const lockedAt = Date.parse(state.locked_at || state.updated_at || "");
  return Number.isFinite(lockedAt) && now - lockedAt >= EXECUTION_LOCK_LEASE_MS;
}
...
export async function acquireExecutionLock(input) {
  ...
  if (state?.execution_lock && !reclaimableStaleLock(state)) return false;
