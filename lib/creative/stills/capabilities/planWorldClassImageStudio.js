...
import { buildCreativeStillConversationContinuity } from "@/lib/creative/stills/runtime/CreativeStillConversationContinuityRuntime.js";
...
export async function execute({ context = {}, payload = {} }) {
  const continuity = buildCreativeStillConversationContinuity({
    request: payload.objective,
    project_state: context?.metadata?.operatorProjectState || {},
  });
  const workingObjective = continuity.working_context || text(payload.objective);
  const deliverables = list(payload.deliverables).length
    ? payload.deliverables
    : [{
        type: text(payload.deliverable) || "STILL_IMAGE",
        purpose: workingObjective,
        channels: list(payload.channels),
      }];
  const plan = planCreativeStillWorldClassProduction({
    mission: { title: text(payload.title), business_goal: workingObjective },
    project: { name: text(payload.title), objective: workingObjective },
    brief: {
      title: text(payload.title),
      creative_objective: workingObjective,
  ...
  return { ...plan, conversation_continuity: continuity };
}
