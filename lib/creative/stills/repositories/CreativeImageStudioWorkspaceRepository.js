...
export async function upsertImageStudioArtboard(record = {}) {
  ...
  const expectedUpdatedAt = record.expected_updated_at || null;
  const { expected_updated_at: _expectedUpdatedAt, ...values } = record;
  ...
  const { data, error } = await supabaseAdmin.from(TABLES.artboards).update(payload)
    .eq("id", record.id).eq("organization_id", record.organization_id)
    .eq("creative_project_id", record.creative_project_id).eq("updated_at", expectedUpdatedAt)
    .select().maybeSingle();
  if (error) throw error;
  if (!data) throw new Error("IMAGE_STUDIO_ARTBOARD_CONFLICT");
  return data;
}
export async function upsertImageStudioLayer(record = {}) {
  ...
  const { data, error } = await supabaseAdmin.from(TABLES.layers).update(payload)
    .eq("id", record.id).eq("organization_id", record.organization_id)
    .eq("creative_project_id", record.creative_project_id).eq("updated_at", expectedUpdatedAt)
    .select().maybeSingle();
  if (error) throw error;
  if (!data) throw new Error("IMAGE_STUDIO_LAYER_CONFLICT");
  return data;
}
...
export async function updateImageStudioComment(record = {}) {
  required(record.organization_id, "organization_id");
  required(record.creative_project_id, "creative_project_id");
  required(record.id, "id");
  const patch = {};
  if (Object.prototype.hasOwnProperty.call(record, "assigned_to")) patch.assigned_to = record.assigned_to || null;
  if (record.status) {
    patch.status = record.status;
    patch.resolved_at = record.status === "RESOLVED" ? new Date().toISOString() : null;
  }
  if (!Object.keys(patch).length) throw new Error("comment update required");
...
export async function createImageStudioSnapshot(record = {}) {
  ...
  required(record.artboard_id, "artboard_id");
  required(record.id, "id");
  const { data, error } = await supabaseAdmin.rpc("create_image_studio_snapshot_atomic", {
    p_id: record.id,
    p_organization_id: record.organization_id,
    p_creative_project_id: record.creative_project_id,
    p_artboard_id: record.artboard_id,
    p_snapshot_asset_id: record.snapshot_asset_id || null,
    p_based_on_version_id: record.based_on_version_id || null,
    p_status: record.status || "WORKING",
    p_summary: record.summary || "",
    p_snapshot: record.snapshot || {},
    p_created_by: record.created_by || null,
  });
  if (error) throw error;
  return data;
}
...
export const CreativeImageStudioWorkspaceRepository = Object.freeze({
  ...
  updateComment: updateImageStudioComment,
