...
export function imageStudioAdjustmentPreviewDescriptors(layers=[],target={},options={}){
  ...
  return adjustmentLayersForTarget(layers,target.id).map((layer)=>{
    ...
    if(maskLayer?.visible===false){
      return {id:layer.id,sort_order:Number(layer.sort_order||0),opacity,filter:adjustmentStyle.filter,mask_layer_id:maskId,mask_style:{},preview_supported:true,fidelity:"APPROXIMATE_COLOR_FULL_SCOPE",failures:[],mask_hidden:true};
    }
    ...
    const maskPreview=imageStudioMaskPreviewDescriptor(target,maskLayer,{mask_url:maskUrlByLayerId[maskId]||maskLayer?.metadata?.semantic_matte_preview_url||"",source_url:sourceUrl,preview_image:previewImage});
