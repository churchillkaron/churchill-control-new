...
export function imageStudioBrushFeatherSigma(stroke={}){
  const sizePx=clamp(n(stroke?.size_px,40),1,500);
  const hardness=clamp(n(stroke?.hardness,.8),0,1);
  return Math.max(0,(1-hardness)*sizePx*.45);
}
...
export function imageStudioBrushStrokeSvg(stroke={},region={},width=1,height=1){
  ...
  const blur=imageStudioBrushFeatherSigma(normalized);
...
export const CreativeImageStudioBrushMaskRuntime=Object.freeze({
  ...
  featherSigma:imageStudioBrushFeatherSigma,
