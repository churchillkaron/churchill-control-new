...
function extremaAlpha(raw,width,height,channels,radius,wantMin){
  const alpha=extractAlpha(raw,width,height,channels),horizontal=new Uint8Array(alpha.length);
  for(let y=0;y<height;y++)horizontal.set(extremaLine(alpha.subarray(y*width,(y+1)*width),radius,wantMin),y*width);
  ...
  for(let x=0;x<width;x++){
    ...
    const filtered=extremaLine(column,radius,wantMin);
...
export function applyImageStudioLightWrap(raw,width,height,channels,style={}){
  ...
  const eroded=extremaAlpha(raw,width,height,channels,settings.light_wrap_width_px,true);
  const dilated=extremaAlpha(raw,width,height,channels,settings.light_wrap_width_px,false);
  let wrappedPixelCount=0;
  for(let p=0,i=0;p<width*height;p++,i+=channels){
    const alpha=out[i+3];
    if(alpha===0)continue;
    const edge=clamp(Math.max(alpha-eroded[p],dilated[p]-alpha)/255,0,1);
    if(edge<=0)continue;
  ...
  return {bytes:out,width,height,channels,settings,wrapped_pixel_count:wrappedPixelCount,boundary_band_only:true};
}
