...
export function imageStudioCurvesAreIdentity(curves={}){
  const normalized=normalizeImageStudioCurves(curves);
  return IMAGE_STUDIO_CURVE_CHANNELS.every((name)=>normalized[name].length===2&&normalized[name][0].x===0&&normalized[name][0].y===0&&normalized[name][1].x===1&&normalized[name][1].y===1);
}
...
export const CreativeImageStudioCurvesRuntime=Object.freeze({contract:CREATIVE_IMAGE_STUDIO_CURVES_CONTRACT,channels:IMAGE_STUDIO_CURVE_CHANNELS,normalize:normalizeImageStudioCurves,lut:buildImageStudioCurveLut,isIdentity:imageStudioCurvesAreIdentity,applyPixels:applyImageStudioCurvesToPixels,updatePoint:updateImageStudioCurvePoint,addPoint:addImageStudioCurvePoint,removePoint:removeImageStudioCurvePoint});
