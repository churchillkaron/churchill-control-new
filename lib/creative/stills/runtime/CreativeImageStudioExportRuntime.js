...
export async function renderImageStudioMaster({
  ...
  for (const layer of visible) {
    ...
    if (zoom > 1) {
      const left = Math.max(0, Math.min(scaledWidth - width, Math.round(focalX * scaledWidth - width / 2)));
      const top = Math.max(0, Math.min(scaledHeight - height, Math.round(focalY * scaledHeight - height / 2)));
      prepared = await sharp(prepared).extract({ left, top, width, height }).png().toBuffer();
    }
    const radius = Math.max(0, Math.min(Math.min(width, height) / 2, num(layer.metadata?.mask_radius)));
    if (radius > 0) {
