...
async function semanticExternalMatteBuffer({maskLayer,assets,geometry,width,height}){
  ...
  const asset=assets.get(maskLayer.source_asset_id);
  if(!asset) throw new Error(`IMAGE_STUDIO_SEMANTIC_MASK_ASSET_SCOPE_FAILED:${maskLayer.source_asset_id}`);
  const input=await fetchImage(asset.image_url||asset.file_url||asset.thumbnail_url);
...
async function applyImageStudioEffects(input, style = {}) {
  ...
  const adjustmentActive = adjustment.exposure !== 0 || adjustment.temperature !== 0 || adjustment.tint !== 0 || adjustment.shadows !== 0 || adjustment.highlights !== 0 || adjustment.levels.black !== 0 || adjustment.levels.gamma !== 1 || adjustment.levels.white !== 255 || !imageStudioCurvesAreIdentity(adjustment.curves);
  if (adjustmentActive) {
...
export async function renderImageStudioMaster({
    ...
    if (clipMaskId) {
      ...
      if(semanticValidation.semantic){
        ...
        if(["SUBJECT","BACKGROUND"].includes(semanticValidation.mode)){
          maskBuffer=await semanticExternalMatteBuffer({maskLayer:clipMaskLayer,assets,geometry,width,height});
        }else{
  ...
  return { contract: "CREATIVE_IMAGE_STUDIO_DETERMINISTIC_EXPORT_V2", format: target, width, height, mime_type: mime, file_extension: extension, bytes, typography: { exact_font_asset_ids: [...fontBindings.keys()], embedded_exact_fonts: true }, effects: { contract: "CREATIVE_IMAGE_STUDIO_EFFECTS_V2", non_destructive: true }, adjustments: { contract: "CREATIVE_IMAGE_STUDIO_ADJUSTMENT_V2", curves_contract: "CREATIVE_IMAGE_STUDIO_CURVES_V1", non_destructive: true, deterministic_pixel_pipeline: true, rgb_channel_curves: true }, retouch: { contract: "CREATIVE_IMAGE_STUDIO_RETOUCH_V2", non_destructive: true, bounded_region_operations: true }, adjustment_layers: { contract: "CREATIVE_IMAGE_STUDIO_ADJUSTMENT_LAYER_V1", non_destructive: true, explicit_target_scope: true }, masks: { contract: "CREATIVE_IMAGE_STUDIO_REUSABLE_DESIGN_V3", semantic_contract: "CREATIVE_IMAGE_STUDIO_SEMANTIC_MASK_V1", non_destructive: true, feather_invert_opacity: true, dedicated_mask_layers: true, mask_shapes: ["RECT","ROUNDED_RECT","ELLIPSE"], semantic_modes:["LUMINANCE","COLOR_RANGE","ALPHA","SUBJECT","BACKGROUND"], brush_refinement_contract:"CREATIVE_IMAGE_STUDIO_BRUSH_MASK_V1" } };
}
