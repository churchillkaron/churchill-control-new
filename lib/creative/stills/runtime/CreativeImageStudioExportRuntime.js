...
export async function renderImageStudioMaster({
  ...
  for (const layer of visible) {
    ...
    if (clipMaskId) {
      ...
      if (!maskGeometry.visible) {
        prepared = await sharp({ create: { width, height, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } }).png().toBuffer();
      } else {
        const maskSvg = Buffer.from(`<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><rect x="${maskGeometry.x}" y="${maskGeometry.y}" width="${maskGeometry.width}" height="${maskGeometry.height}" fill="white"/></svg>`);
        prepared = await sharp(prepared).ensureAlpha().composite([{ input: maskSvg, blend: "dest-in" }]).png().toBuffer();
      }
    }
  ...
  return { contract: "CREATIVE_IMAGE_STUDIO_DETERMINISTIC_EXPORT_V2", format: target, width, height, mime_type: mime, file_extension: extension, bytes, typography: { exact_font_asset_ids: [...fontBindings.keys()], embedded_exact_fonts: true }, effects: { contract: "CREATIVE_IMAGE_STUDIO_EFFECTS_V1", non_destructive: true }, masks: { contract: "CREATIVE_IMAGE_STUDIO_REUSABLE_DESIGN_V1", non_destructive: true } };
}
