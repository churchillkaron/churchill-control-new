...
import { normalizeImageStudioTextureIntegration, buildImageStudioGrainTile, restoreImageStudioTextureAlphaAuthority } from "./CreativeImageStudioTextureIntegrationRuntime.js";
...
async function applyImageStudioTextureFinishing(input,style={}){
  ...
  if(settings.grain_amount>0){
    const alphaAuthority=await sharp(rendered).ensureAlpha().raw().toBuffer({resolveWithObject:true});
    ...
    const grained=await sharp(rendered).composite([{input:tilePng,tile:true,blend:"overlay"}]).ensureAlpha().raw().toBuffer({resolveWithObject:true});
    const restored=restoreImageStudioTextureAlphaAuthority(alphaAuthority.data,grained.data,width,height,grained.info.channels);
    rendered=await sharp(restored.bytes,{raw:{width,height,channels:restored.channels}}).png().toBuffer();
  }
