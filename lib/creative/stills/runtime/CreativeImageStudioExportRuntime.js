  ...
  for (const layer of visible) {
    ...
    if (clipMaskId) {
      ...
      if(semanticValidation.semantic){
        ...
        const matteAsset=assets.get(clipMaskLayer.source_asset_id);
        if(!matteAsset) throw new Error(`IMAGE_STUDIO_EXPORT_SMART_MASK_ASSET_MISSING:${clipMaskLayer.source_asset_id||"EMPTY"}`);
        const matteUrl=matteAsset.image_url||matteAsset.file_url||matteAsset.thumbnail_url;
