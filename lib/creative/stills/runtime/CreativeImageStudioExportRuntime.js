...
import { resolveImageStudioSourceUrl } from "./CreativeImageStudioSourceMaterializationRuntime.js";
...
import { imageStudioLookIsIdentity } from "./CreativeImageStudioLookRuntime.js";
...
import { applyImageStudioPerspectiveWarp, normalizeImageStudioPerspectiveWarp } from "./CreativeImageStudioPerspectiveWarpRuntime.js";
...
import { imageStudioPdfPageSize, imageStudioSharpIccProfile, normalizeImageStudioOutputColorSpace, resolveImageStudioOutputDensity } from "./CreativeImageStudioColorManagementRuntime.js";
import { normalizeImageStudioExportFormat } from "./CreativeImageStudioExportFormatRuntime.js";
...
async function buildImageStudioRasterMaskBuffer(input, geometry, semantics){
  const { width,height,zoom,scaledWidth,scaledHeight,extractLeft,extractTop,renderedWidth,renderedHeight,coverLeft,coverTop }=geometry;
  let matte=sharp(input).resize(renderedWidth,renderedHeight,{fit:"fill"}).extract({left:coverLeft,top:coverTop,width:scaledWidth,height:scaledHeight});
  let bytes=await matte.greyscale().raw().toBuffer({resolveWithObject:true});
  if(zoom>1){
    const png=await sharp(bytes.data,{raw:{width:bytes.info.width,height:bytes.info.height,channels:bytes.info.channels}}).png().toBuffer();
    bytes=await sharp(png).extract({left:extractLeft,top:extractTop,width,height}).greyscale().raw().toBuffer({resolveWithObject:true});
  }
  const alpha=Buffer.alloc(width*height*4);
  const opacity=Math.max(0,Math.min(1,Number(semantics.opacity??1)));
  const invert=semantics.invert===true;
  for(let i=0;i<width*height;i++){
    const source=bytes.data[i*bytes.info.channels];
    const value=invert?255-source:source;
    const a=Math.round(value*opacity);
    const o=i*4; alpha[o]=255;alpha[o+1]=255;alpha[o+2]=255;alpha[o+3]=a;
  }
  let out=await sharp(alpha,{raw:{width,height,channels:4}}).png().toBuffer();
  const feather=Math.max(0,Number(semantics.feather||0));
  if(feather>0)out=await sharp(out).blur(Math.max(.3,feather)).png().toBuffer();
  return out;
}
...
async function applyImageStudioEffects(input, style = {}, {apply_retouch=true} = {}) {
  ...
  const retouchOperations = apply_retouch && Array.isArray(style.retouch_operations) ? style.retouch_operations : [];
...
async function normalizeImageStudioSourceOrientation(input){
  const sourceMetadata=await sharp(input).metadata();
  const orientation=Number(sourceMetadata.orientation||1);
  if(orientation<=1){
    return {bytes:input,width:sourceMetadata.width,height:sourceMetadata.height,source_orientation:orientation,orientation_normalized:false};
  }
  const bytes=await sharp(input).autoOrient().png().toBuffer();
  const normalizedMetadata=await sharp(bytes).metadata();
  return {bytes,width:normalizedMetadata.width,height:normalizedMetadata.height,source_orientation:orientation,orientation_normalized:true};
}
...
export async function renderImageStudioMaster({
  ...
  const unapprovedSemanticMattes=layers.filter((layer)=>
    layer.visible!==false && layer.layer_type==="MASK" &&
  ...
  const unapprovedSmartMasks=layers.filter((layer)=>
    layer.visible!==false && layer.layer_type==="MASK" && layer.metadata?.smart_mask===true && layer.metadata?.smart_mask_review_approved!==true
  );
  if(unapprovedSmartMasks.length){
    throw new Error(`IMAGE_STUDIO_EXPORT_SMART_MASK_REVIEW_REQUIRED:${unapprovedSmartMasks.map((layer)=>layer.id).join(",")}`);
  }
  ...
  const composites = [];
  const sourceOrientationEvidence=[];
  for (const layer of visible) {
    if (layer.layer_type === "TEXT") {
      const svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">${renderFontFaces(fontBindings)}${textSvg(layer, fontBindings)}</svg>`;
      let preparedText = await sharp(Buffer.from(svg)).png().toBuffer();
      const effectedText = await applyImageStudioEffects(preparedText, layer.style || {});
      preparedText = effectedText.bytes;
      composites.push({ input: preparedText, left: 0, top: 0, blend: effectedText.effects.sharp_blend_mode });
      continue;
    }
    ...
    const sourceReference = asset.image_url || asset.file_url || asset.thumbnail_url;
    const url = await resolveImageStudioSourceUrl({organization_id,value:sourceReference});
    const input = await fetchImage(url);
    const orientedSource=await normalizeImageStudioSourceOrientation(input);
    sourceOrientationEvidence.push({layer_id:layer.id,source_orientation:orientedSource.source_orientation,orientation_normalized:orientedSource.orientation_normalized,width:orientedSource.width,height:orientedSource.height});
    const b = layer.bounds || {};
    const geometry = imageStudioSourceCropGeometry(layer, { width: orientedSource.width, height: orientedSource.height });
    ...
    let prepared = await sharp(orientedSource.bytes)
    ...
    const semanticSourcePrepared=Buffer.from(prepared);
    if (maskRadius > 0) {
    ...
    const clipMaskLayer = clipMaskId ? layerById.get(clipMaskId) : null;
    if (clipMaskId && !clipMaskLayer) throw new Error(`IMAGE_STUDIO_EXPORT_CLIP_MASK_MISSING:${clipMaskId}`);
    if (clipMaskLayer?.visible!==false && clipMaskLayer) {
      ...
      if(semanticValidation.semantic){
        ...
        if(["SUBJECT","BACKGROUND"].includes(semanticValidation.mode)){
          ...
          maskBuffer=await semanticAlphaMaskFromBuffer(semanticSourcePrepared,width,height,clipMaskLayer.metadata||{});
        }
      }else if(clipMaskLayer.metadata?.mask_source_kind==="RASTER_MATTE"){
        const matteReference=String(clipMaskLayer.metadata?.mask_provenance?.matte_storage_reference||"").trim();
        if(!matteReference.startsWith("storage://")) throw new Error(`IMAGE_STUDIO_EXPORT_SMART_MASK_STORAGE_REFERENCE_REQUIRED:${clipMaskLayer.id}`);
        const matteUrl=await signCreativeStorageReference({organization_id,reference:matteReference,expires_in:900});
        const matteInput=await fetchImage(matteUrl);
        maskBuffer=await buildImageStudioRasterMaskBuffer(matteInput,geometry,maskGeometry);
    ...
    const retouchOperations=Array.isArray(layer.style?.retouch_operations)?layer.style.retouch_operations:[];
    if(retouchOperations.length){
      const retouchRaw=await sharp(prepared).ensureAlpha().raw().toBuffer({resolveWithObject:true});
      const retouched=applyImageStudioRetouchOperations(retouchRaw.data,retouchRaw.info.width,retouchRaw.info.height,retouchRaw.info.channels,retouchOperations);
      prepared=await sharp(retouched.bytes,{raw:{width:retouched.width,height:retouched.height,channels:retouched.channels}}).png().toBuffer();
    }
    ...
    const effected = await applyImageStudioEffects(prepared, layer.style || {}, {apply_retouch:false});
    ...
    if (adjustmentLayers.length) {
      ...
      for(const adjustmentLayer of adjustmentLayers){
        ...
        if(adjustmentMaskLayer?.visible===false)continue;
        ...
        if(semanticValidation.semantic){
          ...
          if(["SUBJECT","BACKGROUND"].includes(semanticValidation.mode)){
            ...
            adjustmentMaskBuffer=await semanticAlphaMaskFromBuffer(semanticSourcePrepared,width,height,adjustmentMaskLayer.metadata||{});
          }
        }else if(adjustmentMaskLayer.metadata?.mask_source_kind==="RASTER_MATTE"){
          const matteReference=String(adjustmentMaskLayer.metadata?.mask_provenance?.matte_storage_reference||"").trim();
          if(!matteReference.startsWith("storage://"))throw new Error(`IMAGE_STUDIO_ADJUSTMENT_SMART_MASK_STORAGE_REFERENCE_REQUIRED:${adjustmentMaskLayer.id}`);
          const matteUrl=await signCreativeStorageReference({organization_id,reference:matteReference,expires_in:900});
          const matteInput=await fetchImage(matteUrl);
          adjustmentMaskBuffer=await buildImageStudioRasterMaskBuffer(matteInput,geometry,maskGeometry);
        }else if(!maskGeometry.visible&&!maskGeometry.invert){
    ...
    const perspective=normalizeImageStudioPerspectiveWarp(layer.style||{});
    if(perspective.enabled){
      const warpRaw=await sharp(prepared).ensureAlpha().raw().toBuffer({resolveWithObject:true});
      const warped=applyImageStudioPerspectiveWarp(warpRaw.data,warpRaw.info.width,warpRaw.info.height,warpRaw.info.channels,layer.style||{});
      prepared=await sharp(warped.bytes,{raw:{width:warped.width,height:warped.height,channels:warped.channels}}).png().toBuffer();
    }
    const rotation = num(layer.transform?.rotation);
    if (rotation) {
  ...
  const outputColorSpace=normalizeImageStudioOutputColorSpace(artboard.metadata?.output_color_space||"SRGB");
  const iccProfile=imageStudioSharpIccProfile(outputColorSpace);
  const outputDensity=resolveImageStudioOutputDensity(artboard);
  let base = sharp({ create: { width, height, channels: 4, background } }).composite(composites).withIccProfile(iccProfile);
  if(outputDensity)base=base.withMetadata({density:outputDensity});
  let bytes;
  let mime;
  let extension;
  const target = normalizeImageStudioExportFormat(format);
  if (target === "JPEG") {
    bytes = await base.clone().jpeg({ quality: 95, chromaSubsampling:"4:4:4" }).toBuffer();
    mime = "image/jpeg";
    extension = "jpg";
  } else if (target === "PDF") {
    ...
    const pdfPage=imageStudioPdfPageSize(width,height,outputDensity);
    const page = pdf.addPage([pdfPage.width_points, pdfPage.height_points]);
    const image = await pdf.embedPng(png);
    page.drawImage(image, { x: 0, y: 0, width:pdfPage.width_points, height:pdfPage.height_points });
    bytes = Buffer.from(await pdf.save());
    mime = "application/pdf";
    extension = "pdf";
  } else {
    bytes = await base.clone().png({ compressionLevel: 9 }).toBuffer();
    mime = "image/png";
    extension = "png";
  }
  return { contract: "CREATIVE_IMAGE_STUDIO_DETERMINISTIC_EXPORT_V2", format: target, width, height, mime_type: mime, file_extension: extension, bytes, source_orientation: { normalized_before_geometry:true, layers:sourceOrientationEvidence }, color_management: { contract:"CREATIVE_IMAGE_STUDIO_COLOR_MANAGEMENT_V1", output_color_space:outputColorSpace, icc_profile:iccProfile, raster_profile_embedded:true, output_density_dpi:outputDensity, density_metadata_embedded:Boolean(outputDensity), pdf_output_intent_embedded:false }, typography: { exact_font_asset_ids: [...fontBindings.keys()], embedded_exact_fonts: true }, effects: { contract: "CREATIVE_IMAGE_STUDIO_EFFECTS_V2", non_destructive: true }, adjustments: { contract: "CREATIVE_IMAGE_STUDIO_ADJUSTMENT_V4", curves_contract: "CREATIVE_IMAGE_STUDIO_CURVES_V1", color_grade_contract: "CREATIVE_IMAGE_STUDIO_COLOR_GRADE_V1", look_contract: "CREATIVE_IMAGE_STUDIO_LOOK_V1", non_destructive: true, deterministic_pixel_pipeline: true, rgb_channel_curves: true, color_balance: true, channel_mixer: true, selective_color: true, gradient_map: true, split_toning: true, creative_looks: true }, retouch: { contract: "CREATIVE_IMAGE_STUDIO_RETOUCH_V3", non_destructive: true, bounded_region_operations: true, rotation_aware_regions: true }, edge_integration: { contract: "CREATIVE_IMAGE_STUDIO_EDGE_INTEGRATION_V1", non_destructive: true, matte_morphology: true, despill: true, decontamination: true }, contact_realism: { contract: "CREATIVE_IMAGE_STUDIO_CONTACT_REALISM_V1", non_destructive: true, light_wrap: true, ground_shadow: true }, texture_integration: { contract: "CREATIVE_IMAGE_STUDIO_TEXTURE_INTEGRATION_V1", non_destructive: true, sharpen: true, bloom: true, deterministic_grain: true }, adjustment_layers: { contract: "CREATIVE_IMAGE_STUDIO_ADJUSTMENT_LAYER_V2", non_destructive: true, explicit_target_scope: true, independent_masks: true, semantic_masks: true, brush_refinement: true }, masks: { contract: "CREATIVE_IMAGE_STUDIO_REUSABLE_DESIGN_V3", semantic_contract: "CREATIVE_IMAGE_STUDIO_SEMANTIC_MASK_V2", non_destructive: true, feather_invert_opacity: true, dedicated_mask_layers: true, mask_shapes: ["RECT","ROUNDED_RECT","ELLIPSE"], semantic_modes:["LUMINANCE","COLOR_RANGE","ALPHA","SUBJECT","BACKGROUND"], brush_refinement_contract:"CREATIVE_IMAGE_STUDIO_BRUSH_MASK_V1", raster_matte_sources:true, source_geometry_locked:true, smart_mask_review_gate:true } };
}
