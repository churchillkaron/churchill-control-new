  ...
  for (const layer of visible) {
    ...
    if (adjustmentLayers.length) {
      ...
      for(const adjustmentLayer of adjustmentLayers){
        ...
        if(!adjustmentMaskLayer)throw new Error(`IMAGE_STUDIO_ADJUSTMENT_MASK_MISSING:${adjustmentMaskId}`);
        if(adjustmentMaskLayer.metadata?.clip_mask_target_id!==layer.id)throw new Error(`IMAGE_STUDIO_ADJUSTMENT_MASK_TARGET_MISMATCH:${adjustmentMaskId}`);
