...
export function buildCreativeImageStudioOperatingState(runtime = {}) {
  ...
  const refinementReady = refineTasks.some(completed) || images.some((asset) => Number(asset?.revision || asset?.version || 1) > 1);
  const reviewReady = reviewTasks.some(completed) || images.some((asset) => ["APPROVED", "PASSED", "RELEASE_READY"].includes(text(asset?.approval_state || asset?.status).toUpperCase()));
  const deliveryReady = Boolean(publishing?.published_at || publishing?.status === "COMPLETED" || render?.release_ready) || deliveryTasks.some(completed);
