...
export function imageStudioMaskGeometry(target = {}, mask = {}) {
  ...
  return { visible, x:Math.max(0,left-t.x), y:Math.max(0,top-t.y), width:Math.max(0,right-left), height:Math.max(0,bottom-top), target_width:t.width, target_height:t.height };
}
export function imageStudioMaskPreviewStyle(target = {}, mask = {}) {
  const g=imageStudioMaskGeometry(target,mask); if(!g.visible)return { clipPath:"inset(100% 0 0 0)" };
