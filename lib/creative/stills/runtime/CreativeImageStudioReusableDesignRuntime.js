...
function geometricMaskSvg(g){
  ...
  const filter=blur>0?`<defs><filter id="f" x="-50%" y="-50%" width="200%" height="200%" color-interpolation-filters="sRGB"><feGaussianBlur stdDeviation="${Math.max(.3,blur)}"/></filter></defs>`:"";
  const filterAttr=blur>0?' filter="url(#f)"':"";
  const shape=g.shape==="ELLIPSE"
    ?`<ellipse cx="${x+w/2}" cy="${y+h/2}" rx="${w/2}" ry="${h/2}"${filterAttr}/>`
    :`<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${g.shape==="ROUNDED_RECT"?g.radius||0:0}" ry="${g.shape==="ROUNDED_RECT"?g.radius||0:0}"${filterAttr}/>`;
  const body=g.invert
    ?`<mask id="m" maskUnits="userSpaceOnUse" x="0" y="0" width="${width}" height="${height}"><rect width="100%" height="100%" fill="white"/><g fill="black">${shape}</g></mask><rect width="100%" height="100%" fill="white" fill-opacity="${g.opacity}" mask="url(#m)"/>`
    :shape.replace("/>",` fill="white" fill-opacity="${g.opacity}"/>`);
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none">${filter}${body}</svg>`;
}
...
export function imageStudioMaskPreviewStyle(target = {}, mask = {}) {
  ...
  return geometricMaskCssStyle(g);
}
...
export function imageStudioMaskPreviewDescriptor(target = {}, mask = {}) {
  ...
  const fidelity=g.feather>0?"APPROXIMATE_RASTER_EXACT_GEOMETRY":(g.invert||g.opacity<.999)?"EXACT_GEOMETRIC_ALPHA_SCOPE":"EXACT_GEOMETRIC_SCOPE";
