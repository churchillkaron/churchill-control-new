import { imageStudioBrushFeatherSigma, normalizeImageStudioBrushStrokes } from "./CreativeImageStudioBrushMaskRuntime.js";
...
function safeBrushStrokes(mask={}){
  const raw=Array.isArray(mask?.metadata?.mask_brush_strokes)?mask.metadata.mask_brush_strokes:[];
  if(!raw.length)return {ready:true,strokes:[]};
  try{return {ready:true,strokes:normalizeImageStudioBrushStrokes(raw)};}
  catch{return {ready:false,strokes:[]};}
}
function brushStrokeSvgParts(strokes,g){
  const defs=[],paths=[];
  strokes.forEach((stroke,index)=>{
    const points=(stroke.points||[]).map((point)=>({x:g.x+point.x*g.width,y:g.y+point.y*g.height}));
    if(!points.length)return;
    const d=points.length===1
      ?`M ${points[0].x} ${points[0].y} L ${points[0].x+.01} ${points[0].y+.01}`
      :`M ${points.map((point)=>`${point.x} ${point.y}`).join(" L ")}`;
    const blur=imageStudioBrushFeatherSigma(stroke);
    const filterId=`brush-${index}`;
    if(blur>0)defs.push(`<filter id="${filterId}" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="${blur}"/></filter>`);
    paths.push(`<path d="${d}" fill="none" stroke="${stroke.mode==="SUBTRACT"?"black":"white"}" stroke-width="${stroke.size_px}" stroke-linecap="round" stroke-linejoin="round" opacity="${stroke.opacity}"${blur>0?` filter="url(#${filterId})"`:""}/>`);
  });
  return {defs,paths};
}
function geometricMaskSvg(g,strokes=[]){
  ...
  const blur=Math.max(0,Number(g.feather||0));
  const baseFilter=blur>0?`<filter id="base-feather" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="${Math.max(.3,blur)}"/></filter>`:"";
  const baseFilterAttr=blur>0?' filter="url(#base-feather)"':"";
  ...
  const base=g.invert
    ?`<g${baseFilterAttr}><rect width="100%" height="100%" fill="white" opacity="${g.opacity}"/><g fill="black">${shape}</g></g>`
    :`<g${baseFilterAttr}>${shape.replace("/>",` fill="white" fill-opacity="${g.opacity}"/>`)}</g>`;
  const brush=brushStrokeSvgParts(strokes,g);
  const defs=`${baseFilter}${brush.defs.join("")}`;
  const mask=`<mask id="preview-mask" maskUnits="userSpaceOnUse" x="0" y="0" width="${width}" height="${height}"><rect width="100%" height="100%" fill="black"/>${base}${brush.paths.join("")}</mask>`;
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none"><defs>${defs}${mask}</defs><rect width="100%" height="100%" fill="white" mask="url(#preview-mask)"/></svg>`;
}
function geometricMaskCssStyle(g,strokes=[]){
  const encoded=encodeURIComponent(geometricMaskSvg(g,strokes));
...
function rasterMatteCssStyle(maskUrl,previewImage={},maskMode="luminance"){
  const url=String(maskUrl||"").trim();
  if(!/^https?:\/\//i.test(url))return null;
  const width=Math.max(1,n(previewImage.width,1)),height=Math.max(1,n(previewImage.height,1));
  const left=n(previewImage.left),top=n(previewImage.top);
  const image=`url("${url.replaceAll("\\","\\\\").replaceAll('"','\\"')}")`;
  return {
    maskImage:image,WebkitMaskImage:image,maskMode:maskMode==="alpha"?"alpha":"luminance",
    maskSize:`${width}px ${height}px`,WebkitMaskSize:`${width}px ${height}px`,
    maskRepeat:"no-repeat",WebkitMaskRepeat:"no-repeat",
    maskPosition:`${left}px ${top}px`,WebkitMaskPosition:`${left}px ${top}px`,
  };
}
export function imageStudioMaskPreviewStyle(target = {}, mask = {}) {
  const g=imageStudioMaskGeometry(target,mask);
  const brush=safeBrushStrokes(mask);
  const strokes=brush.ready?brush.strokes:[];
  if(!g.visible && !g.invert&&!strokes.length)return { clipPath:"inset(100% 0 0 0)" };
  if(g.feather<=0 && !g.invert && g.opacity>=.999&&!strokes.length){
    if(g.shape==="ELLIPSE"){
  ...
  return geometricMaskCssStyle(g,strokes);
}

export function imageStudioMaskPreviewDescriptor(target = {}, mask = {}, options = {}) {
  ...
  const brush=safeBrushStrokes(mask);
  const g=imageStudioMaskGeometry(target,mask);
  const simpleRasterSemantics=!brushRefined&&!g.invert&&g.opacity>=.999&&g.feather<=0;
  if(semantic&&String(metadata.semantic_mask_mode||"").toUpperCase()==="ALPHA"&&simpleRasterSemantics){
    const alphaStyle=rasterMatteCssStyle(options.source_url,options.preview_image||{},"alpha");
    if(alphaStyle)return {preview_supported:true,fidelity:"SOURCE_ALPHA_EXACT_GEOMETRY",style:alphaStyle,reason:null,geometry:g};
  }
  if((semantic||raster)&&simpleRasterSemantics){
    const matteStyle=rasterMatteCssStyle(options.mask_url||metadata.semantic_matte_preview_url,options.preview_image||{});
    if(matteStyle)return {preview_supported:true,fidelity:"BROWSER_LUMINANCE_MATTE_EXACT_GEOMETRY",style:matteStyle,reason:null,geometry:g};
  }
  ...
  if(brushRefined&&!brush.ready)return {preview_supported:false,fidelity:"EXPORT_ONLY",style:{},reason:"MASK_BRUSH_PREVIEW_INVALID",geometry:g};
  const fidelity=(g.feather>0||brushRefined)?"APPROXIMATE_RASTER_EXACT_GEOMETRY":(g.invert||g.opacity<.999)?"EXACT_GEOMETRIC_ALPHA_SCOPE":"EXACT_GEOMETRIC_SCOPE";
  return {preview_supported:true,fidelity,style:imageStudioMaskPreviewStyle(target,mask),reason:null,geometry:g};
}
