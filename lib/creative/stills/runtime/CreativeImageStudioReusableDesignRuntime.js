...
export function imageStudioMaskPreviewDescriptor(target = {}, mask = {}) {
  ...
  const metadata=mask.metadata||{};
  const semantic=String(metadata.mask_source_kind||"").toUpperCase()==="SEMANTIC";
  const raster=String(metadata.mask_source_kind||"").toUpperCase()==="RASTER";
  ...
  if(g.feather>0||g.invert||g.opacity<.999)return {preview_supported:false,fidelity:"EXPORT_ONLY",style:{},reason:"MASK_SEMANTICS_PREVIEW_COMPLEX",geometry:g};
  return {preview_supported:true,fidelity:"EXACT_GEOMETRIC_SCOPE",style:imageStudioMaskPreviewStyle(target,mask),reason:null,geometry:g};
}
