...
function rasterMatteCssStyle(maskUrl,previewImage={}){
  ...
  return {
    maskImage:image,WebkitMaskImage:image,maskMode:"luminance",
...
export function imageStudioMaskPreviewDescriptor(target = {}, mask = {}, options = {}) {
  ...
  const brush=safeBrushStrokes(mask);
  const g=imageStudioMaskGeometry(target,mask);
  if((semantic||raster)&&!brushRefined&&!g.invert&&g.opacity>=.999&&g.feather<=0){
