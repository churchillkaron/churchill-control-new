...
export function buildImageStudioGrainTile(style={},size=1024){
  ...
  if(settings.grain_amount<=0)return {bytes:out,width:tileSize,height:tileSize,channels:4,settings};
  ...
  for(let by=0;by<tileSize;by+=block)for(let bx=0;bx<tileSize;bx+=block){
    ...
    for(let y=by;y<Math.min(tileSize,by+block);y++)for(let x=bx;x<Math.min(tileSize,bx+block);x++){
      ...
      out[i]=values[0];out[i+1]=values[1];out[i+2]=values[2];
      out[i+3]=Math.round(settings.grain_amount*255);
    }
  }
  return {bytes:out,width:tileSize,height:tileSize,channels:4,settings};
}
