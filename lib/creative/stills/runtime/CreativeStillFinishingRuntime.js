...
import {
  evaluateCreativeStillWorldClassQuality,
} from "./CreativeStillWorldClassQualityRuntime.js";
import {
  directCreativeStillRepairs,
} from "./CreativeStillRepairDirectorRuntime.js";
import {
  settleCreativeStillStyleMemory,
} from "./CreativeStillStyleMemorySettlementRuntime.js";
import {
  evaluateCreativeStillDailies,
} from "./CreativeStillDailiesRuntime.js";
...
function premiumAdvertisingBenchmarkRequired(task = {}, design = {}) {
  const corpus = [
    task.title, task.description,
    task.input?.deliverable?.type, task.input?.deliverable_type, task.metadata?.deliverable_type,
    task.input?.requirements?.deliverable, task.metadata?.requirements?.deliverable,
    task.input?.requirements?.expected_contract?.deliverable_type,
    task.metadata?.requirements?.expected_contract?.deliverable_type,
    design.art_direction_authority?.creative_objective,
  ].map(text).filter(Boolean).join(" ").toLowerCase();
  return ["advert", " ad ", "campaign", "commercial", "launch", "brand", "social", "hero"].some((term) => corpus.includes(term));
}

function premiumGraphicBenchmarkRequired(task = {}, design = {}) {
  if ((design.text_layers || []).length || (design.logo_layers || []).length) return true;
  const corpus = [
    task.input?.deliverable?.type, task.input?.deliverable_type, task.metadata?.deliverable_type,
    task.input?.requirements?.deliverable, task.metadata?.requirements?.deliverable,
    task.input?.requirements?.expected_contract?.deliverable_type,
  ...
  return ["poster", "banner", "advert", "flyer", "menu", "brochure", "social", "carousel", "thumbnail", "signage", "graphic", "campaign"].some((term) => corpus.includes(term));
}
...
function textSvg(layer, variant) {
  ...
  const maximumCharacters = Math.max(
    1,
    Math.floor(width / Math.max(1, fontSize * 0.56 + Number(layer.letter_spacing || 0))),
  );
...
function generatedSourceTask(task, tasks) {
  return dependencyTasks(task, tasks).find((candidate) =>
    stillOutputUrl(candidate.output) || stillOutputBase64(candidate.output),
  ) || null;
}

async function materializeSource(task, sourceTask, policy) {
  const url = stillOutputUrl(sourceTask.output);
  if (url) {
  ...
  const encoded = stillOutputBase64(sourceTask.output);
  if (!encoded) throw new Error("CREATIVE_STILL_SOURCE_MEDIA_REQUIRED");
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-still-source-"));
...
async function resolveAssetNode(task, assetId) {
  const direct = await AssetGraphRepository.getById(assetId);
  if (direct?.organization_id === task.organization_id) return direct;
  const nodes = await AssetGraphRepository.listByProject({
    organization_id: task.organization_id,
    creative_project_id: task.creative_project_id,
  });
  return nodes.find((node) => node.creative_asset_id === assetId) || null;
}
...
async function materializeLogoLayers(task, design, policy) {
  ...
  for (const layer of design.logo_layers) {
    ...
    const node = await resolveAssetNode(task, layer.asset_id);
    if (!node?.url) {
...
async function renderVariant({ sourcePath, logos, design, variant, outputPath }) {
  ...
  else pipeline = pipeline.avif({ quality: variant.quality });
...
export const CreativeStillFinishingRuntime = {
  async finish(task) {
    ...
    const source = await materializeSource(task, sourceTask, policy);
    ...
    return {
      ...
      creative_master_plan_hash: design.art_direction_authority.creative_master_plan_hash,
      art_direction_id: design.art_direction_authority.art_direction_id,
      art_director_decision_id: design.art_direction_authority.art_director_decision_id,
      production_design_id: design.art_direction_authority.production_design_id,
      brand_direction_id: design.art_direction_authority.brand_direction_id,
      art_direction_authority_contract: design.art_direction_authority.contract,
      variants: outputs,
      exact_brand_assets_applied: logos.length,
      exact_brand_asset_ids: logos.map((item) => item.node.id),
      text_layers_applied: design.text_layers.map((layer) => ({
        id: layer.id,
        role: layer.role,
        text: layer.text,
      })),
  ...
  async validate(task) {
    ...
    const finished = dependencies.find((candidate) =>
      list(unwrapStillOutput(candidate.output)?.variants).length,
    ) || null;
    const semantic = dependencies.find((candidate) => candidate !== finished) || null;
    ...
    if (output.art_direction_authority_contract !== "CREATIVE_STILL_ART_DIRECTION_AUTHORITY_V1") {
      failures.push("STILL_ART_DIRECTION_AUTHORITY_EVIDENCE_REQUIRED");
    }
    for (const field of [
      "creative_master_plan_hash",
      "art_direction_id",
      "art_director_decision_id",
      "production_design_id",
      "brand_direction_id",
    ]) {
      if (!text(output[field])) failures.push(`STILL_ART_AUTHORITY_LINEAGE_MISSING:${field}`);
    }
    ...
    if (design.exact_brand_assets_required &&
      Number(output.exact_brand_assets_applied || 0) < design.logo_layers.length) {
      failures.push("STILL_EXACT_BRAND_ASSETS_NOT_APPLIED");
    }
    let worldClassQuality = null;
    let dailies = null;
    if (!semantic || semantic.status !== "COMPLETED") {
      failures.push("STILL_SEMANTIC_REVIEW_REQUIRED");
    } else {
      if (!explicitStillQualityPass(semantic.output)) {
        failures.push(...stillQualityFailures(semantic.output));
        if (!stillQualityFailures(semantic.output).length) {
          failures.push("STILL_SEMANTIC_QUALITY_REJECTED");
        }
      }
      if (task.metadata?.world_class_quality_required === true || task.input?.world_class_quality_required === true) {
        const expected = object(
          task.input?.requirements?.expected_contract ||
          task.metadata?.requirements?.expected_contract,
        );
        worldClassQuality = evaluateCreativeStillWorldClassQuality({
          evidence: semantic.output,
          requirements: {
            identity_required: expected.identity_expected === true,
            product_required: expected.product_expected === true,
            premium_graphic_benchmark_required: premiumGraphicBenchmarkRequired(task, design),
            premium_advertising_benchmark_required: premiumAdvertisingBenchmarkRequired(task, design),
          },
        });
        dailies = evaluateCreativeStillDailies({
          intent: expected,
          evidence: semantic.output,
        });
        if (!worldClassQuality.passed) failures.push(...worldClassQuality.failures);
        if (!dailies.passed) failures.push(...dailies.failures);
      }
    }
    const uniqueFailures = [...new Set(failures)];
    const repairPlan = directCreativeStillRepairs({
      failures: uniqueFailures,
      existing_repair_instructions: worldClassQuality?.repair_instructions || [],
    });
    const releaseValidation = {
      passed: uniqueFailures.length === 0,
      verdict: uniqueFailures.length === 0 ? "PASSED" : "FAILED",
    };
    const styleMemorySettlement = await settleCreativeStillStyleMemory({
      project_id: task.creative_project_id,
      task,
      design,
    ...
    const styleMemory = styleMemorySettlement.memory;
    return {
      ...releaseValidation,
      overall_score: uniqueFailures.length === 0 ? 1 : 0,
      failed_checks: uniqueFailures,
      repair_instructions: repairPlan.repairs.map((repair) => ({
        kind: repair.kind,
        capability: repair.capability,
        failures: repair.failures,
        preserve: repair.preserve,
        regeneration_scope: repair.regeneration_scope,
      })),
      ...
      checks: {
        ...
        exact_brand_assets_applied: !design.exact_brand_assets_required ||
          Number(output.exact_brand_assets_applied || 0) >= design.logo_layers.length,
        art_direction_authority_preserved:
          output.art_direction_authority_contract === "CREATIVE_STILL_ART_DIRECTION_AUTHORITY_V1" &&
          text(output.art_direction_id) === text(design.art_direction_authority.art_direction_id) &&
          text(output.production_design_id) === text(design.art_direction_authority.production_design_id),
        semantic_quality_passed: Boolean(semantic && explicitStillQualityPass(semantic.output)),
        world_class_quality_passed: worldClassQuality ? worldClassQuality.passed === true : null,
      },
      world_class_quality: worldClassQuality,
      dailies,
      style_memory: styleMemory,
      style_memory_settlement: styleMemorySettlement,
    };
  },
};
