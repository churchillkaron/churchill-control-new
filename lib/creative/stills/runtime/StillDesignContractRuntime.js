...
export function unwrapStillOutput(value = {}) {
  ...
  while (current && typeof current === "object" && !seen.has(current)) {
    ...
    if (typeof current.text === "string") {
      const parsed = parseStructuredText(current.text);
      if (parsed) {
        current = parsed;
        continue;
      }
  ...
  if (typeof current === "string") return parseStructuredText(current) || current;
  return current || {};
}
...
export function stillOutputBase64(value = {}) {
  const candidates = [
    value,
    value.output,
    value.result,
  ...
  for (const candidate of candidates.filter(Boolean)) {
    const resolved = unwrapStillOutput(candidate);
    if (!resolved || typeof resolved !== "object") continue;
    const encoded = resolved.image_base64 || resolved.imageBase64 ||
      resolved.b64_json || resolved.images?.[0]?.b64_json ||
      resolved.raw?.data?.[0]?.b64_json || null;
    if (encoded) return String(encoded).replace(/^data:image\/[^;]+;base64,/, "");
  }
  return null;
}
...
export function resolveStillDesign(task = {}) {
  ...
  const authority = object(
    spec.art_direction_authority ||
    task.input?.requirements?.art_direction_authority ||
    task.metadata?.art_direction_authority,
  );
  if (authority.contract !== "CREATIVE_STILL_ART_DIRECTION_AUTHORITY_V1") {
    throw new Error("CREATIVE_STILL_ART_DIRECTION_AUTHORITY_REQUIRED");
  }
  for (const field of [
    ...
    if (!text(authority[field])) {
      throw new Error(`CREATIVE_STILL_ART_DIRECTION_AUTHORITY_FIELD_REQUIRED:${field}`);
    }
  }
  if (!text(authority.dossier?.governing_visual_idea)) {
    throw new Error("CREATIVE_STILL_ART_DIRECTION_DOSSIER_REQUIRED");
  }
  if (authority.image_worker_authority !== "EXECUTE_ONLY" || authority.reinterpretation_allowed !== false) {
    throw new Error("CREATIVE_STILL_IMAGE_WORKER_AUTHORITY_INVALID");
  }
  ...
  return {
    art_direction_authority: authority,
    variants,
...
export function explicitStillQualityPass(value = {}) {
  const unwrapped = unwrapStillOutput(value);
  if (typeof unwrapped === "string") {
    const verdict = text(unwrapped).toUpperCase();
    return /\b(PASS|PASSED|APPROVED|RELEASE_READY)\b/.test(verdict) &&
      !/\b(FAIL|FAILED|REJECTED|BLOCKED)\b/.test(verdict);
  }
  const evidence = object(unwrapped);
...
export function stillQualityFailures(value = {}) {
  const unwrapped = unwrapStillOutput(value);
  if (typeof unwrapped === "string") {
    return /\b(FAIL|FAILED|REJECTED|BLOCKED)\b/i.test(unwrapped)
      ? [unwrapped]
      : [];
  }
  const evidence = object(unwrapped);
...
export const StillDesignContractRuntime = {
  ...
  outputBase64: stillOutputBase64,
