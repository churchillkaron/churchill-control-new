...
function authoritySummary(shot = {}, directionAuthority) {
  const authority = directionAuthority.authority(shot);
  const lockedFields = directionAuthority.lockedFields(shot);
  return {
...
function shotSummary(shot = {}, directionAuthority) {
  return {
    ...
    professional_direction: authoritySummary(shot, directionAuthority),
  };
}

function shotDetail(shot = {}, directionAuthority) {
  return {
    ...shotSummary(shot, directionAuthority),
...
export async function execute({ context, payload = {} }) {
  const [
    { ShotRuntime },
    { resolveOperatorCreativeProject },
    ...
    import("@/lib/creative/studio/OperatorCreativeProjectReferenceRuntime"),
    import("@/lib/creative/continuity/runtime/CreativeProfessionalDirectionAuthorityRuntime"),
    import("@/lib/creative/studio/runtime/CreativeChatShotReferenceRuntime"),
  ]);
  ...
  const returnedShots = selected
    ? [shotDetail(selected, CreativeProfessionalDirectionAuthorityRuntime)]
    : shots.slice(0, boundedLimit(payload.limit)).map((shot) =>
        shotSummary(shot, CreativeProfessionalDirectionAuthorityRuntime),
      );
