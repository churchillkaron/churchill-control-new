...
export const manifest = defineCapability({
  ...
  description:
    "Create a read-only, server-verified multi-shot directing plan before any mutation. Resolves exact shot sets, natural preservation clauses such as 'shots 4-7 except shot 5', explicit excluded shot IDs/references, snapshots revision numbers for both editable and preserved shots, surfaces Pro Studio lock conflicts, and returns a deterministic plan fingerprint required by confirmed execution. Never revises shots, generates media, spends wallet balance, or publishes.",
  permissions: [REQUIRED_PERMISSION],
  events: [],
  tags: ["creative", "video", "chat", "multi-shot", "plan", "direction", "governance", "preserved-shots"],
  transactional: false,
  aiEnabled: true,
  ...
  outputSchema: {
    ...
    required: ["status", "creative_project_id", "plan_fingerprint", "shot_count", "shots", "preserved_shot_count", "preserved_shots", "professional_lock_conflicts"],
  },
});
