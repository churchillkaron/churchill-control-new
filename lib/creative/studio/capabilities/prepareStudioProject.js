...
export const manifest = defineCapability({
  ...
  tags: ["creative", "studio", "brief", "project", "draft", "operator-reference", "video", "film"],
  operatorAliases: [
    "start a video project", "create a video project", "prepare a video project",
    "start a film project", "create a film project", "prepare the video project",
    "set up the video project", "turn this brief into a video project",
  ],
  operatorExamples: [
    "Start a video project from the creative direction we just agreed on.",
    "Prepare this investor film as a Studio project without generating media yet.",
  ],
  transactional: true,
  aiEnabled: true,
...
export async function execute({ context, payload = {} }) {
  ...
  const mission = await CreativeMissionRuntime.create({
    ...
    metadata: {
      ...
      research_grounding_version: "DYNAMIC_MISSION_GROUNDING_V1",
      dynamic_grounding_required: true,
    },
  });
