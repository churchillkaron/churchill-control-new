  ...
  inputSchema: {
    ...
    properties: {
      ...
      anchor_shot_id: {
        type: "string",
        description:
          "Optional server-verified active shot. If it is not one of the restored shots, Avantiqo anchors to the final restored shot instead.",
      },
    },
...
export async function execute({ context, payload = {} }) {
  ...
  const checkpointShotIds = new Set(
    (Array.isArray(restored.checkpoint_shot_ids)
      ? restored.checkpoint_shot_ids
      : [])
      .map((id) => text(id, 180))
      .filter(Boolean),
  );
  if (!checkpointShotIds.size) {
    const error = new Error("CREATIVE_DIRECTION_CHECKPOINT_SHOT_SET_MISSING");
    error.status = 500;
    throw error;
  }
  ...
  const restoredShots = projectShots
    .filter((shot) => checkpointShotIds.has(text(shot.id, 180)))
    .map(verifiedShot);
  if (restoredShots.length !== checkpointShotIds.size) {
    const error = new Error("CREATIVE_DIRECTION_CHECKPOINT_REREAD_INCOMPLETE");
    error.status = 500;
    throw error;
  }

  const requestedAnchorId = text(payload.anchor_shot_id, 180);
  const verifiedAnchor =
    restoredShots.find((shot) => shot.shot_id === requestedAnchorId) ||
    restoredShots[restoredShots.length - 1] ||
    null;
  ...
  return {
    ...
    creative_project_id: resolved.project.id,
    checkpoint_id: checkpointId,
    restored_shot_count: restoredShots.length,
    restored_shots: restoredShots,
    ...
    verification: {
      source: "CANONICAL_CHECKPOINT_SHOT_SET_REREAD",
