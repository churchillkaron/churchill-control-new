...
export const manifest = defineCapability({
  ...
  description:
    "Execute one previously planned and explicitly confirmed multi-shot directing change. The submitted plan_fingerprint must match the current canonical project, exact shot set, revision numbers, instruction and scope; stale plans fail closed before revision. Pro Studio locks are preflighted before execution. Each revised shot is canonically re-read afterward. Never generates media, publishes, or authorizes production.",
  permissions: [REQUIRED_PERMISSION],
  events: ["creative.studio.shot_set.revised"],
  tags: ["creative", "video", "chat", "multi-shot", "revision", "confirmation", "stale-plan-guard"],
  transactional: true,
  aiEnabled: true,
  ...
  outputSchema: {
    ...
    properties: {
      ...
      revised_shots: { type: "array", items: { type: "object", additionalProperties: true } },
    ...
    required: ["success", "creative_project_id", "plan_fingerprint", "revised_shot_count", "revised_shots", "professional_locks_preserved", "media_generation_executed", "publish_authorized"],
  },
});
...
export async function execute({ context, payload = {} }) {
  ...
  for (const shot of plan.shots) {
    ...
    revised.push({ shot_id: shot.id, result });
  }

  const canonical = [];
  for (const row of revised) {
    const shot = await ShotRuntime.get(row.shot_id);
    if (!shot) {
      const error = new Error("CREATIVE_CHAT_MULTI_REVISION_VERIFY_SHOT_NOT_FOUND");
      error.status = 500;
      throw error;
    }
    canonical.push(verifiedShot(shot));
  }

  const last = canonical[canonical.length - 1] || null;
  return {
    success: true,
    contract: "AVANTIQO_CHAT_MULTI_SHOT_REVISION_V1",
    ...
    verification: {
      source: "CANONICAL_MULTI_SHOT_REREAD",
