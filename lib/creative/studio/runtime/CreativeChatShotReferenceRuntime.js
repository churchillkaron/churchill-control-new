...
function sceneShotNumber(value) {
  const source = normalized(value);
  const match = source.match(
    /\bscene\s*#?\s*(\d{1,4})\s*(?:[,.:/-]?\s*)shot\s*#?\s*(\d{1,4})\b/i,
  );
  return match
    ? { scene_number: Number(match[1]), shot_number: Number(match[2]) }
    : null;
}
...
function resolveRelative(shots, reference, anchorShotId) {
  ...
  const isCurrent = CURRENT_REFERENCES.has(value);
  const isPrevious = PREVIOUS_REFERENCES.has(value);
  const isNext = NEXT_REFERENCES.has(value);
  ...
  if (isCurrent) return shots[anchorIndex];
  if (isPrevious) {
    if (anchorIndex === 0) {
      throw candidateError(
...
export async function resolveCreativeChatShotReference({
  ...
  const sceneShot = sceneShotNumber(reference);
  if (sceneShot) {
    const matches = shots
      .map((shot, index) => ({ shot, index }))
      .filter(({ shot }) =>
        Number(shot.scene_number) === sceneShot.scene_number &&
        Number(shot.shot_number) === sceneShot.shot_number,
      );
    if (matches.length === 1) {
      const match = matches[0];
      return {
        contract: CREATIVE_CHAT_SHOT_REFERENCE_CONTRACT,
        resolution: "SCENE_SHOT_NUMBER",
        reference,
        anchor_shot_id: text(anchor_shot_id, 180) || null,
        shot: match.shot,
    ...
    if (matches.length > 1) {
      throw candidateError(
        "CREATIVE_CHAT_SHOT_REFERENCE_SCENE_SHOT_AMBIGUOUS",
        reference,
        matches.map(({ shot, index }) => shotSummary(shot, index)),
      );
    }
    throw candidateError(
      "CREATIVE_CHAT_SHOT_REFERENCE_SCENE_SHOT_NOT_FOUND",
      reference,
      shots
        .map((shot, index) => ({ shot, index }))
        .filter(({ shot }) => Number(shot.scene_number) === sceneShot.scene_number)
        .slice(0, 8)
        .map(({ shot, index }) => shotSummary(shot, index)),
      {
        scene_number: sceneShot.scene_number,
        shot_number: sceneShot.shot_number,
      },
    );
  }
  ...
  if (numeric !== null) {
    ...
    if (byShotNumber.length > 1) {
      ...
      throw candidateError(
        ...
        {
          resolution:
            "Use scene N shot M, the shot title, scene context, or inspect the candidate list before revising.",
        },
      );
    }
  ...
  if (!candidates.length) {
    throw candidateError(
      ...
      {
        resolution:
          "Inspect direction to see the canonical shot list, then refer to scene N shot M, a shot number, title, or exact visual description.",
      },
    );
  }
