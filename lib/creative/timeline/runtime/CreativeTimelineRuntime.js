...
export const CreativeTimelineRuntime = {
  async compose({
    ...
    for (let index = 0; index < requested.length; index += 1) {
      ...
      let moment = requirementList.length
        ? candidate(moments, used, minimumScore, index)
        : candidate(moments, used, minimumScore);
      if (!moment && requirementList.length && allowFallback) {
        moment = candidate(moments, used, minimumScore);
      }
      if (!moment) {
