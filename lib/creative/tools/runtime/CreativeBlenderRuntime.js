...
import {
  CreativeToolSnapshotRuntime,
} from "@/lib/creative/tools/runtime/CreativeToolSnapshotRuntime";
import {
  CreativeBlenderSequencePackagingRuntime,
} from "@/lib/creative/tools/runtime/CreativeBlenderSequencePackagingRuntime";
...
export function normalizeCreativeBlenderScene(scene = {}) {
  ...
  const normalizedObjects = objects.map((item, index) => {
    ...
    return {
      ...
      keyframes: Array.isArray(item?.keyframes)
        ? item.keyframes.slice(0, 40).map((keyframe) => ({
        ...
        : [],
      material_keyframes: Array.isArray(item?.material_keyframes)
        ? item.material_keyframes.slice(0, 40).map((keyframe) => ({
            frame: integer(keyframe?.frame, 1, 1, 10000),
            metallic: keyframe?.metallic === undefined ? null : Math.max(0, Math.min(1, finite(keyframe.metallic, 0))),
            roughness: keyframe?.roughness === undefined ? null : Math.max(0, Math.min(1, finite(keyframe.roughness, 0.35))),
            emission: keyframe?.emission === undefined ? null : Math.max(0, Math.min(20, finite(keyframe.emission, 0))),
            color: keyframe?.color ? color(keyframe.color) : null,
          }))
        : [],
    };
  });
...
export function createCreativeBlenderScriptSource(encodedScene, outputPath) {
  return `
import base64
import json
import math
...
if selected_engine is None:
    raise RuntimeError('CREATIVE_BLENDER_RENDER_ENGINE_UNAVAILABLE')
...
def material_for(item):
    ...
    if alpha < 0.999 and hasattr(mat,'surface_render_method'):
        try: mat.surface_render_method='DITHERED'
        except Exception: pass
    for keyframe in item.get('material_keyframes', []):
        scene.frame_set(keyframe['frame'])
        if keyframe.get('color') is not None:
            bsdf.inputs['Base Color'].default_value = keyframe['color']
            bsdf.inputs['Base Color'].keyframe_insert(data_path='default_value')
        if keyframe.get('metallic') is not None:
            bsdf.inputs['Metallic'].default_value = keyframe['metallic']
            bsdf.inputs['Metallic'].keyframe_insert(data_path='default_value')
        if keyframe.get('roughness') is not None:
            bsdf.inputs['Roughness'].default_value = keyframe['roughness']
            bsdf.inputs['Roughness'].keyframe_insert(data_path='default_value')
        if keyframe.get('emission') is not None:
            if 'Emission Strength' in bsdf.inputs:
                bsdf.inputs['Emission Strength'].default_value = keyframe['emission']
                bsdf.inputs['Emission Strength'].keyframe_insert(data_path='default_value')
    scene.frame_set(1)
    return mat
...
def add_object(item):
    ...
    if kind == 'TEXT':
        ...
        bpy.ops.mesh.primitive_torus_add(major_radius=1.0, minor_radius=0.035, major_segments=64, minor_segments=16)
        obj = bpy.context.object
    else:
...
export async function renderCreativeBlenderScene({
  ...
  const ensured = await CreativeToolSnapshotRuntime.ensure({ project, tool_id: TOOL_ID });
  const snapshot = ensured.snapshot_id;
  ...
  try {
    await CreativeSandboxRuntime.writeText({
      sandbox,
      path: scriptPath,
      content: createCreativeBlenderScriptSource(encodedScene, outputPath),
    });
    ...
    let packaging = null;
    if (normalized.frames > 1) {
      packaging = await CreativeBlenderSequencePackagingRuntime.package({
        sandbox,
        frame_pattern: `${base}/frames/frame_%04d.png`,
        fps: normalized.fps,
