...
const CONTRACT = "CREATIVE_OPENCV_CAMERA_TRACK_RUNTIME_V3";
...
function neutralResult(reason) {
  return {
    ...
    result: {
      operation: "CAMERA_TRACK",
      degraded: true,
      degradation_reason: text(reason) || "CAMERA_TRACK_UNAVAILABLE",
      source: {},
      samples: [{
        frame: 0,
        time_seconds: 0,
...
export async function executeCreativeOpenCVCameraTrack({
  ...
  if (!snapshot) return neutralResult("CREATIVE_OPENCV_SNAPSHOT_REQUIRED");
  ...
  try {
    ...
    const base = `/tmp/avantiqo-opencv-track-v3-${identity}`;
    ...
    sandbox = await CreativeSandboxRuntime.fromSnapshot({
    ...
    if (!Array.isArray(result.samples) || !result.samples.length) {
      return neutralResult("CREATIVE_OPENCV_NO_TRACK_SAMPLES");
    }
    return {
      contract: CONTRACT,
      tool_id: TOOL_ID,
    ...
    if (
      message.includes("CREATIVE_SANDBOX_COMMAND_API_FAILED:410") ||
      message.includes("sandbox_stopped") ||
      message.includes("CREATIVE_OPENCV_CAMERA_TRACK_FAILED")
    ) {
      return neutralResult(message);
    }
    throw error;
  } finally {
    if (sandbox) await CreativeSandboxRuntime.stop(sandbox);
  }
}
