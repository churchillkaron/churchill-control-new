...
image_operation = operation in ("DEPTH_NORMALS", "PRACTICAL_LIGHTS", "IMAGE_SEGMENTATION")
static_image = cv2.imread(source_path, cv2.IMREAD_UNCHANGED) if image_operation else None
if image_operation:
    if static_image is None:
        raise RuntimeError("CREATIVE_OPENCV_IMAGE_SOURCE_OPEN_FAILED")
    height, width = static_image.shape[:2]
    fps = 0.0
    frame_count = 1
    duration = None
    cap = None
...
def depth_normals():
    depth = static_image
    if depth is None:
        raise RuntimeError("CREATIVE_OPENCV_DEPTH_IMAGE_REQUIRED")
    if depth.ndim == 3:
        depth = cv2.cvtColor(depth, cv2.COLOR_BGR2GRAY)
    depth = depth.astype(np.float32)
    minimum = float(np.percentile(depth, 1))
    maximum = float(np.percentile(depth, 99))
    scale = max(1e-6, maximum - minimum)
    normalized = np.clip((depth - minimum) / scale, 0.0, 1.0)
    smoothed = cv2.GaussianBlur(normalized, (0, 0), 1.2)
    dx = cv2.Sobel(smoothed, cv2.CV_32F, 1, 0, ksize=3)
    dy = cv2.Sobel(smoothed, cv2.CV_32F, 0, 1, ksize=3)
    strength = float(cfg.get("normal_strength", 2.0))
    nx = -dx * strength
    ny = -dy * strength
    nz = np.ones_like(nx)
    ...
    cv2.imwrite(mask_path, normal_bgr)
    return {
        "depth_min_p01": minimum,
        "depth_max_p99": maximum,
        "normal_strength": strength,
        "mean_normal_z": float(np.mean(nz)),
        "edge_energy": float(np.mean(np.sqrt(dx * dx + dy * dy))),
...
def practical_lights():
    image = static_image
    if image is None:
        raise RuntimeError("CREATIVE_OPENCV_LIGHT_IMAGE_REQUIRED")
    if image.ndim == 2:
        luminance = image.astype(np.float32)
    else:
        bgr = image[:, :, :3]
        luminance = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY).astype(np.float32)
    p50 = float(np.percentile(luminance, 50))
    p95 = float(np.percentile(luminance, 95))
    p99 = float(np.percentile(luminance, 99))
    threshold = max(180.0, p99)
    ...
    for label in range(1, count):
        ...
        if area < max(4, int(frame_area * 0.00001)):
            continue
        mask = labels == label
    ...
    sources.sort(key=lambda item: (item["max_luminance"], item["area_pixels"]), reverse=True)
    return {
        "ambient_luminance_p50": p50,
        "highlight_luminance_p95": p95,
        "highlight_luminance_p99": p99,
        "dynamic_range_proxy": float(max(0.0, p99 - p50)),
        "threshold": threshold,
        "sources": sources[:32],
        "source_count": min(32, len(sources)),
...
def image_segmentation():
    frame = static_image
    if frame is None:
        raise RuntimeError("CREATIVE_OPENCV_IMAGE_SEGMENTATION_SOURCE_REQUIRED")
    bbox = cfg.get("bbox")
    ...
    cv2.grabCut(frame[:, :, :3] if frame.ndim == 3 else cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR), mask, (x, y, w, h), bgd, fgd, 7, cv2.GC_INIT_WITH_RECT)
...
if operation == "CAMERA_TRACK":
    ...
    payload = image_segmentation()
elif operation == "DEPTH_NORMALS":
    payload = depth_normals()
elif operation == "PRACTICAL_LIGHTS":
    payload = practical_lights()
else:
...
export async function executeCreativeOpenCV({
  ...
  if (!["CAMERA_TRACK", "OBJECT_TRACK", "OPTICAL_FLOW", "SEGMENTATION", "IMAGE_SEGMENTATION", "MOTION_ANALYSE", "DEPTH_NORMALS", "PRACTICAL_LIGHTS"].includes(mode)) {
  ...
  const sourcePath = `${base}/${["DEPTH_NORMALS", "PRACTICAL_LIGHTS", "IMAGE_SEGMENTATION"].includes(mode) ? "source.png" : "source.mp4"}`;
  ...
  try {
    ...
    let mask = null;
    if (["SEGMENTATION", "IMAGE_SEGMENTATION", "DEPTH_NORMALS"].includes(mode)) {
      ...
      mask = {
        ...
        buffer,
        bytes: buffer.length,
        artifact_kind: mode === "DEPTH_NORMALS" ? "SURFACE_NORMAL_MAP" : mode === "IMAGE_SEGMENTATION" ? "IMAGE_SEGMENTATION_MASK" : "SEGMENTATION_MASK",
      };
    }
