...
image_operation = operation in ("DEPTH_NORMALS", "PRACTICAL_LIGHTS")
...
export async function executeCreativeOpenCV({
  ...
  if (!["CAMERA_TRACK", "OBJECT_TRACK", "OPTICAL_FLOW", "SEGMENTATION", "MOTION_ANALYSE", "DEPTH_NORMALS", "PRACTICAL_LIGHTS"].includes(mode)) {
  ...
  const sourcePath = `${base}/${["DEPTH_NORMALS", "PRACTICAL_LIGHTS"].includes(mode) ? "source.png" : "source.mp4"}`;
  ...
  try {
    ...
    let mask = null;
    if (mode === "SEGMENTATION" || mode === "DEPTH_NORMALS") {
      ...
      mask = {
        ...
        buffer,
        bytes: buffer.length,
        artifact_kind: mode === "DEPTH_NORMALS" ? "SURFACE_NORMAL_MAP" : "SEGMENTATION_MASK",
      };
    }
