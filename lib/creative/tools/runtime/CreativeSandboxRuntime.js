...
const CONTRACT = "CREATIVE_SANDBOX_RUNTIME_V2";
...
async function outputText(value) {
  if (typeof value === "function") {
    return text(await value());
  }
  return text(value);
}
...
async function normalizeResult(result) {
  return {
    ...
    stdout: await outputText(result?.stdout),
    stderr: await outputText(result?.stderr),
  };
}
