...
function decodeOidcClaims(token) {
  ...
  if (parts.length !== 3) throw new Error("CREATIVE_SANDBOX_OIDC_TOKEN_INVALID");
  try {
    return JSON.parse(Buffer.from(parts[1], "base64url").toString("utf8"));
  } catch {
    throw new Error("CREATIVE_SANDBOX_OIDC_TOKEN_INVALID");
  }
}

function activeSessionId(sandbox) {
  const sessionId = text(sandbox?.currentSession?.()?.sessionId);
  if (!sessionId) throw new Error("CREATIVE_SANDBOX_SESSION_ID_REQUIRED");
  return sessionId;
}
...
async function directCommand({
  ...
  const teamId = text(claims.owner_id);
  if (!teamId) throw new Error("CREATIVE_SANDBOX_OIDC_TEAM_REQUIRED");

  const sessionId = activeSessionId(sandbox);
  const url = new URL(`${VERCEL_API_BASE}/v2/sandboxes/sessions/${encodeURIComponent(sessionId)}/cmd`);
  ...
  if (!response.ok) {
    const error = new Error(`CREATIVE_SANDBOX_COMMAND_API_FAILED:${response.status}`);
    error.details = { status: response.status, body: raw.slice(0, 4000) };
    throw error;
  }
  ...
  for (const line of raw.split(/\r?\n/).filter(Boolean)) {
    ...
    if (chunk?.stream === "stdout") stdout += String(chunk.data ?? "");
    if (chunk?.stream === "stderr") stderr += String(chunk.data ?? "");
    if (chunk?.stream === "error") {
      const message = text(chunk?.data?.message || chunk?.data || "Sandbox stream failed");
      throw new Error(`CREATIVE_SANDBOX_STREAM_FAILED:${message}`);
    }
    if (chunk?.command && Number.isFinite(Number(chunk.command.exitCode))) {
      exitCode = Number(chunk.command.exitCode);
    }
  }
