...
const CONTRACT = "CREATIVE_SPATIAL_PRODUCT_TWIN_RUNTIME_V7";
...
const glass=(accent)=>({background:'linear-gradient(135deg,rgba(11,17,23,.20),rgba(255,255,255,.05))',border:'1px solid '+accent,boxShadow:'0 22px 80px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.17)',backdropFilter:'blur(20px) saturate(150%)',WebkitBackdropFilter:'blur(20px) saturate(150%)'});
...
export async function renderCreativeSpatialProductTwin({ organization_id, project, scenes, width = 1920, height = 1080, fps = 24 } = {}) {
  ...
  const identity = jobId({ organization_id, project_id: project?.id, normalized, safeWidth, safeHeight, safeFps, snapshot });
  const base = `/tmp/avantiqo-spatial-product-twin-v7-${identity}`;
  ...
  try {
    ...
    return {
      contract: CONTRACT,
      tool_id: TOOL_ID,
      browser_tool_id: BROWSER_TOOL_ID,
      mime_type: "video/mp4",
      codec: "h264",
      buffer,
      bytes: buffer.length,
