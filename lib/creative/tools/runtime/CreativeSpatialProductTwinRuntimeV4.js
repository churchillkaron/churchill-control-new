...
const CONTRACT = "CREATIVE_SPATIAL_PRODUCT_TWIN_RUNTIME_V6";
...
const browserRelativePath = (project) => text(project?.metadata?.creative_tool_snapshots?.[TOOL_ID]?.browser_binary);
...
const glass=(accent)=>({background:'linear-gradient(135deg,rgba(11,17,23,.22),rgba(255,255,255,.045))',border:'1px solid '+accent,boxShadow:'0 22px 80px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.16)',backdropFilter:'blur(20px) saturate(150%)',WebkitBackdropFilter:'blur(20px) saturate(150%)'});
const Objects=({scene,frame})=>{const t=frame/fps,s=nearest(tracks[scene.id]||[],t),frames=Math.round(scene.duration_seconds*fps);const enter=interpolate(frame,[0,Math.round(fps*.62)],[0,1],clamp),exit=interpolate(frame,[Math.max(0,frames-Math.round(fps*.38)),frames],[1,0],clamp),vis=Math.min(enter,exit);const left=scene.side==='left',solid=scene.accent==='gold'?'rgba(230,198,143,.94)':'rgba(195,242,255,.94)',edge=scene.accent==='gold'?'rgba(230,198,143,.24)':'rgba(195,242,255,.24)';const ax=Math.max(-24,Math.min(24,-s.x*.20)),ay=Math.max(-15,Math.min(15,-s.y*.15)),ar=Math.max(-1.3,Math.min(1.3,-s.rotation*.14));const base='perspective(1500px) translate3d('+ax+'px,'+(ay+Math.sin(t)*4)+'px,0) rotateY('+((left?7:-7)+ar)+'deg)';const style=(extra={})=>({...glass(edge),position:'absolute',boxSizing:'border-box',opacity:vis,transform:base,...extra});const titleX=left?88:width-520;const positions=left?[[100,390],[360,485],[115,580],[390,675]]:[[width-430,390],[width-690,485],[width-405,580],[width-705,675]];return <><div style={style({left:titleX,top:175,width:430,padding:'22px 26px',borderRadius:25})}><div style={{fontFamily:'Arial,Helvetica,sans-serif',fontSize:13,fontWeight:700,letterSpacing:4.2,color:solid,marginBottom:8}}>{scene.kicker}</div><div style={{fontFamily:'Arial,Helvetica,sans-serif',fontSize:42,fontWeight:650,letterSpacing:-1.2,color:'#f7fbfd',lineHeight:1.02}}>{scene.domain}</div></div>{scene.capabilities.map((cap,i)=>{const p=positions[i]||positions[0],iv=interpolate(frame,[Math.round(fps*(.45+i*.13)),Math.round(fps*(.95+i*.13))],[0,1],clamp);return <div key={cap} style={style({left:p[0],top:p[1]+Math.sin(t*1.05+i)*5,width:245,padding:'14px 17px',borderRadius:18,opacity:vis*iv,transform:base+' translateY('+(10*(1-iv))+'px)'})}><div style={{fontFamily:'Arial,Helvetica,sans-serif',fontSize:20,fontWeight:520,color:'#eef5f7',whiteSpace:'nowrap'}}>{cap}</div></div>;})}{scene.ai_signal?<div style={style({left:left?300:width-720,top:805,width:520,padding:'16px 19px',borderRadius:20,display:'flex',alignItems:'center',gap:12})}><div style={{width:9,height:9,borderRadius:99,background:solid,boxShadow:'0 0 22px '+solid}}/><div style={{fontFamily:'Arial,Helvetica,sans-serif',fontSize:18,color:'#e2eaee',lineHeight:1.25}}>{scene.ai_signal}</div></div>:null}<svg width={width} height={height} style={{position:'absolute',inset:0,opacity:.42*vis,pointerEvents:'none'}}><path d={left?'M 440 320 C 610 390 690 550 800 730':'M 1480 320 C 1310 390 1230 550 1120 730'} fill="none" stroke={solid} strokeOpacity=".20" strokeWidth="1.1"/>{[0,1,2,3,4].map(i=><circle key={i} cx={(left?500:1420)+(left?1:-1)*i*85+Math.sin((frame+i*17)/30)*8} cy={335+i*90+Math.cos((frame+i*11)/31)*7} r={3+(i%2)} fill={solid} opacity={.26+i*.08}/>)}</svg></>;};
const Scene=({scene,index})=>{const frame=useCurrentFrame(),frames=Math.max(1,Math.round(scene.duration_seconds*fps));const fi=interpolate(frame,[0,Math.round(fps*.2)],[0,1],clamp),fo=interpolate(frame,[Math.max(0,frames-Math.round(fps*.22)),frames],[1,0],clamp);return <AbsoluteFill style={{backgroundColor:'#020406',opacity:Math.min(fi,fo)}}><OffthreadVideo src={staticFile('scene-'+index+'.mp4')} startFrom={Math.round(scene.source_in_seconds*fps)} volume={0} style={{width:'100%',height:'100%',objectFit:'cover'}}/><AbsoluteFill style={{background:'radial-gradient(circle at 50% 46%,transparent 38%,rgba(0,0,0,.035) 70%,rgba(0,0,0,.22) 100%)'}}/><Objects scene={scene} frame={frame}/></AbsoluteFill>;};
...
async function ensureBrowserRuntime({ sandbox, browserExecutable }) {
  const command = `set -e; BROWSER=${JSON.stringify(browserExecutable)}; test -x "$BROWSER"; if ldd "$BROWSER" 2>/dev/null | grep -q 'not found'; then apt-get update >/dev/null; DEBIAN_FRONTEND=noninteractive apt-get install -y libnspr4 libnss3 libatk1.0-0 libatk-bridge2.0-0 libcups2 libdrm2 libdbus-1-3 libxkbcommon0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 libgbm1 libasound2 libatspi2.0-0 libx11-xcb1 libxshmfence1 fonts-liberation >/dev/null; fi; MISSING=$(ldd "$BROWSER" 2>/dev/null | grep 'not found' || true); test -z "$MISSING"; "$BROWSER" --version`;
  return CreativeSandboxRuntime.run({
    sandbox,
    cmd: "bash",
    args: ["-lc", command],
    sudo: true,
    timeout_ms: 240000,
    error_prefix: "CREATIVE_REMOTION_BROWSER_RUNTIME_REPAIR_FAILED",
  });
}
...
export async function renderCreativeSpatialProductTwin({ organization_id, project, scenes, width = 1920, height = 1080, fps = 24 } = {}) {
  ...
  const browserPath = browserRelativePath(project);
  if (!browserPath) throw new Error("CREATIVE_REMOTION_BROWSER_PATH_REQUIRED");
  const browserExecutable = `/tmp/avantiqo-remotion/${browserPath}`;
  ...
  const identity = jobId({ organization_id, project_id: project?.id, normalized, safeWidth, safeHeight, safeFps, browserPath });
  const base = `/tmp/avantiqo-spatial-product-twin-v6-${identity}`;
  ...
  for (const scene of normalized) {
    ...
    const samples = trackSamples(tracking);
    if (!samples.length) throw new Error(`SPATIAL_PRODUCT_TWIN_TRACKING_REQUIRED:${scene.id}`);
    buffers.push(buffer);
    tracks[scene.id] = samples;
  }
  ...
  try {
    const browserRuntime = await ensureBrowserRuntime({ sandbox, browserExecutable });
    ...
    await CreativeSandboxRuntime.writeText({
      sandbox,
      path: scriptPath,
      content: renderScript({ entryPath, publicDir, outputPath, browserExecutable }),
    });
    ...
    return {
      ...
      design_policy: "LIVE_ENVIRONMENT_PRIMARY_FLOATING_GLASS_OBJECTS_ONLY",
      browser_baked: true,
      browser_runtime_verified: Boolean(browserRuntime?.stdout || browserRuntime?.stderr),
      metadata,
    };
  } finally {
