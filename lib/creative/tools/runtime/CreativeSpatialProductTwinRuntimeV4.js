...
const CONTRACT = "CREATIVE_SPATIAL_PRODUCT_TWIN_RUNTIME_V4";
...
export async function renderCreativeSpatialProductTwin({ organization_id, project, scenes, width = 1920, height = 1080, fps = 24 } = {}) {
  ...
  const base = `/tmp/avantiqo-spatial-product-twin-v4-${identity}`;
  ...
  try {
    ...
    await CreativeSandboxRuntime.writeText({
      sandbox,
      path: scriptPath,
      content: renderScript({ entryPath, publicDir, outputPath }),
    });
