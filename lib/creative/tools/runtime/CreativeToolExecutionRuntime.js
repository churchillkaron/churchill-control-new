...
} from "@/lib/creative/tools/runtime/CreativeSpatialProductTwinRuntimeV2";

const CONTRACT = "CREATIVE_TOOL_EXECUTION_RUNTIME_V4";
...
async function executeSandboxCapability({
  ...
  if (selected_tool_id === "chromium-playwright") {
    ...
    if (capability === CREATIVE_TOOL_CAPABILITIES.BROWSER_RECORD) {
      return CreativeBrowserCaptureRuntime.record({
        project,
        ...input,
      });
    }
  ...
  if (selected_tool_id === "remotion") {
    if (capability === CREATIVE_TOOL_CAPABILITIES.SPATIAL_PRODUCT_TWIN) {
      return CreativeSpatialProductTwinRuntime.render({
        organization_id,
        project,
        ...input,
      });
  ...
  if (selected_tool_id === "blender") {
    if (capability === CREATIVE_TOOL_CAPABILITIES.THREE_D_RENDER) {
      return CreativeBlenderRuntime.render({
        project,
  ...
  if (selected_tool_id === "opencv") {
    const openCvOperation = {
      [CREATIVE_TOOL_CAPABILITIES.CAMERA_TRACK]: "CAMERA_TRACK",
    ...
    if (openCvOperation) {
      return CreativeOpenCVRuntime.execute({
        organization_id,
        project,
  ...
  throw new Error(
    `CREATIVE_SANDBOX_CAPABILITY_EXECUTOR_MISSING:${selected_tool_id}:${capability}`,
  );
}
