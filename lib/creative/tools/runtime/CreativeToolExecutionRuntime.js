...
import {
  CreativeBlenderLocalNodeRuntime,
} from "@/lib/creative/tools/runtime/CreativeBlenderLocalNodeRuntime";
import {
  CreativeCinematicMotionDesignRuntime,
} from "@/lib/creative/motion-graphics/runtime/CreativeCinematicMotionDesignRuntime";
import {
...
async function executeSandboxCapability({
  ...
  if (selected_tool_id === "blender" && capability === CREATIVE_TOOL_CAPABILITIES.CINEMATIC_MOTION_DESIGN) {
    const plan = input?.plan?.contract === CreativeCinematicMotionDesignRuntime.contract
      ? input.plan
      : CreativeCinematicMotionDesignRuntime.plan(input || {});
    return CreativeCinematicMotionDesignRenderRuntime.render({ project, plan });
  }
...
export async function executeCreativeCapability({
  ...
  if (selected.runtime === CREATIVE_TOOL_RUNTIME.LOCAL) {
    if (selected.tool_id === "blender" && requestedCapability === CREATIVE_TOOL_CAPABILITIES.THREE_D_RENDER) {
      const output = await CreativeBlenderLocalNodeRuntime.render({
        organization_id,
        creative_project_id,
