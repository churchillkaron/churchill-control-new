...
function threatSource(nodes=[],shot={}){
  ...
  if(explicit) return nodes.find(n=>text(n.id)===explicit)||null;
  ...
  if(imageStudio) return imageStudio;
  return nodes.find(n=>{
...
export async function ensurePremiumLayerTasks({organization_id,creative_project_id}={}){
  ...
  for(const pass of passes){
    ...
    const source=threatSource(assets,shot||{});
    if(!source?.url){blocked.push({pass_node_id:pass.id,shot_id:id,reason:"THREAT_HERO_SOURCE_ASSET_REQUIRED"});continue;}
    const continuityGroup=shot?.requirements?.generation_strategy?.shared_state_group_id||null;
