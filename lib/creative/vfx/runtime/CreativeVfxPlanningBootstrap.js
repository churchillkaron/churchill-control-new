import {
  ProductionGraphRuntime,
} from "@/lib/creative/production-graph/runtime/ProductionGraphRuntime";
import {
  CreativeVfxAuthoringRuntime,
} from "@/lib/creative/vfx/runtime/CreativeVfxAuthoringRuntime";

const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.vfx-planning-bootstrap.v1",
);

function object(value) {
  return value && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
}

function list(value) {
  return Array.isArray(value) ? value.filter(Boolean) : [];
}

function text(value) {
  return String(value ?? "").trim();
}

function enrichPlanningInput(input = {}) {
  const authored = CreativeVfxAuthoringRuntime.build({
    scenes: input.scenes,
    shots: input.shots,
    creative_plan: input.creative_plan,
  });
  return {
    ...input,
    scenes: authored.scenes,
    shots: authored.shots,
    creative_plan: authored.creative_plan,
    __vfx_authoring: authored.metadata,
  };
}

function injectGraphContracts(graph = {}, shots = [], authoringMetadata = {}) {
  const byId = new Map(list(shots).map((shot) => [text(shot.id), shot]));
  let injected = 0;
  const nodes = list(graph.nodes).map((node) => {
    if (text(node.type).toUpperCase() !== "SHOT") return node;
    const shot = byId.get(text(node.id));
    if (!shot?.vfx_contract) return node;
    injected += 1;
    return {
      ...node,
      requirements: {
        ...object(node.requirements),
        vfx: shot.vfx,
        vfx_contract: shot.vfx_contract,
      },
      metadata: {
        ...object(node.metadata),
        vfx_contract: shot.vfx_contract.contract,
        vfx_status: "READY",
        vfx_effect_count: shot.vfx_contract.effects?.length || 0,
        vfx_authored_before_materialization: true,
        vfx_execution_authorship_forbidden: true,
      },
    };
  });

  return {
    ...graph,
    nodes,
    metadata: {
      ...object(graph.metadata),
      ...object(authoringMetadata),
      vfx_injected_shot_count: injected,
      vfx_contracts_in_graph: true,
      vfx_execution_authorship_forbidden: true,
    },
  };
}

function install() {
  if (ProductionGraphRuntime[INSTALL_FLAG]) return;

  const previewWithoutVfx = ProductionGraphRuntime.preview.bind(
    ProductionGraphRuntime,
  );

  Object.defineProperty(ProductionGraphRuntime, INSTALL_FLAG, {
    value: true,
    enumerable: false,
    configurable: false,
  });

  ProductionGraphRuntime.preview = async function previewWithVfx(input = {}) {
    const enriched = enrichPlanningInput(input);
    const graph = await previewWithoutVfx(enriched);
    return injectGraphContracts(
      graph,
      enriched.shots,
      enriched.__vfx_authoring,
    );
  };

  ProductionGraphRuntime.plan = async function planWithVfx(input = {}) {
    const graph = await ProductionGraphRuntime.preview(input);
    return ProductionGraphRuntime.create(graph);
  };
}

install();

export const CreativeVfxPlanningBootstrap = Object.freeze({
  installed: true,
  contract: "AVANTIQO_VFX_PLANNING_BOOTSTRAP_V1",
  authoring_contract: CreativeVfxAuthoringRuntime.contract,
  execution_authorship_forbidden: true,
  provider_neutral: true,
});
