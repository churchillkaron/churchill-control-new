...
async function seal(review, source, evaluation) {
  if (source?.id) {
    const vfxAssetNodeId = source.output?.asset_node_id || null;
    if (vfxAssetNodeId) {
      await CreativeMultiPassArtifactRuntime.sealVfx({
        asset_node_id: vfxAssetNodeId,
        vfx_qc_seal_hash: evaluation.seal_hash,
        vfx_contract_hash: evaluation.vfx_contract_hash,
      });
    }
    await ProductionTaskRuntime.update(source.id, {
      metadata: {
