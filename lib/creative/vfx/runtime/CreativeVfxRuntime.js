...
function authoredIntegrationParameters(effect = {}, index = 0) {
  const p = object(effect.integration_parameters || effect.execution_parameters || effect.composite_parameters);
  return {
    contract: "AVANTIQO_VFX_INTEGRATION_NUMERIC_PROFILE_V1",
    seed: Math.round(clamp(p.seed ?? effect.seed, 1, 2147483646, 52000 + index * 1297)),
    depth_position_normalized: clamp(p.depth_position_normalized ?? effect.depth_position_normalized, 0, 1, 0.5),
    depth_foreground_rule: text(p.depth_foreground_rule || effect.depth_foreground_rule, 120) || "GREATER_THAN_EFFECT_DEPTH_OCCLUDES",
    occlusion_softness_pixels: clamp(p.occlusion_softness_pixels, 0, 128, 6),
    matte_erode_pixels: Math.round(clamp(p.matte_erode_pixels, 0, 32, 1)),
    matte_feather_pixels: clamp(p.matte_feather_pixels, 0, 64, 2.5),
    exposure_ev: clamp(p.exposure_ev, -6, 6, 0),
    saturation: clamp(p.saturation, 0, 3, 1),
    black_level: clamp(p.black_level, -0.25, 0.25, 0),
    white_level: clamp(p.white_level, 0.25, 4, 1),
    motion_blur_pixels: clamp(p.motion_blur_pixels, 0, 128, 3),
    motion_blur_angle_degrees: clamp(p.motion_blur_angle_degrees, -180, 180, 0),
    dof_blur_sigma: clamp(p.dof_blur_sigma, 0, 20, 0.8),
    grain_strength: clamp(p.grain_strength, 0, 0.25, 0.018),
    light_wrap_pixels: clamp(p.light_wrap_pixels, 0, 64, 4),
    light_wrap_strength: clamp(p.light_wrap_strength, 0, 2, 0.25),
    reflection_strength: clamp(p.reflection_strength, 0, 2, 0.18),
    contact_shadow_strength: clamp(p.contact_shadow_strength, 0, 2, 0.35),
    contact_shadow_blur_pixels: clamp(p.contact_shadow_blur_pixels, 0, 64, 8),
...
function normalizeEffect(effect = {}, src = {}, index = 0) {
  ...
  return {
    ...
    integration_parameters: authoredIntegrationParameters(effect, index),
...
function evaluateEffect(effect = {}) {
  ...
  if (text(effect.integration_parameters?.contract, 300) !== "AVANTIQO_VFX_INTEGRATION_NUMERIC_PROFILE_V1") {
    blockers.push(issue("VFX_INTEGRATION_NUMERIC_PROFILE_REQUIRED", "integration_parameters", "VFX requires authored numeric integration parameters before render execution."));
  }
