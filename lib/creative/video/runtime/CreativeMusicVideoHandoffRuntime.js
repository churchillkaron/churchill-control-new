...
export async function handoffMusicMasterToVideoProject({
  ...
  const resolvedMasterId = text(master_asset_id || currentMasterId);
  if (!resolvedMasterId) throw new Error("MUSIC_VIDEO_HANDOFF_CURRENT_MASTER_REQUIRED");
  const audioAuthorityKind = currentMasterId && resolvedMasterId === currentMasterId
    ? "CURRENT_MUSIC_MASTER"
    : "EXPLICIT_MUSIC_SOURCE";
  const master = await CreativeAssetsRuntime.get(resolvedMasterId);
  if (!master || text(master.organization_id) !== organizationId || assetProjectId(master) !== musicProjectId) throw new Error("MUSIC_VIDEO_HANDOFF_MASTER_SCOPE_MISMATCH");
  if (!isAudio(master)) throw new Error("MUSIC_VIDEO_HANDOFF_AUDIO_MASTER_REQUIRED");
