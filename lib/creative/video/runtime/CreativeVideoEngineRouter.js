const CONTRACT = "CREATIVE_VIDEO_ENGINE_ROUTE_V2";
...
function modelForImageMode(provider, hasSource) {
  ...
  if (provider === "google-veo") return "veo-3.1-generate-preview";
  if (provider === "veo") {
    return hasSource
...
export function resolveCreativeVideoEngine({ shot_bible = {} } = {}) {
  ...
  if (evidence.hard_precision_required) {
    ...
    const model = modelForImageMode(provider, hasSource);
    return {
      contract: CONTRACT,
      status: "ROUTED",
      decision: "DIRECT_GOOGLE_VEO_PRECISION",
      primary: {
        provider,
        model,
      },
      ...
      evidence: {
        ...evidence,
        precision_provider_required: true,
        nonprecision_fallback_allowed: false,
        fal_veo_precision_eligible: false,
      },
      fail_closed: true,
  ...
  if (duration !== null && duration > 10 && duration <= 15) {
    ...
    const allowed = ["seedance", "veo", "runway", "gemini"];
    return {
      contract: CONTRACT,
      status: "ROUTED",
  ...
  if (height !== null && height > 720) {
    const provider = "veo";
    const allowed = ["veo", "gemini", "seedance", "runway"];
    return {
      contract: CONTRACT,
      status: "ROUTED",
  ...
  const allowed = ["gemini", "veo", "seedance", "runway"];
  return {
    contract: CONTRACT,
    status: "ROUTED",
    decision: hasSource
