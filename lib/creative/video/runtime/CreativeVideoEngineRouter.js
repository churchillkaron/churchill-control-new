import { CreativeVideoProductionCertificationRuntime } from "./CreativeVideoProductionCertificationRuntime";

const CONTRACT = "CREATIVE_VIDEO_EXECUTION_ROUTE_V5";
...
function filmCraftRequirements(bible = {}) {
  const craft = object(bible.professional_filmcraft);
  const evidence = object(craft.evidence);
  const movement = object(craft.movement_choreography);
  const performance = object(craft.performance_control);
  const compoundMove = Array.isArray(movement.compound_move)
    ? movement.compound_move.filter(Boolean)
    : [];
  const physical = craft.applicability === "PHYSICAL_VIDEO";
  return {
    professional_filmcraft_required: physical,
    camera_motion_reference_required:
      evidence.camera_motion_reference_required === true ||
      movement.motion_reference_required === true,
    performance_reference_required:
      evidence.performance_reference_required === true ||
      performance.performance_reference_required === true,
    compound_camera_choreography_required: compoundMove.length > 1,
    lens_character_required:
      physical && Boolean(text(craft.lens_character?.projection)),
    focus_choreography_required:
      physical && Boolean(craft.focus_choreography),
    spatial_continuity_required:
      physical && Boolean(craft.spatial_continuity),
    sound_perspective_required:
      physical && Boolean(craft.sound_perspective),
    look_development_required:
      physical && Boolean(craft.look_development),
    story_motivated_optical_fx_required:
      Array.isArray(craft.optical_practical_fx?.effects) &&
      craft.optical_practical_fx.effects.length > 0,
  };
}

function routeEvidence(bible = {}, capability = null) {
  const precision = precisionRequirements(bible);
  const filmCraft = filmCraftRequirements(bible);
  ...
  return {
    ...
    hard_precision_required:
      Object.values(precision).some(Boolean) ||
      filmCraft.camera_motion_reference_required ||
      filmCraft.performance_reference_required ||
      filmCraft.compound_camera_choreography_required,
    ...precision,
    ...filmCraft,
  };
}
...
function serviceRuntimePolicy(evidence = {}) {
  const certification = CreativeVideoProductionCertificationRuntime.status(evidence.capability);
  return {
    selection_weights: {
      quality: 5,
      reliability: 4,
    ...
    video_requirements: {
      ...
      professional_filmcraft_required:
        evidence.professional_filmcraft_required === true,
      camera_motion_reference_required:
        evidence.camera_motion_reference_required === true,
      performance_reference_required:
        evidence.performance_reference_required === true,
      compound_camera_choreography_required:
        evidence.compound_camera_choreography_required === true,
      lens_character_required:
        evidence.lens_character_required === true,
      focus_choreography_required:
        evidence.focus_choreography_required === true,
      spatial_continuity_required:
        evidence.spatial_continuity_required === true,
      sound_perspective_required:
        evidence.sound_perspective_required === true,
      look_development_required:
        evidence.look_development_required === true,
      story_motivated_optical_fx_required:
        evidence.story_motivated_optical_fx_required === true,
    },
    provider_selection_boundary: "SERVICE_RUNTIME_ONLY",
    execution_scope: certification.execution_scope,
    benchmark_only: certification.benchmark_only,
    production_certified: certification.production_certified,
    certification,
    studio_preproduction_review: !certification.production_certified,
...
export function resolveCreativeVideoEngine({
  ...
  const certification = CreativeVideoProductionCertificationRuntime.status(executionCapability);
  if (!certification.implemented) {
    return {
      contract: CONTRACT,
      status: "BLOCKED",
      decision: "VIDEO_CAPABILITY_NOT_IMPLEMENTED",
      execution_capability: executionCapability,
      primary: null, challengers: [], provider_policy: null,
      evidence: { ...evidence, certification },
      fail_closed: true, paid_execution_authorized: false,
    };
  }
  ...
  return {
    contract: CONTRACT,
    status: "ROUTED",
    decision: "SERVICE_RUNTIME_AVANTIQO_OWNED_ONLY_CAPABILITY_SELECTION",
    execution_capability: executionCapability,
    primary: null,
    challengers: [],
    ...
    evidence: {
      ...
      service_runtime_owned_only_required: true,
      external_provider_role: "FORBIDDEN",
      capability_certification_remains_mandatory: true,
      studio_preproduction_review: true,
      production_certification_not_implied: certification.production_certified !== true,
      capability_certification: certification,
      production_execution_eligible: certification.production_certified === true,
      unsupported_required_control_must_block: true,
    },
    fail_closed: true,
