...
function activeAuthorization(task = {}) {
  ...
  if (authorization.contract !== APPROVAL_CONTRACT) return null;
  if (
...
export async function inspectCreativeVideoGenerationApproval({
  ...
  const preflight = await CreativeVideoGenerationPreflightRuntime.resolve({
    task_id,
    currency,
  });
  ...
  const dossierApproval = dossier
    ...
    : null;
  const authorization = activeAuthorization(task);
  ...
  const staleAuthorization = Boolean(authorization && !samePreflight);
  ...
  return {
    contract: "CREATIVE_VIDEO_GENERATION_APPROVAL_INSPECTION_V1",
    task,
    preflight,
    dossier,
    ...
    can_approve: samePreflight || blockingReasons.length === 0,
