...
import {
  CreativeBrandFidelityExecutionGate,
} from "@/lib/creative/assets/intelligence/runtime/CreativeBrandFidelityExecutionGate";
import {
  prepareCreativeVideoProductionTask,
} from "./CreativeVideoProductionDispatchBootstrap";
import {
...
export async function dispatchCreativeVideoGeneration({
  ...
  let task = await ProductionTaskRuntime.get(task_id);
  ...
  task = await CreativeBrandFidelityExecutionGate.enforce(task);
