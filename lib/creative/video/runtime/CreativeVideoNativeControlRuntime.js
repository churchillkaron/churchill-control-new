...
function explicitMediaReference(value) {
  const source = text(value);
  if (!source || /\s/.test(source)) return false;
  return /^(?:https?:\/\/|data:image\/|blob:|file:|supabase:|storage:|asset:|[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$)/i.test(source);
}
function candidate(value) {
  if (!value) return null;
  if (typeof value === "string") return explicitMediaReference(value) ? text(value) : null;
...
function referenceConditions(opening, closing, shotKeyframes, fallbackSource = null) {
  const conditions = [];
  const first = opening || fallbackSource;
  if (first) conditions.push({ reference: first, role: "OPENING_FRAME", frame_index: 0, frame_fraction: 0, strength: 1, crf: 0 });
...
function providerReferencePriority(item = {}) {
  const role = text(item.role).toUpperCase();
  if (["OPENING_FRAME","CLOSING_FRAME"].includes(role)) return 0;
  if (["IMAGE_STUDIO_SELECTED_HERO_FRAME","IMAGE_STUDIO_CONTINUITY_REFERENCE","IMAGE_STUDIO_PERFORMANCE_REFERENCE","IMAGE_STUDIO_CONTACT_DETAIL_REFERENCE"].includes(role)) return 1;
  if (role.startsWith("IMAGE_STUDIO_CHARACTER_") || role.startsWith("IMAGE_STUDIO_THREAT_")) return 2;
  if (["CAMERA_MOTION_REFERENCE","PERFORMANCE_REFERENCE"].includes(role)) return 3;
  if (role.startsWith("SHOT_BIBLE_KEYFRAME")) return 4;
  if (role.startsWith("IMAGE_STUDIO_MATERIAL_TRUTH_")) return 5;
  return 6;
}
function mergedSourceAssets(input = {}, conditions = [], craftReferences = []) {
  ...
  for (const item of values) {
    const key = sourceAssetKey(item);
    if (!key) continue;
    const prior = seen.get(key);
    if (!prior || providerReferencePriority(item) < providerReferencePriority(prior)) {
      seen.set(key, item);
    }
  }
  const unique = [...seen.values()].sort(
    (a, b) => providerReferencePriority(a) - providerReferencePriority(b),
  );
  return input.image_provider_reference_manifest?.contract
    ? unique.slice(0, 12)
    : unique;
}
export function applyCreativeVideoNativeControls(input = {}) {
  ...
  const capability = text(input.capability || input.generation?.capability || input.generation?.service).toLowerCase();
  const fallbackSource = capability === "ai.video.image_to_video"
    ? candidate(list(input.source_assets)[0])
    : null;
  const conditions = referenceConditions(opening, closing, shotKeyframes, fallbackSource);
  ...
  const sourceAssetCandidates = [
    ...
    ...conditions.map((item) => item.reference),
    ...craftReferences.map((item) => item.reference),
  ];
  const imageReferenceTransportBounded = Boolean(input.image_provider_reference_manifest?.contract);
  const transportedKeys = new Set(sourceAssets.map(sourceAssetKey).filter(Boolean));
  const imageManifest = object(input.image_provider_reference_manifest);
  const mandatoryImageAuthority = list(imageManifest.source_assets).filter((item) =>
    list(imageManifest.mandatory_roles).some((role) =>
      text(item.role).toUpperCase() === text(role).toUpperCase() ||
      list(item.role_aliases).some((alias) => text(alias).toUpperCase() === text(role).toUpperCase()),
    ),
  );
  const missingMandatoryImageAuthority = mandatoryImageAuthority.filter(
    (item) => !transportedKeys.has(sourceAssetKey(item)),
  );
  if (missingMandatoryImageAuthority.length) {
    throw new Error(
      "VIDEO_NATIVE_CONTROL_DROPPED_MANDATORY_IMAGE_AUTHORITY:" +
      missingMandatoryImageAuthority
        .map((item) => text(item.role || item.asset_node_id || item.url))
        .join(","),
    );
  }
  ...
  return {
    ...
    metadata: {
      ...
      creative_video_native_control: {
        ...
        native_audio_required: nativeAudioRequired,
        source_assets_preserved:
          !imageReferenceTransportBounded ||
          sourceAssets.length === sourceAssetCandidates.length,
        image_reference_transport_bounded: imageReferenceTransportBounded,
        image_reference_transport_limit: imageReferenceTransportBounded ? 12 : null,
        image_reference_transport_count: sourceAssets.length,
        image_reference_candidate_count: sourceAssetCandidates.length,
        image_reference_transport_safe:
          (!imageReferenceTransportBounded || sourceAssets.length <= 12) &&
          missingMandatoryImageAuthority.length === 0,
        mandatory_image_authority_count: mandatoryImageAuthority.length,
        missing_mandatory_image_authority_count:
          missingMandatoryImageAuthority.length,
