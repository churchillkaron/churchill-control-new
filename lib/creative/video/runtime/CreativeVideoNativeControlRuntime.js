...
function providerReferencePriority(item = {}) {
  ...
  if (["OPENING_FRAME","CLOSING_FRAME"].includes(role) || role.startsWith("SHOT_BIBLE_KEYFRAME")) return 0;
  ...
  if (["CAMERA_MOTION_REFERENCE","PERFORMANCE_REFERENCE"].includes(role)) return 2;
  if (role.startsWith("IMAGE_STUDIO_CHARACTER_") || role.startsWith("IMAGE_STUDIO_THREAT_")) return 3;
  if (role.startsWith("IMAGE_STUDIO_MATERIAL_TRUTH_")) return 4;
  return 5;
}
  ...
  return {
    ...
    metadata: {
      ...
      creative_video_native_control: {
        ...
        image_reference_transport_safe: !imageReferenceTransportBounded || sourceAssets.length <= 12,
