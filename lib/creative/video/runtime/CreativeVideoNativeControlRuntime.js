...
function sourceAsset(reference, role) {
  ...
  if (typeof reference === "object") return { ...reference, role };
...
function sourceAssetKey(value = {}) {
  return text(value.asset_id || value.assetId || value.id || value.url || value.file_url || value.fileUrl || value.storage_reference || value.storageReference);
}
...
function normalizedKeyframe(value, index) {
  ...
  return {
    ...
    frame_fraction: frameFraction === null ? null : Math.max(0, Math.min(1, frameFraction)),
...
function mergedSourceAssets(input = {}, conditions = [], craftReferences = []) {
  const values = [
    ...list(input.source_assets).map((item) => ({ ...object(item), role: text(item?.role || "SOURCE_REFERENCE") })),
  ...
  return {
    ...
    metadata: {
      ...
      creative_video_native_control: {
        ...
        reference_conditions: conditions.map((item, index) => ({
          source_asset_index: index,
          role: item.role,
          frame_index: item.frame_index,
          frame_fraction: item.frame_fraction,
