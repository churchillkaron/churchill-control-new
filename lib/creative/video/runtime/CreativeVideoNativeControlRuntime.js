...
function mergedSourceAssets(input = {}, conditions = [], craftReferences = []) {
  ...
  const unique = [...seen.values()].sort((a,b) => providerReferencePriority(a) - providerReferencePriority(b));
  const governedImagePackage = Boolean(input.image_provider_reference_manifest?.contract);
  return governedImagePackage ? unique.slice(0, 12) : unique;
}
