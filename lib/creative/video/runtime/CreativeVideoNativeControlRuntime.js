...
export function applyCreativeVideoNativeControls(input = {}) {
  ...
  const sourceAssets = conditions.map((item) => ({ ...object(typeof item.reference === "object" ? item.reference : {}), ...(typeof item.reference === "string" ? { url: item.reference } : {}), role: item.role }));
  return {
    ...input,
