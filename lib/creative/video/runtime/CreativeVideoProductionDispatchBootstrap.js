...
function install() {
  ...
  ProductionTaskRuntime.dispatch = async function dispatchCreativeVideo(id) {
    ...
    const imageProductionPackage = object(task.input?.image_production_package);
    if (Object.keys(imageProductionPackage).length) {
      const assetNodes = await CreativeAssetGraphRuntime.list({
        organization_id: task.organization_id,
        creative_project_id: task.creative_project_id,
      });
      const currentPackage = CreativeImageProductionPackageRuntime.build({
        task: scopedTask,
        asset_nodes: assetNodes,
        previsualization_authority: task.input?.image_previsualization_authority || null,
      });
      if (currentPackage.passed !== true) {
        throw new Error(
          `IMAGE_STUDIO_PRODUCTION_PACKAGE_STALE_OR_BLOCKED:${currentPackage.failures.join(",")}`,
        );
      }
      if (
        text(currentPackage.production_package_digest) !==
        text(imageProductionPackage.production_package_digest)
      ) {
        throw new Error("IMAGE_STUDIO_PRODUCTION_PACKAGE_STALE");
      }
    }
    ...
    const imageCameraAuthority = object(task.input?.image_camera_authority);
    if (Object.keys(imageCameraAuthority).length) {
      const comparison = CreativeImageCameraAuthorityRuntime.compare({
      ...
      if (comparison.passed !== true) {
        throw new Error(
          `IMAGE_STUDIO_CAMERA_AUTHORITY_VIDEO_DISPATCH_DRIFT:${comparison.failures.join(",")}`,
        );
      }
    }
    ...
    const baseShotBible = CreativeShotBibleRuntime.assert(
      CreativeShotBibleRuntime.build({ shot, task: scopedTask }),
    );
    const shotBible = {
      ...baseShotBible,
      cinematography_acquisition: cinematographyAcquisition,
      ...
      metadata: {
        ...
        creative_cinematography_acquisition: {
          ...
          image_camera_authority_hash:
            imageCameraAuthority.authority_hash || null,
    ...
    task = await ProductionTaskRuntime.update(task.id, {
      ...
      metadata: {
        ...
        image_camera_authority_contract:
          imageCameraAuthority.contract || null,
        image_camera_authority_hash:
          imageCameraAuthority.authority_hash || null,
        image_camera_authority_validated:
          Object.keys(imageCameraAuthority).length > 0,
        image_production_package_digest:
          imageProductionPackage.production_package_digest || null,
        image_production_package_stale_check_passed:
          Object.keys(imageProductionPackage).length > 0,
