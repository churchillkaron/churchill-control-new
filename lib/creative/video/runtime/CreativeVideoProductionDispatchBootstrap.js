...
const INSTALL_FLAG = Symbol.for(
  "avantiqo.creative.video-production-dispatch.v4-human-controlled",
);
...
function assertGenericVideoDispatchAllowed(task = {}) {
  ...
  throw new Error(
    ...
    "USE_AVANTIQO_VIDEO_HUMAN_MULTI_KEYFRAME_NATIVE_MASTER_V1",
  );
}
...
function install() {
  ...
  ProductionTaskRuntime.dispatch = async function dispatchCreativeVideo(id) {
    ...
    // Investor footage containing people has its own governed renderer. It is a
    // correctness failure to let those tasks reach the generic single-reference
    // video route, even if the selected provider is Avantiqo-owned.
    ...
    const controlledInput = CreativeVideoNativeControlRuntime.apply({
      ...sanitizedInput,
      shot_id: shotId,
      capability,
      shot_bible: shotBible,
