...
function resolutionPriority(values = [], supported = []) {
  const configured = supportedResolutions(values);
  return configured.length ? configured : supported;
}

function resolutionConstraint(provider_capabilities = {}, resolution) {
  const constraints = object(provider_capabilities.resolution_constraints);
  return object(constraints[resolution]);
}

function allowedDurations(constraint = {}) {
  return (Array.isArray(constraint.allowed_duration_seconds)
    ? constraint.allowed_duration_seconds
    : [])
    .map(Number)
...
export function resolveCreativeVideoExecutionQuality({
  ...
  const capabilities = object(provider_capabilities);
  const supported = supportedResolutions(capabilities.supported_resolutions);
  ...
  if (preference === "AUTO") {
    const bestResolution = resolutionPriority(
      capabilities.auto_resolution_priority,
      supported,
  ...
  if (definition.resolution) {
    const constraint = resolutionConstraint(capabilities, definition.resolution);
    const durations = allowedDurations(constraint);
    if (
      durations.length &&
      (!Number.isFinite(duration) || !durations.includes(duration))
    ) {
      reasons.push(
        `VIDEO_PROVIDER_RESOLUTION_DURATION_UNSUPPORTED:${definition.resolution}:${Number.isFinite(duration) ? duration : "missing"}`,
      );
    }
  }

  const supportedAspectRatios = Array.isArray(capabilities.supported_aspect_ratios)
    ? capabilities.supported_aspect_ratios.map(text).filter(Boolean)
    : [];
  if (
    supportedAspectRatios.length &&
    !supportedAspectRatios.includes(text(aspect_ratio))
  ) {
  ...
  return {
    ...
    provider_native_frame_rate: Number.isFinite(Number(capabilities.native_frame_rate))
      ? Number(capabilities.native_frame_rate)
      : null,
    native_audio: typeof capabilities.native_audio === "boolean"
