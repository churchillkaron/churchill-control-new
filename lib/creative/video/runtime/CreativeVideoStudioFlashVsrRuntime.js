...
import {
  ...
  deleteAvantiqoVideoRunpodVolumeObject,
  presignAvantiqoVideoRunpodVolumeObject,
...
export const AVANTIQO_VIDEO_FLASHVSR_MODEL_REVISION = "a258bf2d5b99bf54cf048d901edc866591d5ea0b";
...
function fourXTarget(width, height) {
...
async function uploadFileToVolume(key, filePath) {
  const signed = presignAvantiqoVideoRunpodVolumeObject({ key, method: "PUT", expires_seconds: 1800 });
  ...
  if (!response.ok) throw new Error(`AVANTIQO_VIDEO_RUNPOD_S3_UPLOAD_HTTP_${response.status}`);
  return stat.size;
}

async function downloadVolumeObject(key, filePath, maxBytes = 16 * 1024 * 1024 * 1024) {
  const signed = presignAvantiqoVideoRunpodVolumeObject({ key, method: "GET", expires_seconds: 1800 });
  const response = await fetch(signed.url, { signal: AbortSignal.timeout(30 * 60 * 1000) });
  if (!response.ok) throw new Error(`AVANTIQO_VIDEO_RUNPOD_S3_DOWNLOAD_HTTP_${response.status}`);
  const length = finite(response.headers.get("content-length"), null);
  if (length !== null && length > maxBytes) throw new Error(`AVANTIQO_VIDEO_RUNPOD_S3_OBJECT_TOO_LARGE:${length}`);
  if (!response.body) throw new Error("AVANTIQO_VIDEO_RUNPOD_S3_DOWNLOAD_BODY_REQUIRED");
  const handle = await fs.open(filePath, "w");
  let written = 0;
  try {
    for await (const chunk of Readable.fromWeb(response.body)) {
      written += chunk.length;
      if (written > maxBytes) throw new Error(`AVANTIQO_VIDEO_RUNPOD_S3_OBJECT_TOO_LARGE:${written}`);
...
export async function prepareCreativeVideoFlashVsrInput({ organization_id, source_url, owner_request_id, media_tools = {} } = {}) {
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-video-flashvsr-prep-"));
  const rawPath = path.join(root, "input.rgb");
  const keys = avantiqoVideoRunpodMasterObjectKeys(owner_request_id);
  try {
    const probe = await probeVideo(ffprobe, source.file_path);
    const target = fourXTarget(probe.width, probe.height);
    ...
    const frameBytes = target.width * target.height * 3;
    if (stat.size < frameBytes || stat.size % frameBytes !== 0) throw new Error("AVANTIQO_VIDEO_STUDIO_FLASHVSR_RAW_BYTES_INVALID");
    const sourceFrames = stat.size / frameBytes;
    const paddedFrames = nextCausalFrameWindow(sourceFrames);
    if (paddedFrames > sourceFrames) {
      const handle = await fs.open(rawPath, "r+");
      try {
        const last = Buffer.allocUnsafe(frameBytes);
        const { bytesRead } = await handle.read(last, 0, frameBytes, stat.size - frameBytes);
        if (bytesRead !== frameBytes) throw new Error("AVANTIQO_VIDEO_STUDIO_FLASHVSR_LAST_FRAME_READ_INVALID");
        let position = stat.size;
        for (let frame = sourceFrames; frame < paddedFrames; frame += 1) {
          await handle.write(last, 0, last.length, position);
    ...
    const bytes = await uploadFileToVolume(keys.input_rgb, rawPath);
    return {
      success: true,
      ...
      input_key: keys.input_rgb,
      input_path: avantiqoVideoRunpodVolumePath(keys.input_rgb),
...
export async function readCreativeVideoFlashVsrReceipt(receiptKey) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-video-flashvsr-receipt-"));
  const receiptPath = path.join(root, "receipt.json");
  try {
    try {
      await downloadVolumeObject(receiptKey, receiptPath, 2 * 1024 * 1024);
    } catch (error) {
      if (text(error?.message).includes("_HTTP_404")) return null;
      throw error;
    }
...
export async function finalizeCreativeVideoFlashVsrMaster({ organization_id, source_url, prepared, receipt, media_tools = {} } = {}) {
  ...
  try {
    ...
    await downloadVolumeObject(prepared.output_key, rawPath);
...
export async function cleanupCreativeVideoFlashVsrObjects(prepared = {}) {
  const keys = [prepared.input_key, prepared.output_key, prepared.receipt_key].map(text).filter(Boolean);
  for (const key of keys) await deleteAvantiqoVideoRunpodVolumeObject(key).catch(() => null);
}
