...
import { CreativeLocalMediaQueueRuntime } from "@/lib/creative/media/runtime/CreativeLocalMediaQueueRuntime";
...
export async function assembleCreativeVideoStudioFoundation({
  ...
  try {
    if (await CreativeLocalMediaQueueRuntime.available()) {
      const local = await CreativeLocalMediaQueueRuntime.execute({
        organization_id,
        usage_id: `video-foundation-${randomUUID()}`,
        operation: "raw_frames_to_mp4",
        source: frame_tensor_reference,
        options: { fps: effectiveFps },
        extension: "mp4",
      });
      const remote = await materializeMedia({
        organization_id,
        url: local.output?.asset_url || local.output?.storage_reference,
        file_name: "avantiqo-video-foundation-local.mp4",
        mime_type: "video/mp4",
        policy: { max_bytes: 2_147_483_648, timeout_ms: 300_000, max_redirects: 0 },
      });
      try {
        const buffer = await fs.readFile(remote.file_path);
        return { success: true, contract: AVANTIQO_VIDEO_STUDIO_FOUNDATION_CONTRACT, buffer, fps: local.output?.fps || effectiveFps, frame_count: local.output?.frame_count || null, width: local.output?.width || null, height: local.output?.height || null, duration_seconds: local.output?.duration_seconds || null, processing_ms: Number(local.metrics?.elapsed_ms || 0), infrastructure_provider: local.infrastructure_provider, node_id: local.node_id, studio_compute_only: true, gpu_compute_used: false, paid_provider_contacted: false, ffmpeg_location: "AVANTIQO_LOCAL_NODE" };
      } finally { await remote.cleanup().catch(() => {}); }
    }
  } catch (error) {
    if (String(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED || "").trim().toLowerCase() === "true") throw error;
    console.error("AVANTIQO_VIDEO_FOUNDATION_LOCAL_FALLBACK", { error: String(error?.message || error).slice(0, 500) });
  }
