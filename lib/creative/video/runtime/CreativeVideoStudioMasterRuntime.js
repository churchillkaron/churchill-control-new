  ...
  try {
    if (await CreativeLocalMediaQueueRuntime.available()) {
      const local = await CreativeLocalMediaQueueRuntime.execute({
        organization_id,
        usage_id: `video-master-${randomUUID()}`,
        operation: "video_master",
        source: source_url,
      ...
      const remote = await materializeMedia({
        ...
        file_name: "avantiqo-video-master-local.mp4",
        mime_type: "video/mp4",
      ...
      try {
        ...
        return {
          ...
          backend: "AVANTIQO_LOCAL_NODE_FFMPEG_LANCZOS",
          buffer,
          input_probe: local.output?.input_probe || null,
          output_probe: local.output?.output_probe || null,
          target_resolution: text(target_resolution).toLowerCase(),
          target_width: local.output?.target_width || null,
          target_height: local.output?.target_height || null,
          processing_ms: Number(local.metrics?.elapsed_ms || 0),
          infrastructure_provider: local.infrastructure_provider,
          node_id: local.node_id,
          studio_compute_only: true, gpu_compute_used: false, paid_provider_contacted: false, fal_contacted: false, external_mastering_provider_contacted: false,
        };
    ...
    console.error("AVANTIQO_VIDEO_MASTER_LOCAL_FALLBACK", { error: String(error?.message || error).slice(0, 500) });
  }
