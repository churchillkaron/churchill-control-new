...
function sectionHtml(section) {
  ...
  const media = section.media?.url
    ? `<img src="${escapeHtml(section.media.url)}" alt="${escapeHtml(section.media.alt || section.title || "")}" loading="lazy">` : "";
  return `<section id="${escapeHtml(section.id)}" class="section section-${escapeHtml(section.type)}">` +
    ...
    `<h2>${escapeHtml(section.title)}</h2><p>${escapeHtml(section.body)}</p>${cta}</div>${media}` +
...
function websiteFiles(contract) {
  ...
  const css = `:root{--bg:${theme.background};--surface:${theme.surface};--fg:${theme.foreground};--muted:${theme.muted};--accent:${theme.accent};font-family:${theme.font_family};}` +
    ...
    `h1,h2,h3{line-height:1.08;margin:0 0 1rem}h2{font-size:clamp(2.2rem,7vw,6rem);letter-spacing:-.045em}p{color:var(--muted);max-width:68ch}` +
  ...
  const manifest = JSON.stringify({ contract: "AVANTIQO_STATIC_WEBSITE_V1", title: contract.title, generated_at: new Date().toISOString(), files: ["index.html", "styles.css", "app.js"] }, null, 2);
  return { "index.html": html, "styles.css": css, "app.js": js, "site-manifest.json": manifest };
}
...
function staticAudit(files, contract) {
  ...
  if (!/<h2[\s>]/i.test(html)) failures.push("WEBSITE_HEADING_REQUIRED");
...
export const CreativeWebsiteProductionRuntime = {
  async build(task) {
    ...
    try {
      ...
      const packageBuffer = createStoredZip(Object.entries(files).map(([name, data]) => ({ name, data })));
      const uploadedFiles = [];
  ...
  async validate(task) {
    ...
    for (const required of ["index.html", "styles.css", "app.js", "site-manifest.json", "website.zip"]) {
