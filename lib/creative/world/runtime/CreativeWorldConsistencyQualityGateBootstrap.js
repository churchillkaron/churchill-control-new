...
const REQUIRED_DIMENSIONS = Object.freeze([
  ...
  "wind_precipitation_wetness",
  "ground_deformation_tracks",
  "wardrobe_wetness",
  "atmosphere_density_scattering",
  "practical_light_lightning_state",
  "moving_threat_spatial_state",
...
const REQUIRED_TRUE = Object.freeze([
  ...
  "environmental_causality_valid",
  "weather_wetness_continuity_valid",
  "moving_threat_spatial_lineage_valid",
]);
...
const REQUIRED_FALSE = Object.freeze([
  ...
  "wind_direction_flip_detected",
  "precipitation_reset_detected",
  "wetness_reset_detected",
  "ground_deformation_reset_detected",
  "footprint_track_disappearance_detected",
  "wardrobe_wetness_reset_detected",
  "fog_density_reset_detected",
  "practical_light_teleportation_detected",
  "lightning_consequence_reset_detected",
  "moving_threat_teleportation_detected",
]);
...
function promptExtension(contract, contractHash, previous) {
  return `\n\n${PROMPT_MARKER}\nWorld consistency is a hard release gate. Inspect the ACTUAL rendered frames. A camera angle change does not authorize the world itself to mutate. Compare location identity, geography, architecture, set dressing, props, materials, readable signage/text, lighting sources/direction, time/weather/atmosphere, wind direction, precipitation, wetness accumulation, mud/ground deformation, footprints/tracks, wardrobe wetness, fog/volumetric density, practical/search lights, lightning consequences, moving threat positions, background population, scale/perspective, reflections, shadows and occlusion against the governed world contract and any supplied previous approved cinematic state. Return strict JSON and add world_consistency_qc with: contract exactly "${CONTRACT}", world_contract_hash exactly "${contractHash}", passed boolean, failures array, world_observations containing every dimension ${JSON.stringify(REQUIRED_DIMENSIONS)}, explicit booleans ${REQUIRED_TRUE.map((key) => `${key}=true`).join(", ")}, and ${REQUIRED_FALSE.map((key) => `${key}=false`).join(", ")}. A beautiful or high-scoring shot still FAILS if the venue geometry, doors/windows/walls, furniture layout, props, signs, material identity, light direction, weather/time state, wetness, mud, footprints, wardrobe condition, atmosphere density, search-light position, lightning aftereffects, moving threat geography, crowd/background, reflections/shadows or object scale silently changes. Only an explicit authorized world change in the contract may alter locked state. Do not infer hidden geometry from wishful intent; judge visible evidence. Governed world contract: ${JSON.stringify(contract)}. Previous approved world context: ${JSON.stringify(previous)}\n`;
}
