...
export async function exchangeCustomerPortalAccessToken(rawToken) {
  ...
  let exchanged = await supabaseAdmin.rpc("consume_customer_portal_access_token_atomic", {
    p_token_hash: hash(normalized),
    p_session_token_hash: sessionTokenHash,
    p_session_expires_at: sessionExpiresAt,
  });
  if (exchanged.error && /function .* does not exist|schema cache/i.test(String(exchanged.error.message || ""))) {
    exchanged = await supabaseAdmin.rpc("customer_portal_exchange_access_token_atomic", {
      p_token_hash: hash(normalized),
      p_session_token_hash: sessionTokenHash,
      p_session_expires_at: sessionExpiresAt,
    });
  ...
  if (exchanged.error) {
    ...
    if (message.includes("CUSTOMER_PORTAL_ACCESS_INVALID")) throw new Error("CUSTOMER_PORTAL_ACCESS_INVALID");
  ...
  if (outcome && outcome !== "SUCCESS") throw new Error("CUSTOMER_PORTAL_ACCESS_INVALID");
  const session = exchange?.session || (exchange?.session_id ? {
  ...
  if (!session?.id) throw new Error("CUSTOMER_PORTAL_ACCESS_INVALID");
...
export async function resolveCustomerPortalSession(rawCookieToken) {
  ...
  const session = touched.data || null;
