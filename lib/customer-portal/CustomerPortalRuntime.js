...
export async function exchangeCustomerPortalAccessToken(rawToken) {
  ...
  const exchanged = await supabaseAdmin.rpc("customer_portal_exchange_access_token_atomic", {
    p_token_hash: hash(normalized),
    p_session_token_hash: hash(rawSession),
  ...
  const session = exchanged.data?.session || null;
  if (!session?.id) throw new Error("CUSTOMER_PORTAL_SESSION_CREATE_FAILED");
  return { raw_session_token: rawSession, session };
}
...
export async function revokeCustomerPortalSession(rawSessionToken) {
  const normalized = text(rawSessionToken, 1000);
