...
export async function developerUsageSummary(organizationId, { externalDeveloper = false, userId = null } = {}) {
  ...
  try {
    ...
    const result = await supabaseAdmin
      ...
      .select("id,organization_id,provider,capability,operation,status,execution_status,provider_model,customer_price,supplier_cost,currency,latency_ms,error_message,created_at")
      .eq("organization_id", id)
      .order("created_at", { ascending: false })
      .limit(100);
