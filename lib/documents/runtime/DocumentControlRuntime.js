...
export async function createControlledDocumentPack({
  organizationId,
  entityId = null,
  actor = null,
  documents = [],
  ...
  if (!organizationId) throw new Error("organizationId required");
  if (!Array.isArray(documents) || documents.length < 2 || documents.length > 20) {
    throw new Error("controlled document pack requires 2 to 20 documents");
  }
  ...
  try {
    for (const item of documents) {
      ...
      if (!file || typeof file.arrayBuffer !== "function") throw new Error("pack document file required");
      const documentId = randomUUID();
      const filename = safeName(file.name || item.documentName || "document");
      const buffer = Buffer.from(await file.arrayBuffer());
      const checksum = sha256(buffer);
      const mimeType = optionalText(file.type, 200) || "application/octet-stream";
      const path = storagePath({ organizationId, entityId, documentId, version: 1, filename });
      const metadata = {
      ...
      prepared.push({
        id: documentId, entity_id: entityId || null,
        document_type: clean(item.documentType || "FILE", 120).toUpperCase(),
        document_name: clean(item.documentName || file.name || "Document", 500),
        document_number: optionalText(item.documentNumber, 160),
        classification: cleanClassification(item.classification),
        storage_path: path, file_size_bytes: buffer.length, mime_type: mimeType,
        checksum_sha256: checksum, source_filename: file.name || filename,
        created_by: actorStaffId, owner_staff_id: item.ownerStaffId || actorStaffId,
        effective_date: cleanDate(item.effectiveDate, "effectiveDate"),
        expiry_date: cleanDate(item.expiryDate, "expiryDate"),
        review_due_at: cleanDate(item.reviewDueAt, "reviewDueAt"),
        retention_until: cleanDate(item.retentionUntil, "retentionUntil"),
        reference_table: optionalText(item.referenceType, 160), reference_id: item.referenceId || null,
        source_organization_document_id: item.sourceOrganizationDocumentId || null, metadata,
      });
    }

    const rpc = await supabaseAdmin.rpc("create_enterprise_document_pack_atomic", {
      p_organization_id: organizationId,
    ...
    await Promise.all(uploadedPaths.map((path) => removePrivateFile(path)));
    throw error;
  }
}
...
export async function updateControlledDocument({
  ...
  let updateQuery = supabaseAdmin
    ...
    .eq("id", documentId);
  if (expectedStatus) {
    updateQuery = updateQuery.eq("document_status", clean(expectedStatus, 80).toLowerCase());
  }
  ...
  if (error) throw error;
  if (!data) {
    const conflict = new Error("Controlled document changed before the update could be applied");
    conflict.status = 409;
    conflict.code = "CONTROLLED_DOCUMENT_STATE_CONFLICT";
    throw conflict;
  }
