...
export function createDocumentsFilePackCreateCapability() {
  const manifest = defineCapability({
    ...
    risk: "medium",
    contextScope: "organization",
    operatorVerification: { capability_key: "documents.files.verifyPack", payload_array_from_result: { document_ids: { path: "pack.documents", item_path: "id" } }, derivation: "declared_result_bound_collection_verifier" },
    inputSchema: {
      type: "object",
