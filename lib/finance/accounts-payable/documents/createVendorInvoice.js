...
function normalizeLines(lines) {
  ...
  return lines.map((line, index) => {
    ...
    return {
      ...
      source_item_code: String(line?.source_item_code || "").trim() || null,
      source_uom_text: String(line?.source_uom_text || "").trim() || null,
      uom_id: optionalId(line?.uom_id),
      factor_to_item_base: line?.factor_to_item_base === null || line?.factor_to_item_base === undefined || line?.factor_to_item_base === "" ? null : (Number.isFinite(Number(line.factor_to_item_base)) ? Number(line.factor_to_item_base) : null),
      base_unit_cost: line?.base_unit_cost === null || line?.base_unit_cost === undefined || line?.base_unit_cost === "" ? null : (Number.isFinite(Number(line.base_unit_cost)) ? Number(line.base_unit_cost) : null),
      cost_evidence_status: String(line?.cost_evidence_status || "").trim() || null,
    };
  });
}
...
export async function createVendorInvoice({
  ...
  try {
    ...
    throw attachActionIdentityEvidence(rpcError, [`id:${invoiceId}`, `invoice_id:${invoiceId}`]);
  }

  if (error) {
    throw attachActionIdentityEvidence(new Error(`Idempotent vendor invoice failed: ${error.message}`), [`id:${invoiceId}`, `invoice_id:${invoiceId}`]);
  }

  const resolvedInvoiceId = data?.invoice?.id || data?.vendor_invoice?.id || invoiceId;
  let costEvidencePersisted = true;
  let costEvidenceError = null;
  for (let index = 0; index < normalizedLines.length; index += 1) {
    const line = normalizedLines[index];
    if (!line.source_item_code && !line.source_uom_text && !line.item_id) continue;
    const { error: evidenceError } = await supabaseAdmin
      .from("vendor_invoice_lines")
      .update({
        source_item_code: line.source_item_code, source_uom_text: line.source_uom_text,
        uom_id: line.uom_id, factor_to_item_base: line.factor_to_item_base, base_unit_cost: line.base_unit_cost,
      })
      .eq("organization_id", resolvedOrganizationId).eq("entity_id", resolvedEntityId)
      .eq("vendor_invoice_id", resolvedInvoiceId).eq("line_number", index + 1);
    if (evidenceError) { costEvidencePersisted = false; costEvidenceError = evidenceError.message; break; }
  }

  return {
    ...data,
    lines: normalizedLines,
    cost_evidence: { persisted: costEvidencePersisted, error: costEvidenceError },
  };
}
