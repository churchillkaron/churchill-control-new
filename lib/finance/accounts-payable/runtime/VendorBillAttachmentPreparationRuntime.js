...
import { resolveVendorBillProcurementEvidence } from "./VendorBillProcurementEvidenceRuntime";
...
function rawLines(source = {}) {
  ...
  if (rows.length) return rows.map((entry, index) => {
    ...
    return { description: text(row.description || row.name || row.item || `Supplier invoice line ${index + 1}`), quantity: Number.isFinite(quantity) && quantity > 0 ? quantity : 1,
      ...
      purchase_order_item_id: text(row.purchase_order_item_id || row.po_line_id, 80) || null };
  });
...
export async function prepareVendorBillAttachment({ file = {}, organizationId, entityId } = {}) {
  ...
  const procurement = await resolveVendorBillProcurementEvidence({
    organizationId,
    entityId,
    vendorPartyId: supplier.party_id,
    source,
    lines: normalizedLines,
  });
  if (!["NOT_REFERENCED", "EXACT_MATCH"].includes(procurement.status)) {
    return {
      contract: CONTRACT,
      recognized: true,
      status: "CLARIFICATION_REQUIRED",
      clarification_required: true,
      clarification_question: procurement.blocker || "Review the purchase-order and goods-receipt evidence before creating this supplier invoice.",
      vendor: { party_id: supplier.party_id, vendor_code: supplier.vendor_code || null },
      procurement_evidence: procurement,
      authorization_effect: "NONE",
    };
  }
  const resolvedLines = procurement.lines || normalizedLines;
  return {
    contract: CONTRACT,
    recognized: true,
    status: "READY_FOR_REVIEW",
    clarification_required: false,
    touchless_stage: procurement.touchless_eligible ? "READY_FOR_CREATE_AND_MATCH" : "READY_FOR_REVIEW",
    vendor: { party_id: supplier.party_id, vendor_code: supplier.vendor_code || null },
    bill: { invoice_number: invoiceNumber, invoice_date: invoiceDate, due_date: dueDate, currency_code: currency, lines: resolvedLines },
    procurement_evidence: procurement,
    import_payload: {
      vendor_party_id: supplier.party_id,
      purchase_order_id: procurement.purchase_order_id || null,
      goods_receipt_id: procurement.goods_receipt_id || null,
      ...
      ocr_confidence: Number(evidence(file).confidence || object(file.analysis).confidence || 0),
      lines: resolvedLines,
    },
    authorization_effect: "NONE",
  };
