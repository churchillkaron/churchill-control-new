...
export default async function postCustomerPayment({
  ...
  const paidBy = system_automation === true
    ? (paid_by ? uuid(paid_by, "paid_by") : null)
    : uuid(paid_by, "paid_by");
  ...
  const { data, error } = await supabaseAdmin.rpc(
    "finance_post_customer_receipt_party_idempotent",
    {
  ...
  if (error) {
    throw attachActionIdentityEvidence(
      new Error(`Customer receipt allocation failed: ${error.message}`),
