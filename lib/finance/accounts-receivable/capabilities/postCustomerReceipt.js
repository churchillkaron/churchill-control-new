...
export const manifest = defineCapability({
  ...
  reversible: true,
  contextScope: "entity",
  operatorVerification: { capability_key: "finance.customer_receipt.read", payload_from_result: { payment_id: ["payment_id"] }, derivation: "declared_result_bound_payment_verifier" },
  operatorAliases: [
    "mark invoice paid",
...
async function resolveBankAccount({ organizationId, entityId, currencyCode, requestedId }) {
  let query = supabaseAdmin
    .from("bank_accounts")
    .select("id,bank_name,account_name,account_number,currency,currency_code,active,is_default,finance_account_id")
    .eq("organization_id", organizationId)
    .eq("entity_id", entityId)
    .eq("active", true);
  ...
  return {
    ...
    paid_to_bank_account: {
      ...
      currency_code: text(bank.currency_code || bank.currency || currencyCode).toUpperCase(),
    },
    artifacts: [{
