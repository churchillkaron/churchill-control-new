...
export default async function createCustomerInvoice(input) {
  ...
  const canonicalInvoiceId = data?.invoice?.id || data?.invoice_id || prepared.invoiceId;
  const portalProvisioning = await provisionCustomerPortalAccess({
    organizationId: prepared.organizationId,
    partyId: prepared.partyId,
    sourceType: "CUSTOMER_INVOICE",
    sourceId: canonicalInvoiceId,
