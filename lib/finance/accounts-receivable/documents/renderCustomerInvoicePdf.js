...
async function loadPaymentEvidence({ organizationId, entityId, invoiceId }) {
  ...
  let paymentQuery = supabaseAdmin
    ...
    .select("id,payment_number,payment_date,amount,payment_method,reference_number,currency_code,status,allocated_amount,unapplied_amount,bank_account_id")
    .eq("organization_id", organizationId)
  ...
  if (payment.bank_account_id) {
    ...
    const bankResult = await bankQuery.maybeSingle();
    if (bankResult.error) throw bankResult.error;
    bank = bankResult.data || null;
  }

  return { kind: "posted", allocation, payment, bank };
}
...
function legalAddress(brand = {}) {
  ...
  return [legal.address, legal.phone, legal.email]
    .flatMap((value) => text(value).split(/\r?\n/))
    .map(text)
    .filter(Boolean);
}

function partyAddressLines(party = {}) {
  return [party.address, party.phone, party.email]
    .flatMap((value) => text(value).split(/\r?\n/))
    .map(text)
    .filter(Boolean);
}
...
export async function renderCustomerInvoicePdf({ organizationId, entityId, invoiceId, mode = "auto" }) {
  ...
  // Document heading stays independent from the two party columns.
  if (brandLogo) {
    try {
      pdf.addImage(brandLogo.data, brandLogo.format, left, 24, 118, 78, undefined, "FAST");
    } catch {
      // Legal name below remains the fallback if the image cannot be embedded.
    }
  }
  pdf.setTextColor(22, 22, 22);
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(28);
  pdf.text(receiptMode ? "RECEIPT" : "INVOICE", right, 50, { align: "right" });
  pdf.setFontSize(10);
  pdf.setTextColor(...accent);
  pdf.text(text(document.invoice_number || "DRAFT"), right, 68, { align: "right" });

  // Seller and customer are parallel columns at the same vertical level.
  const sellerX = left;
  const billToX = 310;
  const blockTop = 126;
  const labelY = blockTop;
  const nameY = blockTop + 20;
  const detailY = blockTop + 37;

  pdf.setTextColor(105, 105, 105);
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(8);
  pdf.text("FROM", sellerX, labelY);
  pdf.text("BILL TO", billToX, labelY);

  pdf.setTextColor(25, 25, 25);
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(13);
  pdf.text(text(legal.legal_name || brand.name || "Company"), sellerX, nameY);
  pdf.text(customerName(document, context), billToX, nameY);

  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(8.5);
  pdf.setTextColor(70, 70, 70);
  const sellerLines = legalAddress(brand);
  let sellerCursor = detailY;
  sellerLines.forEach((line) => {
    const wrapped = pdf.splitTextToSize(line, 220);
    pdf.text(wrapped, sellerX, sellerCursor);
    sellerCursor += Math.max(1, wrapped.length) * 10;
  });
  if (legal.tax_id) pdf.text(`Tax ID: ${legal.tax_id}`, sellerX, sellerCursor);

  const party = context.party || document.party || {};
  const partyLines = partyAddressLines(party);
  let partyCursor = detailY;
  partyLines.forEach((line) => {
    const wrapped = pdf.splitTextToSize(line, 225);
    pdf.text(wrapped, billToX, partyCursor);
    partyCursor += Math.max(1, wrapped.length) * 10;
  });
  if (party.tax_id) pdf.text(`Tax ID: ${text(party.tax_id)}`, billToX, partyCursor);
  ...
  const paidToBank = paymentEvidence?.bank || null;
  ...
  const metaTop = 224;
  const metaColumnWidth = (right - left) / metaRows.length;
  pdf.setFontSize(7.5);
  metaRows.forEach(([label, value], index) => {
    const x = left + index * metaColumnWidth;
    pdf.setTextColor(120, 120, 120);
    pdf.setFont("helvetica", "normal");
    pdf.text(label, x, metaTop);
    pdf.setTextColor(25, 25, 25);
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(9);
    pdf.text(value, x, metaTop + 14);
    pdf.setFontSize(7.5);
  });
  ...
  const receiptPaymentLines = receiptMode
    ? [
        ...
        paidToBank?.bank_name ? `Paid to bank: ${paidToBank.bank_name}` : null,
        paidToBank?.account_name ? `Paid to account name: ${paidToBank.account_name}` : null,
        paidToBank?.account_number ? `Paid to account: ${paidToBank.account_number}` : null,
  ...
  if (payments.length) {
    const paymentBoxHeight = 31 + payments.length * 11;
    if (y + paymentBoxHeight > pageHeight - 88) {
      pdf.addPage();
      y = 70;
    }
    const paymentTop = Math.max(y + 10, pageHeight - 82 - paymentBoxHeight);
    pdf.setFillColor(249, 249, 249);
    pdf.setDrawColor(232, 232, 232);
    pdf.roundedRect(left, paymentTop, 270, paymentBoxHeight, 5, 5, "FD");
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(8);
    pdf.setTextColor(95, 95, 95);
    pdf.text(receiptMode ? "PAYMENT RECEIVED" : "BANK / PAYMENT DETAILS", left + 12, paymentTop + 17);
    pdf.setFont("helvetica", "normal");
    pdf.setTextColor(45, 45, 45);
    payments.forEach((line, index) => pdf.text(line, left + 12, paymentTop + 32 + index * 11));
  }

  // Legal footer is fixed to the physical page bottom and never depends on body height.
  ...
  pdf.text(footer || "Generated by Avantiqo", pageWidth / 2, pageHeight - 43, { align: "center" });
