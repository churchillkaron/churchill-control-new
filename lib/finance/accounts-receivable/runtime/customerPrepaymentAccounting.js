...
export async function getCustomerPrepaymentAccountingReadiness({
  ...
  // CUSTOMER_PAYMENT_RECEIVED belongs here because the Shopify refund path posts with it.
  //
  // a789686d6 ("Align prepayment readiness with active posting paths") removed it as unused, but
  // ShopifyFinanceLifecycleRuntime imports prepareCustomerPrepaymentJournal from this module and calls it with
  // eventType: "CUSTOMER_PAYMENT_RECEIVED" on the refund branch. So it is on an active posting path, and
  // dropping it made readiness report ready: true for an entity that cannot actually post.
  //
  // The cost of that gap is a partial write rather than a clean refusal: the refund path creates the customer
  // credit row first, then prepares the journal, so a missing rule throws after the credit note exists and
  // before anything balances it. Readiness exists to catch the missing rule while nothing has been written yet.
  //
  // Anything posted through this module belongs in this list. If a rule is removed from it, check the callers
  // for the eventType first -- the list is a claim about what the callers post, not about what this file uses.
  const requiredRules = [
