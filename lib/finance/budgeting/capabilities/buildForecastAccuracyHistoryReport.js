...
function absoluteErrorChange(latest, previous, metricKey) {
  const latestError = latest?.comparisons?.[metricKey]?.absolute_error_percent;
  const previousError = previous?.comparisons?.[metricKey]?.absolute_error_percent;
  ...
  return Number((Number(latestError) - Number(previousError)).toFixed(2));
}

function buildTrend(latest, previous) {
  const available = Boolean(latest && previous);

  return {
    available,
    convention:
      "Latest final absolute-error percentage minus previous final absolute-error percentage; negative points indicate improved accuracy",
    latest_final_period_id: latest?.period_id || null,
    latest_final_period_name: latest?.period_name || null,
    previous_final_period_id: previous?.period_id || null,
    previous_final_period_name: previous?.period_name || null,
    revenue_absolute_error_change_points: available
      ? absoluteErrorChange(latest, previous, "revenue")
      : null,
    cogs_absolute_error_change_points: available
      ? absoluteErrorChange(latest, previous, "cogs")
      : null,
    operating_expenses_absolute_error_change_points: available
      ? absoluteErrorChange(latest, previous, "operating_expenses")
      : null,
    operating_profit_absolute_error_change_points: available
      ? absoluteErrorChange(latest, previous, "operating_profit")
      : null,
  };
}

function errorChangeRows(trend) {
  if (!trend.available) {
    return [{
      label: "Trend",
  ...
  const valueByMetric = {
    revenue: trend.revenue_absolute_error_change_points,
    cogs: trend.cogs_absolute_error_change_points,
    operating_expenses: trend.operating_expenses_absolute_error_change_points,
    operating_profit: trend.operating_profit_absolute_error_change_points,
  };

  return TREND_METRICS.map(([key, label]) => {
    const delta = valueByMetric[key];
    return {
      label: `${label} absolute-error change`,
      value: delta === null
        ? "Unavailable"
        : `${Number(delta).toFixed(2)} pts (negative means improved accuracy)`,
    };
  });
}
...
export default async function buildForecastAccuracyHistoryReport({
  ...
  const trend = buildTrend(latestFinal, previousFinal);
  ...
  const document = {
    ...
    sections: [
      ...
      { title: "Latest Final-period Change", rows: errorChangeRows(trend) },
      { title: "Approved Forecast Period History", rows: periodHistoryRows(history) },
      {
        ...
        rows: [
          ...
          { label: "Trend Convention", value: trend.convention },
        ],
      },
    ],
