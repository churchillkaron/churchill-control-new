...
function persistedCaseItem(caseRow) {
  return {
...
export default async function buildForecastExceptionOversightReport({
  ...
  const derivedActive = (exceptionsReport.exceptions || []).map(item =>
    activeQueueRow(item, caseByOccurrence.get(item.occurrence_key) || null, today)
  );
  const derivedOccurrenceKeys = new Set(
    (exceptionsReport.exceptions || []).map(item => item.occurrence_key).filter(Boolean)
  );
  const persistedOnlyUnresolved = (cases || [])
    .filter(row => row.status !== "RESOLVED" && !derivedOccurrenceKeys.has(row.occurrence_key))
    .map(row => activeQueueRow(persistedCaseItem(row), row, today));
  const active = [...derivedActive, ...persistedOnlyUnresolved];
  ...
  const overrideReviews = unresolved.filter(item => item.exception_type === "APPROVAL_OVERRIDE_REVIEW");
  ...
  const summary = {
    ...
    override_review_unresolved: overrideReviews.length,
    override_review_attention: overrideReviews.filter(item => item.escalation_level === "ATTENTION").length,
    override_review_escalated: overrideReviews.filter(item => item.escalation_level === "ESCALATED").length,
    override_review_critical: overrideReviews.filter(item => item.escalation_level === "CRITICAL").length,
    ...
    sections: [
      {
        ...
        rows: [
          ...
          { label: "Override Reviews Unresolved", value: String(summary.override_review_unresolved) },
          { label: "Override Reviews Escalated", value: String(summary.override_review_escalated + summary.override_review_critical) },
      ...
      { title: "Unresolved Aging", rows: documentRows(queues.unresolved_aging, "Unresolved Exceptions") },
      {
        ...
        rows: [
          ...
          { label: "Override Reviews", value: "Persisted exceptional-approval review cases remain in canonical oversight until resolved, even though they are not derived forecast measurement exceptions" },
