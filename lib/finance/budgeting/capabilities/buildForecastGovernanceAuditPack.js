...
function governanceComplete(review, closureEvent) {
  if (!review) return false;
  return (
    review.status === "RESOLVED" &&
    Boolean(review.assigned_to) &&
    Boolean(review.acknowledged_by) &&
    Boolean(review.acknowledged_at) &&
    Boolean(review.resolved_by) &&
    Boolean(review.resolved_at) &&
    Boolean(String(review.resolution_note || "").trim()) &&
    Boolean(closureEvent)
  );
}
...
export default async function buildForecastGovernanceAuditPack({
  ...
  const versionById = mapById(allVersions);
  ...
  const records = versions.map(version => {
    ...
    return {
      ...
      governance_complete: governanceComplete(review, closureEvent),
    };
  });
  ...
  const summary = {
    ...
    closure_audit_missing: records.filter(row => row.review?.status === "RESOLVED" && !row.evidence.protected_closure_event).length,
