...
function controlStatusForVersion(row, closureByCase = new Map()) {
  const review = row.governance?.approval_override_review || null;
  const closureEvent = review?.id ? closureByCase.get(String(review.id)) || null : null;
  return forecastGovernanceControlStatus({ review, closureEvent });
}

function approvalSummary(versions, closureByCase) {
  ...
  const overrideVersions = versions.filter(row => row.approval_override === true);
  const overrideReviews = overrideVersions
    .map(row => row.governance?.approval_override_review || null)
    .filter(Boolean);
  const overrideControls = overrideVersions.map(row => controlStatusForVersion(row, closureByCase));
  ...
  return {
    ...
    override_review_total: overrideReviews.length,
    override_review_open: countBy(overrideReviews, row => row.status === "OPEN"),
    override_review_acknowledged: countBy(overrideReviews, row => row.status === "ACKNOWLEDGED"),
    override_review_resolved: countBy(overrideReviews, row => row.status === "RESOLVED"),
    override_review_overdue: countBy(
      overrideReviews,
      row => row.status !== "RESOLVED" && row.due_date && String(row.due_date) < new Date().toISOString().slice(0, 10)
    ),
    override_review_unassigned: countBy(
      overrideReviews,
      row => row.status !== "RESOLVED" && !row.assigned_to
    ),
    override_governance_complete: countBy(overrideControls, row => row.governance_complete),
    override_governance_incomplete: countBy(overrideControls, row => !row.governance_complete),
    override_closure_audit_missing: countBy(
      overrideControls,
      row => row.missing_controls.includes("CLOSURE_AUDIT_MISSING")
    ),
...
function compactVersion(row, closureByCase = new Map()) {
  const overrideEvidence = row.governance?.approval_override || null;
  const review = row.governance?.approval_override_review || null;
  const controlStatus = row.approval_override === true
    ? controlStatusForVersion(row, closureByCase)
    : null;
  return {
    id: row.id,
    ...
    approval_override_review: review,
    governance_complete: controlStatus?.governance_complete ?? null,
    governance_missing_controls: controlStatus?.missing_controls || [],
    governance_missing_control_labels: controlStatus?.missing_control_labels || [],
    protected_closure_evidence_at: review?.id
      ? closureByCase.get(String(review.id))?.created_at || null
      : null,
  };
}
...
export default async function buildForecastGovernanceDashboardReport({ organizationId, limit = 12 } = {}) {
  ...
  const reviewCaseIds = versions
    .filter(row => row.approval_override === true)
    .map(row => row.governance?.approval_override_review?.id)
    .filter(Boolean);
  const closureByCase = new Map();
  if (reviewCaseIds.length) {
    const { review_events: reviewEvents } = await listForecastGovernanceAuditEvents({
      organizationId,
      reviewCaseIds,
    });
    for (const event of reviewEvents || []) {
      if (event.action_type !== "FORECAST_OVERRIDE_REVIEW_CLOSED" || !event.entity_id) continue;
      closureByCase.set(String(event.entity_id), event);
    }
  }
  ...
  const approvals = approvalSummary(versions, closureByCase);
  ...
  const currentDrafts = versions.filter(row => row.status === "DRAFT").slice(0, 25).map(row => compactVersion(row, closureByCase));
  const approvedOverrides = versions
    .filter(row => row.status === "APPROVED" && row.approval_override === true)
    .slice(0, 25)
    .map(row => compactVersion(row, closureByCase));
  const overrideReviews = versions
    .filter(row => row.approval_override === true && row.governance?.approval_override_review)
    .sort((left, right) => String(right.governance?.approval_override_review?.updated_at || "").localeCompare(String(left.governance?.approval_override_review?.updated_at || "")))
    .slice(0, 50)
    .map(row => compactVersion(row, closureByCase));
    ...
    sections: [
      {
        ...
        rows: [
          ...
          { label: "Governance-complete Overrides", value: String(summary.override_governance_complete) },
          { label: "Governance-incomplete Overrides", value: String(summary.override_governance_incomplete) },
          { label: "Open Override Reviews", value: String(summary.override_review_open + summary.override_review_acknowledged) },
          { label: "Overdue Override Reviews", value: String(summary.override_review_overdue) },
      ...
      {
        title: "Override Review Governance",
        rows: [
          { label: "Review Cases", value: String(summary.override_review_total) },
          { label: "Open", value: String(summary.override_review_open) },
          { label: "Acknowledged", value: String(summary.override_review_acknowledged) },
          { label: "Resolved", value: String(summary.override_review_resolved) },
          { label: "Governance Complete", value: String(summary.override_governance_complete) },
          { label: "Governance Incomplete", value: String(summary.override_governance_incomplete) },
          { label: "Protected Closure Evidence Missing", value: String(summary.override_closure_audit_missing) },
          { label: "Overdue", value: String(summary.override_review_overdue) },
          { label: "Unassigned", value: String(summary.override_review_unassigned) },
        ],
      },
      {
        ...
        rows: [
          ...
          { label: "Exceptional Override", value: "Requires Finance accounting management plus Finance configuration management, a non-empty reason, and audit evidence" },
          { label: "Governance Complete", value: "Requires a resolved review, assigned owner, acknowledgement actor/time, resolution actor/time/evidence, and protected FORECAST_OVERRIDE_REVIEW_CLOSED evidence" },
          { label: "Closure Evidence Protection", value: "Dedicated closure evidence is database-protected against normal UPDATE and DELETE operations" },
