...
export default async function buildRevenueForecast({
  ...
  try {
    ...
    if (revenueAccountIds.length === 0) {
      return emptyForecast("No revenue accounts are configured for this scope");
    }
    ...
    if (!latestRevenueDate) {
      return emptyForecast("No posted revenue history is available for this scope");
    }
    ...
    const averageDailyRevenue =
      observationDays > 0
        ...
        : 0;
    const forecastReady = observationDays >= MIN_OBSERVATION_DAYS;

    return {
      success: true,
      forecast_ready: forecastReady,
      forecast_reason: forecastReady
        ? null
        : `At least ${MIN_OBSERVATION_DAYS} calendar days of observed revenue history are required`,
      observation_start_date: firstObservedDate,
      observation_end_date: latestRevenueDate,
