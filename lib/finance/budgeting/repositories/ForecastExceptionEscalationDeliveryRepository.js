...
export async function listForecastExceptionDeliveryOrganizations() {
  const population = await fetchCompleteFinancePopulation({
    label: "Forecast exception delivery organizations",
    buildQuery: (from, to) => supabaseAdmin.from("finance_forecast_exception_cases")
      .select("id,organization_id")
      .neq("status", "RESOLVED")
      .order("id", { ascending: true })
      .range(from, to),
  });
  return [...new Set((population.rows || []).map(row => row.organization_id).filter(Boolean))];
}
