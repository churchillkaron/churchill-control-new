...
function reviewOccurrenceKey(version) {
  if (!version?.entity_id || !version?.id) return null;
  return `${version.entity_id}:APPROVAL_OVERRIDE_REVIEW:${version.id}`;
}

async function attachGovernanceEvidence({ organizationId, versions }) {
  const rows = versions || [];
  const versionIds = rows.map(version => version?.id).filter(Boolean);
  if (!versionIds.length) return rows;

  const eventPages = await Promise.all(
    chunks(versionIds).map(async ids => {
  ...
  const reviewKeys = rows.map(reviewOccurrenceKey).filter(Boolean);
  const reviewPages = await Promise.all(
    chunks(reviewKeys).map(async keys => {
      const { data, error } = await supabaseAdmin
        .from("finance_forecast_exception_cases")
        .select("id, entity_id, occurrence_key, status, assigned_to, assigned_to_name, due_date, acknowledged_by, acknowledged_by_name, acknowledged_at, resolved_by, resolved_by_name, resolved_at, resolution_note, escalation_level, escalation_reason, escalation_changed_at, escalation_revision, revision, created_at, updated_at")
        .eq("organization_id", organizationId)
        .eq("exception_type", "APPROVAL_OVERRIDE_REVIEW")
  ...
  const reviewByOccurrence = new Map(
    reviewPages.flat().map(row => [row.occurrence_key, row])
  );
  const evidenceByVersion = new Map();

  for (const event of eventPages.flat()) {
    ...
    if (event.action_type === "FORECAST_SCENARIO_VERSION_DRAFT_CREATED") {
      ...
      evidence.previous_approved_version_id =
        event.metadata?.previous_approved_version_id || null;
    } else if (event.action_type === "FORECAST_SCENARIO_VERSION_SUPERSEDED") {
      evidence.superseded = actor;
      evidence.superseded_by_version_id =
        event.metadata?.superseded_by_version_id || null;
    } else if (event.action_type === "FORECAST_SCENARIO_VERSION_APPROVAL_OVERRIDE") {
      evidence.approval_override = {
        ...actor,
        reason:
          event.new_data?.approval_override_reason ||
          event.metadata?.approval_override_reason ||
          null,
        blockers:
          event.new_data?.approval_policy_blockers ||
          event.metadata?.approval_policy_blockers ||
          [],
      };
    }
  ...
  return rows.map(version => {
    ...
    const review = reviewByOccurrence.get(reviewOccurrenceKey(version)) || null;
    return {
      ...version,
      governance: {
        ...evidence,
        ...(review ? { approval_override_review: review } : {}),
      },
    };
  });
}
...
export async function listForecastScenarioVersions({
  ...
  return await attachGovernanceEvidence({
    organizationId,
    versions: data || [],
  });
}

export async function listForecastScenarioVersionsForOrganization({
  organizationId,
} = {}) {
  ...
  return await attachGovernanceEvidence({ organizationId, versions: rows });
}
...
export async function createForecastScenarioVersion({
  ...
  createdBy = null,
  performedByName = "Authenticated User",
}) {
  const { data, error } = await supabaseAdmin.rpc(
    "finance_create_forecast_scenario_version_draft",
    {
...
export async function overrideForecastScenarioVersionApproval({
  organizationId,
  versionId,
  approvedBy,
  performedByName = "Authenticated User",
  ...
  const { data, error } = await supabaseAdmin.rpc(
    "finance_override_forecast_scenario_version_approval",
    {
      p_organization_id: organizationId,
      p_version_id: versionId,
      p_approved_by: approvedBy,
