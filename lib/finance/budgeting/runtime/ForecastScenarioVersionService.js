...
import {
  ...
  overrideForecastScenarioVersionApproval,
...
export async function overrideForecastScenarioVersionApprovalCommand(input = {}) {
  const organizationId = required(input.organizationId || input.organization_id, "organization_id");
  const versionId = required(input.versionId || input.version_id, "version_id");
  const approvedBy = required(input.approvedBy || input.approved_by, "approved_by");
  const overrideReason = required(input.overrideReason || input.override_reason, "approval override reason");
  const version = await overrideForecastScenarioVersionApproval({
    organizationId,
    versionId,
    approvedBy,
    overrideReason,
