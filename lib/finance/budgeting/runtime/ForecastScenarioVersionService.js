...
function normalizeScenarioKind(value) {
  ...
  if (!SCENARIO_KINDS.has(kind)) {
    throw new Error("Invalid scenario_kind");
  }
  return kind;
}
...
async function generateSnapshot({
  ...
  return await buildForecastScenarios({
...
export async function createForecastScenarioVersionDraft(input = {}) {
  ...
  const periodId = required(
    input.periodId || input.period_id,
    "period_id"
  );
  const scenarioKind = normalizeScenarioKind(
    input.scenarioKind || input.scenario_kind
  );
  ...
  if (!snapshot?.success) {
    throw new Error(snapshot?.error || "Forecast scenario generation failed");
  }
  ...
  const version = await createForecastScenarioVersion({
    ...
    budgetAvailable:
      scenarioKind === "SCENARIOS_VS_BUDGET"
        ? snapshot.budget_available === true
        : null,
    budgetComplete:
      scenarioKind === "SCENARIOS_VS_BUDGET"
        ? snapshot.budget_complete === true
        : null,
...
export async function listForecastScenarioVersionsCommand(input = {}) {
  ...
  const scenarioKind = input.scenarioKind || input.scenario_kind
    ? normalizeScenarioKind(input.scenarioKind || input.scenario_kind)
    : null;

  const versions = await listForecastScenarioVersions({
    organizationId,
    entityId,
    periodId,
    scenarioKind,
...
export async function approveForecastScenarioVersionCommand(input = {}) {
  ...
  const approvedBy = required(
    input.approvedBy || input.approved_by,
    "approved_by"
  );
