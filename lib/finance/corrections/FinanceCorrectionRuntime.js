...
async function validateAccounts({ organizationId, entityId, lines }) {
  ...
  const { data, error } = await supabaseAdmin.from("chart_of_accounts")
    ...
    .eq("organization_id", organizationId).eq("entity_id", entityId).in("id", ids);
  if (error) throw error;
  const found = new Set((data || []).filter((row) => row.is_active !== false).map((row) => row.id));
  const missing = ids.filter((id) => !found.has(id));
  if (missing.length) throw new Error("Correction journal contains inactive or out-of-scope accounts");
  return data || [];
}

async function loadCase({ accountingFirmId, correctionId }) {
  const { data, error } = await supabaseAdmin.from("finance_approval_requests")
    .select("*").eq("id", correctionId).eq("organization_id", accountingFirmId)
    .eq("document_type", CORRECTION_DOCUMENT_TYPE).maybeSingle();
  if (error) throw error;
...
export async function listFinanceCorrections({ organizationId, entityId, periodId }) {
  let query = supabaseAdmin.from("finance_approval_requests").select("*")
    .eq("organization_id", organizationId).eq("document_type", CORRECTION_DOCUMENT_TYPE)
...
export async function listCorrectionAccounts({ organizationId, entityId }) {
  const { data, error } = await supabaseAdmin.from("chart_of_accounts")
    ...
    .order("account_code", { ascending: true }).limit(1000);
  if (error) throw error;
  return data || [];
}
...
export async function createFinanceCorrection({ accountingFirmId, clientOrganizationId, entityId, periodId, accountId, requestedBy, currency }) {
  ...
  const metadata = {
    ...
    journal_draft: { posting_date: health.context?.as_of || health.context?.period_end, document_date: health.context?.as_of || health.context?.period_end, journal_type: "CORRECTION", reference: null, description: `Correction for ${exception.account_code || exception.account_name}`, currency_code: currency || health.context?.currency || null, exchange_rate: 1, lines: [] },
...
export async function saveFinanceCorrection({ accountingFirmId, correctionId, actorId, resolutionMode, treatment, journalDraft, documentIds, submit = false }) {
  ...
  const metadata = {
    ...
    treatment: { ...(current.metadata?.treatment || {}), ...(treatment || {}) },
    journal_draft: draft,
    evidence: { ...(current.metadata?.evidence || {}), document_ids: [...new Set((documentIds || current.metadata?.evidence?.document_ids || []).map(clean).filter(Boolean))] },
  };
...
export async function recheckFinanceCorrection({ accountingFirmId, correctionId }) {
  ...
  const health = await loadFinanceAccountHealthRuntime({ organizationId: meta.client_organization_id, entityId: current.entity_id, periodId: current.period_id, currency: current.currency_code });
  const accountId = meta.exception?.account_id;
  const result = health.health?.accounts?.find((row) => row.account_id === accountId) || null;
  const resolved = !result || !OPEN_EXCEPTION_STATES.has(result.state);
  const recheck = { checked_at: new Date().toISOString(), prior_state: meta.exception?.state || null, resulting_state: result?.state || "NOT_PRESENT", resolved, reason: result?.reason || "Original structural exception is no longer surfaced." };
  const { data, error } = await supabaseAdmin.from("finance_approval_requests").update({ metadata: { ...meta, recheck }, updated_at: new Date().toISOString() }).eq("id", current.id).eq("organization_id", accountingFirmId).select("*").single();
  if (error) throw error;
  return { correction: data, recheck, health };
}
...
export async function postFinanceCorrection({ accountingFirmId, correctionId, actorId }) {
  ...
  const draft = current.metadata?.journal_draft || {};
  const lines = normalizeLines(draft.lines);
  validateJournalIntegrity(lines);
  await validateAccounts({ organizationId: current.metadata?.client_organization_id, entityId: current.entity_id, lines });

  const posted = await postJournalEntrySafe({
    organizationId: current.metadata?.client_organization_id,
    ...
    reference: draft.reference || `Correction ${current.id.slice(0, 8)}`,
    sourceModule: "finance_correction",
    sourceDocument: CORRECTION_DOCUMENT_TYPE,
    sourceDocumentId: current.id,
    description: draft.description || current.metadata?.treatment?.summary || "Accounting correction",
    currencyCode: draft.currency_code || current.currency_code,
    exchangeRate: number(draft.exchange_rate || 1),
    lines,
    createdBy: actorId || null,
    idempotencyKey: `finance-correction:${current.id}`,
  });
  const journalId = posted?.journal?.id || posted?.ledger?.journalEntryId || null;
  const metadata = { ...(current.metadata || {}), posting: { journal_entry_id: journalId, posted_at: new Date().toISOString(), idempotency_key: `finance-correction:${current.id}` } };
  const { error } = await supabaseAdmin.from("finance_approval_requests").update({ status: "POSTED", metadata, updated_at: new Date().toISOString() }).eq("id", current.id).eq("organization_id", accountingFirmId);
  if (error) throw error;
  const rechecked = await recheckFinanceCorrection({ accountingFirmId, correctionId: current.id });
