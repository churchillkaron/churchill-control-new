...
function active(row = {}) {
  ...
  return !["INACTIVE", "ARCHIVED", "DISABLED", "SUSPENDED", "TERMINATED"].includes(
...
async function requireEntity({ organizationId, entityId }) {
  if (!entityId) throw new Error("Active Legal Entity required");
  const { data, error } = await supabaseAdmin
    .from("legal_entities")
    .select("*")
    .eq("organization_id", organizationId)
...
async function requireResponsibleOwner({ organizationId, staffAccountId }) {
  if (!staffAccountId) return null;
  ...
  if (!staff || !active(staff)) {
    throw new Error("Responsible Owner must be an active staff member");
  }
  ...
  if (
    ...
    throw new Error("Responsible Owner is outside organisation scope");
  }
  if (membership && !active(membership)) {
    throw new Error("Responsible Owner membership is inactive");
  }
  return staff;
}
...
async function requireParent({
  ...
  if (!current) {
    throw new Error("Parent Cost Centre is outside the active Legal Entity");
  }
...
export async function upsertFinanceCostCenter({
  ...
  const entityId = text(contextEntityId || existing?.entity_id);
  ...
  if (
    ...
    throw new Error("Cost Centre belongs to another active Legal Entity");
  }
  ...
  const isActive = existing ? existing.is_active !== false : true;

  await requireEntity({ organizationId, entityId });
  await requireDepartment({ organizationId, entityId, departmentId });
  const responsibleOwner = await requireResponsibleOwner({
    organizationId,
    staffAccountId: responsibleOwnerId,
  });
  await requireParent({
    organizationId,
    entityId,
  ...
  if (existing && upper(existing.code) !== code) {
    const usage = await countAccountingUsage({
      organizationId,
      costCenterId: existing.id,
    });
    if (usage > 0) {
      throw new Error("Cost Centre Code cannot change after accounting use");
    }
  }
  ...
  const record = {
    ...
    manager_user_id: responsibleOwnerId,
    manager:
      text(responsibleOwner?.full_name) ||
      text(responsibleOwner?.name) ||
      text(responsibleOwner?.email),
    description,
    is_active: isActive,
