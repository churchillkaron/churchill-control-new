...
import { fetchCompleteFinancePopulation } from "../data/fetchCompleteFinancePopulation.js";
...
async function loadPairRates({ organizationId, entityId, leftCurrency, rightCurrency, effectiveDate }) {
  const pairFilter = [
    `and(base_currency.eq.${leftCurrency},quote_currency.eq.${rightCurrency})`,
    `and(from_currency.eq.${leftCurrency},to_currency.eq.${rightCurrency})`,
    `and(base_currency.eq.${rightCurrency},quote_currency.eq.${leftCurrency})`,
    `and(from_currency.eq.${rightCurrency},to_currency.eq.${leftCurrency})`,
  ].join(",");
  const population = await fetchCompleteFinancePopulation({
    label: `Finance exchange rates ${leftCurrency}/${rightCurrency}`,
  ...
  return (population.rows || []).filter((row) => activeRecord(row) && (!row.entity_id || String(row.entity_id) === String(entityId)));
}
...
export async function resolveFinanceExchangeRate({
  ...
  let activeRates;
  try {
    activeRates = await loadPairRates({ organizationId, entityId, leftCurrency: currency, rightCurrency: functionalCurrency, effectiveDate: postingDate });
  } catch (error) {
    if (MISSING_RELATION_CODES.has(String(error?.code || ""))) {
      throw new Error(`No Finance exchange-rate table is available for ${currency}/${functionalCurrency}`);
    }
    throw error;
  }
