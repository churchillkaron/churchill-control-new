...
function authHeaders({ secret, config, token }) {
  ...
  const serverKey = text(credentials.server_key || config.server_key);
  if (serverKey) headers[text(config.server_key_header || "x-server-key")] = serverKey;
  return headers;
}
...
const CertifiedEtaxRestProvider = {
  ...
  async verifyCredential({ secret, config = {} }) {
    const healthPath = text(config.health_path);
    if (healthPath) {
      const token = await providerToken({ secret, config });
      const response = await fetch(joinUrl(config.base_url, healthPath), {
        method: text(config.health_method || "GET").toUpperCase(),
        headers: authHeaders({ secret, config, token }),
        cache: "no-store",
      });
      await jsonResponse(response);
      return { verified: true, provider_status: "VERIFIED", verification_mode: "HEALTH_ENDPOINT" };
    }
    if (text(config.auth_path)) {
      await providerToken({ secret, config });
      return { verified: true, provider_status: "VERIFIED", verification_mode: "AUTH_HANDSHAKE" };
    }
    return { verified: false, provider_status: "CONFIGURED_UNVERIFIED", verification_mode: "NO_NON_MUTATING_PROVIDER_CHECK", reason: "Configure a read-only health endpoint or provider auth endpoint to verify without transmitting a document." };
  },
