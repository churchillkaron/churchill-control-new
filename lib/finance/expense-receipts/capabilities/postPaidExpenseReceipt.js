...
export const manifest = defineCapability({
  ...
  reversible: false,
  contextScope: "entity",
  operatorVerification: { capability_key: "finance.expense_receipts.read", payload_from_result: { receipt_id: ["receipt_id", "id"] }, derivation: "declared_result_bound_receipt_verifier" },
  inputSchema: {
    type: "object",
    properties: {
...
export async function execute({ context, payload = {} }) {
  ...
  if (result.error) throw attachActionIdentityEvidence(
    new Error(`Paid expense receipt posting failed: ${result.error.message}`),
  ...
  return {
    success: true,
    ...(result.data || {}),
    receipt_id: result.data?.receipt_id || result.data?.id || receiptId,
