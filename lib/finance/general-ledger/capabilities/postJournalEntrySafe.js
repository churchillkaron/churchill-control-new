...
export async function postJournalEntrySafe({
  ...
  const resolvedCurrency = String(currencyCode || "")
    .trim()
    .toUpperCase();
  ...
  const { data, error } = await supabaseAdmin.rpc(
    "finance_post_journal_atomic_rpc",
    {
      p_organization_id: organizationId,
      p_entity_id: entityId,
      p_posting_date: postingDate,
      p_document_date: documentDate || postingDate,
      p_journal_type: journalType,
      p_reference: reference || null,
      p_source_module: sourceModule || null,
      p_source_document: sourceDocument || null,
  ...
  if (error) {
    throw new Error(
      `Atomic journal posting failed: ${error.message}`
    );
  }
