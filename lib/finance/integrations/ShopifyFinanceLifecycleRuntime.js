...
import { prepareCustomerPrepaymentJournal } from "@/lib/finance/accounts-receivable/runtime/customerPrepaymentAccounting";
...
async function issueRefundCreditTransaction({
  event,
  store,
  salesOrder,
  invoice,
  ...
  if (amount <= 0.005) return { skipped: true, reason: "NO_INVOICED_REFUND_AMOUNT" };
  ...
  if (outstanding > 0.005) {
    return {
      skipped: true,
      reason: "BLOCKED_REFUND_REQUIRES_SETTLED_INVOICE",
      amount,
    };
  }

  const creditDate = isoDate(
    transaction.processed_at || transaction.created_at || refund?.processed_at || refund?.updated_at || refund?.created_at
  );
  const creditNoteId = deterministicUuid(
    `shopify-credit-note:${event.payload.connection_id}:${transaction.id}`
  );
  const creditResult = await supabaseAdmin.rpc("finance_issue_customer_credit_note_idempotent", {
    ...
    p_amount: amount,
    p_reason: refund?.note || `Shopify refund ${transaction.id}`,
    p_created_by: null,
    p_idempotency_key: `shopify-credit-note:${event.payload.connection_id}:${transaction.id}`,
    p_prefix: "CN",
  });
  ...
  const currency = text(transaction.currency || refund?.currency || salesOrder.currency_code).toUpperCase();
  const rate = await exchangeRate({ event, store, currency, date: creditDate });

  const receiptJournal = await prepareCustomerPrepaymentJournal({
    organizationId: event.organization_id,
    entityId: store.entity_id,
    partyId: salesOrder.party_id,
    eventType: "CUSTOMER_PAYMENT_RECEIVED",
    sourceId: deterministicUuid(`shopify-refund-journal:${event.payload.connection_id}:${transaction.id}`),
    accountingDate: creditDate,
    amount,
  ...
  const refundId = deterministicUuid(
    `shopify-credit-refund:${event.payload.connection_id}:${transaction.id}`
  );
  const refundResult = await supabaseAdmin.rpc("finance_refund_customer_credit_idempotent", {
    ...
    p_idempotency_key: `shopify-credit-refund:${event.payload.connection_id}:${transaction.id}`,
  });
...
async function processRefunds({ event, store, salesOrder, invoice, lifecycle, bank }) {
  ...
  for (const item of successfulRefundTransactions(lifecycle)) {
    ...
    let invoicedRefund = null;

    if (remaining > 0.005) {
      invoicedRefund = invoice
        ? await issueRefundCreditTransaction({
            event,
            store,
            salesOrder,
            invoice,
        ...
        : {
            skipped: true,
            reason: "BLOCKED_REFUND_REQUIRES_INVOICE",
            amount: remaining,
          };
    }
...
async function postFinance({ event, store, salesOrder, order, lifecycle }) {
  ...
  const refundedPrepayments = refunds.filter(
    (row) => row?.deposit_refund && !row.deposit_refund.skipped
  ).length;
  const postedCreditRefunds = refunds.filter(
    (row) => row?.invoiced_refund && !row.invoiced_refund.skipped
  ).length;
  const blockedRefunds = refunds.filter(
    (row) => row?.invoiced_refund?.skipped || row?.deposit_refund?.reason === "BLOCKED_REFUND_SOURCE_MAPPING"
  );
  ...
  if (invoice && blockedRefunds.length === 0) status = "RECONCILED";
  ...
  return {
    ...
    posted_refunds: refundedPrepayments + postedCreditRefunds,
