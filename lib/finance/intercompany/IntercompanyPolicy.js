...
async function resolveConfiguredRate({
  ...
  const pairFilter = [
    `and(base_currency.eq.${baseCurrency},quote_currency.eq.${quoteCurrency})`,
    `and(from_currency.eq.${baseCurrency},to_currency.eq.${quoteCurrency})`,
    `and(base_currency.eq.${quoteCurrency},quote_currency.eq.${baseCurrency})`,
    `and(from_currency.eq.${quoteCurrency},to_currency.eq.${baseCurrency})`,
  ].join(",");
  let population;
  try {
    population = await fetchCompleteFinancePopulation({
      label: `Intercompany exchange rates ${baseCurrency}/${quoteCurrency}`,
  ...
  const activeRates = (population.rows || []).filter(activeRecord);
