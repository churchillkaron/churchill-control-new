...
async function findLegalEntityConflict({ organizationId, recordId, field, value }) {
  if (!cleanText(value)) return null;
  let query = supabaseAdmin.from("legal_entities")
    .select("id")
    .eq("organization_id", organizationId)
    .ilike(field, escapeLikeLiteral(value))
    .limit(1);
...
async function assertUniqueFields({ organizationId, candidate, recordId }) {
  const [codeConflict, registrationConflict, taxConflict] = await Promise.all([
    findLegalEntityConflict({ organizationId, recordId, field: "code", value: candidate.code }),
    findLegalEntityConflict({ organizationId, recordId, field: "registration_number", value: candidate.registration_number }),
    findLegalEntityConflict({ organizationId, recordId, field: "tax_id", value: candidate.tax_id }),
  ]);
  if (codeConflict) throw new Error("A Legal Entity with this code already exists");
  if (registrationConflict) throw new Error("A Legal Entity with this registration number already exists");
  if (taxConflict) throw new Error("A Legal Entity with this tax registration already exists");
}
