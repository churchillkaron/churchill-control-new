...
export async function recordFinanceClosePackageBaseline({ organizationId, entityId, periodId, closeType }) {
  ...
  const fingerprint = buildFinanceClosePackageFingerprint(snapshot);
  const closeRun = await persistFinanceClosePackageFingerprint({ organizationId, entityId, periodId, closeType, fingerprint });
  return { close_run: closeRun, fingerprint };
}
