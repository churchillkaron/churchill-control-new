...
async function fetchCompleteByIds({ ids, label, buildQuery }) {
  const all = [];
...
async function loadRunsAndWork(grant, engagement) {
  const runPopulation = await fetchCompleteFinancePopulation({
    label: "Client portal engagement runs",
  ...
  const runs = runPopulation.rows || [];
  const runIds = runs.map((row) => row.id);
  const items = runIds.length ? await fetchCompleteByIds({
    ids: runIds,
    label: "Client portal work items",
...
async function loadFilings(grant, engagement) {
  ...
  const population = await fetchCompleteFinancePopulation({
    label: "Client portal tax filings",
    buildQuery: (from, to) => {
  ...
  return (population.rows || []).map((row) => ({
    id: row.id,
...
async function loadMessages(grant, engagement, { markRead = true, displayLimit = 500 } = {}) {
  const safeLimit = Math.max(1, Math.min(Number(displayLimit) || 500, 500));
  const { data, error, count } = await supabaseAdmin.from("accounting_client_portal_messages")
    .select("id,sender_type,sender_name,sender_email,body,read_by_client_at,read_by_firm_at,created_at", { count: "exact" })
    .eq("accounting_firm_id", grant.accounting_firm_id)
    .eq("engagement_id", engagement.id)
    .order("created_at", { ascending: false })
  ...
  if (markRead) {
    ...
    const { error: updateError } = await supabaseAdmin.from("accounting_client_portal_messages")
      .update({ read_by_client_at: stamp })
      .eq("accounting_firm_id", grant.accounting_firm_id)
      .eq("engagement_id", engagement.id)
      .eq("sender_type", "ACCOUNTING_FIRM")
      .is("read_by_client_at", null);
    if (updateError) throw updateError;
  }
  const rows = [...(data || [])].reverse();
  const total = Number(count || 0);
  return { rows, total, displayed: rows.length, has_more: total > rows.length, display_limit: safeLimit };
}
...
export async function loadFinanceClientPortalProjection({ grant, engagement, markMessagesRead = true } = {}) {
  ...
  const [identity, work, filings, messageProjection] = await Promise.all([
  ...
  return {
    ...
    filings,
    messages: messageProjection.rows,
    message_meta: { total: messageProjection.total, displayed: messageProjection.displayed, has_more: messageProjection.has_more, display_limit: messageProjection.display_limit },
  };
}
