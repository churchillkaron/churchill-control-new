...
export async function createFinanceEngagementSignatureRequest({
  ...
  let signature;
  try {
    signature = await createSignatureRequest({
    ...
    if (String(error?.code || "") === "23505" && /document_signature_requests_native_active_slot_unique/i.test(String(error?.message || error?.details || ""))) {
      const conflict = new Error("An active signature request already exists for this document version");
      conflict.status = 409;
      throw conflict;
    }
    throw error;
