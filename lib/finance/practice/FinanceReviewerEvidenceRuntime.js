...
const MAX_REVIEW_POPULATION_ROWS = 50000;
const MAX_CANDIDATE_IDENTIFIERS = 250;
...
function bounded(values, max, label) {
  ...
  if (rows.length > max) {
    throw new Error(`${label} exceeds the reviewer completeness boundary (${rows.length} > ${max}); narrow the governed accounting scope before review`);
  }
  return rows;
}

async function fetchCompleteRows(buildQuery, label, maxRows = MAX_REVIEW_POPULATION_ROWS) {
  const rows = [];
  let pages = 0;
  let from = 0;

  while (from <= maxRows) {
    const requestedRows = Math.min(REVIEW_PAGE_SIZE, maxRows + 1 - from);
    ...
    if (rows.length + page.length > maxRows) {
      throw new Error(`${label} exceeds the ${maxRows}-row reviewer completeness boundary; Avantiqo will not present a silently truncated accounting population`);
    }
  ...
  throw new Error(`${label} reached the ${maxRows}-row reviewer completeness boundary; Avantiqo will not present a silently truncated accounting population`);
}
...
async function loadPeriod(run) {
  ...
  const { data: current, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  if (!current?.start_date) return { current: current || null, previous: null, source: current ? "accounting_periods" : null };
  ...
  if (run.entity_id) previousQuery = previousQuery.eq("entity_id", run.entity_id);

  const { data: previousRows, error: previousError } = await previousQuery;
  if (previousError) throw previousError;
  return { current, previous: previousRows?.[0] || null, source: "accounting_periods" };
}
...
async function loadPreviousWork(accountingFirmId, run, item) {
  ...
  return { run: previousRun, work_item: previousItem, review };
}
...
async function loadJournalEntries(run, candidateIds) {
  if (!candidateIds.length) return { rows: [], direct_pages: 0, document_pages: 0 };
  ...
  const [direct, document] = await Promise.all([
    fetchCompleteRows(
      (from, to) => supabaseAdmin.from("journal_entries").select(fields)
        .eq("organization_id", run.organization_id).in("id", candidateIds)
        .order("posting_date", { ascending: true }).order("id", { ascending: true }).range(from, to),
      "Direct journal evidence population",
      10000,
    ),
    fetchCompleteRows(
      (from, to) => supabaseAdmin.from("journal_entries").select(fields)
        .eq("organization_id", run.organization_id).in("source_document_id", candidateIds)
        .order("posting_date", { ascending: true }).order("id", { ascending: true }).range(from, to),
      "Source-document journal evidence population",
      10000,
    ),
  ]);

  return {
    rows: unique([...direct.rows, ...document.rows])
      .filter((row) => !run.entity_id || row.entity_id === run.entity_id || row.legal_entity_id === run.entity_id),
    direct_pages: direct.pages,
...
async function loadExactLedgerLines(run, period, candidateIds, journalIds) {
  ...
  if (candidateIds.length) {
    const referencePopulation = await fetchCompleteRows((from, to) => {
      let query = supabaseAdmin.from("general_ledger")
      ...
      query = applyLedgerPeriod(query, period, run);
      return query.order("posting_date", { ascending: true }).order("id", { ascending: true }).range(from, to);
    }, "Reference-linked ledger evidence population");
    populations.push({ source: "reference_id", ...referencePopulation });
  }
  if (journalIds.length) {
    const journalPopulation = await fetchCompleteRows((from, to) => {
      let query = supabaseAdmin.from("general_ledger")
        .select("id,account_id,journal_entry_id,reference_type,reference_id,description,debit,credit,currency_code,posting_date,period_id")
        .eq("organization_id", run.organization_id)
        .eq("entity_id", run.entity_id)
      ...
      query = applyLedgerPeriod(query, period, run);
      return query.order("posting_date", { ascending: true }).order("id", { ascending: true }).range(from, to);
    }, "Journal-linked ledger evidence population");
    populations.push({ source: "journal_entry_id", ...journalPopulation });
  }

  return {
    rows: unique(populations.flatMap((population) => population.rows)),
    sources: populations.map((population) => ({
...
async function loadPeriodMovement(run, period, accountIds, label) {
  if (!period?.start_date || !period?.end_date || !accountIds.length) {
    return { rows: [], pages: 0, complete: true };
  }
  return fetchCompleteRows(
    (from, to) => supabaseAdmin.from("general_ledger")
      .select("id,account_id,debit,credit,posting_date")
      .eq("organization_id", run.organization_id)
      .eq("entity_id", run.entity_id)
...
async function loadControlRecords(run, candidateIds) {
  ...
  const [reconciliationResult, filingResult, closeResult] = await Promise.all([
    supabaseAdmin.from("finance_bank_reconciliation_runs")
      ...
      .eq("organization_id", run.organization_id).eq("entity_id", run.entity_id).in("id", candidateIds)
      .order("id", { ascending: true }),
    supabaseAdmin.from("finance_statutory_filings")
      ...
      .eq("organization_id", run.organization_id).eq("entity_id", run.entity_id).in("id", candidateIds)
      .order("id", { ascending: true }),
    supabaseAdmin.from("finance_period_close_runs")
...
export async function buildFinanceReviewerEvidence({ accountingFirmId, runId, workItemId }) {
  ...
  const [runResult, itemResult] = await Promise.all([
    supabaseAdmin.from("accounting_engagement_runs")
      ...
      .eq("id", scopedRunId).eq("accounting_firm_id", firmId).maybeSingle(),
    supabaseAdmin.from("accounting_engagement_work_items")
      ...
      .eq("id", scopedWorkItemId).eq("accounting_firm_id", firmId).eq("run_id", scopedRunId).maybeSingle(),
  ]);
  ...
  const [periodContext, evidenceLinksResult, previousWork] = await Promise.all([
    ...
    supabaseAdmin.from("accounting_work_program_evidence_links")
      ...
      .eq("accounting_firm_id", firmId).eq("run_id", run.id).eq("work_item_id", item.id)
  ...
  if (item.finance_review_item_id) {
    const { data, error } = await supabaseAdmin.from("finance_review_items")
      ...
      .eq("id", item.finance_review_item_id).eq("organization_id", run.organization_id).maybeSingle();
    if (error) throw error;
  ...
  const [documents, notesResult, signoffsResult] = await Promise.all([
    ...
    reviewItem
      ? supabaseAdmin.from("finance_review_notes")
          .select("id,note_type,body,status,assigned_to,created_by,resolved_by,resolved_at,created_at,updated_at")
          .eq("organization_id", run.organization_id).eq("review_item_id", reviewItem.id)
          .order("created_at", { ascending: false })
      : Promise.resolve({ data: [], error: null }),
    reviewItem
      ? supabaseAdmin.from("finance_review_signoffs")
  ...
  if (documentIds.length) {
    const [legacyResult, controlledResult] = await Promise.all([
      supabaseAdmin.from("organization_documents")
        .select("id,destination_record_id,destination_module")
        .eq("organization_id", run.organization_id).in("id", documentIds),
  ...
  const candidateIds = bounded([...candidateMap.keys()], MAX_CANDIDATE_IDENTIFIERS, "Reviewer evidence identifier population");
  const journals = await loadJournalEntries(run, candidateIds);
  const journalEntries = journals.rows;
  const journalIds = bounded(journalEntries.map((row) => row.id), 10000, "Reviewer linked journal population");
  const exactPopulation = await loadExactLedgerLines(run, periodContext.current, candidateIds, journalIds);
  const exactLedgerLines = exactPopulation.rows;
  const accountIds = bounded(exactLedgerLines.map((row) => row.account_id), MAX_LINKED_ACCOUNTS, "Reviewer linked account population");

  const [accountMetadata, currentPopulation, previousPopulation, controlRecords] = await Promise.all([
    loadAccountMetadata(run, accountIds),
    loadPeriodMovement(run, periodContext.current, accountIds, "Current-period ledger movement population"),
    loadPeriodMovement(run, periodContext.previous, accountIds, "Prior-period ledger movement population"),
    loadControlRecords(run, candidateIds),
  ]);
  const currentMovementRows = currentPopulation.rows;
  const previousMovementRows = previousPopulation.rows;
  ...
  const candidateBasis = Object.fromEntries([...candidateMap.entries()].map(([id, bases]) => [id, [...bases]]));

  return {
    run,
    work_item: item,
    review_item: reviewItem,
    period: { source: periodContext.source, current: periodContext.current, previous: periodContext.previous },
    ...
    ledger_impact: {
      ...
      population: {
        ...
        maximum_rows_per_population: MAX_REVIEW_POPULATION_ROWS,
        linked_lines: exactLedgerLines.length,
        linked_sources: exactPopulation.sources,
        current_period_lines: currentMovementRows.length,
        current_period_pages: currentPopulation.pages,
        previous_period_lines: previousMovementRows.length,
        previous_period_pages: previousPopulation.pages,
        journal_entries: journalEntries.length,
        journal_direct_pages: journals.direct_pages,
        journal_document_pages: journals.document_pages,
      },
