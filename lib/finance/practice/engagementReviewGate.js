import { evaluateFinancePartnerLifecycleFreshness } from "@/lib/finance/practice/FinancePartnerLifecycleFreshnessGate";
...
export async function evaluateEngagementReviewGate({ run, workItem }) {
  ...
  const baseResult = evaluateLoadedStage(loaded, stage);
  if (stage.key !== "PARTNER_FINAL_CLEARANCE" || !baseResult.satisfied) return baseResult;

  const evidenceFreshness = await evaluateFinancePartnerLifecycleFreshness({
    run,
    reviewItems: baseResult.review_items,
  });
  return {
    ...baseResult,
    satisfied: evidenceFreshness.satisfied === true,
    blockers: evidenceFreshness.satisfied
      ? baseResult.blockers
      : [...baseResult.blockers, ...evidenceFreshness.blockers].slice(0, 100),
    evidence_freshness: evidenceFreshness,
    evaluated_at: evidenceFreshness.checked_at || baseResult.evaluated_at,
  };
}
...
export async function requireEngagementReviewGate({ run, workItem }) {
  ...
  const error = new Error(
    result.stage === "PARTNER_FINAL_CLEARANCE"
      ? result.evidence_freshness?.satisfied === false
        ? "Partner clearance cannot complete because reviewer evidence is stale or unproven"
