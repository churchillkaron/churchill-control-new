...
export async function planRecurringAccountingCycles({ accountingFirmId, horizonDays = DEFAULT_HORIZON_DAYS, now = new Date() }) {
  ...
  const [documents, signatures, billingProfiles] = await Promise.all([
  ...
  const sortRank = {
    ...
    BLOCKED_ENTITY_CONFIGURATION: 2,
    BLOCKED_PERIOD_CONFIGURATION: 3,
    BLOCKED_YEAR_END_CONFIGURATION: 4,
    TEMPLATE_MISSING: 5,
    ALREADY_EXISTS: 6,
  };
  candidates.sort((a, b) => {
