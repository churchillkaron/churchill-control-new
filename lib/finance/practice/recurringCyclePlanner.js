...
import { evaluatePracticeStaffingReadiness } from "@/lib/finance/practice/FinancePracticeStaffingReadiness";
...
export async function planRecurringAccountingCycles({ accountingFirmId, horizonDays = DEFAULT_HORIZON_DAYS, now = new Date() }) {
  ...
  const contractLinks = engagementIds.length ? await loadCompletePracticeRowsByIds({
    ids: engagementIds,
    label: "Recurring planner engagement contract links",
    buildQuery: (batch, from, to) => supabaseAdmin.from("enterprise_document_links")
      .select("id,enterprise_document_id,reference_id,created_at")
  ...
  const documentIds = [...new Set(contractLinks.map((row) => row.enterprise_document_id).filter(Boolean))];
  const [documents, signatures, billingProfiles, clientProfiles] = await Promise.all([
    documentIds.length ? loadCompletePracticeRowsByIds({
      ids: documentIds,
      label: "Recurring planner engagement documents",
      buildQuery: (batch, from, to) => supabaseAdmin.from("enterprise_documents")
        .select("id,document_status,approved_at")
        .eq("organization_id", firmId)
        .in("id", batch)
        .order("id", { ascending: true })
        .range(from, to),
    ...
    documentIds.length ? loadCompletePracticeRowsByIds({
      ids: documentIds,
      label: "Recurring planner engagement signatures",
      buildQuery: (batch, from, to) => supabaseAdmin.from("document_signature_requests")
        .select("id,enterprise_document_id,status,requested_at")
        .eq("organization_id", firmId)
    ...
    engagementIds.length ? loadCompletePracticeRowsByIds({
      ids: engagementIds,
      label: "Recurring planner billing profiles",
      buildQuery: (batch, from, to) => supabaseAdmin.from("accounting_practice_billing_profiles")
        .select("id,engagement_id,billing_method,currency_code,default_hourly_rate,fixed_fee_amount,billing_entity_id,customer_party_id,revenue_account_id,tax_rule_id,tax_rate_percent,tax_treatment_confirmed,payment_terms_days,billing_cadence,next_billing_date,status")
        .eq("accounting_firm_id", firmId)
        .eq("status", "ACTIVE")
    ...
    organizationIds.length ? loadCompletePracticeRowsByIds({
      ids: organizationIds,
      label: "Recurring planner client staffing profiles",
  ...
  for (const link of contractLinks) if (!linkMap.has(link.reference_id)) linkMap.set(link.reference_id, link);
  ...
  for (const signature of signatures) {
    ...
    rows.push(signature);
    signaturesByDocument.set(signature.enterprise_document_id, rows);
  }
  const billingMap = new Map(billingProfiles.map((row) => [row.engagement_id, row]));
  const billingReferenceBlockers = await loadPracticeBillingReferenceBlockers({ accountingFirmId: firmId, profiles: billingProfiles });
  const profileMap = new Map(clientProfiles.map((row) => [row.organization_id, row]));
  const assignedStaffIds = [...new Set(clientProfiles.flatMap((row) => [
    row.assigned_accountant_id,
    row.assigned_reviewer_id,
    row.assigned_partner_id,
  ]).filter(Boolean))];
  const [activeStaff, activeMemberships] = await Promise.all([
    assignedStaffIds.length ? loadCompletePracticeRowsByIds({
      ids: assignedStaffIds,
      label: "Recurring planner active accounting staff",
    ...
    assignedStaffIds.length ? loadCompletePracticeRowsByIds({
      ids: assignedStaffIds,
      label: "Recurring planner active firm memberships",
      buildQuery: (batch, from, to) => supabaseAdmin.from("organization_users")
        .select("staff_account_id,status")
        .eq("organization_id", firmId)
        .eq("status", "active")
        .in("staff_account_id", batch)
        .order("staff_account_id", { ascending: true })
        .range(from, to),
    }) : Promise.resolve([]),
  ]);
  const portalReadyStaffIds = new Set(activeStaff.filter((row) => row.auth_user_id).map((row) => row.id));
  const activeMembershipIds = new Set(activeMemberships.map((row) => row.staff_account_id));
  const validStaffIds = new Set([...portalReadyStaffIds].filter((id) => activeMembershipIds.has(id)));
  ...
  for (const engagement of engagements) {
    ...
    const onboarding = evaluatePracticeEngagementReadiness({
      engagement,
      link,
      document,
      signatures: signatureRows,
      billingProfile,
      billingReferenceBlockers: billingProfile ? billingReferenceBlockers.get(billingProfile.id) || [] : [],
    });
    if (onboarding.state !== "READY") {
      candidates.push({
        idempotency_key: `${engagement.id}:onboarding:${onboarding.state}`,
        engagement_id: engagement.id,
        organization_id: engagement.organization_id,
        ...
        blockers: [onboarding.next_action],
        due_at: null,
        cadence: null,
      });
      continue;
    ...
    if (monthlyServiceEnabled(engagement)) {
      ...
      if (!template) {
        ...
        const staffing = evaluatePracticeStaffingReadiness({
          steps,
          profile: profileMap.get(engagement.organization_id) || null,
          validStaffIds,
        });
        if (staffing.state !== "READY") {
          candidates.push({
            idempotency_key: `${engagement.id}:${template.id}:staffing`,
            engagement_id: engagement.id,
            organization_id: engagement.organization_id,
            entity_id: engagement.entity_id,
            client_name: clientName,
            ...
            template_id: template.id,
            template_name: template.name,
            status: "BLOCKED_STAFF_ASSIGNMENT",
            required_roles: staffing.required_roles,
            missing_roles: staffing.missing_roles,
            invalid_roles: staffing.invalid_roles,
            assignment_conflicts: staffing.conflicts,
            blockers: staffing.blockers,
            due_at: null,
          });
        } else {
  ...
  const sortRank = {
    READY_TO_CREATE: 0,
    BLOCKED_ONBOARDING: 1,
    BLOCKED_STAFF_ASSIGNMENT: 2,
    BLOCKED_ENTITY_CONFIGURATION: 3,
    BLOCKED_PERIOD_CONFIGURATION: 4,
    BLOCKED_YEAR_END_CONFIGURATION: 5,
    TEMPLATE_MISSING: 6,
    ALREADY_EXISTS: 7,
  };
  candidates.sort((a, b) => {
