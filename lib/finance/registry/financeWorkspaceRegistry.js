...
function convergeAccountingSettingsWorkspace(item) {
  ...
  // Closed Finance workspaces serialize actions from their contract. Keep this
  // setup workflow attached to the Accounting Settings contract so users never
  // need to manage the three low-level customer-deposit posting rules directly.
...
export function applyFinanceWorkspaceRegistry(registry) {
  ...
  convergeCustomerPaymentsWorkspace(
    getFinanceWorkspaceItem(registry, "customer_payments")
  );
  convergeAccountingSettingsWorkspace(
    getFinanceWorkspaceItem(registry, "accounting_settings")
  );
