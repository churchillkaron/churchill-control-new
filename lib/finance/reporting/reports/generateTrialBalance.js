...
export default async function generateTrialBalance({
  ...
  if (!entity_id) throw new Error("legal_entity_id required");
  ...
  const totalDebits = rows.reduce((sum, row) => sum + row.total_debits, 0);
  const totalCredits = rows.reduce((sum, row) => sum + row.total_credits, 0);
