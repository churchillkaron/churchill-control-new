...
function nextRunAt(currentValue, frequency) {
...
async function executeSchedule(schedule) {
  ...
  const { error: updateError } = await supabaseAdmin
    ...
    .update({
      last_run_at: now,
      last_run_id: run.id,
      next_run_at: nextRunAt(schedule.next_run_at, schedule.frequency),
      updated_at: now,
    })
    .eq("organization_id", schedule.organization_id)
    .eq("id", schedule.id);
  ...
  return { schedule_id: schedule.id, run_id: run.id, report_type: reportType };
}
...
export async function processDueScheduledReports({ limit = 25 } = {}) {
  ...
  const { data: schedules, error } = await supabaseAdmin
    .from("finance_scheduled_reports")
    .select("id, organization_id, entity_id, report_template_id, frequency, next_run_at, status")
    .eq("status", "ACTIVE")
    .lte("next_run_at", now)
