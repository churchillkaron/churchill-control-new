...
function localDateTimeToUtc(value, timeZone) {
  ...
  const finalParts = partsInZone(candidate, timeZone);
...
export function normalizeScheduledReportPayload(payload) {
  ...
  if (normalized.next_run_at && normalized.timezone) {
...
export async function validateScheduledReportWrite({ organizationId, payload, recordId = null }) {
  ...
  if (!TIMEZONES.has(candidate.timezone)) throw new Error("Scheduled Report timezone is not supported");
