...
export async function execute({ context, payload = {} }) {
  ...
  if (text(payload.assignment_id)) {
    const assignments = await listUserFinanceRoles({ organizationId });
    const assignment = assignments.find(
      (row) => String(row.id) === text(payload.assignment_id)
    );
    if (!assignment) throw new Error("Finance role assignment not found");
    return { success: true, assignment, rows: [assignment] };
  }

  const grants = await listFinancePermissionGrants(organizationId);
  const grant = grants.find(
    (row) => String(row.id) === text(payload.permission_grant_id)
  );
