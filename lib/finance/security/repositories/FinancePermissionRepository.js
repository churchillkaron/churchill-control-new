...
export async function getFinancePermissionGrantById({ organizationId, permissionGrantId }) {
  requireOrganizationId(organizationId);
  if (!permissionGrantId) throw new Error("permissionGrantId required");

  const { data: permission, error } = await supabaseAdmin
    .from("finance_permissions")
    .select("id, organization_id, role_id, permission_key, created_at")
    .eq("organization_id", organizationId)
    .eq("id", permissionGrantId)
    .maybeSingle();
  ...
  const roles = await listFinanceRoles(organizationId);
  const role = roles.find((candidate) => String(candidate.id) === String(permission.role_id)) || null;
...
export async function getFinanceRoleAssignmentById({ organizationId, assignmentId }) {
  requireOrganizationId(organizationId);
  ...
  const [roles, staffResult] = await Promise.all([
    listFinanceRoles(organizationId),
    supabaseAdmin
      .from("staff_accounts")
      .select("auth_user_id, name, email, position, department, party_id")
      .eq("auth_user_id", assignment.user_id)
      .limit(1)
      .maybeSingle(),
  ]);

  if (staffResult.error) throw staffResult.error;
  const role = roles.find((candidate) => String(candidate.id) === String(assignment.role_id)) || null;
  const person = staffResult.data || null;
