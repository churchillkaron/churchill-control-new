...
async function loadVendorEvidence({ organizationId, entityId, vatReturn, rulesById, functionalCurrency, dependencyCode }) {
  ...
  const eligibleVatSummaryByInvoice = new Map();
  for (const invoice of invoices) {
    const invoiceLines = linesByInvoice.get(invoice.id) || [];
    ...
    if (eligibleVatLines.length) {
      ...
      eligibleVatSummaryByInvoice.set(invoice.id, {
        tax_amount: eligibleVatLines.reduce((sum, line) => sum + numeric(line.tax_amount), 0),
        vat_line_count: eligibleVatLines.length,
      });
    }
  ...
  if (dependencyCode === "POTENTIAL_DUPLICATES") {
    ...
    for (const group of groups.values()) {
      ...
      const duplicateRecords = group.map(row => {
        const summary = eligibleVatSummaryByInvoice.get(row.id) || {};
        const postingJournal = row.journal_entry_id ? journalsById.get(row.journal_entry_id) || null : null;
        return {
          ...sourceRecord(row, "VENDOR_INVOICE"),
          tax_amount: summary.tax_amount ?? null,
          vat_line_count: summary.vat_line_count ?? 0,
          posting_journal: journalRecord(postingJournal),
        };
      });
      issues.push(issue({
        ...
        detail: `${group.length} vendor invoices share the same supplier and invoice number.`,
        source: {
          ...sourceRecord(invoice, "VENDOR_INVOICE"),
          duplicate_record_ids: group.map(row => row.id),
