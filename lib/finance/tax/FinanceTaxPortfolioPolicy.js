...
function clientDependencyPriorityBoost(analysis) {
  const state = upper(analysis?.state);
  if (state === "CLIENT_RESPONDED") return 24;
  if (state === "ACCESS_EXPIRED") return 18;
  if (state === "FOLLOW_UP_DUE") return 14;
  if (state === "MANUAL_FOLLOW_UP") return 12;
  if (["NOT_ISSUED", "REQUEST_MISSING"].includes(state)) return 8;
  return 0;
}
...
export function buildFinanceTaxDependencyPortfolioRows({ filingRows, guidanceByReturnId, envelopes = [], clientRequests = [], currentUserId = null }) {
  ...
  for (const [returnId, guidance] of guidanceByReturnId instanceof Map ? guidanceByReturnId.entries() : []) {
    ...
    for (const dependency of guidance.dependencies || []) {
      ...
      const clientEvidence = dependency.responsibility === "CLIENT_EVIDENCE_ACCOUNTANT_VALIDATION";
      const clientDependency = clientEvidence || request
        ? resolveFinanceClientDependency(request, {
            workItem: { title: dependency.title },
            today: guidance.legal_date,
          })
        : null;
      ...
      const coordinationBoost = (ownedByMe ? 30 : !envelope?.assigned_to ? 20 : 0) + (targetOverdue ? 25 : 0);
      const clientDependencyBoost = clientDependencyPriorityBoost(clientDependency);
      rows.push({
        id: dependency.id,
        ...
        legal_date: guidance.legal_date || null,
        ...
        client_dependency_state: clientDependency?.state || null,
        client_dependency_title: clientDependency?.title || null,
        client_dependency_detail: clientDependency?.detail || null,
        client_dependency_action: clientDependency?.nextAction || null,
        client_dependency_action_kind: clientDependency?.nextActionKind || null,
        client_should_wait: clientDependency?.shouldWait === true,
        client_safe_to_follow_up: clientDependency?.safeToFollowUp === true,
        client_next_eligible_follow_up_at: clientDependency?.nextEligibleFollowUpAt || null,
        evidence_count: Number(dependency.evidence_count || 0),
        priority: Number(filing.priority || 0) * 100 + dependencyUrgency * 5 + coordinationBoost + clientDependencyBoost,
...
export function summarizeFinanceTaxDependencyPortfolio(rows) {
  ...
  return {
    ...
    client_responded: values.filter(row => row.client_dependency_state === "CLIENT_RESPONDED").length,
    follow_up_due: values.filter(row => ["FOLLOW_UP_DUE", "MANUAL_FOLLOW_UP"].includes(row.client_dependency_state)).length,
    do_not_chase: values.filter(row => ["WAITING_NO_CHASE", "WAITING_MANUAL"].includes(row.client_dependency_state)).length,
    access_expired: values.filter(row => row.client_dependency_state === "ACCESS_EXPIRED").length,
  };
}
