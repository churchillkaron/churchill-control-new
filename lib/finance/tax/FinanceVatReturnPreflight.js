...
export async function buildFinanceVatReturnPreflight({ organizationId, entityId, vatReturnId }) {
  ...
  const [journalEntries, customerLines, vendorLines] = await Promise.all([
    fetchByIds({
  ...
  for (const invoice of vendorInvoices) {
    ...
    const readyInvoice = upper(invoice.status) === "POSTED" && upper(invoice.approval_status) === "APPROVED" && Boolean(invoice.journal_entry_id);
    const foreignRateMissing = eligibleVatLines.length > 0 && needsExchangeRate(invoice, functionalCurrency);
    ...
    if (eligibleVatLines.length && !readyInvoice) {
      ...
      exceptions.push(exception({ code: "INPUT_NOT_APPROVED_POSTED", sourceType: "VENDOR_INVOICE", sourceId: invoice.id, reference, date: invoiceDate, detail: "Input VAT is only claimable here when the vendor invoice is approved, posted and linked to its journal entry.", amount: eligibleVatLines.reduce((sum, line) => sum + numeric(line.tax_amount), 0) }));
    }
    ...
    const eligibleTax = eligibleVatLines.reduce((sum, line) => sum + numeric(line.tax_amount), 0);
    if (eligibleVatLines.length) { relevantVendorRows.push(invoice); relevantVendorLines.push(...eligibleVatLines); }
