...
function nonZero(value) {
  return Math.abs(numeric(value)) > 0.0000001;
}
...
function afterOrEqual(left, right) {
  return !left || !right || String(left) >= String(right);
}
...
function currencyOf(row) {
  return upper(row?.currency_code || row?.currency);
}
...
function referenceOf(row, fallback) {
  return text(row?.invoice_number || row?.return_number || row?.reference || row?.document_number || fallback);
}
...
function check({ code, label, status, detail, count = 0, blocksCalculation = false, blocksSubmission = false }) {
  return {
    ...
    detail,
    count,
    blocks_calculation: blocksCalculation,
...
async function fetchPaged(makeQuery, label) {
  ...
  while (true) {
    ...
    if (rows.length > MAX_PREFLIGHT_ROWS) {
      throw new Error(`${label} exceeds the ${MAX_PREFLIGHT_ROWS.toLocaleString("en-US")} row interactive preflight limit`);
    }
...
async function loadContext({ organizationId, entityId }) {
  ...
  const profile = profileRows.find(row => row.entity_id === entityId)
    || profileRows.find(row => !row.entity_id)
    || null;
  const functionalCurrency = upper(
...
export async function loadFinanceTaxWorkspaceSetup({ organizationId, entityId }) {
  ...
  const countryCandidates = country === "TH" || country === "THA" || country === "THAILAND"
    ? ["THAILAND", country]
    : [country];
...
export async function buildFinanceVatReturnPreflight({ organizationId, entityId, vatReturnId }) {
  ...
  const [journalEntries, vendorLines] = await Promise.all([
    fetchByIds({
  ...
  for (const line of vendorLines) {
    ...
    const current = linesByInvoice.get(line.vendor_invoice_id) || [];
    current.push(line);
    linesByInvoice.set(line.vendor_invoice_id, current);
  }

  const rulesById = new Map(context.rules.map(rule => [rule.id, rule]));
  const periodVatRules = context.rules.filter(rule =>
    upper(rule.tax_type) === "VAT"
    && upper(rule.tax_regime) === jurisdiction
    && ruleOverlapsPeriod(rule, periodStart, periodEnd)
  );
  ...
  let uncodedOutputIssues = 0;

  for (const invoice of customerInvoices) {
    const invoiceJurisdiction = upper(invoice.tax_regime);
    const taxAmount = numeric(invoice.tax_amount);
    const reference = referenceOf(invoice, invoice.id);

    if (nonZero(taxAmount) && !invoiceJurisdiction) {
      uncodedOutputIssues += 1;
      exceptions.push(exception({
        code: "OUTPUT_TAX_REGIME_MISSING",
        sourceType: "CUSTOMER_INVOICE",
        sourceId: invoice.id,
        reference,
        date: invoice.invoice_date,
        detail: "Taxable customer invoice has no tax regime, so it cannot be assigned safely to a return.",
        amount: taxAmount,
      }));
      continue;
    }

    if (invoiceJurisdiction !== jurisdiction) continue;
    ...
    const enginePosted = postedJournals.length > 0;
    const validPosted = validPostedJournals.length > 0;
    const foreignRateMissing = nonZero(taxAmount) && needsExchangeRate(invoice, functionalCurrency);

    if (enginePosted) {
      const sign = upper(invoice.document_type || "INVOICE") === "CREDIT_NOTE" ? -1 : 1;
      outputTax += sign * taxAmount * exchangeRate(invoice);
      outputDocumentCount += 1;
      if (sign < 0) customerCreditNoteCount += 1;
    }

    if (nonZero(taxAmount) && !validPosted) {
      ...
      exceptions.push(exception({
        ...
        detail: postedJournals.length
          ? "The taxable sales document only has reversed posting evidence. Create a valid posting before calculating the return."
          : "Taxable sales document is not backed by a posted journal entry.",
        amount: taxAmount,
      }));
    }
    ...
    outputEvidence.push({
      ...
      functional_tax_amount: enginePosted
        ? roundMoney((upper(invoice.document_type || "INVOICE") === "CREDIT_NOTE" ? -1 : 1) * taxAmount * exchangeRate(invoice))
        : 0,
      posted: validPosted,
  ...
  for (const invoice of vendorInvoices) {
    ...
    if (missingCodeLines.length || unresolvedCodeLines.length || ineligibleVatLines.length) {
      ...
      for (const line of missingCodeLines) {
        exceptions.push(exception({
      ...
      for (const line of unresolvedCodeLines) {
        exceptions.push(exception({
    ...
    if (eligibleVatLines.length && !readyInvoice) {
      ...
      exceptions.push(exception({
        code: "INPUT_NOT_APPROVED_POSTED",
        sourceType: "VENDOR_INVOICE",
        sourceId: invoice.id,
        reference,
        date: invoiceDate,
        detail: "Input VAT is only claimable here when the vendor invoice is approved, posted and linked to its journal entry.",
    ...
    if (foreignRateMissing) {
      inputRateIssues += 1;
      exceptions.push(exception({
        code: "INPUT_EXCHANGE_RATE_MISSING",
        sourceType: "VENDOR_INVOICE",
        sourceId: invoice.id,
        reference,
        date: invoiceDate,
        detail: `Foreign-currency input VAT cannot use an implicit 1.0 rate against ${functionalCurrency}.`,
        amount: eligibleVatLines.reduce((sum, line) => sum + numeric(line.tax_amount), 0),
      }));
    }
    ...
    if (eligibleVatLines.length && readyInvoice) {
      inputTax += eligibleTax * exchangeRate(invoice);
  ...
  for (const group of duplicateRows) {
    ...
    exceptions.push(exception({
      code: "POTENTIAL_DUPLICATE_VENDOR_INVOICE",
      severity: "WARNING",
      sourceType: "VENDOR_INVOICE",
      sourceId: invoice.id,
      reference: referenceOf(invoice, invoice.id),
      date: invoice.invoice_date,
      detail: `${group.length} vendor invoices share the same supplier and invoice number. Review before filing.`,
    }));
  }
  ...
  const codingIssues = inputCodingIssues + uncodedOutputIssues;
  const today = new Date().toISOString().slice(0, 10);
  const overdue = Boolean(vatReturn.filing_due_date && vatReturn.filing_due_date < today && status !== "SUBMITTED");

  const checks = [
    check({
      code: "REGISTRATION_REFERENCE",
      label: "Tax registration",
      status: missingRegistration ? "BLOCK" : "PASS",
      detail: missingRegistration ? "Add the VAT registration reference before calculation." : registrationReference,
      count: missingRegistration ? 1 : 0,
      blocksCalculation: missingRegistration,
      blocksSubmission: missingRegistration,
    }),
    check({
      code: "VAT_RULES",
      label: "VAT configuration",
      status: missingRules ? "BLOCK" : "PASS",
      detail: missingRules
        ? `No active VAT rule covers ${jurisdiction || "this jurisdiction"} for the filing period.`
        : `${periodVatRules.length} active VAT rule${periodVatRules.length === 1 ? "" : "s"} cover the filing period.`,
      count: missingRules ? 1 : 0,
      blocksCalculation: missingRules,
      blocksSubmission: missingRules,
    }),
    check({
      code: "OUTPUT_POSTING",
      label: "Sales posting evidence",
      status: outputPostingIssues ? "BLOCK" : "PASS",
      detail: outputPostingIssues
        ? `${outputPostingIssues} taxable sales document${outputPostingIssues === 1 ? " is" : "s are"} not backed by valid posted accounting evidence.`
        : `${outputDocumentCount} posted sales document${outputDocumentCount === 1 ? "" : "s"} included.`,
      count: outputPostingIssues,
      blocksCalculation: outputPostingIssues > 0,
      blocksSubmission: outputPostingIssues > 0,
    }),
    check({
      code: "INPUT_CODING",
      label: "Purchase VAT coding",
      status: codingIssues ? "BLOCK" : "PASS",
      detail: codingIssues
        ? `${codingIssues} tax line${codingIssues === 1 ? " needs" : "s need"} governed tax coding or effective-rule repair.`
        : "Tax-bearing purchase lines resolve to governed VAT codes.",
      count: codingIssues,
      blocksCalculation: codingIssues > 0,
      blocksSubmission: codingIssues > 0,
    }),
    check({
      code: "INPUT_POSTING",
      label: "Purchase approval & posting",
      status: inputPostingIssues ? "BLOCK" : "PASS",
      detail: inputPostingIssues
        ? `${inputPostingIssues} VAT-bearing vendor invoice${inputPostingIssues === 1 ? " is" : "s are"} not fully approved and posted.`
        : `${inputDocumentCount} posted purchase document${inputDocumentCount === 1 ? "" : "s"} included.`,
      count: inputPostingIssues,
      blocksCalculation: inputPostingIssues > 0,
      blocksSubmission: inputPostingIssues > 0,
    }),
    check({
      code: "EXCHANGE_RATES",
      label: "Currency conversion",
      status: rateIssues ? "BLOCK" : "PASS",
      detail: rateIssues
        ? `${rateIssues} foreign-currency tax document${rateIssues === 1 ? " is" : "s are"} missing a real exchange rate.`
        : `Tax evidence can be translated to ${functionalCurrency} without an implicit 1.0 rate.`,
      count: rateIssues,
      blocksCalculation: rateIssues > 0,
      blocksSubmission: rateIssues > 0,
    }),
    check({
      code: "FILING_DEADLINE",
      label: "Filing deadline",
      status: !vatReturn.filing_due_date || overdue ? "WARNING" : "PASS",
      detail: !vatReturn.filing_due_date
        ? "No filing due date is recorded. Add the statutory deadline so the work queue can prioritise it."
        : overdue
          ? `This return was due ${vatReturn.filing_due_date}. Filing can continue, but it should be treated as overdue work.`
          : `Due ${vatReturn.filing_due_date}.`,
      count: !vatReturn.filing_due_date || overdue ? 1 : 0,
    }),
    check({
      code: "POTENTIAL_DUPLICATES",
      label: "Duplicate review",
      status: duplicateRows.length ? "WARNING" : "PASS",
      detail: duplicateRows.length
        ? `${duplicateRows.length} potential duplicate vendor invoice group${duplicateRows.length === 1 ? " needs" : "s need"} human review.`
        : "No supplier/invoice-number duplicates detected in VAT-bearing purchases.",
      count: duplicateRows.length,
    }),
  ];

  const current = {
    method: "POSTED_ACCOUNTING_EVIDENCE_V1",
  ...
  const stored = vatReturn.calculation && typeof vatReturn.calculation === "object"
    ? vatReturn.calculation
    : {};
  ...
  if (status === "CALCULATED") {
    ...
    for (const [label, oldValue, newValue, tolerance] of comparable) {
      if (!Number.isFinite(oldValue) || Math.abs(oldValue - newValue) > tolerance) {
        freshnessReasons.push(`${label} changed`);
      }
    }

    const jurisdictionCustomerIds = new Set(customerInvoices.filter(row => upper(row.tax_regime) === jurisdiction).map(row => row.id));
    const sourceChanged = [
      ...customerInvoices.filter(row => upper(row.tax_regime) === jurisdiction),
      ...journalEntries.filter(row => jurisdictionCustomerIds.has(row.source_document_id)),
      ...relevantVendorRows,
      ...relevantVendorLines,
      ...periodVatRules,
  ...
  checks.push(check({
    ...
    detail: status === "DRAFT"
      ? "Calculate after all blocking evidence checks pass."
      : calculationStale
        ? `Recalculate before filing: ${[...new Set(freshnessReasons)].join(", ")}.`
        : status === "SUBMITTED"
          ? "Filed return is immutable in Avantiqo."
          : `Calculation matches current governed evidence${calculatedAt ? ` from ${calculatedAt}` : ""}.`,
  ...
  if (status === "SUBMITTED") {
    checks.push(check({
      code: "SUBMITTED_IMMUTABLE",
      label: "Filing state",
      status: "PASS",
      detail: `Submitted${vatReturn.submission_reference ? ` · ${vatReturn.submission_reference}` : ""}. Accounting filing evidence is immutable.`,
    }));
  }
  ...
  if (status !== "CALCULATED" && status !== "SUBMITTED") {
    submissionBlockers.push(check({
      code: "CALCULATION_REQUIRED",
      label: "Calculation required",
      status: "BLOCK",
      detail: "Calculate the return from governed accounting evidence before recording submission.",
      count: 1,
  ...
  const state = status === "SUBMITTED"
    ...
    : readyToSubmit
      ? "READY_TO_FILE"
      : readyToCalculate
        ? status === "CALCULATED" ? "CALCULATED" : "READY_TO_CALCULATE"
        : "NEEDS_ATTENTION";

  return {
    return: {
      ...vatReturn,
      registration_reference: registrationReference || vatReturn.registration_reference || null,
    },
    entity: {
      id: context.entity.id,
      code: context.entity.code || null,
    ...
    calculated: {
      at: calculatedAt,
      by: vatReturn.calculated_by || null,
      values: stored,
      freshness_reasons: [...new Set(freshnessReasons)],
    },
    evidence: {
    ...
    vat_rules: periodVatRules.map(rule => ({
      id: rule.id,
      tax_code: rule.tax_code,
      tax_name: rule.tax_name,
      tax_rate: rule.tax_rate,
    ...
    due: {
      filing_due_date: vatReturn.filing_due_date || null,
      overdue,
      missing: !vatReturn.filing_due_date,
    },
  };
}
