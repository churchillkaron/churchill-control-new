...
const FAMILY_META = Object.freeze({
  ledger: { label: "Accounting records", layout: "ledger", reviewLabel: "Review accounting evidence" },
  receivables: { label: "Receivables", layout: "transaction", reviewLabel: "Collect, allocate and clear exceptions" },
  payables: { label: "Payables", layout: "transaction", reviewLabel: "Review, match, approve and pay" },
  treasury: { label: "Treasury", layout: "reconciliation", reviewLabel: "Review cash and reconciliation exceptions" },
  compliance: { label: "Tax & compliance", layout: "review", reviewLabel: "Prepare, evidence, approve and file" },
  assets: { label: "Fixed assets", layout: "asset", reviewLabel: "Review asset value and lifecycle" },
  enterprise: { label: "Entity & consolidation", layout: "control", reviewLabel: "Control multi-entity accounting" },
  planning: { label: "Planning & performance", layout: "planning", reviewLabel: "Compare plan, actual and forecast" },
  reporting: { label: "Reporting", layout: "reporting", reviewLabel: "Review and sign off reporting outputs" },
  configuration: { label: "Finance configuration", layout: "configuration", reviewLabel: "Maintain governed accounting configuration" },
});

const FAMILY_BY_CAPABILITY = Object.freeze({
  chart_of_accounts: "ledger", general_ledger: "ledger", journals: "ledger", trial_balance: "ledger", fiscal_periods: "ledger", dimensions: "ledger", opening_balances: "ledger", recurring_journals: "ledger",
  customers: "receivables", customer_invoices: "receivables", accounts_receivable: "receivables", customer_payments: "receivables", collections: "receivables", customer_statements: "receivables", revenue_recognition: "receivables",
  vendors: "payables", purchase_orders: "payables", goods_receipts: "payables", vendor_bills: "payables", invoice_matching: "payables", accounts_payable: "payables", vendor_payments: "payables", vendor_statements: "payables",
  bank_accounts: "treasury", cash_management: "treasury", bank_statements: "treasury", cash_flow: "treasury", bank_reconciliation: "treasury", payments: "treasury", fx_revaluation: "treasury",
  tax: "compliance", vat_returns: "compliance", tax_codes: "configuration", fixed_assets: "assets", depreciation: "assets", audit_trail: "compliance", period_close: "compliance", year_end: "compliance", statutory_filings: "compliance",
  legal_entities: "enterprise", cost_centers: "enterprise", currencies: "configuration", intercompany: "enterprise", payment_terms: "configuration", consolidation: "enterprise",
  budgeting: "planning", forecasting: "planning", finance_kpis: "planning", executive_dashboard: "planning", financial_health: "planning", ai_insights: "planning",
  financial_statements: "reporting", management_reports: "reporting", finance_analytics: "reporting", report_builder: "reporting", scheduled_reports: "reporting",
  organization_profile: "configuration", accounting_settings: "configuration", number_sequences: "configuration", posting_rules: "configuration", approval_workflows: "configuration", government_connections: "configuration", banking_integrations: "configuration", exchange_rates: "configuration", e_invoicing: "configuration", document_templates: "configuration", finance_permissions: "configuration",
});
...
const COLUMN_SETS = Object.freeze({
  ...
  customer_invoice: [
    { label: "Document", keys: ["invoice_number"] },
    { label: "Customer", keys: ["customer_name", "party_name", "legal_name"] },
    { label: "Date", keys: ["invoice_date", "created_at"], format: "date" },
    { label: "Due", keys: ["due_date"], format: "date" },
    { label: "Invoice total", keys: ["total_amount", "grand_total", "amount"], format: "money", align: "right" },
    { label: "Outstanding", keys: ["outstanding_amount", "outstanding_balance", "open_balance"], format: "money", align: "right" },
...
function hasExecutableRuntimeEvidence(capability, runtimeDefinition) {
  if (!runtimeDefinition) return false;
  const contract = getFinanceWorkspaceContract(capability?.id);
  const configuredApi = runtimeDefinition.api || capability?.ui?.api || capability?.runtime?.listApi || null;
  const create = capability?.create;
  const executableCreate = Boolean(create?.enabled === true && (create?.form || create?.schema?.length || create?.api || create?.endpoint || create?.engine));
  const executableAction = actionList(capability?.actions || capability?.topMenu || capability?.ui?.topMenu).some(action =>
    action?.endpoint || action?.api || action?.engine || action?.href || action?.capability || action?.type === "report"
  );
  return Boolean(configuredApi || contract || executableCreate || executableAction);
}
...
function resolveRuntimeReadiness(capability) {
  ...
  const runtimeBacked = hasExecutableRuntimeEvidence(capability, runtimeDefinition);

  return {
    runtimeBacked,
    runtimeDefinition,
    declaredStatus,
    effectiveStatus: runtimeBacked && declaredStatus.toLowerCase() === "planned" ? "active" : declaredStatus,
  };
}
...
export function resolveFinanceCapabilityPresentation(capability, groupId = null) {
  ...
  return {
    ...
    sticky_header: true,
    review_first: true,
    default_sort_index: capabilityId === "customer_invoices" ? 2 : 0,
    default_sort_direction: capabilityId === "customer_invoices" ? "desc" : "asc",
    columns: capabilityId === "customer_invoices"
      ? COLUMN_SETS.customer_invoice
      : COLUMN_SETS[familyMeta.layout] || COLUMN_SETS.configuration,
  };
}
...
export function applyFinanceCapabilityPresentation(registry) {
  ...
  for (const group of groups) {
    for (const item of Array.isArray(group?.items) ? group.items : []) {
      ...
      if (readiness.runtimeBacked && readiness.declaredStatus.toLowerCase() === "planned") item.status = readiness.effectiveStatus;

      const presentation = resolveFinanceCapabilityPresentation(item, group.id);
      item.ui = { ...(item.ui || {}), financePresentation: presentation };
      item.runtime = {
        ...(item.runtime || {}),
...
export function getFinancePresentationCoverage() {
  ...
  return { total: manifestIds.length, covered: coveredIds.length, missing: manifestIds.filter(id => !FAMILY_BY_CAPABILITY[id]) };
}
