...
function clientAccess(request) {
  const access = request?.metadata?.client_access;
...
export function resolveFinanceClientDependency(request, { workItem = null, today = null } = {}) {
  ...
  const accessExpiresAt = dateKey(access?.expires_at);
  const accessRevoked = Boolean(access?.revoked_at);
  const accessExpired = Boolean(accessExpiresAt && accessExpiresAt < currentDay);
  ...
  if (accessExpired || accessRevoked) {
    return {
      state: "ACCESS_EXPIRED",
      priority: 1,
      title: accessRevoked ? "Client access is revoked" : "Client access expired",
      detail: accessRevoked
        ? "The existing secure client grant is revoked. Restore governed access before asking the client to act again."
        : `The secure client grant expired${accessExpiresAt ? ` on ${accessExpiresAt}` : ""}. Restore access before any reminder or follow-up.`,
      nextAction: "Reissue secure access for the existing request",
      nextActionKind: "reissue_access",
      safeToFollowUp: false,
      shouldWait: false,
      overdue,
      accessExpired: true,
      accessExpiresAt,
      lastContactAt: sent,
      blocks: `${workTitle} remains blocked and the client cannot safely complete the existing request until access is restored.`,
    };
  }
...
export function buildFinanceClientDependencySummary(workItems = []) {
  ...
  return {
    ...
    counts: {
      ...
      action_due: dependencies.filter((row) => ["CLIENT_RESPONDED", "NOT_ISSUED", "REQUEST_MISSING", "ACCESS_EXPIRED", "FOLLOW_UP_DUE", "MANUAL_FOLLOW_UP"].includes(row.analysis.state)).length,
      ...
      access_expired: dependencies.filter((row) => row.analysis.state === "ACCESS_EXPIRED").length,
    },
  };
}