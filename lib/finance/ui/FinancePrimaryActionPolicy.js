export const FINANCE_PRIMARY_ACTION_POLICY = Object.freeze({
  chart_of_accounts: { mode: "create" },
  general_ledger: { mode: "none" },
  journals: { mode: "create" },
  trial_balance: { mode: "none" },
  fiscal_periods: {
    mode: "create",
    create: {
      enabled: true,
      type: "document",
      engine: "create",
      id: "fiscal_period",
      form: "fiscal-period",
      api: "/api/finance/periods/open",
      label: "+ Fiscal Period",
      title: "New Fiscal Period",
      submitLabel: "Create Period",
    },
  },
  dimensions: {
    mode: "create",
    create: {
      enabled: true,
      type: "document",
      engine: "create",
      id: "finance_dimension",
      form: "cost-center",
      api: "/api/finance/cost-centers/create",
      label: "+ Dimension",
      title: "New Reporting Dimension",
      submitLabel: "Create Dimension",
    },
  },
  opening_balances: { mode: "create" },
  recurring_journals: { mode: "create" },

  customers: { mode: "create" },
  customer_invoices: { mode: "create" },
  accounts_receivable: { mode: "none" },
  customer_payments: {
    mode: "create",
    create: {
      enabled: true,
      type: "document",
      engine: "create",
      id: "customer_payment",
      form: "customer-payment",
      api: "/api/finance/customer-payments/create",
      label: "+ Customer Receipt",
      title: "Receive Customer Payment",
      submitLabel: "Post Receipt",
    },
  },
  collections: { mode: "none" },
  customer_statements: {
    mode: "action",
    action: {
      id: "generate_statement",
      type: "reports",
      label: "Generate Statement",
      title: "Generate Customer Statement",
    },
  },
  revenue_recognition: { mode: "create" },

  vendors: { mode: "create" },
  purchase_orders: { mode: "none" },
  goods_receipts: { mode: "none" },
  vendor_bills: { mode: "create" },
  invoice_matching: { mode: "none" },
  accounts_payable: { mode: "create" },
  vendor_payments: {
    mode: "create",
    create: {
      enabled: true,
      type: "document",
      engine: "create",
      id: "vendor_payment",
      form: "vendor-payment",
      api: "/api/finance/accounts-payable/pay",
      label: "+ Vendor Payment",
      title: "Pay Vendor",
      submitLabel: "Post Payment",
    },
  },
  vendor_statements: { mode: "create" },

  bank_accounts: { mode: "create" },
  cash_management: {
    mode: "action",
    action: {
      id: "refresh_liquidity",
      type: "runtime",
      label: "Refresh Liquidity",
      title: "Refresh Liquidity Position",
      endpoint: "/api/finance/treasury/liquidity",
      method: "POST",
    },
  },
  bank_statements: { mode: "create" },
  cash_flow: {
    mode: "action",
    action: {
      id: "run_cash_flow",
      type: "runtime",
      label: "Run Cash Flow",
      title: "Run Cash Flow",
      endpoint: "/api/finance/cash-flow/run",
      method: "POST",
    },
  },
  bank_reconciliation: {
    mode: "action",
    action: {
      id: "run_reconciliation",
      type: "runtime",
      label: "Start Reconciliation",
      title: "Start Bank Reconciliation",
      endpoint: "/api/finance/reconciliation",
      method: "POST",
    },
  },
  payments: { mode: "none" },
  fx_revaluation: {
    mode: "action",
    action: {
      id: "run_fx_revaluation",
      type: "runtime",
      label: "Run FX Revaluation",
      title: "Run Foreign Currency Revaluation",
      endpoint: "/api/finance/period-close/steps",
      method: "POST",
      payload: { step_type: "FX_REVALUATION" },
      schema: [
        {
          name: "revaluations",
          label: "Foreign Currency Balances",
          type: "table",
          required: true,
          width: "full",
          columns: [
            { name: "account_id", label: "Account", type: "lookup", lookup: "chart_of_accounts", required: true },
            { name: "foreign_currency", label: "Foreign Currency", type: "currency", required: true },
            { name: "foreign_balance", label: "Foreign Balance", type: "number", required: true },
            { name: "carrying_value", label: "Carrying Value", type: "number", required: true },
            { name: "closing_rate", label: "Closing Rate", type: "number" },
          ],
        },
      ],
    },
  },

  tax: {
    mode: "action",
    action: {
      id: "run_tax_close",
      type: "runtime",
      label: "Run Tax Close",
      title: "Run Period Tax Close",
      endpoint: "/api/finance/period-close/steps",
      method: "POST",
      payload: { step_type: "TAX_CLOSE" },
      schema: [
        { name: "tax_type", label: "Tax Type", type: "text", required: true },
        { name: "confirmation", label: "Confirm Tax Close", type: "select", options: ["RUN"], required: true },
      ],
    },
  },
  vat_returns: { mode: "create" },
  tax_codes: { mode: "create" },
  fixed_assets: { mode: "create" },
  depreciation: {
    mode: "action",
    action: {
      id: "run_depreciation",
      type: "runtime",
      label: "Run Depreciation",
      title: "Run Period Depreciation",
      endpoint: "/api/finance/period-close/steps",
      method: "POST",
      payload: { step_type: "DEPRECIATION" },
      schema: [
        { name: "confirmation", label: "Confirm Period Depreciation", type: "select", options: ["RUN"], required: true },
      ],
    },
  },
  audit_trail: { mode: "none" },
  period_close: {
    mode: "action",
    action: {
      id: "run_period_close",
      type: "runtime",
      label: "Run Period Close",
      title: "Run Month-End Close",
      endpoint: "/api/finance/month-end/close-period",
      method: "POST",
      schema: [
        { name: "confirmation", label: "Type CLOSE to confirm", type: "text", required: true },
      ],
    },
  },
  year_end: {
    mode: "action",
    action: {
      id: "run_year_end",
      type: "runtime",
      label: "Run Year End",
      title: "Close Fiscal Year",
      endpoint: "/api/finance/year-end/close-fiscal-year",
      method: "POST",
      schema: [
        { name: "confirmation", label: "Type CLOSE YEAR to confirm", type: "text", required: true },
      ],
    },
  },
  statutory_filings: { mode: "create" },

  legal_entities: { mode: "create" },
  cost_centers: { mode: "create" },
  currencies: { mode: "create" },
  intercompany: { mode: "create" },
  payment_terms: { mode: "create" },
  consolidation: {
    mode: "action",
    action: {
      id: "run_consolidation",
      type: "runtime",
      label: "Run Consolidation",
      title: "Run Consolidation",
      endpoint: "/api/finance/consolidation",
      method: "POST",
    },
  },

  budgeting: { mode: "create" },
  forecasting: {
    mode: "action",
    action: {
      id: "run_forecast",
      type: "runtime",
      label: "Run Forecast",
      title: "Run Revenue Forecast",
      endpoint: "/api/finance/forecast",
      method: "POST",
      schema: [
        {
          name: "source_start_date",
          label: "Source Start Date",
          type: "date",
        },
        {
          name: "source_end_date",
          label: "Source End Date",
          type: "date",
        },
        {
          name: "horizon_days",
          label: "Forecast Horizon Days",
          type: "number",
          required: true,
          min: 1,
          max: 366,
          defaultValue: 30,
        },
        {
          name: "growth_rate_percent",
          label: "Growth Assumption (%)",
          type: "number",
          min: -100,
          max: 1000,
          defaultValue: 0,
        },
        {
          name: "currency_code",
          label: "Currency",
          type: "currency",
        },
      ],
    },
  },
  finance_kpis: { mode: "none" },
  executive_dashboard: { mode: "none" },
  financial_health: { mode: "none" },
  ai_insights: { mode: "none" },

  financial_statements: {
    mode: "action",
    action: {
      id: "generate_statements",
      type: "reports",
      label: "Generate Statements",
      title: "Generate Financial Statements",
    },
  },
  management_reports: {
    mode: "action",
    action: {
      id: "generate_management_report",
      type: "reports",
      label: "Generate Report",
      title: "Generate Management Report",
    },
  },
  finance_analytics: {
    mode: "action",
    action: {
      id: "open_finance_reports",
      type: "reports",
      label: "Open Reports",
      title: "Open Finance Reports",
    },
  },
  report_builder: { mode: "create" },
  scheduled_reports: { mode: "create" },

  organization_profile: { mode: "create" },
  accounting_settings: { mode: "create" },
  number_sequences: { mode: "create" },
  posting_rules: { mode: "create" },
  approval_workflows: { mode: "create" },
  government_connections: { mode: "create" },
  banking_integrations: { mode: "create" },
  exchange_rates: { mode: "create" },
  e_invoicing: { mode: "create" },
  document_templates: { mode: "create" },
  finance_permissions: { mode: "create" },
});

export function getFinancePrimaryActionPolicy(workspaceId) {
  return FINANCE_PRIMARY_ACTION_POLICY[workspaceId] || null;
}

export function resolveFinanceOperationalAction(workspaceId) {
  const policy = getFinancePrimaryActionPolicy(workspaceId);

  if (!policy || policy.mode !== "action") {
    return null;
  }

  return {
    enabled: true,
    ...policy.action,
  };
}
