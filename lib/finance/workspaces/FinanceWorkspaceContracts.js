...
const postingEventOptions = [
  ...
  { value: "FX_REVALUATION", label: "Foreign Currency Revaluation" },
  ...
  { value: "TAX_POSTING", label: "Tax Posting" },
  { value: "INTERCOMPANY", label: "Intercompany Posting" },
...
export const FINANCE_WORKSPACE_CONTRACTS = Object.freeze({
  ...
  collections: entityWorkspace({
    ...
    search: ["case_number", "priority", "status", "promise_date", "next_follow_up_at"],
    readOnly: true,
  }),
  customer_statements: entityWorkspace({
    table: "customer_statement_runs",
    search: ["statement_number", "statement_date", "period_start", "period_end", "currency_code", "status"],
    readOnly: true,
  }),
  revenue_recognition: entityWorkspace({
    ...
    schema: [lookup("customer_id", "Customer", "customers"), select("source_document_type", "Source Document", [
      { value: "CUSTOMER_INVOICE", label: "Customer Invoice" },
      { value: "CONTRACT", label: "Contract" },
      { value: "SALES_ORDER", label: "Sales Order" },
      { value: "MANUAL", label: "Manual" },
    ], true), lookup("source_document_id", "Customer Invoice", "customer_invoices", false, { visibleWhen: { field: "source_document_type", equals: "CUSTOMER_INVOICE" } }), text("contract_reference", "Contract Reference", false, { visibleWhen: { field: "source_document_type", notEquals: "CUSTOMER_INVOICE" } }), select("recognition_method", "Recognition Method", ["STRAIGHT_LINE", "MILESTONE", "USAGE", "DELIVERY", "MANUAL"], true), date("start_date", "Start Date", true), date("end_date", "End Date", true), number("total_amount", "Total Amount", true, { min: 0 }), currency(), lookup("revenue_account_id", "Revenue Account", "revenue_accounts", true), lookup("deferred_revenue_account_id", "Deferred Revenue Account", "deferred_revenue_accounts"), textarea("notes", "Notes")],
  }),
  ...
  posting_rules: organizationWorkspace({
    ...
    schema: [
      lookup("entity_id", "Legal Entity", "legal_entities", true, { width: "full" }),
      text("name", "Rule Name", true),
      select("event_type", "Accounting Event", postingEventOptions, true),
      select("source_module", "Source Domain", postingSourceOptions, true),
      ...
      number("priority", "Priority", true, { min: 1, step: 1, defaultValue: 100 }),
    ],
  }),
  ...
  e_invoicing: organizationWorkspace({
    ...
    schema: [
      select("provider_code", "Certified Provider", [{ value: "netbay_invoicechain", label: "Netbay InvoiceChain" }, { value: "inet_etax", label: "INET One E-Tax" }, { value: "certified_etax_rest", label: "Other Certified REST Provider" }], true),
      select("network", "Network / Scheme", [{ value: "TH_RD_ETAX", label: "Thailand RD e-Tax Invoice / e-Receipt" }], true, { defaultValue: "TH_RD_ETAX" }),
      select("jurisdiction_code", "Jurisdiction", [{ value: "TH", label: "Thailand" }], true, { defaultValue: "TH" }),
      select("document_type", "Document Type", [{ value: "CUSTOMER_INVOICE", label: "Customer Invoice" }, { value: "CREDIT_NOTE", label: "Credit Note" }, { value: "DEBIT_NOTE", label: "Debit Note" }, { value: "SELF_BILLED_INVOICE", label: "Self-Billed Invoice" }, { value: "OTHER", label: "Other" }], true),
      text("sender_identifier", "Registered Sender Identifier", true, { placeholder: "Tax, participant or endpoint identifier" }),
      select("status", "Connection Status", [{ value: "INACTIVE", label: "Inactive" }, { value: "ACTIVE", label: "Active" }, { value: "SUSPENDED", label: "Suspended" }], true, { defaultValue: "INACTIVE" }),
    ],
  }),
