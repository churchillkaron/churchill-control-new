...
function normalizeRooms(reservation) {
  ...
  return (reservation?.rooms || []).map((room, ordinal) => {
    ...
    return {
      ...room,
      channel_room_stay_id: roomStayId(room, ordinal, duplicateOrdinal),
    };
  });
}
...
function allAlreadyMatched(events) {
  return events.length > 0 && events.every(({ event }) => clean(event.status).toUpperCase() === 'MATCHED' && event.booking_id);
}
...
async function reconcileEvents({ supabase, organizationId, connection, events, bookings }) {
  ...
  for (const item of events) {
    ...
    if (clean(item.event.status).toUpperCase() === 'MATCHED' && item.event.booking_id === booking.id) continue;
...
async function processReservation({ supabase, organizationId, connection, transport, reservation }) {
  ...
  let rooms = normalizeRooms(reservation);
  let events;
  ...
  if (!allAlreadyMatched(events)) {
    const guestId = reservation.event_type === 'CANCEL'
      ? null
      : await resolveGuest({ supabase, organizationId, reservation });
