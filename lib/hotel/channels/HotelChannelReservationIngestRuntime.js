...
async function reservationEventIdsForVersion({ supabase, organizationId, connectionId, externalReservationId, eventVersion }) {
  let query = supabase
    ...
    .eq('external_reservation_id', externalReservationId);
  query = clean(eventVersion) ? query.eq('event_version', eventVersion) : query.is('event_version', null);
  const { data, error } = await query;
  if (error) throw error;
  return (data || []).map((row) => row.id).filter(Boolean);
}

async function loadProviderRetryEvent({ supabase, organizationId, reservationEventId }) {
  const { data, error } = await supabase
    .from('hotel_channel_reservation_events')
    .select('id,connection_id,provider,external_reservation_id,event_version,status,booking_id,provider_ack_status,normalized_payload')
    .eq('organization_id', organizationId)
    .eq('id', reservationEventId)
    .maybeSingle();
  if (error) throw error;
  if (!data) throw new Error('HOTEL_CHANNEL_RESERVATION_EVENT_NOT_FOUND');
  if (clean(data.provider).toLowerCase() !== 'booking_com') throw new Error('HOTEL_CHANNEL_RESERVATION_PROVIDER_NOT_SUPPORTED');
  const eventStatus = clean(data.status).toUpperCase();
  const ackStatus = clean(data.provider_ack_status).toUpperCase() || 'PENDING';
  if (eventStatus !== 'RECONCILED' || !data.booking_id || !['PENDING', 'RETRY_REQUIRED'].includes(ackStatus)) {
    throw new Error('HOTEL_CHANNEL_RESERVATION_RETRY_NOT_ALLOWED');
  }
  const endpointKind = clean(data.normalized_payload?.endpoint_kind).toUpperCase();
  if (!['NEW', 'MODIFY'].includes(endpointKind)) throw new Error('BOOKING_COM_OTA_RETRY_ENDPOINT_KIND_INVALID');
  return { ...data, endpointKind };
}
...
export class HotelChannelReservationIngestRuntime {
  ...
  static async retryProviderHandoff({ supabase, organizationId, reservationEventId }) {
    if (!supabase || !clean(organizationId) || !clean(reservationEventId)) throw new Error('HOTEL_CHANNEL_RESERVATION_RETRY_CONTEXT_REQUIRED');
    const event = await loadProviderRetryEvent({ supabase, organizationId, reservationEventId });
    const connection = await loadConnection({ supabase, organizationId, connectionId: event.connection_id });
    const transport = requireHotelChannelTransport('booking_com');
    if (!transport?.adapter?.pullReservations || !transport?.adapter?.acknowledgeReservation) {
      throw new Error('BOOKING_COM_RESERVATION_TRANSPORT_NOT_IMPLEMENTED');
    }

    const latestReservation = await pullLatestExactReservation({
      organizationId,
      connection,
      transport,
      reservation: {
    ...
    if (clean(latestReservation.event_version) !== clean(event.event_version)) {
      const supersededEventIds = await reservationEventIdsForVersion({
        supabase,
        organizationId,
        connectionId: connection.id,
      ...
      await HotelChannelEvidenceRuntime.recordReservationProviderAcknowledgement({
        ...
        organizationId,
        connectionId: connection.id,
        reservationEventIds: supersededEventIds,
        status: 'SUPERSEDED',
        detail: {
          endpoint_kind: event.endpointKind,
          event_version: event.event_version,
          superseded_by_event_version: latestReservation.event_version,
          operator_retry_event_id: event.id,
        },
      });
    }
    ...
    return {
      ...
      requestedEventVersion: event.event_version,
      latestEventVersion: latestReservation.event_version,
      result,
    };
  }
}
