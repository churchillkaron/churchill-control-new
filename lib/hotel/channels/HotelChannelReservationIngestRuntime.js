...
async function processReservation({ supabase, organizationId, connection, transport, reservation, staleAttempt = 0 }) {
  ...
  try {
    ...
    if (error?.code !== 'BOOKING_COM_OTA_ACK_STALE_RESERVATION') throw error;
    if (staleAttempt >= MAX_STALE_ACK_CONVERGENCE_ATTEMPTS) throw new Error('BOOKING_COM_OTA_STALE_RESERVATION_CONVERGENCE_LIMIT');
