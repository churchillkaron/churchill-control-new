...
function roomPayload(roomStay, fallbackCurrency) {
  ...
  const amount = numberOrNull(attr(total, 'AmountAfterTax') || attr(total, 'AmountBeforeTax'));
  return {
...
function normalizedReservation(reservation, endpointKind) {
  ...
  const totalAmount = numberOrNull(attr(globalTotal, 'AmountAfterTax') || attr(globalTotal, 'AmountBeforeTax'));
  const payload = {
