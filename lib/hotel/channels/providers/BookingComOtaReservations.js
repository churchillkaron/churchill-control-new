...
function bookingAmount(node) {
  ...
  if (!/^\d+$/.test(raw) || (decimalsRaw && !/^\d+$/.test(decimalsRaw)) || !Number.isInteger(decimalPlaces) || decimalPlaces < 0 || decimalPlaces > 6) {
    throw new Error('BOOKING_COM_OTA_AMOUNT_INVALID');
  }
  const minorUnits = Number(raw);
  if (!Number.isSafeInteger(minorUnits)) throw new Error('BOOKING_COM_OTA_AMOUNT_UNSAFE_INTEGER');
  const scale = 10 ** decimalPlaces;
  const amount = minorUnits / scale;
  const digits = String(minorUnits).padStart(decimalPlaces + 1, '0');
  const canonical = decimalPlaces === 0
    ? digits
    : `${digits.slice(0, -decimalPlaces)}.${digits.slice(-decimalPlaces)}`;
  if (!Number.isFinite(amount) || amount.toFixed(decimalPlaces) !== canonical) {
    throw new Error('BOOKING_COM_OTA_AMOUNT_PRECISION_LOSS');
  }
  return amount;
}
