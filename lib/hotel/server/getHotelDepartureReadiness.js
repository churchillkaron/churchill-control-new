...
export function evaluateHotelDepartureReadiness({
  ...
  const operationalDate = dateValue(businessDate);
  const scheduledDeparture = dateValue(booking?.check_out_date);
  const earlyDepartureReviewStatus = normalizedStatus(booking?.early_departure_review_status);
  ...
  if (bookingStatus === "CHECKED_IN" && scheduledDeparture && !operationalDate) {
    blockers.push(issue(
      "OPERATIONAL_DAY_REQUIRED",
      "Property operating day required",
      "Avantiqo needs the governed property operating day before it can decide whether this is a normal or early departure.",
    ));
  }
  ...
  if (
    ...
    scheduledDeparture &&
    operationalDate &&
    scheduledDeparture > operationalDate &&
    earlyDepartureReviewStatus !== "CONFIRMED"
  ) {
    blockers.push(issue(
      "EARLY_DEPARTURE_REVIEW_REQUIRED",
      "Review early departure",
      `This stay is booked through ${scheduledDeparture}. Record the guest's early departure and confirm the unused-night / refund / fee treatment before check-out.`,
    ));
  }
  ...
  return Object.freeze({
    ...
    early_departure_review_status: earlyDepartureReviewStatus || null,
  });
}
