...
function configuredCutoffMinutes(property) {
  const raw = property?.business_day_cutoff_minutes;
  if (raw === null || raw === undefined || raw === "") return null;
  const parsed = Number(raw);
  if (!Number.isInteger(parsed) || parsed < 0 || parsed > 12 * 60) return null;
  return parsed;
}

function compatibilityCutoffMinutes(property) {
  const raw = property?.business_day_cutoff_minutes ?? property?.business_day_start_minutes;
...
export function deriveHotelOperationalDate(property, at = new Date()) {
  const configuredTimezone = validTimezone(property?.time_zone);
  const configuredCutoff = configuredCutoffMinutes(property);
  const explicitlyConfigured = Boolean(
    configuredTimezone &&
    configuredCutoff !== null &&
    clean(property?.operational_day_configured_at),
  );

  // Legacy fields can preserve a useful compatibility display while an operator repairs
  // property settings, but they can never make the operating day authoritative. Database
  // Day Close uses only the canonical settings above and fails closed until they exist.
  const compatibilityTimezone = configuredTimezone || validTimezone(property?.timezone);
  const timezone = compatibilityTimezone || "UTC";
  const cutoff = configuredCutoff ?? compatibilityCutoffMinutes(property);
  ...
  return Object.freeze({
    ...
    compatibilityFallback: !explicitlyConfigured,
  });
}
