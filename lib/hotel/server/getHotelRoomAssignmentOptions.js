...
export async function getHotelRoomAssignmentOptions({ organizationId, bookingId }) {
  ...
  const roomResults = await Promise.all([
    ...
    supabaseAdmin.from("hotel_maintenance_requests").select("id,room_id,status,priority,issue_title,created_at").eq("organization_id", organization),
  ]);
  ...
  const propertyRoomIds = new Set(rooms.map((room) => room.id));
  ...
  for (const task of housekeeping) {
    if (!task.room_id || !propertyRoomIds.has(task.room_id)) continue;
  ...
  for (const request of maintenance) {
    if (!request.room_id || !propertyRoomIds.has(request.room_id) || CLOSED_MAINTENANCE_STATUSES.has(upper(request.status))) continue;
