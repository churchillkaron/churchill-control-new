...
export async function getHotelShiftHandover({ supabase, organizationId, propertyId }) {
  ...
  const [bookingsResult, foliosResult, housekeepingResult, channelResult, contextResult] = await Promise.all([
    ...
    supabase.from("hotel_housekeeping_tasks").select("id,booking_id,room_id,task_type,task_status,scheduled_at,updated_at,hotel_rooms(room_number,status)").eq("organization_id", organization).eq("property_id", property).in("task_status", ACTIVE_HOUSEKEEPING_STATUSES),
    supabase.from("hotel_channel_sync_jobs").select("id,status,last_error,created_at,updated_at").eq("organization_id", organization).eq("property_id", property).in("status", ACTIVE_CHANNEL_SYNC_STATUSES),
  ...
  for (const task of housekeepingResult.data || []) {
    ...
    items.push(workItem({
      ...
      route: "housekeeping", businessDate, bookingId: task.booking_id || null, roomId: task.room_id || null, propertyId: property,
    }));
  }
