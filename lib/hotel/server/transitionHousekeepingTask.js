...
async function requireMaintenanceClearForRelease({ supabase, organizationId, roomId }) {
  ...
  const unresolved = (data || []).find((request) => !TERMINAL_MAINTENANCE.has(normalizedStatus(request.status)));
  if (unresolved) {
    throw new HousekeepingTransitionError("Room still has unresolved or unclassified maintenance and cannot be released to Front Desk", 409);
  }
}
