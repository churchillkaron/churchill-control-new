...
export async function reconcileAvantiqoArenaWeaknessCurriculum({organizationId=learningOrganizationId()}={}){
  ...
  const failures=await supabaseAdmin.from(MEMORY_TABLE).select("subject,metadata,updated_at").eq("organization_id",organizationId).eq("memory_scope",FAILURE_SCOPE).eq("active",true).order("updated_at",{ascending:false}).limit(200);
  ...
  return {success:true,status:"WEAKNESS_CURRICULUM_READY",contract:AVANTIQO_ARENA_WEAKNESS_CURRICULUM_CONTRACT,arena_fingerprint:fingerprint,selected_dimensions:ranked,local_synthetic_only:true,hidden_benchmark_answers_exposed:false,external_provider_spend_allowed:false,automatic_training_started:false};
}
