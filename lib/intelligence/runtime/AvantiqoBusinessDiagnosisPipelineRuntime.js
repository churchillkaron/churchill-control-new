...
export function completeBusinessDiagnosisPipeline({ plan, baseline_value, actual_value, driver_rows = [], comparison_results = [], external_evidence = [], residual_materiality_threshold = 0.05 } = {}) {
  ...
  const evidence_bundle = comparison_results.length
    ...
    : null;
  const effectiveRows = evidence_bundle ? evidence_bundle.contribution_rows : driver_rows;
  const variance = decomposeBusinessVariance({ metric: plan.metric, baseline_value, actual_value, driver_rows: effectiveRows, external_evidence: [] });
