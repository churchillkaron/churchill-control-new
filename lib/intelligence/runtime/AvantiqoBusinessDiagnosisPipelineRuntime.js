...
import { selectBusinessFactorDecomposition } from "./AvantiqoBusinessFactorDecompositionSelectorRuntime.js";
...
export function completeBusinessDiagnosisPipeline({ plan, baseline_value, actual_value, target_dimension_key = null, driver_rows = [], comparison_results = [], factor_observations = [], external_evidence = [], residual_materiality_threshold = 0.05 } = {}) {
  ...
  const factor_decomposition = factor_observations.length
    ? selectBusinessFactorDecomposition({ metric: plan.metric, factor_observations, target_metric })
    : null;
  const effectiveRows = accounting_decomposition?.status === "DECOMPOSITION_READY" && accounting_decomposition.driver_rows.length
    ...
    : factor_decomposition?.status === "DECOMPOSITION_READY" && factor_decomposition.driver_rows.length
      ? factor_decomposition.driver_rows
  ...
  const internalCoverageIncomplete = Boolean(evidence_bundle) && evidence_bundle.coverage_complete !== true;
  const numericReconciliationComplete = residualMaterial === false;
  const finalEvidenceState = internalCoverageIncomplete
    ? (causal.supported_context_ids.length ? "INTERNAL_COVERAGE_INCOMPLETE_WITH_SUPPORTED_EXTERNAL" : "INTERNAL_COVERAGE_INCOMPLETE")
    : causal.supported_context_ids.length
      ? "INTERNAL_AND_SUPPORTED_EXTERNAL"
      : residualMaterial
  ...
  return {
    ...
    causal,
    internal_coverage_incomplete: internalCoverageIncomplete,
    numeric_reconciliation_complete: numericReconciliationComplete,
    final_evidence_state: finalEvidenceState,
    policy: {
      ...
      factor_model_requires_exact_scope_unit_basis_and_target_reconciliation: true,
      no_mutation_authority_from_diagnosis: true,
    },
    authority_effect: "NONE",
  };
