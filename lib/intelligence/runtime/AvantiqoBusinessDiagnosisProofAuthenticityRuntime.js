...
export function verifyBusinessDiagnosisProofScope(proof = {}, { organization_id = null, conversation_id = null, entity_id = null, period_id = null, require_period_context = false } = {}) {
  ...
  if (require_period_context === true && storedPeriod && !expectedPeriod) {
    return { status: "SCOPE_PERIOD_CONTEXT_MISSING", verified: false, legacy: false };
  }
  if (expectedPeriod && storedPeriod && storedPeriod !== expectedPeriod) {
    return { status: "SCOPE_PERIOD_MISMATCH", verified: false, legacy: false };
  }
  if (expectedPeriod && !storedPeriod) {
    return { status: "SCOPE_PERIOD_MISSING", verified: false, legacy: false };
  }
