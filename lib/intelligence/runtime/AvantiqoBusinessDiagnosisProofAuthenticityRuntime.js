...
export const AVANTIQO_BUSINESS_DIAGNOSIS_PROOF_AUTHENTICITY_ALGORITHM =
  "HMAC-SHA256";
export const AVANTIQO_BUSINESS_DIAGNOSIS_SCOPE_CHECKSUM_CONTRACT =
  "AVANTIQO_BUSINESS_DIAGNOSIS_SCOPE_CHECKSUM_V1";
...
const SCOPE_CHECKSUM_REQUIRED_ENV = "AVANTIQO_BUSINESS_DIAGNOSIS_SCOPE_CHECKSUM_REQUIRED";
const ORIGIN_USER_REQUIRED_ENV = "AVANTIQO_BUSINESS_DIAGNOSIS_ORIGIN_USER_REQUIRED";
...
function scopeChecksumRequired(env = process.env) {
  return text(env?.[SCOPE_CHECKSUM_REQUIRED_ENV], 20).toLowerCase() === "true";
}


function originUserRequired(env = process.env) {
  return text(env?.[ORIGIN_USER_REQUIRED_ENV], 20).toLowerCase() === "true";
}
...
function scopePayload(proof = {}) {
  return {
    ...
    scope_period_id: text(proof?.scope_period_id, 160) || null,
    scope_user_turn_id: text(proof?.scope_user_turn_id, 160) || null,
    scope_user_content_fingerprint: text(proof?.scope_user_content_fingerprint, 64).toLowerCase() || null,
  };
}

export function businessDiagnosisUserTurnContentFingerprint(content = "") {
  const canonicalContent = String(content ?? "").trim();
  return createHash("sha256")
    .update("AVANTIQO_BUSINESS_DIAGNOSIS_USER_TURN_CONTENT_V1\u0000", "utf8")
    .update(canonicalContent, "utf8")
    .digest("hex");
}

export function businessDiagnosisScopeChecksum(proof = {}) {
  return createHash("sha256")
    .update(`${AVANTIQO_BUSINESS_DIAGNOSIS_SCOPE_CHECKSUM_CONTRACT}\u0000diagnosis-scope\u0000`, "utf8")
    .update(stableJson(scopePayload(proof)), "utf8")
    .digest("hex");
}

export function bindBusinessDiagnosisScopeChecksum(proof = {}) {
  const source = proof && typeof proof === "object" && !Array.isArray(proof) ? proof : {};
  return {
    ...source,
    scope_checksum_contract: AVANTIQO_BUSINESS_DIAGNOSIS_SCOPE_CHECKSUM_CONTRACT,
    scope_checksum: businessDiagnosisScopeChecksum(source),
  };
}

export function verifyBusinessDiagnosisScopeChecksum(proof = {}) {
  const contract = text(proof?.scope_checksum_contract, 160) || null;
  const supplied = text(proof?.scope_checksum, 64).toLowerCase() || null;
  const hasScope = Object.values(scopePayload(proof)).some(Boolean);
  if (!hasScope && !contract && !supplied) return { status: "SCOPE_CHECKSUM_NOT_APPLICABLE", verified: true, legacy: true };
  if (!contract && !supplied) return { status: "SCOPE_CHECKSUM_NOT_AVAILABLE", verified: true, legacy: true };
  if (contract !== AVANTIQO_BUSINESS_DIAGNOSIS_SCOPE_CHECKSUM_CONTRACT || !MAC_HEX_RE.test(supplied || "")) {
    return { status: "SCOPE_CHECKSUM_FORMAT_INVALID", verified: false, legacy: false };
  }
  const expected = businessDiagnosisScopeChecksum(proof);
  const verified = safeEqualHex(supplied, expected);
  return { status: verified ? "SCOPE_CHECKSUM_VERIFIED" : "SCOPE_CHECKSUM_MISMATCH", verified, legacy: false, expected_checksum: expected };
}
...
function authenticityPayload(proof = {}) {
  return {
    ...
    scope_user_turn_id: text(proof?.scope_user_turn_id, 160) || null,
    scope_user_content_fingerprint: text(proof?.scope_user_content_fingerprint, 64).toLowerCase() || null,
    scope_checksum_contract: text(proof?.scope_checksum_contract, 160) || null,
    scope_checksum: text(proof?.scope_checksum, 64).toLowerCase() || null,
  };
}
...
export function getBusinessDiagnosisProofAuthenticityStatus({ env = process.env, require_active = false } = {}) {
  ...
  return {
    ...
    scope_checksum_required: scopeChecksumRequired(env),
    origin_user_required: originUserRequired(env),
...
export function verifyBusinessDiagnosisProofScope(proof = {}, { organization_id = null, conversation_id = null, entity_id = null, period_id = null, require_period_context = false } = {}) {
  const checksumVerification = verifyBusinessDiagnosisScopeChecksum(proof);
  if (!checksumVerification.verified) {
    return { status: checksumVerification.status, verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  ...
  const storedPeriod = text(proof?.scope_period_id, 160) || null;
  ...
  const expectedPeriod = text(period_id, 160) || null;
  if (!storedOrganization && !storedConversation && !storedEntity && !storedPeriod) {
    return { status: "SCOPE_NOT_BOUND", verified: true, legacy: true, checksum_status: checksumVerification.status };
  }
  if (!storedOrganization || !storedConversation) {
    return { status: "SCOPE_INCOMPLETE", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (expectedOrganization && storedOrganization !== expectedOrganization) {
    return { status: "SCOPE_ORGANIZATION_MISMATCH", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (expectedConversation && storedConversation !== expectedConversation) {
    return { status: "SCOPE_CONVERSATION_MISMATCH", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (expectedEntity && storedEntity && storedEntity !== expectedEntity) {
    return { status: "SCOPE_ENTITY_MISMATCH", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (expectedEntity && !storedEntity) {
    return { status: "SCOPE_ENTITY_MISSING", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (require_period_context === true && storedPeriod && !expectedPeriod) {
    return { status: "SCOPE_PERIOD_CONTEXT_MISSING", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (expectedPeriod && storedPeriod && storedPeriod !== expectedPeriod) {
    return { status: "SCOPE_PERIOD_MISMATCH", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  if (expectedPeriod && !storedPeriod) {
    return { status: "SCOPE_PERIOD_MISSING", verified: false, legacy: false, checksum_status: checksumVerification.status };
  }
  return { status: "SCOPE_VERIFIED", verified: true, legacy: false, checksum_status: checksumVerification.status };
}


export function businessDiagnosisProofScopeAcceptable(verification = {}, { env = process.env } = {}) {
  const checksumRequired = scopeChecksumRequired(env);
  if (verification?.status === "SCOPE_VERIFIED") {
    if (checksumRequired && verification?.checksum_status !== "SCOPE_CHECKSUM_VERIFIED") return false;
    return true;
  }
  if (verification?.status === "SCOPE_NOT_BOUND") return !scopeRequired(env) && !checksumRequired;
  return verification?.verified === true && !scopeRequired(env) && !checksumRequired;
}

export function verifyBusinessDiagnosisOriginatingUserTurn(proof = {}, userTurn = {}) {
  const expectedId = text(proof?.scope_user_turn_id, 160) || null;
  const expectedFingerprint = text(proof?.scope_user_content_fingerprint, 64).toLowerCase() || null;
  if (!expectedId && !expectedFingerprint) return { status: "ORIGIN_USER_NOT_BOUND", verified: true, legacy: true };
  if (!expectedId || !expectedFingerprint) return { status: "ORIGIN_USER_BINDING_INCOMPLETE", verified: false, legacy: false };
  const actualId = text(userTurn?.id, 160) || null;
  if (!actualId || actualId !== expectedId) return { status: "ORIGIN_USER_ID_MISMATCH", verified: false, legacy: false };
  const actualFingerprint = businessDiagnosisUserTurnContentFingerprint(userTurn?.content || "");
  const verified = safeEqualHex(expectedFingerprint, actualFingerprint);
  return { status: verified ? "ORIGIN_USER_VERIFIED" : "ORIGIN_USER_CONTENT_MISMATCH", verified, legacy: false };
}


export function businessDiagnosisOriginatingUserAcceptable(verification = {}, { env = process.env } = {}) {
  if (verification?.status === "ORIGIN_USER_VERIFIED") return true;
  if (verification?.status === "ORIGIN_USER_NOT_BOUND") return !originUserRequired(env);
  return verification?.verified === true && !originUserRequired(env);
}
...
export function redactBusinessDiagnosisProofForClient(proof = {}) {
  ...
  const {
    ...
    scope_period_id,
    scope_user_turn_id,
    scope_user_content_fingerprint,
    scope_checksum_contract,
    scope_checksum,
    ...safe
  } = source;
  return safe;
...
export const BusinessDiagnosisProofAuthenticityRuntime = Object.freeze({
  ...
  userTurnContentFingerprint: businessDiagnosisUserTurnContentFingerprint,
  verifyOriginatingUserTurn: verifyBusinessDiagnosisOriginatingUserTurn,
  originUserAcceptable: businessDiagnosisOriginatingUserAcceptable,
  bindScopeChecksum: bindBusinessDiagnosisScopeChecksum,
  verifyScopeChecksum: verifyBusinessDiagnosisScopeChecksum,
