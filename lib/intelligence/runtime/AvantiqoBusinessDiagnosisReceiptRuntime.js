...
function diagnosisAuditProjection({
  receipt_fingerprint=null, diagnosis_class=null, final_evidence_state=null, residual_material=false,
...
export function businessDiagnosisAuditProjectionFingerprint(input={}){
  return sha(stable(diagnosisAuditProjection(input)));
}
...
export function verifyBusinessDiagnosisAuditProjection(evidence={}){
  ...
  const expected=businessDiagnosisAuditProjectionFingerprint({
  ...
  return {status:expected===fingerprint?"VERIFIED":"MISMATCH",verified:expected===fingerprint,expected_fingerprint:expected};
}
...
export function buildBusinessDiagnosisReceipt({organization_id=null,entity_id=null,metric=null,diagnosis_class=null,baseline_period_id=null,baseline_period_start_date=null,baseline_period_end_date=null,current_period_id=null,current_period_start_date=null,current_period_end_date=null,final_diagnosis=null,external_research_plan=null,external_evidence_assessment=null,external_diagnosis_closure=null,answer_brief=null,answer_boundary=null}={}){
  ...
  const auditProjectionFingerprint=businessDiagnosisAuditProjectionFingerprint({
    receipt_fingerprint:fingerprint,diagnosis_class:payload.diagnosis_class,final_evidence_state:payload.final_evidence_state,residual_material:payload.residual_material,
  ...
  return {...payload,receipt_fingerprint:fingerprint,audit_projection_fingerprint:auditProjectionFingerprint};
}

export const AvantiqoBusinessDiagnosisReceiptRuntime=Object.freeze({contract:AVANTIQO_BUSINESS_DIAGNOSIS_RECEIPT_CONTRACT,build:buildBusinessDiagnosisReceipt,auditFingerprint:businessDiagnosisAuditProjectionFingerprint,verifyAudit:verifyBusinessDiagnosisAuditProjection});
