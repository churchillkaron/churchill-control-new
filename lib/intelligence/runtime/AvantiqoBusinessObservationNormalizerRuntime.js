...
const arrayField = (o, k) => has(o, k) && Array.isArray(o[k]);
const schemaGap = (reason="SCHEMA_GAP") => ({ status: reason, series:[] });

function currencyRows(payload={}) {
  if (!arrayField(payload, "currency_positions")) return null;
  return payload.currency_positions.map(r => ({
...
function summarize(capabilityKey, raw) {
  ...
  switch (capabilityKey) {
    case "finance.cash_management.read": {
      const rows=currencyRows(payload);
      return rows===null ? schemaGap() : { status:"OK", series: rows };
    }
    case "finance.profit_loss.read": {
      if (!payload?.document || !payload.document.summary) return schemaGap();
      const summary=obj(payload.document.summary);
      if (!["revenue","cogs","expenses","netProfit"].every(k=>has(summary,k))) return schemaGap();
    ...
    case "supply_chain.stock_position.read":
      if (!payload?.metrics || (!has(payload.metrics,"totalQuantity") && !has(payload.metrics,"itemCount"))) return schemaGap();
    ...
    case "people.attendance.read":
      if (!["staff","schedules","shifts","attendance","lateShifts","pendingShifts"].every(k=>arrayField(payload,k))) return schemaGap();
      return { status:"OK", series:[{key:"count",values:{staff_count:payload.staff.length,scheduled_shifts:payload.schedules.length,worked_shifts:payload.shifts.length,attendance_records:payload.attendance.length,late_shifts:payload.lateShifts.length,pending_shifts:payload.pendingShifts.length}}] };
    case "finance.customer_invoices.read": {
      if (!arrayField(payload,"invoices")) return schemaGap();
    ...
    case "solutions.hotel_bookings.read":
      if (!arrayField(payload,"bookings")) return schemaGap();
      return { status:"OK", series:[{key:"count",values:{booking_count:payload.bookings.length}}] };
    case "commercial.customers.read": {
      const explicit=has(payload,"rowCount") ? finite(payload.rowCount) : null;
      const rows=arrayField(payload,"customers") ? payload.customers : (arrayField(payload,"rows") ? payload.rows : null);
      if (explicit===null && rows===null) return schemaGap();
      return { status:"OK", series:[{key:"count",values:{customer_count:explicit ?? rows.length}}] };
    }
    case "commercial.quotations.read":
      if (!arrayField(payload,"rows")) return schemaGap();
      return { status:"OK", series:[{key:"count",values:{quotation_count:payload.rows.length}}] };
    case "supply_chain.inventory_items.read":
      if (!arrayField(payload,"items")) return schemaGap();
      return { status:"OK", series:[{key:"count",values:{inventory_item_count:payload.items.length}}] };
    default:
