...
function aggregateCaseScores(caseResults = [], requiredCaseIds = []) {
  ...
  for (const dimension of BUSINESS_PARTNER_QUALITY_DIMENSIONS) {
    const values = requiredCaseIds
      .map((id) => finite(object(byId.get(id)?.scores)[dimension]))
      .filter((value) => value !== null);
    dimensionScores[dimension] = values.length === requiredCaseIds.length
...
export function evaluateBusinessPartnerBenchmarkEvidence({
  ...
  const requiredCaseIds = list(suite.cases).map((item) => text(item.id, 160));
  ...
  const candidate = aggregateCaseScores(object(source.candidate).cases, requiredCaseIds);
  const references = {};
  const missingReferenceCases = {};
  for (const family of BUSINESS_PARTNER_REFERENCE_FAMILIES) {
    const aggregate = aggregateCaseScores(object(object(source.references)[family]).cases, requiredCaseIds);
