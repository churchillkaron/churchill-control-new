...
function validCaseEvidence(item = {}, suiteCase = {}) {
  const source = object(item);
  const required = list(suiteCase.requires);
  const scores = object(source.scores);
  const requiredScoresValid = required.every((dimension) => {
    const value = finite(scores[dimension]);
    return value !== null && value >= 0 && value <= 1;
  });
  return Boolean(
    text(source.case_id, 160) === text(suiteCase.id, 160) &&
    requiredScoresValid &&
    text(source.scorer_contract, 180) === "AVANTIQO_BUSINESS_PARTNER_DETERMINISTIC_CASE_SCORER_V1" &&
    text(source.score_provenance, 80) === "MEASURED" &&
    sha256(source.raw_output_sha256) &&
    sha256(source.tool_trace_sha256) &&
    sha256(source.evidence_packet_sha256) &&
    finite(source.latency_ms) !== null &&
    finite(source.latency_ms) >= 0
  );
}
...
function aggregateCaseScores(caseResults = [], suiteCases = []) {
  ...
  const invalid = list(suiteCases)
    .filter((suiteCase) => {
      const id = text(suiteCase.id, 160);
      return byId.has(id) && !validCaseEvidence(byId.get(id), suiteCase);
    })
    .map((suiteCase) => text(suiteCase.id, 160));
...
export function evaluateBusinessPartnerBenchmarkEvidence({
  ...
  const invalidReferenceCases = Object.fromEntries(
    BUSINESS_PARTNER_REFERENCE_FAMILIES.map((family) => [
      family,
      aggregateCaseScores(object(object(source.references)[family]).cases, suiteCases).invalid_case_ids,
    ]),
  );

  const completeCaseEvidence =
    candidate.missing_case_ids.length === 0 &&
    candidate.invalid_case_ids.length === 0 &&
    BUSINESS_PARTNER_REFERENCE_FAMILIES.every(
      (family) =>
        missingReferenceCases[family].length === 0 &&
        invalidReferenceCases[family].length === 0,
    );
  ...
  return {
    ...
    invalid_candidate_cases: candidate.invalid_case_ids,
    missing_reference_cases: missingReferenceCases,
    invalid_reference_cases: invalidReferenceCases,
