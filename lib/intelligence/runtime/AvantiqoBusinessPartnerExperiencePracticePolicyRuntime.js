...
export function buildAvantiqoExperiencePracticeCases(summaries=[]){
  const candidates=[];
  for(const summary of list(summaries)){
    ...
    for(const [failureClass,count] of failures){
      ...
      const priority=count*(1+experienceStrength);
      candidates.push({capability_key:text(summary.capability_key,300),capability_domain:text(summary.capability_domain,120)||null,failure_class:failureClass,failure_count:count,experience_strength:experienceStrength,priority,expected:drill.expected,scenario:`Capability ${text(summary.capability_key,300)}. ${drill.scenario}`});
    }
  }
