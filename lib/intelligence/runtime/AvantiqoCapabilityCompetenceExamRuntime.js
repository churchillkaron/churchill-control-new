...
export async function runAvantiqoCapabilityCompetenceExam({ organizationId = learningOrganizationId() } = {}) {
  ...
  const targetCoverage = list(coverageResult.data).sort((a,b) => Number(object(b.metadata).learning_priority_score||0)-Number(object(a.metadata).learning_priority_score||0) || Number(object(a.metadata).intelligence_coverage_score||0)-Number(object(b.metadata).intelligence_coverage_score||0) || a.subject.localeCompare(b.subject))[0];
