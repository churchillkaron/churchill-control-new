...
export async function runAvantiqoCapabilityCompetenceExam({ organizationId = learningOrganizationId() } = {}) {
  ...
  const [coverageResult, catalog] = await Promise.all([
  ...
  const learningEvidenceFingerprint = hash(JSON.stringify({
    exact_knowledge_count:Number(coverageMetadata.exact_knowledge_count||0),
    verified_outcome_count:Number(coverageMetadata.verified_outcome_count||0),
