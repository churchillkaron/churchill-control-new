...
export async function reconcileAvantiqoCapabilityIntelligenceCurriculum({ organizationId = null } = {}) {
  ...
  const assessments = list(capabilities).map((capability) => ({ capability, coverage:coverageFor(capability,list(knowledge.data),list(outcomes.data),competenceRows,list(readiness.data)) }))
