...
export async function reconcileAvantiqoCapabilityIntelligenceCurriculum({ organizationId = null } = {}) {
  ...
  const assessments = list(capabilities).map((capability) => {
    ...
    const learning_priority=scoreAvantiqoCapabilityLearningPriority({capability,coverage,experience,dependency});
    return { capability, coverage, learning_priority, experience, dependency };
  ...
  const coverageRows = assessments.map(({capability,coverage,learning_priority,experience,dependency}) => {
