...
export async function reconcileAvantiqoCapabilityIntelligenceCurriculum({ organizationId = null } = {}) {
  ...
  const [capabilities, knowledge, outcomes, competence, readiness, existingAgendas] = await Promise.all([
  ...
  const assessments = list(capabilities).map((capability) => {
    ...
    const learning_priority=scoreAvantiqoCapabilityLearningPriority({capability,coverage});
    return { capability, coverage, learning_priority };
  ...
  const coverageRows = assessments.map(({capability,coverage,learning_priority}) => {
