...
import { assessCapabilityReadinessEvidence } from "@/lib/intelligence/runtime/AvantiqoCapabilityReadinessEvidenceRuntime";
import { scoreAvantiqoCapabilityLearningPriority } from "@/lib/intelligence/runtime/AvantiqoCapabilityLearningPriorityRuntime";
import { deriveAvantiqoCapabilityDependencyPriority } from "@/lib/intelligence/runtime/AvantiqoCapabilityDependencyPriorityRuntime";
import { summarizeAvantiqoBusinessPartnerExperience } from "@/lib/intelligence/runtime/AvantiqoBusinessPartnerExperienceRuntime";
import { assessAvantiqoCapabilityCriticality } from "@/lib/intelligence/runtime/AvantiqoCapabilityCriticalityRuntime";
...
const ADAPTIVE_FAILURE_SOURCE = "adaptive_execution_learning";
...
function coverageFor(capability, knowledgeRows, outcomeRows, competenceRows, readinessRows) {
  ...
  const readinessEvidence = assessCapabilityReadinessEvidence({outcomeRows,readinessRows,capabilityKey:capability.key});
  ...
  return { score:Number(score.toFixed(4)), catalog_score:catalogScore, knowledge_count:knowledgeCount, outcome_count:outcomeEvidence.counted_outcome_count, outcome_score:outcomeScore, outcome_weighted_evidence_units:outcomeEvidence.weighted_evidence_units, outcome_weighted_success_units:outcomeEvidence.weighted_success_units, outcome_weighted_failure_units:outcomeEvidence.weighted_failure_units, outcome_reliability_estimate:outcomeEvidence.reliability_estimate, outcome_reliability_confidence:outcomeEvidence.reliability_confidence, outcome_experience_strength:outcomeEvidence.experience_strength, outcome_reliability_maturity:outcomeEvidence.reliability_maturity, live_verified_outcome_count:outcomeEvidence.live_verified_count, historical_backfill_outcome_count:outcomeEvidence.historical_backfill_count, verified_failure_count:outcomeEvidence.verified_failure_count, readiness:readinessEvidence, verification_score:verificationScore, competence_score:Number(competenceScore.toFixed(4)), gaps };
}
...
function coverageRow(organizationId, capability, coverage, now) {
  return {
    ...
    metadata:{contract:AVANTIQO_CAPABILITY_INTELLIGENCE_CURRICULUM_CONTRACT,capability_key:capability.key,domain:capability.domain,
      ...
      verified_outcome_count:coverage.outcome_count,verified_outcome_score:coverage.outcome_score,weighted_verified_outcome_units:coverage.outcome_weighted_evidence_units,weighted_verified_success_units:coverage.outcome_weighted_success_units,weighted_verified_failure_units:coverage.outcome_weighted_failure_units,capability_reliability_estimate:coverage.outcome_reliability_estimate,capability_reliability_confidence:coverage.outcome_reliability_confidence,capability_reliability_experience_strength:coverage.outcome_experience_strength,capability_reliability_maturity:coverage.outcome_reliability_maturity,live_verified_outcome_count:coverage.live_verified_outcome_count,historical_backfill_outcome_count:coverage.historical_backfill_outcome_count,verified_failure_count:coverage.verified_failure_count,capability_readiness_status:coverage.readiness.status,capability_readiness_score:coverage.readiness.score,readiness_success_evidence_units:coverage.readiness.success_evidence_units,readiness_prerequisite_friction_units:coverage.readiness.prerequisite_friction_units,readiness_prerequisite_failure_family_count:coverage.readiness.prerequisite_failure_family_count,readiness_prerequisite_failure_occurrence_count:coverage.readiness.prerequisite_failure_occurrence_count,readiness_is_contextual_observation:true,readiness_is_not_capability_reliability:true,readiness_is_not_execution_authority:true,verification_score:coverage.verification_score,competence_score:coverage.competence_score,intelligence_coverage_score:coverage.score,
...
export async function reconcileAvantiqoCapabilityIntelligenceCurriculum({ organizationId = null } = {}) {
  ...
  const [capabilities, knowledge, outcomes, competence, readiness, existingAgendas, experienceSummary, experienceRows] = await Promise.all([
    ...
    supabaseAdmin.from(MEMORY_TABLE).select("subject,metadata,active,updated_at").eq("organization_id",organizationId).eq("source",ADAPTIVE_FAILURE_SOURCE).eq("active",true).limit(5000),
    ...
    summarizeAvantiqoBusinessPartnerExperience({ organizationId }),
    supabaseAdmin.from(MEMORY_TABLE).select("subject,metadata,created_at").eq("organization_id",organizationId).eq("memory_scope","platform_business_partner_experience").eq("active",true).order("created_at",{ascending:false}).limit(5000),
  ]);
  ...
  if (experienceRows.error) throw experienceRows.error;
  ...
  const experienceByCapability = new Map(list(experienceSummary?.summaries).map((item) => [text(item.capability_key,300), item]));
  const dependencyByCapability = new Map(deriveAvantiqoCapabilityDependencyPriority({capabilities:list(capabilities),experienceRows:list(experienceRows.data)}).map((item)=>[item.capability_key,item]));
  const assessments = list(capabilities).map((capability) => {
    const coverage=coverageFor(capability,list(knowledge.data),list(outcomes.data),competenceRows,list(readiness.data));
    const experience=object(experienceByCapability.get(capability.key));
    const dependency=object(dependencyByCapability.get(capability.key));
    const criticality=assessAvantiqoCapabilityCriticality({capability,dependency,experience});
    const learning_priority=scoreAvantiqoCapabilityLearningPriority({capability,coverage,experience,dependency,criticality});
    return { capability, coverage, learning_priority, experience, dependency, criticality };
  }).sort((a,b) => b.learning_priority.priority-a.learning_priority.priority || a.coverage.score-b.coverage.score || a.capability.key.localeCompare(b.capability.key));
  const coverageRows = assessments.map(({capability,coverage,learning_priority,experience,dependency,criticality}) => {
    const row=coverageRow(organizationId,capability,coverage,now);
    row.metadata.learning_priority_score=learning_priority.priority;
    row.metadata.learning_priority_usage_score=learning_priority.usage_score;
    row.metadata.learning_priority_usage_evidence_units=learning_priority.usage_evidence_units;
    row.metadata.learning_priority_business_risk_score=learning_priority.business_risk_score;
    row.metadata.learning_priority_exploration_credit=learning_priority.exploration_credit;
    row.metadata.business_partner_experience_turn_count=Number(experience.turn_count||0);
    row.metadata.business_partner_experience_strength=Number(experience.experience_strength||0);
    row.metadata.learning_priority_experience_signal_units=learning_priority.experience_signal_units;
    row.metadata.dependency_centrality_score=Number(dependency.dependency_centrality_score||0);
    row.metadata.verification_dependent_count=Number(dependency.verification_dependent_count||0);
    row.metadata.observed_dependency_occurrence_count=Number(dependency.observed_dependency_occurrence_count||0);
    row.metadata.learning_priority_dependency_centrality_score=learning_priority.dependency_centrality_score;
    row.metadata.capability_criticality_score=criticality.criticality_score;
    row.metadata.capability_criticality_tier=criticality.criticality_tier;
    row.metadata.capability_criticality_policy=criticality.tier_policy;
    row.metadata.learning_priority_criticality_score=learning_priority.criticality_score;
    row.metadata.criticality_is_not_authority=true;
    row.metadata.dependency_centrality_is_not_authority=true;
    row.metadata.learning_priority_is_not_authority=true;
    return row;
  });
  ...
  return { success:true,status:"CAPABILITY_CURRICULUM_READY",contract:AVANTIQO_CAPABILITY_INTELLIGENCE_CURRICULUM_CONTRACT,
    ...
    selected_learning_capability_key:selected?.capability?.key || null,selected_learning_capability_coverage:selected?.coverage?.score ?? null,selected_learning_priority:selected?.learning_priority?.priority ?? null,
    tier_a_capability_count:assessments.filter(x=>x.criticality.criticality_tier==="A").length,tier_b_capability_count:assessments.filter(x=>x.criticality.criticality_tier==="B").length,tier_c_capability_count:assessments.filter(x=>x.criticality.criticality_tier==="C").length,
    tier_a_low_coverage_count:assessments.filter(x=>x.criticality.criticality_tier==="A"&&x.coverage.score<0.85).length,
