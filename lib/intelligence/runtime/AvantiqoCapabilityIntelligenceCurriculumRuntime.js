...
function coverageFor(capability, knowledgeRows, outcomeRows) {
  const knowledgeCount = knowledgeRows.filter((row) => exactCapability(row) === capability.key).length;
  const outcomeCount = outcomeRows.filter((row) => exactCapability(row) === capability.key).length;
  const catalogScore = catalogCompleteness(capability);
  const knowledgeScore = Math.min(1, knowledgeCount / 3);
  const outcomeScore = Math.min(1, outcomeCount / 5);
  const verificationScore = capability.operator_verification ? 1 : capability.mode === "read" ? 0.7 : 0;
  const score = catalogScore * 0.3 + knowledgeScore * 0.3 + outcomeScore * 0.25 + verificationScore * 0.15;
  const gaps = [];
  if (catalogScore < 0.8) gaps.push("CATALOG_SEMANTICS_INCOMPLETE");
  if (knowledgeCount < 2) gaps.push("EXACT_CAPABILITY_KNOWLEDGE_THIN");
  if (outcomeCount < 3) gaps.push("VERIFIED_OUTCOME_EXPERIENCE_THIN");
  if (!capability.operator_verification && capability.mode !== "read") gaps.push("VERIFICATION_CONTRACT_MISSING");
  return { score:Number(score.toFixed(4)), catalog_score:catalogScore, knowledge_count:knowledgeCount, outcome_count:outcomeCount, verification_score:verificationScore, gaps };
}
...
function coverageRow(organizationId, capability, coverage, now) {
  return {
    ...
    memory_scope:COVERAGE_SCOPE, memory_key:`capability-intelligence:${hash(capability.key).slice(0,40)}`, memory_type:"assessment",
    ...
    metadata:{contract:AVANTIQO_CAPABILITY_INTELLIGENCE_CURRICULUM_CONTRACT,capability_key:capability.key,domain:capability.domain,
      ...
      verified_outcome_count:coverage.outcome_count,verification_score:coverage.verification_score,intelligence_coverage_score:coverage.score,
...
export async function reconcileAvantiqoCapabilityIntelligenceCurriculum({ organizationId = learningOrganizationId() } = {}) {
  if (!organizationId) return { success:true,status:"DISABLED",reason:"LEARNING_ORGANIZATION_ID_REQUIRED",contract:AVANTIQO_CAPABILITY_INTELLIGENCE_CURRICULUM_CONTRACT };
  const [capabilities, knowledge, outcomes, existingAgendas] = await Promise.all([
  ...
  const assessments = list(capabilities).map((capability) => ({ capability, coverage:coverageFor(capability,list(knowledge.data),list(outcomes.data)) }))
