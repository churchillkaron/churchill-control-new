...
export async function reconcileAvantiqoCapabilityIntelligenceCurriculum({ organizationId = null } = {}) {
  ...
  const [capabilities, knowledge, outcomes, competence, readiness, existingAgendas, experienceSummary] = await Promise.all([
  ...
  const assessments = list(capabilities).map((capability) => {
    ...
    const learning_priority=scoreAvantiqoCapabilityLearningPriority({capability,coverage,experience});
    return { capability, coverage, learning_priority, experience };
  ...
  const coverageRows = assessments.map(({capability,coverage,learning_priority,experience}) => {
