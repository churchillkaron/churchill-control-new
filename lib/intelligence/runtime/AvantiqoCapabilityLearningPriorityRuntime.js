...
export function scoreAvantiqoCapabilityLearningPriority({capability={},coverage={},experience={},dependency={},criticality={}}={}){
  ...
  const criticalityScore=bounded(criticality.criticality_score,0);
  const exploration=usageEvidence===0?0.05:0;
  const priority=gap*(0.24+usage*0.32+risk*0.12+dependencyCentrality*0.12+criticalityScore*0.2)+exploration*gap;
  return {
    ...
    criticality_score:Number(criticalityScore.toFixed(4)),
    criticality_tier:String(criticality.criticality_tier||"C"),
