...
export function routeAvantiqoCognition({
  ...
  const businessDriverDiagnosisRequired = BUSINESS_DRIVER_DIAGNOSIS_PATTERN.test(goalText) || ctx.business_driver_diagnosis_required === true;
  ...
  if (businessDriverDiagnosisRequired) {
    deepScore = Math.max(deepScore + 0.2, 0.58);
    addReason(reasons, "BUSINESS_DRIVER_DIAGNOSIS", 0.2);
  }
  ...
  return {
    ...
    requirements: {
      ...
      business_driver_diagnosis_required: businessDriverDiagnosisRequired,
    },
    signals: {
