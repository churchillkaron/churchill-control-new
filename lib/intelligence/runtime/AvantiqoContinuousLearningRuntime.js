...
import {
  runOperatorWebResearch as collectOperatorWebResearch,
} from "@/lib/platform/research/runtime/OperatorWebResearchRuntime";
import {
...
export async function runKnowledgeAwareWebResearch({ context = {}, payload = {} } = {}) {
  ...
  const research = await collectOperatorWebResearch({ context, payload });
  return {
    ...research,
    contract: AVANTIQO_KNOWLEDGE_AWARE_RESEARCH_CONTRACT,
    knowledge_reuse: {
...
async function replaceTopicKnowledge({ organizationId, topic, claims, comparison }) {
  if (!claims.length) return [];
  const now = new Date();
  const nowIso = now.toISOString();
  const subject = `knowledge:${topic.key}`;
  ...
  const memoryType = topic.stability === "mutable" ? "fact" : "lesson";
  const rows = claims.map((claim) => ({
    ...
    memory_key: hashKey("knowledge", topic.key, claim.claim),
    memory_type: memoryType,
    subject,
    content: claim.claim,
    importance: Math.min(1, 0.72 + topic.priority * 0.22),
    confidence: claim.confidence,
    source: "verified_continuous_web_research",
    active: true,
...
async function recordLearningRun({ organizationId, topic, status, claimCount, sourceCount, error = null }) {
  ...
  const result = await supabaseAdmin.from(MEMORY_TABLE).insert({
    ...
    subject: topic.key,
    content: successful
      ? `Continuous learning verified ${claimCount} reusable claim(s) for ${topic.key} from ${sourceCount} source(s).`
...
async function processAgendaRow({ organizationId, row }) {
  ...
  try {
    ...
    const research = await collectOperatorWebResearch({
      context,
      payload: {
        query: topic.query,
        objective:
          "Collect current public evidence that can be generalized into reusable Avantiqo product and domain knowledge. Do not collect customer-specific or private information.",
        preferred_domains: topic.preferred_domains,
        freshness_days: topic.freshness_days,
        minimum_sources: 2,
        max_sources: 8,
    ...
    const written = await replaceTopicKnowledge({
      organizationId,
      topic,
      claims: supported,
      comparison,
    ...
    const status = written.length ? "READY" : "INSUFFICIENT_EVIDENCE";
    await updateAgendaAfterRun({
      organizationId,
      row: claimed.row,
      status,
      claimCount: written.length,
