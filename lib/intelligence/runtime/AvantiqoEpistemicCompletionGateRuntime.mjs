...
const RESEARCH_MARGINAL_UTILITY_CONTRACT = "AVANTIQO_RESEARCH_MARGINAL_UTILITY_V1";
...
function normalizedEvidence(value) {
  ...
  return {
    ...
    marginal_utility_contract: text(evidence.marginal_utility_contract, 160) || null,
    research_round: count(evidence.research_round),
    marginal_comparison_available: evidence.marginal_comparison_available === true,
    marginal_new_source_count: count(evidence.marginal_new_source_count),
    marginal_new_independent_source_count: count(evidence.marginal_new_independent_source_count),
    marginal_new_source_backed_claim_count: count(evidence.marginal_new_source_backed_claim_count),
    marginal_uncertainty_reduction_count: count(evidence.marginal_uncertainty_reduction_count),
    marginal_follow_up_reduction_count: count(evidence.marginal_follow_up_reduction_count),
    marginal_conflict_reduction_count: count(evidence.marginal_conflict_reduction_count),
...
function latestSuccessfulMatching(calls, matcher, afterSequence = -1) {
...
function researchEvidence(calls) {
  return latestSuccessfulMatching(
    calls,
...
function researchStoppingEvidence(call) {
  ...
  if (sourceBacked && evidence.independent_source_count >= 2) {
    return { proven: true, reason: "INDEPENDENT_SOURCE_SUPPORT" };
  }
  if (sourceBacked && evidence.official_primary_source_count >= 1) {
    return { proven: true, reason: "OFFICIAL_PRIMARY_SOURCE_SUPPORT" };
  }
  if (sourceBacked && evidence.quality_verified === true) {
    return { proven: true, reason: "SOURCE_BACKED_MECHANISM_SUPPORT" };
  }
  if (evidence.quality_verified === true) {
    return { proven: false, reason: "QUALITY_VERIFICATION_WITHOUT_SOURCE_PROOF" };
  }
  return { proven: false, reason: "SOURCE_BACKED_SUPPORT_INSUFFICIENT" };
}

function diminishingReturnsEvidence(call) {
  const base = researchStoppingEvidence(call);
  if (!base.proven) return base;
  const evidence = object(call?.epistemic_evidence);
  if (evidence.marginal_utility_contract !== RESEARCH_MARGINAL_UTILITY_CONTRACT) {
    return { proven: false, reason: "MARGINAL_UTILITY_ATTESTATION_REQUIRED" };
  }
  if (evidence.research_round < 2 || evidence.marginal_comparison_available !== true) {
    return { proven: false, reason: "MARGINAL_COMPARISON_REQUIRED" };
  }
  const materialGain =
    evidence.marginal_new_source_count +
    evidence.marginal_new_independent_source_count +
    evidence.marginal_new_source_backed_claim_count +
    evidence.marginal_uncertainty_reduction_count +
    evidence.marginal_follow_up_reduction_count +
    evidence.marginal_conflict_reduction_count;
  if (materialGain > 0) {
    return { proven: false, reason: "LATEST_RESEARCH_ROUND_STILL_ADDS_MATERIAL_VALUE" };
  }
  return { proven: true, reason: "OBSERVED_ZERO_MARGINAL_RESEARCH_UTILITY" };
}

function evidenceCanReconcileConflict(call) {
  return Boolean(
    call?.epistemic_evidence &&
    (
      epistemicRoleMatches(call, "research", RESEARCH_TOOL_PATTERN) ||
      epistemicRoleMatches(call, "verification", VERIFICATION_TOOL_PATTERN)
    )
  );
}

function conflictResolutionEvidence(calls, conflictExpectedBeforeTools = false) {
  const evidenceCalls = calls.filter((call) =>
    call.outcome === "succeeded" && evidenceCanReconcileConflict(call)
  );
  const conflictedCalls = evidenceCalls.filter(
    (call) => object(call.epistemic_evidence).conflict_count > 0,
  );
  const latestConflict = conflictedCalls.length
    ? conflictedCalls[conflictedCalls.length - 1]
    : null;
  const required = conflictExpectedBeforeTools || Boolean(latestConflict);
  ...
  if (!required) {
    return {
      ...
      proven: true,
      reason: "NOT_REQUIRED",
      conflict_sequence: null,
      resolution_sequence: null,
    };
  }

  const afterSequence = latestConflict?.sequence ?? -1;
  const laterEvidence = evidenceCalls.filter((call) => call.sequence > afterSequence);
  for (let index = laterEvidence.length - 1; index >= 0; index -= 1) {
    const candidate = laterEvidence[index];
    const proof = researchStoppingEvidence(candidate);
    if (proof.proven) {
      return {
        required: true,
        observed_conflict: Boolean(latestConflict),
        proven: true,
        reason: `TRUSTED_RECONCILIATION_${proof.reason}`,
        conflict_sequence: latestConflict?.sequence ?? null,
        resolution_sequence: candidate.sequence,
      };
    }
  }
...
export function applyAvantiqoEpistemicCompletionGate({
  ...
  const diminishingReturns = diminishingReturnsEvidence(researchCall);
  const liveReadObserved = Boolean(liveReadEvidence(calls));
  const verification = verificationEvidence({ route, calls });
  const conflictExpectedBeforeTools =
    routeHasReason(route, "CONFLICTING_EVIDENCE") ||
    declared.unresolved_contradictions.length > 0;
  const conflict = conflictResolutionEvidence(calls, conflictExpectedBeforeTools);
  const violations = [];
  ...
  if (completionClaimed) {
    ...
    if (
      requirements.research_required === true &&
      declared.stop_reason === "sufficient_evidence" &&
      !researchStop.proven
    ) {
      violations.push("EPISTEMIC_RESEARCH_SUFFICIENT_EVIDENCE_UNPROVEN");
    }
    if (
      requirements.research_required === true &&
      declared.stop_reason === "diminishing_returns" &&
      !diminishingReturns.proven
    ) {
      violations.push("EPISTEMIC_RESEARCH_DIMINISHING_RETURNS_UNPROVEN");
    }
    if (
      requirements.research_required === true &&
      declared.stop_reason === "not_required"
    ) {
    ...
    if (conflict.required) {
      if (declared.conflict_status === "unresolved") {
        violations.push("EPISTEMIC_CONFLICT_UNRESOLVED");
      } else if (!conflict.proven) {
        violations.push(
          declared.conflict_status === "resolved"
    ...
    if (["blocked", "more_research_needed"].includes(declared.stop_reason)) {
      violations.push("EPISTEMIC_RESEARCH_STOP_CONDITION_NOT_MET");
    }
  }
  ...
  return {
    ...
    epistemic_state: {
      ...
      research_evidence_sequence: researchCall?.sequence ?? null,
      research_stop_proven: researchStop.proven,
      research_stop_evidence_reason: researchStop.reason,
      diminishing_returns_proven: diminishingReturns.proven,
      diminishing_returns_evidence_reason: diminishingReturns.reason,
      conflict_observed_in_evidence: conflict.observed_conflict,
      conflict_resolution_required: conflict.required,
      conflict_resolution_proven: conflict.proven,
      conflict_resolution_evidence_reason: conflict.reason,
      conflict_evidence_sequence: conflict.conflict_sequence,
      conflict_resolution_sequence: conflict.resolution_sequence,
