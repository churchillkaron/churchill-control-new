...
function successfulMatching(calls, pattern, afterSequence = -1) {
  return calls.find((call) => {
    const normalizedName = call.name.replace(/[_./:-]+/g, " ");
    return call.sequence > afterSequence &&
...
function verificationEvidence({ route, calls }) {
  ...
  if (signals.mutation_intent === true) {
    const mutation = successfulMatching(calls, MUTATION_TOOL_PATTERN);
    if (!mutation) return { observed: false, mutation_observed: false, post_mutation_observed: false };
    const verification = successfulMatching(calls, VERIFICATION_TOOL_PATTERN, mutation.sequence);
    return {
  ...
  return {
    observed: Boolean(successfulMatching(calls, VERIFICATION_TOOL_PATTERN)),
...
export function applyAvantiqoEpistemicCompletionGate({
  ...
  const researchObserved = Boolean(successfulMatching(calls, RESEARCH_TOOL_PATTERN));
  const liveReadObserved = Boolean(successfulMatching(calls, LIVE_READ_TOOL_PATTERN));
