...
async function purgeExpiredEstimatorCalibrationEvidence(organizationId, limit = 100) {
  const scopes = [OUTCOME_ASSESSMENT_SCOPE, CALIBRATION_EVENT_SCOPE];
  let deletedCount = 0;
...
function calibrationProfileRow({ organizationId, estimatorFingerprint, events, nowIso }) {
  ...
  return {
    ...
    memory_type: quarantined ? "lesson" : "fact",
...
export async function reconcileAvantiqoExperimentEstimatorCalibration({
  ...
  const expiredEvidencePurged = persist
    ? await purgeExpiredEstimatorCalibrationEvidence(organizationId).catch((error) => {
        console.error("AVANTIQO_EXPERIMENT_ESTIMATOR_CALIBRATION_PURGE_FAILED", error);
        return 0;
      })
    : 0;
  ...
  return {
    ...
    expired_calibration_evidence_purged_count: expiredEvidencePurged,
