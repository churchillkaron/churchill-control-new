...
import {
  ...
  verifyAvantiqoManagedProviderClaimEvidence,
  verifyAvantiqoRunpodClaimEvidence,
...
async function purgeExpiredUnusedClaims(organizationId, limit = 100) {
  const candidates = await supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id")
    .eq("organization_id", organizationId)
    .eq("memory_scope", CLAIM_SCOPE)
    .eq("source", "one_time_experiment_execution_claim")
    .eq("active", true)
  ...
  const ids = (candidates.data || []).map((row) => row.id).filter(Boolean);
...
async function verifyModePrerequisites({
  mode,
  ...
  if (mode === "LOCAL_PROVIDER_FREE") {
    ...
    return {
      external_execution_evidence_contract:
        AVANTIQO_EXPERIMENT_EXTERNAL_EXECUTION_EVIDENCE_CONTRACT,
      external_claim_evidence_verified: true,
      ...
      runpod_lease_expires_at: null,
    };
  }

  if (mode === "MANAGED_PROVIDER_API") {
    if (!organization_service_id || !service_usage_id || !wallet_reservation_transaction_id) {
      throw new Error(
        `${AVANTIQO_EXPERIMENT_EXECUTION_CLAIM_CONTRACT}_PROVIDER_LEDGER_IDENTIFIERS_REQUIRED`,
      );
    }
    const verified = await verifyAvantiqoManagedProviderClaimEvidence({
      organization_service_id,
      service_usage_id,
      wallet_reservation_transaction_id,
      provider_service_runtime_authorization_fingerprint,
    ...
    return {
      ...
      caller_supplied_fingerprint_is_authority: false,
      provider_service_runtime_authorization_fingerprint:
        verified.provider_service_runtime_authorization_fingerprint,
      wallet_reservation_fingerprint: verified.wallet_reservation_fingerprint,
      runpod_safe_lease_fingerprint: null,
      organization_service_id: verified.organization_service_id,
      service_usage_id: verified.service_usage_id,
      wallet_reservation_transaction_id: verified.wallet_reservation_transaction_id,
      runpod_endpoint_id: null,
      runpod_lease_lane: null,
      runpod_lease_expires_at: null,
      service_runtime_ledger_verified: verified.service_runtime_ledger_verified === true,
      wallet_reservation_ledger_verified: verified.wallet_reservation_ledger_verified === true,
    };
  }

  if (!runpod_endpoint_id) {
    throw new Error(
      `${AVANTIQO_EXPERIMENT_EXECUTION_CLAIM_CONTRACT}_RUNPOD_ENDPOINT_ID_REQUIRED`,
    );
  }
  const verified = verifyAvantiqoRunpodClaimEvidence({
    runpod_endpoint_id,
    runpod_lease_lane,
    runpod_safe_lease_fingerprint,
  });
  return {
    ...
    provider_service_runtime_authorization_fingerprint: null,
    wallet_reservation_fingerprint: null,
    runpod_safe_lease_fingerprint: verified.runpod_safe_lease_fingerprint,
    organization_service_id: null,
    service_usage_id: null,
    wallet_reservation_transaction_id: null,
    runpod_endpoint_id: verified.runpod_endpoint_id,
    runpod_lease_lane: verified.runpod_lease_lane,
    runpod_lease_expires_at: verified.runpod_lease_expires_at,
    runpod_safe_lease_environment_verified:
      verified.safe_lease_environment_verified === true,
    runpod_safe_lease_db_persisted: false,
  };
}
...
export async function createAvantiqoExperimentExecutionClaim({
  ...
  await purgeExpiredUnusedClaims(organizationId).catch((error) => {
    console.error("AVANTIQO_EXPERIMENT_EXECUTION_CLAIM_PURGE_FAILED", error);
  });
  ...
  const modePrerequisites = await verifyModePrerequisites({
    mode,
    organization_service_id,
    service_usage_id,
    wallet_reservation_transaction_id,
  ...
  const row = {
    ...
    content:
      "Single-use execution claim bound to one current approval, one exact experiment version, one execution mode, one executor and one cost ceiling. External execution prerequisites are verified against authoritative ledgers or the active safe-lease boundary before the claim is created; caller fingerprints are never authority.",
    importance: 1,
    confidence: 1,
    ...
    metadata: {
      ...
      ...modePrerequisites,
      runpod_safe_lease_contract: null,
      single_use: true,
...
export async function consumeAvantiqoExperimentExecutionClaim({
  ...
  if (
    ...
    metadata.external_claim_evidence_verified !== true ||
...
export async function assertAvantiqoExperimentExecutionClaimCurrent({
  ...
  return {
    ...
    external_claim_evidence_verified: metadata.external_claim_evidence_verified === true,
    caller_supplied_fingerprint_is_authority: false,
    organization_service_id: text(metadata.organization_service_id, 80) || null,
    service_usage_id: text(metadata.service_usage_id, 80) || null,
    wallet_reservation_transaction_id:
      text(metadata.wallet_reservation_transaction_id, 80) || null,
    runpod_endpoint_id: text(metadata.runpod_endpoint_id, 200) || null,
    runpod_lease_lane: text(metadata.runpod_lease_lane, 160) || null,
    runpod_lease_expires_at: text(metadata.runpod_lease_expires_at, 160) || null,
