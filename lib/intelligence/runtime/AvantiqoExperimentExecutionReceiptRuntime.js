...
async function purgeExpiredExperimentExecutionReceipts(organizationId, limit = 100) {
  const candidates = await supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id")
    .eq("organization_id", organizationId)
    .eq("memory_scope", RECEIPT_SCOPE)
    .eq("source", "experiment_execution_receipt_provenance")
  ...
  return { scanned: ids.length, purged: (deleted.data || []).length };
}
...
export async function recordAvantiqoExperimentExecutionReceipt({
  ...
  await purgeExpiredExperimentExecutionReceipts(organizationId).catch((error) => {
    console.error("AVANTIQO_EXPERIMENT_EXECUTION_RECEIPT_PURGE_FAILED", error);
  });
  ...
  if (!["LOCAL_PROVIDER_FREE", "MANAGED_PROVIDER_API"].includes(mode)) {
...
export async function assertAvantiqoExperimentExecutionReceiptCurrent({
  ...
  if (!receipt || !activeAndUnexpired(receipt)) {
