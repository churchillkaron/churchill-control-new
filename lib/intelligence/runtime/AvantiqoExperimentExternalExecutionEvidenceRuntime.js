...
function assertProviderUsagePreExecution(usage, serviceId) {
  ...
  if (status !== "PENDING" || ["SUCCESS", "FAILED", "CANCELLED"].includes(executionStatus)) {
    throw new Error(
      `${AVANTIQO_EXPERIMENT_EXTERNAL_EXECUTION_EVIDENCE_CONTRACT}_PROVIDER_USAGE_ALREADY_TERMINAL`,
    );
  }
  if (usage.execution_finished_at) {
    throw new Error(
      `${AVANTIQO_EXPERIMENT_EXTERNAL_EXECUTION_EVIDENCE_CONTRACT}_PROVIDER_USAGE_ALREADY_FINISHED`,
    );
  }
}
