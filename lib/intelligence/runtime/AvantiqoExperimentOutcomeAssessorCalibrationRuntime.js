...
async function loadAssessments(organizationId, nowIso = new Date().toISOString()) {
...
async function purgeExpiredOutcomeAssessorCalibrationEvidence(organizationId, limit = 100) {
  const nowIso = new Date().toISOString();
  let deletedCount = 0;
  for (const scope of [OUTCOME_ASSESSMENT_SCOPE, CALIBRATION_EVENT_SCOPE]) {
    const candidates = await supabaseAdmin.from(MEMORY_TABLE).select("id").eq("organization_id", organizationId).eq("memory_scope", scope).eq("active", true).not("valid_until", "is", null).lt("valid_until", nowIso).limit(Math.max(1, Math.min(100, Number(limit) || 100)));
    if (candidates.error) throw candidates.error;
    const ids = list(candidates.data).map((row) => row.id).filter(Boolean);
    if (!ids.length) continue;
    const deleted = await supabaseAdmin.from(MEMORY_TABLE).delete().eq("organization_id", organizationId).eq("memory_scope", scope).in("id", ids).select("id");
    if (deleted.error) throw deleted.error;
    deletedCount += list(deleted.data).length;
  }
  return deletedCount;
}
...
function calibrationProfileRow({ organizationId, assessorFingerprint, events, nowIso }) {
  ...
  return {
    ...
    memory_type: unsafeOptimism ? "lesson" : "fact",
...
export async function reconcileAvantiqoExperimentOutcomeAssessorCalibration({
  ...
  let expiredCalibrationEvidencePurged = 0;
  if (persist) {
    expiredCalibrationEvidencePurged = await purgeExpiredOutcomeAssessorCalibrationEvidence(organizationId).catch((error) => {
      console.error("AVANTIQO_OUTCOME_ASSESSOR_CALIBRATION_PURGE_FAILED", error);
      return 0;
    });
  }
  const assessments = (await loadAssessments(organizationId, nowIso)).filter((row) =>
  ...
  return {
    ...
    expired_calibration_evidence_purged: expiredCalibrationEvidencePurged,
