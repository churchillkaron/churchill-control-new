...
function watermarkIndex(value) {
  if (value === null || value === undefined || text(value, 80) === "") return null;
  const number = Number(value);
  if (!Number.isInteger(number) || number <= 0) {
    throw new Error(
      `${AVANTIQO_EXPERIMENT_POLICY_INTERVAL_RESOLUTION_CONTRACT}_ACTIVATION_HISTORY_WATERMARK_INDEX_INVALID`,
    );
  }
  return number;
}

function watermarkFingerprint(value) {
  const candidate = text(value, 128).toLowerCase();
  if (!candidate) return null;
  if (!/^[a-f0-9]{32,128}$/.test(candidate)) {
    throw new Error(
      `${AVANTIQO_EXPERIMENT_POLICY_INTERVAL_RESOLUTION_CONTRACT}_ACTIVATION_HISTORY_WATERMARK_FINGERPRINT_INVALID`,
    );
  }
  return candidate;
}

function normalizedWatermark(data) {
  const index = watermarkIndex(data.activation_history_watermark_index);
  const fingerprint = watermarkFingerprint(
    data.activation_history_watermark_fingerprint,
  );
  if ((index === null) !== (fingerprint === null)) {
    throw new Error(
      `${AVANTIQO_EXPERIMENT_POLICY_INTERVAL_RESOLUTION_CONTRACT}_ACTIVATION_HISTORY_WATERMARK_SHAPE_INVALID`,
    );
  }
...
function normalizedBinding(payload) {
  ...
  const watermark = normalizedWatermark(data);
  if (kind === "NO_PERSISTENT_POLICY_INTERVAL") {
    return {
      ...
      activation_history_watermark_index: watermark.index,
      activation_history_watermark_fingerprint: watermark.fingerprint,
  ...
  const generationFingerprint = text(
    data.activation_generation_fingerprint,
    128,
  ).toLowerCase();
...
export function policyIntervalBindingFromMetadata(metadata) {
  ...
  return normalizedBinding({
    ...
    activation_history_watermark_index:
      source.policy_activation_history_watermark_index,
    activation_history_watermark_fingerprint:
      source.policy_activation_history_watermark_fingerprint,
...
export function policyIntervalBindingMetadata(binding) {
  ...
  return {
    ...
    policy_activation_history_watermark_index:
      normalized.activation_history_watermark_index,
    policy_activation_history_watermark_fingerprint:
      normalized.activation_history_watermark_fingerprint,
  ...
  if (!organizationId) {
    return {
      ...
      binding: {
        ...
        activation_started_at: null,
        activation_closed_at: null,
        activation_history_watermark_index: null,
        activation_history_watermark_fingerprint: null,
        no_policy_epoch_reentry_allowed: false,
...
export async function assertAvantiqoExperimentPolicyIntervalBindingAt({
  ...
  const same = Boolean(
    expected.attribution_kind === actual.attribution_kind &&
      ...
      expected.activation_history_watermark_index ===
        actual.activation_history_watermark_index &&
      expected.activation_history_watermark_fingerprint ===
        actual.activation_history_watermark_fingerprint
  );
  if (!same) {
    throw new Error(
