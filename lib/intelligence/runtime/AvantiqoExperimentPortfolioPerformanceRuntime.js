...
function performanceProfileRow({ organizationId, cohortKey, outcomes, nowIso }) {
  ...
  return {
    ...
    memory_type: reviewRecommended ? "lesson" : "fact",
...
async function purgeExpiredPortfolioOutcomes(organizationId, nowIso, limit = 100) {
  const candidates = await supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id")
    .eq("organization_id", organizationId)
    .eq("memory_scope", OUTCOME_SCOPE)
    .eq("source", "experiment_portfolio_performance_outcome")
    .eq("active", true)
...
export async function reconcileAvantiqoExperimentPortfolioPerformance({
  ...
  let expiredPortfolioOutcomesPurged = 0;
  if (persist) {
    expiredPortfolioOutcomesPurged = await purgeExpiredPortfolioOutcomes(
      organizationId,
      nowIso,
    ).catch((error) => {
      console.error("AVANTIQO_EXPERIMENT_PORTFOLIO_OUTCOME_PURGE_FAILED", error);
      return 0;
    });
  }
