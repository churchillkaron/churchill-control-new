...
function agendaRow({ organizationId, item, index, activeIndex, nowMs, exam = null }) {
  ...
  const practiceDue = weaknessScore >= 0.2;
  ...
  const adaptiveImportance = practiceDue ? Math.min(0.97, 0.72 + weaknessScore * 0.25) : 0.58;
  ...
  return {
    ...
    metadata: {
      ...
      latest_exam_passed: examMeta.passed === true,
...
export async function reconcileAvantiqoGeneralIntelligenceCurriculum({ now = new Date() } = {}) {
  ...
  const rows = WORLD_CURRICULUM.map((item, index) => {
    ...
    return agendaRow({ organizationId, item, index, activeIndex, nowMs, exam: latestExamByTopic.get(topic) || null });
  });
