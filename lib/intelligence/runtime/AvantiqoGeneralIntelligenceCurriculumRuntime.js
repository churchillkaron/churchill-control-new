...
const WORLD_CURRICULUM = Object.freeze([
  ...
  ["science-foundations", "science", "Study durable foundations across physics, chemistry, biology and systems science, emphasizing mechanisms, measurement, falsifiability, conservation laws, uncertainty and experimental evidence. Prefer universities, national laboratories, scientific societies and peer-reviewed primary research."],
...
function agendaRow({ organizationId, item, index, activeIndex, nowMs }) {
  const [key, domain, query] = item;
  const isActive = index === activeIndex;
  const due = isActive ? new Date(nowMs).toISOString() : new Date(nowMs + ((index - activeIndex + WORLD_CURRICULUM.length) % WORLD_CURRICULUM.length) * DAY_MS).toISOString();
  ...
  return {
    ...
    content: query, importance: isActive ? 0.995 : 0.58, confidence: 1, source: "general_intelligence_curriculum", active: true,
...
export async function reconcileAvantiqoGeneralIntelligenceCurriculum({ now = new Date() } = {}) {
  ...
  const rows = WORLD_CURRICULUM.map((item, index) => agendaRow({ organizationId, item, index, activeIndex, nowMs }));
