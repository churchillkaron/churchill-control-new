...
function agendaRow({ organizationId, item, index, activeIndex, nowMs, exam = null, retention = null }) {
  ...
  // Model/exam weakness is a local-practice signal, never a research trigger.
  // Research cadence is controlled only by the world-knowledge rotation/freshness schedule.
  const due = new Date(rotationDueMs).toISOString();
  ...
  return {
    ...
    metadata: {
      ...
      local_practice_only: practiceDue,
      research_due_to_model_failure: false,
      model_failure_can_trigger_research: false,
      research_trigger: isActive ? "ROTATION_FRESHNESS" : "SCHEDULED_ROTATION",
