...
export async function runAvantiqoGeneralIntelligenceExam({organizationId=text(process.env.AVANTIQO_INTELLIGENCE_LEARNING_ORGANIZATION_ID),topicKey=null}={}){
  ...
  const row={organization_id:organizationId,party_id:null,entity_id:null,conversation_id:null,source_turn_id:null,memory_scope:EXAM_SCOPE,memory_key:key,memory_type:"fact",subject:topic,content:`General intelligence exam ${grading.passed?"passed":"needs practice"}: ${(grading.score*100).toFixed(1)}%.`,importance:0.75,confidence:1,source:"general_intelligence_exam",active:true,valid_until:null,superseded_by:null,superseded_at:null,forgotten_at:null,metadata:{contract:AVANTIQO_GENERAL_INTELLIGENCE_EXAM_CONTRACT,topic_key:topic,model:MODEL,infrastructure_provider:infra,local_4b:true,score:grading.score,weakness_score:grading.weakness_score,passed:grading.passed,diagnostic_class:grading.passed?"NONE":"MODEL_REASONING_OR_EVIDENCE_DISCIPLINE",local_practice_required:grading.passed!==true,research_gap_detected:false,research_eligible_from_exam:false,grading,case_count:cases.length,synthesis_fingerprint:fp,exam_cases:cases,automatic_model_training:false,automatic_model_promotion:false,automatic_knowledge_promotion:false,customer_private_content_included:false,raw_reasoning_persisted:false,created_at:now},updated_at:now}; const wr=await supabaseAdmin.from(MEMORY_TABLE).upsert(row,{onConflict:"organization_id,memory_scope,memory_key"}); if(wr.error) throw wr.error;
  return {success:true,status:"COMPLETED",contract:AVANTIQO_GENERAL_INTELLIGENCE_EXAM_CONTRACT,topic_key:topic,score:grading.score,weakness_score:grading.weakness_score,passed:grading.passed,diagnostic_class:grading.passed?"NONE":"MODEL_REASONING_OR_EVIDENCE_DISCIPLINE",local_practice_required:grading.passed!==true,research_gap_detected:false,research_eligible_from_exam:false,case_count:cases.length,local_4b:true,model:MODEL,infrastructure_provider:infra};
}
