...
export async function runAvantiqoGeneralIntelligenceTransferPractice({organizationId=text(process.env.AVANTIQO_INTELLIGENCE_LEARNING_ORGANIZATION_ID)}={}){
  ...
  if(!list(retention.data).length)return {success:true,status:"IDLE",reason:"NO_MASTERY_CANDIDATE_SOURCE",contract:AVANTIQO_GENERAL_INTELLIGENCE_TRANSFER_PRACTICE_CONTRACT};
  ...
  const priorTransfers=await supabaseAdmin.from(MEMORY_TABLE).select("memory_key,metadata").eq("organization_id",organizationId).eq("memory_scope",TRANSFER_SCOPE).eq("active",true).limit(5000); if(priorTransfers.error)throw priorTransfers.error;
  ...
  const synthesisRows=await supabaseAdmin.from(MEMORY_TABLE).select("subject,metadata,updated_at").eq("organization_id",organizationId).eq("memory_scope",SYNTHESIS_SCOPE).eq("active",true).order("updated_at",{ascending:false}).limit(500); if(synthesisRows.error)throw synthesisRows.error;
  const practicedPairs=new Set(list(priorTransfers.data).map(r=>`${text(object(r.metadata).source_topic_key,240)}|${text(object(r.metadata).target_topic_key,240)}`).filter(x=>!x.startsWith("|")));
  let source=null,target=null,sourceTopic=null,targetTopic=null,sourceDomain=null,targetDomain=null;
  outer: for(const candidateSource of list(retention.data)){
    const candidateSourceTopic=candidateSource.subject;
    const candidateSourceDomain=domainOf(list(agendas.data).find(r=>topicOf(r)===candidateSourceTopic));
    const candidateSynthesis=list(synthesisRows.data).find(r=>text(object(r.metadata).root_topic_key,240)===candidateSourceTopic);
    const candidateSourceEvidence=evidencePacket(list(knowledge.data).filter(r=>topicOf(r)===candidateSourceTopic&&Number(r.confidence||0)>=0.72&&object(r.metadata).customer_private_memory!==true),"S");
    if(!candidateSourceDomain||!candidateSynthesis||candidateSourceEvidence.length<2)continue;
    for(const candidateTarget of list(agendas.data)){
      const candidateTargetTopic=topicOf(candidateTarget),candidateTargetDomain=domainOf(candidateTarget);
      const candidateTargetEvidence=evidencePacket(list(knowledge.data).filter(r=>topicOf(r)===candidateTargetTopic&&Number(r.confidence||0)>=0.72&&object(r.metadata).customer_private_memory!==true),"T");
      if(!candidateTargetTopic||candidateTargetTopic===candidateSourceTopic||!candidateTargetDomain||candidateTargetDomain===candidateSourceDomain||candidateTargetEvidence.length<2)continue;
      if(practicedPairs.has(`${candidateSourceTopic}|${candidateTargetTopic}`))continue;
      source=candidateSource; target=candidateTarget; sourceTopic=candidateSourceTopic; targetTopic=candidateTargetTopic; sourceDomain=candidateSourceDomain; targetDomain=candidateTargetDomain; break outer;
    }
  }
  if(!source)return {success:true,status:"IDLE",reason:"NO_UNPRACTICED_CROSS_DOMAIN_PAIR",contract:AVANTIQO_GENERAL_INTELLIGENCE_TRANSFER_PRACTICE_CONTRACT};
  const sourceSynthesis=list(synthesisRows.data).find(r=>text(object(r.metadata).root_topic_key,240)===sourceTopic);
  ...
  const prompt=["Cross-domain transfer practice.","Do not copy surface patterns. Treat analogy as a hypothesis only.","Use both SOURCE and TARGET evidence.","Choose at most one source mechanism to test against the target domain.","Return JSON with: source_mechanism, target_application, invariants[], boundary_conditions[], falsifiers[], missing_prerequisites[], evidence_ids[], verdict, explanation.","verdict must be TRANSFER_HYPOTHESIS or UNSUPPORTED_TRANSFER.",JSON.stringify({source_topic:sourceTopic,source_domain:sourceDomain,source_mechanisms:sourceMechanisms,source_evidence:sourceEvidence,target_topic:targetTopic,target_domain:targetDomain,target_evidence:targetEvidence})].join("\n");
