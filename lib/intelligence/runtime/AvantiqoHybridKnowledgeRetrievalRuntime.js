...
} from "@/lib/intelligence/runtime/AvantiqoFinalKnowledgeReleaseReceiptAtomicBindingRuntime";
import {
...
export const AVANTIQO_REUSABLE_KNOWLEDGE_ROW_ADMISSION_CONTRACT =
  "AVANTIQO_REUSABLE_KNOWLEDGE_ROW_ADMISSION_V1";
...
const MEMORY_SELECT = "id,organization_id,party_id,entity_id,conversation_id,source_turn_id,memory_scope,memory_key,memory_type,subject,content,importance,confidence,source,active,valid_until,superseded_by,superseded_at,forgotten_at,metadata,updated_at,created_at";
...
const CONCEPT_FAMILIES = Object.freeze({
  ...
  contradiction_conflict: [
    "contradiction", "contradictions", "contradictory", "contradicted", "conflict", "conflicts", "conflicting",
...
function receiptMemoryKeysForReleaseRows(rows = []) {
  return [...new Set(list(rows).map((row) => {
    const metadata = object(row?.metadata);
    const identity = createAvantiqoFinalKnowledgeReleaseReceiptIdentity({
      organization_id: row?.organization_id,
      authorization_id: metadata.final_release_authorization_id,
      release_id: metadata.release_id,
    });
    return identity.success === true ? identity.memory_key : null;
  }).filter(Boolean))];
}

async function loadExactReleaseReceipts({ organization, releaseRows = [] } = {}) {
  const keys = receiptMemoryKeysForReleaseRows(releaseRows);
  if (!keys.length) return [];
  const batches = [];
  for (let index = 0; index < keys.length; index += RECEIPT_LOOKUP_BATCH_SIZE) {
    batches.push(keys.slice(index, index + RECEIPT_LOOKUP_BATCH_SIZE));
  }
  const results = await Promise.all(batches.map((batch) =>
    supabaseAdmin.from(MEMORY_TABLE)
  ...
  return results.flatMap((result) => list(result.data));
}
...
export function assessAvantiqoReusableKnowledgeRowEligibility(
  row = {},
  ...
  const internal = source === INTERNAL_SOURCE;
  const finalRelease = source === FINAL_RELEASE_SOURCE;

  if (internal) {
    if (include_internal !== true) blockers.push("INTERNAL_KNOWLEDGE_NOT_REQUESTED");
    if (scope && scope !== KNOWLEDGE_SCOPE) blockers.push("PLATFORM_KNOWLEDGE_SCOPE_REQUIRED");
  } else {
    if (!finalRelease) blockers.push("EXPLICIT_FINAL_RELEASE_SOURCE_REQUIRED");
    if (scope !== KNOWLEDGE_SCOPE) blockers.push("PLATFORM_KNOWLEDGE_SCOPE_REQUIRED");
    ...
    if (metadata.explicit_final_release_approved !== true) {
      blockers.push("EXPLICIT_FINAL_RELEASE_APPROVAL_REQUIRED");
    }
    if (metadata.reusable_platform_knowledge !== true) {
      blockers.push("REUSABLE_PLATFORM_KNOWLEDGE_TRUE_REQUIRED");
    }
    if (metadata.knowledge_router_reuse_allowed !== true) {
      blockers.push("KNOWLEDGE_ROUTER_REUSE_TRUE_REQUIRED");
    }
    if (metadata.automatic_knowledge_promotion !== false) {
      blockers.push("AUTOMATIC_KNOWLEDGE_PROMOTION_FALSE_REQUIRED");
    }
    if (metadata.customer_private_content_included !== false) {
      blockers.push("CUSTOMER_PRIVATE_CONTENT_FALSE_REQUIRED");
    }
    if (metadata.raw_reasoning_persisted !== false) {
      blockers.push("RAW_REASONING_PERSISTENCE_FALSE_REQUIRED");
    }
  ...
  return {
    ...
    status: blockers.length === 0
      ? internal
        ? "CANONICAL_INTERNAL_KNOWLEDGE_ADMITTED"
        : "EXPLICITLY_RELEASED_PLATFORM_KNOWLEDGE_ADMITTED"
      : "KNOWLEDGE_ROW_REJECTED_BY_REUSE_ADMISSION",
    source_class: internal ? "canonical_internal" : finalRelease ? "explicit_final_release" : "untrusted",
    blockers,
    policy: {
      fail_closed: true,
      mechanism_agenda_reuse_allowed: false,
      evidence_candidate_reuse_allowed: false,
      provisional_knowledge_reuse_allowed: false,
      legacy_pre_epistemic_platform_knowledge_reuse_allowed: false,
      ...
      customer_private_content_forbidden: true,
...
export function rankAvantiqoKnowledgeRows({
  ...
  const ranked = list(rows)
    ...
    .filter((row) =>
      assessAvantiqoReusableKnowledgeRowEligibility(row, { include_internal }).eligible,
    )
    ...
    .map((row) => {
      ...
      const score =
        lexical * 0.29 + concepts * 0.24 + phrases * 0.1 + aliases * 0.08 + fuzzy * 0.09 +
        domainMatch * 0.08 + jurisdictionMatch * 0.05 + confidence * 0.06 + sources.score * 0.04 +
        freshness.score * 0.03 + importance * 0.02 + (semanticBridge ? 0.05 : 0);
      return {
        row, metadata, score: bounded(score),
        signals: {
          lexical: bounded(lexical), concepts: bounded(concepts), phrases: bounded(phrases),
          alias_match: bounded(aliases), fuzzy: bounded(fuzzy), domain_match: domainMatch === 1,
          jurisdiction_match: jurisdictionMatch === 1, semantic_bridge: semanticBridge,
          confidence, importance, source_quality: sources.score, source_diversity: sources.diversity,
          official_primary_source: sources.officialPrimary, freshness: freshness.score,
          age_days: Number.isFinite(freshness.ageDays) ? Number(freshness.ageDays.toFixed(2)) : null,
          freshness_valid: freshness.valid,
        },
        sources: sources.sources, verifiedAt: freshness.verifiedAt,
    ...
    .filter((entry) =>
      entry.score >= 0.28 &&
      (entry.signals.lexical >= 0.12 || entry.signals.concepts >= 0.34 ||
        entry.signals.alias_match >= 0.5 || entry.signals.fuzzy >= 0.44 || entry.signals.domain_match),
    )
    .sort((left, right) => {
      if (right.score !== left.score) return right.score - left.score;
      if (right.signals.confidence !== left.signals.confidence) return right.signals.confidence - left.signals.confidence;
  ...
  return {
    ...
    row_admission_contract: AVANTIQO_REUSABLE_KNOWLEDGE_ROW_ADMISSION_CONTRACT,
    query: question, inferred_domain: inferredQueryDomain, requested_domain: requestedDomain || null,
    requested_jurisdiction: requestedJurisdiction || null, current_question: currentQuestion,
    query_concepts: [...queryConcepts], ranked,
  };
}
...
function resultKnowledge(entry) {
  return {
    id: entry.row.id, type: entry.row.memory_type, subject: entry.row.subject,
    content: text(entry.row.content, 2400), relevance: Number(entry.score.toFixed(4)),
    confidence: bounded(entry.row.confidence, 0), importance: bounded(entry.row.importance, 0),
    verified_at: entry.verifiedAt || null, valid_until: entry.row.valid_until || null,
    domain: text(entry.metadata.knowledge_domain, 120) || null,
    jurisdiction: text(entry.metadata.jurisdiction, 120) || null,
    stability: text(entry.metadata.stability, 80) || null, sources: entry.sources,
    retrieval_signals: entry.signals,
    provenance: {
      source: entry.row.source, topic_key: text(entry.metadata.topic_key, 240) || null,
      evidence_comparison_contract: text(entry.metadata.evidence_comparison_contract, 180) || null,
    },
    authorization_effect: "NONE",
  };
}
...
export async function recallAvantiqoHybridKnowledge({
  organizationId = null, query, domain = null, jurisdiction = null, freshness_days = null,
  minimum_sources = 2, limit = MAX_RESULTS,
} = {}) {
  ...
  if (!organization) {
    return {
      contract: AVANTIQO_HYBRID_KNOWLEDGE_RETRIEVAL_CONTRACT, available: false, sufficient: false,
      reason: "LEARNING_ORGANIZATION_NOT_CONFIGURED", query: question, knowledge: [], sources: [],
      evidence_graph: null,
    };
  }
  ...
  const result = await supabaseAdmin.from(MEMORY_TABLE)
    .select(MEMORY_SELECT)
    .eq("organization_id", organization).eq("memory_scope", KNOWLEDGE_SCOPE)
    .eq("source", FINAL_RELEASE_SOURCE).eq("active", true)
    .order("importance", { ascending: false }).order("updated_at", { ascending: false }).limit(MAX_CANDIDATES);
  if (result.error) throw result.error;

  const receiptRows = await loadExactReleaseReceipts({
    organization,
    releaseRows: result.data,
  });
  const receiptVerifier = createAvantiqoFinalKnowledgeReleaseReceiptVerifier();
  const verifiedReceipts = receiptRows.filter(
    (receipt) => receiptVerifier.available === true && receiptVerifier.verify(receipt),
  );
  const rowsWithReceiptProof = list(result.data).map((row) => ({
    ...row,
    _immutable_release_receipt_verified: verifiedReceipts.some(
      (receipt) => verifyAvantiqoFinalKnowledgeReleaseReceiptBinding(receipt, row),
    ),
  }));

  const ranked = rankAvantiqoKnowledgeRows({
    rows: rowsWithReceiptProof, query: question, domain, jurisdiction, freshness_days,
    include_internal: false, limit,
  });

  const evidenceGraph = await inspectAvantiqoEvidenceGraph({
    organizationId: organization, query: question, domain: domain || ranked.requested_domain,
    jurisdiction, freshnessDays: freshness_days ?? (ranked.current_question ? 7 : 180), limit: 8,
  }).catch((error) => ({
    contract: "AVANTIQO_EVIDENCE_GRAPH_V1", available: false, block_knowledge_reuse: true,
    reason: "EVIDENCE_GRAPH_READ_FAILED", error: text(error?.message || error, 500), matches: [], conflicts: [],
  }));
  ...
  const sourceRequirementMet = Boolean(sources.length >= minimumSourceCount || top?.signals?.official_primary_source === true);
  const semanticMatch = Boolean(top && (
    top.signals.lexical >= 0.2 || top.signals.concepts >= 0.5 || top.signals.alias_match >= 0.5 ||
    top.signals.semantic_bridge === true || top.signals.fuzzy >= 0.55
  ));
  const sufficient = Boolean(
    top && top.score >= 0.4 && top.signals.confidence >= 0.72 && semanticMatch && sourceRequirementMet &&
    evidenceGraph.block_knowledge_reuse !== true,
  );

  return {
    contract: AVANTIQO_HYBRID_KNOWLEDGE_RETRIEVAL_CONTRACT, available: true, sufficient,
    reason: evidenceGraph.block_knowledge_reuse === true
      ? evidenceGraph.reason || "EVIDENCE_GRAPH_BLOCKED_REUSE"
      : sufficient ? "HYBRID_VERIFIED_KNOWLEDGE_REUSABLE" : "FRESH_RESEARCH_REQUIRED",
    query: question, inferred_domain: ranked.inferred_domain, requested_domain: ranked.requested_domain,
    current_question: ranked.current_question, query_concepts: ranked.query_concepts, knowledge, sources,
    evidence_graph: {
      available: evidenceGraph.available === true, block_knowledge_reuse: evidenceGraph.block_knowledge_reuse === true,
      reason: evidenceGraph.reason || null, relevant_graph_count: list(evidenceGraph.matches).length,
      relevant_conflict_count: list(evidenceGraph.conflicts).length,
    },
    retrieval: {
      candidate_count: list(result.data).length, matched_count: ranked.ranked.length,
      receipt_lookup_key_count: receiptMemoryKeysForReleaseRows(result.data).length,
      receipt_rows_loaded: receiptRows.length,
      receipt_lookup_targeted_to_candidate_releases: true,
      receipt_history_global_limit_dependency_removed: true,
      ...
      lexical_only: Boolean(ranked.ranked.length && ranked.ranked.every((entry) => entry.signals.concepts === 0)),
      source_requirement_met: sourceRequirementMet, minimum_source_count: minimumSourceCount,
      explicit_final_release_only: true, reusable_row_admission_fail_closed: true,
      released_knowledge_authenticity_required: true,
    ...
    governance: {
      deterministic_pre_model_retrieval: true, external_embedding_provider_used: false,
      external_intelligence_provider_used: false, evidence_graph_checked: true,
      conflict_can_block_reuse: true, stale_current_knowledge_can_block_reuse: true,
      expired_valid_until_blocks_reuse: true, mechanism_agenda_reused_as_knowledge: false,
      evidence_candidate_reused_as_knowledge: false, legacy_pre_epistemic_platform_knowledge_reused: false,
      ...
      immutable_release_receipt_checked_before_reuse: true,
      receipt_lookup_scales_with_candidate_releases_not_receipt_history: true,
      unsigned_or_forged_release_receipt_reused: false,
      unsigned_or_forged_released_knowledge_reused: false,
      database_only_released_claim_mutation_reused: false,
      memory_never_authorizes_actions: true, raw_reasoning_persisted: false,
    },
  };
}
...
export const AvantiqoHybridKnowledgeRetrievalRuntime = Object.freeze({
  contract: AVANTIQO_HYBRID_KNOWLEDGE_RETRIEVAL_CONTRACT,
  row_admission_contract: AVANTIQO_REUSABLE_KNOWLEDGE_ROW_ADMISSION_CONTRACT,
  assessRowEligibility: assessAvantiqoReusableKnowledgeRowEligibility,
