...
const CONCEPT_FAMILIES = Object.freeze({
  ...
  contradiction_conflict: [
    "contradiction", "contradictions", "conflict", "conflicts", "disagree", "disagreement",
    "inconsistent", "inconsistency", "contested",
  ],
  workflow_process: [
...
function queryLooksCurrent(query) {
  return /\b(current|currently|latest|today|newest|recent|version|regulation|regulatory|law|legal|tax|rate|price|pricing|api|standard|specification|requirement|release|availability)\b/i.test(
    text(query, 12000),
  );
}
