...
} from "./AvantiqoCodeMissionKnowledgeUtilityReusePolicyRuntime.js";
...
export function createAvantiqoIntelligenceCodeMissionResumeCapsule({
  ...
  return {
    ...
    preparation: {
      ...
      knowledge_utility_reuse_policy_evaluated:
        preparationRecord.governance?.knowledge_utility_reuse_policy_evaluated === true,
...
async function resolveKnowledge({
  ...
  const utilityEvaluation = await evaluateKnowledgeUtilityReuse({
    learned_knowledge: learnedKnowledge,
  });
  const governedKnowledge = object(utilityEvaluation?.learned_knowledge);
  if (governedKnowledge.evaluated !== true) {
    throw new Error(
      "AVANTIQO_INTELLIGENCE_CODE_MISSION_PREPARATION_UTILITY_KNOWLEDGE_INVALID",
    );
  }
  return {
    skipped: false,
...
export async function prepareAvantiqoIntelligenceCodeMission({
  ...
  const evaluateKnowledgeUtilityReuse =
    typeof supplied.evaluateKnowledgeUtilityReuse === "function"
      ? supplied.evaluateKnowledgeUtilityReuse
      : evaluateAvantiqoCodeMissionKnowledgeUtilityReuse;
  ...
  if (complexity === "large") {
    ...
    return {
      ...
      knowledge_utility_reuse_policy: knowledge.utility_evaluation,
      ...
      governance: {
        ...
        knowledge_utility_reuse_policy_evaluated:
          knowledge.utility_evaluation?.applicable === true,
        mature_negative_utility_can_suppress_hybrid_reuse:
          knowledge.utility_evaluation?.block_reuse === true,
        canonical_product_authority_affected_by_utility: false,
        web_research_automatically_performed: false,
