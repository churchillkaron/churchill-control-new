...
export async function runAvantiqoIntelligenceImprovementLoop({organizationId=learningOrganizationId()}={}){
  ...
  const fingerprint=hash(JSON.stringify({arena_version:ARENA_VERSION,caps,outcome_ids:outcomes.map(o=>o.id),world_edge_count:world.edge_count})); const key=`arena:${fingerprint.slice(0,40)}`;
