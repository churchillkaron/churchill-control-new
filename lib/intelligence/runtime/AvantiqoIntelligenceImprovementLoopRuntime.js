...
export async function runAvantiqoIntelligenceImprovementLoop({organizationId=learningOrganizationId()}={}){
  ...
  const [catalog,outcomeResult,arenaHistory,practiceHistory]=await Promise.all([
    ...
    supabaseAdmin.from(MEMORY_TABLE).select("id,metadata,updated_at").eq("organization_id",organizationId).eq("memory_scope",PRACTICE_SCOPE).eq("active",true).order("updated_at",{ascending:false}).limit(30),
