...
async function localArena({organizationId,cases,capabilities}){
  ...
  const out=object(settled.output),nested=object(out.output),infra=text(nested.infrastructure_provider||out.infrastructure_provider,200); if(infra!==LOCAL_INFRA)throw new Error(`${AVANTIQO_INTELLIGENCE_IMPROVEMENT_LOOP_CONTRACT}_LOCAL_REQUIRED:${infra||"NONE"}`);
