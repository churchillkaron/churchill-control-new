...
import { intelligenceContextFingerprint } from "../../operator/runtime/IntelligenceContextFingerprintPolicy.js";
import {
...
const PENDING_SETTLEMENT_MAX_POLLS = 480;
const FAST_PENDING_SETTLEMENT_DEADLINE_MS = 35_000;
const DEEP_PENDING_SETTLEMENT_DEADLINE_MS = 480_000;
const FAST_PENDING_QUEUE_GRACE_MS = 10_000;
const DEEP_PENDING_QUEUE_GRACE_MS = 180_000;
const SETTLEMENT_CALL_MAX_MS = 10_000;
const REASONING_CONTEXT_POLICY = Object.freeze({
  fast: Object.freeze({ message_count: 10, message_chars: 12000, system_chars: 9000, tool_result_chars: 8000 }),
  deep: Object.freeze({ message_count: 20, message_chars: 30000, system_chars: 18000, tool_result_chars: 18000 }),
});
...
function reasoningReservationEstimate({ conversation = [], tools = [], response_format = null, max_output_tokens = DEFAULT_MAX_OUTPUT_TOKENS } = {}) {
  const characters = JSON.stringify({ messages: conversation, tools, response_format }).length;
  return {
    estimated_input_tokens: Math.max(1, Math.ceil(characters / 3) + 512),
    estimated_output_tokens: Math.max(1, Math.floor(Number(max_output_tokens) || DEFAULT_MAX_OUTPUT_TOKENS)),
...
function pendingSettlementDeadlineMs(lane) {
  return lane === "fast"
    ? FAST_PENDING_SETTLEMENT_DEADLINE_MS
    : DEEP_PENDING_SETTLEMENT_DEADLINE_MS;
}
function pendingQueueGraceMs(lane) {
  return lane === "fast"
    ? FAST_PENDING_QUEUE_GRACE_MS
    : DEEP_PENDING_QUEUE_GRACE_MS;
}
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          reject(new Error("AVANTIQO_INTELLIGENCE_SETTLEMENT_CALL_DEADLINE_EXCEEDED"));
...
function assertOwnedProvider(value, phase) {
  ...
  if (!provider) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_PROVIDER_EVIDENCE_REQUIRED:${phase}`,
    );
  }
  if (provider !== OWNED_PROVIDER) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_OWNED_PROVIDER_REQUIRED:${phase}:${provider}`,
    );
  }
  return provider;
}
...
function recordToolOutcome(transcript, {
  ...
  matched.epistemic_evidence = epistemicEvidence
    ...
    : null;
  matched.invocation_identity = invocationIdentity
    ? { ...object(invocationIdentity) }
    : null;
}
...
function boundedInitialReasoningMessages(messages = [], lane = "deep") {
  const policy = REASONING_CONTEXT_POLICY[lane] || REASONING_CONTEXT_POLICY.deep;
  const source = list(messages);
  const system = source.find((message) => text(message?.role).toLowerCase() === "system");
  const nonSystem = source.filter((message) => text(message?.role).toLowerCase() !== "system");
  const selected = [];
  let used = 0;
  for (let index = nonSystem.length - 1; index >= 0 && selected.length < policy.message_count; index -= 1) {
    const message = object(nonSystem[index]);
    const content = typeof message.content === "string" ? message.content : stableJson(message.content ?? null);
    if (!text(content)) continue;
    const remaining = policy.message_chars - used;
    if (remaining <= 0) break;
    const bounded = content.length > remaining ? content.slice(content.length - remaining) : content;
    selected.push({ ...message, content: bounded });
    used += bounded.length;
  }
  selected.reverse();
  if (system) {
    selected.unshift({ ...system, content: text(system.content).slice(0, policy.system_chars) });
  }
  return selected;
}

function reasoningContextTelemetry(messages = [], lane = "deep") {
  const policy = REASONING_CONTEXT_POLICY[lane] || REASONING_CONTEXT_POLICY.deep;
  return {
    contract: "AVANTIQO_INTELLIGENCE_REASONING_CONTEXT_BUDGET_V1",
    lane,
    message_count: list(messages).length,
    message_chars: list(messages).reduce((sum, message) => sum + (typeof message?.content === "string" ? message.content.length : 0), 0),
    tool_result_char_ceiling: policy.tool_result_chars,
  };
}
...
async function settlePendingReasoningExecution({
  ...
  executionLane,
  metadata,
  settlementDeadlineMs = null,
  settlementQueueGraceMs = null,
} = {}) {
  ...
  if (execution?.pending !== true) {
    return {
      ...object(execution),
      provider,
      intelligence_owned_provider_verified: true,
  ...
  const defaultDeadlineMs = pendingSettlementDeadlineMs(executionLane);
  const requestedDeadlineMs = Number(settlementDeadlineMs);
  const deadlineMs = Number.isFinite(requestedDeadlineMs) && requestedDeadlineMs > 0
    ? Math.min(defaultDeadlineMs, requestedDeadlineMs)
    : defaultDeadlineMs;
  const defaultQueueGraceMs = pendingQueueGraceMs(executionLane);
  const requestedQueueGraceMs = Number(settlementQueueGraceMs);
  const queueGraceMs = Number.isFinite(requestedQueueGraceMs) && requestedQueueGraceMs >= 0
    ? Math.min(defaultQueueGraceMs, requestedQueueGraceMs)
    : defaultQueueGraceMs;
  let deadlineAt = startedAt + deadlineMs;
  let queueGraceApplied = false;
  ...
  for (let poll = 1; poll <= PENDING_SETTLEMENT_MAX_POLLS; poll += 1) {
      ...
      settled = await withinDeadline(
        ServiceExecutionRuntime.settle({
          ...
          metadata: {
            ...
            pending_settlement_deadline_ms: deadlineMs,
        ...
        Math.min(remainingMs, SETTLEMENT_CALL_MAX_MS),
      );
    } catch (error) {
      if (text(error?.message) !== "AVANTIQO_INTELLIGENCE_SETTLEMENT_CALL_DEADLINE_EXCEEDED") {
        throw error;
      }
      const sleepMs = Math.min(
        PENDING_SETTLEMENT_POLL_INTERVAL_MS,
        Math.max(0, deadlineAt - Date.now()),
      );
      if (sleepMs > 0 && poll < PENDING_SETTLEMENT_MAX_POLLS) await sleep(sleepMs);
      continue;
    }

    if (settled?.pending === true) {
      const pendingProvider = text(settled?.provider) || provider;
      assertOwnedProvider(pendingProvider, "PENDING_SETTLEMENT");
      const providerStatus = text(settled?.provider_status).toLowerCase();
      if (!queueGraceApplied && providerStatus === "queued") {
        deadlineAt += queueGraceMs;
        queueGraceApplied = true;
      }
      const sleepMs = Math.min(
        PENDING_SETTLEMENT_POLL_INTERVAL_MS,
        Math.max(0, deadlineAt - Date.now()),
    ...
    const settledProvider = text(settled?.provider) || provider;
  ...
  await withinDeadline(
    ServiceExecutionRuntime.cancelPending({
      organization_id,
      provider,
      provider_job_id: providerJobId,
      usage_id: usageId,
      ...
      credential_id: execution?.credential_id || null,
      execution_lane: executionLane,
      reason: `AVANTIQO_INTELLIGENCE_PENDING_SETTLEMENT_TIMEOUT:${executionLane}`,
      metadata: {
        ...object(metadata),
        intelligence_contract: CONTRACT,
        intelligence_execution_lane: executionLane,
        exact_provider_job_cleanup: true,
        raw_reasoning_persisted: false,
      },
    }),
    SETTLEMENT_CALL_MAX_MS,
  ).catch(() => null);

  throw new Error(`AVANTIQO_INTELLIGENCE_PENDING_SETTLEMENT_TIMEOUT:${executionLane}`);
}
...
export async function runIntelligenceReasoningLoop({
  ...
  settlement_deadline_ms = null,
  settlement_queue_grace_ms = null,
} = {}) {
  const executionLane = normalizeExecutionLane(execution_lane);
  const conversation = boundedInitialReasoningMessages(
    normalizeMessages({ system, messages, input }),
    executionLane,
  );
  validateScope({ organization_id, messages: conversation });
  const contextBudgetTelemetry = reasoningContextTelemetry(conversation, executionLane);
  ...
  const toolDescriptorBudget = typeof registry.descriptorBudget === "function"
    ? registry.descriptorBudget()
    : {
        contract: "AVANTIQO_INTELLIGENCE_TOOL_DESCRIPTOR_BUDGET_V1",
        available_tools: toolDescriptors.length,
        exposed_tools: toolDescriptors.length,
        descriptor_chars: JSON.stringify(toolDescriptors).length,
        bounded: false,
      };
  const descriptorFingerprint = intelligenceContextFingerprint({ toolDescriptors });
  ...
  const persistedContextFingerprint = object(object(metadata).intelligence_context_fingerprint);
  const cacheContext = persistedContextFingerprint.cacheable === true && text(persistedContextFingerprint.static_context_fingerprint)
    ? {
        contract: "AVANTIQO_INTELLIGENCE_PREFIX_CACHE_CONTEXT_V1",
        eligible: true,
        stable_context_fingerprint: text(persistedContextFingerprint.static_context_fingerprint),
        tool_descriptor_fingerprint: descriptorFingerprint.tool_descriptor_fingerprint,
        raw_content_included: false,
      }
    : null;
  const reservationEstimate = reasoningReservationEstimate({
    conversation,
  ...
  for (let turn = 1; turn <= turnLimit; turn += 1) {
    let execution = await ServiceExecutionRuntime.execute({
      ...
      input: {
        ...
        interactive_code: text(object(metadata).module).startsWith("CODE_AI"),
        origin_module: text(object(metadata).module) || null,
        ...(toolDescriptors.length ? { tools: toolDescriptors, tool_choice: "auto" } : {}),
        ...(response_format ? { response_format: object(response_format) } : {}),
        execution_lane: executionLane,
        temperature,
        max_output_tokens,
        ...(cacheContext ? { cache_context: cacheContext } : {}),
      },
      metadata: {
        ...object(metadata),
        module: object(metadata).module || "INTELLIGENCE",
        ...
        external_fallback_allowed: false,
        reasoning_turn: turn,
        intelligence_context_budget: contextBudgetTelemetry,
        intelligence_tool_descriptor_budget: toolDescriptorBudget,
        ...(cacheContext ? { intelligence_prefix_cache_context: cacheContext } : {}),
        intelligence_tool_descriptor_fingerprint: {
          contract: descriptorFingerprint.contract,
          tool_descriptor_fingerprint: descriptorFingerprint.tool_descriptor_fingerprint,
          cacheable_static_chars: descriptorFingerprint.cacheable_static_chars,
      ...
      cost_guard: {
        ...
        ...reservationEstimate,
        reference: `${CONTRACT}:${executionLane}:turn:${turn}`,
      },
      category: "AI",
    });
    ...
    execution = await settlePendingReasoningExecution({
      ...
      executionService,
      executionLane,
      settlementDeadlineMs: settlement_deadline_ms,
      settlementQueueGraceMs: settlement_queue_grace_ms,
      metadata: {
        ...object(metadata),
        reasoning_turn: turn,
      },
    ...
    if (!calls.length) {
      ...
      return {
        ...
        organization_id,
        provider: execution.provider,
        owned_provider_verified:
          execution.intelligence_owned_provider_verified === true,
        external_ai_fallback_used:
          execution.external_ai_fallback_used === true,
        model: execution?.model || model || null,
        execution_lane: executionLane,
        service_id: executionService,
        capability: executionService,
        ...
        context_budget: contextBudgetTelemetry,
        tool_descriptor_budget: toolDescriptorBudget,
        transcript,
      };
    }
    ...
    for (const call of calls) {
      ...
      if (toolName === "__avantiqo_invalid_tool_call__") {
        const invalidToolCall = {
          ok: false,
          blocked: true,
          code: "AVANTIQO_INTELLIGENCE_TOOL_CALL_JSON_INVALID",
          tool: null,
          retry_allowed: true,
        ...
        recordToolOutcome(transcript, {
          ...
          toolName: null,
          outcome: "blocked",
          code: invalidToolCall.code,
        });
        conversation.push(toolResultMessage(call, invalidToolCall));
        totalToolCalls += 1;
        continue;
      }
      let args;
      ...
      recordToolOutcome(transcript, {
        ...
        invocationIdentity: {
          capability_key: text(args.capability_key) || null,
        },
      });
      totalToolCalls += 1;
      const toolResultCeiling = (REASONING_CONTEXT_POLICY[executionLane] || REASONING_CONTEXT_POLICY.deep).tool_result_chars;
      const requestedResultChars = Number(result?.max_result_chars);
      const boundedToolResultChars = Number.isFinite(requestedResultChars) && requestedResultChars > 0
        ? Math.min(toolResultCeiling, requestedResultChars)
        : toolResultCeiling;
      conversation.push(toolResultMessage(call, result, boundedToolResultChars));
    }
  }
