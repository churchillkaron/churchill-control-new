...
const OVERFLOW_CONTRACT = "AVANTIQO_INTELLIGENCE_MODAL_OVERFLOW_POLICY_V1";
const OVERFLOW_LANE = "deep";
const MAX_ROWS = 500;
const MIN_CASES = 50;
...
function benchmarkPlanRow({ organizationId, provisional, shadowEvaluation, graph, nowIso }) {
  ...
  return {
    ...
    metadata: {
      ...
      status: "READY_FOR_GOVERNED_OVERFLOW_COUNTERFACTUAL_BENCHMARK",
      ...
      modal_overflow_contract_required: OVERFLOW_CONTRACT,
      modal_overflow_lane_required: OVERFLOW_LANE,
      ...
      automatic_external_compute_submission: false,
...
export async function reconcileAvantiqoKnowledgeCounterfactualBenchmarkPlans({ persist = true } = {}) {
  ...
  return {
    ...
    benchmark_policy: {
      ...
      customer_private_cases_allowed: false,
      governed_modal_overflow_required_if_local_insufficient: true,
      explicit_spend_approval_required: true,
    },
    governance: {
      provider_free: true,
      benchmark_execution_performed: false,
      modal_overflow_job_submitted: false,
      external_endpoint_mutated: false,
      reusable_platform_knowledge_created: false,
      final_promotion_candidate_created: false,
      automatic_knowledge_promotion: false,
...
function evaluationEligible({ baseline, candidate, regressionCount, qualityDelta, hallucinationDelta, evidence }) {
  return Boolean(
    baseline.case_count >= MIN_CASES &&
      ...
      evidence.modal_overflow_contract === OVERFLOW_CONTRACT &&
      evidence.modal_overflow_lane === OVERFLOW_LANE &&
      evidence.local_first_required === true &&
      evidence.local_insufficient === true &&
      evidence.explicit_owner_approval_consumed === true
  );
}
...
export async function recordAvantiqoKnowledgeCounterfactualBenchmarkEvaluation({
  ...
  if (text(planMetadata.status, 120) !== "READY_FOR_GOVERNED_OVERFLOW_COUNTERFACTUAL_BENCHMARK") {
  ...
  const structuralEvidence = {
    ...
    modal_overflow_contract: text(evidence.modal_overflow_contract, 180),
    modal_overflow_lane: text(evidence.modal_overflow_lane, 120),
    local_first_required: evidence.local_first_required === true,
    local_insufficient: evidence.local_insufficient === true,
    explicit_owner_approval_consumed: evidence.explicit_owner_approval_consumed === true,
  };
  ...
  const row = {
    ...
    memory_type: eligible ? "completed_step" : "fact",
