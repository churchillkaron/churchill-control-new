...
function finalCandidateRow({ organizationId, evaluation, graph, nowIso }) {
...
export async function reconcileAvantiqoKnowledgeFinalPromotionCandidates({ persist = true } = {}) {
  ...
  for (const evaluation of evaluations) {
    ...
    candidates.push(finalCandidateRow({ organizationId, evaluation, graph, nowIso }));
  }
  ...
  return {
    success: true,
    contract: AVANTIQO_KNOWLEDGE_COUNTERFACTUAL_BENCHMARK_CONTRACT,
    status: candidates.length ? "FINAL_KNOWLEDGE_RELEASE_REVIEW_CANDIDATES_READY" : "NO_FINAL_KNOWLEDGE_RELEASE_REVIEW_CANDIDATE",
