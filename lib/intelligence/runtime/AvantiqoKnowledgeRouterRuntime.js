...
export const AVANTIQO_REUSABLE_KNOWLEDGE_EVALUATION_CONTRACT =
...
function canonicalMissionKnowledge(recall) {
  return list(recall?.knowledge).slice(0, MAX_RESULTS).map((entry) => ({
    id: text(entry?.id, 240) || null,
    subject: text(entry?.subject, 1000) || null,
    content: text(entry?.content, 5000),
    verification_status: "AVANTIQO_CANONICAL_PRODUCT",
    reusable: true,
    confidence: 1,
    verified_at: text(entry?.updated_at, 120) || null,
    valid_until: null,
    freshness: "CANONICAL_PRODUCT_SYNC",
    provenance: {
      source: INTERNAL_SOURCE,
      authority: INTERNAL_AUTHORITY,
      internal_reference: text(entry?.internal_reference, 500) || null,
      domain: text(entry?.domain, 120) || null,
      product_object_type: text(entry?.product_object_type, 160) || null,
    },
    sources: [],
    authorization_effect: "NONE",
  }));
...
function hybridMissionKnowledge(recall) {
  return list(recall?.knowledge).slice(0, MAX_RESULTS).map((entry) => ({
    id: text(entry?.id, 240) || null,
    subject: text(entry?.subject, 1000) || null,
    content: text(entry?.content, 5000),
    ...
    freshness: entry?.retrieval_signals?.freshness_valid === false
      ? "STALE"
      : "VERIFIED_RETRIEVAL",
    provenance: object(entry?.provenance),
    sources: list(entry?.sources).slice(0, 8),
    authorization_effect: "NONE",
  }));
}

function missionKnowledgeEvaluation({
  status,
  knowledge,
  provenanceContracts,
  freshnessChecked,
  evidenceGraphChecked,
} = {}) {
  return {
    evaluated: true,
    status,
    knowledge: list(knowledge),
    provenance_contracts: list(provenanceContracts).filter(Boolean),
    freshness_checked: freshnessChecked === true,
    evidence_graph_checked: evidenceGraphChecked === true,
...
export async function evaluateAvantiqoReusableKnowledge({
  ...
  if (!query) throw new Error("AVANTIQO_REUSABLE_KNOWLEDGE_QUERY_REQUIRED");
  const organizationId = learningOrganizationId();
  const internalProductState = productStateQuery(query);
  let canonicalRecall = null;

  if (internalProductState) {
    canonicalRecall = await recallCanonicalProductKnowledge(query).catch((error) => ({
      available: false,
      sufficient: false,
      reason: "CANONICAL_PRODUCT_RECALL_READ_FAILED",
      error: text(error?.message || error, 500),
      knowledge: [],
    }));
    if (canonicalRecall.sufficient) {
      const learnedKnowledge = missionKnowledgeEvaluation({
        status: "REUSED_VERIFIED_KNOWLEDGE",
        knowledge: canonicalMissionKnowledge(canonicalRecall),
        provenanceContracts: [AVANTIQO_KNOWLEDGE_ROUTER_CONTRACT],
        freshnessChecked: true,
        evidenceGraphChecked: false,
      });
      return {
        success: true,
        contract: AVANTIQO_REUSABLE_KNOWLEDGE_EVALUATION_CONTRACT,
        status: "REUSED_VERIFIED_KNOWLEDGE",
        query,
  ...
  if (hybridRecall?.sufficient === true) {
    const learnedKnowledge = missionKnowledgeEvaluation({
      status: "REUSED_VERIFIED_KNOWLEDGE",
      knowledge: hybridMissionKnowledge(hybridRecall),
      provenanceContracts: [
        AVANTIQO_KNOWLEDGE_ROUTER_CONTRACT,
        hybridRecall.contract,
      ],
      freshnessChecked: true,
      evidenceGraphChecked: true,
    });
    return {
      success: true,
      contract: AVANTIQO_REUSABLE_KNOWLEDGE_EVALUATION_CONTRACT,
  ...
  const learnedKnowledge = missionKnowledgeEvaluation({
    ...
    freshnessChecked: Boolean(organizationId),
    evidenceGraphChecked: hybridRecall?.evidence_graph != null,
  });
  return {
    success: true,
    contract: AVANTIQO_REUSABLE_KNOWLEDGE_EVALUATION_CONTRACT,
    ...
    query,
    learned_knowledge: learnedKnowledge,
    route: organizationId ? "NO_SAFE_REUSABLE_KNOWLEDGE" : "LEARNING_ORGANIZATION_NOT_CONFIGURED",
    reason: hybridRecall?.reason || canonicalRecall?.reason ||
      (organizationId ? "NO_RELEVANT_VERIFIED_KNOWLEDGE" : "LEARNING_ORGANIZATION_NOT_CONFIGURED"),
    governance: {
      platform_learning_organization_only: true,
      customer_organization_used_for_platform_knowledge: false,
      customer_private_memory_reused: false,
      weak_or_stale_knowledge_reused: false,
      web_research_fallback_performed: false,
      internet_search_performed: false,
      fresh_research_performed: false,
      external_intelligence_provider_used: false,
      database_write_performed: false,
