...
export async function runAvantiqoKnowledgeAwareResearch({ context = {}, payload = {} } = {}) {
  ...
  if (payload.force_refresh !== true && productStateQuery(query)) {
  ...
  const result = await runKnowledgeAwareWebResearch({ context, payload });
  return {
    ...result,
    knowledge_router_contract: AVANTIQO_KNOWLEDGE_ROUTER_CONTRACT,
    canonical_product_recall_attempted: productStateQuery(query),
  };
}
