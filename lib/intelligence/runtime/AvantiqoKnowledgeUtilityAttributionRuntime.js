...
} from "../../operator/runtime/IntelligenceFailureLearningPolicy.js";
...
async function resolveDatabase(database = null) {
  if (database && typeof database.from === "function") return database;
...
function rollingUtilityBucket(receipt, now) {
  const dayNumber = Math.floor(
    Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()) / DAY_MS,
  );
  const slot = ((dayNumber % RETENTION_DAYS) + RETENTION_DAYS) % RETENTION_DAYS;
  const fingerprint = sha([
    text(receipt?.receipt_fingerprint, 80),
    text(receipt?.outcome, 80),
    text(receipt?.failure_fingerprint, 120),
  ].join("|"), 32);
  return {
    slot,
    fingerprint,
    memory_key: `knowledge-utility:${fingerprint}:slot-${slot}`,
  };
}
...
export async function recordAvantiqoKnowledgeUtilityObservation({
  ...
  const observationKey = text(observation_key, 2000) || null;
  const observationKeyFingerprint = observationKey ? sha(observationKey, 48) : null;
  const now = new Date();
  const nowIso = now.toISOString();
  const bucket = rollingUtilityBucket(receipt, now);
  const memoryKey = observationKeyFingerprint
    ? `knowledge-utility:${observationKeyFingerprint}`
    : bucket.memory_key;
  ...
  const row = {
    ...
    metadata: {
      ...receipt,
      observed_at: nowIso,
      observation_key_fingerprint: observationKeyFingerprint,
      idempotent_observation: Boolean(observationKeyFingerprint),
      rolling_bucket: !observationKeyFingerprint,
      rolling_bucket_slot: observationKeyFingerprint ? null : bucket.slot,
      rolling_bucket_fingerprint: observationKeyFingerprint ? null : bucket.fingerprint,
  ...
  if (!observationKeyFingerprint) row.created_at = nowIso;
  const write = client
    .from(MEMORY_TABLE)
