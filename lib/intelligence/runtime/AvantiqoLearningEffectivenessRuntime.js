...
function topicEffectiveness({ agenda, state, nowMs }) {
...
export async function evaluateAvantiqoLearningEffectiveness({
  ...
  const state = await loadState(organization, lookback);
  const nowMs = Date.now();
  const items = state.agendas.map((agenda) => topicEffectiveness({ agenda, state, nowMs }));
  ...
  return {
    success: true,
    contract: AVANTIQO_LEARNING_EFFECTIVENESS_CONTRACT,
    status: weak.length ? "LEARNING_EFFECTIVENESS_GAPS_IDENTIFIED" : "LEARNING_EFFECTIVENESS_HEALTHY",
    ...
    highest_attention_topics: items
      .slice()
      .sort((left, right) => {
        const leftNeed = (1 - left.effectiveness_score) + left.runtime_training_signal_count * 0.05;
        const rightNeed = (1 - right.effectiveness_score) + right.runtime_training_signal_count * 0.05;
