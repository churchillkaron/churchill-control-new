...
export const AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT =
  "AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_V1";

export const AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_ALGORITHM =
  "HMAC-SHA256";
...
const DOMAIN_SEPARATOR = "avantiqo-learning-evidence-candidate-authenticity-v1";
...
function parseKeyring({ requireActive = false, env = process.env } = {}) {
  ...
  if (!rawKeyring) {
    return {
      valid: false,
      reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_KEYRING_REQUIRED",
  ...
  try {
    ...
    return {
      valid: false,
      reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_KEYRING_JSON_INVALID",
      active_key_id: activeKeyId || null,
      key_count: 0,
      key_ids: [],
      keys: new Map(),
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return {
      valid: false,
      reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_KEYRING_OBJECT_REQUIRED",
      active_key_id: activeKeyId || null,
      key_count: 0,
      key_ids: [],
      keys: new Map(),
  ...
  if (!entries.length || entries.length > MAX_KEYRING_KEYS) {
    return {
      ...
      reason: entries.length
        ? "EVIDENCE_CANDIDATE_AUTHENTICITY_KEYRING_TOO_LARGE"
        : "EVIDENCE_CANDIDATE_AUTHENTICITY_KEYRING_EMPTY",
      active_key_id: activeKeyId || null,
  ...
  for (const [rawKeyId, rawSecretHex] of entries) {
    ...
    if (!KEY_ID_RE.test(keyId)) {
      return {
        valid: false,
        reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_KEY_ID_INVALID",
        active_key_id: activeKeyId || null,
        key_count: entries.length,
        key_ids: [],
        keys: new Map(),
    ...
    if (!KEY_HEX_RE.test(secretHex)) {
      return {
        valid: false,
        reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_KEY_MUST_BE_256_OR_512_BIT_HEX",
        active_key_id: activeKeyId || null,
        key_count: entries.length,
        key_ids: [],
        keys: new Map(),
  ...
  if (requireActive) {
    if (!KEY_ID_RE.test(activeKeyId)) {
      return {
        valid: false,
        reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_ACTIVE_KEY_ID_REQUIRED",
        active_key_id: activeKeyId || null,
        key_count: keys.size,
        key_ids: keyIds,
        keys,
    ...
    if (!keys.has(activeKeyId)) {
      return {
        valid: false,
        reason: "EVIDENCE_CANDIDATE_AUTHENTICITY_ACTIVE_KEY_NOT_IN_KEYRING",
...
function authenticityPayload(row) {
  ...
  delete metadata.evidence_candidate_authenticity_mac;
  return {
...
function macWithSecretHex(row, secretHex) {
  ...
  return createHmac("sha256", key)
    ...
    .update(`${AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT}\u0000`, "utf8")
...
export function getAvantiqoLearningEvidenceCandidateAuthenticityStatus({
  require_active = false,
} = {}) {
  const configuration = parseKeyring({ requireActive: require_active === true });
  return {
    success: true,
    contract: AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT,
    algorithm: AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_ALGORITHM,
...
export function sealAvantiqoLearningEvidenceCandidateAuthenticity(row) {
  const configuration = parseKeyring({ requireActive: true });
  if (!configuration.valid) {
    return {
      success: false,
      contract: AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT,
      status: "EVIDENCE_CANDIDATE_AUTHENTICITY_NOT_SEALED",
      reason: configuration.reason,
      row: null,
    };
  }
  ...
  sealed.metadata = {
    ...
    evidence_candidate_authenticity_key_id: configuration.active_key_id,
  };
  const secretHex = configuration.keys.get(configuration.active_key_id);
  sealed.metadata.evidence_candidate_authenticity_mac = macWithSecretHex(
    sealed,
    secretHex,
  );
  ...
  return {
    success: true,
    contract: AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT,
    status: "EVIDENCE_CANDIDATE_AUTHENTICITY_SEALED",
    key_id: configuration.active_key_id,
    row: sealed,
  };
}

export function createAvantiqoLearningEvidenceCandidateAuthenticityVerifier() {
  ...
  return Object.freeze({
    ...
    verify(row) {
      ...
      const metadata = object(row?.metadata);
      if (
        text(metadata.evidence_candidate_authenticity_contract, 180) !==
          AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT ||
        text(metadata.evidence_candidate_authenticity_algorithm, 40) !==
          AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_ALGORITHM
      ) {
        return false;
      }
      const keyId = text(metadata.evidence_candidate_authenticity_key_id, 80);
      const suppliedMac = text(metadata.evidence_candidate_authenticity_mac, 64).toLowerCase();
...
export default Object.freeze({
  contract: AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_CONTRACT,
  algorithm: AVANTIQO_LEARNING_EVIDENCE_CANDIDATE_AUTHENTICITY_ALGORITHM,
  getStatus: getAvantiqoLearningEvidenceCandidateAuthenticityStatus,
  seal: sealAvantiqoLearningEvidenceCandidateAuthenticity,
  createVerifier: createAvantiqoLearningEvidenceCandidateAuthenticityVerifier,
});
