...
export const AVANTIQO_LEARNING_EVIDENCE_MECHANISM_AGENDA_CONTRACT =
  "AVANTIQO_LEARNING_EVIDENCE_MECHANISM_AGENDA_V1";
...
function normalizedNow(now) {
  ...
  const parsed = new Date(text(now, 120));
...
export function buildAvantiqoLearningEvidenceMechanismAgendaRow({
  ...
  if (!normalizedOrganizationId) {
    throw new Error("AVANTIQO_LEARNING_EVIDENCE_MECHANISM_AGENDA_ORGANIZATION_REQUIRED");
  }

  const normalizedNowValue = normalizedNow(now);
  const admission = assessAvantiqoLearningEvidenceCandidateBridgeEligibility(
    candidate,
    { now: normalizedNowValue },
  );
  if (!admission.eligible) {
    throw new Error(
      `AVANTIQO_LEARNING_EVIDENCE_MECHANISM_AGENDA_CANDIDATE_NOT_ADMITTED:${admission.blockers.join(",")}`,
    );
  }
  ...
  const nowIso = normalizedNowValue.toISOString();
  ...
  return {
    ...
    metadata: {
      ...
      agenda_contract: AVANTIQO_LEARNING_EVIDENCE_MECHANISM_AGENDA_CONTRACT,
      ...
      candidate_admission_rechecked: true,
      ...
      synthesis_local_first_required: true,
      synthesis_modal_overflow_allowed_with_approval: true,
      synthesis_modal_overflow_contract: "AVANTIQO_INTELLIGENCE_MODAL_OVERFLOW_POLICY_V1",
...
export async function reconcileAvantiqoLearningEvidenceCandidates({
  ...
  return {
    ...
    policy: {
      ...
      mechanism_agenda_is_reusable_knowledge: false,
      mechanism_agenda_router_reuse_allowed: false,
      mechanism_agenda_direct_platform_knowledge_write_allowed: false,
      direct_platform_knowledge_promotion_allowed: false,
      ...
      local_first_required_for_owned_deep_synthesis: true,
      governed_modal_overflow_allowed_for_irreducible_deep_synthesis: true,
