...
function scopeQuery(organizationId, scope) {
...
export async function inspectAvantiqoLearningHealth({ now = new Date() } = {}) {
  ...
  const [activeAgenda, dueAgenda, errorAgenda, activeKnowledge, trainingCandidates, latestRun] =
    await Promise.all([
      countRows(scopeQuery(organizationId, "platform_learning_agenda").eq("active", true)),
      countRows(
        scopeQuery(organizationId, "platform_learning_agenda")
          .eq("active", true)
          .lte("metadata->>next_research_at", nowIso),
      ),
      countRows(
        scopeQuery(organizationId, "platform_learning_agenda")
          .eq("active", true)
      ...
      countRows(scopeQuery(organizationId, "platform_knowledge").eq("active", true)),
      countRows(scopeQuery(organizationId, "platform_training_candidates").eq("active", true)),
      supabaseAdmin
        .from(MEMORY_TABLE)
