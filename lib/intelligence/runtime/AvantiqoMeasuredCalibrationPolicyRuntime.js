...
export async function loadAvantiqoMeasuredCalibrationPolicy({ organizationId } = {}) {
  ...
  const capabilityProfiles = coverageRows.map((row) => {
    ...
    const reliabilityEstimate = metadata.capability_reliability_estimate === null || metadata.capability_reliability_estimate === undefined ? null : bounded(metadata.capability_reliability_estimate, 0);
    const reliabilityConfidence = bounded(metadata.capability_reliability_confidence, 0);
    const reliabilityMaturity = text(metadata.capability_reliability_maturity, 80) || "UNPROVEN";
    const repeatedVerifiedRisk = verifiedFailureCount >= 2 && reliabilityEstimate !== null && reliabilityEstimate < 0.7 && reliabilityConfidence >= 0.25;
    const profileClass = repeatedVerifiedRisk
    ...
    return {
      ...
      reliability_maturity: reliabilityMaturity,
      repeated_verified_outcome_risk: repeatedVerifiedRisk,
