...
function programRow({ organizationId, agenda, gap, mode, tracks, runs, allRuns, nowIso }) {
  ...
  const synthesisLane = mode === "invention" ? "deep" : "fast";
  const status = mode === "evidence"
    ? "EVIDENCE_LEARNING_ONLY"
    : evidenceReady
      ? "READY_FOR_LOCAL_SYNTHESIS"
  ...
  return {
    ...
    metadata: {
      ...
      synthesis_runtime_contract: "AVANTIQO_INTELLIGENCE_LOCAL_NODE_V1",
      synthesis_local_only: mode !== "evidence",
      synthesis_model: mode === "mechanism" ? "qwen3:4b-instruct" : null,
      synthesis_external_compute_allowed: false,
      synthesis_spend_approval_required: false,
...
export async function reconcileAvantiqoMechanismFirstLearning({
  ...
  const existingAgendaKeys = new Set(admissibleAgendas.map(topicKey));
  ...
  const synthesisReady = programRows.filter((row) =>
    object(row.metadata).status === "READY_FOR_LOCAL_SYNTHESIS",
  );
  ...
  return {
    ...
    local_synthesis_ready_count: synthesisReady.length,
    ...
    highest_priority_programs: programRows.slice(0, 10).map((row) => ({
      ...
      synthesis_local_only: object(row.metadata).synthesis_local_only === true,
    })),
    principles: {
    ...
    governance: {
      ...
      hypothesis_synthesis_requires_owned_local_compute: true,
      synthesis_lane_policy: "mechanism=local-fast-4b,invention=local-deep-or-fail-closed",
      automatic_gpu_execution: false,
      automatic_external_compute_submission: false,
      automatic_experiment_execution: false,
      automatic_model_weight_mutation: false,
