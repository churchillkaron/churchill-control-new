...
function programRow({ organizationId, agenda, gap, mode, tracks, runs, allRuns, nowIso }) {
  ...
  return {
    ...
    metadata: {
      ...
      synthesis_runtime_contract: synthesisLane === "fast"
        ? "AVANTIQO_LOCAL_NODE_V1"
        : "AVANTIQO_INTELLIGENCE_MODAL_H100_V1",
      synthesis_local_first: mode === "mechanism",
      synthesis_model: mode === "mechanism" ? "qwen3:4b-instruct" : null,
      synthesis_modal_only: mode === "invention",
      synthesis_spend_approval_required: mode === "invention",
...
export async function reconcileAvantiqoMechanismFirstLearning({
  ...
  return {
    ...
    governance: {
      ...
      synthesis_lane_policy: "mechanism=local-fast-4b,invention=deep",
      automatic_gpu_execution: false,
      automatic_non_modal_submission: false,
      automatic_experiment_execution: false,
      automatic_model_weight_mutation: false,
