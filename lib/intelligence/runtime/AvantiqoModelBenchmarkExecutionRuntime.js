...
function benchmarkConfig() {
  ...
  const endpointId = text(process.env.RUNPOD_AVANTIQO_INTELLIGENCE_BENCHMARK_ENDPOINT_ID, 160);
  const apiKey = text(process.env.RUNPOD_API_KEY, 1000);
  if (!endpointId) throw new Error("RUNPOD_AVANTIQO_INTELLIGENCE_BENCHMARK_ENDPOINT_ID_REQUIRED");
  if (!/^[A-Za-z0-9_-]+$/.test(endpointId)) {
    throw new Error("RUNPOD_AVANTIQO_INTELLIGENCE_BENCHMARK_ENDPOINT_ID_INVALID");
  }
  ...
  return {
    ...
    timeoutMs: Math.max(1000, Number(process.env.AVANTIQO_INTELLIGENCE_BENCHMARK_TIMEOUT_MS || DEFAULT_TIMEOUT_MS)),
  };
}
...
async function submitGeneration({ config, mode, suite, adapterArtifactReference = null }) {
  ...
  if (!jobId) throw new Error(`AVANTIQO_BENCHMARK_${mode.toUpperCase()}_PROVIDER_JOB_ID_REQUIRED`);
  return jobId;
}

async function persistRun({ organizationId, trainingJob, suite, baselineJobId, candidateJobId, readiness }) {
  ...
  const metadata = {
    ...
    baseline_provider_job_id: baselineJobId,
    candidate_provider_job_id: candidateJobId,
  ...
  const result = await supabaseAdmin.from(MEMORY_TABLE).insert({
    ...
    memory_type: "goal",
    subject: runId,
    content: `Submitted matched baseline/candidate benchmark ${runId}.`,
    importance: 0.98,
    confidence: 1,
...
function validatedOutputs(body, expectedCount, expectedMode) {
  ...
  if (text(output.mode, 40) !== expectedMode) throw new Error("AVANTIQO_BENCHMARK_WORKER_MODE_MISMATCH");
  if (Number(output.case_count || 0) !== expectedCount) throw new Error("AVANTIQO_BENCHMARK_WORKER_CASE_COUNT_MISMATCH");
...
export async function submitAvantiqoModelBenchmark({ trainingJobId, benchmarkSuiteId, approved = false } = {}) {
  ...
  const baselineJobId = await submitGeneration({ config, mode: "baseline", suite });
  const candidateJobId = await submitGeneration({ config, mode: "candidate", suite, adapterArtifactReference: adapter });
  const run = await persistRun({ organizationId, trainingJob, suite, baselineJobId, candidateJobId, readiness });
  return {
    contract: AVANTIQO_MODEL_BENCHMARK_EXECUTION_CONTRACT,
    status: "BENCHMARK_SUBMITTED",
    run,
...
export async function refreshAvantiqoModelBenchmark({ benchmarkRunId } = {}) {
  ...
  const [baseline, candidate] = await Promise.all([
    providerStatus(config, text(metadata.baseline_provider_job_id, 240)),
    providerStatus(config, text(metadata.candidate_provider_job_id, 240)),
  ]);
  ...
  const baselineOutputs = validatedOutputs(baseline.body, caseCount, "baseline");
  const candidateOutputs = validatedOutputs(candidate.body, caseCount, "candidate");
