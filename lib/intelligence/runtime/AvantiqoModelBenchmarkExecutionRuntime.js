...
async function persistRun({ organizationId, trainingJob, suite, baselineJobId, candidateJobId }) {
...
export async function submitAvantiqoModelBenchmark({ trainingJobId, benchmarkSuiteId, approved = false } = {}) {
  ...
  const run = await persistRun({ organizationId, trainingJob, suite, baselineJobId, candidateJobId });
  return {
    contract: AVANTIQO_MODEL_BENCHMARK_EXECUTION_CONTRACT,
    status: "BENCHMARK_SUBMITTED",
    run,
