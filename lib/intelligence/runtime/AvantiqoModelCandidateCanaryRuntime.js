...
import {
  certifyAvantiqoModelCandidateReadiness,
} from "./AvantiqoModelCandidateReadinessRuntime";
import {
...
function config() {
  ...
  if (!configuredEndpointId) {
    throw new Error("RUNPOD_AVANTIQO_INTELLIGENCE_CANDIDATE_ENDPOINT_ID_REQUIRED");
  }
  const safeLease = requireAvantiqoModelImprovementSafeLease("candidate", {
...
export async function certifyAvantiqoModelCandidateCanary({
  ...
  const candidateReadiness = await certifyAvantiqoModelCandidateReadiness({
    modelCandidateId: candidate.id,
  });
  if (candidateReadiness.status !== "CANDIDATE_LINEAGE_CURRENT") {
    throw new Error(
      `AVANTIQO_CANDIDATE_LINEAGE_READINESS_FAILED:${text(candidateReadiness.status, 120)}`,
    );
  }
  const adapterArtifactReference = text(candidate?.metadata?.adapter_artifact_reference, 1000);
  if (
    text(candidateReadiness.adapter_artifact_reference, 1000) !==
    adapterArtifactReference
  ) {
    throw new Error("AVANTIQO_CANDIDATE_LINEAGE_ADAPTER_MISMATCH");
  }
  ...
  return {
    ...
    lineage_readiness: candidateReadiness,
    health: {
    ...
    certification: {
      ...
      safe_lease_contract: candidateConfig.safeLease.safe_lease_contract,
      safe_lease_lane: candidateConfig.safeLease.lease_lane,
      leased_endpoint_binding_verified:
        candidateConfig.safeLease.endpoint_id === candidateConfig.endpointId,
      current_candidate_lineage_verified: true,
      current_training_job_verified: true,
      current_benchmark_run_verified: true,
      current_benchmark_suite_verified: true,
      current_dataset_and_examples_verified: true,
    ...
    governance: {
      ...
      candidate_lineage_readiness_required: true,
