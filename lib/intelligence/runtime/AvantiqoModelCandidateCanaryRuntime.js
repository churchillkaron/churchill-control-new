...
const CANDIDATE_MODEL = "avantiqo-intelligence-candidate-v1";
...
function parseFinalMessage(response = {}) {
  ...
  return {
    ...
    finishReason: text(choice.finish_reason, 80) || null,
  };
}
...
export async function certifyAvantiqoModelCandidateCanary({
  ...
  const registered = list(models?.data).some((item) => text(item?.id, 300) === CANDIDATE_MODEL);
  if (!registered) throw new Error("AVANTIQO_CANDIDATE_LORA_MODEL_NOT_REGISTERED");
    ...
    `${candidateConfig.openaiBase}/chat/completions`,
    {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: 'Return exactly this semantic result: {"status":"ok","candidate":"avantiqo-intelligence-candidate-v1"}.' },
        ],
        temperature: 0,
        max_tokens: 1024,
  ...
  if (
    ...
    object(structuredObject).candidate !== CANDIDATE_MODEL
  ) {
  ...
  return {
    ...
    adapter_artifact_reference: text(candidate?.metadata?.adapter_artifact_reference, 1000),
    health: {
