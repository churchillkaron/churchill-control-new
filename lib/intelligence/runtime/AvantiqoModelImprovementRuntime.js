...
function adapterRecipe(dataset = {}) {
  ...
  return {
    ...
    train_count: Number(metadata.train_count || 0),
    holdout_count: Number(metadata.holdout_count || 0),
...
export async function prepareAvantiqoModelTrainingJob({ datasetId } = {}) {
  ...
  const jobFingerprint = stableHash(`${datasetFingerprint}|${FOUNDATION_MODEL}|PEFT_ADAPTER`);
  const jobId = `avantiqo-intelligence-train-${jobFingerprint.slice(0, 16)}`;
  const now = new Date().toISOString();
  const recipe = adapterRecipe(dataset);
  const row = {
