...
import { requireAvantiqoModelImprovementSafeLease } from "@/lib/intelligence/runtime/AvantiqoModelImprovementSafeLeaseGuard";
...
function trainerConfig() {
  ...
  const safeLease = requireAvantiqoModelImprovementSafeLease("trainer", {
...
export async function submitAvantiqoModelTrainingJob({
  ...
  if (readiness.status !== "READY_FOR_RESOURCE_PREFLIGHT") {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_TRAINER_READINESS_FAILED:${text(readiness.status, 120)}`,
    );
  }
  ...
  const sharedTrainerReservation = await assertAvantiqoSharedTrainerReservation({
    trainerEndpointId: config.endpointId,
    queueApiKey: config.apiKey,
    managementApiKey: config.managementApiKey,
    timeoutMs: config.timeoutMs,
  });
  if (sharedTrainerReservation.exclusive_trainer_reservation !== true) {
    throw new Error("AVANTIQO_INTELLIGENCE_TRAINER_EXCLUSIVE_RESERVATION_REQUIRED");
  }
  ...
  const updated = await updateJob(organizationId, job, {
    ...
    shared_trainer_reservation: sharedTrainerReservation,
    safe_lease: config.safeLease,
  ...
  return {
    ...
    governance: {
      ...
      safe_lease_contract: config.safeLease.safe_lease_contract,
      ...
      live_training_readiness_verified: true,
      ...
      shared_trainer_exclusive_reservation_verified:
        sharedTrainerReservation.exclusive_trainer_reservation === true,
      shared_trainer_stable_observations:
        Number(sharedTrainerReservation.stable_observations || 0),
      code_or_intelligence_reservation_present:
        sharedTrainerReservation.code_or_intelligence_reservation_present === true,
...
export async function refreshAvantiqoModelTrainingJob({ trainingJobId } = {}) {
  ...
  if (submittedLeaseEndpoint && submittedLeaseEndpoint !== config.endpointId) {
    throw new Error("AVANTIQO_INTELLIGENCE_TRAINER_SAFE_LEASE_ENDPOINT_CHANGED");
  }
  const response = await fetchWithTimeout(
