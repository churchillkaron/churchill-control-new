...
const SUPPORTED_CANDIDATE_CONTRACTS = Object.freeze({
  CANONICAL_PRODUCT_GROUNDING: "AVANTIQO_CANONICAL_CURRICULUM_CANDIDATE_V1",
  CREATIVE_PREFERENCE_PAIR: "AVANTIQO_CREATIVE_TRAINING_CANDIDATE_V1",
  VERIFIED_FAILURE_RECOVERY: null,
  GENERAL_INTELLIGENCE_TRANSFER_DISCIPLINE: "AVANTIQO_GENERAL_INTELLIGENCE_TRAINING_CANDIDATE_V1",
});
...
function assertCandidates(datasetMetadata, candidates) {
  ...
  for (const candidateId of candidateIds) {
    ...
    const candidateKind = text(metadata.candidate_kind, 120);
    const expectedContract = SUPPORTED_CANDIDATE_CONTRACTS[candidateKind];
    const contractValid = Object.prototype.hasOwnProperty.call(SUPPORTED_CANDIDATE_CONTRACTS, candidateKind) &&
      (expectedContract === null || metadata.contract === expectedContract);
    if (
      !contractValid ||
      metadata.training_ready !== true ||
