...
const EXPECTED_INFRASTRUCTURE = "AVANTIQO_LOCAL_NODE_V1";
...
function normalizeArm(value = {}) {
  ...
  return {
    ...
    local_node: value.local_node === true,
    external_compute_performed: value.external_compute_performed === true,
    raw_reasoning_persisted: value.raw_reasoning_persisted === true,
    model_elapsed_seconds: Math.max(0, number(value.model_elapsed_seconds)),
...
function evaluateArm(benchmarkCase, arm) {
  ...
  const infrastructureSafe =
    ...
    arm.local_node === true &&
    arm.external_compute_performed === false &&
...
export function evaluateAvantiqoOwnedModelMissionLearningCertification({
  ...
  const baselineLatency = mean(evaluations.map((row) => row.baseline.model_elapsed_seconds).filter((value) => value > 0));
  const candidateLatency = mean(evaluations.map((row) => row.candidate.model_elapsed_seconds).filter((value) => value > 0));
