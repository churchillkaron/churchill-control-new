...
const OWNED_RESEARCH_SOURCES_BY_DOMAIN = Object.freeze({
  ...
  "energy-offshore": Object.freeze([
    openEvidenceSource({
      id: "energy-equinor-johan-sverdrup",
      url: "https://www.equinor.com/energy/johan-sverdrup",
      title: "Johan Sverdrup",
      publisher: "Equinor",
      sourceType: "official_operator",
    }),
    openEvidenceSource({
      id: "energy-equinor-troll",
      url: "https://www.equinor.com/energy/troll",
      title: "Troll",
      publisher: "Equinor",
      sourceType: "official_operator",
    }),
    openEvidenceSource({
      id: "energy-offshore-norge-helicopter-safety",
      url: "https://www.offshorenorge.no/en/topics/hse/storulykkerisiko/helicopter-safety/",
      title: "Helicopter safety",
      publisher: "Offshore Norge",
      sourceType: "official_industry_guidance",
    }),
    openEvidenceSource({
      id: "energy-offshore-norge-helideck-manual",
      url: "https://www.offshorenorge.no/en/faginnhold/guidelines/guidelines/operations/aviatonhelicopter/074-recommended-guidelines-for-helideck-personnel/",
      title: "Recommended guidelines - Helideck Manual",
      publisher: "Offshore Norge",
      sourceType: "official_industry_guidance",
    }),
    openEvidenceSource({
      id: "energy-norwegian-petroleum-production",
      url: "https://www.norskpetroleum.no/en/production-and-exports/production-forecasts/",
      title: "Production forecasts",
      publisher: "Norwegian Petroleum",
      sourceType: "official_public_information",
    }),

    openEvidenceSource({
      id: "energy-equinor-heidrun-helicopter",
      url: "https://www.equinor.com/energy/heidrun",
      title: "Heidrun - SAR helicopter at offshore platform",
      publisher: "Equinor",
      sourceType: "official_operator",
    }),
    openEvidenceSource({
      id: "energy-equinor-snorre-helicopter",
      url: "https://web-equinor-web-sites-prod.c2.radix.equinor.com/no/energi/snorre",
      title: "Snorre - helicopter landing on Snorre A",
      publisher: "Equinor",
      sourceType: "official_operator",
    }),
  ]),
...
const DOMAIN_ALIASES = Object.freeze({
  ...
  petroleum: "energy-offshore",
  oil_gas: "energy-offshore",
});

const QUERY_DOMAIN_SIGNALS = Object.freeze([
  ["energy-offshore", /\b(norway|norwegian|north sea|offshore|oil rig|oil platform|petroleum|helideck|helicopter|rotor)\b/i],
...
export async function collectAvantiqoOwnedWebEvidence({
  ...
  const sourceResults = await Promise.all(
    sourceSeeds.map(async (seed, index) => {
      ...
      try {
        ...
        if (!evidence) {
          throw new Error("AVANTIQO_OWNED_WEB_EVIDENCE_SOURCE_CONTENT_REQUIRED");
        }
        const finalUrl = text(
          result?.final_url || result?.source_url || seed.url,
          2_000,
        );
        ...
        return {
          source: {
            ...
            retrieved_at:
              text(result?.retrieved_at, 120) || new Date().toISOString(),
            content_hash_sha256:
              text(result?.content_hash_sha256, 128) || null,
          },
          failure: null,
        };
      } catch (error) {
  ...
  const sources = sourceResults.map((item) => item.source).filter(Boolean);
  const failures = sourceResults.map((item) => item.failure).filter(Boolean);
