...
export async function reconcileAvantiqoPersistentOrderingPolicyApplication() {
  ...
  const activePolicyResult = await supabaseAdmin
    .from("avantiqo_intelligence_persistent_ordering_policies")
    .select("policy_fingerprint,state,metadata")
    .eq("organization_id", organizationId)
    .eq("state", "ACTIVE")
    .maybeSingle();
  if (activePolicyResult.error) throw activePolicyResult.error;

  const activePolicy = activePolicyResult.data || null;
  const isPhase41Successor =
    activePolicy?.metadata?.policy_generation_kind ===
    "REBASED_SUCCESSOR_COMPOSITE_V1";
  const rpcName = isPhase41Successor
    ? "apply_avantiqo_policy_successor_v1"
    : "apply_avantiqo_intelligence_persistent_ordering_policy_v1";

  const result = await supabaseAdmin.rpc(rpcName, {
    p_organization_id: organizationId,
  });
  if (result.error) throw result.error;
  ...
  return {
    contract: AVANTIQO_PERSISTENT_ORDERING_POLICY_AUTHORITY_CONTRACT,
    ...payload,
    phase41_successor_dispatch: isPhase41Successor,
    automatic_activation: false,
    candidate_membership_change_authorized: false,
    source_numeric_score_mutation_authorized: false,
    execution_authorized: false,
