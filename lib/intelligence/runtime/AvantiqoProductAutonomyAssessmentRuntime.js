...
const CODE_AI_EXECUTION_KEY_HINT =
  "Use one opaque 12-160 character execution_key across Code AI execution, engineering verification, optional governed commit, and commit verification. The key has no authorization effect.";
...
function assessmentSystem() {
  return [
    ...
    "Engineering verification and GitHub persistence are distinct. If persistence is later requested, platform.code_ai_commit.execute must consume the same opaque execution_key only after its separate commit permission and confirmation gates, and platform.code_ai_commit_status.verify must independently verify the persisted main-branch result.",
    "Do not authorize commits, production deployments, destructive database operations or governance bypasses. You may describe the separately governed persistence handoff without granting it.",
...
export async function assessAvantiqoProductAutonomy({
  ...
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    ...
    critique_instructions: [
      ...
      "Keep GitHub persistence separate from engineering completion. Never imply that the commit capability is authorized merely because it exists; it still requires its own permission and confirmation, followed by platform.code_ai_commit_status.verify.",
    ].join(" "),
  ...
  return {
    ...
    evidence_limits: [
      ...
      "Engineering verification is not GitHub persistence evidence; use the separately governed commit capability and registered commit status verifier if persistence is requested.",
