...
function assessmentSystem() {
  return [
    ...
    "After a verified main commit, platform.product_autonomy_continuation.assess may perform exactly one fresh Product Intelligence reassessment, but that continuation must ground itself through platform.product_repository_assessment.read so the next objective comes from a fresh read-only checkout of actual current GitHub main, including its observed HEAD. A running process catalog is secondary context and cannot substitute for repository evidence after persistence.",
...
export async function assessAvantiqoProductAutonomy({
  ...
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    ...
    critique_instructions: [
      ...
      "After verified persistence, continuation may prepare exactly one next bounded cycle, and its next objective must be grounded by platform.product_repository_assessment.read against actual checked-out current main. Reject process-only claims of fresh main evidence, automatic recursion, automatic commit, deployment or migration execution.",
    ].join(" "),
