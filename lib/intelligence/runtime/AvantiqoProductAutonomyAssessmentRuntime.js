...
function assessmentSystem() {
  return [
    ...
    "The current cycle's completed focus is historical evidence after engineering completes; it must not automatically become the focus of the post-commit next-objective assessment. By default the next objective is selected fresh from current main without bias from the completed task.",
    "Only an explicitly supplied continuation_focus may constrain the single post-commit reassessment to the same broader product area. continuation_focus is context only, has no authorization effect, and still cannot override fresh current-main evidence.",
    ...
    "If a server-owned commit-attempt marker exists, exact recovery finds no matching verified commit, and current GitHub main has advanced beyond the attested base, platform.product_persistence_decision.assess must return STAY_LOCAL with STALE_BASE_REPLAN_REQUIRED. The attempted artifact becomes historical engineering evidence only: never retry it, never auto-rebase it, never create a replacement commit from it, and never let model reasoning override the stale-base decision. Start a fresh repository-grounded Product Engineering Cycle from observed current main and produce new verification evidence before persistence is considered again.",
...
export async function assessAvantiqoProductAutonomy({
  ...
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    ...
    critique_instructions: [
      ...
      "Do not carry the completed engineering focus into the next post-commit assessment by default. The next objective must be fresh from current main unless the caller explicitly supplied continuation_focus; even then treat it only as prioritization context.",
      ...
      "If a prior commit attempt cannot be exactly recovered and current main advanced beyond the attested base, require STAY_LOCAL / STALE_BASE_REPLAN_REQUIRED. Treat the old artifact as historical evidence only and reject retry, automatic rebase, replacement commit or model override; require a fresh current-main Product Engineering Cycle.",
  ...
  return {
    ...
    recommended_product_engineering_cycle_handoff: {
      ...
      completed_focus_automatically_reused_after_cycle: false,
      continuation_focus_required_to_bias_next_assessment: true,
    ...
    optional_persistence_handoff: {
      ...
      attempted_artifact_stale_base_reason_code: "STALE_BASE_REPLAN_REQUIRED",
      ...
      continuation_focus_is_optional_explicit_context_only: true,
    ...
    evidence_limits: [
      ...
      "A completed engineering focus is not automatically reused to choose the next post-commit objective; only explicit continuation_focus may constrain that fresh reassessment, and it remains context rather than authority.",
      ...
      "An attempted artifact whose exact commit cannot be recovered after main advanced is stale historical evidence only; it must not be retried, auto-rebased or converted into a replacement commit. Replan from fresh current main.",
