...
export async function assessAvantiqoProductAutonomy({
  ...
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    ...
    critique_instructions: [
      ...
      "Keep GitHub persistence separate from engineering completion. Never imply that the persistence decision or commit capability is authorized merely because it exists; the decision is read-only, and commit still requires its own permission and confirmation followed by platform.code_ai_commit_status.verify.",
  ...
  return {
    ...
    evidence_limits: [
      ...
      "A Product Intelligence persistence recommendation is not commit authorization.",
