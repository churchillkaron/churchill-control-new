...
} from "@/lib/code/runtime/CodeAIProductPortfolioProgressIntegrityRuntime";
...
function safeHistoryEntry(entry = {}) {
  return {
    ...
    progress_integrity_verified: entry.progress_integrity_verified === true,
  };
}
...
export function completePortfolioNodeAfterVerifiedPersistence({
  ...
  executionKey,
  verifiedCommitSha,
  repositoryAncestryVerified = false,
} = {}) {
  const portfolio = structuredClone(object(portfolioValue));
  const node = activeNode(portfolio);
  ...
  const progressIntegrity = assertProductPortfolioProgressIntegrity({
    node,
    completed_objectives: portfolio.completed_objectives,
    verified_commit_sha: commit,
    repository_ancestry_verified: repositoryAncestryVerified === true,
  });

  node.status = "PERSISTED_VERIFIED";
  node.progress_integrity = progressIntegrity;
  ...
  const historyEntry = safeHistoryEntry({
    ...
    progress_integrity_verified: progressIntegrity.verified === true,
  });
...
export function compactProductEngineeringPortfolio(value = {}) {
  ...
  const roadmap = list(source.roadmap).slice(0, MAX_NODES).map((node) => ({
    ...
    progress_integrity_verified: node?.progress_integrity?.verified === true,
  }));
  return {
    contract: AVANTIQO_PRODUCT_ENGINEERING_PORTFOLIO_CONTRACT,
