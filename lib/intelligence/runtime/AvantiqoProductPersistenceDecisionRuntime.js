...
async function recoveredPersistenceEvidence({ context, executionKey }) {
  ...
  if (!artifact.found || !artifact.mission_state) return null;
  ...
  try {
    ...
    if (text(error?.message, 240) === RECOVERY_NOT_FOUND) return null;
    throw error;
  }
}
...
export async function decideAvantiqoProductPersistence({
  ...
  const recoveredCommit = await recoveredPersistenceEvidence({
    context,
    executionKey: key,
  });
  if (recoveredCommit?.verified === true) {
    return resultEnvelope({
      ...
      decision: {
        decision: "ALREADY_PERSISTED",
        rationale:
          "The normal server commit-state record is unavailable, but the retained attested Code AI artifact independently matches a verified GitHub main commit reachable from current main. The same source persistence must not be proposed again.",
