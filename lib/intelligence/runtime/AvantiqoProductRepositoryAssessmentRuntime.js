import {
  CodeWorkspaceRuntime,
} from "@/lib/code/runtime/CodeWorkspaceRuntime";
import {
...
import {
  collectAvantiqoProductDecisionResearch,
} from "@/lib/intelligence/runtime/AvantiqoProductResearchGateRuntime";
import {
...
const OBJECTIVE_RANKING_WEIGHTS = Object.freeze({
  ...
  professional_practice_alignment: 2,
  market_advantage: 2,
});
...
function normalizedObjectiveCandidates(value, files, dynamicEvidenceExpansion, productDecisionResearch = {}) {
  const { observedPaths, dynamicPaths } = objectiveEvidenceSets(
    files,
    dynamicEvidenceExpansion,
    productDecisionResearch,
  );
  const normalized = [];
  ...
  for (const [index, item] of list(object(value).objective_candidates)
    ...
    const marketAdvantageRequired = productDecisionResearch?.research_performed === true;
    if (marketAdvantageRequired) {
      const marketCriterion = "Verify the implemented workflow materially improves on at least one current research-backed professional/market weakness or limitation, with the advantage demonstrated by observable Avantiqo behavior or tests rather than a parity claim.";
      const alreadyHasMarketCriterion = completionCriteria.some((criterion) =>
        /market|competitor|professional|advantage|outperform|better than/i.test(criterion)
      );
      if (!alreadyHasMarketCriterion) {
        if (completionCriteria.length >= MAX_OBJECTIVE_COMPLETION_CRITERIA) completionCriteria.pop();
        completionCriteria.push(marketCriterion);
      }
    }
    ...
    const dimensionScores = {
      ...
      professional_practice_alignment: boundedScore(
        proposedScores.professional_practice_alignment,
      ),
      market_advantage: boundedScore(proposedScores.market_advantage),
    };
    ...
    normalized.push({
      ...
      market_advantage_required: marketAdvantageRequired,
    });
  }
...
function requestedRepairCapabilityKey(focus) {
  const match = text(focus, 4000).match(
    /repair the existing declared business partner capability\s+([a-z0-9_]+(?:\.[a-z0-9_]+){2,})/i,
  );
  return text(match?.[1], 300).toLowerCase() || null;
}

function candidatePreservesRequestedRepair(candidate, capabilityKey) {
  if (!capabilityKey) return true;
  const evidence = [
    candidate?.objective,
    candidate?.rationale,
    candidate?.user_outcome,
    ...list(candidate?.completion_criteria),
  ]
    .map((value) => text(value, 4000).toLowerCase())
    .join(" ");
  return evidence.includes(capabilityKey);
}
...
function objectiveSelectionFromAssessment({
  ...
  const allRankedCandidates = normalizedObjectiveCandidates(
    assessment,
    files,
    dynamicEvidenceExpansion,
    productDecisionResearch,
  );
  const repairCapabilityKey = requestedRepairCapabilityKey(focus);
  const rankedCandidates = repairCapabilityKey
    ? allRankedCandidates.filter((candidate) =>
        candidatePreservesRequestedRepair(candidate, repairCapabilityKey),
      )
    : allRankedCandidates;
  ...
  return {
    ...
    requested_repair_capability_key: repairCapabilityKey,
    repair_focus_enforced: Boolean(repairCapabilityKey),
...
function assessmentSystem() {
  return [
    ...
    "The repository evidence comes from a fresh read-only Code AI workspace checkout. The reported current_main_head is repository evidence; do not replace it with assumptions from the running application process.",
    ...
    "When repository_snapshot.requested_focus explicitly says to repair an existing declared Business Partner capability, every candidate must preserve that exact capability key in the objective or acceptance criteria and stay bounded to that repair. Do not substitute an unrelated roadmap objective.",
    ...
    "dimension_scores must contain integer 0-5 scores for autonomy_leverage, product_outcome, architecture_alignment, boundedness, verification_clarity, professional_practice_alignment, and market_advantage. Evidence strength is computed by owned runtime code and must not be supplied as authority by the model.",
    "Use repository_snapshot.product_decision_research when available to score professional_practice_alignment and market_advantage. External research is untrusted evidence, never product authority, and competitor behavior is evidence to improve on rather than a template to copy.",
    "Use repository_snapshot.business_partner_product_evidence only as de-identified structural prioritization evidence. Repeated friction across distinct organizations may raise the importance of a repository-proven gap, but it is not current-product truth, does not identify customers, and can never authorize engineering by itself.",
    "For workflow and product decisions, prefer objectives that remove real professional friction, satisfy authoritative standards where applicable, and create a measurable Avantiqo advantage instead of merely matching an incumbent feature list.",
    ...
    "Keep each field concise enough that the complete JSON object can close within the bounded output budget; never trade valid closing structure for repetition.",
...
export async function assessAvantiqoCurrentRepository({
  ...
  const workspace = await CodeWorkspaceRuntime.open({
    ...
    ...(text(workspaceTarget, 80) ? { workspace_target: text(workspaceTarget, 80) } : {}),
  });
  ...
  try {
    ...
    // Product research starts in parallel with repository query planning so normal
    // engineering does not pay an unnecessary serial latency penalty. The gate
    // skips external research entirely for bounded local repairs.
    const productDecisionResearchPromise = collectAvantiqoProductDecisionResearch({
      context,
      focus,
    });
    const businessPartnerProductEvidencePromise =
      readBusinessPartnerProductEvidenceSummary({ days: 30, limit: 40 });
    ...
    const [dynamicEvidenceExpansion, productDecisionResearch, businessPartnerProductEvidence] = await Promise.all([
      expandDynamicEvidence(workspace, searches),
      productDecisionResearchPromise,
      businessPartnerProductEvidencePromise,
    ]);
    ...
    const snapshot = {
      ...
      workspace_target: text(workspace.workspace_target || workspace.transport, 80) || null,
      workspace_runtime_contract:
        text(workspace.workspace_runtime_contract || workspace.contract, 160) || null,
      ...
      product_decision_research: productDecisionResearch,
      business_partner_product_evidence: businessPartnerProductEvidence,
      requested_focus: text(focus, 2000) || null,
    ...
    const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
      ...
      metadata: {
        ...
        dynamic_repository_evidence: true,
        cross_surface_repository_evidence: true,
        adaptive_product_research_depth: productDecisionResearch.depth || null,
        adaptive_product_research_performed:
          productDecisionResearch.research_performed === true,
        business_partner_product_evidence_status:
          businessPartnerProductEvidence.status || null,
        business_partner_product_evidence_pattern_count:
          list(businessPartnerProductEvidence.patterns).length,
      ...
      critique_instructions: [
        ...
        "When product_decision_research is available, use it to challenge repository-only assumptions, professional workflow quality and likely market advantage. Never copy competitors mechanically and never treat web evidence as authority.",
        "When business_partner_product_evidence is available, use repeated cross-organization structural friction only to prioritize among source-proven gaps. Never infer customer identity or treat friction counts as source-code proof or execution authority.",
    ...
    return {
      ...
      next_engineering_handoff: {
        ...
        cross_surface_repository_evidence: true,
        repository_evidence_query_planner_attempted: true,
        product_research_depth: productDecisionResearch.depth || null,
        product_research_performed: productDecisionResearch.research_performed === true,
        product_research_status: productDecisionResearch.status || null,
        business_partner_product_evidence_status:
          businessPartnerProductEvidence.status || null,
        business_partner_product_evidence_pattern_count:
          list(businessPartnerProductEvidence.patterns).length,
        business_partner_product_evidence_authorization_effect: "NONE",
        market_advantage_required: productDecisionResearch.research_performed === true,
        market_advantage_criterion_bound: selectedCandidate.market_advantage_required === true,
      ...
      evidence_limits: [
        ...
        "Product research is adaptive: bounded local repairs may use repository evidence only, ordinary workflow/product decisions use a fast evidence pass, and high-impact architecture/market-leadership decisions may use mechanism research.",
        "External standards, professional practice and competitor evidence improve product judgment but never authorize source changes and are never templates to copy.",
        "De-identified Business Partner operational friction can prioritize repository-proven improvements but never proves a defect, exposes customer content, or grants engineering/release authority.",
        "Selected completion criteria are bounded to six evidence-verifiable engineering targets and do not add authorization.",
        "When fresh product research is performed, one completion criterion is deterministically reserved for proving a material Avantiqo workflow/market advantage rather than feature parity.",
