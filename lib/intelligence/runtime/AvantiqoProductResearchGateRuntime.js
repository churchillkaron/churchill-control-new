...
const LOCAL_REPAIR_PATTERN =
  /\b(bug|broken|error|exception|lint|import|typo|css|spacing|button|mobile|responsive|scroll|layout|dropdown|not working|fails?|failure|repair)\b/i;
...
export function classifyAvantiqoProductResearchDepth({ focus } = {}) {
  ...
  // Product/workflow defects still need current professional and market evidence.
  // Only truly bounded local implementation repairs may skip external research.
...
export async function collectAvantiqoProductDecisionResearch({
  ...
  try {
    const result = depth === DEEP_MECHANISM
      ? await (async () => {
          const { runOperatorMechanismResearch } = await import(
            "@/lib/platform/research/runtime/OperatorMechanismResearchRuntime"
          );
          return runOperatorMechanismResearch({
          context,
          payload: {
            query,
      ...
      : await (async () => {
          const { collectAvantiqoOwnedWebEvidence } = await import(
            "@/lib/intelligence/runtime/AvantiqoOwnedWebEvidenceRuntime"
          );
          return collectAvantiqoOwnedWebEvidence({
            context,
            payload: {
              query,
              objective:
                text(focus, 2000) ||
                "Select the strongest Avantiqo product improvement.",
              minimum_sources: 2,
              max_sources: 4,
            },
          });
