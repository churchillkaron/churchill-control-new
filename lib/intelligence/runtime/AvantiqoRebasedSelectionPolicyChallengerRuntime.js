...
const ACTIVATION_GENERATION_CONTRACT =
...
async function loadCurrentEpoch(
  organizationId,
  policyFingerprint,
  activationGenerationFingerprint,
) {
  const result = await supabaseAdmin
    .from(MEMORY_TABLE)
...
async function loadBaselineEvidence(organizationId, policy) {
  const [applications, monitors, outcomes] = await Promise.all([
    supabaseAdmin
      ...
      .gte("applied_at", policy.activated_at)
    ...
    supabaseAdmin
      ...
      .eq("organization_id", organizationId)
      .eq("policy_id", policy.id)
      .gte("evaluated_at", policy.activated_at)
...
function proposalRow({ organizationId, policy, epoch, observations, completeCycles, nowIso }) {
  ...
  return {
    ...
    content:
      "Distinct post-activation challenger proposal calibrated against the current persistent ordering-policy score using only governed outcomes from completed non-regressive cycles after that exact policy activation interval began. This proposal is research evidence only and cannot change live ordering or authorize promotion, execution, provider use, spend, knowledge release or training.",
    importance: 0.99,
    confidence: 1,
    ...
    metadata: {
      ...
      activation_generation_fingerprint: text(
        epochMetadata.activation_generation_fingerprint,
        128,
      ),
      ...
      prior_activation_interval_evidence_used: false,
  ...
  return {
    ...
    metadata: {
      ...
      activation_generation_fingerprint:
        proposalMetadata.activation_generation_fingerprint,
      current_baseline_policy_fingerprint: policy.policy_fingerprint,
...
export async function reconcileAvantiqoRebasedSelectionPolicyChallenger({
  ...
  const policyMetadata = object(policy.metadata);
  const activationGenerationFingerprint = text(
    policyMetadata.phase43_activation_generation_fingerprint,
    128,
  ).toLowerCase();
  const activationGenerationIndex = Number(
    policyMetadata.phase43_activation_generation_index,
  );
  const activationGenerationStartedAt = text(
    policyMetadata.phase43_activation_started_at,
    120,
  );
  if (
    ...
    text(policyMetadata.phase43_activation_generation_contract, 180) !==
      ACTIVATION_GENERATION_CONTRACT ||
    !Number.isInteger(activationGenerationIndex) ||
    activationGenerationIndex <= 0 ||
    !validFingerprint(activationGenerationFingerprint) ||
    !Number.isFinite(Date.parse(activationGenerationStartedAt)) ||
    activationGenerationStartedAt !== text(policy.activated_at, 120)
  ) {
    return {
      success: false,
      contract: AVANTIQO_REBASED_SELECTION_POLICY_CHALLENGER_CONTRACT,
  ...
  const epoch = await loadCurrentEpoch(
    organizationId,
    policy.policy_fingerprint,
    activationGenerationFingerprint,
  );
  ...
  if (
    ...
    text(epochMetadata.activation_generation_fingerprint, 128) !==
      activationGenerationFingerprint ||
    Number(epochMetadata.activation_generation_index) !==
      activationGenerationIndex ||
    text(epochMetadata.activation_generation_started_at, 120) !==
      activationGenerationStartedAt ||
    epochMetadata.research_epoch_binds_exact_activation_generation !== true ||
    epochMetadata.prior_active_interval_evidence_reusable !== false ||
    epochMetadata.stale_canary_reusable !== false ||
    epochMetadata.stale_approval_reusable !== false ||
    ...
    epochMetadata.future_challenger_must_bind_activation_generation_fingerprint !== true ||
  ...
  if (!proposal) {
    ...
    if (
      ...
      return {
        ...
        proposal_ready: false,
        activation_generation_verified: true,
        activation_generation_fingerprint: activationGenerationFingerprint,
  ...
  if (
    ...
    text(proposalMetadata.current_baseline_policy_fingerprint, 128) !==
      text(policy.policy_fingerprint, 128) ||
    text(proposalMetadata.activation_generation_fingerprint, 128) !==
      activationGenerationFingerprint ||
    text(proposalMetadata.challenger_policy_version, 180) ===
      text(policy.challenger_policy_version, 180) ||
    proposalMetadata.historical_pre_activation_outcomes_used !== false ||
    proposalMetadata.prior_activation_interval_evidence_used !== false ||
  ...
  return {
    ...
    activation_generation_verified: true,
    activation_generation_index: activationGenerationIndex,
    activation_generation_fingerprint: activationGenerationFingerprint,
    activation_generation_started_at: activationGenerationStartedAt,
    prior_activation_interval_evidence_used: false,
