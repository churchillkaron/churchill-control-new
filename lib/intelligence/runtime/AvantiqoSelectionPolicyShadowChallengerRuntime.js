...
async function loadState(organizationId, nowIso = new Date().toISOString()) {
...
function shadowReviewRow({ organizationId, evaluations, nowIso }) {
  ...
  return {
    ...
    memory_type: reviewCandidate ? "lesson" : "fact",
...
async function purgeExpiredShadowEvidence(organizationId, nowIso, limitPerScope = 100) {
  const targets = [
    [SHADOW_SNAPSHOT_SCOPE, "selection_policy_shadow_challenger_snapshot"],
    [SHADOW_EVALUATION_SCOPE, "selection_policy_shadow_challenger_evaluation"],
  ];
  const boundedLimit = Math.max(1, Math.min(100, Number(limitPerScope) || 100));
  let deletedCount = 0;
  for (const [scope, source] of targets) {
...
export async function reconcileAvantiqoSelectionPolicyShadowChallenger({
  ...
  let expiredShadowEvidencePurged = 0;
  if (persist) {
    expiredShadowEvidencePurged = await purgeExpiredShadowEvidence(
      organizationId,
      nowIso,
    ).catch((error) => {
      console.error("AVANTIQO_SELECTION_POLICY_SHADOW_PURGE_FAILED", error);
      return 0;
    });
  }
  const state = await loadState(organizationId, nowIso);
  ...
  return {
    ...
    expired_shadow_evidence_purged: expiredShadowEvidencePurged,
