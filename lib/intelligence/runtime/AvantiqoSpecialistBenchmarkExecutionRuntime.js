...
function governedContext(input = {}) {
  ...
  return {
    ...
    intelligence_safe_lease_endpoint_id: text(
...
function normalizeProviderObservation({
  ...
  const ttftMs = finiteOrNull(
    payload.ttft_ms ?? output.ttft_ms ?? payload.first_token_latency_ms,
  );
  const reportedTotalMs = finiteOrNull(
    payload.total_latency_ms ?? output.total_latency_ms,
  );
  ...
  if (
    ttftMs !== null &&
    effectiveTotalMs !== null &&
    ttftMs > effectiveTotalMs
  ) {
...
export async function executeAvantiqoSpecialistBenchmarkCase(input = {}) {
  const benchmarkCase = getAvantiqoSpecialistBenchmarkCase(
  ...
  const observation = normalizeProviderObservation({
    benchmarkCase,
    result,
    totalLatencyMs,
  });
