...
const COMPILER_MINIMUM_TOKEN_BUDGETS = Object.freeze({
  COMPARE_EXTERNAL_EVIDENCE: 3200,
});
const BOUNDED_FAST_STRUCTURED_OPERATIONS = new Set([
  "COMPARE_EXTERNAL_EVIDENCE",
  "MUSIC_STUDIO_CRITIC_PANEL",
  "MUSIC_STUDIO_PREPRODUCTION_REPAIR",
]);
...
function executionLaneForMode(mode) {
  return normalizeMode(mode) === "deep" ? "deep" : "fast";
}

function tokenBudgetFor(mode, requested) {
  const normalizedMode = normalizeMode(mode);
  const fallback = normalizedMode === "fast"
    ? 900
    : normalizedMode === "balanced"
      ? 1600
      : 4096;
  const minimum = normalizedMode === "deep" ? 4096 : 128;
  return Math.max(
    minimum,
    Math.min(8192, Number(requested) || fallback),
  );
}
...
function compilerTokenBudget(metadata, requested) {
  const minimum = Number(
    COMPILER_MINIMUM_TOKEN_BUDGETS[operationName(metadata)] || 0,
  );
  const maximum = minimum > 0 ? 4096 : 1600;
  return Math.max(
    128,
    Math.min(maximum, Math.max(Number(requested) || 0, minimum)),
  );
}
...
function singlePassBoundedStructuredSystem(system, critiqueInstructions) {
  const critique = text(critiqueInstructions, 12000);
  return [
    text(system, 50000),
    "Analyze this bounded structured decision directly using Avantiqo's owned non-thinking fast lane.",
    "Do not emit private chain-of-thought, hidden reasoning, reasoning_content, or <think> content.",
    "Return exactly one valid JSON object matching the caller's requested contract, with no markdown or explanatory prose.",
    "Preserve source identifiers, uncertainty, conflicts and evidence limits. Never invent facts, approvals, execution results, capabilities or completed work.",
    critique
      ? `Before finalizing the JSON, verify the result against these constraints: ${critique}`
      : "Before finalizing the JSON, verify every claim against the supplied evidence and caller constraints.",
  ].join("\n");
...
async function reasoningPhase({
  ...
  const executionLane = text(execution_lane, 40).toLowerCase() ||
    executionLaneForMode(mode);
  return AvantiqoIntelligenceReasoningRuntime.run({
    ...
    metadata: {
      ...
      structured_supervisor_execution_lane: executionLane,
...
async function singlePassBoundedStructuredPhase({
  organization_id,
  party_id,
  entity_id,
  system,
  ...
  return AvantiqoIntelligenceReasoningRuntime.run({
    ...
    party_id,
    entity_id,
    system: singlePassBoundedStructuredSystem(system, critique_instructions),
    messages: conversation,
    tools: [],
    ...
    metadata: {
      ...object(metadata),
      structured_supervisor_contract: CONTRACT,
      structured_supervisor_phase: "bounded_fast_reason_and_contract",
      structured_supervisor_mode: "bounded_fast_single_pass",
      structured_boundary_compilation: true,
    ...
    temperature: 0.1,
    response_format: { type: "json_object" },
    max_output_tokens: Math.max(128, Math.min(4096, max_output_tokens)),
    max_turns: 1,
    max_tool_calls: 1,
  });
}
...
async function compilerPhase({
  ...
  const compilerExecutionLane =
    object(metadata).owned_shape_repair === true ? "deep" : "fast";
  return AvantiqoIntelligenceReasoningRuntime.run({
    organization_id,
    party_id,
    entity_id,
    ...
    metadata: {
      ...
      structured_supervisor_execution_lane: compilerExecutionLane,
      structured_boundary_compilation: true,
      private_reasoning_transport_expected: false,
      bounded_non_thinking_fast_lane: compilerExecutionLane === "fast",
    },
    execution_lane: compilerExecutionLane,
    temperature: 0.1,
    response_format: { type: "json_object" },
    max_output_tokens: compilerTokenBudget(metadata, max_output_tokens),
    max_turns: 1,
    max_tool_calls: 1,
  });
}
...
export async function runStructuredIntelligenceSupervisor({
  ...
  const boundedFastStructured = BOUNDED_FAST_STRUCTURED_OPERATIONS.has(
    operationName(metadata),
  );
  const tokenBudget = tokenBudgetFor(
    boundedFastStructured ? "fast" : normalizedMode,
    max_output_tokens,
  );
  ...
  if (boundedFastStructured) {
    const singlePass = await singlePassBoundedStructuredPhase({
      organization_id,
      party_id,
      entity_id,
      system,
  ...
  if (normalizedMode === "deep") {
    ...
    try {
      ...
      critiqueRepair = {
        success: false,
        optional: true,
        preserved_initial_brief: true,
        error: text(critiqueError?.message || critiqueError, 1000) || "CRITIQUE_REPAIR_FAILED",
      };
    }
  }
  ...
  let compiled = null;
  let parsed = null;
  let compilerFallbackToDecisionBrief = false;
  try {
    compiled = await compilerPhase({
      organization_id,
      party_id,
      entity_id,
      system,
    ...
    parsed = parseJson(compiled.text);
  } catch (compilerError) {
    const directParsed = parseJson(decisionBrief);
    if (!directParsed) throw compilerError;
    parsed = directParsed;
    compilerFallbackToDecisionBrief = true;
    compiled = {
      success: false,
      optional: true,
      preserved_valid_decision_brief: true,
      error: text(compilerError?.message || compilerError, 1000) || "CONTRACT_COMPILE_FAILED",
    };
  }
  if (!parsed) {
    throw new Error("AVANTIQO_STRUCTURED_SUPERVISOR_INVALID_COMPILED_JSON");
  ...
  return {
    ...
    repaired: Boolean(critiqueRepair && critiqueRepair.success !== false),
    direct_machine_boundary: false,
    deterministic_machine_boundary: false,
    single_pass_structured_reasoning: false,
    compiler_fallback_to_valid_decision_brief: compilerFallbackToDecisionBrief,
    phases: {
