...
function singlePassProductTokenBudget(metadata, requested) {
  ...
  const minimum = operation === "PRODUCT_REPOSITORY_ASSESSMENT"
    ...
    : operation === "PRODUCT_PERSISTENCE_DECISION"
      ? 8192
      : 0;
  return Math.max(128, Math.min(16384, Math.max(Number(requested) || 0, minimum)));
}
...
function singlePassProductSystem(system, critiqueInstructions) {
  ...
  return [
    text(system, 50000),
    "Reason through the Product decision naturally in the model's private reasoning channel.",
    "Do not place private chain-of-thought, hidden reasoning, or <think> content in the final answer.",
    ...
    critique
      ? `Before finalizing the JSON, internally verify the result against these constraints: ${critique}`
      : "Before finalizing the JSON, internally verify every claim against the supplied evidence and caller constraints.",
  ].join("\n");
}
...
async function singlePassProductPhase({
  ...
  return AvantiqoIntelligenceReasoningRuntime.run({
    ...
    metadata: {
      ...
      private_reasoning_transport_expected: true,
