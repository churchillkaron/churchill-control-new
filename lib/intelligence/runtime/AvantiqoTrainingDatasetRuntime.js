...
const FOUNDATION_MODEL = "Qwen/Qwen3-4B";
const TRAINING_METHOD = "LORA_PEFT_QWEN3_4B_LOCAL";
const TRAINING_BACKEND = "AVANTIQO_LOCAL_NODE_TRAINER_V1";
...
function behaviorInstruction(candidate = {}) {
  if (candidate.candidate_kind === "GENERAL_INTELLIGENCE_TRANSFER_DISCIPLINE") {
    return (
      "Apply cross-domain knowledge only as a testable hypothesis: require evidence from both source and target contexts, identify invariants, boundary conditions and falsifiers, and abstain when support is insufficient. Train the reasoning pattern, not mutable world facts."
    );
  }
  if (candidate.candidate_kind === "CREATIVE_PREFERENCE_PAIR") {
    return (
      "Prefer creative decisions that satisfy the immutable world-class quality floor and improve the weakest verified craft dimensions over alternatives that fail or score lower. " +
      "Use only de-identified structural evidence such as failure families, repair success and score deltas. Never reproduce prior customer content, imagery, wording, brand assets or style; infer the transferable craft principle and apply fresh judgment to the current evidence."
    );
  }
...
function trainingRecipe({ trainCount, holdoutCount }) {
  return {
    ...
    preferred_method: TRAINING_METHOD,
    base_precision: "FP32_CPU_OR_LOCAL_GPU_MIXED",
    base_quantized: false,
    execution_backend: TRAINING_BACKEND,
    dedicated_trainer_required: true,
    trainer_disabled_by_default: true,
    explicit_training_execution_required: true,
