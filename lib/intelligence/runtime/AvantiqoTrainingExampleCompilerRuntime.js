...
const SUPPORTED_CANDIDATE_KINDS = new Set([
  ...
  "CREATIVE_PREFERENCE_PAIR",
  "GENERAL_INTELLIGENCE_TRANSFER_DISCIPLINE",
]);
...
function candidateCurriculumFingerprint(candidate = {}) {
  const metadata = object(candidate.metadata);
  return stableHash([
...
function curriculumInstruction(candidateKind) {
  if (candidateKind === "GENERAL_INTELLIGENCE_TRANSFER_DISCIPLINE") {
    return (
      "When applying a mechanism or principle across domains, treat analogy as a hypothesis rather than evidence. Require evidence from both source and target contexts, identify the invariant that may transfer, state boundary conditions and falsifiers, and choose UNSUPPORTED or INSUFFICIENT when the evidence does not justify transfer. Train this reasoning discipline only; do not memorize mutable world facts from prior learning episodes."
    );
  }
  if (candidateKind === "CREATIVE_PREFERENCE_PAIR") {
    return (
      "When comparative Creative evidence shows one candidate passed the immutable world-class gates and was selected over a reviewed alternative, learn the transferable structural reason for that preference. Favor stronger purpose, physical truth, continuity, performance, camera plausibility, pacing and low synthetic-artifact risk when supported by evidence. Never copy the prior creative, brand, imagery, wording or style; produce a fresh solution for the current mission and keep all quality and governance gates intact."
    );
  }
...
function curriculum(candidate = {}, split) {
  ...
  return {
    ...
    source_candidate_fingerprint: text(metadata.source_fingerprint, 128) || null,
    source_benchmark_id: text(metadata?.benchmark?.benchmark_id, 240) || null,
    source_benchmark_suite: text(metadata?.benchmark?.benchmark_suite, 240) || null,
    split,
    instruction,
  };
}
...
function compilerSystem() {
  return [
    ...
    "For CREATIVE_PREFERENCE_PAIR examples, train transferable craft judgment from de-identified winner-vs-alternative evidence. Never reconstruct or imitate prior customer creative, brands, assets, prompts, stories or styles.",
    "For GENERAL_INTELLIGENCE_TRANSFER_DISCIPLINE examples, train evidence-bound transfer reasoning only: evidence on both sides, invariants, boundaries, falsifiers, and abstention when unsupported. Never encode mutable learned world facts as the target behavior.",
...
function normalizeExample(value = {}, curriculumByCapability = new Map()) {
  ...
  const curriculumItem = curriculumByCapability.get(capability);
  ...
  return {
    ...
    source_candidate_id: text(curriculumItem.source_candidate_id, 160),
    source_candidate_fingerprint: text(curriculumItem.source_candidate_fingerprint, 128),
    source_benchmark_id: text(curriculumItem.source_benchmark_id, 240),
    source_benchmark_suite: text(curriculumItem.source_benchmark_suite, 240),
  };
}
  ...
  const rows = examples.map((example) => {
    ...
    return {
      ...
      metadata: {
        ...
        dataset_fingerprint: text(datasetMetadata.dataset_fingerprint, 128) || null,
        ...
        source_candidate_fingerprint: example.source_candidate_fingerprint,
        source_benchmark_id: example.source_benchmark_id,
        source_benchmark_suite: example.source_benchmark_suite,
...
export async function compileAvantiqoTrainingExamples({
  ...
  if (
    ...
    throw new Error("AVANTIQO_TRAINING_COMPILER_DATASET_VERSION_BINDING_REQUIRED");
  }
  ...
  if (bindings.size < candidateIds.length) {
    throw new Error("AVANTIQO_TRAINING_COMPILER_DATASET_CANDIDATE_BINDINGS_REQUIRED");
  }
  ...
  const curriculumItems = candidates.map((candidate) => {
    const metadata = object(candidate.metadata);
    const fingerprint = candidateCurriculumFingerprint(candidate);
  ...
  if (curriculumItems.length !== candidates.length) {
    throw new Error(
      `AVANTIQO_TRAINING_COMPILER_DATASET_BINDING_MISMATCH:${curriculumItems.length}:${candidates.length}`,
    );
  }
  ...
  const curriculumByCapability = new Map(curriculumItems.map((item) => [item.capability_key, item]));
  const examples = list(result?.parsed?.examples)
    .map((item) => normalizeExample(item, curriculumByCapability))
    .filter(Boolean)
  ...
  return {
    ...
    governance: {
      ...
      dataset_source_version_binding_required: true,
      dataset_benchmark_version_binding_required: true,
      customer_private_content_available: false,
