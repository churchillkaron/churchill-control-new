...
function rollingOutcomeBucket({ observation, now }) {
  ...
  const signature = [
    text(observation?.capability_key, 300).toLowerCase(),
    text(observation?.outcome, 80),
    text(observation?.verification_mode, 120),
    text(observation?.failure_fingerprint, 120),
  ].join("|");
  ...
  return {
    slot,
    fingerprint,
    memory_key: `verified-outcome:${fingerprint}:slot-${slot}`,
  };
}
...
function outcomeObservation(execution = {}) {
  ...
  if (failure?.affects_capability_reliability === true) {
    return {
      capability_key: failure.capability_key,
      outcome: "VERIFIED_FAILURE",
      verification_mode: failure.post_action_verification_failed ? "post_action_verification" : "verified_prerequisite_failure",
...
export async function recordAvantiqoVerifiedExecutionOutcome({ execution = {}, sourceTurnId = null } = {}) {
  const learningScopeId = await resolveLearningOrganizationId();
  if (!learningScopeId) {
    return {
  ...
  const bucket = rollingOutcomeBucket({ observation, now });
  const row = {
    ...
    memory_key: text(sourceTurnId, 160)
      ? `verified-outcome-turn:${createHash("sha256").update(text(sourceTurnId, 160)).digest("hex").slice(0, 40)}`
      : `verified-outcome:${randomUUID()}`,
    ...
    metadata: {
      ...
      observed_at: nowIso,
      observation_day: nowIso.slice(0, 10),
      rolling_bucket: true,
      rolling_bucket_slot: bucket.slot,
      rolling_bucket_fingerprint: bucket.fingerprint,
...
export async function summarizeAvantiqoVerifiedExecutionOutcomes({
  ...
  const scopedOrganizationId = text(organizationId, 160);
