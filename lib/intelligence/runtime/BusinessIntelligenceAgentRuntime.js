...
export async function runBusinessIntelligenceAgent({
  ...
  const businessDiagnosisReceipt = buildBusinessDiagnosisReceipt({
    organization_id: organizationId, entity_id, metric, diagnosis_class: context?.business_diagnosis_class || null, business_timezone: context?.business_timezone || null, answer_content: boundedAnswer?.result?.response || "", baseline_period_id: baselinePeriodId, baseline_period_start_date: context?.baseline_period_start_date || null, baseline_period_end_date: context?.baseline_period_end_date || null, current_period_id: currentPeriodId, current_period_start_date: context?.current_period_start_date || null, current_period_end_date: context?.current_period_end_date || null,
