...
const DEFAULT_MAX_DESCRIPTOR_TOOLS = 24;
const MAX_DESCRIPTOR_TOOLS = 64;
const DEFAULT_MAX_DESCRIPTOR_CHARS = 24000;
const MAX_DESCRIPTOR_CHARS = 100000;
...
function descriptorPolicy(options = {}) {
  const source = object(options);
  return {
    max_tools: Math.min(
      MAX_DESCRIPTOR_TOOLS,
      positiveInteger(source.max_tools || source.maxTools, DEFAULT_MAX_DESCRIPTOR_TOOLS),
    ),
    max_chars: Math.min(
      MAX_DESCRIPTOR_CHARS,
      positiveInteger(source.max_chars || source.maxChars, DEFAULT_MAX_DESCRIPTOR_CHARS),
    ),
  };
}

function boundedPublicToolDescriptors(tools = [], options = {}) {
  const policy = descriptorPolicy(options);
  const descriptors = [];
  let descriptorChars = 0;

  for (const tool of tools) {
    if (descriptors.length >= policy.max_tools) break;
    const descriptor = publicToolDescriptor(tool);
    const chars = JSON.stringify(descriptor).length;
    if (descriptorChars + chars > policy.max_chars) break;
    descriptors.push(descriptor);
    descriptorChars += chars;
  }
  ...
  return {
    ...
    telemetry: Object.freeze({
      ...
      exposed_tools: descriptors.length,
      descriptor_chars: descriptorChars,
      max_tools: policy.max_tools,
      max_chars: policy.max_chars,
      bounded: descriptors.length < tools.length,
    }),
  };
}
...
function governedPlanBinding(args = {}) {
  const source = object(args);
  return {
    plan_id: text(source.plan_id || source.planId).slice(0, 160) || null,
    plan_step_id: text(source.plan_step_id || source.planStepId).slice(0, 120) || null,
  };
}
...
function governedOutcomeReceipt({
  ...
  const planBinding = governedPlanBinding(args);
  return Object.freeze({
    ...
    plan_id: planBinding.plan_id,
    plan_step_id: planBinding.plan_step_id,
...
export function createIntelligenceToolRegistry(definitions = []) {
  ...
  function descriptorSnapshot(options = {}) {
    return boundedPublicToolDescriptors([...byName.values()], options);
  }
  ...
  return Object.freeze({
    ...
    size: byName.size,
    names: () => [...byName.keys()],
    descriptors: (options = {}) => descriptorSnapshot(options).descriptors,
    descriptorBudget: (options = {}) => descriptorSnapshot(options).telemetry,
