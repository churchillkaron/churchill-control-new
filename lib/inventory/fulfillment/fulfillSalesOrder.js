...
export async function fulfillSalesOrder({
  ...
  if (
    !resolvedActorId ||
    !UUID_PATTERN.test(String(resolvedActorId))
  ) {
    const error = new Error(
  ...
  if (result.error) {
    const unavailable =
      ...
      /inventory_fulfill_sales_order_atomic/i.test(
        result.error.message || ""
      );

    if (unavailable) {
      const error = new Error(
        "Retail inventory fulfillment migration is not deployed"
      );
      error.status = 503;
      throw error;
    }
