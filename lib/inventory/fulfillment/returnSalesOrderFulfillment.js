...
async function loadOriginalSaleMovements({ organizationId, entityId, salesOrderId }) {
  const { data, error } = await supabaseAdmin
    ...
    .select(
      "id, item_id, warehouse_id, location_id, quantity, unit_cost, total_cost, movement_date"
    )
...
async function findFulfillmentJournal({ organizationId, entityId, salesOrderId }) {
  const { data, error } = await supabaseAdmin
    ...
    .select(
      "id, reversed, reversal_journal_id, reversed_journal_entry_id, posting_date, currency_code"
    )
    .eq("organization_id", organizationId)
    .eq("entity_id", entityId)
    .eq("source_module", "inventory")
...
export async function returnSalesOrderFulfillment({
  access,
  body = {},
  organizationId,
}) {
  ...
  if (!resolvedActorId || !UUID_PATTERN.test(String(resolvedActorId))) {
    const error = new Error(
      "Authenticated staff identity is required to return a sales-order fulfillment"
    );
    error.status = 403;
    throw error;
  }
  ...
  const originalMovements = await loadOriginalSaleMovements({
    organizationId,
    entityId,
    salesOrderId,
  });
  ...
  for (const original of originalMovements) {
    ...
    if (quantity <= 0) {
      throw new Error(`Original fulfillment movement ${original.id} has invalid quantity`);
    }
    if (unitCost < 0) {
      throw new Error(`Original fulfillment movement ${original.id} has invalid unit cost`);
    }
    ...
    const result = await createInventoryMovement({
      ...
      notes:
        reason ||
        `Full return of sales order ${order.order_number || salesOrderId}`,
  ...
  const fulfillmentJournal = await findFulfillmentJournal({
    organizationId,
    entityId,
    salesOrderId,
  });
  ...
  let finance = {
    reversed: false,
    skipped: true,
    reason: "FULFILLMENT_FINANCE_JOURNAL_NOT_FOUND",
  };

  if (restoredCost > 0) {
    if (!fulfillmentJournal) {
      const error = new Error(
        "Fulfillment inventory was restored but the canonical COGS journal was not found for reversal"
      );
      error.status = 409;
      throw error;
    }
    ...
    finance = alreadyReversed
      ...
      : {
          ...
          ...(await createJournalReversal({
            ...
            reversalReason:
              reason ||
              `Sales order fulfillment return ${order.order_number || salesOrderId}`,
