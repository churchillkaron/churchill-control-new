...
function signedQuantity(type, quantity) {
  ...
  if (INBOUND_TYPES.has(normalizedType)) return qty;
  if (OUTBOUND_TYPES.has(normalizedType)) return -qty;
  return qty;
}
...
function movementBalance(movements = []) {
  return movements.reduce(
    (balance, movement) => {
      const signedQty = signedQuantity(movement.type, movement.quantity);
      const totalCost = Number(movement.total_cost || 0);

      balance.quantity += signedQty;
      balance.value += signedQty >= 0 ? totalCost : -Math.abs(totalCost);
      return balance;
    },
    { quantity: 0, value: 0 }
  );
...
export async function updateStockLedger({
  ...
  const itemUpdate = await supabaseAdmin
    .from("inventory_items")
    .update({ quantity: globalBalance.quantity })
    .eq("organization_id", resolvedOrganizationId)
