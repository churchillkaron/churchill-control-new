...
export function createPurchaseOrderCapability() {
  const manifest = defineCapability({
    ...
    operatorVerification: { capability_key: "supply_chain.purchase_orders.read", payload_from_result: { id: ["purchase_order.id", "id"] }, recovery_payload_from_evidence: { idempotency_key: "idempotency_key" }, derivation: "declared_result_bound_record_verifier_with_idempotency_recovery" },
  ...
  async function execute({ context, payload = {} }) {
    ...
    if (result.error) throw attachActionIdentityEvidence(result.error, [`idempotency_key:${idempotencyKey}`]);
