...
export default async function receivePurchaseOrder({
  ...
  try {
    ...
    const goodsReceiptId = data.goods_receipt?.id || data.goods_receipt_id || null;
    await emitBusinessPartnerBusinessEvent({
      organization_id,
      event_source: "supply-chain",
      event_type: "GOODS_RECEIPT_RECEIVED",
      correlations: [
        `purchase_order:${purchase_order_id}`,
        goodsReceiptId ? `goods_receipt:${goodsReceiptId}` : null,
      ],
      identity: `${purchase_order_id}:${goodsReceiptId || "received"}`,
      evidence: {
        purchase_order_id,
        goods_receipt_id: goodsReceiptId,
        entity_id: resolvedEntityId,
        inventory_movement_count: inventoryMovements.length,
      },
    }).catch((error) => console.error("BUSINESS_PARTNER_GOODS_RECEIPT_EVENT_ADAPTER_FAILED", error));
    return result;
  } catch (error) {
    return {
      success: false,
