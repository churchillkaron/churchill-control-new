...
export function createSupplierCapability() {
  const manifest = defineCapability({
    ...
    operatorVerification: { capability_key: "supply_chain.suppliers.read", payload_from_result: { party_id: ["party_id", "id"] }, recovery_payload_from_evidence: { idempotency_key: "idempotency_key" }, derivation: "declared_result_bound_record_verifier_with_idempotency_recovery" },
  ...
  async function execute({ context, payload = {} }) {
    ...
    try {
      ...
      throw attachActionIdentityEvidence(error, [`idempotency_key:${idempotencyKey}`]);
    }
  }
