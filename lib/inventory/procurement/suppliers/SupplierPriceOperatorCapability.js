...
export function createSupplierPriceImportCapability() {
  const manifest = defineCapability({
    ...
    operatorVerification:{ capability_key:"supply-chain.supplier_prices.verifyImport", payload_from_input:{ supplier_party_id:"supplier_party_id", rows:"rows" }, derivation:"declared_input_bound_value_set_verifier" },
  ...
  async function execute({ context, payload = {} }) {
    ...
    if (new Set(rows.map((row)=>row.item_id)).size !== rows.length) throw new Error("duplicate supplier price item_id in import");
