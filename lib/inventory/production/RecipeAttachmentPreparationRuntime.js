...
export async function prepareRecipeAttachment({file={},organizationId,entityId}={}){
  ...
  for(let i=0;i<rows.length;i++){
    const r=object(rows[i]); const code=text(r.item_code||r.sku||r.ingredient_code||r.code,160); const componentCode=text(r.component_dish_code||r.component_recipe_code||r.sub_recipe_code||r.preparation_code,160); const quantity=Number(r.quantity??r.qty??r.amount); const unit=text(r.uom||r.unit||r.unit_of_measure,80); const yieldRaw=r.yield_percent??r.yield??r.usable_yield_percent; const yieldEvidenced=yieldRaw!==undefined&&yieldRaw!==null&&yieldRaw!==""; const yieldPercent=Number(yieldEvidenced?yieldRaw:100);
    ...
    if(code){
      ...
      prepared.push({item_id:item.id,component_dish_id:null,quantity,uom_id:uom?.id||item.uom_id||null,item_code:code,item_name:item.name||null,yield_percent:yieldPercent,yield_evidenced:yieldEvidenced});
      continue;
    }
    ...
    prepared.push({item_id:null,component_dish_id:component.id,component_dish_code:componentCode,component_name:component.name||null,quantity,uom_id:sourceUom.id,yield_percent:yieldPercent,yield_evidenced:yieldEvidenced});
  }
