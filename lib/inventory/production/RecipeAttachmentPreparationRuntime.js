...
export async function prepareRecipeAttachment({file={},organizationId,entityId}={}){
  ...
  for(let i=0;i<rows.length;i++){
    const r=object(rows[i]); const code=text(r.item_code||r.sku||r.ingredient_code||r.code,160); const quantity=Number(r.quantity??r.qty??r.amount); const unit=text(r.uom||r.unit||r.unit_of_measure,80);
    ...
    prepared.push({item_id:item.id,quantity,uom_id:uom?.id||item.uom_id||null,item_code:code});
  }
  const dish=dishResult.data[0]; return {contract:CONTRACT,recognized:true,status:"READY_FOR_REVIEW",clarification_required:false,dish,recipe_items:prepared,import_payload:{dish_id:dish.id,items:prepared.map(({item_id,quantity,uom_id})=>({item_id,quantity,uom_id}))},authorization_effect:"NONE"};
}
