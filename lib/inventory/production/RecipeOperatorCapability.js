...
export function createRecipeUpsertCapability(){
  ...
  async function execute({context,payload={}}){
    ...
    const rows=payload.items.map(row=>({item_id:text(row?.item_id),quantity:Number(row?.quantity),uom_id:text(row?.uom_id)||null,yield_percent:row?.yield_percent==null?100:Number(row.yield_percent)}));
    if(rows.some(row=>!row.item_id||!Number.isFinite(row.quantity)||row.quantity<=0||!Number.isFinite(row.yield_percent)||row.yield_percent<=0||row.yield_percent>100)) throw new Error("invalid recipe items");
