...
export async function createRecipe({dish_id,items,organization_id,organizationId,entity_id,entityId,actor_id,actorId}){
  ...
  const rows=items.map((item)=>({item_id:text(item?.item_id),quantity:Number(item?.quantity),uom_id:text(item?.uom_id)||null,yield_percent:item?.yield_percent==null?100:Number(item.yield_percent)}));
  if(rows.some((row)=>!row.item_id||!Number.isFinite(row.quantity)||row.quantity<=0||!Number.isFinite(row.yield_percent)||row.yield_percent<=0||row.yield_percent>100)) throw new Error("Invalid recipe item");
