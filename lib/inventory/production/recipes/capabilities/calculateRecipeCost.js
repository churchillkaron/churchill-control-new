...
export async function calculateRecipeCost({organizationId,organization_id,entityId,entity_id,dishId,dish_id,recipeId,laborCost=0,overheadCost=0,sellingPrice=null}={}){
  ...
  const recipeResult=await supabaseAdmin.from("recipe_items").select("id,item_id,quantity,uom_id,unit,yield_percent").eq("organization_id",org).eq("entity_id",entity).eq("dish_id",dishKey);
  ...
  for(const row of rows){const item=items.get(text(row.item_id)); if(!item||item.is_active===false) throw new Error("recipe inventory item unavailable"); let factor=1;
    ...
    const quantity=num(row.quantity),yieldPercent=num(row.yield_percent)||100,unitCost=num(item.cost),purchaseQuantity=(quantity*factor)/(yieldPercent/100),componentCost=purchaseQuantity*unitCost; ingredientCost+=componentCost;
    breakdown.push({item_id:item.id,item_code:item.code||null,item_name:item.name,quantity,uom_id:row.uom_id||item.uom_id||null,factor_to_item_base:factor,yield_percent:yieldPercent,purchase_quantity_in_item_base:Number(purchaseQuantity.toFixed(4)),base_unit_cost:unitCost,component_cost:Number(componentCost.toFixed(4))});}
