...
async function renderGlassScene(ffmpeg, sourceUrl, panelRaw, scene, output) {
  const start = 0.62;
  const end = 1.72;
  const fadeOut = 4.42;
  ...
  await run(ffmpeg, [
    ...
    "-f", "rawvideo", "-pixel_format", "rgba", "-video_size", "960x540", "-framerate", "24", "-i", panelRaw,
    "-filter_complex", filter,
    "-map", "[v]", "-an",
    "-c:v", "libx264", "-preset", "ultrafast", "-crf", "19", "-r", "24", "-t", String(SCENE_DURATION), output,
  ]);
}
  ...
  async render() {
    ...
    try {
      ...
      for (let index = 0; index < SCENES.length; index += 1) {
        ...
        await renderGlassScene(ffmpeg, sourceUrls[scene.source], panelRaw, scene, output);
