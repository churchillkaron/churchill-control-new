...
const OUTPUT_PATH = `${OUTPUT_DIR}/avantiqo-business-loop-vfx-v2.mp4`;
...
const SCENES = Object.freeze([
  ...
  { id: "customer", source: "communications", eyebrow: "CUSTOMER CONTEXT", title: "A response becomes a business event", subtitle: "Campaign source, customer context and next action stay connected.", rows: [["Campaign source", "LINKED"], ["Customer context", "IDENTIFIED"], ["Intent", "CAPTURED"], ["History", "AVAILABLE"]], action: "Continue into the operating flow", current: 2, origin: [646, 468], target: [420, 96] },
  ...
  { id: "finance", source: "finance", eyebrow: "FINANCE", title: "The books retain the operational story", subtitle: "Finance sees what happened and the business event that caused it.", rows: [["Revenue", "LINKED"], ["Tax", "RECORDED"], ["Cost", "CONNECTED"], ["Margin", "UPDATED"]], action: "Send the complete event to Intelligence", current: 7, origin: [870, 338], target: [420, 94] },
...
function run(command, args, timeoutMs = 290000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error("AVANTIQO_BUSINESS_LOOP_V2_RENDER_TIMEOUT"));
    }, timeoutMs);
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        reject(new Error(log.slice(-14000) || `FFMPEG_EXIT_${code}`));
        return;
      }
      resolve(log);
    });
...
function panelSvg(scene) {
  const rowY = [238, 294, 350, 406];
  const rows = scene.rows.map(([label, value], index) => `
    <rect x="48" y="${rowY[index] - 34}" width="864" height="46" rx="13" fill="#ffffff" fill-opacity="0.038" stroke="#ffffff" stroke-opacity="0.09"/>
    <text x="70" y="${rowY[index] - 5}" fill="#d7d9de" font-family="Arial, Helvetica, sans-serif" font-size="18">${esc(label)}</text>
    <text x="888" y="${rowY[index] - 5}" fill="#d9bd7d" font-family="Arial, Helvetica, sans-serif" font-size="16" font-weight="700" text-anchor="end">${esc(value)}</text>
  `).join("");
  ...
  const flow = FLOW.map((label, index) => {
    ...
    return `
      <rect x="${x}" y="490" width="${stepWidth}" height="34" rx="10" fill="${active ? "#d8bf83" : "#ffffff"}" fill-opacity="${active ? "0.94" : "0.035"}" stroke="${active ? "#f0dda6" : "#ffffff"}" stroke-opacity="${active ? "0.9" : "0.07"}"/>
      <text x="${x + stepWidth / 2}" y="512" fill="${active ? "#08090b" : "#8d919a"}" font-family="Arial, Helvetica, sans-serif" font-size="9.6" font-weight="${active ? "700" : "500"}" text-anchor="middle">${esc(label)}</text>
    `;
  }).join("");
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="960" height="540" viewBox="0 0 960 540">
    <defs>
      <linearGradient id="glass" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stop-color="#0a0d12" stop-opacity="0.80"/>
        <stop offset="0.58" stop-color="#05070b" stop-opacity="0.70"/>
        <stop offset="1" stop-color="#11151d" stop-opacity="0.61"/>
      </linearGradient>
      <linearGradient id="gold" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0" stop-color="#6f5c35" stop-opacity="0.18"/>
        <stop offset="0.5" stop-color="#e0c98f" stop-opacity="0.96"/>
        <stop offset="1" stop-color="#6f5c35" stop-opacity="0.12"/>
      </linearGradient>
      <radialGradient id="light" cx="0.16" cy="0.02" r="0.76">
        <stop offset="0" stop-color="#ffffff" stop-opacity="0.13"/>
        <stop offset="1" stop-color="#ffffff" stop-opacity="0"/>
      </radialGradient>
      <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="13"/></filter>
    </defs>
    <rect x="17" y="23" width="926" height="500" rx="32" fill="#000000" fill-opacity="0.44" filter="url(#shadow)"/>
    <rect x="20" y="20" width="920" height="500" rx="32" fill="url(#glass)" stroke="#d2ba7c" stroke-opacity="0.58" stroke-width="1.4"/>
    <rect x="22" y="22" width="916" height="496" rx="30" fill="url(#light)"/>
    <rect x="44" y="46" width="872" height="1.5" rx="1" fill="url(#gold)"/>
    <circle cx="62" cy="82" r="6" fill="#d8bf83"/>
    <text x="82" y="88" fill="#d8bf83" font-family="Arial, Helvetica, sans-serif" font-size="14" font-weight="700" letter-spacing="1.8">AVANTIQO · ${esc(scene.eyebrow)}</text>
    <text x="48" y="145" fill="#f7f7f4" font-family="Arial, Helvetica, sans-serif" font-size="36" font-weight="700">${esc(scene.title)}</text>
    <text x="48" y="178" fill="#acb0b9" font-family="Arial, Helvetica, sans-serif" font-size="17">${esc(scene.subtitle)}</text>
    ${rows}
    <rect x="48" y="436" width="864" height="38" rx="12" fill="#d8bf83" fill-opacity="0.10" stroke="#d8bf83" stroke-opacity="0.30"/>
    <text x="70" y="461" fill="#dac28a" font-family="Arial, Helvetica, sans-serif" font-size="15" font-weight="700">NEXT · ${esc(scene.action)}</text>
    ${flow}
  </svg>`);
}
...
async function renderScene(ffmpeg, sourceUrl, panelRaw, scene, output) {
  const riseStart = 0.72;
  const riseEnd = 1.92;
  const fadeOut = 4.44;
  const originWidth = 170;
  const originHeight = 96;
  const targetWidth = 790;
  const targetHeight = 444;
  const [originCenterX, originCenterY] = scene.origin;
  ...
  const x = easeExpression(riseStart, riseEnd, originCenterX - originWidth / 2, targetX);
  const y = easeExpression(riseStart, riseEnd, originCenterY - originHeight / 2, targetY);

  const filter = [
    `[0:v]scale=1280:720:force_original_aspect_ratio=increase,crop=1280:720,fps=${FPS},setsar=1,setpts=PTS-STARTPTS,eq=contrast=1.035:saturation=.93:brightness=-.012[base]`,
    `[1:v]setpts=PTS-STARTPTS,format=rgba,fade=t=in:st=${riseStart}:d=0.18:alpha=1,fade=t=out:st=${fadeOut}:d=0.30:alpha=1,scale=w='${width}':h='${height}':eval=frame[glass]`,
    `[base][glass]overlay=x='${x}':y='${y}':eval=frame:repeatlast=1:shortest=0[stage]`,
    `[stage]drawbox=x=${originCenterX - 5}:y=${originCenterY - 5}:w=10:h=10:color=c8a96a@0.92:t=fill:enable='between(t,${(riseStart - 0.10).toFixed(2)},${(riseStart + 0.52).toFixed(2)})',format=yuv420p[v]`,
  ].join(";");
...
async function inspectDuration(ffmpeg, file) {
  ...
  if (!match) throw new Error("BUSINESS_LOOP_V2_DURATION_UNREADABLE");
  const duration = Number(match[1]) * 3600 + Number(match[2]) * 60 + Number(match[3]);
  if (duration < TARGET_DURATION - 0.5 || duration > TARGET_DURATION + 0.8) {
    throw new Error(`BUSINESS_LOOP_V2_DURATION_INVALID:${duration.toFixed(3)}`);
  }
  return duration;
}

export const AvantiqoInvestorFilmBusinessLoopRuntimeV2 = {
  BUCKET,
  ORGANIZATION_ID,
  ENTITY_ID,
  OUTPUT_PATH,
  ...
  async status() {
    ...
    return {
      ...
      vfx_language: "DEVICE_ORIGIN_TRANSPARENT_GLASS_RISE_V2",
      timing_contract: "10 scenes × 4.8 seconds; render rejected unless final media duration is ~48 seconds",
  ...
  async render() {
    ...
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-business-loop-v2-"));
    try {
      const sourceUrls = {};
      for (const [key, storagePath] of Object.entries(SOURCES)) {
      ...
      for (let index = 0; index < SCENES.length; index += 1) {
        ...
        await renderScene(ffmpeg, sourceUrls[scene.source], panelRaw, scene, segment);
        segments.push(segment);
      }

      const final = path.join(directory, "avantiqo-business-loop-vfx-v2.mp4");
      await concatenate(ffmpeg, segments, final);
      const duration = await inspectDuration(ffmpeg, final);
