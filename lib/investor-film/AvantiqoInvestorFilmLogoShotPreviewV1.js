...
async function upload(localPath) {
  ...
  const { error } = await supabaseAdmin.storage.from(BUCKET).upload(OUTPUT, bytes, {
    ...
    upsert: true,
    cacheControl: "3600",
    metadata: { contract: CONTRACT, exact_frames: FRAMES, fps: FPS, preview: true, floating_channel_marks: true, badge_boxes: false, screenshot_panels: false, image_generation_used: false },
  });
  if (error) throw error;
...
async function rawAsset(directory, name, svg, width, height) {
  const target = path.join(directory, `${name}.raw`);
  const pixels = await sharp(svg).resize(width, height, { fit: "fill" }).ensureAlpha().raw().toBuffer();
  await fs.writeFile(target, pixels);
