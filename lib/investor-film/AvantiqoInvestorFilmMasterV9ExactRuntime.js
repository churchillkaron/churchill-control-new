...
async function renderMaster() {
  ...
  try {
    ...
    await run(ffmpeg, [
      "-y", "-threads", "1", "-protocol_whitelist", "file,http,https,tcp,tls,crypto", "-f", "concat", "-safe", "0", "-i", concatList,
    ...
    await run(ffmpeg, [
      ...
      "-map", "[v]", "-map", "[aout]",
      "-c:v", "libx264", "-threads", "1", "-x264-params", "threads=1:lookahead_threads=1", "-preset", "veryfast", "-crf", "18", "-pix_fmt", "yuv420p", "-r", String(FPS),
    ...
    const masterState = {
      ...
      checksum: stored.checksum, bytes: stored.bytes, technical_qc, low_memory_render: true, updated_at: new Date().toISOString(),
    };
