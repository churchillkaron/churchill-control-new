...
async function probe(ffprobe, input) {
  const raw = await run(ffprobe, ["-v", "error", "-count_frames", "-show_entries", "format=duration:stream=codec_type,codec_name,width,height,r_frame_rate,avg_frame_rate,nb_read_frames,sample_rate,channels", "-of", "json", input], 120000);
...
async function uploadVideo(buffer) {
  ...
  const { error } = await supabaseAdmin.storage.from(BUCKET).upload(OUTPUT_PATH, buffer, {
    ...
    upsert: true,
    cacheControl: "3600",
    metadata: { contract: CONTRACT, organization_id: ORG, creative_project_id: PROJECT, exact_frames: TARGET_FRAMES, fps: FPS, duration_seconds: TARGET_DURATION, exact_frame_normalized: true, checksum },
  });
  if (error) throw error;
...
async function renderMaster() {
  ...
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "avantiqo-v9-exact-"));
  ...
  try {
    await fs.writeFile(concatList, segmentUrls.map((url) => `file '${String(url).replace(/'/g, "'\\''")}'`).join("\n"), "utf8");

    await run(ffmpeg, [
      "-y", "-protocol_whitelist", "file,http,https,tcp,tls,crypto", "-f", "concat", "-safe", "0", "-i", concatList,
      "-map", "0:v:0", "-an", "-c:v", "copy", "-fflags", "+genpts", joined,
    ], 300000);

    await run(ffmpeg, [
      "-y", "-i", joined, "-i", narrationUrl, "-stream_loop", "-1", "-i", scoreUrl,
      "-filter_complex",
      `[0:v]fps=${FPS},tpad=stop_mode=clone:stop_duration=1,trim=end_frame=${TARGET_FRAMES},setpts=N/(${FPS}*TB)[v];[1:a]atrim=0:229.5,asetpts=PTS-STARTPTS,adelay=8000:all=1,aresample=48000,volume=1[voice];[2:a]atrim=0:237.5,asetpts=PTS-STARTPTS,aresample=48000,volume=.22,afade=t=in:st=0:d=2.5,afade=t=out:st=233.5:d=4[score];[voice][score]amix=inputs=2:duration=longest:dropout_transition=0,alimiter=limit=.95[aout]`,
      "-map", "[v]", "-map", "[aout]",
    ...
    const technical_qc = {
      ...
      av_streams_present: true, exact_frame_normalized: true,
    };
    const masterState = {
      ...
      score_locked: true, exact_frame_normalized: true, source_concat_frames_observed: 5694,
      checksum: stored.checksum, bytes: stored.bytes, technical_qc, updated_at: new Date().toISOString(),
    };
    ...
    return { success: true, rendered: true, status: masterState.status, output_path: OUTPUT_PATH, signed_url: await signed(OUTPUT_PATH, 86400), checksum: stored.checksum, bytes: stored.bytes, technical_qc, timeline: SEGMENTS.map(({ id, role, frames }) => ({ id, role, frames })) };
  } finally {
    await fs.rm(directory, { recursive: true, force: true }).catch(() => {});
  }
}

export const AvantiqoInvestorFilmMasterV9ExactRuntime = Object.freeze({
  ...
  async status() {
    ...
    return { contract: CONTRACT, output_path: OUTPUT_PATH, final_ready: finalReady, exact_frames: TARGET_FRAMES, duration_seconds: TARGET_DURATION, timeline_valid: SEGMENT_FRAMES === TARGET_FRAMES, readiness: ready, state: p.metadata?.spatial_investor_master_v9 || null, signed_url: finalReady ? await signed(OUTPUT_PATH, 86400) : null };
  },
