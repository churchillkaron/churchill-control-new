...
function run(command, args, timeoutMs = 600000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error("V9_SPATIAL_INTELLIGENCE_MEDIA_TIMEOUT"));
      }
    }, timeoutMs);
...
async function correctCommunication(ffmpeg, directory) {
  ...
  await run(ffmpeg, ["-y","-i",sourceUrl,"-loop","1","-framerate",String(FPS),"-i",nodes,"-filter_complex",`[1:v]format=rgba,scale=1500:-1,fade=t=in:st=4.8:d=.7:alpha=1,fade=t=out:st=29.7:d=.8:alpha=1[n];[0:v][n]overlay=x='210+8*sin(t*.45)':y='118+4*sin(t*.31)':enable='between(t,4.6,30.5)':shortest=0,format=yuv420p[v]`,`-map`,`[v]`,`-map`,`0:a?`,`-c:v`,`libx264`,`-preset`,`veryfast`,`-crf`,`17`,`-r`,String(FPS),`-c:a`,`copy`,`-frames:v`,`911`,output]);
...
async function correctStudio(ffmpeg, directory) {
  ...
  await run(ffmpeg,["-y","-i",sourceUrl,"-loop","1","-framerate",String(FPS),"-i",posterUrl,"-stream_loop","-1","-i",videoUrl,"-loop","1","-framerate",String(FPS),"-i",posterGlass,"-loop","1","-framerate",String(FPS),"-i",videoGlass,"-loop","1","-framerate",String(FPS),"-i",channels,"-filter_complex",`[1:v]scale=382:510:force_original_aspect_ratio=decrease,pad=382:510:(ow-iw)/2:(oh-ih)/2:color=black@0,format=rgba,fade=t=in:st=7.5:d=.7:alpha=1,fade=t=out:st=20.8:d=.7:alpha=1[p];[2:v]scale=650:366:force_original_aspect_ratio=increase,crop=650:366,format=rgba,fade=t=in:st=10.2:d=.8:alpha=1,fade=t=out:st=27.2:d=.8:alpha=1[m];[3:v]scale=430:578,format=rgba,fade=t=in:st=7.2:d=.6:alpha=1,fade=t=out:st=21:d=.7:alpha=1[pg];[4:v]scale=720:457,format=rgba,fade=t=in:st=9.9:d=.7:alpha=1,fade=t=out:st=27.5:d=.8:alpha=1[vg];[5:v]scale=1500:266,format=rgba,fade=t=in:st=21.2:d=.8:alpha=1,fade=t=out:st=34.5:d=.8:alpha=1[ch];[0:v][pg]overlay=x='120-14*sin(t*.34)':y='80+6*sin(t*.28)':enable='between(t,7,21.8)'[a];[a][p]overlay=x='144-14*sin(t*.34)':y='138+6*sin(t*.28)':enable='between(t,7.3,21.5)'[b];[b][vg]overlay=x='1030+18*sin(t*.29)':y='145+7*sin(t*.23)':enable='between(t,9.7,28.3)'[c];[c][m]overlay=x='1065+18*sin(t*.29)':y='220+7*sin(t*.23)':enable='between(t,10,28)'[d];[d][ch]overlay=x=210:y='780+4*sin(t*.3)':enable='between(t,21,35.3)':shortest=0,format=yuv420p[v]`,`-map`,`[v]`,`-map`,`0:a?`,`-c:v`,`libx264`,`-preset`,`veryfast`,`-crf`,`17`,`-r`,String(FPS),`-c:a`,`copy`,`-frames:v`,`881`,output],600000);
...
async function probe(ffprobe, url) {
  const raw = await run(ffprobe,["-v","error","-count_frames","-show_entries","stream=codec_type,width,height,r_frame_rate,nb_read_frames","-of","json",url],120000);
