...
async function renderLocal() {
  ...
  try {
    ...
    const filter = [
      ...
      `[2:v]format=rgba,colorchannelmixer=aa=.78[atm]`,
      `[base][atm]overlay=0:0:format=auto[b1]`,
      `[1:v]format=rgba,fade=t=in:st=2.05:d=.72:alpha=1,fade=t=out:st=6.55:d=.60:alpha=1[title]`,
      `[b1][title]overlay=x='2*sin(t*.12)':y='1.4*sin(t*.10)':enable='between(t,1.85,7.25)',fade=t=in:st=0:d=.32,fade=t=out:st=7.66:d=.28,format=yuv420p[v]`,
      `[3:a]volume=.075,afade=t=in:st=0:d=1.1,afade=t=out:st=6.85:d=.75[a]`,
    ].join(";");
    await run(ffmpeg, [
