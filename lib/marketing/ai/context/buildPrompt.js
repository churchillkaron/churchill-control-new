import { getTopPerformingCampaigns } from "@/lib/marketing/ai/utils/getTopPerformingCampaigns";

export async function buildPrompt(state) {
  let referenceMemory = "";

  try {
    const topCampaigns = await getTopPerformingCampaigns({
      organizationId: state.organizationId,
      limit: 3,
    });

    if (topCampaigns?.length) {
      referenceMemory = topCampaigns
        .map((campaign) => `
...
`)
    ...
    console.error("PROMPT_MEMORY_ERROR", {
      message: error?.message || "Unable to load campaign memory",
    });
  }
