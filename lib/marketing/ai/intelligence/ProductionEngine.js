...
function capabilityFromDecision(decision) {
  if (decision.production_method === "existing_asset") {
    return "ai.video.edit";
  }

  if (decision.production_method === "motion_graphic") {
    return "ai.video.edit";
  }

  if (decision.production_method === "generated_scene") {
...
export function buildProductionGraph({
  ...
  const scenes = decisions.map(
    (decision, index) => ({
      ...
      creative_decision: decision.description,
      production_method: decision.production_method,
      required_assets: decision.required_assets || [],
      capability_needed: capabilityFromDecision(decision),
  ...
  return {
    production_version: "creative-search-runtime-v3-capability-native",
    status: verification.passed ? "verified_plan" : "needs_revision",
    objective: knowledgeGraph.objective,
    platform: knowledgeGraph.platform,
    duration_seconds: knowledgeGraph.duration_seconds,
    budget_mode: knowledgeGraph.budget_mode,
    business: knowledgeGraph.business,
