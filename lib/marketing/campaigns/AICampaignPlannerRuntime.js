...
export const AICampaignPlannerRuntime = {
  async plan({
    ...
    if (String(wallet.billing_policy || "PREPAID").toUpperCase() !== "PREPAID") {
      throw stagedError({
        stage: "WALLET_READINESS",
        code: "PREPAID_WALLET_REQUIRED",
        message: "A prepaid organization wallet is required",
        correction:
          "Use the Avantiqo prepaid wallet before autonomous campaign planning.",
      });
    }
    ...
    try {
      result = await ServiceExecutionRuntime.execute({
        ...
        provider_id: selectedProvider.provider,
        capability: "ai.reasoning.execute",
        country: organizationCountry(organization),
        currency: readiness?.wallet?.currency || null,
        input: {
        ...
        metadata: {
          module: "MARKETING",
          operation: "AI_CAMPAIGN_PLANNING",
          contract: "UNIVERSAL_CAMPAIGN_PLAN_V1",
          prepaid_provider_execution: true,
          provider_billed_to: "AVANTIQO",
          customer_payment_source: "AVANTIQO_PREPAID_WALLET",
        },
        category: "MARKETING_AI_CAMPAIGN_PLANNING",
      });
    } catch (error) {
      throw stagedError({
        stage: "AI_PLANNING",
        ...
        correction:
          "Verify the prepaid wallet, selected managed reasoning credential and provider availability, then retry.",
        cause: error,
      });
    }
