...
export const MarketingCampaignExecutionRuntime = {
  async executeApprovedPlan({
    ...
    if (!validation.ready) {
      throw stageError({
        stage: "PLAN_APPROVAL",
        code: "CAMPAIGN_PLAN_NOT_EXECUTABLE",
        message: "Campaign plan is not ready for execution",
        correction: "Resolve every plan blocker and record owner approval before wallet reservation.",
    ...
    for (const channelPlan of normalizedPlan.channels) {
      ...
      if (!channelReadiness?.available) {
        throw stageError({
          ...
          correction:
            ...
            "Install and configure the required organization service and provider connection.",
      ...
      if (!adapter || adapter.status !== "ACTIVE") {
        throw stageError({
          ...
          correction: "Implement and register the provider adapter before presenting this channel as executable.",
        });
      }
      ...
      try {
        ...
        throw stageError({
          ...
          correction: "Correct the provider-specific error and retry. No channel should be activated automatically.",
        });
      }
    }
