...
function normalizeLocation(location = {}) {
  ...
  const result = {
    ...
    country_code:
      text(location.country_code || location.country).toUpperCase() || null,
  ...
  if (type === "country" && !result.country_code) {
...
function normalizeAudience(audience = {}, { requireIncludedLocations = true } = {}) {
  ...
  if (requireIncludedLocations && !included.length) {
  ...
  return {
    ...
    location_presence: text(
      audience.location_presence ||
        audience.locationPresence ||
        "living_or_recent",
    ).toLowerCase(),
    age_min: ageMin,
    age_max: ageMax,
...
function normalizeChannelPlan(channelPlan = {}) {
  const channelId = text(
    channelPlan.channel_id || channelPlan.channelId,
  ).toLowerCase();
  const channel = getMarketingChannel(channelId);

  if (!channel) {
    throw new Error(`Unknown campaign channel: ${channelId}`);
  }
  ...
  return {
    ...
    billing_event:
      text(channelPlan.billing_event || channelPlan.billingEvent).toUpperCase() ||
      null,
    placements: list(channelPlan.placements),
    conversion_event:
      text(channelPlan.conversion_event || channelPlan.conversionEvent) || null,
    tracking: channelPlan.tracking || {},
    provider_settings:
      channelPlan.provider_settings || channelPlan.providerSettings || {},
  };
}
...
export function normalizeUniversalCampaignPlan(plan = {}) {
  ...
  if (!channels.length) {
    throw new Error("Campaign plan requires at least one channel");
  }

  const channelIds = channels.map((channel) => channel.channel_id);
  if (new Set(channelIds).size !== channelIds.length) {
    throw new Error("Campaign plan cannot contain duplicate channel entries");
  }
  const hasPaidMedia = channels.some((channel) => getMarketingChannel(channel.channel_id)?.kind === "paid_media");

  const budgetAmount = number(
    plan.budget?.amount ??
      plan.budget?.total_budget ??
      plan.budget?.totalBudget,
  );
  if (budgetAmount === null || budgetAmount < 0) {
    throw new Error("Campaign budget cannot be negative");
  }
  if (hasPaidMedia && budgetAmount <= 0) {
    throw new Error("Paid-media campaign budget must be positive");
  }

  const currency = text(plan.budget?.currency).toUpperCase();
  if (!currency) {
    throw new Error("Campaign budget currency is required");
  }

  const name = text(plan.name);
  if (!name) throw new Error("Campaign name is required");

  const goal = text(plan.goal).toUpperCase();
  if (!goal) throw new Error("Campaign goal is required");
  ...
  return {
    ...
    audience: normalizeAudience(plan.audience || {}, { requireIncludedLocations: hasPaidMedia }),
    budget: {
      amount: budgetAmount,
      currency,
      ...
      daily_amount: number(
        plan.budget?.daily_amount ??
          plan.budget?.dailyAmount ??
          plan.budget?.daily_budget ??
          plan.budget?.dailyBudget,
      ),
      bid_strategy: text(
        plan.budget?.bid_strategy ||
          plan.budget?.bidStrategy ||
          "lowest_cost",
      ),
    ...
    schedule: {
      start_time:
        plan.schedule?.start_time || plan.schedule?.startTime || null,
    ...
    creative: {
      asset_ids: list(
        plan.creative?.asset_ids || plan.creative?.assetIds,
      ),
      exact_asset_required: plan.creative?.exact_asset_required !== false,
      primary_text: text(
        plan.creative?.primary_text || plan.creative?.primaryText,
      ),
      headline: text(plan.creative?.headline),
      headlines: list(plan.creative?.headlines).map(text).filter(Boolean),
      description: text(plan.creative?.description),
      descriptions: list(plan.creative?.descriptions).map(text).filter(Boolean),
      destination_url:
        text(
          plan.creative?.destination_url ||
            plan.creative?.destinationUrl,
        ) || null,
      call_to_action: text(
        plan.creative?.call_to_action ||
          plan.creative?.callToAction ||
          "LEARN_MORE",
      ).toUpperCase(),
      utm_parameters:
        plan.creative?.utm_parameters ||
        plan.creative?.utmParameters ||
        {},
      language_variants: list(
        plan.creative?.language_variants ||
          plan.creative?.languageVariants,
      ),
      placement_variants: list(
        plan.creative?.placement_variants ||
          plan.creative?.placementVariants,
      ),
    },
    channels,
    ai: {
    ...
    approval: {
      ...
      source: text(plan.approval?.source) || null,
    },
  };
}

export function validateCampaignPlanReadiness(
  plan = {},
  ...
  for (const channel of normalized.channels) {
    if (
      !["ACTIVE", "ACTIVE_IF_CONFIGURED"].includes(
      ...
      blockers.push({
        ...
        message: `${
          getMarketingChannel(channel.channel_id)?.name || channel.channel_id
  ...
  if (
    ...
    !normalized.approval.approved
  ) {
    blockers.push({
      code: "OWNER_APPROVAL_REQUIRED",
      message:
        "Campaign plan requires owner approval before funds can be reserved",
    });
  }
  ...
  if (
    requireApproval &&
    normalized.approval.approved &&
    (!normalized.approval.approved_by || !normalized.approval.approved_at)
  ) {
    blockers.push({
      code: "OWNER_APPROVAL_AUDIT_REQUIRED",
      message:
        "Approved campaign plan requires server-recorded approver and approval time",
    });
  }
