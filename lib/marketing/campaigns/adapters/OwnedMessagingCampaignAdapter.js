...
async function senderAsset({ organizationId, translated }) {
  ...
  const [{ data, error }, connections] = await Promise.all([
    supabaseAdmin
  ...
  const activeConnection = (connections || []).find((connection) =>
    ...
    text(connection.id) === text(data.connection_id),
  ) || null;
  if (!data.connection_id || !activeConnection) {
    throw executionError({
      stage: "CHANNEL_ASSET_PREFLIGHT",
      code: "OWNED_MESSAGING_SENDER_CONNECTION_INACTIVE",
      provider: translated.channel_id,
      message: `Selected ${translated.channel_id} sender is not attached to an active organization connection`,
      correction: "Reconnect or reselect the organization sender before approval.",
      details: { sender_asset_id: data.id, connection_id: data.connection_id || null },
    });
  }
  return {
    ...data,
    connection_credentials_reference: text(activeConnection.credentials_reference) || null,
  };
}
...
export const OwnedMessagingCampaignAdapter = {
  ...
  async preflight({ organizationId, plan, channel }) {
    ...
    return {
      ...
      sender_credential_binding: prepared.asset.connection_credentials_reference ? "CONNECTION_SPECIFIC" : "ORGANIZATION_DEFAULT",
    ...
    for (const recipient of prepared.recipients) {
      ...
      try {
        const result = await executeService({
          ...
          credential_id: prepared.asset.connection_credentials_reference || undefined,
