...
function rankScaleCandidates(campaigns) {
  return [...campaigns]
    ...
    .sort((a, b) => {
      ...
      if (b.metrics.profit_after_media !== a.metrics.profit_after_media) {
...
function buildCapitalAllocationProposal({ scaleCandidates, repairCandidates, blindCampaigns }) {
  ...
  return {
    ...
    allocation_principle:
      "The next advertising baht should be considered first for the strongest verified profit candidate, but only through a controlled increment with explicit authorization and post-change measurement.",
    ...
    marginal_model: {
      available: false,
      reason:
        "No validated campaign spend-response curve or controlled increment history is available. Average historical return must not be presented as the marginal return of the next baht.",
      required_evidence: [
...
export function buildAdsPortfolioIntelligence(group = {}) {
  ...
  const portfolioProfitAfterMedia = totalGrossProfit - totalSpend;
  const portfolioProfitOnAdSpend = totalSpend > 0 ? totalGrossProfit / totalSpend : null;
  ...
  return {
    version: "ads-intelligence-v3-next-baht",
    ...
    portfolio: {
      ...
      media_break_even_covered:
        totalSpend > 0 ? totalGrossProfit >= totalSpend : null,
    ...
    world_class_principles: [
      ...
      "Rank where the next advertising baht should be tested, but never invent a marginal-return curve or budget amount from average historical performance.",
