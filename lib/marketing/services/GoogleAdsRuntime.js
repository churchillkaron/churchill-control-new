...
function keywordOperations({ adGroupResource, keywords = [], negative = false }) {
...
function campaignCriterionOperations({ campaignResource, includedLocations = [], excludedLocations = [], languageTargets = [] }) {
  const operations = [];
  for (const item of includedLocations || []) {
    const resourceName = text(item?.resource_name || item?.resourceName || item);
    if (!resourceName) continue;
    operations.push({ create: { campaign: campaignResource, location: { geoTargetConstant: resourceName } } });
  }
  for (const item of excludedLocations || []) {
    const resourceName = text(item?.resource_name || item?.resourceName || item);
    if (!resourceName) continue;
    operations.push({ create: { campaign: campaignResource, negative: true, location: { geoTargetConstant: resourceName } } });
  }
  for (const item of languageTargets || []) {
    const resourceName = text(item?.resource_name || item?.resourceName || item);
    if (!resourceName) continue;
    operations.push({ create: { campaign: campaignResource, language: { languageConstant: resourceName } } });
  }
  return operations;
}
...
export const GoogleAdsRuntime = {
  ...
  async createSearchCampaign({
    ...
    const providerStartDate = text(startDate);
    const providerEndDate = text(endDate);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(providerStartDate) || !/^\d{4}-\d{2}-\d{2}$/.test(providerEndDate)) {
      throw new Error("Google Ads organization-local start and end dates are required");
    }
    ...
    const durationDays = Math.max(1, Math.ceil((endTime.getTime() - startTime.getTime()) / 86_400_000));
    if (perDay * durationDays > totalBudget + 0.000001) {
      throw new Error("Google Ads daily budget across the campaign period exceeds the authorized wallet budget");
    }
    ...
    const reservation = await ManagedMediaSpendRuntime.reserveBudget({
      ...
      targeting: { keywords, negative_keywords: negativeKeywords, included_locations: includedLocations, excluded_locations: excludedLocations, languages: languageTargets },
      schedule: {
      ...
      metadata: {
        ...
        daily_budget: perDay,
        search_partners: searchPartners === true,
    ...
    try {
        ...
        operations: [
          {
            create: {
              ...
              startDate: providerStartDate.replace(/-/g, ""),
              endDate: providerEndDate.replace(/-/g, ""),
              networkSettings: {
                targetGoogleSearch: true,
                targetSearchNetwork: true,
                targetContentNetwork: false,
                targetPartnerSearchNetwork: searchPartners === true,
              },
            },
          },
        ],
      ...
      const campaignCriteria = campaignCriterionOperations({
        campaignResource: providerResources.campaign,
        includedLocations,
        excludedLocations,
        languageTargets,
      });
      if (campaignCriteria.length) {
        await mutate({
          organizationId,
          entityId: account.entity_id,
          currency: campaignCurrency,
          ...
          operation: "CREATE_CAMPAIGN_TARGETING",
          operations: campaignCriteria,
        });
      }
      ...
      const negativeCriteria = keywordOperations({
        adGroupResource: providerResources.adGroup,
        keywords: negativeKeywords,
        negative: true,
      });
      if (negativeCriteria.length) {
        await mutate({
          organizationId,
          entityId: account.entity_id,
          currency: campaignCurrency,
