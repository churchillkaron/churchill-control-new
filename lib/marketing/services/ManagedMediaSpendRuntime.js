...
async function reserveBudget({
  ...
  try {
    ...
    if (
      error?.code === "23505" &&
      (
        error?.constraint === "managed_media_campaigns_marketing_campaign_provider_active_uidx" ||
        String(error?.message || "").includes("managed_media_campaigns_marketing_campaign_provider_active_uidx") ||
        String(error?.details || "").includes("managed_media_campaigns_marketing_campaign_provider_active_uidx")
      )
    ) {
      const duplicate = new Error("Managed-media execution already exists for this Marketing Campaign and provider");
      duplicate.name = "CampaignExecutionError";
      duplicate.stage = "EXECUTION_IDEMPOTENCY";
      duplicate.code = "MANAGED_MEDIA_EXECUTION_ALREADY_EXISTS";
      duplicate.correction = "Use or reconcile the existing managed-media campaign instead of creating another provider campaign.";
