...
async function resolveChannelIdentityAsset({ organizationId, assetId, assetType, provider = "meta" }) {
  required(assetId, `${assetType} asset id`);
  const { data, error } = await supabaseAdmin
    .from("organization_channel_assets")
    .select("id,organization_id,channel_provider,asset_type,external_id,name,metadata")
  ...
  if (!data?.external_id) throw new Error(`Selected ${assetType} identity was not found in this organization`);
  return data;
}

async function listIdentityAssets({ organizationId, provider, assetType }) {
  const { data, error } = await supabaseAdmin
    .from("organization_channel_assets")
    .select("id,organization_id,channel_provider,asset_type,external_id,name,metadata")
    .eq("organization_id", organizationId)
  ...
  if (error) throw error;
  return (data || []).filter((row) => row.external_id);
}
...
export const MetaAdsRuntime = {
  async readiness({ organizationId }) {
    ...
    const [connection, assets, service, wallet, campaigns, credential, pageIdentities, instagramIdentities, whatsappIdentities] =
      await Promise.all([
        ...
        listIdentityAssets({ organizationId, provider: "meta", assetType: "facebook_page" }).catch(() => []),
        listIdentityAssets({ organizationId, provider: "meta", assetType: "instagram_business" }).catch(() => []),
        listIdentityAssets({ organizationId, provider: "whatsapp", assetType: "whatsapp_phone_number" }).catch(() => []),
      ]);

    const metadata = channelMetadata(connection);
    const configuration = serviceConfiguration(service);
    const pageId = pageIdentities[0]?.external_id || metadata.page_id || metadata.facebook_page_id || null;
    const instagramBusinessId = instagramIdentities[0]?.external_id || metadata.instagram_business_id || metadata.instagram_actor_id || null;
    const whatsappDestination = whatsappIdentities[0]?.external_id || null;
    ...
    return {
      ...
      identity_assets: {
        facebook_pages: pageIdentities.map((row) => ({ id: row.id, name: row.name, external_id: row.external_id })),
        instagram_accounts: instagramIdentities.map((row) => ({ id: row.id, name: row.name, external_id: row.external_id, facebook_page_id: row.metadata?.facebook_page_id || null })),
        whatsapp_numbers: whatsappIdentities.map((row) => ({ id: row.id, name: row.name, external_id: row.external_id })),
      },
      default_country:
  ...
  async createCampaign({
    ...
    const metadata = channelMetadata(connection);
    const configuration = serviceConfiguration(service);
    const [pageIdentity, instagramIdentity, whatsappIdentity] = await Promise.all([
      resolveChannelIdentityAsset({ organizationId, assetId: pageAssetId, assetType: "facebook_page" }),
      instagramAssetId ? resolveChannelIdentityAsset({ organizationId, assetId: instagramAssetId, assetType: "instagram_business" }) : Promise.resolve(null),
      whatsappAssetId ? resolveChannelIdentityAsset({ organizationId, assetId: whatsappAssetId, assetType: "whatsapp_phone_number", provider: "whatsapp" }) : Promise.resolve(null),
    ]);
    const pageId = pageIdentity.external_id;
    const instagramBusinessId = instagramIdentity?.external_id || null;
    const whatsappDestination = whatsappIdentity?.external_id || null;
    if (instagramIdentity && text(instagramIdentity.metadata?.facebook_page_id) !== text(pageId)) {
      throw new Error("Selected Instagram identity is not linked to the selected Facebook Page");
    }
    ...
    if (destination === "WHATSAPP") {
      required(whatsappAssetId, "Exact organization WhatsApp phone-number asset");
      const whatsappAdsReady = Boolean(
        whatsappDestination &&
        configuration.whatsapp_ads_enabled === true &&
        credential?.whatsapp_ads_enabled === true
      ...
      if (!whatsappAdsReady) {
        throw new Error("WhatsApp Ads is not configured for this managed provider account and selected organization phone number");
      }
    }
