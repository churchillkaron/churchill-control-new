...
export function calculateAutonomousPaperOrder({
  ...
  const volatilityScale = candidateVolatility !== null && candidateVolatility > 0
    ...
    : 1;
  const regimeScale = clamp(number(marketRegimeSizingScale, 1), 0.25, 1);
  const scaledTargetPct = confidenceScaledTargetPct * volatilityScale * regimeScale;
  ...
  if (action === "BUY") {
    ...
    return {
      ...
      market_regime_sizing_scale: regimeScale,
      confidence,
