...
async function rolling24hPaperFills({
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select("id,notional,fee_amount,filled_at,metadata")
    .eq("organization_id", organizationId)
    .eq("portfolio_id", portfolioId)
...
export async function runAutonomousPaperCycle({
  ...
  try {
    ...
    const breakerMarked = markPortfolio({
      account: state.account,
      positions: state.positions,
      snapshots: state.latestBySymbol,
    });
    let breaker = evaluatePortfolioCircuitBreaker({
    ...
    if (breaker.breached) {
      ...
      state = await loadAutomationState({ organizationId, portfolioId });
      const postBreakerMarked = markPortfolio({
        account: state.account,
        positions: state.positions,
        snapshots: state.latestBySymbol,
      });
    ...
    const finalMarked = markPortfolio({
      account: state.account,
      positions: state.positions,
      snapshots: state.latestBySymbol,
    });
