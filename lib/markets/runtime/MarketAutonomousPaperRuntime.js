...
async function updateWatchlistMonitoringMetadata({
  organizationId,
  portfolioId,
  item,
  patch,
...
async function latchPortfolioCircuitBreaker({
  ...
  const { data: authorityCleanup, error: cancelError } = await supabaseAdmin.rpc(
    "market_cancel_open_paper_buy_authority_on_breaker",
    {
  ...
  return {
    ...
    latched: true,
    reason,
    buy_authority_cleanup: authorityCleanup || null,
  };
}
  ...
  try {
    ...
    if (state.riskPolicy?.protective_exits_enabled !== false) {
      for (const position of state.positions) {
        ...
        if (
          ...
          const { data: highWaterPromotion, error: highWaterError } = await supabaseAdmin.rpc(
            "market_raise_paper_position_high_water",
            {
              p_organization_id: organizationId,
              p_portfolio_id: portfolioId,
              p_position_id: position.id,
              p_high_water_price: executableHighWater,
            },
          );
          if (highWaterError) throw highWaterError;
          position.high_water_price = number(
            highWaterPromotion?.high_water_price,
            executableHighWater,
          );
          if (highWaterPromotion?.promoted === true) {
            state.account.execution_revision = number(
              highWaterPromotion.execution_revision,
              state.account.execution_revision,
            );
          }
        }
    ...
    for (const item of state.watchlist) {
      ...
      try {
        ...
        const newsDue = researchRefreshRequired({
          lastResearchAt: item?.metadata?.news_refreshed_at || null,
          maxAgeMinutes: 5,
        });
        ...
        if (researchDue) {
          try {
            ...
            await updateWatchlistMonitoringMetadata({
              organizationId,
              portfolioId,
              item,
              patch: {
          ...
          try {
            const refreshedNews = await MarketIntelligenceIngestionRuntime.refreshNews({
              organizationId,
              portfolioId,
              symbol,
              exchange: item.exchange || null,
            ...
            await updateWatchlistMonitoringMetadata({
              ...
              item,
              patch: {
                news_refreshed_at: refreshedNews.refreshed_at || new Date().toISOString(),
              },
            });
          } catch (newsRefreshError) {
            errors.push({
              symbol,
              stage: "NEWS_REFRESH",
              error: clean(newsRefreshError?.message || newsRefreshError || "MARKETS_NEWS_REFRESH_FAILED").slice(0, 500),
            });
          }
        }

        if (ordersCreated >= maxTrades) {
          skipped.push({ symbol, reason: "MAX_TRADES_PER_CYCLE_REACHED_MONITORING_ONLY" });
          continue;
        }
