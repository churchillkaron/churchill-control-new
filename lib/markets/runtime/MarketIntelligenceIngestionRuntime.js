...
async function persistSnapshot({ organizationId, portfolioId, instrument, snapshot }) {
  const { data, error } = await supabaseAdmin.from("market_snapshots").insert({
    ...
    latest_trade_at: snapshot.latest_trade_at,
    latest_trade_id: snapshot.latest_trade_id,
    latest_quote_at: snapshot.latest_quote_at,
    latest_quote_fingerprint: snapshot.latest_quote_fingerprint,
...
function dateOffsetDay(days, now = new Date()) {
...
async function persistCorporateActions({
  ...
  const rows = (Array.isArray(actions) ? actions : [])
    .filter((action) => action?.provider_action_id && action?.symbol)
    .map((action) => ({
      organization_id: organizationId,
      portfolio_id: portfolioId,
      symbol: upper(action.symbol),
      provider: "alpaca",
      provider_action_id: text(action.provider_action_id),
      action_type: text(action.action_type).toLowerCase(),
      ...
      data_quality: action.data_quality === "all" ? "all" : "complete",
  ...
  const { data, error } = await supabaseAdmin
    ...
    .upsert(rows, {
      onConflict: "organization_id,portfolio_id,provider,provider_action_id",
    })
    .select("*");
  if (error) throw error;
  return data || [];
...
export async function refreshMarketNews({
  ...
  const news = await AlpacaMarketDataProvider.news({
    organizationId,
...
export async function refreshMarketSymbol({
  ...
  const [snapshot, bars, news, corporateActionResult] = await Promise.all([
    ...
    AlpacaMarketDataProvider.corporateActions({
      organizationId,
      symbols: [instrument.symbol],
      start: dateOffsetDay(-90),
      end: dateOffsetDay(30),
      ...
      .catch((error) => ({
        rows: [],
        error: text(error?.message || error || "CORPORATE_ACTIONS_UNAVAILABLE").slice(0, 700),
      })),
  ]);
  ...
  const corporateActionRows = await persistCorporateActions({
    organizationId,
    portfolioId,
    actions: corporateActionResult.rows,
  });
  ...
  return {
    ...
    news_analysis: newsAnalysis,
    corporate_actions: corporateActionRows,
    corporate_action_status: corporateActionResult.error
      ? {
          status: "UNAVAILABLE",
          error: corporateActionResult.error,
...
export async function refreshMarketBenchmarkBars({
  organizationId,
  portfolioId,
  symbol,
  start = null,
  ...
  const bars = await AlpacaMarketDataProvider.bars({
    organizationId,
    symbol: instrument.symbol,
    timeframe: "1Day",
    start,
  ...
  const persisted = await persistBars({
    organizationId,
    portfolioId,
    instrument,
    bars,
...
export const MarketIntelligenceIngestionRuntime = {
  ...
  refreshNews: refreshMarketNews,
  refreshBenchmarkBars: refreshMarketBenchmarkBars,
};
