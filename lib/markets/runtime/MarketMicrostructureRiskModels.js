...
export function evaluateMarketMicrostructureRisk({
  ...
  const capturedMs = new Date(snapshot?.captured_at || 0).getTime();
  const nowMs = new Date(now).getTime();
  const maxAgeSeconds = number(policy.max_market_data_age_seconds, 120);
  const ageSeconds = Number.isFinite(capturedMs) && Number.isFinite(nowMs)
    ? Math.max(0, (nowMs - capturedMs) / 1000)
