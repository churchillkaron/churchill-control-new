...
export function simulatePaperFillPrice({
  ...
  const adversePrice = normalizedSide === "SELL"
    ? price * (1 - (bps / 10000))
    : price * (1 + (bps / 10000));

  if (String(orderType || "").trim().toUpperCase() !== "LIMIT") {
    return adversePrice;
  }

  const limit = number(limitPrice, null);
  if (!(limit > 0)) throw new Error("valid limit price required for LIMIT order");

  return normalizedSide === "BUY"
    ? Math.min(adversePrice, limit)
    : Math.max(adversePrice, limit);
}
