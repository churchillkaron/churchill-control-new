...
async function loadExecutionRiskState({ organizationId, portfolioId }) {
  const [portfolioResult, policyResult, automationResult, accountResult, positionsResult] = await Promise.all([
    supabaseAdmin
      .from("market_portfolios")
      .select("id,status,execution_mode,updated_at")
      .eq("organization_id", organizationId)
      .eq("id", portfolioId)
      .maybeSingle(),
    supabaseAdmin
  ...
  for (const result of [portfolioResult, policyResult, automationResult, accountResult, positionsResult]) {
...
export async function processPaperOrder({
  ...
  if (
    !executionState.portfolio ||
    executionState.portfolio.status !== "ACTIVE" ||
    executionState.portfolio.execution_mode !== "PAPER"
  ) {
    const cancellationReason = !executionState.portfolio
      ? "PAPER_PORTFOLIO_REQUIRED"
      : executionState.portfolio.status !== "ACTIVE"
        ? "PAPER_PORTFOLIO_NOT_ACTIVE"
        : "PAPER_PORTFOLIO_EXECUTION_MODE_INVALID";
    const cancelledAt = new Date().toISOString();
    ...
    return {
      ...
      portfolio: executionState.portfolio,
    };
  }
  ...
  if (executionState.account.status !== "ACTIVE") {
    const cancellationReason = "PAPER_ACCOUNT_NOT_ACTIVE";
    const cancelledAt = new Date().toISOString();
    ...
    return {
      ...
      reason: cancellationReason,
      order: cancelledOrder || order,
      account_status: executionState.account.status,
    };
  }
  ...
  const automationGeneratedOrder = order?.risk_snapshot?.automation === true;
  const ordinaryAutomationOrder = (
    automationGeneratedOrder &&
    !protectiveDecision &&
  ...
  if (
    automationGeneratedOrder &&
    executionState.automation?.auto_paper_enabled !== true &&
    !(circuitBreakerDecision && executionState.automation?.circuit_breaker_latched === true)
  ) {
    const cancellationReason = "AUTO_PAPER_DISABLED";
    const cancelledAt = new Date().toISOString();
  ...
  if (
    ordinaryAutomationOrder &&
    side === "BUY" &&
    executionState.automation?.allow_buys === false
  ) {
    const cancellationReason = "AUTOMATED_BUYS_DISABLED";
    const cancelledAt = new Date().toISOString();
    const { data: cancelledOrder, error: cancelOrderError } = await supabaseAdmin
      .from("market_paper_orders")
      .update({
  ...
  if (
    ordinaryAutomationOrder &&
    side === "SELL" &&
    executionState.automation?.allow_sells === false
  ) {
    const cancellationReason = "AUTOMATED_SELLS_DISABLED";
    const cancelledAt = new Date().toISOString();
    const { data: cancelledOrder, error: cancelOrderError } = await supabaseAdmin
      .from("market_paper_orders")
      .update({
  ...
  if (executionState.automation?.kill_switch === true) {
    const cancellationReason = "PAPER_AUTOMATION_KILL_SWITCH_ACTIVE";
    const cancelledAt = new Date().toISOString();
  ...
  let automationSizingRevalidation = {
    approved: true,
    status: ordinaryAutomationOrder ? "CURRENT_POLICY_ALLOWED" : "NOT_APPLICABLE",
    reason: null,
    current_sizing: null,
  };

  if (ordinaryAutomationOrder) {
    const currentAutomationSizing = calculateAutonomousPaperOrder({
      decision,
      automationPolicy: executionState.automation || {},
      riskPolicy: executionState.policy || {},
      account: {
        ...account,
        equity,
      },
      ...
      marketPrice,
      candidateAnnualizedVolatilityPct: number(
        order?.risk_snapshot?.sizing?.candidate_annualized_volatility_pct,
        null,
      ),
      marketRegimeSizingScale: number(
        order?.risk_snapshot?.sizing?.market_regime_sizing_scale,
        1,
      ),
    });

    const currentSizingExecutable = currentAutomationSizing?.executable === true;
    const currentSizingSideMatches = text(
      currentAutomationSizing?.side,
    ).toUpperCase() === side;
    const buyRemainderWithinCurrentSizing = (
      side !== "BUY" ||
      number(currentAutomationSizing?.quantity, 0) + 1e-12 >= remainingQuantity
    );

    automationSizingRevalidation = {
      approved:
        currentSizingExecutable &&
        currentSizingSideMatches &&
        buyRemainderWithinCurrentSizing,
      status: currentSizingExecutable
        ? buyRemainderWithinCurrentSizing
          ? "CURRENT_POLICY_ALLOWED"
          : "CURRENT_POLICY_REQUIRES_SMALLER_BUY"
        : "CURRENT_POLICY_NOT_EXECUTABLE",
      reason: currentAutomationSizing?.reason || null,
      current_sizing: currentAutomationSizing || null,
      order_remaining_quantity: remainingQuantity,
      evaluated_at: new Date().toISOString(),
    };

    if (!automationSizingRevalidation.approved) {
      const cancellationReason = currentSizingExecutable
        ? "AUTOMATION_SIZING_POLICY_CHANGED"
        : "AUTOMATION_SIZING_NO_LONGER_EXECUTABLE";
      const cancelledAt = new Date().toISOString();
  ...
  const revalidationReasons = [
    ...
    ...(strategyReadiness.reasons || []),
    ...(strategyHealth.reasons || []),
    ...(automationSizingRevalidation.approved
      ? []
      : [automationSizingRevalidation.reason || automationSizingRevalidation.status]),
