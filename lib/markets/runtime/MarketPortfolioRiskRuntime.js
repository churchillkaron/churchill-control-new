...
async function ensureClassificationRevision({ organizationId }) {
  const { error } = await supabaseAdmin
    .from("market_instrument_classification_revisions")
    .upsert({
      organization_id: organizationId,
      revision: 0,
    }, {
...
async function readClassificationRevision({ organizationId }) {
  const { data, error } = await supabaseAdmin
    .from("market_instrument_classification_revisions")
    .select("revision,updated_at")
    .eq("organization_id", organizationId)
...
async function ensureBenchmarkRevision({ organizationId, portfolioId }) {
  const { error } = await supabaseAdmin
    .from("market_portfolio_benchmark_revisions")
    .upsert({
      organization_id: organizationId,
      portfolio_id: portfolioId,
      revision: 0,
...
async function readBenchmarkRevision({ organizationId, portfolioId }) {
  const { data, error } = await supabaseAdmin
    .from("market_portfolio_benchmark_revisions")
    .select("revision,updated_at")
    .eq("organization_id", organizationId)
...
export async function evaluateMarketPortfolioRisk({
  ...
  await Promise.all([
    ensureBenchmarkRevision({ organizationId, portfolioId }),
    ensureClassificationRevision({ organizationId }),
  ]);
  const [benchmarkRevisionBefore, classificationRevisionBefore] = await Promise.all([
    readBenchmarkRevision({ organizationId, portfolioId }),
    readClassificationRevision({ organizationId }),
  ]);
  ...
  const [dailyBarRevisionAfter, benchmarkRevisionAfter, classificationRevisionAfter] = await Promise.all([
    readDailyBarRevision({ organizationId, portfolioId }),
    readBenchmarkRevision({ organizationId, portfolioId }),
    readClassificationRevision({ organizationId }),
  ]);
  ...
  const benchmarkStable = (
    benchmarkRevisionBefore.revision === benchmarkRevisionAfter.revision
  );
  const classificationStable = (
    classificationRevisionBefore.revision === classificationRevisionAfter.revision
  );

  if ((!dailyBarStable || !benchmarkStable || !classificationStable) && side === "BUY") {
    return {
      approved: false,
      reasons: [
        ...(!dailyBarStable ? ["Daily historical bar dataset changed during portfolio-risk evaluation."] : []),
        ...(!benchmarkStable ? ["Portfolio benchmark changed during portfolio-risk evaluation."] : []),
        ...(!classificationStable ? ["Instrument classification changed during portfolio-risk evaluation."] : []),
      ],
      metrics: {
        benchmark_symbol: benchmarkSymbol,
        benchmark_revision: benchmarkRevisionAfter.revision,
        benchmark_revision_before: benchmarkRevisionBefore.revision,
        benchmark_consistency_status: benchmarkStable ? "STABLE" : "CHANGED_DURING_EVALUATION",
        classification_revision: classificationRevisionAfter.revision,
        classification_revision_before: classificationRevisionBefore.revision,
        classification_consistency_status: classificationStable ? "STABLE" : "CHANGED_DURING_EVALUATION",
  ...
  return {
    ...
    metrics: {
      benchmark_symbol: benchmarkSymbol,
      benchmark_revision: benchmarkRevisionAfter.revision,
      benchmark_revision_updated_at: benchmarkRevisionAfter.updated_at,
      benchmark_revision_before: benchmarkRevisionBefore.revision,
      benchmark_consistency_status: benchmarkStable ? "STABLE" : "CHANGED_DURING_EVALUATION",
      classification_revision: classificationRevisionAfter.revision,
      classification_revision_updated_at: classificationRevisionAfter.updated_at,
      classification_revision_before: classificationRevisionBefore.revision,
      classification_consistency_status: classificationStable ? "STABLE" : "CHANGED_DURING_EVALUATION",
      daily_bar_revision: dailyBarRevisionAfter.revision,
      daily_bar_revision_updated_at: dailyBarRevisionAfter.updated_at,
