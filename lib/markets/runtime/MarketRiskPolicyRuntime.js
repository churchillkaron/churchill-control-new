...
export function evaluatePaperTradeRisk({
  ...
  if (equity > 0) {
    const projectedPct = ((currentPositionValue + (String(order.side).toUpperCase() === "BUY" ? orderNotional : -orderNotional)) / equity) * 100;
    if (projectedPct > number(policy.max_position_pct, 10)) {
    ...
    if (dailyLossPct >= number(policy.max_daily_loss_pct, 2)) {
      reasons.push("Daily loss limit has been reached.");
    }
  }

  if (drawdownPct >= number(policy.max_portfolio_drawdown_pct, 10)) {
