...
export function buildNewsThesis({ symbol, evidence, now = new Date() }) {
  ...
  const scored = rows
    .map((row) => {
      const observedMs = new Date(row.observed_at || row.received_at || 0).getTime();
      const ageHours = Number.isFinite(observedMs) && Number.isFinite(nowMs)
        ? Math.max(0, (nowMs - observedMs) / (60 * 60 * 1000))
        : Number.POSITIVE_INFINITY;
      return { row, ageHours };
    })
    .filter(({ row, ageHours }) => number(row.sentiment) !== null && ageHours <= (7 * 24));
  ...
  if (!scored.length) {
    return {
      ...
      rationale: {
        ...
        reason: rows.length
          ? "No governed news score remains inside the seven-day short-horizon evidence window."
  ...
  let weighted = 0;
  let weightTotal = 0;
  let freshestAgeHours = Number.POSITIVE_INFINITY;
  for (const { row, ageHours } of scored) {
    const materiality = number(row.materiality, 0.5);
    const sentiment = number(row.sentiment, 0);
    const freshnessWeight = Math.exp(-ageHours / 36);
    const weight = Math.max(materiality, 0.05) * freshnessWeight;
    weighted += sentiment * weight;
    weightTotal += weight;
    freshestAgeHours = Math.min(freshestAgeHours, ageHours);
  }
  const score = weightTotal > 0 ? weighted / weightTotal : 0;
  const freshnessConfidence = Number.isFinite(freshestAgeHours)
    ? Math.max(0.45, Math.exp(-freshestAgeHours / 72))
    : 0.45;
  const confidence = clamp(
    (0.35 + Math.min(weightTotal / 8, 0.4)) * freshnessConfidence,
    0,
    0.82,
  );
  ...
  return {
    ...
    rationale: {
      ...
      freshest_age_hours: Number.isFinite(freshestAgeHours) ? freshestAgeHours : null,
...
export function assessFundamentalFreshness({
  fundamental,
  now = new Date(),
}) {
  const facts = Object.values(fundamental?.facts_used || {})
    .filter((fact) => fact?.filed)
    .map((fact) => ({
      filed: fact.filed,
      form: String(fact.form || "").toUpperCase(),
      filed_ms: new Date(fact.filed).getTime(),
    }))
    .filter((fact) => Number.isFinite(fact.filed_ms))
    .sort((left, right) => right.filed_ms - left.filed_ms);
  ...
  if (!latest || !Number.isFinite(nowMs)) {
    return {
      fresh: false,
      reason: "MISSING_FILING_FRESHNESS_EVIDENCE",
      latest_filed: latest?.filed || null,
      latest_form: latest?.form || null,
      age_days: null,
      max_age_days: null,
    };
  }

  const ageDays = (nowMs - latest.filed_ms) / (24 * 60 * 60 * 1000);
  if (ageDays < -1) {
    return {
      fresh: false,
      reason: "FUTURE_DATED_FILING_EVIDENCE",
  ...
  const annual = ["10-K", "20-F"].includes(latest.form);
  const maxAgeDays = annual ? 450 : 180;
  return {
    fresh: ageDays <= maxAgeDays,
    reason: ageDays <= maxAgeDays ? null : "STALE_FILING_EVIDENCE",
    latest_filed: latest.filed,
    latest_form: latest.form,
    age_days: ageDays,
...
export function buildFundamentalThesis({
  symbol,
  filings,
  fundamentals,
  now = new Date(),
  ...
  const fundamentalRows = (Array.isArray(fundamentals) ? fundamentals : [])
    .filter((row) => !row.symbol || row.symbol === symbol)
    .sort((left, right) => new Date(right.period_end || right.generated_at || 0) - new Date(left.period_end || left.generated_at || 0));
  const latest = fundamentalRows[0] || null;
  const confidence = number(latest?.confidence, 0);
  const stance = String(latest?.stance || "").toUpperCase();
  ...
  if (
    ...
    !freshness.fresh ||
    confidence <= 0 ||
    !["BULLISH", "BEARISH", "NEUTRAL"].includes(stance)
  ) {
    return {
      symbol,
      ...
      rationale: {
        ...
        freshness,
        reason: latest && !freshness.fresh
          ? "The latest admitted fundamental snapshot is stale or lacks valid filing freshness evidence."
          : rows.length
            ? "SEC filings exist but no admitted point-in-time fundamental snapshot is available."
            : "No SEC filing evidence is available.",
      },
    };
  }
  ...
  return {
    ...
    downside_risk: null,
    rationale: {
      fundamental_score: number(latest.score),
      period_end: latest.period_end || null,
      freshness,
      ratios: latest.ratios || {},
      metrics_available: Object.keys(latest.metrics || {}),
      filing_count: rows.length,
      latest_forms: rows.slice(0, 8).map((row) => row.form_type),
      method: "SEC XBRL deterministic fundamental composite",
    },
  };
}
...
export function synthesizeMarketDecision({
  symbol,
  theses,
  horizon = "MEDIUM",
  agentWeights = {},
  ...
  for (const thesis of eligible) {
    const agentType = String(thesis.agent_type || "").toUpperCase();
    const reliabilitySource = agentWeights?.[agentType];
    const reliabilityWeight = clamp(
      number(
        reliabilitySource && typeof reliabilitySource === "object"
          ? reliabilitySource.weight
          : reliabilitySource,
        1,
      ),
      0.5,
    ...
    const regimeWeight = clamp(
      number(marketRegime?.specialist_weight_multipliers?.[agentType], 1),
      0.5,
      1.5,
    );
    const epistemicConfidence = clamp(number(thesis.confidence, 0), 0, 1);
    const weight = epistemicConfidence * reliabilityWeight * regimeWeight;

    reliabilityWeights[agentType] = reliabilityWeight;
    regimeWeights[agentType] = regimeWeight;
    effectiveWeights[agentType] = weight;
  ...
  return {
    ...
    decision_payload: {
      ...
      agent_reliability_weights: reliabilityWeights,
      agent_regime_weights: regimeWeights,
      agent_effective_weights: effectiveWeights,
      market_regime: marketRegime
        ? {
            regime: marketRegime.regime || "UNKNOWN",
            confidence: number(marketRegime.confidence, 0),
            metrics: marketRegime.metrics || {},
            sizing_scale: number(marketRegime.sizing_scale, 1),
          }
        : null,
      method: "confidence-performance-and-regime-weighted independent specialist ensemble",
