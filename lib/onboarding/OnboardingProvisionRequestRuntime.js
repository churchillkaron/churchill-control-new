...
function migrationMissing(error) {
  ...
  return (
    ...
    (
      code === "PGRST202" &&
      message.includes("retire_stale_onboarding_provision_request")
    ) ||
    (
      message.includes("onboarding_provision_requests") &&
      message.includes("schema cache")
    )
  );
}

function onboardingMigrationRequiredError() {
  const error = new Error("Business setup is temporarily unavailable. Please try again shortly.");
  error.status = 503;
  error.code = "ONBOARDING_MIGRATION_REQUIRED";
  return error;
}
...
export async function acquireOnboardingProvisionRequest({
  ...
  if (migrationMissing(inserted.error)) {
    throw onboardingMigrationRequiredError();
  }
  ...
  if (stale && org.data?.organization_status === "PROVISIONING") {
    ...
    if (retired.error) {
      if (migrationMissing(retired.error)) throw onboardingMigrationRequiredError();
      throw retired.error;
    }
