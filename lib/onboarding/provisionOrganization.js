...
const ORGANIZATION_CREATOR_ROLES = new Set([
  "OWNER",
  "ORGANIZATION_OWNER",
  "ORG_OWNER",
  "PLATFORM_OWNER",
...
function getFiscalYearDates({
  ...
  const beforeStart = currentMonth < month || (currentMonth === month && currentDay < day);
  const startYear = beforeStart ? currentYear - 1 : currentYear;
  const crossesYear = Number(endMonth) < month || (Number(endMonth) === month && Number(endDay) < day);
  const endYear = crossesYear ? startYear + 1 : startYear;

  return {
    year: endYear,
    startDate: `${startYear}-${pad2(month)}-${pad2(day)}`,
    endDate: `${endYear}-${pad2(endMonth || 12)}-${pad2(endDay || 31)}`,
  };
}
...
async function resolveOwnerAccount({ ownerEmail, authUserId }) {
  const { data, error } = await supabaseAdmin
    .from("staff_accounts")
    .select("id,email,name,party_id,active_organization_id,auth_user_id,active,role")
  ...
  if (!data?.length) {
    throw new Error(
      "The owner email must match an active staff account for the authenticated Avantiqo user"
    );
  }

  if (data.length > 1) {
    throw new Error(
      "Multiple active Avantiqo staff accounts match this login and owner email; resolve the duplicate identity before onboarding"
    );
  }

  const role = String(data[0]?.role || "").trim().toUpperCase();
  if (!ORGANIZATION_CREATOR_ROLES.has(role)) {
    throw new Error("Organization creation requires owner authority");
  }
...
export async function provisionOrganization(payload) {
  ...
  // Validate all governed preconditions before creating any organization records.
  const onboardingIndustry = await requireOnboardingIndustry(industry);
  const staffAccount = await resolveOwnerAccount({ ownerEmail, authUserId });

  const previousActiveOrganizationId = staffAccount.active_organization_id || null;
  let organization = null;

  try {
    organization = await createOrganization({
    id: payload?.requestedOrganizationId || null,
    name,
    organizationType:
      payload?.organization?.organizationType ||
      payload?.organization?.organization_type ||
      onboardingIndustry.organizationType ||
      "client_company",
  ...
  const taxSetup = await applyTaxSetup({
    ...
    vatRegistered: payload?.finance?.vatRegistered === true,
    withholdingTaxEnabled: payload?.finance?.withholdingTaxEnabled === true,
    vatRate: payload?.finance?.vatRate ?? null,
    vatRateValidThrough: payload?.finance?.vatRateValidThrough || null,
    fiscalYearStartMonth: payload?.finance?.fiscalYearStartMonth || 1,
    fiscalYearStartDay: payload?.finance?.fiscalYearStartDay || 1,
    fiscalYearEndMonth: payload?.finance?.fiscalYearEndMonth || 12,
    fiscalYearEndDay: payload?.finance?.fiscalYearEndDay || 31,
  });

  if (!taxSetup.success) {
    throw new Error(taxSetup.error || "Tax and accounting setup failed");
  }
  ...
  const legalEntityResult = await createLegalEntity({
    ...
    tax_id: payload?.finance?.taxRegistrationNumber || null,
    registration_number: payload?.finance?.companyRegistrationNumber || null,
    country: payload?.finance?.country || country,
    currency,
    timezone: payload?.finance?.timezone || null,
    locale: payload?.finance?.locale || null,
    phone: ownerPhone,
    email: ownerEmail,
  ...
  if (!legalEntityResult.success) {
    throw new Error(legalEntityResult.error || "Legal entity setup failed");
  }
  ...
  const dates = getFiscalYearDates({
    startMonth: payload?.finance?.fiscalYearStartMonth || 1,
    startDay: payload?.finance?.fiscalYearStartDay || 1,
    endMonth: payload?.finance?.fiscalYearEndMonth || 12,
    endDay: payload?.finance?.fiscalYearEndDay || 31,
  });
  ...
  const activated = await supabaseAdmin
    ...
    .eq("organization_status", "PROVISIONING")
    .select("*")
    .single();

  if (activated.error || !activated.data?.id) {
    throw activated.error || new Error("Organization activation failed");
  }
  organization = activated.data;

  let payments = null;
  let paymentSetupWarning = null;
  try {
    const setup = payload?.paymentSetup || {};
    const requested =
      Boolean(setup.enableBankTransfer) ||
      Boolean(setup.enableCards) ||
      Boolean(setup.enablePromptPay);

    if (requested) {
      payments = await OrganizationPaymentOnboardingRuntime.configure({
        organizationId: organization.id,
        entity,
        ownerEmail,
        businessName: organization.name || name,
    ...
    paymentSetupWarning =
      paymentSetupError?.message || "Payment setup could not be completed";
  }
  ...
  return {
    ...
    if (organization?.id) {
      await Promise.allSettled([
        ...
        supabaseAdmin
          .from("staff_accounts")
          .update({ active_organization_id: previousActiveOrganizationId })
          .eq("id", staffAccount.id)
          .eq("auth_user_id", staffAccount.auth_user_id)
          .eq("active_organization_id", organization.id),
      ]);
    }
    throw error;
  }
