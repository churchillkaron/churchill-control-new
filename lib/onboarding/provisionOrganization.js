...
async function resolveOwnerAccount(ownerEmail) {
  const { data, error } = await supabaseAdmin
    .from("staff_accounts")
    .select("id,email,name,party_id,active_organization_id")
  ...
  if (!data?.id) {
    throw new Error(
      "The owner email must belong to an existing Avantiqo account before an organization can be created"
    );
  }
...
export async function provisionOrganization(payload) {
  ...
  const staffAccount = await resolveOwnerAccount(ownerEmail);
