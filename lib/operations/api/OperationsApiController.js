...
function projectLifecycle(capability, record) {
  if (!record) return record;

  return {
    ...record,
    allowed_commands: capability.readOnly
      ? []
      : getAllowedOperationsCommands({
...
export function createOperationsApiController({
  ...
  async function list({ capabilityId, context, filters = {} }) {
    try {
      ...
      const rows = persistedRows.map((record) => projectLifecycle(capability, record));
    ...
    try {
      ...
      return {
        ...
        body: {
          ok: true,
          capability,
          record: projectLifecycle(capability, persistedRecord),
        },
      };
    } catch (error) {
      return normaliseError(error);
