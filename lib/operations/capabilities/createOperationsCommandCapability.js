...
export function createOperationsCommandCapability(capability, command) {
  ...
  const manifest = defineCapability({
    ...
    operatorVerification: targetsExistingRecord
      ? { capability_key: `operations.${operationsUbteCapabilityName(capability.id)}.list`, payload_keys: ["id"], derivation: "declared_exact_operations_record_verifier" }
      : { capability_key: `operations.${operationsUbteCapabilityName(capability.id)}.list`, payload_from_result: { id: ["id", "record.id", "data.id"] }, derivation: "declared_result_bound_operations_create_verifier" },
  ...
  async function execute({ context, payload = {} }) {
    ...
    const { serverOperationsApi } = await import(
      "@/lib/operations/api/createServerOperationsApi"
    );
