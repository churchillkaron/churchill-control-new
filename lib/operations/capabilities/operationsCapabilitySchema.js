...
export function fieldsToJsonSchema(fields = [], { required = [], respectFieldRequired = true } = {}) {
  const properties = {};
  ...
  const requiredNames = [
    ...new Set([
      ...required,
      ...(respectFieldRequired ? fields.filter((field) => field?.required && field?.name).map((field) => field.name) : []),
    ]),
