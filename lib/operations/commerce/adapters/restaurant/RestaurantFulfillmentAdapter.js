import { supabaseAdmin } from "@/lib/shared/supabase/admin";
import { resolveEntity } from "@/lib/platform/entities/resolveEntity";
import updateKitchenTicketItemStatus from "@/lib/restaurant/operations/updateKitchenTicketItemStatus";
import updateBarTicketItemStatus from "@/lib/restaurant/operations/updateBarTicketItemStatus";

const CLOSED_STATUSES = new Set(["COMPLETED", "CANCELLED", "VOID"]);
const BAR_SOURCE_TYPES = new Set([
  "restaurant_bar_ticket",
  "bar_ticket",
  "bar",
]);

function statusOf(value) {
  return String(value || "").trim().toUpperCase();
}

function sourceTypeOf(value) {
  return String(value || "").trim().toLowerCase();
}

function tableLabel(table) {
  return table?.table_number || table?.table_name || null;
}

async function requireEntity({ organizationId, entityId }) {
  if (!entityId) {
    const error = new Error("Select a legal entity before loading restaurant fulfillment");
    error.status = 409;
    throw error;
  }

  const entity = await resolveEntity({ organizationId, entityId });
  if (!entity) {
    const error = new Error("Selected legal entity is outside the organization or inactive");
    error.status = 409;
    throw error;
  }

  return entity;
}

function normalizeTicket({
  ticket,
  sourceType,
  orderById,
  tableById,
  workCenterById,
}) {
  const order = orderById.get(ticket.order_id) || null;
  const workCenter = workCenterById.get(ticket.work_center_id) || null;
  const contextReference =
    ticket.table_number ||
    tableLabel(tableById.get(ticket.table_id || order?.table_id));
  const items = (Array.isArray(ticket.items) ? ticket.items : []).map((item) => ({
    id: item.id || item.order_item_id,
    source_id: item.id || item.order_item_id,
    order_item_id: item.order_item_id || item.id || null,
    name: item.item_name || item.name || "Item",
    quantity: Number(item.quantity || 1),
    status: statusOf(item.status || "NEW"),
    notes: item.notes || null,
    attributes: {
      cooking_level: item.cooking_level || null,
      seat_position: item.seat_position || null,
    },
    raw: item,
  }));

  const fallbackName =
    sourceType === "restaurant_bar_ticket" ? "Bar" : "Production";
  const workCenterName =
    workCenter?.name || ticket.station || workCenter?.code || fallbackName;

  return {
    id: ticket.id,
    entity_id: ticket.entity_id || null,
    queue_id:
      ticket.work_center_id ||
      ticket.station ||
      (sourceType === "restaurant_bar_ticket"
        ? "restaurant-bar"
        : "restaurant-production"),
    queue_name: workCenterName,
    work_center: {
      id: ticket.work_center_id || null,
      name: workCenterName,
    },
    status: statusOf(ticket.status || "NEW"),
    priority: ticket.priority || null,
    created_at: ticket.created_at,
    started_at: ticket.started_at || null,
    ready_at: ticket.ready_at || null,
    completed_at: ticket.completed_at || null,
    source: {
      type: sourceType,
      id: ticket.id,
    },
    demand: {
      type: "order",
      id: ticket.order_id || null,
      reference: ticket.order_id || null,
    },
    context: {
      type: "service_location",
      id: ticket.table_id || order?.table_id || null,
      reference: contextReference || null,
      label: contextReference ? `Table ${contextReference}` : null,
    },
    work_items: items,
    raw: ticket,
  };
}

async function listQueue({ organizationId, entityId, scope = "active", allowedSourceTypes = [] }) {
  const entity = await requireEntity({ organizationId, entityId });
  const allowed = new Set((allowedSourceTypes || []).map(sourceTypeOf));
  const allowKitchen = allowed.has("restaurant_kitchen_ticket");
  const allowBar = allowed.has("restaurant_bar_ticket");
  if (!allowKitchen && !allowBar) {
    const error = new Error("Restaurant fulfillment access denied");
    error.status = 403;
    throw error;
  }

  const [
    kitchenTicketsResult,
    barTicketsResult,
    ordersResult,
    tablesResult,
    workCentersResult,
  ] = await Promise.all([
    allowKitchen
      ? supabaseAdmin
          .from("kitchen_tickets")
          .select("*")
          .eq("organization_id", organizationId)
          .eq("entity_id", entity.id)
          .order("created_at", { ascending: true })
          .limit(500)
      : Promise.resolve({ data: [], error: null }),
    allowBar
      ? supabaseAdmin
          .from("bar_tickets")
          .select("*")
          .eq("organization_id", organizationId)
          .eq("entity_id", entity.id)
          .order("created_at", { ascending: true })
          .limit(500)
      : Promise.resolve({ data: [], error: null }),
    supabaseAdmin
      .from("orders")
      .select("id, table_id, status")
      .eq("organization_id", organizationId)
      .eq("entity_id", entity.id)
      .order("created_at", { ascending: false })
      .limit(500),
    supabaseAdmin
      .from("restaurant_tables")
      .select("id, table_number, table_name")
      .eq("organization_id", organizationId),
    supabaseAdmin
      .from("work_centers")
      .select("id, name, code")
      .eq("organization_id", organizationId)
      .eq("active", true),
  ]);

  if (kitchenTicketsResult.error) throw kitchenTicketsResult.error;
  if (barTicketsResult.error) throw barTicketsResult.error;
  if (ordersResult.error) throw ordersResult.error;
  if (tablesResult.error) throw tablesResult.error;
  if (workCentersResult.error) throw workCentersResult.error;

  const orderById = new Map(
    (ordersResult.data || []).map((order) => [order.id, order])
  );
  const tableById = new Map(
    (tablesResult.data || []).map((table) => [table.id, table])
  );
  const workCenterById = new Map(
    (workCentersResult.data || []).map((workCenter) => [workCenter.id, workCenter])
  );

  const shared = { orderById, tableById, workCenterById };
  const queueEntries = [
    ...(kitchenTicketsResult.data || []).map((ticket) =>
      normalizeTicket({
        ticket,
        sourceType: "restaurant_kitchen_ticket",
        ...shared,
      })
    ),
    ...(barTicketsResult.data || []).map((ticket) =>
      normalizeTicket({
        ticket,
        sourceType: "restaurant_bar_ticket",
        ...shared,
      })
    ),
  ].sort((a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0));

  const normalizedScope = String(scope || "active").toLowerCase();
  const visibleEntries = queueEntries.filter((entry) => {
    if (normalizedScope === "all") return true;
    if (normalizedScope === "ready") {
      return (
        entry.status === "READY" ||
        entry.work_items.some((item) => item.status === "READY")
      );
    }
    return !CLOSED_STATUSES.has(entry.status);
  });

  return {
    application_id: "restaurant",
    entity_id: entity.id,
    queue_type: "fulfillment",
    entries: visibleEntries,
    metrics: {
      total: queueEntries.length,
      active: queueEntries.filter((entry) => !CLOSED_STATUSES.has(entry.status)).length,
      ready: queueEntries.filter(
        (entry) =>
          entry.status === "READY" ||
          entry.work_items.some((item) => item.status === "READY")
      ).length,
    },
  };
}

async function transitionWorkItem({ body, organizationId, entityId, access }) {
  const entity = await requireEntity({ organizationId, entityId });
  const sourceType = sourceTypeOf(
    body.sourceType ||
      body.source_type ||
      body.ticketType ||
      body.ticket_type ||
      body.source?.type
  );
  const updateTicketItemStatus = BAR_SOURCE_TYPES.has(sourceType)
    ? updateBarTicketItemStatus
    : updateKitchenTicketItemStatus;

  const result = await updateTicketItemStatus({
    organizationId,
    entityId: entity.id,
    actorId:
      access?.access?.staffAccountId ||
      access?.staff?.id ||
      access?.user?.id ||
      null,
    ticketId:
      body.queueEntryId ||
      body.queue_entry_id ||
      body.ticketId ||
      body.ticket_id ||
      null,
    itemId:
      body.workItemId ||
      body.work_item_id ||
      body.itemId ||
      body.item_id,
    status: body.status || body.transition,
  });

  if (!result.success) {
    const error = new Error(result.error || "Fulfillment transition failed");
    error.status = 400;
    throw error;
  }

  return result;
}

const RestaurantFulfillmentAdapter = Object.freeze({
  id: "restaurant-fulfillment",
  listQueue,
  transitionWorkItem,
});

export default RestaurantFulfillmentAdapter;
