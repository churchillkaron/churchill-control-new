...
async function listQueue({ organizationId, scope = "active" }) {
  const [ticketsResult, ordersResult, tablesResult, workCentersResult] =
    await Promise.all([
      supabaseAdmin
  ...
  const queueEntries = (ticketsResult.data || []).map((ticket) => {
    ...
    const workCenterName =
      workCenter?.name || ticket.station || workCenter?.code || "Production";

    return {
      id: ticket.id,
      queue_id: ticket.work_center_id || ticket.station || "restaurant-production",
