...
async function createOrder({ body, access, entityId, organizationId, request }) {
  ...
  const resolvedEntityId = uuidOrNull(
    entityId ||
  ...
  const rpcResult = await supabaseAdmin.rpc("pos_create_order_atomic", {
    ...
    p_items: items,
    p_customer_id: uuidOrNull(readValue(body, "partyId", "party_id")),
  ...
  return {
    ...
    context,
    entity_id: financialPolicy.entityId || resolvedEntityId || null,
...
async function listPayableContexts({ organizationId }) {
  const [tablesResult, ordersResult] = await Promise.all([
    ...
    supabaseAdmin
      .from("orders")
      .select("id, table_id, status, total, total_amount, amount_paid, remaining_balance")
      .eq("organization_id", organizationId)
