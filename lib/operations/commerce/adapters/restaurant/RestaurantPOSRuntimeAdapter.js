...
async function restaurantItemCorrectionsReady() {
...
export async function loadRestaurantPOSRuntime({ organizationId }) {
  const [zones, tables, dishes, itemCorrectionsReady] = await Promise.all([
    queryRows(
      supabaseAdmin
    ...
    restaurantItemCorrectionsReady(),
  ]);
  ...
  return {
    ...
    capabilities: {
      item_corrections_ready: itemCorrectionsReady,
    },
