...
function normalizeReceiptItem(item) {
  ...
  const originalAmount = round(numeric(item.price) * numeric(item.quantity || 1));
  const billable = !NON_BILLABLE_ITEM_STATUSES.has(status);
  ...
  return {
    ...
    billable,
    total: billable ? originalAmount : 0,
    adjustment_type: billable ? null : status,
  };
}
  ...
  return persistedOrders
    .map((order) => {
      ...
      return {
        ...
        items: (order.order_items || []).map(normalizeReceiptItem),
