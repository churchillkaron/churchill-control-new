import { supabaseAdmin } from "@/lib/shared/supabase/admin";

const NON_BILLABLE_ITEM_STATUSES = new Set([
  "VOID",
  "VOIDED",
  "CANCELLED",
  "CANCELED",
]);

const SETTLED_PAYMENT_STATUSES = new Set([
  "PAID",
  "COMPLETED",
  "SUCCEEDED",
  "SUCCESS",
  "SETTLED",
  "POSTED",
]);

function numeric(value) {
  const parsed = Number(value || 0);
  return Number.isFinite(parsed) ? parsed : 0;
}

function round(value) {
  return Number(numeric(value).toFixed(2));
}

function statusOf(value) {
  return String(value || "").trim().toUpperCase();
}

function isMissingRelation(error, relation) {
  const message = String(error?.message || "").toLowerCase();
  return (
    error?.code === "42P01" ||
    error?.code === "PGRST205" ||
    message.includes(String(relation || "").toLowerCase())
  );
}

function requestEntityId(request) {
  try {
    const searchParams = new URL(request?.url || "http://localhost").searchParams;
    return (
      searchParams.get("entityId") ||
      searchParams.get("entity_id") ||
      searchParams.get("legalEntityId") ||
      searchParams.get("legal_entity_id") ||
      null
    );
  } catch {
    return null;
  }
}

function tableReference(table) {
  return table?.table_number || table?.table_name || table?.name || null;
}

function receiptContext(order, table) {
  const reference = tableReference(table);
  return {
    type: "service_location",
    id: table?.id || order.table_id || null,
    reference: reference == null ? null : String(reference),
    label:
      reference == null
        ? "Unassigned service location"
        : `Table ${reference}`,
  };
}

function normalizeReceiptItem(item, correction) {
  const status = statusOf(item.status) || "ORDERED";
  const correctionType = statusOf(correction?.correction_type);
  const originalAmount = round(
    correction?.original_amount ?? numeric(item.price) * numeric(item.quantity || 1)
  );
  const billable =
    !NON_BILLABLE_ITEM_STATUSES.has(status) && correctionType !== "COMP";
  const adjustmentType = correctionType || (billable ? null : status);

  return {
    ...item,
    status,
    original_total: originalAmount,
    billable,
    total: billable ? originalAmount : round(correction?.corrected_amount || 0),
    adjustment_type: adjustmentType,
    correction: correction
      ? {
          id: correction.id,
          type: correctionType,
          reason: correction.reason || null,
          created_at: correction.created_at || null,
          created_by: correction.created_by || null,
        }
      : null,
  };
}

export async function listRestaurantReceipts({ organizationId, orderId, request }) {
  const entityId = requestEntityId(request);
  if (!entityId) {
    const error = new Error("Select an active legal entity before loading receipts");
    error.status = 400;
    throw error;
  }

  let orderQuery = supabaseAdmin
    .from("orders")
    .select("*, order_items(*)")
    .eq("organization_id", organizationId)
    .eq("entity_id", entityId)
    .order("created_at", { ascending: false })
    .limit(250);

  if (orderId) orderQuery = orderQuery.eq("id", orderId);

  const { data: orders, error: orderError } = await orderQuery;
  if (orderError) throw orderError;

  const persistedOrders = orders || [];
  const orderIds = persistedOrders.map((order) => order.id).filter(Boolean);
  const sessionIds = [
    ...new Set(persistedOrders.map((order) => order.session_id).filter(Boolean)),
  ];
  const tableIds = [
    ...new Set(persistedOrders.map((order) => order.table_id).filter(Boolean)),
  ];

  let tables = [];
  if (tableIds.length) {
    const tableResult = await supabaseAdmin
      .from("restaurant_tables")
      .select("id, table_number, table_name")
      .eq("organization_id", organizationId)
      .in("id", tableIds);
    if (tableResult.error) throw tableResult.error;
    tables = tableResult.data || [];
  }

  const tableById = new Map(tables.map((table) => [table.id, table]));

  let payments = [];
  if (sessionIds.length) {
    const paymentResult = await supabaseAdmin
      .from("payments")
      .select("*")
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
      .in("session_id", sessionIds)
      .order("created_at", { ascending: true });
    if (paymentResult.error) throw paymentResult.error;
    payments = paymentResult.data || [];
  }

  let allocations = [];
  if (orderIds.length) {
    const allocationResult = await supabaseAdmin
      .from("restaurant_payment_allocations")
      .select("payment_id, order_id, amount, allocation_type")
      .eq("organization_id", organizationId)
      .in("order_id", orderIds);
    if (allocationResult.error) {
      if (!isMissingRelation(allocationResult.error, "restaurant_payment_allocations")) {
        throw allocationResult.error;
      }
    } else {
      allocations = allocationResult.data || [];
    }
  }

  let corrections = [];
  if (orderIds.length) {
    const correctionResult = await supabaseAdmin
      .from("restaurant_order_item_corrections")
      .select("id, order_id, order_item_id, correction_type, reason, original_amount, corrected_amount, created_at, created_by")
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
      .in("order_id", orderIds);
    if (correctionResult.error) {
      if (!isMissingRelation(correctionResult.error, "restaurant_order_item_corrections")) {
        throw correctionResult.error;
      }
    } else {
      corrections = correctionResult.data || [];
    }
  }

  const correctionByItemId = new Map(
    corrections.map((correction) => [correction.order_item_id, correction])
  );
  const correctionsByOrderId = new Map();
  for (const correction of corrections) {
    if (!correctionsByOrderId.has(correction.order_id)) {
      correctionsByOrderId.set(correction.order_id, []);
    }
    correctionsByOrderId.get(correction.order_id).push(correction);
  }

  const paymentById = new Map(payments.map((payment) => [payment.id, payment]));
  const paymentsBySession = new Map();
  for (const payment of payments) {
    if (!payment.session_id) continue;
    if (!paymentsBySession.has(payment.session_id)) {
      paymentsBySession.set(payment.session_id, []);
    }
    paymentsBySession.get(payment.session_id).push(payment);
  }

  const paymentsByOrder = new Map();
  for (const allocation of allocations) {
    const payment = paymentById.get(allocation.payment_id);
    if (!payment || !allocation.order_id) continue;
    if (!paymentsByOrder.has(allocation.order_id)) {
      paymentsByOrder.set(allocation.order_id, []);
    }
    const rows = paymentsByOrder.get(allocation.order_id);
    if (!rows.some((row) => row.id === payment.id)) rows.push(payment);
  }

  return persistedOrders
    .map((order) => {
      const table = tableById.get(order.table_id) || null;
      const orderPayments =
        paymentsByOrder.get(order.id) ||
        paymentsBySession.get(order.session_id) ||
        [];
      const orderCorrections = correctionsByOrderId.get(order.id) || [];
      const total = round(order.total_amount ?? order.total);
      const settledPayments = orderPayments.filter(
        (payment) =>
          statusOf(payment.status) !== "VOID" &&
          SETTLED_PAYMENT_STATUSES.has(statusOf(payment.status)) &&
          numeric(payment.amount) > 0.01
      );
      const paid = round(
        settledPayments.reduce((sum, payment) => sum + numeric(payment.amount), 0)
      );
      const hasCompEvidence = orderCorrections.some(
        (correction) => statusOf(correction.correction_type) === "COMP"
      );
      const orderStatus = statusOf(order.status);
      const noPaymentDue =
        total <= 0.01 &&
        paid <= 0.01 &&
        hasCompEvidence &&
        ["CLOSED", "COMPLETED"].includes(orderStatus) &&
        statusOf(order.payment_status || "UNPAID") === "UNPAID";
      const isPaid =
        statusOf(order.payment_status) === "PAID" ||
        (settledPayments.length > 0 && total > 0.01 && paid >= total - 0.01);
      const lifecycleStatus = noPaymentDue
        ? "NO_PAYMENT_DUE"
        : isPaid
          ? "PAID"
          : order.status || "OPEN";

      return {
        application_id: "restaurant",
        entity_id: entityId,
        order_id: order.id,
        receipt_number:
          order.receipt_number ||
          order.order_number ||
          `R-${String(order.id).slice(0, 8).toUpperCase()}`,
        context: receiptContext(order, table),
        created_at: order.paid_at || order.updated_at || order.created_at,
        status: lifecycleStatus,
        no_payment_due: noPaymentDue,
        payment_evidence_present: settledPayments.length > 0,
        items: (order.order_items || []).map((item) =>
          normalizeReceiptItem(item, correctionByItemId.get(item.id))
        ),
        subtotal: round(order.subtotal),
        discount: round(order.discount_amount),
        tax: round(order.vat_amount ?? order.tax_amount),
        service_charge: round(order.service_charge_amount),
        total,
        paid,
        remaining: Math.max(0, round(total - paid)),
        payment_breakdown: settledPayments,
        table_number: tableReference(table) || null,
      };
    })
    .filter(
      (receipt) =>
        orderId ||
        receipt.status === "PAID" ||
        receipt.status === "NO_PAYMENT_DUE"
    );
}

const RestaurantReceiptAdapter = Object.freeze({
  listReceipts: listRestaurantReceipts,
});

export default RestaurantReceiptAdapter;
