...
export async function loadRetailPOSReadiness({
  ...
  return {
    ...
    fulfillment: {
      ...
      state: "blocked",
      reason:
        "Paid Retail sales orders do not yet consume their inventory reservations into final stock movements.",
    },
    readiness: {
      state: orderCaptureReady ? "partial" : "blocked",
      reason: orderCaptureReady
        ? "Catalog, entity-scoped movement availability, Commercial confirmation/reservation, Finance cash settlement and receipts are connected. Inventory fulfilment consumption remains pending."
        : availabilityError ||
          catalogError ||
