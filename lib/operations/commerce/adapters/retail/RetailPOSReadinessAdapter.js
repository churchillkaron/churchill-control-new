...
function entityIdFrom({ access, organization } = {}) {
  return (
    access?.entityId ||
    access?.entity_id ||
...
function catalogItem(item, availability) {
  const onHand = availability ? numeric(availability.on_hand) : null;
  const price = numeric(
  ...
  return {
    ...
    available: onHand === null ? true : onHand > 0,
    availability: {
      status:
...
export async function loadRetailPOSReadiness({
  ...
  const entityId = entityIdFrom({ access, organization });
  let items = [];
  let ledger = [];
  ...
  const catalogItems = items.map((item) =>
    catalogItem(item, availability.get(item.id) || null)
  );
  const bindings = [
    bindingState(
      "catalog",
    ...
    bindingState(
      "availability",
      availabilityError ? "error" : "active",
      availabilityError ||
        `${ledger.length} inventory ledger entries evaluated${entityId ? " for the selected entity" : ""}`
    ),
    bindingState(
      "orders",
      "blocked",
  ...
  const readReady = !catalogError && !availabilityError;
  ...
  return {
    ...
    availability_ready: !availabilityError,
    ...
    readiness: {
      ...
      reason: readReady
        ? "Retail catalog and availability are connected. Order creation and settlement remain blocked until their canonical domain contracts are implemented."
        : "Retail catalog or inventory availability could not be loaded.",
      bindings,
