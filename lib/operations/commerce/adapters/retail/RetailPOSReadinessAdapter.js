...
async function loadFulfillmentDeployment() {
  const result = await supabaseAdmin.rpc(
    "inventory_fulfill_sales_order_atomic",
    {
  ...
  if (!result.error) {
    return {
      ready: false,
      message: "Retail fulfillment deployment probe returned an unexpected success response",
    };
  }

  const message = String(result.error.message || "");

  if (/organization_id required/i.test(message)) {
    return {
      ready: true,
      message:
        "Inventory fulfillment RPC is deployed and available for paid reserved Retail sales orders",
    };
  }

  if (
    ["PGRST202", "PGRST205", "42883"].includes(result.error.code) ||
    /inventory_fulfill_sales_order_atomic|could not find the function/i.test(message)
  ) {
    return {
      ready: false,
      message:
        "Inventory fulfillment migration is not deployed to the active database",
    };
  }

  return {
    ready: false,
    message: message || "Unable to verify Retail fulfillment deployment",
  };
}
...
export async function loadRetailPOSReadiness({
  ...
  const fulfillmentDeploymentPromise = loadFulfillmentDeployment();
  ...
  const fulfillmentDeployment = await fulfillmentDeploymentPromise;
  ...
  const bindings = [
    ...
    bindingState(
      "fulfillment",
      fulfillmentDeployment.ready ? "active" : "blocked",
      fulfillmentDeployment.message
    ),
  ];
  ...
  return {
    ...
    receipts_ready: true,
    fulfillment_ready: fulfillmentDeployment.ready,
    ...
    fulfillment: {
      ...
      ready: fulfillmentDeployment.ready,
      state: fulfillmentDeployment.ready ? "active" : "blocked",
      reason: fulfillmentDeployment.message,
    },
    readiness: {
      state: orderCaptureReady ? "partial" : "blocked",
      reason: orderCaptureReady
        ? fulfillmentDeployment.ready
          ...
          : "Catalog, entity-scoped availability, Commercial confirmation/reservation, Finance cash settlement and receipts are connected. Inventory fulfillment code is present but its database migration is not active."
        : availabilityError ||
          catalogError ||
