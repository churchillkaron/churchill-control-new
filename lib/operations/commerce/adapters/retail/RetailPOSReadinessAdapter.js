...
function availabilityByItem(ledger = []) {
  ...
  for (const row of ledger) {
    ...
    const key = [row.item_id, row.warehouse_id || "", row.location_id || ""].join(":");
...
function catalogItem(item, availability, availabilityReady) {
  ...
  return {
    ...
    availability: {
      status: onHand === null ? "unknown" : onHand > 0 ? "in_stock" : "out_of_stock",
      on_hand: onHand,
...
export async function loadRetailPOSReadiness({
  ...
  const bindings = [
    ...
    bindingState(
      ...
      availabilityError || `${ledger.length} inventory ledger entries evaluated for the selected entity`
    ),
    bindingState(
      "orders",
      orderCaptureReady ? "active" : "blocked",
      orderCaptureReady
        ? "Commercial draft sales-order creation is available"
        : "Catalog and entity-scoped stock are required before draft order creation"
    ),
    bindingState(
      "settlement",
      "blocked",
  ...
  return {
    ...
    readiness: {
      ...
      reason: orderCaptureReady
        ? "Catalog, entity-scoped availability and draft sales orders are connected. Confirmation, stock reservation and Finance settlement remain blocked."
        : availabilityError || catalogError || "Retail catalog or availability could not be loaded.",
      bindings,
