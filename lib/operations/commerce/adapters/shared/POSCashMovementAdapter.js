...
async function resolveCashAccountId({ organizationId, entityId }) {
  ...
  if (exact.error && exact.error.code !== "PGRST116") throw exact.error;
  if (exact.data?.debit_account_id) return exact.data.debit_account_id;
  ...
  return fallback.data?.debit_account_id || null;
}
