...
function normalizeSession(row, actorNames = new Map(), reviewLogs = new Map()) {
  ...
  const paidInTotal = numeric(row.paid_in_total);
  const paidOutTotal = numeric(row.paid_out_total);
  const adjustmentInTotal = numeric(row.adjustment_in_total);
  const adjustmentOutTotal = numeric(row.adjustment_out_total);
  ...
  return {
    ...
    paid_in_total: paidInTotal,
    paid_out_total: paidOutTotal,
    adjustment_in_total: adjustmentInTotal,
    adjustment_out_total: adjustmentOutTotal,
    net_sales: numeric(row.net_sales),
    expected_cash: expectedCash,
    variance,
    variance_journal_entry_id: row.variance_journal_entry_id || null,
    variance_posted_at: row.variance_posted_at || null,
    ...
    approved_by_name: row.approved_by
      ? actorNames.get(String(row.approved_by)) || null
      : null,
    accounting_confirmed_by_name: row.accounting_confirmed_by
      ? actorNames.get(String(row.accounting_confirmed_by)) || null
      : null,
    review_log: reviewLog
      ? {
          ...reviewLog,
          acted_by_name:
            actorNames.get(String(reviewLog.acted_by)) || null,
        }
      : null,
...
function throwRpcError(error, functionName) {
  if (rpcUnavailable(error, functionName)) {
    const unavailable = new Error(
      functionName === "pos_review_cash_session_atomic"
        ? "POS cash-session review governance is not deployed in the database"
...
async function loadReviewEvidence({ organizationId, rows }) {
  ...
  if (!sessionIds.length) {
    return { actorNames: new Map(), reviewLogs: new Map() };
  }

  const { data: logs, error: logError } = await supabaseAdmin
    .from("approval_logs")
    .select(
      "id, entity_id, from_status, to_status, acted_by, role, notes, created_at"
    )
  ...
  const actorIds = [
    ...new Set(
      [
        ...(rows || []).flatMap((row) => [
          row.approved_by,
          row.accounting_confirmed_by,
        ]),
        ...(logs || []).map((log) => log.acted_by),
      ].filter(Boolean)
    ),
  ];
  ...
  if (actorIds.length) {
    ...
    for (const person of staff || []) {
      actorNames.set(
        String(person.id),
        person.name || person.email || String(person.id)
      );
    }
  }
...
async function loadActiveSessionLiveTotals({
  organizationId,
  entityId,
  applicationId,
  session,
  ...
  const [paymentResult, correctionResult, movementResult] = await Promise.all([
    supabaseAdmin
      .from("payments")
    ...
    supabaseAdmin
      .from("pos_payment_corrections")
      .select("amount, correction_type")
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
      .eq("application_id", applicationId)
      .eq("cash_session_id", session.id)
  ...
  const paymentTotals = (paymentResult.data || []).reduce(
    (totals, payment) => {
      ...
      else if (["CARD", "CREDIT_CARD"].includes(method)) totals.card += amount;
      else if (["QR", "QR_PAYMENT"].includes(method)) totals.qr += amount;
      else if (["TRANSFER", "BANK_TRANSFER"].includes(method)) {
        totals.transfer += amount;
      }
      return totals;
    },
    { gross: 0, cash: 0, card: 0, qr: 0, transfer: 0 }
  );

  const correctionTotals = (correctionResult.data || []).reduce(
    (totals, correction) => {
      const amount = numeric(correction.amount);
      const type = String(correction.correction_type || "").toUpperCase();
      if (type === "REFUND") totals.refund += amount;
      if (type === "REVERSAL") totals.reversal += amount;
      return totals;
    },
    { refund: 0, reversal: 0 }
  );

  const movementTotals = (movementResult.data || []).reduce(
    (totals, movement) => {
      const amount = numeric(movement.amount);
      const type = String(movement.movement_type || "").toUpperCase();
      if (type === "PAID_IN") totals.paidIn += amount;
      else if (type === "PAID_OUT") totals.paidOut += amount;
      else if (type === "ADJUSTMENT_IN") totals.adjustmentIn += amount;
      else if (type === "ADJUSTMENT_OUT") totals.adjustmentOut += amount;
      return totals;
    },
    { paidIn: 0, paidOut: 0, adjustmentIn: 0, adjustmentOut: 0 }
  );
  ...
  return {
    ...
    qr_total: paymentTotals.qr,
    transfer_total: paymentTotals.transfer,
    refund_total: correctionTotals.refund,
    reversal_total: correctionTotals.reversal,
    paid_in_total: movementTotals.paidIn,
    paid_out_total: movementTotals.paidOut,
    adjustment_in_total: movementTotals.adjustmentIn,
    adjustment_out_total: movementTotals.adjustmentOut,
    net_sales:
      paymentTotals.gross - correctionTotals.refund - correctionTotals.reversal,
    expected_cash:
      numeric(session.opening_cash) +
      paymentTotals.cash +
      movementTotals.paidIn +
      movementTotals.adjustmentIn -
      movementTotals.paidOut -
      movementTotals.adjustmentOut -
      correctionTotals.refund -
      correctionTotals.reversal,
  };
}
...
async function dispatchGovernanceEvent({ organizationId, eventId }) {
  ...
  try {
    ...
    const pending =
      dispatch?.success === false || Number(dispatch?.failed || 0) > 0;
    return {
      pending,
      error: pending
        ? dispatch?.failures?.[0]?.error ||
          dispatch?.error ||
          "Cash-session governance event dispatch incomplete"
        : null,
    };
  } catch (error) {
    return {
      pending: true,
      error: error?.message || "Cash-session governance event dispatch failed",
    };
  }
}
...
export async function loadCashSessions({
  ...
  const activeIndex = rows.findIndex((row) =>
    ACTIVE_STATUSES.includes(String(row.status || "").toUpperCase())
  );
  if (activeIndex >= 0) {
    rows[activeIndex] = await loadActiveSessionLiveTotals({
      organizationId,
      entityId: scope.entityId,
      applicationId: scope.applicationId,
      session: rows[activeIndex],
    });
  }

  const evidence = await loadReviewEvidence({ organizationId, rows });
  const sessions = rows.map((row) =>
    normalizeSession(row, evidence.actorNames, evidence.reviewLogs)
  );
  const activeSession =
...
export async function executeCashSession({
  ...
  if (action === "CLOSE") {
    ...
    return {
      ...
      reconciliation: {
        ...
        paid_in_total: session?.paid_in_total || 0,
        paid_out_total: session?.paid_out_total || 0,
        adjustment_in_total: session?.adjustment_in_total || 0,
        adjustment_out_total: session?.adjustment_out_total || 0,
  ...
  if (action === "APPROVE" || action === "REJECT") {
    if (!actor.can_review) {
      const error = new Error(
        "Manager or owner role required for POS cash-session review"
      );
      error.status = 403;
      throw error;
    }
    ...
    if (result.error) throwRpcError(result.error, functionName);
    const eventDispatch = await dispatchGovernanceEvent({
      organizationId,
    ...
    return {
      ...
      dispatch_pending: eventDispatch.pending,
      dispatch_error: eventDispatch.error,
      ...scope,
    };
  }
