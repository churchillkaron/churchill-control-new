...
import { canExecutePOSAction, getPOSAccessSnapshot } from "@/lib/operations/commerce/security/POSActionPolicy";
...
function actorFromAccess(access = {}) {
  ...
  return {
    ...
    can_correct: canExecutePOSAction({ access, action: "PAYMENT_CORRECTION" }),
  };
}

function requireCorrectionAuthority(actor) {
  if (actor?.can_correct && actor?.staff_id) return;
  const error = new Error("Manager or owner role required for POS payment corrections");
  error.status = 403;
  throw error;
}
