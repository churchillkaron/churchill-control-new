...
import { OPERATIONS_CAPABILITY_CATALOG } from "@/lib/operations/runtime/OperationsCapabilityCatalog";
import {
...
async function checkTable(table, organizationId) {
  ...
  const scoped = organizationId && table !== "operations_roles"
...
export async function getOperationsReadiness({ context } = {}) {
  ...
  const catalogueCount = OPERATIONS_CAPABILITY_CATALOG.length;
