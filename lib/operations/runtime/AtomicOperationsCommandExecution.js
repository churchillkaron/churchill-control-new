...
function attachExecutionMetadata(context, capability, payload = {}) {
  ...
  return {
    ...
    attributes: {
      ...(payload?.attributes || {}),
      _operations_lifecycle: capability?.lifecycle || "master",
    },
    ...(actorId
      ? {
...
export function createAtomicOperationsCommandExecution({
  ...
  async function execute({
    ...
    const commandPayload = attachExecutionMetadata(
      context,
      capability,
      payload,
    );
