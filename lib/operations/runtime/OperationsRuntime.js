...
import {
  ...
  getOperationsCommerceCapability,
} from "./CommerceCapabilityCatalog";

const OPERATIONS_RUNTIME_CAPABILITY_CATALOG = Object.freeze([
  ...OPERATIONS_CAPABILITY_CATALOG,
  ...OPERATIONS_COMMERCE_CAPABILITY_CATALOG,
]);
...
function createCapabilityRuntime({
  ...
  const repository =
    repositories?.get?.(capability.id) ||
...
export function buildOperationsRuntime(
  ...
  const capabilities = Object.freeze(
    Object.fromEntries(
      OPERATIONS_RUNTIME_CAPABILITY_CATALOG.map((capability) => [
        capability.id,
  ...
  return Object.freeze({
    ...
    catalogue: OPERATIONS_RUNTIME_CAPABILITY_CATALOG,
    capabilities,
    repositories,
    getCapability,
    requireCapability,
    ...
    pointOfSale: capabilities["point-of-sale"],
    orderCapture: capabilities["order-capture"],
    checkout: capabilities.checkout,
    receipts: capabilities.receipts,
    cashControl: capabilities["cash-control"],
    fulfillmentDispatch: capabilities["fulfillment-dispatch"],
  });
}

export function getOperationsRuntimeCapability(capabilityId) {
  return (
    getOperationsCapability(capabilityId) ||
    getOperationsCommerceCapability(capabilityId)
  );
}

export { getOperationsCapability, getOperationsCommerceCapability };
