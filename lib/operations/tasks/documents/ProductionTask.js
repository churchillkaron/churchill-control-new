...
export function createProductionTask(data = {}) {
  ...
  return {
    ...
    type: data.type ?? PRODUCTION_TASK_TYPES.GENERATE_IMAGE,
    status: data.status ?? PRODUCTION_TASK_STATUS.PLANNED,
    title: data.title ?? "",
    ...
    cost: {
      ...
      estimated: numberOrNull(data.cost?.estimated),
      actual: numberOrNull(data.cost?.actual),
      approved: data.cost?.approved ?? false,
    },
    timing: {
      estimated_seconds: numberOrNull(data.timing?.estimated_seconds),
