...
function stillImageVisualGeneration(task = {}) {
  const workflow = text(
    task.metadata?.workflow_kind ||
    task.input?.workflow_kind ||
    task.input?.requirements?.workflow_kind,
  ).toUpperCase();
  const capability = text(task.capability || task.service_code).toLowerCase();
  return workflow === "STILL" && capability.startsWith("ai.image.");
}
  ...
  async dispatch(id) {
    ...
    try {
      ...
      const providerPolicy = {
        ...buildCreativeStillProviderPolicy(task),
