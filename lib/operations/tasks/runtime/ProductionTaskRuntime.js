...
} from "@/lib/creative/stills/runtime/CreativeStillPrevisualizationRuntime";
import {
  verifyCreativeStillProductionAuthority,
} from "@/lib/creative/stills/runtime/CreativeStillProductionAuthorityRuntime";
...
function assertStudioVisualGenerationCertified(task = {}) {
  ...
  if (workflowKind === "STILL") {
    const stillPrevisualization = task.input?.requirements?.still_previsualization_blueprint || {};
    if (!verifyCreativeStillPrevisualizationBlueprint(stillPrevisualization)) {
      throw new Error("STUDIO_STILL_PREVISUALIZATION_REQUIRED");
    }
    const stillProductionAuthority = task.input?.requirements?.still_production_authority || {};
    if (!verifyCreativeStillProductionAuthority(stillProductionAuthority, stillPrevisualization)) {
      throw new Error("STUDIO_STILL_PRODUCTION_AUTHORITY_REQUIRED");
    }
