...
async function executeClaimedTask(runtime, task, workerId) {
  if (!task) throw new Error("PRODUCTION_TASK_DISPATCH_TASK_REQUIRED");
  if (task.status !== PRODUCTION_TASK_STATUS.RUNNING) {
    throw new Error("PRODUCTION_TASK_DISPATCH_CLAIM_REQUIRED");
  }
  if (!workerId || text(task.worker_id) !== text(workerId)) {
    throw new Error("PRODUCTION_TASK_DISPATCH_WORKER_MISMATCH");
  }
  if (task.output?.provider_job_id) {
    return runtime.poll(task.id);
  }
  ...
  try {
    ...
    if (result?.pending) {
      ...
      return Repository.update(task.id, {
        ...
        metadata: {
          ...(task.metadata || {}),
          dispatch_worker_id: workerId,
          dispatch_submitted_at: new Date().toISOString(),
        },
      });
    }

    return runtime.complete(task.id, {
      provider_submission: result,
    ...
    return runtime.fail(task.id, error, {
      dispatch_worker_id: workerId,
      dispatch_failed_before_completion: true,
    });
  }
}
...
export const ProductionTaskRuntime = {
  ...
  async poll(id) {
    ...
    const usageQuantity = Number(pending.usage.quantity);
    if (!Number.isFinite(usageQuantity) || usageQuantity <= 0) {
      throw new Error("RUNNING_TASK_USAGE_QUANTITY_REQUIRED");
    }
    const usageUnit = text(pending.usage.unit);
    if (!usageUnit) {
      throw new Error("RUNNING_TASK_USAGE_UNIT_REQUIRED");
    }
  ...
  async dispatchClaimed(id, { worker_id } = {}) {
    const task = await Repository.getById(id);
    return executeClaimedTask(this, task, worker_id);
  },
  ...
  async dispatch(id) {
    ...
    const workerId = `production-task:${task.id}:${Date.now()}`;
    const claimed = await Repository.claimForDispatch({
      id: task.id,
      organization_id: task.organization_id,
    ...
    if (!claimed) {
      throw new Error("PRODUCTION_TASK_DISPATCH_ALREADY_CLAIMED");
    }

    await Repository.update(task.id, {
      timing: {
        ...(claimed.timing || task.timing || {}),
        started_at:
    ...
    return executeClaimedTask(this, claimed, workerId);
  },
};
