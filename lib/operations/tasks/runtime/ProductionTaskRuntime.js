...
function assertCreateAuthorizationMetadataAbsent(input = {}) {
  const metadata = input?.metadata;
  if (!metadata || typeof metadata !== "object" || Array.isArray(metadata)) return;
  for (const key of EXECUTION_AUTHORIZATION_METADATA_KEYS) {
    if (Object.prototype.hasOwnProperty.call(metadata, key)) {
      throw new Error(`PRODUCTION_TASK_AUTHORIZATION_METADATA_CREATE_FORBIDDEN:${key}`);
    }
  }
  if (input?.input?.approved_cost_guard) {
    throw new Error("PRODUCTION_TASK_AUTHORIZATION_INPUT_CREATE_FORBIDDEN:approved_cost_guard");
  }
}
...
function assertStudioVisualGenerationCertified(task = {}) {
  ...
  const workflowKind = text(task.metadata?.workflow_kind || task.input?.requirements?.workflow_kind || task.input?.intent?.workflow_kind).toUpperCase();
  const certification =
  ...
  if (!certificationValid) throw new Error("STUDIO_VISUAL_GENERATION_FROZEN_UNTIL_100_PERCENT_CERTIFIED");

  const imageAssetAuthority = task.input?.requirements?.image_asset_authority || {};
  const assetScope = task.input?.requirements?.asset_scope || {};
  const productionImageAsset =
    text(task.capability || task.service_code).toLowerCase().startsWith("ai.image.") &&
    imageAssetAuthority.contract === "CREATIVE_IMAGE_ASSET_AUTHORITY_V1";
  if (productionImageAsset) {
    const imageAuthorityValid =
      imageAssetAuthority.production_authority_required === true &&
      imageAssetAuthority.provider_selection_owner === "SERVICE_RUNTIME" &&
      imageAssetAuthority.provider_model_selection_forbidden_in_creative === true &&
      assetScope.contract === "STRICT_SHOT_ASSET_SCOPE_V3" &&
      task.input?.generation?.provider_parameters?.production_asset_generation === true;
    if (!imageAuthorityValid) {
      throw new Error("STUDIO_IMAGE_ASSET_PRODUCTION_AUTHORITY_REQUIRED");
    }
    return;
  }

  if (workflowKind === "STILL") {
    const authority = task.input?.requirements?.art_direction_authority || task.input?.output_spec?.art_direction_authority || {};
    const dossier = authority.dossier || {};
    const stillValid =
      authority.contract === "CREATIVE_STILL_ART_DIRECTION_AUTHORITY_V1" &&
      authority.image_worker_authority === "EXECUTE_ONLY" &&
      authority.reinterpretation_allowed === false &&
      text(dossier.governing_visual_idea);
    if (!stillValid) throw new Error("STUDIO_STILL_ART_DIRECTION_AUTHORITY_REQUIRED");
    return;
  }
...
const EXECUTION_TRANSPORT_OVERLAYS = new Map();

function executionTransportOverlay(id) {
  const overlay = EXECUTION_TRANSPORT_OVERLAYS.get(String(id || "")) || null;
  if (overlay) EXECUTION_TRANSPORT_OVERLAYS.delete(String(id || ""));
  return overlay;
}

export const ProductionTaskRuntime = {
  setExecutionTransportOverlay(id, overlay = {}) {
    if (!id) throw new Error("PRODUCTION_TASK_ID_REQUIRED");
    const allowed = {};
    for (const key of ["prompt", "provider_prompt", "negative_prompt", "instructions_text"]) {
      if (typeof overlay?.[key] === "string" && overlay[key].trim()) {
        allowed[key] = overlay[key];
      }
    }
    if (!Object.keys(allowed).length) {
      throw new Error("PRODUCTION_TASK_EXECUTION_TRANSPORT_OVERLAY_REQUIRED");
    }
    EXECUTION_TRANSPORT_OVERLAYS.set(String(id), allowed);
    return { task_id: String(id), keys: Object.keys(allowed) };
  },
  ...
  async create(input = {}) {
    assertCreateAuthorizationMetadataAbsent(input);
    const task = createProductionTask(input);
    return Repository.create(task);
  },

  async createMany(inputs = []) {
    const tasks = (Array.isArray(inputs) ? inputs : []).map((input) => {
      assertCreateAuthorizationMetadataAbsent(input);
      return createProductionTask(input);
    });
    return Repository.createMany(tasks);
  },
  ...
  async dispatch(id) {
    ...
    try {
      ...
      const durableExecutionInput = executionInput(task);
      const transportOverlay = executionTransportOverlay(task.id);
