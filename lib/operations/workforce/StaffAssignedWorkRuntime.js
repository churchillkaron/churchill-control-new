...
const COMPLETION_EVIDENCE_CAPABILITY_ID = "completion-evidence";
...
function requiresLocationConfirmation(record = {}) {
  const service = serviceDelivery(record);
  const protocol = executionProtocol(record);
  const explicit = service.requires_location_confirmation ?? service.location_confirmation;
  if (explicit === true) return true;
  if (explicit === false) return false;
  if (protocol?.evidence_requirements?.location_confirmation === true) return true;
  return coordinatesFromRecord(record) !== null;
}

function workSubject(record = {}) {
  const service = serviceDelivery(record);
  const vocabulary = [
  ...
  const fallback = cleanText(service.customer_name) || cleanText(record.name) || "Assigned work";

  if (/school|education|class|student|learner|teaching/.test(vocabulary)) {
    return { label: "Class / learner", name: cleanText(service.class_name) || cleanText(service.learner_name) || fallback };
  }
  if (/automotive|mechanic|garage|vehicle|car repair|workshop/.test(vocabulary)) {
    return { label: "Vehicle / customer", name: cleanText(service.vehicle_name) || cleanText(service.vehicle_registration) || fallback };
  }
  if (/hotel|housekeep|room|guest/.test(vocabulary)) {
    return { label: "Room / guest", name: cleanText(service.room_name) || cleanText(service.guest_name) || fallback };
  }
  if (/manufactur|factory|production/.test(vocabulary)) {
    return { label: "Production work", name: cleanText(record.name) || fallback };
  }
  if (/security|guard|patrol/.test(vocabulary)) {
    return { label: "Post / incident", name: cleanText(service.location_name) || fallback };
  }
  if (/salon|spa|beauty|wellness|barber/.test(vocabulary)) {
    return { label: "Client", name: fallback };
  }
  if (/logistic|transport|delivery|courier|freight/.test(vocabulary)) {
    return { label: "Load / stop", name: cleanText(service.delivery_name) || fallback };
  }
  if (/facility|maintenance|property/.test(vocabulary)) {
    return { label: "Asset / location", name: cleanText(service.asset_name) || cleanText(service.location_name) || fallback };
  }

  return { label: "Work", name: fallback };
}
...
function normalizeJob(record = {}) {
  ...
  const service = serviceDelivery(record);
  const protocol = executionProtocol(record);
  const subject = workSubject(record);
  const requiresLocation = requiresLocationConfirmation(record);
  ...
  return {
    ...
    requiresLocationConfirmation: requiresLocation,
    navigationAvailable: Boolean(destination || destinationCoordinates),
    ...
    completionEvidenceId:
      record?.attributes?.staff_execution?.completed?.completion_evidence_id || null,
...
async function recordStaffCompletionEvidence({
  record,
  staffId,
  actorId,
  gps,
  ...
  const sourceRevision = cleanText(record.updated_at || record.created_at) || "current";
  const response = await serverOperationsApi.execute({
    capabilityId: COMPLETION_EVIDENCE_CAPABILITY_ID,
    command: "record",
    context: {
    ...
    payload: {
      name: `${record.name || "Work order"} — Completion evidence`,
      description: "Completion evidence captured from Staff My Day before work-order completion.",
      priority: record.priority || "normal",
      source_domain: record.source_domain || "operations",
      source_type: "work-order-completion",
      source_id: record.id,
      source: "staff-work-order-completion",
      source_revision: sourceRevision,
      idempotency_key: `staff-completion-evidence:${record.id}:${sourceRevision}`,
      attributes: {
        ...
        service_delivery: serviceDelivery(record),
  ...
  if (response.status >= 400 || !response.body?.ok) {
    const error = new Error(response.body?.error || "Unable to record completion evidence.");
    error.status = response.status || 500;
    throw error;
  }

  const evidenceRecord = response.body.execution?.result || null;
  if (!evidenceRecord?.id) {
    const error = new Error("Completion evidence recording returned no evidence record.");
    error.status = 500;
    throw error;
  }
...
export async function executeAssignedWorkForStaff({
  ...
  const requiresLocation = requiresLocationConfirmation(record);
  const gps = requiresLocation ? requireGps(location) : null;
  const destinationCoordinates = coordinatesFromRecord(record);
  const distance = destinationCoordinates && gps
  ...
  const protocolSubmission = command === "complete"
    ...
    : null;
  const completionEvidence = command === "complete"
    ? await recordStaffCompletionEvidence({
        record,
        staffId,
        actorId,
        gps,
  ...
  const staffExecution = {
    ...
    [command === "start" ? "started" : "completed"]: {
      ...
      ...(completionEvidence?.id
        ? { completion_evidence_id: completionEvidence.id }
        : {}),
    },
  };
