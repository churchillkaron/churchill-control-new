...
function normalizeStep(step, index) {
  ...
  return {
    ...
    ...(candidate.wait_for_external && typeof candidate.wait_for_external === "object" ? { wait_for_external: candidate.wait_for_external } : {}),
  };
}

function canonicalRunValue(value) {
  if (Array.isArray(value)) return value.map(canonicalRunValue);
  ...
  return Object.fromEntries(
    Object.keys(value)
      ...
      .map((key) => [key, canonicalRunValue(value[key])]),
  );
}

function sameRunValue(left, right) {
  return (
    JSON.stringify(canonicalRunValue(left)) ===
    JSON.stringify(canonicalRunValue(right))
  );
}
...
export function normalizeOperatorAutonomousRun(value = {}) {
  ...
  const pendingBindingDeclared = Object.prototype.hasOwnProperty.call(
    candidate,
    "pending_binding",
  );
  const pendingBinding = text(candidate.pending_binding);
...
export function operatorAutonomousRunRequiresPendingExecutionBinding(runValue) {
  const sourceRun = object(runValue);
  if (!text(sourceRun.run_id)) return false;

  const run = normalizeOperatorAutonomousRun(sourceRun);
  if (text(run.run_kind).toLowerCase() !== "single_action") return false;

  const status = text(run.status).toLowerCase();
  if (["awaiting_confirmation", "awaiting_approval"].includes(status)) {
    return text(run.current_step_id) === "requested_action";
  }
  ...
  const actionStep = run.planned_steps.find(
    (step) => text(step.id) === "requested_action",
  );
  const verificationStep = run.planned_steps.find(
    (step) => text(step.id) === "post_action_verification",
  );
  ...
  return Boolean(
    actionStep &&
      ...
      text(verificationStep.kind).toLowerCase() === "verify" &&
...
export function operatorPendingExecutionRunIdMatchesAutonomousRun(
  ...
  const sourceRun = object(runValue);
  const sourceRunId = text(sourceRun.run_id);
  if (!sourceRunId) return false;

  const runBindingDeclared = Object.prototype.hasOwnProperty.call(
    sourceRun,
    "pending_binding",
  );
  const runIdDeclared = Object.prototype.hasOwnProperty.call(pending, "run_id");
  ...
  const pendingRunId = text(pending.run_id);
...
export function operatorPendingExecutionMatchesAutonomousRun(
  ...
  const pending = object(pendingExecution);
  const sourceRun = object(runValue);
  if (!text(pending.capability_key) || !text(sourceRun.run_id)) return false;

  const run = normalizeOperatorAutonomousRun(sourceRun);
  if (text(run.run_kind).toLowerCase() !== "single_action") return false;
  if (!operatorPendingExecutionRunIdMatchesAutonomousRun(pending, run)) {
    return false;
  }

  const resumeKind = text(pending.resume_kind).toLowerCase();
  if (resumeKind && resumeKind !== "verification") return false;

  if (resumeKind === "verification") {
    if (
      text(run.status).toLowerCase() !== "blocked" ||
      text(run.current_step_id) !== "post_action_verification"
    ) {
      return false;
    }
    ...
    if (!actionStep || !verificationStep) return false;

    return (
      text(actionStep.status).toLowerCase() === "completed" &&
      text(verificationStep.kind).toLowerCase() === "verify" &&
      text(verificationStep.status).toLowerCase() === "failed" &&
      text(pending.capability_key) === text(verificationStep.capability_key) &&
      sameRunValue(object(pending.payload), object(verificationStep.payload))
    );
  }

  const runStatus = text(run.status).toLowerCase();
  if (!["awaiting_confirmation", "awaiting_approval"].includes(runStatus)) {
    return false;
  }
  if (text(run.current_step_id) !== "requested_action") return false;

  const actionStep = run.planned_steps.find(
    (step) => text(step.id) === "requested_action",
  );
  if (!actionStep || text(actionStep.kind).toLowerCase() !== "action") {
    return false;
  }
  if (
  ...
  if (runStatus === "awaiting_confirmation") {
    return (
      text(actionStep.status).toLowerCase() === "awaiting_confirmation" &&
      text(actionStep.gate).toLowerCase() === "confirmation" &&
      !text(pending.approval_request_id) &&
      !text(actionStep.approval_request_id)
    );
  }

  const approvalRequestId = text(pending.approval_request_id);
  return (
    text(actionStep.status).toLowerCase() === "awaiting_approval" &&
...
export function createOperatorMissionRun({
  ...
  const steps = list(state.steps).slice(0, MAX_RUN_STEPS).map((step, index) => {
    ...
    if (candidate.id === currentStepId && requestedStatus === "waiting_external") {
      return { ...candidate, status: "waiting_external", gate: "none" };
    }
...
export function transitionOperatorAutonomousRun(
  ...
  const requestedStepId = text(stepId);
  const targetStepId =
