import {
  ...
  normalizeOperatorPredictionVerification,
  reconcileOperatorPredictionAccountability,
} from "@/lib/operator/contracts/OperatorPredictionAccountability";

const ATTENTION_LEVELS = new Set(["clear", "watch", "important", "urgent"]);
const SIGNAL_KINDS = new Set(["risk", "opportunity", "decision", "execution", "anomaly", "watch"]);
const OUTLOOK_HORIZONS = new Set(["immediate", "near_term", "this_period", "longer_term"]);
const LEVEL_RANK = Object.freeze({ clear: 0, watch: 1, important: 2, urgent: 3 });
...
function strings(value, limit = 8, itemLimit = 240) {
  return list(value).slice(0, limit).map((item) => text(item, itemLimit)).filter(Boolean);
}
...
function normalizedSignal(value) {
  ...
  return {
    ...
    recommended_next_step: text(source.recommended_next_step, 600) || null,
    recommended_action_key: text(source.recommended_action_key || source.recommended_action?.capability_key, 240) || null,
  };
}
function normalizedOutlook(value) {
  ...
  return {
    ...
    verification: normalizeOperatorPredictionVerification(source.verification),
  };
}
...
function evidenceFingerprint(attention = {}) {
  ...
  const steps = list(evidence.steps).map((step) => ({
    ...
    capability_key: text(step?.capability_key || step?.capability?.key, 240) || null,
...
export function normalizeOperatorBusinessThesis(value) {
  const source = object(value);
  if (!Object.keys(source).length) return null;
  const signals = list(source.signals).map(normalizedSignal).filter(Boolean).slice(0, 8);
  const outlook = list(source.outlook).map(normalizedOutlook).filter(Boolean).slice(0, 4);
  ...
  return {
    ...
    signals,
    outlook,
    prediction_accountability: normalizeOperatorPredictionAccountability(source.prediction_accountability),
    recommended_next_move: text(source.recommended_next_move, 800) || null,
    recommendation_reason: text(source.recommendation_reason, 1200) || null,
    evidence_fingerprint: text(source.evidence_fingerprint, 120) || null,
...
function changeSummary({ initial, evidenceChanged, delta, recommendationChanged }) {
  ...
  const parts = [];
  if (delta.newSignals.length) parts.push(`New: ${delta.newSignals.join(", ")}.`);
  if (delta.escalatedSignals.length) parts.push(`Escalated: ${delta.escalatedSignals.join(", ")}.`);
  if (delta.resolvedSignals.length) parts.push(`No longer present: ${delta.resolvedSignals.join(", ")}.`);
  if (delta.deescalatedSignals.length) parts.push(`De-escalated: ${delta.deescalatedSignals.join(", ")}.`);
...
function interruptionPolicy({ thesisLevel, currentSignals, previous, delta, evidenceChanged }) {
  const previousLevel = attentionLevel(previous?.attention_level, "clear");
  const newUrgent = currentSignals.some((signal) => attentionLevel(signal.severity, "watch") === "urgent" && delta.newSignals.includes(signal.title));
  const escalatedUrgent = currentSignals.some((signal) => attentionLevel(signal.severity, "watch") === "urgent" && delta.escalatedSignals.includes(signal.title));
  const firstUrgent = !previous && thesisLevel === "urgent";
  const shouldInterrupt = Boolean(evidenceChanged && (newUrgent || escalatedUrgent || firstUrgent));
  ...
  const reasonSignal = currentSignals.find((signal) => attentionLevel(signal.severity, "watch") === "urgent");
  return {
    mode: shouldInterrupt ? "interrupt" : shouldSurface ? "surface" : "none",
    should_interrupt: shouldInterrupt,
    should_surface: shouldSurface,
    level: thesisLevel,
    reason: text(reasonSignal?.title, 180) || (shouldSurface ? "The evidence-backed business thesis materially changed." : null),
  };
}

export function buildOperatorBusinessThesis({ attention = {}, previousThesis = null } = {}) {
  const previous = normalizeOperatorBusinessThesis(previousThesis);
  const currentSignals = list(attention?.items).map(normalizedSignal).filter(Boolean).slice(0, 8);
  const outlook = list(attention?.outlook).map(normalizedOutlook).filter(Boolean).slice(0, 4);
  const generatedAt = text(attention?.generated_at, 80) || new Date().toISOString();
  const fingerprint = evidenceFingerprint(attention);
  const evidenceChanged = !previous?.evidence_fingerprint || previous.evidence_fingerprint !== fingerprint;
  const delta = signalDelta(currentSignals, list(previous?.signals));
  const recommendedNextMove = text(attention?.recommended_next_move, 800) || text(currentSignals[0]?.recommended_next_step, 800) || null;
  const recommendationReason = text(attention?.recommendation_reason, 1200) || text(currentSignals[0]?.why_now, 1200) || null;
  const recommendationChanged = Boolean(previous && text(previous.recommended_next_move, 800) !== text(recommendedNextMove, 800));
  const thesisLevel = strongestLevel(currentSignals, attention?.attention_level || (currentSignals.length ? "watch" : "clear"));
  ...
  const interruption = interruptionPolicy({ thesisLevel, currentSignals, previous, delta, evidenceChanged });
  interruption.dedupe_key = `${fingerprint}:${interruption.mode}:${thesisLevel}`;
  const predictionAccountability = reconcileOperatorPredictionAccountability({
    previousAccountability: previous?.prediction_accountability,
    outlook,
    attention,
    generatedAt,
  });
  ...
  return normalizeOperatorBusinessThesis({
    version: 2,
    summary: text(attention?.summary, 1200) || (currentSignals.length
    ...
    change: {
      ...
      summary: changeSummary({ initial: !previous, evidenceChanged, delta, recommendationChanged }),
...
export const OPERATOR_BUSINESS_THESIS_ATTENTION_LEVELS = Object.freeze(Array.from(ATTENTION_LEVELS));
