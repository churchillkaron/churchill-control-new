...
import {
  normalizeOperatorCodePersistenceHistory,
...
const CREATIVE_CONTEXT_CAPABILITY_KEYS = new Set([
  ...
  "creative.studio.inspectDirection",
  "creative.studio.reviseShot",
  "creative.studio.planShotSetRevision",
  "creative.studio.reviseShotSet",
  "creative.studio.restoreShotSetRevision",
...
function creativeContext(value) {
  ...
  const checkpointStatus = text(
    candidate.last_direction_checkpoint_status,
    40,
  ).toUpperCase();
  ...
  return {
    ...
    workflow_stage: text(candidate.workflow_stage, 120) || null,
    workflow_updated_at: text(candidate.workflow_updated_at, 120) || null,
    execution_locked: candidate.execution_locked === true,
    execution_locked_at: text(candidate.execution_locked_at, 120) || null,
    active_shot_id: text(candidate.active_shot_id, 180) || null,
    active_scene_id: text(candidate.active_scene_id, 180) || null,
    active_shot_number: finiteNumber(candidate.active_shot_number),
    active_shot_title: text(candidate.active_shot_title, 500) || null,
    active_revision_number: finiteNumber(candidate.active_revision_number),
    last_direction_checkpoint_id:
      text(candidate.last_direction_checkpoint_id, 180) || null,
    last_direction_checkpoint_status:
      ["APPLIED", "RESTORED", "SUPERSEDED"].includes(checkpointStatus)
        ? checkpointStatus
        : null,
    last_direction_plan_fingerprint:
      text(candidate.last_direction_plan_fingerprint, 128) || null,
...
function creativeContextFromOutput(value, sourceCapability, verifiedAt) {
  ...
  const verifiedDirection = object(output.verified_direction);
  const isAtomicRevision = sourceCapability === "creative.studio.reviseShotSet";
  const isAtomicRestore =
    sourceCapability === "creative.studio.restoreShotSetRevision";
  ...
  return creativeContext({
    ...
    production_status:
      ...
      sourceCapability === "creative.production.run"
        ? output.status
        : null,
    workflow_stage: output.workflow_stage,
    workflow_updated_at: output.workflow_updated_at,
    execution_locked: output.execution_locked === true,
    execution_locked_at: output.execution_locked_at,
    active_shot_id:
      output.selected_shot_id ||
      (sourceCapability === "creative.studio.reviseShot" ? output.shot_id : null) ||
      verifiedDirection.shot_id,
    active_scene_id: output.selected_scene_id || verifiedDirection.scene_id,
    active_shot_number:
      output.selected_shot_number ?? verifiedDirection.shot_number ?? null,
    active_shot_title:
      output.selected_shot_title || verifiedDirection.title || null,
    active_revision_number:
      output.selected_revision_number ?? verifiedDirection.revision_number ?? null,
    last_direction_checkpoint_id:
      isAtomicRevision || isAtomicRestore ? output.checkpoint_id : null,
    last_direction_checkpoint_status:
      isAtomicRevision ? "APPLIED" : isAtomicRestore ? "RESTORED" : null,
    last_direction_plan_fingerprint:
      isAtomicRevision ? output.plan_fingerprint : null,
...
function withCreativeContextProgress(summaryValue, contextValue) {
  ...
  const existing = text(summaryValue, 700)
    .split("\n")
    .map((line) => line.trim())
    .filter(
      (line) =>
  ...
  const details = [
    ...
    context.workflow_stage ? `stage=${context.workflow_stage}` : null,
    context.execution_locked ? "execution_lock=active" : null,
    context.active_shot_id ? `shot=${context.active_shot_id}` : null,
    context.active_shot_number !== null
      ? `shot_number=${context.active_shot_number}`
      : null,
    context.active_shot_title ? `shot_title=${context.active_shot_title}` : null,
    context.active_revision_number !== null
      ? `revision=${context.active_revision_number}`
      : null,
    context.last_direction_checkpoint_id
      ? `direction_checkpoint=${context.last_direction_checkpoint_id}`
      : null,
    context.last_direction_checkpoint_status
      ? `checkpoint_status=${context.last_direction_checkpoint_status}`
      : null,
  ]
    .filter(Boolean)
    .join("; ");
  const marker = `${CREATIVE_CONTEXT_PROGRESS_PREFIX} ${details}. Server-verified continuity only; checkpoint status APPLIED means a confirmed restore may be offered, while RESTORED/SUPERSEDED must not be treated as reusable undo authority. This context never authorizes generation, spend, publication, or another write.`;
...
export function mergeOperatorProjectState(
  ...
  const previous = object(previousState);
  const rawProposed = object(proposedState);
  const previousObjective = text(previous.objective, 600);
  const hasProposedObjective =
    hasOwn(rawProposed, "objective") || hasOwn(rawProposed, "goal");
  const proposedObjective = hasProposedObjective
    ? text(rawProposed.objective ?? rawProposed.goal, 600)
    : previousObjective;
  const objectiveChanged =
    hasProposedObjective && proposedObjective !== previousObjective;
  const proposed = normalizeOperatorProjectState(rawProposed, {
    previousState: previous,
  });
  const nextActivity = object(activity);
  const nextPersistenceHistory =
    operatorCodePersistenceHistoryFromExecution(nextActivity.last_execution) ||
    normalizeOperatorCodePersistenceHistory(
      nextActivity.last_verified_code_persistence,
    ) ||
    normalizeOperatorCodePersistenceHistory(
      rawProposed.last_verified_code_persistence,
    ) ||
    (objectiveChanged
      ? null
      : normalizeOperatorCodePersistenceHistory(
          previous.last_verified_code_persistence,
        ));
  ...
  if (nextPersistenceHistory) {
    merged.last_verified_code_persistence = nextPersistenceHistory;
  } else {
