...
import {
  readOperatorAuthorizationEvidence,
  sanitizedOperatorAuditPayload,
} from "@/lib/operator/governance/operatorAuthorizationEvidence";
import {
  enforceOperatorIntelligenceMutationGuard,
...
async function resolveExistingApprovalRequest({
  ...
  const requesterMatches =
    ...
    text(approvalRequest.requested_by) === text(actorId);

  if (!referenceMatches || !workflowMatches || !requesterMatches) {
    return {
      allowed: false,
      governed: true,
      approvalRequest,
...
export async function resolveOperatorExecutionApproval({
  ...
  const cognitiveExecutionBinding = enforceOperatorIntelligenceMutationGuard(capability, {
    organizationId,
    entityId,
    periodId,
    partyId,
  ...
  if (text(approvalRequestId)) {
    const resolved = await resolveExistingApprovalRequest({
      approvalRequestId,
      capability,
      organizationId,
      entityId,
...
export async function recordOperatorExecutionAudit({
  ...
  authorizationMode = null,
  authorizationOriginMode = null,
  cognitiveExecutionBinding = null,
  verificationAttestation = null,
}) {
  if (!isGovernedOperatorMode(capability?.mode)) return null;

  const runtimeEvidence = readOperatorAuthorizationEvidence(payload);
  const authorization = resolveOperatorAuthorizationProvenance({
    capability,
    source,
    approval,
    authorizationMode:
      runtimeEvidence?.mode || authorizationMode,
    authorizationOriginMode:
      runtimeEvidence?.origin_mode || authorizationOriginMode,
  });
  const auditPayload = sanitizedOperatorAuditPayload(payload);
  ...
  try {
    return await logAuditEvent({
      ...
      metadata: {
        ...
        outcome,
        cognitive_execution_binding:
          cognitiveExecutionBinding || approval?.cognitiveExecutionBinding || null,
        cognitive_verification_attestation: verificationAttestation || null,
        authorization,
        authorization_mode: authorization.mode,
        authorization_origin_mode: authorization.origin_mode,
        authorization_evidence_source: runtimeEvidence
