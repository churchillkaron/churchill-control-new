...
function inferredAuthorizationOrigin({ capability, source }) {
  ...
  // Current voice execution policy requires an affirmative conversational turn
  // before any non-read capability reaches execution.
  ...
  // Mission child audit callers do not currently expose the child manifest's
  // auto-execute/confirmation booleans. Do not invent a user confirmation.
...
export function resolveOperatorAuthorizationProvenance({
  ...
  const approvalResumed =
    approval?.resumed === true || explicitMode === "approval_resumed";
