...
function humanPendingActionDetail(pendingExecution = {}) {
  const pending = object(pendingExecution);
  const capabilityKey = text(pending.capability_key);
  if (capabilityKey !== "finance.accounts_receivable.CreateCustomerInvoice") return null;
  const payload = object(pending.payload);
  const lines = list(payload.lines);
  const currency = text(payload.currency_code) || "THB";
  const reason = text(pending.reason);
  const customerMatch = reason.match(/for\s+(.+?)\.?$/i);
  const customer = customerMatch?.[1] || "the selected customer";
  const total = lines.reduce((sum, line) => sum + Number(line?.line_total ?? ((Number(line?.quantity || 1) * Number(line?.unit_price || 0)) || 0)), 0);
  const serviceDates = lines.map((line) => text(line?.description).match(/\b20\d{2}-\d{2}-\d{2}\b/)?.[0]).filter(Boolean);
  const shortDate = (iso) => iso ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(`${iso}T12:00:00Z`)) : null;
  const dates = serviceDates.map(shortDate).filter(Boolean).join(" & ");
  return `${customer} · ${currency} ${total.toLocaleString("en-US", { maximumFractionDigits: 2 })}${dates ? ` · ${dates}` : ""} · invoice/due ${shortDate(text(payload.invoice_date)) || "not set"}. Nothing has been created yet.`;
}

function governedRunPendingPresentation(agreementState, pendingExecution) {
  const capabilityKey = text(pendingExecution?.capability_key);
  ...
  if (runStatus === "awaiting_confirmation") {
    return {
      ...
      detail: humanPendingActionDetail(pendingExecution) || `${capabilityKey} is prepared only and is waiting for your exact confirmation. Nothing has executed yet.`,
  ...
  if (runStatus === "awaiting_approval") {
    return {
      ...
      detail: `${capabilityKey} is paused at its exact approval gate. Nothing has executed yet.`,
      business_effect_verified: false,
    };
  }
  ...
  if (runStatus === "verifying") {
    return {
      tone: "pending",
      label: "Verifying",
      detail: `${capabilityKey} already executed and is waiting on its registered verification. The write will not be replayed.`,
      business_effect_verified: false,
    };
  }
  ...
  if (runStatus === "executing") {
    return {
      tone: "pending",
      label: "Executing",
      detail: `${capabilityKey} is executing under the governed run. It is not complete until its business effect is verified.`,
      business_effect_verified: false,
    };
  }
  ...
  if (runStatus === "blocked") {
    return {
      tone: "blocked",
      label: "Not completed",
      detail: text(run.blocker)
        ? `The governed run is blocked: ${text(run.blocker)} No action is assumed complete.`
        : "The governed run is blocked. No action is assumed complete.",
      business_effect_verified: false,
    };
  }
  ...
  return {
    tone: "pending",
    label: "Awaiting approval",
    detail: humanPendingActionDetail(pendingExecution) || `${capabilityKey} is prepared only. Nothing has executed yet.`,
    business_effect_verified: false,
  };
}
...
export function operatorExecutionStatePresentation(result = {}) {
  ...
  const capabilityMode = text(
    execution?.capability?.mode || execution?.mode,
  ).toLowerCase();
  ...
  if (text(pendingExecution?.capability_key)) {
    ...
    return governedRunPendingPresentation(agreementState, pendingExecution);
  }
