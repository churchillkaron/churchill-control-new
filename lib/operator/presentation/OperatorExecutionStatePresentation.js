...
function humanPendingActionDetail(pendingExecution = {}) {
  ...
  const lineSummary = lines.slice(0, 6).map((line) => `${text(line?.description) || "Invoice line"} — ${currency} ${Number(line?.line_total ?? ((Number(line?.quantity || 1) * Number(line?.unit_price || 0)) || 0)).toLocaleString("en-US", { maximumFractionDigits: 2 })}`).join("; ");
  return `Ready to create an invoice for ${customer}. ${lineSummary}. Invoice date ${text(payload.invoice_date) || "not set"}; due date ${text(payload.due_date) || "not set"}; total ${currency} ${total.toLocaleString("en-US", { maximumFractionDigits: 2 })}. Nothing has executed yet.`;
}
