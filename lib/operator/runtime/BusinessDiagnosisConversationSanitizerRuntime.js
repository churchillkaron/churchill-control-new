import { verifyBusinessDiagnosisProofAuthenticity, businessDiagnosisProofAuthenticityAcceptable, redactBusinessDiagnosisProofForClient, verifyBusinessDiagnosisProofScope } from "../../intelligence/runtime/AvantiqoBusinessDiagnosisProofAuthenticityRuntime.js";
...
export function businessDiagnosisTurnVerification(row = {}, expectedScope = {}) {
  ...
  const scopeSafe = scopeVerification.verified === true;
  const status = !metadataSafe ? verification.status : !answerSafe ? "ANSWER_MISMATCH" : !authenticitySafe ? authenticityVerification.status : !scopeSafe ? scopeVerification.status : verification.status;
  return {
    diagnosis: true,
    ...
    scope_status: scopeVerification.status,
