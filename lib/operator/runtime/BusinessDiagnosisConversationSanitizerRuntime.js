...
export function sanitizeBusinessDiagnosisConversation(rows = [], expectedScope = {}) {
  ...
  const userTurnsById = new Map(
    sourceRows.filter((row) => row?.role === "user" && text(row?.id)).map((row) => [text(row.id), row]),
  );
  ...
  for (const row of sourceRows) {
    ...
    if (pairedUserTurnId) {
      const originVerification = verifyBusinessDiagnosisOriginatingUserTurn(
        ...
        userTurnsById.get(pairedUserTurnId) || {},
      );
      if (!originVerification.verified) {
        unsafeDiagnosisUserIds.add(pairedUserTurnId);
  ...
  for (const row of chronological) {
    ...
    const verification = businessDiagnosisTurnVerification(row, expectedScope);
    if (originUnsafeDiagnosisIds.has(text(row?.id))) continue;
...
export function sanitizeBusinessDiagnosisSnapshotTurn(turn = {}, expectedScope = {}, originatingUserTurn = null) {
  ...
  const originVerification = verification.safe === true && diagnosisPairedUserTurnId(turn)
    ? verifyBusinessDiagnosisOriginatingUserTurn(diagnosis, originatingUserTurn || {})
    : { status: "ORIGIN_USER_NOT_BOUND", verified: true, legacy: true };
  const proofVerified = verification.safe === true && originVerification.verified === true;
