...
export function assessBusinessPartnerAmbiguousWriteOutcome({ result = {}, verifier = {}, expected = {}, server_scope_bound = false } = {}) {
  ...
  if (!authoritative || !exactScope) {
    return { contract: BUSINESS_PARTNER_AMBIGUOUS_WRITE_RECOVERY_CONTRACT, status: "UNCERTAIN", reason: "AUTHORITATIVE_EXACT_SCOPE_OUTCOME_REQUIRED", business_effect_outcome: evidence, mutation_replay_allowed: false, authorization_effect: "NONE" };
  }
  if (state === "COMPLETED" && evidence.business_effect_observed === true) {
    return { contract: BUSINESS_PARTNER_AMBIGUOUS_WRITE_RECOVERY_CONTRACT, status: "COMPLETED", reason: null, business_effect_verified: true, business_effect_outcome: evidence, mutation_replay_allowed: false, authorization_effect: "NONE" };
  }
  if (state === "NOT_COMPLETED" && evidence.business_effect_absent === true && evidence.safe_to_retry === true) {
    return { contract: BUSINESS_PARTNER_AMBIGUOUS_WRITE_RECOVERY_CONTRACT, status: "NOT_COMPLETED", reason: null, business_effect_verified: false, business_effect_outcome: evidence, mutation_replay_allowed: true, retry_requires_fresh_governance: true, authorization_effect: "SAME_ACTION_ONLY" };
  }
  return { contract: BUSINESS_PARTNER_AMBIGUOUS_WRITE_RECOVERY_CONTRACT, status: "UNCERTAIN", reason: "EXPLICIT_COMPLETION_STATE_NOT_PROVEN", business_effect_outcome: evidence, mutation_replay_allowed: false, authorization_effect: "NONE" };
}
