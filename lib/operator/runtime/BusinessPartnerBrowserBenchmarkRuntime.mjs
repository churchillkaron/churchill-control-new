...
function previousBenchmarkDecision(conversation = []) {
  for (const turn of [...(Array.isArray(conversation) ? conversation : [])].reverse()) {
    if (turn?.role !== 'assistant') continue;
    const raw = text(turn?.content, 12000);
    if (!raw.startsWith('{')) continue;
    try {
      const parsed = JSON.parse(raw);
      if (parsed && typeof parsed === 'object' && text(parsed.capability_or_tool, 300)) return parsed;
    } catch {}
  }
  return null;
}

function baseAgreementState(packet,message,conversation=[]){
  const prior=object(packet?.durable_conversation?.last_completed_business_action);
  const completedCapabilityKey=text(prior.capability_key,300)||'finance.accounts_receivable.CreateCustomerInvoice';
  const previous=previousBenchmarkDecision(conversation);
  const pendingCapabilityKey = affirmative(message) && previous?.confirmation_required === true && previous?.finality === 'pending_confirmation'
    ? text(previous.capability_or_tool,300)
    : '';
  const base={autonomous_run:{status:'completed',planned_steps:[{status:'completed',capability_key:completedCapabilityKey,payload:{party_id:text(prior.party_id,200)||null}}]}};
  if(affirmative(message)) base.pending_execution={capability_key:pendingCapabilityKey||completedCapabilityKey,payload:{party_id:text(prior.party_id,200)||null},requires_confirmation:true};
  return base;
}
...
function project({message,understanding,evidencePacket}){
  ...
  const localOnlyProductChange=productChange&&understanding?.local_only_constraint===true;
  const recovery=understanding?.deterministic_recovery_continuation===true;
  const finishAndVerifyRecovery=recovery&&understanding?.deterministic_finish_and_verify_continuation===true;
  ...
  const wouldExecuteNow=!clarificationRequired&&!evidenceFailure&&(['read','conversation','verify','recover'].includes(actionType)||(actionType==='write'&&(confirmedPending||!confirmationRequired))||(actionType==='product_change'&&(localOnlyProductChange||hasCoreOwnerAuthority)));
  let finality='answer';
  if(clarificationRequired) finality='clarify'; else if(evidenceFailure) finality='blocked'; else if(actionType==='recover'&&finishAndVerifyRecovery) finality='answer'; else if(actionType==='recover') finality='recover'; else if(actionType==='verify') finality='verified'; else if(actionType==='product_change'&&localOnlyProductChange) finality='local_verify'; else if(actionType==='product_change'&&!hasCoreOwnerAuthority) finality='blocked'; else if(actionType==='write'&&confirmationRequired) finality='pending_confirmation'; else if(actionType==='write') finality='execute_then_verify';
  let response;
  ...
  else if(actionType==='recover'&&finishAndVerifyRecovery) response='I would carry the mission through the remaining governed steps and independent verification, and only then report it as done.'; else if(actionType==='recover') response='I would continue from the durable verified progress, avoid replaying completed work, and recover the original mission safely before claiming completion.';
  ...
  else if(actionType==='product_change'&&localOnlyProductChange) response='I would make and verify the requested change locally only. I would not deploy it.'; else if(actionType==='product_change'&&!hasCoreOwnerAuthority) response='This is a core product change, but the supplied role has no Avantiqo core-owner authority. I would not execute or deploy it.';
  ...
  const projectedTool = actionType==='conversation' ? 'none' : text(capability?.key||understanding?.execution_domain,500)||null;
  return {understanding:text(understanding?.user_goal,1400)||text(message,1400),goal_relation:text(understanding?.goal_relation,80)||'unknown',action_type:actionType,capability_or_tool:projectedTool,needs_current_evidence:needsCurrentEvidence,confirmation_required:confirmationRequired,clarification_required:clarificationRequired,would_execute_now:wouldExecuteNow,finality,response};
}

export async function runBusinessPartnerBrowserBenchmarkTurn({organizationId,partyId,entityId,message,conversation=[]}={}){
  ...
  const understanding=await understandHumanBusinessPartnerTurn({
    ...
    conversation:[...baseConversation(evidencePacket),...(Array.isArray(conversation)?conversation.slice(-4):[])],agreementState:baseAgreementState(evidencePacket,casePrompt,conversation),projectState:baseProjectState(evidencePacket),longTermMemory:[],
