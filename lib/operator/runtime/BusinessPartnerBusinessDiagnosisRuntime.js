...
export async function resolveBusinessDiagnosisPeriods({organizationId,entityId=null,currentPeriodId=null,now=new Date(),loadPeriods=periodRows}={}){
  ...
  return {status:"PERIOD_PAIR_READY",baseline_period_id:text(baseline.id,160),current_period_id:text(current.id,160),baseline_start_date:baseline.start_date||null,current_start_date:current.start_date||null,current_status:text(current.status,80)||null,baseline_status:text(baseline.status,80)||null};
}
...
export async function runBusinessPartnerBusinessDiagnosis(options={}){
  ...
  return {
    ...
    business_diagnosis:{contract:AVANTIQO_BUSINESS_PARTNER_BUSINESS_DIAGNOSIS_CONTRACT,periods,receipt_fingerprint:receipt.receipt_fingerprint||null,final_evidence_state:receipt.final_evidence_state||null,residual_material:receipt.residual_material===true,answer_boundary_status:receipt.answer_boundary_status||null,answer_unsupported_recommendation_outcome_detected:receipt.answer_unsupported_recommendation_outcome_detected===true,authority_effect:"NONE"},
