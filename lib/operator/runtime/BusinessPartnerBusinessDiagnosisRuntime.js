...
const DIAGNOSIS_PATTERN=/\b(why|reason|cause|driver|explain|diagnose|root cause)\b.{0,180}\b(profit|margin|revenue|sales|cost|expense|cogs|food cost|labor cost|occupancy|bookings?|cash|cash flow|demand|staffing|attendance|turnover|inventory|stock|churn|retention|customer acquisition|service quality|complaints?|project delay|delivery)\b|\b(profit|margin|revenue|sales|cost|expense|cogs|food cost|labor cost|occupancy|bookings?|cash|cash flow|demand|staffing|attendance|turnover|inventory|stock|churn|retention|customer acquisition|service quality|complaints?|project delay|delivery)\b.{0,180}\b(drop|down|declin|increase|up|worse|lower|higher|change|slow|late|delay|shortage|stockout|churn)\b/i;

export function isBusinessDiagnosisQuestion(message){return DIAGNOSIS_PATTERN.test(text(message));}
...
export async function resolveBusinessDiagnosisPeriods({organizationId,entityId=null,currentPeriodId=null}={}){
  const rows=await periodRows({organizationId,entityId});
  if(!rows.length) return {status:"PERIODS_UNAVAILABLE",baseline_period_id:null,current_period_id:null};
  let current=currentPeriodId?rows.find(r=>text(r.id,160)===text(currentPeriodId,160)):rows[0];
  if(!current) return {status:"CURRENT_PERIOD_NOT_FOUND",baseline_period_id:null,current_period_id:null};
  const candidates=rows.filter(r=>text(r.id,160)!==text(current.id,160)&&text(r.start_date,40)<text(current.start_date,40));
  ...
  return {status:"PERIOD_PAIR_READY",baseline_period_id:text(baseline.id,160),current_period_id:text(current.id,160),baseline_start_date:baseline.start_date||null,current_start_date:current.start_date||null};
}

export async function runBusinessPartnerBusinessDiagnosis(options={}){
  ...
  const periods=await resolveBusinessDiagnosisPeriods({organizationId,entityId:text(options.entityId,160)||null,currentPeriodId:text(options.periodId,160)||null});
  if(periods.status!=="PERIOD_PAIR_READY") return null;
  const { BusinessIntelligenceAgentRuntime } = await import("../../intelligence/runtime/BusinessIntelligenceAgentRuntime.js");
  const result=await BusinessIntelligenceAgentRuntime.run({
    ...
    context:{baseline_period_id:periods.baseline_period_id,current_period_id:periods.current_period_id},memories:list(options.longTermMemory),actor:object(options.actor),permissions:list(options.permissions),callerRequest:options.callerRequest||null,period_id:periods.current_period_id,mode:"deep",
  });
  ...
  return {
    ...
    business_diagnosis:{contract:AVANTIQO_BUSINESS_PARTNER_BUSINESS_DIAGNOSIS_CONTRACT,periods,receipt_fingerprint:receipt.receipt_fingerprint||null,final_evidence_state:receipt.final_evidence_state||null,residual_material:receipt.residual_material===true,answer_boundary_status:receipt.answer_boundary_status||null,authority_effect:"NONE"},
...
export const BusinessPartnerBusinessDiagnosisRuntime=Object.freeze({contract:AVANTIQO_BUSINESS_PARTNER_BUSINESS_DIAGNOSIS_CONTRACT,isQuestion:isBusinessDiagnosisQuestion,resolvePeriods:resolveBusinessDiagnosisPeriods,run:runBusinessPartnerBusinessDiagnosis});
