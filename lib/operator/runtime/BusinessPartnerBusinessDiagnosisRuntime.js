import { buildBusinessDiagnosisAuditProjectionFromReceipt, verifyBusinessDiagnosisAuditProjection, businessDiagnosisProofIntegrityError } from "../../intelligence/runtime/AvantiqoBusinessDiagnosisReceiptRuntime.js";
...
export async function runBusinessPartnerBusinessDiagnosis(options={},dependencies={}){
  ...
  if(liveProof.status!=="VERIFIED") throw businessDiagnosisProofIntegrityError("BUSINESS_PARTNER_LIVE_RETURN");
  return {
    ...
    business_diagnosis:{contract:AVANTIQO_BUSINESS_PARTNER_BUSINESS_DIAGNOSIS_CONTRACT,class:receipt.diagnosis_class||diagnosisClass,business_timezone:receipt.business_timezone||businessTimezone,periods,receipt_fingerprint:receipt.receipt_fingerprint||null,audit_projection_contract:receipt.audit_projection_contract||null,audit_projection_fingerprint:receipt.audit_projection_fingerprint||null,audit_projection_verification_status:liveProof.status,audit_projection_verified:true,final_evidence_state:receipt.final_evidence_state||null,residual_material:receipt.residual_material===true,answer_boundary_status:receipt.answer_boundary_status||null,answer_unsupported_recommendation_outcome_detected:receipt.answer_unsupported_recommendation_outcome_detected===true,validated_external_context_count:list(receipt.validated_external_context_ids).length,unresolved_external_context_count:list(receipt.rejected_or_unresolved_external_contexts).length,authority_effect:"NONE"},
