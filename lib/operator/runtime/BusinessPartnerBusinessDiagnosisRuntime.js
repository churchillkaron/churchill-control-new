import { assertBusinessDiagnosisReadiness } from "../../intelligence/runtime/AvantiqoBusinessDiagnosisReadinessRuntime.js";
import { verifyBusinessDiagnosisProofAuthenticity, businessDiagnosisProofAuthenticityAcceptable } from "../../intelligence/runtime/AvantiqoBusinessDiagnosisProofAuthenticityRuntime.js";
import { buildBusinessDiagnosisAuditProjectionFromReceipt, verifyBusinessDiagnosisAuditProjection, verifyBusinessDiagnosisAnswerContent, businessDiagnosisProofIntegrityError } from "../../intelligence/runtime/AvantiqoBusinessDiagnosisReceiptRuntime.js";
...
export async function runBusinessPartnerBusinessDiagnosis(options={},dependencies={}){
  ...
  const businessTimezone=validTimezone(text(options.timezone,160))||"UTC";
  const periods=await resolveBusinessDiagnosisPeriods({organizationId,entityId:text(options.entityId,160)||null,currentPeriodId:text(options.periodId,160)||null,timezone:businessTimezone,loadPeriods:dependencies.loadPeriods||periodRows,now:dependencies.now||new Date()});
  ...
  assertBusinessDiagnosisReadiness();
  const result=await runDiagnosis({
    ...
    context:{baseline_period_id:periods.baseline_period_id,current_period_id:periods.current_period_id,baseline_period_start_date:periods.baseline_start_date,baseline_period_end_date:periods.baseline_end_date,current_period_start_date:periods.current_start_date,current_period_end_date:periods.current_end_date,business_timezone:businessTimezone,business_diagnosis_class:diagnosisClass},memories:list(options.longTermMemory),actor:object(options.actor),permissions:list(options.permissions),callerRequest:options.callerRequest||null,period_id:periods.current_period_id,mode:"deep",
  });
  ...
  const liveProjection=buildBusinessDiagnosisAuditProjectionFromReceipt(receipt);
  const liveProof=verifyBusinessDiagnosisAuditProjection({
    receipt_contract:liveProjection.receipt_contract,
    class:liveProjection.diagnosis_class,
    business_timezone:liveProjection.business_timezone,
    answer_content_fingerprint:liveProjection.answer_content_fingerprint,
    receipt_fingerprint:liveProjection.receipt_fingerprint,
    ...
    final_evidence_state:liveProjection.final_evidence_state,
    residual_material:liveProjection.residual_material,
    answer_boundary_status:liveProjection.answer_boundary_status,
    answer_unsupported_recommendation_outcome_detected:liveProjection.answer_unsupported_recommendation_outcome_detected,
    validated_external_context_count:liveProjection.validated_external_context_count,
    unresolved_external_context_count:liveProjection.unresolved_external_context_count,
    periods:liveProjection.periods,
  });
  const liveAnswerProof=verifyBusinessDiagnosisAnswerContent({audit_projection_contract:receipt.audit_projection_contract,answer_content_fingerprint:receipt.answer_content_fingerprint},response);
  const liveAuthenticity=verifyBusinessDiagnosisProofAuthenticity(receipt);
  const authenticityAcceptable=businessDiagnosisProofAuthenticityAcceptable(liveAuthenticity);
  if(liveProof.status!=="VERIFIED"||liveAnswerProof.status!=="VERIFIED"||!authenticityAcceptable) throw businessDiagnosisProofIntegrityError("BUSINESS_PARTNER_LIVE_RETURN");
  return {
    ...
    business_diagnosis:{contract:AVANTIQO_BUSINESS_PARTNER_BUSINESS_DIAGNOSIS_CONTRACT,receipt_contract:receipt.contract||null,authenticity_contract:receipt.authenticity_contract||null,authenticity_algorithm:receipt.authenticity_algorithm||null,authenticity_key_id:receipt.authenticity_key_id||null,authenticity_mac:receipt.authenticity_mac||null,authenticity_status:liveAuthenticity.status,authenticity_verified:liveAuthenticity.verified===true,class:receipt.diagnosis_class||diagnosisClass,business_timezone:receipt.business_timezone||businessTimezone,answer_content_fingerprint:receipt.answer_content_fingerprint||null,periods,receipt_fingerprint:receipt.receipt_fingerprint||null,audit_projection_contract:receipt.audit_projection_contract||null,audit_projection_fingerprint:receipt.audit_projection_fingerprint||null,audit_projection_verification_status:liveProof.status,audit_projection_verified:true,receipt_contract_verification_status:liveProof.receipt_verification_status||null,answer_content_verification_status:liveAnswerProof.status,final_evidence_state:receipt.final_evidence_state||null,residual_material:receipt.residual_material===true,answer_boundary_status:receipt.answer_boundary_status||null,answer_unsupported_recommendation_outcome_detected:receipt.answer_unsupported_recommendation_outcome_detected===true,validated_external_context_count:list(receipt.validated_external_context_ids).length,unresolved_external_context_count:list(receipt.rejected_or_unresolved_external_contexts).length,authority_effect:"NONE"},
