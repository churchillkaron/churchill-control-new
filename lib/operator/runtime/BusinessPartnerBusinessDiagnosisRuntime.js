...
export async function runBusinessPartnerBusinessDiagnosis(options={},dependencies={}){
  ...
  const result=await runDiagnosis({
    ...
    context:{baseline_period_id:periods.baseline_period_id,current_period_id:periods.current_period_id,business_diagnosis_class:diagnosisClass},memories:list(options.longTermMemory),actor:object(options.actor),permissions:list(options.permissions),callerRequest:options.callerRequest||null,period_id:periods.current_period_id,mode:"deep",
  });
  ...
  return {
    ...
    business_diagnosis:{contract:AVANTIQO_BUSINESS_PARTNER_BUSINESS_DIAGNOSIS_CONTRACT,class:diagnosisClass,periods,receipt_fingerprint:receipt.receipt_fingerprint||null,final_evidence_state:receipt.final_evidence_state||null,residual_material:receipt.residual_material===true,answer_boundary_status:receipt.answer_boundary_status||null,answer_unsupported_recommendation_outcome_detected:receipt.answer_unsupported_recommendation_outcome_detected===true,authority_effect:"NONE"},
