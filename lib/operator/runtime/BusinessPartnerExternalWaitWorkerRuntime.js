...
function activeWaitBinding(conversation, wait) {
  ...
  const resume = object(pending.payload).resume;
  ...
  return Boolean(
    text(run.run_id,180) === text(wait.run_id,180) &&
      ...
      (!text(object(resume).run_id,180) || text(object(resume).run_id,180) === text(wait.run_id,180)) &&
      text(object(resume).current_step_id,180) === text(wait.step_id,180)
  );
}
