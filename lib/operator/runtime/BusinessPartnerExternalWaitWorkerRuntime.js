...
import { findPersistedAssistantContinuationTurn, persistAssistantTurnAndConversationState } from "./IntelligenceConversationRuntime";
import { authorizeBusinessPartnerExternalBackoffWake } from "./BusinessPartnerExternalBackoffRecoveryRuntime.mjs";
...
function activeWaitBinding(conversation, wait) {
  ...
  const recovery = object(agreement.business_partner_recovery);
  const continuity = object(recovery.mission_continuity);
  const externalBackoff = object(recovery.external_backoff_recovery);
  const backoffWait =
    text(wait.event_source,120) === "system:business_partner" &&
    text(wait.event_type,180) === "BACKOFF_DUE" &&
    text(object(wait.metadata).wait_kind,80) === "EXTERNAL_BACKOFF";
  if (backoffWait) {
    const run = object(agreement.autonomous_run);
    return (
      text(continuity.contract,180) === "AVANTIQO_BUSINESS_PARTNER_MISSION_RECOVERY_V1" &&
      text(continuity.run_id,180) === text(wait.run_id,180) &&
      text(continuity.failed_step_id,180) === text(wait.step_id,180) &&
      text(externalBackoff.wait_key,128) === text(wait.wait_key,128) &&
      text(externalBackoff.status,80) === "WAITING_BACKOFF" &&
      externalBackoff.read_only_retry === true &&
      externalBackoff.mutation_replay_allowed === false &&
      text(run.run_id,180) === text(wait.run_id,180) &&
      text(run.current_step_id,180) === text(wait.step_id,180) &&
      text(run.status,80).toLowerCase() === "blocked"
    );
  }
  const run = object(agreement.autonomous_run);
...
function agreementForWaitWake(conversation, wait) {
  const agreement = object(conversation?.agreement_state);
  const recovery = object(agreement.business_partner_recovery);
  const backoff = object(recovery.external_backoff_recovery);
  if (
    text(wait.event_source,120) !== "system:business_partner" ||
    text(wait.event_type,180) !== "BACKOFF_DUE" ||
    text(backoff.wait_key,128) !== text(wait.wait_key,128)
  ) return agreement;
  const authorized = authorizeBusinessPartnerExternalBackoffWake(backoff);
  if (text(authorized.status,80) !== "RETRY_DUE" || authorized.resume_authorized !== true) return agreement;
  return {
    ...agreement,
    business_partner_recovery: {
      ...recovery,
      external_backoff_recovery: authorized,
      status: "EXTERNAL_BACKOFF_RETRY_DUE",
...
export async function processBusinessPartnerExternalWait(wait) {
  ...
  try {
    ...
    const continuationIdempotencyKey=`external-wait:${text(active.id,160)}:${text(active.event_id || active.wait_key,240)}`;
    const existingTurn=await findPersistedAssistantContinuationTurn({
      organizationId:active.organization_id, conversationId:conversation.id, partyId:active.party_id, idempotencyKey:continuationIdempotencyKey,
    });
    if (existingTurn) {
      await finish(active,claimed.token,"RESUMED",{resumed_at:new Date().toISOString(),continuation_result:{status:"ALREADY_PERSISTED",turn_id:existingTurn.id,authorization_effect:"NONE"}});
      await cancelApprovalSiblingWaits(active);
      return {success:true,skipped:true,reason:"CONTINUATION_ALREADY_PERSISTED",wait_id:active.id,conversation_id:conversation.id};
    }
    ...
    const result=await runSyntheticIntelligenceTurn({
      ...
      source:"event",
      pathname:null,
      agreementState:agreementForWaitWake(conversation, active),
...
async function promoteDueBackoffWaits({ limit = 12, now = new Date() } = {}) {
  const rows = await supabaseAdmin.from(TABLE).select("*")
    .eq("status","WAITING_EXTERNAL")
    .eq("event_source","system:business_partner")
    .eq("event_type","BACKOFF_DUE")
    .order("created_at",{ascending:true})
    .limit(Math.max(1,Math.min(Number(limit)||12,50)));
  if (rows.error) throw rows.error;
  let promoted = 0;
  for (const row of rows.data || []) {
    const notBefore = Date.parse(text(object(row.metadata).retry_not_before,80));
    if (!Number.isFinite(notBefore) || notBefore > now.getTime()) continue;
    const update = await supabaseAdmin.from(TABLE).update({
      status:"EVENT_RECEIVED",
      event_id:`backoff:${text(row.wait_key,128)}:${Number(object(row.metadata).backoff_attempt||0)}`,
      event_evidence:{backoff_due:true,authorization_effect:"NONE"},
      event_received_at:now.toISOString(),
      updated_at:now.toISOString(),
    }).eq("id",row.id).eq("status","WAITING_EXTERNAL").select("id").maybeSingle();
    if (update.error) throw update.error;
    if (update.data) promoted += 1;
  }
  return promoted;
}
...
export async function runBusinessPartnerExternalWaitBatch({limit=4}={}) {
  ...
  await promoteDueBackoffWaits({limit:max*3});
