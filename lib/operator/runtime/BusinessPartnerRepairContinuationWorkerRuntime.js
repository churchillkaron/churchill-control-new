...
export async function processBusinessPartnerRepairContinuation(candidate){
  ...
  try {
    ...
    const continuationIdempotencyKey=`repair-continuation:${text(active.id,160)}:${text(recovery.execution_key,160)}`;
    const existingTurn=await findPersistedAssistantContinuationTurn({organizationId:active.organization_id,conversationId:active.id,partyId:active.party_id,idempotencyKey:continuationIdempotencyKey});
    if (existingTurn) {
      await updateClaimedRecovery(active,claimed.token,{replay_required:false,resume_authorized:false,status:"REPAIR_CONTINUATION_ALREADY_PERSISTED",repair_continuation_turn_id:existingTurn.id});
      return {success:true,skipped:true,reason:"CONTINUATION_ALREADY_PERSISTED",conversation_id:active.id};
    }
    ...
    await persistAssistantTurnAndConversationState({organizationId:active.organization_id,conversationId:active.id,partyId:active.party_id,source:"event",content:responseText,
      ...
      execution:object(result?.execution),navigation:object(result?.navigation),agreementState:nextAgreement,projectState:nextProject,continuationIdempotencyKey});
