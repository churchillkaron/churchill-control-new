...
export async function prepareCustomerInvoiceAction({organizationId,entityId=null,message,timezone="Asia/Bangkok"}={}) {
  ...
  if(copyIntent(message)) {
    ...
    const dates=serviceDatesFromMessage(message,timezone,source.lines.length); const invoiceDate=/\b(?:today'?s?|today)\b[\s\S]{0,30}\binvoice\s+date\b|\binvoice\s+date[\s\S]{0,20}\btoday\b/i.test(message)?today:(source.invoice_date||today); const dueDate=/\bdue\s+date[\s\S]{0,20}\btoday\b|\btoday'?s?[\s\S]{0,30}\bdue\s+date\b/i.test(message)?today:(source.due_date||invoiceDate);
    const lines=source.lines.map((line,index)=>copiedLine(line,dates[index]||null));
    return {clarification_required:false,payload:{party_id:partyId,invoice_date:invoiceDate,due_date:dueDate,currency_code:source.currency_code||"THB",exchange_rate:Number(source.exchange_rate||1),lines},resolved_entities:{customer_party_id:partyId,customer_name:customerName,source_invoice_id:source.id,source_invoice_number:source.invoice_number||null},summary:`Prepared a copy of ${source.invoice_number||"the latest invoice"} for ${customerName} with ${lines.length} line${lines.length===1?"":"s"}. Confirmation is still required.`};
  }
  ...
  return {clarification_required:false,payload:{party_id:partyId,invoice_date:today,due_date:today,currency_code:currencyFromMessage(message)||"THB",exchange_rate:1,lines:[{description,quantity:1,unit_price:amount,line_total:amount}]},resolved_entities:{customer_party_id:partyId,customer_name:customerName},summary:`Prepared the requested invoice for ${customerName}. Confirmation is still required.`};
}
...
export function revisePendingCustomerInvoicePayload({ payload = {}, message, timezone = "Asia/Bangkok" } = {}) {
  ...
  return {
    ...
    dates,
    summary: `Updated the staged invoice line date${dates.length === 1 ? "" : "s"} to ${dates.join(" and ")}. The invoice is still waiting for confirmation.`,
  };
}
