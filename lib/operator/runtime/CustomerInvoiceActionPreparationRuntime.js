...
function invoiceDatePlanFromMessage(message, timezone, fallbackInvoiceDate, fallbackDueDate) {
  const source=text(message,12000);
  const lower=source.toLowerCase();
  const today=todayIso(timezone);
  const weekdayPattern="(sunday|monday|tuesday|wednesday|thursday|friday|saturday)";
  const shared = lower.match(new RegExp(`\\b${weekdayPattern}\\b[^.?!\\n]{0,50}\\b(?:due\\s+date[^.?!\\n]{0,30}invoice\\s+date|invoice\\s+date[^.?!\\n]{0,30}due\\s+date)\\b`,"i"));
  const invoiceOnly = lower.match(new RegExp(`\\b${weekdayPattern}\\b[^.?!\\n]{0,24}\\binvoice\\s+date\\b|\\binvoice\\s+date[^.?!\\n]{0,24}\\b${weekdayPattern}\\b`,"i"));
  const dueOnly = lower.match(new RegExp(`\\b${weekdayPattern}\\b[^.?!\\n]{0,24}\\bdue\\s+date\\b|\\bdue\\s+date[^.?!\\n]{0,24}\\b${weekdayPattern}\\b`,"i"));
  const extractWeekday = (match) => {
    if (!match) return null;
    const value=String(match[0]).match(/\\b(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\\b/i)?.[1]?.toLowerCase();
    return value && WEEKDAY.has(value) ? WEEKDAY.get(value) : null;
  };
  const sharedWeekday=extractWeekday(shared);
  const invoiceWeekday=sharedWeekday ?? extractWeekday(invoiceOnly);
  const dueWeekday=sharedWeekday ?? extractWeekday(dueOnly);
  const invoiceDate=invoiceWeekday==null ? (fallbackInvoiceDate||today) : nextWeekday(today,invoiceWeekday);
  const dueDate=dueWeekday==null ? (fallbackDueDate||invoiceDate) : nextWeekday(today,dueWeekday);
  return {invoiceDate,dueDate,anchorDate:invoiceDate};
}
function previousServiceDatesFromMessage(message, anchorIso, expected=2) {
  const lower=text(message,12000).toLowerCase();
  const match=lower.match(/\\b(?:previous|last)\\s+(sunday|monday|tuesday|wednesday|thursday|friday|saturday)(?:\\s*(?:,|and|&)\\s*(sunday|monday|tuesday|wednesday|thursday|friday|saturday))?/i);
  if(!match) return [];
  const names=[match[1],match[2]].filter(Boolean);
  return [...new Set(names.map((name)=>previousWeekday(anchorIso,WEEKDAY.get(name))))].slice(0,Math.max(1,expected));
}
...
export async function prepareCustomerInvoiceAction({organizationId,entityId=null,message,timezone="Asia/Bangkok"}={}) {
  ...
  if(copyIntent(message)) {
    ...
    const datePlan=invoiceDatePlanFromMessage(message,timezone,source.invoice_date||today,source.due_date||source.invoice_date||today);
    const invoiceDate=/\b(?:today'?s?|today)\b[\s\S]{0,30}\binvoice\s+date\b|\binvoice\s+date[\s\S]{0,20}\btoday\b/i.test(message)?today:datePlan.invoiceDate;
    const dueDate=/\bdue\s+date[\s\S]{0,20}\btoday\b|\btoday'?s?[\s\S]{0,30}\bdue\s+date\b/i.test(message)?today:datePlan.dueDate;
    const previousDates=previousServiceDatesFromMessage(message,invoiceDate,source.lines.length);
    const dates=previousDates.length ? previousDates : serviceDatesFromMessage(message,timezone,source.lines.length);
...
export function resolveCustomerInvoiceDatePlan({ message, timezone="Asia/Bangkok", fallbackInvoiceDate=null, fallbackDueDate=null, expectedLines=2 }={}) {
  const plan=invoiceDatePlanFromMessage(message,timezone,fallbackInvoiceDate,fallbackDueDate);
  return {
    invoice_date: plan.invoiceDate,
    due_date: plan.dueDate,
    service_dates: previousServiceDatesFromMessage(message,plan.invoiceDate,expectedLines),
  };
}
