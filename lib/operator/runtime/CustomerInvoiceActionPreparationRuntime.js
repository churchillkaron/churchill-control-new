...
function invoiceConfirmationSummary({ customerName, sourceInvoiceNumber = null, payload }) {
  ...
  const lineText = lines.map((line) => `${text(line?.description, 500)} — ${payload?.currency_code || "THB"} ${Number(line?.line_total ?? ((Number(line?.quantity || 1) * Number(line?.unit_price || 0)) || 0)).toLocaleString("en-US", { maximumFractionDigits: 2 })}`).join("; ");
  const source = sourceInvoiceNumber ? ` based on ${sourceInvoiceNumber}` : "";
  return `I'm ready to create a new invoice for ${customerName}${source}: ${lineText}. Invoice date ${payload?.invoice_date}; due date ${payload?.due_date}; total ${payload?.currency_code || "THB"} ${total.toLocaleString("en-US", { maximumFractionDigits: 2 })}.`;
}
...
export async function prepareCustomerInvoiceAction({organizationId,entityId=null,message,timezone="Asia/Bangkok"}={}) {
  ...
  if(copyIntent(message)) {
    ...
    const payload={party_id:partyId,invoice_date:invoiceDate,due_date:dueDate,currency_code:source.currency_code||"THB",exchange_rate:Number(source.exchange_rate||1),lines};
  ...
  const payload={party_id:partyId,invoice_date:today,due_date:today,currency_code:currencyFromMessage(message)||"THB",exchange_rate:1,lines:[{description,quantity:1,unit_price:amount,line_total:amount}]};
