...
export async function loadIntelligenceConversationSnapshot({
  ...
  const verifiedTurns=(turns.data||[]).map((turn)=>{
    ...
    const verification=verifyBusinessDiagnosisAuditProjection(diagnosis);
    return {...turn,evidence:{...evidence,business_diagnosis:{...diagnosis,audit_projection_verification_status:verification.status,audit_projection_verified:verification.verified===true}}};
  });
