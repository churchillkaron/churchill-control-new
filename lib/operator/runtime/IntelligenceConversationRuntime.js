...
function boundedTurns(rows = []) {
  ...
  for (const row of chronological) {
    if (row?.role === "assistant" && !diagnosisTurnIsSafeForConversation(row)) {
...
async function loadVerifiedRecentConversationTurns({ organizationId, conversationId } = {}) {
  ...
  return boundedTurns(turns.data || []);
}
...
export async function loadIntelligenceConversationSnapshot({
  ...
  const verifiedTurns=(turns.data||[]).map((turn)=>{
    const evidence=turn?.evidence&&typeof turn.evidence==="object"&&!Array.isArray(turn.evidence)?turn.evidence:{};
    const diagnosis=evidence?.business_diagnosis;
    if(!diagnosis||typeof diagnosis!=="object") return turn;
    const verification=verifyBusinessDiagnosisAuditProjection(diagnosis);
    const proofVerified=verification.status==="VERIFIED"||verification.status==="VERIFIED_LEGACY";
    const verifiedDiagnosis={...diagnosis,audit_projection_verification_status:verification.status,audit_projection_verified:verification.verified===true};
    if(proofVerified) return {...turn,evidence:{...evidence,business_diagnosis:verifiedDiagnosis}};
    return {
      ...turn,
      content:"This historical diagnosis is hidden because its proof could not be verified. No action was executed. Re-run the diagnosis to obtain a verified answer.",
      decision:{...object(turn?.decision),response_text:"This historical diagnosis is hidden because its proof could not be verified. No action was executed. Re-run the diagnosis to obtain a verified answer."},
