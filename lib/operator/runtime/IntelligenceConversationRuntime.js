...
import { boundRecentConversationTurns } from "./IntelligenceMemoryGovernorPolicy.js";
import {
  learnAdaptiveExecutionLesson,
  retireAdaptiveLessonsAfterVerifiedSuccess,
} from "./IntelligenceAdaptiveLearningRuntime";
import { learnAdaptiveConversationLesson } from "./AdaptiveConversationLearningRuntime.js";
import {
...
async function loadMissingDiagnosisVerificationUserTurns({ organizationId, conversationId, rows = [], maxSupportRows = 24 } = {}) {
  ...
  const support = await supabaseAdmin
    ...
    .select("id,role,content,decision,evidence,created_at")
    .eq("organization_id", organizationId)
    .eq("conversation_id", conversationId)
    .eq("role", "user")
    .in("id", missingIds.slice(0, Math.max(1, Number(maxSupportRows) || 24)));
  if (support.error) {
...
function reusableConversationRows(rows = []) {
  const source = Array.isArray(rows) ? rows : [];
  const supersededAssistantIds = new Set();
  const supersededUserIds = new Set();
  for (const row of source) {
    if (
      text(row?.role).toLowerCase() === "assistant" &&
      object(row?.decision).superseded_live_execution === true
    ) {
      if (text(row?.id)) supersededAssistantIds.add(text(row.id));
      const pairedUserTurnId = text(object(row?.decision).paired_user_turn_id);
      if (pairedUserTurnId) supersededUserIds.add(pairedUserTurnId);
    }
  }
  return source.filter((row) =>
    !supersededAssistantIds.has(text(row?.id)) &&
    !supersededUserIds.has(text(row?.id)),
  );
}
...
async function loadVerifiedRecentConversationTurns({ organizationId, conversationId, entityId = null, periodId = null } = {}) {
  ...
  const reusableRows = reusableConversationRows(turns.data || []);
  ...
  return sanitizeBusinessDiagnosisConversation(reusableRows, {
...
export async function persistAssistantTurnAndConversationState({
  ...
  await Promise.all([
    learnAdaptiveConversationLesson({
      organizationId,
      partyId,
      conversationId,
      evidence: object(evidence),
    }).catch((error) => {
      console.error("INTELLIGENCE_ADAPTIVE_CONVERSATION_LEARN_FAILED", {
        organizationId,
        conversationId,
        error: error?.message || String(error),
      });
...
export async function loadIntelligenceConversationSnapshot({
  ...
  const visibleTurns = (turns.data || []).slice().reverse();
  const snapshotSupportRows = await loadMissingDiagnosisVerificationUserTurns({
    organizationId,
    conversationId: conversation.data.id,
    rows: visibleTurns,
    maxSupportRows: 100,
  });
  const verifiedTurns=sanitizeBusinessDiagnosisSnapshot(visibleTurns, {
    organization_id: organizationId,
