...
function pendingAuthorizationState({
  ...
  if (resumeKind === "verification") {
    ...
    // The pending action is waiting for consent. Never claim user confirmation
    // before the run has actually advanced beyond this gate.
    mode = null;
    originMode = null;
    ...
    if (previous.requirement === "user_confirmation") {
      ...
      // A text action can reach approval without a confirmation turn only when
      // its registered capability contract allowed automatic execution.
