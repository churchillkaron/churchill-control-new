...
async function recoverPendingProjectSelection({
  ...
  const decision = object(latest.data?.decision);
  const clarification = object(decision.clarification);
  const selectionMarker = decision.project_continuity_selection_required === true;
  const expectedQuestion = text(clarification.question) === CONTINUITY_SELECTION_QUESTION;
  if (!selectionMarker && !expectedQuestion) return null;
