...
function distinctProjects(rows = []) {
  ...
  for (const row of rows) {
    ...
    if (!candidate || byIdentity.has(candidate.identity)) continue;
...
export async function recoverCrossConversationProject({
  ...
  if (!projects.length) {
    return { recovered: false, ambiguous: false, reason: "NO_ACTIVE_PROJECT" };
  }
  ...
  if (projects.length > 1) {
    return {
      ...
      projects: projects.slice(0, MAX_PROJECT_CHOICES).map((project) => ({
