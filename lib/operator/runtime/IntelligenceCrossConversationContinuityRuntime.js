import { semanticMemorySimilarity } from "./IntelligenceSemanticMemoryPolicy.js";
...
function continuityStrings(value, limit = 10, itemLimit = 600) {
  return list(value)
    .slice(0, limit)
...
function normalizeContinuityProjectState(value) {
  const source = object(value);
  if (!Object.keys(source).length) return {};
  const objective = text(source.objective ?? source.goal, 600) || null;
  const requestedStatus = text(source.status, 40).toLowerCase();
  const status = requestedStatus || (objective ? "active" : "idle");
  return {
    objective,
    status,
    success_criteria: continuityStrings(source.success_criteria),
    constraints: continuityStrings(source.constraints),
    decisions: continuityStrings(source.decisions),
    assumptions: continuityStrings(source.assumptions),
    risks: continuityStrings(source.risks),
    opportunities: continuityStrings(source.opportunities),
    completed_steps: continuityStrings(source.completed_steps),
    progress_summary: text(source.progress_summary, 1200) || null,
    next_step: text(source.next_step, 600) || null,
    ...
    recommendation_confidence: Number.isFinite(Number(source.recommendation_confidence))
      ? Math.max(0, Math.min(1, Number(source.recommendation_confidence)))
      : null,
    open_questions: continuityStrings(source.open_questions),
    blocker: text(source.blocker, 600) || null,
    last_system_snapshot: null,
    last_decision_supersession: null,
...
function hasActiveProject(projectState = {}) {
  const state = normalizeContinuityProjectState(projectState);
...
function safeRecoveredProjectState(projectState = {}) {
  const normalized = normalizeContinuityProjectState(projectState);
...
async function recoverPendingProjectSelection({
  ...
  let query = supabaseAdmin
    ...
    .select("conversation_id,decision,evidence,created_at")
  ...
  const pending = rows.find(
    (row) => !diagnosisDerivedTurn(row) && isContinuitySelectionDecision(object(row?.decision)),
  );
