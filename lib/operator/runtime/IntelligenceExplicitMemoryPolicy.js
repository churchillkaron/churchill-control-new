...
function containsSensitiveCredentialMaterial(value) {
  const source = text(value, 1400);
  if (!source) return false;

  if (/-----BEGIN (?:RSA |EC |OPENSSH |PGP )?PRIVATE KEY-----/i.test(source)) {
    return true;
  }
  if (/\b(?:seed phrase|recovery phrase|mnemonic)\b\s*(?:is|=|:|-)+\s*\S+/i.test(source)) {
    return true;
  }
  if (
    /\b(?:password|passcode|pin|api[ _-]?key|access[ _-]?key|secret|client[ _-]?secret|auth[ _-]?token|access[ _-]?token|refresh[ _-]?token|bearer[ _-]?token|credential)\b\s*(?:is|=|:|-)+\s*\S+/i.test(source)
  ) {
    return true;
  }
  if (/\b(?:sk|pk|rk)_[a-z0-9_-]{16,}\b/i.test(source)) return true;
  if (/\b(?:ghp|github_pat)_[a-z0-9_]{16,}\b/i.test(source)) return true;
...
function candidate(type, clause, marker) {
  const raw = text(clause, 1400);
  if (containsSensitiveCredentialMaterial(raw)) return null;

  const clean = cleanClause(raw);
  if (clean.length < 4 || containsSensitiveCredentialMaterial(clean)) return null;
