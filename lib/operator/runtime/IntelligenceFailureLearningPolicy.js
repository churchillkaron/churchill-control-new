...
function classifyExecutionFailure(execution = {}, rawReason = "") {
  const normalized = normalizedFailureReason(rawReason);
  const verification = object(execution?.post_action_verification);
  const actionCompleted = execution?.action_call_completed === true;
  const businessEffectVerified = execution?.business_effect_verified === true || verification?.business_effect_verified === true;
  const verificationFailed = actionCompleted && businessEffectVerified === false;

  if (verificationFailed) return "BUSINESS_OUTCOME_FAILURE";
  if (/\b(timeout|timed out|network|socket|connection|dns|econn|fetch failed|service unavailable|provider unavailable|upstream|gateway|rate limit|429|502|503|504)\b/i.test(normalized)) {
    return "TRANSPORT_RUNTIME_FAILURE";
  }
  if (/\b(unauthorized|forbidden|permission|not configured|missing configuration|missing env|missing credential|prerequisite|required field|validation|not found)\b/i.test(normalized)) {
    return "PREREQUISITE_FAILURE";
  }
  if (/\b(model|reasoning|planner|tool selection|hallucinat|invalid plan|parse|schema output)\b/i.test(normalized)) {
...
export function observeVerifiedExecutionFailure(execution = {}) {
  ...
  const failureClass = classifyExecutionFailure(execution, rawReason);
  return {
    capability_key: capability,
    raw_reason: rawReason,
    normalized_reason: normalizedReason,
    fingerprint: fingerprint(capability, `${failureClass}|${normalizedReason}`),
    execution_status: status,
    failure_class: failureClass,
    affects_capability_reliability: failureClass === "BUSINESS_OUTCOME_FAILURE",
    prerequisite_signal: failureClass === "PREREQUISITE_FAILURE",
    model_weakness_signal: failureClass === "MODEL_REASONING_FAILURE",
    transport_runtime_signal: failureClass === "TRANSPORT_RUNTIME_FAILURE",
    post_action_verification_failed: failureClass === "BUSINESS_OUTCOME_FAILURE",
  };
}
...
export function observeVerifiedExecutionSuccess(execution = {}) {
  ...
  const businessEffectVerified =
    ...
    execution?.deterministic_business_effect_verified === true;
  if (mode !== "read" && (verificationStatus !== "completed" || !businessEffectVerified)) return null;
...
export function deriveAdaptiveFailureLearning({
  ...
  return {
    ...
    blocker_metadata: {
      ...
      affects_capability_reliability: observation.affects_capability_reliability === true,
      model_weakness_signal: observation.model_weakness_signal === true,
      transport_runtime_signal: observation.transport_runtime_signal === true,
      prerequisite_signal: observation.prerequisite_signal === true,
