...
function normalizedFailureReason(value) {
  return text(value, 700)
    ...
    .replace(/\b(?:req|request|run|job|usage|task|trace)[-_ ]?id\s*[:=]\s*[^\s,;]+/gi, "$1_id=<id>")
...
export function observeVerifiedExecutionFailure(execution = {}) {
  ...
  const gateReason = rawReason.toUpperCase();

  if (status !== "failed" || !capability || !rawReason) return null;
  if (WORKFLOW_GATE_REASONS.has(gateReason)) return null;
