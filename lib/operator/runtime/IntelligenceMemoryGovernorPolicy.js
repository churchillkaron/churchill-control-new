...
export function boundRecentConversationTurns(rows = [], overrides = {}) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    const artifacts = Array.isArray(evidence.presentation_artifacts)
      ? evidence.presentation_artifacts.slice(0, 8).map((artifact) => ({
          url: text(artifact?.url),
          label: text(artifact?.label),
          mime_type: text(artifact?.mime_type),
          source_key: text(artifact?.source_key),
    ...
    const continuity = {
      ...
      ...(Object.keys(execution).length ? { execution: {
        ...
        capability_key: text(execution?.capability?.key || execution.capability_key),
      } } : {}),
    };
    const continuityBytes = Buffer.byteLength(JSON.stringify(continuity), "utf8");
    selected.push(continuity);
    usedBytes += Math.max(bytes, continuityBytes);
    usedTokens += tokens + Math.ceil(Math.max(0, continuityBytes - bytes) / 3);
  }
...
export const CANONICAL_EXPLICIT_MEMORY_SOURCE = "explicit_user_statement";
export const LEGACY_EXPLICIT_MEMORY_SOURCES = Object.freeze(["persisted_user_turn"]);
export const OPERATOR_CONSOLIDATION_SOURCES = Object.freeze([
  "operator_project_state",
  CANONICAL_EXPLICIT_MEMORY_SOURCE,
  ...LEGACY_EXPLICIT_MEMORY_SOURCES,
]);
export const OPERATOR_OBSERVABILITY_SOURCES = Object.freeze([
  ...OPERATOR_CONSOLIDATION_SOURCES,
  "adaptive_execution_learning",
  "adaptive_execution_recovery_learning",
  "adaptive_conversation_learning",
]);

const MAX_RECALL_CANDIDATES_PER_SCOPE = 100;
const MAX_RECALL_CANDIDATES_TOTAL = 200;

export function memoryRecallCandidateLimit(scopeCount = 1) {
  const scopes = Math.max(1, Math.min(3, Number(scopeCount) || 1));
  return Math.max(
    50,
    Math.min(
      MAX_RECALL_CANDIDATES_PER_SCOPE,
      Math.floor(MAX_RECALL_CANDIDATES_TOTAL / scopes),
    ),
  );
}
...
export function shouldRefreshRecallTelemetry(memory = {}, nowMs = Date.now(), minimumIntervalMs = 60 * 60 * 1000) {
  const lastRecalledAt = Date.parse(memory?.last_recalled_at || "");
  if (!Number.isFinite(lastRecalledAt) || lastRecalledAt <= 0) return true;
  return nowMs - lastRecalledAt >= Math.max(60 * 1000, Number(minimumIntervalMs) || 60 * 60 * 1000);
}
...
export const IntelligenceMemoryGovernorPolicy = Object.freeze({
  ...
  recallCandidateLimit: memoryRecallCandidateLimit,
  refreshRecallTelemetry: shouldRefreshRecallTelemetry,
  canonicalExplicitSource: CANONICAL_EXPLICIT_MEMORY_SOURCE,
  consolidationSources: OPERATOR_CONSOLIDATION_SOURCES,
  observabilitySources: OPERATOR_OBSERVABILITY_SOURCES,
