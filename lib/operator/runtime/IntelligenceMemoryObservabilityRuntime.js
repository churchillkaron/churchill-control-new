...
const OBSERVABILITY_SAMPLE_LIMIT = 500;
const OBSERVABILITY_METADATA_SAMPLE_LIMIT = 100;
...
export async function readIntelligenceMemoryObservability({ organizationId } = {}) {
  ...
  const [
    ...
    base()
      .select("memory_type,importance,recall_count,updated_at,created_at")
      .eq("active", true)
      .in("source", OPERATOR_OBSERVABILITY_SOURCES)
      .order("updated_at", { ascending: false })
      .limit(OBSERVABILITY_SAMPLE_LIMIT),
    base()
      .select("metadata")
      .eq("active", true)
      .in("source", OPERATOR_OBSERVABILITY_SOURCES)
      .order("updated_at", { ascending: false })
      .limit(OBSERVABILITY_METADATA_SAMPLE_LIMIT),
  ]);
  if (activeRows.error) throw activeRows.error;
  if (metadataRows.error) throw metadataRows.error;
  ...
  for (const row of rows(metadataRows.data)) {
  ...
  return {
    ...
    memory: {
      ...
      metadata_sampled_operator_active: rows(metadataRows.data).length,
      metadata_sample_capped: operatorActiveTotal > rows(metadataRows.data).length,
      sampled_metadata_bytes: sampledMetadataBytes,
      average_metadata_bytes: rows(metadataRows.data).length
        ? Math.round(sampledMetadataBytes / rows(metadataRows.data).length)
        : 0,
    },
    privacy: {
