...
} from "./IntelligenceMemoryGovernorPolicy.js";
import { operatorMemoryConsolidationDisposition } from "./IntelligenceMemoryTemperaturePolicy.js";
...
export function normalizeIntelligenceMemory(row) {
  ...
  return {
    ...
    temperature: intelligenceMemoryTemperature(row),
    requires_live_read: row.memory_type === "fact",
    business_effect_verified: metadata.business_effect_verified === true,
    cognitive_binding_required: metadata.cognitive_binding_required === true,
    cognitive_verification_attested:
      metadata.cognitive_verification_attested === true,
    cognitive_verification_provenance:
      object(metadata.cognitive_verification_provenance),
  };
}

function sameCognitiveExecutionScope(left = {}, right = {}) {
  const a = object(left);
  const b = object(right);
  return ["organization_id", "entity_id", "period_id", "party_id"].every(
    (key) => text(a[key], 300) === text(b[key], 300),
  );
}

function cognitiveAuditReceiptMatchesMemory(row, audit) {
  const metadata = object(row?.metadata);
  if (metadata.cognitive_binding_required !== true) return false;
  const provenance = object(metadata.cognitive_verification_provenance);
  const auditMetadata = object(audit?.metadata);
  const attestation = object(auditMetadata.cognitive_verification_attestation);
  const receiptId = text(provenance.audit_receipt_id, 300);
  if (!receiptId || text(audit?.id, 300) !== receiptId) return false;
  if (text(audit?.entity_type, 120) !== "operator_execution") return false;
  if (text(auditMetadata.source, 120) !== "AVANTIQO_OPERATOR") return false;
  if (text(auditMetadata.outcome, 120) !== "verified") return false;
  if (text(audit?.action_type, 500) !== `operator.${text(provenance.capability_key, 300)}.verified`) return false;
  if (text(attestation.contract, 160) !== text(provenance.contract, 160)) return false;
  if (text(attestation.plan_id, 300) !== text(provenance.plan_id, 300)) return false;
  if (text(attestation.step_id, 300) !== text(provenance.step_id, 300)) return false;
  if (text(attestation.capability_key, 300) !== text(provenance.capability_key, 300)) return false;
  if (text(attestation.payload_fingerprint, 128) !== text(provenance.payload_fingerprint, 128)) return false;
  if (text(attestation.verification_capability_key, 300) !== text(provenance.verification_capability_key, 300)) return false;
  if (!sameCognitiveExecutionScope(attestation.execution_scope, provenance.execution_scope)) return false;
  return attestation.business_effect_verified === true &&
    object(attestation.assertion).passed === true &&
    text(attestation.authorization_effect, 80) === "NONE";
}

async function verifiedCognitiveAuditReceipts(organization, selected = []) {
  const ids = [...new Set(selected
    .map((entry) => text(object(entry?.row?.metadata).cognitive_verification_provenance?.audit_receipt_id, 300))
    .filter(Boolean))];
  if (!ids.length) return new Set();

  const result = await supabaseAdmin
    .from("audit_logs")
    .select("id,organization_id,entity_type,action_type,metadata")
    .eq("organization_id", organization)
    .in("id", ids);

  if (result.error) {
    console.error("INTELLIGENCE_MEMORY_AUDIT_RECEIPT_VERIFY_FAILED", result.error);
    return new Set();
  }

  const byId = new Map(list(result.data).map((audit) => [text(audit.id, 300), audit]));
  return new Set(selected
    .filter((entry) => cognitiveAuditReceiptMatchesMemory(
      entry.row,
      byId.get(text(object(entry?.row?.metadata).cognitive_verification_provenance?.audit_receipt_id, 300)),
    ))
...
export async function recallIntelligenceMemory({
  ...
  const candidateLimit = memoryRecallCandidateLimit(scopes.length);
  if (telemetry && typeof telemetry === "object") {
    telemetry.recall_passes = Number(telemetry.recall_passes || 0) + 1;
    telemetry.recall_scope_count = Number(telemetry.recall_scope_count || 0) + scopes.length;
    telemetry.recall_candidate_limit = Number(telemetry.recall_candidate_limit || 0) + candidateLimit * scopes.length;
  }
  const scopeResults = await Promise.all(
    scopes.map((scope) =>
      supabaseAdmin
        .from(MEMORY_TABLE)
        .select("id,memory_scope,memory_type,subject,content,importance,confidence,active,recall_count,last_recalled_at,valid_until,superseded_by,superseded_at,forgotten_at,updated_at,created_at")
        .eq("organization_id", organization)
        .eq("active", true)
  ...
  const scopeError = scopeResults.find((result) => result.error)?.error;
  if (scopeError) throw scopeError;

  if (telemetry && typeof telemetry === "object") {
    telemetry.recall_candidate_rows = Number(telemetry.recall_candidate_rows || 0) +
      scopeResults.reduce((sum, result) => sum + list(result.data).length, 0);
  }
  ...
  if (telemetry && typeof telemetry === "object") {
    telemetry.recall_unique_candidates = Number(telemetry.recall_unique_candidates || 0) + candidatesById.size;
  }
  ...
  const selectedIds = selected.map((entry) => entry.row.id).filter(Boolean);
  let hydratedSelected = selected;
  if (selectedIds.length) {
    ...
    const metadataById = new Map(
      list(metadataResult.data).map((row) => [row.id, object(row.metadata)]),
    );
    if (telemetry && typeof telemetry === "object") {
      telemetry.recall_metadata_rows_hydrated =
        Number(telemetry.recall_metadata_rows_hydrated || 0) + list(metadataResult.data).length;
    }
    hydratedSelected = selected.map((entry) => ({
      ...entry,
      row: {
        ...entry.row,
        metadata: metadataById.get(entry.row.id) || {},
      },
    }));
  }

  const recallTelemetryRows = hydratedSelected
    .map((entry) => entry.row)
    .filter((row) => row?.id && shouldRefreshRecallTelemetry(row))
    .slice(0, 3);
  if (telemetry && typeof telemetry === "object") {
    telemetry.recall_selected_rows = Number(telemetry.recall_selected_rows || 0) + selectedIds.length;
    telemetry.recall_telemetry_writes = Number(telemetry.recall_telemetry_writes || 0) + recallTelemetryRows.length;
  }
  if (recallTelemetryRows.length) {
    const recalledAt = new Date().toISOString();
    Promise.all(
      recallTelemetryRows.map(async (row) => {
        const nextCount = Number(row?.recall_count || 0) + 1;
        await supabaseAdmin
          .from(MEMORY_TABLE)
  ...
  const verifiedAuditReceiptMemoryIds =
    await verifiedCognitiveAuditReceipts(organization, hydratedSelected);

  return hydratedSelected.map((entry) => ({
    ...normalizeIntelligenceMemory(entry.row),
    cognitive_audit_receipt_verified:
      verifiedAuditReceiptMemoryIds.has(entry.row.id),
...
function executionOutcomeCandidates(previousState = {}, nextState = {}) {
  ...
  const verificationState = executionMemoryVerificationState(execution);
  const output = [];

  if (
    status === "completed" &&
    capabilityMode !== "read" &&
    shouldLearnCompletedExecutionMemory(execution)
  ) {
    output.push(candidate(
      ...
      `Executed ${capabilityKey} successfully and verified the business effect.`,
      {
        ...
        metadata: {
          ...
          cognitive_binding_required:
            verificationState.cognitive_binding_required === true,
          cognitive_verification_attested:
            verificationState.cognitive_verification_attested === true,
          cognitive_verification_provenance:
            verificationState.cognitive_verification_provenance || null,
        },
      },
    ));
  }
  ...
  if (status === "completed" && capabilityMode !== "read") {
    if (verificationStatus && !verificationState.business_effect_verified) {
...
export async function learnProjectStateMemories({
  ...
  const candidateResults = projectMemoryCandidates(previous, next);
  const candidates = candidateResults
    .map((item) => ({ item, admission: memoryWriteAdmission(item) }))
    .filter(({ admission }) => admission.admit)
    .map(({ item, admission }) => ({ ...item, ttlDays: admission.ttlDays ?? item.ttlDays }));
  ...
  const candidateKeys = [...new Set(rows.map((row) => row.memory_key))];
  const candidateScopes = [...new Set(rows.map((row) => row.memory_scope))];
  const existingExact = await supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id,memory_scope,memory_key,memory_type,subject,active,valid_until,durability:metadata->>durability")
    .eq("organization_id", organization)
    .in("memory_scope", candidateScopes)
    .in("memory_key", candidateKeys);
  if (existingExact.error) throw existingExact.error;

  const exactKey = (row) => `${row.memory_scope}:${row.memory_key}`;
  const existingByKey = new Map(
    list(existingExact.data).map((row) => [exactKey(row), row]),
  );
  const repairRows = rows
    .map((row) => ({ row, existing: existingByKey.get(exactKey(row)) }))
    .filter(({ row, existing }) => (
      existing?.active === true &&
      DURABLE_MEMORY_TYPES.has(text(row.memory_type, 80)) &&
      text(row?.metadata?.durability, 80) === "durable" &&
      Boolean(existing.valid_until)
    ));
  const repairIds = repairRows.map(({ existing }) => existing.id).filter(Boolean);
  if (repairIds.length) {
  ...
  const reusedRows = rows
    .map((row) => existingByKey.get(exactKey(row)))
    .filter((row) => row?.active === true);
  const rowsToWrite = rows.filter(
    (row) => existingByKey.get(exactKey(row))?.active !== true,
  );
  ...
  if (text(previous.objective) && text(next.objective) && text(previous.objective) !== text(next.objective)) {
    ...
    const newGoal = effectiveRows.find((row) => row.memory_key === newGoalKey);
    await retireMemoryByKey({
      organization,
      scope: partyScope,
      type: "goal",
  ...
  if (decisionSupersession) {
    ...
    const replacementMemory = effectiveRows.find(
  ...
  if (
    ...
    shouldRetireExecutionBlockerMemory(execution)
  ) {
    const scopes = ["organization"];
...
async function repairExpiredDurableOperatorMemory({ organization, scopes, limit = 100 } = {}) {
  const candidates = await supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id,memory_type,durability:metadata->>durability")
    .eq("organization_id", organization)
    .eq("active", true)
    .in("memory_scope", scopes)
    .eq("source", "operator_project_state")
    .not("valid_until", "is", null)
    .lt("valid_until", new Date().toISOString())
    .in("memory_type", [...DURABLE_MEMORY_TYPES])
    .limit(Math.max(1, Math.min(100, Number(limit) || 100)));
  if (candidates.error) throw candidates.error;

  const ids = list(candidates.data)
    .filter((row) => text(row?.durability, 80) === "durable")
    .map((row) => row.id)
    .filter(Boolean);
  if (!ids.length) return { scanned: list(candidates.data).length, repaired: 0 };
  ...
  const repaired = await supabaseAdmin
    ...
    .select("id");
  if (repaired.error) throw repaired.error;
  return { scanned: list(candidates.data).length, repaired: list(repaired.data).length };
}

async function purgeArchivedTransientMemory({ organization, scopes, limit = 100 } = {}) {
  ...
  const candidates = await supabaseAdmin
    ...
    .in("memory_scope", scopes)
    .eq("source", "operator_project_state")
    .in("memory_type", ["completed_step", "blocker", "fact"])
  ...
  const ids = list(candidates.data)
    ...
    .map((row) => row.id)
    .filter(Boolean);
  if (!ids.length) return { scanned: list(candidates.data).length, purged: 0 };
  ...
  return { scanned: list(candidates.data).length, purged: list(deleted.data).length };
}

export async function consolidateOperatorMemory({ organizationId, partyId = null, entityId = null, limit = 200 } = {}) {
  const organization = text(organizationId, 120);
  if (!organization) throw new Error("INTELLIGENCE_MEMORY_ORGANIZATION_REQUIRED");
  const activeWorkingSetTarget = 500;
  const scopes = ["organization"];
  if (partyId) scopes.push(memoryScope({ partyId, scope: "party" }));
  if (entityId) scopes.push(memoryScope({ entityId, scope: "entity" }));
  const durableRepair = await repairExpiredDurableOperatorMemory({
    organization,
    scopes,
  }).catch((error) => {
    console.error("INTELLIGENCE_MEMORY_DURABLE_TTL_REPAIR_FAILED", error);
  ...
  const countResult = await supabaseAdmin
    ...
    .eq("active", true)
    .in("memory_scope", scopes)
    .in("source", OPERATOR_CONSOLIDATION_SOURCES);
  if (countResult.error) throw countResult.error;
  const activeCount = Number(countResult.count || 0);
  const underPressure = activeCount > activeWorkingSetTarget;
  const coldCutoff = new Date(Date.now() - 120 * 24 * 60 * 60 * 1000).toISOString();
  let candidateQuery = supabaseAdmin
    .from(MEMORY_TABLE)
    .select("id,memory_scope,memory_type,importance,recall_count,source,durability:metadata->>durability,updated_at,created_at")
    .eq("organization_id", organization)
    .eq("active", true)
    .in("memory_scope", scopes)
    .in("source", OPERATOR_CONSOLIDATION_SOURCES)
  ...
  if (!underPressure) {
    candidateQuery = candidateQuery
      .lt("importance", 0.6)
      .lte("recall_count", 1)
      .lte("updated_at", coldCutoff);
  }
  const result = await candidateQuery;
  if (result.error) throw result.error;
  const consolidationRows = list(result.data).map((row) => ({
    ...row,
    metadata: { durability: row?.durability || null },
  }));
  const policyArchiveIds = consolidationRows
    .filter((row) => operatorMemoryConsolidationDisposition(row).disposition === "ARCHIVE")
    .map((row) => row.id)
    .filter(Boolean);
  const overflow = underPressure
    ? Math.max(0, activeCount - activeWorkingSetTarget - policyArchiveIds.length)
    : 0;
  const overflowArchiveIds = consolidationRows
    .filter((row) => !policyArchiveIds.includes(row.id))
    .filter((row) => text(row.memory_type, 80) === "completed_step")
    .filter((row) => text(row?.durability, 80) !== "durable")
    .slice(0, overflow)
    .map((row) => row.id)
    .filter(Boolean);
  const archiveIds = [...new Set([...policyArchiveIds, ...overflowArchiveIds])];
  if (!archiveIds.length) return {
    scanned: consolidationRows.length,
    archived: 0,
    active_count: activeCount,
    working_set_target: activeWorkingSetTarget,
    pressure_scan: underPressure,
    durable_repair_scanned: durableRepair.scanned,
    durable_repaired: durableRepair.repaired,
  };
  const now = new Date().toISOString();
  ...
  const archivedCount = list(archived.data).length;
  const purge = archivedCount > 0
    ? await purgeArchivedTransientMemory({ organization, scopes }).catch((error) => {
        console.error("INTELLIGENCE_MEMORY_TRANSIENT_PURGE_FAILED", error);
  ...
  return {
    ...
    active_count: activeCount,
    working_set_target: activeWorkingSetTarget,
    pressure_archived: overflowArchiveIds.length,
    durable_repair_scanned: durableRepair.scanned,
    durable_repaired: durableRepair.repaired,
...
export function boundedLongTermMemory(memories = []) {
  return boundLongTermMemoryContext(list(memories).slice(0, MAX_RECALL)).map((memory) => ({
    ...
    semantic_relevance: Number(memory.semantic_relevance || 0),
    semantic_mode: text(memory.semantic_mode, 80) || null,
    valid_until: memory.valid_until || null,
    freshness: text(memory.freshness, 40) || null,
    temperature: text(memory.temperature, 20) || null,
    requires_live_read: memory.requires_live_read === true,
    business_effect_verified: memory.business_effect_verified === true,
    cognitive_binding_required: memory.cognitive_binding_required === true,
    cognitive_verification_attested:
      memory.cognitive_verification_attested === true,
    cognitive_verification_provenance:
      object(memory.cognitive_verification_provenance),
    cognitive_audit_receipt_verified:
      memory.cognitive_audit_receipt_verified === true,
    historical_business_effect: memory.historical_business_effect === true,
    current_state_authority: memory.current_state_authority === true,
    requires_live_read_for_current_state:
      memory.requires_live_read_for_current_state === true,
  }));
}
