...
const MAX_RECALL = 12;
const MAX_CANDIDATES_PER_SCOPE = 100;
...
export async function recallIntelligenceMemory({
  ...
  const scopeResults = await Promise.all(
    scopes.map((scope) =>
      supabaseAdmin
        .from(MEMORY_TABLE)
        .select("id,memory_scope,memory_type,subject,content,importance,confidence,active,recall_count,last_recalled_at,valid_until,superseded_by,superseded_at,forgotten_at,metadata,updated_at,created_at")
        .eq("organization_id", organization)
        .eq("active", true)
        .eq("memory_scope", scope)
        .order("importance", { ascending: false })
        .order("updated_at", { ascending: false })
        .limit(MAX_CANDIDATES_PER_SCOPE),
    ),
  );
  ...
  const recallTelemetryRows = selected
    .map((entry) => entry.row)
  ...
  const verifiedAuditReceiptMemoryIds =
    await verifiedCognitiveAuditReceipts(organization, selected);
...
export async function consolidateOperatorMemory({ organizationId, partyId = null, entityId = null, limit = 200 } = {}) {
  ...
  const countResult = await supabaseAdmin
    ...
    .eq("active", true)
    .in("memory_scope", scopes)
    .in("source", ["operator_project_state", "explicit_user_statement"]);
