...
export async function consolidateOperatorMemory({ organizationId, partyId = null, entityId = null, limit = 200 } = {}) {
  ...
  if (activeCount <= activeWorkingSetTarget) {
    return {
      scanned: 0,
      archived: 0,
      active_count: activeCount,
      working_set_target: activeWorkingSetTarget,
      pressure_scan_skipped: true,
    };
  }
  const result = await supabaseAdmin
    .from(MEMORY_TABLE)
  ...
  const policyArchiveIds = consolidationRows
    ...
    .map((row) => row.id)
    .filter(Boolean);
  const overflow = Math.max(0, activeCount - activeWorkingSetTarget - policyArchiveIds.length);
  ...
  if (!archiveIds.length) return { scanned: list(result.data).length, archived: 0, active_count: activeCount, working_set_target: activeWorkingSetTarget };
  const now = new Date().toISOString();
  ...
  return {
    scanned: list(result.data).length,
