...
import { boundLongTermMemoryContext } from "./IntelligenceMemoryGovernorPolicy.js";
...
export async function consolidateOperatorMemory({ organizationId, partyId = null, entityId = null, limit = 200 } = {}) {
  ...
  const archiveIds = list(result.data)
    .filter((row) => operatorMemoryConsolidationDisposition(row).disposition === "ARCHIVE")
    .map((row) => row.id)
    .filter(Boolean);
  if (!archiveIds.length) return { scanned: list(result.data).length, archived: 0 };
  const now = new Date().toISOString();
  ...
  return { scanned: list(result.data).length, archived: list(archived.data).length };
}
