...
export function summarizeIntelligenceUsage(rows = []) {
  ...
  for (const row of list(rows)) {
    ...
    if (runtimeInfrastructure && pricingInfrastructure) {
      summary.pricing_basis_observed_calls += 1;
      if (runtimeInfrastructure.toLowerCase() !== pricingInfrastructure.toLowerCase()) summary.pricing_basis_drift_calls += 1;
    }
