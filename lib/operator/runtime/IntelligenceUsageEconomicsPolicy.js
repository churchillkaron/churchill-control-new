...
export function summarizeIntelligenceUsage(rows = []) {
  ...
  for (const row of list(rows)) {
    ...
    if (has(metadata, "cached_input_tokens") || has(providerUsage, "cached_input_tokens") || has(resultUsage, "cached_input_tokens")) summary.cache_observed_calls += 1;
    if (cachedInputTokens > 0) summary.cache_hit_calls += 1;
