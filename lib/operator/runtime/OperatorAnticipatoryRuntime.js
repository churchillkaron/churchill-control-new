...
import {
  buildBusinessPartnerProductOpportunityGoal,
} from "@/lib/platform/capabilities/createBusinessPartnerProductOpportunityCapability";
import {
...
const PRODUCT_OPPORTUNITY_KEY = "platform.business_partner_product_opportunities.read";
...
function orderedCandidateReads(capabilities, context, organizationalContext) {
  ...
  const output = [];
  const seen = new Set();
  const productOpportunity = safe.find((capability) => text(capability?.key) === PRODUCT_OPPORTUNITY_KEY);
...
function productOpportunityPattern(readResult, evidenceRefs = []) {
  const refs = new Set(list(evidenceRefs).map(text).filter(Boolean));
  const step = list(readResult?.steps).find((entry) =>
    refs.has(text(entry?.id)) &&
    text(entry?.capability_key) === PRODUCT_OPPORTUNITY_KEY &&
    text(entry?.status) === "completed"
  );
  if (!step) return null;
  const sample = list(object(step.evidence).sample);
  for (const item of sample) {
    const pattern = object(item?.product_evidence);
    if (
      Number(pattern.occurrences || 0) >= 3 &&
      Number(pattern.distinct_organization_count || 0) >= 2 &&
      text(pattern.mission_family, 160) &&
      text(pattern.friction_class, 120)
    ) return pattern;
  }
  return null;
}

function enrichProductOpportunityRecommendations(items, readResult) {
  return list(items).map((item) => {
    if (text(item?.recommended_action?.capability_key) !== "platform.product_engineering_portfolio.execute") return item;
    const pattern = productOpportunityPattern(readResult, item.evidence_refs);
    if (!pattern) return { ...item, recommended_action: null };
    return {
      ...item,
      recommended_action: {
        ...object(item.recommended_action),
        payload: { business_goal: buildBusinessPartnerProductOpportunityGoal(pattern) },
        payload_source: "SERVER_AUTHENTICATED_PRODUCT_FRICTION_PATTERN",
        model_authored_payload: false,
...
`.trim();
  ...
  let execution = await ServiceExecutionRuntime.execute({
    ...
    metadata: {
      ...
      production_certified_routing_required: true,
  ...
  execution = await settleOperatorIntelligenceExecution({
    ...
    service_id: "ai.text.generate",
    execution_lane: "fast",
    metadata: { module: "OPERATOR", operation: "SYNTHESIZE_ATTENTION_SETTLEMENT" },
  });
  ...
  return {
    ...
    items: enrichProductOpportunityRecommendations(
      normalizeAttentionItems(parsed, completedSteps, actions),
      readResult,
    ),
