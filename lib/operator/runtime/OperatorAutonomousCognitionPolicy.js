...
export function shouldDeepenAutonomousThesis({
  budgetDecision,
  previewThesis,
  previousThesis = null,
} = {}) {
  if (budgetDecision?.allowed !== true) {
    return { allowed: false, reason: budgetDecision?.reason || "COGNITION_BUDGET_BLOCKED" };
  }
  if (budgetDecision?.policy?.deep_reasoning_on_change === false) {
    return { allowed: false, reason: "DEEP_REASONING_DISABLED" };
  }

  const preview = object(previewThesis);
  const previous = object(previousThesis);
  if (!Object.keys(previous).length) {
    return { allowed: true, reason: "BOOTSTRAP_BUSINESS_THESIS" };
  }
  if (preview?.change?.evidence_changed !== true) {
    return { allowed: false, reason: "EVIDENCE_UNCHANGED" };
  }
  if (preview?.change?.recommendation_changed === true) {
    return { allowed: true, reason: "RECOMMENDATION_CHANGED" };
  }
  if (
    ["important", "urgent"].includes(text(previous?.attention_level, 40).toLowerCase())
  ) {
    return { allowed: true, reason: "HIGH_ATTENTION_EVIDENCE_CHANGED" };
  }
  if (list(preview?.change?.new_signals).length) {
    return { allowed: true, reason: "NEW_EVIDENCE_SIGNAL" };
  }

  return { allowed: false, reason: "DETERMINISTIC_THESIS_SUFFICIENT" };
}
