...
function explicitOrganizationalContextRead(message, capabilities = []) {
  ...
  const asksContext = /\b(organization|organisation|company|business|legal entity|entity)\b/.test(utterance);
  const asksCurrent = /\b(current|currently|right now|live|verify|verified|context)\b/.test(utterance);
  if (!asksContext || !asksCurrent) return null;
