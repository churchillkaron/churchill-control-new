...
function explicitOrganizationalContextRead(message, capabilities = []) {
  const utterance = normalizedPhrase(message);
  if (!utterance) return null;

  const asksContext = [
    /\b(organization|organisation|company|business) context\b/,
    /\blegal entity context\b/,
    /\bwhat (is|are) (the )?(current )?(organization|organisation|company|business|legal entity)\b/,
    /\bwhich (organization|organisation|company|business|legal entity)\b/,
    /\bwhat (organization|organisation|company|business|legal entity) (are|am) (we|i) (using|in)\b/,
    /\bwhat do we know about (this |the )?(organization|organisation|company|business)\b/,
  ].some((pattern) => pattern.test(utterance));
  if (!asksContext) return null;

  return list(capabilities).find((capability) =>
    text(capability?.key) === ORGANIZATIONAL_CONTEXT_KEY &&
...
function explicitReadSubjectMatch(message, capabilities = []) {
  const utterance = normalizedPhrase(message);
  if (!utterance) return null;

  const matches = list(capabilities).filter((capability) => {
    if (normalizedMode(capability?.mode) !== "read") return false;
    ...
    const subject = normalizedPhrase(
      text(capability?.capability || text(capability?.key).split(".")[1]).replace(/[_-]+/g, " "),
    );
    if (!subject || subject.length < 4) return false;
    return utterance.includes(subject);
  });
...
export function resolveOperatorBusinessDataReflex({
  ...
  const organizationalContextRead = explicitOrganizationalContextRead(message, capabilities);
  if (organizationalContextRead) {
    return {
      matched: true,
      execute: true,
      confidence: 1,
      capability: organizationalContextRead,
      capability_key: organizationalContextRead.key,
      payload: { focus: text(message).slice(0, 700) },
      reason: `Registry-resolved explicit organizational context read: ${organizationalContextRead.key}`,
      provider_evidence: {
        provider: "avantiqo-local",
        model: "registry-organizational-context-reflex-v1",
        usage_id: null,
      },
    };
  }

  const explicitRead = explicitReadSubjectMatch(message, capabilities);
  if (explicitRead) {
    if (entityRequired(explicitRead, entityId)) {
      return {
        matched: true,
        execute: false,
    ...
    return {
      ...
      execute: true,
      confidence: 0.995,
      capability: explicitRead,
      capability_key: explicitRead.key,
      payload: {},
      reason: `Registry-resolved explicit read subject: ${explicitRead.key}`,
      provider_evidence: {
        provider: "avantiqo-local",
        model: "registry-explicit-read-subject-v1",
        usage_id: null,
      },
    };
  }
