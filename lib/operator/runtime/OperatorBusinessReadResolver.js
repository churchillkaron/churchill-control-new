...
export function prioritizeOperatorBusinessReads({
  ...
  if (resolution) {
    // Reserve one reasoning-candidate slot for the governed read-chain control.
    // Without this reservation, a full ranked read set can consume the entire
    // budget and make multi-read comparison/evidence-first composition
    // impossible even though the reasoning contract explicitly requires it.
    const directReadLimit = chain && boundedLimit > 1 ? boundedLimit - 1 : boundedLimit;
    appendUnique(selected, seen, resolution.capabilities, directReadLimit);
    appendUnique(selected, seen, fallback, directReadLimit);
    if (chain) {
