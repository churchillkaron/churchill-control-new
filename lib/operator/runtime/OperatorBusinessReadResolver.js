...
function schemaVocabulary(schema = {}) {
...
function capabilityVocabulary(capability = {}) {
...
function readRelevance(capability, message) {
  if (normalized(capability?.mode) !== "read") return null;
...
export function rankOperatorReadCapabilities({
  ...
  const ranked = list(capabilities)
    .map((capability, index) => ({
      index,
      relevance: readRelevance(capability, message),
    }))
...
export function resolveOperatorBusinessRead({
  ...
  const ranked = rankOperatorReadCapabilities({
    message,
    capabilities,
    limit,
  });
