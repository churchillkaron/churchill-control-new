...
async function buildCatalog() {
  ...
  const loaded = await boundedParallelMap(candidates, async (candidate) => {
    ...
    return {
      ...
      requires_confirmation: manifest?.operatorRequiresConfirmation === true || manifest?.operator_requires_confirmation === true || mode === "approve" || ["high", "critical"].includes(normalizeRisk(manifest?.risk || manifest?.riskLevel || manifest?.risk_level)),
