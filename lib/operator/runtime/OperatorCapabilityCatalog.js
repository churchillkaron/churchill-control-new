...
async function buildCatalog() {
  ...
  return catalog.map((item) => {
    const explicit = normalizeOperatorVerificationDeclaration(item.declared_operator_verification, item, catalog);
    const operatorVerification = explicit || inferredVerificationDeclaration(item, catalog);
    const { declared_operator_verification: _rawDeclaration, ...publicItem } = item;
    return { ...publicItem, operator_verification: operatorVerification };
  });
}
