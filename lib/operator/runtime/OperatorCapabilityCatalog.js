import { inferredVerificationDeclaration, normalizeOperatorVerificationDeclaration } from "./OperatorCapabilityVerificationDeclaration.mjs";
import { normalizeOperatorExecutionBoundary } from "./OperatorExecutionBoundaryRuntime.mjs";
import { normalizeOperatorAmbiguousWriteRecovery } from "./OperatorAmbiguousWriteRecoveryRuntime.mjs";
import {
  getDomainRuntime,
...
async function buildCatalog() {
  ...
  const loaded = await boundedParallelMap(candidates, async (candidate) => {
    ...
    const operatorFlagDeclared =
      manifest?.operatorEnabled !== undefined ||
      manifest?.operator_enabled !== undefined;
    const operatorEnabled = operatorFlagDeclared
      ? manifest?.operatorEnabled === true || manifest?.operator_enabled === true
      : aiEnabled;
    ...
    return {
      ...
      declared_operator_verification: manifest?.operatorVerification || manifest?.operator_verification || null,
      declared_operator_execution_boundary: manifest?.operatorExecutionBoundary || manifest?.operator_execution_boundary || null,
      declared_ambiguous_write_recovery: manifest?.operatorAmbiguousWriteRecovery || manifest?.operator_ambiguous_write_recovery || null,
    };
  });

  const catalog = loaded.filter(Boolean).sort((a, b) => a.key.localeCompare(b.key));
  return catalog.map((item) => {
    const hasExplicitVerification = item.declared_operator_verification !== null && item.declared_operator_verification !== undefined;
    const explicit = hasExplicitVerification
      ? normalizeOperatorVerificationDeclaration(item.declared_operator_verification, item, catalog)
      : null;
    const operatorVerification = hasExplicitVerification
      ? explicit
      : inferredVerificationDeclaration(item, catalog);
    const operatorExecutionBoundary = normalizeOperatorExecutionBoundary(item.declared_operator_execution_boundary);
    const declaredAmbiguousWriteRecovery = normalizeOperatorAmbiguousWriteRecovery(item.declared_ambiguous_write_recovery);
    const secretaryFailClosedRecovery = item.domain === "platform" && (item.capability === "secretary" || item.capability.startsWith("secretary_")) && !["read", "navigate"].includes(item.mode) ? "UNCERTAIN_NO_REPLAY" : null;
    const ambiguousWriteRecovery = declaredAmbiguousWriteRecovery || secretaryFailClosedRecovery;
    const { declared_operator_verification: _rawDeclaration, declared_operator_execution_boundary: _rawExecutionBoundary, declared_ambiguous_write_recovery: _rawRecovery, ...publicItem } = item;
    return {
      ...publicItem,
      operator_verification: operatorVerification,
      operator_execution_boundary: operatorExecutionBoundary,
      operator_execution_boundary_status: item.declared_operator_execution_boundary ? operatorExecutionBoundary ? "EXPLICIT_VALID" : "INVALID_DECLARATION_BLOCKED" : "NONE",
      ambiguous_write_recovery: ambiguousWriteRecovery,
      ambiguous_write_recovery_status: item.declared_ambiguous_write_recovery
        ? declaredAmbiguousWriteRecovery ? "EXPLICIT_VALID" : "INVALID_DECLARATION_BLOCKED"
        : secretaryFailClosedRecovery ? "INFERRED_FAIL_CLOSED" : "NONE",
      operator_verification_status: hasExplicitVerification
        ? explicit ? "EXPLICIT_VALID" : "INVALID_DECLARATION_BLOCKED"
        : operatorVerification ? "INFERRED" : "NONE",
    };
  });
}
