...
const QUERY_CONCEPT_EXPANSIONS = [
  { test: /\b(on duty|staffing|staff coverage|who owns the work|who is assigned|who's assigned|people assigned|staff assigned|assigned people|assigned staff|assigned to .*work|operational ownership)\b/i, add: ["assignment", "assign", "responsibility", "ownership"] },
  ...
  { test: /\b(customer conversation|customer message|customer reply)\b/i, add: ["commercial", "communication", "customer"] },
  { test: /\b(business project|entity project|company project)\b/i, add: ["projects", "project", "entity"] },
...
function domainConceptAffinity(message, capability) {
  const query = normalized(message);
  const domain = normalized(capability?.domain);
  const key = normalized(capability?.key);
  if (!query || !domain) return 0;
  if (/\b(customer conversation|customer message|customer reply)\b/i.test(query)) {
    return domain === "commercial" && /communication/.test(key) ? 1 : 0;
  }
  if (/\b(business project|entity project|company project)\b/i.test(query)) {
    return domain === "projects" ? 1 : 0;
  }
  if (/\b(on duty|staffing|staff coverage|who owns the work|who is assigned|people assigned|staff assigned|assigned to .*work|operational ownership)\b/i.test(query)) {
    return domain === "operations" && /assignments/.test(key) ? 1 : 0;
  }
  return 0;
}
...
function relevance(capability, message, modes) {
  ...
  const domainConcept = domainConceptAffinity(message, capability);
  ...
  return {
    ...
    domain_concept_affinity: domainConcept,
  };
}
