...
function verifierFor(item, catalog, capabilityKey) {
  ...
  if (item.domain !== verifier.domain) return null;
  return verifier;
}

function requiredSchemaKeys(schema = {}) {
  return Array.isArray(schema?.required)
    ? [...new Set(schema.required.map(safeKey).filter(Boolean))]
    : [];
}
...
export function normalizeOperatorVerificationDeclaration(raw, item, catalog = []) {
  ...
  const mutationKeys = new Set(schemaPropertyKeys(item.input_schema));
  const requiredVerifierKeys = new Set(requiredSchemaKeys(verifier.input_schema));
  ...
  if (payloadKeys.length) {
    if (payloadKeys.length > MAX_BINDINGS || payloadKeys.some((key) => !verifierKeys.has(key) || !mutationKeys.has(key))) return null;
    payloadKeys.forEach((key) => satisfiedKeys.add(key));
  ...
  for (const field of ["payload_from_result", "payload_from_input"]) {
    ...
    Object.keys(out).forEach((key) => satisfiedKeys.add(key));
    count += entries.length;
  }
  const recoveryEntries = Object.entries(plainObject(declaration.recovery_payload_from_evidence));
  if (recoveryEntries.length) {
    if (count + recoveryEntries.length > MAX_BINDINGS) return null;
    const out = {};
    for (const [rawKey, rawEvidenceKey] of recoveryEntries) {
      const key = safeKey(rawKey);
      const evidenceKey = safeKey(rawEvidenceKey);
      if (!key || !evidenceKey || !verifierKeys.has(key)) return null;
      out[key] = evidenceKey;
    }
    normalized.recovery_payload_from_evidence = out;
  ...
  if ([...requiredVerifierKeys].some((key) => !satisfiedKeys.has(key))) return null;
...
export function inferredVerificationDeclaration(item, catalog = []) {
  ...
  // Never guess a generated identifier from a verifier schema. Create-like mutations
  // must declare their result/evidence binding explicitly so verification remains exact.
