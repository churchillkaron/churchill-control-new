...
export function routeOperatorCognition({
  ...
  // A clear, single registered action does not need Deep cognition merely
  // because it is consequential. Permissions, confirmation, approval,
  // transactional execution and verification are enforced later by the
  // governed capability runtime. Deep is for ambiguity and deliberation.
  if (strongActions.length === 1 && actions.length === 1) {
    return { path: "fast", reason: "REGISTERED_ROUTINE_ACTION" };
  }
