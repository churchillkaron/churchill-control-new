...
function needsDeliberation(value) {
  ...
  // A short standalone "why?" is a genuine reasoning request even though its
  // token count is tiny. Keep deterministic greetings/navigation elsewhere.
  if (/^why\??$/.test(utterance)) return true;
...
export function routeOperatorCognition({
  ...
  // Voice has already passed the zero/low-latency layers before this router:
  // instant conversation, deterministic navigation, and deterministic business
  // reads. Any voice turn that still reaches model reasoning should therefore
  // get one deliberate high-quality reasoning pass, never a fast model call
  // followed serially by a deep fallback call.
