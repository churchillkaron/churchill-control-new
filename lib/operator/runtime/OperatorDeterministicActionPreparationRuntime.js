...
import { prepareCustomerInvoiceAction } from "./CustomerInvoiceActionPreparationRuntime.js";
...
export async function prepareDeterministicGovernedAction({
  ...
  if (capabilityKey === "finance.accounts_receivable.CreateCustomerInvoice") {
    prepared = await prepareCustomerInvoiceAction({ message, organizationId, entityId, timezone });
    if (!prepared) prepared = await prepareCustomerInvoice({ message, organizationId, timezone });
  }
  ...
  const preparedSummary = text(prepared.summary, 1200);
  return {
    contract: CONTRACT,
    decision: {
      response_text: preparedSummary || `I prepared the exact governed invoice for ${resolvedName}. I have not created it yet; confirmation is still required.`,
      response_language: text(locale, 80) || null,
      intent: "execute",
      confidence: 1,
      agreement_state: object(agreementState),
