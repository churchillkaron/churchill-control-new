...
export const OPERATOR_FAST_ACTION_INDEX = Object.freeze([
  ...
  action({
    ...
    aliases: [
      "mark invoice paid", "mark the invoice paid", "record invoice payment",
      "record customer payment", "receive customer payment", "post customer receipt",
      "send paid receipt", "get paid receipt", "create the receipt",
      "mark paid and create receipt", "mark invoice paid and create receipt",
    ],
    permissions: ["finance.receivables.manage"],
    scope: "entity",
    risk: "high",
    ...
    reversible: true,
    preparationReads: [
      { capability_key: "finance.customer_invoices.read", payload: { include_lines: true, limit: 100 }, purpose: "Resolve the exact open customer invoice and current outstanding balance before receipt posting." },
      { capability_key: "finance.bank_balances.read", payload: {}, purpose: "Resolve the active receiving bank account from current Finance evidence." },
    ],
    inputSchema: {
      type: "object",
      required: ["payment_date"],
      properties: {
        customer_invoice_id: { type: "string" },
        invoice_number: { type: "string" },
        latest_open_invoice: { type: "boolean" },
    ...
    verification: {
      capability_key: "finance.customer_receipt.read",
      payload_from_result: { payment_id: ["payment_id", "id"] },
      derivation: "declared_result_bound_payment_verifier",
    },
  }),
  action({
