...
export async function runFastConversationTurn({
  ...
  if (evidenceRequired) {
    ...
    if (semanticProductInspectionEvidence) {
      try {
        ...
        const inspectionExecution = await executeUbteCapability({
          ...
          action: "read",
          payload: { message: text(message).slice(0, 4000), ref: "main" },
          actor: object(actor),
          runtime: {
    ...
    if (directReadRecommended && readTool?.execute) {
      try {
        ...
        const front = await runFrontEvidencePresentation({
          evidence: directEvidence?.result ?? directEvidence,
    ...
    const runEvidenceTurn = () => reasoningModule.AvantiqoIntelligenceReasoningRuntime.run({
      ...
      execution_lane: "fast",
      temperature: 0.1,
      max_output_tokens: researchSynthesis ? 420 : strategic ? 220 : 180,
      max_turns: researchSynthesis ? 6 : 3,
      max_tool_calls: researchSynthesis ? 8 : 4,
    });
    ...
    try {
      ...
      const fallbackText = text(front.responseText) || "I could not finish verifying that current fact within the conversational time limit. I stopped the stalled read safely; no business action was replayed. Your question is still here and I can continue verification from this context.";
      return {
        success: true,
        decision: {
...
`.trim();
  ...
  try {
    ...
    await publishFastConversationRecovery(
      { organizationId, partyId, actor },
      "My fast conversation lane stalled, so I stopped that stalled job safely and I’m retrying this reply once. No business action is being replayed.",
    );
    execution = await runFastGeneration();
  }
