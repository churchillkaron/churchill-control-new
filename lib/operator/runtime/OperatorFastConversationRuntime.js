...
async function publishFastConversationRecovery({ organizationId, partyId, actor }, description) {
  try {
...
function directEvidenceHumanResponse({ capabilityKey, evidence } = {}) {
...
function productEvidenceHumanResponse({ message, evidence } = {}) {
  const data = object(evidence);
  ...
  if (finance && benchmark) {
    return [
      ...
      paths.length ? `Current source evidence includes ${paths.join(", ")}.` : "",
    ].filter(Boolean).join(" ");
  }
  if (finance && mobile) {
    return [
  ...
  if (finance) {
    return [
      "The first Finance improvement should be a task-first accountant workbench rather than a feature-first menu: surface what needs attention now, then let the accountant complete invoice, bank, reconciliation and close work without leaving the flow.",
      "I would prioritize exception visibility, fewer required fields, clear posting/confirmation state, strong document preview, and consistent next actions before adding more configuration or dashboards.",
      paths.length ? `I grounded that against the current Finance source, including ${paths.join(", ")}.` : "",
    ].filter(Boolean).join(" ");
  }
...
export async function runFastConversationTurn({
  ...
  const projectContext = strategic || semanticContinuity || semanticCorrection || semanticUnderstanding ? compactProjectContext(projectState) : null;
  ...
  if (evidenceRequired) {
    ...
    if (semanticProductInspectionEvidence) {
      try {
        const [{ execute: executeUbteCapability }, live] = await Promise.all([
          import("@/lib/ubte/runtime/ExecutionEngine"),
          import("@/lib/platform/runtime/AvantiqoLiveExecutionRuntime"),
        ]);
        await live.publishAvantiqoLiveExecution({
          context: { organizationId, partyId, actor },
          event: {
        ...
        const inspectionContextMessage = [
          ...
          ...recent.slice(-4).map((item) => text(item?.content).slice(0, 500)),
          text(message).slice(0, 1200),
        ].filter(Boolean).join("\n").slice(0, 4000);
        const inspectionExecution = await executeUbteCapability({
          organizationId,
          domain: "platform",
          capability: "code_ai_readonly_inspection",
          action: "read",
          payload: { message: inspectionContextMessage || text(message).slice(0, 4000), ref: "main" },
          actor: object(actor),
          runtime: {
            ...
            metadata: {
              ...
              channel,
              partyId: text(partyId).slice(0, 160) || null,
              operatorCapabilityKey: "platform.code_ai_readonly_inspection.read",
              evidenceOnly: true,
            },
          },
        });
        productInspectionEvidence = object(inspectionExecution?.result);
      } catch {
    ...
    const runFrontEvidencePresentation = async ({ evidence, capabilityKey = null, verified = true } = {}) => {
      const [serviceModule, ownedModule] = await Promise.all([
        import("@/lib/platform/service-runtime/execution/ServiceExecutionRuntime"),
        import("./OperatorOwnedIntelligenceServiceRuntime"),
      ]);
      ...
      const prompt = [
        ...
        text(projectState?.objective) ? `Active goal: ${text(projectState.objective).slice(0, 1200)}` : "",
        recent.length ? `Recent conversation: ${JSON.stringify(recent.slice(-4)).slice(0, 3000)}` : "",
        `User: ${text(message)}`,
        `Evidence: ${evidenceJson}`,
      ].filter(Boolean).join("\n");
      let execution = await serviceModule.ServiceExecutionRuntime.execute({
        organization_id: organizationId,
        party_id: partyId,
        entity_id: entityId,
        service_id: "ai.text.generate",
        ...ownedModule.ownedOperatorIntelligenceSelectionPolicy(),
        input: { prompt, execution_lane: "front", max_output_tokens: strategic ? 220 : 160 },
        metadata: {
          module: "OPERATOR",
      ...
      execution = await ownedModule.settleOperatorIntelligenceExecution({
        organization_id: organizationId,
        execution,
        service_id: "ai.text.generate",
        execution_lane: "front",
        metadata: { module: "OPERATOR", operation: "FRONT_EVIDENCE_PRESENTATION_SETTLEMENT", channel, raw_reasoning_persisted: false },
      });
    ...
    if (directReadRecommended && readTool?.execute) {
      try {
        const directEvidence = await readTool.execute({ capability_key: directReadKey, payload: {} });
        const evidencePayload = directEvidence?.result ?? directEvidence;
        let responseText = text(directEvidenceHumanResponse({
        ...
        await publishFastConversationRecovery(
          { organizationId, partyId, actor },
          `The direct read ${directReadKey} did not complete, so I’m using the guarded evidence path without replaying any business action.`,
        );
      }
    }
    ...
    const runEvidenceTurn = () => reasoningModule.AvantiqoIntelligenceReasoningRuntime.run({
      ...
      execution_lane: "fast",
      temperature: 0.1,
      max_output_tokens: researchSynthesis ? 360 : strategic ? 220 : 180,
...
`.trim();
  ...
  const runFastGeneration = async () => {
    ...
    input: {
      ...
      max_output_tokens: voice
        ? strategic ? 160 : 100
        : semanticResponseDetail === "deep" ? 360
          : strategic ? 260
            : semanticResponseDetail === "brief" ? 90 : 140,
    },
    metadata: {
      module: "OPERATOR",
    ...
    execution = await ownedModule.settleOperatorIntelligenceExecution({
      ...
      metadata: {
        module: "OPERATOR",
        operation: strategic ? "FRONT_PROJECT_CONVERSATION_SETTLEMENT" : "FRONT_CONVERSATION_SETTLEMENT",
        channel,
  ...
  let execution;
  try {
    execution = await runFastGeneration();
  } catch (error) {
    if (!fastIntelligenceTimeout(error)) throw error;
    await publishFastConversationRecovery(
      { organizationId, partyId, actor },
      "The CPU conversation lane stalled, so I stopped that exact job safely and escalated this same reply once. No business action is being replayed.",
    );
    const { AvantiqoIntelligenceReasoningRuntime } = await import("@/lib/intelligence/runtime/AvantiqoIntelligenceReasoningRuntime");
    execution = await AvantiqoIntelligenceReasoningRuntime.run({
      organization_id: organizationId,
      party_id: partyId,
      entity_id: entityId,
      system: prompt,
      ...
      metadata: {
        module: "OPERATOR",
        operation: strategic ? "FAST_PROJECT_CONVERSATION_ESCALATION" : "FAST_CONVERSATION_ESCALATION",
        channel,
        latency_class: "interactive",
        raw_reasoning_persisted: false,
      },
      execution_lane: "fast",
      temperature: 0.15,
      max_output_tokens: voice ? (strategic ? 160 : 100) : (strategic ? 260 : 140),
      max_turns: 1,
      max_tool_calls: 0,
