...
export async function runFastConversationTurn({
  ...
  if (evidenceRequired) {
    ...
    if (simpleExternalFact && readTool?.execute && list(readTool?.metadata?.research_capability_keys).includes("platform.research.search")) {
      try {
        ...
        await publishFastConversationRecovery(
          ...
          "The single external evidence read did not complete, so I’m continuing through the normal read-only evidence path without replaying any business action.",
        );
      }
    }
    ...
    const runFrontEvidencePresentation = async ({ evidence, capabilityKey = null, verified = true } = {}) => {
      ...
      const execution = await runOperatorFrontCognition({
        ...
        temperature: 0.12,
        max_output_tokens: strategic ? 220 : 160,
...
${voice ? "Do not output JSON or markdown." : "Do not output JSON. Use light markdown naturally when it improves readability."}
...
`.trim();
  ...
  const runFastGeneration = async () => runOperatorFrontCognition({
    ...
    front_task_mode: "conversation",
    temperature: strategic ? 0.18 : 0.12,
    max_output_tokens: voice
      ? strategic ? 220 : 140
      : semanticResponseDetail === "deep" ? 520
        : strategic ? 400
          : semanticResponseDetail === "brief" ? 140 : 300,
  ...
  try {
    ...
    execution = await AvantiqoIntelligenceReasoningRuntime.run({
      ...
      execution_lane: "fast",
      temperature: 0.15,
      max_output_tokens: voice ? (strategic ? 220 : 140) : (strategic ? 420 : semanticResponseDetail === "deep" ? 520 : semanticResponseDetail === "brief" ? 140 : 300),
      max_turns: 1,
      max_tool_calls: 0,
