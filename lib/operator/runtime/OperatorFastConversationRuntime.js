  ...
  if (evidenceRequired) {
    ...
    try {
      ...
      await publishFastConversationRecovery(
        ...
        "My fast reasoning lane stalled, so I stopped that stalled job safely and I’m retrying the same read-only question once without replaying any business action.",
      );
      liveReadReceipts.length = 0;
      evidenceExecution = await runEvidenceTurn();
    }
