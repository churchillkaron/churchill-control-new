...
const TEXT_NEUTRAL_CONVERSATION_PATTERN = /^(why|why not|how so|what do you mean|what does that mean|can you explain(?: that)?|explain that|tell me more|what do you think about (?:that|this|it)|what are the tradeoffs|what are the risks|is (?:that|this) safe|what exactly will (?:you|this|that) do|what will (?:this|that) do|thank(?:s| you)|got it|understood|i see|makes sense|that makes sense|sounds good)\s*[?.!]*$/i;
