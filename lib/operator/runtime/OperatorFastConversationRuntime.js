...
const CURRENT_OR_EXTERNAL_FACT_PATTERN = /\b(current|currently|latest|today|recent|newest|now|price|pricing|rate|exchange rate|weather|news|market|competitor|law|legal|regulation|legislation|standard|release|version|availability|opening hours|address|located|location|where is|who is|when is)\b/i;
...
export async function runFastConversationTurn({
  ...
  if (evidenceRequired) {
    ...
    const evidenceExecution = await reasoningModule.AvantiqoIntelligenceReasoningRuntime.run({
    ...
    return {
      success: true,
      decision: {
        response_text: responseText.slice(0, 1600),
        response_language: text(locale) || null,
