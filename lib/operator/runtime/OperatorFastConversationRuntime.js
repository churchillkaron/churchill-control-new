...
export async function runFastConversationTurn({
  ...
  if (evidenceRequired) {
    const { createOperatorIntelligenceReadTools } = await import(
      "./OperatorIntelligenceToolBridgeRuntime"
    );
    ...
    const evidenceExecution = await AvantiqoIntelligenceReasoningRuntime.run({
