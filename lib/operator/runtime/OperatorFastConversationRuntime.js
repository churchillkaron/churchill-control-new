...
export async function runFastConversationTurn({
  ...
  const projectReply = projectContinuityReply({
    message,
    projectState,
    agreementState,
  });
  ...
  const evidenceRequired = !voice && fastConversationNeedsEvidence(message);
  ...
  if (evidenceRequired) {
    ...
    const directReadRecommended = readTool?.metadata?.direct_read_recommended === true && Boolean(directReadKey);
    ...
    const runEvidenceTurn = () => reasoningModule.AvantiqoIntelligenceReasoningRuntime.run({
      ...
      system: [
        ...
        "This question requires current or externally verifiable evidence. Before stating the answer as current fact, use operator_live_read with the best matching internal read or governed research capability.",
        "Prefer Avantiqo internal live reads for Avantiqo-owned mutable business facts. Use governed external research for public/current facts.",
      ...
      metadata: {
        ...
        external_research_requested: externalResearchRequested(message),
      },
      execution_lane: "fast",
      temperature: 0.1,
      max_output_tokens: strategic ? 220 : 180,
      max_turns: 3,
      max_tool_calls: 4,
    });
