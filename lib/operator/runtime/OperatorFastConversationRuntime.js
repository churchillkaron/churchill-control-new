...
export async function runFastConversationTurn({
  ...
  const evidenceRequired = !voice && (semanticUnderstanding ? semanticEvidenceRequired : fastConversationNeedsEvidence(message));
  const externalEvidence = ["external", "both"].includes(semanticEvidenceScope);
  const researchSynthesis = semanticRoute === "evidence" && externalEvidence;
