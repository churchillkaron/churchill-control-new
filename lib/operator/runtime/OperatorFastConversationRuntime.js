...
`.trim();
  ...
  const compactLightPrompt = [
    ...
    "Keep simple questions short and complete. For a normal substantive question, aim for roughly 70 to 110 useful words, usually 1 to 3 short paragraphs. Finish every thought naturally; concise must not mean tiny or canned. Add a recommendation or next move only when it materially changes what the user should consider; do not append one by habit.",
  ...
  const runFastGeneration = async () => runOperatorFrontCognition({
    ...
    front_task_mode: strategic ? "conversation" : "conversation_light",
    temperature: strategic ? 0.18 : 0.12,
    max_output_tokens: voice
      ? strategic ? 220 : 110
      : strategic
        ? semanticResponseDetail === "deep" ? 360 : 280
        : semanticResponseDetail === "brief" ? 100 : 180,
  ...
  try {
    ...
    execution = {
      ...
      execution_lane: "fast",
      escalated: true,
      zero_price_owned_cpu_lane: false,
    };
  }
