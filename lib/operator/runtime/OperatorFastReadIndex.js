function read({ key, domain, capability, endpoint = null, aliases = [], description, scope = "organization", queryFields = [], inputSchema = null, externalEvidence = false }) {
...
export const OPERATOR_FAST_READ_INDEX = Object.freeze([
  read({ key: "platform.weather.read", domain: "platform", capability: "weather", description: "Read current weather conditions and today's forecast for a specified location from a live public weather provider.", aliases: ["weather", "current weather", "weather today", "weather forecast", "temperature today", "is it raining", "rain today"], externalEvidence: true, inputSchema: { type: "object", required: ["location"], properties: { location: { type: "string", description: "City, area, island, province, or other place whose weather should be checked." } }, additionalProperties: false } }),
  read({ key: "finance.customer_invoices.read", domain: "finance", capability: "customer_invoices", endpoint: "/api/finance/customer-invoices", scope: "entity", description: "Read current customer invoices, invoice status, customer details, invoice lines, PDF preview and paid receipt preview.", aliases: ["customer invoice", "customer invoices", "latest invoice", "latest customer invoice", "invoice moonshine", "show invoice", "show invoice preview", "invoice pdf", "paid receipt", "receipt preview"], queryFields:["id","invoice_id","partyId","party_id","include_lines","limit"] }),
