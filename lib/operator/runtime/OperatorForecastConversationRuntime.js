...
function points(value) {
  const number = numericOrNull(value);
  return number === null ? null : `${Math.round(Math.abs(number) * 100)} percentage points`;
}
function decimal(value) {
  const number = numericOrNull(value);
  return number === null ? null : number.toFixed(3);
}
...
function calibrationReply(summary, direct = false) {
  ...
  const hitRate = numericOrNull(summary?.observed_success_rate);
  const confidence = numericOrNull(summary?.mean_confidence);
  if (hitRate === null || confidence === null) {
...
function trackRecordReply(summary) {
  ...
  const scoreSentence = rate
    ...
    : `My recent scored forecast record is ${confirmed} confirmed and ${contradicted} contradicted out of ${scored}. The observed success-rate metric is not available yet.`;
  return [
    scoreSentence,
    confidence ? `Mean stated confidence is ${confidence}${gap ? ` with a ${gap} calibration gap` : ""}.` : "Mean stated confidence is not available yet, so I will not claim calibration.",
