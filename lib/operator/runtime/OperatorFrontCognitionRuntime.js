...
import { executeIntelligenceModalDirect } from "../../platform/service-runtime/providers/avantiqo-intelligence/AvantiqoIntelligenceModalDirectRuntime.js";
...
import "../../platform/service-runtime/providers/avantiqo-intelligence/AvantiqoIntelligenceCredentialRegistration.js";
import { resolveProviderCredential } from "../../platform/service-runtime/providers/ProviderCredentialRuntime.js";
...
async function runFront({ organizationId, partyId, entityId, system, messages, operation, temperature, maxOutputTokens, metadata, frontTaskMode, expectJson }) {
  ...
  const processModalConfigured = Boolean(
    text(process.env.MODAL_TOKEN_ID || process.env.AVANTIQO_MODAL_TOKEN_ID) &&
    text(process.env.MODAL_TOKEN_SECRET || process.env.AVANTIQO_MODAL_TOKEN_SECRET)
  );
  ...
  const credential = (processModalConfigured || ownedFrontConfigured) ? null : await Promise.race([
    resolveProviderCredential({
      organization_id: organizationId,
      provider: "avantiqo-intelligence",
    }),
    new Promise((_, reject) => setTimeout(() => reject(new Error("OPERATOR_FRONT_COGNITION_CREDENTIAL_TIMEOUT")), 2500)),
  ]);
  if (!processModalConfigured && !ownedFrontConfigured && !credential) throw new Error("OPERATOR_FRONT_COGNITION_MANAGED_CREDENTIAL_REQUIRED");
  ...
  if (shouldUseLocalIntelligenceQueue(executionInput)) {
    try {
      ...
      console.error("OPERATOR_FRONT_COGNITION_QUEUE_FALLBACK", { operation, error: text(error?.message || error, 500) });
    }
  }
  if (!execution && shouldUseLocalIntelligence(executionInput)) {
    try {
      ...
      if (String(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED || "").trim().toLowerCase() === "true") throw error;
      console.error("OPERATOR_FRONT_COGNITION_LOCAL_FALLBACK_MODAL", { operation, error: text(error?.message || error, 500) });
    }
  }
  if (!execution) execution = await executeIntelligenceModalDirect(executionInput);
  ...
  if (rawOutput.mutation_authority !== false || rawOutput.tools_allowed !== false || rawOutput.modal_gpu) {
...
export async function runOperatorFrontCognition({ organization_id, party_id = null, entity_id = null, system, messages = [], operation = "FRONT_COGNITION", temperature = 0.05, max_output_tokens = 220, metadata = {}, expect_json = false, front_task_mode = "semantic_classifier", allow_fast_escalation = true } = {}) {
