...
async function runFront({ organizationId, partyId, entityId, system, messages, operation, temperature, maxOutputTokens, metadata, frontTaskMode, expectJson }) {
  ...
  const credential = processModalConfigured ? null : await Promise.race([
  ...
  if (!processModalConfigured && !credential) throw new Error("OPERATOR_FRONT_COGNITION_MANAGED_CREDENTIAL_REQUIRED");
  const startedAt = Date.now();
  const execution = await executeIntelligenceModalDirect({
