...
async function runFront({ organizationId, partyId, entityId, system, messages, operation, temperature, maxOutputTokens, metadata, frontTaskMode, expectJson }) {
  ...
  if (!localQueueConfigured && !localFrontConfigured) throw new Error("OPERATOR_FRONT_COGNITION_LOCAL_NODE_REQUIRED");
  ...
  if (!execution && shouldUseLocalIntelligence(executionInput)) {
    try {
      ...
      console.error("OPERATOR_FRONT_COGNITION_LOCAL_DIRECT_FAILED", { operation, error: text(error?.message || error, 500) });
      throw error;
    }
  }
  if (!execution) throw new Error("OPERATOR_FRONT_COGNITION_LOCAL_NODE_REQUIRED");
  ...
  if (rawOutput.mutation_authority !== false || rawOutput.tools_allowed !== false) {
