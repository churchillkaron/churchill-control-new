...
async function runFront({ organizationId, partyId, entityId, system, messages, operation, temperature, maxOutputTokens, metadata }) {
  ...
  let execution = await ServiceExecutionRuntime.execute({
    ...
    ...ownedOperatorIntelligenceSelectionPolicy(),
    input: { messages: promptMessages, execution_lane: "front", front_task_mode: "semantic_classifier", temperature, max_output_tokens: maxOutputTokens },
...
export async function runOperatorFrontCognition({ organization_id, party_id = null, entity_id = null, system, messages = [], operation = "FRONT_COGNITION", temperature = 0.05, max_output_tokens = 220, metadata = {}, expect_json = false } = {}) {
  ...
  const request = { organizationId, partyId: party_id, entityId: entity_id, system, messages, operation, temperature, maxOutputTokens: max_output_tokens, metadata };
