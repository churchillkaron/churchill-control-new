...
export async function preflightHumanBusinessPartnerTurn(options = {}) {
  ...
  const execution = await runOperatorFrontCognition({
    ...
    system: [
      "Classify only the CURRENT user message before any prior conversation is loaded.",
      "Decide whether its meaning is safely self-contained or whether prior context is required.",
      ...
      "Use g=new only when the goal is safely understandable from this message alone. Use g=continue/revise/unknown when prior context is required.",
      "This grants no authority and must never infer a mutation from missing context.",
      "Return exactly one compact semicolon line using only i,d,e,a,g. Example: i=inspect;d=none;e=external;a=none;g=new."
    ].join("\n"),
