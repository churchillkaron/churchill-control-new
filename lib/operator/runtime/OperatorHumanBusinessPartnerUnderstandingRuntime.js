...
function operatorUtteranceExplicitlyRequestsResumeAfterInterruption(message) {
  ...
  const preserve = /\b(?:where you were|from where|without starting over|do not start over|don't start over|same mission|last verified step|without replaying|do not replay|don't replay|already completed)\b/.test(source);
...
function deterministicRecoveryContinuationUnderstanding(message, projectState = {}) {
  ...
  return {
    ...
    recovery_objective_context: text(project.objective, 1400) || null,
    ...
    deterministic_finish_and_verify_continuation: finishAndVerifyRecovery,
    authorization_effect: "NONE",
  };
}
...
function deterministicEvidenceRequestUnderstanding(message, conversation = []) {
  ...
  const presentationDirective =
    /^(?:give|show|tell)\s+me\s+(?:the\s+)?result\s+first\b.*\b(?:then|before)\b.*\bevidence\b/.test(normalized);
  if (scopeConstraint || presentationDirective) {
    return {
      contract: HUMAN_BUSINESS_PARTNER_UNDERSTANDING_CONTRACT,
      route: "conversation",
      evidence_scope: "internal",
      ...
      conversation_mode: "light",
      context_depth: "expanded",
      response_detail: presentationDirective ? "normal" : "brief",
      continuity_required: true,
      correction_or_revision: false,
      execution_domain: "business",
      engineering_scope: null,
      ...
      needs_current_evidence: presentationDirective,
      deterministic_scope_constraint: scopeConstraint,
      deterministic_presentation_directive: presentationDirective,
      authorization_effect: "NONE",
    };
  }
  ...
  if (registeredRead && !explicitMutationCue) {
    return {
      contract: HUMAN_BUSINESS_PARTNER_UNDERSTANDING_CONTRACT,
      ...registeredRead,
      continuity_required: scopeConstraint || priorReference || registeredRead.continuity_required === true,
      context_depth: (scopeConstraint || priorReference) ? "expanded" : registeredRead.context_depth,
      goal_relation: (scopeConstraint || priorReference) && registeredRead.goal_relation === "new"
        ? "continue"
...
function deterministicAmbiguousBusinessMutationUnderstanding(message) {
  ...
  const bareRecordMutation =
    /^(?:fix|change|correct|revise|update)\s+(?:the\s+|this\s+|that\s+)?record$/.test(normalized);
  const missingDelta =
  ...
  if (!bareInvoiceMutation && !bareRecordMutation && !missingDelta && !unresolvedOther && !unsafeGuess) return null;
  ...
  const question = unsafeGuess
    ...
    : missingDelta
      ...
      : unresolvedOther
        ...
        : bareRecordMutation
          ? "Which exact record do you want me to change, and what should I change on it?"
  ...
  return {
    ...
    user_goal: source,
    action_shape: "single",
    goal_relation: unsafeGuess ? "continue" : "unknown",
    artifact_intent: "none",
    artifact_type: null,
    requires_mutation: true,
    needs_current_evidence: true,
...
function deterministicRegisteredBusinessWriteUnderstanding(
  ...
  const contextualInvoiceCreation =
    /\b(?:create|make|issue)\b.*\binvoice\b/.test(normalized) &&
    /\b(?:the customer|that customer|this customer|services? i just gave you|services? just given|those services|these services|details? i just gave you)\b/.test(normalized);
  ...
  const customerReceipt =
    /\binvoice\b/.test(normalized) &&
    /\b(?:paid|payment|receipt)\b/.test(normalized) &&
    /\b(?:mark|record|receive|post|create|get|send)\b/.test(normalized);

  if (
    !explicitInvoiceCorrection &&
    !contextualInvoiceCreation &&
    !reusePriorInvoice &&
  ...
  const capabilityKey = customerReceipt
    ? "finance.customer_receipt.post"
    : contextualInvoiceCreation
      ? "finance.accounts_receivable.CreateCustomerInvoice"
    : wrongCustomerRecovery || reusePriorInvoice
      ? (
        anchorCapability ||
        "finance.accounts_receivable.CreateCustomerInvoice"
        )
  ...
  return {
    ...
    response_detail: "normal",
    continuity_required: true,
    correction_or_revision: customerReceipt ? false : true,
    execution_domain: "business",
    engineering_scope: null,
    engineering_mode: null,
    engineering_deliverable: null,
    ...
    user_goal: source,
    action_shape: "single",
    goal_relation: customerReceipt || contextualInvoiceCreation ? "continue" : "revise",
    artifact_intent: "none",
    artifact_type: null,
    requires_mutation: true,
    needs_current_evidence: true,
...
function deterministicPendingActionConfirmationUnderstanding(message, agreementState = {}) {
  const source = text(message, 12000);
  const normalized = source.toLowerCase().replace(/[.!?]+$/g, "").trim();
  const pending = object(object(agreementState).pending_execution);
  const capabilityKey = text(pending.capability_key, 300);
  if (!capabilityKey) return null;
  if (!/^(?:yes(?:,)?(?: do it)?|do it|go ahead|continue|confirmed|confirm|approved|approve)$/.test(normalized)) return null;
  const registered = findOperatorFastAction(capabilityKey);
  if (!registered) return null;
  return {
    contract: HUMAN_BUSINESS_PARTNER_UNDERSTANDING_CONTRACT,
    ...
    clarification_question: null,
    location_hint: null,
    user_goal: text(pending.description || source, 1400) || null,
    action_shape: "single",
    goal_relation: "continue",
    artifact_intent: "none",
    artifact_type: null,
    ...
    needs_current_evidence: true,
    registered_write_capability_key: text(registered.key, 300) || null,
    deterministic_pending_action_confirmation: true,
    authorization_effect: "NONE",
  };
}

function deterministicLocalOnlyProductChangeUnderstanding(message) {
  const source = text(message, 12000);
  const normalized = source.toLowerCase().replace(/[.!?]+$/g, "").trim();
  const noDeploy = /\b(?:do not|don't|no)\s+(?:deploy|deployment)\b|\bwithout\s+(?:a\s+)?(?:deploy|deployment)\b/.test(normalized);
  const localChange = /\b(?:fix|change|update|repair|adjust)\b/.test(normalized) && /\blocal(?:ly)?\b/.test(normalized);
  const verify = /\b(?:verify|test|check)\b/.test(normalized);
  if (!(noDeploy && localChange && verify)) return null;
  return {
    contract: HUMAN_BUSINESS_PARTNER_UNDERSTANDING_CONTRACT,
    route: "governed",
    evidence_scope: "internal",
    ...
    requires_mutation: true,
    needs_current_evidence: true,
    local_only_constraint: true,
    production_deploy_allowed: false,
    deterministic_local_only_product_change: true,
    authorization_effect: "NONE",
  };
}

function deterministicContextualConversationQuestionUnderstanding(message) {
  const source = text(message, 12000);
  const normalized = source.toLowerCase().replace(/[.!?]+$/g, "").trim();
  const asksNeededInput = /^(?:what|which)\s+(?:else\s+)?do\s+you\s+need\s+from\s+me\b/.test(normalized) ||
    /^what\s+information\s+do\s+you\s+need\b/.test(normalized);
  if (!asksNeededInput) return null;
  return {
    contract: HUMAN_BUSINESS_PARTNER_UNDERSTANDING_CONTRACT,
    route: "conversation",
    evidence_scope: "none",
    ...
    deterministic_contextual_conversation_question: true,
    authorization_effect: "NONE",
  };
}

function deterministicContextualBusinessRevisionUnderstanding(message, agreementState = {}, projectState = {}, conversation = []) {
  const source = text(message, 12000);
  const normalized = source.toLowerCase().replace(/[.!?]+$/g, "").trim();
  const samePriorPattern = /\b(?:same|same thing|same like)\b.*\b(?:last time|before|previous|prior)\b/.test(normalized);
  const weekdayChange = /\b(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/.test(normalized) &&
    /\b(?:instead|rather|change|move|shift)\b/.test(normalized);
  if (!(samePriorPattern && weekdayChange)) return null;
  const project = object(projectState);
  const anchor = latestVerifiedWriteExecution(conversation) || latestCompletedAgreementBusinessAction(agreementState) || null;
  const projectActive = text(project.status, 80).toLowerCase() === "active";
  if (!projectActive && !anchor) return null;
  const capabilityKey = text(anchor?.capability_key, 300) || "finance.accounts_receivable.CreateCustomerInvoice";
  const registered = findOperatorFastAction(capabilityKey) || findOperatorFastAction("finance.accounts_receivable.CreateCustomerInvoice");
  if (!registered) return null;
  return {
    contract: HUMAN_BUSINESS_PARTNER_UNDERSTANDING_CONTRACT,
    route: "governed",
    ...
    needs_current_evidence: true,
    registered_write_capability_key: text(registered.key, 300) || null,
    deterministic_contextual_business_revision: true,
    authorization_effect: "NONE",
  };
}
...
export async function preflightHumanBusinessPartnerTurn(options = {}) {
  ...
  const deterministicLocalOnlyProductChange = deterministicLocalOnlyProductChangeUnderstanding(message);
  if (deterministicLocalOnlyProductChange) return deterministicLocalOnlyProductChange;

  const deterministicPendingConfirmation = deterministicPendingActionConfirmationUnderstanding(message, options.agreementState);
  if (deterministicPendingConfirmation) return deterministicPendingConfirmation;

  const deterministicContextQuestion = deterministicContextualConversationQuestionUnderstanding(message);
  if (deterministicContextQuestion) return deterministicContextQuestion;

  const deterministicContextRevision = deterministicContextualBusinessRevisionUnderstanding(
    message,
    options.agreementState,
    options.projectState,
    options.immediateConversation,
  );
  if (deterministicContextRevision) return deterministicContextRevision;
