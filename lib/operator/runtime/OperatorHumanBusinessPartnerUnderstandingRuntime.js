...
export async function understandHumanBusinessPartnerTurn(options = {}) {
  ...
  const execution = await AvantiqoIntelligenceReasoningRuntime.run({
    ...
    system: [
      ...
      "Return JSON only with route, evidence_scope, reasoning_depth, user_goal, requires_mutation, needs_current_evidence."
    ].join("\n"),
  ...
  try {
    return normalize(JSON.parse(text(execution?.text, 5000)));
  } catch {
    return null;
  }
}
