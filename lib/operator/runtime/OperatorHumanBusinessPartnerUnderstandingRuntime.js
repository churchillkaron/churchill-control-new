...
export function normalizeHumanBusinessPartnerUnderstanding(value) {
  ...
  if (["chat", "inspect", "operate", "followup", "revise", "artifact", "unclear"].includes(compactIntent)) {
    ...
    return {
      ...
      conversation_mode: domain === "product_engineering" && !inspection && !operation ? "strategic" : "light",
...
export async function preflightHumanBusinessPartnerTurn(options = {}) {
  ...
  const execution = await runOperatorFrontCognition({
    ...
    system: [
      ...
      "For a standalone current-fact request that cannot be answered correctly without one essential parameter that is missing (for example a location, date, unit, entity, or identifier), use i=unclear, preserve the evidence scope, use g=new, and include q=<one concise question asking only for that missing parameter>.",
      ...
      "Return exactly one compact semicolon line using i,d,e,a,g and optional q only when i=unclear. Example: i=inspect;d=none;e=external;a=none;g=new. Missing parameter example: i=unclear;d=none;e=external;a=none;g=new;q=Which location should I use?"
    ].join("\n"),
  ...
  const contextRequired =
    normalized.continuity_required === true ||
    normalized.correction_or_revision === true ||
    ["continue", "revise", "unknown"].includes(text(normalized.goal_relation, 40).toLowerCase()) ||
