import { operatorUtteranceDependsOnImmediateContext, operatorUtteranceRequiresPriorContext, operatorUtteranceExplicitlyNamesProductSurface } from "../contracts/OperatorSymbolicReference.js";
...
export function deterministicExplicitBusinessMutationUnderstanding(message) {
  const source = text(message, 12000);
  if (!source) return null;
  const invoiceMutation =
    /\b(?:create|make|issue|raise|prepare|generate|copy|duplicate)\b[^.?!\n]{0,120}\binvoices?\b/i.test(source) ||
    /\bnew\s+(?:customer\s+)?invoice\b/i.test(source);
  const negatedMutation =
    /\b(?:do\s+not|dont|not\s+to)\s+(?:create|make|issue|raise|prepare|generate|copy|duplicate)\b/i.test(source);
  if (!invoiceMutation || negatedMutation) return null;
  const continuityRequired =
    /\b(?:same\s+as|last\s+one|latest|previous|prior|again|copy)\b/i.test(source);
  return {
    contract: "AVANTIQO_OPERATOR_DETERMINISTIC_BUSINESS_MUTATION_V1",
    route: "governed",
    evidence_scope: "internal",
    reasoning_depth: "fast",
    conversation_mode: "light",
    context_depth: continuityRequired ? "expanded" : "compact",
    ...
    deterministic_capability_key: "finance.accounts_receivable.CreateCustomerInvoice",
    deterministic_operator_alias: "create customer invoice",
...
export async function preflightHumanBusinessPartnerTurn(options = {}) {
  ...
  const deterministicMutation = deterministicExplicitBusinessMutationUnderstanding(message);
  if (deterministicMutation) {
    return {
      ...deterministicMutation,
      context_free_preflight: true,
      context_required: deterministicMutation.continuity_required === true,
      immediate_context_sufficient: false,
      structural_context_reference: deterministicMutation.continuity_required === true,
      prior_context_reference: deterministicMutation.continuity_required === true,
      standalone_context_sanity_applied: false,
      explicit_product_surface: false,
      preflight_provider: "avantiqo-local",
      preflight_model: "deterministic-business-mutation-v1",
      preflight_execution_lane: "local",
      preflight_front_task_mode: "deterministic",
      authorization_effect: "NONE",
    };
  }
...
export async function understandHumanBusinessPartnerTurn(options = {}) {
  ...
  const deterministicMutation = deterministicExplicitBusinessMutationUnderstanding(message);
  if (deterministicMutation) return deterministicMutation;
