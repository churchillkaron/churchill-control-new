...
function normalizeCandidate(value = {}, index = 0) {
  ...
  return {
    ...
    requires_human: source.requires_human === true || mutates || irreversible || RISK[risk] >= RISK.high,
...
function candidateIssues(candidate) {
  const issues = [];
  if (candidate.constraint_violations.length) issues.push("CONSTRAINT_VIOLATION");
...
function compareAction(left, right) {
  const leftSafety = left.candidate.irreversible ? 3 : left.candidate.mutates ? (left.candidate.reversible ? 1 : 2) : 0;
  const rightSafety = right.candidate.irreversible ? 3 : right.candidate.mutates ? (right.candidate.reversible ? 1 : 2) : 0;
  return (
    RISK[left.candidate.risk] - RISK[right.candidate.risk] ||
...
export function deliberateOperatorIntelligenceDecision({ goal, candidates = [], evidence = [], uncertainties = [], decision_critical = true } = {}) {
  ...
  const normalizedUncertainties = list(uncertainties).slice(0, MAX_UNCERTAINTIES).map(normalizeUncertainty);
  ...
  const evaluated = normalizedCandidates.map((candidate) => ({
    ...
    issues: [...candidateIssues(candidate), ...(duplicates.includes(candidate.id) ? ["DUPLICATE_CANDIDATE_ID"] : [])],
  }));
  ...
  if (unresolvedCritical.length && evidenceFirst.length) {
    ...
    status = selected.candidate.requires_human ? "RECOMMENDATION_REQUIRES_HUMAN" : "SELECTED_FOR_PLANNING";
  ...
  return {
    ...
    governance: {
      ...
      all_mutating_candidates_require_human_or_operator_governance: true,
