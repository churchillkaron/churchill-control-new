...
export function evaluateOperatorIntelligenceExecutionGuard({
  ...
  return Object.freeze({
    ...
    cognitive_plan_id: text(governedPlan.plan_id, 160) || null,
...
export function operatorIntelligenceMutationBindingProof(capability, execution = {}) {
  const guard = currentOperatorIntelligenceExecutionGuard();
  if (!guard || guard.required !== true) return null;
  ...
  const capabilityKey = text(capability?.key, 300);
  const payloadFingerprint = mutationPayloadFingerprint(execution?.payload);
  const binding = list(guard.allowed_mutation_bindings).find(
    (candidate) =>
      text(candidate?.capability_key, 300) === capabilityKey &&
      text(candidate?.payload_fingerprint, 128) === payloadFingerprint,
  );
  if (!binding) return null;

  return Object.freeze({
    contract: "AVANTIQO_COGNITIVE_MUTATION_BINDING_PROOF_V1",
    plan_id: text(guard.cognitive_plan_id, 160) || null,
    step_id: text(binding.step_id, 120) || null,
    capability_key: capabilityKey,
    execution_scope: Object.freeze({
      ...normalizedExecutionScope(execution),
    }),
...
export function enforceOperatorIntelligenceMutationGuard(capability, execution = {}) {
  const blocked = operatorIntelligenceMutationBlock(capability, execution);
  if (!blocked) {
    return operatorIntelligenceMutationBindingProof(capability, execution);
  }
