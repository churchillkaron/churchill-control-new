...
import {
  OperatorIntelligenceDecisionProvenanceRuntime,
  buildOperatorIntelligenceDecisionProvenance,
} from "./OperatorIntelligenceDecisionProvenanceRuntime.js";

const CONTRACT = "AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V8";
...
function createPlanGraphTool() {
  return {
    name: PLAN_TOOL_NAME,
    description: [
      "Build, assess, boundedly revise, deliberatively compare alternatives, stress-test decision robustness, assess decision validity over time, prioritize unresolved uncertainty, synthesize decision readiness, or build structured decision provenance for a governed Avantiqo Intelligence plan graph.",
      ...
      "Use build_provenance to produce structured, machine-readable lineage from the selected candidate to exact evidence IDs, assumption dependencies, validity conditions, changed material stress scenarios, uncertainty state, readiness gates, and explicit invalidation triggers. Provenance never stores or requires raw chain-of-thought and never grants execution authority.",
    ...
    parameters: {
      ...
      properties: {
        operation: {
          type: "string",
          enum: ["build", "assess", "revise", "deliberate", "stress_test", "validate_decision", "prioritize_uncertainty", "assess_readiness", "build_provenance"],
        },
        goal: { type: "string" },
    ...
    async execute(args = {}, context = {}) {
      ...
      if (operation === "build_provenance") {
        return buildOperatorIntelligenceDecisionProvenance({
          deliberation: object(args.deliberation_result),
          ...
          validity: object(args.validity_result),
          uncertainty_priority: object(args.uncertainty_priority_result),
          readiness: object(args.readiness_result),
        });
      }
