...
import { OperatorIntelligenceDecisionOutcomeAssessmentRuntime, assessOperatorIntelligenceDecisionOutcome } from "./OperatorIntelligenceDecisionOutcomeAssessmentRuntime.js";
import { OperatorIntelligenceDecisionCommitmentRuntime, assessOperatorIntelligenceDecisionCommitment } from "./OperatorIntelligenceDecisionCommitmentRuntime.js";
import { OperatorIntelligenceGoalConstraintConsistencyRuntime, assessOperatorIntelligenceGoalConstraintConsistency } from "./OperatorIntelligenceGoalConstraintConsistencyRuntime.js";
import { OperatorIntelligenceCognitionLifecycleRuntime, assessOperatorIntelligenceCognitionLifecycle } from "./OperatorIntelligenceCognitionLifecycleRuntime.js";

const CONTRACT = "AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V14";
const PREVIOUS_CONTRACT = "AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V13";
...
function createPlanGraphTool() {
  return {
    name: PLAN_TOOL_NAME,
    description: [
      "Build, assess, boundedly revise, deliberate, stress-test, validate, prioritize uncertainty, assess readiness, build structured provenance, assess contingency readiness, build a falsifiable decision outcome contract, assess later verified outcomes, assess commitment, verify hierarchical goal and inherited-constraint consistency, or determine the next valid stage in the complete cognition lifecycle for governed Avantiqo Intelligence planning.",
      "This tool is planning-and-verification-only. It never executes business actions, persists records, confirms, approves, publishes, sends, pays, deploys, schedules monitoring, triggers recovery, promotes learning state, rolls back, switches business state, rewrites parent goals, waives hard constraints, or mutates business state.",
      "Use assess_lifecycle to enforce the canonical cognition order. Later-stage output never substitutes for an unresolved earlier stage, and lifecycle completion is never execution authority.",
      ...
      "Use assess_outcome only with a ready outcome contract and later observations. Only verified current observations from the declared source can decide criteria. Failure dominates success, warnings block a success claim, missing or conflicting evidence stays inconclusive, and outcome success is neither plan completion nor execution authority.",
      "Use assess_commitment only after current decision cognition is available. Verified invalidation or failure overrides prior commitment, warnings and stale validity pause commitment, unresolved decision-relevant uncertainty is resolved first, validated fallbacks are planning-only, already-applied mutating or irreversible adverse decisions escalate, and sunk cost never justifies continuing an invalid decision.",
      "Use assess_goal_constraints before relying on a local plan or decision whose parent goals or inherited constraints matter. Hard ancestor constraints inherit downward, children may tighten but never weaken them, verified parent-child goal conflicts block descendant planning, and current sourced verification is required before claiming consistency.",
    ...
    parameters: {
      type: "object",
      properties: {
        operation: { type: "string", enum: ["build", "assess", "revise", "deliberate", "stress_test", "validate_decision", "prioritize_uncertainty", "assess_readiness", "build_provenance", "assess_contingency", "build_outcome_contract", "assess_outcome", "assess_commitment", "assess_goal_constraints", "assess_lifecycle"] },
        goal: { type: "string" },
        ...
        evidence_state: { type: "object", additionalProperties: true },
        ...
        outcome_contract_result: { type: "object", additionalProperties: true },
        outcome_assessment_result: { type: "object", additionalProperties: true },
        commitment_result: { type: "object", additionalProperties: true },
        fallback_candidate: { type: "object", additionalProperties: true },
        governance_state: { type: "object", additionalProperties: true },
        ...
        alignment_assertions: { type: "array", items: { type: "object", additionalProperties: true } },
        subject: { type: "object", additionalProperties: true },
        goal_constraint_result: { type: "object", additionalProperties: true },
    ...
    metadata: {
      ...
      previous_planning_contract: PREVIOUS_CONTRACT,
      ...
      decision_outcome_assessment_contract: OperatorIntelligenceDecisionOutcomeAssessmentRuntime.contract,
      decision_commitment_contract: OperatorIntelligenceDecisionCommitmentRuntime.contract,
      goal_constraint_consistency_contract: OperatorIntelligenceGoalConstraintConsistencyRuntime.contract,
      cognition_lifecycle_contract: OperatorIntelligenceCognitionLifecycleRuntime.contract,
      ...
      deterministic_cognition_lifecycle_ordering: true,
      ...
      deterministic_verified_outcome_assessment: true,
      deterministic_decision_commitment_policy: true,
      deterministic_hierarchical_goal_constraint_consistency: true,
      ...
      only_verified_current_exact_source_observations_can_decide_outcome: true,
      outcome_failure_dominates_success: true,
      outcome_warning_blocks_success_claim: true,
      outcome_success_is_not_plan_completion: true,
      outcome_assessment_never_triggers_recovery_or_learning_promotion: true,
      verified_invalidation_or_failure_overrides_commitment: true,
      sunk_cost_never_increases_commitment: true,
      fallback_switch_abandonment_and_commitment_are_cognitive_only: true,
      already_applied_adverse_mutating_decision_requires_human_review: true,
    ...
    async execute(args = {}, context = {}) {
      ...
      if (operation === "assess_outcome") return assessOperatorIntelligenceDecisionOutcome({ outcome_contract: object(args.outcome_contract_result), observations: list(args.observations) });
      if (operation === "assess_commitment") return assessOperatorIntelligenceDecisionCommitment({ decision: object(args.decision), readiness: object(args.readiness_result), validity: object(args.validity_result), uncertainty_priority: object(args.uncertainty_priority_result), contingency: object(args.contingency_result), outcome_contract: object(args.outcome_contract_result), outcome_assessment: object(args.outcome_assessment_result), fallback_candidate: object(args.fallback_candidate), governance_state: object(args.governance_state), progress: object(args.progress), decision_critical: args.decision_critical !== false });
      if (operation === "assess_goal_constraints") return assessOperatorIntelligenceGoalConstraintConsistency({ goals: list(args.goals), target_goal_id: args.target_goal_id, constraints: list(args.constraints), alignment_assertions: list(args.alignment_assertions), subject: object(args.subject), decision_critical: args.decision_critical !== false });
      if (operation === "assess_lifecycle") return assessOperatorIntelligenceCognitionLifecycle({ goal_constraint_result: object(args.goal_constraint_result), evidence_state: object(args.evidence_state), deliberation_result: object(args.deliberation_result), robustness_result: object(args.robustness_result), validity_result: object(args.validity_result), uncertainty_priority_result: object(args.uncertainty_priority_result), readiness_result: object(args.readiness_result), provenance_result: object(args.provenance_result), contingency_result: object(args.contingency_result), outcome_contract_result: object(args.outcome_contract_result), outcome_assessment_result: object(args.outcome_assessment_result), commitment_result: object(args.commitment_result), decision_critical: args.decision_critical !== false });
...
export const OperatorIntelligencePlanningToolRuntime = Object.freeze({
  ...
  decisionOutcomeAssessmentContract: OperatorIntelligenceDecisionOutcomeAssessmentRuntime.contract,
  decisionCommitmentContract: OperatorIntelligenceDecisionCommitmentRuntime.contract,
  goalConstraintConsistencyContract: OperatorIntelligenceGoalConstraintConsistencyRuntime.contract,
  cognitionLifecycleContract: OperatorIntelligenceCognitionLifecycleRuntime.contract,
  ...
  assessDecisionOutcome: assessOperatorIntelligenceDecisionOutcome,
  assessDecisionCommitment: assessOperatorIntelligenceDecisionCommitment,
  assessGoalConstraintConsistency: assessOperatorIntelligenceGoalConstraintConsistency,
  assessCognitionLifecycle: assessOperatorIntelligenceCognitionLifecycle,
});
