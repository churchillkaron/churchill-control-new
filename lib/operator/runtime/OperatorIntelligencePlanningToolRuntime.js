...
const CONTRACT = "AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V11";
...
function createPlanGraphTool() {
  return {
    name: PLAN_TOOL_NAME,
    description: [
      "Build, assess, boundedly revise, deliberate, stress-test, validate, prioritize uncertainty, assess readiness, build structured provenance, assess contingency readiness, build a falsifiable decision outcome contract, or assess later verified outcomes for governed Avantiqo Intelligence planning.",
      "This tool is planning-and-verification-only. It never executes business actions, persists records, confirms, approves, publishes, sends, pays, deploys, schedules monitoring, triggers recovery, promotes learning state, or mutates business state.",
    ...
    parameters: {
      type: "object",
      properties: {
        operation: { type: "string", enum: ["build", "assess", "revise", "deliberate", "stress_test", "validate_decision", "prioritize_uncertainty", "assess_readiness", "build_provenance", "assess_contingency", "build_outcome_contract", "assess_outcome"] },
        goal: { type: "string" },
