...
const CONTRACT = "AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V9";
...
function createPlanGraphTool() {
  return {
    name: PLAN_TOOL_NAME,
    description: [
      "Build, assess, boundedly revise, deliberate, stress-test, validate, prioritize uncertainty, assess readiness, build structured provenance, or assess contingency readiness for governed Avantiqo Intelligence planning.",
      "This tool is planning-only and never executes or authorizes business actions.",
      ...
      "Provenance and contingency never require or persist raw chain-of-thought.",
      "Every candidate remains subject to normal Operator permissions, confirmation, approval, wallet, execution and verification controls.",
    ].join(" "),
    parameters: {
      type: "object",
      properties: {
        operation: { type: "string", enum: ["build", "assess", "revise", "deliberate", "stress_test", "validate_decision", "prioritize_uncertainty", "assess_readiness", "build_provenance", "assess_contingency"] },
        goal: { type: "string" },
