...
const CONTRACT = "AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V6";
...
function createPlanGraphTool() {
  return {
    name: PLAN_TOOL_NAME,
    description: [
      "Build, assess, boundedly revise, deliberatively compare alternatives, stress-test decision robustness, assess decision validity over time, or prioritize unresolved uncertainty for a governed Avantiqo Intelligence plan graph.",
      ...
      "Use stress_test after deliberation when a material decision should be checked for brittleness. Stress scenarios may alter safe planning attributes, remove referenced evidence, or add unresolved uncertainty. Hypothetical scenarios are planning probes only and never become live evidence. Verified scenario changes that destabilize the decision require normal evidence-aware replanning and governance.",
      "Use validate_decision before relying on a prior recommendation as current. It deterministically checks verified evidence dependencies and validity conditions against bounded freshness policy. Stale, unverified, unknown-volatility, superseded, or future-dated dependencies require revalidation; a verified changed condition requires replanning. Decision validity never authorizes execution and prior approval never substitutes for current governance.",
      "Use prioritize_uncertainty when several unresolved questions compete for attention. Safety-critical and decision-flipping unknowns come first, then governance/completion blockers, decision impact, information gain, resolvability, lower cost and lower latency. Model numeric priority scores never decide the order, resolved unknowns cannot be selected, and low-value unknowns may be deferred.",
    ...
    parameters: {
      ...
      properties: {
        operation: {
          type: "string",
          enum: ["build", "assess", "revise", "deliberate", "stress_test", "validate_decision", "prioritize_uncertainty"],
        },
        goal: { type: "string" },
    ...
    async execute(args = {}, context = {}) {
      ...
      if (operation === "stress_test") {
        return stressTestOperatorIntelligenceDecision({
          goal: args.goal,
          candidates: list(args.candidates),
          evidence: list(args.evidence),
          uncertainties: list(args.uncertainties),
      ...
      if (operation === "validate_decision") {
        return assessOperatorIntelligenceDecisionValidity({
          decision: object(args.decision),
          evidence_dependencies: list(args.evidence_dependencies),
          validity_conditions: list(args.validity_conditions),
          now: args.now,
        });
      }
      if (operation === "prioritize_uncertainty") {
        return prioritizeOperatorIntelligenceUncertainties({
          goal: args.goal,
          uncertainties: list(args.uncertainties),
        });
      }
