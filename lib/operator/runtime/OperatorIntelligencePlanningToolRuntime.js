...
function createPlanGraphTool() {
  return {
    ...
    description: [
      ...
      "Use assess after observed step outcomes to determine ready work, failed dependencies, verification proof gaps, replan need and whether completion is actually proven.",
      "Use revise only after a failure or blocker. Replanning is bounded and cannot delete or rewrite completed history.",
    ].join(" "),
    parameters: {
      type: "object",
      properties: {
    ...
    async execute(args = {}) {
      ...
      if (operation === "assess") {
        return assessOperatorIntelligencePlan({
          plan: object(args.plan),
          observations: list(args.observations),
        });
      }
      if (operation === "revise") {
        return reviseOperatorIntelligencePlan({
          plan: object(args.plan),
...
export const OperatorIntelligencePlanningToolRuntime = Object.freeze({
  ...
  assessPlan: assessOperatorIntelligencePlan,
  revisePlan: reviseOperatorIntelligencePlan,
});
