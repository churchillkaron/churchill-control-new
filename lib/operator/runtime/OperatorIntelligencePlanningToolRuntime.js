...
function createPlanGraphTool() {
  return {
    ...
    description: [
      ...
      "Use assess after observed step outcomes to determine ready work, failed dependencies, verification proof gaps, verified evidence that invalidates assumptions, replan need and whether completion is actually proven.",
      "Use revise after a failure, blocker, or verified observation that contradicts, invalidates, or materially changes the active plan. Replanning is bounded and cannot delete or rewrite completed history.",
    ...
    async execute(args = {}) {
      ...
      if (operation === "assess") {
        return assessOperatorIntelligencePlanWithEvidenceRevision({
          plan: object(args.plan),
          observations: list(args.observations),
        });
      }
      if (operation === "revise") {
        return reviseOperatorIntelligencePlanWithEvidenceRevision({
          plan: object(args.plan),
...
export const OperatorIntelligencePlanningToolRuntime = Object.freeze({
  ...
  assessPlan: assessOperatorIntelligencePlanWithEvidenceRevision,
  revisePlan: reviseOperatorIntelligencePlanWithEvidenceRevision,
});
