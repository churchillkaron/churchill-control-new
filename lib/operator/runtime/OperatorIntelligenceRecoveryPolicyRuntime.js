...
function normalizedFailureObservation(value = {}) {
  ...
  return {
    ...
    claimed_attempts: isFailure ? Math.max(1, rawAttempts) : rawAttempts,
  };
}
...
function normalizedGovernedOutcome(value = {}) {
  ...
  return {
    ...
    plan_step_id: text(source.plan_step_id, 120) || null,
...
function governedOutcomeIndex(values = []) {
  const receipts = list(values).map(normalizedGovernedOutcome);
  const byCallId = new Map();
  for (const receipt of receipts) {
    if (!receipt.tool_call_id) continue;
    byCallId.set(receipt.tool_call_id, receipt);
  }
  return { receipts, byCallId };
}

function safeReceipt(receipt) {
  return Boolean(
    receipt &&
    receipt.contract === GOVERNED_TOOL_OUTCOME_CONTRACT &&
    receipt.raw_result_persisted !== true &&
    receipt.raw_error_persisted !== true,
  );
}

function attestedFailure(planId, step, observation, receipt) {
  ...
  if (!planId || !receipt.plan_id || !receipt.plan_step_id) {
    return { trusted: false, reason: "GOVERNED_PLAN_STEP_BINDING_REQUIRED", code: null };
  }
  if (receipt.plan_id !== planId || receipt.plan_step_id !== step.id) {
    return { trusted: false, reason: "GOVERNED_PLAN_STEP_BINDING_MISMATCH", code: null };
  }
...
function governedFailedAttemptCount(planId, step, receipts = []) {
  if (!planId || !step?.id || !step?.capability_key) return 0;
  const callIds = new Set();
  for (const receipt of list(receipts)) {
    if (!safeReceipt(receipt)) continue;
    if (receipt.plan_id !== planId) continue;
    if (receipt.plan_step_id !== step.id) continue;
    if (receipt.binding_key !== step.capability_key) continue;
    if (receipt.outcome !== "failed") continue;
    if (!receipt.tool_call_id || callIds.has(receipt.tool_call_id)) continue;
    callIds.add(receipt.tool_call_id);
  }
  return callIds.size;
}

function recoveryDecision(planId, step, observation, receipt, receipts = []) {
  ...
  const claimedAttempts = boundedInteger(sourceObservation.claimed_attempts, 1, 1, 20);
  const governedAttempts = governedFailedAttemptCount(planId, sourceStep, receipts);
  const consumedRetries = Math.max(0, governedAttempts - 1);
  const remainingRetries = Math.max(0, retryBudget - consumedRetries);
  const attestation = attestedFailure(planId, sourceStep, sourceObservation, receipt);
  const base = {
    ...
    governed_attempts: governedAttempts,
  ...
  if (governedAttempts <= 0) {
    return {
      ...base,
      reason: "GOVERNED_ATTEMPT_HISTORY_REQUIRED",
    };
  }
...
function recoveryDecisions(plan = {}, observations = [], governedToolOutcomes = []) {
  ...
  const planId = text(sourcePlan.plan_id, 160) || null;
  const byStep = stepIndex(sourcePlan);
  const outcomeIndex = governedOutcomeIndex(governedToolOutcomes);
  return list(observations)
    ...
    .map((observation) => {
      ...
      const receipt = observation.tool_call_id
        ? outcomeIndex.byCallId.get(observation.tool_call_id) || null
        : null;
...
function recoveryGovernance(base = {}) {
  return {
    ...
    model_attempt_counts_never_authorize_retry: true,
    governed_attempt_history_controls_retry_budget: true,
    tool_call_binding_required_for_retry: true,
    capability_binding_required_for_retry: true,
    plan_step_binding_required_for_retry: true,
