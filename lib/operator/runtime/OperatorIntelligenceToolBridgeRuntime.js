...
export async function createOperatorIntelligenceReadTools({
  ...
  const catalog = fastResolution.strong && !externalResearchRequested(message)
    ? fastSafe
  ...
  return [
    {
      ...
      metadata: {
        ...
        read_only: true,
        direct_read_recommended: Boolean(fastResolution.strong && fastResolution.ranked[0]?.capability?.key && !externalResearchRequested(message)),
