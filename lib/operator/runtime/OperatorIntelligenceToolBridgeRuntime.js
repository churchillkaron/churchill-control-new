...
async function executeDirectAuthenticatedRead({ capability, context, callerRequest, payload = {} }) {
  ...
  const response = await fetch(url, { method: "GET", cache: "no-store", headers: { Accept: "application/json", ...(cookie ? { cookie } : {}) } });
...
export async function createOperatorIntelligenceReadTools({
  ...
  const normalizedEvidenceScope = text(evidenceScope, 40).toLowerCase();
  const researchRequested = ["external", "both"].includes(normalizedEvidenceScope)
    ? true
  ...
  return [
    {
      ...
      metadata: {
        ...
        read_only: true,
        direct_read_recommended: Boolean(fastResolution.strong && fastResolution.ranked[0]?.capability?.key && !researchRequested),
        primary_capability_key: text(fastResolution.ranked[0]?.capability?.key, 300) || null,
        primary_match_score: Number(fastResolution.ranked[0]?.score || 0),
        primary_phrase_affinity: Number(fastResolution.ranked[0]?.phrase_affinity || 0),
      ...
      async execute(args = {}) {
        ...
        try {
          const execution = capability.direct_endpoint
