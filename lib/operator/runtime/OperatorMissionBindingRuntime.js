...
const MAX_STRING_VALUE = 1000;
const FORBIDDEN_PATH_PARTS = new Set([
  "__proto__",
  "prototype",
  "constructor",
]);
const FORBIDDEN_SOURCE_PARTS = new Set([
  "secret",
  "secrets",
  "password",
  "passwords",
...
function pathParts(value) {
  ...
  if (
    ...
    parts.some((part) =>
      ...
      FORBIDDEN_PATH_PARTS.has(part.toLowerCase()),
    )
  ) {
    return [];
  }
...
function normalizeBinding(binding = {}, targetStepId, priorStepIds) {
  ...
  if (sourcePath.some((part) => FORBIDDEN_SOURCE_PARTS.has(part.toLowerCase()))) {
    throw new Error("OPERATOR_MISSION_BINDING_SENSITIVE_SOURCE_BLOCKED");
  }
