...
import {
  operatorMissionDispatchKey,
  operatorMissionPayloadFingerprint,
} from "@/lib/operator/runtime/OperatorMissionDispatchIdentityRuntime";

export { operatorMissionDispatchKey, operatorMissionPayloadFingerprint };
...
function isMissingDispatchSchema(error) {
  const code = text(error?.code).toUpperCase();
  const message = text(error?.message).toLowerCase();
  return (
    ["42P01", "PGRST205"].includes(code) ||
    (message.includes("operator_mission_dispatches") &&
      (message.includes("schema cache") || message.includes("does not exist")))
  );
}

function dispatchSchemaRequiredError(error) {
  const normalized = new Error("OPERATOR_MISSION_DISPATCH_SCHEMA_REQUIRED");
  normalized.code = "OPERATOR_MISSION_DISPATCH_SCHEMA_REQUIRED";
  normalized.cause = error;
  normalized.operatorMissionDispatchSchema = {
    table: TABLE,
    migration: "20260902053633_operator_mission_dispatch_journal.sql",
...
function throwDispatchError(error) {
  if (isMissingDispatchSchema(error)) throw dispatchSchemaRequiredError(error);
  throw error;
}
...
async function loadExisting({ organizationId, dispatchKey }) {
  ...
  if (error) throwDispatchError(error);
  return data || null;
}
...
export async function claimOperatorMissionDispatch({
  ...
  if (text(error.code) !== "23505") throwDispatchError(error);
...
export async function updateOperatorMissionDispatchState({
  ...
  if (updateError) throwDispatchError(updateError);
