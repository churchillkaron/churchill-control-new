...
function successfulServerReadCapabilityKeys(source = {}) {
  return new Set(
    list(object(source).server_live_read_receipts)
      .filter((receipt) =>
      ...
      .map((receipt) => text(receipt?.capability_key, 300))
      .filter(Boolean),
  );
}

function planHasCapabilityBoundRead(plan = {}, serverReadKeys = new Set()) {
  return list(object(plan).steps).some((step) =>
    step?.mutates !== true &&
    text(step?.kind, 80).toLowerCase() === "read" &&
    serverReadKeys.has(text(step?.capability_key, 300)),
  );
}

function readAncestors(plan = {}, stepId, serverReadKeys = new Set()) {
  const steps = new Map(
...
function temporalMemoryPlanIssues(plan = {}, source = {}) {
  ...
  const issues = [];
  const serverReadKeys = successfulServerReadCapabilityKeys(source);
  ...
  if (currentStateIntent && !planHasCapabilityBoundRead(governed, serverReadKeys)) {
    issues.push({ code: "CURRENT_STATE_LIVE_READ_REQUIRED" });
  }
  for (const step of list(governed.steps).filter((item) => item?.mutates === true)) {
    if (!readAncestors(governed, step.id, serverReadKeys)) {
      issues.push({
