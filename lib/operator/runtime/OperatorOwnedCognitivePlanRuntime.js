...
function planHasCapabilityBoundRead(plan = {}) {
  return list(object(plan).steps).some((step) =>
    step?.mutates !== true &&
    text(step?.kind, 80).toLowerCase() === "read" &&
    Boolean(text(step?.capability_key, 300)),
  );
}

function readAncestors(plan = {}, stepId) {
  const steps = new Map(
