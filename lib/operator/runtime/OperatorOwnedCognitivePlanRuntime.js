...
function readSubjectMatchesMutation(readStep = {}, mutationStep = {}) {
  const mutationIdentities = new Set(list(mutationStep?.business_identities));
  if (!mutationIdentities.size) return true;
  const readIdentities = list(readStep?.business_identities);
  return readIdentities.some((identity) => mutationIdentities.has(identity));
}
