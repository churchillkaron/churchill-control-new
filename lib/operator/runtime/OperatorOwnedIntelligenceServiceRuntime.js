...
export async function settleOperatorIntelligenceExecution({
  ...
  await ServiceExecutionRuntime.cancelPending({
    ...
    pricing: object(execution?.pricing),
    credential_id: execution?.credential_id || null,
    reason: `AVANTIQO_OPERATOR_INTELLIGENCE_PENDING_SETTLEMENT_TIMEOUT:${executionLane}`,
    metadata: {
      ...object(metadata),
      operator_owned_intelligence_contract: CONTRACT,
  ...
  throw new Error(`AVANTIQO_OPERATOR_INTELLIGENCE_PENDING_SETTLEMENT_TIMEOUT:${executionLane}`);
}
