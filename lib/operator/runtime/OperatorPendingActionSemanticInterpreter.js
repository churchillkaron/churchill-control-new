...
export async function interpretPendingActionReply({
  ...
  try {
    const execution = await runOperatorFrontCognition({
      ...
      system: [
        "Understand the user's meaning from normal language, the recent conversation, and the exact staged action. Do not classify by literal keywords.",
        "The staged action is context only; this understanding step grants no authority and must never mutate anything.",
        "Decide the user's relation to the staged action: confirm, cancel, revise, discuss, new_goal, or unclear.",
        "Use confirm when the user clearly wants the staged action carried out now, regardless of phrasing, customer names, pronouns, politeness, or word order.",
        "Use cancel when they clearly reject or stop the staged action.",
        "Use revise when they want to change details of the staged action while keeping the same business goal.",
        "Use discuss when they want to inspect, preview, download, ask about, or discuss the staged action without authorizing execution.",
        "Use new_goal only when they clearly move to a different business goal.",
        "Use unclear only when two or more materially different meanings remain after using the supplied context.",
        "For discuss, infer result_presentation as preview, pdf, download, both, default, or none.",
        "If unclear, write one short natural clarification question that names the staged action when possible and asks what the user means. Never return an error-style message.",
        "Return one compact semicolon-separated line only. Fields: q=<relation>;p=<presentation>;cf=<0..1>;cq=<clarification question>. Omit cq unless q=unclear.",
      ].join("\\n"),
      messages: [{ role: "user", content: JSON.stringify({ message: userMessage, recent, pending_action: pendingAction }) }],
      authorization: { allow_mutating_tools: false },
      tools: [],
      expect_json: false,
      temperature: 0.02,
      max_output_tokens: 96,
      front_task_mode: "semantic_classifier",
      allow_fast_escalation: true,
      metadata: { module: "OPERATOR", operation: "PENDING_ACTION_SEMANTIC_UNDERSTANDING", raw_reasoning_persisted: false },
    });
    const envelope = Object.fromEntries(
      text(execution?.text, 2000).split(";")
        .map((part) => {
    ...
    const result = normalizeResult({
      relation: envelope.q,
      result_presentation: envelope.p,
      confidence: envelope.cf,
    });
    if (!result) return {
      contract: OPERATOR_PENDING_ACTION_SEMANTIC_CONTRACT,
      relation: "unclear",
    ...
    result.clarification_question = result.relation === "unclear"
      ? (text(envelope.cq, 700) || "Do you mean you want me to continue with the staged action, change it, or just show it to you first?")
      : null;
