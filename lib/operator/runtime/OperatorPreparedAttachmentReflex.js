...
export function hasPreparedAttachmentReflexCandidate(attachments = [], message = "") {
  ...
  if (bank.length === 1 && universal.length === 0 && inventory.length === 0 && paidExpenses.length === 0) return true;
  if (inventory.length === 1 && bank.length === 0 && universal.length === 0 && paidExpenses.length === 0 && SIMPLE_ATTACHMENT_PATTERN.test(text(message))) return true;
  if (paidExpenses.length === 1 && bank.length === 0 && inventory.length === 0 && universal.length === 0) return true;
  return universal.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && SIMPLE_ATTACHMENT_PATTERN.test(text(message));
}
