...
export function resolvePreparedAttachmentReflex({
  ...
  if (inventory.length === 1 && candidates.length === 0 && universal.length === 0) {
    ...
    return {
      response_text: requested
        ? `${summary} The review is ready, but Supply Chain does not yet have a governed atomic bulk-import capability, so I have not written any inventory records.`
        : `${summary} This is a review only; no inventory records have been changed.`,
