...
const SUPPLY_CHAIN_GOODS_RECEIPT_CAPABILITY = "supply-chain.goods_receipts.receive";
const SUPPLY_CHAIN_SUPPLIER_CAPABILITY = "supply-chain.suppliers.create";
const SUPPLY_CHAIN_SUPPLIER_PRICE_CAPABILITY = "supply-chain.supplier_prices.import";
const SUPPLY_CHAIN_RECIPE_CAPABILITY = "supply-chain.recipes.upsert";
const MUSIC_WORLD_CLASS_EXECUTE_CAPABILITY = "creative.music.executeWorldClassProduction";
...
const SIMPLE_ATTACHMENT_PATTERN = /\b(what is this|what kind|where does|where should|which area|import|upload|record|process|reconcile|receive|save|add|create|update|replace|register|onboard|hire|open|file|put|bring)\b/i;
const ACTION_PATTERN = /\b(import|upload|record|process|reconcile|receive|save|add|create|update|replace|register|onboard|hire|open|file|attach|bring\s+(?:it|this)\s+in)\b|\bput\b[\s\S]{0,30}\b(correct|right|finance|system)\b/i;
const FILING_PATTERN = /\b(file|upload|save|attach|add)\b/i;
const MUSIC_ATTACHMENT_PATTERN = /\b(backing track|karaoke|remove (?:the )?vocals|without vocals|separate (?:the )?stems|stems|isolate (?:the )?vocals|acapella|a cappella|mix (?:this|the)|master (?:this|the)|clean up (?:this|the) audio|repair (?:this|the) audio|make (?:this|the) (?:song|track|audio) professional|music for (?:this|the) video|soundtrack|remix (?:this|the)|extend (?:this|the) (?:song|track|audio))\b/i;
...
function goodsReceiptCandidates(attachments = []) {
  return list(attachments)
    .map((file) => ({ file, candidate: object(file?.prepared_candidate) }))
    .filter(({ candidate }) => candidate.type === "goods_receipt" && candidate.recognized === true);
}

function recipeCandidates(attachments = []) {
  return list(attachments).map((file) => ({ file, candidate: object(file?.prepared_candidate) })).filter(({ candidate }) => candidate.type === "recipe" && candidate.recognized === true);
}

function supplierPriceCandidates(attachments = []) {
  return list(attachments)
    .map((file) => ({ file, candidate: object(file?.prepared_candidate) }))
    .filter(({ candidate }) => candidate.type === "supplier_price" && candidate.recognized === true);
}

function supplierCandidates(attachments = []) {
  return list(attachments)
    .map((file) => ({ file, candidate: object(file?.prepared_candidate) }))
    .filter(({ candidate }) => candidate.type === "supplier" && candidate.recognized === true);
}
...
function musicSourceCandidates(attachments = []) {
  return list(attachments).filter((file) => {
...
export function hasPreparedAttachmentReflexCandidate(attachments = [], message = "") {
  ...
  const goodsReceipts = goodsReceiptCandidates(attachments);
  const suppliers = supplierCandidates(attachments);
  const supplierPrices = supplierPriceCandidates(attachments);
  const recipes = recipeCandidates(attachments);
  const music = musicSourceCandidates(attachments);
  ...
  if (music.length === 1 && list(attachments).length === 1 && MUSIC_ATTACHMENT_PATTERN.test(text(message))) return true;
  ...
  if (bank.length === 1 && universal.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0) return true;
  if (inventory.length === 1 && bank.length === 0 && universal.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0 && SIMPLE_ATTACHMENT_PATTERN.test(text(message))) return true;
  if (paidExpenses.length === 1 && bank.length === 0 && inventory.length === 0 && universal.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0) return true;
  if (vendorBills.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && universal.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0) return true;
  if (purchaseOrders.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && universal.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0) return true;
  if (goodsReceipts.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && universal.length === 0 && supplierPrices.length === 0) return true;
  if (suppliers.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && universal.length === 0 && supplierPrices.length === 0) return true;
  if (supplierPrices.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && universal.length === 0) return true;
  if (recipes.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0 && universal.length === 0) return true;
  return universal.length === 1 && bank.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0 && SIMPLE_ATTACHMENT_PATTERN.test(text(message));
}
...
export function resolvePreparedAttachmentReflex({
  ...
  if (music.length === 1 && list(attachments).length === 1 && MUSIC_ATTACHMENT_PATTERN.test(text(message))) {
    const file = music[0];
    const capability = list(capabilities).find((item) => item?.key === MUSIC_WORLD_CLASS_EXECUTE_CAPABILITY);
    if (capability) {
      const mime = text(file.mime_type, 160).toLowerCase();
      const source = text(file.url, 4000);
      const payload = {
        title: text(file.name, 240) || "Music Studio source",
        objective: text(message, 4000),
        source_media: source,
        ...(mime.startsWith("audio/") ? { source_audio: source } : {}),
        source_rights_confirmed: true,
      };
      return {
        response_text: "I can run this through Music Studio. Before I execute it, confirm that you have the rights or permission required to use this source audio and that you want me to proceed with the requested music work.",
        response_language: null, intent: "execute", confidence: 1,
        agreement_state: object(agreementState), project_state: object(projectState),
        clarification: { required: false, question: null, options: [] },
        navigation: { target_id: null },
        execution: { capability_key: MUSIC_WORLD_CLASS_EXECUTE_CAPABILITY, payload, reason: "PREPARED_MUSIC_STUDIO_SOURCE_EXECUTION" },
        plan: [],
      };
    }
  }
  ...
  if (recipes.length === 1 && candidates.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && supplierPrices.length === 0 && universal.length === 0) {
    const candidate=recipes[0].candidate;
    if(candidate.status === "CLARIFICATION_REQUIRED") { const question=text(candidate.clarification_question)||"I need one more recipe detail before I can continue."; return { response_text:question,response_language:null,intent:"clarify",confidence:1,agreement_state:object(agreementState),project_state:object(projectState),clarification:{required:true,question,options:[]},navigation:{target_id:null},execution:{capability_key:null,payload:{},reason:null},plan:[] }; }
    const capability=list(capabilities).find((item)=>item?.key===SUPPLY_CHAIN_RECIPE_CAPABILITY);
    if(candidate.status === "READY_FOR_REVIEW" && ACTION_PATTERN.test(text(message)) && capability) return { response_text:`I matched this recipe to ${text(candidate.dish?.name)||"the selected dish"} with ${list(candidate.recipe_items).length} exact ingredient line${list(candidate.recipe_items).length===1?"":"s"}. Replacing the operational recipe and recalculating its food cost requires your confirmation.`,response_language:null,intent:"execute",confidence:1,agreement_state:object(agreementState),project_state:object(projectState),clarification:{required:false,question:null,options:[]},navigation:{target_id:null},execution:{capability_key:SUPPLY_CHAIN_RECIPE_CAPABILITY,payload:object(candidate.import_payload),reason:"PREPARED_RECIPE_UPSERT"},plan:[] };
    return { response_text:"I identified this as a recipe. No operational recipe has been changed.",response_language:null,intent:"answer",confidence:1,agreement_state:object(agreementState),project_state:object(projectState),clarification:{required:false,question:null,options:[]},navigation:{target_id:null},execution:{capability_key:null,payload:{},reason:null},plan:[] };
  }

  if (supplierPrices.length === 1 && candidates.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && suppliers.length === 0 && universal.length === 0) {
    const candidate = supplierPrices[0].candidate;
    if (candidate.status === "CLARIFICATION_REQUIRED") {
      const question = text(candidate.clarification_question) || "I need one more supplier-price detail before I can continue.";
    ...
    const capability = list(capabilities).find((item) => item?.key === SUPPLY_CHAIN_SUPPLIER_PRICE_CAPABILITY);
    if (candidate.status === "READY_FOR_REVIEW" && ACTION_PATTERN.test(text(message)) && capability) {
      const payload = object(candidate.import_payload);
      const count = list(payload.rows).length;
      if (!count) return { response_text: `This supplier price list is already up to date. I found ${Number(candidate.unchanged_count)||0} unchanged price${Number(candidate.unchanged_count)===1?"":"s"}.`, response_language: null, intent: "answer", confidence: 1, agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: null }, plan: [] };
      return { response_text: `I reviewed ${Number(candidate.row_count)||0} supplier price rows: ${Number(candidate.new_count)||0} new, ${Number(candidate.changed_count)||0} changed, and ${Number(candidate.unchanged_count)||0} unchanged. I prepared ${count} price update${count===1?"":"s"}; applying them requires your confirmation.`, response_language: null, intent: "execute", confidence: 1, agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: SUPPLY_CHAIN_SUPPLIER_PRICE_CAPABILITY, payload, reason: "PREPARED_SUPPLIER_PRICE_IMPORT" }, plan: [] };
    }
    return { response_text: `I identified a supplier price list with ${Number(candidate.row_count)||0} row${Number(candidate.row_count)===1?"":"s"}. No prices have been changed.`, response_language: null, intent: "answer", confidence: 1, agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: null }, plan: [] };
  }

  if (suppliers.length === 1 && candidates.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && goodsReceipts.length === 0 && universal.length === 0) {
    const entry = suppliers[0];
    const candidate = entry.candidate;
    const file = entry.file;
    if (candidate.status === "CLARIFICATION_REQUIRED") {
      const question = text(candidate.clarification_question) || "I need one more supplier identity detail before I can create this supplier.";
      return { response_text: question, response_language: null, intent: "clarify", confidence: 1,
        agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: true, question, options: [] },
        navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: null }, plan: [] };
    }
    if (candidate.status === "EXISTING_RECORD") {
      const existing = object(candidate.existing_record);
      ...
      if (FILING_PATTERN.test(text(message)) && documentCapability) {
        return { response_text: `This supplier already exists as ${text(existing.label) || "an existing supplier"}. I prepared the upload as controlled evidence linked to that supplier; filing requires your confirmation.`,
          ...
          execution: { capability_key: DOCUMENT_FILE_CAPABILITY, payload: documentCreatePayload(file, {}, existing), reason: "PREPARED_EXISTING_SUPPLIER_EVIDENCE" }, plan: [] };
      }
      return { response_text: `This supplier already exists as ${text(existing.label) || "an existing supplier"}. I have not created a duplicate.`,
    ...
    const supplier = object(candidate.supplier);
    const capability = list(capabilities).find((item) => item?.key === SUPPLY_CHAIN_SUPPLIER_CAPABILITY);
    if (candidate.status === "READY_FOR_REVIEW" && ACTION_PATTERN.test(text(message)) && capability && object(candidate.import_payload).legal_name) {
      return { response_text: `I identified ${text(supplier.legal_name) || "this business"} as a new supplier and prepared the atomic supplier master. Creating it requires your confirmation.`,
        response_language: null, intent: "execute", confidence: 1, agreement_state: object(agreementState), project_state: object(projectState),
        clarification: { required: false, question: null, options: [] }, navigation: { target_id: null },
        execution: { capability_key: SUPPLY_CHAIN_SUPPLIER_CAPABILITY, payload: object(candidate.import_payload), reason: "PREPARED_SUPPLIER_CREATE" }, plan: [] };
    }
    return { response_text: `I identified ${text(supplier.legal_name) || "this business"} as a supplier. No supplier record has been created.`,
      response_language: null, intent: "answer", confidence: 1, agreement_state: object(agreementState), project_state: object(projectState),
      clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: null }, plan: [] };
  }

  if (goodsReceipts.length === 1 && candidates.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && purchaseOrders.length === 0 && suppliers.length === 0 && universal.length === 0) {
    const candidate = goodsReceipts[0].candidate;
    const po = object(candidate.purchase_order);
    if (["CLARIFICATION_REQUIRED", "PARTIAL_OR_UNVERIFIED"].includes(candidate.status)) {
      const question = text(candidate.clarification_question) || "Review this delivery before receiving stock.";
      return { response_text: question, response_language: null, intent: "clarify", confidence: 1,
        agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: true, question, options: [] }, navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: null }, plan: [] };
    }
    if (candidate.status === "ALREADY_RECEIVED") {
      return { response_text: `Purchase order ${text(po.po_number) || ""} is already received. I have not created another goods receipt.`, response_language: null, intent: "answer", confidence: 1,
    ...
    const capability = list(capabilities).find((item) => item?.key === SUPPLY_CHAIN_GOODS_RECEIPT_CAPABILITY);
    if (candidate.status === "READY_FOR_REVIEW" && ACTION_PATTERN.test(text(message)) && capability) {
      return { response_text: `This delivery exactly matches approved purchase order ${text(po.po_number) || ""}. I prepared the canonical goods receipt and inventory movement. Receiving it requires your confirmation.`, response_language: null, intent: "execute", confidence: 1,
        agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: SUPPLY_CHAIN_GOODS_RECEIPT_CAPABILITY, payload: object(candidate.import_payload), reason: "PREPARED_GOODS_RECEIPT_RECEIVE" }, plan: [] };
    }
    return { response_text: `I matched this delivery to purchase order ${text(po.po_number) || ""}. No stock has been received yet.`, response_language: null, intent: "answer", confidence: 1,
      agreement_state: object(agreementState), project_state: object(projectState), clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: null }, plan: [] };
  }

  if (purchaseOrders.length === 1 && candidates.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && vendorBills.length === 0 && suppliers.length === 0 && universal.length === 0) {
  ...
  if (vendorBills.length === 1 && candidates.length === 0 && inventory.length === 0 && paidExpenses.length === 0 && suppliers.length === 0 && universal.length === 0) {
