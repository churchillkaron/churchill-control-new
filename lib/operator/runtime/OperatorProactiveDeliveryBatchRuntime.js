...
function alertPredatesOptIn(watch, alert) {
  const enabledAt = timestamp(watch?.delivery_policy_enabled_at);
  if (enabledAt === null) return true;
  const createdAt = timestamp(alert?.created_at);
  return createdAt === null || createdAt < enabledAt;
}
...
async function prepareExternalAlert(scope) {
  return mutateOperatorWatchProjectState({
    ...
    mutate: ({ projectState }) => {
      ...
      if (alertPredatesOptIn(watch, alert)) {
        return {
          skip: true,
          outcome: { queued: false, reason: "ALERT_PREDATES_OPT_IN" },
        };
      }
...
async function deliverForWatchResult(result, now) {
  ...
  try {
    ...
    return {
      ...
      queue_reason: text(prepared?.outcome?.reason, 120) || null,
