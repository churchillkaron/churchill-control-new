...
function failedProviderReadiness({ providerId, reason, serviceState = {} }) {
  return {
...
async function providerReadiness({
  ...
  const serviceState = {
    organization_service_exists: Boolean(organizationService),
  ...
  if (!organizationService) {
    return failedProviderReadiness({ providerId, reason: "ORGANIZATION_SERVICE_NOT_ENABLED", serviceState });
  }
  if (!serviceState.active) {
    return failedProviderReadiness({ providerId, reason: "ORGANIZATION_SERVICE_INACTIVE", serviceState });
  }
  if (!serviceState.usage_enabled) {
    return failedProviderReadiness({ providerId, reason: "ORGANIZATION_SERVICE_USAGE_DISABLED", serviceState });
  }
  if (!capabilityEnabled) {
    return failedProviderReadiness({ providerId, reason: "SERVICE_CAPABILITY_NOT_ENABLED", serviceState });
  }
  ...
  try {
    ...
    return failedProviderReadiness({
      providerId,
      reason: text(error?.message || error, 300) || "PROVIDER_OR_PRICING_UNAVAILABLE",
      serviceState,
    });
  }
  ...
  try {
    ...
    credential = {
      ...
      credential_configured: false,
      credential_ready: false,
      credential_reason: text(error?.message || error, 300) || "PROVIDER_CREDENTIAL_STATUS_UNAVAILABLE",
    };
  }
  ...
  return {
    provider_id: providerId,
    provider_selected: Boolean(selectedProvider),
    pricing_ready: Boolean(pricing),
    runtime_ready: runtimeReady,
    ...credential,
    ready_for_execution: ready,
    readiness_reason: ready
      ? "STATIC_EXECUTION_PREFLIGHT_READY"
      : runtimeReason || credential?.credential_reason || "PROVIDER_EXECUTION_PREFLIGHT_FAILED",
    pricing: pricing
      ? {
  ...
  return Promise.all(
    operatorProactiveDeliveryChannelCatalog().map(async (descriptor) => {
      ...
      try {
        ...
        serviceError = text(error?.message || error, 300) || "ORGANIZATION_SERVICE_STATUS_UNAVAILABLE";
      }

      const serviceCapabilities = resolveServiceCapabilities(descriptor.service_id);
      const enabledCapabilities = resolveExecutionCapabilities(serviceCapabilities?.capabilities || []);
      ...
      return {
        ...
        ready_for_execution: providers.some((provider) => provider.ready_for_execution),
        providers,
