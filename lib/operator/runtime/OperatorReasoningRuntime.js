...
function boundedCapabilities(capabilities = [], message = "", source = "text") {
  const voice = text(source).toLowerCase() === "voice";
  const selected = rankedCapabilities(capabilities, message, voice ? 12 : 56);
...
function executionNeedsEntity(
  ...
  if (capabilityKey === OPERATOR_READ_CHAIN_KEY) {
    for (const step of list(execution.payload?.steps)) {
...
Treat executable_capabilities[].context_scope as authoritative. If the needed capability has context_scope "entity" and business_context.entity_id is missing, do not execute it: ask one focused clarification for the legal entity and never guess one. Apply the same rule to every child capability inside platform.operator_read_chain.execute.
