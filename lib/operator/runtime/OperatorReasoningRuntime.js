...
function modalOverflowAgreementProposal(agreementState = {}) {
...
function modalOverflowRequestIdentity({ organizationId, entityId, partyId, message, source } = {}) {
  return {
    ...
    original_user_message: text(message),
  };
}

function modalOverflowProposalDecision({ error, agreementState, projectState, locale, message, source } = {}) {
  const proposal = object(error?.modal_overflow_proposal);
  const proposalId = text(proposal.proposal_id);
  if (!proposalId) return null;
  const ceiling = Number(proposal.maximum_supplier_cost_thb || 0);
  const lane = text(proposal.execution_lane).toLowerCase() === "fast" ? "Fast" : "Deep";
  const cost = Number.isFinite(ceiling) && ceiling > 0 ? `${ceiling.toFixed(2)} THB` : "the shown approved ceiling";
  const question = `Node01 cannot safely handle this exact job within its bounded local capacity. Modal ${lane} can handle it with a maximum bounded supplier cost of ${cost}. Approve this one job?`;
  const nextAgreementState = {
    ...object(agreementState),
    intelligence_modal_overflow_proposal: {
      ...proposal,
      status: "PROPOSED",
      original_message: text(message).slice(0, 8000),
      original_source: text(source) || "text",
  ...
  return {
    decision: normalizeOperatorDecision({
      ...
      clarification: {
        ...
        question,
        options: [
          { id: "approve", label: "Approve this one job" },
          { id: "cancel", label: "Do not use Modal" },
        ],
      },
      navigation: { target_id: null },
      execution: { capability_key: null, payload: {}, reason: "INTELLIGENCE_MODAL_OVERFLOW_APPROVAL_REQUIRED" },
      plan: [],
    }, { projectState }),
    provider_evidence: {
      provider: "avantiqo-local",
      model: "modal-overflow-proposal-v1",
      usage_id: null,
      modal_overflow_proposal: proposal,
      external_compute_started: false,
...
`;
  ...
  try {
    ...
    input: {
      ...
      modal_overflow_proposal_allowed: true,
      modal_overflow_request_identity: modalOverflowRequestIdentity({
        organizationId, entityId, partyId, message, source,
      }),
      ...(modalOverflowAgreementProposal(agreementState)?.status === "APPROVED"
        ? { modal_overflow_proposal_id: modalOverflowAgreementProposal(agreementState).proposal_id }
        : {}),
    },
    metadata: {
      module: "OPERATOR",
    ...
    if (text(error?.code || error?.message) === "AVANTIQO_INTELLIGENCE_MODAL_OVERFLOW_APPROVAL_REQUIRED") {
      const proposalDecision = modalOverflowProposalDecision({
        error, agreementState, projectState, locale, message, source,
      });
