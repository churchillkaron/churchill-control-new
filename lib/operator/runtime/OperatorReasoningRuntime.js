...
// The catalogue is the dominant cost of a turn: a full JSON Schema plus
// permissions and tags for every candidate pushed the planning prompt past
// 28k tokens once the Operations domain was exposed, which both slowed the turn
// and buried the user's actual message. Ranking still runs against the complete
// capability, so dropping these fields from the payload does not affect which
// candidates are chosen. Field names and required keys are kept so the model can
// still populate an execution payload; types and descriptions are not needed for
// that, and the server validates the payload regardless.
...
// Relevance ranking is lexical, so a business question never matches the
// accounting artefact that answers it: "total income" shares no tokens with
// trial_balance, general_ledger or management_reports, so all 42 finance reads
// scored zero and the Operator reported having no capability while the right one
// existed. This index lists every capability compactly, and execution resolves
// against the full catalogue rather than the ranked detail, so naming a key from
// the index is enough to run it. About 2.4k tokens for complete awareness.
...
function boundedCapabilities(capabilities = [], message = "", source = "text") {
  const voice = text(source).toLowerCase() === "voice";
  const selected = ranked(capabilities, message, voice ? 28 : 56);
...
`;
  ...
  // Key order matters: this is serialised into a single input string, and the
  // catalogues are far larger than everything else. Emitting them first keeps the
  // live turn -- state, recent conversation and the user's actual message -- next
  // to the generation point instead of thousands of tokens away from it, which is
  // what made the Operator answer earlier questions rather than the current one.
  const request = {
