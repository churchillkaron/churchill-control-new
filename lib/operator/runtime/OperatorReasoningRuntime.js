...
import {
  reasonRecipeClarification,
} from "./RecipeClarificationIntelligenceRuntime.js";
...
export async function reasonAboutOperatorTurn({
  ...
  if (attachmentReflex) {
    const recipeEntry = list(conversationAttachments).find((entry) => {
      const candidate = object(entry?.prepared_candidate);
      return text(candidate.contract).startsWith("AVANTIQO_RECIPE_ATTACHMENT_PREPARATION_") &&
        ["CLARIFICATION_REQUIRED", "READY_FOR_REVIEW"].includes(candidate.status);
    });
    if (recipeEntry) {
      try {
        const intelligence = await reasonRecipeClarification({
          organizationId, partyId, entityId, locale, message,
          candidate: object(recipeEntry.prepared_candidate), attachment: recipeEntry,
        });
        if (intelligence.clarification_required) {
          ...
          return {
            decision: normalizeOperatorDecision(decision, { projectState }),
            provider_evidence: intelligence.provider_evidence,
          };
        }
      } catch (error) {
        console.error("RECIPE_CLARIFICATION_INTELLIGENCE_FALLBACK", error);
      }
    }
    return {
