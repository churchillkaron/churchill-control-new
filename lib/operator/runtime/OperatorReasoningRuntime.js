...
Understand the current user message and respond quickly.
Use the supplied current project state and recent conversation only as context.
...
function fastVoiceNeedsFallback(parsed, request) {
  if (!parsed || parsed.needs_full_catalog === true) return true;
