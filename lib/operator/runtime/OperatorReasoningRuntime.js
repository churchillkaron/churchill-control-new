...
} from "./OperatorBusinessReadResolver.js";
import {
...
} from "./OperatorPreparedAttachmentReflex.js";
import {
  preparedAttachmentNeedsIntelligence,
  reasonPreparedAttachmentClarification,
} from "./PreparedAttachmentClarificationIntelligenceRuntime.js";
...
const LOCAL_DEEP_CAPACITY_ERRORS = new Set([
  "AVANTIQO_INTELLIGENCE_LOCAL_RUNTIME_REQUIRED",
  "AVANTIQO_LOCAL_COMPUTE_REQUIRED_UNAVAILABLE",
  "AVANTIQO_HIERARCHICAL_LOCAL_CHUNK_LIMIT",
  "AVANTIQO_HIERARCHICAL_LOCAL_FINAL_CONTEXT_TOO_LARGE",
  "AVANTIQO_HIERARCHICAL_LOCAL_REDUCTION_LIMIT",
]);

function localDeepCapacityDecision({ error, agreementState, projectState, locale } = {}) {
  const code = text(error?.code || error?.message).split(":")[0];
  const failure = text(error?.local_failure_code || error?.message);
  const hierarchyChunkLimit = failure.match(/AVANTIQO_HIERARCHICAL_LOCAL_CHUNK_LIMIT:(\d+):(\d+)/i);
  const hierarchyFinalContext = failure.match(/AVANTIQO_HIERARCHICAL_LOCAL_FINAL_CONTEXT_TOO_LARGE/i);
  const localCapacityFits = error?.local_capacity_fits;

  if (!LOCAL_DEEP_CAPACITY_ERRORS.has(code) && !hierarchyChunkLimit && !hierarchyFinalContext) {
    return null;
  }

  let reason = "Node01 could not safely complete this Deep request inside the current local envelope.";
  if (hierarchyChunkLimit) {
    reason = `This request needs ${hierarchyChunkLimit[1]} local evidence chunks, above the current safe limit of ${hierarchyChunkLimit[2]}.`;
  } else if (hierarchyFinalContext) {
    reason = "The local hierarchical reduction still leaves too much evidence for a reliable final Deep synthesis on Node01.";
  } else if (localCapacityFits === false) {
    reason = "The request does not fit a reliable single local Deep pass and the local hierarchical path did not complete safely.";
  } else if (/HIERARCHICAL/i.test(failure)) {
    reason = "The local hierarchical Deep path could not complete safely on Node01.";
  }

  const responseText = `${reason} I kept the work local and did not start external compute. Narrow the scope, split the job, or continue with a smaller evidence set.`;
  return {
    decision: normalizeOperatorDecision({
      ...
      clarification: {
        ...
        options: [
          { id: "narrow_scope", label: "Narrow the scope" },
          { id: "split_job", label: "Split into local jobs" },
        ],
      },
      navigation: { target_id: null },
      execution: { capability_key: null, payload: {}, reason: code || "LOCAL_DEEP_CAPACITY" },
      plan: [],
    }, { projectState }),
    provider_evidence: {
      provider: "avantiqo-local",
      model: "node01-local-capacity-guard-v2",
      usage_id: null,
      local_deep_capacity_error: code || null,
      local_failure_code: failure || null,
      local_capacity_fits: localCapacityFits === true,
      external_compute_started: false,
      external_compute_authority: "NONE",
      modal_inference_performed: false,
    },
  };
}

function operatorHierarchicalAnchor(request = {}) {
  const source = object(request);
  const capabilityKeys = list(source.executable_capabilities)
    .map((item) => text(item?.key || item?.capability_key || item?.id))
    .filter(Boolean);
  const navigationIds = list(source.navigation_targets)
    .map((item) => text(item?.id || item?.target_id))
    .filter(Boolean);
  return {
    contract: "AVANTIQO_OPERATOR_HIERARCHICAL_ANCHOR_V1",
    user_input: object(source.user_input),
    business_context: object(source.business_context),
    current_project_state: object(source.current_project_state),
    agreement_state: object(source.agreement_state),
    executable_capability_keys: capabilityKeys,
    navigation_target_ids: navigationIds,
    output_contract: object(source.output_contract),
    authority_effect: "NONE",
    external_compute_authority: "NONE",
  };
}

function operatorHierarchicalFinalInstructions() {
  return [
    "Answer the current user_input.message, not an older conversation turn.",
    "Return exactly one valid JSON object with keys response_text, response_language, intent, confidence, agreement_state, project_state, clarification, navigation, execution, plan.",
    "Preserve the immutable anchor's business context, project state, agreement state, exact identifiers, and authority boundaries.",
    "Execution capability_key must be null or a real supplied capability key. Never invent authority or a capability.",
    "Navigation target_id must be null or a supplied navigation target id.",
    "If a material mutation input is missing or ambiguous, clarify instead of guessing.",
    "Read evidence never auto-authorizes a write. Stronger reasoning never increases authority.",
    "Do not invent facts missing from the supplied evidence.",
  ].join("\\n");
}
...
function boundedCapabilities(capabilities = [], message = "", source = "text") {
  ...
  return selected.map((capability) => {
    ...
    return {
      ...
      ...(capability.operator_verification ? { verification: capability.operator_verification } : {}),
    };
  });
}
...
const CURRENT_STATE_EVIDENCE_PATTERN = /\b(current|currently|right now|live|latest|today|status|state|still|verify|verified)\b/i;
...
function leadingActionClause(message) {
  return text(message).split(/[.!?\n]/, 1)[0].slice(0, 240).trim();
}

function explicitActionVerbAligned(message, capability = {}) {
  let clause = normalizedActionText(leadingActionClause(message));
  if (!clause) return false;
  if (/^(what|why|how|when|where|who|which|is|are|do|does|did|should i|can i|could i)\b/.test(clause)) {
    return false;
  }
  ...
  const verbs = new Set([
    normalizedActionText(capability.action).split(" ")[0],
    ...list(capability.operator_aliases)
      .map((alias) => normalizedActionText(alias).split(" ")[0]),
  ].filter(Boolean));
  const first = clause.split(" ")[0];
  return Boolean(first && verbs.has(first));
}

export function resolveExplicitWriteActionRequest({ message, capabilities = [] } = {}) {
  const actionCapabilities = list(capabilities).filter((capability) =>
    ["draft", "write", "approve"].includes(text(capability?.mode).toLowerCase()),
  );
  if (!actionCapabilities.length) return null;
  const clause = leadingActionClause(message);
  ...
  if (!top?.capability || !explicitActionVerbAligned(message, top.capability)) return null;
...
function actionResolutionRepairRequired(parsed, request, fullCapabilities = []) {
  const intent = normalizeIntent(parsed?.intent);
  if (!["answer", "plan"].includes(intent)) return null;
  if (text(parsed?.execution?.capability_key || parsed?.capability_key)) return null;
  if (object(parsed?.clarification).required === true) return null;
  return resolveExplicitWriteActionRequest({
    message: request?.user_input?.message,
    capabilities:
      list(fullCapabilities).length > 0
        ? list(fullCapabilities)
        : list(request?.executable_capabilities),
  });
}

function actionResolutionRepairAccepted(parsed, resolution) {
  const intent = normalizeIntent(parsed?.intent);
  if (intent === "clarify") {
    return object(parsed?.clarification).required === true &&
      Boolean(text(parsed?.clarification?.question));
  }
  if (intent !== "execute") return false;
  return text(parsed?.execution?.capability_key || parsed?.capability_key) ===
    text(resolution?.capability?.key);
}

function unresolvedActionClarification(parsed, resolution) {
  const capability = resolution?.capability || {};
  const required = list(capability?.input_schema?.required)
    .map((field) => text(field))
    .filter(Boolean);
  const question = required.length
    ? `I matched this to ${text(capability.key)}, but I could not safely bind all required action values. Please provide or restate: ${required.join(", ")}.`
    : `I matched this to ${text(capability.key)}, but I could not safely bind the requested action payload. Please restate the material values for that action.`;
  return {
    ...object(parsed),
    response_text: question,
    intent: "clarify",
    confidence: Math.max(0.95, Number(parsed?.confidence || 0)),
    clarification: { required: true, question, options: [] },
    navigation: { target_id: null },
    execution: { capability_key: null, payload: {}, reason: "ACTION_RESOLUTION_REPAIR_REQUIRED" },
  };
}

export function fastCurrentStateEvidenceRequired(request = {}) {
  const message = text(request?.user_input?.message);
  if (!message || !CURRENT_STATE_EVIDENCE_PATTERN.test(message)) return false;

  const reads = list(request?.executable_capabilities).filter(
  ...
  const top = list(resolution?.ranked)[0] || null;
  if (!top) return false;

  return (
    Number(top.score || 0) >= 0.16 ||
    Number(top.phrase_affinity || 0) >= 0.35 ||
    Number(top.primary_coverage || 0) >= 0.25
  );
}

const MULTI_READ_COMPARISON_PATTERN = /\b(compare|comparison|versus|vs|against|match(?:es|ed|ing)?|cross[- ]?check|reconcile|enough\b[^.!?]{0,80}\bfor|at risk because of|impact on|effect on|affect(?:s|ed|ing)?)\b/i;
const MULTI_READ_SPECIALIZED_SINGLE_CAPABILITIES = new Set([
  "platform.research_compare.analyze",
]);

function multiReadClauses(message) {
  const source = text(message);
  const enoughFor = source.match(/^(.*?\benough\b.+?)\bfor\b(.+)$/i);
  if (enoughFor) {
    return [enoughFor[1], enoughFor[2]]
      .map((item) => item.trim())
      .filter((item) => tokens(item).length >= 2)
      .slice(0, 6);
  }
  return source
    .split(/\b(?:compare|comparison|versus|vs|against|match(?:es|ed|ing)?|cross[- ]?check|reconcile|with|and does|and tell|at risk because of|impact on|effect on|affect(?:s|ed|ing)?)\b|[,;]/i)
    .map((item) => item.trim())
    .filter((item) => tokens(item).length >= 2)
    .slice(0, 6);
}

function strongReadMatch(entry) {
  return Boolean(
    entry && (
      Number(entry.score || 0) >= 0.075 ||
      Number(entry.primary_coverage || 0) >= 0.10 ||
      Number(entry.phrase_affinity || 0) >= 0.3 ||
      Number(entry.concept_affinity || 0) >= 0.35 ||
      Number(entry.clause_affinity || 0) >= 0.3
    )
  );
}

export function resolveOperatorMultiReadRequirement({ message, capabilities = [] } = {}) {
  const sourceMessage = text(message);
  if (!sourceMessage || !MULTI_READ_COMPARISON_PATTERN.test(sourceMessage)) return null;
  const reads = list(capabilities).filter((capability) =>
    text(capability?.mode).toLowerCase() === "read" &&
    text(capability?.key) !== OPERATOR_READ_CHAIN_KEY
  );
  if (reads.length < 2) return null;

  const fullRanked = rankOperatorCapabilities({
    message: sourceMessage,
    capabilities: reads,
    modes: ["read"],
    limit: 12,
  });
  const top = fullRanked[0] || null;
  const second = fullRanked[1] || null;
  if (
    top &&
    MULTI_READ_SPECIALIZED_SINGLE_CAPABILITIES.has(text(top.capability?.key)) &&
    (!second || Number(top.score || 0) - Number(second.score || 0) >= 0.08)
  ) {
    return null;
  }
  ...
  const seen = new Set();
  const add = (entry) => {
    const key = text(entry?.capability?.key);
    if (!key || seen.has(key) || !strongReadMatch(entry)) return;
    seen.add(key);
    selected.push(entry);
  };

  for (const clause of multiReadClauses(sourceMessage)) {
    const ranked = rankOperatorCapabilities({
      message: clause,
      capabilities: reads,
      modes: ["read"],
  ...
  return {
    capability_keys: selected.slice(0, 4).map((entry) => text(entry.capability?.key)),
    ranked: selected.slice(0, 4).map((entry) => ({
      capability_key: text(entry.capability?.key),
      score: Number(entry.score || 0),
    })),
    read_chain_available: list(capabilities).some(
...
function multiReadDecisionAccepted(parsed, requirement) {
  if (!requirement) return true;
  const intent = normalizeIntent(parsed?.intent);
  if (intent === "clarify") return fastVoiceSafeClarification(parsed);
  if (intent !== "execute") return false;
  const capabilityKey = text(parsed?.execution?.capability_key || parsed?.capability_key);
  if (capabilityKey !== OPERATOR_READ_CHAIN_KEY) return false;
  const execution = object(parsed?.execution);
  const stepKeys = new Set(
    list(execution?.payload?.steps).map((step) => text(step?.capability_key)).filter(Boolean),
  );
  return list(requirement.capability_keys).every((key) => stepKeys.has(text(key)));
}

function unresolvedMultiReadClarification(parsed, requirement) {
  const keys = list(requirement?.capability_keys).map((key) => text(key)).filter(Boolean);
  const question = `I need the exact records or filters needed to compare ${keys.join(" and ")} without answering from partial evidence. What scope should I use?`;
  return {
    ...object(parsed),
    response_text: question,
    intent: "clarify",
    ...
    clarification: { required: true, question, options: [] },
    navigation: { target_id: null },
    execution: { capability_key: null, payload: {}, reason: "MULTI_READ_EVIDENCE_REQUIRED" },
    plan: [],
  };
}
...
Be a useful collaborative partner: develop the idea, identify the important tradeoff, and choose the best safe next step when the recorded context is sufficient. Do not wait for the user to ask "what do you recommend?" when a materially useful next move, risk, opportunity, or stronger alternative is already clear from verified context. Offer that recommendation naturally, while keeping recommendation separate from authorization or execution.
For material recommendations, use a proof-oriented decision discipline: separate verified evidence from inference and proposal; test the preferred option against the strongest credible alternative; identify the uncertainty most likely to reverse the choice; and recommend the smallest next evidence or action that resolves that uncertainty. Do not present confidence as proof.
Never claim a fix, deployment, business effect, or mission completion without the exact durable verification required by the corresponding capability or runtime contract. Recommendation quality never increases execution authority.
For creative work such as video, music, campaigns, design, storytelling or product concepts, act like a strong creative partner rather than a command parser: use imagination, propose original alternatives, connect ideas across the existing project context, and think beyond the user's literal wording while preserving their intent and constraints.
When the user asks for ideas or a better creative direction, give concrete alternatives with a point of view. Prefer 2 to 4 genuinely different options over generic advice, and say which one you would choose and why when that helps.
Short creative discussion must remain conversational. Do not request Deep cognition merely because the user asks what you think, asks for ideas, challenges a scene, or explores a what-if. Escalate only when the problem is materially complex, evidence-heavy, or explicitly asks for deep analysis.
...
If the user's request requires evidence-first reads followed by one concrete downstream business action, you may construct platform.operator_read_chain.execute on the fast path with 1 to 4 exact read steps and one exact follow_up when the supplied capability metadata and user request are sufficient to bind it safely. The read chain only stages the follow_up; it never executes the write. Include verify_after when a supplied registered read can verify the resulting business effect. If the exact action, payload, scope, or verification cannot be bound from the supplied candidates and current context, set needs_full_catalog to true instead of guessing.
...
When the request explicitly asks for current, live, latest, status, or verified business state and a materially matching read capability is supplied, you must select that read capability (or set needs_full_catalog=true). Do not answer from model knowledge, old conversation, or with a generic availability/refusal sentence.
...
import { fastConversationalLowConfidenceRequiresDeep } from "./OperatorFastFallbackPolicy.js";

export function fastVoiceFallbackReason(parsed, request, fullCapabilities = []) {
  ...
  const multiReadRequirement = resolveOperatorMultiReadRequirement({
    message: request?.user_input?.message,
    capabilities: request?.executable_capabilities,
  });
  if (multiReadRequirement && !multiReadDecisionAccepted(parsed, multiReadRequirement)) {
    return "multi_read_chain_required";
  }

  const currentStateEvidenceRequired = fastCurrentStateEvidenceRequired(request);
  if (currentStateEvidenceRequired && intent !== "execute") {
    return "current_state_read_required";
  }

  if (actionResolutionRepairRequired(parsed, request, fullCapabilities)) {
    return "explicit_write_action_requires_deep";
  }

  if (fastConversationalLowConfidenceRequiresDeep({
    parsed,
    intent,
  ...
  if (intent === "execute") {
    ...
    // Fast cognition is allowed to select one exact governed write/draft/approve action.
    // Selection is not execution authority: Operator governance still enforces required
    // inputs, permissions, confirmation/approval, transactional policy, and verification.
    // Likewise, an evidence-first read chain may stage one exact follow-up action; the
    // read-chain capability itself never executes that mutation.
...
export async function reasonAboutOperatorTurn({
  ...
  const attachmentReflex = resolvePreparedAttachmentReflex({
    message,
    entityId,
  ...
  if (attachmentReflex) {
    const clarificationEntry = list(conversationAttachments).find((entry) =>
      preparedAttachmentNeedsIntelligence(entry)
    );
    if (clarificationEntry) {
      try {
        const intelligence = await reasonPreparedAttachmentClarification({
          organizationId, partyId, entityId, locale, message,
          candidate: object(clarificationEntry.prepared_candidate), attachment: clarificationEntry,
        });
        if (intelligence.clarification_required) {
          const question = text(intelligence.question);
          const decision = {
            ...attachmentReflex, response_text: question, intent: "clarify",
            clarification: { required: true, question, options: [] },
            execution: { capability_key: null, payload: {}, reason: null }, plan: [],
          };
          return { decision: normalizeOperatorDecision(decision, { projectState }), provider_evidence: intelligence.provider_evidence };
        }
      } catch (error) {
        console.error("PREPARED_ATTACHMENT_CLARIFICATION_INTELLIGENCE_FALLBACK", error);
      }
    }
    return {
      decision: normalizeOperatorDecision(attachmentReflex, { projectState }),
      provider_evidence: {
        provider: "avantiqo-local",
        model: "prepared-attachment-reflex-v1",
        usage_id: null,
      },
    };
  }
  ...
  if (reflex?.matched && reflex.execute) {
    return {
      decision: normalizeOperatorDecision({
        response_text: `Reading current ${text(reflex?.capability?.capability || reflex.capability_key).replace(/[._-]+/g, " ")} for the selected business context.`,
        response_language: text(locale) || null,
        intent: "execute",
  ...
  if (useFastVoice) {
    ...
    try {
      ...
      fastVoiceFallbackReasonCode = fastVoiceFallbackReason(fastParsed, fastRequest, capabilities);
...
`;
  ...
  const hierarchicalAnchor = operatorHierarchicalAnchor(reasoningRequest);
  const hierarchicalFinalInstructions = operatorHierarchicalFinalInstructions();
  ...
  try {
    ...
    ...ownedOperatorIntelligenceSelectionPolicy(),
    input: {
      input: JSON.stringify(reasoningRequest),
      instructions_text: instructions,
      hierarchical_anchor: hierarchicalAnchor,
      hierarchical_final_instructions: hierarchicalFinalInstructions,
      hierarchical_max_chunks: voice ? 4 : 6,
      hierarchical_child_timeout_ms: voice ? 45000 : 60000,
    ...
    metadata: {
      ...
      temporal_grounding: true,
      completed_step_context: true,
      provider_hierarchical_local_allowed: true,
      operator_hierarchical_anchor_contract: hierarchicalAnchor.contract,
      entity_scope_guard: true,
      read_chain_nested_entity_guard: true,
    ...
    const capacityDecision = localDeepCapacityDecision({
      error, agreementState, projectState, locale,
    });
    if (capacityDecision) return capacityDecision;
    throw error;
  }
  ...
  let parsed = parseOperatorReasoningResponse(rawText);

  if (!parsed) {
    throw new Error("OPERATOR_REASONING_INVALID_RESPONSE");
  }

  const actionResolution = actionResolutionRepairRequired(parsed, request, capabilities);
  let actionResolutionRepairUsed = false;
  let initialReasoningUsageId = null;
  if (actionResolution) {
    initialReasoningUsageId = execution?.usage?.id || null;
    let repairedExecution = await ServiceExecutionRuntime.execute({
      organization_id: organizationId,
      party_id: partyId,
      entity_id: entityId,
      service_id: "ai.reasoning.execute",
      ...
      input: {
        input: JSON.stringify({
          ...reasoningRequest,
          server_action_resolution: {
            capability_key: actionResolution.capability.key,
            action_clause: actionResolution.action_clause,
            input_schema: actionResolution.capability.input_schema || null,
            authorization_effect: "NONE",
          },
        }),
        instructions_text: `${instructions}\n\nACTION RESOLUTION REPAIR: The previous reasoning response incorrectly returned answer/plan without resolving an explicit business action. The server identified server_action_resolution.capability_key as the one strong registered write-action match. Re-evaluate the original user_input.message. Return either (1) intent execute with exactly that capability_key and a payload containing only values explicitly supplied by the user/current business context, or (2) intent clarify with one focused question for material values that are actually missing. Do not return answer or plan. Do not invent values. This repair has no authorization effect and does not execute anything.`,
        max_output_tokens: voice ? 480 : 1200,
        response_format: { type: "json_object" },
      },
      metadata: {
        module: "OPERATOR",
        operation: "REASON_TURN_ACTION_RESOLUTION_REPAIR",
        channel: text(source) || "text",
        action_resolution_capability_key: actionResolution.capability.key,
        action_resolution_repair_only: true,
        raw_reasoning_persisted: false,
      },
      category: "AI",
    });
    repairedExecution = await settleOperatorIntelligenceExecution({
      organization_id: organizationId,
      ...
      metadata: {
        module: "OPERATOR",
        operation: "REASON_TURN_ACTION_RESOLUTION_REPAIR_SETTLEMENT",
        channel: text(source) || "text",
        raw_reasoning_persisted: false,
      },
    });
    const repairedParsed = parseOperatorReasoningResponse(findText(repairedExecution));
    parsed = actionResolutionRepairAccepted(repairedParsed, actionResolution)
      ? repairedParsed
      : unresolvedActionClarification(parsed, actionResolution);
    if (repairedParsed && actionResolutionRepairAccepted(repairedParsed, actionResolution)) {
      execution = repairedExecution;
    }
    actionResolutionRepairUsed = true;
  }
  ...
  let multiReadRepairUsed = false;
  let multiReadInitialUsageId = null;
  if (multiReadRequirement && !multiReadDecisionAccepted(parsed, multiReadRequirement)) {
    multiReadInitialUsageId = execution?.usage?.id || null;
    let repairedExecution = await ServiceExecutionRuntime.execute({
      organization_id: organizationId,
      party_id: partyId,
      entity_id: entityId,
      ...
      input: {
        input: JSON.stringify({
          ...reasoningRequest,
          server_multi_read_requirement: {
            capability_key: OPERATOR_READ_CHAIN_KEY,
            required_read_capability_keys: multiReadRequirement.capability_keys,
            authorization_effect: "NONE",
          },
        }),
        instructions_text: `${instructions}\n\nMULTI-READ EVIDENCE REPAIR: The previous reasoning response attempted to answer a comparison/cross-check request without all materially required current evidence. The server identified server_multi_read_requirement.required_read_capability_keys from the registered read catalog. Return either (1) intent execute using exactly platform.operator_read_chain.execute with 2 to 4 read steps that include every required_read_capability_key and payload values only from the user/current business context, or (2) intent clarify with one focused question only when a material payload value is truly missing. Do not answer from one read, do not invent facts, and do not add any write follow_up. This repair grants no authority.`,
        max_output_tokens: voice ? 560 : 1400,
        response_format: { type: "json_object" },
      },
      metadata: {
        module: "OPERATOR",
        operation: "REASON_TURN_MULTI_READ_REPAIR",
        channel: text(source) || "text",
        required_read_capability_keys: multiReadRequirement.capability_keys,
        multi_read_repair_only: true,
        raw_reasoning_persisted: false,
      },
      category: "AI",
    });
    repairedExecution = await settleOperatorIntelligenceExecution({
      ...
      metadata: {
        module: "OPERATOR",
        operation: "REASON_TURN_MULTI_READ_REPAIR_SETTLEMENT",
        channel: text(source) || "text",
        raw_reasoning_persisted: false,
      },
    });
    const repairedParsed = parseOperatorReasoningResponse(findText(repairedExecution));
    if (repairedParsed && multiReadDecisionAccepted(repairedParsed, multiReadRequirement)) {
      parsed = repairedParsed;
      execution = repairedExecution;
    } else {
      parsed = unresolvedMultiReadClarification(parsed, multiReadRequirement);
    }
    multiReadRepairUsed = true;
  }
  ...
  return {
    ...
    provider_evidence: {
      ...providerEvidence(execution),
      action_resolution_repair_used: actionResolutionRepairUsed,
      action_resolution_initial_usage_id: initialReasoningUsageId,
      multi_read_repair_used: multiReadRepairUsed,
      multi_read_initial_usage_id: multiReadInitialUsageId,
    },
  };
}
