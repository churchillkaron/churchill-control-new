...
export function resolveRecommendationRefinementCapability({
  ...
  if (restoredOriginal) {
    ...
    return {
      ...
      separation: null,
      resolution_text_source: "RESTORED_ORIGINAL_IDENTITY",
    };
  }

  const capabilityClarification = text(
    proposal?.capability_resolution_text,
  ).slice(0, 4000);
  const proposalText = text(proposal?.proposal_text).slice(0, 4000);
  const resolutionText = capabilityClarification || proposalText;
  if (!resolutionText || !available.length) {
    return {
      capability: null,
      ...
      top_primary_coverage: null,
      separation: null,
      resolution_text_source: capabilityClarification
        ? "EXPLICIT_CAPABILITY_CLARIFICATION"
        : "SELECTED_REFINEMENT_PROPOSAL",
    };
  }
