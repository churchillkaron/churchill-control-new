...
const CONTRACT = "AVANTIQO_OPERATOR_REPAIR_SUPERVISION_V2";
...
function systemInstructions() {
  return [
    ...
    "Diagnose only from the supplied observed execution evidence, durable project context, and any live read-only evidence you obtain with operator_live_read.",
    "Use a registered live read when it can distinguish between plausible causes or verify whether the blocker still exists.",
  ...
  try {
    const supervised = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
      ...
      critique_instructions: [
        ...
        "If another registered live read can safely resolve uncertainty, use it. Correct the JSON without changing its keys.",
        "If the cause still cannot be known from evidence, require reinspection instead of guessing.",
      ].join(" "),
