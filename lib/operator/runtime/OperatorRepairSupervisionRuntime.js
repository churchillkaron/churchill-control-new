...
export async function superviseOperatorFailure({
  ...
  try {
    ...
    return {
      ...
      execution_failed: eligibility.execution_failed === true,
      verification_failed: eligibility.verification_failed === true,
      recovery_classification: supervisorTransient ? "TRANSIENT_RUNTIME" : recoveryClassification.classification,
      initial_recovery_classification: recoveryClassification.classification,
      code_engineering_candidate: false,
      repair: supervisorTransient ? {
        diagnosis: "REPAIR_SUPERVISOR_TRANSIENT_TIMEOUT",
        repairable: false,
