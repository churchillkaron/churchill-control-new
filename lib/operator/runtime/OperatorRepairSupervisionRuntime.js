...
const CONTRACT = "AVANTIQO_OPERATOR_REPAIR_SUPERVISION_V1";
...
function systemInstructions() {
  return [
    ...
    "Diagnose only from the supplied observed execution evidence and durable project context.",
    "Do not claim the issue is fixed. Do not retry or execute anything in this phase.",
    ...
    "Return exactly one JSON object with keys: diagnosis, repairable, proposed_next_step, evidence_needed, retry_policy, needs_human, question.",
  ...
  try {
    const supervised = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
      ...
      critique_instructions: [
        "Audit the repair proposal for unsupported diagnosis, unsafe retry behavior, governance bypass, and assumptions not supported by the supplied execution evidence.",
        "Correct the JSON without changing its keys. If the cause cannot be known from evidence, require reinspection instead of guessing.",
      ].join(" "),
