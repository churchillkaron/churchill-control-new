...
function recoverySemanticsForResolved(classification, initial = {}) {
  if (classification === initial.classification) {
    return { failure_class: initial.failure_class || null, recovery_strategy: initial.recovery_strategy || null };
  }
  if (classification === "VERIFICATION_ONLY") return { failure_class: "BUSINESS_OUTCOME_FAILURE", recovery_strategy: "INVESTIGATE_BUSINESS_EFFECT" };
  if (classification === "TRANSIENT_RUNTIME") return { failure_class: "TRANSPORT_RUNTIME_FAILURE", recovery_strategy: "RETRY_INFRASTRUCTURE_SAFELY" };
  if (["DATA_OR_CLARIFICATION", "CONFIGURATION_OR_EXTERNAL", "HUMAN_GATE"].includes(classification)) return { failure_class: "PREREQUISITE_FAILURE", recovery_strategy: "REPAIR_PREREQUISITE" };
  if (classification === "PRODUCT_DEFECT_CANDIDATE") return { failure_class: "PREREQUISITE_FAILURE", recovery_strategy: "INVESTIGATE_CAPABILITY_IMPLEMENTATION" };
  return { failure_class: "MODEL_REASONING_FAILURE", recovery_strategy: "REPLAN_WITH_CRITIQUE" };
}
...
export async function superviseOperatorFailure({
  ...
  if (!eligibility.applicable) {
    return {
      ...
      failure_class: recoveryClassification.failure_class,
      recovery_strategy: recoveryClassification.recovery_strategy,
  ...
  try {
    ...
    return {
      ...
      failure_class: recoverySemanticsForResolved(resolvedRecoveryClassification, recoveryClassification).failure_class,
      recovery_strategy: recoverySemanticsForResolved(resolvedRecoveryClassification, recoveryClassification).recovery_strategy,
    ...
    const internalError = text(error?.message || error, 800);
    const supervisorTransient = /PENDING_SETTLEMENT_TIMEOUT|TIMEOUT|ETIMEDOUT|ECONNRESET|TEMPORAR|TRANSIENT/i.test(internalError);
    ...
    const fallbackClassification = supervisorTransient
      ? "TRANSIENT_RUNTIME"
      : recoveryClassification.classification;
    const fallbackSemantics = recoverySemanticsForResolved(
    ...
    return {
      contract: CONTRACT,
      applicable: true,
      reason: supervisorTransient
        ? "REPAIR_SUPERVISOR_TRANSIENT_TIMEOUT"
        : "REPAIR_SUPERVISOR_UNAVAILABLE",
      execution_failed: eligibility.execution_failed === true,
      verification_failed: eligibility.verification_failed === true,
      recovery_classification: fallbackClassification,
      initial_recovery_classification: recoveryClassification.classification,
      failure_class: fallbackSemantics.failure_class,
      recovery_strategy: fallbackSemantics.recovery_strategy,
      code_engineering_candidate: supervisorTransient
        ? false
        : recoveryClassification.code_engineering_candidate === true,
      repair: null,
