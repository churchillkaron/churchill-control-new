...
export function bindOperatorSelfEngineeringDecision({
  ...
  const ownerConstraints = list(request.owner_constraints)
    .map((item) => text(item, 500))
    .filter(Boolean)
    .slice(-8);
  const payload = capabilityKey === PORTFOLIO_KEY
    ? { business_goal: originalMessage, ...(ownerConstraints.length ? { owner_constraints: ownerConstraints } : {}) }
    : { focus: originalMessage, ...(ownerConstraints.length ? { owner_constraints: ownerConstraints } : {}) };
  return {
    applied: true,
