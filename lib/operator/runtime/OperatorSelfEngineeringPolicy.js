...
function semanticEngineeringIntent(options = {}) {
  const semantic = object(options.semanticUnderstanding);
  if (text(semantic.execution_domain, 40).toLowerCase() !== "product_engineering") return null;
  return {
    scope: text(semantic.engineering_scope, 40).toLowerCase() || "single",
    mode: text(semantic.engineering_mode, 40).toLowerCase() || (semantic.requires_mutation === true ? "change" : "inspect"),
    requiresMutation: semantic.requires_mutation === true,
  };
}
...
const READ_ONLY_CODE_INSPECTION_PATTERN = /\b(check|review|inspect|audit|analy[sz]e|assess)\b/i;
const READ_ONLY_REPORT_PATTERN = /\b(tell me|let me know|finished|what .*missing|what .*need(?:s)? (?:to )?fix|what do you think|report back)\b/i;
const DIRECT_CODE_CHANGE_PATTERN = /^(?:(?:please|can you|could you|would you|now|ok|okay)\s+)*(?:fix|repair|change|update|implement|build|add|rewrite|refactor|make)\b/i;
...
const DIRECT_CODE_EXECUTION_PATTERN =
  /^(?:(?:use|run|start|enter|open|launch|switch to|go to|go into)\s+(?:the\s+)?(?:avantiqo\s+)?code|(?:avantiqo\s+)?code)(?:\s+(?:now|please|now please))?$/i;
...
function isDirectCodeExecutionCommand(message, pathname) {
  const current = normalized(message);
  if (!current || !DIRECT_CODE_EXECUTION_PATTERN.test(current)) return false;
  return /^\/workspace(?:\/|$)/i.test(text(pathname, 500));
}
...
export function isAvantiqoEngineeringPortfolioRequest(options = {}) {
  const semantic = semanticEngineeringIntent(options);
  if (semantic) return semantic.scope === "portfolio";
...
export function isAvantiqoSelfEngineeringTopic({
  ...
  if (semanticEngineeringIntent({ semanticUnderstanding })) return true;
  ...
  const directCodeExecution = isDirectCodeExecutionCommand(message, pathname);
...
export function isAvantiqoReadOnlyCodeInspectionRequest(options = {}) {
  const semantic = semanticEngineeringIntent(options);
  if (semantic) return semantic.mode === "inspect" && semantic.requiresMutation !== true;

  const current = normalized(options.message);
  if (!current) return false;
  if (!READ_ONLY_CODE_INSPECTION_PATTERN.test(current)) return false;
  if (!READ_ONLY_REPORT_PATTERN.test(current)) return false;
  if (DIRECT_CODE_CHANGE_PATTERN.test(current)) return false;
  const technicalSurface = TECHNICAL_SURFACE_PATTERN.test(current);
  const namedProductArea = PORTFOLIO_PRODUCT_AREA_PATTERN.test(current);
  const ownedProduct = OWNED_PRODUCT_PATTERN.test(current);
  const workspaceSurface = /^\/workspace(?:\/|$)/i.test(text(options.pathname, 500));
  return technicalSurface && (namedProductArea || ownedProduct || workspaceSurface);
}

export function isAvantiqoSelfEngineeringRequest(options = {}) {
  const semantic = semanticEngineeringIntent(options);
  if (semantic) return semantic.mode === "change" || semantic.requiresMutation === true;

  const current = normalized(options.message);
  const directCodeExecution = isDirectCodeExecutionCommand(
    options.message,
    options.pathname,
  );
  if (
    !current ||
    (!DEFECT_OR_ACTION_PATTERN.test(current) && !directCodeExecution)
  ) {
    return false;
  }
