...
import { materializeRegisteredRevisionDeterministically as compileRegisteredRevision } from "./OperatorRegisteredRevisionMaterializer.mjs";
...
function latestAgreementExecution(agreementState = {}) {
  ...
  for (let index = steps.length - 1; index >= 0; index -= 1) {
    ...
    return {
      ...
      request_payload: object(step.payload),
...
function priorCompletedActionPayload(options = {}) {
  const execution = object(effectiveLastExecution(options));
  return object(execution.request_payload || execution.payload || object(execution.result).request_payload);
}

function bindDeclaredReadsToPriorAction(options, action, reads) {
  const priorPayload = priorCompletedActionPayload(options);
  const priorPartyId = text(priorPayload.party_id || priorPayload.partyId, 200);
  return list(action?.operator_preparation_reads).map((entry) => {
    const read = object(entry);
    const registered = reads.find((candidate) => text(candidate?.key, 300) === text(read.capability_key, 300));
    const queryFields = new Set(list(registered?.direct_query_fields).map((field) => text(field, 120)));
    const payload = { ...object(read.payload) };
    if (priorPartyId && (queryFields.has("party_id") || queryFields.has("partyId"))) payload.party_id = priorPartyId;
    return { ...read, payload };
  });
}

function deterministicRegisteredRevisionPlan(options, actions, reads) {
  const understanding = object(options.semanticUnderstanding);
  if (understanding.deterministic_registered_revision !== true) return null;
  const capabilityKey = text(understanding.registered_write_capability_key, 300);
  ...
  return {
    ...
    plan: {
      capability_key: capabilityKey,
      evidence_reads: bindDeclaredReadsToPriorAction(options, action, reads),
      payload: {},
      materialization_instruction: text(options.message, 3000),
      clarification_required: false,
      clarification_question: null,
      summary: text(understanding.user_goal, 1000) || text(options.message, 1000),
      deterministic_registered_revision: true,
    },
    provider: {
      provider: "avantiqo-local",
      model: "registered-revision-compiler-v1",
      usage_id: null,
...
function materializeRegisteredRevisionDeterministically(options = {}, action = {}, evidence = []) {
  const understanding = object(options.semanticUnderstanding);
  if (understanding.deterministic_registered_revision !== true) return null;
  const parsed = compileRegisteredRevision({
    message: options.message,
    priorPayload: priorCompletedActionPayload(options),
    actionKey: action.key,
    evidence,
    correctionDate: temporalReference(options.timezone).today,
  });
...
export async function prepareSemanticGovernedAction(options = {}) {
  ...
  const deterministicRevision = deterministicRegisteredRevisionPlan(options, actions, reads);
  const plannedExecution = deterministicRevision?.provider || await planAction(options, actions, reads);
  const plan = deterministicRevision?.plan || parseJson(plannedExecution.text);
  if (!plan) {
  ...
  const action = deterministicRevision?.action || actions.find((candidate) => text(candidate.key, 300) === capabilityKey);
  if (!action) {
    return clarificationTurn({
  ...
  const materialized =
    materializeRegisteredRevisionDeterministically(options, action, evidence) ||
    await materializeAction(options, action, plan, evidence);
