    ...
    decision: {
      response_text:
        status === "INVALIDATED_BY_VERIFIED_CHANGE"
          ? "I stopped before executing because the evidence supporting this recommendation has materially changed. The selected action is preserved, but it must be reconsidered before anything mutates."
...
export async function runOperatorTurn(options = {}) {
  ...
  const required = pendingControlDecision
    ? false
    : semanticDeepRequired || needsOwnedCognitiveBrief({
        source: effectiveOptions.source,
        message: effectiveOptions.message,
      });
