...
async function executeCapability({
  ...
  const actorName =
    text(actor?.name || actor?.email) || null;
...
export async function runOperatorTurn({
  ...
  // A pending confirmation is only live while the user is actually answering it.
  // Previously any other message left it in place, which blocked the fast
  // conversational path and instant navigation, kept feeding the stale action
  // into the planning prompt so later answers were steered back to it, and never
  // cleared, so the turn stayed stuck on it indefinitely. Asking "how are you"
  // while a supply chain invoice was pending returned an answer about that
  // invoice. Anything that is not a yes or a no now supersedes it.
  ...
  if (pending && isAffirmative(message)) {
    ...
    try {
      ...
      const governanceState = clearedAgreementState(agreementState);
    ...
    try {
      ...
      responseText =
        verification?.decision?.response_text ||
        responseText;
