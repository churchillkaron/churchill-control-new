...
const CONTEXTUAL_RECOMMENDATION_PATTERN = /^(why|why not|how so|tell me more|explain that|what do you mean|what does that mean|what are the tradeoffs|what about(?:\s+.+)?|is that a good idea|is this a good idea|and then|then what)\s*[?.!]*$/i;
...
function pendingReplyClass(message, agreementState, recommendation) {
  return classifyPendingOperatorReply({
    message,
    pending: hasPendingExecution(agreementState),
...
function validatedRecommendation(candidate, options, result) {
  ...
  return {
    ...
    source: attentionResultBody(result)
      ...
      : "operator_reasoning_recommendation",
  };
}

function postCommitContinuationCandidate(result, capabilities) {
  if (text(result?.execution?.capability?.key) !== OPERATOR_MISSION_KEY) return null;
  const mission = object(result?.execution?.result);
  ...
  const continuation = object(continuationStep?.result);
  if (text(continuation.status) !== "READY_FOR_ONE_NEXT_BOUNDED_CYCLE") return null;
  ...
  const capability = capabilityByKey(capabilities, PRODUCT_ENGINEERING_CYCLE_KEY);
  ...
  return {
    capability,
    payload: { focus },
    reason: `Product Intelligence reassessed verified current main and selected the next bounded objective: ${focus}`.slice(0, 800),
    objective: focus,
  };
}
...
function postCommitContinuationText(recommendation) {
  ...
  return focus
    ? `The verified commit is complete. Product Intelligence reassessed current main and selected the next bounded objective: ${focus} Say “do it” when you want me to run that next engineering cycle.`
    : "The verified commit is complete. Product Intelligence selected one next bounded engineering cycle. Say “do it” when you want me to run it.";
}
...
function promoteEmbeddedPersistenceMission(result, options) {
  ...
  const baseAgreement = responseAgreementState(result, options.agreementState);
...
export async function runOperatorTurn(options = {}) {
  ...
  const safeCapabilities = allCapabilities.filter((capability) =>
    recommendationEligible(capability, options),
  );
  const postCommitCandidate = postCommitContinuationCandidate(result, safeCapabilities);
  if (postCommitCandidate) {
    const nextRecommendation = validatedRecommendation(
      postCommitCandidate,
      options,
      result,
    );
    if (nextRecommendation) {
      ...
      nextAgreementState = agreementWithOperatorRecommendation(
        ...
        {
          objective:
            text(postCommitCandidate.objective) ||
            text(result?.decision?.project_state?.objective) ||
            text(options.projectState?.objective),
        },
      );
