...
function embeddedPersistenceMission(result) {
  if (text(result?.execution?.capability?.key) !== PRODUCT_PERSISTENCE_HANDOFF_KEY) {
    return null;
  }
  const handoff = object(result?.execution?.result);
  ...
  return {
    ...
    decision: {
      ...
      ...(persistedRecommendation
        ? {
            response_text: appendExecutionOffer(baseDecision.response_text),
            intent: baseDecision.intent === "answer" ? "plan" : baseDecision.intent,
          }
        : {}),
      agreement_state: nextAgreementState,
      project_state: nextProjectState,
