...
export async function runOperatorTurn({
      ...
      decision: {
        ...
        response_text:
          capability?.requires_confirmation
            ? `${decision.response_text} I have prepared the action, but it requires confirmation before I can execute it.`
            : `${decision.response_text} This capability is connected to the Operator, but automatic execution has not been enabled for it yet.`,
        clarification: {
          required: capability?.requires_confirmation === true,
          question:
  ...
  return {
    ...
    decision: {
      ...decision,
      response_text:
        decision.response_text || "Done. The requested Avantiqo action completed successfully.",
    },
  };
}
