...
function withCodeCustomerArtifactReply(result) {
  const source = object(result);
  const artifact = findCodeAICustomerArtifact(source);
...
export async function runOperatorTurn(options = {}) {
  ...
  return runWithOperatorIntelligenceExecutionGuard(guard, async () => {
    try {
      ...
      const guarded = stagedMutationRequiresCognitiveBlock(result, guard)
        ? cognitiveMutationBlockedTurn(effectiveOptions, guard, result)
        : result;
      return withCodeCustomerArtifactReply(guarded);
    } catch (error) {
