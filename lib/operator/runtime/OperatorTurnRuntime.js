...
function boundedContinuationHandoff(value) {
  ...
  if (text(continuation.status) !== "READY_FOR_ONE_NEXT_BOUNDED_CYCLE") {
    return null;
  }
  ...
  if (!focus || handoff.automatic_execution_started === true) return null;
  return { focus };
}
...
function postCommitContinuationHandoff(result) {
  ...
  if (capabilityKey === PRODUCT_PERSISTENCE_HANDOFF_KEY) {
    ...
    if (direct) return { ...direct, source: "verified_persistence_handoff" };
  }
