...
const RECOMMENDATION_ALTERNATIVE_PATTERN = /^(what about\s+.+|how about\s+.+|what if\s+.+|could we\s+.+|would it be better to\s+.+|should we\s+.+\s+instead|(?:change|switch|replace|adjust|refine|modify)\s+.+|.+\binstead\b.+|.+\brather than\b.+)\s*[?.!]*$/i;
...
function agreementWithRecommendationDisarmed(agreementState, recommendation) {
...
async function recommendationDiscussionTurn(options, recommendation, kind) {
  ...
  const responseText = alternative
    ? `${baseText} I’m treating that as an alternative, so the previous recommendation is paused while we compare them. Nothing will run until we settle the exact direction.`.trim()
    : baseText;
