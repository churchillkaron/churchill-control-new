...
} from "./OperatorSelfEngineeringDecisionBinding.js";
import {
  buildImplementationRepairResumeBinding,
  validateImplementationRepairResumeBinding,
} from "./OperatorImplementationRepairResumeBinding.js";
import {
...
import { findOperatorFastAction } from "./OperatorFastActionIndex.js";
import {
...
import {
  ...
  resolveExplicitWriteActionRequest,
} from "./OperatorReasoningRuntime";
import { prepareDeterministicGovernedAction } from "./OperatorDeterministicActionPreparationRuntime.js";
import { prepareSemanticGovernedAction } from "./OperatorSemanticActionPreparationRuntime.js";
...
import {
  recordBusinessPartnerCancellationProductEvidence,
} from "./BusinessPartnerProductEvidenceRuntime.js";
import {
  assertAvantiqoLiveExecutionCurrent,
} from "@/lib/platform/runtime/AvantiqoLiveExecutionRuntime";
import {
  collectStableBusinessIdentities,
...
import { verifyPlatformSelfHealingReplay } from "@/lib/platform/self-healing/PlatformSelfHealingReplayVerificationRuntime.mjs";
...
import {
  ...
  getIntelligenceModalOverflowProposal,
  rejectIntelligenceModalOverflowProposal,
} from "@/lib/platform/service-runtime/governance/IntelligenceModalOverflowProposalRuntime.js";
...
const FINANCE_VENDOR_BILL_KEY = "finance.vendor_bills.create";
const SUPPLY_CHAIN_VENDOR_INVOICE_COST_KEY = "supply-chain.purchase_costs.apply_vendor_invoice";
const MUSIC_PLAN_WORLD_CLASS_KEY = "creative.music.planWorldClassProduction";
const MUSIC_EXECUTE_WORLD_CLASS_KEY = "creative.music.executeWorldClassProduction";
const MUSIC_REVIEWED_EDIT_CONFIRMATION_CONTRACT = "AVANTIQO_MUSIC_REVIEWED_EDIT_CONFIRMATION_V1";
...
export function automaticProductReleaseAllowed(permissions = [], environment = process.env.NODE_ENV) {
...
function missionRunRequiresPendingExecutionBinding(run) {
  return Boolean(
    text(run?.run_id) &&
      ...
      ["awaiting_confirmation", "awaiting_approval", "waiting_external", "verifying"].includes(
        text(run?.status).toLowerCase(),
      ),
  );
}
...
function autonomousRunStatusText(run) {
  ...
  if (status === "waiting_external") {
    return currentDescription
      ? `The mission is waiting for external evidence at: ${currentDescription}. It will continue automatically when the exact registered event arrives.`
      : "The mission is waiting for an external event and will continue automatically when the exact registered evidence arrives.";
  }
  if (status === "executing") {
    return currentDescription
...
function explicitDirectActionAuthorization({ message, capability, capabilities = [], source = "text" } = {}) {
  if (!capability || text(source).toLowerCase() === "voice") return false;
  const mode = text(capability.mode).toLowerCase();
  const risk = text(capability.risk).toLowerCase();
  if (mode === "approve" || ["high", "critical"].includes(risk)) return false;
  const resolution = resolveExplicitWriteActionRequest({ message, capabilities });
  return Boolean(
    resolution?.capability?.key &&
    text(resolution.capability.key) === text(capability.key)
  );
}
...
function normalizedExecutionPayload({
  ...
  const businessPayload = payload && typeof payload === "object" ? payload : {};
  const businessPartyId = text(businessPayload.party_id || businessPayload.partyId) || null;
  return {
    ...businessPayload,
    organizationId,
    ...
    periodId,
    period_id: periodId,
    // party_id may identify the business subject (customer/vendor/employee).
    // Never overwrite that identity with the operator's own party id.
    partyId: businessPartyId || partyId,
    party_id: businessPartyId || partyId,
    operatorPartyId: partyId || null,
    operator_party_id: partyId || null,
  };
}
...
function valueAtPath(source, path) {
  ...
  if (!raw || raw.length > 240 || keys.length > 8) return null;
  if (keys.some((key) => !/^[A-Za-z_][A-Za-z0-9_]*$/.test(key) || ["__proto__", "prototype", "constructor"].includes(key))) return null;
  let current = source;
...
function resultBoundVerification(capability = {}, actionResult = null, actionPayload = {}) {
  const declaration = object(capability.operator_verification);
  const capabilityKey = text(declaration.capability_key);
  const bindings = object(declaration.payload_from_result);
  const arrayBindings = object(declaration.payload_array_from_result);
  const inputBindings = object(declaration.payload_from_input);
  if (!capabilityKey || (!Object.keys(bindings).length && !Object.keys(arrayBindings).length && !Object.keys(inputBindings).length)) return null;
  const payload = {};
  for (const [payloadKey, candidates] of Object.entries(bindings)) {
    const paths = Array.isArray(candidates) ? candidates : [candidates];
    const value = paths.map((path) => valueAtPath(actionResult, path)).find((item) => item !== null && item !== undefined && ["string", "number"].includes(typeof item) && text(item));
    if (value === undefined) return null;
    payload[payloadKey] = value;
  }
  for (const [payloadKey, configValue] of Object.entries(arrayBindings)) {
    const config = object(configValue);
    const source = valueAtPath(actionResult, config.path);
    if (!Array.isArray(source) || source.length > 500 || (source.length < 1 && config.allow_empty !== true)) return null;
    const itemPath = text(config.item_path);
    const values = source.map((item) => itemPath ? valueAtPath(item, itemPath) : item);
    if (values.some((item) => !["string", "number"].includes(typeof item) || !text(item))) return null;
    payload[payloadKey] = values;
  }
  for (const [payloadKey, sourceKeyValue] of Object.entries(inputBindings)) {
    const sourceKey = text(sourceKeyValue, 120);
    if (!sourceKey || !Object.prototype.hasOwnProperty.call(object(actionPayload), sourceKey)) return null;
    const value = actionPayload[sourceKey];
    ...
    if (Array.isArray(value)) {
      if (!value.length || value.length > 500) return null;
      payload[payloadKey] = value;
      continue;
    }
    return null;
  }
  return { capability_key: capabilityKey, description: `Verify ${text(capability.description) || capability.key || "created record"}`, payload };
}
...
function implementationRepairResumeAction(agreementState = {}, context = {}) {
  const candidate = object(agreementState?.implementation_repair_resume);
  const validation = validateImplementationRepairResumeBinding(candidate, context);
  ...
  return {
    ...
    resume_kind: "implementation_repair",
  };
}


function businessPartnerRecoveryResumeAction(agreementState = {}, context = {}) {
  const recovery = object(agreementState?.business_partner_recovery);
  const recoveryCapability = object(recovery.capability);
  const recoveryCapabilityKey = text(recoveryCapability.key);
  const recoveryPayload = object(recovery.payload);
  const replayBindingValidation = validateBusinessPartnerRecoveryReplayBinding(
    recovery.replay_binding,
    {
      organizationId: context.organizationId,
      entityId: context.entityId,
      periodId: context.periodId,
      partyId: context.partyId,
      capabilityKey: recoveryCapabilityKey,
      payload: recoveryPayload,
    },
  );
  if (!replayBindingValidation.valid) return null;
  const ambiguousWriteRecovery = object(recovery.ambiguous_write_recovery);
  const ambiguousWriteCompleted =
    text(recovery.contract) === "AVANTIQO_BUSINESS_PARTNER_RECOVERY_STATE_V1" &&
    text(ambiguousWriteRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_AMBIGUOUS_WRITE_RECOVERY_V1" &&
    text(ambiguousWriteRecovery.status) === "COMPLETED" &&
    ambiguousWriteRecovery.business_effect_verified === true &&
    ambiguousWriteRecovery.mutation_replay_allowed === false;
  if (ambiguousWriteCompleted) {
    if (!recoveryCapabilityKey) return null;
    return {
      capability_key: recoveryCapabilityKey,
      payload: recoveryPayload,
      reason: "Continue original mission after authoritative reinspection proved the ambiguous write already completed",
      original_message: text(recovery.original_goal).slice(0, 4000) || null,
      resume_kind: "business_partner_recovery",
      recovery_completed_without_replay: true,
    };
  }
  if (
    text(recovery.contract) !== "AVANTIQO_BUSINESS_PARTNER_RECOVERY_STATE_V1" ||
    recovery.resume_authorized !== true ||
    text(recovery.authorization_effect) !== "SAME_ACTION_ONLY"
  ) return null;
  const humanGateRecovery = object(recovery.human_gate_recovery);
  const verifiedHumanGateReinspection =
    text(humanGateRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_HUMAN_GATE_RECOVERY_V1" &&
    text(humanGateRecovery.status) === "READY_FOR_GOVERNED_REINSPECTION" &&
    humanGateRecovery.resume_authorized === true &&
    humanGateRecovery.pre_mutation_gate_required === true &&
    humanGateRecovery.mutation_completion_proven === false;
  const configurationRecovery = object(recovery.configuration_recovery);
  const verifiedSelfHealing =
    recovery.production_deploy_performed === true &&
    recovery.activation_verified === true &&
    recovery.replay_required === true;
  const verifiedConfigurationRecovery =
    text(configurationRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_CONFIGURATION_RECOVERY_V1" &&
    text(configurationRecovery.status) === "READY_TO_RETRY" &&
    configurationRecovery.auto_resume_allowed === true &&
    configurationRecovery.retry_attempted !== true;
  const dataRecovery = object(recovery.data_recovery);
  const verifiedDataRecovery =
    text(dataRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_DATA_RECOVERY_V1" &&
    text(dataRecovery.status) === "READY_TO_RETRY" &&
    dataRecovery.auto_resume_allowed === true &&
    dataRecovery.payload_validated === true &&
    dataRecovery.retry_attempted !== true;
  const ambiguousWriteNotCompleted =
    text(ambiguousWriteRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_AMBIGUOUS_WRITE_RECOVERY_V1" &&
    text(ambiguousWriteRecovery.status) === "NOT_COMPLETED" &&
    ambiguousWriteRecovery.mutation_replay_allowed === true &&
    ambiguousWriteRecovery.retry_requires_fresh_governance === true;
  const transientRecovery = object(recovery.transient_recovery);
  const verifiedTransientRecovery =
    text(transientRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_TRANSIENT_RECOVERY_V1" &&
    text(transientRecovery.status) === "READY_TO_RETRY" &&
    transientRecovery.auto_resume_allowed === true &&
    transientRecovery.read_only_retry === true &&
    transientRecovery.retry_attempted !== true;
  const externalBackoffRecovery = object(recovery.external_backoff_recovery);
  const verifiedExternalBackoffRecovery =
    text(externalBackoffRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_EXTERNAL_BACKOFF_RECOVERY_V1" &&
    text(externalBackoffRecovery.status) === "RETRY_DUE" &&
    externalBackoffRecovery.resume_authorized === true &&
    externalBackoffRecovery.read_only_retry === true &&
    externalBackoffRecovery.mutation_replay_allowed === false;
  if (!verifiedSelfHealing && !verifiedConfigurationRecovery && !verifiedTransientRecovery && !verifiedDataRecovery && !ambiguousWriteNotCompleted && !verifiedHumanGateReinspection && !verifiedExternalBackoffRecovery) return null;
  if (!recoveryCapabilityKey) return null;
  return {
    capability_key: recoveryCapabilityKey,
    payload: recoveryPayload,
    reason: verifiedExternalBackoffRecovery
      ? "Retry exact original read after durable external backoff"
      : verifiedHumanGateReinspection
      ? "Reinspect exact original action after human-resolved pre-mutation gate"
      : verifiedConfigurationRecovery
      ? "Resume exact original action after verified configuration recovery"
      : ambiguousWriteNotCompleted
        ? "Retry exact original action after authoritative reinspection proved it did not complete"
      : verifiedDataRecovery
        ? "Resume exact original action after required business data was supplied"
        : verifiedTransientRecovery
        ? "Retry exact original read after transient runtime failure"
        : "Resume exact original action after verified self-healing production release",
    original_message: text(recovery.original_goal).slice(0, 4000) || null,
    resume_kind: "business_partner_recovery",
    verify_after: normalizedPendingVerificationRead(recovery.verify_after),
  };
}

function pendingIntelligenceModalOverflowProposal(agreementState = {}) {
  const proposal = object(object(agreementState).intelligence_modal_overflow_proposal);
  return text(proposal.proposal_id) ? proposal : null;
}

function clearIntelligenceModalOverflowProposal(agreementState = {}, proposalId = null) {
  const source = object(agreementState);
  const current = pendingIntelligenceModalOverflowProposal(source);
  if (!current) return source;
  if (proposalId && text(current.proposal_id) !== text(proposalId)) return source;
  const next = { ...source };
  delete next.intelligence_modal_overflow_proposal;
  return next;
}

function modalOverflowDecision({ responseText, agreementState, projectState, locale, intent = "answer", question = null } = {}) {
  return {
    success: true,
    decision: {
      response_text: responseText,
    ...
    provider_evidence: {
      provider: "avantiqo-local",
      model: "modal-overflow-governance-v1",
      usage_id: null,
    ...
    operator_catalog: {
      navigation_target_count: 0,
      executable_capability_count: 0,
      bypassed_for_modal_overflow_governance: true,
      execution_authorized: false,
    },
  };
}
...
function pendingAction(agreementState = {}) {
  ...
  return {
    ...
    action_identity_evidence: Array.isArray(candidate.action_identity_evidence)
      ? candidate.action_identity_evidence.map((value) => text(value, 600)).filter(Boolean).slice(0, 50)
      : [],
  };
}
...
function missionStepProjection(value) {
  const candidate = object(value);
  const wait = object(candidate.wait_for_external);
  return {
    ...
    wait_for_external: Object.keys(wait).length
      ? {
          event_source: text(wait.event_source),
          event_type: text(wait.event_type),
          correlation_key: text(wait.correlation_key),
        }
      : null,
  };
}
...
function missionResumeProjectionMatches(pending, run) {
  ...
  const resumeRunId = text(resume.run_id);
  if (resumeRunId && resumeRunId !== text(run?.run_id)) return false;
  ...
  if (runStatus === "waiting_external") {
    return (
      confirmed &&
      !approvalRequestId &&
      !verificationStepId &&
      text(currentRunStep.status).toLowerCase() === "waiting_external"
    );
  }
...
function clearedAgreementState(agreementState = {}) {
  ...
  delete next.implementation_repair_resume;
  return next;
}

function clearBusinessPartnerRecovery(agreementState = {}) {
  const next = { ...object(agreementState) };
  delete next.business_partner_recovery;
  return next;
}
...
function missionPauseText(result, currentStep) {
  ...
  if (result?.pause_reason === "external") {
    return step
      ? `${prefix} The mission is waiting at ${step}. It will continue automatically when the exact registered external event arrives.`
      : `${prefix} The mission is waiting for an external event and will continue automatically when the exact registered evidence arrives.`;
  }
...
function capabilityIdentityFromKey(value) {
...
function missionFailureRecoveryState({ result, run, originalMessage, objective, context = {} }) {
  const evidence = object(result?.failure_evidence);
  const evidenceCapability = object(evidence.capability);
  const capability = capabilityIdentityFromKey(evidenceCapability.key);
  if (!capability || !run || text(run.run_kind).toLowerCase() !== "mission") return null;
  const failedStepId = text(evidence.step_id || run.current_step_id);
  const steps = Array.isArray(run.planned_steps) ? run.planned_steps : [];
  const failedIndex = steps.findIndex((step) => text(step?.id) === failedStepId);
  if (failedIndex < 0) return null;
  const failedStep = steps[failedIndex];
  if (text(failedStep.capability_key) !== capability.key) return null;
  const completed = Array.isArray(run.completed_steps) ? run.completed_steps.map(text).filter(Boolean) : [];
  const expectedPrefix = steps.slice(0, failedIndex).map((step) => text(step.id));
  if (completed.length !== expectedPrefix.length || completed.some((id, index) => id !== expectedPrefix[index])) return null;
  return {
    contract: "AVANTIQO_BUSINESS_PARTNER_RECOVERY_STATE_V1",
    status: "FAILED_MISSION_STEP_CAPTURED",
    original_goal: text(objective || originalMessage, 4000) || null,
    capability,
    payload: object(failedStep.payload),
    ...
    verify_after: normalizedPendingVerificationRead(
      object(evidence.verification_recovery).capability_key
        ? { capability_key: evidence.verification_recovery.capability_key, payload: object(evidence.verification_recovery.payload), description: "Reinspect exact failed write identity" }
        : failedStep.verify_after,
    ),
    ...
    mission_continuity: {
      contract: "AVANTIQO_BUSINESS_PARTNER_MISSION_RECOVERY_V1",
      run_id: text(run.run_id),
      objective: text(run.objective || objective || originalMessage, 1200) || null,
      failed_step_id: failedStepId,
      failed_step_index: failedIndex,
      completed_step_ids: completed,
      planned_steps: steps.map((step) => ({
        id: text(step.id),
        label: text(step.description || step.label),
        capability_key: text(step.capability_key),
        payload: object(step.payload),
        ...(step.verify_after ? { verify_after: normalizedPendingVerificationRead(step.verify_after) } : {}),
      })),
...
function businessPartnerMissionContinuation(agreementState = {}, pending = {}, context = {}) {
  const recovery = object(agreementState?.business_partner_recovery);
  const continuity = object(recovery.mission_continuity);
  if (text(continuity.contract) !== "AVANTIQO_BUSINESS_PARTNER_MISSION_RECOVERY_V1") return null;
  const run = autonomousRunFromAgreementState(agreementState);
  if (!run || text(run.run_kind).toLowerCase() !== "mission" || text(run.run_id) !== text(continuity.run_id)) return null;
  const steps = Array.isArray(continuity.planned_steps) ? continuity.planned_steps : [];
  const failedStepId = text(continuity.failed_step_id);
  const failedIndex = steps.findIndex((step) => text(step?.id) === failedStepId);
  if (failedIndex < 0) return null;
  const failedStep = steps[failedIndex];
  if (text(failedStep.capability_key) !== text(pending.capability_key) || !sameMissionValue(object(failedStep.payload), object(pending.payload))) return null;
  const priorCompleted = Array.isArray(continuity.completed_step_ids) ? continuity.completed_step_ids.map(text).filter(Boolean) : [];
  const expectedPrefix = steps.slice(0, failedIndex).map((step) => text(step.id));
  if (priorCompleted.length !== expectedPrefix.length || priorCompleted.some((id, index) => id !== expectedPrefix[index])) return null;
  const nextStep = steps[failedIndex + 1] || null;
  if (!nextStep) {
    return {
      completed: true,
      run_id: text(run.run_id),
      objective: text(run.objective) || null,
      completion_result: {
        status: "completed",
        ...
        mission_state: {
          status: "completed",
          completed_step_ids: steps.map((step) => text(step.id)),
          current_step_id: null,
          blocker: null,
          steps: steps.map((step) => ({
            id: text(step.id),
            kind: text(step.kind).toLowerCase() || "action",
            description: text(step.label || step.description),
            capability_key: text(step.capability_key),
            payload: object(step.payload),
            status: "completed",
            gate: "none",
  ...
  const actorId = text(context?.actor?.id || context?.actor?.user_id);
  if (!text(context.organizationId) || !text(context.partyId) || !actorId) return null;
  return {
    completed: false,
    run_id: text(run.run_id),
    objective: text(run.objective || continuity.objective) || null,
    payload: {
      steps,
      resume: {
        scope: {
          ...
          party_id: text(context.partyId),
          actor_id: actorId,
        },
        completed_step_ids: [...priorCompleted, failedStepId],
        current_step_id: text(nextStep.id),
...
function missionResultTurn({
  ...
  let nextAgreementState = missionResultAgreementState({
    agreementState,
    result,
    originalMessage,
    objective,
  });
  let run = autonomousRunFromAgreementState(nextAgreementState);
  const blockedMissionRecovery = text(result?.status) === "blocked"
    ? missionFailureRecoveryState({ result, run, originalMessage, objective, context: businessContext })
    : null;
  if (blockedMissionRecovery) {
    nextAgreementState = {
      ...object(nextAgreementState),
      business_partner_recovery: blockedMissionRecovery,
    };
    run = autonomousRunFromAgreementState(nextAgreementState);
  }
  ...
  return {
    ...
    execution: {
      ...
      result,
      ...(blockedMissionRecovery ? {
        failure_evidence: {
          ...object(result?.failure_evidence),
          original_goal: text(objective || originalMessage, 4000) || null,
          exact_action_bound: true,
          mission_run_id: text(run?.run_id) || null,
        },
      } : {}),
    },
    operator_catalog: {
...
function humanConfirmationQuestion(capability, payload = {}) {
  if (text(capability?.key) === "finance.accounts_receivable.CreateCustomerInvoice") {
    return "Do you want me to create this invoice?";
  }
  return "Should I proceed with that exact action?";
}
...
function pendingVerificationExecution(
  ...
  return {
    ...
    action_identity_evidence: Array.from(collectStableBusinessIdentities(actionResult)).slice(0, 50),
  };
}

function vendorInvoiceCostFollowUp({ capability, result, capabilities, agreementState, originalMessage, objective }) {
  if (text(capability?.key) !== FINANCE_VENDOR_BILL_KEY || result?.success === false) return null;
  if (object(result?.cost_evidence).persisted !== true) return null;
  const invoiceId = text(result?.invoice?.id || result?.vendor_invoice?.id || result?.id);
  const lines = Array.isArray(result?.lines) ? result.lines : [];
  const hasResolvedCostEvidence = lines.some((line) =>
    text(line?.item_id) && Number.isFinite(Number(line?.base_unit_cost)) && Number.isFinite(Number(line?.factor_to_item_base)) && Number(line.factor_to_item_base) > 0,
  );
  if (!invoiceId || !hasResolvedCostEvidence) return null;
  const followUpCapability = capabilities.find((item) => text(item?.key) === SUPPLY_CHAIN_VENDOR_INVOICE_COST_KEY);
  if (!followUpCapability) return null;
  const payload = { vendor_invoice_id: invoiceId };
  const reason = "Apply verified vendor invoice purchase costs and recalculate affected recipe/dish food costs";
  const nextAgreementState = agreementWithPendingConfirmationRun({
    agreementState, capability: followUpCapability, payload, reason, originalMessage, objective,
  });
  return { capability: followUpCapability, payload, nextAgreementState };
}

function musicReviewedEditFollowUp({ capability, result, capabilities, agreementState, originalMessage, objective }) {
  if (text(capability?.key) !== MUSIC_PLAN_WORLD_CLASS_KEY) return null;
  const direct = object(result?.result);
  const planned = Object.keys(direct).length ? direct : object(result);
  const validated = validateReviewedMusicEditContinuation(
    planned.reviewed_edit_reference,
    planned.reviewed_edit_continuation,
  );
  if (!validated.valid) return null;
  const reference = validated.reference;
  const payload = object(validated.payload);
  const followUpCapability = capabilities.find((item) => text(item?.key) === MUSIC_EXECUTE_WORLD_CLASS_KEY);
  if (!followUpCapability) return null;
  const confirmedPayload = { ...payload, source_rights_confirmed: true };
  const reason = "Confirm source-audio edit rights and execute the exact reviewed Music change";
  let nextAgreementState = agreementWithPendingConfirmationRun({
    agreementState, capability: followUpCapability, payload: confirmedPayload, reason, originalMessage, objective,
  });
  nextAgreementState = {
    ...nextAgreementState,
    pending_execution: {
      ...object(nextAgreementState.pending_execution),
      confirmation_contract: MUSIC_REVIEWED_EDIT_CONFIRMATION_CONTRACT,
      confirmation_question: "Do you confirm you have the right to edit this source audio and want me to execute this exact reviewed Music change?",
      reviewed_edit_reference: planned.reviewed_edit_reference,
      rights_attestation_pending: true,
    },
  };
  return { capability: followUpCapability, payload: confirmedPayload, nextAgreementState, reference };
}

function completedAgreementState(agreementState, pending, postActionVerification, actionResult = null) {
  let next = clearedAgreementState(agreementState);
  next = agreementWithRunTransition(next, {
  ...
  if (
    text(postActionVerification?.status).toLowerCase() === "completed" &&
    postActionVerification?.business_effect_verified === true
  ) {
...
async function executeCapability({
  ...
  const capabilityMode = text(capability?.mode).toLowerCase();
  if (capabilityMode !== "read") {
    ...
    await assertAvantiqoLiveExecutionCurrent({
      ...
      error_code: "OPERATOR_MUTATION_SUPERSEDED_EXECUTION",
    });
  }
  ...
  try {
    const result = await executeUbteCapability({
      ...
      runtime: {
        ...
        metadata: {
          ...
          operatorProjectState: normalizeOperatorProjectState(projectState),
    ...
    executionError.operatorCapability = {
      key: capability.key,
      domain: capability.domain,
      capability: capability.capability,
      action: capability.action,
    ...
    if (!executionError.operatorCapability) {
      executionError.operatorCapability = {
        key: capability.key,
        domain: capability.domain,
        capability: capability.capability,
    ...
    if (!executionError.operatorExecutionPayload) {
      executionError.operatorExecutionPayload = normalizedPayload;
    }
...
async function runPendingPostActionVerification({
  ...
  try {
    ...
    const deterministicProof = deterministicBusinessEffectProof({
      result: actionResult,
      action_identity_evidence: actionIdentityEvidence,
      post_action_verification: completedVerification,
    });
    const businessEffectOutcome = normalizeAuthoritativeBusinessEffectOutcome({
      result,
      expected: { action_result: actionResult, action_identity_evidence: actionIdentityEvidence },
      server_scope_bound: true,
    });
    const attestedVerification = {
      ...completedVerification,
      business_effect_verified: deterministicProof.passed === true,
      business_effect_outcome: deterministicProof.passed === true
        ? {
            ...
            safe_to_retry: false,
            matched_identity: deterministicProof.matched_identity || businessEffectOutcome.matched_identity || null,
          }
        : businessEffectOutcome,
      assertion: {
...
async function retryPendingVerification({
  ...
  return runPendingPostActionVerification({
    ...
    callerRequest,
    conversationId,
    actionIdentityEvidence: pending.action_identity_evidence,
  });
}
...
export async function runOperatorTurn({
  ...
  const currentBusinessContext = { organizationId, entityId, periodId, partyId };

  const modalOverflowProposal = pendingIntelligenceModalOverflowProposal(agreementState);
  if (modalOverflowProposal && text(modalOverflowProposal.status).toUpperCase() === "PROPOSED") {
    const proposalId = text(modalOverflowProposal.proposal_id);
    if (isNegative(message)) {
      try {
        const rejected = await rejectIntelligenceModalOverflowProposal({ organizationId, proposalId });
        if (!rejected) throw new Error("INTELLIGENCE_MODAL_OVERFLOW_PROPOSAL_REJECTION_NOT_RECORDED");
        const cleared = clearIntelligenceModalOverflowProposal(agreementState, proposalId);
        return modalOverflowDecision({
          responseText: "Okay. I will not use Modal for that job, and no external compute has been started.",
          agreementState: cleared,
          projectState,
          locale,
        });
      } catch {
        const question = "No external compute has been started, but I could not durably record the cancellation. The proposal remains pending and I will not execute it until its state is resolved.";
        return modalOverflowDecision({
          responseText: question, agreementState, projectState, locale, intent: "clarify", question,
        });
      }
    }
    if (isAffirmative(message)) {
      const actorId = text(actor?.id || actor?.user_id);
      if (!FULL_ACCESS_ROLES.has(normalizeRole(role)) || !actorId) {
        const question = "This one-job Modal overflow can only be approved by an owner or super administrator. No external compute has been started.";
        return modalOverflowDecision({
          responseText: question, agreementState, projectState, locale, intent: "clarify", question,
        });
      }
      const approved = await approveIntelligenceModalOverflowProposal({
        organizationId,
        proposalId,
        approvedBy: actorId,
        role,
      });
      const originalMessage = text(modalOverflowProposal.original_message);
      const originalSource = text(modalOverflowProposal.original_source) || source;
      if (!originalMessage) {
        const question = "The overflow proposal is approved, but the exact original request is no longer bound to this conversation. I will not guess or start paid compute. Please restate the request.";
        return modalOverflowDecision({
          responseText: question, agreementState, projectState, locale, intent: "clarify", question,
        });
      }
      const approvedAgreement = {
        ...object(agreementState),
        intelligence_modal_overflow_proposal: {
          ...modalOverflowProposal,
          status: "APPROVED",
          approval_id: approved.approval_id,
          approved_at: new Date().toISOString(),
          authorization_effect: "ONE_EXACT_MODAL_JOB_ONLY",
          execution_authorized: true,
        },
      };
      try {
        const resumed = await runOperatorTurn({
          organizationId, entityId, periodId, partyId, actor, role, permissions, locale, timezone,
          message: originalMessage, source: originalSource, pathname, agreementState: approvedAgreement,
          projectState, conversation, conversationAttachments, callerRequest, conversationId,
          selfEngineeringRequest, semanticUnderstanding, pendingSemanticRelation,
        });
        const resumedAgreement = object(resumed?.agreement_state || resumed?.decision?.agreement_state || approvedAgreement);
        const returnedProposal = pendingIntelligenceModalOverflowProposal(resumedAgreement);
        const finalAgreement = returnedProposal && text(returnedProposal.proposal_id) !== proposalId
          ? resumedAgreement
          : clearIntelligenceModalOverflowProposal(resumedAgreement, proposalId);
        return {
          ...resumed,
          agreement_state: finalAgreement,
          decision: resumed?.decision ? { ...resumed.decision, agreement_state: finalAgreement } : resumed?.decision,
          operator_catalog: {
            ...object(resumed?.operator_catalog),
            modal_overflow_owner_approved_and_resumed: true,
          },
        };
      } catch (error) {
        const currentProposal = await getIntelligenceModalOverflowProposal({ organizationId, proposalId }).catch(() => null);
        const currentStatus = text(currentProposal?.status || "APPROVED").toUpperCase();
        const uncertain = currentStatus === "CONSUMED";
        const nextAgreement = {
          ...approvedAgreement,
          intelligence_modal_overflow_proposal: {
            ...object(approvedAgreement.intelligence_modal_overflow_proposal),
            status: uncertain ? "EXECUTION_UNCERTAIN" : currentStatus,
            execution_authorized: !uncertain,
          },
        };
        const question = uncertain
          ? "The approved Modal job may already have been submitted, so I will not retry it blindly. I preserved the exact job state for verification."
          : "The one-job approval is recorded, but paid execution did not start successfully. I did not create another approval or submit a replacement job.";
        return modalOverflowDecision({
          responseText: question, agreementState: nextAgreement, projectState, locale, intent: "answer",
        });
      }
    }
  }

  const storedPending = pendingAction(agreementState);
  const implementationRepairResume = storedPending
    ? null
    : implementationRepairResumeAction(agreementState, {
        organizationId, entityId, periodId, partyId,
      });
  const businessPartnerRecoveryResume = storedPending || implementationRepairResume
    ? null
    : businessPartnerRecoveryResumeAction(agreementState, currentBusinessContext);
  const offeredPending = storedPending || implementationRepairResume || businessPartnerRecoveryResume;
  const activeRun = autonomousRunFromAgreementState(agreementState);

  const pendingInvoiceRevision =
    storedPending?.capability_key === "finance.accounts_receivable.CreateCustomerInvoice" &&
    !isAffirmative(message) &&
    !isNegative(message)
      ? revisePendingCustomerInvoicePayload({ payload: storedPending.payload, message, timezone, reason: storedPending.reason })
      : null;
  if (pendingInvoiceRevision && activeRun && operatorPendingExecutionMatchesAutonomousRun(storedPending, activeRun)) {
    const nextPending = {
      ...object(agreementState.pending_execution),
      payload: pendingInvoiceRevision.payload,
      original_message: text(message).slice(0, 4000) || object(agreementState.pending_execution).original_message || null,
    };
    const nextSteps = Array.isArray(activeRun.planned_steps)
      ? activeRun.planned_steps.map((step) => text(step?.id) === "requested_action"
          ? { ...step, payload: pendingInvoiceRevision.payload, status: "awaiting_confirmation", gate: "confirmation" }
          : step)
      : [];
    const nextRun = { ...activeRun, planned_steps: nextSteps, status: "awaiting_confirmation", current_step_id: "requested_action", updated_at: new Date().toISOString() };
    const nextAgreementState = agreementWithAutonomousRun({ ...object(agreementState), pending_execution: nextPending }, nextRun);
    return {
      success: true,
      decision: {
        response_text: `${pendingInvoiceRevision.summary} Do you want me to create this invoice?`,
        response_language: locale || null,
        intent: "plan",
        confidence: 1,
        agreement_state: nextAgreementState,
        project_state: projectState,
        clarification: { required: true, question: "Do you want me to create this invoice?", options: [{ id: "confirm", label: "Yes, proceed" }, { id: "cancel", label: "No, cancel" }] },
        navigation: { target_id: null },
        execution: { capability_key: storedPending.capability_key, payload: pendingInvoiceRevision.payload, reason: storedPending.reason },
        plan: [],
      },
      agreement_state: nextAgreementState,
      current_screen: null,
      provider_evidence: { provider: "avantiqo-local", model: "pending-action-revision-v1", usage_id: null },
      navigation: null,
      execution: { status: "blocked", reason: "CONFIRMATION_REQUIRED", capability_key: storedPending.capability_key, requested_payload: pendingInvoiceRevision.payload },
      operator_catalog: { navigation_target_count: 0, executable_capability_count: 1, pending_action_revised: true, execution_authorized: false },
    };
  }
  ...
  const resumeMission = Boolean(
    resumeRequested &&
      ...
      ["awaiting_approval", "waiting_external", "verifying"].includes(
  ...
  const resumeImplementationRepair = Boolean(implementationRepairResume && !isNegative(message));
  const resumeBusinessPartnerRecovery = Boolean(businessPartnerRecoveryResume && !isNegative(message));

  const retryVerificationRequested = Boolean(
    offeredPending?.resume_kind === "verification" &&
      (isAffirmative(message) || resumeVerification),
  );
  const semanticNewGoal = text(pendingSemanticRelation, 40).toLowerCase() === "new_goal";
  const semanticRevision = text(pendingSemanticRelation, 40).toLowerCase() === "revise";
  if (offeredPending && semanticRevision) {
    const nextAgreementState = clearPendingAndSupersedeRun(agreementState, true);
    return {
      success: true,
      decision: {
        response_text: "I understood that you want to change the staged action. I disarmed the old staged action so it cannot execute unchanged. Please state the corrected action or values you want me to use, and I’ll prepare it again under fresh governance.",
        ...
        clarification: {
          required: true,
          question: "What should the corrected action be?",
          options: [],
        },
        navigation: { target_id: null },
        execution: {
          capability_key: null,
          payload: {},
          reason: "PENDING_ACTION_REVISION_REQUIRES_FRESH_GOVERNANCE",
        },
        plan: [],
      },
      agreement_state: nextAgreementState,
      current_screen: currentScreen,
      provider_evidence: {
        pending_action_revision_disarmed: true,
        prior_pending_execution_cleared: true,
  ...
  const respondsToPending = Boolean(
    offeredPending &&
      ...
      (
        ...
        resumeImplementationRepair ||
        resumeBusinessPartnerRecovery
      ),
  );

  const genericPendingBindingMismatch = Boolean(
    respondsToPending &&
      !["mission", "implementation_repair", "business_partner_recovery"].includes(offeredPending?.resume_kind) &&
  ...
  if (fastConversation) {
    ...
    const fastConversationAgreementState = semanticNewGoal
  ...
  const exactDeterministicCapabilityKey =
    text(semanticUnderstanding?.deterministic_capability_key, 300) ||
    text(pending?.capability_key, 300);
  const fastActionCapability = findOperatorFastAction(exactDeterministicCapabilityKey);
  const discoveredCapabilities = fastActionCapability
    ? [fastActionCapability]
  ...
  if (pending && isNegative(message)) {
    ...
    const cancellingBusinessPartnerRecovery = pending.resume_kind === "business_partner_recovery";
    const cancelledStepId = cancellingMission
      ? text(activeRun?.current_step_id)
      : cancellingVerification
        ? "post_action_verification"
        : "requested_action";
    const cancelledBaseAgreementState = cancellingBusinessPartnerRecovery
      ? clearBusinessPartnerRecovery(clearedAgreementState(agreementState))
      : clearedAgreementState(agreementState);
    const nextAgreementState = agreementWithRunTransition(
      cancelledBaseAgreementState,
      {
        status: "cancelled",
    ...
    const cancellationProductEvidence = cancellingMission && activeRun
      ? await recordBusinessPartnerCancellationProductEvidence({
          organizationId,
          run: activeRun,
        })
      : null;
    ...
    return {
      ...
      operator_catalog: {
        ...
        ...(cancellationProductEvidence ? { product_evidence: cancellationProductEvidence } : {}),
      },
    };
  }
  ...
  if (retryVerificationRequested && pending) {
    ...
    const retryCompleted =
      ...
      retryResult?.business_effect_verified === true;
    const nextAgreementState = agreementWithRunTransition(
  ...
  if (resumeBusinessPartnerRecovery && pending?.recovery_completed_without_replay === true) {
    const missionContinuation = businessPartnerMissionContinuation(
      agreementState, pending, { organizationId, entityId, periodId, partyId, actor },
    );
    if (!missionContinuation) {
      return {
        success: true,
        decision: {
          response_text: "The authoritative reinspection proves the write completed, but I could not safely bind that result to the original mission continuation. I preserved the recovery state and will not replay the write.",
          response_language: locale || null, intent: "answer", confidence: 1, agreement_state: agreementState, project_state: projectState,
          clarification: { required: false, question: null, options: [] }, navigation: { target_id: null },
          execution: { capability_key: null, payload: {}, reason: "AMBIGUOUS_WRITE_COMPLETION_CONTINUITY_UNRESOLVED" }, plan: activeRun?.planned_steps || [],
        },
        agreement_state: agreementState, current_screen: currentScreen, provider_evidence: null, navigation: null,
        execution: { status: "blocked", reason: "AMBIGUOUS_WRITE_COMPLETION_CONTINUITY_UNRESOLVED", mutation_replay_allowed: false },
        operator_catalog: { navigation_target_count: navigationTargets.length, executable_capability_count: capabilities.length },
      };
    }
    const missionCapability = capabilities.find((item) => item.key === OPERATOR_MISSION_KEY);
    if (!missionCapability) {
      return { success: true, decision: { response_text: "The write is authoritatively verified as completed, but mission continuation is not available on this running version. I preserved state and will not replay the write.", response_language: locale || null, intent: "answer", confidence: 1, agreement_state: agreementState, project_state: projectState, clarification: { required: false, question: null, options: [] }, navigation: { target_id: null }, execution: { capability_key: null, payload: {}, reason: "MISSION_CONTINUATION_CAPABILITY_NOT_AVAILABLE" }, plan: activeRun?.planned_steps || [] }, agreement_state: agreementState, current_screen: currentScreen, provider_evidence: null, navigation: null, execution: { status: "blocked", reason: "MISSION_CONTINUATION_CAPABILITY_NOT_AVAILABLE", mutation_replay_allowed: false }, operator_catalog: { navigation_target_count: navigationTargets.length, executable_capability_count: capabilities.length } };
    }
    const baseAgreementState = clearBusinessPartnerRecovery(agreementState);
    if (missionContinuation.completed) {
      return missionResultTurn({ result: missionContinuation.completion_result, capability: missionCapability, agreementState: baseAgreementState, projectState, locale, currentScreen, navigationTargetCount: navigationTargets.length, executableCapabilityCount: capabilities.length, originalMessage: pending.original_message || message, objective: missionContinuation.objective || text(activeRun?.objective), resumedRun: true, businessContext: currentBusinessContext });
    }
    const resumedMission = await executeCapability({
      capability: missionCapability, payload: missionContinuation.payload, organizationId, entityId, periodId, partyId, actor, permissions, source, callerRequest, conversationId,
      runtimeMetadata: { operatorMissionResume: true, operatorMissionConfirmed: false, businessPartnerAmbiguousWriteMissionResume: true, repairedStepBusinessEffectVerified: true, mutationReplayPerformed: false },
    });
    return missionResultTurn({ result: resumedMission, capability: missionCapability, agreementState: baseAgreementState, projectState, locale, currentScreen, navigationTargetCount: navigationTargets.length, executableCapabilityCount: capabilities.length, originalMessage: pending.original_message || message, objective: missionContinuation.objective || text(activeRun?.objective), resumedRun: true, businessContext: currentBusinessContext });
  }

  if (pending && (isAffirmative(message) || resumeFromApproval || resumeMission || resumeImplementationRepair || resumeBusinessPartnerRecovery)) {
    const capability = capabilities.find(
      (item) => item.key === pending.capability_key,
    );

    if (!capability) {
      if (resumeImplementationRepair || resumeBusinessPartnerRecovery) {
        const nextAgreementState = {
          ...object(agreementState),
          implementation_repair_resume: {
            ...object(agreementState?.implementation_repair_resume),
            resume_attempted: false,
            last_activation_check: "CAPABILITY_NOT_AVAILABLE",
          },
        };
        return {
          success: true,
          decision: {
            response_text: "The verified repair is still bound to the original action, but this running server does not expose the repaired capability yet. I preserved the continuation instead of discarding or faking it.",
            response_language: locale || null, intent: "answer", confidence: 1,
            agreement_state: nextAgreementState, project_state: projectState,
            clarification: { required: false, question: null, options: [] },
            navigation: { target_id: null },
            execution: { capability_key: pending.capability_key, payload: pending.payload, reason: pending.reason }, plan: [],
          },
          agreement_state: nextAgreementState, current_screen: currentScreen, provider_evidence: null, navigation: null,
          execution: { status: "repairing", reason: "REPAIRED_CAPABILITY_NOT_ACTIVE_ON_RUNNING_VERSION", capability_key: pending.capability_key, resumed_run: false, resume_kind: resumeBusinessPartnerRecovery ? "business_partner_recovery" : "implementation_repair" },
          operator_catalog: { navigation_target_count: navigationTargets.length, executable_capability_count: capabilities.length },
        };
      }
      const nextAgreementState = agreementWithRunTransition(
    ...
    if (resumeImplementationRepair || resumeBusinessPartnerRecovery) {
      const blocked = executionBlockedReason(capability, { source, confirmed: false });
      const needsConfirmation = blocked === "VOICE_CONFIRMATION_REQUIRED" || blocked === "CONFIRMATION_REQUIRED";
      if (blocked) {
        const baseAgreementState = clearedAgreementState(agreementState);
        const recoveryContinuity = object(agreementState?.business_partner_recovery?.mission_continuity);
        const preserveRecoveryMission = Boolean(
          resumeBusinessPartnerRecovery &&
          text(recoveryContinuity.contract) === "AVANTIQO_BUSINESS_PARTNER_MISSION_RECOVERY_V1" &&
          text(activeRun?.run_kind).toLowerCase() === "mission" &&
          text(activeRun?.run_id) === text(recoveryContinuity.run_id)
        );
        const nextAgreementState = needsConfirmation
          ? preserveRecoveryMission
            ? {
                ...baseAgreementState,
                pending_execution: {
                  capability_key: capability.key,
                  payload: object(pending.payload),
                  reason: text(pending.reason) || null,
                  original_message: text(pending.original_message || message).slice(0, 4000) || null,
                  resume_kind: "business_partner_recovery",
                },
              }
            : agreementWithPendingConfirmationRun({
                agreementState: baseAgreementState,
                capability, payload: pending.payload, reason: pending.reason,
                originalMessage: pending.original_message || message,
                objective: pending.original_message || message,
              })
          : baseAgreementState;
        const question = needsConfirmation ? "Should I proceed with that exact repaired action?" : null;
        return {
          success: true,
          decision: {
            response_text: needsConfirmation
              ? "The recovered exact action is ready for its normal confirmation. I preserved its original mission and did not treat the supplied data as approval."
              : `The recovered exact action is ready, but its current governance still blocks execution (${blocked}).`,
            response_language: locale || null, intent: needsConfirmation ? "plan" : "answer", confidence: 1,
            agreement_state: nextAgreementState, project_state: projectState,
            clarification: { required: needsConfirmation, question, options: needsConfirmation ? [{ id: "confirm", label: "Yes, proceed" }, { id: "cancel", label: "No, cancel" }] : [] },
            navigation: { target_id: null },
            execution: { capability_key: capability.key, payload: pending.payload, reason: pending.reason }, plan: [],
          },
          agreement_state: nextAgreementState, current_screen: currentScreen, provider_evidence: null, navigation: null,
          execution: { status: "blocked", reason: blocked, capability_key: capability.key, resumed_run: true, resume_kind: resumeBusinessPartnerRecovery ? "business_partner_recovery" : "implementation_repair" },
          operator_catalog: { navigation_target_count: navigationTargets.length, executable_capability_count: capabilities.length },
        };
      }
    }
    ...
    try {
      ...
      conversationId,
        ...
        runtimeMetadata: {
          operatorRunId: text(pending.run_id) || null,
          operatorConfirmedAction: isAffirmative(message),
          ...(missionResume
            ? {
      ...
      const implementationRepair = await attemptDeclaredImplementationRepair({
        error: executionError,
        failedCapability: capability,
        failedPayload: pending.payload,
        capabilities,
        originalMessage: pending.original_message || message,
        organizationId,
        entityId,
      ...
      if (implementationRepair?.repaired_and_resumed === true) {
        result = implementationRepair.retry_result;
      } else if (implementationRepair?.attempted) {
        return {
          ...implementationRepairBlockedTurn({
            response: { success: true, current_screen: currentScreen },
            decision: {
              execution: {
                capability_key: capability.key,
                payload: pending.payload,
    ...
    const resolvedResultVerification = pending.verify_after || resultBoundVerification(capability, result, pending.payload);
    const postActionVerification = await runPendingPostActionVerification({
      pending: resolvedResultVerification ? { ...pending, verify_after: resolvedResultVerification } : pending,
      capabilities,
      organizationId,
      entityId,
      periodId,
      ...
      conversationId,
      actionResult: result,
      actionIdentityEvidence: Array.from(collectStableBusinessIdentities(result)).slice(0, 50),
    ...
    const verificationResult = postActionVerification
      ...
      : result;
    const completedBaseAgreementState = resumeBusinessPartnerRecovery
      ? clearBusinessPartnerRecovery(agreementState)
      : agreementState;
    let nextAgreementState = completedAgreementState(
      completedBaseAgreementState,
      pending,
      postActionVerification,
      result,
    );
    const costFollowUp = vendorInvoiceCostFollowUp({
      capability, result, capabilities, agreementState: nextAgreementState,
      originalMessage: pending.original_message || message,
      objective: text(activeRun?.objective) || pending.original_message || message,
    });
    if (costFollowUp) nextAgreementState = costFollowUp.nextAgreementState;

    const businessEffectVerified =
      !pending.verify_after || postActionVerification?.business_effect_verified === true;
    const activeRecovery = object(agreementState?.business_partner_recovery);
    const activeConfigurationRecovery = object(activeRecovery.configuration_recovery);
    const configurationRecoveryResume = Boolean(
      resumeBusinessPartnerRecovery &&
      text(activeConfigurationRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_CONFIGURATION_RECOVERY_V1" &&
      text(activeConfigurationRecovery.status) === "READY_TO_RETRY" &&
      activeConfigurationRecovery.auto_resume_allowed === true
    );
    const activeTransientRecovery = object(activeRecovery.transient_recovery);
    const transientRecoveryResume = Boolean(
      resumeBusinessPartnerRecovery &&
      text(activeTransientRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_TRANSIENT_RECOVERY_V1" &&
      text(activeTransientRecovery.status) === "READY_TO_RETRY" &&
      activeTransientRecovery.auto_resume_allowed === true &&
      activeTransientRecovery.read_only_retry === true
    );
    const activeDataRecovery = object(activeRecovery.data_recovery);
    const dataRecoveryResume = Boolean(
      resumeBusinessPartnerRecovery &&
      text(activeDataRecovery.contract) === "AVANTIQO_BUSINESS_PARTNER_DATA_RECOVERY_V1" &&
      text(activeDataRecovery.status) === "READY_TO_RETRY" &&
      activeDataRecovery.auto_resume_allowed === true &&
      activeDataRecovery.payload_validated === true
    );

    let selfHealingReplayVerification = null;
    if (resumeBusinessPartnerRecovery && businessEffectVerified) {
      const recovery = activeRecovery;
      if (!configurationRecoveryResume) {
        if (!transientRecoveryResume) {
        if (!dataRecoveryResume) {
        const selfHealingExecution = object(recovery.self_healing_execution);
        const productionRelease = object(recovery.production_release);
        const commit = object(productionRelease.commit);
        const deployment = object(productionRelease.deployment);
        const readOnlyReplay = text(capability?.mode).toLowerCase() === "read";
        const replayActionBinding = validateBusinessPartnerRecoveryReplayBinding(
          recovery.replay_binding,
          {
        ...
        const exactReplayBindingVerified = replayActionBinding.valid === true;
        const replayedActionBinding = exactReplayBindingVerified
          ? buildBusinessPartnerRecoveryReplayBinding({
              ...currentBusinessContext,
              capabilityKey: pending.capability_key,
              payload: pending.payload,
          ...
          : null;
        const authoritativeBusinessOutcome =
          exactReplayBindingVerified &&
          (readOnlyReplay || postActionVerification?.business_effect_verified === true);

        selfHealingReplayVerification = verifyPlatformSelfHealingReplay({
          execution: selfHealingExecution,
          evidence: {
            mission_id: text(selfHealingExecution.mission_id, 240) || null,
            signal_key: text(selfHealingExecution.signalKey || selfHealingExecution.signal_key, 240) || null,
            repaired_commit: text(commit.commit_sha, 80) || null,
            observed_commit: text(deployment.observed_commit_sha, 80) || null,
            original_action_replayed: exactReplayBindingVerified,
            original_action_binding_sha256: text(recovery?.replay_binding?.binding_sha256, 80) || null,
            replayed_action_binding_sha256: text(replayedActionBinding?.binding_sha256, 80) || null,
            replay_binding_verified: exactReplayBindingVerified,
            replay_binding_reason: replayActionBinding.reason || null,
            safe_deterministic_equivalent: false,
            original_failure_absent: authoritativeBusinessOutcome,
            expected_outcome_observed: authoritativeBusinessOutcome,
            authoritative_server_evidence: authoritativeBusinessOutcome,
            verification_status: postActionVerification?.business_effect_verified === true
              ? "completed"
              : readOnlyReplay
                ? "read_replay_completed"
                : null,
          },
        });

        if (selfHealingReplayVerification.fixed !== true) {
          const preservedAgreementState = {
            ...object(agreementState),
            business_partner_recovery: {
              ...recovery,
              status: "POST_RELEASE_REPLAY_UNVERIFIED",
              resume_authorized: false,
              replay_verification: selfHealingReplayVerification,
            },
          };
          return {
            success: true,
            decision: {
              response_text: "The repaired action returned, but I could not yet prove the production defect fixed through the authoritative replay contract. I preserved the recovery state and will not replay the write.",
              response_language: locale || null,
              intent: "answer",
              confidence: 1,
              agreement_state: preservedAgreementState,
              project_state: projectState,
              clarification: { required: false, question: null, options: [] },
              navigation: { target_id: null },
              execution: { capability_key: capability.key, payload: pending.payload, reason: "POST_RELEASE_REPLAY_UNVERIFIED" },
              plan: activeRun?.planned_steps || [],
            },
            agreement_state: preservedAgreementState,
            current_screen: currentScreen,
            ...
            execution: {
              status: "verifying",
              reason: "POST_RELEASE_REPLAY_UNVERIFIED",
              capability_key: capability.key,
              resumed_run: true,
              resume_kind: "business_partner_recovery",
        ...
        nextAgreementState = {
          ...object(nextAgreementState),
          self_healing_replay_receipt: selfHealingReplayVerification,
        };

        } else {
          nextAgreementState = {
            ...object(nextAgreementState),
            data_recovery_receipt: {
              ...activeDataRecovery,
              status: "DATA_RETRY_VERIFIED",
          ...
          nextAgreementState = {
            ...object(nextAgreementState),
            transient_recovery_receipt: {
              ...activeTransientRecovery,
              status: "READ_RETRY_VERIFIED",
              retry_attempted: true,
              auto_resume_allowed: false,
              business_effect_verified: true,
              completed_at: new Date().toISOString(),
        ...
        nextAgreementState = {
          ...object(nextAgreementState),
          configuration_recovery_receipt: {
            ...activeConfigurationRecovery,
            status: "RETRY_VERIFIED",
            retry_attempted: true,
            auto_resume_allowed: false,
            business_effect_verified: true,
            completed_at: new Date().toISOString(),
      ...
      const missionContinuation = businessPartnerMissionContinuation(
        agreementState,
        pending,
        { organizationId, entityId, periodId, partyId, actor },
      );
      if (missionContinuation) {
        const missionCapability = capabilities.find((item) => item.key === OPERATOR_MISSION_KEY);
        if (!missionCapability) {
          const preservedAgreementState = {
            ...object(agreementState),
            business_partner_recovery: {
              ...object(agreementState?.business_partner_recovery),
              status: "MISSION_CONTINUATION_CAPABILITY_NOT_AVAILABLE",
          ...
          return {
            success: true,
            decision: {
              response_text: "The repaired business step is verified, but the mission continuation capability is not active on this running version. I preserved the remaining mission instead of discarding it.",
              response_language: locale || null,
              intent: "answer",
              confidence: 1,
              agreement_state: preservedAgreementState,
              ...
              clarification: { required: false, question: null, options: [] },
              navigation: { target_id: null },
              execution: { capability_key: OPERATOR_MISSION_KEY, payload: {}, reason: "MISSION_CONTINUATION_CAPABILITY_NOT_AVAILABLE" },
              plan: activeRun?.planned_steps || [],
            },
            agreement_state: preservedAgreementState,
            current_screen: currentScreen,
            provider_evidence: null,
            navigation: null,
            execution: { status: "repairing", reason: "MISSION_CONTINUATION_CAPABILITY_NOT_AVAILABLE", resumed_run: true, resume_kind: "business_partner_recovery" },
            operator_catalog: { navigation_target_count: navigationTargets.length, executable_capability_count: capabilities.length },
          };
        }

        const missionAgreementState = {
          ...clearBusinessPartnerRecovery(nextAgreementState),
          pending_execution: {
            capability_key: OPERATOR_MISSION_KEY,
            payload: object(missionContinuation.payload),
            reason: "Continue original mission after verified repaired step",
        ...
        if (missionContinuation.completed) {
          return missionResultTurn({
            result: missionContinuation.completion_result,
            capability: missionCapability,
            agreementState: missionAgreementState,
            projectState,
            locale,
            currentScreen,
            navigationTargetCount: navigationTargets.length,
            executableCapabilityCount: capabilities.length,
            originalMessage: pending.original_message || message,
            objective: missionContinuation.objective || text(activeRun?.objective) || pending.original_message || message,
            resumedRun: true,
            businessContext: currentBusinessContext,
          });
        }
        ...
        const resumedMission = await executeCapability({
          capability: missionCapability,
          payload: missionContinuation.payload,
          organizationId,
          entityId,
          periodId,
          partyId,
      ...
      conversationId,
          runtimeMetadata: {
            operatorMissionResume: true,
            operatorMissionConfirmed: false,
            ...(configurationRecoveryResume
              ? { businessPartnerConfigurationMissionResume: true }
              : transientRecoveryResume
                ? { businessPartnerTransientMissionResume: true }
                : dataRecoveryResume
                  ? { businessPartnerDataMissionResume: true }
                  : { businessPartnerSelfHealingMissionResume: true }),
            repairedStepBusinessEffectVerified: true,
          },
        });
        if (isMissionExecutionResult(missionCapability, resumedMission)) {
          return missionResultTurn({
            result: resumedMission,
            capability: missionCapability,
            agreementState: missionAgreementState,
            projectState,
            locale,
    ...
    let responseText = costFollowUp
      ? "Vendor bill created successfully. I also found verified inventory purchase-cost evidence on the invoice. Updating those ingredient costs and recalculating affected recipe/dish food costs is a separate Supply Chain write and requires your confirmation."
      : businessEffectVerified
        ? "Done. The confirmed action completed successfully."
        : "The action call completed, but I could not independently verify the intended business effect. I preserved verification-only continuation and will not replay the write.";
    const requestedPresentation = text(pending.result_presentation, 40).toLowerCase();
    const resultArtifacts = businessEffectVerified
      ? collectOperatorPresentationArtifacts(result, 24)
      : [];
    let verificationEvidence = null;
    let nextProjectState = projectState;
    const deterministicVerification = costFollowUp ? null : deterministicFinanceExecutionVerification({
      capability,
      result,
    });

    if (costFollowUp) {
      verificationEvidence = { provider: "avantiqo-local", model: "vendor-invoice-cost-follow-up-v1", usage_id: null };
    } else if (deterministicVerification) {
    ...
    if (businessEffectVerified && resultArtifacts.length > 0) {
      if (requestedPresentation === "preview") {
        responseText = `${responseText} Here’s the preview.`;
      } else if (requestedPresentation === "pdf") {
        responseText = `${responseText} The PDF is ready below.`;
      } else if (requestedPresentation === "download") {
        responseText = `${responseText} The download is ready below.`;
      } else if (requestedPresentation === "both") {
        responseText = `${responseText} Here’s the preview, and the PDF is ready below.`;
      }
    }
    ...
    return {
      ...
      decision: {
        response_text: responseText,
        response_language: locale || null,
        intent: costFollowUp ? "plan" : "answer",
        confidence: 1,
        agreement_state: nextAgreementState,
        project_state: nextProjectState,
        clarification: costFollowUp ? {
          required: true,
          question: "Should I apply the verified purchase costs and refresh the affected recipe/dish food costs?",
          options: [{ id: "confirm", label: "Yes, update food costs" }, { id: "cancel", label: "No, leave costs unchanged" }],
        } : {
          required: false, question: null, options: [],
        },
        navigation: { target_id: null },
        execution: costFollowUp ? {
          capability_key: costFollowUp.capability.key, payload: costFollowUp.payload,
          reason: "Apply verified vendor invoice costs after Finance vendor bill creation",
        } : {
          capability_key: capability.key, payload: pending.payload, reason: pending.reason,
        },
        plan: [],
      },
      agreement_state: nextAgreementState,
      ...
      provider_evidence: {
        verification: verificationEvidence,
        ...(requestedPresentation && requestedPresentation !== "default" ? { preferred_presentation: requestedPresentation } : {}),
      },
      navigation: null,
      execution: {
        status: businessEffectVerified ? "completed" : "blocked",
        ...(businessEffectVerified ? {} : { reason: text(postActionVerification?.assertion?.reason || postActionVerification?.reason || postActionVerification?.error) || "POST_ACTION_VERIFICATION_INCOMPLETE" }),
        capability: {
          key: capability.key,
          domain: capability.domain,
          capability: capability.capability,
  ...
  const deterministicPreparation = await prepareDeterministicGovernedAction({
  ...
  let semanticPreparation = null;
  let semanticPreparationFailed = false;
  if (!deterministicPreparation) {
    try {
      semanticPreparation = await prepareSemanticGovernedAction({
        semanticUnderstanding,
        message,
        organizationId,
        partyId,
      ...
      semanticPreparationFailed = true;
      console.error("OPERATOR_SEMANTIC_ACTION_PREPARATION_FAILED_CLOSED", {
  ...
  const governedSingleMutation = Boolean(
    text(semanticUnderstanding?.route, 40).toLowerCase() === "governed" &&
    semanticUnderstanding?.requires_mutation === true &&
    text(semanticUnderstanding?.action_shape, 40).toLowerCase() === "single"
  );
  const semanticMutationFailClosed =
    !deterministicPreparation &&
    governedSingleMutation &&
    (semanticPreparationFailed || !semanticPreparation)
      ? {
          decision: {
            response_text: "I understood that you want a real business action, but I could not safely prepare one exact registered action from the current context. Please restate the exact action or missing business detail; I will not guess or execute a fallback interpretation.",
            response_language: text(locale, 80) || null,
            intent: "clarify",
            confidence: 1,
            agreement_state: object(activeAgreementState),
            project_state: object(projectState),
            clarification: {
              required: true,
              question: "What exact business action or missing value should I use?",
              options: [],
            },
            navigation: { target_id: null },
            execution: { capability_key: null, payload: {}, reason: "SEMANTIC_ACTION_PREPARATION_FAILED_CLOSED" },
            plan: [],
          },
          provider_evidence: {
            semantic_action_preparation_failed_closed: true,
            mutation_candidate_created: false,
            authorization_effect: "NONE",
          },
        }
  ...
  const reasoning = deterministicPreparation || semanticPreparation || semanticMutationFailClosed || await reasonAboutOperatorTurn({
    organizationId,
    partyId,
    entityId,
    locale,
  ...
  const selfEngineeringBinding = bindOperatorSelfEngineeringDecision({
    decision: reasoning.decision,
    selfEngineeringRequest,
    capabilities,
  });
  const decision = selfEngineeringBinding.decision;
  const response = {
    success: true,
    decision,
    ...
    operator_catalog: {
      navigation_target_count: navigationTargets.length,
      executable_capability_count: capabilities.length,
      self_engineering_decision_binding: selfEngineeringBinding.applied === true,
      self_engineering_decision_binding_reason: selfEngineeringBinding.reason || null,
    },
  };
    ...
    if (gapBuild?.attempted) {
      ...
      const resumeBinding = (deployed || pendingDeploy || persisted)
        ? buildImplementationRepairResumeBinding({
            ...
            periodId,
            partyId,
            capabilityKey: decision.execution.capability_key,
            payload: object(decision.execution.payload),
            reason: decision.execution.reason,
            originalMessage: message,
          })
  ...
  const explicitlyAuthorized = explicitDirectActionAuthorization({
    message,
    capability,
    capabilities,
    source,
  ...
  const blocked = executionBlockedReason(capability, {
    source,
    confirmed: explicitlyAuthorized,
  });
      ...
      decision: {
        ...
        response_text:
          needsConfirmation && capability
            ? `${decision.response_text} ${humanConfirmationQuestion(capability, decision.execution.payload)}`
            : capability
        ...
        clarification: {
          ...
          question:
            needsConfirmation && capability
              ? humanConfirmationQuestion(capability, decision.execution.payload)
              : !capability
  ...
  try {
    ...
    const implementationRepair = await attemptDeclaredImplementationRepair({
      error: executionError,
      failedCapability: capability,
      failedPayload: decision.execution.payload,
      capabilities,
      originalMessage: message,
      organizationId,
      entityId,
    ...
    if (implementationRepair?.repaired_and_resumed === true) {
      ...
      return implementationRepairBlockedTurn({
        response,
        decision,
        capability,
        repair: implementationRepair,
  ...
  const musicReviewedFollowUp = musicReviewedEditFollowUp({
    capability,
    result,
    capabilities,
    agreementState: verifiedDecision.agreement_state || decision.agreement_state,
    originalMessage: message,
    objective:
      text(decision.project_state?.objective) ||
      text(projectState?.objective) ||
  ...
  const finalAgreementState =
    musicReviewedFollowUp?.nextAgreementState ||
    verifiedDecision.agreement_state ||
    decision.agreement_state;
  const musicConfirmationQuestion = text(
    musicReviewedFollowUp?.nextAgreementState?.pending_execution?.confirmation_question,
  );
  ...
  return {
    ...response,
    agreement_state: finalAgreementState,
    provider_evidence: {
      planning: reasoning.provider_evidence,
      verification: verificationEvidence,
    },
    execution: {
      ...
      result,
      reviewed_edit_pending_confirmation: musicReviewedFollowUp ? true : false,
    },
    decision: {
      ...verifiedDecision,
      agreement_state: finalAgreementState,
      intent: musicReviewedFollowUp ? "plan" : verifiedDecision.intent,
      response_text: musicReviewedFollowUp && musicConfirmationQuestion
        ? `${baseResponseText} ${musicConfirmationQuestion}`
        : baseResponseText,
      clarification: musicReviewedFollowUp
        ? {
            required: true,
            question: musicConfirmationQuestion,
            options: [
              { id: "confirm", label: "Yes, I confirm and want to proceed" },
              { id: "cancel", label: "No, cancel" },
            ],
          }
        : verifiedDecision.clarification,
    },
  };
}