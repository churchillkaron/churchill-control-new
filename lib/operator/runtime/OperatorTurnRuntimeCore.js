...
function implementationRepairResumeAction(agreementState = {}) {
  const candidate = object(agreementState?.implementation_repair_resume);
  if (text(candidate.contract) !== "AVANTIQO_IMPLEMENTATION_REPAIR_RESUME_V1") return null;
  if (candidate.resume_attempted === true) return null;
  if (text(candidate.authorization_effect) !== "SAME_ACTION_ONLY") return null;
  const capabilityKey = text(candidate.capability_key);
  if (!capabilityKey) return null;
  return {
    capability_key: capabilityKey,
...
export async function runOperatorTurn({
  ...
  const implementationRepairResume = storedPending ? null : implementationRepairResumeAction(agreementState);
