...
function resultBoundVerification(capability = {}, actionResult = null) {
  ...
  if (!capabilityKey || (!Object.keys(bindings).length && !Object.keys(arrayBindings).length)) return null;
  const payload = {};
...
export async function runOperatorTurn({
  ...
  if (pending && (isAffirmative(message) || resumeFromApproval || resumeMission || resumeImplementationRepair || resumeBusinessPartnerRecovery)) {
    ...
    const resolvedResultVerification = pending.verify_after || resultBoundVerification(capability, result);
