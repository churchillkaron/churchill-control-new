...
function resultBoundVerification(capability = {}, actionResult = null) {
  ...
  if (!capabilityKey || !Object.keys(bindings).length) return null;
  const payload = {};
  for (const [payloadKey, candidates] of Object.entries(bindings)) {
    const paths = Array.isArray(candidates) ? candidates : [candidates];
    const value = paths.map((path) => valueAtPath(actionResult, path)).find((item) => item !== null && item !== undefined && text(item));
