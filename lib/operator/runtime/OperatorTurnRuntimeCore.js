...
export async function runOperatorTurn({
  ...
  const offeredPending = storedPending || implementationRepairResume;
  ...
  const genericPendingBindingMismatch = Boolean(
    respondsToPending &&
      !["mission", "implementation_repair"].includes(offeredPending?.resume_kind) &&
  ...
  if (pending && (isAffirmative(message) || resumeFromApproval || resumeMission || resumeImplementationRepair)) {
    const capability = capabilities.find(
      (item) => item.key === pending.capability_key,
    );

    if (!capability) {
      if (resumeImplementationRepair) {
        ...
        return {
          ...
          execution: { status: "repairing", reason: "REPAIRED_CAPABILITY_NOT_ACTIVE_ON_RUNNING_VERSION", capability_key: pending.capability_key, resumed_run: false, resume_kind: "implementation_repair" },
    ...
    if (resumeImplementationRepair) {
      ...
      if (blocked) {
        ...
        return {
          ...
          agreement_state: nextAgreementState, current_screen: currentScreen, provider_evidence: null, navigation: null,
          execution: { status: "blocked", reason: blocked, capability_key: capability.key, resumed_run: true, resume_kind: "implementation_repair" },
          operator_catalog: { navigation_target_count: navigationTargets.length, executable_capability_count: capabilities.length },
        };
      }
    }
