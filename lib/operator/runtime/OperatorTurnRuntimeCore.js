...
function businessPartnerRecoveryResumeAction(agreementState = {}) {
  ...
  if (ambiguousWriteCompleted) {
    ...
    return {
      capability_key: capabilityKey,
      payload: object(recovery.payload),
...
function missionFailureRecoveryState({ result, run, originalMessage, objective }) {
...
function missionResultTurn({
  ...
  const blockedMissionRecovery = text(result?.status) === "blocked"
    ? missionFailureRecoveryState({ result, run, originalMessage, objective })
    : null;
...
export async function runOperatorTurn({
  ...
  const businessPartnerRecoveryResume = storedPending || implementationRepairResume
    ? null
    : businessPartnerRecoveryResumeAction(agreementState);
  ...
  if (resumeBusinessPartnerRecovery && pending?.recovery_completed_without_replay === true) {
    ...
    if (missionContinuation.completed) {
      return missionResultTurn({ result: missionContinuation.completion_result, capability: missionCapability, agreementState: baseAgreementState, projectState, locale, currentScreen, navigationTargetCount: navigationTargets.length, executableCapabilityCount: capabilities.length, originalMessage: pending.original_message || message, objective: missionContinuation.objective || text(activeRun?.objective), resumedRun: true });
    }
