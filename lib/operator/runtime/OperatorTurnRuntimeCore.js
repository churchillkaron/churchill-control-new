...
function businessPartnerRecoveryResumeAction(agreementState = {}) {
  ...
  if (!verifiedSelfHealing && !verifiedConfigurationRecovery && !verifiedTransientRecovery && !verifiedDataRecovery && !ambiguousWriteNotCompleted) return null;
  const capability = object(recovery.capability);
  const capabilityKey = text(capability.key);
  if (!capabilityKey) return null;
  return {
    capability_key: capabilityKey,
    payload: object(recovery.payload),
    reason: verifiedConfigurationRecovery
