...
function governedProgressFromResult(result) {
  ...
  if (status === "verifying") return { phase: "ACTION_VERIFYING", description: "The action returned and I’m verifying the actual business result now." };
  ...
  if (status === "completed") return { phase: "ACTION_COMPLETED", description: "The governed business action completed and I’m preparing the verified result for you." };
  return { phase: "ACTION_RESPONSE_READY", description: "The governed business step returned and I’m preparing the result now." };
}

async function legacyRunOperatorTurn(options) {
  await publishBusinessPartnerProgress(options, {
    phase: "ACTION_GOVERNED_RUNTIME",
    description: "I’m validating the required business details and governance for this action now.",
  });
