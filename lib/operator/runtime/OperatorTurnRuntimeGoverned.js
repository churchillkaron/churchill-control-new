...
import {
  resolveOperatorRecommendationOutcomeLearning,
...
async function recommendationProposalDecisionTurn(options, recommendation, decision) {
  if (decision === "reject") {
  ...
  let outcomeLearning = null;
  try {
    outcomeLearning = await resolveOperatorRecommendationOutcomeLearning(recommendation);
  } catch (error) {
    console.error("OPERATOR_RECOMMENDATION_OUTCOME_LEARNING_BIND_FAILED", {
      capability_key: text(recommendation?.capability_key, 300),
      error: text(error?.message || error, 500),
      execution_authority_effect: "NONE",
    });
  }
  const selectedRecommendation = outcomeLearning
    ? { ...recommendation, outcome_learning: outcomeLearning }
    : recommendation;
...
function progressOperationLabel(result) {
  const decision = object(result?.decision);
  const execution = object(result?.execution);
  const capability = object(execution.capability);
  const key = text(capability.key || decision?.execution?.capability_key, 300);
  const raw = text(capability.name || capability.capability || capability.action || key, 300);
  if (!raw) return "business action";
  return raw.replace(/[._-]+/g, " ").replace(/\b\w/g, (ch) => ch.toUpperCase());
}

function governedProgressFromResult(result) {
  const decision = object(result?.decision);
  const execution = object(result?.execution);
  const status = text(execution.status, 80).toLowerCase();
  const operation = progressOperationLabel(result);
  const question = text(decision?.clarification?.question, 800);
  if (decision?.clarification?.required === true) {
    return { phase: "ACTION_NEEDS_INPUT", description: question || "I need one more business detail before I can continue." };
  }
  if (status === "repairing") return { phase: "ACTION_REPAIRING", description: "I found an Avantiqo implementation problem and the recovery path is working on it now." };
  if (status === "verifying") return { phase: "ACTION_VERIFYING", description: `Verifying the saved result for ${operation}.` };
  if (status === "blocked") return { phase: "ACTION_BLOCKED", description: text(decision.response_text, 800) || "The action is paused at a governance or business-data gate." };
  if (status === "failed") return { phase: "ACTION_FAILED", description: "The business action failed. I’m moving the exact request into diagnosis and recovery." };
  if (status === "completed") return { phase: "ACTION_COMPLETED", description: `${operation} completed. Preparing the verified result and any previewable output.` };
  return { phase: "ACTION_RESPONSE_READY", description: `${operation} returned. Preparing the current result.` };
}
...
async function legacyRunOperatorTurn(options) {
  ...
  await publishBusinessPartnerProgress(options, governedProgressFromResult(result));
...
export async function runOperatorTurn(options = {}) {
  ...
  if (!proposal) {
    ...
    if (recommendation && proposalDecision) {
      return await recommendationProposalDecisionTurn(
        options,
        recommendation,
        proposalDecision,
      );
