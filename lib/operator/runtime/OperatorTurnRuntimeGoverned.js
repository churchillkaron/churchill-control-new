...
function governedRefinementResponse({
  ...
  const responseText = ready && boundRecommendation
    ...
    : bindingFailed
      ? "I stopped before creating a pending action because the freshly prepared refinement did not bind to the exact governed recommendation contract. The selected direction is preserved, but nothing executed."
      : result?.clear_preparation === true
        ? "The action or authority changed while we were preparing this refinement, so I discarded the stale preparation state. The selected direction is preserved, but no new pending action or run was created. Ask me to prepare it again from current state."
  ...
  return {
    ...
    decision: {
      ...
      clarification: clarificationRequired
        ? {
            required: true,
            question: clarificationQuestion(result),
            options: [],
          }
...
async function legacyRunOperatorTurn(options) {
  const legacy = await import("./OperatorTurnRuntime.js");
