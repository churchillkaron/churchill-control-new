...
function recommendationProofSummary(recommendation) {
  const proof = object(recommendation?.proof);
  if (!proof.contract) return "";
  const parts = [];
  if (proof.evidence_class === "LIVE_EVIDENCE_BACKED") {
    parts.push(`It was grounded in ${list(proof.evidence_refs).length || 0} live evidence reference${list(proof.evidence_refs).length === 1 ? "" : "s"}.`);
  } else if (proof.evidence_class === "CONTEXTUAL_INFERENCE") {
    parts.push("This was a contextual inference, not a verified business fact.");
  } else if (proof.evidence_class === "SELECTED_REFINEMENT") {
    parts.push("This direction came from a selected refinement of the previous recommendation.");
  }
  if (text(proof.strongest_alternative)) parts.push(`The strongest recorded alternative was ${text(proof.strongest_alternative, 500)}.`);
  if (text(proof.critical_uncertainty)) parts.push(`The key uncertainty is ${text(proof.critical_uncertainty, 500)}.`);
  if (proof.requires_revalidation === true) parts.push("It should be revalidated against current evidence before a consequential action relies on it.");
  return parts.join(" ");
}
...
function recommendationStatusTurn(options, recommendation) {
  ...
  const proofSummary = recommendationProofSummary(recommendation);
  ...
  const responseBase = description
    ? exactBinding
      ? reason
  ...
  const responseText = `${responseBase}${proofSummary ? ` ${proofSummary}` : ""}`.slice(0, 4000);
...
function recommendationRefinementStatusTurn(options, proposal) {
  const status = text(proposal?.status).toUpperCase();
  const proposalText = text(proposal?.proposal_text).slice(0, 700);
  ...
  const materializedBindingActive = Boolean(
    status === "MATERIALIZED" &&
      text(proposal?.materialized_recommendation_id) &&
      text(pending.recommendation_id) ===
        text(proposal?.materialized_recommendation_id) &&
      text(pending.capability_key) ===
        text(proposal?.materialized_capability_key) &&
      text(run.run_id) &&
  ...
  let responseText;
  if (status === "PROPOSED") {
    responseText = `We are currently comparing this refinement proposal: ${proposalText} It is proposal context only. The old pending recommendation is disarmed, and this proposal has no execution authority.`;
  } else if (status === "SELECTED") {
    responseText = `We selected this direction: ${proposalText} It is direction only and has not been turned into a pending action. If you want it prepared as the exact governed action, ask me to make that the new recommendation.`;
  } else if (status === "MATERIALIZED") {
    responseText = materializedBindingActive
      ? `We selected this direction: ${proposalText} It has been materialized into a fresh exact governed recommendation and its pending binding is still active. Nothing executes from this status check.`
      : `We selected this direction: ${proposalText} It was materialized previously, but that exact pending binding is no longer active. I will not replay it from history or shorthand; restate the exact action if you want a new governed action.`;
  } else {
    responseText = `That refinement was rejected: ${proposalText} It has no execution authority.`;
  }
  ...
  return {
    ...
    provider_evidence: {
      provider: "avantiqo-local",
      model: "recommendation-refinement-status-v1",
      usage_id: null,
    },
    navigation: null,
    execution: null,
...
function recommendationRefinementDecisionTurn(options, proposal, outcome) {
  const restoredOriginal = outcome === "restore_original";
  const selected = outcome === "select" || restoredOriginal;
  const nextAgreementState = agreementWithRecommendationRefinementDecision(
    options.agreementState,
    {
      outcome,
  ...
  const nextProposal = recommendationRefinementProposalFromAgreementState(
    nextAgreementState,
  );
  const proposalText = text(nextProposal?.proposal_text || proposal?.proposal_text)
    .slice(0, 700);

  return {
    success: true,
    decision: {
      response_text: restoredOriginal
        ? `Restored as direction only. We are back to the original recommendation context: ${proposalText} I did not revive its old payload or create pending execution. Continue or ask me to prepare it when you want a fresh exact governed action.`
        : selected
          ? `Selected. I’ll use this as the preferred direction: ${proposalText} This selection is direction only. It created no pending execution and started no run. The direction must be materialized into an exact governed action before anything can execute.`
          : "Understood. I rejected that refinement proposal. No action was created or executed. The previous recommendation remains visible only as non-executable context until you choose or restate the direction you want.",
      response_language: text(options.locale) || null,
      intent: selected ? "plan" : "answer",
      confidence: 1,
    ...
    provider_evidence: {
      provider: "avantiqo-local",
      model: "recommendation-refinement-decision-v1",
      usage_id: null,
    },
    navigation: null,
    execution: null,
...
function recommendationConversationContext(recommendation) {
  ...
  return [
    ...
    proof.contract ? `Persisted recommendation proof: ${JSON.stringify({
      evidence_class: proof.evidence_class || null,
      evidence_refs: list(proof.evidence_refs).slice(0, 12),
      basis: text(proof.basis).slice(0, 900) || null,
      strongest_alternative: text(proof.strongest_alternative).slice(0, 700) || null,
      critical_uncertainty: text(proof.critical_uncertainty).slice(0, 700) || null,
      falsification_condition: text(proof.falsification_condition).slice(0, 700) || null,
      next_proof_step: text(proof.next_proof_step).slice(0, 700) || null,
      requires_revalidation: proof.requires_revalidation === true,
...
function attentionCandidate(result, capabilities) {
  ...
  if (!body) return null;
  const evidenceSteps = list(body?.evidence?.steps);
  const observedAt = text(body?.generated_at, 120) || null;
  const attentionItems = list(body.items);
  for (const item of attentionItems) {
    ...
    const alternativeItem = attentionItems.find((candidate) => {
      if (candidate === item) return false;
      const alternative = object(candidate?.recommended_action);
      return Boolean(
        text(alternative.capability_key) &&
        text(alternative.capability_key) !== text(recommended.capability_key) &&
        capabilityByKey(capabilities, alternative.capability_key)
      );
    });
    const alternativeRecommendation = object(alternativeItem?.recommended_action);
    const evidenceRefs = list(item?.evidence_refs).map(text).filter(Boolean).slice(0, 12);
    const refSet = new Set(evidenceRefs);
    const evidenceDependencies = evidenceSteps
      .filter((step) => refSet.has(text(step?.id)) && text(step?.status).toLowerCase() === "completed")
      .map((step) => ({
        id: text(step?.id, 160),
        source_kind: "registered_operator_read",
        capability_key: text(step?.capability_key, 300) || null,
        payload: object(step?.requested_payload || step?.payload),
        result_fingerprint: operatorRecommendationEvidenceFingerprint(step?.evidence ?? null),
        required: true,
        verified: true,
        current: true,
        superseded: false,
    ...
    return {
      capability,
      payload: object(recommended.payload),
      reason:
        text(item?.recommended_next_step) ||
        text(item?.why_now) ||
        text(recommended.description),
      proof: {
        evidence_class: "LIVE_EVIDENCE_BACKED",
        evidence_refs: evidenceRefs,
        evidence_dependencies: evidenceDependencies,
        basis: text(item?.why_now || item?.recommended_next_step || recommended.description, 1200) || null,
        strongest_alternative: alternativeItem
          ? {
              capability_key: text(alternativeRecommendation.capability_key, 300) || null,
              description: text(
                alternativeItem?.recommended_next_step ||
                alternativeItem?.why_now ||
                alternativeRecommendation.description,
                900,
              ) || null,
            }
          : null,
        critical_uncertainty: evidenceRefs.length === 1
          ? "The recommendation currently rests on one live evidence reference; contradictory independent evidence could change the ranking."
          : null,
        falsification_condition: "Any required live evidence dependency fingerprint changes during pre-execution revalidation.",
        as_of: observedAt || undefined,
        requires_revalidation: true,
        next_proof_step: text(recommended?.verification_description, 900) ||
          "Revalidate the exact live evidence dependencies immediately before execution.",
      },
    };
  }
  return null;
...
function learnedRecommendationBias(options = {}, capabilityKey = "") {
  const key = text(capabilityKey, 300);
  if (!key) return 0;
  let bias = 0;
  for (const memory of list(options.longTermMemory)) {
    const subject = text(memory?.subject, 400);
    const confidence = Math.max(0, Math.min(1, Number(memory?.confidence || 0)));
    if (subject === `conversation.recommendation_selected.${key}`) {
      bias += 0.04 * (confidence || 0.75);
    } else if (subject === `conversation.recommendation_rejected.${key}`) {
      bias -= 0.06 * (confidence || 0.75);
    }
  }
  return Math.max(-0.06, Math.min(0.04, bias));
}
...
function rankedCandidate(options, result, capabilities) {
  ...
  const advisoryRanked = ranked
    .map((entry) => ({
      ...entry,
      learned_recommendation_bias: learnedRecommendationBias(options, entry?.capability?.key),
    }))
    .sort((left, right) =>
      (Number(right.score || 0) + Number(right.learned_recommendation_bias || 0)) -
      (Number(left.score || 0) + Number(left.learned_recommendation_bias || 0))
    );
  const top = advisoryRanked[0];
  if (!top?.capability) return null;
  const second = advisoryRanked[1] || null;
  const topAdjusted = Number(top.score || 0) + Number(top.learned_recommendation_bias || 0);
  const secondAdjusted = second
    ? Number(second.score || 0) + Number(second.learned_recommendation_bias || 0)
    : 0;
  const separation = second ? topAdjusted - secondAdjusted : topAdjusted;
  const strong =
  ...
  return {
    ...
    reason:
      ...
      text(result?.decision?.response_text),
    proof: {
      evidence_class: "CONTEXTUAL_INFERENCE",
      evidence_refs: [],
      basis: text(result?.decision?.project_state?.progress_summary || result?.decision?.response_text, 1200) || null,
      strongest_alternative: second?.capability
        ? {
            capability_key: text(second.capability.key, 300),
            description: text(second.capability.description || second.capability.name, 900) || null,
          }
        : null,
      critical_uncertainty: second && separation < 0.2
        ? `The leading capability is only ${separation.toFixed(3)} ahead of the strongest alternative in the current deterministic match.`
        : null,
      requires_revalidation: true,
      next_proof_step: "Re-check current business evidence before relying on this recommendation for a state-changing action.",
    },
  };
}
...
function validatedRecommendation(candidate, options, result) {
  ...
  return {
    ...
    proof: object(candidate?.proof),
  };
}

async function recommendationRefinementMaterializationTurn(options, proposal) {
  const proposalText = text(proposal?.proposal_text).slice(0, 2000);
  const safeCapabilities = await safeRecommendationCapabilities(options);
    ...
    ? validatedRecommendation(
        {
          capability,
          payload,
          reason: proposalText || text(capability.description),
          objective: proposalText,
          source: "selected_refinement_materialization",
        },
        options,
        { decision: { project_state: object(options.projectState) } },
      )
    : null;
  ...
  if (!recommendation) {
    return {
      ...
      decision: {
        response_text:
          "I kept the selected direction, but I cannot safely materialize it into an exact registered action yet. The current capability is unavailable or requires exact inputs that are not present in the selected proposal. Please restate the exact action with the required details; I will not reuse the old payload or guess missing fields.",
        response_language: text(options.locale) || null,
        intent: "clarify",
        confidence: 1,
        agreement_state: object(options.agreementState),
        ...
        clarification: {
          required: true,
          question: "What exact action and required details should I materialize from the selected direction?",
          options: [],
        },
        navigation: { target_id: null },
        execution: { capability_key: null, payload: {}, reason: null },
      ...
      provider_evidence: {
        provider: "avantiqo-local",
        model: "recommendation-refinement-materialization-v1",
        usage_id: null,
      },
      navigation: null,
      execution: null,
  ...
  const withRecommendation = agreementWithOperatorRecommendation(
    options.agreementState,
    recommendation,
    { objective: proposalText || recommendation.description },
  );
  const boundRecommendation = operatorRecommendationFromAgreementState(
    withRecommendation,
  );
  if (!boundRecommendation) {
    return {
      success: true,
      decision: {
        response_text:
          "I could validate the selected direction, but the new recommendation did not bind to an exact governed pending action. I stopped without executing anything and preserved the selected direction.",
        response_language: text(options.locale) || null,
        intent: "clarify",
        confidence: 1,
        agreement_state: object(options.agreementState),
  ...
  const nextAgreementState = agreementWithRecommendationRefinementMaterialized(
    withRecommendation,
    boundRecommendation,
  );
  ...
  return {
    success: true,
    decision: {
      response_text: `Prepared. The selected direction is now a fresh exact governed recommendation for ${boundRecommendation.description}. I did not reuse the old payload, and nothing executed. Say “do it” when you want me to execute this exact newly bound action.`,
      response_language: text(options.locale) || null,
      intent: "plan",
      confidence: 1,
      agreement_state: nextAgreementState,
    ...
    operator_catalog: {
      ...
      materialized_capability_key: boundRecommendation.capability_key,
...
function recommendationInvalidationFromAgreementState(agreementState = {}) {
  const invalidation = object(agreementState?.recommendation_invalidation);
  return invalidation.old_action_disarmed === true ? invalidation : null;
}

function semanticInvalidationReassessment(options = {}, invalidation = null) {
  if (!invalidation) return false;
  const semantic = object(options.semanticUnderstanding);
  return Boolean(
    semantic.continuity_required === true ||
    semantic.goal_relation === "continue" ||
    semantic.correction_or_revision === true ||
    ["strategic", "analytical"].includes(text(semantic.conversation_mode).toLowerCase())
  );
}
...
export async function runOperatorTurn(options = {}) {
  ...
  const recommendationInvalidation = recommendationInvalidationFromAgreementState(
    options.agreementState,
  );
  const invalidationReassessment = semanticInvalidationReassessment(
    options,
    recommendationInvalidation,
  );
  const refinementProposal = recommendationRefinementProposalFromAgreementState(
    options.agreementState,
  );
  ...
  const refinementMaterializationRequested =
    classifyRecommendationRefinementMaterializationRequest({
      message: options.message,
  ...
  if (refinementProposal && refinementReply) {
  ...
  const coreResult = await runOperatorTurnCore({
    ...options,
    ...(invalidationReassessment
      ? {
          semanticUnderstanding: {
            ...object(options.semanticUnderstanding),
            route: "evidence",
  ...
  if (
    ...
    (STRATEGIC_RECOMMENDATION_PATTERN.test(text(options.message)) || invalidationReassessment) &&
    ...
    if (nextRecommendation) {
      persistedRecommendation = nextRecommendation;
      if (invalidationReassessment) {
        const cleanAgreement = { ...object(nextAgreementState) };
        delete cleanAgreement.recommendation_invalidation;
        nextAgreementState = cleanAgreement;
      }
      nextAgreementState = agreementWithOperatorRecommendation(
        nextAgreementState,
        nextRecommendation,
