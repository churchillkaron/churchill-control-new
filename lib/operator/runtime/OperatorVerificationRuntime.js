...
function collectionOf(value, depth = 0, path = []) {
  ...
  const candidates = Object.entries(value)
    ...
    .map(([key, candidate]) =>
      collectionOf(candidate, depth + 1, [...path, key]),
    )
    .filter(Boolean)
...
function normalizedVerificationRead(value) {
  ...
  return {
    ...
    description:
      text(candidate.description) || "Verify the requested action took effect",
    payload: object(candidate.payload),
    contract: object(candidate.contract),
  };
}
...
function stagedFollowUp(value) {
  ...
  return {
    ...
    verify_after: normalizedVerificationRead(staged.verify_after),
...
function followUpRequestAligned({ parsed, staged, originalMessage }) {
  const followUp = object(parsed?.follow_up);
  if (!text(originalMessage)) return false;
  if (followUp.request_aligned !== true) return false;
  if (text(followUp.request_alignment).toLowerCase() !== "explicit") return false;
  return text(followUp.requested_capability_key) === staged.capability_key;
}

async function supportedPendingExecution({ result, parsed, originalMessage }) {
  const staged = stagedFollowUp(result);
  if (!staged) return null;
  if (!readChainCompleted(result)) return null;

  const followUp = object(parsed?.follow_up);
  if (followUp.supported !== true) return null;
  if (!followUpRequestAligned({ parsed, staged, originalMessage })) return null;

  let verifyAfter = staged.verify_after;
  if (!verifyAfter) {
    const catalog = await listOperatorCapabilities();
    const action = catalog.find((item) => text(item?.key) === staged.capability_key);
    const declaration = object(action?.operator_verification);
    const verificationKey = text(declaration.capability_key);
    const payloadKeys = list(declaration.payload_keys).map(text).filter(Boolean);
    if (verificationKey && payloadKeys.length && payloadKeys.every((key) => hasOwn(staged.payload, key))) {
      verifyAfter = {
        capability_key: verificationKey,
        description: `Verify ${staged.description}`,
        payload: Object.fromEntries(payloadKeys.map((key) => [key, staged.payload[key]])),
        contract: { derivation: text(declaration.derivation) || "catalog_declared_verification" },
      };
    }
  }
...
function agreementWithPendingExecution(
  agreementState,
  pendingExecution,
  ...
  return agreementWithAutonomousRun(withPending, autonomousRun);
}
...
If the evidence contains action_result plus post_action_verification, distinguish the action response from the independently re-read business state. Treat post_action_verification.status "completed" as fresh verification evidence. If post_action_verification is failed, blocked, missing, or unavailable, do not claim the intended business effect was independently confirmed; state only that the action call completed and that final state verification did not complete.
...
- A staged_follow_up, when present, is an exact candidate action selected earlier from registered capabilities. Do not assume the earlier reasoning interpreted the user's intent correctly; independently compare the staged action with the user's original request below.
- You are not allowed to change its capability, payload, verification read, or business values.
- Set follow_up.request_aligned true only when the user's original request itself explicitly asks Avantiqo to perform that exact downstream business action, including a conditional instruction such as "check X and if needed do Y".
- Questions asking whether an action would be wise, requests for recommendations, "what should we do" questions, analysis-only requests, or an assistant recommendation are not authorization. For those, request_aligned must be false even when the action would be sensible.
- When request_aligned is true, set request_alignment to "explicit" and requested_capability_key to the staged capability_key exactly. Otherwise set request_alignment to "not_explicit" and requested_capability_key to null.
- Set follow_up.supported true only when request_aligned is true, all read steps completed successfully, and the evidence is sufficient to justify that exact requested action.
- Set follow_up.supported false when request_aligned is false, evidence is incomplete, contradictory, insufficient, the action is no longer justified, or any read step failed.
- Supporting a follow-up does not execute it. The server will only place the exact staged action and its optional preflighted verify_after read into pending confirmation. The user must still explicitly confirm, and normal permissions, approval governance, execution verification, and any post-action verification read still apply.
...
- If unsupported because request alignment is not explicit, answer the user's actual information/advice request and do not ask them to confirm the staged action.
- If unsupported for an evidence reason, explain the evidence-based reason and do not ask the user to confirm that action.
...
${staged ? JSON.stringify({
  ...
  verify_after: staged.verify_after,
...
Return exactly one JSON object:
{
  ...
  "follow_up":{
    ...
    "request_alignment":"not_explicit",
    "requested_capability_key":null,
    "supported":false,
    "reason":null
  }
}
`.trim();
  ...
  const execution = await ServiceExecutionRuntime.execute({
    ...
    metadata: {
      ...
      evidence_compaction: "collection-aware-dynamic-v2",
      interpretive_synthesis: true,
      evidence_gated_follow_up: Boolean(staged),
      follow_up_request_alignment_guard: Boolean(staged),
      post_action_verification_aware: true,
      bounded_autonomous_run: Boolean(staged),
    },
    category: "AI",
  });
  ...
  const pendingExecution = await supportedPendingExecution({
    result,
    parsed: parsed || {},
    originalMessage,
  });
  const resultBody = capabilityResult(result);
  const nextAgreementState = agreementWithPendingExecution(
    agreementState,
    pendingExecution,
    {
      objective: text(projectState?.objective) || text(originalMessage),
      evidenceSteps: list(resultBody.steps),
    },
  );
