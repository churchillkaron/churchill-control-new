...
export async function cancelVoiceJob(job, contract) {
  ...
  if (job.provider_job_id && job.usage_id) {
    providerCancellation = await ServiceExecutionRuntime.cancelPending({
      organization_id: job.organization_id,
      provider: OWNED_PROVIDER,
      provider_job_id: job.provider_job_id,
      usage_id: job.usage_id,
      pricing: job.pricing || {},
      credential_id: job.credential_id || null,
      reason: "AVANTIQO_OPERATOR_ASYNC_VOICE_CANCELLED",
      metadata: { ...(job.metadata || {}), async_voice_contract: contract, async_voice_job_id: job.id },
    });
  }
  ...
  return { success: true, pending: false, contract, job_id: job.id, provider: OWNED_PROVIDER, status: "CANCELLED", exact_provider_job_cancel_requested: Boolean(job.provider_job_id), exact_provider_job_cancelled: providerCancellation?.cancelled === true, blind_queue_purge_requested: false };
}
