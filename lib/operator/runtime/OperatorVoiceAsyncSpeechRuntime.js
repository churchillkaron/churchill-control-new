...
async function assertOwnedTtsProviderReady(organizationId) {
  ...
  if (text(selected?.provider) !== OWNED_PROVIDER) {
    throw new Error("AVANTIQO_OPERATOR_ASYNC_SPEECH_OWNED_PROVIDER_REQUIRED");
  }
  return selected;
}
...
export async function startOperatorAsyncSpeech({
  ...
  // Provider eligibility is resolved before RunPod mutation. If the owned TTS
  // row is not production-certified, fail closed without waking an endpoint.
  await assertOwnedTtsProviderReady(organization);
  ...
  try {
    ...
    if (text(execution?.provider) !== OWNED_PROVIDER) {
      throw new Error("AVANTIQO_OPERATOR_ASYNC_SPEECH_EXTERNAL_PROVIDER_FORBIDDEN");
    }
