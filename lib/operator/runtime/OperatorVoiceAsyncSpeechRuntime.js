...
export async function startOperatorAsyncSpeech({ organizationId, entityId = null, partyId, speechText, language, locale = null, voiceLibraryProfileId = null, deliveryProfile = null, quantity = null, metadata = {} }) {
  ...
  const execution = await submitVoiceService({ job, organizationId: organization, entityId, partyId: party, capability: CAPABILITY, input: { input: text, language: language || undefined, locale: locale || undefined, voice_profile: deliveryProfile || library?.voice_profile || undefined, voice_reference: library?.voice_reference || undefined, response_format: "wav", quantity: quantity || undefined, execution_mode: "background" }, metadata: { module: "OPERATOR", operation: "VOICE_SPEECH_ASYNC", async_voice_contract: CONTRACT, ...metadata } });
