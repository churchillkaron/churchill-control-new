...
function transcriptionFromResult(result) {
  ...
  if (!transcript) {
    throw new Error("AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_TEXT_REQUIRED");
  }
  ...
  const languageSource =
    findVoiceField(result, "language_source") ||
...
async function loadJob({ jobId, organizationId }) {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) {
    throw new Error(`AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_JOB_LOOKUP_FAILED:${error.code || "DB"}`);
  }
...
async function loadLease(leaseId) {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) {
    throw new Error(`AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_LEASE_LOOKUP_FAILED:${error.code || "DB"}`);
  }
...
async function updateJob(jobId, patch) {
  const { data, error } = await supabaseAdmin
    ...
    .single();
  if (error) {
    throw new Error(`AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_JOB_UPDATE_FAILED:${error.code || "DB"}`);
  }
  return data;
}
...
export async function startOperatorAsyncTranscription({
  ...
  try {
    ...
    if (insertError) {
      throw new Error(
        `AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_JOB_CREATE_FAILED:${insertError.code || "DB"}`,
      );
    }
    job = inserted;
    ...
    if (execution?.pending !== true) {
      ...
      await releaseForJob({
        ...
        state: "RELEASED",
        reason: "VOICE_STT_COMPLETED_IMMEDIATELY",
      });
    ...
    if (job?.id) {
      await updateJob(job.id, {
        status: "FAILED",
        error_code:
          text(error?.message).split(":")[0].slice(0, 180) ||
          "AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_START_FAILED",
        completed_at: new Date().toISOString(),
      }).catch(() => null);
    }
...
export async function pollOperatorAsyncTranscription({ jobId, organizationId }) {
  ...
  if (job.status === "STARTING") {
    if (Date.parse(job.expires_at) <= Date.now()) {
      return {
        ...
        job_id: job.id,
        status: "EXPIRED",
        error: "AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_START_EXPIRED",
      };
    }
    return {
      success: true,
  ...
  if (settled?.failed === true || settled?.success === false) {
    await releaseForJob({
      ...
      lease,
      state: "FAILED",
      reason: settled?.error || "VOICE_STT_PROVIDER_FAILED",
    ...
    await updateJob(job.id, {
      ...
      error_code:
        text(settled?.error).slice(0, 180) ||
        "AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_PROVIDER_FAILED",
      completed_at: new Date().toISOString(),
    });
    return {
      success: false,
      ...
      job_id: job.id,
      status: "FAILED",
      error: settled?.error || "Voice transcription failed",
    };
  }
