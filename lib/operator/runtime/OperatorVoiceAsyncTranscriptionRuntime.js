...
async function assertOwnedSttProviderReady(organizationId) {
  const selected = await resolveProvider({
    organization_id: organizationId,
    capability: CAPABILITY,
    preferredProvider: OWNED_PROVIDER,
    policy: OWNED_PROVIDER_POLICY,
  });
  if (text(selected?.provider) !== OWNED_PROVIDER) {
    throw new Error("AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_OWNED_PROVIDER_REQUIRED");
  }
  return selected;
}
...
export async function startOperatorAsyncTranscription({ organizationId, entityId = null, partyId, audio, fileName = null, mimeType = null, language = null, prompt = null, locale = null, metadata = {} }) {
  ...
  // Provider eligibility is resolved before RunPod mutation. If the owned STT
  // row is not production-certified, fail closed without waking an endpoint.
  ...
  try {
    ...
    if (text(execution?.provider) !== OWNED_PROVIDER) {
      throw new Error("AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_EXTERNAL_PROVIDER_FORBIDDEN");
    }
    ...
    if (execution?.pending !== true) {
      ...
      return { success: true, pending: false, contract: CONTRACT, job_id: job.id, provider: OWNED_PROVIDER, ...transcription };
    }
    ...
    return { success: true, pending: true, contract: CONTRACT, job_id: job.id, provider: OWNED_PROVIDER, provider_status: job.provider_status, expires_at: job.expires_at };
  } catch (error) {
    if (job?.id) {
      await updateJob(job.id, {
        status: "FAILED",
...
export async function pollOperatorAsyncTranscription({ jobId, organizationId }) {
  ...
  if (text(job.provider) !== OWNED_PROVIDER) {
    throw new Error("AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_EXTERNAL_PROVIDER_FORBIDDEN");
  }
  ...
  if (settled?.failed === true || settled?.success === false) {
    ...
    return { success: false, pending: false, contract: CONTRACT, job_id: job.id, status: "FAILED", error: "Voice transcription failed" };
  }
  ...
  return { success: true, pending: false, contract: CONTRACT, job_id: job.id, provider: OWNED_PROVIDER, status: "COMPLETED", ...transcription };
}
...
export async function cancelOperatorAsyncTranscription({ jobId, organizationId, reason = null }) {
  ...
  if (job.provider && text(job.provider) !== OWNED_PROVIDER) {
    throw new Error("AVANTIQO_OPERATOR_ASYNC_TRANSCRIPTION_EXTERNAL_PROVIDER_FORBIDDEN");
  }
  const lease = await loadLease(job.lease_id);
  if (lease.state === "ACTIVE") {
