...
function conversationAttachmentContextMessage(attachments) {
  ...
  const lines = files.slice(0, 8).map((file, index) => {
    ...
    const preparedLine = prepared.type
      ? `prepared_candidate=${text(prepared.type, 80)}; prepared_status=${text(prepared.status, 120) || "unknown"}; prepared_data=${text(JSON.stringify(prepared.statement || {}), 3200)}; missing_fields=${list(prepared.missing_fields).join(",") || "none"}; authorization_effect=NONE`
      : null;
    return [
...
export async function runSyntheticIntelligenceTurn(options = {}) {
  ...
  const preparedAttachmentReflex = hasPreparedAttachmentReflexCandidate(effectiveOptions.conversationAttachments);
