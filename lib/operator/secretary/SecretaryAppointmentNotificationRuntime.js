...
export async function materializeSecretaryAppointmentReminders({ now = new Date() } = {}) {
  ...
  const result = await supabaseAdmin.rpc("secretary_materialize_appointment_reminders", {
    p_now: at,
  });
  if (result.error) throw result.error;
  return result.data || null;
...
export async function claimSecretaryAppointmentNotification({ workerId, leaseSeconds = 180 } = {}) {
  ...
  const result = await supabaseAdmin.rpc("claim_secretary_appointment_notification", {
    p_worker_id: worker,
...
async function currentEvent(notification) {
  return one(
    supabaseAdmin
      ...
      .select("id,organization_id,contact_party_id,self_service_reference,event_type,status,starts_at,ends_at,timezone,location")
      .eq("organization_id", notification.organization_id)
      .eq("id", notification.calendar_event_id)
      .maybeSingle(),
  );
}
...
async function contactPreferences(notification) {
  const [profile, latestLanguage, settings] = await Promise.all([
    one(
      supabaseAdmin
        ...
        .select("preferred_language,preferred_channel,allow_messages")
        .eq("organization_id", notification.organization_id)
        .eq("party_id", notification.contact_party_id)
        .maybeSingle(),
    ),
    one(
      supabaseAdmin
        .from("secretary_message_reception_requests")
        .select("detected_language,completed_at")
        .eq("organization_id", notification.organization_id)
        .eq("contact_party_id", notification.contact_party_id)
  ...
  return {
    ...
    language:
      text(profile?.preferred_language, 80) ||
      text(latestLanguage?.detected_language, 80) ||
      text(settings?.default_language, 80) ||
      "en",
  };
}
...
async function communicationConversation(notification, preferredChannel) {
  const rows = await many(
    supabaseAdmin
      ...
      .select("id,organization_id,connection_id,provider,channel_type,external_participant_id,external_participant_address,customer_party_id,subject,status,last_message_at,updated_at")
      .eq("organization_id", notification.organization_id)
      .eq("customer_party_id", notification.contact_party_id)
      .eq("status", "OPEN")
  ...
  return (
    rows.find(
      (row) =>
        text(row.provider, 120).toLowerCase() === preferredChannel ||
        text(row.channel_type, 120).toLowerCase() === preferredChannel,
    ) || rows[0]
  );
}
...
function safeNotificationEvidence(notification, event, language) {
  return {
    ...
    reminder_minutes_before:
      notification.reminder_minutes_before === null
        ? null
        : Number(notification.reminder_minutes_before),
    appointment: {
...
async function notificationText(notification, event, language) {
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    ...
    messages: [
      {
        role: "user",
        content: JSON.stringify(safeNotificationEvidence(notification, event, language)),
      },
    ],
    tools: [],
    ...
    metadata: {
      module: "SECRETARY",
      operation: "APPOINTMENT_CONTACT_NOTIFICATION",
...
async function reserveNotificationMessage({ notification, conversation, body }) {
  const result = await supabaseAdmin.rpc("secretary_reserve_appointment_notification_message", {
  ...
  if (!message?.id) throw new Error("SECRETARY_APPOINTMENT_NOTIFICATION_MESSAGE_REQUIRED");
  return message;
}
...
export async function processSecretaryAppointmentNotification(notification) {
  ...
  if (!stateStillCurrent(notification, event)) {
    const row = await updateNotification(notification.id, {
      ...
      metadata: {
        ...
        superseded_reason: "APPOINTMENT_STATE_CHANGED",
      },
    });
  ...
  if (!preferences.allow_messages) {
    const row = await updateNotification(notification.id, {
      ...
      lease_token: null,
      lease_expires_at: null,
      last_error: "SECRETARY_CONTACT_MESSAGES_DISABLED",
    });
    return { status: "skipped", reason: "CONTACT_MESSAGES_DISABLED", notification: row };
  }

  const conversation = await communicationConversation(
    notification,
  ...
  if (!conversation) {
    const row = await updateNotification(notification.id, {
      ...
      lease_token: null,
      lease_expires_at: null,
      last_error: "SECRETARY_APPOINTMENT_NOTIFICATION_CHANNEL_UNAVAILABLE",
    });
  ...
  if (!message) {
    const body = await notificationText(notification, event, preferences.language);
    message = await reserveNotificationMessage({ notification, conversation, body });
  }
  ...
  if (["SENDING", "FAILED"].includes(priorStatus)) {
    const row = await updateNotification(notification.id, {
      ...
      lease_token: null,
      lease_expires_at: null,
      last_error: `SECRETARY_APPOINTMENT_NOTIFICATION_REDELIVERY_SUPPRESSED:${priorStatus}`,
    });
    return {
      status: "skipped",
      reason: "AMBIGUOUS_OR_FAILED_DELIVERY_RETRY_SUPPRESSED",
      notification: row,
      message,
    };
  }
  ...
  if (deliveredStatus === "SENT") {
    const row = await updateNotification(notification.id, {
      status: "SENT",
      sent_at: delivered.sent_at || new Date().toISOString(),
      conversation_id: conversation.id,
      message_id: delivered.id || message.id,
      lease_token: null,
      lease_expires_at: null,
      last_error: null,
    });
  ...
  const row = await updateNotification(notification.id, {
    status: "FAILED",
    available_at: new Date(Date.now() + Math.min(300, 15 * 2 ** Math.min(Number(notification.attempt_count || 1), 5)) * 1000).toISOString(),
    conversation_id: conversation.id,
    message_id: delivered?.id || message.id,
    lease_token: null,
    lease_expires_at: null,
    last_error: `SECRETARY_APPOINTMENT_NOTIFICATION_NOT_SENT:${deliveredStatus || "UNKNOWN"}`,
  });
