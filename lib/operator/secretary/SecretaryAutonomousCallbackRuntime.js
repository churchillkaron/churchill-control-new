...
} from "./SecretaryCoverageRoutingRuntime";
...
async function coverageRoutingForFollowUp(row) {
  const ownerPartyId = text(row?.owner_party_id, 120);
  const organizationId = text(row?.organization_id, 120);
  if (!ownerPartyId || !organizationId) return null;
...
async function promoteFollowUp(row) {
  ...
  const routing = await coverageRoutingForFollowUp(row);
  const routingMetadata = routing ? secretaryCoverageRoutingMetadata(routing) : {};
  const operationalAssignee = routing?.operational_assignee_party_id || row.owner_party_id || null;
  ...
  if (
    ...
    text(metadata.operational_assignee_party_id, 120) === text(operationalAssignee, 120)
  ) {
    return row;
  }
...
export async function runSecretaryMessageReceptionAutonomous(request) {
  ...
  return {
    ...
    callback_operational_assignee_party_id: promoted?.metadata?.operational_assignee_party_id || null,
    callback_coverage_applied: promoted?.metadata?.secretary_coverage_applied === true,
  };
}
