...
export async function rescheduleSecretaryBookedMeeting({ context, payload = {} } = {}) {
  ...
  if (!evidenceId) throw new Error("SECRETARY_BOOKED_MEETING_EVIDENCE_REQUIRED");
  ...
  const rpc = await secretaryRecoveryMutationResult(
    supabaseAdmin.rpc("secretary_reschedule_booked_meeting_coordination", {
    ...
    [["coordination_id", coordinationId], ["transition_kind", "RESCHEDULE_BOOKED"], ["evidence_id", evidenceId]],
  );
  ...
  try {
    ...
    return {
      ...
      transition_kind: "RESCHEDULE_BOOKED",
...
export async function cancelSecretaryBookedMeeting({ context, payload = {} } = {}) {
  ...
  const rpc = await secretaryRecoveryMutationResult(
    supabaseAdmin.rpc("secretary_cancel_booked_meeting_coordination", {
      p_organization_id: organization,
      p_coordination_id: coordinationId,
      p_changed_by_party_id: actor,
      p_evidence_id: evidenceId,
    ...
    [["coordination_id", coordinationId], ["transition_kind", "CANCEL_BOOKED"], ["evidence_id", evidenceId]],
  );
  const change = object(rpc.data);
