...
import { loadSecretaryBusinessBookingContext, executeSecretaryBusinessAgreement } from "./SecretaryBusinessAgreementExecutionRuntime";
...
function externalPublicContext(publicContext, ownAppointments, businessBookingContext = {}) {
...
function decisionSystem() {
  return [
    ...
    "When BOOK_APPOINTMENT is clearly for a supported business service in public_context.business_booking_context, classify business_booking_type. Use PEST_CONTROL_SERVICE only for an explicit pest-control service booking.",
    "For PEST_CONTROL_SERVICE, business_agreement_explicit may be true only after the caller explicitly accepts/confirms the booking. service_name must come from the conversation, service_protocol must exactly match a supplied active protocol code or name when unambiguous, and location must be explicit or uniquely resolved. Never invent any of these.",
    "Preserve the caller's language. Do not switch languages unless requested.",
    "Return exactly one JSON object with keys: action, response_language, response_text, starts_at, ends_at, appointment_reference, appointment_title, appointment_description, location, callback_reason, callback_due_at, message_text, business_booking_type, business_agreement_explicit, service_name, service_protocol, duration_minutes.",
...
export async function runSecretaryCallerTurn({ callId, message, language = null } = {}) {
  ...
  const [rawPublicContext, ownAppointments, conversation, businessBookingContext] = await Promise.all([
    ...
    loadSecretaryBusinessBookingContext({
      ...
      contactPartyId: call.contact_party_id || null,
    }).catch(() => ({})),
  ]);
  const publicContext = externalPublicContext(rawPublicContext, ownAppointments, businessBookingContext);
  ...
  try {
    ...
    if (decision.action === "BOOK_APPOINTMENT" && actionResult?.appointment) {
      const businessExecution = await executeSecretaryBusinessAgreement({
        organizationId: call.organization_id,
        contactPartyId: call.contact_party_id || null,
        callId: call.id,
        decision,
        appointment: actionResult.appointment,
        bookingPolicy: object(rawPublicContext?.booking_policy),
      });
