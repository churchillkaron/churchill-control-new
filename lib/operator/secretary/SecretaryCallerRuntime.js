...
async function callContext(callId) {
  ...
  const call = await one(
    supabaseAdmin
      ...
      .select("id,organization_id,phone_line_id,contact_party_id,remote_address,status,started_at")
      .eq("id", id)
      .maybeSingle(),
  );
...
async function nextSequence(organizationId, callId) {
  const result = await supabaseAdmin
    ...
    .maybeSingle();
  if (result.error) throw result.error;
  return Number(result.data?.sequence_number || 0) + 1;
}
...
export async function appendSecretaryCallTurn({ callId, speaker, transcript, language = null, intent = null, decision = {}, metadata = {} } = {}) {
  ...
  const sequence = await nextSequence(call.organization_id, call.id);
  const row = await one(
    supabaseAdmin
      .from("secretary_call_turns")
      .insert({
        ...
        transcript: content,
        language: text(language, 80) || null,
        intent: text(intent, 120) || null,
...
export async function bookCallerOwnAppointment({ callId, startsAt, endsAt, title = "Appointment", description = null, location = null } = {}) {
  ...
  const availability = await checkCallerAvailability({ callId, startsAt: starts, endsAt: ends });
  if (!availability.available) throw new Error("SECRETARY_CALLER_SLOT_UNAVAILABLE");
  ...
  const event = await one(
    supabaseAdmin
      ...
      .select("id,status,starts_at,ends_at,timezone,event_type")
      .single(),
  );
  return { status: "completed", appointment: event, internal_calendar_details_disclosed: false };
}
