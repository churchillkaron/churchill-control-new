...
async function bookingSettings(organizationId) {
  ...
  return {
    default_timezone: text(settings?.default_timezone, 120) || "UTC",
    booking_policy: object(settings?.booking_policy),
  };
}

function enforceSelfServiceWindow(policy, startsAt) {
  const starts = Date.parse(startsAt);
  const minNoticeMinutes = Math.max(0, Number(policy.min_notice_minutes || 0));
  const maxDaysAhead = Math.max(1, Math.min(730, Number(policy.max_days_ahead || 365)));
  const now = Date.now();
  if (starts < now + minNoticeMinutes * 60 * 1000) throw new Error("SECRETARY_CALLER_BOOKING_MIN_NOTICE");
  if (starts > now + maxDaysAhead * 24 * 60 * 60 * 1000) throw new Error("SECRETARY_CALLER_BOOKING_TOO_FAR_AHEAD");
}
...
async function ownAppointmentByReference({ organizationId, contactPartyId, appointmentReference }) {
  ...
  if (!reference) throw new Error("SECRETARY_CALLER_APPOINTMENT_REFERENCE_REQUIRED");
  return one(
    supabaseAdmin
...
export async function readCallerPublicContext({ callId } = {}) {
  ...
  return {
    ...
    booking_policy: {
      appointment_duration_minutes: Number(bookingPolicy.appointment_duration_minutes || 0) || null,
...
export async function listCallerOwnAppointments({ callId, from = null, limit = 10 } = {}) {
  const { call } = await callContext(callId);
  if (!call.contact_party_id) {
    return { status: "completed", count: 0, appointments: [], contact_identified: false };
  }
  const fromAt = iso(from || new Date().toISOString(), "appointments_from", true);
  const cappedLimit = Math.max(1, Math.min(Number(limit) || 10, 20));
  const result = await supabaseAdmin
    .from("secretary_calendar_events")
    .select("self_service_reference,status,starts_at,ends_at,timezone,location,event_type")
    .eq("organization_id", call.organization_id)
    .eq("contact_party_id", call.contact_party_id)
    .eq("event_type", "APPOINTMENT")
...
export async function bookCallerOwnAppointment({ callId, startsAt, endsAt, title = "Appointment", description = null, location = null } = {}) {
  ...
  const settings = await bookingSettings(call.organization_id);
  const bookingPolicy = settings.booking_policy;
  if (bookingPolicy.public_booking_enabled === false) throw new Error("SECRETARY_CALLER_PUBLIC_BOOKING_DISABLED");

  const starts = iso(startsAt, "starts_at", true);
  const ends = iso(endsAt, "ends_at", true);
  if (Date.parse(ends) <= Date.parse(starts)) throw new Error("SECRETARY_CALLER_BOOKING_WINDOW_INVALID");
  enforceSelfServiceWindow(bookingPolicy, starts);
  ...
  return {
    status: "completed",
    appointment: publicAppointment(event),
    internal_calendar_details_disclosed: false,
  };
}

export async function rescheduleCallerOwnAppointment({
  callId,
  ...
  const { call, line } = await callContext(callId);
  if (!call.contact_party_id) throw new Error("SECRETARY_CALLER_CONTACT_REQUIRED_FOR_APPOINTMENT_CHANGE");
  const reference = text(appointmentReference, 120);
  if (!reference) throw new Error("SECRETARY_CALLER_APPOINTMENT_REFERENCE_REQUIRED");
  const starts = iso(startsAt, "starts_at", true);
  const ends = iso(endsAt, "ends_at", true);
  ...
  if (policy.public_reschedule_enabled === false) throw new Error("SECRETARY_CALLER_PUBLIC_RESCHEDULE_DISABLED");
  enforceSelfServiceWindow(policy, starts);
  ...
  if (!prior) throw new Error("SECRETARY_CALLER_APPOINTMENT_NOT_FOUND");
  if (
    ["TENTATIVE", "CONFIRMED"].includes(text(prior.status, 40).toUpperCase()) &&
    prior.starts_at === starts &&
    prior.ends_at === ends
  ) {
    return { status: "completed", appointment: publicAppointment(prior), replayed: true };
  }
  if (!["TENTATIVE", "CONFIRMED"].includes(text(prior.status, 40).toUpperCase())) {
    throw new Error("SECRETARY_CALLER_APPOINTMENT_NOT_ACTIVE");
  }

  const result = await supabaseAdmin.rpc("secretary_reschedule_own_appointment_ref", {
    p_organization_id: call.organization_id,
    p_contact_party_id: call.contact_party_id,
    p_self_service_reference: reference,
    p_starts_at: starts,
    p_ends_at: ends,
    p_timezone: text(timezone || line?.timezone || settings.default_timezone, 120) || "UTC",
  });
  if (result.error) {
  ...
  return { status: "completed", appointment: publicAppointment(event), replayed: false };
}

export async function cancelCallerOwnAppointment({ callId, appointmentReference } = {}) {
  const { call } = await callContext(callId);
  if (!call.contact_party_id) throw new Error("SECRETARY_CALLER_CONTACT_REQUIRED_FOR_APPOINTMENT_CHANGE");
  const reference = text(appointmentReference, 120);
  if (!reference) throw new Error("SECRETARY_CALLER_APPOINTMENT_REFERENCE_REQUIRED");
  ...
  const settings = await bookingSettings(call.organization_id);
  if (settings.booking_policy.public_cancellation_enabled === false) {
    throw new Error("SECRETARY_CALLER_PUBLIC_CANCELLATION_DISABLED");
  }
