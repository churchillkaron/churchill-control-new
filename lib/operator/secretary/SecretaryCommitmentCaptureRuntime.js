...
async function sourceEvidence(extraction) {
  ...
  if (kind === "MESSAGE") {
    ...
    return {
      ...
      message: text(message.body, 12000),
...
function extractionSystem() {
  return [
    ...
    "Examples worth capturing: 'call me Friday', 'please send it tomorrow', 'I will send the document Monday' (create a REVIEW follow-up to check), 'let us meet Tuesday'.",
    ...
    "Return exactly one JSON object: {\"commitments\":[{\"explicit\":true,\"action_type\":\"CALL\",\"reason\":\"...\",\"due_at\":\"ISO timestamp or null\"}]}.",
  ].join("\n");
}
  ...
  return list(result?.parsed?.commitments)
    ...
    .map((item) => {
      ...
      return {
        ...
        action_type: ACTION_TYPES.has(actionType) ? actionType : "OTHER",
        reason,
        due_at: Number.isFinite(dueParsed) ? new Date(dueParsed).toISOString() : null,
      };
    })
...
function itemKey(extractionId, item) {
  const fingerprint = createHash("sha256")
    .update(`${item.action_type}|${item.reason}|${item.due_at || ""}`)
    .digest("hex")
    .slice(0, 24);
