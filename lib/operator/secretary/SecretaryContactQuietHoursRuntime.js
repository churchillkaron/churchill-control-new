...
export function evaluateSecretaryContactQuietHours({
  ...
  const effectiveTimezone = validTimezone(timezone || policy.timezone || "UTC");
  ...
  const untilMs = Date.parse(text(policy.until || policy.blocked_until || policy.do_not_disturb_until, 120));
...
export const SECRETARY_CONTACT_QUIET_HOURS_CONTRACT = Object.freeze({
  do_not_disturb: {
    permanent_flags: ["active", "enabled", "blocked", "do_not_disturb"],
