...
function sameInstant(left, right) {
  const leftMs = Date.parse(text(left, 180));
  const rightMs = Date.parse(text(right, 180));
...
export async function updateSecretaryContactRecord({ context, payload = {} } = {}) {
  ...
  if (!sameInstant(initial.updated_at, expectedUpdatedAt)) throw new Error("SECRETARY_CONTACT_MAINTENANCE_STALE_RECORD");
