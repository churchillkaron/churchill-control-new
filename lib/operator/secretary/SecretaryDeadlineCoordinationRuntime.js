...
function followUpId(taskId, kind, scope, version, dueAt) {
  return deterministicUuid(`avantiqo-secretary-deadline-follow-up-v1:${taskId}:${kind}:${scope || "deadline"}:${version}:${dueAt}`);
}
...
async function ensureFollowUp({ task, kind, scope = null, version = 1, dueAt, partyId = null, internal = false, instruction }) {
  ...
  const id = followUpId(task.id, kind, scope, version, normalizedDueAt);
