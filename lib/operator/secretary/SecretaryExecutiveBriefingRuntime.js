...
export async function readSecretaryExecutiveBriefing({ context, payload = {} } = {}) {
  ...
  const [agenda, dueWork, tasks, followUps, calls, jobResult, inboxTriage] = await Promise.all([
    ...
    readSecretaryInboxTriage({ context, payload: { limit } }),
  ]);
  ...
  const inboxAttention = Array.isArray(inboxTriage.executive_attention) ? inboxTriage.executive_attention : [];
  const inboxWaitingExternal = Array.isArray(inboxTriage.waiting_external) ? inboxTriage.waiting_external : [];
  const inboxSecretaryHandling = Array.isArray(inboxTriage.secretary_handling) ? inboxTriage.secretary_handling : [];
  const inboxHandled = Array.isArray(inboxTriage.secretary_handled) ? inboxTriage.secretary_handled : [];
  const inboxFyi = Array.isArray(inboxTriage.fyi) ? inboxTriage.fyi : [];
  ...
  const secretaryHandling = {
    ...
    inbox_handling: inboxSecretaryHandling,
    inbox_waiting_external: inboxWaitingExternal,
    count: active.length + secretaryFollowUps.length + inboxSecretaryHandling.length + inboxWaitingExternal.length,
  };
  const executiveAttentionCount = decisionsRequired.length + inboxAttention.length;
  const urgentExecutiveTaskCount = executiveTasks.filter((task) => task.priority === "URGENT").length;
  const executiveInterruptCount = executiveAttentionCount + urgentExecutiveTaskCount;

  const headline = executiveAttentionCount > 0
    ? `${executiveAttentionCount} decision or review item${executiveAttentionCount === 1 ? "" : "s"} need your attention; Secretary is handling ${secretaryHandling.count} active item${secretaryHandling.count === 1 ? "" : "s"}.`
    : `No executive decision or inbox review is required; Secretary is handling ${secretaryHandling.count} active item${secretaryHandling.count === 1 ? "" : "s"}.`;

  return {
    status: "completed",
    contract: "AVANTIQO_EXECUTIVE_SECRETARY_DESK_BRIEFING_V3",
    generated_at: new Date().toISOString(),
    window: { from, to },
    headline,
    executive_desk: {
      decisions_required: decisionsRequired,
      correspondence_attention: inboxAttention,
      ...
      correspondence: {
        attention_required: inboxAttention,
        secretary_handling: inboxSecretaryHandling,
        waiting_external: inboxWaitingExternal,
        handled: inboxHandled,
        fyi: inboxFyi,
      },
      secretary_handling: secretaryHandling,
      recently_completed: recentCompleted,
      executive_interrupt_count: executiveInterruptCount,
      secretary_owned_count: secretaryHandling.count,
      no_action_required: executiveInterruptCount === 0,
    },
    ...
    inbox_attention_is_exception_based: true,
    approval_extends_authority: false,
