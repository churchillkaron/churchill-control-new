...
export async function protectSecretaryExecutiveTime({ context, payload = {} } = {}) {
  ...
  try {
    ...
    throw attachActionIdentityEvidence(error, [
      `owner_party_id:${auth.owner}`,
      `protection_key:${key}`,
      `evidence_id:${evidenceId}`,
    ]);
...
export async function releaseSecretaryExecutiveProtection({ context, payload = {} } = {}) {
  ...
  if (event.status === "CANCELLED") {
    if (metadata.release_evidence_id === evidenceId && metadata.released_at === releasedAt) {
      return { status: "released", contract: CONTRACT, protection: event, event_id: protectionId, evidence_id: evidenceId, released_at: releasedAt, replay_safe: true, ...safetyFlags() };
    }
  ...
  const updateResult = await secretaryRecoveryMutationResult(
    supabaseAdmin.from("secretary_calendar_events")
      .update({
        status: "CANCELLED",
      ...
      .select("*")
      .maybeSingle(),
    [["event_id", protectionId], ["evidence_id", evidenceId], ["released_at", releasedAt]],
  );
