...
export async function processSecretaryFollowUpExecution(execution) {
  ...
  const action = text(execution.action_type, 40).toUpperCase();
  const outcome = action === "CALL" ? await processCall(execution, preferences) : await processMessage(execution, preferences);
  if (outcome.status === "deferred") {
    const row = await updateExecution(execution, {
      status: "FAILED",
      attempt_count: Math.max(0, Number(execution.attempt_count || 1) - 1),
      available_at: outcome.defer_until,
      completed_at: null,
      lease_token: null,
      lease_expires_at: null,
      last_error: `${outcome.reason}:DEFERRED_UNTIL:${outcome.defer_until}`,
      metadata: { ...object(execution.metadata), quiet_hours_deferred_at: new Date().toISOString(), quiet_hours_deferred_until: outcome.defer_until, quiet_hours_timezone: outcome.quiet_hours?.timezone || preferences.timezone },
    });
    return { status: "deferred", execution: row, outcome };
  }
