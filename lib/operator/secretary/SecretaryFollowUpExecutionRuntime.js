...
import { secretaryFollowUpRecoveryDirective } from "@/lib/operator/secretary/SecretaryFollowUpRecoveryDirectiveRuntime";
import { escalateSecretaryFollowUpExecution } from "@/lib/operator/secretary/SecretaryFollowUpEscalationRuntime";
import {
...
export async function processSecretaryFollowUpExecution(execution) {
  ...
  if (coverage.routing.coverage_routing_review_required === true) {
    ...
    const recovery = secretaryFollowUpRecoveryDirective({ reason, execution, followUp: current });
    const row = await updateExecution(execution, {
      ...
      metadata: {
        ...
        business_partner_recovery: recovery,
        external_authority_used: false,
      },
    });
    await escalateSecretaryFollowUpExecution({ execution: row, reason }).catch(() => null);
    return { status: "skipped", execution: row, outcome: { status: "skipped", reason }, recovery, coverage_routing: coverage.routing };
  }
  ...
  const recoveryDirective = outcome.status === "completed" || outcome.status === "queued"
    ? null
    : secretaryFollowUpRecoveryDirective({
        reason: outcome.reason,
        execution: executionWithCoverage,
        followUp: currentWithCoverage,
      });
  const row = await updateExecution(executionWithCoverage, {
    ...
    metadata: {
      ...object(executionWithCoverage.metadata),
      ...(recoveryDirective ? { business_partner_recovery: recoveryDirective } : {}),
    },
  });
  if (recoveryDirective) {
    await escalateSecretaryFollowUpExecution({ execution: row, reason: outcome.reason }).catch(() => null);
  }
  return { status: outcome.status, execution: row, outcome, recovery: recoveryDirective, coverage_routing: coverage.routing };
}
...
export async function reconcileQueuedSecretaryFollowUpExecutions({ limit = 100 } = {}) {
  ...
  for (const execution of rows) {
    ...
    if (status === "COMPLETED") {
      ...
      const reason = `OUTBOUND_CALL_${status}`;
      const recovery = secretaryFollowUpRecoveryDirective({ reason, execution });
      const updated = await updateExecution(execution, {
        status: "SKIPPED",
        completed_at: new Date().toISOString(),
        last_error: reason,
        metadata: { ...object(execution.metadata), business_partner_recovery: recovery },
      });
      await escalateSecretaryFollowUpExecution({ execution: updated, reason }).catch(() => null);
