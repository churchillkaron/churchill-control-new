...
async function mutateProfile({ organization, partyId, producer }) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const recoveryEvidence = list(produced.output?.recovery_evidence);
    const updated = recoveryEvidence.length
      ? await secretaryRecoveryMutationResult(
          supabaseAdmin.from("secretary_contact_profiles")
      ...
      : await supabaseAdmin.from("secretary_contact_profiles")
          .update({ metadata, updated_at: new Date().toISOString() })
          .eq("organization_id", organization)
          .eq("party_id", partyId)
          .eq("updated_at", profile.updated_at)
