...
const BUSINESS_DECISION_PATTERN = /\b(approve|approval|authorize|authorization|decision|decide|discount|price reduction|exception|policy exception|waive|waiver|negotiate|negotiation|offer terms|special terms)\b/i;
...
function receptionFallbackCategory(action, resultStatus) {
  ...
  if (normalizedAction === "LEAVE_MESSAGE") return "SECRETARY_HANDLE";
  if (normalizedAction === "ATTACHMENT_REVIEW") return "NEEDS_REVIEW";
  return "NEEDS_REVIEW";
}
...
function boundaryEvidence(value) {
  const body = text(value, 12000);
  return {
    highAuthority: secretaryJobInstructionRequiresHighAuthority(body),
    businessDecision: BUSINESS_DECISION_PATTERN.test(body),
  };
}
...
export function normalizeSecretaryInboxTriageDecision({
  ...
  const { highAuthority, businessDecision } = boundaryEvidence(inboundBody);
  if (highAuthority || businessDecision) category = "EXECUTIVE_DECISION";
  ...
  if (RECEPTION_CLOSED_ACTIONS.has(action) && ["SECRETARY_HANDLE", "NEEDS_REVIEW"].includes(category) && !highAuthority && !businessDecision) {
  ...
  return {
    ...
    business_decision_boundary_detected: businessDecision,
...
async function administrativeCoverage({ organization, ownerPartyId, scope, at }) {
  if (!ownerPartyId) return null;
  return resolveSecretaryActiveCoverage({
    context: {
...
function routeMetadata(routing) {
  return routing ? secretaryCoverageRoutingMetadata(routing) : {};
}
...
async function ensureSecretaryTriageJob({ request, conversation, inbound, triage }) {
  ...
  if (triage.high_authority_boundary_detected || triage.business_decision_boundary_detected) return null;
  ...
  const coverageRouting = await administrativeCoverage({
    organization: request.organization_id,
    ownerPartyId,
    scope: "CORRESPONDENCE_TRIAGE",
    at: inbound.received_at || inbound.created_at || new Date().toISOString(),
  });
  const coverageMetadata = routeMetadata(coverageRouting);
  ...
  const insert = await supabaseAdmin
    ...
    .insert({
      ...
      metadata: {
        ...
        secretary_coverage_scope: "CORRESPONDENCE_TRIAGE",
        ...coverageMetadata,
        external_authority_used: false,
      },
    })
...
export async function recordSecretaryInboundTriage({ request = {}, result = {} } = {}) {
  ...
  const triageEvidence = {
    ...
    canonical_owner_party_id: jobMetadata.canonical_owner_party_id || null,
    operational_assignee_party_id: jobMetadata.operational_assignee_party_id || null,
    secretary_coverage_applied: jobMetadata.secretary_coverage_applied === true,
    secretary_coverage_id: jobMetadata.secretary_coverage_id || null,
    secretary_coverage_scope: jobMetadata.secretary_coverage_scope || null,
    triaged_at: now,
  };
...
export async function repairSecretaryMissingInboundTriage({ limit = 50 } = {}) {
  ...
  const rows = await many(
    supabaseAdmin
      ...
      .select("*")
      .eq("status", "COMPLETED")
      .is("metadata->secretary_inbox_triage", null)
  ...
  for (const request of rows) {
    ...
    const repaired = await recordSecretaryInboundTriage({
      request,
      result: {
        status: "completed",
        action: request.decision_action || decision.action || null,
        response_language: decision.response_language || null,
        response_text: decision.response_text || null,
        action_result: object(request.action_result),
        response_message: request.response_message_id ? { id: request.response_message_id } : null,
      },
    });
    results.push({ request_id: request.id, conversation_id: repaired.conversation_id, triage: repaired.triage });
  }
  return {
    status: "completed",
    contract: "AVANTIQO_EXECUTIVE_SECRETARY_INBOX_TRIAGE_V1",
    ...
    oldest_untriaged_first: true,
    completed_reception_not_lost_after_triage_interruption: true,
    external_authority_used: false,
  };
}
...
async function ensureWaitingExternalFollowUp({ organization, conversation, outbound, waitHours, now }) {
  ...
  const boundary = boundaryEvidence(outboundBody);
  if (!outboundBody || boundary.highAuthority || boundary.businessDecision) return null;
  ...
  const ownerPartyId = await secretaryOwnerPartyId(organization);
  const coverageRouting = await administrativeCoverage({
    organization,
    ownerPartyId,
...
export async function reconcileSecretaryWaitingExternal({
  ...
  for (const conversation of conversations) {
    ...
    const boundary = boundaryEvidence(text(outbound.body, 8000));
    const blocked = boundary.highAuthority || boundary.businessDecision;
    ...
    const triage = {
      category: blocked ? "EXECUTIVE_DECISION" : "WAITING_EXTERNAL",
      priority: blocked ? "HIGH" : "NORMAL",
      rationale: blocked
        ? "Latest outbound thread contains a governed authority or executive-decision boundary and cannot be autonomously chased."
        : "Awaiting an external response to an existing outbound request.",
      executive_attention_required: blocked,
      secretary_owns_follow_through: !blocked,
      handled_by_reception: false,
      high_authority_boundary_detected: boundary.highAuthority,
      business_decision_boundary_detected: boundary.businessDecision,
      source_outbound_message_id: outbound.id,
      waiting_external_since: conversation.last_outbound_at,
      waiting_external_follow_up_id: followUp?.id || null,
      canonical_owner_party_id: followUpMetadata.canonical_owner_party_id || null,
      operational_assignee_party_id: followUpMetadata.operational_assignee_party_id || null,
      secretary_coverage_applied: followUpMetadata.secretary_coverage_applied === true,
  ...
  return {
    ...
    business_decision_auto_chase_blocked: true,
    coverage_routing_respected: true,
    external_authority_used: false,
  };
}
...
export default Object.freeze({
  ...
  repairMissingInbound: repairSecretaryMissingInboundTriage,
