...
function canonicalOwnerPartyId(job = {}) {
...
export async function approveSecretaryJobStep({ context, payload = {} } = {}) {
  ...
  const canonicalOwner = canonicalOwnerPartyId(job);
  if (!canonicalOwner) throw new Error("SECRETARY_JOB_CANONICAL_OWNER_REQUIRED");
  if (approvedByPartyId !== canonicalOwner) throw new Error("SECRETARY_JOB_APPROVAL_CANONICAL_OWNER_REQUIRED");
  ...
  const exactApproval = {
    ...
    coverage_authority_delegated: false,
