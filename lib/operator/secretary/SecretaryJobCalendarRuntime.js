...
export async function executeSecretaryJobCalendarStep({ job, step, routing = null, routingMetadata = null } = {}) {
  const resolution = await resolveCalendarEvent(job, step);
  if (resolution.status !== "resolved") return resolution;

  const canonicalOwnerPartyId = text(routing?.canonical_owner_party_id || job.requested_by_party_id, 120) || null;
  const executionActorPartyId = text(routing?.execution_actor_party_id || canonicalOwnerPartyId, 120) || null;
  const coverageMetadata = object(routingMetadata);
  ...
  try {
    const created = await createSecretaryCalendarEventAtomic({
      context: {
        ...
        actor: { partyId: executionActorPartyId },
        metadata: { partyId: executionActorPartyId },
      },
      payload: {
        ...
        event_type: "MEETING",
        status: "CONFIRMED",
        owner_party_id: canonicalOwnerPartyId,
        ...
        metadata: {
          ...
          operational_assignee_party_id: executionActorPartyId,
          secretary_calendar_owner_preserved: true,
          external_authority_used: false,
        },
      },
    });
