...
import { emitBusinessPartnerBusinessEvent } from "@/lib/operator/runtime/BusinessPartnerBusinessEventAdapterRuntime";
...
import { createTask } from "@/lib/operator/secretary/SecretaryRuntime";
import { discoverSecretaryProspects } from "@/lib/operator/secretary/SecretaryProspectDiscoveryRuntime";
import { executeSecretaryJobCalendarStep } from "@/lib/operator/secretary/SecretaryJobCalendarRuntime";
import {
...
} from "@/lib/operator/secretary/SecretaryJobCoverageExecutionRuntime";
import {
  collectSecretaryJobResponses,
  compareSecretaryJobResponses,
  ensureSecretaryJobResponseWatcher,
} from "@/lib/operator/secretary/SecretaryJobResponseRuntime";
...
const TRAVEL_HIGH_AUTHORITY_PATTERN = /(?:\b(?:book|reserve|purchase|buy|issue)\b[\s\S]{0,100}\b(?:flight|airfare|airline\s+ticket|ticket|hotel|room|accommodation|lodging|car\s+rental|rental\s+car|transfer|taxi|ride|train|rail\s+ticket|ferry|boat\s+ticket)\b)|(?:\b(?:accept|confirm|commit\s+to)\b[\s\S]{0,100}\b(?:fare|rate|paid\s+reservation|booking\s+terms|cancellation\s+fee)\b)/i;
...
const SAFE_TRAVEL_INFORMATION_PATTERN = /\b(?:confirm|check|find\s+out|request|ask|research|compare|obtain|get|inquire|enquire)\b[\s\S]{0,120}\b(?:fare|rate|booking\s+terms|cancellation\s+fee)\b[\s\S]{0,80}\b(?:amount|availability|information|only)\b/i;
const BINDING_TRAVEL_ACTION_PATTERN = /\b(?:accept|commit(?:\s+to)?|agree(?:\s+to)?|book|reserve|purchase|buy|issue|pay|make\s+(?:a\s+)?payment)\b/i;
...
export function secretaryJobInstructionRequiresHighAuthority(value) {
  const instruction = text(value, 5000);
  if (!instruction) return false;
  const generalHighAuthority = HIGH_AUTHORITY_PATTERN.test(instruction);
  const travelHighAuthority = TRAVEL_HIGH_AUTHORITY_PATTERN.test(instruction);
  const safeTravelInformation = SAFE_TRAVEL_INFORMATION_PATTERN.test(instruction)
    && !BINDING_TRAVEL_ACTION_PATTERN.test(instruction);
  if (safeTravelInformation && !generalHighAuthority) return false;
  const highAuthority = generalHighAuthority || travelHighAuthority;
  if (SAFE_COMMERCIAL_INFORMATION_PATTERN.test(instruction) && !highAuthority) return false;
  return highAuthority;
}

function requiresHighAuthority(value) {
  return secretaryJobInstructionRequiresHighAuthority(value);
}

function hasExactStepApproval(job, step) {
  ...
  return approval.kind === "EXPLICIT_STEP_APPROVAL"
    ...
    && secretaryJobExactApprovalOwnedByCanonicalOwner(job, step)
...
async function emitSecretaryJobTerminalEvent(job) {
  const organizationId = text(job?.organization_id, 120);
  const jobId = text(job?.id, 120);
  const status = text(job?.status, 40).toUpperCase();
  if (!organizationId || !jobId || !["COMPLETED", "FAILED", "CANCELLED"].includes(status)) return null;
  return emitBusinessPartnerBusinessEvent({
    organization_id: organizationId,
    event_source: "secretary",
    event_type: "JOB_TERMINAL",
    correlations: [`secretary_job:${jobId}`],
    identity: `${jobId}:${status}:${text(job?.updated_at || job?.completed_at, 120)}`,
    evidence: {
      secretary_job_id: jobId,
      secretary_job_status: status,
      result_summary: text(job?.result_summary, 4000) || null,
      completed_at: text(job?.completed_at, 120) || null,
      authorization_effect: "NONE",
    },
  });
}

class SecretaryJobCancellationInterrupt extends Error {
  constructor(job) {
    super("SECRETARY_JOB_CANCELLED_DURING_EXECUTION");
    this.name = "SecretaryJobCancellationInterrupt";
    this.job = job;
  }
}

async function currentSecretaryJob(job) {
  return one(
    supabaseAdmin
      .from("secretary_jobs")
      .select("*")
...
async function assertSecretaryJobActive(job) {
  const current = await currentSecretaryJob(job);
  if (!current) throw new Error("SECRETARY_JOB_NOT_FOUND");
  if (current.status === "CANCELLED") throw new SecretaryJobCancellationInterrupt(current);
  return current;
}

async function cancelledSecretaryJobOutcome(job) {
  ...
  await emitSecretaryJobTerminalEvent(job).catch((error) =>
    console.error("BUSINESS_PARTNER_SECRETARY_JOB_TERMINAL_EVENT_FAILED", error),
  );
  return {
    status: "cancelled",
    job,
...
function plannerSystem() {
  return [
    ...
    "Any purchase, order, actual payment, contract, legal commitment, hiring/firing, credential use, irreversible financial action, acceptance of commercial terms, external travel booking/reservation, ticket purchase, paid accommodation, paid transport, fare/rate acceptance, or cancellation-fee commitment requires REVIEW unless the supplied approval policy explicitly and unambiguously authorizes that exact action and limit.",
    "Requesting or comparing quotations, prices, payment terms, minimum order, delivery terms, promotions, availability, travel schedules, fares, room availability or transport options is information gathering and does not itself require approval.",
...
async function persistPlan(job, plan) {
  ...
  await assertSecretaryJobActive(job);
...
async function updateStep(step, patch) {
  ...
  if (!current) throw new Error("SECRETARY_JOB_STEP_NOT_FOUND");
  return current;
}
...
async function updateJob(job, patch) {
  ...
  if (!current) throw new Error("SECRETARY_JOB_NOT_FOUND");
  if (current.status === "CANCELLED") throw new SecretaryJobCancellationInterrupt(current);
  throw new Error("SECRETARY_JOB_STATE_UPDATE_CONFLICT");
}

async function completePlanOnlyJob(job, steps, planSummary = null) {
  ...
  for (const step of steps) {
    ...
    skippedSteps.push(await updateStep(step, {
      status: "SKIPPED",
      result: "SECRETARY_JOB_PLAN_ONLY_NO_EXECUTION",
      last_error: null,
  ...
  await assertSecretaryJobActive(job);
  const summaryText = text(planSummary, 12000) || "Secretary plan prepared; no execution performed.";
...
async function executeResearch(job, step, routingMetadata) {
...
async function executeDiscovery(job, step, routingMetadata) {
...
async function executeCommunication(job, step, routing, routingMetadata) {
  ...
  const followUp = await one(
    supabaseAdmin
      ...
      .insert({
        ...
        owner_party_id: routing.canonical_owner_party_id,
        ...
        metadata: {
          ...
          secretary_job_step_id: step.id,
          ...routingMetadata,
          secretary_job_communication_coverage_snapshot: true,
          external_authority_used: false,
        },
      })
      .select("*")
...
async function executeCreateTask(job, step, routing, routingMetadata) {
  const created = await createTask({
    context: {
      organizationId: job.organization_id,
      entityId: job.entity_id || null,
      actor: { partyId: routing.execution_actor_party_id },
      metadata: { partyId: routing.execution_actor_party_id },
    },
    payload: {
      title: text(step.instruction, 500),
      details: step.instruction,
      owner_party_id: routing.artifact_owner_party_id,
      metadata: {
        secretary_job_id: job.id,
        secretary_job_step_id: step.id,
        ...routingMetadata,
  ...
  return {
    result: `Created internal task ${created.task?.id || ""}`.trim(),
    metadata: {
      ...object(step.metadata),
      ...routingMetadata,
      created_task_id: created.task?.id || null,
      external_authority_used: false,
    },
  };
}

async function executeCreateEvent(job, step, routing, routingMetadata) {
  return executeSecretaryJobCalendarStep({ job, step, routing, routingMetadata });
}

async function executeStep(job, step) {
  const exactApproval = hasExactStepApproval(job, step);
  const highAuthority = requiresHighAuthority(step.instruction);
  if (step.requires_approval || step.status === "APPROVAL_REQUIRED") {
    return { status: "review", reason: "SECRETARY_JOB_STEP_APPROVAL_REQUIRED" };
  }
  if (highAuthority && !exactApproval) {
  ...
  const routing = await resolveSecretaryJobStepExecutionCoverage({
    job,
    step,
    highAuthority,
    at: new Date().toISOString(),
  });
  const routingMetadata = secretaryJobExecutionCoverageMetadata(routing);

  if (step.action_type === "RESEARCH") return { status: "completed", ...(await executeResearch(job, step, routingMetadata)) };
  if (step.action_type === "DISCOVER_CONTACTS") return { status: "completed", ...(await executeDiscovery(job, step, routingMetadata)) };
  if (COMMUNICATION_ACTIONS.has(step.action_type)) return executeCommunication(job, step, routing, routingMetadata);
  if (step.action_type === "CREATE_TASK") return { status: "completed", ...(await executeCreateTask(job, step, routing, routingMetadata)) };
  if (step.action_type === "CREATE_EVENT") return executeCreateEvent(job, step, routing, routingMetadata);
  return {
    status: "review",
    reason: "SECRETARY_JOB_ACTION_NOT_AUTONOMOUSLY_EXECUTABLE",
    metadata: { ...object(step.metadata), ...routingMetadata, external_authority_used: false },
  };
}
...
async function communicationStepCompleted(job, step) {
  ...
  if (!row) return { terminal: true, failed: true, result: "Secretary follow-up disappeared", response_watcher: null };
  if (row.status === "COMPLETED") {
    const responseWatcher = await ensureSecretaryJobResponseWatcher({ job, step });
    return {
      terminal: true,
      failed: false,
      result: row.result || "Secretary communication completed",
      response_watcher: responseWatcher,
    };
  }
  if (row.status === "CANCELLED") return { terminal: true, failed: true, result: row.result || "Secretary communication cancelled", response_watcher: null };
  return { terminal: false };
}

async function summarizeJob(job, steps, comparison = null) {
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    organization_id: job.organization_id,
    party_id: job.requested_by_party_id || null,
    system: [
      "You are Avantiqo Executive Secretary closing a completed business job.",
      "Summarize only the supplied execution and comparison evidence. Do not invent supplier prices, quotes, replies, decisions or recommendations.",
      "State whether the success criteria were satisfied and clearly identify any remaining uncertainty.",
      "A supplier recommendation is advisory only. Never present it as authorization to order, accept terms, pay, sign or contract.",
    ...
    messages: [{
      ...
      content: JSON.stringify({
        objective: job.objective,
        success_criteria: job.success_criteria,
        steps: steps.map((step) => ({ action_type: step.action_type, instruction: step.instruction, status: step.status, result: step.result, metadata: step.metadata })),
...
async function processSecretaryJobInternal(job) {
  job = await assertSecretaryJobActive(job);
  let steps = await jobSteps(job);
  let planSummary = text(object(job.metadata).plan_summary, 4000) || null;
  if (!steps.length) {
  ...
  if (job.autonomy_level === "PLAN_ONLY") {
    return completePlanOnlyJob(job, steps, planSummary);
  }
  ...
  for (const step of steps) {
    ...
    if (step.status === "WAITING" && COMMUNICATION_ACTIONS.has(step.action_type)) {
      ...
      await updateStep(step, {
        ...
        metadata: {
          ...object(step.metadata),
          response_watcher_id: completion.response_watcher?.id || null,
          response_due_at: completion.response_watcher?.response_due_at || null,
        },
    ...
    try {
      ...
      if (outcome.status === "review") {
        const reviewed = await updateStep(step, {
      ...
      if (error instanceof SecretaryJobCancellationInterrupt) throw error;
      ...
      if (exhausted) {
        await emitSecretaryJobTerminalEvent(updatedJob).catch((eventError) =>
          console.error("BUSINESS_PARTNER_SECRETARY_JOB_TERMINAL_EVENT_FAILED", eventError),
        );
      }
      return {
        status: exhausted ? "failed" : "waiting",
  ...
  await assertSecretaryJobActive(job);
  const responseState = await collectSecretaryJobResponses({ job });
  await assertSecretaryJobActive(job);
  let comparison = null;
  if (responseState.responses.length > 0) {
    if (!responseState.terminal) {
      return {
        status: "waiting",
        job: await updateJob(job, {
          status: "WAITING",
          next_action_at: nextDelay(10),
          last_error: null,
          metadata: {
            ...object(job.metadata),
            awaiting_external_responses: true,
            supplier_response_watchers: responseState.responses.length,
            supplier_responses_extracted: responseState.extracted,
            supplier_responses_timed_out: responseState.timed_out,
            purchase_authority_created: false,
          },
        }),
    ...
    comparison = await compareSecretaryJobResponses({ job, responses: responseState.responses });
    await assertSecretaryJobActive(job);
  }

  const summary = await summarizeJob(job, finalSteps, comparison);
  await assertSecretaryJobActive(job);
  const completedJob = await updateJob(job, {
    status: "COMPLETED",
    result_summary: text(summary.summary, 12000) || text(comparison?.recommendation, 12000) || "Secretary job completed",
    next_action_at: null,
    last_error: null,
    completed_at: new Date().toISOString(),
    metadata: {
      ...
      awaiting_external_responses: false,
      supplier_response_count: responseState.responses.length,
      supplier_responses_extracted: responseState.extracted,
      supplier_responses_timed_out: responseState.timed_out,
      supplier_comparison_id: comparison?.id || null,
      supplier_recommendation_advisory_only: Boolean(comparison),
      purchase_authority_created: false,
  ...
  await emitSecretaryJobTerminalEvent(completedJob).catch((error) =>
    console.error("BUSINESS_PARTNER_SECRETARY_JOB_TERMINAL_EVENT_FAILED", error),
  );
  ...
  return { status: "completed", job: completedJob, summary, comparison, response_state: responseState, steps: finalSteps };
}

export async function processSecretaryJob(job) {
  try {
    return await processSecretaryJobInternal(job);
  } catch (error) {
    if (error instanceof SecretaryJobCancellationInterrupt) {
      return cancelledSecretaryJobOutcome(error.job);
    }
    throw error;
  }
}
