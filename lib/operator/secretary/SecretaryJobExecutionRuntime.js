...
const HIGH_AUTHORITY_PATTERN = /\b(purchase|buy|order|pay|payment|contract|sign|agreement|hire|fire|terminate|legal|lawsuit|settle|credential|password|secret|api key|bank transfer|refund|commit funds|price acceptance|accept quote)\b/i;
...
function plannerSystem() {
  return [
    ...
    "Allowed action types: RESEARCH, CALL, MESSAGE, EMAIL, CREATE_TASK, CREATE_EVENT, REVIEW, OTHER.",
    ...
    "If the job requires contacting newly discovered companies that are not yet known contacts, first use RESEARCH to discover them, then use REVIEW explaining that a governed prospect/contact materialization step is required. Do not fabricate a contact.",
    "Any purchase, order, payment, contract, legal commitment, hiring/firing, credential use, irreversible financial action, or acceptance of commercial terms requires REVIEW unless the supplied approval policy explicitly and unambiguously authorizes that exact action and limit.",
    "Researching vendors, requesting quotations, asking for payment terms, collecting offers, comparing suppliers, scheduling, reminders, and ordinary information requests are safe to plan.",
...
async function planJob(job) {
  ...
  const steps = list(parsed.steps).slice(0, 20).map((raw, index) => {
    ...
    const highAuthority = HIGH_AUTHORITY_PATTERN.test(instruction);
    return {
...
async function executeStep(job, step) {
  ...
  if (HIGH_AUTHORITY_PATTERN.test(step.instruction)) {
