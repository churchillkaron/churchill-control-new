...
async function mutateCoordination({ context, payload, instruction, allowedStates, producer }) {
  ...
  const register = registerFromTask(current);
  if (TERMINAL_STATES.has(register.state) && !allowedStates.has(register.state)) {
    throw new Error("SECRETARY_MAIL_COURIER_ALREADY_TERMINAL");
  }
  if (!allowedStates.has(register.state)) throw new Error(`SECRETARY_MAIL_COURIER_STATE_INVALID:${register.state}`);
