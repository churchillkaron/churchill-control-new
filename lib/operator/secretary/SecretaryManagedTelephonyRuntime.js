...
async function recordSettlementPending(connection, status, error) {
  const message = text(error?.message || error, 800) || "UNKNOWN_SETTLEMENT_ERROR";
  const metadata = {
    ...object(connection.metadata),
    wallet_settlement_pending: true,
    wallet_settlement_status: status,
    wallet_settlement_error: message,
    wallet_settlement_updated_at: new Date().toISOString(),
  };
  const result = await supabaseAdmin
    .from("secretary_telephony_connections")
    .update({
      metadata,
      last_error: `WALLET_SETTLEMENT_PENDING:${message}`,
      updated_at: new Date().toISOString(),
    })
...
async function clearSettlementPending(connection) {
  const metadata = { ...object(connection.metadata) };
  delete metadata.wallet_settlement_pending;
  delete metadata.wallet_settlement_status;
  delete metadata.wallet_settlement_error;
  delete metadata.wallet_settlement_updated_at;
  const result = await supabaseAdmin
    .from("secretary_telephony_connections")
    .update({ metadata, last_error: null, updated_at: new Date().toISOString() })
    .eq("organization_id", connection.organization_id)
    .eq("id", connection.id)
    .select("*")
    .single();
...
async function applyOrderState(connection, order) {
  ...
  let storedConnection = result.data;
  let walletSettlementPending = false;
  if (status === "ACTIVE" || status === "FAILED") {
    try {
      await settleWallet(storedConnection, status);
      if (object(storedConnection.metadata).wallet_settlement_pending === true) {
        storedConnection = await clearSettlementPending(storedConnection);
      }
    } catch (error) {
      walletSettlementPending = true;
      storedConnection = await recordSettlementPending(storedConnection, status, error);
    }
  }
  ...
  return {
    ...
    wallet_settlement_pending: walletSettlementPending,
  };
}

async function markPostOrderReconciliationRequired(connection, { orderId = null, error } = {}) {
  const message = text(error?.message || error, 800) || "UNKNOWN_POST_ORDER_ERROR";
  const metadata = {
    ...object(connection.metadata),
    provider_order_accepted: true,
    wallet_reservation_held: true,
  ...
  const result = await supabaseAdmin
    ...
    .update({
      status: "PROVISIONING",
      provider_number_order_id: text(orderId, 200) || connection.provider_number_order_id || null,
      metadata,
      last_error: `POST_ORDER_RECONCILIATION_REQUIRED:${message}`,
      updated_at: new Date().toISOString(),
    })
    .eq("organization_id", connection.organization_id)
    .eq("id", connection.id)
...
export async function requestManagedSecretaryNumber({
  ...
  let walletReserveAttempted = false;
  let providerOrderAccepted = false;
  ...
  try {
    ...
    providerOrderAccepted = true;
    providerOrderId = text(order?.id, 200) || null;
    if (!providerOrderId) throw new Error("SECRETARY_TELEPHONY_PROVIDER_ORDER_ID_MISSING");

    const stored = await supabaseAdmin
      .from("secretary_telephony_connections")
      .update({
        provider_number_order_id: providerOrderId,
        updated_at: new Date().toISOString(),
      })
      .eq("organization_id", organization)
      .eq("id", connection.id)
    ...
    return {
      ...
      wallet_settlement_pending: applied.wallet_settlement_pending,
      carrier_credentials_required_from_customer: false,
      external_secretary_authority_used: false,
    };
  } catch (error) {
    if (providerOrderAccepted) {
      await markPostOrderReconciliationRequired(connection, {
    ...
    if (walletReserveAttempted) {
      await WalletRuntime.release({
        ...
        metadata: { telephony_connection_id: connection.id, reason: "PRE_ORDER_PROVISIONING_EXCEPTION" },
      }).catch(() => null);
    }
    await supabaseAdmin
      .from("secretary_telephony_connections")
...
export async function syncManagedSecretaryNumber({ organizationId, connectionId } = {}) {
  ...
  if (!connection.provider_number_order_id) {
    if (object(connection.metadata).reconciliation_required === true) {
      throw new Error("SECRETARY_TELEPHONY_PROVIDER_ORDER_ID_REQUIRED_FOR_RECONCILIATION");
    }
