...
async function deletePartialSegmentsForChunk(organization, meetingId, chunkNumber) {
  const result = await supabaseAdmin
    .from("secretary_meeting_segments")
    .delete()
    .eq("organization_id", organization)
...
async function claimAudioChunk({ organization, meeting, chunkNumber, startedOffsetMs, mimeType, fileName }) {
  ...
  if (existing.status === "COMPLETED") {
    return { chunk: existing, replay: replayResult(meeting, existing, alreadyCreated) };
  }
  ...
  const expectedSegmentCount = Number(object(existing.metadata).expected_segment_count);
  const hasExpectedSegmentCount = Number.isInteger(expectedSegmentCount) && expectedSegmentCount >= 0;
  if (
    alreadyCreated.length > 0 &&
    hasExpectedSegmentCount &&
    expectedSegmentCount > 0 &&
    alreadyCreated.length === expectedSegmentCount
  ) {
    const recovered = await supabaseAdmin
      ...
      .update({
        ...
        metadata: {
          ...
          segment_write_phase: "RECOVERED_COMPLETE",
    ...
    return {
      chunk: { ...existing, status: "COMPLETED", segment_ids: alreadyCreated.map((segment) => segment.id) },
      replay: replayResult(meeting, existing, alreadyCreated),
    };
  }

  if (alreadyCreated.length > 0) {
    await deletePartialSegmentsForChunk(organization, meeting.id, chunkNumber);
  }
    ...
    supabaseAdmin
      ...
      .update({
        ...
        metadata: {
          ...object(existing.metadata),
          expected_segment_count: null,
          segment_write_phase: "RETRYING",
          partial_segments_cleared: alreadyCreated.length,
          raw_audio_persisted: false,
          external_authority_used: false,
        },
        updated_at: new Date().toISOString(),
...
async function markAudioChunkSegmentPlan(chunkId, expectedSegmentCount, detectedLanguage) {
  ...
  const result = await supabaseAdmin
    ...
    .update({
      ...
      metadata: {
        ...
        segment_write_phase: "WRITING_SEGMENTS",
        raw_audio_persisted: false,
        external_authority_used: false,
      },
      updated_at: new Date().toISOString(),
...
async function failAudioChunk(chunkId, error) {
  ...
  try {
    ...
    const result = await supabaseAdmin
      ...
      .eq("id", chunkId);
    if (result.error) {
      console.error("SECRETARY_MEETING_AUDIO_CHUNK_FAILURE_STATE_ERROR", result.error.message || result.error);
    }
  } catch (cleanupError) {
    console.error("SECRETARY_MEETING_AUDIO_CHUNK_FAILURE_CLEANUP_FAILED", cleanupError?.message || cleanupError);
  }
}
...
export async function ingestSecretaryMeetingAudio({
  ...
  try {
    ...
    await markAudioChunkSegmentPlan(claim.chunk.id, normalized.length, detectedLanguage);
