...
export async function createSecretaryMeetingCoordination({ context, payload = {} } = {}) {
  ...
  const result = await secretaryPreboundMutationResult(supabaseAdmin.rpc("secretary_create_meeting_coordination", {
  ...
  return {
    status: "collecting",
    coordination_id: result.data.id,
...
export async function cancelSecretaryMeetingCoordination({ context, payload = {} } = {}) {
  ...
  if (!id) throw new Error("SECRETARY_MEETING_COORDINATION_ID_REQUIRED");
  if (!evidenceId) throw new Error("SECRETARY_MEETING_COORDINATION_EVIDENCE_REQUIRED");
  const result = await secretaryRecoveryMutationResult(
    supabaseAdmin.rpc("secretary_cancel_meeting_coordination", {
      p_organization_id: organization,
      p_coordination_id: id,
      p_cancelled_by_party_id: actor,
      p_evidence_id: evidenceId,
    }),
    [["coordination_id", id], ["transition_kind", "CANCEL_COORDINATION"], ["evidence_id", evidenceId]],
  );
  return { status: "cancelled", coordination: result.data, coordination_id: id, transition_kind: "CANCEL_COORDINATION", evidence_id: evidenceId, calendar_event_cancelled: false, external_authority_used: false };
}
