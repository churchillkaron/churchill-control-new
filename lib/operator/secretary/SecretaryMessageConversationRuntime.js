...
async function ensureContactParty({ request, conversation, inbound }) {
  ...
  const alias = await one(
    supabaseAdmin
      ...
      .eq("provider", text(conversation.provider, 120).toLowerCase())
      .eq("channel_type", text(conversation.channel_type || conversation.provider, 120).toLowerCase())
      .eq("external_participant_id", text(conversation.external_participant_id, 500))
      .maybeSingle(),
  );
  if (alias?.party_id) return alias.party_id;

  const address = text(conversation.external_participant_address || inbound.sender_address || conversation.external_participant_id, 500);
  let party = null;
  if (address.includes("@")) {
    party = await one(
      supabaseAdmin
  ...
  if (!party) {
    const inserted = await supabaseAdmin
      ...
      .insert({
        ...
        party_type: "person",
        display_name: text(conversation.external_participant_name, 500) || address || "External contact",
        email: address.includes("@") ? address.toLowerCase() : null,
        phone: address && !address.includes("@") ? address : null,
        status: "active",
  ...
  try {
    const channel = await supabaseAdmin
      ...
      .insert({
        organization_id: request.organization_id,
        party_id: party.id,
        provider: text(conversation.provider, 120).toLowerCase(),
        channel_type: text(conversation.channel_type || conversation.provider, 120).toLowerCase(),
        external_participant_id: text(conversation.external_participant_id, 500),
        external_address: address || null,
        display_name: text(conversation.external_participant_name, 500) || party.display_name || null,
        last_inbound_at: inbound.received_at || inbound.created_at || new Date().toISOString(),
        metadata: { source: "secretary_message_reception" },
      })
      .select("id")
      .single();
    if (channel.error && channel.error.code !== "23505") throw channel.error;
  } catch (error) {
    if (created) {
      const cleanup = await supabaseAdmin
        ...
        .eq("organization_id", request.organization_id)
        .eq("id", party.id);
      if (cleanup.error) console.error("SECRETARY_MESSAGE_PARTY_ROLLBACK_FAILED", cleanup.error.message || cleanup.error);
    }
    throw error;
  }
  ...
  await Promise.all([
    supabaseAdmin
      .from("secretary_message_reception_requests")
      .update({ contact_party_id: party.id, updated_at: new Date().toISOString() })
      .eq("id", request.id),
    supabaseAdmin
      .from("communication_conversations")
      .update({ customer_party_id: party.id, updated_at: new Date().toISOString() })
      .eq("organization_id", request.organization_id)
