...
import { loadSecretaryBusinessBookingContext, executeSecretaryBusinessAgreement } from "@/lib/operator/secretary/SecretaryBusinessAgreementExecutionRuntime";
import { createCustomerPortalAccessLink, reissueCustomerPortalAccess } from "@/lib/customer-portal/CustomerPortalRuntime";
import { deliverCustomerPortalInviteIfEligible } from "@/lib/customer-portal/CustomerPortalInviteDeliveryRuntime";
...
function externalPublicContext(settings, ownAppointments, businessBookingContext = {}) {
  return {
    ...
    own_appointments: Array.isArray(ownAppointments?.appointments)
      ...
      : [],
    business_booking_context: object(businessBookingContext),
  };
}
...
function externalActionResult(value) {
  ...
  if (result.appointment) {
    ...
    const businessExecution = object(result.business_execution);
    const executionResult = object(businessExecution.result);
    return {
      status: "completed",
      appointment: publicAppointment(appointment),
      customer_portal_url: text(executionResult?.portal_access?.url || result?.portal_access?.url, 2000) || null,
      payment_request_id: text(executionResult?.payment_request_id, 120) || null,
    };
  }
...
function decisionSystem() {
  return [
    ...
    "If the sender explicitly asks for their customer portal link, login link, portal access, or says they lost/deleted the portal link, use SEND_PORTAL_LINK. This action is only for the already-resolved sender Party and the server will independently verify a real customer relationship before issuing anything.",
    ...
    "When a BOOK_APPOINTMENT is clearly for a supported business service in public_context.business_booking_context, also classify business_booking_type. Use PEST_CONTROL_SERVICE only when the sender is explicitly agreeing to a pest-control service booking; otherwise return null.",
    "For PEST_CONTROL_SERVICE, resolve customer_name, customer_phone and customer_address from the sender's explicit messages or public_context.business_booking_context.known_customer_identity. Never invent or silently guess them.",
    "If any of customer_name, customer_phone, customer_address/site, exact date/time, service_name, or an unambiguous active service_protocol is still missing, use CLARIFY and ask only for the missing fact(s). Do not BOOK_APPOINTMENT yet.",
    "For PEST_CONTROL_SERVICE, business_agreement_explicit must be true only when the sender explicitly accepts/confirms the exact booking after the material booking facts are resolved. service_name must be grounded in the conversation, service_protocol must exactly match one supplied active protocol code or name when unambiguous, and location must be the explicit or uniquely resolved customer site. billing_amount and billing_currency_code may be populated only when an exact price and currency are explicitly evidenced in public_context or the conversation; otherwise both must be null. Never invent a protocol, service, site, identity, phone, address, price, currency, or agreement.",
    "Preserve the sender's language. Do not switch languages unless requested.",
    "Return exactly one JSON object with keys: action, response_language, response_text, starts_at, ends_at, appointment_reference, appointment_title, appointment_description, location, callback_reason, callback_due_at, message_text, business_booking_type, business_agreement_explicit, service_name, service_protocol, duration_minutes, customer_name, customer_phone, customer_address, billing_amount, billing_currency_code.",
...
function finalResponseSystem() {
  return [
    ...
    "For booking/rescheduling/cancellation, state only returned appointment time/status or cancellation status. If action_result includes customer_portal_url, include that exact secure portal link and say the customer can follow the booking there; if payment_request_id is present, say card payment is available in the portal.",
    "For unavailable times never explain why.",
    "For recorded callback/message requests confirm only that the request was recorded.",
    "For SEND_PORTAL_LINK, say only that a fresh secure portal link was sent in this conversation. Never include the raw portal URL in response_text because secure delivery is handled separately.",
...
export async function runSecretaryMessageReceptionRequest(request) {
  ...
  const [conversationHistory, ownAppointments, businessBookingContext] = await Promise.all([
    recentConversation(
  ...
  const modelPublicInfo = externalPublicContext(settings, ownAppointments, businessBookingContext);
  ...
  try {
    switch (decision.action) {
      ...
      case "BOOK_APPOINTMENT": {
        ...
        const businessExecution = await executeSecretaryBusinessAgreement({
          organizationId: contextRows.request.organization_id,
          contactPartyId,
          conversationId: contextRows.request.conversation_id,
          requestId: contextRows.request.id,
          decision,
          appointment: actionResult?.appointment || null,
          bookingPolicy: settings.booking_policy,
        });
        const portalAccess = businessExecution?.result?.portal_access || (actionResult?.appointment?.id
          ? await createCustomerPortalAccessLink({
              organizationId: contextRows.request.organization_id,
              partyId: contactPartyId,
              sourceType: "SECRETARY_BOOKING",
              sourceId: actionResult.appointment.id,
            })
          : null);
        actionResult = { ...actionResult, business_execution: businessExecution, portal_access: portalAccess };
        break;
      }
      ...
      case "SEND_PORTAL_LINK": {
        const access = await reissueCustomerPortalAccess({
          organizationId: contextRows.request.organization_id,
          partyId: contactPartyId,
        });
        const delivery = await deliverCustomerPortalInviteIfEligible({
          organizationId: contextRows.request.organization_id,
          partyId: contactPartyId,
          access,
          sourceType: access.source_type || "PORTAL_REISSUE",
          sourceId: access.source_id || contextRows.request.conversation_id,
          ignoreExistingMessage: true,
          conversationId: contextRows.request.conversation_id,
        });
        if (!delivery?.delivered) {
          throw new Error(delivery?.reason || "CUSTOMER_PORTAL_REISSUE_DELIVERY_FAILED");
        }
        actionResult = {
          status: "completed",
          portal_access_sent: true,
          access_link_id: access.access_link_id || null,
  ...
  let responseText = await finalResponse({
    organizationId: contextRows.request.organization_id,
    contactPartyId,
    senderMessage,
    decision,
  ...
  const portalUrl = text(actionResult?.portal_access?.url, 2000);
  if (!actionError && decision.action === "BOOK_APPOINTMENT" && portalUrl && !String(responseText || "").includes(portalUrl)) {
    const portalLine = `Customer portal: ${portalUrl}`;
