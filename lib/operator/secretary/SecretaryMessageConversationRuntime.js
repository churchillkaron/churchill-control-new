...
function externalPublicContext(settings) {
  return {
  ...
  if (result.appointment) {
    ...
    return {
      ...
      appointment: {
        ...
        event_type: text(appointment.event_type, 80) || "APPOINTMENT",
      },
    };
  }
...
export async function runSecretaryMessageReceptionRequest(request) {
  ...
  const conversationHistory = await recentConversation(
    contextRows.request.organization_id,
    contextRows.request.conversation_id,
  );
  const modelPublicInfo = externalPublicContext(settings);
