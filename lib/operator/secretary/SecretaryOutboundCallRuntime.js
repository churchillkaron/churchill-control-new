...
async function resolveContact(organizationId, contactPartyId, remoteAddress) {
  ...
  if (partyId) {
    ...
    const profile = await one(
      supabaseAdmin
        ...
        .select("allow_calls,preferred_language,do_not_disturb")
        .eq("organization_id", organizationId)
  ...
  return { party_id: null, remote_address: address, preferred_language: null, do_not_disturb: {} };
}
...
export async function queueSecretaryOutboundCall({ context, payload = {} } = {}) {
  ...
  return { status: "queued", outbound_call_request: request, external_authority_used: false };
}
