...
import { endSecretaryCall } from "./SecretaryCallerRuntime";
...
async function currentOutboundRequest(id) {
  const result = await supabaseAdmin
    .from("secretary_outbound_call_requests")
    .select("id,status,call_id,attempt_count,max_attempts,lease_expires_at,claim_token,metadata,last_error")
    .eq("id", id)
    .maybeSingle();
  if (result.error) throw result.error;
  return result.data || null;
...
async function activeClaim(request) {
  const current = await currentOutboundRequest(request.id);
  const currentStatus = text(current?.status, 40).toUpperCase();
  if (!current || currentStatus !== "CLAIMED" || text(current.claim_token, 120) !== text(request.claim_token, 120)) {
    return { active: false, current };
  }
...
export async function dispatchSecretarySipOutboundCall(request) {
  ...
  const claim = await activeClaim(request);
  if (!claim.active) {
    return {
      status: "cancelled_or_stale",
      request: claim.current,
      gateway_acknowledged: false,
      dispatch_performed: false,
      stale_claim_fenced: true,
    };
  }
  ...
  try {
    const preDispatchClaim = await activeClaim(request);
    if (!preDispatchClaim.active) {
      return {
        status: "cancelled_or_stale",
        request: preDispatchClaim.current,
        gateway_acknowledged: false,
        dispatch_performed: false,
        stale_claim_fenced: true,
      };
    ...
    const update = await supabaseAdmin
      ...
      .eq("claim_token", request.claim_token)
      .eq("status", "CLAIMED")
      .select("id,status,call_id,attempt_count,max_attempts,lease_expires_at,metadata")
      .maybeSingle();
    if (update.error) throw update.error;
    if (update.data) {
      return { status: "dialing", request: update.data, gateway_acknowledged: true, dispatch_performed: true };
    }

    const current = await currentOutboundRequest(request.id);
    if (current && ["DIALING", "CONNECTED", "COMPLETED"].includes(text(current.status, 40).toUpperCase())) {
      return {
        status: text(current.status, 40).toLowerCase(),
        request: current,
        gateway_acknowledged: true,
        dispatch_performed: true,
        fast_status_callback_preserved: true,
      };
    }
    throw new Error("SECRETARY_SIP_DISPATCH_CLAIM_LOST");
  } catch (error) {
    const current = await currentOutboundRequest(request.id).catch(() => null);
    if (current && ["DIALING", "CONNECTED", "COMPLETED"].includes(text(current.status, 40).toUpperCase())) {
      return {
        status: text(current.status, 40).toLowerCase(),
        request: current,
    ...
    if (current && text(current.status, 40).toUpperCase() === "CANCELLED") {
      return {
        status: "cancelled_or_stale",
        request: current,
        gateway_acknowledged: false,
...
export async function reconcileStaleSecretarySipCalls({ dialingMinutes = 10, connectedMinutes = 180, limit = 100 } = {}) {
  const now = new Date();
  const dialingCutoff = new Date(now.getTime() - Math.max(2, Number(dialingMinutes) || 10) * 60_000).toISOString();
  const connectedCutoff = new Date(now.getTime() - Math.max(30, Number(connectedMinutes) || 180) * 60_000).toISOString();
  const [dialingResult, connectedResult] = await Promise.all([
    supabaseAdmin.from("secretary_outbound_call_requests").select("*").eq("status", "DIALING").lt("updated_at", dialingCutoff).order("updated_at", { ascending: true }).limit(Math.max(1, Math.min(Number(limit) || 100, 200))),
    supabaseAdmin.from("secretary_outbound_call_requests").select("*").eq("status", "CONNECTED").lt("updated_at", connectedCutoff).order("updated_at", { ascending: true }).limit(Math.max(1, Math.min(Number(limit) || 100, 200))),
  ]);
  if (dialingResult.error) throw dialingResult.error;
  if (connectedResult.error) throw connectedResult.error;
  const rows = [...(dialingResult.data || []), ...(connectedResult.data || [])];
  const reconciled = [];

  for (const row of rows) {
    const reason = row.status === "DIALING" ? "SIP_GATEWAY_DIAL_TIMEOUT" : "SIP_GATEWAY_CONNECTED_TIMEOUT";
    if (row.call_id) {
      try {
        await endSecretaryCall({ callId: row.call_id, status: "FAILED", summary: reason });
      } catch {
        // Request reconciliation remains authoritative even when the call row already reached a terminal state.
      }
    }
    const update = await supabaseAdmin
      .from("secretary_outbound_call_requests")
      .update({
        ...
        metadata: {
          ...
          transport_timeout_reason: reason,
          transport_timeout_at: now.toISOString(),
        },
        updated_at: now.toISOString(),
      })
      .eq("id", row.id)
      .eq("status", row.status)
      .select("id,status,call_id,last_error")
      .maybeSingle();
    if (update.error) throw update.error;
    if (update.data) reconciled.push(update.data);
  }

  return { status: "completed", reconciled: reconciled.length, requests: reconciled };
}
