...
import { secretaryRecoveryMutationResult } from "@/lib/operator/secretary/SecretaryPreboundMutationRuntime.mjs";
...
async function mutateLedger({ context, payload = {}, eventName, instruction, producer }) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const updated = await secretaryRecoveryMutationResult(
      supabaseAdmin.from("secretary_jobs")
        ...
        .select("*")
        .maybeSingle(),
      [["job_id", job.id], ["event", eventName], ["evidence_id", evidenceId], ["payload_sha256", payloadSha]],
    );
    if (updated.error) throw updated.error;
    if (updated.data) return {
      job: updated.data, ledger: nextLedger, auth, replaySafe: false, output: object(produced.output),
      job_id: job.id, event: eventName, evidence_id: evidenceId, payload_sha256: payloadSha,
    };
  }
...
export async function startSecretaryTravelDocumentReadiness({ context, payload = {} } = {}) {
  ...
  const updated = await secretaryRecoveryMutationResult(
    supabaseAdmin.from("secretary_jobs")
      ...
      .select("*")
      .maybeSingle(),
    [["job_id", job.id], ["event", "TRAVEL_DOCUMENT_READINESS_STARTED"], ["evidence_id", evidenceId], ["payload_sha256", payloadSha]],
  );
  if (updated.error) throw updated.error;
  if (!updated.data) throw new Error("SECRETARY_TRAVEL_DOCUMENT_CONCURRENT_UPDATE_RETRY_REQUIRED");
  return {
    status: "started", contract: CONTRACT, job: updated.data, register: ledger, replay_safe: false,
    job_id: job.id, event: "TRAVEL_DOCUMENT_READINESS_STARTED", evidence_id: evidenceId, payload_sha256: payloadSha,
    ...secretaryAdministrativeCoverageMetadata(auth.routing), ...safetyFlags(),
  };
}
...
export async function addSecretaryTravelDocumentRequirement({ context, payload = {} } = {}) {
  ...
  return {
    status: "added", contract: CONTRACT, job: result.job, register: result.ledger, item: result.output.item || null, replay_safe: result.replaySafe,
    job_id: result.job_id || result.job?.id, event: result.event || "TRAVEL_DOCUMENT_REQUIREMENT_ADDED",
    evidence_id: result.evidence_id || text(payload.evidence_id || payload.evidenceId, 500), payload_sha256: result.payload_sha256 || sha256(payload),
    ...secretaryAdministrativeCoverageMetadata(result.auth.routing), ...safetyFlags(),
  };
}
...
export async function reopenSecretaryTravelDocumentReadiness({ context, payload = {} } = {}) {
  ...
  return {
    status: "reopened", contract: CONTRACT, job: result.job, register: result.ledger, replay_safe: result.replaySafe,
    job_id: result.job_id || result.job?.id, event: result.event || "TRAVEL_DOCUMENT_READINESS_REOPENED",
    evidence_id: result.evidence_id || text(payload.evidence_id || payload.evidenceId, 500), payload_sha256: result.payload_sha256 || sha256(payload),
    ...secretaryAdministrativeCoverageMetadata(result.auth.routing), ...safetyFlags(),
  };
}
