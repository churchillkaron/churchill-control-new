...
export async function readSecretaryTravelOperations({ context, payload = {} } = {}) {
  ...
  const confirmations = ledger.confirmations
  ...
  const supersededConfirmations = ledger.confirmations.filter((row) => row.status === "SUPERSEDED");
  ...
  return {
    ...
    superseded_confirmations: supersededConfirmations,
    confirmation_history: ledger.history,
    ...
    evidence_summary: {
      ...
      superseded_items: supersededConfirmations.length,
