...
export async function startSecretaryVisitorCoordination({ context, payload = {} } = {}) {
  ...
  if (!task) {
    ...
    const inserted = await secretaryRecoveryMutationResult(supabaseAdmin.from("secretary_tasks").insert({
      id: taskId,
      ...
      metadata,
    }).select("*").single(), [["calendar_event_id", event.id], ["visitor_party_id", visitorId]]);
    if (inserted.error) {
