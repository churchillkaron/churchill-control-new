...
function findValue(value, keys, depth = 0) {
  ...
  for (const key of ["output", "result", "data", "response", "raw", "provider_result", "provider_status_input"]) {
...
async function settleVoiceExecution({ execution, call, capability }) {
...
async function transcribe({ call, audio, mimeType, fileName, language, durationSeconds }) {
...
async function synthesize({ call, responseText, detectedLanguage }) {
  ...
  const estimatedOutputSeconds = Number(Math.max(0.1, Math.min(600, words / 2.5)).toFixed(6));
  const submitted = await ServiceExecutionRuntime.execute({
    organization_id: call.organization_id,
    party_id: call.contact_party_id || null,
    ...
    metadata: {
      ...
      voice_billing_quantity_source: "TEXT_150_WPM_OUTPUT_SECONDS_ESTIMATE",
      voice_billing_contract: "AVANTIQO_VOICE_PER_SECOND_BILLING_V1",
  ...
  const audioBase64 = await storedVoiceAudioBase64(execution);
...
export async function runSecretaryVoiceCallChunk({
  ...
  const stt = await transcribe({ call, audio, mimeType, fileName, language, durationSeconds });
