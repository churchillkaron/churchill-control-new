...
async function mutateRegister({ context, instruction, producer }) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const recoveryEntry = object(produced.output?.preference || produced.output?.history_entry);
    const recoveryEvidence = recoveryEntry.entry_id && recoveryEntry.domain && recoveryEntry.key
      ? [["entry_id", recoveryEntry.entry_id], ["domain", recoveryEntry.domain], ["key", recoveryEntry.key]]
      : [];
