...
export async function startRealtimeTranscription({
  ...
  // Cancellation is an expected lifecycle event when Avantiqo returns to
  // passive wake. Mark the promise handled immediately while keeping the
  // original promise rejectable for callers that are actively committing.
  void transcriptPromise.catch(() => null);
  ...
  function startAudioCapture() {
    ...
    if (audioCaptureStarted) return true;
  ...
  function publishTranscript({ final = false, delta = "" } = {}) {
    if (typeof onTranscript !== "function") return;
    try {
      onTranscript({
        transcript: text(finalTranscript),
        delta: text(delta),
        final,
      });
    } catch {}
  ...
  function handleServerEvent(event) {
    ...
    if (event.type === "conversation.item.input_audio_transcription.delta") {
      if (typeof event.delta === "string") {
        finalTranscript += event.delta;
        publishTranscript({ final: false, delta: event.delta });
      }
      return;
    }
  ...
  try {
    ...
    if (!deferAudioCapture) startAudioCapture();
  } catch (error) {
