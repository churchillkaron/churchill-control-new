...
const DEFAULT_THRESHOLD = 0.115;
const MIN_DURATION_RATIO = 0.78;
const MAX_DURATION_RATIO = 1.28;
...
function median(values = []) {
  const clean = values.map((value) => finite(value)).filter((value) => value > 0).sort((a, b) => a - b);
...
export function averageWakeTemplates(samples = []) {
  ...
  if (durations.length >= 3) {
    ...
    if (durationMs < 450 || durationMs > 2600 || minDuration / maxDuration < 0.62) {
      return null;
    }
  }
  ...
  const frames = Array.from({ length: FRAME_POINTS }, (_, frameIndex) =>
    Array.from({ length: FEATURE_SIZE }, (_, featureIndex) => {
      const sum = prepared.reduce(
        (total, sample) => total + finite(sample.frames[frameIndex]?.[featureIndex]),
        0,
      );
...
export function scoreWakeCandidate(candidateFrames = [], template, candidateDurationMs = null) {
  ...
  if (!candidate.length || !reference.length || Number(template?.version) !== 2) {
  ...
  const threshold = Math.min(finite(template?.threshold, DEFAULT_THRESHOLD), DEFAULT_THRESHOLD);
