...
async function resolvePayrollCurrency({
  organizationId,
  entityId,
  staffId,
  payrollMonth,
  ...
  if (!currency) {
    throw new Error("Payroll compensation currency is not configured");
  }
...
export default async function postPayrollAccrual({
  ...
  if (!record) throw new Error("Payroll record not found for legal entity");
  ...
  const currencyCode = await resolvePayrollCurrency({
    organizationId,
    entityId,
  ...
  results.push(
    await postComponent({
      ...
      record,
      currencyCode,
      exchangeRate: exchangeRateContext.exchange_rate,
      postingDate,
  ...
  return {
    ...
    postingDate,
    currencyCode,
    functionalCurrency: exchangeRateContext.functional_currency,
    exchangeRate: exchangeRateContext.exchange_rate,
    exchangeRateSource: exchangeRateContext.source,
    gross,
