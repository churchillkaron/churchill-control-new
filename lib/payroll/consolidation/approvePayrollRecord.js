...
export async function approvePayrollRecord({
  ...
  const pendingReview = monthRecords.find(
    (item) => item.review_required === true && item.review_status === "PENDING"
  );

  if (pendingReview) {
    throw new Error(
      `Manager review required before month approval: ${pendingReview.staff_name || "Employee"}`
    );
  }
