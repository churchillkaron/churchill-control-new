...
export default async function disputePayrollRecord({
  ...
  if (PRE_APPROVAL_STATUSES.has(record.status)) {
    ...
    const { data: updated, error: updateError } = await supabaseAdmin
      ...
      .or("employee_acknowledged.is.null,employee_acknowledged.eq.false")
      .select("id,status")
      .maybeSingle();

    if (updateError) throw updateError;
    if (!updated) {
      const conflict = new Error("Payroll changed before dispute was submitted; reload and retry");
      conflict.status = 409;
      conflict.code = "PAYROLL_DISPUTE_CONFLICT";
      throw conflict;
    }
