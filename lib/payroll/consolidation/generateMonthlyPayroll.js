...
function approvedTimeOffForDate(rows, shiftDate) {
  return (
    (rows || []).find(
      (row) => row?.status === "APPROVED" && row?.start_date <= shiftDate && row?.end_date >= shiftDate
    ) || null
  );
}
...
function calculateLatenessPenaltyProposal({
  ...
  const deductibleMinutes = lateShifts.reduce(
    (sum, shift) => sum + Math.max(0, Number(shift?.late_minutes || 0) - lateThresholdMinutes),
    0
  );
  if (deductibleMinutes <= 0) return { deductibleMinutes: 0, hourlyRate: 0, amount: 0 };
  const configuredHourlyRate = Number(profile?.hourly_rate || 0);
  const monthlySalary = Number(profile?.monthly_salary || 0);
  const derivedHourlyRate = configuredHourlyRate > 0
...
export async function calculateMonthlyPayroll({ organizationId, entityId, payrollMonth }) {
  ...
  const [payrollSettings, workforceSettings, timeContext, calendarRows] = await Promise.all([
  ...
  const [staffResult, compensationResult, shiftsResult, attendanceResult, schedulesResult, ordersResult] = await Promise.all([
    supabaseAdmin.from("staff_accounts").select("*").eq("active_organization_id", organizationId).eq("active", true),
    supabaseAdmin.from("employee_compensation_profiles").select("*").eq("organization_id", organizationId).eq("entity_id", entityId).lte("effective_from", monthEnd(payrollMonth)).or(`effective_to.is.null,effective_to.gte.${range.start}`),
    supabaseAdmin.from("staff_shifts").select("*").eq("organization_id", organizationId).gte("clock_in", rangeStart.toISOString()).lt("clock_in", rangeEnd.toISOString()),
    supabaseAdmin.from("staff_attendance").select("*").eq("organization_id", organizationId).gte("shift_date", range.start).lt("shift_date", range.end),
    supabaseAdmin.from("staff_schedules").select("*").eq("organization_id", organizationId).eq("status", "PUBLISHED").gte("shift_date", range.start).lt("shift_date", range.end),
    supabaseAdmin.from("orders").select("service_charge,service_charge_amount").eq("organization_id", organizationId).eq("payment_status", "PAID").gte("completed_at", rangeStart.toISOString()).lt("completed_at", rangeEnd.toISOString()),
  ]);
  for (const result of [staffResult, compensationResult, shiftsResult, attendanceResult, schedulesResult, ordersResult]) {
  ...
  const staff = (staffResult.data || []).filter((member) => compensationByStaff.has(member.id));
  ...
  const effectiveRawShifts = applyEffectiveShiftCorrections({ shifts: rawShifts, corrections });
  const excludedShiftIds = new Set(
    effectiveRawShifts.filter((shift) => !payrollEligibleShift(shift)).map((shift) => shift.id).filter(Boolean)
  );
  ...
  const totalServiceCharge = (ordersResult.data || []).reduce(
    (sum, order) => sum + Number(order.service_charge_amount ?? order.service_charge ?? 0),
    0
  );
  ...
  const staffPerformance = staff.map((member) => {
    ...
    const expectedHours = payrollSettings?.use_schedule_expected_hours && plannedHours > 0
      ? plannedHours
    ...
    const overtimeHours = Number(
      (memberShifts.reduce((sum, shift) => sum + Number(shift.overtime_minutes || 0), 0) / 60).toFixed(2)
    );
    const workedHours = Number(totalHours.toFixed(2));
    const normalizedExpectedHours = Number(expectedHours.toFixed(2));
    const approvedHours = Number((workedHours + attendanceReconciliation.creditedHours).toFixed(2));
    const varianceHours = Number((approvedHours - normalizedExpectedHours).toFixed(2));
    const hasPayrollExposure = Number(profile?.monthly_salary || 0) > 0 || Number(profile?.hourly_rate || 0) > 0;
    const hoursVarianceReviewRequired = hasPayrollExposure && Boolean(payrollSettings?.manager_approval_required) && Math.abs(varianceHours) > Number(payrollSettings?.variance_threshold_hours || 0);
    const unresolvedAttendanceReviewRequired = attendanceReconciliation.unresolvedSchedules > 0;
    ...
    const reviewRequired = unresolvedAttendanceReviewRequired || hoursVarianceReviewRequired || latenessReviewRequired;
    const reviewReasons = [];
    if (unresolvedAttendanceReviewRequired) {
      reviewReasons.push(`${attendanceReconciliation.unresolvedSchedules} expired published schedule${attendanceReconciliation.unresolvedSchedules === 1 ? " has" : "s have"} no worked or approved attendance outcome`);
    }
    if (hoursVarianceReviewRequired) reviewReasons.push("Hours variance requires manager review");
    if (latenessReviewRequired) {
      reviewReasons.push(`Proposed lateness deduction ${latenessPenalty.amount.toFixed(2)} for ${latenessPenalty.deductibleMinutes} minute${latenessPenalty.deductibleMinutes === 1 ? "" : "s"} above the configured grace threshold`);
    }
    const attendanceScore = calculateAttendanceScore({ lateMinutes: finalLateMinutes, overtimeHours });
    ...
    return {
      ...
      publicHolidayScheduleCount: attendanceReconciliation.publicHolidayScheduleIds.length,
  ...
  const payrollData = payoutResult.map((member) => ({ ...member, serviceChargeBonus: Number(member.payout || 0) }));
  const records = await buildPayrollRecords({ organizationId, entityId, payrollMonth, payrollData });
  ...
  return {
    ...
    approvedTimeOffCount: approvedTimeOffEntries.reduce((sum, [, rows]) => sum + rows.length, 0),
    publicHolidayCount: publicHolidayByDate.size,
    records,
  };
}

export default async function generateMonthlyPayroll({ organizationId, entityId, payrollMonth }) {
  const calculation = await calculateMonthlyPayroll({ organizationId, entityId, payrollMonth });
  ...
  const lockedPayroll = (existingPayroll || []).filter((record) => !replaceableStatuses.has(record.status));
  if (lockedPayroll.length > 0) throw new Error("Payroll already approved or locked for this month");
