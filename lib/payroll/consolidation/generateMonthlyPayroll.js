...
function fallbackExpectedHours(payrollMonth, settings) {
  ...
  const workingDaysPerWeek = Number(
    settings?.default_working_days_per_week || 0
  );
  const hoursPerShift = Number(
    settings?.default_hours_per_shift || 0
  );
...
function countMissedSchedules({ schedules, shifts, timezone }) {
  ...
  for (const shift of completedShifts) {
    ...
    legacyWorkedByDate.set(
      date,
      (legacyWorkedByDate.get(date) || 0) + 1
    );
  }
  ...
  for (const schedule of schedules) {
    if (directlyWorkedScheduleIds.has(schedule.id)) {
      continue;
    }

    const availableLegacyShift =
      legacyWorkedByDate.get(schedule.shift_date) || 0;

    if (availableLegacyShift > 0) {
      legacyWorkedByDate.set(
        schedule.shift_date,
        availableLegacyShift - 1
      );
      continue;
    }
...
function calculateLatenessPenaltyProposal({
  ...
  const deductibleMinutes = lateShifts.reduce(
    (sum, shift) =>
      ...
      Math.max(
        0,
        Number(shift?.late_minutes || 0) - lateThresholdMinutes
      ),
    0
  );
...
export async function calculateMonthlyPayroll({
  ...
  const latenessDeductionEnabled =
    payrollSettings?.lateness_deduction_enabled === true;
  ...
  const compensationByStaff = new Map(
    (compensationResult.data || []).map((profile) => [
  ...
  const staffPerformance = staff.map((member) => {
    ...
    const plannedHours = memberSchedules.reduce(
      (sum, schedule) =>
        sum + scheduleHours(schedule, timeContext.timezone),
      0
    );
    ...
    const totalHours = memberShifts.reduce(
      (sum, shift) =>
        sum + hoursBetween(shift.clock_in, shift.clock_out),
      0
    );
    ...
    const missedShifts = countMissedSchedules({
      schedules: memberSchedules,
      shifts: memberShifts,
      timezone: timeContext.timezone,
    });

    const classifiedLateShifts = memberShifts.filter(
      (shift) => shift.is_late === true
    );
    const classifiedLateShiftIds = new Set(
      classifiedLateShifts.map((shift) => shift.id).filter(Boolean)
    );
    ...
    const attendanceLateMinutes = memberAttendance.reduce(
      (sum, row) =>
        row.shift_id && classifiedLateShiftIds.has(row.shift_id)
          ? sum + Number(row.late_minutes || 0)
          : sum,
      0
    );
    ...
    const normalizedExpectedHours = Number(
      expectedHours.toFixed(2)
    );
    const varianceHours = Number(
      (workedHours - normalizedExpectedHours).toFixed(2)
    );
    ...
    const hoursVarianceReviewRequired =
      ...
      Math.abs(varianceHours) >
        Number(
          payrollSettings?.variance_threshold_hours || 0
        );
    ...
    const reviewRequired =
      hoursVarianceReviewRequired || latenessReviewRequired;
    const reviewReasons = [];
    ...
    if (latenessReviewRequired) {
      reviewReasons.push(
        `Proposed lateness deduction ${latenessPenalty.amount.toFixed(2)} for ${latenessPenalty.deductibleMinutes} minute${latenessPenalty.deductibleMinutes === 1 ? "" : "s"} above the configured grace threshold`
      );
    }
...
export default async function generateMonthlyPayroll({
  ...
  const existingIds = (existingPayroll || []).map(
    (record) => record.id
  );
