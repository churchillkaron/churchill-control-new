...
function reconcileScheduledAttendance({
  ...
  for (const schedule of schedules) {
    ...
    if (scheduleExpired(schedule, timezone, now)) {
...
export async function calculateMonthlyPayroll({
  ...
  const staffPerformance = staff.map((member) => {
    ...
    const varianceHours = Number(
      (approvedHours - normalizedExpectedHours).toFixed(2)
    );
    ...
    if (unresolvedAttendanceReviewRequired) {
      reviewReasons.push(
        `${attendanceReconciliation.unresolvedSchedules} expired published schedule${attendanceReconciliation.unresolvedSchedules === 1 ? " has" : "s have"} no worked or manager-classified attendance outcome`
      );
    }
