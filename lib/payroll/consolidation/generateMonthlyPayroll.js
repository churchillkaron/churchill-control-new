...
export async function calculateMonthlyPayroll({
  ...
  const staffPerformance = staff.map((member) => {
    ...
    const lateCount = memberShifts.filter(
      (shift) => shift.is_late
    ).length;

    const shiftLateMinutes = memberShifts.reduce(
      (sum, shift) =>
        sum + Number(shift.late_minutes || 0),
      0
    );
    ...
    return {
      ...
      reviewReason: reviewRequired
        ? "Hours variance requires manager review"
        : null,
      overtimeHours,
      attendanceScore,
      completedShifts,
...
export default async function generateMonthlyPayroll({
  ...
  const records = await generatePayrollRecords({
    ...
    entityId,
    payrollMonth,
    payrollData: calculation.records.map((record) => ({
      id: record.staff_id,
      partyId: record.party_id,
      name: record.staff_name,
      role: record.role,
      department: record.department_cost_center,
      totalHours: record.total_hours,
      expectedHours: record.expected_hours,
      workedHours: record.worked_hours,
      approvedHours: record.approved_hours,
      varianceHours: record.variance_hours,
      ...
      completedShifts: record.completed_shifts,
      missedShifts: record.missed_shifts,
      lateCount: record.late_count,
      totalLateMinutes: record.total_late_minutes,
      baseSalary: record.base_salary,
      serviceChargeBonus: record.service_charge_bonus,
    })),
