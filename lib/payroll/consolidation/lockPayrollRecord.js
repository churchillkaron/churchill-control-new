...
export default async function lockPayrollRecord({
  ...
  const accounting = await postPayrollAccrual({
    payrollRecordId,
    organizationId,
  });
  ...
  if (!updated) {
    throw new Error("Payroll status changed before lock completed; reload and retry");
  }
  ...
  await createPayrollAuditLog({
    ...
    notes: `Payroll locked and accrued for ${record.staff_name}`,
  });
