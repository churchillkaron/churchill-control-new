...
export const PAYROLL_ACTIONS = {
  APPROVE: {
    label: "APPROVE",
    nextStatus: PAYROLL_STATUS.APPROVED,
    roles: ["OWNER", "ACCOUNTING_ADMIN", "PAYROLL_ADMIN", "MANAGER"],
    handler: approvePayrollRecord,
  },

  REJECT: {
    label: "REJECT",
    nextStatus: PAYROLL_STATUS.REJECTED,
    roles: ["OWNER", "ACCOUNTING_ADMIN", "PAYROLL_ADMIN", "MANAGER"],
    handler: rejectPayrollRecord,
  },

  RECALCULATE: {
    label: "RECALCULATE",
    nextStatus: PAYROLL_STATUS.RECALCULATED,
    roles: ["OWNER", "ACCOUNTING_ADMIN", "PAYROLL_ADMIN"],
    handler: recalculatePayrollRecord,
  },

  LOCK: {
    label: "LOCK",
    nextStatus: PAYROLL_STATUS.LOCKED,
    roles: ["OWNER", "ACCOUNTING_ADMIN", "PAYROLL_ADMIN"],
    handler: lockPayrollRecord,
  },

  FINALIZE: {
    label: "FINALIZE",
    nextStatus: PAYROLL_STATUS.FINALIZED,
    roles: ["OWNER", "ACCOUNTING_ADMIN", "PAYROLL_ADMIN"],
    handler: finalizePayrollRecord,
  },

  ACCOUNTING_CLOSE: {
    label: "ACCOUNTING CLOSE",
    nextStatus: PAYROLL_STATUS.ACCOUNTING_CLOSED,
    roles: ["OWNER", "ACCOUNTING_ADMIN"],
    handler: closePayrollAccountingPeriod,
  },

  CERTIFY: {
    label: "CERTIFY",
    nextStatus: PAYROLL_STATUS.CERTIFIED,
    roles: ["OWNER", "ACCOUNTING_ADMIN"],
    handler: certifyPayrollRecord,
  },

  ARCHIVE: {
    label: "ARCHIVE",
    nextStatus: PAYROLL_STATUS.ARCHIVED,
    roles: ["OWNER", "ACCOUNTING_ADMIN"],
    handler: archivePayrollRecord,
  },
};
...
export function getAllowedPayrollActions({
  ...
  const allowedStatuses = PAYROLL_TRANSITIONS[currentStatus] || [];

  return Object.entries(PAYROLL_ACTIONS)
    .filter(
      ([, action]) =>
        allowedStatuses.includes(action.nextStatus) &&
