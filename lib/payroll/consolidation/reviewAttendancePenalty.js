...
export default async function reviewAttendancePenalty({
  ...
  if (!ALLOWED_ROLES.has(normalizedRole)) {
    throw new Error("Unauthorized attendance penalty review");
  }
  ...
  if (!["APPROVE", "WAIVE", "ADJUST"].includes(normalizedDecision)) {
    throw new Error("Attendance penalty decision must be APPROVE, WAIVE, or ADJUST");
  }
  if (normalizedDecision === "ADJUST" && !String(notes || "").trim()) {
  ...
  if (!REVIEWABLE_STATUSES.has(record.status)) {
    throw new Error("Attendance penalty can only be reviewed before payroll approval");
  }
  ...
  await createPayrollAuditLog({
    ...
    action: `ATTENDANCE_PENALTY_${reviewStatus}`,
    performedBy: actorName,
    targetStaffId: record.staff_id,
    targetPartyId: record.party_id || null,
    notes: `${currentAttendancePenalty.toFixed(2)} -> ${approvedAttendancePenalty.toFixed(2)}${cleanNotes ? ` · ${cleanNotes}` : ""}`,
  });
