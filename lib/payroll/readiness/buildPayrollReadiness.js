...
export default async function buildPayrollReadiness({
  ...
  if (missingSchedules.length) {
    blockers.push(
      issue(
        "SCHEDULES_MISSING",
        `${missingSchedules.length} paid staff member${missingSchedules.length === 1 ? "" : "s"} have no schedule for ${payrollMonth}, while payroll is configured to use scheduled expected hours.`,
