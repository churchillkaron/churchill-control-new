...
export default async function buildPayrollReadiness({
  ...
  const [
    ...
    supabaseAdmin
      ...
      .select("id,staff_id,entity_id,shift_date")
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
      .eq("status", "PUBLISHED")
    ...
    supabaseAdmin
      .from("staff_shifts")
      .select("id,staff_id,entity_id,clock_in,clock_out,shift_status,approval_status,is_valid")
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
    ...
    supabaseAdmin
      ...
      .select("id,staff_id,entity_id,shift_id,shift_date,attendance_status")
      .eq("organization_id", organizationId)
      .eq("entity_id", entityId)
      .gte("shift_date", range.start)
      .lt("shift_date", range.end),
  ...
  for (const profile of compensationResult.data || []) {
    if (
      entityStaffIds.has(profile.staff_account_id) &&
      !compensationByStaff.has(profile.staff_account_id)
    ) {
  ...
  const rawShifts = (shiftsResult.data || []).filter((row) =>
    entityStaffIds.has(row.staff_id)
  );
  ...
  const legacyUnscopedShifts = (legacyShiftsResult.data || []).filter((row) =>
    entityStaffIds.has(row.staff_id)
  );
  const legacyUnscopedAttendance = (legacyAttendanceResult.data || []).filter((row) =>
    entityStaffIds.has(row.staff_id)
  );
  ...
  const paymentMethods = allPaymentMethods.filter((method) => {
    ...
    return currency === jurisdiction.currency && (!country || country === jurisdiction.country);
  });
  ...
  const resolvedEvidenceStaffIds = new Set([
    ...shiftStaffIds,
    ...attendanceStaffIds,
  ]);
  ...
  const missingSchedules = settings?.use_schedule_expected_hours
    ...
    : [];
  const unresolvedScheduledEvidence = payrollExposedStaff.filter(
    (member) =>
      scheduledStaffIds.has(member.id) &&
      !resolvedEvidenceStaffIds.has(member.id)
  );
  ...
  if (payrollMonth < currentMonth && unresolvedScheduledEvidence.length) {
    blockers.push(
      issue(
        "WORKFORCE_EVIDENCE_UNRESOLVED",
        `${unresolvedScheduledEvidence.length} scheduled paid employee${unresolvedScheduledEvidence.length === 1 ? " has" : "s have"} neither approved entity-scoped shift evidence nor approved attendance/absence evidence for the closed payroll month. Resolve attendance before generation.`,
        unresolvedScheduledEvidence.length,
        unresolvedScheduledEvidence
      )
    );
  }
  ...
  if (scheduledWithoutShiftEvidence.length) {
    warnings.push(
      issue(
        ...
        `${scheduledWithoutShiftEvidence.length} scheduled paid employee${scheduledWithoutShiftEvidence.length === 1 ? " has" : "s have"} no approved entity-scoped shift evidence for the month. Approved attendance/absence evidence can resolve the schedule without a worked shift.`,
  ...
  if (scheduledWithoutAttendance.length) {
    warnings.push(
      issue(
        ...
        `${scheduledWithoutAttendance.length} scheduled paid employee${scheduledWithoutAttendance.length === 1 ? " has" : "s have"} no approved entity-scoped attendance evidence for the month. Approved shift evidence still counts as worked evidence.`,
  ...
  if (legacyUnscopedShifts.length || legacyUnscopedAttendance.length) {
    warnings.push({
      code: "LEGACY_UNSCOPED_WORKFORCE_EVIDENCE_IGNORED",
      message:
        `${legacyUnscopedShifts.length} legacy shift row${legacyUnscopedShifts.length === 1 ? "" : "s"} and ${legacyUnscopedAttendance.length} legacy attendance row${legacyUnscopedAttendance.length === 1 ? "" : "s"} for employees in this legal-entity cohort have no entity_id and were intentionally excluded from payroll readiness.`,
      count: legacyUnscopedShifts.length + legacyUnscopedAttendance.length,
      affectedStaff: [],
    });
  }
  ...
  return {
    ...
    summary: {
      ...
      unresolvedScheduledEvidence: unresolvedScheduledEvidence.length,
      legacyUnscopedShiftRowsIgnored: legacyUnscopedShifts.length,
      legacyUnscopedAttendanceRowsIgnored: legacyUnscopedAttendance.length,
    ...
    canCompleteLifecycle:
      blockers.length === 0 && lifecycleBlockers.length === 0,
  };
}
