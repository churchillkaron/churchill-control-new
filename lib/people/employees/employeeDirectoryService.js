...
function isOwnerLevelRole(value) {
  return OWNER_LEVEL_ROLES.has(normalizeRole(value));
}
...
async function bestEffortDeleteNewEmployee({
  organizationId,
  staffId = null,
  partyId = null,
  deleteParty = false,
...
async function bestEffortRestoreParty({ organizationId, partyId, values }) {
  try {
    await supabaseAdmin
      .from("parties")
      .update(values)
    ...
    // Preserve the original employee update failure.
  }
}
...
async function assertOwnerDeactivationSafe({ organizationId, staffId, staffRole }) {
  ...
  const targetIsOwner =
    isOwnerLevelRole(staffRole) || isOwnerLevelRole(membership?.role);
  ...
  const membershipRoleByStaff = new Map(
    (membershipResult.data || []).map((row) => [
      String(row.staff_account_id),
      row.role,
    ])
  );

  const anotherOwnerExists = (staffResult.data || []).some((staff) => {
    if (String(staff.id) === String(staffId)) return false;

    return (
      isOwnerLevelRole(staff.role) ||
      isOwnerLevelRole(membershipRoleByStaff.get(String(staff.id)))
    );
  });
...
export async function createEmployeeRecord({
  ...
  try {
    ...
    await bestEffortDeleteNewEmployee({
      organizationId,
      staffId: createdStaffId,
      partyId: party.id,
      deleteParty: partyCreated,
    });
    throw error;
  }
}
...
export async function updateEmployeeRecord({
  ...
  if (staffError) {
    await bestEffortRestoreParty({
      organizationId,
      partyId: party.id,
      values: previousParty,
    });
    throw staffError;
  }
