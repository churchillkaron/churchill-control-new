import { randomBytes } from "crypto";

import { supabaseAdmin } from "@/lib/shared/supabase/admin";

function normalizeEmail(value) {
  return String(value || "").trim().toLowerCase();
}

function normalizeRole(value) {
  return String(value || "").trim().toUpperCase();
}

async function findAuthUserByEmail(email) {
  const normalizedEmail = normalizeEmail(email);
  const perPage = 200;

  for (let page = 1; page <= 50; page += 1) {
    const { data, error } = await supabaseAdmin.auth.admin.listUsers({
      page,
      perPage,
    });

    if (error) throw error;

    const users = data?.users || [];
    const match = users.find(
      (user) => normalizeEmail(user?.email) === normalizedEmail
    );

    if (match) return match;
    if (users.length < perPage) return null;
  }

  throw new Error("Unable to resolve authentication user safely");
}

async function ensureParty({ organizationId, name, email, existingPartyId = null }) {
  if (existingPartyId) {
    const { data, error } = await supabaseAdmin
      .from("parties")
      .select("id,organization_id,display_name,email,status")
      .eq("id", existingPartyId)
      .eq("organization_id", organizationId)
      .maybeSingle();

    if (error) throw error;
    if (data) return data;
  }

  const { data: existingParty, error: existingPartyError } = await supabaseAdmin
    .from("parties")
    .select("id,organization_id,display_name,email,status")
    .eq("organization_id", organizationId)
    .ilike("email", email)
    .limit(1)
    .maybeSingle();

  if (existingPartyError) throw existingPartyError;
  if (existingParty) return existingParty;

  const { data: party, error: partyError } = await supabaseAdmin
    .from("parties")
    .insert({
      organization_id: organizationId,
      party_type: "person",
      display_name: name,
      email,
      status: "active",
    })
    .select("id,organization_id,display_name,email,status")
    .single();

  if (partyError) throw partyError;
  return party;
}

async function ensureEmployeeRelationship({ organizationId, partyId }) {
  const { data: existing, error: existingError } = await supabaseAdmin
    .from("party_relationships")
    .select("id")
    .eq("organization_id", organizationId)
    .eq("party_id", partyId)
    .eq("relationship_type", "employee")
    .eq("status", "active")
    .is("end_date", null)
    .limit(1)
    .maybeSingle();

  if (existingError) throw existingError;
  if (existing) return existing.id;

  const { data, error } = await supabaseAdmin
    .from("party_relationships")
    .insert({
      organization_id: organizationId,
      party_id: partyId,
      relationship_type: "employee",
      status: "active",
    })
    .select("id")
    .single();

  if (error) throw error;
  return data.id;
}

async function ensureMembership({ organizationId, staffAccountId, role }) {
  const { data: existing, error: existingError } = await supabaseAdmin
    .from("organization_users")
    .select("id,role,status")
    .eq("organization_id", organizationId)
    .eq("staff_account_id", staffAccountId)
    .limit(1)
    .maybeSingle();

  if (existingError) throw existingError;

  if (existing) {
    if (existing.status !== "active" || normalizeRole(existing.role) !== role) {
      const { error: updateError } = await supabaseAdmin
        .from("organization_users")
        .update({
          role: role.toLowerCase(),
          status: "active",
        })
        .eq("id", existing.id)
        .eq("organization_id", organizationId);

      if (updateError) throw updateError;
    }

    return existing.id;
  }

  const inserted = await supabaseAdmin
    .from("organization_users")
    .insert({
      organization_id: organizationId,
      staff_account_id: staffAccountId,
      role: role.toLowerCase(),
      status: "active",
    })
    .select("id")
    .maybeSingle();

  if (!inserted.error && inserted.data?.id) return inserted.data.id;
  if (inserted.error?.code !== "23505") {
    if (inserted.error) throw inserted.error;
    throw new Error("Organization membership was not created");
  }

  const raced = await supabaseAdmin
    .from("organization_users")
    .select("id,role,status")
    .eq("organization_id", organizationId)
    .eq("staff_account_id", staffAccountId)
    .limit(1)
    .maybeSingle();
  if (raced.error) throw raced.error;
  if (!raced.data?.id) throw inserted.error;

  if (raced.data.status !== "active" || normalizeRole(raced.data.role) !== role) {
    const repaired = await supabaseAdmin
      .from("organization_users")
      .update({ role: role.toLowerCase(), status: "active" })
      .eq("id", raced.data.id)
      .eq("organization_id", organizationId)
      .eq("staff_account_id", staffAccountId);
    if (repaired.error) throw repaired.error;
  }

  return raced.data.id;
}

async function unlinkAuthentication({ staffId, authUserId }) {
  await supabaseAdmin
    .from("staff_accounts")
    .update({ auth_user_id: null })
    .eq("id", staffId)
    .eq("auth_user_id", authUserId);
}

export default async function provisionStaffAccess({
  organizationId,
  name,
  email,
  role,
  position = null,
  redirectTo = null,
}) {
  const normalizedName = String(name || "").trim();
  const normalizedEmail = normalizeEmail(email);
  const normalizedRole = normalizeRole(role);

  if (!organizationId) throw new Error("organizationId required");
  if (!normalizedName) throw new Error("name required");
  if (!normalizedEmail) throw new Error("email required");
  if (!normalizedRole) throw new Error("role required");
  if (!redirectTo) throw new Error("redirectTo required");

  const staffFields =
    "id,name,email,role,position,department,active,auth_user_id,party_id,active_organization_id";
  const { data: staffMatches, error: existingStaffError } = await supabaseAdmin
    .from("staff_accounts")
    .select(staffFields)
    .ilike("email", normalizedEmail)
    .limit(2);

  if (existingStaffError) throw existingStaffError;
  if ((staffMatches || []).length > 1) {
    throw new Error("Multiple Staff identities use this email");
  }

  let staff = (staffMatches || [])[0] || null;
  const existingPartyId =
    staff?.active_organization_id === organizationId ? staff?.party_id || null : null;
  const party = await ensureParty({
    organizationId,
    name: normalizedName,
    email: normalizedEmail,
    existingPartyId,
  });

  if (!staff) {
    const inserted = await supabaseAdmin
      .from("staff_accounts")
      .insert({
        name: normalizedName,
        email: normalizedEmail,
        role: normalizedRole,
        position: position || null,
        active: true,
        active_organization_id: organizationId,
        party_id: party.id,
      })
      .select(staffFields)
      .maybeSingle();

    if (inserted.error?.code === "23505") {
      const recovered = await supabaseAdmin
        .from("staff_accounts")
        .select(staffFields)
        .ilike("email", normalizedEmail)
        .limit(2);
      if (recovered.error) throw recovered.error;
      if ((recovered.data || []).length !== 1) {
        throw new Error("Concurrent Staff identity creation could not be resolved safely");
      }
      staff = recovered.data[0];
    } else {
      if (inserted.error) throw inserted.error;
      if (!inserted.data?.id) throw new Error("Staff identity was not created");
      staff = inserted.data;
    }
  } else {
    const updates = {};
    const sameActiveOrganization =
      String(staff.active_organization_id || "") === String(organizationId);

    if (staff.active === false) updates.active = true;
    if (!staff.name) updates.name = normalizedName;

    if (staff.active === false || !staff.active_organization_id) {
      updates.active_organization_id = organizationId;
      updates.party_id = party.id;
      updates.role = normalizedRole;
      if (position !== null) updates.position = position || null;
    } else if (sameActiveOrganization) {
      if (!staff.party_id) updates.party_id = party.id;
      if (normalizeRole(staff.role) !== normalizedRole) updates.role = normalizedRole;
      if (position !== null && String(staff.position || "") !== String(position || "")) {
        updates.position = position || null;
      }
    }

    if (Object.keys(updates).length) {
      const { data, error } = await supabaseAdmin
        .from("staff_accounts")
        .update(updates)
        .eq("id", staff.id)
        .select(staffFields)
        .single();

      if (error) throw error;
      staff = data;
    }
  }

  await ensureEmployeeRelationship({
    organizationId,
    partyId: party.id,
  });

  await ensureMembership({
    organizationId,
    staffAccountId: staff.id,
    role: normalizedRole,
  });

  if (staff.auth_user_id) {
    return {
      staff,
      party,
      authUserId: staff.auth_user_id,
      inviteSent: false,
      accessEmailSent: false,
      alreadyLinked: true,
    };
  }

  let authUser = await findAuthUserByEmail(normalizedEmail);
  let createdAuthUser = false;

  if (!authUser) {
    const created = await supabaseAdmin.auth.admin.createUser({
      email: normalizedEmail,
      password: randomBytes(32).toString("base64url"),
      email_confirm: true,
    });

    if (created.error) {
      const racedUser = await findAuthUserByEmail(normalizedEmail);
      if (!racedUser) throw created.error;
      authUser = racedUser;
    } else {
      authUser = created.data?.user || null;
      createdAuthUser = true;
    }
  }

  if (!authUser?.id) {
    throw new Error("Authentication user was not created");
  }

  const { data: conflictingStaff, error: conflictError } = await supabaseAdmin
    .from("staff_accounts")
    .select("id,active_organization_id")
    .eq("auth_user_id", authUser.id)
    .neq("id", staff.id)
    .limit(1)
    .maybeSingle();

  if (conflictError) throw conflictError;

  if (conflictingStaff) {
    throw new Error("Authentication user is already linked to another Staff identity");
  }

  const linkResult = await supabaseAdmin
    .from("staff_accounts")
    .update({ auth_user_id: authUser.id })
    .eq("id", staff.id)
    .ilike("email", normalizedEmail)
    .is("auth_user_id", null)
    .select(staffFields)
    .maybeSingle();

  let linkedStaff = linkResult.data || null;
  if (linkResult.error || !linkedStaff) {
    const observed = await supabaseAdmin
      .from("staff_accounts")
      .select(staffFields)
      .eq("id", staff.id)
      .maybeSingle();

    if (!observed.error && String(observed.data?.auth_user_id || "") === String(authUser.id)) {
      linkedStaff = observed.data;
    } else {
      if (
        createdAuthUser &&
        !observed.error &&
        !observed.data?.auth_user_id
      ) {
        await supabaseAdmin.auth.admin.deleteUser(authUser.id).catch(() => null);
      }
      if (linkResult.error) throw linkResult.error;
      if (observed.error) throw observed.error;
      throw new Error("Unable to link authentication user to staff account");
    }
  }

  const { error: resetError } = await supabaseAdmin.auth.resetPasswordForEmail(
    normalizedEmail,
    { redirectTo }
  );

  if (resetError) {
    await unlinkAuthentication({
      staffId: linkedStaff.id,
      authUserId: authUser.id,
    });

    if (createdAuthUser) {
      await supabaseAdmin.auth.admin.deleteUser(authUser.id).catch(() => null);
    }

    throw resetError;
  }

  return {
    staff: linkedStaff,
    party,
    authUserId: authUser.id,
    inviteSent: true,
    accessEmailSent: true,
    alreadyLinked: false,
  };
}
