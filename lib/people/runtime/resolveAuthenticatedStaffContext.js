...
import { requireStaffActivation } from "@/lib/people/workforce/StaffActivationRuntime";
...
export default async function resolveAuthenticatedStaffContext({
  ...
  allowIncompleteActivation = false,
} = {}) {
  ...
  const authoritativeOrganizationId =
    explicitOrganizationId || requestOrganizationId || null;

  if (
    authoritativeOrganizationId &&
    !availableOrganizationIds.includes(authoritativeOrganizationId)
  ) {
    return failure({
      status: 403,
  ...
  const safePersistedOrganizationId =
    persistedOrganizationId && availableOrganizationIds.includes(persistedOrganizationId)
      ? persistedOrganizationId
      : null;
  const safeStaffOrganizationId =
    staffOrganizationId && availableOrganizationIds.includes(staffOrganizationId)
      ? staffOrganizationId
      : null;

  const preferredOrganizationId =
    authoritativeOrganizationId ||
    safePersistedOrganizationId ||
    safeStaffOrganizationId ||
  ...
  let activation = null;
  if (!allowIncompleteActivation) {
    try {
      activation = await requireStaffActivation({
        organizationId: access.organizationId,
        staff: access.staff,
        user,
      });
    } catch (error) {
      if (error?.code === "STAFF_ACTIVATION_REQUIRED") {
        return {
          ...failure({
          ...
          activation: error.activation || null,
        };
      }
      throw error;
    }
