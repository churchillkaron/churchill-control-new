...
function primaryStaffOrganizationId(staffRows) {
...
export default async function resolveAuthenticatedStaffContext({
  ...
  const requestOrganizationId =
    headerOrganizationId(request) || queryOrganizationId(request);
  const persistedOrganizationId =
    cookieOrganizationId() || metadataOrganizationId(user);
  const primaryOrganizationId = primaryStaffOrganizationId(activeStaffRows);
  ...
  const explicitSelection = Boolean(
    explicitOrganizationId || requestOrganizationId
  );
  ...
  for (const candidateOrganizationId of candidateIds) {
    if (
      explicitSelection &&
      preferredOrganizationId &&
      candidateOrganizationId !== preferredOrganizationId
    ) {
      continue;
    }
      ...
      selectionSource: explicitOrganizationId
        ...
        : requestOrganizationId
          ...
          : persistedOrganizationId
            ...
            : primaryOrganizationId
              ? "primary"
              : availableOrganizationIds.length === 1
                ? "single"
                : "fallback",
    };
  }
  ...
  return failure({
    ...
    error: lastDenied?.error || "Organization membership required",
    code: explicitSelection
