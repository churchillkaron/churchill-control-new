...
function normalizeReturnPath(value) {
  const path = String(value || "/workforce").trim();
  if (!path.startsWith("/") || path.startsWith("//")) return "/workforce";
  return path.startsWith("/workforce") ? path : "/workforce";
}
...
export async function resolveOrganizationStaffPortalOrigin(organizationId) {
  ...
  if (String(organizationId) === "33336a72-acb5-474e-856b-8be0269360e2") {
    return "https://app.churchillkaron.com";
  }
  if (String(organizationId) === "9550b843-b83c-4d15-b02d-a0b5ca23346e") {
    return "https://coleley.com";
  }
...
export async function createStaffPasskeyBrokerAuthorization({ request, returnPath = "/workforce" } = {}) {
