...
export async function loadWorkPermitReviewQueue({ organizationId } = {}) {
  ...
  const membershipResult = staffIds.length
    ? await supabaseAdmin
  ...
  const [staffResult, entityResult, obligationResult] = await Promise.all([
    memberStaffIds.length
      ? supabaseAdmin.from("staff_accounts").select("id,name,role,position,department,active").eq("active", true).in("id", memberStaffIds)
