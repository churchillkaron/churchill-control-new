...
function encodeRequestMetadata({ reason, targets, failureCode = null }) {
  return JSON.stringify({
    kind: CLOCK_IN_EXCEPTION_REFERENCE,
    version: 1,
    reason: String(reason || "").trim(),
    targets: normalizeTargets(targets),
    failureCode: String(failureCode || "").trim() || null,
  });
}
...
export async function requestClockInException({
  ...
  const { data: existing, error: existingError } = await supabaseAdmin
    ...
    .in("status", ["pending", "approved", "consuming"])
    .order("created_at", { ascending: false })
    .limit(20);

  if (existingError) throw existingError;

  await expireRequests({ organizationId, requests: existing || [], now });
  const metadataMap = await loadMetadataMap({
    organizationId,
    referenceIds: (existing || []).map((request) => request.reference_id),
  });

  for (const request of existing || []) {
    if (request.status === "approved" && grantExpired(request, now)) continue;
    const metadata = metadataMap.get(request.reference_id);
    const overlaps = cleanTargets.some((target) => metadata?.targets?.includes(target));
    if (!overlaps) continue;

    return {
      created: false,
      request: serializeRequest(request, metadata, now),
    };
  }
  ...
  const { data: approvalRequest, error } = await supabaseAdmin
    ...
    .insert({
      ...
      workflow_id: null,
      reference_table: CLOCK_IN_EXCEPTION_REFERENCE,
  ...
  await createApprovalLog({
    ...
    toStatus: "pending",
    actedBy: staff.id,
    role: normalizeRole(staff.role || staff.position),
    notes: encodeRequestMetadata(metadata),
  });

  return {
    created: true,
    request: serializeRequest(approvalRequest, metadata, now),
  };
}
