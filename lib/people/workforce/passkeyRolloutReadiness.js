...
function errorCode(error) {
  return String(error?.code || error?.error_code || "").trim().toLowerCase();
}
...
export async function loadOrganizationPasskeyReadiness({
  ...
  let passkeyFeatureDisabled = false;
  ...
  for (const account of linkedMembers) {
    try {
      ...
      if (errorCode(error) === "passkey_disabled") {
        passkeyFeatureDisabled = true;
        break;
      }
      throw error;
    }
  ...
  const activationReady =
    !passkeyFeatureDisabled &&
    fullAuthCoverage &&
    fullEnrollmentCoverage &&
    recentVerificationProven;

  const blockers = [];
  if (passkeyFeatureDisabled) {
    blockers.push("Supabase Passkey authentication is disabled for this project.");
  }
