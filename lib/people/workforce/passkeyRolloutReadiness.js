import { supabaseAdmin } from "@/lib/shared/supabase/admin";
import { loadStaffPasskeyStatus } from "@/lib/people/workforce/passkeyClockInVerification";
import probeHostedPasskeyConfiguration, {
  CANONICAL_WORKFORCE_ORIGIN,
  REQUIRED_PASSKEY_RP_ID,
} from "@/lib/people/workforce/hostedPasskeyConfiguration";

const RECENT_VERIFICATION_WINDOW_MS = 30 * 60 * 1000;
const NON_CLOCK_IN_ROLES = new Set([
  "OWNER",
  "ORGANIZATION_OWNER",
  "ORG_OWNER",
  "PLATFORM_OWNER",
  "SUPER_ADMIN",
  "ADMIN",
  "ACCOUNTING",
  "FINANCE",
]);

function normalizedRole(value) {
  return String(value || "").trim().toUpperCase();
}

export function isClockInRole(value) {
  const role = normalizedRole(value);
  return Boolean(role) && !NON_CLOCK_IN_ROLES.has(role);
}

export async function loadOrganizationPasskeyReadiness({
  organizationId,
  now = new Date(),
}) {
  if (!organizationId) {
    throw new Error("organizationId required");
  }

  const hostedConfiguration = await probeHostedPasskeyConfiguration({ now });

  const { data: memberships, error: membershipError } = await supabaseAdmin
    .from("organization_users")
    .select("staff_account_id,status")
    .eq("organization_id", organizationId)
    .eq("status", "active");

  if (membershipError) throw membershipError;

  const activeMemberships = Array.isArray(memberships) ? memberships : [];
  const staffAccountIds = Array.from(
    new Set(activeMemberships.map((row) => row?.staff_account_id).filter(Boolean))
  );

  let staffAccounts = [];
  if (staffAccountIds.length) {
    const { data, error } = await supabaseAdmin
      .from("staff_accounts")
      .select("id,name,email,auth_user_id,active,role")
      .in("id", staffAccountIds);

    if (error) throw error;
    staffAccounts = Array.isArray(data) ? data : [];
  }

  const accountById = new Map(staffAccounts.map((account) => [account.id, account]));
  const clockInMembers = activeMemberships
    .map((membership) => accountById.get(membership?.staff_account_id) || null)
    .filter((account) => {
      if (!account || account.active === false) return false;
      return isClockInRole(account.role);
    });

  const linkedMembers = clockInMembers.filter((account) => account?.auth_user_id);
  const statusByAccountId = new Map();

  let providerAvailable = true;
  let providerError = null;
  let enrolledMembers = 0;
  let recentlyVerifiedMembers = 0;
  let latestVerifiedAt = null;

  for (const account of linkedMembers) {
    try {
      const status = await loadStaffPasskeyStatus({ userId: account.auth_user_id });
      statusByAccountId.set(account.id, {
        checked: true,
        enrolled: status.enrolled,
        count: status.count,
        lastUsedAt: status.lastUsedAt,
      });

      if (status.enrolled) enrolledMembers += 1;

      if (status.lastUsedAt) {
        const usedAt = new Date(status.lastUsedAt);
        if (!Number.isNaN(usedAt.getTime())) {
          if (!latestVerifiedAt || usedAt > latestVerifiedAt) {
            latestVerifiedAt = usedAt;
          }

          const ageMs = now.getTime() - usedAt.getTime();
          if (ageMs >= -60 * 1000 && ageMs <= RECENT_VERIFICATION_WINDOW_MS) {
            recentlyVerifiedMembers += 1;
          }
        }
      }
    } catch (error) {
      providerAvailable = false;
      providerError =
        error?.message || "Unable to verify Supabase Passkey administration API";
      statusByAccountId.set(account.id, {
        checked: false,
        enrolled: false,
        count: 0,
        lastUsedAt: null,
      });
      break;
    }
  }

  const activeMemberCount = clockInMembers.length;
  const authLinkedMemberCount = linkedMembers.length;
  const missingAuthMemberCount = Math.max(
    0,
    activeMemberCount - authLinkedMemberCount
  );
  const missingEnrollmentCount = Math.max(
    0,
    activeMemberCount - enrolledMembers
  );
  const fullAuthCoverage =
    activeMemberCount > 0 && authLinkedMemberCount === activeMemberCount;
  const fullEnrollmentCoverage =
    activeMemberCount > 0 && enrolledMembers === activeMemberCount;
  const recentVerificationProven = recentlyVerifiedMembers > 0;
  const hostedConfigurationReady =
    hostedConfiguration?.configurationReady === true;
  const activationReady =
    providerAvailable &&
    hostedConfigurationReady &&
    fullAuthCoverage &&
    fullEnrollmentCoverage &&
    recentVerificationProven;

  const blockers = [];
  if (!providerAvailable) {
    blockers.push(
      "Supabase Passkey administration API could not be verified."
    );
  }
  if (!hostedConfigurationReady) {
    if (hostedConfiguration?.enabled === false) {
      blockers.push(
        "Hosted Supabase Passkeys are disabled. Enable Passkey authentication before rollout."
      );
    } else if (hostedConfiguration?.enabled === true && !hostedConfiguration?.rpIdMatches) {
      blockers.push(
        `Hosted Supabase Passkey RP ID must be ${REQUIRED_PASSKEY_RP_ID}.`
      );
    } else {
      blockers.push(
        hostedConfiguration?.error ||
          "Hosted Supabase Passkey configuration could not be verified."
      );
    }
  }
  if (activeMemberCount === 0) {
    blockers.push("No active clock-in staff are available for rollout verification.");
  }
  if (missingAuthMemberCount > 0) {
    blockers.push(
      `${missingAuthMemberCount} active clock-in staff member${missingAuthMemberCount === 1 ? " is" : "s are"} not linked to a Supabase Auth identity.`
    );
  }
  if (missingEnrollmentCount > 0) {
    blockers.push(
      `${missingEnrollmentCount} active clock-in staff member${missingEnrollmentCount === 1 ? " still needs" : "s still need"} a registered passkey.`
    );
  }
  if (!recentVerificationProven) {
    blockers.push(
      "No successful passkey verification has been recorded in the last 30 minutes."
    );
  }

  const staff = clockInMembers
    .map((account) => {
      const passkey = statusByAccountId.get(account.id) || null;
      return {
        staffId: account.id,
        name: account.name || account.email || "Staff",
        email: account.email || null,
        role: normalizedRole(account.role) || "STAFF",
        authLinked: Boolean(account.auth_user_id),
        passkeyChecked: Boolean(passkey?.checked),
        enrolled: Boolean(passkey?.enrolled),
        passkeyCount: Number(passkey?.count || 0),
        lastUsedAt: passkey?.lastUsedAt || null,
      };
    })
    .sort((left, right) =>
      String(left.name || "").localeCompare(String(right.name || ""))
    );

  return {
    canonicalOrigin: CANONICAL_WORKFORCE_ORIGIN,
    requiredRpId: REQUIRED_PASSKEY_RP_ID,
    verificationWindowMinutes: Math.round(
      RECENT_VERIFICATION_WINDOW_MS / 60000
    ),
    providerAvailable,
    providerError,
    hostedConfiguration: {
      reachable: hostedConfiguration?.reachable === true,
      enabled: hostedConfiguration?.enabled ?? null,
      rpId: hostedConfiguration?.rpId || null,
      rpIdMatches: hostedConfiguration?.rpIdMatches === true,
      ready: hostedConfigurationReady,
      error: hostedConfiguration?.error || null,
    },
    activeMemberCount,
    authLinkedMemberCount,
    enrolledMemberCount: enrolledMembers,
    recentlyVerifiedMemberCount: recentlyVerifiedMembers,
    missingAuthMemberCount,
    missingEnrollmentCount,
    fullAuthCoverage,
    fullEnrollmentCoverage,
    recentVerificationProven,
    latestVerifiedAt: latestVerifiedAt?.toISOString() || null,
    activationReady,
    blockers,
    staff,
  };
}

export { RECENT_VERIFICATION_WINDOW_MS };

export default loadOrganizationPasskeyReadiness;
