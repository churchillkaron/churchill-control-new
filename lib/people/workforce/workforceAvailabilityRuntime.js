...
export async function replaceStaffAvailabilityPattern({ organizationId, staff, effectiveFrom, rules } = {}) {
  ...
  const today = localDateString(new Date(), timeContext.timezone);
  if (effective < today) {
    const error = new Error("Availability changes cannot be backdated");
    error.status = 400;
    error.code = "AVAILABILITY_BACKDATE_FORBIDDEN";
    throw error;
  }
...
export async function createAvailabilityException({ organizationId, staff, exceptionDate, availabilityType, startTime, endTime, notes } = {}) {
  if (!staff?.id) throw new Error("staff required");
  const normalizedDate = cleanDate(exceptionDate, "exceptionDate");
  const timeContext = await resolveOrganizationTimeContext({ organizationId });
  const today = localDateString(new Date(), timeContext.timezone);
  if (normalizedDate < today) {
    const error = new Error("Availability exceptions cannot be created in the past");
    error.status = 400;
    error.code = "AVAILABILITY_EXCEPTION_PAST_DATE";
    throw error;
  }
  ...
  const { data, error } = await supabaseAdmin.from("staff_availability_exceptions").insert({ organization_id: organizationId, staff_id: staff.id, party_id: staff.party_id || null, exception_date: normalizedDate, availability_type: cleanType(availabilityType), start_time: start, end_time: end, notes: cleanNotes(notes), status: "ACTIVE", created_by_staff_id: staff.id }).select("*").single();
  if (error) {
