...
async function businessDate(organizationId) {
  const timeContext = await resolveOrganizationTimeContext({ organizationId });
  return localDateString(new Date(), timeContext.timezone);
}
...
export async function createTimeOffRequest({
  ...
  const today = await businessDate(organizationId);
  if (start < today) {
    const past = new Error("Time-off requests cannot start in the past");
    past.status = 400;
    past.code = "TIME_OFF_PAST_DATE";
    throw past;
  }
  ...
  if ((overlap.data || []).length) {
    const conflict = new Error("An active time-off request already overlaps this date range");
    conflict.status = 409;
    conflict.code = "TIME_OFF_REQUEST_OVERLAP";
    throw conflict;
  }
...
export async function createShiftSwapRequest({ organizationId, staff, scheduleId, targetStaffId, reason }) {
  ...
  if (schedule.shift_date < today) {
    throw Object.assign(new Error("Past shifts cannot be swapped"), { status: 409, code: "SHIFT_SWAP_PAST_SHIFT" });
  }
