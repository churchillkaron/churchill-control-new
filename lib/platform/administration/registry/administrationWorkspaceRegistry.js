import PayrollSetupWorkspace from "@/lib/platform/administration/onboarding/workspaces/PayrollSetupWorkspace";
import OrganizationSetupWorkspace from "@/lib/platform/administration/onboarding/workspaces/OrganizationSetupWorkspace";

const ONBOARDING_GROUP_ID = "onboarding_setup";
const ACCESS_GROUP_ID = "access_security";
const ADMINISTRATION_ROUTE = "/administration";
const LEGACY_SETTINGS_ROUTE = "/settings";

const OrganizationPolicyWorkspace = {
  id: "organization_policy",
  name: "Organization Policy",
  route: "/administration/access-policy",
  description:
    "Control organization access, workforce timing, passkey verification, GPS requirements and per-business clock-in geofences.",
  order: 10,
  status: "active",
  type: "business-workspace",
  document: "OrganizationPolicy",
};


const ComputeWorkspace = {
  id: "compute_control",
  name: "Compute",
  route: "/administration/compute",
  description: "Monitor Avantiqo-owned compute nodes, GPU capacity, local models, queues and cloud overflow routing.",
  order: 30,
  status: "active",
  type: "business-workspace",
  document: "ComputeControl",
};


const ProductControlWorkspace = {
  id: "product_control",
  name: "Product Control",
  route: "/administration/product-control",
  description: "Owner-only portfolio control for product completion, release readiness and sellable product specifications.",
  order: 40,
  status: "active",
  type: "business-workspace",
  document: "ProductControl",
};

const DomainsWorkspace = {
  id: "domains_external_access",
  name: "Domains & External Access",
  route: "/administration/domains",
  description:
    "Register and verify customer-owned hostnames for branded access, Staff Portal and external organization entry.",
  order: 15,
  status: "active",
  type: "business-workspace",
  document: "OrganizationHostname",
};

const PasskeyReadinessWorkspace = {
  id: "passkey_rollout_readiness",
  name: "Passkey Rollout Readiness",
  route: "/administration/passkey-readiness",
  description:
    "Verify hosted Passkey availability, staff Auth linkage, enrollment coverage and recent WebAuthn proof before mandatory clock-in activation.",
  order: 20,
  status: "active",
  type: "business-workspace",
  document: "WorkforcePasskeyReadiness",
};

function canonicalizeAdministrationRoute(route) {
  const value = String(route || "").trim();

  if (value === LEGACY_SETTINGS_ROUTE) {
    return ADMINISTRATION_ROUTE;
  }

  if (value.startsWith(`${LEGACY_SETTINGS_ROUTE}/`)) {
    return `${ADMINISTRATION_ROUTE}${value.slice(LEGACY_SETTINGS_ROUTE.length)}`;
  }

  return value;
}

function canonicalizeAdministrationDomain(registry) {
  const domain = (registry?.domains || []).find(
    (item) => item?.id === "administration"
  );

  if (domain) {
    domain.route = ADMINISTRATION_ROUTE;
  }
}

function canonicalizeAdministrationItems(administration) {
  for (const group of administration?.groups || []) {
    for (const item of group?.items || []) {
      if (item?.route) {
        item.route = canonicalizeAdministrationRoute(item.route);
      }
    }
  }
}

function addPayrollSetup(administration) {
  const groups = administration.groups || [];
  const existingGroup = groups.find((group) => group?.id === ONBOARDING_GROUP_ID);

  if (existingGroup) {
    const items = existingGroup.items || [];

    const onboardingItems = [OrganizationSetupWorkspace, PayrollSetupWorkspace];
    const additions = onboardingItems.filter((workspace) => !items.some((item) => item?.id === workspace.id));
    if (additions.length) {
      existingGroup.items = [...items, ...additions].sort(
        (left, right) => Number(left?.order || 0) - Number(right?.order || 0)
      );
    }

    return;
  }

  administration.groups = [
    ...groups,
    {
      id: ONBOARDING_GROUP_ID,
      name: "Onboarding & Setup",
      description:
        "Prepare organization configuration and operational readiness before go-live.",
      order: 1,
      items: [OrganizationSetupWorkspace, PayrollSetupWorkspace],
    },
  ].sort((left, right) => Number(left?.order || 0) - Number(right?.order || 0));
}

function addOrganizationPolicy(administration) {
  const groups = administration.groups || [];
  const existingGroup = groups.find((group) => group?.id === ACCESS_GROUP_ID);
  const accessItems = [OrganizationPolicyWorkspace, DomainsWorkspace, PasskeyReadinessWorkspace, ComputeWorkspace, ProductControlWorkspace];

  if (existingGroup) {
    const items = existingGroup.items || [];
    const additions = accessItems.filter(
      (workspace) => !items.some((item) => item?.id === workspace.id)
    );

    if (additions.length) {
      existingGroup.items = [...items, ...additions].sort(
        (left, right) => Number(left?.order || 0) - Number(right?.order || 0)
      );
    }
    return;
  }

  administration.groups = [
    ...groups,
    {
      id: ACCESS_GROUP_ID,
      name: "Access & Security",
      description:
        "Control organization entry, portal availability, identity verification and workforce clock-in location policy.",
      order: 2,
      items: accessItems,
    },
  ].sort((left, right) => Number(left?.order || 0) - Number(right?.order || 0));
}

export function applyAdministrationWorkspaceRegistry(registry) {
  const administration = registry?.workspaces?.administration;
  if (!administration) return registry;

  addPayrollSetup(administration);
  addOrganizationPolicy(administration);
  canonicalizeAdministrationDomain(registry);
  canonicalizeAdministrationItems(administration);

  return registry;
}

export default applyAdministrationWorkspaceRegistry;
