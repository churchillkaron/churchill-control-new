...
export function createCodeAIAutonomousCapability() {
  ...
  async function execute({ context, payload = {} }) {
    ...
    const emitCapabilityProgress = async (phase, description) => {
      await publishCodeAILiveProgress({
        context,
        state: {
          mission_id: missionId,
          objective: text(payload.objective, 1200) || null,
          repository_url: text(payload.repository_url, 1000) || null,
          ref: text(payload.ref, 160) || "main",
          status: "running",
          device_id: requestedDeviceIdForProgress,
          device_session_id: requestedDeviceSessionIdForProgress,
          objective_context: {
            organization_id: text(context?.organizationId, 160) || null,
            workspace_target: text(payload.workspace_target, 80) || null,
            device_id: requestedDeviceIdForProgress,
            device_session_id: requestedDeviceSessionIdForProgress,
            mission_id: missionId,
          },
    ...
    const resumedExecution = Boolean(
      payload.resume_existing_mission === true ||
      (resumeState && Object.keys(object(resumeState)).length)
    );
    const emitSetupProgress = resumedExecution
      ? async () => null
      : emitCapabilityProgress;
