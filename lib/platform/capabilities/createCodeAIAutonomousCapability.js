...
export function createCodeAIAutonomousCapability() {
  const manifest = defineCapability({
    ...
    action: "execute",
    description:
      "Run Avantiqo Code as an employee-style software engineer. The public capability preserves the existing permission, attestation, server-owned verification evidence and separately governed commit boundaries, while execution uses bounded multi-operation work packages instead of one paid model call per tiny repository action. Code inspects current repository evidence, batches coherent reads/searches and multi-file implementation, deterministically verifies the result, repairs only when evidence changes, and continues until Product-selected completion criteria and risk-scaled world-class quality are proven or a genuine blocker remains. Default reasoning spend is capped by the Code employee runtime; persistent GitHub commits remain a separate governed capability.",
