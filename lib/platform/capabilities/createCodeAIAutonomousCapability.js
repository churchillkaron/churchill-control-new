...
} from "@/lib/code/runtime/CodeAIInteractivePreviewContextRuntime";
import {
...
import {
  isCodeAICommitArtifactEligible,
  persistCodeAICommitArtifact,
} from "@/lib/code/runtime/CodeAICommitArtifactRuntime";
import {
  extractCodeAIExplicitRepositoryPaths,
} from "@/lib/code/runtime/CodeAIReadOnlyIntentRuntime";
import {
...
} from "@/lib/code/runtime/CodeAIOwnerVerificationCommandRuntime";
import {
...
function authoritativeVerificationBinding(baseContext = {}, objective = "") {
  const resolved = resolveCodeAIOwnerVerificationCommand({
    objective,
    objective_context: baseContext,
  });
  return resolved
    ? {
        authoritative_verification_command: resolved.command,
        authoritative_verification_args: resolved.args,
        authoritative_verification_source: resolved.source,
      }
    : {};
}
...
export function createCodeAIAutonomousCapability() {
      ...
      properties: {
        ...
        objective_context: {
          ...
          properties: {
            ...
            authoritative_verification_command: { type: "string", maxLength: 300 },
            authoritative_verification_args: {
              type: "array",
              maxItems: 24,
        ...
        local_compute_required: {
          ...
          default: false,
          description:
            "Restriction-only compute policy. When true, Code reasoning must execute on Avantiqo-owned local compute and must fail closed if local compute is unavailable. It grants no execution, commit, deployment, migration, publication, or secret authority.",
        },
        infrastructure_policy: {
          type: "string",
          enum: ["local_only"],
          description:
            "Optional explicit local-only infrastructure policy. It can restrict compute but never grant authority.",
        },
        workspace_target: {
          type: "string",
  ...
  async function execute({ context, payload = {} }) {
    const missionId = text(payload.objective_context?.mission_id, 240) || null;
    const requestedDeviceIdForProgress = text(payload.device_id, 160) || null;
    const requestedDeviceSessionIdForProgress = text(payload.device_session_id, 160) || null;

    assertResumeScope(payload.resume_state, context);
    const resumeState = resumeStateForExecution(payload.resume_state);
    // Setup/control assembly is internal plumbing, not useful customer narration.
    // Talk should begin with concrete repository evidence or explicit recovery.
    const emitSetupProgress = async () => null;
    await emitSetupProgress(
      "CAPABILITY_CONTEXT_RESOLVING",
      "I’m resolving the current Code mission context before I touch the repository.",
    );
    ...
    await emitSetupProgress(
      "CAPABILITY_CONTEXT_READY",
      "The mission context is ready. I’m preparing the engineering controls for the local repository pass.",
    );
    const missionInput = object(unified.mission_context);
    await emitSetupProgress(
      "CAPABILITY_BINDING_PREPARING",
      "I’m binding the resolved mission to this exact local repository session.",
    );
    ...
    await emitSetupProgress(
      ...
      "The mission is bound to the local repository session. I’m preparing the engineering operating controls.",
    );
    ...
    const ownerNamedEvidencePaths = extractCodeAIExplicitRepositoryPaths(
      payload.owner_intent || payload.objective,
      4,
    );
    const baseExecutionObjectiveContext = {
      ...baseObjectiveContext,
      ...authoritativeVerificationBinding(baseObjectiveContext, payload.objective),
      evidence_path_1: text(baseObjectiveContext.evidence_path_1, 1000) || ownerNamedEvidencePaths[0] || null,
      evidence_path_2: text(baseObjectiveContext.evidence_path_2, 1000) || ownerNamedEvidencePaths[1] || null,
      evidence_path_3: text(baseObjectiveContext.evidence_path_3, 1000) || ownerNamedEvidencePaths[2] || null,
      evidence_path_4: text(baseObjectiveContext.evidence_path_4, 1000) || ownerNamedEvidencePaths[3] || null,
    ...
    await emitSetupProgress(
      "ENGINEERING_OS_PREPARING",
      "I’m building the engineering controls for this task from the current objective and workspace state.",
    );
    ...
    await emitSetupProgress(
      "ENGINEERING_OS_READY",
      "The engineering controls are ready. I’m preparing the precision and verification controls next.",
    );
    ...
    await emitSetupProgress(
      "PRECISION_OS_READY",
      "The precision controls are ready. I’m assembling the bounded Code objective for the local employee.",
    );
    ...
    const executionObjectiveContext = {
      ...
      organization_id: text(context?.organizationId, 160) || null,
      workspace_target: requestedWorkspaceTarget || text(engineeringOSPrepared.objective_context?.workspace_target, 80) || null,
      device_id: requestedDeviceId || text(engineeringOSPrepared.objective_context?.device_id, 160) || null,
      device_session_id: requestedDeviceSessionId || text(engineeringOSPrepared.objective_context?.device_session_id, 160) || null,
    };

    await emitSetupProgress(
      "CODE_EMPLOYEE_TRANSPORT_RESOLVING",
      "I’m selecting the local Code execution transport for the attached workspace.",
    );
    const transport = resolveCodeAIEmployeeExecutionTransport();
    await emitSetupProgress(
      "CODE_EMPLOYEE_STARTING",
      "The engineering controls are ready. I’m starting the local Code employee on the attached workspace now.",
    );
    const result = await executeCanonicalCodeAIEmployeeMission({
      ...
      local_compute_required:
        ...
        text(payload.infrastructure_policy, 40).toLowerCase() === "local_only",
      infrastructure_policy:
        payload.local_compute_required === true ||
        text(payload.infrastructure_policy, 40).toLowerCase() === "local_only"
          ? "local_only"
    ...
    if (payload.execution_key && result?.state) {
      const persisted = await persistCodeAIAutonomousExecutionState({
        context,
        executionKey: payload.execution_key,
        result,
      });
      const commitEligible = isCodeAICommitArtifactEligible(result.state);
      const commitArtifact = commitEligible
      ...
      result.verification_evidence = {
        ...
        commit_artifact_eligible: commitEligible,
