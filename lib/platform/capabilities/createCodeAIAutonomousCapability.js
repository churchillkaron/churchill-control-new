import { executeWorldClassCodeMission } from "@/lib/code/runtime/CodeAIWorldClassRuntime";
import {
...
export function createCodeAIAutonomousCapability() {
  const manifest = defineCapability({
    ...
    action: "execute",
    description:
      "Run an Avantiqo-owned autonomous software-engineering mission with mandatory world-class completion quality: inspect the repository, plan from observed evidence, use shared governed research when needed, edit through bounded source operations, execute development checks, inspect failures, repair, verify after the final edit, explicitly review the final diff, and continue until complete or genuinely blocked. Risk-sensitive completion requires fresh verification evidence after the latest source revision; high/critical changes require two distinct fresh verification gates. Optional objective_context carries bounded Product Intelligence selection provenance and up to six Product-selected evidence-verifiable completion criteria. This context has no authorization effect; Code AI must independently inspect current main and must satisfy every bound completion criterion with observed mission evidence before completion is accepted. Persistent GitHub commits remain a separate governed capability. When execution_key is supplied, the attested outcome is persisted both as compact server-owned verification evidence and as a non-recallable full commit artifact that can only be consumed later by the separately governed Code AI commit capability.",
  ...
  async function execute({ context, payload = {} }) {
    ...
    const result = await executeWorldClassCodeMission({
      context,
