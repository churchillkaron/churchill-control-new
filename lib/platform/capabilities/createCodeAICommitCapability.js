...
async function recoverPriorAttempt(artifact) {
  ...
  try {
    ...
    if (text(error?.message) === RECOVERY_NOT_FOUND) return null;
    throw error;
  }
}
...
export function createCodeAICommitCapability() {
  const manifest = defineCapability({
    ...
    action: "execute",
    description:
      "Atomically commit a completed and verified Avantiqo Code AI mission to the allowed GitHub repository main branch. Preferred execution uses the opaque Code AI execution_key to load the full attested source artifact server-side, so patches never need to travel through Intelligence or mission bindings. Before the first GitHub side effect, the artifact records a server-owned commit-attempt marker. If a later invocation sees a prior attempt, it performs bounded read-only recovery first and heals server verification state when the exact commit already exists instead of replaying the write. A genuinely absent prior commit may be retried only after that recovery check, and exact-base/no-force protection still applies. The action requires the dedicated commit permission plus explicit confirmation and post-commit branch/parent/tree verification. Once GitHub persistence is independently verified, later server-state housekeeping cannot turn that external write into a generic retryable failure.",
