...
export function createCodeAICommitStatusCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Verify a governed Code AI GitHub commit without ever replaying the write. The normal path reads the server-owned commit verification record. If that record is missing after a commit was actually attempted, the verifier may use the retained attested Code AI commit artifact only when its server-owned pre-write attempt marker is present, then perform bounded read-only recovery against recent history reachable from current main, requiring the exact base parent, exact changed-path set and exact resulting file contents. An unattempted artifact never triggers GitHub recovery. This read never commits, deploys, edits source, repairs database state, or changes authorization.",
