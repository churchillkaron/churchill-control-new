...
function assertResumeScope(resumeState, context) {
  ...
  if (expectedActorId && expectedActorId !== currentActorId) {
...
export function createCodeAIMissionCapability() {
  const manifest = defineCapability({
    ...
    action: "execute",
    description:
      "Run a bounded, isolated Avantiqo Code AI repository mission. It can inspect, search, read, edit an ephemeral workspace, run development commands, test, preserve repair state, and return a verified patch. It cannot push, deploy, publish, or execute database tooling.",
    permissions: [REQUIRED_PERMISSION],
    events: [],
    tags: ["platform", "code-ai", "repository", "mission", "sandbox", "development"],
    transactional: false,
    aiEnabled: true,
