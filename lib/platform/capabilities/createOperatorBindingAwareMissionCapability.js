...
import {
  recordBusinessPartnerMissionProductEvidence,
...
export function createOperatorBindingAwareMissionCapability() {
  ...
  async function execute({ context, payload = {} }) {
    ...
    const productEvidenceObservationToken = randomBytes(32).toString("hex");
    const result = await base.execute({ context, payload });

    result.product_evidence = await recordBusinessPartnerMissionProductEvidence({
      context,
      payload,
      result,
      observation_token: productEvidenceObservationToken,
    });
