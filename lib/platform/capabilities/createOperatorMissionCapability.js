...
function normalizeExternalWait(value, payload = {}) {
  ...
  if (!eventSource && !eventType && !correlationKey) return null;
  if (!eventSource || !eventType || !correlationKey) {
    return { error: "OPERATOR_MISSION_EXTERNAL_WAIT_CONTRACT_INCOMPLETE" };
  }
  return { event_source: eventSource, event_type: eventType, correlation_key: correlationKey };
}
...
function blocked({ preflight, steps, completedIds, currentStepId, reason, detail = null, results = [], failureEvidence = null }) {
  return {
    status: "blocked",
    reason,
    detail,
        ...
        if (text(wait?.status).toUpperCase() !== "EVENT_RECEIVED") {
          return paused({
            ...
            externalWait: {
              ...
              event_source: text(step.wait_for_external?.event_source),
              event_type: text(step.wait_for_external?.event_type),
              correlation_key: text(step.wait_for_external?.correlation_key),
