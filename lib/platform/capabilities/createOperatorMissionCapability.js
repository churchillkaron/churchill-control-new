...
import { deterministicBusinessEffectProof, collectStableBusinessIdentities } from "@/lib/operator/runtime/OperatorDeterministicBusinessEffectRuntime";
import { normalizeAuthoritativeBusinessEffectOutcome } from "@/lib/operator/runtime/BusinessEffectOutcomeRuntime.mjs";
import { normalizedActionIdentityEvidence, verifierPayloadFromActionIdentityEvidence } from "@/lib/operator/runtime/ActionIdentityEvidenceRuntime.mjs";
import {
...
const SPECIAL_GOVERNED_MISSION_VERIFIERS = Object.freeze({
  "platform.code_ai_commit.execute": Object.freeze({
    capability_key: "platform.code_ai_commit_status.verify",
    payload_from_input: Object.freeze({ execution_key: "execution_key" }),
    derivation: "SERVER_OWNED_SPECIAL_GOVERNED_CODE_COMMIT_VERIFIER",
  }),
});

function specialGovernedMissionVerification(targetKey, stepPayload = {}) {
  const declaration = SPECIAL_GOVERNED_MISSION_VERIFIERS[targetKey];
  if (!declaration) return null;
  const payload = {};
  for (const [destination, source] of Object.entries(declaration.payload_from_input || {})) {
    const value = object(stepPayload)[source];
    if (!["string", "number"].includes(typeof value) || !text(value)) return null;
    payload[destination] = value;
  }
  return {
    capability_key: declaration.capability_key,
    description: "Verify the server-owned special-governed business effect",
    payload,
...
function valueAtSafePath(source, path) {
  const parts = text(path).split(".");
  if (!parts.length || parts.length > 8 || parts.some((part) => !/^[A-Za-z0-9_]+$/.test(part) || ["__proto__","prototype","constructor"].includes(part))) return null;
  let current = source;
...
function bindCatalogVerification(declaration = {}, actionResult = null, actionPayload = {}) {
  const capabilityKey = text(declaration.capability_key);
  if (!capabilityKey) return null;
  const payload = {};
  for (const key of list(declaration.payload_keys).map(text).filter(Boolean)) {
    if (!Object.prototype.hasOwnProperty.call(object(actionPayload), key)) return null;
    payload[key] = actionPayload[key];
  }
  for (const [payloadKey, rawPaths] of Object.entries(object(declaration.payload_from_result))) {
    const paths = list(rawPaths).length ? list(rawPaths) : [rawPaths];
    const value = paths.map((path) => valueAtSafePath(actionResult, path)).find((candidate) => ["string","number"].includes(typeof candidate) && text(candidate));
    if (value === undefined) return null;
    payload[payloadKey] = value;
  }
  for (const [payloadKey, rawConfig] of Object.entries(object(declaration.payload_array_from_result))) {
    const config = object(rawConfig);
    const source = valueAtSafePath(actionResult, config.path);
    if (!Array.isArray(source) || source.length > 500 || (!source.length && config.allow_empty !== true)) return null;
    const values = source.map((item) => text(config.item_path) ? valueAtSafePath(item, config.item_path) : item);
    if (values.some((value) => !["string","number"].includes(typeof value) || !text(value))) return null;
    payload[payloadKey] = values;
  }
  for (const [payloadKey, sourceKeyRaw] of Object.entries(object(declaration.payload_from_input))) {
    const sourceKey = text(sourceKeyRaw);
    ...
    if (!["string","number","boolean"].includes(typeof value) && !Array.isArray(value)) return null;
    if (Array.isArray(value) && (!value.length || value.length > 500)) return null;
    payload[payloadKey] = value;
  }
  return { capability_key: capabilityKey, description: "Verify the authoritative business effect", payload };
}
...
async function preflightStep(step, context) {
  ...
  if (mode !== "read" && !externalWait) {
    ...
    const catalogAction = catalog.find((item) => item.key === target.key);
    catalogVerification = object(catalogAction?.operator_verification);
    const registeredKey = text(catalogVerification.capability_key);
    const specialGovernedVerification = registeredKey
      ? null
      : specialGovernedMissionVerification(target.key, step.payload);
    const requestedVerification = registeredKey
      ? { capability_key: registeredKey, description: "Verify the authoritative business effect", payload: {} }
      : specialGovernedVerification;
    if (!requestedVerification) {
      return { ok: false, reason: "OPERATOR_MISSION_SERVER_VERIFICATION_REQUIRED", step };
    }
    verification = await preflightRead(requestedVerification, context);
...
async function executeVerification(entry, context, resolvedVerification = null) {
  const verification = entry.verification;
  const effective = resolvedVerification || verification.verification;
  const payload = scopedPayload(context, effective.payload);
  const result = await executeUbteCapability({
    organizationId: context.organizationId,
    ...
    runtime: childRuntime(
      context,
      { id: entry.step.id, capability_key: effective.capability_key },
...
function missionStepFailureEvidence(error, entry, phase) {
  ...
  return {
    ...
    action_identity_evidence: normalizedActionIdentityEvidence(error?.action_identity_evidence),
    verification_recovery: (() => {
      const declaration = object(entry?.catalog_verification);
      const capabilityKey = text(declaration.capability_key);
      const payload = verifierPayloadFromActionIdentityEvidence(declaration, error?.action_identity_evidence);
      return capabilityKey && payload ? { capability_key: capabilityKey, payload, authorization_effect: "NONE" } : null;
    })(),
  ...
  async function execute({ context, payload = {} }) {
    ...
    for (
      ...
      if (verificationPending) {
        try {
          const verificationResult = await executeVerification(entry, context, verificationPending);
          const proof = verificationPending.proof_required === true
            ? deterministicBusinessEffectProof({ result: null, action_identity_evidence: list(verificationPending.action_identity_evidence), post_action_verification: { status: "completed", result: verificationResult } })
            : { passed: true, matched_identity: null };
          const normalizedOutcome = verificationPending.proof_required === true
            ? normalizeAuthoritativeBusinessEffectOutcome({
                result: verificationResult,
                expected: { action_identity_evidence: list(verificationPending.action_identity_evidence) },
            ...
            : {
                ...
                derivation: "SERVER_OWNED_SPECIAL_GOVERNED_VERIFICATION_READ",
              };
          const businessEffectOutcome = proof.passed === true
            ? { ...normalizedOutcome, state: "COMPLETED", authoritative_server_evidence: true, exact_business_scope_matched: true, business_effect_observed: true, business_effect_absent: false, safe_to_retry: false, matched_identity: proof.matched_identity || normalizedOutcome.matched_identity || null }
            : { ...normalizedOutcome, state: "UNCERTAIN", business_effect_observed: false, business_effect_absent: false, safe_to_retry: false };
          if (businessEffectOutcome.state !== "COMPLETED") throw Object.assign(new Error(proof.reason || "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED"), { code: "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED" });
          results.push({ id: step.id, capability_key: step.capability_key, status: "verification_completed", verification: bounded(verificationResult), business_effect_verified: true, business_effect_outcome: businessEffectOutcome });
          completedIds.push(step.id);
          verificationPending = null;
          currentStepConfirmed = false;
          approvalRequestId = null;
      ...
      const resolvedVerification = Object.keys(object(entry.catalog_verification)).length
        ? bindCatalogVerification(entry.catalog_verification, action.result, step.payload)
        : entry.verification.verification;
      if (!resolvedVerification) {
        return blocked({ preflight, steps: normalized.steps, completedIds, currentStepId: step.id, reason: "OPERATOR_MISSION_VERIFICATION_BINDING_UNRESOLVED", results, runId: missionRunId });
      }
      const actionIdentityEvidence = Array.from(collectStableBusinessIdentities(action.result)).slice(0, 50);
      try {
        const verificationResult = await executeVerification(entry, context, resolvedVerification);
        const proofRequired = Object.keys(object(entry.catalog_verification)).length > 0;
        const proof = proofRequired
          ? deterministicBusinessEffectProof({ result: action.result, action_identity_evidence: actionIdentityEvidence, post_action_verification: { status: "completed", result: verificationResult } })
          : { passed: true, matched_identity: null };
        const normalizedOutcome = proofRequired
          ? normalizeAuthoritativeBusinessEffectOutcome({
              result: verificationResult,
              expected: { action_result: action.result, action_identity_evidence: actionIdentityEvidence },
              server_scope_bound: true,
            })
          : {
              contract: "AVANTIQO_AUTHORITATIVE_BUSINESS_EFFECT_OUTCOME_V1",
        ...
        const businessEffectOutcome = proof.passed === true
          ...
          : { ...normalizedOutcome, state: "UNCERTAIN", business_effect_observed: false, business_effect_absent: false, safe_to_retry: false };
        if (businessEffectOutcome.state !== "COMPLETED") throw Object.assign(new Error(proof.reason || "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED"), { code: "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED" });
        results.push({ id: step.id, capability_key: step.capability_key, status: "completed", verification: bounded(verificationResult), business_effect_verified: true, business_effect_outcome: businessEffectOutcome });
        completedIds.push(step.id);
        currentStepConfirmed = false;
        approvalRequestId = null;
        verificationPending = null;
        ...
        verificationPending = {
          step_id: step.id,
          capability_key: resolvedVerification.capability_key,
          description: resolvedVerification.description,
          payload: resolvedVerification.payload,
          action_identity_evidence: actionIdentityEvidence,
          proof_required: Object.keys(object(entry.catalog_verification)).length > 0,
          business_effect_outcome: {
            contract: "AVANTIQO_AUTHORITATIVE_BUSINESS_EFFECT_OUTCOME_V1",
            state: "UNCERTAIN",
            authoritative_server_evidence: false,
            exact_business_scope_matched: false,
            business_effect_observed: false,
            business_effect_absent: false,
            safe_to_retry: false,
            matched_identity: null,
            derivation: "MISSION_VERIFICATION_PENDING",
          },
        };
        return paused({
          preflight,
