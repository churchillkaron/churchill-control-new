    ...
    for (
      ...
      if (verificationPending) {
        try {
          ...
          const proof = verificationPending.proof_required === true
            ...
            : { passed: true };
          if (!proof.passed) throw Object.assign(new Error(proof.reason || "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED"), { code: "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED" });
          results.push({ id: step.id, capability_key: step.capability_key, status: "verification_completed", verification: bounded(verificationResult), business_effect_verified: true });
          completedIds.push(step.id);
          verificationPending = null;
          currentStepConfirmed = false;
          approvalRequestId = null;
      ...
      try {
        ...
        const proof = proofRequired
          ...
          : { passed: true };
        if (!proof.passed) throw Object.assign(new Error(proof.reason || "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED"), { code: "OPERATOR_MISSION_BUSINESS_EFFECT_UNVERIFIED" });
        results.push({ id: step.id, capability_key: step.capability_key, status: "completed", verification: bounded(verificationResult), business_effect_verified: true });
        completedIds.push(step.id);
        currentStepConfirmed = false;
        approvalRequestId = null;
        verificationPending = null;
