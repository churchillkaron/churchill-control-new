...
function hasApprovalPolicy(manifest = {}) {
  return Boolean(
    manifest.approval ||
...
function normalizeSteps(payload = {}) {
  ...
  if (steps.some((step) => seenIds.has(step.id) || !seenIds.add(step.id))) {
    return {
...
async function preflightStep(step, context) {
  ...
  if (mode !== "read") {
    ...
    if (hasApprovalPolicy(manifest)) {
      return {
        ok: false,
        reason: "OPERATOR_MISSION_APPROVAL_POLICY_STEP_BLOCKED",
        step,
      };
    }
  }
...
export function createOperatorMissionCapability() {
  const manifest = defineCapability({
    ...
    action: "execute",
    description:
      "Run one bounded autonomous mission containing 2 to 6 independent registered operations in order. Use this only when the user's explicit command requires several steps whose exact payloads are already known before execution. Every step is preflighted before the first side effect. Reads are allowed. Non-read steps are allowed only when their own capability is low-risk, explicitly auto-executable, confirmation-free, approval-free, Operator-enabled, and permitted for the current organization/entity context. Use ordinary evidence-first read-chain reasoning instead when a later action depends on what an earlier read discovers. The mission stops on the first runtime failure and never retries external effects automatically.",
    permissions: [],
    events: [],
    tags: [
      "platform",
    ...
    inputSchema: {
      ...
      properties: {
        steps: {
          ...
          maxItems: MAX_STEPS,
          description:
            "Ordered independent registered operations. Every payload must be complete before the mission starts; later steps cannot depend on earlier results.",
          items: {
            type: "object",
  ...
  async function execute({ context, payload = {} }) {
    ...
    for (const entry of preflight) {
      const { step, target, contract } = entry;
      try {
