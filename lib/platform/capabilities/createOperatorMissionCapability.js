...
function normalizeResume(payload, steps, context) {
  ...
  const completed = list(resume.completed_step_ids)
    .map(text)
  ...
  return {
    completed_step_ids: Array.from(new Set(completed)),
    current_step_id: currentStepId,
    current_step_confirmed: resume.current_step_confirmed === true,
    approval_request_id: text(resume.approval_request_id) || null,
    verification_pending: text(verification.step_id) === currentStepId ? verification : null,
  };
}
...
function resumePayload({ steps, completedIds, currentStepId, confirmed, approvalRequestId = null, verificationPending = null }) {
  return {
    steps,
    resume: {
      completed_step_ids: completedIds,
...
function paused({ preflight, steps, completedIds, currentStepId, pauseReason, confirmed, results, blocker, approvalRequest = null, verificationPending = null }) {
  const status =
...
export function createOperatorMissionCapability() {
  ...
  async function execute({ context, payload = {} }) {
    ...
    const resume = normalizeResume(payload, normalized.steps, context);
    if (resume.error) {
      return blocked({
        preflight,
