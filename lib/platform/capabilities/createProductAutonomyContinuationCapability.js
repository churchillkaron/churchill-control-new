...
const RECOVERY_VERIFICATION_SOURCE = "GITHUB_RECOVERY_FROM_ATTESTED_ARTIFACT";

function text(value, limit = 4000) {
  return String(value ?? "").trim().slice(0, limit);
}

function trustedRecoveryBindingContext(context = {}) {
...
export function createProductAutonomyContinuationCapability() {
  const manifest = defineCapability({
    ...
    description:
      "After a governed Code AI commit has been independently verified on main, clone the actual current GitHub main branch into Avantiqo's read-only Code AI workspace and let Avantiqo-owned Product Intelligence select exactly one next bounded engineering objective from that repository evidence. Normal continuation requires the server-owned commit verification state. If that state was unavailable after a GitHub write already succeeded, this capability may accept only the exact scalar commit evidence bound by the registered commit verifier inside the durable reassess_verified_main mission step, and only while the matching attested commit artifact remains retained with its server-owned pre-write commit-attempt marker. The verified commit is context only; concurrent main progress is preserved and reported. This capability never starts Code AI, commits again, deploys production, applies database migrations, publishes, or creates an unbounded self-modification loop.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        verified_commit_sha: {
          ...
          description:
            "Internal durable-mission binding from the registered commit verifier. It has no standalone authorization effect and is ignored as recovery evidence outside the exact trusted continuation step.",
        },
        verified_commit_verification_source: {
          type: "string",
          maxLength: 120,
          description:
            "Internal durable-mission binding identifying the registered verifier evidence source.",
        },
        verified_commit_server_state_found: {
          type: "boolean",
          description:
            "Internal durable-mission binding indicating whether the normal server-owned commit execution state existed.",
        },
      },
  ...
  async function execute({ context, payload = {} }) {
    ...
    let verifiedCommit = loaded.found && loaded.commit?.verified === true
      ? loaded.commit
      : null;
    let verificationEvidenceSource = verifiedCommit
      ? "SERVER_OWNED_COMMIT_EXECUTION_STATE"
      : null;

    if (verifiedCommit) {
      if (text(verifiedCommit.branch, 160) !== "main") {
        throw new Error("PRODUCT_AUTONOMY_CONTINUATION_MAIN_COMMIT_REQUIRED");
      }
      const boundSha = text(payload.verified_commit_sha, 40);
      if (boundSha && boundSha !== text(verifiedCommit.commit_sha, 40)) {
        throw new Error("PRODUCT_AUTONOMY_CONTINUATION_BOUND_COMMIT_MISMATCH");
      }
    } else {
      const boundSha = text(payload.verified_commit_sha, 40);
      const boundSource = text(payload.verified_commit_verification_source, 120);
      if (
        !trustedRecoveryBindingContext(context) ||
        !COMMIT_SHA_PATTERN.test(boundSha) ||
        boundSource !== RECOVERY_VERIFICATION_SOURCE ||
      ...
      if (!artifact.found || !artifact.mission_state) {
        throw new Error("PRODUCT_AUTONOMY_CONTINUATION_RECOVERY_ARTIFACT_REQUIRED");
      }
      ...
      if (text(artifact.mission_state.ref, 160) !== "main") {
        throw new Error("PRODUCT_AUTONOMY_CONTINUATION_RECOVERY_ARTIFACT_MAIN_REQUIRED");
      }
      ...
      verifiedCommit = {
        ...
        branch: "main",
        commit_sha: boundSha,
        previous_commit: text(artifact.mission_state.base_commit, 160) || null,
        repository: repositoryNameFromUrl(artifact.mission_state.repository_url),
      ...
      verificationEvidenceSource = RECOVERY_VERIFICATION_SOURCE;
    }

    const verifiedCommitSha = text(verifiedCommit?.commit_sha, 160);
    ...
    const commitContext = [
      ...
      `Commit verification source: ${verificationEvidenceSource}.`,
    ...
    return {
      ...
      commit_verification_source: verificationEvidenceSource,
