...
export function createProductEngineeringCycleCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Run one bounded Avantiqo-owned product-owner-to-engineering cycle through the durable Operator mission engine: assess the highest-impact autonomy gap against the Product Constitution, bind only the resulting bounded engineering objective into Code AI, let Code AI work and verify locally, independently verify the attested engineering outcome, then let Product Intelligence decide whether the verified result should remain local or request separately governed commit confirmation. By default no commit occurs. When commit_message is explicitly supplied by the caller, the same durable mission may append the existing separately governed Code AI commit action; that action still requires its own permission, confirmation/approval gates, exact-base protection and registered post-write verification. Production deployment, database migrations, publication and governance bypass remain outside this cycle.",
    ...
    operatorExamples: [
      ...
      "Find what Avantiqo needs next, let Code AI implement and verify it locally, then prepare governed persistence only when Product Intelligence recommends it.",
    ],
    transactional: false,
    aiEnabled: true,
