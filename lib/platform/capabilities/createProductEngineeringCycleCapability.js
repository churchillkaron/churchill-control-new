  ...
  return [
    ...
    },
    {
      ...
      payload: {
        ...
        ...(timeoutMs ? { timeout_ms: timeoutMs } : {}),
        ...(ownerConstraints.length ? { objective_context: { owner_constraints: ownerConstraints } } : {}),
      },
      bindings: [
        {
        ...
        },
        {
          source_step_id: "assess_repository",
          source: "result",
          source_path: "next_engineering_handoff.product_research_performed",
        ...
        },
        {
          source_step_id: "assess_repository",
          source: "result",
          source_path: "next_engineering_handoff.market_advantage_required",
        ...
        },
        {
          source_step_id: "assess_repository",
          source: "result",
          source_path: "next_engineering_handoff.market_advantage_criterion_bound",
...
function missionBindingValue(missionResult, sourcePath) {
  const values = object(missionResult?.binding_state?.values);
  const key = `assess_repository:result:${text(sourcePath, 500)}`;
...
function repositoryAssessment(missionResult) {
  const boundedAssessment = missionStepCapabilityResult(
    missionResult,
    "assess_repository",
  );
  const boundedSelection = object(boundedAssessment.objective_selection);
  const boundedHandoff = object(boundedAssessment.next_engineering_handoff);
  const boundedSnapshot = object(boundedAssessment.repository_snapshot);

  const repositoryHeadObserved =
    text(
      missionBindingValue(
        missionResult,
        "next_engineering_handoff.repository_head_observed",
      ),
      160,
    ) ||
    text(boundedHandoff.repository_head_observed, 160) ||
    text(boundedSnapshot.current_main_head, 160) ||
    null;
  const selectedObjective =
    text(
      missionBindingValue(missionResult, "next_engineering_handoff.focus"),
      4000,
    ) ||
    text(boundedHandoff.focus, 4000) ||
    text(boundedSelection.selected_objective, 4000) ||
    null;
  const selectionContract =
    text(
      missionBindingValue(
        missionResult,
        "next_engineering_handoff.objective_selection_contract",
      ),
      240,
    ) ||
    text(boundedHandoff.objective_selection_contract, 240) ||
    text(boundedSelection.contract, 240) ||
    null;
  const selectedCandidateId =
    text(
      missionBindingValue(
        missionResult,
        "next_engineering_handoff.selected_candidate_id",
      ),
      240,
    ) ||
    text(boundedHandoff.selected_candidate_id, 240) ||
    text(boundedSelection.selected_candidate_id, 240) ||
    null;
  const boundSelectionScore = missionBindingValue(
    missionResult,
    "next_engineering_handoff.objective_selection_score",
  );
  const selectionScore =
    boundSelectionScore !== undefined
      ? boundSelectionScore
      : boundedHandoff.objective_selection_score ??
        boundedSelection.selected_weighted_score;
  const boundEvidenceBacked = missionBindingValue(
    missionResult,
    "next_engineering_handoff.objective_selection_evidence_backed",
  );
  const evidenceBacked =
    typeof boundEvidenceBacked === "boolean"
      ? boundEvidenceBacked
      : typeof boundedHandoff.objective_selection_evidence_backed === "boolean"
        ? boundedHandoff.objective_selection_evidence_backed
        : typeof boundedSelection.evidence_backed === "boolean"
          ? boundedSelection.evidence_backed
          : undefined;
  const boundCompletionCriteria = Array.from({ length: 6 }, (_, index) =>
    text(
      missionBindingValue(
        missionResult,
        `objective_selection.selected_completion_criteria.${index}`,
      ),
      4000,
    ),
  ).filter(Boolean);
  const boundedCompletionCriteria = list(
    boundedSelection.selected_completion_criteria,
  )
    .map((criterion) => text(criterion, 4000))
    .filter(Boolean)
    .slice(0, 6);
  const selectedCompletionCriteria = boundCompletionCriteria.length
    ? boundCompletionCriteria
    : boundedCompletionCriteria;
  const bindingState = object(missionResult?.binding_state);

  return {
    ...boundedAssessment,
    ...(repositoryHeadObserved
      ? {
          repository_snapshot: {
            ...boundedSnapshot,
            current_main_head: repositoryHeadObserved,
          },
        }
      : {}),
    objective_selection: {
      ...
      ...(selectedCandidateId
        ...
        : {}),
      ...(selectedObjective ? { selected_objective: selectedObjective } : {}),
      ...(selectionScore !== undefined
        ? { selected_weighted_score: selectionScore }
        : {}),
      ...(typeof evidenceBacked === "boolean" ? { evidence_backed: evidenceBacked } : {}),
      ...(selectedCompletionCriteria.length
        ? { selected_completion_criteria: selectedCompletionCriteria }
        : {}),
    },
    next_engineering_handoff: {
      ...boundedHandoff,
      ...(selectedObjective ? { focus: selectedObjective } : {}),
      ...(repositoryHeadObserved
        ? { repository_head_observed: repositoryHeadObserved }
        : {}),
      ...(selectionContract
        ? { objective_selection_contract: selectionContract }
        : {}),
      ...(selectedCandidateId
        ? { selected_candidate_id: selectedCandidateId }
        : {}),
      ...(selectionScore !== undefined
        ? { objective_selection_score: selectionScore }
        : {}),
      ...(typeof evidenceBacked === "boolean"
        ? { objective_selection_evidence_backed: evidenceBacked }
        : {}),
    },
    mission_binding_readback: {
      contract: text(bindingState.contract, 240) || null,
      declared_count: Number(bindingState.declared_count || 0),
      captured_count: Number(bindingState.captured_count || 0),
      server_authoritative: true,
      raw_step_result_remains_bounded: true,
    ...
    const execution = await executeUbteCapability({
      ...
      runtime: {
        ...
        metadata: {
          ...
          ownerConstraintsBound: ownerConstraints.length > 0,
          ownerConstraintsAuthorizationEffect: "NONE_RESTRICTION_ONLY",
...
export function createProductEngineeringCycleCapability() {
  const manifest = defineCapability({
    ...
    tags: [
      ...
      "no-recursion",
      "verified-production-release-optional",
    ],
    operatorAliases: [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        owner_constraints: {
          ...
          maxItems: 8,
          description:
            "Bounded owner constraints carried from Business Partner. They may restrict engineering behavior but never grant source mutation, commit, production deployment, migration, publication, secret access, or governance authority.",
        ...
        release_to_production: {
          ...
          default: false,
          description:
            "When true, a Product Intelligence REQUEST_COMMIT_CONFIRMATION decision may be converted into the internal verified production-release path. The release still requires dedicated commit and production-deploy permissions, exact attested main persistence, and successful Vercel production verification. It never applies database migrations or changes secrets.",
        },
        commit_message: {
          type: "string",
          minLength: 1,
    ...
    outputSchema: {
      ...
      properties: {
        ...
        production_release: { type: ["object", "null"] },
        deployment_pending: { type: "boolean" },
      },
  ...
  async function execute({ context, payload = {} }) {
    ...
    const releaseToProduction = payload.release_to_production === true;
    ...
    const repositoryHeadObserved =
      text(assessment?.repository_snapshot?.current_main_head, 160) ||
      text(assessment?.next_engineering_handoff?.repository_head_observed, 160) ||
      null;
    ...
    if (
      ...
      const selectedCommitMessage = governedCommitMessage(
        commitMessage,
        decision?.persistence?.commit_message,
      );
      if (!selectedCommitMessage) {
        throw new Error("PRODUCT_ENGINEERING_PRODUCTION_RELEASE_COMMIT_MESSAGE_REQUIRED");
      }
      const released = await executeUbteCapability({
        ...
        payload: {
          execution_key: key,
          commit_message: selectedCommitMessage,
        },
        actor: context.actor,
        runtime: {
          entityId: context.entityId,
          ...
          metadata: {
            ...object(context.metadata),
            source: "AVANTIQO_PRODUCT_ENGINEERING_CYCLE_AUTOMATIC_RELEASE",
            productEngineeringAutomaticRelease: true,
            databaseMigrationExecutionAllowed: false,
    ...
    return {
      ...
      commit_requested: commitConfirmationPrepared || Boolean(productionRelease),
      commit_completed: productionRelease?.commit_completed === true,
      persistent_source_changed: productionRelease?.commit_completed === true,
      production_deployed: productionRelease?.production_deployed === true,
      ...
      governance: {
        ...
        product_repository_assessment_readback_uses_server_binding_state: true,
        bounded_operator_mission_step_results_remain_bounded: true,
        ...
        commit_is_only_allowed_inside_persistence_handoff:
          !releaseToProduction || !productionRelease,
        commit_is_allowed_inside_verified_production_release:
          releaseToProduction && Boolean(productionRelease),
        ...
        code_ai_commit_capability_completed:
          productionRelease?.commit_completed === true,
        ...
        production_deployment_capability_invoked: Boolean(productionRelease),
        automatic_production_release_requested: releaseToProduction,
        automatic_production_release_completed: productionRelease?.production_deployed === true,
        automatic_production_release_pending: productionRelease?.deployment_pending === true,
      },
    };
  }
