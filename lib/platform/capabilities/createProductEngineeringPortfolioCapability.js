...
function verifiedCommitFromHandoff(handoff = {}) {
  ...
  return (
    ...
    text(handoff?.persistence_decision?.engineering_evidence?.current_main_head, 160) ||
    null
  );
}
    ...
    if (portfolioId) {
      ...
      if (portfolio.status === "READY" && portfolio.current_node_id) {
        ...
        return {
          ...
          portfolio: compactProductEngineeringPortfolio(run.portfolio),
          repository_assessment: null,
          cycle: run.cycle,
          persistence_handoff: null,
      ...
      return {
        ...
        portfolio_id: portfolioId,
        portfolio: compactProductEngineeringPortfolio(resumed.portfolio),
        repository_assessment:
          freshAssessmentFromHandoff(resumed.handoff) || null,
        cycle: resumed.newCycle,
        persistence_handoff: resumed.handoff,
        new_cycle_started: resumed.cycleStarted === true,
        production_deployed: false,
        database_migrations_applied: false,
        automatic_commit_performed: false,
      };
