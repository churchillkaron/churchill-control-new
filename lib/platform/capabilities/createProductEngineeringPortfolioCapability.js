...
function formatCodePerformanceEvidence(value) {
  ...
  if (!metrics.mission_count) return null;
  return [
    "ATTESTED CODE MISSION PERFORMANCE EVIDENCE (ADVISORY TO FRESH-MAIN REPOSITORY ASSESSMENT):",
    `missions=${Number(metrics.mission_count || 0)}; verified_completion=${Math.round(Number(metrics.verified_completion_rate || 0) * 100)}%; first_pass=${Math.round(Number(metrics.first_pass_success_rate || 0) * 100)}%; avg_reasoning_calls=${Number(metrics.average_reasoning_calls || 0)}; efficiency=${Number(metrics.engineering_efficiency_score || 0)}/100`,
    ...backlog.slice(0, 4).map((item) => `measured_gap[${text(item?.priority, 20)}:${text(item?.area, 120)}]: ${text(item?.target, 800)}`),
    "Use these metrics to challenge roadmap priority, but select objectives only when current-main repository evidence supports a concrete improvement. Metrics have no mutation, commit, deployment or governance authority.",
  ].filter(Boolean).join("\n");
}
