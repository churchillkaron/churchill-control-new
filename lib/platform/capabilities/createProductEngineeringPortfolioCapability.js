...
import {
  benchmarkCodeAIEngineeringPerformance,
} from "@/lib/code/runtime/CodeAIEngineeringPerformanceBenchmarkRuntime";
import {
  governProductEngineeringPortfolioWithOwnerControl,
  loadProductEngineeringPortfolioOwnerControl,
  ownerControlAllowsNewEngineeringCycle,
...
async function governedPortfolioForExecution({ context, portfolio }) {
...
async function verifyRepositoryCommitAncestry({ repositoryUrl, baseCommit, verifiedCommitSha, timeoutMs = null } = {}) {
  const base = text(baseCommit, 160);
  const verified = text(verifiedCommitSha, 160);
  if (!/^[a-f0-9]{40}$/i.test(base) || !/^[a-f0-9]{40}$/i.test(verified)) {
    return { verified: false, reason: "PRODUCT_ENGINEERING_PORTFOLIO_ANCESTRY_COMMIT_REQUIRED" };
  }
  if (base === verified) {
    return { verified: false, reason: "PRODUCT_ENGINEERING_PORTFOLIO_ZERO_REPOSITORY_PROGRESS" };
  }
  const workspace = await CodeWorkspaceRuntime.open({
    repository_url: repositoryUrl || DEFAULT_REPOSITORY,
    ref: DEFAULT_REF,
  ...
  try {
    const first = await workspace.run({
      command: "git",
      args: ["merge-base", "--is-ancestor", base, verified],
      timeout_ms: Math.min(Number(timeoutMs || 120000), 120000),
    });
    if (first.exit_code === 0) return { verified: true, reason: null };
    if (first.exit_code === 1) return { verified: false, reason: "PRODUCT_ENGINEERING_PORTFOLIO_COMMIT_NOT_DESCENDANT" };
    await workspace.run({
      command: "git",
      args: ["fetch", "--deepen", "512", "origin", DEFAULT_REF],
      timeout_ms: Math.min(Number(timeoutMs || 120000), 120000),
    });
    const retry = await workspace.run({
      command: "git",
      args: ["merge-base", "--is-ancestor", base, verified],
      timeout_ms: Math.min(Number(timeoutMs || 120000), 120000),
    });
    if (retry.exit_code === 0) return { verified: true, reason: null };
    return {
      verified: false,
      reason: retry.exit_code === 1
        ? "PRODUCT_ENGINEERING_PORTFOLIO_COMMIT_NOT_DESCENDANT"
        : "PRODUCT_ENGINEERING_PORTFOLIO_REPOSITORY_ANCESTRY_UNPROVEN",
    };
  } finally {
...
function codeEngineeringImprovementGoal(value) {
  return /\b(?:code studio|code ai|coding system|coding agent|codex|claude code|software engineering intelligence)\b/i.test(
    text(value, 5000),
  );
}

async function codePerformanceEvidenceForPortfolio({ context, repositoryUrl, businessGoal }) {
  if (!codeEngineeringImprovementGoal(businessGoal)) return null;
  try {
    return await benchmarkCodeAIEngineeringPerformance({
      context,
      repositoryUrl,
      ref: DEFAULT_REF,
      limit: 50,
    ...
    console.error(JSON.stringify({
      event: "AVANTIQO_PRODUCT_ENGINEERING_CODE_PERFORMANCE_EVIDENCE_UNAVAILABLE",
      reason: text(error?.message || error, 500),
      portfolio_execution_blocked: false,
...
function formatCodePerformanceEvidence(value) {
  const benchmark = object(value);
  const metrics = object(benchmark.metrics);
  const backlog = list(benchmark?.improvement_backlog?.items);
  const hotspotBacklog = list(benchmark?.engineering_hotspot_backlog?.items);
  const competitive = object(benchmark.competitive_evidence);
  const competitiveBacklog = list(competitive.improvement_backlog);
  if (!metrics.mission_count && !competitive.contract) return null;
  return [
    metrics.mission_count
      ? "ATTESTED CODE MISSION PERFORMANCE EVIDENCE (ADVISORY TO FRESH-MAIN REPOSITORY ASSESSMENT):"
      : null,
    metrics.mission_count
      ? `missions=${Number(metrics.mission_count || 0)}; verified_completion=${Math.round(Number(metrics.verified_completion_rate || 0) * 100)}%; direct_first_pass=${Math.round(Number(metrics.first_pass_success_rate || 0) * 100)}%; avg_reasoning_calls=${Number(metrics.average_reasoning_calls || 0)}; efficiency=${Number(metrics.engineering_efficiency_score || 0)}/100`
      : null,
    ...backlog.slice(0, 4).map((item) => `measured_gap[${text(item?.priority, 20)}:${text(item?.area, 120)}]: ${text(item?.target, 800)}`),
    ...hotspotBacklog.slice(0, 4).map((item) =>
      `engineering_hotspot[${text(item?.priority, 20)}:${text(item?.area, 120)}]: occurrences=${Number(item?.occurrence_count || 0)}; score=${Number(item?.max_score || 0)}; ${text(item?.recommendation, 800)}`
    ),
    competitive.contract
      ? "GLOBAL CODE COMPETITIVE BENCHMARK EVIDENCE (PLATFORM-OWNED, ADVISORY):"
      : null,
    competitive.contract
      ? `competitive_certified=${competitive.competitive_certified === true}; evidence_current=${competitive.evidence_current === true}; superiority_claim_allowed=${competitive.superiority_claim_allowed === true}; reference_count=${list(competitive.comparisons).length}`
      : null,
    ...competitiveBacklog.slice(0, 4).map((item) =>
      `competitive_gap[${text(item?.case_id, 160)}:${text(item?.category, 160)}]: ${text(item?.title, 700)}; losses=${Number(item?.losses || 0)}`
    ),
    competitive.contract
      ? "Competitive evidence can prioritize investigation, but its source report is not repository-commit authoritative. Re-prove every candidate improvement against current main before mutation."
      : null,
    "Use these metrics to challenge roadmap priority, but select objectives only when current-main repository evidence supports a concrete improvement. Metrics have no mutation, commit, deployment, provider-routing or governance authority.",
  ].filter(Boolean).join("\n");
}
...
async function assessCurrentMain({
  ...
  const performanceEvidence = await codePerformanceEvidenceForPortfolio({
    context,
    repositoryUrl,
    businessGoal,
  });
  const formattedPerformance = formatCodePerformanceEvidence(performanceEvidence);
...
async function runOneEngineeringCycle({
  ...
  const governed = await governedPortfolioForExecution({
    context,
    portfolio: portfolioValue,
  });
  const portfolio = governed.portfolio;
  if (!ownerControlAllowsNewEngineeringCycle(portfolio)) {
    return {
      portfolio,
      cycle: null,
      started: false,
      reason: portfolio?.owner_control?.paused === true
        ? "PORTFOLIO_PAUSED_BY_OWNER"
        : "PRODUCT_ENGINEERING_PORTFOLIO_NO_OWNER_ELIGIBLE_OBJECTIVE",
    };
  }
  ...
  const cycle = await executeCapability({
    ...
    payload: {
      ...
      ref: "main",
      ...(list(portfolio.owner_constraints).length ? { owner_constraints: list(portfolio.owner_constraints).map((item) => text(item, 500)).filter(Boolean).slice(-8) } : {}),
  ...
  const afterControl = await governedPortfolioForExecution({
    context,
    portfolio: attached,
  });
  return {
    portfolio: afterControl.portfolio,
    cycle,
    started: true,
    reason: null,
  };
...
async function resumePersistenceBoundary({
  ...
  const ancestry = await verifyRepositoryCommitAncestry({
    repositoryUrl: portfolio.repository_url || DEFAULT_REPOSITORY,
    baseCommit: currentNode?.base_commit,
    verifiedCommitSha,
    timeoutMs,
  });
  if (ancestry.verified !== true) {
    portfolio.status = "WAITING_VERIFIED_PERSISTENCE";
    if (currentNode) {
      currentNode.status = "WAITING_VERIFIED_PERSISTENCE";
      currentNode.progress_integrity = {
        verified: false,
        repository_ancestry_verified: false,
        blockers: [ancestry.reason || "PRODUCT_ENGINEERING_PORTFOLIO_REPOSITORY_ANCESTRY_UNPROVEN"],
        authorization_effect: "NONE",
      };
    }
    portfolio.updated_at = new Date().toISOString();
    await persistProductEngineeringPortfolio({ context, portfolio });
    return {
      portfolio, handoff, newCycle: null, cycleStarted: false,
      status: ancestry.reason || "PRODUCT_ENGINEERING_PORTFOLIO_REPOSITORY_ANCESTRY_UNPROVEN",
    };
  }
  ...
  const completed = completePortfolioNodeAfterVerifiedPersistence({
    ...
    executionKey,
    verifiedCommitSha,
    repositoryAncestryVerified: true,
  });
  const rebuilt = buildProductEngineeringPortfolio({
    context,
...
async function continueExistingPortfolio({
  ...
  if (portfolio?.owner_control?.paused === true) {
    return {
      status: "PORTFOLIO_PAUSED_BY_OWNER",
      portfolio,
...
export function createProductEngineeringPortfolioCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Turn one broad Avantiqo business/product improvement goal into a bounded current-main engineering roadmap using the existing repository assessment's two-to-four evidence-backed ranked objective candidates. Derive overlap dependencies deterministically from exact evidence paths and stable source areas. Execute at most one local Product Engineering Cycle per invocation. Queued objectives are provisional, never authority. Durable owner controls may pause future progression or promote, defer, remove, and restore queued objectives; claimed work is immutable and controls apply only at safe boundaries. If the current objective reaches the governed persistence boundary, preserve explicit commit confirmation; only a separately verified main commit may retire that objective. Then reassess actual current main, preserve concurrent progress, rebuild and re-rank the remaining roadmap, re-apply owner directives, and start at most one fresh successor. Exact same-goal continuation is resolved to the actor-scoped persisted portfolio instead of rebuilding history. Never fan out branches/worktrees, run parallel source-mutating Code agents, commit automatically, deploy production, apply migrations, expose raw reasoning, or recurse without bound.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        owner_constraints: {
          ...
          maxItems: 8,
          description:
            "Bounded Business Partner owner constraints persisted with the portfolio and forwarded to each engineering cycle. Restriction-only; never authority.",
  ...
  async function execute({ context, payload = {} }) {
    ...
    const timeoutMs = Number.isInteger(Number(payload.timeout_ms))
      ...
      : null;
    const ownerConstraints = list(payload.owner_constraints).map((item) => text(item, 500)).filter(Boolean).slice(-8);
    ...
    if (portfolioId) {
      ...
      if (ownerConstraints.length) {
        loaded.portfolio.owner_constraints = ownerConstraints;
        loaded.portfolio.updated_at = new Date().toISOString();
        await persistProductEngineeringPortfolio({ context, portfolio: loaded.portfolio });
      }
    ...
    if (
      ...
      if (ownerConstraints.length) {
        latest.portfolio.owner_constraints = ownerConstraints;
        latest.portfolio.updated_at = new Date().toISOString();
        await persistProductEngineeringPortfolio({ context, portfolio: latest.portfolio });
      }
      const continued = await continueExistingPortfolio({
        context,
    ...
    if (ownerConstraints.length) portfolio.owner_constraints = ownerConstraints;
    await persistProductEngineeringPortfolio({ context, portfolio });
    ...
    return {
      ...
      governance: {
        ...
        owner_control_reapplied_before_each_new_cycle: true,
        owner_pause_blocks_persistence_continuation_until_resumed: true,
        current_claimed_objective_immutable_to_owner_reordering: true,
