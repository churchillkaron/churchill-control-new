...
export function createProductPersistenceHandoffCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Convert Avantiqo Product Intelligence's read-only persistence decision into a durable governed handoff. The capability first re-evaluates server-owned verified engineering evidence. STAY_LOCAL returns without a write mission. If persistence is already verified, a bounded two-read durable mission independently verifies that persisted commit again and binds only registered verifier scalars into one repository-grounded Product Intelligence reassessment, surfacing exactly one next bounded engineering objective without another commit or automatic engineering execution. Only REQUEST_COMMIT_CONFIRMATION creates the separately permissioned Code AI commit action; that mission pauses before the commit because the commit capability requires explicit confirmation, then independently verifies main and binds only scalar verifier evidence into the same one-shot continuation. Read-only recovery is allowed only through the existing commit-attempt evidence boundary and never replays the write. No production deployment, migration, publication, force push, hidden approval, or unbounded recursion is allowed.",
