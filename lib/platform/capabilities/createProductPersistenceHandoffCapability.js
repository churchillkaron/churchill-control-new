  ...
  return [
    {
      ...
      verify_after: {
        ...
        description:
          "Independently verify the resulting GitHub main commit before any continuation is allowed",
...
export function createProductPersistenceHandoffCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Convert Avantiqo Product Intelligence's read-only persistence decision into a durable governed handoff. The capability first re-evaluates the server-owned verified engineering evidence. If Product Intelligence says STAY_LOCAL, it returns without creating a write mission. If the result is already verified as persisted, it returns the verified state. Only when Product Intelligence returns REQUEST_COMMIT_CONFIRMATION does it create a bounded Operator mission whose first step is the existing separately permissioned Code AI commit action. That mission pauses before the commit because the commit capability requires explicit confirmation; after confirmation it independently verifies the main commit and then performs exactly one read-only fresh Product Intelligence reassessment. No production deployment, migration, publication, force push, hidden approval, or unbounded recursion is allowed.",
