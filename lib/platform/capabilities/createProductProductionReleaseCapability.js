...
export function createProductProductionReleaseCapability() {
  ...
  async function execute({ context, payload = {} }) {
    ...
    if (!artifact.found || !artifact.mission_state) {
      throw new Error("PRODUCT_PRODUCTION_RELEASE_CODE_ARTIFACT_NOT_FOUND");
    }
    assertCodeAIProductionReleasePrecisionReady(artifact.mission_state);
    const rollingReleasePolicy = await verifyVercelRollingReleasePolicy();
    ...
    return {
      ...
      commit: commitResult,
      deployment,
      rolling_release_policy: rollingReleasePolicy,
