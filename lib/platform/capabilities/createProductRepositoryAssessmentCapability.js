...
} from "@/lib/platform/runtime/AvantiqoDeveloperResourceReadRuntime";
...
async function assessWithDeveloperResources(assessmentInput) {
  const [assessment, developerResources] = await Promise.all([
    assessAvantiqoCurrentRepository(assessmentInput),
    readAvantiqoDeveloperResources({
      focus: assessmentInput.focus,
      repository: assessmentInput.repositoryUrl,
    }),
  ]);
  return {
    ...assessment,
...
export function createProductRepositoryAssessmentCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Clone the actual GitHub main branch into Avantiqo's read-only Code AI workspace, capture bounded current source evidence including the real HEAD, read sanitized current Vercel production/deployment evidence when server credentials are available, and let Avantiqo-owned Product Intelligence select one next bounded engineering objective from that evidence. This capability never edits source, commits, deploys, changes Vercel environment variables, applies migrations, publishes, or starts the next engineering cycle.",
    ...
    operatorAliases: [
      ...
      "check current github and vercel evidence",
      "inspect current deployment and repository state",
    ],
    transactional: false,
    aiEnabled: true,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        focus: {
          ...
          maxLength: 2000,
          description:
            "Optional read-only diagnostic focus. Vercel build/runtime log evidence is fetched only when this focus materially concerns deployment, runtime, logs, errors, or production state.",
        },
        timeout_ms: {
          type: "integer",
          minimum: 30000,
    ...
    outputSchema: {
      ...
      properties: {
        ...
        developer_resources: { type: "object" },
  ...
  async function execute({ context, payload = {} }) {
    ...
    try {
      const result = await assessWithDeveloperResources(assessmentInput);
      return {
        ...result,
