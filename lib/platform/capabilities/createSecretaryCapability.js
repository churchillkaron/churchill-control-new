...
const ACTIONS = {
  ...
  createCalendarEvent: {
    ...
    reversible: true,
    confirm: true,
    description: "Create a meeting, appointment, call block, reminder or other event in Avantiqo's native calendar with atomic no-overlap enforcement.",
    aliases: ["book an appointment", "schedule a meeting", "add this to my calendar", "book a call"],
    execute: createSecretaryCalendarEventAtomic,
  },
  updateCalendarEvent: {
    mode: "write",
    risk: "high",
...
function verificationFor(action) {
  const declarations = {
    createCalendarEvent: { capability_key: "platform.secretary_calendar_event.read", payload_from_result: { event_id: ["event.id"] } },
    updateCalendarEvent: { capability_key: "platform.secretary_calendar_event.read", payload_from_input: { event_id: "event_id" } },
    createContact: { capability_key: "platform.secretary_contact.read", payload_from_result: { party_id: ["party.id"] } },
    upsertContactProfile: { capability_key: "platform.secretary_contact.read", payload_from_input: { party_id: "party_id" } },
    createTask: { capability_key: "platform.secretary_task.read", payload_from_result: { task_id: ["task.id"] } },
    updateTask: { capability_key: "platform.secretary_task.read", payload_from_input: { task_id: "task_id" } },
    createFollowUp: { capability_key: "platform.secretary_follow_up.read", payload_from_result: { follow_up_id: ["follow_up.id"] } },
    updateSettings: { capability_key: "platform.secretary_settings.read", payload_from_result: { organization_id: ["settings.organization_id"] } },
  };
  const declaration = declarations[action];
  return declaration ? { ...declaration, derivation: "secretary_exact_record_verifier" } : null;
}
...
export function createSecretaryCapability(action) {
  ...
  const manifest = defineCapability({
    ...
    ...(verificationFor(action) ? { operatorVerification: verificationFor(action) } : {}),
