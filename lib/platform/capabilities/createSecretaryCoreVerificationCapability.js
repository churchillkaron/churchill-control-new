...
export function createSecretaryMeetingTransitionVerificationCapability() {
  const manifest = defineCapability({
    domain: "platform", capability: "secretary_meeting_transition", action: "read",
    description: "Read one exact evidence-bound Secretary meeting coordination transition for authoritative mutation verification.",
    permissions: [], events: [], tags: ["platform", "secretary", "verification", "meeting", "read"], transactional: false, aiEnabled: false,
    operatorEnabled: true, operatorMode: "read", operatorAutoExecute: true, operatorRequiresConfirmation: false, contextScope: "organization", risk: "low",
    inputSchema: {
      type: "object", required: ["coordination_id", "transition_kind", "evidence_id"],
      properties: {
        coordination_id: { type: "string" },
        transition_kind: { type: "string", enum: ["CANCEL_COORDINATION", "RESCHEDULE_BOOKED", "CANCEL_BOOKED"] },
  ...
  async function execute({ context, payload = {} }) {
    const organizationId = text(context?.organizationId);
    const coordinationId = text(payload.coordination_id);
    const transitionKind = text(payload.transition_kind).toUpperCase();
    const evidenceId = text(payload.evidence_id);
    if (!organizationId || !coordinationId || !transitionKind || !evidenceId) throw new Error("SECRETARY_MEETING_TRANSITION_VERIFICATION_SCOPE_REQUIRED");
    const { data, error } = await supabaseAdmin.from("secretary_meeting_coordinations").select("status,calendar_event_id,metadata").eq("organization_id", organizationId).eq("id", coordinationId).maybeSingle();
    if (error) throw error;
    if (!data) {
      return { contract: "AVANTIQO_AUTHORITATIVE_BUSINESS_EFFECT_OUTCOME_V1", state: "UNCERTAIN", authoritative_server_evidence: true, exact_business_scope_matched: true, business_effect_observed: false, business_effect_absent: false, safe_to_retry: false, record: null, locator_key: "coordination_id", locator: coordinationId };
    }
    const history = Array.isArray(data?.metadata?.meeting_transition_evidence_history) ? data.metadata.meeting_transition_evidence_history : [];
    const exact = history.find((item) => text(item?.evidence_id) === evidenceId && text(item?.kind).toUpperCase() === transitionKind) || null;
    const conflictingEvidence = history.find((item) => text(item?.evidence_id) === evidenceId && text(item?.kind).toUpperCase() !== transitionKind) || null;
    const currentStateAllowsRetry = transitionKind === "CANCEL_COORDINATION"
      ? text(data.status).toUpperCase() !== "BOOKED" && text(data.status).toUpperCase() !== "CANCELLED"
      : transitionKind === "RESCHEDULE_BOOKED" || transitionKind === "CANCEL_BOOKED"
        ? text(data.status).toUpperCase() === "BOOKED"
        : false;
    const definitelyAbsent = !exact && !conflictingEvidence && currentStateAllowsRetry;
    return {
      contract: "AVANTIQO_AUTHORITATIVE_BUSINESS_EFFECT_OUTCOME_V1",
      state: exact ? "COMPLETED" : definitelyAbsent ? "NOT_COMPLETED" : "UNCERTAIN",
      authoritative_server_evidence: true, exact_business_scope_matched: true,
      business_effect_observed: Boolean(exact), business_effect_absent: definitelyAbsent, safe_to_retry: definitelyAbsent,
      record: exact, conflicting_record: conflictingEvidence, current_status: data.status,
      locator_key: "coordination_id", locator: coordinationId,
    };
  }
  return { manifest, execute };
}
