...
import { readSecretaryExecutiveBriefingV7 } from "@/lib/operator/secretary/SecretaryExecutiveBriefingV7Runtime";
...
export function createSecretaryExecutiveBriefingCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Read one evidence-backed organization-scoped daily, weekly, or custom executive desk briefing across the durable executive decision register, durable executive directive register, correspondence, calendar, commitments, deadlines, document gaps, relationship touchpoints, calls, current travel operations including evidenced cancellations and voids, explicit working preferences, expenses, visitors, absence coverage, delegated work and Secretary-owned follow-through.",
    permissions: [],
    events: ["platform.secretary_briefing.read"],
    tags: ["platform", "secretary", "executive-secretary", "briefing", "daily", "weekly", "decisions", "directives", "commitments", "travel", "preferences", "read"],
    operatorAliases: [
      "brief me",
  ...
  async function execute({ context, payload = {} }) {
    return readSecretaryExecutiveBriefingV7({ context, payload });
  }
