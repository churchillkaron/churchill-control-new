...
export function createSecretaryExecutiveBriefingCapability() {
  const manifest = defineCapability({
    ...
    description:
      "Read one evidence-backed organization-scoped daily, weekly, or custom executive desk briefing across decisions, correspondence, calendar, commitments, deadlines, document gaps, relationship touchpoints, calls, current travel operations, explicit working preferences, expenses, visitors, absence coverage, delegated work and Secretary-owned follow-through.",
    permissions: [],
    events: ["platform.secretary_briefing.read"],
    tags: ["platform", "secretary", "executive-secretary", "briefing", "daily", "weekly", "commitments", "travel", "preferences", "read"],
    operatorAliases: [
      "brief me",
  ...
  async function execute({ context, payload = {} }) {
    return readSecretaryExecutiveBriefingV5({ context, payload });
  }
