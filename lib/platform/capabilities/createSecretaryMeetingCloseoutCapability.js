...
import { reviseSecretaryMeetingMinutesGoverned } from "@/lib/operator/secretary/SecretaryMeetingMinutesRevisionGovernedRuntime";
...
const ACTIONS = Object.freeze({
  ...
  reviseMinutes: {
    ...
    description: "Apply an explicit evidence-backed factual revision to the current meeting minutes only when its evidence matches a recorded correction request, preserve prior versions, stale-fence superseded revisions and distributions, reset acknowledgements for the new version, and queue deterministic revised-minutes redistribution without changing the captured meeting record or creating approval, acceptance, attendance or binding authority.",
  },
  refresh: {
    mode: "write",
    risk: "low",
...
export function createSecretaryMeetingCloseoutCapability(action) {
  ...
  async function execute({ context, payload = {} }) {
    ...
    if (action === "reviseMinutes") return reviseSecretaryMeetingMinutesGoverned({ context, payload });
