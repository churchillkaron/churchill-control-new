...
import { recordSecretaryTravelCancellation } from "@/lib/operator/secretary/SecretaryTravelCancellationRuntime";
...
const ACTIONS = Object.freeze({
  read: {
    ...
    description: "Read an evidence-backed operational itinerary for an existing Secretary travel job, including confirmed travel items, cancelled or voided confirmation history, disruptions, reminders, unresolved work and exact approval gates. Researched options and cancellation intent are never treated as confirmations or completed cancellations.",
    execute: readSecretaryTravelOperationsV2,
  },
  recordConfirmation: {
    mode: "write",
    risk: "low",
  ...
  recordCancellation: {
    mode: "write",
    risk: "low",
    aliases: ["record cancelled flight", "record travel cancellation", "mark this confirmed hotel cancelled", "record voided travel confirmation"],
    description: "Record explicit evidence that a current confirmed travel item has already been cancelled or voided. This does not send a cancellation request, infer cancellation from intent, accept a cancellation fee, settle a refund, or create replacement booking, payment or commercial authority.",
    execute: recordSecretaryTravelCancellation,
  },
  recordDisruption: {
    mode: "write",
    risk: "low",
...
function schema(action) {
  ...
  if (action === "recordCancellation") return {
    type: "object",
    properties: {
      job_id: { type: "string" },
      confirmation_id: { type: "string" },
      evidence_id: { type: "string" },
      outcome: { type: "string", enum: ["CANCELLED", "VOIDED"] },
      cancelled_at: { type: "string" },
      cancellation_reference: { type: "string" },
      reason: { type: "string" },
      source_reference: { type: "string" },
      notes: { type: "string" },
    },
    required: ["job_id", "confirmation_id", "evidence_id", "outcome", "cancelled_at"],
    additionalProperties: false,
  };
