...
function customerSetup(mode, label, technicalInputRequired = false) {
  return {
    mode,
    label,
    technicalInputRequired: technicalInputRequired === true,
  };
}
...
export const BUSINESS_CONNECTION_REGISTRY = [
  ...
  },
  {
    ...
    name: "Pinterest",
    category: "Social media",
    description: "Connect the organization Pinterest account for organic Pin publishing and board discovery.",
    connectionProviders: ["pinterest"],
    assetProviders: ["pinterest"],
    assetTypes: ["pinterest_account"],
    connectPath: "/api/pinterest/auth",
    authModel: "OAUTH",
    availability: "active",
    platformRequirements: ["PINTEREST_APP_ID", "PINTEREST_APP_SECRET"],
    platformSetup: setup({
      summary: "Avantiqo uses Pinterest OAuth for organization-owned Pinterest accounts and organic publishing.",
      callbackPaths: ["/api/pinterest/auth/callback"],
      steps: [
        "Configure the Pinterest app and redirect URL.",
        "Request Standard access for customer-facing production use where required by Pinterest.",
      ],
      approval: "Pinterest app access level controls which production customer accounts and publishing capabilities can be used.",
    }),
    customerSetup: customerSetup("PROVIDER_AUTHORIZATION", "Sign in with Pinterest and authorize the business account"),
  },
  {
    id: "youtube",
    name: "YouTube",
    category: "Social media",
    description: "Connect the organization YouTube channel for governed video publishing and channel analytics.",
    connectionProviders: ["youtube"],
    assetProviders: ["youtube"],
    assetTypes: ["youtube_channel"],
    connectPath: "/api/youtube/auth",
    authModel: "OAUTH",
    availability: "active",
    platformRequirements: ["GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET"],
    platformSetup: setup({
      summary: "Avantiqo uses the shared Google OAuth project and YouTube Data API for customer-owned YouTube channels.",
      callbackPaths: ["/api/youtube/auth/callback"],
      steps: [
        "Enable the YouTube Data API in the Avantiqo Google project.",
        "Add the YouTube OAuth callback URL to the shared Google OAuth client.",
        "Complete Google OAuth verification and YouTube upload audit where required for public publishing.",
      ],
      approval: "Unverified or unaudited YouTube API projects may have upload/publication restrictions imposed by Google.",
    }),
    customerSetup: customerSetup("PROVIDER_AUTHORIZATION", "Sign in with Google and authorize the business YouTube channel"),
  },
  {
    id: "whatsapp",
  ...
  },
  {
    ...
    name: "SMS — Twilio",
    category: "Messaging",
    description: "Connect the organization’s Twilio SMS account for outbound texts, delivery tracking, and inbound Unified Inbox conversations.",
    connectionProviders: ["sms"],
    assetProviders: ["sms"],
    assetTypes: ["sms_sender"],
    connectPath: "/api/sms/auth",
    authModel: "API_CREDENTIAL",
    availability: "active",
    platformRequirements: [],
    platformSetup: setup({
      summary: "Avantiqo supports organization-owned Twilio SMS credentials and configures inbound/status webhooks automatically.",
      callbackPaths: ["/api/commercial/communications/webhooks/sms/[connectionId]", "/api/commercial/communications/webhooks/sms/[connectionId]/status"],
      steps: ["The customer supplies its Twilio Account SID/Auth Token and either a Twilio phone number or Messaging Service SID."],
    }),
    customerSetup: customerSetup("API_CREDENTIAL", "Connect the organization Twilio SMS account", true),
  },
  {
    id: "telegram",
    name: "Telegram",
    category: "Messaging",
    description: "Connect an organization-owned Telegram bot for customer conversations and notifications.",
    connectionProviders: ["telegram"],
    assetProviders: ["telegram"],
    assetTypes: ["telegram_bot"],
    connectPath: "/api/telegram/auth",
    authModel: "BOT_TOKEN",
    availability: "active",
    platformRequirements: [],
    platformSetup: setup({
      summary: "Telegram Bot API uses an organization-owned bot token. Avantiqo validates, vaults, and configures the webhook automatically.",
      callbackPaths: ["/api/commercial/communications/webhooks/telegram/[connectionId]"],
      steps: ["The customer creates a Telegram bot with BotFather and copies its bot token into Avantiqo once."],
    }),
    customerSetup: customerSetup("BOT_TOKEN", "Paste the Telegram bot token from BotFather", true),
  },
  {
    id: "email",
    name: "Email",
  ...
  },
  {
    ...
    name: "Yelp",
    category: "Reputation",
    description: "Yelp business reviews and owner-response workflows through Yelp Partner APIs.",
    connectionProviders: ["yelp"],
    assetProviders: ["yelp"],
    assetTypes: ["yelp_business"],
    connectPath: null,
    authModel: "PARTNER_OAUTH",
    availability: "platform_setup_required",
    platformRequirements: [],
    platformSetup: setup({
      summary: "Yelp review-response access is a partner capability and is disabled by default until Yelp grants Avantiqo access.",
      steps: [
        "Obtain Yelp Partner API approval for review retrieval and Respond to Reviews.",
        "Configure the approved Yelp OAuth application and business-owner authorization flow.",
      ],
      approval: "Yelp Respond to Reviews is a Partner API and requires Yelp approval before customer setup can be enabled.",
    }),
    customerSetup: customerSetup("PROVIDER_AUTHORIZATION", "Authorize the claimed Yelp business after Avantiqo partner access is approved"),
  },
  {
    id: "opentable",
    name: "OpenTable",
    category: "Reservations",
    description: "OpenTable reservation and guest-data integration for restaurant organizations.",
    connectionProviders: ["opentable"],
    assetProviders: ["opentable"],
    assetTypes: ["opentable_restaurant"],
    connectPath: null,
    authModel: "PARTNER_OAUTH",
    availability: "platform_setup_required",
    platformRequirements: [],
    platformSetup: setup({
      summary: "OpenTable production APIs require approved partner access and a formal production agreement before restaurant onboarding.",
      steps: [
        "Complete OpenTable integration-partner approval and sandbox certification.",
        "Configure the approved OAuth client and restaurant onboarding integration.",
      ],
      approval: "OpenTable production API access is subject to partner approval, contracting, and capability eligibility.",
    }),
    customerSetup: customerSetup("PARTNER_ONBOARDING", "Connect the restaurant through OpenTable partner onboarding after Avantiqo production approval"),
  },
  {
    id: "push",
    name: "Push Notifications",
    category: "Owned messaging",
    description: "Web and app push notifications for customers who have explicitly opted in.",
    connectionProviders: ["push"],
    assetProviders: ["push"],
    assetTypes: ["push_application"],
    connectPath: null,
    authModel: "PLATFORM_CHANNEL",
    availability: "platform_setup_required",
    platformRequirements: [],
    platformSetup: setup({
      summary: "Push requires a certified Avantiqo web/mobile push transport, subscription lifecycle, device-token governance, and consent enforcement before customer setup is enabled.",
      steps: [
        "Certify web/mobile push transport and credential management.",
        "Certify opt-in, token revocation, and delivery-status lifecycle.",
      ],
    }),
    customerSetup: customerSetup("PLATFORM_CHANNEL", "Enable push after Avantiqo push transport is certified"),
  },
  {
    id: "tripadvisor",
    name: "Tripadvisor",
