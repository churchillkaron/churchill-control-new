...
function trustedHostnameAsset(record = {}) {
  if (!activeRecord(record)) return false;
  const status = String(record?.metadata?.status || "").trim().toUpperCase();
...
async function organizationHostContext({ asset, organization, supabaseAdmin }) {
  ...
  let logoSrc = metadata.logo_src ? String(metadata.logo_src) : null;
  const storagePath = String(metadata.logo_storage_path || "").trim();
  const storageBucket = String(metadata.logo_storage_bucket || "creative-assets").trim() || "creative-assets";
  if (storagePath) {
    const signed = await supabaseAdmin.storage.from(storageBucket).createSignedUrl(storagePath, 60 * 60);
    if (signed?.data?.signedUrl) logoSrc = signed.data.signedUrl;
  } else if (metadata.logo_direct_url) {
    logoSrc = String(metadata.logo_direct_url);
  }
  ...
  return {
    ...
    logoLayout: String(metadata.logo_layout || "compact"),
    identityLabel,
...
export async function resolveRegisteredPlatformHostContext(hostname) {
  ...
  // Platform-owned and current static hosts resolve without database access or
  // loading the privileged Supabase admin client.
  ...
  const asset = (assets || []).find(trustedHostnameAsset);
  ...
  return organizationHostContext({ asset, organization, supabaseAdmin });
}
