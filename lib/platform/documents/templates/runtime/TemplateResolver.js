...
function creativeTemplateModel(asset, financeTemplate = null) {
  ...
  return {
    ...
    asset,
    finance_template: financeTemplate,
  };
}

async function resolveActiveFinanceTemplate({
  organizationId,
  entityId,
  documentType,
}) {
  ...
  const { data: catalogues, error: catalogueError } = await supabaseAdmin
  ...
  if (catalogueError || !catalogues?.length) return null;

  const sourceUrls = catalogues
    .map((catalogue) => catalogue.template_source_url)
    .filter(Boolean);
  ...
  const { data: assets, error: assetError } = await supabaseAdmin
    ...
    .eq("archived", false)
    .in("file_url", sourceUrls);

  if (assetError || !assets?.length) return null;

  const assetBySource = new Map(
    assets.map((asset) => [asset.file_url, asset])
  );

  const candidates = catalogues
    .map((catalogue) => ({
      catalogue,
      asset: assetBySource.get(catalogue.template_source_url) || null,
    }))
    .filter((candidate) => candidate.asset);

  const entitySpecific = entityId
    ? candidates.find(
        ({ asset }) =>
          asset.metadata?.scope === "ENTITY" &&
          String(asset.metadata?.entity_id || "") === String(entityId)
      )
    : null;

  if (entitySpecific) {
    return creativeTemplateModel(
      entitySpecific.asset,
      entitySpecific.catalogue
    );
  }

  const organizationTemplate = candidates.find(
    ({ asset }) => asset.metadata?.scope !== "ENTITY"
  );

  return organizationTemplate
    ? creativeTemplateModel(
        organizationTemplate.asset,
        organizationTemplate.catalogue
      )
    : null;
}
