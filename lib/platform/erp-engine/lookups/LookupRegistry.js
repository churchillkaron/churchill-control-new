...
const CustomerLookup = new ScopedFinanceLookup({
  // `parties` is the canonical customer identity used by Accounts Receivable
  // (`customer_invoices.party_id`). Keep legacy tables as compatibility fallbacks.
  tables: ["parties", "customers", "customer_loyalty_accounts"],
  valueKeys: ["id", "party_id", "customer_id"],
  labelKeys: ["display_name", "legal_name", "customer_name", "company_name", "id"],
  descriptionKeys: ["email", "phone", "customer_email", "customer_phone"],
});
