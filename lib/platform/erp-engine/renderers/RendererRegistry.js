...
import FinanceFxRevaluationWorkCenter from "@/components/workspace/finance/FinanceFxRevaluationWorkCenter";
import FinancePaymentsWorkCenter from "@/components/workspace/finance/FinancePaymentsWorkCenter";
import FinanceTaxWorkCenter from "@/components/workspace/finance/FinanceTaxWorkCenter";
...
function RegisteredMasterDataRuntimeWorkCenter(props) {
  ...
  if (isFinanceCapability(capability)) {
    return createElement(FinanceAccountantRecordsWorkCenter, { ...base, capability });
  }
...
function RegisteredFinanceReportRuntimeWorkCenter(props) {
  const capability = sanitizeCapability(props.capability);
  if (isUnavailableFinanceCapability(capability, "finance")) return createElement(FinanceUnavailableWorkCenter, { capability });
...
function RegisteredFinanceCashFlowWorkCenter(props) {
  const capability = sanitizeCapability(props.capability);
  if (isUnavailableFinanceCapability(capability, "finance")) return createElement(FinanceUnavailableWorkCenter, { capability });
  return createElement(FinanceCashFlowWorkCenter, baseFinanceProps(props, capability));
}

function RegisteredFinanceCashManagementWorkCenter(props) {
  const capability = sanitizeCapability(props.capability);
  if (isUnavailableFinanceCapability(capability, "finance")) return createElement(FinanceUnavailableWorkCenter, { capability });
  return createElement(FinanceCashManagementWorkCenter, baseFinanceProps(props, capability));
}

function RegisteredFinanceFxRevaluationWorkCenter(props) {
  const capability = sanitizeCapability(props.capability);
  if (isUnavailableFinanceCapability(capability, "finance")) return createElement(FinanceUnavailableWorkCenter, { capability });
  return createElement(FinanceFxRevaluationWorkCenter, baseFinanceProps(props, capability));
}
...
function RegisteredFinanceTaxWorkCenter(props) {
  const capability = sanitizeCapability(props.capability);
  if (isUnavailableFinanceCapability(capability, "finance")) return createElement(FinanceUnavailableWorkCenter, { capability });
  return createElement(FinanceTaxWorkCenter, baseFinanceProps(props, capability));
}
...
function RegisteredChannelConnectionWorkCenter(props) {
  return createElement(ChannelConnectionWorkCenter, { capability: props.capability, organizationId: props.context?.organization_id });
}
...
registerRenderer("FinanceFxRevaluationWorkCenter", RegisteredFinanceFxRevaluationWorkCenter);
registerRenderer("FinancePaymentsWorkCenter", RegisteredFinancePaymentsWorkCenter);
registerRenderer("FinanceTaxWorkCenter", RegisteredFinanceTaxWorkCenter);
...
registerRenderer("OrganizationSetupWorkCenter", OrganizationSetupWorkCenter);
