...
export default function buildPlatformOperatorControl({
  ...
  const diagnosisHealth = health?.services?.business_diagnosis || null;
  if (diagnosisHealth && diagnosisHealth.ready !== true) {
    rawSignals.push({
      id: "business-diagnosis-readiness",
      severity: diagnosisHealth.authenticity_required === true ? "critical" : "high",
      sourceSeverity: diagnosisHealth.authenticity_required === true ? "critical" : "high",
      score: diagnosisHealth.authenticity_required === true ? 96 : 84,
      state: "live",
      category: "runtime",
      title: "Business Diagnosis is not ready",
      summary: diagnosisHealth.authenticity_required === true
        ? "Required diagnosis-proof authenticity is not operational, so governed business diagnosis is blocked before analysis starts."
        : "Business Diagnosis readiness is degraded and should be inspected before relying on diagnosis proofs.",
      organizationId: null,
      source: "checkSystemHealth",
      target: "business_diagnosis",
      occurredAt: health?.timestamp || null,
      actionable: false,
      evidence: {
        diagnosisStatus: text(diagnosisHealth.status) || null,
        authenticityRequired: diagnosisHealth.authenticity_required === true,
        authenticityAvailable: diagnosisHealth.authenticity_available === true,
        blockerCount: Number(diagnosisHealth.blocker_count || 0),
