...
import {
  PaymentSettlementVerificationRuntime,
} from "../verification/PaymentSettlementVerificationRuntime";
...
export async function confirmPayment({
  ...
  const verifiedEvidence = await PaymentSettlementVerificationRuntime.verifyPaymentSettlement({
    payment,
    verificationSource,
    sourceReference,
  });

  const settlement = assertSettlementEvidence(payment, verifiedEvidence);

  const paymentSource = cleanText(payment.metadata?.source)?.toLowerCase();
  if (paymentSource === "wallet_topup") {
  ...
  if (!(
    ...
    completedPayment = await PaymentTransactionRepository.update(
      paymentId,
      {
        status: "completed",
        provider_reference: settlement.provider_reference,
  ...
  await emitBusinessPartnerBusinessEvent({
    ...
    identity: `${payment.id}:${settlement.provider_reference}:completed`,
    evidence: {
      payment_id: payment.id,
      provider: settlement.provider,
      provider_reference: settlement.provider_reference,
  ...
  }).catch((error) => console.error("BUSINESS_PARTNER_PAYMENT_EVENT_ADAPTER_FAILED", error));
