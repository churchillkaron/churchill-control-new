...
export const FinanceBankReconciliationVerifier = {
  ...
  async verify({ payment, sourceReference }) {
    ...
    const bankingProvider = cleanText(
      integration.provider_code || integration.provider_name,
    );
    if (!bankingProvider) {
      throw new Error("PAYMENT_SETTLEMENT_PROVIDER_REQUIRED");
    }

    const paymentProvider = cleanText(payment.provider);
    if (!paymentProvider) {
      throw new Error("PAYMENT_PROVIDER_REQUIRED");
    }

    const bankReconciledRail = ["bank_transfer", "promptpay"].includes(
      paymentProvider.toLowerCase(),
    );
    if (!bankReconciledRail && paymentProvider !== bankingProvider) {
