...
import { serializeCapability } from "./serializeCapability.js";
...
function declaredCreateEndpoint(item) {
  ...
  const api = text(create.api);
  if (api) return { endpoint: api, source: "create.api" };

  const endpoint = text(create.endpoint);
  if (endpoint) return { endpoint, source: "create.endpoint" };

  return { endpoint: null, source: null };
}
  ...
  for (const [domain, workspace] of Object.entries(registry?.workspaces || {})) {
    for (const group of workspace?.groups || []) {
      for (const item of group?.items || []) {
        if (
          !item?.id ||
          item?.create?.enabled !== true ||
          item?.create?.operatorEnabled === false
        ) continue;

        const declared = declaredCreateEndpoint(item);
        const fallbackEndpoint = collectionCreateEndpoint(item);
        const endpoint = declared.endpoint || fallbackEndpoint;
        ...
        if (endpoint) {
          coverage.push({
            ...
            endpoint,
            endpoint_source: declared.source || "safe_collection_fallback",
