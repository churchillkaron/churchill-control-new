...
export const ERP_REGISTRY = {
  ...
  workspaces: {
    ...
    finance: {
      ...
      groups: [
          ...
          items: [
            { id:  "customers", name: "Customers", route: "/finance/customers", description: "Manage finance customer records.", order: 10,
              ...
              ui: {
                ...
                search: ["customer_name","customer_email","customer_phone","tax_id","tier","status"],
                name: r => r.customer_name || r.name || "Unnamed Customer",
                subtitle: r => [
              ...
              actions: [
...

                {
                  ...
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/customer-invoices/new?customerId=${row.id}`
                },

                {
                  label:"Invoices",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/customer-invoices?customerId=${row.id}`
                },

                {
                  label:"New Payment",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/customer-payments/new?customerId=${row.id}`
                },

                {
                  label:"Credit Notes",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/customer-invoices?customerId=${row.id}&type=credit-note`
                },
...

                {
                  ...
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/ar?customerId=${row.id}`
                },

                {
                  label:"Collections",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/ar/collections?customerId=${row.id}`
                },
...

                {
                  label:"Loyalty",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/commercial/customers/loyalty?customerId=${row.id}`
                },

                {
                  label:"Timeline",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/customers/timeline?customerId=${row.id}`
                },

                {
                  label:"Documents",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/documents?customerId=${row.id}`
                },
          ...
          items: [
            { id:  "vendors", name: "Vendors", route: "/finance/vendors", description: "Manage vendor finance records.", order: 10,
              ...
              ui: {
                ...
                search: ["vendor_name","name","legal_name","vendor_email","email","vendor_phone","phone","tax_id","category","payment_terms","status"],
                name: r => r.vendor_name || r.name || r.legal_name || "Unnamed Vendor",
                subtitle: r => [
                  r.vendor_code || r.code || "-",
                  r.vendor_email || r.email || "-",
                  r.vendor_phone || r.phone || "-"
                ],
              } },
        ...
        },
        {
          ...
          order: 40,
          items: [
            { id:  "bank_accounts", name: "Bank Accounts", route: "/finance/bank-accounts", description: "Manage bank accounts.", order: 10,

              ui: {
                api: "/api/finance/bank-accounts",
                rowsKey: "bankAccounts",
                primaryActionLabel: "+ Bank Account",
                search: ["bank_name","account_name","account_number","currency_code","branch_name"],
                name: r => r.account_name || "Unnamed Account",
                subtitle: r => [
                  r.bank_name || "-",
                  r.account_number || "-"
                ],
              ...
              actions: [
...

                {
                  label:"Transactions",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/treasury/transactions?accountId=${row.id}`
                },

                {
                  label:"Payments",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/treasury/payments?accountId=${row.id}`
                },

                {
                  label:"Reconciliation",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/treasury/reconciliation?accountId=${row.id}`
                },

                {
                  label:"Statement",
                  href:({organizationId,row})=>
                    `/workspace/${organizationId}/finance/treasury/statements?accountId=${row.id}`
                },
        ...
        },
        {
          ...
          items: [
            { id:  "legal_entities", name: "Legal Entities", route: "/finance/entities", description: "Manage legal entities.", order: 10,
              ...
              ui: {
                api: "/api/finance/legal-entities",
                rowsKey: "entities",
                primaryActionLabel: "+ Legal Entity",
                search: ["name","legal_name","code","country","tax_number","tax_id","registration_number"],
                name: r => r.name || r.legal_name || "Unnamed Entity",
                subtitle: r => [
                  r.country || "-",
                  r.code || "-"
                ],
            ...
            { id:  "cost_centers", name: "Cost Centers", route: "/finance/cost-centers", description: "Manage cost centers.", order: 20,
              ...
              ui: {
                ...
                rowsKey: "costCenters",
                primaryActionLabel: "+ Cost Center",
                search: ["name","code","manager_name","department","description"],
                name: r => r.name || "Unnamed Cost Center",
                subtitle: r => [
                  r.code || "-",
                  r.department || "-"
                ],
    ...
    services: {
      title: "Services",
      description: "Manage platform integrations, wallet, usage, billing and service operations.",
      groups: [
        {
          id: "platform",
          name: "Platform",
          ...
          items: [
            {
              ...
              description: "Manage connected platform services.",
              order: 10,
              status: "active",
            },
            {
            ...
            },
            {
              ...
              description: "Review platform usage.",
              order: 30,
              status: "active",
            },
            {
            ...
            },
            {
              ...
              description: "Execution policies.",
              order: 70,
              status: "planned",
            },
            {
