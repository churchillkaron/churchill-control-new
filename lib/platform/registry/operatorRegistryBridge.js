import { defineCapability } from "../../ubte/runtime/contracts/CapabilityManifest.js";
import { attachActionIdentityEvidence } from "@/lib/operator/runtime/ActionIdentityEvidenceRuntime.mjs";
import { operatorMissionDispatchKey } from "../../operator/runtime/OperatorMissionDispatchIdentityRuntime.js";
...
function declaredCreateEndpoint(item) {
  return text(item?.create?.api || item?.create?.endpoint) || null;
}
...
function missionIdempotencyKey({ context, payload, domainKey, capabilityKey }) {
  if (text(payload?.idempotency_key)) return text(payload.idempotency_key);
  const metadata = context?.metadata || {};
  const journaledMissionChild =
    text(metadata.parentCapabilityKey) === "platform.operator_mission.execute" &&
    text(metadata.operatorMissionExecutionId) &&
    text(metadata.missionStepId) &&
    text(metadata.source) === "AVANTIQO_OPERATOR_MISSION";
  const confirmedSingleAction =
    text(metadata.source) === "AVANTIQO_OPERATOR" &&
    text(metadata.operatorRunId) &&
    metadata.operatorConfirmedAction === true &&
    metadata.conversationallyConfirmed === true;
  if (!journaledMissionChild && !confirmedSingleAction) return null;

  return operatorMissionDispatchKey({
    organizationId: context?.organizationId,
    missionExecutionId: journaledMissionChild
      ? metadata.operatorMissionExecutionId
      : metadata.operatorRunId,
    missionStepId: journaledMissionChild ? metadata.missionStepId : "requested_action",
    capabilityKey: `${domainKey}.${capabilityKey}.create`,
    payload,
  });
}
...
function boundCreate(item) {
  ...
  return { capability, action, api: declaredCreateEndpoint(item), label: text(create.label) || null };
}

function createOperatorAliases(item) {
  const title = text(item?.create?.title || item?.name || item?.create?.label)
    .replace(/^\+\s*/, "")
    .trim();
  if (!title) return [];
  ...
  return [
    ...
    `make a ${lower}`,
    `make an ${lower}`,
    ...(lower === "quotation" ? ["create quote", "make quote", "create a quote", "make a quote"] : []),
  ];
}

function createPermissions(domain, item, group = null) {
  const declared = Array.isArray(item?.create?.permissions)
    ? item.create.permissions.map(text).filter(Boolean)
    : [text(item?.create?.permission)].filter(Boolean);
  if (declared.length) return declared;

  const domainKey = ubteName(domain);
  const groupKey = ubteName(group);
  if (domainKey === "finance") return ["finance.manage"];
  if (domainKey === "administration") return ["management.manage"];
  if (domainKey === "commercial") return ["management.manage"];
  if (domainKey === "supply_chain") {
    return groupKey === "procurement"
      ? ["procurement.manage"]
      : ["inventory.manage"];
  }

  return [`${domainKey}.${ubteName(item.id)}.create`];
}
...
export function createRegistryReadCapability({ domain, item, group = null }) {
  ...
  const identityField = text(item?.data?.identity) || null;
  const manifest = defineCapability({
    ...
    inputSchema: {
      type: "object",
      properties: {
        ...(identityField ? { [identityField]: { type: "string" } } : {}),
...
export function createRegistryCreateCapability({ domain, item, group = null, endpoint: resolved }) {
  ...
  const endpoint = text(resolved) || declaredCreateEndpoint(item);
  ...
  const identityField = text(item?.data?.identity) || null;
  const declaredRisk = text(item?.create?.risk).toLowerCase();
  const risk = declaredRisk || (domainKey === "finance" ? "high" : "medium");
  const manifest = defineCapability({
    domain: domainKey,
    ...
    permissions: createPermissions(domain, item, group),
    events: [],
    tags: [ubteName(domain), ubteName(item.id), "registry", "write"],
    operatorAliases: createOperatorAliases(item),
    operatorExamples: createOperatorAliases(item),
    transactional: true,
    aiEnabled: false,
    operatorEnabled: true,
    operatorMode: "write",
    ...
    ...(identityField ? {
      operatorVerification: {
        capability_key: `${domainKey}.${capabilityKey}.read`,
        payload_from_result: { [identityField]: [identityField, "id"] },
        recovery_payload_from_evidence: { idempotency_key: "idempotency_key" },
        derivation: "registry_declared_result_identity_with_idempotency_recovery",
      },
    } : {}),
    ...(scope ? { contextScope: scope } : {}),
    registryRoute: text(item.route) || null,
    registryEndpoint: endpoint,
    inputSchema:
      item?.create?.inputSchema ||
      item?.create?.input_schema ||
      {
        type: "object",
        properties: {},
        additionalProperties: true,
  ...
  async function execute({ context, payload = {} }) {
    ...
    const cookie = callerCookie(context);
    const declaredInputSchema = item?.create?.inputSchema || item?.create?.input_schema || null;
    const acceptsIdempotencyKey = Boolean(declaredInputSchema?.properties?.idempotency_key);
    const derivedIdempotencyKey = acceptsIdempotencyKey
      ? missionIdempotencyKey({ context, payload, domainKey, capabilityKey })
      : null;
    const requestPayload = derivedIdempotencyKey && !text(payload?.idempotency_key)
      ? { ...payload, idempotency_key: derivedIdempotencyKey }
      : payload;
    ...
    if (!response.ok || body?.success === false) {
      ...
      error.status = response.status;
      throw attachActionIdentityEvidence(error, Array.isArray(body?.action_identity_evidence) ? body.action_identity_evidence : []);
    }
      ...
      for (const item of group?.items || []) {
        ...
        if (item?.create?.enabled) {
          const createEndpoint =
            declaredCreateEndpoint(item) || collectionCreateEndpoint(item) || "";
          ...
          // never shadowed.
          const authoritativeIdentity = text(item?.data?.identity);
          if (createEndpoint && authoritativeIdentity) {
            let cachedCreate;
            domains[domainKey][capabilityKey].create = async () => {
              if (!cachedCreate) {
                cachedCreate = createRegistryCreateCapability({
                  domain,
                  item,
                  group: group?.id || group?.name || null,
                  endpoint: createEndpoint,
                });
              }
              return cachedCreate;
            };
          } else if (createEndpoint && !authoritativeIdentity) {
            skippedCreates.push(`${domain}.${item.id}:missing_authoritative_identity`);
