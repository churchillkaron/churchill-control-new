...
} from "../../finance/ui/FinancePrimaryActionPolicy.js";
import {
  getFinanceWorkspaceContract,
} from "../../finance/workspaces/FinanceWorkspaceContracts.js";
import {
  resolveFinanceWorkspaceMutationPolicy,
} from "../../finance/workspaces/FinanceWorkspaceMutationPolicy.js";
...
function createLabel(capability, contract) {
  ...
  return `+ ${humanDocumentName(capability.document || capability.name)}`;
}
...
function mutationRowMenu(capability, contract, endpoint) {
  ...
  const name = humanDocumentName(capability.document || capability.name || "Record");
  const mutationEndpoint = contract.mutationEndpoint || endpoint;
  const mutationMethod = contract.mutationMethod || "PATCH";
  const actions = [
  ...
  if (mutation.duplicable) {
    actions.push({
      ...
      engine: "finance_record_mutation",
      schema: contract.schema,
      endpoint: contract.createEndpoint || endpoint,
      method: contract.createMethod || "POST",
      document: capability.document,
  ...
  actions.push(...actionList(contract.rowActions));
...
function normalizeClosedFinanceWorkspace(capability, contract) {
  ...
  const primaryPolicy = getFinancePrimaryActionPolicy(capability.id);
  const explicitCreate = primaryPolicy?.mode === "create" ? primaryPolicy.create || null : null;
  const contractCreateEndpoint = contract.createEndpoint || null;
  const contractCreateMethod = contract.createMethod || "POST";
  ...
  const defaultCreate = {
    ...
    api: isDocumentTemplate
      ...
      : contractCreateEndpoint || endpoint,
    endpoint: isDocumentTemplate
      ? "/api/finance/document-templates"
      : contractCreateEndpoint || endpoint,
    method: isDocumentTemplate ? "POST" : contractCreateMethod,
    label: createLabel(capability, contract),
    title: isDocumentTemplate
      ? "Create Document Template"
      : contract.singleton
        ? `Configure ${capability.name}`
        : `New ${humanDocumentName(capability.document || capability.name)}`,
  };
  ...
  const create = writable
    ? {
        ...
        ...(explicitCreate || {}),
        enabled: true,
        schema: explicitCreate?.schema || defaultCreate.schema,
      }
