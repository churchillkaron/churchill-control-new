...
function mutationRowMenu(capability, contract, endpoint) {
  const mutation = resolveFinanceWorkspaceMutationPolicy(
    capability.id,
    contract
  );
  const actions = [
  ...
  if (mutation.editable) {
    actions.push({
      ...
      label: `Edit ${capability.document || capability.name}`,
      title: `Edit ${capability.document || capability.name}`,
    });
  }
  ...
  if (mutation.duplicable) {
    actions.push({
      ...
      method: "POST",
      document: capability.document,
      label: `Duplicate ${capability.document || capability.name}`,
      title: `Duplicate ${capability.document || capability.name}`,
    });
  }
  ...
  if (mutation.archivable) {
    actions.push({
      ...
      method: "DELETE",
      danger: true,
      label: `Archive ${capability.document || capability.name}`,
    });
  }
