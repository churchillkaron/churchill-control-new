...
function normalizeFinanceChartOfAccountsCapability(capability) {
  if (capability?.id !== "chart_of_accounts") return capability;
  ...
  const create = {
    ...
    endpoint: "/api/finance/chart-of-accounts/upsert",
    label: "+ Account",
    title: "New Account",
  ...
  const rowMenu = [
    ...
    },
    {
      ...
      endpoint: "/api/finance/chart-of-accounts/delete",
      method: "POST",
      danger: true,
    },
  ];
  ...
  const kpis = [
    { label: "Accounts", key: "metrics.totalCount", hint: "Entity accounts" },
    { label: "Active", key: "metrics.activeCount", hint: "Available for posting" },
    { label: "Inactive", key: "metrics.inactiveCount", hint: "Archived accounts" },
    { label: "System", key: "metrics.systemCount", hint: "Protected accounts" },
  ];
  ...
  return {
    ...
    runtime: {
      ...
      createApi: "/api/finance/chart-of-accounts/upsert",
    },
    ui: {
      ...(capability.ui || {}),
    ...
    data: {
      ...
      capability: "account",
      scope: "entity",
      primaryTable: "chart_of_accounts",
    },
  };
}
...
function normalizeFinanceCapability(capability) {
  ...
  capability = normalizeFinanceChartOfAccountsCapability(capability);

  const closedContract = getFinanceWorkspaceContract(capability.id);
  if (closedContract) {
    capability = normalizeClosedFinanceWorkspace(capability, closedContract);
  }
  ...
  if (policy.mode === "create") {
    ...
    const topMenu = capability.hideCreateInTopMenu
      ? dedupeActions(currentTopMenu)
      : dedupeActions([
          { id: "new", type: "create" },
          ...currentTopMenu,
        ]);
    return {
      ...
      runtime: {
        ...(capability.runtime || {}),
        createApi: create.endpoint || create.api || capability.runtime?.createApi || null,
      },
      topMenu,
      ui: { ...(capability.ui || {}), topMenu },
    };
...
function hasExecutableCreate(capability) {
  ...
  return Boolean(
    create?.enabled === true &&
    (create?.form || create?.schema?.length || create?.api || create?.endpoint)
  );
}
...
function hasExecutableAction(capability) {
  ...
  ).some(action => action?.endpoint || action?.api || action?.engine || action?.type === "report");
}
...
function applyFinanceRuntimeManifest(capability) {
  ...
  return {
    ...
    contextScope: runtimeDefinition.scope || capability.contextScope || "entity",
    ...
    ui: {
      ...
      ...(runtimeDefinition.rowsKey ? { rowsKey: runtimeDefinition.rowsKey } : {}),
    },
  };
}
