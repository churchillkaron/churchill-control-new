...
} from "../../../intelligence/runtime/AvantiqoStructuredIntelligenceSupervisorRuntime.js";
import {
  persistAvantiqoEvidenceGraphSnapshot,
} from "../../../intelligence/runtime/AvantiqoEvidenceGraphRuntime.js";

export const OPERATOR_RESEARCH_EVIDENCE_COMPARISON_CONTRACT =
  "AVANTIQO_RESEARCH_EVIDENCE_COMPARISON_V2";
...
function evidenceTopicKey(context, payload, question) {
  const metadata = object(context?.metadata);
  const explicit = text(
    payload?.topic_key || metadata.topic_key || metadata.learning_topic_key,
    240,
  );
...
function comparisonSystem() {
  return [
    ...
    "Keep the response compact: conclusion at most 700 characters; at most 6 claims with each claim at most 360 characters; at most 3 conflicts; exactly one compact source assessment per supplied source with notes at most 120 characters; at most 4 uncertainty items; and at most 3 recommended next-research items.",
    "Do not repeat source excerpts or restate the full question. recommended_next_research must only describe evidence gaps; it must not authorize actions.",
  ].join("\n");
}
...
export async function compareOperatorResearchEvidence({
  ...
  const result = await AvantiqoStructuredIntelligenceSupervisorRuntime.run({
    ...
    metadata: {
      ...
      continuous_learning_dependency: true,
    ...
    critique_instructions: [
      ...
      "Keep the final machine-boundary JSON inside the compact output limits.",
    ].join(" "),
  ...
  const continuousLearning = context?.metadata?.continuous_learning === true;
  let evidenceGraph = null;
  if (continuousLearning) {
    const metadata = object(context.metadata);
    evidenceGraph = await persistAvantiqoEvidenceGraphSnapshot({
      organizationId,
      topic: {
        key: evidenceTopicKey(context, payload, question),
        query: question,
        domain: payload.domain || metadata.knowledge_domain || metadata.domain || null,
        jurisdiction: payload.jurisdiction || metadata.jurisdiction || null,
        stability: payload.stability || metadata.stability || "stable",
        priority: payload.priority ?? metadata.priority ?? 0.7,
        review_interval_days:
          payload.review_interval_days ?? metadata.review_interval_days ?? 120,
      },
      research: {
        contract: text(payload.research_contract, 180) || null,
        query: question,
      },
      comparison,
      sources,
  ...
  return {
    ...comparison,
    evidence_graph: continuousLearning
      ? {
          persisted: evidenceGraph?.persisted === true,
          contract: evidenceGraph?.contract || null,
          status: evidenceGraph?.status || null,
          conflicted_claim_count: Number(evidenceGraph?.conflicted_claim_count || 0),
          supported_claim_count: Number(evidenceGraph?.supported_claim_count || 0),
        }
      : {
          persisted: false,
          reason: "NOT_CONTINUOUS_LEARNING",
        },
  };
}
