...
export async function readOperatorWeather({ payload = {} } = {}) {
  ...
  const suppliedLatitude = Number(payload.latitude);
  const suppliedLongitude = Number(payload.longitude);
  const coordinatesProvided = Number.isFinite(suppliedLatitude) && Number.isFinite(suppliedLongitude) && suppliedLatitude >= -90 && suppliedLatitude <= 90 && suppliedLongitude >= -180 && suppliedLongitude <= 180;
  ...
  if (coordinatesProvided) {
    place = {
      name: text(payload.location_label, 160) || "Current location",
      admin1: null, country: null,
    ...
    if (!place || !Number.isFinite(Number(place.latitude)) || !Number.isFinite(Number(place.longitude))) {
      return {
        status: "CLARIFICATION_REQUIRED", missing: ["location"],
