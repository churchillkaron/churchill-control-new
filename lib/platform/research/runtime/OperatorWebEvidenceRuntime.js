import { ServiceExecutionRuntime } from "../../service-runtime/execution/ServiceExecutionRuntime.js";

export const OPERATOR_WEB_EVIDENCE_CONTRACT = "AVANTIQO_GOVERNED_WEB_EVIDENCE_V1";
...
- For technical questions, prefer sources that expose mechanisms, specifications, measurements, constraints, failure modes and primary research over snippets of implementation code.
- Existing code can be evidence about an approach, but the evidence collector must not assume an existing implementation defines the only possible architecture.
...
export async function runOperatorWebEvidenceResearch({
  context = {},
  payload = {},
} = {}) {
  ...
  const result = await ServiceExecutionRuntime.execute({
    ...
    metadata: {
      research_contract: OPERATOR_WEB_EVIDENCE_CONTRACT,
  ...
  return {
    contract: OPERATOR_WEB_EVIDENCE_CONTRACT,
...
export default runOperatorWebEvidenceResearch;
