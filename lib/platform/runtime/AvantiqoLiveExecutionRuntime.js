...
export async function publishAvantiqoLiveExecution({
  ...
  const expectedExecutionId = text(executionId, 200);
  ...
  if (!Object.keys(previousState).length) {
    if (expectedExecutionId) {
      return {
        persisted: false,
        ignored: true,
        reason: "AVANTIQO_LIVE_EXECUTION_ID_MISMATCH",
        execution_id: null,
      };
    }
  ...
  if (
    expectedExecutionId &&
    text(previousState.execution_id, 200) !== expectedExecutionId
  ) {
    return {
      persisted: false,
      ignored: true,
      reason: "AVANTIQO_LIVE_EXECUTION_ID_MISMATCH",
      execution_id: previousState.execution_id || null,
    };
  }
  const compact = compactEvent(event);
  const terminal = ["completed", "failed", "cancelled", "blocked", "stopped"].includes(
...
export async function requestAvantiqoLiveExecutionStop({
  context = {},
  executionId = null,
} = {}) {
  const expectedExecutionId = text(executionId, 200);
  if (!expectedExecutionId) {
    return { requested: false, reason: "AVANTIQO_LIVE_EXECUTION_ID_REQUIRED" };
  }
  ...
  if (text(previous.execution_id, 200) !== expectedExecutionId) {
    return {
      requested: false,
      reason: "AVANTIQO_LIVE_EXECUTION_ID_MISMATCH",
...
export async function avantiqoLiveExecutionStopRequested({
  context = {},
  executionId = null,
} = {}) {
  const loaded = await loadAvantiqoLiveExecution({ context });
  if (loaded.found !== true) return false;
  const expectedExecutionId = text(executionId, 200);
  if (
    expectedExecutionId &&
    text(loaded.live_execution?.execution_id, 200) !== expectedExecutionId
  ) {
    return false;
  }
  return loaded.live_execution?.stop_requested === true;
}

export async function assertAvantiqoLiveExecutionCurrent({
  context = {},
  executionId = null,
  error_code = "AVANTIQO_LIVE_EXECUTION_SUPERSEDED",
} = {}) {
  const expectedExecutionId = text(executionId, 200);
  if (!expectedExecutionId) return { current: true, checked: false };
  const loaded = await loadAvantiqoLiveExecution({ context });
  const currentExecutionId = text(loaded?.live_execution?.execution_id, 200);
  if (loaded?.found !== true || currentExecutionId !== expectedExecutionId) {
    const error = new Error(error_code);
    error.code = error_code;
    error.expected_execution_id = expectedExecutionId;
    error.current_execution_id = currentExecutionId || null;
    error.authorization_effect = "NONE";
    error.mutation_executed = false;
    throw error;
  }
  return { current: true, checked: true, execution_id: currentExecutionId };
}
...
export async function assertAvantiqoLiveExecutionContinue({
  ...
  if (await avantiqoLiveExecutionStopRequested({ context, executionId })) {
