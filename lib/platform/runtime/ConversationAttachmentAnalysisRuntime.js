...
const UNIVERSAL_VISUAL_ANALYSIS_INSTRUCTION = [
  ...
  "For recipes or recipe cards, preserve dish_code/menu_item_code when visibly present. Preserve recipe_output_quantity and recipe_output_uom when a batch/preparation yield is explicitly shown. Return ingredients/recipe_items with quantity, unit/UOM, usable yield percentage when explicitly present, and exactly one visible item_code/SKU or component_recipe_code/sub_recipe_code/preparation_code when the line explicitly references a reusable preparation. Never invent an inventory or component recipe code from a name, never infer a unit that is not evidenced, and never invent a yield percentage or batch output.",
