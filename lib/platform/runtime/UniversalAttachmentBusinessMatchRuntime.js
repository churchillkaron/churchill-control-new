...
async function matchCustomerParty({ fields, organizationId }) {
  const customerNumber = value(fields, ["customer_number", "customer_no", "account_number"]);
  const taxId = value(fields, ["tax_id", "tax_number", "vat_number"]);
  const email = value(fields, ["customer_email", "email", "contact_email"]);
  if (!customerNumber && !taxId && !email) return null;

  let partyIds = [];
  let basis = null;
  if (customerNumber) {
    const result = await supabaseAdmin.from("customer_profiles")
      .select("party_id,customer_number").eq("organization_id", organizationId).eq("customer_number", customerNumber).limit(8);
    if (result.error) throw result.error;
    partyIds = (result.data || []).map((row) => row.party_id).filter(Boolean);
    basis = "customer_number";
  } else {
    let query = supabaseAdmin.from("parties")
      .select("id,display_name,email,tax_id").eq("organization_id", organizationId).limit(20);
    if (taxId) { query = query.eq("tax_id", taxId); basis = "tax_id"; }
    else { query = query.eq("email", email); basis = "customer_email"; }
    const result = await query;
    if (result.error) throw result.error;
    partyIds = (result.data || []).map((row) => row.id).filter(Boolean);
  }
  if (!partyIds.length) return baseResult("NO_MATCH", { match_basis: [basis] });
  const relationships = await supabaseAdmin.from("party_relationships")
    .select("party_id,status").eq("organization_id", organizationId).eq("relationship_type", "customer").in("party_id", partyIds).limit(20);
  if (relationships.error) throw relationships.error;
  const customerIds = new Set((relationships.data || []).filter((row) => normalized(row.status) !== "archived").map((row) => row.party_id));
  if (!customerIds.size) return baseResult("NO_MATCH", { match_basis: [basis] });
  const parties = await supabaseAdmin.from("parties").select("id,display_name,email,tax_id").eq("organization_id", organizationId).in("id", [...customerIds]).limit(8);
  if (parties.error) throw parties.error;
  return statusFromCandidates(
    (parties.data || []).map((row) => candidate("customer", row, row.display_name || row.email, [basis, "customer_relationship"])),
    [basis, "customer_relationship"],
  );
}

async function matchSupplierParty({ fields, organizationId }) {
  const vendorCode = value(fields, ["vendor_code", "supplier_code", "vendor_number", "supplier_number"]);
  const taxId = value(fields, ["supplier_tax_id", "vendor_tax_id", "tax_id", "tax_number", "vat_number"]);
  const email = value(fields, ["supplier_email", "vendor_email", "email", "contact_email"]);
  if (!vendorCode && !taxId && !email) return null;
  let partyIds = []; let basis = null;
  if (vendorCode) {
    const result = await supabaseAdmin.from("supplier_profiles").select("party_id,vendor_code,is_active,is_blocked")
      .eq("organization_id", organizationId).eq("vendor_code", vendorCode).limit(8);
    if (result.error) throw result.error;
    partyIds = (result.data || []).filter((row) => row.is_active !== false && row.is_blocked !== true).map((row) => row.party_id).filter(Boolean);
    basis = "vendor_code";
  } else {
    let query = supabaseAdmin.from("parties").select("id,display_name,email,tax_id,status").eq("organization_id", organizationId).limit(20);
    if (taxId) { query = query.eq("tax_id", taxId); basis = "supplier_tax_id"; }
    else { query = query.eq("email", email); basis = "supplier_email"; }
    const result = await query; if (result.error) throw result.error; partyIds = (result.data || []).map((row) => row.id).filter(Boolean);
  }
  if (!partyIds.length) return baseResult("NO_MATCH", { match_basis: [basis] });
  const profiles = await supabaseAdmin.from("supplier_profiles").select("id,party_id,vendor_code,is_active,is_blocked")
    .eq("organization_id", organizationId).in("party_id", partyIds).limit(20);
  if (profiles.error) throw profiles.error;
  const eligible = (profiles.data || []).filter((row) => row.is_active !== false && row.is_blocked !== true);
  if (!eligible.length) return baseResult("NO_MATCH", { match_basis: [basis] });
  const parties = await supabaseAdmin.from("parties").select("id,display_name,legal_name,email,tax_id").eq("organization_id", organizationId)
    .in("id", eligible.map((row) => row.party_id)).limit(8);
  if (parties.error) throw parties.error;
  return statusFromCandidates((parties.data || []).map((row) => candidate("supplier", row, row.display_name || row.legal_name || row.email, [basis, "supplier_profile"])), [basis, "supplier_profile"]);
}

async function matchVendorInvoice({ fields, organizationId, entityId, vendorPartyId = null }) {
  const invoiceNumber = value(fields, ["supplier_invoice_number", "vendor_invoice_number", "invoice_number", "invoice_no", "document_number"]);
  if (!invoiceNumber || !entityId || !vendorPartyId) return null;
  let query = supabaseAdmin.from("vendor_invoices").select("id,invoice_number,invoice_date,total_amount,currency_code,status,vendor_party_id")
    .eq("organization_id", organizationId).eq("entity_id", entityId).eq("invoice_number", invoiceNumber).limit(8);
  query = query.eq("vendor_party_id", vendorPartyId);
  const { data, error } = await query; if (error) throw error;
  return statusFromCandidates((data || []).map((row) => candidate("vendor_invoice", row, row.invoice_number, ["invoice_number", "entity_id", ...(vendorPartyId ? ["vendor_party_id"] : [])])), ["invoice_number", "entity_id", ...(vendorPartyId ? ["vendor_party_id"] : [])]);
}

async function matchPurchaseOrder({ fields, organizationId, entityId, supplierPartyId = null }) {
  const reference = value(fields, ["purchase_order_number", "po_number", "purchase_order_reference", "order_number", "source_reference"]);
  if (!reference || !entityId || !supplierPartyId) return null;
  let query = supabaseAdmin.from("purchase_orders")
    .select("id,po_number,source_reference,supplier_party_id,status,total_amount,currency,entity_id")
    .eq("organization_id", organizationId).eq("entity_id", entityId).eq("supplier_party_id", supplierPartyId)
    .or(`po_number.eq.${reference},source_reference.eq.${reference}`).limit(8);
  let result = await query;
  if (result.error && String(result.error.message || "").includes("source_reference")) {
    result = await supabaseAdmin.from("purchase_orders")
      .select("id,po_number,supplier_party_id,status,total_amount,currency,entity_id")
      .eq("organization_id", organizationId).eq("entity_id", entityId).eq("supplier_party_id", supplierPartyId)
      .eq("po_number", reference).limit(8);
  }
  if (result.error) throw result.error;
  return statusFromCandidates((result.data || []).map((row) => candidate("purchase_order", row, row.po_number || reference, ["purchase_order_reference", "supplier_party_id", "entity_id"])), ["purchase_order_reference", "supplier_party_id", "entity_id"]);
}

async function matchPaidExpenseReceipt({ file, organizationId, entityId }) {
  const sha = text(file.sha256, 128);
  if (!sha || !entityId) return null;
  const { data, error } = await supabaseAdmin.from("finance_expense_receipts")
    .select("id,receipt_number,receipt_date,total_amount,currency_code,status,evidence_checksum")
    .eq("organization_id", organizationId)
    .eq("entity_id", entityId)
    .eq("evidence_checksum", sha)
    .limit(8);
  if (error) {
    if (String(error.message || "").includes("finance_expense_receipts")) return null;
    throw error;
  }
  return statusFromCandidates(
    (data || []).map((row) => candidate("finance_expense_receipt", row, row.receipt_number || `Expense receipt ${row.receipt_date}`, ["evidence_checksum", "entity_id"])),
    ["evidence_checksum", "entity_id"],
  );
}
...
export async function matchAnalyzedAttachmentToBusiness({ file = {}, organizationId, entityId = null } = {}) {
  ...
  const financialDocument = /receipt|invoice|purchase_order|goods_receipt|delivery_note/.test(kind);
  if (/receipt|cash_purchase|card_purchase|expense/.test(kind)) {
    const postedExpense = await matchPaidExpenseReceipt({ file, organizationId, entityId });
    if (postedExpense && postedExpense.status !== "NO_MATCH") return postedExpense;
  }

  const exactDocument = await matchControlledDocument({ file, organizationId, entityId });
  if (exactDocument && !financialDocument) return exactDocument;

  const candidateDomains = domains(file);
  const attempts = [];

  if (/supplier|vendor/.test(kind) && !/invoice|bill|purchase_order|purchase order|delivery_note|goods_receipt/.test(kind)) {
    const supplier = await matchSupplierParty({ fields, organizationId });
    if (supplier && supplier.status !== "NO_MATCH") return supplier;
  }
  if (/purchase_order|purchase order/.test(kind)) {
    const supplier = await matchSupplierParty({ fields, organizationId });
    const supplierPartyId = supplier?.status === "UNIQUE_MATCH" ? supplier.candidates?.[0]?.record_id || null : null;
    const existingOrder = await matchPurchaseOrder({ fields, organizationId, entityId, supplierPartyId });
    if (existingOrder && existingOrder.status !== "NO_MATCH") return existingOrder;
    if (supplier && supplier.status !== "NO_MATCH") return supplier;
  }
  if (/supplier|vendor/.test(kind) && /invoice|bill/.test(kind)) {
    const supplier = await matchSupplierParty({ fields, organizationId });
    const vendorPartyId = supplier?.status === "UNIQUE_MATCH" ? supplier.candidates?.[0]?.record_id || null : null;
    const existingBill = await matchVendorInvoice({ fields, organizationId, entityId, vendorPartyId });
    if (existingBill && existingBill.status !== "NO_MATCH") return existingBill;
    if (supplier && supplier.status !== "NO_MATCH") return supplier;
  }
  ...
  if (candidateDomains.includes("commercial") || /customer|client|customer_profile|customer_onboarding/.test(kind)) {
    attempts.push(() => matchCustomerParty({ fields, organizationId }));
  }
