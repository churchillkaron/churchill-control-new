function domainFromRegistry(registry = {}, value) {
  const target = normalized(value);
  const domains = Array.isArray(registry?.domains) ? registry.domains : [];
  return domains.find((item) => {
    const id = normalized(item?.id);
    const name = normalized(item?.name);
    return target === id || target === name;
  }) || null;
}

function registryWorkspace(registry = {}, workspaceId) {
  return registry?.workspaces?.[workspaceId] || null;
}

function registryItem(registry = {}, { workspaceId, groupId = null, itemId }) {
  const workspace = registryWorkspace(registry, workspaceId);
  if (!workspace) return null;
  for (const group of workspace.groups || []) {
    if (groupId && normalized(group.id) !== normalized(groupId)) continue;
    for (const item of group.items || []) {
      if (normalized(item.id) !== normalized(itemId)) continue;
      const domain = domainFromRegistry(registry, workspaceId);
      return {
        domain: domain?.name || workspace.title || workspaceId,
        domain_id: domain?.id || workspaceId,
        workspace: workspaceId,
        workspace_name: workspace.title || null,
        group: group.name || group.id || null,
        group_id: group.id || null,
        item_id: item.id,
        route: item.route || domain?.route || null,
        label: item.name || workspace.title || domain?.name || item.id,
      };
    }
  }
  return null;
}

function registryDomainDestination(registry = {}, value) {
  const domain = domainFromRegistry(registry, value);
  if (!domain) return null;
  const workspace = registryWorkspace(registry, domain.id);
  return {
    domain: domain.name,
    domain_id: domain.id,
    workspace: domain.id,
    workspace_name: workspace?.title || domain.name,
    group: null,
    group_id: null,
    item_id: null,
    route: domain.route || null,
    label: workspace?.title || domain.name,
  };
}

function text(value) {
  return String(value ?? "").trim();
}
function object(value) {
  return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}
function list(value) {
  return Array.isArray(value) ? value : [];
}
function normalized(value) {
  return text(value).toLowerCase().replace(/[_-]+/g, " ").replace(/\s+/g, " ");
}
function assetDestination(evidence = {}, registry = {}) {
  const asset = object(evidence.asset_details);
  const joined = [
    evidence.object_type,
    evidence.document_type,
    asset.type,
    asset.asset_type,
    asset.category,
  ].map(normalized).join(" ");
  if (!/\b(asset|equipment|vehicle|property|domain|ssl|software account|digital asset|device|machine|tool|fleet|building)\b/.test(joined)) return null;
  if (/\b(vehicle|car|truck|motorcycle|trailer|fleet)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "compliance", groupId: "assets", itemId: "vehicles" });
  }
  if (/\b(property|building|leasehold|premises|real estate)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "compliance", groupId: "assets", itemId: "properties" });
  }
  if (/\b(domain|ssl|software account|digital asset)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "compliance", groupId: "assets", itemId: "digital_assets" });
  }
  if (/\b(equipment|device|machine|tool|asset)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "compliance", groupId: "assets", itemId: "equipment" });
  }
  return null;
}

function customerDestination(evidence = {}, registry = {}) {
  const joined = [evidence.object_type, evidence.document_type, ...list(evidence.candidate_domains)].map(normalized).join(" ");
  if (!/\b(customer|client|customer onboarding|customer profile|customer master)\b/.test(joined)) return null;
  return registryItem(registry, { workspaceId: "commercial", groupId: "customer_management", itemId: "customers" });
}

function financeDestination(evidence = {}, registry = {}) {
  const joined = [
    evidence.object_type,
    evidence.document_type,
    evidence.title,
    ...list(evidence.candidate_domains),
  ].map(normalized).join(" ");

  if (/\b(customer invoice|sales invoice|accounts receivable invoice|ar invoice)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "finance", groupId: "order_to_cash", itemId: "customer_invoices" });
  }
  if (/\b(vendor bill|supplier invoice|purchase invoice|accounts payable invoice|ap invoice)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "finance", groupId: "procure_to_pay", itemId: "vendor_bills" });
  }
  if (/\b(bank statement|bank transaction|bank transactions|reconciliation statement)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "finance", groupId: "treasury", itemId: "bank_reconciliation" });
  }
  if (/\b(receipt|expense receipt|paid expense|proof of payment)\b/.test(joined)) {
    return registryDomainDestination(registry, "finance");
  }
  if (/\b(vat return|vat report|tax return|withholding tax|tax filing|tax document)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "finance", groupId: "compliance", itemId: "tax" });
  }
  if (/\b(journal|journal entry|general ledger import|trial balance)\b/.test(joined)) {
    return registryItem(registry, { workspaceId: "finance", groupId: "accounting", itemId: "journals" });
  }
  return null;
}

function operationalDestination(evidence = {}) {
  const joined = [
    evidence.object_type,
    evidence.document_type,
    evidence.title,
    ...list(evidence.candidate_domains),
  ].map(normalized).join(" ");

  if (/\b(housekeeping|room clean|room cleaning|room inspection|linen|turndown|cleaning evidence)\b/.test(joined)) {
    return {
      domain: "Operations",
      domain_id: "operations",
      workspace: "operations",
      workspace_name: "Operations",
      group: "Hotel Operations",
      group_id: "hotel_operations",
      item_id: "housekeeping",
      route: "/operations/housekeeping",
      label: "Housekeeping",
    };
  }
  if (/\b(maintenance|repair|service report|equipment inspection|room defect|engineering)\b/.test(joined)) {
    return {
      domain: "Operations",
      domain_id: "operations",
      workspace: "operations",
      workspace_name: "Operations",
      group: "Hotel Operations",
      group_id: "hotel_operations",
      item_id: "maintenance",
      route: "/operations/maintenance",
      label: "Maintenance",
    };
  }
  if (/\b(front desk|arrival|check in|check-in|guest registration|departure|check out|check-out)\b/.test(joined)) {
    return {
      domain: "Operations",
      domain_id: "operations",
      workspace: "operations",
      workspace_name: "Operations",
      group: "Hotel Operations",
      group_id: "hotel_operations",
      item_id: "front-desk",
      route: "/operations/front-desk",
      label: "Front Desk",
    };
  }
  return null;
}

function specializedDestination(evidence = {}, registry = {}) {
  const doc = normalized(evidence.document_type);
  const objectType = normalized(evidence.object_type);
  const kind = `${doc} ${objectType}`;
  if (/\b(supplier|vendor|supplier onboarding|vendor onboarding|supplier master|vendor master)\b/.test(kind) && !/invoice|bill|purchase order|purchase_order|delivery note|delivery_note|goods receipt|goods_receipt/.test(kind)) {
    return registryItem(registry, { workspaceId: "supply-chain", groupId: "procurement", itemId: "suppliers" });
  }
  if (/\b(purchase order|purchase_order|po document|supplier purchase order)\b/.test(kind)) {
    return registryItem(registry, { workspaceId: "supply-chain", groupId: "procurement", itemId: "purchase_orders" });
  }
  if (/\b(goods receipt|goods_receipt|delivery note|delivery_note|receiving report|receiving_report)\b/.test(kind)) {
    return registryItem(registry, { workspaceId: "supply-chain", groupId: "receiving", itemId: "goods_receipts" });
  }
  const operations = operationalDestination(evidence);
  if (operations) return operations;
  const finance = financeDestination(evidence, registry);
  if (finance) return finance;
  const asset = assetDestination(evidence, registry);
  if (asset) return asset;
  const customer = customerDestination(evidence, registry);
  if (customer) return customer;
  if (/\b(cv|resume|curriculum vitae|employee profile|employee record|staff profile|personnel record)\b/.test(`${doc} ${objectType}`)) {
    return registryItem(registry, { workspaceId: "people", groupId: "workforce", itemId: "employees" });
  }
  if (/\b(contract|agreement)\b/.test(`${doc} ${objectType}`)) {
    return registryItem(registry, { workspaceId: "documents", groupId: "document_management", itemId: "contracts" });
  }
  return null;
}
function domainCandidates(evidence = {}, registry = {}) {
  const unique = new Map();
  for (const value of list(evidence.candidate_domains)) {
    const key = normalized(value);
    const destination = registryDomainDestination(registry, key);
    if (destination) unique.set(key, destination);
  }
  return [...unique.values()];
}

export function routeAnalyzedAttachment(file = {}, { registry = null } = {}) {
  const analysis = object(file.analysis);
  const evidence = object(analysis.evidence);
  if (analysis.status !== "ANALYZED") return null;

  if (analysis.clarification_required === true || evidence.clarification_required === true) {
    return {
      type: "universal_destination",
      status: "CLARIFICATION_REQUIRED",
      destination: null,
      clarification_required: true,
      clarification_question: text(analysis.clarification_question || evidence.clarification_question) ||
        "What business purpose should I use for this file?",
      authorization_effect: "NONE",
    };
  }

  const specialized = specializedDestination(evidence, registry || {});
  const candidates = domainCandidates(evidence, registry || {});
  if (specialized) {
    return {
      type: "universal_destination",
      status: "DESTINATION_RESOLVED",
      destination: specialized,
      evidence_classification: {
        object_type: text(evidence.object_type) || null,
        document_type: text(evidence.document_type) || null,
        confidence: Number(evidence.confidence || analysis.confidence) || 0,
      },
      clarification_required: false,
      authorization_effect: "NONE",
    };
  }

  if (candidates.length === 1) {
    return {
      type: "universal_destination",
      status: "DESTINATION_RESOLVED",
      destination: candidates[0],
      evidence_classification: {
        object_type: text(evidence.object_type) || null,
        document_type: text(evidence.document_type) || null,
        confidence: Number(evidence.confidence || analysis.confidence) || 0,
      },
      clarification_required: false,
      authorization_effect: "NONE",
    };
  }

  return {
    type: "universal_destination",
    status: "CLARIFICATION_REQUIRED",
    destination: null,
    candidate_destinations: candidates,
    clarification_required: true,
    clarification_question: candidates.length > 1
      ? `This file could belong to ${candidates.map((item) => item.domain).join(" or ")}. Which business purpose should I use?`
      : "I analyzed the file but cannot determine which Avantiqo area should own it. What business purpose should I use?",
    authorization_effect: "NONE",
  };
}

export default routeAnalyzedAttachment;
