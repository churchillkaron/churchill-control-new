...
export async function buildWorkspaceRuntime({
      ...
      organizationTree.organizations.map(
        async organization => {
          ...
          return await buildOrganizationRuntime({
            ...
            modules:
              Array.isArray(moduleRuntime?.modules)
  ...
  const rawActiveModules =
    Array.isArray(activeModuleRuntime?.modules)
      ? activeModuleRuntime.modules
      : [];

  const workspaceModules =
    rawActiveModules;

  const activeOrganizationWithModules =
    activeOrganization
      ? {
          ...activeOrganization,
  ...
  return {
    ...
    organizationRuntimes:
      Array.isArray(organizationRuntimes)
        ? organizationRuntimes
        : [],
