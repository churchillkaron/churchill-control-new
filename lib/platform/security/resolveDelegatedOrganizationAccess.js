...
async function resolveOrganizationParty({ staff, organizationId }) {
  if (staff?.party_id) {
  ...
  const assignmentPartyIds = [...new Set(
    (assignments.data || []).map((row) => text(row.party_id)).filter(Boolean)
  )];
  if (assignmentPartyIds.length > 1) {
    throw new Error("Automation creator has multiple Party identities in the target organization");
  }
  if (assignmentPartyIds.length === 1) return assignmentPartyIds[0];

  const email = text(staff.email).toLowerCase();
  ...
  if ((parties.data || []).length > 1) {
    throw new Error("Automation creator Party identity is ambiguous in the target organization");
  }
