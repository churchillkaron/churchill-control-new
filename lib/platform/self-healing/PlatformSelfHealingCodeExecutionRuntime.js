...
function authoritativeClassification(prepared = {}) {
  ...
  if (classification === "GOVERNED_CHANGE") return classification;
  ...
  if (source !== "ERP_REGISTRY") return null;
...
function objectiveContext(prepared = {}) {
...
export async function executePlatformSelfHealingCodeMission({
  ...
  const classification = authoritativeClassification(prepared);
  const mission = object(prepared.intelligence_mission_preparation);
  const replay = replayContract(prepared);
  const result = await executeCodeAIEmployeeMission({
    ...
    owner_intent: [
      ...
      "Repair only the canonically authorized Avantiqo surface described by the attached objective/context.",
    ...
    objective_context: objectiveContext(prepared),
