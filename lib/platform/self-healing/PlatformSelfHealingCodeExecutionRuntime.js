...
import { persistCodeAIAutonomousExecutionState } from "@/lib/code/runtime/CodeAIAutonomousExecutionStateRuntime";
import { persistCodeAICommitArtifact } from "@/lib/code/runtime/CodeAICommitArtifactRuntime";
import {
...
export async function executePlatformSelfHealingCodeMission({
  ...
  if (completion.complete === true && completion.verified === true && text(execution_key, 160) && result?.state) {
    result.state = attestCodeMissionState({
      ...result.state,
      organization_id: organizationId,
      actor_id: text(context?.actor?.id || context?.actor?.user_id, 160) || null,
    });
    const [persisted, commitArtifact] = await Promise.all([
      persistCodeAIAutonomousExecutionState({ context, executionKey: execution_key, result }),
      persistCodeAICommitArtifact({ context, executionKey: execution_key, missionState: result.state }),
    ]);
    verificationEvidence = {
      execution_key: text(execution_key, 160),
  ...
  return {
    ...
    execution_key: verificationEvidence?.execution_key || null,
