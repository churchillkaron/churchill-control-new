...
export async function preparePlatformSelfHealingCodeMission({
  ...
  const privilegedClassification = resolvePrivilegedSelfHealingClassification(
    payload,
    authoritativePreparedClassification(payload),
  );
  const classification = privilegedClassification || classifyPlatformSelfHealingFailure(payload);
  ...
  if (!classification.code_execution_allowed) {
    return {
      ...base,
      status: classification.blocking_status ||
        (classification.classification === "NON_CODE_CONFIGURATION"
