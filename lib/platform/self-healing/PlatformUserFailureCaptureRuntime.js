...
export const PLATFORM_USER_FAILURE_SCHEMA_VERSION = 1;
...
function recommendationFor(category, errorMessage) {
  const source = `${category} ${errorMessage}`.toLowerCase();
  if (/credential|oauth|access token|quota|api.*disabled|api.*not enabled|accessnotconfigured/.test(source)) {
...
async function deriveOrganizationIdsForUser(user) {
  ...
  const { data: staffRows, error: staffError } = await supabaseAdmin
    .from("staff_accounts")
    .select("id,organization_id,active_organization_id,status,active,is_active,enabled,archived")
  ...
  if (staffIds.length) {
    const { data: memberships, error: membershipError } = await supabaseAdmin
      ...
      .select("organization_id,status,active,is_active,enabled,archived")
...
export async function capturePlatformUserFailure({ request, input = {} } = {}) {
  ...
  const statusCode = Number.isFinite(Number(input.statusCode || input.status_code))
    ? Math.max(0, Math.min(599, Number(input.statusCode || input.status_code)))
    : null;
  ...
  const classification = recommendationFor(category, errorMessage);
  ...
  const payload = {
    ...
    autonomous_repair_eligible: Boolean(scope.organizationId) && ["AUTO_REPAIR", "AUTO_COMPLETE"].includes(classification),
    replay: {
      route: pathname,
      action,
      capability,
  ...
  return {
    ...
    autonomousRepairEligible: payload.autonomous_repair_eligible,
  };
}
