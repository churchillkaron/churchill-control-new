...
function providerStatus({ runtimeAvailable, pricingReady, supplierAccount, adapter }) {
  if (!runtimeAvailable) {
    return {
      ready: false,
  ...
  if (adapter?.requires_supplier_account_config !== false && !supplierAccount?.ready) {
    return {
      ready: false,
...
function providerSnapshot(provider = {}, pricingRows = [], supplierAccount = null) {
  ...
  const readiness = providerStatus({ runtimeAvailable, pricingReady, supplierAccount, adapter });
  ...
  return {
    ...
    customer_direct_provider_billing_allowed: adapter.customer_direct_provider_billing_allowed === true,
    customer_provider_payment_method_allowed: adapter.customer_provider_payment_method_allowed === true,
    prepaid_provider_execution_required: adapter.customer_direct_provider_billing_allowed !== true,
...
async function snapshot({ payerOrganizationId = null } = {}) {
  const [pricingRows, supplierGovernance] = await Promise.all([
    activePricingRows(),
    ProviderSupplierAccountRuntime.snapshot({ payerOrganizationId }),
  ]);
  ...
  return {
    ...
    summary: {
      ...
      customer_direct_provider_billing_allowed: providers.filter((provider) => provider.customer_direct_provider_billing_allowed).length,
    ...
    contract: {
      ...
      payer_organization_required: true,
      payer_entity_required: true,
      supplier_party_required: true,
