...
function providerStatus({ runtimeAvailable, pricingReady, supplierAccount }) {
  ...
  if (!supplierAccount?.ready) {
    return {
      ready: false,
      status: supplierAccount?.status || "SUPPLIER_ACCOUNT_REQUIRED",
      blocker:
        supplierAccount?.blocker || "AVANTIQO_PROVIDER_SUPPLIER_ACCOUNT_REQUIRED",
    };
  }
...
async function snapshot({ payerOrganizationId = null } = {}) {
  ...
  const supplierAccountMap = new Map(
    supplierGovernance.accounts.map((account) => [account.provider_id, account]),
  );

  const providers = Object.values(PROVIDER_REGISTRY)
    .filter((provider) => provider && provider.active !== false)
    .map((provider) => {
      const account = supplierAccountMap.get(provider.id) || null;
      return providerSnapshot(provider, pricingRows, {
        account,
        ready: account?.ready === true,
        status: account?.status || "SUPPLIER_ACCOUNT_REQUIRED",
        blocker:
          account?.blocker || "AVANTIQO_PROVIDER_SUPPLIER_ACCOUNT_REQUIRED",
      });
    })
    .sort((left, right) => {
