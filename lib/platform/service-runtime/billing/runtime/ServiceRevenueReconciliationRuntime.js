...
function batches(values, size = QUERY_BATCH_SIZE) {
...
async function walletCharges(usageIds) {
  ...
  if (!usageIds.length) return totals;

  for (const usageIdBatch of batches(usageIds)) {
    const { data, error } = await supabaseAdmin
...
async function financeJournals(usageIds) {
  ...
  if (!usageIds.length) return journals;
