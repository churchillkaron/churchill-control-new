...
async function walletCharges(usageIds) {
  if (!usageIds.length) return new Map();
...
async function financeJournals(usageIds) {
  ...
  return new Set(
    (data || [])
      ...
      .map((row) => `${row.organization_id}:${row.source_document_id}`),
  );
}
...
export async function reconcileServiceRevenue({ limit = 500 } = {}) {
  ...
  if (rows.length) {
    const { error } = await supabaseAdmin
      ...
      .upsert(rows, { onConflict: "usage_id" });
    if (error) throw error;
  }
