...
async function acquire({ organizationId, endpointId, ownerRequestId }) {
  ...
  if (error) {
    throw new Error(`AVANTIQO_INTELLIGENCE_FAST_POD_LEASE_ACQUIRE_FAILED:${error.code || "UNKNOWN"}`);
  }
  if (
    !data ||
...
async function release({ leaseId, ownerRequestId, state, reason }) {
  ...
  if (error) {
    throw new Error(`AVANTIQO_INTELLIGENCE_FAST_POD_LEASE_RELEASE_FAILED:${error.code || "UNKNOWN"}`);
  }
}
