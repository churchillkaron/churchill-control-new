...
function queueKey(config) {
  const key = text(process.env[config.queueKeyEnv]) || text(process.env.RUNPOD_API_KEY) || managementKey();
  if (!key) throw new Error("AVANTIQO_INTELLIGENCE_REQUEST_LEASE_RUNPOD_QUEUE_KEY_REQUIRED");
  return key;
}
...
async function requestJson(url, key, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(`AVANTIQO_INTELLIGENCE_REQUEST_LEASE_HTTP_${response.status}`);
  }
  if (body === null) {
...
async function releaseDistributed({ leaseId, ownerRequestId, state, reason }) {
  ...
  if (error) throw new Error(`AVANTIQO_INTELLIGENCE_REQUEST_LEASE_DB_RELEASE_FAILED:${error.code || "UNKNOWN"}`);
}
...
function normalizeLane(payload = {}) {
  const lane = text(payload?.execution_lane || payload?.executionLane).toLowerCase() || "deep";
...
export async function withOwnedIntelligenceRequestLease({
  ...
  const lane = normalizeLane(payload);
