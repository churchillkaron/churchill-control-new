...
const OPEN_PROPAGATION_TIMEOUT_MS = 30000;
const OPEN_PROPAGATION_POLL_MS = 1000;
...
async function waitForGlobalOpenPropagation(endpointId, config) {
  const deadline = Date.now() + OPEN_PROPAGATION_TIMEOUT_MS;
  while (Date.now() < deadline) {
    const body = await rest("/endpoints?includeTemplate=false&includeWorkers=false");
    const endpoints = normalizeListResponse(body, ["endpoints", "serverlessEndpoints"]);
    const endpoint = endpoints?.find(
      (entry) => text(entry?.id) === endpointId && text(entry?.name) === config.endpointName,
    );
    if (
      endpoint &&
    ...
    await sleep(OPEN_PROPAGATION_POLL_MS);
  }
  throw new Error("AVANTIQO_INTELLIGENCE_REQUEST_LEASE_OPEN_PROPAGATION_TIMEOUT");
}
...
async function patchWorkers(endpointId, workersMax, config) {
  await rest(`/endpoints/${encodeURIComponent(endpointId)}/update`, {
    method: "POST",
  ...
  if (workersMax === 1) {
    await waitForGlobalOpenPropagation(endpointId, config);
  }
  return endpoint;
}
...
async function normalizeOwnedRestingEndpoint(endpointId, config) {
  let endpoint = await endpointState(endpointId);
  assertEndpointIdentity(endpoint, endpointId, config);
  ...
  if (finite(endpoint?.workersMax, -1) !== 0 || activeWorkers(endpoint).length !== 0) {
    endpoint = await parkAndVerify(endpointId, config);
  }

  const verifiedHealth = healthJobs(await queue(endpointId, "/health", config));
  if (
    finite(endpoint?.workersMax, -1) !== 0 ||
    activeWorkers(endpoint).length !== 0 ||
    verifiedHealth.inQueue !== 0 ||
    verifiedHealth.inProgress !== 0
  ) {
...
export async function withOwnedIntelligenceRequestLease({
  ...
  try {
    await normalizeOwnedRestingEndpoint(endpointId, config);
    endpointOpened = true;
