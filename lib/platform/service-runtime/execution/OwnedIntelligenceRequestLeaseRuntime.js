...
async function normalizeRestingEndpoint(endpointId, config) {
  ...
  const workersMax = finite(endpoint?.workersMax, -1);
  if (workers.length !== 0) {
    throw new Error("AVANTIQO_INTELLIGENCE_REQUEST_LEASE_REST_STATE_REQUIRED");
  }

  if (workersMax !== 0) {
    endpoint = await patchWorkers(endpointId, 0, config);
  }
...
export async function withOwnedIntelligenceRequestLease({
  ...
  await normalizeRestingEndpoint(endpointId, config);
