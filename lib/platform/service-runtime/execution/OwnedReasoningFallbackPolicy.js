...
function fallbackExplicitlyEnabled(policy = {}) {
  return (
    policy.allow_owned_reasoning_fallback === true ||
    policy.allowOwnedReasoningFallback === true ||
    policy.owned_reasoning_fallback === true ||
    policy.ownedReasoningFallback === true
  );
}
...
export function ownedReasoningFallbackDecision({
  ...
  // Owned Avantiqo reasoning is fail-closed by default. External failover is
  // permitted only when a caller makes an explicit, auditable opt-in.
  if (!fallbackExplicitlyEnabled(providerPolicy)) {
    return { allowed: false, reason: "OWNED_REASONING_FALLBACK_NOT_EXPLICITLY_ENABLED" };
  }
