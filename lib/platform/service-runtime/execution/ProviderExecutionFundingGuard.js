// Zero-cost controls may execute without supplier billing only when pricing explicitly proves no supplier/customer charge.
import { supabaseAdmin } from "../../../shared/supabase/admin.js";
import { getProvider } from "../providers/ProviderRegistry.js";
import { assertAvantiqoSupplierBilling } from "../billing/adapters/ProviderSupplierBillingAdapters.js";
import { ProviderPayerRuntime } from "../billing/runtime/ProviderPayerRuntime.js";

const EXECUTION_PHASE = "EXECUTE";
const STATUS_RECONCILIATION_PHASE = "STATUS_RECONCILIATION";

function text(value) {
  return String(value ?? "").trim();
}

function upper(value) {
  return text(value).toUpperCase();
}

function object(value) {
  return value && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
}

async function usageRecord({ organization_id, usage_id }) {
  const { data, error } = await supabaseAdmin
    .from("platform_service_usage")
    .select("id,organization_id,bill_to_organization_id,organization_service_id,status,category,provider,provider_request_id,currency,metadata")
    .eq("id", usage_id)
    .eq("organization_id", organization_id)
    .maybeSingle();

  if (error) throw error;
  return data || null;
}

async function organizationServiceRecord({ organizationId, serviceId }) {
  if (!serviceId) return null;

  const { data, error } = await supabaseAdmin
    .from("organization_services")
    .select("id,organization_id,status,usage_enabled,billing_enabled,billing_mode,pricing_mode")
    .eq("id", serviceId)
    .eq("organization_id", organizationId)
    .maybeSingle();

  if (error) throw error;
  return data || null;
}

async function walletRecord(organizationId) {
  const { data, error } = await supabaseAdmin
    .from("organization_wallets")
    .select("id,organization_id,currency,available_balance,reserved_balance,billing_policy,status")
    .eq("organization_id", organizationId)
    .maybeSingle();

  if (error) throw error;
  return data || null;
}

async function prepaidReservation({ organizationId, usageId }) {
  const { data, error } = await supabaseAdmin
    .from("wallet_transactions")
    .select("id,wallet_id,type,amount,currency,provider,reference,metadata,created_at")
    .eq("organization_id", organizationId)
    .eq("type", "RESERVE")
    .contains("metadata", { usage_id: usageId })
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();

  if (error) throw error;
  return data || null;
}

async function providerSupplierAccount({ provider, payerOrganizationId }) {
  const { data, error } = await supabaseAdmin
    .from("provider_supplier_billing_accounts")
    .select("id,provider_id,payer_organization_id,payer_entity_id,supplier_party_id,billing_mode,status,currency,verification_status,verification_method,verification_reference,verified_at,verified_by")
    .eq("provider_id", text(provider).toLowerCase())
    .eq("payer_organization_id", payerOrganizationId)
    .maybeSingle();

  if (error) throw error;
  return data || null;
}

async function supplierProfileRecord({ payerOrganizationId, supplierPartyId }) {
  if (!supplierPartyId) return null;

  const { data, error } = await supabaseAdmin
    .from("supplier_profiles")
    .select("id,organization_id,party_id,is_active,is_blocked")
    .eq("organization_id", payerOrganizationId)
    .eq("party_id", supplierPartyId)
    .maybeSingle();

  if (error) throw error;
  return data || null;
}

function providerSupplierAccountStatus({ account, payer, supplierProfile }) {
  if (!account) {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_PAYER_ORGANIZATION_REQUIRED",
    };
  }

  if (
    !account.payer_organization_id ||
    account.payer_organization_id !== payer.organization_id
  ) {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_PAYER_ORGANIZATION_REQUIRED",
    };
  }

  if (
    !account.payer_entity_id ||
    account.payer_entity_id !== payer.entity_id
  ) {
    return {
      ready: false,
      blocker: "AVANTIQO_PAYER_ENTITY_REQUIRED",
    };
  }

  if (
    !account.supplier_party_id ||
    !supplierProfile?.party_id ||
    supplierProfile.organization_id !== payer.organization_id
  ) {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_SUPPLIER_REQUIRED",
    };
  }

  if (
    supplierProfile.is_active === false ||
    supplierProfile.is_blocked === true
  ) {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_SUPPLIER_NOT_ACTIVE",
    };
  }

  const verification = upper(account.verification_status) || "UNVERIFIED";
  if (verification === "REJECTED") {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_PAYER_VERIFICATION_REJECTED",
    };
  }

  if (verification !== "VERIFIED") {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_PAYER_VERIFICATION_REQUIRED",
    };
  }

  if (upper(account.status) !== "ACTIVE") {
    return {
      ready: false,
      blocker: "AVANTIQO_PROVIDER_SUPPLIER_ACCOUNT_INACTIVE",
    };
  }

  return { ready: true, blocker: null };
}

function zeroCostPricing(usage = {}) {
  const metadata = object(usage.metadata);
  const pricing = object(metadata.reservation_pricing);
  if (
    pricing.zero_price !== true ||
    Number(pricing.customer_price || 0) !== 0 ||
    Number(pricing.supplier_cost || 0) !== 0
  ) {
    return null;
  }

  return {
    pricing,
    pricingMetadata: object(pricing.pricing_metadata),
  };
}

function isInternalZeroCostControl(usage = {}) {
  const zeroCost = zeroCostPricing(usage);
  if (!zeroCost) return false;

  const category = upper(usage.category);
  return (
    category === "ADMINISTRATION" ||
    category.startsWith("ADMINISTRATION_") ||
    category.startsWith("PLATFORM_") ||
    category === "ADMINISTRATION_PROVIDER_BILLING"
  );
}

function isCustomerDirectProviderZeroCost(usage = {}, billing = {}) {
  const zeroCost = zeroCostPricing(usage);
  if (!zeroCost) return false;

  return (
    billing.billing_owner === "CUSTOMER_ORGANIZATION" &&
    billing.external_provider_billing === true &&
    billing.customer_direct_provider_billing_allowed === true &&
    billing.customer_provider_payment_method_allowed === true &&
    zeroCost.pricingMetadata.customer_direct_provider_billing_allowed === true &&
    zeroCost.pricingMetadata.external_provider_billing === true &&
    zeroCost.pricingMetadata.supplier_billing_required === false &&
    zeroCost.pricingMetadata.provider_supplier_account_verification_required === false
  );
}

function isExplicitSupplierExemptZeroCost(usage = {}) {
  const zeroCost = zeroCostPricing(usage);
  if (!zeroCost) return false;

  return (
    zeroCost.pricingMetadata.supplier_billing_required === false &&
    zeroCost.pricingMetadata.provider_supplier_account_verification_required === false
  );
}

function pendingStatusReconciliation({ usage, provider, executionPhase, providerJobId }) {
  if (executionPhase !== STATUS_RECONCILIATION_PHASE) return false;
  if (upper(usage.status) !== "PENDING") {
    throw new Error(`PROVIDER_PENDING_STATUS_USAGE_REQUIRED:${provider}:${usage.status || "UNKNOWN"}`);
  }

  const boundJobId = text(usage.provider_request_id);
  if (!providerJobId || !boundJobId || providerJobId !== boundJobId) {
    throw new Error(`PROVIDER_PENDING_STATUS_JOB_BINDING_REQUIRED:${provider}`);
  }

  return true;
}

export async function assertProviderExecutionFunded({
  provider,
  context = {},
  execution_phase = EXECUTION_PHASE,
  provider_job_id = null,
}) {
  const providerDefinition = getProvider(provider);
  if (!providerDefinition) {
    throw new Error(`Unknown provider: ${provider}`);
  }

  const billing = assertAvantiqoSupplierBilling(providerDefinition);
  const organizationId = text(context.organization_id);
  const usageId = text(context.usage_id);
  const executionPhase = upper(execution_phase) || EXECUTION_PHASE;
  const providerJobId = text(provider_job_id);

  if (![EXECUTION_PHASE, STATUS_RECONCILIATION_PHASE].includes(executionPhase)) {
    throw new Error(`PROVIDER_EXECUTION_PHASE_UNSUPPORTED:${executionPhase}`);
  }
  if (!organizationId || !usageId) {
    throw new Error(`PROVIDER_GOVERNED_SERVICE_USAGE_REQUIRED:${provider}`);
  }

  const usage = await usageRecord({
    organization_id: organizationId,
    usage_id: usageId,
  });

  if (!usage) {
    throw new Error(`PROVIDER_USAGE_RECORD_REQUIRED:${provider}`);
  }
  if (text(usage.provider) && text(usage.provider) !== text(provider)) {
    throw new Error(`PROVIDER_USAGE_PROVIDER_MISMATCH:${provider}:${usage.provider}`);
  }

  const reconcilingPendingStatus = pendingStatusReconciliation({
    usage,
    provider,
    executionPhase,
    providerJobId,
  });

  const billToOrganizationId = text(usage.bill_to_organization_id) || organizationId;
  if (billToOrganizationId !== organizationId) {
    throw new Error(
      `PROVIDER_BILL_TO_ORGANIZATION_MUST_MATCH_WALLET_ORGANIZATION:${organizationId}:${billToOrganizationId}`,
    );
  }

  const organizationService = await organizationServiceRecord({
    organizationId,
    serviceId: usage.organization_service_id,
  });
  if (!organizationService) {
    throw new Error(`ORGANIZATION_SERVICE_REQUIRED:${provider}`);
  }
  if (upper(organizationService.status) !== "ACTIVE") {
    throw new Error(`ORGANIZATION_SERVICE_ACTIVE_REQUIRED:${provider}`);
  }
  if (organizationService.usage_enabled === false && !reconcilingPendingStatus) {
    throw new Error(`ORGANIZATION_SERVICE_USAGE_REQUIRED:${provider}`);
  }

  const customerDirectProvider = isCustomerDirectProviderZeroCost(usage, billing);
  if (customerDirectProvider || isInternalZeroCostControl(usage) || isExplicitSupplierExemptZeroCost(usage)) {
    return {
      authorized: true,
      mode: customerDirectProvider
        ? "CUSTOMER_DIRECT_PROVIDER_BILLING_ZERO_AVANTIQO_CHARGE"
        : isInternalZeroCostControl(usage)
          ? "AVANTIQO_INTERNAL_ZERO_COST_CONTROL"
          : "AVANTIQO_EXPLICIT_SUPPLIER_EXEMPT_ZERO_COST",
      billing,
      customer_organization_id: organizationId,
      bill_to_organization_id: billToOrganizationId,
      organization_service_id: organizationService.id,
      usage_id: usageId,
      execution_phase: executionPhase,
      pending_status_reconciliation: reconcilingPendingStatus,
    };
  }

  if (organizationService.billing_enabled === false) {
    throw new Error(`ORGANIZATION_SERVICE_BILLING_REQUIRED:${provider}`);
  }

  const [payer, wallet, reservation] = await Promise.all([
    ProviderPayerRuntime.resolve(),
    walletRecord(organizationId),
    prepaidReservation({
      organizationId,
      usageId,
    }),
  ]);

  if (!wallet?.id) {
    throw new Error("ORGANIZATION_WALLET_UNAVAILABLE");
  }
  if (upper(wallet.status) !== "ACTIVE") {
    throw new Error("ACTIVE_PREPAID_WALLET_REQUIRED");
  }
  if (upper(wallet.billing_policy) !== "PREPAID") {
    throw new Error("PREPAID_WALLET_REQUIRED");
  }

  if (!reservation || Number(reservation.amount || 0) <= 0) {
    throw new Error(`PROVIDER_PREPAID_RESERVATION_REQUIRED:${provider}`);
  }

  const account = await providerSupplierAccount({
    provider,
    payerOrganizationId: payer.organization_id,
  });
  const supplierProfile = await supplierProfileRecord({
    payerOrganizationId: payer.organization_id,
    supplierPartyId: account?.supplier_party_id,
  });
  const supplierStatus = providerSupplierAccountStatus({
    account,
    payer,
    supplierProfile,
  });

  if (!supplierStatus.ready) {
    throw new Error(`${supplierStatus.blocker}:${provider}`);
  }

  return {
    authorized: true,
    mode: "AVANTIQO_SUPPLIER_AND_PREPAID_RESERVED",
    billing,
    supplier_account_id: account.id,
    supplier_party_id: account.supplier_party_id || null,
    payer_organization_id: account.payer_organization_id || null,
    payer_entity_id: account.payer_entity_id || null,
    customer_organization_id: organizationId,
    bill_to_organization_id: billToOrganizationId,
    organization_service_id: organizationService.id,
    wallet_id: wallet.id,
    reservation_id: reservation.id,
    reserved_amount: Number(reservation.amount || 0),
    currency: reservation.currency || wallet.currency || usage.currency || null,
    usage_id: usageId,
    execution_phase: executionPhase,
    pending_status_reconciliation: reconcilingPendingStatus,
  };
}

export const ProviderExecutionFundingGuard = {
  assert: assertProviderExecutionFunded,
};
