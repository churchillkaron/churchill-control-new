...
} from "./ServiceExecutionRuntime.js";
import {
  PricingRuntime,
...
function installPricingGuard() {
  ...
  const resolveRecordWithoutGuard = PricingRuntime.resolveRecord.bind(PricingRuntime);
  ...
  PricingRuntime.resolveById = async function resolveByIdWithCostGuard(input = {}) {
    const guard = storage.getStore();
    const pricing = await resolveByIdWithoutGuard(
      guard
        ? {
            ...input,
            usage: guardedUsage(input, guard),
  ...
  PricingRuntime.resolveRecord = function resolveRecordWithCostGuard(input = {}) {
    const guard = storage.getStore();
    const pricing = resolveRecordWithoutGuard(
      guard
        ? {
            ...input,
            usage: guardedUsage(input.usage || {}, guard),
          }
        : input,
    );
    if (guard) validatePricing(pricing, guard);
