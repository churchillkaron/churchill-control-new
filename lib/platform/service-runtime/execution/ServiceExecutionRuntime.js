...
async function actualPricing({
  ...
  if (pricing?.estimated !== true && !settleQuantityFromProvider) return pricing;
...
async function settleReservation({
  ...
  const authorizedMaximum = finite(maximumAmount);
  ...
  if (charge > reserved) {
    if (authorizedMaximum === null || authorizedMaximum <= 0 || charge > authorizedMaximum) {
      throw new Error(
        `SERVICE_ACTUAL_PRICE_EXCEEDS_RESERVATION:${charge}:${reserved}`,
      );
    }
    reservationTopUp = Number((charge - reserved).toFixed(6));
    if (reservationTopUp > 0) {
      await WalletRuntime.reserve({
        organization_id: organizationId,
        amount: reservationTopUp,
        provider,
        reference: `${usageId}:settlement-topup`,
        currency,
        metadata: {
          usage_id: usageId,
          settlement: "ACTUAL_USAGE_TOPUP",
          initial_reserved_amount: reserved,
          actual_charge_amount: charge,
          authorized_maximum_amount: authorizedMaximum,
        },
      });
      effectiveReserved = Number((reserved + reservationTopUp).toFixed(6));
    }
  }
...
async function intelligencePricingPolicy({
  ...
  const benchmarkLocalExecutionEligible =
    benchmarkLocalPreview &&
    localLaneEligible &&
    !explicitlyRequestsOtherModel &&
    (!Array.isArray(payload.tools) || payload.tools.length === 0);
  const localExecutionEligible = benchmarkLocalExecutionEligible || (
    localLaneEligible && !explicitlyRequestsOtherModel && (
      shouldUseLocalIntelligenceQueue({ ...payload, capability: executionCapability, execution_lane: intelligenceLane }) ||
      shouldUseHierarchicalLocalIntelligence({ ...payload, capability: executionCapability, execution_lane: intelligenceLane })
    )
  );
...
export async function executeService(input = {}) {
  ...
  const pricing = localOwnedCodePreviewPricing(resolvedPricing, {
    provider,
    capability: executionCapability,
    policy: effectiveProviderPolicy,
  });
  ...
  const providerExecutionInput = intelligenceExecutionLane && !payload.execution_lane && !payload.executionLane
    ? { ...payload, execution_lane: intelligenceExecutionLane }
    : payload;
  ...
  try {
    ...
    const settlement = await settleReservation({
      ...
      maximumAmount: finite(metadata?.service_cost_guard_maximum_customer_price),
      currency: pricing.currency,
    });
    reservationRemaining = 0;
...
export async function settlePendingService(input = {}) {
  ...
  const settlement = await settleReservation({
    ...
    reservedAmount,
    chargeAmount: settledPricing.customer_price,
    maximumAmount: finite(usage.metadata?.service_cost_guard_maximum_customer_price),
    currency: reservationPricing.currency || usage.currency || null,
  });
