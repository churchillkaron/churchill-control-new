...
async function settleReservation({
  ...
  if (charge > effectiveReserved) {
    const authorized = finite(maximumAuthorizedCharge);
    if (!(authorized > 0) || charge > authorized + 0.000001) {
      throw new Error(
        `SERVICE_ACTUAL_PRICE_EXCEEDS_RESERVATION:${charge}:${effectiveReserved}`,
      );
    }
    const difference = Number((charge - effectiveReserved).toFixed(6));
    if (difference > 0) {
      await WalletRuntime.reserve({
        organization_id: organizationId,
        amount: difference,
        provider,
        reference: `${usageId}:actual-usage-difference`,
        currency,
        idempotency_key: `${usageId}:actual-usage-difference`,
        metadata: {
          usage_id: usageId,
          settlement: "AUTHORIZED_ACTUAL_USAGE_DIFFERENCE",
          prior_reserved_amount: effectiveReserved,
          actual_charge_amount: charge,
          maximum_authorized_charge: authorized,
        },
      });
      effectiveReserved = Number((effectiveReserved + difference).toFixed(6));
    }
  }
...
export async function executeService(input = {}) {
  ...
  try {
    ...
    const settlement = await settleReservation({
      ...
      currency: settledPricing.currency || pricing.currency || null,
      maximumAuthorizedCharge: finite(metadata?.service_cost_guard_maximum_customer_price),
    });
    reservationRemaining = 0;
...
export async function settlePendingService(input = {}) {
  ...
  const settlement = await settleReservation({
    ...
    reservedAmount,
    chargeAmount: settledPricing.customer_price,
    currency: settledPricing.currency || reservationPricing.currency || null,
    maximumAuthorizedCharge: finite(usage.metadata?.service_cost_guard_maximum_customer_price),
  });
