...
function generatedImageAlreadyCanonical(result = {}) {
  const providerResult = result?.output || result || {};
  const output = providerResult?.output || providerResult || {};
  const evidence =
    output?.canonical_storage ||
    output?.provider_media_storage ||
    output?.immutable_composite ||
    providerResult?.canonical_storage ||
    providerResult?.provider_media_storage ||
    providerResult?.immutable_composite ||
    null;

  return evidence?.canonical_storage === true;
}
...
async function persistProviderImage({
  ...
  if (generatedImageAlreadyCanonical(result)) return result;

  if (typeof generatedImage !== "string") {
    const error = new Error(
      "CREATIVE_PROVIDER_IMAGE_OUTPUT_MUST_BE_STRING",
    );
    error.code = error.message;
    error.details = {
      received_type: Array.isArray(generatedImage)
        ? "array"
  ...
  if (isDataImage(generatedImage)) {
    stored = await CreativeStorageRuntime.uploadDataUrl({
      organization_id,
      creative_project_id: creativeProjectId,
      asset_id: assetId,
      data_url: generatedImage,
      filename: "provider-master-still.png",
    });
  } else if (isHttpsUrl(generatedImage)) {
    stored = await CreativeStorageRuntime.uploadFromUrl({
      organization_id,
      creative_project_id: creativeProjectId,
      asset_id: assetId,
      url: generatedImage,
    ...
    const error = new Error(
      "CREATIVE_PROVIDER_IMAGE_OUTPUT_URL_INVALID",
    );
    error.code = error.message;
    throw error;
  }

  return replaceResultImage(
    result,
    stored.signed_url,
    {
      ...
      source_transport: isDataImage(generatedImage)
        ? "DATA_URL"
        : "HTTPS_URL",
      persisted_before_metadata_sanitization: true,
    },
  );
}
...
export async function executeService(input = {}) {
  ...
  const organizationCurrency =
    await resolveOrganizationCurrency({
      organization_id,
      entity_id,
    });
  const executionCurrency = normalizeCurrency(
    input.currency || organizationCurrency,
  );

  if (!executionCurrency) {
    throw new Error(
      "SERVICE_EXECUTION_CURRENCY_REQUIRED",
    );
  }

  if (executionCurrency !== organizationCurrency) {
    const error = new Error(
      "SERVICE_EXECUTION_ORGANIZATION_CURRENCY_MISMATCH",
    );
    error.code = error.message;
    error.details = {
      organization_currency: organizationCurrency,
      requested_currency: executionCurrency,
    };
    throw error;
  }
  ...
  if (normalizeCurrency(pricing.currency) !== executionCurrency) {
    const error = new Error(
      "SERVICE_PRICING_ORGANIZATION_CURRENCY_MISMATCH",
    );
    error.code = error.message;
    error.details = {
      organization_currency: organizationCurrency,
  ...
  try {
    ...
    result = await persistProviderImage({
      organization_id,
      metadata: rawMetadata,
      result,
    });
