...
async function intelligencePricingPolicy({
  ...
  const localLaneEligible = intelligenceLane === "front" || intelligenceLane === "fast";

  let localReady = false;
  if (localLaneEligible && !explicitlyRequestsOtherModel && shouldUseLocalIntelligenceQueue({ ...payload, capability: executionCapability, execution_lane: intelligenceLane })) {
    try {
...
export async function executeService(input = {}) {
  ...
  const pricingRecord = selectedProvider.pricing_record;
  ...
  const intelligenceExecutionLane = ["ai.reasoning.execute", "ai.text.generate"].includes(executionCapability)
    ? (String(payload.execution_lane || payload.executionLane || metadata?.intelligence_execution_lane || "").trim().toLowerCase()
      || (executionCapability === "ai.text.generate" ? "fast" : "deep"))
    : null;
  const intelligenceLocalLaneEligible = intelligenceExecutionLane === "front" || intelligenceExecutionLane === "fast";
