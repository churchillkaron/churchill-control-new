...
export async function consumeModalComputeApproval({
  ...
  let resolvedApprovalId = approvalId(input);
  ...
  if (!resolvedApprovalId) {
    const { data: activeApproval, error: activeLookupError } = await supabaseAdmin
      .from("modal_compute_approvals")
      .select("id")
      .eq("organization_id", organizationId)
      .eq("provider", "modal")
      .eq("capability", resolvedCapability)
      .eq("status", "APPROVED")
    ...
    resolvedApprovalId = text(activeApproval?.id, 120);
  }
