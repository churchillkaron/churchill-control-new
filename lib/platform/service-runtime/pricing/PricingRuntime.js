...
} from "./repositories/ProviderPricingRepository.js";
import {
  codeBenchmarkPricingSettlementAllowed,
} from "./CodeBenchmarkPricingSettlementPolicy.js";
...
function benchmarkPricingStatusAllowed(pricing = {}) {
  const status = String(pricing?.metadata?.pricing_status || "").trim().toUpperCase();
  if (status === MARKET_PARITY_READY) return true;
  return (
    pricing?.provider === "avantiqo-audio" &&
    pricing?.capability === "ai.music.generate" &&
    status === LEGACY_AUDIO_BENCHMARK_READY
  );
}
...
function benchmarkInactivePricingAllowed(pricing = {}) {
  return (
    ...
    benchmarkPricingStatusAllowed(pricing) &&
...
export const PricingRuntime = {
  ...
  async resolveById({ pricing_id, currency = null, usage = {} }) {
    ...
    if (
      ...
      (
        pricing.active !== true &&
        !codeBenchmarkPricingSettlementAllowed(pricing)
      )
    ) {
