...
function ownedIntelligenceModalConfigured(options = {}) {
  ...
  const baseUrl = text(process.env.AVANTIQO_INTELLIGENCE_MODAL_BASE_URL);
  const token = text(process.env.AVANTIQO_INTELLIGENCE_MODAL_GATEWAY_TOKEN);
  return /^https:\/\//i.test(baseUrl) && token.length >= 40;
}
...
export async function executeProvider(options = {}) {
  ...
  try {
    ...
    // Modal is a fully separate owned scale-to-zero infrastructure path. When
    // its authenticated gateway is configured, do not touch RunPod at all.
    // RunPod Safe Lease remains mandatory when Modal is absent and the provider
    // falls back to the canonical RunPod implementation.
