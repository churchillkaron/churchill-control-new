  ...
  try {
    ...
    if (inheritedSafeLease) {
      ...
      try {
        ...
        // The request-scoped Serverless lease only rejects after its finally
        // block has parked the endpoint and released that lease. Reacquire the
        // same distributed Fast lane before opening Pod capacity so no second
        // Fast request can race the transport handoff.
        result = await withOwnedIntelligenceFastPodDistributedLease({
          organizationId: options?.context?.organization_id || null,
          execute: async () => executeAvantiqoIntelligenceFastPodProvider(options),
        });
      }
    }
