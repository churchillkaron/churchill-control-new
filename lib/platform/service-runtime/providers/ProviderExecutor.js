...
function managedCredentialRequired(provider = {}) {
  return String(provider.connectionModel || "").toLowerCase() === "managed" ||
...
function assertManagedCredential(providerId, credential) {
  const provider = getProvider(providerId);
  if (managedCredentialRequired(provider) && !credential) {
    throw new Error(`PROVIDER_MANAGED_CREDENTIAL_REQUIRED:${providerId}`);
  }
}
...
export async function assertProviderReady({
  ...
  assertManagedCredential(provider, credential);

  if (typeof runtime.assertReady === "function") {
    const readiness = await runtime.assertReady(providerInput({
      capability,
      model,
      input,
      context,
    ...
    return {
      ...
      provider,
      model: model || readiness?.model || null,
