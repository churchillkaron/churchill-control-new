...
const CREATIVE_IMAGE_PROMPT_MAX_CHARACTERS = 32000;
...
function directionEnrichment(input = {}) {
  return (
...
function approvedDirectionBound(input = {}) {
  const direction = directionEnrichment(input);

  return Boolean(
    direction?.approved_story_bound === true &&
    direction?.proof_authorization_hash &&
    direction?.authorized_shot_hash &&
    direction?.blocking_contract?.completeness?.complete === true,
  );
}
...
function prepareCreativeBlockingInput(capability, input = {}) {
  ...
  const direction = directionEnrichment(input);
  const isApprovedDirection = approvedDirectionBound(input);
  const prompt = isApprovedDirection
    ? [
        input.prompt || "",
        "APPROVED HASH-BOUND SHOT DIRECTION:",
        `Proof authorization hash: ${direction.proof_authorization_hash}`,
        `Authorized shot hash: ${direction.authorized_shot_hash}`,
        "The approved structured shot direction and blocking contract supplied with this request are authoritative. Render exactly one decisive still frame that makes the declared story action readable without captions. Preserve declared narrative roles, subject paths, body orientation, gaze, interaction targets, screen direction, opening state, closing state, evidence boundaries and forbidden interpretations. Do not reverse a declared action or direction, merge opposing actions, confuse declared roles, combine multiple time states, or substitute generic posing for the required story event. Camera language controls framing only and may never override physical subject blocking.",
      ].filter(Boolean).join("\n\n")
    : [
        input.prompt || "",
        "AUTHORITATIVE SHOT BLOCKING CONTRACT:",
        JSON.stringify(contract),
        "Render exactly one decisive still frame that makes the declared story action readable without captions. Narrative roles, subject paths, body orientation, gaze, interaction targets, screen direction, opening state, closing state and forbidden interpretations are mandatory. Do not reverse a declared action or direction, merge opposing actions, confuse declared roles, combine multiple time states, or substitute generic posing for the required story event. Camera language controls framing only and may never override physical subject blocking.",
      ].filter(Boolean).join("\n\n");
  ...
  return {
    ...
    prompt_contract: {
      ...
      approved_story_bound: isApprovedDirection,
      duplicate_blocking_contract_omitted: isApprovedDirection,
      prompt_characters: prompt.length,
      maximum_characters:
        CREATIVE_IMAGE_PROMPT_MAX_CHARACTERS,
    },
  };
}
...
async function prepareImageGenerationReferences(capability, input = {}) {
  ...
  const requestedLocation = assets.some((asset) =>
    hasRole(asset, /LOCATION|ENVIRONMENT|SOURCE_PLATE|VENUE/),
  );
  const requestedBrand = assets.some((asset) =>
    hasRole(asset, /BRAND|LOGO/),
  );
  const acceptedLocation = accepted.some((asset) =>
    hasRole(asset, /LOCATION|ENVIRONMENT|SOURCE_PLATE|VENUE/),
  );
  ...
  if (requestedLocation && !acceptedLocation) {
    const error = new Error("VALID_LOCATION_REFERENCE_REQUIRED");
    error.code = "VALID_LOCATION_REFERENCE_REQUIRED";
    error.details = { rejected };
    throw error;
  }
  ...
  return {
    ...
    reference_validation: {
      ...
      rejected,
      location_preserved:
        requestedLocation ? acceptedLocation : null,
...
async function prepareMasterStillQaInput(capability, input = {}) {
  ...
  return {
    ...
    prompt: [
      input.prompt || "",
      "The supplied image is one labelled visual evidence board. The large first panel is the generated master still. Every lower panel is an original reference image. Compare their visible pixels directly and fail closed when identity, location, product, brand, anatomy, realism, brief accuracy or commercial craft cannot be verified.",
      blockingContract
        ? `AUTHORITATIVE SHOT BLOCKING CONTRACT: ${JSON.stringify(blockingContract)}`
        : "No authoritative shot blocking contract was supplied; fail brief accuracy because story action, screen direction and declared roles cannot be verified.",
      "Explicitly verify declared role readability, action, start-to-end travel direction, body orientation, gaze, interaction target, screen direction, decisive moment and every forbidden interpretation. A visually attractive image must fail when the action reads as an opposite event, declared roles are ambiguous, multiple time states appear, or subjects merely pose instead of performing the story.",
    ].filter(Boolean).join("\n\n"),
...
function assertCreativePromptBudget(capability, input = {}) {
  if (capability !== "ai.image.generate") return input;

  const prompt = String(input.prompt || "");

  if (prompt.length > CREATIVE_IMAGE_PROMPT_MAX_CHARACTERS) {
    const error = new Error(
      "CREATIVE_IMAGE_PROMPT_BUDGET_EXCEEDED",
    );
    error.code = "CREATIVE_IMAGE_PROMPT_BUDGET_EXCEEDED";
    error.details = {
      capability,
...
export async function executeProvider({
  ...
  const qaPreparedInput = await prepareMasterStillQaInput(
    capability,
    imagePreparedInput,
  );
  const preparedInput = assertCreativePromptBudget(
    capability,
    qaPreparedInput,
  );
