  ...
  try {
    ...
    if (inheritedSafeLease) {
      ...
      try {
        ...
        if (!fastPodFallbackEligible(options, error)) throw error;
        // withOwnedIntelligenceRequestLease does not reject until its finally
        // block has parked the Serverless endpoint and released the request
        // lease. The Pod runtime independently verifies the 0/0 rest state
        // before opening paid Pod capacity, preventing dual paid capacity.
        result = await executeAvantiqoIntelligenceFastPodProvider(options);
      }
    }
