...
export async function executeProvider(options = {}) {
  ...
  try {
    ...
    // the same direct Modal SDK credentials used by Audio are configured, do
    // not touch RunPod at all. RunPod Safe Lease remains mandatory only when
    // direct Modal is absent and the canonical RunPod fallback is selected.
