...
function assertGovernedAvantiqoIntelligenceExecution({ provider, context = {} }) {
  if (provider !== "avantiqo-intelligence") return;
  if (!value(context.organization_id) || !value(context.organization_service_id) || !value(context.usage_id)) {
    throw new Error("AVANTIQO_INTELLIGENCE_GOVERNED_SERVICE_EXECUTION_REQUIRED");
  }
}

function assertGovernedAvantiqoVideoExecution({ provider, context = {}, preparedInput = {} }) {
  if (provider !== "avantiqo-video") return;
  if (!value(context.organization_id) || !value(context.organization_service_id) || !value(context.usage_id)) {
    throw new Error("AVANTIQO_VIDEO_GOVERNED_SERVICE_EXECUTION_REQUIRED");
  }
  const contract = preparedInput.prompt_serialization_contract || {};
  if (
    contract.contract !== "CREATIVE_PROVIDER_INSTRUCTION_SERIALIZATION_V1" ||
    ...
    throw new Error("AVANTIQO_VIDEO_PROVIDER_INSTRUCTION_SERIALIZATION_REQUIRED");
  }
}
...
export async function executeProvider({ provider, capability, model, input = {}, context = {} }) {
  ...
  assertGovernedAvantiqoIntelligenceExecution({ provider, context, preparedInput });
  assertGovernedAvantiqoVideoExecution({ provider, context, preparedInput });
