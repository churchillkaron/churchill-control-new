...
import {
  assertProviderExecutionFunded,
} from "@/lib/platform/service-runtime/execution/ProviderExecutionFundingGuard";
...
const RUNTIME_LOADERS = {
  email_google: () =>
    import("./email/EmailUnifiedProvider.js")
      .then(
        module =>
          module.GoogleEmailUnifiedProvider
      ),
  email_microsoft: () =>
    import("./email/EmailUnifiedProvider.js")
      .then(
        module =>
          module.MicrosoftEmailUnifiedProvider
      ),
  email_imap: () =>
    import("./email/EmailUnifiedProvider.js")
      .then(
        module =>
          module.ImapEmailUnifiedProvider
      ),
  shopify: () =>
    import("./shopify/ShopifyProvider.js")
      .then(
        module =>
          module.ShopifyProvider
      ),
  threads: () =>
    import("./threads/ThreadsProvider.js")
      .then(
        module =>
          module.ThreadsProvider
      ),
  x: () =>
    import("./x/XProvider.js")
      .then(
        module =>
          module.XProvider
      ),
  tiktok: () =>
    import("./tiktok/TikTokProvider.js")
      .then(
        module =>
          module.TikTokProvider
      ),
  tripadvisor: () =>
    import("./tripadvisor/TripadvisorProvider.js")
      .then(
        module =>
          module.TripadvisorProvider
      ),
  linkedin: () =>
  ...
  google_ads_managed: () =>
    import("./google/GoogleAdsManagedProvider")
      .then(
        module =>
          module.GoogleAdsManagedProvider
      ),
  gemini: () =>
    import("./gemini/GeminiApprovedDependencyFrameProvider.js")
      .then(
        module =>
          module.GeminiApprovedDependencyFrameProvider
      ),
  google_veo: () =>
    import("./gemini/GeminiVeoProviderRuntime.js")
      .then(
        module =>
          module.GeminiVeoProviderRuntime
      ),
  meta: () =>
  ...
  grok: () =>
    import("./grok/GrokProvider.js")
      .then(
        module =>
          module.GrokProvider
      ),
  seedance: () =>
    import("./seedance/SeedanceProvider.js")
      .then(
        module =>
          module.SeedanceProvider
      ),
  veo: () =>
    import("./veo/VeoProvider.js")
      .then(
        module =>
          module.VeoProvider
      ),
...
function credentialOverlay(credential = null) {
  ...
  return Object.fromEntries(
    Object.entries(credential).filter(
      ([key]) => !RESERVED_BUSINESS_INPUT_KEYS.has(key),
    ),
  );
}
...
function assertGovernedOpenAIExecution({
  provider,
  context = {},
  preparedInput = {},
}) {
...
function providerInput({
  ...
  const providerPayload = {
    ...input,
    ...(
      credential?.login_customer_id &&
      !input?.login_customer_id
        ? {
...
async function preparedProviderInput({
  ...
  const structured = providerInput({
    capability,
    model,
    input,
    context,
  ...
  if (provider === "runway") {
    return prepareRunwayProviderInputByProbe(structured);
  }
...
export async function executeProvider({
  ...
  await assertProviderExecutionFunded({
    provider,
    context,
  });
  ...
  const preparedInput = await prepareProviderInputForExecution({
    provider,
    capability,
    model,
    input,
