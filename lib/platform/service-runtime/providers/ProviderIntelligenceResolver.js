...
function musicOwnedOnlyCapability(capability) {
  const requested = String(capability ?? "").trim().toLowerCase();
  return requested === "ai.music.generate" ||
    requested === "ai.sfx.generate" ||
    requested.startsWith("ai.audio.");
}

function ownedOnlyCandidates(candidates = []) {
  if (!candidates.some((candidate) => musicOwnedOnlyCapability(candidate?.capability))) {
    return candidates;
  }
  return candidates.filter((candidate) =>
    !musicOwnedOnlyCapability(candidate?.capability) ||
    candidate?.metadata?.owned_provider === true,
  );
}


function meetsMinimum(candidate = {}, policy = {}) {
  const quality = finite(candidate.quality_score ?? candidate.metadata?.quality_score);
  const reliability = finite(candidate.reliability_score ?? candidate.metadata?.reliability_score);
  const speed = finite(candidate.speed_score ?? candidate.metadata?.speed_score);
  const minimumQuality = finite(policy.minimum_quality_score ?? policy.minimumQualityScore);
  const minimumReliability = finite(policy.minimum_reliability_score ?? policy.minimumReliabilityScore);
  const minimumSpeed = finite(policy.minimum_speed_score ?? policy.minimumSpeedScore);
  if (minimumQuality !== null && (quality === null || quality < minimumQuality)) return false;
  if (minimumReliability !== null && (reliability === null || reliability < minimumReliability)) return false;
  if (minimumSpeed !== null && (speed === null || speed < minimumSpeed)) return false;
  return true;
}

export function providerMeetsMinimumPolicy(candidate = {}, policy = {}) {
  return meetsMinimum(candidate, policy);
}
...
export function rankProviders(candidates = [], policy = {}) {
  const eligibleCandidates = ownedOnlyCandidates(candidates).filter((candidate) => meetsMinimum(candidate, policy));
  const costScores = relativeCostScores(eligibleCandidates);
