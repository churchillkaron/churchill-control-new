...
function providerDescriptor(providerId, registryProvider = null) {
  return {
    id: providerId,
    name: registryProvider?.name || providerId,
    runtime: registryProvider?.runtime || providerId,
...
export async function resolveProviders({
  ...
  for (const provider of registryProviders) {
    byId.set(provider.id, providerDescriptor(provider.id, provider));
  }
  ...
  for (const row of pricing) {
    ...
    if (registryProvider?.active === false || registryProvider?.runtimeAvailable === false) {
      continue;
    }
  ...
  return {
    providers: [...byId.values()].filter((provider) => provider.active && provider.runtimeAvailable),
    pricing,
  };
}
