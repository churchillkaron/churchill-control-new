...
function benchmarkReviewPolicy({ capability, preferredProvider, policy = {} } = {}) {
  ...
  return Boolean(
    ...
    text(process.env.AVANTIQO_AUDIO_MODAL_CERT_BENCHMARK_PREVIEW).toUpperCase() === "YES" &&
    text(capability) === "ai.music.generate" &&
    ownedProvider === "avantiqo-audio" &&
    text(preferredProvider) === "avantiqo-audio" &&
    policy.owned_only_required === true &&
    policy.external_fallback_allowed === false &&
    allowed.length === 1 &&
    allowed[0] === "avantiqo-audio"
  );
}
...
export async function resolveProvider({ organization_id, capability, preferredProvider = null, country = null, currency = null, policy = {} }) {
  ...
  const ownedOnlyRequired =
    ...
    (!benchmarkReview && AVANTIQO_OWNED_ONLY_CAPABILITIES.has(text(capability)));
