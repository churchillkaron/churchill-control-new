...
function benchmarkReviewPolicy({ capability, preferredProvider, policy = {} } = {}) {
  ...
  const allowed = Array.isArray(policy.allowed_providers)
    ? policy.allowed_providers.filter(Boolean)
    : [];
  return Boolean(
    ownedProvider &&
...
function benchmarkPricingEstimate(policy = {}) {
  const source = policy.benchmark_pricing_estimate ||
    policy.benchmarkPricingEstimate ||
    null;
...
function benchmarkPreproductionPricingStatus({ provider, capability, status }) {
  ...
  // Historical Audio certification rows used this explicit pre-production
  // status before MARKET_PARITY_READY became the canonical label. It remains
  // safe only for the exact owned Music benchmark lane. It must never make
  // ordinary production selection eligible.
  return (
    provider?.id === "avantiqo-audio" &&
    text(capability) === "ai.music.generate" &&
    normalized === LEGACY_AUDIO_BENCHMARK_READY
  );
}
...
function benchmarkPricingEligible({ provider, price, capability, ownedProvider }) {
  ...
  return Boolean(
    ...
    benchmarkPreproductionPricingStatus({
      provider,
      capability,
      status: details.pricing_status,
    }) &&
    details.production_routing_allowed === false
  );
}

function pricedExecutionCandidate({
  provider,
  price,
  capability,
  currency,
  ownedProvider,
  benchmarkReview = false,
  policy = {},
}) {
  ...
  const benchmarkEligible = benchmarkReview && benchmarkPricingEligible({
    provider,
    price,
    capability,
    ownedProvider,
  ...
  const previewEstimate = benchmarkEligible
    ? benchmarkPricingEstimate(policy)
    : null;
...
export async function resolveProvider({
  ...
  const benchmarkReview = benchmarkReviewPolicy({
    capability,
    preferredProvider,
    policy,
  });
  const resolved = await resolveProviders({
    ...
    country,
    currency,
    includeInactivePricing: benchmarkReview,
  });
  ...
  return {
    ...
    metadata: {
      ...
      external_provider_selected: Boolean(
        ownedProvider && selected.provider !== ownedProvider,
      ),
