...
function providerDescriptor(providerId, registryProvider) {
  return {
    ...
    connection_model: registryProvider.connectionModel || null,
...
function managedProvider(provider = {}) {
  return String(provider.connection_model || "").toLowerCase() === "managed" ||
    provider.metadata?.managed_credentials_required === true;
}
...
export async function resolveProvider({
  ...
  for (const price of resolved.pricing) {
    ...
    const credentialId = price.credential_id ?? provider.credential_id ?? null;
    let credentialReady = true;

    if (managedProvider(provider)) {
      try {
        credentialReady = Boolean(await resolveProviderCredential({
          organization_id,
        ...
        rejected.push({
          ...
          provider: provider.id,
          capability,
          reason: error?.message || "PROVIDER_CREDENTIAL_RESOLUTION_FAILED",
        });
        continue;
      }
      ...
      if (!credentialReady) {
        rejected.push({
          ...
          provider: provider.id,
          capability,
          reason: "PROVIDER_MANAGED_CREDENTIAL_NOT_CONNECTED",
        });
        continue;
      }
    }
    ...
    candidates.push({
      ...
      credential_id: credentialId,
      credential_resolved: credentialReady,
      metadata: {
        ...(provider.metadata || {}),
        ...(price.metadata || {}),
        provider_name: provider.name,
        preferred: provider.id === preferredProvider,
        managed_credential_verified: managedProvider(provider),
      },
    });
  }
  ...
  if (!candidates.length) {
    const reasons = rejected
      ...
      .join(",");
    throw new Error(
      `No ready priced provider available for ${capability}${reasons ? `; rejected=${reasons}` : ""}`,
    );
  }
