...
export function prepareAudioDurationGuard(input = {}) {
  ...
  if (requested === null || requested > SHORT_FORM_MAX_SECONDS) return { input, guard: null };
  ...
  return {
    input: {
      ...
      generation: { ...object(input.generation), duration_seconds: generation, duration: generation },
      output_spec: { ...object(input.output_spec), duration_seconds: generation, duration: generation },
      provider_parameters: {
...
export function cropAndValidateWavDuration(buffer, requestedDurationSeconds) {
  ...
  const croppedWav = parseWav(combined);
