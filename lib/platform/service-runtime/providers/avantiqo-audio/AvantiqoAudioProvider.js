...
export const AvantiqoAudioProvider = {
  ...
  async execute(input = {}) {
    ...
    if (isSeparatorCapability(input.capability)) return AvantiqoMusicSeparatorModalProvider.execute(input);
    if (isElasticCapability(input.capability)) return AvantiqoMusicElasticModalProvider.execute(input);
    if (isVocalCorrectionCapability(input.capability)) return AvantiqoMusicVocalCorrectionModalProvider.execute(input);
