...
export const AvantiqoAudioProvider = {
  ...
  async execute(input = {}) {
    ...
    if (isSfxCapability(input.capability)) {
      try {
        if (await AvantiqoSfxLocalQueueProvider.available()) return await AvantiqoSfxLocalQueueProvider.execute(input);
      } catch (error) {
        if (String(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED || "").trim().toLowerCase() === "true") throw error;
        console.error("AVANTIQO_SFX_LOCAL_FALLBACK_MODAL", { error: String(error?.message || error).slice(0, 500) });
      }
      throw new Error("AVANTIQO_SFX_LOCAL_NODE_UNAVAILABLE");
    }
    if (isSeparatorCapability(input.capability)) {
      ...
      throw new Error("AVANTIQO_MUSIC_SEPARATOR_LOCAL_NODE_UNAVAILABLE");
    }
    if (isElasticCapability(input.capability)) {
      ...
      throw new Error("AVANTIQO_MUSIC_ELASTIC_LOCAL_NODE_UNAVAILABLE");
    }
    if (isVocalCorrectionCapability(input.capability)) {
      ...
      throw new Error("AVANTIQO_MUSIC_VOCAL_CORRECTION_LOCAL_NODE_UNAVAILABLE");
    }
