...
const MODAL_GENERATION_WORKER = createAvantiqoOwnedModalWorker({
  ...
  family: "audio",
  engineContract: "AVANTIQO_AUDIO_ENGINE_V1",
  httpContract: "AVANTIQO_AUDIO_MODAL_HTTP_V1",
  transport: "modal-function-call",
  jobPrefix: MODAL_JOB_PREFIX,
  baseUrlEnv: "AVANTIQO_AUDIO_MODAL_BASE_URL",
  tokenEnv: "AVANTIQO_AUDIO_MODAL_GATEWAY_TOKEN",
  enabledEnv: "AVANTIQO_AUDIO_ENGINE_ENABLED",
  timeoutEnv: "AVANTIQO_AUDIO_ENGINE_TIMEOUT_MS",
  defaultModel: "avantiqo-audio-v1",
  outputExtension: "wav",
...
function modalConfigured() {
  const baseUrl = text(process.env.AVANTIQO_AUDIO_MODAL_BASE_URL);
  const token = text(process.env.AVANTIQO_AUDIO_MODAL_GATEWAY_TOKEN);
...
export const AvantiqoAudioProvider = {
  ...
  async execute(input = {}) {
    ...
    if (modalConfigured() && isModalMainCapability(input.capability)) {
