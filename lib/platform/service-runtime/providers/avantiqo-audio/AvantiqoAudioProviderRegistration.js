...
function certifiedCapabilities(value) {
  ...
  const unsupported = requested.filter((item) => !IMPLEMENTED_CAPABILITIES.includes(item));
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    benchmark_required_capabilities: IMPLEMENTED_CAPABILITIES.filter(
      (capability) => !DEFAULT_CERTIFIED_CAPABILITIES.includes(capability),
    ),
    base_model_required_capabilities: ["ai.audio.extend"],
    ...
    separator_runtime: {
      ...
      runtime_status: "IMPLEMENTATION_AND_BENCHMARK_REQUIRED",
