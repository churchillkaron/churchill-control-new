...
const localMusicRuntimeAvailable = Boolean(engineEnabled && localComputeConfigured && localMusicEnabled && baseCertifiedCapabilities.includes("ai.music.generate"));
const primaryAudioRuntimeAvailable = Boolean(localMusicRuntimeAvailable || modalPrimaryAvailable);
...
const vocalCorrectionRuntimeAvailable = Boolean(engineEnabled && modalConfigured && vocalCorrectionEngineEnabled && vocalCorrectionEngineCertified);
...
const elasticAudioRuntimeAvailable = Boolean(engineEnabled && modalConfigured && elasticAudioEngineEnabled && elasticAudioEngineCertified);
const capabilities = [
  ...new Set([
    ...(primaryAudioRuntimeAvailable ? modalCertifiedCapabilities : []),
...
const sfxCertificationEvidenceFingerprint = text(process.env.AVANTIQO_SFX_CERTIFICATION_EVIDENCE_SHA256);
const sfxCertificationEvidenceBound = sha256(sfxCertificationEvidenceFingerprint);
const sfxRuntimeAvailable = Boolean(engineEnabled && modalConfigured && sfxEngineEnabled && sfxEngineCertified && sfxCertificationEvidenceBound);
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    sfx_runtime: {
      owner: "AVANTIQO", capability: SFX_CAPABILITY, product_model: SFX_PRODUCT_MODEL, foundation_model: SFX_FOUNDATION_MODEL, quality_profile: SFX_QUALITY_PROFILE, infrastructure_provider: "MODAL_INTERACTIVE_LOCAL_BATCH_OPTIONAL", runtime_status: sfxRuntimeAvailable ? "CERTIFIED_CONFIGURED" : "CERTIFICATION_OR_CONFIGURATION_REQUIRED", production_routing_allowed: sfxRuntimeAvailable, certification_required: true, output_sample_rate_hz: 48000, maximum_duration_seconds: 30, endpoint_configured: false, local_cpu_runtime: "OPENMOSS_GGML_CPU_V1", local_first: false, local_batch_available: true, local_batch_execution_classes: ["background", "batch", "local_batch"], interactive_runtime: "MODAL_DIRECT_A10G_ASYNC_V1", modal_fallback: true, modal_direct_configured: modalConfigured, engine_enabled: sfxEngineEnabled, engine_certified: sfxEngineCertified, certification_evidence_fingerprint_configured: sfxCertificationEvidenceBound, certification_evidence_sha256: sfxCertificationEvidenceBound ? sfxCertificationEvidenceFingerprint : null, external_provider_fallback_allowed: false,
    },
    ...
    vocal_role_separator_runtime: {
      ...
      preserve_backing_vocals_supported: false,
      remove_lead_vocal_only_supported: false,
      ...
      candidate_models: [SEPARATOR_CANDIDATE_MODEL],
