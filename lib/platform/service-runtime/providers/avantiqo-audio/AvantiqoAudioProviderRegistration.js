...
const primaryAudioRuntimeAvailable = Boolean(
  engineEnabled && endpointId && apiKey && foundationModel === "ACE-Step/Ace-Step1.5" && modelFamily === "ACE_STEP_1_5" && modelVariant === EXPECTED_MODEL_VARIANT && lmEnabled && lmModel === EXPECTED_LM_MODEL && lmBackend === EXPECTED_LM_BACKEND && baseCertifiedCapabilities.length,
);
...
const capabilities = [
  ...new Set([
    ...(primaryAudioRuntimeAvailable ? baseCertifiedCapabilities : []),
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    elastic_audio_runtime: {
      owner: "AVANTIQO",
      capability: ELASTIC_AUDIO_CAPABILITY,
      model: ELASTIC_AUDIO_MODEL,
      engine_contract: ELASTIC_AUDIO_ENGINE_CONTRACT,
      quality_profile: ELASTIC_AUDIO_QUALITY_PROFILE,
      runtime_status: elasticAudioRuntimeAvailable ? "CERTIFIED_CONFIGURED" : "CERTIFICATION_OR_CONFIGURATION_REQUIRED",
      production_routing_allowed: capabilities.includes(ELASTIC_AUDIO_CAPABILITY),
      ...
      endpoint_configured: Boolean(elasticAudioEndpointId),
    },
