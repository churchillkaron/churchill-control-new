...
const modalBaseUrl = text(process.env.AVANTIQO_AUDIO_MODAL_BASE_URL);
const modalGatewayToken = text(process.env.AVANTIQO_AUDIO_MODAL_GATEWAY_TOKEN);
const modalConfigured = Boolean(/^https:\/\//i.test(modalBaseUrl) && modalGatewayToken.length >= 40);
...
const runpodPrimaryAvailable = Boolean(engineEnabled && endpointId && apiKey && modelContractReady && baseCertifiedCapabilities.length);
const modalPrimaryAvailable = Boolean(engineEnabled && modalConfigured && modelContractReady && modalCertifiedCapabilities.length);
const primaryAudioRuntimeAvailable = modalPrimaryAvailable || runpodPrimaryAvailable;
...
const vocalCorrectionRuntimeAvailable = Boolean(vocalCorrectionEngineEnabled && vocalCorrectionEngineCertified && vocalCorrectionEndpointId && apiKey);
const elasticAudioEndpointId = text(process.env.RUNPOD_AVANTIQO_MUSIC_ELASTIC_ENDPOINT_ID);
...
const elasticAudioRuntimeAvailable = Boolean(elasticAudioEngineEnabled && elasticAudioEngineCertified && elasticAudioEndpointId && apiKey);
const capabilities = [
  ...new Set([
    ...(modalPrimaryAvailable ? modalCertifiedCapabilities : runpodPrimaryAvailable ? baseCertifiedCapabilities : []),
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    infrastructure_provider: modalPrimaryAvailable ? "MODAL_A10G_ASYNC_V1" : "RUNPOD_SERVERLESS",
    ...
    modal_http_contract: "AVANTIQO_AUDIO_MODAL_HTTP_V1",
    ...
    runtime_configuration: {
      ...
      runpod_fallback_configured: runpodPrimaryAvailable,
      ...
      elastic_audio_endpoint_configured: Boolean(elasticAudioEndpointId),
      queue_endpoint: true,
      scale_to_zero: true,
      runpod_safe_lease_required_for_primary_lane: !modalPrimaryAvailable,
    },
  },
};
