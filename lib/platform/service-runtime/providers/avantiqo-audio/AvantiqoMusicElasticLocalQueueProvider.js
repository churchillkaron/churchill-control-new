...
export const AvantiqoMusicElasticLocalQueueProvider = {
  ...
  async execute(input = {}) {
    ...
    return { success:true, provider:"avantiqo-audio", model:MODEL, output:{ provider_job_id:`${AVANTIQO_MUSIC_ELASTIC_LOCAL_JOB_PREFIX}${inserted.id}`, status:"queued", capability:CAPABILITY, engine_contract:ENGINE_CONTRACT, infrastructure_provider:INFRASTRUCTURE, local_node:true, raw_reasoning_persisted:false } };
  },
  async cancel(input = {}) {
    const jobId=text(input.job_id || input.jobId || input.provider_job_id), organizationId=text(input.context?.organization_id);
    if (!organizationId) throw new Error("AVANTIQO_MUSIC_ELASTIC_LOCAL_CANCEL_SCOPE_REQUIRED");
    ...
    if (result.error) throw result.error; return {success:true,cancelled:Boolean(result.data),provider_job_id:jobId,node_id:result.data?.node_id||null,exact_job_only:true};
  },
