...
export const AvantiqoMusicGenerationLocalQueueProvider={
  ...
  async execute(input={}){
    ...
    const inserted=await supabaseAdmin.from("avantiqo_local_compute_jobs").insert({organization_id:organizationId,usage_id:usageId,capability:CAPABILITY,lane:"cpu",workload:"music_generation",model:MODEL,payload,priority:42,max_attempts:2}).select("id").single();
    if(inserted.error||!inserted.data?.id) throw inserted.error||new Error("AVANTIQO_MUSIC_GENERATION_LOCAL_QUEUE_INSERT_FAILED");
    return {success:true,provider:"avantiqo-audio",model:PRODUCT_MODEL,output:{provider_job_id:`${AVANTIQO_MUSIC_GENERATION_LOCAL_JOB_PREFIX}${inserted.data.id}`,status:"queued",capability:CAPABILITY,engine_contract:"AVANTIQO_AUDIO_ENGINE_V1",foundation_model:MODEL,storage_reference:target.storage_reference,infrastructure_provider:INFRASTRUCTURE,local_node:true,execution_resource:"LOCAL_CPU_FLOAT32",ace_step_lm_used:false,raw_reasoning_persisted:false}};
  },
