...
export const AvantiqoMusicVocalCorrectionLocalQueueProvider={
  ...
  async execute(input={}){
    ...
    const inserted=await supabaseAdmin.from("avantiqo_local_compute_jobs").insert({organization_id:organizationId,usage_id:usageId,capability:CAPABILITY,lane:"gpu",workload:"music_vocal_correction",model:MODEL,payload,priority:65,max_attempts:2}).select("id").single();
    if(inserted.error||!inserted.data?.id) throw inserted.error||new Error("AVANTIQO_MUSIC_VOCAL_CORRECTION_LOCAL_QUEUE_INSERT_FAILED");
    return {success:true,provider:"avantiqo-audio",model:MODEL,output:{provider_job_id:`${AVANTIQO_MUSIC_VOCAL_CORRECTION_LOCAL_JOB_PREFIX}${inserted.data.id}`,status:"queued",engine_contract:ENGINE_CONTRACT,capability:CAPABILITY,infrastructure_provider:INFRASTRUCTURE,local_node:true,raw_reasoning_persisted:false}};
  },
