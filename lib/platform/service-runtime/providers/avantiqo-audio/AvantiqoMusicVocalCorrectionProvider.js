...
export const AvantiqoMusicVocalCorrectionProvider = {
  ...
  async execute(input = {}) {
    ...
    const approvedSource = object(providerParameters.approved_tuning_plan);
    const approvedTuningPlan = Object.keys(approvedSource).length
      ? cleanOutput(approvedSource)
      : null;
    const sourceWindowSource = object(providerParameters.source_window);
    const sourceWindow = Object.keys(sourceWindowSource).length
      ? cleanOutput(sourceWindowSource)
      : null;
    ...
    if (approvedTuningPlan && text(approvedTuningPlan.contract) !== TUNING_PLAN_CONTRACT) {
      throw new Error("AVANTIQO_MUSIC_VOCAL_CORRECTION_APPROVED_PLAN_CONTRACT_INVALID");
    }
    if (approvedTuningPlan && !sourceWindow) {
      throw new Error("AVANTIQO_MUSIC_VOCAL_CORRECTION_APPROVED_PLAN_SOURCE_WINDOW_REQUIRED");
    }
    ...
    const response = await fetchWithTimeout(`${baseUrl}/run`, {
      ...
      body: JSON.stringify({
        input: {
          ...
          approved_tuning_plan: approvedTuningPlan,
    ...
    return {
      ...
      output: {
        ...
        capability: CAPABILITY,
        quality_profile: QUALITY_PROFILE,
        execution_mode: approvedTuningPlan ? "MUSICIAN_APPROVED_PLAN" : "AUTOMATIC_CERTIFICATION",
