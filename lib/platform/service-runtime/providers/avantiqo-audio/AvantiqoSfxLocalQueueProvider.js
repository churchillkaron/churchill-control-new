...
export const AvantiqoSfxLocalQueueProvider = {
  ...
  async execute(input = {}) {
    ...
    if (inserted.error || !inserted.data?.id) throw inserted.error || new Error("AVANTIQO_SFX_LOCAL_QUEUE_INSERT_FAILED");
    return { success: true, provider: "avantiqo-audio", model: PRODUCT_MODEL, output: {
      provider_job_id: `${AVANTIQO_SFX_LOCAL_JOB_PREFIX}${inserted.data.id}`, status: "queued",
