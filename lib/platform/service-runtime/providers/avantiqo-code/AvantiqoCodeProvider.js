...
async function executeServerlessWithOwnedWake(input = {}) {
          ...
          ...(wake
            ? {
                zero_idle_lifecycle: {
                  ...
                  workers_min: wake.workers_min,
                  workers_max: wake.workers_max,
  ...
  throw new Error("AVANTIQO_CODE_ZERO_IDLE_WAKE_PROPAGATION_EXHAUSTED");
}

async function settleServerlessWithOwnedReap(input = {}) {
  const result = await serverlessWorker.getStatus(input);
