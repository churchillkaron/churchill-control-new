...
export const AvantiqoCodeProviderV2 = {
  ...
  async execute(input = {}) {
    if (isCodeLocalCapability(input.capability)) {
      try {
        if (await AvantiqoCodeLocalQueueProvider.available()) return await AvantiqoCodeLocalQueueProvider.execute(input);
      } catch (error) {
        if (text(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED).toLowerCase() === "true") throw error;
        console.error("AVANTIQO_CODE_LOCAL_FALLBACK_MODAL", { error: text(error?.message || error).slice(0, 500) });
      }
    }
