  ...
  async execute(input = {}) {
    ...
    if (capability === "ai.image.upscale") {
      if (await AvantiqoImageUpscaleLocalQueueProvider.available()) {
        return AvantiqoImageUpscaleLocalQueueProvider.execute(input);
      }
      throw new Error("AVANTIQO_IMAGE_UPSCALE_LOCAL_NODE_REQUIRED");
    }

    if (localVisionCapability) {
      if (await AvantiqoDocumentVisionLocalQueueProvider.available(capability)) {
        return AvantiqoDocumentVisionLocalQueueProvider.execute(input);
      }
      if (
        input.infrastructure_policy === "local_only" ||
        input.local_compute_required === true ||
        input.metadata?.local_only === true
      ) {
        throw new Error("AVANTIQO_DOCUMENT_VISION_LOCAL_NODE_REQUIRED");
      }
    }

    const nonGenerativeCapability = localVisionCapability || depthCapability;
    if (!nonGenerativeCapability && String(process.env.AVANTIQO_STUDIO_VISUAL_GENERATION_ENABLED || "").trim() !== "1") {
