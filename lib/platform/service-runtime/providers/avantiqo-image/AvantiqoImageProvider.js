...
export const AvantiqoImageProvider = {
  ...
  async execute(input = {}) {
    ...
    if (capability === "document.ocr" || capability === "document.classify") {
      try {
        if (await AvantiqoDocumentVisionLocalQueueProvider.available()) return await AvantiqoDocumentVisionLocalQueueProvider.execute(input);
      } catch (error) {
        if (String(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED || "").trim().toLowerCase() === "true") throw error;
        console.error("AVANTIQO_DOCUMENT_VISION_LOCAL_FALLBACK_MODAL", { error: String(error?.message || error).slice(0, 500) });
      }
    }
    if (capability === "ai.image.upscale") {
      try {
        if (await AvantiqoImageUpscaleLocalQueueProvider.available()) return await AvantiqoImageUpscaleLocalQueueProvider.execute(input);
      } catch (error) {
        if (String(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED || "").trim().toLowerCase() === "true") throw error;
        console.error("AVANTIQO_IMAGE_UPSCALE_LOCAL_FALLBACK_MODAL", { error: String(error?.message || error).slice(0, 500) });
      }
    }
