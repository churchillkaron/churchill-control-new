...
export const AvantiqoImageUpscaleLocalQueueProvider = {
  ...
  async execute(input = {}) {
    ...
    const scale = Number(input.scale) === 2 ? 2 : 4;
    const runtimeModel = scale === 2 ? "caidas/swin2SR-classical-sr-x2-64" : MODEL;
    const inserted = await enqueueLocalComputeJob({ organization_id: organizationId, usage_id: usageId, capability: CAPABILITY, lane: "gpu", workload: "image_upscale", model: runtimeModel, payload: { source_url: sourceUrl, storage_upload: storageUpload, scale }, priority: 70, max_attempts: 2, input });
    return { success: true, provider: "avantiqo-image", model: "avantiqo-image-v1", output: { provider_job_id: JOB_PREFIX + inserted.id, status: "queued", capability: CAPABILITY, scale, storage_reference: storageUpload.storage_reference, infrastructure_provider: INFRASTRUCTURE, local_node: true, raw_reasoning_persisted: false } };
  },
