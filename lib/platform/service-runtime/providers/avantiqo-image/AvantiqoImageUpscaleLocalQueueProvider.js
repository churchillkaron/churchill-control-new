...
export const AvantiqoImageUpscaleLocalQueueProvider = {
  ...
  async execute(input = {}) {
    ...
    const inserted = await supabaseAdmin.from("avantiqo_local_compute_jobs").insert({ organization_id: organizationId, usage_id: usageId, capability: CAPABILITY, lane: "gpu", workload: "image_upscale", model: MODEL, payload: { source_url: sourceUrl, storage_upload: storageUpload }, priority: 70, max_attempts: 2 }).select("id").single();
    if (inserted.error || !inserted.data?.id) throw inserted.error || new Error("AVANTIQO_LOCAL_IMAGE_UPSCALE_QUEUE_INSERT_FAILED");
    return { success: true, provider: "avantiqo-image", model: "avantiqo-image-v1", output: { provider_job_id: `${JOB_PREFIX}${inserted.data.id}`, status: "queued", capability: CAPABILITY, storage_reference: storageUpload.storage_reference, infrastructure_provider: INFRASTRUCTURE, local_node: true, raw_reasoning_persisted: false } };
  },
