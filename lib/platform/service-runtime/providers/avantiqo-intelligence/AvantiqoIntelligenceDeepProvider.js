...
const DEFAULT_COLD_START_RETRY_DEADLINE_MS = 90000;
const DEFAULT_COLD_START_RETRY_DELAY_MS = 10000;
const MAX_COLD_START_RETRY_ATTEMPTS = 10;
...
function coldStartRetryDeadlineMs(input = {}) {
  const value = Number(
    input.cold_start_retry_deadline_ms ||
      input.coldStartRetryDeadlineMs ||
      process.env.AVANTIQO_INTELLIGENCE_COLD_START_RETRY_DEADLINE_MS ||
      DEFAULT_COLD_START_RETRY_DEADLINE_MS,
  );
...
function coldStartRetryDelayMs() {
  const value = Number(
    process.env.AVANTIQO_INTELLIGENCE_COLD_START_RETRY_DELAY_MS ||
      DEFAULT_COLD_START_RETRY_DELAY_MS,
  );
  return Number.isFinite(value)
    ? Math.max(1000, Math.min(30000, value))
    : DEFAULT_COLD_START_RETRY_DELAY_MS;
}

function retryableColdStartHttpError(error) {
  const message = text(error?.message || error);
  if (/^AVANTIQO_INTELLIGENCE_REQUEST_FAILED:409:/i.test(message)) return true;
  if (!/^AVANTIQO_INTELLIGENCE_REQUEST_FAILED:500:/i.test(message)) return false;
...
async function chatCompletion(input = {}) {
  ...
  const deadlineMs = coldStartRetryDeadlineMs(input);
  const retryDelayMs = coldStartRetryDelayMs();
  const startedAt = Date.now();

  for (let attempt = 1; attempt <= MAX_COLD_START_RETRY_ATTEMPTS; attempt += 1) {
    try {
      return await requestJson(`${baseUrl}/chat/completions`, requestOptions);
    } catch (error) {
      const elapsedMs = Date.now() - startedAt;
      const remainingMs = deadlineMs - elapsedMs;
      const retryable = retryableColdStartHttpError(error);
      if (
        !retryable ||
        attempt >= MAX_COLD_START_RETRY_ATTEMPTS ||
  ...
  throw new Error("AVANTIQO_INTELLIGENCE_COLD_START_RETRY_EXHAUSTED");
}
...
export function getAvantiqoIntelligenceRuntimeConfiguration() {
  ...
  return {
    ...
    cold_start_http_retry: "SAFE_LEASE_ONLY_EXPLICIT_409_OR_GENERIC_500_V1",
    cold_start_retry_deadline_ms: DEFAULT_COLD_START_RETRY_DEADLINE_MS,
    runtime_ready:
      apiKeyConfigured &&
      endpointResolvable &&
