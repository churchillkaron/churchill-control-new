...
function openAiResponseFromProxyOutput(value, depth = 0) {
  if (depth > 6 || value == null) return null;
  if (typeof value === "string") {
    const parsed = parseJson(value);
    return parsed && parsed !== value
      ? openAiResponseFromProxyOutput(parsed, depth + 1)
      : null;
  }
  if (Array.isArray(value)) {
    for (const item of value) {
      const response = openAiResponseFromProxyOutput(item, depth + 1);
  ...
  for (const key of ["body", "response", "data", "output", "result"]) {
    if (!Object.prototype.hasOwnProperty.call(value, key)) continue;
    const response = openAiResponseFromProxyOutput(value[key], depth + 1);
    if (response) return response;
  }
  return null;
}

function jobErrorDetail(job = {}) {
  const output = job?.output;
  if (typeof output === "string") return text(output).slice(0, 700);
  if (Array.isArray(output)) {
    for (const item of output) {
      const detail = text(
        item?.error?.message || item?.error || item?.message || item?.detail,
      );
      if (detail) return detail.slice(0, 700);
    }
  }
  return text(
    job?.error?.message || job?.error || job?.message || job?.detail,
  ).slice(0, 700);
}

async function cancelJob(apiBase, apiKey, jobId) {
  try {
    await requestJson(`${apiBase}/cancel/${encodeURIComponent(jobId)}`, {
      method: "POST",
...
async function chatCompletion(input = {}) {
  ...
  const submission = await requestJson(`${apiBase}/run`, {
    method: "POST",
    headers: headers(apiKey),
    requestTimeoutMs: Math.min(30000, timeoutMs),
    body: JSON.stringify({
      input: {
  ...
  if (!jobId) {
    throw new Error("AVANTIQO_INTELLIGENCE_FAST_NATIVE_JOB_ID_REQUIRED");
  }

  const pollMs = Math.max(
    250,
    Number(process.env.AVANTIQO_INTELLIGENCE_FAST_POLL_MS || DEFAULT_POLL_MS),
  );
  while (Date.now() - startedAt <= timeoutMs) {
    const job = await requestJson(
      `${apiBase}/status/${encodeURIComponent(jobId)}`,
      {
        method: "GET",
        headers: headers(apiKey),
        requestTimeoutMs: Math.min(15000, Math.max(3000, timeoutMs)),
      },
    );
    const status = text(job?.status).toUpperCase();
    if (status === "COMPLETED") {
      const response = openAiResponseFromProxyOutput(job?.output);
      if (!response) {
        const detail = jobErrorDetail(job) || "proxy output did not contain OpenAI choices";
        throw new Error(
          `AVANTIQO_INTELLIGENCE_FAST_PROXY_OUTPUT_INVALID:${detail}`,
        );
      }
      return response;
    }
    if (["FAILED", "CANCELLED", "TIMED_OUT"].includes(status)) {
      const detail = jobErrorDetail(job) || "no detail";
      throw new Error(
        `AVANTIQO_INTELLIGENCE_FAST_NATIVE_JOB_${status}:${detail}`,
      );
    }
    await sleep(pollMs);
  }

  const cancelled = await cancelJob(apiBase, apiKey, jobId);
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_NATIVE_JOB_TIMEOUT:timeout_ms=${timeoutMs}:cancelled=${cancelled ? "YES" : "NO"}`,
  );
}
...
export function getAvantiqoIntelligenceFastRuntimeConfiguration() {
  ...
  return {
    ...
    transport: "RUNPOD_NATIVE_ASYNC_VLLM_PROXY",
