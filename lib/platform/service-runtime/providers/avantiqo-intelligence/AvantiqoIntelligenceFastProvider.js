...
function fastColdStartRetryDeadlineMs(input = {}) {
  return boundedMilliseconds(
    input.cold_start_retry_deadline_ms ||
      input.coldStartRetryDeadlineMs ||
      process.env.AVANTIQO_INTELLIGENCE_FAST_COLD_START_RETRY_DEADLINE_MS,
...
function fastColdStartRetryDelayMs() {
  return boundedMilliseconds(
    process.env.AVANTIQO_INTELLIGENCE_FAST_COLD_START_RETRY_DELAY_MS,
...
function retryableFastColdStartHttpError(error) {
  const message = text(error?.message || error);
  if (/^AVANTIQO_INTELLIGENCE_FAST_REQUEST_FAILED:409:/i.test(message)) return true;
  if (!/^AVANTIQO_INTELLIGENCE_FAST_REQUEST_FAILED:500:/i.test(message)) return false;
...
async function chatCompletion(input = {}, { signal = null } = {}) {
  ...
  const retryDeadlineMs = fastColdStartRetryDeadlineMs(input);
  const retryDelayMs = fastColdStartRetryDelayMs();
  ...
  for (let attempt = 1; attempt <= MAX_COLD_START_RETRY_ATTEMPTS; attempt += 1) {
    try {
      ...
      const remainingMs = retryDeadlineMs - elapsedMs;
      const retryable = retryableFastColdStartHttpError(error);
      if (
        signal?.aborted ||
        !retryable ||
  ...
  throw new Error("AVANTIQO_INTELLIGENCE_FAST_COLD_START_RETRY_EXHAUSTED");
}
...
export function getAvantiqoIntelligenceFastRuntimeConfiguration() {
  ...
  return {
    ...
    cold_start_retry_policy: "RUNPOD_409_OR_GENERIC_500_BOUNDED_V1",
    cold_start_retry_deadline_ms: fastColdStartRetryDeadlineMs(),
    cold_start_retry_delay_ms: fastColdStartRetryDelayMs(),
    cold_start_retry_max_attempts: MAX_COLD_START_RETRY_ATTEMPTS,
