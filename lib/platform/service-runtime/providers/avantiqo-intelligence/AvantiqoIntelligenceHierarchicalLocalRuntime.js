...
function requestedChildTimeoutMs(input = {}) {
  ...
  return Math.max(10_000, Math.min(Math.floor(value), 120_000));
}

function hierarchicalStageExecutionKey(input = {}, stage = "") {
  const base = text(
  ...
  return base && text(stage)
    ? `${base}:hierarchical:${text(stage)}`
    : null;
}

async function runLocalPass(input, prompt, maxOutputTokens, stage) {
  const stageExecutionKey = hierarchicalStageExecutionKey(input, stage);
...
export async function executeHierarchicalLocalIntelligence(input = {}) {
  ...
  const finalStageExecutionKey = hierarchicalStageExecutionKey(input, 'final');
  const finalInput = {
    ...input,
