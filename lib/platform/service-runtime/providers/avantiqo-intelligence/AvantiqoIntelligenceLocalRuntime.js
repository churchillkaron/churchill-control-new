...
const FRONT_FOUNDATION_MODEL = "Qwen/Qwen3-4B-GGUF:Q4_K_M";
...
export function shouldUseLocalIntelligence(input = {}) {
  ...
  if (lane === "front" && !localIntelligenceContextFits(input, lane)) return false;
  if (lane === "front") return true;
...
export async function executeIntelligenceLocal(input = {}) {
  ...
  const payload = {
    ...
    options: {
      ...
      num_predict: Math.max(1, Math.min(Number(input.max_output_tokens || input.maxOutputTokens) || 1024, localOutputTokenCap(lane))),
    },
    ...(tools.length ? { tools } : {}),
