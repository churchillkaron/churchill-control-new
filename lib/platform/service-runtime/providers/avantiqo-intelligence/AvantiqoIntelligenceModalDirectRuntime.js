...
export async function prewarmIntelligenceModalFront(input = {}) {
  ...
  const method = await frontMethod(client, lookupOptions, "warmup");
  const result = clean(await method.remote([]));
  if (result?.status !== "ready" || text(result?.runtime_contract) !== FRONT_RUNTIME_CONTRACT) {
    throw new Error("AVANTIQO_INTELLIGENCE_FRONT_PREWARM_INVALID");
  }
  return {
    success: true,
    ready: true,
...
export async function getIntelligenceModalDirectHealth(input = {}) {
  ...
  const lookupOptions = cfg.environment ? { environment: cfg.environment } : {};
  await Promise.all([
    client.cls.fromName(FRONT_APP_NAME, FRONT_CLASS_NAME, lookupOptions),
  ...
  return {
    ...
    modal_classes: [FRONT_CLASS_NAME],
