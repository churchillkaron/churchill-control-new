...
const FRONT_MODEL = "Qwen/Qwen3-1.7B-GGUF:Q8_0";
...
function modelForLane(lane) {
  if (lane === "front") return FRONT_MODEL;
...
export function buildIntelligenceModalDirectPayload(input = {}, context = {}, lane = "deep") {
  ...
  const payload = clean({
    ...
    front_task_mode: input.front_task_mode || input.frontTaskMode,
    ...(tools ? { tools } : {}),
...
export async function executeIntelligenceModalDirect(input = {}) {
  ...
  if (lane === "front") {
    ...
    const completed = validateCompletedOutput(clean(await worker.remote([payload])));
    return {
      success: true,
