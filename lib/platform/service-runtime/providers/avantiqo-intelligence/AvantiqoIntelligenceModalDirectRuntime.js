...
function modalPayload(input, context, lane) {
  const tools = Array.isArray(input.tools) && input.tools.length > 0 ? input.tools : undefined;
...
function validateCompletedOutput(value) {
  ...
  if (output.runpod_inference_performed !== false) {
    throw new Error("AVANTIQO_INTELLIGENCE_MODAL_DIRECT_RUNPOD_BOUNDARY_INVALID");
  }
...
export async function executeIntelligenceModalDirect(input = {}) {
  ...
  const payload = modalPayload(input, context, lane);
  const { client } = await clientFor(cfg);
  const lookupOptions = cfg.environment ? { environment: cfg.environment } : {};
