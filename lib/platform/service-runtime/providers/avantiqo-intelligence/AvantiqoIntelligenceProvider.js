...
function requireWarmWorker() {
  return !disabled(process.env.AVANTIQO_INTELLIGENCE_REQUIRE_WARM_WORKER);
}
...
export async function probeAvantiqoIntelligenceRuntime({ health: providedHealth = null } = {}) {
  const model = configuredModel();
  const startedAt = Date.now();
  const health = providedHealth || await endpointHealth();
