...
const runpodFastEndpointId = text(process.env.RUNPOD_AVANTIQO_INTELLIGENCE_FAST_ENDPOINT_ID);
const runpodApiKey = text(process.env.RUNPOD_API_KEY);
const runpodManagementKey = text(process.env.RUNPOD_MANAGEMENT_API_KEY) || runpodApiKey;
const modalBaseUrl = text(process.env.AVANTIQO_INTELLIGENCE_MODAL_BASE_URL);
const modalGatewayToken = text(process.env.AVANTIQO_INTELLIGENCE_MODAL_GATEWAY_TOKEN);
const modalConfigured = Boolean(/^https:\/\//i.test(modalBaseUrl) && modalGatewayToken.length >= 40);
const runpodEndpointResolvable = Boolean(runpodEndpointId || runpodFastEndpointId || runpodManagementKey);
const runpodConfigured = Boolean(runpodApiKey && runpodManagementKey && runpodEndpointResolvable);
...
const runtimeAvailable = Boolean(
  (modalConfigured || runpodConfigured) && (engineEnabled || localReviewRuntimeAllowed),
);
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    modal_http_contract: "AVANTIQO_INTELLIGENCE_MODAL_HTTP_V1",
    ...
    runtime_configuration: {
      ...
      raw_reasoning_persisted: false,
      runpod_safe_lease_required: !modalConfigured,
    },
