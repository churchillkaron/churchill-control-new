...
export const AvantiqoIntelligenceProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    if (selectedLocalModel && shouldUseHierarchicalLocalIntelligence(input)) {
      try {
        return await executeHierarchicalLocalIntelligence(input);
      } catch (error) {
        if (localComputeRequired(input)) throw error;
        console.error("AVANTIQO_HIERARCHICAL_LOCAL_FALLBACK_MODAL", {
