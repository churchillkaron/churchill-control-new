...
function candidateAssets(input = {}) {
  return [
    ...
    input.sourceAssets,
    input.assets,
    input.generation?.primary_source_asset_id,
    input.provider_parameters?.primary_source_asset_id,
    input.requirements?.primary_source_asset_id,
...
async function resolveSemanticAssetRoles(input = {}) {
  ...
  const roleCandidates = {
    source_image: [input.source_image, input.sourceImage, input.image, input.source, input.generation?.primary_source_asset_id, input.provider_parameters?.primary_source_asset_id, input.requirements?.primary_source_asset_id],
  ...
  return {
    ...
    async execute(input = {}) {
      ...
      const modalApproval = await consumeModalComputeApproval({
        input,
        capability,
        infrastructureProvider: "MODAL_DIRECT_ASYNC_V1",
      });
