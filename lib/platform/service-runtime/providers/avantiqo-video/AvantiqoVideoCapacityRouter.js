...
const MIN_OWNED_STOCK_RANK = 3; // MEDIUM or HIGH
const DEFAULT_CACHE_MS = 10_000;

const STOCK_RANK = Object.freeze({
  HIGH: 4,
  MEDIUM: 3,
  LOW: 2,
});
...
function rank(value) {
  return STOCK_RANK[text(value).toUpperCase()] || 0;
}
...
async function snapshot() {
  ...
  const usable = rows.filter((row) =>
    ...
    finite(row.memory_gb, 0) >= MINIMUM_VRAM_GB
  );
...
export function decideAvantiqoVideoRoute({ capability, capacity, fallbackReady = fallbackConfigured() }) {
  ...
  if (workerReady) {
    return {
      route: "OWNED",
      reason: "OWNED_WORKER_ALREADY_ALLOCATED",
      fallback_ready: fallbackReady,
    };
  }

  const endpointCanScale = finite(capacity?.workers_max, 0) > 0;
  if (!endpointCanScale) {
    if (fallbackReady) {
      return {
        route: "MANAGED_FALLBACK",
        reason: "OWNED_ENDPOINT_CERTIFICATION_REST_0_0",
        fallback_ready: true,
      };
    }
    return {
      route: "UNAVAILABLE",
      reason: "OWNED_ENDPOINT_NOT_CUSTOMER_RUNNABLE_AND_FALLBACK_UNAVAILABLE",
      fallback_ready: false,
    };
  }

  if (capacity?.management_capacity_visible === true &&
      finite(capacity?.best_stock_rank) >= MIN_OWNED_STOCK_RANK) {
    return {
      route: "OWNED",
