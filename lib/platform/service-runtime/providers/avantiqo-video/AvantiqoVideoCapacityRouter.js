...
function fallbackConfigured() {
  const externalPaidFallbackApproved = enabled(
    process.env.AVANTIQO_VIDEO_EXTERNAL_PAID_FALLBACK_APPROVED,
    false,
  );
  if (!externalPaidFallbackApproved) return false;

  const managedFallbackEnabled = enabled(
    process.env.AVANTIQO_VIDEO_MANAGED_FALLBACK_ENABLED,
    false,
  );
  if (!managedFallbackEnabled) return false;
...
export function decideAvantiqoVideoRoute({ capability, capacity, fallbackReady = fallbackConfigured() }) {
  ...
  if (capacity?.management_capacity_visible === true && finite(capacity?.best_stock_rank) >= MIN_OWNED_STOCK_RANK) {
    const parked = finite(capacity?.workers_max, 0) === 0;
    return {
      route: "OWNED",
      reason: parked
        ? `OWNED_PRODUCTION_PARKED_LEASEABLE_${text(capacity.best_stock) || "MEDIUM"}_STOCK`
        : `OWNED_${text(capacity.best_stock) || "MEDIUM"}_STOCK_VISIBLE`,
      fallback_ready: fallbackReady,
      runpod_lease_required: parked,
    };
  }
