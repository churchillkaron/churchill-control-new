...
} from "./AvantiqoVideoProviderV2.js";

const FLAG = Symbol.for(
  "avantiqo.video.cinematic-state-memory-provider-bootstrap.v2",
);
...
function resolution(input = {}) {
  const candidate = object(
    input.cinematic_continuity_resolution ||
    input.requirements?.cinematic_continuity_resolution,
  );
  return text(candidate.contract) === RESOLUTION_CONTRACT ? candidate : null;
}

function continuityGate(input = {}) {
  const candidate = object(input.requirements?.cinematic_continuity_gate);
  if (text(candidate.contract) !== GATE_CONTRACT) return null;
  if (candidate.passed !== true) {
    throw new Error("AVANTIQO_VIDEO_CINEMATIC_CONTINUITY_GATE_NOT_PASSED");
  }
  return candidate;
}
...
function bindInput(input = {}) {
  ...
  if (!memory) return { input, memory: null, resolved: null, gate: null };
  const gate = continuityGate(input);
  const resolved = resolution(input);
  ...
  const governedShotSpecification = {
    ...shotSpecification,
    cinematic_state_memory: memory,
    ...(resolved ? { cinematic_continuity_resolution: compact(resolved) } : {}),
  };
  const governedContinuity = {
    ...continuity,
    ...
    ...(resolved
      ? {
          ...
          resolved_cinematic_state: compact(resolved),
        }
      : {}),
    ...(gate
      ? {
          cinematic_continuity_gate_contract: GATE_CONTRACT,
          cinematic_continuity_gate_analysis_hash: gate.analysis_hash || null,
  ...
  return {
    ...
    input: {
      ...input,
      shot_specification: governedShotSpecification,
      continuity: governedContinuity,
      generation: {
...
function bindOutput(result = {}, memory = null, resolved = null, gate = null) {
  ...
  return {
    ...
    output: {
      ...
      cinematic_continuity_resolution_bound: Boolean(resolved),
      cinematic_continuity_resolution_contract:
        resolved ? RESOLUTION_CONTRACT : null,
      cinematic_continuity_gate_passed: gate?.passed === true,
      cinematic_continuity_gate_analysis_hash: gate?.analysis_hash || null,
    },
  };
}
...
function install() {
  if (AvantiqoVideoProviderV2[FLAG]) return;
  const executeWithoutStateMemory = AvantiqoVideoProviderV2.execute.bind(
    AvantiqoVideoProviderV2,
  );
  Object.defineProperty(AvantiqoVideoProviderV2, FLAG, {
    value: true,
    enumerable: false,
    configurable: false,
  });

  AvantiqoVideoProviderV2.execute = async function executeWithCinematicStateMemory(
    input = {},
  ) {
    const bound = bindInput(input);
    const result = await executeWithoutStateMemory(bound.input);
    return bindOutput(result, bound.memory, bound.resolved, bound.gate);
  };
}
