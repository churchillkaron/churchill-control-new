...
function governedShotPhrases(input = {}, target = []) {
  const requirements = object(input.requirements);
  const frame = object(requirements.frame_plan);
  const location = object(requirements.location);
  const camera = object(requirements.camera);
  const lighting = object(requirements.lighting);
  const design = object(requirements.production_design);
  const dna = object(requirements.cinematic_dna);
  const iconic = object(dna.iconic_frame);
  for (const value of [
    requirements.subject,
    requirements.action,
    frame.opening_frame,
    frame.progression || frame.progression_frames,
    frame.closing_frame,
    location.description || location.name,
    camera.framing || camera.angle,
    camera.movement || camera.movement_path,
    lighting.source,
    lighting.quality || lighting.direction,
    dna.visual_hierarchy,
    dna.depth_strategy,
    dna.lighting_philosophy,
    dna.camera_philosophy,
    dna.material_behavior,
    iconic.target,
    requirements.beauty_intent,
    requirements.focal_path,
    requirements.depth_layers,
    ...Object.values(design).slice(0, 4),
  ]) pushUnique(target, value);
  return target;
}
...
export function compileAvantiqoVideoFastPreviewPrompt(input = {}) {
  ...
  if (!visualRepairFound) {
    governedShotPhrases(input, phrases);
    beatPhrases(input, phrases);
  }
  const body = phrases.slice(0, 14).join(". ").slice(0, 2200).replace(/[. ]+$/, "");
  if (!body) throw new Error("AVANTIQO_VIDEO_FAST_PREVIEW_VISUAL_DIRECTION_REQUIRED");
  return assertCleanPrompt(body + ".");
}
