...
const WEIGHT_STAT_RETRYABLE_PATTERN = /_STAT_HTTP_(404|502|503|504)$/;
const WEIGHT_STAT_ATTEMPTS = 8;
const WEIGHT_STAT_RETRY_MS = 2_000;
...
async function statRequiredWeight(volumeBridge, key) {
  let lastError = null;
  for (let attempt = 1; attempt <= WEIGHT_STAT_ATTEMPTS; attempt += 1) {
    try {
      return await statAvantiqoVideoVolumeObjectViaCpuBridge(volumeBridge, key);
    } catch (error) {
      lastError = error;
      if (!WEIGHT_STAT_RETRYABLE_PATTERN.test(text(error?.message)) || attempt === WEIGHT_STAT_ATTEMPTS) throw error;
      await sleep(WEIGHT_STAT_RETRY_MS);
    }
  }
  throw lastError || new Error("AVANTIQO_VIDEO_FLASHDREAMS_WEIGHT_STAT_FAILED");
}

async function requireWeightsPreloaded(volumeBridge) {
  for (const key of REQUIRED_WEIGHT_KEYS) {
    const stat = await statRequiredWeight(volumeBridge, key);
...
function normalizePodRows(raw) {
  if (Array.isArray(raw)) return raw;
  for (const key of ["pods", "data", "items", "results"]) {
...
async function findExistingMasterPod(ownerRequestId, snapshot) {
  ...
  const pods = normalizePodRows(await podRest("/pods"));
  const matches = pods.filter((pod) => text(pod?.name) === name && !podTerminal(pod));
...
function masterPodBody({ ownerRequestId, snapshot, prepared, gpuTypeId, minRAMPerGPU }) {
  return {
...
async function createMasterPod({ ownerRequestId, snapshot, prepared }) {
  ...
  let created = null;
  let requestedGpu = null;
  const placementAttempts = [];
  for (const gpuTypeId of AVANTIQO_VIDEO_FLASHDREAMS_GPU_POOL) {
    for (const minRAMPerGPU of PLACEMENT_RAM_GB) {
      try {
        created = await podRest("/pods", {
          method: "POST",
          timeoutMs: 45_000,
          body: masterPodBody({ ownerRequestId, snapshot, prepared, gpuTypeId, minRAMPerGPU }),
        });
        requestedGpu = gpuTypeId;
        placementAttempts.push({ gpu_type_id: gpuTypeId, min_ram_gb: minRAMPerGPU, result: "PLACED" });
        break;
      } catch (error) {
        if (!placementUnavailable(error)) throw error;
        placementAttempts.push({ gpu_type_id: gpuTypeId, min_ram_gb: minRAMPerGPU, result: "NO_INSTANCE" });
  ...
  if (!created) {
    const exhausted = new Error(`AVANTIQO_VIDEO_FLASHDREAMS_PLACEMENT_EXHAUSTED:${AVANTIQO_VIDEO_POD_DC}:${placementAttempts.map((row) => row.gpu_type_id).join("|")}`);
    exhausted.httpStatus = 500;
    exhausted.placementAttempts = placementAttempts;
    throw exhausted;
  }
  ...
  return { id, gpu_type_id: gpu || requestedGpu || "PLACEMENT_PENDING", placement_attempts: placementAttempts };
}
...
export async function submitAvantiqoVideoFlashDreamsMaster({ organizationId, sourceUrl } = {}) {
  ...
  try {
    ...
    return {
      ...
      placement_attempts: pod.placement_attempts,
