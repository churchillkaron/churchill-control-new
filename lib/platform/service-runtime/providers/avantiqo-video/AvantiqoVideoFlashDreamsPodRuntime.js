...
const PLACEMENT_RAM_GB = Object.freeze([64, 48]);
...
const stockRank = (value) => ({ HIGH: 4, MEDIUM: 3, LOW: 2, AVAILABLE: 1 })[text(value).toUpperCase()] || 0;
...
async function flashDreamsCapacity() {
  const query = `query FlashDreamsCapacity($input:GpuAvailabilityInput){gpuTypes{id memoryInGb secureCloud} dataCenters{id gpuAvailability(input:$input){available stockStatus gpuTypeId}}}`;
  const body = await runpodJson(GRAPHQL, {
    method: "POST",
    timeoutMs: 30_000,
    body: { query, variables: { input: { gpuCount: 1, minDisk: 5, minMemoryInGb: 48, secureCloud: true } } },
  });
  const errors = list(body?.errors).map((entry) => text(entry?.message)).filter(Boolean);
  if (errors.length) throw new Error(`AVANTIQO_VIDEO_FLASHDREAMS_CAPACITY_GRAPHQL:${errors.join(" | ").slice(0, 500)}`);
  const dc = list(body?.data?.dataCenters).find((row) => text(row?.id) === AVANTIQO_VIDEO_POD_DC);
  if (!dc) throw new Error(`AVANTIQO_VIDEO_FLASHDREAMS_DATACENTER_REQUIRED:${AVANTIQO_VIDEO_POD_DC}`);
  ...
  const rows = AVANTIQO_VIDEO_FLASHDREAMS_GPU_POOL.map((gpuTypeId, poolIndex) => {
    ...
    return {
      ...
      stock: text(stock?.stockStatus).toUpperCase() || "UNAVAILABLE",
      stock_rank: stockRank(stock?.stockStatus),
    };
  });
  const available = rows
    .filter((row) => row.available && row.secure_cloud !== false)
    .sort((a, b) => (b.stock_rank - a.stock_rank) || (a.pool_index - b.pool_index));
  if (!available.length) {
    const summary = rows.map((row) => `${row.gpu_type_id}=${row.stock}`).join(",");
    throw new Error(`AVANTIQO_VIDEO_FLASHDREAMS_NO_COMPATIBLE_STOCK:${AVANTIQO_VIDEO_POD_DC}:${summary}`);
  }
  return {
    data_center_id: AVANTIQO_VIDEO_POD_DC,
    selected_gpu_type_ids: available.map((row) => row.gpu_type_id),
    selected_primary_gpu_type_id: available[0].gpu_type_id,
    rows,
  };
}
...
async function findExistingMasterPod(ownerRequestId, snapshot) {
  const name = `avantiqo-video-flashdreams-${ownerRequestId}`;
  const matches = (await listVideoPods()).filter((pod) => text(pod?.name) === name && !podTerminal(pod));
  if (matches.length > 1) throw new Error(`AVANTIQO_VIDEO_FLASHDREAMS_POD_CREATE_AMBIGUOUS:${matches.length}`);
  if (!matches.length) return null;
  const existing = matches[0];
  const volumeId = text(existing?.networkVolume?.id ?? existing?.networkVolumeId ?? existing?.network_volume_id);
  if (volumeId && volumeId !== text(snapshot.volume?.id)) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_POD_VOLUME_MISMATCH");
  return existing;
}

function masterPodBody({ ownerRequestId, snapshot, prepared, gpuTypeIds, minRAMPerGPU }) {
  return {
...
async function createMasterPod({ ownerRequestId, snapshot, prepared, capacity }) {
  ...
  const gpuTypeIds = list(capacity?.selected_gpu_type_ids).filter((gpu) => AVANTIQO_VIDEO_FLASHDREAMS_GPU_POOL.includes(gpu));
  if (!gpuTypeIds.length) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_CAPACITY_SELECTION_REQUIRED");
  let created = null;
  let lastPlacementError = null;
  for (const minRAMPerGPU of PLACEMENT_RAM_GB) {
    try {
      created = await podRest("/pods", {
        method: "POST",
        timeoutMs: 45_000,
        body: masterPodBody({ ownerRequestId, snapshot, prepared, gpuTypeIds, minRAMPerGPU }),
      });
      break;
    } catch (error) {
      if (!placementUnavailable(error)) throw error;
      lastPlacementError = error;
      const existing = await findExistingMasterPod(ownerRequestId, snapshot);
      if (existing) {
        created = existing;
        break;
      }
  ...
  if (!created) {
    const detail = text(lastPlacementError?.message).split(":").slice(1).join(":") || "NO_INSTANCES";
    const exhausted = new Error(`AVANTIQO_VIDEO_FLASHDREAMS_PLACEMENT_EXHAUSTED:${gpuTypeIds.join("|")}:${detail}`);
  ...
  return { id, gpu_type_id: gpu || capacity.selected_primary_gpu_type_id || "PLACEMENT_PENDING" };
}
...
async function readFlashDreamsReceipt(receiptKey, volumeBridge) {
  try {
    ...
    if (receipt?.contract !== AVANTIQO_VIDEO_FLASHDREAMS_RECEIPT_CONTRACT) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_RECEIPT_CONTRACT_INVALID");
    if (receipt?.flashdreams_commit !== AVANTIQO_VIDEO_FLASHDREAMS_SOURCE_COMMIT) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_SOURCE_COMMIT_DRIFT");
    if (Number(receipt?.sparse_ratio) !== 1.5 || Number(receipt?.chunk_size) !== 8) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_PERFORMANCE_PROFILE_DRIFT");
    return receipt;
  } catch (error) {
...
async function retrieveAndFinalize(masterJob) {
  ...
  try {
    ...
    if (!receipt) {
      ...
      return { status: "failed", error: "AVANTIQO_VIDEO_FLASHDREAMS_POD_EXITED_WITHOUT_RECEIPT", runpod_lease_active: false, gpu_deleted_before_studio_encode: true, cpu_bridge_deleted: false };
    }
    if (!receipt.success) {
      await cleanupCreativeVideoFlashVsrObjects(masterJob.prepared, bridge).catch(() => null);
      return { status: "failed", error: receipt.error_code || "AVANTIQO_VIDEO_FLASHDREAMS_GPU_FAILED", receipt, runpod_lease_active: false, gpu_deleted_before_studio_encode: true, cpu_bridge_deleted: false };
    }
    const final = await finalizeCreativeVideoFlashVsrMaster({ organization_id: masterJob.organization_id, source_url: masterJob.source_url, prepared: masterJob.prepared, receipt, volume_bridge: bridge });
    await cleanupCreativeVideoFlashVsrObjects(masterJob.prepared, bridge).catch(() => null);
    return { status: "completed", final, receipt, runpod_lease_active: false, gpu_deleted_before_studio_encode: true, cpu_bridge_deleted: false };
  } catch (error) {
    if (bridge) await cleanupCreativeVideoFlashVsrObjects(masterJob.prepared, bridge).catch(() => null);
    return { status: "failed", error: text(error?.message).split(":")[0] || "AVANTIQO_VIDEO_FLASHDREAMS_STUDIO_FINALIZE_FAILED", studio_finalize_retryable: false, runpod_lease_active: false, gpu_deleted_before_studio_encode: true, cpu_bridge_deleted: false };
  } finally {
    if (bridge) await deleteAvantiqoVideoVolumeCpuBridge(bridge).catch(() => null);
  }
}
...
export async function submitAvantiqoVideoFlashDreamsMaster({ organizationId, sourceUrl } = {}) {
  ...
  if (!AVANTIQO_VIDEO_FLASHDREAMS_IMAGE || !AVANTIQO_VIDEO_FLASHDREAMS_IMAGE.includes("@sha256:")) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_IMMUTABLE_IMAGE_REQUIRED");
  const snapshot = await videoPodCandidateSnapshot();
  const capacity = await flashDreamsCapacity();
  ...
  try {
    ...
    prepared = await prepareCreativeVideoFlashVsrInput({ organization_id: organizationId, source_url: sourceUrl, owner_request_id: owner, volume_bridge: uploadBridge });
    ...
    if (uploadBridgeDelete.confirmed_terminal !== true) throw new Error("AVANTIQO_VIDEO_FLASHDREAMS_UPLOAD_BRIDGE_DELETE_NOT_CONFIRMED");
    uploadBridge = null;

    lease = await acquireVideoPodLease({ organizationId, ownerRequestId: owner });
    const pod = await createMasterPod({ ownerRequestId: owner, snapshot, prepared, capacity });
    podId = pod.id;
    return {
...
export async function getAvantiqoVideoFlashDreamsMasterStatus(masterJob = {}) {
  ...
  if (!pod && age < 120_000) {
    ...
    return { status: "processing", phase: "POD_CREATE_PROPAGATING", lease_expires_at: fresh.expires_at, runpod_lease_active: true, cpu_bridge_active: false };
  }
  if (!pod || podTerminal(pod)) {
    if (pod) await deleteVideoPod(podId).catch(() => null);
    await confirmAvantiqoVideoPodTerminal(podId);
    await releaseVideoPodLease({ leaseId, ownerRequestId: owner, state: "RELEASED", reason: "VIDEO_FLASHDREAMS_GPU_POD_TERMINAL_RETRIEVE_RECEIPT" }).catch(() => null);
  ...
  const fresh = await refreshVideoPodLease({ leaseId, ownerRequestId: owner });
  return { status: "processing", phase: "GPU_SUPER_RESOLUTION_FLASHDREAMS", lease_expires_at: fresh.expires_at, runpod_lease_active: true, cpu_bridge_active: false };
}
...
export async function abortAvantiqoVideoFlashDreamsMaster(masterJob = {}, reason = "VIDEO_FLASHDREAMS_ABORTED") {
  ...
  if (masterJob.lease_id && masterJob.owner_request_id) await releaseVideoPodLease({ leaseId: masterJob.lease_id, ownerRequestId: masterJob.owner_request_id, state: "FAILED", reason }).catch(() => null);
  if (masterJob.prepared && masterJob.owner_request_id) await cleanupViaFreshBridge(masterJob.prepared, text(masterJob.owner_request_id)).catch(() => null);
}
