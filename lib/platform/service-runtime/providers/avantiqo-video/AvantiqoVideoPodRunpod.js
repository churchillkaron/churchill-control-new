...
export async function runpodJson(url, options = {}, explicitKey = key()) {
  ...
  if (!response.ok) {
    const error = new Error(`AVANTIQO_VIDEO_POD_HTTP_${response.status}:${text(body?.message || body?.error || body?.detail || raw).replace(/\s+/g, " ").slice(0, 600) || "UNKNOWN"}`);
...
const status = (pod = {}) => text(pod.desiredStatus ?? pod.desired_status ?? pod.status).toUpperCase();
export const podTerminal = (pod = {}) => Boolean(status(pod) && TERMINAL.has(status(pod)));
function activeWorkers(endpoint = {}) {
  return list(endpoint.workers).filter((worker) => {
    const state = text(worker?.desiredStatus ?? worker?.desired_status ?? worker?.status ?? worker?.workerStatus ?? worker?.runtimeStatus).toUpperCase();
...
function volumeIds(endpoint = {}) {
  return [...new Set([
    ...
    ...list(endpoint.networkVolumeIds ?? endpoint.network_volume_ids).map((entry) => text(typeof entry === "string" ? entry : entry?.id ?? entry?.networkVolumeId)),
  ].filter(Boolean))];
}
...
export async function videoPodCapacity() {
  ...
  if (!response.ok || list(body.errors).length) throw new Error("AVANTIQO_VIDEO_POD_CAPACITY_QUERY_FAILED");
...
export async function videoPodCandidateSnapshot() {
  const [rawEndpoints, volumes, pods] = await Promise.all([
    podRest("/endpoints?includeTemplate=true&includeWorkers=true"),
    podRest("/networkvolumes"),
    podRest("/pods"),
  ]);
  const endpoints = Array.isArray(rawEndpoints) ? rawEndpoints : list(rawEndpoints?.endpoints || rawEndpoints?.data || rawEndpoints?.items);
  ...
  if (finite(candidate.workersMin, -1) !== 0 || finite(candidate.workersMax, -1) !== 0 || activeWorkers(candidate).length) {
    throw new Error("AVANTIQO_VIDEO_POD_CANDIDATE_MUST_REMAIN_0_0");
  }
  const ids = volumeIds(candidate);
  const matches = list(volumes).filter((volume) => ids.includes(text(volume?.id)) && text(volume?.name) === AVANTIQO_VIDEO_POD_CACHE_VOLUME);
  ...
  for (const peer of endpoints.filter((row) => volumeIds(row).includes(text(volume.id)))) {
  ...
  const activePods = list(pods).filter((pod) => text(pod?.networkVolume?.id ?? pod?.networkVolumeId ?? pod?.network_volume_id) === text(volume.id) && !podTerminal(pod));
...
export async function getVideoPod(id) {
  try { return await podRest(`/pods/${encodeURIComponent(id)}`, { timeoutMs: 20_000 }); }
  catch (error) { if (Number(error?.httpStatus) === 404) return null; throw error; }
}
...
export async function listVideoPods() {
  const pods = await podRest("/pods");
  return list(pods).filter((pod) => text(pod?.name).startsWith(AVANTIQO_VIDEO_POD_NAME_PREFIX));
}
