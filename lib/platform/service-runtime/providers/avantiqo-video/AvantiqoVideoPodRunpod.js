...
export const AVANTIQO_VIDEO_POD_GPU_POOL = Object.freeze([
...
export const AVANTIQO_VIDEO_POD_IMAGE = "ghcr.io/churchillkaron/avantiqo-video-worker-gpu-only@sha256:2f477f95fcc46fdcb7aff1dda03944ad282eb3a7d33c95098bd13d00a76c3425";
...
const PRODUCTION = "avantiqo-cinema-production-v1";
...
async function directTemplateRegistryAuthId(template = {}) {
  ...
  try {
    ...
    return templateRegistryAuthId(directTemplate);
  } catch {
    return "";
  }
}

async function verifyRegistryAuthId(registryAuthId) {
  if (!registryAuthId) return;
  ...
  const matches = registryAuths.filter((item) => text(item?.id) === registryAuthId);
  if (matches.length !== 1) throw new Error(`AVANTIQO_VIDEO_POD_REGISTRY_AUTH_PARITY_INVALID:${matches.length}`);
}

async function resolveRegistryAuthParity({ candidate, candidateTemplate, production, templates }) {
  const explicit = text(
    process.env.AVANTIQO_VIDEO_RUNPOD_REGISTRY_AUTH_ID ||
    process.env.AVANTIQO_VIDEO_32GB_CANDIDATE_RUNPOD_REGISTRY_AUTH_ID,
  );
  if (explicit) {
    await verifyRegistryAuthId(explicit);
    return { id: explicit, mode: "EXPLICIT_ENV" };
  }

  const candidateEmbedded = candidate?.template && typeof candidate.template === "object" ? candidate.template : null;
  const candidateId = templateRegistryAuthId(candidateEmbedded) || await directTemplateRegistryAuthId(candidateTemplate);
  if (candidateId) {
    await verifyRegistryAuthId(candidateId);
    return { id: candidateId, mode: "CANDIDATE_TEMPLATE" };
  }

  const productionTemplateId = endpointTemplateId(production);
  if (!productionTemplateId) throw new Error("AVANTIQO_VIDEO_POD_PRODUCTION_TEMPLATE_ID_REQUIRED");
  const productionTemplate =
    production?.template && typeof production.template === "object" && text(production.template?.id) === productionTemplateId
      ? production.template
      : templates.find((row) => text(row?.id) === productionTemplateId) || null;
  if (!productionTemplate) throw new Error("AVANTIQO_VIDEO_POD_PRODUCTION_TEMPLATE_REQUIRED");

  const productionId = await directTemplateRegistryAuthId(productionTemplate);
  if (productionId) {
    await verifyRegistryAuthId(productionId);
    return { id: productionId, mode: "PRODUCTION_VIDEO_TEMPLATE" };
  }

  return { id: null, mode: "PRODUCTION_VIDEO_TEMPLATE_NO_AUTH" };
}
...
export async function videoPodCapacity() {
  ...
  return {
    ...
    gpu_type_pool: [...AVANTIQO_VIDEO_POD_GPU_POOL],
...
export async function videoPodCandidateSnapshot() {
  ...
  const productions = endpoints.filter((row) => text(row?.name) === PRODUCTION);
  if (productions.length !== 1) throw new Error(`AVANTIQO_VIDEO_POD_PRODUCTION_AMBIGUOUS:${productions.length}`);
  ...
  if (!volumeIds(production).includes(text(volume.id))) throw new Error("AVANTIQO_VIDEO_POD_CACHE_NOT_SHARED_WITH_PRODUCTION");
  ...
  const registryAuth = await resolveRegistryAuthParity({ candidate, candidateTemplate: template, production, templates });
  ...
  return { candidate, production, volume, template, templateEnv: env, registryAuthId: registryAuth.id, registryAuthMode: registryAuth.mode };
}
...
function podCreateBody({ ownerRequestId, snapshot, env, command, minRAMPerGPU }) {
  return {
    ...
    env,
    gpuCount: 1,
    gpuTypeIds: [...AVANTIQO_VIDEO_POD_GPU_POOL],
    gpuTypePriority: "availability",
    imageName: AVANTIQO_VIDEO_POD_IMAGE,
    interruptible: false,
    minRAMPerGPU,
    minVCPUPerGPU: POD_MIN_VCPU_PER_GPU,
...
async function findExistingOwnerPod(ownerRequestId, snapshot) {
  ...
  if (matches.length > 1) throw new Error(`AVANTIQO_VIDEO_POD_CREATE_AMBIGUOUS_AFTER_RETRY:${matches.length}`);
  if (!matches.length) return null;
  const existing = matches[0];
  const volumeId = text(existing?.networkVolume?.id ?? existing?.networkVolumeId ?? existing?.network_volume_id);
  if (volumeId && volumeId !== text(snapshot.volume.id)) throw new Error("AVANTIQO_VIDEO_POD_CREATE_RETRY_VOLUME_MISMATCH");
  return existing;
}
...
export async function createVideoPod({ ownerRequestId, snapshot, env, command }) {
  ...
  for (const minRAMPerGPU of POD_PLACEMENT_RAM_GB) {
    try {
      created = await podRest("/pods", {
        method: "POST",
        timeoutMs: 45_000,
        body: podCreateBody({ ownerRequestId, snapshot, env, command, minRAMPerGPU }),
      });
      break;
    } catch (error) {
      ...
      let existing = null;
      try {
        existing = await findExistingOwnerPod(ownerRequestId, snapshot);
      } catch (inspectionError) {
        const retryError = new Error(`AVANTIQO_VIDEO_POD_PLACEMENT_RETRY_INSPECTION_FAILED:${text(inspectionError?.message).split(":")[0] || "UNKNOWN"}`);
        retryError.httpStatus = Number(error?.httpStatus) || null;
        throw retryError;
      }
      if (existing) {
        created = existing;
  ...
  if (!created) {
    const exhausted = new Error(`AVANTIQO_VIDEO_POD_PLACEMENT_EXHAUSTED:${POD_PLACEMENT_RAM_GB.join("_")}:${text(lastPlacementError?.message).split(":").slice(1).join(":") || "NO_INSTANCES"}`);
  ...
  if (selectedGpuTypeId && !AVANTIQO_VIDEO_POD_GPU_POOL.includes(selectedGpuTypeId)) {
    await deleteVideoPod(id).catch(() => null);
    throw new Error(`AVANTIQO_VIDEO_POD_CREATE_GPU_OUTSIDE_CERTIFIED_POOL:${selectedGpuTypeId}`);
  }
  return { ...verified, avantiqoSelectedGpuTypeId: selectedGpuTypeId || null };
}
