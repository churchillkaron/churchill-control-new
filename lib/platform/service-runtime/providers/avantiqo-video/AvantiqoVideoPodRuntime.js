...
import requests,runpod,handler_v5
...
started=time.time()
try:
 out=handler_v5.handler(job); receipt={"success":True,"contract":"AVANTIQO_VIDEO_EPHEMERAL_POD_RECEIPT_V1","status":"completed","output":out,"receipt_storage_reference":receipt_ref,"elapsed_seconds":round(time.time()-started,3)}
...
async function job(input, upload, org) {
  ...
  return { input: {
    ...
    usage_id: text(input.context?.usage_id), certification_execution: false, storage_upload: upload,
  }};
}
...
export async function inspectAvantiqoVideoPodReadiness() {
  ...
  try {
    ...
    return { ready, reason: ready ? "RTX_PRO_4500_EU_RO_1_MEDIUM_OR_HIGH" : "RTX_PRO_4500_EU_RO_1_NOT_MEDIUM_OR_HIGH", contract: AVANTIQO_VIDEO_POD_RUNTIME_CONTRACT, capacity, immutable_image: AVANTIQO_VIDEO_POD_IMAGE, network_volume_id: text(snapshot.volume.id), network_volume_name: AVANTIQO_VIDEO_POD_CACHE_VOLUME, gpu_type_id: AVANTIQO_VIDEO_POD_GPU, data_center_id: AVANTIQO_VIDEO_POD_DC, mutation_performed: false };
  } catch (error) {
    return { ready: false, reason: "POD_READINESS_CHECK_FAILED", error: text(error.message).split(":")[0], contract: AVANTIQO_VIDEO_POD_RUNTIME_CONTRACT, mutation_performed: false };
  }
}
export async function submitAvantiqoVideoPodGeneration(input = {}) {
  ...
  const owner = crypto.randomUUID(), fpath = foundationPath(org, usage), rpath = receiptPath(org, owner);
  await remove([fpath, rpath]);
  const [foundation, receiptUpload] = await Promise.all([signedUpload(fpath), signedUpload(rpath)]);
  const payload = await job(input, foundation, org);
  const lease = await acquireVideoPodLease({ organizationId: org, ownerRequestId: owner });
  let podId = null;
  try {
    const pod = await createVideoPod({ ownerRequestId: owner, snapshot, env: {
    ...
    podId = text(pod.id);
    return { contract: AVANTIQO_VIDEO_POD_RUNTIME_CONTRACT, pod_id: podId, lease_id: lease.id, lease_owner_request_id: owner, lease_endpoint_id: `${AVANTIQO_VIDEO_POD_LEASE_PREFIX}${owner}`, lease_expires_at: lease.expires_at, foundation_storage_reference: foundation.storage_reference, receipt_storage_reference: receiptUpload.storage_reference, submitted_at: new Date().toISOString(), hard_timeout_ms: HARD_TIMEOUT, startup_timeout_ms: STARTUP_TIMEOUT, immutable_image: AVANTIQO_VIDEO_POD_IMAGE, gpu_type_id: AVANTIQO_VIDEO_POD_GPU, data_center_id: AVANTIQO_VIDEO_POD_DC, prompt_persisted: false };
  } catch (error) {
    ...
    await remove([fpath, rpath]).catch(() => null);
    throw error;
  }
}
export async function getAvantiqoVideoPodGenerationStatus({ organizationId, podJob }) {
  ...
  if (saved) {
    ...
    await releaseVideoPodLease({ leaseId, ownerRequestId: owner, state: saved.success ? "RELEASED" : "FAILED", reason: saved.success ? "VIDEO_POD_RECEIPT_COMPLETED" : text(saved.error_code) || "VIDEO_POD_RECEIPT_FAILED" }).catch(() => null);
    if (!saved.success) return { status: "failed", error: text(saved.error_code) || "AVANTIQO_VIDEO_POD_GENERATION_FAILED" };
    const storage = text(saved?.output?.storage_reference || podJob.foundation_storage_reference);
    return { status: "completed", storage_reference: storage, video_url: await resolveCreativeProviderAssetUrl({ organization_id: organizationId, value: storage }), receipt: saved };
  }
...
export async function reconcileAvantiqoVideoPodLeases({ limit = 10 } = {}) {
  ...
  for (const lease of leases) {
    ...
    try {
      const saved = await receipt(text(lease.organization_id), owner).catch(() => null), age = Date.now() - Date.parse(text(lease.acquired_at));
      if (saved) { if (pod?.id) await deleteVideoPod(text(pod.id)).catch(() => null); await releaseVideoPodLease({ leaseId: lease.id, ownerRequestId: owner, state: saved.success ? "RELEASED" : "FAILED", reason: "VIDEO_POD_RECONCILED_RECEIPT" }); results.push({ lease_id: lease.id, action: "RECEIPT_FINALIZED" }); continue; }
      if (!pod && age < 120_000) { await refreshVideoPodLease({ leaseId: lease.id, ownerRequestId: owner }); results.push({ lease_id: lease.id, action: "CREATE_PROPAGATION_REFRESHED" }); continue; }
      const started = pod ? Date.parse(text(pod.lastStartedAt ?? pod.last_started_at)) : NaN;
      if (!pod || podTerminal(pod) || age >= HARD_TIMEOUT || (age >= STARTUP_TIMEOUT && !Number.isFinite(started))) {
        ...
        results.push({ lease_id: lease.id, action: "TERMINATED_RELEASED" }); continue;
      }
      await refreshVideoPodLease({ leaseId: lease.id, ownerRequestId: owner }); results.push({ lease_id: lease.id, action: "ACTIVE_REFRESHED" });
    } catch (error) { results.push({ lease_id: lease.id, action: "RECONCILE_ERROR", error: text(error.message).slice(0,180) }); }
  }
  return { success: results.every((row) => row.action !== "RECONCILE_ERROR"), contract: AVANTIQO_VIDEO_POD_RUNTIME_CONTRACT, results };
}
