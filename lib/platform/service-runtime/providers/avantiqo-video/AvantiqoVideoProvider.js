...
function runpodStatus(value) {
  ...
  if (["IN_QUEUE", "QUEUED", "PENDING"].includes(normalized)) {
    return "queued";
  }
  return "processing";
}
...
async function outputUploadTarget({ organizationId, usageId }) {
  ...
  if (!data?.signedUrl) {
    throw new Error("AVANTIQO_VIDEO_SIGNED_UPLOAD_URL_REQUIRED");
  }
...
async function signedCompletedOutput({ body, organizationId }) {
  ...
  const storageReference = text(
    output.storage_reference || output.storageReference,
  );
...
export const AvantiqoVideoProvider = {
  ...
  async execute(input = {}) {
    ...
    if (!normalized.jobId) {
      throw new Error("AVANTIQO_VIDEO_RUNPOD_JOB_ID_REQUIRED");
    }
  ...
  async getStatus(input = {}) {
    ...
    const completedOutput = await signedCompletedOutput({
      body: result,
      organizationId,
    });
    ...
    return {
      ...
      output: {
        ...
        ...(completedOutput
          ? {
              storage_reference: completedOutput.storageReference,
              video_url: completedOutput.signedUrl,
            }
          : {}),
      },
    };
