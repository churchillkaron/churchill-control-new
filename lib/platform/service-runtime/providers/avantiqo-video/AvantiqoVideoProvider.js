...
function engineConfiguration() {
  ...
  const certificationEndpointId = text(process.env.RUNPOD_AVANTIQO_VIDEO_ENDPOINT_ID);
  const endpointId = productionEndpointId || certificationEndpointId;
  const endpointRole = productionEndpointId ? "PRODUCTION" : "CERTIFICATION";
...
export const AvantiqoVideoProvider = {
  ...
  async execute(input = {}) {
    const { baseUrl, apiKey, endpointRole } = engineConfiguration();
    const organizationId = text(input.context?.organization_id);
