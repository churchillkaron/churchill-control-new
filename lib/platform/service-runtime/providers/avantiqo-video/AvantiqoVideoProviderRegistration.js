...
const engineUrl = text(process.env.AVANTIQO_VIDEO_ENGINE_URL);
const engineToken = text(process.env.AVANTIQO_VIDEO_ENGINE_TOKEN);
const engineEnabled = enabled(process.env.AVANTIQO_VIDEO_ENGINE_ENABLED);
const runtimeAvailable = Boolean(engineEnabled && engineUrl && engineToken);
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    runtime_configuration: {
      ...
      endpoint_configured: Boolean(engineUrl),
      token_configured: Boolean(engineToken),
    },
    video_capabilities: {
