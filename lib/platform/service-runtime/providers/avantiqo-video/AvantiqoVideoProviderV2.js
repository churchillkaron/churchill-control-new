...
const modalVideoWorker = createAvantiqoOwnedModalWorker({
  ...
  baseUrlEnv: "AVANTIQO_VIDEO_MODAL_BASE_URL",
  tokenEnv: "AVANTIQO_VIDEO_MODAL_GATEWAY_TOKEN",
...
export const AvantiqoVideoProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    if (ROUTED_MASTERED_CAPABILITIES.has(capability)) {
      return prefixJob(await modalVideoWorker.execute(advancedInput(input)), MODAL_VIDEO_JOB_PREFIX);
    }
  ...
  async getStatus(input = {}) {
    ...
    const rawModalJobId = stripPrefix(suppliedJobId, MODAL_VIDEO_JOB_PREFIX);
    if (rawModalJobId) {
      const result = await modalVideoWorker.getStatus({
        ...input,
        job_id: rawModalJobId,
        provider_job_id: rawModalJobId,
      });
      return {
        ...result,
        provider_job_id: `${MODAL_VIDEO_JOB_PREFIX}${rawModalJobId}`,
      };
    }
