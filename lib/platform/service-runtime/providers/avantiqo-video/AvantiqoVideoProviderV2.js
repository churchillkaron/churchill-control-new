...
const LOCAL_ONLY_CAPABILITIES = new Set([
  "ai.video.upscale",
]);
...
export const AvantiqoVideoProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    if (!ROUTED_MASTERED_CAPABILITIES.has(capability)) {
      throw new Error(`AVANTIQO_VIDEO_CAPABILITY_NOT_IMPLEMENTED:${capability || "UNKNOWN"}`);
    }

    // Every Modal execution is gated again by AvantiqoOwnedModalWorker through
    // AVANTIQO_MODAL_COMPUTE_OWNER_APPROVAL_V1. No implicit fallback is allowed.
  ...
  async getStatus(input = {}) {
    ...
    throw new Error("AVANTIQO_VIDEO_JOB_TRANSPORT_UNSUPPORTED");
  },
};
