...
import { AvantiqoVideoFlashVsrProvider } from "./AvantiqoVideoFlashVsrProvider.js";
...
function fastPreviewRequested(input = {}) {
  ...
  if (["NATIVE_MASTER", "NATIVE_4K_MASTER", "FINAL_NATIVE_MASTER"].includes(profile)) {
    throw new Error("AVANTIQO_VIDEO_NATIVE_MASTER_ROUTE_DISABLED");
  }
  return true;
}
...
export const AvantiqoVideoProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    if (capability === "ai.video.generate" && fastPreviewRequested(input)) {
      return fastPreviewVideoWorker.execute(fastPreviewInput(input));
    }
    if (["ai.video.image_to_video", "ai.video.first_last_frame_to_video"].includes(capability)) {
      return modalVideoWorker.execute(advancedInput(input));
    }
    if (capability === "ai.video.upscale") {
      return AvantiqoVideoFlashVsrProvider.execute(input);
    }
