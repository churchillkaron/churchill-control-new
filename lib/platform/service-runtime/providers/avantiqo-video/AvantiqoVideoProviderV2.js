...
import {
  AvantiqoVideoWorkflowRuntimeV5,
  AVANTIQO_VIDEO_WORKFLOW_V5_JOB_PREFIX,
} from "./AvantiqoVideoWorkflowRuntimeV5.js";
...
function advancedInput(input = {}) {
  ...
  const requestedResolution = text(
    input.resolution || generation.resolution || providerParameters.resolution,
  );
  if (requestedResolution.toLowerCase() === "720p") {
    throw new Error("AVANTIQO_VIDEO_720P_FORBIDDEN");
  }
  return {
    ...input,
    generation: {
      ...
      resolution: requestedResolution || "native-4k",
...
function workflowV5Job(value) {
  return text(value).startsWith(AVANTIQO_VIDEO_WORKFLOW_V5_JOB_PREFIX);
}
...
export const AvantiqoVideoProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    if (ROUTED_MASTERED_CAPABILITIES.has(capability)) {
      return AvantiqoVideoWorkflowRuntimeV5.execute(input);
    }
    if (!ADVANCED_CAPABILITIES.has(capability)) {
      throw new Error(`AVANTIQO_VIDEO_CAPABILITY_REQUIRES_LTX25_MIGRATION:${capability || "MISSING"}`);
    }
  ...
  async getStatus(input = {}) {
    ...
    if (workflowV5Job(suppliedJobId)) return AvantiqoVideoWorkflowRuntimeV5.getStatus(input);

    // Compatibility only: status for jobs created before V5 remains readable.
    // New generation execution above never enters V4/V2/legacy managed fallback paths.
    ...
    // Legacy direct provider status remains reachable only for a pre-existing job ID.
