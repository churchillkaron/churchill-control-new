...
function queuedResult(state) {
  return {
    ...
    output: {
      ...
      status: state.stage === "COMPLETED" ? "completed" : state.stage === "FAILED" || state.stage === "STUDIO_MASTERING_FAILED" ? "failed" : "queued",
      stage: state.stage,
...
export const AvantiqoVideoWorkflowRuntimeV3 = {
  async execute(input = {}) {
    ...
    const state = {
      ...
      master_model: AVANTIQO_VIDEO_STUDIO_MASTER_MODEL,
      master_contract: AVANTIQO_VIDEO_STUDIO_MASTER_CONTRACT,
  ...
  async getStatus(input = {}) {
    ...
    if (state.stage === "FAILED" || state.stage === "STUDIO_MASTERING_FAILED") {
      return {
        provider: "avantiqo-video",
