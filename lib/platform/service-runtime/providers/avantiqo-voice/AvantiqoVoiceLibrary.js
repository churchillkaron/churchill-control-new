...
const VOICE_USE_SCOPES = new Set(["SPEECH", "SINGING"]);
...
function normalizeUseScopes(value, fallback = ["SPEECH"]) {
  const values = Array.isArray(value) ? value : (value ? [value] : fallback);
  const scopes = [...new Set(values.map((item) => text(item).toUpperCase()).filter(Boolean))];
  if (!scopes.length || scopes.some((scope) => !VOICE_USE_SCOPES.has(scope))) {
    throw new Error("AVANTIQO_VOICE_LIBRARY_USE_SCOPE_INVALID");
  }
  return scopes;
}
...
function publicProfile(profile = {}) {
  return {
    ...
    use_scopes: normalizeUseScopes(profile.use_scopes),
  };
}
...
export async function createRecordedVoiceProfile({
  ...
  const use_scopes = normalizeUseScopes(useScopes);
...
export async function resolveVoiceReferenceForExecution({
  ...
  const requestedPurpose = text(purpose).toUpperCase() || "SPEECH";
  const scopes = normalizeUseScopes(profile.use_scopes);
  if (!scopes.includes(requestedPurpose)) throw new Error(`AVANTIQO_VOICE_LIBRARY_USE_SCOPE_REQUIRED:${requestedPurpose}`);
  ...
  const executionTtlSeconds = 1800;
  const { data: executionUrl, error: executionUrlError } = await supabaseAdmin.storage
    .from(profile.storage_bucket || AVANTIQO_VOICE_LIBRARY_BUCKET)
    .createSignedUrl(profile.storage_path, executionTtlSeconds);
  if (executionUrlError || !executionUrl?.signedUrl) throw new Error("AVANTIQO_VOICE_LIBRARY_EXECUTION_URL_REQUIRED");
  ...
  return {
    ...
    voice_reference: {
      ...
      execution_url: executionUrl.signedUrl,
      execution_url_expires_in_seconds: executionTtlSeconds,
      checksum_sha256: checksum,
