...
async function classMethod(client, cfg, functionName) {
  const className = CLASS_BY_FUNCTION[functionName];
  if (!className) throw new Error(`AVANTIQO_VOICE_MODAL_DIRECT_CLASS_INVALID:${functionName}`);
  const lookupOptions = cfg.environment ? { environment: cfg.environment } : {};
  const cls = await client.cls.fromName(APP_NAME, className, lookupOptions);
  const instance = await cls.instance();
  const method = instance.method(functionName);
...
async function persistTtsOutput(output, { organizationId, usageId }) {
  ...
  if (!billingQuantitySeconds) {
    throw new Error("AVANTIQO_VOICE_MODAL_DIRECT_TTS_DURATION_REQUIRED_FOR_SETTLEMENT");
  }
...
export async function executeVoiceModalDirect({ capability, payload, productModel, foundationModel, ttsVoiceSelection = null } = {}) {
  ...
  const { className, method } = await classMethod(client, cfg, functionName);
...
export async function getVoiceModalDirectStatus(input = {}) {
  ...
  try {
    ...
    return clean({
      ...
      modal_transport: DIRECT_TRANSPORT,
      modal_app: APP_NAME,
      modal_class: CLASS_BY_FUNCTION[parsed.functionName],
