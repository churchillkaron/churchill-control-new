...
async function resolveTtsVoiceSelection(input, context) {
  ...
  const requestedLibraryProfileId = text(
    input.voice_library_profile_id ||
    input.voiceLibraryProfileId ||
    input.voice_identity_profile_id ||
    input.voiceIdentityProfileId,
  ) || null;
  ...
  return {
    voiceProfile:
      ...
      librarySelection?.voice_profile ||
      "avantiqo-secretary-v1",
    voiceReference: librarySelection?.voice_reference || null,
    identitySource: librarySelection
      ? "organization_voice_library"
      : "avantiqo_builtin",
...
async function requireSafeLeaseForSubmission(endpointId, capability, input = {}) {
  ...
  if (distributedLease) {
    if (text(distributedLease.contract) !== RUNPOD_SAFE_LEASE_CONTRACT) {
      throw new Error("AVANTIQO_VOICE_RUNPOD_SAFE_LEASE_V2_REQUIRED");
    }
    if (text(distributedLease.lane) !== expectedLane) {
      throw new Error(`AVANTIQO_VOICE_RUNPOD_SAFE_LEASE_LANE_MISMATCH:expected=${expectedLane}`);
    }
    if (text(distributedLease.endpoint_id) !== endpointId) {
    ...
    return {
      mode: "DISTRIBUTED_DATABASE",
      contract: RUNPOD_SAFE_LEASE_CONTRACT,
      lane: expectedLane,
    };
  }
  ...
  if (text(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_CONTRACT) !== RUNPOD_SAFE_LEASE_CONTRACT) {
    throw new Error("AVANTIQO_VOICE_RUNPOD_SAFE_LEASE_V2_REQUIRED");
  }

  const leasedLane = text(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_LANE);
  if (leasedLane !== expectedLane) {
    throw new Error(`AVANTIQO_VOICE_RUNPOD_SAFE_LEASE_LANE_MISMATCH:expected=${expectedLane}`);
  }
  ...
  if (!Number.isFinite(expiresAt)) {
    throw new Error("AVANTIQO_VOICE_RUNPOD_SAFE_LEASE_EXPIRY_REQUIRED");
  }
  if (expiresAt - Date.now() < RUNPOD_SAFE_LEASE_MIN_REMAINING_MS) {
    throw new Error("AVANTIQO_VOICE_RUNPOD_SAFE_LEASE_EXPIRED_OR_EXPIRING");
  }
  return {
    mode: "LOCAL_CONTROLLER_ENV",
    contract: RUNPOD_SAFE_LEASE_CONTRACT,
    lane: expectedLane,
  };
}
...
async function submitJob({ endpointId, capability, payload, input }) {
  const safeLease = await requireSafeLeaseForSubmission(
    endpointId,
    capability,
    input,
  );
  ...
  while (true) {
    ...
    const result = await runpodRequest(endpointId, "/run", {
      method: "POST",
      body: { input: payload },
    });
    ...
    /*
     * RunPod has separate management and queue control planes.
     * After workersMax changes from 0 -> 1, management can report
     * the endpoint open before the queue gateway stops returning 409.
     *
     * Retry ONLY a definite HTTP 409 and ONLY when RunPod has not
     * returned a job id. Network errors and all other HTTP failures
     * remain fail-closed so an accepted job can never be duplicated.
     */
    if (
      response.status === 409 &&
      !responseJobId &&
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, gatewayRetryMs)
      );
      continue;
    }
    ...
    throw new Error(
      `AVANTIQO_VOICE_RUNPOD_REQUEST_FAILED:${response.status}:${detail}`,
    );
  }
  ...
  if (!jobId) {
    throw new Error(
      "AVANTIQO_VOICE_RUNPOD_JOB_ID_REQUIRED"
    );
  }
  ...
  return {
    ...
    immediateOutput,
    safeLease,
    gatewaySubmissionAttempts: attempts,
  };
}
...
export const AvantiqoVoiceProvider = {
  ...
  async execute(input = {}) {
    ...
    if (capability === "ai.speech.to.text") {
      ...
      ttsVoiceSelection = await resolveTtsVoiceSelection(input, context);
      workload = {
