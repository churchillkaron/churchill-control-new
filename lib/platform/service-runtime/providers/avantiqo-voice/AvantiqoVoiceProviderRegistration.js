...
const sttFoundationModel =
  text(process.env.AVANTIQO_VOICE_STT_FOUNDATION_MODEL) ||
  DEFAULT_STT_FOUNDATION_MODEL;
const ttsFoundationModel =
  text(process.env.AVANTIQO_VOICE_TTS_FOUNDATION_MODEL) ||
  DEFAULT_TTS_FOUNDATION_MODEL;
const requested = requestedCapabilities(process.env.AVANTIQO_VOICE_CERTIFIED_CAPABILITIES);

// The provider runtime resolves canonical RunPod endpoint IDs by name when an
// explicit endpoint env var is absent. Registration must not reject the provider
// merely because legacy endpoint/model env bindings are missing.
...
const runtimeAvailable = Boolean(
  engineEnabled && apiKey && managementKey && capabilities.length,
);
...
PROVIDER_REGISTRY[PROVIDER_ID] = {
  ...
  metadata: {
    ...
    configured_foundation_models: {
      stt: sttFoundationModel,
      tts: ttsFoundationModel,
    },
    runtime_configuration: {
      ...
      stt_endpoint_discovery_enabled: true,
      tts_endpoint_discovery_enabled: true,
      ...
      scale_to_zero: true,
      runpod_submission_requires_safe_lease: true,
    },
  },
};
