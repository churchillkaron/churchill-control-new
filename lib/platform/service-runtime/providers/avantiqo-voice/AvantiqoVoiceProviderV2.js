  ...
  async execute(input = {}) {
    ...
    if (capability === "ai.speech.to.text") {
      try {
        ...
        console.error("AVANTIQO_VOICE_STT_LOCAL_FAILED", { error: text(error?.message || error).slice(0, 500) });
        throw error;
      }
    }
    if (capability === "ai.text.to.speech") {
      try {
        ...
        console.error("AVANTIQO_VOICE_TTS_LOCAL_FAILED", { error: text(error?.message || error).slice(0, 500) });
        throw error;
      }
    }
    throw new Error(`AVANTIQO_VOICE_LOCAL_NODE_UNAVAILABLE:${capability}`);
  },

  async cancel(input = {}) {
    const jobId = text(input.job_id || input.jobId || input.provider_job_id);
    if (!jobId) throw new Error("AVANTIQO_VOICE_JOB_ID_REQUIRED");
    if (isVoiceSttLocalJob(jobId)) return AvantiqoVoiceSttLocalQueueProvider.cancel(input);
    if (isVoiceTtsLocalJob(jobId)) return AvantiqoVoiceTtsLocalQueueProvider.cancel(input);
