...
export const AvantiqoVoiceProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    const backgroundTts = capability === "ai.text.to.speech" && text(input.execution_mode || input.executionMode).toLowerCase() === "background";
    if (backgroundTts) {
      try {
        ...
        if (text(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED).toLowerCase() === "true") throw error;
        console.error("AVANTIQO_VOICE_TTS_LOCAL_BATCH_FALLBACK_MODAL", { error: text(error?.message || error).slice(0, 500) });
      }
    }
