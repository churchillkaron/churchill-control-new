...
import { AvantiqoVoiceSttLocalQueueProvider, isVoiceSttLocalJob } from "./AvantiqoVoiceSttLocalQueueProvider.js";
import { AvantiqoVoiceTtsLocalQueueProvider, isVoiceTtsLocalJob } from "./AvantiqoVoiceTtsLocalQueueProvider.js";
import {
...
export const AvantiqoVoiceProviderV2 = {
  ...
  async execute(input = {}) {
    ...
    const payload = enginePayload({ endpoint, capability, context, workload: prepared.workload });
    if (capability === "ai.speech.to.text") {
      try {
        if (await AvantiqoVoiceSttLocalQueueProvider.available()) {
          return await AvantiqoVoiceSttLocalQueueProvider.execute({ context, payload, productModel: endpoint.productModel });
        }
      } catch (error) {
        if (text(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED).toLowerCase() === "true") throw error;
        console.error("AVANTIQO_VOICE_STT_LOCAL_FALLBACK_MODAL", { error: text(error?.message || error).slice(0, 500) });
      }
    }
    if (capability === "ai.text.to.speech") {
      try {
        if (await AvantiqoVoiceTtsLocalQueueProvider.available()) {
          return await AvantiqoVoiceTtsLocalQueueProvider.execute({ context, payload, productModel: endpoint.productModel, ttsVoiceSelection: prepared.ttsVoiceSelection });
        }
      } catch (error) {
        if (text(process.env.AVANTIQO_LOCAL_COMPUTE_REQUIRED).toLowerCase() === "true") throw error;
        console.error("AVANTIQO_VOICE_TTS_LOCAL_FALLBACK_MODAL", { error: text(error?.message || error).slice(0, 500) });
      }
    }
    if (!voiceModalDirectConfigured()) {
      throw new Error("AVANTIQO_VOICE_MODAL_DIRECT_CONFIGURATION_REQUIRED");
    }
    await consumeModalComputeApproval({
      input,
      capability,
      infrastructureProvider: "MODAL_H100_ASYNC_V1",
    });
  ...
  async getStatus(input = {}) {
    ...
    if (isVoiceSttLocalJob(jobId)) return AvantiqoVoiceSttLocalQueueProvider.getStatus(input);
    if (isVoiceTtsLocalJob(jobId)) return AvantiqoVoiceTtsLocalQueueProvider.getStatus(input);
    if (isVoiceModalDirectJob(jobId)) return getVoiceModalDirectStatus(input);
    throw new Error("AVANTIQO_VOICE_LEGACY_JOB_TRANSPORT_RETIRED");
  },
};
