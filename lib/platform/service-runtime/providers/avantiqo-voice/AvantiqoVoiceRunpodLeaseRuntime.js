...
const OPEN_PROPAGATION_SETTLE_MS = 15000;
...
async function resolveTarget(lane) {
  ...
  const canonicalMatches = snapshot.endpoints.filter(
    (endpoint) => text(endpoint?.name) === policy.endpointName,
  );
  if (canonicalMatches.length !== 1) {
    throw new Error(
      `AVANTIQO_VOICE_RUNPOD_LEASE_TARGET_RESOLUTION_FAILED:${policy.lane}:matches=${canonicalMatches.length}`,
    );
  }
  const endpoint = canonicalMatches[0];
  const endpointId = text(endpoint?.id);
  if (!endpointId) {
    throw new Error(`AVANTIQO_VOICE_RUNPOD_LEASE_TARGET_ID_MISSING:${policy.lane}`);
  }
  if (configuredId && configuredId !== endpointId) {
    const configuredEndpoint = snapshot.endpoints.find(
      (candidate) => text(candidate?.id) === configuredId,
    );
    const configuredName = text(configuredEndpoint?.name);
    if (configuredName === policy.endpointName) {
      throw new Error(
        `AVANTIQO_VOICE_RUNPOD_LEASE_TARGET_RESOLUTION_FAILED:${policy.lane}:duplicate_canonical_name`,
      );
    }
  }
...
async function patchScaling(endpointId, workersMax, idleTimeout) {
  const desiredMax = Math.max(0, Number(workersMax) || 0);
  const desiredIdleTimeout = Math.max(1, Number(idleTimeout) || 5);

  await rest(`/endpoints/${encodeURIComponent(endpointId)}/update`, {
    method: "POST",
    body: {
      workersMin: 0,
      workersMax: desiredMax,
      idleTimeout: desiredIdleTimeout,
    },
  });

  const readbackDeadline = Date.now() + 60000;
  let verified = null;

  while (Date.now() < readbackDeadline) {
    verified = await rest(`/endpoints/${encodeURIComponent(endpointId)}`);

    const verifiedMin = finite(verified?.workersMin, null);
    const verifiedMax = finite(verified?.workersMax, null);
    const verifiedIdleTimeout = finite(verified?.idleTimeout, null);

    if (
      verifiedMin === 0 &&
      verifiedMax === desiredMax &&
      verifiedIdleTimeout === desiredIdleTimeout
    ) {
      return verified;
    }
  ...
  throw new Error(
    `AVANTIQO_VOICE_RUNPOD_LEASE_SCALING_READBACK_TIMEOUT:${endpointId}:expected_max=${desiredMax}:actual_min=${finite(
      verified?.workersMin,
      null,
    )}:actual_max=${finite(verified?.workersMax, null)}`,
  );
}
