...
async function onlineNodeAvailable() {
  ...
  if (!enabled(process.env.AVANTIQO_LOCAL_VOICE_STT_ENABLED)) return false;
...
export const AvantiqoVoiceSttLocalQueueProvider = {
  ...
  async execute({ context = {}, payload = {}, productModel = "avantiqo-voice-stt-v1" } = {}) {
    ...
    if (inserted.error || !inserted.data?.id) throw inserted.error || new Error("AVANTIQO_VOICE_STT_LOCAL_QUEUE_INSERT_FAILED");
    return { success: true, provider: "avantiqo-voice", model: productModel, output: {
      provider_job_id: `${AVANTIQO_VOICE_STT_LOCAL_JOB_PREFIX}${inserted.data.id}`, status: "queued",
