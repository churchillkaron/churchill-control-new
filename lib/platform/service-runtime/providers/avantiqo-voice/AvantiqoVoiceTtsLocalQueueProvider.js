...
export const AvantiqoVoiceTtsLocalQueueProvider = {
  ...
  async execute({ context = {}, payload = {}, productModel = "avantiqo-voice-tts-v2", ttsVoiceSelection = null } = {}) {
    ...
    const inserted = await supabaseAdmin.from("avantiqo_local_compute_jobs").insert({ organization_id: organizationId, usage_id: usageId, capability: CAPABILITY, lane: "gpu", workload: "voice_tts", model: MODEL, payload: queuePayload, priority: 35, max_attempts: 2 }).select("id").single();
    if (inserted.error || !inserted.data?.id) throw inserted.error || new Error("AVANTIQO_VOICE_TTS_LOCAL_QUEUE_INSERT_FAILED");
    return { success: true, provider: "avantiqo-voice", model: productModel, output: { provider_job_id: `${AVANTIQO_VOICE_TTS_LOCAL_JOB_PREFIX}${inserted.data.id}`, status: "queued", capability: CAPABILITY, engine_contract: "AVANTIQO_VOICE_ENGINE_V1", foundation_model: MODEL, storage_reference: storageUpload.storage_reference, voice_reference_contract: ttsVoiceSelection?.voiceReference?.contract || null, recorded_reference_voice_requested: Boolean(ttsVoiceSelection?.voiceReference), voice_identity_source: ttsVoiceSelection?.identitySource || null, voice_identity_profile_id: ttsVoiceSelection?.identityProfileId || null, voice_delivery_profile: ttsVoiceSelection?.voiceProfile || null, infrastructure_provider: INFRASTRUCTURE, local_node: true, local_tts_mode: "BATCH_BACKGROUND_ONLY", raw_reasoning_persisted: false } };
  },
