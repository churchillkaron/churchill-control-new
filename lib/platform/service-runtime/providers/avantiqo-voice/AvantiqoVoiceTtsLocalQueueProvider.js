...
export const AvantiqoVoiceTtsLocalQueueProvider = {
  ...
  async cancel(input = {}) {
    ...
    if (!organizationId) throw new Error("AVANTIQO_VOICE_TTS_LOCAL_CANCEL_SCOPE_REQUIRED");
    const result = await supabaseAdmin.from("avantiqo_local_compute_jobs")
      ...
      .eq("id", rawJobId(jobId)).eq("organization_id", organizationId).in("status", ["QUEUED", "RUNNING"])
      .select("id,status,node_id").maybeSingle();
    if (result.error) throw result.error;
    return { success: true, cancelled: Boolean(result.data), provider_job_id: jobId, node_id: result.data?.node_id || null, exact_job_only: true };
  },
