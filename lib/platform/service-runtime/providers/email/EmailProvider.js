...
async function mimeMessage({ from, to, subject, body, attachments = [], unsubscribeUrl = null }) {
...
async function sendGoogle(input) {
  ...
  const raw = await mimeMessage({
    ...
    unsubscribeUrl: text(input.list_unsubscribe_url) || null,
  });
  const response = await fetch(
...
async function sendMicrosoft(input) {
  ...
  const message = {
    ...
    ...(text(input.list_unsubscribe_url) ? {
      internetMessageHeaders: [
        { name: "List-Unsubscribe", value: `<${text(input.list_unsubscribe_url)}>` },
