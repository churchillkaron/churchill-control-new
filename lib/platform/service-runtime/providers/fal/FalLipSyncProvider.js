...
function queueRouteCandidates(requestId) {
  const encoded = encodeURIComponent(requestId);
  return [
    `${QUEUE_BASE}/${MODEL}/requests/${encoded}`,
    `${QUEUE_BASE}/fal-ai/sync-lipsync/v2/requests/${encoded}`,
    `${QUEUE_BASE}/fal-ai/sync-lipsync/requests/${encoded}`,
  ];
}

async function requestQueueWithFallback({ requestId, apiKey, suffix = "" }) {
  let lastFailure = null;
  for (const base of queueRouteCandidates(requestId)) {
...
export const FalLipSyncProvider = {
  ...
  async submit({ video_url, audio_url, sync_mode = "cut_off", credential_id = null } = {}) {
    ...
    const result = await requestJson({
      ...
      apiKey,
      method: "POST",
      body: { video_url, audio_url, sync_mode },
    });
    ...
    return {
      ...
      output_url: output,
      status_url: text(result.status_url) || (requestId ? `${QUEUE_BASE}/${MODEL}/requests/${encodeURIComponent(requestId)}/status` : null),
      response_url: text(result.response_url) || (requestId ? `${QUEUE_BASE}/${MODEL}/requests/${encodeURIComponent(requestId)}` : null),
      raw: result,
    };
  },
  ...
  async poll({ request_id, credential_id = null } = {}) {
    ...
    const { apiKey } = await resolveCredential(credential_id);
    const statusAttempt = await requestQueueWithFallback({ requestId, apiKey, suffix: "/status" });
    ...
    if (sameFamily.response.ok) {
      ...
      resultAttempt = await requestQueueWithFallback({ requestId, apiKey });
    } else {
