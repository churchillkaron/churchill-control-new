...
PROVIDER_REGISTRY[FAL_PROVIDER_ID] = {
  ...
  capabilities: [
    ...new Set([
      ...(existingFal.capabilities || []).filter(
        (capability) => capability !== "ai.music.generate",
      ),
      "ai.sfx.generate",
    ]),
  ],
  ...
  metadata: {
    ...
    supported_models: [
      ...new Set([
        ...(existingFal.metadata?.supported_models || []).filter(
          (model) => model !== "fal-ai/ace-step/prompt-to-audio",
        ),
