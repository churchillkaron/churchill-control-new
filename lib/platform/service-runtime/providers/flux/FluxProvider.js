...
function firstUrl(value) {
  ...
  return value.file_url || value.fileUrl || value.image_url || value.imageUrl || value.url || null;
}
...
export const FluxProvider = {
  ...
  async execute(input = {}) {
    ...
    const sourceImageUrl = firstUrl(
      input.source || input.image || input.image_url || input.imageUrl || selectedAssets(input),
    );
