...
const INTERACTION_ID_PATTERN = /^v1_[A-Za-z0-9_-]{10,512}$/;
...
function previousInteractionId(input = {}) {
  ...
  const id = text(
    ...
    providerParameters.previous_interaction_id ||
    providerParameters.previousInteractionId ||
    generationParameters.previous_interaction_id ||
    generationParameters.previousInteractionId,
  );
  if (!id) return null;
  if (!INTERACTION_ID_PATTERN.test(id)) {
    throw new Error("GEMINI_PREVIOUS_INTERACTION_ID_INVALID");
  }
  return id;
}
...
async function executeStatefulVideoEdit(input = {}, previousInteraction) {
  ...
  if (model !== DEFAULT_MODEL) {
    throw new Error(`GEMINI_STATEFUL_EDIT_MODEL_NOT_ALLOWED:${model}`);
  }
  ...
  const request = {
    model,
    previous_interaction_id: previousInteraction,
    input: serialized,
  };
  ...
  const provenance = {
    previous_interaction_id: previousInteraction,
    stateful_edit: true,
    visual_input_mode: "STATEFUL_PREVIOUS_INTERACTION",
    source_binding_mode: "STATEFUL_VIDEO_EDIT",
  };
...
export const GeminiApprovedDependencyFrameProvider = Object.freeze({
  ...
  async execute(input = {}) {
    const previousInteraction = previousInteractionId(input);
    if (previousInteraction) {
      return executeStatefulVideoEdit(input, previousInteraction);
    }
