...
function videoDurationSeconds(input = {}) {
  ...
  if (!Number.isInteger(value) || value < 3 || value > 10) {
    throw new Error(`GEMINI_VIDEO_DURATION_UNSUPPORTED:${Number.isFinite(value) ? value : "missing"}`);
  }
  return value;
}
...
export const GeminiProvider = {
  ...
  async execute(input = {}) {
    ...
    const duration = videoDurationSeconds(input);
