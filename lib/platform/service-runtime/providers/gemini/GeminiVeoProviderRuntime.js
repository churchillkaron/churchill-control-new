...
const CONTRACT = "GEMINI_VEO_3_1_PRECISION_EXECUTION_V5";
...
async function buildRequest(input = {}) {
  ...
  const instance = {
    ...
    ...(first ? { image: { inlineData: first.inline_data } } : {}),
    ...(last ? { lastFrame: { inlineData: last.inline_data } } : {}),
    ...(references.length
      ? {
          referenceImages: references.map((reference) => ({
            image: { inlineData: reference.inline_data },
            referenceType: "asset",
          })),
        }
      : {}),
    ...(extensionVideo ? { video: { inlineData: extensionVideo.inline_data } } : {}),
  };
  ...
  return {
    ...
    evidence: {
      contract: CONTRACT,
      media_serialization: "GEMINI_INLINE_DATA",
      request_profile: referenceOnly
        ? "OFFICIAL_GEMINI_VEO_REFERENCE_IMAGE_REST"
        : "GEMINI_VEO_PRECISION",
