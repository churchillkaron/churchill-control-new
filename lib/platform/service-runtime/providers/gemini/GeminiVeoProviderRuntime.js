...
import {
  uploadCreativeAsset,
} from "../../../../creative/assets/storage/uploadCreativeAsset.js";

const API_BASE = "https://generativelanguage.googleapis.com/v1beta";
const PROVIDER_ID = "google-veo";
const MODEL = "veo-3.1-generate-preview";
const CONTRACT = "GEMINI_VEO_3_1_PRECISION_EXECUTION_V6";
...
async function buildRequest(input = {}) {
  ...
  const instance = {
    ...
    ...(first ? { image: first.media } : {}),
    ...(last ? { lastFrame: last.media } : {}),
    ...(references.length
      ? {
      ...
      : {}),
    ...(extensionVideo ? { video: extensionVideo.media } : {}),
  };

  const referenceOnly =
    references.length > 0 && !first && !last && !extensionVideo;

  const parameters = referenceOnly
    ? {}
    : {
        durationSeconds: durationSeconds(input),
        aspectRatio: aspectRatio(input),
        resolution: requestedResolution,
        // Google rejects personGeneration for video-extension requests. Keep the
        // parameter on fresh generation, but omit it when an existing Veo clip is
        // the motion/continuity authority.
        ...(wantsExtension ? {} : { personGeneration: "allow_adult" }),
      };
  ...
  return {
    ...
    evidence: {
      contract: CONTRACT,
      media_serialization: "BYTES_BASE64_ENCODED",
      request_profile: referenceOnly
        ? "VEO_REFERENCE_IMAGE_REST_LIVE"
        : "VEO_IMAGE_TO_VIDEO_REST_LIVE",
...
function filteredCompletion(result = {}) {
  const response = object(result?.response?.generateVideoResponse);
  const reasons = list(response.raiMediaFilteredReasons)
    .map(text)
    .filter(Boolean);
  const countValue = Number(response.raiMediaFilteredCount || 0);
  return {
...
export const GeminiVeoProviderRuntime = {
  ...
  async getStatus(input = {}) {
    ...
    if (!uri) {
      const filtered = filteredCompletion(result);
      return {
        success: false,
        failed: true,
        pending: false,
        ...
        model: selectedModel,
        provider_job_id: providerJobId,
        provider_status: filtered.count > 0 ? "filtered" : "failed",
        error:
          filtered.reasons[0] ||
          (filtered.count > 0
            ? "GEMINI_VEO_RAI_FILTERED"
            : "GEMINI_VEO_COMPLETED_VIDEO_URI_REQUIRED"),
        rai_media_filtered_count: filtered.count,
        rai_media_filtered_reasons: filtered.reasons,
      };
    }
