...
export async function getManagedGoogleAdsBillingStatus({
  ...
  return {
    ready:
      Boolean(billing) &&
      attachedAccountId === account.payments_account_id &&
      ["PENDING", "APPROVED_HELD", "APPROVED"].includes(status),
    advertiser,
    billing,
    ...
    blocker: billing
      ? attachedAccountId === account.payments_account_id
        ? null
        : "GOOGLE_ADS_DIFFERENT_PAYMENTS_ACCOUNT_ATTACHED"
      : "GOOGLE_ADS_BILLING_SETUP_REQUIRED",
  };
}
