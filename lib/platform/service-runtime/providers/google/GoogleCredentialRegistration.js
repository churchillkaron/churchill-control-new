...
} from "../ProviderCredentialRuntime.js";
import {
  listActiveByProvider,
} from "../../credentials/repositories/CredentialRepository.js";
...
async function refreshedCredential(
  ...
  return {
    ...
    credential_purpose:
      routingMetadata?.purpose || row.metadata?.purpose || null,
    delegated: Boolean(routingCredentialId),
    login_customer_id: text(routingMetadata?.login_customer_id) || null,
    delegated_from_organization_id:
      text(routingMetadata?.delegated_from_organization_id) || null,
  };
}
  ...
  registerProviderCredentialResolver(
    ...
    async ({ organization_id, credential_id = null }) => {
      ...
      const candidates = rows
        ...
        .sort((a, b) => {
          const aDelegated =
            text(a.credential_type).toLowerCase() === "delegated_oauth_token";
          const bDelegated =
            text(b.credential_type).toLowerCase() === "delegated_oauth_token";
          return Number(bDelegated) - Number(aDelegated);
        });
      ...
      if (text(selected.credential_type).toLowerCase() === "delegated_oauth_token") {
        ...
        return refreshedCredential(source, selected.id, selected.metadata || {});
      }
