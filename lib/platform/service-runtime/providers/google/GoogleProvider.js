...
import { ProviderEventRuntime } from "../../events/runtime/ProviderEventRuntime.js";
...
function shouldUseLegacyBusinessDiscovery(error) {
  const message = text(error?.message).toLowerCase();
  return (
    Number(error?.status) === 429 ||
    message.includes("mybusinessaccountmanagement.googleapis.com") ||
    message.includes("quota exceeded") ||
    message.includes("resource_exhausted")
  );
}

async function readPersonalBusinessLocations({ access_token }) {
  const locations = [];
  let pageToken = null;

  do {
    const url = new URL(`${GOOGLE_BUSINESS_INFO_API}/accounts/me/locations`);
    ...
    locations.push(
      ...pageLocations.map((location) => ({
        ...location,
        account_name: "accounts/me",
        account_title: "Authenticated Google Business Profile account",
        review_parent: `accounts/me/${location.name}`,
        discovery_source: "business_information_v1_accounts_me",
      }))
    );
  ...
  return {
    ...
    output: {
      accounts: [
        {
          name: "accounts/me",
          accountName: "Authenticated Google Business Profile account",
          type: "PERSONAL",
        },
      ],
      locations,
...
async function readLegacyBusinessLocations({ access_token }) {
  const accounts = [];
  let accountPageToken = null;

  do {
    const url = new URL(`${GOOGLE_BUSINESS_API}/accounts`);
  ...
  for (const account of accounts) {
    ...
    do {
      const url = new URL(`${GOOGLE_BUSINESS_API}/${account.name}/locations`);
      url.searchParams.set("pageSize", "100");
      if (pageToken) url.searchParams.set("pageToken", pageToken);
...
async function readBusinessLocations({ access_token }) {
  ...
  try {
    ...
    if (!shouldUseLegacyBusinessDiscovery(error)) throw error;

    try {
      return await readPersonalBusinessLocations({ access_token });
    } catch (personalError) {
      try {
        return await readLegacyBusinessLocations({ access_token });
      } catch (legacyError) {
        const combined = new Error(
          `Google Business Profile discovery failed. accounts/me: ${personalError?.message || "unknown error"}; legacy v4: ${legacyError?.message || "unknown error"}`
        );
        combined.status = personalError?.status || legacyError?.status || error?.status;
        combined.code = personalError?.code || legacyError?.code || error?.code || null;
        throw combined;
      }
    }
  }
    ...
    do {
      ...
      locations.push(
        ...pageLocations.map((location) => ({
          ...
          account_title: account.accountName || account.name,
          review_parent: `${account.name}/${location.name}`,
          discovery_source: "business_information_v1",
        }))
      );
      pageToken = result.nextPageToken || null;
    } while (pageToken);
...
async function manageGoogleAds({ access_token, payload = {} }) {
  ...
  } else if (action === "create_customer_client") {
    if (!customerId) throw new Error("GOOGLE_ADS_MANAGER_CUSTOMER_ID_REQUIRED");
    const descriptiveName = text(payload.descriptive_name || payload.descriptiveName);
    const currencyCode = text(payload.currency_code || payload.currencyCode).toUpperCase();
    const timeZone = text(payload.time_zone || payload.timeZone);
    if (!descriptiveName) throw new Error("GOOGLE_ADS_ACCOUNT_NAME_REQUIRED");
    if (!currencyCode) throw new Error("GOOGLE_ADS_ACCOUNT_CURRENCY_REQUIRED");
    if (!timeZone) throw new Error("GOOGLE_ADS_ACCOUNT_TIME_ZONE_REQUIRED");
    url = `https://googleads.googleapis.com/${apiVersion}/customers/${customerId}:createCustomerClient`;
    body = {
      customerClient: {
        descriptiveName,
        currencyCode,
  ...
  } else if (action === "mutate_customer_client_links") {
    if (!customerId) throw new Error("GOOGLE_ADS_MANAGER_CUSTOMER_ID_REQUIRED");
    if (!Array.isArray(payload.operations) || !payload.operations.length) {
      throw new Error("GOOGLE_ADS_OPERATIONS_REQUIRED");
    }
    url = `https://googleads.googleapis.com/${apiVersion}/customers/${customerId}/customerClientLinks:mutate`;
    body = { operations: payload.operations };
  } else if (action === "mutate_customer_manager_links") {
    if (!customerId) throw new Error("GOOGLE_ADS_CUSTOMER_ID_REQUIRED");
    if (!Array.isArray(payload.operations) || !payload.operations.length) {
      throw new Error("GOOGLE_ADS_OPERATIONS_REQUIRED");
    }
    url = `https://googleads.googleapis.com/${apiVersion}/customers/${customerId}/customerManagerLinks:mutate`;
    body = { operations: payload.operations };
