import {
  CredentialRuntime,
} from "../../credentials/runtime/CredentialRuntime.js";
import {
  resolveFirstCreativeProviderAssetUrl,
} from "../../../../creative/assets/storage/resolveCreativeProviderAssetUrl.js";

const DEFAULT_TEXT_MODEL = "xai/grok-imagine-video/text-to-video";
const DEFAULT_IMAGE_MODEL = "xai/grok-imagine-video/image-to-video";
...
async function resolveCredential(credentialId = null) {
  ...
  if (!apiKey) throw new Error("GROK_FAL_CREDENTIAL_REQUIRED");
...
function normalizedModel(value, sourceImageUrl = null) {
  ...
  if (!/^xai\/grok-imagine-video\/[a-z0-9._/-]+$/i.test(model)) {
    throw new Error(`GROK_MODEL_INVALID:${model}`);
  }
  return model;
}
...
function decodeJobId(value, fallbackModel = DEFAULT_TEXT_MODEL) {
  ...
  if (!raw) throw new Error("GROK_REQUEST_ID_REQUIRED");
...
function trustedFalUrl(value, label) {
  ...
  try {
    ...
    throw new Error(`GROK_${label}_URL_INVALID`);
  }
  ...
  if (
    ...
    throw new Error(`GROK_${label}_URL_UNTRUSTED`);
  }
...
function falErrorMessage(result = {}, status, rawText = "") {
  return [
    `Grok/FAL request failed with status ${status}`,
...
export const GrokProvider = {
  ...
  async execute(input = {}) {
    if (text(input.capability) !== "ai.video.generate") {
      throw new Error(`GROK_CAPABILITY_NOT_SUPPORTED:${text(input.capability)}`);
    }
    ...
    if (!prompt) throw new Error("GROK_PROMPT_REQUIRED");

    const duration = finite(
      input.duration_seconds ?? input.duration ?? output.duration_seconds ?? generation.estimated_seconds,
      6,
    );
    const body = compactObject({
      ...
      duration,
      aspect_ratio: input.aspect_ratio ?? input.aspectRatio ?? output.aspect_ratio ?? providerParameters.aspect_ratio,
    ...
    if (model.includes("image-to-video") && !sourceImageUrl) {
      throw new Error("GROK_IMAGE_TO_VIDEO_SOURCE_REQUIRED");
    }
    ...
    if (!requestId && !videoUrl) throw new Error("GROK_OUTPUT_OR_REQUEST_ID_REQUIRED");
  ...
  async getStatus(input = {}) {
    ...
    if (failed) {
      return {
        ...
        error: text(status?.error?.message || status?.error || status?.message || status?.detail) || "Grok video generation failed",
        queue,
        raw: status,
      };
    }
    ...
    const videoUrl = outputVideoUrl(result);
    if (!videoUrl) throw new Error("GROK_COMPLETED_VIDEO_URL_REQUIRED");
