...
async function uploadExactImage({ access_token, ad_account_id, image_url }) {
  ...
  if (!source.ok) {
    throw new Error(`Exact source image could not be fetched (${source.status})`);
  }

  const contentType = supportedImageContentType(source.headers.get("content-type"));
  if (!contentType) {
    throw new Error("Exact source image must be JPEG, PNG or WebP");
  }

  const bytes = Buffer.from(await source.arrayBuffer());
  if (!bytes.length) throw new Error("Exact source image is empty");
  if (bytes.length > 30 * 1024 * 1024) {
    throw new Error("Exact source image exceeds the 30 MB upload limit");
  }
  ...
  const result = await graphRequest({
    ...
    params: {
      ...
      name: `avantiqo-${crypto.randomUUID()}`,
    },
  });
...
function creativeLink({ creative, page_id }) {
  ...
  return `https://www.facebook.com/${encodeURIComponent(page_id)}`;
}
...
function callToAction({ creative, destination, link }) {
  ...
  return {
    ...
    value: creative.call_to_action_value || { link },
  };
}
...
async function createAdsBundle({
  ...
  if (destination === "WHATSAPP" && !whatsapp_destination) {
  ...
  try {
    ...
    return {
      ...
      ad_id: adResult.id,
      image_hash: uploadedImage.hash,
      image_byte_length: uploadedImage.byte_length,
