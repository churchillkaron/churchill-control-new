...
} from "./OpenAIResponsesJsonModeRuntime.js";

async function getOpenAIClient(input = {}) {
  const resolvedApiKey =
    input?.credential?.api_key ||
    input?.api_key ||
    null;
  ...
  const credentialId = input?.credential_id || null;
...
async function generateText(client, input = {}) {
  ...
  const compatibleRequestInput = ensureResponsesJsonModeInput(requestInput, format);
...
async function analyseMedia(client, input = {}) {
  ...
  const options = providerOptions(input);
  const format = responsesTextFormat(input);
  const requestInput = ensureResponsesJsonModeInput([
    {
      role: "user",
      content: [
...
async function synthesizeSpeech(client, input = {}) {
  const model = String(input.model || "").trim();
  const speechText = String(input.input || input.text || input.message || "").trim();
  const voice = String(
    input.voice ||
    input.payload?.voice ||
  ...
  const response = await client.audio.speech.create(compactObject({
    model,
    voice,
    input: speechText.slice(0, 4096),
    instructions: instructions || undefined,
    response_format: responseFormat,
    speed: Number.isFinite(Number(input.speed))
      ? Number(input.speed)
      : undefined,
  }));

  const audio = Buffer.from(await response.arrayBuffer());
  if (!audio.length) throw new Error("OPENAI_TTS_AUDIO_REQUIRED");
  ...
  return {
    ...
    output: {
      ...
      mime_type: mimeTypes[responseFormat] || "audio/mpeg",
      response_format: responseFormat,
...
async function createRealtimeTranscriptionSession(client, input = {}) {
  const model = String(input.model || "").trim();
  const ttl = Math.max(
    10,
    Math.min(120, Number(input.session_ttl_seconds || 60)),
  );

  if (!model) throw new Error("OPENAI_REALTIME_TRANSCRIPTION_MODEL_REQUIRED");
  ...
  const clientSecret = String(response?.value || "").trim();
...
export const OpenAIProvider = {
  ...
  async execute(input = {}) {
    const client = await getOpenAIClient(input);
    ...
    switch (input.capability) {
      ...
      case "ai.speech.to.text.realtime":
        return createRealtimeTranscriptionSession(client, input);
