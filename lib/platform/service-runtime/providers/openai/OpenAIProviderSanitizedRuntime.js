...
async function createRealtimeTranscriptionSession(input = {}) {
  const apiKey = text(input.credential?.api_key);
  const model = text(input.model) || "gpt-realtime-whisper";
  if (!apiKey) throw new Error("OPENAI_REALTIME_MANAGED_CREDENTIAL_REQUIRED");
  if (model !== "gpt-realtime-whisper") {
    throw new Error(`OPENAI_REALTIME_TRANSCRIPTION_MODEL_UNSUPPORTED:${model}`);
  }
  ...
  const response = await client.realtime.clientSecrets.create({
    expires_after: {
      anchor: "created_at",
      seconds: input.session_ttl_seconds,
    },
    session: {
      type: "transcription",
      audio: {
  ...
  const sessionId = text(response?.session?.id);
  const clientSecret = text(response?.value);
  if (!sessionId || !clientSecret) {
    throw new Error("OPENAI_REALTIME_TRANSCRIPTION_SESSION_INVALID");
  }
...
export const OpenAIProvider = {
  ...
  async execute(input = {}) {
    ...
    if (sanitized.capability === "ai.speech.to.text.realtime") {
      return createRealtimeTranscriptionSession(sanitized);
    }
