...
function assertAvantiqoManagedOpenAIInput(input = {}) {
  ...
  if (
    ...
    throw new Error("OPENAI_AVANTIQO_GOVERNED_CONTEXT_REQUIRED");
  }

  if (
    text(credential.managed_by).toUpperCase() !== "AVANTIQO" ||
    text(credential.credential_purpose).toUpperCase() !== "AVANTIQO_MANAGED_AI" ||
    text(credential.api_family).toUpperCase() !== "OPENAI_API" ||
    !text(credential.api_key) ||
    !text(input.credential_id)
  ) {
  ...
  if (
    ...
    throw new Error("OPENAI_AVANTIQO_CREDENTIAL_TRANSPORT_REQUIRED");
  }
}
...
function isExactGptImage1(model = "") {
  return String(model || "").trim().toLowerCase() === "gpt-image-1";
}

function deterministicGptImage1Dimension(value, {
  allowed,
  fallback,
  label,
} = {}) {
  const candidate = text(value).toLowerCase();
  const resolved = !candidate || candidate === "auto" ? fallback : candidate;
  if (!allowed.includes(resolved)) {
    throw new Error(`OPENAI_GPT_IMAGE_1_${label}_INVALID:${resolved}`);
  }
  return resolved;
}

function gptImage1PricingDimensions({ model, quality, size } = {}) {
  if (!isExactGptImage1(model)) {
    return { quality, size };
  }
  return {
    quality: deterministicGptImage1Dimension(quality, {
      allowed: ["low", "medium", "high"],
      fallback: "medium",
      label: "QUALITY",
    }),
    size: deterministicGptImage1Dimension(size, {
...
function sanitizeImage(input = {}) {
  ...
  const dimensions = gptImage1PricingDimensions({
    model,
  ...
  return {
    ...
    provider_sanitization: {
      ...
      deterministic_pricing_dimensions: isExactGptImage1(model),
      pricing_quality: dimensions.quality || null,
      pricing_size: dimensions.size || null,
...
function sanitizeRealtimeTranscription(input = {}) {
  const ttl = Number(input.session_ttl_seconds || input.sessionTtlSeconds || 60);
  const language = text(input.language).toLowerCase().split("-")[0] || undefined;

  return {
    ...input,
    language,
    session_ttl_seconds: Math.max(10, Math.min(120, Number.isFinite(ttl) ? ttl : 60)),
    provider_options: {},
    provider_parameters: {},
    provider_sanitization: {
      contract: "OPENAI_PROVIDER_INPUT_SANITIZATION_V1",
...
function sanitize(input = {}) {
  switch (input.capability) {
    ...
    case "ai.speech.to.text.realtime":
      return sanitizeRealtimeTranscription(input);
    default:
      return {
        ...input,
        provider_options: {},
...
export const OpenAIProvider = {
  ...
  async execute(input = {}) {
    assertAvantiqoManagedOpenAIInput(input);
...
export const OpenAIProviderSanitizedRuntime = {
  ...
  sanitizeRealtimeTranscription,
  supportsInputFidelity,
  gptImage1PricingDimensions,
  localizeAnalysisMedia,
  assertAvantiqoManagedOpenAIInput,
};
