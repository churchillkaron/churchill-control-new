...
function configuredBinary(policy, snakeName, camelName, environmentName, errorCode) {
  const binary = text(
    policy?.[snakeName] ||
    policy?.[camelName] ||
    process.env[environmentName],
  );
  if (!binary) throw new Error(errorCode);
  return binary;
}
...
function probeVideo(filePath, mediaPolicy = {}) {
  const ffprobe = configuredBinary(
    mediaPolicy,
    "ffprobe_path",
    "ffprobePath",
    "CREATIVE_MEDIA_FFPROBE_PATH",
    "OPENAI_VIDEO_ANALYSIS_FFPROBE_NOT_CONFIGURED",
  );
  const raw = runBinary(
    ffprobe,
    [
...
function extractFrame({ inputPath, outputPath, second, mediaPolicy = {} }) {
  const ffmpeg = configuredBinary(
    mediaPolicy,
    "ffmpeg_path",
    "ffmpegPath",
    "CREATIVE_MEDIA_FFMPEG_PATH",
    "OPENAI_VIDEO_ANALYSIS_FFMPEG_NOT_CONFIGURED",
  );
  runBinary(
    ffmpeg,
    [
...
async function buildFrames({ videoUrl, organizationId, mediaPolicy = {} }) {
  ...
  const preparation = (async () => {
    ...
    try {
      const probe = probeVideo(materialized.file_path, mediaPolicy);
