...
import {
  materializeMedia,
} from "../../../../creative/media/runtime/CreativeMediaInspectionRuntime.js";
import {
  creativeStorageReference,
  creativeStorageUri,
} from "../../../../creative/assets/storage/CreativePrivateStorageRuntime.js";
import {
  resolveCreativeFfmpegPath,
  resolveCreativeFfprobePath,
} from "../../../../creative/media/runtime/CreativeMediaBinaryRuntime.js";
...
function videoExpected(input = {}) {
  ...
  const explicitVideo = Boolean(
    directUrl(input.video) ||
    directUrl(payloadInput(input).video),
  );
  if (kind && kind !== "VIDEO") return explicitVideo;
  return kind === "VIDEO" || explicitVideo || Boolean(generatedVideoUrl(input));
}

function expectedContract(input = {}) {
  const payload = payloadInput(input);
  return object(
    input.requirements?.expected_contract ||
    input.metadata?.requirements?.expected_contract ||
    payload.requirements?.expected_contract ||
    payload.metadata?.requirements?.expected_contract,
  );
}

function durationHint(input = {}) {
  const payload = payloadInput(input);
  const expected = expectedContract(input);
  return finite(
    expected.output_spec?.duration_seconds ||
    expected.performance_contract?.duration_seconds ||
    ...
    payload.generation?.duration_seconds ||
    payload.generation?.estimated_seconds,
  );
}

function dimensionHint(input = {}) {
  const expected = expectedContract(input);
  const resolution = text(expected.output_spec?.resolution);
  const match = resolution.match(/^(\d+)\s*[xX]\s*(\d+)$/);
  if (!match) {
    return { source_width: 0, source_height: 0 };
  }
  return {
    source_width: Number(match[1]),
    source_height: Number(match[2]),
  };
}

function sameProjectStorageReference(value) {
  const source = text(value);
  if (!source) return null;
  if (creativeStorageReference(source)) return source;

  try {
    const candidate = new URL(source);
    const configured = new URL(text(process.env.NEXT_PUBLIC_SUPABASE_URL));
...
function cacheKey(value, input = {}) {
  try {
    const parsed = new URL(value);
    return `${parsed.origin}${parsed.pathname}:${durationHint(input) || "probe"}`;
  } catch {
    return `${text(value)}:${durationHint(input) || "probe"}`;
  }
}
...
function probeVideo(filePath, mediaPolicy = {}) {
  const ffprobe = resolveCreativeFfprobePath(mediaPolicy);
  if (!ffprobe) {
    throw new Error("OPENAI_VIDEO_ANALYSIS_FFPROBE_NOT_CONFIGURED");
  }
  const raw = runBinary(
    ffprobe,
    [
...
function timelineEvidence(filePath, input = {}, mediaPolicy = {}) {
  const duration = durationHint(input);
  if (duration) {
    return {
      duration_seconds: duration,
      ...dimensionHint(input),
      duration_source: "APPROVED_CONTRACT",
    };
  }
  return probeVideo(filePath, mediaPolicy);
}

function extractFrame({ inputPath, outputPath, second, mediaPolicy = {} }) {
  const ffmpeg = resolveCreativeFfmpegPath(mediaPolicy);
  if (!ffmpeg) {
    throw new Error("OPENAI_VIDEO_ANALYSIS_FFMPEG_NOT_CONFIGURED");
  }
  const attempts = [...new Set([
    Number(second),
    Math.max(0, Number(second) - 0.25),
    Math.max(0, Number(second) - 0.75),
...
async function buildFrames({
  ...
  const internalStorageReference = sameProjectStorageReference(videoUrl);
  const materializationUrl = internalStorageReference || videoUrl;
  const key = cacheKey(materializationUrl, sourceInput);
  if (frameCache.has(key)) return frameCache.get(key);

  const preparation = (async () => {
    const materialized = await materializeMedia({
      url: materializationUrl,
    ...
    try {
      ...
      for (const [offset, fraction] of FRAME_FRACTIONS.entries()) {
        const second = Math.max(
          0,
          Math.min(
            Math.max(0, timeline.duration_seconds - 0.05),
            timeline.duration_seconds * fraction,
          ),
        );
        const outputPath = path.join(
          directory,
          `openai-review-frame-${String(offset + 1).padStart(2, "0")}.png`,
        );
        extractFrame({
      ...
      return {
        ...
        source_transport: internalStorageReference
        ...
        source_duration_seconds: Number(
          timeline.duration_seconds.toFixed(6),
        ),
        source_duration_basis: timeline.duration_source,
...
export async function prepareOpenAIVideoAnalysisInput(input = {}) {
  ...
  if (!creativeStorageReference(videoUrl) && !/^https?:\/\//i.test(videoUrl)) {
    throw new Error("OPENAI_VIDEO_ANALYSIS_SOURCE_PROTOCOL_REQUIRED");
  }
  ...
  return {
    ...
    openai_video_analysis_frame_contract: {
      ...
      source_transport: frameSet.source_transport,
      ...
      source_duration_basis: frameSet.source_duration_basis,
