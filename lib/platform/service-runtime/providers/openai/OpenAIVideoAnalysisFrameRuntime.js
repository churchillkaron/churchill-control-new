...
function primaryVideoUrl(input = {}) {
  const explicit = directUrl(input.video);
  if (explicit) return explicit;
  if (mediaKind(input) !== "VIDEO") return null;
  return directUrl(input.media) ||
    directUrl(input.source) ||
    directUrl(input.image) ||
    list(input.assets)
      .map(directUrl)
      .find(Boolean) ||
    directUrl(input.provider_parameters?.generated_media_url) ||
...
async function normalizeFrame({ filePath, fraction, second, index }) {
  ...
  return {
    ...
    url: dataUri,
    role: "GENERATED_VIDEO_FRAME_UNDER_REVIEW",
  };
}
...
async function buildFrames({ videoUrl, organizationId, mediaPolicy = {} }) {
  ...
  const preparation = (async () => {
    ...
    try {
      ...
      return {
        contract: "OPENAI_VIDEO_ANALYSIS_FRAME_SET_V1",
...
function removeVideoAsset(values, videoUrl) {
  return list(values).filter((value) => directUrl(value) !== videoUrl);
}
...
export async function prepareOpenAIVideoAnalysisInput(input = {}) {
  if (input.openai_video_analysis_frame_contract?.prepared === true) {
    return input;
  }
  if (text(input.capability) !== "ai.image.analyze") return input;

  const videoUrl = primaryVideoUrl(input);
  if (!videoUrl) return input;
  if (!/^https?:\/\//i.test(videoUrl)) {
    throw new Error("OPENAI_VIDEO_ANALYSIS_HTTPS_SOURCE_REQUIRED");
  }

  const organizationId = text(
    input.context?.organization_id || input.organization_id,
  );
  if (!organizationId) {
  ...
  const frameSet = await buildFrames({
    videoUrl,
    organizationId,
    mediaPolicy: input.media_policy || input.mediaPolicy || {},
  });
  const preservedAssets = removeVideoAsset(input.assets, videoUrl);
