...
function resolveSecret(reference) {
  ...
  return name ? text(process.env[name]) || null : null;
}

registerProviderCredentialResolver(
  "shopify",
  async ({ organization_id, credential_id = null }) => {
    const rows = await listActiveByProvider("shopify");
    const selected = rows
      .filter((row) => {
        const metadata = object(row.metadata);
        return (
          text(metadata.organization_id) === text(organization_id) &&
          text(metadata.purpose).toUpperCase() === "ORGANIZATION_SHOPIFY_STORE" &&
    ...
    const shop = text(metadata.shop).toLowerCase();
    ...
    return {
      ...
      credential_purpose: "ORGANIZATION_SHOPIFY_STORE",
