...
registerProviderCredentialResolver(
  ...
  async ({ organization_id, credential_id = null }) => {
    const rows = await listActiveByProvider("tiktok");
    const selected = rows
      .filter((row) => {
    ...
    return {
      ...
      refresh_expires_in: secret.refresh_expires_in || null,
      open_id: text(secret.open_id || metadata.external_account_id) || null,
