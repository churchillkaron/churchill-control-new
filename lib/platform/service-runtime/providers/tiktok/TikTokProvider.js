import { save as saveCredential } from "../../credentials/repositories/CredentialRepository.js";
...
async function apiPost(path, accessToken, body) {
  ...
  if (!response.ok || (code && code !== "ok")) {
    ...
    error.status = code === "access_token_invalid" ? 401 : response.status;
...
function uploadPlan(size) {
  const MB = 1024 * 1024;
  if (size < 5 * MB) return { chunkSize: size, totalChunks: 1 };
  const chunkSize = 10 * MB;
  return {
    chunkSize,
    totalChunks: Math.floor(size / chunkSize) || 1,
  };
}

async function uploadChunks(uploadUrl, buffer, type, chunkSize, totalChunks) {
  const totalBytes = buffer.length;
  let start = 0;
  for (let index = 0; index < totalChunks; index += 1) {
    const isFinal = index === totalChunks - 1;
    const endExclusive = isFinal ? totalBytes : Math.min(start + chunkSize, totalBytes);
  ...
  if (start !== totalBytes) throw new Error("TIKTOK_VIDEO_UPLOAD_INCOMPLETE");
}
...
async function directVideo(input, accessToken, creator) {
  ...
  const { chunkSize, totalChunks } = uploadPlan(buffer.length);
  ...
  await uploadChunks(data.upload_url, buffer, type, chunkSize, totalChunks);
