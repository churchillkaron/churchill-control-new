...
function normalizedModel(value, sourceImageUrl = null) {
  ...
  if (!/^fal-ai\/veo3\.1\/fast(?:\/image-to-video)?$/i.test(model)) {
...
export const VeoProvider = {
  ...
  async execute(input = {}) {
    ...
    const body = compactObject({
      ...
      duration: input.duration_seconds ?? input.duration ?? output.duration_seconds ?? providerParameters.duration,
      ...
      generate_audio: input.generate_audio ?? input.generateAudio ?? providerParameters.generate_audio ?? false,
