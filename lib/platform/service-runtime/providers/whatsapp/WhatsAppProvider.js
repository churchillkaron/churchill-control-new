...
function whatsappDocumentPayload({ recipient, message, attachment }) {
  const link = String(attachment?.url || "").trim();
  if (!/^https?:\/\//i.test(link)) {
    throw new Error("WHATSAPP_DOCUMENT_PUBLIC_URL_REQUIRED");
  }
...
async function sendMessage({ phone_number_id, access_token, recipient, message, attachments = [] }) {
  ...
  const body = files.length === 1
    ? whatsappDocumentPayload({ recipient, message, attachment: files[0] })
    : {
        messaging_product: "whatsapp",
        to: recipient,
        type: "text",
