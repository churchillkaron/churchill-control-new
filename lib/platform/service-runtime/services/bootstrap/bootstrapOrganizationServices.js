...
import {
  SERVICE_CATALOG,
} from "../../../registry/business-services/BusinessServiceRegistry.js";

import {
  MANAGED_PLATFORM_SERVICE_CATEGORIES,
} from "../catalog/ManagedPlatformServiceCatalog.js";
...
const OPERATOR_VOICE_SERVICE_IDS = new Set([
  "ai.speech.to.text",
  "ai.text.generate",
...
function operatorVoicePlatformCategory() {
  return {
    id: "platform-ai",
    name: "Platform AI",
    description:
      "Avantiqo-managed AI services required for the real-time Operator voice experience across every organization.",
    services: (PLATFORM_AI_SERVICES || [])
      .filter((service) => OPERATOR_VOICE_SERVICE_IDS.has(service.id))
      .map((service) => ({
        ...service,
        ...
        connection_model: "AVANTIQO_MANAGED_PROVIDER",
      })),
  };
}
