...
} from "../../ai/PlatformAIServiceCatalog.js";
...
function platformAiEntitlement(serviceId, organizationService = null) {
  const normalizedServiceId = String(serviceId || "").trim();
  if (!normalizedServiceId) return null;
  const standard = platformAiEntitlements().find(
    (service) => service.service_id === normalizedServiceId,
  );
  if (!standard) return null;

  // Preserve a real organization's operational metadata when it exists, while
  // platform-standard availability remains authoritative for Avantiqo AI.
  return {
    ...(organizationService || {}),
    ...standard,
    id: organizationService?.id || null,
...
export async function resolveOrganizationService({
  ...
  const organizationService = await OrganizationServices.getByService({
    organization_id,
    service_id,
  });

  return platformAiEntitlement(service_id, organizationService) || organizationService;
