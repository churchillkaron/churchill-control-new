import {
  SERVICE_CATALOG,
} from "../../../registry/business-services/BusinessServiceRegistry.js";

import {
  resolveOrganizationServices,
} from "./OrganizationServiceResolver.js";

import {
  summarizeByOrganization,
} from "../../usage/repositories/ServiceUsageRepository.js";

import {
  resolveExecutionCapabilities,
} from "./CapabilityExecutionResolver.js";


function findEnabledService(
  organizationServices,
  serviceId
){

  return organizationServices.find(
    item =>
      item.service_id === serviceId
  );

}


export async function resolveOrganizationServiceDomains({

  organization_id,

}) {


  const organizationServices =
    await resolveOrganizationServices({

      organization_id,

    });


  const usageRows =
    await summarizeByOrganization(
      organization_id
    );

  const usageByCapability = new Map(
    usageRows.map((row) => [String(row.capability || "").toUpperCase(), row]),
  );


  const enabled = [];


  for (
    const category
    of SERVICE_CATALOG
  ) {


    const domain = {

      id:
        category.id,

      name:
        category.name,

      description:
        category.description,

      services: [],

      capabilities: [],

      usage:0,

      cost:0,

      status:"AVAILABLE",

    };


    for (
      const service
      of category.services || []
    ) {


      const organizationService =
        findEnabledService(
          organizationServices.flatMap(
            group =>
              group.services || []
          ),
          service.id
        );


      const serviceUsage = [
        ...(service.requires || []).map((capability) =>
          usageByCapability.get(String(capability).toUpperCase()),
        ),
        usageByCapability.get(String(service.id || "").toUpperCase()),
      ].filter(Boolean);


      domain.services.push({

        id:
          service.id,

        name:
          service.name,

        description:
          service.description,

        package:
          service.package,

        enabled:
          Boolean(
            organizationService ||
            service.default_enabled
          ),

        status:
          organizationService?.status ||
          (
            service.default_enabled
              ? "ACTIVE"
              : "AVAILABLE"
          ),

        capabilities:
          service.requires || [],

        usage:
          serviceUsage.reduce(
            (sum,row)=>
              sum +
              Number(
                row.quantity || 0
              ),
            0
          ),

        cost:
          serviceUsage.reduce(
            (sum,row)=>
              sum +
              Number(
                row.customer_price || 0
              ),
            0
          ),

      });


      domain.capabilities.push(
        ...(service.requires || [])
      );


      domain.usage +=
        serviceUsage.reduce(
          (sum,row)=>
            sum +
            Number(
              row.quantity || 0
            ),
          0
        );


      domain.cost +=
        serviceUsage.reduce(
          (sum,row)=>
            sum +
            Number(
              row.customer_price || 0
            ),
          0
        );


    }


    domain.service_count =
      domain.services.length;


    domain.capability_count =
      [
        ...new Set(
          domain.capabilities
        )
      ].length;


    domain.capabilities =
      [
        ...new Set(
          domain.capabilities
        )
      ];


    domain.service_names =
      domain.services.map(
        service =>
          service.name
      );


    domain.capability_names =
      domain.capabilities;



    if (
      domain.services.length
    ) {

      enabled.push(domain);

    }

  }


  return enabled;

}
