...
function assertIdentity(existing, expected) {
  const fields = [
    "id",
    "organization_id",
...
function assertRecoverable(existing, expected, { allow_success_replay = false } = {}) {
  assertIdentity(existing, expected);
  const status = text(existing?.status).toUpperCase();
  if (status === "SUCCESS" && allow_success_replay === true) {
    const providerResult = existing?.metadata?.provider_result;
    if (!providerResult || typeof providerResult !== "object") {
      throw new Error("SERVICE_USAGE_IDEMPOTENT_START_SUCCESS_RESULT_MISSING");
    }
    return existing;
  }
  if (status !== "PENDING") {
    throw new Error(
      `SERVICE_USAGE_IDEMPOTENT_START_STATE_CONFLICT:${status || "missing"}`,
    );
  }
...
export async function createIdempotentUsageRecord({
  ...
  max_attempts = 4,
  sleep = delay,
  allow_preexisting_recovery = false,
  allow_preexisting_success_replay = false,
} = {}) {
  ...
  if (preexisting) {
    if (allow_preexisting_recovery === true) {
      return assertRecoverable(preexisting, record, {
        allow_success_replay: allow_preexisting_success_replay === true,
      });
    }
