...
export function createOrganizationWallet(data = {}) {
  ...
  if (!currency) throw new Error("WALLET_CURRENCY_REQUIRED");
  ...
  return {
    ...
    billing_policy: data.billing_policy || BILLING_POLICIES.PREPAID,
    auto_topup: Boolean(data.auto_topup),
    auto_topup_threshold: Number(data.auto_topup_threshold || 0),
    auto_topup_amount: Number(data.auto_topup_amount || 0),
