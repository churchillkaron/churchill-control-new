...
async function refund({ organization_id, amount, currency = null, usage_id = null, reference = null, provider = null, metadata = {} }) {
  const operation = WALLET_TRANSACTION_TYPES.REFUND;
  const result = await WalletRepository.applyTransaction({
    organization_id,
    operation,
    ...
    usage_id,
    reference,
    idempotency_key: idempotencyKey(operation, { usage_id, reference }),
    metadata: {
      ...metadata,
      refund_contract: metadata.refund_contract || "WALLET_REFUND_V1",
    },
  });
  return transactionResult(result);
}
