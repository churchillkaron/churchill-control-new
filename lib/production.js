...
export async function runProduction({
  ...
  for (const recipeItem of recipeItems) {
    const requiredQty =
      Number(recipeItem.quantity) * Number(quantity);

    const stock =
      Number(ingredientMap[recipeItem.ingredient_id]?.quantity || 0);

    if (stock < requiredQty) {
      throw new Error(
        `Not enough ${ingredientMap[recipeItem.ingredient_id]?.name}`
      );
    }
  }
