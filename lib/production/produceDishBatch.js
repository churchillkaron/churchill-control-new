...
export async function produceDishBatch({
  ...
  try {
    ...
    if (existingDishStock) {
      const newDishQty =
        Number(existingDishStock.quantity || 0) + Number(quantity);

      const { error: updateDishError } = await supabaseAdmin
        .from("dish_stock")
        .update({ quantity: newDishQty })
        .eq("tenant_id", tenantId)
        .eq("dish_id", dishId);

      if (updateDishError) throw updateDishError;
    } else {
      const { error: insertDishError } = await supabaseAdmin
        .from("dish_stock")
        .insert({
          tenant_id: tenantId,
          dish_id: dishId,
    ...
    return {
      ...
      ingredient_deductions: deductions.map((item) => ({
        ingredient_id: item.ingredient_id,
        ingredient_name: item.ingredient_name,
    ...
    console.error("PRODUCE DISH BATCH ERROR:", error);
