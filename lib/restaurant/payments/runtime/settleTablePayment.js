...
function terminalItemStatus(value) { return ["VOID", "VOIDED", "CANCELLED", "CANCELED"].includes(String(value || "").trim().toUpperCase()); }
function missingRelation(error, relation) { const message = String(error?.message || "").toLowerCase(); return error?.code === "42P01" || error?.code === "PGRST205" || message.includes(String(relation || "").toLowerCase()); }
...
async function validateSelectedItemSettlement({ organizationId, entityId, tableNumber, itemIds, amount, idempotencyKey }) {
  ...
  const itemResult = await supabaseAdmin.from("order_items").select("id, order_id, price, quantity, status").eq("organization_id", organizationId).eq("entity_id", entityId).in("order_id", orderIds).in("id", itemIds);
  ...
  if (items.some((item) => terminalItemStatus(item.status))) return { success: false, error: "Voided or cancelled restaurant items cannot be settled", status: 409 };

  const correctionResult = await supabaseAdmin
    .from("restaurant_order_item_corrections")
    .select("order_item_id, correction_type")
    .eq("organization_id", organizationId)
    .eq("entity_id", entityId)
    .eq("correction_type", "COMP")
    .in("order_item_id", itemIds);
  if (correctionResult.error && !missingRelation(correctionResult.error, "restaurant_order_item_corrections")) {
    return { success: false, error: correctionResult.error.message, status: 500 };
  }
  if ((correctionResult.data || []).length) {
    return { success: false, error: "Comped restaurant items are non-billable and cannot be settled", status: 409 };
  }
...
export async function settleTablePayment(request, { partial }) {
  try {
    ...
    const applicationId = String(readValue(body, "applicationId", "application_id") || "restaurant").trim().toLowerCase(); const requestedCashSessionId = readValue(body, "cashSessionId", "cash_session_id"); const tableNumber = readValue(body, "tableNumber", "table_number"); const paymentMethod = String(readValue(body, "paymentMethod", "payment_method") || "").trim().toUpperCase(); const amount = Number(readValue(body, "paidAmount", "paid_amount") ?? body.amount ?? 0); const requestedTenderedAmount = Number(readValue(body, "tenderedAmount", "tendered_amount") ?? amount); const tenderedAmount = paymentMethod === "CASH" ? requestedTenderedAmount : amount; const idempotencyKey = request.headers.get("idempotency-key") || readValue(body, "idempotencyKey", "idempotency_key"); const itemIds = normalizeItemIds(body.itemIds ?? body.item_ids ?? []);
    if (!entityId) return errorResponse("entityId required", 400); if (tableNumber === null || tableNumber === undefined || tableNumber === "") return errorResponse("tableNumber required", 400); if (!paymentMethod) return errorResponse("paymentMethod required", 400); if (!financeEventTypeForPaymentMethod(paymentMethod)) return errorResponse("Unsupported restaurant payment method", 400); if (!Number.isFinite(amount) || amount <= 0) return errorResponse("payment amount must be greater than zero", 400); if (!Number.isFinite(tenderedAmount) || tenderedAmount <= 0) return errorResponse("tendered amount must be greater than zero", 400); if (paymentMethod === "CASH" && tenderedAmount + 0.001 < amount) return errorResponse("Cash received cannot be less than the payment amount", 400); if (!idempotencyKey) return errorResponse("idempotencyKey required", 400);
    ...
    const { data, error } = await supabaseAdmin.rpc("restaurant_settle_table_atomic", { p_organization_id: access.organizationId, p_table_number: String(tableNumber), p_amount: amount, p_tendered_amount: tenderedAmount, p_payment_method: paymentMethod, p_partial: Boolean(partial), p_item_ids: itemIds.length ? itemIds : null, p_idempotency_key: String(idempotencyKey), p_actor_id: actorId, p_entity_id: entityId, p_application_id: applicationId, p_cash_session_id: settlement.cash_session_id, p_currency_code: settlement.currency_code });
    if (error) { const functionMissing = error.code === "PGRST202" || /restaurant_settle_table_atomic/i.test(error.message || ""); if (functionMissing) return errorResponse("Cash-evidence restaurant settlement is not deployed in the database", 503); const selectedItemConflict = error.code === "23505" && /restaurant_payment_allocations_unique_item_settlement_idx/i.test(error.message || ""); if (selectedItemConflict) return errorResponse("One or more selected items are already paid", 409); return errorResponse(error.message || "Payment settlement failed", 400); }
    ...
    return Response.json({ success: true, ...result, entityId, cashSessionId: settlement.cash_session_id, tenderedAmount: numeric(result.tenderedAmount ?? tenderedAmount), changeAmount: numeric(result.changeAmount ?? Math.max(0, tenderedAmount - amount)), remainingBalance, fullyPaid: remainingBalance <= 0, financeDispatchPending: financeDispatch.pending, financeDispatchError: financeDispatch.error });
