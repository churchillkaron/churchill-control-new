...
function resolveEntityId(context, payload) {
  ...
  if (contextEntityId && requestedEntityId && String(contextEntityId) !== String(requestedEntityId)) {
    throw conflict("Restaurant seat movement entity does not match the active business context");
  }

  return requireUuid(contextEntityId || requestedEntityId, "entityId");
}
...
export async function execute({ context, payload }) {
  ...
  const actorId = context.actor?.staffAccountId || context.actor?.id || null;

  const result = await supabaseAdmin.rpc("restaurant_move_seat_entity_atomic", {
  ...
  if (result.error) {
    const unavailable =
      ...
      /restaurant_move_seat_entity_atomic/i.test(result.error.message || "");

    if (unavailable) {
      const error = new Error(
        "Entity-safe atomic restaurant seat movement is not deployed in the database"
      );
      error.status = 503;
      throw error;
    }
  ...
  return {
    ...movement,
    entityId: movement.entityId || entityId,
