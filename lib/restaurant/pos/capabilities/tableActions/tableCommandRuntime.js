...
function payloadEntityId(payload) {
  return payload.entityId || payload.entity_id || payload.legalEntityId || payload.legal_entity_id;
}

function resolveEntityId({ context, payload }) {
  const contextEntityId = context?.entityId || context?.entity_id || null;
  const requestedEntityId = payloadEntityId(payload) || null;

  if (contextEntityId && requestedEntityId && String(contextEntityId) !== String(requestedEntityId)) {
    throw conflict("Restaurant table command entity does not match the active business context");
  }

  return requireValue(contextEntityId || requestedEntityId, "entityId");
}

async function moveGuests({ organizationId, entityId, payload }) {
  const tableId = requireValue(payload.tableId, "tableId");
  const guestCount = normalizedGuests(payload.guestCount);

  return executeAtomic("restaurant_set_guest_count_entity_atomic", {
...
async function closeTable({ organizationId, entityId, payload }) {
  const tableId = requireValue(payload.tableId, "tableId");

  return executeAtomic("restaurant_close_table_entity_atomic", {
    p_organization_id: organizationId,
    p_entity_id: entityId,
    p_table_id: tableId,
    p_actor_id: payload.authenticatedStaffId || null,
...
async function transferTable({ organizationId, entityId, payload }) {
  ...
  return executeAtomic("restaurant_transfer_table_entity_atomic", {
    p_organization_id: organizationId,
    p_entity_id: entityId,
...
async function mergeTables({ organizationId, entityId, payload }) {
  ...
  return executeAtomic("restaurant_merge_table_group_entity_atomic", {
    p_organization_id: organizationId,
    p_entity_id: entityId,
...
export async function executeTableCommand(command, { context, payload }) {
  const organizationId = requireValue(context?.organizationId, "organizationId");
  const entityId = resolveEntityId({ context, payload });
  ...
  if (command === "MOVE_GUESTS") {
    return moveGuests({ organizationId, entityId, payload: canonicalPayload });
  }

  if (command === "CLOSE_TABLE") {
    return closeTable({ organizationId, entityId, payload: canonicalPayload });
  }

  if (command === "TRANSFER_TABLE") {
    return transferTable({ organizationId, entityId, payload: canonicalPayload });
  }

  if (command === "MERGE_TABLES") {
    return mergeTables({ organizationId, entityId, payload: canonicalPayload });
  }
