...
import { generateRecommendations } from '@/lib/shared/recommendations/recommendationEngine'
...
export async function intelligenceHandler(payload) {
  const { tenantId, orderId, finance, inventory, kpi } = payload

  if (!tenantId || !orderId) {
    throw new Error('intelligenceHandler requires tenantId and orderId')
  }

  const revenue = Number(finance?.revenue || 0)
  const cost = Number(finance?.cogs || 0)
  const foodCost = revenue > 0 ? (cost / revenue) * 100 : 0
  const lowStock = inventory?.movements?.filter(m => Number(m.quantity || 0) < 0).length || 0
  ...
  const record = {
    ...
    food_cost: intelligence.foodCost,
    alerts: intelligence.alerts,
    anomalies,
    recommendations,
    kpi_state: kpi?.kpis?.state || null,
