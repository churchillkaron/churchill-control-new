...
function normalizeQualificationCodes(values) {
  if (!Array.isArray(values)) return [];
  return [...new Set(values.map((value) => code(value)).filter(Boolean))];
}
...
export function createServiceExecutionTemplateDocument(input = {}) {
  ...
  const fieldSchema = Array.isArray(input.field_schema || input.fieldSchema)
    ...
    : [];
  const requiredQualificationCodes = normalizeQualificationCodes(
    input.required_qualification_codes || input.requiredQualificationCodes,
  );
