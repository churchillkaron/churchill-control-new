...
export function normalizeServiceBilling(input = {}, defaults = {}) {
  ...
  const recurringSchedule = mode === "recurring"
    ? normalizeServiceRecurrence(input.recurrence || defaults.recurrence || {})
    : null;
  const firstBillingAt = mode === "recurring"
    ? isoOrNull(
        input.first_billing_at
        || input.firstBillingAt
        || defaults.first_billing_at
        || defaults.firstBillingAt,
      )
    : null;

  if (mode === "recurring" && !firstBillingAt) {
    const error = new Error("Recurring service billing requires first_billing_at.");
    error.status = 400;
    throw error;
  }
...
export function createServicePlanDocument(input = {}) {
  ...
  const recurrence = normalizeServiceRecurrence(input.recurrence || {});
  const billing = normalizeServiceBilling(input.billing || input, {
    recurrence,
    first_billing_at: contractStart,
  });

  if (
    billing.mode === "recurring"
    && contractEnd
    && new Date(billing.first_billing_at) > new Date(contractEnd)
  ) {
    const error = new Error("Recurring service billing must start on or before contract_end.");
    error.status = 400;
    throw error;
  }
...
export function servicePlanAttributes(plan) {
  const recurringBillingRuntime = plan.billing.mode === "recurring"
    ? {
        schema_version: 1,
        status: "active",
        next_billing_at: plan.billing.first_billing_at,
  ...
  return {
    service_delivery: {
      ...
      ...(recurringBillingRuntime ? { billing_runtime: recurringBillingRuntime } : {}),
