...
async function resolveOriginatingFollowUp({ occurrence, projection, newFollowUpWorkRequest = null }) {
  const corrective = occurrence?.attributes?.corrective_service || null;
  if (!corrective?.originating_occurrence_id || !corrective?.work_request_id) return null;
  if (newFollowUpWorkRequest) return null;

  const outcome = cleanText(projection?.protocol_submission?.outcome)?.toLowerCase() || "completed";
  if (FOLLOW_UP_OUTCOMES.has(outcome)) return null;

  const original = await loadServiceOccurrence({
    organizationId: occurrence.organization_id,
    occurrenceId: corrective.originating_occurrence_id,
  });
  if (!original) {
    const error = new Error("Corrective visit cannot close its follow-up because the originating occurrence was not found.");
    error.status = 409;
    throw error;
  }

  const originalCompletion = original.attributes?.completion || null;
  if (!originalCompletion) {
    const error = new Error("Corrective visit cannot close its follow-up because the originating service is not reconciled.");
    error.status = 409;
    throw error;
  }
  if (originalCompletion.follow_up_work_request_id !== corrective.work_request_id) {
    const error = new Error("Corrective visit follow-up lineage does not match the originating service completion.");
    error.status = 409;
    throw error;
  }

  const resolvedAt = projection.completed_at || new Date().toISOString();
  const resolution = {
    schema_version: 1,
    status: "resolved",
    resolved_at: resolvedAt,
    work_request_id: corrective.work_request_id,
    corrective_occurrence_id: occurrence.id,
    corrective_work_order_id: occurrence.work_order_id,
    completion_evidence_id: projection.completion_evidence_id || null,
    resolved_by_staff_id: projection.assigned_staff_id || null,
  };
...
async function reconcileOccurrenceRecord({ occurrence, actorId = null, permissions = [] }) {
  ...
  if (occurrence.status === "completed" && existingCompletion) {
    return {
      ...
      follow_up_resolution: existingCompletion.follow_up_resolution || null,
  ...
  const followUpResolution = await resolveOriginatingFollowUp({
    occurrence,
    projection,
    newFollowUpWorkRequest: followUpWorkRequest,
  });
  ...
  const completion = {
    ...
    resolves_follow_up: Boolean(followUpResolution),
...
export async function reconcileGeneratedServiceOccurrences({ limit = 100 } = {}) {
  ...
  return {
    ...
    follow_up_resolutions: results.filter(
      (row) => row.success && row.follow_up_resolution,
    ).length,
