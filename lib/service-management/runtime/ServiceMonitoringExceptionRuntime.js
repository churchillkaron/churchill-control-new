...
export async function getMonitoringExceptions({ context }) {
  ...
  const workOrdersByAction = new Map();
  ...
  for (const row of workOrders) {
    if (!row.source_id || !monitoringFollowUp(row)) continue;
    const bucket = workOrdersByAction.get(row.source_id) || [];
    bucket.push(row);
    workOrdersByAction.set(row.source_id, bucket);
  }
  ...
  for (const record of points) {
    ...
    const followUpWorkOrder = activeAction
      ? (workOrdersByAction.get(activeAction.id) || []).sort((a, b) => millis(b.updated_at || b.created_at) - millis(a.updated_at || a.created_at))[0] || null
      : null;
    ...
    const projected = {
      ...
      active_action: activeAction ? {
        ...
        follow_up_work_order: followUpWorkOrder ? {
          id: followUpWorkOrder.id,
          status: followUpWorkOrder.status,
          priority: followUpWorkOrder.priority || null,
          assigned_to: followUpWorkOrder.assigned_to || null,
          due_at: followUpWorkOrder.due_at || null,
          created_at: followUpWorkOrder.created_at,
          updated_at: followUpWorkOrder.updated_at,
          details: monitoringFollowUp(followUpWorkOrder),
        } : null,
      } : null,
    };
  ...
  return {
    ...
    metrics: {
      ...
      follow_up_ready: rows.filter((row) => row.active_action && !row.active_action.follow_up_work_order).length,
      follow_up_open: rows.filter((row) => Boolean(row.active_action?.follow_up_work_order)).length,
    ...
    authority: {
      ...
      rule: "Signals are deterministic projections from governed monitoring evidence. Human action creates the canonical corrective action; a separate human decision creates its governed follow-up work order. Completed actions suppress the signal until newer bad evidence appears.",
    },
  };
}
