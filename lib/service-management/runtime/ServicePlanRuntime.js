...
async function resolveExecutionTemplateSnapshot({ organizationId, plan }) {
  const reference = String(plan.execution_template_id || "").trim();
  ...
  if (!template) {
    const error = new Error(
      `Execution protocol ${reference} could not be resolved for this organization.`
    );
    error.status = 409;
    throw error;
  }

  return createServiceExecutionTemplateSnapshot(template);
}

function occurrenceAttributes(plan, occurrenceAt, executionTemplateSnapshot = null) {
  return {
    service_delivery: {
      schema_version: 2,
      ...
      execution_template_id:
        executionTemplateSnapshot?.template_id || plan.execution_template_id || null,
      execution_template_version: executionTemplateSnapshot?.version || null,
      execution_template_snapshot: executionTemplateSnapshot,
...
function workOrderPayload(
  ...
  occurrence,
  generationKey,
  executionTemplateSnapshot = null
) {
...
export async function generateNextServiceVisit({ context, planId }) {
  ...
  const executionTemplateSnapshot = await resolveExecutionTemplateSnapshot({
    organizationId: runtimeContext.organization_id,
    plan,
  });
