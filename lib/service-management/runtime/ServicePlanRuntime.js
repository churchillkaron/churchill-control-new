...
export async function setServicePlanStatus({ context, planId, status }) {
  ...
  if (!["active", "paused"].includes(normalizedStatus)) {
    const error = new Error("Service plan status endpoint supports only active or paused.");
    error.status = 400;
    throw error;
  }
  ...
  if (!["active", "paused"].includes(plan.status)) {
    const error = new Error(`Service plan cannot transition from ${plan.status} through recurring pause/resume.`);
    error.status = 409;
    throw error;
  }

  if (plan.status === normalizedStatus) return plan;
