...
export async function generateNextServiceVisit({ context, planId }) {
  ...
  if (!serviceOccurrenceWithinContract(occurrenceAt, plan.contract_end)) {
    const completed = await updateServicePlanState({ organizationId: runtimeContext.organization_id, planId: plan.id, actorId: runtimeContext.actor_id || null, values: { status: "completed" } });
  ...
  let occurrence = await getOrCreateServiceOccurrence({ organizationId: runtimeContext.organization_id, entityId: plan.entity_id || runtimeContext.entity_id || null, servicePlanId: plan.id, occurrenceAt, generationKey, attributes: occurrenceAttributes(plan, occurrenceAt, protocol) });
  ...
  if (occurrence.status === "generated" && occurrence.work_order_id) {
    ...
    const assignment = workOrder ? await applyPreferredAssignment({ runtimeContext, plan, occurrence, workOrder }) : { assigned: false, reason: "work-order-not-found", record: null };
    const reconciledPlan = await advancePlanAfterGeneratedOccurrence({ runtimeContext, plan, occurrenceAt, workOrderId: occurrence.work_order_id });
    return { generated: false, idempotent_replay: true, recovered_plan_cursor: true, plan: reconciledPlan, occurrence, assignment };
  }

  const operationsResponse = await serverOperationsApi.execute({ capabilityId: "work-orders", command: "create", context: runtimeContext, payload: workOrderPayload(plan, occurrence, generationKey, protocol) });
  if (operationsResponse.status >= 400 || !operationsResponse.body?.ok) {
    occurrence = await updateServiceOccurrence({ organizationId: runtimeContext.organization_id, occurrenceId: occurrence.id, values: { status: "failed", attributes: { ...(occurrence.attributes || {}), generation_error: operationsResponse.body?.error || "Operations work-order generation failed." } } });
    const error = new Error(operationsResponse.body?.error || "Operations work-order generation failed."); error.status = operationsResponse.status || 500; throw error;
  }

  const workOrder = operationsResponse.body.execution?.result || null;
  const workOrderId = workOrder?.id || null;
  if (!workOrderId) { const error = new Error("Operations work-order generation returned no work_order_id."); error.status = 500; throw error; }

  const assignment = await applyPreferredAssignment({ runtimeContext, plan, occurrence, workOrder });
  occurrence = await updateServiceOccurrence({ organizationId: runtimeContext.organization_id, occurrenceId: occurrence.id, values: { status: "generated", work_order_id: workOrderId, generated_at: new Date().toISOString(), attributes: { ...(occurrence.attributes || {}), assignment: { preferred_staff_id: preferredStaff(plan).id, assigned: Boolean(assignment.assigned), reason: assignment.reason || null, assigned_to: assignment.record?.assigned_to || null } } } });
  const updatedPlan = await advancePlanAfterGeneratedOccurrence({ runtimeContext, plan, occurrenceAt, workOrderId });
  return { generated: true, idempotent_replay: Boolean(operationsResponse.body.execution?.idempotent_replay), plan: updatedPlan, occurrence, work_order: assignment.record || workOrder, assignment };
}

export async function setServicePlanStatus({ context, planId, status }) {
  const runtimeContext = requireContext(context);
  const allowed = new Set(["active", "paused", "cancelled", "archived"]);
  if (!allowed.has(status)) { const error = new Error("Unsupported service plan status."); error.status = 400; throw error; }
  const plan = await getServicePlan({ organizationId: runtimeContext.organization_id, planId });
  if (!plan) { const error = new Error("Service plan not found."); error.status = 404; throw error; }
  return updateServicePlanState({ organizationId: runtimeContext.organization_id, planId, actorId: runtimeContext.actor_id || null, values: { status } });
}

export default Object.freeze({ createServicePlan, getServicePlans, getServicePlanOccurrences, generateNextServiceVisit, setServicePlanStatus });
