...
function evidenceAttachments(report = {}) {
  ...
  const attachments = [];
  const before = Array.isArray(evidence.before_photos) ? evidence.before_photos : [];
  const after = Array.isArray(evidence.after_photos) ? evidence.after_photos : [];

  before.forEach((item, index) => {
    const attachment = evidenceAttachment(item, `Before photo ${index + 1}`, "before_photo");
    if (attachment) attachments.push(attachment);
  });
  after.forEach((item, index) => {
    const attachment = evidenceAttachment(item, `After photo ${index + 1}`, "after_photo");
    if (attachment) attachments.push(attachment);
  });
...
export function createServiceReportDeliveryDraft(report = {}) {
  ...
  if (protocol.length) {
    lines.push("", "Service record:", ...protocol);
  }

  if (materials.length) {
    lines.push("", "Materials used:", ...materials);
  }
