...
import { deliverSupplierConnectionRequestEmail } from "@/lib/supplier-network/SupplierNetworkNotificationRuntime";
...
export async function searchSupplierNetwork({ query = "", limit = 60 } = {}) {
  ...
  const { data: accounts, error: accountError } = await supabaseAdmin
    ...
    .select("id,business_name,display_name,website,logo_url,shop_verified,business_organization_id,supplier_categories,service_areas")
  ...
  const { data: products, error: productError } = await supabaseAdmin
    ...
    .select("id,storefront_id,sku,name,description,category,uom,base_price,currency_code,minimum_order_quantity,lead_time_days,image_url")
  ...
  return (storefronts || [])
    .map((storefront) => {
      ...
      return {
        ...
        supplier: {
          ...
          categories: account.supplier_categories || [],
          service_areas: account.service_areas || [],
        },
        products: items,
      };
    })
    .filter((row) => {
      ...
      const haystack = [
        ...
        ...(row.supplier?.categories || []),
        ...(row.supplier?.service_areas || []),
...
export async function requestSupplierConnection({ organizationId, storefrontId, userId, buyerNote = "" }) {
  ...
  const { data: account, error: accountError } = await supabaseAdmin
    .from("supplier_portal_accounts")
    .select("id,shop_discoverable,email,business_name,display_name")
  ...
  let notification = { sent: false, status: "SKIPPED", reason: "SUPPLIER_EMAIL_MISSING" };
  if (account.email) {
    try {
      const { data: buyerOrganization } = await supabaseAdmin
        ...
        .eq("id", orgId)
        .maybeSingle();
      notification = await deliverSupplierConnectionRequestEmail({
        organizationId: orgId,
        recipient: account.email,
        buyerName: buyerOrganization?.name || buyerOrganization?.legal_name || "An Avantiqo business",
        supplierName: account.business_name || account.display_name || "your supplier business",
        buyerNote,
      });
      ...
      notification = {
        ...
        reason: String(notificationError?.message || notificationError || "SUPPLIER_CONNECTION_NOTIFICATION_FAILED").slice(0, 500),
      };
    }
  }

  return { success: true, connected: false, request, notification };
}
...
export async function getPublicSupplierShop(slug) {
  ...
  const [{ data: account, error: accountError }, { data: products, error: productError }] = await Promise.all([
    supabaseAdmin
      .from("supplier_portal_accounts")
      .select("id,business_name,display_name,website,logo_url,shop_verified,business_organization_id,shop_discoverable,supplier_categories,service_areas")
      .eq("id", storefront.supplier_account_id)
      .maybeSingle(),
    supabaseAdmin
      .from("supplier_storefront_products")
      .select("id,sku,name,description,category,uom,base_price,currency_code,minimum_order_quantity,lead_time_days,image_url")
      .eq("storefront_id", storefront.id)
      .eq("is_active", true)
      .order("sort_order", { ascending: true })
