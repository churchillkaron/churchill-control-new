...
export async function execute({
  ...
  const missionBindingEnabled =
    capabilityKey(domain, capability, action) === OPERATOR_MISSION_KEY &&
    hasMissionBindings(payload);
  ...
  try {
    ...
    if (capabilityKey(domain, capability, action) === OPERATOR_MISSION_KEY) {
    ...
    await observeOperatorMissionBindingResult({
      metadata: normalizedMetadata,
      mode: operatorMode(loaded.manifest || {}, capability, action),
      result,
    });
