...
import { processOperationsEvidenceEvents } from "@/lib/workers/operations/processOperationsEvidenceEvents";
...
export async function runEventProcessors({
  ...
  for (const event of events) {
    try {
      ...
      const runOperations = EVENT_SUBSCRIPTIONS.OPERATIONS.includes(type);
      ...
      if (runOperations) {
        assertProcessorSuccess(
          await processOperationsEvidenceEvents([event]),
          "Operations evidence processor"
        );
      }
