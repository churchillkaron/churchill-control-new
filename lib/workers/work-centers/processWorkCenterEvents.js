...
function getKitchenItemId(item) {
...
async function appendCanonicalKitchenTicket({
  organizationId,
  orderId,
  sessionId,
  tableId,
  ...
  const incomingItems = items.map(toKitchenItem);
  ...
  if (!existingTicket) {
    ...
    ticket.station = items.find((item) => item.station)?.station || null;
  ...
  const currentItemIds = new Set(
    currentItems.map(getKitchenItemId).filter(Boolean)
  );
  const newItems = incomingItems.filter(
    (item) => !currentItemIds.has(getKitchenItemId(item))
  );
  ...
  try {
    ...
    for (const event of pendingEvents) {
      ...
      for (const [workCenterId, groupItems] of Object.entries(groupedItems)) {
        const result = await appendCanonicalKitchenTicket({
          organizationId,
          orderId,
          sessionId,
          tableId,
