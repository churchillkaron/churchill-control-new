...
function toFulfillmentItem(item) {
  return {
    ...
    modifiers: item.modifiers || item.transaction_configuration_selections || null,
    status: "NEW",
...
async function loadWorkCenter({ organizationId, workCenterId }) {
  ...
  if (error) throw error;
  if (!data) throw new Error(`Active work center not found: ${workCenterId}`);
  return data;
}
...
async function appendCanonicalTicket({
  ...
  if (!existingTicket) {
    ...
    await target.createTicket({ document: ticket });
...
export async function processWorkCenterEvents(events = null) {
  try {
    ...
    if (!pendingEvents.length) return { success: true, processed: 0 };
    ...
    for (const event of pendingEvents) {
      ...
      const entityId = orderResult.data.entity_id || event.payload?.entity_id || null;
      if (!entityId) {
        throw new Error(`Restaurant order ${orderId} has no legal entity scope`);
      }
      ...
      const dispatchableItems = (items || []).filter(
        (item) => !TERMINAL_ORDER_ITEM_STATUSES.has(normalizedStatus(item?.status))
      );

      if (!dispatchableItems.length) {
        if (ownsEventLifecycle) await markEventProcessed(event.id);
        continue;
      }
      ...
      for (const item of dispatchableItems) {
        ...
        routedItems.push({ workCenterId, item });
      }
