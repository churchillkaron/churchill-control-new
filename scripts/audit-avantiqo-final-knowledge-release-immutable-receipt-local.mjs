...
} from "../lib/intelligence/runtime/AvantiqoFinalKnowledgeReleaseReceiptAtomicBindingRuntime.js";
...
const provisional = {
  ...
  memory_type: "fact", subject: "knowledge:test",
  content: "A governed synthetic claim used only to certify immutable release receipt lineage.",
  active: true, valid_until: null, updated_at: "2026-09-05T07:58:00.000Z",
  metadata: {
...
const candidate = {
  ...
  memory_type: "decision", subject: "candidate:test", content: "Synthetic candidate.",
  active: true, updated_at: "2026-09-05T07:59:00.000Z",
  metadata: {
    hypothesis_fingerprint: hypothesis,
    final_promotion_candidate_authenticity_contract: AVANTIQO_FINAL_PROMOTION_CANDIDATE_AUTHENTICITY_CONTRACT,
...
const authorization = {
  ...
  memory_type: "decision", subject: "authorization:test", content: "Synthetic authorization.",
  active: true, valid_until: "2026-09-05T09:00:00.000Z", updated_at: "2026-09-05T07:59:30.000Z",
  metadata: {
    contract: AVANTIQO_FINAL_KNOWLEDGE_RELEASE_AUTHORIZATION_CONTRACT,
    status: "READY", authorization_id: authorizationId, hypothesis_fingerprint: hypothesis,
    candidate_memory_key: candidate.memory_key, candidate_authenticity_mac: candidateMac,
    provisional_claim_memory_key: provisional.memory_key,
    provisional_claim_digest: claimBinding.binding.provisional_claim_digest,
    approver_id: `staff:${randomUUID()}`,
    approver_staff_account_id: randomUUID(), approver_auth_user_id: randomUUID(),
    approver_role_at_issue: "OWNER", authority_function: "public.can_manage_organization(uuid)",
    authority_verified: true, authority_verified_at: "2026-09-05T07:59:29.000Z",
...
const releaseRow = {
  id: releaseRowId, organization_id: organizationId,
  party_id: null, entity_id: null, conversation_id: null, source_turn_id: null,
  memory_scope: "platform_knowledge",
  memory_key: `released-knowledge:${hypothesis.slice(0, 40)}`,
  memory_type: "fact", subject: "knowledge:test", content: provisional.content,
  importance: 0.96, confidence: 0.91,
  source: "avantiqo_explicit_final_knowledge_release", active: true,
  valid_until: "2026-12-04T08:00:00.000Z",
  superseded_by: null, superseded_at: null, forgotten_at: null, updated_at: nowIso,
  metadata: {
    contract: "AVANTIQO_FINAL_KNOWLEDGE_RELEASE_V1", release_id: releaseId,
    release_status: "RELEASED_MONITORED", hypothesis_fingerprint: hypothesis,
    ...
    reusable_platform_knowledge: true, knowledge_router_reuse_allowed: true,
  },
};

const binding = createAvantiqoFinalKnowledgeReleaseBindingDigest(releaseRow);
assert.equal(binding.success, true);
const lifecycleResealed = clone(releaseRow);
lifecycleResealed.metadata.released_knowledge_authenticity_mac = "f".repeat(64);
assert.equal(createAvantiqoFinalKnowledgeReleaseBindingDigest(lifecycleResealed).digest, binding.digest);

const env = signerEnv();
const draft = createAvantiqoFinalKnowledgeReleaseReceiptDraft({
  organization_id: organizationId, authorization, candidate, provisional, release_row: releaseRow,
  consumption_memory_key: `final-knowledge-release-authorization-consumed:${authorizationId.slice(0, 40)}`,
  consumption_row_id: consumptionRowId, release_row_id: releaseRowId, receipt_row_id: receiptRowId,
  transaction_id: transactionId, committed_at: nowIso,
});
...
const verifier = createAvantiqoFinalKnowledgeReleaseReceiptVerifier({ env: {
  AVANTIQO_KNOWLEDGE_RELEASE_RECEIPT_PUBLIC_KEYRING_JSON: env.AVANTIQO_KNOWLEDGE_RELEASE_RECEIPT_PUBLIC_KEYRING_JSON,
} });
...
assert.equal(verifyAvantiqoFinalKnowledgeReleaseReceiptLineage(sealed.row, { authorization, candidate, provisional, releaseRow }), true);
...
for (const mutate of [
  ...
  const copy = clone(sealed.row); mutate(copy); assert.equal(verifier.verify(copy), false);
}
...
for (const mutate of [
  ...
  const copy = clone(releaseRow); mutate(copy);
  assert.equal(verifyAvantiqoFinalKnowledgeReleaseReceiptBinding(sealed.row, copy), false);
}

const atomicRuntime = fs.readFileSync("lib/intelligence/runtime/AvantiqoFinalKnowledgeReleaseAtomicCommitRuntime.js", "utf8");
const releaseRuntime = fs.readFileSync("lib/intelligence/runtime/AvantiqoFinalKnowledgeReleaseRuntime.js", "utf8");
const retrievalRuntime = fs.readFileSync("lib/intelligence/runtime/AvantiqoHybridKnowledgeRetrievalRuntime.js", "utf8");
const migration = fs.readFileSync("supabase/migrations/20260905065000_atomic_final_knowledge_release.sql", "utf8");
assert.match(atomicRuntime, /AvantiqoFinalKnowledgeReleaseReceiptAtomicBindingRuntime/);
...
assert.match(releaseRuntime, /immutable_release_receipt_persisted: true/);
