...
function collectRequestObjects(value, output = [], seen = new Set(), depth = 0) {
  ...
  if (id && (value.status || value.state || value.input || value.request || value.payload)) output.push(value);
  for (const child of Object.values(value)) collectRequestObjects(child, output, seen, depth + 1);
  return output;
}
...
async function requestJson(url, options = {}) {
  ...
  if (!response.ok) throw new Error(`RUNPOD_DIAGNOSTIC_REQUEST_CLEANUP_FAILED:${response.status}:${text(raw).slice(0, 500)}`);
  return body;
}
...
for (const request of requests) {
  const status = statusOf(request) || "UNKNOWN";
  statusCounts[status] = Number(statusCounts[status] || 0) + 1;
}
const candidates = requests.filter((request) => !TERMINAL.has(statusOf(request)));
console.log(`AVANTIQO_DIAGNOSTIC_REQUEST_SCAN request_objects=${requests.length} nonterminal=${candidates.length} statuses=${JSON.stringify(statusCounts)}`);

let cancelled = 0;
for (const request of candidates) {
  const id = text(request.id || request.jobId || request.job_id || request.requestId || request.request_id);
  if (!id) continue;
  const result = await requestJson(`${RUNPOD_API_BASE}/${endpointId}/cancel/${encodeURIComponent(id)}`, { method: "POST" });
  console.log(`AVANTIQO_DIAGNOSTIC_REQUEST_CANCELLED state=${statusOf(result) || "UNKNOWN"}`);
  cancelled += 1;
}
