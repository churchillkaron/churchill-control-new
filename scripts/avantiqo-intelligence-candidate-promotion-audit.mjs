...
assert(inspector.includes('EXPECTED_TARGET_MODULES = {"q_proj", "v_proj"}'), "CANDIDATE_DENSE_TARGET_MODULES_REQUIRED");
assert(inspector.includes("EXPECTED_TARGET_PARAMETERS"), "CANDIDATE_TARGET_PARAMETERS_REQUIRED");
assert(inspector.includes('"mlp.experts.gate_up_proj"'), "CANDIDATE_GATE_UP_TARGET_REQUIRED");
assert(inspector.includes('"mlp.experts.down_proj"'), "CANDIDATE_DOWN_TARGET_REQUIRED");
assert(inspector.includes("ADAPTER_QWEN3_MOE_TARGET_MODULES_MISMATCH"), "CANDIDATE_TARGET_MODULE_GUARD_REQUIRED");
assert(inspector.includes("ADAPTER_QWEN3_MOE_TARGET_PARAMETERS_MISMATCH"), "CANDIDATE_TARGET_PARAMETER_GUARD_REQUIRED");
assert(inspector.includes("ADAPTER_QWEN3_MOE_RANK_PATTERN_REQUIRED"), "CANDIDATE_RANK_PATTERN_REQUIRED");
assert(inspector.includes("ADAPTER_QWEN3_MOE_LORA_DROPOUT_MUST_BE_ZERO"), "CANDIDATE_ZERO_DROPOUT_REQUIRED");
assert(inspector.includes("handle.get_slice(key).get_shape()"), "CANDIDATE_TENSOR_SHAPE_INSPECTION_REQUIRED");
assert(inspector.includes("FUSED_GATE_UP_KEY"), "CANDIDATE_FUSED_GATE_UP_KEY_REQUIRED");
assert(inspector.includes("FUSED_DOWN_KEY"), "CANDIDATE_FUSED_DOWN_KEY_REQUIRED");
assert(inspector.includes("PER_EXPERT_KEY"), "CANDIDATE_PER_EXPERT_DETECTION_REQUIRED");
assert(inspector.includes("ADAPTER_MOE_PER_EXPERT_LAYOUT_FORBIDDEN"), "CANDIDATE_PER_EXPERT_LAYOUT_FORBIDDEN");
assert(inspector.includes("ADAPTER_MOE_FUSED_FACTOR_NOT_2D"), "CANDIDATE_PEFT_SERIALIZED_FACTOR_2D_REQUIRED");
assert(inspector.includes("ADAPTER_MOE_FUSED_LAYER_INCOMPLETE"), "CANDIDATE_FUSED_AB_COVERAGE_REQUIRED");
assert(inspector.includes('"layout": "MOE_3D_FUSED_PEFT"'), "CANDIDATE_VLLM_3D_FORMAT_REQUIRED");
assert(inspector.includes('"serialization": "PEFT_FUSED_EXPERT_FACTORS_2D"'), "CANDIDATE_PEFT_SERIALIZATION_REQUIRED");
assert(inspector.includes('"is_3d_lora_weight": True'), "CANDIDATE_VLLM_3D_DECLARATION_REQUIRED");
assert(inspector.includes('"per_expert_layout_forbidden": True'), "CANDIDATE_PER_EXPERT_GOVERNANCE_REQUIRED");
...
console.log("AVANTIQO_INTELLIGENCE_CANDIDATE_MOE_LAYOUT=VLLM_3D_FORMAT_FUSED_PEFT");
console.log("AVANTIQO_INTELLIGENCE_CANDIDATE_SERIALIZED_FACTORS=2D_STACKED");
console.log("AVANTIQO_INTELLIGENCE_CANDIDATE_TARGETS=VERIFIED");
