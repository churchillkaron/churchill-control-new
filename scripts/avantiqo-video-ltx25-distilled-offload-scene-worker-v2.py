...
QUALITY_CONTRACT = "AVANTIQO_VIDEO_LTX25_DISTILLED_TWO_STAGE_OFFLOAD_V2"
...
def cinematic_instruction(data: dict[str, Any]) -> str:
    ...
    return f"{base}\n\nGOVERNED CINEMATIC CONTROL:\n{json.dumps(bounded, separators=(',', ':'), ensure_ascii=True)}"
...
def install_torch27_compat(tmp: Path) -> Path:
    compat = tmp / "torch27-compat"
...
def run_scene(job: dict[str, Any], tmp: Path) -> dict[str, Any]:
    ...
    width, height = native_4k_dimensions(text(data.get("aspect_ratio") or "16:9"))
    frames = frame_count(int(data.get("duration_seconds") or 5), fps)
    ...
    output = tmp / "ltx25-scene1-distilled-offload.mp4"
    command = [
        "python", "-m", "ltx_pipelines.distilled",
        "--transformer-path", required_file(TRANSFORMER, "AVANTIQO_VIDEO_LTX25_BF16_MODEL_REQUIRED"),
    ...
    compat = install_torch27_compat(tmp)
    env = os.environ.copy()
    env["PYTHONPATH"] = ":".join([
        str(compat),
    ...
    completed = subprocess.run(
        ...
        text=True,
        timeout=int(os.getenv("AVANTIQO_VIDEO_LTX25_HARD_TIMEOUT_SECONDS", "3600")),
        check=False,
    )
    if completed.returncode != 0:
        raise RuntimeError(f"AVANTIQO_VIDEO_LTX25_DISTILLED_OFFLOAD_FAILED:{completed.returncode}:{sanitize_detail(completed.stdout)}")
    if not output.is_file() or output.stat().st_size <= 0:
        raise RuntimeError("AVANTIQO_VIDEO_LTX25_OUTPUT_MISSING")
    upload_file(output, output_signed_url)
    return {
        "status": "completed",
        ...
        "pipeline": "DISTILLED_TWO_STAGE_CPU_OFFLOAD",
        "quality_lane": "hero",
        "precision": "BF16_CPU_OFFLOAD",
        "seed": seed,
        "fps": fps,
        "frame_count": frames,
        "width": width,
...
def main() -> None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="avantiqo-ltx25-offload-v2-") as tmp_dir:
    ...
    response.raise_for_status()
    print(f"AVANTIQO_VIDEO_LTX25_OFFLOAD_V2_RECEIPT_WRITTEN={str(receipt.get('success') is True).lower()}", flush=True)
