...
globalThis.fetch = async (input, init) => {
  ...
  while (true) {
    ...
    if (!(await isEndpointPaused(response))) {
      if (retries > 0) {
        console.log(`AVANTIQO_IMAGE_BENCHMARK_ENDPOINT_READY=true retries=${retries}`);
      }
      return response;
    }
    ...
    if (Date.now() >= deadline) {
      console.log(`AVANTIQO_IMAGE_BENCHMARK_ENDPOINT_READY_TIMEOUT=true retries=${retries}`);
      return response;
    }
    if (retries === 1 || retries % 10 === 0) {
      console.log(`AVANTIQO_IMAGE_BENCHMARK_ENDPOINT_UNPAUSE_WAIT=true retries=${retries}`);
    }
    await sleep(UNPAUSE_POLL_MS);
  }
};

console.log("AVANTIQO_IMAGE_BENCHMARK_ENDPOINT_READY_GUARD=true");
console.log(`AVANTIQO_IMAGE_BENCHMARK_ENDPOINT_READY_WAIT_MS=${UNPAUSE_WAIT_MS}`);
console.log("AVANTIQO_IMAGE_BENCHMARK_ENDPOINT_READY_RETRY_ONLY_ON_409_PAUSED=true");
console.log("AVANTIQO_IMAGE_BENCHMARK_DUPLICATE_JOB_RETRY=false");

await import("./benchmark-avantiqo-image.mjs");
