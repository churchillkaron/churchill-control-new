...
const BENCHMARK_CONTRACT = "AVANTIQO_IMAGE_Z_IMAGE_V8_DEFAULT_QUALITY_CERTIFICATE_V1";
...
const MAX_WAIT_MS = Math.max(
  60_000,
  Math.min(20 * 60 * 1000, Number(process.env.AVANTIQO_IMAGE_Z_V8_DEFAULT_TIMEOUT_MS || 12 * 60 * 1000)),
);
const DEFAULT_REPORT = "/tmp/avantiqo-z-image-v8-default-quality.json";
const DEFAULT_IMAGE = "/tmp/avantiqo-z-image-v8-default-quality.png";
...
async function rest(pathname, key) {
  ...
  }), "AVANTIQO_IMAGE_Z_V8_DEFAULT_REST");
}
async function queue(endpointId, pathname, key, options = {}) {
  ...
  }), "AVANTIQO_IMAGE_Z_V8_DEFAULT_QUEUE");
}
...
async function selectQueueCredential(endpointId, managementKey) {
  ...
  throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_QUEUE_CREDENTIAL_NOT_FOUND");
}
async function readEvidence() {
  ...
  ) throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_EVIDENCE_INVALID");
  ...
  if (!/^ghcr\.io\/churchillkaron\/avantiqo-image-worker@sha256:[a-f0-9]{64}$/i.test(immutableImage)) {
    throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_IMMUTABLE_IMAGE_INVALID");
  }
...
async function endpointBoundTemplates(managementKey) {
  ...
  if (!templates) throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_TEMPLATE_LIST_INVALID");
  return templates;
}
function resolveEndpoint(endpoints, configuredId) {
  ...
  if (matches.length !== 1) throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_ENDPOINT_RESOLUTION_FAILED:${matches.length}`);
  return matches[0];
}
function resolveTemplate(templates, templateId) {
  const matches = templates.filter((entry) => text(entry?.id) === templateId);
  if (matches.length !== 1) throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_TEMPLATE_RESOLUTION_FAILED:${matches.length}`);
  return matches[0];
}
async function cancelJob(endpointId, jobId, apiKey) {
  try {
    ...
    console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_CANCELLED_JOB=${jobId}`);
  } catch (error) {
    console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_CANCEL_FAILED=${text(error?.message).slice(0, 300)}`);
  }
}

const nodeMajor = Number(process.versions.node.split(".")[0]);
if (!Number.isFinite(nodeMajor) || nodeMajor < 24) {
  throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_NODE24_REQUIRED:actual=${process.version}`);
}
if (!process.argv.includes("--apply") || !yes(process.env.AVANTIQO_IMAGE_Z_V8_DEFAULT_APPROVED)) {
  throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_APPROVED=YES_AND_--apply_REQUIRED");
}
...
const reportPath = resolve(process.env.AVANTIQO_IMAGE_Z_V8_DEFAULT_REPORT || DEFAULT_REPORT);
const localImagePath = resolve(process.env.AVANTIQO_IMAGE_Z_V8_DEFAULT_LOCAL_OUTPUT || DEFAULT_IMAGE);
const instruction = text(process.env.AVANTIQO_IMAGE_Z_V8_DEFAULT_INSTRUCTION) || DEFAULT_INSTRUCTION;
...
if (!endpoints) throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_ENDPOINT_LIST_INVALID");
const endpoint = resolveEndpoint(endpoints, endpointId);
if (finite(endpoint.workersMin, -1) !== 0 || finite(endpoint.workersMax, -1) !== 1) {
  throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_SCALING_INVALID");
}
...
if (text(template.imageName) !== immutableImage || !text(template.name).startsWith("avantiqo-image-immutable-v8-")) {
  throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_TEMPLATE_NOT_V8_IMMUTABLE");
}
...
) throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_EXISTING_ACTIVITY_BLOCK");

const supabase = createClient(supabaseUrl, serviceRoleKey, {
  auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
});
const runId = new Date().toISOString().replace(/[^0-9]/g, "").slice(0, 14);
const remotePath = `platform-certification/owned-media-local/${runId}/outputs/z-image-v8-default-quality.png`;
const { data: uploadData, error: uploadError } = await supabase.storage
  .from(BUCKET)
  .createSignedUploadUrl(remotePath, { upsert: true });
if (uploadError || !uploadData?.signedUrl) {
  throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_UPLOAD_TARGET_FAILED:${uploadError?.message || "NO_SIGNED_URL"}`);
}
const storageReference = `storage://${BUCKET}/${remotePath}`;

console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_NODE=${process.version}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_CONTRACT=${BENCHMARK_CONTRACT}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_ENDPOINT_ID=${endpointId}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_TEMPLATE_ID=${templateId}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_IMMUTABLE_IMAGE=${immutableImage}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_FOUNDATION=${FOUNDATION_MODEL}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_DIMENSIONS=${EXPECTED_WIDTH}x${EXPECTED_HEIGHT}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_SEED=${SEED}`);
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_REQUEST_INFERENCE_STEPS_OVERRIDE=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_REQUEST_GUIDANCE_SCALE_OVERRIDE=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_REQUEST_NEGATIVE_PROMPT_OVERRIDE=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_SINGLE_SUBMISSION=true");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_AUTOMATIC_RETRY=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_ENDPOINT_MUTATION=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_PRODUCTION_DEPLOY=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_PRICING_ACTIVATION=false");
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_SECRETS_PRINTED=false");
...
try {
  submitted = await queue(endpointId, "/run", queueCredential.key, {
    ...
    body: {
      input: {
        ...
        usage_id: `benchmark-z-image-v8-default-${runId}`,
        instruction,
  ...
  throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_SUBMIT_RESULT_UNKNOWN_DO_NOT_RETRY:${text(error?.message).slice(0, 800)}`);
}

const jobId = text(submitted?.id);
if (!jobId) throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_JOB_ID_MISSING_DO_NOT_RETRY");
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_JOB_ID=${jobId}`);
console.log("AVANTIQO_IMAGE_Z_V8_DEFAULT_SUBMITTED_ONCE=YES");
...
while (Date.now() - startedAt < MAX_WAIT_MS) {
  ...
  if (status !== lastStatus) {
    console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_STATUS=${status || "UNKNOWN"}`);
    lastStatus = status;
  }
  if (status === "COMPLETED") break;
  if (terminalFailure(status)) {
    throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_JOB_FAILED:job_id=${jobId}:status=${status}:error=${text(body?.error).slice(0, 1000)}`);
  }
  await sleep(POLL_MS);
...
if (text(body?.status).toUpperCase() !== "COMPLETED") {
  ...
  throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_TIMEOUT_CANCELLED:job_id=${jobId}`);
}
...
const checks = {
  ...
  antitext_policy_contract: text(guidance.antitext_policy_contract) === EXPECTED_ANTITEXT,
  negative_policy_applied: guidance.negative_policy_applied === true,
  antitext_policy_applied: guidance.antitext_policy_applied === true,
...
if (previewError || !previewData?.signedUrl) {
  throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_PREVIEW_FAILED:${previewError?.message || "NO_SIGNED_URL"}`);
}
const imageResponse = await fetch(previewData.signedUrl, { signal: AbortSignal.timeout(60_000) });
if (!imageResponse.ok) throw new Error(`AVANTIQO_IMAGE_Z_V8_DEFAULT_DOWNLOAD_HTTP_${imageResponse.status}`);
const imageBytes = Buffer.from(await imageResponse.arrayBuffer());
if (imageBytes.length < 10_000) throw new Error("AVANTIQO_IMAGE_Z_V8_DEFAULT_IMAGE_TOO_SMALL");
...
const report = {
  ...
  next_action: passed ? "HUMAN_REVIEW_FINAL_V8_DEFAULT_QUALITY_IMAGE" : "STOP_AND_INSPECT_V8_DEFAULT_QUALITY_CONTRACT",
};

await writeFile(reportPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_PREVIEW_URL=${previewData.signedUrl}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_LOCAL_IMAGE=${localImagePath}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_REPORT=${reportPath}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_CHECKS=${JSON.stringify(checks)}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_RESULT=${passed ? "PASS" : "FAIL"}`);
console.log(`AVANTIQO_IMAGE_Z_V8_DEFAULT_NEXT_ACTION=${report.next_action}`);
