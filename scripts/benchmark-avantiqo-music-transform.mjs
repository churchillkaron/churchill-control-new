...
if (!Number.isFinite(SPEND_CEILING_THB) || SPEND_CEILING_THB <= 0) throw new Error("AVANTIQO_MUSIC_TRANSFORM_SPEND_CEILING_THB_REQUIRED");
...
while (!output) {
  ...
  if (conservativeCostThb >= SPEND_CEILING_THB * 0.98) { try { await call.cancel({ terminateContainers:true }); } catch {} throw new Error(`AVANTIQO_MUSIC_TRANSFORM_SPEND_CEILING_WATCHDOG:${conservativeCostThb.toFixed(6)}THB`); }
  try { output=await call.get({timeoutMs:5000}); } catch(error) { if (!String(error?.name||error?.code||error?.message||"").toUpperCase().includes("TIMEOUT")) throw new Error(`AVANTIQO_MUSIC_TRANSFORM_MODAL_JOB_FAILED:${text(error?.message||error)}`); }
}
...
if (conservativeSupplierCostThb > SPEND_CEILING_THB) throw new Error("AVANTIQO_MUSIC_TRANSFORM_SPEND_CEILING_EXCEEDED");
