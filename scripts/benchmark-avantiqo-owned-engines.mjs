...
function evidencePassed(engine, evidence) {
  if (!evidence || typeof evidence !== "object") return false;
  if (engine.id === "voice") {
    return evidence?.summary?.passed === true && evidence?.tts?.summary?.passed === true && evidence?.stt?.summary?.passed === true && evidence?.stt?.skipped !== true;
  }
  return evidence?.summary?.passed === true;
}

const engines = [
  { id:"intelligence", provider:"avantiqo-intelligence", models:["Qwen/Qwen3-30B-A3B-Thinking-2507"], capabilities:["ai.reasoning.execute","ai.text.generate"], endpoint_env:["RUNPOD_AVANTIQO_INTELLIGENCE_ENDPOINT_ID"], source_ready:true, benchmark_script:"scripts/benchmark-avantiqo-intelligence.mjs", benchmark_output:"/tmp/avantiqo-intelligence-certification-benchmark.json", output_env:"AVANTIQO_INTELLIGENCE_CERTIFICATION_OUTPUT", isolation_contract:"AVANTIQO_INTELLIGENCE_NON_QUEUE_CERTIFICATION_V1" },
  ...
  { id:"voice", provider:"avantiqo-voice", models:["openai/whisper-large-v3-turbo","resemble-ai/chatterbox:multilingual-v3"], capabilities:["ai.speech.to.text","ai.text.to.speech"], endpoint_env:["RUNPOD_AVANTIQO_VOICE_STT_ENDPOINT_ID","RUNPOD_AVANTIQO_VOICE_TTS_ENDPOINT_ID"], source_ready:true, benchmark_script:"scripts/benchmark-avantiqo-voice.mjs", benchmark_output:"/tmp/avantiqo-voice-certification-benchmark.json", output_env:"AVANTIQO_VOICE_BENCHMARK_OUTPUT" },
