...
const CONTRACT = "AVANTIQO_CODE_RUNPOD_IMMUTABLE_IMAGE_BIND_V3";
...
const APPROVAL_ENV = "AVANTIQO_CODE_IMMUTABLE_IMAGE_BIND_V3_APPROVED";
...
function assertParked(endpoint, summary, label) {
  ...
  if (summary.jobs.in_queue !== 0 || summary.jobs.in_progress !== 0) throw new Error(`${label}_QUEUE_NOT_EMPTY`);
  if (activeManagementWorkers(endpoint).length) throw new Error(`${label}_ACTIVE_MANAGEMENT_WORKER`);
  if (Object.values(summary.workers).some((value) => Number(value) !== 0)) throw new Error(`${label}_ACTIVE_HEALTH_WORKER`);
}

const ENDPOINT_QUERY = `
query AvantiqoCodeImmutableBindV3Read {
  myself {
    endpoints {
      id
      name
...
async function graphqlEndpoint(managementKey) {
  ...
  const matches = list(body?.data?.myself?.endpoints).filter((row) => text(row?.id) === ENDPOINT_ID && text(row?.name) === ENDPOINT_NAME);
...
async function snapshot(managementKey, queueKey) {
  const [endpoint, healthRaw, gqlEndpoint, templatesRaw, endpointsRaw] = await Promise.all([
    ...
    retryRead(() => health(ENDPOINT_ID, queueKey), "HEALTH_READ"),
  ...
  const templateMatches = normalizeRows(templatesRaw, ["templates"]).filter((row) => text(row?.id) === templateId);
  ...
  return { endpoint, health: healthSummary(healthRaw), gqlEndpoint, template: templateMatches[0], consumers };
}

const apply = process.argv.includes("--apply");
if (apply && upper(process.env[APPROVAL_ENV]) !== "YES") throw new Error(`${APPROVAL_ENV}=YES_REQUIRED`);
...
const exclusive = initial.consumers.length === 1 && text(initial.consumers[0]?.id) === ENDPOINT_ID;
if (!exclusive) throw new Error(`${CONTRACT}_SHARED_TEMPLATE_BLOCKED:${initial.consumers.length}`);
...
const baseResult = {
  ...
  source_gate: {
    exact_serverless_inputs_only: SERVERLESS_IMAGE_INPUTS,
    worker_source_equivalent_to_image: true,
    unrelated_pod_runtime_changes_ignored: true,
  },
  endpoint: {
    id: ENDPOINT_ID,
    name: ENDPOINT_NAME,
  ...
  next_action: mutationRequired ? "APPLY_IMMUTABLE_IMAGE_BIND_V3" : "RUN_METADATA_RUNTIME_PROBE_SAFE_LEASE",
};

console.log(`AVANTIQO_CODE_IMMUTABLE_IMAGE_BIND_V3_MODE=${apply ? "APPLY" : "PLAN"}`);
console.log("AVANTIQO_CODE_IMMUTABLE_IMAGE_BIND_V3_SCALING_MUTATION=false");
console.log("AVANTIQO_CODE_IMMUTABLE_IMAGE_BIND_V3_GENERATION_SUBMITTED=false");
console.log("AVANTIQO_CODE_IMMUTABLE_IMAGE_BIND_V3_INFERENCE_PERFORMED=false");
...
const preservationBefore = templatePreservationKey(initial.template);
sourceGate();
...
if (templatePreservationKey(after.template) !== preservationBefore) throw new Error(`${CONTRACT}_TEMPLATE_CONTENT_CHANGED`);
...
console.log(JSON.stringify({
  ...
  mutation_response_uncertain_but_readback_succeeded: Boolean(mutationError),
  endpoint: {
    ...baseResult.endpoint,
  ...
  template: { ...baseResult.template, image_name: text(after.template.imageName) },
  health: after.health,
