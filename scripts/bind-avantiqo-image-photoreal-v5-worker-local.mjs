...
const CONTRACT = "AVANTIQO_IMAGE_PHOTOREAL_V5_IMMUTABLE_BIND_V1";
...
const EXPECTED_VOLUME_QUOTA_GB = 80;
...
const DRAIN_TIMEOUT_MS = Math.max(
  ...
  Math.min(
    10 * 60 * 1000,
    Number(process.env.AVANTIQO_IMAGE_V5_BIND_DRAIN_TIMEOUT_MS || 3 * 60 * 1000),
  ),
);
...
async function endpointBoundTemplates(key) {
  ...
  if (!Array.isArray(templates)) throw new Error("AVANTIQO_IMAGE_V5_BIND_TEMPLATE_LIST_INVALID");
  return templates;
}
function resolveTemplate(endpoint, templates) {
  ...
  if (!templateId) throw new Error("AVANTIQO_IMAGE_V5_BIND_TEMPLATE_ID_REQUIRED");
  ...
  if (matches.length !== 1) {
    throw new Error(
      `AVANTIQO_IMAGE_V5_BIND_TEMPLATE_RESOLUTION_FAILED:id=${templateId}:matches=${matches.length}`,
    );
  }
  return matches[0];
}
...
function assertNoJobsOrExecution(health) {
  if (health.jobs.in_queue || health.jobs.in_progress) {
    throw new Error(
      `AVANTIQO_IMAGE_V5_BIND_BLOCKED_JOBS:in_queue=${health.jobs.in_queue}:in_progress=${health.jobs.in_progress}`,
    );
  }
  if (health.workers.running || health.workers.throttled || health.workers.unhealthy) {
    throw new Error(
      `AVANTIQO_IMAGE_V5_BIND_BLOCKED_WORKERS:running=${health.workers.running}:throttled=${health.workers.throttled}:unhealthy=${health.workers.unhealthy}`,
    );
  }
}
...
function templateBody(template, imageName) {
  ...
  if (!body.name) throw new Error("AVANTIQO_IMAGE_V5_BIND_TEMPLATE_NAME_REQUIRED");
...
function assertPlacementPreserved(before, after, allowImageChange = false, allowWorkerMaxChange = false) {
  for (const key of [
    ...
    if (before[key] !== after[key]) {
      throw new Error(`AVANTIQO_IMAGE_V5_BIND_UNRELATED_FIELD_CHANGED:${key}`);
    }
  }
  if (!allowWorkerMaxChange && before.workers_max !== after.workers_max) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_WORKERS_MAX_CHANGED");
  }
  if (!sameSet(before.network_volume_ids, after.network_volume_ids)) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_NETWORK_VOLUME_CHANGED");
  }
  if (!sameSet(before.gpu_type_ids, after.gpu_type_ids)) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_GPU_POOL_CHANGED");
  }
  if (!sameSet(before.data_center_ids, after.data_center_ids)) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_DATACENTER_IDS_CHANGED");
  }
  if (!allowImageChange && before.template_image !== after.template_image) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_IMAGE_CHANGED_UNEXPECTEDLY");
  }
}
function validateCostGuard(endpoint) {
  const gpuPool = unique(list(endpoint.gpuTypeIds));
  if (!sameSet(gpuPool, COST_GUARDED_GPU_POOL)) {
    throw new Error(`AVANTIQO_IMAGE_V5_BIND_COST_GUARD_GPU_POOL_INVALID:${gpuPool.join("|")}`);
  }
  if (finite(endpoint.idleTimeout) !== REQUIRED_IDLE_TIMEOUT_SECONDS) {
    throw new Error(
      `AVANTIQO_IMAGE_V5_BIND_COST_GUARD_IDLE_TIMEOUT_INVALID:${finite(endpoint.idleTimeout)}`,
    );
  }
  if (finite(endpoint.workersMin) !== 0 || finite(endpoint.workersMax) !== 1) {
    throw new Error(
      `AVANTIQO_IMAGE_V5_BIND_COST_GUARD_SCALING_INVALID:min=${finite(endpoint.workersMin)}:max=${finite(endpoint.workersMax)}`,
    );
  }
}
function validateEvidence() {
  ...
  if (command("git", ["branch", "--show-current"], "GIT_BRANCH_READ_FAILED") !== "main") {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_MAIN_REQUIRED");
  }
  ...
  if (head !== origin) {
    throw new Error(`AVANTIQO_IMAGE_V5_BIND_LOCAL_MAIN_NOT_CURRENT:head=${head}:origin=${origin}`);
  }
  ...
  if (
    ...
    finite(evidence.network_volume_quota_gb) !== EXPECTED_VOLUME_QUOTA_GB ||
    ...
    throw new Error("AVANTIQO_IMAGE_V5_BIND_EVIDENCE_INVALID_OR_NOT_READY");
  }
  ...
  if (!/^ghcr\.io\/churchillkaron\/avantiqo-image-worker@sha256:[a-f0-9]{64}$/i.test(image)) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_IMAGE_REFERENCE_INVALID");
  }

  const diff = commandStatus("git", ["diff", "--quiet", sourceSha, head, "--", SOURCE_PATH]);
  if (diff.status === 1) {
    throw new Error(`AVANTIQO_IMAGE_V5_BIND_SOURCE_CHANGED_AFTER_BUILD:source=${sourceSha}:head=${head}`);
  }
  if (diff.status !== 0) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_SOURCE_EQUIVALENCE_CHECK_FAILED");
  }
  ...
  command(
    ...
    [
      ...
      `${SOURCE_PATH}/handler_v5.py`,
    ],
    "AVANTIQO_IMAGE_V5_BIND_PYTHON_SYNTAX_FAILED",
  );
...
async function waitForDrain(endpointId, managementKey, inferenceKey) {
  ...
  throw new Error(`AVANTIQO_IMAGE_V5_BIND_DRAIN_TIMEOUT:${JSON.stringify(latest)}`);
}
...
if (!Array.isArray(endpoints) || !Array.isArray(volumes)) {
  throw new Error("AVANTIQO_IMAGE_V5_BIND_INVENTORY_INVALID");
}
...
if (matches.length !== 1) {
  throw new Error(`AVANTIQO_IMAGE_V5_BIND_ENDPOINT_RESOLUTION_FAILED:matches=${matches.length}`);
}
...
if (!policy.policy_compliant || groupVolumes.length !== 1) {
  throw new Error("AVANTIQO_IMAGE_V5_BIND_SHARED_VOLUME_POLICY_INVALID");
}
const canonical = groupVolumes[0];
if (
  text(canonical.name) !== GROUP.canonical_name ||
  finite(canonical.size, 0) !== EXPECTED_VOLUME_QUOTA_GB
) {
  throw new Error(
    `AVANTIQO_IMAGE_V5_BIND_CANONICAL_VOLUME_INVALID:size=${finite(canonical.size, 0)}:expected=${EXPECTED_VOLUME_QUOTA_GB}`,
  );
}
const attached = endpointVolumeIds(state.endpoint);
if (attached.length !== 1 || attached[0] !== text(canonical.id)) {
  throw new Error(
    `AVANTIQO_IMAGE_V5_BIND_CANONICAL_VOLUME_ATTACHMENT_INVALID:${attached.join("|")}`,
  );
}
if (unique(list(state.endpoint.dataCenterIds)).length !== 0) {
  throw new Error("AVANTIQO_IMAGE_V5_BIND_DATACENTER_PINNING_FORBIDDEN");
}
...
const plan = {
  ...
  volume_quota_guard: {
    contract: VOLUME_QUOTA_GUARD_CONTRACT,
    quota_gb: EXPECTED_VOLUME_QUOTA_GB,
  ...
  next_action: mutationRequired
    ? apply
      ? "BIND_AND_RECYCLE_V5"
      : "RUN_WITH_APPLY"
    : "V5_BOUND_NO_GPU_JOB_REQUIRED",
};

console.log(`AVANTIQO_IMAGE_V5_BIND_CONTRACT=${CONTRACT}`);
console.log(`AVANTIQO_IMAGE_V5_BIND_MODE=${apply ? "APPLY" : "PLAN"}`);
console.log("AVANTIQO_IMAGE_V5_BIND_NEW_GENERATION=false");
console.log("AVANTIQO_IMAGE_V5_BIND_CACHE_OPERATION=false");
console.log("AVANTIQO_IMAGE_V5_BIND_MODEL_DOWNLOAD=false");
console.log("AVANTIQO_IMAGE_V5_BIND_PRODUCTION_DEPLOY=false");
console.log("AVANTIQO_IMAGE_V5_BIND_BACKING_FS_CAPACITY_ALLOWED=false");
console.log("AVANTIQO_IMAGE_V5_BIND_B200_ALLOWED=false");
console.log("AVANTIQO_IMAGE_V5_BIND_H100_H200_ALLOWED=false");
...
if (headBeforeWrite !== local.head || originBeforeWrite !== local.head) {
  throw new Error(
    `AVANTIQO_IMAGE_V5_BIND_MAIN_MOVED_REPLAN_REQUIRED:planned=${local.head}:head=${headBeforeWrite}:origin=${originBeforeWrite}`,
  );
}
...
try {
  ...
  assertPlacementPreserved(
    baseline,
    stableEndpoint(pausedState.endpoint, pausedState.template),
    false,
    true,
  );
  if (finite(pausedState.endpoint.workersMin) !== 0 || finite(pausedState.endpoint.workersMax) !== 0) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_PAUSE_VERIFY_FAILED");
  }
  if (finite(pausedState.endpoint.idleTimeout) !== REQUIRED_IDLE_TIMEOUT_SECONDS) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_IDLE_TIMEOUT_CHANGED_DURING_PAUSE");
  }
  if (!sameSet(unique(list(pausedState.endpoint.gpuTypeIds)), COST_GUARDED_GPU_POOL)) {
    throw new Error("AVANTIQO_IMAGE_V5_BIND_GPU_COST_GUARD_CHANGED_DURING_PAUSE");
  }
...
if (text(verified.template.imageName) !== local.image) {
  throw new Error("AVANTIQO_IMAGE_V5_BIND_FINAL_IMAGE_VERIFY_FAILED");
}
if (unique(list(verified.endpoint.dataCenterIds)).length !== 0) {
  throw new Error("AVANTIQO_IMAGE_V5_BIND_FINAL_DATACENTER_PINNING_DETECTED");
}

console.log("AVANTIQO_IMAGE_V5_BIND_COMPLETE=YES");
console.log(JSON.stringify({
  ...
  volume_quota_guard_required: true,
  ...
  next_action: "V5_BOUND_DO_NOT_CACHE_UNTIL_VOLUME_PLAN_RESOLVED",
}, null, 2));
