...
const CONTRACT = "CREATIVE_VERSIONED_DIRECTION_APPROVAL_ENVELOPE_V2";
const PREFLIGHT_CONTRACT = "CREATIVE_VERSIONED_STORY_LINEAGE_RECONCILIATION_PREFLIGHT_V1";
const RECONCILIATION_CONTRACT = "CREATIVE_STORY_LINEAGE_HISTORICAL_RECONCILIATION_PLAN_V1";
...
function sourceEvidence() {
  return SOURCE_FILES.map((file) => {
    ...
    if (!fs.existsSync(absolute)) throw new Error(`DIRECTION_WORKLOAD_SOURCE_MISSING:${file}`);
...
function assertMarkers(sources = []) {
  ...
  const markers = [
    ...
    "CREATIVE_BUSINESS_ACTION_PRODUCTION_GRAPH_V1",
  ...
  if (missing.length) throw new Error(`DIRECTION_WORKLOAD_SOURCE_MARKERS_MISSING:${missing.join(",")}`);
  return markers;
}
...
function temporalDuration(project = {}, brief = {}) {
  ...
  if (!value || value <= 0) throw new Error("CREATIVE_FULL_TEMPORAL_DURATION_REQUIRED");
  return value;
}
...
async function priceCandidate({ row, operations, PricingRuntime, currency }) {
  const tokenPriced = Number(row.input_cost_per_1m || 0) > 0 || Number(row.output_cost_per_1m || 0) > 0;
  const estimatedInputTokens = finite(row.metadata?.estimated_input_tokens_per_request);
  const blockers = [];
  ...
  if (!blockers.length) {
    for (const operation of operations) {
      ...
      priced.push({
        ...
        estimated_stage_ceiling: money(Number(pricing.customer_price) * operation.count),
      });
    }
  }
  const maximum = blockers.length
    ? null
    : money(priced.reduce((sum, item) => sum + Number(item.estimated_stage_ceiling || 0), 0));
  const perCall = blockers.length
    ? null
    : money(Math.max(...priced.map((item) => Number(item.estimated_customer_price_per_call || 0)), 0));
  const core = {
    pricing_id: row.id,
...
const reconciliationFile = readJson(process.argv[2], "HISTORICAL_RECONCILIATION_PLAN");
const preflightFile = readJson(process.argv[3], "VERSIONED_RECONCILIATION_PREFLIGHT");
const reconciliation = object(reconciliationFile.value);
const preflight = object(preflightFile.value);

if (text(reconciliation.contract) !== RECONCILIATION_CONTRACT) throw new Error("DIRECTION_ENVELOPE_RECONCILIATION_CONTRACT_INVALID");
if (text(preflight.contract) !== PREFLIGHT_CONTRACT) throw new Error("DIRECTION_ENVELOPE_PREFLIGHT_CONTRACT_INVALID");
if (preflight.authority_ready !== true) throw new Error("DIRECTION_ENVELOPE_AUTHORITY_NOT_READY");
if (text(preflight.reconciliation_plan_hash) !== text(reconciliation.plan_hash)) throw new Error("DIRECTION_ENVELOPE_PLAN_PREFLIGHT_HASH_MISMATCH");

const organizationId = text(reconciliation.organization_id);
const projectId = text(reconciliation.creative_project_id);
if (!organizationId || !projectId) throw new Error("DIRECTION_ENVELOPE_SCOPE_REQUIRED");
...
if (!project || text(project.organization_id) !== organizationId) throw new Error("DIRECTION_ENVELOPE_PROJECT_NOT_FOUND");
const briefs = await CreativeBriefRuntime.list({ organization_id: organizationId, creative_project_id: projectId });
...
if (!currency) throw new Error("DIRECTION_ENVELOPE_WALLET_CURRENCY_REQUIRED");
...
const pricingRows = await listCapabilityPricing({ capability: CAPABILITY, currency });
const uniqueRows = [...new Map(list(pricingRows).map((row) => [row.id, row])).values()];
const candidates = [];
...
const ready = candidates.filter((item) => !item.blockers.length && Number(item.maximum_customer_price) > 0);

const core = {
  organization_id: organizationId,
  creative_project_id: projectId,
  reconciliation_plan_hash: reconciliation.plan_hash,
  research_identity: preflight.current_research_authority?.research_identity || null,
  ...
  workload: workloads,
  exact_allowed_operations: workloads.maximum.operations.map((item) => item.operation),
  maximum_calls: workloads.maximum.call_count,
  runtime_source_evidence: sources.map(({ file, sha256: hash }) => ({ file, sha256: hash })),
...
const envelopeHash = sha256(core);
const blockers = [
  ...(ready.length ? [] : ["NO_APPROVAL_READY_REASONING_PRICING_CANDIDATE"]),
  ...(!project.metadata?.command_identity ? ["PROJECT_COMMAND_IDENTITY_REQUIRED"] : []),
];
const output = {
  contract: CONTRACT,
...
const outputPath = writeJson(
  process.env.CREATIVE_VERSIONED_DIRECTION_APPROVAL_ENVELOPE_V2_OUTPUT ||
    "/tmp/creative-versioned-direction-approval-envelope-v2.json",
  output,
);

console.log("============================================================");
console.log("READ-ONLY AUTONOMOUS ADVERTISING DIRECTION APPROVAL ENVELOPE");
console.log("============================================================");
...
console.log(`SCENE_COUNT_MIN=${range.minimum}`);
console.log(`SCENE_COUNT_PREFERRED=${range.preferred}`);
console.log(`SCENE_COUNT_MAX=${range.maximum}`);
console.log(`CALL_COUNT_MIN=${workloads.minimum.call_count}`);
console.log(`CALL_COUNT_PREFERRED=${workloads.preferred.call_count}`);
console.log(`CALL_COUNT_MAX=${workloads.maximum.call_count}`);
console.log(`ALLOWED_OPERATIONS=${JSON.stringify(output.exact_allowed_operations)}`);
console.log(`WALLET_CURRENCY=${currency}`);
console.log(`WALLET_AVAILABLE_BALANCE=${output.wallet.available_balance}`);
console.log(`PRICING_CANDIDATE_COUNT=${candidates.length}`);
console.log(`APPROVAL_READY_PRICING_CANDIDATE_COUNT=${ready.length}`);
