...
async function requireQuiescent(endpointId, key, label) {
  ...
  const workerTotal = ["idle", "initializing", "ready", "running", "throttled", "unhealthy"].reduce((sum, workerKey) => sum + finite(workers[workerKey], 0), 0);
  if (queue !== 0 || progress !== 0 || workerTotal !== 0) {
    throw new Error(`AVANTIQO_VIDEO_I2V_V18_${label}_NOT_QUIESCENT:queue=${queue}:progress=${progress}:workers=${workerTotal}`);
  }
...
console.log(JSON.stringify({
  ...
  shared_image_endpoint_read_only_guard: true,
...
const workersMin = finite(endpoint.workersMin ?? endpoint.workers_min, -1);
const workersMax = finite(endpoint.workersMax ?? endpoint.workers_max, -1);
if (workersMin !== 0 || workersMax !== 0) {
  throw new Error(`AVANTIQO_VIDEO_I2V_V18_CINEMA_NOT_RESTING_0_0:min=${workersMin}:max=${workersMax}`);
}
await requireQuiescent(ENDPOINT_ID, queueKey, "CINEMA");
...
console.log("AVANTIQO_VIDEO_I2V_V18_IMAGE_QUIESCENT_CONFIRMED=true");
