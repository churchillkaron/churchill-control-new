...
const CONTRACT = "AVANTIQO_VIDEO_WAN22_T2V_A14B_DIRECT_POD_CACHE_V9";
...
const TARGET_MODEL = "Wan-AI/Wan2.2-T2V-A14B-Diffusers";
const CACHE_ROOT = "/runpod-volume/huggingface-cache/hub";
const STATUS_ROOT = "/runpod-volume/avantiqo-cache-status";
const MARKER_NAME = "avantiqo-video-t2v-cache-marker.json";
const BOOTSTRAP_NAME_PREFIX = "avantiqo-video-t2v-cache-bootstrap";
const HTTP_PORT = 8000;
const POLL_MS = Math.max(10_000, Number(process.env.AVANTIQO_VIDEO_WAN22_POD_CACHE_POLL_MS || 20_000));
const MAX_WAIT_MS = Math.max(
  30 * 60 * 1000,
  Number(process.env.AVANTIQO_VIDEO_WAN22_POD_CACHE_WAIT_MS || 3 * 60 * 60 * 1000),
);
...
function requireScopedMain() {
  shell("git", ["fetch", "origin", "main"], "AVANTIQO_VIDEO_POD_CACHE_FETCH_MAIN_FAILED");
  const branch = shell("git", ["branch", "--show-current"], "AVANTIQO_VIDEO_POD_CACHE_BRANCH_READ_FAILED");
  if (branch !== "main") throw new Error(`AVANTIQO_VIDEO_POD_CACHE_MAIN_REQUIRED:${branch || "DETACHED"}`);
  const head = shell("git", ["rev-parse", "HEAD"], "AVANTIQO_VIDEO_POD_CACHE_HEAD_READ_FAILED");
  const remote = shell("git", ["rev-parse", "origin/main"], "AVANTIQO_VIDEO_POD_CACHE_REMOTE_READ_FAILED");
  const relation = shell(
    "git",
    ["rev-list", "--left-right", "--count", `${head}...${remote}`],
    "AVANTIQO_VIDEO_POD_CACHE_MAIN_RELATION_FAILED",
  ).split(/\s+/).map(Number);
  if (Number(relation[0] || 0) !== 0) {
    throw new Error(`AVANTIQO_VIDEO_POD_CACHE_MAIN_DIVERGED:head=${head}:origin=${remote}`);
  }
  if (head !== remote) {
    const changed = shell(
      "git",
      ["diff", "--name-only", head, remote, "--", ...VIDEO_OWNED_PATHS],
      "AVANTIQO_VIDEO_POD_CACHE_SCOPED_DIFF_FAILED",
    );
    if (changed) {
      throw new Error(`AVANTIQO_VIDEO_POD_CACHE_VIDEO_INPUTS_CHANGED_ON_MAIN:${changed.replace(/\n/g, ",")}`);
    }
    console.log(`AVANTIQO_VIDEO_POD_CACHE_UNRELATED_MAIN_MOVEMENT_TOLERATED=true head=${head} origin=${remote}`);
  }
  const dirty = shell(
    "git",
    ["status", "--porcelain", "--untracked-files=no", "--", ...VIDEO_OWNED_PATHS],
    "AVANTIQO_VIDEO_POD_CACHE_SOURCE_STATUS_FAILED",
  );
  if (dirty) throw new Error("AVANTIQO_VIDEO_POD_CACHE_VIDEO_OWNED_FILES_HAVE_LOCAL_CHANGES");
  return head;
}

function assertScopedInputsUnchangedFrom(startSha) {
  shell("git", ["fetch", "origin", "main"], "AVANTIQO_VIDEO_POD_CACHE_FINAL_FETCH_MAIN_FAILED");
  const remote = shell("git", ["rev-parse", "origin/main"], "AVANTIQO_VIDEO_POD_CACHE_FINAL_REMOTE_READ_FAILED");
  const changed = shell(
    "git",
    ["diff", "--name-only", startSha, remote, "--", ...VIDEO_OWNED_PATHS],
    "AVANTIQO_VIDEO_POD_CACHE_FINAL_SCOPED_DIFF_FAILED",
  );
  if (changed) {
    throw new Error(`AVANTIQO_VIDEO_POD_CACHE_INPUTS_CHANGED_DURING_RUN:${changed.replace(/\n/g, ",")}`);
  }
  return remote;
}
...
async function rest(pathname, key, options = {}) {
  ...
  return readJson(response, "AVANTIQO_VIDEO_POD_CACHE_REST", { allow404: options.allow404 });
}
...
async function queueRequest(endpointId, pathname, key) {
  ...
  return readJson(response, "AVANTIQO_VIDEO_POD_CACHE_QUEUE");
}
...
function validateImageLock(lock) {
  if (
    ...
    throw new Error("AVANTIQO_VIDEO_POD_CACHE_IMAGE_V9_LOCK_INVALID");
  }
}
...
function validateBaseline(image, cinema, volume) {
  ...
  ) throw new Error(`AVANTIQO_VIDEO_POD_CACHE_IMAGE_BASELINE_INVALID:${JSON.stringify(safeEndpoint(image))}`);
  ...
  ) throw new Error(`AVANTIQO_VIDEO_POD_CACHE_CINEMA_BASELINE_INVALID:${JSON.stringify(safeEndpoint(cinema))}`);
  ...
  ) throw new Error(`AVANTIQO_VIDEO_POD_CACHE_VOLUME_INVALID:${JSON.stringify({
    id: text(volume.id),
...
async function inventory(managementKey) {
  ...
  if (!endpoints || !volumes || !pods) throw new Error("AVANTIQO_VIDEO_POD_CACHE_INVENTORY_INVALID");
...
function validateNoExistingBootstrapPod(pods) {
  ...
  if (matches.length) {
    throw new Error(`AVANTIQO_VIDEO_POD_CACHE_EXISTING_BOOTSTRAP_POD:${matches.map((pod) => text(pod.id)).filter(Boolean).join(",")}`);
  }
}
...
MARKER = os.path.join(STATUS_ROOT, "avantiqo-video-t2v-cache-marker.json")
REPO_CACHE = os.path.join(CACHE_ROOT, "models--Wan-AI--Wan2.2-T2V-A14B-Diffusers")
...
marker = {
    "success": False,
    "contract": "AVANTIQO_VIDEO_WAN22_T2V_A14B_DIRECT_POD_CACHE_MARKER_V1",
    "target_model": MODEL,
...
try:
    ...
    info = api.model_info(MODEL, files_metadata=True, token=token)
        ...
        for root, _, files in os.walk(REPO_CACHE):
            for filename in files:
                if filename.endswith(".incomplete"):
                    incomplete.append(os.path.relpath(os.path.join(root, filename), REPO_CACHE))

    model_index = os.path.join(snapshot, "model_index.json")
    success = (
        bool(revision)
        and os.path.isfile(model_index)
        and not missing
        and not mismatched
        and not incomplete
        and actual_bytes > 1_000_000_000
    ...
    if not success:
        marker["error"] = "CACHE_INTEGRITY_CHECK_FAILED"
...
async function fetchMarker(podId) {
  const url = `https://${encodeURIComponent(podId)}-${HTTP_PORT}.proxy.runpod.net/${MARKER_NAME}?t=${Date.now()}`;
  try {
...
function validateMarker(marker) {
  if (
    marker?.success !== true ||
    text(marker.contract) !== "AVANTIQO_VIDEO_WAN22_T2V_A14B_DIRECT_POD_CACHE_MARKER_V1" ||
    text(marker.target_model) !== TARGET_MODEL ||
    text(marker.cache_root) !== CACHE_ROOT ||
    marker.video_generation_submitted !== false ||
    marker.inference_performed !== false ||
    !text(marker.revision) ||
    marker.model_index_present !== true ||
    !Array.isArray(marker.missing_required_files) ||
    marker.missing_required_files.length !== 0 ||
    !Array.isArray(marker.size_mismatches) ||
    marker.size_mismatches.length !== 0 ||
    !Array.isArray(marker.incomplete_files) ||
    marker.incomplete_files.length !== 0 ||
    finite(marker.actual_snapshot_bytes, 0) <= 1_000_000_000
  ) {
    throw new Error(`AVANTIQO_VIDEO_POD_CACHE_MARKER_INVALID:${redact(JSON.stringify(marker)).slice(0, 2400)}`);
  }
}
...
async function deleteCreatedPod(podId, managementKey) {
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    try {
      ...
      if (current?.__not_found === true) {
        console.log(`AVANTIQO_VIDEO_POD_CACHE_TEMP_POD_DELETED=true pod=${podId}`);
        return true;
      }
    } catch (error) {
      lastError = error;
  ...
  console.error(`AVANTIQO_VIDEO_POD_CACHE_TEMP_POD_DELETE_FAILED=true pod=${podId} error=${redact(text(lastError?.message || lastError))}`);
  return false;
}

if (Number(process.versions.node.split(".")[0]) < 24) {
  throw new Error(`AVANTIQO_VIDEO_POD_CACHE_NODE24_REQUIRED:${process.version}`);
}

const apply = process.argv.includes("--apply");
if (apply && !approved(process.env.AVANTIQO_VIDEO_WAN22_T2V_POD_CACHE_APPROVED)) {
  throw new Error("AVANTIQO_VIDEO_WAN22_T2V_POD_CACHE_APPROVED=YES_REQUIRED");
}

const startSha = requireScopedMain();
const managementKey = text(process.env.RUNPOD_MANAGEMENT_API_KEY || process.env.RUNPOD_API_KEY);
if (!managementKey) throw new Error("AVANTIQO_VIDEO_POD_CACHE_MANAGEMENT_CREDENTIAL_REQUIRED");
...
if (volumeMatches.length !== 1) throw new Error(`AVANTIQO_VIDEO_POD_CACHE_VOLUME_RESOLUTION_FAILED:${volumeMatches.length}`);
...
assertSharedQuiescent(imageHealth, "AVANTIQO_VIDEO_POD_CACHE_IMAGE");
assertSharedQuiescent(cinemaHealth, "AVANTIQO_VIDEO_POD_CACHE_CINEMA");
...
console.log(JSON.stringify({
  ...
  strategy: {
    ...
    compute_type: "CPU",
    reason: "MODEL_CACHE_DOWNLOAD_DOES_NOT_REQUIRE_GPU_AND_SERVERLESS_GPU_SCHEDULING_FAILED_FOR_ALL_LIVE_COMPATIBLE_TYPES",
  ...
  next_action: apply ? "CREATE_DIRECT_CACHE_POD_VERIFY_MARKER_TERMINATE" : "APPLY_DIRECT_CACHE_POD",
}, null, 2));
...
if (freshVolumeMatches.length !== 1) throw new Error("AVANTIQO_VIDEO_POD_CACHE_FRESH_VOLUME_RESOLUTION_FAILED");
const freshImage = resolveEndpoint(
  fresh.endpoints,
  text(image.id),
  new Set([IMAGE_NAME]),
  "AVANTIQO_VIDEO_POD_CACHE_FRESH_IMAGE",
);
const freshCinema = resolveEndpoint(
  fresh.endpoints,
  text(cinema.id),
  CINEMA_NAMES,
  "AVANTIQO_VIDEO_POD_CACHE_FRESH_CINEMA",
);
...
assertSharedQuiescent(freshImageHealth, "AVANTIQO_VIDEO_POD_CACHE_FRESH_IMAGE");
assertSharedQuiescent(freshCinemaHealth, "AVANTIQO_VIDEO_POD_CACHE_FRESH_CINEMA");
...
try {
  ...
  if (!createdPodId) throw new Error("AVANTIQO_VIDEO_POD_CACHE_CREATED_POD_ID_MISSING");
  console.log(`AVANTIQO_VIDEO_POD_CACHE_TEMP_POD_CREATED=true pod=${createdPodId}`);
  ...
  while (Date.now() <= deadline) {
    ...
    if (Date.now() - lastPrintedAt >= 60_000) {
      ...
      if (current?.__not_found === true) {
        throw new Error(`AVANTIQO_VIDEO_POD_CACHE_TEMP_POD_DISAPPEARED:${createdPodId}`);
      }
      console.log(`AVANTIQO_VIDEO_POD_CACHE_PROGRESS ${JSON.stringify(safePod(current))}`);
      lastPrintedAt = Date.now();
    }
    await sleep(POLL_MS);
  }

  if (!marker) throw new Error(`AVANTIQO_VIDEO_POD_CACHE_MARKER_TIMEOUT:${createdPodId}`);
  ...
  assertSharedQuiescent(beforeDeleteImageHealth, "AVANTIQO_VIDEO_POD_CACHE_POST_DOWNLOAD_IMAGE");
  assertSharedQuiescent(beforeDeleteCinemaHealth, "AVANTIQO_VIDEO_POD_CACHE_POST_DOWNLOAD_CINEMA");

  console.log(`AVANTIQO_VIDEO_POD_CACHE_MARKER_VERIFIED=true revision=${text(marker.revision)} bytes=${finite(marker.actual_snapshot_bytes, 0)}`);
} finally {
  if (createdPodId) podDeleted = await deleteCreatedPod(createdPodId, managementKey);
}

if (!podDeleted) {
  throw new Error(`AVANTIQO_VIDEO_POD_CACHE_TEMP_POD_CLEANUP_REQUIRED:${createdPodId || "UNKNOWN"}`);
}
...
if (finalVolumeMatches.length !== 1) throw new Error("AVANTIQO_VIDEO_POD_CACHE_FINAL_VOLUME_RESOLUTION_FAILED");
const finalImage = resolveEndpoint(
  final.endpoints,
  text(image.id),
  new Set([IMAGE_NAME]),
  "AVANTIQO_VIDEO_POD_CACHE_FINAL_IMAGE",
);
const finalCinema = resolveEndpoint(
  final.endpoints,
  text(cinema.id),
  CINEMA_NAMES,
  "AVANTIQO_VIDEO_POD_CACHE_FINAL_CINEMA",
);
...
assertSharedQuiescent(finalImageHealth, "AVANTIQO_VIDEO_POD_CACHE_FINAL_IMAGE");
assertSharedQuiescent(finalCinemaHealth, "AVANTIQO_VIDEO_POD_CACHE_FINAL_CINEMA");
...
console.log(JSON.stringify({
  ...
  cache_ready: true,
  cache_marker: {
    contract: text(marker.contract),
    target_model: text(marker.target_model),
    revision: text(marker.revision),
    snapshot_path: text(marker.snapshot_path),
    required_file_count: finite(marker.required_file_count, 0),
    actual_snapshot_bytes: finite(marker.actual_snapshot_bytes, 0),
    expected_snapshot_bytes: finite(marker.expected_snapshot_bytes, 0),
    missing_required_files: marker.missing_required_files,
    size_mismatches: marker.size_mismatches,
    incomplete_files: marker.incomplete_files,
  },
  temporary_pod: {
    id: createdPodId,
    deleted: true,
  ...
  next_action: "INSPECT_T2V_CACHE_THEN_PREPARE_I2V",
}, null, 2));

console.log("AVANTIQO_VIDEO_WAN22_T2V_DIRECT_POD_CACHE_V9_APPLIED=true");
