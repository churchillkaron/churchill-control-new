...
const CONTRACT = "AVANTIQO_VIDEO_WAN22_T2V_A14B_SINGLE_GPU_PROVIDER_STARTUP_V4";
...
function requireCurrentMain() {
  shell("git", ["fetch", "origin", "main"], "AVANTIQO_VIDEO_SINGLE_GPU_V4_FETCH_MAIN_FAILED");
  const branch = shell("git", ["branch", "--show-current"], "AVANTIQO_VIDEO_SINGLE_GPU_V4_BRANCH_READ_FAILED");
  if (branch !== "main") throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_MAIN_REQUIRED:${branch || "DETACHED"}`);
  const head = shell("git", ["rev-parse", "HEAD"], "AVANTIQO_VIDEO_SINGLE_GPU_V4_HEAD_READ_FAILED");
  const remote = shell("git", ["rev-parse", "origin/main"], "AVANTIQO_VIDEO_SINGLE_GPU_V4_REMOTE_READ_FAILED");
  const relation = shell(
    "git",
    ["rev-list", "--left-right", "--count", `${head}...${remote}`],
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_MAIN_RELATION_FAILED",
  ).split(/\s+/).map(Number);
  if (Number(relation[0] || 0) !== 0) {
    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_MAIN_DIVERGED:head=${head}:origin=${remote}`);
  }
  if (head !== remote) {
    const changed = shell(
      "git",
      ["diff", "--name-only", head, remote, "--", ...VIDEO_OWNED_PATHS],
      "AVANTIQO_VIDEO_SINGLE_GPU_V4_SCOPED_DIFF_FAILED",
    );
    if (changed) {
      throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_VIDEO_INPUTS_CHANGED_ON_MAIN:${changed.replace(/\n/g, ",")}`);
    }
    console.log(`AVANTIQO_VIDEO_SINGLE_GPU_V4_UNRELATED_MAIN_MOVEMENT_TOLERATED=true head=${head} origin=${remote}`);
  }
  const dirty = shell(
    "git",
    ["status", "--porcelain", "--untracked-files=no", "--", ...VIDEO_OWNED_PATHS],
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_SOURCE_STATUS_FAILED",
  );
  if (dirty) throw new Error("AVANTIQO_VIDEO_SINGLE_GPU_V4_VIDEO_OWNED_FILES_HAVE_LOCAL_CHANGES");
  return head;
}
...
function compatibilitySource(baseSource) {
  const oldChildRequireCurrentMain = [
    "function requireCurrentMain() {",
  ...
  const newChildRequireCurrentMain = [
    "function requireCurrentMain() {",
    "  const refs = currentMainRefs();",
    "  const scopedPaths = [",
    "    VIDEO_SOURCE_PATH,",
    ...
    "  const relation = shell(",
    "    \"git\",",
    "    [\"rev-list\", \"--left-right\", \"--count\", `${refs.head}...${refs.remote}`],",
    "    \"AVANTIQO_VIDEO_SINGLE_GPU_MAIN_RELATION_FAILED\",",
    "  ).split(/\\s+/).map(Number);",
    "  if (Number(relation[0] || 0) !== 0) {",
    "    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_MAIN_DIVERGED:head=${refs.head}:origin=${refs.remote}`);",
    "  }",
    "  if (refs.head !== refs.remote) {",
    "    const changed = shell(",
    "      \"git\",",
    "      [\"diff\", \"--name-only\", refs.head, refs.remote, \"--\", ...scopedPaths],",
    "      \"AVANTIQO_VIDEO_SINGLE_GPU_SCOPED_REMOTE_DIFF_FAILED\",",
    "    );",
    "    if (changed) {",
    "      throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_VIDEO_INPUTS_CHANGED_ON_MAIN:${changed.replace(/\\n/g, \",\")}`);",
    "    }",
    "    console.log(`AVANTIQO_VIDEO_SINGLE_GPU_UNRELATED_MAIN_MOVEMENT_TOLERATED_AT_GATE=true head=${refs.head} origin=${refs.remote}`);",
    "  }",
    "  const dirty = shell(",
    "    \"git\",",
    "    [\"status\", \"--porcelain\", \"--untracked-files=no\", \"--\", ...scopedPaths],",
    "    \"AVANTIQO_VIDEO_SINGLE_GPU_SOURCE_STATUS_FAILED\",",
    "  );",
    "  if (dirty) throw new Error(\"AVANTIQO_VIDEO_SINGLE_GPU_VIDEO_OWNED_FILES_HAVE_LOCAL_CHANGES\");",
    "  return refs.head;",
  ...
  const newCreateCompatibilityBlock = [
    ...
    "    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_WAIT_FOR_JOB_ANCHOR_INVALID:${waitAnchorCount}`);",
    "  }",
  ...
  let source = replaceExactlyOnce(
    baseSource,
    oldChildRequireCurrentMain,
    newChildRequireCurrentMain,
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_CHILD_MAIN_GATE",
  );
  source = replaceExactlyOnce(
    source,
    oldWorkerHelpers,
    newWorkerHelpers,
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_WORKER_HELPERS",
  );
  source = replaceExactlyOnce(
    source,
    oldRestoreCondition,
    newRestoreCondition,
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_RESTORE_CONDITION",
  );
  source = replaceExactlyOnce(
    source,
    oldCreateCompatibilityLine,
    newCreateCompatibilityBlock,
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_BASE_WAIT_GUARD",
  );
  source = replaceExactlyOnce(
    source,
    oldMainMovement,
    newMainMovement,
    "AVANTIQO_VIDEO_SINGLE_GPU_V4_MAIN_MOVEMENT",
  );
  return source;
}

if (Number(process.versions.node.split(".")[0]) < 24) {
  throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_NODE24_REQUIRED:${process.version}`);
}
...
const dir = await mkdtemp(join(tmpdir(), "avantiqo-video-single-gpu-v4-"));
const path = join(dir, "cache-avantiqo-video-wan22-t2v-a14b-single-gpu-v4-compat.mjs");
...
try {
  ...
  if (syntax.status !== 0) {
    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_COMPATIBILITY_SYNTAX_FAILED:${text(syntax.stderr || syntax.stdout).slice(0, 1200)}`);
  }
  ...
  console.log(JSON.stringify({
    ...
    main_movement_policy: {
      unrelated_main_commits_tolerated_at_startup: true,
    ...
    image_mutation_performed_by_v4: false,
    video_mutation_performed_by_v4: false,
  ...
  if (child.status !== 0) {
    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V4_CHILD_FAILED:exit=${child.status}`);
  }
} finally {
