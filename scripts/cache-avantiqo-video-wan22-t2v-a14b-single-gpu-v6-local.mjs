...
const CONTRACT = "AVANTIQO_VIDEO_WAN22_T2V_A14B_SINGLE_BLACKWELL_SERVER_V6";
...
function requireScopedMain() {
  shell("git", ["fetch", "origin", "main"], "AVANTIQO_VIDEO_SINGLE_GPU_V6_FETCH_MAIN_FAILED");
  const branch = shell("git", ["branch", "--show-current"], "AVANTIQO_VIDEO_SINGLE_GPU_V6_BRANCH_READ_FAILED");
  if (branch !== "main") throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_MAIN_REQUIRED:${branch || "DETACHED"}`);
  const head = shell("git", ["rev-parse", "HEAD"], "AVANTIQO_VIDEO_SINGLE_GPU_V6_HEAD_READ_FAILED");
  const remote = shell("git", ["rev-parse", "origin/main"], "AVANTIQO_VIDEO_SINGLE_GPU_V6_REMOTE_READ_FAILED");
  const relation = shell(
    "git",
    ["rev-list", "--left-right", "--count", `${head}...${remote}`],
    "AVANTIQO_VIDEO_SINGLE_GPU_V6_MAIN_RELATION_FAILED",
  ).split(/\s+/).map(Number);
  if (Number(relation[0] || 0) !== 0) {
    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_MAIN_DIVERGED:head=${head}:origin=${remote}`);
  }
  if (head !== remote) {
    const changed = shell(
      "git",
      ["diff", "--name-only", head, remote, "--", ...VIDEO_OWNED_PATHS],
      "AVANTIQO_VIDEO_SINGLE_GPU_V6_SCOPED_DIFF_FAILED",
    );
    if (changed) throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_VIDEO_INPUTS_CHANGED_ON_MAIN:${changed.replace(/\n/g, ",")}`);
    console.log(`AVANTIQO_VIDEO_SINGLE_GPU_V6_UNRELATED_MAIN_MOVEMENT_TOLERATED=true head=${head} origin=${remote}`);
  }
  const dirty = shell(
    "git",
    ["status", "--porcelain", "--untracked-files=no", "--", ...VIDEO_OWNED_PATHS],
    "AVANTIQO_VIDEO_SINGLE_GPU_V6_SOURCE_STATUS_FAILED",
  );
  if (dirty) throw new Error("AVANTIQO_VIDEO_SINGLE_GPU_V6_VIDEO_OWNED_FILES_HAVE_LOCAL_CHANGES");
  return head;
}
...
if (Number(process.versions.node.split(".")[0]) < 24) {
  throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_NODE24_REQUIRED:${process.version}`);
}
...
const newSelectSingleGpu = [
  ...
  "    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_ONLY_BLACKWELL_SERVER_ALLOWED:${requested}`);",
  "  }",
  "  const exact = candidates.find((gpu) => gpu.id === required && gpu.stock_score > 0);",
  "  if (!exact) {",
  "    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_BLACKWELL_SERVER_NOT_LIVE:${JSON.stringify(candidates)}`);",
  "  }",
  "  return exact;",
  "}",
].join("\n");
...
if (selectionCount !== 1) {
  throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_SELECTION_ANCHOR_COUNT:${selectionCount}`);
}
const patchedSingleGpu = singleGpuSource.replace(oldSelectSingleGpu, newSelectSingleGpu);

const dir = await mkdtemp(join(tmpdir(), "avantiqo-video-single-gpu-v6-"));
const patchedSinglePath = join(dir, "cache-avantiqo-video-wan22-t2v-a14b-single-blackwell-server.mjs");
const patchedV4Path = join(dir, "cache-avantiqo-video-wan22-t2v-a14b-single-gpu-v6-compat.mjs");
...
try {
  ...
  const patchedV4 = replaceExactlyOnce(
    ...
    "AVANTIQO_VIDEO_SINGLE_GPU_V6_V4_BASE_RUNNER",
  );
  ...
  for (const [label, path] of [["SINGLE_GPU", patchedSinglePath], ["V4", patchedV4Path]]) {
    ...
    if (syntax.status !== 0) {
      throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_${label}_SYNTAX_FAILED:${text(syntax.stderr || syntax.stdout).slice(0, 1200)}`);
    }
  }
  ...
  console.log(JSON.stringify({
    ...
    selected_gpu_policy: {
      exact_gpu_type_id: REQUIRED_GPU,
      reason: "ONLY_REMAINING_LIVE_COMPATIBLE_GPU_AFTER_B200_PROVIDER_NO_START",
      one_gpu_one_attempt: true,
      automatic_fallback: false,
      no_runpod_mutation_if_not_live: true,
    },
    inherited_v4_guards: {
      no_worker_startup_timeout_ms: 120000,
      exact_job_cancel_on_guard_failure: true,
      baseline_restore_to_full_blackwell_pool: true,
      image_shared_volume_quiescence_guard: true,
      unrelated_main_movement_tolerated: true,
    },
    image_mutation_performed_by_v6: false,
    video_mutation_performed_by_v6: false,
  ...
  if (child.status !== 0) {
    throw new Error(`AVANTIQO_VIDEO_SINGLE_GPU_V6_CHILD_FAILED:exit=${child.status}`);
  }
} finally {
