...
const CONTRACT = "AVANTIQO_INTELLIGENCE_FAST_LIVE_WORKER_CAPTURE_RECOVERY_V3";
...
function validateCurrentMain() {
  ...
  const branch = shell(
    "git",
    ["branch", "--show-current"],
    "AVANTIQO_FAST_LIVE_CAPTURE_GIT_BRANCH_FAILED",
  );
  if (branch !== "main") {
  ...
  let head = shell(
    "git",
    ["rev-parse", "HEAD"],
    "AVANTIQO_FAST_LIVE_CAPTURE_GIT_HEAD_FAILED",
  );
  const remote = shell(
    "git",
    ["rev-parse", "origin/main"],
    "AVANTIQO_FAST_LIVE_CAPTURE_GIT_REMOTE_FAILED",
  );
  ...
  if (head !== remote) {
    ...
    if (head !== remote) {
      throw new Error(
        `AVANTIQO_FAST_LIVE_CAPTURE_MAIN_CONVERGENCE_FAILED:head=${head}:origin_main=${remote}`,
      );
    }
...
function resolveOne(rows, name) {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `AVANTIQO_FAST_LIVE_CAPTURE_ENDPOINT_RESOLUTION_FAILED:name=${name}:matches=${matches.length}`,
    );
  }
  return matches[0];
}
  ...
  try {
    ...
    if (!response.body) {
      return {
        ...
        entries,
        error: "AVANTIQO_FAST_LIVE_CAPTURE_LOG_BODY_REQUIRED",
      };
    }
...
function classify(workerCount, signals) {
  ...
  if (signals.application_startup_complete) {
    return "FAST_RUNTIME_STARTED_SERVERLESS_SIGNATURE_NOT_CAPTURED";
  }
...
try {
  const endpoints = await requestJson(
  ...
  fastId = text(fast?.id);
  if (!deepId || !fastId) {
    throw new Error("AVANTIQO_FAST_LIVE_CAPTURE_ENDPOINT_IDS_REQUIRED");
  }
  ...
  if (![0, 1].includes(deepMax) || ![0, 1].includes(fastMax)) {
    throw new Error(
      `AVANTIQO_FAST_LIVE_CAPTURE_SLOT_STATE_INVALID:deep_max=${deepMax}:fast_max=${fastMax}`,
    );
  }
  ...
  const relevantLogs = entries
    .filter((entry) =>
      /serverless|handler|queue|application startup|uvicorn|vllm|engine|model|cuda|nvidia|traceback|exception|runtimeerror|error|failed|fatal|out of memory|oom|killed|unauthorized|forbidden/i.test(
        entry.line,
      ),
    )
    .slice(-180);
  ...
  report = {
    success: true,
    contract: CONTRACT,
    mode: "CAPTURE_FAST_WORKER_IF_PRESENT_THEN_RESTORE_DEEP",
    ...
    fast_worker_present_at_capture: rawWorkers.length > 0,
  ...
  if (fastId && deepId) {
    try {
      const currentEndpoints = await requestJson(
        `${REST_BASE}/endpoints?includeTemplate=true&includeWorkers=true`,
        managementKey,
      );
      const currentDeep = resolveOne(currentEndpoints, DEEP_NAME);
      const currentFast = resolveOne(currentEndpoints, FAST_NAME);

      if (
        finite(currentFast?.workersMin, -1) !== 0 ||
        finite(currentFast?.workersMax, -1) !== 0
      ) {
      ...
      if (
        finite(currentDeep?.workersMin, -1) !== 0 ||
        finite(currentDeep?.workersMax, -1) !== 1
      ) {
      ...
      if (
        ...
        throw new Error(
          `AVANTIQO_FAST_LIVE_CAPTURE_CLEANUP_VERIFY_FAILED:${JSON.stringify(cleanup.canonical_state_after)}`,
        );
      }
    } catch (error) {
...
if (!report) {
  throw new Error(
    `AVANTIQO_FAST_LIVE_CAPTURE_FAILED_BEFORE_REPORT:cleanup=${JSON.stringify(cleanup)}`,
  );
}
...
report.next_action =
  report.diagnosis === "FAST_WORKER_DISAPPEARED_BEFORE_LOG_CAPTURE"
    ? "CANONICAL_SLOT_RECOVERED_PROCEED_TO_ONE_BOUNDED_FAST_FIRST_RESPONSE"
...
const reportPath = resolve(
  process.env.AVANTIQO_INTELLIGENCE_FAST_LIVE_WORKER_CAPTURE_RECOVERY_OUTPUT ||
    REPORT_DEFAULT,
);
...
console.log(
  `AVANTIQO_INTELLIGENCE_FAST_LIVE_WORKER_CAPTURE_RECOVERY_REPORT=${reportPath}`,
);
console.log(
  `AVANTIQO_INTELLIGENCE_FAST_LIVE_WORKER_CAPTURE_RECOVERY=${report.success ? "PASS" : "FAIL"}`,
);
