...
const CONTRACT = "AVANTIQO_INTELLIGENCE_MODAL_DIRECT_DEEP_SERVICE_CERTIFICATION_V1";
...
const OUTPUT_DIR = resolve(
  process.env.AVANTIQO_INTELLIGENCE_MODAL_DEEP_CERT_OUTPUT_DIR ||
    "local-audit-output/avantiqo-intelligence-modal-direct-deep-service-certification",
);
...
function requirePaidApproval() {
  if (!yes(process.env.AVANTIQO_INTELLIGENCE_MODAL_DEEP_REAL_INFERENCE_APPROVED)) {
    throw new Error("AVANTIQO_INTELLIGENCE_MODAL_DEEP_REAL_INFERENCE_APPROVED=YES_REQUIRED");
  }
...
if (text(selected?.model, 300) !== DEEP_MODEL) {
  throw new Error(`${CONTRACT}_DEEP_MODEL_RESOLUTION_REQUIRED:${text(selected?.model, 300) || "EMPTY"}`);
}
...
const maxCustomerCharge = Math.max(0, finite(
  process.env.AVANTIQO_INTELLIGENCE_MODAL_DEEP_CERT_MAX_CUSTOMER_CHARGE,
...
if (existingState?.terminal === true && existingState?.success === false) {
  ...
  if (runMode === "RESUME") throw new Error(`${CONTRACT}_TERMINAL_FAILURE_CANNOT_RESUME`);
  ...
  try {
    execution = await executeService({
      ...
      input: {
        capability: CAPABILITY,
        execution_lane: "deep",
        prompt: "Use the Deep reasoning lane to solve 17 multiplied by 19. Return only one short sentence with the final answer and confirmation that Avantiqo owned Intelligence served it. Do not reveal chain-of-thought or private reasoning.",
        max_output_tokens: 120,
        temperature: 0.1,
      },
      metadata: {
        certification_contract: CONTRACT,
        certification_scope: "PLATFORM_OWNED_INTELLIGENCE_DIRECT_MODAL_ONE_DEEP_JOB",
...
for (let poll = 1; poll <= MAX_POLLS; poll += 1) {
  ...
  try {
    settled = await settlePendingService({
      ...
      provider_status_input: { capability: CAPABILITY, execution_lane: "deep" },
...
if (!outputText.includes("323")) throw new Error(`${CONTRACT}_REASONING_RESULT_INVALID`);
...
if (executionLane !== "deep") throw new Error(`${CONTRACT}_DEEP_LANE_REQUIRED:${executionLane}`);
if (model !== DEEP_MODEL) throw new Error(`${CONTRACT}_DEEP_MODEL_REQUIRED:${model}`);
...
const report = {
  ...
  reasoning_result_verified: true,
