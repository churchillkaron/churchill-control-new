...
try {
  ...
  process.env.AVANTIQO_LOCAL_COMPUTE_QUEUE_ENABLED = "true";
  process.env.AVANTIQO_LOCAL_CODE_ENABLED = "true";
  ...
  for (let cycle = 1; ; cycle += 1) {
    ...
    const result = await executeAutonomousCodeMission({
      ...
      objective_context: {
        ...
        completion_criterion_3: "The final diff contains only the intended bounded fixture repair.",
      },
  ...
  console.log(JSON.stringify({
    ...
    execution_infrastructure: "AVANTIQO_LOCAL_NODE_V1",
    local_code_queue_forced: true,
