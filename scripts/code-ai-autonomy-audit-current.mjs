...
markers("WORKER", loaded.worker, ["ai.code.generate", "ai.code.edit", "ai.code.review", "code_text", "AVANTIQO_NODE01_WORKER_V6_MODEL_AWARE_CODE", "LOCAL_QWEN4B_GPU_STRONG_MUTATION_QWEN17B_INTERACTIVE_QWEN06B_DISCOVERY_CPU_FALLBACK", "$CodeCpuFallbackModel = 'qwen3:0.6b'", "$forceCpu = (-not $runtimeModelAlreadyGpuResident) -and ($freeGpuMb -lt $codeGpuMinFreeMb)", "codeGpuMinFreeMb = 1800", "codeGpuMinFreeMb = 3000", "codeGpuMinFreeMb = 4300", "interactiveGpuWaitDeadline = $interactiveGpuWaitStarted.AddSeconds(10)", "ReleaseIdleOllamaModelsForStrongCode", "Global\\AvantiqoNode01OllamaTextGpu", "$codeGpuReclaimAttempted = $true", "code_gpu_reclaim_attempted=[bool]$codeGpuReclaimAttempted", "$runtimeModel = $CodeCpuFallbackModel", "$body.options.num_gpu = 0", "AVANTIQO_CODE_OLLAMA_TIMEOUT", "$retryable = $false", "$ollamaTimeoutSeconds"]);
...
console.log("CODE_AI_RUNTIME_MODEL=QWEN4B_GPU_STRONG_QWEN1_7B_INTERACTIVE_QWEN0_6B_CPU_FALLBACK");
