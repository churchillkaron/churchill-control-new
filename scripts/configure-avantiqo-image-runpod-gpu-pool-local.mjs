...
const persistentPool = APPROVED_GPU_TYPES.filter((id) => supportedIds.has(id));
const currentPool = list(endpoint?.gpuTypeIds).map(text).filter(Boolean);
const unknownCurrent = currentPool.filter((id) => !APPROVED_GPU_TYPES.includes(id));
if (unknownCurrent.length) {
  throw new Error(`AVANTIQO_IMAGE_GPU_POOL_UNAPPROVED_CURRENT_TYPES:${unknownCurrent.join("|")}`);
}
if (persistentPool.length < 4) {
  throw new Error(`AVANTIQO_IMAGE_GPU_POOL_INSUFFICIENT_SUPPORTED_TYPES:${persistentPool.length}`);
}
...
const plan = {
  success: true,
  contract: "AVANTIQO_IMAGE_PERSISTENT_GPU_POOL_V1",
...
const liveUnknown = livePool.filter((id) => !APPROVED_GPU_TYPES.includes(id));
if (liveUnknown.length) {
  throw new Error(`AVANTIQO_IMAGE_GPU_POOL_CONCURRENT_UNAPPROVED_TYPES:${liveUnknown.join("|")}`);
}
...
console.log(JSON.stringify({
  ...
  next_action: "NO_PER_JOB_GPU_UNBLOCK_COMMAND_REQUIRED",
}, null, 2));
