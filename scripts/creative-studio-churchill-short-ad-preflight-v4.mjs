...
function trustedSource(item = {}) {
  return safeJson({
    ...
    metadata: {
      ...
      generation_type: item.metadata?.generation_type,
    },
    intelligence: {
...
const DERIVED_CREATIVE_TERMS = [
  "campaign design", "facebook feed", "instagram feed", "social feed",
  "social post", "poster", "flyer", "banner", "storyboard", "thumbnail",
  "creative output", "generated design", "rendered design", "ad design",
  "campaign asset", "layout", "mockup",
];

const GENERATED_LINEAGE_TERMS = [
  "generated", "generation", "campaign", "design", "render", "derivative",
  "output", "composition", "template",
];
...
function derivedCreative(item = {}) {
  const trusted = trustedSource(item);
  const lineage = safeJson({
    lineage: item.lineage,
    generation_type: item.metadata?.generation_type,
    source_type: item.metadata?.source_type,
    parent_asset_node_id: item.parent_asset_node_id,
  });
  return includesAny(trusted, DERIVED_CREATIVE_TERMS) ||
    includesAny(lineage, GENERATED_LINEAGE_TERMS);
}

function sourceVisual(item = {}) {
  if (!["IMAGE", "VIDEO"].includes(mediaKind(item))) return false;
  if (!verified(item) || !usable(item) || derivedCreative(item)) return false;

  const ext = extension(item);
  const hasVisualFile = IMAGE_EXTENSIONS.has(ext) || VIDEO_EXTENSIONS.has(ext) ||
    mimeType(item).startsWith("image/") || mimeType(item).startsWith("video/");
  const uploadedOrRoot = item._record_source === "ASSET" ||
    (!item.parent_asset_node_id && Boolean(originalFileName(item)));
  return hasVisualFile && uploadedOrRoot;
}
...
function score(item = {}, category) {
  ...
  if (sourceVisual(item)) result += 120;
  if (category === "LOGO" && includesAny(trustedSource(item), ["cc logo", "cc-logo", "cc_logo"])) result += 150;
  ...
  if (derivedCreative(item)) result -= 500;
  return result;
}
...
function summarize(item = {}, category = null) {
  return {
    ...
    source_visual: sourceVisual(item),
    derived_creative: derivedCreative(item),
    record_source: item._record_source || null,
    score: category ? score(item, category) : null,
    review: item.review || null,
  };
}

function top(items, category, count = 10, predicate = () => true) {
  return items
...
const records = [
  ...assets.map((asset) => ({ ...asset, _record_source: "ASSET" })),
  ...nodes.map((node) => {
...
for (const item of records) {
  ...
  const representativeScore = (candidate) => (
    (verified(candidate) ? 100 : 0) +
    categories(candidate).length * 10 +
    (sourceVisual(candidate) ? 100 : 0) +
    (candidate._record_source === "ASSET" ? 20 : 0) +
    (text(candidate.url) ? 3 : 0) -
    (derivedCreative(candidate) ? 200 : 0)
  );
  if (!current || representativeScore(item) > representativeScore(current)) {
    representativeByAsset.set(id, item);
  }
}

const library = [...representativeByAsset.values()];
const logoCandidates = top(library, "LOGO", 10, (item) => mediaKind(item) === "IMAGE");
const foodCandidates = top(library, "FOOD", 15, sourceVisual);
const gamesCandidates = top(library, "GAMES", 15, sourceVisual);
const atmosphereCandidates = top(library, "ATMOSPHERE", 15, sourceVisual);
...
const checks = [
  ...
  {
    id: "verified_source_food_asset_found",
    passed: Boolean(
      food && sourceVisual(food) && categories(food).includes("FOOD"),
    ),
    evidence: food ? summarize(food, "FOOD") : null,
  },
  {
    id: "verified_source_experience_asset_found",
    passed: Boolean(
      experience &&
      sourceVisual(experience) &&
  ...
  {
    id: "derived_campaign_assets_excluded_from_source_shots",
    passed: [food, experience].filter(Boolean).every((item) => !derivedCreative(item)),
    evidence: [food, experience].filter(Boolean).map((item) => summarize(item)),
  },
  {
...
const report = {
  ...
  mode: "READ_ONLY_CHURCHILL_VERIFIED_SOURCE_ASSET_PREFLIGHT_V4",
  organization,
  ready,
  ...
  storyboard: [
    {
      ...
      generation_policy: "USE_VERIFIED_ORIGINAL_SOURCE_ASSET_FIRST",
    },
    {
      shot: 2,
      ...
      generation_policy: "USE_VERIFIED_ORIGINAL_SOURCE_ASSET_OR_ONE_CONTROLLED_ANIMATION",
    },
    {
      shot: 3,
  ...
  discovery: {
    ...
    verified_source_visual_count: library.filter(sourceVisual).length,
    excluded_derived_creative_count: library.filter(derivedCreative).length,
    verified_logo_candidate_count: logoCandidates.length,
    verified_source_food_candidate_count: foodCandidates.length,
    verified_source_games_candidate_count: gamesCandidates.length,
    verified_source_atmosphere_candidate_count: atmosphereCandidates.length,
...
const outputPath = text(process.env.CHURCHILL_SHORT_AD_PREFLIGHT_OUTPUT) || path.join(
  os.homedir(),
  "Downloads",
  `CHURCHILL_SHORT_AD_PREFLIGHT_V4_${new Date()
    .toISOString()
    .replace(/[-:]/g, "")
    .replace(/\..+$/, "")}.json`,
);
...
console.log("============================================================");
console.log("CHURCHILL SHORT AD VERIFIED SOURCE-ASSET PREFLIGHT V4");
console.log("============================================================");
...
console.log(`VERIFIED_SOURCE_VISUAL_COUNT=${report.discovery.verified_source_visual_count}`);
console.log(`EXCLUDED_DERIVED_CREATIVE_COUNT=${report.discovery.excluded_derived_creative_count}`);
console.log(`VERIFIED_LOGO_CANDIDATE_COUNT=${logoCandidates.length}`);
console.log(`VERIFIED_SOURCE_FOOD_CANDIDATE_COUNT=${foodCandidates.length}`);
console.log(`VERIFIED_SOURCE_GAMES_CANDIDATE_COUNT=${gamesCandidates.length}`);
console.log(`VERIFIED_SOURCE_ATMOSPHERE_CANDIDATE_COUNT=${atmosphereCandidates.length}`);
...
console.log(`SELECTED_FOOD_SOURCE_VISUAL=${food ? (sourceVisual(food) ? "YES" : "NO") : ""}`);
...
console.log(`SELECTED_EXPERIENCE_SOURCE_VISUAL=${experience ? (sourceVisual(experience) ? "YES" : "NO") : ""}`);
console.log(`SELECTED_EXPERIENCE_DERIVED_CREATIVE=${experience ? (derivedCreative(experience) ? "YES" : "NO") : ""}`);
console.log(`SHORT_AD_PREFLIGHT_READY=${ready ? "PASS" : "FAIL"}`);
console.log(`BLOCKING_REASONS=${blockers.join(",")}`);
console.log("AUTO_SELECTION_SOURCE=VERIFIED_CHURCHILL_SOURCE_ASSET_INTELLIGENCE");
