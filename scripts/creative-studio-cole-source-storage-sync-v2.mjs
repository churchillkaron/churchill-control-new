...
async function signedRangeReadable(supabase, bucket, storagePath) {
  ...
  if (response.body) {
    try {
      ...
      // HTTP status remains the remote-read evidence.
    }
  }
...
const bucket = required("CREATIVE_MEDIA_ASSET_BUCKET");
...
if (videoFiles.length !== 8 || sourcePaths.length !== 9) {
  throw new Error(
    `COLE_SOURCE_SET_INVALID:videos=${videoFiles.length}:total=${sourcePaths.length}`,
  );
}
...
for (const filePath of sourcePaths) {
  ...
  if (!stat.isFile() || stat.size <= 0) {
    throw new Error(`SOURCE_FILE_INVALID:${absolutePath}`);
  }
...
const largestSourceBytes = Math.max(...sources.map((source) => source.sizeBytes));
const capacity = await ensureCreativeBucketCapacity({
  supabase,
  bucket,
  requiredBytes: largestSourceBytes,
});

console.log(`BUCKET=${bucket}`);
console.log(`LARGEST_SOURCE_BYTES=${largestSourceBytes}`);
console.log(`BUCKET_LIMIT_CHANGED=${capacity.changed ? "YES" : "NO"}`);
console.log(`BUCKET_EFFECTIVE_LIMIT_BYTES=${capacity.effective_limit_bytes ?? "UNBOUNDED_OR_GLOBAL"}`);
...
for (const source of sources) {
  ...
  if (recordedChecksum && recordedChecksum !== source.checksumSha256.toLowerCase()) {
    throw new Error(`SOURCE_CHECKSUM_MISMATCH:${source.fileName}`);
  }

  let storage = parseStorageReference(node, bucket);
  let remote = storage
  ...
  if (!remote.readable) {
    ...
    console.log(`RESUMABLE_UPLOAD_START=${source.fileName}`);
    await uploadCreativeFileResumable({
      ...
      onProgress({ percentage }) {
        console.log(`UPLOAD_PROGRESS=${source.fileName}:${percentage}%`);
      },
    });

    storage = { bucket, storagePath };
    remote = await signedRangeReadable(supabase, bucket, storagePath);
    if (!remote.readable) {
      throw new Error(
  ...
  const requiresRepair =
    ...
    text(currentMetadata.checksum_sha256).toLowerCase() !== source.checksumSha256 ||
    text(currentTechnical.checksum_sha256).toLowerCase() !== source.checksumSha256;
  ...
  if (requiresRepair) {
    ...
    const { error: nodeError } = await supabase
      ...
      .update({
        ...
        metadata: {
          ...
          source_storage_sync_mode: uploaded ? "RESUMABLE_OBJECT_UPLOADED" : "REFERENCE_REPAIRED",
        },
        updated_at: syncedAt,
      })
      .eq("id", node.id)
    ...
    if (nodeError) {
      throw new Error(`SOURCE_NODE_UPDATE_FAILED:${source.fileName}:${nodeError.message}`);
    }
    ...
    if (node.creative_asset_id) {
      ...
      if (assetReadError || !asset) {
        throw new Error(
          `SOURCE_ASSET_READ_FAILED:${source.fileName}:${assetReadError?.message || "missing"}`,
        );
      }
      ...
      if (assetError) {
        throw new Error(`SOURCE_ASSET_UPDATE_FAILED:${source.fileName}:${assetError.message}`);
      }
    }
  ...
  confirmedCount += 1;
  console.log(
    `SOURCE=${source.fileName} NODE_ID=${node.id} SIZE_BYTES=${source.sizeBytes} ` +
    `REMOTE_READ=PASS ACTION=${uploaded ? "RESUMABLE_UPLOAD" : requiresRepair ? "REFERENCE_REPAIRED" : "CONFIRMED_EXISTING"}`,
  );
}
...
console.log("SOURCE_VIDEO_COUNT=8");
...
console.log("RESUMABLE_UPLOAD_PATH=PASS");
