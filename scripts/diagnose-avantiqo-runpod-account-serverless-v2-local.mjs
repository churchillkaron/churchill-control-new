...
const CONTRACT = "AVANTIQO_RUNPOD_SERVERLESS_ACCOUNT_DIAGNOSTIC_V2";
...
async function accountGraphql(managementKey) {
  const query = `
    query AvantiqoRunpodServerlessAccountDiagnosticV2 {
      myself {
        underBalance
        minBalance
  ...
  if (!response.ok || body?.errors?.length || !body?.data?.myself) {
    ...
    throw new Error(`AVANTIQO_RUNPOD_ACCOUNT_V2_GRAPHQL_FAILED:${response.status}:${detail || "INVALID_RESPONSE"}`);
  }
...
async function rest(pathname, managementKey) {
  return readJsonResponse(await fetch(`${REST_BASE}${pathname}`, {
    ...
    signal: AbortSignal.timeout(30_000),
  }), "AVANTIQO_RUNPOD_ACCOUNT_V2_REST");
}
...
async function queue(endpointId, inferenceKey, pathname) {
  return readJsonResponse(await fetch(`${QUEUE_BASE}/${encodeURIComponent(endpointId)}${pathname}`, {
    ...
    signal: AbortSignal.timeout(30_000),
  }), "AVANTIQO_RUNPOD_ACCOUNT_V2_QUEUE");
}
...
async function controlWorkers(endpointId, managementKey) {
  ...
  ), "AVANTIQO_RUNPOD_ACCOUNT_V2_CONTROL");
}
...
if (lock?.contract !== LOCK_CONTRACT) throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V2_LOCK_CONTRACT_MISMATCH");
if (lock?.generation_submitted !== true || Number(lock?.accepted_generation_count) !== 1) {
  throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V2_ONE_ACCEPTED_GENERATION_REQUIRED");
}
if (lock?.new_generation_allowed !== false || lock?.stt_submitted !== false) {
  throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V2_GENERATION_LOCK_REQUIRED");
}

const endpointId = text(lock.endpoint_id);
const jobId = text(lock.job_id);
if (!endpointId || !jobId) throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V2_LOCK_IDS_REQUIRED");
