...
const CONTRACT = "AVANTIQO_RUNPOD_SERVERLESS_ACCOUNT_DIAGNOSTIC_V3";
...
async function accountGraphql(managementKey) {
  const query = `
    query AvantiqoRunpodServerlessAccountDiagnosticV3 {
      myself {
        underBalance
        minBalance
  ...
  if (!response.ok || body?.errors?.length || !body?.data?.myself) {
    ...
    throw new Error(`AVANTIQO_RUNPOD_ACCOUNT_V3_GRAPHQL_FAILED:${response.status}:${detail || "INVALID_RESPONSE"}`);
  }
...
async function rest(pathname, managementKey) {
  return readJsonResponse(await fetch(`${REST_BASE}${pathname}`, {
    ...
    signal: AbortSignal.timeout(30_000),
  }), "AVANTIQO_RUNPOD_ACCOUNT_V3_REST");
}

async function queueRead(endpointId, credential, pathname) {
  return readJsonResponse(await fetch(`${QUEUE_BASE}/${encodeURIComponent(endpointId)}${pathname}`, {
    ...
    signal: AbortSignal.timeout(30_000),
  }), "AVANTIQO_RUNPOD_ACCOUNT_V3_QUEUE");
}

async function queueReadWithAvailableCredential(endpointId, managementKey, pathname) {
  const primary = text(process.env.RUNPOD_API_KEY);
  const candidates = [];
  if (primary) candidates.push({ source: "RUNPOD_API_KEY", value: primary });
  candidates.push({ source: "RUNPOD_MANAGEMENT_API_KEY_READ_FALLBACK", value: managementKey });
  ...
  for (const candidate of candidates) {
    ...
    try {
      const body = await queueRead(endpointId, candidate.value, pathname);
      return {
        ok: true,
        source: candidate.source,
        body,
        attempts: [...attempts, { source: candidate.source, authorized: true }],
      };
    } catch (error) {
  ...
  return {
    ...
    body: null,
    attempts,
    error: "NO_AVAILABLE_CREDENTIAL_AUTHORIZED_FOR_QUEUE_READ",
  };
}
...
async function controlWorkers(endpointId, managementKey) {
  ...
  ), "AVANTIQO_RUNPOD_ACCOUNT_V3_CONTROL");
}
...
if (lock?.contract !== LOCK_CONTRACT) throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V3_LOCK_CONTRACT_MISMATCH");
if (lock?.generation_submitted !== true || Number(lock?.accepted_generation_count) !== 1) {
  throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V3_ONE_ACCEPTED_GENERATION_REQUIRED");
}
if (lock?.new_generation_allowed !== false || lock?.stt_submitted !== false) {
  throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V3_GENERATION_LOCK_REQUIRED");
}

const endpointId = text(lock.endpoint_id);
const jobId = text(lock.job_id);
if (!endpointId || !jobId) throw new Error("AVANTIQO_RUNPOD_ACCOUNT_V3_LOCK_IDS_REQUIRED");

const managementKey = required("RUNPOD_MANAGEMENT_API_KEY");

const [account, endpointsRaw, exactWorkersRaw, statusRead, healthRead] = await Promise.all([
  ...
  queueReadWithAvailableCredential(endpointId, managementKey, `/status/${encodeURIComponent(jobId)}`),
  queueReadWithAvailableCredential(endpointId, managementKey, "/health"),
]);
...
const exactStatus = statusRead.ok
  ? (text(statusRead.body?.status).toUpperCase() || "UNKNOWN")
  : "UNREADABLE_WITH_AVAILABLE_CREDENTIALS";
const exactHealth = healthRead.ok ? healthSummary(healthRead.body) : null;
...
} else if (statusRead.ok && exactStatus !== "IN_QUEUE") {
  diagnosis = "EXISTING_ACCEPTED_JOB_LEFT_QUEUE";
  nextAction = "RESUME_EXISTING_ACCEPTED_JOB_ONLY";
} else if (
  exactControlWorkers.length > 0 ||
  (exactHealth && Object.values(exactHealth.workers).some((value) => value > 0))
) {
...
console.log(JSON.stringify({
  ...
  exact_voice_job: {
    ...
    status_read: {
      ok: statusRead.ok,
      credential_source: statusRead.source,
      authorization_attempts: statusRead.attempts,
      error: statusRead.error || null,
    },
    health: exactHealth,
    health_read: {
      ok: healthRead.ok,
      credential_source: healthRead.source,
      authorization_attempts: healthRead.attempts,
      error: healthRead.error || null,
    },
