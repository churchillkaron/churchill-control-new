...
const OPERATOR_APPROVAL="AVANTIQO_MUSIC_SFX_MATRIX_OPERATOR_APPROVAL_V1";
const text=(v)=>String(v??"").trim();
const arg=(p)=>text(process.argv.slice(2).find((e)=>e.startsWith(p))?.slice(p.length));
const operatorApprovalPath=arg("--operator-approval=");
const paths=arg("--pairs=").split(",").map((x)=>x.trim()).filter(Boolean);
if(!operatorApprovalPath&&paths.length!==6) throw new Error("AVANTIQO_MUSIC_SFX_MATRIX_REVIEW_EVIDENCE_REQUIRED");
const categories=new Set(); const items=[]; let totalThb=0;
if(operatorApprovalPath){
  const approval=JSON.parse(await readFile(resolve(operatorApprovalPath),"utf8"));
  if(approval?.contract!==OPERATOR_APPROVAL||approval?.decision!=="APPROVED"||approval?.human_review_status!=="APPROVED"||approval?.review_mode!=="EXPLICIT_OPERATOR_MATRIX_ATTESTATION"||approval?.numeric_scores_asserted!==false||Number(approval?.sample_count)!==6) throw new Error("AVANTIQO_MUSIC_SFX_MATRIX_OPERATOR_APPROVAL_INVALID");
  for(const item of approval.items||[]){ categories.add(text(item.category)); totalThb+=Number(item.cost_thb||0); items.push({...item,reviewer:text(approval.reviewer),review_mode:approval.review_mode}); }
}else for(const pair of paths){
...
const canonical=JSON.stringify(items.map(({job_id,category})=>({job_id,category})));
const evidenceFingerprint=crypto.createHash("sha256").update(canonical).digest("hex");
const out={success:true,contract:CONTRACT,generated_at:new Date().toISOString(),capability:"ai.sfx.generate",provider:"avantiqo-audio",product_model:"avantiqo-sfx-v1",foundation_model:"OpenMOSS-Team/MOSS-SoundEffect-v2.0",required_categories:[...MUSIC_SFX_CERTIFICATION_REQUIRED_CATEGORIES],sample_count:items.length,items,total_supplier_cost_thb:Number(totalThb.toFixed(6)),benchmark_certified:true,economics_certified:true,human_quality_certified:true,matrix_certified:true,model_license_verified:true,human_review_status:"APPROVED",human_review_mode:operatorApprovalPath?"EXPLICIT_OPERATOR_MATRIX_ATTESTATION":"SIX_SCORED_SAMPLE_REVIEWS",human_reviewers:[...new Set(items.map((x)=>x.reviewer).filter(Boolean))],numeric_scores_asserted:!operatorApprovalPath,human_review_minimum_score:operatorApprovalPath?null:Math.min(...items.map((x)=>x.minimum_score)),human_review_average_score:operatorApprovalPath?null:Number((items.reduce((s,x)=>s+x.average_score,0)/items.length).toFixed(2)),operator_approval_path:operatorApprovalPath?resolve(operatorApprovalPath):null,certification_evidence_sha256:evidenceFingerprint,production_routing_allowed:false,activation_allowed:false,pricing_activation_allowed:false,production_deployment_performed:false};
