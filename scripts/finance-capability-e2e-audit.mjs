...
function capabilityBlock(financeRegistry, id) {
  ...
  return financeRegistry.slice(start, next > start ? next : start + 9000);
}

function actionEntries(text) {
  if (!text) return [];
  const entries = [];
  ...
  while ((match = objectPattern.exec(text))) {
    ...
    const id = firstMatch(source, /\bid\s*:\s*["']([^"']+)["']/);
    const type = firstMatch(source, /\btype\s*:\s*["']([^"']+)["']/);
    const label = firstMatch(source, /\blabel\s*:\s*["']([^"']+)["']/);
    if (id || type || label) entries.push({ id, type, label });
  }
  return entries;
}
...
function registryDefinition(capabilityId, financeRegistry) {
  ...
  const createBlock = firstMatch(block, /create\s*:\s*\{([\s\S]*?)\}\s*,?\s*(?:ui|runtime|data|actions)\s*:/);
  const topMenuBlock = firstMatch(block, /topMenu\s*:\s*\[([\s\S]*?)\]/);
  const rowMenuBlock = firstMatch(block, /rowMenu\s*:\s*\[([\s\S]*?)\]/);
  const createEnabled = Boolean(createBlock && /enabled\s*:\s*true/.test(createBlock));
  return {
    createEnabled,
    createForm: createBlock ? firstMatch(createBlock, /\bform\s*:\s*["']([^"']+)["']/) : null,
    createTitle: createBlock ? firstMatch(createBlock, /\btitle\s*:\s*["']([^"']+)["']/) : null,
    createLabel: createBlock ? firstMatch(createBlock, /\blabel\s*:\s*["']([^"']+)["']/) : null,
    topActions: actionEntries(topMenuBlock),
    rowActions: actionEntries(rowMenuBlock),
  };
}
...
async function main() {
  ...
  try {
    console.log("============================================================");
    console.log("AVANTIQO FINANCE 67-CAPABILITY END-TO-END AUDIT");
    ...
    let capabilities = harness.resolveCapabilities();
    ...
    for (const [index, capability] of capabilities.entries()) {
      ...
      try {
        ...
        const api = await harness.fetchApi(client, capability.api, context);
    ...
    const report = {
      suite: "Avantiqo Finance 67-Capability End-to-End Acceptance Audit",
...
main().catch(error => {
  console.error("FINANCE 67-CAPABILITY E2E AUDIT FAILED");
