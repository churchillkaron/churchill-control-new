...
const CONTRACT = "AVANTIQO_VIDEO_OWNED_24_39GB_CAPACITY_INSPECTION_V68";
const REST = "https://rest.runpod.io/v1";
const GQL = "https://api.runpod.io/graphql";
const MIN_MEMORY_GB = 24;
const MAX_MEMORY_GB_EXCLUSIVE = 40;
const PREFERRED_MEMORY_GB = 32;
...
async function rest(path, key) {
  ...
  }), "AVANTIQO_VIDEO_V68_REST");
}
async function graphql(key) {
  const query = `
    query AvantiqoVideo24To39GbCapacity($input: GpuAvailabilityInput) {
  ...
  const body = await json(response, "AVANTIQO_VIDEO_V68_GRAPHQL");
  const errors = list(body.errors).map((entry) => text(entry?.message)).filter(Boolean);
  if (errors.length) {
    throw new Error(`AVANTIQO_VIDEO_V68_GRAPHQL_ERROR:${errors.join(" | ").slice(0, 500)}`);
  }
  return body.data || {};
}
...
for (const dc of list(capacity.dataCenters)) {
  ...
  for (const availability of list(dc.gpuAvailability)) {
    ...
    rows.push({
      ...
      memory_gb: memoryGb,
      optimization_tier: memoryGb >= PREFERRED_MEMORY_GB
        ? "PREFERRED_32_39GB"
        : "AGGRESSIVE_24_31GB",
...
const preferredRows = mediumOrHighRows.filter((row) => row.memory_gb >= PREFERRED_MEMORY_GB);
const preferredStorageRows = preferredRows.filter((row) => row.storage_support);
const candidateGpuTypes = [...new Map(
  mediumOrHighRows.map((row) => [row.gpu_type_id, {
    gpu_type_id: row.gpu_type_id,
    display_name: row.display_name,
    memory_gb: row.memory_gb,
    optimization_tier: row.optimization_tier,
...
let recommendation = "NO_MEDIUM_HIGH_24_39GB_SECURE_CAPACITY_VISIBLE";
if (attachedMediumOrHighRows.length) {
  recommendation = attachedMediumOrHighRows.some((row) => row.memory_gb >= PREFERRED_MEMORY_GB)
    ? "BUILD_OWNED_32_39GB_PROFILE_IN_EXISTING_CACHE_REGION"
    : "BUILD_OWNED_24_31GB_PROFILE_IN_EXISTING_CACHE_REGION";
} else if (preferredStorageRows.length) {
  recommendation = "BUILD_OWNED_32_39GB_PROFILE_AND_ADD_CACHE_VOLUME";
} else if (mediumOrHighStorageRows.length) {
  recommendation = "BUILD_OWNED_24_31GB_PROFILE_AND_ADD_CACHE_VOLUME";
...
console.log(JSON.stringify({
  ...
  candidate_contract: {
    ...
    quantization_or_offload_required_before_endpoint_use: true,
    quality_profile_must_be_recertified: true,
  },
  candidate_gpu_types: candidateGpuTypes,
  preferred_32_39gb_medium_or_high_rows: preferredRows,
  ...
  all_available_24_39gb_rows: rows,
  recommendation,
