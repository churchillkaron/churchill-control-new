...
const CONTRACT = "AVANTIQO_IMAGE_V6_RUNPOD_IMMUTABLE_ENDPOINT_MIGRATION_V1";
...
async function rest(pathname, credential, options = {}) {
  ...
  return readJson(response, "AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_REST");
}
...
async function queueHealth(endpointId, credential) {
  ...
  return readJson(response, "AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_QUEUE");
}
...
async function endpointBoundTemplates(managementKey) {
  ...
  if (!templates) throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_TEMPLATE_LIST_INVALID");
  return templates;
}
...
function templateById(templates, templateId, label) {
  ...
  if (matches.length !== 1) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_${label}_TEMPLATE_RESOLUTION_FAILED:${matches.length}`);
  }
  return matches[0];
}
...
async function readEvidence() {
  ...
  if (evidence?.success !== true || evidence?.contract !== IMAGE_EVIDENCE_CONTRACT) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_VALID_EVIDENCE_REQUIRED");
  }
  ...
  if (
    ...
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_SOURCE_LOCK_INVALID");
  }
  if (
    ...
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_SAFETY_EVIDENCE_INVALID");
  }
  ...
  if (!/^ghcr\.io\/churchillkaron\/avantiqo-image-worker@sha256:[a-f0-9]{64}$/i.test(immutableImage)) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_IMAGE_REFERENCE_INVALID");
  }
  return { evidence, sourceSha, immutableImage };
}

function assertImageSourceStable(sourceSha, immutableImage) {
  command("git", ["fetch", "origin", "main"], "AVANTIQO_IMAGE_V6_IMMUTABLE_GIT_FETCH_FAILED");
  const branch = command("git", ["branch", "--show-current"], "AVANTIQO_IMAGE_V6_IMMUTABLE_GIT_BRANCH_FAILED");
  if (branch !== "main") throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_MAIN_REQUIRED:${branch || "DETACHED"}`);

  const localRelevant = commandStatus(
    "git",
    ["diff", "--quiet", "--", IMAGE_SOURCE_PATH, IMAGE_EVIDENCE_PATH],
  );
  if (localRelevant.status === 1) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_LOCAL_RELEVANT_CHANGES_BLOCK");
  }
  if (localRelevant.status !== 0) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_LOCAL_RELEVANT_DIFF_FAILED");
  }

  const head = command("git", ["rev-parse", "HEAD"], "AVANTIQO_IMAGE_V6_IMMUTABLE_GIT_HEAD_FAILED");
  const originMain = command("git", ["rev-parse", "origin/main"], "AVANTIQO_IMAGE_V6_IMMUTABLE_GIT_ORIGIN_FAILED");
  const sourceExists = commandStatus("git", ["cat-file", "-e", `${sourceSha}^{commit}`]);
  if (sourceExists.status !== 0) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_SOURCE_COMMIT_MISSING:${sourceSha}`);
  }
  const sourceDiff = commandStatus("git", ["diff", "--quiet", sourceSha, originMain, "--", IMAGE_SOURCE_PATH]);
  if (sourceDiff.status === 1) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_IMAGE_SOURCE_MOVED:source=${sourceSha}:origin=${originMain}`);
  }
  if (sourceDiff.status !== 0) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_SOURCE_EQUIVALENCE_FAILED");
  }

  const originEvidence = JSON.parse(command(
    "git",
    ["show", `origin/main:${IMAGE_EVIDENCE_PATH}`],
    "AVANTIQO_IMAGE_V6_IMMUTABLE_ORIGIN_EVIDENCE_READ_FAILED",
  ));
  if (
    originEvidence?.success !== true ||
    originEvidence?.contract !== IMAGE_EVIDENCE_CONTRACT ||
    ...
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_IMAGE_EVIDENCE_MOVED_REPLAN_REQUIRED");
  }
  ...
  if (head !== originMain) {
    const relevantDiff = commandStatus(
      "git",
      ["diff", "--quiet", head, originMain, "--", IMAGE_SOURCE_PATH, IMAGE_EVIDENCE_PATH],
    );
    if (relevantDiff.status === 1) {
      throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_RELEVANT_MAIN_DRIFT_SYNC_REQUIRED");
    }
    if (relevantDiff.status !== 0) {
      throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_MAIN_DRIFT_INSPECTION_FAILED");
    }
...
async function prepareLocalEnvUpdate(newEndpointId, oldEndpointId) {
  ...
  return replace(
    replace(source, "RUNPOD_AVANTIQO_IMAGE_ENDPOINT_ID", newEndpointId),
    "RUNPOD_AVANTIQO_IMAGE_GITHUB_RETIRED_ENDPOINT_ID",
    oldEndpointId,
  );
}

async function atomicWriteLocalEnv(content) {
  const temp = `${ENV_PATH}.avantiqo-image-v6-migration-${process.pid}`;
...
async function loadRunPodState(managementKey) {
  ...
  if (!endpoints) throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_LIST_INVALID");
  if (!volumes) throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_VOLUME_LIST_INVALID");
...
function resolveCanonical(endpoints) {
  ...
  if (matches.length !== 1) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_CANONICAL_ENDPOINT_RESOLUTION_FAILED:${matches.length}`);
  }
  return matches[0];
}
...
function assertVolumeReady(endpoint, volumes) {
  ...
  if (!volumeIds.length) throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_NETWORK_VOLUME_REQUIRED");
  const attached = volumes.filter((volume) => volumeIds.includes(text(volume?.id)));
  if (!attached.some((volume) => finite(volume?.size, 0) >= MIN_NETWORK_VOLUME_GB)) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_NETWORK_VOLUME_TOO_SMALL:min_gb=${MIN_NETWORK_VOLUME_GB}`);
  }
  return attached.map((volume) => ({
    id: text(volume?.id),
...
function assertEndpointParity(candidate, canonical) {
  if (endpointContractKey(candidate) !== endpointContractKey(canonical)) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_CONTRACT_DRIFT");
  }
}

const apply = process.argv.includes("--apply");
if (apply) approved("AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_MIGRATION_APPROVED");
...
if (!pullProof.public_pull) {
  throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_PUBLIC_PULL_REQUIRED:status=${pullProof.manifest_status ?? "NONE"}`);
}
...
const canonicalConsumers = runpod.endpoints.filter(
  (entry) => text(entry?.templateId || entry?.template?.id) === canonicalTemplateId,
);
if (canonicalConsumers.length !== 1 || text(canonicalConsumers[0]?.id) !== canonicalId) {
  throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_CANONICAL_TEMPLATE_SHARED:${canonicalConsumers.length}`);
}
...
const canonicalIdle = assertIdle(
  canonicalQueue,
  canonicalManagement,
  "AVANTIQO_IMAGE_V6_IMMUTABLE_CANONICAL",
);
...
if (
  ...
  if (apply && retiredId) {
    await atomicWriteLocalEnv(await prepareLocalEnvUpdate(canonicalId, retiredId));
  }
  console.log(JSON.stringify({
    ...
    template_name: text(canonicalTemplate.name),
    immutable_image: text(canonicalTemplate.imageName),
    network_volumes: attachedVolumes,
    idle_guard: canonicalIdle,
  ...
  console.log(`AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_MIGRATION_APPLIED=${apply ? "true" : "false"}`);
  process.exit(0);
}

const targetMatches = runpod.templates.filter((entry) => text(entry?.name) === targetTemplateName);
if (targetMatches.length > 1) {
  throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_TARGET_TEMPLATE_AMBIGUOUS:${targetMatches.length}`);
}
const candidateMatches = runpod.endpoints.filter((entry) => text(entry?.name) === candidateName);
if (candidateMatches.length > 1) {
  throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_AMBIGUOUS:${candidateMatches.length}`);
}
const retiredCollisions = runpod.endpoints.filter(
  (entry) => text(entry?.name) === retiredName && text(entry?.id) !== canonicalId,
);
if (retiredCollisions.length) {
  throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_RETIRED_NAME_COLLISION:${retiredCollisions.length}`);
}
...
const plan = {
  ...
  current_endpoint: {
    ...
    template_id: canonicalTemplateId,
    template_name: text(canonicalTemplate.name),
    image_name: text(canonicalTemplate.imageName),
    workers_min: finite(canonical.workersMin),
    workers_max: finite(canonical.workersMax),
    gpu_type_ids: list(canonical.gpuTypeIds).map(text).filter(Boolean),
    network_volume_ids: endpointVolumeIds(canonical),
  },
  target: {
    template_name: targetTemplateName,
    existing_template_found: targetMatches.length === 1,
    candidate_endpoint_name: candidateName,
    existing_candidate_found: candidateMatches.length === 1,
    bootstrap_workers_max: 1,
    candidate_pre_cutover_workers_max: 0,
    final_workers_max: finite(canonical.workersMax, 1),
  },
  network_volumes: attachedVolumes,
  queue: canonicalQueue,
  management_workers: canonicalManagement,
  idle_guard: canonicalIdle,
  ...
  safety: {
    ...
    stale_initializing_requires_zero_non_exited_management_workers: true,
...
if (!apply) {
  console.log("AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_MIGRATION_APPLIED=false");
  process.exit(0);
}
...
if (!targetTemplate) {
  targetTemplate = await rest("/templates", managementKey, {
    method: "POST",
    body: {
      ...templateBody(canonicalTemplate, immutableImage, targetTemplateName),
      category: "NVIDIA",
      isServerless: true,
    },
  });
  runpod = await loadRunPodState(managementKey);
  const createdMatches = runpod.templates.filter((entry) => text(entry?.name) === targetTemplateName);
  if (createdMatches.length !== 1) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_CREATED_TEMPLATE_RESOLUTION_FAILED:${createdMatches.length}`);
  }
  targetTemplate = createdMatches[0];
} else if (
  text(targetTemplate.imageName) !== immutableImage ||
  templateContractKey(targetTemplate) !== canonicalTemplateContract
) {
  const targetId = text(targetTemplate.id);
  const targetConsumers = runpod.endpoints.filter(
  ...
  if (foreignConsumers.length) {
    throw new Error(`AVANTIQO_IMAGE_V6_IMMUTABLE_TARGET_TEMPLATE_IN_USE:${foreignConsumers.length}`);
  }
  await rest(`/templates/${encodeURIComponent(targetId)}/update`, managementKey, {
    method: "POST",
    body: templateBody(canonicalTemplate, immutableImage, targetTemplateName),
  });
  runpod = await loadRunPodState(managementKey);
  targetTemplate = templateById(runpod.templates, targetId, "UPDATED_TARGET");
}

const targetTemplateId = text(targetTemplate.id);
if (!targetTemplateId) throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_TARGET_TEMPLATE_ID_REQUIRED");
if (text(targetTemplate.imageName) !== immutableImage) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_TARGET_TEMPLATE_IMAGE_VERIFY_FAILED");
}
if (templateContractKey(targetTemplate) !== canonicalTemplateContract) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_TARGET_TEMPLATE_CONTRACT_VERIFY_FAILED");
}
...
if (!candidate) {
  candidate = await rest("/endpoints", managementKey, {
    method: "POST",
    body: endpointBody(canonical, targetTemplateId, candidateName, 1),
  });
}
const candidateId = text(candidate.id);
if (!candidateId) throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_ID_REQUIRED");
...
if (!candidate || text(candidate.name) !== candidateName) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_RESOLUTION_FAILED");
}
if (text(candidate.templateId || candidate.template?.id) !== targetTemplateId) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_TEMPLATE_BINDING_FAILED");
}
assertEndpointParity(candidate, canonical);
if (finite(candidate.workersMin, -1) !== 0 || finite(candidate.workersMax, -1) !== 0) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_ZERO_SCALE_FAILED");
}
...
const candidateIdle = assertIdle(
  candidateQueue,
  candidateManagement,
  "AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE",
);
...
candidate = runpod.endpoints.find((entry) => text(entry?.id) === candidateId);
if (!canonical || text(canonical.name) !== IMAGE_ENDPOINT_NAME) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANONICAL_MOVED_REPLAN_REQUIRED");
}
if (!candidate || text(candidate.name) !== candidateName) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_MOVED_REPLAN_REQUIRED");
}
const freshCanonicalTemplateId = text(canonical.templateId || canonical.template?.id);
const freshCanonicalTemplate = templateById(runpod.templates, freshCanonicalTemplateId, "FRESH_CANONICAL");
if (freshCanonicalTemplateId !== canonicalTemplateId) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANONICAL_TEMPLATE_MOVED_REPLAN_REQUIRED");
}
if (templateContractKey(freshCanonicalTemplate) !== canonicalTemplateContract) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANONICAL_TEMPLATE_CONTRACT_MOVED_REPLAN_REQUIRED");
}
const freshTargetTemplate = templateById(runpod.templates, targetTemplateId, "FRESH_TARGET");
if (
  text(freshTargetTemplate.name) !== targetTemplateName ||
  text(freshTargetTemplate.imageName) !== immutableImage ||
  templateContractKey(freshTargetTemplate) !== canonicalTemplateContract
) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_TARGET_TEMPLATE_MOVED_REPLAN_REQUIRED");
}
if (text(candidate.templateId || candidate.template?.id) !== targetTemplateId) {
  throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_CANDIDATE_TEMPLATE_MOVED_REPLAN_REQUIRED");
}
assertEndpointParity(candidate, canonical);
assertVolumeReady(canonical, runpod.volumes);
assertVolumeReady(candidate, runpod.volumes);

const freshCanonicalQueue = queueSummary(await queueHealth(canonicalId, queueKey));
const freshCandidateQueue = queueSummary(await queueHealth(candidateId, queueKey));
const freshCanonicalManagement = managementSummary(canonical);
const freshCandidateManagement = managementSummary(candidate);
const freshCanonicalIdle = assertIdle(
  freshCanonicalQueue,
  freshCanonicalManagement,
  "AVANTIQO_IMAGE_V6_IMMUTABLE_FRESH_CANONICAL",
);
const freshCandidateIdle = assertIdle(
  freshCandidateQueue,
  freshCandidateManagement,
  "AVANTIQO_IMAGE_V6_IMMUTABLE_FRESH_CANDIDATE",
);
...
try {
  await rest(`/endpoints/${encodeURIComponent(canonicalId)}`, managementKey, {
    method: "PATCH",
  ...
  await rest(`/endpoints/${encodeURIComponent(candidateId)}`, managementKey, {
    method: "PATCH",
    body: { name: IMAGE_ENDPOINT_NAME, workersMin: originalWorkersMin, workersMax: originalWorkersMax },
  });
  ...
  if (!verifiedOld || !verifiedNew) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_FINAL_ENDPOINT_RESOLUTION_FAILED");
  }
  if (
    text(verifiedOld.name) !== retiredName ||
    finite(verifiedOld.workersMin, -1) !== 0 ||
    finite(verifiedOld.workersMax, -1) !== 0
  ) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_RETIRED_ENDPOINT_VERIFY_FAILED");
  }
  if (
    text(verifiedNew.name) !== IMAGE_ENDPOINT_NAME ||
    finite(verifiedNew.workersMin, -1) !== originalWorkersMin ||
    finite(verifiedNew.workersMax, -1) !== originalWorkersMax
  ) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_NEW_CANONICAL_VERIFY_FAILED");
  }
  const verifiedTemplateId = text(verifiedNew.templateId || verifiedNew.template?.id);
  const verifiedTemplate = templateById(runpod.templates, verifiedTemplateId, "FINAL");
  if (
    verifiedTemplateId !== targetTemplateId ||
    text(verifiedTemplate.name) !== targetTemplateName ||
    text(verifiedTemplate.imageName) !== immutableImage
  ) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_FINAL_TEMPLATE_VERIFY_FAILED");
  }
  if (templateContractKey(verifiedTemplate) !== canonicalTemplateContract) {
    throw new Error("AVANTIQO_IMAGE_V6_IMMUTABLE_FINAL_TEMPLATE_CONTRACT_VERIFY_FAILED");
  }
  assertEndpointParity(verifiedNew, canonical);
  ...
  console.log(JSON.stringify({
    ...
    new_endpoint: {
      ...
      gpu_type_ids: list(verifiedNew.gpuTypeIds).map(text).filter(Boolean),
      endpoint_contract_preserved: true,
      network_volumes: verifiedVolumes,
    },
    idle_guard: {
      canonical_before_cutover: freshCanonicalIdle,
      candidate_before_cutover: freshCandidateIdle,
      stale_initializing_tolerated: freshCanonicalIdle.stale_initializing_tolerated,
    },
  ...
  console.log("AVANTIQO_IMAGE_V6_IMMUTABLE_ENDPOINT_MIGRATION_APPLIED=true");
} catch (error) {
  ...
  if (candidateCutover) {
    try {
      await rest(`/endpoints/${encodeURIComponent(candidateId)}`, managementKey, {
        method: "PATCH",
        body: { name: candidateName, workersMin: 0, workersMax: 0 },
      });
    } catch (rollbackError) {
