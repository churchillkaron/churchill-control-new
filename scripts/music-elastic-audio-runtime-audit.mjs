...
assert.match(source.worker, /AVANTIQO_MUSIC_ELASTIC_RUNTIME_PROBE_V1/);
...
assert.match(source.worker, /def _runtime_probe\(\)/);
assert.match(source.worker, /importlib\.metadata\.version\("python-stretch"\)/);
assert.match(source.worker, /\["ffmpeg", "-version"\]/);
assert.match(source.worker, /stretch = ps\.Signalsmith\.Stretch\(\)/);
assert.match(source.worker, /stretch\.preset\(2, 48000\)/);
assert.match(source.worker, /source_download_performed.*False/);
assert.match(source.worker, /render_performed.*False/);
assert.match(source.worker, /output_upload_performed.*False/);
assert.match(source.worker, /automatic_apply_performed.*False/);
assert.match(source.worker, /provider_job_submitted.*False/);
assert.match(source.worker, /production_certified.*False/);
assert.match(source.worker, /human_listening_review_required.*True/);

const handlerStart = source.worker.indexOf("def handler(job):");
const probeBranch = source.worker.indexOf('if _text(data.get("mode")).lower() == "runtime_probe":', handlerStart);
const sourceUrlRead = source.worker.indexOf('source_url = _text(data.get("source_audio_url"))', handlerStart);
const tempDirectory = source.worker.indexOf('with tempfile.TemporaryDirectory(prefix="avantiqo-elastic-")', handlerStart);
const sourceDownload = source.worker.indexOf("source_bytes = _download(source_url, downloaded)", handlerStart);
assert.ok(handlerStart >= 0, "Elastic worker handler missing");
assert.ok(probeBranch > handlerStart, "Elastic runtime probe branch missing from handler");
assert.ok(sourceUrlRead > probeBranch, "Elastic runtime probe must branch before source URL handling");
assert.ok(tempDirectory > sourceUrlRead, "Elastic render temp workspace unexpectedly reordered");
assert.ok(sourceDownload > tempDirectory, "Elastic source download unexpectedly reordered");
...
console.log("AVANTIQO_MUSIC_ELASTIC_AUDIO_RUNTIME_PROBE=V1");
console.log("AVANTIQO_MUSIC_ELASTIC_AUDIO_RUNTIME_PROBE_NO_INFERENCE=true");
console.log("AVANTIQO_MUSIC_ELASTIC_AUDIO_RUNTIME_PROBE_SOURCE_DOWNLOAD=false");
console.log("AVANTIQO_MUSIC_ELASTIC_AUDIO_RUNTIME_PROBE_RENDER=false");
console.log("AVANTIQO_MUSIC_ELASTIC_AUDIO_RUNTIME_PROBE_UPLOAD=false");
...
console.log("AVANTIQO_MUSIC_ELASTIC_AUDIO_HUMAN_LISTENING_REVIEW_REQUIRED=true");
