...
assert.match(reasoningSource, /current_project_state:\s*normalizeOperatorProjectState\(projectState\)/);
assert.match(reasoningSource, /user_confirmed_complete/);
assert.match(reasoningSource, /awaiting_confirmation/);
assert.match(reasoningSource, /assistant-only recommendation is not yet a decision/i);
...
assert.match(reasoningSource, /routeOperatorCognition\(\{ message, source, capabilities \}\)/);
...
assert.match(verificationSource, /evidence_compaction:\s*"collection-aware-dynamic-v2"/);
...
assert.doesNotMatch(readChainSource, /executeUbteCapability\([^)]*followUp/s);
...
assert.match(readResolverSource, /const OPERATOR_READ_CHAIN_KEY = "platform\.operator_read_chain\.execute"/);
assert.match(readResolverSource, /function diverseReads/);
assert.match(readResolverSource, /const groups = new Map\(\)/);
assert.match(readResolverSource, /const group = text\(capability\?\.domain\) \|\| "_"/);
assert.match(readResolverSource, /function readChainCapability/);
assert.doesNotMatch(readResolverSource, /revenue|profit|inventory|receivable|payable|payroll|customer|supplier|hotel|restaurant/i);
...
assert.doesNotMatch(dataReflexSource, /date_from|date_to|from_date|to_date|start_date|end_date|\btoday\b|\byesterday\b/i);
...
console.log("OPERATOR_BUSINESS_PARTNER=RECOMMEND_DECIDE_EXECUTE_VERIFY");
