...
assert.match(runtime, /SECRETARY_VISITOR_COORDINATION_\$\{role\}_EVIDENCE_REQUIRED/);
assert.match(runtime, /confirmed !== true && confirmed !== false/);
...
assert.match(runtime, /task_patch:\s*terminal\s*\?\s*\{\s*status:\s*"DONE"/);
