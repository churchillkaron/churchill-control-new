...
const CONTRACT = "AVANTIQO_CODE_AI_PLANNER_SAFE_READ_RETRY_PATCH_V1";
...
const delayBefore = `function delay(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

async function responseJson(response) {`;
const delayAfter = `function delay(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
...
const preflightBefore = `  let response;
  try {
...
const preflightAfter = `  let response;
  try {
...
const queueBefore = `async function runpodQueueJson(endpointId, pathname, options = {}) {
...
const queueAfter = `async function runpodQueueJson(endpointId, pathname, options = {}) {
  const apiKey = text(process.env.RUNPOD_API_KEY);
  if (!apiKey) throw new Error("RUNPOD_CODE_QUEUE_CREDENTIAL_REQUIRED");
...
const assertionsBefore = `assert.match(plannerExecution, /stale_queue_recovery_count/);
assert.match(autonomousRuntime, /const logicalIterations = new Set\\(\\)/);`;
const assertionsAfter = `assert.match(plannerExecution, /stale_queue_recovery_count/);
...
const verifiedBefore = `    stale_queued_provider_job_detected_by_age_and_health: true,
    stale_queued_provider_job_exact_cancel_before_replacement: true,`;
const verifiedAfter = `    stale_queued_provider_job_detected_by_age_and_health: true,
...
console.log(JSON.stringify({
  ...
  runpod_safe_read_attempts: RUNPOD_HEALTH_MAX_ATTEMPTS,
