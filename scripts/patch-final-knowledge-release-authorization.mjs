...
const file = "lib/intelligence/runtime/AvantiqoFinalKnowledgeReleaseRuntime.js";
...
const helperAnchor = `async function writeEvent(row) {
...
`;
const helperReplacement = `${helperAnchor}async function loadFinalReleaseAuthorization(organizationId, authorizationMemoryKey) {
  const key = text(authorizationMemoryKey, 240);
  if (!key.startsWith("final-knowledge-release-authorization:")) {
    throw new Error(
      \\`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_REQUIRED\\`,
    );
  }
  const result = await supabaseAdmin
    .from(MEMORY_TABLE)
  ...
  if (!result.data?.id) {
    throw new Error(
      \\`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_NOT_FOUND\\`,
    );
  }
  return result.data;
}
...
async function consumeFinalReleaseAuthorization({ organizationId, authorization, nowIso }) {
  ...
  if (!authorizationId) {
    throw new Error(
      \\`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_ID_REQUIRED\\`,
    );
  }
  const consumptionRow = {
    ...
    metadata: {
      ...
      authorization_signature_key_id:
        text(metadata.release_authorization_signature_key_id, 80) || null,
  ...
  if (consumption.error) {
    if (String(consumption.error?.code || "") === "23505") {
      throw new Error(
        \\`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_ALREADY_CONSUMED\\`,
      );
    }
  ...
  if (!consumption.data?.id) {
    throw new Error(
      \\`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_CONSUMPTION_FAILED\\`,
    );
  }
  ...
  if (!consumed.data?.id) {
    throw new Error(
      \\`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_OPTIMISTIC_CONSUMPTION_CONFLICT\\`,
    );
  }
  return {
    authorization_id: authorizationId,
...
`;
replaceExact(helperAnchor, helperReplacement, "authorization helpers");
...
`  if (!verifyAvantiqoFinalPromotionCandidateClaimBinding(state.candidate, state.provisional)) {
  ...
  const authorization = await loadFinalReleaseAuthorization(
    organizationId,
    authorization_memory_key,
  );
  ...
  if (authorizationVerifier.available !== true) {
    throw new Error(
      \`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_PUBLIC_KEYRING_REQUIRED\`,
    );
  }
  if (!authorizationVerifier.verify(authorization)) {
    throw new Error(
      \`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_INVALID_OR_EXPIRED\`,
    );
  }
  if (!verifyAvantiqoFinalKnowledgeReleaseAuthorizationBinding(
    authorization,
    state.candidate,
    state.provisional,
  )) {
    throw new Error(
      \`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_BINDING_MISMATCH\`,
    );
  }
  ...
  if (!approvalReason) {
    throw new Error(
      \`${AVANTIQO_FINAL_KNOWLEDGE_RELEASE_CONTRACT}_SIGNED_RELEASE_AUTHORIZATION_REASON_REQUIRED\`,
    );
  }
...
`,
`      approval_reason: text(approvalReason, 800),
      final_release_authorization_contract:
        AVANTIQO_FINAL_KNOWLEDGE_RELEASE_AUTHORIZATION_CONTRACT,
      final_release_authorization_id:
        text(authorizationConsumption?.authorization_id, 128) || null,
      final_release_authorization_consumption_memory_key:
        text(authorizationConsumption?.consumption_memory_key, 240) || null,
      final_release_authorized_by:
        text(authorizationConsumption?.approver_id, 160) || null,
...
`        production_knowledge_release_authorized: true,
        production_knowledge_release_authorization_contract:
          AVANTIQO_FINAL_KNOWLEDGE_RELEASE_AUTHORIZATION_CONTRACT,
        production_knowledge_release_authorization_id:
          authorizationConsumption.authorization_id,
...
console.log("AVANTIQO_FINAL_KNOWLEDGE_RELEASE_AUTHORIZATION_WIRING_PATCHED");
