...
const CONTRACT = "AVANTIQO_MUSIC_ELASTIC_CHURCHILL_EXECUTION_BINDING_PLAN_V1";
...
const endpointId = text(process.env.RUNPOD_AVANTIQO_MUSIC_ELASTIC_ENDPOINT_ID);
const runpodKeyConfigured = Boolean(text(process.env.RUNPOD_API_KEY));
...
for (const [name, result] of Object.entries({
  ...
  churchill_binding: churchillBindingResult,
  global_binding: globalBindingResult,
  reference_service: referenceServiceResult,
  wallet: walletResult,
})) {
...
requireCheck("ELASTIC_ENGINE_ENABLED_REQUIRED", engineEnabled === true);
requireCheck("ELASTIC_ENGINE_CERTIFIED_REQUIRED", engineCertified === true);
requireCheck("ELASTIC_ENDPOINT_MISMATCH", endpointId === EXPECTED_ENDPOINT_ID);
requireCheck("RUNPOD_API_KEY_CONFIGURATION_REQUIRED", runpodKeyConfigured === true);
...
const proposedBinding = {
  ...
  metadata: {
    ...
    production_deploy_required_for_binding: false,
  },
};
...
if (churchillBindings.length === 1) {
  ...
  const exactCompatible = (
    ...
    existing.usage_enabled !== false &&
    existing.billing_enabled !== false &&
    text(existing.managed_by).toLowerCase() === "avantiqo" &&
    text(existing.entity_id) === text(referenceService.entity_id) &&
    text(existing.default_provider_id) === PROVIDER &&
    existing.fallback_enabled === false &&
    text(existing.default_currency).toUpperCase() === CURRENCY &&
    text(existing.default_model) === MODEL &&
    existingMetadata.execution_binding_only === true &&
    existingMetadata.customer_entitlement_gate === false &&
...
const output = {
  ...
  platform_entitlement: {
    ...
    organization_row_is_entitlement_gate: false,
  },
  execution_binding: {
  ...
  runtime: {
    ...
    endpoint_matches_certified: endpointId === EXPECTED_ENDPOINT_ID,
    runpod_key_configured: runpodKeyConfigured,
  ...
  pricing: {
    ...
    unit: pricing.unit,
    supplier_cost_per_second: finite(pricing.cost_per_unit, null),
    customer_price_per_second: oneSecondCustomerPrice,
    reviewer,
  ...
  wallet: {
    ...
    reserved_balance: finite(wallet.reserved_balance, 0),
  ...
  next_action: planReady
    ? proposedOperation === "NOOP"
      ? "AUDIT_CHURCHILL_ELASTIC_RUNTIME_READINESS"
      : "BUILD_EXPLICIT_CHURCHILL_ELASTIC_EXECUTION_BINDING_APPLY_GATE"
    : "RESOLVE_CHURCHILL_ELASTIC_EXECUTION_BINDING_PLAN_BLOCKERS",
};

const outputPath = path.join(
  os.tmpdir(),
  `avantiqo-music-elastic-churchill-execution-binding-plan-${Date.now()}.json`,
);
fs.writeFileSync(outputPath, `${JSON.stringify(output, null, 2)}\n`, "utf8");

console.log(JSON.stringify({ ...output, output_path: outputPath }, null, 2));
console.log(`AVANTIQO_MUSIC_ELASTIC_CHURCHILL_EXECUTION_BINDING_PLAN=${planReady ? "PASS" : "BLOCKED"}`);
console.log(`AVANTIQO_MUSIC_ELASTIC_CHURCHILL_EXECUTION_BINDING_PLAN_READY=${planReady}`);
...
console.log(`AVANTIQO_MUSIC_ELASTIC_CHURCHILL_EXECUTION_BINDING_PLAN_OUTPUT=${outputPath}`);
