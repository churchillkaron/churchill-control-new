...
function endpointVolumeIds(endpoint = {}) {
  return [...new Set([
    ...
    ...(Array.isArray(endpoint.networkVolumeIds ?? endpoint.network_volume_ids) ? (endpoint.networkVolumeIds ?? endpoint.network_volume_ids).map(text) : []),
  ].filter(Boolean))];
}
...
async function resolveEndpointTemplate(endpoint, managementKey) { const inline = object(endpoint?.template); if (Object.keys(inline).length) return inline; const templateId = text(endpoint?.templateId); const templates = await runpodRest("/templates?includeEndpointBoundTemplates=true&includePublicTemplates=false&includeRunpodTemplates=false", managementKey); const matches = templates.filter((template) => text(template?.id) === templateId); if (matches.length !== 1) throw new Error(`AVANTIQO_MUSIC_PREFLIGHT_TEMPLATE_RESOLUTION_FAILED:matches=${matches.length}`); return matches[0]; }
...
if (text(endpoint?.name) !== AUDIO_ENDPOINT_NAME) throw new Error(`AVANTIQO_MUSIC_PREFLIGHT_ENDPOINT_NAME_MISMATCH:${text(endpoint?.name)}`);
...
if (attachedVolumes.length !== attachedVolumeIds.length) throw new Error("AVANTIQO_MUSIC_PREFLIGHT_ATTACHED_VOLUME_LOOKUP_FAILED");
const wrongSharedGroup = attachedVolumes.filter((volume) => classifyManagedVolumeName(volume?.name)?.id !== AUDIO_VOICE_GROUP.id);
if (wrongSharedGroup.length) throw new Error("AVANTIQO_MUSIC_PREFLIGHT_ATTACHED_VOLUME_WRONG_SHARED_GROUP");
...
if (!attachedVolumeIds.includes(text(reusableAudioVoiceVolume.volume.id))) throw new Error("AVANTIQO_MUSIC_PREFLIGHT_SHARED_AUDIO_VOICE_VOLUME_NOT_ATTACHED");
...
const invalidEnv = Object.entries(requiredTemplateEnv).filter(([key, expected]) => env[key] !== expected).map(([key]) => key);
...
console.log(JSON.stringify({ success: true, contract: "AVANTIQO_MUSIC_LOCAL_PREFLIGHT_V2", endpoint: { configured: true, exact_audio_identity: true, workers_min: 0, scale_to_zero: true, gpu_type_ids: gpuEconomics.gpu_type_ids }, model_cache: { network_volume_attached: true, attached_volume_count: attachedVolumeIds.length, shared_volume_group: AUDIO_VOICE_GROUP.id, shared_volume_name: text(reusableAudioVoiceVolume.volume.name), persistent: true }, model_contract: { foundation_model: EXPECTED_FOUNDATION_MODEL, variant: EXPECTED_VARIANT }, safety: { read_only_except_signed_url_creation: true, runpod_generation_jobs_submitted: 0, endpoint_mutations_performed: 0, production_deploy_performed: false } }, null, 2));
