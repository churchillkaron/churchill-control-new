...
console.log(JSON.stringify({ success: true, contract: "AVANTIQO_MUSIC_LOCAL_PREFLIGHT_V2", endpoint: { configured: true, exact_audio_identity: true, workers_min: 0, scale_to_zero: true, quiet_for_controlled_benchmark: true, gpu_type_ids: gpuEconomics.gpu_type_ids }, model_cache: { shared_volume_group: AUDIO_VOICE_GROUP.id, shared_volume_name: text(reusableAudioVoiceVolume.volume.name), persistent: true }, model_contract: { foundation_model: EXPECTED_FOUNDATION_MODEL, variant: EXPECTED_VARIANT, certified_capabilities: ["ai.music.generate"] }, economics: gpuEconomics, ready_for_controlled_benchmark: true, safety: { runpod_generation_jobs_submitted: 0, endpoint_mutations_performed: 0, production_deploy_performed: false, secret_values_printed: false } }, null, 2));
