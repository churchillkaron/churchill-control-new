...
const POLL_MS = 5_000;
const MAX_WAIT_MS = Math.max(60_000, Number(process.env.AVANTIQO_IMAGE_V8_PROBE_TIMEOUT_MS || 12 * 60 * 1000));
...
async function selectQueueCredential(endpointId, managementKey) {
  ...
  throw new Error("AVANTIQO_IMAGE_V8_PROBE_QUEUE_CREDENTIAL_NOT_FOUND");
}
async function endpointBoundTemplates(managementKey) {
  ...
  if (!templates) throw new Error("AVANTIQO_IMAGE_V8_PROBE_TEMPLATE_LIST_INVALID");
  return templates;
}
...
async function readEvidence() {
  ...
  ) throw new Error("AVANTIQO_IMAGE_V8_PROBE_EVIDENCE_NOT_READY");
  ...
  if (!/^ghcr\.io\/churchillkaron\/avantiqo-image-worker@sha256:[a-f0-9]{64}$/i.test(immutableImage)) {
    throw new Error("AVANTIQO_IMAGE_V8_PROBE_IMMUTABLE_IMAGE_INVALID");
  }
...
function resolveEndpoint(endpoints, configuredId) {
  ...
  if (matches.length !== 1) throw new Error(`AVANTIQO_IMAGE_V8_PROBE_ENDPOINT_RESOLUTION_FAILED:${matches.length}`);
  return matches[0];
}
function resolveTemplate(templates, templateId) {
  const matches = templates.filter((entry) => text(entry?.id) === templateId);
  if (matches.length !== 1) throw new Error(`AVANTIQO_IMAGE_V8_PROBE_TEMPLATE_RESOLUTION_FAILED:${matches.length}`);
  return matches[0];
}
...
async function cancelJob(endpointId, jobId, apiKey) {
  ...
  try {
    ...
    console.log(`AVANTIQO_IMAGE_V8_PROBE_CANCELLED_JOB=${jobId}`);
  } catch (error) {
    console.log(`AVANTIQO_IMAGE_V8_PROBE_CANCEL_FAILED=${text(error?.message).slice(0, 300)}`);
  }
}
async function runOperation(endpointId, apiKey, input, label) {
  ...
  try {
    ...
    throw new Error(`AVANTIQO_IMAGE_V8_${label}_SUBMIT_RESULT_UNKNOWN_DO_NOT_RETRY:${text(error?.message).slice(0, 500)}`);
  }
  const jobId = text(submitted?.id);
  if (!jobId) throw new Error(`AVANTIQO_IMAGE_V8_${label}_JOB_ID_MISSING_DO_NOT_RETRY`);
  console.log(`AVANTIQO_IMAGE_V8_${label}_JOB_ID=${jobId}`);
  let body = submitted;
  while (Date.now() - started < MAX_WAIT_MS) {
    const status = text(body?.status).toUpperCase();
    if (status === "COMPLETED") return { body, jobId };
    if (terminalFailure(status)) throw new Error(`AVANTIQO_IMAGE_V8_${label}_FAILED:${status}:${text(body?.error).slice(0, 500)}`);
    await sleep(POLL_MS);
    body = await queue(endpointId, `/status/${encodeURIComponent(jobId)}`, apiKey);
    console.log(JSON.stringify({
      event: "AVANTIQO_IMAGE_V8_PROBE_PROGRESS",
      label,
      job_id: jobId,
  ...
  throw new Error(`AVANTIQO_IMAGE_V8_${label}_TIMEOUT_CANCELLED:${MAX_WAIT_MS}`);
}
function validateRuntime(body) {
  const output = object(body?.output);
  if (
    text(output.status) !== "completed" ||
    text(output.engine_contract) !== ENGINE_CONTRACT ||
    text(output.probe_contract) !== EXPECTED_PROBE_CONTRACT ||
    text(output.operation) !== RUNTIME_OPERATION
  ) throw new Error("AVANTIQO_IMAGE_V8_RUNTIME_BASE_CONTRACT_INVALID");
  if (
    text(output.entrypoint) !== EXPECTED_ENTRYPOINT ||
    text(output.entrypoint_revision) !== EXPECTED_ENTRYPOINT_REVISION ||
    text(output.runtime_revision) !== EXPECTED_RUNTIME
  ) throw new Error("AVANTIQO_IMAGE_V8_RUNTIME_REVISION_INVALID");
  assertFalse(output.generation_requested, "AVANTIQO_IMAGE_V8_RUNTIME_GENERATION_FORBIDDEN");
  assertFalse(output.inference_performed, "AVANTIQO_IMAGE_V8_RUNTIME_INFERENCE_FORBIDDEN");
  assertFalse(output.model_download_performed, "AVANTIQO_IMAGE_V8_RUNTIME_DOWNLOAD_FORBIDDEN");
  assertFalse(output.storage_mutation_performed, "AVANTIQO_IMAGE_V8_RUNTIME_STORAGE_MUTATION_FORBIDDEN");
  const guard = object(output.volume_quota_guard);
  if (
    text(guard.contract) !== EXPECTED_QUOTA_GUARD ||
    text(guard.physical_usage_contract) !== EXPECTED_PHYSICAL_USAGE ||
    text(guard.allocation_decision_basis) !== EXPECTED_ALLOCATION_BASIS ||
    guard.backing_filesystem_capacity_used_for_decision !== false
  ) throw new Error("AVANTIQO_IMAGE_V8_RUNTIME_PHYSICAL_GUARD_INVALID");
  const candidate = object(output.photoreal_candidate);
  if (
    text(candidate.foundation_model) !== TARGET_MODEL ||
    text(candidate.quality_profile) !== EXPECTED_PROFILE ||
    text(candidate.quality_policy) !== EXPECTED_POLICY ||
    text(candidate.quality_compiler_contract) !== EXPECTED_COMPILER ||
    text(candidate.antitext_policy_contract) !== EXPECTED_ANTITEXT ||
    candidate.negative_policy_applied !== true ||
    candidate.antitext_policy_applied !== true ||
    Number(candidate.default_inference_steps) !== 28 ||
    Number(candidate.default_guidance_scale) !== 4 ||
    candidate.prompt_rewrite_applied !== false ||
    candidate.positive_constraint_suffix_applied !== false ||
    candidate.automatic_production_routing_enabled !== false
  ) throw new Error("AVANTIQO_IMAGE_V8_RUNTIME_QUALITY_COMPILER_INVALID");
  if (candidate.cache_ready !== true) throw new Error("AVANTIQO_IMAGE_V8_RUNTIME_Z_IMAGE_CACHE_NOT_READY");
  return output;
}
function validateCapacity(body) {
  ...
  if (
    text(output.status) !== "completed" ||
    text(output.engine_contract) !== ENGINE_CONTRACT ||
    text(output.operation) !== CAPACITY_OPERATION ||
    text(output.runtime_revision) !== EXPECTED_RUNTIME ||
    text(output.target_model) !== TARGET_MODEL
  ) throw new Error("AVANTIQO_IMAGE_V8_CAPACITY_BASE_CONTRACT_INVALID");
  assertFalse(output.download_requested, "AVANTIQO_IMAGE_V8_CAPACITY_DOWNLOAD_REQUEST_FORBIDDEN");
  assertFalse(output.model_download_performed, "AVANTIQO_IMAGE_V8_CAPACITY_DOWNLOAD_FORBIDDEN");
  assertFalse(output.generation_requested, "AVANTIQO_IMAGE_V8_CAPACITY_GENERATION_FORBIDDEN");
  assertFalse(output.inference_performed, "AVANTIQO_IMAGE_V8_CAPACITY_INFERENCE_FORBIDDEN");
  assertFalse(output.storage_mutation_performed, "AVANTIQO_IMAGE_V8_CAPACITY_STORAGE_MUTATION_FORBIDDEN");
  if (output.candidate_cache_ready !== true) throw new Error("AVANTIQO_IMAGE_V8_CAPACITY_CACHE_NOT_READY");
  const storage = object(output.candidate_storage);
  if (
    text(storage.physical_usage_contract) !== EXPECTED_PHYSICAL_USAGE ||
    text(storage.allocation_decision_basis) !== EXPECTED_ALLOCATION_BASIS ||
    storage.backing_filesystem_capacity_used_for_decision !== false
  ) throw new Error("AVANTIQO_IMAGE_V8_CAPACITY_PHYSICAL_GUARD_INVALID");
  return output;
}
...
if (!endpoints) throw new Error("AVANTIQO_IMAGE_V8_PROBE_ENDPOINT_LIST_INVALID");
...
if (text(template.imageName) !== immutableImage || !text(template.name).startsWith("avantiqo-image-immutable-v8-")) {
  throw new Error("AVANTIQO_IMAGE_V8_PROBE_TEMPLATE_NOT_REBOUND");
}
if (finite(endpoint.workersMin, -1) !== 0 || finite(endpoint.workersMax, -1) !== 1) {
  throw new Error("AVANTIQO_IMAGE_V8_PROBE_SCALING_INVALID");
}
...
) throw new Error("AVANTIQO_IMAGE_V8_PROBE_EXISTING_ACTIVITY_BLOCK");

console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_PROVIDER_JOBS_MAX=2");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_GENERATION_REQUESTED=false");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_INFERENCE_PERFORMED=false");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_MODEL_DOWNLOAD_PERFORMED=false");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_STORAGE_MUTATION=false");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_ENDPOINT_MUTATION=false");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_PRODUCTION_DEPLOY=false");
console.log("AVANTIQO_IMAGE_V8_RUNTIME_PROBE_SECRETS_PRINTED=false");
console.log(`AVANTIQO_IMAGE_V8_RUNTIME_PROBE_QUEUE_CREDENTIAL_SOURCE=${queueCredential.source}`);

const runtimeJob = await runOperation(endpointId, queueCredential.key, {
  contract: ENGINE_CONTRACT,
  operation: RUNTIME_OPERATION,
}, "RUNTIME");
const runtimeProbe = validateRuntime(runtimeJob.body);

const capacityJob = await runOperation(endpointId, queueCredential.key, {
  contract: ENGINE_CONTRACT,
  operation: CAPACITY_OPERATION,
  target_model: TARGET_MODEL,
}, "CAPACITY");
const foundationCapacity = validateCapacity(capacityJob.body);

const finalHealth = healthSummary(await queue(endpointId, "/health", queueCredential.key));
const result = {
  success: true,
  contract: "AVANTIQO_IMAGE_V8_RUNTIME_CERTIFICATION_PROBE_V1",
  endpoint: {
    id: endpointId,
  ...
  foundation_capacity: foundationCapacity,
  ...
  next_action: "RUN_ONE_CONTROLLED_V8_DEFAULT_QUALITY_GENERATION",
};
console.log(JSON.stringify(result, null, 2));
console.log("AVANTIQO_IMAGE_V8_RUNTIME_CERTIFICATION_PROBE=PASS");
