...
const CONTRACT = "AVANTIQO_MUSIC_ELASTIC_RUNPOD_PROVISION_V1";
...
async function parkEndpoint(endpoint, credential, expectedTemplateId) {
  ...
  if (!endpointId) throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_ID_REQUIRED_FOR_PARK");
  if (endpointVolumeIds(endpoint).length) throw new Error("AVANTIQO_MUSIC_ELASTIC_NETWORK_VOLUME_FORBIDDEN");
  ...
  if (text(parked.name) !== ENDPOINT_NAME) throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_PARK_NAME_MISMATCH");
  if (expectedTemplateId && text(parked.templateId ?? parked.template_id) !== expectedTemplateId) {
    throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_PARK_TEMPLATE_MISMATCH");
  }
  if (endpointVolumeIds(parked).length) throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_PARK_NETWORK_VOLUME_FORBIDDEN");
  if (endpointWorkersMin(parked) !== 0 || endpointWorkersMax(parked) !== 0) {
    throw new Error(`AVANTIQO_MUSIC_ELASTIC_ENDPOINT_PARK_VERIFY_FAILED:${endpointWorkersMin(parked)}/${endpointWorkersMax(parked)}`);
  }
  return parked;
}
...
async function imageEvidence() {
  ...
  try {
    ...
    throw new Error(`AVANTIQO_MUSIC_ELASTIC_IMAGE_EVIDENCE_REQUIRED:${error?.code || "READ_FAILED"}`);
  }
  if (
    report?.success !== true ||
    ...
    throw new Error("AVANTIQO_MUSIC_ELASTIC_IMAGE_EVIDENCE_INVALID");
  }
  const image = text(report.immutable_image_reference);
  if (!/^ghcr\.io\/.+@sha256:[a-f0-9]{64}$/i.test(image)) throw new Error("AVANTIQO_MUSIC_ELASTIC_IMMUTABLE_IMAGE_REQUIRED");
  return { image, source_sha: text(report.source_sha), digest: text(report.image_digest) };
}

function registryAuth(registryAuths) {
  const explicit = text(process.env.AVANTIQO_MUSIC_ELASTIC_RUNPOD_REGISTRY_AUTH_ID);
  if (explicit) {
...
if (!Array.isArray(endpoints) || !Array.isArray(templates) || !Array.isArray(registryAuths)) {
  throw new Error("AVANTIQO_MUSIC_ELASTIC_RUNPOD_LIST_INVALID");
}

const endpointMatches = endpoints.filter((endpoint) => text(endpoint.name) === ENDPOINT_NAME);
if (endpointMatches.length > 1) throw new Error(`AVANTIQO_MUSIC_ELASTIC_ENDPOINT_AMBIGUOUS:${endpointMatches.length}`);
if (endpointMatches.length === 1) {
  let existing = endpointMatches[0];
  if (endpointVolumeIds(existing).length) throw new Error("AVANTIQO_MUSIC_ELASTIC_NETWORK_VOLUME_FORBIDDEN");
  const existingTemplate = endpointTemplate(existing, templates);
  assertExactTemplateImage(existingTemplate, image.image, "AVANTIQO_MUSIC_ELASTIC_EXISTING_ENDPOINT_IMAGE_DIGEST_MISMATCH");
  ...
  if (parkingRequired && !apply) {
    console.log(JSON.stringify({
      ...
      runtime_probe_job_submitted: false,
  ...
  console.log(JSON.stringify({
    ...
    production_certified: false,
    next_action: "RUNTIME_PROBE_ONLY_THROUGH_AVANTIQO_RUNPOD_SAFE_LEASE_V2",
  }, null, 2));
  process.exit(0);
}
...
if (templateMatches.length > 1) throw new Error(`AVANTIQO_MUSIC_ELASTIC_TEMPLATE_AMBIGUOUS:${templateMatches.length}`);
if (templateMatches[0]) assertExactTemplateImage(templateMatches[0], image.image, "AVANTIQO_MUSIC_ELASTIC_TEMPLATE_IMAGE_MISMATCH");

const gpuTypeIds = unique(
  text(process.env.AVANTIQO_MUSIC_ELASTIC_RUNPOD_GPU_TYPE_IDS || "NVIDIA L4,NVIDIA RTX A5000,NVIDIA GeForce RTX 4090").split(","),
);
if (!gpuTypeIds.length) throw new Error("AVANTIQO_MUSIC_ELASTIC_GPU_TYPE_IDS_REQUIRED");
...
const plan = {
  ...
  execution_timeout_ms: 10 * 60 * 1000,
  network_volume_required: false,
  runtime_probe_job_submitted: false,
  audio_render_performed: false,
  production_certified: false,
  human_listening_review_required: true,
  safe_lease_lane: "music-elastic-audio",
...
if (!template) {
  template = await rest("/templates", managementKey, {
    ...
    body: {
      ...
      isServerless: true,
      ports: [],
      readme: "Avantiqo Music Elastic Audio. Signalsmith Stretch; immutable image; no network volume; no-inference runtime probe; production certification requires human listening review.",
      volumeInGb: 0,
      volumeMountPath: "/workspace",
    },
  });
}
assertExactTemplateImage(template, image.image, "AVANTIQO_MUSIC_ELASTIC_CREATED_TEMPLATE_IMAGE_DIGEST_MISMATCH");
const templateId = text(template?.id);
if (!templateId) throw new Error("AVANTIQO_MUSIC_ELASTIC_TEMPLATE_ID_REQUIRED");
...
if (freshMatches.length) throw new Error(`AVANTIQO_MUSIC_ELASTIC_ENDPOINT_APPEARED_REPLAN_REQUIRED:${freshMatches.length}`);
...
const endpointId = text(endpoint?.id);
if (!endpointId) throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_ID_REQUIRED");
...
if (text(verified.name) !== ENDPOINT_NAME || text(verified.templateId ?? verified.template_id) !== templateId) {
  throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_VERIFY_FAILED");
}
if (endpointVolumeIds(verified).length) throw new Error("AVANTIQO_MUSIC_ELASTIC_ENDPOINT_UNEXPECTED_NETWORK_VOLUME");
...
if (endpointWorkersMin(verified) !== 0 || endpointWorkersMax(verified) !== 0) {
  throw new Error(`AVANTIQO_MUSIC_ELASTIC_ENDPOINT_NOT_PARKED_0_0:${endpointWorkersMin(verified)}/${endpointWorkersMax(verified)}`);
}
const verifiedTemplate = endpointTemplate(verified, [template, ...templates]);
assertExactTemplateImage(verifiedTemplate, image.image, "AVANTIQO_MUSIC_ELASTIC_ENDPOINT_VERIFY_IMAGE_DIGEST_MISMATCH");
...
console.log(JSON.stringify({
  ...
  exact_image_digest_verified: true,
  explicit_parking_mutation_performed: explicitParkingMutationPerformed,
