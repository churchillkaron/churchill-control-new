...
function replaceExactlyOnce(source, search, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`AVANTIQO_VIDEO_V59_SOURCE_TRANSFORM_${label}_MISMATCH:occurrences=${count}`);
  }
...
source = replaceExactlyOnce(
  ...
  `function templateContract(template = {}) {\n  const canonicalEnv = Object.fromEntries(\n    Object.entries(normalizeEnv(template.env)).sort(([left], [right]) => left.localeCompare(right)),\n  );\n  return JSON.stringify({\n    imageName: text(template.imageName),\n    containerDiskInGb: finite(template.containerDiskInGb, 0),\n    containerRegistryAuthConfigured: Boolean(text(template.containerRegistryAuthId)),\n    dockerEntrypoint: list(template.dockerEntrypoint),\n    dockerStartCmd: list(template.dockerStartCmd),\n    env: canonicalEnv,\n    isPublic: template.isPublic === true,\n    isServerless: template.isServerless === false ? false : true,\n    volumeMountPath: text(template.volumeMountPath),\n  });\n}`,
  "CANONICAL_SAFETY_CRITICAL_TEMPLATE_CONTRACT",
);
...
source = replaceExactlyOnce(
  ...
  `    creation_transport: "RUNPOD_GRAPHQL_SAVE_ENDPOINT",\n    create_network_volume_input_shape: "OBJECTS",\n    post_create_multi_region_placement_transport: "RUNPOD_REST_PATCH",\n    post_create_network_volume_input_shape: "STRING_IDS",\n    post_create_gpu_pool_reasserted: true,\n    immutable_image_verification_source: "FULL_TEMPLATE_REST_RECORD",\n    idle_billing_verification_source: "RUNPOD_MANAGEMENT_ENDPOINT_WORKERS",\n    queue_health_worker_counter_role: "DIAGNOSTIC_ONLY",\n    safe_lease_idle_accounting_aligned: true,\n    existing_template_verification: "CANONICAL_SAFETY_CRITICAL_FIELDS",\n    runpod_normalized_ports_ignored: true,\n    canonical_env_ordering: true,\n    cuda_patch_transport: "RUNPOD_REST_PATCH",`,
  "RESULT_CANONICAL_TEMPLATE_AND_TRANSPORT",
);

source = source.replaceAll("V53", "V59");
if (source.includes("V53")) {
  throw new Error("AVANTIQO_VIDEO_V59_SOURCE_TRANSFORM_V53_REMAINS");
}

console.log(`AVANTIQO_VIDEO_V59_SOURCE_TRANSFORM_ACTIVE=${JSON.stringify({
  base: BASE,
  fixes: [
    "OMIT_VOLUME_IN_GB_FROM_SERVERLESS_TEMPLATE",
    "CANONICALIZE_EXISTING_TEMPLATE_SAFETY_CRITICAL_FIELDS",
    "IGNORE_RUNPOD_NORMALIZED_PORTS_IN_WORKER_IDENTITY",
    "SORT_TEMPLATE_ENV_KEYS_BEFORE_COMPARISON",
  ...
  template_safety_fields: [
    "immutable_image",
    "container_disk",
    "registry_auth_presence",
    "docker_entrypoint",
