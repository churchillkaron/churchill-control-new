...
async function readDrainSnapshot(endpointId, queueKey, managementKey) {
  ...
  if (text(endpoint?.id) !== endpointId || text(endpoint?.name) !== ENDPOINT_NAME) throw new Error("AVANTIQO_IMAGE_V7_REBIND_DRAIN_ENDPOINT_IDENTITY_CHANGED");
...
async function waitForStableDrain(endpointId, queueKey, managementKey) {
  ...
  while (Date.now() - started < DRAIN_TIMEOUT_MS) {
    ...
    console.log(JSON.stringify({
      event: "AVANTIQO_IMAGE_V7_REBIND_DRAIN_PROGRESS",
  ...
  throw new Error("AVANTIQO_IMAGE_V7_REBIND_STABLE_DRAIN_TIMEOUT");
}
function templateBody(base, immutableImage, name) {
  const body = {
    containerDiskInGb: Math.max(1, finite(base?.containerDiskInGb, 30)),
    dockerEntrypoint: list(base?.dockerEntrypoint), dockerStartCmd: list(base?.dockerStartCmd), env: normalizeEnv(base?.env),
    imageName: immutableImage, isPublic: false, isServerless: true, name, ports: list(base?.ports),
    ...
    volumeInGb: Math.max(0, finite(base?.volumeInGb, 0)), volumeMountPath: text(base?.volumeMountPath) || "/workspace", category: "NVIDIA",
  };
...
function endpointContract(endpoint) {
  return {
    id: text(endpoint?.id), name: text(endpoint?.name), workers_min: finite(endpoint?.workersMin), workers_max: finite(endpoint?.workersMax),
    gpu_type_ids: list(endpoint?.gpuTypeIds).map(text).filter(Boolean), network_volume_ids: endpointVolumeIds(endpoint),
    idle_timeout: finite(endpoint?.idleTimeout), scaler_type: text(endpoint?.scalerType), scaler_value: finite(endpoint?.scalerValue),
  };
}
...
if (!pullProof.public_pull) throw new Error(`AVANTIQO_IMAGE_V7_REBIND_PUBLIC_PULL_REQUIRED:manifest_status=${pullProof.manifest_status ?? "NONE"}`);
...
const [templates, initialHealthRaw] = await Promise.all([endpointBoundTemplates(managementKey), queueHealth(endpointId, queueCredential.key)]);
...
if (!drainedCandidate(initialHealth, initialManagement)) throw new Error("AVANTIQO_IMAGE_V7_REBIND_INITIAL_DRAIN_BLOCK");
...
console.log(JSON.stringify({
  success: true, contract: "AVANTIQO_IMAGE_V7_IMMUTABLE_TEMPLATE_REBIND_V2", mode: apply ? "APPLY" : "PLAN",
  endpoint: endpointContract(endpoint), current_template: { id: currentTemplateId, name: text(currentTemplate?.name), image: text(currentTemplate?.imageName) },
  target: { source_sha: text(evidence.source_sha), immutable_image: immutableImage, template_name: targetName, existing_template_found: Boolean(targetTemplate), quality_profile: EXPECTED_PROFILE, quality_policy: EXPECTED_POLICY, quality_compiler_contract: EXPECTED_COMPILER, default_inference_steps: 28, default_guidance_scale: 4 },
  public_pull_proof: pullProof, initial_health: initialHealth, initial_management_workers: initialManagement,
  queue_credential_source: queueCredential.source, stable_drain_observations_required: STABLE_DRAIN_OBSERVATIONS, mutation_required: mutationRequired,
  safety: { provider_jobs_submitted: 0, image_generation_submitted: false, model_download_submitted: false, production_web_deploy: false, pricing_activation: false, endpoint_deleted: false, template_deleted: false, secret_values_printed: false },
}, null, 2));
...
if (!sameEndpointContract(endpoint, stableDrain.snapshot.endpoint)) throw new Error("AVANTIQO_IMAGE_V7_REBIND_ENDPOINT_CONTRACT_MOVED_BEFORE_APPLY");

if (!targetTemplate) {
  const created = await rest("/templates", managementKey, { method: "POST", body: templateBody(currentTemplate, immutableImage, targetName) });
...
if (!prePatchDrain.drained || !sameEndpointContract(endpoint, prePatchDrain.endpoint)) throw new Error("AVANTIQO_IMAGE_V7_REBIND_PREPATCH_STATE_CHANGED");

await rest(`/endpoints/${encodeURIComponent(endpointId)}`, managementKey, { method: "PATCH", body: { templateId: targetTemplateId } });

const verified = await rest(`/endpoints/${encodeURIComponent(endpointId)}?includeTemplate=true&includeWorkers=true`, managementKey);
if (!sameEndpointContract(endpoint, verified)) throw new Error("AVANTIQO_IMAGE_V7_REBIND_VERIFY_ENDPOINT_CONTRACT_FAILED");
...
console.log(JSON.stringify({
  success: true, contract: "AVANTIQO_IMAGE_V7_IMMUTABLE_TEMPLATE_REBIND_V2", applied: true, endpoint: endpointContract(verified),
  template_id: targetTemplateId, template_name: targetName, immutable_image: immutableImage, public_pull_proof: pullProof,
  stable_drain_observations: stableDrain.stable_observations, quality_profile: EXPECTED_PROFILE, quality_policy: EXPECTED_POLICY,
  quality_compiler_contract: EXPECTED_COMPILER, provider_jobs_submitted: 0, image_generation_submitted: false, model_download_submitted: false,
  production_web_deploy: false, pricing_activation: false, endpoint_deleted: false, template_deleted: false, secrets_in_output: false,
