...
async function queueHealth(endpointId, key) {
  return readJson(await fetch(`${QUEUE_BASE}/${encodeURIComponent(endpointId)}/health`, {
    ...
    signal: AbortSignal.timeout(30_000),
  }), "AVANTIQO_IMAGE_V9_REBIND_QUEUE");
}
...
async function selectQueueCredential(endpointId, managementKey) {
  ...
  throw new Error("AVANTIQO_IMAGE_V9_REBIND_QUEUE_CREDENTIAL_NOT_FOUND");
}
async function endpointBoundTemplates(key) {
  ...
  if (!templates) throw new Error("AVANTIQO_IMAGE_V9_REBIND_TEMPLATE_LIST_INVALID");
  return templates;
}
async function readEvidence() {
  ...
  ) throw new Error("AVANTIQO_IMAGE_V9_REBIND_EVIDENCE_NOT_READY");
  ...
  if (!/^ghcr\.io\/churchillkaron\/avantiqo-image-worker@sha256:[a-f0-9]{64}$/i.test(immutableImage)) {
    throw new Error("AVANTIQO_IMAGE_V9_REBIND_IMMUTABLE_IMAGE_INVALID");
  }
...
function resolveEndpoint(endpoints, configuredId) {
  ...
  throw new Error(`AVANTIQO_IMAGE_V9_REBIND_CONFIGURED_ENDPOINT_INVALID:${matches.length}`);
}
function resolveTemplate(templates, templateId) {
  const matches = templates.filter((entry) => text(entry?.id) === templateId);
  if (matches.length !== 1) throw new Error(`AVANTIQO_IMAGE_V9_REBIND_TEMPLATE_RESOLUTION_FAILED:${matches.length}`);
  return matches[0];
}
...
async function readDrainSnapshot(endpointId, queueKey, managementKey) {
  ...
  if (text(endpoint?.id) !== endpointId || text(endpoint?.name) !== ENDPOINT_NAME) {
    throw new Error("AVANTIQO_IMAGE_V9_REBIND_DRAIN_ENDPOINT_IDENTITY_CHANGED");
  }
...
async function waitForStableDrain(endpointId, queueKey, managementKey) {
  ...
  while (Date.now() - started < DRAIN_TIMEOUT_MS) {
    ...
    console.log(JSON.stringify({
      event: "AVANTIQO_IMAGE_V9_REBIND_DRAIN_PROGRESS",
  ...
  throw new Error("AVANTIQO_IMAGE_V9_REBIND_STABLE_DRAIN_TIMEOUT");
}
function templateBody(base, immutableImage, name) {
  const body = {
    ...
    readme: "Avantiqo Image V9 immutable Z-Image default-generation worker preserving V8 anti-text quality compiler and V6 physical-volume accounting. Exact digest evidence is recorded in audits/results/avantiqo-image-worker-image.json.",
...
if (apply) approved("AVANTIQO_IMAGE_V9_IMMUTABLE_REBIND_APPROVED");
...
if (!pullProof.public_pull) {
  throw new Error(`AVANTIQO_IMAGE_V9_REBIND_PUBLIC_PULL_REQUIRED:manifest_status=${pullProof.manifest_status ?? "NONE"}`);
}
...
if (!endpoints) throw new Error("AVANTIQO_IMAGE_V9_REBIND_ENDPOINT_LIST_INVALID");
...
if (!drainedCandidate(initialHealth, initialManagement)) throw new Error("AVANTIQO_IMAGE_V9_REBIND_INITIAL_DRAIN_BLOCK");
if (finite(endpoint.workersMin, -1) !== 0 || finite(endpoint.workersMax, -1) !== 1) throw new Error("AVANTIQO_IMAGE_V9_REBIND_SCALING_CONTRACT_INVALID");
const volumeIds = endpointVolumeIds(endpoint);
if (volumeIds.length !== 1) throw new Error(`AVANTIQO_IMAGE_V9_REBIND_VOLUME_COUNT_INVALID:${volumeIds.length}`);

const digestSuffix = immutableImage.split("sha256:")[1].slice(0, 12);
const targetName = `avantiqo-image-immutable-v9-${digestSuffix}`;
const targetMatches = templates.filter((entry) => text(entry?.name) === targetName);
if (targetMatches.length > 1) throw new Error(`AVANTIQO_IMAGE_V9_REBIND_TARGET_TEMPLATE_AMBIGUOUS:${targetMatches.length}`);
...
console.log(JSON.stringify({
  success: true,
  contract: "AVANTIQO_IMAGE_V9_IMMUTABLE_TEMPLATE_REBIND_V1",
  ...
  target: {
    ...
    configured_generation_foundation: TARGET_MODEL,
    default_generation_routing_contract: EXPECTED_DEFAULT_ROUTING,
...
if (!apply) {
  console.log("AVANTIQO_IMAGE_V9_IMMUTABLE_REBIND_APPLIED=false");
  process.exit(0);
}
if (!mutationRequired) {
  console.log("AVANTIQO_IMAGE_V9_IMMUTABLE_REBIND_APPLIED=true");
  console.log("AVANTIQO_IMAGE_V9_IMMUTABLE_REBIND_MUTATION_REQUIRED=false");
  process.exit(0);
}
...
if (!sameEndpointContract(endpoint, stableDrain.snapshot.endpoint)) {
  throw new Error("AVANTIQO_IMAGE_V9_REBIND_ENDPOINT_CONTRACT_MOVED_BEFORE_APPLY");
}
...
if (!targetTemplate) {
  ...
  if (!createdId) throw new Error("AVANTIQO_IMAGE_V9_REBIND_TEMPLATE_CREATE_ID_REQUIRED");
...
if (issues.length) throw new Error(`AVANTIQO_IMAGE_V9_REBIND_TARGET_TEMPLATE_INVALID:${issues.join("|")}`);
const targetTemplateId = text(targetTemplate.id);
if (!targetTemplateId) throw new Error("AVANTIQO_IMAGE_V9_REBIND_TARGET_TEMPLATE_ID_REQUIRED");
...
if (!prePatchDrain.drained || !sameEndpointContract(endpoint, prePatchDrain.endpoint)) {
  throw new Error("AVANTIQO_IMAGE_V9_REBIND_PREPATCH_STATE_CHANGED");
}
...
if (!sameEndpointContract(endpoint, verified)) throw new Error("AVANTIQO_IMAGE_V9_REBIND_VERIFY_ENDPOINT_CONTRACT_FAILED");
if (text(verified?.templateId || verified?.template?.id) !== targetTemplateId) {
  throw new Error("AVANTIQO_IMAGE_V9_REBIND_VERIFY_TEMPLATE_ID_FAILED");
}
const verifiedTemplate = resolveTemplate(await endpointBoundTemplates(managementKey), targetTemplateId);
if (templateIssues(verifiedTemplate, immutableImage).length) throw new Error("AVANTIQO_IMAGE_V9_REBIND_VERIFY_TEMPLATE_FAILED");
...
console.log(JSON.stringify({
  ...
  next_action: "RUN_IMAGE_V9_RUNTIME_PROBE",
}, null, 2));
console.log("AVANTIQO_IMAGE_V9_IMMUTABLE_REBIND_APPLIED=true");
