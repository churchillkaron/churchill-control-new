...
const CONTRACT = "AVANTIQO_VOICE_TTS_PULL_STALL_RELOCATION_V1";
...
function assertEndpointBaseline(endpoint, endpointId, expectedImage) {
  ...
  if (text(endpoint.template?.imageName) !== expectedImage) {
    throw new Error("AVANTIQO_VOICE_TTS_PULL_STALL_ENDPOINT_IMAGE_MISMATCH");
  }
...
const [endpoint, healthRaw, workersRaw, capacity] = await Promise.all([
...
assertEndpointBaseline(endpoint, endpointId, expected.image);
...
const plan = {
  ...
  secrets_in_output: false,
  next_action: apply
    ? "DRAIN_STUCK_WORKER_AND_RELOCATE_ENDPOINT"
    : "APPLY_PULL_STALL_RELOCATION",
};
...
const [freshEndpoint, freshHealthRaw, freshWorkersRaw] = await Promise.all([
  rest(`/endpoints/${encodeURIComponent(endpointId)}?includeTemplate=true&includeWorkers=true`, managementKey),
  queue(endpointId, managementKey),
  controlWorkers(endpointId, managementKey),
]);
assertEndpointBaseline(freshEndpoint, endpointId, expected.image);
...
const [verifiedEndpoint, verifiedHealthRaw, verifiedWorkersRaw] = await Promise.all([
  rest(`/endpoints/${encodeURIComponent(endpointId)}?includeTemplate=true&includeWorkers=true`, managementKey),
  queue(endpointId, managementKey),
  controlWorkers(endpointId, managementKey),
]);
...
if (text(verifiedEndpoint.template?.imageName) !== expected.image) {
