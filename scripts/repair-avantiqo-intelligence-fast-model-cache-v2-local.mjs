...
const CONTRACT = "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2";
const EXPECTED_MAIN_ENV =
  "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_EXPECTED_MAIN";
const APPROVAL_ENV =
  "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_APPROVED";
const VERIFY_TIMEOUT_MS = Math.max(
  15_000,
  Math.min(
    180_000,
    Number(
      process.env.AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_VERIFY_TIMEOUT_MS ||
        60_000,
    ),
  ),
);
...
function validateMain() {
  ...
  if (expectedMain && !/^[0-9a-f]{40}$/i.test(expectedMain)) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_EXPECTED_MAIN_INVALID:${expectedMain.slice(0, 80)}`,
    );
  }
  ...
  const branch = shell(
    "git",
    ["branch", "--show-current"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_GIT_BRANCH_FAILED",
  );
  if (branch !== "main") {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_MAIN_REQUIRED:actual=${branch || "DETACHED"}`,
    );
  }
  ...
  const head = shell(
    "git",
    ["rev-parse", "HEAD"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_GIT_HEAD_FAILED",
  );

  if (expectedMain) {
    if (head !== expectedMain) {
      throw new Error(
        `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_PINNED_MAIN_MISMATCH:head=${head}:expected=${expectedMain}`,
      );
    }
  ...
  shell(
    "git",
    ["fetch", "origin", "main"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_GIT_FETCH_FAILED",
  );
  const remote = shell(
    "git",
    ["rev-parse", "origin/main"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_GIT_REMOTE_FAILED",
  );
  if (head !== remote) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_LOCAL_MAIN_NOT_CURRENT:head=${head}:origin_main=${remote}`,
    );
  }
...
async function requestJson(url, key, options = {}) {
  ...
  if (!response.ok || body === null) {
    ...
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_HTTP_${response.status}:${detail || "EMPTY_BODY"}`,
    );
  }
  return body;
}
...
async function graphql(query, variables, key) {
  ...
  if (Array.isArray(response?.errors) && response.errors.length > 0) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_GRAPHQL:${redact(response.errors[0]?.message).slice(0, 700)}`,
    );
  }
  return response;
}
...
function assertFastModelBinding(endpoint) {
  ...
  if (!match) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_MODEL_BINDING_MISMATCH:expected=${FAST_MODEL}`,
    );
  }
  return match;
}
...
function saveInput(endpoint, refs) {
  const input = {
    ...
    executionTimeoutMs: Math.max(
      1,
      finite(endpoint?.executionTimeoutMs, 90_000),
    ),
...
const MODEL_STATE_QUERY = `
query AvantiqoFastModelState {
  myself {
    endpoints {
      id
      name
...
async function graphqlModelState(endpointId, managementKey) {
  const response = await graphql(MODEL_STATE_QUERY, {}, managementKey);
  const endpoints = list(response?.data?.myself?.endpoints);
  const matches = endpoints.filter((entry) => text(entry?.id) === endpointId);
  if (matches.length !== 1) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_GRAPHQL_ENDPOINT_RESOLUTION_FAILED:id=${endpointId}:matches=${matches.length}`,
    );
  }
...
async function saveEndpoint(endpoint, refs, managementKey) {
  ...
  if (!saved?.id) {
    throw new Error(
      "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_SAVE_ENDPOINT_EMPTY",
    );
  }
  return saved;
}
...
async function readCanonicalLive(managementKey, queueKey) {
  ...
  const deepList = resolveOne(
    endpoints,
    DEEP_NAME,
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_DEEP_RESOLUTION_FAILED",
  );
  const fastList = resolveOne(
    endpoints,
    FAST_NAME,
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_FAST_RESOLUTION_FAILED",
  );
  const deepId = text(deepList?.id);
  const fastId = text(fastList?.id);
  if (!deepId || !fastId) {
    throw new Error(
      "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_ENDPOINT_IDS_REQUIRED",
    );
  }

  const [deepEndpoint, fastEndpoint, deepHealthRaw, fastHealthRaw] =
    await Promise.all([
...
async function pollModelState(endpointId, managementKey) {
  ...
  while (Date.now() - started <= VERIFY_TIMEOUT_MS) {
    const [graphqlState, restState] = await Promise.all([
      graphqlModelState(endpointId, managementKey),
      exactEndpoint(endpointId, managementKey),
    ]);
    lastGraphql = graphqlState;
    lastRest = {
      model_status: text(restState?.modelStatus) || null,
    ...
    console.log(
      `AVANTIQO_FAST_MODEL_CACHE_VERIFY_PROGRESS=${JSON.stringify({
        elapsed_seconds: elapsedSeconds,
        graphql_model_status: lastGraphql.model_status,
    ...
    if (
      lastGraphql.expected_reference_present ||
      lastRest.expected_reference_present
    ) {
      return {
        visible: true,
...
async function rollback(beforeEndpoint, beforeRefs, managementKey, queueKey) {
  ...
  const state = await graphqlModelState(live.fastId, managementKey);
  const invariantDiffs = differentFields(
    invariantSummary(beforeEndpoint),
    invariantSummary(live.fastEndpoint),
  );
  return (
    canonicalState(live.deep, live.fast) &&
...
if (!canonicalState(live.deep, live.fast)) {
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_CANONICAL_STATE_REQUIRED:deep=${JSON.stringify(live.deep)}:fast=${JSON.stringify(live.fast)}`,
  );
}

const beforeGraphqlState = await graphqlModelState(
  live.fastId,
  managementKey,
);
const beforeRefs = beforeGraphqlState.model_references;
const expectedAlreadyAttached =
  beforeGraphqlState.expected_reference_present && beforeRefs.length === 1;

if (beforeRefs.length > 0 && !expectedAlreadyAttached) {
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_EXISTING_MODEL_REFERENCES_REVIEW_REQUIRED:${JSON.stringify(beforeRefs)}`,
  );
}
...
const plan = {
  ...
  before_model_status: beforeGraphqlState.model_status,
  expected_model_reference_already_attached: expectedAlreadyAttached,
  canonical_deep_active_fast_parked: true,
  verification_sources: [
    "GRAPHQL_SAVE_RESPONSE",
    "GRAPHQL_ENDPOINT_MODEL_STATE",
    "REST_EXACT_ENDPOINT",
  ],
  verify_timeout_ms: VERIFY_TIMEOUT_MS,
  proposed_change: expectedAlreadyAttached
    ? "NONE"
    : "SET_FAST_MODEL_REFERENCES_ONLY",
...
if (!apply || expectedAlreadyAttached) {
  ...
  console.log(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2=${expectedAlreadyAttached ? "ALREADY_APPLIED" : "PLAN_READY"}`,
  );
  process.exit(0);
}
...
const saveResponse = {
  model_status: text(saved?.modelStatus) || null,
  model_references: modelReferences(saved),
  expected_reference_present: hasExpectedReference(saved),
};

const verification = await pollModelState(live.fastId, managementKey);
...
const acceptedByAnyAuthoritativeSource =
  saveResponse.expected_reference_present || verification.visible;
const modelStatus =
  verification.graphql?.model_status ||
  verification.rest?.model_status ||
  saveResponse.model_status ||
  null;
const explicitModelFailure = /FAIL|ERROR/i.test(text(modelStatus));

if (
  !canonicalAfter ||
  invariantDifferences.length > 0 ||
  explicitModelFailure ||
  !acceptedByAnyAuthoritativeSource
) {
  ...
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2_VERIFY_FAILED:canonical=${canonicalAfter}:accepted=${acceptedByAnyAuthoritativeSource}:model_status=${text(modelStatus) || "UNKNOWN"}:invariant_differences=${invariantDifferences.join(",") || "NONE"}:rollback=${rollbackSucceeded ? "PASS" : "FAIL"}`,
  );
}

const visibility = verification.visible
  ? "PERSISTENCE_VISIBLE"
  : "SAVE_ACCEPTED_PENDING_READ_VISIBILITY";
  ...
  JSON.stringify(
    {
      ...
      cache_registration_visibility: visibility,
      ...
      gpu_activation_performed: false,
      production_deploy_performed: false,
      next_action: verification.visible
        ? "BENCHMARK_FAST_COLD_MODEL_ROUTE_AFTER_HOST_CACHE"
        : "RECHECK_FAST_MODEL_CACHE_STATE_BEFORE_BENCHMARK",
    },
    null,
    2,
  ),
);
console.log("AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V2=PASS");