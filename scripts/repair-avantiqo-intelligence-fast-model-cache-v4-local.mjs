...
const MODEL_REFERENCE = FAST_MODEL;
const CONTRACT = "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4";
const EXPECTED_MAIN_ENV = "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_EXPECTED_MAIN";
const APPROVAL_ENV = "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_APPROVED";
...
function validateMain() {
  ...
  if (expectedMain && !/^[0-9a-f]{40}$/i.test(expectedMain)) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_EXPECTED_MAIN_INVALID:${expectedMain.slice(0, 80)}`,
    );
  }
  ...
  const branch = shell(
    "git",
    ["branch", "--show-current"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_GIT_BRANCH_FAILED",
  );
  if (branch !== "main") {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_MAIN_REQUIRED:actual=${branch || "DETACHED"}`,
    );
  }
  ...
  const head = shell(
    "git",
    ["rev-parse", "HEAD"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_GIT_HEAD_FAILED",
  );

  if (expectedMain) {
    if (head !== expectedMain) {
      throw new Error(
        `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_PINNED_MAIN_MISMATCH:head=${head}:expected=${expectedMain}`,
      );
    }
  ...
  shell(
    "git",
    ["fetch", "origin", "main"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_GIT_FETCH_FAILED",
  );
  const remote = shell(
    "git",
    ["rev-parse", "origin/main"],
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_GIT_REMOTE_FAILED",
  );
  if (head !== remote) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_LOCAL_MAIN_NOT_CURRENT:head=${head}:origin_main=${remote}`,
    );
  }
...
async function requestJson(url, key, options = {}) {
  ...
  if (!response.ok || body === null) {
    ...
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_HTTP_${response.status}:${detail || "EMPTY_BODY"}`,
    );
  }
  return body;
}
...
async function graphql(query, variables, key) {
  ...
  if (Array.isArray(response?.errors) && response.errors.length > 0) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_GRAPHQL:${redact(response.errors[0]?.message).slice(0, 700)}`,
    );
  }
  return response;
}
...
function assertFastModelBinding(endpoint) {
  ...
  if (!match) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_MODEL_BINDING_MISMATCH:expected=${FAST_MODEL}`,
    );
  }
  return match;
}
...
async function graphqlModelReferences(endpointId, managementKey) {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_GRAPHQL_ENDPOINT_RESOLUTION_FAILED:id=${endpointId}:matches=${matches.length}`,
    );
  }
...
async function saveEndpoint(endpoint, refs, managementKey) {
  ...
  if (!saved?.id) {
    throw new Error(
      "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_SAVE_ENDPOINT_EMPTY",
    );
  }
  return saved;
}
...
async function readCanonicalLive(managementKey, queueKey) {
  ...
  const deepList = resolveOne(
    endpoints,
    DEEP_NAME,
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_DEEP_RESOLUTION_FAILED",
  );
  const fastList = resolveOne(
    endpoints,
    FAST_NAME,
    "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_FAST_RESOLUTION_FAILED",
  );
  const deepId = text(deepList?.id);
  const fastId = text(fastList?.id);
  if (!deepId || !fastId) {
    throw new Error(
      "AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_ENDPOINT_IDS_REQUIRED",
    );
  }
...
if (!canonicalState(live.deep, live.fast)) {
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_CANONICAL_STATE_REQUIRED:deep=${JSON.stringify(live.deep)}:fast=${JSON.stringify(live.fast)}`,
  );
}

const beforeGraphqlState = await graphqlModelReferences(
  live.fastId,
  managementKey,
);
...
if (beforeRefs.length > 0 && !expectedAlreadyAttached) {
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_EXISTING_MODEL_REFERENCES_REVIEW_REQUIRED:${JSON.stringify(beforeRefs)}`,
  );
}
...
const plan = {
  ...
  model_reference_format: "HUGGING_FACE_ORG_NAME",
  runpod_model_field_value: FAST_MODEL,
...
if (!apply || expectedAlreadyAttached) {
  console.log(
    JSON.stringify({ ...plan, mutation_performed: false }, null, 2),
  );
  console.log(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4=${expectedAlreadyAttached ? "ALREADY_APPLIED" : "PLAN_READY"}`,
  );
  process.exit(0);
}
...
if (
  ...
  throw new Error(
    `AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4_VERIFY_FAILED:canonical=${canonicalAfter}:accepted=${acceptedByAnyAuthoritativeSource}:invariant_differences=${invariantDifferences.join(",") || "NONE"}:rollback=${rollbackSucceeded ? "PASS" : "FAIL"}`,
  );
}
  ...
  JSON.stringify(
    {
      ...
      verification,
      cache_registration_visibility: verification.visible
        ? "PERSISTENCE_VISIBLE"
        : "SAVE_RESPONSE_ACCEPTED",
...
console.log("AVANTIQO_INTELLIGENCE_FAST_MODEL_CACHE_REPAIR_V4=PASS");
