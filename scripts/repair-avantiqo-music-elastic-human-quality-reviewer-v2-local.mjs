...
const CONTRACT = "AVANTIQO_MUSIC_ELASTIC_HUMAN_QUALITY_REVIEWER_REPAIR_V2";
...
const APPROVAL_ENV = "AVANTIQO_MUSIC_ELASTIC_HUMAN_QUALITY_REVIEWER_REPAIR_V2_APPROVED";
...
function reviewerIdentityValid(value) {
  ...
  if (reviewer.length < 4 || reviewer.length > 120) return false;
  ...
  if (words.every((word) => genericTokens.has(word.toLowerCase().replace(/[^\p{L}]/gu, "")))) return false;
...
function requireCertified(row, phase) {
  const certification = certify(row);
...
if (reviewerIdentityValid(beforeReviewer)) {
  ...
  console.log("AVANTIQO_MUSIC_ELASTIC_HUMAN_QUALITY_REVIEWER_REPAIR_V2=PASS");
...
const { data: staged, error: stageError } = await supabase
  ...
  .update({
    ...
    metadata: {
      ...
      human_quality_reviewer_repair_v2_staged_at: new Date().toISOString(),
    },
...
requireCertified(candidate, "REPAIRED_INACTIVE_CANDIDATE");
