...
const CONTRACT = "AVANTIQO_VIDEO_WAN22_CROSS_REGION_S3_REPLICATION_V32";
const APPROVAL_ENV = "AVANTIQO_VIDEO_WAN22_CROSS_REGION_REPLICATION_V32_APPROVED";
const HELPER = "scripts/replicate-avantiqo-video-wan22-cache-s3-v32-helper.py";
...
async function json(url, key) {
  ...
  if (!response.ok) {
    throw new Error(`AVANTIQO_VIDEO_V32_HTTP_${response.status}:${redact(body?.message || body?.error || raw).slice(0, 900)}`);
  }
  return body ?? {};
}
...
async function resolveS3Credential() {
  ...
  if (runpodShaped.length > 1) throw new Error("AVANTIQO_VIDEO_V32_MULTIPLE_RUNPOD_S3_PROFILES_SET_AWS_PROFILE");
...
async function requireQuiescent(endpointId, key, label) {
  ...
  if (queue !== 0 || progress !== 0 || workerTotal !== 0) {
    throw new Error(`AVANTIQO_VIDEO_V32_${label}_NOT_QUIESCENT:queue=${queue}:progress=${progress}:workers=${workerTotal}`);
  }
  return { queue, progress, workerTotal };
}

if (Number(process.versions.node.split(".")[0]) < 24) throw new Error(`AVANTIQO_VIDEO_V32_NODE24_REQUIRED:${process.version}`);
...
if (workersMin !== 0 || workersMax !== 0) {
  throw new Error(`AVANTIQO_VIDEO_V32_CINEMA_NOT_RESTING_0_0:min=${workersMin}:max=${workersMax}`);
}
const boundVolumeIds = endpointVolumeIds(cinema);
if (!boundVolumeIds.includes(SOURCE_VOLUME_ID)) throw new Error("AVANTIQO_VIDEO_V32_SOURCE_VOLUME_NOT_BOUND_TO_CINEMA");
if (boundVolumeIds.includes(DESTINATION_VOLUME_ID)) throw new Error("AVANTIQO_VIDEO_V32_DESTINATION_VOLUME_MUST_NOT_BE_BOUND_YET");
if (text(sourceVolume.id) !== SOURCE_VOLUME_ID || text(sourceVolume.dataCenterId) !== SOURCE_REGION) {
  throw new Error("AVANTIQO_VIDEO_V32_SOURCE_VOLUME_ID_OR_REGION_INVALID");
}
if (
  text(destinationVolume.id) !== DESTINATION_VOLUME_ID ||
  text(destinationVolume.dataCenterId) !== DESTINATION_REGION ||
  text(destinationVolume.name) !== DESTINATION_VOLUME_NAME ||
  finite(destinationVolume.size ?? destinationVolume.sizeGb, 0) < 400
) {
  throw new Error(`AVANTIQO_VIDEO_V32_DESTINATION_VOLUME_INVALID:${JSON.stringify({
    id: destinationVolume.id,
    name: destinationVolume.name,
    size: destinationVolume.size,
    dataCenterId: destinationVolume.dataCenterId,
  })}`);
}
...
console.log(JSON.stringify({
  ...
  exact_not_found_handling: true,
  transient_only_retry: true,
  resumable_by_exact_destination_size: true,
...
if (!apply) {
  console.log("AVANTIQO_VIDEO_WAN22_CROSS_REGION_S3_REPLICATION_V32_APPLIED=false");
  process.exit(0);
}
if (!s3Credential.accessKey || !s3Credential.secretKey) {
  throw new Error("AVANTIQO_VIDEO_V32_RUNPOD_S3_CREDENTIAL_REQUIRED");
}

const dir = await mkdtemp(join(tmpdir(), "avantiqo-video-v32-"));
try {
  const venv = join(dir, "venv");
  let result = spawnSync("python3", ["-m", "venv", venv], { cwd: process.cwd(), env: process.env, stdio: "inherit" });
  if (result.status !== 0) throw new Error(`AVANTIQO_VIDEO_V32_VENV_FAILED:exit=${result.status}`);
  ...
  if (result.status !== 0) throw new Error(`AVANTIQO_VIDEO_V32_DEPENDENCY_INSTALL_FAILED:exit=${result.status}`);

  const env = {
    ...process.env,
    AVANTIQO_V32_SOURCE_BUCKET: SOURCE_VOLUME_ID,
    AVANTIQO_V32_DESTINATION_BUCKET: DESTINATION_VOLUME_ID,
    AVANTIQO_V32_SOURCE_S3_ENDPOINT: SOURCE_S3_ENDPOINT,
    AVANTIQO_V32_DESTINATION_S3_ENDPOINT: DESTINATION_S3_ENDPOINT,
    AVANTIQO_V32_SOURCE_REGION: SOURCE_REGION,
    AVANTIQO_V32_DESTINATION_REGION: DESTINATION_REGION,
    AVANTIQO_V32_ACCESS_KEY: s3Credential.accessKey,
    AVANTIQO_V32_SECRET_KEY: s3Credential.secretKey,
  };
  result = spawnSync(python, [HELPER], { cwd: process.cwd(), env, stdio: "inherit" });
  if (result.status !== 0) throw new Error(`AVANTIQO_VIDEO_V32_HELPER_FAILED:exit=${result.status}`);
  ...
  if (
    finite(finalCinema.workersMin ?? finalCinema.workers_min, -1) !== 0 ||
    finite(finalCinema.workersMax ?? finalCinema.workers_max, -1) !== 0
  ) {
    throw new Error("AVANTIQO_VIDEO_V32_FINAL_CINEMA_NOT_RESTING_0_0");
  }
  const finalBound = endpointVolumeIds(finalCinema);
  if (!finalBound.includes(SOURCE_VOLUME_ID) || finalBound.includes(DESTINATION_VOLUME_ID)) {
    throw new Error("AVANTIQO_VIDEO_V32_FINAL_CINEMA_BINDING_CHANGED_UNEXPECTEDLY");
  }
  console.log("AVANTIQO_VIDEO_WAN22_CROSS_REGION_S3_REPLICATION_V32=PASS");
  console.log("AVANTIQO_VIDEO_WAN22_CROSS_REGION_S3_REPLICATION_V32_APPLIED=true");
} finally {
