...
if (!organizationId || !projectId || !graphId) {
  throw new Error("SEALED_RUNWAY_SIGNED_URL_RESET_SCOPE_REQUIRED");
}
if (approvalLiteral !== expectedLiteral || approvedMaximum !== 367.366602) {
  throw new Error("SEALED_RUNWAY_SIGNED_URL_RESET_APPROVAL_INVALID");
}
...
const unexpectedActive = graphTasks.filter((task) =>
  ["RUNNING", "READY", "REVIEW"].includes(text(task.status).toUpperCase()),
);

if (graphTasks.length !== 27) {
  throw new Error(
    `SEALED_RUNWAY_SIGNED_URL_RESET_TASK_COUNT_INVALID:${graphTasks.length}`,
  );
}
if (failedVideos.length !== 13) {
  throw new Error(
    `SEALED_RUNWAY_SIGNED_URL_RESET_FAILED_VIDEO_COUNT_INVALID:${failedVideos.length}`,
  );
}
if (completedSoundtracks.length !== 1) {
  throw new Error(
    `SEALED_RUNWAY_SIGNED_URL_RESET_SOUNDTRACK_STATE_INVALID:${completedSoundtracks.length}`,
  );
}
if (unexpectedActive.length) {
  throw new Error(
    `SEALED_RUNWAY_SIGNED_URL_RESET_UNEXPECTED_ACTIVE_TASKS:${unexpectedActive.map((task) => task.id).join(",")}`,
  );
}
...
for (const task of failedVideos) {
  if (providerJobId(task)) {
    throw new Error(
      `SEALED_RUNWAY_SIGNED_URL_RESET_PROVIDER_JOB_EXISTS:${task.id}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `SEALED_RUNWAY_SIGNED_URL_RESET_TASK_APPROVAL_INVALID:${task.id}`,
    );
  }
}
...
for (const usage of usages) {
  ...
  if (
    ...
    throw new Error(
      `SEALED_RUNWAY_SIGNED_URL_RESET_USAGE_NOT_RELEASED:${usage.id}:${state}`,
    );
  }
}
...
for (const task of failedVideos) {
  await ProductionTaskRuntime.update(task.id, {
    ...
    metadata: {
      ...
      runway_signed_url_refresh_retry_contract:
        "CREATIVE_RUNWAY_SIGNED_URL_REFRESH_RETRY_V1",
      runway_signed_url_refresh_retry_count:
        Number(
          task.metadata?.runway_signed_url_refresh_retry_count || 0,
        ) + 1,
      runway_signed_url_refresh_retry_reason:
        "STALE_OR_MALFORMED_SUPABASE_SIGNED_SOURCE_URL",
...
if (walletAfter !== walletBefore) {
  throw new Error(
    `SEALED_RUNWAY_SIGNED_URL_RESET_WALLET_CHANGED:${walletBefore}:${walletAfter}`,
  );
}
