...
const CONTRACT = "AVANTIQO_CODE_CAPACITY_TIMEOUT_RECOVERY_V1";
...
const [job, healthRaw] = await Promise.all([
  serverless(apiKey, endpointId, `/status/${encodeURIComponent(failedJobId)}`),
  serverless(apiKey, endpointId, "/health"),
]);
const failedStatus = text(job?.status).toUpperCase();
const health = healthCounters(healthRaw || {});

if (!TERMINAL_TIMEOUT_STATUSES.has(failedStatus)) {
  throw new Error(`CODE_TIMEOUT_RECOVERY_JOB_NOT_TIMEOUT_TERMINAL:job=${failedJobId}:status=${failedStatus || "UNKNOWN"}`);
}
...
const tempPath = resolve(scriptDir, `.avantiqo-code-capacity-timeout-recovery-${process.pid}.mjs`);
...
console.log(JSON.stringify({
  ...
  failed_job: {
    id: failedJobId,
    status: failedStatus,
    verified_timeout_terminal: true,
  },
  endpoint: {
    id: endpointId,
  ...
  health,
  equal_stock_rank_allowed_only_after_verified_timeout: true,
  equal_rank_requires_material_scheduler_advantage: true,
