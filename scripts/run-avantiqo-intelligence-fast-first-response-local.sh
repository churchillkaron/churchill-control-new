...
git -C "$SHADOW_ROOT" fetch --quiet --depth 1 origin "$ORIGIN_MAIN" \
  || fail "ISOLATED_EXACT_MAIN_FETCH_FAILED"
git -C "$SHADOW_ROOT" checkout --quiet -B main FETCH_HEAD \
  || fail "ISOLATED_EXACT_MAIN_CHECKOUT_FAILED"
SHADOW_HEAD="$(git -C "$SHADOW_ROOT" rev-parse HEAD 2>/dev/null || true)"
[ "$SHADOW_HEAD" = "$ORIGIN_MAIN" ] || fail "ISOLATED_HEAD_NOT_FETCHED_ORIGIN_MAIN"
ln -s "$SOURCE_ROOT/node_modules" "$SHADOW_ROOT/node_modules" \
  || fail "NODE_MODULES_LINK_FAILED"
...
const fastConfiguration = configuration?.execution_lanes?.fast || {};
console.log(`AVANTIQO_FAST_MODEL=${fastConfiguration.model || "UNKNOWN"}`);
console.log(`AVANTIQO_FAST_RUNTIME_READY=${fastConfiguration.runtime_ready === true ? "YES" : "NO"}`);
    ...
    messages: [
      {
        role: "system",
        content: [
          "You are Avantiqo Intelligence, Avantiqo's owned business and engineering intelligence.",
          "Answer naturally, concisely, and practically.",
          "Do not output JSON unless explicitly requested.",
          "Do not expose private chain-of-thought or fabricate live facts.",
        ].join(" "),
      },
