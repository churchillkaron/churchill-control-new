...
WARM_PID=""
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/avantiqo-fast-warm-response-v2.XXXXXX")"
WARM_RESULT="$TMP_DIR/warm-result.txt"
WARM_STATUS="$TMP_DIR/warm-status.txt"

fail() {
  echo ""
  echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2=FAIL"
  echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2_REASON=$1"
  exit 1
}

stop_warm_child() {
  if [ -n "$WARM_PID" ] && kill -0 "$WARM_PID" 2>/dev/null; then
...
if (!endpointId || !apiKey) throw new Error("FAST_CLEANUP_CONFIGURATION_REQUIRED");
...
const request = async (path, method = "GET") => {
  ...
  if (!response.ok || !body) throw new Error(`FAST_CLEANUP_HTTP_${response.status}:${path}`);
  return body;
};
...
if (progress !== 0) {
  console.log(`AVANTIQO_FAST_CLEANUP_REFUSED_IN_PROGRESS=${progress}`);
  process.exit(2);
}
if (queued > 1) {
  console.log(`AVANTIQO_FAST_CLEANUP_REFUSED_MULTIPLE_QUEUED=${queued}`);
  process.exit(3);
}
if (queued === 1) await request("/purge-queue", "POST");
...
throw new Error("FAST_CLEANUP_VERIFY_FAILED");
NODE
  )
}
...
restore_deep() {
  ...
  echo ""
  echo "================ SAFE FAST CLEANUP ================"
  set +e
  safe_cleanup_fast_queue
  local queue_status=$?
  set -e
  if [ "$queue_status" -ne 0 ]; then
    echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2_QUEUE_CLEANUP=REFUSED"
    RESTORED=NO
    return "$queue_status"
  fi
  echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2_QUEUE_CLEANUP=PASS"
  ...
  if [ "$restore_status" -eq 0 ]; then
    FAST_ACTIVE=NO
    echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2_RESTORE=PASS"
    return 0
  fi
  RESTORED=NO
  echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2_RESTORE=FAIL"
...
echo "============================================================"
echo "AVANTIQO FAST INTELLIGENCE - VISIBLE WARM FIRST RESPONSE V2"
echo "============================================================"
...
[ -z "$(git status --porcelain --untracked-files=no)" ] || fail "LOCAL_MAIN_TRACKED_DIRTY"
echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2_MAIN=$HEAD_SHA"
...
echo ""
echo "================ VERIFY CANONICAL LIVE STATE ================"
LIVE_STATE_OUTPUT="$(node --env-file=.env.local scripts/diagnose-avantiqo-intelligence-lane-live-state-local.mjs)" \
  || fail "LIVE_STATE_DIAGNOSTIC_FAILED"
printf '%s\n' "$LIVE_STATE_OUTPUT"
printf '%s\n' "$LIVE_STATE_OUTPUT" \
...
if (!healthResponse.ok) throw new Error(`FAST_WARM_HEALTH_HTTP_${healthResponse.status}`);
...
WARM_MONITOR_START=$SECONDS
while kill -0 "$WARM_PID" 2>/dev/null; do
  sleep 15
  if ! kill -0 "$WARM_PID" 2>/dev/null; then break; fi
  ELAPSED=$((SECONDS - WARM_MONITOR_START))
  echo ""
  echo "================ FAST WARM PROGRESS ${ELAPSED}s ================"
  set +e
  PREFLIGHT_ELAPSED_SECONDS="$ELAPSED" node --env-file=.env.local --input-type=module <<'NODE'
const endpointId = String(process.env.RUNPOD_AVANTIQO_INTELLIGENCE_FAST_ENDPOINT_ID || "").trim();
const apiKey = String(process.env.RUNPOD_API_KEY || "").trim();
...
try {
  ...
  try { body = raw ? JSON.parse(raw) : null; } catch { body = null; }
  if (!response.ok || !body) {
    console.log(`AVANTIQO_FAST_WARM_PROGRESS_HEALTH=UNAVAILABLE:${response.status}`);
    process.exit(0);
  }
  console.log(`AVANTIQO_FAST_WARM_PROGRESS=${JSON.stringify({
    elapsed_seconds: elapsed,
    jobs: body?.jobs || {},
    workers: body?.workers || {},
  })}`);
} catch (error) {
  console.log(`AVANTIQO_FAST_WARM_PROGRESS_HEALTH=UNAVAILABLE:${String(error?.name || "Error")}`);
}
NODE
  set -e
done
...
cat "$WARM_RESULT"
WARM_EXIT="1"
[ -f "$WARM_STATUS" ] && WARM_EXIT="$(cat "$WARM_STATUS")"
[ "$WARM_EXIT" -eq 0 ] || fail "FAST_WARM_ROUTE_FAILED"
...
echo ""
echo "================ FINAL CANONICAL LIVE STATE ================"
FINAL_STATE_OUTPUT="$(node --env-file=.env.local scripts/diagnose-avantiqo-intelligence-lane-live-state-local.mjs)" \
...
echo "AVANTIQO_INTELLIGENCE_FAST_WARM_FIRST_RESPONSE_V2=PASS"
