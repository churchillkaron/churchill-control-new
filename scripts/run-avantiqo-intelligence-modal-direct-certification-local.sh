...
CONTRACT="AVANTIQO_INTELLIGENCE_MODAL_DIRECT_CERT_WRAPPER_V1"
...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/avantiqo-intelligence-modal-cert.XXXXXX")"
...
if [[ "$modal_version" != "$PINNED_MODAL_VERSION" ]]; then
  echo "AVANTIQO_INTELLIGENCE_MODAL_JS_SDK_LOCAL_SYNC_START pinned=${PINNED_MODAL_VERSION} scripts=false"
  ...
  echo "AVANTIQO_INTELLIGENCE_MODAL_JS_SDK_LOCAL_SYNC=PASS version=${modal_version} tracked_package_metadata_preserved=true"
fi
...
OUTPUT_DIR="$ROOT/local-audit-output/avantiqo-intelligence-modal-direct-service-certification"
mkdir -p "$OUTPUT_DIR"
...
run_certification() {
  ...
  case "$MODE" in
    ...
    *) node --env-file="$ROOT/.env.local" scripts/certify-avantiqo-intelligence-modal-direct-service-live.mjs ;;
  esac
}
...
(
  ...
  export AVANTIQO_INTELLIGENCE_MODAL_CERT_OUTPUT_DIR="$OUTPUT_DIR"
