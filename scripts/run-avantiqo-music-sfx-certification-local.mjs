...
loadAvantiqoEnv();
const CONTRACT="AVANTIQO_MUSIC_SFX_CERTIFICATION_V1";
...
const benchmarkId=`sfx-${Date.now()}-${crypto.randomUUID().slice(0,8)}`;
const organizationId=`benchmark-${crypto.randomUUID()}`;
const outputPath=`${organizationId}/benchmark/music-sfx/${benchmarkId}/alarm-clock.wav`;
...
const instruction="A harsh digital bedside alarm clock ringing repeatedly at 06:00 in a dark quiet bedroom, urgent electronic beeps with realistic small-room reflections, no music and no voice.";
const payload={ capability:CAPABILITY, instruction, duration_seconds:4,
  storage_upload:{signed_url:upload.signedUrl,storage_reference:outputReference},
  structured_specification:{generation:{duration_seconds:4},provider_parameters:{num_inference_steps:100,cfg_scale:4.0}},
  certification:{contract:CONTRACT,benchmark_only:true,max_provider_jobs:1,production_activation_allowed:false,pricing_activation_allowed:false} };
