...
token = os.environ.get('HF_TOKEN') or os.environ.get('HUGGINGFACE_HUB_TOKEN') or os.environ.get('HUGGING_FACE_HUB_TOKEN') or ''
receipt = {'success': False, 'contract': 'AVANTIQO_VIDEO_LTX25_PRELOAD_V1', 'repo': repo, 'revision': None, 'files': []}
try:
...
async function runPreload(snapshot) {
  ...
  const receiptPath = `${ORGANIZATION_ID}/generated/avantiqo-video/.ltx25-receipts/scene1-preload-${RUN_ID}.json`;
  const receiptUpload = await signedUpload(receiptPath);
  let lease = null;
  let pod = null;
  try {
    lease = await acquireLease(ownerRequestId);
    const templateEnv = object(snapshot.templateEnv);
    pod = await createFromPool({
      snapshot,
      name: `avantiqo-video-ltx25-preload-${RUN_ID}`.slice(0, 100),
    ...
    if (receipt?.success !== true) throw new Error(receipt?.error_code || "AVANTIQO_VIDEO_LTX25_PRELOAD_FAILED");
    ...
    console.log("AVANTIQO_VIDEO_LTX25_PRELOAD=PASS");
    return receipt;
  } finally {
    if (pod?.id) await deleteAndConfirm(pod.id).catch(() => null);
    if (lease?.id) await releaseLease(lease.id, ownerRequestId, "RELEASED", "VIDEO_LTX25_PRELOAD_COMPLETE").catch(() => null);
  }
}
...
async function uploadOpeningFrame() {
  ...
  return { storage_path: storagePath, storage_reference: storageRef(storagePath), signed_url: data.signedUrl, size_bytes: bytes.length };
}
    ...
    input: {
      ...
      cinematic_control: {
        ...
        shot_specification: {
          ...
          purpose: "Premium dawn establishing shot that opens the Avantiqo film before the business wakes.",
          duration_seconds: 5,
          opening_frame: "Exact supplied approved reference frame",
          closing_frame: "A physically coherent continuation of the same skyline five seconds later",
        },
        camera: {
          ...
          movement_speed: "restrained commercial-film pace",
          yaw: "locked",
          roll: "locked",
        },
        continuity: {
        ...
        frame_contract: {
          first_frame: "SUPPLIED_REFERENCE_FRAME",
          no_reset: true,
          no_jump_cut: true,
        },
...
async function runScene(snapshot, reference) {
  ...
  const receiptPath = `${ORGANIZATION_ID}/generated/avantiqo-video/.ltx25-receipts/scene1-${RUN_ID}.json`;
  const [outputUpload, receiptUpload] = await Promise.all([signedUpload(outputPath), signedUpload(receiptPath)]);
  ...
  try {
    ...
    pod = await createFromPool({
      snapshot,
      name: `avantiqo-video-ltx25-scene1-${RUN_ID}`.slice(0, 100),
      imageName: IMMUTABLE_IMAGE,
      ...
      env: {
        ...
        AVANTIQO_VIDEO_LTX25_MODEL_ROOT: "/runpod-volume/ltx-2.5",
        AVANTIQO_VIDEO_LTX25_NVFP4_ENABLED: "0",
        AVANTIQO_VIDEO_LTX25_NVFP4_KERNEL_CERTIFIED: "0",
        AVANTIQO_VIDEO_LTX25_HARD_TIMEOUT_SECONDS: "6300",
      },
      command: ["python", "-u", "/app/ltx25_worker.py"],
    });
    let lastRefresh = Date.now();
    ...
    if (receipt?.success !== true) throw new Error(receipt?.error_code || "AVANTIQO_VIDEO_LTX25_SCENE1_FAILED");
...
async function main() {
  ...
  const report = {
    success: false,
    contract: "AVANTIQO_VIDEO_LTX25_SCENE1_PROOF_V1",
  ...
  try {
    ...
    const preloadSnapshot = await videoPodCandidateSnapshot();
    const preload = await runPreload(preloadSnapshot);
    report.preload = {
      success: true,
    ...
    const sceneSnapshot = await videoPodCandidateSnapshot();
    const generation = await runScene(sceneSnapshot, openingFrame);
