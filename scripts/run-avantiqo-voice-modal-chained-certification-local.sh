...
CONTRACT="AVANTIQO_VOICE_MODAL_CHAINED_CERT_WRAPPER_V1"
...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/avantiqo-voice-modal-cert.XXXXXX")"
...
OUTPUT_DIR="$ROOT/local-audit-output/avantiqo-voice-modal-chained-service-certification"
mkdir -p "$OUTPUT_DIR"
STATE_PATH="$OUTPUT_DIR/state.json"

# Preserve Audio's safety model: only retry exact failures that happen before
# any Modal provider job can exist. Ambiguous submission/runtime failures are
# never cleared automatically because that could duplicate paid GPU work.
if [[ "$MODE" == "execute" && -f "$STATE_PATH" ]]; then
  retryable_state="$(node -e '
    ...
    const allowed = new Set([
      "AVANTIQO_PROVIDER_PAYER_ORGANIZATION_REQUIRED:avantiqo-voice",
      "AVANTIQO_PROVIDER_PAYER_VERIFICATION_REQUIRED:avantiqo-voice",
    ]);
    const retryable =
      s?.contract === "AVANTIQO_VOICE_MODAL_CHAINED_SERVICE_CERTIFICATION_V1" &&
  ...
  if [[ "$retryable_state" == "YES" ]]; then
    ...
    mv "$STATE_PATH" "$OUTPUT_DIR/state-pre-provider-funding-failure-${stamp}.json"
    echo "AVANTIQO_VOICE_MODAL_CERT_PRE_PROVIDER_FAILURE_REOPENED=PASS duplicate_job_possible=false"
  fi
fi

run_certification() {
  if [[ "$MODE" == "execute" ]]; then
    node --env-file="$ROOT/.env.local" scripts/certify-avantiqo-voice-modal-chained-service-live.mjs --execute
    return
  fi
  if [[ "$MODE" == "resume" ]]; then
    node --env-file="$ROOT/.env.local" scripts/certify-avantiqo-voice-modal-chained-service-live.mjs --resume
    return
  fi
  node --env-file="$ROOT/.env.local" scripts/certify-avantiqo-voice-modal-chained-service-live.mjs
}

(
  cd "$WORKTREE"
  export NODE_ENV=development
  export AVANTIQO_VOICE_MODAL_CHAINED_REAL_INFERENCE_APPROVED="${AVANTIQO_VOICE_MODAL_CHAINED_REAL_INFERENCE_APPROVED:-NO}"
  export AVANTIQO_VOICE_MODAL_CHAIN_CERT_EXPECTED_MAIN_COMMIT="$MAIN_SHA"
  export AVANTIQO_VOICE_MODAL_CHAIN_CERT_SOURCE_MAIN_COMMIT="$MAIN_SHA"
  export AVANTIQO_VOICE_MODAL_CHAIN_CERT_OUTPUT_DIR="$OUTPUT_DIR"
  run_certification
)
