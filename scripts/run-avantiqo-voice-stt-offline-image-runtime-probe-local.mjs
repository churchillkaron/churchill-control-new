...
const CONTRACT = "AVANTIQO_VOICE_STT_OFFLINE_IMAGE_RUNTIME_PROBE_V1";
...
const SOURCE_LOCK = Object.freeze({
  ...
  dockerfile_blob: "20f78962557f83f826ea06ba3310adcfc8a1655e",
...
function runGit(args) {
  const result = spawnSync("git", args, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], env: process.env });
  if (result.status !== 0) throw new Error(`GIT_${String(args[0] || "COMMAND").toUpperCase()}_FAILED:${redact(result.stderr || result.stdout)}`);
  return text(result.stdout);
}
function gitSucceeds(args) {
  return spawnSync("git", args, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], env: process.env }).status === 0;
}
...
function sourceBlobs(ref) {
  return {
    handler_blob: runGit(["rev-parse", `${ref}:${SOURCE_LOCK.handler_path}`]),
    dockerfile_blob: runGit(["rev-parse", `${ref}:${SOURCE_LOCK.dockerfile_path}`]),
    requirements_blob: runGit(["rev-parse", `${ref}:${SOURCE_LOCK.requirements_path}`]),
  };
}
function assertLockedBlobs(blobs, label) {
  for (const key of ["handler_blob", "dockerfile_blob", "requirements_blob"]) {
    if (blobs[key] !== SOURCE_LOCK[key]) throw new Error(`${CONTRACT}_${label}_${key.toUpperCase()}_CHANGED:${blobs[key]}`);
  }
}
function assertOfflineDockerfile(ref, label) {
  const dockerfile = runGit(["show", `${ref}:${SOURCE_LOCK.dockerfile_path}`]);
  const requiredFragments = [
    `expected_revision = "${FOUNDATION_REVISION}"`,
...
function certifiedImageEvidence() {
  ...
  const evidence = gitJson("origin/main", EVIDENCE_PATH);
  ...
  ) throw new Error(`${CONTRACT}_CERTIFIED_OFFLINE_IMAGE_EVIDENCE_REQUIRED`);
  const resolvedSource = runGit(["rev-parse", `${sourceSha}^{commit}`]);
  if (resolvedSource !== sourceSha) throw new Error(`${CONTRACT}_SOURCE_SHA_INVALID:${resolvedSource}`);
  if (!gitSucceeds(["merge-base", "--is-ancestor", sourceSha, "origin/main"])) throw new Error(`${CONTRACT}_SOURCE_NOT_ON_MAIN:${sourceSha}`);
  const imageBlobs = sourceBlobs(sourceSha);
  const mainBlobs = sourceBlobs("origin/main");
  assertLockedBlobs(imageBlobs, "IMAGE_SOURCE");
  assertLockedBlobs(mainBlobs, "NEWEST_MAIN");
  assertOfflineDockerfile(sourceSha, "IMAGE_SOURCE");
  assertOfflineDockerfile("origin/main", "NEWEST_MAIN");
  return {
    image,
    image_digest: text(evidence.image_digest),
    github_run_id: text(evidence.github_run_id) || null,
    source_sha: sourceSha,
    source_is_ancestor_of_main: true,
    newest_main_voice_equivalent: JSON.stringify(imageBlobs) === JSON.stringify(mainBlobs),
    blobs: imageBlobs,
...
async function selectQueueCredential(endpointId, managementKey) {
  ...
  throw new Error(`${CONTRACT}_QUEUE_CREDENTIAL_NOT_FOUND`);
}
...
if (!approved(process.env.AVANTIQO_VOICE_STT_OFFLINE_RUNTIME_PROBE_APPROVED)) {
  throw new Error("AVANTIQO_VOICE_STT_OFFLINE_RUNTIME_PROBE_APPROVED=YES_REQUIRED");
}

if (!approved(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_ACTIVE)) {
  if (!approved(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_APPROVED)) throw new Error("AVANTIQO_RUNPOD_SAFE_LEASE_APPROVED=YES_REQUIRED");
...
if (text(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_CONTRACT) !== SAFE_LEASE_CONTRACT || text(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_LANE) !== LANE) {
  throw new Error(`${CONTRACT}_VALID_SAFE_LEASE_REQUIRED`);
}
...
if (text(process.env.AVANTIQO_RUNPOD_SAFE_LEASE_ENDPOINT_ID) !== endpointId) throw new Error(`${CONTRACT}_LEASE_ENDPOINT_MISMATCH`);
if (finite(endpoint.workersMin, -1) !== 0 || finite(endpoint.workersMax, -1) !== 1) throw new Error(`${CONTRACT}_LEASE_CAPACITY_REQUIRED:${finite(endpoint.workersMin)}/${finite(endpoint.workersMax)}`);
if (text(template.imageName) !== evidence.image) throw new Error(`${CONTRACT}_CERTIFIED_IMAGE_NOT_BOUND:${text(template.imageName) || "MISSING"}`);
if (!text(template.containerRegistryAuthId)) throw new Error(`${CONTRACT}_GHCR_REGISTRY_AUTH_REQUIRED`);
if (list(template.dockerEntrypoint).length || list(template.dockerStartCmd).length) throw new Error(`${CONTRACT}_LAUNCH_OVERRIDE_PRESENT`);
...
if (before.jobs.in_queue !== 0 || before.jobs.in_progress !== 0) throw new Error(`${CONTRACT}_QUEUE_NOT_CLEAN:${JSON.stringify(before.jobs)}`);

console.log(JSON.stringify({
  event: `${CONTRACT}_PREFLIGHT`,
  contract: CONTRACT,
  endpoint_name: ENDPOINT_NAME,
  workers_min: finite(endpoint.workersMin),
  workers_max: finite(endpoint.workersMax),
  certified_image: evidence.image,
...
try {
  ...
  console.log(JSON.stringify({ event: `${CONTRACT}_SUBMITTED`, job_id_present: true, probe_jobs_submitted: 1, transcription_jobs_submitted: 0 }));
  ...
  while (Date.now() - started < COMPLETE_TIMEOUT_MS) {
    ...
    console.log(JSON.stringify({ event: `${CONTRACT}_PROGRESS`, elapsed_seconds: Math.round((Date.now() - started) / 1000), state: state || null, claimed, jobs: summary.jobs, workers: summary.workers }));
    if (state === "COMPLETED") {
      terminal = status;
      completed = true;
      break;
    }
    if (!claimed && Date.now() - started > CLAIM_TIMEOUT_MS) throw new Error(`${CONTRACT}_NOT_CLAIMED:state=${state || "UNKNOWN"}`);
    if (["FAILED", "CANCELLED", "CANCELED", "TIMED_OUT"].includes(state)) throw new Error(`${CONTRACT}_TERMINAL_${state}:${redact(status.error || status.output?.error)}`);
  }

  if (!completed || !terminal) throw new Error(`${CONTRACT}_COMPLETION_TIMEOUT`);
  const output = object(terminal.output);
  if (text(output.status) !== "completed") throw new Error(`${CONTRACT}_OUTPUT_STATUS_INVALID`);
  if (text(output.operation) !== "runtime_probe") throw new Error(`${CONTRACT}_OPERATION_INVALID`);
  if (text(output.probe_contract) !== EXPECTED_PROBE_CONTRACT) throw new Error(`${CONTRACT}_PROBE_CONTRACT_INVALID:${text(output.probe_contract)}`);
  if (text(output.runtime_revision) !== EXPECTED_RUNTIME_REVISION) throw new Error(`${CONTRACT}_RUNTIME_REVISION_INVALID:${text(output.runtime_revision)}`);
  if (text(output.foundation_model) !== EXPECTED_FOUNDATION) throw new Error(`${CONTRACT}_FOUNDATION_INVALID`);
  if (output.cuda_available !== true) throw new Error(`${CONTRACT}_CUDA_NOT_AVAILABLE`);
  if (output.inference_performed !== false || output.model_download_performed !== false || output.storage_mutation_performed !== false) throw new Error(`${CONTRACT}_SIDE_EFFECT_CONTRACT_INVALID`);
  ...
  console.log(JSON.stringify({
    ...
    job_claimed_observed: claimed,
    job_completed: true,
    certified_offline_image: evidence.image,
    foundation_revision_baked: evidence.foundation_revision,
    runtime: {
    ...
    offline_image_runtime_ready: true,
    real_transcription_required_for_this_hardening: false,
    tts_touched: false,
    music_touched: false,
    secrets_printed: false,
  }, null, 2));
