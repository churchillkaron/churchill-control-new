...
CREATIVE_MIGRATIONS=(
  ...
  "$PROJECT_ROOT/supabase/migrations/20260726101500_creative_publication_idempotency.sql"
)
...
echo "============================================================"
echo "AVANTIQO FINANCE WORKSPACE CRUD CERTIFICATION"
echo "============================================================"
...
if [ -n "$(git status --porcelain)" ]; then
  git status --short
  fail "Working tree must be clean before CRUD certification"
fi
...
[ -f "supabase/migrations/$MIGRATION" ] || fail "CRUD certification migration is missing"
[ -f scripts/finance-workspace-crud-certification.mjs ] || fail "CRUD certification runner is missing"
...
echo ""
echo "================ CERTIFICATION MIGRATION DRY RUN ================"
npx supabase db push --dry-run 2>&1 | tee "$DRY_LOG"
DRY_STATUS=${PIPESTATUS[0]}
if [ "$DRY_STATUS" -ne 0 ]; then
  fail "CRUD certification migration dry run failed"
fi

if grep -q "Remote database is up to date" "$DRY_LOG"; then
  echo "CRUD certification probe is already deployed."
elif grep -q "$MIGRATION" "$DRY_LOG"; then
  UNEXPECTED_VERSIONS="$(grep -Eo '[0-9]{14}' "$DRY_LOG" | sort -u | grep -v '^20260726121500$' || true)"
  if [ -n "$UNEXPECTED_VERSIONS" ]; then
    echo "$UNEXPECTED_VERSIONS"
    fail "Unexpected pending migrations detected"
  fi
  echo ""
  printf "Press Enter to deploy the rollback-safe CRUD certification probe, or Control-C to stop..."
  IFS= read -r _
  npx supabase db push --yes || fail "CRUD certification migration deployment failed"
else
  fail "Dry run did not show the expected CRUD certification migration"
fi
...
echo "This certification inserts, reads, updates and archives temporary Finance workspace records inside a PostgreSQL subtransaction."
echo "Every temporary row and provisioned prerequisite is deliberately rolled back before the probe returns."
printf "Type RUN_ROLLBACK_SAFE_FINANCE_CRUD_CERTIFICATION exactly: "
IFS= read -r FINANCE_CERT_CONFIRM
if [ "$FINANCE_CERT_CONFIRM" != "RUN_ROLLBACK_SAFE_FINANCE_CRUD_CERTIFICATION" ]; then
  fail "CRUD certification confirmation did not match"
fi

echo ""
echo "================ RUN CRUD CERTIFICATION ================"
FINANCE_CERT_BASE_URL="$BASE_URL" \
FINANCE_CERT_ORGANIZATION_ID="$ORGANIZATION_ID" \
FINANCE_CERT_ENTITY_ID="$ENTITY_ID" \
FINANCE_CERT_ACTOR_ID="$ACTOR_ID" \
FINANCE_CERT_ACCESS_TOKEN="$ACCESS_TOKEN" \
FINANCE_CERT_COOKIE="$COOKIE_HEADER" \
FINANCE_CERT_CONFIRM="$FINANCE_CERT_CONFIRM" \
FINANCE_CERT_REPORT="$REPORT" \
NEXT_PUBLIC_SUPABASE_URL="$SUPABASE_URL" \
SUPABASE_SERVICE_ROLE_KEY="$SERVICE_ROLE_KEY" \
node scripts/finance-workspace-crud-certification.mjs
CERT_STATUS=$?
...
if [ "$CERT_STATUS" -eq 0 ]; then
  echo "FINANCE WORKSPACE CRUD CERTIFICATION PASSED"
else
  echo "FINANCE WORKSPACE CRUD CERTIFICATION FAILED"
fi
