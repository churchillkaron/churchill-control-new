"""Avantiqo Code Qwen3.8 Private12 V10 certification wrapper.

V10 returns to the paid-proven Runtime V7 / 320-token reviewer envelope and
changes only the repository agent to Repo Agent V11. V11 replaces prompt bloat
with compact visible-contract reasoning for monetary aggregation, comparator hot
paths, and shared producer/consumer field authority.

The inherited strict certification runner preserves:
* 12 sealed quality dimensions;
* <=4 quality batches;
* exactly three representative warm single-request latency probes;
* <=7 total Modal calls;
* <=4000 ms warm p95 requirement;
...
CONTRACT = "AVANTIQO_CODE_QWEN38_PRIVATE12_CERT_V10"
RUN_SEED = "20260904-qwen38-private12-v10"
...
@app.local_entrypoint(name="qwen38_private12_v10")
def qwen38_private12_v10(approved: bool = False) -> None:
