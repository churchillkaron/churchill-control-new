"""Avantiqo Code Qwen3.8 Private12 V14 certification wrapper.

V14 restores the implementation side to the V12-quality architecture while
keeping only the compact reviewer improvement from V13:

* Repo Agent V13 delegates actor, parser, repair and recovery to Repo Agent V11;
* Runtime V9 delegates actor structured output to Runtime V7 exactly;
* reviewer uses compact four-field JSON at 256 tokens;
* repair actor keeps 1024-token headroom;
* initial actor stays 640 tokens;
* call ceilings and <=4s warm certification target remain unchanged.

No hidden material, benchmark-specific rewriter, extra model call, new model,
production routing or storage change is introduced.
"""
...
CONTRACT = "AVANTIQO_CODE_QWEN38_PRIVATE12_CERT_V14"
RUN_SEED = "20260904-qwen38-private12-v14"
...
@app.local_entrypoint(name="qwen38_private12_v14")
def qwen38_private12_v14(approved: bool = False) -> None:
