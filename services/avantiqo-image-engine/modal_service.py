"""Authenticated async HTTP transport for Avantiqo Image on Modal."""
...
def image_api():
    ...
    web = FastAPI(title="Avantiqo Image Modal Transport", docs_url=None, redoc_url=None)

    @web.middleware("http")
    async def authenticate(request: Request, call_next):
        supplied = _text(request.headers.get("authorization"))
        if not hmac.compare_digest(supplied, expected):
            return JSONResponse(status_code=401, content={"detail": "AVANTIQO_IMAGE_MODAL_GATEWAY_UNAUTHORIZED"})
    ...
    async def status(job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "contract": HTTP_CONTRACT, "transport": TRANSPORT, "status": "FAILED", "job_id": normalized, "error_code": "AVANTIQO_IMAGE_MODAL_GENERATION_FAILED", "error_type": type(exc).__name__[:120], "error_message": _text(exc)[:800]}
        if not isinstance(result, dict):
            return {"success": False, "contract": HTTP_CONTRACT, "transport": TRANSPORT, "status": "FAILED", "job_id": normalized, "error_code": "AVANTIQO_IMAGE_MODAL_OUTPUT_OBJECT_REQUIRED"}
