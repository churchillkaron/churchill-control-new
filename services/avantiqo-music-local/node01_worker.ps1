...
function ResourceProfile($Job) {
  ...
  if ($workload -eq 'music_generation') { return @{ class='heavy_cpu'; gpu_vram_mb=0; cpu_weight='heavy'; exclusive_gpu=$false; product='Music / Generation'; mode='CPU_FLOAT32' } }
...
function ClaimJobs {
  return @(Rpc 'claim_avantiqo_local_compute_jobs' @{
    p_node_id=$NodeId; p_node_token=(NodeToken); p_capabilities=$Capabilities; p_limit=1; p_lease_seconds=$(if ($Lane -eq 'cpu' -and $CpuCapabilities -contains 'ai.music.generate') { 1800 } else { 300 })
  })
}
...
function RunMusicGenerationJob($Job) {
  ...
  try {
    ...
    $psi.Arguments = ('"' + $runner + '" --input "' + $tmp + '"')
    $psi.EnvironmentVariables['PATH'] = $ffmpeg + ';' + [Environment]::GetEnvironmentVariable('PATH')
    $psi.EnvironmentVariables['HF_HOME'] = 'C:\Avantiqo\hf-cache'
    $psi.UseShellExecute = $false; $psi.RedirectStandardOutput = $true; $psi.RedirectStandardError = $true; $psi.CreateNoWindow = $true
    $process = New-Object System.Diagnostics.Process; $process.StartInfo = $psi; [void]$process.Start()
    $stdoutTask = $process.StandardOutput.ReadToEndAsync(); $stderrTask = $process.StandardError.ReadToEndAsync()
    $lastLeaseRenewal = Get-Date
    while (-not $process.HasExited) {
      if (((Get-Date) - $lastLeaseRenewal).TotalSeconds -ge 240) {
        try {
          [void](Rpc 'renew_avantiqo_local_compute_job_lease' @{ p_node_id=$NodeId; p_node_token=(NodeToken); p_job_id=$Job.id; p_lease_seconds=1800 })
        } catch {}
        $lastLeaseRenewal = Get-Date
      }
      Start-Sleep -Seconds 5
    ...
    $process.WaitForExit()
    $rawOutput = [string]$stdoutTask.Result; $stderr = [string]$stderrTask.Result
    if ([int]$process.ExitCode -ne 0) { [IO.File]::WriteAllText('C:\ProgramData\Avantiqo\last-music-generation-error.txt',$stderr); $tail=$(if($stderr.Length -gt 1800){$stderr.Substring($stderr.Length-1800)}else{$stderr}); throw ('AVANTIQO_LOCAL_MUSIC_GENERATION_PROCESS_FAILED:' + $tail) }
    $json=$rawOutput.Trim(); if(-not $json){ throw 'AVANTIQO_LOCAL_MUSIC_GENERATION_OUTPUT_REQUIRED' }
    $result=$json | ConvertFrom-Json; $elapsed=[int](((Get-Date)-$started).TotalMilliseconds)
    $result | Add-Member -NotePropertyName node_id -NotePropertyValue $NodeId -Force
