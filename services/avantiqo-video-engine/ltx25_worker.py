...
def native_dimensions(aspect_ratio: str, quality_lane: str) -> tuple[int, int]:
    # LTX-2.5 DFR defaults are model-native high resolution. Keep dimensions aligned
    # to the model's preferred blocks and never introduce a 720p pixel stage.
    hero = quality_lane == "hero"
    if aspect_ratio == "9:16":
        return (1536, 2688) if hero else (1088, 1920)
    if aspect_ratio == "1:1":
        return (1536, 1536) if hero else (1088, 1088)
    return (2688, 1536) if hero else (1920, 1088)
...
def quality_lane(data: dict[str, Any]) -> str:
    explicit = text(data.get("quality_lane") or obj(data.get("quality_profile")).get("lane")).lower()
...
def validate(job: dict[str, Any]) -> dict[str, Any]:
    ...
    requested_resolution = text(data.get("resolution") or "native-high").lower()
...
def transformer_args(lane: str) -> tuple[str, list[str], str]:
    if lane == "production" and TRANSFORMER_NVFP4.is_file():
...
def run_pipeline(data: dict[str, Any], root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    width, height = native_dimensions(data["aspect_ratio"], lane)
