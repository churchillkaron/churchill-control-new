"""Pure text-to-video LTX-2.5 distilled lane for Avantiqo investor-film shots.

No source image, prior video, screenshot, browser capture, or imported visual asset
is accepted by this runtime. It exists specifically for purpose-generated
cinematic investor footage. Studio owns deterministic 4K mastering after the
new 1920x1088 distilled master is generated.
"""
...
CONTRACT = "AVANTIQO_INVESTOR_T2V_DISTILLED_MODAL_V1"
QUALITY_CONTRACT = "AVANTIQO_INVESTOR_T2V_1920X1088_NEW_ASSET_ONLY_V1"
WIDTH = 1920
HEIGHT = 1088
STAGE1_WIDTH = 960
STAGE1_HEIGHT = 544
...
SUBPROCESS_TIMEOUT_SECONDS = 7 * 60 + 30
DISTILLED_TRANSFORMER = "diffusion_models/ltx-2.5-22b-distilled-transformer-bf16.safetensors"
DISTILLED_UPSAMPLER = "latent_upscale_models/ltx-2.5-latent-spatial-upscaler-x2-bf16-1.0.safetensors"
REQUIRED = (
...
def _sanitize(value: Any, limit: int = 1800) -> str:
...
def seed_investor_t2v_cache() -> dict[str, Any]:
    from huggingface_hub import snapshot_download

    model_volume.reload()
    try:
        root = _snapshot()
        return {"success": True, "already_cached": True, "revision": root.name, "gpu_inference_performed": False}
    except RuntimeError:
        pass
    ...
    resolved = Path(snapshot_download(
        ...
        allow_patterns=[DISTILLED_TRANSFORMER, DISTILLED_UPSAMPLER],
        max_workers=8,
    ))
    ...
    model_volume.reload()
    root = _snapshot()
    return {"success": True, "already_cached": False, "revision": root.name, "gpu_inference_performed": False}
...
def _prompt(instruction: str) -> str:
    ...
    return value + (
        " Premium cinematic live-action realism. Purpose-generated frame only. "
        "Natural materials, believable skin, physically plausible hands and motion, restrained camera movement, "
        "controlled highlights, deep blacks, subtle warm practical light, sophisticated composition. "
        "No typography, captions, numbers, logos, watermark, browser, dashboard, floating hologram, neon UI, "
        "generic corporate stock-footage look, plastic faces, warped hands, morphing, flicker or frame collapse."
    )
...
def generate_investor_t2v_master(
    output_relative: str,
    instruction: str,
    ...
    try:
        ...
        raise RuntimeError(f"{CONTRACT}_TIMEOUT:{_sanitize(getattr(exc, 'stdout', '') or getattr(exc, 'output', ''))}") from exc
...
def generate_investor_t2v_job(data: dict[str, Any]) -> dict[str, Any]:
    ...
    relative = Path("runtime-investor-t2v") / _token(organization_id, "org") / _token(usage_id, "usage") / uuid.uuid4().hex / "master.mp4"
    output = Path("/models") / relative
    try:
        generation = generate_investor_t2v_master.remote(str(relative), instruction, duration, seed)
