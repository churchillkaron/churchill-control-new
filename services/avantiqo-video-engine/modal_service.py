"""Lightweight authenticated async gateway for Avantiqo Cinema on Modal.

The gateway is a separate CPU-only Modal App. It never imports the Video GPU
worker module. Authenticated job submission lazily resolves the already deployed
`avantiqo-video-owned.generate` function and spawns it asynchronously.
Health checks therefore cannot initialize the Video GPU worker.
"""
...
GATEWAY_APP_NAME = "avantiqo-video-gateway"
GPU_APP_NAME = "avantiqo-video-owned"
...
def video_api():
    ...
    web = FastAPI(title="Avantiqo Video Modal Gateway", docs_url=None, redoc_url=None)
    ...
    async def authenticate(request: Request, call_next):
        ...
        if not hmac.compare_digest(supplied, expected):
            return JSONResponse(
                status_code=401,
                content={
                    "detail": "AVANTIQO_VIDEO_MODAL_GATEWAY_UNAUTHORIZED",
    ...
    async def status(job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": "FAILED",
                "job_id": normalized,
                "error_code": "AVANTIQO_VIDEO_MODAL_OUTPUT_EXPIRED",
                "raw_reasoning_persisted": False,
            }
            ...
            return {
                ...
                "status": "FAILED",
                "job_id": normalized,
                "error_code": "AVANTIQO_VIDEO_MODAL_GENERATION_FAILED",
        ...
        if not isinstance(result, dict):
            return {
                ...
                "status": "FAILED",
                "job_id": normalized,
                "error_code": "AVANTIQO_VIDEO_MODAL_OUTPUT_OBJECT_REQUIRED",
                "raw_reasoning_persisted": False,
            }
        output = _safe(result)
        if output.get("raw_reasoning_persisted") is not False:
            return {
                ...
                "status": "FAILED",
                "job_id": normalized,
                "error_code": "AVANTIQO_VIDEO_MODAL_REASONING_BOUNDARY_INVALID",
                "raw_reasoning_persisted": False,
            }
        return {
            "success": True,
