...
begin
  ...
  begin
    if to_regclass('public.bank_ledger') is not null then
      execute 'select count(*) from public.bank_ledger where organization_id = $1 and entity_id = $2'
      into v_read_count using p_organization_id, p_entity_id;
      v_table := 'bank_ledger';
    elsif to_regclass('public.cash_flow_snapshots') is not null then
      execute 'select count(*) from public.cash_flow_snapshots where organization_id = $1 and entity_id = $2'
      into v_read_count using p_organization_id, p_entity_id;
      v_table := 'cash_flow_snapshots';
    else
      raise exception 'No cash-management source table exists';
    end if;
