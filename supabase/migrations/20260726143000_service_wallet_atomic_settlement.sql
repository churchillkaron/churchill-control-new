...
begin
  ...
  if v_idempotency_key is not null then
    ...
    select * into v_transaction from public.wallet_transactions where organization_id = p_organization_id and idempotency_key = v_idempotency_key limit 1;
    if found then
      ...
      then raise exception 'WALLET_IDEMPOTENCY_CONFLICT:%', v_idempotency_key; end if;
      select * into v_wallet from public.organization_wallets where id = v_transaction.wallet_id;
  ...
  select * into v_wallet from public.organization_wallets where organization_id = p_organization_id for update;
  if not found then
    if v_currency is null then raise exception 'WALLET_CURRENCY_REQUIRED'; end if;
    insert into public.organization_wallets (id, organization_id, currency, available_balance, reserved_balance, billing_policy, auto_topup, auto_topup_threshold, auto_topup_amount, status, created_at, updated_at)
    values (gen_random_uuid(), p_organization_id, v_currency, 0, 0, 'PREPAID', false, 0, 0, 'ACTIVE', now(), now())
  ...
  if v_operation = 'RESERVE' then
    ...
    update public.organization_wallets set available_balance = coalesce(available_balance, 0) - v_amount, reserved_balance = coalesce(reserved_balance, 0) + v_amount, updated_at = now() where id = v_wallet.id returning * into v_wallet;
    ...
    update public.organization_wallets set reserved_balance = reserved_balance - v_amount, updated_at = now() where id = v_wallet.id returning * into v_wallet;
  elsif v_operation = 'RELEASE' then
    if coalesce(v_wallet.reserved_balance, 0) < v_amount then raise exception 'INSUFFICIENT_RESERVED_WALLET_BALANCE'; end if;
    update public.organization_wallets set available_balance = coalesce(available_balance, 0) + v_amount, reserved_balance = reserved_balance - v_amount, updated_at = now() where id = v_wallet.id returning * into v_wallet;
  elsif v_operation in ('TOPUP', 'REFUND') then
    update public.organization_wallets set available_balance = coalesce(available_balance, 0) + v_amount, updated_at = now() where id = v_wallet.id returning * into v_wallet;
  end if;

  insert into public.wallet_transactions (id, organization_id, wallet_id, type, amount, currency, provider, usage_id, invoice_id, reference, metadata, idempotency_key, created_at)
  values (gen_random_uuid(), p_organization_id, v_wallet.id, v_operation, v_amount, v_wallet.currency, v_provider, p_usage_id, p_invoice_id, v_reference, coalesce(p_metadata, '{}'::jsonb), v_idempotency_key, now())
