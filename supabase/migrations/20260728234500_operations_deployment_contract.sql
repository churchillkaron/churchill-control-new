...
begin
  call add_check(
    'operations_records',
    to_regclass('public.operations_records') is not null
  );
  call add_check(
    'operations_command_ledger',
    to_regclass('public.operations_command_ledger') is not null
  );
  call add_check(
    'operations_event_outbox',
    to_regclass('public.operations_event_outbox') is not null
  );
  call add_check(
    'operations_events',
    to_regclass('public.operations_events') is not null
  );
  call add_check(
    'operations_roles',
    to_regclass('public.operations_roles') is not null
  );
  call add_check(
    'operations_role_permissions',
    to_regclass('public.operations_role_permissions') is not null
  );
  call add_check(
    'user_operations_roles',
    to_regclass('public.user_operations_roles') is not null
  );

  call add_check(
    'execute_operations_command',
    to_regprocedure(
      'public.execute_operations_command(uuid,uuid,uuid,text,text,text,text,jsonb)'
    ) is not null
  );
  call add_check(
    'publish_operations_event_batch',
    to_regprocedure(
      'public.publish_operations_event_batch(uuid,integer)'
    ) is not null
  );
  call add_check(
    'get_operations_event_delivery_health',
    to_regprocedure(
      'public.get_operations_event_delivery_health(uuid,uuid,uuid,integer)'
    ) is not null
  );
  call add_check(
    'retry_operations_dead_letter',
    to_regprocedure(
      'public.retry_operations_dead_letter(uuid,uuid,uuid,uuid)'
    ) is not null
  );
  call add_check(
    'operations_lifecycle_target_status',
    to_regprocedure(
      'public.operations_lifecycle_target_status(text,text,text)'
    ) is not null
  );
...
declare
  v_status jsonb;
begin
  select public.get_operations_deployment_status()
    into v_status;
