...
set code = upper(btrim(code)),
    name = btrim(name),
    type = upper(coalesce(nullif(btrim(type), ''), 'OPERATIONAL')),
...
alter table public.cost_centers
  add constraint cost_centers_type_check
    check (type in ('OPERATIONAL','ADMINISTRATIVE','SALES','SERVICE','PROJECT','SHARED_SERVICE','OTHER')),
...
create unique index if not exists cost_centers_entity_code_uq
  on public.cost_centers (organization_id, entity_id, upper(code))
  where entity_id is not null;

create index if not exists cost_centers_parent_idx
  on public.cost_centers (organization_id, entity_id, parent_cost_center_id);

create index if not exists cost_centers_department_idx
  on public.cost_centers (organization_id, entity_id, department_id);
...
begin
  ...
  new.type := upper(coalesce(nullif(btrim(new.type), ''), 'OPERATIONAL'));
  new.updated_at := now();

  if new.organization_id is null or new.entity_id is null then
    raise exception 'Cost Centre requires organization_id and entity_id';
  end if;

  perform 1 from public.legal_entities
   where id = new.entity_id and organization_id = new.organization_id;
  if not found then
    raise exception 'Cost Centre Legal Entity is outside organisation scope';
  end if;

  if new.parent_cost_center_id is not null then
    select parent_cost_center_id into v_parent
    ...
    if not found then
      raise exception 'Parent Cost Centre is outside the selected Legal Entity or inactive';
    end if;
...
drop trigger if exists finance_cost_center_validate on public.cost_centers;
create trigger finance_cost_center_validate
before insert or update on public.cost_centers
for each row execute function public.finance_validate_cost_center();
...
create table if not exists public.finance_dimensions (
  ...
  constraint finance_dimensions_scope_check check (scope in ('ENTITY','ORGANISATION')),
  constraint finance_dimensions_value_type_check check (value_type in ('LIST','TEXT','NUMBER','DATE','BOOLEAN')),
  constraint finance_dimensions_scope_entity_check check (
    (scope = 'ENTITY' and entity_id is not null) or
    (scope = 'ORGANISATION' and entity_id is null)
  ),
  constraint finance_dimensions_effective_dates_check check (effective_to is null or effective_to >= effective_from),
  constraint finance_dimensions_code_format_check check (code ~ '^[A-Z0-9][A-Z0-9._/-]{0,31}$')
);

create unique index if not exists finance_dimensions_org_scope_code_uq
  on public.finance_dimensions (organization_id, coalesce(entity_id, '00000000-0000-0000-0000-000000000000'::uuid), upper(code));
...
create table if not exists public.finance_dimension_values (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint finance_dimension_values_parent_self_check check (parent_value_id is null or parent_value_id <> id),
  constraint finance_dimension_values_effective_dates_check check (effective_to is null or effective_to >= effective_from),
  constraint finance_dimension_values_code_format_check check (code ~ '^[A-Z0-9][A-Z0-9._/-]{0,31}$')
);

create unique index if not exists finance_dimension_values_dimension_code_uq
  on public.finance_dimension_values (organization_id, dimension_id, upper(code));

create index if not exists finance_dimension_values_parent_idx
  on public.finance_dimension_values (organization_id, dimension_id, parent_value_id);
...
drop trigger if exists finance_dimensions_normalize on public.finance_dimensions;
create trigger finance_dimensions_normalize
before insert or update on public.finance_dimensions
for each row execute function public.finance_normalize_dimension_master();

drop trigger if exists finance_dimension_values_normalize on public.finance_dimension_values;
