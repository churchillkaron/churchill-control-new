...
begin
  ...
  if found then
    select round(coalesce(sum(remaining_balance), 0)::numeric, 2)
    into v_result_remaining
    from public.orders
    where organization_id = p_organization_id
      and (
        id = v_existing_payment.order_id
        or session_id = v_existing_payment.session_id
      );
    ...
    return jsonb_build_object(
      ...
      'fullyPaid', coalesce(v_result_remaining, 0) <= 0,
  ...
  select
    coalesce(sum(allocation_weight), 0),
    count(*)
