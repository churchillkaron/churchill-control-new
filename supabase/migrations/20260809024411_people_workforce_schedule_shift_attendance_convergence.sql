...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'staff_shifts_schedule_id_fkey'
      and conrelid = 'public.staff_shifts'::regclass
  ) then
    alter table public.staff_shifts
      add constraint staff_shifts_schedule_id_fkey
      foreign key (schedule_id)
      references public.staff_schedules(id)
      on delete set null
      not valid;
  end if;
end
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'staff_attendance_shift_id_fkey'
      and conrelid = 'public.staff_attendance'::regclass
  ) then
    alter table public.staff_attendance
      add constraint staff_attendance_shift_id_fkey
...
create index if not exists staff_schedules_org_staff_date_idx
  on public.staff_schedules (organization_id, staff_id, shift_date);

create index if not exists staff_shifts_org_staff_clock_idx
  on public.staff_shifts (organization_id, staff_id, clock_in desc);

create index if not exists staff_shifts_schedule_idx
  on public.staff_shifts (organization_id, schedule_id)
  where schedule_id is not null;

create index if not exists staff_attendance_org_staff_date_idx
  on public.staff_attendance (organization_id, staff_id, shift_date);
...
drop trigger if exists trg_staff_schedules_enforce_people_scope
...
drop trigger if exists trg_staff_shifts_enforce_people_scope
...
drop trigger if exists trg_staff_attendance_enforce_people_scope
