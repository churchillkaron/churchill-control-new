do $$
declare
  context_row record;
  expense_account_id uuid;
  payable_account_id uuid;
  candidate_code integer;
begin
  for context_row in
    select distinct
      profile.organization_id,
      profile.entity_id,
      upper(
        coalesce(
          nullif(btrim(profile.functional_currency), ''),
          nullif(btrim(profile.base_currency), ''),
          nullif(btrim(profile.reporting_currency), ''),
          nullif(btrim(entity.currency), '')
        )
      ) as currency_code
    from public.finance_organization_profiles profile
    join public.legal_entities entity
      on entity.id = profile.entity_id
     and entity.organization_id = profile.organization_id
     and coalesce(entity.is_active, true) = true
    where profile.entity_id is not null
      and upper(btrim(coalesce(profile.status, 'ACTIVE'))) = 'ACTIVE'
      and exists (
        select 1
        from public.organization_services service
        where service.organization_id = profile.organization_id
          and upper(btrim(coalesce(service.status, 'ACTIVE'))) = 'ACTIVE'
          and coalesce(service.usage_enabled, true) = true
          and coalesce(service.billing_enabled, true) = true
      )
  loop
    if context_row.currency_code is null then
      raise exception
        'SERVICE_USAGE_FINANCE_CURRENCY_REQUIRED:%:%',
        context_row.organization_id,
        context_row.entity_id;
    end if;

    expense_account_id := null;
    payable_account_id := null;

    select account.id
      into expense_account_id
    from public.chart_of_accounts account
    where account.organization_id = context_row.organization_id
      and account.entity_id = context_row.entity_id
      and lower(btrim(account.account_name)) = 'platform service expense'
      and coalesce(account.is_active, true) = true
    order by account.account_code
    limit 1;

    if expense_account_id is null then
      for candidate_code in 6800..6899 loop
        if not exists (
          select 1
          from public.chart_of_accounts account
          where account.organization_id = context_row.organization_id
            and account.entity_id = context_row.entity_id
            and account.account_code = candidate_code::text
        ) then
          insert into public.chart_of_accounts (
            organization_id,
            entity_id,
            account_code,
            account_name,
            account_category,
            normal_balance,
            is_active,
            is_system,
            currency_code
          )
          values (
            context_row.organization_id,
            context_row.entity_id,
            candidate_code::text,
            'Platform Service Expense',
            'Operating Expense',
            'Debit',
            true,
            true,
            context_row.currency_code
          )
          returning id into expense_account_id;
          exit;
        end if;
      end loop;
    end if;

    if expense_account_id is null then
      raise exception
        'SERVICE_USAGE_EXPENSE_ACCOUNT_CODE_UNAVAILABLE:%:%',
        context_row.organization_id,
        context_row.entity_id;
    end if;

    select account.id
      into payable_account_id
    from public.chart_of_accounts account
    where account.organization_id = context_row.organization_id
      and account.entity_id = context_row.entity_id
      and lower(btrim(account.account_name)) = 'accounts payable'
      and coalesce(account.is_active, true) = true
    order by account.account_code
    limit 1;

    if payable_account_id is null then
      for candidate_code in 2000..2099 loop
        if not exists (
          select 1
          from public.chart_of_accounts account
          where account.organization_id = context_row.organization_id
            and account.entity_id = context_row.entity_id
            and account.account_code = candidate_code::text
        ) then
          insert into public.chart_of_accounts (
            organization_id,
            entity_id,
            account_code,
            account_name,
            account_category,
            normal_balance,
            is_active,
            is_system,
            currency_code
          )
          values (
            context_row.organization_id,
            context_row.entity_id,
            candidate_code::text,
            'Accounts Payable',
            'Liabilities',
            'Credit',
            true,
            true,
            context_row.currency_code
          )
          returning id into payable_account_id;
          exit;
        end if;
      end loop;
    end if;

    if payable_account_id is null then
      raise exception
        'SERVICE_USAGE_PAYABLE_ACCOUNT_CODE_UNAVAILABLE:%:%',
        context_row.organization_id,
        context_row.entity_id;
    end if;

    if not exists (
      select 1
      from public.finance_posting_rules rule
      where rule.organization_id = context_row.organization_id
        and rule.entity_id = context_row.entity_id
        and upper(btrim(rule.event_type)) = 'SERVICE_USAGE_BILLED'
        and upper(btrim(rule.source_module)) = 'SERVICE'
        and upper(btrim(coalesce(rule.status, 'ACTIVE'))) = 'ACTIVE'
    ) then
      insert into public.finance_posting_rules (
        organization_id,
        entity_id,
        name,
        event_type,
        source_module,
        debit_account_id,
        credit_account_id,
        effective_from,
        priority,
        status
      )
      values (
        context_row.organization_id,
        context_row.entity_id,
        'Platform Service Usage',
        'SERVICE_USAGE_BILLED',
        'SERVICE',
        expense_account_id,
        payable_account_id,
        current_date,
        100,
        'ACTIVE'
      );
    end if;
  end loop;
end
$$;
