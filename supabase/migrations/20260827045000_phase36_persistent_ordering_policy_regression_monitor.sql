...
-- actually affected by the policy may trigger rollback.
...
create table if not exists public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations (
  ...
  constraint avantiqo_persistent_monitor_regret_check
    check (baseline_observed_rank_regret >= 0 and persistent_observed_rank_regret >= 0)
);

create unique index if not exists avantiqo_persistent_monitor_one_evaluation_per_application_idx
  on public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations(policy_id, application_id);

create index if not exists avantiqo_persistent_monitor_org_updated_idx
  on public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations(organization_id, updated_at desc);

alter table public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations enable row level security;
revoke all on table public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations from public, anon, authenticated;
grant select, insert, update on table public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations to service_role;
...
declare
  ...
  v_baseline_correct integer;
  v_persistent_correct integer;
  v_baseline_regret numeric;
  v_persistent_regret numeric;
  v_ambiguous boolean;
  v_regression boolean;
  v_status text;
  v_system_actor text := md5('AVANTIQO_PHASE36_AUTOMATIC_REGRESSION_MONITOR');
  v_reason text;
begin
  ...
  then
    ...
    perform public.rollback_avantiqo_intelligence_persistent_ordering_policy_v1(
      p_organization_id,
  ...
  loop
    v_ambiguous := false;
    v_assignment_count := jsonb_array_length(v_application.assignments);
    ...
    if not v_ambiguous and exists (
      ...
      from (
        select
          a->>'selection_fingerprint' as selection_fingerprint,
          count(*) as occurrence_count
        from jsonb_array_elements(v_application.assignments) a
        group by a->>'selection_fingerprint'
      ) d
    ...
    if not v_ambiguous and exists (
      select 1
      from jsonb_array_elements(v_application.assignments) a
      where coalesce(a->>'experiment_fingerprint', '') !~ '^[a-f0-9]{32,128}$'
         or coalesce(a->>'baseline_rank', '') !~ '^[0-9]+$'
         or coalesce(a->>'persistent_rank', '') !~ '^[0-9]+$'
         or (a->>'baseline_rank')::integer <= 0
         or (a->>'persistent_rank')::integer <= 0
         or (a->>'ordering_influence_fraction')::numeric <> v_policy.ordering_influence_fraction
    ) then
      v_ambiguous := true;
    end if;
    ...
    if not v_ambiguous and exists (
      ...
      from (
        select
          ...
          count(o.id) filter (where o.metadata->>'experiment_fingerprint' <> a->>'experiment_fingerprint') as mismatch_count
        from jsonb_array_elements(v_application.assignments) a
        left join public.intelligence_memories o
      ...
      where outcome_count > 1 or mismatch_count > 0
    ) then
      v_ambiguous := true;
    end if;
    ...
    if v_ambiguous then
      ...
      insert into public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations (
        organization_id, policy_id, application_id, policy_fingerprint,
        selection_cycle_fingerprint, status, lineage_ambiguity_detected, evidence
      ) values (
        p_organization_id, v_policy.id, v_application.id, v_policy.policy_fingerprint,
        v_application.selection_cycle_fingerprint, v_status, true,
        jsonb_build_object(
    ...
    with assignments as (
      select
        a->>'selection_fingerprint' as selection_fingerprint,
        a->>'experiment_fingerprint' as experiment_fingerprint,
        (a->>'baseline_rank')::integer as baseline_rank,
        (a->>'persistent_rank')::integer as persistent_rank
      from jsonb_array_elements(v_application.assignments) a
    ), governed as (
      select
        a.*,
        (o.metadata->>'realized_information_gain_per_cost')::numeric as realized
      from assignments a
      ...
      select l.*, r.selection_fingerprint as r_selection, r.baseline_rank as r_baseline_rank,
             r.persistent_rank as r_persistent_rank, r.realized as r_realized
      from governed l
      join governed r on l.selection_fingerprint < r.selection_fingerprint
      where l.realized <> r.realized
        and ((l.baseline_rank < r.baseline_rank) <> (l.persistent_rank < r.persistent_rank))
    ), scored as (
      select
        case when baseline_rank < r_baseline_rank then (realized > r_realized)::integer
             else (r_realized > realized)::integer end as baseline_correct,
        case when persistent_rank < r_persistent_rank then (realized > r_realized)::integer
             else (r_realized > realized)::integer end as persistent_correct,
        case when baseline_rank < r_baseline_rank then greatest(0::numeric, r_realized - realized)
             else greatest(0::numeric, realized - r_realized) end as baseline_regret,
        case when persistent_rank < r_persistent_rank then greatest(0::numeric, r_realized - realized)
             else greatest(0::numeric, realized - r_realized) end as persistent_regret
      from pairs
    )
    select
    ...
    into v_observed_count, v_pair_count, v_baseline_correct, v_persistent_correct,
         v_baseline_regret, v_persistent_regret
    from scored;

    v_regression := v_pair_count > 0 and (
      v_persistent_correct < v_baseline_correct
    ...
    elsif v_observed_count = v_assignment_count then
    ...
    insert into public.avantiqo_intelligence_persistent_ordering_policy_monitor_evaluations (
      organization_id, policy_id, application_id, policy_fingerprint,
      selection_cycle_fingerprint, status, observed_assignment_count,
      informative_pair_count, baseline_pairwise_correct_count,
      persistent_pairwise_correct_count, baseline_observed_rank_regret,
      persistent_observed_rank_regret, regression_detected,
      lineage_ambiguity_detected, evidence
    ) values (
      p_organization_id, v_policy.id, v_application.id, v_policy.policy_fingerprint,
      v_application.selection_cycle_fingerprint, v_status, v_observed_count,
      v_pair_count, v_baseline_correct, v_persistent_correct, v_baseline_regret,
      v_persistent_regret, v_regression, false,
      jsonb_build_object(
        'governed_phase28_realized_outcomes_only', true,
    ...
    if v_regression then
      ...
      return jsonb_build_object(
        ...
        'informative_pair_count', v_pair_count,
        'baseline_pairwise_correct_count', v_baseline_correct,
        'persistent_pairwise_correct_count', v_persistent_correct,
