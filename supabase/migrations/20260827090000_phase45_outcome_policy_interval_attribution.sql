...
create table if not exists public.avantiqo_intelligence_policy_outcome_attributions (
  ...
  constraint avantiqo_phase45_outcome_fp_check check (outcome_fingerprint ~ '^[a-f0-9]{32,128}$'),
  constraint avantiqo_phase45_receipt_fp_check check (execution_receipt_fingerprint ~ '^[a-f0-9]{16,128}$'),
  constraint avantiqo_phase45_request_fp_check check (request_fingerprint ~ '^[a-f0-9]{16,128}$'),
  constraint avantiqo_phase45_selection_fp_check check (selection_fingerprint ~ '^[a-f0-9]{16,128}$'),
  constraint avantiqo_phase45_attribution_fp_check check (attribution_fingerprint ~ '^[a-f0-9]{32,128}$'),
  constraint avantiqo_phase45_kind_check check (attribution_kind in ('PERSISTENT_POLICY_INTERVAL','NO_PERSISTENT_POLICY_INTERVAL')),
  constraint avantiqo_phase45_time_order_check check (execution_completed_at >= execution_started_at),
  constraint avantiqo_phase45_metadata_object_check check (jsonb_typeof(metadata) = 'object'),
  constraint avantiqo_phase45_policy_binding_shape_check check (
...
grant select, insert on table public.avantiqo_intelligence_policy_outcome_attributions to service_role;
...
grant execute on function public.avantiqo_phase45_reject_outcome_attribution_mutation_v1() to service_role;
...
revoke all on function public.resolve_avantiqo_policy_activation_interval_v1(uuid,timestamptz)
  from public, anon, authenticated;
grant execute on function public.resolve_avantiqo_policy_activation_interval_v1(uuid,timestamptz) to service_role;
...
begin
  ...
  where a.organization_id = p_organization_id
    and (
      ...
      or nullif(m.metadata->>'policy_activation_binding_observed_at','')::timestamptz is distinct from a.binding_observed_at
      or nullif(m.metadata->>'execution_started_at','')::timestamptz is distinct from a.execution_started_at
      or nullif(m.metadata->>'execution_completed_at','')::timestamptz is distinct from a.execution_completed_at
      or (
...
revoke all on function public.verify_avantiqo_policy_outcome_attribution_v1(uuid)
  from public, anon, authenticated;
grant execute on function public.verify_avantiqo_policy_outcome_attribution_v1(uuid) to service_role;
