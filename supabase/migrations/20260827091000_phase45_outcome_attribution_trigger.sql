-- AVANTIQO PHASE 45 ATOMIC OUTCOME ATTRIBUTION
...
begin
  ...
  if v_outcome_fp !~ '^[a-f0-9]{32,128}$'
    ...
    or coalesce(v_meta->>'policy_activation_binding_contract', '')
      <> 'AVANTIQO_EXPERIMENT_OUTCOME_POLICY_INTERVAL_ATTRIBUTION_V1'
    or coalesce(v_meta->>'policy_activation_resolution_contract', '')
  ...
  if v_binding_at is null or v_started_at is null or v_completed_at is null
    or v_completed_at < v_started_at
  then
    raise exception 'AVANTIQO_PHASE45_OUTCOME_BINDING_TIMELINE_INVALID_FAIL_CLOSED';
  end if;

  v_resolved := public.resolve_avantiqo_policy_activation_interval_v1(
    new.organization_id,
    v_binding_at
  );
  ...
  if v_kind = 'PERSISTENT_POLICY_INTERVAL' then
    ...
    if v_policy_id is null
      ...
      or nullif(v_resolved->>'activation_closed_at', '')::timestamptz is distinct from v_activation_closed_at
    then
  ...
  -- Execution start must still resolve to the exact binding that was current when the
  -- execution authority was consumed. Completion may legitimately occur after closure.
  v_resolved := public.resolve_avantiqo_policy_activation_interval_v1(
    new.organization_id,
    v_started_at
  );
  if coalesce((v_resolved->>'success')::boolean, false) is not true
    ...
    or (
      ...
      and coalesce(v_resolved->>'activation_generation_fingerprint', '') <> v_generation_fp
    )
  then
  ...
  v_attribution_fp := md5(
    'avantiqo-phase45-outcome-attribution-v1|' || new.organization_id::text || '|' ||
    new.id::text || '|' || v_outcome_fp || '|' || v_receipt_fp || '|' ||
    v_request_fp || '|' || v_selection_fp || '|' || v_kind || '|' ||
    coalesce(v_generation_fp, '') || '|' || v_binding_at::text || '|' ||
    v_started_at::text || '|' || v_completed_at::text
  );

  select * into v_existing
  from public.avantiqo_intelligence_policy_outcome_attributions
  where organization_id = new.organization_id
    and outcome_memory_id = new.id
  limit 1;
  ...
  insert into public.avantiqo_intelligence_policy_outcome_attributions (
    ...
    v_generation_index,
    v_generation_fp,
    v_activation_started_at,
    v_activation_closed_at,
    v_binding_at,
    v_started_at,
    v_completed_at,
    v_attribution_fp,
...
grant execute on function public.avantiqo_phase45_enforce_outcome_attribution_v1()
  to service_role;

drop trigger if exists avantiqo_phase45_enforce_outcome_attribution_v1
  on public.intelligence_memories;
create trigger avantiqo_phase45_enforce_outcome_attribution_v1
after insert or update of metadata on public.intelligence_memories
for each row
execute function public.avantiqo_phase45_enforce_outcome_attribution_v1();
