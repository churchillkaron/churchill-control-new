...
begin
  ...
  if not exists (select 1 from public.legal_entities where id = p_entity_id and organization_id = v_engagement.organization_id) then raise exception 'ENTITY_SCOPE_MISMATCH'; end if;
  ...
  if not exists (select 1 from public.accounting_work_program_template_steps where template_id = p_template_id and active = true) then raise exception 'TEMPLATE_HAS_NO_ACTIVE_STEPS'; end if;

  if p_period_id is null then raise exception 'PERIOD_REQUIRED'; end if;
  select * into v_period from public.financial_periods where id = p_period_id and organization_id = v_engagement.organization_id and entity_id = p_entity_id;
  if not found then raise exception 'PERIOD_SCOPE_MISMATCH'; end if;

  select * into v_profile
  from public.accounting_client_profiles
  where accounting_firm_id = p_accounting_firm_id and organization_id = v_engagement.organization_id;
  ...
  insert into public.accounting_engagement_runs (
    ...
    jsonb_build_object('template_version', v_template.version, 'entity_id', p_entity_id, 'source', 'recurring_materializer')
  )
  on conflict (accounting_firm_id, engagement_id, run_key) do nothing
  returning * into v_run;

  if v_run.id is null then
    select * into v_existing from public.accounting_engagement_runs
    where accounting_firm_id = p_accounting_firm_id and engagement_id = p_engagement_id and run_key = p_run_key;
    return jsonb_build_object('created', false, 'run_id', v_existing.id, 'status', 'ALREADY_EXISTS');
  end if;
  ...
  select
    p_accounting_firm_id, v_engagement.organization_id, p_entity_id, v_run.id, s.id, s.step_key, s.sequence_no,
    s.title, s.description, s.work_type, s.required_role,
    case s.required_role when 'PREPARER' then v_profile.assigned_accountant_id when 'REVIEWER' then v_profile.assigned_reviewer_id when 'PARTNER' then v_profile.assigned_partner_id else null end,
    ...
    (case when s.due_anchor = 'RUN_START' then p_start_at else p_due_at end) + make_interval(days => coalesce(s.relative_due_days, 0)),
    null,
    coalesce(s.dependency_step_keys, array[]::text[]), s.capability_id, coalesce(s.budget_minutes, 0),
    jsonb_build_object('template_version', v_template.version, 'evidence_required', coalesce(s.evidence_required, false), 'source', 'recurring_materializer')
  ...
  select
    p_accounting_firm_id, v_engagement.organization_id, p_entity_id, v_run.id, i.id, i.title, i.description,
    'DRAFT', i.due_at, jsonb_build_object('mode', 'manual_until_sent'), p_created_by,
    jsonb_build_object('source', 'recurring_materializer')
  from public.accounting_engagement_work_items i
  where i.run_id = v_run.id and i.work_type = 'CLIENT_REQUEST';
  get diagnostics v_requests = row_count;

  return jsonb_build_object('created', true, 'run_id', v_run.id, 'status', 'CREATED', 'work_items', v_items, 'client_requests', v_requests);
end;
$$;

revoke all on function public.materialize_accounting_engagement_run(uuid,uuid,uuid,uuid,uuid,text,timestamptz,timestamptz,uuid) from public, anon, authenticated;
