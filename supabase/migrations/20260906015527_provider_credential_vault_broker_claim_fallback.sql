...
begin
  ...
  select pc.secret_reference, pc.provider_id, pc.status, pc.metadata
  into v_reference, v_provider_id, v_status, v_metadata
  from public.provider_credentials pc
  where pc.id = p_credential_id;

  if not found then raise exception 'PROVIDER_CREDENTIAL_NOT_FOUND'; end if;
  if upper(coalesce(v_status, '')) <> 'ACTIVE' then raise exception 'PROVIDER_CREDENTIAL_NOT_ACTIVE'; end if;
  ...
  if v_scoped_organization_id is not null then
    if p_organization_id is null or v_scoped_organization_id <> p_organization_id::text then
      raise exception 'PROVIDER_CREDENTIAL_ORGANIZATION_MISMATCH' using errcode = '42501';
    end if;
  end if;
  ...
  if v_vault_id is null then raise exception 'PROVIDER_CREDENTIAL_VAULT_REFERENCE_INVALID'; end if;
