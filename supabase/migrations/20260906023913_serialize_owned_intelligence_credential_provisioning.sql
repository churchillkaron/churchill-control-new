...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('avantiqo-intelligence:credential-provision:' || p_organization_id::text, 0)
  );
  ...
  order by
    case
      when coalesce(pc.metadata ->> 'priority', '') ~ '^[0-9]+$'
        then (pc.metadata ->> 'priority')::integer
      else 0
    end desc,
    pc.created_at desc
  limit 1
    ...
    if found then
      ...
      set secret_reference = v_reference,
          ...
          metadata = coalesce(v_existing.metadata, '{}'::jsonb) || jsonb_build_object(
            ...
            'updated_by_runtime', 'OWNED_INTELLIGENCE_CREDENTIAL_PROVISIONER_V2'
          ),
          updated_at = v_now
      ...
      insert into public.provider_credentials (
        ...
        jsonb_build_object(
          ...
          'secret_transport', 'SUPABASE_VAULT',
          'priority', 100,
          'created_by_runtime', 'OWNED_INTELLIGENCE_CREDENTIAL_PROVISIONER_V2'
        ),
        v_now,
        v_now
      )
