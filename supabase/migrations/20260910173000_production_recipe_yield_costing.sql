-- Canonical usable-yield costing for recipe components.
-- A recipe quantity is the usable amount required by the dish; purchase cost is
-- grossed up by yield_percent (for example 150g usable at 82% costs 182.9268g raw).
alter table public.recipe_items
  add column if not exists yield_percent numeric not null default 100;

alter table public.recipe_items
  drop constraint if exists recipe_items_yield_percent_check;
alter table public.recipe_items
  add constraint recipe_items_yield_percent_check
  check (yield_percent > 0 and yield_percent <= 100);

comment on column public.recipe_items.yield_percent is
  'Usable yield percentage for this ingredient line. 100 means no trim/preparation loss.';
...
declare
  ...
  if jsonb_typeof(p_items) <> 'array' or jsonb_array_length(p_items)=0 then
    raise exception 'recipe items required';
  end if;
  if not exists (
  ...
  loop
    ...
    insert into pg_temp.recipe_stage(item_id,quantity,uom_id,factor,yield_percent)
      values(v_item.id,v_quantity,v_uom_id,v_factor,v_yield_percent);
    v_total := v_total + ((v_quantity*v_factor)/(v_yield_percent/100))*v_item.cost;
    v_count := v_count+1;
  end loop;
  ...
  insert into public.recipe_items(
    organization_id,entity_id,dish_id,item_id,quantity,unit,uom_id,yield_percent
  )
  select p_organization_id,p_entity_id,p_dish_id,s.item_id,s.quantity,u.abbreviation,s.uom_id,s.yield_percent
  from pg_temp.recipe_stage s
  left join public.inventory_uoms u on u.id=s.uom_id;
  ...
  return jsonb_build_object(
    ...
    'cost_basis','CURRENT_OPERATIONAL_ITEM_COST_WITH_YIELD_V2'
  );
end;
$$;

revoke all on function public.production_upsert_recipe_atomic(uuid,uuid,uuid,jsonb,uuid)
  from public,anon,authenticated;
grant execute on function public.production_upsert_recipe_atomic(uuid,uuid,uuid,jsonb,uuid)
  to service_role;