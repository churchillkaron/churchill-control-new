...
alter table public.market_feed_status
  add column if not exists last_stock_message_at timestamptz,
  add column if not exists last_news_message_at timestamptz,
  add column if not exists last_stock_flush_at timestamptz;

create index if not exists market_feed_status_execution_health_idx
  on public.market_feed_status (
    organization_id, portfolio_id, provider, feed, last_stock_message_at desc
  );
