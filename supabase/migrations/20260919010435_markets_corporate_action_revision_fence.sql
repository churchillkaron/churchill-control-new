...
create table if not exists public.market_corporate_action_revisions (
  ...
  constraint market_corporate_action_revision_nonnegative_check check (revision >= 0)
);

alter table public.market_corporate_action_revisions enable row level security;

insert into public.market_corporate_action_revisions (
  organization_id,
  portfolio_id,
  symbol,
  revision,
...
on conflict (organization_id, portfolio_id, symbol) do nothing;

create or replace function public.market_bump_corporate_action_revision(
...
begin
  if p_organization_id is null
    ...
    raise exception 'MARKET_CORPORATE_ACTION_REVISION_SCOPE_REQUIRED';
  end if;
  ...
  do update set
    revision = public.market_corporate_action_revisions.revision + 1,
    updated_at = greatest(
      clock_timestamp(),
      public.market_corporate_action_revisions.updated_at + interval '1 microsecond'
    );
end;
$$;

create or replace function public.market_touch_corporate_action_revision()
returns trigger
language plpgsql
security invoker
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    perform public.market_bump_corporate_action_revision(
      new.organization_id,
      new.portfolio_id,
      new.symbol
    );
...
drop trigger if exists market_corporate_action_revision_touch
  on public.market_corporate_actions;

create trigger market_corporate_action_revision_touch
after insert or update or delete on public.market_corporate_actions
for each row
execute function public.market_touch_corporate_action_revision();

revoke all on table public.market_corporate_action_revisions
from public, anon, authenticated;

revoke all on function public.market_bump_corporate_action_revision(uuid, uuid, text)
from public, anon, authenticated;
grant execute on function public.market_bump_corporate_action_revision(uuid, uuid, text)
to service_role;

revoke all on function public.market_touch_corporate_action_revision()
from public, anon, authenticated;

comment on table public.market_corporate_action_revisions is
  'Monotonic per-symbol corporate-action dataset revision used to fence PAPER BUY execution against concurrent corporate-action ingestion.';
