...
create table if not exists public.market_daily_bar_revisions (
  ...
  constraint market_daily_bar_revision_nonnegative_check check (revision >= 0)
);

alter table public.market_daily_bar_revisions enable row level security;

insert into public.market_daily_bar_revisions (
  organization_id,
  portfolio_id,
  revision,
  updated_at
...
create or replace function public.market_bump_daily_bar_revision(
...
begin
  if p_organization_id is null or p_portfolio_id is null then
    raise exception 'MARKET_DAILY_BAR_REVISION_SCOPE_REQUIRED';
  end if;
  ...
  do update set
    revision = public.market_daily_bar_revisions.revision + 1,
    updated_at = greatest(
      clock_timestamp(),
      public.market_daily_bar_revisions.updated_at + interval '1 microsecond'
    );
end;
$$;

create or replace function public.market_touch_daily_bar_revision()
returns trigger
language plpgsql
security invoker
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    if new.portfolio_id is not null and new.timeframe = '1Day' then
      perform public.market_bump_daily_bar_revision(
        new.organization_id,
        new.portfolio_id
      );
    end if;
    ...
    if old.portfolio_id is not null and old.timeframe = '1Day' then
      perform public.market_bump_daily_bar_revision(
        old.organization_id,
        old.portfolio_id
      );
...
drop trigger if exists market_daily_bar_revision_touch on public.market_bars;

create trigger market_daily_bar_revision_touch
after insert or update or delete on public.market_bars
for each row
execute function public.market_touch_daily_bar_revision();

revoke all on table public.market_daily_bar_revisions
from public, anon, authenticated;

revoke all on function public.market_bump_daily_bar_revision(uuid, uuid)
from public, anon, authenticated;
grant execute on function public.market_bump_daily_bar_revision(uuid, uuid)
to service_role;

revoke all on function public.market_touch_daily_bar_revision()
from public, anon, authenticated;

comment on table public.market_daily_bar_revisions is
  'Monotonic portfolio-level revision for 1Day market bars used to fence PAPER BUY historical-risk evaluation against concurrent daily-bar refreshes.';
