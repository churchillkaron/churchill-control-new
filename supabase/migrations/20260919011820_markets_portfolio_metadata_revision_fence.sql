...
create table if not exists public.market_portfolio_benchmark_revisions (
  organization_id uuid not null,
  portfolio_id uuid not null references public.market_portfolios(id) on delete cascade,
  revision bigint not null default 0,
  updated_at timestamptz not null default now(),
  primary key (organization_id, portfolio_id),
  constraint market_portfolio_benchmark_revision_nonnegative check (revision >= 0)
);

alter table public.market_portfolio_benchmark_revisions enable row level security;

insert into public.market_portfolio_benchmark_revisions (
  organization_id,
  portfolio_id,
  revision,
  updated_at
)
select organization_id, id, 1, updated_at
from public.market_portfolios
on conflict (organization_id, portfolio_id) do nothing;

create or replace function public.market_bump_portfolio_benchmark_revision(
...
as $$
begin
  if p_organization_id is null or p_portfolio_id is null then
    raise exception 'MARKET_BENCHMARK_REVISION_SCOPE_REQUIRED';
  end if;

  insert into public.market_portfolio_benchmark_revisions (
    organization_id, portfolio_id, revision, updated_at
  )
  values (p_organization_id, p_portfolio_id, 1, clock_timestamp())
  on conflict (organization_id, portfolio_id)
  do update set
    revision = public.market_portfolio_benchmark_revisions.revision + 1,
    updated_at = greatest(
      clock_timestamp(),
      public.market_portfolio_benchmark_revisions.updated_at + interval '1 microsecond'
    );
end;
$$;

create or replace function public.market_touch_portfolio_benchmark_revision()
returns trigger
language plpgsql
security invoker
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    perform public.market_bump_portfolio_benchmark_revision(
      new.organization_id,
      new.id
    );
    return new;
  ...
  if old.benchmark_symbol is distinct from new.benchmark_symbol then
    perform public.market_bump_portfolio_benchmark_revision(
      new.organization_id,
      new.id
    );
...
drop trigger if exists market_portfolio_benchmark_revision_touch
  on public.market_portfolios;

create trigger market_portfolio_benchmark_revision_touch
after insert or update on public.market_portfolios
for each row
execute function public.market_touch_portfolio_benchmark_revision();

revoke all on table public.market_portfolio_benchmark_revisions
from public, anon, authenticated;

revoke all on function public.market_bump_portfolio_benchmark_revision(uuid, uuid)
from public, anon, authenticated;
grant execute on function public.market_bump_portfolio_benchmark_revision(uuid, uuid)
to service_role;

revoke all on function public.market_touch_portfolio_benchmark_revision()
from public, anon, authenticated;

comment on table public.market_portfolio_benchmark_revisions is
  'Monotonic per-portfolio benchmark revision used to fence PAPER BUY beta risk against concurrent benchmark changes.';

create table if not exists public.market_instrument_classification_revisions (
  organization_id uuid primary key,
  revision bigint not null default 0,
  updated_at timestamptz not null default now(),
  constraint market_instrument_classification_revision_nonnegative check (revision >= 0)
);

alter table public.market_instrument_classification_revisions enable row level security;

insert into public.market_instrument_classification_revisions (
  organization_id,
  revision,
  updated_at
)
...
create or replace function public.market_bump_instrument_classification_revision(
  p_organization_id uuid
)
returns void
language plpgsql
...
begin
  if p_organization_id is null then
    raise exception 'MARKET_CLASSIFICATION_REVISION_SCOPE_REQUIRED';
  end if;
  ...
  do update set
    revision = public.market_instrument_classification_revisions.revision + 1,
    updated_at = greatest(
      clock_timestamp(),
      public.market_instrument_classification_revisions.updated_at + interval '1 microsecond'
    );
end;
$$;

create or replace function public.market_touch_instrument_classification_revision()
returns trigger
language plpgsql
security invoker
set search_path = public
...
begin
  if tg_op = 'INSERT' then
    perform public.market_bump_instrument_classification_revision(new.organization_id);
    return new;
  elsif tg_op = 'DELETE' then
    perform public.market_bump_instrument_classification_revision(old.organization_id);
    return old;
  end if;

  if old.organization_id is distinct from new.organization_id
     or old.sector is distinct from new.sector
     or old.industry is distinct from new.industry then
    perform public.market_bump_instrument_classification_revision(old.organization_id);
...
drop trigger if exists market_instrument_classification_revision_touch
  on public.market_instruments;

create trigger market_instrument_classification_revision_touch
after insert or update or delete on public.market_instruments
for each row
execute function public.market_touch_instrument_classification_revision();

revoke all on table public.market_instrument_classification_revisions
from public, anon, authenticated;

revoke all on function public.market_bump_instrument_classification_revision(uuid)
from public, anon, authenticated;
grant execute on function public.market_bump_instrument_classification_revision(uuid)
to service_role;

revoke all on function public.market_touch_instrument_classification_revision()
from public, anon, authenticated;

comment on table public.market_instrument_classification_revisions is
  'Monotonic organization-level revision for sector/industry classifications used by PAPER BUY concentration risk.';
