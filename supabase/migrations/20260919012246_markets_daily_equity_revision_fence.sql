...
create or replace function public.market_roll_paper_daily_equity_if_needed(
...
revoke all on function public.market_roll_paper_daily_equity_if_needed(uuid, uuid)
from public, anon, authenticated;
grant execute on function public.market_roll_paper_daily_equity_if_needed(uuid, uuid)
to service_role;

comment on function public.market_roll_paper_daily_equity_if_needed(uuid, uuid) is
  'Atomically rolls the PAPER daily-equity baseline and advances execution_revision so in-flight fills evaluated under the prior day fail closed.';
