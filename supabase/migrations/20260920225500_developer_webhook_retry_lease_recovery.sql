  ...
  with due as (
    ...
    where d.attempt < 20
      ...
      and (
        ...
        (
          d.status = 'RETRYING'
          and d.retry_lease_expires_at is not null
          and d.retry_lease_expires_at <= now()
        )
      )
    order by coalesce(d.next_attempt_at, d.retry_lease_expires_at) asc, d.created_at asc
