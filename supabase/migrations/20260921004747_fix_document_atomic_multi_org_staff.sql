...
begin
  ...
  if p_owner_staff_id is not null and not exists (
    select 1
    from public.staff_accounts sa
    left join public.organization_users ou
      on ou.staff_account_id = sa.id
...
comment on function public.create_enterprise_document_atomic is
  'Creates the canonical controlled-document row, immutable version 1 and initial business-object link in one transaction. Multi-organization staff authorization accepts active direct organization ownership or active organization membership.';
comment on function public.append_enterprise_document_version_atomic is
  'Serializes controlled document revision numbers and invalidates prior approval when a new version is appended. Multi-organization staff authorization accepts active direct organization ownership or active organization membership.';
