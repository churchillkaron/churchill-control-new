...
test("single explicit measure becomes contribution ready",()=>{const b=build({comparison_results:[result([{driver_id:"staff_coverage",measure_id:"scheduled_shifts",baseline_value:10,actual_value:12,source_capability_key:"people.attendance.read"}])],required_driver_ids:["staff_coverage"]});assert.equal(b.contribution_rows.length,1);assert.equal(b.contribution_rows[0].delta,2);assert.equal(b.coverage_complete,true);});
