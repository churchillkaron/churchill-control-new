...
function evidence(id = "ev-main") {
  return { id, trusted: true, current: true };
}

function recommendationA(overrides = {}) {
  return {
    id: "option-a",
    title: "Option A",
    kind: "recommendation",
...
function recommendationB(overrides = {}) {
  return {
    id: "option-b",
    title: "Option B",
    kind: "recommendation",
...
test("robustness runtime exposes canonical contract and never gains execution authority", () => {
  const result = stressTestOperatorIntelligenceDecision({
    goal: "Choose a robust path",
    candidates: [recommendationA(), recommendationB()],
    evidence: [evidence()],
    scenarios: [
    ...
    scenarios: [
      {
        ...
        kind: "verified",
        candidate_overrides: [
          { candidate_id: "option-a", overrides: { risk: "critical", reversible: false, irreversible: true } },
        ],
      },
...
test("plausible assumption sensitivity requests more evidence instead of pretending robustness", () => {
  const result = stressTestOperatorIntelligenceDecision({
    goal: "Choose under uncertain economics",
    candidates: [recommendationA(), recommendationB()],
    evidence: [evidence()],
    scenarios: [
      {
...
test("one material scenario is insufficient for a robustness claim", () => {
  const result = stressTestOperatorIntelligenceDecision({
    goal: "Require more than one stress probe",
    candidates: [recommendationA(), recommendationB()],
    evidence: [evidence()],
    scenarios: [
