...
function decision() {
  return { candidate_id: "option-a", status: "SELECTED_FOR_PLANNING" };
}

function freshDependency(overrides = {}) {
  return {
    id: "ev-current",
    required: true,
    verified: true,
    current: true,
    volatility: "dynamic",
    observed_at: "2026-08-27T06:45:00.000Z",
    ...overrides,
  };
}

function stableCondition(overrides = {}) {
  return {
    id: "condition-a",
    required: true,
    verified: true,
...
test("decision validity exposes canonical contract and never authorizes execution", () => {
  const result = assessOperatorIntelligenceDecisionValidity({
    decision: decision(),
    evidence_dependencies: [freshDependency()],
    validity_conditions: [stableCondition()],
    now: NOW,
  });
...
test("stale evidence forces revalidation", () => {
  const result = assessOperatorIntelligenceDecisionValidity({
    decision: decision(),
    evidence_dependencies: [freshDependency({ observed_at: "2026-08-27T06:00:00.000Z" })],
    validity_conditions: [stableCondition()],
    now: NOW,
  });
...
test("unverified evidence and unknown volatility fail closed", () => {
  const result = assessOperatorIntelligenceDecisionValidity({
    decision: decision(),
    evidence_dependencies: [freshDependency({ verified: false, volatility: "unknown" })],
    validity_conditions: [stableCondition()],
    now: NOW,
  });
...
test("verified changed condition invalidates prior decision and requires replan", () => {
  const result = assessOperatorIntelligenceDecisionValidity({
    decision: decision(),
    evidence_dependencies: [freshDependency()],
    validity_conditions: [stableCondition({ status: "changed" })],
    now: NOW,
  });
...
test("future timestamps never make evidence look current", () => {
  const result = assessOperatorIntelligenceDecisionValidity({
    decision: decision(),
    evidence_dependencies: [freshDependency({ observed_at: "2026-08-27T07:50:00.000Z" })],
    validity_conditions: [stableCondition()],
    now: NOW,
  });
...
test("caller freshness setting can tighten but never loosen policy", () => {
  const result = assessOperatorIntelligenceDecisionValidity({
    decision: decision(),
    evidence_dependencies: [freshDependency({ max_age_ms: 999999999 })],
    validity_conditions: [stableCondition()],
    now: NOW,
  });
