...
test("canonical Fast Intelligence scales to zero five seconds after idle", () => {
  ...
  assert.match(worker, /FAST_SCALEDOWN_WINDOW_SECONDS = 5/);
  ...
  assert.match(provider, /fast_bounded_warm_idle_seconds: 5/);
  assert.match(provider, /deep_scale_to_zero: true/);
  assert.match(provider, /min_containers: 0/);
  assert.match(provider, /scaledown_window_seconds: 5/);
});
