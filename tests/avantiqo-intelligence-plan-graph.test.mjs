...
function validatedMutationStep(overrides = {}) {
  return {
    ...
    rollback: {
      available: true,
      strategy: "Restore the previous verified value through the registered rollback path.",
    },
    ...overrides,
  };
}
...
test("plan graph orders dependencies and never auto-executes mutations", () => {
  const plan = buildOperatorIntelligencePlan({
    goal: "Change a governed setting safely",
    brief: { completion_test: "Live evidence proves the final state." },
    ...
    plan_steps: [
      {
        ...
        candidate_validation: {
          ...
          missing_required_fields: ["organization_id"],
        },
...
test("planning runtime exposes plan graph without gaining business execution authority", () => {
  const source = fs.readFileSync(
    new URL("../lib/operator/runtime/OperatorIntelligencePlanningToolRuntime.js", import.meta.url),
    "utf8",
  );
  assert.match(source, /AVANTIQO_OPERATOR_INTELLIGENCE_PLANNING_TOOLS_V2/);
