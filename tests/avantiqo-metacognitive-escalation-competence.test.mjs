...
test("metacognition requires a passed adversarial result",()=>{assert.match(runtime,/UPSTREAM_ADVERSARIAL_REQUIRED/);assert.match(runtime,/UPSTREAM_ADVERSARIAL_NOT_PASSED/);assert.match(runtime,/upstreamMetadata\.passed!==true/);assert.match(runtime,/upstream_adversarial_passed:true/);assert.match(runtime,/import \{ createHash \} from "node:crypto"/);});
