...
const policySource = fs.readFileSync(
  new URL("../lib/operator/runtime/OperatorOwnedCognitiveBriefPolicy.js", import.meta.url),
  "utf8",
);
...
test("owned cognitive brief cannot bypass execution governance", () => {
  assert.match(source, /not authorization/i);
  assert.match(source, /zero execution authority/i);
  assert.match(source, /permissions, confirmation, approval, wallet and verification rules/i);
  assert.match(source, /execution_guidance_allowed=false/i);
...
test("voice path avoids a serial deep preflight to preserve latency", () => {
  assert.match(source, /needsOwnedCognitiveBrief/);
  assert.match(policySource, /toLowerCase\(\) === "voice"\) return false/);
});
