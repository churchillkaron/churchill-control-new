...
assert.match(selector, /score\.cinematic >= WORLD_CLASS_FLOOR/);
...
assert.match(bridge, /CreativeDailiesRoomRuntime/);
assert.match(bridge, /recordAssetDecision/);
assert.match(bridge, /DIRECTING.*CINEMATOGRAPHY.*CONTINUITY.*TECHNICAL_TRUTH.*PERCEPTUAL_QUALITY/s);
assert.match(bridge, /existing-review-evidence/);
