...
test("Studio Cinema is Avantiqo-owned only and external video fallback is impossible", () => {
  ...
  assert.match(router, /execution_scope:\s*certification\.execution_scope/);
  assert.match(router, /benchmark_only:\s*certification\.benchmark_only/);
  assert.match(router, /production_certified:\s*certification\.production_certified/);
  assert.match(router, /studio_preproduction_review:\s*!certification\.production_certified/);
...
test("Avantiqo Cinema review generation uses the owned Node01 LTX 2.5 lane with no external fallback", () => {
  ...
  assert.doesNotMatch(provider, /createAvantiqoOwnedModalWorker|FAST_PREVIEW_FUNCTION_NAME|generate_investor_t2v_job/);
});
