...
test("Voice readiness models true 0\/1 serverless cold start", async () => {
  ...
  assert.match(readiness, /managementWorkerRecords\.every\(\(worker\) => terminalManagementRecord\(worker\)\)/);
  assert.match(readiness, /controlHasOnlyIdleWorkers/);
  assert.match(readiness, /explicitlyRetiredIdlePlane/);
  ...
  assert.match(readiness, /retired_idle_plane_history_requires_all_management_records_terminal: true/);
  assert.match(readiness, /retired_health_idle_history_ignored_only_with_explicit_retirement_evidence: true/);
