...
test("Voice readiness models true 0\/1 serverless cold start", async () => {
  ...
  assert.match(readiness, /terminalManagementRecord\(managementWorker\)/);
  assert.match(readiness, /retired_control_ghost_records_ignored/);
  ...
  assert.match(readiness, /retired_control_ghost_history_requires_matching_terminal_management_record: true/);
