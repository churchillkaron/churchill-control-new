...
test("Business Partner sends turns through the live execution wrapper and renders runtime wording verbatim", () => {
  ...
  assert.match(home, /return text\(latest\?\.description\) \|\| text\(latest\?\.phase\)/);
  assert.doesNotMatch(home, /I’m researching the relevant evidence/);
  assert.doesNotMatch(home, /I’m working through your request now/);
});
