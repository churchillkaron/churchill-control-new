...
test("shared Code history API and UI expose competitive evidence without making it authoritative", async () => {
  ...
  assert.match(route, /history_head_valid/);
  assert.match(route, /publicHistoryIntegrityReason/);
  assert.match(route, /ATTESTATION_UNAVAILABLE/);
  assert.match(route, /HISTORY_RECORD_INVALID/);
  ...
  assert.match(panel, /History integrity failed — quarantined and advisory only/);
