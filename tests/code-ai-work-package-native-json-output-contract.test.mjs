...
test("work-package planner requests native JSON and compact repair output cap", () => {
  ...
  assert.match(workPackage, /max_output_tokens: plannerOutputRepair\.compact_json_only === true \? 1400 : 4096/);
  assert.match(workPackage, /temperature: plannerOutputRepair\.compact_json_only === true \|\| plannerOutputRepair\.operation_limit_exceeded === true \? 0 : 0\.1/);
});
