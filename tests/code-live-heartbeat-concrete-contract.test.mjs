...
test("background heartbeat does not overwrite fresh concrete progress", () => {
  ...
  assert.match(route, /const freshRealProgress = Number\.isFinite\(latestAt\) && \(Date\.now\(\) - latestAt\) < 4000/);
  assert.match(route, /if \(freshRealProgress\) return/);
  assert.match(route, /latestDescription/);
  assert.match(route, /const concreteRepositoryLatest =/);
  assert.match(route, /REPOSITORY_OPERATION_\|VERIFICATION_\|DETERMINISTIC_\|COMMIT_/);
  assert.match(route, /if \(concreteRepositoryLatest\) return/);
