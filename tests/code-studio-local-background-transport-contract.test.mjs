...
test("background local worker owns bounded capability slices and continuation tranches", () => {
  ...
  assert.match(route, /has not produced its first repository operation yet/);
  assert.match(route, /bounded recovery from the preserved mission state/);
  assert.match(route, /let heartbeatCount = 0/);
  assert.match(route, /heartbeatCount \+= 1/);
  assert.match(route, /CODE_STUDIO_BACKGROUND_HEARTBEAT_FAILED/);
});
