...
const conceptHash = (concept) => crypto.createHash("sha256").update(JSON.stringify(concept)).digest("hex");
const weightedCritics = [
  ["originality", 0.20, 96],
  ["music_energy", 0.25, 96],
  ["brand_commercial", 0.20, 96],
  ["production", 0.15, 96],
  ["mission_fidelity", 0.20, 96],
  ["story_creativity", 0, 96],
].map(([critic_id, weight, score]) => ({
  critic_id,
  weight,
  evaluations: concepts.map((concept) => ({ concept_id: concept.id, concept_hash: conceptHash(concept), score, passed: true, mandatory_repairs: [], failures: [] })),
}));
function council() {
  return {
...
test("human/place/patience critic preserves weighted score while adding veto coverage", () => {
  ...
  assert.equal(baseline.council.critic_reports.length, 7);
  assert.equal(baseline.selected_scorecard.weighted_score, 96);
...
test("zero-weight human/place/patience critic still vetoes selected concept", () => {
  ...
  assert.equal(result.selected_scorecard.weighted_score, 96);
