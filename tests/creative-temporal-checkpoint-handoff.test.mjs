...
const stateRepository = await readFile(
  "lib/creative/state/CreativeStateRepository.js",
  "utf8",
);

test("approved temporal council resume persists canonical master before clearing temporary checkpoints", () => {
  ...
  const clearIndex = resolution.indexOf("await clearResolvedDirectionCheckpoints(context)", persistIndex);
  assert.ok(persistIndex >= 0);
  assert.ok(clearIndex > persistIndex);
});

test("autonomous scheduler can recover planning and production lifecycle stages", () => {
  for (const stage of [
    "UNDERSTANDING", "RESEARCHING", "BUILDING_STRATEGY", "BUILDING_CONCEPT",
    "BUILDING_STORYBOARD", "PLANNING_PRODUCTION", "READY_FOR_EXECUTION",
    "EXECUTING", "PRODUCING", "RENDERING", "REVIEWING", "MONITORING",
