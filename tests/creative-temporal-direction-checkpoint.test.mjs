...
test('validated temporal direction is durable and tied to tribunal identity', () => {
  ...
  assert.match(source, /durableTemporalMaster \|\| tribunalApprovedMaster \|\| await CreativeUniversalTemporalDirectionRuntime\.create/);
...
test('workflow resolution resumes validated temporal direction before council or tribunal work', async () => {
  ...
  assert.match(workflowSource, /let temporalCheckpoint = forceDirectionRestart[\s\S]*storedTemporalDirectionCheckpoint\(context\.project, context\)/);
  assert.match(workflowSource, /masterStoryRequired\(context\)[\s\S]*!completeMasterStory\(temporalCheckpoint\.plan\)/);
