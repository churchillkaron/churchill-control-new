...
test("Meta review and readiness are organization-specific for multi-organization campaigns", () => {
  assert.match(component, /Meta \/ Facebook & Instagram/);
  assert.match(component, /\{organizationName\}/);
...
test("organic social review is organization-specific before owner approval", () => {
  ...
  assert.match(component, /\["Execution state", state\?\.label \|\| "Setup"\]/);
...
test("paid-channel editors keep provider depth behind clear advanced controls", () => {
  assert.match(component, /function AdvancedSettingsSection/);
  assert.match(component, /title="Advanced Google controls"/);
  assert.match(component, /title="Advanced delivery controls"/);
  assert.match(component, /title="Advanced audience"/);
  assert.match(component, /title="Advanced bidding"/);
  assert.match(component, /title="Tracking"/);
  assert.match(component, /Include Google Search Partners/);
  assert.match(component, /Facebook placements/);
  assert.match(component, /Lookalike audiences/);
  assert.match(component, /Bid cap/);
  assert.match(component, /UTM campaign/);
});

test("paid-media review keeps essentials visible and provider detail collapsible", () => {
  assert.match(component, /function ReviewDetails/);
  assert.match(component, /Advanced review details/);
  assert.match(component, /Placements, optimization, audience and tracking details/);
  assert.match(component, /Account, exclusions, network and tracking details/);
  assert.match(component, /\["Facebook Page", page\?\.name/);
  assert.match(component, /\["Spending account", account\?\.name/);
  assert.match(component, /\["Authorized Meta budget"/);
  assert.match(component, /\["Authorized Google budget"/);
  assert.match(component, /\["Tracking",/);
});

test("active channel readiness is actionable in the Campaign builder", () => {
  ...
  assert.match(component, /blockers\.slice\(0, 4\)/);
});
