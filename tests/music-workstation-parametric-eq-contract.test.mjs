...
const rack = fs.readFileSync(new URL("../components/creative/ProductionStudio/workspaces/MusicMixerSendsPanel.jsx", import.meta.url), "utf8");
...
test("audible preview maps precision bands to real Biquad filters", () => {
  ...
  const broadTone = strip.indexOf("highShelf.gain.value");
  const precisionEq = strip.indexOf("chain = connectParametricEq(context, highShelf, track)");
...
test("engineer UI exposes frequency gain Q and filter type per band", () => {
  ...
  assert.match(panel, /Field label="Q"/);
